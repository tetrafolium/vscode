...
async function runTestsInBrowser(
		...
		async (
			...
			try {
				for (const log of logs) {
					...
					await promises.mkdir(path.dirname(absoluteLogsPath), {
						recursive: true,
					});
			...
			try {
				...
				console.error(
					`Error when killing server process tree (pid: ${server.pid}): ${error}`,
				);
			}
	...
	const targetUrl =
		path.extname(testWorkspacePath) === ".code-workspace"
	...
	for (let attempt = 1; ; attempt++) {
		try {
			...
			const isConnectionRefused =
				/NS_ERROR_CONNECTION_REFUSED|net::ERR_CONNECTION_REFUSED|ECONNREFUSED/i.test(
					message,
				);
			...
			console.log(
				`page.goto failed with connection refused (attempt ${attempt}/${maxAttempts}), retrying in ${delayMs}ms...`,
			);
...
async function launchServer(
	...
	const stdio: cp.StdioOptions = args.debug
		? "pipe"
		: ["ignore", "pipe", "ignore"];
	const shell: boolean = process.platform === "win32";
	const serverProcess = cp.spawn(serverLocation, serverArgs, {
		env,
		stdio,
		shell,
	});
