...
export async function launch(
	...
	// Launch browser
	const { browser, context, page, pageLoadedPromise } = await launchBrowser(
		options,
		endpoint,
	);
...
async function launchServer(options: LaunchOptions) {
	...
	if (codeServerPath) {
		const { serverApplicationName } = require(
			join(codeServerPath, "product.json"),
		);
		serverLocation = join(
			codeServerPath,
			"bin",
	...
	logger.log(
		`Started server for browser smoke tests (pid: ${serverProcess.pid})`,
	);
...
async function launchBrowser(options: LaunchOptions, endpoint: string) {
	...
	const browser = await measureAndLog(
		() =>
			playwrightImpl[
				browserType as unknown as "chromium" | "webkit" | "firefox"
			].launch({
	...
	browser.on("disconnected", () =>
		logger.log(`Playwright: browser disconnected`),
	);
	...
	if (tracing) {
		try {
			...
			logger.log(
				`Playwright (Browser): Failed to start playwright tracing (${error})`,
			); // do not fail the build when this fails
		}
	}

	const page = await measureAndLog(
		() => context.newPage(),
		"context.newPage()",
		logger,
	);
	...
	if (options.verbose) {
		context.on("page", () =>
			logger.log(`Playwright (Browser): context.on('page')`),
		);
		...
		page.on("dialog", () =>
			logger.log(`Playwright (Browser): page.on('dialog')`),
		);
		...
		page.on("popup", () =>
			logger.log(`Playwright (Browser): page.on('popup')`),
		);
		page.on("framenavigated", () =>
			logger.log(`Playwright (Browser): page.on('framenavigated')`),
		);
	...
	let url = `${endpoint}&`;
	if (workspacePath) {
		const workspaceParam = workspacePath.endsWith(".code-workspace")
			? "workspace"
			: "folder";
