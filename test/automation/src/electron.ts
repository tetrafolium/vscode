...
export async function resolveElectronConfiguration(
	...
	if (remote) {
		...
		if (codePath) {
			if (!extensionsPath) {
				throw new Error(
					"Extensions path is required when running against a build at the moment.",
				);
			}
	...
	switch (process.platform) {
		case "darwin": {
			const packageJson = require(
				join(root, "Contents", "Resources", "app", "package.json"),
			);
			const product = require(
				join(root, "Contents", "Resources", "app", "product.json"),
			);
		...
		case "win32": {
			const productPath = findFilePath(
				root,
				join("resources", "app", "product.json"),
			);
	...
	switch (process.platform) {
		case "darwin":
			return require(join(root, "Contents", "Resources", "app", "package.json"))
				.version;
		case "win32": {
			const packagePath = findFilePath(
				root,
