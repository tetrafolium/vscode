...
function toChange(element: IElement): Change {
	...
	const actionElementList = findElements(element, (e) =>
...
export class SCM extends Viewlet {
	...
	async waitForChange(name: string, type?: string): Promise<void> {
		const func = (change: Change) =>
			change.name === name && (!type || change.type === type);
	...
	async stage(name: string): Promise<void> {
		await this.code.waitAndClick(
			SCM_RESOURCE_ACTION_CLICK(name, "Stage Changes"),
		);
	...
	async unstage(name: string): Promise<void> {
		await this.code.waitAndClick(
			SCM_RESOURCE_ACTION_CLICK(name, "Unstage Changes"),
		);
