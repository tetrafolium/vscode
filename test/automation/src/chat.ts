...
export class Chat {
	...
	async sendMessage(message: string): Promise<void> {
		...
		await this.code.driver.currentPage
			.locator(this.chatInputSelector)
			.pressSequentially(sanitizedMessage);
	...
	async sendEditorMessage(message: string): Promise<void> {
		...
		const sanitizedMessage = message.replace(/\n/g, " ");
		await this.code.driver.currentPage
			.locator(this.chatEditorInputSelector)
			.pressSequentially(sanitizedMessage);
	...
	async getLatestEditorResponseText(): Promise<string> {
		const response = this.code.driver.currentPage
			.locator(CHAT_EDITOR_RESPONSE_COMPLETE)
			.last();
