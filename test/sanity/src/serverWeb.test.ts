...
export function setup(context: TestContext) {
	context.test(
		"server-web-alpine-arm64",
		["alpine", "arm64", "browser"],
		async () => {
	...
	context.test(
		"server-web-darwin-arm64",
		["darwin", "arm64", "browser"],
		async () => {
	...
	context.test(
		"server-web-win32-arm64",
		["windows", "arm64", "browser"],
		async () => {
	...
	async function testServer(entryPoint: string) {
		...
		await context.runCliApp(
			...
			async (line) => {
				...
				const url = context
					.getWebServerUrl(port, token, test.workspaceDir)
					.toString();
