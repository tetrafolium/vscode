...
export function setup(context: TestContext) {
	...
	context.test(
		"wsl-server-web-arm64",
		["windows", "arm64", "wsl", "browser"],
		async () => {
	...
	async function testServer(entryPoint: string) {
		...
		await context.runCliApp(
			...
			[
				...
				"--connection-token",
				context.getRandomToken(),
				"--host",
				"0.0.0.0",
				"--port",
				context.getUniquePort(),
				"--server-data-dir",
				context.createWslTempDir(),
				"--extensions-dir",
				context.createWslTempDir(),
			],
			async (line) => {
				...
				assert.strictEqual(
					...
					`Expected commit ${context.options.commit} but got ${version}`,
				);
	...
	async function testServerWeb(entryPoint: string) {
		...
		await context.runCliApp(
			...
			async (line) => {
				...
				const url = context
					.getWebServerUrl(port, token, wslWorkspaceDir)
					.toString();
	...
	async function testDesktopApp(entryPoint: string, dataDir: string) {
		...
		const args = [
			...
			test.userDataDir,
			"--folder-uri",
			`vscode-remote://wsl+${wslDistro}${wslWorkspaceDir}`,
		];
		...
		try {
			...
			try {
				...
				await window
					.getByRole("button", { name: "Install and Reload" })
					.click();
...
class WslUITest extends UITest {
	...
	protected override verifyTextFileCreated() {
		...
		const result = this.context.runNoErrors(
			"wsl",
			"cat",
			`${this.wslWorkspaceDir}/helloWorld.txt`,
		);
		assert.strictEqual(
			result.stdout.trim(),
			"Hello, World!",
			"File contents in WSL do not match expected value",
		);
	}

	protected override verifyExtensionInstalled() {
		this.context.log(
			`Verifying extension is installed in WSL at ${this.wslExtensionsDir}`,
		);
		const result = this.context.runNoErrors("wsl", "ls", this.wslExtensionsDir);
		const hasExtension = result.stdout
			.split("\n")
			.some((ext) => ext.startsWith("github.vscode-pull-request-github"));
		assert.strictEqual(
			hasExtension,
			true,
			"GitHub Pull Requests extension is not installed in WSL",
		);
	}
}
