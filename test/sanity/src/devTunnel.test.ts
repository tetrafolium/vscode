...
export function setup(context: TestContext) {
	...
	*/
	...
	context.test(
		"dev-tunnel-darwin-x64",
		["darwin", "x64", "browser", "github-account"],
		async () => {
	...
	*/
	...
	async function testCliApp(entryPoint: string) {
		...
		try {
			...
			await context.runCliApp(
				...
				async (line) => {
					const deviceCode = /To grant access .* use code ([A-Z0-9-]+)/.exec(
						line,
					)?.[1];
					if (deviceCode) {
						context.log(
							`Device code detected: ${deviceCode}, starting device flow authentication`,
						);
					...
					const tunnelUrl =
						/Open this link in your browser (https?:\/\/[^\s]+)/.exec(
							line,
						)?.[1];
					if (tunnelUrl) {
	...
	async function connectToTunnel(
		...
		try {
			...
			await page
				.locator("span.monaco-highlighted-label", { hasText: "GitHub" })
				.click();
			...
			await page
				.getByRole("button", { name: `remote ${tunnelId}` })
