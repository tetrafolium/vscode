 ...
 */
export function applyQuickAccessTools(
	server: McpServer,
		...
		server.tool(
			...
			"Open quick file search and select a file",
			{
				...
				exactFileName: z
					.string()
					.optional()
					.describe("Exact file name to select from results"),
			},
			async (args) => {
				const { fileName, exactFileName } = args;
				const app = await appService.getOrCreateApplication();
				await app.workbench.quickaccess.openFileQuickAccessAndWait(
					fileName,
