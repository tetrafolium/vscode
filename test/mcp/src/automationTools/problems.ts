 ...
 */
export function applyProblemsTools(
	server: McpServer,
	...
	tools.push(
		server.tool(
			...
			"Get CSS selector for problems of a specific severity in the problems view",
			{
				severity: z
					.enum(["WARNING", "ERROR"])
					.describe("Problem severity (WARNING or ERROR)"),
			},
			async (args) => {
				const { severity } = args;
				...
				const selector = (
					app.workbench.problems.constructor as any
				).getSelectorInProblemsView(severityMap[severity]);
				return {
					content: [
						{
							type: "text" as const,
