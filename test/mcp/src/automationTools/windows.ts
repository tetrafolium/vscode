 ...
 */
export function applyWindowTools(
	server: McpServer,
		...
		server.tool(
			...
			'Switch to a different VS Code window by index or URL pattern (e.g., "agent.html")',
			{
				indexOrUrl: z
					...
					.describe(
						'Window index (0-based) or URL pattern to match (e.g., "agent.html", "workbench")',
					),
			},
			async ({ indexOrUrl }) => {
				const app = await appService.getOrCreateApplication();
				...
				const availableWindows = windowInfo
					...
					.join("\n");
				return textResponse(
					`Failed to switch window. Window not found for: ${indexOrUrl}\n\nAvailable windows:\n${availableWindows}`,
				);
			},
		),
	);
	...
	tools.push(
		server.tool(
			...
			async ({ fullPage }) => {
				const app = await appService.getOrCreateApplication();
				const driver = app.code.driver;
				const screenshotBuffer = await driver.screenshotBuffer(
					fullPage ?? false,
				);
		...
		server.tool(
			...
			"Get information about elements matching a selector in the current window",
			{
				...
				action: z
					...
					.optional()
					.describe("Action to perform on matched elements"),
			},
	...
	tools.push(
		server.tool(
			...
			async ({ selector, state, timeout }) => {
				...
				return textResponse(
					`Element "${selector}" is now ${state ?? "visible"}`,
				);
			},
		),
	);
	...
	tools.push(
		server.tool(
			...
			async ({ sourceSelector, targetSelector }) => {
				...
				return textResponse(
					`Dragged from "${sourceSelector}" to "${targetSelector}"`,
				);
			},
		),
	);
		...
		server.tool(
			...
			"Press a key or key combination in the current window",
			{
				key: z
					.string()
					.describe(
						'Key to press (e.g., "Enter", "Tab", "Control+c", "Meta+v")',
					),
			},
			async ({ key }) => {
				const app = await appService.getOrCreateApplication();
	...
	tools.push(
		server.tool(
			...
			async ({ selector, value }) => {
				...
				return textResponse(
					`Selected "${selected.join(", ")}" in "${selector}"`,
				);
			},
		),
	);
	...
	tools.push(
		server.tool(
			...
			async ({ text, textGone, timeout }) => {
				...
				return textResponse(
					`Waited for ${text ? `"${text}" to appear` : ""}${textGone ? `"${textGone}" to disappear` : ""}`,
				);
			},
		),
	);
