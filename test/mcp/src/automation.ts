	...
	const server = new McpServer(
		{
			...
			version: "1.0.0",
			title:
				"An MCP Server that can interact with a local build of VS Code. Used for verifying UI behavior.",
		},
	...
	server.tool(
		...
		"Start VS Code Build. If workspacePath is not provided, VS Code will open with the last used workspace or an empty window.",
		{
			recordVideo: z
				.boolean()
				.optional()
				.describe("Whether to record a video of the session"),
			workspacePath: z
				.string()
				.optional()
				.describe(
					"Optional path to a workspace or folder to open. If not provided, opens the last used workspace.",
				),
		},
				...
				content: [
					{
						type: "text" as const,
						text: app
							? `VS Code started successfully${workspacePath ? ` with workspace: ${workspacePath}` : ""}`
