...
export function setup(logger: Logger) {
	describe("Agents Window", () => {
		...
		before(async function () {
			...
			registerScenario(
				COPILOT_SCENARIO_ID,
				new ScenarioBuilder().emit(COPILOT_REPLY).build(),
			);
			registerScenario(
				LOCAL_SCENARIO_ID,
				new ScenarioBuilder().emit(LOCAL_REPLY).build(),
			);
			registerScenario(
				CLAUDE_SCENARIO_ID,
				new ScenarioBuilder().emit(CLAUDE_REPLY).build(),
			);
		...
		it("Test Copilot CLI session", async function () {
			...
			await app.workbench.agentsWindow.submitNewSessionPrompt(
				`hello world [scenario:${COPILOT_SCENARIO_ID}]`,
			);

			const text =
				await app.workbench.agentsWindow.waitForAssistantText(COPILOT_REPLY);
		...
		it("Test Claude session", async function () {
			...
			const requestsBefore = mockServer.requestCount();
			await app.workbench.agentsWindow.submitNewSessionPrompt(
				`hello world [scenario:${CLAUDE_SCENARIO_ID}]`,
			);

			const text =
				await app.workbench.agentsWindow.waitForAssistantText(CLAUDE_REPLY);
		...
		it("Test Local session", async function () {
			...
			const requestsBefore = mockServer.requestCount();
			await app.workbench.agentsWindow.submitNewSessionPrompt(
				`hello world [scenario:${LOCAL_SCENARIO_ID}]`,
			);

			const text =
				await app.workbench.agentsWindow.waitForAssistantText(LOCAL_REPLY);
