	...
	describe("Extension Host Restart", () => {
		...
		it("kills blocked extension host on window reload (windowLifecycleBound)", async function () {
			...
			await app.workbench.quickaccess.runCommand(
				"smoketest.getExtensionHostPidAndBlock",
			);
			...
			if (!fs.existsSync(pidFile)) {
				throw new Error(
					"PID file was not created - extension may not have activated",
				);
			}
			...
			if (stillExists) {
				throw new Error(
					`Extension host ${pid} still running after reload (waited ${maxWaitMs}ms)`,
				);
			}
		...
		it("allows extensions to gracefully deactivate on window reload (windowLifecycleGraceTime)", async function () {
			...
			const pidFile = path.join(
				os.tmpdir(),
				"vscode-ext-host-pid-graceful.txt",
			);
			const markerFile = path.join(
				os.tmpdir(),
				"vscode-ext-host-deactivated.txt",
			);
			...
			await app.workbench.quickaccess.runCommand(
				"smoketest.setupGracefulDeactivation",
			);
			...
			if (!fs.existsSync(markerFile)) {
				throw new Error(
					`Deactivation marker file was not created within ${maxWaitMs}ms - extension may not have been given time to deactivate gracefully`,
				);
			}
			...
			logger.log(
				"Extension host was properly terminated after graceful deactivation",
			);
		});
		...
		it("kills blocked extension host on restart extension host (issue #296681)", async function () {
			...
			const app = this.app as Application;
			const pidFile = path.join(os.tmpdir(), "vscode-ext-host-pid.txt");
			const activationPidFile = path.join(
				os.tmpdir(),
				"vscode-ext-host-pid-on-activate.txt",
			);
			...
			if (!newPid) {
				throw new Error(
					`New extension host PID was not observed after restart (waited ${maxWaitMs}ms)`,
				);
			}

			if (newPid === oldPid) {
				throw new Error(
					`Extension host PID did not change after restart (pid: ${oldPid})`,
				);
			}
			...
			if (processExists(oldPid)) {
				throw new Error(
					`Old extension host ${oldPid} still running after restart (waited ${maxWaitMs}ms)`,
				);
			}

			logger.log(
				`Extension host restarted successfully (old: ${oldPid}, new: ${newPid})`,
			);
		});
	});
}
