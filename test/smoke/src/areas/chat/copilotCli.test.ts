...
import {
	getCopilotSmokeTestEnv,
	getMockLlmServerPath,
	installAllHandlers,
	MockLlmServer,
} from "../../utils";
...
export function setup(logger: Logger) {
	describe("Copilot CLI", function () {
		...
		before(async function () {
			const { startServer, ScenarioBuilder, registerScenario } = require(
				getMockLlmServerPath(),
			);

			registerScenario("text-only", new ScenarioBuilder().emit("OK").build());
			registerScenario(
				COPILOT_CLI_SCENARIO_ID,
				new ScenarioBuilder().emit(COPILOT_CLI_REPLY).build(),
			);
		...
		it("opens a Copilot CLI session and receives a response", async function () {
			...
			try {
				await app.workbench.quickaccess.runCommand(
					"smoketest.openCopilotCliChat",
				);
				await app.workbench.chat.waitForChatEditor(600);
				await app.workbench.chat.sendEditorMessage(
					`Reply with one short sentence. Do not run tools or edit files. [scenario:${COPILOT_CLI_SCENARIO_ID}]`,
				);
				await app.workbench.chat.waitForEditorResponse(1500);
			} catch (error) {
				const diagnostics = await getCopilotCliDiagnostics(app, mockServer);
				throw new Error(
					`Copilot CLI smoke test failed: ${diagnostics.summary}\n\nUI symptom: ${summarizeUiFailure(error)}\n\n${diagnostics.details}`,
				);
			}

			const responseText = (
				await app.workbench.chat.getLatestEditorResponseText()
			).trim();
			const diagnostics =
				responseText.length > 0
					? undefined
					: await getCopilotCliDiagnostics(app, mockServer);
			assert.ok(
				responseText.length > 0,
				`Expected Copilot CLI to produce a non-empty response.${diagnostics ? `\nLikely cause: ${diagnostics.summary}\n\n${diagnostics.details}` : ""}`,
			);
			assert.ok(
			...
			assert.ok(
				...
				"Expected the mock LLM server to receive a request from the Copilot CLI editor session",
			);

			const normalizedResponse = responseText.toLowerCase();
			const matchedFailureMarker = failureMarkers.find((marker) =>
				normalizedResponse.includes(marker.toLowerCase()),
			);
			if (matchedFailureMarker) {
				const diagnostics = await getCopilotCliDiagnostics(app, mockServer);
				assert.fail(
					`Copilot CLI response contained failure marker "${matchedFailureMarker}". Likely cause: ${diagnostics.summary}\n\nResponse:\n${responseText}\n\n${diagnostics.details}`,
				);
			}
		});
	});
...
async function getCopilotCliDiagnostics(
	app: Application,
	mockServer?: MockLlmServer,
): Promise<CopilotCliDiagnostics> {
	...
	const sessionEventsTail = readSessionEventsTail(
		extractLatestSessionId(copilotChatTail),
	);
	...
	return {
		...
		details: [
			...
			sessionEventsTail &&
			!sessionEventsTail.startsWith("Session events file not found:")
...
function summarizeUiFailure(error: unknown): string {
	if (error instanceof Error) {
		const timeout = error.message.match(
			/Timeout: get element '([^']+)' after ([^.]+)\./,
		);
		if (timeout) {
...
function summarizeCopilotCliFailure(diagnostics: string): string {
	const nativeAddon = diagnostics.match(
		/Native addon "([^"]+)" not found for ([^\s.]+)/,
	);
	if (nativeAddon) {
...
function isQueryFailure(message: string): boolean {
	return /\[CopilotCLISession\]CopilotCLI error: \(query\)|\(query\) Execution failed|Native addon ".*" not found|Failed to load @github\/copilot\/sdk|Unable to find node-pty binaries|Failed to create (?:node-pty|ripgrep) shim|Cannot find module|MODULE_NOT_FOUND|runtime\.node|pty\.node/.test(
		message,
	);
}

function isAuthFailure(message: string): boolean {
	return /Authorization failed|Unauthorized|\b401\b|No model available|policy enablement|sign in to GitHub|getGitHubSession.*undefined|Authentication (?:failed|required)|Timed out waiting for authentication provider|authentication provider ['"]github['"]/i.test(
		message,
	);
}
...
function getRelevantLogTail(diagnostics: string, summary: string): string {
	const nativeFailure =
		summary.startsWith("native SDK module missing") ||
		summary.startsWith("native module / SDK boot failure");
	return diagnostics
		.split(/\r?\n/)
		.map((line) => line.trim())
		.filter((line) => line && !isKnownScenarioAutomationNoise(line))
		.filter((line) =>
			nativeFailure ? isNativeFailureLine(line) : isRelevantFailureLine(line),
		)
		.slice(-40)
		.join("\n");
}
...
function isNativeFailureLine(line: string): boolean {
	return /Native addon ".*" not found|runtime\.node|pty\.node|MODULE_NOT_FOUND|Cannot find module|Failed to load @github\/copilot\/sdk|Failed to initialize|CopilotCLI error: \(query\)|Using Copilot CLI session|Invoking session/i.test(
		line,
	);
}
...
function isRelevantFailureLine(line: string): boolean {
	return (
		isNativeFailureLine(line) ||
		isAuthFailure(line) ||
		/\[CopilotCLI\]|\[CopilotCLISession\]|No model available/i.test(line)
	);
}

function isKnownScenarioAutomationNoise(line: string): boolean {
	return /GitHubOrgCustomAgentProvider|GitHubOrgInstructionsProvider|getCurrentAuthedUser is not a function|Invalid response format/.test(
		line,
	);
}

function findNewestLog(
	logs: readonly { relativePath: string; contents: string }[],
	fileName: string,
...
function extractLatestSessionId(log: string): string | undefined {
	...
	for (const match of log.matchAll(
		/Using Copilot CLI session: ([0-9a-f-]+)/g,
	)) {
		sessionId = match[1];
	}
	return sessionId;
