	...
	describe("Language Features", () => {
		...
		it("verifies problems view (css)", async function () {
			...
			await app.code.waitForElement(
				Problems.getSelectorInEditor(ProblemSeverity.WARNING),
			);

			await app.workbench.problems.showProblemsView();
			await app.code.waitForElement(
				Problems.getSelectorInProblemsView(ProblemSeverity.WARNING),
			);
		...
		it("verifies settings (css)", async function () {
			...
			await app.code.waitForElement(
				Problems.getSelectorInEditor(ProblemSeverity.ERROR),
			);

			await app.workbench.problems.showProblemsView();
			await app.code.waitForElement(
				Problems.getSelectorInProblemsView(ProblemSeverity.ERROR),
			);
			await app.workbench.problems.hideProblemsView();
		});
	});
