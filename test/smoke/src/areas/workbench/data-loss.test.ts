...
export function setup(
	ensureStableCode: () => {
		stableCodePath: string | undefined;
		stableCodeVersion:
	...
	describe("Data Loss (insiders -> insiders)", function () {
		...
		it("verifies opened editors are restored", async function () {
			...
			await app.workbench.quickaccess.openFile(
				join(app.workspacePathOrFolder, "bin", "www"),
			);
			await app.workbench.quickaccess.runCommand("View: Keep Editor");
			await app.workbench.quickaccess.openFile(
				join(app.workspacePathOrFolder, "app.js"),
			);
			await app.workbench.quickaccess.runCommand("View: Keep Editor");
		...
		async function testHotExit(
			...
			await app.workbench.editors.newUntitledFile();
			await app.workbench.editor.waitForTypeInEditor(
				"Untitled-1",
	...
	describe("Data Loss (stable -> insiders)", function () {
		...
		it("verifies opened editors are restored", async function () {
			...
			stableOptions.version = stableCodeVersion ?? {
				major: 0,
				minor: 0,
				patch: 0,
			};
		...
		async function testHotExit(
			...
			await stableApp.workbench.editors.newUntitledFile();
			await stableApp.workbench.editor.waitForTypeInEditor(
				"Untitled-1",
				textToTypeInUntitled,
			);
