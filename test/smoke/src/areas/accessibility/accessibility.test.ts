...
export function setup(
	...
	describe("Accessibility", function () {
		...
		describe("Workbench", function () {
			(opts.web ? it.skip : it)(
				"workbench has no accessibility violations",
				async function () {
					...
					await app.code.driver.assertNoAccessibilityViolations({
						...
						excludeRules: {
							...
							"link-in-text-block": [
								"command:workbench.action.chat.generateAgentInstructions",
							],
		...
		if (!opts.web) {
			describe("Chat", function () {
				...
				it("chat response has no accessibility violations", async function () {
					...
					await app.workbench.chat.sendMessage(
						'Explain what "Hello World" means in programming. Include a short fenced code block that shows "Hello World".',
					);
				...
				(quality === Quality.Dev || quality === Quality.OSS ? it.skip : it)(
