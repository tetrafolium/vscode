		...
		gulp
			...
			.pipe(
				es.through(function (data) {
					...
					try {
						const markedManifestPath = path.join(
							root,
							"src/vs/base/common/marked/cgmanifest.json",
						);
						const dompurifyManifestPath = path.join(
							root,
							"src/vs/base/browser/dompurify/cgmanifest.json",
						);

						const markedManifest = JSON.parse(
							fs.readFileSync(markedManifestPath, "utf8"),
						);
						const dompurifyManifest = JSON.parse(
							fs.readFileSync(dompurifyManifestPath, "utf8"),
						);
						...
						if (!markedVersion || !dompurifyVersion) {
							throw new Error(
								"Unable to read versions from cgmanifest.json files",
							);
						}
					} catch (error) {
						throw new Error(
							`Failed to read cgmanifest.json files for monaco-editor-core dependencies: ${error.message}`,
						);
					}
...
export const monacoTypecheckWatchTask = task.define(
	"monaco-typecheck-watch",
	createTscCompileTask(true),
);

export const monacoTypecheckTask = task.define(
	"monaco-typecheck",
...
function setUnsetField(
	...
	if (obj[field] !== undefined) {
		throw new Error(
			`Field "${field}" is already set (but was expected to not be).`,
		);
	}
	obj[field] = value;
}
