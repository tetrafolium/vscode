...
const setupDir = (arch: string, target: string) =>
...
function packageInnoSetup(
	iss: string,
	options: { definitions?: Record<string, unknown> },
...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		const quality =
			(product as typeof product & { quality?: string }).quality || "dev";
		...
		if (quality === "stable" || quality === "insider") {
			...
			const ctxMenu = (
				product as { win32ContextMenu?: Record<string, { clsid: string }> }
			).win32ContextMenu;
