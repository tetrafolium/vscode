...
} from "./gulpfile.extensions.ts";
import {
	vscodeWebResourceIncludes,
	createVSCodeWebFileContentMapper,
} from "./gulpfile.vscode.web.ts";
...
import {
	...
	getRipgrepExcludeFilter,
	prepareBuiltInCopilotRipgrepShim,
} from "./lib/copilot.ts";
...
const serverResources = [...serverResourceIncludes, ...serverResourceExcludes];
...
function extractAlpinefromDocker(
	nodeVersion: string,
	platform: string,
	arch: string,
) {
	...
	return es.readArray([
		new File({
			path: "node",
			contents,
			stat: { mode: parseInt("755", 8) } as fs.Stats,
		}),
	]);
}
...
function nodejs(
	platform: string,
	arch: string,
): NodeJS.ReadWriteStream | undefined {
	if (arch === "alpine") {
		platform = "alpine";
		arch = "x64";
	}
	...
	const checksumSha256 = expectedName
		? getNodeChecksum(expectedName)
		: undefined;
...
function packageTask(
	...
	return () => {
		...
		const productJsonStream = gulp
			...
			.pipe(
				jsonEditor({ commit, date: readISODate(sourceFolderName), version }),
			)
			.pipe(
				es.through(function (file) {
		...
		const dependenciesSrc = productionDependencies
			...
			.flat();
		const cleanedDeps = gulp
			.src(dependenciesSrc, { base: "remote", dot: true })
			// filter out unnecessary files, no source maps in server build
			.pipe(filter(["**", "!**/package-lock.json", "!**/*.{js,css}.map"]))
			.pipe(
				util.cleanNodeModules(path.join(import.meta.dirname, ".moduleignore")),
			)
			.pipe(
				util.cleanNodeModules(
					path.join(import.meta.dirname, `.moduleignore.${process.platform}`),
				),
			);
		const copilotRuntimePrebuilds = gulp.src(
			getCopilotRuntimePrebuildFiles(platform, arch, "remote/node_modules"),
			{ base: "remote", dot: true, allowEmpty: true },
		);
		const deps = es
			.merge(cleanedDeps, copilotRuntimePrebuilds)
		...
		if (platform === "win32") {
			result = es.merge(
				result,
				gulp
					.src("resources/server/bin/remote-cli/code.cmd", { base: "." })
					.pipe(replace("@@VERSION@@", version))
					.pipe(replace("@@COMMIT@@", commit || ""))
					.pipe(replace("@@APPNAME@@", product.applicationName))
					.pipe(rename(`bin/remote-cli/${product.applicationName}.cmd`)),
				gulp
					.src("resources/server/bin/helpers/browser.cmd", { base: "." })
					.pipe(replace("@@VERSION@@", version))
					.pipe(replace("@@COMMIT@@", commit || ""))
					.pipe(replace("@@APPNAME@@", product.applicationName))
		...
		if (platform === "linux" || platform === "alpine") {
			result = es.merge(
				result,
				gulp
					.src(`resources/server/bin/helpers/check-requirements-linux.sh`, {
						base: ".",
					})
	...
	await new Promise<void>((resolve, reject) => {
		...
		proc.on("exit", (code) => {
			if (code === 0) {
				...
				reject(
					new Error(`signtool remove /s failed for ${filePath} (exit ${code})`),
				);
			}
		});
	});
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		const productContents = JSON.parse(
			await fs.promises.readFile(path.join(cwd, "product.json"), "utf8"),
		);
...
function prepareCopilotRipgrepShimTaskREH(
	platform: string,
	arch: string,
	destinationFolderName: string,
) {
	return async () => {
		...
		const builtInCopilotExtensionDir = path.join(
			outputDir,
			"extensions",
			"copilot",
		);
...
function tweakProductForServerWeb(product: typeof import("../product.json")) {
	const result: typeof product & { webEndpointUrlTemplate?: string } = {
		...product,
	};
