...
try {
	...
	const outPackageJsonPath = path.resolve(
		process.cwd(),
		"DefinitelyTyped/types/vscode/package.json",
	);
	...
	console.log(
		`Done updating vscode.d.ts at ${outDtsPath} and package.json to version ${packageJson.version}`,
	);
} catch (err) {
	console.error(err);
...
function getNewFileContent(content: string, shorttag: string) {
	...
	return convertTabsToSpaces(
		getNewFileHeader(shorttag) + content.slice(oldheader.length),
	);
}
