			...
			return [
				{
					...
					parameters: [
						...
						{ parameterName: "PageHash", parameterValue: "/NPH" },
						{
							...
							parameterValue:
								'/tr "http://rfc3161.gtm.corp.microsoft.com/TSS/HttpTspServer" /td sha256',
						},
					],
					toolName: "sign",
					toolVersion: "1.0",
...
export function main([esrpCliPath, type, folderPath, pattern]: string[]) {
	...
	const encryptedToken =
		cipher.update(process.env["SYSTEM_ACCESSTOKEN"]!.trim(), "utf8", "hex") +
		cipher.final("hex");
	...
	const args = [
		...
		"-k",
		JSON.stringify({ akv: "vscode-esrp" }),
		"-z",
		JSON.stringify({ akv: "vscode-esrp", cert: "esrp-sign" }),
		"-f",
		folderPath,
		"-p",
		patternPath,
		...
		"-w",
		dotnetVersion,
		"-skipAdoReportAttachment",
		"false",
		"-pendingAnalysisWaitTimeoutMinutes",
		"5",
		"-adoTaskVersion",
		adoTaskVersion,
		"-resourceUri",
		"https://msazurecloud.onmicrosoft.com/api.esrp.microsoft.com",
		"-esrpClientId",
		process.env["ESRP_CLIENT_ID"]!,
		"-useMSIAuthentication",
		"true",
		"-federatedTokenData",
		JSON.stringify(federatedTokenData),
	];
