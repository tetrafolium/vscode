...
async function getDistroBranchHead(
	branch: string,
	token: string,
): Promise<string> {
	...
	if (!response.ok) {
		throw new Error(
			`Failed to fetch branch ${branch} from vscode-distro: ${response.status} ${response.statusText}`,
		);
	}
...
async function checkDistroCommit(): Promise<void> {
	...
	if (!branchMatch) {
		console.log(
			`Cannot determine branch from BUILD_SOURCEBRANCH: ${sourceBranch}, skipping check`,
		);
		return;
	}
	...
	try {
		...
		console.log(
			`Could not fetch branch '${branch}' from vscode-distro: ${error}`,
		);
		console.log(
			"This is expected for feature branches that have not been merged to distro",
		);
		return;
	}
	...
	if (distroCommit === distroBranchHead) {
		...
		console.log(
			`##vso[task.logissue type=warning]Distro commit mismatch: package.json has ${distroCommit.substring(0, 8)} but ${branch} HEAD is ${distroBranchHead.substring(0, 8)}`,
		);
		console.log(
			`##vso[task.complete result=SucceededWithIssues;]Distro commit does not match branch HEAD`,
		);
		console.log("");
		console.log(
			`⚠️  WARNING: Distro commit in package.json does not match the HEAD of branch '${branch}' in vscode-distro`,
		);
