...
const BUILD_STAGINGDIRECTORY =
	process.env.BUILD_STAGINGDIRECTORY ??
	fs.mkdtempSync(path.join(os.tmpdir(), "vscode-telemetry-"));
const BUILD_SOURCESDIRECTORY =
...
for (const repo of repos) {
	cp.execSync(`git clone --depth 1 ${repo}`, {
		cwd: extractionDir,
		stdio: "inherit",
	});
}
...
const telemetryConfig = path.join(
	BUILD_SOURCESDIRECTORY,
	"build",
	"azure-pipelines",
	"common",
...
const telemetryConfigEntries = JSON.parse(
	fs.readFileSync(telemetryConfig, "utf8"),
) as ITelemetryConfigEntry[];
...
const resolvedTelemetryConfigEntries = telemetryConfigEntries.map((entry) => {
	const sourceDirs = entry.sourceDirs.map((sourceDir) => {
		...
		const extensionRelativePath = sourceDir.slice(
			pipelineExtensionsPathPrefix.length,
		);
		const sourceDirInWorkspace = path.join(
			BUILD_SOURCESDIRECTORY,
			"extensions",
...
if (hasLocalConfigOverrides) {
	fs.writeFileSync(
		telemetryConfigForExtraction,
		JSON.stringify(resolvedTelemetryConfigEntries, null, "\t"),
	);
}

try {
	cp.execSync(
		`node "${extractor}" --sourceDir "${BUILD_SOURCESDIRECTORY}" --excludedDir "${path.join(BUILD_SOURCESDIRECTORY, "extensions")}" --outputDir . --applyEndpoints`,
		{ cwd: extractionDir, stdio: "inherit" },
	);
	cp.execSync(
		`node "${extractor}" --config "${telemetryConfigForExtraction}" -o .`,
		{ cwd: extractionDir, stdio: "inherit" },
	);
} catch (error) {
...
fs.renameSync(
	path.join(extractionDir, "declarations-resolved.json"),
	path.join(telemetryDir, "telemetry-core.json"),
);
fs.renameSync(
	path.join(extractionDir, "config-resolved.json"),
	path.join(telemetryDir, "telemetry-extensions.json"),
);
