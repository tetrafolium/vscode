...
async function main() {
	...
	const codeSignClientTask = spawnCodesignProcess(
		esrpCliDLLPath,
		"sign-darwin",
		clientFolder,
		clientGlob,
	);
	const codeSignDmgTask = spawnCodesignProcess(
		esrpCliDLLPath,
		"sign-darwin",
		dmgFolder,
		dmgGlob,
	...
	if (arch !== "universal") {
		codeSignServerTask = spawnCodesignProcess(
			esrpCliDLLPath,
			"sign-darwin",
			serverFolder,
			serverGlob,
		);
		codeSignWebTask = spawnCodesignProcess(
			esrpCliDLLPath,
			"sign-darwin",
			serverFolder,
			webGlob,
	...
	const notarizeClientTask = spawnCodesignProcess(
		esrpCliDLLPath,
		"notarize-darwin",
		clientFolder,
		clientGlob,
	);
	const notarizeDmgTask = spawnCodesignProcess(
		esrpCliDLLPath,
		"notarize-darwin",
		dmgFolder,
		dmgGlob,
	...
	if (arch !== "universal") {
		notarizeServerTask = spawnCodesignProcess(
			esrpCliDLLPath,
			"notarize-darwin",
			serverFolder,
			serverGlob,
		);
		notarizeWebTask = spawnCodesignProcess(
			esrpCliDLLPath,
			"notarize-darwin",
			serverFolder,
			webGlob,
