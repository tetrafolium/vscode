...
async function retrySignOnKeychainError<T>(
	fn: () => Promise<T>,
	maxRetries: number = 3,
): Promise<T> {
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			const isKeychainError = errorMessage.includes(
			...
			console.log(
				`Signing attempt ${attempt} failed with keychain error, retrying...`,
			);
			console.log(`Error: ${errorMessage}`);

			const delay = 1000 * Math.pow(2, attempt - 1);
			console.log(
				`Waiting ${Math.round(delay)}ms before retry ${attempt}/${maxRetries}...`,
			);
...
if (import.meta.main) {
	main(process.argv[2]).catch(async (err) => {
		...
		if (tempDir) {
			...
			const identities = await spawn("security", [
				"find-identity",
				"-p",
				"codesigning",
				"-v",
