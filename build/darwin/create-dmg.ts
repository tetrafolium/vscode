...
async function findSuitablePython(): Promise<string> {
	...
	console.log(
		`No Python >= ${MIN_PYTHON_VERSION[0]}.${MIN_PYTHON_VERSION[1]} found, installing via Homebrew...`,
	);
	...
	throw new Error(
		`Could not find Python >= ${MIN_PYTHON_VERSION[0]}.${MIN_PYTHON_VERSION[1]} even after Homebrew install at ${brewPrefix}.`,
	);
}
...
async function ensureDmgBuild(): Promise<void> {
	...
	console.log(
		`Cloning dmgbuild from ${DMGBUILD_REPO} at ${DMGBUILD_COMMIT}...`,
	);
...
async function runDmgBuild(
	...
	await spawn(
		pythonPath,
		["-m", "dmgbuild", "-s", settingsFile, volumeName, artifactPath],
		{
			stdio: "inherit",
		},
	);
...
async function main(buildDir?: string, outDir?: string): Promise<void> {
	...
	console.log(
		`Successfully created DMG: ${artifactPath} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`,
	);
}
