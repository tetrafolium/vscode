...
checkMachOFiles(process.env["APP_PATH"], archToCheck)
	.then((invalidFiles) => {
		...
		const actualInvalidFiles = invalidFiles.filter(
			(file) => !isFileSkipped(file),
		);
		if (actualInvalidFiles.length > 0) {
			console.error(
				"\x1b[31mThese files are built for the wrong architecture:\x1b[0m",
			);
			actualInvalidFiles.forEach((file) =>
