 ...
 */
export default function gulpstylelint(
	reporter: Reporter,
	...
	const restrictedPathPattern =
		/^src[\/\\]vs[\/\\](base|platform|editor)[\/\\]/;
	const layerCheckerDisablePattern =
		/\/\*\s*stylelint-disable\s+layer-checker\s*\*\//;

	return es.through(
		function (this, file: FileWithLines) {
			const lines =
			...
			const isLayerCheckerDisabled = lines.some((line) =>
				layerCheckerDisablePattern.test(line),
			);
			...
			lines.forEach((line, i) => {
				...
				if (
					isRestrictedPath &&
					!isLayerCheckerDisabled &&
					monacoWorkbenchPattern.test(line)
				) {
					reporter(
						file.relative +
							"(" +
							(i + 1) +
							",1): The class .monaco-workbench cannot be used in files under src/vs/{base,platform,editor} because only src/vs/workbench applies it",
						true,
					);
					errorCount++;
				}
...
function stylelint(): NodeJS.ReadWriteStream {
	return vfs
		.src(Array.from(stylelintFilter), {
			base: ".",
			follow: true,
			allowEmpty: true,
		})
