...
const getBaseUrl = (out: string) =>
	`https://main.vscode-cdn.net/sourcemaps/${commit}/${out}`;

function rewriteTsgoSourceMappingUrlsIfNeeded(
	build: boolean,
	out: string,
	baseUrl: string,
): Promise<void> {
...
const tasks = compilations.map(function (tsconfigFile) {
	...
	const tsBuildInfoFile = path.join(
		...
		path.basename(absolutePath, ".json") + ".tsbuildinfo",
	);
	...
	const compileTask = task.define(
		...
		task.series(cleanTask, async () => {
			...
			const tsgo = spawnTsgo(absolutePath, { taskName: "extensions" }, () =>
				rewriteTsgoSourceMappingUrlsIfNeeded(false, out, baseUrl),
			);
	...
	const watchTask = task.define(
		...
		task.series(cleanTask, () => {
			...
			const watchNonTs = watchInput
				.pipe(filter(["**", "!**/*.ts"], { dot: true }))
				.pipe(gulp.dest(out));
			const tsgoStream = watchInput.pipe(
				util.debounce(() => {
					onExtensionCompilationStart();
					const stream = createTsgoStream(
						absolutePath,
						{ taskName: "extensions" },
						() => rewriteTsgoSourceMappingUrlsIfNeeded(false, out, baseUrl),
					);
			...
			const watchStream = es.merge(
				nonts.pipe(gulp.dest(out)),
				watchNonTs,
				tsgoStream,
			);
...
export const compileExtensionsTask = task.define(
	"compile-extensions",
	task.parallel(...tasks.map((t) => t.compileTask)),
);
task.task(compileExtensionsTask);

export const watchExtensionsTask = task.define(
	"watch-extensions",
...
export const compileExtensionMediaTask = task.define(
	"compile-extension-media",
	() => ext.buildExtensionMedia(false),
);
task.task(compileExtensionMediaTask);

export const watchExtensionMedia = task.define("watch-extension-media", () =>
	ext.buildExtensionMedia(true),
);
task.task(watchExtensionMedia);

export const compileExtensionMediaBuildTask = task.define(
 ...
 */
export const cleanExtensionsBuildTask = task.define(
 ...
 */
export const compileNonNativeExtensionsBuildTask = task.define(
	"compile-non-native-extensions-build",
	task.series(
		bundleMarketplaceExtensionsBuildTask,
		task.define("bundle-non-native-extensions-build", () =>
			ext
				.packageNonNativeLocalExtensionsStream(false, false)
 ...
 * @note this does not clean the directory ahead of it. See {@link cleanExtensionsBuildTask} for that.
 */
export const compileNativeExtensionsBuildTask = task.define(
	"compile-native-extensions-build",
	() =>
		ext
			.packageNativeLocalExtensionsStream(false, false)
			.pipe(gulp.dest(".build")),
);
 ...
 */
export const compileCopilotExtensionBuildTask = task.define(
	"compile-copilot-extension-build",
	() => ext.packageCopilotExtensionStream(false).pipe(gulp.dest(".build")),
);
 ...
 */
export const compileAllExtensionsBuildTask = task.define(
...
export const compileWebExtensionsTask = task.define("compile-web", () =>
	buildWebExtensions(false),
);
task.task(compileWebExtensionsTask);

export const watchWebExtensionsTask = task.define("watch-web", () =>
...
async function buildWebExtensions(isWatch: boolean): Promise<void> {
	...
	if (esbuildConfigLocations.length > 0) {
		promises.push(
			ext.esbuildExtensions(
				"packaging web extension (esbuild)",
				isWatch,
				esbuildConfigLocations.map((script) => ({ script })),
			),
