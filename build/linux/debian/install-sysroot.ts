		...
		try {
			...
			if (response.ok && response.status >= 200 && response.status < 300) {
				...
				if (
					...
					console.log(
						`Fetched response body buffer: ${(assetContents as Buffer).byteLength} bytes`,
					);
					if (options.checksumSha256) {
						...
						if (actualSHA256Checksum !== options.checksumSha256) {
							throw new Error(
								`Checksum mismatch for ${asset.url} (expected ${options.checksumSha256}, actual ${actualSHA256Checksum}))`,
							);
						}
					}
				...
				throw new Error(
					`Request ${asset.url} failed with status code: ${assetResponse.status}`,
				);
			}
			throw new Error(
				`Request https://api.github.com failed with status code: ${response.status}`,
			);
		} finally {
...
export async function getChromiumSysroot(
	...
	if (sha !== tarballSha) {
		throw new Error(
			`Tarball checksum is wrong. Expected ${tarballSha}, actual ${sha}`,
		);
	}
