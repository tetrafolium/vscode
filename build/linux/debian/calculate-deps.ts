...
export function generatePackageDeps(
	files: string[],
...
function calculatePackageDeps(
	...
	const chromiumManifest = manifests.registrations.filter((registration) => {
		return (
			registration.component.type === "git" &&
	...
	const requiresList = dpkgShlibdepsResult.stdout
		.toString("utf-8")
		.trimEnd()
		.split("\n");
	let depsStr = "";
