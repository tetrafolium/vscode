...
function prepareDebPackage(arch: string) {
	...
	return async function () {
		...
		const desktop = gulp
			.src("resources/linux/code.desktop", { base: "." })
			.pipe(
				rename(
					"usr/share/applications/" + product.applicationName + ".desktop",
				),
			);
		...
		const desktops = es
			.merge(desktop, desktopUrlHandler)
		...
		const appdata = gulp
			.src("resources/linux/code.appdata.xml", { base: "." })
			.pipe(replace("@@NAME_LONG@@", product.nameLong))
			.pipe(replace("@@NAME@@", product.applicationName))
			.pipe(replace("@@LICENSE@@", product.licenseName))
			.pipe(
				rename("usr/share/appdata/" + product.applicationName + ".appdata.xml"),
			);

		const workspaceMime = gulp
			.src("resources/linux/code-workspace.xml", { base: "." })
			.pipe(replace("@@NAME_LONG@@", product.nameLong))
			.pipe(replace("@@NAME@@", product.applicationName))
			.pipe(
				rename(
		...
		const bash_completion = gulp
			.src("resources/completions/bash/code")
			.pipe(replace("@@APPNAME@@", product.applicationName))
			.pipe(
				rename(
					"usr/share/bash-completion/completions/" + product.applicationName,
				),
			);

		const zsh_completion = gulp
			.src("resources/completions/zsh/_code")
			.pipe(replace("@@APPNAME@@", product.applicationName))
			.pipe(
				rename("usr/share/zsh/vendor-completions/_" + product.applicationName),
			);
		...
		let size = 0;
		const control = code.pipe(
			es.through(
				function (f) {
					size += f.isDirectory() ? 4096 : f.contents.length;
				},
				function () {
					const that = this;
					gulp
						.src("resources/linux/debian/control.template", { base: "." })
						.pipe(replace("@@NAME@@", product.applicationName))
						.pipe(
							replace(
								"@@VERSION@@",
						...
						.pipe(
							replace("@@INSTALLEDSIZE@@", Math.ceil(size / 1024).toString()),
						)
		...
		const prerm = gulp
			.src("resources/linux/debian/prerm.template", { base: "." })
			.pipe(replace("@@NAME@@", product.applicationName))
			.pipe(rename("DEBIAN/prerm"));

		const postrm = gulp
			.src("resources/linux/debian/postrm.template", { base: "." })
			.pipe(replace("@@NAME@@", product.applicationName))
			.pipe(rename("DEBIAN/postrm"));

		const postinst = gulp
			.src("resources/linux/debian/postinst.template", { base: "." })
			.pipe(replace("@@NAME@@", product.applicationName))
			.pipe(replace("@@ARCHITECTURE@@", debArch))
			.pipe(rename("DEBIAN/postinst"));

		const templates = gulp
			.src("resources/linux/debian/templates.template", { base: "." })
			.pipe(replace("@@NAME@@", product.applicationName))
...
function buildDebPackage(arch: string) {
	...
	return async () => {
		...
		await exec(
			`fakeroot dpkg-deb -Zxz -b ${product.applicationName}-${debArch} deb`,
			{ cwd },
		);
	};
}
...
function prepareRpmPackage(arch: string) {
	...
	return async function () {
		...
		const appdata = gulp
			...
			.pipe(
				rename(
					"BUILD/usr/share/appdata/" + product.applicationName + ".appdata.xml",
				),
			);
		...
		const icon = gulp
			.src("resources/linux/code.png", { base: "." })
			.pipe(
				rename("BUILD/usr/share/pixmaps/" + product.linuxIconName + ".png"),
			);
		...
		const zsh_completion = gulp
			...
			.pipe(
				rename(
					"BUILD/usr/share/zsh/site-functions/_" + product.applicationName,
				),
			);
		...
		const code = gulp.src(binaryDir + "/**/*", { base: binaryDir }).pipe(
			rename(function (p) {
				p.dirname =
					"BUILD/usr/share/" + product.applicationName + "/" + p.dirname;
			}),
		);

		const spec = gulp
			.src("resources/linux/rpm/code.spec.template", { base: "." })
			.pipe(replace("@@NAME@@", product.applicationName))
			.pipe(replace("@@NAME_LONG@@", product.nameLong))
			.pipe(replace("@@ICON@@", product.linuxIconName))
			...
			.pipe(
				replace(
					"@@QUALITY@@",
					(product as typeof product & { quality?: string }).quality ||
						"@@QUALITY@@",
				),
			)
			.pipe(
				replace(
					"@@UPDATEURL@@",
					(product as typeof product & { updateUrl?: string }).updateUrl ||
						"@@UPDATEURL@@",
				),
			)
		...
		const specIcon = gulp
			.src("resources/linux/rpm/code.xpm", { base: "." })
...
function prepareSnapPackage(arch: string) {
	...
	return function () {
		...
		const desktops = es
			...
			.pipe(
				replace("@@ICON@@", `\${SNAP}/meta/gui/${product.linuxIconName}.png`),
			)
			.pipe(replace("@@URLPROTOCOL@@", product.urlProtocol));
		...
		const snapcraft = gulp
			.src("resources/linux/snap/snapcraft.yaml", { base: "." })
			.pipe(replace("@@NAME@@", product.applicationName))
		...
		const electronLaunch = gulp
			.src("resources/linux/snap/electron-launch", { base: "." })
...
const BUILD_TARGETS = [{ arch: "x64" }, { arch: "armhf" }, { arch: "arm64" }];

BUILD_TARGETS.forEach(({ arch }) => {
	const debArch = getDebPackageArch(arch);
	const prepareDebTask = task.define(
		`vscode-linux-${arch}-prepare-deb`,
		task.series(rimraf(`.build/linux/deb/${debArch}`), prepareDebPackage(arch)),
	);
	...
	const rpmArch = getRpmPackageArch(arch);
	const prepareRpmTask = task.define(
		`vscode-linux-${arch}-prepare-rpm`,
		task.series(rimraf(`.build/linux/rpm/${rpmArch}`), prepareRpmPackage(arch)),
	);
	...
	const prepareSnapTask = task.define(
		`vscode-linux-${arch}-prepare-snap`,
		task.series(rimraf(`.build/linux/snap/${arch}`), prepareSnapPackage(arch)),
	);
