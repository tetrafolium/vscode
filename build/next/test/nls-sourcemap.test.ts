...
async function bundleWithNLS(
	...
	const tmpDir = await fs.promises.mkdtemp(
		path.join(os.tmpdir(), "nls-sm-test-"),
	);
...
suite("NLS plugin source maps", () => {
	...
	test("sourcesContent should contain original source, not NLS-transformed", async () => {
		...
		try {
			...
			const greetingIdx = sources.findIndex((s: string) =>
				s.includes("greeting"),
			);
			...
			assert.ok(
				!greetingContent.includes("%%NLS:"),
				"sourcesContent should NOT contain NLS placeholder.\nActual:\n" +
					greetingContent,
			);
			assert.ok(
				greetingContent.includes('localize("myKey", "Hello World")'),
	...
	test("NLS-affected nested file keeps a non-duplicated source path", async () => {
		...
		try {
			const sources: string[] = mapJson.sources ?? [];
			const nestedSource = sources.find((s: string) =>
				s.endsWith("/nested/deep/file.ts"),
			);
			assert.ok(nestedSource, "Should find nested/deep/file.ts in sources");
			assert.ok(
				!nestedSource.includes("/nested/deep/nested/deep/file.ts"),
	...
	test("column mapping for code on same line after localize call", async () => {
		...
		try {
			assert.ok(
				js.includes("%%NLS:"),
				"Bundle should contain NLS placeholders",
			);
	...
	test("class with localize - method positions map correctly", async () => {
		...
		try {
			...
			assert.strictEqual(
				pos.line,
				13,
				"Should map dispose method body to line 13",
			);
		} finally {
			cleanup();
		}
	...
	test("post-processed NLS - source map still has original content", async () => {
		...
		try {
			assert.ok(
				!js.includes("%%NLS:"),
				"JS should not contain NLS placeholders after post-processing",
			);
