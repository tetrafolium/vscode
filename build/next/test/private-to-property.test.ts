...
suite("convertPrivateFields", () => {
	test("no # characters — quick bail-out", () => {
		const result = convertPrivateFields(
	...
	test("multiple private fields in one class", () => {
		const code =
			"class Foo { #x = 1; #y = 2; get() { return this.#x + this.#y; } }";
		const result = convertPrivateFields(code, "test.js");
	...
	test("static private field — no clash with inherited public property", () => {
		...
		assert.ok(result.code.includes("$a"));
		assert.ok(
			result.code.includes("data.name"),
			"public property should be preserved",
		);
	});
	...
	test("#field in expr — brand check uses quoted string", () => {
		const code =
			"class Foo { #brand; static check(x) { if (#brand in x) return true; } }";
		const result = convertPrivateFields(code, "test.js");
	...
	test("string #brand in obj is not treated as private field", () => {
		const code =
			'class Foo { #brand = true; isFoo(obj) { return "#brand" in obj; } }';
		const result = convertPrivateFields(code, "test.js");
		assert.ok(
			result.code.includes('"#brand" in obj'),
	...
	test("static async private method — no token fusion", async () => {
		const code =
			"class Foo{static async#init(){return await Promise.resolve(1)}static go(){return Foo.#init()}}";
		const result = convertPrivateFields(code, "test.js");
		assert.doesNotThrow(
			() => new Function(result.code),
	...
	test("heritage clause — extends expression resolves outer private field, not inner", () => {
		...
		assert.ok(
			result.code.includes("this.$a, Object"),
	...
	test("generated name must not collide with existing public property", () => {
		...
		assert.ok(
			!fieldDecls || fieldDecls.length <= 1,
...
suite("adjustSourceMap", () => {
	...
	test("single edit shrinks token - columns after edit shift left", () => {
		...
		gen.addMapping({
			...
			original: { line: 1, column: 14 },
			source: "test.js",
		});
		// Map 'var' at col 19
		gen.addMapping({
			generated: { line: 1, column: 19 },
			original: { line: 1, column: 19 },
			source: "test.js",
		});
		// Map 'y' at col 23
		gen.addMapping({
		...
		assert.strictEqual(
			pos.column,
			23,
			"y should map back to original column 23",
		);
		...
		assert.strictEqual(
			pos2.column,
			14,
			"= should map back to original column 14",
		);
	});
	...
	test("edit on line does not affect other lines", () => {
		...
		const result = adjustSourceMap(map, code, [
			{ start: hashPos, end: hashPos + 2, newText: "$a" },
		]);
	...
	test("multiple edits on same line accumulate shifts", () => {
		...
		gen.addMapping({
			...
			original: { line: 1, column: 24 },
			source: "test.js",
		}); // 'this'
		gen.addMapping({
			generated: { line: 1, column: 29 },
			original: { line: 1, column: 29 },
			source: "test.js",
		}); // '#ccc'
		gen.addMapping({
			generated: { line: 1, column: 33 },
			original: { line: 1, column: 33 },
			source: "test.js",
		}); // ';'
		const map = JSON.parse(gen.toString());
		...
		assert.strictEqual(
			pos.column,
			29,
			"third edit position should map to original column",
		);
		...
		assert.strictEqual(
			pos2.column,
			22,
			"plus after second edit should map correctly",
		);
	});
	...
	test("end-to-end: convertPrivateFields + adjustSourceMap", () => {
		...
		assert.strictEqual(
			pos.column,
			origGetValueCol,
			"getValue column should match original",
		);
	});
