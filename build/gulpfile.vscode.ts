		...
		optimize.bundleTask({
			...
			esm: {
				...
				skipTSBoilerplateRemoval: (entryPoint) =>
					...
					entryPoint === "vs/sessions/electron-browser/sessions",
			},
		}),
	),
);
...
function packageTask(
	...
	const task = () => {
		...
		const versionedResourcesFolder = util.getVersionedResourcesFolder(
			platform,
			commit!,
		);
		...
		const productJsonStream = gulp
			...
			.pipe(
				jsonEditor((json: Record<string, unknown>) => {
					json.commit = commit;
		...
		const copilotRuntimePrebuilds = gulp.src(
			getCopilotRuntimePrebuildFiles(platform, arch),
		...
		if (platform === "win32") {
			...
			const policyDest = gulp
				.src(".build/policies/linux/**", { base: ".build/policies/linux" })
			...
			.pipe(
				filter(
					[
						...
						"!version",
						...(platform === "darwin"
							? ["!**/Contents/Applications", "!**/Contents/Applications/**"]
							: []),
		...
		if (platform === "win32") {
			...
			if (versionedResourcesFolder) {
				result = es.merge(
					result,
					gulp
						.src("resources/win32/versioned/bin/code.cmd", {
							base: "resources/win32/versioned",
						})
						.pipe(replace("@@NAME@@", product.nameShort))
				...
				result = es.merge(
					result,
					gulp
						.src("resources/win32/versioned/bin/code.sh", {
							base: "resources/win32/versioned",
						})
						.pipe(replace("@@NAME@@", product.nameShort))
					...
					gulp
						...
						.pipe(
							replace(
								...
								).win32ContextMenu![arch].clsid,
							),
						)
						.pipe(
							replace(
								"@@FileExplorerContextMenuDLL@@",
								`${quality === "stable" ? "code" : "code_insider"}_explorer_command_${arch}.dll`,
							),
						)
			...
			const esbuildBundleTask = task.define(
				...
				() =>
					runEsbuildBundle(
						...
						true,
						"desktop",
						minified && useCdnSourceMapsForPackagingTasks
							? `${sourceMappingURLBase}/core`
							: undefined,
					),
			);
			vscodeTask = task.define(
