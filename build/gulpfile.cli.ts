...
const compileFromSources = (callback: (err?: string) => void) => {
	const proc = cp.spawn("cargo", ["--color", "always", "build"], {
		...
		env: existsSync(platformOpensslDir)
			? { OPENSSL_DIR: platformOpensslDir, ...process.env }
			: process.env,
	});
...
const compileWithOpenSSLCheck = (
	reporter: import("./lib/reporter.ts").IReporter,
) =>
	es.map((_, callback) => {
		compileFromSources((err) => {
			if (!err) {
				...
				err
					.toString()
					.includes("Could not find directory of OpenSSL installation") &&
...
const watchCliTask = task.define("watch-cli", () => {
	warnIfRustNotInstalled();
	return watcher(`${src}/**`, { read: false }).pipe(
		debounce(compileCliTask as task.StreamTask),
	);
});
