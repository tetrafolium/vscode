	...
	return new Promise((resolve, reject) => {
		...
		proc.on("close", (code) => {
			if (code === 0) {
				...
				reject(
					new Error(
						`esbuild web bundle failed with exit code ${code} (outDir: ${outDir}, minify: ${minify}, nls: ${nls})`,
					),
				);
			}
		});
 ...
 */
export const createVSCodeWebFileContentMapper = (
	extensionsRoot: string,
	product: typeof import("../product.json"),
) => {
...
const bundleVSCodeWebTask = task.define(
	"bundle-vscode-web-OLD",
	task.series(
...
const esbuildBundleVSCodeWebTask = task.define("esbuild-vscode-web", () =>
	runEsbuildBundle("out-vscode-web", false, true),
);
const esbuildBundleVSCodeWebMinTask = task.define(
	"esbuild-vscode-web-min",
	() =>
		runEsbuildBundle(
			"out-vscode-web-min",
			true,
...
function packageTask(sourceFolderName: string, destinationFolderName: string) {
	...
	return () => {
		...
		const packageJsonStream = gulp
			.src(["remote/web/package.json"], { base: "remote/web" })
		...
		const deps = gulp
			...
			.pipe(
				util.cleanNodeModules(path.join(import.meta.dirname, ".webignore")),
			);
...
const compileWebExtensionsBuildTask = task.define(
	...
	task.series(
		...
		task.define("bundle-web-extensions-build", () =>
			extensions
				.packageAllLocalExtensionsStream(true, false)
...
["", "min"].forEach((minified) => {
	...
	const vscodeWebTask = task.define(
		...
		task.series(compileBuildWithManglingTask, vscodeWebTaskCI),
	);
