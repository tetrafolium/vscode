...
const getOverrides = (() => {
	...
	return (fileName: string) => {
		...
		if (!cache.has(rootTsfmtPath)) {
			cache.set(
				rootTsfmtPath,
				JSON.parse(fs.readFileSync(rootTsfmtPath, "utf8")),
			);
		}
...
if (import.meta.main) {
	...
	for (const file of files) {
		...
		if (
			...
			!verifyFormatting(
				absolutePath,
				replace ? fs.readFileSync(absolutePath, "utf8") : text,
			)
		) {
			console.error(
				`File not formatted. Run the 'Format Document' command to fix it: ${file}`,
			);
			errorCount++;
		}
	}
