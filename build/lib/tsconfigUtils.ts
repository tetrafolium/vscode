...
export function getTargetStringFromTsConfig(configFilePath: string): string {
	...
	if (parsed.error) {
		throw new Error(
			`Cannot determine target from ${configFilePath}. TS error: ${parsed.error.messageText}`,
		);
	}
	...
	const resolved =
		typeof cmdLine.options.target !== "undefined"
			? ts.ScriptTarget[cmdLine.options.target]
			: undefined;
	if (!resolved) {
