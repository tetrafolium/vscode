...
export function extractEditor(
	options: tss.ITreeShakingOptions & {
		destRoot: string;
		tsOutDir: string;
		additionalFilesToCopyOut?: string[];
	},
): void {
	...
	if (tsConfig.extends) {
		const extendedConfig = JSON.parse(
			fs
				.readFileSync(path.join(options.sourcesRoot, tsConfig.extends))
				.toString(),
		);
	...
	options.typings = (tsConfig.include as string[]).filter((includedFile) =>
	...
	const writeOutputFile = (fileName: string, contents: string | Buffer) => {
		const relativePath = path.isAbsolute(fileName)
			? path.relative(options.sourcesRoot, fileName)
			: fileName;
	...
	copyFile(
		"../node_modules/@vscode/tree-sitter-wasm/wasm/web-tree-sitter.d.ts",
		"@vscode/tree-sitter-wasm.d.ts",
	);
}
