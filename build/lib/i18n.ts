...
export class XLF {
	...
	static parse = function (xlfString: string): Promise<l10nJsonDetails[]> {
		return new Promise((resolve, reject) => {
			...
			parser.parseString(
				xlfString,
				function (err: Error | undefined, result: any) {
					if (err) {
						reject(
							new Error(
								`XLF parsing error: Failed to parse XLIFF string. ${err}`,
							),
						);
					}
...
function processCoreBundleFormat(
	...
	languages: Language[],
	json: NLSKeysFormat,
	emitter: eventStream.ThroughStream,
) {
...
export function processNlsFiles(opts: {
	...
	fileHeader: string;
	languages: Language[];
}): eventStream.ThroughStream {
	return eventStream.through(function (
		this: eventStream.ThroughStream,
		file: File,
	) {
...
function createL10nBundleForExtension(
	...
	return gulp
		...
		.pipe(
			eventStream.map(function (data, callback) {
...
export function createXlfFilesForExtensions(): eventStream.ThroughStream {
	...
	return eventStream.through(
		function (this: eventStream.ThroughStream, extensionFolder: File) {
								...
								if (basename === "package.nls.json") {
									...
									for (const file in json) {
										...
										for (let i = 0; i < fileContent.messages.length; i++) {
											...
											const { key, comment } = isLocalizeInfo(
												fileContent.keys[i],
											)
...
export function prepareI18nPackFiles(
	...
	return eventStream.through(
		function (this: eventStream.ThroughStream, xlf: File) {
