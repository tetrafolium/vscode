...
export function runEsbuildTranspile(
	outDir: string,
	...
	return new Promise((resolve, reject) => {
		...
		proc.on("close", (code) => {
			if (code === 0) {
				...
				reject(
					new Error(
						`esbuild transpile failed with exit code ${code} (outDir: ${outDir})`,
					),
				);
			}
		});
...
export function runEsbuildBundle(
	...
	minify: boolean,
	nls: boolean,
	target: "desktop" | "server" | "server-web" = "desktop",
	...
	return new Promise((resolve, reject) => {
		...
		proc.on("close", (code) => {
			if (code === 0) {
				...
				reject(
					new Error(
						`esbuild bundle failed with exit code ${code} (outDir: ${outDir}, minify: ${minify}, nls: ${nls}, target: ${target})`,
					),
				);
			}
		});
