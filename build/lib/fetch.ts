			...
			if (response.ok && response.status >= 200 && response.status < 300) {
				...
				if (options.checksumSha256) {
					...
					if (actualSHA256Checksum !== options.checksumSha256) {
						throw new Error(
							`Checksum mismatch for ${ansiColors.cyan(url)} (expected ${options.checksumSha256}, actual ${actualSHA256Checksum}))`,
						);
					} else if (verbose) {
...
export function fetchGithub(
	...
	return fetchUrls(
		`/repos/${repo.replace(/^\/|\/$/g, "")}/releases/tags/v${options.version}`,
		{
