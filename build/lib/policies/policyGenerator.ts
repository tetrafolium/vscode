...
const product: ProductJson = JSON.parse(
	fs.readFileSync(
		path.join(import.meta.dirname, "../../../product.json"),
		"utf8",
	),
);
...
async function parsePolicies(policyDataFile: string): Promise<Policy[]> {
	const contents = JSONC.parse(
		await fs.promises.readFile(policyDataFile, { encoding: "utf8" }),
	) as ExportedPolicyDataDto;
...
async function darwinMain(policies: Policy[], translations: Translations) {
	...
	const { profile, manifests } = renderMacOSPolicy(
		product,
		policies,
		translations,
	);
...
async function linuxMain(policies: Policy[]) {
	...
	const policyFileContents = JSON.stringify(
		renderJsonPolicies(policies),
		undefined,
		4,
	);
...
async function main() {
	...
	if (args._.length !== 2) {
		console.error(
			`Usage: node build/lib/policies <policy-data-file> <darwin|win32|linux>`,
		);
		process.exit(1);
	}
