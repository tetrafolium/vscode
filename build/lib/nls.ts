...
const _nls = (() => {
	const moduleToNLSKeys: {
		[name: string /* module ID */]: ILocalizeKey[]; /* keys */
	} = {};
	const moduleToNLSMessages: {
		[name: string /* module ID */]: string[]; /* messages */
	} = {};
	const allNLSMessages: string[] = [];
	const allNLSModulesAndKeys: Array<
		[string /* module ID */, string[] /* keys */]
	> = [];
	...
	function patchJavascript(patches: IPatch[], contents: string): string {
		...
		lazy(patches)
			.reverse()
			.each((p) => model.apply(p.span, p.content));
	...
	function patch(
		...
		javascript: string,
		sourcemap: sm.RawSourceMap,
		options: { preserveEnglish: boolean },
	): INlsPatchResult {
		...
		const nlsMessages = localizeCalls
			.map((lc) => parseLocalizeKeyOrValue(lc.value) as string)
			.concat(
				localize2Calls.map((lc) => parseLocalizeKeyOrValue(lc.value) as string),
			);
