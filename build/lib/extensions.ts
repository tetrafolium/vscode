...
function fromLocal(
	...
	forWeb: boolean,
	_disableMangle: boolean,
): Stream {
	let esbuildConfigFileName = forWeb ? "esbuild.browser.mts" : "esbuild.mts";

	let hasEsbuild = fs.existsSync(
		path.join(extensionPath, esbuildConfigFileName),
	);
	...
	if (hasEsbuild) {
		...
		input = isStandardEsbuild
			? es.merge(
					...
					...getBuildRootsForExtension(extensionPath).map((root) =>
						typeCheckExtensionStream(root, forWeb),
					),
				)
...
export function typeCheckExtension(
	...
	return spawnTsgo(tsconfigPath, {
		taskName: "typechecking extension (tsgo)",
		noEmit: true,
	});
}

export function typeCheckExtensionStream(
	extensionPath: string,
	forWeb: boolean,
): Stream {
	const tsconfigFileName = forWeb ? "tsconfig.browser.json" : "tsconfig.json";
...
function fromLocalEsbuild(
	extensionPath: string,
	esbuildConfigFileName: string,
): Stream {
	const vsce = require("@vscode/vsce") as typeof import("@vscode/vsce");
	const result = es.through();
	...
	const packagedDependencies =
		packagedDependenciesByExtension[extensionName] ?? [];
	...
	new Promise<void>((resolve, reject) => {
		const proc = cp.execFile(
			...
			(error, _stdout, stderr) => {
				...
				fancyLog(
					`Bundled extension: ${ansiColors.yellow(path.join(path.basename(extensionPath), esbuildConfigFileName))} with ${matches ? matches.length : 0} errors.`,
				);
			...
			if (packagedDependencies.length > 0) {
				const packagedDependencyFileNames = packagedDependencies.flatMap(
					(dependency) =>
						glob
							.sync(
								path.join(extensionPath, "node_modules", dependency, "**"),
								{ nodir: true, dot: true },
							)
				...
				fileNames = Array.from(
					new Set([...fileNames, ...packagedDependencyFileNames]),
				);
			}
...
const webBuiltInExtensions: IExtensionDefinition[] =
	productJson.webBuiltInExtensions || [];
 ...
 * @returns a stream
 */
export function packageNonNativeLocalExtensionsStream(
	forWeb: boolean,
 ...
 * @returns a stream
 */
export function packageNativeLocalExtensionsStream(
	forWeb: boolean,
	disableMangle: boolean,
): Stream {
 ...
 * @returns a stream
 */
export function packageAllLocalExtensionsStream(
	forWeb: boolean,
	disableMangle: boolean,
): Stream {
	return es.merge([
		...
		if (dependenciesSrc.length) {
			result = es.merge(
				...
				gulp
					...
					.pipe(
						util2.cleanNodeModules(path.join(root, "build", ".moduleignore")),
					)
					.pipe(
...
export function packageCopilotExtensionStream(disableMangle: boolean): Stream {
	...
	const localExtensionsStream = minifyExtensionResources(
		fromLocal(extensionPath, false, disableMangle).pipe(
			rename((p) => (p.dirname = `extensions/copilot/${p.dirname}`)),
		),
	);
		...
		for (const extensionFolder of extensionsFolders) {
			...
			const packageNLS = packageNLSPath
				? JSON.parse(
						fs
							.readFileSync(
								path.join(extensionsRoot, extensionFolder, packageNLSPath),
							)
							.toString(),
					)
				: undefined;
...
export async function esbuildExtensions(
	taskName: string,
	isWatch: boolean,
	...
	function reporter(stdError: string, script: string) {
		...
		fancyLog(
			`Finished ${ansiColors.green(taskName)} ${script} with ${matches ? matches.length : 0} errors.`,
		);
		for (const match of matches || []) {
			fancyLog.error(match);
		}
...
const esbuildMediaScripts: { script: string; tsconfig: string }[] = [
	{
		script: "ipynb/esbuild.notebook.mts",
		tsconfig: "ipynb/notebook-src/tsconfig.json",
	},
	{
		script: "markdown-language-features/esbuild.notebook.mts",
		tsconfig: "markdown-language-features/notebook/tsconfig.json",
	},
	{
		script: "markdown-language-features/esbuild.webview.mts",
		tsconfig: "markdown-language-features/preview-src/tsconfig.json",
	},
	{
		script: "markdown-math/esbuild.notebook.mts",
		tsconfig: "markdown-math/notebook/tsconfig.json",
	},
	{
		script: "mermaid-markdown-features/esbuild.webview.mts",
		tsconfig: "mermaid-markdown-features/preview-src/tsconfig.json",
	},
	{
		script: "notebook-renderers/esbuild.notebook.mts",
		tsconfig: "notebook-renderers/tsconfig.json",
	},
	{
		script: "simple-browser/esbuild.webview.mts",
		tsconfig: "simple-browser/preview-src/tsconfig.json",
	},
];

export function buildExtensionMedia(
	isWatch: boolean,
	outputRoot?: string,
): Promise<void> {
	const esbuildTask = esbuildExtensions(
		"esbuilding extension media",
		isWatch,
		esbuildMediaScripts.map(({ script }) => ({
			script: path.join(extensionsPath, script),
			outputRoot: outputRoot
				? path.join(root, outputRoot, path.dirname(script))
				: undefined,
		})),
	);
	...
	const typeCheckTasks = esbuildMediaScripts.map(({ tsconfig }) => {
		...
		const config = {
			taskName: "typechecking extension media (tsgo)",
			noEmit: true,
		};
		if (!isWatch) {
...
function watchTypeCheckExtensionMedia(
	tsconfigPath: string,
	config: { taskName: string; noEmit?: boolean },
): Promise<void> {
...
export function getBuildRootsForExtension(extensionPath: string): string[] {
	...
	if (
		extensionPath.endsWith("css-language-features") ||
		extensionPath.endsWith("html-language-features") ||
		extensionPath.endsWith("json-language-features")
	) {
		return [
