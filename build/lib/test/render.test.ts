...
suite("Render Functions", () => {
	suite("renderADMLString", () => {
		test("should render ADML string without translations", () => {
			...
			assert.strictEqual(
				result,
				'<string id="TestPrefix_test_description">Test description</string>',
			);
		});
	...
	suite("renderProfileString", () => {
		test("should render profile string without translations", () => {
			...
			const result = renderProfileString(
				"ProfilePrefix",
				"testModule",
				nlsString,
			);
	...
	suite("renderADMX", () => {
		...
		const mockPolicy: Policy = {
			...
			renderProfileManifest: () =>
				"<dict><key>pfm_name</key><string>TestPolicy</string></dict>",
			renderJsonValue: () => null,
		};

		test("should render ADMX with correct XML structure", () => {
			const result = renderADMX(
				"VSCode",
				["1.85"],
				[mockCategory],
				[mockPolicy],
		...
		test("should include policy namespaces with regKey", () => {
			...
			assert.ok(
				result.includes(
					'<target prefix="TestApp" namespace="Microsoft.Policies.TestApp"',
				),
			);
		});
		...
		test("should include categories in correct structure", () => {
			...
			assert.ok(
				result.includes(
					`<category displayName="$(string.Category_${mockCategory.name.nlsKey})"`,
				),
			);
			assert.ok(result.includes("</categories>"));
		});
		...
		test("should handle multiple categories", () => {
			const category1: Category = {
				moduleName: "testModule",
				name: { value: "Cat1", nlsKey: "cat1" },
			};
			const category2: Category = {
				moduleName: "testModule",
				name: { value: "Cat2", nlsKey: "cat2" },
			};
		...
		test("should handle multiple policies", () => {
			const policy2: Policy = {
				...
				renderADMLStrings: () => [
					'<string id="TestPolicy2">Test Policy 2</string>',
				],
				renderADMLPresentation: () => '<presentation id="TestPolicy2"/>',
				renderProfile: () => ["<key>TestPolicy2</key>", "<string/>"],
				renderProfileManifest: () =>
					"<dict><key>pfm_name</key><string>TestPolicy2</string></dict>",
				renderJsonValue: () => null,
			};
			const result = renderADMX(
				"VSCode",
	...
	suite("renderADML", () => {
		...
		const mockPolicy: Policy = {
			...
			renderADMLPresentation: () =>
				'<presentation id="TestPolicy"><textBox refId="TestPolicy"/></presentation>',
			renderProfile: () => [],
			renderProfileManifest: () => "",
			renderJsonValue: () => null,
		};
		...
		test("should include application name", () => {
			...
			assert.ok(
				result.includes('<string id="Application">My Application</string>'),
			);
		});
...
<string>${translations?.["testModule"]?.["test.desc"] || "Default Desc"}</string>
		...
		test("should render profile manifest with correct XML structure", () => {
			const result = renderProfileManifest(
				"VS Code",
		...
		test("should include VS Code specific URLs", () => {
			...
			assert.ok(
				result.includes("https://code.visualstudio.com/docs/setup/enterprise"),
			);
		});
...
<plist version="1.0">
	<dict>
		...
		test("should include en-us manifest by default", () => {
			...
			assert.ok(
				result.manifests[0].contents.includes("VS Code Managed Settings"),
			);
		});
			...
			const translations = [
				{
					...
					languageTranslations: {
						testModule: { "test.desc": "Description Française" },
					},
				},
				{
					languageId: "de-de",
					languageTranslations: {
						testModule: { "test.desc": "Deutsche Beschreibung" },
					},
				},
			];
		...
		test("should use provided UUIDs in profile", () => {
			...
			assert.ok(
				result.profile.includes("<string>custom-payload-uuid</string>"),
			);
			assert.ok(result.profile.includes("<string>custom-uuid</string>"));
			assert.ok(
				result.profile.includes("<string>com.example.app.custom-uuid</string>"),
			);
		});
	...
	suite("renderGP", () => {
		...
		test("should include regKey in ADMX", () => {
			...
			assert.ok(
				result.admx.includes("Software\\Policies\\Microsoft\\CustomRegKey"),
			);
		});
			...
			const translations = [
				{
					languageId: "fr-fr",
					languageTranslations: {
						testModule: { "test.policy": "Politique de test" },
					},
				},
				{
					languageId: "de-de",
					languageTranslations: {
						testModule: { "test.policy": "Testrichtlinie" },
					},
				},
			];
	...
	suite("renderJsonPolicies", () => {
		...
		test("should handle object JSON value", () => {
			...
			assert.deepStrictEqual(result, {
				ComplexObjectPolicy: { nested: { value: 123 } },
			});
		});
	});
});
