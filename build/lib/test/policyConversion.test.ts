...
function parsePolicies(policyData: ExportedPolicyDataDto): Policy[] {
	...
	for (const policy of policyData.policies) {
		...
		if (!result) {
			throw new Error(
				`Unsupported policy type: ${policy.type} for policy ${policy.name}`,
			);
		}
	...
	policies.sort((a, b) => {
		const categoryCompare = a.category.name.value.localeCompare(
		...
		},
		{
			...
			localization: {
				description: {
					...
					value:
						"Enables reusable prompt and instruction files in Chat sessions.",
				},
			},
			type: "boolean",
			default: true,
...
Cette fonctionnalité désactive [les protections de sécurité critiques](https://code.visualstudio.com/docs/copilot/security) et facilite considérablement la compromission de la machine par un attaquant.`,
				...
				"extensions.allowed.policy":
					"Spécifiez une liste d’extensions autorisées. Cela permet de maintenir un environnement de développement sécurisé et cohérent en limitant l’utilisation d’extensions non autorisées. Plus d’informations : https://code.visualstudio.com/docs/setup/enterprise#_configure-allowed-extensions",
				...
				"telemetry.feedback.enabled":
					"Activez les mécanismes de commentaires tels que le système de rapport de problèmes, les sondages et autres options de commentaires.",
				"telemetry.telemetryLevel.policyDescription":
					"Contrôle le niveau de télémétrie.",
				...
				"telemetry.telemetryLevel.off":
					"Désactive toutes les données de télémétrie du produit.",
				...
				none: "Aucun",
				manual:
					"Désactivez la recherche de mises à jour automatique en arrière-plan. Les mises à jour sont disponibles si vous les rechercher manuellement.",
				start: "Démarrer",
				default: "Système",
			},
		},
...
suite("Policy E2E conversion", () => {
	test("should render macOS policy profile from policies list", async () => {
		...
		assert.strictEqual(
			result.profile,
			expectedContent,
			"macOS policy profile should match the fixture",
		);
	});
	...
	test("should render macOS manifest from policies list", async () => {
		...
		const normalizeTimestamp = (content: string) =>
			content.replace(/<date>.*?<\/date>/, "<date>TIMESTAMP</date>");
		assert.strictEqual(
	...
	test("should render Windows ADMX from policies list", async () => {
		...
		assert.strictEqual(
			result.admx,
			expectedContent,
			"Windows ADMX should match the fixture",
		);
	});
	...
	test("should render Windows ADML from policies list", async () => {
		...
		assert.strictEqual(
			enUsAdml.contents,
			expectedContent,
			"Windows ADML should match the fixture",
		);
	});

	test("should render macOS manifest with fr-fr locale", async () => {
		const parsedPolicies = parsePolicies(policies);
		const result = renderMacOSPolicy(
			mockProduct,
			parsedPolicies,
			frenchTranslations,
		);
	...
	test("should render Windows ADML with fr-fr locale", async () => {
		...
		assert.strictEqual(
			frFrAdml.contents,
			expectedContent,
			"Windows fr-fr ADML should match the fixture",
		);
	});
	...
	test("should render Linux policy JSON from policies list", async () => {
		...
		assert.deepStrictEqual(
			result,
			expectedJson,
			"Linux policy JSON should match the fixture",
		);
	});

	test("should successfully parse the checked-in policyData.jsonc", async () => {
		const policyDataPath = path.join(
			import.meta.dirname,
			"..",
			"policies",
			"policyData.jsonc",
		...
		assert.strictEqual(
			errors.length,
			0,
			`policyData.jsonc should be valid JSONC: ${JSON.stringify(errors)}`,
		);
		...
		assert.ok(
			parsed.length > 0,
			"Should parse at least one policy from policyData.jsonc",
		);
	});
});
