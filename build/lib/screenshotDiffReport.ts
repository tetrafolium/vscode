...
function readPngPixels(buf: Buffer): RawImage {
	...
	while (offset < buf.length) {
		...
		if (type === "IHDR") {
			...
			if (data[8] !== 8) {
				throw new Error(`Unsupported bit depth: ${data[8]}`);
			}
			colorType = data[9];
...
class ImageDiffResult {
	...
	get isSignificant(): boolean {
		return (
			this.changedPixelCount > MAX_INSIGNIFICANT_PIXELS ||
	...
	await Promise.all(
		changed.map(async (entry) => {
			try {
				...
				console.error(
					`  Warning: pixel diff failed for ${entry.fixtureId}: ${err}`,
				);
			}
		}),
	);
	...
	for (const cur of local.fixtures) {
		...
		const errorMessage =
			errorEvents
				.map((e) => e.message)
				.filter(Boolean)
				.join("; ") || "unknown error (no image hash produced)";
		const errorStack = errorEvents
			.map((e) => e.stack)
			.find((s) => s !== undefined);
		errored.push({
...
function generateMarkdown(
	...
	if (
		...
		if (insignificantChangedCount > 0) {
			console.error(
				`All ${insignificantChangedCount} changed screenshot(s) are insignificant — suppressing PR comment.`,
			);
		}
		return "";
	}
	...
	if (baseSha) {
		lines.push(
			`**Base:** \`${baseSha.slice(0, 8)}\` **Current:** \`${currentSha.slice(0, 8)}\``,
		);
	} else {
		lines.push(
			`**Current:** \`${currentSha.slice(0, 8)}\` _(base manifest unavailable — only rendering errors are reported)_`,
		);
	}
	lines.push("");
	...
	if (significantChanged.length > 0 || insignificantChangedCount > 0) {
		if (significantChanged.length > 0) {
			...
			for (let i = 0; i < significantChanged.length; i++) {
				...
				lines.push(
					`<details${open}><summary><code>${entry.fixtureId}</code></summary>`,
				);
				lines.push("");
				lines.push("| Before | After |");
				lines.push("|--------|-------|");
				lines.push(
					`| ![before](${loadImageUrl(serviceUrl, entry.beforeHash)}) | ![after](${loadImageUrl(serviceUrl, entry.afterHash)}) |`,
				);
				lines.push("");
				lines.push("</details>");
				lines.push("");
		...
		if (insignificantChangedCount > 0) {
			lines.push(
				`_${insignificantChangedCount} insignificant change(s) omitted (\u2264${MAX_INSIGNIFICANT_PIXELS} px, \u0394\u2264${MAX_INSIGNIFICANT_CHANNEL_DELTA}). See CI logs for details._`,
			);
			lines.push("");
		}
	}
	...
	if (errored.length > 0) {
		...
		for (let i = 0; i < errored.length; i++) {
			...
			const headerMessage =
				entry.errorMessage
					.split("\n")
					.map((l) => l.trim())
					.find((l) => l.length > 0) ?? entry.errorMessage;
			...
			const minimalBudget =
				remainingBudget - Buffer.byteLength(minimalBlock, "utf8");
			if (minimalBudget < 0) {
				truncatedAt = i;
				break;
			}
		...
		if (truncatedAt !== -1) {
			lines.push(
				`_…and ${errored.length - truncatedAt} more errored fixture(s) omitted to stay under the ${Math.round(MAX_BODY_BYTES / 1024)}KB body limit. See workflow logs for the full list._`,
			);
			lines.push("");
		}
	}
...
async function main(): Promise<void> {
	...
	if (!serviceUrl || !currentSha || !localManifestPath) {
		console.error(
			'Usage: node build/lib/screenshotDiffReport.ts <service-url> <base-sha|""> <current-sha> <base-manifest|""> <local-manifest>',
		);
		process.exit(1);
	}
	...
	const baseManifestPath =
		baseManifestPathArg && fs.existsSync(baseManifestPathArg)
			? baseManifestPathArg
			: null;
	if (baseManifestPathArg && !baseManifestPath) {
		console.error(
			`Base manifest not found: ${baseManifestPathArg}. Reporting fixture errors only.`,
		);
	}
	...
	const base = baseManifestPath
		? (JSON.parse(
				fs.readFileSync(baseManifestPath, "utf8"),
			) as BaseCommitResponse)
		: null;

	const diff = diffManifests(local, base);
	console.error(
		`Compare result: ${diff.changed.length} changed, ${diff.added.length} added, ${diff.removed.length} removed, ${diff.errored.length} errored.`,
	);

	const localManifestDir = path.dirname(path.resolve(localManifestPath));
	const pixelDiffs = await computePixelDiffs(
		diff.changed,
		serviceUrl,
		localManifestDir,
	);
	for (const entry of diff.changed) {
		...
		console.error(
			`  ${entry.fixtureId}: ${pd?.toString() ?? "pixel diff unavailable"}`,
		);
	}
