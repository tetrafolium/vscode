...
export function spawnTsgo(
	...
	function runReporter(output: string) {
		...
		fancyLog(
			`Finished ${ansiColors.green(config.taskName)} ${projectPath} with ${errorLines.length} errors.`,
		);
	...
	return new Promise<void>((resolve, reject) => {
		child.on("exit", (code) => {
			...
			const lines = allOutput
				...
				.filter(
					(line) =>
						!/Starting compilation|File change detected|Compilation complete/i.test(
							line,
						),
				);
...
export function createTsgoStream(
	projectPath: string,
	config: { taskName: string; noEmit?: boolean },
	onComplete?: () => Promise<void> | void,
	...
	spawnTsgo(projectPath, config, onComplete)
		.then(() => {
			stream.emit("end");
		})
		.catch((err) => {
