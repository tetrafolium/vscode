...
(globalThis as { vscodeServerStartTime?: number }).vscodeServerStartTime =
	performance.now();
...
if (shouldSpawnCli) {
	...
	server.listen(nodeListenOptions, async () => {
		...
		(globalThis as { vscodeServerListenTime?: number }).vscodeServerListenTime =
			performance.now();
...
async function loadCode(nlsConfiguration: INLSConfiguration) {
	...
	if (process.env["VSCODE_DEV"]) {
		...
		process.env["VSCODE_DEV_INJECT_NODE_MODULE_LOOKUP_PATH"] =
			process.env["VSCODE_DEV_INJECT_NODE_MODULE_LOOKUP_PATH"] ||
			path.join(import.meta.dirname, "..", "remote", "node_modules");
		devInjectNodeModuleLookupPath(
			process.env["VSCODE_DEV_INJECT_NODE_MODULE_LOOKUP_PATH"],
		);
	} else {
