							...
							class CommandCenterQuickPickItem extends BaseActionViewItem {
								...
								override render(container: HTMLElement): void {
									...
									const aiFeaturesDisabled =
										that._configurationService.getValue<boolean>(
											AI_DISABLED_SETTING,
										) === true;
									const aiCustomizationsDisabled =
										that._configurationService.getValue<boolean>(
											"disableAICustomizations",
										) === true ||
										that._configurationService.getValue<boolean>(
											"workbench.disableAICustomizations",
										) === true;
									const forcedHidden =
										aiFeaturesDisabled && aiCustomizationsDisabled;
									const agentControlValue = that._configurationService.getValue(
										AGENT_STATUS_ENABLED_SETTING,
									);
									const isCompactMode =
										!forcedHidden &&
										(agentControlValue === true ||
											agentControlValue === undefined ||
											agentControlValue === "compact");
