...
export class WindowTitle extends Disposable {
	...
	getWindowTitle(): string {
		...
		if (this.contextService.getWorkbenchState() === WorkbenchState.FOLDER) {
			...
			folder =
				this.contextService.getWorkspaceFolder(editorResource) ?? undefined;
		}
		...
		const dirty =
			editor?.isDirty() && !editor.isSaving() ? WindowTitle.TITLE_DIRTY : "";
	...
	isCustomTitleFormat(): boolean {
		...
		return (
			title.defaultValue !==
			configurationProperties[WindowSettingNames.title]?.defaultDefaultValue
		);
	}
}
