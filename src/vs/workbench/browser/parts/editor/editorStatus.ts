...
class TabFocusMode extends Disposable {
	...
	constructor(
		...
		const tabFocusModeConfig =
			configurationService.getValue<boolean>("editor.tabFocusMode") === true;
...
class EditorStatus extends Disposable {
	...
	private readonly inputModeElement = this._register(
		new MutableDisposable<IStatusbarEntryAccessor>(),
	);
			...
			Event.runAndSubscribe(this.tabFocusMode.onDidChange, (tabFocusMode) => {
				if (tabFocusMode !== undefined) {
					...
					this.onTabFocusModeChange(
						this.configurationService.getValue("editor.tabFocusMode"),
					);
				}
			}),
		);
		this._register(
			Event.runAndSubscribe(this.inputMode.onDidChange, (inputMode) =>
	...
	private updateInputModeElement(
		inputMode: "overtype" | "insert" | undefined,
	): void {
							...
							for (const change of inner.changes) {
								if (
									selections.some((selection) =>
										Range.areIntersecting(selection, change.range),
									)
								) {
									this.onSelectionChange(activeCodeEditor);
									break;
	...
	private onLanguageChange(
		...
		if (
			...
			if (textModel) {
				...
				info.languageId =
					this.languageService.getLanguageName(languageId) ?? undefined;
			}
		}
	...
	implements IWorkbenchContribution
{
	...
	private createEditorStatus(part: IEditorPart): void {
		...
		disposables.add(
			scopedInstantiationService.createInstance(EditorStatus, part.windowId),
		);
	}
}
...
class ShowCurrentMarkerInStatusbarContribution extends Disposable {
	...
	private hasToUpdateStatus(
		...
		return (
			IMarkerData.makeKey(previousMarker) !== IMarkerData.makeKey(currentMarker)
		);
	}
	...
	private getMarker(): IMarker | null {
		...
		return (
			this.markers.find((marker) => Range.containsPosition(marker, position)) ||
			null
		);
	}
...
export class ChangeLanguageAction extends Action2 {
	...
	private configureFileAssociation(
		...
		const currentAssociation =
			languageService.guessLanguageIdByFilepathOrFirstLine(URI.file(base));
			...
			if (language) {
				...
				if (
					fileAssociationsConfig.workspaceValue?.[
						associationKey as keyof typeof fileAssociationsConfig.workspaceValue
					]
				) {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (typeof encoding.id !== "undefined" && activeEncodingSupport) {
			...
			if (
				...
				await editorService.activeEditorPane.input.revert(
