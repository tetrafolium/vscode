	...
	implements IEditorTabsControl
{
	...
	private readonly editorLayoutActionsToolbarDisposables = this._register(
		new DisposableStore(),
	);
	private readonly editorLayoutActionsDisposables = this._register(
		new DisposableStore(),
	);
	...
	protected createEditorActionsToolBar(
		...
		this.handleEditorLayoutActionsToolBarVisibility(
	...
	private doCreateEditorLayoutActionsToolBar(container: HTMLElement): void {
		...
		this.editorLayoutActionsToolbar =
			this.editorLayoutActionsToolbarDisposables.add(
				this.instantiationService.createInstance(WorkbenchToolBar, container, {
					actionViewItemProvider: (action, options) =>
						this.actionViewItemProvider(action, options),
					orientation: ActionsOrientation.HORIZONTAL,
					...
					getKeyBinding: (action) => this.getKeybinding(action),
					actionRunner: this.editorLayoutActionsToolbarDisposables.add(
						new EditorCommandsContextActionRunner(context),
					),
					anchorAlignmentProvider: () => AnchorAlignment.RIGHT,
					renderDropdownAsChildElement: this.renderDropdownAsChildElement,
		...
		// Action Run Handling
		this.editorLayoutActionsToolbarDisposables.add(
			this.editorLayoutActionsToolbar.actionRunner.onDidRun((e) => {
				// Notify for Error
				if (e.error && !isCancellationError(e.error)) {
					this.notificationService.error(e.error);
				}
	...
	private updateEditorLayoutActionsToolbar(): void {
		...
		const editorActions = this.groupView.createEditorActions(
			this.editorLayoutActionsDisposables,
			MenuId.EditorTitleLayout,
		);
		this.editorLayoutActionsDisposables.add(
			editorActions.onDidChange(() => this.updateEditorLayoutActionsToolbar()),
		);

		const { primary, secondary } = this.prepareEditorLayoutActions(
			editorActions.actions,
		);
	...
	protected abstract prepareEditorLayoutActions(
		editorActions: IToolbarActions,
	): IToolbarActions;
