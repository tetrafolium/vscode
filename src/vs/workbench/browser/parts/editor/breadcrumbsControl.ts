...
class FileItem extends BreadcrumbsItem {
	...
	render(container: HTMLElement): void {
		...
		label.setFile(this.element.uri, {
			hidePath: true,
			hideIcon:
				this.element.kind === FileKind.FOLDER || !this.options.showFileIcons,
...
export class BreadcrumbsControl {
	...
	>;
	private readonly _cfTitleScrollbarVisibility: BreadcrumbsConfig<
		IEditorPartOptions["titleScrollbarVisibility"]
	>;
	...
	constructor(
		...
		this._cfTitleScrollbarVisibility =
			BreadcrumbsConfig.TitleScrollbarVisibility.bindTo(configurationService);
	...
	update(): boolean {
		...
		const updateScrollbarSizing = () => {
			...
			this._widget.setHorizontalScrollbarVisibility(
				BreadcrumbsControl.SCROLLBAR_VISIBILITY[visibility],
			);
		};
		...
		const updateScrollbarVisibilityListener =
			this._cfTitleScrollbarVisibility.onDidChange(updateScrollbarSizing);
			...
			getAnchor: () => {
				if (!pickerAnchor) {
					...
					if (y + maxHeight >= window.innerHeight) {
						maxHeight =
							window.innerHeight - y - 30 /* room for shadow and status bar*/;
					}
					let x = data.left;
...
export class BreadcrumbsControlFactory {
	...
	private createControl(): BreadcrumbsControl {
		...
		this._controlDisposables.add(
			control.onDidVisibilityChange(() => this._onDidVisibilityChange.fire()),
		);
...
registerAction2(
	class CopyBreadcrumbPath extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const elements =
				outline.config.breadcrumbsDataSource.getBreadcrumbElements();
			...
			const config =
				BreadcrumbsConfig.SymbolPathSeparator.bindTo(configurationService);
