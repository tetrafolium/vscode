...
export class MultiEditorTabsControl extends EditorTabsControl {
	...
	private updateTabSizing(fromEvent: boolean): void {
		const [tabsContainer, tabSizingFixedDisposables] = assertReturnsAllDefined(
			this.tabsContainer,
	...
	private registerTabsContainerListeners(
		...
		this._register(
			addDisposableListener(tabsContainer, TouchEventType.Contextmenu, (e) =>
				showContextMenu(e),
			),
		);
		this._register(
	...
	private handleOpenedEditors(): boolean {
		...
		const [tabsContainer, tabsScrollbar] = assertReturnsAllDefined(
			this.tabsContainer,
			this.tabsScrollbar,
		);
		...
		if (
			...
			oldTabLabels.some(
				(label, index) =>
					!this.equalsEditorInputLabel(label, this.tabLabels.at(index)),
			) // editor labels changed
		) {
	...
	override updateOptions(
		...
		if (
			oldOptions.titleScrollbarVisibility !==
			newOptions.titleScrollbarVisibility
		) {
		...
		if (
			oldOptions.alwaysShowEditorActions !== newOptions.alwaysShowEditorActions
		) {
			this.updateEditorActionsToolbar();
		}
	...
	private toEditorIndex(tabIndex: number): number {
		...
		const editor = assertReturnsDefined(
	...
	private registerTabListeners(
			...
			if (editor) {
				if (e.shiftKey) {
					...
					if (
						...
						const activeEditor = assertReturnsDefined(
		...
		disposables.add(
			addDisposableListener(tab, TouchEventType.Change, (e: GestureEvent) => {
				tabsScrollbar.setScrollPosition({
					scrollLeft:
						tabsScrollbar.getScrollPosition().scrollLeft - e.translationX,
				});
			}),
		);
		...
		disposables.add(
			new DragAndDropObserver(tab, {
				...
				onDragOver: (e, dragDuration) => {
					if (
						dragDuration >= MultiEditorTabsControl.DRAG_OVER_OPEN_TAB_THRESHOLD
					) {
	...
	private redrawTabLabel(
		...
		if (
			...
			name = tabLabel.name;
			namePrefix = options.showTabIndex
				? `${this.toEditorIndex(tabIndex) + 1}: `
				: undefined;
		...
		tabLabelWidget.setResource(
			...
			},
			{
				...
				extraClasses: coalesce(
					[
						"tab-label",
						fileDecorationBadges ? "tab-label-has-badge" : undefined,
	...
	protected override prepareEditorLayoutActions(
		editorActions: IToolbarActions,
	): IToolbarActions {
		return editorActions;
	}
	...
	private doLayoutTabsWrapping(
		...
		const editorToolbarWidth = () =>
			editorToolbarContainer.offsetWidth +
			(layoutActionsContainer?.offsetWidth ?? 0);
	...
	private updateTabsControlVisibility(): void {
		const tabsAndActionsContainer = assertReturnsDefined(
			this.tabsAndActionsContainer,
		);
...
registerThemingParticipant((theme, collector) => {
	...
	if (!isHighContrast(theme.type) && !isSafari && !activeContrastBorderColor) {
		...
		if (
			...
			collector.addRule(
				makeTabHoverBackgroundRule(adjustedColor, adjustedColorDrag, true),
			);
		}
		...
		if (
			...
			collector.addRule(
				makeTabBackgroundRule(adjustedColor, adjustedColorDrag, true, true),
			);
		}
		...
		if (
			...
			collector.addRule(
				makeTabBackgroundRule(adjustedColor, adjustedColorDrag, false, true),
			);
		}
		...
		if (
			...
			collector.addRule(
				makeTabBackgroundRule(adjustedColor, adjustedColorDrag, true, false),
			);
		}
		...
		if (
			...
			collector.addRule(
				makeTabBackgroundRule(adjustedColor, adjustedColorDrag, false, false),
			);
		}
	}
});
