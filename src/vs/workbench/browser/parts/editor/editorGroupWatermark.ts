...
const showChatContextKey = ContextKeyExpr.and(
...
const openChat: WatermarkEntry = {
	...
	when: { native: showChatContextKey, web: showChatContextKey },
};
...
const baseEntries: WatermarkEntry[] = [openChat, showCommands];
...
export class EditorGroupWatermark extends Disposable {
				...
				if (
					...
					this.enabled !==
						this.configurationService.getValue<boolean>(
							EditorGroupWatermark.SETTINGS_KEY,
						)
				) {
					this.render();
				}
	...
	private render(): void {
		...
		if (entries.length < EditorGroupWatermark.MINIMUM_ENTRIES) {
			...
			entries.push(
				...additionalEntries.slice(
					0,
					EditorGroupWatermark.MINIMUM_ENTRIES - entries.length,
				),
			);
		}
	...
	private filterEntries(entries: WatermarkEntry[]): WatermarkEntry[] {
		const filteredEntries = entries
			.filter((entry) => {
				...
				return (
					!contextKey /* works without context */ ||
					this.contextKeyService.contextMatchesRules(contextKey)
				);
			})
