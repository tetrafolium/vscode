...
class FileVirtualDelegate implements IListVirtualDelegate<
	IFileStat | IWorkspaceFolder
> {
...
class FileIdentityProvider implements IIdentityProvider<
	IWorkspace | IWorkspaceFolder | IFileStat | URI
> {
...
class FileDataSource implements IAsyncDataSource<
	IWorkspace | URI,
...
class FileRenderer implements ITreeRenderer<
	IFileStat | IWorkspaceFolder,
	FuzzyScore,
	IResourceLabel
> {
...
class FileNavigationLabelProvider implements IKeyboardNavigationLabelProvider<
	IWorkspaceFolder | IFileStat
> {
...
class FileAccessibilityProvider implements IListAccessibilityProvider<
	IWorkspaceFolder | IFileStat
> {
		...
		const update = () => {
			_workspaceService.getWorkspace().folders.forEach((folder) => {
				...
				this._cachedExpressions.set(
					folder.uri.toString(),
					glob.parse(adjustedConfig, { ignoreCase }),
				);
			});
		};
		update();
