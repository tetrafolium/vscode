...
export const MOVE_EDITOR_INTO_ABOVE_GROUP =
	"workbench.action.moveEditorToAboveGroup";
export const MOVE_EDITOR_INTO_BELOW_GROUP =
	"workbench.action.moveEditorToBelowGroup";
export const MOVE_EDITOR_INTO_LEFT_GROUP =
	"workbench.action.moveEditorToLeftGroup";
export const MOVE_EDITOR_INTO_RIGHT_GROUP =
...
export const CLOSE_MODAL_EDITOR_COMMAND_ID =
	"workbench.action.closeModalEditor";
export const MOVE_MODAL_EDITOR_TO_MAIN_COMMAND_ID =
	"workbench.action.moveModalEditorToMain";
export const MOVE_MODAL_EDITOR_TO_WINDOW_COMMAND_ID =
	"workbench.action.moveModalEditorToWindow";
export const TOGGLE_MODAL_EDITOR_MAXIMIZED_COMMAND_ID =
	"workbench.action.toggleModalEditorMaximized";
export const NAVIGATE_MODAL_EDITOR_PREVIOUS_COMMAND_ID =
	"workbench.action.navigateModalEditorPrevious";
export const NAVIGATE_MODAL_EDITOR_NEXT_COMMAND_ID =
	"workbench.action.navigateModalEditorNext";
export const TOGGLE_MODAL_EDITOR_SIDEBAR_COMMAND_ID =
	"workbench.action.toggleModalEditorSidebar";
...
const isSelectedEditorsMoveCopyArg = function (
	arg: SelectedEditorsMoveCopyArguments,
): boolean {
	if (!isObject(arg)) {
		return false;
	}
...
function registerEditorMoveCopyCommand(): void {
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		handler: (accessor, args) =>
			moveCopySelectedEditors(
				true,
				args as SelectedEditorsMoveCopyArguments | undefined,
				accessor,
			),
		metadata: {
	...
	function moveTabs(
		...
		if (to === "first" || to === "right") {
			...
			to === "position" &&
			(args.value ?? 1) < group.getIndexOfEditor(editors[0])
		) {
			editors = [...editors].reverse();
		}
...
function registerOpenEditorAPICommands(): void {
	...
	CommandsRegistry.registerCommand(
		...
		async function (
			...
			if (
				...
				if (
					untitledTextEditorService.isUntitledWithAssociatedResource(resource)
				) {
	...
	CommandsRegistry.registerCommand(
		...
		async (
			...
			const revealUri = options.reveal?.modifiedUri
				? URI.revive(options.reveal.modifiedUri)
				: undefined;
			const revealResource =
				revealUri && resources
					? resources.find((r) => isEqual(r.modified.resource, revealUri))
					: undefined;
	...
	async function reopenEditorWith(
		...
		for (const { group, editors } of resolvedContext.groupedEditors) {
			for (const editor of editors) {
				...
				const untypedEditor = isDiffEditor
					? editor.toUntyped()
					: editorToResolve.toUntyped();
				if (!untypedEditor) {
				...
				let forceReplaceDirty =
					!!resource &&
					(resource.scheme === Schemas.untitled ||
						textFileService.isDirty(resource));
				if (forceReplaceDirty && editorToResolve.isDirty()) {
					for (const workingCopy of workingCopyService.dirtyWorkingCopies) {
						...
						if (
							workingCopyEditorService.findEditor(workingCopy)?.editor ===
							editorToResolve
						) {
				...
				>("workbenchEditorReopen", {
					...
					ext: editorToResolve.resource
						? extname(editorToResolve.resource)
						: "",
		...
		class extends Action2 {
			constructor() {
				super({
					...
					title: localize2(
						"toggleModalEditorSidebar",
						"Toggle Modal Editor Sidebar",
					),
					category: Categories.View,
					f1: true,
