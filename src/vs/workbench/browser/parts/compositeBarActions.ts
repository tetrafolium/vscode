...
export class CompositeBarActionViewItem extends BaseActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		const role =
			this.options.isTabList || !this.options.hasPopup ? "tab" : "button";
		...
		if (badges.length > 0 && shouldRenderBadges) {
			...
			// Number
				...
				if (total > 0) {
					...
					if (total > 999) {
						...
						badgeNumber =
							noOfThousands > floor ? `${floor}K+` : `${noOfThousands}K`;
					}
			...
			// Icon
				...
				const badgeContentClassess = [
					"icon-overlay",
					...ThemeIcon.asClassNameArray((badges[0] as IconBadge).icon),
				];
				this.badgeContent.classList.add(...badgeContentClassess);
				this.badgeDisposable.value.add(
					toDisposable(() =>
						this.badgeContent?.classList.remove(...badgeContentClassess),
					),
				);
				show(this.badge);
			}
...
export class CompositeActionViewItem extends CompositeBarActionViewItem {
	...
	private showContextMenu(container: HTMLElement): void {
		...
		const compositeContextMenuActions =
			this.compositeContextMenuActionsProvider(this.compositeBarActionItem.id);
...
export class SwitchCompositeViewAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const visibleCompositeIds = paneCompositeService.getVisiblePaneCompositeIds(
			this.location,
		);
		for (let i = 0; i < visibleCompositeIds.length; i++) {
			if (visibleCompositeIds[i] === activeComposite.getId()) {
				targetCompositeId =
					visibleCompositeIds[
						(i + visibleCompositeIds.length + this.offset) %
							visibleCompositeIds.length
					];
				break;
			}
		}
