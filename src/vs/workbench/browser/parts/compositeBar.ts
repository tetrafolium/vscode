	...
	drop(
		...
		if (dragData.type === "composite") {
			...
			const currentLocation =
				this.viewDescriptorService.getViewContainerLocation(currentContainer);
			let moved = false;
		...
		if (dragData.type === "view") {
			...
			if (viewToMove.canMoveView) {
				...
				const newContainer =
					this.viewDescriptorService.getViewContainerByViewId(viewToMove.id)!;
...
export class CompositeBar extends Widget implements ICompositeBar {
	...
	private compositeOverflowAction = this._register(
		new MutableDisposable<CompositeOverflowActivityAction>(),
	);
	...
	create(parent: HTMLElement): HTMLElement {
				...
				actionViewItemProvider: (action, options) => {
					...
					return (
						...
						this.instantiationService.createInstance(
							...
							(compositeId) =>
								this.options.getContextMenuActionsForComposite(compositeId),
		...
		this._register(
			CompositeDragAndDropObserver.INSTANCE.registerTarget(parent, dndCallback),
		);
	...
	private computeSizes(items: ICompositeBarModelItem[]): void {
		...
		if (size) {
			...
			if (
				...
				items.forEach(() =>
					compositeSwitcherBar.pull(compositeSwitcherBar.viewItems.length - 1),
				);
			}
		}
	}
	...
	private updateCompositeSwitcher(donotTrigger?: boolean): void {
		...
		if (
			...
			compositesToShow.every(
				(compositeId) =>
					!!this.model.activeItem && compositeId !== this.model.activeItem.id,
			)
		) {
		...
		if (
			...
			this.compositeOverflowActionViewItem.value =
				this.instantiationService.createInstance(
					CompositeOverflowActivityActionViewItem,
					this.compositeOverflowAction.value,
...
class CompositeBarModel {
	...
	setItems(items: ICompositeBarItem[]): void {
		...
		this._items = items.map((i) =>
			this.createCompositeBarItem(i.id, i.name, i.order, i.pinned, i.visible),
		);
	}
