...
export class TreeViewPane extends ViewPane {
	...
	constructor(
		...
		this._actionRunner = this._register(
			new MultipleSelectionActionRunner(notificationService, () =>
...
abstract class AbstractTreeView extends Disposable implements ITreeView {
		...
		if (dataProvider) {
			...
			this._dataProvider = new (class implements ITreeViewDataProvider {
				...
				private updateEmptyState(
					nodes: ITreeItem[],
					childrenGroups: (readonly ITreeItem[])[],
				): void {
				...
				async getChildrenBatch(
					nodes?: ITreeItem[],
				): Promise<(readonly ITreeItem[])[]> {
	...
	protected createTree() {
		...
		const aligner = this.treeDisposables.add(
			new Aligner(this.themeService, this.logService),
		);
				...
				dataSource,
				{
					...
					keyboardNavigationLabelProvider: {
						getKeyboardNavigationLabel: (item: ITreeItem) => {
							if (item.label) {
								return isMarkdownString(item.label.label)
	...
	implements ITreeRenderer<ITreeItem, FuzzyScore, ITreeExplorerTemplateData>
{
			...
			if (resource && !node.tooltip) {
				...
				return {
					...
					markdownNotSupportedFallback: resource
						? undefined
						: renderAsPlaintext(node.tooltip),
			...
			for (const syntax of syntaxes) {
				if (text.startsWith(syntax.open) && text.endsWith(syntax.close)) {
					...
					if (
						matches?.some(
							(match) =>
								match.start < syntax.open.length ||
								match.end > text.length - syntax.close.length,
						)
					) {
						return false;
					}
					...
					text = text.substring(
						syntax.open.length,
						text.length - syntax.close.length,
					);
	...
	renderElement(
		...
		const matches =
			treeItemLabel?.highlights && labelStr
				...
				: undefined;
		const { label, bold, italic, strikethrough, supportIcons } =
	...
	private onDidChangeContext(e: IContextKeyChangeEvent) {
		const affectsEntireMenuContexts = e.affectsSome(
			this.menus.getEntireMenuContexts(),
		);
		...
		if (this._tree) {
			...
			try {
				...
				this.logService.error(
					`[TreeView] Failed to resolve parent for ${treeItem.handle}`,
					error,
				);
				return false;
			}
...
export class CustomTreeViewDragAndDrop implements ITreeDragAndDrop<ITreeItem> {
	...
	getDragLabel?(elements: ITreeItem[]): string | undefined {
		...
		if (element.label) {
			return isMarkdownString(element.label.label)
				? element.label.label.value
				: element.label.label;
		}
		return element.resourceUri
