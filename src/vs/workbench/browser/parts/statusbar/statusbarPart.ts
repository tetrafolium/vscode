...
class StatusbarPart extends Part implements IStatusbarEntryContainer {
	...
	private registerListeners(): void {
		...
		this._register(
			this.contextService.onDidChangeWorkbenchState(() => this.updateStyles()),
		);
	}
	...
	private doAddEntry(
		...
		// View model entry
		const viewModelEntry: IStatusbarViewModelEntry =
			new (class implements IStatusbarViewModelEntry {
				readonly id = id;
...
export interface IAuxiliaryStatusbarPart
	extends IStatusbarEntryContainer, IView {
	...
	implements IStatusbarService
{
	...
	createAuxiliaryStatusbarPart(
		...
		// Statusbar Part
		const statusbarPart = instantiationService.createInstance(
	...
	implements IStatusbarService
{
	...
	createAuxiliaryStatusbarPart(
		...
		return this.statusbarService.createAuxiliaryStatusbarPart(
			container,
			instantiationService,
		);
	}
