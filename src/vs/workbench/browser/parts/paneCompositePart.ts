...
import {
	CompositePart,
	ICompositePartOptions,
	ICompositeTitleLabel,
} from "./compositePart.js";
import {
	...
	implements IPaneCompositePart
{
	...
	private blockOpening: DeferredPromise<PaneComposite | undefined> | undefined =
		undefined;
	...
	private createEmptyPaneMessage(parent: HTMLElement): void {
		...
		messageElement.textContent = localize(
			"pane.emptyMessage",
		...
		const setDropBackgroundFeedback = (visible: boolean) => {
			const updateActivityBarBackground =
				!this.getActiveComposite() || !visible;
			...
			if (
				...
				updateActivityBarBackground
			) {
				this.headerFooterCompositeBarContainer.style.backgroundColor =
					backgroundColor;
			}
						...
						if (this.paneCompositeBar.value) {
							...
							if (dragData.type === "composite") {
								...
								if (viewToMove.canMoveView) {
									...
									this.openPaneComposite(newContainer.id, true).then(
										(composite) => {
	...
	protected override createTitleArea(
		parent: HTMLElement,
	...
	protected override createTitleLabel(
		...
		this.titleLabelElement!.draggable =
			this.viewDescriptorService.canMoveViews();
	...
	private async doOpenPaneComposite(
		id: string,
		focus?: boolean,
	): Promise<PaneComposite | undefined> {
	...
	private layoutCompositeBar(): void {
		if (
			...
			const padding =
				this.compositeBarPosition === CompositeBarPosition.TITLE ? 16 : 8;
