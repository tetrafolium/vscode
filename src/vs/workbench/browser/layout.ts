	...
	implements IWorkbenchLayoutService
{
	...
	private getContainerFromDocument(targetDocument: Document): HTMLElement {
		if (targetDocument === this.mainContainer.ownerDocument) {
			...
			return targetDocument.body.getElementsByClassName(
				"monaco-workbench",
			)[0] as HTMLElement; // auxiliary window
		}
	}
	...
	private registerLayoutListeners(): void {
		...
		this.editorGroupService.whenRestored.then(() => {
			...
			this._register(
				this.mainPartEditorService.onDidVisibleEditorsChange((e) => {
				...
				if (
					...
					const enabledCommandCenterAction = COMMAND_CENTER_SETTINGS.some(
						(setting) =>
							...
							this.configurationService.getValue<boolean>(setting) === true,
					);
		...
		// Theme changes
		this._register(
			this.themeService.onDidColorThemeChange(() => this.updateWindowBorder()),
		);
		...
		if (
			isWeb &&
			typeof (navigator as { windowControlsOverlay?: EventTarget })
				.windowControlsOverlay === "object"
		) {
			this._register(
				addDisposableListener(
					(navigator as unknown as { windowControlsOverlay: EventTarget })
	...
	private isShadowsDisabled(): boolean {
		return (
			this.configurationService.getValue<boolean>(LayoutSettings.SHADOWS) ===
			false
		);
	}
	...
	private shouldRestoreEditors(
		...
		if (
			this.configurationService.getValue<boolean>(
				WorkbenchLayoutSettings.EDITOR_RESTORE_EDITORS,
			) === false
		) {
			return false;
		}
		...
		this._register(
			Event.any(
				...
				this.stateModel.setInitializationValue(
					...
					this.paneCompositeService.getPaneCompositeIds(
						ViewContainerLocation.AuxiliaryBar,
					).length === 0,
				);
			}),
	...
	isAuxiliaryBarMaximized(): boolean {
		return this.stateModel.getRuntimeValue(
			LayoutStateKeys.AUXILIARYBAR_WAS_LAST_MAXIMIZED,
		);
	}
	...
	setAuxiliaryBarMaximized(maximized: boolean): boolean {
		...
		if (maximized) {
			...
			try {
				...
				const size = this.workbenchGrid.getViewSize(
					this.auxiliaryBarPartView,
				).width;
				this.stateModel.setRuntimeValue(
			...
			try {
				...
				this.workbenchGrid.resizeView(this.auxiliaryBarPartView, {
					width: this.stateModel.getRuntimeValue(
						LayoutStateKeys.AUXILIARYBAR_LAST_NON_MAXIMIZED_SIZE,
					),
					height: size.height,
				});
	...
	private panelOpensMaximized(): boolean {
		...
		return (
			...
			(panelOpensMaximized === PartOpensMaximizedOptions.REMEMBER_LAST &&
	...
	private createGridDescriptor(): ISerializedGrid {
		...
		const auxiliaryBarSize = this.stateModel.getInitializationValue(
			LayoutStateKeys.AUXILIARYBAR_SIZE,
		);
		const panelSize = this.stateModel.getInitializationValue(
			LayoutStateKeys.PANEL_SIZE,
...
abstract class WorkbenchLayoutStateKey<
...
class LayoutStateModel extends Disposable {
	...
	constructor(
		...
		this.isNew = {
			[StorageScope.WORKSPACE]: this.storageService.isNew(
				StorageScope.WORKSPACE,
			),
			[StorageScope.PROFILE]: this.storageService.isNew(StorageScope.PROFILE),
			[StorageScope.APPLICATION]: this.storageService.isNew(
				StorageScope.APPLICATION,
			),
			[StorageScope.APPLICATION_SHARED]: this.storageService.isNew(
	...
	load(configuration: ILayoutStateLoadConfiguration): void {
		...
		LayoutStateKeys.SIDEBAR_HIDDEN.defaultValue =
			...
			auxiliaryBarForceMaximized === true;
		...
		LayoutStateKeys.AUXILIARYBAR_HIDDEN.defaultValue = (() => {
			...
			const configuration = this.configurationService.inspect(
	...
	private applyOverrides(configuration: ILayoutStateLoadConfiguration): void {
		...
		if (
			...
			configuration.mainContainerDimension.width <=
				DEFAULT_WORKSPACE_WINDOW_DIMENSIONS.width
		) {
	...
	private applyAuxiliaryBarMaximizedOverride(): void {
		this.setRuntimeValue(
			LayoutStateKeys.AUXILIARYBAR_LAST_NON_MAXIMIZED_VISIBILITY,
			{
				...
				auxiliaryBarVisible: !this.getRuntimeValue(
					LayoutStateKeys.AUXILIARYBAR_HIDDEN,
				),
			},
		);
