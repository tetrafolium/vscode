	...
	class QuickAccessAction extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const aiFeaturesDisabled =
				configurationService.getValue<boolean>("chat.disableAIFeatures") ===
				true;
			const useUnifiedQuickAccess =
				!aiFeaturesDisabled &&
				configurationService.getValue<boolean>(UNIFIED_AGENTS_BAR_SETTING) ===
					true;
			if (useUnifiedQuickAccess) {
				try {
					await commandService.executeCommand(
						"workbench.action.unifiedQuickAccess",
					);
				} catch {
