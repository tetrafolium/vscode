...
abstract class BaseOpenRecentAction extends Action2 {
	...
	private readonly activeWindowOpenedRecentlyOpenedFolder: IQuickInputButton = {
		iconClass:
			"opened-workspace " + ThemeIcon.asClassName(Codicon.windowActive),
		tooltip: localize(
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const [mainWindows, recentlyOpened, dirtyWorkspacesAndFolders] =
			await Promise.all([
		...
		for (const recent of recentlyOpened.workspaces) {
			...
			workspacePicks.push(
				this.toQuickPick(modelService, languageService, labelService, recent, {
					isDirty,
					windowState,
				}),
			);
			...
			if (
				...
				workspacePicks.push(
					this.toQuickPick(
						...
						{ isDirty: true, windowState: undefined },
					),
				);
			} else if (
		...
		const filePicks = recentlyOpened.files.map((p) =>
			this.toQuickPick(modelService, languageService, labelService, p, {
				isDirty: false,
	...
	private toQuickPick(
		...
		if (isRecentFolder(recent)) {
			...
			fullLabel =
				recent.label ||
				labelService.getWorkspaceLabel(resource, { verbose: Verbosity.LONG });
		}
		...
		// File
			...
			fullLabel =
				recent.label ||
				labelService.getUriLabel(resource, { appendWorkspaceSuffix: true });
		}
