...
} from "../../services/policies/common/accountPolicyService.js";
...
class ToggleScreencastModeAction extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		...
		function registerContainerListeners(
			...
			windowDisposables: DisposableStore,
		): void {
...
class PolicyDiagnosticsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
			...
			if (account) {
				...
				try {
					...
					for (const providerId of providerIds) {
						const sessions =
							await authenticationService.getSessions(providerId);
						const matchingSession = sessions.find(
							(session) => session.id === account.sessionId,
						);
						if (matchingSession) {
		...
		try {
			...
			const approvedOrgsRaw = policyService.getPolicyValue(
				APPROVED_ACCOUNT_ORGANIZATIONS_POLICY_NAME,
			);
			content += "| Property | Value |\n";
			content += "|----------|-------|\n";
		...
		try {
			...
			const fetchStatusDisplay =
				fetchStatus === null ? "*not yet fetched*" : `\`${fetchStatus}\``;
		...
		const policyConfigurations =
			configurationRegistry.getPolicyConfigurations();
		...
		if (policyConfigurations.size > 0) {
			...
			for (const [policyName, settingKey] of policyConfigurations) {
				const property =
					configurationProperties[settingKey] ?? excludedProperties[settingKey];
				if (property) {
					...
					if (policyServiceConstructorName === "MultiplexPolicyService") {
						...
						if (multiplexService.policyServices) {
							...
							const componentServices =
								multiplexService.policyServices as ReadonlyArray<any>;
							for (const service of componentServices) {
								if (
									service.getPolicyValue &&
									service.getPolicyValue(policyName) !== undefined
								) {
			...
			appliedPolicy.sort(
				(a, b) =>
					getPolicySource(a.name).localeCompare(getPolicySource(b.name)) ||
					a.name.localeCompare(b.name),
			);
			if (appliedPolicy.length > 0) {
				content +=
					"| Setting Key | Policy Name | Policy Source | Default Value | Current Value | Policy Value |\n";
				content +=
					"|-------------|-------------|---------------|---------------|---------------|-------------|\n";
			...
			if (notAppliedPolicy.length > 0) {
				...
				content +=
					"*All policy-controllable settings are currently being enforced*\n\n";
			}
		} else {
				...
				for (const providerId of providerIds) {
					try {
						...
						const uniqueAccounts = Array.from(
							new Set(accounts.map((account) => account.label)),
						);
							...
							for (const session of sessions) {
								...
								try {
									const allowedExtensions =
										authenticationAccessService.readAllowedExtensions(
											providerId,
											accountName,
										);
