...
export abstract class BaseWindow extends Disposable {
		...
		targetWindow.setTimeout = function (
			...
			for (const { window, disposables } of dom.getWindows()) {
				...
				const timeoutDisposable = toDisposable(() => {
					...
					(
						window as {
							vscodeOriginalClearTimeout?: typeof window.clearTimeout;
						}
					).vscodeOriginalClearTimeout?.apply(this, [handle]);
	...
	private registerContextMenuListeners(targetWindow: Window): void {
		...
		const update = (visible: boolean) =>
			this.layoutService.activeContainer.classList.toggle(
				"context-menu-visible",
				visible,
			);
		this._register(
			this.contextMenuService.onDidShowContextMenu(() => update(true)),
		);
		this._register(
			this.contextMenuService.onDidHideContextMenu(() => update(false)),
		);
	}
}
...
export class BrowserWindow extends BaseWindow {
	...
	private onWillShutdown(): void {
		...
		Event.toPromise(
			Event.any(
				Event.once(
					new DomEmitter(mainWindow.document.body, EventType.KEY_DOWN, true)
						.event,
				),
				Event.once(
					new DomEmitter(mainWindow.document.body, EventType.MOUSE_DOWN, true)
						.event,
				),
			),
		).then(async () => {
		...
		this.openerService.setDefaultExternalOpener({
			openExternal: async (href: string) => {
				...
				if (
					...
					for (const trustedPopupPrefix of this.browserEnvironmentService
						.options.openerAllowedExternalUrlPrefixes) {
