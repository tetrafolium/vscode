	...
	implements ExtHostTelemetryShape
{
	...
	getBuiltInCommonProperties(
		...
		commonProperties["common.remotename"] = cleanRemoteAuthority(
...
export class ExtHostTelemetryLogger {
	...
	mixInCommonPropsAndCleanData(
		data: ExtHostTelemetryEventData,
	): Record<string, any> {
	...
	logError(
		...
		if (typeof eventNameOrException === "string") {
			...
			const cleanedError = new Error(
				typeof cleanedErrorData.message === "string"
					? cleanedErrorData.message
					: undefined,
				{
			...
			cleanedError.stack =
				typeof cleanedErrorData.stack === "string"
					? cleanedErrorData.stack
					: undefined;
			cleanedError.name =
				typeof cleanedErrorData.name === "string"
					? cleanedErrorData.name
					: "unknown";
			data = this.mixInCommonPropsAndCleanData(data || {});
			if (!this._inLoggingOnlyMode) {
...
export interface IExtHostTelemetry
	extends ExtHostTelemetry, ExtHostTelemetryShape {}
