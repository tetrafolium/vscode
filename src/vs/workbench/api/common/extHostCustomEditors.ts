	...
	implements extHostProtocol.ExtHostCustomEditorsShape
{
	...
	public registerCustomEditorProvider(
		...
		if (isCustomTextEditorProvider(provider)) {
			disposables.add(
				this._editorProviders.addTextProvider(viewType, extension, provider),
			);
			this._proxy.$registerTextEditorProvider(
				...
				options.webviewOptions || {},
				{
					...
					supportsInlineDiff:
						isProposedApiEnabled(extension, "customEditorDiffs") &&
						isCustomTextEditorProviderWithInlineDiffCapability(provider),
					supportsSideBySideDiff:
						isProposedApiEnabled(extension, "customEditorDiffs") &&
						isCustomTextEditorProviderWithSideBySideDiffCapability(provider),
				},
				shouldSerializeBuffersForPostMessage(extension),
			);
		} else {
			disposables.add(
				this._editorProviders.addCustomProvider(viewType, extension, provider),
			);
			const supportsCustomEditorDiffs = isProposedApiEnabled(
				extension,
				"customEditorDiffs",
			);
			...
			this._proxy.$registerCustomEditorProvider(
				...
				{
					supportsInlineDiff:
						supportsCustomEditorDiffs &&
						isCustomEditorProviderWithInlineDiffCapability(provider),
					supportsSideBySideDiff:
						supportsCustomEditorDiffs &&
						isCustomEditorProviderWithSideBySideDiffCapability(provider),
				},
	...
	async $resolveCustomEditor(
		...
		this._extHostWebview.ensureDefaultContentOptions(
			handle,
			initData.contentOptions,
			entry.extension,
		);
	...
	async $resolveCustomEditorInlineDiff(
		...
		const { entry, panel } = this.createCustomEditorDiffPanel(
			handle,
			viewType,
			initData,
			position,
		...
		if (entry.type === CustomEditorType.Text) {
			...
			const originalDocument = this._extHostDocuments.getDocument(
				revivedOriginalResource,
			);
			const modifiedDocument = this._extHostDocuments.getDocument(
				revivedModifiedResource,
			);
			return entry.provider.resolveCustomTextEditorInlineDiff(
				{ original: originalDocument, modified: modifiedDocument },
				panel,
				cancellation,
			);
		}
		...
		if (!isCustomEditorProviderWithInlineDiffCapability(entry.provider)) {
			throw new Error(
				`Provider for '${viewType}' does not support inline custom editor diffs`,
			);
		}

		const { document: originalDocument } = this.getCustomDocumentEntry(
			viewType,
			revivedOriginalResource,
		);
		const { document: modifiedDocument } = this.getCustomDocumentEntry(
			viewType,
			revivedModifiedResource,
		);
	...
	async $resolveCustomEditorSideBySideDiff(
		...
		const { entry, panel: originalPanel } = this.createCustomEditorDiffPanel(
			webviewHandles.original,
			viewType,
			initData.original,
			position,
		);
		const { panel: modifiedPanel } = this.createCustomEditorDiffPanel(
			webviewHandles.modified,
			viewType,
			initData.modified,
			position,
		);
		...
		if (entry.type === CustomEditorType.Text) {
			if (
				!isCustomTextEditorProviderWithSideBySideDiffCapability(entry.provider)
			) {
				throw new Error(
			...
			return entry.provider.resolveCustomTextEditorSideBySideDiff(
				{ original: originalDocument, modified: modifiedDocument },
				{ original: originalPanel, modified: modifiedPanel },
				cancellation,
			);
		}
	...
	async $onMoveCustomEditor(
		...
		if (
			!(entry.provider as vscode.CustomTextEditorProvider).moveCustomTextEditor
		) {
...
function isCustomTextEditorProviderWithInlineDiffCapability(
	provider: vscode.CustomTextEditorProvider,
): provider is vscode.CustomTextEditorProvider &
	Required<
		Pick<vscode.CustomTextEditorProvider, "resolveCustomTextEditorInlineDiff">
	> {
	return typeof provider.resolveCustomTextEditorInlineDiff === "function";
}

function isCustomTextEditorProviderWithSideBySideDiffCapability(
	provider: vscode.CustomTextEditorProvider,
): provider is vscode.CustomTextEditorProvider &
	Required<
		Pick<
...
function isCustomEditorProviderWithInlineDiffCapability(
	provider: vscode.CustomReadonlyEditorProvider,
): provider is vscode.CustomReadonlyEditorProvider &
	Required<
		Pick<vscode.CustomReadonlyEditorProvider, "resolveCustomEditorInlineDiff">
	> {
	return typeof provider.resolveCustomEditorInlineDiff === "function";
}

function isCustomEditorProviderWithSideBySideDiffCapability(
	provider: vscode.CustomReadonlyEditorProvider,
): provider is vscode.CustomReadonlyEditorProvider &
	Required<
		Pick<
