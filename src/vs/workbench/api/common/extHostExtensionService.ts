	...
	implements ExtHostExtensionServiceShape
{
				...
				this._globalRegistry,
				{
					...
					actualActivateExtension: async (
						...
						const extensionDescription =
							this._myRegistry.getExtensionDescription(extensionId)!;
	...
	private async _handleWorkspaceContainsEagerExtension(
		...
		const host: IExtensionActivationHost = {
			...
			checkExists: (folders, includes, token) =>
				this._mainThreadWorkspaceProxy.$checkExists(folders, includes, token),
		};
	...
	private async _doHandleExtensionTests(): Promise<number> {
		...
		const extensionDescription = (
			await this.getExtensionPathIndex()
		).findSubstr(extensionTestsLocationURI);
	...
	protected _isESM(
		extensionDescription: IExtensionDescription | undefined,
		modulePath?: string,
	): boolean {
		modulePath ??= extensionDescription
			? this._getEntryPoint(extensionDescription)
			: modulePath;
		return (
			modulePath?.endsWith(".mjs") ||
			(extensionDescription?.type === "module" && !modulePath?.endsWith(".cjs"))
		);
	}
