...
export function createApiFactoryAndRegisterActors(
	...
	const extHostChatQuota = rpcProtocol.set(
		ExtHostContext.ExtHostChatQuota,
		new ExtHostChatQuota(rpcProtocol),
	);
	...
	return function (
		...
		const env: typeof vscode.env = {
			...
			get devDeviceId() {
				...
				return (
					initData.telemetryInfo.devDeviceId ?? initData.telemetryInfo.machineId
				);
			},
			get isAppPortable() {
			...
			get onDidChangeMeteredConnection(): vscode.Event<boolean> {
				checkProposedApiEnabled(extension, "envIsConnectionMetered");
				return _asExtensionEvent(
					extHostMeteredConnection.onDidChangeIsConnectionMetered,
				);
			},
			get isNewAppInstall() {
				...
				return extHostWindow.openUri(uri, {
					allowTunneling:
						...
						(initData.remote.authority
							? await extHostTunnelService.hasTunnelProvider()
							: false),
		...
		const window: typeof vscode.window = {
			...
			onDidChangeActiveChatPanelSessionResource: (
				...
				checkProposedApiEnabled(extension, "chatParticipantPrivate");
				return _asExtensionEvent(
					extHostChatAgents2.onDidChangeActiveChatPanelSessionResource,
			...
			onDidOpenBrowserTab(listener, thisArg?, disposables?) {
				checkProposedApiEnabled(extension, "browser");
				return _asExtensionEvent(extHostBrowsers.onDidOpenBrowserTab)(
					listener,
					thisArg,
					disposables,
				);
			...
			onDidCloseBrowserTab(listener, thisArg?, disposables?) {
				checkProposedApiEnabled(extension, "browser");
				return _asExtensionEvent(extHostBrowsers.onDidCloseBrowserTab)(
					listener,
					thisArg,
					disposables,
				);
			...
			onDidChangeActiveBrowserTab(listener, thisArg?, disposables?) {
				checkProposedApiEnabled(extension, "browser");
				return _asExtensionEvent(extHostBrowsers.onDidChangeActiveBrowserTab)(
					listener,
					thisArg,
					disposables,
				);
			...
			onDidChangeBrowserTabState(listener, thisArg?, disposables?) {
				checkProposedApiEnabled(extension, "browser");
				return _asExtensionEvent(extHostBrowsers.onDidChangeBrowserTabState)(
					listener,
					thisArg,
					disposables,
				);
		...
		const workspace: typeof vscode.workspace = {
			...
			getTextDiff(
				...
				options?: vscode.TextDiffOptions,
				token?: vscode.CancellationToken,
			): vscode.TextDiffResponse {
				...
				const diffPromise = token
					? raceCancellationError(resultPromise, token)
					: resultPromise;
				const mappedPromise = diffPromise.then((result) => {
					if (!result) {
						throw new Error(
							"Could not compute diff. Make sure both documents are available.",
						);
					}
					return result;
				});
				...
				return {
					changes: new AsyncIterableObject<vscode.TextDiffChange>(
						async (emitter) => {
			...
			onDidChangeTextDocument: (listener, thisArgs?, disposables?) => {
				if (isProposedApiEnabled(extension, "textDocumentChangeReason")) {
					return _asExtensionEvent(
						extHostDocuments.onDidChangeDocumentWithReason,
					)(listener, thisArgs, disposables);
				}
			...
			onDidChangeWorkspaceTrustedFolders: (
				...
				checkProposedApiEnabled(extension, "workspaceTrust");
				return _asExtensionEvent(
					extHostWorkspace.onDidChangeWorkspaceTrustedFolders,
				)(listener, thisArgs, disposables);
			},
		...
		const chat: typeof vscode.chat = {
			...
			registerChatSessionItemProvider: (
				...
				return extHostChatSessions.registerChatSessionItemProvider(
					extension,
					chatSessionType,
					provider,
				);
			...
			createChatSessionItemController: (
				...
				checkProposedApiEnabled(extension, "chatSessionsProvider");
				return extHostChatSessions.createChatSessionItemController(
					extension,
					chatSessionType,
					refreshHandler,
				);
			...
			registerChatSessionContentProvider(
				...
				checkProposedApiEnabled(extension, "chatSessionsProvider");
				return extHostChatSessions.registerChatSessionContentProvider(
					extension,
					scheme,
					chatParticipant,
					provider,
			...
			registerChatOutputRenderer: (
				...
				return extHostChatOutputRenderer.registerChatOutputRenderer(
					extension,
					viewType,
					renderer,
				);
			...
			registerChatWorkspaceContextProvider(
				id: string,
				provider: vscode.ChatWorkspaceContextProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatContextProvider");
				return extHostChatContext.registerChatWorkspaceContextProvider(
					`${extension.id}-${id}`,
					provider,
				);
			},
			registerChatExplicitContextProvider(
				id: string,
				provider: vscode.ChatExplicitContextProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatContextProvider");
				return extHostChatContext.registerChatExplicitContextProvider(
					`${extension.id}-${id}`,
					provider,
				);
			},
			registerChatResourceContextProvider(
				selector: vscode.DocumentSelector,
				id: string,
				provider: vscode.ChatResourceContextProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatContextProvider");
				return extHostChatContext.registerChatResourceContextProvider(
					checkSelector(selector),
					`${extension.id}-${id}`,
					provider,
				);
			...
			registerChatContextProvider(
				...
				id: string,
				provider: vscode.ChatContextProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatContextProvider");
			...
			registerCustomAgentProvider(
				provider: vscode.ChatCustomAgentProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.registerPromptFileProvider(
					extension,
					PromptsType.agent,
					provider,
				);
			...
			registerInstructionsProvider(
				provider: vscode.ChatInstructionsProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.registerPromptFileProvider(
					extension,
			...
			registerPromptFileProvider(
				provider: vscode.ChatPromptFileProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.registerPromptFileProvider(
					extension,
			...
			registerSkillProvider(
				provider: vscode.ChatSkillProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.registerPromptFileProvider(
					extension,
			...
			registerHookProvider(
				provider: vscode.ChatHookProvider,
			): vscode.Disposable {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.registerPromptFileProvider(
					extension,
			...
			registerChatDebugLogProvider(
				provider: vscode.ChatDebugLogProvider,
			): vscode.Disposable {
			...
			onDidReceiveChatDebugEvent: (listener, thisArgs?, disposables?) => {
				checkProposedApiEnabled(extension, "chatDebug");
				return extHostChatDebug.onDidAddCoreEvent(
					listener,
					thisArgs,
					disposables,
				);
			...
			getCustomAgents(token: vscode.CancellationToken) {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.provideCustomAgents(token) as Thenable<
					readonly vscode.ChatCustomAgent[]
				>;
			},
			onDidChangeCustomAgents: (listener, thisArgs?, disposables?) => {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.onDidChangeCustomAgents(
					listener,
					thisArgs,
					disposables,
				);
			...
			getInstructions(token: vscode.CancellationToken) {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.provideInstructions(token) as Thenable<
					readonly vscode.ChatInstruction[]
				>;
			},
			onDidChangeInstructions: (listener, thisArgs?, disposables?) => {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.onDidChangeInstructions(
					listener,
					thisArgs,
					disposables,
				);
			...
			getSkills(token: vscode.CancellationToken) {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.provideSkills(token) as Thenable<
					readonly vscode.ChatSkill[]
				>;
			},
			onDidChangeSkills: (listener, thisArgs?, disposables?) => {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.onDidChangeSkills(
					listener,
					thisArgs,
					disposables,
				);
			...
			getSlashCommands(token: vscode.CancellationToken) {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.provideSlashCommands(token) as Thenable<
					readonly vscode.ChatSlashCommand[]
				>;
			},
			onDidChangeSlashCommands: (listener, thisArgs?, disposables?) => {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.onDidChangeSlashCommands(
					listener,
					thisArgs,
					disposables,
				);
			...
			getHooks(token: vscode.CancellationToken) {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.provideHooks(token) as Thenable<
					readonly vscode.ChatHook[]
				>;
			},
			onDidChangeHooks: (listener, thisArgs?, disposables?) => {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.onDidChangeHooks(
					listener,
					thisArgs,
					disposables,
				);
			...
			getPlugins(token: vscode.CancellationToken) {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.providePlugins(token) as Thenable<
					readonly vscode.ChatPlugin[]
				>;
			},
			onDidChangePlugins: (listener, thisArgs?, disposables?) => {
				checkProposedApiEnabled(extension, "chatPromptFiles");
				return extHostChatAgents2.onDidChangePlugins(
					listener,
					thisArgs,
					disposables,
				);
			...
			registerChatSessionCustomizationProvider(
				...
				return extHostChatAgents2.registerChatSessionCustomizationProvider(
					extension,
					chatSessionType,
					metadata,
					provider,
			...
			createInputNotification(id: string): vscode.ChatInputNotification {
				...
				return extHostChatInputNotification.createInputNotification(
					extension,
					id,
				);
			},
		...
		const lm: typeof vscode.lm = {
			...
			registerLanguageModelChatProvider: (vendor, provider) => {
				return extHostLanguageModels.registerLanguageModelChatProvider(
					extension,
					vendor,
					provider,
				);
			...
			onDidChangeModelProxyAvailability: (
				...
				checkProposedApiEnabled(extension, "languageModelProxy");
				return extHostLanguageModels.onDidChangeModelProxyAvailability(
					listener,
					thisArgs,
					disposables,
				);
			...
			registerLanguageModelProxyProvider: (provider) => {
				checkProposedApiEnabled(extension, "chatParticipantPrivate");
				return extHostLanguageModels.registerLanguageModelProxyProvider(
					extension,
					provider,
				);
			},
			...
			registerToolDefinition<T>(
				definition: vscode.LanguageModelToolDefinition,
				tool: vscode.LanguageModelTool<T>,
			) {
				return extHostLanguageModelTools.registerToolDefinition(
					extension,
					definition,
					tool,
				);
			...
			invokeTool<T>(
				nameOrInfo: string | vscode.LanguageModelToolInformation,
			...
			onDidChangeMcpServerDefinitions: (...args) => {
				...
				return _asExtensionEvent(extHostMcp.onDidChangeMcpServerDefinitions)(
					...args,
				);
			},
			...
			onDidChangeChatRequestTools(...args) {
				checkProposedApiEnabled(extension, "chatParticipantAdditions");
				return _asExtensionEvent(
					extHostChatAgents2.onDidChangeChatRequestTools,
				)(...args);
			},
		};
		...
		return <typeof vscode>{
			...
			InlineCompletionsDisposeReasonKind:
				extHostTypes.InlineCompletionsDisposeReasonKind,
			...
			InlineCompletionDisplayLocationKind:
				extHostTypes.InlineCompletionDisplayLocationKind,
			...
			ChatResponseThinkingProgressPart:
				extHostTypes.ChatResponseThinkingProgressPart,
			...
			ChatResponseQuestionCarouselPart:
				extHostTypes.ChatResponseQuestionCarouselPart,
			...
			ChatResponseClearToPreviousToolInvocationReason:
				extHostTypes.ChatResponseClearToPreviousToolInvocationReason,
			...
			ChatSubagentToolInvocationData:
				extHostTypes.ChatSubagentToolInvocationData,
			...
			ChatDebugSubagentInvocationEvent:
				extHostTypes.ChatDebugSubagentInvocationEvent,
			...
			ChatDebugEventModelTurnContent:
				extHostTypes.ChatDebugEventModelTurnContent,
			...
			LanguageModelToolExtensionSource:
				extHostTypes.LanguageModelToolExtensionSource,
