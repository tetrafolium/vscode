...
export class ExtHostOutputService implements ExtHostOutputServiceShape {
	...
	private readonly extensionLogDirectoryCreationPromise = new ResourceMap<
		Thenable<void>
	>();
	...
	createOutputChannel(
		...
		let logLevel = this.initData.environment.extensionLogLevel?.find(
			([identifier]) =>
		...
		if (log) {
			...
			extHostOutputChannelPromise = this.doCreateOutputChannel(
				name,
				languageId,
				extension,
				channelDisposables,
	...
	private createExtensionLogDirectory(file: URI): Thenable<void> {
		const extensionLogDirectory =
			this.extHostFileSystemInfo.extUri.dirname(file);
		let extensionLogDirectoryPromise =
			this.extensionLogDirectoryCreationPromise.get(extensionLogDirectory);
