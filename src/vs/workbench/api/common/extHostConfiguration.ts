...
function isWorkspaceFolder(thing: unknown): thing is vscode.WorkspaceFolder {
	return (
		isObject(thing) &&
		(thing as Record<string, unknown>).uri instanceof URI &&
		(!(thing as Record<string, unknown>).name ||
			typeof (thing as Record<string, unknown>).name === "string") &&
		(!(thing as Record<string, unknown>).index ||
			typeof (thing as Record<string, unknown>).index === "number")
	);
}
						...
						if (isObject(target)) {
							...
							return new Proxy(target, {
								get: (
									...
									if (clonedConfig) {
										clonedTarget = clonedTarget
											? clonedTarget
											: lookUp(clonedConfig, accessor);
										return (clonedTarget as Record<PropertyKey, unknown>)[
											property
										];
									}
								...
								set: (
									...
									cloneTarget();
									if (clonedTarget) {
										(clonedTarget as Record<PropertyKey, unknown>)[property] =
											value;
									}
									return true;
								},
								deleteProperty: (
									...
									cloneTarget();
									if (clonedTarget) {
										delete (clonedTarget as Record<PropertyKey, unknown>)[
											property
										];
									}
									return true;
								...
								defineProperty: (
									...
									if (clonedTarget) {
										Object.defineProperty(
											clonedTarget as Record<string, unknown>,
											property,
											descriptor,
										);
									}
				...
				if (config) {
					return {
						...
						globalLanguageValue: deepClone(
							config.user?.override ?? config.application?.override,
						),
