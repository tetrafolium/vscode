...
export class ExtHostNotebookController implements ExtHostNotebookShape {
	...
	async createNotebookDocument(options: {
		...
		const canonicalUri = await this._notebookDocumentsProxy.$tryCreateNotebook({
			...
			content:
				options.content && typeConverters.NotebookData.from(options.content),
		});
		...
		try {
			...
			if (document.versionId !== versionId) {
				throw new NotebookSaveError(
					"Document version mismatch, expected: " +
						versionId +
						", actual: " +
						document.versionId,
				);
			...
			const fileStats = {
				...
				size: stat.size,
				readonly:
					Boolean((stat.permissions ?? 0) & files.FilePermission.Readonly) ||
				...
				executable: Boolean(
					(stat.permissions ?? 0) & files.FilePermission.Executable,
				),
													...
													(target) => {
														...
														if (
															...
															return target.filenamePatterns.some(
																(targetFilePattern) =>
																	globMatchesResource(targetFilePattern, uri),
															);
														}
													},
												);
					...
					cells.forEach((e) =>
						simpleCells.push({
							...
							outputs: e.outputs.flatMap((value) =>
								value.items.map((output) => output.data.toString()),
							),
						}),
					);
				} else {
					...
					data.cells.forEach((cell) =>
						simpleCells.push({
							...
							outputs: cell.outputs.flatMap((value) =>
								value.items.map((output) => output.valueBytes.toString()),
							),
						}),
					);
				}
	...
	private async _validateWriteFile(uri: URI, options: files.IWriteFileOptions) {
		...
		if (
			...
			options.etag !==
				files.etag({
					mtime:
						options.mtime /* not using stat.mtime for a reason, see above */,
					size: stat.size,
				})
		) {
			throw new files.FileOperationError(
		...
		if (delta.value.addedDocuments) {
			...
			for (const modelData of delta.value.addedDocuments) {
				...
				addedCellDocuments.push(
					...modelData.cells.map((cell) => ExtHostCell.asModelAddData(cell)),
				);
	...
	private static _registerApiCommands(extHostCommands: ExtHostCommands) {
		...
		const commandDataToNotebook = new ApiCommand(
			...
			new ApiCommandResult<
				...
				vscode.NotebookData
			>("Notebook Data", (data) => typeConverters.NotebookData.to(data.value)),
		);
