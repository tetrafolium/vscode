...
export class ExtHostAuthentication implements ExtHostAuthenticationShape {
	...
	protected readonly _xaaAuthProviderCtor =
		XaaifyAuthProvider(DynamicAuthProvider);
	...
	async getSession(
		...
		if (isAuthenticationWwwAuthenticateRequest(scopesOrRequest)) {
			const challenge =
				scopesOrRequest as vscode.AuthenticationWwwAuthenticateRequest;
			const challengeStr = challenge.wwwAuthenticate;
			const scopesStr = challenge.fallbackScopes
				? [...challenge.fallbackScopes].sort().join(" ")
				: "";
		...
		return this._providerOperations.queue(providerId, async () => {
			...
			if (providerData) {
				...
				if (typeof provider.createSessionFromChallenges === "function") {
					options.authorizationServer = URI.revive(options.authorizationServer);
					return await provider.createSessionFromChallenges(
						constraint,
						options,
					);
				}
	...
	async $registerDynamicAuthProvider(
		...
		if (!clientId) {
			...
			if (!clientId) {
				this._logService.info(
					`Prompting user for client registration details for ${authorizationServer.toString()}`,
				);
				const clientDetails = await this._proxy.$promptForClientRegistration(
				...
				this._logService.info(
					`User provided client registration for ${authorizationServer.toString()}`,
				);
				if (clientSecret) {
					this._logService.trace(
						`User provided client secret for ${authorizationServer.toString()}`,
					);
				} else {
					this._logService.trace(
						`User did not provide client secret for ${authorizationServer.toString()}`,
					);
				}
			}
		}
		...
		await this._providerOperations.queue(provider.id, async () => {
			...
			await this._proxy.$registerDynamicAuthenticationProvider({
				...
				resourceServer: resourceMetadata
					? URI.parse(resourceMetadata.resource)
					: undefined,
				clientId: provider.clientId,
				clientSecret: provider.clientSecret,
			});
...
export class DynamicAuthProvider implements vscode.AuthenticationProvider {
	...
	constructor(
		...
		const scopedEvent = Event.chain(
			...
			($) =>
				$.filter(
					(e) => e.authProviderId === this.id && e.clientId === _clientId,
				).map((e) => e.tokens),
		);
	...
	protected async exchangeCodeForToken(
		...
		if (isAuthorizationTokenResponse(result)) {
			...
			isAuthorizationErrorResponse(result) &&
			result.error === AuthorizationErrorType.InvalidClient
		) {
			this._logger.warn(
	...
	protected async _generateNewClientId(): Promise<void> {
		try {
			...
			try {
				const clientDetails = await this._proxy.$promptForClientRegistration(
					this.authorizationServer.toString(),
				);
				if (!clientDetails) {
					throw new Error("User did not provide client details");
				}
				...
				this._logger.info(
					`User provided client ID for ${this.authorizationServer.toString()}`,
				);
				if (clientDetails.clientSecret) {
					this._logger.info(
						`User provided client secret for ${this.authorizationServer.toString()}`,
					);
				} else {
					this._logger.info(
						`User did not provide client secret for ${this.authorizationServer.toString()} (optional)`,
					);
				}
				...
				this._logger.error(
					`Failed to fetch new client ID and user did not provide one: ${err}`,
				);
				throw new Error(
					`Failed to fetch new client ID and user did not provide one: ${err}`,
				);
