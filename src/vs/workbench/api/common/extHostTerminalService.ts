...
export interface IExtHostTerminalService
	extends ExtHostTerminalServiceShape, IDisposable {
	...
	implements IExtHostTerminalService, ExtHostTerminalServiceShape
{
	...
	public async $createContributedProfileTerminal(
		...
		let profile =
			await profileProviderData.provider.provideTerminalProfile(token);
		if (token.isCancellationRequested) {
			return;
		}
		...
		const hasTerminalTitleProposal = isProposedApiEnabled(
			profileProviderData.extension,
			"terminalTitle",
		);
		if (
			!hasTerminalTitleProposal &&
			profile.options.titleTemplate !== undefined
		) {
			console.error(
...
class UnifiedEnvironmentVariableCollection extends Disposable {
	...
	protected readonly _onDidChangeCollection: Emitter<void> = this._register(
		new Emitter<void>(),
	);
