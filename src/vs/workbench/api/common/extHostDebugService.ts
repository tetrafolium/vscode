...
export interface IExtHostDebugService extends ExtHostDebugServiceShape {
	...
	readonly onDidChangeActiveDebugSession: Event<
	...
	readonly onDidChangeActiveStackItem: Event<
	...
	implements IExtHostDebugService, ExtHostDebugServiceShape
{
	...
	private _debugSessions: Map<DebugSessionUUID, ExtHostDebugSession> = new Map<
		DebugSessionUUID,
		ExtHostDebugSession
	>();
		...
		return this._debugServiceProxy.$startDebugging(
			...
			nameOrConfig,
			{
				...
				suppressDebugStatusbar:
					options.suppressDebugStatusbar ?? (options as any).debugUI?.simple,
				// eslint-disable-next-line local/code-no-any-casts
				suppressDebugToolbar:
					options.suppressDebugToolbar ?? (options as any).debugUI?.simple,
				// eslint-disable-next-line local/code-no-any-casts
				suppressDebugView:
					options.suppressDebugView ?? (options as any).debugUI?.simple,
			},
		);
	}
					...
					if (
						...
						try {
							...
							const type =
								message.type +
								"_" +
								((message as any).command ?? (message as any).event ?? "");
	...
	public async $provideDebugAdapter(
		...
		const session = await this.getSession(sessionDto);
		return this.getAdapterDescriptor(adapterDescriptorFactory, session).then(
			(adapterDescriptor) => {
...
class MultiTracker implements vscode.DebugAdapterTracker {
	...
	onWillReceiveMessage(message: any): void {
		this.trackers.forEach((t) =>
			t.onWillReceiveMessage ? t.onWillReceiveMessage(message) : undefined,
		);
	}
