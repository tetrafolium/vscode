...
export const IExtHostMpcService =
	createDecorator<IExtHostMpcService>("IExtHostMpcService");
...
export class ExtHostMcpService
	extends Disposable
	implements IExtHostMpcService
{
	protected _proxy: MainThreadMcpShape;
	private readonly _initialProviderPromises = new Set<Promise<void>>();
	protected readonly _sseEventSources = this._register(
		new DisposableMap<number, McpHTTPHandle>(),
	);
	private readonly _unresolvedMcpServers = new Map<
		/* collectionId */ string,
		{
			provider: vscode.McpServerDefinitionProvider;
	...
	private readonly _onDidChangeMcpServerDefinitions = this._register(
		new Emitter<void>(),
	);
	readonly onDidChangeMcpServerDefinitions: Event<void> =
	...
	private readonly _activeGateways = new Map<
		string,
		{
	...
	constructor(
		...
		private readonly _extHostInitData: IExtHostInitDataService,
		@IExtHostWorkspace protected readonly _workspaceService: IExtHostWorkspace,
		@IExtHostVariableResolverProvider
		private readonly _variableResolver: IExtHostVariableResolverProvider,
	) {
		super();
	...
	$onDidChangeMcpServerDefinitions(
		...
		this._mcpServerDefinitions = servers.map((dto) =>
			Convert.McpServerDefinition.to(dto),
		);
	...
	async $substituteVariables<T>(
		_workspaceFolder: UriComponents | undefined,
		value: T,
	): Promise<T> {
		const folderURI = URI.revive(_workspaceFolder);
		const folder =
			folderURI &&
			(await this._workspaceService.resolveWorkspaceFolder(folderURI));
		const variableResolver = await this._variableResolver.getResolver();
		return variableResolver.resolveAsync(
			folder && {
				uri: folder.uri,
				name: folder.name,
				index: folder.index,
	...
	async $resolveMcpLaunch(
		...
		const resolved = await rec.provider.resolveMcpServerDefinition(
			server,
	...
	public registerMcpConfigurationProvider(
		...
		const metadata = extension.contributes?.mcpServerDefinitionProviders?.find(
			(m) => m.id === id,
		);
		if (!metadata) {
			throw new Error(
		...
		const mcp: McpCollectionDefinition.FromExtHost = {
			...
			configTarget: this._extHostInitData.remote.isRemote
		...
		const update = async () => {
			...
			for (const item of list ?? []) {
				let id =
					ExtensionIdentifier.toKey(extension.identifier) + "/" + item.label;
				...
				const castAs2 = item as
					| McpStdioServerDefinition
					| McpHttpServerDefinition;
				if (
					isProposedApiEnabled(extension, "mcpToolDefinitions") &&
					castAs2.metadata
				) {
					staticMetadata = {
						capabilities: castAs2.metadata
							.capabilities as MCP.ServerCapabilities,
						...
						tools: castAs2.metadata.tools?.map((t) => ({
							availability:
								t.availability === McpToolAvailability.Dynamic
									? McpServerStaticToolAvailability.Dynamic
	...
	public async startMcpGateway(
		chatSessionResource?: URI,
	): Promise<vscode.McpGateway | undefined> {
		const result = await this._proxy.$startMcpGateway(
			chatSessionResource?.toJSON(),
		);
		if (!result) {
			return undefined;
		}
		...
		const onDidChangeServers = new Emitter<
			readonly vscode.McpGatewayServer[]
		>();
	...
	$onDidChangeGatewayServers(
		gatewayId: string,
		newServers: { label: string; address: UriComponents }[],
	): void {
...
export class McpHTTPHandle extends Disposable {
	...
	private readonly _postEndpoint = new DeferredPromise<{
		url: string;
	...
	async close() {
		if (
			this._mode.value === HttpMode.Http &&
			this._mode.sessionId &&
			!this._didSendClose
		) {
	 ...
	 */
	private async _sendStreamableHttp(
		message: string,
		sessionId: string | undefined,
	) {
		const asBytes = new TextEncoder().encode(
			message,
		...
		if (res.status >= 300) {
			...
			this._proxy.$onDidChangeState(this._id, {
				state: McpConnectionState.Kind.Error,
				message:
					`${res.status} status sending message to ${this._launch.uri}: ${await this._getErrText(res)}` +
					(retryWithSessionId ? `; will retry with new session ID` : ""),
	...
	private async _handleSuccessfulStreamableHttp(
		res: CommonResponse,
		message: string,
	) {
		if (res.status === 202) {
		...
		if (contentType.startsWith("text/event-stream")) {
			...
			if (isJSON(responseBody)) {
				// try to read as JSON even if the server didn't set the content type
	...
	private async _attachStreamableBackchannel() {
		...
		for (let retry = 0; !this._store.isDisposed; retry++) {
			if (canReconnectAt !== undefined) {
				await timeout(
					Math.max(0, canReconnectAt - Date.now()),
					this._cts.token,
				);
			...
			try {
				...
				if (
					this._mode.value === HttpMode.Http &&
					this._mode.sessionId !== undefined
				) {
			...
			try {
				...
				this._log(
					LogLevel.Info,
					`Error reading from async stream, we will reconnect: ${e}`,
				);
			}
		}
	}
	...
	private async _attachSSE(): Promise<string | undefined> {
		...
		const parser = new SSEParser((event) => {
			if (event.type === "message") {
				...
				postEndpoint.complete(
					new URL(event.data, this._launch.uri.toString(true)).toString(),
				);
			}
		});
		...
		this._doSSE(parser, res).catch((err) => {
			this._proxy.$onDidChangeState(this._id, {
				state: McpConnectionState.Kind.Error,
				message: `Error reading SSE stream: ${String(err)}`,
			});
		});
	...
	private async _addAuthHeader(
		...
		const errorOnUserInteraction =
			options?.errorOnUserInteraction ?? this._errorOnUserInteraction;
		if (this._authMetadata) {
			try {
				...
				this._log(
					LogLevel.Warning,
					`Error getting token from server metadata: ${String(e)}`,
				);
			}
		}
	...
	private async _fetch(
		...
		for (
			let redirectCount = 0;
			redirectCount < MAX_FOLLOW_REDIRECTS;
			redirectCount++
		) {
...
class AuthMetadata implements IAuthMetadata {
	...
	constructor(
		...
		public readonly resourceMetadata:
			| IAuthorizationProtectedResourceMetadata
			| undefined,
	...
	private _parseScopesFromResponse(
		...
		for (const challenge of challenges) {
			if (challenge.scheme === "Bearer" && challenge.params["scope"]) {
				const scopes = challenge.params["scope"]
					.split(AUTH_SCOPE_SEPARATOR)
...
export async function createAuthMetadata(
	...
	} = parseWWWAuthenticateHeaderForChallenges(
		initialResponseHeaders.get("WWW-Authenticate") ?? undefined,
		log,
	);
	...
	try {
		const { metadata, discoveryUrl, errors } = await fetchResourceMetadata(
			resourceUrl,
			resourceMetadataChallenge,
			{
				sameOriginHeaders,
		...
		resourceMetadataSource = resourceMetadataChallenge
			? IAuthResourceMetadataSource.Header
			: IAuthResourceMetadataSource.WellKnown;
		...
		if (!serverMetadataUrl) {
			...
			log(
				LogLevel.Info,
				`Using auth server metadata url: ${serverMetadataUrl}`,
			);
	...
	try {
		...
		const { metadata, discoveryUrl, errors } =
		...
		for (const err of errors) {
			log(
				LogLevel.Warning,
				`Error fetching authorization server metadata: ${err}`,
			);
		}
		log(
			LogLevel.Info,
