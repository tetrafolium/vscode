...
export class ExtHostTesting extends Disposable implements ExtHostTestingShape {
			...
			processArgument: (arg) => {
				switch (arg?.$mid) {
					case MarshalledId.TestItemContext: {
						...
						return (
							controller?.collection.tree.get(targetTest)?.actual ??
							toItemFromContext(arg)
						);
					}
	...
	public createTestController(
			...
			invalidateTestResults: (items) => {
				if (items === undefined) {
					...
					this.proxy.$markTestRetired(
						itemsArr.map((i) =>
							TestId.fromExtHostTestItem(i!, controllerId).toString(),
						),
					);
				}
			},
		...
		disposable.add(
			collection.onDidGenerateDiff((diff) =>
				proxy.$publishDiff(controllerId, diff.map(TestsDiffOp.serialize)),
			),
		);
			...
			[...this.controllers.values()].map(async (c) => {
				...
				if (tests) {
					for (const test of tests) {
						related.push(
							TestId.fromExtHostTestItem(test, c.controller.id).toString(),
						);
					}
	...
	public $acceptDiff(diff: TestsDiffOp.Serialized[]): void {
		this.observer.applyDiff(
			diff.map((d) => TestsDiffOp.deserialize({ asCanonicalUri: (u) => u }, d)),
		);
	}
	...
	public async $startContinuousRun(
		...
		const res = await Promise.all(
			reqs.map((req) => this.runControllerTestRequest(req, true, cts.token)),
		);
	...
	private async runControllerTestRequest(
		...
		const excludeTests = req.excludeExtIds
			...
			.filter((exclude) =>
				includeTests.some(
					(include) =>
						include.fullId.compare(exclude.fullId) === TestPosition.IsChild,
				),
			);
...
class TestRunTracker extends Disposable {
	...
	constructor(
		...
		const forciblyEnd = this._register(
			new RunOnceScheduler(() => this.forciblyEndTasks(), RUN_CANCEL_DEADLINE),
		);
		this._register(
			this.cts.token.onCancellationRequested(() => forciblyEnd.schedule()),
		);
	...
	public createRun(name: string | undefined): vscode.TestRun {
		...
		const run: vscode.TestRun = {
			...
			addCoverage: (coverage) => {
				...
				const includesTests =
					coverage instanceof FileCoverage ? coverage.includesTests : [];
		...
		this.proxy.$startedTestRunTask(runId, {
			...
			name:
				name || this.extension.displayName || this.extension.identifier.value,
			running: true,
		});
...
export class TestRunCoordinator {
	...
	public getCoverageDetails(
		...
		return (
			this.trackedById.get(runId)?.getCoverageDetails(id, testId, token) || []
		);
	}
...
export class TestRunProfileImpl
	extends TestRunProfileBase
	implements vscode.TestRunProfile
{
	...
	public get onDidChangeDefault() {
		return Event.chain(this.#onDidChangeDefaultProfiles, ($) =>
			$.map((ev) => ev.get(this.controllerId)?.get(this.profileId)).filter(
				isDefined,
			),
		);
	}
