...
export function isExpired(
	entry: { token: { expires_in?: number }; created_at: number },
	now: number = Date.now(),
): boolean {
 ...
 */
export function XaaifyAuthProvider<TBase extends Ctor<DynamicAuthProvider>>(
	Base: TBase,
): TBase {
	return class XaaAuthenticationProvider extends Base {
		...
		private _resourceClientSecretKey(
			resource: string,
			clientId: string,
		): string {
		...
		constructor(...args: any[]) {
			...
			this._logger.trace(
				`[XAA] Provider constructed for issuer ${issuer.toString(true)}. authorization_endpoint=${this._serverMetadata.authorization_endpoint}, token_endpoint=${this._serverMetadata.token_endpoint}`,
			);
		}
		...
		override async getSessions(
			...
			try {
				...
				this._logger.warn(
					`[XAA] Silent token mint failed for resource=${resource}; falling back to interactive. Error: ${(err as Error).message}`,
				);
				return [];
			}
		}
		...
		private async _mintResourceToken(
			...
			const jag = await this._exchangeForIdJag(
				idpSession.idToken!,
				audience,
				resource,
				scopes,
			...
			const resourceTokenEndpoint =
				await this._discoverResourceTokenEndpoint(audience);
			...
			const configuredResourceClientId =
			...
			if (configuredResourceClientId) {
				...
				try {
					...
					if (
						typeof jagClaims.client_id === "string" &&
						jagClaims.client_id.length > 0
					) {
					...
					this._logger.warn(
						`[XAA] Could not decode ID-JAG to read resource client_id; falling back to IdP client_id. Error: ${(err as Error).message}`,
					);
				}
			}
			...
			if (configuredResourceClientSecret) {
				...
				if (this._resourceClientSecrets.has(secretCacheKey)) {
					...
					const promptedSecret =
						await this._proxy.$promptForResourceClientSecret(
							resourceClientId,
							resource,
						);
					...
					resourceClientSecret =
						promptedSecret.length > 0 ? promptedSecret : undefined;
				}
			}
		 ...
		 */
		private async _tryGetSilentIdpSession(): Promise<
			vscode.AuthenticationSession | undefined
		> {
			const cleanOptions: vscode.AuthenticationProviderSessionOptions = {};
			const existing = await super.getSessions(
				IDP_SCOPES as string[],
				cleanOptions,
			);
		...
		private async _ensureIdpSession(): Promise<vscode.AuthenticationSession> {
			...
			if (silent?.idToken) {
				this._logger.trace(
					`[XAA] _ensureIdpSession: reusing existing IdP session`,
				);
				return silent;
			}
			this._logger.trace(
		...
		private async _exchangeForIdJag(
			idToken: string,
			audience: string,
			resource: string,
			scopes: string[],
		...
		private async _discoverResourceTokenEndpoint(
			audience: string,
		): Promise<string> {
		...
		private async _redeemAtResource(
...
function toSession(
	...
	return {
		...
		account: {
			...
			label:
				claims?.preferred_username || claims?.name || claims?.email || "XAA",
		},
		scopes: [...scopes],
