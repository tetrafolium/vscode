	...
	setOutputItems(
		...
		if (output) {
			...
			if (
				...
				output.items.every((item) => isTextStreamMime(item.mime))
			) {
...
export class ExtHostNotebookDocument {
	...
	acceptModelChanged(
		...
		for (const rawEvent of event.rawEvents) {
			if (
				rawEvent.kind === notebookCommon.NotebookCellsChangeType.ModelChange
			) {
				this._spliceNotebookCells(
					rawEvent.changes,
					false,
				...
				rawEvent.kind === notebookCommon.NotebookCellsChangeType.OutputItem
			) {
				...
				rawEvent.kind === notebookCommon.NotebookCellsChangeType.ChangeCellMime
			) {
				...
				rawEvent.kind ===
				notebookCommon.NotebookCellsChangeType.ChangeCellInternalMetadata
			) {
