...
interface ProviderEntry {
	type: ProviderType;
	provider:
		| vscode.ChatWorkspaceContextProvider
		| vscode.ChatExplicitContextProvider
		| vscode.ChatResourceContextProvider;
	disposables: DisposableStore;
}

export class ExtHostChatContext
	extends Disposable
	implements ExtHostChatContextShape
{
	...
	async $provideWorkspaceChatContext(
		...
		const result =
			(await provider.provideWorkspaceChatContext?.(token)) ??
	...
	async $provideExplicitChatContext(
		handle: number,
		token: CancellationToken,
	): Promise<IChatContextItem[]> {
		this._clearProviderItems(handle);
		...
		const result =
			(await provider.provideExplicitChatContext?.(token)) ??
			(await provider.provideChatContext?.(token)) ??
			[];
		return this._convertItems(handle, result);
	}

	async $resolveExplicitChatContext(
		handle: number,
		...
		return this._doResolve(
			(
				provider.resolveExplicitChatContext ?? provider.resolveChatContext
			)?.bind(provider),
			context,
			extItem,
			token,
	...
	async $provideResourceChatContext(
		...
		const result =
			(await provider.provideResourceChatContext?.(
		...
		if (options.withValue && !item.value) {
			const resolved = await (
				provider.resolveResourceChatContext ?? provider.resolveChatContext
			)?.bind(provider)(result, token);
			item.value = resolved?.value;
			item.tooltip = resolved?.tooltip
				? MarkdownString.from(resolved.tooltip)
				: item.tooltip;
		}
	...
	async $resolveResourceChatContext(
		handle: number,
		context: IChatContextItem,
		token: CancellationToken,
	): Promise<IChatContextItem> {
	...
	async $executeChatContextItemCommand(itemHandle: number): Promise<void> {
		...
		const args = extItem.command.arguments
			? [extItem, ...extItem.command.arguments]
			: [extItem];
	...
	registerChatContextProvider(
		...
		if (provider.provideWorkspaceChatContext) {
			const workspaceProvider: vscode.ChatWorkspaceContextProvider = {
				onDidChangeWorkspaceChatContext:
					provider.onDidChangeWorkspaceChatContext,
				provideWorkspaceChatContext: (token) =>
					provider.provideWorkspaceChatContext!(token),
			};
			disposables.push(
		...
		if (provider.provideChatContextExplicit) {
			const explicitProvider: vscode.ChatExplicitContextProvider = {
				provideExplicitChatContext: (token) =>
					provider.provideChatContextExplicit!(token),
		...
		if (provider.provideChatContextForResource && selector) {
			const resourceProvider: vscode.ChatResourceContextProvider = {
				provideResourceChatContext: (options, token) =>
					provider.provideChatContextForResource!(options, token),
				resolveResourceChatContext: provider.resolveChatContext
					? (context, token) => provider.resolveChatContext!(context, token)
					: (context) => context,
			};
			disposables.push(
				this.registerChatResourceContextProvider(
					selector,
					id,
					resourceProvider,
				),
	...
	private async _doResolve(
		resolveFn: (
			item: vscode.ChatContextItem,
			token: CancellationToken,
		) => vscode.ProviderResult<vscode.ChatContextItem>,
		context: IChatContextItem,
		...
		if (extResult) {
			return {
				...
				tooltip: extResult.tooltip
					? MarkdownString.from(extResult.tooltip)
					: undefined,
	...
	private _listenForWorkspaceContextChanges(
		...
		const provideWorkspaceContext = async () => {
			const workspaceContexts =
				(await provider.provideWorkspaceChatContext?.(
		...
		disposables.add(
			provider.onDidChangeWorkspaceChatContext(async () =>
