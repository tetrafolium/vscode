...
function toGitBranchDto(branch: Branch): GitBranchDto {
	return {
		...
		upstream: branch.upstream
			? toGitUpstreamRefDto(branch.upstream)
			: undefined,
		ahead: branch.ahead,
...
export const IExtHostGitExtensionService =
	createDecorator<IExtHostGitExtensionService>("IExtHostGitExtensionService");

export class ExtHostGitExtensionService
	extends Disposable
	implements IExtHostGitExtensionService
{
	...
	private readonly _repositoryStateChangeListeners = new DisposableMap<
		number,
		vscode.Disposable
	>();
