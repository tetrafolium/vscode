...
class CodeActionAdapter {
	...
	async provideCodeActions(
		...
		for (let i = 0; i < commandsOrActions.length; i++) {
			...
			if (
				CodeActionAdapter._isCommand(candidate) &&
				!(candidate instanceof CodeAction)
			) {
				...
				actions.push({
					...
					command:
						toConvert.command &&
						this._commands.toInternal(toConvert.command, disposables),
					diagnostics:
						toConvert.diagnostics &&
						toConvert.diagnostics.map(typeConvert.Diagnostic.from),
					edit:
						toConvert.edit &&
						typeConvert.WorkspaceEdit.from(toConvert.edit, undefined),
...
class DocumentPasteEditProvider {
	...
	private readonly _editsCache = new Cache<vscode.DocumentPasteEdit>(
	...
	async providePasteEdits(
		...
		const items = dataTransferDto.items.map(
			([mime, value]): [string, vscode.DataTransferItem] => {
	...
	async resolvePasteEdit(
		...
		const resolvedItem =
			(await this._provider.resolveDocumentPasteEdit(item, token)) ?? item;
		return {
...
class NavigateTypeAdapter {
	...
	async resolveWorkspaceSymbol(
		...
		if (item) {
			...
			return (
				value && mixin(symbol, typeConvert.WorkspaceSymbol.from(value), true)
			);
		}
		return undefined;
	}
...
class CompletionsAdapter {
	...
	private _convertCompletionItem(
		...
		const result: extHostProtocol.ISuggestDataDto = {
			...
			[extHostProtocol.ISuggestDataDtoField.commandArguments]: command?.$ident
				? undefined
...
class InlineCompletionAdapter {
	...
	constructor(
		...
		this._isAdditionsProposedApiEnabled = isProposedApiEnabled(
			this._extension,
	...
	public get modelInfo():
		| extHostProtocol.IInlineCompletionModelInfoDto
		| undefined {
	...
	public get providerOptions():
		| readonly extHostProtocol.IInlineCompletionProviderOptionDto[]
		| undefined {
		if (!this._isAdditionsProposedApiEnabled) {
			return undefined;
		}
	...
	async provideInlineCompletions(
		...
		const result = await this._provider.provideInlineCompletionItems(
			...
			pos,
			{
				selectedCompletionInfo: context.selectedSuggestionInfo
					...
					: undefined,
				triggerKind:
					this.languageTriggerKindToVSCodeTriggerKind[context.triggerKind],
		...
		const items = {
			...
			items: resultItems.map<extHostProtocol.IdentifiableInlineCompletion>(
				(item, idx) => {
					...
					return {
						...
						showRange:
							this._isAdditionsProposedApiEnabled && item.showRange
						...
						isInlineEdit: this._isAdditionsProposedApiEnabled
							? item.isInlineEdit
							: false,
						showInlineEditMenu: this._isAdditionsProposedApiEnabled
							? item.showInlineEditMenu
							: false,
						hint:
							item.displayLocation && this._isAdditionsProposedApiEnabled
								? {
										...
										style: item.displayLocation.kind
											? typeConvert.InlineCompletionHintStyle.from(
													item.displayLocation.kind,
												)
						...
						warning:
							item.warning && this._isAdditionsProposedApiEnabled
								...
								: undefined,
						correlationId: this._isAdditionsProposedApiEnabled
							? item.correlationId
							: undefined,
						suggestionId: undefined,
						uri:
							this._isAdditionsProposedApiEnabled && item.uri
								? item.uri
								: undefined,
						supportsRename: this._isAdditionsProposedApiEnabled
							? item.supportsRename
							: false,
						jumpToPosition:
			...
			commands: commands.map((c) => {
				...
				return typeConvert.CompletionCommand.from(
					c,
					this._commands,
					disposableStore,
				);
	...
	disposeCompletions(
		...
		if (
			this._provider.handleListEndOfLifetime &&
	...
	handleEndOfLifetime(
		...
		if (completionItem) {
			if (
				this._provider.handleEndOfLifetime &&
				this._isAdditionsProposedApiEnabled
			) {
	...
	async provideLinks(
		...
		if (typeof this._provider.resolveDocumentLink !== "function") {
			...
			for (let i = 0; i < links.length; i++) {
				...
				const dto: extHostProtocol.ILinkDto = typeConvert.DocumentLink.from(
					links[i],
				);
				dto.cacheId = [pid, i];
	...
	implements extHostProtocol.ExtHostLanguageFeaturesShape
{
	...
	readonly onDidChangeInlineCompletionsUnificationState =
		this._onDidChangeInlineCompletionsUnificationState.event;
	...
	$provideDefinition(
		...
		return this._withAdapter(
			...
			DefinitionAdapter,
			(adapter) =>
				adapter.provideDefinition(URI.revive(resource), position, token),
			[],
			token,
		);
	}
	...
	$provideDeclaration(
		...
		return this._withAdapter(
			...
			DeclarationAdapter,
			(adapter) =>
				adapter.provideDeclaration(URI.revive(resource), position, token),
			[],
			token,
		);
	}
	...
	$provideImplementation(
		...
		return this._withAdapter(
			...
			ImplementationAdapter,
			(adapter) =>
				adapter.provideImplementation(URI.revive(resource), position, token),
			[],
			token,
		);
	}
	...
	$provideTypeDefinition(
		...
		return this._withAdapter(
			...
			TypeDefinitionAdapter,
			(adapter) =>
				adapter.provideTypeDefinition(URI.revive(resource), position, token),
			[],
			token,
		);
	}
	...
	$provideHover(
		...
		return this._withAdapter(
			...
			HoverAdapter,
			(adapter) =>
				adapter.provideHover(URI.revive(resource), position, context, token),
			undefined,
			token,
		);
	}
	...
	$resolveRenameLocation(
		...
		return this._withAdapter(
			...
			RenameAdapter,
			(adapter) =>
				adapter.resolveRenameLocation(URI.revive(resource), position, token),
			undefined,
			token,
		);
	}
	...
	registerInlineCompletionsProvider(
		...
		const supportsOnDidChange =
			...
			typeof provider.onDidChange === "function";
		if (supportsOnDidChange) {
			const subscription = provider.onDidChange!((e) =>
				this._proxy.$emitInlineCompletionsChange(
					handle,
		...
		if (supportsOnDidChangeModelInfo) {
			const subscription = provider.onDidChangeModelInfo!((_) =>
				this._proxy.$emitInlineCompletionModelInfoChange(
					handle,
					adapter.modelInfo,
				),
			);
		...
		if (supportsOnDidChangeProviderOptions) {
			const subscription = provider.onDidChangeProviderOptions!((_) =>
				this._proxy.$emitInlineCompletionProviderOptionsChange(
					handle,
		...
		this._proxy.$registerInlineCompletionsSupport(
			...
			extension.version,
			metadata?.groupId
				? ExtensionIdentifier.toKey(metadata.groupId)
				: undefined,
	...
	$provideSelectionRanges(
		...
		return this._withAdapter(
			...
			SelectionRangeAdapter,
			(adapter) =>
				adapter.provideSelectionRanges(URI.revive(resource), positions, token),
			[],
			token,
		);
	}
	...
	$prepareCallHierarchy(
		...
		return this._withAdapter(
			...
			(adapter) =>
				Promise.resolve(
					adapter.prepareSession(URI.revive(resource), position, token),
				),
			undefined,
			token,
		);
