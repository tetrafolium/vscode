...
export class ExtHostChatDebug
	extends Disposable
	implements ExtHostChatDebugShape
{
	...
	private readonly _onDidAddCoreEvent = this._register(
		new Emitter<vscode.ChatDebugEvent>({
			onWillAddFirstListener: () => this._proxy.$subscribeToCoreDebugEvents(),
			onDidRemoveLastListener: () =>
				this._proxy.$unsubscribeFromCoreDebugEvents(),
		}),
	);
	...
	async $provideChatDebugLog(
		...
		store.add(
			emitter.event((event) => {
				...
				if (!dto.sessionResource) {
					(dto as { sessionResource?: UriComponents }).sessionResource =
						sessionResource;
				}
		...
		try {
			...
			const result = await this._provider.provideChatDebugLog(
				sessionUri,
				progress,
				token,
			);
	...
	private _serializeEvent(event: vscode.ChatDebugEvent): IChatDebugEventDto {
		const base = {
			id: event.id,
			sessionResource: (event as { sessionResource?: vscode.Uri })
				.sessionResource,
		...
		switch (kind) {
			case "toolCall": {
				...
				return {
					...
					output: e.output,
					result:
						e.result === ChatDebugToolCallResult.Success
							? "success"
							: e.result === ChatDebugToolCallResult.Error
								? "error"
								: undefined,
			...
			case "subagentInvocation": {
				...
				return {
					...
					status:
						e.status === ChatDebugSubagentStatus.Running
							? "running"
							: e.status === ChatDebugSubagentStatus.Completed
								? "completed"
								: e.status === ChatDebugSubagentStatus.Failed
									? "failed"
									: undefined,
					durationInMillis: e.durationInMillis,
	...
	async $resolveChatDebugLogEvent(
		_handle: number,
		eventId: string,
		token: CancellationToken,
		...
		const result = await this._provider.resolveChatDebugLogEvent(
			eventId,
			token,
		);
		if (!result) {
		...
		switch (kind) {
			case "text":
				return {
					kind: "text",
					value: (result as vscode.ChatDebugEventTextContent).value,
				};
			case "messageContent": {
				...
				return {
					kind: "message",
					type:
						msg.type === ChatDebugMessageContentType.User ? "user" : "agent",
					message: msg.message,
			...
			case "toolCallContent": {
				...
				return {
					...
					result:
						tc.result === ChatDebugToolCallResult.Success
							? "success"
							: tc.result === ChatDebugToolCallResult.Error
								? "error"
								: undefined,
			...
			case "hookContent": {
				...
				return {
					...
					command: hk.command,
					result:
						hk.result === ChatDebugHookResult.Success
							? "success"
							: hk.result === ChatDebugHookResult.Error
								? "error"
								: hk.result === ChatDebugHookResult.NonBlockingError
									? "nonBlockingError"
									: undefined,
	...
	private _deserializeEvent(
		...
		const sessionResource = dto.sessionResource
			? URI.revive(dto.sessionResource)
			: undefined;
		switch (dto.kind) {
			case "toolCall": {
				...
				evt.result =
					dto.result === "success"
						? ChatDebugToolCallResult.Success
						: dto.result === "error"
							? ChatDebugToolCallResult.Error
							: undefined;
			...
			case "generic": {
				const evt = new ChatDebugGenericEvent(
					dto.name,
			...
			case "subagentInvocation": {
				const evt = new ChatDebugSubagentInvocationEvent(
					dto.agentName,
					created,
				);
				evt.id = dto.id;
				...
				evt.status =
					dto.status === "running"
						? ChatDebugSubagentStatus.Running
						: dto.status === "completed"
							? ChatDebugSubagentStatus.Completed
							: dto.status === "failed"
								? ChatDebugSubagentStatus.Failed
								: undefined;
				evt.durationInMillis = dto.durationInMillis;
			...
			case "userMessage": {
				...
				evt.sessionResource = sessionResource;
				evt.parentEventId = dto.parentEventId;
				evt.sections = dto.sections.map(
					(s) => new ChatDebugMessageSection(s.name, s.content),
				);
				return evt;
			}
			case "agentResponse": {
	...
	async $exportChatDebugLog(
		_handle: number,
		sessionResource: UriComponents,
		coreEventDtos: IChatDebugEventDto[],
		...
		const sessionUri = URI.revive(sessionResource);
		const coreEvents = coreEventDtos
			.map((dto) => this._deserializeEvent(dto))
			.filter((e): e is vscode.ChatDebugEvent => e !== undefined);
		const options: vscode.ChatDebugLogExportOptions = {
			coreEvents,
			sessionTitle,
		};
		const result = await this._provider.provideChatDebugLogExport(
			sessionUri,
			options,
			token,
		);
	...
	async $importChatDebugLog(
		...
		const result = await this._provider.resolveChatDebugLogImport(
			data.buffer,
			token,
		);
		if (!result) {
	...
	async $getAvailableDebugSessionResources(
		_handle: number,
		token: CancellationToken,
		...
		const result =
			await this._provider.provideAvailableDebugSessionResources(token);
		return result ?? [];
	}
