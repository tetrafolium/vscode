...
} from "../../contrib/chat/common/promptSyntax/hookSchema.js";
			...
			.map((c) => {
				if (c.type === "text") {
					...
					const content: (
						...
						c.value.map((part) => {
							if (part.type === "text") {
								return new types.LanguageModelTextPart(
									part.value,
									part.audience,
								);
...
export namespace ChatResponseQuestionCarouselPart {
	...
	function stringToQuestionType(
		type: "text" | "singleSelect" | "multiSelect",
	): vscode.ChatQuestionType {
		switch (type) {
			case "text":
	...
	export function from(
		part: vscode.ChatResponseQuestionCarouselPart,
	): Dto<IChatQuestionCarousel> {
		return {
	...
	export function to(
		part: Dto<IChatQuestionCarousel>,
	): vscode.ChatResponseQuestionCarouselPart {
		const questions = part.questions.map(
			(q) =>
				new types.ChatQuestion(q.id, stringToQuestionType(q.type), q.title, {
					message: q.message
						? typeof q.message === "string"
							? new types.MarkdownString(q.message)
							: MarkdownString.to(q.message)
						: undefined,
					options: q.options?.map((opt) => ({
						id: opt.id,
						label: opt.label,
...
export namespace ChatResponseMultiDiffPart {
	export function from(
		part: vscode.ChatResponseMultiDiffPart,
	): IChatMultiDiffDataSerialized {
		return {
			kind: "multiDiffData",
			multiDiffData: {
	...
	export function to(
		part: IChatMultiDiffDataSerialized,
		...
		const resources = part.multiDiffData.resources.map((resource) => ({
			originalUri: resource.originalUri
				? URI.revive(resource.originalUri)
				: undefined,
			modifiedUri: resource.modifiedUri
				? URI.revive(resource.modifiedUri)
				: undefined,
			goToFileUri: resource.goToFileUri
				? URI.revive(resource.goToFileUri)
				: undefined,
...
export namespace ChatResponseThinkingProgressPart {
	export function from(
		part: vscode.ChatResponseThinkingProgressPart,
	): Dto<IChatThinkingPart> {
		return {
			kind: "thinking",
			value: part.value,
			id: part.id,
...
export namespace ChatResponseInfoPart {
	export function from(
		part: vscode.ChatResponseInfoPart,
	): Dto<IChatInfoMessage> {
		return {
			kind: "info",
			content: MarkdownString.from(part.value),
		};
...
export namespace ChatResponsePullRequestPart {
	export function from(
		part: Omit<vscode.ChatResponsePullRequestPart, "command"> & {
			command?: vscode.Command;
		},
		commandsConverter: CommandsConverter,
		commandDisposables: DisposableStore,
	): Dto<IChatPullRequestContent> {
...
export namespace ChatToolInvocationPart {
	export function from(
	...
	): IChatToolInvocationSerialized | IChatExternalToolInvocationUpdate {
		...
		if (
			...
			isChatMcpToolInvocationData(part.toolSpecificData)
		) {
			...
			toolSpecificData = part.toolSpecificData
				? convertToolSpecificData(part.toolSpecificData)
				: undefined;
		}
		...
		if (part.enablePartialUpdate) {
			return {
				...
				invocationMessage: part.invocationMessage
					? MarkdownString.from(part.invocationMessage)
					: undefined,
				pastTenseMessage: part.pastTenseMessage
					? MarkdownString.from(part.pastTenseMessage)
					: undefined,
				toolSpecificData,
		...
		return {
			...
			originMessage: part.originMessage
				? MarkdownString.from(part.originMessage)
				: undefined,
			pastTenseMessage: part.pastTenseMessage
				? MarkdownString.from(part.pastTenseMessage)
				: undefined,
	...
	function isChatMcpToolInvocationData(
		data: any,
	...
	function convertMcpToResultDetails(
		data: vscode.ChatMcpToolInvocationData,
		isError?: boolean,
	): IToolResultInputOutputDetails {
		return {
			input: data.input,
	...
	function convertToolSpecificData(data: any): any {
		...
		if ("command" in data && "language" in data) {
			...
			const presentationOverrides =
				...
				typeof data.presentationOverrides.commandLine === "string"
					? {
	...
	function todoStatusEnumToString(
		status: types.ChatTodoStatus | string,
	): string {
		// Handle enum values
		switch (status) {
...
export namespace ChatResponseWorkspaceEditPart {
	export function from(
		part: vscode.ChatResponseWorkspaceEditPart,
	): IChatWorkspaceEdit {
		return {
			kind: "workspaceEdit",
...
export namespace ChatAgentRequest {
	export function to(
		...
		const sessionId =
			LocalChatSessionUri.parseLocalSessionId(request.sessionResource) ??
		...
		const requestWithAllProps: vscode.ChatRequest = {
			...
			location2,
			toolInvocationToken: Object.freeze<IToolInvocationContext>({
				sessionResource: request.sessionResource,
			...
			modeInstructions2: ChatRequestModeInstructions.to(
			...
			hooks: request.hooks
				? ChatRequestHooksConverter.to(request.hooks)
				: undefined,
...
export namespace ChatPromptReference {
	export function to(
		...
		let toolReferences;
		if (
			isPromptFileVariableEntry(variable) ||
			isPromptTextVariableEntry(variable)
		) {
...
namespace ChatLanguageModelToolReferences {
	export function to(
		variables: readonly ChatRequestToolReferenceEntry[],
	): vscode.ChatLanguageModelToolReference[] {
...
export namespace ChatRequestModeInstructions {
	export function to(
		mode:
			...
			| Dto<IChatRequestModeInstructions>
			| undefined,
	): vscode.ChatRequestModeInstructions | undefined {
		if (mode) {
			return {
	...
	export function from(
		mode: vscode.ChatRequestModeInstructions | undefined,
	): IChatRequestModeInstructions | undefined {
		if (mode) {
			return {
				uri: mode.uri,
				name: mode.name,
	...
	function reviveMetadata(metadata: IChatAgentResult["metadata"]) {
		return cloneAndChange(metadata, (value) => {
			if (value.$mid === MarshalledId.LanguageModelToolResult) {
				...
				return new types.LanguageModelThinkingPart(
					value.value,
					value.id,
					value.metadata,
				);
	...
	export function from(
		...
		return {
			...
			resourceOptions: completions.resourceOptions
				? TerminalCompletionResourceOptions.from(
...
export namespace InlineCompletionHintStyle {
	export function from(
		value: vscode.InlineCompletionDisplayLocationKind,
	): languages.InlineCompletionHintStyle {
	...
	export function to(
		kind: languages.InlineCompletionHintStyle,
	): types.InlineCompletionDisplayLocationKind {
		switch (kind) {
...
export namespace LanguageModelToolSource {
	...
	): vscode.LanguageModelToolInformation["source"] {
		if (source.type === "mcp") {
...
export namespace LanguageModelToolResult {
	export function to(
		result: IToolResult,
	): vscode.ExtendedLanguageModelToolResult {
	...
	export function from(
		result: vscode.ExtendedLanguageModelToolResult2,
		extension: IExtensionDescription,
		...
		const checkAudienceApi = (
			item: LanguageModelTextPart | LanguageModelDataPart,
		) => {
			if (item.audience) {
		...
		if (Array.isArray(result.toolResultDetails)) {
			detailsDto = result.toolResultDetails?.map((detail) => {
				return URI.isUri(detail)
					? detail
					: Location.from(detail as vscode.Location);
			});
		} else {
			if (result.toolResultDetails2) {
				detailsDto = {
					output: {
						type: "data",
						mimeType: (result.toolResultDetails2 as vscode.ToolResultDataOutput)
							.mime,
						value: VSBuffer.wrap(
							(result.toolResultDetails2 as vscode.ToolResultDataOutput).value,
						),
					},
...
export namespace IconPath {
	...
	export function from(
		value: vscode.IconPath | undefined,
	): extHostProtocol.IconPathDto | undefined;
	export function from(
		value: vscode.IconPath | undefined,
	): extHostProtocol.IconPathDto | undefined {
		if (!value) {
			...
			const dark =
				typeof value.dark === "string" ? URI.file(value.dark) : value.dark;
			const light =
				typeof value.light === "string" ? URI.file(value.light) : value.light;
	...
	export function to(
		value: extHostProtocol.IconPathDto | undefined,
...
export namespace McpServerDefinition {
		...
		return McpServerLaunch.toSerialized(
			isHttpConfig(item)
				? {
						...
						authentication: (item as vscode.McpHttpServerDefinition2)
							.authentication
							? {
									providerId: (item as vscode.McpHttpServerDefinition2)
										.authentication!.providerId,
									scopes: (item as vscode.McpHttpServerDefinition2)
	...
	export function to(
		dto: McpServerDefinitionType.Serialized,
...
export namespace ChatHookCommand {
	export function to(
		hook: IParsedHookCommand,
	): vscode.ChatHookCommand | undefined {
...
export namespace ChatSessionItem {
	function convertStatus(
		status: vscode.ChatSessionStatus | undefined,
	...
	export function from(
		sessionContent: vscode.ChatSessionItem,
	): Dto<IChatSessionItem> {
		...
		return {
			...
			description: sessionContent.description
				? MarkdownString.from(sessionContent.description)
				: undefined,
			badge: sessionContent.badge
				? MarkdownString.from(sessionContent.badge)
				: undefined,
			...
			changes:
				sessionContent.changes instanceof Array
