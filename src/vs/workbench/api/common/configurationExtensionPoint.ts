...
configurationExtPoint.setHandler((extensions, { added, removed }) => {
	...
	function handleConfiguration(
		...
		extension: IExtensionPointUser<unknown>,
	): IConfigurationNode {
	...
	function validateProperties(
		...
		if (properties) {
			...
			for (const key in properties) {
				...
				const message = validateProperty(
					key,
				...
				if (
					...
					!EXTENSION_UNIFICATION_EXTENSION_IDS.has(
						extension.description.identifier.value.toLowerCase(),
					)
				) {
					delete properties[key];
					extension.collector.warn(
				...
				if (
					propertyConfiguration.tags?.some(
						(tag) => tag.toLowerCase() === "onexp",
					)
				) {
				...
				if (
					propertyConfiguration.agentsWindow &&
...
class ConfigurationDefaultsTableRenderer
	extends Disposable
	implements IExtensionFeatureTableRenderer
{
	readonly type = "table";
	...
	render(manifest: IExtensionManifest): IRenderedData<ITableData> {
		...
		for (const key of Object.keys(configurationDefaults).sort((a, b) =>
			a.localeCompare(b),
		)) {
			const value = configurationDefaults[key];
			if (OVERRIDE_PROPERTY_REGEX.test(key)) {
				const languages = overrideIdentifiersFromKey(key);
				const languageMarkdown = new MarkdownString().appendMarkdown(
					`${languages.join(", ")}`,
				);
