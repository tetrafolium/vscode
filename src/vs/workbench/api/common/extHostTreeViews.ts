...
{
	private _treeViews: Map<string, ExtHostTreeView<any>> = new Map<
		string,
		ExtHostTreeView<any>
	>();
	...
	async $handleDrop(
		...
		if (sourceViewId === destinationViewId && sourceTreeItemHandles) {
			await this._addAdditionalTransferItems(
				treeDataTransfer,
				treeView,
	...
	private async _addAdditionalTransferItems(
		...
		if (existingTransferOperation) {
			...
			this._treeDragAndDropService.addDragOperationTransfer(
				operationUuid,
				willDropPromise,
			);
...
class ExtHostTreeView<T> extends Disposable {
	...
	private readonly _dndController:
	...
	private readonly _childrenFetchTokens = new Map<
		T | typeof ExtHostTreeView.ROOT_FETCH_KEY,
		number
	>();
			...
			(result, current) => {
				...
				if (current.element !== false) {
					if (!refreshingPromise) {
						...
						this._refreshPromise = this._refreshPromise.then(
							() => refreshingPromise!,
						);
					}
						...
						return this._refresh(elements)
							...
							.catch((e) => {
								...
								this._logService.error(
									`Unable to refresh tree view ${this._viewId}: ${message}`,
								);
								return _promiseCallback();
							});
					});
	...
	private _debugCollectHandles(elements: (T | Root)[]): {
		changed: string[];
		roots: string[];
		clearing?: string[];
	} {
	...
	private _debugLogRefresh(
		...
		try {
			...
			this._logService.debug(
				`[TreeView:${this._viewId}] refresh ${phase} (snapshot failed)`,
			);
		}
	}
	...
	async getChildren(
	...
	): Promise<readonly ITreeItem[] | undefined> {
		...
		let childrenNodes: TreeNode[] | undefined =
			this._getChildrenNodes(parentHandle); // Get it from cache
	...
	reveal(element: T | undefined, options?: IRevealOptions): Promise<void> {
		...
		if (element) {
			...
			return this._proxy.$reveal(this._viewId, undefined, {
				select,
				focus,
				expand,
			});
	...
	async handleDrag(
		...
		await this._dndController.handleDrag(
			extensionTreeItems,
			treeDataTransfer,
			token,
		);
	...
	async onDrop(
		...
		return asPromise(() =>
			this._dndController?.handleDrop
	...
	async resolveTreeItem(
		...
		if (element) {
			...
			if (node) {
				const resolve =
					(await this._dataProvider.resolveTreeItem(
						node.extensionItem,
						element,
						token,
					)) ?? node.extensionItem;
				...
				node.item.command = this._getCommand(
	...
	private _resolveUnknownParentChain(element: T): Promise<TreeNode[]> {
		return this._resolveParent(element).then((parent) => {
			...
			return this._resolveUnknownParentChain(parent).then((result) =>
				this._resolveTreeNode(parent, result[result.length - 1]).then(
					(parentNode) => {
	...
	private _resolveParent(element: T): Promise<T | Root> {
		...
		if (node) {
			return Promise.resolve(
				node.parent ? this._elements.get(node.parent.item.handle) : undefined,
			);
		}
		return asPromise(() => this._dataProvider.getParent!(element));
	}

	private async _resolveTreeNode(
		element: T,
		parent?: TreeNode,
	): Promise<TreeNode> {
		const node = this._nodes.get(element);
		...
		const extTreeItem = await asPromise(() =>
			this._dataProvider.getTreeItem(element),
		);
		...
		this._logService.error(
			`[TreeView:${this._viewId}] Failed to resolve tree node for element ${handle}`,
		);
	...
	private _getFetchKey(
		parentElement?: T,
	): T | typeof ExtHostTreeView.ROOT_FETCH_KEY {
		return parentElement ?? ExtHostTreeView.ROOT_FETCH_KEY;
	}

	private async _fetchChildrenNodes(
		parentElement?: T,
		...
		try {
			...
			const items = treeItems.map((item, index) =>
				item
					? this._createAndRegisterTreeNode(
	...
	private _getHandlesToRefresh(elements: T[]): TreeItemHandle[] {
		...
		for (const elementNode of elementNodes) {
			if (elementNode && !elementsToUpdate.has(elementNode.item.handle)) {
				...
				while (
					...
					const parentElement: T | undefined = this._elements.get(
		...
		elementsToUpdate.forEach((handle) => {
			...
			if (element) {
				...
				if (
					node &&
					(!node.parent || !elementsToUpdate.has(node.parent.item.handle))
				) {
	...
	private _refreshHandles(itemHandles: TreeItemHandle[]): Promise<void> {
		...
		return Promise.all(
			...
			Object.keys(itemsToRefresh).length
				? this._proxy.$refresh(this._viewId, itemsToRefresh)
				: undefined,
		);
	}

	private _refreshNode(
		...
		if (extElement) {
			...
			if (existing) {
				...
				return asPromise(() => this._dataProvider.getTreeItem(extElement)).then(
					(extTreeItem) => {
						if (extTreeItem) {
							const newNode = this._createTreeNode(
								extElement,
								extTreeItem,
								existing.parent,
							);
	...
	private _createAndRegisterTreeNode(
		element: T,
		extTreeItem: vscode.TreeItem,
		parentNode: TreeNode | Root,
	): TreeNode {
		const duplicateHandle = extTreeItem.id
			? `${ExtHostTreeView.ID_HANDLE_PREFIX}/${extTreeItem.id}`
			: undefined;
		if (duplicateHandle) {
	...
	private _getCommand(
		...
		return command
			? {
					...this._commands.toInternal(command, disposable),
	...
	private _createHandle(
		...
		elementId =
			elementId.indexOf("/") !== -1 ? elementId.replace("/", "//") : elementId;
		const existingHandle = this._nodes.has(element)
			? this._nodes.get(element)!.item.handle
			: undefined;
	...
	private _getDarkIconPath(
		extensionTreeItem: vscode.TreeItem,
	): URI | undefined {
		if (
			extensionTreeItem.iconPath &&
			!(extensionTreeItem.iconPath instanceof extHostTypes.ThemeIcon) &&
			(<{ light: string | URI; dark: string | URI }>extensionTreeItem.iconPath)
				.dark
		) {
			return this._getIconPath(
				(<{ light: string | URI; dark: string | URI }>(
