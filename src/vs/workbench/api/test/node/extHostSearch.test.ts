...
function extensionResultIsMatch(
	data: vscode.TextSearchResult,
): data is vscode.TextSearchMatch {
...
suite("ExtHostSearch", () => {
	...
	async function registerTestTextSearchProvider(
	...
	async function registerTestFileSearchProvider(
	...
	async function runFileSearch(
		query: IFileQuery,
		cancel = false,
	): Promise<{ results: URI[]; stats: ISearchCompleteStats }> {
		...
		await rpcProtocol.sync();
		return {
			results: (<UriComponents[]>mockMainThreadSearch.results).map((r) =>
				URI.revive(r),
			),
			stats: stats!,
		};
	...
	async function runTextSearch(
		query: ITextQuery,
	): Promise<{ results: IFileMatch[]; stats: ISearchCompleteStats }> {
		let stats: ISearchCompleteStats;
		try {
			const cancellation = new CancellationTokenSource();
		...
		await rpcProtocol.sync();
		const results: IFileMatch[] = revive(
			<IRawFileMatch2[]>mockMainThreadSearch.results,
		);
							...
							override async getConfigProvider(): Promise<ExtHostConfigProvider> {
								return {
									onDidChangeConfiguration(
										_listener: (event: vscode.ConfigurationChangeEvent) => void,
									) {},
	...
	suite("File:", () => {
		...
		function compareURIs(actual: URI[], expected: URI[]) {
			const sortAndStringify = (arr: URI[]) =>
				arr.sort().map((u) => u.toString());
		...
		test("basic sibling exclude clause", async () => {
			...
			await registerTestFileSearchProvider({
				provideFileSearchResults(
					...
					return Promise.resolve(
						reportedResults.map((relativePath) =>
							joinPath(options.folder, relativePath),
						),
					);
				},
			});
			...
			await registerTestFileSearchProvider({
				provideFileSearchResults(
					...
					if (options.folder.fsPath === rootFolderA.fsPath) {
						...
						reportedResults = ["fileB.ts", "fileB.js", "file3.js"].map(
		...
		test("multiroot max results", async () => {
			...
			await registerTestFileSearchProvider({
				async provideFileSearchResults(
					...
					return ["file1.ts", "file2.ts", "file3.ts"].map((relativePath) =>
						joinPath(options.folder, relativePath),
					);
				},
			});
	...
	suite("Text:", () => {
		...
		function assertResults(
			actual: IFileMatch[],
			expected: vscode.TextSearchResult[],
		) {
			...
			const rangeToString = (r: vscode.Range) =>
				`(${r.start.line}, ${r.start.character}), (${r.end.line}, ${r.end.character})`;

			const makeComparable = (results: vscode.TextSearchResult[]) =>
				results
					.sort((a, b) => {
						const compareKeyA =
							a.uri.toString() +
							": " +
							(extensionResultIsMatch(a) ? a.preview.text : a.text);
						const compareKeyB =
							b.uri.toString() +
							": " +
							(extensionResultIsMatch(b) ? b.preview.text : b.text);
						return compareKeyB.localeCompare(compareKeyA);
					})
					.map((r) =>
						extensionResultIsMatch(r)
							? {
									uri: r.uri.toString(),
		...
		test("multiroot max results", async () => {
			...
			await registerTestTextSearchProvider({
				async provideTextSearchResults(
					...
					disposables.add(token.onCancellationRequested(() => cancels++));
					await new Promise((r) => process.nextTick(r));
					["file1.ts", "file2.ts", "file3.ts"].forEach((f) =>
						progress.report(makeTextResult(options.folder, f)),
					);
					return null!;
				},
			});
