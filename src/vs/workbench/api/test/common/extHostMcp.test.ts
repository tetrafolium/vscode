...
async function createTestAuthMetadata(options: {
	...
	const mockLogger = (level: LogLevel, message: string) =>
...
suite("ExtHostMcp", () => {
	...
	suite("IAuthMetadata", () => {
		suite("properties", () => {
			test("should expose readonly properties", async () => {
				...
				assert.ok(
					authMetadata.authorizationServer
						.toString()
						.startsWith(TEST_AUTH_SERVER),
				);
				assert.strictEqual(
					authMetadata.serverMetadata.issuer,
					TEST_AUTH_SERVER,
				);
	...
	suite("createAuthMetadata", () => {
		...
		test("should fall back to default metadata when server metadata fetch fails", async () => {
			...
			assert.ok(
				authMetadata.serverMetadata.issuer.startsWith("https://example.com"),
			);
			assert.ok(
				authMetadata.serverMetadata.authorization_endpoint?.startsWith(
					"https://example.com/authorize",
				),
			);
			assert.ok(
				authMetadata.serverMetadata.token_endpoint?.startsWith(
		...
		test("should use resource_metadata challenge URL from WWW-Authenticate header", async () => {
			...
			const originalResponse = createMockResponse({
				...
				headers: {
					"WWW-Authenticate":
						'Bearer resource_metadata="https://example.com/custom-resource-metadata"',
				},
			});
		...
		test("should pass launch headers when fetching metadata from same origin", async () => {
			...
			assert.ok(
				firstCallArgs.length >= 2,
				"fetch should have been called with options",
			);
		...
		test("should handle empty scope string in WWW-Authenticate header", async () => {
			...
			assert.ok(
				authMetadata.scopes === undefined ||
					(Array.isArray(authMetadata.scopes) &&
						authMetadata.scopes.length === 0) ||
					(Array.isArray(authMetadata.scopes) &&
						authMetadata.scopes.every((s) => s === "")),
