...
suite("extHostTypeConverters", function () {
	...
	suite("IconPath", function () {
		suite("from", function () {
			...
			test("ThemeIcon", function () {
				const themeIcon = new ThemeIcon(
					"account",
					new ThemeColor("testing.iconForeground"),
				);
			...
			test("dark only", function () {
				...
				// eslint-disable-next-line local/code-no-any-casts
				const result = IconPath.from(input as any) as unknown as {
					dark: URI;
					light: URI;
				};
			...
			test("dark/light", function () {
				const input = {
					light: URI.file("/path/to/light.png"),
					dark: URI.file("/path/to/dark.png"),
				};
		...
		suite("to", function () {
			...
			test("URI", function () {
				const uri: UriComponents = {
					scheme: "data",
					path: "image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==",
				};
			...
			test("dark/light", function () {
				...
				assert.ok(URI.isUri(result.dark));
				assert.strictEqual(
					result.dark.toString(),
					URI.revive(input.dark).toString(),
				);
				assert.strictEqual(
					result.light.toString(),
					URI.revive(input.light).toString(),
				);
			});
		});
	});
	...
	suite("ChatRequestModeInstructions", function () {
		...
		test("from returns undefined for undefined input", function () {
			assert.strictEqual(
				ChatRequestModeInstructions.from(undefined),
				undefined,
			);
		});
