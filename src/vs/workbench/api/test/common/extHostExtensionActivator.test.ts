...
suite("ExtensionsActivator", () => {
	...
	test("calls activate only once with sequential activations", async () => {
		...
		const activator = createActivator(host, [desc(idA)], [], disposables);
	...
	test("activates dependencies first", async () => {
		...
		const activator = createActivator(
			host,
			[desc(idA, [idB], ["evt1"]), desc(idB, [], ["evt1"])],
			[],
			disposables,
		);
	...
	function createActivator(
		...
		const registry = disposables.add(
			new ExtensionDescriptionRegistry(
				basicActivationEventsReader,
				extensionDescriptions,
			),
		);
		const globalRegistry = disposables.add(
			new ExtensionDescriptionRegistry(
				basicActivationEventsReader,
