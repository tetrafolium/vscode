...
} from "../../browser/mainThreadChatSessions.js";
import {
...
suite("ObservableChatSession", function () {
	...
	setup(function () {
		...
		proxy = {
			...
			$provideChatSessionProviderOptions: sinon
				.stub<
					[providerHandle: number, token: CancellationToken],
	...
	async function createInitializedSession(
		sessionContent: any,
		sessionId = "test-id",
	): Promise<ObservableChatSession> {
		const resource = LocalChatSessionUri.forSession(sessionId);
		const session = new ObservableChatSession(
			resource,
			1,
			proxy,
			logService,
		...
		asSinonMethodStub(proxy.$provideChatSessionContent).resolves(
			sessionContent,
		);
	...
	test("constructor creates session with proper initial state", function () {
		...
		const session = disposables.add(
			new ObservableChatSession(resource, 1, proxy, logService, dialogService),
		);
	...
	test("session queues progress before initialization and processes it after", async function () {
		...
		const progress1: IChatProgress = {
			kind: "progressMessage",
			content: { value: "Hello", isTrusted: false },
		};
		const progress2: IChatProgress = {
			kind: "progressMessage",
			content: { value: "World", isTrusted: false },
		};
	...
	test("initialization loads session history and sets up capabilities", async function () {
		const sessionHistory = [
			{ type: "request", prompt: "Previous question" },
			{
				...
				parts: [
					{
						kind: "progressMessage",
						content: { value: "Previous answer", isTrusted: false },
					},
				],
			},
		];
		...
		const session = disposables.add(
			await createInitializedSession(sessionContent),
		);
	...
	test("toRequestDto passes modeInstructions through", async function () {
		const session = disposables.add(
			await createInitializedSession(
				createSessionContent({ hasForkHandler: true }),
			),
		);
	...
	test("initialization sets forkSession and revives forked items", async function () {
		...
		assert.ok(
			asSinonMethodStub(proxy.$forkChatSession).calledOnceWithExactly(
				1,
	...
	test("initialization forwards initial session options context", async function () {
		...
		assert.ok(
			asSinonMethodStub(proxy.$provideChatSessionContent).calledOnceWith(
				1,
				resource,
				{ initialSessionOptions },
				CancellationToken.None,
	...
	test("progress handling works correctly after initialization", async function () {
		...
		const progress: IChatProgress = {
			kind: "progressMessage",
			content: { value: "New progress", isTrusted: false },
		};
	...
	test("progress completion updates session state correctly", async function () {
		...
		const progress: IChatProgress = {
			kind: "progressMessage",
			content: { value: "Processing...", isTrusted: false },
		};
		session.handleProgressChunk("req1", [progress]);
	...
	test("request handler forwards requests to proxy", async function () {
		...
		assert.ok(
			asSinonMethodStub(proxy.$invokeChatSessionRequestHandler).calledOnceWith(
				1,
				session.sessionResource,
				request,
				[],
	...
	test("request handler forwards progress updates to external callback", async function () {
		...
		asSinonMethodStub(proxy.$invokeChatSessionRequestHandler).returns(
			requestPromise,
		);

		const requestHandlerPromise = session.requestHandler!(
			request,
			progressCallback,
			[],
			CancellationToken.None,
		...
		const progress1: IChatProgress = {
			kind: "progressMessage",
			content: { value: "Progress 1", isTrusted: false },
		};
		const progress2: IChatProgress = {
			kind: "progressMessage",
			content: { value: "Progress 2", isTrusted: false },
		};
	...
	test("dispose properly cleans up resources and notifies listeners", function () {
		...
		assert.ok(
			asSinonMethodStub(proxy.$disposeChatSessionContent).calledOnceWith(
				1,
				resource,
			),
		);
	...
	test("session with multiple request/response pairs in history", async function () {
		const sessionHistory = [
			{ type: "request", prompt: "First question" },
			{
				...
				parts: [
					{
						kind: "progressMessage",
						content: { value: "First answer", isTrusted: false },
					},
				],
			},
			{ type: "request", prompt: "Second question" },
			{
				...
				parts: [
					{
						kind: "progressMessage",
						content: { value: "Second answer", isTrusted: false },
					},
				],
			},
		];
		...
		assert.strictEqual(session.history[1].type, "response");
		assert.strictEqual(
			(session.history[1].parts[0] as IChatProgressMessage).content.value,
			"First answer",
		);
		...
		assert.strictEqual(
			(session.history[3].parts[0] as IChatProgressMessage).content.value,
			"Second answer",
		);
...
suite("MainThreadChatSessions", function () {
	...
	setup(function () {
		...
		chatSessionsService = disposables.add(
			instantiationService.createInstance(ChatSessionsService),
		);
	...
	test("provideChatSessionContent creates and initializes session", async function () {
		...
		const session1 = await chatSessionsService.getOrCreateChatSession(
			resource,
			CancellationToken.None,
		);
		...
		const session2 = await chatSessionsService.getOrCreateChatSession(
			resource,
			CancellationToken.None,
		);
	...
	test("provideChatSessionContent propagates title", async function () {
		...
		const session = await chatSessionsService.getOrCreateChatSession(
			resource,
			CancellationToken.None,
		);
	...
	test("$handleProgressChunk routes to correct session", async function () {
		...
		const progressDto: IChatProgressDto = {
			kind: "progressMessage",
			content: { value: "Test", isTrusted: false },
		};
	...
	test("$onDidChangeChatSessionProviderOptions refreshes option groups", async function () {
		...
		const provideOptionsStub = asSinonMethodStub(
			proxy.$provideChatSessionProviderOptions,
		);
		provideOptionsStub
			.onFirstCall()
			.resolves({ optionGroups: optionGroups1 } as IChatSessionProviderOptions);
		provideOptionsStub
			.onSecondCall()
			.resolves({ optionGroups: optionGroups2 } as IChatSessionProviderOptions);
		...
		let storedGroups =
			chatSessionsService.getOptionGroupsForSessionType(sessionScheme);
	...
	test("getSessionOption returns undefined for unset options", async function () {
		...
		await chatSessionsService.getOrCreateChatSession(
			resource,
			CancellationToken.None,
		);
		...
		assert.strictEqual(
			chatSessionsService.getSessionOption(resource, "models"),
			undefined,
		);
		assert.strictEqual(
			chatSessionsService.getSessionOption(resource, "anyOption"),
			undefined,
		);
	...
	test("getSessionOption returns value for explicitly set options", async function () {
		...
		assert.deepStrictEqual(
			chatSessionsService.getSessionOption(resource, "region"),
			{ id: "us-east", name: "US East" },
		);

		// getSessionOption should return undefined for options not in the session
		assert.strictEqual(
			chatSessionsService.getSessionOption(resource, "notConfigured"),
			undefined,
		);
	...
	test("option change notifications fail silently when provider not registered", async function () {
		...
		chatSessionsService.updateSessionOptions(
			resource,
			new Map([["models", "gpt-4-turbo"]]),
		);

		// Verify the extension was NOT notified (no provider registered)
		assert.strictEqual(
			asSinonMethodStub(proxy.$provideHandleOptionsChange).callCount,
			0,
		);
	});
	...
	test("$updateChatSessionInputState applies selected options only to the targeted session", async function () {
		...
		mainThread.$registerChatSessionItemController(
			controllerHandle,
			sessionScheme,
			false,
		);
		...
		asSinonMethodStub(proxy.$provideChatSessionContent)
			.withArgs(
				sinon.match.any,
				sinon.match((r: URI) => r.toString() === resourceA.toString()),
				sinon.match.any,
				sinon.match.any,
			)
			.resolves({
				...
				hasForkHandler: false,
				supportsInterruption: false,
			} satisfies IChatSessionDto);
		asSinonMethodStub(proxy.$provideChatSessionContent)
			.withArgs(
				sinon.match.any,
				sinon.match((r: URI) => r.toString() === resourceB.toString()),
				sinon.match.any,
				sinon.match.any,
			)
			.resolves({
		...
		assert.deepStrictEqual(
			chatSessionsService.getSessionOption(resourceA, "models"),
			{ id: "modelB", name: "Model B" },
		);
		assert.strictEqual(
			chatSessionsService.getSessionOption(resourceB, "models"),
			undefined,
		);
	...
	test("resolveChatSessionItem invokes proxy and updates item", async function () {
		...
		const result = await chatSessionsService.resolveChatSessionItem(
			sessionScheme,
			resource,
			CancellationToken.None,
		);
	...
	test("resolveChatSessionItem cache is invalidated on item update", async function () {
		...
		const resolvedItem1: Dto<IChatSessionItem> = {
			resource,
			label: "Session A",
			timing,
			badge: "first",
		};
		const resolvedItem2: Dto<IChatSessionItem> = {
			resource,
			label: "Session A",
			timing,
			badge: "second",
		...
		const result2 = await chatSessionsService.resolveChatSessionItem(
			sessionScheme,
			resource,
			CancellationToken.None,
		);
	...
	test("resolveChatSessionItem caches undefined result until item update invalidates it", async function () {
		...
		const result3 = await chatSessionsService.resolveChatSessionItem(
			sessionScheme,
			resource,
			CancellationToken.None,
		);
	...
	test("resolveChatSessionItem ignores stale in-flight resolve result after item update", async function () {
		...
		asSinonMethodStub(proxy.$resolveChatSessionItem).returns(
			new Promise<Dto<IChatSessionItem>>((resolve) => {
				resolvePending = resolve;
			}),
		);

		const pendingResolve = chatSessionsService.resolveChatSessionItem(
			sessionScheme,
			resource,
			CancellationToken.None,
		);
...
suite("ExtHostChatSessions", function () {
	...
	setup(function () {
		...
		const languageModels = new ExtHostLanguageModels(
			rpcProtocol,
			new NullLogService(),
			new (class extends mock<IExtHostAuthentication>() {})(),
		);

		extHostChatSessions = disposables.add(
	...
	test("controller only advertises resolve support after resolve handler is assigned", function () {
		...
		const controller = disposables.add(
			extHostChatSessions.createChatSessionItemController(
				nullExtensionDescription,
				sessionScheme,
				async () => {},
			),
		...
		assert.ok(
			mainThreadChatSessionsProxy.$registerChatSessionItemController.calledOnceWithExactly(
				0,
				sessionScheme,
				false,
			),
		...
		assert.ok(
			mainThreadChatSessionsProxy.$updateChatSessionItemControllerCapabilities
				.notCalled,
		);

		controller.resolveChatSessionItem = async () => {};
		assert.ok(
			mainThreadChatSessionsProxy.$updateChatSessionItemControllerCapabilities.calledOnceWithExactly(
				0,
				true,
			),
		);
		...
		assert.ok(
			mainThreadChatSessionsProxy.$updateChatSessionItemControllerCapabilities.secondCall.calledWithExactly(
				0,
				false,
			),
		);
	...
	test("advertises controller fork support when only the controller registers a fork handler", async function () {
		...
		controller.forkHandler = async (resource) =>
			controller.createChatSessionItem(
				resource.with({ path: "/forked-session" }),
				"Forked Session",
			);

		disposables.add(
			extHostChatSessions.registerChatSessionContentProvider(
				nullExtensionDescription,
				sessionScheme,
				undefined!,
		...
		const session = await extHostChatSessions.$provideChatSessionContent(
			0,
			sessionResource,
			{ initialSessionOptions: [] },
			CancellationToken.None,
	...
	test("prefers controller fork handler over deprecated session fork handler", async function () {
		...
		const controllerItem = controller.createChatSessionItem(
			URI.parse(`${sessionScheme}:/forked-by-controller`),
		...
		await extHostChatSessions.$provideChatSessionContent(
			0,
			sessionResource,
			{ initialSessionOptions: [] },
			CancellationToken.None,
		);
		const result = await extHostChatSessions.$forkChatSession(
			0,
			sessionResource,
			{
				type: "request",
		...
		assert.ok(
			controllerForkHandler.calledOnceWithExactly(
				sessionResource,
				requestTurn,
				CancellationToken.None,
			),
		...
		assert.strictEqual(
			...
			controllerItem.resource.toString(),
		);
	...
	test("falls back to deprecated session fork handler when no controller fork handler exists", async function () {
		...
		assert.ok(
			deprecatedSessionForkHandler.calledOnceWithExactly(
				sessionResource,
				requestTurn,
				CancellationToken.None,
			),
		...
		assert.strictEqual(
			result.resource.toString(),
			`${sessionScheme}:/forked-by-session`,
		);
