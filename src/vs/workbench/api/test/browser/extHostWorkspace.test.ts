...
function createExtHostWorkspace(
	...
	mainContext.set(
		...
		new (class extends mock<MainThreadTelemetryShape>() {
...
suite("ExtHostWorkspace", function () {
	...
	test("asRelativePath", () => {
		const ws = createExtHostWorkspace(
			new TestRPCProtocol(),
			{
				id: "foo",
				folders: [
					aWorkspaceFolderData(URI.file("/Coding/Applications/NewsWoWBot"), 0),
				],
				name: "Test",
			},
			new NullLogService(),
	...
	function asUpdateWorkspaceFolderData(
		uri: URI,
		name?: string,
	): { uri: URI; name?: string } {
		return { uri, name };
	...
	suite("findFiles -", function () {
		test("string include", () => {
			...
			return ws
				.findFiles("foo", undefined, 10, new ExtensionIdentifier("test"))
				.then(() => {
		...
		function testFindFilesInclude(pattern: RelativePattern) {
			...
			return ws
				.findFiles(pattern, undefined, 10, new ExtensionIdentifier("test"))
				.then(() => {
					assert(mainThreadCalled, "mainThreadCalled");
				});
		}
		...
		test("no dups", () => {
			...
			return ws
				...
				.then((uris) => {
					...
					assert.equal(
						uris[0].toString(),
						URI.file(root + "/main.py").toString(),
					);
				});
		});
