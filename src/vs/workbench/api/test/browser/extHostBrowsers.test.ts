...
suite("ExtHostBrowsers", () => {
	...
	function createExtHostBrowsers(
		overrides?: Partial<MainThreadBrowsersShape>,
	): ExtHostBrowsers {
		const proxy = new (class extends mock<MainThreadBrowsersShape>() {
			override $openBrowserTab(): Promise<BrowserTabDto> {
				return Promise.resolve(createDto());
			}
			override $startCDPSession(): Promise<void> {
				return Promise.resolve();
			}
			override $closeCDPSession(): Promise<void> {
				return Promise.resolve();
			}
			override $sendCDPMessage(): Promise<void> {
				return Promise.resolve();
			}
			override $closeBrowserTab(): Promise<void> {
				return Promise.resolve();
			}
		})();
		if (overrides) {
	...
	test("browserTabs populates from $onDidOpenBrowserTab", () => {
		...
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", url: "https://one.com", title: "One" }),
		);
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b2", url: "https://two.com", title: "Two" }),
		);
	...
	test("openBrowserTab reuses existing tab when IDs match", async () => {
		const extHost = createExtHostBrowsers({
			$openBrowserTab: () =>
				Promise.resolve(createDto({ id: "same", url: "https://updated.com" })),
		});

		extHost.$onDidOpenBrowserTab(
			createDto({ id: "same", url: "https://original.com" }),
		);
	...
	test("openBrowserTab forwards options to proxy", async () => {
		...
		let capturedOptions:
			| { preserveFocus?: boolean; inactive?: boolean }
			| undefined;
		const extHost = createExtHostBrowsers({
			$openBrowserTab: (
				_url: string,
				viewColumn?: number,
				options?: { preserveFocus?: boolean; inactive?: boolean },
			) => {
	...
	test("$onDidOpenBrowserTab fires event", () => {
		...
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", url: "https://opened.com" }),
		);
	...
	test("$onDidCloseBrowserTab removes tab and fires event", () => {
		...
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", url: "https://old.com" }),
		);
		extHost.$onDidChangeBrowserTabState(
			createDto({ id: "b1", url: "https://new.com" }),
		);
	...
	test("$onDidChangeBrowserTabState does not fire when data is unchanged", () => {
		const extHost = createExtHostBrowsers();
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", url: "https://example.com", title: "Old Title" }),
		);

		extHost.$onDidChangeBrowserTabState(
			createDto({ id: "b1", url: "https://example.com", title: "New Title" }),
		);
	...
	test("$onDidChangeActiveBrowserTab fires event", () => {
		...
		store.add(
			extHost.onDidChangeActiveBrowserTab((tab) =>
				activeChanges.push(tab?.url),
			),
		);
	...
	test("icon is globe ThemeIcon when no favicon", () => {
		...
		assert.strictEqual(
			(extHost.browserTabs[0].icon as { id: string }).id,
			"globe",
		);
	});
	...
	test("icon is URI when favicon is provided", () => {
		const extHost = createExtHostBrowsers();
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", favicon: "https://example.com/favicon.ico" }),
		);

		assert.strictEqual(
			String(extHost.browserTabs[0].icon),
	...
	test("icon updates when favicon changes", () => {
		...
		extHost.$onDidChangeBrowserTabState(
			createDto({ id: "b1", favicon: "https://example.com/new.ico" }),
		);
		assert.strictEqual(
			String(extHost.browserTabs[0].icon),
			"https://example.com/new.ico",
		);
	});
	...
	test("icon reverts to globe when favicon is cleared", () => {
		const extHost = createExtHostBrowsers();
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", favicon: "https://example.com/icon.ico" }),
		);
		assert.strictEqual(
			String(extHost.browserTabs[0].icon),
		...
		extHost.$onDidChangeBrowserTabState(
			createDto({ id: "b1", favicon: undefined }),
		);
		assert.strictEqual(
			(extHost.browserTabs[0].icon as { id: string }).id,
			"globe",
	...
	test("tab properties are not directly writable", () => {
		const extHost = createExtHostBrowsers();
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", url: "https://example.com", title: "Title" }),
		);
		const tab = extHost.browserTabs[0];

		// Attempting to assign to getter-only properties should either throw or be silently ignored
		assert.throws(() => {
			(tab as unknown as Record<string, unknown>).url = "https://hacked.com";
		});
		assert.throws(() => {
			(tab as unknown as Record<string, unknown>).title = "Hacked";
		});
	...
	test("sendMessage validates message structure", async () => {
		...
		await assert.rejects(
			Promise.resolve().then(() => session.sendMessage(null as never)),
			/must be an object/,
		);
		await assert.rejects(
			Promise.resolve().then(() =>
				session.sendMessage({ method: "Foo" } as never),
			),
			/numeric id/,
		);
		await assert.rejects(
			Promise.resolve().then(() => session.sendMessage({ id: 1 } as never)),
			/method string/,
		);
	});
	...
	test("sendMessage rejects after session is closed", async () => {
		...
		await assert.rejects(
			Promise.resolve().then(() =>
				session.sendMessage({ id: 1, method: "Foo" }),
			),
			/closed/,
		);
	});
	...
	test("$onCDPSessionMessage delivers to correct session", async () => {
		...
		extHost.$onCDPSessionMessage(capturedIds[1], {
			id: 1,
			result: { data: "hello" },
		});
	...
	test("tab object reference is stable across updates", () => {
		const extHost = createExtHostBrowsers();
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", url: "https://old.com", title: "Old" }),
		);
		const tabBefore = extHost.browserTabs[0];

		extHost.$onDidChangeBrowserTabState(
			createDto({ id: "b1", url: "https://new.com", title: "New" }),
		);
	...
	test("closing one tab does not affect others", () => {
		const extHost = createExtHostBrowsers();
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b1", url: "https://one.com" }),
		);
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b2", url: "https://two.com" }),
		);
		extHost.$onDidOpenBrowserTab(
			createDto({ id: "b3", url: "https://three.com" }),
		);
		...
		assert.deepStrictEqual(
			extHost.browserTabs.map((t) => t.url),
			["https://one.com", "https://three.com"],
		);
	});
