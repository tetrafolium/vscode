...
suite("ExtHostConfiguration", function () {
	class RecordingShape extends mock<MainThreadConfigurationShape>() {
		...
		override $updateConfigurationOption(
	...
	function createExtHostConfiguration(
		contents: any = Object.create(null),
	...
	test("getConfiguration fails regression test 1.7.1 -> 1.8 #15552", function () {
		...
		assert.strictEqual(
			extHostConfig.getConfiguration("search").get<any>("exclude")[
				"**/node_modules"
			],
			true,
		);
	...
	test("get nested config", () => {
		...
		assert.deepStrictEqual(
			all.getConfiguration("farboo.nested").get("config1"),
			42,
		);
		assert.deepStrictEqual(
			all.getConfiguration("farboo.nested").get("config2"),
			"Das Pferd frisst kein Reis.",
		);
		assert.deepStrictEqual(
			all.getConfiguration("farboo.nested")["config1"],
			42,
		);
		assert.deepStrictEqual(
			all.getConfiguration("farboo.nested")["config2"],
			"Das Pferd frisst kein Reis.",
		);
		assert.deepStrictEqual(
			all.getConfiguration("farboo.nested1").get("config1"),
			undefined,
		);
		assert.deepStrictEqual(
			all.getConfiguration("farboo.nested1").get("config2"),
			undefined,
		);
		assert.deepStrictEqual(
			all.getConfiguration("farboo.config0.config1").get("a"),
			undefined,
		);
		assert.deepStrictEqual(
			all.getConfiguration("farboo.config0.config1")["a"],
			undefined,
		);
	});
	...
	test("inspect in no workspace context", function () {
		const testObject = new ExtHostConfigProvider(
			...
			createExtHostWorkspace(),
			{
				...
				userLocal: new ConfigurationModel(
					...
					["editor.wordWrap", "editor.lineNumbers"],
					[],
					undefined,
					new NullLogService(),
				),
		...
		let actual: ConfigurationInspect<string> = testObject
			.getConfiguration()
		...
		assert.strictEqual(
			testObject.getConfiguration("editor").get("fontSize"),
			"12px",
		);
	...
	test("inspect in single root context", function () {
		...
		let actual1: ConfigurationInspect<string> = testObject
			.getConfiguration()
			.inspect("editor.wordWrap")!;
		...
		let actual2: ConfigurationInspect<string> = testObject
	...
	test("inspect in multi root context", function () {
		...
		actual2 = testObject
			.getConfiguration("editor", firstRoot)
			.inspect("wordWrap")!;
		assert.strictEqual(actual2.defaultValue, "off");
		assert.strictEqual(actual2.globalValue, "on");
		assert.strictEqual(actual2.globalLocalValue, "on");
		...
		actual2 = testObject
			.getConfiguration(undefined, secondRoot)
			.inspect("editor.wordWrap")!;
		assert.strictEqual(actual2.defaultValue, "off");
		assert.strictEqual(actual2.globalValue, "on");
		assert.strictEqual(actual2.globalLocalValue, "on");
		...
		actual2 = testObject
			.getConfiguration("editor", secondRoot)
			.inspect("wordWrap")!;
		assert.strictEqual(actual2.defaultValue, "off");
		assert.strictEqual(actual2.globalValue, "on");
		assert.strictEqual(actual2.globalLocalValue, "on");
		...
		actual2 = testObject
			.getConfiguration(undefined, thirdRoot)
			.inspect("editor.wordWrap")!;
		assert.strictEqual(actual2.defaultValue, "off");
		assert.strictEqual(actual2.globalValue, "on");
		assert.strictEqual(actual2.globalLocalValue, "on");
		...
		actual2 = testObject
			.getConfiguration("editor", thirdRoot)
			.inspect("wordWrap")!;
		assert.strictEqual(actual2.defaultValue, "off");
		assert.strictEqual(actual2.globalValue, "on");
		assert.strictEqual(actual2.globalLocalValue, "on");
	...
	test("configuration change event", (done) => {
		...
		const configEventData: IConfigurationChange = {
		...
		store.add(
			testObject.onDidChangeConfiguration((e) => {
				...
				assert.ok(
					e.affectsConfiguration("farboo.updatedConfig", workspaceFolder.uri),
				);
				assert.ok(
					e.affectsConfiguration("farboo.updatedConfig", URI.file("any")),
				);

				assert.ok(e.affectsConfiguration("farboo.newConfig"));
				assert.ok(
					e.affectsConfiguration("farboo.newConfig", workspaceFolder.uri),
				);
				assert.ok(e.affectsConfiguration("farboo.newConfig", URI.file("any")));

				assert.ok(!e.affectsConfiguration("farboo.config"));
				assert.ok(
					!e.affectsConfiguration("farboo.config", workspaceFolder.uri),
				);
