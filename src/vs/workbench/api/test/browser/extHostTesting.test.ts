...
const assertTreesEqual = (
	a: TestItemImpl | undefined,
	b: TestItemImpl | undefined,
) => {
	if (!a) {
	...
	aChildren.forEach((key) =>
		assertTreesEqual(
			a.children.get(key) as TestItemImpl,
			b.children.get(key) as TestItemImpl,
		),
	);
};
...
suite("ExtHost Testing", () => {
	...
	setup(() => {
		...
		ds.add(
			single.onDidGenerateDiff((d) =>
				single.setDiff(d /* don't clear during testing */),
			),
		);
	});
	...
	suite("OwnedTestCollection", () => {
				...
				},
				{
					...
					item: {
						...
						item: convert.TestItem.from(
							a.children.get("id-aa") as TestItemImpl,
						),
					},
				},
				{
					...
					item: {
						...
						expand: TestItemExpandState.NotExpandable,
						item: convert.TestItem.from(
							a.children.get("id-ab") as TestItemImpl,
						),
					},
				},
				{
		...
		test("watches property mutations", () => {
			single.expand(single.root.id, Infinity);
			single.collectDiff();
			single.root.children.get("id-a")!.description =
				"Hello world"; /* item a */
		...
		test("replaces on uri change", () => {
			...
			const uri = single.root.children
				.get("id-a")!
				.uri?.with({ path: "/different" });
			...
			single.root.children.replace(
				[...single.root.children].map(([id, i]) => (id === "id-a" ? newA : i)),
			);
			...
			assert.deepStrictEqual(single.collectDiff(), [
				...
				},
				{
					...
					item: {
						...
						expand: TestItemExpandState.NotExpandable,
						item: convert.TestItem.from(
							newA.children.get("id-aa") as TestItemImpl,
						),
					},
				},
				{
					...
					item: {
						...
						expand: TestItemExpandState.NotExpandable,
						item: convert.TestItem.from(
							newA.children.get("id-ab") as TestItemImpl,
						),
					},
				},
			]);
	...
	suite("TestRunTracker", () => {
		...
		setup(async () => {
			...
			configuration = new TestRunProfileImpl(
				mockObject<MainThreadTestingShape>()(),
				new Map(),
				new Set(),
				Event.None,
				"ctrlId",
		...
		test("tracks a run started from a main thread request", () => {
			const tracker = ds.add(
				c.prepareForMainThreadTestRun(ext, req, dto, configuration, cts.token),
			);
			...
			expectedArgs.push([
				...
				[
					...
					convert.TestItem.from(
						single.root.children.get("id-a") as TestItemImpl,
					),
					convert.TestItem.from(
						single.root.children
							.get("id-a")!
							.children.get("id-aa") as TestItemImpl,
					),
				],
			]);
			assert.deepStrictEqual(proxy.$addTestsToRun.args, expectedArgs);
			...
			expectedArgs.push([
				...
				[
					...
					convert.TestItem.from(
						single.root.children
							.get("id-a")!
							.children.get("id-ab") as TestItemImpl,
					),
				],
			]);
			assert.deepStrictEqual(proxy.$addTestsToRun.args, expectedArgs);
		...
		test("sets state of test with identical local IDs (#131827)", () => {
			...
			assert.deepStrictEqual(proxy.$addTestsToRun.args, [
				[
					"ctrl",
					tracker.id,
					[single.root, testA, childA].map((t) =>
						convert.TestItem.from(t as TestItemImpl),
					),
				],
				[
					"ctrl",
					tracker.id,
					[single.root, testB, childB].map((t) =>
						convert.TestItem.from(t as TestItemImpl),
					),
				],
			]);
	...
	suite("service", () => {
		...
		test("exposes active profiles correctly", async () => {
			...
			const changeA = Event.toPromise(
				profAA.onDidChangeDefault as Event<boolean>,
			);
			const changeBA = Event.toPromise(
				profBA.onDidChangeDefault as Event<boolean>,
			);
			const changeBB = Event.toPromise(
				profBB.onDidChangeDefault as Event<boolean>,
			);
