...
suite("ExtHostLanguageFeatures", function () {
	...
	setup(() => {
		...
		rpcProtocol.set(
			...
			disposables.add(inst.createInstance(MainThreadCommands, rpcProtocol)),
		);
	...
	test("DocumentSymbols, register/deregister", async () => {
		...
		const d1 = extHost.registerDocumentSymbolProvider(
			defaultExtension,
			defaultSelector,
			new (class implements vscode.DocumentSymbolProvider {
	...
	test("Declaration, data conversion", async () => {
		disposables.add(
			extHost.registerDeclarationProvider(
				defaultExtension,
				defaultSelector,
				new (class implements vscode.DeclarationProvider {
	...
	test("Quick Fix, command data conversion", async () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			disposables.add(
				extHost.registerCodeActionProvider(defaultExtension, defaultSelector, {
	...
	test("Navigate types, evil provider", async () => {
		disposables.add(
			extHost.registerWorkspaceSymbolProvider(
				defaultExtension,
				new (class implements vscode.WorkspaceSymbolProvider {
					...
					provideWorkspaceSymbols(): any {
						return [
							new types.SymbolInformation(
								...
								types.SymbolKind.Array,
								undefined,
								new types.Location(uri, new types.Range(0, 0, 1, 1)),
							),
						];
					}
				})(),
					...
					provideWorkspaceSymbols(): any {
						return [
							new types.SymbolInformation(
								...
								types.SymbolKind.Array,
								undefined,
								new types.Location(uri, undefined!),
							),
		...
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const value = await provideSuggestionItems(
				...
				new CompletionOptions(
					undefined,
					new Set<languages.CompletionItemKind>().add(
	...
	const NullWorkerService = new (class extends mock<IEditorWorkerService>() {
		override computeMoreMinimalEdits(
			resource: URI,
