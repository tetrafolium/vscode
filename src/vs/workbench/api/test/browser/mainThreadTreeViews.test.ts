...
suite("MainThreadHostTreeView", function () {
	...
	test("handleDrag reconstructs URI list from uriListData", async () => {
		...
		const mockExtHostWithDrag =
			new (class extends mock<ExtHostTreeViewsShape>() {
				override async $getChildren(
					treeViewId: string,
					treeItemHandle?: string[],
				): Promise<(number | ITreeItem)[][]> {
				...
				override async $handleDrag(
					_sourceViewId: string,
					_sourceTreeItemHandles: string[],
					_operationUuid: string,
									...
									uriListData: [
										{
											scheme: "file",
											authority: "",
											path: "/transformed/correct/path.txt",
											query: "",
											fragment: "",
										},
									],
		...
		const dragTestMainThreadTreeViews = disposables.add(
			new MainThreadTreeViews(
				new (class implements IExtHostContext {
					remoteAuthority = "";
					extensionHostKind = ExtensionHostKind.LocalProcess;
		...
		dragTestMainThreadTreeViews.$registerTreeViewDataProvider(
		...
		const dragTestTreeView: ITreeView = (<ITreeViewDescriptor>(
			ViewsRegistry.getView(testTreeViewIdWithDrag)
		)).treeView;
		const dragController = dragTestTreeView.dragAndDropController;
		assert(dragController, "Drag controller should exist");

		// Call handleDrag
		const result = await dragController.handleDrag(
			["item1"],
			"test-operation-uuid",
