...
import {
	...
	IMainThreadEditorLocator,
} from "../../browser/mainThreadEditors.js";
...
suite("MainThreadEditors", () => {
	...
	setup(() => {
		...
		const documents = instaService.createInstance(
			MainThreadDocuments,
			SingleProxyRPCProtocol(null),
		);
		...
		editors = instaService.createInstance(
			MainThreadTextEditors,
			editorLocator,
			SingleProxyRPCProtocol(null),
		);
		modelService = instaService.invokeFunction((accessor) =>
	...
	test("applyWorkspaceEdit: noop eol edit keeps undo stack clean", async () => {
		...
		const model = disposables.add(
			modelService.createModel(initialText, null, resource),
		);
		...
		const insertResult = await bulkEdits.$tryApplyWorkspaceEdit(
			new SerializableObjectWithBuffers({ edits: [insertEdit] }),
		);
		...
		const eolResult = await bulkEdits.$tryApplyWorkspaceEdit(
			new SerializableObjectWithBuffers({ edits: [eolEdit] }),
		);
	...
	test("applyWorkspaceEdit can control undo/redo stack 1", async () => {
		...
		const applied1 = await editors.$tryApplyEdits(
			editorId,
			model.getVersionId(),
			[edit1],
		...
		const applied2 = await editors.$tryApplyEdits(
			editorId,
			model.getVersionId(),
			[edit2],
			{ undoStopBefore: false, undoStopAfter: false },
