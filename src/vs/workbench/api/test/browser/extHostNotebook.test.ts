...
suite("NotebookCell#Document", function () {
	...
	setup(async function () {
		...
		const reg = extHostNotebooks.registerNotebookSerializer(
			...
			"test",
			new (class extends mock<vscode.NotebookSerializer>() {})(),
		);
	...
	test("cell document goes when notebook closes", async function () {
		...
		extHostNotebooks.$acceptDocumentAndEditorsDelta(
			new SerializableObjectWithBuffers({ removedDocuments: [notebook.uri] }),
		);
		reg.dispose();
	...
	test("unset active notebook editor", function () {
		...
		extHostNotebooks.$acceptDocumentAndEditorsDelta(
			new SerializableObjectWithBuffers({ newActiveEditor: undefined }),
		);
	...
	async function replaceOutputs(
		...
		extHostNotebookDocuments.$acceptModelChanged(
			notebook.uri,
			new SerializableObjectWithBuffers<NotebookCellsChangedEventDto>({
				versionId: notebook.apiNotebook.version + 1,
				rawEvents: [
					{
						kind: NotebookCellsChangeType.Output,
	...
	async function appendOutputItem(
		cellIndex: number,
		outputId: string,
		outputItems: NotebookOutputItemDto[],
	) {
