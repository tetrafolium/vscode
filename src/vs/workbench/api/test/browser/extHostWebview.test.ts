...
suite("ExtHostWebview", () => {
	...
	function createWebview(
		rpcProtocol: (IExtHostRpcService & IExtHostContext) | undefined,
		...
		const extHostWebviewPanels = disposables.add(
			new ExtHostWebviewPanels(rpcProtocol!, extHostWebviews, undefined),
		);
	...
	test("Cannot register multiple serializers for the same view type", async () => {
		...
		let lastInvokedDeserializer: vscode.WebviewPanelSerializer | undefined =
			undefined;
	...
	test("asWebviewUri for local file paths", () => {
		...
		assert.strictEqual(
			webview.webview
				.asWebviewUri(URI.parse("file:///Users/codey/file.html"))
				.toString(),
		...
		assert.strictEqual(
			webview.webview
				.asWebviewUri(URI.parse("file:///Users/codey/file.html#frag"))
				.toString(),
		...
		assert.strictEqual(
			webview.webview
				.asWebviewUri(URI.parse("file:///Users/codey/f%20ile.html"))
				.toString(),
		...
		assert.strictEqual(
			webview.webview
				.asWebviewUri(URI.parse("file://localhost/Users/codey/file.html"))
				.toString(),
		...
		assert.strictEqual(
			webview.webview
				.asWebviewUri(URI.parse("file:///c:/codey/file.txt"))
				.toString(),
	...
	suite("ensureDefaultContentOptions", () => {
		function createExtHostWebviewsWithCapture(
			...
			const workspace: IExtHostWorkspace | undefined = workspaceFolders
				? new (class extends mock<IExtHostWorkspace>() {
						override getWorkspaceFolders() {
							return workspaceFolders.map((uri, index) => ({
								uri,
								name: `f${index}`,
								index,
		...
		test("fills default localResourceRoots from workspace folders and extension location when caller did not supply them", () => {
			...
			const { extHostWebviews, setOptionsCalls } =
				createExtHostWebviewsWithCapture([folderA, folderB]);

			disposables.add(
				extHostWebviews.createNewWebview(
					"handle-1",
					{ enableScripts: true },
					extension,
				),
			);
			extHostWebviews.ensureDefaultContentOptions(
				"handle-1",
				{ enableScripts: true },
				extension,
			);
			...
			assert.strictEqual(
				setOptionsCalls.length,
				1,
				"expected $setOptions to be called once",
			);
		...
		test("does nothing when caller already supplied localResourceRoots", () => {
			const { extHostWebviews, setOptionsCalls } =
				createExtHostWebviewsWithCapture([URI.file("/workspace/a")]);
			const explicit = [URI.file("/explicit/root")];

			disposables.add(
				extHostWebviews.createNewWebview(
					"handle-2",
					{ localResourceRoots: explicit },
					extension,
				),
			);
			extHostWebviews.ensureDefaultContentOptions(
			...
			assert.strictEqual(
				setOptionsCalls.length,
				0,
				"expected $setOptions not to be called",
			);
		});

		test("falls back to just the extension location when there are no workspace folders", () => {
			const { extHostWebviews, setOptionsCalls } =
				createExtHostWebviewsWithCapture(undefined);

			disposables.add(
				extHostWebviews.createNewWebview("handle-3", {}, extension),
			);
