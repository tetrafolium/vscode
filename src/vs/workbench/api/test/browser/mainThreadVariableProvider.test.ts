...
suite("MainThreadNotebookKernelVariableProvider", function () {
	...
	setup(async function () {
		const proxy = new (class extends mock<ExtHostNotebookKernelsShape>() {
			override async $provideVariables(
				handle: number,
				requestId: string,
				...
				for (const variable of variables) {
					...
					const result =
						typeof variable === "function" ? await variable() : variable;
		...
		variables = [];
		mainThreadKernels = store.add(
			new TestMainThreadNotebookKernels(extHostContext),
		);
	});
	...
	test("get variables from kernel", async function () {
		...
		const vars = kernel.provideVariables(
			URI.file("nb.ipynb"),
			undefined,
			"named",
			0,
	...
	test("get variables twice", async function () {
		...
		const vars2 = kernel.provideVariables(
			URI.file("nb.ipynb"),
			undefined,
			"named",
			0,
...
async function verifyVariables(
	variables: AsyncIterableProducer<VariablesResult>,
	expectedIds: number[],
) {
	let varIx = 0;
