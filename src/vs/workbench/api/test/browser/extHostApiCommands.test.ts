...
function assertRejects(
	fn: () => Promise<any>,
	message: string = "Expected rejection",
) {
	return fn().then(
...
): value is vscode.Location {
	const candidate = value as vscode.Location;
	return (
		candidate &&
		candidate.uri instanceof URI &&
		candidate.range instanceof types.Range
	);
}
...
suite("ExtHostLanguageFeatureCommands", function () {
	...
	suiteSetup(() => {
		...
		services.set(
			ICommandService,
			new SyncDescriptor(
				class extends mock<ICommandService>() {
		...
		services.set(
			...
			new (class extends mock<ITextModelService>() {
				override async createModelReference() {
					return new ImmortalReference<IResolvedTextEditorModel>(
	...
	test("WorkspaceSymbols, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				...
				>("vscode.executeWorkspaceSymbolProvider", "testing")
				.then((value) => {
	...
	test("executeWorkspaceSymbolProvider should accept empty string, #39522", async function () {
		...
		let symbols = await commands.executeCommand<vscode.SymbolInformation[]>(
			"vscode.executeWorkspaceSymbolProvider",
			"",
		);
		...
		await rpcProtocol.sync();
		symbols = await commands.executeCommand<vscode.SymbolInformation[]>(
			"vscode.executeWorkspaceSymbolProvider",
			"*",
		);
		assert.strictEqual(symbols.length, 1);
	...
	test("executeFormatDocumentProvider, back and forth", async function () {
		disposables.push(
			extHost.registerDocumentFormattingEditProvider(
				nullExtensionDescription,
				defaultSelector,
				new (class implements vscode.DocumentFormattingEditProvider {
		...
		await rpcProtocol.sync();
		const edits = await commands.executeCommand<vscode.SymbolInformation[]>(
	...
	test("vscode.prepareRename", async function () {
		...
		const data = await commands.executeCommand<{
			range: vscode.Range;
			placeholder: string;
		}>("vscode.prepareRename", model.uri, new types.Position(0, 12));
	...
	test("vscode.executeDocumentRenameProvider", async function () {
		...
		const edit = await commands.executeCommand<vscode.WorkspaceEdit>(
	...
	test("Definition, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.Location[]
				>("vscode.executeDefinitionProvider", model.uri, new types.Position(0, 0))
				.then((values) => {
	...
	test("Declaration, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.Location[]
				>("vscode.executeDeclarationProvider", model.uri, new types.Position(0, 0))
				.then((values) => {
					assert.strictEqual(values.length, 4);
					for (const v of values) {
						assert.ok(v.uri instanceof URI);
	...
	test("Type Definition, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.Location[]
				>("vscode.executeTypeDefinitionProvider", model.uri, new types.Position(0, 0))
				.then((values) => {
					assert.strictEqual(values.length, 4);
					for (const v of values) {
						assert.ok(v.uri instanceof URI);
	...
	test("Implementation, invalid arguments", function () {
		const promises = [
			...
			assertRejects(() =>
				commands.executeCommand("vscode.executeImplementationProvider", null),
			),
			assertRejects(() =>
				commands.executeCommand(
	...
	test("Implementation, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.Location[]
				>("vscode.executeImplementationProvider", model.uri, new types.Position(0, 0))
				.then((values) => {
					assert.strictEqual(values.length, 4);
					for (const v of values) {
						assert.ok(v.uri instanceof URI);
	...
	test("reference search, back and forth", function () {
		...
		return commands
			.executeCommand<
				vscode.Location[]
			>("vscode.executeReferenceProvider", model.uri, new types.Position(0, 0))
			.then((values) => {
	...
	test('"vscode.executeDocumentHighlights" API has stopped returning DocumentHighlight[]#200056', async function () {
		...
		return commands
			...
			>("vscode.executeDocumentHighlights", model.uri, new types.Position(0, 0))
			.then((values) => {
	...
	test("Outline, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.SymbolInformation[]
				>("vscode.executeDocumentSymbolProvider", model.uri)
				.then((values) => {
					assert.strictEqual(values.length, 2);
	...
	testApiCmd(
		...
		async function () {
			...
			await commands.executeCommand<vscode.CompletionList>(
	...
	testApiCmd("Suggest, back and forth", async function () {
		...
		assert.strictEqual(
			(<types.MarkdownString>first.documentation).value,
			"hello_md_string",
		);
	...
	test("Parameter Hints, back and forth", async () => {
		...
		const firstValue = await commands.executeCommand<vscode.SignatureHelp>(
	...
	testApiCmd("QuickFix, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.Command[]
				>("vscode.executeCodeActionProvider", model.uri, new types.Range(0, 0, 1, 1))
				.then((value) => {
		...
		function () {
			...
			return rpcProtocol.sync().then(() => {
				return commands
					.executeCommand<
						vscode.CodeAction[]
					>("vscode.executeCodeActionProvider", model.uri, selection)
					.then((value) => {
						assert.strictEqual(value.length, 1);
						const [first] = value;
						assert.ok(first.command);
	...
	testApiCmd("resolving code action", async function () {
			...
			extHost.registerCodeActionProvider(
				...
				defaultSelector,
				{
					provideCodeActions(document, rangeOrSelection): vscode.CodeAction[] {
						return [
							new MyAction("title", types.CodeActionKind.Empty.append("foo")),
						];
					},
		...
		const value = await commands.executeCommand<vscode.CodeAction[]>(
	...
	testApiCmd("CodeLens, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.CodeLens[]
				>("vscode.executeCodeLensProvider", model.uri)
				.then((value) => {
					assert.strictEqual(value.length, 1);
					const [first] = value;
	...
	testApiCmd("CodeLens, resolve", async function () {
		...
		let value = await commands.executeCommand<vscode.CodeLens[]>(
		...
		resolveCount = 0;
		value = await commands.executeCommand<vscode.CodeLens[]>(
			"vscode.executeCodeLensProvider",
			model.uri,
		);
	...
	testApiCmd("Links, back and forth", function () {
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.DocumentLink[]
				>("vscode.executeLinkProvider", model.uri)
				.then((value) => {
					assert.strictEqual(value.length, 1);
					const [first] = value;
	...
	testApiCmd(
		...
		async function () {
			disposables.push(
				extHost.registerDocumentLinkProvider(
					...
					<vscode.DocumentLinkProvider>{
						provideDocumentLinks(): any {
							return [
								new types.DocumentLink(new types.Range(0, 0, 0, 20), undefined),
							];
						},
			...
			const links1 = await commands.executeCommand<vscode.DocumentLink[]>(
			...
			const links2 = await commands.executeCommand<vscode.DocumentLink[]>(
				"vscode.executeLinkProvider",
				model.uri,
				1000,
			);
	...
	test("Color provider", function () {
		disposables.push(
			extHost.registerColorProvider(nullExtensionDescription, defaultSelector, <
			...
			>{
				...
				provideColorPresentations(): vscode.ColorPresentation[] {
					...
					cp.additionalTextEdits = [
						types.TextEdit.insert(new types.Position(2, 20), "*"),
					];
					return [cp];
				},
			}),
		...
		return rpcProtocol
			...
			.then(() => {
				return commands
					...
					>("vscode.executeDocumentColorProvider", model.uri)
					.then((value) => {
						assert.strictEqual(value.length, 1);
						const [first] = value;
			...
			.then(() => {
				...
				return commands
					...
					>("vscode.executeColorPresentationProvider", color, { uri: model.uri, range })
					.then((value) => {
						assert.strictEqual(value.length, 1);
						const [first] = value;
	...
	test('"TypeError: e.onCancellationRequested is not a function" calling hover provider in Insiders #54174', function () {
		disposables.push(
			extHost.registerHoverProvider(nullExtensionDescription, defaultSelector, <
		...
		return rpcProtocol.sync().then(() => {
			return commands
				.executeCommand<
					vscode.Hover[]
				>("vscode.executeHoverProvider", model.uri, new types.Position(1, 1))
				.then((value) => {
					assert.strictEqual(value.length, 1);
	...
	testApiCmd("Inlay Hints, back and forth", async function () {
		...
		const value = await commands.executeCommand<vscode.InlayHint[]>(
			...
			extHost.registerInlayHintsProvider(
				...
				<vscode.InlayHintsProvider>{
					provideInlayHints() {
						...
						const hint = new types.InlayHint(new types.Position(10, 11), [
							part,
						]);
	...
	test("CallHierarchy, back and forth", async function () {
		disposables.push(
			extHost.registerCallHierarchyProvider(
				nullExtensionDescription,
				defaultSelector,
				new (class implements vscode.CallHierarchyProvider {
		...
		const root = await commands.executeCommand<vscode.CallHierarchyItem[]>(
		...
		const incoming = await commands.executeCommand<
			vscode.CallHierarchyIncomingCall[]
		>("vscode.provideIncomingCalls", root[0]);
		assert.strictEqual(incoming.length, 1);
		assert.strictEqual(incoming[0].from.name, "INCOMING");

		const outgoing = await commands.executeCommand<
			vscode.CallHierarchyOutgoingCall[]
		>("vscode.provideOutgoingCalls", root[0]);
	...
	test("TypeHierarchy, back and forth", async function () {
		disposables.push(
			extHost.registerTypeHierarchyProvider(
				nullExtensionDescription,
				defaultSelector,
				new (class implements vscode.TypeHierarchyProvider {
		...
		const root = await commands.executeCommand<vscode.TypeHierarchyItem[]>(
		...
		const incoming = await commands.executeCommand<vscode.TypeHierarchyItem[]>(
		...
		const outgoing = await commands.executeCommand<vscode.TypeHierarchyItem[]>(
