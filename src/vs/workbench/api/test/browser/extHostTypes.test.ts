...
suite("ExtHostTypes", function () {
	...
	test("Range", () => {
		...
		// eslint-disable-next-line local/code-no-any-casts
		assert.throws(() => {
			(range as any).start = new types.Position(0, 3);
		});
	});
	...
	test("NotebookCellOutputItem - factories", function () {
		...
		assert.deepStrictEqual(
			...
			new TextEncoder().encode(
				JSON.stringify([true, 1, "ddd"], undefined, "\t"),
			),
		);
	...
	test("runtime stable, type-def changed", function () {
		...
		const m = new types.LanguageModelChatMessage(
		...
		m.content = "Hello";
		assert.deepStrictEqual(m.content, [
			new types.LanguageModelTextPart("Hello"),
		]);
	});
});
