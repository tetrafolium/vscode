...
function unBatchChildren(
	result: (readonly (number | ITreeItem)[])[] | undefined,
): readonly ITreeItem[] | undefined {
...
suite("ExtHostTreeView", function () {
	...
	class RecordingShape extends mock<MainThreadTreeViewsShape>() {
		...
		override async $registerTreeViewDataProvider(
			treeViewId: string,
		): Promise<void> {}
	...
	test("construct node tree", () => {
		return testObject.$getChildren("testNodeTreeProvider").then((elements) => {
			...
			return Promise.all([
				testObject
					.$getChildren("testNodeTreeProvider", ["0/0:a"])
					.then((children) => {
						...
						return Promise.all([
							testObject
								...
								.then((children) =>
									assert.strictEqual(unBatchChildren(children)?.length, 0),
								),
							testObject
				...
				testObject
					.$getChildren("testNodeTreeProvider", ["0/0:b"])
					.then((children) => {
						const actuals = unBatchChildren(children)?.map((e) => e.handle);
	...
	test("construct id tree", () => {
		return testObject
			.$getChildren("testNodeWithIdTreeProvider")
			.then((elements) => {
				const actuals = unBatchChildren(elements)?.map((e) => e.handle);
				...
				return Promise.all([
					...
					testObject
						.$getChildren("testNodeWithIdTreeProvider", ["1/b"])
						.then((children) => {
							const actuals = unBatchChildren(children)?.map((e) => e.handle);
	...
	test("construct highlights tree", () => {
		return testObject
			.$getChildren("testNodeWithHighlightsTreeProvider")
			.then((elements) => {
				...
				return Promise.all([
					testObject
						.$getChildren("testNodeWithHighlightsTreeProvider", ["1/a"])
						.then((children) => {
							assert.deepStrictEqual(
								removeUnsetKeys(unBatchChildren(children)),
								[
									{
										handle: "1/aa",
										parentHandle: "1/a",
					...
					testObject
						.$getChildren("testNodeWithHighlightsTreeProvider", ["1/b"])
						.then((children) => {
							assert.deepStrictEqual(
								removeUnsetKeys(unBatchChildren(children)),
								[
					...
					.then((elements) => {
						...
						return testObject
							...
							.then((elements) => {
								...
								const children = unBatchChildren(elements)?.map(
									(e) => e.handle,
								);
		...
		store.add(
			target.onRefresh.event(() => {
				testObject.$getChildren("testNodeTreeProvider").then((elements) => {
					assert.deepStrictEqual(
						unBatchChildren(elements)?.map((e) => e.handle),
						["0/0:a//0:b"],
					);
					done();
				});
		...
		store.add(
			target.onRefresh.event(() => {
				testObject.$getChildren("testNodeTreeProvider").then((elements) => {
					...
					return testObject
						.$getChildren("testNodeTreeProvider", ["0/1:b"])
						.then((elements) => {
							const actuals = unBatchChildren(elements)?.map((e) => e.handle);
							assert.deepStrictEqual(actuals, [
								"0/1:b/0:h",
	...
	test("dispose and re-register tree view", async () => {
		...
		const treeView1 = testObject.createTreeView(
			"reRegisterTreeProvider",
			{ treeDataProvider: aNodeTreeDataProvider() },
			extensionsDescription,
		);
		treeView1.dispose();
		const treeView2 = testObject.createTreeView(
			"reRegisterTreeProvider",
			{ treeDataProvider: aNodeTreeDataProvider() },
			extensionsDescription,
		);
	...
	test("reveal will return parents array for an element when hierarchy is not loaded", () => {
		...
		return treeView.reveal({ key: "aa" }).then(() => {
			...
			assert.deepStrictEqual(
				...
				(<Array<any>>revealTarget.args[0][1]!.parentChain).map((arg) =>
					removeUnsetKeys(arg),
				),
			);
			assert.deepStrictEqual(
	...
	function loadCompleteTree(treeId: string, element?: string): Promise<null> {
		return testObject
			...
			.then((elements) => {
				...
				return elements[0]
					.slice(1)
					.map((e) => loadCompleteTree(treeId, (e as ITreeItem).handle));
			})
			.then(() => null);
	}
	...
	function aNodeWithHighlightedLabelTreeDataProvider(): TreeDataProvider<{
		key: string;
	}> {
		return {
			getChildren: (element: { key: string }): { key: string }[] => {
