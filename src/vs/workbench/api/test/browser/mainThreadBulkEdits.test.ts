...
suite("MainThreadBulkEdits", function () {
	...
	test('"Rename failed to apply edits" in monorepo with pnpm #158845', function () {
		...
		assert.strictEqual(
			(<IWorkspaceTextEdit>out.edits[0]).resource.path,
			"/hello/WORLD/foo.txt",
		);
		assert.strictEqual(
			(<IWorkspaceTextEdit>out.edits[1]).resource.path,
			"/hello/WORLD/foo.txt",
		); // the FIRST occurrence defined the shape!
		assert.strictEqual(
			(<IWorkspaceTextEdit>out.edits[2]).resource.path,
			"/other/path.txt",
		);
		assert.strictEqual(
			(<IWorkspaceTextEdit>out.edits[3]).resource.path,
			"/other/path.txt",
		);
