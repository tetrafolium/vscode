				...
				({
					...
					fullReferenceName:
						tool.source.type === "mcp"
							? this._languageModelToolsService.getFullReferenceName(tool)
							: undefined,
					tags: tool.tags,
		...
		const disposable =
			this._languageModelToolsService.registerToolImplementation(id, {
				...
				prepareToolInvocation: (context, token) =>
					this._proxy.$prepareToolInvocation(id, context, token),
				handleToolStream: hasHandleToolStream
					? (context, token) =>
							this._proxy.$handleToolStream(id, context, token)
					: undefined,
			});
	...
	$registerToolWithDefinition(
		...
		if (definition.icon) {
			if (ThemeIcon.isThemeIcon(definition.icon)) {
				...
				typeof definition.icon === "object" &&
				definition.icon !== null &&
				isUriComponents(definition.icon)
			) {
				icon = { dark: URI.revive(definition.icon as UriComponents) };
			} else {
				const iconObj = definition.icon as {
					light?: UriComponents;
					dark: UriComponents;
				};
				icon = {
		...
		const isBuiltinTool = this._productService.defaultChatAgent?.chatExtensionId
			? ExtensionIdentifier.equals(
					extensionId,
					this._productService.defaultChatAgent.chatExtensionId,
				)
			: false;
		...
		const toolData: IToolData = {
			...
			canBeReferencedInPrompt:
				!!definition.userDescription && !definition.toolSet,
		};
		...
		store.add(
			this._languageModelToolsService.registerTool(toolData, {
				invoke: async (dto, countTokens, progress, token) => {
					try {
						this._runningToolCalls.set(dto.callId, { countTokens, progress });
						const resultSerialized = await this._proxy.$invokeTool(dto, token);
		...
		if (definition.toolSet) {
			const ts =
				this._languageModelToolsService.getToolSet(
					toToolSetKey(extensionId, definition.toolSet),
				) || this._languageModelToolsService.getToolSet(definition.toolSet);
