		...
		this._store.add(
			this._model.onDidChangeContent((e) => {
				...
				if (e.detailedReasonsChangeLengths.length !== 1) {
					onUnexpectedError(
						new Error(
							`Unexpected reasons: ${e.detailedReasons.map((r) => r.toString())}`,
						),
					);
				}
	...
	implements MainThreadDocumentsShape
{
	...
	constructor(
		...
		this._store.add(
			_modelService.onModelLanguageChanged(this._onModelModeChanged, this),
		);
	...
	async $tryOpenDocument(
		uriData: UriComponents,
	...
	private async _handleAsResourceInput(
		uri: URI,
		options?: { encoding?: string },
	): Promise<URI> {
		if (options?.encoding) {
	...
	private async _handleUntitledScheme(
		uri: URI,
		options?: { encoding?: string },
	): Promise<URI> {
