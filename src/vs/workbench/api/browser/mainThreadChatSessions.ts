...
} from "../../contrib/chat/browser/chat.js";
...
import {
	...
	IChatSessionItemsChange,
	IChatSessionRequestHistoryItemDto,
	MainContext,
	MainThreadChatSessionsShape,
} from "../common/extHost.protocol.js";

function stringOrMarkdownEqual(
	a: string | IMarkdownString | undefined,
	b: string | IMarkdownString | undefined,
): boolean {
	if (a === b) {
		return true;
	}
...
export class ObservableChatSession extends Disposable implements IChatSession {
	...
	private readonly _progressObservable = observableValue<IChatProgress[]>(
		this,
		[],
	);
	private readonly _isCompleteObservable = observableValue<boolean>(
		this,
		false,
	);
	...
	private async _doInitializeContent(
		token: CancellationToken,
		context: ChatSessionContentContextDto,
	): Promise<void> {
		try {
			const sessionContent = await raceCancellationError(
				this._proxy.$provideChatSessionContent(
			...
			this._options = sessionContent.options
				? ChatSessionOptionsMap.fromRecord(sessionContent.options)
				: undefined;
			this.title = sessionContent.title;
			this.history.length = 0;
			this.history.push(
				...sessionContent.history.map((turn: IChatSessionHistoryItemDto) => {
					if (turn.type === "request") {
						...
						return {
							...
							modeInstructions: turn.modeInstructions
								? revive(turn.modeInstructions)
								: undefined,
					...
					return {
						...
						parts: turn.parts.map(
							(part: IChatProgressDto) => revive(part) as IChatProgress,
						),
						participant: turn.participant,
			...
			if (
				sessionContent.hasActiveResponseCallback &&
				!this.interruptActiveResponseCallback
			) {
				this.interruptActiveResponseCallback = async () => {
					const confirmInterrupt = () => {
						if (this._disposalPending) {
							this._proxy.$disposeChatSessionContent(
								this._providerHandle,
								this.sessionResource,
							);
			...
			if (sessionContent.hasRequestHandler && !this.requestHandler) {
				this.requestHandler = async (
					...
					try {
						await this._proxy.$invokeChatSessionRequestHandler(
							this._providerHandle,
							this.sessionResource,
							request,
							history,
						...
						if (
							!this._isCompleteObservable.get() &&
							!this.interruptActiveResponseCallback
						) {
							this._markComplete();
						}
			...
			this._logService.error(
				`Failed to initialize chat session ${this.sessionResource}:`,
				error,
			);
			throw error;
		}
...
class MainThreadChatSessionItemController
	extends Disposable
	implements IChatSessionItemController
{
	private readonly _proxy: ExtHostChatSessionsShape;
	...
	private readonly _resolveCache = new ResourceMap<
	...
	constructor(
		...
		const addModelListeners = async (model: IChatModel) => {
			...
			const requestChangeListener = model.lastRequestObs.map(
				(last) =>
					last?.response &&
					observableSignalFromEvent(
						"chatSessions.modelRequestChangeListener",
						last.response.onDidChange,
					),
			);
		...
		this._register(
			_chatService.onDidCreateModel((model) => addModelListeners(model)),
		);
		...
		this._register(
			_chatService.onDidDisposeSession((e) => {
	...
	async newChatSessionItem(
		...
		const dto = await raceCancellationError(
			this._proxy.$newChatSessionItem(
				this._handle,
				{
					prompt: request.prompt,
					command: request.command,
	...
	private async addOrUpdateItem(
		dto: Dto<IChatSessionItem>,
	): Promise<MainThreadChatSessionItem> {
	...
	async getNewChatSessionInputState(
		sessionResource: URI,
		token: CancellationToken,
	): Promise<readonly IChatSessionProviderOptionGroup[] | undefined> {
		const optionGroups = await this._proxy.$provideChatSessionInputState(
			this._handle,
			sessionResource,
			token,
		);
	...
	private async _doResolveItem(
		...
		try {
			dto = await raceCancellationError(
				this._proxy.$resolveChatSessionItem(this._handle, resource, token),
				token,
			);
		} finally {
...
class MainThreadChatSessionItem implements IChatSessionItem {
	...
	constructor(
		...
		detailOverrides: IChatDetail | undefined,
	) {
		...
		this.legacyResource = dto.legacyResource
			? URI.revive(dto.legacyResource)
			: undefined;

		this.description =
			(model && getInProgressSessionDescription(model)) ??
			reviveMarkdownString(dto.description);
...
export class MainThreadChatSessions
	extends Disposable
	implements MainThreadChatSessionsShape
{
	private readonly _itemControllerRegistrations = this._register(
		new DisposableMap<
			number,
			IDisposable & {
	...
	private readonly _contentProvidersRegistrations = this._register(
	...
	constructor(
		...
		private readonly _chatArtifactsService: IChatArtifactsService,
		...
		this._proxy = this._extHostContext.getProxy(
			ExtHostContext.ExtHostChatSessions,
		);

		this._register(
			this._chatSessionsService.onDidChangeSessionOptions(
				({ sessionResource, updates }) => {
		...
		this._register(
			this._agentSessionsService.model.onDidChangeSessionArchivedState(
				(session) => {
					for (const [handle, { chatSessionType }] of this
	...
	$registerChatSessionItemController(
		...
		disposables.add(
			this._chatSessionsService.registerChatSessionItemController(
				chatSessionType,
				controller,
			),
		);
	...
	$updateChatSessionItemControllerCapabilities(
		...
		if (!registration) {
			this._logService.warn(
				`No chat session item controller found for handle ${handle}`,
			);
			return;
		}
	...
	private _refreshControllerInputState(
		...
		this._proxy
			.$provideChatSessionInputState(handle, undefined, CancellationToken.None)
			.then((optionGroups) => {
				if (optionGroups?.length) {
			...
			.catch((err) =>
				this._logService.error("Error fetching chat session input state", err),
			);
	}
	...
	private _applyOptionGroups(
		...
		this._chatSessionsService.setOptionGroupsForSessionType(
			chatSessionType,
			handle,
			optionGroups,
		);
	...
	private getController(handle: number): MainThreadChatSessionItemController {
		...
		if (!registration) {
			throw new Error(
				`No chat session controller registered for handle ${handle}`,
			);
		}
		return registration.controller;
	}

	async $updateChatSessionItems(
	...
	async $addOrUpdateChatSessionItem(
		controllerHandle: number,
	...
	$onDidChangeChatSessionOptions(
		...
		const sessionResource = URI.revive(sessionResourceComponents);
		this._chatSessionsService.updateSessionOptions(
			sessionResource,
			ChatSessionOptionsMap.fromRecord(updates),
		);
	}

	async $onDidCommitChatSessionItem(
		handle: number,
		originalComponents: UriComponents,
		modifiedCompoennts: UriComponents,
	): Promise<void> {
		...
		const chatSessionType =
			this._itemControllerRegistrations.get(handle)?.chatSessionType;
		if (!chatSessionType) {
			this._logService.error(
				`No chat session type found for provider handle ${handle}`,
			);
			return;
		}

		const originalEditor = this._editorService.editors.find(
			(editor) => editor.resource?.toString() === originalResource.toString(),
		);
		const originalModel =
			this._chatService.acquireExistingSession(originalResource);
		const contribution = this._chatSessionsService
			.getAllChatSessionContributions()
			.find((c) => c.type === chatSessionType);

		try {
			// Migrate todos from old session to new session
			this._chatTodoListService.migrateTodos(
				originalResource,
				modifiedResource,
			);

			// Migrate artifacts from old session to new session
			this._chatArtifactsService
				.getArtifacts(originalResource)
				.migrate(this._chatArtifactsService.getArtifacts(modifiedResource));
			...
			const originalGroup =
				this.editorGroupService.groups.find((group) =>
					group.editors.some((editor) =>
						isEqual(editor.resource, originalResource),
					),
			...
			if (originalEditor) {
				newSession.transferredState =
					originalEditor instanceof ChatEditorInput
						? {
			...
			const chatViewWidget =
				this._chatWidgetService.getWidgetBySessionResource(originalResource);
			if (
				chatViewWidget &&
				isIChatViewViewContext(chatViewWidget.viewContext)
			) {
				...
				const ref = await this._chatService.acquireOrLoadSession(
					modifiedResource,
			...
			this._chatSessionsService.fireSessionCommitted(
				originalResource,
				modifiedResource,
			);
		} finally {
	...
	private async _provideChatSessionContent(
		providerHandle: number,
		sessionResource: URI,
		token: CancellationToken,
		...
		try {
			const initialSessionOptions =
				this._chatSessionsService.getSessionOptions(sessionResource);
			await session.initialize(token, {
				initialSessionOptions: initialSessionOptions
					? [...initialSessionOptions].map(([optionId, value]) => ({
							optionId,
	...
	$registerChatSessionContentProvider(
		...
		const provider: IChatSessionContentProvider = {
			provideChatSessionContent: (resource, token) =>
				this._provideChatSessionContent(handle, resource, token),
		};

		this._sessionTypeToHandle.set(chatSessionScheme, handle);
		this._contentProvidersRegistrations.set(
			handle,
			this._chatSessionsService.registerChatSessionContentProvider(
				chatSessionScheme,
				provider,
			),
		);
	...
	$handleAnchorResolve(
		handle: number,
		sesssionResource: UriComponents,
		requestId: string,
	...
	$updateChatSessionInputState(
		...
		const registration =
			this._itemControllerRegistrations.get(controllerHandle);
		if (!registration) {
			this._logService.warn(
	...
	private _refreshProviderOptions(
		...
		this._proxy
			.$provideChatSessionProviderOptions(handle, CancellationToken.None)
			.then((options) => {
