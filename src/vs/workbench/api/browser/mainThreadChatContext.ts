...
export class MainThreadChatContext
	extends Disposable
	implements MainThreadChatContextShape
{
	...
	private readonly _providers = new Map<
		number,
		{ id: string; selector?: IDocumentFilterDto[] }
	>();
	...
	constructor(
		...
		this._chatContextService.setExecuteCommandCallback((itemHandle) =>
			this._proxy.$executeChatContextItemCommand(itemHandle),
		);
	}
	...
	$registerChatWorkspaceContextProvider(handle: number, id: string): void {
		...
		this._chatContextService.registerChatWorkspaceContextProvider(id, {
			provideWorkspaceChatContext: async (token: CancellationToken) => {
				const items = await this._proxy.$provideWorkspaceChatContext(
					handle,
					token,
				);
	...
	$registerChatExplicitContextProvider(handle: number, id: string): void {
		...
		this._chatContextService.registerChatExplicitContextProvider(id, {
			provideChatContext: async (token: CancellationToken) => {
				const items = await this._proxy.$provideExplicitChatContext(
					handle,
					token,
				);
				return reviveContextItems(items);
			...
			resolveChatContext: async (
				...
				const result = await this._proxy.$resolveExplicitChatContext(
					handle,
					context,
					token,
				);
	...
	$registerChatResourceContextProvider(
		...
		this._chatContextService.registerChatResourceContextProvider(id, selector, {
			provideChatContext: async (
				...
				const result = await this._proxy.$provideResourceChatContext(
					handle,
					{ resource, withValue },
					token,
				);
			...
			resolveChatContext: async (
				...
				const result = await this._proxy.$resolveResourceChatContext(
					handle,
					context,
					token,
				);
