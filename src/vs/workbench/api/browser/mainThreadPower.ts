...
export class MainThreadPower
	extends Disposable
	implements MainThreadPowerShape
{
	...
	constructor(
		...
		this._register(
			this.powerService.onDidSuspend(this.proxy.$onDidSuspend, this.proxy),
		);
		this._register(
			this.powerService.onDidResume(this.proxy.$onDidResume, this.proxy),
		);
		this._register(
			this.powerService.onDidChangeOnBatteryPower(
				this.proxy.$onDidChangeOnBatteryPower,
				this.proxy,
			),
		);
		this._register(
			this.powerService.onDidChangeThermalState(
				(state: PowerThermalState) =>
					this.proxy.$onDidChangeThermalState(state),
				this,
			),
		);
		this._register(
			this.powerService.onDidChangeSpeedLimit(
				this.proxy.$onDidChangeSpeedLimit,
				this.proxy,
			),
		);
		this._register(
			this.powerService.onWillShutdown(this.proxy.$onWillShutdown, this.proxy),
		);
		this._register(
			this.powerService.onDidLockScreen(
				this.proxy.$onDidLockScreen,
				this.proxy,
			),
		);
		this._register(
			this.powerService.onDidUnlockScreen(
				this.proxy.$onDidUnlockScreen,
				this.proxy,
			),
		);
	}
