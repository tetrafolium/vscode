...
} from "../../contrib/scm/common/artifact.js";
...
class MainThreadSCMArtifactProvider implements ISCMArtifactProvider {
	...
	): Promise<ISCMArtifactGroup[] | undefined> {
		const artifactGroups = await this.proxy.$provideArtifactGroups(
			this.handle,
			token ?? CancellationToken.None,
		);
		return artifactGroups?.map((group) => ({
			...group,
			icon: getIconFromIconDto(group.icon),
		}));
	}
	...
	async provideArtifacts(
		group: string,
		token?: CancellationToken,
	): Promise<ISCMArtifact[] | undefined> {
		const artifacts = await this.proxy.$provideArtifacts(
			this.handle,
			group,
			token ?? CancellationToken.None,
		);
		return artifacts?.map((artifact) => ({
			...artifact,
			icon: getIconFromIconDto(artifact.icon),
		}));
	}
...
class MainThreadSCMHistoryProvider implements ISCMHistoryProvider {
	...
	async resolveHistoryItemChatContext(
		...
		return this.proxy.$resolveHistoryItemChatContext(
			this.handle,
			historyItemId,
			token ?? CancellationToken.None,
		);
	...
	$onDidChangeCurrentHistoryItemRefs(
		...
		transaction((tx) => {
			...
			this._historyItemRemoteRef.set(
				toISCMHistoryItemRef(historyItemRemoteRef, historyItemRemoteRefColor),
				tx,
			);
			this._historyItemBaseRef.set(
				toISCMHistoryItemRef(historyItemBaseRef, historyItemBaseRefColor),
				tx,
			);
		});
	}
...
class MainThreadSCMProvider implements ISCMProvider {
	...
	private readonly _contextValue = observableValue<string | undefined>(
		this,
		undefined,
	);
	...
	private readonly _artifactProvider = observableValue<
		MainThreadSCMArtifactProvider | undefined
	>(this, undefined);
	...
	$updateSourceControl(features: SCMProviderFeatures): void {
		...
		if (features.hasSecondaryQuickDiffProvider && !this._stagedQuickDiff) {
			this._stagedQuickDiff = this._quickDiffService.addQuickDiffProvider({
				...
				getOriginalResource: async (uri: URI) => {
					...
					const result = await this.proxy.$provideSecondaryOriginalResource(
						this.handle,
						uri,
						CancellationToken.None,
					);
			...
			features.hasSecondaryQuickDiffProvider === false &&
			this._stagedQuickDiff
		) {
		...
		if (features.hasArtifactProvider && !this.artifactProvider.get()) {
			const artifactProvider = new MainThreadSCMArtifactProvider(
				this.proxy,
				this.handle,
			);
			this._artifactProvider.set(artifactProvider, undefined);
		} else if (
			features.hasArtifactProvider === false &&
			this.artifactProvider.get()
		) {
	...
	$registerGroups(
		...
		const groups = _groups.map(
			([handle, id, label, features, multiDiffEditorEnableViewChanges]) => {
...
export class MainThreadSCM implements MainThreadSCMShape {
	...
	async $registerSourceControl(
		...
		if (repository.input.value) {
			setTimeout(
				() =>
					this._proxy.$onInputBoxValueChange(handle, repository.input.value),
				0,
			);
		}
