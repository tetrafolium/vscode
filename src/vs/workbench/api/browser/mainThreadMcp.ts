...
import {
	IMcpEnterpriseManagedAuthIdpConfig,
	mcpEnterpriseManagedAuthIdpSection,
} from "../../contrib/mcp/common/mcpConfiguration.js";
...
export class MainThreadMcp extends Disposable implements MainThreadMcpShape {
	...
	constructor(
		...
		private readonly authenticationMcpServersService: IAuthenticationMcpService,
		@IAuthenticationMcpAccessService
		private readonly authenticationMCPServerAccessService: IAuthenticationMcpAccessService,
		@IAuthenticationMcpUsageService
		private readonly authenticationMCPServerUsageService: IAuthenticationMcpUsageService,
		@IDynamicAuthenticationProviderStorageService
		private readonly _dynamicAuthenticationProviderStorageService: IDynamicAuthenticationProviderStorageService,
		@IExtensionService private readonly _extensionService: IExtensionService,
		@IContextKeyService private readonly _contextKeyService: IContextKeyService,
		@ITelemetryService private readonly _telemetryService: ITelemetryService,
		@IWorkbenchMcpGatewayService
		private readonly _mcpGatewayService: IWorkbenchMcpGatewayService,
		...
		super();
		this._register(
			_authenticationService.onDidChangeSessions((e) =>
				this._onDidChangeAuthSessions(e.providerId, e.label),
			),
		);
		...
		this._register(
			this._mcpRegistry.registerDelegate({
				...
				priority:
					_extHostContext.extensionHostKind === ExtensionHostKind.LocalWebWorker
						? 0
						: 1,
				...
				canStart(collection, serverDefinition) {
					...
					if (
						serverDefinition.launch.type === McpServerTransportType.Stdio &&
		...
		if (existing) {
			...
			const register = () => {
				handle.value ??= this._mcpRegistry.registerCollection({
					...
					resolveServerLanch: collection.canResolveLaunch
						? async (def) => {
								const r = await this._proxy.$resolveMcpLaunch(
									collection.id,
									def.label,
								);
						...
						: undefined,
					trustBehavior: collection.isTrustedByDefault
			...
			const whenClauseStr = mapFindFirst(
				...
				(e) =>
					ExtensionIdentifier.equals(extensionId, e.identifier)
						? e.contributes?.mcpServerDefinitionProviders?.find(
								(p) =>
									extensionPrefixedIdentifier(extensionId, p.id) ===
									collection.id,
							)?.when
						: undefined,
			);
			const whenClause =
				whenClauseStr && ContextKeyExpr.deserialize(whenClauseStr);
	...
	async $getTokenFromServerMetadata(
		...
		const resourceServer = authDetails.resourceMetadata?.resource
			? URI.parse(authDetails.resourceMetadata.resource)
			: undefined;
		const resolvedScopes =
			authDetails.scopes ??
			authDetails.resourceMetadata?.scopes_supported ??
			authDetails.authorizationServerMetadata.scopes_supported ??
			[];
		...
		if (authDetails.enterpriseManaged) {
			...
			const resourceAuthServers =
				authDetails.resourceMetadata?.authorization_servers ?? [];
			...
			const xaaProviderId =
				await this._authenticationService.createOrGetXaaProvider(issuer);
			if (!xaaProviderId) {
				return undefined;
			}
			...
			if (resourceClientId) {
				try {
					resourceClientSecret = await this._secretStorageService.get(
						mcpOAuthClientSecretStorageKey(resource, resourceClientId),
					);
				} catch {
		...
		let providerId =
			await this._authenticationService.getOrActivateProviderIdForServer(
				authorizationServer,
				resourceServer,
			);
		...
		const mcpServerUrl =
			server.launch.type === McpServerTransportType.HTTP
				? server.launch.uri.toString(true)
				: undefined;
		let clientSecret: string | undefined;
		let didLookupClientSecret = false;
		if (resolvedClientId && mcpServerUrl) {
			try {
				clientSecret = await this._secretStorageService.get(
		...
		if (
			didLookupClientSecret &&
			providerId &&
			!forceNewRegistration &&
			this._authenticationService.isDynamicAuthenticationProvider(providerId)
		) {
			const registered =
				await this._dynamicAuthenticationProviderStorageService.getClientRegistration(
					providerId,
				);
		...
		if (!providerId) {
			const provider =
				await this._authenticationService.createDynamicAuthenticationProvider(
					authorizationServer,
	...
	private _ensureXaaIssuer(): URI {
		const config =
			this._configurationService.getValue<
				IMcpEnterpriseManagedAuthIdpConfig | undefined
			>(mcpEnterpriseManagedAuthIdpSection) ?? {};
	...
	private async _getSessionForProvider(
		...
		const sessions = await this._authenticationService.getSessions(
			providerId,
			scopes,
			{ authorizationServer, clientId, clientSecret, resource, audience },
			true,
		);
		const accountNamePreference =
			this.authenticationMcpServersService.getAccountPreference(
				server.id,
				providerId,
			);
	...
	private async _onDidChangeAuthSessions(
		providerId: string,
	...
	$logMcpAuthSetup(data: IAuthMetadataSource): void {
		type McpAuthSetupClassification = {
			...
			resourceMetadataSource: {
				...
				comment: "How resource metadata was discovered (header, wellKnown, or none)";
			};
			serverMetadataSource: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				comment: "How authorization server metadata was discovered (resourceMetadata, wellKnown, or default)";
			};
		};
		this._telemetryService.publicLog2<
			IAuthMetadataSource,
			McpAuthSetupClassification
		>("mcp/authSetup", data);
	}
	...
	async $startMcpGateway(
		...
		| { servers: { label: string; address: URI }[]; gatewayId: string }
		| undefined
	> {
	...
	pushMessage(message: string): void {
		...
		if (parsed) {
			if (Array.isArray(parsed)) {
				// streamable HTTP supports batching
...
class McpServerAuthTracker {
	...
	get(
		providerId: string,
	): ReadonlyArray<{ serverId: number; scopes: string[] }> | undefined {
