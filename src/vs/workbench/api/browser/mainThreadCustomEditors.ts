...
} from "../../contrib/customEditor/browser/customEditorDiffInput.js";
	...
	implements extHostProtocol.MainThreadCustomEditorsShape
{
	...
	>();
	private readonly _pendingSideBySideDiffResolutions = new Map<
		string,
		PendingCustomEditorSideBySideDiffResolution
	>();
	...
	constructor(
		...
		this._register(
			_webviewWorkbenchService.registerResolver({
				canResolve: (webview: WebviewInput) => {
					if (
						webview instanceof CustomEditorInput ||
						webview instanceof CustomEditorDiffInput ||
						webview instanceof CustomEditorSideBySideDiffInput
					) {
		...
		this._register(
			workingCopyFileService.onWillRunWorkingCopyFileOperation(async (e) =>
		...
		disposables.add(
			this._webviewWorkbenchService.registerResolver({
				canResolve: (webviewInput) => {
					return (
						(webviewInput instanceof CustomEditorInput ||
							webviewInput instanceof CustomEditorDiffInput ||
							webviewInput instanceof CustomEditorSideBySideDiffInput) &&
				...
				resolveWebview: async (
					...
					const resource =
						webviewInput instanceof CustomEditorDiffInput
							? webviewInput.modifiedResource
							: webviewInput.resource;
					...
					try {
						...
						if (
							webviewInput instanceof CustomEditorDiffInput &&
							!isEqual(webviewInput.originalResource, resource)
						) {
							additionalModelRefs.add(
								await this.getOrCreateCustomEditorModel(
									modelType,
									webviewInput.originalResource,
									viewType,
									{},
									cancellation,
								),
							...
							modelType === CustomEditorModelType.Text &&
							webviewInput instanceof CustomEditorSideBySideDiffInput
						) {
							const otherResource =
								webviewInput.side === "original"
									? webviewInput.modifiedResource
									: webviewInput.originalResource;
						...
						webviewInput.webview.setHtml(
							this.mainThreadWebview.getWebviewResolvedFailedContent(viewType),
						);
					...
					if (
						webviewInput instanceof CustomEditorInput &&
						capabilities.supportsMove
					) {
						webviewInput.onMove(async (newResource: URI) => {
							const oldModel = resolvedModelRef;
							resolvedModelRef = await this.getOrCreateCustomEditorModel(
								modelType,
								newResource,
								viewType,
								{},
					...
					try {
						...
						const position = editorGroupToColumn(
						...
						if (webviewInput instanceof CustomEditorDiffInput) {
							const originalResource =
								modelType === CustomEditorModelType.Text
									? this._uriIdentityService.asCanonicalUri(
											webviewInput.originalResource,
										)
									: webviewInput.originalResource;
	...
	private resolveCustomEditorSideBySideDiff(
		...
		let pending = this._pendingSideBySideDiffResolutions.get(
			webviewInput.diffId,
		);
		if (!pending) {
			pending = {
					...
					() => {
						return Array.from(
							...
							(editor) =>
								...
								(editor instanceof CustomEditorDiffInput &&
									(isEqual(editor.originalResource, resource) ||
										isEqual(editor.modifiedResource, resource))) ||
								(editor instanceof CustomEditorSideBySideDiffInput &&
									isEqual(editor.resource, resource)),
						) as CustomEditorWebviewInput[];
					},
					cancellation,
				);
				return this._customEditorService.models.add(resource, viewType, model);
	...
	implements ICustomEditorModel
{
	...
	public static async create(
		...
		const primaryCustomEditorInput = editors.find(
			(editor) => editor instanceof CustomEditorInput,
		);
	...
	private readonly _onDidSave: Emitter<IWorkingCopySaveEvent> = this._register(
