	...
	implements IAuthenticationProvider
{
	...
	createSessionFromChallenges(
		...
		return this._proxy.$createSessionFromChallenges(
			this.id,
			constraint,
			options,
		);
	...
	implements MainThreadAuthenticationShape
{
		...
		this._register(
			authenticationService.registerAuthenticationProviderHostDelegate({
				...
				create: async (
					...
					const clientDetails =
						await this.dynamicAuthProviderStorageService.getClientRegistration(
							authProviderId,
						);
				...
				createXaa: async (issuer) => {
					...
					const configuredClientId =
						configuredIdp.clientId?.trim() || undefined;
					const configuredClientSecret =
						configuredIdp.clientSecret?.trim() || undefined;
					const cached =
						await this.dynamicAuthProviderStorageService.getClientRegistration(
							authProviderId,
						);
	...
	private async loginPrompt(
		...
		const customMessage = provider.confirmation?.(
			extensionName,
			recreatingSession,
		);
		if (customMessage) {
	...
	private async doGetSession(
		...
		if (options.createIfNone || options.forceNewSession) {
			...
			this.authenticationExtensionsService.updateNewSessionRequests(
				providerId,
				[session],
			);
	...
	async $promptForClientRegistration(
		...
		const sharedTitle = nls.localize(
	...
	async $promptForResourceClientSecret(
		...
		try {
			...
			this.logService.warn(
				`[XAA] Failed to persist resource client secret for ${resource} / ${resourceClientId}: ${(err as Error).message}`,
			);
		}
