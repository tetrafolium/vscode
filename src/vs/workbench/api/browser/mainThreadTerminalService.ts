	...
	implements MainThreadTerminalServiceShape
{
	...
	private readonly _dataEventTracker = this._register(
		new MutableDisposable<TerminalDataEventTracker>(),
	);
	private readonly _sendCommandEventListener = this._register(
	...
	constructor(
		...
		this._register(
			_terminalService.onAnyInstanceTitleChange(
				(instance) =>
					instance && this._onTitleChanged(instance.instanceId, instance.title),
			),
		);
		this._register(
		...
		this._register(
			_terminalService.onAnyInstanceShellTypeChanged((instance) =>
				this._onShellTypeChanged(instance.instanceId),
			),
		);
	...
	public $sendProcessReady(
		...
		this._terminalProcessProxies
			.get(terminalId)
			?.proxy.emitReady(pid, cwd, windowsPty);
	}
	...
	public $sendProcessProperty(
		...
		if (property.type === ProcessPropertyType.Title) {
			...
			instance?.rename(
				property.value as IProcessPropertyMap[ProcessPropertyType.Title],
			);
		}
		this._terminalProcessProxies
			.get(terminalId)
			?.proxy.emitProcessProperty(property);
	}
