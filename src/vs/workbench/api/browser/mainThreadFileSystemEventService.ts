...
export class MainThreadFileSystemEventService implements MainThreadFileSystemEventServiceShape {
	...
	constructor(
		...
		const that = this;
		const fileOperationParticipant =
			new (class implements IWorkingCopyFileOperationParticipant {
				async participate(
						...
						if (data.extensionNames.length === 1) {
							...
							if (operation === FileOperation.CREATE) {
								message = localize(
									{
										key: "ask.N.create",
										comment: ['{0} is a number, e.g "3 extensions want..."'],
									},
	...
	async $watch(
		...
		if (correlate && !opts.recursive) {
			...
			const subscription = watcherDisposables.add(
				this._fileService.createWatcher(uri, { ...opts, recursive: false }),
			);
