...
} from "../../../editor/contrib/inlineCompletions/browser/telemetry.js";
	...
	implements MainThreadLanguageFeaturesShape
{
	...
	constructor(
		...
		private readonly _inlineCompletionsUnificationService: IInlineCompletionsUnificationService,
	) {
		super();
		...
		if (this._inlineCompletionsUnificationService) {
			this._register(
				this._inlineCompletionsUnificationService.onDidStateChange(() => {
					this._proxy.$acceptInlineCompletionsUnificationState(
						this._inlineCompletionsUnificationService.state,
					);
				}),
			);
			this._proxy.$acceptInlineCompletionsUnificationState(
		...
		this._registrations.set(
			...
			this._languageFeaturesService.newSymbolNamesProvider.register(selector, {
				supportsAutomaticNewSymbolNamesTriggerKind:
					this._proxy.$supportsAutomaticNewSymbolNamesTriggerKind(handle),
	...
	$registerInlineCompletionsSupport(
		...
		const providerId = new languages.ProviderId(
			extensionId,
			extensionVersion,
			groupId,
		);
	...
	implements languages.InlineCompletionsProvider<IdentifiableInlineCompletions>
{
	...
	public readonly _onDidChangeEmitter = this._register(
		new Emitter<languages.IInlineCompletionChangeHint | void>(),
	);
	public readonly onDidChangeInlineCompletions:
		| Event<languages.IInlineCompletionChangeHint | void>
		| undefined;

	public readonly _onDidChangeModelInfoEmitter = this._register(
		new Emitter<void>(),
	);
	public readonly onDidChangeModelInfo: Event<void> | undefined;

	public readonly setProviderOption:
		| ((optionId: string, valueId: string) => Promise<void>)
		| undefined;
	public readonly _onDidProviderOptionsChangeEmitter = this._register(
		new Emitter<void>(),
	);
	...
	constructor(
		...
		public providerOptions:
			| readonly languages.IInlineCompletionProviderOption[]
			| undefined,
		...
		private readonly _aiEditTelemetryService: IAiEditTelemetryService,
		@IInstantiationService
		private readonly _instantiationService: IInstantiationService,
	) {
		super();
		...
		this.setModelId = this._supportsSetModelId
			? async (modelId: string) => {
					await this._proxy.$handleInlineCompletionSetCurrentModelId(
						this.handle,
						modelId,
					);
				}
		...
		this.setProviderOption = this._supportsSetProviderOption
			? async (optionId: string, valueId: string) => {
					await this._proxy.$handleInlineCompletionSetProviderOption(
						this.handle,
						optionId,
						valueId,
					);
		...
		this.onDidChangeInlineCompletions = this._supportsOnDidChange
	...
	public _setModelInfo(
		newModelInfo: languages.IInlineCompletionModelInfo | undefined,
	) {
	...
	public async provideInlineCompletions(
		...
		const result = await this._proxy.$provideInlineCompletions(
			this.handle,
			model.uri,
			position,
			context,
	...
	public async handleEndOfLifetime(
		...
		function mapReason<T1, T2>(
			...
			if (
				reason.kind === languages.InlineCompletionEndOfLifeReasonKind.Ignored
			) {
				return {
					...reason,
					supersededBy: reason.supersededBy
						? f(reason.supersededBy)
						: undefined,
				};
			}
			return reason;
		...
		if (
			...
			reason.kind === languages.InlineCompletionEndOfLifeReasonKind.Rejected
		) {
			if (item.suggestionId !== undefined) {
		...
		const endOfLifeSummary: InlineCompletionEndOfLifeEvent = {
			...
			typingIntervalCharacterCount:
				lifetimeSummary.typingIntervalCharacterCount,
			...
			groupId:
				extractEngineFromCorrelationId(lifetimeSummary.correlationId) ??
				this.groupId,
			...
			partiallyAcceptedRatioSinceOriginal:
				lifetimeSummary.partiallyAcceptedRatioSinceOriginal,
			partiallyAcceptedCharactersSinceOriginal:
				lifetimeSummary.partiallyAcceptedCharactersSinceOriginal,
			superseded:
				reason.kind === InlineCompletionEndOfLifeReasonKind.Ignored &&
				!!reason.supersededBy,
			reason:
				reason.kind === InlineCompletionEndOfLifeReasonKind.Accepted
					? "accepted"
			...
			...forwardToChannelIf(
				isCopilotLikeExtension(this.providerId.extensionId!),
			),
		};

		const dataChannelForwardingTelemetryService =
...
function extractEngineFromCorrelationId(
