...
export class MainThreadChatDebug
	extends Disposable
	implements MainThreadChatDebugShape
{
	...
	$subscribeToCoreDebugEvents(): void {
		this._coreEventForwarder.value = this._chatDebugService.onDidAddEvent(
			(event) => {
	...
	$registerChatDebugLogProvider(handle: number): void {
		...
		disposables.add(
			this._chatDebugService.registerProvider({
				...
				resolveChatDebugLogEvent: async (eventId, token) => {
					const dto = await this._proxy.$resolveChatDebugLogEvent(
						handle,
						eventId,
						token,
					);
				...
				provideChatDebugLogExport: async (sessionResource, token) => {
					...
					const coreEventDtos = this._chatDebugService
						.getEvents(sessionResource)
						.filter((e) => this._chatDebugService.isCoreEvent(e))
						.map((e) => this._serializeEvent(e));
					const sessionTitle =
						this._chatService.getSessionTitle(sessionResource);
					const result = await this._proxy.$exportChatDebugLog(
						handle,
						sessionResource,
						coreEventDtos,
						sessionTitle,
				...
				resolveChatDebugLogImport: async (data, token) => {
					const result = await this._proxy.$importChatDebugLog(
						handle,
						VSBuffer.wrap(data),
						token,
					);
					...
					if (result.sessionTitle) {
						this._chatDebugService.setImportedSessionTitle(
							uri,
							result.sessionTitle,
						);
					}
		...
		this._chatDebugService.registerAvailableSessionsFetcher(async (token) => {
			const entries = await this._proxy.$getAvailableDebugSessionResources(
				handle,
				token,
			);
	...
	$acceptChatDebugEvent(handle: number, dto: IChatDebugEventDto): void {
		const sessionResource =
			(dto.sessionResource ? URI.revive(dto.sessionResource) : undefined) ??
	...
	private _reviveResolvedContent(
		dto: IChatDebugResolvedEventContentDto,
	): IChatDebugResolvedEventContent {
		switch (dto.kind) {
			case "text":
			...
			case "hook":
				return {
					...
					result:
						dto.result === "success"
							? ChatDebugHookResult.Success
							: dto.result === "error"
								? ChatDebugHookResult.Error
								: dto.result === "nonBlockingError"
									? ChatDebugHookResult.NonBlockingError
									: undefined,
					durationInMillis: dto.durationInMillis,
					input: dto.input,
					output: dto.output,
