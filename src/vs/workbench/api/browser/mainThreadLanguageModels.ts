...
export class MainThreadLanguageModels implements MainThreadLanguageModelsShape {
	...
	constructor(
		...
		this._store.add(
			this._chatProviderService.onDidChangeLanguageModels(() => {
				const currentModelIds = new Set(
					this._chatProviderService.getLanguageModelIds(),
				);
	...
	$registerLanguageModelProvider(vendor: string): void {
		...
		try {
			disposables.add(
				this._chatProviderService.registerLanguageModelProvider(vendor, {
					onDidChange: Event.filter(
						this._lmProviderChange.event,
						(e) => e.vendor === vendor,
						disposables,
					) as unknown as Event<void>,
					provideLanguageModelChatInfo: async (options, token) => {
						const modelsAndIdentifiers =
							await this._proxy.$provideLanguageModelChatInfo(
								vendor,
								options,
								token,
							);
		...
		const streaming = (async () => {
			try {
				for await (const part of response.stream) {
					...
					await this._proxy.$acceptResponsePart(
						requestId,
						new SerializableObjectWithBuffers(part),
					);
				}
				this._logService.trace(
					"[CHAT] request DONE",
	...
	private _registerAuthenticationProvider(
		...
		const disposables = new DisposableStore();
		const provider = new LanguageModelAccessAuthProvider(
			authProviderId,
			auth.providerLabel,
			accountLabel,
		);
