...
if (process.env.VSCODE_DEV) {
	...
	process.on("warning", (warning: any) => {
		if (
			warning.code === "ExperimentalWarning" ||
...
function readReconnectionValue(envKey: string, fallback: number): number {
	...
	const result =
		millis > Number.MAX_SAFE_INTEGER ? Number.MAX_SAFE_INTEGER : millis;
	console.log(
	...
	if (extHostConnection.type === ExtHostConnectionType.MessagePort) {
		return new Promise<IMessagePassingProtocol>((resolve, reject) => {
			const withPorts = (ports: MessagePortMain[]) => {
				...
				port.on("message", (e) =>
					onMessage.fire(VSBuffer.wrap(e.data as Uint8Array)),
				);
				port.on("close", () => {
		...
		return new Promise<PersistentProtocol>((resolve, reject) => {
			...
			const reconnectionGraceTime = readReconnectionValue(
				"VSCODE_RECONNECTION_GRACE_TIME",
