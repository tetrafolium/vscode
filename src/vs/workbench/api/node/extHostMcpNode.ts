...
export class NodeExtHostMpcService extends ExtHostMcpService {
	private nodeServers = this._register(
		new DisposableMap<number, McpStdioStateHandler>(),
	);

	protected override _startMcp(
		id: number,
	...
	private async startNodeMpc(
		id: number,
		launch: McpServerTransportStdio,
		defaultCwd?: URI,
	): Promise<void> {
		...
		for (const [key, value] of Object.entries(launch.env)) {
			...
			if (key.toUpperCase() === "PATH" && value !== null) {
				env[key] = env[key]
					? `${env[key]}${path.delimiter}${String(value)}`
					: String(value);
				continue;
			}
		...
		try {
			const home = homedir();
			let cwd = launch.cwd
				? untildify(launch.cwd, home)
				: defaultCwd?.fsPath || home;
			if (!path.isAbsolute(cwd)) {
				cwd = defaultCwd
					? path.join(defaultCwd.fsPath, cwd)
					: path.join(home, cwd);
			}
...
class McpHTTPHandleNode extends McpHTTPHandle {
	...
	protected override async _fetchInternal(
		url: string,
