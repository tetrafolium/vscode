			...
			return http.createServer((req, res) => {
				...
				switch (reqUrl.pathname) {
					case "/": {
						...
						if (error) {
							res.writeHead(302, {
								location: `/done?error=${reqUrl.searchParams.get("error_description") || error}`,
							});
							res.end();
						...
						if (this.state !== state) {
							res.writeHead(302, {
								location: `/done?error=${encodeURIComponent("State does not match.")}`,
							});
							res.end();
