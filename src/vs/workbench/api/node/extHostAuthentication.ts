...
import {
	...
	IAuthorizationDeviceResponse,
	isAuthorizationDeviceResponse,
	isAuthorizationTokenResponse,
	IAuthorizationDeviceTokenErrorResponse,
...
export class NodeDynamicAuthProvider extends DynamicAuthProvider {
	constructor(
		...
		if (!initData.remote.isRemote && serverMetadata.authorization_endpoint) {
			...
			this._createFlows.unshift({
				...
				handler: (scopes, progress, token) =>
					this._createWithLoopbackServer(scopes, progress, token),
			});
		}
		...
		if (serverMetadata.device_authorization_endpoint) {
			this._createFlows.push({
				...
				handler: (scopes, progress, token) =>
					this._createWithDeviceCode(scopes, progress, token),
			});
		}
	}

	private async _createWithLoopbackServer(
		scopes: string[],
		...
		const callbackUri = URI.parse(
			`${this._initData.environment.appUriScheme}://dynamicauthprovider/${this.authorizationServer.authority}/redirect?nonce=${nonce}`,
		);
		let appUri: URI;
		try {
		...
		try {
			...
			this._logger.info(
				`Authorization code received for scopes: ${scopeString}`,
			);
	...
	private async _createWithDeviceCode(
		scopes: string[],
		progress: vscode.Progress<IProgressStep>,
		token: vscode.CancellationToken,
	): Promise<IAuthorizationTokenResponse> {
		...
		if (!this._serverMetadata.device_authorization_endpoint) {
			throw new Error(
				"Device authorization endpoint not available in server metadata",
			);
		}
		...
		if (!deviceCodeResponse.ok) {
			...
			throw new Error(
				`Device code request failed: ${deviceCodeResponse.status} ${deviceCodeResponse.statusText} - ${text}`,
			);
		}

		const deviceCodeData: IAuthorizationDeviceResponse =
		...
		while (Date.now() < expiresAt) {
			...
			try {
				...
				if (tokenResponse.ok) {
					const tokenData: IAuthorizationTokenResponse =
					...
					this._logger.info(
						`Device code flow completed successfully for scopes: ${scopeString}`,
					);
					return tokenData;
				} else {
					...
					try {
						...
						throw new Error(
							`Token request failed with status ${tokenResponse.status}: ${tokenResponse.statusText}`,
						);
					}
					...
					if (
						...
						errorData.error === AuthorizationDeviceCodeErrorType.SlowDown
					) {
						// Server is asking us to slow down
						await new Promise((resolve) => setTimeout(resolve, pollInterval));
						continue;
					} else if (
						errorData.error === AuthorizationDeviceCodeErrorType.ExpiredToken
					) {
						throw new Error("Device code expired. Please try again.");
					} else if (
						errorData.error === AuthorizationDeviceCodeErrorType.AccessDenied
					) {
						throw new CancellationError();
					} else if (errorData.error === AuthorizationErrorType.InvalidClient) {
						this._logger.warn(
							`Client ID (${this._clientId}) was invalid, generated a new one.`,
						);
						await this._generateNewClientId();
						throw new Error(
							`Client ID was invalid, generated a new one. Please try again.`,
						);
					} else {
						throw new Error(
							`Token request failed: ${errorData.error_description || errorData.error || "Unknown error"}`,
						);
					}
				}
			} catch (error) {
...
export class NodeExtHostAuthentication
	extends ExtHostAuthentication
	implements IExtHostAuthentication
{
	protected override readonly _dynamicAuthProviderCtor =
		NodeDynamicAuthProvider;
	protected override readonly _xaaAuthProviderCtor = XaaifyAuthProvider(
		NodeDynamicAuthProvider,
	);
