...
export class Memento<T extends object> {
	private static readonly applicationMementos = new Map<
		string,
		ScopedMemento<unknown>
	>();
	private static readonly applicationSharedMementos = new Map<
		string,
		ScopedMemento<unknown>
	>();
	private static readonly profileMementos = new Map<
		string,
		ScopedMemento<unknown>
	>();
	...
	getMemento(scope: StorageScope, target: StorageTarget): Partial<T> {
		switch (scope) {
			...
			case StorageScope.APPLICATION_SHARED: {
				let applicationSharedMemento = Memento.applicationSharedMementos.get(
					this.id,
				);
				if (!applicationSharedMemento) {
					applicationSharedMemento = new ScopedMemento(
						this.id,
						scope,
						target,
						this.storageService,
					);
					Memento.applicationSharedMementos.set(
						this.id,
						applicationSharedMemento,
					);
				}
