...
class CapturingConfigurationService extends TestConfigurationService {
	override async updateValue(
		key: string,
		value: unknown,
		arg3?: ConfigurationTarget | unknown,
	): Promise<void> {
		const target =
			typeof arg3 === "number" ? (arg3 as ConfigurationTarget) : undefined;
	...
	lastUpdate:
		| { key: string; value: unknown; target?: ConfigurationTarget }
		| undefined;
}
...
function createStubFileService(opts?: {
	...
	return {
		realpath: async (resource: URI) =>
			opts?.realpathReturns ? opts.realpathReturns(resource) : resource,
...
suite("AgentHostPermissionService", () => {
	...
	function createService(
		initial?: AgentHostPermissionsSetting,
		fileService = createStubFileService(),
	): {
		service: AgentHostPermissionService;
		config: CapturingConfigurationService;
	} {
		const config = new CapturingConfigurationService();
		if (initial) {
			void config.setUserConfiguration(
				AgentHostLocalFilePermissionsSettingId,
				initial,
			);
		}
		const service = disposables.add(
			new AgentHostPermissionService(config, fileService, new NullLogService()),
		);
	...
	test("implicit read grant covers descendants but not parent or sibling", async () => {
		const { service } = createService();
		disposables.add(
			service.grantImplicitRead("host", URI.file("/plugins/foo")),
		);
	...
	test("request resolves immediately when already granted", async () => {
		...
		await service.request("host", {
			channel: "ahp-root://",
			uri: URI.file("/plugins/foo/x.md").toString(),
			read: true,
		});
	...
	test("allow grants in-memory until connection closes", async () => {
		...
		const promise = service.request("host", {
			channel: "ahp-root://",
			uri: uri.toString(),
			read: true,
		});
		...
		assert.strictEqual(
			await service.check("host", uri, AgentHostPermissionMode.Read),
			true,
		);
	...
	test("allowAlways covers immediate retry without waiting for the settings write", async () => {
		...
		const service = disposables.add(
			new AgentHostPermissionService(
				config,
				createStubFileService(),
				new NullLogService(),
			),
		);
	...
	test("concurrent identical requests share one pending entry", async () => {
		const { service } = createService();
		const a = service.request("host", {
			channel: "ahp-root://",
			uri: URI.file("/etc/foo").toString(),
			read: true,
		});
		const b = service.request("host", {
			channel: "ahp-root://",
			uri: URI.file("/etc/foo").toString(),
			read: true,
		});
	...
	test("grants for one host do not leak into another host", async () => {
		...
		disposables.add(
			service.grantImplicitRead("host-a", URI.file("/plugins/p")),
		);
	...
	test("connectionClosed only affects the named address", async () => {
		const { service } = createService();
		disposables.add(
			service.grantImplicitRead("host-a", URI.file("/plugins/a")),
		);
		disposables.add(
			service.grantImplicitRead("host-b", URI.file("/plugins/b")),
		);

		const pendingA = service.request("host-a", {
			channel: "ahp-root://",
			uri: URI.file("/etc/a").toString(),
			read: true,
		});
		const pendingB = service.request("host-b", {
			channel: "ahp-root://",
			uri: URI.file("/etc/b").toString(),
			read: true,
		});
		await new Promise((resolve) => setTimeout(resolve, 0));
	...
	test("pendingFor returns only this host's requests, with normalized address", async () => {
		const { service } = createService();
		const a = service.request("host-a", {
			channel: "ahp-root://",
			uri: URI.file("/etc/a").toString(),
			read: true,
		});
		const b = service.request("ws://host-b", {
			channel: "ahp-root://",
			uri: URI.file("/etc/b").toString(),
			read: true,
		});
		...
		assert.strictEqual(
			service.pendingFor("host-b").get()[0].uri.toString(),
			URI.file("/etc/b").toString(),
		);
	...
	test("grantImplicitRead asynchronously upgrades to realpath", async () => {
		...
		const fileService = createStubFileService({
			realpathReturns: (uri) =>
				uri.path === "/home/me/plugin-link" ? URI.file("/real/plugin") : uri,
		});
		const { service } = createService(undefined, fileService);
		disposables.add(
			service.grantImplicitRead("host", URI.file("/home/me/plugin-link")),
		);
	...
	test("allowAlways merges with existing APPLICATION-scoped grants instead of overwriting them", async () => {
		...
		const existing: AgentHostPermissionsSetting = {
			"other-host": {
				[URI.file("/etc/preexisting").toString()]:
					AgentHostAccessMode.ReadWrite,
			},
		};
		const originalInspect = config.inspect.bind(config);
		(config as { inspect: (key: string) => unknown }).inspect = (
			key: string,
		) => {
	...
	test("persisted entries with malformed URI keys or unknown modes are ignored", async () => {
		const { service } = createService({
			host: {
				...
				[URI.file("/etc/garbage").toString()]:
					"unknown" as unknown as AgentHostAccessMode,
