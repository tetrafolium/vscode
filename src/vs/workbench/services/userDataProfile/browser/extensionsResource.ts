		...
		if (extensionsToInstall.length) {
			...
			await Promise.all(
				extensionsToInstall.map(async (e) => {
					...
					if (
						(await this.extensionManagementService.canInstall(extension)) ===
						true
					) {
						this.logService.trace(
							...
							for (const installExtensionInfo of installExtensionInfos) {
								...
								progress?.(
									localize(
										...
										installExtensionInfo.extension.displayName ??
		...
		return this.withProfileScopedServices(
			...
			async (extensionEnablementService) => {
				...
				for (const extension of installedExtensions) {
					...
					const disabled = disabledExtensions.some((disabledExtension) =>
...
export abstract class ExtensionsResourceTreeItem implements IProfileResourceTreeItem {
	...
	> {
		...
		const that = this;
		return extensions.map<IProfileResourceChildTreeItem & IProfileExtension>(
			(e) => ({
				...e,
...
export class ExtensionsResourceExportTreeItem extends ExtensionsResourceTreeItem {
	...
	isFromDefaultProfile(): boolean {
		return (
			!this.profile.isDefault && !!this.profile.useDefaultFlags?.extensions
		);
	}
