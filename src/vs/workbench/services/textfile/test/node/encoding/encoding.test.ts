...
export async function detectEncodingByBOM(
	file: string,
): Promise<
	| typeof encoding.UTF16be
	| typeof encoding.UTF16le
	| typeof encoding.UTF8_with_bom
	| null
> {
	try {
...
suite("Encoding", () => {
	...
	async function readAndDecodeFromDisk(
		path: string,
		...
		return new Promise<string>((resolve, reject) => {
			fs.readFile(path, (err, data) => {
				if (err) {
					...
					resolve(
						importAMDNodeModule<typeof import("@vscode/iconv-lite-umd")>(
	...
	[
		...
		test(`toEncodeReadable - empty readable to ${utfEncoding} with BOM`, async function () {
			...
			const encodedReadable = encoding.toEncodeReadable(source, utfEncoding, {
				addBOM: true,
			});

			const expected = VSBuffer.wrap(Buffer.from(relatedBom)).toString();
			const actual = streams
				.consumeReadable(await encodedReadable, VSBuffer.concat)
				.toString();
