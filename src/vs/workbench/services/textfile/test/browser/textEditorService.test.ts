...
suite("TextEditorService", () => {
	...
	class FileServiceProvider extends Disposable {
		constructor(scheme: string, @IFileService fileService: IFileService) {
			...
			this._register(
				fileService.registerProvider(scheme, new NullFileSystemProvider()),
			);
		}
	}
	...
	test("createTextEditor - basics", async function () {
		...
		let fileModel = disposables.add(
			(await contentInput.resolve()) as ITextFileEditorModel,
		);
		...
		let model = disposables.add(
			(await input.resolve()) as UntitledTextEditorModel,
		);
		...
		input = disposables.add(
			service.createTextEditor({ resource: URI.parse("custom:resource") }),
		);
		...
		const untypedSideBySideInput =
			input.toUntyped() as IResourceSideBySideEditorInput;
		assert.strictEqual(
