...
function normalizeRegisteredEditorInfo(
	...
	implements IEditorResolverService
{
	...
	> = new Map<string | glob.IRelativePattern, Map<string, RegisteredEditors>>();
	...
	async resolveEditor(
		...
		const editorAssociationType = isResourceDiffEditorInput(untypedEditor)
			? EditorAssociationType.DiffEditor
			: isResourceMergeEditorInput(untypedEditor)
				? EditorAssociationType.MergeEditor
				: EditorAssociationType.Editor;
		if (
			this.cache &&
			resource &&
			(this.resourceMatchesCache(resource) ||
				this.resourceMatchesUserAssociation(resource, editorAssociationType))
		) {
		...
		if (
			!selectedEditor &&
			(untypedEditor.options?.override || isEditorInputWithOptions(editor))
		) {
			return ResolvedStatus.NONE;
	...
	private getAssociationsForResourceByType(
		resource: URI,
		associationType: EditorAssociationType,
	): EditorAssociations {
		if (
			associationType === EditorAssociationType.DiffEditor ||
			associationType === EditorAssociationType.MergeEditor
		) {
	...
	private getRawAssociationsForResourceByType(
		...
		return diffAssociations.length
			? diffAssociations
	...
	private getRawAssociationsForResourceFromSetting(
		...
		return matchingAssociations.sort(
			(a, b) =>
				(b.filenamePattern?.length ?? 0) - (a.filenamePattern?.length ?? 0),
		);
	}
	...
	private getAllUserAssociationsForSetting(
		...
		const inspectedEditorAssociations =
			this.configurationService.inspect<{ [fileNamePattern: string]: string }>(
				settingId,
			) || {};
	...
	private updateUserAssociationsForSetting(
		...
		const currentAssociations =
			this.getAllUserAssociationsForSetting(settingId);
	...
	private findMatchingEditors(
		resource: URI,
		associationType: EditorAssociationType = EditorAssociationType.Editor,
	): RegisteredEditor[] {
		...
		for (const [key, editors] of this._flattenedEditors) {
			for (const editor of editors) {
				if (
					associationType === EditorAssociationType.DiffEditor &&
					!editor.editorFactoryObject.createDiffEditorInput
				) {
					continue;
				}
				if (
					associationType === EditorAssociationType.MergeEditor &&
					!editor.editorFactoryObject.createMergeEditorInput
				) {
					continue;
				}
				...
				if (
					(foundInSettings &&
						this.getEffectivePriority(editor.editorInfo, associationType) !==
		...
		return matchingEditors.sort((a, b) => {
			const aPriority = this.getEffectivePriority(
				a.editorInfo,
				associationType,
			);
			const bPriority = this.getEffectivePriority(
				b.editorInfo,
				associationType,
			);
			// Very crude if priorities match longer glob wins as longer globs are normally more specific
			if (
				priorityToRank(bPriority) === priorityToRank(aPriority) &&
	...
	public getEditors(resource?: URI): RegisteredEditorInfo[] {
		...
		if (URI.isUri(resource)) {
			...
			if (
				editors.find(
					(e) =>
						e.editorInfo.priority.editor === RegisteredEditorPriority.exclusive,
				)
			) {
				return [];
			}
	...
	private async doResolveEditor(
		...
		const inputWithOptions =
			await selectedEditor.editorFactoryObject.createEditorInput(editor, group);
		options = inputWithOptions.options ?? options;
	...
	private async doHandleConflictingDefaults(
		...
		const associationType = isResourceDiffEditorInput(untypedInput)
			? EditorAssociationType.DiffEditor
			: isResourceMergeEditorInput(untypedInput)
				? EditorAssociationType.MergeEditor
				: EditorAssociationType.Editor;
		const editors = this.findMatchingEditors(resource, associationType);
	...
	private mapEditorsToQuickPickEntry(
		resource: URI,
		showDefaultPicker: boolean | undefined,
		associationType: EditorAssociationType,
	) {
		...
		registeredEditors = registeredEditors.sort((a, b) => {
			if (a.editorInfo.id === DEFAULT_EDITOR_ASSOCIATION.id) {
				...
				return (
					priorityToRank(
						this.getEffectivePriority(b.editorInfo, associationType),
					) -
					priorityToRank(
						this.getEffectivePriority(a.editorInfo, associationType),
					)
				);
			}
		});
	...
	private async doPickEditor(
		...
		const associationType = isResourceDiffEditorInput(editor)
			? EditorAssociationType.DiffEditor
			: EditorAssociationType.Editor;
			...
			disposables.add(
				editorPicker.onDidAccept((e) => {
					...
					if (resource && showDefaultPicker && result?.item.id) {
						this.updateUserAssociationsForType(
							associationType,
							`*${extname(resource)}`,
							result.item.id,
						);
	...
	private cacheEditors() {
		...
		const userAssociations = [
			...
			...this.getAllUserAssociationsForSetting(
				diffEditorsAssociationsSettingId,
			),
		];
