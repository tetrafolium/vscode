...
suite("Custom Editor Label Service", () => {
	...
	async function createCustomLabelService(
		...
		[CustomEditorLabelService, TestConfigurationService, TestServiceAccessor]
	> {
	...
	async function updatePattern(
		...
		configService.onDidChangeConfigurationEmitter.fire({
			affectsConfiguration: (key: string) =>
				key === CustomEditorLabelService.SETTING_ID_PATTERNS,
	...
	test("Custom Labels: filename.extname", async () => {
		const [customLabelService, configService] =
			await createCustomLabelService();
