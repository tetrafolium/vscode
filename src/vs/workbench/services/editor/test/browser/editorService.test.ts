...
suite("EditorService", () => {
		...
		disposables.add(
			registerTestEditor(
				...
				[
					...
					new SyncDescriptor(TestForceRevealFileEditorInput),
				],
	...
	): Promise<[EditorPart, EditorService, TestServiceAccessor]> {
	...
	async function testOpenBasics(
		...
		assert.strictEqual(
			editorService.getVisibleTextEditorControls(
	...
	test("openEditor() - force-reveal typed editors reveal instead of split", async () => {
		...
		const input1 = disposables.add(
			new TestForceRevealFileEditorInput(
		...
		const input1Group = (await service.openEditor(input1, { pinned: true }))
			?.group;
		const input2Group = (
			await service.openEditor(input2, { pinned: true }, SIDE_GROUP)
		)?.group;
	...
	test("locked groups - workbench.editor.revealIfOpen", async () => {
		...
		const [part, service, accessor] =
			await createEditorService(instantiationService);
	...
	async function testOpenEditors(useOpenEditors: boolean) {
		...
		let lastUntitledEditorFactoryEditor:
			| IUntitledTextResourceEditorInput
			| undefined = undefined;
		let lastDiffEditorFactoryEditor: IResourceDiffEditorInput | undefined =
			undefined;
			...
			accessor.editorResolverService.registerEditor(
				...
				{},
				{
					...
					createDiffEditorInput: (diffEditor) => {
						...
						return {
							editor: createTestFileEditorInput(
								URI.file(`diff-editor-${diffEditorFactoryCalled}`),
								TEST_EDITOR_INPUT_ID,
							),
						};
					},
		...
		// untyped
		{
			...
			{
				const untypedEditor: IResourceEditorInput = {
				...
				const untypedEditorReplacement: IResourceEditorInput = {
			...
			// untyped resource editor, options (sticky: true, preserveFocus: true), no group
			{
				...
				assert.strictEqual(
					(lastEditorFactoryEditor as IResourceEditorInput).resource.toString(),
					untypedEditor.resource.toString(),
				);
				assert.strictEqual(
					(lastEditorFactoryEditor as IResourceEditorInput).options
						?.preserveFocus,
					true,
				);
				assert.ok(!lastUntitledEditorFactoryEditor);
		...
		// Untyped untitled
		{
			...
			{
				const untypedEditor: IUntitledTextResourceEditorInput = {
					resource: undefined,
					options: { override: TEST_EDITOR_INPUT_ID },
				};
				const pane = await openEditor(untypedEditor);
			...
			// untyped untitled editor, options (sticky: true, preserveFocus: true), no group
			{
				...
				assert.strictEqual(lastUntitledEditorFactoryEditor, untypedEditor);
				assert.strictEqual(
					(lastUntitledEditorFactoryEditor as IUntitledTextResourceEditorInput)
		...
		// Untyped diff
		{
			// untyped diff editor, no options, no group
			{
				const untypedEditor: IResourceDiffEditorInput = {
					original: {
						resource: URI.file("file-original.editor-service-override-tests"),
					},
					modified: {
						resource: URI.file("file-modified.editor-service-override-tests"),
					},
					options: { override: TEST_EDITOR_INPUT_ID },
				};
				const pane = await openEditor(untypedEditor);
			...
			// untyped diff editor, options (sticky: true, preserveFocus: true), no group
			{
				...
				assert.strictEqual(lastDiffEditorFactoryEditor, untypedEditor);
				assert.strictEqual(
					(lastDiffEditorFactoryEditor as IUntitledTextResourceEditorInput)
						.options?.preserveFocus,
					true,
				);
				assert.strictEqual(
		...
		if (useOpenEditors) {
			...
			{
				const untypedEditor1: IResourceEditorInput = {
					resource: URI.file("file1.editor-service-override-tests"),
				};
				const untypedEditor2: IResourceEditorInput = {
					resource: URI.file("file2.editor-service-override-tests"),
				};
				const untypedEditor3: EditorInputWithOptions = {
					editor: createTestFileEditorInput(
						URI.file("file3.editor-service-override-tests"),
						TEST_EDITOR_INPUT_ID,
					),
				};
				const untypedEditor4: EditorInputWithOptions = {
					editor: createTestFileEditorInput(
						URI.file("file4.editor-service-override-tests"),
						TEST_EDITOR_INPUT_ID,
					),
				};
				const untypedEditor5: IResourceEditorInput = {
	...
	test("openEditors() handles workspace trust (typed editors)", async () => {
		...
		const oldHandler =
			accessor.workspaceTrustRequestService.requestOpenUrisHandler;
		...
		try {
			...
			await service.openEditors(
				[{ editor: input1 }, { editor: input2 }, { editor: sideBySideInput }],
				undefined,
				{ validateTrust: true },
			);
	...
	test("openEditors() extracts proper resources from untyped editors for workspace trust", async () => {
		...
		try {
			...
			await service.openEditors([input, otherInput], undefined, {
				validateTrust: true,
			});
	...
	test("active editor change / visible editor change events", async function () {
		...
		async function closeEditorAndWaitForNextToOpen(
			group: IEditorGroup,
			input: EditorInput,
		): Promise<void> {
	...
	test("editors change event", async function () {
		...
		async function assertEditorsChangeEvent(
			fn: () => Promise<unknown>,
			expected: number,
		) {
	...
	async function testSaveRevertUntitled(
		options: IBaseSaveRevertAllEditorOptions,
		expectUntitled: boolean,
		expectScratchpad: boolean,
	) {
		const [, service] = await createEditorService();
		const input1 = createTestFileEditorInput(
			URI.parse("my://resource1"),
		...
		scratchpadInput.capabilities =
			EditorInputCapabilities.Scratchpad | EditorInputCapabilities.Untitled;
	...
	function awaitActiveEditorChange(
		editorService: IEditorService,
	): Promise<void> {
	...
	test("activeEditorPane scopedContextKeyService", async function () {
		...
		const editorContextKeyService =
			service.activeEditorPane?.scopedContextKeyService;
