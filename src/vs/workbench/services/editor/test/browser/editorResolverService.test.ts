...
suite("EditorResolverService", () => {
	...
	async function createEditorResolverService(
		instantiationService: ITestInstantiationService = workbenchInstantiationService(
			undefined,
			disposables,
		),
	): Promise<[EditorPart, EditorResolverService, TestServiceAccessor]> {
	...
	function constructDisposableFileEditorInput(
		uri: URI,
		typeId: string,
	...
	function constructDisposableDiffEditorInput(
		accessor: TestServiceAccessor,
		original: { readonly resource?: URI },
		modified: { readonly resource?: URI },
		typeId: string,
	): DiffEditorInput {
		return accessor.instantiationService.createInstance(
			...
			constructDisposableFileEditorInput(
				original.resource ?? URI.from({ scheme: Schemas.untitled }),
				typeId,
				disposables,
			),
			constructDisposableFileEditorInput(
				modified.resource ?? URI.from({ scheme: Schemas.untitled }),
				typeId,
				disposables,
			),
			undefined,
	...
	test("Simple Resolve", async () => {
		...
		if (
			resultingResolution !== ResolvedStatus.ABORT &&
			resultingResolution !== ResolvedStatus.NONE
		) {
			assert.strictEqual(
	...
	test("Untitled Resolve", async () => {
		...
		resultingResolution = await service.resolveEditor(
			{ resource: URI.from({ scheme: Schemas.untitled, path: "foo.test" }) },
			part.activeGroup,
		);
		assert.ok(resultingResolution);
		assert.notStrictEqual(typeof resultingResolution, "number");
	...
	test("Diff editor Resolve - Falls back to editor associations", async () => {
		...
		const [part, service, accessor] =
			await createEditorResolverService(instantiationService);
		...
		const defaultRegisteredEditor = service.registerEditor(
			...
			{},
			{
				...
				createDiffEditorInput: ({ modified, original }) => {
					defaultDiffCounter++;
					return {
						editor: constructDisposableDiffEditorInput(
							accessor,
							original,
							modified,
							TEST_EDITOR_INPUT_ID,
		...
		const resultingResolution = await service.resolveEditor(
			{
				original: {
					resource: URI.file("resource-basics.test-diff-association"),
				},
				modified: {
					resource: URI.file("resource-basics.test-diff-association"),
				},
	...
	test("Diff editor Resolve - Diff associations override editor associations", async () => {
		...
		if (
			diffResolution !== ResolvedStatus.ABORT &&
			diffResolution !== ResolvedStatus.NONE
		) {
		...
		const editorResolution = await service.resolveEditor(
			{ resource: URI.file("resource-basics.test-diff-association") },
			part.activeGroup,
		);
		...
		if (
			editorResolution !== ResolvedStatus.ABORT &&
			editorResolution !== ResolvedStatus.NONE
		) {
			assert.strictEqual(
				editorResolution.editor.typeId,
				EDITOR_ASSOCIATION_INPUT_ID,
			);
	...
	test("Diff editor Resolve - priority.diff overrides priority.editor for diffs", async () => {
		...
		const editorResolution = await service.resolveEditor(
			{ resource: URI.file("my://resource.test-diff-priority") },
			part.activeGroup,
		);
		assert.ok(editorResolution);
		assert.notStrictEqual(typeof editorResolution, "number");
