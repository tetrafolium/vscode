...
export function findGroup(
	...
	preferredGroup:
		| Exclude<PreferredGroup, AUX_WINDOW_GROUP_TYPE | MODAL_GROUP_TYPE>
		| undefined,
): FindGroupResult;
export function findGroup(
	accessor: ServicesAccessor,
...
function handleGroupResult(
	...
	const modalEditorMode = configurationService.getValue<string>(
		"workbench.editor.useModal",
	);
	const editorInput = isEditorInputWithOptions(editor)
		? editor.editor
		: isEditorInput(editor)
			? editor
			: undefined;
	const requiresModal =
		editorInput instanceof EditorInput &&
		editorInput.hasCapability(EditorInputCapabilities.RequiresModal);
	if (
		modalEditorPart &&
...
function doFindGroup(
	...
	if (
		isEditorInput(editor) &&
		editor.hasCapability(EditorInputCapabilities.RequiresModal)
	) {
		group = editorGroupService
			.createModalEditorPart(options?.modal)
	...
	// Group: Aux Window
		...
		group = editorGroupService
			.createAuxiliaryEditorPart(options?.auxiliary)
	...
	if (
		!group &&
		configurationService.getValue<string>("workbench.editor.useModal") === "all"
	) {
		group = editorGroupService
			.createModalEditorPart(options?.modal)
			.then((part) => part.activeGroup);
