...
export class FileWalker {
	...
	constructor(config: IFileQuery) {
		...
		const globOptions = config.ignoreGlobCase
			? { ignoreCase: true }
			: undefined;
		this.includePattern =
			config.includePattern && glob.parse(config.includePattern, globOptions);
		...
		this.globalExcludePattern =
			config.excludePattern && glob.parse(config.excludePattern, globOptions);
			...
			config.folderQueries
				...
				.forEach((otherRootFolder) => {
					...
					if (isEqualOrParent(otherRootFolder, fqPath, config.ignoreGlobCase)) {
						folderExcludeExpression[path.relative(fqPath, otherRootFolder)] =
							true;
					}
				});
	...
	private addDirectoryEntries(
		...
		const filePatternMatch =
			this.filePattern &&
			relativeFiles.find((f) =>
				strings.equals(f, this.filePattern, this.config.ignoreGlobCase),
			);
		if (filePatternMatch) {
			this.matchFile(onResult, {
				base,
			...
			for (let i = 0, n = entries.length; i < n; i++) {
				...
				if (
					excludePattern.test(
						relativePath,
						basename,
						!strings.equals(filePattern, basename, ignoreGlobCase)
							? hasSibling
							: undefined,
					)
				) {
	...
	private doWalk(
		...
		this.parallel(
			files,
			(file: string, clb: (error: Error | null, _?: any) => void): void => {
				// Check canceled
						...
						if (stat.isDirectory()) {
							...
							return this.realPathIfNeeded(
								...
								(error, realpath) => {
									...
									// Continue walking
									return Promises.readdir(currentAbsolutePath).then(
										(children) => {
											if (this.isCanceled || this.isLimitHit) {
												return clb(null);
											}
...
class AbsoluteAndRelativeParsedExpression {
	...
	private init(expr: glob.IExpression): void {
		...
		this.absoluteParsedExpr =
			absoluteGlobExpr && glob.parse(absoluteGlobExpr, globOptions);
		this.relativeParsedExpr =
			relativeGlobExpr && glob.parse(relativeGlobExpr, globOptions);
	}
