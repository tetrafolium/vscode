	...
	implements ISearchResultProvider
{
	...
	private readonly _onDidReceiveTextSearchMatch = this._register(
		new Emitter<{ match: IFileMatch<UriComponents>; queryId: number }>(),
	);
			...
			await Promise.all(
				query.folderQueries.map(async (fq) => {
					...
					const ignorePathCasing =
						this.uriIdentityService.extUri.ignorePathCasing(fq.folder);
		...
		if (!this._worker) {
			try {
				this._worker = this._register(
					this.webWorkerService.createWorkerClient<ILocalFileSearchWorker>(
