	...
	implements ILocalFileSearchWorker, IWebWorkerServerRequestHandler
{
	...
	private async walkFolderQuery(
		...
		extUri: ExtUri,
		onFile: (file: FileNode) => Promise<unknown> | unknown,
		token: CancellationToken,
	): Promise<void> {
		const ignoreGlobCase =
			queryProps.ignoreGlobCase || folderQuery.ignoreGlobCase;
		...
		const resolveDirectory = async (
			directory: DirNode,
			onFile: (f: FileNode) => Promise<unknown> | unknown,
		) => {
			if (token.isCancellationRequested) {
				return;
			}
...
function pathExcludedInQuery(
	...
	if (
		...
		glob.match(queryProps.excludePattern, fsPath, globOptions)
	) {
		return true;
	}
	return false;
...
function pathIncludedInQuery(
	...
	if (
		queryProps.excludePattern &&
		glob.match(queryProps.excludePattern, path, globOptions)
	) {
		return false;
	}
	...
	if (queryProps.includePattern || queryProps.usingSearchPaths) {
		if (
			...
			glob.match(queryProps.includePattern, path, globOptions)
		) {
			return true;
		}
			...
			return (
				...
				queryProps.folderQueries.some((fq) => {
					...
					if (extUri.isEqualOrParent(uri, searchPath)) {
						...
						return (
							!fq.includePattern ||
							!!glob.match(fq.includePattern, relPath, globOptions)
						);
					} else {
						return false;
					}
