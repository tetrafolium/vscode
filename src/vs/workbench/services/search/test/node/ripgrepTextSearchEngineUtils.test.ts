...
suite("RipgrepTextSearchEngine", () => {
	...
	test("fixRegexNewline - re", () => {
		function testFixRegexNewline([inputReg, testStr, shouldMatch]: readonly [
			string,
			string,
			boolean,
		]): void {
	...
	suite("RipgrepParser", () => {
		...
		function testParser(
			inputData: string[],
			expectedResults: TextSearchResult2[],
		): void {
			const testParser = new RipgrepParser(
				1000,
				TEST_FOLDER,
	...
	suite("getRgArgs", () => {
		test("simple includes", () => {
			...
			function testGetRgArgs(
				includes: string[],
				expectedFromIncludes: string[],
			): void {
			...
			].forEach(([includes, expectedFromIncludes]) =>
				testGetRgArgs(<string[]>includes, <string[]>expectedFromIncludes),
			);
		});
	});
	...
	test("brace expansion for ripgrep", () => {
		...
		[
			...
			testBraceExpansion(<string>includePattern, <string[]>expectedPatterns),
		);
	});
});
