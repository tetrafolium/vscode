...
flakySuite("TextSearch-integration", function () {
	...
	test("Text: 语", () => {
		...
		return doSearchTest(config, 1).then((results) => {
			const matchRange = (<ITextSearchMatch>(
				results[0].results![0]
			)).rangeLocations.map((e) => e.source);
	...
	test("Multiple matches on line: h\\d,", () => {
		...
		return doSearchTest(config, 15).then((results) => {
			...
			assert.strictEqual(
				(<ISearchRange[]>match.rangeLocations.map((e) => e.source)).length,
				5,
			);
		});
	});
	...
	test("Search with context matches", () => {
		...
		return doSearchTest(config, 3).then((results) => {
			...
			assert.strictEqual(
				(<ITextSearchContext>results[0].results![0]).lineNumber,
				24,
			);
			assert.strictEqual(
			...
			assert.strictEqual(
				(<ITextSearchContext>results[0].results![2]).lineNumber,
				26,
			);
			assert.strictEqual(
				(<ITextSearchContext>results[0].results![2]).text,
				"        compiler.emit();",
			);
		});
	});
			...
			return doSearchTest(config, 0).then(
				...
				(err) => {
					...
					const regexParseErrorForLookAround =
						"Regex parse error: length of lookbehind assertion is not limited";
