...
suite("FolderQuerySearchTree", () => {
	...
	test("match correct entry with query/fragment", () => {
		...
		const result2 = tree.findQueryFragmentAwareSubstr(
			URI.parse("file:///folder3/file.txt?query3"),
		);
		assert.deepStrictEqual(result2, { folder: fq4.folder, index: 3 });

		const result3 = tree.findQueryFragmentAwareSubstr(
			URI.parse("file:///folder3/file.txt"),
		);
	...
	test("can find substr of non-query/fragment URIs", () => {
		...
		const result2 = tree.findQueryFragmentAwareSubstr(
			URI.parse("file:///folder3/hello/world"),
		);
	...
	test("`/` as a path", () => {
		...
		assert.deepStrictEqual(
			trie.findQueryFragmentAwareSubstr(URI.parse("memfs:/file.txt?q=1")),
			{ folder: URI.parse("memfs:/?q=1"), index: 0 },
		);
	});
});
