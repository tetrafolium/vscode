...
function runAssert(
	...
	return (prefix: string) => {
		...
		const matcher = new IgnoreFile(
			ignoreFile,
			prefix + ignoreFileLocation,
			undefined,
			ignoreCase,
		);
		if (traverse) {
...
suite("Parsing .gitignore files", () => {
	...
	test("child negation overrides parent ignore", () => {
		...
		const childNoNegation = new IgnoreFile(
			"node_modules/\n",
			"/",
			parentIgnore,
		);
		...
		assert(
			childNoNegation.isArbitraryPathIgnored(
				"/.myconfig/settings/test.md",
				false,
			),
