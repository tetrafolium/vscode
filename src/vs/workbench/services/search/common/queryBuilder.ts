...
export class QueryBuilder {
	...
	text(
		...
		return {
			...
			userDisabledExcludesAndIgnoreFiles:
				options.disregardExcludeSettings && options.disregardIgnoreFiles,
		};
	}
	...
	private commonQuery(
		...
		let excludePatterns: string | string[] | undefined = Array.isArray(
			...
			: options.excludePattern;
		excludePatterns =
			excludePatterns?.length === 1 ? excludePatterns[0] : excludePatterns;
		const includeSearchPathsInfo: ISearchPathsInfo = this.handleIncludeExclude(
			options.includePattern,
			options.expandPatterns,
		);
		const excludeSearchPathsInfo: ISearchPathsInfo = this.handleIncludeExclude(
			excludePatterns,
			options.expandPatterns,
		);
		...
		if (options.changedFileUris !== undefined) {
			const changedFilesInQuery = options.changedFileUris.filter((uri) =>
				pathIncludedInQuery(queryProps, uri.fsPath),
			);
			const changedFilesQueryProps =
				this.commonQueryFromFileList(changedFilesInQuery);
			this.logService.trace(
	...
	private commonQueryFromFileList(files: URI[]): ICommonQueryProps<URI> {
		...
		files.forEach((file) => {
			...
			if (providerExists) {
				const searchRoot =
					this.workspaceContextService.getWorkspaceFolder(file)?.uri ??
				...
				assertReturnsDefined(folderQuery.includePattern)[
					escapeGlobPattern(relPath.replace(/\\/g, "/"))
				] = true;
			} else {
				if (file.fsPath) {
					hasIncludedFile = true;
	...
	private expandSearchPathPatterns(
		...
		const expandedSearchPaths = searchPaths.flatMap((searchPath) => {
			...
			return oneExpanded.flatMap((oneExpandedResult) =>
				this.resolveOneSearchPathPattern(oneExpandedResult, globPortion),
			);
		});
	...
	private getFolderQueryForRoot(
		...
		let excludeFolderRoots = options.excludePattern?.map((excludePattern) => {
			const excludeRoot =
				options.excludePattern && isISearchPatternBuilder(excludePattern)
					? excludePattern.uri
					: undefined;
			...
			if (contextService.getWorkbenchState() === WorkbenchState.FOLDER) {
				...
				if (owningFolder) {
					...
					const isUniqueFolder =
						workspace.folders.filter((folder) => folder.name === owningRootName)
							.length === 1;
					if (isUniqueFolder) {
