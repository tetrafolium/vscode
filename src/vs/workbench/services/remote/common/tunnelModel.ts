...
export class PortsAttributes extends Disposable {
	...
	private hasStartEnd(
		...
		return (
			(value as Partial<PortRange>).start !== undefined &&
			(value as Partial<PortRange>).end !== undefined
		);
	}
	...
	private hasHostAndPort(
		...
		return (
			(value as Partial<HostAndPort>).host !== undefined &&
			(value as Partial<HostAndPort>).port !== undefined &&
			isString((value as Partial<HostAndPort>).host) &&
			isNumber((value as Partial<HostAndPort>).port)
		);
	}
	...
	private readSetting(): PortAttributes[] {
		...
		for (const attributesKey in settingValue) {
			...
			const setting = (settingValue as Record<string, PortAttributes>)[
				attributesKey
			];
		...
		const defaults = this.configurationService.getValue(
			PortsAttributes.DEFAULTS,
		) as Partial<Attributes> | undefined;
		if (defaults) {
...
export class TunnelModel extends Disposable {
			...
			for (const tunnel of tunnels) {
				if (tunnel.localAddress) {
					...
					this.forwarded.set(key, {
						...
						protocol:
							attributes?.get(tunnel.tunnelRemotePort)?.protocol ??
							TunnelProtocol.Http,
	...
	async restoreForwarded() {
		...
		if (this.configurationService.getValue("remote.restoreForwardedPorts")) {
			...
			if (
				...
				const tunnels =
					<RestorableTunnel[] | undefined>JSON.parse(tunnelRestoreValue) ?? [];
	...
	addEnvironmentTunnels(tunnels: TunnelDescription[] | undefined): void {
		if (tunnels) {
			for (const tunnel of tunnels) {
				...
				this.detected.set(
					makeAddress(tunnel.remoteAddress.host, tunnel.remoteAddress.port),
					{
	...
	private async updateAttributes() {
		...
		for (const forwarded of tunnels) {
			...
			if (
				(attributes?.protocol || forwarded.protocol !== TunnelProtocol.Http) &&
