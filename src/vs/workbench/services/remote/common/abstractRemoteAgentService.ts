			...
			this._environment = this._withChannel(async (channel, connection) => {
				...
				if (typeof env.reconnectionGraceTime === "number") {
					this._logService.info(
						`[reconnection-grace-time] Client received grace time from server: ${env.reconnectionGraceTime}ms (${Math.floor(env.reconnectionGraceTime / 1000)}s)`,
					);
					connection.updateGraceTime(env.reconnectionGraceTime);
				} else {
					this._logService.info(
						`[reconnection-grace-time] Server did not provide grace time, using default`,
					);
				}
				return env;
			}, null);
	...
	implements IRemoteAgentConnection
{
	...
	getChannel<T extends IChannel>(channelName: string): T {
		return <T>(
			getDelayedChannel(
				this._getOrCreateConnection().then((c) => c.getChannel(channelName)),
			)
		);
	}
