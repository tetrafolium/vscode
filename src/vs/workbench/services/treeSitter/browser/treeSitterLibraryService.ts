	...
	implements ITreeSitterLibraryService
{
	...
	private readonly _treeSitterImport = new Lazy(async () => {
		const TreeSitter = await importAMDNodeModule<
	...
	private readonly _languagesCache = new CachedFunction(
		(languageId: string) => {
	...
	getLanguage(
		...
		const lang = this._languagesCache
			.get(languageId)
			.resolvedValue.read(reader);
		return lang;
	}
	...
	getInjectionQueries(
		...
		const query = this._injectionQueries
			.get({ languageId, kind: "injections" })
			.read(reader);
		return query;
	}
	...
	getHighlightingQueries(
		...
		const query = this._injectionQueries
			.get({ languageId, kind: "highlights" })
			.read(reader);
		return query;
	}
...
async function tryReadFile(
	...
	uri: URI,
): Promise<IFileContent | undefined> {
	try {
