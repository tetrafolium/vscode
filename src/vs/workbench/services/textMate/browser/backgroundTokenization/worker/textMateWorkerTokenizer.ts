...
export class TextMateWorkerTokenizer extends MirrorTextModel {
		...
		while (true) {
			...
			while (true) {
				...
				fontTokensUpdate.push(
					...this._getFontTokensUpdate(lineToTokenize.lineNumber, r),
				);
			...
			const fontUpdate =
				AnnotationsUpdate.create<IFontTokenOption>(fontTokensUpdate);
			const serializedFontUpdate = fontUpdate.serialize<IFontTokenOption>(
				serializeFontTokenOptions(),
			);
	...
	private _getFontTokensUpdate(
		...
		const offsetAtLineEnd =
			offsetAtNextLineStart > 0 ? offsetAtNextLineStart - 1 : 0;
		fontTokens.push({
	...
	private _getOffsetAtLineStart(lineNumber: number): number {
		...
		return lineNumber - 1 > 0
			? this._lineStarts!.getPrefixSum(lineNumber - 2)
			: 0;
	}
}
