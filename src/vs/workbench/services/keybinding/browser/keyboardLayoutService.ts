...
export class BrowserKeyboardMapperFactoryBase extends Disposable {
	...
	private readonly _onDidChangeKeyboardMapper = this._register(
		new Emitter<void>(),
	);
	...
	private keyboardLayoutMapAllowed: boolean =
		(navigator as INavigatorWithKeyboard).keyboard !== undefined;
	...
	private async _getBrowserKeyMapping(
		...
		if (this.keyboardLayoutMapAllowed) {
			try {
				return await (navigator as INavigatorWithKeyboard).keyboard
					.getLayoutMap()
					.then((e: any) => {
	...
	implements IKeyboardLayoutService
{
	...
	constructor(
		...
		super();
		const keyboardConfig = configurationService.getValue<{ layout: string }>(
			"keyboard",
		);
	...
	setUserKeyboardLayoutIfMatched() {
		...
		if (layout && this._userKeyboardLayout.keyboardLayout) {
			if (
				getKeyboardLayoutId(this._userKeyboardLayout.keyboardLayout.layout) ===
					layout &&
