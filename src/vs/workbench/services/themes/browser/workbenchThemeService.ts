	...
	implements IWorkbenchThemeService
{
	...
	constructor(
		...
		this.colorThemeRegistry = this._register(
			new ThemeRegistry(colorThemesExtPoint, ColorThemeData.fromExtensionTheme),
		);
		...
		this.onColorThemeChange = this._register(
		...
		this.onFileIconThemeChange = this._register(
			new Emitter<IWorkbenchFileIconTheme>({
				leakWarningThreshold: 400,
		...
		this.onProductIconThemeChange = this._register(
		...
		let themeData: ColorThemeData | undefined = ColorThemeData.fromStorageData(
			this.storageService,
		);
		...
		if (!themeData) {
			const colorScheme =
				this.settings.getPreferredColorScheme() ??
		...
		extensionService.whenInstalledExtensionsRegistered().then((_) => {
			...
			this.initialize(previousColorThemeSetting).catch(
	...
	private async initialize(
		themePreviousSettingsId: string | undefined,
	): Promise<
		[
	...
	> {
		...
		const initializeFileIconTheme = async () => {
			const devThemes =
				this.fileIconThemeRegistry.findThemeByExtensionLocation(extDevLoc);
			if (devThemes.length) {
		...
		const initializeProductIconTheme = async () => {
			const devThemes =
				this.productIconThemeRegistry.findThemeByExtensionLocation(extDevLoc);
			if (devThemes.length) {
	...
	private static readonly NEW_THEME_NOTIFICATION_KEY =
		"workbench.newDefaultThemeNotification";

	private async showNewDefaultThemeNotification(
		previousSettingsId: string | undefined,
	): Promise<void> {
		if (
		...
		if (!keepTheme) {
			const previousTheme =
				this.colorThemeRegistry.findThemeBySettingsId(previousSettingsId);
			if (previousTheme) {
		...
		this._register(
			this.configurationService.onDidChangeConfiguration((e) => {
				...
				if (this.currentColorTheme) {
					...
					if (
						e.affectsConfiguration(ThemeSettings.TOKEN_COLOR_CUSTOMIZATIONS)
					) {
