...
class CSSFileWatcher implements IDisposable {
	private readonly watchedLocations = new Map<
		string,
		{ readonly uri: URI; readonly disposables: DisposableStore }
	>();
...
export class CSSExtensionPoint {
	...
	private readonly pendingExtensions = new Map<
		string,
		IExtensionPointUser<ICSSExtensionPoint[]>
	>();
	...
	constructor(
		...
		this.watcher = this.disposables.add(
			new CSSFileWatcher(fileService, environmentService, (uri) =>
				this.reloadStylesheet(uri),
			),
		);
			...
			for (const extension of delta.added) {
				if (!isProposedApiEnabled(extension.description, "css")) {
					extension.collector.error(
						`The '${cssExtensionPoint.name}' contribution point is proposed API.`,
					);
					continue;
				}
	...
	private onThemeChange(): void {
		...
		for (const [extensionId, extension] of this.pendingExtensions) {
			if (
				!this.stylesheetsByExtension.has(extensionId) &&
	...
	private activateExtensionCSS(
		extension: IExtensionPointUser<ICSSExtensionPoint[]>,
	): void {
		const extensionId = extension.description.identifier.value;
