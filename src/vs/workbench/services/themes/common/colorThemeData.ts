...
export class ColorThemeData implements IWorkbenchColorTheme {
	...
	get tokenColors(): ITextMateThemingRule[] {
		if (!this.textMateThemingRules) {
			...
			const foreground =
				this.getColor(editorForeground) || this.getDefault(editorForeground)!;
			const background =
				this.getColor(editorBackground) || this.getDefault(editorBackground)!;
			result.push({
				settings: {
	...
	public getTokenFontIndex(): TokenFontIndex {
		if (!this.tokenFontIndex) {
			...
			this.tokenColors.forEach((r) =>
				index.add(
					r.settings.fontFamily,
					r.settings.fontSize,
					r.settings.lineHeight,
				),
			);
...
async function _loadColorTheme(
	...
	if (resources.extname(themeLocation) === ".json") {
		const content =
			await extensionResourceLoaderService.readExtensionResource(themeLocation);
...
export function findMetadata(
	...
	const fontStyle =
		definitions.foreground?.settings.fontStyle ||
		definitions.bold?.settings.fontStyle;
