...
suite("ExtensionManifestPropertiesService - SessionsWindowSupport", () => {
	...
	function getExtensionManifest(
		properties: Partial<IExtensionManifest> = {},
	): IExtensionManifest {
	...
	test("defaults to declarative extensions without executable code and supported contributions", () => {
		...
		assert.deepStrictEqual(
			[
				testObject.canExecuteOnSessionsWindow(
					getExtensionManifest({ contributes: { themes: [] } }),
				),
				testObject.canExecuteOnSessionsWindow(
				...
				testObject.canExecuteOnSessionsWindow(
					getExtensionManifest({ contributes: { commands: [] } }),
				),
			],
			[true, false, false],
		);
	...
	test("uses configured sessions window support override", async () => {
		...
		assert.deepStrictEqual(
			[
				...
				testObject.canExecuteOnSessionsWindow(
					getExtensionManifest({ name: "b", contributes: { themes: [] } }),
				),
			],
			[true, false],
		);
