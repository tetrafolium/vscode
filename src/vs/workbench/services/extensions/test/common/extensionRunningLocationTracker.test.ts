...
suite("ExtensionRunningLocationTracker - extensionAffinity", () => {
	...
	function createTracker(
		...
		const registry: IReadOnlyExtensionDescriptionRegistry = {
			...
			getExtensionDescription: (id: string | ExtensionIdentifier) =>
				extensions.find(
					(e) =>
						e.identifier.value === (typeof id === "string" ? id : id.value),
				),
	...
	test("extensions with extensionAffinity should have the same affinity", () => {
		...
		const extB = createExtension("publisher.extB", undefined, [
			"publisher.extA",
		]);

		const tracker = createTracker([extA, extB]);
		const runningLocations = tracker.computeRunningLocation(
			[extA, extB],
			[],
			true,
		);
		...
		assert.strictEqual(
			locA!.affinity,
			locB!.affinity,
			"Extensions with extensionAffinity should have the same affinity",
		);
	});
	...
	test("transitive extensionAffinity should group all extensions together", () => {
		...
		const extC = createExtension("publisher.extC", undefined, [
			"publisher.extB",
		]);
		...
		assert.ok(
			locA && locB && locC,
			"All extensions should have running locations",
		);
		assert.strictEqual(
			locA!.affinity,
			locB!.affinity,
			"A and B should have the same affinity",
		);
		assert.strictEqual(
			locB!.affinity,
			locC!.affinity,
			"B and C should have the same affinity",
		);
	});

	test("extensionAffinity with non-installed extension should be ignored", () => {
		const extA = createExtension("publisher.extA", undefined, [
			"publisher.notInstalled",
		]);
	...
	test("extensionAffinity combined with extensionDependencies", () => {
		...
		assert.strictEqual(
			locA!.affinity,
			locB!.affinity,
			"A and B (dependency) should have the same affinity",
		);
		assert.strictEqual(
			locA!.affinity,
			locC!.affinity,
			"A and C (extensionAffinity) should have the same affinity",
		);
	});
	...
	test("one-way extensionAffinity is sufficient", () => {
		...
		assert.strictEqual(
			locA!.affinity,
			locB!.affinity,
			"One-way extensionAffinity should be sufficient to group extensions",
		);
	});
});
