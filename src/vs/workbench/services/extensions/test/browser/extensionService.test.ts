...
suite("ExtensionService", () => {
	class MyTestExtensionService extends AbstractExtensionService {
		...
		public readonly activationEvents: {
			event: string;
		...
		protected override _doCreateExtensionHostManager(
			...
			return new (class extends mock<IExtensionHostManager>() {
				...
				override get isReady() {
					return extHostKind === ExtensionHostKind.Remote
						? extService.remoteExtHostIsReady
						: extService.localExtHostIsReady;
				}
				override disconnect() {
				...
				override representsRunningLocation(
					runningLocation: ExtensionRunningLocation,
				): boolean {
	...
	setup(() => {
		...
		extService = <MyTestExtensionService>(
	...
	test("Immediate activation includes ready remote extension hosts (issue #297019)", async () => {
		...
		assert.ok(
			activatedKinds.includes(ExtensionHostKind.LocalProcess),
			"Should activate on LocalProcess",
		);
		assert.ok(
			activatedKinds.includes(ExtensionHostKind.LocalWebWorker),
			"Should activate on LocalWebWorker",
		);
		assert.ok(
			activatedKinds.includes(ExtensionHostKind.Remote),
			"Should activate on ready Remote host",
		);
	});
	...
	test("Immediate activation excludes not-ready remote extension hosts", async () => {
		...
		assert.ok(
			!activatedKinds.includes(ExtensionHostKind.Remote),
			"Should NOT activate on not-ready Remote host",
		);
	});
	...
	test("Immediate activation still activates local extension hosts even when not ready", async () => {
		...
		assert.ok(
			activatedKinds.includes(ExtensionHostKind.LocalProcess),
			"Should activate on LocalProcess even when not ready",
		);
		assert.ok(
			activatedKinds.includes(ExtensionHostKind.LocalWebWorker),
			"Should activate on LocalWebWorker even when not ready",
		);
	});
