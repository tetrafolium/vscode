					...
					this._notificationService.prompt(
						...
						invalidExtensions.length === 1
							? localize(
									...
									invalidExtensions[0].location.fsPath,
									invalidExtensions[0].validations[0][1],
								)
							: localize(
									"extensionsUnderDevelopment.invalid",
									"Failed loading extensions {0} under development because they are invalid: {1}",
									invalidExtensions
										.map((ext) => `'${ext.location.fsPath}'`)
										.join(", "),
									invalidExtensions
										.map((ext) => `${ext.validations[0][1]}`)
										.join(", "),
								),
						[],
					);
