	...
	implements IExtensionHost
{
	...
	private readonly _onExit: Emitter<[number, string]> = this._register(
		new Emitter<[number, string]>(),
	);
	...
	constructor(
		...
		private readonly _defaultLogLevelsService: IDefaultLogLevelsService,
	) {
		super();
	...
	public async disconnect(): Promise<void> {
		...
		if (
			...
			!this._mainProcessHandlesExtHostShutdown
		) {
			this._extensionHostProcess
				.waitForExit(extensionHostGraceTimeMs)
				.catch(() => {
					/* best-effort */
				});
		}
	...
	private async _start(): Promise<IMessagePassingProtocol> {
		...
		const onStdout = this._register(
			this._handleProcessOutputStream(this._extensionHostProcess.onStdout),
		);
		const onStderr = this._register(
			this._handleProcessOutputStream(this._extensionHostProcess.onStderr),
		);
			...
			onDebouncedOutput((output) => {
				...
				if (inspectorUrlMatch) {
					...
					if (
						...
						console.debug(
							`%c[Extension Host] %cdebugger inspector at ${devtoolsUrl}`,
							"color: blue",
							"color:",
						);
					}
	...
	private async _createExtHostInitData(): Promise<IExtensionHostInitData> {
		...
		return {
			...
			environment: {
				...
				appHost:
					(this._environmentService.isSessionsWindow
						? this._productService.agentsTelemetryAppName
						: undefined) ||
				...
				extensionLogLevel:
					this._defaultLogLevelsService.defaultLogLevels.extensions,
			...
			telemetryInfo: {
				...
				devDeviceId:
					this._telemetryService.devDeviceId ??
					this._telemetryService.machineId,
				firstSessionDate: this._telemetryService.firstSessionDate,
				msftInternal: this._telemetryService.msftInternal,
			},
			remoteExtensionTips: this._productService.remoteExtensionTips,
			virtualWorkspaceExtensionTips:
				this._productService.virtualWorkspaceExtensionTips,
