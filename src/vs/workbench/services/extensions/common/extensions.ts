	...
	private _readMyActivationEvents(): Set<string> {
		...
		for (const extensionDescription of this._allExtensions) {
			...
			const activationEvents =
				ImplicitActivationEvents.readActivationEvents(extensionDescription);
...
export interface IResponsiveStateChangeEvent {
	...
	): Promise<IExtensionInspectInfo | undefined>;
}
...
export function toExtension(
	...
	return {
		type: extensionDescription.isBuiltin
			...
			: ExtensionType.User,
		isBuiltin:
			extensionDescription.isBuiltin || extensionDescription.isUserBuiltin,
		identifier: {
