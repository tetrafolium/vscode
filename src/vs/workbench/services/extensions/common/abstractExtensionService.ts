	...
	implements IExtensionService
{
	...
	constructor(
		...
		this._register(
			this._extensionManagementService.onDidEnableExtensions((extensions) => {
		...
		this._register(
			this._extensionManagementService.onDidInstallExtensions((result) => {
				...
				if (extensions.length) {
					...
					this._handleDeltaExtensions(
						new DeltaExtensionsQueueItem(extensions, toRemove),
					);
				}
			}),
		);
		...
		this._register(
			this._extensionManagementService.onDidUninstallExtension((event) => {
				if (!event.error) {
					...
					this._handleDeltaExtensions(
						new DeltaExtensionsQueueItem([], [event.identifier.id]),
					);
				}
			}),
		);
	...
	private async _updateExtensionsOnExtHost(
		...
		const myToRemove = filterExtensionIdentifiers(
			...
			removedRunningLocation,
			(extRunningLocation) =>
				extensionHostManager.representsRunningLocation(extRunningLocation),
		);
	...
	private async _activateAddedExtensionIfNeeded(
		...
		const activationEvents =
			this._activationEventReader.readActivationEvents(extensionDescription);
	...
	private async _activateDeferredRemoteEvents(): Promise<void> {
		...
		for (const activationEvent of this._pendingRemoteActivationEvents) {
			const result = Promise.all(
				remoteExtensionHosts.map((extHostManager) =>
					extHostManager.activateByEvent(
						activationEvent,
	...
	private async _resolveAndProcessExtensions(
		lock: ExtensionDescriptionRegistryLock,
	): Promise<void> {
		...
		const remoteExtensionsThatNeedToRunLocally = this
		...
		if (resolverExtensions.length) {
			...
			toAdd = allExtensions.filter(
				(extension) =>
					!resolverExtensions.some(
						(e) =>
							ExtensionIdentifier.equals(e.identifier, extension.identifier) &&
							e.extensionLocation.toString() ===
					...
					(registered) =>
						!allExtensions.some(
							(e) =>
								...
								e.extensionLocation.toString() ===
									registered.extensionLocation.toString(),
						),
				);
				if (toRemove.length) {
	...
	protected async _resolveAuthorityOnExtensionHosts(
		...
		for (const result of results) {
			...
			const bestErrorIsUnknown =
				bestErrorResult.error.code === RemoteAuthorityResolverErrorCode.Unknown;
	...
	public async stopExtensionHosts(
		reason: string,
		auto?: boolean,
	): Promise<boolean> {
	...
	private async _onRemoteExtensionHostCrashed(
		...
		try {
			const info =
				await this._getExtensionHostExitInfoWithTimeout(reconnectionToken);
			if (info) {
				this._logService.error(
	...
	private _activateByEvent(
		...
		if (activationKind === ActivationKind.Immediate) {
			...
			managers = this._extensionHostManagers.filter(
				(extHostManager) =>
					extHostManager.kind === ExtensionHostKind.LocalProcess ||
	...
	private _doHandleExtensionPoints(
		...
		onlyResolverExtensionPoints: boolean,
	): void {
		...
		for (const extensionDescription of affectedExtensions) {
			if (extensionDescription.contributes) {
				for (const extPointName in extensionDescription.contributes) {
					if (
						hasOwnProperty.call(extensionDescription.contributes, extPointName)
					) {
		...
		for (const extensionPoint of extensionPoints) {
			if (
				affectedExtensionPoints[extensionPoint.name] &&
...
export class ResolverExtensions {
	constructor(public readonly extensions: IExtensionDescription[]) {}
}
...
export function isResolverExtension(extension: IExtensionDescription): boolean {
	return !!extension.activationEvents?.some((activationEvent) =>
		activationEvent.startsWith("onResolveRemoteAuthority:"),
	);
}
