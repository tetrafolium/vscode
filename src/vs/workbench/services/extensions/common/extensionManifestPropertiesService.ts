...
export const EXTENSIONS_SUPPORT_AGENTS_WINDOW =
	"extensions.supportAgentsWindow";

const SESSIONS_WINDOW_ALLOWED_CONTRIBUTION_POINTS: ReadonlySet<
	keyof IExtensionContributions
> = new Set([
	"themes",
	"iconThemes",
	"productIconThemes",
	...
	implements IExtensionManifestPropertiesService
{
	...
	private _configuredVirtualWorkspaceSupportMap: ExtensionIdentifierMap<boolean> | null =
		null;
	private _configuredSessionsWindowSupportMap: ExtensionIdentifierMap<boolean> | null =
		null;
	...
	canExecuteOnSessionsWindow(manifest: IExtensionManifest): boolean {
		const configuredSessionsWindowSupport =
			this.getConfiguredSessionsWindowSupport(manifest);
		...
		const contributionPoints = Object.keys(manifest.contributes || {}) as Array<
			keyof IExtensionContributions
		>;
		return contributionPoints.every((point) =>
			SESSIONS_WINDOW_ALLOWED_CONTRIBUTION_POINTS.has(point),
		);
	}
	...
	private getConfiguredSessionsWindowSupport(
		...
		if (this._configuredSessionsWindowSupportMap === null) {
			const configuredSessionsWindowSupportMap =
				new ExtensionIdentifierMap<boolean>();
			const configuredSessionsWindowSupport =
				this.configurationService.getValue<{ [key: string]: boolean }>(
			...
			for (const id of Object.keys(configuredSessionsWindowSupport)) {
				if (configuredSessionsWindowSupport[id] !== undefined) {
					configuredSessionsWindowSupportMap.set(
						id,
						configuredSessionsWindowSupport[id],
					);
				}
			}
			this._configuredSessionsWindowSupportMap =
				configuredSessionsWindowSupportMap;
		}
