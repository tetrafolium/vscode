	...
	implements IExtensionHost
{
	...
	private async _getWebWorkerExtensionHostIframeSrc(): Promise<string> {
		...
		const relativeExtensionHostIframeSrc = this._webWorkerService.getWorkerUrl(
			new WebWorkerDescriptor({
				esmModuleLocation: FileAccess.asBrowserUri(iframeModulePath),
				esmModuleLocationBundler: new URL(
					...
					iframe.contentWindow!.postMessage(
						{
							...
							data: {
								workerUrl: this._webWorkerService.getWorkerUrl(
									extensionHostWorkerMainDescriptor,
								),
	...
	private async _createExtHostInitData(): Promise<IExtensionHostInitData> {
		...
		return {
			...
			autoStart:
				...
				this.startup === ExtensionHostStartup.LazyAutoStart,
			remote: {
...
const extensionHostWorkerMainDescriptor = new WebWorkerDescriptor({
	...
	esmModuleLocationBundler: () =>
		new URL(
			"../../../api/worker/extensionHostWorkerMain.ts?esm",
			import.meta.url,
		),
});
