...
export class NodeTestWorkingCopyBackupService extends NativeWorkingCopyBackupService {
	...
	override async discardBackups(filter?: {
...
suite("WorkingCopyBackupService", () => {
	...
	suite("backup", () => {
		...
		test("joining", async () => {
			...
			assert.strictEqual(
				(await fileService.resolve(joinPath(workspaceBackupPath, "file")))
					.children?.length,
				1,
			);
		...
		test("untitled file", async () => {
			...
			assert.strictEqual(
				(await fileService.resolve(joinPath(workspaceBackupPath, "untitled")))
					.children?.length,
				1,
			);
			assert.strictEqual(await fileService.exists(backupPath), true);
		...
		async function testLargeTextFile(
			largeString: string,
			buffer: VSBufferReadable | VSBufferReadableStream,
		) {
			const identifier = toUntypedWorkingCopyId(fooFile);
			const backupPath = joinPath(
				workspaceBackupPath,
			...
			assert.strictEqual(
				(await fileService.readFile(backupPath)).value.toString(),
				toExpectedPreamble(identifier, largeString, { largeTest: true }),
			);
			assert.ok(service.hasBackupSync(identifier));
		}
	...
	suite("discardBackups (all)", () => {
		...
		test("untitled file", async () => {
			...
			assert.strictEqual(await fileService.exists(backupPath), false);
			assert.strictEqual(
				await fileService.exists(joinPath(workspaceBackupPath, "untitled")),
				false,
			);
		});
	...
	suite("resolve", () => {
		...
		async function testResolveBackup(
			resource: URI,
			contents: string,
			meta?: IBackupTestMetaData,
			expectNoMeta?: boolean,
		) {
		...
		async function doTestResolveBackup(
			identifier: IWorkingCopyIdentifier,
			contents: string,
			meta?: IBackupTestMetaData,
			expectNoMeta?: boolean,
		...
		async function testShouldRestoreOriginalContentsWithBrokenBackup(
			identifier: IWorkingCopyIdentifier,
		): Promise<void> {
			const contents = [
				"Lorem ipsum ",
			...
			const fileContents = (
				await fileService.readFile(backupPath)
			).value.toString();
			assert.strictEqual(
			...
			const newFileContents =
				fileContents.substring(0, metaIndex) +
				"{{" +
				fileContents.substr(metaIndex);
		...
		async function testShouldUpdateMetaFromFileWhenResolving(
			identifier: IWorkingCopyIdentifier,
		): Promise<void> {
			...
			assert.strictEqual(
				service.hasBackupSync(identifier, undefined, meta),
				false,
			);
			assert.strictEqual(
				service.hasBackupSync(identifier, undefined, updatedMeta),
				true,
			);
		...
		test("should ignore invalid backups (empty file)", async () => {
			...
			backup = await service.resolve<IBackupTestMetaData>(
				toUntypedWorkingCopyId(fooFile),
			);
			assert.ok(!backup);
		});
		...
		test("file with binary data", async () => {
			...
			const buffer = Uint8Array.from([
				...
				73, 0, 0, 0, 67, 8, 2, 0, 0, 0, 95, 138, 191, 237, 0, 0, 0, 1, 115, 82,
				71, 66, 0, 174, 206, 28, 233, 0, 0, 0, 4, 103, 65, 77, 65, 0, 0, 177,
				143, 11, 252, 97, 5, 0, 0, 0, 9, 112, 72, 89, 115, 0, 0, 14, 195, 0, 0,
				14, 195, 1, 199, 111, 168, 100, 0, 0, 0, 71, 116, 69, 88, 116, 83, 111,
				117, 114, 99, 101, 0, 83, 104, 111, 116, 116, 121, 32, 118, 50, 46, 48,
				46, 50, 46, 50, 49, 54, 32, 40, 67, 41, 32, 84, 104, 111, 109, 97, 115,
				32, 66, 97, 117, 109, 97, 110, 110, 32, 45, 32, 104, 116, 116, 112, 58,
				47, 47, 115, 104, 111, 116, 116, 121, 46, 100, 101, 118, 115, 45, 111,
				110, 46, 110, 101, 116, 44, 132, 21, 213, 0, 0, 0, 84, 73, 68, 65, 84,
				120, 218, 237, 207, 65, 17, 0, 0, 12, 2, 32, 211, 217, 63, 146, 37, 246,
				218, 65, 3, 210, 191, 226, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 230, 230, 230, 118, 100, 169, 4, 173, 8, 44, 248, 184, 40, 0, 0, 0,
			...
			const backupBuffer = await consumeStream(backup.value, (chunks) =>
	...
	suite("WorkingCopyBackupsModel", () => {
		test("simple", async () => {
			...
			assert.strictEqual(
				model.has(resource4, undefined, { bar: "foo" }),
				false,
			);

			model.update(resource4, { foo: "nothing" });
			assert.strictEqual(
				model.has(resource4, undefined, { foo: "nothing" }),
				true,
			);
			assert.strictEqual(
				model.has(resource4, undefined, { foo: "bar" }),
				false,
			);
	...
	suite("typeId migration", () => {
		test("works (when meta is missing)", async () => {
			...
			await fileService.writeFile(
				untitledBackupPath,
				VSBuffer.fromString(`${untitledFile.toString()}\ntest untitled`),
			);
			await fileService.writeFile(
			...
			assert.ok(
				backups.some((backup) => isEqual(backup.resource, untitledFile)),
			);
