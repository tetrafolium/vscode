...
suite("WorkingCopyFileService", () => {
	...
	test("move - source identical to target", async function () {
		const sourceModel: TextFileEditorModel =
		...
		const eventCounter = await testEventsMoveOrCopy(
			[
				{
					file: { source: sourceModel.resource, target: sourceModel.resource },
					overwrite: true,
				},
			],
			true,
	...
	test("move - one source == target and another source != target", async function () {
		const sourceModel1: TextFileEditorModel =
			instantiationService.createInstance(
				TextFileEditorModel,
				toResource.call(this, "/path/file1.txt"),
				"utf8",
	...
	test("copy - one source == target and another source != target", async function () {
		...
		const eventCounter = await testEventsMoveOrCopy([
			{
				file: { source: sourceModel1.resource, target: sourceModel1.resource },
				overwrite: true,
			},
			{
				file: { source: sourceModel2.resource, target: targetModel2.resource },
				overwrite: true,
			},
		]);
