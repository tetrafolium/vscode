...
suite("UntitledFileWorkingCopyManager", () => {
	...
	let manager: IFileWorkingCopyManager<
		TestStoredFileWorkingCopyModel,
		TestUntitledFileWorkingCopyModel
	>;
	...
	test("basics", async () => {
		...
		assert.strictEqual(
			manager.untitled.get(
				URI.file("/some/invalidPath").with({ scheme: Schemas.untitled }),
			),
			undefined,
		);
	...
	test("save - without associated resource", async () => {
		...
		assert.strictEqual(
			savedEvent!.source.toString(),
			workingCopy.resource.toString(),
		);
		assert.strictEqual(
			savedEvent!.target.toString(),
			URI.file("simple/file.txt").toString(),
		);
	...
	test("save - with associated resource", async () => {
		...
		assert.strictEqual(
			savedEvent!.target.toString(),
			URI.file("/some/associated.txt").toString(),
		);
