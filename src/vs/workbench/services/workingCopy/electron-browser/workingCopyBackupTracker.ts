	...
	protected async onBeforeShutdownWithModified(
		...
		const workingCopiesToAutoSave = modifiedWorkingCopies.filter(
			(wc) =>
				...
				this.filesConfigurationService.getAutoSaveMode(wc.resource).mode !==
					AutoSaveMode.OFF,
		);
		...
		return this.withProgressAndCancellation(
			...
			workingCopies.some(
				(workingCopy) =>
					...
					workingCopy.capabilities & WorkingCopyCapabilities.Scratchpad,
			)
	...
	private doRevertAllBeforeShutdown(
		...
		return this.withProgressAndCancellation(
			async () => {
				...
				if (
					modifiedWorkingCopies.length === this.workingCopyService.modifiedCount
				) {
