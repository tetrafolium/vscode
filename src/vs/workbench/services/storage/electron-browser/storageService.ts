...
export class NativeWorkbenchStorageService extends RemoteStorageService {
	...
	protected override createApplicationSharedStorage(): IStorage {
		...
		const storageDataBaseClient = this._register(
			new ApplicationSharedStorageDatabaseClient(channel),
		);
		const applicationSharedStorage = this._register(
			new MigratingStorage(storageDataBaseClient),
		);
		this._register(
			applicationSharedStorage.onDidChangeStorage((e) =>
	...
	protected override async doInitialize(): Promise<void> {
		...
		if (applicationSharedStorage instanceof MigratingStorage) {
			...
			if (this.workbenchEnvironmentService.isSessionsWindow) {
				const channel = this.remoteService.getChannel("storage");
				applicationSharedFallbackStorage = this._register(
					new Storage(
						this._register(
							new FallbackApplicationStorageDatabaseClient(channel),
						),
					),
				);
				await applicationSharedFallbackStorage.init();
			} else {
				applicationSharedFallbackStorage = this.getStorage(
					StorageScope.APPLICATION,
				);
			}
			if (applicationSharedFallbackStorage) {
				applicationSharedStorage.setFallbackStorage(
					applicationSharedFallbackStorage,
					this.workbenchEnvironmentService.isSessionsWindow,
				);
			}
		}
	}
