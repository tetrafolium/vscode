...
export class BrowserStorageService extends AbstractStorageService {
	...
	private async createApplicationStorage(): Promise<void> {
		const applicationStorageIndexedDB =
			await IndexedDBStorageDatabase.createApplicationStorage(this.logService);
	...
	private async createApplicationSharedStorage(): Promise<void> {
		const applicationSharedStorageIndexedDB =
			await IndexedDBStorageDatabase.createApplicationSharedStorage(
				this.logService,
			);

		this.applicationSharedStorageDatabase = this._register(
			applicationSharedStorageIndexedDB,
		);
		this.applicationSharedStorage = this._register(
			new Storage(this.applicationSharedStorageDatabase),
		);

		this._register(
			this.applicationSharedStorage.onDidChangeStorage((e) =>
				this.emitDidChangeValue(StorageScope.APPLICATION_SHARED, e),
			),
		);
	...
	implements IIndexedDBStorageDatabase
{
	...
	static async createApplicationSharedStorage(
		logService: ILogService,
	): Promise<IIndexedDBStorageDatabase> {
		return IndexedDBStorageDatabase.create(
			{ id: "global-shared", broadcastChanges: true },
			logService,
		);
	}
	...
	private constructor(
		...
		this.broadcastChannel = options.broadcastChanges
			? this._register(
					new BroadcastDataChannel<IStorageItemsChangeEvent>(this.name),
				)
			: undefined;
