...
export interface IUntitledTextEditorModel
	extends ITextEditorModel, ILanguageSupport, IEncodingSupport, IWorkingCopy {
	/**
	...
	implements IUntitledTextEditorModel
{
	...
	private registerListeners(): void {
		...
		this._register(
			this.textResourceConfigurationService.onDidChangeConfiguration((e) =>
				this.onConfigurationChange(e, true),
			),
		);
	}
	...
	protected override installModelListeners(model: ITextModel): void {
		this._register(
			model.onDidChangeContent((e) => this.onModelContentChanged(model, e)),
		);
		this._register(
	...
	private updateNameFromFirstLine(textEditorModel: ITextModel): void {
		...
		let firstLineText = textEditorModel
			.getValueInRange({
				...
				startColumn: 1,
				endColumn:
					UntitledTextEditorModel.FIRST_LINE_NAME_CANDIDATE_MAX_LENGTH + 1, // first cap at FIRST_LINE_NAME_CANDIDATE_MAX_LENGTH
			})
			.trim()
