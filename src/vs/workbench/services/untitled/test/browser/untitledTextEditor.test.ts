...
suite("UntitledTextEditorWorkingCopyEditorHandler", () => {
	...
	test("handles only untitled working copies with no type id", () => {
		...
		assert.strictEqual(
			handler.handles({ resource: untitledResource, typeId: NO_TYPE_ID }),
			true,
		);
		assert.strictEqual(
			handler.handles({ resource: untitledResource, typeId: "someTypeId" }),
			false,
		);
		assert.strictEqual(
			handler.handles({ resource: URI.file("/test.txt"), typeId: NO_TYPE_ID }),
			false,
		);
	});
	...
	test("isOpen matches UntitledTextEditorInput with same resource", () => {
		...
		const untitledInput = disposables.add(
			instantiationService.createInstance(
				UntitledTextEditorInput,
				instantiationService
					.createInstance(TestServiceAccessor)
					.untitledTextEditorService.create({ untitledResource }),
			),
		);
	...
	test("isOpen matches non-UntitledTextEditorInput editors with same untitled resource", () => {
		...
		const customEditorInput = disposables.add(
			new TestEditorInput(untitledResource, "customEditorType"),
		);
	...
	test("isOpen does not match editors with different resource", () => {
		const untitledResource1 = URI.from({
			scheme: Schemas.untitled,
			path: "Untitled-1",
		});
		const untitledResource2 = URI.from({
			scheme: Schemas.untitled,
			path: "Untitled-2",
		});
		...
		const otherInput = disposables.add(
			new TestEditorInput(untitledResource2, "customEditorType"),
		);
	...
	test("isOpen returns false for non-untitled working copies", () => {
		...
		const editor = disposables.add(
			new TestEditorInput(fileResource, "testType"),
		);
