...
suite("Workbench - TextModelResolverService", () => {
	...
	test("even loading documents should be refcounted", async () => {
		...
		const modelRefPromise1 =
			accessor.textModelResolverService.createModelReference(uri);
		const modelRefPromise2 =
			accessor.textModelResolverService.createModelReference(uri);
	...
	test("resolve inMemory", async () => {
		...
		const ref =
			await accessor.textModelResolverService.createModelReference(resource);
		const model = ref.object;
		assert.ok(model);
		...
		assert(
			!textModel.isDisposed(),
			"the inMemory text model should not be disposed before releasing the reference",
		);
		...
		assert(
			textModel.isDisposed(),
			"the inMemory text model should be disposed after the reference is released",
		);
	});
	...
	test("resolve inMemory disposes when last reference released", async () => {
		...
		assert(
			textModel.isDisposed(),
			"the inMemory text model should be disposed after last reference is released",
		);
	});
	...
	test("resolve inMemory is refcounted", async () => {
		...
		assert(
			!textModel.isDisposed(),
			"should not dispose while ref2 is still alive",
		);
		...
		assert(
			textModel.isDisposed(),
			"should dispose after last reference released",
		);
	});
