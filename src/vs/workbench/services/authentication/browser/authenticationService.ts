	...
	implements IAuthenticationService
{
		...
		if (authProvider) {
			...
			if (isAuthenticationWwwAuthenticateRequest(scopeListOrRequest)) {
				...
				return await authProvider.getSessionsFromChallenges(
					{
						challenges: parseWWWAuthenticateHeader(
							scopeListOrRequest.wwwAuthenticate,
						),
			...
			return await authProvider.getSessions(
				scopeListOrRequest ? [...scopeListOrRequest] : undefined,
				{ ...options },
			);
		} else {
			throw new Error(
	...
	async getOrActivateProviderIdForServer(
		...
		const providers = this._declaredProviders
			...
			.filter(
				(p) =>
					!!p.authorizationServerGlobs?.some((i) =>
						match(i, authServerStr, { ignoreCase: true }),
					),
			);
	...
	async createDynamicAuthenticationProvider(
		...
		const providerId = await delegate.create(
			authorizationServer,
			serverMetadata,
			resource,
			clientId,
		...
		if (provider) {
			this._logService.debug(
				`Created dynamic authentication provider: ${providerId}`,
			);
			this._dynamicAuthenticationProviderIds.add(providerId);
			return provider;
		}
		this._logService.error(
			`Failed to create dynamic authentication provider: ${providerId}`,
		);
		return undefined;
	}
	...
	async createOrGetXaaProvider(issuer: URI): Promise<string | undefined> {
		...
		if (!delegate) {
			this._logService.error(
				"No authentication provider host delegate supports XAA",
			);
			return undefined;
		}
		...
		this._logService.error(
			`Failed to create XAA authentication provider for issuer: ${issuer.toString(true)}`,
		);
		return undefined;
	}
	...
	private matchesProvider(
		...
		if (provider.authorizationServers) {
			...
			for (const server of provider.authorizationServers) {
				...
				if (
					equalsIgnoreCase(str, authServerStr) ||
					match(str, authServerStr, { ignoreCase: true })
				) {
					return true;
				}
			}
