...
export const IAuthenticationMcpAccessService =
	createDecorator<IAuthenticationMcpAccessService>(
	...
	implements IAuthenticationMcpAccessService
{
	...
	private _onDidChangeMcpSessionAccess: Emitter<{
		providerId: string;
		accountName: string;
	}> = this._register(
	...
	isAccessAllowed(
		...
		if (Array.isArray(trustedMCPServerAuthAccess)) {
			...
			trustedMCPServerAuthAccess?.[providerId]?.includes(mcpServerId)
		) {
			return true;
		}
