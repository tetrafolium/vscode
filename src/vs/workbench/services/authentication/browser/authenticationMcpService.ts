...
export const IAuthenticationMcpService =
	createDecorator<IAuthenticationMcpService>("IAuthenticationMcpService");
...
export class AuthenticationMcpService
	extends Disposable
	implements IAuthenticationMcpService
{
	...
	private _onDidAccountPreferenceChange: Emitter<{
		...
		new Emitter<{ providerId: string; mcpServerIds: string[] }>(),
	);
	...
	constructor(
		...
		private readonly _authenticationAccessService: IAuthenticationMcpAccessService,
	) {
		super();
		...
		this._inheritAuthAccountPreferenceChildToParent = Object.entries(
			this._inheritAuthAccountPreferenceParentToChildren,
		).reduce<{ [mcpServerId: string]: string }>((acc, [parent, children]) => {
	...
	private async updateNewSessionRequests(
		...
		Object.keys(existingRequestsForProvider).forEach((requestedScopes) => {
			...
			if (
				addedSessions.some((session) =>
					scopesMatch(session.scopes, requestedScopesArray),
				)
			) {
			...
			Object.keys(providerRequests).forEach((mcpServerId) => {
				removedSessions.forEach((removed) => {
					const indexOfSession = providerRequests[
						mcpServerId
					].possibleSessions.findIndex((session) => session.id === removed.id);
					if (indexOfSession) {
						providerRequests[mcpServerId].possibleSessions.splice(
							indexOfSession,
							1,
						);
					}
	...
	private updateBadgeCount(): void {
		...
		this._signInRequestItems.forEach((providerRequests) => {
			Object.keys(providerRequests).forEach((request) => {
				numberOfRequests +=
					providerRequests[request].requestingMcpServerIds.length;
			});
		});
	...
	updateAccountPreference(
		...
		const parentMcpServerId =
			this._inheritAuthAccountPreferenceChildToParent[mcpServerId] ??
			mcpServerId;
		...
		const childrenMcpServers =
			this._inheritAuthAccountPreferenceParentToChildren[parentMcpServerId];
		const mcpServerIds = childrenMcpServers
			? [parentMcpServerId, ...childrenMcpServers]
			: [parentMcpServerId];
	...
	private async showGetSessionPrompt(
		...
		if (result !== SessionPromptChoice.Cancel) {
			this._authenticationAccessService.updateAllowedMcpServers(
				provider.id,
				accountName,
				[
					{
			...
			disposables.add(
				quickPick.onDidAccept(async (_) => {
					...
					this._authenticationAccessService.updateAllowedMcpServers(
						providerId,
						accountName,
						[{ id: mcpServerId, name: mcpServerName, allowed: true }],
					);
	...
	async requestNewSession(
		...
		const mcpServerHasExistingRequest =
			providerRequests &&
		...
		if (providerRequests) {
			const existingRequest = providerRequests[scopesList] || {
				disposables: [],
				requestingMcpServerIds: [],
			};

			providerRequests[scopesList] = {
				disposables: [...existingRequest.disposables, menuItem, signInCommand],
				requestingMcpServerIds: [
					...existingRequest.requestingMcpServerIds,
					mcpServerId,
				],
			};
