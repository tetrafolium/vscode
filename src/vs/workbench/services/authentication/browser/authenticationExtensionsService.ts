	...
	implements IAuthenticationExtensionsService
{
	...
	updateAccountPreference(
		...
		const childrenExtensions =
			this._inheritAuthAccountPreferenceParentToChildren[parentExtensionId];
	...
	async requestNewSession(
		...
		const signInRequestKey = isAuthenticationWwwAuthenticateRequest(
			scopeListOrRequest,
		)
		...
		const extensionHasExistingRequest =
			...
			providerRequests[signInRequestKey].requestingExtensionIds.includes(
				extensionId,
			);
