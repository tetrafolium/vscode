 ...
 */
class AccountExtensionQuery
	extends BaseQuery
	implements IAccountExtensionQuery
{
	constructor(
		providerId: string,
	...
	getUsage(): {
		...
		const allUsages =
			this.queryService.authenticationUsageService.readAccountUsages(
				this.providerId,
				this.accountName,
			);
		return allUsages
			.filter(
				(usage) =>
					usage.extensionId === ExtensionIdentifier.toKey(this.extensionId),
			)
			.map((usage) => ({
	...
	removeUsage(): void {
		...
		const filteredUsages = allUsages.filter(
			(usage) => usage.extensionId !== this.extensionId,
		);

		// Clear all usages and re-add the filtered ones
		this.queryService.authenticationUsageService.removeAccountUsage(
			this.providerId,
			this.accountName,
		);
	...
	isTrusted(): boolean {
		const allowedExtensions =
			this.queryService.authenticationAccessService.readAllowedExtensions(
				this.providerId,
				this.accountName,
			);
		const extension = allowedExtensions.find(
			(ext) => ext.id === this.extensionId,
		);
 ...
 */
class AccountMcpServerQuery
	extends BaseQuery
	implements IAccountMcpServerQuery
{
	constructor(
		providerId: string,
		public readonly accountName: string,
	...
	isAccessAllowed(): boolean | undefined {
		return this.queryService.authenticationMcpAccessService.isAccessAllowed(
			this.providerId,
			this.accountName,
			this.mcpServerId,
		);
	...
	getUsage(): {
		...
		const allUsages =
			this.queryService.authenticationMcpUsageService.readAccountUsages(
				this.providerId,
				this.accountName,
			);
		return allUsages
	...
	removeUsage(): void {
		...
		const filteredUsages = allUsages.filter(
			(usage) => usage.mcpServerId !== this.mcpServerId,
		);

		// Clear all usages and re-add the filtered ones
		this.queryService.authenticationMcpUsageService.removeAccountUsage(
			this.providerId,
			this.accountName,
		);
		for (const usage of filteredUsages) {
	...
	isPreferred(): boolean {
		const preferredAccount =
			this.queryService.authenticationMcpService.getAccountPreference(
				this.mcpServerId,
				this.providerId,
			);
		return preferredAccount === this.accountName;
	...
	isTrusted(): boolean {
		const allowedMcpServers =
			this.queryService.authenticationMcpAccessService.readAllowedMcpServers(
				this.providerId,
				this.accountName,
			);
		const mcpServer = allowedMcpServers.find(
			(server) => server.id === this.mcpServerId,
		);
 ...
 */
class AccountExtensionsQuery
	extends BaseQuery
	implements IAccountExtensionsQuery
{
	constructor(
		providerId: string,
		public readonly accountName: string,
	...
	getAllowedExtensions(): {
		...
		return allowedExtensions
			...
			.map((ext) => {
				...
				const extensionUsages = usages.filter(
					(usage) => usage.extensionId === ext.id,
				);
				const lastUsed =
					extensionUsages.length > 0
						? Math.max(...extensionUsages.map((u) => u.lastUsed))
						: undefined;
 ...
 */
class AccountMcpServersQuery
	extends BaseQuery
	implements IAccountMcpServersQuery
{
	constructor(
		providerId: string,
		public readonly accountName: string,
	...
	getAllowedMcpServers(): {
		...
		return this.queryService.authenticationMcpAccessService
			.readAllowedMcpServers(this.providerId, this.accountName)
	...
	forEach(callback: (mcpServerQuery: IAccountMcpServerQuery) => void): void {
		...
		for (const mcpServerId of mcpServerIds) {
			const mcpServerQuery = new AccountMcpServerQuery(
				this.providerId,
				this.accountName,
				mcpServerId,
				this.queryService,
...
class AccountEntitiesQuery extends BaseQuery implements IAccountEntitiesQuery {
	...
	getEntityCount(): { extensions: number; mcpServers: number; total: number } {
		...
		const allowedMcpServers = this.queryService.authenticationMcpAccessService
			.readAllowedMcpServers(this.providerId, this.accountName)
	...
	removeAllAccess(): void {
		...
		const extensionsQuery = new AccountExtensionsQuery(
			this.providerId,
			this.accountName,
			this.queryService,
		);
		...
		const mcpServersQuery = new AccountMcpServersQuery(
			this.providerId,
			this.accountName,
			this.queryService,
		);
	...
	forEach(
		callback: (entityId: string, entityType: "extension" | "mcpServer") => void,
	): void {
...
class AccountQuery extends BaseQuery implements IAccountQuery {
	...
	extension(extensionId: string): IAccountExtensionQuery {
		return new AccountExtensionQuery(
			this.providerId,
			this.accountName,
			extensionId,
			this.queryService,
	...
	mcpServer(mcpServerId: string): IAccountMcpServerQuery {
		return new AccountMcpServerQuery(
			this.providerId,
			this.accountName,
			mcpServerId,
			this.queryService,
	...
	extensions(): IAccountExtensionsQuery {
		return new AccountExtensionsQuery(
			this.providerId,
			this.accountName,
			this.queryService,
		);
	...
	mcpServers(): IAccountMcpServersQuery {
		return new AccountMcpServersQuery(
			this.providerId,
			this.accountName,
			this.queryService,
		);
	...
	entities(): IAccountEntitiesQuery {
		return new AccountEntitiesQuery(
			this.providerId,
			this.accountName,
			this.queryService,
		);
	...
	remove(): void {
		...
		this.queryService.authenticationAccessService.removeAllowedExtensions(
			this.providerId,
			this.accountName,
		);
		this.queryService.authenticationUsageService.removeAccountUsage(
		...
		this.queryService.authenticationMcpAccessService.removeAllowedMcpServers(
			this.providerId,
			this.accountName,
		);
		this.queryService.authenticationMcpUsageService.removeAccountUsage(
 ...
 */
class ProviderExtensionQuery
	extends BaseQuery
	implements IProviderExtensionQuery
{
	constructor(
		providerId: string,
		public readonly extensionId: string,
	...
	removeAccountPreference(): void {
		this.queryService.authenticationExtensionsService.removeAccountPreference(
			this.extensionId,
			this.providerId,
		);
	}
 ...
 */
class ProviderMcpServerQuery
	extends BaseQuery
	implements IProviderMcpServerQuery
{
	constructor(
		providerId: string,
		public readonly mcpServerId: string,
	...
	async getLastUsedAccount(): Promise<string | undefined> {
		try {
			const accounts =
				await this.queryService.authenticationService.getAccounts(
					this.providerId,
				);
			...
			for (const account of accounts) {
				...
				const mcpServerUsages = usages.filter(
					(usage) => usage.mcpServerId === this.mcpServerId,
				);
	...
	getPreferredAccount(): string | undefined {
		return this.queryService.authenticationMcpService.getAccountPreference(
			this.mcpServerId,
			this.providerId,
		);
	}
	...
	removeAccountPreference(): void {
		this.queryService.authenticationMcpService.removeAccountPreference(
			this.mcpServerId,
			this.providerId,
		);
	}
...
class ProviderQuery extends BaseQuery implements IProviderQuery {
	...
	extension(extensionId: string): IProviderExtensionQuery {
		return new ProviderExtensionQuery(
			this.providerId,
			extensionId,
			this.queryService,
		);
	...
	mcpServer(mcpServerId: string): IProviderMcpServerQuery {
		return new ProviderMcpServerQuery(
			this.providerId,
			mcpServerId,
			this.queryService,
		);
	...
	async getUsageStats(): Promise<IAuthenticationUsageStats> {
		...
		try {
			...
			totalSessions = recentActivity.reduce(
				(sum, activity) => sum + activity.usageCount,
				0,
			);
		} catch {
	...
	async forEachAccount(
		callback: (accountQuery: IAccountQuery) => void,
	): Promise<void> {
		try {
			const accounts =
				await this.queryService.authenticationService.getAccounts(
...
class ExtensionQuery implements IExtensionQuery {
	...
	async getProvidersWithAccess(includeInternal?: boolean): Promise<string[]> {
		...
		const providerIds =
			this.queryService.authenticationService.getProviderIds();

		for (const providerId of providerIds) {
			// Skip internal providers unless explicitly requested
			if (
				!includeInternal &&
				providerId.startsWith(INTERNAL_AUTH_PROVIDER_PREFIX)
			) {
				continue;
			}
			...
			try {
				const accounts =
					await this.queryService.authenticationService.getAccounts(providerId);
				const hasAccess = accounts.some((account) => {
					const accessAllowed =
						this.queryService.authenticationAccessService.isAccessAllowed(
							providerId,
							account.label,
							this.extensionId,
						);
	...
	async getProvidersWithAccess(includeInternal?: boolean): Promise<string[]> {
		...
		for (const providerId of providerIds) {
			...
			try {
				...
				const hasAccess = accounts.some((account) => {
					const accessAllowed =
						this.queryService.authenticationMcpAccessService.isAccessAllowed(
							providerId,
							account.label,
							this.mcpServerId,
						);
...
export class AuthenticationQueryService
	extends Disposable
	implements IAuthenticationQueryService
{
	...
	constructor(
		...
		public readonly authenticationUsageService: IAuthenticationUsageService,
		@IAuthenticationMcpUsageService
		public readonly authenticationMcpUsageService: IAuthenticationMcpUsageService,
		@IAuthenticationAccessService
		public readonly authenticationAccessService: IAuthenticationAccessService,
		@IAuthenticationMcpAccessService
		public readonly authenticationMcpAccessService: IAuthenticationMcpAccessService,
		@IAuthenticationExtensionsService
		public readonly authenticationExtensionsService: IAuthenticationExtensionsService,
		@IAuthenticationMcpService
		public readonly authenticationMcpService: IAuthenticationMcpService,
		...
		this._register(
			this.authenticationMcpService.onDidChangeAccountPreference((e) => {
				this._onDidChangePreferences.fire({
					providerId: e.providerId,
		...
		this._register(
			this.authenticationAccessService.onDidChangeExtensionSessionAccess(
				(e) => {
		...
		this._register(
			this.authenticationMcpAccessService.onDidChangeMcpSessionAccess((e) => {
				this._onDidChangeAccess.fire({
					providerId: e.providerId,
					accountName: e.accountName,
				});
	...
	getProviderIds(includeInternal?: boolean): string[] {
		return this.authenticationService.getProviderIds().filter((providerId) => {
			...
			return (
				includeInternal || !providerId.startsWith(INTERNAL_AUTH_PROVIDER_PREFIX)
			);
		});
	}

	async clearAllData(
		confirmation: "CLEAR_ALL_AUTH_DATA",
		includeInternal: boolean = true,
	): Promise<void> {
		if (confirmation !== "CLEAR_ALL_AUTH_DATA") {
			throw new Error(
				"Must provide confirmation string to clear all authentication data",
			);
		}
		...
		for (const providerId of providerIds) {
			try {
				...
				for (const account of accounts) {
					...
					this.authenticationAccessService.removeAllowedExtensions(
						providerId,
						account.label,
					);
					this.authenticationUsageService.removeAccountUsage(
						providerId,
						account.label,
					);

					// Clear MCP server data
					this.authenticationMcpAccessService.removeAllowedMcpServers(
						providerId,
						account.label,
					);
					this.authenticationMcpUsageService.removeAccountUsage(
						providerId,
						account.label,
					);
				}
				...
				this.logService.error(
					`Error clearing data for provider ${providerId}:`,
					error,
				);
			}
		}
