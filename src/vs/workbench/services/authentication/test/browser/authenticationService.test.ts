...
suite("AuthenticationService", () => {
	...
	suite("authenticationSessions", () => {
		...
		test("getSessions - authorization server is not registered", async () => {
			...
			assert.rejects(() =>
				authenticationService.getSessions(provider.id, [], {
					authorizationServer: URI.parse("https://example.com"),
				}),
			);
			assert.ok(!isCalled);
		});
		...
		test("createSession", async () => {
			...
			const session = await authenticationService.createSession(provider.id, [
				"repo",
			]);
		...
		test("getSessions - forwards resource option to provider", async () => {
			...
			await authenticationService.getSessions(provider.id, ["scope"], {
		...
		test("createSession - forwards resource option to provider", async () => {
			...
			await authenticationService.createSession(provider.id, ["scope"], {
				resource: "https://api.example.com/",
			});
		...
		test("removeSession", async () => {
			...
			const provider = createProvider({
				onDidChangeSessions: emitter.event,
				removeSession: async () =>
					emitter.fire({ added: [], removed: [session], changed: [] }),
			});
			const changed = Event.toPromise(
				authenticationService.onDidChangeSessions,
			);
...
suite("AuthenticationService - tryActivateProvider", () => {
	...
	test("should resolve when provider registers even if activateByEvent never resolves (#315841)", async () => {
		...
		const provider = createProvider({
			getSessions: async () => [createSession()],
		});
