...
suite("AuthenticationQueryService Integration Tests", () => {
	...
	setup(() => {
		...
		instantiationService.stub(
			IAuthenticationExtensionsService,
			disposables.add(new TestExtensionsService()),
		);
		instantiationService.stub(
			IAuthenticationMcpService,
			disposables.add(new TestMcpService()),
		);

		// Create the query service
		queryService = disposables.add(
			instantiationService.createInstance(AuthenticationQueryService),
		);
	});

	test("usage tracking stores and retrieves data correctly", () => {
		const extensionQuery = queryService
			.provider("github")
	...
	test("access control persists across queries", () => {
		...
		const sameExtensionQuery = queryService
			.provider("github")
			.account("user@example.com")
			.extension("my-extension");
	...
	test("account preferences work across services", () => {
		...
		extensionQuery.setPreferredAccount({
			id: "user1",
		...
		assert.strictEqual(
			extensionQuery.getPreferredAccount(),
			"user@example.com",
		);
		assert.strictEqual(mcpQuery.getPreferredAccount(), "admin@example.com");

		// Test preference detection
		const userExtensionQuery = queryService
			.provider("github")
			.account("user@example.com")
			.extension("my-extension");
		const adminMcpQuery = queryService
			.provider("github")
		...
		const wrongExtensionQuery = queryService
			.provider("github")
	...
	test("account removal cleans up all related data", () => {
		const accountQuery = queryService
			.provider("github")
			.account("user@example.com");
		...
		assert.strictEqual(
			accountQuery.extension("ext1").isAccessAllowed(),
			undefined,
		);
		assert.strictEqual(accountQuery.extension("ext1").getUsage().length, 0);
		assert.strictEqual(
			accountQuery.mcpServer("mcp1").isAccessAllowed(),
			undefined,
		);
	...
	test("provider registration and listing works", () => {
		...
		assert.strictEqual(
			authService.isAuthenticationProviderRegistered("github"),
			true,
		);
	});
	...
	test("MCP usage and access work independently from extensions", () => {
		...
		assert.strictEqual(
			extensionQuery.getUsage()[0].extensionId,
			"my-extension",
		);
	...
	test("getAllAccountPreferences returns synchronously", () => {
		...
		extensionQuery
			.provider("github")
			.setPreferredAccount({ id: "user1", label: "github-user@example.com" });
		extensionQuery
			.provider("azure")
			.setPreferredAccount({ id: "user2", label: "azure-user@example.com" });
		mcpQuery
			.provider("github")
			.setPreferredAccount({ id: "user3", label: "github-mcp@example.com" });
		...
		assert.strictEqual(
			extensionPreferences.get("github"),
			"github-user@example.com",
		);
		assert.strictEqual(
			extensionPreferences.get("azure"),
	...
	test("cross-provider extension queries work correctly", () => {
		...
		extensionQuery
			.provider("github")
			.setPreferredAccount({ id: "user1", label: "user@example.com" });
		extensionQuery
			.provider("azure")
			.setPreferredAccount({ id: "user2", label: "admin@example.com" });

		assert.strictEqual(
			extensionQuery.provider("github").getPreferredAccount(),
			"user@example.com",
		);
		assert.strictEqual(
			extensionQuery.provider("azure").getPreferredAccount(),
			"admin@example.com",
		);
	});
	...
	test("error handling for invalid inputs works correctly", () => {
		...
		const emptyAccountQuery = queryService
			.provider("github")
			.account("")
			.extension("my-extension");
		assert.doesNotThrow(() => {
		...
		const emptyExtensionQuery = queryService
			.provider("github")
			.account("user@example.com")
			.extension("");
		assert.doesNotThrow(() => {
	...
	test("data consistency across different query paths", () => {
		...
		const extensionQuery1 = queryService
			.provider("github")
			.account("user@example.com")
			.extension("my-extension");
		...
		const extensionQuery2 = queryService
			.extension("my-extension")
			.provider("github");
		...
		extensionQuery2.setPreferredAccount({
			id: "user",
			label: "user@example.com",
		});
		assert.strictEqual(
			extensionQuery2.getPreferredAccount(),
			"user@example.com",
		);
	...
	test("preference management handles complex scenarios", () => {
		...
		assert.strictEqual(
			queryService
				.provider("github")
				.account("github-user@example.com")
				.extension("my-extension")
				.isPreferred(),
			true,
		);
		assert.strictEqual(
			queryService
				.provider("azure")
				.account("azure-user@example.com")
				.extension("my-extension")
				.isPreferred(),
			true,
		);
	...
	test("MCP server vs extension data isolation is complete", () => {
		...
		queryService
			.extension(sameId)
			.provider("github")
			.setPreferredAccount({ id: "ext-user", label: "ext@example.com" });
		queryService
			.mcpServer(sameId)
			.provider("github")
			.setPreferredAccount({ id: "mcp-user", label: "mcp@example.com" });

		assert.strictEqual(
			queryService.extension(sameId).provider("github").getPreferredAccount(),
			"ext@example.com",
		);
		assert.strictEqual(
			queryService.mcpServer(sameId).provider("github").getPreferredAccount(),
			"mcp@example.com",
		);
	});
	...
	test("provider listing and registration integration", () => {
		...
		assert.strictEqual(
			authService.isAuthenticationProviderRegistered("test-provider"),
			true,
		);
	});
	...
	test("addUsage calls addAccountUsage with correct parameters", () => {
		...
		const [providerId, accountName, scopes, extensionId, extensionName] =
			calls[0].args;
	...
	test("MCP addUsage calls addAccountUsage with correct parameters", () => {
		...
		const [providerId, accountName, scopes, serverId, serverName] =
			calls[0].args;
		assert.strictEqual(providerId, "azure");
		assert.strictEqual(accountName, "admin@company.com");
	...
	test("account removal calls all appropriate cleanup methods", () => {
		...
		const extensionUsageRemoval =
			usageService.getCallsFor("removeAccountUsage");
		const mcpUsageRemoval = mcpUsageService.getCallsFor("removeAccountUsage");
		const extensionAccessRemoval = accessService.getCallsFor(
			"removeAllowedExtensions",
		);
		const mcpAccessRemoval = mcpAccessService.getCallsFor(
			"removeAllowedMcpServers",
		);
		...
		[
			extensionUsageRemoval[0],
			mcpUsageRemoval[0],
			extensionAccessRemoval[0],
			mcpAccessRemoval[0],
		].forEach((call) => {
	...
	test("multiple operations accumulate service calls correctly", () => {
		...
		// Verify call counts
		assert.strictEqual(
			accessService.getCallsFor("updateAllowedExtensions").length,
			2,
		);
	...
	test("getProvidersWithAccess filters internal providers by default", async () => {
		...
		const githubProvider = createProvider({ id: "github", label: "GitHub" });
		const azureProvider = createProvider({ id: "azure", label: "Azure" });
		const internalProvider1 = createProvider({
			id: "__internal1",
			label: "Internal Provider 1",
		});
		const internalProvider2 = createProvider({
			id: "__internal2",
			label: "Internal Provider 2",
		});
		...
		authService.addAccounts("github", [
			{ id: "user1", label: "user@github.com" },
		]);
		authService.addAccounts("azure", [
			{ id: "user2", label: "user@azure.com" },
		]);
		authService.addAccounts("__internal1", [
			{ id: "user3", label: "internal1@example.com" },
		]);
		authService.addAccounts("__internal2", [
			{ id: "user4", label: "internal2@example.com" },
		]);
	...
	test("getProvidersWithAccess includes internal providers when requested", async () => {
		...
		authService.addAccounts("__internal1", [
			{ id: "user2", label: "internal@example.com" },
		]);
		...
		const providersWithAccess =
			await extensionQuery.getProvidersWithAccess(true);
	...
	test("MCP server getProvidersWithAccess includes internal providers when requested", async () => {
		...
		const mcpServerQuery = queryService.mcpServer("my-server");
		const providersWithAccess =
			await mcpServerQuery.getProvidersWithAccess(true);
	...
	test("internal provider filtering works with mixed access patterns", async () => {
		...
		const noAccessProvider = createProvider({
			id: "no-access",
			label: "No Access Provider",
		});
		...
		// Add accounts to all providers
		authService.addAccounts("normal", [
			{ id: "user1", label: "user@normal.com" },
		]);
		authService.addAccounts("__internal", [
			{ id: "user2", label: "internal@example.com" },
		]);
		authService.addAccounts("no-access", [
			{ id: "user3", label: "user@noaccess.com" },
		]);
		...
		const providersWithoutInternal =
			await extensionQuery.getProvidersWithAccess(false);
	...
	test("internal provider filtering respects the __ prefix exactly", async () => {
		...
		const regularProvider = createProvider({
			id: "regular",
			label: "Regular Provider",
		});
		const underscoreProvider = createProvider({
			id: "_single",
			label: "Single Underscore Provider",
		});
		const doubleUnderscoreProvider = createProvider({
			id: "__double",
			label: "Double Underscore Provider",
		});
		const tripleUnderscoreProvider = createProvider({
			id: "___triple",
			label: "Triple Underscore Provider",
		});
		const underscoreInMiddleProvider = createProvider({
			id: "mid_underscore",
			label: "Middle Underscore Provider",
		});
		...
		authService.registerAuthenticationProvider(
			"__double",
			doubleUnderscoreProvider,
		);
		authService.registerAuthenticationProvider(
			"___triple",
			tripleUnderscoreProvider,
		);
		authService.registerAuthenticationProvider(
			"mid_underscore",
			underscoreInMiddleProvider,
		);

		// Add accounts to all providers
		authService.addAccounts("regular", [
			{ id: "user1", label: "user@regular.com" },
		]);
		authService.addAccounts("_single", [
			{ id: "user2", label: "user@single.com" },
		]);
		authService.addAccounts("__double", [
			{ id: "user3", label: "user@double.com" },
		]);
		authService.addAccounts("___triple", [
			{ id: "user4", label: "user@triple.com" },
		]);
		authService.addAccounts("mid_underscore", [
			{ id: "user5", label: "user@middle.com" },
		]);
	...
	test("getAllAccountPreferences filters internal providers by default for extensions", () => {
		...
		authService.registerAuthenticationProvider(
			"github",
			createProvider({ id: "github", label: "GitHub" }),
		);
		authService.registerAuthenticationProvider(
			"azure",
			createProvider({ id: "azure", label: "Azure" }),
		);
		authService.registerAuthenticationProvider(
			"__internal",
			createProvider({ id: "__internal", label: "Internal" }),
		);
		...
		extensionQuery
			.provider("github")
			.setPreferredAccount({ id: "user1", label: "user@github.com" });
		extensionQuery
			.provider("azure")
			.setPreferredAccount({ id: "user2", label: "user@azure.com" });
		extensionQuery
			.provider("__internal")
			.setPreferredAccount({ id: "user3", label: "internal@example.com" });
		...
		assert.strictEqual(
			prefsWithInternal.get("__internal"),
	...
	test("isTrusted method works with mock service", () => {
		...
		// Test trusted server
		const trustedQuery = queryService
			.provider("github")
			.account("user@github.com")
		...
		const nonTrustedQuery = queryService
			.provider("github")
			.account("user@github.com")
			.mcpServer("non-trusted-server");
	...
	test("getAllowedMcpServers method returns servers with trusted state", () => {
		...
		const allowedServers = queryService
			.provider("github")
			.account("user@github.com")
			.mcpServers()
	...
	suite("Account entities query", () => {
		test("hasAnyUsage returns false for clean account", () => {
			const entitiesQuery = queryService
				.provider("github")
				.account("clean@example.com")
				.entities();
		...
		test("hasAnyUsage returns true when extension has access", () => {
			...
			accountQuery
				.extension("test-ext")
				.setAccessAllowed(true, "Test Extension");
		...
		test("hasAnyUsage returns true when MCP server has access", () => {
			...
			accountQuery
				.mcpServer("test-server")
				.setAccessAllowed(true, "Test Server");
		...
		test("removeAllAccess removes access for all entity types", () => {
			...
			assert.strictEqual(
				accountQuery.extension("ext2").isAccessAllowed(),
				true,
			);
			assert.strictEqual(
				accountQuery.mcpServer("server1").isAccessAllowed(),
				true,
			);
			assert.strictEqual(
				accountQuery.mcpServer("server2").isAccessAllowed(),
				true,
			);
		...
		test("forEach iterates over all entity types", () => {
			...
			const entitiesQuery = accountQuery.entities();
			const visitedEntities: Array<{
				id: string;
				type: "extension" | "mcpServer";
			}> = [];
			...
			assert.ok(
				visitedEntities.some((e) => e.id === "ext1" && e.type === "extension"),
			);
			assert.ok(
				visitedEntities.some((e) => e.id === "ext2" && e.type === "extension"),
			);
			assert.ok(
				visitedEntities.some(
					(e) => e.id === "server1" && e.type === "mcpServer",
				),
			);
			assert.ok(
				visitedEntities.some(
					(e) => e.id === "server2" && e.type === "mcpServer",
				),
			);
		});
	});
