...
} from "../../browser/authenticationMcpAccessService.js";
...
suite("AuthenticationMcpAccessService", () => {
	...
	let productService: IProductService & {
		trustedMcpAuthAccess?: string[] | Record<string, string[]>;
	};
	...
	setup(() => {
		...
		authenticationMcpAccessService = disposables.add(
			instantiationService.createInstance(AuthenticationMcpAccessService),
		);
	});

	suite("isAccessAllowed", () => {
		test("returns undefined for unknown MCP server with no product configuration", () => {
			const result = authenticationMcpAccessService.isAccessAllowed(
				"github",
				"user@example.com",
				"unknown-server",
			);
		...
		test("returns true for trusted MCP server from product.json (object format)", () => {
			...
			const result1 = authenticationMcpAccessService.isAccessAllowed(
				"github",
				"user@example.com",
				"github-server",
			);
			...
			const result2 = authenticationMcpAccessService.isAccessAllowed(
				"microsoft",
				"user@microsoft.com",
				"microsoft-server",
			);
	...
	suite("readAllowedMcpServers", () => {
		test("returns empty array when no data exists", () => {
			const result = authenticationMcpAccessService.readAllowedMcpServers(
				"github",
				"user@example.com",
			);
		...
		test("includes trusted servers from product.json (object format)", () => {
			...
			const githubResult = authenticationMcpAccessService.readAllowedMcpServers(
				"github",
				"user@example.com",
			);
			...
			const microsoftResult =
				authenticationMcpAccessService.readAllowedMcpServers(
					"microsoft",
					"user@microsoft.com",
				);
		...
		test("handles malformed JSON in storage gracefully", () => {
			...
			storageService.store(
				"mcpserver-github-user@example.com",
				"invalid json",
	...
	suite("updateAllowedMcpServers", () => {
		...
		test("does not store trusted servers from product.json", () => {
			...
			const storedData = JSON.parse(
				storageService.get(storageKey, StorageScope.APPLICATION) || "[]",
			);
			...
			const allServers = authenticationMcpAccessService.readAllowedMcpServers(
				"github",
				"user@example.com",
			);
		...
		test("fires onDidChangeMcpSessionAccess event", () => {
			...
			const disposable =
				authenticationMcpAccessService.onDidChangeMcpSessionAccess((event) => {
					eventFired = true;
					eventData = event;
				});
	...
	suite("removeAllowedMcpServers", () => {
		test("removes all stored MCP servers for account", () => {
			...
			// Verify they exist
			let result = authenticationMcpAccessService.readAllowedMcpServers(
				"github",
				"user@example.com",
			);
			assert.strictEqual(result.length, 2);
			...
			// Remove them
			authenticationMcpAccessService.removeAllowedMcpServers(
				"github",
				"user@example.com",
			);

			// Verify they're gone
			result = authenticationMcpAccessService.readAllowedMcpServers(
				"github",
				"user@example.com",
			);
			assert.strictEqual(result.length, 0);
	...
	suite("onDidChangeMcpSessionAccess event", () => {
		...
		test("multiple listeners receive events", () => {
			...
			const disposable1 =
				authenticationMcpAccessService.onDidChangeMcpSessionAccess(() => {
					listener1Fired = true;
				});
	...
	suite("integration scenarios", () => {
		test("complete workflow: add, update, query, remove", () => {
			...
			// Initially unknown
			assert.strictEqual(
				authenticationMcpAccessService.isAccessAllowed(
					providerId,
					accountName,
					serverId,
				),
