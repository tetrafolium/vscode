...
suite("AuthenticationAccessService", () => {
	...
	let productService: IProductService & {
		trustedExtensionAuthAccess?: string[] | Record<string, string[]>;
	};
	...
	setup(() => {
		...
		productService = {
			...TestProductService,
			trustedExtensionAuthAccess: undefined,
		};
		instantiationService.stub(IProductService, productService);

		// Create the service instance
		authenticationAccessService = disposables.add(
	...
	suite("isAccessAllowed", () => {
		...
		test("returns true for trusted extension from product.json (object format)", () => {
			...
			const result1 = authenticationAccessService.isAccessAllowed(
				"github",
				"user@example.com",
				"github-extension",
			);
	...
	suite("readAllowedExtensions", () => {
		test("returns empty array when no data exists", () => {
			const result = authenticationAccessService.readAllowedExtensions(
				"github",
				"user@example.com",
			);
		...
		test("includes trusted extensions from product.json (array format)", () => {
			...
			const trustedExtension1 = result.find(
				(e) => e.id === "trusted-extension-1",
			);
			...
			const trustedExtension2 = result.find(
				(e) => e.id === "trusted-extension-2",
			);
		...
		test("includes trusted extensions from product.json (object format)", () => {
			...
			const githubResult = authenticationAccessService.readAllowedExtensions(
				"github",
				"user@example.com",
			);
			...
			const microsoftResult = authenticationAccessService.readAllowedExtensions(
				"microsoft",
				"user@microsoft.com",
			);
			...
			const unknownResult = authenticationAccessService.readAllowedExtensions(
				"unknown",
				"user@unknown.com",
			);
	...
	suite("updateAllowedExtensions", () => {
		...
		test("does not store trusted extensions - they should only come from product.json", () => {
			...
			const storedData = storageService.get(
				"github-user@example.com",
				StorageScope.APPLICATION,
			);
		...
		test("filters out trusted extensions before storing", () => {
			...
			const extensions: AllowedExtension[] = [
				...
				{
					id: "another-regular-ext",
					name: "Another Regular Extension",
					allowed: false,
				},
			...
			assert.ok(
				parsedData.find((e: AllowedExtension) => e.id === "regular-ext"),
			);
			assert.ok(
				parsedData.find(
					(e: AllowedExtension) => e.id === "another-regular-ext",
				),
			);
			assert.ok(
				!parsedData.find((e: AllowedExtension) => e.id === "trusted-ext-1"),
			);
			assert.ok(
				!parsedData.find((e: AllowedExtension) => e.id === "trusted-ext-2"),
			);
		});
	...
	suite("removeAllowedExtensions", () => {
		...
		test("does not affect trusted extensions from product.json", () => {
			...
			let result = authenticationAccessService.readAllowedExtensions(
				"github",
				"user@example.com",
			);
			...
			result = authenticationAccessService.readAllowedExtensions(
				"github",
				"user@example.com",
			);
			assert.strictEqual(result.length, 1);
