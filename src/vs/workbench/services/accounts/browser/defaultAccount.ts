...
export const DEFAULT_ACCOUNT_SIGN_IN_COMMAND =
...
function toDefaultAccountConfig(
	defaultChatAgent: IDefaultChatAgent,
): IDefaultAccountConfig {
	return {
		preferredExtensions: [
...
export class DefaultAccountService
	extends Disposable
	implements IDefaultAccountService
{
	...
	private readonly _onDidChangePolicyData = this._register(
		new Emitter<IPolicyData | null>(),
	);
	readonly onDidChangePolicyData = this._onDidChangePolicyData.event;

	private readonly _onDidChangeCopilotTokenInfo = this._register(
		new Emitter<ICopilotTokenInfo | null>(),
	);
	readonly onDidChangeCopilotTokenInfo =
		this._onDidChangeCopilotTokenInfo.event;
	...
	setDefaultAccountProvider(provider: IDefaultAccountProvider): void {
		...
		provider
			...
			.finally(() => {
				...
				this._register(
					provider.onDidChangeDefaultAccount((account) =>
						this.setDefaultAccount(account),
					),
				);
				this._register(
					provider.onDidChangePolicyData((policyData) =>
						this._onDidChangePolicyData.fire(policyData),
					),
				);
				this._register(
					provider.onDidChangeCopilotTokenInfo((tokenInfo) =>
						this._onDidChangeCopilotTokenInfo.fire(tokenInfo),
					),
				);
			});
	}
...
type DefaultAccountStatusTelemetryClassification = {
	...
	status: {
		...
		comment: "Indicates whether default account is available or not.";
	};
	initial: {
		classification: "SystemMetaData";
		purpose: "PerformanceAndHealth";
		comment: "Indicates whether this is the initial status report.";
	};
};
...
type ManagedSettingsFetchTelemetryClassification = {
	...
	outcome: {
		classification: "SystemMetaData";
		purpose: "PerformanceAndHealth";
		comment: "High-level outcome: a numeric HTTP status (`status:NNN`), or one of `ok` / `no-response` / `parse-error`.";
	};
	rateLimitBackoffActive: {
		classification: "SystemMetaData";
		purpose: "PerformanceAndHealth";
		isMeasurement: true;
		comment: "True when the request was short-circuited because a prior rate-limit Retry-After window was still active.";
	};
};

class DefaultAccountProvider
	extends Disposable
	implements IDefaultAccountProvider
{
	...
	private readonly accountDataPollScheduler = this._register(
	...
	constructor(
		...
		this.accountStatusContext =
			CONTEXT_DEFAULT_ACCOUNT_STATE.bindTo(contextKeyService);
		...
		this.initPromise = this.init().finally(() => {
			this.telemetryService.publicLog2<
				DefaultAccountStatusTelemetry,
				DefaultAccountStatusTelemetryClassification
		...
		if (cached) {
			try {
				...
				if (accountId && policyData) {
					...
					const result: ICachedAccountData = {
						accountPolicyData: {
							accountId,
							policyData,
							tokenEntitlementsFetchedAt,
				...
				// New format
				const { accountPolicyData, copilotTokenInfo: wrappedCopilotTokenInfo } =
					parsed;
				if (accountPolicyData?.accountId && accountPolicyData?.policyData) {
					this.logService.debug(
						"[DefaultAccount] Initializing with cached policy data",
					);
					return {
						accountPolicyData,
	...
	private async init(): Promise<void> {
		...
		if (
			...
			this.logService.debug(
				"[DefaultAccount] Running in web without remote, skipping initialization",
			);
			return;
		}
		...
		try {
			...
			this.logService.debug(
				"[DefaultAccount] Installed extensions registered.",
			);
		} catch (error) {
			this.logService.error(
		...
		this._register(
			this.onDidChangeDefaultAccount((account) => {
				this.telemetryService.publicLog2<
					DefaultAccountStatusTelemetry,
					DefaultAccountStatusTelemetryClassification
				>("defaultaccount:status", {
		...
		this._register(
			this.authenticationService.onDidChangeSessions((e) => {
				const defaultAccountProvider =
					this.getDefaultAccountAuthenticationProvider();
				if (e.providerId !== defaultAccountProvider.id) {
					return;
				}
				if (
					this.defaultAccount &&
					e.event.removed?.some(
		...
		this._register(
			this.authenticationService.onDidRegisterAuthenticationProvider((e) => {
				const defaultAccountProvider =
					this.getDefaultAccountAuthenticationProvider();
	...
	private async updateDefaultAccount(options?: {
		forceRefresh?: boolean;
	}): Promise<void> {
		await this.updateThrottler.trigger(() =>
			this.doUpdateDefaultAccount(options),
		);
	}

	private async doUpdateDefaultAccount(options?: {
		forceRefresh?: boolean;
	}): Promise<void> {
		try {
	...
	private async fetchDefaultAccount(options?: {
		forceRefresh?: boolean;
	}): Promise<IDefaultAccountData | null> {
		const defaultAccountProvider =
			this.getDefaultAccountAuthenticationProvider();
		this.logService.debug(
			"[DefaultAccount] Default account provider ID:",
			defaultAccountProvider.id,
		);

		const declaredProvider = this.authenticationService.declaredProviders.find(
			(provider) => provider.id === defaultAccountProvider.id,
		);
		if (!declaredProvider) {
			this.logService.info(
				`[DefaultAccount] Authentication provider is not declared.`,
				defaultAccountProvider,
			);
			return null;
		}

		return await this.getDefaultAccountForAuthenticationProvider(
			defaultAccountProvider,
			options,
		);
	}
	...
	private setDefaultAccount(account: IDefaultAccountData | null): void {
		...
		this.logService.trace(
			"[DefaultAccount] Updating default account:",
			account,
		);
		if (account) {
			...
			this.logService.debug(
				"[DefaultAccount] Account status set to Unavailable",
			);
		}
	}
	...
	private async getDefaultAccountFromAuthenticatedSessions(
		...
		try {
			...
			const accountPolicyData =
				this._policyData?.accountId === accountId
					? this._policyData
					: undefined;

			const entitlementsResult = await this.getEntitlements(
				sessions,
				accountPolicyData,
				options,
			);
			...
			const [tokenEntitlementsResult, managedSettingsResult] =
				entitlementsData?.chat_enabled
					? await Promise.all([
			...
			const tokenEntitlementsFetchedAt: number | undefined =
				tokenEntitlementsResult?.fetchedAt;
			const managedSettingsFetchedAt: number | undefined =
				managedSettingsResult?.fetchedAt;
			let mcpRegistryDataFetchedAt: number | undefined;
			let policyData: Mutable<IPolicyData> | undefined =
				accountPolicyData?.policyData
					? { ...accountPolicyData.policyData }
					: undefined;
			if (entitlementsData) {
				policyData = policyData ?? {};
				policyData.cloud_session_storage_enabled =
					entitlementsData.cloud_session_storage_enabled;
			}
			if (tokenEntitlementsResult?.data) {
				const tokenEntitlementsData = tokenEntitlementsResult.data;
				policyData = policyData ?? {};
				policyData.chat_agent_enabled =
					tokenEntitlementsData.policyData.chat_agent_enabled;
				policyData.chat_preview_features_enabled =
					tokenEntitlementsData.policyData.chat_preview_features_enabled;
				policyData.mcp = tokenEntitlementsData.policyData.mcp;
				if (policyData.mcp) {
					const mcpRegistryResult = await this.getMcpRegistryProvider(
						sessions,
						accountPolicyData,
						options,
					);
			...
			const defaultAccount: IDefaultAccount = {
				...
				enterprise:
					authenticationProvider.enterprise ||
					sessions[0].account.label.includes("_"),
				entitlementsData,
			};
			this.logService.debug(
			...
			const accountPolicyResult: IAccountPolicyData | null =
				policyData || entitlementsFetchedAt
					? {
							accountId,
							policyData: policyData ?? {},
			...
			return {
				...
				copilotTokenInfo:
					tokenEntitlementsResult?.data?.copilotTokenInfo ?? null,
			};
		} catch (error) {
			this.logService.error(
	...
	private async findMatchingProviderSession(
		...
		for (const session of sessions) {
			this.logService.debug(
				"[DefaultAccount] Checking session with scopes",
				session.scopes,
			);
	...
	private async getSessions(
		...
		for (let attempt = 1; attempt <= 3; attempt++) {
			try {
				...
				for (const account of await this.authenticationService.getAccounts(
					authProviderId,
				)) {
				...
				return await this.authenticationService.getSessions(
					authProviderId,
					undefined,
	...
	private async getTokenEntitlements(
		...
		data:
			| {
					policyData: Partial<IPolicyData>;
					copilotTokenInfo: ICopilotTokenInfo;
			  }
			| undefined;
		fetchedAt: number;
	}> {
		if (
			...
			this.logService.debug(
				"[DefaultAccount] Using last fetched token entitlements data",
			);
			return {
				data: {
	...
	private async requestTokenEntitlements(
		sessions: AuthenticationSession[],
	): Promise<
		| { policyData: Partial<IPolicyData>; copilotTokenInfo: ICopilotTokenInfo }
		| undefined
	> {
		...
		this.logService.debug(
			"[DefaultAccount] Fetching token entitlements from:",
				...
				return {
					policyData: {
						...
						chat_preview_features_enabled:
							tokenMap.get("editor_preview_features") !== "0",
	...
	private async getEntitlements(
		...
		data: IEntitlementsData | undefined | null;
		fetchedAt: number | undefined;
	}> {
		const accountId = sessions[0].account.id;
		const existingData =
			this._defaultAccount?.accountId === accountId
				? this._defaultAccount?.defaultAccount.entitlementsData
				: undefined;
		if (
			!options?.forceRefresh &&
			existingData &&
			accountPolicyData?.entitlementsFetchedAt &&
			!this.isDataStale(accountPolicyData.entitlementsFetchedAt)
		) {
			this.logService.debug(
				"[DefaultAccount] Using last fetched entitlements data",
			);
			return {
				data: existingData,
		...
		this.logService.debug(
			"[DefaultAccount] Fetching entitlements from:",
			entitlementUrl,
		);
		const response = await this.request(
			entitlementUrl,
		...
		try {
			...
			this.logService.error(
				"[DefaultAccount] Failed to fetch entitlements",
				"No data returned",
			);
		} catch (error) {
			this.logService.error(
	...
	private async getMcpRegistryProvider(
		sessions: AuthenticationSession[],
		accountPolicyData: IAccountPolicyData | undefined,
		options?: { forceRefresh?: boolean },
	): Promise<
		{ data: IMcpRegistryProvider | null; fetchedAt: number } | undefined
	> {
		if (
			!options?.forceRefresh &&
			accountPolicyData?.mcpRegistryDataFetchedAt &&
			!this.isDataStale(accountPolicyData.mcpRegistryDataFetchedAt)
		) {
			this.logService.debug(
				"[DefaultAccount] Using last fetched MCP registry data",
			);
			const data =
				accountPolicyData.policyData.mcpRegistryUrl &&
				accountPolicyData.policyData.mcpAccess
					? {
	...
	private async requestMcpRegistryProvider(
		sessions: AuthenticationSession[],
	): Promise<IMcpRegistryProvider | null | undefined> {
		...
		this.logService.debug(
			"[DefaultAccount] Fetching MCP registry data from:",
			mcpRegistryDataUrl,
		);
		const response = await this.request(
			mcpRegistryDataUrl,
		...
		try {
			...
			if (data) {
				this.logService.debug(
					"Fetched MCP registry providers",
					data.mcp_registries,
				);
				return data.mcp_registries[0] ?? null;
			}
			this.logService.debug(
				"No MCP registry providers content found in response",
			);
			return null;
		} catch (error) {
			this.logService.error(
				"Failed to fetch MCP registry providers",
				getErrorMessage(error),
			);
			return undefined;
		}
	...
	private async getManagedSettings(
		...
		accountPolicyData: IAccountPolicyData | undefined,
		options?: { forceRefresh?: boolean },
	): Promise<{ data: Partial<IPolicyData> | undefined; fetchedAt: number }> {
		if (
			!options?.forceRefresh &&
			accountPolicyData?.managedSettingsFetchedAt &&
			!this.isDataStale(accountPolicyData.managedSettingsFetchedAt)
		) {
			this.logService.debug(
				"[DefaultAccount] Using last fetched managed settings data",
			);
			...
			return {
				data: {
					...
					extraKnownMarketplaces:
						accountPolicyData.policyData.extraKnownMarketplaces,
					strictKnownMarketplaces:
						accountPolicyData.policyData.strictKnownMarketplaces,
				},
	...
	private async requestManagedSettings(
		sessions: AuthenticationSession[],
	): Promise<Partial<IPolicyData> | undefined> {
		...
		this.logService.debug(
			"[DefaultAccount] Fetching managed settings from:",
			managedSettingsUrl,
		);
		...
		try {
			...
			const adapted = adaptManagedSettings(data ?? {}, (msg) =>
				this.logService.warn(msg),
			);
			// An empty response (`{}`) is a successful "no policy file present" signal.
			const pluginCount = adapted.enabledPlugins
				? Object.keys(adapted.enabledPlugins).length
				: 0;
			...
			if (pluginCount === 0 && marketplaceCount === 0 && !strictSet) {
				...
				this.logService.trace(
					"[DefaultAccount] Managed settings payload:",
	...
	private reportManagedSettingsOutcome(
		status: Exclude<ManagedSettingsFetchStatus, null | "no-url">,
		rateLimitBackoffActive: boolean,
	): void {
		this._managedSettingsFetchStatus = status;
		this.telemetryService.publicLog2<
			ManagedSettingsFetchTelemetry,
			ManagedSettingsFetchTelemetryClassification
		>("defaultaccount:managedSettings:fetch", {
	...
	private isRateLimited(response: IRequestContext): boolean {
		...
		if (
			status === 403 &&
			readHeader(response.res.headers, "x-ratelimit-remaining") === "0"
		) {
			return true;
		}
		...
		if (
			!isSuccess(response) &&
			readHeader(response.res.headers, "retry-after") !== undefined
		) {
			return true;
		}
		return false;
	...
	private async request(
		...
		if (Date.now() < this._rateLimitBackoffUntil) {
			const remainingSec = Math.ceil(
				(this._rateLimitBackoffUntil - Date.now()) / 1000,
			);
			this.logService.debug(
		...
		for (const session of sessions) {
			...
			try {
				...
				if (this.isRateLimited(response)) {
					const retryAfterSec =
						retryAfterFromHeaders(response.res.headers) ?? 60;
				...
				if (!token.isCancellationRequested) {
					this.logService.error(
						`[DefaultAccount] request: error ${error}`,
						url,
					);
				}
			}
		...
		if (!lastResponse) {
			this.logService.trace(
				"[DefaultAccount]: No response received for request",
				url,
			);
			return undefined;
		}
	...
	getDefaultAccountAuthenticationProvider(): IDefaultAccountAuthenticationProvider {
		if (
			this.configurationService.getValue<string | undefined>(
				this.defaultAccountConfig.authenticationProvider
					.enterpriseProviderConfig,
			) === this.defaultAccountConfig.authenticationProvider.enterprise.id
		) {
			return {
	...
	async signIn(options?: {
		...
		const defaultAccountScopes =
			this.defaultAccountConfig.authenticationProvider.scopes[0];
		const scopes = additionalScopes
			? distinct([...defaultAccountScopes, ...additionalScopes])
...
class DefaultAccountProviderContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		super();
		const defaultAccountProvider = this._register(
