...
export abstract class BaseConfigurationResolverService extends AbstractVariableResolverService {
		...
		super(
			{
				...
				getWorkspaceFolderPathForFile: (): string | undefined => {
					...
					const wsFolder =
						workspaceContextService.getWorkspaceFolder(fileResource);
					if (!wsFolder) {
						return undefined;
					}
	...
	override async resolveWithInteractionReplace(
		...
		const resolved = await this.resolveWithInteraction(
			folder,
			parsed,
			section,
			variables,
	...
	override async resolveWithInteraction(
		...
		for (const variable of expr.unresolved()) {
			...
			if (variable.name === "command") {
				const commandId =
					(variableToCommandMap
						? variableToCommandMap[variable.arg!]
			...
			// Contributed variable
				...
				result = {
					value: await this._contributedVariables.get(variable.inner)!(),
				};
			} else {
			...
			switch (info.type) {
				case "promptString": {
					...
					const inputOptions: IInputOptions = {
						...
						value:
							variableToCommandMap?.[`input:${variable}`] ??
							previousPickedValue ??
							info.default,
					};
					if (info.password) {
					...
					return this.userInputAccessQueue
						...
						.then((resolvedInput) => {
							if (typeof resolvedInput === "string" && !info.password) {
								this.storeInputLru(
									defaultValueMap.set(defaultValueKey, resolvedInput),
								);
							}
							return resolvedInput !== undefined
								? { value: resolvedInput as string, input: info }
								: undefined;
						});
				}
				...
				case "pickString": {
					...
					return this.userInputAccessQueue
						.queue(() =>
							this.quickInputService.pick(picks, pickOptions, undefined),
						)
						.then((resolvedInput) => {
							if (resolvedInput) {
