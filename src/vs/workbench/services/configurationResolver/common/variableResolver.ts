...
export abstract class AbstractVariableResolverService implements IConfigurationResolverService {
	...
	public async resolveAsync<T>(
		...
		return expr.toObject() as T extends ConfigurationResolverExpression<infer R>
			? R
			: T;
	}
		...
		const environment: Environment = {
			env:
				processEnvironment !== undefined
					? this.prepareEnv(processEnvironment)
		...
		switch (variable) {
			case "env":
				if (argument) {
					if (environment.env) {
						const env =
							environment.env[isWindows ? argument.toLowerCase() : argument];
			...
			default: {
				switch (variable) {
					...
					case "workspaceFolderBasename": {
						...
						return uri
							? normalizeDriveLetter(paths.basename(this.fsPath(uri)))
							: undefined;
					}
					...
					case "fileWorkspaceFolderBasename":
						return paths.basename(
							getFolderPathForFile(VariableKind.FileWorkspaceFolderBasename),
						);
					...
					case "fileDirnameBasename":
						return paths.basename(
							paths.dirname(getFilePath(VariableKind.FileDirnameBasename)),
						);
