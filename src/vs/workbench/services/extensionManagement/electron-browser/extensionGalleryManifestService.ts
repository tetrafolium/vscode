	...
	implements IExtensionGalleryManifestService
{
	...
	private currentStatus: ExtensionGalleryManifestStatus =
	...
	private async handleDefaultAccountAccess(
		...
		if (!account) {
			...
			this.logService.debug(
				"[Marketplace] User signed in but lacks access to enterprise marketplace",
			);
			...
			try {
				...
				this.logService.error(
					"[Marketplace] Error retrieving enterprise gallery manifest",
					error,
				);
				this.update(null, ExtensionGalleryManifestStatus.AccessDenied);
			}
	...
	private update(
		...
		status?: ExtensionGalleryManifestStatus,
	): void {
