...
} from "../../../chat/common/chatEntitlementService.js";
...
export class TestExtensionEnablementService extends ExtensionEnablementService {
	constructor(
		...
		const extensionManagementServerService =
			instantiationService.get(IExtensionManagementServerService) ||
				...
				anExtensionManagementServerService(
					{
						...
						>{
							...
							onDidChangeProfile: disposables.add(
								new Emitter<DidChangeProfileEvent>(),
							).event,
		...
		const workbenchExtensionManagementService =
			instantiationService.get(IWorkbenchExtensionManagementService) ||
			instantiationService.stub(
				IWorkbenchExtensionManagementService,
				extensionManagementService,
			);
		const workspaceTrustManagementService =
			instantiationService.get(IWorkspaceTrustManagementService) ||
			instantiationService.stub(
				IWorkspaceTrustManagementService,
		...
		super(
			...
			new (class extends mock<IExtensionBisectService>() {
				override isDisabledByBisect() {
					return false;
				}
			})(),
			instantiationService.stub(
	...
	public reset(): void {
		let extensions =
			this.globalExtensionEnablementService.getDisabledExtensions();
		...
		if (workspaceEnabledExtensions.length) {
			extensions = extensions.filter(
				(r) => !workspaceEnabledExtensions.some((e) => areSameExtensions(e, r)),
			);
		}
...
suite("ExtensionEnablementService Test", () => {
	...
	test("test disable an extension globally triggers the change event", async () => {
		...
		assert.deepStrictEqual((<IExtension>target.args[0][0][0]).identifier, {
			id: "pub.a",
		});
	});
	...
	test("test enable an extension with a dependency extension that cannot be enabled", async () => {
		instantiationService.stub(
			IExtensionManagementServerService,
			anExtensionManagementServerService(
				anExtensionManagementServer("vscode-local", instantiationService),
		...
		const remoteWorkspaceDepExtension = aLocalExtension2(
			"pub.b",
			{ extensionKind: ["workspace"] },
			{ location: URI.file(`pub.b`).with({ scheme: Schemas.vscodeRemote }) },
		);
		installed.push(
	...
	test("test canChangeEnablement return true for auth extension", () => {
		assert.strictEqual(
			testObject.canChangeEnablement(
				aLocalExtension("pub.a", { authentication: [{ id: "a", label: "a" }] }),
			),
			true,
		);
	});
	...
	test("test canChangeEnablement return true for auth extension when user data sync account does not depends on it", () => {
		instantiationService.stub(IUserDataSyncAccountService, <
			Partial<IUserDataSyncAccountService>
		>{
	...
	test("test extension is enabled globally when enabled in environment", async () => {
		...
		instantiationService.stub(IWorkbenchEnvironmentService, {
			enableExtensions: <readonly string[]>["pub.a"],
		});
		testObject = disposableStore.add(
			new TestExtensionEnablementService(instantiationService),
		);
	...
	test("test web extension from web extension management server and does not support vitrual workspace is enabled in virtual workspace", async () => {
		...
		const extension = aLocalExtension2(
			"pub.a",
			{ capabilities: { virtualWorkspaces: false }, browser: "browser.js" },
	...
	test("test extension does not support untrusted workspaces is disabled in untrusted workspace", () => {
		...
		instantiationService.stub(IWorkspaceTrustManagementService, <
			Partial<IWorkspaceTrustManagementService>
		>{
			isWorkspaceTrusted() {
				return false;
			},
	...
	test("test web extension on local server is disabled by kind when web worker is not enabled", async () => {
		...
		(<TestConfigurationService>(
			instantiationService.get(IConfigurationService)
		)).setUserConfiguration("extensions", { webWorker: false });
		testObject = disposableStore.add(
			new TestExtensionEnablementService(instantiationService),
		);
	...
	test("test web extension on local server is not disabled by kind when web worker is enabled", async () => {
		...
		(<TestConfigurationService>(
			instantiationService.get(IConfigurationService)
		)).setUserConfiguration("extensions", { webWorker: true });
		testObject = disposableStore.add(
			new TestExtensionEnablementService(instantiationService),
		);
	...
	test("test extension is not disabled when it has a missing dependency", async () => {
		const target = aLocalExtension2("pub.b", {
			extensionDependencies: ["pub.a"],
		});
	...
	test("test extension is not disabled when it has a dependency in another server", async () => {
		...
		const target = aLocalExtension2(
			"pub.a",
			{ extensionDependencies: ["pub.b"], extensionKind: ["ui"] },
			{ location: URI.file(`pub.a`) },
		);
		const depdencyOnAnotherServer = aLocalExtension2(
			"pub.b",
			{},
			{ location: URI.file(`pub.b`).with({ scheme: Schemas.vscodeRemote }) },
		);
	...
	test("test extension is disabled by dependency when it has a dependency in another server  which is disabled and with no exports and has browser entry point", async () => {
		...
		const depdencyOnAnotherServer = aLocalExtension2(
			"pub.b",
			{ api: "none", browser: "browser.js", extensionKind: "ui" },
			{ location: URI.file(`pub.b`) },
		);
		installed.push(...[target, depdencyOnAnotherServer]);
		testObject = disposableStore.add(
	...
	test("test extension is disabled by dependency when it has a dependency that is invalid", async () => {
		...
		installed.push(
			...[target, aLocalExtension2("pub.a", {}, { isValid: false })],
		);
		testObject = disposableStore.add(
			new TestExtensionEnablementService(instantiationService),
		);
	...
	test("test extension is enabled when its dependency becomes valid", async () => {
		const extension = aLocalExtension2("pub.b", {
			extensionDependencies: ["pub.a"],
		});
		installed.push(
			...[extension, aLocalExtension2("pub.a", {}, { isValid: false })],
		);
		testObject = disposableStore.add(
			new TestExtensionEnablementService(instantiationService),
		);
	...
	test("test update extensions enablements on trust change triggers change events for extensions depending on workspace trust", async () => {
		...
		assert.deepStrictEqual((<IExtension>target.args[0][0][1]).identifier, {
			id: "pub.c",
		});
	});
	...
	test("test chat extension is disabled on profile switch when setup is not completed", async () => {
		...
		storageService.store(
			"builtinChatExtensionEnablementMigration",
			false,
		...
		chatEntitlementService.context = new Lazy(() => ({
			state: { completed: false },
			onDidChange: Event.None,
		})) as unknown as Lazy<ChatEntitlementContext>;

		testObject = disposableStore.add(
			new TestExtensionEnablementService(
				instantiationService,
		...
		assert.strictEqual(
			testObject.getEnablementState(chatExtension),
			EnablementState.DisabledGlobally,
		);
	...
	test("test installed malicious extension triggers change event", async () => {
		...
		assert.strictEqual(
			testObject.getEnablementState(local),
			EnablementState.EnabledGlobally,
		);
	...
	test("test extensions are disabled in sessions window unless they only contribute themes", () => {
		...
		const themeOnly = aLocalExtension2("pub.themeOnly", {
			contributes: aContributes("themes"),
		});
		const iconTheme = aLocalExtension2("pub.iconTheme", {
			contributes: aContributes("iconThemes"),
		});
		const productIconTheme = aLocalExtension2("pub.productIconTheme", {
			contributes: aContributes("productIconThemes"),
		});
		const grammar = aLocalExtension2("pub.grammar", {
			contributes: aContributes("grammars"),
		});
		const withMain = aLocalExtension2("pub.withMain", {
			main: "main.js",
			contributes: aContributes("themes"),
		});
		const withBrowser = aLocalExtension2("pub.withBrowser", {
			browser: "main.browser.js",
			contributes: aContributes("themes"),
		});
		const nonThemeContrib = aLocalExtension2("pub.nonThemeContrib", {
			contributes: aContributes("commands"),
		});
		const builtinWithMain = aLocalExtension2(
			"pub.builtinWithMain",
			{ main: "main.js" },
	...
	test("test configured extensions are enabled in sessions window", async () => {
		...
		).setUserConfiguration(EXTENSIONS_SUPPORT_AGENTS_WINDOW, {
			"pub.withMain": true,
			"pub.nonThemeContrib": true,
		});
		instantiationService.stub(IWorkbenchEnvironmentService, {
			isSessionsWindow: true,
		});
		...
		assert.deepStrictEqual(
			[withMain, nonThemeContrib, withBrowser].map((ext) =>
				testObject.getEnablementState(ext),
			),
			[
				EnablementState.EnabledGlobally,
				EnablementState.EnabledGlobally,
	...
	test("test extensions are not disabled in non-sessions window", () => {
		...
		const commandContrib = aLocalExtension2("pub.commands", {
			contributes: aContributes("commands"),
		});

		assert.deepStrictEqual(
			[withMain, withBrowser, commandContrib].map((ext) =>
				testObject.getEnablementState(ext),
			),
			[
				EnablementState.EnabledGlobally,
...
function anExtensionManagementServer(
	authority: string,
	instantiationService: TestInstantiationService,
): IExtensionManagementServer {
	return {
...
export function anExtensionManagementServerService(
	localExtensionManagementServer: IExtensionManagementServer | null,
	remoteExtensionManagementServer: IExtensionManagementServer | null,
	webExtensionManagementServer: IExtensionManagementServer | null,
): IExtensionManagementServerService {
	return {
...
function aContributes(
	...points: Array<keyof IExtensionContributions>
): IExtensionContributions {
