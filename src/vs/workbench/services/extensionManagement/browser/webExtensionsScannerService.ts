	...
	implements IWebExtensionsScannerService
{
			...
			this._customBuiltinExtensionsInfoPromise = (async () => {
				...
				for (const e of customBuiltinExtensionsInfo) {
					if (isGalleryExtensionInfo(e)) {
						...
						if (
							await this.extensionResourceLoaderService.isExtensionGalleryResource(
								extensionLocation,
							)
						) {
	...
	private async checkAdditionalBuiltinExtensions(
		...
		for (const extension of extensions) {
			if (
				isMalicious({ id: extension.id }, extensionsControlManifest.malicious)
			) {
				this.logService.info(
			...
			const deprecationInfo =
				extensionsControlManifest.deprecated[extension.id.toLowerCase()];
	...
	private async updateCustomBuiltinExtensionsCache(): Promise<IWebExtension[]> {
		if (!this._updateCustomBuiltinExtensionsCachePromise) {
			this._updateCustomBuiltinExtensionsCachePromise = (async () => {
				...
				const { extensions, extensionGalleryResources } =
					await this.readCustomBuiltinExtensionsInfoFromEnv();
				const [galleryWebExtensions, extensionGalleryResourceWebExtensions] =
					await Promise.all([
	...
	private async resolveBuiltinGalleryExtensions(
		...
		const galleryExtensionsMap =
			await this.getExtensionsWithDependenciesAndPackedExtensions(extensions);
		...
		if (isSystem) {
			await this.writeSystemExtensionsCache((systemExtensions) => {
				...
				systemExtensions = systemExtensions.filter(
					(extension) =>
						!areSameExtensions(extension.identifier, webExtension.identifier),
				);
			...
			await this.extensionResourceLoaderService.getExtensionGalleryResourceURL(
				{
					...
					targetPlatform:
						galleryExtension.properties.targetPlatform === TargetPlatform.WEB
							? TargetPlatform.WEB
							: undefined,
				},
				"extension",
	...
	private async getExtensionManifest(
		...
		const content =
			await this.extensionResourceLoaderService.readExtensionResource(url);
	...
	private async getTranslations(
		...
		try {
			const content =
				await this.extensionResourceLoaderService.readExtensionResource(nlsUrl);
			return JSON.parse(content);
		} catch (error) {
			...
			try {
				...
				for (const e of storedWebExtensions) {
					...
					if (e.packageNLSUris) {
						...
						Object.entries(e.packageNLSUris).forEach(([key, value]) =>
