...
suite("HistoryService", function () {
	...
	async function createServices(
		scope = GoScope.DEFAULT,
		configureSearchExclude = false,
	): Promise<
		[
			EditorPart,
			HistoryService,
	...
	test("back / forward: in-editor text selection changes (user)", async function () {
		const [, historyService, editorService, , instantiationService] =
			await createServices();
	...
	test("back / forward: in-editor text selection changes (navigation)", async function () {
		...
		await setTextSelection(
			...
			EditorPaneSelectionChangeReason.NAVIGATION,
		); // this is our second target definition
		await setTextSelection(
			historyService,
			pane,
			new Selection(300, 3, 300, 20),
		); // unrelated user navigation
		await setTextSelection(
			historyService,
			pane,
	...
	async function setTextSelection(
		...
		pane: TestTextFileEditor,
		selection: Selection,
		reason = EditorPaneSelectionChangeReason.USER,
	): Promise<void> {
		const promise = Event.toPromise(
			(historyService as HistoryService).onDidChangeEditorNavigationStack,
		);
	...
	test("back / forward: editor navigation stack - mutations", async function () {
		...
		const unrelatedResource: URI = toResource.call(
			this,
			"/path/unrelated.html",
		);
		const pane = await editorService.openEditor({
	...
	test("back / forward: editor  scope", async function () {
		const [part, historyService, editorService, , instantiationService] =
			await createServices(GoScope.EDITOR);
	...
	test("getHistory", async () => {
		...
		const [part, historyService, editorService, , instantiationService] =
			await createServices(undefined, true);
		...
		assert.strictEqual(
			(history[0] as IResourceEditorInput).options?.override,
			"testOverride",
		);
	...
	test("transient editors suspends editor change tracking", async () => {
		...
		let editorChangePromise: Promise<void> = Event.toPromise(
			editorService.onDidActiveEditorChange,
		);
		await part.activeGroup.openEditor(input1, { pinned: true });
		assert.strictEqual(part.activeGroup.activeEditor, input1);
