...
suite("AccountPolicyService", () => {
	...
	setup(async () => {
		...
		defaultAccountService = disposables.add(
			new DefaultAccountService(TestProductService),
		);
	...
	async function assertDefaultBehavior(policyData: IPolicyData | undefined) {
		defaultAccountService.setDefaultAccountProvider(
			new DefaultAccountProvider(BASE_DEFAULT_ACCOUNT, policyData),
		);
	...
	class FakeManagedPolicyService
	...
	async function setupGate(opts: {
		...
		policyService: AccountPolicyService;
		managed: FakeManagedPolicyService;
	}> {
		...
		if (opts.approvedOrgs !== undefined) {
			...
			const value =
				typeof opts.approvedOrgs === "string"
					? opts.approvedOrgs
					: JSON.stringify(opts.approvedOrgs);
			managed.setPolicy(APPROVED_ACCOUNT_ORGANIZATIONS_POLICY_NAME, value);
		}

		const accountService = disposables.add(
			new DefaultAccountService(TestProductService),
		);
		if (opts.account !== null && opts.account !== undefined) {
			const policyData = opts.policyData === undefined ? {} : opts.policyData;
			accountService.setDefaultAccountProvider(
				new DefaultAccountProvider(opts.account, policyData),
			);
			await accountService.refresh();
		}

		const service = disposables.add(
			new AccountPolicyService(logService, accountService, managed),
		);
		const defaultConfiguration = disposables.add(
			new DefaultConfiguration(new NullLogService()),
		);
		await defaultConfiguration.initialize();
		const config = disposables.add(
			new PolicyConfiguration(
				defaultConfiguration,
				service,
				new NullLogService(),
	...
	test("gate inactive (no approved orgs set): behaves identically to today", async () => {
		...
		assert.strictEqual(
			policyService.gateInfo.state,
			AccountPolicyGateState.Inactive,
		);
	...
	test("gate active, no account signed in: restricted", async () => {
		const { policyService } = await setupGate({
			approvedOrgs: ["ApprovedOrg"],
			account: null,
		});
		assert.strictEqual(
			policyService.gateInfo.state,
			AccountPolicyGateState.Restricted,
		);
		assert.strictEqual(
			policyService.gateInfo.reason,
			AccountPolicyGateUnsatisfiedReason.NoAccount,
		);
		...
		assert.strictEqual(
			policyService.getPolicyValue("PolicySettingA"),
			undefined,
		);
	});
	...
	test("gate active, signed in but org not approved: restricted", async () => {
		const { policyService } = await setupGate({
			approvedOrgs: ["ApprovedOrg"],
			account: UNAPPROVED_ORG_ACCOUNT,
			policyData: {},
		});
		assert.strictEqual(
			policyService.gateInfo.state,
		...
		assert.strictEqual(
			policyService.gateInfo.reason,
			AccountPolicyGateUnsatisfiedReason.OrgNotApproved,
		);
	});
	...
	test("gate active, signed in with non-GitHub provider: WrongProvider reason", async () => {
		...
		managed.setPolicy(
			APPROVED_ACCOUNT_ORGANIZATIONS_POLICY_NAME,
			JSON.stringify(["ApprovedOrg"]),
		);
		const accountService = disposables.add(
			new DefaultAccountService(TestProductService),
		);
		accountService.setDefaultAccountProvider(
			new MismatchedProvider(NON_GITHUB_ACCOUNT, {}),
		);
		await accountService.refresh();
		const service = disposables.add(
			new AccountPolicyService(logService, accountService, managed),
		...
		assert.strictEqual(
			service.gateInfo.state,
			AccountPolicyGateState.Restricted,
		);
		assert.strictEqual(
			service.gateInfo.reason,
			AccountPolicyGateUnsatisfiedReason.WrongProvider,
		);
	});
	...
	test("explicit `restrictedValue` is honored when gate is restricted", async () => {
		...
		try {
			...
			assert.strictEqual(
				policyService.getPolicyValue("PolicySettingRV"),
				"locked",
			);
		} finally {
			Registry.as<IConfigurationRegistry>(
				Extensions.Configuration,
			).deregisterConfigurations([node]);
		}
	});

	test("onDidChangeGateInfo fires on state/reason transitions", async () => {
		const { policyService, managed } = await setupGate({
			approvedOrgs: ["ApprovedOrg"],
			account: APPROVED_ORG_ACCOUNT,
			policyData: {},
		});
		...
		disposables.add(
			policyService.onDidChangeGateInfo((info) => events.push(info)),
		);

		// Satisfied → Restricted (org no longer approved)
		managed.setPolicy(
			APPROVED_ACCOUNT_ORGANIZATIONS_POLICY_NAME,
			JSON.stringify(["OnlyOtherOrg"]),
		);
	...
	test("boot race: gate is fail-closed until async managed policy service resolves", async () => {
		...
		const managed = disposables.add(
			new AsyncManagedPolicyService(JSON.stringify(["OnlyOtherOrg"])),
		);
		const accountService = disposables.add(
			new DefaultAccountService(TestProductService),
		);
		accountService.setDefaultAccountProvider(
			new DefaultAccountProvider(APPROVED_ORG_ACCOUNT, {}),
		);
		await accountService.refresh();
	...
	test("managed policy change re-evaluates the gate and fires onDidChange", async () => {
		...
		disposables.add(
			policyService.onDidChange((names) => changes.push(...names)),
		);
		...
		assert.ok(
			changes.length > 0,
			"expected onDidChange to fire when gate flips",
		);
	});
});
