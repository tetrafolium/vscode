 ...
 */
export const APPROVED_ACCOUNT_ORGANIZATIONS_POLICY_NAME =
 ...
 * gate decision.
 */
export const IAccountPolicyGateService =
	createDecorator<IAccountPolicyGateService>("accountPolicyGateService");
...
export class AccountPolicyService
	...
	implements IPolicyService, IAccountPolicyGateService
{
	declare readonly _serviceBrand: undefined;

	private _gateInfo: IAccountPolicyGateInfo = {
		state: AccountPolicyGateState.Inactive,
	};
	get gateInfo(): IAccountPolicyGateInfo {
		return this._gateInfo;
	}

	private readonly _onDidChangeGateInfo = this._register(
		new Emitter<IAccountPolicyGateInfo>(),
	);
	...
	constructor(
		...
		this._register(
			this.defaultAccountService.onDidChangePolicyData(() => {
				this._updatePolicyDefinitions(this.policyDefinitions);
			}),
		);
		this._register(
		...
		if (this.managedPolicyReader) {
			this._register(
				this.managedPolicyReader.onDidChange((names) => {
	...
	protected async _updatePolicyDefinitions(
		...
		const previousApprovedOrgs =
			previousInfo.approvedOrganizations?.join("\n") ?? "";
		const currentApprovedOrgs =
			this._gateInfo.approvedOrganizations?.join("\n") ?? "";
		const gateInfoChanged =
			previousInfo.state !== this._gateInfo.state ||
		...
		const gateRestricted =
			this._gateInfo.state === AccountPolicyGateState.Restricted &&
			this._gateInfo.reason !==
		...
		for (const key in policyDefinitions) {
			...
			if (
				gateRestricted &&
				(policy.value !== undefined || policy.restrictedValue !== undefined)
			) {
	...
	private computeGateInfo(): IAccountPolicyGateInfo {
		...
		const approvedRaw = this.managedPolicyReader.getPolicyValue(
		...
		if (!account) {
			return {
				...
				reason: AccountPolicyGateUnsatisfiedReason.NoAccount,
		...
		if (account.authenticationProvider.id !== configuredProvider.id) {
			return {
				state: AccountPolicyGateState.Restricted,
				reason: AccountPolicyGateUnsatisfiedReason.WrongProvider,
				approvedOrganizations: approvedOrgs,
			};
		}
		...
		if (!approvedOrgs.includes("*")) {
			const accountOrgs = (
				account.entitlementsData?.organization_login_list ?? []
			).map((o) => o.toLowerCase());
