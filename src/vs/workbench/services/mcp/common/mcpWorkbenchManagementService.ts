...
import {
	...
	ILocalMcpServerInfo,
} from "../../../../platform/mcp/common/mcpManagementService.js";
...
export const IWorkbenchMcpManagementService = refineServiceDecorator<
	IMcpManagementService,
...
export interface IWorkbenchMcpManagementService extends IMcpManagementService {
	...
	readonly onDidInstallMcpServersInCurrentProfile: Event<
		readonly IWorkbenchMcpServerInstallResult[]
	>;
	readonly onDidUpdateMcpServersInCurrentProfile: Event<
		readonly IWorkbenchMcpServerInstallResult[]
	>;
	...
	install(
		server: IInstallableMcpServer | URI,
		options?: IWorkbencMcpServerInstallOptions,
	): Promise<IWorkbenchLocalMcpServer>;
	installFromGallery(
...
export class WorkbenchMcpManagementService
	extends AbstractMcpManagementService
	implements IWorkbenchMcpManagementService
{
	private _onInstallMcpServer = this._register(
		new Emitter<InstallMcpServerEvent>(),
	);
	readonly onInstallMcpServer = this._onInstallMcpServer.event;

	private _onDidInstallMcpServers = this._register(
		new Emitter<readonly InstallMcpServerResult[]>(),
	);
	readonly onDidInstallMcpServers = this._onDidInstallMcpServers.event;

	private _onDidUpdateMcpServers = this._register(
		new Emitter<readonly InstallMcpServerResult[]>(),
	);
	readonly onDidUpdateMcpServers = this._onDidUpdateMcpServers.event;

	private _onUninstallMcpServer = this._register(
		new Emitter<UninstallMcpServerEvent>(),
	);
	readonly onUninstallMcpServer = this._onUninstallMcpServer.event;

	private _onDidUninstallMcpServer = this._register(
		new Emitter<DidUninstallMcpServerEvent>(),
	);
	readonly onDidUninstallMcpServer = this._onDidUninstallMcpServer.event;

	private readonly _onInstallMcpServerInCurrentProfile = this._register(
		new Emitter<InstallWorkbenchMcpServerEvent>(),
	);
	readonly onInstallMcpServerInCurrentProfile =
		this._onInstallMcpServerInCurrentProfile.event;

	private readonly _onDidInstallMcpServersInCurrentProfile = this._register(
		new Emitter<readonly IWorkbenchMcpServerInstallResult[]>(),
	);
	readonly onDidInstallMcpServersInCurrentProfile =
		this._onDidInstallMcpServersInCurrentProfile.event;

	private readonly _onDidUpdateMcpServersInCurrentProfile = this._register(
		new Emitter<readonly IWorkbenchMcpServerInstallResult[]>(),
	);
	readonly onDidUpdateMcpServersInCurrentProfile =
		this._onDidUpdateMcpServersInCurrentProfile.event;

	private readonly _onUninstallMcpServerInCurrentProfile = this._register(
		new Emitter<UninstallWorkbenchMcpServerEvent>(),
	);
	readonly onUninstallMcpServerInCurrentProfile =
		this._onUninstallMcpServerInCurrentProfile.event;

	private readonly _onDidUninstallMcpServerInCurrentProfile = this._register(
		new Emitter<DidUninstallWorkbenchMcpServerEvent>(),
	);
	readonly onDidUninstallMcpServerInCurrentProfile =
		this._onDidUninstallMcpServerInCurrentProfile.event;
	...
	constructor(
		...
		this.workspaceMcpManagementService = this._register(
			instantiationService.createInstance(WorkspaceMcpManagementService),
		);
		const remoteAgentConnection = remoteAgentService.getConnection();
		if (remoteAgentConnection) {
			this.remoteMcpManagementService = this._register(
				instantiationService.createInstance(
					McpManagementChannelClient,
					remoteAgentConnection.getChannel<IChannel>("mcpManagement"),
				),
			);
		}

		this._register(
			this.mcpManagementService.onInstallMcpServer((e) => {
		...
		this._register(
			this.mcpManagementService.onDidInstallMcpServers((e) => {
				const {
					mcpServerInstallResult,
					mcpServerInstallResultInCurrentProfile,
				} = this.createInstallMcpServerResultsFromEvent(
					e,
				...
				if (mcpServerInstallResultInCurrentProfile.length) {
					this._onDidInstallMcpServersInCurrentProfile.fire(
						mcpServerInstallResultInCurrentProfile,
					);
				}
			}),
		...
		this._register(
			this.mcpManagementService.onDidUpdateMcpServers((e) => {
				const {
					mcpServerInstallResult,
					mcpServerInstallResultInCurrentProfile,
				} = this.createInstallMcpServerResultsFromEvent(
				...
				if (mcpServerInstallResultInCurrentProfile.length) {
					this._onDidUpdateMcpServersInCurrentProfile.fire(
						mcpServerInstallResultInCurrentProfile,
					);
				}
			}),
		...
		this._register(
			this.mcpManagementService.onUninstallMcpServer((e) => {
		...
		this._register(
			this.mcpManagementService.onDidUninstallMcpServer((e) => {
		...
		this._register(
			this.workspaceMcpManagementService.onInstallMcpServer(async (e) => {
				this._onInstallMcpServer.fire(e);
				this._onInstallMcpServerInCurrentProfile.fire({
					...e,
		...
		this._register(
			this.workspaceMcpManagementService.onDidInstallMcpServers(async (e) => {
		...
		this._register(
			this.workspaceMcpManagementService.onUninstallMcpServer(async (e) => {
				this._onUninstallMcpServer.fire(e);
				this._onUninstallMcpServerInCurrentProfile.fire({
					...e,
					scope: LocalMcpServerScope.Workspace,
		...
		this._register(
			this.workspaceMcpManagementService.onDidUninstallMcpServer(async (e) => {
				this._onDidUninstallMcpServer.fire(e);
				this._onDidUninstallMcpServerInCurrentProfile.fire({
					...e,
					scope: LocalMcpServerScope.Workspace,
		...
		this._register(
			this.workspaceMcpManagementService.onDidUpdateMcpServers((e) => {
				const { mcpServerInstallResult } =
					this.createInstallMcpServerResultsFromEvent(
						e,
						LocalMcpServerScope.Workspace,
		...
		if (this.remoteMcpManagementService) {
			this._register(
				this.remoteMcpManagementService.onInstallMcpServer(async (e) => {
					this._onInstallMcpServer.fire(e);
					const remoteMcpResource = await this.getRemoteMcpResource(
						this.userDataProfileService.currentProfile.mcpResource,
					);
					if (
						remoteMcpResource
			...
			this._register(
				this.remoteMcpManagementService.onDidInstallMcpServers((e) =>
					this.handleRemoteInstallMcpServerResultsFromEvent(
						e,
			...
			this._register(
				this.remoteMcpManagementService.onDidUpdateMcpServers((e) =>
					this.handleRemoteInstallMcpServerResultsFromEvent(
						e,
						this._onDidInstallMcpServers,
						this._onDidInstallMcpServersInCurrentProfile,
			...
			this._register(
				this.remoteMcpManagementService.onUninstallMcpServer(async (e) => {
					this._onUninstallMcpServer.fire(e);
					const remoteMcpResource = await this.getRemoteMcpResource(
						this.userDataProfileService.currentProfile.mcpResource,
					);
			...
			this._register(
				this.remoteMcpManagementService.onDidUninstallMcpServer(async (e) => {
					this._onDidUninstallMcpServer.fire(e);
					const remoteMcpResource = await this.getRemoteMcpResource(
						this.userDataProfileService.currentProfile.mcpResource,
					);
			...
			userDataProfileService.onDidChangeCurrentProfile((e) => {
				if (
					!this.uriIdentityService.extUri.isEqual(
						e.previous.mcpResource,
						e.profile.mcpResource,
					)
				) {
	...
	private createInstallMcpServerResultsFromEvent(
		e: readonly InstallMcpServerResult[],
		scope: LocalMcpServerScope,
	): {
		mcpServerInstallResult: IWorkbenchMcpServerInstallResult[];
		mcpServerInstallResultInCurrentProfile: IWorkbenchMcpServerInstallResult[];
	} {
		...
		for (const result of e) {
			const workbenchResult = {
				...result,
				local: result.local
					? this.toWorkspaceMcpServer(result.local, scope)
					: undefined,
			};
	...
	private async handleRemoteInstallMcpServerResultsFromEvent(
		e: readonly InstallMcpServerResult[],
		emitter: Emitter<readonly InstallMcpServerResult[]>,
		currentProfileEmitter: Emitter<readonly IWorkbenchMcpServerInstallResult[]>,
	): Promise<void> {
		const mcpServerInstallResult: IWorkbenchMcpServerInstallResult[] = [];
		const mcpServerInstallResultInCurrentProfile: IWorkbenchMcpServerInstallResult[] =
			[];
	...
	async getInstalled(): Promise<IWorkbenchLocalMcpServer[]> {
		...
		const [userServers, remoteServers, workspaceServers] = await Promise.all([
			this.mcpManagementService.getInstalled(
				this.userDataProfileService.currentProfile.mcpResource,
			),
			this.remoteMcpManagementService?.getInstalled(
				await this.getRemoteMcpResource(),
			) ?? Promise.resolve<ILocalMcpServer[]>([]),
			this.workspaceMcpManagementService?.getInstalled() ??
				Promise.resolve<ILocalMcpServer[]>([]),
		]);

		for (const server of userServers) {
			installed.push(
				this.toWorkspaceMcpServer(server, LocalMcpServerScope.User),
			);
		}
		for (const server of remoteServers) {
			installed.push(
				this.toWorkspaceMcpServer(server, LocalMcpServerScope.RemoteUser),
			);
		}
		for (const server of workspaceServers) {
			installed.push(
				this.toWorkspaceMcpServer(server, LocalMcpServerScope.Workspace),
			);
		}
	...
	async install(
		...
		if (
			options.target === ConfigurationTarget.WORKSPACE ||
			isWorkspaceFolder(options.target)
		) {
			const mcpResource =
				options.target === ConfigurationTarget.WORKSPACE
					? this.workspaceContextService.getWorkspace().configuration
					: options.target.toResource(
							WORKSPACE_STANDALONE_CONFIGURATIONS[MCP_CONFIGURATION_KEY],
						);
			if (!mcpResource) {
			...
			const result = await this.workspaceMcpManagementService.install(
				server,
				options,
			);
		...
		if (options.target === ConfigurationTarget.USER_REMOTE) {
			...
			const result = await this.remoteMcpManagementService.install(
				server,
				options,
			);
	...
	async installFromGallery(
		...
		if (
			...
			options.mcpResource = mcpResource;
			const result =
				await this.workspaceMcpManagementService.installFromGallery(
					server,
					options,
				);
			return this.toWorkspaceMcpServer(result, LocalMcpServerScope.Workspace);
		...
		if (options.target === ConfigurationTarget.USER_REMOTE) {
			...
			const result = await this.remoteMcpManagementService.installFromGallery(
				server,
				options,
			);
			return this.toWorkspaceMcpServer(result, LocalMcpServerScope.RemoteUser);
		...
		const result = await this.mcpManagementService.installFromGallery(
			server,
			options,
		);
		return this.toWorkspaceMcpServer(result, LocalMcpServerScope.User);
	...
	async updateMetadata(
		local: IWorkbenchLocalMcpServer,
		server: IGalleryMcpServer,
		profileLocation: URI,
	): Promise<IWorkbenchLocalMcpServer> {
		if (local.scope === LocalMcpServerScope.Workspace) {
			const result = await this.workspaceMcpManagementService.updateMetadata(
				local,
				server,
				profileLocation,
			);
		...
		if (local.scope === LocalMcpServerScope.RemoteUser) {
			...
			const result = await this.remoteMcpManagementService.updateMetadata(
				local,
				server,
				profileLocation,
			);
		...
		const result = await this.mcpManagementService.updateMetadata(
			local,
			server,
			profileLocation,
		);
	...
	private async getRemoteMcpResource(
		mcpResource?: URI,
		...
		let profile = this.userDataProfilesService.profiles.find((p) =>
			this.uriIdentityService.extUri.isEqual(p.mcpResource, mcpResource),
		);
		if (profile) {
			profile =
...
class WorkspaceMcpResourceManagementService extends AbstractMcpResourceManagementService {
	...
	override async installFromGallery(
		...
		try {
			const packageType =
				options?.packageType ??
				server.configuration.packages?.[0]?.registryType ??
				RegistryType.REMOTE;

			const { mcpServerConfiguration, notices } =
				this.getMcpServerConfigurationFromManifest(
	...
	protected override async getLocalServerInfo(
		name: string,
		mcpServerConfig: IMcpServerConfiguration,
	): Promise<ILocalMcpServerInfo | undefined> {
...
class WorkspaceMcpManagementService
	extends AbstractMcpManagementService
	implements IMcpManagementService
{
	...
	private readonly workspaceMcpManagementServices = new ResourceMap<
		{ service: WorkspaceMcpResourceManagementService } & IDisposable
	>();
	...
	private async initialize(): Promise<void> {
		try {
			...
			this._register(
				this.workspaceContextService.onDidChangeWorkbenchState((e) =>
	...
	private async onDidChangeWorkspaceFolders(
		e: IWorkspaceFoldersChangeEvent,
	): Promise<void> {
		try {
			await Promise.allSettled(
				e.removed.map((folder) =>
					this.removeWorkspaceService(
						folder.toResource(
							WORKSPACE_STANDALONE_CONFIGURATIONS[MCP_CONFIGURATION_KEY],
						),
					),
	...
	private async addWorkspaceService(
		mcpResource: URI,
		target: McpResourceTarget,
	): Promise<void> {
		...
		try {
			...
			if (installedServers.length > 0) {
				const installResults: InstallMcpServerResult[] = installedServers.map(
					(server) => ({
						name: server.name,
						local: server,
			...
			this.logService.warn(
				"Failed to get installed servers from",
				mcpResource.toString(),
				error,
			);
		}

		disposables.add(
			service.onInstallMcpServer((e) => this._onInstallMcpServer.fire(e)),
		);
		disposables.add(
			service.onDidInstallMcpServers((e) => {
		...
		disposables.add(
			service.onDidUpdateMcpServers((e) => {
		...
		disposables.add(
			service.onUninstallMcpServer((e) => this._onUninstallMcpServer.fire(e)),
		);
		disposables.add(
	...
	private async removeWorkspaceService(mcpResource: URI): Promise<void> {
		...
		if (serviceItem) {
			try {
				...
				this.allMcpServers = this.allMcpServers.filter(
					(server) =>
						!installedServers.some((uninstalled) =>
							this.uriIdentityService.extUri.isEqual(
								uninstalled.mcpResource,
								server.mcpResource,
							),
						),
				);
	...
	async install(
		...
		const mcpManagementServiceItem = this.workspaceMcpManagementServices.get(
			options?.mcpResource,
		);
	...
	async uninstall(
		...
		const mcpManagementServiceItem =
			this.workspaceMcpManagementServices.get(mcpResource);
		if (!mcpManagementServiceItem) {
			throw new Error(
				`No MCP management service found for resource: ${mcpResource.toString()}`,
			);
		}
	...
	installFromGallery(
		...
		return mcpManagementServiceItem.service.installFromGallery(
			gallery,
			options,
		);
	}
