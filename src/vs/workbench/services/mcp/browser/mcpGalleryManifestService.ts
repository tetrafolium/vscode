...
export class WorkbenchMcpGalleryManifestService
	extends McpGalleryManifestService
	implements IMcpGalleryManifestService
{
	...
	override readonly onDidChangeMcpGalleryManifest =
		this._onDidChangeMcpGalleryManifest.event;

	private currentStatus: McpGalleryManifestStatus =
		McpGalleryManifestStatus.Unavailable;
	override get mcpGalleryManifestStatus(): McpGalleryManifestStatus {
		return this.currentStatus;
	}
	private _onDidChangeMcpGalleryManifestStatus = this._register(
		new Emitter<McpGalleryManifestStatus>(),
	);
	override readonly onDidChangeMcpGalleryManifestStatus =
		this._onDidChangeMcpGalleryManifestStatus.event;
	...
	constructor(
		...
		if (remoteConnection) {
			...
			this.getMcpGalleryManifest().then((manifest) => {
				...
				this._register(
					this.onDidChangeMcpGalleryManifest((manifest) =>
						channel.call("setMcpGalleryManifest", [manifest]),
					),
				);
			});
	...
	private async doGetMcpGalleryManifest(): Promise<void> {
		...
		this._register(
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(mcpGalleryServiceUrlConfig) ||
					e.affectsConfiguration("chat.mcp.gallery.version")
				) {
	...
	private async getAndUpdateMcpGalleryManifest(): Promise<void> {
		const mcpGalleryConfig = this.configurationService.getValue<
			IMcpGalleryConfig | undefined
		>("chat.mcp.gallery");
		if (mcpGalleryConfig?.serviceUrl) {
			this.update(
				await this.createMcpGalleryManifest(
					mcpGalleryConfig.serviceUrl,
					mcpGalleryConfig.version,
				),
			);
		} else {
	...
	private update(manifest: IMcpGalleryManifest | null): void {
		if (
			this.mcpGalleryManifest?.url === manifest?.url &&
			this.mcpGalleryManifest?.version === manifest?.version
		) {
			return;
		}
		...
		this.currentStatus = this.mcpGalleryManifest
