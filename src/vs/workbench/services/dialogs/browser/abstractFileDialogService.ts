...
export abstract class AbstractFileDialogService implements IFileDialogService {
	...
	async defaultFilePath(
		...
		if (!candidate) {
			...
			this.logService.debug(
				`[FileDialogService] Default file path using preferred home: ${candidate}`,
			);
		}
	...
	async preferredHome(
		...
		this.logService.debug(
			`[FileDialogService] Preferred home using user home: ${userHome}`,
		);
		return userHome;
	}
	...
	private skipDialogs(): boolean {
		if (this.environmentService.enableSmokeTestDriver) {
			this.logService.warn(
				"DialogService: Dialog requested during smoke test.",
			);
		}
		// integration tests
		return (
	...
	private async isRemoteUserData(resource: URI): Promise<boolean> {
		...
		if (remoteEnv) {
			const remoteDataHome = resources.dirname(
				resources.dirname(remoteEnv.settingsPath),
			);
			if (
				!resources.isEqual(
					remoteDataHome,
					remoteDataHome.with({ path: "/" }),
				) &&
