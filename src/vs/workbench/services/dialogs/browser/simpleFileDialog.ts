...
export class SimpleFileDialog extends Disposable implements ISimpleFileDialog {
	...
	public async showOpenDialog(
		...
		this.scopedPathPrefix =
			options.defaultUri && this.scopedAuthority
				? this.computeScopedPathPrefix(options.defaultUri)
				: "";
	...
	private async pickResource(
		...
		this.separator = this.scopedAuthority
			? "/"
			: this.labelService.getSeparator(this.scheme, this.remoteAuthority);
		this.hidden = false;
		this.isWindows = this.scopedAuthority
			? false
			: await this.checkIsWindowsOS();
		...
		return new Promise<URI[] | URI | undefined>((resolve) => {
			...
			this.filePickBox.placeholder = nls.localize(
			...
			const handleAccept = () => {
				if (this.busy) {
					...
					busyDisposable.value = this.onBusyChangeEmitter.event(
						(busy: boolean) => {
							if (!busy) {
								handleAccept();
							}
	...
	private async createBackItem(
		...
		if (this.scopedPathPrefix) {
			const pathAfterPrefix = currFolder.path.substring(
				this.scopedPathPrefix.length,
			);
