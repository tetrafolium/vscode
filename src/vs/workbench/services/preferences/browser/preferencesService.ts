...
} from "../common/preferencesEditorInput.js";
import {
	...
	implements IPreferencesService
{
	...
	private _cachedSettingsEditor2Input: SettingsEditor2Input | undefined =
		undefined;
	...
	private createOrGetCachedSettingsEditor2Input(): SettingsEditor2Input {
		if (
			!this._cachedSettingsEditor2Input ||
			this._cachedSettingsEditor2Input.isDisposed()
		) {
	...
	private open(
		...
		if (options.jsonEditor && options.query && !options.revealSetting) {
			...
			if (idMatch) {
				...
				).getConfigurationProperties()[query.trim()]
			) {
				key = query.trim();
			}
			options.query = undefined;
	...
	async openGlobalKeybindingSettings(
		...
		if (textual) {
			...
			if (openDefaultKeybindings) {
				const sourceGroupId =
					options.groupId ?? this.editorGroupService.activeGroup.id;
	...
	private getEditorGroupFromOptions(options: {
		groupId?: number;
		openToSide?: boolean;
	}): PreferredGroup {
		if (
			...
			) !== "off" && // modal editors enabled in settings
		...
		if (options?.groupId !== undefined) {
			return (
				this.editorGroupService.getGroup(options.groupId) ??
	...
	private async getPositionToReveal(
		...
		if (!setting && edit) {
			...
			defaultValue =
				defaultValue === undefined && isOverrideProperty ? {} : defaultValue;
			...
			if (edit) {
				if (isObject(setting.value) || Array.isArray(setting.value)) {
					...
					await this.instantiationService.invokeFunction((accessor) => {
						return CoreEditingCommands.LineBreakInsert.runEditorCommand(
							accessor,
							codeEditor,
							null,
						);
