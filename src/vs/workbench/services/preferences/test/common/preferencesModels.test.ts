...
suite("DefaultSettings", () => {
	...
	setup(() => {
		configurationRegistry = Registry.as<IConfigurationRegistry>(
	...
	test("groups settings by title when they share the same extension id", () => {
		...
		disposables.add(
			toDisposable(() =>
				configurationRegistry.deregisterConfigurations([config1, config2]),
			),
		);

		const defaultSettings = disposables.add(
			new DefaultSettings([], ConfigurationTarget.USER, configurationService),
		);
		const groups = defaultSettings.getRegisteredGroups();

		const extensionGroups = groups.filter(
		...
		assert.strictEqual(
			extensionGroups[0].sections[0].settings[0].key,
			"test.setting1",
		);

		assert.strictEqual(extensionGroups[1].sections[0].settings.length, 1);
		assert.strictEqual(
			extensionGroups[1].sections[0].settings[0].key,
			"test.setting2",
		);
	});
