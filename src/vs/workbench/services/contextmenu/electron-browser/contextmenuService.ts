	...
	constructor(
		...
		if (isMacintosh) {
			this.listener = configurationService.onDidChangeConfiguration((e) => {
				...
				const newIsNativeContextMenu =
					hasNativeContextMenu(configurationService);
	...
	implements IContextMenuService
{
	...
	showContextMenu(
		...
		if (actions.length) {
					...
					if (delegate.anchorAxisAlignment === AnchorAxisAlignment.HORIZONTAL) {
						...
						if (!isMacintosh) {
							...
							if (
								...
								actions.length *
									(isWindows ? 45 : 32) /* guess of 1 menu item height */
							) {
			...
			popup(
				menu,
				{ x, y, positioningItem: delegate.autoSelectFirstItem ? 0 : undefined },
				() => onHide(),
			);
	...
	private createMenu(
		...
		return coalesce(
			entries.map((entry) =>
				this.createMenuItem(delegate, entry, onHide, submenuIds),
			),
		);
	}
	...
	private async runAction(
		...
		if (!delegate.skipTelemetry) {
			this.telemetryService.publicLog2<
				WorkbenchActionExecutedEvent,
				WorkbenchActionExecutedClassification
			>("workbenchActionExecuted", { id: actionToRun.id, from: "contextMenu" });
		}
