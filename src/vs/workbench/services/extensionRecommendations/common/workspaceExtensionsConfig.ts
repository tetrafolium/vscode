	...
	implements IWorkspaceExtensionsConfigService
{
	...
	async getRecommendations(): Promise<string[]> {
		...
		return distinct(
			configs.flatMap((c) =>
				c.recommendations ? c.recommendations.map((c) => c.toLowerCase()) : [],
			),
		);
	}
		...
		await Promise.all(
			workspace.folders.map(async (workspaceFolder) => {
				const extensionsConfigContent =
					await this.resolveWorkspaceFolderExtensionConfig(workspaceFolder);
	...
	private parseExtensionConfig(
		...
		return {
			...
			unwantedRecommendations: distinct(
				(extensionsConfigContent.unwantedRecommendations || []).map((e) =>
					e.toLowerCase(),
				),
			),
		};
