...
export const IInlineCompletionsUnificationService =
	createDecorator<IInlineCompletionsUnificationService>(
...
export const isRunningUnificationExperiment = new RawContextKey<boolean>(
...
export class InlineCompletionsUnificationImpl
	extends Disposable
	implements IInlineCompletionsUnificationService
{
	readonly _serviceBrand: undefined;

	private _state = new InlineCompletionsUnificationState(
		false,
		false,
		false,
		[],
	);
	public get state(): IInlineCompletionsUnificationState {
		return this._state;
	}
	...
	private readonly _onDidChangeExtensionUnificationState = this._register(
		new Emitter<void>(),
	);
	private readonly _onDidChangeExtensionUnificationSetting = this._register(
		new Emitter<void>(),
	);
	...
	constructor(
		...
		this._completionsExtensionId =
			productService.defaultChatAgent?.extensionId.toLowerCase();
		this._chatExtensionId =
			productService.defaultChatAgent?.chatExtensionId.toLowerCase();
		const relevantExtensions = [
			this._completionsExtensionId,
			this._chatExtensionId,
		].filter((id): id is string => !!id);

		this.isRunningUnificationExperiment = isRunningUnificationExperiment.bindTo(
			this._contextKeyService,
		);

		this._assignmentService.addTelemetryAssignmentFilter({
			exclude: (assignment) =>
				assignment.startsWith(EXTENSION_UNIFICATION_PREFIX) &&
				this._state.extensionUnification !==
					this._configurationService.getValue<boolean>(
						ExtensionUnificationSetting,
					),
		...
		this._register(
			this._extensionEnablementService.onEnablementChanged((extensions) => {
				if (
					extensions.some((ext) =>
						relevantExtensions.includes(ext.identifier.id.toLowerCase()),
					)
				) {
					this._update();
				}
		...
		this._register(
			this._extensionService.onDidChangeExtensions(({ added }) => {
				if (
					added.some((ext) =>
						relevantExtensions.includes(ext.identifier.value.toLowerCase()),
					)
				) {
					this._update();
				}
		...
		this._register(
			this._assignmentService.onDidRefetchAssignments(() => this._update()),
		);
		this._update();
	}

	private async _update(): Promise<void> {
		const [codeUnificationFF, modelUnificationFF, extensionUnificationEnabled] =
			await Promise.all([
		...
		const extensionStatesMatchUnificationSetting =
			this._configurationService.getValue<boolean>(
				ExtensionUnificationSetting,
		...
		const currentExperiments =
			await this._assignmentService.getCurrentExperiments();
		const newState = new InlineCompletionsUnificationState(
			...
			currentExperiments?.filter(
				(exp) =>
					exp.startsWith(CODE_UNIFICATION_PREFIX) ||
					(extensionStatesMatchUnificationSetting &&
						exp.startsWith(EXTENSION_UNIFICATION_PREFIX)),
			) ?? [],
		);
		...
		this.isRunningUnificationExperiment.set(
			this._state.codeUnification ||
				this._state.modelUnification ||
				this._state.extensionUnification,
		);
		this._onDidStateChange.fire();

		if (
			previousState.extensionUnification !== this._state.extensionUnification
		) {
	...
	private async _isExtensionUnificationActive(): Promise<boolean> {
		if (
			!this._configurationService.getValue<boolean>(ExtensionUnificationSetting)
		) {
			return false;
		}
		...
		const [completionsExtension, chatExtension, installedExtensions] =
			await Promise.all([
		...
		const completionExtensionInstalled = installedExtensions.filter(
			(ext) => ext.identifier.id.toLowerCase() === this._completionsExtensionId,
		);
		if (completionExtensionInstalled.length === 0) {
			return true;
		}

		const completionsExtensionDisabledByUnification =
			completionExtensionInstalled.some(
				(ext) =>
					this._extensionEnablementService.getEnablementState(ext) ===
...
class InlineCompletionsUnificationState implements IInlineCompletionsUnificationState {
	...
	equals(other: IInlineCompletionsUnificationState): boolean {
		return (
			this.codeUnification === other.codeUnification &&
