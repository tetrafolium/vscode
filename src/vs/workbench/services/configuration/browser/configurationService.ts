...
function getLocalUserConfigurationScopes(
	...
	const isDefaultProfile =
		userDataProfile.isDefault || userDataProfile.useDefaultFlags?.settings;
	if (isDefaultProfile) {
	...
	implements IWorkbenchConfigurationService, IWorkspaceContextService
{
	...
	private cachedFolderConfigs: DisposableMap<URI, FolderConfiguration> =
		this._register(new DisposableMap(new ResourceMap()));
	...
	constructor(
		{
			remoteAuthority,
			configurationCache,
		}: { remoteAuthority?: string; configurationCache: IConfigurationCache },
	...
	async updateValue(
		...
		await Promises.settled(
			targets.map((target) =>
				this.writeConfigurationValue(key, value, target, overrides, options),
			),
		);
	}
	...
	private async updateWorkspaceAndInitializeConfiguration(
		...
		if (hasWorkspaceBefore) {
			...
			const newWorkspacePath = this.workspace.configuration
				...
				: undefined;
			if (
				(previousWorkspacePath && newWorkspacePath !== previousWorkspacePath) ||
	...
	private compareFolders(
		...
		result.added = newFolders.filter(
			(newFolder) =>
				!currentFolders.some(
					(currentFolder) =>
						newFolder.uri.toString() === currentFolder.uri.toString(),
				),
		);
		for (
			let currentIndex = 0;
			...
			for (
				...
				currentFolder.uri.toString() !== newFolders[newIndex].uri.toString();
				newIndex++
			) {}
			...
			if (scope && APPLICATION_SCOPES.includes(scope)) {
				...
				if (changedKey === APPLY_ALL_PROFILES_SETTING) {
					for (const previousAllProfileSetting of previousAllProfilesSettings) {
						if (
							!currentAllProfilesSettings.includes(previousAllProfileSetting)
						) {
					...
					for (const currentAllProfileSetting of currentAllProfilesSettings) {
						if (
							!previousAllProfilesSettings.includes(currentAllProfileSetting)
						) {
	...
	private updateRestrictedSettings(): void {
		...
		for (const workspaceFolder of this.workspace.folders) {
			...
			const previous =
				this._restrictedSettings.workspaceFolder?.get(workspaceFolder.uri) ||
				[];
	...
	private async updateWorkspaceConfiguration(
		...
		const change =
			this._configuration.compareAndUpdateWorkspaceConfiguration(configuration);
	...
	private async onFoldersChanged(): Promise<IConfigurationChange> {
		...
		for (const key of this.cachedFolderConfigs.keys()) {
			if (
				...
				changes.push(
					this._configuration.compareAndDeleteFolderConfiguration(key),
				);
			}
		}
	...
	private async writeConfigurationValue(
		...
		if (
			editableConfigurationTarget === EditableConfigurationTarget.USER_REMOTE &&
			!this.remoteUserConfiguration
		) {
			throw new Error("Invalid configuration target");
		}
	...
	private async createConfigurationEditingService(
		...
		const remoteSettingsResource =
			(await this.remoteAgentService.getEnvironment())?.settingsPath ?? null;
	...
	private registerConfigurationSchemas(): void {
		...
		for (const key of Object.keys(allSettings.properties)) {
			...
			if (
				...
				prop.deprecationMessage === prop.markdownDeprecationMessage
			) {
				prop.deprecationMessage = renderAsPlaintext({
					value: fixSettingLinks(prop.markdownDeprecationMessage),
				});
			}
		}
		...
		const defaultSettingsSchema = {
			properties: Object.keys(allSettings.properties).reduce<IJSONSchemaMap>(
				(result, key) => {
...
class ConfigurationDefaultOverridesContribution
	extends Disposable
	implements IWorkbenchContribution
{
	static readonly ID =
		"workbench.contrib.configurationDefaultOverridesContribution";
	...
	constructor(
		...
		@IConfigurationService
		private readonly configurationService: WorkspaceService,
		...
		this._register(
			workbenchAssignmentService.onDidRefetchAssignments(() =>
				this.throttler.queue(() =>
					this.processExperimentalSettings(this.autoExperimentalSettings, true),
				),
			),
		);
	...
	private async updateDefaults(): Promise<void> {
		...
		try {
			...
			this.logService.trace(
				"ConfigurationService#updateDefaults: resetting the defaults",
			);
	...
	private async processExperimentalSettings(
		...
		const allProperties =
			this.configurationRegistry.getConfigurationProperties();
		const defaultConfigurationsPreventingExperimentOverrides =
