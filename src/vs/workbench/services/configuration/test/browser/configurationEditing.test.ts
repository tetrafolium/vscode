...
suite("ConfigurationEditing", () => {
	...
	test("errors cases - ERROR_POLICY_CONFIGURATION", async () => {
		...
		try {
			await testObject.writeConfiguration(
				EditableConfigurationTarget.USER_LOCAL,
				{ key: "configurationEditing.service.policySetting", value: "value" },
				{ donotNotifyError: true },
			);
		} catch (error) {
			assert.strictEqual(
	...
	test("remove an existing setting - existing file", async () => {
		...
		await testObject.writeConfiguration(
			EditableConfigurationTarget.USER_LOCAL,
			{ key: "configurationEditing.service.testSetting", value: undefined },
		);
	...
	test("write overridable settings to workspace settings", async () => {
		...
		await testObject.writeConfiguration(EditableConfigurationTarget.WORKSPACE, {
			key,
			value,
		});
	...
	test("write user standalone mcp setting - existing file", async () => {
		const target = joinPath(
			environmentService.userRoamingDataHome,
			USER_STANDALONE_CONFIGURATIONS["mcp"],
		);
		await fileService.writeFile(
			target,
			VSBuffer.fromString('{ "my.super.setting": "my.super.value" }'),
		...
		await testObject.writeConfiguration(
			EditableConfigurationTarget.USER_LOCAL,
			{ key: "mcp.service.testSetting", value: "value" },
		);
