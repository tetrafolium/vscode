...
} from "../../../environment/browser/environmentService.js";
...
suite("WorkspaceContextService - Folder", () => {
	...
	setup(async () => {
		...
		await (<WorkspaceService>testObject).initialize(
	...
	test("getWorkspaceFolder() - queries in workspace folder", () =>
	...
	test("getWorkspaceFolder() - queries in resource", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const logService = new NullLogService();
			const fileService = disposables.add(new FileService(logService));
			const fileSystemProvider = disposables.add(
			...
			const actual = testObject.getWorkspaceFolder(
				joinPath(folder, "a").with({ query: "myquery=1" }),
			);
...
suite("WorkspaceContextService - Workspace", () => {
	...
	setup(async () => {
		...
		const remoteAgentService = disposables.add(
			disposables.add(instantiationService.createInstance(RemoteAgentService)),
		);
...
suite("WorkspaceContextService - Workspace Editing", () => {
	...
	test("remove folders and add them back by writing into the file", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const workspace = {
				folders: [{ path: folders[0].uri.path }, { path: folders[1].uri.path }],
			};
			await fileService.writeFile(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				await fileService.writeFile(
					...
					VSBuffer.fromString('{ "initialization.testSetting1": "userValue" }'),
				);
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[3][0]).added.map((f) =>
						f.uri.toString(),
					),
					[folder.toString()],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[3][0]).removed,
					[],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[3][0]).changed,
					[],
				);
			}),
	);
	...
	(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					[...(<IConfigurationChangeEvent>target.args[0][0]).affectedKeys],
					["initialization.testSetting1"],
				);
				assert.deepStrictEqual(target.args[1], [WorkbenchState.FOLDER]);
				assert.deepStrictEqual(target.args[2], [undefined]);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[4][0]).added.map((f) =>
						f.uri.toString(),
					),
					[folder.toString()],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[4][0]).removed,
					[],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[4][0]).changed,
					[],
				);
			}),
	);
	...
	(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(target.args[1], [undefined]);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[3][0]).added.map(
						(folder) => folder.uri.toString(),
					),
					[joinPath(ROOT, "a").toString(), joinPath(ROOT, "b").toString()],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[3][0]).removed,
					[],
	...
	(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					[...(<IConfigurationChangeEvent>target.args[0][0]).affectedKeys],
					["initialization.testSetting1", "initialization.testSetting2"],
				);
				assert.deepStrictEqual(target.args[1], [WorkbenchState.WORKSPACE]);
				assert.deepStrictEqual(target.args[2], [undefined]);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[4][0]).added.map(
						(folder) => folder.uri.toString(),
					),
					[joinPath(ROOT, "a").toString(), joinPath(ROOT, "b").toString()],
				);
	...
	(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[1][0]).added.map(
						(folder_1) => folder_1.uri.toString(),
					),
					[joinPath(ROOT, "b").toString()],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[1][0]).removed.map(
						(folder_2) => folder_2.uri.toString(),
					),
					[joinPath(ROOT, "a").toString()],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[1][0]).changed,
					[],
				);
			}),
	);
	...
	(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[2][0]).added.map(
						(folder_1) => folder_1.uri.toString(),
					),
					[joinPath(ROOT, "b").toString()],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[2][0]).removed.map(
						(folder_2) => folder_2.uri.toString(),
					),
					[joinPath(ROOT, "a").toString()],
				);
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[2][0]).changed,
					[],
				);
			}),
	);
...
suite("WorkspaceConfigurationService - Folder", () => {
	...
	test("application settings are not read from workspace", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.folder.applicationSetting"),
				"userValue",
			);
		}));

	test("application settings are not read from workspace when workspace folder uri is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("machine settings are not read from workspace", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.folder.machineSetting", {
					resource: workspaceService.getWorkspace().folders[0].uri,
				}),
				"userValue",
			);
	...
	test("machine settings are not read from workspace when workspace folder uri is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("application machine overridable settings are not read from workspace", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
					'{ "configurationService.folder.applicationMachineSetting": "userValue" }',
				),
			);
			await fileService.writeFile(
				joinPath(
				...
				VSBuffer.fromString(
					'{ "configurationService.folder.applicationMachineSetting": "workspaceValue" }',
				),
			);
	...
	test("application machine overridable settings are not read from workspace when workspace folder uri is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("get application scope settings are loaded after defaults are registered", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
			...
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.getValue("configurationService.folder.applicationSetting-2"),
				"workspaceValue",
			);
	...
	test("get application scope settings are not loaded after defaults are registered when workspace folder uri is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("get application machine overridable scope settings are loaded after defaults are registered", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
					'{ "configurationService.folder.applicationMachineSetting-2": "userValue" }',
				),
			);
			await fileService.writeFile(
				joinPath(
				...
				VSBuffer.fromString(
					'{ "configurationService.folder.applicationMachineSetting-2": "workspaceValue" }',
				),
			);
	...
	test("get application machine overridable scope settings are loaded after defaults are registered when workspace folder uri is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
					'{ "configurationService.folder.applicationMachineSetting-3": "userValue" }',
				),
			);
			await fileService.writeFile(
				joinPath(
				...
				VSBuffer.fromString(
					'{ "configurationService.folder.applicationMachineSetting-3": "workspaceValue" }',
				),
			);
	...
	test("get machine scope settings are not loaded after defaults are registered", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.getValue("configurationService.folder.machineSetting-2"),
				"workspaceValue",
			);
	...
	test("get machine scope settings are not loaded after defaults are registered when workspace folder uri is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
			...
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.getValue("configurationService.folder.machineSetting-3", {
					resource: workspaceService.getWorkspace().folders[0].uri,
				}),
				"workspaceValue",
			);
	...
	test("policy value override all", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.folder.policySetting"),
				"policyValue",
			);
			assert.strictEqual(
				testObject.inspect("configurationService.folder.policySetting")
					.policyValue,
				"policyValue",
			);
		}));

	test("policy settings when policy value is not set", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("policy value override all for object type setting", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				await fileService.writeFile(
					environmentService.policyFile!,
					VSBuffer.fromString(
						'{ "configurationService.folder.policyObjectSetting": {"a": true} }',
					),
				);
				return promise;
			});
			...
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
					'{ "configurationService.folder.policyObjectSetting": {"b": true} }',
				),
			);
			await testObject.reloadConfiguration();

			assert.deepStrictEqual(
				testObject.getValue("configurationService.folder.policyObjectSetting"),
				{ a: true },
			);
		}));
	...
	test("inspect restricted settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("inspect restricted settings after change", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(false);
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
	...
	test("keys", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.ok(
				actual.default.indexOf("configurationService.folder.testSetting") !==
					-1,
			);
	...
	test("update workspace configuration", () => {
		return testObject
			...
			.then(() =>
				assert.strictEqual(
					testObject.getValue(TasksSchemaProperties.ServiceTestSetting),
					"value",
				),
			);
	});
	...
	test("update language configuration using configuration overrides", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.folder.languageSetting", {
					overrideIdentifier: "abclang",
				}),
				"abcLangValue",
			);
	...
	test("update language configuration using configuration update overrides", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"configurationService.folder.languageSetting",
				"abcLangValue",
	...
	test("update language configuration for multiple languages", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"configurationService.folder.languageSetting",
				"multiLangValue",
			...
			assert.deepStrictEqual(
				testObject.getValue(keyFromOverrideIdentifiers(["deflang", "xyzlang"])),
				{ "configurationService.folder.languageSetting": "multiLangValue" },
			);
		}));

	test("update language configuration for multiple languages when already set", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("update resource language configuration for a language using configuration overrides", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			assert.strictEqual(
				testObject.getValue("configurationService.folder.languageSetting", {
					resource: workspaceService.getWorkspace().folders[0].uri,
	...
	test("update resource language configuration for a language using configuration update overrides", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			assert.strictEqual(
				testObject.getValue("configurationService.folder.languageSetting", {
					resource: workspaceService.getWorkspace().folders[0].uri,
	...
	test("remove setting from all targets", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("update user configuration to default value when target is not passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"configurationService.folder.testSetting",
				"value",
	...
	test("update user configuration to default value when target is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"configurationService.folder.testSetting",
				"value",
		...
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			await new Promise<void>((c, e) => {
				const disposable = testObject.onDidChangeConfiguration((e) => {
					assert.ok(
						e.affectsConfiguration("configurationService.folder.testSetting"),
					);
					assert.strictEqual(
						testObject.getValue("configurationService.folder.testSetting"),
						"workspaceValue",
	...
	test("deleting workspace settings", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("restricted setting is read from workspace when workspace is trusted", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(true);
			...
			assert.strictEqual(
				testObject.getValue("configurationService.folder.restrictedSetting", {
					resource: workspaceService.getWorkspace().folders[0].uri,
				}),
				"workspaceValue",
			);
	...
	test("restricted setting is not read from workspace when workspace is changed to trusted", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(true);
	...
	test("change event is triggered when workspace is changed to untrusted", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(true);
			...
			const event = await promise;
			assert.ok(
				event.affectedKeys.has("configurationService.folder.restrictedSetting"),
			);
			assert.ok(
				event.affectsConfiguration(
					"configurationService.folder.restrictedSetting",
	...
	test("restricted setting is not read from workspace when workspace is not trusted", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(false);
	...
	test("restricted setting is read when workspace is changed to trusted", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(false);
	...
	test("change event is triggered when workspace is changed to trusted", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(false);
	...
	test("adding an restricted setting triggers change event", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("remove an unregistered setting", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
...
suite("WorkspaceConfigurationService - Profiles", () => {
	...
	test("initialize", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.profiles.testSetting2"),
				"profileValue",
			);
		}));
	...
	test("update application scope setting", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"configurationService.profiles.applicationSetting",
				"applicationValue",
			...
			assert.strictEqual(
				testObject.getValue("configurationService.profiles.applicationSetting"),
				"applicationValue",
			);
		}));

	test("update application machine setting", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"configurationService.profiles.applicationMachineSetting",
				"applicationValue",
	...
	test("update normal setting", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.profiles.testSetting"),
				"profileValue",
			);
		}));

	test("registering normal setting after init", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
			...
			assert.strictEqual(
				testObject.getValue("configurationService.profiles.testSetting3"),
				"isSet",
			);
		}));

	test("registering application scope setting after init", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
	...
	test("non registering setting should not be read from default profile", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
				VSBuffer.fromString(
					'{ "configurationService.profiles.nonregistered": "defaultProfile" }',
				),
			);
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.getValue("configurationService.profiles.nonregistered"),
				undefined,
			);
		}));

	test("initialize with custom all profiles settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				APPLY_ALL_PROFILES_SETTING,
				["configurationService.profiles.testSetting2"],
	...
	test("update all profiles settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const promise = Event.toPromise(testObject.onDidChangeConfiguration);
			await testObject.updateValue(
				APPLY_ALL_PROFILES_SETTING,
	...
	test("setting applied to all profiles is registered later", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
			...
			assert.strictEqual(
				testObject.getValue("configurationService.profiles.testSetting4"),
				"userValue",
			);
	...
	test("update setting that is applied to all profiles", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				APPLY_ALL_PROFILES_SETTING,
				["configurationService.profiles.testSetting2"],
	...
	test("test isSettingAppliedToAllProfiles", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			assert.strictEqual(
	...
	test("switch to default profile", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
	...
	test("switch to non default profile", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
			...
			const profile = toUserDataProfile(
				"custom2",
				"custom2",
				joinPath(environmentService.userRoamingDataHome, "profiles", "custom2"),
				joinPath(environmentService.cacheHome, "profilesCache"),
			);
			await fileService.writeFile(
	...
	test("switch to non default profile using settings from default profile", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
	...
	test("In non-default profile, changing application settings shall include only application scope settings in the change event", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile
					.settingsResource,
	...
	test("switch to default profile with settings applied to all profiles", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				APPLY_ALL_PROFILES_SETTING,
				["configurationService.profiles.testSetting2"],
	...
	test("switch to non default profile with settings applied to all profiles", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				APPLY_ALL_PROFILES_SETTING,
				["configurationService.profiles.testSetting2"],
	...
	test("switch to non default from default profile with settings applied to all profiles", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				APPLY_ALL_PROFILES_SETTING,
				["configurationService.profiles.testSetting2"],
...
suite("WorkspaceConfigurationService-Multiroot", () => {
	let workspaceContextService: IWorkspaceContextService,
		jsonEditingServce: IJSONEditingService,
		testObject: WorkspaceService,
		fileService: IFileService,
		environmentService: BrowserWorkbenchEnvironmentService,
		userDataProfileService: IUserDataProfileService;
	...
	test("application settings are not read from workspace when folder is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("machine settings are not read from workspace when folder is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("get application scope settings are not loaded after defaults are registered", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.getValue("configurationService.workspace.newSetting"),
				"workspaceValue",
			);
	...
	test("get application scope settings are not loaded after defaults are registered when workspace folder is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
			...
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.getValue("configurationService.workspace.newSetting-2", {
					resource: workspaceContextService.getWorkspace().folders[0].uri,
				}),
				"workspaceValue",
			);
		...
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				workspaceContextService
					.getWorkspace()
					.folders[0].toResource(".vscode/settings.json"),
				VSBuffer.fromString(
	...
	test("application settings are not read from workspace folder when workspace folder is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("machine settings are not read from workspace folder", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.workspace.machineSetting"),
				"userValue",
			);
		}));

	test("machine settings are not read from workspace folder when workspace folder is passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("machine settings are not read from workspace folder after defaults are registered", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("update user configuration", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue("configurationService.workspace.testSetting"),
				"userValue",
			);
		}));
	...
	test("update machine overridable setting in folder", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const workspace = workspaceContextService.getWorkspace();
			await testObject.updateValue(
				"configurationService.workspace.machineOverridableSetting",
	...
	test("configuration of newly added folder is available on configuration change event", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("restricted setting is read from workspace folders when workspace is trusted", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			testObject.updateWorkspaceTrust(true);
...
suite("WorkspaceConfigurationService - Remote Folder", () => {
	let testObject: WorkspaceService,
		...
		remoteSettingsResource: URI,
		fileSystemProvider: InMemoryFileSystemProvider,
		resolveRemoteEnvironment: () => void,
		instantiationService: TestInstantiationService,
		fileService: IFileService,
		environmentService: BrowserWorkbenchEnvironmentService,
		userDataProfileService: IUserDataProfileService;
	...
	setup(async () => {
		...
		const remoteEnvironmentPromise = new Promise<
			Partial<IRemoteAgentEnvironment>
		>(
			(c) =>
				(resolveRemoteEnvironment = () =>
					c({ settingsPath: remoteSettingsResource })),
		);
		const remoteAgentService = instantiationService.stub(IRemoteAgentService, <
			Partial<IRemoteAgentService>
		...
		disposables.add(
			instantiationService
				...
				.registerProvider(
					...
					new RemoteFileSystemProvider(fileSystemProvider, remoteAuthority),
				),
		);
	}
	...
	function registerRemoteFileSystemProviderOnActivation(): void {
		const disposable = disposables.add(
			instantiationService
				.get(IFileService)
				.onWillActivateFileSystemProvider((e) => {
					if (e.scheme === Schemas.vscodeRemote) {
						disposable.dispose();
						e.join(
							Promise.resolve().then(() => registerRemoteFileSystemProvider()),
						);
					}
				}),
		);
	...
	test("remote machine settings override globals", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
			...
			await initialize();
			assert.strictEqual(
				testObject.getValue("configurationService.remote.machineSetting"),
				"remoteValue",
			);
		}));

	test("remote machine settings override globals after remote provider is registered on activation", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
	...
	test("remote machine settings override globals after remote environment is resolved", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
	...
	test("remote settings override globals after remote provider is registered on activation and remote environment is resolved", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
	...
	test("machine settings in local user settings does not override defaults", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("remote application machine settings override globals", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
					'{ "configurationService.remote.applicationMachineSetting": "remoteValue" }',
				),
			);
			registerRemoteFileSystemProvider();
			resolveRemoteEnvironment();
	...
	test("remote application machine settings override globals after remote provider is registered on activation", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
	...
	test("remote application machine settings override globals after remote environment is resolved", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
	...
	test("remote application machine settings override globals after remote provider is registered on activation and remote environment is resolved", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
	...
	test("application machine settings in local user settings does not override defaults", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
					'{ "configurationService.remote.applicationMachineSetting": "globalValue" }',
				),
			);
			registerRemoteFileSystemProvider();
			resolveRemoteEnvironment();
	...
	test("machine overridable settings in local user settings does not override defaults", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("non machine setting is written in local settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			registerRemoteFileSystemProvider();
			resolveRemoteEnvironment();
			await initialize();
			...
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.inspect("configurationService.remote.applicationSetting")
					.userLocalValue,
				"applicationValue",
			);
		}));

	test("machine setting is written in remote settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			registerRemoteFileSystemProvider();
			resolveRemoteEnvironment();
			await initialize();
			...
			await testObject.reloadConfiguration();
			assert.strictEqual(
				testObject.inspect("configurationService.remote.machineSetting")
					.userRemoteValue,
				"machineValue",
			);
		}));

	test("application machine setting is written in remote settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			registerRemoteFileSystemProvider();
			resolveRemoteEnvironment();
			await initialize();
	...
	test("machine overridable setting is written in remote settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			registerRemoteFileSystemProvider();
			resolveRemoteEnvironment();
			await initialize();
	...
	test("machine settings in local user settings does not override defaults after defalts are registered ", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
			...
			assert.strictEqual(
				testObject.getValue("configurationService.remote.newMachineSetting"),
				"isSet",
			);
		}));

	test("machine overridable settings in local user settings does not override defaults after defaults are registered ", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
