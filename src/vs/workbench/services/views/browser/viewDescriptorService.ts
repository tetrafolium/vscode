	...
	implements IViewDescriptorService
{
	...
	get viewContainers(): ReadonlyArray<ViewContainer> {
		return this.viewContainersRegistry.all.filter((vc) =>
			this.isViewContainerEnabled(vc),
		);
	}
		...
		this._register(
			this.viewContainersRegistry.onDidDeregister(
				({ viewContainer, viewContainerLocation }) => {
					...
					this._onDidChangeViewContainers.fire({
						removed: [
							{ container: viewContainer, location: viewContainerLocation },
						],
						added: [],
					});
				},
	...
	private migrateToViewsCustomizationsStorage(): void {
		...
		const viewsCustomizations: IViewsCustomizations = {
			...
			viewLocations: viewDescriptorLocations.reduce<IStringDictionary<string>>(
	...
	private registerGroupedViews(
		...
		for (const [containerId, views] of groupedViews.entries()) {
			...
			const viewsToAdd = views.filter(
				(view) =>
					this.getViewContainerModel(viewContainer).allViewDescriptors.filter(
						(vd) => vd.id === view.id,
					).length === 0,
			);
	...
	private onDidDeregisterViews(
		...
		this.contextKeyService.bufferChangeEvents(() => {
			views.forEach((viewDescriptor) =>
				this.getOrCreateMovableViewContextKey(viewDescriptor).set(false),
			);
		});
	}
	...
	private isEnabled(enablement: WindowEnablement | undefined): boolean {
		if (this.isSessionsWindow) {
			return (
				enablement === WindowEnablement.Sessions ||
				enablement === WindowEnablement.Both
			);
		}
		return (
			!enablement ||
			enablement === WindowEnablement.Editor ||
			enablement === WindowEnablement.Both
		);
	}
	...
	getDefaultViewContainer(
		...
		const viewContainers =
			this.viewContainersRegistry.getDefaultViewContainers(location);
		return viewContainers.find((viewContainer) =>
			this.isViewContainerEnabled(viewContainer),
		);
	}
		...
		const viewsToMove = views.filter(
			(view) =>
				...
				(!this.viewContainers.includes(from) &&
					this.viewDescriptorsCustomLocations.get(view.id) === from.id), // Move views which are moved from a removed container
		);
	...
	private onDidChangeVisibleViews({
		...
		this.contextKeyService.bufferChangeEvents(() => {
			...
			removed.forEach((viewDescriptor) =>
				this.getOrCreateVisibleViewContextKey(viewDescriptor).set(false),
			);
		});
	}
	...
	private registerViewsVisibilityActions(
		viewContainer: ViewContainer,
		{
			viewContainerModel,
			disposables,
		}: { viewContainerModel: ViewContainerModel; disposables: DisposableStore },
	): void {
		this.viewsVisibilityActionDisposables.deleteAndDispose(viewContainer);
											...
											},
											{
												...
												when: ContextKeyExpr.or(
													...viewContainerModel.visibleViewDescriptors.map(
														(v) => ContextKeyExpr.equals("view", v.id),
													),
												),
												order: index,
						...
						registerAction2(
							class extends ViewPaneContainerAction<ViewPaneContainer> {
								...
								async runInViewPaneContainer(
									...
									if (
										viewPaneContainer.getView(viewDescriptor.id)?.isVisible()
									) {
										viewPaneContainer.toggleViewVisibility(viewDescriptor.id);
									}
								}
		...
		this.getViewContainerModel(container).add(
			views.map((view) => {
				return {
					viewDescriptor: view,
					collapsed:
						visibilityState === ViewVisibilityState.Default ? undefined : false,
					visible:
						visibilityState === ViewVisibilityState.Default ? undefined : true,
				};
			}),
		);
	}
	...
	private getOrCreateMovableViewContextKey(
		...
		if (!contextKey) {
			contextKey = new RawContextKey(movableViewContextKeyId, false).bindTo(
				this.contextKeyService,
			);
