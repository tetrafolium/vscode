...
export class ViewsService extends Disposable implements IViewsService {
			...
			if (viewContainerLocation !== null) {
				return isActive
					? this.layoutService.setPartHidden(
							true,
							this.paneCompositeService.getPartId(viewContainerLocation),
						)
					: undefined;
			}
		}
	...
	private registerOpenViewContainerAction(
		...
		if (viewContainer.openCommandActionDescriptor) {
			...
			const title =
				viewContainer.openCommandActionDescriptor.title ?? viewContainer.title;
			const that = this;
			disposables.add(
				registerAction2(
						...
						constructor() {
							super({
								...
								get title(): ICommandActionTitle {
									...
									const originalTitle =
										typeof title === "string" ? title : title.original;
	...
	private registerOpenViewAction(viewDescriptor: IViewDescriptor): IDisposable {
		const disposables = new DisposableStore();
		const title =
			viewDescriptor.openCommandActionDescriptor?.title ?? viewDescriptor.name;
		const commandId =
			viewDescriptor.openCommandActionDescriptor?.id ??
			`${viewDescriptor.id}.open`;
		const that = this;
		disposables.add(
			registerAction2(
				class OpenViewAction extends Action2 {
					constructor() {
						super({
							...
							keybinding: viewDescriptor.openCommandActionDescriptor
								?.keybindings
								? {
					...
					public async run(
						...
						if (
							focusedViewId === viewDescriptor.id &&
							...
							if (
								viewDescriptorService.getViewLocationById(viewDescriptor.id) ===
								...
								layoutService.setPartHidden(
									true,
									that.paneCompositeService.getPartId(viewLocation),
								);
							}
						} else {
							await viewsService.openView(
								viewDescriptor.id,
								!options?.preserveFocus,
							);
						}
		...
		if (viewDescriptor.openCommandActionDescriptor?.mnemonicTitle) {
			const defaultViewContainer =
				this.viewDescriptorService.getDefaultContainerById(viewDescriptor.id);
			if (defaultViewContainer) {
				...
				disposables.add(
					MenuRegistry.appendMenuItem(MenuId.MenubarViewMenu, {
						...
						order:
							viewDescriptor.openCommandActionDescriptor.order ??
							Number.MAX_VALUE,
					}),
				);
			}
			...
			class FocusViewAction extends Action2 {
				constructor() {
					const title = localize2(
						{
							key: "focus view",
							comment: ["{0} indicates the name of the view to be focused."],
						},
						"Focus on {0} View",
						viewDescriptor.name.value,
					);
			...
			class ResetViewLocationAction extends Action2 {
				...
				run(accessor: ServicesAccessor): void {
					...
					const defaultContainer =
						viewDescriptorService.getDefaultContainerById(viewDescriptor.id)!;
					const containerModel =
						viewDescriptorService.getViewContainerModel(defaultContainer)!;
	...
	private createViewPaneContainer(
		...
		disposables.add(
			viewPaneContainer.onDidAddViews((views) => this.onViewsAdded(views)),
		);
		disposables.add(
		...
		disposables.add(
			viewPaneContainer.onDidRemoveViews((views) => this.onViewsRemoved(views)),
		);
		disposables.add(
