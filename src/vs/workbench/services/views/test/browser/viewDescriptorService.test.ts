...
suite("ViewDescriptorService", () => {
	...
	test("move views to generated containers", async function () {
		...
		const generatedPanel = assertReturnsDefined(
			testObject.getViewContainerByViewId(viewDescriptors[0].id),
		);
		const generatedSidebar = assertReturnsDefined(
	...
	test("reset", async function () {
		...
		const actual = JSON.parse(
			instantiationService
				.get(IStorageService)
				.get("views.customizations", StorageScope.PROFILE)!,
		);
	...
	test("initialize with custom locations", async function () {
		...
		const viewContainer1Views =
			testObject.getViewContainerModel(viewContainer1);
		assert.deepStrictEqual(
	...
	test("orphan view containers", async function () {
		...
		assert.deepStrictEqual(
			testObject.isViewContainerRemovedPermanently(generatedViewContainerId),
			true,
		);
	...
	test("storage change move views and retain visibility state", async function () {
		...
		assert.deepStrictEqual(
			viewContainer1Views.allViewDescriptors
				.map((v) => v.id)
