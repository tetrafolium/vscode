...
export class NativeWindow extends BaseWindow {
	...
	protected registerListeners(): void {
		...
		ipcRenderer.on(
			"vscode:runAction",
			async (event: unknown, ...argsRaw: unknown[]) => {
		...
		ipcRenderer.on(
			...
			(event: unknown, ...argsRaw: unknown[]) =>
				this.onAddRemoveFoldersRequest(argsRaw[0] as IAddRemoveFoldersRequest),
		);
		...
		ipcRenderer.on(
			...
			(event: unknown, ...argsRaw: unknown[]) =>
				this.notificationService.info(argsRaw[0] as string),
		);
		...
		this._register(
			this.hostService.onDidChangeActiveWindow(() => this.updateWindowBorder()),
		);
		this._register(
			this.hostService.onDidChangeFocus(() => this.updateWindowBorder()),
		);
	...
	private async handleWarnings(): Promise<void> {
		...
		if (isMacintosh) {
			...
			const eolReleases = new Map<string, string>([["20", "macOS Big Sur"]]);
	...
	private updateWindowBorder(): void {
		...
		const borderSetting =
			this.configurationService.getValue<string>("window.border");
		...
		this.nativeHostService.updateWindowAccentColor(
			activeBorder,
			inactiveBorder,
		);
	}
...
class ZoomStatusEntry extends Disposable {
	...
	private createZoomEntry(visibleOrText: string): void {
		...
		zoomResetAction.tooltip = this.keybindingService.appendKeybinding(
