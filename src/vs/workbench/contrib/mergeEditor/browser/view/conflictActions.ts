...
export class ConflictActionsFactory extends Disposable {
	...
	constructor(private readonly _editor: ICodeEditor) {
		...
		this._styleClassName =
			"_conflictActionsFactory_" + hash(this._editor.getId()).toString(16);
...
export class ActionsSource {
	...
	private getItemsInput(
		...
		return derived((reader) => {
			...
			const inputData =
				inputNumber === 1 ? viewModel.model.input1 : viewModel.model.input2;
			...
			if (
				...
				if (
					...
					if (modifiedBaseRange.canBeCombined) {
						result.push(
							command(
								localize("combine", "Accept Combination", inputData.title),
								async () => {
									transaction((tx) => {
										model.setState(
											modifiedBaseRange,
				...
				if (
					!model.isInputHandled(modifiedBaseRange, inputNumber).read(reader)
				) {
					result.push(
						command(
