...
export class MergeEditorViewModel extends Disposable {
	...
	constructor(
		...
		this.focusedEditorType = derived<MergeEditorType | undefined>(
			this,
			(reader) => {
				const lastFocusedEditor = this.lastFocusedEditor.read(reader);
				...
				if (lastFocusedEditor.view === this.inputCodeEditorView1) {
					...
					lastFocusedEditor.view === this.baseCodeEditorView.read(reader)
				) {
					return "base";
				}
			...
			model.onDidChangeContent((e) => {
				...
				if (e.isRedoing) {
					...
					while (
						...
						this.attachedHistory[this.attachedHistory.length - 1]!.altId >
							this.previousAltId
					) {
