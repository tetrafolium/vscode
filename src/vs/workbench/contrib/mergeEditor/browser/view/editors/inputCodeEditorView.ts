...
export class InputCodeEditorView extends CodeEditorView {
	...
	constructor(
		...
		this.modifiedBaseRangeGutterItemInfos = derivedOpts(
			{
		...
		this.decorations = derivedOpts(
			...
			(reader) => {
				...
				const model = viewModel.model;
				const textModel = (this.inputNumber === 1 ? model.input1 : model.input2)
					.textModel;
		...
		this._register(
			new EditorGutter(this.editor, this.htmlElements.gutterDiv, {
				...
				createView: (item, target) =>
					new MergeConflictGutterItemView(item, target, contextMenuService),
			}),
		);
		...
		this._register(
			autorun((reader) => {
				...
				const iconMap: Record<
					InputState,
					{ icon: ThemeIcon | undefined; checked: boolean; title: string }
				> = {
					[InputState.excluded]: {
						icon: undefined,
						checked: false,
