		...
		transaction((tx) => {
			for (const m of modifiedBaseRanges) {
				...
				if (
					...
					!state.isInputIncluded(1) &&
					(!state.isInputIncluded(2) ||
						!viewModel.shouldUseAppendInsteadOfAccept.get()) &&
					m.canBeCombined
				) {
					model.setState(
						m,
...
export class ToggleBetweenInputs extends MergeEditorAction2 {
	...
	override runWithMergeEditor(
		{ viewModel }: MergeEditorAction2Args,
		accessor: ServicesAccessor,
	) {
		const input1IsFocused =
			viewModel.inputCodeEditorView1.editor.hasWidgetFocus();
