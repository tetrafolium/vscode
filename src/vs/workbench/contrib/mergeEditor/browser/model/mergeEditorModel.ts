...
export class MergeEditorModel extends EditorModel {
	...
	constructor(
		...
		this.resultInput1Mapping = derived(this, (reader) =>
		...
		this.resultInput2Mapping = derived(this, (reader) =>
		...
		this.resultBaseMapping = derived(this, (reader) =>
		...
		this.inputDiffComputingState = derived(this, (reader) => {
			const states = [this.input1TextModelDiffs, this.input2TextModelDiffs].map(
				(s) => s.state.read(reader),
			);
		...
		this.isUpToDate = derived(
			this,
			(reader) =>
				this.diffComputingState.read(reader) === MergeEditorModelState.upToDate,
		);
						...
						transaction((tx) => {
							...
							if (shouldRecomputeHandledFromAccepted) {
								...
								for (const [_range, observableState] of states) {
									...
									const handled = !(
										state.kind === ModifiedBaseRangeStateKind.base ||
	...
	private computeAutoMergedResult(): string {
		...
		function appendLinesToResult(
			source: string[],
			lineRange: MergeEditorLineRange,
		) {
			for (
		...
		for (const baseRange of baseRanges) {
			appendLinesToResult(
				baseLines,
				MergeEditorLineRange.fromLineNumbers(
					baseStartLineNumber,
	...
	public translateInputRangeToBase(input: 1 | 2, range: Range): Range {
		const baseInputDiffs =
			input === 1 ? this.baseInput1Diffs.get() : this.baseInput2Diffs.get();
	...
	public findModifiedBaseRangesInRange(
		rangeInBase: MergeEditorLineRange,
	): ModifiedBaseRange[] {
		// TODO use binary search
		return this.modifiedBaseRanges
			.get()
			.filter((r) => r.baseRange.intersectsOrTouches(rangeInBase));
	}
	...
	public async getResultValueWithConflictMarkers(): Promise<string> {
		...
		for (const [range, state] of states) {
			...
			outputLines.push("<<<<<<<");
			if (
				state.accepted.get().kind === ModifiedBaseRangeStateKind.unrecognized
			) {
...
class ModifiedBaseRangeData {
	constructor(private readonly baseRange: ModifiedBaseRange) {
		this.accepted = observableValue(
			`BaseRangeState${this.baseRange.baseRange}`,
			ModifiedBaseRangeState.base,
		);
		this.handledInput1 = observableValue(
			`BaseRangeHandledState${this.baseRange.baseRange}.Input1`,
			false,
		);
		this.handledInput2 = observableValue(
