...
class RenderWhitespaceNoneAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2(
				"renderWhitespace.setNone",
				"Set Render Whitespace to None",
			),
			shortTitle: localize2("renderWhitespace.none", "None"),
			category: Categories.View,
			f1: false,
			toggled: ContextKeyExpr.equals(
				`config.${renderWhitespaceSetting}`,
				"none",
			),
			menu: {
	...
	override run(accessor: ServicesAccessor): Promise<void> {
		return accessor
			...
			.updateValue(renderWhitespaceSetting, "none");
	}
}
...
class RenderWhitespaceBoundaryAction extends Action2 {
	...
	override run(accessor: ServicesAccessor): Promise<void> {
		return accessor
			.get(IConfigurationService)
			.updateValue(renderWhitespaceSetting, "boundary");
	}
}
...
class RenderWhitespaceSelectionAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2(
				"renderWhitespace.setSelection",
				"Set Render Whitespace to Selection",
			),
	...
	override run(accessor: ServicesAccessor): Promise<void> {
		return accessor
			.get(IConfigurationService)
			.updateValue(renderWhitespaceSetting, "selection");
	}
}
...
class RenderWhitespaceTrailingAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2(
				"renderWhitespace.setTrailing",
				"Set Render Whitespace to Trailing",
			),
	...
	override run(accessor: ServicesAccessor): Promise<void> {
		return accessor
			.get(IConfigurationService)
			.updateValue(renderWhitespaceSetting, "trailing");
	}
}
...
class RenderWhitespaceAllAction extends Action2 {
	...
	override run(accessor: ServicesAccessor): Promise<void> {
		return accessor
			.get(IConfigurationService)
			.updateValue(renderWhitespaceSetting, "all");
	}
}
