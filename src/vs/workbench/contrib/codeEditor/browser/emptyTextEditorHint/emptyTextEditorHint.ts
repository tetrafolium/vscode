...
export class EmptyTextEditorHintContribution
	extends Disposable
	implements IEditorContribution
{
	...
	protected shouldRenderHint() {
		...
		const hasEditorAgents = Boolean(
			this.chatAgentService.getDefaultAgent(ChatAgentLocation.EditorInline),
		);
		const shouldRenderDefaultHint =
	...
	implements IContentWidget
{
	...
	private getHint() {
		const hasInlineChatProvider =
			this.chatAgentService
				.getActivatedAgents()
				.filter((candidate) =>
					candidate.locations.includes(ChatAgentLocation.EditorInline),
				).length > 0;
		...
		const hintHandler: IContentActionHandler = {
			...
			callback: (index, event) => {
				switch (index) {
					case "0":
						hasInlineChatProvider
							? askSomething(event.browserEvent)
		...
		const keybindingLabels = keybindingsLookup.map((id) =>
			this.keybindingService.lookupKeybinding(id)?.getLabel(),
		);
	...
	getDomNode(): HTMLElement {
		if (!this.domNode) {
			...
			this.editor.applyFontInfo(this.domNode);
			const lineHeight = this.editor.getLineHeightForPosition(
				new Position(1, 1),
			);
