...
export class DocumentSymbolDragAndDrop implements ITreeDragAndDrop<DocumentSymbolItem> {
	...
	getDragURI(element: DocumentSymbolItem): string | null {
		...
		if (element instanceof OutlineElement) {
			...
			return (
				symbolUri.fsPath + (symbolUri.fragment ? "#" + symbolUri.fragment : "")
			);
		} else {
	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
		const elements = (
			data as ElementsDragAndDropData<DocumentSymbolItem, DocumentSymbolItem[]>
		).elements;
		...
		const outlineElements =
			item instanceof OutlineElement
				? [item]
...
export class DocumentSymbolGroupRenderer implements ITreeRenderer<
	OutlineGroup,
	FuzzyScore,
	DocumentSymbolGroupTemplate
> {
...
export class DocumentSymbolRenderer implements ITreeRenderer<
	OutlineElement,
	FuzzyScore,
	DocumentSymbolTemplate
> {
	...
	renderElement(
		...
		if (this._configurationService.getValue(OutlineConfigKeys.icons)) {
			...
			template.iconClass.classList.add(
				...
				"inline",
				"codicon-colored",
				...ThemeIcon.asClassNameArray(SymbolKinds.toIcon(element.symbol.kind)),
			);
		}
