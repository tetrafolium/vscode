...
class ManageTrustedMcpServersForAccountActionImpl {
	...
	async run(options?: { providerId: string; accountLabel: string }) {
		const accountQuery = await this._resolveAccountQuery(
			options?.providerId,
	...
	private async _resolveAccountQuery(
		...
		if (providerId && accountLabel) {
			return this._authenticationQueryService
				.provider(providerId)
		...
		return pick
			? this._authenticationQueryService
	...
	private async _getAllAvailableAccounts() {
		...
		for (const providerId of this._mcpServerAuthenticationService.getProviderIds()) {
			const provider =
				this._mcpServerAuthenticationService.getProvider(providerId);
			const sessions =
				await this._mcpServerAuthenticationService.getSessions(providerId);
	...
	private async _getItems(accountQuery: IAccountQuery) {
		...
		const serverIdToLabel = new Map<string, string>(
			this._mcpServerService.servers
				.get()
				.map((s) => [s.definition.id, s.definition.label]),
		);
		const filteredMcpServers = allowedMcpServers
			...
			.map((server) => {
				...
				return {
					...
					lastUsed:
						usage.length > 0
							? Math.max(...usage.map((u) => u.lastUsed))
							: server.lastUsed,
				};
			});
	...
	private _createQuickPick(accountQuery: IAccountQuery) {
		...
		disposableStore.add(
			quickPick.onDidAccept(() => {
				...
				const allServers = quickPick.items
					.filter(
						(item: any): item is TrustedMcpServersQuickPickItem =>
							item.type !== "separator",
					)
					.map((i: any) => i.mcpServer);

				const selectedServers = new Set(
					quickPick.selectedItems.map((i: any) => i.mcpServer),
				);
				...
				for (const mcpServer of allServers) {
					...
					accountQuery
						...
						.setAccessAllowed(isAllowed, mcpServer.name);
				}
			}),
		);
		...
		disposableStore.add(
			quickPick.onDidTriggerItemButton((e: any) =>
