...
class ManageAccountsActionImpl {
	...
	private async listAccounts(): Promise<AccountQuickPickItem[]> {
		...
		for (const providerId of this.authenticationService.getProviderIds()) {
			...
			for (const { label, id } of await this.authenticationService.getAccounts(
				providerId,
			)) {
				accounts.push({
					label,
					...
					canSignOut: async () =>
						this.canSignOut(provider, id, await activeSession.value),
				});
			}
		}
		return accounts;
	...
	private async canSignOut(
		...
		if (session && !session.canSignOut && session.providerId === provider.id) {
			...
			return !sessions.some(
				(o) => o.id === session.id && o.account.id === accountId,
			);
		}
		return true;
	}

	private async showAccountActions(
		account: AccountQuickPickItem,
	): Promise<void> {
		const { providerId, label: accountLabel, canUseMcp, canSignOut } = account;

		const store = new DisposableStore();
		const quickPick = store.add(
			this.quickInputService.createQuickPick<AccountActionQuickPickItem>(),
		);
