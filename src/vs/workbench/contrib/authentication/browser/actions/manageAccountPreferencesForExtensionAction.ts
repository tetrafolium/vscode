...
class ManageAccountPreferenceForExtensionActionImpl {
	...
	async run(extensionId?: string, providerId?: string) {
				...
				.filter(
					(ext) =>
						this._authenticationQueryService
							.extension(ext.identifier.value)
							.getAllAccountPreferences().size > 0,
				)
				.sort((a, b) =>
		...
		if (!providerId) {
			...
			const extensionQuery =
				this._authenticationQueryService.extension(extensionId);
		...
		const currentAccountNamePreference = this._authenticationQueryService
			.provider(providerId)
			.extension(extensionId)
			.getPreferredAccount();
		const items: Array<QuickPickInput<AccountPreferenceQuickPickItem>> =
			this._getItems(accounts, providerId, currentAccountNamePreference);
	...
	private async _accept(
		...
		for (const item of selectedItems) {
			...
			const extensionQuery = this._authenticationQueryService
				.provider(providerId)
				.extension(extensionId);
