...
export class RemoveDynamicAuthenticationProvidersAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const dynamicAuthStorageService = accessor.get(
			IDynamicAuthenticationProviderStorageService,
		);
		const authenticationService = accessor.get(IAuthenticationService);
		const dialogService = accessor.get(IDialogService);

		const interactedProviders =
			dynamicAuthStorageService.getInteractedProviders();
		...
		const items: IDynamicProviderQuickPickItem[] = interactedProviders.map(
			(provider) => ({
		...
		for (const item of selected) {
			...
			if (
				authenticationService.isAuthenticationProviderRegistered(providerId)
			) {
