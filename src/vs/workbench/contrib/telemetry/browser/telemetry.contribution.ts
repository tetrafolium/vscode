	...
	implements IWorkbenchContribution
{
	...
	private onTextFileModelResolved(e: ITextFileResolveEvent): void {
		...
		if (!settingsType) {
			...
			this.telemetryService.publicLog2<TelemetryData, FileGetClassification>(
				"fileGet",
				this.getTelemetryData(e.model.resource, e.reason),
			);
		}
	}
	...
	private onTextFileModelSaved(e: ITextFileSaveEvent): void {
		...
		if (!settingsType) {
			...
			this.telemetryService.publicLog2<TelemetryData, FilePutClassfication>(
				"filePUT",
				this.getTelemetryData(e.model.resource, e.reason),
			);
		}
	...
	private getTypeIfSettings(resource: URI): string {
		...
		for (const folder of folders) {
			if (isEqualOrParent(resource, folder.toResource(".vscode"))) {
				...
				if (
					TelemetryContribution.ALLOWLIST_WORKSPACE_JSON.indexOf(filename) > -1
				) {
	...
	private async handleTelemetryOutputVisibility(): Promise<void> {
		...
		if (
			![...this.loggerService.getRegisteredLoggers()].find(
				(logger) => logger.id === telemetryLogId,
			)
		) {
			await Event.toPromise(
				Event.filter(this.loggerService.onDidChangeLoggers, (e) =>
					[...e.added].some((logger) => logger.id === telemetryLogId),
				),
			);
		}
		...
		for (const logger of this.loggerService.getRegisteredLoggers()) {
			if (logger.id === telemetryLogId) {
				showTelemetry =
					this.loggerService.getLogLevel() === LogLevel.Trace || !logger.hidden;
				if (showTelemetry) {
		...
		if (showTelemetry) {
			...
			this._register(
				this.loggerService.onDidChangeLoggers((e) =>
					showExtensionTelemetry(e.added),
				),
			);
		}
	}
	...
	private reportTelemetry(
		...
		switch (key) {
			...
			case TerminalContribSettingId.OutputLocation:
				...
				>("terminal.integrated.chatAgentTools.outputLocation", {
					settingValue: this.getValueToReport(key, target),
					source,
				});
				return;
