...
const UPDATE_TITLE_BAR_CONTEXT = new RawContextKey<boolean>(
	"updateTitleBar",
	false,
);
...
const ACTIONABLE_STATES: readonly StateType[] = [
...
let additionalMenuPlacement:
	| { readonly menuId: MenuId; readonly item: Omit<IMenuItem, "command"> }
	| undefined;

export function registerUpdateTitleBarMenuPlacement(
	menuId: MenuId,
	item: Omit<IMenuItem, "command"> = {},
): void {
	if (additionalMenuPlacement) {
		throw new Error(
			"An additional update title bar menu placement is already registered",
		);
	}
 ...
 */
export class UpdateTitleBarContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this.tooltip = this._register(
			instantiationService.createInstance(UpdateTooltip),
		);
		...
		if (additionalMenuPlacement) {
			...
			MenuRegistry.appendMenuItem(menuId, {
				...
				when: item.when
					? ContextKeyExpr.and(UPDATE_TITLE_BAR_CONTEXT, item.when)
					: UPDATE_TITLE_BAR_CONTEXT,
			});
			this._register(
				actionViewItemService.register(
	...
	private createEntry(
		...
		this.entry = instantiationService.createInstance(
			UpdateTitleBarEntry,
			action,
			options,
			this.tooltip,
			() => {
				...
				if (
					!ACTIONABLE_STATES.includes(this.state.type) &&
					!DETAILED_STATES.includes(this.state.type)
				) {
	...
	private async onStateChange(startup = false) {
		...
		if (
			this.configurationService.getValue<boolean>(UPDATE_TITLE_BAR_SETTING) ===
			false
		) {
			this.context.set(false);
			return;
		...
		switch (this.state.type) {
			case StateType.Disabled:
				if (startup) {
					...
					if (
						reason === DisablementReason.InvalidConfiguration ||
						reason === DisablementReason.RunningAsAdmin
					) {
						const lastShown = this.storageService.getNumber(
							DISABLED_REMINDER_LAST_SHOWN_KEY,
							StorageScope.APPLICATION,
						);
						showTooltip =
							lastShown === undefined ||
							Date.now() - lastShown >= DISABLED_REMINDER_PERIOD;
					}
				}
				break;
			case StateType.Idle:
...
export class UpdateTitleBarEntry extends BaseActionViewItem {
	...
	constructor(
		...
		this._register(
			this.updateService.onStateChange((state) => this.onStateChange(state)),
		);
	}
	...
	private async runAction() {
		...
		this.telemetryService.publicLog2<
			WorkbenchActionExecutedEvent,
			WorkbenchActionExecutedClassification
		>("workbenchActionExecuted", { id: commandId, from: "titlebar" });
