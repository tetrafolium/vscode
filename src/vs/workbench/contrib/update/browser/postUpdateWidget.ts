...
} from "../common/updateInfoParser.js";
import {
	getUpdateInfoUrl,
 ...
 */
export class PostUpdateWidgetContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private async getUpdateInfo(
		input?: string | null,
	): Promise<IParsedUpdateInfoInput | undefined> {
		if (!input) {
			try {
	...
	private buildContent(
		...
		titleEl.textContent =
			title ??
			localize("postUpdate.title", "New in {0}", this.productService.version);
