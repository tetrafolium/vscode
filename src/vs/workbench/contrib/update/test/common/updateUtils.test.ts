...
suite("UpdateUtils", () => {
	...
	suite("computeProgressPercent", () => {
		test("handles invalid values", () => {
			...
			assert.strictEqual(
				computeProgressPercent(undefined, undefined),
				undefined,
			);
	...
	suite("computeDownloadTimeRemaining", () => {
		test("returns undefined for invalid or incomplete input", () => {
			...
			// Missing parameters
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState()),
				undefined,
			);
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(500, undefined, now)),
				undefined,
			);
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(undefined, 1000, now)),
				undefined,
			);
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(500, 1000, undefined)),
				undefined,
			);

			// Zero or negative values
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(0, 1000, now - 1000)),
				undefined,
			);
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(500, 0, now - 1000)),
				undefined,
			);
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(500, 1000, now + 1000)),
				undefined,
			);
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(-100, 1000, now - 1000)),
				undefined,
			);
		});
		...
		test("returns 0 when download is complete or over-downloaded", () => {
			const now = Date.now();
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(1000, 1000, now - 1000)),
				0,
			);
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(1500, 1000, now - 1000)),
				0,
			);
		});
		...
		test("computes correct time remaining", () => {
			...
			assert.strictEqual(
				computeDownloadTimeRemaining(
					DownloadingState(downloadedBytes, totalBytes, now - 10000),
				),
				10,
			);
			...
			assert.strictEqual(
				computeDownloadTimeRemaining(DownloadingState(900, 1000, now - 900)),
				1,
			);
			...
			assert.strictEqual(
				computeDownloadTimeRemaining(
					DownloadingState(downloaded50MB, total100MB, now - 50000),
				),
				50,
			);
		});
	...
	suite("computeDownloadSpeed", () => {
		test("returns undefined for invalid or incomplete input", () => {
			const now = Date.now();
			assert.strictEqual(
				computeDownloadSpeed(DownloadingState(undefined, 1000, now - 1000)),
				undefined,
			);
			assert.strictEqual(
				computeDownloadSpeed(DownloadingState(500, 1000, undefined)),
				undefined,
			);
			assert.strictEqual(
				computeDownloadSpeed(DownloadingState(undefined, undefined, undefined)),
				undefined,
			);
		});
		...
		test("returns undefined for zero or negative elapsed time", () => {
			const now = Date.now();
			assert.strictEqual(
				computeDownloadSpeed(DownloadingState(500, 1000, now + 1000)),
				undefined,
			);
		});
		...
		test("returns undefined for zero downloaded bytes", () => {
			const now = Date.now();
			assert.strictEqual(
				computeDownloadSpeed(DownloadingState(0, 1000, now - 1000)),
				undefined,
			);
		});
		...
		test("computes correct download speed in bytes per second", () => {
			...
			const speed1 = computeDownloadSpeed(
				DownloadingState(1000, 2000, now - 1000),
			);
			...
			const speed2 = computeDownloadSpeed(
				DownloadingState(tenMB, tenMB * 2, now - 10000),
			);
	...
	suite("computeUpdateInfoVersion", () => {
		test("returns minor .0 version when minor differs", () => {
			assert.strictEqual(
				computeUpdateInfoVersion("1.108.2", "1.109.5"),
				"1.109",
			);
			assert.strictEqual(
				computeUpdateInfoVersion("1.108.0", "1.109.0"),
				"1.109",
			);
			assert.strictEqual(
				computeUpdateInfoVersion("1.107.3", "1.110.1"),
				"1.110",
			);
		});
		...
		test("returns target version as-is when same minor", () => {
			assert.strictEqual(
				computeUpdateInfoVersion("1.109.2", "1.109.5"),
				"1.109.5",
			);
			assert.strictEqual(
				computeUpdateInfoVersion("1.109.0", "1.109.3"),
				"1.109.3",
			);
		});
		...
		test("returns undefined for invalid versions", () => {
			assert.strictEqual(
				computeUpdateInfoVersion("invalid", "1.109.5"),
				undefined,
			);
			assert.strictEqual(
				computeUpdateInfoVersion("1.109.2", "invalid"),
				undefined,
			);
		});
	});
	...
	suite("getUpdateInfoUrl", () => {
		test("constructs correct URL for .0 versions", () => {
			assert.strictEqual(
				getUpdateInfoUrl("1.109.0"),
				"https://code.visualstudio.com/raw/v1_109_update.md",
			);
		});

		test("constructs correct URL for patch versions", () => {
			assert.strictEqual(
				getUpdateInfoUrl("1.109.5"),
				"https://code.visualstudio.com/raw/v1_109_5_update.md",
			);
		});
	});
	...
	suite("isMajorMinorVersionChange", () => {
		...
		test("returns false when both versions are undefined", () => {
			assert.strictEqual(
				isMajorMinorVersionChange(undefined, undefined),
				false,
			);
		});
