...
export class NullTestService extends Disposable implements ITestService {
	...
	readonly excluded = this._register(
		new NullTestExclusions(),
	) as unknown as TestExclusions;
	readonly collection: IMainThreadTestCollection;
	readonly showInlineOutput = this._register(
		new MutableObservableValue<boolean>(false),
	);

	constructor(@IUriIdentityService uriIdentityService: IUriIdentityService) {
		super();
		this.collection = new MainThreadTestCollection(uriIdentityService, () =>
			Promise.resolve(),
		);
	}

	registerExtHost(): IDisposable {
		return Disposable.None;
	}
	registerTestController(): IDisposable {
		return Disposable.None;
	}
	getTestController() {
		return undefined;
	...
	async runResolvedTests(): Promise<ITestResult> {
		throw new Error("Tests are not supported in this window.");
	}
	...
	async getCodeRelatedToTest(): Promise<Location[]> {
		return [];
	}
}

export class NullTestProfileService
	extends Disposable
...
export class NullTestResultService
	extends Disposable
	implements ITestResultService
{
	...
	clear(): void {}
	createLiveResult(): LiveTestResult {
		throw new Error("Tests are not supported in this window.");
	}
	push<T extends ITestResult>(result: T): T {
		return result;
	}
	getResult(): ITestResult | undefined {
		return undefined;
	}
	getStateById() {
		return undefined;
