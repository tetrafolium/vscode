	...
	implements IEditorContribution
{
	...
	private readonly hasInlineCoverageDetails = observableValue(
		"hasInlineCoverageDetails",
		false,
	);
	...
	constructor(
		...
		this.summaryWidget = new Lazy(() =>
			this._register(
				instantiationService.createInstance(CoverageToolbarWidget, this.editor),
			),
		);

		const modelObs = observableFromEvent(this, editor.onDidChangeModel, () =>
			editor.getModel(),
		);
		...
		const minimapEnabled = observableConfigValue(
...
export class CoverageDetailsModel {
	...
	public describe(
		...
		if (detail.type === DetailType.Declaration) {
			...
			const text = wrapName(
				model.getValueInRange(tidyLocation(detail.detail.location)).trim() ||
					`<empty statement>`,
			);
...
class CoverageToolbarWidget extends Disposable implements IOverlayWidget {
	...
	private setActions() {
		...
		const hasUncoveredStmt =
			current.coverage.statement.covered < current.coverage.statement.total;
		...
		constructor() {
			super({
				...
				menu: [
					...
					{
						id: MenuId.EditorTitle,
						when: TestingContextKeys.hasCoverageInFile,
						group: "coverage",
						order: 1,
					},
				],
	...
	class FilterCoverageToTestInEditor extends Action2 {
		...
		run(
			...
			const items: QuickPickInput<TItem>[] = [
				...
				{ type: "separator" },
				...tests.map((id) => ({
					...coverUtils.getLabelForItem(result, id, commonPrefix),
					testId: id,
					buttons,
				})),
			];
			...
			quickInputService
				.pick(items, {
					activeItem: items.find(
						(item): item is TItem =>
							"testId" in item &&
							item.testId?.toString() === previousSelection?.toString(),
					),
					placeHolder: coverUtils.labels.pickShowCoverage,
					onDidTriggerItemButton: (context) => {
						commandService.executeCommand(
							"vscode.revealTest",
							context.item.testId?.toString(),
						);
					},
					onDidFocus: (entry) => {
						if (!entry.testId) {
								...
								(details) => {
									...
									if (!cts.token.isCancellationRequested && first) {
										activeEditor?.revealLineNearTop(
											first.location instanceof Position
			...
			super({
				...
				toggled: {
					condition: ContextKeyExpr.equals(
						"config.testing.showCoverageInExplorer",
						true,
					),
					title: localize(
...
registerAction2(
	class GoToNextMissedCoverageLine extends Action2 {
		...
		run(accessor: ServicesAccessor): void {
			...
			const contribution =
				activeEditor.getContribution<CodeCoverageDecorations>(
					CodeCoverageDecorations.ID,
				);
