...
import {
	...
	getTestProgressText,
} from "../common/testingProgressMessages.js";
import {
	cmpPriority,
	isFailedState,
	isStateWithResult,
...
class ResultSummaryView extends Disposable {
	...
	private renderActivityBadge(countSummary: CountSummary, isRunning: boolean) {
		if (isRunning) {
			...
			this.lastBadge = new IconBadge(spinningLoading, () =>
				localize("testingRunningBadge", "Tests are running"),
			);
		} else if (
			countSummary &&
...
class TestingExplorerViewModel extends Disposable {
	...
	private readonly welcomeVisibilityEmitter = this._register(
