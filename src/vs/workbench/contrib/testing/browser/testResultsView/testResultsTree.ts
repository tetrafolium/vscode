...
class TestCaseElement implements ITreeElement {
	...
	public get onDidChange() {
		...
		return Event.filter(
			...
			(e) =>
				e.item.item.extId === this.test.item.extId &&
...
export class OutputPeekTree extends Disposable {
	...
	constructor(
		...
		const getTaskChildren = (
			...
			let result: Iterable<ICompressedTreeElement<TreeElement>> = Iterable.map(
				tests,
				(test) => ({
			...
			new RunOnceScheduler(() => {
				for (const taskNode of taskChildrenToUpdate) {
					if (this.tree.hasElement(taskNode)) {
						this.tree.setChildren(taskNode, getTaskChildren(taskNode), {
				...
				if (subject instanceof TaskSubject) {
					const resultItem = this.tree.getNode(null).children.find((c) => {
						if (c.element instanceof TaskElement) {
							return (
								c.element.results.id === subject.result.id &&
		...
		this._register(
			explorerFilter.onDidSelectTestInExplorer((testId) => {
				if (
					this.tree
						.getSelection()
						.some((e) => e && "test" in e && e.test.item.extId === testId)
				) {
					return;
				}
...
class TestRunElementRenderer implements ICompressibleTreeRenderer<
	ITreeElement,
	FuzzyScore,
	TemplateData
> {
	...
	public provideActionBar(element: ITreeElement) {
		...
		if (element instanceof TaskElement) {
			...
			if (element.task.running) {
				...
				const hasFailedTests = Iterable.some(element.results.tests, (test) =>
					isFailedState(test.ownComputedState),
				);
				if (hasFailedTests) {
					primary.push(
						new Action(
		...
		if (element instanceof TestResultElement) {
			...
			const hasFailedTests = Iterable.some(element.value.tests, (test) =>
				isFailedState(test.ownComputedState),
			);
			if (hasFailedTests) {
				primary.push(
