...
CommandsRegistry.registerCommand({
	...
	handler: async (
		accessor: ServicesAccessor,
		profileRef: ITestRunProfileReference,
		tests: readonly ITestItem[],
	) => {
	...
	handler: async (
		...
		const position =
			accessor
				...
				.getDecoratedTestPosition(uri, extId) || range?.getStartPosition();
...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor: ServicesAccessor, uri: URI) => {
		const testService = accessor.get(ITestService);
		return [...testService.collection.getNodeByUrl(uri)].map((t) =>
			TestId.split(t.item.extId),
		);
	},
});
