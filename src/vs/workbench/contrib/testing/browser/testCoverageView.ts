...
export class TestCoverageView extends ViewPane {
	private readonly tree = this._register(
		new MutableDisposable<TestCoverageTree>(),
	);
	...
	constructor(
		...
		const storedOrder = this.storageService.getNumber(
...
class TestCoverageTree extends Disposable {
	...
	public setInput(coverage: TestCoverage, showOnlyTest?: TestId) {
		...
		const filteredTo =
			showOnlyTest && coverage.result.getTestById(showOnlyTest.toString());
		if (filteredTo) {
...
> {
	...
	private renderInner(
		element: CurrentlyFilteredTo,
		container: IFilteredToTemplate,
	) {
	...
	class TestCoverageChangePerTestFilterAction extends Action2 {
		...
		override run(accessor: ServicesAccessor): void {
			...
			const items: QuickPickInput<TItem>[] = [
				...
				{ type: "separator" },
				...tests.map((testId) => ({
					...coverUtils.getLabelForItem(result, testId, commonPrefix),
					testId,
				})),
			];
