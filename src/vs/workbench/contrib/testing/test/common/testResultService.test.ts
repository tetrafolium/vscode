...
suite("Workbench - Test Results Service", () => {
	const getLabelsIn = (it: Iterable<TestResultItem>) =>
		[...it].map((t) => t.item.label).sort();
	const getChangeSummary = () =>
		[...changed].map((c) => ({ reason: c.reason, label: c.item.item.label }));
	...
	setup(async () => {
		...
		if (!ok) {
			throw new Error(
				"timed out while expanding, diff: " +
					JSON.stringify(tests.collectDiff()),
			);
		}
	...
	suite("LiveTestResult", () => {
		...
		test("setAllToState", () => {
			...
			assert.deepStrictEqual(
				r.getStateById(new TestId(["ctrlId", "id-a"]).toString())?.tasks[0]
					.state,
				TestResultState.Failed,
			);
			assert.deepStrictEqual(getChangeSummary(), [
		...
		const makeHydrated = async (
			...
			new HydratedTestResult(
				...
				} as IUriIdentityService,
				{
					...
					id: "some-id",
					tasks: [
						{ id: "t", name: undefined, ctrlId: "ctrl", hasCoverage: false },
					],
					name: "hello world",
					request: defaultOpts([]),
					items: [
