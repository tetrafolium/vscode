...
import {
	...
	mergeLineRanges,
	getFailureDetails,
} from "../../common/testingChatAgentTool.js";
...
suite("Workbench - RunTestTool", () => {
	...
	function createTestCoverage(
		files: {
			...
			declaration?: { covered: number; total: number };
	...
	function createResultWithCoverage(
		...
		const taskCov = result.tasks[0].coverage as ReturnType<
			typeof observableValue<TestCoverage | undefined>
		>;
		taskCov.set(coverageData, undefined);
		return result;
	}

	function createResultWithTests(
		tests: {
			extId: string;
			label: string;
				...
				for (const msg of t.messages) {
					result.appendMessage(t.extId, "t", {
						...
						location: msg.location
							? { uri: msg.location.uri, range: msg.location.range }
							: undefined,
						stackTrace: msg.stackTrace?.map((f) => ({
							uri: f.uri,
							position: f.position
								? new Position(f.position.lineNumber, f.position.column)
								: undefined,
							label: f.label,
						})),
					});
	...
	setup(() => {
		...
		const mockProfileService = upcastPartial<ITestProfileService>({
			capabilitiesForTest: () =>
				TestRunProfileBitset.Run | TestRunProfileBitset.Coverage,
		});
		...
		const mockWorkspaceContext = upcastPartial<IWorkspaceContextService>({
			getWorkspace: () =>
				upcastPartial<IWorkspace>({
					id: "test",
					folders: [
						upcastPartial<IWorkspaceFolder>({ uri: URI.file("/workspace") }),
					],
				}),
		});
		...
		test("returns error when no tests found", async () => {
			const result = await tool.invoke(
				upcastPartial<IToolInvocation>({
					parameters: { files: ["/nonexistent/test.ts"] },
				}),
				noopCountTokens,
				noopProgress,
			...
			assert.ok(
				result.content[0].kind === "text" &&
					result.content[0].value.includes("No tests found"),
			);
		});
	});
	...
	suite("_buildSummary", () => {
		test("includes pass/fail counts", async () => {
			const result = createResultWithTests([
				{
					extId: new TestId(["ctrlId", "a"]).toString(),
					label: "a",
					state: TestResultState.Passed,
				},
				{
					extId: new TestId(["ctrlId", "b"]).toString(),
					label: "b",
					state: TestResultState.Failed,
					messages: [{ type: TestMessageType.Error, message: "boom" }],
				},
			]);
			result.markComplete();
		...
		test("combines errored and failed in failure count", async () => {
			const result = createResultWithTests([
				...
				},
				{
					...
					messages: [{ type: TestMessageType.Error, message: "error" }],
				},
				{
					extId: new TestId(["ctrlId", "c"]).toString(),
					label: "c",
					state: TestResultState.Passed,
				},
			]);
			...
			const coverageData = createTestCoverage([
				{
					uri: fileA,
					statement: { covered: 8, total: 10 },
					details: [makeStatement(5, 0)],
				},
				{
					uri: fileB,
					statement: { covered: 3, total: 10 },
					details: [makeDeclaration("fn", 1, 0)],
				},
			]);
			const result = createResultWithCoverage(coverageData);

			const summary = await getCoverageSummary(result, [
				fileA.fsPath,
				fileB.fsPath,
			]);
			assert.ok(summary.includes(fileA.fsPath));
	...
	suite("getFileCoverageDetails", () => {
		...
		test("lists uncovered declarations", async () => {
			...
			const file = createFileCov(uri, { covered: 8, total: 10 }, details, {
				declaration: { covered: 1, total: 3 },
			});
			const output = await getFileCoverageDetails(file, uri.fsPath);
			assert.ok(
				output.includes(
					"uncovered functions: handleError(L89), processQueue(L120)",
				),
			);
		...
		test("lists uncovered branches with labels", async () => {
			...
			const file = createFileCov(uri, { covered: 8, total: 10 }, details, {
				branch: { covered: 2, total: 4 },
			});
			const output = await getFileCoverageDetails(file, uri.fsPath);
			assert.ok(
				output.includes('uncovered branches: L36(else), L56(case "foo")'),
			);
		});
		...
		test("omits uncovered sections when all covered", async () => {
			...
			const details: CoverageDetails[] = [
				...
				makeStatement(20, 5),
				makeStatement(30, 1, undefined, [
					makeBranch(30, 1, "then"),
					makeBranch(32, 2, "else"),
				]),
			];
		...
		test("handles details() throwing gracefully", async () => {
			...
			const file = new FileCoverage(
				{ id: "err", uri, statement: { covered: 5, total: 10 } },
				result,
				accessor,
			);
			const output = await getFileCoverageDetails(file, uri.fsPath);
	...
	suite("getFailureDetails", () => {
		...
		test("includes stack trace frames", async () => {
			...
			assert.ok(
				output.includes(`path="${helperUri.fsPath}">helperFn</stackFrame>`),
			);
		...
		test("skips passing tests", async () => {
			const result = createResultWithTests([
				{
					extId: new TestId(["ctrlId", "pass"]).toString(),
					label: "pass",
					state: TestResultState.Passed,
				},
				{
					extId: new TestId(["ctrlId", "fail"]).toString(),
					label: "fail",
					state: TestResultState.Failed,
					messages: [{ type: TestMessageType.Error, message: "boom" }],
				},
	...
	suite("prepareToolInvocation", () => {
		test("shows file names in confirmation", async () => {
			const prepared = await tool.prepareToolInvocation(
				upcastPartial<IToolInvocationPreparationContext>({
					parameters: { files: ["/path/to/test1.ts", "/path/to/test2.ts"] },
					toolCallId: "call-1",
