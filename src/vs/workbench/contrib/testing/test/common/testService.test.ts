	...
	suite("testsInFile", () => {
		test("canonicalizes URI before comparing with stored test URIs (#275268)", async () => {
			...
			collection.resolveHandler = (item) => {
				if (item === undefined) {
					collection.root.children.add(
						new TestTestItem(
							new TestId(["ctrlId", "test1"]),
							"test1",
							canonicalUri,
						),
					);
			...
			const ident = upcastPartial<IUriIdentityService>({
				asCanonicalUri: (u: URI) =>
					extUri.isEqual(u, rawUri) ? canonicalUri : u,
				extUri,
			});
			...
			const found: string[] = [];
			for await (const batch of testsInFile(
				testService,
				ident,
				rawUri,
				false,
			...
			assert.deepStrictEqual(found, [
				new TestId(["ctrlId", "test1"]).toString(),
			]);
		});
		...
		test("indexes live diff-added tests by canonical URI and still finds them from raw URI input", async () => {
			...
			const collection = new MainThreadTestCollection(
				{
					asCanonicalUri: (u: URI) =>
						u.toString() === rawUri.toString() ? canonicalUri : u,
				},
				async () => {},
			);
			...
			assert.ok(
				extUri.isEqual(added.item.uri, canonicalUri),
				"added test URI should be canonicalized on apply",
			);
			assert.deepStrictEqual(
				[...collection.getNodeByUrl(canonicalUri)].map((t) => t.item.extId),
				[testId],
			);
			assert.deepStrictEqual(
				[...collection.getNodeByUrl(rawUri)].map((t) => t.item.extId),
				[],
			);
