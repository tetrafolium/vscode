...
suite("Code Coverage Decorations", () => {
	...
	test("hasInlineCoverageDetails context key", () => {
		...
		const modelWithRanges = new CoverageDetailsModel(
			detailsWithRanges,
			textModel,
		);
		...
		assert.strictEqual(
			modelWithRanges.ranges.length > 0,
			true,
			"Model with coverage details should have ranges",
		);
		...
		assert.strictEqual(
			emptyModel.ranges.length === 0,
			true,
			"Model with no coverage details should have no ranges",
		);
	});
});
