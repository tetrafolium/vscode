...
import {
	Extensions as DndExtensions,
	IDragAndDropContributionRegistry,
	IResourceDropHandler,
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this.hasProfilesContext.set(
			this.userDataProfilesService.profiles.filter((p) => !p.isInternal)
				.length > 1,
		);
		this._register(
	...
	private registerDropHandler(): void {
		const dndRegistry = Registry.as<IDragAndDropContributionRegistry>(
			DndExtensions.DragAndDropContribution,
		);
		const that = this;
		this._register(
			dndRegistry.registerDropHandler(
				new (class UserDataProfileDropHandler implements IResourceDropHandler {
					async handleDrop(
						resource: URI,
						...
						if (
							uriIdentityService.extUri.extname(resource) ===
							`.${PROFILE_EXTENSION}`
						) {
							const template =
								await userDataProfileImportExportService.resolveProfileTemplate(
									resource,
								);
							if (!template) {
								...
								editorService.openEditor(
									textEditorService.createTextEditor({ resource }),
								);
								return true;
							}
				...
				async run(accessor: ServicesAccessor) {
					...
					for (const profile of that.userDataProfilesService.profiles) {
						...
						items.push({
							id: profile.id,
							label:
								profile.id === that.userDataProfileService.currentProfile.id
									? `$(check) ${profile.name}`
									: profile.name,
							profile,
						});
					...
					const result = await quickInputService.pick(
						items.sort((a, b) => a.profile.name.localeCompare(b.profile.name)),
						{
				...
				class ManageProfilesAction extends Action2 {
						...
						super({
							...
							title: {
								...
								mnemonicTitle: localize(
									{ key: "miOpenProfiles", comment: ["&& denotes a mnemonic"] },
									"&&Profiles",
								),
							},
							menu: [
					...
					run(accessor: ServicesAccessor) {
						...
						return editorService.openEditor(
							new UserDataProfilesEditorInput(instantiationService),
						);
					}
				},
			),
						...
						if (profiles.length) {
							const picks = await quickInputService.pick(
								profiles.map((profile) => ({
									label: profile.name,
									description:
										profile.id === userDataProfileService.currentProfile.id
