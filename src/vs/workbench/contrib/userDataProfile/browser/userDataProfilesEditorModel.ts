...
import {
	McpProfileResource,
	McpResourceTreeItem,
} from "../../../services/userDataProfile/browser/mcpProfileResource.js";
...
export abstract class AbstractUserDataProfileElement extends Disposable {
	...
	protected async getChildrenFromProfile(
		...
		switch (resourceType) {
			...
			case ProfileResourceType.Mcp:
				children = await this.instantiationService
					.createInstance(McpResourceTreeItem, profile)
					.getChildren();
				break;
	...
	protected toUserDataProfileResourceChildElement(
		...
		return {
			...
			label: child.label
				? isMarkdownString(child.label.label)
					? child.label.label.value
					: child.label.label
				: "",
		...
		switch (resourceType) {
			...
			case ProfileResourceType.Mcp:
				if (profileTemplate.mcp) {
					await this.instantiationService
						.createInstance(McpProfileResource)
						.apply(profileTemplate.mcp, profile);
					return this.getChildrenFromProfile(profile, resourceType);
				}
				return [];
			case ProfileResourceType.Extensions:
				...
				if (createAction.enabled) {
					if (
						...
						this.userDataProfilesService.profiles.some(
							(p) => !p.isInternal && p.name === this.newProfileElement?.name,
						)
					) {
