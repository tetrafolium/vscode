...
suite("LanguageStatus - Dedicated Entry Deduplication", () => {
	...
	test("reuses existing entry from previous update", () => {
		...
		assert.strictEqual(
			result.get("status-A"),
			oldEntry,
			"should reuse the existing entry",
		);
		assert.strictEqual(
			oldEntry.updateCount,
			1,
		...
		assert.strictEqual(
			oldEntry.disposed,
			false,
			"should not dispose reused entry",
		);
	});
	...
	test("creates new entry when none exists", () => {
		...
		assert.strictEqual(
			entry.updateCount,
			0,
			"should not have called update on new entry",
		);
		assert.strictEqual(
			entry.disposed,
			false,
			"new entry should not be disposed",
		);
	});
	...
	test("duplicate status IDs - fixed version reuses entry from current update", () => {
		...
		assert.strictEqual(
			createdEntries[0].updateCount,
			1,
			"entry should be updated once for the duplicate",
		);
		assert.strictEqual(
			createdEntries[0].disposed,
			false,
			"the entry should not be disposed",
		);
	});
	...
	test("duplicate status IDs - buggy version leaks entry", () => {
		...
		assert.strictEqual(
			createdEntries.length,
			2,
			"buggy version creates two entries",
		);
		assert.strictEqual(
			result.size,
			1,
			"result map has one entry (second overwrites first)",
		);
		// The first entry is orphaned - it's not in the result map and not disposed
		assert.strictEqual(
			createdEntries[0].disposed,
			false,
			"first entry is NOT disposed (leaked!)",
		);
		assert.notStrictEqual(
			result.get("status-A"),
			createdEntries[0],
			"first entry is not in the result",
		);
		assert.strictEqual(
			result.get("status-A"),
			createdEntries[1],
			"second entry is in the result",
		);
	});
	...
	test("duplicate IDs with existing entry - fixed version reuses existing", () => {
		...
		assert.strictEqual(
			oldEntry.updateCount,
			2,
			"should be updated twice (once per duplicate)",
		);
	...
	test("mixed unique and duplicate IDs", () => {
		...
		assert.strictEqual(
			createdEntries.length,
			1,
			"should create one new entry (for first status-A)",
		);
		assert.strictEqual(result.size, 2, "result map should have two entries");
		assert.strictEqual(
			result.get("status-A"),
			createdEntries[0],
			"status-A should use created entry",
		);
		assert.strictEqual(
			createdEntries[0].updateCount,
			1,
			"status-A entry updated once for duplicate",
		);
		assert.strictEqual(
			result.get("status-B"),
			existingB,
			"status-B should reuse existing entry",
		);
