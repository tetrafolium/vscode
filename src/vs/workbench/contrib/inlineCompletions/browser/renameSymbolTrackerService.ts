		...
		this._register(
			this._model.onDidChangeContent((e) => {
				if (!isUserEdit(e)) {
					...
					if (
						position !== undefined &&
						position.lineNumber <= this._model.getLineCount()
					) {
...
class RenameSymbolTrackerService
	extends Disposable
	implements IRenameSymbolTrackerService
{
	public _serviceBrand: undefined;

	private readonly _modelTrackers = new Map<
		ITextModel,
		ModelSymbolRenameTracker
	>();
	private readonly _editorFocusTrackingDisposables = new Map<
		ICodeEditor,
		IDisposable
	>();

	private readonly _focusedModelTracker = observableValue<
		ModelSymbolRenameTracker | undefined
	>(this, undefined);

	public readonly trackedWord: IObservable<ITrackedWord | undefined> = derived(
		this,
		(reader) => {
		...
		this._register(
			this._codeEditorService.onCodeEditorRemove((editor) => {
				const focusDisposable =
					this._editorFocusTrackingDisposables.get(editor);
				if (focusDisposable) {
