		...
		this._someAccessibilitySignalIsEnabled = derived(this, (reader) =>
			this._textProperties
				...
				.some((signal) =>
					observableFromValueWithChangeEvent(
						this,
						this._accessibilitySignalService.getEnabledState(signal, false),
					).read(reader),
				),
		);
		...
		store.add(
			editor.onDidChangeCursorPosition((args) => {
				...
				const trigger = (
					...
					const signal =
						mode === "line" ? property.lineSignal : property.positionSignal;
					if (
						!signal ||
						!this._accessibilitySignalService.getEnabledState(signal, false)
							.value ||
							...
							timeouts.add(
								disposableTimeout(() => {
									if (source.isPresent(position, mode, undefined)) {
										if (
											!(mode === "line") ||
											!ignoredLineSignalsForCurrentLine.has(property)
										) {
											this._accessibilitySignalService.playSignal(signal, {
												modality,
											});
										}
				...
				for (const s of propertySources) {
					if (
						![s.property.lineSignal, s.property.positionSignal].some(
							(s) =>
								s &&
								this._accessibilitySignalService.getEnabledState(s, false)
									.value,
						)
					) {
						return;
