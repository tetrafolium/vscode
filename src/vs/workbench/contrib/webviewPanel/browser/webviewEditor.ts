...
export class WebviewEditor extends EditorPane {
	...
	public override focus(): void {
		...
		if (!this._onFocusWindowHandler.value && !isWeb) {
			...
			this._onFocusWindowHandler.value = this._hostService.onDidChangeFocus(
				(focused) => {
					if (
						focused &&
	...
	private claimWebview(input: WebviewInput): void {
		...
		const modalEditorContainer =
			this._editorGroupsService.activeModalEditorPart?.modalElement;
		const isModal =
			isHTMLElement(modalEditorContainer) &&
			this._element &&
			modalEditorContainer.contains(this._element);
		this._clippingContainer = isModal
			? undefined
			: this._workbenchLayoutService.getContainer(
					this.window,
					Parts.EDITOR_PART,
				);
	...
	private setWebviewAnchorElement(webview: IOverlayWebview) {
		...
		webview.setAnchorElement(
			this._element.parentElement!,
			this._clippingContainer,
		);
	}
