...
import {
	IKeymapInfo,
	KeymapInfo,
} from "../../../services/keybinding/common/keymapInfo.js";
...
import {
	KeybindingIO,
	OutputBuilder,
} from "../../../services/keybinding/common/keybindingIO.js";
import { getAllUnboundCommands } from "../../../services/keybinding/browser/unboundCommands.js";

export class KeybindingsExportContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		if (outputPath !== undefined) {
			...
			void this.extensionService
				...
				.then(() => {
					return this.exportDefaultKeybindingsAndQuit(
						outputPath || defaultPath,
					);
				})
				.catch(async (error) => {
					this.logService.error(
						`[${KeybindingsExportContribution.ID}] Failed to register installed extensions before exporting default keybindings`,
						error,
					);
	...
	private async exportDefaultKeybindingsAndQuit(
		outputPath: string,
	): Promise<void> {
		try {
			...
			for (const { os, filename } of platforms) {
				const content =
					KeybindingsExportContribution._getDefaultKeybindingsContentForOS(
						os,
						extensions,
					);
				...
				this.logService.info(
					`[${KeybindingsExportContribution.ID}] Wrote ${filePath}`,
				);
			}
			...
			this.logService.error(
				`[${KeybindingsExportContribution.ID}] Failed to generate default keybindings`,
				error,
			);
			await this.nativeHostService.closeWindow();
		}
	...
	private static _getDefaultKeybindingsContentForOS(
		...
		const resolved =
			KeybindingsExportContribution._resolveKeybindingItemsWithMapper(
				items,
				mapper,
			);
		...
		return (
			KeybindingsExportContribution._formatDefaultKeybindings(
				defaultKeybindings,
			) +
			"\n\n" +
	...
	private static _getExtensionKeybindingsForOS(
		extensions: readonly IExtensionDescription[],
		os: OperatingSystem,
	): IKeybindingItem[] {
		...
		for (const ext of extensions) {
			...
			for (let i = 0; i < bindings.length; i++) {
				...
				const keyStr = KeybindingsExportContribution._bindToOS(
					binding.key,
					binding.mac,
					binding.linux,
					binding.win,
				...
				let when = binding.when
					? ContextKeyExpr.deserialize(binding.when)
					: undefined;
	...
	private static _createKeyboardMapperForOS(
		os: OperatingSystem,
	): IKeyboardMapper {
		...
		const keymapInfo = new KeymapInfo(
			layout.layout,
			layout.secondaryLayouts,
			layout.mapping,
		);
		switch (os) {
			case OperatingSystem.Windows:
				return new WindowsKeyboardMapper(
					true,
					<IWindowsKeyboardMapping>keymapInfo.mapping,
					false,
				);
			case OperatingSystem.Macintosh:
				return new MacLinuxKeyboardMapper(
					true,
					<IMacLinuxKeyboardMapping>keymapInfo.mapping,
					false,
	...
	private static _resolveKeybindingItemsWithMapper(
	...
	private static _formatDefaultKeybindings(
	...
	private static _formatAllCommandsAsComment(
