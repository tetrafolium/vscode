...
suite("Debug - ANSI Handling", () => {
	...
	function getSequenceOutput(sequence: string): HTMLSpanElement {
		...
		const root: HTMLSpanElement = handleANSIOutput(
			sequence,
			linkDetector,
			session.root,
			[],
	 ...
	 */
	function assertSingleSequenceElement(
		sequence: string,
	...
	function assertInlineColor(
		...
		if (color !== undefined) {
			...
			if (colorType === "background") {
				...
				assert(
					(styleBefore === element.style.textDecorationColor) ===
						colorShouldMatch,
					message ||
						`Incorrect ${colorType} color style found (found color: ${styleBefore}, expected ${cssColor}).`,
				);
	 ...
	 */
	function assertMultipleSequenceElements(
		sequence: string,
		...
		assertMultipleSequenceElements(
			...
			[
				...
				(strikethrougheoff) => {
					...
					assert(
						strikethrougheoff.classList.contains("code-strike-through") ===
							false,
						"Strike through class found after strikethrough off code 29m",
					);
					assert.strictEqual(
		...
		for (let red = 0; red <= 5; red++) {
			for (let green = 0; green <= 5; green++) {
				for (let blue = 0; blue <= 5; blue++) {
					const colorOut: any = calcANSI8bitColor(
