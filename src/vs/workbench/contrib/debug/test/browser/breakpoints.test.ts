...
function addBreakpointsAndCheckEvents(
	model: DebugModel,
	uri: uri,
	data: IBreakpointData[],
) {
	let eventCount = 0;
	const toDispose = model.onDidChangeBreakpoints((e) => {
		...
		for (let i = 0; i < data.length; i++) {
			...
			assert.strictEqual(
				(e.added![i] as Breakpoint).lineNumber,
				data[i].lineNumber,
			);
		}
	});
...
suite("Debug - Breakpoints", () => {
	...
	test("exception breakpoints", () => {
		...
		assert.strictEqual(eventCount, 1);
		let exceptionBreakpoints =
			model.getExceptionBreakpointsForSession("session-id-1");
		...
		exceptionBreakpoints =
			model.getExceptionBreakpointsForSession("session-id-2");
	...
	test("exception breakpoints multiple sessions", () => {
		...
		let exceptionBreakpointsForSession =
			model.getExceptionBreakpointsForSession("session-id-4");
		...
		assert.strictEqual(eventCount, 2);
		exceptionBreakpointsForSession =
			model.getExceptionBreakpointsForSession("session-id-5");
		let exceptionBreakpointsForUndefined =
			model.getExceptionBreakpointsForSession(undefined);
		assert.strictEqual(exceptionBreakpointsForSession.length, 2);
		...
		assert.strictEqual(eventCount, 2);
		exceptionBreakpointsForUndefined =
			model.getExceptionBreakpointsForSession(undefined);
		assert.strictEqual(exceptionBreakpointsForUndefined.length, 2);
		assert.strictEqual(exceptionBreakpointsForUndefined[0].filter, "uncaught");
		assert.strictEqual(exceptionBreakpointsForUndefined[1].filter, "caught");
