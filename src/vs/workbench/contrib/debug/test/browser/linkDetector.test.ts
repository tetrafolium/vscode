...
suite("Debug - Link Detector", () => {
	...
	test("singleLineLinkAndText", function () {
		...
		const input = isWindows
			? "The link: C:/foo/bar.js:12:34"
			: "The link: /Users/foo/bar.js:12:34";
		const expectedOutput =
			/^<span>The link: <a tabindex="0">.*\/foo\/bar.js:12:34<\/a><\/span>$/;
		const output = linkDetector.linkify(input, hoverBehavior);
	...
	test("highlightNoLinks", () => {
		...
		const expectedOutput =
			'<span>I <span class="highlight">am </span>a string</span>';
		const output = linkDetector.linkify(
			input,
			hoverBehavior,
			false,
	...
	test("csharpStackTraceFormatWithLineAndColumn", () => {
		...
		const input = isWindows
			? "C:\\foo\\bar.cs:line 6:10"
			: "/Users/foo/bar.cs:line 6:10";
		const expectedOutput = isWindows
