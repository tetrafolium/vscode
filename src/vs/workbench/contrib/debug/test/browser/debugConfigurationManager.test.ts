...
suite("debugConfigurationManager", () => {
	...
	function createConfigurationManager(): ConfigurationManager {
		...
		return disposables.add(
			instantiationService.createInstance(ConfigurationManager, adapterManager),
		);
	}
	...
	test("uses remote target OS when computing visible configurations", async () => {
		...
		configurationService.setUserConfiguration("launch", {
			...
			configurations: [
				...
				},
				{
					...
					request: "launch",
					name: "linux-hidden",
					linux: { presentation: { hidden: true } },
				},
			],
		});
		...
		if (OS !== OperatingSystem.Linux) {
			await Event.toPromise(
				_debugConfigurationManager.onDidSelectConfiguration,
			);
		}

		assert.deepStrictEqual(
			_debugConfigurationManager.getAllConfigurations().map(({ name }) => name),
			["visible"],
		);
	});
