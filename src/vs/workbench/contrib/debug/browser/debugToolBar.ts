...
export class DebugToolBar extends Themable implements IWorkbenchContribution {
	...
	private readonly trackPixelRatioListener = this._register(
	...
	constructor(
		...
		const controlsOnTitlebar =
			getTitleBarStyle(this.configurationService) === TitlebarStyle.CUSTOM;
		...
		this.updateScheduler = this._register(
			new RunOnceScheduler(() => {
				...
				const actions = getFlatActionBarActions(
					this.debugToolBarMenu.getActions({ shouldForwardArgs: true }),
				);
				if (
					!arrays.equals(
						actions,
						this.activeActions,
						(first, second) =>
							first.id === second.id && first.enabled === second.enabled,
					)
				) {
	...
	private registerListeners(): void {
		this._register(
			this.debugService.onDidChangeState(() => this.updateScheduler.schedule()),
		);
		this._register(
		...
		this._register(
			this.debugToolBarMenu.onDidChange(() => this.updateScheduler.schedule()),
		);
		this._register(
				...
				(e: MouseEvent) => {
					...
					const mouseMoveListener = dom.addDisposableGenericMouseMoveListener(
						...
						(e: MouseEvent) => {
							...
							this.setCoordinates(
								originX +
									(mouseMoveEvent.posx - originEvent.posx) /
		...
		this._register(
			this.layoutService.onDidChangePartVisibility(() => this.setCoordinates()),
		);
	...
	private getStoredXPosition() {
		...
		return storedPercentage !== undefined && !isNaN(storedPercentage)
			? storedPercentage
			: 0.5;
	}
	...
	private get yRange(): [number, number] {
		if (!this._yRange) {
			...
			if (
				this.configurationService.getValue(LayoutSettings.EDITOR_TABS_MODE) !==
				EditorTabsMode.NONE
			) {
				yMax += 35;
			}
