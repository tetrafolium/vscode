...
export class BreakpointsView extends ViewPane {
	...
	private getPresentation(): "tree" | "list" {
		return this.configurationService.getValue<"tree" | "list">(
			...
			],
			{
				...
				keyboardNavigationLabelProvider: {
					...
					getCompressedNodeKeyboardNavigationLabel: (
						elements: BreakpointTreeElement[],
					) => {
						return elements
							.map((e) => {
			...
			this.tree.onMouseMiddleClick(async ({ element }) => {
				if (element instanceof Breakpoint) {
					...
					await this.debugService.removeBreakpoints(
						element.breakpoints.map((bp) => bp.getId()),
					);
				}
			}),
		);
		...
		this._register(
			this.tree.onKeyDown((e) => {
				...
				if (
					...
					!dom.isEditableElement(e.target as HTMLElement)
				) {
	...
	private onTreeContextMenu(
		e: ITreeContextMenuEvent<BreakpointTreeElement | null>,
	): void {
		const element = e.element;
		if (element instanceof BreakpointsFolderItem) {
		...
		this.breakpointHasMultipleModes.set(
			this.debugService
				.getModel()
				.getBreakpointModes(getModeKindForBreakpoint(element as IBreakpoint))
				.length > 1,
		);
		...
		const debuggerHasUnverifiedBps =
			...
			this.debugService
				...
				.filter((bp) => {
					...
					const langId =
						this.languageService.guessLanguageIdByFilepathOrFirstLine(bp.uri);
	...
	private onStateChange(): void {
		if (this.isBodyVisible()) {
			...
			if (!found) {
				...
				if (
					this.autoFocusedElement &&
					equals(focus, selection) &&
					selection.includes(this.autoFocusedElement)
				) {
					this.tree.setFocus([]);
	...
	private getTreeElements(): ICompressedTreeElement<BreakpointTreeElement>[] {
		...
		if (showAsTree && sourceBreakpoints.length > 0) {
			...
			for (const [uriStr, breakpoints] of breakpointsByUri) {
				...
				const children: ICompressedTreeElement<BreakpointTreeElement>[] =
					breakpoints.map((bp) => ({
						element: bp,
...
class BreakpointsDelegate implements IListVirtualDelegate<BreakpointTreeElement> {
	...
	getTemplateId(element: BreakpointTreeElement): string {
		...
		if (element instanceof FunctionBreakpoint) {
			...
			if (
				!element.name ||
				(inputBoxBreakpoint && inputBoxBreakpoint.getId() === element.getId())
			) {
...
> {
	...
	renderElement(
		...
		data.name.textContent = this.labelService.getUriBasenameLabel(
			folderItem.uri,
		);
		data.container.classList.toggle(
			"disabled",
			!this.debugService.getModel().areBreakpointsActivated(),
		);
...
class BreakpointsRenderer implements ICompressibleTreeRenderer<
	IBreakpoint,
	void,
...
> {
	...
	renderElement(
		node: ITreeNode<IBreakpoint, void>,
		index: number,
	...
	private renderBreakpointCommon(
	...
	private renderBreakpointFileLabel(
		breakpoint: IBreakpoint,
		data: IBreakpointTemplateData,
	): void {
	...
	private renderBreakpointLineLabel(
		breakpoint: IBreakpoint,
		data: IBreakpointTemplateData,
	): void {
		...
		this.textModelService
			...
			.then((reference) => {
				...
				if (model && breakpoint.lineNumber <= model.getLineCount()) {
					const lineContent = model
						.getLineContent(breakpoint.lineNumber)
						.trim();
					data.name.textContent =
						lineContent || localize("emptyLine", "(empty line)");
				} else {
			...
			.catch(() => {
				if (data.context === breakpoint) {
					data.name.textContent = localize(
						"cannotLoadLine",
						"(cannot load line)",
					);
				}
...
class DataBreakpointsRenderer implements ICompressibleTreeRenderer<
	DataBreakpoint,
	void,
	IDataBreakpointTemplateData
> {
	constructor(
		private menu: IMenu,
		private breakpointHasMultipleModes: IContextKey<boolean>,
	...
	renderCompressedElements(
		...
		this.renderDataBreakpoint(
			node.element.elements[node.element.elements.length - 1],
			data,
		);
	}

	private renderDataBreakpoint(
...
> {
	...
	renderCompressedElements(
		...
		this.renderInstructionBreakpoint(
			node.element.elements[node.element.elements.length - 1],
			data,
		);
	}
...
export function getBreakpointMessageAndIcon(
	...
	if (breakpoint instanceof DataBreakpoint) {
		...
		return {
			...
			message:
				breakpoint.message || localize("dataBreakpoint", "Data Breakpoint"),
		};
	}
	...
	if (
		...
		if (breakpoint.logMessage) {
			messages.push(
				localize("logMessage", "Log Message: {0}", breakpoint.logMessage),
			);
		}
		if (breakpoint.condition) {
			messages.push(
...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const currentPresentation = configurationService.getValue<
				"list" | "tree"
			>("debug.breakpointsView.presentation");
