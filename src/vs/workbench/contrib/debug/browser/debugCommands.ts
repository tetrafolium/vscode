...
export const TOGGLE_EXCEPTION_BREAKPOINTS_ID =
	"debug.toggleExceptionBreakpoints";
export const ATTACH_TO_CURRENT_CODE_RENDERER =
	"debug.attachToCurrentCodeRenderer";
...
export function setDataBreakpointInfoResponse(
	resp: IDataBreakpointInfoResponse | undefined,
) {
...
function isThreadContext(obj: any): obj is CallStackContext {
	return (
		obj && typeof obj.sessionId === "string" && typeof obj.threadId === "string"
	);
}
...
registerAction2(
	class AddConfigurationAction extends Action2 {
		...
		async run(accessor: ServicesAccessor, launchUri: string): Promise<void> {
			...
			const launch =
				manager.getLaunches().find((l) => l.uri.toString() === launchUri) ||
...
registerAction2(
	class ToggleExceptionBreakpointsAction extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const session =
				debugService.getViewModel().focusedSession ||
				debugModel.getSessions()[0];
			...
			if (exceptionBreakpoints.length === 1) {
				...
				await debugService.enableOrDisableBreakpoints(
					!breakpoint.enabled,
					breakpoint,
				);
				return;
			...
			const quickPick = disposables.add(
				quickInputService.createQuickPick<IExceptionBreakpointItem>(),
			);
	...
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor): Promise<any> {
			...
			if (!env.isExtensionDevelopment && !env.extensionTestsLocationURI) {
				throw new Error(
					"Refusing to attach to renderer outside of development context",
				);
			}
			...
			const result =
				await extDebugService.attachToCurrentWindowRenderer(windowId);
