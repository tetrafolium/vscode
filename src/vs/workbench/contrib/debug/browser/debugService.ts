...
export class DebugService implements IDebugService {
		...
		if (configByProviders && configByProviders.type) {
			try {
				...
				if (
					...
					resolvedConfig &&
					resolvedConfig.suppressMultipleSessionWarning !== true
				) {
					...
					if (existingSession) {
						...
						const confirmed = await this.confirmConcurrentSession(
							existingSession.getLabel(),
						);
						if (!confirmed) {
							return false;
						}
	...
	private async confirmConcurrentSession(
		sessionLabel: string,
	): Promise<boolean> {
	...
	private async makeTriggeredBreakpointsMatchEnablement(
		...
		await Promise.all(
			this.model
				...
				.filter(
					(bp) =>
						bp.triggeredBy === breakpoint.getId() && bp.enabled !== enable,
				)
