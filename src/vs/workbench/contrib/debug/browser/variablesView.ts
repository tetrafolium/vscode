...
async function getContextForVariableMenuWithDataAccess(
	...
	const dataBreakpointInfoResponse = await session.dataBreakpointInfo(
		variable.name,
...
class ScopesRenderer implements ITreeRenderer<
	IScope,
	FuzzyScore,
	IScopeTemplateData
> {
...
class ScopeErrorRenderer implements ITreeRenderer<
	IScope,
	FuzzyScore,
	IScopeErrorTemplateData
> {
...
export class VariablesRenderer extends AbstractExpressionsRenderer {
	...
	protected getInputBoxOptions(expression: IExpression): IInputBoxOptions {
		...
		return {
			...
			validationOptions: {
				validation: () =>
					variable.errorMessage ? { content: variable.errorMessage } : null,
			},
			onFinish: (value: string, success: boolean) => {
		...
		this.visualization
			...
			.then((result) => {
				...
				const originalExpression =
					(expression instanceof VisualizedExpression && expression.original) ||
					expression;
...
class VariablesAccessibilityProvider implements IListAccessibilityProvider<
	IExpression | IScope
> {
	...
	handler: async (
		...
		if (!arg) {
			...
			if (focusedView?.id === WATCH_VIEW_ID) {
				view =
					viewService.getActiveViewWithId<IDebugViewWithVariables>(
						WATCH_VIEW_ID,
					);
		...
		if (
			...
			*/
			telemetryService.publicLog("debug/didViewMemory", {
				debugType: debugService.getModel().getSession(sessionId)?.configuration
					.type,
			});
...
CommandsRegistry.registerCommand({
	...
	handler: async (
		accessor: ServicesAccessor,
		context: IVariablesContext | Variable,
	) => {
		const clipboardService = accessor.get(IClipboardService);
