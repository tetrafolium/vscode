			...
			for (const watch of watches) {
				picks.push({
					...
					asAttachment: (): IChatRequestVariableEntry[] =>
						createDebugAttachments(stackFrame, createDebugVariableEntry(watch)),
				});
			}
		}
					...
					picks.push({
						...
						asAttachment: (): IChatRequestVariableEntry[] =>
							createDebugAttachments(
								stackFrame,
								createScopeEntry(scope, variables),
							),
					});
				}
				for (const variable of variables) {
					picks.push({
						...
						asAttachment: (): IChatRequestVariableEntry[] =>
							createDebugAttachments(
								stackFrame,
								createDebugVariableEntry(variable),
							),
					});
				}
			} catch {
				...
				return [
					{
						...
						iconClass: ThemeIcon.asClassName(Codicon.symbolVariable),
						asAttachment: (): IChatRequestVariableEntry[] =>
							createDebugAttachments(stackFrame, {
								kind: "debugVariable",
			...
			return [
				{
					...
					description:
						err instanceof Error
							? err.message
							: localize("evaluationError", "Evaluation error"),
					disabled: true,
					asAttachment: () => "noop",
				},
			];
...
function createDebugVariableEntry(
...
function createPausedLocationEntry(
...
function createDebugAttachments(
	stackFrame: IStackFrame,
	variableEntry: IDebugVariableEntry,
): IChatRequestVariableEntry[] {
	return [createPausedLocationEntry(stackFrame), variableEntry];
}

function createScopeEntry(
	scope: IScope,
	variables: IExpression[],
): IDebugVariableEntry {
	const variablesSummary = variables
...
export class DebugChatContextContribution
	extends Disposable
...
function createDebugVariableEntryFromContext(
	context: unknown,
): IDebugVariableEntry | undefined {
