...
export class StartDebugActionViewItem extends BaseActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		this.start = dom.append(
			container,
			dom.$(ThemeIcon.asCSSSelector(debugStart)),
		);
		const title = this.keybindingService.appendKeybinding(
			this.action.label,
			this.action.id,
		);
		this.toDispose.push(
		...
		this.configurationContainer = dom.append(
			container,
			dom.$(".configuration"),
		);

		this.dropdown = new ActionWidgetDropdown(
			this.configurationContainer,
			{
				...
				listOptions: {
					showFilter: true,
					filterPlaceholder: nls.localize(
		...
		this.toDispose.push(
			this.dropdown.onDidChangeVisibility((visible) => {
	...
	private renderDropdownLabel(): void {
		...
		const currentLabel =
			this.debugOptions[this.selected]?.label ??
			nls.localize("noConfigurations", "No Configurations");
		const labelSpan = dom.$(
			"span.start-debug-action-item-label",
			undefined,
			currentLabel,
		);
		...
		this.dropdownLabel.setAttribute(
			"aria-label",
			nls.localize(
	...
	private updateOptions(): void {
		...
		const pushOption = (
			option: { label: string; handler: () => Promise<boolean> },
			category: { label: string; order: number } | undefined,
		) => {
		...
		const registerSessionListeners = (session: IDebugSession) => {
			...
			sessionListeners.add(
				session.onDidEndAdapter(() =>
					sessionListenersStore.delete(sessionListeners),
				),
			);
		};
		this._register(
