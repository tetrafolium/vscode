...
export class WatchExpressionsView
	extends ViewPane
	implements IDebugViewWithVariables
{
	...
	private onMouseDblClick(e: ITreeMouseEvent<IExpression>): void {
		if (
			(e.browserEvent.target as HTMLElement).className.indexOf("twistie") >= 0
		) {
	...
	return parentContext.createOverlay([
		...
		[
			...
			!!session?.capabilities.supportsReadMemoryRequest &&
				expression.memoryReference !== undefined,
		],
		[
			CONTEXT_VARIABLE_IS_READONLY.key,
			!!expression.presentationHint?.attributes?.includes("readOnly") ||
				expression.presentationHint?.lazy,
		],
...
class WatchExpressionsDragAndDrop implements ITreeDragAndDrop<IExpression> {
	...
	getDragURI(element: IExpression): string | null {
		if (
			!(element instanceof Expression) ||
			element ===
				this.debugService.getViewModel().getSelectedExpression()?.expression
		) {
			return null;
		}
...
export const COPY_ALL_WATCH_EXPRESSIONS_COMMAND_ID =
	"workbench.debug.viewlet.action.copyAllWatchExpressions";

registerAction2(
	class CopyAllWatchExpressions extends ViewAction<WatchExpressionsView> {
		constructor() {
			super({
