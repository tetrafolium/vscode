...
): ICallStackItemContext | undefined {
		...
		this.onCallStackChangeScheduler = this._register(
			new RunOnceScheduler(async () => {
				...
				const stoppedDetails =
					sessions.length === 1 ? sessions[0].getStoppedDetails() : undefined;
				if (
					stoppedDetails &&
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this._register(
			this.tree.onDidOpen(async (e) => {
				...
				if (element instanceof Array) {
					element.forEach((sf) =>
						this.dataSource.deemphasizedStackFramesToShow.add(sf),
					);
...
function getSessionContextOverlay(
	session: IDebugSession,
): [string, ContextKeyValue][] {
	return [
...
class SessionsRenderer implements ICompressibleTreeRenderer<
	IDebugSession,
	FuzzyScore,
		...
		const actionBar = templateDisposable.add(
			new ActionBar(session, {
				actionViewItemProvider: (action, options) => {
					if (
						(action.id === STOP_ID || action.id === DISCONNECT_ID) &&
...
class ThreadsRenderer implements ICompressibleTreeRenderer<
	IThread,
	FuzzyScore,
	IThreadTemplateData
> {
	...
	disposeElement(
		_element: ITreeNode<IThread, FuzzyScore>,
		_index: number,
...
function getStackFrameContextOverlay(
...
class StackFramesRenderer implements ICompressibleTreeRenderer<
	IStackFrame,
	FuzzyScore,
...
class ErrorsRenderer implements ICompressibleTreeRenderer<
	string,
	FuzzyScore,
	IErrorTemplateData
> {
...
class ShowMoreRenderer implements ICompressibleTreeRenderer<
	IStackFrame[],
	FuzzyScore,
	ILabelTemplateData
> {
...
class CallStackDataSource implements IAsyncDataSource<
	IDebugModel,
	CallStackItem
> {
	...
	hasChildren(element: IDebugModel | CallStackItem): boolean {
		...
		return (
			isDebugModel(element) || (element instanceof Thread && element.stopped)
		);
	}
			...
			children.forEach((child, index) => {
				if (
					...
					if (!this.deemphasizedStackFramesToShow.has(child)) {
						...
						const nextChild =
							index < children.length - 1 ? children[index + 1] : undefined;
						if (
	...
	private async getThreadCallstack(
		...
		let callStack: Array<IStackFrame | string | ThreadAndSessionIds> =
			thread.getCallStack();
