	...
	implements IPrivateBreakpointWidgetService
{
	...
	private createTriggerBreakpointInput(container: HTMLElement) {
		...
		const index = this.availableBreakpoints.findIndex(
		...
		if (index !== -1) {
			...
			!this.breakpoint?.triggeredBy &&
			this.availableBreakpoints.length > 0
		) {
			...
			selectBreakpointBox.onDidSelect((e) => {
				if (e.index === 0) {
					...
					this.triggeredByBreakpointInput =
						this.availableBreakpoints[e.index - 1];
				}
			}),
		);
								...
								return {
									suggestions: suggestions.items.map((s) => {
										s.completion.range = Range.fromPositions(
											position.delta(0, -overwriteBefore),
											position,
										);
										return s.completion;
									}),
