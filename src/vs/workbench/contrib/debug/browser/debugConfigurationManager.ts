...
abstract class AbstractLaunch implements ILaunch {
	...
	private getDeduplicatedConfig(): IGlobalConfig | undefined {
		...
		const compounds =
			original.compounds?.filter(
				(compound): compound is ICompound =>
					!!compound && typeof compound.name === "string",
			) ?? [];
		const configurations =
			original.configurations?.filter(
				(configuration): configuration is IConfig =>
					!!configuration && typeof configuration.name === "string",
			) ?? [];
		return {
