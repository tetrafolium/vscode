...
export class DebugHoverWidget implements IContentWidget {
	...
	private async doShow(
		...
		if (!expression.hasChildren) {
			...
			this.safeTriangle =
				mouseEvent &&
				new dom.SafeTriangle(mouseEvent.posx, mouseEvent.posy, this.domNode);
		...
		store.add(
			this.expressionRenderer.renderValue(this.complexValueTitle, expression, {
				hover: false,
				session,
			}),
		);
	...
	private layoutTree(): void {
		...
		if (this.showAtPosition) {
			...
			const hoveredCharTop =
				this.editor.getTopForLineNumber(this.showAtPosition.lineNumber, true) -
