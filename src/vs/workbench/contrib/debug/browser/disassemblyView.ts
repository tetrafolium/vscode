...
export class DisassemblyView extends EditorPane {
	...
	constructor(
		...
		this._onDidChangeStackFrame = this._register(
			new Emitter<void>({
				...
				leakWarningName: "DisassemblyView._onDidChangeStackFrame",
			}),
		);
		...
		this._register(
			_configurationService.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration("debug")) {
					...
					const newValue =
						this._configurationService.getValue<IDebugConfiguration>("debug")
	...
	protected createEditor(parent: HTMLElement): void {
		...
		const thisOM = this;
		const delegate =
			new (class implements ITableVirtualDelegate<IDisassembledInstructionEntry> {
				...
				getHeight(row: IDisassembledInstructionEntry): number {
					if (
						...
						if (row.instruction.endLine) {
							return (
								lineHeight *
								Math.max(2, row.instruction.endLine - row.instruction.line + 2)
							);
						} else {
		...
		this._disassembledInstructions = this._register(
			this._instantiationService.createInstance(
				...
				{
					identityProvider: {
						getId: (e: IDisassembledInstructionEntry) => e.instruction.address,
					},
		...
		this._register(
			this._disassembledInstructions.onDidScroll((e) => {
				if (
					this._disassembledInstructions?.row(0) === disassemblyNotAvailable
				) {
					return;
				}
		...
		this._register(
			this._disassembledInstructions.onContextMenu((e) =>
				...
				if (bpEvent && this._disassembledInstructions) {
					...
					bpEvent.added?.forEach((bp) => {
						if (bp instanceof InstructionBreakpoint) {
							...
							if (index >= 0) {
								this._disassembledInstructions!.row(index).isBreakpointSet =
									true;
								this._disassembledInstructions!.row(index).isBreakpointEnabled =
									bp.enabled;
								changed = true;
							}
						}
	...
	private reloadDisassembly(instructionReference: string, offset: number) {
		...
		this.loadDisassembledInstructions(
			...
			if (this._disassembledInstructions!.length > 0) {
				...
				if (refBaseAddress !== undefined) {
					const da = this._disassembledInstructions!;
					targetIndex = binarySearch2(da.length, (i) =>
						Number(da.row(i).address - refBaseAddress),
					);
					if (targetIndex < 0) {
	...
	private onContextMenu(
		e: ITableContextMenuEvent<IDisassembledInstructionEntry>,
	): void {
		...
		const disposables = [
			...
			addStandardDisposableListener(container, "click", () => {
				if (currentElement.element?.allowBreakpoint) {
					...
					const offset = Number(
						address - this._disassemblyView.getReferenceAddress(reference)!,
					);
		...
		if (instruction) {
			...
			const selection = instruction.endLine
				? {
						...
						endColumn:
							instruction.endColumn || Constants.MAX_SAFE_SMALL_INTEGER,
					}
				: {
						startLineNumber: instruction.line!,
...
export class DisassemblyViewContribution implements IWorkbenchContribution {
	...
	constructor(
				...
				this._onDidChangeModelLanguage =
					activeTextEditorControl.onDidChangeModelLanguage((e) => {
						this._languageSupportsDisassembleRequest?.set(
							debugService
								.getAdapterManager()
								.someDebuggerInterestedInLanguage(e.newLanguage),
						);
					});
			} else {
		...
		this._onDidActiveEditorChangeListener =
			editorService.onDidActiveEditorChange(onDidActiveEditorChangeListener);
	}
