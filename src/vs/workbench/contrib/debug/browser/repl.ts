...
export class Repl extends FilterViewPane implements IHistoryNavigationWidget {
			...
			this.onDidChangeBodyVisibility((visible) => {
				...
				if (!this.model) {
					this.model =
						...
						this.modelService.createModel("", null, Repl.URI, true);
				}
							...
							provideCompletionItems: async (
								...
								if (model) {
									...
									const computeRange = (length: number) =>
										Range.fromPositions(position.delta(0, -length), position);
											...
											if (item && item.label) {
												...
												if (typeof item.selectionStart === "number") {
													...
													const selectionLength =
														typeof item.selectionLength === "number"
									...
									if (
										...
										history.forEach((h, i) =>
											suggestions.push({
												...
												sortText:
													"ZZZ" +
													String(history.length - i).padStart(idxLength, "0"),
											}),
										);
									}
	...
	getFilterStats(): { total: number; filtered: number } {
		...
		return {
			...
			filtered:
				this.tree?.getNode().children.filter((c) => c.visible).length ?? 0,
		};
	}
	...
	protected layoutBodyContent(height: number, width: number): void {
		...
		if (this.tree) {
			const lastElementVisible =
				this.tree.scrollTop + this.tree.renderHeight >= this.tree.scrollHeight;
	...
	private get isMultiSessionView(): boolean {
		return (
			this.debugService
				.getModel()
				.getSessions(true)
				.filter((s) => s.hasSeparateRepl() && !sessionsToIgnore.has(s)).length >
			1
		);
	}
	...
	private createReplTree(): void {
		...
		const wordWrap =
			this.configurationService.getValue<IDebugConfiguration>("debug").console
				.wordWrap;
		...
		const tree = (this.tree = this.instantiationService.createInstance(
			...
			this.replDataSource,
			{
				...
				keyboardNavigationLabelProvider: {
					getKeyboardNavigationLabel: (e: IReplElement) => e.toString(true),
				},
	...
	private createReplInput(container: HTMLElement): void {
		...
		this._register(
			this.replInput.onDidFocusEditorText(() => this.updateInputDecoration()),
		);
		this._register(
			this.replInput.onDidBlurEditorText(() => this.updateInputDecoration()),
		);
	...
	private getAriaLabel(): string {
		...
		if (
			!this.configurationService.getValue(AccessibilityVerbositySettingId.Debug)
		) {
			return ariaLabel;
		}
	...
	class extends ViewAction<Repl> {
		...
		async runInView(
			...
			if (
				...
				session = resolveChildSession(
					session,
					debugService.getModel().getSessions(),
				);
	...
	class extends Action2 {
		...
		async run(
			...
			if (selectedText && selectedText.length > 0) {
				...
				const textToCopy =
					retValue || removeAnsiEscapeCodes(element.toString());
