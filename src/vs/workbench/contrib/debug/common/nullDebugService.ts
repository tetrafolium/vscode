...
const nullViewModel: IViewModel = {
	...
	getVisualizedExpression(): IExpression | string | undefined {
		return undefined;
	},
...
const nullDebugModel: IDebugModel = {
	...
	getBreakpoints(): readonly IBreakpoint[] {
		return [];
	},
...
const nullConfigurationManager: IConfigurationManager = {
	...
	hasDebugConfigurationProvider(): boolean {
		return false;
	},
...
const nullAdapterManager: IAdapterManager = {
	...
	someDebuggerInterestedInLanguage(): boolean {
		return false;
	},
	getDebugger() {
		return undefined;
...
export class NullDebugService implements IDebugService {
	...
	async focusStackFrame(
		_focusedStackFrame: IStackFrame | undefined,
		_thread?: IThread,
	...
	async startDebugging(
		_launch: ILaunch | undefined,
		_configOrName?: IConfig | string,
		_options?: IDebugSessionOptions,
		_saveBeforeStart?: boolean,
	): Promise<boolean> {
		return false;
	}
	async restartSession(
		_session: IDebugSession,
		_restartData?: unknown,
	): Promise<void> {}
	async stopSession(
		_session: IDebugSession | undefined,
		_disconnect?: boolean,
		_suspend?: boolean,
	): Promise<void> {}
...
export class NullDebugVisualizerService implements IDebugVisualizerService {
	...
	async getApplicableFor(): Promise<IReference<DebugVisualizer[]>> {
		return { object: [], dispose() {} };
	}
	register(): IDisposable {
		return Disposable.None;
	}
	registerTree(): IDisposable {
		return Disposable.None;
	}
	async getVisualizedNodeFor(): Promise<undefined> {
		return undefined;
	}
	async getVisualizedChildren(): Promise<IExpression[]> {
		return [];
	}
