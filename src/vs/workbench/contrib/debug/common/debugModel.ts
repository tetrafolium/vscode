...
export class ExpressionContainer implements IExpressionContainer {
	...
	get hasChildren(): boolean {
		...
		return (
			!!this.reference && this.reference > 0 && !this.presentationHint?.lazy
		);
	}
	...
	private async fetchVariables(
		...
		try {
			...
			if (this.session!.autoExpandLazyVariables) {
				await Promise.all(
					vars.map((v) => v.presentationHint?.lazy && v.evaluateLazy()),
				);
			}
...
export class VisualizedExpression implements IExpression {
	...
	get hasChildren() {
		return (
			this.treeItem.collapsibleState !== DebugTreeItemCollapsibleState.None
		);
	}
							...
							scheduler: new RunOnceScheduler(() => {
								thread
									...
									.then(() => {
										...
										for (
											let i = 1;
											i < stale.length && !bottomOfCallStackChanged;
											i++
										) {
