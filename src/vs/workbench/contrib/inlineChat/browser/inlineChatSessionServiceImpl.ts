...
import {
	...
	InlineChatSessionTerminationState,
...
export class InlineChatSessionServiceImpl implements IInlineChatSessionService {
	...
	constructor(
		...
		const agentObs = observableFromEvent(
			...
			() => chatAgentService.getDefaultAgent(ChatAgentLocation.EditorInline),
		);
		this.#store.add(
			autorun((r) => {
	...
	createSession(editor: IActiveCodeEditor): IInlineChatSession {
		...
		const chatModelRef = this.#chatService.startNewLocalSession(
		...
		const terminationState = observableValue<
		...
		store.add(
			autorun((r) => {
				...
				const state = entries
					.find((entry) => isEqual(entry.modifiedURI, uri))
					?.state.read(r);
				if (
		...
		const result: IInlineChatSession = {
			uri,
			initialPosition: editor
				.getSelection()
				.getStartPosition()
				.delta(-1) /* one line above selection start */,
...
export class InlineChatEnabler {
	...
	constructor(
		...
		this.#ctxHasNotebookProvider =
			CTX_INLINE_CHAT_HAS_NOTEBOOK_AGENT.bindTo(contextKeyService);
		...
		const notebookAgentObs = observableFromEvent(
			this,
			chatAgentService.onDidChangeAgents,
			() => chatAgentService.getDefaultAgent(ChatAgentLocation.Notebook),
		);
		const notebookAgentConfigObs = observableConfigValue(
		...
		this.#store.add(
			autorun((r) => {
				this.#ctxHasNotebookProvider.set(
					notebookAgentConfigObs.read(r) && !!notebookAgentObs.read(r),
				);
			}),
		);
...
export class InlineChatEscapeToolContribution extends Disposable {
	...
	constructor(
		...
		this._store.add(
			lmTools.registerTool(InlineChatEscapeToolContribution.#data, {
				invoke: async (invocation, _tokenCountFn, _progress, _token) => {
					const sessionResource = invocation.context?.sessionResource;

					if (!sessionResource) {
						logService.warn(
							"InlineChatEscapeToolContribution: no sessionId in tool invocation context",
						);
						return { content: [{ kind: "text", value: "Cancel" }] };
					}

					const session =
						inlineChatSessionService.getSessionBySessionUri(sessionResource);

					if (!session) {
						logService.warn(
							`InlineChatEscapeToolContribution: no session found for id ${sessionResource}`,
						);
						return { content: [{ kind: "text", value: "Cancel" }] };
					}
					...
					if (!lastRequest) {
						logService.warn(
							`InlineChatEscapeToolContribution: no request found for id ${sessionResource}`,
						);
						return {
							content: [{ kind: "text", value: "Cancel" }],
							toolResultMessage: localize("tool.cancel", "Cancel"),
						};
					}

					const response =
						typeof invocation.parameters?.response === "string" &&
						invocation.parameters.response.trim().length > 0
