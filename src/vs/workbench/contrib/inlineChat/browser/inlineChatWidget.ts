...
} from "../../chat/browser/widget/chatWidget.js";
...
export abstract class InlineChatWidget {
	...
	constructor(
		...
		this.scopedContextKeyService = this._store.add(
			contextKeyService.createScoped(this._elements.chatWidget),
		);
		...
		this._store.add(
			this.chatWidget.onDidChangeViewModel(() => {
				...
				viewModelStore.add(
					viewModel.onDidChange(() => {
						this.#requestInProgress.set(
							viewModel.model.requestInProgress.get(),
							undefined,
						);
						...
						ctxResponseError.set(
							isResponseVM(last) && last.errorDetails !== undefined,
						);
						...
						ctxResponseSupportIssues.set(
							isResponseVM(last) &&
								(last.agent?.metadata.supportIssueReporting ?? false),
						);
		...
		// context keys
		this.#ctxResponseFocused =
			CTX_INLINE_CHAT_RESPONSE_FOCUSED.bindTo(contextKeyService);
		const tracker = this._store.add(trackFocus(this.domNode));
		this._store.add(
			tracker.onDidBlur(() => this.#ctxResponseFocused.set(false)),
		);
		this._store.add(
			tracker.onDidFocus(() => this.#ctxResponseFocused.set(true)),
		);

		this.#ctxInputEditorFocused =
			CTX_INLINE_CHAT_FOCUSED.bindTo(contextKeyService);
		this._store.add(
			this.chatWidget.inputEditor.onDidFocusEditorWidget(() =>
				this.#ctxInputEditorFocused.set(true),
			),
		);
		this._store.add(
			this.chatWidget.inputEditor.onDidBlurEditorWidget(() =>
				this.#ctxInputEditorFocused.set(false),
			),
		);
		...
		this._store.add(
			this.#configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(AccessibilityVerbositySettingId.InlineChat)
				) {
		...
		this._store.add(
			this.#chatService.onDidPerformUserAction((e) => {
				if (
					isEqual(
						e.sessionResource,
						this.chatWidget.viewModel?.model.sessionResource,
					) &&
					e.action.kind === "vote"
				) {
					this.updateStatus(
						localize("feedbackThanks", "Thank you for your feedback!"),
						{ resetAfter: 1250 },
					);
				}
			}),
	...
	#updateAriaLabel(): void {
		this._elements.root.ariaLabel = this.#accessibleViewService.getOpenAriaHint(
	...
	#setupDisclaimer(): void {
		...
		this._store.add(
			autorun((reader) => {
				...
				const anonymous =
					this.#chatEntitlementService.anonymousObs.read(reader);
...
export class EditorBasedInlineChatWidget extends InlineChatWidget {
	constructor(
		...
		const overflowWidgetsNode = layoutService
			.getContainer(getWindow(parentEditor.getContainerDomNode()))
