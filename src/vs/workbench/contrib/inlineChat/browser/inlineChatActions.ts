...
export class StartSessionAction extends Action2 {
		...
		super({
			...
			precondition: ContextKeyExpr.and(
				...
				ContextKeyExpr.or(
					...
					CTX_ASK_IN_CHAT_ENABLED.negate(),
				),
			),
			keybinding: {
	...
	private async _runEditorCommand(
...
export class AskInChatAction extends EditorAction2 {
	...
	override async runEditorCommand(
		...
		const session = chatEditingService.editingSessionsObs
			.get()
			.find((s) => s.getEntry(editor.getModel().uri));
		if (!session) {
			return;
		}
		const widget = await chatWidgetService.openSession(
...
export class DismissEditorAffordanceAction extends EditorAction2 {
	constructor() {
		super({
			...
			precondition: ContextKeyExpr.and(
				CTX_INLINE_CHAT_AFFORDANCE_VISIBLE,
				ContextKeyExpr.equals("config.inlineChat.affordance", "editor"),
			),
			keybinding: {
