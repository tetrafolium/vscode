...
} from "../../chat/common/editing/chatEditingService.js";
		...
		if (
			...
			(typeof attachments !== "undefined" &&
				(!Array.isArray(attachments) ||
					!attachments.every((item) => item instanceof URI))) ||
			(typeof modelSelector !== "undefined" &&
				!isILanguageModelChatSelector(modelSelector)) ||
			(typeof resolveOnResponse !== "undefined" &&
				typeof resolveOnResponse !== "boolean") ||
			(typeof attachDiagnostics !== "undefined" &&
				typeof attachDiagnostics !== "boolean")
		) {
			return false;
		}
...
export class InlineChatController implements IEditorContribution {
	...
	static get(editor: ICodeEditor): InlineChatController | undefined {
		return (
			editor.getContribution<InlineChatController>(InlineChatController.ID) ??
			undefined
		);
	}
	...
	constructor(
		...
		const ctxInlineChatVisible =
			CTX_INLINE_CHAT_VISIBLE.bindTo(contextKeyService);
		const ctxFileBelongsToChat =
			CTX_INLINE_CHAT_FILE_BELONGS_TO_CHAT.bindTo(contextKeyService);
		const ctxTerminated = CTX_INLINE_CHAT_TERMINATED.bindTo(contextKeyService);
		const notebookAgentConfig = observableConfigValue(
		...
		this.inputOverlayWidget = this.#store.add(
			this.#instaService.createInstance(InlineChatAffordance, this.#editor),
		);
		...
		const sessionsSignal = observableSignalFromEvent(
			this,
			inlineChatSessionService.onDidChangeSessions,
		);
		...
		this.#currentSession = derived((r) => {
			...
			const model = editorObs.model.read(r);
			const session =
				model && inlineChatSessionService.getSessionByTextModel(model.uri);
			...
			autorun((r) => {
				...
				if (!session) {
					...
					if (lastSession && !lastSession.chatModel.hasRequests) {
						const state =
							lastSession.chatModel.inputModel.state.read(undefined);
						if (
							!state ||
							(!state.inputText && state.attachments.length === 0)
						) {
							lastSession.dispose();
		...
		const visibleSessionObs = observableValue<IInlineChatSession | undefined>(
			this,
			undefined,
		);
			...
			autorun((r) => {
				...
				if (!session) {
					...
					if (!this.#zone.value.position) {
						this.#zone.value.widget.chatWidget.setInputPlaceholder(
							defaultPlaceholderObs.read(r),
						);
			...
			autorun((r) => {
				...
				if (pending) {
					this.#logService.info(
						`[InlineChat] auto-approving: ${pending.detail ?? "unknown"}`,
					);
			...
			autorun((r) => {
				...
				if (session) {
					...
					const otherEntries = entries.filter((entry) => {
						...
						if (
							!!sessionCellUri &&
							isEqual(sessionCellUri.notebook, entry.modifiedURI)
						) {
							return false;
						}
						return true;
					...
					for (const entry of otherEntries) {
						...
						this.#editorService
							.openEditor({ resource: entry.modifiedURI }, SIDE_GROUP)
		...
		const lastResponseProgressObs = lastResponseObs.map((response, r) => {
			...
			return observableFromEvent(this, response.onDidChange, () =>
				response.response.value.findLast(
					(part) => part.kind === "progressMessage",
				),
			).read(r);
		});
			...
			autorun((r) => {
				...
				if (!response?.isInProgress.read(r)) {
					...
					if (response?.result?.errorDetails) {
						...
						this.#zone.rawValue?.showTerminationCard(
							terminationState,
							this.#instaService,
						);
					}
					...
					this.#zone.rawValue?.widget.domNode.classList.toggle(
						"request-in-progress",
						false,
					);
					this.#zone.rawValue?.widget.chatWidget.setInputPlaceholder(
						defaultPlaceholderObs.read(r),
					);
				} else {
					this.#zone.rawValue?.widget.domNode.classList.toggle(
					...
					this.#zone.rawValue?.widget.chatWidget.setInputPlaceholder(
						placeholder || localize("loading", "Working..."),
					);
				}
			}),
		);
		...
		this.#store.add(
			autorun((r) => {
				...
				const pane = this.#editorService.visibleEditorPanes.find(
					(candidate) =>
						candidate.getControl() === this.#editor ||
						isNotebookWithCellEditor(candidate, this.#editor),
				);
				if (pane && entry) {
				...
				if (entry?.diffInfo && this.#zone.rawValue?.position) {
					...
					for (const change of diff.changes) {
						if (change.modified.contains(position.lineNumber)) {
							this.#zone.rawValue?.updatePositionAndHeight(
								new Position(change.modified.startLineNumber - 1, 1),
							);
							break;
						}
					}
	...
	async run(arg?: InlineChatRunOptions): Promise<boolean> {
		...
		const existingSession =
			this.#inlineChatSessionService.getSessionByTextModel(uri);
		if (existingSession) {
		...
		try {
			...
			if (arg?.attachDiagnostics) {
				...
				for (const [
					range,
					marker,
				] of this.#markerDecorationsService.getLiveMarkers(uri)) {
				...
				if (entries.length > 0) {
					this.#zone.value.widget.chatWidget.attachmentModel.addContext(
						...entries,
					);
					const msg =
						entries.length > 1
					...
					arg.message = msg;
					this.#zone.value.widget.chatWidget.inputEditor.setSelection(
			...
			if (arg && InlineChatRunOptions.isInlineChatRunOptions(arg)) {
				...
				if (arg.attachments) {
					await Promise.all(
						arg.attachments.map(async (attachment) => {
							await this.#zone.value.widget.chatWidget.attachmentModel.addFile(
								attachment,
							);
						}),
					);
				...
				if (arg.modelSelector) {
					const id = (
						await this.#languageModelService.selectLanguageModels(
							arg.modelSelector,
						)
					)
						.sort()
			...
			if (!arg?.resolveOnResponse) {
				...
				const rejected =
					session.editingSession.getEntry(uri)?.state.get() ===
	...
	async rephraseSession(): Promise<void> {
		...
		const requestText = this.#instaService.invokeFunction(
			rephraseInlineChat,
			session,
		);
		if (requestText) {
	...
	async #selectVendorDefaultModel(session: IInlineChatSession): Promise<void> {
		const model =
			this.#zone.value.widget.chatWidget.input.selectedLanguageModel.get();
		if (
			model &&
			!model.metadata.isDefaultForLocation[session.chatModel.initialLocation]
		) {
			...
			for (const identifier of ids) {
				const candidate =
					this.#languageModelService.lookupLanguageModel(identifier);
				if (
					candidate?.isDefaultForLocation[session.chatModel.initialLocation]
				) {
					this.#zone.value.widget.chatWidget.input.setCurrentLanguageModel({
						metadata: candidate,
						identifier,
	...
	async #applyModelDefaults(
		session: IInlineChatSession,
		sessionStore: DisposableStore,
	): Promise<void> {
		const userSelectedModel = InlineChatController.#userSelectedModel;
		const defaultModelSetting = this.#configurationService.getValue<string>(
		...
		if (userSelectedModel) {
			modelApplied =
				this.#zone.value.widget.chatWidget.input.switchModelByQualifiedName([
					userSelectedModel,
				]);
			if (!modelApplied) {
		...
		sessionStore.add(
			autorun((r) => {
				const newModel =
					this.#zone.value.widget.chatWidget.input.selectedLanguageModel.read(
						r,
					);
				if (!newModel) {
					return;
				...
				if (initialModelId !== newModel.identifier) {
					...
					InlineChatController.#userSelectedModel =
						ILanguageModelChatMetadata.asQualifiedName(newModel.metadata);
					initialModelId = newModel.identifier;
				}
			}),
		);
