...
class StatusPlaceholder extends Action2 {
	...
	static readonly CtxHasStatus = new RawContextKey<boolean>(
		"inlineChatHasStatus",
		false,
	);
		...
		super({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					...
					StatusPlaceholder.CtxHasStatus,
				),
				group: "navigation",
...
export class InlineChatZoneWidget extends ZoneWidget {
	...
	constructor(
		...
		this.#terminationMarkdownContainer.appendChild(
			this.#terminationMarkdownMessage,
		);
		this.#terminationMarkdownScrollable = this._disposables.add(
			new DomScrollableElement(this.#terminationMarkdownContainer, {
				consumeMouseWheelIfScrollbarIsNeeded: true,
				horizontal: ScrollbarVisibility.Hidden,
				vertical: ScrollbarVisibility.Auto,
			}),
		);
		this.#terminationCard.appendChild(
			this.#terminationMarkdownScrollable.getDomNode(),
		);
		...
		this.#ctxCursorPosition =
			CTX_INLINE_CHAT_OUTER_CURSOR_POSITION.bindTo(contextKeyService);
		this.#ctxHasStatus =
			StatusPlaceholder.CtxHasStatus.bindTo(contextKeyService);
		...
		if (!InlineChatZoneWidget.#factoryRegistration) {
			InlineChatZoneWidget.#factoryRegistration =
		...
		this.widget = instaService.createInstance(
		...
		const updateCursorIsAboveContextKey = () => {
			if (!this.position || !this.editor.hasModel()) {
				...
				this.position.lineNumber === this.editor.getPosition().lineNumber
			) {
		...
		this._disposables.add(
			this.editor.onDidFocusEditorText((e) => updateCursorIsAboveContextKey()),
		);
		updateCursorIsAboveContextKey();
	}
	...
	showTerminationCard(
		...
		const rendered = this.#terminationStore.add(
			renderMarkdown(markdownMessage),
		);
		...
		const actionRunner = this.#terminationStore.add(
	...
	#updatePadding() {
		...
		const marginWithoutIndentation =
			info.glyphMarginWidth + info.lineNumbersWidth + info.decorationsWidth;
