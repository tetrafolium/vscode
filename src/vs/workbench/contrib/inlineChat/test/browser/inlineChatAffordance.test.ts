...
suite("InlineChatAffordance - Telemetry", () => {
	...
	setup(() => {
		...
		instantiationService.stub(
			...
			new (class extends mock<IInlineChatSessionService>() {
				override readonly onWillStartSession = Event.None;
				override readonly onDidChangeSessions = Event.None;
				override getSessionByTextModel() {
					return undefined;
				}
				override getSessionBySessionUri() {
					return undefined;
				}
			})(),
		);
	...
	function setExplicitSelection(sel: Selection): void {
		editor
			.getViewModel()!
			.setCursorStates("test", CursorChangeReason.Explicit, [
				CursorState.fromModelSelection(sel),
			]);
	}

	test('shown event includes mode "editor"', () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			store.add(
				instantiationService.createInstance(InlineChatAffordance, editor),
			);

			setExplicitSelection(new Selection(1, 1, 1, 6));
			await timeout(600);

			const shown = telemetryEvents.filter(
				(e) => e.eventName === "inlineChatAffordance/shown",
			);
	...
	test("shown event does NOT fire when mode is off", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			configurationService.onDidChangeConfigurationEmitter.fire(
				new (class extends mock<IConfigurationChangeEvent>() {
					override affectsConfiguration(key: string) {
	...
	test("shown event does NOT fire for whitespace-only selection", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			model.setValue("   \nhello");
	...
	test("shown event does NOT fire for empty selection", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			store.add(
				instantiationService.createInstance(InlineChatAffordance, editor),
			);
	...
	test("each selection gets a unique affordanceId", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			store.add(
				instantiationService.createInstance(InlineChatAffordance, editor),
			);
