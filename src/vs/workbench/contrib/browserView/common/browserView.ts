...
type IntegratedBrowserNavigationClassification = {
	navigationType: {
		...
		comment: "How the navigation was triggered";
	};
	isLocalhost: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Whether the URL is a localhost address";
	};
	owner: "kycutler";
...
type IntegratedBrowserShareWithAgentClassification = {
	shared: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the content was shared with the agent";
	};
	dontAskAgain: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Whether the user chose to not be asked again";
	};
	owner: "kycutler";
...
export const IBrowserViewWorkbenchService =
	createDecorator<IBrowserViewWorkbenchService>("browserViewWorkbenchService");
...
export interface IBrowserViewWorkbenchService {
	...
	getOrCreateLazy(
		id: string,
		initialState?: IBrowserEditorViewState,
	): BrowserEditorInput;
...
export class BrowserViewModel extends Disposable implements IBrowserViewModel {
	...
	private _certificateError: IBrowserViewCertificateError | undefined =
		undefined;
	private _storageScope: BrowserViewStorageScope =
	...
	constructor(
		...
		void this.playwrightService
			.isPageTracked(this.id)
			.then((shared) => this._setSharedWithAgent(shared))
			.catch((e) => {
				this.logService.warn(
		...
		this._register(
			this.browserViewService.onDynamicDidChangeDeviceEmulation(this.id)(
				(device) => {
		...
		this._register(
			this.onDidChangeElementSelectionActive((active) => {
				if (active) {
					this.telemetryService.publicLog2<
						IntegratedBrowserAddElementToChatStartEvent,
						IntegratedBrowserAddElementToChatStartClassification
					>("integratedBrowser.addElementToChat.start", {});
				}
		...
		this._register(
			this.onDidChangeAreaSelectionActive((active) => {
	...
	get sharingState(): BrowserViewSharingState {
		...
		return this._sharedWithAgent
			...
			: BrowserViewSharingState.NotShared;
	}
	 ...
	 */
	private async setBrowserZoomIndex(
		zoomIndex: number,
		forceApply = false,
	): Promise<void> {
	...
	get onDidChangeAreaSelectionActive(): Event<boolean> {
		return this.browserViewService.onDynamicDidChangeAreaSelectionActive(
			this.id,
		);
	}
	...
	async setSharedWithAgent(shared: boolean): Promise<boolean> {
		if (shared) {
			...
			if (!storedChoice) {
				...
				this.telemetryService.publicLog2<
					IntegratedBrowserShareWithAgentEvent,
					IntegratedBrowserShareWithAgentClassification
	...
	private logNavigationTelemetry(
		navigationType: IntegratedBrowserNavigationEvent["navigationType"],
		url: string,
	): void {
		...
		this.telemetryService.publicLog2<
			IntegratedBrowserNavigationEvent,
