...
} from "../../../../../platform/url/common/trustedDomains.js";
import {
...
const CONTEXT_BROWSER_EDITOR_OPEN = new RawContextKey<boolean>(
	"browserEditorOpen",
	false,
	localize(
		"browser.editorOpen",
...
class BrowserTabQuickPick extends Disposable {
	private readonly _quickPick: IQuickPick<
		IBrowserQuickPickItem,
		{ useSeparators: true }
	>;
	...
	constructor(
		...
		this._quickPick = this._register(
			quickInputService.createQuickPick<IBrowserQuickPickItem>({
				useSeparators: true,
			}),
		);
		this._quickPick.placeholder = localize(
		...
		this._register(
			this._quickPick.onDidTriggerItemButton(async ({ item }) => {
				item.editor?.dispose(true);
			}),
		);

		this._register(
			this._quickPick.onDidTriggerButton(async () => {
				for (const editor of this._browserViewService
	...
	show(): void {
		...
		if (activeEditor instanceof BrowserEditorInput) {
			const activePick = (
				this._quickPick.items as readonly (
					| IBrowserQuickPickItem
					| IQuickPickSeparator
				)[]
			).find(
				(item): item is IBrowserQuickPickItem =>
					item.type !== "separator" && item.editor === activeEditor,
			);
			if (activePick) {
	...
	private _buildItems(): void {
		...
		const groups = this._editorGroupsService.getGroups(
		...
		const groupsWithBrowserEditors = groups
			.map((group) => ({
				group,
				browserEditors: group.editors.filter(
					(e): e is BrowserEditorInput => e instanceof BrowserEditorInput,
				),
			}))
		...
		const backgroundEditors = [
			...this._browserViewService.getKnownBrowserViews().values(),
		].filter((e) => !viewsInGroups.has(e.id));
		...
		type Section = {
			...
			ariaLabel: string;
			groupId: number;
			editors: BrowserEditorInput[];
			isPinned?: (e: BrowserEditorInput) => boolean;
		};
		const sections: Section[] = groupsWithBrowserEditors.map(
			({ group, browserEditors }) => ({
				label: group.label,
			...
			for (const editor of section.editors) {
				...
				const pick: IBrowserQuickPickItem = {
					...
					ariaLabel: hasMultipleSections
						? localize(
								"browserEntryAriaLabelWithGroup",
								"{0}, {1}",
								nameAndDescription,
								section.ariaLabel,
							)
				...
				this._itemListeners.add(
					editor.onDidChangeLabel(() => this._buildItems()),
				);
			}
		}
...
class QuickOpenBrowserAction extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		const picker = accessor
			.get(IInstantiationService)
			.createInstance(BrowserTabQuickPick);
		picker.show();
	}
}
...
class OpenIntegratedBrowserAction extends Action2 {
	...
	async run(
		...
		urlOrOptions?: string | IOpenBrowserOptions,
	): Promise<void> {
		...
		if (options.reuseUrlFilter) {
			...
			const matchingEditor = [
				...
				if (
					filterUri.authority &&
					!match(filterUri.authority, editorUri.authority)
				) {
					return false;
				}
				...
				if (filterUri.query) {
					...
					if (
						![...filterParams].every(([key, value]) =>
							match(value, editorParams.get(key) ?? ""),
						)
					) {
						return false;
					}
		...
		logBrowserOpen(
			telemetryService,
			options.url ? "commandWithUrl" : "commandWithoutUrl",
		);

		const editorPane = await editorService.openEditor(
			{ resource, options: { viewState: { url: options.url } } },
			group,
		);
...
class OpenFileInIntegratedBrowserAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, resource?: URI): Promise<void> {
		...
		const fileUri =
			resource ??
			EditorResourceAccessor.getOriginalUri(editorService.activeEditor, {
		...
		await editorService.openEditor({
			resource: browserUri,
			options: { viewState: { url: fileUri.toString() } },
		});
	}
}
...
class NewTabAction extends Action2 {
	...
	async run(
		accessor: ServicesAccessor,
		_browserEditor = accessor.get(IEditorService).activeEditorPane,
	): Promise<void> {
		const editorService = accessor.get(IEditorService);
		const telemetryService = accessor.get(ITelemetryService);
		const resource = BrowserViewUri.forId(generateUuid());
...
class CloseAllBrowserTabsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		for (const group of editorGroupsService.getGroups(
			GroupsOrder.GRID_APPEARANCE,
		)) {
			const browserEditors = group
...
class CloseAllBrowserTabsInGroupAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const group = editorGroupsService.getGroup(
			editorService.activeEditorPane?.group?.id ??
		...
		const browserEditors = group
			.getEditors(EditorsOrder.SEQUENTIAL)
			.filter((e): e is BrowserEditorInput => e instanceof BrowserEditorInput);
		if (browserEditors.length > 0) {
			await group.closeEditors(browserEditors);
		}
	}
...
class OpenOrListBrowsersAction extends Action2 {
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					ContextKeyExpr.equals(
						"config.workbench.browser.showInTitleBar",
						false,
					).negate(),
					ContextKeyExpr.or(
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const hasOpenBrowserEditor =
			browserViewService.getKnownBrowserViews().size > 0;
...
MenuRegistry.appendMenuItem(MenuId.MenubarViewMenu, {
	...
	command: {
		id: BrowserViewCommandId.OpenOrList,
		title: localize(
			{ key: "miOpenBrowser", comment: ["&& denotes a mnemonic"] },
			"&&Browser",
		),
	},
	order: 2,
...
registerAction2(
	class ToggleBrowserTitleBarButton extends ToggleTitleBarConfigAction {
		constructor() {
			super(
				"workbench.browser.showInTitleBar",
				localize("toggle.browser", "Integrated Browser"),
				localize(
 ...
 */
class BrowserEditorOpenContextKeyContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const update = () =>
			contextKey.set(browserViewService.getKnownBrowserViews().size > 0);
...
registerWorkbenchContribution2(
	BrowserEditorOpenContextKeyContribution.ID,
...
class LocalhostLinkOpenerContribution
	extends Disposable
	implements IWorkbenchContribution, IExternalOpener
{
	...
	async openExternal(
		href: string,
		_ctx: { sourceUri: URI; preferredOpenerId?: string },
		...
		try {
			...
			if (
				!isLocalhostAuthority(parsed.host) &&
				!isAllInterfacesAuthority(parsed.host)
			) {
				return false;
			}
		} catch {
		...
		const isDefaultLinkOpen = !isConfigured(
			this.configurationService.inspect("workbench.browser.openLocalhostLinks"),
		);

		const browserUri = BrowserViewUri.forId(generateUuid());
		await this.editorService.openEditor({
			resource: browserUri,
			options: { pinned: true, viewState: { url: href, isDefaultLinkOpen } },
		});
		return true;
	}
}
...
const LOCALHOST_HINT_DISMISSED_KEY =
	"workbench.browser.linkOpenedHintDismissed";
...
class LinkOpenedHintPill extends BrowserEditorContribution {
	...
	constructor(
			...
			actions: [
				{
					...
					run: () => {
						this.preferencesService.openUserSettings({
							query: "workbench.browser.openLocalhostLinks",
						});
					},
				},
				{
		...
		this._register(
			this.hoverService.setupDelayedHover(this._pill, hoverOptions, {
				setupKeyboardEvents: true,
			}),
		);
		this._register(
			addDisposableListener(this._pill, EventType.CLICK, () => {
...
class BrowserTabUrlSuggestions extends BrowserEditorContribution {
	...
	constructor(
		...
		this._provider = {
			...
			getSuggestions: async ({ input }) => {
				...
				for (const tab of this._browserViewService
					.getKnownBrowserViews()
					.values()) {
					if (tab === input) {
						continue;
	...
	private async _switchToTab(
		source: BrowserEditorInput,
		target: BrowserEditorInput,
	): Promise<void> {
		if (source === target) {
			await this._editorService.openEditor(target);
			return;
		}
		const sourceGroup = this._editorGroupsService
			.getGroups(GroupsOrder.MOST_RECENTLY_ACTIVE)
			.find((g) => g.contains(source));
		if (sourceGroup) {
				...
				localize(
					{
						...
						key: "browser.showInTitleBar.true",
					},
				...
				localize(
					{
						...
						key: "browser.showInTitleBar.false",
					},
				...
				localize(
					{
						...
						key: "browser.showInTitleBar.whenOpen",
					},
					"The button is shown in the title bar when a browser editor is open.",
				),
			],
			default: "whenOpen",
