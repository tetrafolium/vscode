 ...
 */
export const AgentHostChatToolsEnabledSettingId =
...
export class BrowserViewWorkbenchService
	extends Disposable
	implements IBrowserViewWorkbenchService
{
	...
	private readonly _onDidChangeBrowserViews = this._register(
		new Emitter<void>(),
	);
	readonly onDidChangeBrowserViews: Event<void> =
		this._onDidChangeBrowserViews.event;
	...
	private readonly _onDidChangeSharingAvailable = this._register(
		new Emitter<boolean>(),
	);
	readonly onDidChangeSharingAvailable: Event<boolean> =
		this._onDidChangeSharingAvailable.event;
	...
	constructor(
		...
		this._browserViewService =
			ProxyChannel.toService<IBrowserViewService>(channel);
		...
		this.sendTheme();
		this._register(
			this.themeService.onDidColorThemeChange(() => this.sendTheme()),
		);
		...
		this._isSharingAvailable = this.contextKeyService.contextMatchesRules(
			BrowserViewWorkbenchService._sharingAvailableContext,
		);
		const sharingKeys = new Set(
			BrowserViewWorkbenchService._sharingAvailableContext.keys(),
		);
		this._register(
			this.contextKeyService.onDidChangeContext((e) => {
		...
		void this._initializeExistingViews().catch((e) => {
			this.logService.error(
				"[BrowserViewWorkbenchService] Failed to initialize existing browser views.",
				e,
			);
		});
		...
		this._register(
			this._browserViewService.onDidCreateBrowserView((e) => {
				...
				if (editor && e.openOptions) {
					void this._openEditorForCreatedView(
						editor,
						e.info.owner,
						e.openOptions,
					).catch((error) => {
	...
	private async _resolveStorageScope(): Promise<BrowserViewStorageScope> {
		const dataStorageSetting =
			this.configurationService.getValue<BrowserViewStorageScope>(
	...
	private async _initializeExistingViews(): Promise<void> {
		const views = await this._browserViewService.getBrowserViews(
			this._mainWindowId,
		);
	 ...
	 */
	private async _openEditorForCreatedView(
		view: BrowserEditorInput,
		owner: IBrowserViewOwner,
		...
		const [group] = await this.instantiationService.invokeFunction(
			findGroup,
			{ editor: view, options: editorOptions },
			targetGroup,
		);
		if (owner.sessionId) {
			...
			const activeIsSameSession =
				group.activeEditor instanceof ChatEditorInput &&
	...
	private sendConfiguration(): void {
		void this._browserViewService.updateConfiguration({
			aiFeaturesDisabled: !this.contextKeyService.contextMatchesRules(
				ChatContextKeys.enabled,
			),
		});
	}
