...
} from "../common/browserEditorInput.js";
...
).registerEditorSerializer(BrowserEditorInput.ID, BrowserEditorSerializer);
		...
		editorResolverService.registerEditor(
			...
			},
			{
				canSupportResource: (resource) =>
					resource.scheme === Schemas.vscodeBrowser,
			...
			},
			{
				createEditorInput: ({ resource, options }) => {
					...
					if (!parsed) {
						throw new Error(
							`Invalid browser view resource: ${resource.toString()}`,
						);
					}
