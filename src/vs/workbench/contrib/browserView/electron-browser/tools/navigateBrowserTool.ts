	...
	inputSchema: {
		...
		properties: {
			...
			type: {
				...
				description:
					'Navigation type: "url" to navigate to a URL (default, requires "url" param), "back" or "forward" for history, "reload" to refresh.',
			},
			url: {
				type: "string",
		...
		switch (params.type) {
			case "reload":
				return {
					invocationMessage: new MarkdownString(
						localize("browser.reload.invocation", "Reloading {0}", link),
					),
					pastTenseMessage: new MarkdownString(
						localize("browser.reload.past", "Reloaded {0}", link),
					),
			...
			case "back":
				return {
					...
					pastTenseMessage: new MarkdownString(
						localize("browser.goBack.past", "Navigated backward in {0}", link),
					),
			...
			default: {
				if (!params.url) {
					throw new Error(
						'The "url" parameter is required when type is "url".',
					);
				}
		...
		switch (params.type) {
			case "reload":
				return playwrightInvoke(
					...
					sessionId,
					params.pageId,
					(page) => page.reload({ waitUntil: "domcontentloaded" }),
				);
			case "back":
				return playwrightInvoke(
					this.playwrightService,
					sessionId,
					params.pageId,
					(page) => page.goBack({ waitUntil: "domcontentloaded" }),
				);
			case "forward":
				return playwrightInvoke(
					this.playwrightService,
					sessionId,
					params.pageId,
					(page) => page.goForward({ waitUntil: "domcontentloaded" }),
				);
			default: {
				return playwrightInvoke(
					this.playwrightService,
