	...
	inputSchema: {
		...
		properties: {
			...
			fromSelector: {
				type: "string",
				description:
					'Playwright selector of the element to drag when "fromRef" is not available.',
			},
			fromElement: {
				type: "string",
				description:
					'Human-readable description of the element to drag (e.g., "file item", "draggable card").',
			},
			toRef: {
				type: "string",
			...
			toSelector: {
				type: "string",
				description:
					'Playwright selector of the element to drop onto when "toRef" is not available.',
			},
			toElement: {
				type: "string",
				description:
					'Human-readable description of the element to drop onto (e.g., "drop zone", "target folder").',
			},
		},
		required: ["pageId", "fromElement", "toElement"],
		$comment:
			'One of "fromRef" or "fromSelector" is required, and one of "toRef" or "toSelector" is required.',
	},
};
...
export class DragElementTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		const fromElement = escapeMarkdownSyntaxTokens(
			params.fromElement ?? DEFAULT_ELEMENT_LABEL,
		);
		const toElement = escapeMarkdownSyntaxTokens(
			params.toElement ?? DEFAULT_ELEMENT_LABEL,
		);
		return {
			invocationMessage: new MarkdownString(
				localize(
					"browser.drag.invocation",
					"Dragging {0} to {1} in {2}",
					fromElement,
					toElement,
					link,
				),
	...
	async invoke(
		...
		if (!fromSelector) {
			return errorResult(
				'Either a "fromRef" or "fromSelector" parameter is required for the source element.',
			);
		}
		...
		if (!toSelector) {
			return errorResult(
				'Either a "toRef" or "toSelector" parameter is required for the target element.',
			);
		}
		...
		return playwrightInvoke(
			...
			sessionId,
			params.pageId,
			(page, from, to) => page.dragAndDrop(from, to),
			fromSelector,
			toSelector,
		);
	}
