...
export class RunPlaywrightCodeTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		if (params.deferredResultId) {
			return {
				invocationMessage: new MarkdownString(
					localize(
						"browser.runCode.waitInvocation",
						"Waiting for Playwright code to complete...",
					),
				),
				pastTenseMessage: new MarkdownString(
					localize("browser.runCode.waitPast", "Waited for Playwright code"),
				),
			};
		}
		...
		return {
			invocationMessage: new MarkdownString(
				localize("browser.runCode.invocation", "Running Playwright code..."),
			),
			pastTenseMessage: new MarkdownString(
				localize("browser.runCode.past", "Ran Playwright code"),
			),
	...
	async invoke(
		...
		if (params.deferredResultId) {
			try {
				const result = await this.playwrightService.waitForDeferredResult(
					sessionId,
					params.deferredResultId,
		...
		if (!params.code) {
			return errorResult(
				'Either "code" or "deferredResultId" must be provided.',
			);
		}

		let result;
		try {
			result = await this.playwrightService.invokeFunction(
				sessionId,
				params.pageId,
				`async (page) => { ${params.code} }`,
				undefined,
				params.timeoutMs ?? 5_000,
			);
		} catch (e) {
