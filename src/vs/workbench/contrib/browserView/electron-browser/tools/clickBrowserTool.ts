	...
	inputSchema: {
		...
		properties: {
			...
			selector: {
				type: "string",
				description:
					'Playwright selector of the element to click when "ref" is not available.',
			},
			element: {
				type: "string",
				description:
					'Human-readable description of the element to click (e.g., "submit button", "search icon").',
			},
			dblClick: {
				type: "boolean",
...
export class ClickBrowserTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		const element = escapeMarkdownSyntaxTokens(
			params.element ?? DEFAULT_ELEMENT_LABEL,
		);
		return {
			invocationMessage:
				params.button === "right"
					? new MarkdownString(
							localize(
								"browser.click.invocation.right",
								"Right-clicking {0} in {1}",
								element,
								link,
							),
						)
					: params.button === "middle"
						? new MarkdownString(
								localize(
									"browser.click.invocation.middle",
									"Middle-clicking {0} in {1}",
									element,
									link,
								),
							)
						: params.dblClick
							? new MarkdownString(
									localize(
										"browser.dblClick.invocation",
										"Double-clicking {0} in {1}",
										element,
										link,
									),
								)
			...
			pastTenseMessage:
				params.button === "right"
					? new MarkdownString(
							localize(
								"browser.click.past.right",
								"Right-clicked {0} in {1}",
								element,
								link,
							),
						)
					: params.button === "middle"
						? new MarkdownString(
								localize(
									"browser.click.past.middle",
									"Middle-clicked {0} in {1}",
									element,
									link,
								),
							)
						: params.dblClick
							? new MarkdownString(
									localize(
										"browser.dblClick.past",
										"Double-clicked {0} in {1}",
										element,
										link,
									),
								)
	...
	async invoke(
		...
		if (params.dblClick) {
			return playwrightInvoke(
				...
				sessionId,
				params.pageId,
				(page, sel, btn) => page.locator(sel).dblclick({ button: btn }),
				selector,
				button,
			);
		}
		...
		return playwrightInvoke(
			...
			sessionId,
			params.pageId,
			(page, sel, btn) => page.locator(sel).click({ button: btn }),
			selector,
			button,
		);
	}
