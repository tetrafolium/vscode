	...
	inputSchema: {
		...
		properties: {
			...
			text: {
				type: "string",
				description:
					'The text to type. One of "text" or "key" must be provided.',
			},
			key: {
				type: "string",
				description:
					'A key or key combination to press (e.g., "Enter", "Tab", "Control+c"). One of "text" or "key" must be provided.',
			},
			ref: {
				type: "string",
				description:
					"Element reference to target. If omitted, types into the focused element.",
			},
			selector: {
				type: "string",
				description:
					'Playwright selector of element to target when "ref" is not available. If omitted, types into the focused element.',
			},
			element: {
				type: "string",
				description:
					'Human-readable description of the element to type into (e.g., "search box", "comment field"). Required when "ref" or "selector" is specified.',
			},
		},
		required: ["pageId"],
...
export class TypeBrowserTool implements IToolImpl {
	...
	async prepareToolInvocation(
			...
			if (hasTarget && params.element) {
				...
				return {
					invocationMessage: new MarkdownString(
						localize(
							"browser.pressKey.invocation.element",
							"Pressing key `{0}` in {1} in {2}",
							key,
							element,
							link,
						),
					...
					pastTenseMessage: new MarkdownString(
						localize(
							"browser.pressKey.past.element",
							"Pressed key `{0}` in {1} in {2}",
							key,
							element,
							link,
						),
		...
		return {
			invocationMessage: new MarkdownString(
				localize("browser.type.invocation", "Typing text in {0}", link),
			),
			pastTenseMessage: new MarkdownString(
				localize("browser.type.past", "Typed text in {0}", link),
			),
		};
	}
	...
	async invoke(
		...
		if (params.key) {
			if (selector) {
				return playwrightInvoke(
					...
					sessionId,
					params.pageId,
					(page, sel, key) => page.locator(sel).press(key),
					selector,
					params.key,
				);
			}
			return playwrightInvoke(
				...
				sessionId,
				params.pageId,
				(page, key) => page.keyboard.press(key),
				params.key,
			);
		}
		...
		if (selector) {
			return playwrightInvoke(
				...
				sessionId,
				params.pageId,
				(page, sel, text) => page.locator(sel).fill(text),
				selector,
				params.text!,
			);
		}
		return playwrightInvoke(
			...
			sessionId,
			params.pageId,
			(page, text) => page.keyboard.type(text),
			params.text!,
		);
	}
}
