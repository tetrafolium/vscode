	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._browserToolSet = this._register(
			this.toolsService.createToolSet(
		...
		this._register(
			this.browserViewService.onDidChangeSharingAvailable(() => {
				this._updateToolRegistrations();
			}),
		);
		...
		this._register(
			this.chatService.onDidDisposeSession((e) => {
				for (const resource of e.sessionResources) {
					void this.playwrightService
						.disposeSession(resource.toString())
						.catch(() => {});
				}
			}),
		);
	...
	private _updateToolRegistrations(): void {
		...
		if (!this.browserViewService.isSharingAvailable) {
			...
			this._toolsStore.add(
				this.toolsService.registerTool(
					OpenBrowserToolNonAgenticData,
					this.instantiationService.createInstance(OpenBrowserToolNonAgentic),
				),
			);
			this._toolsStore.add(
				this._browserToolSet.addTool(OpenBrowserToolNonAgenticData),
			);
			this.chatContextService.updateWorkspaceContextItems(
				BrowserChatAgentToolsContribution.CONTEXT_ID,
				[],
			);
			return;
		}
		...
		this._toolsStore.add(
			this.toolsService.registerTool(
				OpenBrowserToolData,
				this.instantiationService.createInstance(OpenBrowserTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				ReadBrowserToolData,
				this.instantiationService.createInstance(ReadBrowserTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				ScreenshotBrowserToolData,
				this.instantiationService.createInstance(ScreenshotBrowserTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				NavigateBrowserToolData,
				this.instantiationService.createInstance(NavigateBrowserTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				ClickBrowserToolData,
				this.instantiationService.createInstance(ClickBrowserTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				DragElementToolData,
				this.instantiationService.createInstance(DragElementTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				HoverElementToolData,
				this.instantiationService.createInstance(HoverElementTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				TypeBrowserToolData,
				this.instantiationService.createInstance(TypeBrowserTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				RunPlaywrightCodeToolData,
				this.instantiationService.createInstance(RunPlaywrightCodeTool),
			),
		);
		this._toolsStore.add(
			this.toolsService.registerTool(
				HandleDialogBrowserToolData,
				this.instantiationService.createInstance(HandleDialogBrowserTool),
			),
		);
		...
		this._toolsStore.add(
			this._browserToolSet.addTool(ScreenshotBrowserToolData),
		);
		...
		this._toolsStore.add(
			this._browserToolSet.addTool(RunPlaywrightCodeToolData),
		);
		this._toolsStore.add(
			this._browserToolSet.addTool(HandleDialogBrowserToolData),
		);

		// Subscribe to browser view changes and model sharing state changes
		this._syncModelListeners();
		this._toolsStore.add(
			this.browserViewService.onDidChangeBrowserViews(() => {
				this._syncModelListeners();
		...
		this._toolsStore.add(
			this.agentNetworkFilterService.onDidChange(() =>
				this._updateBrowserContext(),
			),
		);
	...
	private _syncModelListeners(): void {
		...
		for (const [id, input] of views) {
			if (!this._modelListeners.has(id) && input.model) {
				...
				store.add(
					input.model.onDidChangeSharingState(() =>
						this._updateBrowserContext(),
					),
				);
	...
	private _updateBrowserContext(): void {
		...
		const sharedViews = views.filter(
			(v) => v.model?.sharingState === BrowserViewSharingState.Shared,
		);
		...
		if (sharedViews.length > 0) {
			...
			value +=
				"\n" +
				formatBrowserEditorList(this.editorService, sharedViews, {
