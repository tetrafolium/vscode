	...
	inputSchema: {
		...
		properties: {
			...
			ref: {
				type: "string",
				description:
					"Element reference to capture. If omitted, captures the whole viewport.",
			},
			selector: {
				type: "string",
				description:
					'Playwright selector of an element to capture when "ref" is not available. If omitted, captures the whole viewport.',
			},
			element: {
				type: "string",
				description:
					'Human-readable description of the element to capture (e.g., "chart diagram", "product image").',
			},
			scrollIntoViewIfNeeded: {
				type: "boolean",
				description:
					"Whether to scroll the element into view before capturing. Defaults to false.",
			},
		},
		required: ["pageId"],
	},
...
type ScreenshotCapturedClassification = {
	...
	screenshotType: {
		...
		comment: "What kind of screenshot was captured (element or viewport).";
	};
	selectorSource: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "How the screenshot tool received its target element (ref, selector, or none).";
	};
	scrollIntoViewIfNeeded: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Whether the tool was asked to scroll the target element into view before capturing.";
	};
	// Image metadata
	imageWidth: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Width of the captured screenshot image in pixels (undefined if it could not be determined).";
	};
	imageHeight: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Height of the captured screenshot image in pixels (undefined if it could not be determined).";
	};
	byteLength: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Encoded size of the screenshot buffer in bytes.";
	};
	// Conversion factors
	windowZoomFactor: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "VS Code per-window zoom factor for the hosting window (undefined if no editor currently holds the browser view).";
	};
	windowDevicePixelRatio: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "OS HiDPI scaling factor for the hosting window (window.devicePixelRatio). Includes windowZoomFactor. Undefined if no editor currently holds the browser view.";
	};
	browserZoomFactor: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Content zoom factor of the integrated browser view.";
	};
	// Window metadata
	windowInnerWidth: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Inner width of the hosting VS Code window viewport, in workbench CSS pixels (window.innerWidth). Undefined if no editor currently holds the browser view.";
	};
	windowInnerHeight: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Inner height of the hosting VS Code window viewport, in workbench CSS pixels (window.innerHeight). Undefined if no editor currently holds the browser view.";
	};
	isInAuxiliaryWindow: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Whether the browser view was hosted in a VS Code auxiliary window at capture time (undefined if no editor currently holds the browser view).";
	};
	isBrowserViewVisible: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Whether the browser view was mounted and being painted in an editor pane at capture time (false if the model was not bound to a visible editor).";
	};
	// Screen metadata
	screenWidth: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Width of the screen the hosting VS Code window is on, in OS-level CSS pixels (window.screen.width, unaffected by windowZoomFactor). Undefined if no editor currently holds the browser view.";
	};
	screenHeight: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Height of the screen the hosting VS Code window is on, in OS-level CSS pixels (window.screen.height, unaffected by windowZoomFactor). Undefined if no editor currently holds the browser view.";
	};
	screenAvailWidth: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Width of the available screen area (excluding OS chrome such as the taskbar/dock), in OS-level CSS pixels (window.screen.availWidth, unaffected by windowZoomFactor). Undefined if no editor currently holds the browser view.";
	};
	screenAvailHeight: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Height of the available screen area (excluding OS chrome such as the taskbar/dock), in OS-level CSS pixels (window.screen.availHeight, unaffected by windowZoomFactor). Undefined if no editor currently holds the browser view.";
	};
	owner: "jruales";
...
export class ScreenshotBrowserTool implements IToolImpl {
		...
		if (params.element) {
			...
			return {
				invocationMessage: new MarkdownString(
					localize(
						"browser.screenshot.invocation.element",
						"Capturing screenshot of {0}",
						element,
					),
				),
				pastTenseMessage: new MarkdownString(
					localize(
						"browser.screenshot.past.element",
						"Captured screenshot of {0}",
						element,
					),
				),
			};
	...
	async invoke(
		...
		const browserViewModel = await this.browserViewWorkbenchService
		...
		const bounds =
			(selector &&
				(await playwrightInvokeRaw(
					...
					sessionId,
					params.pageId,
					async (page, selector, scrollIntoViewIfNeeded) => {
			...
			undefined;
		const screenshot = await browserViewModel.captureScreenshot({
			pageRect: bounds,
		});
		...
		this.telemetryService.publicLog2<
			...
			ScreenshotCapturedClassification
		>("integratedBrowser.tools.screenshot.captured", {
			// Screenshot tool options
			screenshotType: bounds ? "element" : "viewport",
			selectorSource: params.ref
				? "ref"
				: params.selector
					? "selector"
					: "none",
	...
	private findBrowserViewHostWindow(
