...
export const HoverElementToolData: IToolData = {
	...
	modelDescription:
		"Hover over an element in a browser page. Provide either a Playwright selector or an element reference.",
	icon: Codicon.cursor,
	...
	inputSchema: {
		...
		properties: {
			...
			selector: {
				type: "string",
				description:
					'Playwright selector of the element to hover over when "ref" is not available.',
			},
			element: {
				type: "string",
				description:
					'Human-readable description of the element to hover over (e.g., "navigation menu", "tooltip trigger").',
			},
		},
...
export class HoverElementTool implements IToolImpl {
	...
	async invoke(
		...
		return playwrightInvoke(
			...
			sessionId,
			params.pageId,
			(page, sel) => page.locator(sel).hover(),
			selector,
		);
	}
}
