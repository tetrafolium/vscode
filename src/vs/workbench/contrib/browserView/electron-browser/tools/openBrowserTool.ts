	...
	inputSchema: {
		...
		properties: {
			url: {
				type: "string",
				description:
					"The URL to open in the browser. Must be an absolute URI with a scheme such as file:, http:, or https:. For local files, use the canonical absolute form, for example file:///path/to/file.",
			},
			forceNew: {
				type: "boolean",
				description:
					"Whether to force opening a new page even if a page with the same host already exists. Default is false.",
			},
		},
		$comment:
			'If you omit "url", the user will be prompted to share an existing page instead. Use this if there are unshared pages that the user may be interested in sharing with you.',
	},
};
...
export class OpenBrowserTool implements IToolImpl {
	...
	async invoke(
		...
		if (!params.url) {
			const allPages = [
				...this.browserViewService.getKnownBrowserViews().values(),
			];
			if (allPages.length === 0) {
				return {
					content: [
						{ kind: "text", value: "No browser pages are currently open." },
					],
				};
			}

			const shareResult = await this._promptForUnsharedPages(
				invocation,
				allPages,
				params,
				token,
			...
			if (shareResult) {
				...
				return {
					content: [
						{
							kind: "text",
							value: "The user opted not to share an existing page.",
						},
					],
				};
			}
		...
		if (!params.forceNew) {
			...
			const shared = findExistingPagesByHost(
				this.browserViewService,
				params.url,
				{ includeBlank: true, sharingState: BrowserViewSharingState.Shared },
			);
			const alreadyShared = await getExistingPagesResult(
				this.editorService,
				shared,
				{ agentNetworkFilterService: this.agentNetworkFilterService },
			);
			if (alreadyShared) {
			...
			const unshared = findExistingPagesByHost(
				this.browserViewService,
				params.url,
				{
					includeBlank: false,
	 ...
	 */
	private async _promptForUnsharedPages(
		invocation: IToolInvocation,
		candidateEditors: BrowserEditorInput[],
		params: IOpenBrowserToolParams,
		token: CancellationToken,
		...
		const carousel = this._buildShareCarousel(
			candidateEditors,
			params.url,
		...
		const editor = this.browserViewService
			.getKnownBrowserViews()
			.get(selectedOptionId);
		if (!editor) {
			this.logService.warn(
				`[OpenBrowserTool] Selected option '${selectedOptionId}' not found.`,
			);
			return undefined;
		}
	...
	private _buildShareCarousel(
		...
		for (const editor of editors) {
			const editorTitle = (editor.title || editor.getName()).replaceAll(
				" - ",
				"\u00A0-\u00A0",
			); // nbsp around hyphens to prevent formatting in the carousel
			const editorUrl = editor.url || "about:blank";
			const truncatedUrl =
				editorUrl.length > 40
					? editorUrl.substring(0, 40) + "\u2026"
					: editorUrl;
			options.push({
				id: editor.id,
				label: localize(
		...
		options.push({
			...
			label: url
				...
				: localize(
						{ key: "browser.open.noPagesOption", comment: ['{Locked=" - "}'] },
	...
	private _extractSelectedOption(
	...
	private async _openNewPage(
		sessionId: string,
		url: string,
	): Promise<IToolResult> {
		const { pageId, summary } = await this.playwrightService.openPage(
			sessionId,
			url,
		);
		return this._pageResult(
			...
			localize(
				"browser.open.result",
				"Opened {0}",
				createBrowserPageLink(pageId),
			),
		);
	}

	private async _shareExistingPage(
		sessionId: string,
		...
		if (model.sharingState !== BrowserViewSharingState.Shared) {
			if (!(await model.setSharedWithAgent(true))) {
				return {
					content: [
						{ kind: "text", value: "The user declined to share the page." },
					],
				};
			}
		}
		...
		return this._pageResult(
			...
			localize(
				...
				"User shared {0}",
				createBrowserPageLink(editor.id),
			),
		);
	}
