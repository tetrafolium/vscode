	...
	implements IWorkbenchContribution
{
	constructor(
		...
		const shouldReport = derived(
			(reader) => sessionRandom < probability.read(reader),
		);
	...
	private _logSamples(
		measurements: inputLatency.IInputLatencyMeasurements,
	): void {
		...
		type PerformanceInputLatencyClassification = {
			...
			usedJSHeapSize: {
				classification: "SystemMetaData";
				purpose: "PerformanceAndHealth";
				comment: "The JS heap size in bytes at the time of reporting, or -1 if unavailable.";
			};
			jsHeapSizeLimit: {
				classification: "SystemMetaData";
				purpose: "PerformanceAndHealth";
				comment: "The JS heap size limit in bytes, or -1 if unavailable.";
			};
			jsHeapUsagePercentage: {
				classification: "SystemMetaData";
				purpose: "PerformanceAndHealth";
				comment: "The JS heap usage as a percentage (0-100), or -1 if unavailable.";
			};
		};
		...
		const jsHeapUsagePercentage =
			usedJSHeapSize >= 0 && jsHeapSizeLimit > 0
