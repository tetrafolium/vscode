	...
	implements IPreferencesRenderer
{
	...
	constructor(
		...
		this.mcpSettingsRenderer = this._register(
			instantiationService.createInstance(
				McpSettingsRenderer,
				editor,
				preferencesModel,
...
class EditSettingRenderer extends Disposable {
	...
	constructor(
		...
		this.toggleEditPreferencesForMouseMoveDelayer = this._register(
			new Delayer<void>(75),
		);
			...
			for (const section of settingsGroup.sections) {
				for (const setting of section.settings) {
					...
					if (configuration) {
						...
						if (
							this.handlePolicyConfiguration(setting, configuration, markerData)
						) {
							continue;
						}
	...
	private handleLocalUserConfiguration(
		...
		if (
			...
			if (
				...
				if (
					configuration.scope &&
					APPLICATION_SCOPES.includes(configuration.scope)
				) {
	...
	implements languages.CodeActionProvider
{
	...
	public render(): void {
		...
		if (markerData.length) {
			...
			this.markerService.remove("McpSettingsRenderer", [
				this.settingsEditorModel.uri,
			]);
		}
	}
			...
			for (const section of settingsGroup.sections) {
				for (const setting of section.settings) {
					if (setting.key === mcpConfigurationSection) {
						...
						markerData.push(marker);
						const codeActions = this.generateMcpConfigurationCodeActions([
							marker,
						]);
	...
	private generateMcpConfigurationCodeActions(
		diagnostics: IMarkerData[],
	): languages.CodeAction[] {
		const isRemote =
			this.settingsEditorModel.configurationTarget ===
