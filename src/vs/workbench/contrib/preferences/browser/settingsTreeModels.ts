...
export class SettingsTreeSettingElement extends SettingsTreeElement {
	...
	private update(
		...
		if (inspected.policyValue !== undefined) {
			...
			const overrideValueSource =
				source instanceof Map ? source.get(this.setting.key) : undefined;
		...
		if (this.isSessionsWindow) {
			const property = Registry.as<IConfigurationRegistry>(
				Extensions.Configuration,
			).getConfigurationProperties()[this.setting.key];
	...
	matchesAllTags(tagFilters?: Set<string>): boolean {
		...
		if (tagFilters.has("stable")) {
			...
			const otherFilters = new Set(
				Array.from(tagFilters).filter((tag) => tag !== "stable"),
			);
	...
	matchesAnyFeature(featureFilters?: Set<string>): boolean {
		...
		if (featureFilters.has("chat")) {
			const chatFeatures = tocData.children!.find(
				(child) => child.id === "chat",
			);
...
export class SearchResultModel extends SettingsTreeModel {
	...
	getUniqueSearchResults(): ISearchResult | null {
		const cachedResults = this.cachedUniqueSearchResults.get(
		...
		if (this.aiFilterEnabled) {
			...
			const aiSelectedResult =
				this.rawSearchResults[SearchResultIdx.AiSelected];
			if (aiSelectedResult) {
				aiSelectedResult.filterMatches.forEach((m) =>
					aiSelectedKeys.add(m.setting.key),
				);
				combinedFilterMatches = aiSelectedResult.filterMatches;
			}

			const embeddingsResult =
				this.rawSearchResults[SearchResultIdx.Embeddings];
			if (embeddingsResult) {
				embeddingsResult.filterMatches = embeddingsResult.filterMatches.filter(
					(m) => !aiSelectedKeys.has(m.setting.key),
				);
				combinedFilterMatches = combinedFilterMatches.concat(
					embeddingsResult.filterMatches,
				);
			}
			const result = {
	...
	private getUniqueSearchResultSettings(): ISetting[] {
		return (
			this.getUniqueSearchResults()?.filterMatches.map((m) => m.setting) ?? []
		);
	}
