...
export class KeybindingsEditor
	extends EditorPane<IKeybindingsEditorMemento>
	implements IKeybindingsEditorPane
{
	...
	constructor(
		...
		this.searchHasValueContextKey = CONTEXT_KEYBINDINGS_SEARCH_HAS_VALUE.bindTo(
			this.contextKeyService,
		);
	...
	private getAriaLabel(keybindingsEntries: IKeybindingItemEntry[]): string {
		...
		if (
			...
			const kb = this.keybindingsService
				.lookupKeybinding("widgetNavigation.focusNext")
				?.getAriaLabel();
			if (kb) {
				label +=
					". " +
					localize(
						"navigateToResults",
						"Use {0} to navigate to the results table.",
						kb,
					);
			}
		}
...
class ActionsColumnRenderer implements ITableRenderer<
	IKeybindingItemEntry,
...
class CommandColumnRenderer implements ITableRenderer<
	IKeybindingItemEntry,
	ICommandColumnTemplateData
> {
...
class KeybindingColumnRenderer implements ITableRenderer<
	IKeybindingItemEntry,
	IKeybindingColumnTemplateData
> {
...
class SourceColumnRenderer implements ITableRenderer<
	IKeybindingItemEntry,
	ISourceColumnTemplateData
> {
...
class WhenColumnRenderer implements ITableRenderer<
	IKeybindingItemEntry,
	IWhenColumnTemplateData
> {
