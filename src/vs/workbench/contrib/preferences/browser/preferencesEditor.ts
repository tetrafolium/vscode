...
export class PreferencesEditor extends EditorPane {
	...
	private readonly editorPanesRegistry =
		Registry.as<IPreferencesEditorPaneRegistry>(
	...
	private readonly preferencesEditorPane = this._register(
		new MutableDisposable<IPreferencesEditorPane>(),
	);
	...
	constructor(
		...
		this.searchFocusContextKey =
			CONTEXT_PREFERENCES_SEARCH_FOCUS.bindTo(contextKeyService);

		this.element = DOM.$(".preferences-editor");
		const headerContainer = DOM.append(
			this.element,
			DOM.$(".preferences-editor-header"),
		);
		...
		const preferencesTabsContainer = DOM.append(
			headerContainer,
			DOM.$(".preferences-tabs-container"),
		);
		...
		this.onDidChangePreferencesEditorPane(
			this.editorPanesRegistry.getPreferencesEditorPanes(),
			[],
		);
		this._register(
			this.editorPanesRegistry.onDidRegisterPreferencesEditorPanes(
				(descriptors) => this.onDidChangePreferencesEditorPane(descriptors, []),
			),
		);
		this._register(
			this.editorPanesRegistry.onDidDeregisterPreferencesEditorPanes(
				(descriptors) => this.onDidChangePreferencesEditorPane([], descriptors),
			),
		);

		this.bodyElement = DOM.append(
			this.element,
			DOM.$(".preferences-editor-body"),
		);
	}
	...
	layout(dimension: DOM.Dimension): void {
		...
		this.preferencesEditorPane.value?.layout(
			new DOM.Dimension(
				...
				dimension.height - 87 /* header height */,
			),
		);
	}
	...
	private onDidChangePreferencesEditorPane(
		...
		for (const desc of toRemove) {
			const index = this.preferencesTabActions.findIndex(
				(action) => action.id === desc.id,
			);
			if (index !== -1) {
		...
		if (toAdd.length > 0) {
			...
			for (const desc of toAdd) {
				...
				if (index !== -1) {
					const action = new PreferenceTabAction(desc, () =>
						this.onDidSelectPreferencesEditorPane(desc.id),
					);
		...
		if (selectedAction) {
			this.searchWidget.inputBox.setPlaceHolder(
				localize(
					...
					"Search {0}",
					selectedAction.descriptor.title,
				),
			);
	...
	private renderBody(descriptor?: IPreferencesEditorPaneDescriptor): void {
		...
		if (descriptor) {
			const editorPane =
				this.instantiationService.createInstance<IPreferencesEditorPane>(
