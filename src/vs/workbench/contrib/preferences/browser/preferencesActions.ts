...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor, path?: string | URI): Promise<string | URI> => {
		...
		const toEnable = installed.filter(
			(e) =>
				e.isBuiltin &&
				extensionEnablementService.canChangeEnablement(e) &&
				!extensionEnablementService.isEnabled(e),
		);
		if (toEnable.length) {
			...
			const pending = new ExtensionIdentifierSet(
				toEnable.map((e) => e.identifier.id),
			);
			...
			try {
				await extensionEnablementService.setEnablement(
					toEnable,
		...
		const configurationInformation: IConfigurationInformation =
			Object.fromEntries(
				Object.entries(configurationProperties).map(([key, property]) => {
