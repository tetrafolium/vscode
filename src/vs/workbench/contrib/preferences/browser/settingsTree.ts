...
function getObjectDisplayValue(
	...
	const data = element.isConfigured
		...
		: element.hasPolicyValue || element.isAgentsWindowReadOnly
			? element.scopeValue
			: elementDefaultValue;
	...
	const patternsAndSchemas = Object.entries(objectPatternProperties ?? {}).map(
		([pattern, schema]) => ({
...
export function resolveConfiguredUntrustedSettings(
	...
	return [...allSettings].filter(
		(setting) =>
			setting.restricted &&
			inspectSetting(setting.key, target, languageFilter, configurationService)
				.isConfigured,
	);
}
...
export async function createTocTreeForExtensionSettings(
	...
	const processGroupEntry = async (group: ISettingsGroup) => {
		...
		const settings = filter
			? getMatchingSettings(new Set(flatSettings), filter)
			: flatSettings;
		...
		const settingGroupId =
			group.id && group.id !== extensionId ? group.id : group.title;
...
function _resolveSettingsTree(
	...
	if (tocData.children) {
		children = tocData.children
			...
			.map((child) =>
				_resolveSettingsTree(child, allSettings, filter, logService),
			)
...
function getMatchingSettings(
	...
	allSettings.forEach((setting) => {
		...
		if (shouldInclude && filter.include?.tags?.length) {
			shouldInclude = filter.include.tags.some((tag) =>
				setting.tags?.includes(tag),
			);
		}
		...
		if (!shouldExclude && filter.exclude?.tags?.length) {
			shouldExclude = filter.exclude.tags.some((tag) =>
				setting.tags?.includes(tag),
			);
		}
...
interface ISettingListItemTemplate extends ISettingItemTemplate<
	string[] | undefined
> {
...
interface ISettingObjectItemTemplate extends ISettingItemTemplate<
	...
	implements ITreeRenderer<SettingsTreeElement, never, any>
{
	...
	protected renderCommonTemplate(
		...
		toDispose.add(
			DOM.addDisposableListener(controlElement, DOM.EventType.MOUSE_DOWN, (e) =>
				e.stopPropagation(),
			),
		);

		toDispose.add(
			DOM.addDisposableListener(titleElement, DOM.EventType.MOUSE_ENTER, (e) =>
				container.classList.add("mouseover"),
			),
		);
		toDispose.add(
			DOM.addDisposableListener(titleElement, DOM.EventType.MOUSE_LEAVE, (e) =>
...
class SettingGroupRenderer implements ITreeRenderer<
	SettingsTreeGroupElement,
	never,
	IGroupTitleTemplate
> {
	...
	protected renderValue(
		...
		const onClickOrKeydown = (e: UIEvent) => {
			if (isLanguageTagSetting) {
				this._onApplyFilter.fire(
					`@${LANGUAGE_SETTING_TAG}${plainKey.replaceAll(" ", "")}`,
				);
			} else {
...
class SettingComplexObjectRenderer
	...
	implements
		ITreeRenderer<
			SettingsTreeSettingElement,
			never,
			ISettingComplexObjectItemTemplate
		>
{
	...
	protected override renderValue(
		...
		template.button.parentElement?.classList.toggle(
			"hide",
			dataElement.hasPolicyValue || dataElement.isAgentsWindowReadOnly,
		);
...
class SettingArrayRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<SettingsTreeSettingElement, never, ISettingListItemTemplate>
{
...
abstract class AbstractSettingObjectRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<SettingsTreeSettingElement, never, ISettingObjectItemTemplate>
{
	...
	private onDidChangeObject(
		...
		if (template.context) {
			const settingSupportsRemoveDefault =
				objectSettingSupportsRemoveDefaultValue(template.context.setting.key);
		...
		>
{
	...
	private onDidChangeIncludeExclude(
		...
		if (template.context) {
			...
			if (e.type !== "add") {
				if (
					e.originalItem.value.data.toString() in template.context.defaultValue
				) {
			...
			if (e.type === "change" || e.type === "add" || e.type === "move") {
				if (
					e.newItem.value.data.toString() in template.context.defaultValue &&
					!e.newItem.sibling
				) {
			...
			this._onDidChangeSetting.fire({
				...
				value:
					Object.keys(newValue).length === 0 ? undefined : sortKeys(newValue),
				type: template.context.valueType,
				manualReset: false,
...
abstract class AbstractSettingTextRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<SettingsTreeSettingElement, never, ISettingTextItemTemplate>
{
...
class SettingEnumRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<SettingsTreeSettingElement, never, ISettingEnumItemTemplate>
{
	...
	renderTemplate(container: HTMLElement): ISettingEnumItemTemplate {
		...
		const selectBox = new SelectBox([], 0, this._contextViewService, styles, {
			useCustomDrawn:
				!hasNativeContextMenu(this._configService) ||
...
class SettingNumberRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<SettingsTreeSettingElement, never, ISettingNumberItemTemplate>
{
...
class SettingBoolRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<SettingsTreeSettingElement, never, ISettingBoolItemTemplate>
{
...
export class SettingsTreeFilter implements ITreeFilter<SettingsTreeElement> {
	...
	filter(
		...
		if (element instanceof SettingsTreeGroupElement) {
			...
			if (this.isFilteringGroups && this.viewState.categoryFilter) {
				if (
					!this.groupIsRelatedToCategory(element, this.viewState.categoryFilter)
				) {
					return false;
				}
	...
	private groupIsRelatedToCategory(
		group: SettingsTreeGroupElement,
		category: SettingsTreeGroupElement,
	): boolean {
...
class SettingsTreeAccessibilityProvider implements IListAccessibilityProvider<SettingsTreeElement> {
	...
	getAriaLabel(element: SettingsTreeElement) {
		if (element instanceof SettingsTreeSettingElement) {
			...
			const descriptionWithoutSettingLinks = renderAsPlaintext({
