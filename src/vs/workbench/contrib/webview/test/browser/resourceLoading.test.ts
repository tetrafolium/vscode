...
suite("Webview Resource Loading - getResourceToLoad", () => {
	...
	setup(() => {
		const instantiationService = disposableStore.add(
			new TestInstantiationService(),
		);
		instantiationService.stub(ILogService, NullLogService);
		const fileService = disposableStore.add(
			new FileService(instantiationService.get(ILogService)),
		);
	...
	(!isWindows /* UNC is windows only */ ? suite.skip : suite)(
		"UNC paths",
		() => {
	...
	suite("Fragment and query strings", () => {
		test("preserves fragment in returned URI", () => {
			const root = URI.file("/home/user/project");
			const resource = URI.file("/home/user/project/file.txt").with({
				fragment: "section1",
			});
			const result = getResourceToLoad(resource, [root], uriIdentityService);
		...
		test("still validates path containment with query params", () => {
			const root = URI.file("/home/user/project");
			const resource = URI.file("/home/user/other/file.txt").with({
				query: "version=2",
			});
			const result = getResourceToLoad(resource, [root], uriIdentityService);
			assert.strictEqual(result, undefined);
