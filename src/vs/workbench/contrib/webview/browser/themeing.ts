...
export class WebviewThemeDataProvider extends Disposable {
	...
	constructor(
		...
		this._register(
			this._configurationService.onDidChangeConfiguration((e) => {
				if (
					webviewConfigurationKeys.some((key) => e.affectsConfiguration(key))
				) {
					this._reset();
				}
			}),
	...
	public getWebviewThemeData(): WebviewThemeData {
		if (!this._cachedWebViewThemeData) {
			...
			const editorFontLigatures = new EditorFontLigatures().validate(
				configuration.fontLigatures,
			);
			...
			const exportedSizes = sizeRegistry
				.getSizes()
				.reduce<Record<string, string>>((sizes, entry) => {
					const sizeValue = sizeRegistry.resolveDefaultSize(entry.id, theme);
					if (sizeValue) {
						sizes["vscode-" + entry.id.replace(/\./g, "-")] =
