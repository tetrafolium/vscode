...
export class WebviewElement
	extends Disposable
	implements IWebviewElement, WebviewFindDelegate
{
			...
			this.on("load-resource", async (entry) => {
				try {
					...
					this.loadResource(
						entry.id,
						uri,
						{ ifNoneMatch: entry.ifNoneMatch, range: entry.range },
			...
			switch (result.type) {
				case WebviewResourceResponse.Type.Success: {
					...
					const requestedRangeEnd =
						range?.end !== undefined ? range.end : result.size - 1;
												...
												try {
													controller.enqueue(
														new Uint8Array<ArrayBuffer>(
															chunk.buffer.buffer as ArrayBuffer,
						...
						listenStream(
							result.stream,
							{
								onData: (chunk) => {
									...
									this._send("did-load-resource-chunk", { id, data }, [
										data.buffer,
									]);
								},
								onError: () => {
