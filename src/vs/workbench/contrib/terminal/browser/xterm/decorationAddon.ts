...
export class DecorationAddon
	extends Disposable
	implements ITerminalAddon, IDecorationAddon
{
	protected _terminal: Terminal | undefined;
	private _capabilityDisposables: DisposableMap<TerminalCapability> =
	...
	constructor(
		...
		private readonly _contextPickService: IChatContextPickService,
		...
		this._register(
			this._capabilities.onDidAddCapability((c) =>
				this._createCapabilityDisposables(c.id),
			),
		);
		this._register(
			this._capabilities.onDidRemoveCapability((c) =>
				this._removeCapabilityDisposables(c.id),
			),
		);
		this._register(
			lifecycleService.onWillShutdown(() => this._disposeAllDecorations()),
		);
	}
	...
	private _createCapabilityDisposables(c: TerminalCapability): void {
		...
		switch (capability.type) {
			case TerminalCapability.BufferMarkDetection:
				store.add(
					capability.onMarkAdded((mark) => this.registerMarkDecoration(mark)),
				);
				break;
	...
	private _refreshStyles(refreshOverviewRulerColors?: boolean): void {
		if (refreshOverviewRulerColors) {
			for (const decoration of this._decorations.values()) {
				const color =
					this._getDecorationCssColor(decoration.command)?.toString() ?? "";
		...
		commandDetectionListeners.push(
			capability.onCurrentCommandInvalidated((request) => {
				if (request.reason === CommandInvalidationReason.NoProblemsReported) {
					const lastDecoration = Array.from(this._decorations.entries())[
	...
	private _createHover(
		...
		return this._hoverService.setupDelayedHover(element, () => ({
			content: new MarkdownString(
				getTerminalDecorationHoverContent(command, hoverMessage, true),
			),
		}));
	}
	...
	private _createAttachToChatAction(
		...
		const chatIsEnabled = this._chatWidgetService
			.getWidgetsByLocations(ChatAgentLocation.Chat)
			.some((w) => w.attachmentCapabilities.supportsTerminalAttachments);
		if (!chatIsEnabled) {
			return undefined;
		}
			...
			run: async () => {
				let widget =
					...
					this._chatWidgetService
						.getWidgetsByLocations(ChatAgentLocation.Chat)
						?.find((w) => w.attachmentCapabilities.supportsTerminalAttachments);
				...
				if (this._resource) {
					...
					terminalContext = this._instantiationService.createInstance(
						TerminalContext,
						getTerminalUri(
							parsedUri.workspaceId,
							parsedUri.instanceId!,
							undefined,
							command.id,
						),
					);
				...
				if (
					...
					this._store.add(
						this._contextPickService.registerChatContextItem(terminalContext),
					);
				}
			},
		};
