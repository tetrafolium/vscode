...
export class TerminalTabList extends WorkbenchList<ITerminalInstance> {
	...
	constructor(
		...
		@ITerminalEditingService
		private readonly _terminalEditingService: ITerminalEditingService,
		...
		this.disposables.add(
			this.onMouseDblClick(async (e) => {
				...
				if (
					this._terminalEditingService.getEditingTerminal()?.instanceId ===
		...
		this._isSplitContextKey =
			TerminalContextKeys.splitTerminalTabFocused.bindTo(contextKeyService);
	...
	private _getFocusMode(): "singleClick" | "doubleClick" {
		return this._configurationService.getValue<"singleClick" | "doubleClick">(
	...
	override layout(height?: number, width?: number): void {
		...
		const newHasActionBar =
			actualWidth > TerminalTabsListSizes.ActionbarMinimumWidth;
		if (
...
class TerminalTabsRenderer implements IListRenderer<
	ITerminalInstance,
	...
	renderTemplate(container: HTMLElement): ITerminalTabEntryTemplate {
						...
						return this._hoverService.showDelayedHover(
							{
								...
								position: {
									hoverPosition:
										this._terminalConfigurationService.config.tabs.location ===
										"left"
											? HoverPosition.RIGHT
											: HoverPosition.LEFT,
								},
		...
		const actionBar = templateDisposables.add(
			new ActionBar(actionsContainer, {
				actionRunner: templateDisposables.add(
					new TerminalContextActionRunner(),
				),
