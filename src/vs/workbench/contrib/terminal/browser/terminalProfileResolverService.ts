	...
	implements ITerminalProfileResolverService
{
	...
	resolveIcon(
		...
		if (shellLaunchConfig.icon) {
			shellLaunchConfig.icon =
				this._getCustomIcon(shellLaunchConfig.icon) || this.getDefaultIcon();
			return;
		}
	...
	getDefaultIcon(resource?: URI): TerminalIcon & ThemeIcon {
		return (
			this._iconRegistry.getIcon(
				this._configurationService.getValue(TerminalSettingId.TabsDefaultIcon, {
					resource,
				}),
			) || Codicon.terminal
		);
	...
	async resolveShellLaunchConfig(
		...
		// an issue
		const resource =
			shellLaunchConfig === undefined || isString(shellLaunchConfig.cwd)
				? undefined
	...
	private async _getUnresolvedDefaultProfile(
		...
		if (options.allowAgentHostShell) {
			const raw = this._configurationService.getValue(
				`terminal.integrated.agentHostProfile.${this._getOsKey(options.os)}`,
			);
			if (isString(raw)) {
				await this._terminalProfileService.profilesReady;
			}
			const agentHostShellProfile =
				this._getUnresolvedAgentHostShellProfile(options);
	...
	private _getUnresolvedAgentHostShellProfile(
		options: IShellLaunchConfigResolveOptions,
	): ITerminalProfile | undefined {
		const agentHostProfile = this._configurationService.getValue(
			`terminal.integrated.agentHostProfile.${this._getOsKey(options.os)}`,
		);
		...
		if (isString(agentHostProfile)) {
			const named = this._terminalProfileService.availableProfiles.find(
				(p) => p.profileName === agentHostProfile && !p.isAutoDetected,
			);
			if (named) {
		...
		if (this._isValidAutomationProfile(agentHostProfile, options.os)) {
			agentHostProfile.icon =
				this._getCustomIcon(agentHostProfile.icon) || Codicon.tools;
		...
		super(
			{
				getDefaultSystemShell: async (remoteAuthority, os) => {
					const backend =
						await terminalInstanceService.getBackend(remoteAuthority);
