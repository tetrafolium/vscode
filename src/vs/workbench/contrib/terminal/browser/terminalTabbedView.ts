...
export class TerminalTabbedView extends Disposable {
	...
	constructor(
		...
		this._chatEntry = this._register(
			this._instantiationService.createInstance(
		...
		this._register(
			contextKeyService.onDidChangeContext((e) => {
				if (
					e.affectsSome(
						new Set([TerminalContribContextKeyStrings.ChatHasHiddenTerminals]),
					)
				) {
					this._refreshShowTabs();
					this._updateChatTerminalsEntry();
	...
	private _shouldShowTabs(): boolean {
		...
		const hiddenChatTerminals =
			this._terminalChatService.getToolSessionTerminalInstances(true);
		if (!enabled) {
			return false;
		}
	...
	private _handleOnDidSashReset(): void {
		...
		if (ctx) {
			...
			const maxInstanceWidth = this._terminalGroupService.instances.reduce(
				(p, c) => {
					return Math.max(
						p,
			...
			idealWidth = Math.ceil(
				Math.max(maxInstanceWidth, TerminalTabsListSizes.WideViewMinimumWidth),
			);
		}
	...
	private _getAdditionalWidth(instance: ITerminalInstance): number {
		...
		const statusIconWidth =
			instance.statusList.statuses.length > 0 ? WidthConstants.StatusIcon : 0;
	...
	layout(width: number, height: number): void {
		const chatItemHeight =
			this._chatEntry?.element.style.display === "none"
				? 0
				: this._chatEntry?.element.clientHeight;
		...
		this._register(
			dom.addDisposableListener(
				...
				(event: DragEvent) => {
					if (!this._shouldHandleEmptyAreaDrop(event)) {
						if (
							!this._tabContainer.contains(event.relatedTarget as Node | null)
						) {
							this._resetEmptyAreaDropState();
						}
						return;
					...
					this._emptyAreaDropTargetCount = Math.max(
						0,
						this._emptyAreaDropTargetCount - 1,
					);
	...
	private _shouldHandleEmptyAreaDrop(event: DragEvent): boolean {
		...
		if (
			targetNode &&
			(this._tabListDomElement.contains(targetNode) ||
				this._tabListElement.contains(targetNode))
		) {
			return false;
		}
		return (
			!!event.dataTransfer &&
			containsDragType(event, TerminalDataTransfers.Terminals)
		);
	}
	...
	private async _handleContainerDrop(event: DragEvent): Promise<void> {
			...
			for (const uri of resources) {
				...
				if (instance) {
					...
					if (terminalIdentifier.instanceId) {
						promises.push(
							primaryBackend.requestDetachInstance(
		...
		if (promises.length) {
			const processes = (await Promise.all(promises)).filter(
				(process): process is IProcessDetails => !!process,
			);
