...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	constructor(
		...
		if (
			...
			!this._shellLaunchConfig.titleTemplate
		) {
	...
	async runCommand(
		...
		await this.sendText(
			commandLine,
			shouldExecute,
			!shouldExecute || forceBracketedPasteMode,
		);
	}
	...
	private _open(): void {
		...
		xterm.raw.attachCustomKeyEventHandler((event: KeyboardEvent): boolean => {
			...
			if (
				...
				(event.metaKey ||
					this._terminalConfigurationService.shouldCommandSkipShell(
						resolveResult.commandId,
					))
			) {
				event.preventDefault();
	...
	private async _createProcess(): Promise<void> {
		...
		if (
			!trusted &&
			!(isRemoteTerminal && this._workbenchEnvironmentService.remoteAuthority)
		) {
			this._onProcessExit({
	...
	private _handleShellTypeChange(
...
export class TerminalLabelComputer extends Disposable {
	...
	refreshLabel(
		...
		const useAgentCliTitle =
			tabs.allowAgentCliTitle &&
			TerminalLabelComputer.agentCliShellTypes.has(
				instance.shellType as GeneralShellType,
			);
		const titleTemplate =
			instance.shellLaunchConfig.titleTemplate ??
			(useAgentCliTitle ? "${sequence}" : tabs.title);
