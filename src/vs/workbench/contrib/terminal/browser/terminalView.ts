...
export class TerminalViewPane extends ViewPane {
	...
	private readonly _actionDisposables: DisposableMap<TerminalCommandId> =
	...
	constructor(
		...
		shellIntegrationDisposable.value =
			this._terminalService.onAnyInstanceAddedCapabilityType((c) => {
				if (
			...
			case TerminalCommandId.Focus: {
				if (action instanceof MenuItemAction) {
					const actions = getFlatContextMenuActions(
						this._singleTabMenu.getActions({ shouldForwardArgs: true }),
					);
					const item = this._instantiationService.createInstance(
	...
	override shouldShowWelcome(): boolean {
		return (
			this._hasWelcomeScreen() && this._terminalService.instances.length === 0
		);
	}
}
...
class SwitchTerminalActionViewItem extends SelectActionViewItem {
	constructor(
		...
		this._register(
			_terminalService.onDidChangeInstances(() => this._updateItems(), this),
		);
		this._register(
			_terminalService.onDidChangeActiveGroup(() => this._updateItems(), this),
		);
		this._register(
		...
		this._register(
			_terminalGroupService.onDidChangeGroups(() => this._updateItems(), this),
		);
		this._register(
...
class TerminalThemeIconStyle extends Themable {
	...
	private _registerListeners(): void {
		...
		this._register(
			this._terminalService.onDidChangeInstances(() => this.updateStyles()),
		);
		this._register(
			this._terminalGroupService.onDidChangeGroups(() => this.updateStyles()),
		);
	}
