	...
	implements ITerminalProfileService
{
	...
	> = new Map();
	private _defaultProfileOverride:
		| { extensionIdentifier: string; id: string }
		| undefined;
	...
	private async _updateContributedProfiles(): Promise<boolean> {
		...
		const configProfiles: {
			[key: string]: ITerminalExecutable | null | undefined;
		...
		const filteredContributedProfiles = [
			...
			...this._internalContributedProfiles.filter(
				(p) => !excludedContributedProfiles.includes(p.title),
			),
		];
	...
	private async _detectProfiles(
		...
		if (isWeb && !this._environmentService.remoteAuthority) {
			const hasAnyBackend =
				[...this._terminalInstanceService.getRegisteredBackends()].length > 0;
			if (!hasAnyBackend) {
				return this._availableProfiles || [];
			}
		}
	...
	registerInternalContributedProfile(
	...
	overrideDefaultProfile(extensionIdentifier: string, id: string): IDisposable {
		...
		return toDisposable(() => {
			if (
				this._defaultProfileOverride?.extensionIdentifier ===
					extensionIdentifier &&
				this._defaultProfileOverride?.id === id
			) {
	...
	async getContributedDefaultProfile(
		...
		if (
			...
			!hasKey(shellLaunchConfig, { executable: true })
		) {
			...
			if (this._defaultProfileOverride) {
				const overridden = this.contributedProfiles.find(
					(p) =>
						p.extensionIdentifier ===
							this._defaultProfileOverride!.extensionIdentifier &&
						p.id === this._defaultProfileOverride!.id,
				);
				if (overridden) {
					return overridden;
				}
