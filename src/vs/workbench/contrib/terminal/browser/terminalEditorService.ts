	...
	implements ITerminalEditorService
{
	...
	constructor(
		...
		this._register(
			lifecycleService.onWillShutdown(() => (this._isShuttingDown = true)),
		);
		this._register(
				...
				if (instance) {
					...
					if (instanceIndex !== -1) {
						const wasActiveInstance =
							this.instances[instanceIndex] === this.activeInstance;
				...
				promise: this._editorService.openEditor(
					{
						resource,
						description:
							instance.description || instance.shellLaunchConfig.type,
						options: {
							pinned: true,
							forceReload: true,
	...
	private _registerInstance(
		...
		this._instanceDisposables.set(inputKey, [
			...
			instance.capabilities.onDidChangeCapabilities(() =>
