...
export abstract class BaseTerminalBackend extends Disposable {
	...
	constructor(
		...
		this._register(
			this._ptyHostController.onPtyHostRequestResolveVariables(async (e) => {
	...
	protected _deserializeTerminalState(
		...
		if (crossVersionState.version !== 1) {
			this._logService.warn(
				`Could not revive serialized processes, wrong version "${crossVersionState.version}"`,
				crossVersionState,
			);
			return undefined;
		}
...
function isCrossVersionSerializedTerminalState(
	obj: unknown,
): obj is ICrossVersionSerializedTerminalState {
	return (
		isObject(obj) &&
		"version" in obj &&
