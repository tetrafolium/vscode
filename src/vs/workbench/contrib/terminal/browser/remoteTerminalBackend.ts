	...
	implements ITerminalBackend
{
	...
	async createProcess(
		...
		const shellLaunchConfigDto: IShellLaunchConfigDto = {
			...
			shellIntegrationEnvironmentReporting:
				shellLaunchConfig.shellIntegrationEnvironmentReporting,
		};
	...
	async attachToRevivedProcess(
		...
		try {
			const newId =
				(await this._remoteTerminalChannel.getRevivedPtyNewId(id)) ?? id;
	...
	async setNextCommandId(
		...
		await this._remoteTerminalChannel.setNextCommandId(
			id,
			commandLine,
			commandId,
		);
	...
	async getWslPath(
		...
		return (
			this._remoteTerminalChannel.getWslPath(original, direction) || original
		);
	}
