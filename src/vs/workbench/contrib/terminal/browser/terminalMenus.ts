		...
		},
		{
			...
			item: {
				...
				command: {
					...
					title: localize(
						{ key: "miNewInNewWindow", comment: ["&& denotes a mnemonic"] },
						"New Terminal &&Window",
					),
...
export function getTerminalActionBarArgs(
	...
	const [aiContributedProfiles, otherContributedProfiles] =
	...
	if (dropdownActions.length > 0) {
		dropdownActions.push(
			new SubmenuAction(
				"split.profile",
				localize("split.profile", "Split Terminal with Profile"),
				submenuActions,
			),
		);
		dropdownActions.push(new Separator());
...
): [ITerminalProfile[], ITerminalProfile[]] {
...
function isAiContributedProfile(profile: IExtensionTerminalProfile): boolean {
	...
	if (
		...
		extensionIdentifier === "anthropic.claude-code"
	) {
		return true;
	}
