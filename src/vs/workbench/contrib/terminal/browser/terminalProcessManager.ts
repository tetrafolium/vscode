	...
	implements ITerminalProcessManager
{
	...
	async createProcess(
		...
		if (shellLaunchConfig.customPtyImplementation) {
			...
			terminalEnvironment.mergeEnvironments(
				envForResolver,
				await backend.getShellEnvironment(),
			);
	...
	private async _resolveEnvironment(
		...
		if (shellLaunchConfig.useShellEnvironment) {
			...
			this._logService.debug(
				`Profile environment resolved with ${Object.keys(baseEnv).length} variables`,
			);
		}
			...
			backend.onPtyHostRestart(async () => {
				...
				if (this._shellLaunchConfig) {
					if (
						...
						this._onProcessData.fire({
							data:
								formatMessageForTerminal(message, { loudFormatting: true }) +
								postRestartMessage,
							trackCommit: false,
						});
						await this.relaunch(
	...
	setDimensions(
		...
		if (!this.ptyProcessReady) {
			throw new Error(
				"TerminalProcessManager.setDimensions called before initialization",
			);
		}
		return this.ptyProcessReady.then(() =>
			this._resize(cols, rows, pixelWidth, pixelHeight),
		);
	}
...
class SeamlessRelaunchDataFilter extends Disposable {
	...
	newProcess(process: ITerminalChildProcess, reset: boolean) {
		...
		if (!this._firstRecorder || !reset || this._disableSeamlessRelaunch) {
			[this._firstRecorder, this._firstDisposable.value] =
				this._createRecorder(process);
			if (this._disableSeamlessRelaunch && reset) {
				this._onProcessData.fire("\x1bc");
			}
			this._dataListener.value = process.onProcessData((e) =>
	...
	triggerSwap() {
		...
		this._dataListener.value = this._activeProcess!.onProcessData((e) =>
			this._onProcessData.fire(e),
		);
	...
	private _createRecorder(
		...
		const disposable = process.onProcessData((e) =>
			recorder.handleData(isString(e) ? e : e.data),
		);
