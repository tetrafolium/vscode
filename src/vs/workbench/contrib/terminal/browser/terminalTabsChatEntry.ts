...
export class TerminalTabsChatEntry extends Disposable {
	...
	constructor(
		...
		this._register(
			dom.addDisposableListener(this._entry, dom.EventType.CLICK, (e) => {
				// Don't trigger if clicking on the delete button
				if (
					e.target === this._deleteButton ||
					this._deleteButton.contains(e.target as Node)
				) {
					return;
				}
				e.preventDefault();
		...
		this._register(
			dom.addDisposableListener(this._entry, dom.EventType.KEY_DOWN, (e) => {
	...
	update(): void {
		const hiddenChatTerminalCount =
			this._terminalChatService.getToolSessionTerminalInstances(true).length;
		...
		if (hasText) {
			this._label.textContent =
				hiddenChatTerminalCount === 1
					? localize(
							"terminal.tabs.chatEntryLabelSingle",
							"{0} Hidden Terminal",
							hiddenChatTerminalCount,
						)
					: localize(
