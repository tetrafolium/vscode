...
export function registerTerminalActions() {
		...
		run: async (c) => {
			if (c.service.isProcessSupportRegistered) {
				const instance = await c.service.createTerminal({
					location: c.configService.defaultLocation,
				});
				if (!instance) {
					return;
				}
	...
	registerTerminalAction({
		...
		run: async (c, _, args) => {
			function isCreateTerminalOptions(
				obj: unknown,
			): obj is ICreateTerminalOptions {
				return isObject(obj) && "location" in obj;
			}
			const options = isCreateTerminalOptions(args)
				? args
	...
	registerTerminalAction({
		...
		keybinding: {
			...
			mac: {
				primary: KeyMod.WinCtrl | KeyMod.Shift | KeyMod.Alt | KeyCode.Backquote,
			},
	...
	registerTerminalAction({
		...
		keybinding: {
			...
			when: ContextKeyExpr.and(
				...
				TerminalContextKeys.splitTerminalActive,
			),
	...
	registerTerminalAction({
		...
		run: async (c, accessor) => {
			...
			const remoteAuthority =
				remoteAgentService.getConnection()?.remoteAuthority ?? undefined;
	...
	registerTerminalAction({
		...
		run: async (c, _, args) => {
			const cwd = args
				? toOptionalString((<{ cwd?: string }>args).cwd)
				: undefined;
	...
	registerActiveInstanceAction({
		...
		run: async (activeInstance, c, accessor, args) => {
			const notificationService = accessor.get(INotificationService);
			const name = args
				? toOptionalString((<{ name?: string }>args).name)
				: undefined;
			if (!name) {
				notificationService.warn(
					localize(
	...
	registerTerminalAction({
		...
		run: async (c, accessor, args) => {
			...
			if (c.service.isProcessSupportRegistered) {
				eventOrOptions =
					!eventOrOptions || isMouseEvent(eventOrOptions) ? {} : eventOrOptions;
...
function getSelectedViewInstances2(
	accessor: ServicesAccessor,
	args?: unknown,
			...
			async run(
				...
				if (
					...
					const config = c.profileService.availableProfiles.find(
						(profile) =>
							profile.profileName === eventOrOptionsOrProfile.profileName,
					);
					if (!config) {
						throw new Error(
