...
import {
	...
	vtBoundaryMatches,
} from "../../browser/chatTerminalCommandMirror.js";
...
suite("Workbench - ChatTerminalCommandMirror", () => {
	...
	suite("VT mirroring with XtermTerminal", () => {
		...
		async function mirrorViaVT(
			source: XtermTerminal,
			startLine = 0,
		): Promise<XtermTerminal> {
			const startMarker = source.raw.registerMarker(
				startLine -
					source.raw.buffer.active.baseY -
					source.raw.buffer.active.cursorY,
			);
			const vt = await source.getRangeAsVT(
		...
		test("VT divergence detection prevents corruption (Windows scenario)", async () => {
			...
			strictEqual(
				boundaryMatches,
				false,
				"Boundary check should detect divergence",
			);
		...
		test("boundary check allows append when VT prefix matches", async () => {
			...
			strictEqual(
				boundaryMatches,
				true,
				"Boundary check should pass when prefix matches",
			);
		...
		test("incremental updates use append path (not full rewrite) in normal operation", async () => {
			...
			strictEqual(
				appended2.length > 0,
				true,
				"Should have new content to append",
			);
			strictEqual(
				appended2.length < vt2.length,
				true,
				"Append should be smaller than full rewrite",
			);
	...
	suite("computeMaxBufferColumnWidth", () => {
		...
		test("returns max width across multiple lines", () => {
			const buffer = createMockBuffer(["short", "much longer line", "mid"]);
		...
		test("handles mixed empty and non-empty lines", () => {
			const buffer = createMockBuffer(["", "content", "", "more", ""]);
			strictEqual(computeMaxBufferColumnWidth(buffer, 80), 7);
		});
		...
		test("handles wide terminal with long content", () => {
			const buffer = createMockBuffer(
				["short", "a".repeat(150), "medium content here"],
				200,
			);
		...
		test("max of multiple lines where longest exceeds default cols", () => {
			const buffer = createMockBuffer(
				["a".repeat(50), "b".repeat(120), "c".repeat(90)],
				150,
			);
