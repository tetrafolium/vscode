		...
		setup(() => {
			...
			(
				terminalService as unknown as {
					_primaryBackend: Partial<ITerminalBackend>;
				}
...
function createTerminalInstance(options?: {
	customPtyImplementation?: boolean;
}): ITerminalInstance {
	return {
...
function updateTitle(
	...
	const fn = Reflect.get(terminalService, "_updateTitle") as (
		instance: ITerminalInstance,
	) => void;
...
function updateIcon(
	...
	const fn = Reflect.get(terminalService, "_updateIcon") as (
		instance: ITerminalInstance,
		userInitiated: boolean,
	) => void;
