...
export class TestViewDescriptorService implements Partial<IViewDescriptorService> {
	...
	moveTerminalToLocation(to: ViewContainerLocation) {
		...
		this._onDidChangeLocation.fire({
			views: [{ id: TERMINAL_VIEW_ID } as unknown as IViewDescriptor],
			from: oldLocation,
			to,
		});
	}
...
suite("XtermTerminal", () => {
	...
	suite("getContentsAsText", () => {
		test("should return all buffer contents when no markers provided", async () => {
			...
			strictEqual(
				result.startsWith("line 1\nline 2\nline 3\nline 4\nline 5"),
				true,
				"Should include the content plus empty lines up to buffer length",
			);
			const lines = result.split("\n");
			strictEqual(
				lines.length,
				xterm.raw.buffer.active.length,
				"Should end with empty lines (total buffer size is 30 rows)",
			);
		});
		...
		test("should return contents from start marker to end", async () => {
			...
			strictEqual(
				result.startsWith("line 2\nline 3\nline 4\nline 5"),
				true,
				"Should start with line 2 and include empty lines",
			);
		});
		...
		test("should handle empty buffer", async () => {
			...
			strictEqual(
				lines.length,
				xterm.raw.buffer.active.length,
				"Empty terminal should have empty lines equal to buffer length",
			);
			strictEqual(
		...
		test("should handle mixed content with spaces and special characters", async () => {
			...
			strictEqual(
				...
				true,
				"Should handle spaces and special characters correctly",
			);
		});
		...
		test("should throw error when startMarker is disposed (line === -1)", async () => {
			...
			try {
				...
				strictEqual(
					error.message,
					"Cannot get contents of a disposed startMarker",
				);
			}
		});
		...
		test("should throw error when endMarker is disposed (line === -1)", async () => {
			...
			try {
				...
				strictEqual(
					error.message,
					"Cannot get contents of a disposed endMarker",
				);
			}
		});
		...
		test("should handle markers at buffer boundaries", async () => {
			...
			strictEqual(
				result,
				"line 1\nline 2\nline 3\nline 4\nline 5",
				"Should handle markers at buffer boundaries correctly",
			);
		});
		...
		test("should handle terminal escape sequences properly", async () => {
			...
			const result = xterm.getContentsAsText();
			strictEqual(
				result.startsWith("red text\ngreen text"),
				true,
				"ANSI escape sequences should be filtered out, but there will be trailing empty lines",
			);
		});
	});
	...
	suite("getBufferReverseIterator", () => {
		test("should get text properly within scrollback limit", async () => {
			...
			const result = [...xterm.getBufferReverseIterator()]
				.reverse()
				.join("\r\n");
