...
suite("DecorationAddon", () => {
	...
	setup(async () => {
		...
		capabilities.add(
			...
			store.add(
				instantiationService.createInstance(CommandDetectionCapability, xterm),
			),
		);
	...
	suite("registerDecoration", () => {
		...
		test("should return decoration with mark properties", async () => {
			const marker = xterm.registerMarker(2);
			notEqual(
				decorationAddon.registerCommandDecoration(undefined, undefined, {
					marker,
				}),
				undefined,
			);
