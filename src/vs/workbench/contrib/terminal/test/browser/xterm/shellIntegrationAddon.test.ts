...
suite("ShellIntegrationAddon", () => {
	...
	suite("deserializeMessage", () => {
		...
		cases.forEach(([title, input, expected]) => {
			test(title, () =>
				strictEqual(deserializeVSCodeOscMessage(input), expected),
			);
		});
	});
	...
	suite("serializeVSCodeOscMessage", () => {
		...
		cases.forEach(([title, input, expected]) => {
			test(title, () =>
				strictEqual(serializeVSCodeOscMessage(input), expected),
			);
		});
	});
