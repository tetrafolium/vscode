...
function createMockTerminalInstance(
	instanceId: number,
...
suite("Terminal Events", () => {
	...
	suite("createInstanceCapabilityEventMultiplexer", () => {
		test("should handle existing instances with capabilities", () => {
			...
			let capturedData:
				| { instance: ITerminalInstance; data: string }
				| undefined;
			...
			strictEqual(
				...
				instance,
				"Event should contain correct instance",
			);
			strictEqual(
				capturedData?.data,
				"test-data",
		...
		test("should handle instances without capabilities", () => {
			...
			strictEqual(
				eventFired,
				false,
				"No event should be fired for instances without capabilities",
			);
		});
		...
		test("should handle adding new instances", () => {
			...
			strictEqual(
				capturedData?.instance,
				instance,
				"Event should contain correct new instance",
			);
			strictEqual(
				capturedData?.data,
				"new-instance-data",
		...
		test("should handle removing instances", () => {
			...
			strictEqual(
				eventCount,
				1,
				"Event should not be fired after instance removal",
			);
		});
		...
		test("should handle adding capabilities to existing instances", () => {
			...
			strictEqual(
				eventFired,
				true,
				"Event should be fired from newly added capability",
			);
			strictEqual(
				capturedData?.instance,
				instance,
		...
		test("should handle removing capabilities from existing instances", () => {
			...
			strictEqual(
				eventCount,
				1,
				"Event should be fired before capability removal",
			);
			...
			strictEqual(
				eventCount,
				1,
				"Event should not be fired after capability removal",
			);
		});
		...
		test("should handle multiple instances with same capability", () => {
			...
			strictEqual(
				events[0].instance,
				instance1,
				"First event should be from instance1",
			);
			strictEqual(
				events[0].data,
				"data-from-instance1",
				"First event should have correct data",
			);
			strictEqual(
				events[1].instance,
				instance2,
				"Second event should be from instance2",
			);
			strictEqual(
				events[1].data,
				"data-from-instance2",
				"Second event should have correct data",
			);
		});
		...
		test("should handle empty current instances array", () => {
			...
			strictEqual(
				eventFired,
				false,
				"No events should be fired with empty instances array",
			);
		});
	});
});
