...
class TestTerminalInstanceService
	extends Disposable
	implements Partial<ITerminalInstanceService>
{
	async getBackend() {
		return {
...
suite("Workbench - TerminalInstance", () => {
	...
	suite("TerminalInstance", () => {
		...
		test("should preserve title for task terminals", async () => {
			...
			strictEqual(
				taskTerminal.title,
				"Test Task Name",
				"Task terminal should preserve API-set title",
			);
		});
		...
		test("should preserve agent shell type detected from sequence until the parent shell returns", async () => {
			...
			const onTitleChange = (title: string) =>
				(instance as unknown as Record<string, (value: string) => void>)[
					"_onTitleChange"
				](title);
			const handleShellTypeChange = (
				shellType: GeneralShellType | PosixShellType | undefined,
			) =>
				(
					instance as unknown as Record<
						string,
						(value: GeneralShellType | PosixShellType | undefined) => void
					>
		...
		test("custom key event handler should intercept Meta-modified keys that resolve to a command when sendKeybindingsToShell is disabled", async () => {
			...
			const originalSoftDispatch = keybindingService.softDispatch;
			strictEqual(
				DEFAULT_COMMANDS_TO_SKIP_SHELL.includes("test.metaKeyInterceptCommand"),
				false,
			);
			keybindingService.softDispatch = () => ({
				kind: ResultKind.KbFound,
	...
	suite("DEFAULT_COMMANDS_TO_SKIP_SHELL", () => {
		test("should include zoom commands so they are not consumed by kitty keyboard protocol", () => {
			deepStrictEqual(
				...
				].every((cmd) => DEFAULT_COMMANDS_TO_SKIP_SHELL.includes(cmd)),
				true,
			);
		});
	});
		...
		test("should return friendly message when executable is specified for non-zero exit codes", () => {
			deepStrictEqual(
				...
				),
				{
					code: 1,
					message:
						'The terminal process "foo" failed to launch (exit code: 1).',
				},
			);
			deepStrictEqual(
				parseExitResult(
				...
				parseExitResult(
					{
						code: 5,
						message:
							"A native exception occurred during launch (Cannot create process, error code: 5)",
					},
					{ executable: "foo" },
					ProcessState.KilledDuringLaunch,
					undefined,
				...
				parseExitResult(
					{
						code: 267,
						message:
							"A native exception occurred during launch (Cannot create process, error code: 267)",
					},
					{},
					ProcessState.KilledDuringLaunch,
					"/foo",
				...
				parseExitResult(
					{
						code: 1260,
						message:
							"A native exception occurred during launch (Cannot create process, error code: 1260)",
					},
					{ executable: "foo" },
					ProcessState.KilledDuringLaunch,
					undefined,
				...
				parseExitResult(
					{
						code: 123,
						message:
							"A native exception occurred during launch (Cannot create process, error code: 123)",
					},
					{},
					ProcessState.KilledDuringLaunch,
					undefined,
	...
	suite("TerminalLabelComputer", () => {
		...
		test("should resolve empty sequence to process name", () => {
			const terminalLabelComputer = createLabelComputer({
				terminal: {
					integrated: {
						tabs: {
							separator: " - ",
							title: "${sequence}${separator}${process}",
							description: "${sequence}",
						},
					},
				},
			...
			terminalLabelComputer.refreshLabel(
				createInstance({ capabilities, processName: "zsh", sequence: "" }),
			);
			strictEqual(terminalLabelComputer.title, "zsh");
			strictEqual(terminalLabelComputer.description, "");
		});
		...
		test("should use ${sequence} for agent CLI shell types", () => {
			...
			terminalLabelComputer.refreshLabel(
				createInstance({
					capabilities,
					shellType: GeneralShellType.Copilot,
					sequence: "Copilot Agent",
					processName: "copilot",
				}),
			);
		...
		test("should use ${sequence} for Gemini agent CLI shell type", () => {
			...
			terminalLabelComputer.refreshLabel(
				createInstance({
					capabilities,
					shellType: GeneralShellType.Gemini,
					sequence: "Gemini - my-project",
					processName: "node",
				}),
			);
		...
		test("should prefer shellLaunchConfig.titleTemplate over agent CLI shell type override", () => {
			...
			terminalLabelComputer.refreshLabel(
				createInstance({
					...
					sequence: "Copilot Agent",
					processName: "copilot",
					shellLaunchConfig: { titleTemplate: "${process}" },
				}),
			);
	...
	suite("getCwdResource", () => {
		...
		function createMockTerminalInstance(options: {
			...
			ITerminalInstance,
			"getCwdResource" | "capabilities" | "remoteAuthority"
		> {
			const capabilities = store.add(new TerminalCapabilityStore());
			...
			if (options.cwd) {
				...
				capabilities.add(
					...
					mockCwdDetection as unknown as ICwdDetectionCapability,
				);
			}
			...
			mockPathService = {
				fileURI: async (path: string) => {
					if (options.remoteAuthority) {
						return URI.parse(
							`vscode-remote://${options.remoteAuthority}${path}`,
						);
					}
