...
import {
	ActionType,
	StateAction,
} from "../../../../../platform/agentHost/common/state/protocol/actions.js";
import {
	RootState,
	TerminalClaimKind,
	type TerminalState,
...
class MockAgentConnection implements IAgentConnection {
	...
	readonly onDidNotification: Event<INotification> =
		this._onDidNotification.event;

	readonly dispatchedActions: {
		channel: string;
	...
	fireAction(channel: URI, action: StateAction, serverSeq = 1): void {
		this._onDidAction.fire({
			...
			action,
			serverSeq,
			origin: { clientId: "server", clientSeq: 0 },
		});
	}
	...
	async resourceList(_uri: URI): Promise<ResourceListResult> {
		return { entries: [] };
	}
	async resourceRead(_uri: URI): Promise<ResourceReadResult> {
		return { data: "", encoding: "utf-8" } as ResourceReadResult;
	}
	async resourceWrite(
	...
	async resourceMove(_params: ResourceMoveParams): Promise<ResourceMoveResult> {
		return {};
	}
...
suite("AgentHostPty", () => {
	...
	test("start() creates terminal and subscribes", async () => {
		...
		const pty = disposables.add(
			new AgentHostPty(1, conn, terminalUri, { name: "test" }),
		);
		...
		assert.strictEqual(
			conn.createdTerminals[0].channel,
			terminalUri.toString(),
		);
		assert.strictEqual(conn.createdTerminals[0].name, "test");
		assert.deepStrictEqual(conn.createdTerminals[0].claim, {
	...
	test("replays existing content from snapshot", async () => {
		const conn = new MockAgentConnection({
			content: [{ type: "unclassified", value: "existing output\n" }],
		});
		disposables.add(conn);
		const pty = disposables.add(new AgentHostPty(1, conn, terminalUri));
	...
	test("input() dispatches terminal/input action", async () => {
		...
		const inputActions = conn.dispatchedActions.filter(
			(a) => a.action.type === ActionType.TerminalInput,
		);
		assert.strictEqual(inputActions.length, 1);
		assert.strictEqual(
			(inputActions[0].action as { data: string }).data,
			"hello",
		);
	});
	...
	test("resize() dispatches terminal/resized action", async () => {
		...
		const resizeActions = conn.dispatchedActions.filter(
			(a) => a.action.type === ActionType.TerminalResized,
		);
		assert.strictEqual(resizeActions.length, 1);
		assert.strictEqual(
			(resizeActions[0].action as { cols: number; rows: number }).cols,
			120,
		);
		assert.strictEqual(
			(resizeActions[0].action as { cols: number; rows: number }).rows,
			40,
		);
	});
	...
	test("ignores actions for other terminals", async () => {
		...
		await pty.start();
		conn.fireAction(URI.parse("agenthost-terminal:///other"), {
			type: ActionType.TerminalData,
	...
	test("shutdown() disposes terminal and unsubscribes", async () => {
		...
		assert.strictEqual(
			conn.disposedTerminals[0].toString(),
			terminalUri.toString(),
		);
		assert.ok(exitFired);
	});
	...
	test("reconnect() re-subscribes with new connection and replays content", async () => {
		...
		assert.ok(
			dataReceived.some((d) => d.includes("\x1b[2J")),
			"should clear buffer before replay",
		);
		assert.ok(
			dataReceived.some((d) => d.includes("new output after reconnect")),
			"should replay new content",
		);
	...
	test("reconnect() dispatches input to new connection", async () => {
		...
		const inputActions = conn2.dispatchedActions.filter(
			(a) => a.action.type === ActionType.TerminalInput,
		);
		assert.strictEqual(inputActions.length, 1);
		assert.strictEqual(
		...
		const oldInputActions = conn1.dispatchedActions.filter(
			(a) => a.action.type === ActionType.TerminalInput,
		);
