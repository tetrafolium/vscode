...
suite("Workbench - TerminalConfigurationService", () => {
		...
		test("should update on any change to terminal.integrated", () => {
			...
			configurationService.onDidChangeConfigurationEmitter.fire({
				affectsConfiguration: (configuration) =>
					configuration.startsWith("terminal.integrated"),
	...
	suite("shouldCommandSkipShell", () => {
		test("should include defaults and added commands", () => {
			...
			strictEqual(
				terminalConfigurationService.shouldCommandSkipShell(command),
				true,
			);
			strictEqual(
				terminalConfigurationService.shouldCommandSkipShell(
					DEFAULT_COMMANDS_TO_SKIP_SHELL[0],
				),
				true,
			);
		});
		...
		test("should remove excluded defaults", () => {
			...
			strictEqual(
				terminalConfigurationService.shouldCommandSkipShell(defaultCommand),
				false,
			);
		});
	});

	function createTerminalConfigationService(
		config: any,
		...
		const terminalConfigurationService = store.add(
	...
	suite("getFont", () => {
		...
		test("fontFamily (Linux Ubuntu)", () => {
			...
			ok(
				terminalConfigurationService
					.getFont(getActiveWindow())
					.fontFamily.startsWith("'Ubuntu Mono'"),
		...
		test("fontSize 10", () => {
			...
			strictEqual(
				terminalConfigurationService.getFont(getActiveWindow()).fontSize,
				10,
		...
		test("lineHeight 2", () => {
			...
			strictEqual(
				terminalConfigurationService.getFont(getActiveWindow()).lineHeight,
				2,
	...
	suite("configFontIsMonospace", () => {
		test("isMonospace monospace", () => {
			...
			strictEqual(
				terminalConfigurationService.configFontIsMonospace(),
				true,
