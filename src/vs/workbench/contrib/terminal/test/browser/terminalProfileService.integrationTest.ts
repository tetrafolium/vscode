...
class MockQuickInputService implements Partial<IQuickInputService> {
	...
	async pick(
		...
		options?: any,
		token?: any,
	): Promise<SingleOrMany<IProfileQuickPickItem> | undefined> {
...
class TestTerminalContributionService implements ITerminalContributionService {
	...
	readonly onDidChangeTerminalCompletionProviders =
		this._onDidChangeTerminalCompletionProviders.event;
...
class TestRemoteAgentService implements Partial<IRemoteAgentService> {
	...
	async getEnvironment(): Promise<IRemoteAgentEnvironment | null> {
		return {
			os: this._os,
		} satisfies Partial<IRemoteAgentEnvironment> as unknown as IRemoteAgentEnvironment;
	}
}
