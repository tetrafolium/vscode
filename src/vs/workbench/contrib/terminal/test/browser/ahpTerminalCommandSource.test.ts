...
} from "../../browser/ahpTerminalCommandSource.js";
import {
	AgentHostPty,
	AhpCommandMarkKind,
	getAhpCommandMarkId,
...
suite("AhpTerminalCommandSource", () => {
	...
	setup(() => {
		...
		onCommandExecuted = store.add(
			new Emitter<IAgentHostPtyCommandExecutedEvent>(),
		);
		onCommandFinished = store.add(
	...
	function simulateMark(commandId: string, kind: AhpCommandMarkKind): void {
		...
		const bufferMark = mockInstance.capabilities?.get(
			TerminalCapability.BufferMarkDetection,
		) as ReturnType<typeof createMockBufferMarkCapability>;
	...
	test("markers are resolved from BufferMarkCapability by ID", () => {
		...
		assert.ok(
			executing?.executedMarker,
			"executedMarker should be resolved from BufferMarkCapability",
		);
		...
		assert.ok(
			completed?.endMarker,
			"endMarker should be resolved from BufferMarkCapability",
		);
	});
