...
suite("OpenImagesInCarouselFromExplorerAction", () => {
	...
	function stubFileService(
		resolveMap: Map<string, IFileStat>,
		fileContents: Map<string, VSBuffer>,
	): void {
	...
	function stubEditorService(): void {
		instantiationService.stub(
			IEditorService,
			"openEditor",
			async (input: unknown, _options: unknown, group: unknown) => {
	...
	test("single image file opens carousel with sibling images", async () => {
		...
		const parent = createExplorerItem(
			"/workspace/images",
			true,
			fileService,
			configService,
		);
		const imageItem = createExplorerItem(
		...
		resolveMap.set(
			"/workspace/images",
			createFileStat(URI.file("/workspace/images"), false, false, true, false, [
				...
				{
					resource: URI.file("/workspace/images/subfolder"),
					isDirectory: true,
					isFile: false,
				},
			]),
		...
		const { CommandsRegistry } =
			await import("../../../../../platform/commands/common/commands.js");
		const command = CommandsRegistry.getCommand(
			"workbench.action.openImagesInCarousel",
		);
		...
		assert.strictEqual(
			images.length,
			2,
			"Should include 2 image siblings (png + jpg), not txt",
		);
	...
	test("folder opens carousel with all contained images", async () => {
		const fileService = instantiationService.get(IFileService);
		const folderItem = createExplorerItem(
			"/workspace/images",
			true,
			fileService,
			configService,
		...
		assert.strictEqual(
			images.length,
			2,
			"Should include 2 images (gif + webp), not js",
		);
	...
	test("multiple selected images open in carousel", async () => {
		const fileService = instantiationService.get(IFileService);
		const img1 = createExplorerItem(
			"/workspace/a.png",
			false,
			fileService,
			configService,
		);
		const img2 = createExplorerItem(
			"/workspace/b.svg",
			false,
			fileService,
			configService,
		);
		const txtFile = createExplorerItem(
			"/workspace/notes.txt",
			false,
			fileService,
			configService,
	...
	test("empty selection with resource argument opens carousel from that folder", async () => {
		...
		resolveMap.set(
			"/workspace/photos",
			createFileStat(folderUri, false, false, true, false, [
		...
		assert.strictEqual(
			openedInputs.length,
			1,
			"Should open carousel using resource argument fallback",
		);
		const images = openedInputs[0].input.collection.sections[0].images;
		assert.strictEqual(
			images.length,
			2,
			"Should include 2 images from the folder",
		);
	});
	...
	test("empty selection without resource falls back to first workspace folder", async () => {
		...
		resolveMap.set(
			wsRoot.path,
			createFileStat(wsRoot, false, false, true, false, [
		...
		assert.strictEqual(
			openedInputs.length,
			1,
			"Should open carousel using workspace root fallback",
		);
		const images = openedInputs[0].input.collection.sections[0].images;
		assert.strictEqual(
			images.length,
			1,
			"Should include image from workspace root",
		);
	...
	test("empty selection with no images shows notification", async () => {
		...
		assert.strictEqual(
			openedInputs.length,
			0,
			"Should not open carousel when folder has no images",
		);
	...
	test("folder with no images shows notification", async () => {
		...
		resolveMap.set(
			"/workspace/docs",
			createFileStat(URI.file("/workspace/docs"), false, false, true, false, [
				{ resource: URI.file("/workspace/docs/readme.md"), isFile: true },
		...
		assert.strictEqual(
			infoMessages.length,
			1,
			"Should show notification about no images",
		);
	});
	...
	test("folder read error shows error notification", async () => {
		...
		assert.strictEqual(
			openedInputs.length,
			0,
			"Should not open carousel on folder read error",
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			infoMessages.length,
			0,
			"Should not show info notification",
		);
	});
	...
	test("images with URIs are passed lazily without reading file contents", async () => {
		...
		assert.strictEqual(
			readFileCallCount,
			0,
			"readFile should not be called during action",
		);
		assert.strictEqual(
			openedInputs.length,
			1,
			"Should open carousel with lazy image entries",
		);
		const images = openedInputs[0].input.collection.sections[0].images;
		...
		assert.strictEqual(
			images[0].data,
			undefined,
			"Image data should not be loaded eagerly",
		);
	...
	test("folder includes video files alongside images", async () => {
		...
		resolveMap.set(
			"/workspace/media",
			createFileStat(URI.file("/workspace/media"), false, false, true, false, [
		...
		assert.strictEqual(
			images.length,
			4,
			"Should include mp4 + webm + mov + png, not txt",
		);
	...
	test("single video file opens carousel with sibling media", async () => {
		...
		const videoItem = createExplorerItem(
			"/workspace/media/clip.mp4",
			false,
			fileService,
			configService,
			parent,
		...
		assert.strictEqual(
			input.startIndex,
			0,
			"Start index should point to the selected video",
		);
	});
});
