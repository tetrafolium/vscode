...
async function resourcesToClipboard(
	...
	if (resources.length) {
		...
		const copyRelativeOrFullPathSeparator: "/" | "\\" | undefined =
			configurationService.getValue(copyRelativeOrFullPathSeparatorSection);
		if (
			copyRelativeOrFullPathSeparator === "/" ||
			copyRelativeOrFullPathSeparator === "\\"
		) {
...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor) => {
		...
		if (!editors) {
			...
			if (activeGroup.activeEditor) {
				editors = [
					{ groupId: activeGroup.id, editor: activeGroup.activeEditor },
				];
			}
		}
