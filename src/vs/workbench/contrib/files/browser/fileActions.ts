...
async function deleteFiles(
	...
	// Handle readonly
	if (!skipConfirm) {
		const readonlyResources = distinctElements.filter((e) =>
			filesConfigurationService.isReadonly(e.resource),
		);
...
export const pasteFileHandler = async (
			...
			const sourceTargetPairs = coalesce(
				await Promise.all(
					toPaste.files.map(async (fileToPaste) => {
						...
						if (
							uriIdentityService.extUri.isEqual(element.resource, fileToPaste)
						) {
						...
						const targetFile = await findValidPasteFileTarget(
							...
							target,
							{
								...
								allowOverwrite:
									pasteShouldMove || incrementalNaming === "disabled",
							},
							incrementalNaming,
						);
					...
					toPaste.files.map(async (file) => {
						...
						return {
							...
							edit: new ResourceFileEdit(undefined, targetFile, {
								overwrite: incrementalNaming === "disabled",
								contents: (async () =>
									VSBuffer.wrap(new Uint8Array(await file.arrayBuffer())))(),
							}),
						};
					}),
				),
	...
	async function applyCopyResourceEdit(
		...
		const undoLevel =
			configurationService.getValue<IFilesConfiguration>().explorer.confirmUndo;
		const options = {
			confirmBeforeUndo:
