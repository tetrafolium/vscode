	...
	onSaveError(
		...
		if (
			...
			// Otherwise show the message that will lead the user into the save conflict editor.
				...
				secondaryActions.push(
					this.instantiationService.createInstance(ConfigureSaveConflictAction),
				);
			}
		}
		...
		// Any other save error
			...
			// Save As
			primaryActions.push(
				this.instantiationService.createInstance(SaveModelAsAction, model),
			);

			// Revert
			primaryActions.push(
				this.instantiationService.createInstance(RevertModelAction, model),
			);
