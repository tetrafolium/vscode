...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
	...
	taskShellIntegrationStartSequence(cwd: string | URI | undefined): string {
		return (
			VSCodeSequence(
				...
				`${VSCodeOscProperty.HasRichCommandDetection}=True`,
			) +
	...
	getTaskShellIntegrationOutputSequence(
		commandLineInfo: { commandLine: string; nonce: string } | undefined,
	): string {
		return (
			(commandLineInfo
				? VSCodeSequence(
	...
	constructor(
		...
		this._register(
			(this._taskProblemMonitor =
				instantiationService.createInstance(TaskProblemMonitor)),
		);
	...
	public customExecutionComplete(task: Task, result: number): Promise<void> {
		...
		if (!activeTerminal?.terminal) {
			return Promise.reject(
				new Error("Expected to have a terminal for a custom execution task"),
			);
		}
	...
	public terminate(task: Task): Promise<ITaskTerminateResponse> {
		...
		return new Promise<ITaskTerminateResponse>((resolve, reject) => {
			const onDisposedListener = terminal.onDisposed((terminal) => {
				this._fireTaskEvent(
					TaskEvent.terminated(task, terminal.instanceId, terminal.exitReason),
				);
		...
		const promise = Promise.resolve()
			.then(async () => {
				...
				if (task.configurationProperties.dependsOn) {
					...
					for (const dependency of task.configurationProperties.dependsOn) {
						...
						if (dependencyTask) {
							...
							if (
								!this._taskDependencies[taskMapKey].includes(dependencyMapKey)
							) {
							...
							if (nextLiveDependencies.has(commonKey)) {
								...
								if (!taskResult) {
									const activeTask =
										this._activeTasks[dependencyTask.getMapKey()] ??
										this._getInstances(dependencyTask).pop();
									taskResult =
										activeTask && this._getDependencyPromise(activeTask);
								}
							}
							if (!taskResult) {
								this._fireTaskEvent(
									TaskEvent.general(TaskEventKind.DependsOnStarted, task),
								);
							...
							this._log(
								nls.localize(
									...
									Types.isString(dependency.task)
										? dependency.task
										: JSON.stringify(dependency.task, undefined, 0),
				...
				return Promise.all(promises).then(
					(summaries): Async.MaybePromise<ITaskSummary> => {
	...
	private _resolveVariablesFromSet(
		...
		const options =
			task.command && task.command.options ? task.command.options : undefined;
		...
		if (task.configurationProperties.isBackground) {
			...
			toDispose.add(
				watchingProblemMatcher.onDidStateChange((event) => {
					if (
						event.kind === ProblemCollectorEventKind.BackgroundProcessingBegins
					) {
						eventCounter++;
						...
						event.kind === ProblemCollectorEventKind.BackgroundProcessingEnds
					) {
						eventCounter--;
						...
						this._fireTaskEvent(
							TaskEvent.inactive(
								task,
								terminal?.instanceId,
								this._takeTaskDuration(terminal?.instanceId),
							),
						);
						if (eventCounter === 0) {
							if (
								...
								this._fireTaskEvent(
									TaskEvent.problemMatcherEnded(
										task,
										this._taskHasErrors(task),
										terminal?.instanceId,
									),
								);
							}
			...
			if (!terminal) {
				return Promise.reject(
					new Error(`Failed to create terminal for task ${task._label}`),
				);
			}
			...
			this._busyTasks[mapKey] = task;
			this._fireTaskEvent(
				TaskEvent.general(TaskEventKind.Active, task, terminal.instanceId),
			);
			...
			const problemMatcherListener = startStopProblemMatcher.onDidStateChange(
				(event) => {
					if (
						event.kind === ProblemCollectorEventKind.BackgroundProcessingBegins
					) {
			...
			promise = new Promise<ITaskSummary>((resolve, reject) => {
				const onExit = terminal!.onExit((terminalLaunchResult) => {
					...
					this._fireTaskEvent(
						TaskEvent.inactive(task, terminal?.instanceId, durationMs),
					);
					if (
						startStopProblemMatcher.numberOfMatches &&
						startStopProblemMatcher.maxMarkerSeverity &&
					...
					this._fireTaskEvent(
						TaskEvent.general(TaskEventKind.End, task, terminal?.instanceId),
					);
	...
	private _registerCapturedVariables(
		...
		for (const [name, value] of capturedVariables) {
			...
			if (
				!this._configurationResolverService.resolvableVariables.has(
					`taskVar:${name}`,
				)
			) {
				this._configurationResolverService.contributeVariable(
					`taskVar:${name}`,
					async () => this._capturedTaskVariables.get(name),
				);
			}
		}
	}
	...
	private async _createShellLaunchConfig(
		...
		if (isShellCommand) {
			...
			const basename = path.posix
				.basename(
					(await this._pathService.fileURI(shellLaunchConfig.executable!)).path,
				)
				.toLowerCase();
			...
			if (task.command.presentation && task.command.presentation.echo) {
				const getArgsToEcho = (
					args: Types.SingleOrMany<string> | undefined,
				): string => {
	...
	private async _reconnectToTerminal(
		...
		return reconnectedInstances.find(
			(e) => getReconnectionData(e)?.lastTask === task.getCommonTaskId(),
		);
	}
	...
	private _reconnectToTerminals(): void {
		...
		this._logService.trace(
			`Attempting reconnection of ${reconnectedInstances.length} terminals`,
		);
	...
	private async _createTerminal(
		...
		if (task.command.runtime === RuntimeType.CustomExecution) {
			...
			const resolvedResult: { command: CommandString; args: CommandString[] } =
		...
		if (terminalToReuse) {
			...
			if (terminalToReuse.shellIntegrationNonce) {
				if (
					Types.isString(launchConfigs.initialText) &&
			...
			this._terminals[terminalToReuse.terminal.instanceId.toString()].lastTask =
				taskKey;
	...
	public async getTaskForTerminal(
		instanceId: number,
