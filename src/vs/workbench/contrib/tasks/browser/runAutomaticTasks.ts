	...
	implements IWorkbenchContribution
{
	...
	private async _tryRunTasks() {
		...
		const { value, userValue } = this._configurationService.inspect<string>(
	...
	private _getTaskSource(source: TaskSource): URI | undefined {
		...
		switch (taskKind) {
			...
			case ConfigurationTarget.WORKSPACE: {
				return (
					(<WorkspaceFileTaskSource>source).config.workspace?.configuration ??
					undefined
				);
			}
		}
			...
			notificationService.prompt(
				...
				[
					...
					},
					{
						label:
							locations.size === 1
								? nls.localize("openTask", "Open File")
								: nls.localize("openTasks", "Open Files"),
						run: async () => {
