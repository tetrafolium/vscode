	...
	implements ITaskService
{
	...
	private _onDidRegisterSupportedExecutions = this._register(
		new Emitter<void>(),
	);
	private _onDidRegisterAllSupportedExecutions = this._register(
		new Emitter<void>(),
	);
	...
	constructor(
		...
		this._register(
			this._configurationService.onDidChangeConfiguration(async (e) => {
				...
				const mapStringToFolderTasks: Map<string, IWorkspaceFolderTaskResult> =
				...
				for (const [folderUri, folderResult] of mapStringToFolderTasks) {
					...
					for (const task of folderResult.set.tasks) {
						...
						if (
							lastTask &&
							lastTask === realUniqueId &&
							folderUri !== "setting"
						) {
							const verifiedLastTask = new VerifiedTask(
								task,
								this._taskSystem!.lastTask!.resolver,
								Triggers.command,
							);
		...
		this._tasksAvailableState =
			TasksAvailableContext.bindTo(_contextKeyService);
			...
			this.onDidStateChange(async (e) => {
				...
				switch (e.kind) {
					...
					case TaskEventKind.ProcessEnded: {
						...
						const durationMs =
							processEndedEvent.durationMs ?? Date.now() - startTime;
						if (durationMs !== undefined) {
							this._handleLongRunningTaskCompletion(
								processEndedEvent,
								durationMs,
							);
						}
	...
	public registerSupportedExecutions(
		...
		if (
			ServerlessWebContext.getValue(this._contextKeyService) ||
	...
	private async _handleLongRunningTaskCompletion(
		...
		if (
			...
			(notificationThreshold > 0 && durationMs < notificationThreshold)
		) {
			return;
		}
		...
		const terminalForTask = this._terminalService.instances.find(
			(i) => i.instanceId === event.terminalId,
		);
		if (!terminalForTask) {
			return;
		}
		...
		const { clicked } = await this._hostService.showToast(
			{ title: message },
			cts.token,
		);
		this.toast.clear();
		...
		for (const [, workspaceTasks] of tasks) {
			if (workspaceTasks.configurations) {
				for (const taskName of Object.keys(
					workspaceTasks.configurations.byIdentifier,
				)) {
	...
	private async _findWorkspaceTasksInGroup(
		...
		return this._findWorkspaceTasks((task) => {
			...
			if (taskGroup && typeof taskGroup !== "string") {
				return (
					taskGroup._id === group._id && (!isDefault || !!taskGroup.isDefault)
				);
			}
			return false;
		});
	...
	public getRecentlyUsedTasksV1(): LRUCache<string, string> {
		...
		const quickOpenHistoryLimit = this._configurationService.getValue<number>(
	...
	public async getSavedTasks(
		...
		const folderToTasksMap: Map<
			string,
			(TaskConfig.ICustomTask | TaskConfig.IConfiguringTask)[]
		> = new Map();
		const workspaceToTaskMap: Map<
			string,
			(TaskConfig.ICustomTask | TaskConfig.IConfiguringTask)[]
		> = new Map();
	...
	private _isProblemMatcherPromptEnabled(type?: string): boolean {
		...
		const settingValueMap: IStringDictionary<boolean> =
			settingValue as IStringDictionary<boolean>;
	...
	private _getTypeForTask(task: Task): string {
		...
		if (CustomTask.is(task)) {
			const configProperties = task._source.config
				.element as TaskConfig.ICustomTask;
	...
	private _shouldAttachProblemMatcher(task: Task): boolean {
		...
		if (CustomTask.is(task)) {
			const configProperties = task._source.config
				.element as TaskConfig.IConfigurationProperties;
			return (
	...
	private _createCustomizableTask(
		...
		if (taskConfig && taskConfig.element) {
			...
			Object.keys(identifier).forEach(
				(key) =>
					((toCustomize as unknown as Record<string, unknown>)![key] =
						identifier[key]),
			);
			if (
				task.configurationProperties.problemMatchers &&
		...
		if (!fileConfig) {
			...
			if (editorConfig.editor.insertSpaces) {
				content = content.replace(
					/(\n)(\t+)/g,
					(_, s1, s2) =>
						s1 + " ".repeat(s2.length * editorConfig.editor.tabSize),
				);
			}
		...
		switch (kind) {
			case TaskSourceKind.User: {
				return resources.joinPath(
					resources.dirname(this._preferencesService.userSettingsResource),
					"tasks.json",
				);
			}
			case TaskSourceKind.WorkspaceFile: {
	...
	private async _handleExecuteResult(
		...
		if (executeResult.kind === TaskExecuteKind.Active) {
			...
			if (
				(active && active.same && runSource === TaskRunSource.FolderOpen) ||
				runSource === TaskRunSource.Reconnect
			) {
			...
			if (active && active.same) {
				this._handleInstancePolicy(
					executeResult.task,
					executeResult.task.runOptions!.instancePolicy,
				);
			} else {
				throw new TaskError(
					Severity.Warning,
	...
	private async _findUpdatedTask(
		originalTask: Task,
	): Promise<Task | undefined> {
		...
		for (const [_, folderResult] of mapStringToFolderTasks) {
			if (
				!folderResult.set?.tasks?.length &&
				!folderResult.configurations?.byIdentifier
			) {
				continue;
			}
			...
			if (folderResult.configurations?.byIdentifier) {
				for (const [_, configuringTask] of Object.entries(
					folderResult.configurations.byIdentifier,
				)) {
					// Check if this is the same task by ID
			...
			const error = (error: unknown) => {
				try {
					if (!isCancellationError(error)) {
						if (
							error &&
							Types.isString((error as { message?: string }).message)
						) {
		...
		return customTasksKeyValuePairs.map(async ([key, folderTasks]) => {
			...
			if (this._contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				if (configurations || legacyTaskConfigurations) {
					...
					if (configurations) {
						Object.keys(configurations.byIdentifier).forEach((key) =>
							unUsedConfigurations.add(key),
						);
					}
					...
					const unUsedConfigurationPromises = unUsedConfigurationsAsArray.map(
						async (value) => {
							...
							for (const [handle, provider] of this._providers) {
								...
								if (configuringTask.type === providerType) {
									...
									try {
										const resolvedTask =
											await provider.resolveTask(configuringTask);
										if (
											resolvedTask &&
											resolvedTask._id === configuringTask._id
										) {
											result.add(
												key,
	...
	private async _getAFolder(): Promise<IWorkspaceFolder> {
		let folder =
			this.workspaceFolders.length > 0 ? this.workspaceFolders[0] : undefined;
		if (!folder) {
	...
	protected async _computeWorkspaceTasks(
		...
		const hasAnyTasks = Array.from(result.values()).some(
			(folderResult) =>
				...
				(folderResult.configurations?.byIdentifier &&
					Object.keys(folderResult.configurations.byIdentifier).length > 0),
		);
	...
	private get _jsonTasksSupported(): boolean {
		return (
			ShellExecutionSupportedContext.getValue(this._contextKeyService) ===
				true &&
			ProcessExecutionSupportedContext.getValue(this._contextKeyService) ===
				true
		);
	}
	...
	private async _computeWorkspaceFolderTasks(
		...
		await ProblemMatcherRegistry.onReady();
		const taskSystemInfo: ITaskSystemInfo | undefined = this._getTaskSystemInfo(
			workspaceFolder.uri.scheme,
		);
		const problemReporter = new ProblemReporter(this._outputChannel);
		this._register(
			problemReporter.onDidError((error) =>
				this._showOutput(runSource, undefined, error),
			),
		);
	...
	private _testParseExternalConfig(
		...
		const parseErrors: string[] = (config as unknown as Record<string, unknown>)
	...
	protected _getConfiguration(
		...
		const parseErrors: string[] = (result as unknown as Record<string, unknown>)
			.$parseErrors as string[];
		if (parseErrors) {
			let isAffected = false;
			for (const parseError of parseErrors) {
	...
	private _handleError(err: unknown): void {
		...
		if (showOutput) {
			this._showOutput(
				undefined,
				undefined,
				Types.isString(err) ? (err as string) : undefined,
			);
		}
	}
	...
	private async _showQuickPick(
		...
		const entries: (ITaskQuickPickEntry | IQuickPickSeparator)[] | undefined =
			await raceTimeout(
	...
	private async _trust(): Promise<boolean> {
		...
		if (
			ServerlessWebContext.getValue(this._contextKeyService) &&
			!TaskExecutionSupportedContext?.evaluate(context)
		) {
			return false;
		}
	...
	private async _runTaskCommand(
		...
		const taskName =
			typeof filter === "string" ? filter : (filter.task as string);
	...
	private _doRunTaskCommand(
		...
		const pickThen = (task: Task | undefined | null) => {
			...
			if (task === null) {
				...
				this.run(task, { attachProblemMatcher: true }, TaskRunSource.User).then(
					undefined,
					(reason) => {
	...
	private _reRunTaskCommand(onlyRerun?: boolean): void {
		ProblemMatcherRegistry.onReady().then(() => {
			return this._editorService
				.saveAll({ reason: SaveReason.EXPLICIT })
				.then(() => {
	...
	private _getDefaultTasks(
		...
		for (const task of tasks.filter((t) => !!t.configurationProperties.group)) {
			...
			if (
				taskGlobsInList &&
				typeof (task.configurationProperties.group as TaskGroup).isDefault ===
					"string"
			) {
				defaults.push(task);
			} else if (
				!taskGlobsInList &&
				(task.configurationProperties.group as TaskGroup).isDefault === true
			) {
				defaults.push(task);
			}
		}
	...
	private _runTaskGroupCommand(
		...
		const promise = (async () => {
			async function runSingleTask(
				...
				that
					.run(task, problemMatcherOptions, TaskRunSource.User)
			...
			const { globGroupTasks, globTasksDetected } = await this._getGlobTasks(
				taskGroup._id,
			);
				...
				if (configuredTasks) {
					...
					if (globTasksDetected) {
						...
						const relativePath = workspaceFolder?.uri
							? (resources.relativePath(workspaceFolder.uri, absoluteURI) ??
								absoluteURI.path)
							: absoluteURI.path;
	...
	private async _runRestartTaskCommand(
		arg?: string | ITaskIdentifier,
	): Promise<void> {
	...
	private _openTaskFile(resource: URI, taskSource: string) {
		...
		this._fileService
			...
			.then(async (stat) => {
				...
				let content;
				if (!tasksExistInFile) {
					const pickTemplateResult = await this._quickInputService.pick(
						getTaskTemplates(),
						{
							placeHolder: nls.localize(
	...
	private _isTaskEntry(
		...
		const candidate: IQuickPickItem & { task: Task } =
			value as IQuickPickItem & { task: Task };
	...
	private _isSettingEntry(
		...
		const candidate: IQuickPickItem & { settingType: string } =
			value as IQuickPickItem & { settingType: string };
					...
					this._quickInputService
						...
						.then(async (entry) => {
							...
							const task: Task | undefined | null =
								entry && Object.hasOwn(entry, "task")
									? (entry as IQuickPickItem & { task: Task }).task
									: undefined;
							if (task === undefined || task === null) {
								return;
							}
			...
			this.tasks().then((tasks) => {
				...
				this._showIgnoredFoldersMessage().then(() => {
					this._showQuickPick(
						...
						const task: Task | undefined | null =
							entry && Object.hasOwn(entry, "task") ? entry.task : undefined;
						if (!task) {
							return;
						}
						if (task === selectedTask && CustomTask.is(task)) {
		...
		if (
			...
			if (
				...
				configElement.command = (
					task._source.config.element as Record<string, unknown>
				).command as string | CommandString;
			}
			if (
				task.command.args &&
	...
	private async _upgrade(): Promise<void> {
		...
		if (!this._workspaceTrustManagementService.isWorkspaceTrusted()) {
			this._register(
				Event.once(this._workspaceTrustManagementService.onDidChangeTrust)(
					(isTrusted) => {
						if (isTrusted) {
							this._upgrade();
						}
		...
		for (const folder of this.workspaceFolders) {
			...
			if (
				this._configurationService.getValue(TasksSchemaProperties.ShowOutput, {
					resource: folder.uri,
				})
			) {
