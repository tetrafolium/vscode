...
suite("ProblemPatternRegistry - msCompile", () => {
	...
	test("matches lines without location information", () => {
		...
		const line =
			"Main.cs: warning CS0168: The variable 'x' is declared but never used";
		const result = matcher.handle([line]);
		assert.ok(result.match);
		const marker = result.match!.marker;
	...
	test("matches lines with build prefixes and fatal errors", () => {
		...
		const line =
			"  1>c:/workspace/app.cs(12): fatal error C1002: Fatal diagnostics";
		const result = matcher.handle([line]);
		assert.ok(result.match);
		const marker = result.match!.marker;
	...
	test("matches info diagnostics with codes", () => {
		...
		const line =
			"2>/workspace/app.cs(20,5): info INF1001: Informational diagnostics";
		const result = matcher.handle([line]);
		assert.ok(result.match);
		const marker = result.match!.marker;
	...
	test("matches lines with subcategory prefixes", () => {
		...
		const line =
			"Main.cs(17,20): subcategory warning CS0168: The variable 'x' is declared but never used";
		const result = matcher.handle([line]);
		assert.ok(result.match);
		const marker = result.match!.marker;
		assert.strictEqual(marker.code, "CS0168");
	...
	test("matches complex diagnostics with all qualifiers", () => {
		...
		const line =
			"  12>c:/workspace/Main.cs(42,7,43,2): subcategory fatal error CS9999: Complex diagnostics";
		const result = matcher.handle([line]);
		assert.ok(result.match);
		const marker = result.match!.marker;
