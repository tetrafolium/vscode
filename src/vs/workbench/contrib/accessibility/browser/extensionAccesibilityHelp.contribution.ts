...
export class ExtensionAccessibilityHelpDialogContribution extends Disposable {
	...
	constructor(@IKeybindingService keybindingService: IKeybindingService) {
		super();
		this._register(
			Registry.as<IViewsRegistry>(Extensions.ViewsRegistry).onViewsRegistered(
				(e) => {
					for (const view of e) {
		...
		this._register(
			Registry.as<IViewsRegistry>(Extensions.ViewsRegistry).onViewsDeregistered(
				(e) => {
...
function registerAccessibilityHelpAction(
	...
	disposableStore.add(
		keybindingService.onDidUpdateKeybindings(() => {
			...
			disposableStore.add(
				registerAccessibilityHelpAction(keybindingService, viewDescriptor),
			);
		}),
	);
