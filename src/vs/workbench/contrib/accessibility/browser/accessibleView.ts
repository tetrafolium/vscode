...
export class AccessibleView extends Disposable {
	...
	constructor(
		...
		this._hasUnassignedKeybindings =
			accessibleViewHasUnassignedKeybindings.bindTo(this._contextKeyService);
		...
		const codeEditorWidgetOptions: ICodeEditorWidgetOptions = {
			contributions: EditorExtensionsRegistry.getEditorContributions().filter(
				(c) =>
					c.id !== CodeActionController.ID &&
					c.id !== FloatingEditorClickMenu.ID &&
					c.id !== FloatingEditorToolbar.ID,
			),
		};
		...
		this._register(
			this._configurationService.onDidChangeConfiguration((e) => {
				if (
					isIAccessibleViewContentProvider(this._currentProvider) &&
	...
	private _playDiffSignals(): void {
		if (
			this._currentProvider?.id !== AccessibleViewProviderId.DiffEditor &&
			this._currentProvider?.id !== AccessibleViewProviderId.InlineCompletions
		) {
			return;
		}
	...
	setPosition(position: Position, reveal?: boolean, select?: boolean): void {
		...
		if (select) {
			const lineLength =
				this._editorWidget.getModel()?.getLineLength(position.lineNumber) ?? 0;
			if (lineLength) {
	...
	getCodeBlockContext(): ICodeBlockActionContext | undefined {
		...
		return {
			...
			codeBlockIndex,
			element: undefined,
			chatSessionResource: codeBlock.chatSessionResource,
		};
	}
			...
			onHide: () => {
				if (!showAccessibleViewHelp) {
					...
					if (this._currentProvider) {
						...
						if (currentPosition) {
							this._lastProviderPosition.set(
	...
	getSymbols(): IAccessibleViewSymbol[] | undefined {
		...
		const symbols: IAccessibleViewSymbol[] =
			"getSymbols" in provider ? provider.getSymbols?.() || [] : [];
		if (symbols?.length) {
			return symbols;
		}
		...
		const markdownTokens: marked.TokensList | undefined = marked.marked.lexer(
	...
	private _getStableUri(providerId: string): URI {
		return URI.from({
			path: `accessible-view-${providerId}`,
	...
	private _render(
		...
		const previousPosition = isSameProvider
			? this._editorWidget.getPosition()
			: undefined;
		...
		const hide = (e?: KeyboardEvent | IKeyboardEvent): void => {
			const thisWindowIsFocused = getWindow(
				this._editorWidget.getDomNode(),
			).document.hasFocus();
	...
	private _layout(): void {
		...
		const maxHeight =
			dimension.height && dimension.height * DIMENSIONS.MAX_HEIGHT_RATIO;
		const height = Math.min(maxHeight, this._editorWidget.getContentHeight());
		const width = Math.min(
			dimension.width * DIMENSIONS.WIDTH_RATIO,
			DIMENSIONS.MAX_WIDTH,
		);
