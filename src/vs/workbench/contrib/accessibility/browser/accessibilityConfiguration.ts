...
export const accessibilityHelpIsShown = new RawContextKey<boolean>(
...
export const accessibilityConfigurationNodeBase =
...
export class DynamicSpeechAccessibilityConfiguration
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(@ISpeechService private readonly speechService: ISpeechService) {
		...
		this._register(
			Event.runAndSubscribe(speechService.onDidChangeHasSpeechProvider, () =>
...
Registry.as<IConfigurationMigrationRegistry>(
...
).registerConfigurationMigrations([
	{
		...
		migrateFn: (value, accessor) => {
			...
			const debouncePositionChanges =
				getDebouncePositionChangesFromConfig(accessor);
...
function getDelaysFromConfig(
	...
	return (
		...
		accessor("accessibility.signalOptions")?.["experimental.delays"]?.[
			`${type}`
		] ||
...
function getDebouncePositionChangesFromConfig(
	accessor: (key: string) => any,
): number | undefined {
	return (
			...
			item.legacySoundSettingsKey
				? {
						...
						migrateFn: (sound, accessor) => {
							...
							const legacyAnnouncementSettingsKey =
								item.legacyAnnouncementSettingsKey;
							let announcement: string | undefined;
							if (legacyAnnouncementSettingsKey) {
								announcement =
									accessor(legacyAnnouncementSettingsKey) ?? undefined;
								if (
	...
	AccessibilitySignal.allAccessibilitySignals
		...
		.map((item) => ({
			...
			migrateFn: (announcement, accessor) => {
				const configurationKeyValuePairs: ConfigurationKeyValuePairs = [];
				const sound =
					accessor(item.settingsKey)?.sound ||
					accessor(item.legacySoundSettingsKey!);
				...
				configurationKeyValuePairs.push([
					...
					{
						value:
							announcement !== undefined ? { announcement, sound } : { sound },
					},
				]);
				configurationKeyValuePairs.push([
					`${item.legacyAnnouncementSettingsKey}`,
