 ...
 */
function getSessionsWindowTrustNote(
	...
	implements IWorkbenchContribution
{
	...
	private registerListeners(): void {
		...
		this._register(
			this.workspaceTrustRequestService.onDidInitiateResourcesTrustRequest(
				async (options) => {
					await this.workspaceTrustManagementService.workspaceResolved;
					...
					const sessionsTrustNote = getSessionsWindowTrustNote(
		...
		this._register(
			this.workspaceTrustRequestService.onDidInitiateWorkspaceTrustRequest(
				async (requestOptions) => {
					...
					if (sessionsTrustNote) {
						markdownDetails.push({
							markdown: new MarkdownString(sessionsTrustNote),
						});
					}
			...
			this.workspaceTrustRequestService.onDidInitiateWorkspaceTrustRequestOnStartup(
				async () => {
					...
					if (
						...
						trustOption =
							this.productService.aiGeneratedWorkspaceTrust.trustOption;
						dontTrustOption =
							this.productService.aiGeneratedWorkspaceTrust.dontTrustOption;
					} else {
						console.warn(
					...
					if (
						...
						const name = basename(
							uriDirname(
								(workspaceIdentifier as ISingleFolderWorkspaceIdentifier).uri,
							),
						);
	...
	implements IWorkbenchContribution
{
	constructor(
		...
		this.workspaceTrustManagementService.workspaceTrustInitialized.then(() => {
			...
			this._register(
				this.workspaceTrustManagementService.onDidChangeTrust((isTrusted) =>
					this.logWorkspaceTrust(isTrusted),
				),
			);
		});
	}
