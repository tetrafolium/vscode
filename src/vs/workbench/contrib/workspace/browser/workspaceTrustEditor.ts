...
class WorkspaceTrustedUrisTable extends Disposable {
	private readonly _onDidAcceptEdit: Emitter<ITrustedUriItem> = this._register(
		new Emitter<ITrustedUriItem>(),
	);
	readonly onDidAcceptEdit: Event<ITrustedUriItem> =
		this._onDidAcceptEdit.event;

	private readonly _onDidRejectEdit: Emitter<ITrustedUriItem> = this._register(
		new Emitter<ITrustedUriItem>(),
	);
	...
	private get currentWorkspaceUri(): URI {
		return (
			this.workspaceService.getWorkspace().folders[0]?.uri || URI.file("/")
		);
	}
...
class TrustedUriActionsColumnRenderer implements ITableRenderer<
	ITrustedUriItem,
...
class TrustedUriPathColumnRenderer implements ITableRenderer<
	ITrustedUriItem,
	ITrustedUriPathColumnTemplateData
> {
...
class TrustedUriHostColumnRenderer implements ITableRenderer<
	ITrustedUriItem,
