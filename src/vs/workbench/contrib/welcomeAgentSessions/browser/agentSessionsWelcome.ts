...
import {
	...
	AgentSessionTarget,
} from "../../chat/browser/agentSessions/agentSessions.js";
...
} from "../../chat/browser/chatSessions/chatSessions.contribution.js";
import { IChatEntitlementService } from "../../../services/chat/common/chatEntitlementService.js";
import {
	AgentSessionsControl,
	IAgentSessionsControlOptions,
} from "../../chat/browser/agentSessions/agentSessionsControl.js";
...
} from "../../welcomeGettingStarted/browser/gettingStartedService.js";
import {
	GettingStartedEditorOptions,
	GettingStartedInput,
} from "../../welcomeGettingStarted/browser/gettingStartedInput.js";
...
import {
	canShowAgentsBanner,
	createAgentsBanner,
} from "../../chat/browser/agentSessions/agentSessionsBanner.js";
...
const WELCOME_CHAT_INPUT_MAX_HEIGHT_OVERRIDE =
	WELCOME_CHAT_INPUT_LAYOUT_HEIGHT +
	WELCOME_CHAT_INPUT_RESERVED_LIST_HEIGHT +
	WELCOME_CHAT_INPUT_RESERVED_CHROME_HEIGHT;
...
type AgentSessionsWelcomeClosedClassification = {
	visibleDurationMs: {
		...
		isMeasurement: true;
		comment: "How long the welcome page was visible in milliseconds.";
	};
	closedBy: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "What action caused the welcome page to close.";
	};
	owner: "osortega";
...
type AgentSessionsWelcomeChatSubmittedClassification = {
	mode: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The chat mode used (ask, agent, edit).";
	};
	provider: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The session provider (local, cloud).";
	};
	workspaceKind: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The type of workspace - empty, folder, or workspace.";
	};
	selectedRecentWorkspace: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether a recent workspace was selected before submitting.";
	};
	owner: "osortega";
...
type AgentSessionsWelcomeActionClassification = {
	action: {
		...
		purpose: "FeatureInsight";
		comment: "The action being executed on the agent sessions welcome page.";
	};
	actionId: {
		classification: "PublicNonPersonalData";
		purpose: "FeatureInsight";
		comment: "Identifier of the action being executed, such as command ID or walkthrough ID.";
	};
	welcomeKind: {
		classification: "PublicNonPersonalData";
		purpose: "FeatureInsight";
		comment: "The kind of welcome page";
	};
	owner: "osortega";
...
export class AgentSessionsWelcomePage extends EditorPane {
	...
	private _selectedSessionProvider: AgentSessionTarget =
	...
	override async setInput(
		input: AgentSessionsWelcomeInput,
		options: AgentSessionsWelcomeEditorOptions | undefined,
	...
	override clearInput(): void {
		...
		if (this._openedAt > 0) {
			...
			this.telemetryService.publicLog2<
				AgentSessionsWelcomeClosedEvent,
				AgentSessionsWelcomeClosedClassification
	...
	private async buildStartEntries(container: HTMLElement): Promise<void> {
		...
		for (const entry of entries) {
			...
			button.onclick = () => {
				this.telemetryService.publicLog2<
					AgentSessionsWelcomeActionEvent,
					AgentSessionsWelcomeActionClassification
	...
	private buildChatWidget(container: HTMLElement): void {
		...
		const editorOverflowWidgetsDomNode = this.layoutService
			.getContainer(getWindow(chatWidgetContainer))
		...
		const recreateSessionForProvider = async (provider: AgentSessionTarget) => {
			if (this.chatWidget && this.chatModelRef) {
				...
				this.chatModelRef =
					ref ?? this.chatService.startNewLocalSession(ChatAgentLocation.Chat);
		...
		const workspacePickerDelegate: IWorkspacePickerDelegate | undefined = this
			._isEmptyWorkspace
			? {
					getWorkspaces: () =>
						this._recentTrustedWorkspaces.map((w) => ({
					...
					setSelectedWorkspace: (
						workspace: IWorkspacePickerItem | undefined,
					) => {
		...
		this.chatWidget = this.contentDisposables.add(
			scopedInstantiationService.createInstance(
				...
				{}, // Empty resource view context
				{
					...
					submitHandler: this._isEmptyWorkspace
						? (query, mode) => this.handleWorkspaceSubmission(query, mode)
						: undefined,
				},
				{
		...
		this.contentDisposables.add(
			this.chatService.onDidSubmitRequest(({ chatSessionResource }) => {
				if (
					this.chatModelRef?.object?.sessionResource.toString() ===
					chatSessionResource.toString()
				) {
					// Send chat submitted telemetry
					const mode =
						this.chatWidget?.input.currentModeObs.get().name.get() || "unknown";
					this.telemetryService.publicLog2<
						AgentSessionsWelcomeChatSubmittedEvent,
						AgentSessionsWelcomeChatSubmittedClassification
	...
	private getWorkspaceLabel(
		workspace: IRecentWorkspace | IRecentFolder,
	): string {
	...
	private async handleWorkspaceSubmission(
		query: string,
		mode: ChatModeKind,
	): Promise<boolean> {
		...
		const workspace = this._recentTrustedWorkspaces.find(
			(w) =>
				this.getWorkspaceUri(w).toString() ===
				this._selectedWorkspace?.uri.toString(),
		);
	...
	private buildSessionsGrid(
		...
		const options: IAgentSessionsControlOptions = {
			...
			source: "welcomeView",
			notifySessionOpened: () => {
				const isProjectionEnabled = this.configurationService.getValue<boolean>(
	...
	private buildWalkthroughs(container: HTMLElement): void {
		const activeWalkthroughs = this.walkthroughs
			.filter((w) => !w.when || this.contextService.contextMatchesRules(w.when))
	...
	private static readonly PRIVACY_NOTICE_DISMISSED_KEY =
		"agentSessionsWelcome.privacyNoticeDismissed";
	...
	private buildPrivacyNotice(container: HTMLElement): void {
		...
		const renderedMarkdown =
			this.markdownRendererService.render(descriptionMarkdown);
	...
	private buildFooter(container: HTMLElement): void {
		...
		const showOnStartupCheckbox = this.contentDisposables.add(
			new Toggle({
				icon: Codicon.check,
	...
	private layoutSessionsControl(): void {
		...
		const visibleSessions = Math.min(
			this.agentSessionsService.model.sessions.filter((s) => !s.isArchived())
				.length,
			MAX_SESSIONS,
		);
		const sessionsHeight =
			visibleSessions * AgentSessionsListDelegate.ITEM_HEIGHT;
		...
		const marginOffset =
			Math.floor(visibleSessions / 2) * AgentSessionsListDelegate.ITEM_HEIGHT;
	...
	private async closeEditorAndMaximizeAuxiliaryBar(
		sessionResource?: URI,
	): Promise<void> {
		...
		const chatViewLocation =
			this.viewDescriptorService.getViewLocationById(ChatViewId);
		if (chatViewLocation === ViewContainerLocation.AuxiliaryBar) {
			this.layoutService.setAuxiliaryBarMaximized(true);
		}
	}

	private async getRecentlyOpenedWorkspaces(
		onlyTrusted: boolean = false,
	): Promise<Array<IRecentWorkspace | IRecentFolder>> {
		...
		const trustInfoPromises = workspaces.workspaces.map(async (ws) => {
			const uri = isRecentWorkspace(ws)
				? ws.workspace.configPath
				: ws.folderUri;
			const trustInfo =
				await this.workspaceTrustManagementService.getUriTrustInfo(uri);
