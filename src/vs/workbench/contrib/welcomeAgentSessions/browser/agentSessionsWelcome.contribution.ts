...
const agentSessionsWelcomeInputTypeId =
...
class AgentSessionsWelcomeEditorResolverContribution
	extends Disposable
					...
					createEditorInput: () => {
						return {
							editor: instantiationService.createInstance(
								...
								{ workspaceKind: getWorkspaceKind(workspaceContextService) },
							),
						};
					},
				},
...
registerAction2(
	class OpenAgentSessionsWelcomeAction extends Action2 {
		constructor() {
			super({
...
class AgentSessionsWelcomeRunnerContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private async run(): Promise<void> {
		...
		const input = this.instantiationService.createInstance(
			...
			{
				initiator: "startup",
				workspaceKind: getWorkspaceKind(this.workspaceContextService),
			},
		);
