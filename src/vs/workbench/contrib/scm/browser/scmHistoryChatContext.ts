...
export function extractSCMHistoryItemDropData(
	e: DragEvent,
): SCMHistoryItemTransferData[] | undefined {
...
class SCMHistoryItemContext implements IChatContextPickerItem {
	...
	private readonly _delayer = new ThrottledDelayer<
		IChatContextPickerPickItem[]
	>(200);
	...
	isEnabled(widget: IChatWidget): Promise<boolean> | boolean {
		...
		const supported =
			!!widget.attachmentCapabilities.supportsSourceControlAttachments;
		return (
			activeRepository?.repository.provider.historyProvider.get() !==
				undefined && supported
		);
	}
			...
			picks: picksWithPromiseFn((query: string, token: CancellationToken) => {
				...
				return this._delayer.trigger(() => {
					return historyProvider
						.provideHistoryItems(
							{ historyItemRefs, filterText, limit: 100 },
							token,
						)
						.then((historyItems) => {
							if (!historyItems) {
							...
							return historyItems.map((historyItem) => {
								...
								if (historyItem.statistics) {
									details.push(
										`${historyItem.statistics.files} ${localize("files", "file(s)")}`,
									);
								}
								...
								return {
									...
									asAttachment: () =>
										SCMHistoryItemContext.asAttachment(
											activeRepository.repository.provider,
											historyItem,
										),
...
class SCMHistoryItemContextContentProvider implements ITextModelContentProvider {
	...
	async provideTextContent(resource: URI): Promise<ITextModel | null> {
		...
		const historyItemContext =
			await historyProvider.resolveHistoryItemChatContext(historyItemId);
...
export class SCMHistoryItemChangeRangeContentProvider implements ITextModelContentProvider {
	...
	async provideTextContent(resource: URI): Promise<ITextModel | null> {
		...
		const historyItemChangeRangeContext =
	...
	class extends Action2 {
		...
		override async run(
			...
			widget.attachmentModel.addContext(
				SCMHistoryItemContext.asAttachment(provider, historyItem),
			);
		}
	},
);
