	...
	renderElement(
			...
			if (repositoriesWithRootUri.length > 1) {
				...
				const shortDescription = shorten(
					repositoriesWithSameName.map((r) =>
						this.labelService.getUriLabel(dirname(r.provider.rootUri!), {
							relative: true,
						}),
					),
				);
		...
		templateData.elementDisposables.add(
			autorun((reader) => {
				...
				statusPrimaryActions = commands.map((c) =>
					reader.store.add(new StatusBarAction(c, this.commandService)),
				);
				updateToolbar();
			}),
		);
