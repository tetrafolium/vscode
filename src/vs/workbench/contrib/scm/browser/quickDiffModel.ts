...
export class QuickDiffModel extends Disposable {
	...
	private readonly _originalEditorModels =
		new ResourceMap<IResolvedTextEditorModel>();
	...
	public getQuickDiffResults(): QuickDiffResult[] {
		return this._quickDiffs.map((quickDiff) => {
			const changes = this.allChanges.filter(
				(change) => change.providerId === quickDiff.id,
			);
	...
	private setChanges(
		...
		const diff = sortedDiff(this.changes, changes, (a, b) =>
	...
	private diff(): Promise<{
		...
		const location = this.environmentService.isSessionsWindow
		...
		return this.progressService.withProgress(
			...
			async () => {
				...
				const quickDiffPrimary = quickDiffs.find(
					(quickDiff) => quickDiff.kind === "primary",
				);

				const ignoreTrimWhitespaceSetting = this.configurationService.getValue<
					"true" | "false" | "inherit"
				...
				for (const quickDiff of quickDiffs) {
					...
					if (
						diff.changes &&
						diff.changes2 &&
						diff.changes.length === diff.changes2.length
					) {
						for (let index = 0; index < diff.changes.length; index++) {
							...
							if (quickDiffPrimary && quickDiff.kind === "secondary") {
								...
								if (primaryQuickDiffChange) {
									...
									const primaryModel = this._originalEditorModels.get(
										quickDiffPrimary.originalResource,
									)?.textEditorModel;
									const primaryContent = primaryModel?.getValueInRange(
										primaryQuickDiffChange.change2.toRangeMapping()
											.originalRange,
									);

									const secondaryModel = this._originalEditorModels.get(
										quickDiff.originalResource,
									)?.textEditorModel;
									const secondaryContent = secondaryModel?.getValueInRange(
										change2.toRangeMapping().originalRange,
									);
				...
				const diffsSorted = diffs.sort((a, b) =>
					compareChanges(a.change, b.change),
				);
				const allDiffsSorted = [...diffs, ...secondaryDiffs].sort((a, b) =>
					compareChanges(a.change, b.change),
				);
	...
	private async _diff(
		...
		changes: readonly IChange[] | null;
		changes2: readonly LineRangeMapping[] | null;
	}> {
		const maxComputationTimeMs =
			this.options.maxComputationTimeMs ?? Number.MAX_SAFE_INTEGER;
					...
					await Promise.all(
						quickDiffs.map(async (quickDiff) => {
							try {
								...
								if (
									isTextFileEditorModel(ref.object) &&
									!ref.object.isDirty()
								) {
									const encoding = this._model.getEncoding();

									if (encoding) {
										(ref.object as ITextFileEditorModel).setEncoding(
											encoding,
											EncodingMode.Decode,
										);
									}
	...
	private async getOriginalResource(): Promise<QuickDiff[]> {
		...
		const isBeingModifiedByChatEdits =
			this._chatEditingService.editingSessionsObs
				.get()
				.some(
					(session) =>
						session.getEntry(uri)?.state.get() ===
