...
class SCMRepositoryActionViewItem extends ActionViewItem {
	...
	protected override updateLabel(): void {
		if (this.options.label && this.label) {
			...
			const icon = $(".icon");
			const iconClassNameArray = ThemeIcon.isThemeIcon(
				this._repository.provider.iconPath,
			)
				...
				menu: {
					...
					when: ContextKeyExpr.and(
						...
						ContextKeyExpr.equals(
							"config.scm.repositories.selectionMode",
							"multiple",
						),
					),
					group: "navigation",
		...
		override async run(
			...
			if (
				...
				if (historyItem.parentIds.length > 0) {
					...
					if (
						historyItem.parentIds[0] === SCMIncomingHistoryItemId &&
						historyItemRemoteRef
					) {
						historyItemParentId =
							await historyProvider.resolveHistoryItemRefsCommonAncestor([
...
class ListDelegate implements IListVirtualDelegate<TreeElement> {
	...
	getTemplateId(element: TreeElement): string {
		if (isSCMHistoryItemViewModelTreeElement(element)) {
			...
			isSCMHistoryItemChangeViewModelTreeElement(element) ||
			isSCMHistoryItemChangeNode(element)
		) {
...
> {
	...
	renderElement(
		...
		templateData.graphContainer.classList.toggle(
			"current",
			historyItemViewModel.kind === "HEAD",
		);
		templateData.graphContainer.classList.toggle(
			"incoming-changes",
		...
		templateData.actionBar.setActions(
			getActionBarActions(actions, "inline").primary,
		);
	}

	renderCompressedElements(
		node: ITreeNode<
			ICompressedTreeNode<SCMHistoryItemViewModelTreeElement>,
			LabelFuzzyScore
		>,
		index: number,
		templateData: HistoryItemTemplate,
...
> {
	...
	renderElement(
		...
		const fileKind = isSCMHistoryItemChangeViewModelTreeElement(
			elementOrNode.element,
		)
			? FileKind.FILE
			: FileKind.FOLDER;
	...
	renderCompressedElements(
		...
		const historyItemViewModel =
			compressed.elements[0].context.historyItemViewModel;
		const graphColumns =
			compressed.elements[0].context.historyItemViewModel.outputSwimlanes;
	...
	private _renderGraphPlaceholder(
		...
		const marginLeft =
			graphPlaceholderSvgWidth - 16; /* .monaco-tl-indent left */
		...
		templateData.graphPlaceholder.appendChild(
			renderSCMHistoryGraphPlaceholder(
				graphColumns,
				getHistoryItemIndex(historyItemViewModel),
			),
		);
	}
...
> {
	...
	renderElement(
		...
		const pageOnScroll =
			this._configurationService.getValue<boolean>("scm.graph.pageOnScroll") ===
			true;
	...
	renderCompressedElements(
		node: ITreeNode<
			ICompressedTreeNode<SCMHistoryItemLoadMoreTreeElement>,
			void
		>,
		index: number,
		templateData: LoadMoreTemplate,
...
class SCMHistoryTreeCompressionDelegate implements ITreeCompressionDelegate<TreeElement> {
	isIncompressible(element: TreeElement): boolean {
		if (ResourceTree.isResourceNode(element)) {
			return (
				element.childrenCount === 0 || !element.parent || !element.parent.parent
			);
		}
	...
	implements IAsyncDataSource<SCMHistoryViewModel, TreeElement>
{
	...
	async getChildren(
		...
		if (inputOrElement instanceof SCMHistoryViewModel) {
			...
			const historyProvider =
				inputOrElement.repository.provider.historyProvider.get();
			...
			if (this.viewMode() === ViewMode.List) {
					...
					...historyItemChanges.map(
						(change) =>
							({
								...
								graphColumns:
									inputOrElement.historyItemViewModel.outputSwimlanes,
				...
				// Tree
				const rootUri =
					inputOrElement.repository.provider.rootUri ?? URI.file("/");
			...
			ResourceTree.isResourceNode(inputOrElement) &&
			isSCMHistoryItemChangeNode(inputOrElement)
		) {
			// Tree
	...
	hasChildren(inputOrElement: SCMHistoryViewModel | TreeElement): boolean {
		return (
			...
			(isSCMHistoryItemChangeNode(inputOrElement) &&
				inputOrElement.childrenCount > 0)
		);
	}
}
...
class SCMHistoryTreeDragAndDrop implements ITreeDragAndDrop<TreeElement> {
	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
		...
		const historyItems = this._getDragAndDropData(
		...
		originalEvent.dataTransfer.setData(
			CodeDataTransfers.SCM_HISTORY_ITEM,
	...
	private _getDragAndDropData(
		data: ElementsDragAndDropData<TreeElement, TreeElement[]>,
	): SCMHistoryItemTransferData[] {
...
class SCMHistoryViewModel extends Disposable {
	...
	constructor(
		...
		this._scmHistoryViewModeCtx = ContextKeys.SCMHistoryViewMode.bindTo(
			this._contextKeyService,
		);
	...
	getHistoryItemsFilter(): "all" | "auto" | ISCMHistoryItemRef[] | undefined {
		...
		const filterState =
			this._repositoryFilterState.get(getProviderKey(repository.provider)) ??
			"auto";
	...
	async getHistoryItems(): Promise<SCMHistoryItemViewModelTreeElement[]> {
		...
		if (!state || state.loadMore !== false) {
			...
			const limit = clamp(
				this._configurationService.getValue<number>("scm.graph.pageSize"),
				1,
				1000,
			);
	...
	private _getViewMode(): ViewMode {
		let mode =
			this._configurationService.getValue<"tree" | "list">(
				"scm.defaultViewMode",
			) === "list"
				? ViewMode.List
				: ViewMode.Tree;
	...
	private async _resolveHistoryItemFilter(
		...
		switch (historyItemsFilter) {
			...
			default: {
				...
				const refs = (
					(await historyProvider.provideHistoryItemRefs(historyItemsFilter)) ??
					[]
				).filter((ref) =>
	...
	async pickHistoryItemRef(): Promise<HistoryItemRefsFilter | undefined> {
		...
		if (this._historyItemsFilter === "all") {
			...
			while (index < items.length) {
				...
				if (
					this._historyItemsFilter.some((ref) => ref.id === items[index].id)
				) {
						...
						if (
							...
							quickPick.selectedItems = [
								...quickPick.selectedItems.filter(
									(i) =>
										i.historyItemRef !== "all" && i.historyItemRef !== "auto",
								),
							];
						}
					}
			...
			this._store.add(
				quickPick.onDidAccept(() => {
					if (selectedItems.length === 0) {
						...
						resolve(
							selectedItems.map(
								(item) => (item.historyItemRef as ISCMHistoryItemRef).id,
							),
						);
					}
...
export class SCMHistoryViewPane extends ViewPane {
	...
	private _tree!: WorkbenchCompressibleAsyncDataTree<
		SCMHistoryViewModel,
		TreeElement,
		FuzzyScore
	>;
	...
	constructor(
		...
		this._scmCurrentHistoryItemRefHasRemote =
			ContextKeys.SCMCurrentHistoryItemRefHasRemote.bindTo(
				this.scopedContextKeyService,
			);
		this._scmCurrentHistoryItemRefHasBase =
			ContextKeys.SCMCurrentHistoryItemRefHasBase.bindTo(
				this.scopedContextKeyService,
			);
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this._treeContainer = append(
			container,
			$(".scm-view.scm-history-view.show-file-icons"),
		);
		...
		this.onDidChangeBodyVisibility(
			async (visible) => {
				...
				this._visibilityDisposables.add(
					runOnChange(
						this._scmViewService.graphShowIncomingChangesConfig,
						async () => {
							await this.refresh();
						},
					),
				...
				this._visibilityDisposables.add(
					runOnChange(
						this._scmViewService.graphShowOutgoingChangesConfig,
						async () => {
							await this.refresh();
						},
					),
				...
				this._visibilityDisposables.add(
					autorun((reader) => {
						...
						reader.store.add(
							runOnChange(historyItemRefId, async (historyItemRefIdValue) => {
								...
								this._scmCurrentHistoryItemRefInFilter.set(
									this._isCurrentHistoryItemInFilter(historyItemRefIdValue),
								);
							}),
						);
								...
								async () => {
									...
									this._scmCurrentHistoryItemRefInFilter.set(
										this._isCurrentHistoryItemInFilter(
											historyItemRefId.read(undefined),
										),
									);
								},
							),
						...
						reader.store.add(
							autorun((reader) => {
								this._scmCurrentHistoryItemRefHasRemote.set(
									!!historyProvider.historyItemRemoteRef.read(reader),
								);
							}),
						);
						...
						reader.store.add(
							autorun((reader) => {
								this._scmCurrentHistoryItemRefHasBase.set(
									!!historyProvider.historyItemBaseRef.read(reader),
								);
							}),
						);
	...
	private _createTree(container: HTMLElement): void {
		...
		const resourceLabels = this.instantiationService.createInstance(
			ResourceLabels,
		...
		this._tree = this.instantiationService.createInstance(
			...
			this._treeDataSource,
			{
				...
				twistieAdditionalCssClass: (e: unknown) => {
					return isSCMHistoryItemViewModelTreeElement(e) ||
						isSCMHistoryItemLoadMoreTreeElement(e)
						? "force-no-twistie"
						: undefined;
				},
			},
	...
	private _isCurrentHistoryItemInFilter(
		historyItemRefId: string | undefined,
	): boolean {
		if (!historyItemRefId) {
			return false;
		}
	...
	private _onContextMenu(e: ITreeContextMenuEvent<TreeElement | null>): void {
		...
		if (isSCMHistoryItemViewModelTreeElement(element)) {
			...
			if (
				historyItemRefMenuItems.length > 0 &&
			...
			const contextKeyService = this.scopedContextKeyService.createOverlay([
				[
					...
					historyItem.references?.find(
						(ref) => ref.id === historyItemRef?.id,
					) !== undefined,
				],
			]);
	...
	private _updateIndentStyles(theme: IFileIconTheme, viewMode: ViewMode): void {
		...
		this._treeContainer.classList.toggle(
			...
			(viewMode === ViewMode.List && theme.hasFileIcons) ||
