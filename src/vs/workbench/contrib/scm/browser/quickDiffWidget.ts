...
export const isQuickDiffVisible = new RawContextKey<boolean>(
	"dirtyDiffVisible",
	false,
);
...
export class QuickDiffPickerViewItem extends SelectActionViewItem<IQuickDiffSelectItem> {
	...
	public setSelection(quickDiffs: QuickDiff[], providerId: string) {
		this.optionsItems = quickDiffs.map((quickDiff) => ({
			providerId: quickDiff.id,
			text: quickDiff.label,
		}));
...
class QuickDiffWidget extends PeekViewWidget {
	...
	showChange(index: number, usePosition: boolean = true): void {
		...
		this.contextKeyService.createKey(
			"originalResource",
			this.model.changes[index].original.toString(),
		);
		this.contextKeyService.createKey(
			"originalResourceScheme",
		...
		const height = Math.min(
			getChangeHeight(change) +
				2 /* arrow, frame, header */ +
				6 /* 3 lines above/below the change */,
	...
	private renderTitle(): void {
		...
		if (!this.shouldUseDropdown()) {
			const label =
				this.model.quickDiffs.find(
					(quickDiff) => quickDiff.id === this._providerId,
				)?.label ?? "";
	...
	private getQuickDiffsContainingChange(): QuickDiff[] {
		...
		const quickDiffsWithChange = this.model.changes
			.filter((c) =>
				change.change2.modified.intersectsOrTouches(c.change2.modified),
			)
			.map((c) => c.providerId);

		return this.model.quickDiffs.filter(
			(quickDiff) =>
				quickDiffsWithChange.includes(quickDiff.id) &&
	...
	protected override _fillHead(container: HTMLElement): void {
		...
		const action = new QuickDiffPickerBaseAction(
			(event?: IQuickDiffSelectItem) => this.switchQuickDiff(event),
		);
...
export class QuickDiffEditorController
	extends Disposable
	implements IEditorContribution
{
	...
	static get(editor: ICodeEditor): QuickDiffEditorController | null {
		return editor.getContribution<QuickDiffEditorController>(
	...
	next(lineNumber?: number): void {
		...
		if (
			...
			(typeof lineNumber === "number" || !this.widget.providerId)
		) {
			...
			const providerChanges: number[] =
				this.model.quickDiffChanges.get(this.widget.providerId) ??
	...
	private onEditorMouseUp(e: IEditorMouseEvent): void {
		...
		try {
			const index = modelRef.object.changes.findIndex((change) =>
