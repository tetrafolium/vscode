...
export const SCMInputContextKeys = {
	SCMInputHasValidationMessage: new RawContextKey<boolean>(
...
class SCMInputWidgetActionRunner extends ActionRunner {
	...
	protected override async runAction(action: IAction): Promise<void> {
		try {
			...
			await action.run(
				...[this.input.repository.provider.rootUri, context, this._cts.token],
			);
		} finally {
			...
			if (this._runningActions.size === 0) {
				const actionId =
					action.id === SCMInputWidgetCommandId.SetupAction
...
class SCMInputWidgetToolbar extends WorkbenchToolBar {
	...
	public setInput(input: ISCMInput): void {
		...
		const updateToolbar = () => {
			...
			if (
				(this.actionRunner as SCMInputWidgetActionRunner).runningActions
					.size !== 0
			) {
		...
		this._disposables.value.add(
			input.repository.provider.onDidChangeResources(() => updateToolbar()),
		);
		this._disposables.value.add(
...
class SCMInputWidgetEditorOptions {
	...
	getEditorOptions(): IEditorOptions {
		...
		const wordSegmenterLocales = this.configurationService.getValue<
			string | string[]
		...
		const cursorStyle =
			this.configurationService.getValue<IEditorOptions["cursorStyle"]>(
				"editor.cursorStyle",
			);
		const cursorWidth =
			this.configurationService.getValue<IEditorOptions["cursorWidth"]>(
				"editor.cursorWidth",
			) ?? 1;
		...
		const roundedSelection =
			this.configurationService.getValue<boolean>("editor.roundedSelection") ===
			true;
...
export class SCMInputWidget {
	...
	set input(input: ISCMInput | undefined) {
		...
		this.repositoryDisposables.add(
			input.onDidClearValidation(() => this.clearValidation()),
		);
		...
		this.repositoryDisposables.add(
			this.keybindingService.onDidUpdateKeybindings(updatePlaceholderText),
		);
	...
	constructor(
		...
		this.contextKeyService = this.disposables.add(
		...
		this.validationMessageContextKey =
			SCMInputContextKeys.SCMInputHasValidationMessage.bindTo(
		...
		this.disposables.add(
			this.inputEditorOptions.onDidChange(this.onDidChangeEditorOptions, this),
		);
		...
		this.validationContextView = this.contextViewService.showContextView({
			...
			render: (container) => {
				...
				if (typeof message === "string") {
					...
					disposables.add(
						tracker.onDidFocus(() => (this.validationHasFocus = true)),
					);
					disposables.add(
						tracker.onDidBlur(() => {
	...
	private getToolbarWidth(): number {
		const showInputActionButton = this.configurationService.getValue<boolean>(
			"scm.showInputActionButton",
		);
		if (
			!this.toolbar ||
