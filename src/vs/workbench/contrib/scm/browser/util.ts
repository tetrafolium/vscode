...
export function isSCMResourceNode(
	...
	return (
		ResourceTree.isResourceNode(element) && isSCMResourceGroup(element.context)
	);
}
...
export function isSCMHistoryItemChangeViewModelTreeElement(
	...
	return (
		(element as SCMHistoryItemChangeViewModelTreeElement).type ===
		"historyItemChangeViewModel"
	);
}
...
export function isSCMArtifactGroupTreeElement(
	element: unknown,
): element is SCMArtifactGroupTreeElement {
...
> {
	return (
		ResourceTree.isResourceNode(element) &&
		isSCMArtifactGroupTreeElement(element.context)
	);
}

export function isSCMArtifactTreeElement(
	element: unknown,
): element is SCMArtifactTreeElement {
...
export function collectContextMenuActions(
	menu: IMenu,
	arg?: unknown,
): IAction[] {
...
class StatusBarActionViewItem extends ActionViewItem {
	...
	protected override updateLabel(): void {
		if (this.options.label && this.label) {
			...
			const elements = renderLabelWithIcons(
				this._commandTitle ?? this.action.label,
			).map((element) => {
...
export function getStatusBarCommandGenericName(
	command: Command,
): string | undefined {
