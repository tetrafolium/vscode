...
class ListDelegate implements IListVirtualDelegate<ISCMRepository> {
	...
	getTemplateId(element: TreeElement): string {
		if (isSCMRepository(element)) {
			...
			isSCMArtifactTreeElement(element) ||
			isSCMArtifactNode(element)
		) {
...
> {
	...
	renderElement(
		...
		const repositoryMenus =
			this._scmViewService.menus.getRepositoryMenus(provider);
	...
	renderCompressedElements(
		node: ITreeNode<
			ICompressedTreeNode<SCMArtifactGroupTreeElement>,
			FuzzyScore
		>,
		index: number,
		templateData: ArtifactGroupTemplate,
...
> {
	...
	renderElement(
		...
		if (isSCMArtifactTreeElement(artifactOrFolder)) {
			...
			templateData.timestamp.textContent = artifact.timestamp
				? fromNow(artifact.timestamp)
				: "";
			templateData.timestampContainer.classList.toggle(
				"duplicate",
				artifactOrFolder.hideTimestamp,
			);
	...
	renderCompressedElements(
		...
		const artifactOrFolder =
			compressed.elements[compressed.elements.length - 1];
	...
	implements IAsyncDataSource<ISCMViewService, TreeElement>
{
	...
	async getChildren(
		...
		if (this.scmViewService.explorerEnabledConfig.get() === false) {
			...
			const repositories = this.scmViewService.repositories.filter(
				(r) => r.provider.parentId === parentId,
			);
		...
		if (inputOrElement instanceof SCMViewService) {
			...
			const repositories = this.scmViewService.repositories.filter(
				(r) => r.provider.parentId === undefined,
			);
			...
			const artifactGroups =
				(await inputOrElement.provider.artifactProvider
					.get()
			...
			const artifacts =
				(await repository.provider.artifactProvider
					.get()
					?.provideArtifacts(inputOrElement.artifactGroup.id)) ?? [];

			if (inputOrElement.artifactGroup.supportsFolders) {
				// Resource tree for artifacts
				const artifactsTree = new ResourceTree<
					SCMArtifactTreeElement,
					SCMArtifactGroupTreeElement
				>(inputOrElement);
				for (let index = 0; index < artifacts.length; index++) {
					const artifact = artifacts[index];
					const artifactUri = URI.from({
						scheme: "scm-artifact",
						path: artifact.name,
					});
					const artifactDirectory =
						artifact.id.lastIndexOf("/") > 0
				...
				(artifact, index, artifacts) =>
					({
						...
						hideTimestamp:
							...
							fromNow(artifacts[index - 1].timestamp!) ===
								fromNow(artifact.timestamp),
						type: "artifact",
...
export class SCMRepositoriesViewPane extends ViewPane {
	...
	private readonly providerCountBadgeObs: IObservable<
		"hidden" | "auto" | "visible"
	>;
	...
	constructor(
		...
		this.visibleCountObs = observableConfigValue(
			"scm.repositories.visible",
			10,
			this.configurationService,
		);
		this.providerCountBadgeObs = observableConfigValue<
			"hidden" | "auto" | "visible"
		>("scm.providerCountBadge", "hidden", this.configurationService);
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this._register(
			autorun((reader) => {
				...
				treeContainer.classList.toggle(
		...
		this.onDidChangeBodyVisibility(
			async (visible) => {
				...
				this.treeOperationSequencer.queue(async () => {
					...
					const onDidChangeVisibleRepositoriesSignal =
						observableSignalFromEvent(
							this,
					...
					this.visibilityDisposables.add(
						autorun(async (reader) => {
							const explorerEnabledConfig =
								this.scmViewService.explorerEnabledConfig.read(reader);
							const didFinishLoadingRepositories =
								this.scmViewService.didFinishLoadingRepositories.read(reader);
	...
	private createTree(
		...
		this.treeDataSource = this.instantiationService.createInstance(
			...
			this.treeDataSource,
			{
				...
				collapseByDefault: (e: unknown) => {
					...
					if (
						viewState?.expanded &&
						(isSCMRepository(e) ||
							isSCMArtifactGroupTreeElement(e) ||
							isSCMArtifactTreeElement(e))
					) {
						// Only expand repositories/artifact groups/artifacts that were expanded before
						return (
							viewState.expanded.indexOf(this.treeIdentityProvider.getId(e)) ===
							-1
						);
						...
						return !(
							e.childrenCount === 1 &&
							Iterable.first(e.children)?.element === undefined
						);
					} else {
						return true;
					}
				...
				multipleSelectionSupport:
					this.scmViewService.selectionModeConfig.get() === "multiple",
		...
		this._register(
			autorun((reader) => {
				const selectionMode =
					this.scmViewService.selectionModeConfig.read(reader);
				this.tree.updateOptions({
					multipleSelectionSupport: selectionMode === "multiple",
				});
			}),
		);
		...
		this._register(
			this.tree.onDidChangeSelection(this.onTreeSelectionChange, this),
		);
		...
		this._register(
			this.tree.onDidChangeContentHeight(this.onTreeContentHeightChange, this),
		);
	}
		...
		disposables.add(
			autorun(async (reader) => {
				...
				const artifactsProvider =
					repository.provider.artifactProvider.read(reader);
	...
	private async onDidRemoveRepository(
	...
	private onTreeDidOpen(e: IOpenEvent<TreeElement | undefined>): void {
		if (
			!e.element ||
			!isSCMArtifactTreeElement(e.element) ||
			!e.element.artifact.command
		) {
			return;
		}
	...
	private onTreeSelectionChange(e: ITreeEvent<TreeElement>): void {
		if (e.browserEvent && e.elements.length > 0) {
			...
			if (e.elements.every((e) => isSCMRepository(e))) {
				...
				e.elements.every(
					(e) =>
						isSCMArtifactGroupTreeElement(e) || isSCMArtifactTreeElement(e),
				)
			) {
				this.scmViewService.visibleRepositories = e.elements.map(
					(e) => e.repository,
				);
			}
	...
	private onDidTreeFocus(): void {
		...
		if (focused.length > 0) {
			if (isSCMRepository(focused[0])) {
				...
				isSCMArtifactGroupTreeElement(focused[0]) ||
				isSCMArtifactTreeElement(focused[0])
			) {
	...
	private async updateParentRepository(
		repository: ISCMRepository,
	): Promise<void> {
	...
	private async updateTreeSelection(): Promise<void> {
		...
		if (
			...
			!this.tree.getFocus().includes(visibleSelection[0])
		) {
