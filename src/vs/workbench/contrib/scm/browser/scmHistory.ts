...
export function renderSCMHistoryItemGraph(
	...
	if (historyItemViewModel.kind === "HEAD") {
		...
		const dashedCircle = drawDashedCircle(
			circleIndex,
			CIRCLE_RADIUS + 1,
			CIRCLE_STROKE_WIDTH - 1,
			circleColor,
...
export function toISCMHistoryItemViewModelArray(
	...
	for (let index = 0; index < historyItems.length; index++) {
		...
		const kind =
			historyItem.id === currentHistoryItemRef?.revision ? "HEAD" : "node";
	...
	if (
		...
		if (
			...
			const afterHistoryItemIndex = viewModels.findIndex(
				(vm) => vm.historyItem.id === mergeBase,
			);
			...
			if (beforeHistoryItemIndex !== -1 && afterHistoryItemIndex !== -1) {
				...
				const incomingChangeMerged =
					viewModels[beforeHistoryItemIndex].historyItem.parentIds.length ===
						2 &&
					viewModels[beforeHistoryItemIndex].historyItem.parentIds.includes(
						mergeBase,
					);
				...
				if (!incomingChangeMerged) {
					...
					viewModels[beforeHistoryItemIndex] = {
						...viewModels[beforeHistoryItemIndex],
						inputSwimlanes: viewModels[
							beforeHistoryItemIndex
						].inputSwimlanes.map((node) => {
							return node.id === mergeBase &&
								node.color === historyItemRemoteRefColor
								? { ...node, id: SCMIncomingHistoryItemId }
								: node;
						}),
						outputSwimlanes: viewModels[
							beforeHistoryItemIndex
						].outputSwimlanes.map((node) => {
							return node.id === mergeBase &&
								node.color === historyItemRemoteRefColor
								? { ...node, id: SCMIncomingHistoryItemId }
								: node;
					...
					const inputSwimlanes = viewModels[
						beforeHistoryItemIndex
					].outputSwimlanes.map((i) => deepClone(i));
					const outputSwimlanes = viewModels[
						afterHistoryItemIndex
					].inputSwimlanes.map((i) => deepClone(i));
					const displayIdLength =
						viewModels[0].historyItem.displayId?.length ?? 0;
		...
		if (
			addOutgoingChanges &&
			currentHistoryItemRef?.revision &&
			currentHistoryItemRef.revision !== mergeBase
		) {
			...
			const currentHistoryItemRefIndex = viewModels.findIndex(
				(vm) =>
					vm.kind === "HEAD" &&
					vm.historyItem.id === currentHistoryItemRef.revision,
			);
			...
			if (currentHistoryItemRefIndex !== -1) {
				...
				const inputSwimlanes =
					viewModels[currentHistoryItemRefIndex].inputSwimlanes.slice(0);
...
export function toHistoryItemHoverContent(
	...
	if (includeReferences && historyItem.references?.length) {
		...
		for (const reference of historyItem.references) {
			const labelIconId = ThemeIcon.isThemeIcon(reference.icon)
				? reference.icon.id
				: "";
