...
export const quickDiffDecorationCount = new RawContextKey<number>(
...
class QuickDiffDecorator extends Disposable {
	...
	constructor(
		...
		this._register(
			Event.runAndSubscribe(this.quickDiffModelRef.object.onDidChange, () =>
				this.onDidChange(),
			),
		);
	}
	...
	private onDidChange(): void {
		...
		const primaryQuickDiffChanges =
			this.quickDiffModelRef.object.changes.filter(
				(change) => change.providerId === primaryQuickDiff?.id,
			);

		const decorations: IModelDeltaDecoration[] = [];
		for (const change of this.quickDiffModelRef.object.changes) {
			const quickDiff = this.quickDiffModelRef.object.quickDiffs.find(
				(quickDiff) => quickDiff.id === change.providerId,
			);
			...
			if (
				...
				primaryQuickDiffChanges.some((c) =>
					c.change2.modified.intersectsOrTouches(change.change2.modified),
				)
			) {
			...
			const endLineNumber =
				change.change.modifiedEndLineNumber || startLineNumber;
...
export class QuickDiffWorkbenchController
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private readonly quickDiffProviders: IObservable<
		readonly QuickDiffProvider[]
	>;

	// Resource URI -> Code Editor Id -> Decoration (Disposable)
	private readonly decorators = new ResourceMap<DisposableMap<string>>();
	private viewState: QuickDiffWorkbenchControllerViewState = {
		width: 3,
		visibility: "always",
	};
	...
	constructor(
		...
		private readonly quickDiffModelService: IQuickDiffModelService,
		@IQuickDiffService private readonly quickDiffService: IQuickDiffService,
		@IUriIdentityService
		...
		this.quickDiffDecorationCount =
			quickDiffDecorationCount.bindTo(contextKeyService);
	...
	private onDidChangeConfiguration(): void {
		const enabled =
			this.configurationService.getValue<string>("scm.diffDecorations") !==
			"none";
		...
		this.transientDisposables.add(
			autorun((reader) => {
				...
				const quickDiffModelRef =
					this.quickDiffModelService.createQuickDiffModelReference(
				...
				const visibleDecorationCount = observableFromEvent(
					...
					() => {
						const visibleQuickDiffs =
							quickDiffModelRef.object.quickDiffs.filter((quickDiff) =>
								this.quickDiffService.isQuickDiffProviderVisible(quickDiff.id),
							);
						return quickDiffModelRef.object.changes.filter((change) =>
							visibleQuickDiffs.some(
								(quickDiff) => quickDiff.id === change.providerId,
							),
						).length;
					},
		...
		this.transientDisposables.add(
			autorun((reader) => {
				...
				for (let index = 0; index < providers.length; index++) {
					...
					const visible = this.quickDiffService.isQuickDiffProviderVisible(
						provider.id,
					);
					const group =
						provider.kind !== "contributed" ? "0_scm" : "1_contributed";
					const order = index + 1;
						...
						registerAction2(
							class extends Action2 {
								...
								override run(accessor: ServicesAccessor): void {
									...
									quickDiffService.toggleQuickDiffProviderVisibility(
										provider.id,
									);
								}
							},
	...
	private onEditorsChanged(): void {
		for (const editor of this.editorService.visibleTextEditorControls) {
			...
			const quickDiffModelRef =
				this.quickDiffModelService.createQuickDiffModelReference(textModel.uri);
			if (!quickDiffModelRef) {
				continue;
			}
