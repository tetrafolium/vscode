...
class SCMAccessibilityHelpContentProvider
	extends Disposable
	...
	provideContent(): string {
		...
		if (this._scmViewService.visibleRepositories.length > 1) {
			const repositoryList = this._scmViewService.visibleRepositories
				...
				.join(", ");
			content.push(
				localize("state-msg1", "Visible repositories: {0}", repositoryList),
			);
		}
		...
		if (focusedRepository) {
			...
			// Commit Message
			if (
				focusedRepository.input.visible &&
			...
			if (actionButton) {
				const label =
					actionButton.command.tooltip ?? actionButton.command.title;
		...
		content.push(
			localize("scm-msg3", "Once the Source Control view is opened you can:"),
		);
		content.push(
			localize(
				"scm-msg4",
