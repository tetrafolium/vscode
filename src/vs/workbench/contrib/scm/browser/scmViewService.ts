...
export class SCMViewService implements ISCMViewService {
	...
	get visibleRepositories(): ISCMRepository[] {
		...
		if (this._repositoriesSortKey === ISCMRepositorySortKey.DiscoveryTime) {
			return this._repositories
				.filter(
					(r) =>
						r.repository.provider.isHidden !== true && r.selectionIndex !== -1,
				)
	...
	private readonly _activeEditorRepositoryObs: IObservable<
	 ...
	 */
	private readonly _activeRepositoryObs: IObservable<
		ISCMRepository | undefined
	>;
	private readonly _activeRepositoryPinnedObs: ISettableObservable<
		ISCMRepository | undefined
	>;
	private readonly _focusedRepositoryObs: IObservable<
		ISCMRepository | undefined
	>;
	...
	constructor(
		...
		this.selectionModeConfig =
			observableConfigValue<ISCMRepositorySelectionMode>(
		...
		this.explorerEnabledConfig = derived((reader) => {
			return (
				...
				this.selectionModeConfig.read(reader) ===
					ISCMRepositorySelectionMode.Single
			);
		});
		...
		this._activeRepositoryPinnedObs = observableValue<
			ISCMRepository | undefined
		>(this, undefined);
		...
		>(
			{
				owner: this,
				equalsFn: (r1, r2) =>
					r1?.repository.id === r2?.repository.id && r1?.pinned === r2?.pinned,
			},
			(reader) => {
				const activeRepository = this._activeRepositoryObs.read(reader);
				const activeRepositoryPinned =
					this._activeRepositoryPinnedObs.read(reader);
		...
		this.disposables.add(
			runOnChange(this.selectionModeConfig, (selectionMode) => {
				if (
					selectionMode === ISCMRepositorySelectionMode.Single &&
					this.visibleRepositories.length > 1
				) {
					const repository = this.visibleRepositories[0];
					this.visibleRepositories = [repository];
				} else if (
					selectionMode === ISCMRepositorySelectionMode.Multiple &&
		...
		this._selectionModelContextKey =
			RepositoryContextKeys.RepositorySelectionMode.bindTo(contextKeyService);
		this.disposables.add(
			autorun((reader) => {
	...
	private onDidRemoveRepository(repository: ISCMRepository): void {
		...
		if (
			removed.length === 1 &&
			removed[0].repository === this._activeRepositoryPinnedObs.get()
		) {
