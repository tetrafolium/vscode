...
async function copyAllCommand(
	accessor: ServicesAccessor,
	match: RenderableMatch | undefined | null,
) {
	const viewsService = accessor.get(IViewsService);
	const clipboardService = accessor.get(IClipboardService);
	const labelService = accessor.get(ILabelService);
...
function matchToString(match: ISearchTreeMatch, indent = 0): string {
	...
	const largestPrefixSize = fullMatchLines.reduce((largest, _, i) => {
		const thisSize =
			i === 0 ? getFirstLinePrefix().length : getOtherLinePrefix(i).length;
...
function isAISearchResult(
	element: RenderableMatch | undefined | null,
): boolean {
	if (!element) {
		return false;
	}
