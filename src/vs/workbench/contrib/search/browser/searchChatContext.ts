...
class SearchViewResultChatContextPick implements IChatContextValueItem {
	...
	isEnabled(widget: IChatWidget): Promise<boolean> | boolean {
		return (
			!!SearchContext.HasSearchResults.getValue(this._contextKeyService) &&
					...
					.filter(
						(a) =>
							...
							this._instantiationService.invokeFunction((accessor) =>
								isSupportedChatFileScheme(accessor, a.resource!.scheme),
							),
					)
					.slice(0, 30)
					.forEach((uri) =>
...
export async function searchFilesAndFolders(
	...
	const segmentMatchPattern = fuzzyMatch
