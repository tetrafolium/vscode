...
function mergeSearchPatternIfNotExists(
...
function getMultiSelectedSearchResources(
	...
	sortConfig: ISearchConfigurationProperties | undefined,
): URI[] {
	return getElementsToOperateOn(viewer, currElement, sortConfig)
		.map((renderableMatch) =>
			isSearchTreeMatch(renderableMatch) ? null : renderableMatch.resource,
		)
		.filter(
			(renderableMatch): renderableMatch is URI => renderableMatch !== null,
		);
}
...
async function modifySearchFileTypePattern(
	...
	const patternWidget = isExclude
		? searchView.searchExcludePattern
		: searchView.searchIncludePattern;
	const currentPatterns = patternWidget.getValue();
	const updatedPatterns = mergeSearchPatternIfNotExists(
		currentPatterns,
		newPattern,
	);
