...
export class SearchView extends ViewPane {
	...
	private readonly currentEditorCursorListener = this._register(
	...
	constructor(
		...
		this.searchResultHeaderFocused =
			Constants.SearchContext.SearchResultHeaderFocused.bindTo(
				this.contextKeyService,
			);
		...
		const updateChangedFilesToggleEnabled = () => {
			const hasChanges = [...this.scmService.repositories].some((repo) =>
				repo.provider.groups.some((group) => group.resources.length > 0),
			);
			this.inputPatternIncludes?.setOnlySearchInChangedFilesEnabled(hasChanges);
		};
		const scmRepositoryListeners = this._register(
			new DisposableMap<ISCMRepository>(),
		);
		const registerScmRepositoryListeners = (repository: ISCMRepository) => {
			scmRepositoryListeners.set(
				repository,
	...
	protected override renderBody(parent: HTMLElement): void {
		...
		this.inputPatternIncludes.setOnlySearchInChangedFilesEnabled(
			[...this.scmService.repositories].some((repo) =>
				repo.provider.groups.some((group) => group.resources.length > 0),
			),
		);
		...
		this._register(
			this.inputPatternIncludes.onChangeSearchInChangedFilesBox(() =>
				this.triggerQueryChange(),
			),
		);
			...
			this.searchWidget.onSearchSubmit((options) => {
				const shouldRenderAIResults =
					this.configurationService.getValue<ISearchConfigurationProperties>(
						"search",
					).searchView.semanticSearchBehavior;
				if (shouldRenderAIResults === SemanticSearchBehavior.Auto) {
					this.logService.info(
						`SearchView: Automatically rendering AI results`,
					);
				}
				this.triggerQueryChange({
					...options,
					...
					shouldUpdateAISearch:
						shouldRenderAIResults === SemanticSearchBehavior.Auto,
				});
			}),
		);
		this._register(
	...
	private shouldCollapseAccordingToConfig(match: RenderableMatch): boolean {
		...
		if (
			collapseResults === ObjectTreeElementCollapseState.PreserveOrCollapsed
		) {
			return true;
		}
		return false;
	...
	private replaceAll(): void {
		...
		this.progressService.withProgress(
			{ location: this.getProgressLocation(), delay: 100, total: occurrences },
			(p) => {
				progressReporter = p;
	...
	private createSearchResultsView(container: HTMLElement): void {
		...
		this._register(
			this.editorService.onDidActiveEditorChange(() => {
				...
				this.currentEditorCursorListener.value =
	...
	toggleQueryDetails(
		...
		show =
			typeof show === "undefined"
				? !this.queryDetails.classList.contains("more")
				: Boolean(show);
	...
	private _onQueryChanged(
		...
		const onlySearchInChangedFiles =
			this.inputPatternIncludes.onlySearchInChangedFiles();
	...
	public async requestAIResults() {
		...
		if (
			(!this.cachedResults || this.cachedResults.results.length === 0) &&
			!this._pendingSemanticSearchPromise
		) {
			this.clearAIResults();
		}
		...
		await forcedExpandRecursively(
			this.getControl(),
			this.model.searchResult.aiTextSearchResult,
		);
	}
		...
		return result.asyncResults.then(
			(complete) => {
				...
				if (
					...
					config === SemanticSearchBehavior.RunOnEmpty
				) {
					this.logService.info(
						`SearchView: Requesting semantic results on empty search.`,
					);
					this.model.searchResult.aiTextSearchResult.hidden = false;
				}
				return this.onSearchComplete(
	...
	private updateSearchResultCount(
		...
		const fileCount = this.viewModel.searchResult.fileCount(
			this.viewModel.searchResult.aiTextSearchResult.hidden,
		);
	...
	private async getKeywordSuggestions() {
		...
		if (!aiSearchPromise) {
			this.viewModel.searchResult.setAIQueryUsingTextQuery();
			aiSearchPromise = this._pendingSemanticSearchPromise =
				this.viewModel.aiSearch((result) => {
	...
	private showSearchWithoutFolderMessage(): void {
		...
		const openFolderButton = this.messageDisposables.add(
			new SearchLinkButton(
				...
				() => {
					this.commandService
						.executeCommand(OpenFolderAction.ID)
...
class SearchViewDataSource implements IAsyncDataSource<
	ISearchResult,
	RenderableMatch
> {
	constructor(
		...
		if (isSearchResult(element)) {
			...
			if (
				...
				(!this.searchView.model.hasAIResults ||
					!!this.searchView._pendingSemanticSearchPromise)
			) {
