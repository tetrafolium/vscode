...
export class SearchAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(
		...
		const searchViewModelService = accessor.get(
...
class SearchAccessibilityHelpProvider
	extends Disposable
	implements IAccessibleViewContentProvider
{
	...
	provideContent(): string {
		...
		// Header
		content.push(
			localize("search.header", "Accessibility Help: Search Across Files"),
		);
		content.push(
			localize(
				"search.context",
		...
		if (resultCount !== undefined) {
			if (resultCount === 0) {
				...
				content.push(
					localize("search.resultCount", "{0} results found.", resultCount),
				);
			}
		} else {
			content.push(
				localize("search.noSearch", "Type a search term to find results."),
			);
		}
		content.push("");
		...
		content.push(
			localize("search.inputHeader", "Inside the Search Input (What It Does):"),
		);
		content.push(
			localize(
				"search.inputDesc",
		...
		content.push(
			localize("search.keyboardHeader", "Keyboard Navigation Summary:"),
		);
		content.push("");
		content.push(
			localize(
		...
		content.push(
			localize("search.keyEnter", "- Enter: Run or refresh the search."),
		);
		content.push(
			localize("search.keyTab", "- Tab: Move focus to the results tree below."),
		);
		content.push("");
		content.push(
			localize("search.keyNavResultsHeader", "Navigating search results:"),
		);
		content.push(
			localize(
				"search.keyArrow",
		...
		// Search Options
		content.push(
			localize("search.optionsHeader", "Search Options in the Dialog:"),
		);
		content.push(
			localize(
				"search.optionCase",
