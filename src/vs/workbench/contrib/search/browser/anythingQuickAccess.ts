...
interface IAnythingQuickPickItem
	extends IPickerQuickAccessItem, IQuickPickItemWithResource {}
...
export class AnythingQuickAccessProvider extends PickerQuickAccessProvider<IAnythingQuickPickItem> {
	...
	private async getFileSearchResults(
		...
		if (fileSearchResults.limitHit && query.values && query.values.length > 1) {
			const additionalFileSearchResults = await this.doGetFileSearchResults(
				query.original,
				token,
			);
			if (token.isCancellationRequested) {
			...
			for (const additionalFileSearchResult of additionalFileSearchResults.results) {
				if (
					!existingFileSearchResultsMap.has(additionalFileSearchResult.resource)
				) {
	...
	private async matchFilenameCasing(resource: URI): Promise<URI> {
		...
		if (stat?.children) {
			const match = stat.children.find((child) =>
				this.uriIdentityService.extUri.isEqual(child.resource, resource),
			);
			if (match) {
				...
				if (keyMods.shift) {
					...
					if (widget && resource) {
						widget.attachmentModel.addContext(
							widget.attachmentModel.asFileVariableEntry(resource),
						);
					}
					return;
				}
