...
suite("SearchModel", () => {
	...
	function searchServiceWithResults(
		results: IFileMatch[],
		complete: ISearchComplete | null = null,
	): ISearchService {
	...
	function canceleableSearchService(
		...
		return <ISearchService>{
			textSearch(
				...
				return this.textSearchSplitSyncAsync(query, token, onProgress)
					.asyncResults;
			},
			fileSearch(
				query: IFileQuery,
	...
	function searchServiceWithDeferredPromise(
		p: Promise<ISearchComplete>,
	): ISearchService {
		return <ISearchService>{
			textSearchSplitSyncAsync(
				query: ITextQuery,
	...
	function notebookSearchServiceWithInfo(
		results: INotebookFileMatchWithModel[],
		...
		return <INotebookSearchService>{
			...
			notebookSearch(
				...
				const localResults =
					new ResourceMap<INotebookFileMatchWithModel | null>(
						(uri) => uri.path,
					);
	...
	test("Search Model: Search adds to results", async () => {
		...
		instantiationService.stub(
			INotebookSearchService,
			notebookSearchServiceWithInfo([], undefined),
		);

		const testObject: SearchModelImpl =
			instantiationService.createInstance(SearchModelImpl);
	...
	test("Search Model: Search can return notebook results", async () => {
		...
		assert(
			(notebookFileMatches[0] as MatchInNotebook).cell?.id === "mdInputCell",
		);
		...
		const mdCellMatchProcessed = (notebookFileMatches[0] as MatchInNotebook)
			.cellParent;
		const codeCellMatchProcessed = (notebookFileMatches[1] as MatchInNotebook)
			.cellParent;
	...
	function aRawMatchWithCells(
		resource: string,
		...cells: INotebookCellMatchWithModel[]
	) {
		return {
	...
	function stubNotebookEditorService(
		instantiationService: TestInstantiationService,
	): INotebookEditorService {
		instantiationService.stub(
