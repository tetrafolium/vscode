...
suite("SearchResult", () => {
	...
	setup(() => {
		...
		instantiationService.stub(IReplaceService, "replace", () =>
	...
	test("Line Match", function () {
		...
		assert.strictEqual(
			lineMatch.id(),
			MATCH_PREFIX + "file:///folder/file.txt>[2,1 -> 2,6]foo",
		);
	...
	test("Test that notebook matches get added correctly", function () {
		...
		assert.deepStrictEqual(
			...
			(addFileMatch.getCall(0).args[0][0] as INotebookFileMatchWithModel)
				.cellResults[0].contentResults,
		);
		assert.deepStrictEqual(
			fileMatch1.cellResults[0].webviewResults,
			(addFileMatch.getCall(0).args[0][0] as INotebookFileMatchWithModel)
				.cellResults[0].webviewResults,
		);
		assert.deepStrictEqual(
			fileMatch2.cellResults[0].contentResults,
			(addFileMatch.getCall(0).args[0][1] as INotebookFileMatchWithModel)
				.cellResults[0].contentResults,
		);
		assert.deepStrictEqual(
	...
	test("batchRemove should trigger the onChange event correctly", function () {
		...
		const fileMatch = testObject
			.folderMatches()[1]
			.allDownstreamFileMatches()[0];
		const match = testObject
			.folderMatches()[1]
			.allDownstreamFileMatches()[1]
			.matches()[0];
	...
	test("batchRemove should remove FolderMatchNoRoot (Other files) correctly", function () {
		...
		const otherFilesMatch = folderMatches.find((fm) =>
			isSearchTreeFolderMatchNoRoot(fm),
		);
	...
	test("Creating a model with nested folders should create the correct structure", function () {
		...
		assert.deepStrictEqual(
			(getFolderMatchAtIndex(root0, 0) as FolderMatchImpl).closestRoot,
			root0,
		);
	...
	function aRawFileMatchWithCells(
		resource: string,
		...cellMatches: INotebookCellMatchWithModel[]
	): INotebookFileMatchWithModel {
		return {
	...
	function getFolderMatchAtIndex(
