...
function getOrderedRemotes(
	...
	const remoteIndex = repositoryState.remotes.findIndex(
		(r) => r.name === repositoryState.HEAD?.upstream?.remote,
	);
	if (remoteIndex !== -1) {
		const fetchUrl = repositoryState.remotes[remoteIndex].fetchUrl;
		if (fetchUrl) {
			remotes.set(
				repositoryState.remotes[remoteIndex].name,
				repositoryState.remotes[remoteIndex],
			);
		}
	}

	// If there's a remote named "origin", use that
	const originIndex = repositoryState.remotes.findIndex(
		(r) => r.name === "origin",
	);
	if (originIndex !== -1) {
		...
		if (fetchUrl) {
			remotes.set(
				repositoryState.remotes[originIndex].name,
				repositoryState.remotes[originIndex],
			);
		}
	}
