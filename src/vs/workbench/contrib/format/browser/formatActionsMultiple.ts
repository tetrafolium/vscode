	...
	implements IWorkbenchContribution
{
	...
	private async _updateConfigValues(): Promise<void> {
		...
		const documentFormatters =
			this._languageFeaturesService.documentFormattingEditProvider.allNoModel();
		const rangeFormatters =
			this._languageFeaturesService.documentRangeFormattingEditProvider.allNoModel();
		const formatterExtensionIds = new Set<string>();

		for (const formatter of documentFormatters) {
			if (formatter.extensionId) {
				formatterExtensionIds.add(
					ExtensionIdentifier.toKey(formatter.extensionId),
				);
			}
		}
		...
		extensions = extensions.sort((a, b) => {
			...
			const contributesFormatterA = formatterExtensionIds.has(
				ExtensionIdentifier.toKey(a.identifier),
			);
			const contributesFormatterB = formatterExtensionIds.has(
				ExtensionIdentifier.toKey(b.identifier),
			);
	...
	private async _analyzeFormatter<T extends FormattingEditProvider>(
		...
		if (defaultFormatterId) {
			...
			if (
				extension &&
				this._extensionEnablementService.isEnabled(toExtension(extension))
			) {
