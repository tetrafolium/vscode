...
suite("MarkersFilterOptions", () => {
	...
	setup(() => {
		...
		uriIdentityService =
			instantiationService.createInstance(UriIdentityService);
	});
	...
	test("positive filter - exact match only", () => {
		...
		assert.strictEqual(
			filterOptions.matchesSourceFilters("eslint-plugin"),
			false,
		);
