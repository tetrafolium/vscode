...
suite("MarkersModel Test", () => {
	...
	test("duplicate markers from different owners are deduplicated", function () {
		...
		assert.strictEqual(
			actuals.length,
			1,
			"identical markers from different owners should be deduplicated",
		);
	...
	test("markers with different messages are not deduplicated", function () {
		...
		assert.strictEqual(
			actuals.length,
			2,
			"markers with different messages should not be deduplicated",
		);
	});
