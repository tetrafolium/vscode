...
class MarkerChatContextPick implements IChatContextPickerItem {
	...
	asPicker(): IChatContextPicker {
		return {
			...
			picks: picksWithPromiseFn(
				async (query: string, token: CancellationToken) => {
	...
	getPicksForQuery(
		...
		for (const group of sortedGroups) {
			...
			const isActiveFile =
				activeEditorUri && extUri.isEqual(resource, activeEditorUri);
			const fileLabel = this._labelService.getUriLabel(resource, {
				relative: true,
			});
			const separatorLabel = isActiveFile
...
export class MarkerChatContextContribution
	extends Disposable
