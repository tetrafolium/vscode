...
export class FilterOptions {
	...
	constructor(
		...
		while ((sourceMatch = SOURCE_FILTER_REGEX.exec(filter)) !== null) {
			...
			filter = (
				filter.substring(0, sourceMatch.index) +
				filter.substring(sourceMatch.index + sourceMatch[0].length)
			).trim();
		}
	...
	matchesSourceFilters(markerSource: string | undefined): boolean {
		if (
			this.includeSourceFilters.length === 0 &&
			this.excludeSourceFilters.length === 0
		) {
			return true;
		}
