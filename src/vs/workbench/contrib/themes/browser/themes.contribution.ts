			...
			return new Promise<void>((s, _) => {
				...
				disposables.add(
					quickpick.onDidTriggerButton((button) =>
						this.options.onButton?.(button, quickpick),
					),
				);
				quickpick.matchOnDescription = true;
				disposables.add(
	...
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor) {
			...
			const themes = allThemes.filter((t) =>
				newThemeSettingsIds.has(t.settingsId),
			);
			...
			const picker = disposables.add(
				quickInputService.createQuickPick<IQuickPickItem>(),
			);
			picker.items = items;
			...
			const preferredId =
				previousTheme.type === ColorScheme.LIGHT ||
				previousTheme.type === ColorScheme.HIGH_CONTRAST_LIGHT
					? ThemeSettingDefaults.COLOR_THEME_LIGHT
					: ThemeSettingDefaults.COLOR_THEME_DARK;
			const activeItem = items.find(
				(i) => themes.find((t) => t.id === i.id)?.settingsId === preferredId,
			);
			if (activeItem) {
			...
			disposables.add(
				picker.onDidAccept(() => {
					...
					const theme = selected
						? themes.find((t) => t.id === selected.id)
						: undefined;
