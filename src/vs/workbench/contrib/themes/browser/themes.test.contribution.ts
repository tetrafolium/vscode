...
class Snapper {
	...
	private _moveInjectionCursorToRange(
		cursor: Parser.TreeCursor,
		injectionRange: { startIndex: number; endIndex: number },
	): void {
		...
		while (
			(cursor.startIndex < injectionRange.startIndex ||
	...
	private async _treeSitterTokenize(
		treeSitterTree: TreeSitterTree,
		tokenizationModel: TreeSitterTokenizationImpl,
		languageId: string,
	): Promise<IToken[]> {
		...
		do {
			...
			const isOutsideRange: boolean =
				currentCursor.currentNode.endIndex > current.endOffset;
			...
			if (!isOutsideRange && currentCursor.currentNode.childCount === 0) {
				...
				if (
					...
					while (
						currentCursor.endIndex <= treeSitterRange.endIndex &&
			...
			if (
				cursors.length > 1 &&
				((!cursorResult &&
					currentCursor === cursors[cursors.length - 1].cursor) ||
					isOutsideRange)
			) {
	...
	public async captureTreeSitterSyntaxTokens(
		...
		const languageId =
			this.languageService.guessLanguageIdByFilepathOrFirstLine(resource);
		if (!languageId) {
			return [];
		}

		const model =
			this.modelService.getModel(resource) ??
			this.modelService.createModel(
				content,
				{ languageId, onDidChange: Event.None },
				resource,
			);
		const tokenizationPart = (
		...
		const result = (
			await this._treeSitterTokenize(
				treeSitterTree,
				tokenizationImpl,
				languageId,
			)
