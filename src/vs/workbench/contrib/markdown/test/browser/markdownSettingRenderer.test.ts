...
suite("Markdown Setting Renderer Test", () => {
	...
	test("render code setting button with value", () => {
		...
		const htmlNoValue =
			'<a href="code-oss://settings/example.booleanSetting" codesetting="true">';
	...
	test("actions with value + updating and restoring", async () => {
		...
		const uri = URI.parse(
			settingRenderer.settingToUriString("example.stringSetting", "three"),
		);

		const verifyOriginalState = (
