...
type ResultType =
	| string
	| { type: "tooldata"; value: IToolResultDataPart }
	| { type: "extracted"; value: WebContentExtractResult }
	| undefined;

export class FetchWebPageTool implements IToolImpl {
	constructor(
		@IWebContentExtractorService
		private readonly _readerModeService: IWebContentExtractorService,
	...
	async invoke(
		...
		const { webUris, fileUris, invalidUris, blockedUris } =
			this._parseUris(urls);
		...
		if (webUris.size > 0) {
			...
			webContents = await this._readerModeService.extract(
				[...webUris.values()],
				{ trustedDomains },
			);
		}
			...
			try {
				...
				if (imageMimeType) {
					...
					const detected = detectEncodingFromBuffer({
						buffer: fileContent.value,
						bytesRead: fileContent.value.byteLength,
					});
		...
		for (const url of urls) {
			if (blockedUris.has(url)) {
				results.push(
					this._agentNetworkFilterService.formatError(URI.parse(url)),
				);
		...
		if (
			webContents.every((e) => e.status === "error" || e.status === "redirect")
		) {
	...
	async prepareToolInvocation(
		...
		const { webUris, fileUris, invalidUris, blockedUris } = this._parseUris(
			context.parameters.urls,
		);
		...
		const invalid = [
			...Array.from(invalidUris),
			...additionalInvalidUrls,
			...Array.from(blockedUris),
		];
		// All valid URIs (web + file) for display in messages
		const allFetchedUris = new ResourceSet([
			...webUris.values(),
			...validFileUris,
		]);
		...
		const fileUrisOutsideWorkspace = validFileUris.filter(
			(uri) => !workingDir.getFolder(uri),
		);
		const urlsNeedingConfirmation = new ResourceSet([
			...webUris.values(),
			...fileUrisOutsideWorkspace,
		]);
		...
		if (context.chatSessionResource) {
			...
			const userMessages = model
				?.getRequests()
				.map((r) => r.message.text.toLowerCase());
		...
		const allDomainsTrusted = Iterable.every(urlsNeedingConfirmation, (u) =>
			this._trustedDomainService.isValid(u),
		);
	...
	private _parseUris(urls?: string[]): {
		webUris: Map<string, URI>;
	...
	private _getPromptPartsForResults(
		urls: string[],
		results: ResultType[],
	): (IToolResultTextPart | IToolResultDataPart)[] {
		return results.map((value, i) => {
			const title =
				results.length > 1
					? localize("fetchWebPage.fetchedFrom", "Fetched from {0}", urls[i])
					: undefined;
			if (!value) {
				return {
					kind: "text",
				...
				switch (value.value.status) {
					...
					case "redirect":
						return {
							kind: "text",
							title,
							value: `The webpage has redirected to "${value.value.toURI.toString(true)}". Use the ${InternalFetchWebPageToolId} again to get its contents.`,
						};
					case "error":
						return {
							kind: "text",
							title,
							value: `An error occurred retrieving the fetch result: ${value.value.error}`,
						};
					default:
