...
export function registerExportAgentTracesDbAction() {
	registerAction2(
		class ExportAgentTracesDbAction extends Action2 {
			constructor() {
				super({
					...
					precondition: ContextKeyExpr.equals(
						`config.${AgentHostOTelDbSpanExporterEnabledSettingId}`,
						true,
					),
					f1: true,
				});
			...
			async run(
				...
				const sourceUri = joinPath(
					URI.file(environmentService.userDataPath),
					...AgentHostOTelSpansDbSubPath.split("/"),
				);
				...
				try {
					...
					for (const suffix of ["-wal", "-shm"]) {
						const sidecarSrc = sourceUri.with({
							path: sourceUri.path + suffix,
						});
