...
} from "../../../../browser/parts/titlebar/titlebarActions.js";
...
export class OpenWorkspaceInAgentsWindowAction extends Action2 {
				...
				},
				{
					...
					when: ContextKeyExpr.and(
						...
						ContextKeyExpr.notEquals(
							`config.${ChatConfiguration.TitleBarOpenInAgentsWindowEnabled}`,
							false,
						),
					),
				},
	...
	async run(accessor: ServicesAccessor) {
		...
		await nativeHostService.openAgentsWindow({
			folderUri: folderUri?.scheme === Schemas.file ? folderUri : undefined,
		});
	}
}
			...
			keybinding: [
				{
					...
					when: ContextKeyExpr.and(
						...
						CONTEXT_ACCESSIBILITY_MODE_ENABLED.toNegated(),
					),
				},
				{
...
export class OpenChatSessionInAgentsWindowAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...rest: unknown[]): Promise<void> {
		...
		let preferredSessionType:
			| { providerId?: string; sessionTypeId: string }
			| undefined;
...
class OpenWorkspaceInAgentsTitleBarWidget extends BaseActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		const icon = append(
			container,
			$("span.open-in-agents-titlebar-widget-icon"),
		);
		icon.setAttribute("aria-hidden", "true");

		const labelEl = append(
			container,
			$("span.open-in-agents-titlebar-widget-label"),
		);
		labelEl.textContent = label;
	}
}

export class OpenWorkspaceInAgentsContribution
	extends Disposable
...
type AgentsHandoffTipActionClassification = {
	mode: {
		...
		comment: "The configured tip mode active when the tip was clicked (default, custom).";
	};
	sessionType: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The chat session type / agent harness being handed off (e.g. copilot-cli, agent-host-copilot).";
	};
	owner: "justschen";
 ...
 */
export class AgentsHandoffInputTipContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private static readonly NOTIFICATION_ID =
		"chat.agentsHandoff.openInAgentsWindow";
	 ...
	 */
	private static readonly TIP_OPEN_COMMAND_ID =
		"workbench.action.chat.agentsHandoffTip.open";

	/** Session types eligible for the handoff tip — the same set the Agents window can render directly. */
	private static readonly ELIGIBLE_SESSION_TYPES: ReadonlySet<string> = new Set(
		[SessionType.CopilotCLI, SessionType.AgentHostCopilot],
	);
	 ...
	 */
	private static readonly DISMISSED_STORAGE_KEY =
		"chat.agentsHandoff.tip.dismissedSessions";
	...
	constructor(
		...
		this._register(
			CommandsRegistry.registerCommand(
				AgentsHandoffInputTipContribution.TIP_OPEN_COMMAND_ID,
				(accessor, ...args) => {
					this._telemetryService.publicLog2<
						AgentsHandoffTipActionEvent,
						AgentsHandoffTipActionClassification
					>("chat.agentsHandoffTip.action", {
						mode: this._getMode(),
						sessionType: this._lastPostedSessionType ?? "",
					});
					return accessor
						.get(ICommandService)
						.executeCommand(OpenChatSessionInAgentsWindowAction.ID, ...args);
				},
			),
		);

		this._register(
			this._chatWidgetService.onDidChangeFocusedSession(() => this._update()),
		);
		this._register(
			this._chatWidgetService.onDidAddWidget(() => this._update()),
		);
		...
		this._register(
			this._notificationService.onDidDismiss((id) => {
				if (
	...
	private _update(): void {
		...
		if (mode === AgentsHandoffTipMode.Hidden) {
			if (this._lastPostedFor) {
				this._notificationService.deleteNotification(
					AgentsHandoffInputTipContribution.NOTIFICATION_ID,
				);
				this._lastPostedFor = undefined;
			}
			return;
		...
		const resourceSessionType = sessionResource
			? getChatSessionType(sessionResource)
			: undefined;
		const preconditionMet =
			widget?.scopedContextKeyService.contextMatchesRules(
				OPEN_AGENTS_WINDOW_PRECONDITION,
			) ?? false;
		...
		const eligible =
			...
			!!sessionResource &&
			!!resourceSessionType &&
			AgentsHandoffInputTipContribution.ELIGIBLE_SESSION_TYPES.has(
				resourceSessionType,
			) &&
		...
		this._lastPostedSessionType = eligible
			? resourceSessionType
			: widgetSessionType;
		...
		const commandArgs: unknown[] =
			eligible && sessionResource ? [sessionResource] : [];
		...
		const message = useEmptyWorkspaceCopy
			? localize(
					...
					"Copilot CLI [Local] isn't available without an open folder",
				)
			: localize(
