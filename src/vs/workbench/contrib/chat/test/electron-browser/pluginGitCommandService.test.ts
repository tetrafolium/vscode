...
suite("NativePluginGitCommandService", () => {
	...
	function createLocalGitStub(
		overrides?: Partial<ILocalGitService>,
	): ILocalGitService {
		return {
	...
	test("cloneRepository delegates to ILocalGitService", async () => {
		const calls: string[] = [];
		const service = new NativePluginGitCommandService(
			createLocalGitStub({
		...
		assert.deepStrictEqual(calls, [
			`clone:https://github.com/test/repo.git:${targetDir.fsPath}:main`,
		]);
	});
	...
	test("revListCount delegates to ILocalGitService", async () => {
		...
		const result = await service.revListCount(
			URI.file("/tmp/repo"),
			"HEAD",
			"@{u}",
		);
	...
	test("cancellation token triggers cancel on local git service", async () => {
		...
		const p = service.cloneRepository(
			"https://github.com/test/repo.git",
			URI.file("/tmp/repo"),
			undefined,
			cts.token,
