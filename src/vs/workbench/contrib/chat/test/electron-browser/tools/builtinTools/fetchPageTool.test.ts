...
class ExtendedTestFileService extends TestFileService {
	...
	override async readFile(
		...
		const buffer =
			typeof content === "string" ? VSBuffer.fromString(content) : content;
		return {
			resource,
			value: buffer,
			name: "",
...
suite("FetchWebPageTool", () => {
	...
	test("should handle http/https via web content extractor and other schemes via file service", async () => {
		...
		assert.strictEqual(
			...
			7,
			"Should have result for each input URL",
		);
		...
		assert.strictEqual(
			...
			"HTTPS content",
			"HTTPS URL should return content",
		);
		assert.strictEqual(
			result.content[1].value,
			"HTTP content",
			"HTTP URL should return content",
		);

		// MCP resources should have their content from file service
		assert.strictEqual(
			result.content[2].value,
			"MCP resource content",
			"test:// URL should return content from file service",
		);
		assert.strictEqual(
			result.content[3].value,
			"Custom MCP content",
			"mcp-resource:// URL should return content from file service",
		);

		// Nonexistent file should be marked as invalid
		assert.strictEqual(
			result.content[4].value,
			"Invalid URL",
		...
		assert.strictEqual(
			Array.isArray(result.toolResultDetails)
				? result.toolResultDetails.length
				: 0,
			4,
			"Should have 4 valid URLs in toolResultDetails",
		);
	});
	...
	test("should handle empty and undefined URLs", async () => {
		...
		assert.strictEqual(
			emptyResult.content.length,
			1,
			"Empty array should return single message",
		);
		assert.strictEqual(
			emptyResult.content[0].value,
			"No valid URLs provided.",
			"Should indicate no valid URLs",
		);
		...
		assert.strictEqual(
			undefinedResult.content.length,
			1,
			"Undefined URLs should return single message",
		);
		assert.strictEqual(
			undefinedResult.content[0].value,
			"No valid URLs provided.",
			"Should indicate no valid URLs",
		);
		...
		const invalidResult = await tool.invoke(
			{
				...
				parameters: {
					urls: [
						"",
						" ",
						"invalid-scheme-that-fileservice-cannot-handle://test",
					],
				},
				context: undefined,
			},
		...
		assert.strictEqual(
			invalidResult.content.length,
			3,
			"Should have result for each invalid URL",
		);
		assert.strictEqual(
			invalidResult.content[0].value,
			"Invalid URL",
			"Empty string should be invalid",
		);
		assert.strictEqual(
			invalidResult.content[1].value,
			"Invalid URL",
			"Space-only string should be invalid",
		);
		assert.strictEqual(
			invalidResult.content[2].value,
			"Invalid URL",
			"Unhandleable scheme should be invalid",
		);
	});
	...
	test("should provide correct past tense messages for mixed valid/invalid URLs", async () => {
		...
		const messageText =
			typeof preparation.pastTenseMessage === "string"
				? preparation.pastTenseMessage
				: preparation.pastTenseMessage!.value;
		assert.ok(
			messageText.includes("Fetched"),
			"Should mention fetched resources",
		);
		assert.ok(
			messageText.includes("invalid://invalid"),
			"Should mention invalid URL",
		);
	});
	...
	test("should not show confirmation dialog for file URIs inside the workspace", async () => {
		...
		const preparation = await tool.prepareToolInvocation(
			{
				parameters: { urls: [URI.file("/workspaceRoot/plan.md").toString()] },
				toolCallId: "test-file-in-ws",
				chatSessionResource: undefined,
			},
			CancellationToken.None,
		);
		assert.ok(preparation, "Should return prepared invocation");
		assert.strictEqual(
			preparation.confirmationMessages?.title,
			undefined,
			"File inside workspace should not show confirmation dialog",
		);
		assert.strictEqual(
			preparation.confirmationMessages?.confirmResults,
			false,
			"File inside workspace should not require post-confirmation",
		);
	});
	...
	test("should show confirmation dialog for file URIs outside the workspace", async () => {
		...
		const preparation = await tool.prepareToolInvocation(
			{
				parameters: { urls: [URI.file("/tmp/external-plan.md").toString()] },
				toolCallId: "test-file-outside-ws",
				chatSessionResource: undefined,
			},
			CancellationToken.None,
		);
		...
		assert.ok(
			preparation.confirmationMessages?.title,
			"File outside workspace should show confirmation dialog",
		);
		assert.strictEqual(
			preparation.confirmationMessages?.confirmResults,
			true,
			"File outside workspace should require post-confirmation",
		);
	});
	...
	test("workspace file mixed with untrusted web URI: only web URI triggers confirmation", async () => {
		...
		const preparation = await tool.prepareToolInvocation(
			{
				parameters: {
					urls: [
						"https://example.com",
						URI.file("/workspaceRoot/plan.md").toString(),
					],
				},
		...
		assert.ok(
			preparation.confirmationMessages?.title,
			"Should show confirmation for untrusted web URI",
		);
		// The confirmation message should mention only the web URI, not the workspace file
		const msgValue =
			typeof preparation.confirmationMessages?.message === "string"
				? preparation.confirmationMessages.message
				: (preparation.confirmationMessages?.message?.value ?? "");
		assert.ok(
			!msgValue.includes("/workspaceRoot/"),
			"Confirmation message should not mention workspace file",
		);
		assert.ok(
			msgValue.includes("example.com"),
			"Confirmation message should mention web URI",
		);
	});
	...
	test("should approve when all URLs were mentioned in chat", async () => {
		...
		const preparation1 = await tool.prepareToolInvocation(
			{
				...
				chatSessionResource: LocalChatSessionUri.forSession("a"),
			},
			CancellationToken.None,
		);
		...
		const preparation2 = await tool.prepareToolInvocation(
			{
				parameters: { urls: ["https://other.com"] },
	...
	test("should return message for binary files indicating they are not supported", async () => {
		...
		const result = await tool.invoke(
			{
				...
				toolId: "fetch-page",
				parameters: {
					urls: ["file:///path/to/binary.dat", "file:///path/to/text.txt"],
				},
				context: undefined,
			},
			() => Promise.resolve(0),
		...
		assert.strictEqual(
			...
			"text",
			"Binary file should return text part",
		);
		...
		assert.strictEqual(
			...
			"text",
			"Text file should return text part",
		);
		...
		assert.strictEqual(
			Array.isArray(result.toolResultDetails)
				...
				: 0,
			2,
			"Should have 2 valid URLs in toolResultDetails",
		);
	});
	...
	test("PNG files are now supported as image data parts (regression test)", async () => {
		...
		assert.strictEqual(
			result.content[0].kind,
			"data",
			"PNG file should return data part",
		);
		if (result.content[0].kind === "data") {
			assert.strictEqual(
				result.content[0].value.mimeType,
				"image/png",
				"Should have PNG MIME type",
			);
			assert.strictEqual(
				result.content[0].value.data,
				binaryBuffer,
				"Should have correct binary data",
			);
		}
	});
	...
	test("should correctly distinguish between binary and text content", async () => {
		...
		const realBinaryData = new Uint8Array([
			0x89, 0x50, 0x4e, 0x47, 0x00, 0x00, 0x00, 0x0d, 0xff, 0x00, 0xab,
		]); // More clearly binary
		...
		assert.strictEqual(
			result.content[0].kind,
			"text",
			"JSON should be detected as text",
		);
		if (result.content[0].kind === "text") {
			assert.strictEqual(
				result.content[0].value,
		...
		assert.strictEqual(
			result.content[1].kind,
			"text",
			"Binary content should return text part with message",
		);
		if (result.content[1].kind === "text") {
			assert.strictEqual(
				result.content[1].value,
	...
	test("Supported image files are returned as data parts", async () => {
		...
		assert.strictEqual(
			result.content[1].kind,
			"data",
			"JPEG should be data part",
		);
		if (result.content[1].kind === "data") {
			assert.strictEqual(
				result.content[1].value.mimeType,
				"image/jpeg",
				"JPEG should have correct MIME type",
			);
			assert.strictEqual(
				result.content[1].value.data,
				jpegData,
				"JPEG should have correct data",
			);
		}
		...
		assert.strictEqual(
			...
			"data",
			"GIF should be data part",
		);
		if (result.content[2].kind === "data") {
			assert.strictEqual(
				result.content[2].value.mimeType,
				"image/gif",
				"GIF should have correct MIME type",
			);
			assert.strictEqual(
				result.content[2].value.data,
				gifData,
				"GIF should have correct data",
			);
		}
		...
		assert.strictEqual(
			...
			"data",
			"WebP should be data part",
		);
		if (result.content[3].kind === "data") {
			assert.strictEqual(
				result.content[3].value.mimeType,
				"image/webp",
				"WebP should have correct MIME type",
			);
			assert.strictEqual(
				result.content[3].value.data,
				webpData,
				"WebP should have correct data",
			);
		}

		// Check BMP
		assert.strictEqual(
			result.content[4].kind,
			"data",
			"BMP should be data part",
		);
		if (result.content[4].kind === "data") {
			assert.strictEqual(
				result.content[4].value.mimeType,
				"image/bmp",
				"BMP should have correct MIME type",
			);
			assert.strictEqual(
				result.content[4].value.data,
				bmpData,
				"BMP should have correct data",
			);
		}
	});
	...
	test("Mixed image and text files work correctly", async () => {
		...
		assert.strictEqual(
			result.content[0].kind,
			"text",
			"Text file should be text part",
		);
		if (result.content[0].kind === "text") {
			assert.strictEqual(
				result.content[0].value,
				textData,
				"Text should have correct content",
			);
		}
		...
		assert.strictEqual(
			result.content[1].kind,
			"data",
			"Image file should be data part",
		);
		if (result.content[1].kind === "data") {
			assert.strictEqual(
				result.content[1].value.mimeType,
				"image/png",
				"Image should have correct MIME type",
			);
			assert.strictEqual(
				result.content[1].value.data,
				imageData,
				"Image should have correct data",
			);
		}
	});
	...
	test("Case insensitive image extensions work", async () => {
		...
		assert.strictEqual(
			result.content[0].kind,
			"data",
			"PNG with uppercase extension should be data part",
		);
		if (result.content[0].kind === "data") {
			assert.strictEqual(
				result.content[0].value.mimeType,
		...
		assert.strictEqual(
			result.content[1].kind,
			"data",
			"JPEG with uppercase extension should be data part",
		);
		if (result.content[1].kind === "data") {
			assert.strictEqual(
				result.content[1].value.mimeType,
	...
	suite("toolResultDetails", () => {
		test("should include only successfully fetched URIs in correct order", async () => {
			...
			assert.ok(
				Array.isArray(result.toolResultDetails),
				"toolResultDetails should be an array",
			);
			assert.strictEqual(
				result.toolResultDetails.length,
				4,
				"Should have 4 successful URIs",
			);

			// Check that all entries are URI objects
			const uriDetails = result.toolResultDetails as URI[];
			assert.ok(
				uriDetails.every((uri) => uri instanceof URI),
				"All toolResultDetails entries should be URI objects",
			);
			...
			assert.deepStrictEqual(
				actualUriStrings.sort(),
				expectedUris.sort(),
				"Should contain exactly the expected successful URIs",
			);
			...
			assert.strictEqual(
				result.content.length,
				6,
				"Content should have result for each input URL",
			);
			assert.strictEqual(
				result.content[0].value,
				"Content 1",
				"First web URI content",
			);
			assert.strictEqual(
				result.content[1].value,
				"Invalid URL",
			...
			assert.strictEqual(
				result.content[3].value,
				"Content 2",
				"Second web URI content",
			);
			assert.strictEqual(
				result.content[4].value,
				"Invalid URL",
				"Nonexistent file marked as invalid",
			);
			assert.strictEqual(
				result.content[5].value,
				"MCP content",
		...
		test("should exclude failed web requests from toolResultDetails", async () => {
			...
			try {
				...
				assert.fail(
					"Expected test web content extractor to throw for missing URI",
				);
			} catch (error) {
				// This is expected behavior with the current test setup
				// The TestWebContentExtractorService throws when content is not found
				assert.ok(
					error.message.includes("No content configured for URI"),
					"Should throw for unconfigured URI",
				);
			}
		});
		...
		test("should exclude failed file reads from toolResultDetails", async () => {
			...
			assert.strictEqual(
				result.toolResultDetails.length,
				1,
				"Should have only 1 successful URI",
			);

			const uriDetails = result.toolResultDetails as URI[];
			assert.strictEqual(
				uriDetails[0].toString(),
				"file:///existing.txt",
				"Should contain only the successful file URI",
			);
			...
			assert.strictEqual(
				result.content.length,
				2,
				"Should have results for both input URLs",
			);
			assert.strictEqual(
				result.content[0].value,
				"File exists",
			...
			assert.strictEqual(
				result.content[1].value,
				"Invalid URL",
				"Second file should be marked invalid",
			);
		});
		...
		test("should handle mixed success and failure scenarios", async () => {
			...
			const fileContentMap = new ResourceMap<string | VSBuffer>([
				...
				[
					URI.parse("mcp-resource://good/file.txt"),
					VSBuffer.fromString("MCP binary content"),
				],
			]);
			...
			assert.strictEqual(
				(result.toolResultDetails as URI[]).length,
				3,
				"Should have 3 successful URIs",
			);
			...
			assert.deepStrictEqual(
				...
				"Should contain exactly the successful URIs",
			);
			...
			assert.strictEqual(
				result.content.length,
				6,
				"Should have results for all input URLs",
			);
			assert.strictEqual(
				result.content[0].value,
				"Invalid URL",
				"Invalid scheme marked as invalid",
			);
			assert.strictEqual(
				result.content[1].value,
				"Web success",
			...
			assert.strictEqual(
				result.content[2].value,
				"Invalid URL",
				"Missing file marked as invalid",
			);
			assert.strictEqual(
				result.content[3].value,
				"File success",
				"File success content",
			);
			assert.strictEqual(
				result.content[4].value,
				"Invalid URL",
		...
		test("should return empty toolResultDetails when all requests fail", async () => {
			...
			try {
				...
				assert.strictEqual(
					(result.toolResultDetails as URI[]).length,
					0,
					"Should have no successful URIs",
				);
				assert.strictEqual(
					result.content.length,
					4,
				...
				assert.ok(
					result.content.every((content) => content.value === "Invalid URL"),
					"All content should be marked as invalid",
				);
			} catch (error) {
		...
		test("should handle empty URL array", async () => {
			...
			assert.strictEqual(
				result.content.length,
				1,
				"Should have one content item for empty URLs",
			);
			assert.strictEqual(
				result.content[0].value,
				"No valid URLs provided.",
			...
			assert.ok(
				...
				"toolResultDetails should not be present for empty URLs",
			);
		});
		...
		test("should handle image files in toolResultDetails", async () => {
			...
			const result = await tool.invoke(
				{
					callId: "test-images",
					toolId: "fetch-page",
					parameters: { urls: ["file:///image.png", "file:///document.txt"] },
					context: undefined,
				},
				() => Promise.resolve(0),
				{ report: () => {} },
			...
			assert.strictEqual(
				(result.toolResultDetails as URI[]).length,
				2,
				"Should have 2 successful file URIs",
			);
			...
			assert.strictEqual(
				uriDetails[0].toString(),
				"file:///image.png",
				"Should include image file",
			);
			assert.strictEqual(
				uriDetails[1].toString(),
				"file:///document.txt",
		...
		test("confirmResults is false when all web contents are errors or redirects", async () => {
			...
			assert.strictEqual(
				...
				false,
				"confirmResults should be false when all results are errors",
			);
		});
		...
		test("confirmResults is false when all web contents are redirects", async () => {
			...
			const tool = new FetchWebPageTool(
				new (class extends TestWebContentExtractorService {
					...
					override async extract(
						...
						return uris.map(() => ({
							status: "redirect",
							toURI: URI.parse("https://redirected.com"),
						}));
					}
				})(),
				new ExtendedTestFileService(new ResourceMap<string | VSBuffer>()),
			...
			assert.strictEqual(
				result.confirmResults,
				false,
				"confirmResults should be false when all results are redirects",
			);
		});
		...
		test("confirmResults is undefined when at least one web content succeeds", async () => {
			...
			assert.strictEqual(
				result.confirmResults,
				undefined,
				"confirmResults should be undefined when at least one result succeeds",
			);
		});
		...
		test("redirect result provides correct message with new URL", async () => {
			...
			if (result.content[0].kind === "text") {
				assert.ok(
					result.content[0].value.includes(redirectURI.toString(true)),
					"Redirect message should include target URL",
				);
				assert.ok(
					result.content[0].value.includes(InternalFetchWebPageToolId),
					"Redirect message should suggest using tool again",
				);
			}
		});
	});
