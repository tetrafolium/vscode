...
function makeToolInvocation(
	overrides: Partial<IChatToolInvocationSerialized> = {},
): IChatToolInvocationSerialized {
	return {
...
function makeResponse(
	...
	const sessionResource =
		opts.sessionResource ?? URI.parse("chat-session://test/session");
...
const fakeReadFile = (uri: URI) =>
	Promise.resolve(VSBuffer.fromString(`data-for-${uri.path}`));

function makeRequest(
	variables: IChatRequestViewModel["variables"],
	opts: { id?: string; messageText?: string } = {},
): IChatRequestViewModel {
	return {
		id: opts.id ?? "req-1",
...
function makeImageVariableEntry(
	overrides: Partial<IImageVariableEntry> & Pick<IImageVariableEntry, "value">,
): IImageVariableEntry {
...
suite("extractImagesFromChatResponse", () => {
	...
	test("uses attachment summary title when matching request has no message text", async () => {
		const image = makeImageVariableEntry({
			value: VSBuffer.fromString("image").buffer,
		});
		const response = makeResponse([], {
			requestMessageText: "",
			requestVariables: [image],
		});
		const result = await extractImagesFromChatResponse(response, fakeReadFile);
	...
	test("preserves interleaved order of tool and inline reference images", async () => {
		...
		const inlineRef = makeInlineReference(
			URI.file("/middle.png"),
			"middle.png",
		);
	...
	test("skips inline reference when readFile fails", async () => {
		...
		const failingReadFile = (_uri: URI) =>
			Promise.reject(new Error("File not found"));

		const response = makeResponse([inlineRef]);
		const result = await extractImagesFromChatResponse(
			response,
			failingReadFile,
		);
		assert.strictEqual(result.images.length, 0);
	...
	test("extracts images from tool invocation message URIs", async () => {
		...
		const toolInvocation = makeToolInvocation({
			...
			toolId: "screenshot-tool",
			pastTenseMessage: {
				value: "Took a screenshot",
				isTrusted: false,
				uris: { "0": imageUri.toJSON() },
			},
		});
...
suite("extractImagesFromToolInvocationMessages", () => {
	...
	test("returns empty when message is undefined", async () => {
		...
		const result = await extractImagesFromToolInvocationMessages(
			toolInvocation,
			fakeReadFile,
		);
		assert.deepStrictEqual(result, []);
	...
	test("skips non-image URIs", async () => {
		const toolInvocation = makeToolInvocation({
			pastTenseMessage: {
				value: "Code file",
				isTrusted: false,
				uris: { "0": URI.file("/src/main.ts").toJSON() },
			},
		});
		const result = await extractImagesFromToolInvocationMessages(
			toolInvocation,
