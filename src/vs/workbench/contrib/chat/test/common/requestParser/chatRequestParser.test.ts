...
suite("ChatRequestParser", () => {
	...
	test("prompt slash command", async () => {
		...
		promptSlashCommandService.isValidSlashCommandName.callsFake(
			(command: string) => {
	...
	test("prompt subcommand via space form resolves to colon-named prompt", () => {
		...
		promptsService.hasPromptSlashCommand.callsFake(
			(name: string) => name === "chronicle:tips",
		);
		...
		const result = parser.parseChatRequest(
			testSessionUri,
			"/chronicle tips show me insights",
		);
	...
	test("agent host: forcedAgent + supportsPromptAttachments revives /skill as prompt slash part", async () => {
		...
		const result = parser.parseChatRequestWithReferences(
			...
			ChatAgentLocation.Chat,
			{
				...
				forcedAgent,
				attachmentCapabilities: forcedAgent.capabilities,
			},
		);
		await assertSnapshot(result);
	});
	...
	test("agent host: forcedAgent does not fall back to default agent subcommand", () => {
		...
		agentsService.getDefaultAgent.returns(
			getAgentWithSlashCommands([{ name: "compact", description: "" }]),
		);
		...
		promptsService.isValidSlashCommandName.callsFake(
			(command: string) => command === "compact",
		);
		instantiationService.stub(IPromptsService, promptsService);
		...
		assert.deepStrictEqual(
			{
				hasSubcommand: result.parts.some(
					(part) => part.kind === ChatRequestAgentSubcommandPart.Kind,
				),
	...
	test("agents and tools and multiline", async () => {
		...
		variableService.setSelectedToolAndToolSets(
			testSessionUri,
			new Map([
				[
					{
