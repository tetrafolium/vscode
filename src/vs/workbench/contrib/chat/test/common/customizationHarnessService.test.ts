...
suite("CustomizationHarnessService", () => {
	...
	suite("registerExternalHarness", () => {
		test("forwards item provider changes via onDidChangeSlashCommands with sessionType", () => {
			...
			const listener = store.add(
				service.onDidChangeSlashCommands(
					(e) => (firedSessionType = e.sessionType),
				),
			);
			store.add(listener);
		...
		test("forwards item provider changes via onDidChangeCustomAgents with sessionType", () => {
			...
			let firedSessionType: string | undefined;
			const listener = store.add(
				service.onDidChangeCustomAgents(
					(e) => (firedSessionType = e.sessionType),
				),
			);
			store.add(listener);
		...
		test("external harness provides storage filter", () => {
			...
			assert.deepStrictEqual(
				service.getStorageSourceFilter(PromptsType.agent),
				customFilter,
			);
		});
		...
		test("external harness item provider returns items", async () => {
			...
			const testItems = [
				{
					uri: URI.parse("file:///workspace/.claude/SKILL.md"),
					type: "skill",
					name: "Test Skill",
			...
			const items = await service
				.getActiveDescriptor()
				.itemProvider!.provideChatSessionCustomizations(
					testSessionResource,
		...
		test("external harness with same id as static harness replaces it", () => {
			...
			assert.strictEqual(service.availableHarnesses.get().length, 2);
			const cliHarness = service.availableHarnesses
				.get()
				.find((h) => h.id === "cli")!;
			...
			const service = createService({
				...
				itemProvider: {
					...
					provideChatSessionCustomizations: async (
						...
						{
							uri: URI.parse("file:///workspace/.test/prompts/fix.prompt.md"),
						...
						{
							uri: URI.parse("file:///workspace/.test/skills/lint/SKILL.md"),
						...
						{
							uri: URI.parse(
								"file:///workspace/.test/instructions/rule.instructions.md",
							),
						...
						{
							uri: URI.parse(
								"file:///workspace/.test/skills/disabled/SKILL.md",
							),
							type: PromptsType.skill,
							source: "local",
							name: "disabled",
			...
			const promptsService = new (class extends MockPromptsService {
				override async getPromptSlashCommands() {
					return [
						{
							uri: URI.parse(
								"file:///workspace/.github/prompts/explain.prompt.md",
							),
							name: "explain",
							type: PromptsType.prompt,
						...
						{
							uri: URI.parse(
								"file:///workspace/.github/skills/review/SKILL.md",
							),
							name: "review",
							type: PromptsType.skill,
				...
				override isValidSlashCommandName() {
					return true;
				}
			})();
			const service = new CustomizationHarnessServiceBase(
			...
			store.add(service);
			{
				...
				assert.deepStrictEqual(
					commands.map((command) => ({
						...
						sessionTypes: command.sessionTypes,
					})),
					[
						{
							name: "explain",
	...
	suite("getCustomAgents", () => {
		...
		test("falls back to promptsService and filters by session type", async () => {
			...
			promptsService.setCustomModes([
				createAgent(
					"matching",
					"file:///workspace/.github/agents/matching.agent.md",
					[testSessionType1],
					true,
				),
				createAgent(
					"global",
					"file:///workspace/.github/agents/global.agent.md",
					undefined,
					true,
				),
				createAgent(
					"other",
					"file:///workspace/.github/agents/other.agent.md",
					["other-session"],
					true,
				),
			]);
		...
		test("uses provider item URIs to scope resolved custom agents", async () => {
			...
			promptsService.setCustomModes([
				createAgent(
					"selected",
					"file:///workspace/.test/agents/selected.agent.md",
					undefined,
					true,
				),
				createAgent(
					"not-selected",
					"file:///workspace/.test/agents/not-selected.agent.md",
					undefined,
					false,
				),
			]);
				...
				[
					{
						...
						itemProvider: {
							...
							provideChatSessionCustomizations: async (
								...
								{
									uri: URI.parse(
										"file:///workspace/.test/agents/enabled.agent.md",
									),
								...
								{
									uri: URI.parse(
										"file:///workspace/.test/agents/disabled.agent.md",
									),
									type: PromptsType.agent,
									source: "local",
									name: "disabled",
			...
			store.add(service);
			{
				...
				assert.deepStrictEqual(
					agents.map((agent) => [agent.name, agent.enabled]),
					[
						["enabled", true],
						["disabled", false],
					],
	...
	suite("matchesWorkspaceSubpath", () => {
		test("matches segment boundary", () => {
			assert.ok(
				matchesWorkspaceSubpath("/workspace/.claude/skills/SKILL.md", [
					".claude",
				]),
			);
			assert.ok(
				matchesWorkspaceSubpath("/workspace/.github/instructions.md", [
					".github",
				]),
			);
		});
		...
		test("does not match partial segment", () => {
			assert.ok(
				!matchesWorkspaceSubpath("/workspace/not.claude/file.md", [".claude"]),
			);
		});
		...
		test("matches any of multiple subpaths", () => {
			assert.ok(
				matchesWorkspaceSubpath("/workspace/.copilot/file.md", [
					".github",
					".copilot",
				]),
			);
			assert.ok(
				matchesWorkspaceSubpath("/workspace/.github/file.md", [
					".github",
					".copilot",
				]),
			);
