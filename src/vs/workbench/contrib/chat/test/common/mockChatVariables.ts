...
export class MockChatVariablesService implements IChatVariablesService {
	...
	private _selectedToolAndToolSets =
		new ResourceMap<IToolAndToolSetEnablementMap>();
	...
	setDynamicVariables(
		sessionResource: URI,
		variables: readonly IDynamicVariable[],
	): void {
		this._dynamicVariables.set(sessionResource, variables);
	}

	setSelectedToolAndToolSets(
		sessionResource: URI,
		tools: IToolAndToolSetEnablementMap,
	): void {
