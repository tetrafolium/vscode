...
suite("FileBackedInstalledPluginsStore", () => {
	...
	test("migrates legacy storage to installed.json and removes legacy keys", async () => {
		...
		const legacyEntry = createLegacyEntry(
			"/cache/agentPlugins/github.com/microsoft/plugins/plugins/my-plugin",
		);
		...
		assert.ok(
			parsed.installed[0].pluginUri.includes(
				"/home/user/.vscode/agent-plugins/github.com/microsoft/plugins/plugins/my-plugin",
			),
		);
	...
	test("loads existing installed.json on startup", async () => {
		...
		assert.strictEqual(
			pluginsStore.get()[0].pluginUri.path,
			"/home/user/.vscode/agent-plugins/github.com/microsoft/plugins/plugins/my-plugin",
		);
	});
});
