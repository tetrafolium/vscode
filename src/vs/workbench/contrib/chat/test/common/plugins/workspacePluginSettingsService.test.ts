...
suite("WorkspacePluginSettingsService", () => {
	...
	function createService(): WorkspacePluginSettingsService {
		return store.add(
			new WorkspacePluginSettingsService(
				fileService,
	...
	async function writeCopilotSettings(content: string): Promise<void> {
		const uri = URI.from({
			scheme: Schemas.inMemory,
			path: "/workspace/.github/copilot/settings.json",
		});
		await fileService.writeFile(uri, VSBuffer.fromString(content));
	}
	...
	test("parses enabledPlugins from Claude settings", () =>
	...
	test("settings.local.json overrides settings.json for enabledPlugins", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
					enabledPlugins: {
			...
			await writeClaudeLocalSettings(
				JSON.stringify({
					enabledPlugins: {
						"my-plugin@mp": false,
					},
			...
			const enabled = service.enabledPlugins.get();
			assert.strictEqual(
				enabled.get("my-plugin@mp"),
				false,
				"local should override shared",
			);
			assert.strictEqual(
				enabled.get("other-plugin@mp"),
				true,
				"non-overridden key preserved",
			);
		}));

	test("merges enabledPlugins from Claude and Copilot settings", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
	...
	test("Claude enabledPlugins take precedence over Copilot for same key", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
	...
	test("parses GitHub shorthand from extraKnownMarketplaces", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
			...
			assert.strictEqual(
				marketplaces[0].reference.displayLabel,
				"my-marketplace",
			);
			assert.strictEqual(marketplaces[0].reference.githubRepo, "owner/repo");
		}));

	test("parses marketplace refs from extraKnownMarketplaces", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
					extraKnownMarketplaces: {
			...
			assert.strictEqual(
				marketplaces[0].reference.canonicalId,
				"github:owner/repo#marketplace",
			);
		}));

	test("parses nested source object from extraKnownMarketplaces", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
					extraKnownMarketplaces: {
			...
			assert.strictEqual(marketplaces.length, 1);
			assert.strictEqual(
				marketplaces[0].reference.githubRepo,
				"nested-owner/nested-repo",
			);
			assert.strictEqual(marketplaces[0].reference.displayLabel, "nested-mp");
		}));

	test("deduplicates marketplaces across Claude and Copilot by canonical ID", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
					extraKnownMarketplaces: {
			...
			assert.strictEqual(
				marketplaces.length,
				1,
				"should deduplicate by canonical ID",
			);
			assert.strictEqual(
				marketplaces[0].name,
				"claude-name",
				"Claude entry should win",
			);
		}));

	// --- Invalid input handling ---

	test("ignores invalid enabledPlugins shapes", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
	...
	test("ignores non-object marketplace entries", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			await writeClaudeSettings(
				JSON.stringify({
					extraKnownMarketplaces: {
	...
	test("returns empty observables when no settings files exist", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const service = createService();
			await new Promise<void>((r) => queueMicrotask(r));
