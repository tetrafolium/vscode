...
suite("AgentPlugin format detection", () => {
	...
	setup(() => {
		...
		store.add(
			fileService.registerProvider(
				Schemas.inMemory,
				store.add(new InMemoryFileSystemProvider()),
			),
		);
	...
	test("detects Open Plugin format when .plugin/plugin.json exists", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/my-open-plugin");
			await writeFile(
				"/plugins/my-open-plugin/.plugin/plugin.json",
				JSON.stringify({ name: "my-open-plugin" }),
			);
	...
	test("detects Claude format when .claude-plugin/plugin.json exists", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/my-claude-plugin");
			await writeFile(
				"/plugins/my-claude-plugin/.claude-plugin/plugin.json",
				JSON.stringify({ name: "my-claude-plugin" }),
			);
	...
	test("falls back to Copilot format when no vendor manifest exists", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/my-copilot-plugin");
			await writeFile(
				"/plugins/my-copilot-plugin/plugin.json",
				JSON.stringify({ name: "my-copilot-plugin" }),
			);
	...
	test("plugin label uses manifest `name` when no marketplace metadata is present", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const uri = pluginUri(
				"/plugins/_direct/sukumarp2022--slide-creator-plugin",
			);
			await writeFile(
				"/plugins/_direct/sukumarp2022--slide-creator-plugin/plugin.json",
				JSON.stringify({
					name: "Slide Creator",
				}),
			);
	...
	test("plugin label falls back to basename when manifest `name` is missing or invalid", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			await writeFile(
				"/plugins/blank-name/plugin.json",
				JSON.stringify({ name: "   " }),
			);

			const nonStringUri = pluginUri("/plugins/non-string-name");
			await writeFile(
				"/plugins/non-string-name/plugin.json",
				JSON.stringify({ name: 42 }),
			);

			const discovery = createDiscovery();
			discovery.start(mockEnablementModel);
			await discovery.setSourcesAndRefresh([
				missingUri,
				blankUri,
				nonStringUri,
			]);
			...
			const plugins = discovery.plugins.get();
			assert.deepStrictEqual(plugins.map((p) => p.label).sort(), [
				"blank-name",
				"missing-name",
				"non-string-name",
			]);
	...
	test("Open Plugin format takes priority over Claude format", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			await writeFile(
				"/plugins/dual-plugin/.plugin/plugin.json",
				JSON.stringify({ name: "dual-plugin" }),
			);
			await writeFile(
				"/plugins/dual-plugin/.claude-plugin/plugin.json",
				JSON.stringify({ name: "dual-plugin" }),
			);
			...
			await writeFile(
				...
				JSON.stringify({
					name: "dual-plugin",
					mcpServers: {
						"claude-server": { command: "echo", args: ["claude"] },
					},
				}),
			);
			...
			await waitForState(
				plugins[0].mcpServerDefinitions,
				(defs) => defs.length > 0,
			);
	...
	test("Open Plugin reads MCP definitions from .plugin/plugin.json inline", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/mcp-plugin");
			await writeFile(
				"/plugins/mcp-plugin/.plugin/plugin.json",
				JSON.stringify({
					name: "mcp-plugin",
					mcpServers: {
			...
			const mcpDefs = plugins[0].mcpServerDefinitions.get();
			assert.deepStrictEqual(
				mcpDefs.map((d) => d.name),
				["my-server"],
			);
		}));

	test("Open Plugin reads MCP definitions from standalone .mcp.json", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/mcp-standalone");
			await writeFile(
				"/plugins/mcp-standalone/.plugin/plugin.json",
				JSON.stringify({ name: "mcp-standalone" }),
			);
			await writeFile(
				"/plugins/mcp-standalone/.mcp.json",
				JSON.stringify({
					mcpServers: {
			...
			assert.strictEqual(
				plugins[0].mcpServerDefinitions.get()[0].name,
				"standalone-server",
			);
		}));

	test("reads skills from skills/ subdirectories", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/skills-plugin");
			await writeFile(
				"/plugins/skills-plugin/.plugin/plugin.json",
				JSON.stringify({ name: "skills-plugin" }),
			);
			await writeFile(
				"/plugins/skills-plugin/skills/deploy/SKILL.md",
				"# Deploy skill",
			);
			await writeFile(
				"/plugins/skills-plugin/skills/lint/SKILL.md",
				"# Lint skill",
			);
			...
			const skillNames = plugins[0].skills
				.get()
				.map((s) => s.name)
				.sort();
	...
	test("reads root-level SKILL.md as a fallback skill", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/root-skill");
			await writeFile(
				"/plugins/root-skill/.plugin/plugin.json",
				JSON.stringify({ name: "root-skill" }),
			);
	...
	test("root-level SKILL.md is ignored when skills/ has content", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/root-skill-ignored");
			await writeFile(
				"/plugins/root-skill-ignored/.plugin/plugin.json",
				JSON.stringify({ name: "root-skill-ignored" }),
			);
			await writeFile("/plugins/root-skill-ignored/SKILL.md", "# Root skill");
			await writeFile(
				"/plugins/root-skill-ignored/skills/real/SKILL.md",
				"# Real skill",
			);
	...
	test("reads agents from agents/ directory", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/agents-plugin");
			await writeFile(
				"/plugins/agents-plugin/.plugin/plugin.json",
				JSON.stringify({ name: "agents-plugin" }),
			);
			await writeFile(
				"/plugins/agents-plugin/agents/reviewer.md",
				"---\nname: reviewer\n---\nYou review code.",
			);
	...
	test("manifest skills field adds supplemental skill directories", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/custom-skills");
			await writeFile(
				"/plugins/custom-skills/.plugin/plugin.json",
				JSON.stringify({
					name: "custom-skills",
					skills: "./extra-skills/",
				}),
			...
			await writeFile(
				"/plugins/custom-skills/skills/default-skill/SKILL.md",
				"# Default skill",
			);
			await writeFile(
				"/plugins/custom-skills/extra-skills/bonus-skill/SKILL.md",
				"# Bonus skill",
			);
	...
	test("manifest skills field with exclusive mode skips default directory", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/exclusive-skills");
			await writeFile(
				"/plugins/exclusive-skills/.plugin/plugin.json",
				JSON.stringify({
			...
			await writeFile(
				"/plugins/exclusive-skills/skills/ignored/SKILL.md",
				"# Should be ignored",
			);
			await writeFile(
				"/plugins/exclusive-skills/only-here/visible/SKILL.md",
				"# Should be visible",
			);
	...
	test("manifest commands field with string array scans multiple directories", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/multi-commands");
			await writeFile(
				"/plugins/multi-commands/.plugin/plugin.json",
				JSON.stringify({
					name: "multi-commands",
					commands: ["./cmd1/", "./cmd2/"],
				}),
			...
			await writeFile(
				"/plugins/multi-commands/commands/default.md",
				"# Default",
			);
	...
	test("manifest agents field adds supplemental agent directories", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/custom-agents");
			await writeFile(
				"/plugins/custom-agents/.plugin/plugin.json",
				JSON.stringify({
					name: "custom-agents",
					agents: "./extra-agents/",
				}),
			...
			await writeFile(
				"/plugins/custom-agents/agents/default-agent.md",
				"# Default",
			);
			await writeFile(
				"/plugins/custom-agents/extra-agents/bonus-agent.md",
				"# Bonus",
			);
	...
	test("path traversal in manifest is rejected", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			await writeFile(
				"/plugins/traversal/.plugin/plugin.json",
				JSON.stringify({
					name: "traversal",
					skills: "../outside/",
				}),
	...
	test("duplicate names across directories deduplicate (first wins)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/dedup");
			await writeFile(
				"/plugins/dedup/.plugin/plugin.json",
				JSON.stringify({
					name: "dedup",
			...
			await writeFile(
				"/plugins/dedup/extra-commands/shared.md",
				"# Custom version",
			);
	...
	test("reads hooks from default hooks/hooks.json", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/hooks-default");
			await writeFile(
				"/plugins/hooks-default/.plugin/plugin.json",
				JSON.stringify({ name: "hooks-default" }),
			);
			await writeFile(
				"/plugins/hooks-default/hooks/hooks.json",
				JSON.stringify({
					hooks: {
						PostToolUse: [
							{
	...
	test("reads inline hooks from manifest", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/hooks-inline");
			await writeFile(
				"/plugins/hooks-inline/.plugin/plugin.json",
				JSON.stringify({
					name: "hooks-inline",
					hooks: {
						hooks: {
	...
	test("reads hooks from custom path in manifest", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/hooks-custom");
			await writeFile(
				"/plugins/hooks-custom/.plugin/plugin.json",
				JSON.stringify({
					name: "hooks-custom",
			...
			await writeFile(
				"/plugins/hooks-custom/config/my-hooks.json",
				JSON.stringify({
					hooks: {
						PostToolUse: [
							{
	...
	test("reads MCP from custom path in manifest", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/mcp-custom");
			await writeFile(
				"/plugins/mcp-custom/.plugin/plugin.json",
				JSON.stringify({
					name: "mcp-custom",
			...
			await writeFile(
				"/plugins/mcp-custom/config/servers.json",
				JSON.stringify({
					mcpServers: {
	...
	test("inline MCP in manifest takes priority over standalone .mcp.json", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/mcp-merged");
			await writeFile(
				"/plugins/mcp-merged/.plugin/plugin.json",
				JSON.stringify({
					name: "mcp-merged",
					mcpServers: {
	...
	test("PLUGIN_ROOT expansion in hook commands", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/root-expansion");
			await writeFile(
				"/plugins/root-expansion/.plugin/plugin.json",
				JSON.stringify({
					name: "root-expansion",
					hooks: {
						hooks: {
			...
			assert.ok(
				command && !command.includes("${PLUGIN_ROOT}"),
				`Expected PLUGIN_ROOT to be expanded, got: ${command}`,
			);
		}));

	test("manifest commands field pointing to a specific file", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/cmd-file");
			await writeFile(
				"/plugins/cmd-file/.plugin/plugin.json",
				JSON.stringify({
					name: "cmd-file",
	...
	test("manifest commands field with array of specific files", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/cmd-files");
			await writeFile(
				"/plugins/cmd-files/.plugin/plugin.json",
				JSON.stringify({
					name: "cmd-files",
	...
	test("manifest agents field pointing to a specific file", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/agent-file");
			await writeFile(
				"/plugins/agent-file/.plugin/plugin.json",
				JSON.stringify({
					name: "agent-file",
			...
			await writeFile(
				"/plugins/agent-file/custom/specialist.md",
				"# Specialist",
			);
	...
	test("manifest skills field pointing to a specific skill directory", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/skill-dir");
			await writeFile(
				"/plugins/skill-dir/.plugin/plugin.json",
				JSON.stringify({
					name: "skill-dir",
			...
			await writeFile(
				"/plugins/skill-dir/custom/my-skill/SKILL.md",
				"# My Skill",
			);
	...
	test("manifest hooks field pointing to a specific file", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/hook-file");
			await writeFile(
				"/plugins/hook-file/.plugin/plugin.json",
				JSON.stringify({
					name: "hook-file",
			...
			await writeFile(
				"/plugins/hook-file/config/custom-hooks.json",
				JSON.stringify({
					hooks: {
						SessionStart: [
	...
	test("manifest mcpServers field pointing to a specific file", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/mcp-file");
			await writeFile(
				"/plugins/mcp-file/.plugin/plugin.json",
				JSON.stringify({
					name: "mcp-file",
					mcpServers: "./config/servers.json",
				}),
			...
			await writeFile(
				"/plugins/mcp-file/config/servers.json",
				JSON.stringify({
					mcpServers: {
	...
	test("reads rules from rules/ directory with .mdc extension", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/rules-plugin");
			await writeFile(
				"/plugins/rules-plugin/.plugin/plugin.json",
				JSON.stringify({ name: "rules-plugin" }),
			);
			await writeFile(
				"/plugins/rules-plugin/rules/prefer-const.mdc",
				"---\ndescription: Prefer const\n---\nUse const.",
			);
			await writeFile(
				"/plugins/rules-plugin/rules/error-handling.mdc",
	...
	test("reads rules with .md and .instructions.md extensions", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/rules-mixed");
			await writeFile(
				"/plugins/rules-mixed/.plugin/plugin.json",
				JSON.stringify({ name: "rules-mixed" }),
			);
			await writeFile("/plugins/rules-mixed/rules/rule-a.mdc", "Rule A");
			await writeFile("/plugins/rules-mixed/rules/rule-b.md", "Rule B");
			await writeFile(
				"/plugins/rules-mixed/rules/rule-c.instructions.md",
				"Rule C",
			);
	...
	test("manifest rules field adds supplemental rule directories", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/custom-rules");
			await writeFile(
				"/plugins/custom-rules/.plugin/plugin.json",
				JSON.stringify({
					name: "custom-rules",
					rules: "./extra-rules/",
				}),
			...
			await writeFile(
				"/plugins/custom-rules/rules/default-rule.mdc",
				"Default rule",
			);
			await writeFile(
				"/plugins/custom-rules/extra-rules/bonus-rule.mdc",
				"Bonus rule",
			);
	...
	test("manifest rules field with exclusive mode skips default directory", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/exclusive-rules");
			await writeFile(
				"/plugins/exclusive-rules/.plugin/plugin.json",
				JSON.stringify({
			...
			await writeFile(
				"/plugins/exclusive-rules/rules/ignored.mdc",
				"Should be ignored",
			);
			await writeFile(
				"/plugins/exclusive-rules/only-here/visible.mdc",
				"Should be visible",
			);
	...
	test("rule name strips longest matching suffix first", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/suffix-rules");
			await writeFile(
				"/plugins/suffix-rules/.plugin/plugin.json",
				JSON.stringify({ name: "suffix-rules" }),
			);
			await writeFile(
				"/plugins/suffix-rules/rules/coding-standards.instructions.md",
				"Standards",
			);
			...
			assert.strictEqual(
				plugins[0].instructions.get()[0].name,
				"coding-standards",
			);
		}));

	test("deduplicates rules with the same base name", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/dup-rules");
			await writeFile(
				"/plugins/dup-rules/.plugin/plugin.json",
				JSON.stringify({
					name: "dup-rules",
					rules: "./extra/",
				}),
	...
	test("PLUGIN_ROOT expansion in inline MCP server definitions", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/mcp-root");
			await writeFile(
				"/plugins/mcp-root/.plugin/plugin.json",
				JSON.stringify({
					name: "mcp-root",
					mcpServers: {
						"my-server": {
			...
			assert.ok(
				!config.command.includes("${PLUGIN_ROOT}"),
				`Expected PLUGIN_ROOT to be expanded in command, got: ${config.command}`,
			);
			assert.ok(
				!config.args[1].includes("${PLUGIN_ROOT}"),
				`Expected PLUGIN_ROOT to be expanded in args, got: ${config.args[1]}`,
			);
			assert.ok(
				!config.cwd.includes("${PLUGIN_ROOT}"),
				`Expected PLUGIN_ROOT to be expanded in cwd, got: ${config.cwd}`,
			);
			assert.ok(
				!config.env["CONFIG_DIR"].includes("${PLUGIN_ROOT}"),
				`Expected PLUGIN_ROOT to be expanded in env, got: ${config.env["CONFIG_DIR"]}`,
			);
			assert.strictEqual(
				config.env["PLUGIN_ROOT"],
				uri.fsPath,
				"Expected PLUGIN_ROOT env var to be set",
			);
		}));

	test("CLAUDE_PLUGIN_ROOT expansion in MCP server definitions from .mcp.json", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = pluginUri("/plugins/claude-mcp-root");
			await writeFile(
				"/plugins/claude-mcp-root/.claude-plugin/plugin.json",
				JSON.stringify({ name: "claude-mcp-root" }),
			);
			await writeFile(
				"/plugins/claude-mcp-root/.mcp.json",
				JSON.stringify({
					mcpServers: {
						"claude-server": {
			...
			const config: any = server.configuration;
			assert.ok(
				!config.command.includes("${CLAUDE_PLUGIN_ROOT}"),
				`Expected CLAUDE_PLUGIN_ROOT to be expanded in command, got: ${config.command}`,
			);
			assert.ok(
				!config.args[1].includes("${CLAUDE_PLUGIN_ROOT}"),
				`Expected CLAUDE_PLUGIN_ROOT to be expanded in args, got: ${config.args[1]}`,
			);
			assert.strictEqual(
				config.env["CLAUDE_PLUGIN_ROOT"],
				uri.fsPath,
				"Expected CLAUDE_PLUGIN_ROOT env var to be set",
			);
		}));
});
