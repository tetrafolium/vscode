 ...
 */
function convertBareEnvVarsToVsCodeSyntax(
	def: Omit<IMcpServerDefinition, "customization">,
) {
	return convertBareEnvVarsToVsCodeSyntaxRaw({
		...def,
...
suite("convertBareEnvVarsToVsCodeSyntax", () => {
	...
	function asStdio(
		result: ReturnType<typeof convertBareEnvVarsToVsCodeSyntax>,
	): IMcpStdioServerConfiguration {
	...
	suite("stdio (LOCAL) servers", () => {
		...
		test("converts multiple bare ${VAR} references in a single string", () => {
			...
			assert.strictEqual(
				cfg.command,
				"${env:BIN_DIR}/run --config ${env:CONFIG_DIR}/cfg.json",
			);
		});
