...
suite("PluginMarketplaceService", () => {
	...
	test("parses direct HTTPS and SSH marketplaces ending in .git", () => {
		...
		assert.deepStrictEqual(ssh.cacheSegments, [
			"git@example.com",
			"org",
			"repo",
		]);
	...
	test("parses git URI marketplaces with ref suffix", () => {
		const https = parseMarketplaceReference(
			"https://example.com/org/repo.git#marketplace",
		);
		assert.ok(https);
		assert.strictEqual(https?.cloneUrl, "https://example.com/org/repo.git");
		assert.strictEqual(
			https?.canonicalId,
			"git:example.com/org/repo.git#marketplace",
		);
		assert.deepStrictEqual(https?.cacheSegments, [
			"example.com",
			"org",
			"repo",
			"ref_marketplace",
		...
		const scp = parseMarketplaceReference(
			"git@example.com:org/repo.git#marketplace",
		);
		assert.ok(scp);
		...
		assert.deepStrictEqual(scp?.cacheSegments, [
			"example.com",
			"org",
			"repo",
			"ref_marketplace",
	...
	test("populates githubRepo for GitHub HTTPS URLs", () => {
		const withGit = parseMarketplaceReference(
			"https://github.com/owner/repo.git",
		);
		assert.ok(withGit);
		assert.strictEqual(withGit?.githubRepo, "owner/repo");

		const withoutGit = parseMarketplaceReference(
	...
	test("accepts HTTPS and SSH marketplace entries without .git suffix", () => {
		...
		assert.strictEqual(
			parseMarketplaceReference("git@example.com:org/repo"),
			undefined,
		);
	});

	test("accepts host-only HTTPS marketplace endpoints (per ADR-002 git.url is any string)", () => {
		const parsed = parseMarketplaceReference(
		...
		assert.strictEqual(
			parsed?.canonicalId,
			"git:plugins.internal.example.com/",
		);
		assert.deepStrictEqual(parsed?.cacheSegments, [
			"plugins.internal.example.com",
		]);
		assert.strictEqual(parsed?.githubRepo, undefined);

		// Trailing slash collapses to the host-only form.
		const withSlash = parseMarketplaceReference(
			"https://plugins.internal.example.com/",
		);
		assert.strictEqual(
			withSlash?.canonicalId,
			"git:plugins.internal.example.com/",
		);
	});
	...
	test("readConfiguredMarketplaces converts policy dict to named marketplace entries", () => {
		...
		const { extraValues, effectiveValues } = readConfiguredMarketplaces(
	...
	test("extraKnownMarketplacesToConfigDict: returns undefined for empty/missing input", () => {
		assert.strictEqual(
			extraKnownMarketplacesToConfigDict(undefined),
			undefined,
		);
	...
	test("extraKnownMarketplacesToConfigDict: github source becomes owner/repo shorthand", () => {
		const dict = extraKnownMarketplacesToConfigDict([
			{
				name: "vscode-team-kit",
				source: { source: "github", repo: "microsoft/vscode-team-kit" },
			},
		]);
	...
	test("extraKnownMarketplacesToConfigDict: github source with ref appends #ref", () => {
		...
		assert.deepStrictEqual(dict, {
			"team-kit-beta": "microsoft/vscode-team-kit#beta",
		});
	});
	...
	test("extraKnownMarketplacesToConfigDict: git source becomes raw URL (with optional #ref)", () => {
		const dict = extraKnownMarketplacesToConfigDict([
			{
				name: "acme-internal",
				source: { source: "git", url: "https://plugins.internal.acme.com" },
			},
			{
				name: "acme-tagged",
				source: {
					source: "git",
					url: "https://git.acme.com/plugins.git",
					ref: "v1",
				},
			},
		]);
	...
	test("extraKnownMarketplacesToConfigDict: end-to-end policy → config dict → readConfiguredMarketplaces → parseMarketplaceReferences", () => {
		...
		const policyEntries = [
			...
			},
			{
				...
				source: {
					source: "git" as const,
					url: "https://copilot-plugins.acme.io",
				},
			},
			{
				name: "vscode-team-kit",
		...
		const { extraValues } = readConfiguredMarketplaces(
			configService as unknown as IConfigurationService,
		);
		const refs = parseMarketplaceReferences(extraValues);
		...
		assert.strictEqual(
			refs.length,
			3,
			"all three policy entries are surfaced as marketplace references",
		);
		assert.deepStrictEqual(
			refs.map((r) => r.displayLabel),
			["acme-internal", "acme-public", "vscode-team-kit"],
	...
	test("parses Azure DevOps HTTPS clone URLs without .git suffix", () => {
		...
		assert.strictEqual(
			parsed?.canonicalId,
			"git:dev.azure.com/org/project/_git/repo.git",
		);
		assert.deepStrictEqual(parsed?.cacheSegments, [
			"dev.azure.com",
			"org",
	...
	test("github.com URI form and GitHub shorthand form share the same canonicalId (policy trust comparisons must match)", () => {
		...
		const httpsWithGit = parseMarketplaceReference(
			"https://github.com/microsoft/vscode-team-kit.git",
		);
		const httpsWithoutGit = parseMarketplaceReference(
	...
	test("parseMarketplaceReferences accepts policy-shape objects and uses name as displayLabel", () => {
		...
		assert.strictEqual(
			parsed[0].canonicalId,
			"github:microsoft/vscode-team-kit",
		);
	...
	test("treats different marketplace refs as distinct references", () => {
		...
		assert.deepStrictEqual(
			parsed.map((r) => r.canonicalId),
			["github:microsoft/vscode#main", "github:microsoft/vscode#marketplace"],
		);
	});
});
...
suite("PluginMarketplaceService - GitHub marketplace refs", () => {
	...
	test("fetches GitHub marketplace definitions from the configured ref", async () => {
		...
		} as Partial<IEnvironmentService> as IEnvironmentService);
		...
		const service = store.add(
			instantiationService.createInstance(PluginMarketplaceService),
		);
...
suite("PluginMarketplaceService - getMarketplacePluginMetadata", () => {
	...
	test("returns undefined for a URI that is not installed", () => {
		const service = createService();
		const result = service.getMarketplacePluginMetadata(
...
suite("PluginMarketplaceService - installed plugins lifecycle", () => {
	...
	test("addInstalledPlugin makes plugin appear in installedPlugins", () => {
		const service = createService();
		const uri = URI.file(
			"/agent-plugins/github.com/microsoft/plugins/my-plugin",
		);
	...
	test("getMarketplacePluginMetadata finds metadata for child URI", () => {
		...
		const childUri = URI.file(
			"/agent-plugins/github.com/microsoft/plugins/subdir/file.ts",
		);
	...
	test("multiple plugins can be installed independently", () => {
		const service = createService();
		const uri1 = URI.file(
			"/agent-plugins/github.com/microsoft/plugins/plugin-a",
		);
		const uri2 = URI.file(
			"/agent-plugins/github.com/microsoft/plugins/plugin-b",
		);
...
suite("PluginMarketplaceService - hydration after restart", () => {
	...
	function createPluginRepositoryStub(): IAgentPluginRepositoryService {
		const getRepositoryUri = (marketplace: IMarketplaceReference) =>
			URI.joinPath(CACHE_ROOT, ...marketplace.cacheSegments);
		const getPluginSourceInstallUri = (descriptor: IPluginSourceDescriptor) => {
			...
			if (descriptor.kind === PluginSourceKind.RelativePath) {
				...
				throw new Error(
					"RelativePath should not reach getPluginSourceInstallUri in hydration tests",
				);
			}
	...
	function storeMarketplaceCache(
	...
	test("hydrates a github-sourced plugin from installed.json name and marketplace cache after restart", async () => {
		...
		const awesomeCopilot = parseMarketplaceReference(
		...
		const azurePluginUri = URI.joinPath(
			CACHE_ROOT,
			"github.com",
			"microsoft",
			"azure-skills",
		...
		assert.strictEqual(
			installed.length,
			1,
			"azure plugin should be hydrated from marketplace data",
		);
		assert.strictEqual(installed[0].plugin.name, "azure");
		assert.strictEqual(
			installed[0].plugin.sourceDescriptor.kind,
			PluginSourceKind.GitHub,
		);
		assert.strictEqual(
			installed[0].plugin.marketplaceReference.canonicalId,
			awesomeCopilot.canonicalId,
		);
	});
	...
	test("persists plugin name when a plugin is added so it survives a restart", async () => {
		...
		const persisted = JSON.parse(
			fileService.files.get(installedJson.toString())!,
		);
...
suite("parsePluginSource", () => {
	...
	test("returns base dir for empty source without pluginRoot", () => {
		assert.deepStrictEqual(parsePluginSource("", undefined, logContext), {
			kind: PluginSourceKind.RelativePath,
			path: "",
		});
	});
	...
	test("returns base dir for undefined source without pluginRoot", () => {
		assert.deepStrictEqual(
			parsePluginSource(undefined, undefined, logContext),
			{ kind: PluginSourceKind.RelativePath, path: "" },
		);
	});
	...
	test("parses github object source with path", () => {
		const result = parsePluginSource(
			{ source: "github", repo: "owner/repo", path: "plugins/my-plugin" },
			undefined,
			logContext,
		);
		assert.deepStrictEqual(result, {
	...
	test("returns undefined for github source missing repo", () => {
		assert.strictEqual(
			parsePluginSource({ source: "github" }, undefined, logContext),
			undefined,
		);
	});
	...
	test("returns undefined for github source with invalid sha", () => {
		assert.strictEqual(
			parsePluginSource(
				{ source: "github", repo: "owner/repo", sha: "abc123" },
				undefined,
				logContext,
			),
			undefined,
	...
	test("returns undefined for github source with non-string path", () => {
		assert.strictEqual(
			parsePluginSource(
				{ source: "github", repo: "owner/repo", path: 42 } as never,
				undefined,
				logContext,
			),
			undefined,
	...
	test("returns undefined for url source missing url field", () => {
		assert.strictEqual(
			parsePluginSource({ source: "url" }, undefined, logContext),
			undefined,
		);
	});
	...
	test("returns undefined for url source not ending in .git", () => {
		assert.strictEqual(
			parsePluginSource(
				{ source: "url", url: "https://gitlab.com/team/plugin" },
				undefined,
				logContext,
			),
			undefined,
	...
	test("returns undefined for git-subdir source missing url field", () => {
		assert.strictEqual(
			parsePluginSource(
				{ source: "git-subdir", path: "plugins/foo" },
				undefined,
				logContext,
			),
			undefined,
	...
	test("returns undefined for npm source missing package", () => {
		assert.strictEqual(
			parsePluginSource({ source: "npm" }, undefined, logContext),
			undefined,
		);
	});
		...
		const result = parsePluginSource(
			{
				...
				package: "my-plugin",
				version: "1.0.0",
				registry: "https://pypi.example.com",
			},
			undefined,
			logContext,
		);
	...
	test("returns undefined for pip source missing package", () => {
		assert.strictEqual(
			parsePluginSource({ source: "pip" }, undefined, logContext),
			undefined,
		);
	});
	...
	test("returns undefined for pip source with non-string registry", () => {
		assert.strictEqual(
			parsePluginSource(
				{ source: "pip", package: "my-plugin", registry: 42 } as never,
				undefined,
				logContext,
			),
			undefined,
	...
	test("returns undefined for unknown source kind", () => {
		assert.strictEqual(
			parsePluginSource({ source: "unknown" }, undefined, logContext),
			undefined,
		);
	});
	...
	test("returns undefined for object source without source discriminant", () => {
		assert.strictEqual(
			parsePluginSource({ package: "test" } as never, undefined, logContext),
			undefined,
		);
	});
});
...
suite("getPluginSourceLabel", () => {
	...
	test("formats empty relative path", () => {
		assert.strictEqual(
			getPluginSourceLabel({ kind: PluginSourceKind.RelativePath, path: "" }),
			".",
		);
	});
	...
	test("formats url source with path", () => {
		assert.strictEqual(
			...
			"https://example.com/repo.git/plugins/foo",
		);
	});
