...
suite("ChatModeService", () => {
	...
	setup(async () => {
		...
		customizationHarnessService = testDisposables.add(
		...
		chatModeService = testDisposables.add(
			instantiationService.createInstance(ChatModeService),
		);
	...
	test("should adjust builtin modes based on tools agent availability", async () => {
		...
		assert.strictEqual(
			agents.builtin.find((agent) => agent.id === ChatModeKind.Agent),
			undefined,
		);
	...
	test("should find builtin modes by id", async () => {
		const agentMode = (await chatModeService.getLocalModes()).findModeById(
			ChatModeKind.Agent,
		);
		assert.ok(agentMode);
	...
	test("should return undefined for non-existent mode", async () => {
		const mode = (await chatModeService.getLocalModes()).findModeById(
			"non-existent-mode",
		);
	...
	test("should handle custom modes from prompts service", async () => {
		...
		assert.deepStrictEqual(
			testMode.modeInstructions?.get(),
			customMode.agentInstructions,
		);
		assert.deepStrictEqual(testMode.handOffs?.get(), customMode.handOffs);
		assert.strictEqual(
			testMode.uri?.get().toString(),
			customMode.uri.toString(),
		);
	...
	test("should fire change event when custom modes are updated", async () => {
		...
		testDisposables.add(
			(await chatModeService.getLocalModes()).onDidChange(() => {
				eventFired = true;
			}),
		);
	...
	test("should find custom modes by id", async () => {
		...
		const foundMode = (await chatModeService.getLocalModes()).findModeById(
			customMode.uri.toString(),
		);
		assert.ok(foundMode);
	...
	test("should update existing custom mode instances when data changes", async () => {
		...
		assert.strictEqual(
			initialCustomMode.description.get(),
			"Initial description",
		);
		...
		assert.strictEqual(
			updatedCustomMode.description.get(),
			"Updated description",
		);
		assert.deepStrictEqual(updatedCustomMode.customTools?.get(), [
			"tool1",
			"tool2",
		]);
		assert.deepStrictEqual(updatedCustomMode.modeInstructions?.get(), {
