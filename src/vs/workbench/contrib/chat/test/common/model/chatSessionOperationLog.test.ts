...
suite("ChatSessionOperationLog", () => {
	...
	function simulateFileRoundtrip(
		adapter: Adapt.ObjectMutationLog<TestObject, TestObject>,
		initial: TestObject,
		updates: TestObject[],
	): TestObject {
	...
	suite("Transform factories", () => {
		...
		test("primitive with custom comparator", () => {
			const transform = Adapt.value<string, string>(
	...
	suite("LogAdapter", () => {
		...
		test("write detects array append nested", () => {
			...
			adapter.confirmWrite();
			assert.deepStrictEqual(JSON.parse(result1.data.toString().trim()), {
				kind: 2,
				k: ["items", 0, "value"],
				v: [3],
			});
			...
			assert.deepStrictEqual(JSON.parse(result2.data.toString().trim()), {
				kind: 2,
				k: ["items"],
				i: 0,
				v: [{ id: "b", value: [1, 2, 3] }],
		...
		test("handles differentiated property changes", () => {
			...
			assert.notStrictEqual(
				...
				"",
				"different type should trigger change",
			);
			...
			assert.strictEqual(
				...
				"",
				"same type prefix should not trigger change",
			);
		});
		...
		test("read throws on empty log file", () => {
			...
			assert.throws(
				() => adapter.read(VSBuffer.fromString("")),
				/Empty log file/,
			);
		});
		...
		test("sealed objects skip non-key field comparison when both are sealed", () => {
			...
			const result1 = adapter.write({
				items: [{ id: "a", value: 999, isSealed: true }],
			});
			assert.strictEqual(
				result1.data.toString(),
				"",
				"sealed item value change should be ignored",
			);
		});
		...
		test("sealed objects still detect key changes", () => {
			...
			const result = adapter.write({
				items: [{ id: "b", value: 1, isSealed: true }],
			});
			assert.notStrictEqual(
				result.data.toString(),
				"",
				"key change should be detected even when sealed",
			);
		...
		test("sealed objects diff normally when one is not sealed", () => {
			...
			adapter.createInitial({
				items: [{ id: "a", value: 1, isSealed: false }],
			});

			// Change value - should be detected since prev is not sealed
			const result1 = adapter.write({
				items: [{ id: "a", value: 999, isSealed: false }],
			});
			assert.notStrictEqual(
				result1.data.toString(),
				"",
				"non-sealed item should detect value change",
			);
		...
		test("sealed transition from unsealed to sealed detects final changes", () => {
			...
			assert.notStrictEqual(
				result.data.toString(),
				"",
				"transition to sealed should detect value change",
			);
		...
		test("write without confirmWrite resets to initial on next write", () => {
			...
			assert.deepStrictEqual(
				...
				{
					op: "replace",
					entry: { kind: 0, v: { name: "test", count: 2, items: [] } },
				},
			);
		});
		...
		test("read throws on log file missing initial entry", () => {
			...
			assert.throws(
				() => adapter.read(VSBuffer.fromString(logContent)),
				/missing an initial entry/,
			);
		});
	...
	suite("persistence size safety net", () => {
		test("makeTruncatingReplacer truncates an oversized string", () => {
			...
			const json = JSON.stringify(
				obj,
				Adapt.makeTruncatingReplacer(1024 * 1024, 10 * 1024 * 1024),
			);
		...
		test("makeTruncatingReplacer respects total budget without overshooting", () => {
			...
			const json = JSON.stringify(
				obj,
				Adapt.makeTruncatingReplacer(STRING_CAP, TOTAL_CAP),
			);
			const parsed = JSON.parse(json);
			...
			assert.ok(
				preservedChars <= TOTAL_CAP,
				`preserved ${preservedChars} chars exceeded budget ${TOTAL_CAP}`,
			);
			...
			assert.ok(
				Object.values(parsed).some(
					(v) =>
						typeof v === "string" &&
						(v as string).includes("entry exceeded size budget"),
				),
			);
		});
		...
		test("stringifyEntryWithFallback rethrows non-RangeError", () => {
			...
			assert.throws(
				() => Adapt.stringifyEntryWithFallback(circular),
				TypeError,
			);
		});
		...
		test("stringifyEntryWithFallback applies truncating replacer on RangeError retry", () => {
			...
			assert.notStrictEqual(parsed.content, big);
			assert.ok(
				parsed.content.startsWith("[VS Code:"),
				`unexpected: ${parsed.content.slice(0, 80)}`,
			);
			assert.strictEqual(parsed.label, "ok");
		});
	});
