...
suite("ChatQuestionCarouselData", () => {
	...
	test("creates a carousel with DeferredPromise completion", () => {
		...
		assert.strictEqual(
			carousel.completion.isSettled,
			false,
			"Completion should not be settled initially",
		);
	});
	...
	test("completion promise can be resolved with undefined (skipped)", async () => {
		...
		assert.strictEqual(
			result.answers,
			undefined,
			"Skipped carousel should have undefined answers",
		);
	});
	...
	test("toJSON strips the completion promise", () => {
		...
		assert.strictEqual(
			(json as { completion?: unknown }).completion,
			undefined,
			"toJSON should not include completion",
		);
		assert.strictEqual(
			(json as { draftAnswers?: unknown }).draftAnswers,
			undefined,
			"toJSON should not include draftAnswers",
		);
		assert.strictEqual(
			(json as { draftCurrentIndex?: unknown }).draftCurrentIndex,
			undefined,
			"toJSON should not include draftCurrentIndex",
		);
	});

	test("multiple carousels can have independent completion promises", async () => {
		const carousel1 = new ChatQuestionCarouselData(
			createQuestions(),
			true,
			"resolve-1",
		);
		const carousel2 = new ChatQuestionCarouselData(
			createQuestions(),
			true,
			"resolve-2",
		);
		...
		assert.strictEqual(
			carousel1.completion.isSettled,
			false,
			"Carousel 1 should not be settled",
		);
		assert.strictEqual(
			carousel2.completion.isSettled,
			true,
			"Carousel 2 should be settled",
		);
			...
			await runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				try {
					...
					const [result1, result2] = await Promise.all([
						Promise.race([
							carousel1.completion.p,
							timeoutPromise1.then(() => "timeout"),
						]),
						Promise.race([
							carousel2.completion.p,
							timeoutPromise2.then(() => "timeout"),
						]),
					]);
					...
					assert.notStrictEqual(
						result1,
						"timeout",
						"Carousel 1 should not timeout",
					);
					assert.notStrictEqual(
						result2,
						"timeout",
						"Carousel 2 should not timeout",
					);
					assert.deepStrictEqual(
						(result1 as { answers: unknown }).answers,
						undefined,
					);
					assert.deepStrictEqual((result2 as { answers: unknown }).answers, {
						q1: "answer2",
					});
				} finally {
	...
	suite("dismiss", () => {
		...
		test("is a no-op when already dismissed", async () => {
			...
			assert.deepStrictEqual(
				carousel.data,
				{ q1: "first" },
				"Second dismiss should not overwrite data",
			);
			const result = await carousel.completion.p;
