...
import {
	...
	type IChatStreamStatsInternal,
} from "../../../common/model/chatStreamStats.js";
...
suite("ChatStreamStatsTracker", () => {
	...
	test("drops bootstrap once sufficient markdown streamed", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();

				let data = tracker.update({ totalWordCount: 10 }) as
				...
				await timeout(100);
				data = tracker.update({ totalWordCount: 35 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(data);
	...
	test("large initial chunk uses higher bootstrap minimum", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();

				const data = tracker.update({ totalWordCount: 40 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(data);
				assert.strictEqual(data.bootstrapActive, true);
	...
	test("ignores updates without new words", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();
	...
	test("ignores zero-word totals until words arrive", () =>
		runWithFakedTimers<void>(
			...
			async () => {
				...
				await timeout(100);
				const data = tracker.update({ totalWordCount: 12 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(data);
				assert.strictEqual(data.bootstrapActive, true);
	...
	test("unchanged totals do not advance timers", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();

				const first = tracker.update({ totalWordCount: 6 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(first);
				...
				assert.strictEqual(
					tracker.internalData.lastUpdateTime,
					initialLastUpdateTime,
				);
			},
		));

	test("records first markdown time but keeps bootstrap active", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();
	...
	test("implied rate uses elapsed time after bootstrap drops", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();
	...
	test("keeps bootstrap active until both thresholds satisfied", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();
				let data = tracker.update({ totalWordCount: 8 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(data);
				assert.strictEqual(data.bootstrapActive, true);
				...
				await timeout(200);
				data = tracker.update({ totalWordCount: 12 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(data);
				assert.strictEqual(data.bootstrapActive, false);
	...
	test("caps interval contribution to max interval time", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();
				...
				await timeout(2000);
				const data = tracker.update({ totalWordCount: 9 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(data);
				assert.strictEqual(data.bootstrapActive, true);
	...
	test("uses larger interval cap for large updates", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();
				...
				await timeout(200);
				const exitData = tracker.update({ totalWordCount: 40 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(exitData);
				...
				await timeout(2000);
				const postData = tracker.update({ totalWordCount: 90 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(postData);
	...
	test("tracks words since bootstrap exit for rate calculation", () =>
		runWithFakedTimers<void>(
			{ startTime: 0, useFakeTimers: true },
			async () => {
				const tracker = createTracker();
				...
				await timeout(200);
				const exitData = tracker.update({ totalWordCount: 45 }) as
					| IChatStreamStatsInternal
					| undefined;
				assert.ok(exitData);
				assert.strictEqual(exitData.bootstrapActive, false);
				...
				await timeout(200);
				const postBootstrap = tracker.update({ totalWordCount: 60 }) as
					| IChatStreamStatsInternal
					| undefined;
