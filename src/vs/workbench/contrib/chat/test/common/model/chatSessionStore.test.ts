...
import {
	IDidEnterWorkspaceEvent,
	IWorkspaceEditingService,
} from "../../../../../services/workspaces/common/workspaceEditing.js";
import {
...
import {
	ChatSessionStore,
	IChatTransfer,
} from "../../../common/model/chatSessionStore.js";
...
class MockWorkspaceEditingService
	extends Disposable
	implements Partial<IWorkspaceEditingService>
{
	private readonly _onDidEnterWorkspace = this._register(
		new Emitter<IDidEnterWorkspaceEvent>(),
	);
	readonly onDidEnterWorkspace = this._onDidEnterWorkspace.event;

	fireWorkspaceTransition(
...
suite("ChatSessionStore", () => {
	...
	function createChatSessionStore(
		isEmptyWindow: boolean = false,
	): ChatSessionStore {
		const workspace = isEmptyWindow
			? new Workspace("empty-window-id", [])
			: TestWorkspace;
		...
		return testDisposables.add(
			instantiationService.createInstance(ChatSessionStore),
		);
	}

	setup(() => {
		instantiationService = testDisposables.add(
			new TestInstantiationService(new ServiceCollection()),
		);
		instantiationService.stub(
			IStorageService,
		...
		instantiationService.stub(
			IFileService,
			testDisposables.add(new InMemoryTestFileService()),
		);
		instantiationService.stub(IEnvironmentService, {
			workspaceStorageHome: URI.file("/test/workspaceStorage"),
		});
		instantiationService.stub(
			ILifecycleService,
			testDisposables.add(new TestLifecycleService()),
		);
		...
		mockWorkspaceEditingService = testDisposables.add(
			new MockWorkspaceEditingService(),
		);
		instantiationService.stub(
			IWorkspaceEditingService,
			mockWorkspaceEditingService as unknown as IWorkspaceEditingService,
		);
	});
	...
	test("storeSessions persists session to index", async () => {
		...
		const model = testDisposables.add(
			createMockChatModel(LocalChatSessionUri.forSession("session-1")),
		);
	...
	test("readSession returns stored session data", async () => {
		...
		assert.ok(session);
		assert.strictEqual(
			(session.value as ISerializableChatData3).sessionId,
			"session-1",
		);
	});
	...
	test("clearAllSessions removes all sessions", async () => {
		...
		const model2 = testDisposables.add(
			createMockChatModel(LocalChatSessionUri.forSession("session-2")),
		);
	...
	suite("transferred sessions", () => {
		...
		function createChatSessionStoreWithSingleFolder(
			folderUri: URI,
		): ChatSessionStore {
			instantiationService.stub(
				IWorkspaceContextService,
				new TestContextService(createSingleFolderWorkspace(folderUri)),
			);
			return testDisposables.add(
				instantiationService.createInstance(ChatSessionStore),
			);
		...
		test("storeTransferSession stores and retrieves transfer data", async () => {
			...
			const sessionResource =
				LocalChatSessionUri.forSession("transfer-session");
		...
		test("readTransferredSession returns session data", async () => {
			...
			assert.strictEqual(
				(sessionData.value as ISerializableChatData3).sessionId,
				"transfer-session",
			);
		});
		...
		test("storeTransferSession deletes preexisting transferred session file", async () => {
			...
			// Store first session
			const session1Resource =
				LocalChatSessionUri.forSession("transfer-session-1");
			...
			const userDataProfile = instantiationService.get(
				IUserDataProfilesService,
			).defaultProfile;
			...
			const session2Resource =
				LocalChatSessionUri.forSession("transfer-session-2");
			...
			assert.strictEqual(
				exists1After,
				false,
				"First session file should be deleted",
			);
	...
	suite("workspace migration", () => {
		test("migration is triggered when onDidEnterWorkspace fires", async () => {
			const fileService = instantiationService.get(
				IFileService,
			) as InMemoryTestFileService;
			...
			const sessionFile = URI.joinPath(
				emptyWindowStorageRoot,
				"session-1.json",
			);
			...
			assert.strictEqual(
				fileExists,
				true,
				"Session file should exist in empty window storage",
			);
			...
			const newWorkspace: IAnyWorkspaceIdentifier = {
				id: TestWorkspace.id,
				uri: URI.file("/test/folder"),
			};

			// Fire the workspace transition event - migration happens synchronously via join()
			await mockWorkspaceEditingService.fireWorkspaceTransition(
				oldWorkspace,
				newWorkspace,
			);
			...
			const migratedSessionFile = URI.joinPath(
				newStorageRoot,
				"session-1.json",
			);
			...
			assert.strictEqual(
				migratedFileExists,
				true,
				"Session file should be migrated to workspace storage",
			);
		});
		...
		test("migration handles non-existent old storage location gracefully", async () => {
			...
			const oldWorkspace: IAnyWorkspaceIdentifier = {
		...
		test("storage root is updated after workspace transition", async () => {
			...
			assert.ok(
				initialStorageRoot.path.includes("emptyWindowChatSessions"),
				"Initial storage should be empty window location",
			);
			...
			const newStorageRoot = store.getChatStorageFolder();
			assert.ok(
				newStorageRoot.path.includes("new-workspace-id"),
				"Storage root should be updated to new workspace location",
			);
		});
	});
});
