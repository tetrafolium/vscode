...
} from "../../../../common/tools/builtinTools/confirmationTool.js";
...
suite("ModifiedFilesConfirmationTool", () => {
	...
	test("tool data exposes the expected schema", () => {
		assert.strictEqual(
			ModifiedFilesConfirmationToolData.id,
			"vscode_get_modified_files_confirmation",
		);
		assert.ok(ModifiedFilesConfirmationToolData.inputSchema);
		assert.deepStrictEqual(
			ModifiedFilesConfirmationToolData.inputSchema?.required,
			["title", "message", "options", "modifiedFiles"],
		);
		assert.ok(
			ModifiedFilesConfirmationToolData.inputSchema?.properties?.options,
		);
		assert.ok(
			ModifiedFilesConfirmationToolData.inputSchema?.properties?.modifiedFiles,
		);
	});
	...
	test("prepareToolInvocation parses file data and disables auto confirm", async () => {
		...
		assert.strictEqual(
			result?.toolSpecificData?.kind,
			"modifiedFilesConfirmation",
		);

		assert.deepStrictEqual(result.toolSpecificData.options, [
			"Copy Changes",
			"Move Changes",
		]);
		assert.strictEqual(
			URI.revive(result.toolSpecificData.modifiedFiles[0].uri).toString(),
			"file:///workspace/src/file1.ts",
		);
		assert.strictEqual(
			result.toolSpecificData.modifiedFiles[0].originalUri
				? URI.revive(
						result.toolSpecificData.modifiedFiles[0].originalUri,
					).toString()
				: undefined,
			"file:///workspace/src/file1.original.ts",
		);
