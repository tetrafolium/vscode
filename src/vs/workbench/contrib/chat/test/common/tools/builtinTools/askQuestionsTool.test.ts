...
import {
	...
	IQuestion,
	IQuestionAnswer,
} from "../../../../common/tools/builtinTools/askQuestionsTool.js";
import { ChatQuestionCarouselData } from "../../../../common/model/chatProgressTypes/chatQuestionCarouselData.js";

class TestableAskQuestionsTool extends AskQuestionsTool {
	public testConvertCarouselAnswers(
		questions: IQuestion[],
		...
		return this.convertCarouselAnswers(
			questions,
			carouselAnswers,
			idToHeaderMap,
		);
...
suite("AskQuestionsTool - convertCarouselAnswers", () => {
	...
	test("handles string answers as option selection or free text", () => {
		...
		assert.deepStrictEqual(result.answers["Color"], {
			selected: ["Blue"],
			freeText: null,
			skipped: false,
		});
		assert.deepStrictEqual(result.answers["Comment"], {
			selected: [],
			freeText: "Nice",
			skipped: false,
		});
	...
	test("handles array answers for multi-select", () => {
		...
		const result = tool.testConvertCarouselAnswers(questions, {
			Features: { selectedValues: ["A", "B"] },
		});

		assert.deepStrictEqual(result.answers["Features"], {
			selected: ["A", "B"],
			freeText: null,
			skipped: false,
		});
	...
	test("handles selectedValue object answers", () => {
		...
		assert.deepStrictEqual(result.answers["Range"], {
			selected: ["Yes"],
			freeText: null,
			skipped: false,
		});
		assert.deepStrictEqual(result.answers["Feedback"], {
			selected: [],
			freeText: "Great!",
			skipped: false,
		});
	...
	test("handles selectedValues object answers", () => {
		...
		const result = tool.testConvertCarouselAnswers(questions, {
			Options: { selectedValues: ["X"] },
		});

		assert.deepStrictEqual(result.answers["Options"], {
			selected: ["X"],
			freeText: null,
			skipped: false,
		});
	...
	test("handles freeformValue with no selection", () => {
		...
		const result = tool.testConvertCarouselAnswers(questions, {
			Choice: { freeformValue: "Custom" },
		});

		assert.deepStrictEqual(result.answers["Choice"], {
			selected: [],
			freeText: "Custom",
			skipped: false,
		});
	...
	test("marks unknown formats as skipped", () => {
		const questions: IQuestion[] = [{ header: "Odd", question: "Unknown" }];

		const result = tool.testConvertCarouselAnswers(questions, {
			Odd: 42 as unknown as object,
		});

		assert.deepStrictEqual(result.answers["Odd"], {
			selected: [],
			freeText: null,
			skipped: true,
		});
	...
	test("is case-sensitive when matching options", () => {
		...
		assert.deepStrictEqual(result.answers["Case"], {
			selected: [],
			freeText: "yes",
			skipped: false,
		});
...
suite("AskQuestionsTool - invoke", () => {
	...
	test("marks the carousel used when invocation is cancelled after it is shown", async () => {
		...
		const chatService = {
			...
			appendProgress: (
				_request: unknown,
				progress: ChatQuestionCarouselData,
			) => {
		...
		const invokePromise = tool.invoke(
			{
				parameters: {
					questions: [
						{
							header: "Theme",
							question: "What is your favorite theme in VS Code?",
						},
					],
				},
		...
		assert.ok(
			appendedCarousel,
			"expected question carousel to be appended before cancellation",
		);
		tokenSource.cancel();
		...
		assert.ok(
			appendedCarousel,
			"expected appended carousel to remain available after cancellation",
		);
		...
		assert.deepStrictEqual(appendedCarousel.completion.value, {
			answers: undefined,
		});
	...
	test("uses externally notified answers instead of showing skipped", async () => {
		...
		const chatService = {
			...
			onDidReceiveQuestionCarouselAnswer:
				onDidReceiveQuestionCarouselAnswer.event,
		} as unknown as IChatService;
		const tool = store.add(
			new AskQuestionsTool(
				chatService,
		...
		assert.ok(
			appendedCarousel,
			"expected question carousel to be appended before external answer",
		);
