...
suite("RunSubagentTool", () => {
	...
	suite("prepareToolInvocation", () => {
		test("returns correct toolSpecificData", async () => {
			const mockToolsService = testDisposables.add(
			...
			const tool = testDisposables.add(
				new RunSubagentTool(
		...
		function createToolWithGP(opts?: { customAgents?: ICustomAgent[] }) {
			...
			const tool = testDisposables.add(
				new RunSubagentTool(
					...
					new NullLogService(),
					new TestConfigurationService({
						[ChatConfiguration.GeneralPurposeAgentEnabled]: true,
					}),
					promptsService,
					{} as IInstantiationService,
					{} as IProductService,
	...
	suite("getToolData", () => {
		test("returns basic tool data", () => {
			...
			assert.ok(
				toolData.inputSchema.properties?.agentName,
				"agentName should be in schema properties",
			);
			assert.deepStrictEqual(toolData.inputSchema.required, [
				"prompt",
				"description",
			]);
		});
	...
	suite("onDidInvokeTool event", () => {
		test("mock service fires onDidInvokeTool events with correct data", () => {
			...
			testDisposables.add(
				mockToolsService.onDidInvokeTool((e) => {
	...
	suite("model fallback behavior", () => {
		...
		test("throws error when subagent model has higher multiplier", async () => {
			...
			const qualifiedNameMap = new Map([
				[
					"O3 Pro (TestVendor)",
					{ metadata: expensiveMeta, identifier: "expensive-model-id" },
				],
			]);
		...
		test("uses subagent model when it has equal multiplier", async () => {
			...
			const qualifiedNameMap = new Map([
				[
					...
					{ metadata: sameCostMeta, identifier: "same-cost-model-id" },
				],
			]);

			const agent = createAgent("SameCostAgent", [
				"Claude Sonnet (TestVendor)",
			]);
			const tool = createTool({
				models,
		...
		test("uses subagent model when it has lower multiplier", async () => {
			...
			const qualifiedNameMap = new Map([
				[
					"GPT-4o Mini (TestVendor)",
					{ metadata: cheapMeta, identifier: "cheap-model-id" },
				],
			]);
		...
		test("uses subagent model when main model has no multiplier", async () => {
			...
			const qualifiedNameMap = new Map([
				[
					"O3 Pro (TestVendor)",
					{ metadata: subMeta, identifier: "sub-model-id" },
				],
			]);
	...
	suite("explicit model parameter", () => {
		...
		test("model property is included in tool schema without enum", () => {
			...
			assert.ok(
				toolData.inputSchema?.properties?.model,
				"model should be in schema",
			);
			assert.strictEqual(
				toolData.inputSchema?.properties?.model?.type,
				"string",
			);
			// No enum should be present - validation happens at runtime
			assert.strictEqual(
				toolData.inputSchema?.properties?.model?.enum,
				undefined,
				"model should not have an enum",
			);
		});
		...
		test("resolves explicit model parameter without agentName", async () => {
			...
			const qualifiedNameMap = new Map([
				[
					"Claude Sonnet (TestVendor)",
					{ metadata: explicitMeta, identifier: "explicit-model-id" },
				],
			]);
		...
		test("explicit model overrides agent configured model", async () => {
			...
			const qualifiedNameMap = new Map([
				[
					"Agent Model (TestVendor)",
					{ metadata: agentMeta, identifier: "agent-model-id" },
				],
				[
					"Claude Sonnet (TestVendor)",
					{ metadata: explicitMeta, identifier: "explicit-model-id" },
					...
					tool.prepareToolInvocation(
						{
							parameters: {
								prompt: "test",
								description: "test task",
								model: "Nonexistent Model (Vendor)",
							},
	...
	suite("nested subagent depth tracking", () => {
		...
		function createInvokableTool(opts: {
			...
			const configService = new TestConfigurationService({
				[ChatConfiguration.SubagentsAllowInvocationsFromSubagents]:
					opts.allowInvocationsFromSubagents,
			});
			const promptsService = new MockPromptsService();

			const mockChatAgentService: Pick<
				IChatAgentService,
				"getDefaultAgent" | "invokeAgent"
			> = {
				getDefaultAgent() {
					return { id: "default-agent" } as IChatAgentService extends {
						getDefaultAgent(...args: infer _A): infer R;
					}
						? NonNullable<R>
						: never;
				},
			...
			const mockInstantiationService: Pick<
		...
		test("disables runSubagent tool when nesting is disabled", async () => {
			...
			assert.strictEqual(
				capturedRequests[0].userSelectedTools?.["runSubagent"],
				false,
			);
		});
		...
		test("disables runSubagent tool when depth reaches hard limit", async () => {
			...
			const { tool, mockChatAgentService } = createInvokableTool({
				allowInvocationsFromSubagents: true,
				capturedRequests,
			});
			...
			mockChatAgentService.invokeAgent = async (
				_id: string,
				request: IChatAgentRequest,
			) => {
			...
			const enabledFlags = capturedRequests.map(
				(r) => r.userSelectedTools?.["runSubagent"],
			);
		...
		test("depth is decremented after invoke completes", async () => {
			...
			assert.strictEqual(
				capturedRequests[1].userSelectedTools?.["runSubagent"],
				true,
			);
		});
	});
