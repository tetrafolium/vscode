...
import {
	...
	ManageTodoListTool,
} from "../../../../common/tools/builtinTools/manageTodoListTool.js";
import {
	IChatTodo,
...
suite("ManageTodoListTool Schema", () => {
	...
	function getSchemaProperties(toolData: IToolData): {
		properties: Record<string, IJSONSchema>;
		required: string[];
	} {
		assert.ok(toolData.inputSchema);
		const schema = toolData.inputSchema;
		const todolistItems = schema?.properties?.todoList?.items as
			| IJSONSchema
			| undefined;
		const properties = todolistItems?.properties as
	...
	test("createManageTodoListToolData schema has required todoList field", () => {
		...
		assert.ok(
			toolData.inputSchema?.required?.includes("todoList"),
			"todoList should be required",
		);
		assert.ok(
			toolData.inputSchema?.properties?.todoList,
			"todoList property should exist",
		);
	});
	...
	test("createManageTodoListToolData todoList items have correct required fields", () => {
		...
		assert.deepStrictEqual(
			required,
			["id", "title", "status"],
			"Required fields should be id, title, status",
		);
	});
	...
	test("createManageTodoListToolData status has correct enum values", () => {
		...
		assert.deepStrictEqual(
			...
			"Status should have correct enum values",
		);
	});
});
...
suite("ManageTodoListTool prepareToolInvocation", () => {
	...
	function createMockTodoListService(
		todos: IChatTodo[] = [],
	): IChatTodoListService {
		return {
	...
	test("presentation is undefined for read operation with existing todos", async () => {
		const tool = createTool([
			{ id: 1, title: "Existing", status: "in-progress" },
		]);
		const result = await tool.prepareToolInvocation(
			{
				parameters: { operation: "read" },
