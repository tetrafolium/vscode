...
function makeGenericEvent(
	overrides: Partial<IChatDebugGenericEvent> = {},
): IChatDebugGenericEvent {
	return {
		kind: "generic",
		sessionResource,
...
function makeToolCallEvent(
	overrides: Partial<IChatDebugToolCallEvent> = {},
): IChatDebugToolCallEvent {
	return {
		kind: "toolCall",
		sessionResource,
...
function makeModelTurnEvent(
	overrides: Partial<IChatDebugModelTurnEvent> = {},
): IChatDebugModelTurnEvent {
	return {
		kind: "modelTurn",
		sessionResource,
...
function makeSubagentEvent(
	overrides: Partial<IChatDebugSubagentInvocationEvent> = {},
): IChatDebugSubagentInvocationEvent {
	return {
...
function makeUserMessageEvent(
	overrides: Partial<IChatDebugUserMessageEvent> = {},
): IChatDebugUserMessageEvent {
	return {
		kind: "userMessage",
		sessionResource,
...
function makeAgentResponseEvent(
	overrides: Partial<IChatDebugAgentResponseEvent> = {},
): IChatDebugAgentResponseEvent {
	return {
		kind: "agentResponse",
		sessionResource,
...
suite("chatDebugEvents", () => {
	...
	suite("debugEventMatchesText", () => {
		test("matches event kind", () => {
			assert.strictEqual(
				debugEventMatchesText(makeToolCallEvent(), "toolcall"),
				true,
			);
			assert.strictEqual(
				debugEventMatchesText(makeToolCallEvent(), "generic"),
				false,
			);
		});
		...
		test("matches toolCall tool name", () => {
			assert.strictEqual(
				debugEventMatchesText(
					makeToolCallEvent({ toolName: "readFile" }),
					"readfile",
				),
				true,
			);
		...
		test("matches modelTurn model and requestName", () => {
			assert.strictEqual(
				debugEventMatchesText(
					makeModelTurnEvent({ model: "gpt-4o" }),
					"gpt-4o",
				),
				true,
			);
			assert.strictEqual(
				debugEventMatchesText(
					makeModelTurnEvent({ requestName: "chat-request" }),
					"chat-request",
				),
				true,
			);
		...
		test("matches generic event name, details, and category", () => {
			const event = makeGenericEvent({
				name: "discovery",
				details: "loaded 5 files",
	...
	suite("parseTimeToken", () => {
		...
		test("parses year-month before token", () => {
			...
			assert.strictEqual(
				result,
				new Date(2026, 3, 0, 23, 59, 59, 999).getTime(),
			);
		});
	...
	suite("stripTimestampTokens", () => {
		...
		test("strips after token", () => {
			assert.strictEqual(
				stripTimestampTokens("after:2026-03-10 hello"),
				"hello",
			);
		});
		...
		test("strips both tokens", () => {
			assert.strictEqual(
				stripTimestampTokens("after:2026-03 before:2026-04 hello"),
				"hello",
			);
		});
	...
	suite("filterDebugEventsByText", () => {
		...
		const events: readonly IChatDebugEvent[] = [
			makeGenericEvent({
				name: "discovery",
				category: "instructions",
				created: new Date(2026, 2, 10, 10, 0, 0),
			}),
			makeToolCallEvent({
				toolName: "readFile",
				created: new Date(2026, 2, 10, 11, 0, 0),
			}),
			makeToolCallEvent({
				toolName: "writeFile",
				created: new Date(2026, 2, 10, 12, 0, 0),
			}),
			makeModelTurnEvent({
				model: "gpt-4o",
				created: new Date(2026, 2, 10, 13, 0, 0),
			}),
		];
		...
		test("filters by inclusion term", () => {
			...
			assert.strictEqual(
				(result[0] as IChatDebugToolCallEvent).toolName,
				"readFile",
			);
		});
		...
		test("combines timestamp and text filters", () => {
			const result = filterDebugEventsByText(
				events,
				"after:2026-03-10t11 toolcall",
			);
	...
	suite("filterDebugEvents", () => {
		const events: readonly IChatDebugEvent[] = [
			makeGenericEvent({
				name: "event-1",
				created: new Date("2026-03-10T10:00:00Z"),
			}),
			makeToolCallEvent({
				toolName: "readFile",
				created: new Date("2026-03-10T11:00:00Z"),
			}),
			makeToolCallEvent({
				toolName: "writeFile",
				created: new Date("2026-03-10T12:00:00Z"),
			...
			makeModelTurnEvent({
				model: "gpt-4o",
				created: new Date("2026-03-10T13:00:00Z"),
			}),
			makeSubagentEvent({
				agentName: "explorer",
				created: new Date("2026-03-10T14:00:00Z"),
			}),
		];
		...
		test("combines kind and text filters", () => {
			const result = filterDebugEvents(events, {
				kind: "toolCall",
				filter: "readfile",
			});
			assert.strictEqual(result.length, 1);
		...
		test("combines all three filters", () => {
			...
			const result = filterDebugEvents(allToolCalls, {
				kind: "toolCall",
				filter: "!readfile",
				limit: 1,
			});
