...
suite("promptFileLocations", function () {
	...
	suite("getPromptFileType", () => {
		...
		test("any .json file should be recognized as hook", () => {
			assert.strictEqual(
				getPromptFileType(URI.file("/workspace/.github/hooks/hooks.json")),
				PromptsType.hook,
			);
			assert.strictEqual(
				getPromptFileType(
					URI.file("/workspace/.github/hooks/custom-hooks.json"),
				),
				PromptsType.hook,
			);
			assert.strictEqual(
				getPromptFileType(URI.file("/workspace/.claude/settings.json")),
				PromptsType.hook,
			);
			assert.strictEqual(
				getPromptFileType(URI.file("/workspace/.claude/settings.local.json")),
				PromptsType.hook,
			);
			assert.strictEqual(
				getPromptFileType(URI.file("/workspace/any/path/config.json")),
				PromptsType.hook,
			);
		});
	...
	suite("isPromptOrInstructionsFile", () => {
		test("SKILL.md files should return true", () => {
			assert.strictEqual(
				isPromptOrInstructionsFile(
					URI.file("/workspace/.github/skills/test/SKILL.md"),
				),
				true,
			);
		});
		...
		test("skill.md (lowercase) should return true", () => {
			assert.strictEqual(
				isPromptOrInstructionsFile(
					URI.file("/workspace/.claude/skills/myskill/skill.md"),
				),
				true,
			);
		});
		...
		test("Skill.md (mixed case) should return true", () => {
			assert.strictEqual(
				isPromptOrInstructionsFile(URI.file("/workspace/skills/Skill.md")),
				true,
			);
		});
		...
		test("regular .md files should return false", () => {
			assert.strictEqual(
				isPromptOrInstructionsFile(URI.file("/workspace/SKILL2.md")),
				false,
			);
		});
		...
		test("any .json file should return true", () => {
			...
			assert.strictEqual(
				isPromptOrInstructionsFile(
					URI.file("/workspace/.claude/settings.json"),
				),
				true,
			);
			assert.strictEqual(
				isPromptOrInstructionsFile(
					URI.file("/workspace/.claude/settings.local.json"),
				),
				true,
			);
			assert.strictEqual(
				isPromptOrInstructionsFile(URI.file("/workspace/settings.json")),
				true,
			);
		});
	});
