...
suite("PromptFilesLocator", () => {
	...
	setup(async () => {
		...
		instantiationService.stub(
			IConfigurationService,
			mockConfigService(configValues),
		);
	...
	suite("empty workspace", () => {
		...
		suite("empty filesystem", () => {
			...
			testT("array config value", async () => {
				setLocations(["relative/path/to/prompts/", "/abs/path"]);
				setWorkspaceFolders(EMPTY_WORKSPACE);
				await mockFiles(fileService, []);
				const locator = instantiationService.createInstance(PromptFilesLocator);
			...
			suite("absolute", () => {
				testT("wild card", async () => {
					...
					for (const setting of settings) {
						...
						const locator =
							instantiationService.createInstance(PromptFilesLocator);
	...
	suite("instructions", () => {
		testT(
			"finds instructions files in subdirectories of .github/instructions",
			async () => {
				setLocations({
	...
	suite("skills", () => {
		suite("findAgentSkills", () => {
			...
			testT(
				"returns empty array when skill folder does not exist",
				async () => {
					setLocations({
						".claude/skills": true,
						// disable other defaults
		...
		suite("listFiles with PromptsType.skill", () => {
			testT("does not list skills when location is disabled", async () => {
				...
				const files = await locator.listFiles(
					PromptsType.skill,
					PromptsStorage.local,
					CancellationToken.None,
				);
		...
		suite("toAbsoluteLocationsForSkills path validation", () => {
			testT(
				"rejects glob patterns in skill paths via getConfigBasedSourceFolders",
				async () => {
					setLocations({
						"skills/**": true,
						"skills/*": true,
					...
					const folders = await locator.getConfigBasedSourceFolders(
						PromptsType.skill,
					);
					assertOutcome(
						folders,
			...
			testT(
				"rejects absolute paths in skill paths via getConfigBasedSourceFolders",
				async () => {
					setLocations({
			...
			testT(
				"accepts relative paths in skill paths via getConfigBasedSourceFolders",
				async () => {
					setLocations({
						"./my-skills": true,
						"custom/skills": true,
			...
			testT(
				"accepts parent relative paths for monorepos via getConfigBasedSourceFolders",
				async () => {
					setLocations({
						"../shared-skills": true,
						// disable defaults
	...
	suite("hasGlobPattern", () => {
		testT("detects single wildcard", async () => {
			const pathsWithGlob = ["skills/*", "my-skills/*", "*.md", "*/folder"];
		...
		testT("detects double wildcard", async () => {
			const pathsWithGlob = ["skills/**", "**/skills", "**/*.md", "a/**/b"];
	...
	suite("findAgentMDsInWorkspace", () => {
		testT("finds AGENTS.md files using FileSearchProvider", async () => {
			...
			const result = (
				await locator.findAgentMDsInWorkspace(CancellationToken.None)
			).map((f) => f.uri);
			assertOutcome(
				result,
				[
		...
		testT("handles cancellation token in file service fallback", async () => {
			...
			// Cancel immediately
			source.cancel();
			const result = (await locator.findAgentMDsInWorkspace(source.token)).map(
				(f) => f.uri,
			);
	...
	suite("getWorkspaceFolderRoots", () => {
		...
		testT("returns only workspace folder when it has .git", async () => {
			...
			await mockFiles(fileService, [
				{
					path: "/repos/my-project/.git/HEAD",
		...
		testT(
			"walks up to parent with .git when workspace folder has no .git",
			async () => {
				setWorkspaceFoldersForRoots(["/repos/monorepo/packages/my-app"]);
				await mockFiles(fileService, [
					{
						path: "/repos/monorepo/.git/HEAD",
						contents: ["ref: refs/heads/main"],
					},
					{
						path: "/repos/monorepo/packages/my-app/src/index.ts",
						contents: ["export {};"],
					},
				]);
		...
		testT("excludes vscode-agent-host workspace folders", async () => {
			...
			const agentHostFolder = URI.from({
				scheme: "vscode-agent-host",
				authority: "remote",
				path: "/repos/remote-project",
			});
			const folders = [localFolder, agentHostFolder].map(
				(uri, index) =>
					new (class extends mock<IWorkspaceFolder>() {
						override uri = uri;
						override name = basename(uri);
						override index = index;
					})(),
			...
			instantiationService.stub(
				IWorkspaceContextService,
				mockWorkspaceService(folders),
			);
			locator = instantiationService.createInstance(PromptFilesLocator);
			await mockFiles(fileService, [
				{
					path: "/repos/local-project/.git/HEAD",
					contents: ["ref: refs/heads/main"],
				},
			]);
		...
		testT("returns only workspace folder when no .git is found", async () => {
			...
			await mockFiles(fileService, [
				{
					path: "/Users/legomushroom/my-project/src/index.ts",
					contents: ["export {};"],
				},
			]);
	...
	suite("getHookSourceFolders", () => {
		testT("returns source metadata for hook folders", async () => {
			...
			assert.deepStrictEqual(
				...
				[
					{
						path: "/Users/legomushroom/repos/vscode/.github/hooks",
						source: PromptFileSource.GitHubWorkspace,
						storage: PromptsStorage.local,
					},
					{
						path: "/Users/legomushroom/.copilot/hooks",
		...
		testT("excludes Claude paths", async () => {
			...
			assert.ok(
				!paths.some((p) => p.includes(".claude")),
				"Claude paths must be excluded",
			);
	...
	suite("listFiles with PromptsType.hook", () => {
		testT(
			"only returns targeted json files, not sibling json files",
			async () => {
				configValues[PromptsConfig.HOOKS_LOCATION_KEY] = {
					".claude/settings.json": true,
					".claude/settings.local.json": true,
				...
				await mockFiles(fileService, [
					...
					{
						path: "/Users/legomushroom/repos/vscode/.claude/settings.json",
						contents: ["{}"],
					},
					{
						path: "/Users/legomushroom/repos/vscode/.claude/settings.local.json",
						contents: ["{}"],
					},
					// sibling files in .claude/ that should NOT be found
					{
						path: "/Users/legomushroom/repos/vscode/.claude/config.json",
						contents: ["{}"],
					},
					{
						path: "/Users/legomushroom/repos/vscode/.claude/stats-cache.json",
						contents: ["{}"],
					},
					// hook directory files that should be found
					{
						path: "/Users/legomushroom/repos/vscode/.github/hooks/pre-commit.json",
						contents: ["{}"],
					},
				]);
				const locator = instantiationService.createInstance(PromptFilesLocator);
		...
		testT("returns hook files from user home specific json paths", async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: "/Users/legomushroom/.claude/stats-cache.json",
					contents: ["{}"],
				},
				// hook directory files
				{
					path: "/Users/legomushroom/.copilot/hooks/my-hook.json",
					contents: ["{}"],
				},
			]);
			const locator = instantiationService.createInstance(PromptFilesLocator);
	...
	suite("getSourceDescription", () => {
		test("returns descriptions for all known folder sources", () => {
			...
			for (const source of folderSources) {
				...
				assert.ok(
					...
					`Expected a description for ${source}`,
				);
			}
		});
		...
		test("returns undefined for extension/plugin sources", () => {
			assert.strictEqual(
				getSourceDescription(PromptFileSource.ExtensionContribution),
				undefined,
			);
			assert.strictEqual(
				getSourceDescription(PromptFileSource.ExtensionAPI),
				undefined,
			);
			assert.strictEqual(
				getSourceDescription(PromptFileSource.Plugin),
				undefined,
			);
		});
	});
