...
suite("HookClaudeCompat", () => {
	...
	suite("extractHookCommandsFromItem", () => {
		...
		test("handles nested command without type field", () => {
			const item = {
				matcher: "Bash",
				hooks: [{ command: 'echo "no type nested"' }],
			};
		...
		test("returns empty array for undefined item", () => {
			const result = extractHookCommandsFromItem(
				undefined,
				workspaceRoot,
				userHome,
			);
	...
	suite("resolveClaudeHookType", () => {
		test("resolves PreToolUse", () => {
			assert.strictEqual(
				resolveClaudeHookType("PreToolUse"),
				HookType.PreToolUse,
			);
		});
		...
		test("resolves UserPromptSubmit", () => {
			assert.strictEqual(
				resolveClaudeHookType("UserPromptSubmit"),
	...
	suite("getClaudeHookTypeName", () => {
		test("gets PreToolUse for HookType.PreToolUse", () => {
			assert.strictEqual(
				getClaudeHookTypeName(HookType.PreToolUse),
				"PreToolUse",
			);
		});
		...
		test("gets UserPromptSubmit for HookType.UserPromptSubmit", () => {
			assert.strictEqual(
				getClaudeHookTypeName(HookType.UserPromptSubmit),
				"UserPromptSubmit",
			);
		});
	});
...
suite("HookSourceFormat", () => {
	...
	suite("getHookSourceFormat", () => {
		test("detects Claude format for .claude/settings.json", () => {
			assert.strictEqual(
				getHookSourceFormat(URI.file("/workspace/.claude/settings.json")),
				HookSourceFormat.Claude,
			);
		});

		test("detects Claude format for .claude/settings.local.json", () => {
			assert.strictEqual(
				getHookSourceFormat(URI.file("/workspace/.claude/settings.local.json")),
				HookSourceFormat.Claude,
			);
		});
		...
		test("detects Claude format for ~/.claude/settings.json", () => {
			assert.strictEqual(
				getHookSourceFormat(URI.file("/home/user/.claude/settings.json")),
				HookSourceFormat.Claude,
			);
		});
		...
		test("returns Copilot format for .github/hooks/hooks.json", () => {
			assert.strictEqual(
				getHookSourceFormat(URI.file("/workspace/.github/hooks/hooks.json")),
				HookSourceFormat.Copilot,
			);
		});

		test("returns Copilot format for arbitrary .json file", () => {
			assert.strictEqual(
				getHookSourceFormat(URI.file("/workspace/.github/hooks/my-hooks.json")),
				HookSourceFormat.Copilot,
			);
		});
		...
		test("returns Copilot format for settings.json not inside .claude", () => {
			assert.strictEqual(
				getHookSourceFormat(URI.file("/workspace/.vscode/settings.json")),
				HookSourceFormat.Copilot,
			);
		});
	});
