...
suite("HookSchema", () => {
	...
	suite("resolveEffectiveCommand", () => {
		test("returns base command when no platform override", () => {
			...
			assert.strictEqual(
				resolveEffectiveCommand(hook, OperatingSystem.Windows),
				"default-command",
			);
			assert.strictEqual(
				resolveEffectiveCommand(hook, OperatingSystem.Macintosh),
				"default-command",
			);
			assert.strictEqual(
				resolveEffectiveCommand(hook, OperatingSystem.Linux),
				"default-command",
			);
		});
	...
	suite("formatHookCommandLabel", () => {
		test("formats command when present (no platform override)", () => {
			...
			assert.strictEqual(
				formatHookCommandLabel(hook, OperatingSystem.Windows),
				"echo hello",
			);
			assert.strictEqual(
				formatHookCommandLabel(hook, OperatingSystem.Macintosh),
				"echo hello",
			);
			assert.strictEqual(
				formatHookCommandLabel(hook, OperatingSystem.Linux),
				"echo hello",
			);
		});
	...
	suite("parseSubagentHooksFromYaml", () => {
		...
		function makeScalar(
			value: string,
		): import("../../../common/promptSyntax/promptFileParser.js").IScalarValue {
		...
		function makeMap(
			entries: Record<
				string,
				import("../../../common/promptSyntax/promptFileParser.js").IValue
			>,
		): import("../../../common/promptSyntax/promptFileParser.js").IMapValue {
		...
		function makeSequence(
			items: import("../../../common/promptSyntax/promptFileParser.js").IValue[],
		): import("../../../common/promptSyntax/promptFileParser.js").ISequenceValue {
		...
		test("parses direct command format (without matcher)", () => {
			...
			const result = parseSubagentHooksFromYaml(
				hooksMap,
				workspaceRoot,
				userHome,
			);
			...
			assert.strictEqual(
				result[HookType.PreToolUse]![0].command,
				"./scripts/validate.sh",
			);
		});
		...
		test("parses multiple hook types", () => {
			...
			assert.strictEqual(
				result[HookType.PostToolUse]![0].command,
				"./scripts/lint.sh",
			);
		});
		...
		test("resolves cwd relative to workspace", () => {
			...
			assert.deepStrictEqual(
				result[HookType.SessionStart]![0].cwd,
