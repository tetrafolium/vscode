...
class TestPromptFileReference extends Disposable {
	constructor(
		...
		this._register(
			this.fileService.registerProvider("testFs", fileSystemProvider),
		);
	}
	...
	public async run(): Promise<any> {
		...
		const mockFs = this.instantiationService.createInstance(
			MockFilesystem,
			this.fileStructure,
		);
...
suite("PromptFileReference", function () {
	...
	setup(async () => {
		...
		const nullFileService = testDisposables.add(
			new FileService(nullLogService),
		);
