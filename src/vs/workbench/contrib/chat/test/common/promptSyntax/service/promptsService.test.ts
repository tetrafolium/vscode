...
} from "../../../../common/promptSyntax/computeAutomaticInstructions.js";
...
class TestPromptContextKeyService extends MockContextKeyService {
	private readonly _onDidChangeContextEmitter =
		new Emitter<IContextKeyChangeEvent>();
	...
	fireDidChangeContext(keys: string[]): void {
		...
		this._onDidChangeContextEmitter.fire({
			...
			allKeysContainedIn: (trackedKeys) =>
				Array.from(changedKeys).every((key) => trackedKeys.has(key)),
		});
	}
...
suite("PromptsService", () => {
	...
	setup(async () => {
		...
		testConfigService.setUserConfiguration(PromptsConfig.MODE_LOCATION_KEY, {
		...
		instaService.stub(
			...
			new TestUserDataProfileService(),
		);
		...
		const modelService = disposables.add(
			instaService.createInstance(ModelService),
		);
		...
		instaService.stub(IFilesConfigurationService, {
			updateReadonly: () => Promise.resolve(),
		});
	...
	suite("parse", () => {
		test("explicit", async function () {
			...
			const file4 = URI.joinPath(
				rootFolderUri,
				"folder1/some-other-folder/file4.prompt.md",
			);
			const someOtherFolder = URI.joinPath(
				rootFolderUri,
				"/folder1/some-other-folder",
			);
			const someOtherFolderFile = URI.joinPath(
				rootFolderUri,
				"/folder1/some-other-folder/file.txt",
			);
			const nonExistingFolder = URI.joinPath(
				rootFolderUri,
				"folder1/some-other-folder/non-existing-folder",
			);
			const yetAnotherFile = URI.joinPath(
				rootFolderUri,
				"folder1/some-other-folder/yetAnotherFolder🤭/another-file.instructions.md",
			);

			const result1 = await service.parseNew(
				rootFileUri,
			...
			assert.deepEqual(
				result1.body.fileReferences.map((r) =>
					result1.body?.resolveFilePath(r.content),
				),
				[file3, file4],
			);
			assert.deepEqual(result1.body.variableReferences, [
				{
					name: "my-tool",
					range: new Range(10, 10, 10, 17),
					offset: 240,
					fullLength: 13,
				},
				{
					name: "my-other-tool",
					range: new Range(11, 10, 11, 23),
					offset: 257,
					fullLength: 19,
				},
			]);
			...
			assert.deepEqual(
				result2.body.fileReferences.map((r) =>
					result2.body?.resolveFilePath(r.content),
				),
				[nonExistingFolder, yetAnotherFile],
			);

			const result3 = await service.parseNew(
				yetAnotherFile,
				CancellationToken.None,
			);
			assert.deepEqual(result3.uri, yetAnotherFile);
			assert.deepEqual(
				result3.header?.description,
				"Another file description.",
			);
			assert.deepEqual(result3.header?.applyTo, "**/*.tsx");
			assert.ok(result3.body);
			assert.deepEqual(
				result3.body.fileReferences.map((r) =>
					result3.body?.resolveFilePath(r.content),
				),
			...
			assert.deepEqual(
				result4.header?.description,
				"File 4 splendid description.",
			);
			assert.ok(result4.body);
			assert.deepEqual(
				result4.body.fileReferences.map((r) =>
					result4.body?.resolveFilePath(r.content),
				),
				[
					URI.joinPath(
						rootFolderUri,
						"/folder1/some-other-folder/some-non-existing/file.prompt.md",
					),
					URI.joinPath(
						rootFolderUri,
						"/folder1/some-other-folder/some-non-prompt-file.md",
					),
	...
	suite("findInstructionFilesFor", () => {
		...
		test("finds correct instruction files", async () => {
			...
			sinon.stub(service, "listPromptFiles").returns(
				Promise.resolve([
					// local instructions
					{
						uri: URI.joinPath(
							rootFolderUri,
							".github/prompts/file1.instructions.md",
						),
					...
					},
					{
						uri: URI.joinPath(
							rootFolderUri,
							".github/prompts/file2.instructions.md",
						),
						storage: PromptsStorage.local,
						type: PromptsType.instructions,
					},
					{
						uri: URI.joinPath(
							rootFolderUri,
							".github/prompts/file3.instructions.md",
						),
						storage: PromptsStorage.local,
						type: PromptsType.instructions,
					},
					{
						uri: URI.joinPath(
							rootFolderUri,
							".github/prompts/file4.instructions.md",
						),
						storage: PromptsStorage.local,
						type: PromptsType.instructions,
					},
			...
			const instructionFiles = await service.getInstructionFiles(
				CancellationToken.None,
			);
			...
			await contextComputer.addApplyingInstructions(
				instructionFiles,
				context,
				result,
				newInstructionsCollectionEvent(),
			...
			assert.deepStrictEqual(
				result
					.asArray()
					.map((i) =>
						isPromptFileVariableEntry(i) ? i.value.path : undefined,
					),
				[
					// local instructions
		...
		test("copilot-instructions and AGENTS.md", async () => {
			...
			context.add(
				toFileVariableEntry(URI.joinPath(rootFolderUri, "README.md")),
			);
	...
	suite("getCustomAgents", () => {
		...
		test("header with handOffs", async () => {
			...
			).map((agent) => ({ ...agent, uri: URI.from(agent.uri) }));
			...
			const expected: ICustomAgent[] = [
				{
					...
					agentInstructions: {
						...
						toolReferences: [
							{ name: "tool1", range: { start: 11, endExclusive: 22 } },
						],
						metadata: undefined,
					},
					handOffs: undefined,
		...
		test("header with target", async () => {
			...
			const expected: ICustomAgent[] = [
				...
				},
				{
					...
					uri: URI.joinPath(
						rootFolderUri,
						".github/agents/vscode-agent.agent.md",
					),
					sessionTypes: undefined,
					source: { storage: PromptsStorage.local },
					enabled: true,
				},
				{
					...
					uri: URI.joinPath(
						rootFolderUri,
						".github/agents/generic-agent.agent.md",
					),
					sessionTypes: undefined,
					source: { storage: PromptsStorage.local },
					enabled: true,
			...
			const expected: ICustomAgent[] = [
				{
					...
					uri: URI.joinPath(
						rootFolderUri,
						".github/agents/copilot-agent.agent.md",
					),
					sessionTypes: undefined,
					source: { storage: PromptsStorage.local },
					enabled: true,
			...
			const expected: ICustomAgent[] = [
				{
					...
					uri: URI.joinPath(
						rootFolderUri,
						".github/agents/restricted-agent.agent.md",
					),
					source: { storage: PromptsStorage.local },
					enabled: true,
				},
				{
					...
					description:
						"Agent with no access to subagents, skills, or instructions.",
					agents: [],
					agentInstructions: {
					...
					uri: URI.joinPath(
						rootFolderUri,
						".github/agents/no-access-agent.agent.md",
					),
					source: { storage: PromptsStorage.local },
					enabled: true,
				},
				{
					...
					uri: URI.joinPath(
						rootFolderUri,
						".github/agents/full-access-agent.agent.md",
					),
					source: { storage: PromptsStorage.local },
					enabled: true,
				},
		...
		test("header with infer: false sets agentInvocable to false", async () => {
			...
			const inferFalseAgent = result.find(
				(a) => a.name === "agent-infer-false",
			);
			assert.ok(inferFalseAgent, "Should find agent with infer: false");
			assert.strictEqual(
				inferFalseAgent.visibility.agentInvocable,
				false,
				"infer: false should set agentInvocable to false",
			);

			const inferTrueAgent = result.find((a) => a.name === "agent-infer-true");
			assert.ok(inferTrueAgent, "Should find agent with infer: true");
			assert.strictEqual(
				inferTrueAgent.visibility.agentInvocable,
				true,
				"infer: true should set agentInvocable to true",
			);

			const noInferAgent = result.find((a) => a.name === "agent-no-infer");
			assert.ok(noInferAgent, "Should find agent without infer");
			assert.strictEqual(
				noInferAgent.visibility.agentInvocable,
				true,
				"missing infer should default agentInvocable to true",
			);
		});
		...
		test("agents from user data folder", async () => {
			...
			const customUserDataProfileService = {
				...
				currentProfile: {
					...toUserDataProfile(
						"test",
						"test",
						URI.file(userPromptsFolder).with({ path: "/user-data" }),
						URI.file("/cache"),
					),
			...
			service.dispose();
			const testService = disposables.add(
				instaService.createInstance(PromptsService),
			);
			...
			await mockFiles(fileService, [
				...
				// Another user data agent without header
				{
					...
					contents: ["A simple user agent without header."],
				},
			]);

			const result = (
				await testService.getCustomAgents(CancellationToken.None)
			).map((agent) => ({ ...agent, uri: URI.from(agent.uri) }));
			...
			assert.strictEqual(
				result.length,
				3,
				"Should find 3 agents (1 workspace + 2 user data)",
			);

			const workspaceAgent = result.find(
				(a) => a.source.storage === PromptsStorage.local,
			);
			...
			const userAgents = result.filter(
				(a) => a.source.storage === PromptsStorage.user,
			);
			assert.strictEqual(
				userAgents.length,
				2,
				"Should find 2 user data agents",
			);

			const userAgentWithHeader = userAgents.find(
				(a) => a.name === "user-agent",
			);
			...
			const simpleUserAgent = userAgents.find(
				(a) => a.name === "simple-user-agent",
			);
			assert.ok(simpleUserAgent, "Should find simple user agent");
			assert.strictEqual(
				simpleUserAgent.agentInstructions.content,
		...
		test("disabled agents are reported with enabled: false", async () => {
			...
			const toDisable = initial.filter(
				(a) =>
					a.name === "disabled-agent" || a.name === "another-disabled-agent",
			);
			...
			assert.strictEqual(
				persisted.size,
				2,
				`Expected 2 disabled agents, got ${persisted.size}`,
			);
			...
			assert.strictEqual(
				result.length,
				3,
				"Should still discover all 3 agents",
			);
			...
			assert.strictEqual(
				...
				true,
				"enabled-agent should be enabled",
			);

			const disabledAgent = result.find((a) => a.name === "disabled-agent");
			assert.ok(disabledAgent, "Should find disabled-agent");
			assert.strictEqual(
				disabledAgent.enabled,
				false,
				"disabled-agent should be disabled",
			);

			const anotherDisabledAgent = result.find(
				(a) => a.name === "another-disabled-agent",
			);
			assert.ok(anotherDisabledAgent, "Should find another-disabled-agent");
			assert.strictEqual(
				anotherDisabledAgent.enabled,
				false,
				"another-disabled-agent should be disabled",
			);
		});
		...
		test("getDiscoveryInfo reports enabled and disabled agents", async () => {
			...
			const discoveryInfo = await testService.getDiscoveryInfo(
				PromptsType.agent,
				CancellationToken.None,
			);
			assert.strictEqual(discoveryInfo.type, PromptsType.agent);
			assert.strictEqual(
				discoveryInfo.files.length,
				2,
				"Discovery should include both agents",
			);

			const enabledFile = discoveryInfo.files.find((f) =>
				f.promptPath.uri.path.endsWith("enabled-agent.agent.md"),
			) as IAgentDiscoveryResult | undefined;
			assert.ok(enabledFile, "Should report enabled-agent in discovery info");
			assert.strictEqual(
				enabledFile.status,
				"loaded",
				"Enabled agent should be loaded",
			);
			assert.strictEqual(
				enabledFile.skipReason,
				undefined,
				"Enabled agent should not have a skip reason",
			);
			assert.ok(
				enabledFile.agent,
				"Enabled agent file should carry resolved agent",
			);
			assert.strictEqual(enabledFile.agent.enabled, true);

			const disabledFile = discoveryInfo.files.find((f) =>
				f.promptPath.uri.path.endsWith("disabled-agent.agent.md"),
			) as IAgentDiscoveryResult | undefined;
			assert.ok(disabledFile, "Should report disabled-agent in discovery info");
			assert.strictEqual(
				disabledFile.status,
				"skipped",
				"Disabled agent should be skipped",
			);
			assert.strictEqual(
				disabledFile.skipReason,
				"disabled",
				'Disabled agent should have skipReason "disabled"',
			);
			assert.ok(
				disabledFile.agent,
				"Disabled agent file should still carry resolved agent",
			);
	...
	suite("listPromptFiles - prompts", () => {
		test("prompts from user data folder", async () => {
			...
			const result = await testService.listPromptFiles(
				PromptsType.prompt,
				CancellationToken.None,
			);
			...
			assert.strictEqual(
				result.length,
				2,
				"Should find 2 prompts (1 workspace + 1 user data)",
			);

			const workspacePrompt = result.find(
				(p) => p.storage === PromptsStorage.local,
			);
			assert.ok(workspacePrompt, "Should find workspace prompt");
			assert.ok(
				workspacePrompt.uri.path.includes("workspace-prompt.prompt.md"),
			);
	...
	suite("listPromptFiles - instructions", () => {
		test("instructions from user data folder", async () => {
			...
			assert.strictEqual(
				result.length,
				2,
				"Should find 2 instructions (1 workspace + 1 user data)",
			);

			const workspaceInstructions = result.find(
				(p) => p.storage === PromptsStorage.local,
			);
			assert.ok(workspaceInstructions, "Should find workspace instructions");
			assert.ok(
				workspaceInstructions.uri.path.includes(
					"workspace-instructions.instructions.md",
				),
			);

			const userInstructions = result.find(
				(p) => p.storage === PromptsStorage.user,
			);
			assert.ok(userInstructions, "Should find user data instructions");
			assert.ok(
				userInstructions.uri.path.includes("user-instructions.instructions.md"),
			);
		});
	});
	...
	suite("listPromptFiles - skills ", () => {
		...
		test("should list skill files from user home", async () => {
			...
			const personalSkills = result.filter(
				(s) => s.storage === PromptsStorage.user,
			);
			assert.strictEqual(
				personalSkills.length,
				2,
				"Should find 2 personal skills",
			);

			const copilotSkill = personalSkills.find((s) =>
				s.uri.path.includes(".copilot"),
			);
			assert.ok(copilotSkill, "Should find copilot personal skill");

			const claudeSkill = personalSkills.find((s) =>
				s.uri.path.includes(CLAUDE_CONFIG_FOLDER),
			);
		...
		test("should not list skills when not in skill folder structure", async () => {
			...
			assert.strictEqual(
				result.length,
				0,
				"Should not find any skills in non-skill locations",
			);
		});
		...
		test("should handle mixed workspace and user home skills", async () => {
			...
			const workspaceSkills = result.filter(
				(s) => s.storage === PromptsStorage.local,
			);
			const userSkills = result.filter(
				(s) => s.storage === PromptsStorage.user,
			);
		...
		test("should respect disabled default paths via config", async () => {
			...
			assert.strictEqual(
				result.length,
				1,
				"Should find only 1 skill (from enabled folder)",
			);
			assert.ok(
				result[0].uri.path.includes(".claude/skills"),
				"Should only find skill from .claude/skills",
			);
			assert.ok(
				!result[0].uri.path.includes(".github/skills"),
				"Should not find skill from disabled .github/skills",
			);
		});
		...
		test("should expand tilde paths in custom locations", async () => {
			...
			assert.strictEqual(
				result.length,
				1,
				"Should find 1 skill from tilde-expanded path",
			);
			assert.ok(
				result[0].uri.path.includes("/home/user/my-custom-skills"),
				"Path should be expanded from tilde",
			);
		});
	});
	...
	suite("listPromptFiles - extensions", () => {
		test("Contributed prompt file", async () => {
			const uri = URI.parse(
				"file://extensions/my-extension/textMate.instructions.md",
			);
			...
			const actual = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
		...
		test("Custom agent provider", async () => {
			const agentUri = URI.parse(
				"file://extensions/my-extension/myAgent.agent.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
			...
			const registered = service.registerPromptFileProvider(
				extension,
				PromptsType.agent,
				provider,
			);
			...
			const actualAfterDispose = await service.getCustomAgents(
				CancellationToken.None,
			);
		...
		test("Contributed agent file that does not exist should not crash", async () => {
			const nonExistentUri = URI.parse(
				"file://extensions/my-extension/nonexistent.agent.md",
			);
			const existingUri = URI.parse(
				"file://extensions/my-extension/existing.agent.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
			} as unknown as IExtensionDescription;
			...
			assert.strictEqual(
				agents.length,
				1,
				"Should only return the agent that exists",
			);
		...
		test("Contributed file with when clause is filtered inside PromptsService", async () => {
			const uri = URI.parse(
				"file://extensions/my-extension/conditional.instructions.md",
			);
			const extension = {} as IExtensionDescription;
			const contextKeyService = instaService.get(
				IContextKeyService,
			) as MockContextKeyService;
			const contextMatchesRulesStub = sinon
				.stub(contextKeyService, "contextMatchesRules")
				.returns(false);
			...
			const files = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			assert.strictEqual(
				files.length,
				0,
				"Should be filtered out when the when clause does not match",
			);

			registered.dispose();
			contextMatchesRulesStub.restore();

			const enabledContextMatchesRulesStub = sinon
				.stub(contextKeyService, "contextMatchesRules")
				.returns(true);
			...
			const enabledFiles = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			assert.strictEqual(
				enabledFiles.length,
				1,
				"Should be included when the when clause matches",
			);
		...
		test("Provider file with when clause is filtered inside PromptsService", async () => {
			const uri = URI.parse(
				"file://extensions/test/myInstruction.instructions.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
				enabledApiProposals: ["chatParticipantPrivate"],
			...
			const registered = service.registerPromptFileProvider(
				...
				{
					providePromptFiles: async () => [
						{ uri, when: "chatSessionType == local" },
					],
				},
			);
			...
			const contextMatchesRulesStub = sinon
				.stub(contextKeyService, "contextMatchesRules")
				.returns(false);
			const files = await service.listPromptFilesForStorage(
				PromptsType.instructions,
			...
			const enabledContextMatchesRulesStub = sinon
				.stub(contextKeyService, "contextMatchesRules")
				.returns(true);
			const enabledFiles = await service.listPromptFilesForStorage(
				PromptsType.instructions,
				PromptsStorage.extension,
				CancellationToken.None,
			);
		...
		test("Provider when keys invalidate cached results when context changes", async () => {
			const contextKeyService = disposables.add(
				new TestPromptContextKeyService(),
			);
			instaService.stub(IContextKeyService, contextKeyService);
			const promptsService = disposables.add(
				instaService.createInstance(PromptsService),
			);
			instaService.stub(IPromptsService, promptsService);

			const uri = URI.parse(
				"file://extensions/test/conditional.instructions.md",
			);
			await mockFiles(fileService, [
				{
					path: uri.path,
			...
			const registered = promptsService.registerPromptFileProvider(
				extension,
				PromptsType.instructions,
				{
			...
			const enabledFiles = await promptsService.getInstructionFiles(
				CancellationToken.None,
			);
			assert.strictEqual(
				enabledFiles.length,
				1,
				"Should include the provider instruction when the context matches",
			);

			contextKeyService.setRulesMatch(false);
			contextKeyService.fireDidChangeContext(["myFeature.enabled"]);
			const disabledFiles = await promptsService.getInstructionFiles(
				CancellationToken.None,
			);
			assert.strictEqual(
				disabledFiles.length,
				0,
				"Should invalidate the cached provider instruction when the tracked key changes",
			);
		...
		test("Contributed file sessionTypes metadata is preserved in core prompt models", async () => {
			...
			const agentUri = URI.parse(
				"file://extensions/my-extension/contributed.agent.md",
			);
			const instructionUri = URI.parse(
				"file://extensions/my-extension/contributed.instructions.md",
			);
			const promptUri = URI.parse(
				"file://extensions/my-extension/contributed.prompt.md",
			);
			const skillUri = URI.parse(
				"file://extensions/my-extension/contributed-skill/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
			} as IExtensionDescription;
			...
			const registrations = [
				service.registerContributedFile(
					PromptsType.agent,
					agentUri,
					extension,
					undefined,
			...
			try {
				const agent = (
					await service.getCustomAgents(CancellationToken.None)
				).find((item) => item.uri.toString() === agentUri.toString());
				const instruction = (
					await service.getInstructionFiles(CancellationToken.None)
				).find((item) => item.uri.toString() === instructionUri.toString());
				const prompt = (
					await service.getPromptSlashCommands(CancellationToken.None)
				).find((item) => item.uri.toString() === promptUri.toString());
				const skill = (
					await service.findAgentSkills(CancellationToken.None)
				)?.find((item) => item.uri.toString() === skillUri.toString());
	...
	suite("listPromptFiles - parent repo folder", () => {
		test("should find prompts, instructions, and agents in a parent repo folder", async () => {
			...
			await testConfigService.setUserConfiguration(
				PromptsConfig.INCLUDE_APPLYING_INSTRUCTIONS,
				true,
			);
			await testConfigService.setUserConfiguration(
			...
			let promptFiles = await service.listPromptFiles(
				PromptsType.prompt,
				CancellationToken.None,
			);
			let agentFiles = await service.listPromptFiles(
				PromptsType.agent,
				CancellationToken.None,
			);
			let instructionFiles = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);

			assert.ok(
				!promptFiles.some((f) => f.uri.path.includes(parentFolder)),
				"Should not find parent prompt files when parent search is disabled",
			);
			assert.ok(
				!agentFiles.some((f) => f.uri.path.includes(parentFolder)),
				"Should not find parent agent files when parent search is disabled",
			);
			assert.ok(
				!instructionFiles.some((f) => f.uri.path.includes(parentFolder)),
				"Should not find parent instruction files when parent search is disabled",
			);
			...
			promptFiles = await service.listPromptFiles(
				PromptsType.prompt,
				CancellationToken.None,
			);
			agentFiles = await service.listPromptFiles(
				PromptsType.agent,
				CancellationToken.None,
			);
			instructionFiles = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			...
			assert.ok(
				promptPaths.includes(`${parentFolder}/.github/prompts/help.prompt.md`),
				"Should find parent prompt file when parent search is enabled",
			);
			assert.ok(
				agentPaths.includes(`${parentFolder}/.github/agents/reviewer.agent.md`),
				"Should find parent agent file when parent search is enabled",
			);
			assert.ok(
				instructionPaths.includes(
					`${parentFolder}/.github/instructions/typescript.instructions.md`,
				),
				"Should find parent instruction file when parent search is enabled",
			);
		});
		...
		test("should not find files in an untrusted parent repo folder", async () => {
			...
			const promptFiles = await service.listPromptFiles(
				PromptsType.prompt,
				CancellationToken.None,
			);
			const agentFiles = await service.listPromptFiles(
				PromptsType.agent,
				CancellationToken.None,
			);
			const instructionFiles = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			...
			assert.ok(
				!promptFiles.some((f) => f.uri.path.includes(parentFolder)),
				"Should not find parent prompt files when parent repo is untrusted",
			);
			assert.ok(
				!agentFiles.some((f) => f.uri.path.includes(parentFolder)),
				"Should not find parent agent files when parent repo is untrusted",
			);
			assert.ok(
				!instructionFiles.some((f) => f.uri.path.includes(parentFolder)),
				"Should not find parent instruction files when parent repo is untrusted",
			);
		});
	});
	...
	test("Instructions provider", async () => {
		const instructionUri = URI.parse(
			"file://extensions/my-extension/myInstruction.instructions.md",
		);
		const extension = {
			identifier: { value: "test.my-extension" },
			enabledApiProposals: ["chatParticipantPrivate"],
		...
		await mockFiles(fileService, [
			{
				path: instructionUri.path,
				contents: ["# Test instruction content"],
			},
		]);
		...
		const providerInstruction = actual.find(
			(i) => i.uri.toString() === instructionUri.toString(),
		);

		assert.ok(providerInstruction, "Provider instruction should be found");
		assert.strictEqual(
			providerInstruction!.uri.toString(),
			instructionUri.toString(),
		);
		assert.strictEqual(providerInstruction!.storage, PromptsStorage.extension);
		assert.strictEqual(
			providerInstruction!.source,
		...
		const actualAfterDispose = await service.listPromptFiles(
			PromptsType.instructions,
			CancellationToken.None,
		);
		const foundAfterDispose = actualAfterDispose.find(
			(i) => i.uri.toString() === instructionUri.toString(),
		);
	...
	test("Provider sessionTypes metadata is preserved in core prompt models", async () => {
		...
		const agentUri = URI.parse(
			"file://extensions/my-extension/enabled.agent.md",
		);
		const instructionUri = URI.parse(
			"file://extensions/my-extension/enabled.instructions.md",
		);
		const promptUri = URI.parse(
			"file://extensions/my-extension/enabled.prompt.md",
		);
		const skillUri = URI.parse(
			"file://extensions/my-extension/enabled-skill/SKILL.md",
		);
		const extension = {
			identifier: { value: "test.my-extension" },
			enabledApiProposals: ["chatParticipantPrivate"],
	...
	test("Prompt file provider", async () => {
		const promptUri = URI.parse(
			"file://extensions/my-extension/myPrompt.prompt.md",
		);
		const extension = {
			identifier: { value: "test.my-extension" },
			enabledApiProposals: ["chatParticipantPrivate"],
		...
		const providerPrompt = actual.find(
			(i) => i.uri.toString() === promptUri.toString(),
		);
	...
	test("Skill file provider", async () => {
		const skillUri = URI.parse(
			"file://extensions/my-extension/mySkill/SKILL.md",
		);
		const extension = {
			identifier: { value: "test.my-extension" },
			enabledApiProposals: ["chatParticipantPrivate"],
		...
		const providerSkill = actual.find(
			(i) => i.uri.toString() === skillUri.toString(),
		);
	...
	suite("findAgentSkills", () => {
		...
		test("should find skills in workspace and user home", async () => {
			...
			assert.ok(
				allResult,
				"Should return results when agent skills are enabled",
			);
			...
			const projectSkills = result.filter(
				(skill) => skill.storage === PromptsStorage.local,
			);
			assert.strictEqual(
				projectSkills.length,
				3,
				"Should find 3 project skills",
			);

			const githubSkill1 = projectSkills.find(
				(skill) => skill.name === "GitHub Skill 1",
			);
			assert.ok(githubSkill1, "Should find GitHub skill 1");
			assert.strictEqual(
				githubSkill1.description,
				"A GitHub skill for testing",
			);
			assert.strictEqual(
				githubSkill1.uri.path,
				`${rootFolder}/.github/skills/GitHub Skill 1/SKILL.md`,
			);

			const claudeSkill1 = projectSkills.find(
				(skill) => skill.name === "Claude Skill 1",
			);
			assert.ok(claudeSkill1, "Should find Claude skill 1");
			assert.strictEqual(
				claudeSkill1.description,
				"A Claude skill for testing",
			);
			assert.strictEqual(
				claudeSkill1.uri.path,
				`${rootFolder}/.claude/skills/Claude Skill 1/SKILL.md`,
			);

			// The invalid-skill (no name attribute) should now use folder name as fallback
			const invalidSkill = projectSkills.find(
				(skill) => skill.name === "invalid-skill",
			);
			assert.ok(
				invalidSkill,
				"Should find invalid-skill using folder name as fallback",
			);
			assert.strictEqual(invalidSkill.description, "Invalid skill, no name");
			assert.strictEqual(
				invalidSkill.uri.path,
				`${rootFolder}/.claude/skills/invalid-skill/SKILL.md`,
			);

			// Check personal skills
			const personalSkills = result.filter(
				(skill) => skill.storage === PromptsStorage.user,
			);
			assert.strictEqual(
				personalSkills.length,
				2,
			...
			const personalSkill1 = personalSkills.find(
				(skill) => skill.name === "Personal Skill 1",
			);
			assert.ok(personalSkill1, "Should find Personal Skill 1");
			assert.strictEqual(
				personalSkill1.description,
				"A personal skill for testing",
			);
			assert.strictEqual(
				personalSkill1.uri.path,
				"/home/user/.claude/skills/Personal Skill 1/SKILL.md",
			);

			const copilotSkill1 = personalSkills.find(
				(skill) => skill.name === "Copilot Skill 1",
			);
			assert.ok(copilotSkill1, "Should find Copilot Skill 1");
			assert.strictEqual(
				copilotSkill1.description,
				"A Copilot skill for testing",
			);
			assert.strictEqual(
				copilotSkill1.uri.path,
				"/home/user/.copilot/skills/Copilot Skill 1/SKILL.md",
			);
		});
		...
		test("should handle parsing errors gracefully", async () => {
			...
			assert.ok(
				invalidSkill,
				"Should find skill with folder name as fallback despite malformed YAML",
			);
		...
		test("should truncate long names and descriptions", async () => {
			...
			assert.strictEqual(
				...
				64,
				"Name should be truncated to 64 characters",
			);
			assert.strictEqual(
				result[0].description?.length,
				1024,
				"Description should be truncated to 1024 characters",
			);
		});
		...
		test("should remove XML tags from name and description", async () => {
			...
			assert.strictEqual(
				result[0].name,
				"Skill with XML tags",
				"XML tags should be removed from name",
			);
			assert.strictEqual(
				result[0].description,
				"Description with HTML and other tags",
				"XML tags should be removed from description",
			);
		});
		...
		test("should handle both truncation and XML removal", async () => {
			...
			assert.ok(
				!result[0].name.includes("<"),
				"Name should not contain XML tags",
			);
			assert.ok(
				!result[0].name.includes(">"),
				"Name should not contain XML tags",
			...
			assert.ok(
				...
				"Description should not contain XML tags",
			);
			assert.ok(
				!result[0].description?.includes(">"),
				"Description should not contain XML tags",
		...
		test("should skip duplicate skill names and keep first by priority", async () => {
			...
			assert.strictEqual(
				result.length,
				2,
				"Should find 2 skills (duplicate skipped)",
			);

			const duplicateSkill = result.find((s) => s.name === "Duplicate Skill");
			assert.ok(duplicateSkill, "Should find the duplicate skill");
			assert.strictEqual(
				duplicateSkill.description,
				"Workspace version",
				"Should keep workspace version (higher priority)",
			);
			assert.strictEqual(
				duplicateSkill.storage,
		...
		test("should prioritize skills by source: workspace > user > extension", async () => {
			...
			assert.strictEqual(
				result.length,
				1,
				"Should find 1 skill (duplicates resolved by priority)",
			);
			assert.strictEqual(
				result[0].description,
				"Workspace version - highest priority",
				"Workspace should win over user",
			);
		...
		test("should include skills where name does not match folder name using folder name as fallback", async () => {
			...
			const mismatchedSkill = result.find(
				(s) => s.name === "wrong-folder-name",
			);
			assert.ok(
				mismatchedSkill,
				"Should find skill with folder name as fallback",
			);
			assert.strictEqual(
				mismatchedSkill.description,
				"This skill should use folder name as fallback",
			);
		...
		test("should include skills with missing name attribute using folder name as fallback", async () => {
			...
			assert.strictEqual(
				noNameSkill.description,
				"This skill has no name attribute",
			);
		...
		test("should include extension-provided skills in findAgentSkills", async () => {
			...
			const extensionSkillUri = URI.parse(
				"file://extensions/my-extension/Extension Skill/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
				enabledApiProposals: ["chatParticipantPrivate"],
			...
			assert.strictEqual(
				result.length,
				2,
				"Should find 2 skills (workspace + extension)",
			);
		...
		test("should include contributed skill files in findAgentSkills", async () => {
			...
			const contributedSkillUri = URI.parse(
				"file://extensions/my-extension/Contributed Skill/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
			...
			assert.strictEqual(
				result.length,
				2,
				"Should find 2 skills (local + contributed)",
			);
			...
			const contributedSkill = result.find(
				(s) => s.name === "Contributed Skill",
			);
			...
			const resultAfterDispose = await service.findAgentSkills(
				CancellationToken.None,
			);
			assert.strictEqual(
				resultAfterDispose?.length,
				1,
				"Should find 1 skill after disposal",
			);
		...
		test("should use folder name for contributed skill with missing name", async () => {
			...
			const contributedSkillUri = URI.parse(
				"file://extensions/my-extension/my-skill/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
			} as unknown as IExtensionDescription;
		...
		test("should accept contributed skill with missing description", async () => {
			...
			const contributedSkillUri = URI.parse(
				"file://extensions/my-extension/no-desc-skill/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
			} as unknown as IExtensionDescription;
		...
		test("should override contributed skill name with folder name on mismatch", async () => {
			...
			const contributedSkillUri = URI.parse(
				"file://extensions/my-extension/actual-folder/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
			} as unknown as IExtensionDescription;
			...
			assert.ok(
				skill,
				"Should find skill using folder name instead of mismatched name",
			);
	...
	suite("getPromptSlashCommands - skills", () => {
		...
		test("should include skills from workspace as slash commands", async () => {
			...
			const slashCommands = await service.getPromptSlashCommands(
				CancellationToken.None,
			);

			const workspaceSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "workspace-skill",
			);
			assert.ok(
				workspaceSkillCommand,
				"Should find workspace skill as slash command",
			);
			assert.strictEqual(
				workspaceSkillCommand.description,
				"A workspace skill that should appear as slash command",
			);
			...
			const anotherSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "another-skill",
			);
			assert.ok(
				anotherSkillCommand,
				"Should find another skill as slash command",
			);
			assert.strictEqual(
				anotherSkillCommand.description,
				"Another skill from workspace",
			);
		...
		test("should include skills from user storage as slash commands", async () => {
			...
			const personalSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "personal-skill",
			);
			assert.ok(
				personalSkillCommand,
				"Should find personal skill as slash command",
			);
			assert.strictEqual(
				personalSkillCommand.description,
				"A personal skill from user storage",
			);
			...
			const claudePersonalCommand = slashCommands.find(
				(cmd) => cmd.name === "claude-personal",
			);
			assert.ok(
				claudePersonalCommand,
				"Should find Claude personal skill as slash command",
			);
			assert.strictEqual(
				claudePersonalCommand.description,
				"A Claude personal skill",
			);
		...
		test("should include skills from extension providers as slash commands", async () => {
			...
			const providerSkillUri = URI.parse(
				"file://extensions/my-extension/provider-skill/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
				enabledApiProposals: ["chatParticipantPrivate"],
			...
			const providerSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "provider-skill",
			);
			assert.ok(
				providerSkillCommand,
				"Should find provider skill as slash command",
			);
			assert.strictEqual(
				providerSkillCommand.description,
				"A skill from extension provider",
			);
			assert.strictEqual(
				providerSkillCommand.storage,
				PromptsStorage.extension,
			);
			assert.strictEqual(providerSkillCommand.type, PromptsType.skill);
			assert.strictEqual(
				providerSkillCommand.source,
				PromptFileSource.ExtensionAPI,
			);
			...
			const foundAfterDispose = slashCommandsAfterDispose.find(
				(cmd) => cmd.name === "provider-skill",
			);
			assert.strictEqual(
				foundAfterDispose,
				undefined,
				"Should not find provider skill after disposal",
			);
		});
		...
		test("should include skills from extension contributions as slash commands", async () => {
			...
			const contributedSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "contributed-skill",
			);
			assert.ok(
				contributedSkillCommand,
				"Should find contributed skill as slash command",
			);
			assert.strictEqual(
				contributedSkillCommand.description,
				"A skill from extension contribution",
			);
			assert.strictEqual(
				contributedSkillCommand.storage,
				PromptsStorage.extension,
			);
			assert.strictEqual(contributedSkillCommand.type, PromptsType.skill);
			assert.strictEqual(
				contributedSkillCommand.source,
			...
			assert.strictEqual(
				foundAfterDispose,
				undefined,
				"Should not find contributed skill after disposal",
			);
		});
		...
		test("should combine prompt files and skills as slash commands", async () => {
			...
			const promptCommand = slashCommands.find(
				(cmd) => cmd.name === "my-prompt",
			);
		...
		test("should fire change event when provider registers/unregisters", async () => {
			...
			const providerSkillUri = URI.parse(
				"file://extensions/my-extension/test-skill/SKILL.md",
			);
			const extension = {
				identifier: { value: "test.my-extension" },
				enabledApiProposals: ["chatParticipantPrivate"],
			...
			const commandsWithProvider = await service.getPromptSlashCommands(
				CancellationToken.None,
			);
			const skillCommand = commandsWithProvider.find(
				(cmd) => cmd.name === "test-skill",
			);
			...
			const commandsAfterDispose = await service.getPromptSlashCommands(
				CancellationToken.None,
			);
			const skillAfterDispose = commandsAfterDispose.find(
				(cmd) => cmd.name === "test-skill",
			);
			assert.strictEqual(
				skillAfterDispose,
				undefined,
				"Should not find skill after provider disposal",
			);

			assert.ok(
				changeEventCount >= 2,
				"Change event should fire when provider registers and unregisters",
			);
		...
		test("should use filename as fallback for skills with missing name", async () => {
			...
			const fallbackNameCommand = slashCommands.find(
				(cmd) => cmd.name === "no-name",
			);
			assert.ok(
				fallbackNameCommand,
				"Should find skill with fallback name from folder name",
			);
			assert.strictEqual(fallbackNameCommand.description, "Skill without name");

			// Should include valid skill
			const validSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "valid-skill",
			);
		...
		test("should use folder name as slash command name when frontmatter name differs", async () => {
			...
			const folderNameCommand = slashCommands.find(
				(cmd) => cmd.name === "test",
			);
			assert.ok(
				folderNameCommand,
				"Should find skill using folder name as slash command name",
			);
			assert.strictEqual(
				folderNameCommand.description,
				"A skill with mismatched frontmatter name",
			);

			const frontmatterNameCommand = slashCommands.find(
				(cmd) => cmd.name === "foo",
			);
			assert.strictEqual(
				frontmatterNameCommand,
				undefined,
				"Should not find skill using frontmatter name",
			);
		});
		...
		test("should not duplicate slash commands with same name from different types", async () => {
			...
			const duplicateCommands = slashCommands.filter(
				(cmd) => cmd.name === "duplicate-name",
			);
			...
			assert.strictEqual(
				...
				2,
				"Should return both prompt and skill with same name",
			);

			const promptCommand = duplicateCommands.find(
				(cmd) => cmd.type === PromptsType.prompt,
			);
			assert.ok(promptCommand, "Should find prompt command");

			const skillCommand = duplicateCommands.find(
				(cmd) => cmd.type === PromptsType.skill,
			);
		...
		test("should respect skill disable configuration (USE_AGENT_SKILLS)", async () => {
			...
			assert.ok(
				promptCommand,
				"Should find prompt command even when skills are disabled",
			);
			...
			assert.strictEqual(
				skillCommand,
				undefined,
				"Should not find skill command when skills are disabled",
			);
		});
	});
	...
	suite("getPromptSlashCommands - userInvocable filtering", () => {
		...
		test("should return correct userInvocable value for skills with user-invocable: false", async () => {
			...
			const hiddenSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "hidden-skill",
			);
			assert.ok(
				hiddenSkillCommand,
				"Should find hidden skill in slash commands",
			);
			assert.strictEqual(
			...
			const hiddenSkillInFiltered = filteredCommands.find(
				(cmd) => cmd.name === "hidden-skill",
			);
			assert.strictEqual(
		...
		test("should return correct userInvocable value for skills with user-invocable: true", async () => {
			...
			const visibleSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "visible-skill",
			);
			assert.ok(
				visibleSkillCommand,
				"Should find visible skill in slash commands",
			);
			assert.strictEqual(
			...
			const filteredCommands = slashCommands.filter((c) => c.userInvocable);
			const visibleSkillInFiltered = filteredCommands.find(
				(cmd) => cmd.name === "visible-skill",
			);
			assert.ok(
		...
		test("should default to true for skills without user-invocable attribute", async () => {
			...
			const defaultSkillCommand = slashCommands.find(
				(cmd) => cmd.name === "default-skill",
			);
			assert.ok(
				defaultSkillCommand,
				"Should find default skill in slash commands",
			);
			assert.strictEqual(
				defaultSkillCommand.userInvocable,
				true,
				"Should have userInvocable=true when attribute is not specified",
			);

			// Verify the filtering logic would correctly include this skill (undefined !== false is true)
			const filteredCommands = slashCommands.filter((c) => c.userInvocable);
			const defaultSkillInFiltered = filteredCommands.find(
				(cmd) => cmd.name === "default-skill",
			);
			assert.ok(
		...
		test("should handle prompts with user-invocable: false", async () => {
			...
			const hiddenPromptCommand = slashCommands.find(
				(cmd) => cmd.name === "hidden-prompt",
			);
			assert.ok(
				hiddenPromptCommand,
				"Should find hidden prompt in slash commands",
			);
			assert.strictEqual(
			...
			const filteredCommands = slashCommands.filter((c) => c.userInvocable);
			const hiddenPromptInFiltered = filteredCommands.find(
				(cmd) => cmd.name === "hidden-prompt",
			);
			assert.strictEqual(
		...
		test("should correctly filter mixed user-invocable values", async () => {
			...
			assert.strictEqual(
				...
				2,
				"Should have 2 commands after filtering",
			);
			assert.ok(
				filteredCommands.find((c) => c.name === "visible-prompt"),
				"visible-prompt should be included",
			);
			assert.ok(
				filteredCommands.find((c) => c.name === "visible-skill"),
				"visible-skill should be included",
			);
			assert.strictEqual(
				filteredCommands.find((c) => c.name === "hidden-prompt"),
				undefined,
				"hidden-prompt should be excluded",
			);
			assert.strictEqual(
				filteredCommands.find((c) => c.name === "hidden-skill"),
				undefined,
		...
		test("should handle skills with missing header gracefully", async () => {
			...
			assert.ok(
				noHeaderSkill,
				"Should find skill without header in slash commands",
			);
		...
		test("plugin skills include plugin name prefix in slash command name", async () => {
			...
			const plugin: IAgentPlugin = {
				...
				skills: observableValue<readonly IAgentPluginSkill[]>(
					"testPluginSkills",
					[{ uri: skillUri, name: "deploy" }],
				),
				...
				mcpServerDefinitions: observableValue(
					"testPluginMcpServerDefinitions",
					[],
				),
			};
			...
			const skillCommand = slashCommands.find(
				(cmd) => cmd.name === "my-plugin:deploy",
			);
			assert.ok(
				skillCommand,
				"Plugin skill should have plugin prefix in slash command name",
			);
		...
		test("plugin skill frontmatter name is qualified with plugin prefix", async () => {
			...
			const skillCommand = slashCommands.find(
				(cmd) => cmd.name === "devtools:ci",
			);
			assert.ok(
				skillCommand,
				"Plugin skill folder name should be qualified with plugin prefix",
			);
	...
	suite("hooks", () => {
		const createTestPlugin = (
			path: string,
			initialHooks: readonly IAgentPluginHook[],
		): {
			plugin: IAgentPlugin;
			hooks: ISettableObservable<readonly IAgentPluginHook[]>;
		} => {
			const enablement = observableValue(
				"testPluginEnablement",
				2 /* ContributionEnablementState.EnabledProfile */,
			);
			const hooks = observableValue<readonly IAgentPluginHook[]>(
				"testPluginHooks",
				initialHooks,
			);
			const commands = observableValue<readonly IAgentPluginCommand[]>(
				"testPluginCommands",
				[],
			);
			const skills = observableValue<readonly IAgentPluginSkill[]>(
				"testPluginSkills",
				[],
			);
			const agents = observableValue<readonly IAgentPluginAgent[]>(
				"testPluginAgents",
				[],
			);
			const instructions = observableValue<readonly IAgentPluginInstruction[]>(
				"testPluginInstructions",
				[],
			);
			const mcpServerDefinitions = observableValue<
		...
		test("multi-root workspace resolves cwd to per-hook-file workspace folder", async function () {
			...
			workspaceContextService.setWorkspace(
				testWorkspace(folder1Uri, folder2Uri),
			);
			testConfigService.setUserConfiguration(
			...
			assert.strictEqual(
				preToolUseHooks.length,
				2,
				"Expected two PreToolUse hooks",
			);
			...
			assert.strictEqual(
				hookA.cwd?.path,
				folder1Uri.path,
				"Hook from folder-a should have cwd pointing to workspace-a",
			);
			assert.strictEqual(
				hookB.cwd?.path,
				folder2Uri.path,
				"Hook from folder-b should have cwd pointing to workspace-b",
			);
		});
		...
		test("includes hooks from agent plugins", async function () {
			...
			assert.deepStrictEqual(
				result.hooks[HookType.PreToolUse],
				[
					{
		...
		test("recomputes hooks when agent plugin hooks change", async function () {
			...
			assert.deepStrictEqual(before.hooks[HookType.PreToolUse], [
				{
					command: "echo before",
					sourceUri: URI.file("/plugins/test-plugin/hooks.json"),
				},
			...
			assert.deepStrictEqual(after.hooks[HookType.PreToolUse], [
				{
					command: "echo after",
					sourceUri: URI.file("/plugins/test-plugin/hooks.json"),
				},
		...
		test("returns undefined when workspace is untrusted", async function () {
			workspaceContextService.setWorkspace(
				testWorkspace(URI.file("/test-workspace")),
			);
			testConfigService.setUserConfiguration(
				PromptsConfig.USE_CHAT_HOOKS,
				true,
			...
			assert.strictEqual(
				untrustedResult,
				undefined,
				"Expected undefined hooks when workspace is untrusted",
			);
			...
			assert.ok(
				reTrustedResult,
				"Expected hooks after workspace becomes trusted again",
			);
		...
		test("suppresses plugin hooks when workspace is untrusted", async function () {
			...
			assert.strictEqual(
				result,
				undefined,
				"Expected undefined hooks when workspace is untrusted, even with plugin hooks",
			);
		});
			...
			await mockFiles(fileService, [
				{
					...
					contents: [
						JSON.stringify({
							...
							hooks: {
								PreToolUse: [
									{ type: "command", command: "echo disabled-claude-hook" },
								],
							},
						}),
					],
		...
		test("plugin hooks appear in hook discovery info files", async function () {
			...
			const result = await service.getHooks(CancellationToken.None);
			const capturedDiscoveryInfo = await service.getDiscoveryInfo(
				PromptsType.hook,
				CancellationToken.None,
			);
			...
			assert.ok(
				pluginFile,
				"Plugin hook file should be present in discovery info files",
			);
		});
	});
	...
	suite("plugin instructions", () => {
		function createPluginWithInstructions(
			...
			plugin: IAgentPlugin;
			instructions: ISettableObservable<readonly IAgentPluginInstruction[]>;
		} {
			const enablement = observableValue(
				"testPluginEnablement",
				2 /* ContributionEnablementState.EnabledProfile */,
		...
		test("lists plugin instructions via listPromptFiles", async function () {
			...
			const pluginInstruction = result.find(
				(p) => p.uri.toString() === ruleUri.toString(),
			);
			assert.ok(
				pluginInstruction,
				"Plugin instruction should appear in listPromptFiles",
			);
		...
		test("updates listed instructions when plugin instructions change", async function () {
			...
			const { plugin, instructions } = createPluginWithInstructions(
				"/plugins/test-plugin",
			...
			const before = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			const beforePlugin = before.filter(
				(p) => p.storage === PromptsStorage.plugin,
			);
			...
			const after = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			const afterPlugin = after.filter(
				(p) => p.storage === PromptsStorage.plugin,
			);
		...
		test("removes instructions when plugin is removed", async function () {
			...
			testPluginsObservable.set([plugin], undefined);
			const withPlugin = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			...
			testPluginsObservable.set([], undefined);
			const withoutPlugin = await service.listPromptFiles(
				PromptsType.instructions,
				CancellationToken.None,
			);
			assert.ok(
				!withoutPlugin.some((p) => p.storage === PromptsStorage.plugin),
			);
		});
