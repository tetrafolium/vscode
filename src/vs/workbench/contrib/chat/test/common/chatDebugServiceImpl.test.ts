...
suite("ChatDebugServiceImpl", () => {
	...
	suite("addEvent and getEvents", () => {
		...
		test("should handle different event kinds", () => {
			...
			assert.strictEqual(
				(events[1] as IChatDebugModelTurnEvent).copilotUsageNanoAiu,
				5_000_000_000,
			);
		});
	});
	...
	suite("log", () => {
		test("should create a generic event with defaults", () => {
			...
			assert.strictEqual(
				(event as IChatDebugGenericEvent).details,
				"Some details",
			);
			assert.strictEqual(
				(event as IChatDebugGenericEvent).level,
	...
	suite("MAX_EVENTS_PER_SESSION cap", () => {
		test("should evict oldest events when exceeding per-session cap", () => {
			...
			assert.ok(
				...
				"Should not exceed MAX_EVENTS_PER_SESSION",
			);
			// The first event should have been evicted
			assert.ok(
				!(events as IChatDebugGenericEvent[]).find((e) => e.name === "event-0"),
				"Event-0 should have been evicted",
			);
			// The last event should be present
			assert.ok(
				(events as IChatDebugGenericEvent[]).find(
					(e) => e.name === "event-10000",
				),
		...
		test("should evict oldest session when exceeding MAX_SESSIONS", () => {
			...
			assert.ok(
				!resources.some((r) => r.toString() === sessions[0].toString()),
				"Session-0 should have been evicted",
			);
			assert.strictEqual(
				service.getEvents(sessions[0]).length,
				0,
				"Events from evicted session should be gone",
			);
			// The last session should be present
			assert.ok(
				resources.some((r) => r.toString() === sessions[5].toString()),
				"Session-5 should be present",
			);
		});
		...
		test("should use LRU eviction — recently-used sessions are kept", () => {
			...
			assert.ok(
				resources.some((r) => r.toString() === sessions[0].toString()),
				"Session-0 should be kept (recently used)",
			);
			assert.ok(
				!resources.some((r) => r.toString() === sessions[1].toString()),
				"Session-1 should be evicted (LRU)",
			);
			assert.ok(
				resources.some((r) => r.toString() === session6.toString()),
				"Session-5 should be present",
			);
		});
	});
	...
	suite("registerProvider", () => {
		test("should register and unregister a provider", async () => {
			...
			assert.ok(
				events.some(
					(e) =>
						e.kind === "generic" &&
						(e as IChatDebugGenericEvent).name === "from-provider",
				),
			);
		...
		test("provider returning undefined should not add events", async () => {
			const emptySession = URI.parse(
				"vscode-chat-session://local/empty-session",
			);
			const provider: IChatDebugLogProvider = {
		...
		test("provider errors should be handled gracefully", async () => {
			const errorSession = URI.parse(
				"vscode-chat-session://local/error-session",
			);
			const provider: IChatDebugLogProvider = {
				provideChatDebugLog: async () => {
	...
	suite("invokeProviders", () => {
		test("should invoke multiple providers and merge events", async () => {
			...
			const names = (
				service.getEvents(sessionGeneric) as IChatDebugGenericEvent[]
			).map((e) => e.name);
		...
		test("should fire onDidClearProviderEvents when clearing provider events", async () => {
			...
			disposables.add(
				service.onDidClearProviderEvents((sessionResource) =>
					clearedSessions.push(sessionResource),
				),
			);
			...
			await service.invokeProviders(sessionGeneric);
			assert.strictEqual(
				clearedSessions.length,
				1,
				"Clear event should fire on first invocation",
			);
			...
			assert.strictEqual(
				clearedSessions.length,
				2,
				"Clear event should fire on second invocation",
			);
			assert.strictEqual(
				clearedSessions[1].toString(),
				sessionGeneric.toString(),
			);
		});
		...
		test("should not cancel invocations for different sessions", async () => {
			...
			assert.strictEqual(
				tokenA.isCancellationRequested,
				false,
				"session-a token should not be cancelled",
			);
		});
		...
		test("newly registered provider should be invoked for active sessions", async () => {
			...
			assert.ok(
				lateEvents.length > 0,
				"Late provider should have been invoked",
			);
		});
	});
	...
	suite("resolveEvent", () => {
		...
		test("should return first non-undefined resolution from multiple providers", async () => {
			...
			const provider2: IChatDebugLogProvider = {
				provideChatDebugLog: async () => undefined,
				resolveChatDebugLogEvent: async () => ({
					kind: "text",
					value: "from provider 2",
				}),
			};
	...
	suite("endSession", () => {
		...
		test("late provider should not be invoked for ended session", async () => {
			...
			assert.strictEqual(
				lateCalled,
				false,
				"Late provider should not be invoked for ended session",
			);
		});
	});
