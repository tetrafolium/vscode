...
suite("ChatService", () => {
	...
	function createChatService(): ChatService {
		const service = testDisposables.add(
	...
	function startSessionModel(
		service: IChatService,
		location: ChatAgentLocation = ChatAgentLocation.Chat,
	): IChatModelReference {
		const ref = testDisposables.add(service.startNewLocalSession(location));
		return ref;
	}

	async function getOrRestoreModel(
		service: IChatService,
		resource: URI,
	): Promise<IChatModel | undefined> {
		const ref = await service.acquireOrLoadSession(
			resource,
	...
	setup(async () => {
		...
		instantiationService.stub(IWorkspaceEditingService, {
			onDidEnterWorkspace: Event.None,
		});
		instantiationService.stub(
			IChatDebugService,
			testDisposables.add(new ChatDebugServiceImpl()),
		);
	...
	test("slash commands can share ids across non-overlapping session types", async () => {
		const slashCommandService = testDisposables.add(
			instantiationService.createInstance(ChatSlashCommandService),
		);
		...
		assert.strictEqual(
			slashCommandService.hasCommand("switch", "remote"),
			true,
		);
		assert.strictEqual(
			slashCommandService.hasCommand("switch", "other"),
			false,
		);

		await slashCommandService.executeCommand(
			"switch",
			"",
			progress,
			[],
			ChatAgentLocation.Chat,
			LocalChatSessionUri.forSession("local-session"),
			CancellationToken.None,
		);
		await slashCommandService.executeCommand(
			"switch",
			...
			[],
			ChatAgentLocation.Chat,
			URI.from({ scheme: "remote", path: "/session" }),
			CancellationToken.None,
		);
	...
	test("retrieveSession", async () => {
		...
		const session1Ref = testService.startNewLocalSession(
			ChatAgentLocation.Chat,
		);
		const session1 = session1Ref.object as ChatModel;
		session1.addRequest({ parts: [], text: "request 1" }, { variables: [] }, 0);

		const session2Ref = testService.startNewLocalSession(
			ChatAgentLocation.Chat,
		);
		...
		assert.strictEqual(
			testFileService.writeOperations.length,
			2,
			"Should have written 2 sessions to file service",
		);

		const session1WriteOp = testFileService.writeOperations.find(
			(op: { resource: URI; content: string }) =>
				op.content.includes("request 1"),
		);
		const session2WriteOp = testFileService.writeOperations.find(
			(op: { resource: URI; content: string }) =>
		...
		assert.ok(
			session1WriteOp,
			"Session 1 should have been written to file service",
		);
		assert.ok(
			session2WriteOp,
			"Session 2 should have been written to file service",
		);
		...
		const retrieved1 = await getOrRestoreModel(
			testService2,
			session1.sessionResource,
		);
		const retrieved2 = await getOrRestoreModel(
			testService2,
			session2.sessionResource,
		);
	...
	test("reports modified edit keep-alive holders", () => {
		...
		const rootRef = testService.startNewLocalSession(ChatAgentLocation.Chat, {
			debugOwner: "ChatServiceTest#root",
		});
		...
		assert.deepStrictEqual(
			testService.getChatModelReferenceDebugInfo().models.map((model) => ({
	...
	test("sendRequest allows empty message with explicit file attachment", async () => {
		...
		const modelRef = testDisposables.add(startSessionModel(testService));
		const model = modelRef.object;
		const fileEntry: IChatRequestVariableEntry = {
			kind: "file",
			id: "file",
			name: "README.md",
			value: URI.file("/test/README.md"),
		};
		const response = await testService.sendRequest(model.sessionResource, "", {
			attachedContext: [fileEntry],
		});
		...
		assert.deepStrictEqual(model.getRequests()[0].variableData.variables, [
			fileEntry,
		]);
	});
	...
	test("sendRequest rejects empty message without explicit file attachment", async () => {
		...
		const modelRef = testDisposables.add(startSessionModel(testService));
		const model = modelRef.object;
		const workspaceEntry: IChatRequestVariableEntry = {
			kind: "workspace",
			id: "workspace",
			name: "workspace",
			value: "workspace",
		...
		assert.deepStrictEqual(
			await testService.sendRequest(model.sessionResource, ""),
			{ kind: "rejected", reason: "Empty message" },
		);
		assert.deepStrictEqual(
			await testService.sendRequest(model.sessionResource, "", {
				attachedContext: [workspaceEntry],
			}),
			{ kind: "rejected", reason: "Empty message" },
		);
		assert.strictEqual(model.getRequests().length, 0);
	...
	test("can serialize and deserialize implicit request flag", async () => {
...

		{
			...
			assert.strictEqual(
				serializedChatData.requests[0].isSystemInitiated,
				true,
			);
		}
	...
	test("acquireExistingSession keeps model alive for steering request after refs released", async () => {
		...
		const keepAliveRef = testDisposables.add(
			testService.acquireExistingSession(sessionResource, "test#keepAlive")!,
		);
	...
	test("onDidDisposeSession", async () => {
		...
		let disposed = false;
		testDisposables.add(
			testService.onDidDisposeSession((e) => {
	...
	test("steering message queued triggers setYieldRequested", async () => {
		...
		testDisposables.add(
			chatAgentService.registerAgent("slowAgent", {
				...getAgentData("slowAgent"),
				isDefault: true,
			}),
		);
		testDisposables.add(
			chatAgentService.registerAgentImplementation("slowAgent", slowAgent),
		);
		...
		assert.strictEqual(
			setYieldRequestedCalled,
			true,
			"setYieldRequested should be called when a steering message is queued",
		);
	...
	test("multiple steering messages are combined into a single request", async () => {
		...
		const steering1 = await testService.sendRequest(
			model.sessionResource,
			"steering1",
			{ agentId: "slowAgent", queue: ChatRequestQueueKind.Steering },
		);
		const steering2 = await testService.sendRequest(
			model.sessionResource,
			"steering2",
			{ agentId: "slowAgent", queue: ChatRequestQueueKind.Steering },
		);
		const steering3 = await testService.sendRequest(
			model.sessionResource,
			"steering3",
			{ agentId: "slowAgent", queue: ChatRequestQueueKind.Steering },
		);
		...
		assert.strictEqual(
			invokedRequests.length,
			2,
			"Should have only 2 invocations (initial + combined steering)",
		);
		// The combined message includes all steering texts joined with \n\n
		assert.ok(
			invokedRequests[1].includes("steering1"),
			"Combined message should include steering1",
		);
		assert.ok(
			invokedRequests[1].includes("steering2"),
			"Combined message should include steering2",
		);
		assert.ok(
			invokedRequests[1].includes("steering3"),
			"Combined message should include steering3",
		);
		assert.ok(
			invokedRequests[1].includes("\n\n"),
	...
	test("disabled Claude hooks hint is shown once per workspace (fix for #295079)", async () => {
		...
		assert.strictEqual(
			storageService.getBoolean(disabledHintsKey, StorageScope.WORKSPACE),
			undefined,
		);
		...
		assert.strictEqual(
			storageService.getBoolean(disabledHintsKey, StorageScope.WORKSPACE),
			true,
			"Flag should be set after showing the hint",
		);
		...
		const hasHookHint = responseParts.some(
			(part) => part.kind === "disabledClaudeHooks",
		);
		assert.ok(
			hasHookHint,
			"Response should contain the disabledClaudeHooks hint",
		);
		...
		const hasHookHint2 = responseParts2.some(
			(part) => part.kind === "disabledClaudeHooks",
		);
		assert.ok(
			!hasHookHint2,
			"Response should NOT contain the disabledClaudeHooks hint on second request",
		);
	});
	...
	test("disabled Claude hooks hint is not consumed when no disabled hooks (fix for #295079)", async () => {
		...
		const mockPromptsService = new (class extends MockPromptsService {
			...
			override getHooks(
				...
				return Promise.resolve({
					hooks: {},
					hasDisabledClaudeHooks: this._callCount > 1,
				});
			}
		})();
		instantiationService.stub(IPromptsService, mockPromptsService);
		...
		assert.strictEqual(
			storageService.getBoolean(disabledHintsKey, StorageScope.WORKSPACE),
			undefined,
			"Flag should not be set when no disabled hooks",
		);

		const firstRequest = model.getRequests()[0];
		assert.ok(
			firstRequest,
			"Expected the initial request to exist before resend",
		);
		...
		assert.strictEqual(
			requests.length,
			1,
			"Resend should replace the original request",
		);
		...
		assert.ok(
			hasHookHint2,
			"Response should contain the disabledClaudeHooks hint on second request",
		);
	});
	test("cancelCurrentRequestForSession waits for response completion", async () => {
		...
		await testService.cancelCurrentRequestForSession(
			model.sessionResource,
			"test",
		);
		...
		assert.ok(
			...
			"Response should exist after cancellation completes",
		);
		assert.strictEqual(
			lastRequest.response.state,
			ResponseModelState.Cancelled,
			"Response should be in Cancelled state",
		);
	});
	...
	test("cancelCurrentRequestForSession returns after timeout if response does not complete", async () => {
		...
		testDisposables.add(
			chatAgentService.registerAgent("hangingAgent", {
				...getAgentData("hangingAgent"),
				isDefault: true,
			}),
		);
		testDisposables.add(
	...
	test("pending requests can be removed from one session and re-sent on another", async () => {
		...
		const queued = await testService.sendRequest(
			source.sessionResource,
			"queued request",
			{ agentId: "slowAgent", queue: ChatRequestQueueKind.Queued },
		);
		...
		const resent = await testService.sendRequest(
	...
	test("race condition: processNextPendingRequest dequeues before commit handler runs", async () => {
		...
		const response1 = await testService.sendRequest(
			source.sessionResource,
			"request-1",
			{ agentId: "slowAgent" },
		);
		...
		const q1 = await testService.sendRequest(
			source.sessionResource,
			"queued-1",
			{ agentId: "slowAgent", queue: ChatRequestQueueKind.Queued },
		);
		const q2 = await testService.sendRequest(
			source.sessionResource,
			"queued-2",
			{ agentId: "slowAgent", queue: ChatRequestQueueKind.Queued },
		);
		const q3 = await testService.sendRequest(
			source.sessionResource,
			"queued-3",
			{ agentId: "slowAgent", queue: ChatRequestQueueKind.Queued },
		);
		...
		assert.strictEqual(
			source.getRequests().length,
			1,
			"Only request-1 should be a real request",
		);
		...
		assert.strictEqual(
			source.getPendingRequests().length,
			2,
			"Should have 2 remaining after auto-dequeue",
		);
		...
		const resent1 = await testService.sendRequest(
			target.sessionResource,
			"queued-1",
			{
				agentId: "slowAgent",
		...
		const resent2 = await testService.sendRequest(
			target.sessionResource,
			"queued-2",
			{
				agentId: "slowAgent",
		...
		const resent3 = await testService.sendRequest(
			target.sessionResource,
			"queued-3",
			{
				agentId: "slowAgent",
		...
		assert.strictEqual(
			target.getPendingRequests().length,
			3,
			"Target should have all 3 queued requests",
		);
		...
		const queuedInvocations = invocationOrder.filter((m) =>
			m.includes("queued-"),
		);
		assert.ok(
			queuedInvocations.length >= 3,
			`Expected at least 3 queued invocations, got ${queuedInvocations.length}`,
		);
	...
	test("acquireOrLoadSession returns undefined when remote provider is not registered (fix for #301203)", async () => {
		...
		const testService = createChatService();
		const ref = await testService.acquireOrLoadSession(
			sessionResource,
			ChatAgentLocation.Chat,
			CancellationToken.None,
		);
		assert.strictEqual(
			ref,
			undefined,
			"Should return undefined when no provider is registered",
		);
	});
	...
	test("sendRequest on untitled remote session propagates initialSessionOptions to new model", async () => {
		...
		const untitledResource = URI.from({
			scheme: remoteScheme,
			path: "/untitled-test-session",
		});
		const realResource = URI.from({
			scheme: remoteScheme,
			path: "/real-session-123",
		});
		...
		testDisposables.add(
			mockSessionsService.registerChatSessionContentProvider(remoteScheme, {
		...
		mockSessionsService.setSessionOption(
			untitledResource,
			"model",
			"claude-3.5-sonnet",
		);
		...
		testDisposables.add(
			chatAgentService.registerAgent(remoteScheme, {
				...getAgentData(remoteScheme),
				isDefault: true,
			}),
		);
		testDisposables.add(
			chatAgentService.registerAgentImplementation(remoteScheme, remoteAgent),
		);

		const testService = createChatService();

		// Load the untitled session to create the initial model
		const untitledRef = await testService.acquireOrLoadSession(
			untitledResource,
			ChatAgentLocation.Chat,
			CancellationToken.None,
		);
		...
		const response = await testService.sendRequest(untitledResource, "hello", {
		...
		assert.deepStrictEqual(
			ChatSessionOptionsMap.toStrValueArray(
				mockSessionsService.getSessionOptions(realResource),
			),
			[
	...
	test("sendRequest passes agent host session capabilities to the request parser", async () => {
		...
		testDisposables.add(
			mockSessionsService.registerChatSessionContentProvider(sessionType, {
		...
		const promptsService = mockObject<IPromptsService>()({
			_serviceBrand: undefined,
		});
		promptsService.isValidSlashCommandName.callsFake(
			(command: string) => command === "skill",
		);
		instantiationService.stub(IPromptsService, promptsService);

		testDisposables.add(
			chatAgentService.registerAgent(sessionType, {
				...getAgentData(sessionType),
				isDefault: true,
			}),
		);
		testDisposables.add(
			chatAgentService.registerAgentImplementation(sessionType, {
				async invoke() {
					return {};
				},
			}),
		...
		assert.deepStrictEqual(
			model.getRequests()[0].message.parts.map((part) => ({
				type: part.constructor.name,
				text:
					part instanceof ChatRequestSlashPromptPart ? part.name : undefined,
			})),
			[
	...
	test("sendRequest redacts remote session type in provider invoked telemetry", async () => {
		...
		const response = await testService.sendRequest(sessionResource, "hello", {
			agentId: sessionType,
		});
		ChatSendResult.assertSent(response);
		await response.data.responseCompletePromise;
		...
		assert.deepStrictEqual(
			providerInvokedEvents.map((event) => event.sessionType),
			["remote-agent-host"],
		);
	});
	...
	test("loadRemoteSession passes agent host session capabilities to the request parser", async () => {
		...
		assert.deepStrictEqual(
			ref.object.getRequests()[0].message.parts.map((part) => ({
				type: part.constructor.name,
				text:
					part instanceof ChatRequestSlashPromptPart ? part.name : undefined,
			})),
	...
	test("troubleshoot skill via attachedContext is blocked when fileLogging.enabled is off", async () => {
		...
		testDisposables.add(
			chatAgentService.registerAgent("troubleshootAgent", {
				...getAgentData("troubleshootAgent"),
				isDefault: true,
			}),
		);
		testDisposables.add(
		...
		const skillUri = URI.from({
			scheme: COPILOT_SKILL_URI_SCHEME,
			path: TROUBLESHOOT_SKILL_PATH,
		});
		const response = await testService.sendRequest(
			model.sessionResource,
		...
		assert.ok(
			responseContent?.includes(AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING),
			"Response should mention the fileLogging setting",
		);
	});
	...
	test("troubleshoot skill via attachedContext proceeds when fileLogging.enabled is on", async () => {
				...
				progress([
					{
						...
						content: new MarkdownString("Troubleshooting complete"),
					},
				]);
				return {};
			},
		...
		testDisposables.add(
			chatAgentService.registerAgent("troubleshootAgent2", {
				...getAgentData("troubleshootAgent2"),
				isDefault: true,
			}),
		);
		testDisposables.add(
		...
		assert.ok(
			!responseContent?.includes(AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING),
			"Response should not contain the settings gate message",
		);
	});
	...
	test("switching between sessions disposes previous models and releases all references", async () => {
		...
		for (let i = 0; i < 3; i++) {
			...
			model.addRequest(
				{ parts: [], text: `request in session ${i}` },
				{ variables: [] },
				0,
			);
		...
		for (const s of sessions) {
			assert.strictEqual(
				testService.getSession(s.resource),
				undefined,
				`Session ${s.resource} should be disposed after ref release`,
			);
		}
		...
		for (const s of sessions) {
			const newRef = await testService.acquireOrLoadSession(
				s.resource,
				ChatAgentLocation.Chat,
				CancellationToken.None,
				"test#switch",
		...
		assert.strictEqual(
			...
			undefined,
			"Session 0 model should be disposed",
		);
		assert.strictEqual(
			testService.getSession(sessions[1].resource),
			undefined,
			"Session 1 model should be disposed",
		);
		assert.ok(
			testService.getSession(sessions[2].resource),
			"Session 2 model should still be alive",
		);
	...
	test("previousModelRef pattern in ChatViewPane does not cause double-reference retention", async () => {
		...
		// Load session 0
		const ref0 = await testService.acquireOrLoadSession(
			sessions[0].resource,
			ChatAgentLocation.Chat,
			CancellationToken.None,
			"test",
		...
		// Load session 1
		const ref1 = await testService.acquireOrLoadSession(
			sessions[1].resource,
			ChatAgentLocation.Chat,
			CancellationToken.None,
			"test",
	...
	test("serializer _previous field does not retain data after model disposal", async () => {
		...
		model.addRequest(
			{ parts: [], text: "some request with data" },
			{ variables: [] },
			0,
		);
		...
		const ref2 = await testService.acquireOrLoadSession(
			sessionResource,
			ChatAgentLocation.Chat,
			CancellationToken.None,
			"test",
		...
		assert.ok(
			model2.dataSerializer,
			"Restored model should have a dataSerializer",
		);

		ref2.dispose();
		await testService.waitForModelDisposals();
		assert.strictEqual(
			testService.getSession(sessionResource),
			undefined,
			"Model should be disposed after second cycle",
		);
	});
	...
	test("model becomes unreachable after all references released", async () => {
		...
		assert.strictEqual(
			testService.getSession(sessionResource),
			undefined,
			"Model should be gone from store",
		);
		...
		if (typeof globalThis.gc === "function") {
			...
			assert.strictEqual(
				weakModel.deref(),
				undefined,
				"Model should be GC'd after all references released",
			);
		}
	});
	...
	test("rapid session switching accumulates at most 2 live models", async () => {
		...
		assert.strictEqual(
			finalDebugInfo.totalModels,
			1,
			"Should have exactly 1 model after waiting for disposals",
		);
	...
	suite("loadRemoteSession progress streaming", () => {
		...
		interface IProvidedSessionOptions {
			...
			readonly onDidStartServerRequest?: Event<{
				prompt: string;
		...
		function setupRemoteProvider(opts: IProvidedSessionOptions): {
			resource: URI;
			provided: IChatSession;
		} {
			...
			testDisposables.add(
				chatAgentService.registerAgentImplementation(remoteScheme, {
					async invoke() {
						return {};
					},
				}),
			...
			const provided: IChatSession = {
				...
				history: opts.history ?? [
					{ type: "request", prompt: "hello", participant: remoteScheme },
				],
				onWillDispose: Event.None,
		...
		test("already-complete session at load time: no initial pending request, response is completed via autorun", async () => {
			...
			assert.strictEqual(
				...
				true,
				"Response should be completed through the isComplete autorun",
			);
			...
			assert.strictEqual(
				interruptCalls,
				0,
				"Interrupt callback should not be invoked when there is no pending request",
			);
		});
		...
		test("active session at load time: cancelCurrentRequestForSession invokes the interrupt callback", async () => {
			...
			const model = ref.object as ChatModel;
			assert.strictEqual(
				model.lastRequest?.response?.isComplete,
				false,
				"Response must stay open while session is active",
			);
			...
			assert.strictEqual(
				interruptCalls,
				1,
				"Interrupt callback should be invoked once",
			);
		});
		...
		test("transition of isCompleteObs to true clears pending request and completes response", async () => {
			...
			assert.strictEqual(
				lastRequest.response?.isComplete,
				true,
				"Response should complete when isCompleteObs transitions to true",
			);
			...
			assert.strictEqual(
				interruptCalls,
				0,
				"Interrupt should not fire after the turn has completed",
			);
		});
		...
		test("interrupt callback returning false installs a fresh pending request so cancel can be retried", async () => {
			...
			assert.strictEqual(
				interruptInvocations.length,
				2,
				"Interrupt callback should be invoked on both cancel attempts",
			);
		});
		...
		test("non-streaming session with isCompleteObs=true at load: response completes synchronously", async () => {
			...
			assert.strictEqual(
				model.lastRequest?.response?.isComplete,
				true,
				"Non-streaming session should complete response at load time",
			);
		});
	...
	test("onWillSaveState persists session index synchronously so it survives reload", async () => {
		const testService = createChatService();
		const storageService = instantiationService.get(
			IStorageService,
		) as TestStorageService;
		...
		assert.ok(
			historyItems.some(
				(item) =>
					item.sessionResource.toString() === model.sessionResource.toString(),
			),
	...
	test("removeHistoryEntry marks model as deleted and excludes from getLiveSessionItems", async () => {
		...
		const response = await testService.sendRequest(
			model.sessionResource,
			`@${chatAgentWithMarkdownId} test request`,
		);
		ChatSendResult.assertSent(response);
		await response.data.responseCompletePromise;
		assert.strictEqual(model.getRequests().length, 1);
	...
	return {
		...
		requests: exp.requests.map((r) => {
			...
			const {
				...
				systemInitiatedLabel: _systemInitiatedLabel,
