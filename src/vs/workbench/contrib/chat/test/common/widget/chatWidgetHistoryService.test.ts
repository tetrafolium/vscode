...
suite("ChatWidgetHistoryService", () => {
	...
	function createHistoryService(): ChatWidgetHistoryService {
		...
		return testDisposables.add(
			instantiationService.createInstance(ChatWidgetHistoryService),
		);
	}
	...
	test("should maintain separate history per location", () => {
		...
		historyService.append(
			...
			createInputState("chat query"),
		);
		historyService.append(
			ChatAgentLocation.Terminal,
			createInputState("terminal query"),
		);

		const chatHistory = historyService.getHistory(ChatAgentLocation.Chat);
		const terminalHistory = historyService.getHistory(
			ChatAgentLocation.Terminal,
		);
	...
	test("should limit history to max entries", () => {
		...
		assert.strictEqual(
			history[history.length - 1].inputText,
			`query ${ChatInputHistoryMaxEntries + 9}`,
		);
	});
	...
	test("should fire append event when history is added", () => {
		...
		testDisposables.add(
			historyService.onDidChangeHistory((e) => {
				if (e.kind === "append") {
					eventFired = true;
					firedEntry = e.entry;
				}
	...
	test("should clear all history", () => {
		...
		historyService.append(
			ChatAgentLocation.Terminal,
			createInputState("query 2"),
		);

		historyService.clearHistory();

		assert.strictEqual(
			historyService.getHistory(ChatAgentLocation.Chat).length,
			0,
		);
		assert.strictEqual(
			historyService.getHistory(ChatAgentLocation.Terminal).length,
			0,
		);
	});
...
suite("ChatHistoryNavigator", () => {
	...
	function createNavigator(): ChatHistoryNavigator {
		...
		const historyService = testDisposables.add(
			instantiationService.createInstance(ChatWidgetHistoryService),
		);
	...
	test("should support concurrent navigators", () => {
		...
		const nav1 = testDisposables.add(
			instantiationService.createInstance(
				ChatHistoryNavigator,
				ChatAgentLocation.Chat,
			),
		);
		const nav2 = testDisposables.add(
			instantiationService.createInstance(
				ChatHistoryNavigator,
				ChatAgentLocation.Chat,
			),
