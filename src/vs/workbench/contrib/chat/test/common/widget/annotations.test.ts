...
suite("Annotations", function () {
	...
	suite("extractSubAgentInvocationIdFromText", () => {
		test("extracts subAgentInvocationId from codeblock uri tag", () => {
			...
			const annotated = annotateSpecialMarkdownContent([
				content("code"),
				codeblockUriPart,
			]);
			const markdown = annotated[0] as IChatMarkdownContent;

			const result = extractSubAgentInvocationIdFromText(
		...
		test("handles malformed URL encoding gracefully", () => {
			...
			const malformedTag =
				'<vscode_codeblock_uri isEdit subAgentInvocationId="%ZZ">file:///test.ts</vscode_codeblock_uri>';
	...
	suite("extractCodeblockUrisFromText with subAgentInvocationId", () => {
		...
		test("returns undefined for invalid URI content inside codeblock uri tag", () => {
			...
			const invalidTag =
				"<vscode_codeblock_uri>```typescript\nconst uri: string\n```</vscode_codeblock_uri>";
	...
	suite(
		"annotateSpecialMarkdownContent - inline references in code blocks",
		() => {
			test("inline reference inside backtick code span uses plain text", () => {
				const result = annotateSpecialMarkdownContent([
					content("Run `npx tsx "),
					{
						...
						inlineReference: URI.parse("file:///index.ts"),
						name: "index.ts",
					},
					content(" eval "),
					{
						kind: "inlineReference",
						inlineReference: URI.parse("file:///primer.eval.json"),
						name: "primer.eval.json",
					},
					content(" --repo .`"),
				]);
				...
				assert.strictEqual(
					md.content.value,
					"Run `npx tsx index.ts eval primer.eval.json --repo .`",
				);
			...
			test("inline reference at start of block merges with following markdown", () => {
				const result = annotateSpecialMarkdownContent([
					...
					},
					{
						...
						content: new MarkdownString(" is the entry point", {
							isTrusted: true,
			...
			test("inline reference after regular text does not force-merge incompatible markdown", () => {
				const result = annotateSpecialMarkdownContent([
					...
					{
						kind: "markdownContent",
						content: new MarkdownString(" more info", {
							isTrusted: true,
							supportThemeIcons: true,
						}),
					},
	...
	suite("hasEditCodeblockUriTag", () => {
		test("returns true for edit codeblock URI tags", () => {
			const editTag =
				"<vscode_codeblock_uri isEdit>file:///test.ts</vscode_codeblock_uri>";
		...
		test("returns true for edit codeblock URI tags with subAgentInvocationId", () => {
			const editTagWithSubAgent =
				'<vscode_codeblock_uri isEdit subAgentInvocationId="agent-123">file:///test.ts</vscode_codeblock_uri>';
		...
		test("returns false for non-edit codeblock URI tags with subAgentInvocationId", () => {
			const nonEditTagWithSubAgent =
				'<vscode_codeblock_uri subAgentInvocationId="agent-123">file:///test.ts</vscode_codeblock_uri>';
		...
		test("returns true for text containing multiple edit codeblock URI tags", () => {
			const multipleEditTags =
				"some text <vscode_codeblock_uri isEdit>file:///test.ts</vscode_codeblock_uri> more <vscode_codeblock_uri isEdit>file:///other.ts</vscode_codeblock_uri>";
		...
		test("returns false for text containing only non-edit codeblock URI tags", () => {
			const multipleNonEditTags =
				"some text <vscode_codeblock_uri>file:///test.ts</vscode_codeblock_uri> more <vscode_codeblock_uri>file:///other.ts</vscode_codeblock_uri>";
