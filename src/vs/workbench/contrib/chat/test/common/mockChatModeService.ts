	...
	constructor(
		...
		this._modesView = {
			...
			findModeById(id: string): IChatMode | undefined {
				return (
					modes.builtin.find((mode) => mode.id === id) ??
					modes.custom.find((mode) => mode.id === id)
				);
			},
			findModeByName(name: string): IChatMode | undefined {
				return (
					modes.builtin.find((mode) => mode.name.get() === name) ??
					modes.custom.find((mode) => mode.name.get() === name)
				);
			},
