...
} from "../../../browser/attachments/chatVariables.js";
...
function createMockVariable(
	overrides?: Partial<IDynamicVariable>,
): IDynamicVariable {
	return {
		id: "var-1",
		fullName: "test-var",
...
function createMockAttachment(
	overrides?: Partial<IChatRequestVariableEntry>,
...
function createMockWidget(options: {
	...
	return {
		...
		getContrib: (id: string) =>
			id === ChatDynamicVariableModel.ID ? contribModel : undefined,
		input: {
...
suite("getDynamicVariablesForWidget", () => {
	...
	test("skips attachments with empty range", () => {
		const attachments = [
			createMockAttachment({ range: { start: 5, endExclusive: 5 } }),
		];
		const widget = createMockWidget({
			editing: true,
			attachments,
	...
	test("skips attachments with out-of-bounds range", () => {
		const attachments = [
			createMockAttachment({ range: { start: 0, endExclusive: 200 } }),
		];
		const widget = createMockWidget({
			editing: true,
			attachments,
	...
	test("skips attachments with negative start", () => {
		const attachments = [
			createMockAttachment({ range: { start: -1, endExclusive: 5 } }),
		];
		const widget = createMockWidget({
			editing: true,
			attachments,
...
suite("getSelectedToolAndToolSetsForWidget", () => {
	...
	test("returns the entriesMap from the selected tools model", () => {
		...
		const expectedMap = new Map<IToolData | IToolSet, boolean>([
			[toolData, true],
		]);
