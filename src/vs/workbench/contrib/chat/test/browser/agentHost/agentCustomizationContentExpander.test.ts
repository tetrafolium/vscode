...
function expand(
	expander: AgentCustomizationContentExpander,
	pluginUri: URI,
	groupKey: string,
	...
	return expander.expandPluginContents(
		pluginUri,
		groupKey,
		isBundleItem,
		source,
...
suite("AgentCustomizationContentExpander", () => {
	...
	suite("expandPluginContents – skills", () => {
		test("emits one item per subfolder that has a SKILL.md, skips folders without one", async () => {
			...
			const expander = new AgentCustomizationContentExpander(
				fileService,
				new NullLogService(),
			);
		...
		test("rewrites skill folder URI to point at SKILL.md", async () => {
			...
			assert.strictEqual(items.length, 1);
			assert.ok(
				items[0].uri.path.endsWith("/SKILL.md"),
				`expected SKILL.md URI, got ${items[0].uri}`,
			);
		});
			...
			await mockFiles(fileService, [
				{
					...
					contents: ["---", "name: Invocable", "user-invocable: true", "---"],
				},
				{
					path: "/plugins/r/skills/silent/SKILL.md",
					contents: ["---", "name: Silent", "---"],
				},
			]);
	...
	suite("expandPluginContents – rules", () => {
		test("emits one item per .md file with name/description from frontmatter", async () => {
			...
			const ruleItems = items.filter(
				(i) => i.type === PromptsType.instructions,
			);
			assert.deepStrictEqual(
				ruleItems
		...
		test("userInvocable is NOT surfaced for rules", async () => {
			...
			await mockFiles(fileService, [
				{
					...
					contents: ["---", "name: My Rule", "user-invocable: true", "---"],
				},
			]);
			...
			assert.strictEqual(
				ruleItems[0].userInvocable,
				undefined,
				"rules must not expose userInvocable",
			);
		});
		...
		test("emits one item per .mdc file per the Open Plugins spec", async () => {
			...
			await mockFiles(fileService, [
				{
					path: "/plugins/rules-mdc/rules/style.mdc",
					contents: ["Some rule content"],
				},
				{
					path: "/plugins/rules-mdc/rules/other.mdc",
					contents: ["Another rule"],
				},
				// `.txt` and similar must still be ignored
				{
					path: "/plugins/rules-mdc/rules/readme.txt",
					contents: ["not a rule"],
				},
			]);
			...
			assert.deepStrictEqual(ruleItems.map((i) => i.name).sort(), [
				"other",
				"style",
			]);
		});
	...
	suite("expandPluginContents – commands", () => {
		test("emits one item per .md file, name from filename (no frontmatter parsing)", async () => {
			...
			await mockFiles(fileService, [
				{
					...
					contents: ["---", "name: Fix It", "---", "Fix the code"],
				},
				{
			...
			assert.deepStrictEqual(cmdItems.map((i) => i.name).sort(), [
				"fix",
				"review",
			]);
	...
	suite("expandPluginContents – mixed plugin", () => {
		test("all four folder types are discovered and returned together", async () => {
			...
			const byType = (t: PromptsType) =>
				items.filter((i) => i.type === t).map((i) => i.name);
	...
	suite("expandPluginContents – groupKey and pluginUri", () => {
		test("all child items carry the groupKey passed to expand", async () => {
			...
			for (const item of items) {
				assert.strictEqual(
					item.groupKey,
					REMOTE_CLIENT_GROUP,
					`item ${item.name} should carry remote-client groupKey`,
				);
			}
		});
		...
		test("isBundleItem=true clears pluginUri on child items", async () => {
			...
			const bundleItems = await expand(
				expander,
				pluginRoot,
				REMOTE_CLIENT_GROUP,
				true /* isBundleItem */,
			...
			for (const item of bundleItems) {
				assert.strictEqual(
					item.pluginUri,
					undefined,
					`bundle item ${item.name} must have no pluginUri`,
				);
			}
		});
