...
} from "../../../../../../platform/agentHost/common/agentFeedbackAttachments.js";
import {
	ActionType,
	isSessionAction,
	type ActionEnvelope,
	...
	type TerminalAction,
	type INotification,
	type IToolCallConfirmedAction,
	type ITurnStartedAction,
...
import {
	CompletionItemKind as AhpCompletionItemKind,
...
} from "../../../browser/agentSessions/agentHost/agentHostChatContribution.js";
...
} from "../../../browser/agentSessions/agentHost/agentHostCustomizationService.js";
...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	private readonly _authenticationPending: ISettableObservable<boolean> =
		observableValue("authenticationPending", false);
	override readonly authenticationPending: IObservable<boolean> =
		this._authenticationPending;
	...
	private readonly _liveSubscriptions = new Map<
		string,
		{
			state: SessionState;
			emitter: Emitter<SessionState>;
			onWillApply: Emitter<ActionEnvelope>;
			onDidApply: Emitter<ActionEnvelope>;
		}
	>();
	...
	override async createSession(
		...
		const session =
			config?.session ??
			AgentSession.uri("copilot", `sdk-session-${this._nextId++}`);
		...
		if (config?.activeClient) {
			const summary: SessionSummary = {
				...
				modifiedAt: Date.now(),
				workingDirectory: (
					this.nextResolvedWorkingDirectory ?? config.workingDirectory
				)?.toString(),
			};
	...
	get turnActions() {
		return this.dispatchedActions.filter(
			(d) => d.action.type === "session/turnStarted",
		);
	}
	...
	override getSubscription<T>(
		_kind: StateComponents,
		resource: URI,
	): IReference<IAgentSubscription<T>> {
		const resourceStr = resource.toString();
		...
		const entry = {
			state: initialState,
			emitter: emitter as unknown as Emitter<SessionState>,
			onWillApply,
			onDidApply,
		};
		...
		const sub: IAgentSubscription<T> = {
			get value() {
				return self._liveSubscriptions.get(resourceStr)?.state as unknown as T;
			},
			get verifiedValue() {
				return self._liveSubscriptions.get(resourceStr)?.state as unknown as T;
			},
	...
	override getSubscriptionUnmanaged<T>(
		_kind: StateComponents,
		resource: URI,
	): IAgentSubscription<T> | undefined {
		...
		return {
			get value() {
				return self._liveSubscriptions.get(resource.toString())
	...
	override dispatch(
		...
		if (
			...
			action.type === "session/activeClientChanged"
		) {
			const entry = this._liveSubscriptions.get(channel.toString());
			if (entry) {
				const noop = () => {};
				entry.state = sessionReducer(
					entry.state,
					action as Parameters<typeof sessionReducer>[1],
					noop,
				);
	...
	fireAction(envelope: ActionEnvelope): void {
		...
		if (isSessionAction(envelope.action)) {
			const sessionUri = envelope.channel;
			...
			if (entry) {
				...
				entry.state = sessionReducer(
					entry.state,
					envelope.action as Parameters<typeof sessionReducer>[1],
					noop,
				);
				entry.emitter.fire(entry.state);
...
class MockChatAgentService extends mock<IChatAgentService>() {
	...
	registeredAgents = new Map<
		string,
		{ data: IChatAgentData; impl: IChatAgentImplementation }
	>();

	override registerDynamicAgent(
		data: IChatAgentData,
...
class MockChatWidgetService extends mock<IChatWidgetService>() {
	...
	readonly clearQuestionCarouselCalls: {
		sessionResource: URI;
	...
	private readonly _widgets = new Map<
		string,
		ReturnType<IChatWidgetService["getWidgetBySessionResource"]>
	>();
		...
		this._widgets.set(sessionResource.toString(), {
			input: {
				clearQuestionCarousel: (responseId?: string, resolveId?: string) => {
					this.clearQuestionCarouselCalls.push({
						sessionResource,
						responseId,
						resolveId,
					});
...
function createTestServices(
	...
	const openerService: {
		openedUrls: (string | URI)[];
		openShouldFail: boolean;
		openResult: boolean;
	} & Partial<IOpenerService> = {
		openedUrls: [],
	...
	instantiationService.stub(IConfigurationService, {
		...
		getValue: (...args: any[]) =>
			typeof args[0] === "string" && args[0] === "chat.agentHost.clientTools"
				? []
				: true,
	});
	...
	const chatService = {
		getSession: (sessionResource: URI) =>
			chatModels.get(sessionResource.toString()),
	...
	instantiationService.stub(IAgentHostSessionWorkingDirectoryResolver, {
		...
		isNewSession: (sessionResource) =>
			workingDirectoryResolver?.isNewSession?.(sessionResource) ??
			sessionResource.path.substring(1).startsWith("new-"),
	});
	...
	const customizationsByType = new Map<
		string,
		IObservable<readonly ClientPluginCustomization[]>
	>();
	const seedActiveClient = (
		sessionType: string,
		entry: {
			customizations: IObservable<readonly ClientPluginCustomization[]>;
		},
	): IDisposable => {
	...
	const activeClientService: IAgentHostActiveClientService = {
		...
		getCustomizations: (sessionType: string) =>
			derived(
				(reader) => customizationsByType.get(sessionType)?.read(reader) ?? [],
			),
...
function createContribution(
	...
	const listController = disposables.add(
	...
	const sessionHandler = disposables.add(
		instantiationService.createInstance(AgentHostSessionHandler, {
			provider: "copilot" as const,
			...
			isNewSession: (sessionResource) =>
				listController.isNewSession(sessionResource),
		}),
	);
	const contribution = disposables.add(
		instantiationService.createInstance(AgentHostContribution),
	);
...
function makeRequest(
	...
	return upcastPartial<IChatAgentRequest>({
		sessionResource:
			overrides.sessionResource ??
			URI.from({ scheme: "untitled", path: "/chat-1" }),
		requestId: "req-1",
...
function textOf(
	value: string | IMarkdownString | undefined,
): string | undefined {
	if (value === undefined) {
		return undefined;
	}
...
async function startTurn(
	...
	const sessionResource =
		overrides?.sessionResource ??
		URI.from({ scheme: agentId, path: "/new-turntest" });
	...
	const turnDispatches = agentHostService.dispatchedActions.filter(
		(d) => d.action.type === "session/turnStarted",
	);
	const lastDispatch =
		turnDispatches[turnDispatches.length - 1] ??
		agentHostService.dispatchedActions[
			agentHostService.dispatchedActions.length - 1
		];
	...
	if (lastDispatch) {
		agentHostService.fireAction({
			...
			origin: {
				...
				clientSeq: lastDispatch.clientSeq,
			},
		});
	}
...
suite("AgentHostChatContribution", () => {
	...
	suite("disposal", () => {
		test("fires onWillDispose before session is disposed", async () => {
			...
			assert.strictEqual(
				fired,
				1,
				"onWillDispose should fire exactly once when the session is disposed",
			);
		});
	});
	...
	suite("session list", () => {
		test("refresh populates items from agent host", async () => {
			const { listController, agentHostService } =
				createContribution(disposables);

			agentHostService.addSession({
				session: AgentSession.uri("copilot", "aaa"),
				startTime: 1000,
				modifiedTime: 2000,
				summary: "My session",
			});
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "bbb"),
				startTime: 3000,
				modifiedTime: 4000,
			});
			...
			assert.strictEqual(
				listController.items[0].resource.scheme,
				"agent-host-copilot",
			);
		...
		test("refresh fires onDidChangeChatSessionItems", async () => {
			...
			let fired = false;
			disposables.add(
				listController.onDidChangeChatSessionItems(() => {
					fired = true;
				}),
			);

			agentHostService.addSession({
				session: AgentSession.uri("copilot", "x"),
				startTime: 1000,
				modifiedTime: 2000,
			});
			await listController.refresh(CancellationToken.None);
		...
		test("refresh skips listSessions RPC after first successful call", async () => {
			...
			const originalListSessions =
				agentHostService.listSessions.bind(agentHostService);
			agentHostService.listSessions = async () => {
				listCalls++;
		...
		test("refresh filters out sessions whose workingDirectory is not in any workspace folder", async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "in-ws"),
				startTime: 1000,
				modifiedTime: 2000,
				summary: "In workspace",
				workingDirectory: URI.file("/workspace/root/sub"),
			});
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "out-ws"),
				startTime: 1000,
				modifiedTime: 2000,
				summary: "Outside workspace",
				workingDirectory: URI.file("/other/place"),
			});
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "no-wd"),
				startTime: 1000,
				modifiedTime: 2000,
			...
			assert.deepStrictEqual(
				listController.items.map((item) => item.label),
				["In workspace"],
			);
		});
		...
		test("refresh does not filter when no workspace folders are open", async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "a"),
				startTime: 1000,
				modifiedTime: 2000,
				summary: "A",
				workingDirectory: URI.file("/any/path"),
			});
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "b"),
				startTime: 1000,
				modifiedTime: 2000,
				summary: "B",
			});
		...
		test("workspace folder change re-fetches and updates the filtered session list", async () => {
			...
			const onDidChangeWorkspaceFolders = disposables.add(
				new Emitter<{
					readonly added: never[];
		...
		test("newChatSessionItem creates final-looking resource used for requested backend session", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				const item = await listController.newChatSessionItem(
				...
				assert.strictEqual(
					listController.items.some(
						(existing) =>
							existing.resource.toString() === item.resource.toString(),
					),
					false,
				);

				const { turnPromise, session, turnId, fire } = await startTurn(
					sessionHandler,
					agentHostService,
					chatAgentService,
					disposables,
				...
				assert.strictEqual(
					agentHostService.createSessionCalls[0].session?.toString(),
					AgentSession.uri(
						"copilot",
						item.resource.path.substring(1),
					).toString(),
				);
		...
		test("newChatSessionItem rebinds untitled provisional to real resource so chip-selected config survives first send", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { instantiationService, agentHostService } =
					createTestServices(disposables);

				const workspaceFolder = URI.from({
					scheme: "file",
					path: "/workspace/root",
				});
				instantiationService.stub(IWorkspaceContextService, {
		...
		test("newChatSessionItem skips rebind when no untitled provisional resource is provided", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { instantiationService, agentHostService } =
					createTestServices(disposables);
	...
	suite("session ID resolution", () => {
		test("requests backend session for provider-owned new resource", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.strictEqual(
					agentHostService.turnActions[0].action.type,
					"session/turnStarted",
				);
				assert.strictEqual(
					(agentHostService.turnActions[0].action as ITurnStartedAction).message
						.text,
					"Hello",
				);
		...
		test("reuses SDK session for same resource on second message", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const registered =
					chatAgentService.registeredAgents.get("agent-host-copilot")!;
				...
				agentHostService.fireAction({
					channel: dispatch1.channel.toString(),
					action: dispatch1.action,
					serverSeq: 1,
					origin: {
						clientId: agentHostService.clientId,
						clientSeq: dispatch1.clientSeq,
					},
				});
				agentHostService.fireAction({
					channel: dispatch1.channel.toString(),
				...
				agentHostService.fireAction({
					channel: dispatch2.channel.toString(),
					action: dispatch2.action,
					serverSeq: 3,
					origin: {
						clientId: agentHostService.clientId,
						clientSeq: dispatch2.clientSeq,
					},
				});
				agentHostService.fireAction({
					channel: dispatch2.channel.toString(),
		...
		test("uses sessionId from agent-host scheme resource", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.strictEqual(
					AgentSession.id(URI.parse(session)),
					"existing-session-42",
				);
			}));

		test("recovers from stale failed subscription before first send", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const sessionResource = URI.from({
					scheme: "agent-host-copilot",
					path: "/existing-subscribe-retry",
				});
				const backendSession = AgentSession.uri(
					"copilot",
					"existing-subscribe-retry",
				);
				const { agentHostService, chatAgentService } = createContribution(
					disposables,
					{
						provisionalServiceOverride: {
							get: (resource) =>
								resource.toString() === sessionResource.toString()
									? backendSession
									: undefined,
						},
					},
				...
				assert.deepStrictEqual(
					agentHostService.turnActions.map(
						(d) => (d.action as ITurnStartedAction).message.text,
					),
					["Recovered"],
				);
			}));
		...
		test("rejects generic contributed-chat untitled resource", async () => {
			...
			assert.strictEqual(
				chatAgentService.registeredAgents.has("agent-host-copilot"),
				true,
			);
		});
		test("passes raw model id extracted from language model identifier", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("passes selected model configuration through create session", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("passes model id as-is when no vendor prefix", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("does not create backend session eagerly for untitled sessions", async () => {
			const { sessionHandler, agentHostService } =
				createContribution(disposables);
	...
	suite("progress routing", () => {
		test("delta events become markdownContent progress", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);

				const { turnPromise, collected, session, turnId, fire } =
					await startTurn(
						sessionHandler,
						agentHostService,
						chatAgentService,
				...
				fire({
					...
					session,
					turnId,
					part: { kind: "markdown", id: "md-1", content: "hello " },
				} as SessionAction);
				fire({
					type: "session/delta",
					session,
				...
				const markdownParts = collected
					.flat()
					.filter(
						(p): p is IChatMarkdownContent => p.kind === "markdownContent",
					);
				const totalContent = markdownParts.map((p) => p.content.value).join("");
				assert.strictEqual(totalContent, "hello world");
			}));

		test("live turn marks chat session complete after turnComplete", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);

				const { turnPromise, chatSession, session, turnId, fire } =
					await startTurn(
						sessionHandler,
						agentHostService,
						chatAgentService,
				...
				assert.strictEqual(
					...
					true,
					"should be complete after turn finishes",
				);
			}));

		test("live turn returns model credit details from usage", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const languageModels = new Map<string, ILanguageModelChatMetadata>([
					[
						"agent-host-copilot:opus-4.7",
				...
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables, { languageModels });
		...
		test("live turn emits token usage as chat usage progress", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const usageParts = collected
					.flat()
					.filter((p): p is IChatUsage => p.kind === "usage");
				assert.deepStrictEqual(
					usageParts.map((part) => ({
						kind: part.kind,
		...
		test("tool_start events become toolInvocation progress", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("tool_complete event transitions toolInvocation to completed", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("tool_complete with failure sets error state", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("outstanding tool invocations are completed on idle", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("events from other sessions are ignored", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				const { turnPromise, collected, turnId, fire } = await startTurn(
					sessionHandler,
					agentHostService,
					chatAgentService,
					disposables,
				...
				fire({
					type: "session/responsePart",
					turnId,
					part: { kind: "markdown", id: "md-1", content: "right" },
				} as SessionAction);
				fire({ type: "session/turnComplete", turnId } as SessionAction);

				await turnPromise;

				assert.strictEqual(collected.length, 1);
				assert.strictEqual(
					(collected[0][0] as IChatMarkdownContent).content.value,
					"right",
				);
			}));

		test("input request completion from another client clears local question carousel", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
				...
				const carousel = collected
					.flat()
					.find((part) => part.kind === "questionCarousel");
				...
				assert.deepStrictEqual(
					chatWidgetService.clearQuestionCarouselCalls.map((call) => ({
						responseId: call.responseId,
						resolveId: call.resolveId,
					})),
				...
				assert.ok(
					carousel instanceof ChatQuestionCarouselData,
					"AgentHost input request should use runtime carousel data",
				);
				...
				assert.strictEqual(
					agentHostService.dispatchedActions.some(
						(dispatched) =>
							dispatched.action.type === ActionType.SessionInputCompleted,
					),
					false,
				);

				fire({ type: ActionType.SessionTurnComplete, turnId });
				await turnPromise;
			}));

		test("input request completion echo applies authoritative answers after local submit", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
				...
				const sessionResource = URI.from({
					scheme: "agent-host-copilot",
					path: "/new-local-input-request-test",
				});
				chatWidgetService.setWidgetForSession(sessionResource);
				...
				assert.deepStrictEqual(
					chatWidgetService.clearQuestionCarouselCalls,
					[],
				);
				assert.strictEqual(
					agentHostService.dispatchedActions.some(
		...
		test("input request cancellation does not show draft answers as submitted", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
				...
				const sessionResource = URI.from({
					scheme: "agent-host-copilot",
					path: "/new-cancelled-input-request-test",
				});
				chatWidgetService.setWidgetForSession(sessionResource);
		...
		test("url-style input request renders an elicitation part with the URL", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const part = collected
					.flat()
					.find((p) => (p as { kind?: string }).kind === "elicitation2") as
				...
				assert.ok(
					messageText.includes("authorize"),
					"message should include the request message",
				);
				assert.ok(
					messageText.includes("https://example.com/auth?token=abc"),
					"message should include the URL",
				);
				assert.ok(
					part.acceptButtonLabel.includes("example.com"),
					"accept button should reference the URL authority",
				);
				assert.strictEqual(
					collected.flat().some((p) => p.kind === "questionCarousel"),
					false,
					"url-style requests must not also render a question carousel",
				);
		...
		test("url input request accept opens URL and dispatches Accept", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
				...
				const part = collected
					.flat()
					.find(
						(p) => (p as { kind?: string }).kind === "elicitation2",
					) as ChatElicitationRequestPart;
				assert.ok(part);
				...
				assert.deepStrictEqual(openerService.openedUrls.map(String), [
					"https://example.com/auth",
				]);
				assert.strictEqual(part.state.get(), ElicitationState.Accepted);
				const completions = agentHostService.dispatchedActions.filter(
					(d) => d.action.type === ActionType.SessionInputCompleted,
				);
				assert.strictEqual(completions.length, 1);
				assert.deepStrictEqual(
					{
						requestId: (completions[0].action as { requestId: string })
							.requestId,
						response: (
							completions[0].action as { response: SessionInputResponseKind }
						).response,
					},
		...
		test("url input request decline dispatches Decline", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.strictEqual(
					(completions[0].action as { response: SessionInputResponseKind })
						.response,
					SessionInputResponseKind.Decline,
				);
		...
		test("url input request accept failure dispatches Decline", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
		...
		test("url input request opener returning false dispatches Decline", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
		...
		test("url input request abandoned at turn end dispatches Cancel", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);

				const { turnPromise, turnId, fire } = await startTurn(
					sessionHandler,
					agentHostService,
					chatAgentService,
					disposables,
		...
		test("url input request completion from another client does not redispatch", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("url input request server-side dismissal rejects the part and does not redispatch", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
	...
	suite("cancellation", () => {
		test("cancellation resolves the agent invoke", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const { turnPromise } = await startTurn(
					sessionHandler,
					agentHostService,
					chatAgentService,
					disposables,
				...
				assert.ok(
					agentHostService.dispatchedActions.some(
						(a) => a.action.type === "session/turnCancelled",
					),
				);
			}));

		test("cancellation force-completes outstanding tool invocations", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const toolInvocations = collected
					.flat()
					.filter((p) => p.kind === "toolInvocation");
				for (const inv of toolInvocations) {
					assert.strictEqual(
						IChatToolInvocation.isComplete(inv as IChatToolInvocation),
						true,
					);
				}
			}));

		test("cancellation calls abortSession on the agent host service", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("cancellation marks chat session complete", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const { turnPromise, chatSession } = await startTurn(
					sessionHandler,
					agentHostService,
					chatAgentService,
					disposables,
				...
				assert.strictEqual(
					chatSession.isCompleteObs?.get(),
					true,
					"chat session should be marked complete after cancellation",
				);
			}));

		test("cancellation after natural completion does not dispatch turnCancelled", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const beforeCancelCount = agentHostService.dispatchedActions.filter(
					(a) => a.action.type === "session/turnCancelled",
				).length;
				cts.cancel();
				const afterCancelCount = agentHostService.dispatchedActions.filter(
					(a) => a.action.type === "session/turnCancelled",
				).length;
				assert.strictEqual(
					afterCancelCount,
					beforeCancelCount,
					"turnCancelled should not be dispatched after natural completion",
				);
			}));
	});
	...
	suite("error events", () => {
		test("error event renders error message and finishes the request", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);

				const { turnPromise, collected, session, turnId } = await startTurn(
					sessionHandler,
					agentHostService,
					chatAgentService,
					disposables,
				...
				const errorPart = collected
					.flat()
					.find(
						(p) =>
							p.kind === "markdownContent" &&
							(p as IChatMarkdownContent).content.value.includes(
								"Something went wrong",
							),
					);
				assert.ok(
					errorPart,
					"Should have found a markdownContent part containing the error message",
				);
			}));
	});
	...
	suite("permission requests", () => {
		test("permission_request event shows confirmation and responds when confirmed", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.ok(
					...
					"Should have received tool confirmation progress",
				);
				const permInvocation = toolInvocations[
					toolInvocations.length - 1
				] as IChatToolInvocation;
				assert.strictEqual(permInvocation.kind, "toolInvocation");

				// Confirm the tool
				IChatToolInvocation.confirmWith(permInvocation, {
		...
		test("permission_request denied when user skips", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.ok(
					agentHostService.dispatchedActions.some((a) => {
						...
						const action = a.action as IToolCallConfirmedAction;
						return (
							action.toolCallId === "tc-perm-2" && action.approved === false
						);
					}),
				);
		...
		test("shell permission shows input-style confirmation data with toolInput", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const inputData =
					permInvocation.toolSpecificData as IChatToolInputInvocationData;
		...
		test("read permission shows input-style confirmation data", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				fire({
					...
					turnId,
					toolCallId: "tc-perm-read",
					invocationMessage: "Read file contents",
					toolInput: "/workspace/file.ts",
				} as SessionAction);
		...
		test("local confirmation does not race with pending tc.status: no spurious re-confirm before server echo", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				fire({
					...
					turnId,
					toolCallId: "tc-race",
					invocationMessage: "echo hi",
					toolInput: "echo hi",
				} as SessionAction);
				await timeout(10);

				const beforeConfirm = collected
					.flat()
					.filter((p) => p.kind === "toolInvocation") as IChatToolInvocation[];
				const permInvocation = beforeConfirm[beforeConfirm.length - 1];
				assert.strictEqual(
					permInvocation.state.get().type,
				...
				const afterLocalConfirm = collected
					.flat()
					.filter((p) => p.kind === "toolInvocation") as IChatToolInvocation[];
				assert.strictEqual(
					afterLocalConfirm.length,
					beforeConfirm.length,
					"no spurious invocation should be emitted by local confirm before server echoes",
				);

				// Exactly one toolCallConfirmed dispatch (with approved: true).
				const confirmedDispatches = agentHostService.dispatchedActions.filter(
					(a) => {
						if (a.action.type !== "session/toolCallConfirmed") {
							return false;
						}
				...
				assert.strictEqual(
					confirmedDispatches.length,
					1,
					"exactly one session/toolCallConfirmed should be dispatched",
				);
				assert.strictEqual(
					(confirmedDispatches[0].action as IToolCallConfirmedAction).approved,
					true,
				);
				...
				agentHostService.fireAction({
					...
					origin: {
						clientId: agentHostService.clientId,
						clientSeq: confirmedDispatches[0].clientSeq,
					},
				});
				fire({
					type: "session/toolCallComplete",
				...
				const finalInvocations = collected
					.flat()
					.filter((p) => p.kind === "toolInvocation");
				assert.strictEqual(
					finalInvocations.length,
					beforeConfirm.length,
					"no extra invocations across the full turn",
				);
			}));

		test("genuine re-confirmation (Running → PendingConfirmation) emits a fresh confirmation invocation", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(
					firstInvocation.state.get().type,
					IChatToolInvocation.StateKind.WaitingForConfirmation,
				);

				IChatToolInvocation.confirmWith(firstInvocation, {
					type: ToolConfirmKind.UserAction,
				});
				await timeout(10);
				...
				const firstConfirm = agentHostService.dispatchedActions.find((a) => {
					...
					return (
						(a.action as IToolCallConfirmedAction).toolCallId === "tc-recon"
					);
				})!;
				agentHostService.fireAction({
					...
					origin: {
						clientId: agentHostService.clientId,
						clientSeq: firstConfirm.clientSeq,
					},
				});
				await timeout(10);

				const invocationCountAfterRunning = collected
					.flat()
					.filter((p) => p.kind === "toolInvocation").length;
				...
				fire({
					...
					turnId,
					toolCallId: "tc-recon",
					invocationMessage: "Confirm execution",
					toolInput: "echo hi",
				} as SessionAction);
				await timeout(10);

				// We now expect a *fresh* invocation in WaitingForConfirmation.
				const invocationsAfterReconfirm = collected
					.flat()
					.filter((p) => p.kind === "toolInvocation") as IChatToolInvocation[];
				assert.strictEqual(
					invocationsAfterReconfirm.length,
					invocationCountAfterRunning + 1,
					"a fresh invocation should be emitted on Running → PendingConfirmation transition",
				);
				const reconfirmInvocation =
					invocationsAfterReconfirm[invocationsAfterReconfirm.length - 1];
				assert.strictEqual(
					reconfirmInvocation.state.get().type,
					IChatToolInvocation.StateKind.WaitingForConfirmation,
				);
				...
				IChatToolInvocation.confirmWith(reconfirmInvocation, {
					type: ToolConfirmKind.UserAction,
				});
				await timeout(10);
				...
				assert.strictEqual(
					allConfirms.length,
					2,
					"two toolCallConfirmed dispatches expected (initial + reconfirmation)",
				);
	...
	suite("history loading", () => {
				...
				turns: [
					{
						...
						responseParts: [
							{ kind: ResponsePartKind.Markdown, id: "md-1", content: "4" },
						],
						usage: undefined,
		...
		test("history requests get per-turn modelId from usage, with active turn falling back to session model", async () => {
			...
			const { sessionHandler, agentHostService } = createContribution(
				disposables,
				{ languageModels },
			);
				...
				turns: [
					{
						...
						responseParts: [
							{ kind: ResponsePartKind.Markdown, id: "md-1", content: "A1" },
						],
						usage: { model: "opus-4.7", _meta: { cost: 1.5 } },
						state: TurnState.Complete,
					},
					{
						...
						responseParts: [
							{ kind: ResponsePartKind.Markdown, id: "md-2", content: "A2" },
						],
						usage: undefined,
						state: TurnState.Complete,
					},
			...
			const requests = session.history.filter(
				(h): h is IChatSessionRequestHistoryItem => h.type === "request",
			);
			assert.deepStrictEqual(
	...
	suite("tool invocation rendering", () => {
		test("bash tool renders as terminal command block with output", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				fire({
					...
					result: {
						...
						content: [
							{
								type: "terminal",
								resource: "agenthost-terminal:///tc-shell-term",
							},
		...
		test("bash tool failure sets exit code 1 and error output", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				fire({
					...
					result: {
						...
						content: [
							{
								type: "terminal",
								resource: "agenthost-terminal:///tc-fail-term",
							},
							{ type: "text", text: "command not found: bad_cmd" },
						],
		...
		test("generic tool has invocation message and no toolSpecificData", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("bash tool without arguments has no terminal data", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("view tool shows file path in messages", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
	...
	suite("history with tool events", () => {
		test("tool_start and tool_complete appear as toolInvocationSerialized in history", async () => {
						...
						responseParts: [
							{
								...
								toolCall: {
									...
									success: true,
									pastTenseMessage: "Ran `ls`",
									content: [
										{
											type: "terminal" as const,
											resource: "agenthost-terminal:///tc-1-term",
										},
										{ type: "text" as const, text: "file1\nfile2" },
									],
								},
							},
						],
			...
			if (response.type === "response") {
				...
				const termData =
					toolPart.toolSpecificData as IChatTerminalToolInvocationData;
				assert.strictEqual(
					termData.terminalCommandOutput?.text,
					"file1\r\nfile2",
				);
		...
		test("non-terminal tool_complete sets pastTenseMessage in history", async () => {
						...
						responseParts: [
							{
								...
								toolCall: {
									...
									confirmed: "not-needed" as const,
									success: true,
									pastTenseMessage: "Searched for pattern",
								},
							},
						],
						usage: undefined,
			...
			if (response.type === "response") {
				const toolPart = response.parts[0] as IChatToolInvocationSerialized;
				assert.strictEqual(
					textOf(toolPart.pastTenseMessage),
	...
	suite("server error handling", () => {
		test("server-side error resolves the agent invoke without throwing", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);

				const { turnPromise, session, turnId } = await startTurn(
					sessionHandler,
					agentHostService,
					chatAgentService,
					disposables,
	...
	suite("session list provider filtering", () => {
		test("filters sessions to only the matching provider", async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "cp-1"),
				startTime: 1000,
				modifiedTime: 2000,
			});
			agentHostService.addSession({
				session: URI.from({ scheme: "other-provider", path: "/cl-1" }),
				startTime: 1000,
				modifiedTime: 2000,
			});
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "cp-2"),
				startTime: 3000,
				modifiedTime: 4000,
			});
			...
			assert.strictEqual(listController.items.length, 2);
			assert.ok(
				listController.items.every(
					(item) => item.resource.scheme === "agent-host-copilot",
				),
			);
		});
	});
	...
	suite("language model provider", () => {
		test("maps models with correct metadata", async () => {
			const provider = disposables.add(
				new AgentHostLanguageModelProvider(
					"agent-host-copilot",
					"agent-host-copilot",
				),
			);
			...
			assert.strictEqual(
				models[0].metadata.targetChatSessionType,
				"agent-host-copilot",
			);
		});
		...
		test("filters out disabled models", async () => {
			...
			provider.updateModels([
				...
				},
				{
					...
					supportsVision: false,
					policyState: PolicyState.Disabled,
				},
			]);
		...
		test("maps model config schema to picker configuration schema", async () => {
			...
			assert.deepStrictEqual(
				models[0].metadata.configurationSchema?.properties?.thinkingLevel,
				{
					type: "string",
					title: "Thinking Level",
		...
		test("sendChatRequest throws", async () => {
			...
			await assert.rejects(
				...
				/do not support direct chat requests/,
			);
		});
	});
	...
	suite("attachment context", () => {
		test("file variable with file:// URI becomes file attachment", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
						...
						variables: {
							variables: [
								upcastPartial({
									...
									id: "v-file",
									name: "test.ts",
									value: URI.file("/workspace/test.ts"),
								}),
							],
						},
					},
				...
				const turnAction = agentHostService.turnActions[0]
					.action as ITurnStartedAction;
				assert.deepStrictEqual(turnAction.message.attachments, [
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/workspace/test.ts").toString(),
						label: "test.ts",
		...
		test("screenshot variable becomes embedded image attachment", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const { turnPromise, session, turnId, fire } = await startTurn(
					...
					disposables,
					{
						...
						variables: {
							variables: [convertBufferToScreenshotVariable(screenshotBuffer)],
						},
					},
				);
				fire({
		...
		test("preserves _meta from variable entry on outgoing attachment", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("agent feedback variable becomes simple attachment with structured metadata", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				const sessionResource = URI.from({
		...
		test("directory variable with file:// URI becomes directory attachment", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.deepStrictEqual(turnAction.message.attachments, [
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/workspace/src").toString(),
						label: "src",
		...
		test("implicit selection variable becomes selection attachment", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("file variable with location value becomes selection attachment", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("implicit visible code location does not become selection attachment", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("non-file URI variables are skipped", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("tool variables are skipped", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("mixed variables extracts only supported types", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.deepStrictEqual(turnAction.message.attachments, [
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/workspace/a.ts").toString(),
						label: "a.ts",
						displayKind: "document",
					},
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/workspace/lib").toString(),
						label: "lib",
						displayKind: "directory",
					},
				]);
		...
		test("no variables results in no attachments argument", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
		...
		test("rebases file/directory/selection attachments under requested working dir onto resolved working dir", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
						...
						variables: {
							variables: [
								upcastPartial({
									...
									id: "v-file",
									name: "a.ts",
									value: URI.file("/source/a.ts"),
								}),
								upcastPartial({
									kind: "directory",
									id: "v-dir",
									name: "lib",
									value: URI.file("/source/lib"),
								}),
								upcastPartial({
									kind: "implicit",
									id: "v-implicit",
				...
				assert.deepStrictEqual(turnAction.message.attachments, [
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/worktree/a.ts").toString(),
						label: "a.ts",
						displayKind: "document",
					},
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/worktree/lib").toString(),
						label: "lib",
						displayKind: "directory",
					},
					{
		...
		test("does not rebase when requested and resolved working dirs match", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(turnAction.message.attachments, [
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/source/a.ts").toString(),
						label: "a.ts",
						displayKind: "document",
					},
				]);
		...
		test("attachments outside the requested working dir pass through unchanged", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const requestedDir = URI.file("/source");
				const resolvedDir = URI.file("/worktree");
				const { sessionHandler, agentHostService, chatAgentService } =
						...
						variables: {
							variables: [
								upcastPartial({
									...
									id: "v-file",
									name: "elsewhere.ts",
									value: URI.file("/elsewhere/elsewhere.ts"),
								}),
							],
						},
					},
				...
				assert.deepStrictEqual(turnAction.message.attachments, [
					{
						type: MessageAttachmentKind.Resource,
						uri: URI.file("/elsewhere/elsewhere.ts").toString(),
						label: "elsewhere.ts",
						displayKind: "document",
					},
				]);
	...
	suite("dynamic discovery", () => {
		test("setting gate prevents registration", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
		...
		test("local agent contribution advertises image attachments", () => {
			...
			assert.deepStrictEqual(
				chatSessionContributions.map((c) => ({
					type: c.type,
	...
	suite("IAgentConnection config", () => {
		test("handler uses custom extensionId from config", async () => {
			const { instantiationService, agentHostService, chatAgentService } =
				createTestServices(disposables);
			...
			const registered = chatAgentService.registeredAgents.get(
				"remote-test-copilot",
			);
			assert.ok(registered);
			assert.strictEqual(
				registered.data.extensionId.value,
				"vscode.remote-agent-host",
			);
			assert.strictEqual(
				registered.data.extensionDisplayName,
				"Remote Agent Host",
			);
		});
		...
		test("handler defaults extensionId when not provided", async () => {
			...
			const registered =
				chatAgentService.registeredAgents.get("default-ext-test");
			assert.ok(registered);
			assert.strictEqual(
				registered.data.extensionId.value,
				"vscode.agent-host",
		...
		test("handler uses resolveWorkingDirectory callback", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { instantiationService, agentHostService, chatAgentService } =
					createTestServices(disposables);

				const handler = disposables.add(
					instantiationService.createInstance(AgentHostSessionHandler, {
						provider: "copilot" as const,
				...
				assert.strictEqual(agentHostService.createSessionCalls.length, 1);
				assert.strictEqual(
					agentHostService.createSessionCalls[0].workingDirectory?.toString(),
					URI.file("/custom/working/dir").toString(),
				);
			}));

		test("handler forwards request session config to createSession", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { instantiationService, agentHostService, chatAgentService } =
					createTestServices(disposables);
				...
				const { turnPromise, session, turnId, fire } =
					await startDynamicAgentTurn(
						chatAgentService,
						agentHostService,
						"config-test",
						{
							message: "Add Agent Host session configuration flow",
							agentHostSessionConfig: config,
						},
					);
				fire({
					type: "session/turnComplete",
				...
				assert.strictEqual(agentHostService.createSessionCalls.length, 1);
				assert.deepStrictEqual(
					agentHostService.createSessionCalls[0].config,
					config,
				);
			}));

		test("handler forwards request session config via SessionConfigChanged on eager-create path", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const turnPromise = registered.impl.invoke(
					makeRequest({
						message: "Fix worktree branch hint propagation",
						sessionResource,
						agentHostSessionConfig: config,
					}),
					() => {},
				...
				agentHostService.fireAction({
					channel: turnDispatch.channel.toString(),
					action: turnDispatch.action,
					serverSeq: 1,
					origin: {
						clientId: agentHostService.clientId,
						clientSeq: turnDispatch.clientSeq,
					},
				});
				agentHostService.fireAction({
					channel: turnDispatch.channel.toString(),
				...
				const configChanged = agentHostService.dispatchedActions.find(
					(d) => d.action.type === ActionType.SessionConfigChanged,
				);
				assert.ok(configChanged, "expected a SessionConfigChanged dispatch");
				assert.deepStrictEqual(
					(configChanged!.action as { config: Record<string, unknown> }).config,
					config,
				);
			}));

		test("handler does not clobber picker-set session config on eager-create path", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				const configChanged = agentHostService.dispatchedActions.find(
					...
					| { action: { config: Record<string, unknown>; replace?: boolean } }
					| undefined;
				// Either no dispatch (preferred) or a dispatch that does NOT
				// include `isolation` and is NOT a replace.
				if (configChanged) {
					assert.strictEqual(
						configChanged.action.replace,
						undefined,
						"must not use replace-semantics for picker-set state",
					);
					assert.ok(
						!Object.prototype.hasOwnProperty.call(
							configChanged.action.config,
							"isolation",
						),
						`picker-set isolation must not be overwritten, got ${JSON.stringify(configChanged.action.config)}`,
					);
				}
			}));

		test("handler uses registered working directory resolver", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(
					agentHostService.createSessionCalls[0].workingDirectory?.toString(),
					resolvedWorkingDirectory.toString(),
				);
			}));

		test("handler passes vscode-agent-host URI as-is to createSession", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { instantiationService, agentHostService, chatAgentService } =
					createTestServices(disposables);
		...
		test("list controller includes description in items", async () => {
			...
			const controller = disposables.add(
				instantiationService.createInstance(
					AgentHostSessionListController,
					"remote-test",
					"copilot",
		...
		test("list controller omits description when undefined", async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri("copilot", "sess-2"),
				startTime: 1000,
				modifiedTime: 2000,
				summary: "Test",
			});
		...
		test("handler works with any IAgentConnection, not just IAgentHostService", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { instantiationService, agentHostService, chatAgentService } =
					createTestServices(disposables);
	...
	suite("reconnection to active turn", () => {
		function makeSessionStateWithActiveTurn(
			sessionUri: string,
			overrides?: Partial<{ streamingText: string; reasoning: string }>,
		): SessionState {
			const summary: SessionSummary = {
				resource: sessionUri,
				provider: "copilot",
			...
			if (reasoningText) {
				activeTurnParts.push({
					kind: ResponsePartKind.Reasoning as const,
					id: "reasoning-1",
					content: reasoningText,
				});
			}
			activeTurnParts.push({
				kind: ResponsePartKind.Markdown as const,
				id: "md-active",
				content: overrides?.streamingText ?? "Partial response so far",
			});
			return {
				...createSessionState(summary),
				lifecycle: SessionLifecycle.Ready,
		...
		test("sets isCompleteObs to false and populates progressObs for active turn", async () => {
			...
			assert.strictEqual(
				...
				false,
				"Should not be complete when active turn exists",
			);
			const progress = session.progressObs?.get() ?? [];
			assert.ok(
				progress.length > 0,
				"Should have initial progress from active turn",
			);
			// Should contain the streaming text as markdown
			const markdownPart = progress.find(
				(p) => p.kind === "markdownContent",
			) as IChatMarkdownContent | undefined;
			assert.ok(
				markdownPart,
				"Should have markdown content from streaming text",
			);
			assert.strictEqual(
				markdownPart!.content.value,
		...
		test("provides interruptActiveResponseCallback when reconnecting", async () => {
			...
			assert.ok(
				session.interruptActiveResponseCallback,
				"Should provide interrupt callback",
			);
		});
		...
		test("interrupt callback dispatches turnCancelled action", async () => {
			...
			const cancelAction = agentHostService.dispatchedActions.find(
				(d) => d.action.type === "session/turnCancelled",
			);
			assert.ok(cancelAction, "Should dispatch session/turnCancelled");
		});

		test("streams new text deltas into progressObs after reconnection", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService } =
					createContribution(disposables);

				const sessionUri = AgentSession.uri("copilot", "reconnect-stream");
				const sessionState = makeSessionStateWithActiveTurn(
					sessionUri.toString(),
				...
				const progress = session.progressObs?.get() ?? [];
				assert.ok(
					progress.length > initialLen,
					"Should have appended new progress items",
				);
				// The last markdown part should be the delta
				const lastMarkdown = [...progress]
					.reverse()
					.find((p) => p.kind === "markdownContent") as IChatMarkdownContent;
				assert.ok(lastMarkdown, "Should have a new markdown delta");
				assert.strictEqual(lastMarkdown.content.value, " and more");
			}));

		test("marks session complete when turn finishes", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService } =
					createContribution(disposables);
				...
				assert.strictEqual(
					session.isCompleteObs?.get(),
					true,
					"Should be complete after turnComplete",
				);
			}));
		...
		test("handles active turn with running tool call", async () => {
			...
			const progress = session.progressObs?.get() ?? [];
			const toolInvocation = progress.find(
				(p) => p.kind === "toolInvocation",
			) as IChatToolInvocation | undefined;
			assert.ok(
				toolInvocation,
				"Should have a live tool invocation in progress",
			);
			assert.strictEqual(toolInvocation!.toolCallId, "tc-running");
		});

		test("handles active turn with pending tool confirmation", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService } =
					createContribution(disposables);
				...
				const progress = session.progressObs?.get() ?? [];
				const permInvocation = progress.find(
					(p) => p.kind === "toolInvocation",
				) as IChatToolInvocation | undefined;
				assert.ok(
					permInvocation,
					"Should have a live permission request in progress",
				);
	...
	suite("server-initiated turns", () => {
		function createPendingChatModel(
			sessionResource: URI,
			pendingRequests: IChatPendingRequest[],
		): { model: IChatModel; firePendingRequestsChanged(): void } {
		...
		test("syncs queued messages added to restored active sessions", async () => {
			const { sessionHandler, agentHostService, chatService } =
				createContribution(disposables);

			const backendSession = AgentSession.uri(
				"copilot",
				"restored-pending-sync",
			);
			agentHostService.sessionStates.set(backendSession.toString(), {
				...
				lifecycle: SessionLifecycle.Ready,
				activeTurn: createActiveTurn("active-turn-1", {
					text: "Working",
					origin: { kind: MessageKind.User },
				}),
			});
			...
			const chatModel = createPendingChatModel(
				sessionResource,
				pendingRequests,
			);
			...
			const action = agentHostService.dispatchedActions
				.map((d) => d.action)
				.find(
					(
						action,
					...
					> => action.type === ActionType.SessionPendingMessageSet,
				);
			assert.ok(
				action,
				"queued message should be dispatched to the agent host",
			);
		...
		test("syncs text updates for existing queued pending messages", async () => {
			...
			assert.ok(
				action,
				"queued message text update should be dispatched to the agent host",
			);
			assert.deepStrictEqual(action, {
				type: ActionType.SessionPendingMessageSet,
				kind: "queued",
		...
		test("detects server-initiated turn and fires onDidStartServerRequest", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				disposables.add(
					chatSession.onDidStartServerRequest!((e) =>
						serverRequestEvents.push(e),
					),
				);
		...
		test("server-initiated turn streams progress through progressObs", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				agentHostService.fireAction({
					...
					action: {
						...
						session,
						turnId: serverTurnId,
						part: { kind: "markdown", id: "md-srv", content: "Hello " },
					} as SessionAction,
					serverSeq: 4,
					origin: undefined,
				});
				...
				const markdownParts = progress.filter(
					(p): p is IChatMarkdownContent => p.kind === "markdownContent",
				);
				const totalContent = markdownParts.map((p) => p.content.value).join("");
				assert.strictEqual(totalContent, "Hello world");
		...
		test("disposing chat session does not call disposeSession on connection", async () => {
			...
			assert.strictEqual(
				agentHostService.disposedSessions.length,
				0,
				"Disposing the UI chat session should not dispose the backend session",
			);
		});

		test("client-dispatched turns are not treated as server-initiated", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				assert.strictEqual(
					serverRequestEvents.length,
					0,
					"Client-dispatched turns should not trigger onDidStartServerRequest",
				);
			}));

		test("server-initiated turn does not duplicate tool calls on repeated state changes", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				agentHostService.fireAction({
					...
					action: {
						...
						turnId: serverTurnId,
						toolCallId: "tc-srv-1",
						result: { success: true, pastTenseMessage: "Ran Bash" },
					} as SessionAction,
					serverSeq: 6,
					origin: undefined,
				});
				...
				agentHostService.fireAction({
					...
					action: {
						...
						session,
						turnId: serverTurnId,
						part: { kind: "markdown", id: "md-after", content: "Done." },
					} as SessionAction,
					serverSeq: 7,
					origin: undefined,
				});
				...
				const progress = chatSession.progressObs!.get();
				const toolInvocations = progress.filter(
					(p) => p.kind === "toolInvocation",
				);
				assert.strictEqual(
					toolInvocations.length,
					1,
					"Tool call should not be duplicated",
				);
			}));

		test("server-initiated turn picks up markdown arriving with turnStarted", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				agentHostService.fireAction({
					...
					action: {
						...
						session,
						turnId: serverTurnId,
						part: { kind: "markdown", id: "md-init", content: "Initial text" },
					} as SessionAction,
					serverSeq: 4,
					origin: undefined,
				});
				...
				assert.strictEqual(
					totalContent,
					"Initial text",
					"Markdown arriving with/right after turnStarted should be picked up",
				);
		...
		test("removes consumed queued message from chat model when server-initiated turn appears", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
				...
				const removedQueueIds = chatService.removePendingRequestCalls.filter(
					(c) => c.requestId === "q-1",
				);
				assert.strictEqual(
					removedQueueIds.length,
					1,
					"consumed queued message should be removed from chat model",
				);
			}));

		test("removes steering message from chat model when steering id changes", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const {
					sessionHandler,
					agentHostService,
				...
				const removed = chatService.removePendingRequestCalls.filter(
					(c) => c.requestId === "steer-1",
				);
				assert.strictEqual(
					removed.length,
					1,
					"previously-set steering message should be removed from chat model when it is cleared",
				);
			}));
	});
	...
	suite("customizations", () => {
		test("dispatches activeClientChanged when a new session is created", async () => {
			...
			const customizations = observableValue<ClientPluginCustomization[]>(
				"customizations",
				[
					{
			...
			disposables.add(
				seedActiveClient("agent-host-copilot", { customizations }),
			);
			...
			assert.ok(
				createCall?.activeClient,
				"createSession should carry activeClient",
			);
			assert.strictEqual(
				createCall!.activeClient!.clientId,
				agentHostService.clientId,
			);
			assert.ok(
				Array.isArray(createCall!.activeClient!.tools),
				"activeClient.tools should be a defined array",
			);
			assert.strictEqual(createCall!.activeClient!.customizations?.length, 1);
			assert.strictEqual(
				createCall!.activeClient!.customizations?.[0].uri,
				"file:///plugin-a",
			);
		});
		...
		test("re-dispatches activeClientChanged when customizations observable changes", async () => {
			...
			assert.ok(
				activeClientAction,
				"should re-dispatch activeClientChanged on change",
			);
			const ac = activeClientAction!.action as {
				activeClient: { customizations?: ClientPluginCustomization[] };
			};
			assert.strictEqual(ac.activeClient.customizations?.length, 1);
			assert.strictEqual(
				ac.activeClient.customizations?.[0].uri,
				"file:///plugin-b",
			);
		});
		...
		test("does not dispatch activeClientChanged when an existing session is restored and this client is already active", async () => {
			...
			assert.strictEqual(
				agentHostService.dispatchedActions.filter(
					(d) => d.action.type === "session/activeClientChanged",
				).length,
				0,
			);
		...
		test("dispatches activeClientChanged when restoring a session where another client is active", async () => {
			...
			const activeClientActions = agentHostService.dispatchedActions.filter(
				(d) => d.action.type === "session/activeClientChanged",
			);
			assert.strictEqual(activeClientActions.length, 1);
			assert.strictEqual(
				activeClientActions[0].channel,
		...
		test("dispatches activeClientChanged when restoring a session where current client customizations are stale", async () => {
			const { instantiationService, agentHostService, seedActiveClient } =
				createTestServices(disposables);
			const customizations = observableValue<ClientPluginCustomization[]>(
				"customizations",
				[
	...
	suite("subagent grouping", () => {
		...
		function makeChildState(
			...
			const activeTurn = createActiveTurn("child-turn-1", {
				text: "do work",
				origin: { kind: MessageKind.User },
			});
			activeTurn.responseParts.push({
				kind: ResponsePartKind.ToolCall,
				toolCall: innerTool,
			});
			return {
		...
		test("inner subagent tool calls are forwarded with subAgentInvocationId set", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				agentHostService.sessionStates.set(
					childSessionUri,
					makeChildState(childSessionUri, "tc-child-1"),
				);
				...
				fire({
					...
					_meta: {
						toolKind: "subagent",
						subagentDescription: "do some work",
						subagentAgentName: "helper",
					},
				} as SessionAction);
				fire({
					type: "session/toolCallReady",
				...
				const toolInvocations = allParts.filter(
					(p): p is IChatToolInvocation => p.kind === "toolInvocation",
				);

				const parent = toolInvocations.find(
					(t) => t.toolCallId === parentToolCallId,
				);
				const child = toolInvocations.find(
					(t) => t.toolCallId === "tc-child-1",
				);

				assert.ok(parent, "parent task tool invocation should be emitted");
				assert.strictEqual(
					parent!.toolSpecificData?.kind,
					"subagent",
					"parent should have subagent toolSpecificData",
				);
				assert.strictEqual(
					parent!.subAgentInvocationId,
					undefined,
					"parent should not have a subAgentInvocationId",
				);

				assert.ok(
					child,
					"inner child tool invocation should be forwarded into parent session progress",
				);
				assert.strictEqual(
					child!.subAgentInvocationId,
					parentToolCallId,
					"child should be tagged with parent tool call id for grouping",
				);
			}));

		test("inner subagent tool calls fired AFTER parent observation are also grouped", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const { sessionHandler, agentHostService, chatAgentService } =
					createContribution(disposables);
				...
				const child = toolInvocations.find(
					(t) => t.toolCallId === childToolCallId,
				);
				...
				assert.ok(
					child,
					"child tool invocation fired after subscription should be forwarded",
				);
				assert.strictEqual(
					child!.subAgentInvocationId,
					parentToolCallId,
					"child should be tagged for grouping",
				);
			}));

		test("parent subagent agentName is updated when subagent content arrives later", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				fire({
					...
					_meta: {
						toolKind: "subagent",
						subagentDescription: "Exploring codebase structure",
					},
				} as SessionAction);
				fire({
					type: "session/toolCallReady",
					...
					content: [
						{
							...
							resource: buildSubagentSessionUri(
								session.toString(),
								parentToolCallId,
							),
							title: "Subagent",
				...
				assert.strictEqual(
					(parent!.toolSpecificData as { kind: "subagent"; agentName?: string })
						.agentName,
					"explore",
	...
	suite("auth dedupe", () => {
		...
		function tokenAuthService(tokenRef: {
			current: string;
		}): Partial<IAuthenticationService> {
		...
		test("skips authenticate when no token is resolvable", async () => {
			const noTokenService: Partial<IAuthenticationService> = {
				...
				getSessions:
					(async () => []) as unknown as IAuthenticationService["getSessions"],
			};
			const { agentHostService } = createContribution(disposables, {
				authServiceOverride: noTokenService,
			});
	...
	suite("provideChatInputCompletions", () => {
		test("forwards text/offset to the agent host and maps file attachments back to chat input items", async () => {
			...
			(
				agentHostService as unknown as {
					completions: (p: CompletionsParams) => Promise<CompletionsResult>;
				}
		...
		test("skips attachments of unsupported kinds", async () => {
			...
			assert.strictEqual(
				result?.items[0].attachment?.uri.toString(),
				"file:///workspace/dir",
			);
		});
