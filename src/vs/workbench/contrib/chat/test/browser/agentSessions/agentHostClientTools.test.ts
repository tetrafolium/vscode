...
} from "../../../browser/agentSessions/agentHost/agentHostSessionHandler.js";
import {
	AgentHostActiveClientService,
	IAgentHostActiveClientService,
} from "../../../browser/agentSessions/agentHost/agentHostActiveClientService.js";
...
suite("AgentHostClientTools", () => {
	...
	suite("toolResultToProtocol", () => {
		test("converts successful result with text content", () => {
			const result: IToolResult = {
				content: [{ kind: "text", value: "All 5 tests passed" }],
			...
			assert.deepStrictEqual(proto, {
				...
				content: [
					{ type: ToolResultContentType.Text, text: "All 5 tests passed" },
				],
				error: undefined,
			});
		});
		...
		test("converts text and data content parts", () => {
			...
			assert.deepStrictEqual(proto.content![0], {
				type: ToolResultContentType.Text,
				text: "hello",
			});
			assert.strictEqual(
				proto.content![1].type,
				ToolResultContentType.EmbeddedResource,
			);
			assert.strictEqual(
				(proto.content![1] as { contentType: string }).contentType,
				"image/png",
			);
			...
			assert.deepStrictEqual(proto.content![2], {
				type: ToolResultContentType.Text,
				text: "world",
			});
		});
		...
		test("converts data parts to EmbeddedResource with base64 encoding", () => {
			...
			assert.strictEqual(
				proto.content![0].type,
				ToolResultContentType.EmbeddedResource,
			);
			const embedded = proto.content![0] as {
				data: string;
				contentType: string;
			};
	...
	suite("client tools registration", () => {
		function createMockToolsService(
			...
			return {
				...
				getToolByName: (name: string) =>
					tools.find((t) => t.toolReferenceName === name),
				...
				invokeTool: async (invocation: IToolInvocation) => {
					...
					const toolInvocation = pendingToolCalls.get(
				...
				begunToolCalls,
				invokedToolCalls,
			} satisfies ILanguageModelToolsService & {
				fireOnDidChangeTools: () => void;
				begunToolCalls: ChatToolInvocation[];
				invokedToolCalls: IToolInvocation[];
			};
		}
		...
		class MockAgentHostConnection extends mock<IAgentHostService>() {
			...
			private readonly _onDidAction = disposables.add(
				new Emitter<ActionEnvelope>(),
			);
			override readonly onDidAction = this._onDidAction.event;
			private readonly _onDidNotification = disposables.add(
			...
			private readonly _liveSubscriptions = new Map<
				string,
				{ state: SessionState; emitter: Emitter<SessionState> }
			>();
			...
			override getSubscription<T>(
				...
				const sub: IAgentSubscription<T> = {
					get value() {
						return self._liveSubscriptions.get(resourceStr)
			...
			private _ensureLiveSubscription(resourceStr: string): {
				state: SessionState;
		...
		function createHandlerWithMocks(
			...
			const configValues: Record<string, unknown> = {
				"chat.agentHost.clientTools": configOverrides?.clientTools ?? [
					"runTask",
					"runTests",
				],
			};
			const onDidChangeConfig = disposables.add(
			...
			const activeClientService = disposables.add(
				instantiationService.createInstance(AgentHostActiveClientService),
			);
			instantiationService.stub(
				IAgentHostActiveClientService,
				activeClientService,
		...
		test("maps allowlisted tool data to protocol definitions", async () => {
			const { connection } = createHandlerWithMocks(disposables, [
				testRunTestsTool,
				testRunTaskTool,
				testUnlistedTool,
			]);
		...
		test("filters tool data to entries in configured allowlist", () => {
			createHandlerWithMocks(
				disposables,
				[testRunTestsTool, testRunTaskTool, testUnlistedTool],
				{
			...
			const filteredTools = [
				...
				(t) =>
					t.toolReferenceName !== undefined &&
					["runTests"].includes(t.toolReferenceName),
			);
		...
		test("dispatches activeClientToolsChanged when config changes", () => {
			const { connection, configValues, onDidChangeConfig } =
				createHandlerWithMocks(disposables, [
					testRunTestsTool,
					testRunTaskTool,
				]);
			...
			onDidChangeConfig.fire({
				affectsConfiguration: (key: string) =>
					key === "chat.agentHost.clientTools",
			...
			const toolsChangedActions = connection.dispatchedActions.filter(
				(a) =>
					isSessionAction(a.action) &&
					a.action.type === "session/activeClientToolsChanged",
			);
		...
		test("invokes an owned client tool when reconnecting to an active turn", async () => {
			...
			await handler.provideChatSessionContent(
				sessionResource,
			...
			assert.deepStrictEqual(
				toolsService.invokedToolCalls.map((call) => ({
					callId: call.callId,
					toolId: call.toolId,
			...
			assert.ok(
				connection.dispatchedActions.some(
					(entry) =>
		...
		test("reconnecting to an active turn with owned client tool completes the initial snapshot invocation", async () => {
			...
			const session = await handler.provideChatSessionContent(
				sessionResource,
				CancellationToken.None,
			);
			...
			const snapshotInvocation = (
				session as unknown as { progressObs: { get(): IChatProgress[] } }
			).progressObs
				.get()
				.find(
					(p): p is ChatToolInvocation =>
						p instanceof ChatToolInvocation && p.toolCallId === "tool-call-1",
				);
			assert.ok(
				snapshotInvocation,
				"activeTurnToProgress should have created a snapshot invocation",
			);
		...
		test("invokes a client tool inside a subagent session and dispatches completion against the subagent URI", async () => {
			...
			const subagentBackendSession = buildSubagentSessionUri(
				backendSession,
				parentToolCallId,
			);
			...
			const innerInvocation = toolsService.invokedToolCalls.find(
				(call) => call.callId === "inner-tool-call-1",
			);
			assert.ok(
				innerInvocation,
				"inner client tool inside the subagent should be invoked locally",
			);
			...
			assert.ok(
				completionEntry,
				"completion for the inner client tool should be dispatched",
			);
			assert.strictEqual(
