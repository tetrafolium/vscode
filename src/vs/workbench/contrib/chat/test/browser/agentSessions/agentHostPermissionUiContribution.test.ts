	...
	implements IAgentHostPermissionService
{
	...
	readonly pending: ISettableObservable<readonly IPendingResourceRequest[]> =
		observableValue("pending", []);
...
class StubLabelService extends MockLabelService {
	...
	override getHostLabel(scheme: string, authority?: string): string {
		if (
			...
			authority &&
			this._hostLabels.has(authority)
		) {
...
suite("AgentHostPermissionUiContribution", () => {
	...
	test("renders a markdown notification with three actions when a request arrives", () => {
		...
		assert.ok(
			isMarkdownString(notification.message),
			"message should be an IMarkdownString",
		);
		assert.strictEqual(
		...
		for (const action of notification.actions) {
			assert.deepStrictEqual(
				action.commandArgs,
				[request.id],
				"each action carries the request id",
			);
		}
	});
	...
	test("clears the notification when the queue empties", () => {
		...
		assert.deepStrictEqual(notificationService.deleteCalls, [
	...
	test("paths are wrapped in a markdown code span using a fence longer than any embedded backticks", () => {
		...
		assert.ok(
			...
			"path with embedded backticks should be inside the fence",
		);
	});
