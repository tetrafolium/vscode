...
} from "../../../browser/agentSessions/agentHost/agentHostAuth.js";
...
function createMockAuthService(overrides: {
	...
	getSessions?: (
		...
		options: any,
		activate: boolean,
	) => Promise<readonly { scopes: string[]; accessToken: string }[]>;
	createSession?: (
		providerId: string,
		scopes: string[],
		options: any,
	...
	return {
		getOrActivateProviderIdForServer:
			overrides.getOrActivateProviderIdForServer ??
			(() => Promise.resolve(undefined)),
		getSessions: overrides.getSessions ?? (() => Promise.resolve([])),
		createSession:
			overrides.createSession ??
			(() => Promise.reject(new Error("Unexpected createSession call"))),
...
suite("resolveTokenForResource", () => {
			...
			getSessions: (_providerId, scopes) => {
				if (scopes && scopes.length === 1 && scopes[0] === "read") {
					return Promise.resolve([
						{ scopes: ["read"], accessToken: "exact-token" },
					]);
				}
				return Promise.resolve([]);
			},
	...
	test("tries multiple authorization servers in order", async () => {
		...
		const authService = createMockAuthService({
			...
			getSessions: () =>
				Promise.resolve([{ scopes: ["read"], accessToken: "server2-token" }]),
		});
		const token = await resolveTokenForResource(
			resource,
...
suite("AgentHostAuthTokenCache", () => {
	...
	test("a different token for the same resource is reported as changed", () => {
		...
		cache.updateAndIsChanged("https://api.example.com", "tok1");
		assert.strictEqual(
			cache.updateAndIsChanged("https://api.example.com", "tok2"),
			true,
		);
	...
	test("tokens for distinct resources are tracked independently", () => {
		...
		assert.strictEqual(
			cache.updateAndIsChanged("https://other.example.com", "tok1"),
			true,
		);
		assert.strictEqual(
			cache.updateAndIsChanged("https://api.example.com", "tok1"),
	...
	test("skips authenticate when the cached token is unchanged", async () => {
		const authService = createMockAuthService({
			...
			getSessions: (_providerId, scopes) => {
				if (scopes) {
					return Promise.resolve([
						{ scopes: ["read"], accessToken: "cached-token" },
					]);
				}
		...
		assert.deepStrictEqual(requests, [
			{ resource: protectedResource.resource, token: "cached-token" },
		]);
	});
});
...
suite("resolveAuthenticationInteractively", () => {
	...
	test("uses an existing token before prompting for a new session", async () => {
		...
		const authService = createMockAuthService({
			...
			getSessions: (_providerId, scopes) => {
				if (scopes) {
					return Promise.resolve([
						{ scopes: ["read"], accessToken: "existing-token" },
					]);
				}
		...
		const success = await resolveAuthenticationInteractively(
			[protectedResource],
			{
		...
		assert.deepStrictEqual(requests, [
			{ resource: protectedResource.resource, token: "existing-token" },
		]);
	...
	test("creates a session when no existing token is available", async () => {
		...
		assert.strictEqual(success, true);
		assert.deepStrictEqual(requests, [
			{ resource: protectedResource.resource, token: "new-token" },
		]);
	});
});
