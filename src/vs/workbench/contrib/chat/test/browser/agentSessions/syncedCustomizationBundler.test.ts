...
} from "../../../../../../workbench/services/agentHost/common/agentHostFileSystemService.js";
...
suite("SyncedCustomizationBundler", () => {
	...
	setup(() => {
		...
		disposables.add(
			fileService.registerProvider(SYNCED_CUSTOMIZATION_SCHEME, syncedProvider),
		);
	...
	test("bundles instruction files into rules directory", async () => {
		...
		const result = await bundler.bundle([
			{ uri, type: PromptsType.instructions },
		]);
		...
		const destUri = URI.from({
			...
			path: "/test-agent/rules/my-rules.md",
		});
	...
	test("bundles files into correct directories by type", async () => {
		...
		const ruleContent = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/rules/rule.md",
			}),
		);
		assert.strictEqual(ruleContent.value.toString(), "rule content");

		const cmdContent = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/commands/cmd.prompt.md",
			}),
		);
		assert.strictEqual(cmdContent.value.toString(), "prompt content");

		const agentContent = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/agents/my-agent.md",
			}),
		);
		assert.strictEqual(agentContent.value.toString(), "agent content");

		// Non-SKILL.md skill files are written flat
		const skillContent = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/my-skill.md",
			}),
		);
	...
	test("bundles SKILL.md files into per-skill subdirectories", async () => {
		const bundler = createBundler();
		const skillA = await seedFile(
			"/skills/skill-a/SKILL.md",
			"skill A content",
		);
		const skillB = await seedFile(
			"/skills/skill-b/SKILL.md",
			"skill B content",
		);
		const skillC = await seedFile(
			"/skills/my-cool-skill/SKILL.md",
			"skill C content",
		);
		...
		const contentA = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/skill-a/SKILL.md",
			}),
		);
		assert.strictEqual(contentA.value.toString(), "skill A content");

		const contentB = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/skill-b/SKILL.md",
			}),
		);
		assert.strictEqual(contentB.value.toString(), "skill B content");

		const contentC = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/my-cool-skill/SKILL.md",
			}),
		);
	...
	test("writes plugin manifest", async () => {
		...
		const manifestUri = URI.from({
			scheme: SYNCED_CUSTOMIZATION_SCHEME,
			path: "/test-agent/.plugin/plugin.json",
		});
	...
	test("different authorities do not conflict", async () => {
		...
		const contentA = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/agent-a/rules/shared.md",
			}),
		);
		const contentB = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/agent-b/rules/shared.md",
			}),
		);
	...
	test("SKILL.md files with same basename do not overwrite each other", async () => {
		...
		const contentA = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/alpha/SKILL.md",
			}),
		);
		const contentB = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/beta/SKILL.md",
			}),
		);
	...
	test("non-SKILL.md skill files are written flat", async () => {
		...
		const content = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/my-helper.md",
			}),
		);
	...
	test("mixed SKILL.md and non-SKILL.md skill files coexist", async () => {
		const bundler = createBundler();
		const skillDir = await seedFile(
			"/skills/council-plan/SKILL.md",
			"council plan",
		);
		...
		const contentA = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/council-plan/SKILL.md",
			}),
		);
		...
		const contentB = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/skills/quick-fix.md",
			}),
		);
	...
	test("SKILL.md nonce includes subdirectory path", async () => {
		...
		const resultA = await bundler.bundle([
			{ uri: skillA, type: PromptsType.skill },
		]);
		const resultB = await bundler.bundle([
			{ uri: skillB, type: PromptsType.skill },
		]);
	...
	test("rebundle clears previous tree", async () => {
		...
		const destUri = URI.from({
			scheme: SYNCED_CUSTOMIZATION_SCHEME,
			path: "/test-agent/rules/first.md",
		});
		const content = await fileService.readFile(destUri);
		...
		const newContent = await fileService.readFile(
			URI.from({
				scheme: SYNCED_CUSTOMIZATION_SCHEME,
				path: "/test-agent/rules/second.md",
			}),
		);
	...
	test("bundle description includes file count", async () => {
		...
		assert.ok(
			result.ref.nonce,
			"should produce a nonce reflecting the bundled files",
		);
	});
});
