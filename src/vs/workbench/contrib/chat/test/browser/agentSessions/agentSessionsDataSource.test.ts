...
} from "../../../browser/agentSessions/agentSessionsViewer.js";
import {
	AgentSessionSection,
	IAgentSession,
...
} from "../../../browser/agentSessions/agentSessionsFilter.js";
...
suite("sessionDateFromNow", () => {
	...
	test("returns fromNow result for today", () => {
		...
		assert.ok(
			result.includes("min") ||
				...
				result.includes("hr") ||
				result === "now",
			`Expected minutes/seconds/hours ago or now, got: ${result}`,
		);
	});
	...
	test("returns fromNow result for three or more days ago", () => {
		...
		assert.ok(
			!result.includes("1 day") && !result.includes("2 days"),
			`Should not be 1 or 2 days ago, got: ${result}`,
		);
	});
	...
	test('appends "ago" when appendAgoLabel is true', () => {
		...
		assert.ok(
			result.includes("ago"),
			`Expected "ago" in result, got: ${result}`,
		);
	});
});
...
suite("AgentSessionsDataSource", () => {
	...
	function createMockSession(
		...
		return {
			...
			changes: overrides.hasChanges
				? { files: 1, insertions: 10, deletions: 5 }
				: undefined,
	...
	function createMockFilter(options: {
		...
		return {
			...
			getExcludes: () => ({
				...
				states: [],
				archived: false,
				read: options.excludeRead ?? false,
				repositoryGroupCapped: options.repositoryGroupCapped ?? true,
			}),
			isDefault: () => true,
			reset: () => {},
		};
	...
	function getSectionsFromResult(
		result: Iterable<AgentSessionListItem>,
	): IAgentSessionSection[] {
		return Array.from(result).filter((item): item is IAgentSessionSection =>
			isAgentSessionSection(item),
		);
	}
	...
	suite("groupSessionsIntoSections", () => {
		test("returns flat list when groupResults is false", () => {
			...
			const dataSource = disposables.add(
				new AgentSessionsDataSource(filter, sorter),
			);
		...
		test("in-progress sessions are placed in their date-based section", () => {
			...
			const todaySection = sections.find(
				(s) => s.section === AgentSessionSection.Today,
			);
		...
		test("adds Today header when there are no active sessions", () => {
			...
			assert.strictEqual(
				sections.filter((s) => s.section === AgentSessionSection.Today).length,
				1,
			);
		});
		...
		test("adds Older header for sessions older than week threshold", () => {
			...
			assert.strictEqual(
				sections.filter((s) => s.section === AgentSessionSection.Older).length,
				1,
			);
		});
		...
		test("adds Archived header for archived sessions", () => {
			...
			assert.strictEqual(
				sections.filter((s) => s.section === AgentSessionSection.Archived)
					.length,
				1,
			);
		});
		...
		test("archived sessions come after older sessions", () => {
			...
			const olderIndex = result.findIndex(
				(item) =>
					isAgentSessionSection(item) &&
					item.section === AgentSessionSection.Older,
			);
			const archivedIndex = result.findIndex(
				(item) =>
					isAgentSessionSection(item) &&
					item.section === AgentSessionSection.Archived,
			);

			assert.ok(
				olderIndex < archivedIndex,
				"Older section should come before Archived section",
			);
		});
		...
		test("archived in-progress sessions appear in Archived section", () => {
			...
			const archivedSection = sections.find(
				(s) => s.section === AgentSessionSection.Archived,
			);
			...
			assert.strictEqual(
				archivedSection.sessions[0].label,
				"Session archived-active",
			);
		});
		...
		test("correct order: today, week, older, archived", () => {
			...
			assert.strictEqual(
				(result[0] as IAgentSessionSection).section,
				AgentSessionSection.Today,
			);
			assert.strictEqual(
				(result[0] as IAgentSessionSection).sessions.length,
				2,
			);
			...
			assert.strictEqual(
				(result[1] as IAgentSessionSection).section,
				AgentSessionSection.Week,
			);
			assert.strictEqual(
				(result[1] as IAgentSessionSection).sessions[0].label,
				"Session week",
			);
			...
			assert.strictEqual(
				(result[2] as IAgentSessionSection).section,
				AgentSessionSection.Older,
			);
			assert.strictEqual(
				(result[2] as IAgentSessionSection).sessions[0].label,
				"Session old",
			);
			...
			assert.strictEqual(
				(result[3] as IAgentSessionSection).section,
				AgentSessionSection.Archived,
			);
			assert.strictEqual(
				(result[3] as IAgentSessionSection).sessions[0].label,
				"Session archived",
			);
		});
		...
		test("sessions are sorted within each group", () => {
			...
			const sessions = [
				createMockSession({
					id: "old1",
					status: ChatSessionStatus.Completed,
					startTime: now - WEEK_THRESHOLD - 2 * ONE_DAY,
					endTime: now - WEEK_THRESHOLD - 2 * ONE_DAY,
				}),
				createMockSession({
					id: "old2",
					status: ChatSessionStatus.Completed,
			...
			const weekSection = result.find(
				(item): item is IAgentSessionSection =>
					isAgentSessionSection(item) &&
					item.section === AgentSessionSection.Week,
			);
			...
			const olderSection = result.find(
				(item): item is IAgentSessionSection =>
					isAgentSessionSection(item) &&
					item.section === AgentSessionSection.Older,
			);
		...
		test("archived pinned sessions go to Archived, not Pinned", () => {
			...
			const pinnedSection = sections.find(
				(s) => s.section === AgentSessionSection.Pinned,
			);
			const archivedSection = sections.find(
				(s) => s.section === AgentSessionSection.Archived,
			);
		...
		test("pinned sessions are always shown above the cap with capped grouping", () => {
			...
			const result = Array.from(dataSource.getChildren(mockModel));
			const sections = getSectionsFromResult(result);
			const topSessions = result.filter(
				(r): r is IAgentSession => !isAgentSessionSection(r),
			);

			// Pinned sessions first, then up to 3 non-pinned sessions
			assert.deepStrictEqual(
				topSessions.map((s) => s.label),
				[
					"Session pinned1",
					"Session pinned2",
					"Session s1",
			...
			const moreSection = sections.find(
				(s) => s.section === AgentSessionSection.More,
			);
			assert.ok(moreSection);
			assert.deepStrictEqual(
				moreSection.sessions.map((s) => s.label),
				["Session s4"],
			);
		});
	...
	suite("groupSessionsByRepository", () => {
		...
		test("groups sessions by metadata.owner + metadata.name (cloud sessions)", () => {
			...
			const sessions = [
				createMockSession({
					id: "1",
					startTime: now,
					metadata: { owner: "microsoft", name: "vscode" },
				}),
				createMockSession({
					id: "2",
					startTime: now - 1,
				...
				createMockSession({
					id: "3",
					startTime: now - 2,
					metadata: { owner: "microsoft", name: "typescript" },
				}),
			];

			const filter = createMockFilter({
				groupBy: AgentSessionsGrouping.Repository,
			});
			const dataSource = disposables.add(
				new AgentSessionsDataSource(filter, createMockSorter()),
			);
			const result = getSectionsFromResult(
				dataSource.getChildren(createMockModel(sessions)),
			);
		...
		test("groups sessions by metadata.repositoryNwo", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repositoryNwo: "microsoft/vscode" },
				}),
				createMockSession({
					id: "2",
					metadata: { repositoryNwo: "microsoft/vscode" },
		...
		test("groups sessions by metadata.repository (nwo format)", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repository: "microsoft/vscode" },
				}),
				createMockSession({
					id: "2",
					metadata: { repository: "microsoft/vscode" },
		...
		test("groups sessions by metadata.repository (URL format)", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repository: "https://github.com/microsoft/vscode" },
				}),
			];
		...
		test("strips .git suffix from repository URLs", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repository: "https://github.com/microsoft/vscode.git" },
				}),
				createMockSession({
					id: "2",
					metadata: {
		...
		test("handles git@ SSH URLs", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repository: "git@github.com:microsoft/vscode.git" },
				}),
			];
		...
		test("groups sessions by metadata.repositoryUrl", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repositoryUrl: "https://github.com/microsoft/vscode" },
				}),
			];
		...
		test("groups sessions by metadata.worktreePath", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: {
						worktreePath: "/Users/user/Projects/vscode.worktrees/my-branch",
					},
				}),
			];
		...
		test("groups sessions by metadata.workingDirectoryPath", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { workingDirectoryPath: "/Users/user/Projects/vscode" },
				}),
			];
				...
				createMockSession({
					...
					metadata: {
						workingDirectoryPath:
							"/Users/user/Projects/vscode.worktrees/copilot-branch",
					},
				}),
			];
		...
		test("sessions without any repo info go to Other", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { isolationMode: "workspace" },
				}),
		...
		test('repo named "other" does not collide with the Other fallback group', () => {
			...
			const sessions = [
				createMockSession({
					id: "1",
					startTime: now,
					metadata: { repositoryPath: "/path/other" },
				}),
			...
			assert.ok(
				...
				'should have a group for repo named "other"',
			);
			assert.ok(
				labels.includes("Other"),
				'should have the fallback "Other" group',
			);
			assert.strictEqual(
				result.find((s) => s.label === "other")!.sessions.length,
				1,
			);
			assert.strictEqual(
				result.find((s) => s.label === "Other")!.sessions.length,
		...
		test("archived sessions go to Archived section", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repositoryPath: "/path/vscode" },
				}),
				createMockSession({
					id: "2",
					isArchived: true,
		...
		test("metadata extraction priority: owner+name > repositoryNwo > repository > repositoryUrl > repositoryPath > workingDirectoryPath > badge", () => {
			...
			const ds1 = disposables.add(
				new AgentSessionsDataSource(filter, createMockSorter()),
			);
			assert.strictEqual(
				getSectionsFromResult(
					ds1.getChildren(
						createMockModel([
							createMockSession({
								id: "1",
		...
		test("empty string metadata values are treated as missing", () => {
			const sessions = [
				createMockSession({
					id: "1",
					metadata: { repositoryNwo: "", repositoryPath: "/path/vscode" },
				}),
			];
		...
		test("Other group appears after named repos and before Archived", () => {
			...
			const sessions = [
				...
				createMockSession({
					id: "repo-a",
					startTime: now - 1,
					metadata: { repositoryPath: "/path/alpha" },
				}),
				createMockSession({
					id: "archived",
					startTime: now - 2,
				...
				createMockSession({
					id: "repo-b",
					startTime: now - 3,
					metadata: { repositoryPath: "/path/beta" },
				}),
			...
			for (let i = 0; i < otherIndex; i++) {
				assert.strictEqual(
					result[i].section,
					AgentSessionSection.Repository,
					`section at index ${i} should be a named repository group`,
				);
			}

			// Archived must come after Other
			assert.ok(
				archivedIndex > otherIndex,
				"Archived section should come after Other",
			);
		});
		...
		test("pinned sessions are top-level items before alphabetized repository sections", () => {
			...
			const pinnedSession = createMockSession({
				...
				isPinned: true,
				startTime: now + 10,
				metadata: { repositoryPath: "/path/zebra" },
			});
			const sessions = [
				...
				createMockSession({
					...
					isArchived: true,
					startTime: now + 6,
					metadata: { repositoryPath: "/path/middle" },
				}),
				pinnedSession,
			];
			...
			assert.ok(
				isAgentSession(result[0]),
				"first item should be the pinned session",
			);
			assert.strictEqual(
				result[0].resource.toString(),
				pinnedSession.resource.toString(),
			);

			const sections = result.filter((item): item is IAgentSessionSection =>
				isAgentSessionSection(item),
			);
			assert.deepStrictEqual(
	...
	suite("repositoryGroupLimit", () => {
		test("caps repo group children at limit and appends show-more item", () => {
			...
			const sessions = Array.from({ length: 8 }, (_, i) =>
				createMockSession({
					id: `s${i}`,
					metadata: { repositoryNwo: "owner/vscode" },
			...
			const dataSource = disposables.add(
				new AgentSessionsDataSource(filter, createMockSorter(), 5),
			);
			...
			const section = topLevel.find(
				(item) =>
					isAgentSessionSection(item) &&
					item.section === AgentSessionSection.Repository,
			) as IAgentSessionSection;
			assert.ok(section);
		...
		test("does not cap non-repository sections", () => {
			...
			const model = createMockModel(sessions);
			const topLevel = Array.from(dataSource.getChildren(model));
			const todaySection = topLevel.find(
				(item) =>
					isAgentSessionSection(item) &&
					item.section === AgentSessionSection.Today,
			) as IAgentSessionSection;
	...
	suite("getRepositoryName", () => {
		...
		test("metadata repo name takes priority over badge name", () => {
			const session = createMockSession({
				id: "1",
				metadata: { owner: "microsoft", name: "vscode" },
				badge: "$(folder) copilot-worktree-branch",
			});
			assert.strictEqual(getRepositoryName(session), "vscode");
		});
...
suite("AgentSessionsSorter", () => {
	...
	test("default: does NOT prioritize needs-input sessions", () => {
		const sorter = new AgentSessionsSorter();
		const needsInput = createSession({
			id: "needs",
			status: ChatSessionStatus.NeedsInput,
			created: 1000,
		});
	...
	test("prioritizeActive: needs-input sessions come first", () => {
		...
		const sorted = [completed, needsInput].sort((a, b) =>
			sorter.compare(a, b, true),
		);
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
			["Session needs", "Session done"],
		);
	});
	...
	test("prioritizeActive: archived still come last when not active", () => {
		...
		const sorted = [archived, active].sort((a, b) =>
			sorter.compare(a, b, true),
		);
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
	...
	test("prioritizeActive: uses lastRequestStarted for time sorting when sorted by updated", () => {
		...
		const recentlyActive = createSession({
			id: "recent-active",
			created: 1000,
			lastRequestStarted: 5000,
		});
		const recentlyCreated = createSession({
			id: "recent-created",
			created: 3000,
		});

		const sorted = [recentlyCreated, recentlyActive].sort((a, b) =>
			sorter.compare(a, b, true),
		);
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
			["Session recent-active", "Session recent-created"],
		);
	});
	...
	test("prioritizeActive: uses created time when sorted by created", () => {
		...
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
			["Session recent-created", "Session recent-active"],
		);
	});
	...
	test("pinned sessions come before non-pinned sessions", () => {
		...
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
			["Session pinned", "Session regular"],
		);
	});

	test("archived pinned sessions do not sort before non-archived", () => {
		const sorter = new AgentSessionsSorter();
		const archivedPinned = createSession({
			id: "archived-pinned",
			isPinned: true,
			isArchived: true,
			created: 3000,
		...
		const sorted = [archivedPinned, regular].sort((a, b) =>
			sorter.compare(a, b),
		);
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
			["Session regular", "Session archived-pinned"],
		);
	});

	test("sortBy Created: sorts by creation time regardless of lastRequestEnded", () => {
		const sorter = new AgentSessionsSorter(() => AgentSessionsSorting.Created);
		const olderCreated = createSession({
			id: "older",
			created: 1000,
			lastRequestEnded: 5000,
		});
		const newerCreated = createSession({
			id: "newer",
			created: 3000,
			lastRequestEnded: 2000,
		});

		const sorted = [olderCreated, newerCreated].sort((a, b) =>
			sorter.compare(a, b),
		);
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
	...
	test("sortBy Updated: sorts by lastRequestEnded", () => {
		const sorter = new AgentSessionsSorter(() => AgentSessionsSorting.Updated);
		const recentlyUpdated = createSession({
			id: "updated",
			created: 1000,
			lastRequestEnded: 5000,
		});
		...
		const sorted = [recentlyCreated, recentlyUpdated].sort((a, b) =>
			sorter.compare(a, b),
		);
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
	...
	test("sortBy Updated: falls back to created when lastRequestEnded is undefined", () => {
		const sorter = new AgentSessionsSorter(() => AgentSessionsSorting.Updated);
		const withRequest = createSession({
			id: "with-request",
			created: 1000,
		...
		const sorted = [withRequest, withoutRequest].sort((a, b) =>
			sorter.compare(a, b),
		);
		assert.deepStrictEqual(
			sorted.map((s) => s.label),
			["Session no-request", "Session with-request"],
		);
	});
});
...
suite("groupAgentSessionsByDate with sortBy", () => {
	...
	test("Updated: session created long ago but recently updated goes into Today", () => {
		...
		const oldButUpdated = createSession({
			id: "old-updated",
			created: tenDaysAgo,
			lastRequestEnded: now,
		});

		const grouped = groupAgentSessionsByDate(
			[oldButUpdated],
			AgentSessionsSorting.Updated,
		);
		const todaySessions = grouped.get(AgentSessionSection.Today)!.sessions;
		const olderSessions = grouped.get(AgentSessionSection.Older)!.sessions;
	...
	test("Updated: falls back to created when lastRequestEnded is undefined", () => {
		...
		const oldNoUpdate = createSession({
			id: "old-no-update",
			created: tenDaysAgo,
		});
	...
	test("Updated: pinned and archived sessions are not affected by sortBy", () => {
		...
		const pinnedOld = createSession({
			id: "pinned",
			created: tenDaysAgo,
			lastRequestEnded: now,
			isPinned: true,
		});
		const archivedOld = createSession({
			id: "archived",
			created: tenDaysAgo,
			lastRequestEnded: now,
			isArchived: true,
