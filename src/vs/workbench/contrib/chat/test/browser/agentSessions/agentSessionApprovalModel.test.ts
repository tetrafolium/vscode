...
} from "../../../browser/agentSessions/agentSessionApprovalModel.js";
...
function makeTerminalToolData(
	overrides?: Partial<IChatTerminalToolInvocationData>,
): IChatTerminalToolInvocationData {
	return {
		kind: "terminal",
...
function makePostApprovalState(
	confirm?: (reason: ConfirmedReason) => void,
): IChatToolInvocation.State {
	return {
...
function mockModelWithResponse(
	model: MockChatModel,
	parts: IChatProgressResponseContent[],
): void {
	...
	(model as { lastRequest: IChatRequestModel | undefined }).lastRequest =
		request as IChatRequestModel;
}
...
suite("AgentSessionApprovalModel", () => {
	...
	setup(() => {
		...
		chatModelsObs = chatService.chatModels as ISettableObservable<
			Iterable<IChatModel>
		>;
	});
	...
	function createModel(): AgentSessionApprovalModel {
		const model = new AgentSessionApprovalModel(
			chatService,
			langservice as ILanguageService,
		);
	...
	function addChatModel(uri?: URI): MockChatModel {
		const chatModel = disposables.add(
			new MockChatModel(uri ?? URI.parse(`test://session/${Math.random()}`)),
		);
		chatModelsObs.set(
			[...Array.from(chatModelsObs.get()), chatModel],
			undefined,
		);
		return chatModel;
	}
	...
	test("returns undefined when no models exist", () => {
		...
		const result = approvalModel
			.getApproval(URI.parse("test://nonexistent"))
			.get();
	...
	test("uses forDisplay from commandLine when available", () => {
		...
		assert.strictEqual(
			getApproval(approvalModel, chatModel)?.label,
			"echo display",
		);
	});
	...
	test("uses needsInput.detail when tool is not terminal", () => {
		...
		mockModelWithResponse(chatModel, [part]);
		chatModel.requestNeedsInput.set(
			{ title: "Test", detail: "Custom detail message" },
			undefined,
		);
	...
	test("reacts to tool state changing from waiting to executing", () => {
		...
		const stateObs = observableValue<IChatToolInvocation.State>(
			"toolState",
			makeWaitingState(),
		);
	...
	test("picks the first WaitingForConfirmation part when multiple parts exist", () => {
		...
		const executingPart = makeToolInvocationPart({
			state: makeExecutingState(),
		});
		const waitingPart = makeToolInvocationPart({
			state: makeWaitingState(),
			toolSpecificData: makeTerminalToolData({
				commandLine: { original: "second-cmd" },
			}),
		});
	...
	test("skips non-toolInvocation parts", () => {
		...
		const waitingPart = makeToolInvocationPart({
			state: makeWaitingState(),
			toolSpecificData: makeTerminalToolData({
				commandLine: { original: "the-cmd" },
			}),
		});
		mockModelWithResponse(chatModel, [
			markdownPart as unknown as IChatProgressResponseContent,
			waitingPart,
		]);
		chatModel.requestNeedsInput.set({ title: "Test" }, undefined);
