...
import {
	turnsToHistory as rawTurnsToHistory,
	activeTurnToProgress as rawActiveTurnToProgress,
	toolCallStateToInvocation as rawToolCallStateToInvocation,
	finalizeToolInvocation as rawFinalizeToolInvocation,
	updateRunningToolSpecificData as rawUpdateRunningToolSpecificData,
} from "../../../browser/agentSessions/agentHost/stateToProgressAdapter.js";

// ---- Helper factories -------------------------------------------------------

function createToolCallState(
	overrides?: Partial<ToolCallRunningState>,
...
function createCompletedToolCall(
	overrides?: Partial<ICompletedToolCall>,
): ICompletedToolCall {
	return {
...
function toolCallStateToInvocation(
	tc: Parameters<typeof rawToolCallStateToInvocation>[0],
...
function finalizeToolInvocation(
	invocation: Parameters<typeof rawFinalizeToolInvocation>[0],
	tc: Parameters<typeof rawFinalizeToolInvocation>[1],
) {
	return rawFinalizeToolInvocation(invocation, tc, URI.file("/"), undefined);
}

function turnsToHistory(
	backendSession: Parameters<typeof rawTurnsToHistory>[0],
	turns: Parameters<typeof rawTurnsToHistory>[1],
	participantId: Parameters<typeof rawTurnsToHistory>[2],
	lookup?: Parameters<typeof rawTurnsToHistory>[4],
) {
	return rawTurnsToHistory(
		backendSession,
		turns,
		participantId,
		"local",
...
function makeLookup(
	...
	fallbackRawModelId?: string,
): Parameters<typeof rawTurnsToHistory>[4] {
	const resolveRaw = (raw: string | undefined): string | undefined =>
...
function activeTurnToProgress(
	sessionResource: Parameters<typeof rawActiveTurnToProgress>[0],
	activeTurn: Parameters<typeof rawActiveTurnToProgress>[1],
	connectionAuthority?: Parameters<typeof rawActiveTurnToProgress>[2],
) {
	return rawActiveTurnToProgress(
		sessionResource,
		activeTurn,
		connectionAuthority,
	);
...
function updateRunningToolSpecificData(
	existing: Parameters<typeof rawUpdateRunningToolSpecificData>[0],
	tc: Parameters<typeof rawUpdateRunningToolSpecificData>[1],
) {
	return rawUpdateRunningToolSpecificData(existing, tc, undefined);
}

function assertInputOutputDetails(
	details: unknown,
): asserts details is IToolResultInputOutputDetails {
...
suite("stateToProgressAdapter", () => {
	...
	suite("turnsToHistory", () => {
		...
		test("generic completed tool call maps embedded resources and resource refs", () => {
						...
						toolCall: createCompletedToolCall({
							...
							content: [
								...
								},
								{
									...
									uri: "agenthost-content:///session/result.txt",
			...
			assert.deepStrictEqual(details.output[0], {
				type: "embed",
				value: "aW1hZ2U=",
				mimeType: "image/png",
			});
			...
			assert.strictEqual(
				details.output[1].uri.toString(),
				"agenthost-content:/session/result.txt",
			);
		...
		test("maps turn usage to chat usage progress for restored history", () => {
			const turn = createTurn({
				...
				responseParts: [
					{ kind: ResponsePartKind.Markdown, id: "md-1", content: "Done" },
				],
			});
		...
		test("terminal tool call in history has correct terminal data", () => {
							...
							content: [
								{
									...
									resource: "agenthost-terminal:///t1",
									title: "Terminal",
								},
			...
			const termData = serialized.toolSpecificData as {
				kind: "terminal";
				...
				responseParts: [
					{
						...
						toolCall: createCompletedToolCall({
							...
							content: [
								{
									type: ToolResultContentType.Terminal,
									resource: "agenthost-terminal:///past",
									title: "Terminal",
								},
		...
		test("subagent tool call in history has correct subagent data", () => {
			...
			if (serialized.toolSpecificData.kind === "subagent") {
				...
				assert.strictEqual(
					serialized.toolSpecificData.description,
					"Find related files",
				);
				...
				responseParts: [
					{
						...
						toolCall: createCompletedToolCall({
							...
							content: [
								{ type: ToolResultContentType.Text, text: "Result text" },
							],
							success: true,
						}),
					} as ToolCallResponsePart,
				...
				responseParts: [
					{
						...
						id: "md-links",
						content:
							"See [local](file:///a/b.ts), [content](agenthost-content:///s/x), [external](https://example.com) and [rel](./foo.md).",
					},
				],
			});
		...
		test("markdown link syntax inside fenced code blocks is preserved verbatim", () => {
			...
			const turn = createTurn({
				responseParts: [
					{ kind: ResponsePartKind.Markdown, id: "md-code", content: input },
				],
			});
		...
		test("markdown link syntax inside inline code spans is preserved verbatim", () => {
			const input =
				"Real [one](file:///a.ts) and literal `[two](file:///b.ts)` here.";
			const turn = createTurn({
				responseParts: [
					{
						kind: ResponsePartKind.Markdown,
				...
				responseParts: [
					{
						...
						id: "md-skill",
						content:
							"Loaded [plan](file:///abs/repo/skills/plan/SKILL.md) and [other](file:///abs/repo/foo.ts).",
					},
				],
			});
		...
		test("preserves alt text for image tokens", () => {
			...
			assert.strictEqual(
				value,
				"See ![diagram](vscode-agent-host://my-host/file/-/a/b.png).",
			);
		});
		...
		test("error turn produces error message in history", () => {
			...
			const errorPart = response.parts.find(
				(p) =>
					p.kind === "markdownContent" &&
					(p as IChatMarkdownContent).content.value.includes("boom"),
			);
			assert.ok(
				errorPart,
				"Should have a markdownContent part containing the error message",
			);
		});
				...
				responseParts: [
					{
						...
						toolCall: createCompletedToolCall({
							...
							content: [
								{
									type: ToolResultContentType.Terminal,
									resource: "agenthost-terminal:///t2",
									title: "Terminal",
								},
				...
				responseParts: [
					{
						...
						toolCall: createCompletedToolCall({
							...
							content: [
								{ type: ToolResultContentType.Text, text: "found results" },
							],
						}),
					} as ToolCallResponsePart,
				],
	...
	suite("toolCallStateToInvocation", () => {
		...
		test("sets terminal toolSpecificData when content has terminal block", () => {
			const tc = createToolCallState({
				...
				content: [
					{
						type: ToolResultContentType.Terminal,
						resource: "agenthost-terminal:///t3",
						title: "Terminal",
					},
				],
			});
		...
		test("sets subagent toolSpecificData from _meta for subagent toolKind", () => {
			const tc = createToolCallState({
				_meta: {
					toolKind: "subagent",
					subagentDescription: "Review code",
			...
			if (invocation.toolSpecificData.kind === "subagent") {
				...
				assert.strictEqual(
					invocation.toolSpecificData.agentName,
					"code-reviewer",
				);
			}
		});
	...
	suite("finalizeToolInvocation", () => {
		test("rewrites markdown links in pastTenseMessage through the agent host scheme", () => {
			...
			rawFinalizeToolInvocation(
				invocation,
				{
					...
					success: true,
					pastTenseMessage: {
						markdown: "Read [foo.ts](file:///path/to/foo.ts)",
					},
				} as ICompletedToolCall,
				URI.file("/"),
				"ssh__macbook-air",
			...
			assert.strictEqual(
				value,
				"Read [](vscode-agent-host://ssh__macbook-air/file/-/path/to/foo.ts)",
			);
		});
		...
		test("finalizes terminal tool with output and exit code", () => {
			const tc = createToolCallState({
				...
				content: [
					{
						type: ToolResultContentType.Terminal,
						resource: "agenthost-terminal:///t4",
						title: "Terminal",
					},
				],
			});
			...
			assert.strictEqual(termData.terminalCommandState.exitCode, 0);
			assert.strictEqual(
				IChatToolInvocation.resultDetails(invocation),
				undefined,
			);
		});
		...
		test("normalizes LF line endings to CRLF in terminal output for xterm rendering", () => {
			const tc = createToolCallState({
				...
				content: [
					{
						type: ToolResultContentType.Terminal,
						resource: "agenthost-terminal:///t5",
						title: "Terminal",
					},
				],
			});
		...
		test("returns file edits from completed tool call with FileEdit content", () => {
			...
			assert.strictEqual(
				fileEdits[0].resource.fsPath.replace(/\\/g, "/"),
				"/home/user/file.ts",
			);
			assert.strictEqual(
				fileEdits[0].beforeContentUri?.toString(),
				URI.parse("agenthost-content:///session/snap/before").toString(),
			);
			assert.strictEqual(
				fileEdits[0].afterContentUri?.toString(),
				URI.parse("agenthost-content:///session/snap/after").toString(),
			);
	...
	suite("activeTurnToProgress", () => {
		function createActiveTurnState(
			responseParts?: ActiveTurn["responseParts"],
		): ActiveTurn {
			return {
				id: "turn-active",
		...
		test("empty active turn produces empty progress", () => {
			const result = activeTurnToProgress(
				URI.file("/"),
		...
		test("produces markdown content for streamed text", () => {
			...
			assert.strictEqual(
				(result[0] as IChatMarkdownContent).content.value,
				"Hello world",
			);
		});
		...
		test("produces progress message for system notification", () => {
			...
			assert.strictEqual(
				(result[0] as IChatProgressMessage).content.value,
		...
		test("completed tool call with terminal content block sets terminalCommandUri", () => {
			const tc = createCompletedToolCall({
				...
				content: [
					{
						type: ToolResultContentType.Terminal,
						resource: "agenthost-terminal:///abc123",
						title: "Terminal",
					},
				],
				success: true,
			...
			const termData = serialized.toolSpecificData as {
				kind: "terminal";
				terminalCommandUri?: { toString(): string };
			};
			assert.ok(termData.terminalCommandUri);
			assert.strictEqual(
				termData.terminalCommandUri.toString(),
				"agenthost-terminal:/abc123",
			);
		});
		...
		test("running tool call with terminal content block sets terminalCommandUri", () => {
			const tc = createToolCallState({
				...
				content: [
					{
						type: ToolResultContentType.Terminal,
						resource: "agenthost-terminal:///running-term",
						title: "Terminal",
					},
				],
			});
			...
			assert.strictEqual(
				termData.terminalCommandUri.toString(),
				"agenthost-terminal:/running-term",
			);
		});
		...
		test("finalize preserves terminal URI from content block", () => {
			const tc = createToolCallState({
				...
				content: [
					{
						type: ToolResultContentType.Terminal,
						resource: "agenthost-terminal:///final-term",
						title: "Terminal",
					},
				],
			});
	...
	suite("updateRunningToolSpecificData", () => {
		test("sets subagent toolSpecificData from content and notifies state observers", () => {
			...
			assert.strictEqual(
				stateChanged,
				true,
				"state observers should be notified",
			);
			assert.notStrictEqual(
				invocation.toolSpecificData,
				before,
				"toolSpecificData should be replaced",
			);
			assert.strictEqual(invocation.toolSpecificData?.kind, "subagent");
			if (invocation.toolSpecificData?.kind === "subagent") {
		...
		test("does not notify when no subagent content is present", () => {
			...
			assert.strictEqual(
				invocation.toolSpecificData,
				originalData,
				"toolSpecificData should not change",
			);
		});
	});
});
