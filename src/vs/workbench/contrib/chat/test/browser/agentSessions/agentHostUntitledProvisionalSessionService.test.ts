...
} from "../../../browser/agentSessions/agentHost/agentHostUntitledProvisionalSessionService.js";
...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	override dispatch(
		channel: Parameters<IAgentHostService["dispatch"]>[0],
		action: Parameters<IAgentHostService["dispatch"]>[1],
	): void {
		this.dispatched.push({ channel, ...action } as IDispatchedAction);
	}

	override async resolveSessionConfig(
		...
		if (!next) {
			throw new Error(
				`No queued resolveSessionConfig response (call #${this.resolveCalls.length})`,
			);
		}
		return next;
	}
...
suite("AgentHostUntitledProvisionalSessionService", () => {
	...
	setup(async () => {
		...
		insta.stub(IWorkbenchEnvironmentService, {
			isSessionsWindow: false,
		} as Partial<IWorkbenchEnvironmentService>);
		provisional = ds.add(
			insta.createInstance(AgentHostUntitledProvisionalSessionService),
		);
	...
	test("applyConfigChange dispatches SessionConfigChanged synchronously after mutating entry.config", async () => {
		...
		const promise = provisional.applyConfigChange(ui, "copilot", undefined, {
			isolation: "worktree",
		});
		...
		// is still blocked).
		assert.strictEqual(
			agentHost.dispatched.length,
			1,
			"dispatched before re-resolve await",
		);
		assert.strictEqual(
			agentHost.dispatched[0].type,
			ActionType.SessionConfigChanged,
		);
		assert.deepStrictEqual(agentHost.dispatched[0].config, {
			isolation: "worktree",
		});
		assert.strictEqual(
			agentHost.dispatched[0].channel,
			expectedBackendUri("b").toString(),
		);
	...
	test("getResolvedConfig reflects the re-resolved schema/values after applyConfigChange", async () => {
		...
		await provisional.applyConfigChange(ui, "copilot", undefined, {
			isolation: "worktree",
		});
	...
	test("optimistic merge: overlay.values reflects partial before re-resolve completes", async () => {
		...
		assert.strictEqual(
			provisional.getResolvedConfig(ui)?.values?.["isolation"],
			"worktree",
		);
		...
		assert.strictEqual(
			mid?.values?.["branch"],
			"feature/x",
			"overlay value updated optimistically",
		);
		assert.strictEqual(
			mid?.values?.["isolation"],
			"worktree",
			"previous overlay values preserved",
		);

		blocked.complete({
			schema: makeSchema(false),
			values: { isolation: "worktree", branch: "feature/x" },
		});
		await promise;
	});
	...
	test("racing applyConfigChange calls: the second one wins (sequencer order)", async () => {
		...
		const a = provisional.applyConfigChange(ui, "copilot", undefined, {
			isolation: "worktree",
		});
		const b = provisional.applyConfigChange(ui, "copilot", undefined, {
			isolation: "folder",
		});
	...
	test("equals check skips onDidChange when re-resolved config is identical", async () => {
		...
		assert.strictEqual(
			changeFires,
			0,
			"no onDidChange fire when overlay is unchanged",
		);
	});
	...
	test("tryRebind sees latest entry.config from a synchronously-completed applyConfigChange", async () => {
		...
		void provisional.applyConfigChange(ui, "copilot", undefined, {
			isolation: "worktree",
		});
	...
	test("failed re-resolve preserves the previous overlay", async () => {
		...
		assert.deepStrictEqual(
			after?.schema,
			before.schema,
			"schema unchanged after failed re-resolve",
		);
