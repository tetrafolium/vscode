...
import {
	...
	ICarouselSection,
} from "../../browser/chatImageCarouselService.js";
...
suite("ChatImageCarouselService helpers", () => {
	...
	function makeRequest(
		id: string,
		variables: IChatRequestViewModel["variables"],
		messageText: string = "Request",
	): IChatRequestViewModel {
		return {
			id,
	...
	function makeResponse(
		requestId: string,
		id: string = "resp-1",
		responseValue: IChatResponseViewModel["response"]["value"] = [],
	): IChatResponseViewModel {
		return {
			id,
			requestId,
	...
	function makeImageVariableEntry(
		overrides: Partial<IImageVariableEntry> &
			Pick<IImageVariableEntry, "value">,
	): IImageVariableEntry {
	...
	): { id: string; name: string; mimeType: string; data: Uint8Array } {
	...
	function makeSections(...imageCounts: number[]): ICarouselSection[] {
		return imageCounts.map((count, sectionIdx) => ({
			...
			images: Array.from({ length: count }, (_, imgIdx) =>
				makeImage(
					URI.file(`/image_s${sectionIdx}_i${imgIdx}.png`).toString(),
					`image_s${sectionIdx}_i${imgIdx}.png`,
				),
			),
		}));
	}
	...
	suite("findClickedImageIndex", () => {
		...
		test("falls back to data buffer match", () => {
			...
			assert.strictEqual(
				findClickedImageIndex(sections, unknownUri, new Uint8Array([30, 40])),
				1,
			);
		});

		test("prefers a later exact URI match over an earlier image with identical data", () => {
			const firstUri = URI.parse(
				"vscode-chat-response-resource://session/tool-call-1/0/file.png",
			);
			const secondUri = URI.parse(
				"vscode-chat-response-resource://session/tool-call-2/0/file.png",
			);
			...
			assert.strictEqual(
				findClickedImageIndex(sections, secondUri, identicalData),
				1,
			);
		});
	...
	suite("collectCarouselSections", () => {
		...
		test("merges request images into matching response section", async () => {
			...
			const result = await collectCarouselSections(
				[request, response],
				async (uri) => VSBuffer.fromString(`data-for-${uri.path}`).buffer,
			);
		...
		test("uses tool image URIs as carousel image ids", async () => {
			const request = makeRequest(
				"req-1",
				[],
				"Request with tool output image",
			);
			const toolCallId = "tool-call-1";
			const sessionResource = URI.parse("chat-session://test/session");
			const expectedUri = ChatResponseResource.createUri(
				sessionResource,
				toolCallId,
				0,
				"file.png",
