...
} from "../../../../browser/widget/chatContentParts/toolInvocationParts/terminalToolAutoExpand.js";
...
suite("TerminalToolAutoExpand", () => {
	...
	test("fast command without data should not auto-expand (finishes before timeout)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand for fast command without data",
			);
		}));

	test("fast command with quick data should not auto-expand (data + finish before timeout)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			setupAutoExpandLogic();
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand when command finishes within timeout of first data",
			);
		}));

	test("long-running command with data should auto-expand (data received, command still running after timeout)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				true,
				"Should expand when command still running after first data timeout",
			);

			onCommandFinished.fire(undefined);
		}));

	test("long-running command with data but no real output should NOT auto-expand (like sleep with shell sequences)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand when data is shell sequences, not real output",
			);

			onCommandFinished.fire(undefined);
		}));

	test("long-running command without data should NOT auto-expand if no real output (like sleep)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand when no real output even after timeout",
			);

			onCommandFinished.fire(undefined);
		}));

	test("long-running command without data SHOULD auto-expand if real output exists", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				true,
				"Should expand when real output exists after timeout",
			);

			onCommandFinished.fire(undefined);
		}));

	test("data arriving after command finish should not trigger expand", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			setupAutoExpandLogic();
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand when data arrives after command finished",
			);
		}));

	test("user toggled output prevents auto-expand", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand when user has manually toggled output",
			);
			onCommandFinished.fire(undefined);
		}));

	test("already expanded output prevents additional auto-expand", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			isExpanded = true;
			...
			assert.strictEqual(
				eventFired,
				false,
				"Should NOT fire expand event when already expanded",
			);
			onCommandFinished.fire(undefined);
		}));

	test("data arriving with real output cancels no-data timeout (DataEvent path succeeds)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				true,
				"Should expand via DataEvent path when real output exists",
			);
	...
	test("data arriving without real output does NOT cancel no-data timeout (NoData path can still expand)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand when DataEvent fires without real output",
			);
			...
			assert.strictEqual(
				isExpanded,
				true,
				"NoData path should still expand when real output appears later",
			);

			onCommandFinished.fire(undefined);
		}));

	test("quick finish after data prevents expansion even with real output", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			hasRealOutputValue = true;
			setupAutoExpandLogic();
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand when command finishes before timeouts",
			);
		}));

	test("multiple data events only trigger one timeout", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			hasRealOutputValue = true; // Has real output
			setupAutoExpandLogic();
			...
			assert.strictEqual(
				isExpanded,
				true,
				"Should expand exactly once after first data",
			);
			onCommandFinished.fire(undefined);
		}));

	test("progress bar output detected via multiple data events (receivedDataCount > 1)", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				isExpanded,
				false,
				"Should NOT expand after first data event",
			);
			...
			assert.strictEqual(
				isExpanded,
				true,
				"Should expand when multiple data events detected as real output",
			);
