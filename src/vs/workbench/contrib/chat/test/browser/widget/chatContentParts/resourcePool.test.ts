...
import {
	ResourcePool,
	KeyedResourcePool,
} from "../../../../browser/widget/chatContentParts/chatCollections.js";
...
suite("ResourcePool", () => {
	...
	test("trimming disposes excess idle items after delay", () =>
	...
	test("trim timer is debounced on rapid releases", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.ok(
				!a.isDisposed,
				"should not be disposed yet (timer was debounced)",
			);
	...
	test("no trimming when maxIdleSize is not set", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const pool = createPool();
			...
			await timeout(50);
			assert.ok(
				items.every((i) => !i.isDisposed),
				"no items should be disposed without maxIdleSize",
			);
		}));
});
...
suite("KeyedResourcePool", () => {
	...
	function createPool(options?: {
		maxIdleSize?: number;
		trimIdleDelay?: number;
	}): KeyedResourcePool<MockPoolItem> {
	...
	test("keyed get falls back to any idle item when key not found", () => {
		...
		assert.strictEqual(
			b,
			a,
			"should return the idle item even with a different key",
		);
	});
	...
	test("trimming disposes excess idle items", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const pool = createPool({ maxIdleSize: 1, trimIdleDelay: 50 });
	...
	test("trimming cleans up key associations for disposed items", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.notStrictEqual(
				a,
				b,
				"should create new item since keyed item was trimmed",
			);
			assert.strictEqual(createCount, 2);
		}));
