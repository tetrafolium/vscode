...
suite("ChatTokenDeleter", () => {
	...
	function getChatTokenDeleterCtor() {
		const ctor = ChatWidget.CONTRIBS.find(
			(contrib) => contrib.name === "ChatTokenDeleter",
		);
		assert.ok(
			ctor,
			"ChatTokenDeleter should be registered as a chat widget contribution",
		);
		return ctor;
	}

	function createWidget(
		editor: IChatWidget["inputEditor"],
		onRefreshParsedInput: () => void,
	): IChatWidget {
		return {
			inputEditor: editor,
		...
		const testCases = [
			{
				initialValue: "/",
				insertedText: "/fix ",
				deleteRange: new Range(1, 5, 1, 6),
			},
			{
				initialValue: "@",
				insertedText: "@workspace ",
				deleteRange: new Range(1, 11, 1, 12),
			},
			{
				initialValue: "#",
				insertedText: "#selection",
				deleteRange: new Range(1, 10, 1, 11),
			},
		];
			...
			withTestCodeEditor(testCase.initialValue, {}, (editor) => {
				...
				try {
					...
					editor.executeEdits("test", [
						{ range: new Range(1, 1, 1, 2), text: testCase.insertedText },
					]);
					assert.strictEqual(editor.getValue(), testCase.insertedText);

					editor.executeEdits("test", [
						{ range: testCase.deleteRange, text: "" },
					]);
	...
	test("does not delete the whole token when backspacing inside the inserted token", () => {
		withTestCodeEditor("@", {}, (editor) => {
			...
			try {
				...
				editor.executeEdits("test", [
					{ range: new Range(1, 1, 1, 2), text: "@workspace " },
				]);
				editor.executeEdits("test", [
					{ range: new Range(1, 5, 1, 6), text: "" },
				]);
	...
	test("only deletes on the immediate next backspace after token insertion", () => {
		withTestCodeEditor("@", {}, (editor) => {
			...
			try {
				...
				editor.executeEdits("test", [
					{ range: new Range(1, 11, 1, 11), text: "x" },
				]);
				editor.executeEdits("test", [
					{ range: new Range(1, 11, 1, 12), text: "" },
				]);
