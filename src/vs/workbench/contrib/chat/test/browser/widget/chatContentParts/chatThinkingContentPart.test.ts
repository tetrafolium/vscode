...
} from "../../../../browser/widget/chatContentParts/chatThinkingContentPart.js";
import {
...
suite("ChatThinkingContentPart", () => {
	...
	test("replace thinking phrases suppresses fun default phrases", () => {
		mockConfigurationService.setUserConfiguration(
		...
		assert.strictEqual(
			maybePickFunWorkingMessage(mockConfigurationService, () => 0),
			undefined,
		);
	});
	...
	suite("ThinkingDisplayMode.Collapsed", () => {
		...
		test("should extract title from bold markdown", () => {
			...
			const button = part.domNode.querySelector(
			...
			assert.ok(
				labelElement?.textContent?.includes("Reading configuration files") ||
					button.textContent?.includes("Reading configuration files"),
	...
	suite("ThinkingDisplayMode.CollapsedPreview", () => {
		...
		test("should use lazy rendering when streamingCompleted is true even if element.isComplete is false", () => {
			...
			assert.strictEqual(
				contentList,
				null,
				"Content should not be rendered when streamingCompleted=true (collapsed = lazy)",
			);
		});
	});
	...
	suite("ThinkingDisplayMode.FixedScrolling", () => {
		setup(() => {
			mockConfigurationService.setUserConfiguration(
				"chat.agent.thinkingStyle",
				ThinkingDisplayMode.FixedScrolling,
			);
		});
		...
		test("should init content early (eager rendering)", () => {
			...
			const scrollableContent = part.domNode.querySelector(
				".monaco-scrollable-element",
			);
			assert.ok(
				scrollableContent,
				"Should have scrollable element in fixed mode (eager rendering)",
			);
		});
		...
		test("should create scrollable container", () => {
			...
			const scrollable = part.domNode.querySelector(
				".monaco-scrollable-element",
			);
	...
	suite("Thinking content updates", () => {
		...
		test("updateThinking should update content", () => {
			...
			const updatedContent = createThinkingPart(
				"**Updated thinking**",
				content.id,
			);
	...
	suite("Tool invocation appending", () => {
		...
		test("appendItem should use lazy rendering when collapsed", () => {
			...
			assert.strictEqual(
				factoryCalled,
				false,
				"Factory should not be called when collapsed (lazy rendering)",
			);
		});
		...
		test("lazy items should materialize when first expanded", () => {
			...
			assert.strictEqual(
				factoryCalled,
				true,
				"Factory should be called after expanding",
			);
		});
		...
		test("removeLazyItem should remove pending lazy items", () => {
			...
			assert.strictEqual(
				removed,
				true,
				"Should successfully remove the lazy item",
			);
			assert.strictEqual(
				factoryCalled,
				false,
		...
		test("lazy items should preserve append order when mixing tool and markdown items", () => {
			...
			assert.strictEqual(
				appendOrder.length,
				0,
				"No items should be rendered while collapsed",
			);
			...
			assert.strictEqual(
				appendOrder.length,
				3,
				"All 3 items should be rendered after expanding",
			);
			...
			const toolWrappers = wrapper?.querySelectorAll(
		...
		test("setupThinkingContainer should preserve order with lazy tool items", () => {
			...
			const initialContent = createThinkingPart(
				"**Initial thinking**",
				"thinking-1",
			);
			const context = createMockRenderContext(false);
			...
			const newThinkingContent = createThinkingPart(
			...
			assert.strictEqual(
				tool1Rendered,
				false,
				"Tool 1 should not render while collapsed",
			);
			assert.strictEqual(
				tool2Rendered,
				false,
				"Tool 2 should not render while collapsed",
			);
			...
			assert.strictEqual(
				tool1Rendered,
				true,
				"Tool 1 should render after expand",
			);
			assert.strictEqual(
				tool2Rendered,
				true,
				"Tool 2 should render after expand",
			);
			...
			const thinkingItems = children.filter((el) =>
				el.classList.contains("chat-thinking-item"),
			);
			...
			assert.ok(
				...
				"Should have at least one thinking item",
			);
		...
		test("markdown via updateThinking should preserve order with lazy tool items (BUG: markdown renders before tools)", () => {
			...
			const thinkingContent = createThinkingPart(
			...
			const markdownIndex = children.findIndex(
				(el) =>
					el.classList.contains("chat-thinking-item") &&
					el.classList.contains("markdown-content"),
			);

			assert.ok(
				tool1Index >= 0,
				`Should find tool1 in DOM (found at index ${tool1Index})`,
			);
			assert.ok(
				tool2Index >= 0,
				`Should find tool2 in DOM (found at index ${tool2Index})`,
			);
			assert.ok(
				markdownIndex >= 0,
				`Should find markdown in DOM (found at index ${markdownIndex})`,
			);
		...
		test("lazy thinking items should show updated content after streaming updates", () => {
			...
			const thinkingContent1 = createThinkingPart(
				"**Starting analysis**",
				"thinking-2",
			);
			...
			const thinkingContent2 = createThinkingPart(
				"**Starting analysis** Looking at the code structure...",
				"thinking-2",
			);
			part.updateThinking(thinkingContent2);

			// Step 3: Even more streaming content
			const thinkingContent3 = createThinkingPart(
				"**Starting analysis** Looking at the code structure... Found the issue in the parser module.",
				"thinking-2",
			);
			...
			const thinkingItems = wrapper!.querySelectorAll(
			...
			assert.strictEqual(
				...
				`BUG: Should have exactly 1 thinking item, but got ${thinkingItems.length}. ` +
					...
					`Items: ${Array.from(thinkingItems)
						.map((i) => `"${i.textContent}"`)
						.join(", ")}`,
			);
		...
		test("lazy thinking items should work without streaming updates after setupThinkingContainer", () => {
			...
			assert.strictEqual(
				thinkingItems.length,
				1,
				"Should have exactly 1 thinking item",
			);
	...
	suite("State management", () => {
		...
		test("finalizeTitleIfDefault should retain restored terminal title", () => {
			...
			part.appendItem(
				() => ({ domNode: $("div.test-terminal-tool") }),
				terminalTool.toolId,
				terminalTool,
			);
			part.finalizeTitleIfDefault();
	...
	suite("hasSameContent", () => {
		...
		test("should return true for tool invocations", () => {
			...
			assert.strictEqual(
				result,
				true,
				"Should accept tool invocations as same content",
			);
		});
		...
		test("should return true for markdown content", () => {
			...
			assert.strictEqual(
				result,
				true,
				"Should accept markdown content as same content",
			);
		});
		...
		test("should return false for different thinking part with same id", () => {
			...
			const otherThinking: IChatRendererContent = createThinkingPart(
				"**Different**",
				"id-1",
			);
			...
			assert.strictEqual(
				result,
				false,
				"Should return false for thinking part with same id",
			);
		});
		...
		test("should return true for thinking part with different id", () => {
			...
			assert.strictEqual(
				result,
				true,
				"Should return true for thinking part with different id",
			);
		});
	});
	...
	suite("DOM structure", () => {
		...
		function createMockStreamingToolInvocation(
			toolId: string,
		...
		function createMockSerializedImageToolInvocation(
			toolId: string,
			invocationMessage: string,
			toolCallId: string,
		): IChatToolInvocationSerialized {
		...
		test('should show "Editing files" for streaming edit tools instead of generic display name', () => {
			...
			part.appendItem(
				...
				streamingReplaceTool.toolId,
				streamingReplaceTool,
			);
			...
			assert.ok(button, "Should have collapse button");
			const labelText =
				button.querySelector(".icon-label")?.textContent ??
				button.textContent ??
				"";
			assert.ok(
				labelText.includes("Editing files"),
				`Title should contain "Editing files" but got "${labelText}"`,
			);
		});
		...
		test("should show original message for non-edit streaming tools", () => {
			...
			const labelText =
				...
				"";
			assert.ok(
				labelText.includes("Reading file.ts"),
				`Title should contain "Reading file.ts" but got "${labelText}"`,
			);
		});
		...
		test("should show original message for non-streaming edit tools", () => {
			...
			part.appendItem(
				...
				executingReplaceTool.toolId,
				executingReplaceTool,
			);
			...
			const labelText =
				...
				"";
			assert.ok(
				labelText.includes("Replacing 5 lines in file.ts"),
				`Title should contain "Replacing 5 lines in file.ts" but got "${labelText}"`,
			);
		});
		...
		test("should keep original message for create_file tool even when streaming", () => {
			...
			const labelText =
				...
				"";
			assert.ok(
				labelText.includes("Creating newFile.ts"),
				`Title should contain "Creating newFile.ts" but got "${labelText}"`,
			);
		});
		...
		test("should show external resources for serialized image tools when initially collapsed and hide them when expanded", () => {
			...
			part.appendItem(
				...
				serializedImageTool.toolId,
				serializedImageTool,
			);

			const externalResources = part.domNode.querySelector(
				".chat-thinking-external-resources",
			) as HTMLElement;
			assert.ok(
				externalResources,
				"Should render external resources container",
			);
			assert.notStrictEqual(
				externalResources.style.display,
				"none",
				"Should show external resources while initially collapsed",
			);
			...
			assert.strictEqual(
				externalResources.style.display,
				"none",
				"Should hide external resources when expanded",
			);
			...
			assert.notStrictEqual(
				externalResources.style.display,
				"none",
				"Should show external resources again after collapsing",
			);
		});
	});
	...
	suite("Diff aggregation in thinking header", () => {
		...
		test("should include diff stats in aria-label", () => {
			...
			assert.ok(
				button?.ariaLabel?.includes("7"),
				"aria-label should include added count",
			);
			assert.ok(
				button?.ariaLabel?.includes("2"),
				"aria-label should include removed count",
			);
		});
		...
		test("should not show diff stats when no diff events fired", () => {
			...
			const diffContainer = part.domNode.querySelector(
				".chat-thinking-title-diff",
			);
			assert.strictEqual(
				diffContainer,
				null,
				"Should not render diff container when no diffs exist",
			);
		});
	...
	suite("eagerDisposable lifecycle", () => {
		...
		test("eagerDisposable is disposed when thinking part is disposed even if factory was never called", () => {
			...
			assert.strictEqual(
				disposed,
				true,
				"eagerDisposable should be disposed with the thinking part",
			);
		});
		...
		test("eagerDisposable is disposed when thinking part is disposed after factory was called", () => {
			...
			assert.strictEqual(
				disposed,
				true,
				"eagerDisposable should be disposed even after being materialized",
			);
		});
		...
		test("appendItem without eagerDisposable disposes factory result on thinking part disposal", () => {
			...
			assert.strictEqual(
				disposed,
				true,
				"Factory disposable should be disposed with thinking part",
			);
		});
	});
});
