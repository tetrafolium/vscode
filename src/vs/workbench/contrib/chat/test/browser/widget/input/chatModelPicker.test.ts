...
} from "../../../../browser/widget/input/chatModelPicker.js";
...
const stubLanguageModelsService = {
 ...
 * service.
 */
function createLanguageModelsServiceStub(
	vendors: {
		vendor: string;
		displayName: string;
		groups: { name: string; modelIdentifiers: string[] }[];
	}[],
): ILanguageModelsService {
	return {
		getModelConfigurationActions: () => [],
		getModelConfiguration: () => undefined,
		getVendors: () =>
			vendors.map((v) => ({ vendor: v.vendor, displayName: v.displayName })),
...
suite("buildModelPickerItems", () => {
	...
	test("accessibility provider uses radio semantics for model items", () => {
		...
		assert.strictEqual(
			...
			} as IActionListItem<IActionWidgetDropdownAction>),
			"menuitemradio",
		);
		assert.strictEqual(
			provider.getRole({
	...
	test("edu entitlement uses free featured control manifest", () => {
		...
		assert.strictEqual(
			getControlModelsForEntitlement(manifest, ChatEntitlement.EDU),
			manifest.free,
		);
	});
	...
	test("Other Models places unavailable models after available models", () => {
		...
		const actions = getActionItems(items);
		const otherModelLabels = actions
			.slice(2)
			.map((a) => a.label!)
			.filter((l) => !l.includes("Manage Models"));
		assert.deepStrictEqual(otherModelLabels, ["Zeta", "Alpha"]);
		assert.strictEqual(
			actions.find((a) => a.label === "Alpha")?.disabled,
			true,
		);
	});
	...
	test("no duplicate models across sections", () => {
		...
		const labels = getActionLabels(items).filter(
			(l) => l !== "Other Models" && !l.includes("Manage Models"),
		);
		const uniqueLabels = new Set(labels);
		assert.strictEqual(
			labels.length,
			uniqueLabels.size,
			`Duplicate labels found: ${labels.join(", ")}`,
		);
	});
	...
	test("Other Models grouped by vendor with separator headers", () => {
		...
		const vendorSeparators = items.filter(
			(i) => i.kind === ActionListItemKind.Separator && i.label,
		);
		...
		const actions = getActionItems(items);
		const otherModelLabels = actions
			.filter((a) => !a.isSectionToggle && a.section === "other")
			.map((a) => a.label!);
	...
	test("Other Models splits a single vendor into per-group sections (BYOK)", () => {
		...
		const labelledSeparators = items.filter(
			(i) => i.kind === ActionListItemKind.Separator && i.label,
		);
		assert.deepStrictEqual(
			labelledSeparators.map((s) => s.label),
	...
	test("Other Models keeps a single section when a vendor has only one group (BYOK)", () => {
		...
		const items = callBuild([auto, gpt41], {
			languageModelsService: lmService,
		});
		const labelledSeparators = items.filter(
			(i) => i.kind === ActionListItemKind.Separator && i.label,
	...
	test("onSelect callback is wired into action items", () => {
		...
		const onSelect = (m: ILanguageModelChatMetadataAndIdentifier) => {
			selectedModel = m;
		};
	...
	test("admin unavailable model shows manage settings link in description", () => {
		const auto = createAutoModel();
		const businessEntitlementService = createStubEntitlementService({
		...
		assert.ok(
			description.value.includes("https://aka.ms/github-copilot-settings"),
		);
	});
	...
	test("anonymous user model selection triggers onSelect normally", () => {
		...
		const anonymousEntitlementService = createStubEntitlementService({
			entitlement: ChatEntitlement.Unknown,
			anonymous: true,
		});
		const items = buildModelPickerItems(
		...
		const promotedLabels = actions
			.filter(
				(a) =>
					!a.isSectionToggle &&
					a.section !== "other" &&
					a.item?.id !== "manageModels",
			)
			.map((a) => a.label);
		assert.ok(
			!promotedLabels.includes("GPT-4o"),
			"GPT-4o should not be in promoted section when showFeatured=false",
		);
	});
	...
	test("showUnavailableFeatured=false omits unavailable featured models from promoted section", () => {
		...
		assert.strictEqual(
			premiumItem,
			undefined,
			"Unavailable featured model should not appear when showUnavailableFeatured=false",
		);
	});
	...
	test("showUnavailableFeatured=false still shows available featured models", () => {
		...
		assert.ok(
			gptItem,
			"Available featured model should appear even when showUnavailableFeatured=false",
		);
	});
	...
	test("showUnavailableFeatured=false with version-gated model allows it in Other Models", () => {
		...
		const promotedGpt = actions.find(
			(a) => a.label === "GPT-4o" && a.section !== "other",
		);
		assert.strictEqual(
			promotedGpt?.disabled,
			undefined,
		...
		const otherGpt = actions.find(
			(a) => a.label === "GPT-4o" && a.section === "other",
		);
		assert.ok(
			otherGpt,
			"Version-gated featured model should appear in Other Models when showUnavailableFeatured=false",
		);
	});
	...
	test("model description hides non-multiplier pricing from description", () => {
		...
		modelA.metadata = {
			...modelA.metadata,
			detail: "Provider",
			pricing: "In: 2.04 · Out: 4.34 AICs/1M tokens",
		} as ILanguageModelChatMetadata;
		const items = callBuild([auto, modelA]);
		const gptItem = getActionItems(items).find((a) => a.label === "GPT-4o");
		assert.ok(gptItem);
	...
	test("pinned models appear in dedicated pinned section", () => {
		...
		const pinnedSep = items.find(
			(i) => i.kind === ActionListItemKind.Separator && i.label === "Pinned",
		);
		...
		const firstPinned = afterPinned.find(
			(i) => i.kind === ActionListItemKind.Action,
		);
	...
	test("pinned models do not appear in MRU/promoted section", () => {
		...
		assert.strictEqual(
			gptItems.length,
			1,
			"Pinned model should appear exactly once",
		);
	});
	...
	test("MRU is capped at 3 after filtering pinned models", () => {
		...
		assert.ok(
			!promotedLabels.includes("Model 1"),
			"Pinned model should not be in MRU",
		);
	});
	...
	test("no pinned section when pinnedModelIds is empty", () => {
		...
		assert.strictEqual(
			pinnedSep,
			undefined,
			"No pinned separator when there are no pinned models",
		);
	});
});
 ...
 */
suite(
	"chat model picker - languageModelChatProvider visibility regression",
	() => {
		ensureNoDisposablesAreLeakedInTestSuite();
		...
		function buildLmService(
			...
			return createLanguageModelsServiceStub(
				vendors.map((v) => ({
					...
					groups: [
						{ name: v.displayName, modelIdentifiers: v.modelIdentifiers },
					],
				})),
			);
		}
		...
		test("regression: third-party model with isUserSelectable omitted is shown in the picker", () => {
			...
			const tp = createThirdPartyModel("tp", "TP", {
			...
			const lmService = buildLmService([
				{
					...
					modelIdentifiers: [tp.identifier],
				},
			]);
		...
		test("regression: copilot internal model (isUserSelectable: false) is hidden from the picker", () => {
			const internal = createCopilotModel("internal", "Internal", {
			...
			const lmService = buildLmService([
				{
					...
					modelIdentifiers: [internal.identifier],
				},
			]);

			const labels = runPickerPipeline([internal], lmService).map(
				(i) => i.label,
			);
			assert.deepStrictEqual(
				labels,
		...
		test("regression: copilot model with omitted isUserSelectable defaults to visible", () => {
			...
			const model = createCopilotModel("public", "Public", {
				isUserSelectable: undefined,
			});
			const lmService = buildLmService([
				{
					vendor: "copilot",
					displayName: "GitHub Copilot",
					modelIdentifiers: [model.identifier],
				},
			]);
		...
		test("regression: copilot public model (isUserSelectable: true) is shown in the picker", () => {
			const pub = createCopilotModel("gpt-4o", "GPT-4o", {
				isUserSelectable: true,
			});
			const lmService = buildLmService([
				{
					vendor: "copilot",
					displayName: "GitHub Copilot",
					modelIdentifiers: [pub.identifier],
				},
			]);
		...
		test("regression: mixed vendors - only explicit isUserSelectable: false models are hidden", () => {
			const copilotPublic = createCopilotModel("gpt-4o", "GPT-4o", {
				isUserSelectable: true,
			});
			const copilotInternal = createCopilotModel("internal", "Internal", {
				isUserSelectable: false,
			});
			const tpTrue = createThirdPartyModel("tp-true", "TP True", {
				isUserSelectable: true,
			});
			const tpFalse = createThirdPartyModel("tp-false", "TP False", {
				isUserSelectable: false,
			});
			const tpUndefined = createThirdPartyModel("tp-undef", "TP Undef", {
				isUserSelectable: undefined,
			});
			...
			const lmService = buildLmService([
				{
					...
					displayName: "GitHub Copilot",
					modelIdentifiers: [
						copilotPublic.identifier,
						copilotInternal.identifier,
					],
				},
				{
					vendor: "my-vendor",
		...
		test("regression: third-party models without explicit opt-out match the configuration view", () => {
			...
			const configurationView = lmService
				.getLanguageModelGroups("my-vendor")
				.flatMap((g) => g.modelIdentifiers)
				.sort();
			const picker = runPickerPipeline(allThirdParty, lmService)
				.map(
					(i) =>
						allThirdParty.find((m) => m.metadata.name === i.label)!.identifier,
				)
				.sort();
