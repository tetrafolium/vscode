	...
	for (const rows of tables) {
		...
		rows.forEach((rowData, rowIndex) => {
			...
			for (const text of rowData) {
				const cell =
					rowIndex === 0 ? document.createElement("th") : tr.insertCell();
...
suite("wrapTablesWithScrollable", () => {
	...
	function wrap(container: HTMLDivElement): {
		layoutParticipants: Set<() => void>;
	} {
		const layoutParticipants = new Set<() => void>();
		store.add(
			wrapTablesWithScrollable(container, new Lazy(() => layoutParticipants)),
		);
	...
	test("table is preserved inside the scroll wrapper", () => {
		...
		assert.ok(
			!container.children[0].isSameNode(table),
			"table should not be a direct child anymore",
		);
	});
	...
	test("registers a layout participant for each table", () => {
		...
		assert.strictEqual(
			layoutParticipants.size,
			2,
			"one layout participant registered per table",
		);
	});
	...
	test("does not set min-width on single-character columns", () => {
		...
		const table = container.querySelector("table")!;
		assert.strictEqual(
			table.rows[0].cells[0].style.minWidth,
			"",
			"single-char column should have no min-width",
		);
	});
