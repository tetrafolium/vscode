...
function computeAvailableModels(
	...
	const merged = mergeModelsWithCache(
		liveModels,
		cachedModels,
		contributedVendors,
		resolvedVendors,
...
suite("ChatModelSelectionLogic", () => {
	...
	suite("isModelSupportedForMode", () => {
		test("any model is supported in Ask mode", () => {
			...
			assert.strictEqual(
				isModelSupportedForMode(model, ChatModeKind.Ask),
				true,
			);
		});
		...
		test("any model is supported in Edit mode", () => {
			const model = createModel("basic", "Basic", { capabilities: undefined });
			assert.strictEqual(
				isModelSupportedForMode(model, ChatModeKind.Edit),
				true,
			);
		});
		...
		test("model with tool calling and agent mode is supported in Agent mode", () => {
			...
			assert.strictEqual(
				isModelSupportedForMode(model, ChatModeKind.Agent),
				true,
			);
		});
		...
		test("model with no capabilities is NOT supported in Agent mode", () => {
			const model = createModel("no-caps", "No-Caps", {
	...
	suite("isModelSupportedForInlineChat", () => {
		test("any model is supported when not in EditorInline location", () => {
			const model = createModel("basic", "Basic", { capabilities: undefined });
			assert.strictEqual(
				isModelSupportedForInlineChat(model, ChatAgentLocation.Chat),
				true,
			);
			assert.strictEqual(
				isModelSupportedForInlineChat(model, ChatAgentLocation.Terminal),
				true,
			);
			assert.strictEqual(
				isModelSupportedForInlineChat(model, ChatAgentLocation.Notebook),
				true,
			);
		});
		...
		test("model with tool calling is supported in EditorInline", () => {
			...
			assert.strictEqual(
				isModelSupportedForInlineChat(model, ChatAgentLocation.EditorInline),
				true,
			);
		});
	...
	suite("filterModelsForSession", () => {
		...
		const notSelectable = createModel("hidden", "Hidden", {
		...
		test.skip("filters by mode for session-targeted models", () => {
			const cloudNoTools = createSessionModel(
				"cloud-basic",
				"Cloud Basic",
				"cloud",
				{
		...
		test("excludes non-selectable models from session-targeted results", () => {
			const cloudHidden = createSessionModel(
				"cloud-hidden",
				"Cloud Hidden",
				"cloud",
				{
		...
		test("filters inline chat incompatible models in EditorInline", () => {
			const noToolsSelectable = createModel(
				"no-tools-selectable",
				"No-Tools-Selectable",
				{
					capabilities: { toolCalling: false },
	...
	suite("hasModelsTargetingSession", () => {
		...
		test("returns false for different session type", () => {
			...
			assert.strictEqual(
				hasModelsTargetingSession(models, "enterprise"),
				false,
			);
		});
	});
	...
	suite("isModelValidForSession", () => {
		...
		test("session-targeted model is NOT valid when no models target the session type in pool", () => {
			...
			const generalModel = createModel("gpt", "GPT");
			assert.strictEqual(
				isModelValidForSession(sessionModel, [generalModel], undefined),
				false,
			);
		});
		...
		test("session-targeted model IS valid when pool has models targeting that session", () => {
			...
			assert.strictEqual(
				isModelValidForSession(sessionModel, allModels, "cloud"),
				true,
			);
		});
		...
		test("model targeting wrong session is NOT valid", () => {
			const wrongSessionModel = createSessionModel(
				"ent-gpt",
				"Enterprise GPT",
				"enterprise",
			);
			...
			assert.strictEqual(
				isModelValidForSession(wrongSessionModel, allModels, "cloud"),
				false,
			);
		});
		...
		test("general model is valid when session type is undefined", () => {
			const generalModel = createModel("gpt", "GPT");
			assert.strictEqual(
				isModelValidForSession(generalModel, [generalModel], undefined),
				true,
			);
		});
	});
	...
	suite("findDefaultModel", () => {
		test("returns model marked as default for location", () => {
			...
			const defaultModel = createDefaultModelForLocation(
				"claude",
				"Claude",
				ChatAgentLocation.Chat,
			);
			const result = findDefaultModel(
				[regular, defaultModel],
				ChatAgentLocation.Chat,
			);
		...
		test("returns location-specific default when multiple defaults exist", () => {
			const chatDefault = createDefaultModelForLocation(
				"chat-default",
				"Chat Default",
				ChatAgentLocation.Chat,
			);
			const terminalDefault = createDefaultModelForLocation(
				"terminal-default",
				"Terminal Default",
				ChatAgentLocation.Terminal,
			);
			const result = findDefaultModel(
				[chatDefault, terminalDefault],
				ChatAgentLocation.Chat,
			);
	...
	suite("shouldRestorePersistedModel", () => {
		...
		test("restores model that was default and is still default", () => {
			const model = createDefaultModelForLocation(
				"gpt",
				"GPT",
				ChatAgentLocation.Chat,
			);
		...
		test("default tracking: follows new default when user never explicitly chose", () => {
			...
			const gpt = createModel("gpt-4o", "GPT-4o");
			const claude = createDefaultModelForLocation(
				"claude",
				"Claude",
				ChatAgentLocation.Chat,
			);
	...
	suite("shouldResetModelToDefault", () => {
		...
		test("should reset when current model is undefined", () => {
			assert.strictEqual(
				shouldResetModelToDefault(undefined, [], defaultContext, []),
				true,
			);
		});
		...
		test("should reset when model is no longer available", () => {
			const model = createModel("gpt", "GPT");
			assert.strictEqual(
				shouldResetModelToDefault(model, [], defaultContext, [model]),
				true,
			);
		});
		...
		test("should NOT reset when model is available and compatible", () => {
			const model = createModel("gpt", "GPT");
			assert.strictEqual(
				shouldResetModelToDefault(model, [model], defaultContext, [model]),
				false,
			);
		});
		...
		test("should reset when model is not supported for current mode", () => {
			...
			assert.strictEqual(
				shouldResetModelToDefault(model, [model], context, [model]),
				true,
			);
		});
		...
		test("should NOT reset session model in matching session", () => {
			...
			const context = { ...defaultContext, sessionType: "cloud" };
			assert.strictEqual(
				shouldResetModelToDefault(sessionModel, [sessionModel], context, [
					sessionModel,
				]),
				false,
			);
	...
	suite("mergeModelsWithCache", () => {
		...
		test("merges cached models from vendors not yet resolved", () => {
			const liveModel = createModel("gpt", "GPT");
			const cachedOtherVendor = createModel("other-model", "Other Model", {
				vendor: "other-vendor",
			});
			const result = mergeModelsWithCache(
				[liveModel],
			...
			assert.deepStrictEqual(result.map((m) => m.metadata.id).sort(), [
				"gpt",
				"other-model",
			]);
		});
		...
		test("evicts cached models from vendors no longer contributed", () => {
			const liveModel = createModel("gpt", "GPT");
			const cachedRemovedVendor = createModel(
				"removed-model",
				"Removed Model",
				{ vendor: "removed-vendor" },
			);
			const result = mergeModelsWithCache(
				[liveModel],
				[cachedRemovedVendor],
		...
		test("handles multiple vendors with partial resolution", () => {
			...
			assert.strictEqual(result.length, 2);
			assert.deepStrictEqual(result.map((m) => m.metadata.vendor).sort(), [
				"vendor-a",
				"vendor-b",
			]);
		});
	...
	suite("model switching scenarios", () => {
		...
		test("persisted model selection does NOT restore when a new default is assigned", () => {
			...
			const gpt4o = createModel("gpt-4o", "GPT-4o");
			const newDefault = createDefaultModelForLocation(
				"claude",
				"Claude",
				ChatAgentLocation.Chat,
			);
		...
		test("combining mode switch + session switch validates correctly", () => {
			const cloudToolModel = createSessionModel(
				"cloud-tool",
				"Cloud Tool",
				"cloud",
				{
			...
			const cloudNoToolModel = createSessionModel(
				"cloud-basic",
				"Cloud Basic",
				"cloud",
				{
	...
	suite("onDidChangeLanguageModels race conditions", () => {
		test("model temporarily removed then re-added loses user choice", () => {
			...
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/gpt", [gpt, claude]),
				false,
			);

			// Step 2: Extension reloads, GPT temporarily disappears from model list
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/gpt", [claude]),
				true,
			);
			...
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/claude", [gpt, claude]),
				false,
			);
		...
		test("reset when the selected model is hidden from the available models", () => {
			const gpt = createModel("gpt", "GPT");
			const claude = createModel("claude", "Claude");
			const visibleModels = [gpt, claude].filter(
				(model) => model.identifier !== gpt.identifier,
			);

			assert.strictEqual(
				shouldResetOnModelListChange(gpt.identifier, visibleModels),
				true,
			);
		});
		...
		test("cache bridges the gap when live models temporarily unavailable", () => {
			...
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/gpt", merged),
				false,
			);
		});
		...
		test("cache evicted for uncontributed vendor once live models arrive", () => {
			const cachedGpt = createModel("gpt", "GPT");
			const liveOther = createModel("other", "Other", {
				vendor: "other-vendor",
			});
	...
	suite("late-arriving model restoration", () => {
		...
		test("does NOT restore model that is not user-selectable", () => {
			const model = createModel("internal", "Internal", {
				isUserSelectable: false,
			});
			assert.strictEqual(
				shouldRestoreLateArrivingModel(
		...
		test("restores model with isUserSelectable=undefined (defaults to selectable)", () => {
			const model = createModel("undef-sel", "Undef-Sel", {
	...
	suite("_syncFromModel edge cases", () => {
		...
		test("sync state with model matching different session type falls back to default", () => {
			const enterpriseModel = createSessionModel(
				"ent-gpt",
				"Enterprise GPT",
				"enterprise",
			);
	...
	suite("checkModelSupported interaction patterns", () => {
		...
		test("initSelectedModel → checkModelSupported: restored model passes Agent check", () => {
			...
			const restoreResult = shouldRestorePersistedModel(
				"copilot/agent-model",
				false,
				[agentModel],
				ChatAgentLocation.Chat,
			...
			assert.strictEqual(
				shouldResetModelToDefault(agentModel, [agentModel], agentContext, [
					agentModel,
				]),
				false,
			);
		...
		test("double reset is idempotent", () => {
			...
			assert.strictEqual(
				shouldResetModelToDefault(result1!, allModels, askContext, allModels),
				false,
			);
		});
	});
	...
	suite("multiple session types and cross-contamination", () => {
		test("model from session A rejected in session B", () => {
			...
			assert.strictEqual(
				isModelValidForSession(cloudModel, allModels, "enterprise"),
				false,
			);
			// Enterprise model not valid in cloud session
			assert.strictEqual(
				isModelValidForSession(enterpriseModel, allModels, "cloud"),
				false,
			);
		...
		test("general model is valid when session type has no targeted models", () => {
			...
			assert.strictEqual(
				isModelValidForSession(generalModel, allModels, "enterprise"),
				true,
			);
		});
		...
		test("filterModelsForSession isolates session types correctly", () => {
			...
			const enterprise = createSessionModel(
				"ent-gpt",
				"Enterprise GPT",
				"enterprise",
			);
			...
			const cloudFiltered = filterModelsForSession(
				allModels,
				"cloud",
				ChatModeKind.Ask,
				ChatAgentLocation.Chat,
			...
			assert.deepStrictEqual(
				cloudFiltered.map((m) => m.metadata.id),
				["cloud-gpt"],
			);

			const entFiltered = filterModelsForSession(
				allModels,
				"enterprise",
				ChatModeKind.Ask,
				ChatAgentLocation.Chat,
			...
			assert.deepStrictEqual(
				entFiltered.map((m) => m.metadata.id),
				["ent-gpt"],
			);

			const generalFiltered = filterModelsForSession(
				allModels,
				undefined,
				ChatModeKind.Ask,
				ChatAgentLocation.Chat,
			...
			assert.deepStrictEqual(
				generalFiltered.map((m) => m.metadata.id),
				["gpt"],
			);
		});
	...
	suite("mode with forced model (mode.model property)", () => {
		...
		test("mode forces model — copilot vendor shorthand works", () => {
			...
			const match = [gpt].find((m) =>
				ILanguageModelChatMetadata.matchesQualifiedName("GPT-4o", m.metadata),
			);
			assert.strictEqual(match?.metadata.id, "gpt-4o");
		});
	...
	suite("EditorInline + mode combined scenarios", () => {
		test("EditorInline + Agent requires both agentMode and toolCalling", () => {
			...
			assert.strictEqual(
				isModelSupportedForMode(partialModel, ChatModeKind.Agent),
				false,
			);
	...
	suite("findDefaultModel edge cases", () => {
		...
		test("default for one location does not leak to another", () => {
			...
			assert.strictEqual(
				findDefaultModel([noDefault, chatDefault], ChatAgentLocation.Chat)
					?.metadata.id,
				"chat-def",
			);
			// For Terminal: no model is default, so first model wins
			assert.strictEqual(
				findDefaultModel([noDefault, chatDefault], ChatAgentLocation.Terminal)
					?.metadata.id,
				"no-def",
			);
		});
	...
	suite("realistic multi-step race simulations", () => {
		test("startup: cached model → live models arrive → user choice preserved", () => {
			...
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/gpt", cachedModels),
				false,
			);
			...
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/gpt", liveModels),
				false,
			);
		});
		...
		test("startup: no cache → models arrive late → persisted choice restored", () => {
			...
			const emptyModels = computeAvailableModels(
				[],
				[],
				new Set(["copilot"]),
				undefined,
		...
		test("extension reload: selected model flickers out then back", () => {
			...
			const duringReload = mergeModelsWithCache(
				[],
				[gpt, claude],
				new Set(["copilot"]),
			);
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/gpt", duringReload),
				false,
			);

			// Step 3: Extension finishes loading, live models back
			const afterReload = mergeModelsWithCache(
				[gpt, claude],
				[gpt, claude],
				new Set(["copilot"]),
			);
			assert.strictEqual(
				shouldResetOnModelListChange("copilot/gpt", afterReload),
				false,
			);
		});
		...
		test("session switch race: mode + session change together", () => {
			const generalDefault = createDefaultModelForLocation(
				"gpt",
				"GPT",
				ChatAgentLocation.Chat,
			);
			...
			const cloudDefault = findDefaultModel(
				[cloudModel],
				ChatAgentLocation.Chat,
			);
		...
		test("rapid mode changes: ask → agent resets incompatible, then agent → ask does not restore", () => {
			...
			const toolModel = createDefaultModelForLocation(
				"tool",
				"Tool",
				ChatAgentLocation.Chat,
			);
			...
			const defaultAfterReset = findDefaultModel(
				allModels,
				ChatAgentLocation.Chat,
			);
