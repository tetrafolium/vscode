...
} from "../../../../browser/widget/chatContentParts/chatIncrementalRendering/animations/blockAnimations.js";
...
suite("lastBlockBoundary", () => {
	...
	test("handles mixed backtick and tilde fences", () => {
		...
		assert.ok(
			result > 40,
			`Expected boundary after both fences, got ${result}`,
		);
	});
});
...
suite("IncrementalDOMMorpher", () => {
	...
	function createMorpher(domNode?: HTMLElement): IncrementalDOMMorpher {
		...
		return store.add(
			instantiationService.createInstance(IncrementalDOMMorpher, node),
		);
	}
	...
	suite("tryMorph", () => {
		...
		test("schedules render for content without any paragraph breaks", async () => {
			...
			assert.strictEqual(
				rendered[1],
				"single block no paragraph breaks — more words",
			);
		});
	});
	...
	suite("animation style", () => {
		...
		test("uses configured animation style", () => {
			...
			const child = domNode.children[0] as HTMLElement;
			assert.strictEqual(
				child.classList.contains("chat-smooth-animate-rise"),
				true,
				"Should use rise style",
			);
		});
...
suite("BlockAnimation", () => {
	...
	test("does not strip animation class on bubbled animationend from nested element", () => {
		...
		const container = mainWindow.document.createElement("div");
		const parent = container.appendChild(
			mainWindow.document.createElement("div"),
		);
		const nested = parent.appendChild(
			mainWindow.document.createElement("span"),
		);
	...
	test("staggers delay across multiple new children", () => {
		...
		const delays = Array.from(container.children).map((c) =>
			parseInt(
				(c as HTMLElement).style.getPropertyValue("--chat-smooth-delay"),
			),
		);
		// Each successive child should have a larger delay
		assert.ok(
			delays[1] > delays[0],
			`Second delay ${delays[1]} should be greater than first ${delays[0]}`,
		);
		assert.ok(
			delays[2] > delays[1],
			`Third delay ${delays[2]} should be greater than second ${delays[1]}`,
		);
	});
});
...
suite("WordBuffer", () => {
	...
	test("setRate with undefined rate and isComplete defaults to MIN_RATE_AFTER_COMPLETE", () => {
		...
		assert.ok(
			result !== undefined,
			"Should reveal content with default complete rate",
		);
	});
	...
	test("setRate during streaming clamps between MIN_RATE and MAX_RATE", () => {
		...
		assert.ok(
			result !== undefined,
			"Should reveal content even with low rate (clamped to MIN_RATE)",
		);
	});
	...
	test("setRate with undefined rate during streaming defaults to DEFAULT_RATE", () => {
		...
		assert.ok(
			result !== undefined,
			"Should reveal content with default streaming rate",
		);
	});
	...
	test("needsNextFrame is true when words remain unrevealed", () => {
		...
		assert.strictEqual(
			buffer.needsNextFrame,
			true,
			"Should need another frame when words remain",
		);
	});
	...
	test("needsNextFrame is false when all words are revealed", () => {
		...
		assert.strictEqual(
			buffer.needsNextFrame,
			false,
			"Should not need another frame when all words shown",
		);
	});
});
