...
suite("ChatTodoListWidget Accessibility", () => {
	...
	test("creates proper semantic list structure", () => {
		...
		const todoListContainer = widget.domNode.querySelector(
			".todo-list-container",
		);
		assert.ok(todoListContainer, "Should have todo list container");
		assert.strictEqual(
			todoListContainer?.getAttribute("aria-labelledby"),
			"todo-list-title",
		);
		...
		assert.ok(
			titleElement,
			"Should have title element with ID todo-list-title",
		);
		...
		assert.ok(
			...
			"Should have todo items in the list container",
		);
	});
	...
	test("expand button has proper accessibility attributes", () => {
		...
		assert.strictEqual(
			expandoButton?.getAttribute("aria-controls"),
			"todo-list-container",
		);
		...
		assert.ok(
			titleText?.includes("(2/3)"),
			`Title should show progress format, but got: "${titleText}"`,
		);
		assert.ok(
			titleText?.includes("Second task"),
			`Title should show current task when collapsed, but got: "${titleText}"`,
		);
	});
	...
	test("todo items have complete aria-label with status information", () => {
		...
		assert.ok(
			firstAriaLabel?.includes("First task"),
			"First item aria-label should include title",
		);
		assert.ok(
			firstAriaLabel?.includes("not started"),
			"First item aria-label should include status",
		);
		...
		assert.ok(
			secondAriaLabel?.includes("Second task"),
			"Second item aria-label should include title",
		);
		assert.ok(
			secondAriaLabel?.includes("in progress"),
			"Second item aria-label should include status",
		);
		...
		assert.ok(
			thirdAriaLabel?.includes("Third task"),
			"Third item aria-label should include title",
		);
		assert.ok(
			thirdAriaLabel?.includes("completed"),
			"Third item aria-label should include status",
		);
	});
	...
	test("widget displays properly when no todos exist", () => {
		...
		const emptyWidget = store.add(
			instantiationService.createInstance(ChatTodoListWidget),
		);
		...
		assert.strictEqual(
			emptyWidget.domNode.style.display,
			"none",
			"Widget should be hidden when no todos",
		);
	});
	...
	test("clear button has proper accessibility", () => {
		...
		const clearButton = widget.domNode.querySelector(
			".todo-clear-button-container .monaco-button",
		);
	...
	test("focus expands and places focus on the todo list", () => {
		...
		const expandoButton = widget.domNode.querySelector(
			".todo-list-expand .monaco-button",
		);
		assert.strictEqual(
			expandoButton?.getAttribute("aria-expanded"),
			"false",
			"Todo list should start collapsed",
		);
		...
		assert.strictEqual(
			focused,
			true,
			"Focus should succeed when todos are present",
		);
		assert.strictEqual(
			expandoButton?.getAttribute("aria-expanded"),
			"true",
			"Focus should expand the todo list",
		);
		...
		assert.ok(
			isAncestorOfActiveElement(todoListContainer),
			"Todo list container should contain the active element after focusing",
		);
	});
	...
	test("hasTodos reports visibility state", () => {
		...
		assert.strictEqual(
			widget.hasTodos(),
			true,
			"Widget should report todos are present",
		);
		...
		emptyWidget.render(testSessionUri);
		assert.strictEqual(
			emptyWidget.hasTodos(),
			false,
			"Widget should report no todos when the list is empty",
		);
