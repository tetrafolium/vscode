...
suite("ChatAttachmentsContentPart", () => {
	...
	suite("updateVariables", () => {
		test("should update variables and re-render", () => {
			...
			const part = store.add(
				instantiationService.createInstance(ChatAttachmentsContentPart, {
			...
			const initialAttachments = part.domNode!.querySelectorAll(
				".chat-attached-context-attachment",
			);
			assert.strictEqual(
				initialAttachments.length,
				2,
				"Should have 2 initial attachments",
			);
			...
			const updatedAttachments = part.domNode!.querySelectorAll(
				".chat-attached-context-attachment",
			);
			assert.strictEqual(
				updatedAttachments.length,
				3,
				"Should have 3 attachments after update",
			);
		});
		...
		test("should handle updating from file to image", () => {
			...
			assert.strictEqual(
				part.domNode!.querySelectorAll(".chat-attached-context-attachment")
					.length,
				1,
			);
			...
			assert.strictEqual(
				updatedAttachments.length,
				1,
				"Should have 1 attachment after update",
			);
		});
		...
		test("should preserve contextMenuHandler after update", () => {
			...
			assert.strictEqual(
				part.contextMenuHandler,
				handler,
				"contextMenuHandler should be preserved after update",
			);
		});
		...
		test("should handle empty variables array", () => {
			...
			assert.strictEqual(
				updatedAttachments.length,
				0,
				"Should have 0 attachments after clearing",
			);
		});
	...
	suite("basic rendering", () => {
		test("should render file attachments", () => {
			...
			const attachments = part.domNode!.querySelectorAll(
				".chat-attached-context-attachment",
			);
			assert.strictEqual(
				attachments.length,
				2,
				"Should render 2 file attachments",
			);
		});
		...
		test("should have chat-attached-context class on domNode", () => {
			...
			assert.ok(
				part.domNode!.classList.contains("chat-attached-context"),
				"Should have chat-attached-context class",
			);
		});
	});
});
