...
} from "../../../../../browser/widget/input/editor/chatInputCompletionUtils.js";
import {
	chatAgentLeader,
	chatVariableLeader,
...
suite("computeCompletionRanges", () => {
	...
	suite("with VariableNameDef regex", () => {
		test("matches #variable at start of line", () => {
			const model = store.add(
				createTextModel("#file", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 6),
				variableNameDef(),
			);
		...
		test("matches # alone (just the leader)", () => {
			const model = store.add(
				createTextModel("#", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 2),
		...
		test("cursor in middle of variable produces partial insert range", () => {
			const model = store.add(
				createTextModel("@selection", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 5),
	...
	suite("with toolVariableNameDef regex", () => {
		test("matches #tool at start of line", () => {
			const model = store.add(
				createTextModel("#tool", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 6),
		...
		test("matches #tool after space", () => {
			const model = store.add(
				createTextModel("use #fetch", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 11),
	...
	suite("edge cases", () => {
		test("returns undefined inside a normal word", () => {
			const model = store.add(
				createTextModel("hello", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 3),
		...
		test("returns undefined when no space before cursor mid-line", () => {
			const model = store.add(
				createTextModel("ab", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 3),
		...
		test("returns empty range at blank position after space", () => {
			...
			assert.deepStrictEqual(
				result.insert,
				Range.fromPositions(new Position(1, 7)),
			);
		});
		...
		test("onlyOnWordStart=true rejects variable preceded by a word", () => {
			const model = store.add(
				createTextModel("abc#file", null, undefined, URI.parse("test:input")),
			);
			const result = computeCompletionRanges(
				model,
				new Position(1, 9),
...
suite("isAtTriggerCharacterToken", () => {
	...
	function check(text: string, column: number, expected: boolean): void {
		const model = store.add(
			createTextModel(text, null, undefined, URI.parse("test:input")),
		);
		assert.strictEqual(
	...
	test("returns false when no trigger characters are configured", () => {
		...
		assert.strictEqual(
			isAtTriggerCharacterToken(model, new Position(1, 4), []),
			false,
		);
	});
});
