...
suite("ChatMarkdownRenderer", () => {
	...
	test("plain text fast path preserves rendered markdown shape", () => {
		const md = new MarkdownString("Hello, world. This is plain.", {
			isTrusted: true,
			supportHtml: true,
		...
		assert.deepStrictEqual(
			...
			{
				outerHTML:
					'<div class="rendered-markdown"><p>Hello, world. This is plain.</p></div>',
...
<pre><canvas>canvas here</canvas></pre><details></details>`);
	...
	test("code block ending at end of content does not leak body tag", async () => {
		...
		assert.ok(
			...
			`Rendered text should not contain </body>, got: ${textContent}`,
		);
	});
	...
	test("fillInIncompleteTokens closes bare codespan when supportHtml is set", () => {
		...
		const md = new MarkdownString("Created isolated worktree for branch `xyz", {
			supportHtml: true,
		});
		const result = store.add(
			testRenderer.render(md, { fillInIncompleteTokens: true }),
		);

		const codeEl = result.element.querySelector("code");
		assert.ok(
			codeEl,
			`Expected a <code> element in: ${result.element.outerHTML}`,
		);
		assert.strictEqual(codeEl!.textContent, "xyz");
		assert.ok(
			!result.element.textContent?.includes("`"),
			`Rendered text should not contain a bare backtick, got: ${result.element.textContent}`,
		);
	});
});
