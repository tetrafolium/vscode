...
import {
	CodeBlockPart,
	ICodeBlockData,
} from "../../../../browser/widget/chatContentParts/codeBlockPart.js";
import {
	IChatOutputRendererService,
	type RenderedOutputPart,
} from "../../../../browser/chatOutputItemRenderer.js";
import {
	IChatOutputPartStateCache,
	IOutputPartState,
} from "../../../../browser/widget/chatContentParts/chatOutputPartStateCache.js";
...
suite("ChatMarkdownContentPart", () => {
	...
	function createMarkdownPartWithInlineReferences(
		markdownText: string,
		...
		assert.deepStrictEqual(
			{
				...
				outputBlockCount: part.domNode.querySelectorAll(
					".chat-output-code-block",
				).length,
			},
			{
	...
	test("does not render initial incomplete code fence", () => {
		...
		assert.strictEqual(
			part.domNode.querySelector(".interactive-result-code-block"),
			null,
		);
	});
	...
	test("hasSameContent returns false for different markdown", () => {
		...
		assert.ok(
			!part.hasSameContent({
				kind: "markdownContent",
				content: new MarkdownString("Goodbye"),
			}),
		);
	});
	...
	test("hasSameContent compares inline reference metadata", () => {
		...
		const part = createMarkdownPartWithInlineReferences(content, {
			0: initialReference,
		});
	...
	test("tryIncrementalUpdate requires unchanged inline reference metadata", () => {
		...
		const part = createMarkdownPartWithInlineReferences(
			content,
			{ 0: initialReference },
			context,
			true,
	...
	test("php code blocks get php opening tag prepended", () => {
		...
		assert.strictEqual(renderedCodeBlocks.length, 1);
		assert.ok(
			renderedCodeBlocks[0].text.startsWith("<?php\n"),
	...
	test("php code blocks with existing opening tag are not modified", () => {
		...
		assert.ok(
			!renderedCodeBlocks[0].text.startsWith("<?php\n<?php"),
			"PHP code with existing tag should not be doubled",
		);
	});
	...
	test("strips codeblock uri annotations before rendering standard code blocks", () => {
		...
		assert.strictEqual(
			renderedCodeBlocks[0].codemapperUri?.toString(),
			"file:///test.ts",
		);
	});
	...
	test("code block maintains content when markdown is re-rendered during streaming", () => {
		...
		const ctx = createRenderContext(
			false /* isComplete = false, simulating streaming */,
		);
