...
suite("ChatInlineAnchorWidget Metadata Validation", () => {
	...
	test("renders widget for link with vscodeLinkType query parameter", () => {
		const element = createTestElement(
			"mySkill",
			"file:///test.txt?vscodeLinkType=skill",
		);
		renderFileWidgets(
			element,
		...
		assert.ok(
			widget,
			"Widget should be rendered for link with vscodeLinkType query parameter",
		);
	});
	...
	test("renders widget for empty vscode-agent-host link in chat query title", () => {
		const container = mainWindow.document.createElement("div");
		const titlePart = disposables.add(
			instantiationService.createInstance(
				ChatQueryTitlePart,
				container,
				new MarkdownString(
					"Read [](vscode-agent-host://my-host/file/-/path/to/foo.ts), lines 1 to 2",
				),
				undefined,
			),
		);
		titlePart.setOptions({
		...
		assert.ok(
			widget,
			"Widget should be rendered for empty vscode-agent-host link in chat query title",
		);
		assert.strictEqual(
			widget.querySelector(".icon-label")?.textContent,
			"foo.ts",
		);
	});
	...
	test("renders widget for vscodeLinkType=file", () => {
		const element = createTestElement(
			"document.txt",
			"file:///path/to/document.txt?vscodeLinkType=file",
		);
		renderFileWidgets(
			element,
			instantiationService,
	...
	test("does not render widget for link without vscodeLinkType query parameter", () => {
		...
		assert.ok(
			!widget,
			"Widget should not be rendered for link without vscodeLinkType query parameter",
		);
	});
	...
	test("does not render widget when URI scheme is missing", () => {
		...
		assert.ok(
			!widget,
			"Widget should not be rendered when URI scheme is missing",
		);
	});
	...
	test("renders widget with various vscodeLinkType values", () => {
		const element = createTestElement(
			"customName",
			"file:///test.txt?vscodeLinkType=custom",
		);
		renderFileWidgets(
			element,
			instantiationService,
	...
	test("handles vscodeLinkType with other query parameters", () => {
		const element = createTestElement(
			"skillName",
			"file:///test.txt?other=value&vscodeLinkType=skill&another=param",
		);
		renderFileWidgets(
			element,
			instantiationService,
		...
		assert.ok(
			widget,
			"Widget should be rendered when vscodeLinkType is among multiple query parameters",
		);
	});
	...
	test("handles multiple links in same element", () => {
		...
		validAnchor.setAttribute(
			"data-href",
			"file:///valid.txt?vscodeLinkType=skill",
		);
		...
		assert.strictEqual(
			widgets.length,
			2,
			"Should render widgets for link with vscodeLinkType and empty link text only",
		);
	});
	...
	test("uses link text as fileName in metadata", () => {
		...
		assert.ok(
			labelElement?.textContent?.includes("myCustomFileName"),
			"Label should contain the link text as fileName",
		);
	});
