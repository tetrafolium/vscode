...
} from "../../../../browser/widget/chatContentParts/chatPlanReviewPart.js";
...
function createMockReview(
	overrides?: Partial<IChatPlanReview>,
): IChatPlanReview {
	return {
		kind: "planReview",
		title: "Review Plan",
...
function createMockReviewWithPlan(
	overrides?: Partial<IChatPlanReview>,
): IChatPlanReview {
...
function getFooterButtons(widget: ChatPlanReviewPart): HTMLElement[] {
	const container = widget.domNode.querySelector(
		".chat-plan-review-footer .chat-buttons",
	);
	return container
		? Array.from(container.querySelectorAll(".monaco-button"))
		: [];
}
...
function getInlineButtons(widget: ChatPlanReviewPart): HTMLElement[] {
	const container = widget.domNode.querySelector(
		".chat-plan-review-inline-actions",
	);
	return container
		? Array.from(container.querySelectorAll(".monaco-button"))
		: [];
...
function getReviewButton(widget: ChatPlanReviewPart): HTMLElement | null {
	return widget.domNode.querySelector(
...
suite("ChatPlanReviewPart", () => {
	...
	function createWidget(
		...
		instantiationService.stub(
			IUserInteractionService,
			new MockUserInteractionService(),
		);
	...
	suite("Basic rendering", () => {
		test("renders container with proper structure", () => {
			...
			assert.ok(
				widget.domNode.classList.contains("chat-plan-review-container"),
			);
		...
		test("displays the review title", () => {
			...
			const label = widget.domNode.querySelector(
				".chat-plan-review-title-label",
			);
		...
		test("renders approve and reject buttons in footer", () => {
			...
			assert.ok(
				buttons.length >= 2,
				"should have at least approve and reject buttons",
			);
			assert.ok(
				buttons.some((b) => b.textContent?.includes("Autopilot")),
				"should have approve button",
			);
			assert.ok(
				buttons.some((b) => b.textContent?.includes("Reject")),
		...
		test("does not render Review button when planUri is absent", () => {
			...
			assert.strictEqual(
				getReviewButton(widget),
				null,
				"Review button should not exist without planUri",
			);
		});
		...
		test("does not render Provide Feedback footer button (legacy entry removed)", () => {
			...
			const buttons = getFooterButtons(widget);
			assert.ok(
				!buttons.some((b) => b.textContent?.includes("Provide Feedback")),
				"should not have legacy Provide Feedback button",
			);
		});
	});
	...
	suite("Submit results", () => {
		test("clicking approve submits action with label and rejected=false", () => {
			createWidget(
				createMockReview({ actions: [{ label: "Go", default: true }] }),
			);

			const approveButton = getFooterButtons(widget).find((b) =>
		...
		test("clicking reject submits rejected=true", () => {
			...
			const rejectButton = getFooterButtons(widget).find((b) =>
		...
		test("buttons are removed after submission", () => {
			...
			assert.strictEqual(
				getFooterButtons(widget).length,
				0,
				"footer buttons should be cleared",
			);
		});
	});
	...
	suite("Feedback mode", () => {
		test("clicking Review button opens feedback section and shows Submit Feedback button", async () => {
			...
			const buttons = getFooterButtons(widget);
			assert.ok(
				buttons.some((b) => b.textContent?.includes("Submit Feedback")),
				"should have Submit Feedback button",
			);
			assert.ok(
				buttons.some((b) => b.textContent?.includes("Reject")),
				"should still have Reject button",
			);
			assert.ok(
				!buttons.some((b) => b.textContent?.includes("Autopilot")),
		...
		test("reject button remains visible in feedback mode", async () => {
			...
			assert.ok(
				buttons.some((b) => b.textContent?.includes("Reject")),
				"reject button should still be visible",
			);
		});
		...
		test("clicking Review while in feedback mode exits feedback mode", async () => {
			...
			assert.ok(
				reviewButton.textContent?.includes("Cancel"),
				'review button should read "Cancel" in feedback mode',
			);
			...
			assert.strictEqual(
				feedbackSection.style.display,
				"none",
				"feedback section should be hidden after Cancel",
			);
			assert.ok(
				!reviewButton.textContent?.includes("Cancel"),
				"review button label should revert after exiting feedback mode",
			);

			const buttons = getFooterButtons(widget);
			assert.ok(
				buttons.some((b) => b.textContent?.includes("Autopilot")),
				"approve button should be back",
			);
			assert.ok(
				!buttons.some((b) => b.textContent?.includes("Submit Feedback")),
		...
		test("approving with textarea content sends approval + feedback", () => {
			...
			const textarea = widget.domNode.querySelector(
				".chat-plan-review-feedback-textarea",
			...
			assert.ok(
				approveButton,
				"Approve button should be available even with canProvideFeedback",
			);
			approveButton!.click();
		...
		test("submit is disabled when feedback textarea is empty and no inline comments", async () => {
			...
			const submitButton = getFooterButtons(widget).find((b) =>
				b.textContent?.includes("Submit Feedback"),
			);
			assert.ok(submitButton);
			assert.ok(
				submitButton!.classList.contains("disabled"),
				"Submit Feedback should be disabled when nothing to submit",
			);
		});
	});
	...
	suite("Inline comments list", () => {
		test("renders comments list and updates Submit Feedback count when service has items", async () => {
			...
			const rows = widget.domNode.querySelectorAll(
				".chat-plan-review-comment-row",
			);
			assert.strictEqual(
				rows.length,
				2,
				"should render one row per inline comment",
			);
			...
			assert.ok(submitButton);
			assert.ok(
				(submitButton!.textContent ?? "").includes("(2)"),
				"Submit label should reflect inline count",
			);
		});
		...
		test("inline comments alone are enough to enable Submit Feedback", async () => {
			...
			assert.ok(
				!submitButton!.classList.contains("disabled"),
				"Submit Feedback should be enabled with one inline comment",
			);
		});
		...
		test("inline comments auto-promote into review mode even before Review button is clicked", () => {
			...
			assert.notStrictEqual(
				getFeedbackSection(widget).style.display,
				"none",
				"section should auto-open when comments arrive",
			);
		});
		...
		test("per-row remove button removes only that comment from the service", async () => {
			...
			const removeButtons = widget.domNode.querySelectorAll(
				".chat-plan-review-comment-remove",
			) as NodeListOf<HTMLElement>;
			assert.strictEqual(
				removeButtons.length,
				3,
				"should render one remove button per row",
			);
			...
			assert.deepStrictEqual(
				...
				["Fix this", "Add detail"],
				"middle comment should be removed",
			);
		});
		...
		test("Clear All button is hidden when there are no inline comments", async () => {
			...
			const clearAll = widget.domNode.querySelector(
			...
			assert.strictEqual(
				...
				"none",
				"Clear All should be hidden when list is empty",
			);
		});
		...
		test("Clear All button removes all inline comments after confirmation", async () => {
			...
			assert.notStrictEqual(
				clearAll.style.display,
				"none",
				"Clear All should be visible when list has items",
			);
			clearAll.click();
			await tick();

			assert.strictEqual(
				service.getFeedback(planUri).length,
				0,
				"all comments should be cleared",
			);
		});
		...
		test("Clear All cancellation keeps inline comments intact", async () => {
			...
			assert.strictEqual(
				service.getFeedback(planUri).length,
				2,
				"comments should be untouched when user cancels",
			);
		});
	});
	...
	suite("Collapsed / expanded state", () => {
		test("toggles collapsed state via chevron button", () => {
			...
			const collapseButton = widget.domNode.querySelector(
				".chat-plan-review-title-icon-button:last-child",
			) as HTMLElement;
			...
			assert.ok(
				widget.domNode.classList.contains("chat-plan-review-collapsed"),
			);
		...
		test("collapsed view shows inline actions and hides footer", () => {
			...
			assert.ok(
				...
				"should have inline action buttons when collapsed",
			);
			...
			assert.strictEqual(
				...
				0,
				"footer buttons should be empty when collapsed",
			);
		});
		...
		test("collapsed view does not show reject button", () => {
			...
			const inlineButtons = getInlineButtons(widget);
			assert.ok(
				!inlineButtons.some((b) => b.textContent?.includes("Reject")),
				"reject should be omitted in collapsed view",
			);
		});
		...
		test("toggles expanded state via expand/restore button", () => {
			...
			const expandButton = widget.domNode.querySelector(
				".chat-plan-review-title-icon-button:first-child",
			) as HTMLElement;
			...
			expandButton.click();
			assert.ok(
				!widget.domNode.classList.contains("chat-plan-review-expanded"),
			);
		});
		...
		test("collapsing preserves feedback mode and inline buttons keep Submit Feedback", async () => {
			...
			const inlineButtons = getInlineButtons(widget);
			assert.ok(
				inlineButtons.some((b) => b.textContent?.includes("Submit Feedback")),
				"inline action should be Submit Feedback when feedback mode is active",
			);
			...
			const footerButtons = getFooterButtons(widget);
			assert.ok(
				footerButtons.some((b) => b.textContent?.includes("Submit Feedback")),
				"submit feedback button should remain after expand",
			);
			assert.ok(
				!footerButtons.some((b) => b.textContent?.includes("Autopilot")),
				"approve should still be hidden in feedback mode",
			);
		});

		test("restores draft collapsed state from ChatPlanReviewData", () => {
			const data = new ChatPlanReviewData(
				"Title",
				"Content",
	...
	suite("Multiple actions", () => {
		test("renders dropdown when multiple actions exist", () => {
			...
			assert.ok(
				dropdown,
				"should render a button-with-dropdown for multiple actions",
			);
		});
		...
		test("renders plain button when single action exists", () => {
			...
			assert.strictEqual(
				dropdown,
				null,
				"should not render dropdown for a single action",
			);
		});
		...
		test("emits actionId for a non-default dropdown action when chosen", () => {
			...
			const approveButton = getFooterButtons(widget).find(
				(b) =>
					b.textContent?.includes("Approve") &&
		...
		test("emits actionId when bypass action is the default", () => {
			...
			const bypassButton = getFooterButtons(widget).find((b) =>
				b.textContent?.includes("Bypass"),
			);
	...
	suite("Autopilot confirmation dialog", () => {
		test("shows confirmation dialog for autopilot permission level and proceeds on confirm", async () => {
			...
			createWidget(
				createMockReview({
					actions: [
						{ label: "Autopilot", default: true, permissionLevel: "autopilot" },
					],
				}),
			);
		...
		test("cancels autopilot when dialog is dismissed", async () => {
			...
			assert.strictEqual(
				lastSubmitResult,
				undefined,
				"should not submit when dialog is cancelled",
			);
			assert.ok(
				!widget.domNode.classList.contains("chat-plan-review-used"),
		...
		test("no confirmation dialog for actions without permissionLevel", () => {
			...
			const approveButton = getFooterButtons(widget).find((b) =>
				b.textContent?.includes("Interactive"),
			);
			approveButton!.click();
	...
	suite("Used / submitted state", () => {
		...
		test("disables feedback textarea after submission", () => {
			...
			assert.strictEqual(
				textarea.disabled,
				true,
				"textarea should be disabled after submission",
			);
		});
	});
