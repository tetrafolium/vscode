...
suite("AICustomizationItemsModel", () => {
	...
	suite("basics", () => {
		...
		let listPromptFilesResult: Awaited<
			ReturnType<IPromptsService["listPromptFiles"]>
		>;
		let disabledPromptFilesResult: ResourceSet;

		function createDescriptor(
			id: string,
			provider: ICustomizationItemProvider | undefined,
			...
			return {
				...
				getStorageSourceFilter: (): IStorageSourceFilter => ({
		...
		setup(() => {
			...
			activeSessionResource = observableValue(
				"activeSessionResource",
				URI.parse(`A:///session`),
			);
			activeHarness = derived((reader) =>
				getChatSessionType(activeSessionResource.read(reader)),
			);
			availableHarnesses = observableValue<readonly IHarnessDescriptor[]>(
				"availableHarnesses",
			...
			function customAgentFromPromptPath(
			...
			instaService.stub(IPromptsService, {
				...
				listPromptFiles: async (type: PromptsType) =>
					listPromptFilesResult.filter((f) => f.type === type),
				getCustomAgents: async () =>
					listPromptFilesResult
						.filter((f) => f.type === PromptsType.agent)
						.map(customAgentFromPromptPath),
			...
			instaService.stub(ICustomizationHarnessService, {
				...
				getActiveDescriptor: () =>
					availableHarnesses.get().find((d) => d.id === activeHarness.get())!,
				findHarnessById: (id: string) =>
					availableHarnesses.get().find((d) => d.id === id),
		...
		test("exposes per-section observables for all prompts-based sections", () => {
			const model = disposables.add(
				instaService.createInstance(AICustomizationItemsModel),
			);
		...
		test("preserves provider-supplied plugin storage when pluginUri is omitted", async () => {
			providerA_items = [
				{
					uri: URI.parse(
						"agent-host://test-authority/plugins/my-plugin/skills/my-skill/SKILL.md",
					),
		...
		test("preserves provider-supplied builtin storage when groupKey is omitted", async () => {
			providerA_items = [
				{
					uri: URI.parse(
						"agent-host://test-authority/builtin/skills/github/SKILL.md",
					),
					type: PromptsType.skill,
					name: "Built-in Skill",
		...
		test("prompt service items preserve storage grouping, metadata, and disabled state without sync provider", async () => {
			availableHarnesses.set(
				[createDescriptor("A", undefined), descriptorB],
				undefined,
			);
		...
		test("plugin count includes provider-supplied plugin items", async () => {
			providerA_items = [
				...
				{
					uri: URI.parse(
						"agent-host://test-authority/plugins/remote-two/skills/my-skill/SKILL.md",
					),
					type: PromptsType.skill,
					name: "My Skill",
					source: AICustomizationSources.plugin,
		...
		test("plugin count dedupes provider plugins that are also installed locally", async () => {
			...
			assert.strictEqual(
				count.get(),
				1,
				"before local install: only the harness-reported plugin counts",
			);
			...
			assert.strictEqual(
				count.get(),
				1,
				"after local install: harness duplicate is folded into the local count",
			);
		});
		...
		test("does not double-count local syncable items when itemProvider and syncProvider are both present", async () => {
			...
			availableHarnesses.set(
				[createDescriptor("A", providerWithSync, syncProvider), descriptorB],
				undefined,
			);
		...
		test("syncProvider.onDidChange does not refetch when itemProvider is present", async () => {
			...
			assert.strictEqual(
				providerA_callCount,
				before,
				"syncProvider events must not trigger refetches when itemProvider owns the data path",
			);
		});
	});
	...
	suite("data sources", () => {
		...
		setup(() => {
			...
			const provider: ICustomizationItemProvider = {
				...
				provideChatSessionCustomizations: (
					sessionResource: URI,
					token: CancellationToken,
				) => Promise.resolve(providerItems.slice()),
			};
			...
			const availableHarnesses = observableValue<readonly IHarnessDescriptor[]>(
				"availableHarnesses",
				[descriptor],
			);
			...
			const activeSessionResource = observableValue(
				"activeSessionResource",
				sessionResource,
			);
		...
		function harnessPluginRow(
			name: string,
			overrides: Partial<ICustomizationItem> = {},
		): ICustomizationItem {
			return {
		...
		function providerSkill(
			name: string,
			uri: string = `agent-host://t/skills/${name}/SKILL.md`,
		): ICustomizationItem {
			return {
				uri: URI.parse(uri),
				type: PromptsType.skill,
		...
		for (const [section, type] of sectionsByType) {
			test(`getCount(${section}) mirrors provider items filtered by type=${type}`, async () => {
				...
				assert.strictEqual(
					count.get(),
					expected,
					`${section} count should equal provider items where type === ${type}`,
				);
			});
		}
		...
		test("getCount reacts to provider onDidChange for observed sections", async () => {
			...
			assert.strictEqual(
				count.get(),
				1,
				"initial fetch reflects provider state",
			);
			...
			assert.strictEqual(
				count.get(),
				2,
				"count refetches after provider change",
			);
		});
		...
		test("getPluginCount returns local plugin count when harness has no plugin rows", async () => {
			...
			assert.strictEqual(
				count.get(),
				2,
				"plugin count uses local plugins when the harness exposes none",
			);
		});
		...
		test("getPluginCount returns harness plugin row count when no local plugins are installed", async () => {
			...
			assert.strictEqual(
				count.get(),
				2,
				'remote-client harness rows are excluded; both internal "plugin" and API "plugins" types are recognised',
			);
		});

		test("getPluginCount sums local plugins and unique harness plugin rows", async () => {
			providerItems = [harnessPluginRow("dup"), harnessPluginRow("uniq")];
			...
			assert.strictEqual(
				count.get(),
				3,
				"dup is counted once via the local source; uniq adds, local-only adds",
			);
		});
		...
		test("getPluginCount dedups against URI basename when local plugin label is empty", async () => {
			...
			assert.strictEqual(
				count.get(),
				1,
				"remote row is folded into the labelless local plugin via basename",
			);
		});
	});
	...
	suite("agent host item source caches all types", () => {
		...
		setup(() => {
			...
			instaService.stub(ICustomizationHarnessService, {
				...
				activeHarness,
				availableHarnesses,
				setActiveSession: (next: URI) =>
					activeSessionResource.set(next, undefined),
				getStorageSourceFilter: () => ({ sources: [] }),
				getActiveDescriptor: () =>
					availableHarnesses.get().find((d) => d.id === activeHarness.get())!,
				findHarnessById: (id: string) =>
		...
		test("observing one section does not hide items of other sections", async () => {
			providerItems = [
				{
					uri: URI.parse("agent-host://t/agents/coder.agent.md"),
					type: PromptsType.agent,
					name: "coder",
					source: AICustomizationSources.plugin,
					extensionId: undefined,
				...
				{
					uri: URI.parse("agent-host://t/rules/style.instructions.md"),
				...
				{
					uri: URI.parse("agent-host://t/skills/repo/SKILL.md"),
					type: PromptsType.skill,
					name: "repo",
					source: AICustomizationSources.plugin,
					extensionId: undefined,
			...
			const agentItems = model.getItems(
				AICustomizationManagementSection.Agents,
			);
			await model.whenSectionLoaded(AICustomizationManagementSection.Agents);
			const instructionItems = model.getItems(
				AICustomizationManagementSection.Instructions,
			);
			await model.whenSectionLoaded(
				AICustomizationManagementSection.Instructions,
			);
