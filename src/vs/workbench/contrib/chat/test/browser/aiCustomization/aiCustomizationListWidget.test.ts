...
import {
	...
	truncateToFirstLine,
} from "../../../browser/aiCustomization/aiCustomizationListWidgetUtils.js";
import {
...
suite("aiCustomizationListWidget", () => {
	...
	suite("extractExtensionIdFromPath", () => {
		test("extracts extension ID from copilot-chat extension path", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/.vscode-insiders/extensions/github.copilot-chat-0.43.2026040602/assets/prompts/skills/agent-customization/SKILL.md",
				),
				"github.copilot-chat",
			);
		});
		...
		test("extracts extension ID from PR extension path", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/.vscode-insiders/extensions/github.vscode-pull-request-github-0.135.2026040604/src/lm/skills/SKILL.md",
				),
		...
		test("extracts extension ID from Code OSS dev path", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/.vscode-oss-dev/extensions/github.copilot-chat-0.43.2026040602/assets/prompts/skills/troubleshoot/SKILL.md",
				),
				"github.copilot-chat",
			);
		});
		...
		test("extracts extension ID from Windows-style path", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"C:/Users/dev/.vscode/extensions/ms-python.python-2024.1.1/skills/SKILL.md",
				),
				"ms-python.python",
			);
		});
		...
		test("returns undefined for workspace paths", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/git/vscode/.github/skills/accessibility/SKILL.md",
				),
				undefined,
			);
		});
		...
		test("returns undefined for user home paths", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/.copilot/skills/ios-project-setup/SKILL.md",
				),
				undefined,
			);
		});
		...
		test("returns undefined for plugin paths", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/.vscode-insiders/agent-plugins/github.com/microsoft/vscode-team-kit/model-council/skills/council-review/SKILL.md",
				),
				undefined,
			);
		});
		...
		test("returns undefined for bare extensions folder without version", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/workspace/extensions/my-extension/SKILL.md",
				),
				undefined,
			);
		});
		...
		test("extracts extension ID from User/globalStorage path (Copilot Chat ask agent)", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/.vscode-oss-dev/User/globalStorage/github.copilot-chat/ask-agent/Ask.agent.md",
				),
				"github.copilot-chat",
			);
		});
		...
		test("extracts extension ID from User/globalStorage path on Insiders", () => {
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/Library/Application Support/Code - Insiders/User/globalStorage/github.copilot-chat/ask-agent/Ask.agent.md",
				),
				"github.copilot-chat",
			);
		});
		...
		test("returns undefined for non-extension entries in globalStorage", () => {
			...
			assert.strictEqual(
				extractExtensionIdFromPath(
					"/Users/josh/.vscode-oss-dev/User/globalStorage/state.vscdb",
				),
				undefined,
			);
		});
	...
	suite("disposed widget", () => {
		...
		setup(() => {
			...
			instaService.stub(ICustomizationHarnessService, {
				...
				findHarnessById: (id) =>
					id === descriptor.id ? descriptor : undefined,
		...
		test("generateDebugReport returns empty string when widget is disposed", async () => {
			const widget = disposables.add(
				instaService.createInstance(AICustomizationListWidget),
			);
			widget.dispose();
