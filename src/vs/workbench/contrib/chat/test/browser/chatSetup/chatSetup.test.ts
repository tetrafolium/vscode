...
function parseRedirectUrl(url: string): {
	...
	const returnTo = decodeURIComponent(
		url.slice(questionIdx + "return_to=".length),
	);
...
suite("buildUpgradeUrlWithRedirect", () => {
	...
	test("stable quality uses vscode.dev host", () => {
		...
		assert.strictEqual(
			vscodeUri,
			"vscode://GitHub.copilot-chat/upgrade-success",
		);
	});
	...
	test("insider quality uses insiders.vscode.dev host", () => {
		...
		assert.strictEqual(
			vscodeUri,
			"vscode-insiders://GitHub.copilot-chat/upgrade-success",
		);
	});
	...
	test("undefined quality defaults to insiders.vscode.dev host", () => {
		...
		assert.strictEqual(
			vscodeUri,
			"code-oss://GitHub.copilot-chat/upgrade-success",
		);
	});
	...
	test("appends with & when base URL already has query params", () => {
		...
		assert.ok(
			result.startsWith(
				"https://github.com/github-copilot/upgrade?utm_source=vscode&return_to=",
			),
		);
	});
	...
	test("appends with ? when base URL has no query params", () => {
		...
		assert.ok(
			result.startsWith("https://github.com/github-copilot/upgrade?return_to="),
		);
	});
	...
	test("GHE URL is handled correctly", () => {
		...
		assert.ok(
			result.startsWith(
				"https://github.example.com/github-copilot/upgrade?utm_source=vscode&return_to=",
			),
		);
