...
suite("ChatDebugFilterState", () => {
	...
	suite("parseTimeToken", () => {
		suite("before: prefix", () => {
			...
			test("year-month (February, non-leap) — rounds to end of Feb", () => {
				...
				assert.strictEqual(
					result,
					new Date(2025, 2, 0, 23, 59, 59, 999).getTime(),
				);
			});
			...
			test("date with hour only — rounds to end of hour", () => {
				...
				assert.strictEqual(
					result,
					new Date(2026, 2, 3, 14, 59, 59, 999).getTime(),
				);
			});
			...
			test("date with hour:minute — rounds to end of minute", () => {
				...
				assert.strictEqual(
					result,
					new Date(2026, 2, 3, 14, 30, 59, 999).getTime(),
				);
			});
			...
			test("full date-time with seconds", () => {
				...
				assert.strictEqual(
					result,
					new Date(2026, 2, 3, 14, 30, 45, 999).getTime(),
				);
			});
		});
		...
		suite("after: prefix", () => {
			...
			test("date with hour:minute — start of minute", () => {
				...
				assert.strictEqual(
					result,
					new Date(2026, 2, 3, 14, 30, 0, 0).getTime(),
				);
			});
			...
			test("full date-time with seconds", () => {
				...
				assert.strictEqual(
					result,
					new Date(2026, 2, 3, 14, 30, 45, 0).getTime(),
				);
			});
		});
		...
		suite("embedded in text", () => {
			test("extracts token from surrounding text", () => {
				const result = parseTimeToken(
					"some text before:2026-03-03 more text",
					"before",
				);
				assert.strictEqual(
					result,
	...
	suite("isTimestampVisible", () => {
		...
		test("hidden when before afterTimestamp", () => {
			...
			assert.strictEqual(
				state.isTimestampVisible(new Date(2026, 4, 31)),
				false,
			);
		});
