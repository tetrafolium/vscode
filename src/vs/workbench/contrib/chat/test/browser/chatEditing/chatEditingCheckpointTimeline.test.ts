...
import {
	...
	IChatEditingTimelineFsDelegate,
} from "../../../browser/chatEditing/chatEditingCheckpointTimelineImpl.js";
import {
	FileOperation,
	FileOperationType,
} from "../../../browser/chatEditing/chatEditingOperations.js";
...
suite("ChatEditingCheckpointTimeline", function () {
	...
	function createTextEditOperation(
		uri: URI,
		requestId: string,
		epoch: number,
	...
	function createFileCreateOperation(
		uri: URI,
		requestId: string,
		epoch: number,
	...
	function createFileDeleteOperation(
		uri: URI,
		requestId: string,
		epoch: number,
		finalContent: string,
	...
	function createFileRenameOperation(
		oldUri: URI,
		newUri: URI,
		requestId: string,
		epoch: number,
	...
	setup(function () {
		...
		const insta = store.add(
			workbenchInstantiationService(undefined, store).createChild(collection),
		);

		timeline = insta.createInstance(
			ChatEditingCheckpointTimelineImpl,
			URI.parse("chat://test-session"),
			fileDelegate,
		);
	});
	...
	test("incrementEpoch increases epoch counter", function () {
		...
		assert.strictEqual(
			timeline.getStateForPersistence().epochCounter,
			initialEpoch + 2,
		);
	});
	...
	test("basic undo/redo with text edits", async function () {
		...
		timeline.recordFileOperation(
			createTextEditOperation(uri, "req1", editEpoch, [
	...
	test("file creation and deletion operations", async function () {
		...
		timeline.recordFileOperation(
			createFileCreateOperation(uri, "req1", createEpoch, "new file content"),
		);
		...
		await timeline.navigateToCheckpoint(
			timeline.getStateForPersistence().checkpoints[0].checkpointId,
		);
		assert.strictEqual(fileContents.has(uri), false);

		// Navigate to created checkpoint
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", "created")!,
		);
		...
		timeline.recordFileOperation(
			createFileDeleteOperation(uri, "req1", deleteEpoch, "new file content"),
		);
		...
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", "deleted")!,
		);
		assert.strictEqual(fileContents.has(uri), false);
	...
	test("file rename operations", async function () {
		...
		timeline.recordFileOperation(
			createFileCreateOperation(oldUri, "req1", createEpoch, "content"),
		);
		...
		timeline.recordFileOperation(
			createFileRenameOperation(oldUri, newUri, "req1", renameEpoch),
		);
		...
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", "renamed")!,
		);
	...
	test("multiple sequential edits to same file", async function () {
		...
		// First edit
		timeline.recordFileOperation(
			createTextEditOperation(uri, "req1", timeline.incrementEpoch(), [
		...
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", "edit1")!,
		);
		assert.strictEqual(fileContents.get(uri), "LINE1\nline2\nline3");

		// Navigate to second edit
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", "edit2")!,
		);
		assert.strictEqual(fileContents.get(uri), "LINE1\nLINE2\nline3");

		// Navigate back to start
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", undefined)!,
		);
		assert.strictEqual(fileContents.get(uri), "line1\nline2\nline3");
	});
	...
	test("getCheckpointIdForRequest returns undefined for non-existent checkpoint", function () {
		const checkpoint = timeline.getCheckpointIdForRequest(
			"nonexistent",
			"stop1",
		);
	...
	test("requestDisablement tracks disabled requests", async function () {
		...
		timeline.recordFileOperation(
			createFileCreateOperation(uri, "req1", timeline.incrementEpoch(), "a"),
		);

		timeline.createCheckpoint("req1", "stop1", "Stop req1");
		timeline.recordFileOperation(
			createTextEditOperation(uri, "req1", timeline.incrementEpoch(), [
				{ range: new Range(1, 1, 1, 2), text: "b" },
			]),
		);

		timeline.createCheckpoint("req2", undefined, "Start req2");
		timeline.recordFileOperation(
			createTextEditOperation(uri, "req2", timeline.incrementEpoch(), [
				{ range: new Range(1, 1, 1, 2), text: "c" },
			]),
		);
	...
	test("persistence - save and restore state", function () {
		...
		assert.strictEqual(
			restoredState.checkpoints.length,
			savedState.checkpoints.length,
		);
		assert.strictEqual(
			restoredState.operations.length,
			savedState.operations.length,
		);
	...
	test("navigating between multiple requests", async function () {
		...
		timeline.recordFileOperation(
			createFileCreateOperation(uri1, "req1", create1Epoch, "file1 modified"),
		);
		...
		timeline.recordFileOperation(
			createFileCreateOperation(uri2, "req2", create2Epoch, "file2 modified"),
		);
		...
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", "stop1")!,
		);
		...
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req2", "stop1")!,
		);
		assert.strictEqual(fileContents.get(uri1), "file1 modified");
	...
	test("operations use incrementing epochs", function () {
		...
		const epoch1 = timeline.incrementEpoch();
		timeline.recordFileOperation(
			createTextEditOperation(uri, "req1", epoch1, [
		...
		const epoch2 = timeline.incrementEpoch();
		timeline.recordFileOperation(
			createTextEditOperation(uri, "req1", epoch2, [
	...
	test("navigateToCheckpoint does nothing when already at target epoch", async function () {
		...
		const createEpoch = timeline.incrementEpoch();
		timeline.recordFileOperation(
			createTextEditOperation(uri, "req1", createEpoch, [
				{ range: new Range(1, 1, 1, 8), text: "modified" },
			]),
		);
	...
	test("recording operation after undo truncates future history", async function () {
		...
		assert.strictEqual(
			stateAfterNewEdit.operations[1].type,
	...
	test("redo when there is no checkpoint after operation", async function () {
		...
		// Undo to start
		const startCheckpoint = timeline.getCheckpointIdForRequest(
			"req1",
			undefined,
		)!;
	...
	test("getContentAtStop with epoch-based stopId", async function () {
		...
		const content = await timeline.getContentAtStop(
			"req1",
			uri,
			`__epoch_${editEpoch + 1}`,
		);
	...
	test("multiple text edits to same file are properly replayed", async function () {
		...
		await timeline.navigateToCheckpoint(initialCheckpoint.checkpointId);
		await timeline.navigateToCheckpoint(
			timeline.getCheckpointIdForRequest("req1", "all-edits")!,
		);
	...
	test("checkpoint with same requestId and undoStopId is not duplicated", function () {
		...
		const checkpoints = timeline.getStateForPersistence().checkpoints;
		const req1Stop1Checkpoints = checkpoints.filter(
			(c) => c.requestId === "req1" && c.undoStopId === "stop1",
		);
	...
	test("finding baseline after file rename operation", async function () {
		...
		timeline.recordFileOperation(
			createTextEditOperation(oldUri, "req1", timeline.incrementEpoch(), [
	...
	test("orphaned operations and checkpoints are removed after undo and new changes", async function () {
		...
		timeline.recordFileOperation(
			createFileCreateOperation(uri, "req1", createEpoch, "initial content"),
		);
		...
		assert.strictEqual(
			...
			3,
			"Should still have 3 operations (create + first + replacement)",
		);
		assert.strictEqual(
			state.checkpoints.length,
			4,
			"Should have 4 checkpoints (initial, start, stop1, stop2-new)",
		);
		...
		const stop2NewCheckpoint = timeline.getCheckpointIdForRequest(
			"req1",
			"stop2-new",
		);
		const stop2OldCheckpoint = timeline.getCheckpointIdForRequest(
			"req1",
			"stop2",
		);
		...
		assert.strictEqual(
			stop2OldCheckpoint,
			undefined,
			"Old orphaned checkpoint should be removed",
		);
		...
		const stop1Checkpoint = timeline.getCheckpointIdForRequest(
			"req1",
			"stop1",
		)!;
		const stop2NewCheckpointId = timeline.getCheckpointIdForRequest(
			"req1",
			"stop2-new",
		)!;
		...
		assert.strictEqual(
			...
			"replacement edit",
			"Orphaned edit should never reappear",
		);
		...
		assert.strictEqual(
			fileContents.get(uri),
			"replacement edit",
			"Content should still be correct after full timeline traversal",
		);
	});
	...
	test("undo/redo with multiple no-edit requests advances one request at a time", async function () {
		...
		assert.deepStrictEqual(
			timeline.requestDisablement.get().map((d) => d.requestId),
			["req4"],
		);
