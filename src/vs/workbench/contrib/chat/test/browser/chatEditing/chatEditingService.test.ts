...
suite("ChatEditingService", function () {
	...
	setup(function () {
		...
		collection.set(
			IChatTransferService,
			new SyncDescriptor(ChatTransferService),
		);
		collection.set(
			IChatSessionsService,
			new SyncDescriptor(ChatSessionsService),
		);
		...
		collection.set(
			...
			new SyncDescriptor(NullLanguageModelsService),
		);
		...
		collection.set(
			...
			new (class extends mock<IMultiDiffSourceResolverService>() {
				override registerResolver(
					_resolver: IMultiDiffSourceResolver,
				): IDisposable {
		...
		collection.set(
			...
			new (class extends mock<IWorkspaceEditingService>() {
		...
		const insta = store.add(
			store
				.add(workbenchInstantiationService(undefined, store))
	...
	test("create session", async function () {
		...
		const modelRef = chatService.startNewLocalSession(
		...
		assert.strictEqual(
			session.chatSessionResource.toString(),
	...
	test("create session, file entry & isCurrentlyBeingModifiedBy", async function () {
		...
		const modelRef = store.add(
			chatService.startNewLocalSession(ChatAgentLocation.Chat),
		);
		const model = modelRef.object as ChatModel;
		...
		const entry = await waitForState(
			session.entries.map((value) =>
				value.find((a) => isEqual(a.modifiedURI, uri)),
			),
		);
		...
		await waitForState(
			entry.isCurrentlyBeingModifiedBy.map(
				(value) => value === chatRequest.response,
			),
		);
		assert.ok(
			entry.isCurrentlyBeingModifiedBy.get()?.responseModel ===
	...
	async function idleAfterEdit(
		session: IChatEditingSession,
		model: ChatModel,
		uri: URI,
		edits: TextEdit[],
	) {
		const isStreaming = waitForState(
			session.state.map((s) => s === ChatEditingSessionState.StreamingEdits),
			Boolean,
		);
		...
		const isIdle = waitForState(
			session.state.map((s) => s === ChatEditingSessionState.Idle),
			Boolean,
		);
		await isIdle;
	...
	test("mirror typing outside -> accept", async function () {
		return runWithFakedTimers({}, async () => {
			...
			const entry = await idleAfterEdit(session, model, uri, [
				{ range: new Range(1, 1, 1, 1), text: "FarBoo\n" },
			]);
			const original = store.add(
				await textModelService.createModelReference(entry.originalURI),
			).object.textEditorModel;
			const modified = store.add(
				await textModelService.createModelReference(entry.modifiedURI),
			).object.textEditorModel;
			...
			modified.pushEditOperations(
				null,
				[EditOperation.insert(new Position(3, 1), "USER_TYPE\n")],
				() => null,
			);
	...
	test("NO mirror typing inside -> accept", async function () {
		return runWithFakedTimers({}, async () => {
			...
			modified.pushEditOperations(
				null,
				[EditOperation.replace(new Range(1, 2, 1, 7), "ooBar")],
				() => null,
			);
	...
	test("ChatEditingService merges text edits it shouldn't merge, #272679", async function () {
		return runWithFakedTimers({}, async () => {
			...
			const modified = store.add(
				await textModelService.createModelReference(uri),
			).object.textEditorModel;
