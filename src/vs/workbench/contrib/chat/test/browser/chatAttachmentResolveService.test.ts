...
suite("ChatAttachmentResolveService", () => {
	 ...
	 */
	let directoryTree: Map<
		string,
		{ resource: URI; isFile: boolean; isDirectory: boolean }[]
	>;
	...
	setup(() => {
		...
		service.resolveImageEditorAttachContext = async (
			resource: URI,
...
suite("ChatAttachmentResolveService - resolveBrowserViewAttachContext", () => {
	...
	setup(() => {
		...
		instantiationService.stub(IFileService, {
			resolve: async (resource: URI) =>
				createFileStat(resource, false, true, false),
		});
		instantiationService.stub(IEditorService, {});
		instantiationService.stub(ITextModelService, {});
		instantiationService.stub(IExtensionService, {});
		...
		instantiationService.stub(IBrowserViewWorkbenchService, {
			getKnownBrowserViews: () =>
				browserViews as Map<string, BrowserEditorInput>,
		});
	...
	function makeMockEditor(
		id: string,
		opts: { sharingState: BrowserViewSharingState; setSharedResult?: boolean },
	): Partial<BrowserEditorInput> {
	...
	test("returns entry when already shared", async () => {
		const editor = makeMockEditor("b1", {
	...
	test("prompts for sharing when NotShared and user accepts", async () => {
		const editor = makeMockEditor("b2", {
	...
	test("returns undefined when NotShared and user denies", async () => {
		const editor = makeMockEditor("b3", {
			sharingState: BrowserViewSharingState.NotShared,
	...
	test("resolves model if not yet resolved", async () => {
		...
		const editor: Partial<BrowserEditorInput> = {
			...
			resolve: async () => {
				resolved = true;
				(editor as Partial<BrowserEditorInput>).model =
					model as IBrowserViewModel;
