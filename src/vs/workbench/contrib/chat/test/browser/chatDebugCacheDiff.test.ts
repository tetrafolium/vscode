...
import {
	...
	parseInputMessages,
} from "../../browser/chatDebug/chatDebugCacheDiff.js";

function msg(role: string, content: string, name?: string) {
	const part: { type: string; content: string; name?: string } = {
		type: "text",
		content,
	};
	if (name) {
...
suite("chatDebugCacheDiff", () => {
	...
	suite("parseInputMessages", () => {
			...
			assert.deepStrictEqual(parseInputMessages(json), [
				{
					role: "tool",
					name: undefined,
					text: "Found 12 references.",
					charLength: "Found 12 references.".length,
				},
			]);
		});
		...
		test("extracts tool_search_output content and labels role distinctly", () => {
			...
			const json = JSON.stringify([
				{
					role: "tool_search",
					parts: [{ type: "tool_search_output", ...payload }],
				},
			]);
	...
	suite("diffPromptSignature", () => {
		test("all identical messages produce no break and only identical tokens", () => {
			const a = parseInputMessages(
				JSON.stringify([msg("system", "sys"), msg("user", "q1")]),
			);
			const b = parseInputMessages(
				JSON.stringify([msg("system", "sys"), msg("user", "q1")]),
			);
		...
		test("content drift at index 1 reports a contentDrift break", () => {
			const a = parseInputMessages(
				JSON.stringify([msg("system", "sys"), msg("user", "aaaa")]),
			);
			const b = parseInputMessages(
				JSON.stringify([msg("system", "sys"), msg("user", "bbbb")]),
			);
			const result = diffPromptSignature(a, b);
			assert.deepStrictEqual(
				{
					...
					counts: result.counts,
					kinds: result.signature.map((s) => s.kind),
					drift: result.drift.map(
						(d) => `${d.name}:${d.status}:${d.aSize}->${d.bSize}`,
					),
				},
				{
		...
		test("length change at index 1 reports a lengthChange break", () => {
			const a = parseInputMessages(
				JSON.stringify([msg("system", "sys"), msg("user", "short")]),
			);
			const b = parseInputMessages(
				JSON.stringify([msg("system", "sys"), msg("user", "much longer text")]),
			);
			const result = diffPromptSignature(a, b);
			assert.deepStrictEqual(
				{
		...
		test("B has trailing messages A does not — break at first onlyInB", () => {
			...
			assert.deepStrictEqual(
				...
				},
				{
					...
					kinds: [
						CacheDiffKind.Identical,
						CacheDiffKind.Identical,
						CacheDiffKind.OnlyInB,
						CacheDiffKind.OnlyInB,
					],
				},
			);
		...
		test("appendSystemDrift inserts a system row when system instructions differ", () => {
			...
			assert.deepStrictEqual(drift, [
				{
					name: "system",
					status: CacheDiffKind.LengthChange,
					aSize: 10,
					bSize: 12,
				},
			]);
			...
			const existing = [
				{
					name: "messages[0]",
					role: "user",
					status: CacheDiffKind.ContentDrift,
					aSize: 4,
					bSize: 4,
				},
			];
			assert.deepStrictEqual(
				appendSystemDrift(existing, "sys", "sys"),
				existing,
			);
		});
		...
		test("appendToolsDrift returns input unchanged when tools match", () => {
			...
			assert.deepStrictEqual(
				appendToolsDrift(existing, "[tools]", "[tools]"),
				existing,
			);
			assert.deepStrictEqual(
				appendToolsDrift(existing, undefined, undefined),
				existing,
			);
		});
