...
suite.skip("ChatSessionsService", () => {
	...
	suite("extractFileNameFromLink", () => {
		function callExtractFileNameFromLink(filePath: string): string {
			...
			type ServiceWithPrivateMethod = Record<
				"extractFileNameFromLink",
				(filePath: string) => string
			>;
			return (chatSessionsService as unknown as ServiceWithPrivateMethod)[
				"extractFileNameFromLink"
			](filePath);
		}
		...
		test("should handle multiple file links in same string", () => {
			const input =
				"See [main](file:///main.js) and [utils](file:///utils/helper.ts)";
			const result = callExtractFileNameFromLink(input);
		...
		test("should handle mixed content with file links and regular text", () => {
			const input =
				"Check [config](file:///config.yml) and visit https://example.com";
			const result = callExtractFileNameFromLink(input);
...
suite("ChatSessionOptionsMap", () => {
	...
	suite("toStrValueArray", () => {
		test("should return undefined for undefined input", () => {
			assert.strictEqual(
				ChatSessionOptionsMap.toStrValueArray(undefined),
				undefined,
			);
		});
		...
		test("should handle a plain object as if it were a record (defensive fallback)", () => {
			...
			const plainObject = {
				models: "gpt-4",
				repo: "my-repo",
			} as unknown as ReadonlyChatSessionOptionsMap;
			assert.deepStrictEqual(
				ChatSessionOptionsMap.toStrValueArray(plainObject),
				[
					{ optionId: "models", value: "gpt-4" },
					{ optionId: "repo", value: "my-repo" },
				],
	...
	suite("fromRecord", () => {
		test("should convert a record to a Map", () => {
			const map = ChatSessionOptionsMap.fromRecord({
				models: "gpt-4",
				repo: "my-repo",
			});
