...
suite("BrowserPluginGitCommandService", () => {
	...
	const targetDir = URI.from({
		scheme: Schemas.inMemory,
		path: "/cache/github.com/octocat/hello",
	});
	...
	suite("parseGitHubCloneUrl", () => {
		test("parses canonical github HTTPS clone URL", () => {
			assert.deepStrictEqual(
				parseGitHubCloneUrl("https://github.com/octocat/Hello-World.git"),
		...
		test("strips trailing slash and missing .git suffix", () => {
			assert.deepStrictEqual(
				parseGitHubCloneUrl("https://github.com/octocat/Hello-World/"),
				{ owner: "octocat", repo: "Hello-World" },
			);
			assert.deepStrictEqual(
				parseGitHubCloneUrl("https://github.com/octocat/Hello-World"),
		...
		test("rejects URLs with extra path segments", () => {
			assert.strictEqual(
				parseGitHubCloneUrl("https://github.com/octocat/Hello-World/issues/42"),
				undefined,
			);
			assert.strictEqual(
				parseGitHubCloneUrl("https://github.com/octocat/Hello-World/tree/main"),
				undefined,
			);
		});
		...
		test("rejects non-HTTPS, non-GitHub, and malformed URLs", () => {
			assert.strictEqual(
				parseGitHubCloneUrl("git@github.com:octocat/repo.git"),
				undefined,
			);
			assert.strictEqual(
				parseGitHubCloneUrl("https://gitlab.com/octocat/repo.git"),
				undefined,
			);
			assert.strictEqual(
				parseGitHubCloneUrl("https://github.com/octocat"),
				undefined,
			);
	...
	suite("cloneRepository", () => {
		...
		test("downloads tree + blobs and persists SHA metadata", async () => {
			...
			const readme = await fileService.readFile(
				URI.joinPath(targetDir, "README.md"),
			);
			assert.strictEqual(readme.value.toString(), "hello\n");
			const index = await fileService.readFile(
				URI.joinPath(targetDir, "src/index.js"),
			);
		...
		test("surfaces a GitHubRateLimitError on 403 with X-RateLimit-Remaining: 0", async () => {
			requestStub.queue(
				"GET",
				/\/commits\/main$/,
				plainResponse(403, VSBuffer.fromString("rate limit"), {
			...
			assert.ok(
				captured instanceof Error && captured.name === "GitHubRateLimitError",
				`expected GitHubRateLimitError, got ${(captured as Error)?.name}`,
			);
		});
		...
		test("requests GitHub auth and retries when GitHub returns 403", async () => {
			...
			const file = await fileService.readFile(
				URI.joinPath(targetDir, "private.txt"),
			);
		...
		test("uses an existing signed-in GitHub session before falling back to anonymous requests", async () => {
			service = new BrowserPluginGitCommandService(
				...
				storage,
				stubAuthenticationService({
					sessions: [createAuthenticationSession("signed-in-token")],
				}),
			);
			...
			assert.strictEqual(
				requestStub.requests[0].headers?.Authorization,
				"Bearer signed-in-token",
			);
			assert.strictEqual(
				requestStub.requests[1].headers?.Authorization,
				"Bearer signed-in-token",
			);
			assert.strictEqual(
				requestStub.requests[2].headers?.Authorization,
				"Bearer signed-in-token",
			);
		});
		...
		test("falls back to anonymous when the signed-in GitHub session is rejected", async () => {
			...
			service = new BrowserPluginGitCommandService(
				...
				storage,
				stubAuthenticationService({
					sessions: [createAuthenticationSession("sso-blocked-token")],
					state,
				}),
			);
			...
			await service.cloneRepository(
				"https://github.com/octocat/Public.git",
				targetDir,
				"main",
			);

			const file = await fileService.readFile(
				URI.joinPath(targetDir, "public.txt"),
			);
			assert.strictEqual(file.value.toString(), "public");
			assert.strictEqual(
				requestStub.requests[0].headers?.Authorization,
				"Bearer sso-blocked-token",
			);
			assert.strictEqual(
				requestStub.requests[1].headers?.Authorization,
				undefined,
		...
		test("failed extraction leaves the previous targetDir intact", async () => {
			...
			requestStub.queue(
				"GET",
				/\/commits\/main$/,
				jsonResponse(200, { sha: "sha2" }),
			);
			requestStub.queue(
				"GET",
				/\/git\/trees\/sha2/,
				plainResponse(500, VSBuffer.fromString("boom")),
			);
			...
			const keep = await fileService.readFile(
				URI.joinPath(targetDir, "keep.txt"),
			);
		...
		test("skips symlink and submodule entries", async () => {
			...
			requestStub.queue(
				...
				/\/git\/blobs\/b-readme$/,
				jsonResponse(200, {
					content: encodeBase64(new TextEncoder().encode("hi\n")),
					encoding: "base64",
				}),
			);
			...
			assert.strictEqual(
				(
					await fileService.readFile(URI.joinPath(targetDir, "README.md"))
				).value.toString(),
				"hi\n",
			);
			assert.strictEqual(
				await fileService.exists(URI.joinPath(targetDir, "link.txt")),
				false,
			);
			assert.strictEqual(
				await fileService.exists(URI.joinPath(targetDir, "subrepo")),
				false,
			);
		});
	});
	...
	suite("pull", () => {
		...
		test("clears stale files from a prior extraction", async () => {
			...
			assert.strictEqual(
				await fileService.exists(URI.joinPath(targetDir, "removed.txt")),
				false,
			);
			const keep = await fileService.readFile(
				URI.joinPath(targetDir, "keep.txt"),
		...
		test("rejects path-traversal entries in the tree", async () => {
			...
			requestStub.queue(
				...
				/\/git\/blobs\/b-safe$/,
				jsonResponse(200, {
					content: encodeBase64(new TextEncoder().encode("safe")),
					encoding: "base64",
				}),
			);
			...
			const safe = await fileService.readFile(
				URI.joinPath(targetDir, "safe.txt"),
			);
			assert.strictEqual(safe.value.toString(), "safe");
			const escapedSibling = URI.from({
				scheme: targetDir.scheme,
				path: "/cache/github.com/octocat/escaped.txt",
			});
		...
		test("rejects backslash-traversal entries (Windows path separator)", async () => {
			...
			assert.strictEqual(
				await fileService.exists(
					URI.joinPath(targetDir, "..\\..\\escaped.txt"),
				),
				false,
			);
		});
	...
	suite("checkout", () => {
		test("no-ops when the requested SHA matches the cached SHA", async () => {
			requestStub.queue(
				"GET",
				/\/commits\/main$/,
				jsonResponse(200, { sha: "aabbccddeeff00112233445566778899aabbccdd" }),
			);
			queueRepoFetch(requestStub, "aabbccddeeff00112233445566778899aabbccdd", {
				"a.txt": "a",
			});
			await service.cloneRepository(
				"https://github.com/octocat/Hello-World.git",
		...
		test("re-extracts when the SHA differs", async () => {
			requestStub.queue(
				"GET",
				/\/commits\/main$/,
				jsonResponse(200, { sha: "1111111111111111111111111111111111111111" }),
			);
			queueRepoFetch(requestStub, "1111111111111111111111111111111111111111", {
				"a.txt": "old",
			});
			await service.cloneRepository(
				"https://github.com/octocat/Hello-World.git",
			...
			queueRepoFetch(requestStub, "2222222222222222222222222222222222222222", {
				"a.txt": "new",
			});
			...
			assert.strictEqual(a.value.toString(), "new");
			assert.strictEqual(
				await service.revParse(targetDir, "HEAD"),
				"2222222222222222222222222222222222222222",
			);
		});
		...
		test("throws when called for a target with no cached metadata", async () => {
			await assert.rejects(
				() => service.checkout(targetDir, "abc"),
				/no cached metadata/,
			);
		});
	});
	...
	test("fetch / fetchRepository / revListCount are inert", async () => {
		...
		assert.strictEqual(
			await service.revListCount(targetDir, "HEAD", "@{u}"),
			0,
		);
	});
});
...
class StubRequestService implements Partial<IRequestService> {
	...
	async request(
		...
		const idx = this._queue.findIndex(
			(q) => q.methodMatcher === method && q.urlMatcher.test(url),
		);
		if (idx === -1) {
...
function plainResponse(
	statusCode: number,
	body: VSBuffer = VSBuffer.alloc(0),
 ...
 */
function queueRepoFetch(
	stub: StubRequestService,
	sha: string,
	...
	stub.queue(
		"GET",
		new RegExp(`/git/trees/${escapeForRegExp(sha)}\\?recursive=1$`),
		jsonResponse(200, { sha, tree, truncated: false }),
	);
	entries.forEach(([, content], i) => {
		stub.queue(
			"GET",
			blobShaMatcher(tree[i].sha),
			jsonResponse(200, {
...
function stubAuthenticationService(
	options: IStubAuthenticationServiceOptions = {},
): IAuthenticationService {
	return {
...
function createAuthenticationSession(
	accessToken: string,
