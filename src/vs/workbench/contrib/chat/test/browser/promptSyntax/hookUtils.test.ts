...
} from "../../../browser/promptSyntax/hookUtils.js";
...
function getSelectedText(
	...
	const result: string[] = [];
	result.push(
		lines[selection.startLineNumber - 1].substring(selection.startColumn - 1),
	);
	for (
		let i = selection.startLineNumber;
		i < selection.endLineNumber! - 1;
		i++
	) {
		result.push(lines[i]);
	}
	result.push(
		lines[selection.endLineNumber! - 1].substring(0, selection.endColumn! - 1),
	);
	return result.join("\n");
}
...
suite("hookUtils", () => {
	...
	"hooks": {
		...
		"UserPromptSubmit": [
			...
			test("finds first command in SessionStart", () => {
				const result = findHookCommandSelection(
					simpleFormat,
					"SessionStart",
					0,
					"command",
			...
			test("finds second command in SessionStart", () => {
				...
				assert.ok(result);
				assert.strictEqual(
					getSelectedText(simpleFormat, result),
					"echo second",
				);
				assert.deepStrictEqual(result, {
					startLineNumber: 10,
	...
	"hooks": {
		"UserPromptSubmit": [
			...
			test("finds command inside nested hooks", () => {
				...
				assert.ok(result);
				assert.strictEqual(
					getSelectedText(nestedFormat, result),
					"echo 'foobarbaz5' > ~/foobarbaz.txt",
				);
				assert.deepStrictEqual(result, {
					startLineNumber: 10,
					startColumn: 19,
	...
	"hooks": {
		"PreToolUse": [
			...
			test("finds first command in first nested hooks array", () => {
				...
				assert.ok(result);
				assert.strictEqual(
					getSelectedText(mixedFormat, result),
					"first nested",
				);
				assert.deepStrictEqual(result, {
					startLineNumber: 9,
	...
	"hooks": {
		"SessionStart": [
			...
			test("finds bash field", () => {
				...
				assert.ok(result);
				assert.strictEqual(
					getSelectedText(platformSpecificFormat, result),
	...
	"hooks": {
		"UserPromptSubmit": [
			...
			test("skips empty nested hooks and finds subsequent command", () => {
				...
				assert.ok(result);
				assert.strictEqual(
					getSelectedText(emptyNestedHooks, result),
					"after empty nested",
				);
				assert.deepStrictEqual(result, {
					startLineNumber: 10,
	...
	suite("findHookCommandSelection with buildNewHookEntry", () => {
		...
		test("finds command when appending Claude entry to existing hooks", () => {
			...
			const content = JSON.stringify(
				{ hooks: { PreToolUse: [entry1, entry2] } },
				null,
				"\t",
			);

			const result0 = findHookCommandSelection(
				content,
				"PreToolUse",
				0,
				"command",
	...
	suite("findHookCommandInYaml", () => {
		...
		test("handles command with special characters", () => {
			...
			const result = findHookCommandInYaml(
				content,
				'echo "foo" > /tmp/out.txt',
			);
			assert.ok(result);
			assert.strictEqual(
				getSelectedText(content, result),
				'echo "foo" > /tmp/out.txt',
			);
		});
