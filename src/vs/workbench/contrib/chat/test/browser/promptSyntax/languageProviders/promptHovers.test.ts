...
suite("PromptHoverProvider", () => {
	...
	suite("claude agent hovers", () => {
		...
		async function getClaudeHover(
			content: string,
			line: number,
			column: number,
		): Promise<string | undefined> {
			return getHover(content, line, column, PromptsType.agent, {
				claudeAgent: true,
			});
		}
