...
suite("PromptFileRewriter", () => {
	...
	test("rewriteTools preserves tools key when using value range (array syntax)", () => {
		...
		const enablementMap = toolService.toToolAndToolSetEnablementMap(
			["testTool1", "testTool2"],
			undefined,
		);
		...
		assert.ok(
			...
			`Expected 'tools:' key to be preserved, but got: ${result}`,
		);
		assert.ok(
			result.includes("testTool1"),
			`Expected 'testTool1' in result: ${result}`,
		);
		assert.ok(
			result.includes("testTool2"),
			`Expected 'testTool2' in result: ${result}`,
		);
	});
