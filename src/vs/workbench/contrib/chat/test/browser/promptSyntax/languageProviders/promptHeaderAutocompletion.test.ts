...
suite("PromptHeaderAutocompletion", () => {
	...
	setup(async () => {
		...
		const testModels: ILanguageModelChatMetadata[] = [
			...
			} satisfies ILanguageModelChatMetadata,
			{
				...
				maxInputTokens: 8192,
				maxOutputTokens: 1024,
				capabilities: { agentMode: false, toolCalling: true },
				isDefaultForLocation: { [ChatAgentLocation.Chat]: true },
			} satisfies ILanguageModelChatMetadata,
			{
				id: "bg-agent-model",
				...
				capabilities: { agentMode: true, toolCalling: true },
				isDefaultForLocation: { [ChatAgentLocation.Chat]: true },
				targetChatSessionType: "background",
			} satisfies ILanguageModelChatMetadata,
		];
		...
		completionProvider = instaService.createInstance(
	...
	async function getCompletions(
		...
		const model = disposables.add(
			createTextModel(content, languageId, undefined, uri),
		);
		// get the completion location from  the '|' marker
		const lineColumnMarkerRange = model.findNextMatch(
			"|",
			new Position(1, 1),
			false,
			false,
		...
		assert.ok(
			lineColumnMarkerRange,
			"No completion marker found in test content",
		);
		...
		return result.suggestions.map((s) => {
			...
			return {
				...
				result:
					lineContent.substring(0, s.range.startColumn - 1) +
					s.insertText +
					lineContent.substring(s.range.endColumn - 1),
			};
		});
	}
	...
	suite("agent header completions", () => {
		...
		test("complete model attribute value", async () => {
			const content = ["---", 'description: "Test"', "model: |", "---"].join(
				"\n",
			);
		...
		test("complete model attribute value with partial input", async () => {
			const content = ["---", 'description: "Test"', "model: MA|", "---"].join(
				"\n",
			);
		...
		test("complete model names inside model array", async () => {
			...
			assert.deepStrictEqual(
				...
				[
					{ label: "MAE 4 (olama)", result: `model: ['MAE 4 (olama)']` },
					{
						label: "MAE 4.1 (copilot)",
						result: `model: ['MAE 4.1 (copilot)']`,
					},
				].sort(sortByLabel),
			);
		});
		...
		test("complete model names inside model array with existing entries", async () => {
			...
			assert.deepStrictEqual(
				...
				[
					{
						label: "MAE 4.1 (copilot)",
						result: `model: ['MAE 4 (olama)', 'MAE 4.1 (copilot)']`,
					},
				].sort(sortByLabel),
			);
		});
		...
		test("complete tool names inside tools array", async () => {
			const content = ["---", 'description: "Test"', "tools: [|]", "---"].join(
				"\n",
			);
		...
		test("complete agents inside agents array", async () => {
			...
			assert.deepStrictEqual(
				actual.sort(sortByLabel),
				[{ label: "agent1", result: `agents: [agent1]` }].sort(sortByLabel),
			);
		});

		test("complete infer attribute value", async () => {
			const content = ["---", 'description: "Test"', "infer: |", "---"].join(
				"\n",
			);
		...
		test("exclude models with targetChatSessionType from agent model completions", async () => {
			...
			assert.ok(
				!labels.includes("BG Agent Model (copilot)"),
				"Models with targetChatSessionType should be excluded from agent model completions",
			);
		});
		...
		test("exclude models with targetChatSessionType from agent model array completions", async () => {
			...
			assert.ok(
				!labels.includes("BG Agent Model (copilot)"),
				"Models with targetChatSessionType should be excluded from agent model array completions",
			);
		});

		test("complete hooks value with New Hook snippet", async () => {
			const content = ["---", 'description: "Test"', "hooks: |", "---"].join(
				"\n",
			);
		...
		test("complete hook event names inside hooks map", async () => {
			...
			assert.ok(
				!labels.includes("SessionStart"),
				"SessionStart should not be suggested when already present",
			);
			assert.ok(
				labels.includes("SessionEnd"),
				"SessionEnd should be suggested",
			);
			assert.ok(
				labels.includes("PreToolUse"),
				"PreToolUse should be suggested",
			);
		...
		test("complete hook event names for vscode target excludes existing hooks", async () => {
			...
			assert.ok(
				!labels.includes("PreToolUse"),
				"PreToolUse should not be suggested when already present",
			);
			assert.ok(
				labels.includes("UserPromptSubmit"),
				"UserPromptSubmit should be suggested",
			);
			assert.ok(
				labels.includes("PostToolUse"),
				"PostToolUse should be suggested",
			);
			// SessionEnd is not available for vscode target
			assert.ok(
				!labels.includes("SessionEnd"),
				"SessionEnd should not be available for vscode target",
			);
		});
		...
		test("complete hook event names while editing existing key name", async () => {
			...
			assert.ok(
				labels.includes("SessionStart"),
				"SessionStart should be suggested",
			);
			assert.ok(
				labels.includes("SubagentStart"),
				"SubagentStart should be suggested",
			);
			assert.ok(labels.includes("Stop"), "Stop should be suggested");
		...
		test("hooks: cursor right after colon triggers New Hook snippet", async () => {
			...
			const labels = actual.map((a) => a.label);
			assert.ok(
				labels.includes("New Hook"),
				"New Hook snippet should be suggested",
			);
		});
		...
		test("typing field name after existing field triggers remaining command fields", async () => {
			...
			assert.ok(
				...
				"type should not be suggested when already present",
			);
		});
		...
		test("typing event name after existing hook triggers hook events", async () => {
			...
			assert.ok(
				!labels.includes("SessionEnd"),
				"SessionEnd should not be suggested when already present",
			);
		});
		...
		test("typing event name between existing hooks triggers hook events", async () => {
			...
			assert.ok(
				!labels.includes("UserPromptSubmit"),
				"UserPromptSubmit should not be suggested when already present",
			);
		});
		...
		test("cursor after hook event colon triggers New Command snippet", async () => {
			...
			const labels = actual.map((a) => a.label);
			assert.ok(
				labels.includes("New Command"),
				"New Command snippet should be suggested",
			);
			assert.strictEqual(
				actual.length,
				1,
	...
	suite("claude agent header completions", () => {
		...
		const claudeAgentUri = URI.parse(
			"test:///.claude/agents/security-reviewer.agent.md",
		);
		...
		test("complete attribute names", async () => {
			...
			const actual = await getCompletions(
				content,
				PromptsType.agent,
				claudeAgentUri,
			);
				...
				[
					{
						...
						result: "disallowedTools: ${0:Write, Edit, Bash}",
					},
		...
		test("complete attribute names excludes already present ones", async () => {
			...
			const labels = actual.map((a) => a.label).sort();
			assert.ok(
				!labels.includes("tools"),
				"tools should not be suggested when already present",
			);
			assert.ok(
				!labels.includes("name"),
				"name should not be suggested when already present",
			);
			assert.ok(
				!labels.includes("description"),
				"description should not be suggested when already present",
			);
		});
		...
		test("complete tools inside array syntax", async () => {
			...
			assert.deepStrictEqual(
				actual.find((a) => a.label === "Edit")?.result,
				`tools: [Edit]`,
			);
		});
		...
		test("complete tools inside array with existing entries", async () => {
			...
			assert.deepStrictEqual(
				actual.find((a) => a.label === "Read")?.result,
				`tools: [Edit, Read]`,
			);
			assert.deepStrictEqual(
				actual.find((a) => a.label === "Bash")?.result,
				`tools: [Edit, Bash]`,
			);
		});
		...
		test("complete disallowedTools inside array syntax", async () => {
			...
			assert.deepStrictEqual(
				actual.find((a) => a.label === "Write")?.result,
	...
	suite("prompt header completions", () => {
		...
		test("exclude models with targetChatSessionType from prompt model completions", async () => {
			...
			assert.ok(
				!labels.includes("BG Agent Model (copilot)"),
				"Models with targetChatSessionType should be excluded from prompt model completions",
			);
		});
	});
});
