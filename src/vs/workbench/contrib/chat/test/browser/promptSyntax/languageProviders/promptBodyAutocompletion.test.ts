...
suite("PromptBodyAutocompletion", () => {
	...
	setup(async () => {
		...
		const fileService = disposables.add(
			instaService.createInstance(FileService),
		);
		...
		await fileService.writeFile(
			URI.parse("test:///workspace/src/index.ts"),
			VSBuffer.fromString("export function hello() {}"),
		);
		await fileService.writeFile(
			URI.parse("test:///workspace/README.md"),
			VSBuffer.fromString("# Project"),
		);
		await fileService.writeFile(
			URI.parse("test:///workspace/package.json"),
			VSBuffer.fromString("{}"),
		);
	...
	async function getCompletions(
		...
		const model = disposables.add(
			createTextModel(
				...
				languageId,
				undefined,
				URI.parse("test://workspace/test" + getPromptFileExtension(promptType)),
			),
		);
			...
			].join("\n");
			{
				...
				assert.deepEqual(actual, [
					...
					},
					{
						...
						result:
							"Use #tool:github.vscode-pull-request-github/suggest-fix to reference a tool.",
					},
				]);
			}
		});
