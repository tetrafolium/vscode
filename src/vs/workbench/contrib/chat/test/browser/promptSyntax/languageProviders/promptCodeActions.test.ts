...
suite("PromptCodeActionProvider", () => {
	...
	setup(async () => {
		...
		const deprecatedTool = {
			...
			displayName: "oldTool",
			canBeReferencedInPrompt: true,
			modelDescription: "Deprecated Tool",
			source: ToolDataSource.External,
			inputSchema: {},
		} satisfies IToolData;
		...
		instaService.stub(IPromptsService, {
			...
			getAgentFileURIFromModeFile(uri: URI) {
				...
				if (uri.path.endsWith(".chatmode.md")) {
					return uri.with({
						path: uri.path.replace(".chatmode.md", ".agent.md"),
					});
				}
				return undefined;
			},
	...
	async function getCodeActions(
		...
		{
			title: string;
			textEdits?: IWorkspaceTextEdit[];
			fileEdits?: IWorkspaceFileEdit[];
		}[]
	> {
		const languageId = getLanguageIdForPromptsType(promptType);
		const uri = URI.parse(
			"test:///test" + (fileExtension ?? getPromptFileExtension(promptType)),
		);
		...
		return result.actions.map((action) => ({
			title: action.title,
			textEdits: action.edit?.edits?.filter(
				(edit): edit is IWorkspaceTextEdit => "textEdit" in edit,
			),
			fileEdits: action.edit?.edits?.filter(
				(edit): edit is IWorkspaceFileEdit => "oldResource" in edit,
			),
		}));
	}
	...
	suite("agent code actions", () => {
		...
		test("update deprecated tool names - single replacement", async () => {
			...
			assert.strictEqual(
				actions[0].textEdits![0].textEdit.text,
				`'singleReplacement'`,
			);
		});
	...
	suite("prompt code actions", () => {
		test("rename mode to agent", async () => {
			const content = ["---", 'description: "Test"', "mode: edit", "---"].join(
				"\n",
			);
		...
		test("both mode and tools code actions available", async () => {
			...
			// Test mode action
			const modeActions = await getCodeActions(
				content,
				3,
				1,
				PromptsType.prompt,
			...
			// Test tools action
			const toolActions = await getCodeActions(
				content,
				4,
				10,
				PromptsType.prompt,
