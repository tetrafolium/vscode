...
function isGenericEvent(
	event: IChatDebugEvent,
): event is IChatDebugGenericEvent {
...
suite("PromptsDebugContribution", () => {
	...
	const emptyDiscoveryInfo = (type: PromptsType): IPromptDiscoveryInfo => ({
		type,
		files: [],
		durationInMillis: 0,
	});
	...
	setup(() => {
		...
		willInvokeAgentEmitter = disposables.add(
			new Emitter<IChatAgentInvocationEvent>(),
		);
		instaService.stub(IChatAgentService, {
			onWillInvokeAgent: willInvokeAgentEmitter.event,
		} as Partial<IChatAgentService>);
		instaService.stub(IChatService, {
			onDidDisposeSession: disposables.add(new Emitter()).event,
		} as Partial<IChatService>);
		promptsService = {
	...
	test("should forward discovery events to chat debug service", async () => {
					...
					? [
							{
								...
								promptPath: createLocalPromptPath(
									"/workspace/.github/instructions/test.instructions.md",
		...
		willInvokeAgentEmitter.fire({
			agentId: "test-agent",
			request: {
				sessionResource: LocalChatSessionUri.forSession("session-1"),
			} as IChatAgentInvocationEvent["request"],
		});
		await flushAsyncLogging();

		assert.strictEqual(firedEvents.length, 5);
		const event = firedEvents.find(
			(e): e is IChatDebugGenericEvent =>
				isGenericEvent(e) && e.name === "Instructions Discovery",
		);
		assert.ok(event);
	...
	test("should store discoveryInfo and resolve via resolveEvent", async () => {
		...
		const instructionsEvent = firedEvents.find(
			(e): e is IChatDebugGenericEvent =>
				isGenericEvent(e) && e.name === "Instructions Discovery",
		);
	...
	test("should assign event id when discoveryInfo is empty", async () => {
		...
		assert.strictEqual(firedEvents.length, 5);
		assert.ok(
			firedEvents.every((e) => e.id !== undefined),
			"Events with discovery info should have an id",
		);
	});
	...
	test("should handle discoveryInfo with skipped files", async () => {
			...
			files: [
				{
					status: "loaded" as const,
					promptPath: createLocalPromptPath(
						"/workspace/.github/instructions/loaded.instructions.md",
				...
				},
				{
					...
					promptPath: createLocalPromptPath(
						"/workspace/.github/instructions/skipped.instructions.md",
						"skipped.instructions.md",
					),
					skipReason: "disabled",
				},
			],
		...
		const eventId = firedEvents.find(
			(e): e is IChatDebugGenericEvent =>
				isGenericEvent(e) && e.name === "Instructions Discovery",
		)!.id!;
		const resolved = await chatDebugService.resolveEvent(eventId);
