...
} from "../../../browser/accessibility/chatResponseAccessibleView.js";
...
suite("ChatResponseAccessibleView", () => {
	...
	suite("getToolSpecificDataDescription", () => {
		...
		test("returns command line for terminal data", () => {
			...
			assert.strictEqual(
				getToolSpecificDataDescription(terminalData),
		...
		test("returns raw input for input data (string)", () => {
			...
			assert.strictEqual(
				getToolSpecificDataDescription(inputData),
				"some input string",
			);
		});
		...
		test("returns resources list for resources data with URIs", () => {
			const resourcesData: IChatToolResourcesInvocationData = {
				kind: "resources",
				values: [URI.file("/path/to/file1.ts"), URI.file("/path/to/file2.ts")],
			};
	...
	suite("getProvider", () => {
		test("omits thinking content when disabled in storage", () => {
				...
				response: {
					value: [
						{ kind: "thinking", value: "Hidden reasoning" },
						{
							...
							content: new MarkdownString("Response content"),
						},
					],
				},
			...
			const provider = instantiationService.invokeFunction((accessor) =>
				accessibleView.getProvider(accessor),
			);
			assert.ok(provider);
			store.add(provider);
			...
			const responseItem = {
				response: {
					value: [
						{
							kind: "markdownContent",
							content: new MarkdownString("See file "),
						},
						{
							kind: "inlineReference",
							inlineReference: inlineReferenceUri,
							name: "index.ts",
						},
						{
							kind: "markdownContent",
			...
			const responseItem = {
				response: {
					value: [
						{
							kind: "markdownContent",
							content: new MarkdownString("Error at "),
						},
						{
							kind: "inlineReference",
							inlineReference: fileLocation,
							name: "main.ts",
						},
					],
				},
