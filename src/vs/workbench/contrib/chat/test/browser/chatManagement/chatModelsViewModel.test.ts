...
} from "../../../browser/chatManagement/chatModelsViewModel.js";
...
class MockLanguageModelsService implements ILanguageModelsService {
	...
	private readonly _onDidChangeLanguageModelVendors = new Emitter<
		readonly string[]
	>();
	...
	lookupLanguageModelByQualifiedName(
		...
		for (const [identifier, metadata] of this.models.entries()) {
			if (
				ILanguageModelChatMetadata.matchesQualifiedName(referenceName, metadata)
			) {
	...
	async setModelConfiguration(
		_modelId: string,
		_values: IStringDictionary<unknown>,
	): Promise<void> {}
	...
	async renameLanguageModelsProviderGroup(
		vendorId: string,
		providerGroupName: string,
	): Promise<void> {}

	async updateLanguageModelsProviderGroupApiKey(
		vendorId: string,
		providerGroupName: string,
	): Promise<void> {}

	async addLanguageModelsProviderGroupModel(
		vendorId: string,
		providerGroupName: string,
	): Promise<void> {}

	async openLanguageModelsProviderGroupSettings(
		vendorId: string,
		providerGroupName: string,
	): Promise<void> {}
	...
	isModelPinned(_modelIdentifier: string): boolean {
		return false;
	}
	onDidChangePinnedModels = Event.None;
	isModelHidden(_modelIdentifier: string): boolean {
		return false;
	}
	isGroupHidden(_vendor: string, _groupName: string): boolean {
		return false;
	}
...
suite("ChatModelsViewModel", () => {
	...
	test("should fetch all models without filters", () => {
		...
		const models = results.filter(
			(r) => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r),
		) as ILanguageModelEntry[];
	...
	test("should filter by provider name (vendor ID and display name)", () => {
		...
		assert.strictEqual(
			resultsByCopilotId[0].vendorEntry.vendor.vendor,
			"copilot",
		);
		...
		assert.strictEqual(
			resultsByCopilotId[2].model.identifier,
			"copilot-gpt-4o",
		);
		...
		assert.strictEqual(
			resultsByOpenAIName[0].vendorEntry.vendor.vendor,
			"openai",
		);
		assert.strictEqual(resultsByOpenAIName[1].type, "model");
		assert.strictEqual(
			resultsByOpenAIName[1].model.identifier,
			"openai-gpt-3.5",
		);
		assert.strictEqual(resultsByOpenAIName[2].type, "model");
		assert.strictEqual(
			resultsByOpenAIName[2].model.identifier,
			"openai-gpt-4-vision",
		);
	});
	...
	test("should filter by single capability - tools", () => {
		...
		assert.ok(
			models.every((m) => m.model.metadata.capabilities?.toolCalling === true),
		);
	});
	...
	test("should filter by single capability - vision", () => {
		...
		assert.strictEqual(models.length, 3);
		assert.ok(
			models.every((m) => m.model.metadata.capabilities?.vision === true),
		);
	});
	...
	test("should filter by text matching vendor name", () => {
		...
		assert.strictEqual(models.length, 2);
		assert.ok(
			models.every((m) => m.model.provider.group.name === "GitHub Copilot"),
		);
	});
	...
	test("should toggle vendor collapsed state", () => {
		const vendorEntry = viewModel.viewModelEntries.find(
			(r) =>
				isLanguageModelProviderEntry(r) &&
				r.vendorEntry.vendor.vendor === "copilot",
		) as ILanguageModelProviderEntry;
		viewModel.toggleCollapsed(vendorEntry);

		const results = viewModel.filter("");
		const copilotVendor = results.find(
			(r) =>
				isLanguageModelProviderEntry(r) &&
				(r as ILanguageModelProviderEntry).vendorEntry.vendor.vendor ===
					"copilot",
		) as ILanguageModelProviderEntry;
		...
		const copilotModelsAfterCollapse = results.filter(
			(r) =>
				!isLanguageModelProviderEntry(r) &&
				(r as ILanguageModelEntry).model.provider.vendor.vendor === "copilot",
		);
	...
	test("should remove filter keywords from text search", () => {
		const results = viewModel.filter(
			"@provider:copilot @capability:vision GPT",
		);
		...
		assert.strictEqual(models.length, 2);
		assert.ok(
			models.every((m) => m.model.provider.vendor.vendor === "copilot"),
		);
	});
	...
	test("should support toolcalling alias for tools capability", () => {
		...
		const modelsTools = resultsTools.filter(
			(r) => !isLanguageModelProviderEntry(r),
		);
		const modelsToolCalling = resultsToolCalling.filter(
			(r) => !isLanguageModelProviderEntry(r),
		);
	...
	test("should support agentmode alias for agent capability", () => {
		...
		const modelsAgent = resultsAgent.filter(
			(r) => !isLanguageModelProviderEntry(r),
		);
		const modelsAgentMode = resultsAgentMode.filter(
			(r) => !isLanguageModelProviderEntry(r),
		);
	...
	test("should include matched capabilities in results", () => {
		...
		for (const model of models) {
			...
			assert.ok(
				model.capabilityMatches.some(
					(c) => c === "toolCalling" || c === "vision",
				),
			);
		}
	});

	function createSingleVendorViewModel(includeSecondModel: boolean = true): {
		service: MockLanguageModelsService;
	...
	test("should not show vendor header when only one vendor exists", async () => {
		...
		assert.strictEqual(
			vendors.length,
			0,
			"Should not show vendor header when only one vendor exists",
		);
	...
	test("should show vendor headers when multiple vendors exist", () => {
		...
		assert.strictEqual(
			vendors.length,
			2,
			"Should show vendor headers when multiple vendors exist",
		);
	...
	test("should collapse all groups and models", () => {
		...
		assert.strictEqual(
			models.length,
			0,
			"Should have no models visible after collapse all",
		);
	});
	...
	test("should filter by case-insensitive provider name", () => {
		...
		const models1 = results1.filter(
			(r) => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r),
		) as ILanguageModelEntry[];
		const models2 = results2.filter(
			(r) => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r),
		) as ILanguageModelEntry[];
		const models3 = results3.filter(
			(r) => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r),
		) as ILanguageModelEntry[];
