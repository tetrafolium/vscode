...
suite("RenameTool", () => {
	...
	function createMockBulkEditService(): IBulkEditService & {
	...
	function createTool(
		...
		options?: { bulkEditService?: IBulkEditService },
	): RenameTool {
	...
	suite("getToolData", () => {
		test("returns tool data when no providers are registered", () => {
			const tool = disposables.add(
				createTool(createMockTextModelService(null!)),
			);
		...
		test("description does not include a per-language list", () => {
			...
			const tool = disposables.add(
				createTool(createMockTextModelService(model)),
			);
			disposables.add(
				langFeatures.renameProvider.register("typescript", {
		...
		test("description is identical regardless of which providers are registered", () => {
			...
			disposables.add(
				langFeatures.renameProvider.register("python", {
					provideRenameEdits: () => ({ edits: [] }),
				}),
			);
	...
	suite("invoke", () => {
		...
		test("returns error when rename is rejected", async () => {
			...
			disposables.add(
				langFeatures.renameProvider.register("typescript", provider),
			);
			const tool = disposables.add(
				createTool(createMockTextModelService(model)),
			);
		...
		test("successful rename applies edits via bulk edit and reports result", async () => {
			...
			const tool = disposables.add(
				createTool(createMockTextModelService(model), { bulkEditService }),
			);
		...
		test("successful rename with single edit reports singular message", async () => {
			...
			const edits = [makeEdit(testUri, new Range(1, 10, 1, 17), "MyNewClass")];
			const provider: RenameProvider = {
				provideRenameEdits: (): WorkspaceEdit & Rejection => ({ edits }),
			};
			disposables.add(
		...
		test("resolves filePath via workspace folders", async () => {
			...
			const edits = [makeEdit(fileUri, new Range(1, 10, 1, 17), "MyNewClass")];
			const provider: RenameProvider = {
				provideRenameEdits: (): WorkspaceEdit & Rejection => ({ edits }),
			};
			disposables.add(
