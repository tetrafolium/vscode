...
suite("LanguageModelToolsConfirmationService", () => {
	...
	function createToolRef(
		toolId: string,
		source: ToolDataSource = ToolDataSource.Internal,
		parameters: unknown = {},
	): ILanguageModelToolConfirmationRef {
		return { toolId, source, parameters };
	}

	function createMcpToolRef(
		toolId: string,
		definitionId: string,
		serverLabel: string,
		parameters: unknown = {},
	...
	async function createCombinationRef(
		toolId: string,
		parameters: unknown,
		combinationLabel: string,
		combinationArgs?: string,
	): Promise<ILanguageModelToolConfirmationRef> {
		return {
	...
	test("getPreConfirmActions includes server-level actions for MCP tools", () => {
		...
		assert.ok(
			actions.some(
				(a) => a.label.includes("Test Server") && a.label.includes("Session"),
			),
		);
		assert.ok(
			actions.some(
				(a) => a.label.includes("Test Server") && a.label.includes("Workspace"),
			),
		);
		assert.ok(
			actions.some(
				(a) =>
					a.label.includes("Test Server") && a.label.includes("Always Allow"),
			),
		);
	});
	...
	test("pre-execution session confirmation works", async () => {
		...
		const sessionAction = actions.find(
			(a) => a.label.includes("Session") && !a.label.includes("Server"),
		);
	...
	test("pre-execution workspace confirmation works", async () => {
		...
		const workspaceAction = actions.find(
			(a) => a.label.includes("Workspace") && !a.label.includes("Server"),
		);
	...
	test("pre-execution profile confirmation works", async () => {
		...
		const profileAction = actions.find(
			(a) => a.label.includes("Always Allow") && !a.label.includes("Server"),
		);
	...
	test("tool-level confirmation takes precedence over server-level confirmation", async () => {
		...
		const serverAction = serverActions.find(
			(a) => a.label.includes("Test Server") && a.label.includes("Session"),
		);
		assert.ok(serverAction);
		await serverAction.select();
		...
		const toolAction = toolActions.find(
			(a) => !a.label.includes("Test Server") && a.label.includes("Workspace"),
		);
	...
	test("registerConfirmationContribution allows custom pre-confirm actions", () => {
		...
		store.add(
			service.registerConfirmationContribution("customTool", contribution),
		);
	...
	test("resetToolAutoConfirmation clears all confirmations", async () => {
		...
		const sessionAction1 = actions1.find(
			(a) => a.label.includes("Session") && !a.label.includes("Server"),
		);
		...
		const serverAction = actions2.find(
			(a) => a.label.includes("Test Server") && a.label.includes("Session"),
		);
		assert.ok(serverAction);
		await serverAction.select();
	...
	test("disposing contribution registration removes it", () => {
		...
		const disposable = service.registerConfirmationContribution(
			"customTool",
			contribution,
		);
	...
	test("different tools have independent confirmations", async () => {
		...
		const actions1 = service.getPreConfirmActions(ref1);
		const sessionAction = actions1.find(
			(a) => a.label.includes("Session") && !a.label.includes("Server"),
		);
		assert.ok(sessionAction);
		await sessionAction.select();
		...
		const actions2 = service.getPreConfirmActions(ref2);
		const workspaceAction = actions2.find(
			(a) => a.label.includes("Workspace") && !a.label.includes("Server"),
		);
		assert.ok(workspaceAction);
		await workspaceAction.select();
	...
	test("pre and post execution confirmations are independent", async () => {
		...
		const preSessionAction = preActions.find(
			(a) => a.label.includes("Session") && !a.label.includes("Server"),
		);
		...
		const postWorkspaceAction = postActions.find(
			(a) => a.label.includes("Workspace") && !a.label.includes("Server"),
		);
		...
		assert.deepStrictEqual(preResult, {
			type: ToolConfirmKind.LmServicePerTool,
			scope: "session",
		});
	...
	test("different MCP servers have independent confirmations", async () => {
		...
		const actions1 = service.getPreConfirmActions(ref1);
		const serverAction1 = actions1.find(
			(a) => a.label.includes("Server 1") && a.label.includes("Session"),
		);
		...
		const actions2 = service.getPreConfirmActions(ref2);
		const serverAction2 = actions2.find(
			(a) => a.label.includes("Server 2") && a.label.includes("Workspace"),
		);
	...
	test("combination actions are only offered when combinationLabel is set", async () => {
		const refWithout = createToolRef("testTool", ToolDataSource.Internal, {
			file: "foo.txt",
		});
		const actionsWithout = service.getPreConfirmActions(refWithout);
		assert.ok(!actionsWithout.some((a) => a.label.includes("foo.txt")));

		const refWith = await createCombinationRef(
			"testTool",
			{ file: "foo.txt" },
			'Allow reading "foo.txt"',
		);
		const actionsWith = service.getPreConfirmActions(refWith);
		assert.ok(
			actionsWith.some((a) => a.label.includes('Allow reading "foo.txt"')),
		);
	});

	test("combination actions include session, workspace, and profile scopes", async () => {
		const ref = await createCombinationRef(
			"testTool",
			{ file: "foo.txt" },
			'Allow reading "foo.txt"',
		);
		const actions = service.getPreConfirmActions(ref);
		const combinationActions = actions.filter((a) =>
			a.label.includes('Allow reading "foo.txt"'),
		);
	...
	test("selecting a combination session action auto-confirms the same parameters", async () => {
		...
		const actions = service.getPreConfirmActions(ref);
		const combinationAction = actions.find(
			(a) =>
				a.label.includes('Allow reading "foo.txt"') && a.scope === "session",
		);
	...
	test("selecting a combination workspace action stores at workspace scope", async () => {
		...
		const combinationAction = actions.find(
			(a) =>
				a.label.includes('Allow reading "foo.txt"') && a.scope === "workspace",
		);
		assert.ok(combinationAction);
		await combinationAction.select();

		assert.deepStrictEqual(service.getPreConfirmAction(ref), {
			type: ToolConfirmKind.LmServicePerTool,
			scope: "workspace",
		});
	});
	...
	test("combination approval does not apply to different parameters", async () => {
		const refFoo = await createCombinationRef(
			"testTool",
			{ file: "foo.txt" },
			'Allow reading "foo.txt"',
		);
		const refBar = await createCombinationRef(
			"testTool",
			{ file: "bar.txt" },
			'Allow reading "bar.txt"',
		);
	...
	test("tool-level approval takes precedence over combination approval", async () => {
		...
		const actions = service.getPreConfirmActions(ref);
		const toolSessionAction = actions.find(
			(a) =>
				a.label.includes("Session") &&
				!a.label.includes("foo.txt") &&
				!a.label.includes("Server"),
		);
	...
	test("legacy string[] storage format is read correctly", () => {
		...
		assert.deepStrictEqual(newService.getPreConfirmAction(ref1), {
			type: ToolConfirmKind.LmServicePerTool,
			scope: "workspace",
		});
		assert.deepStrictEqual(newService.getPreConfirmAction(ref2), {
			type: ToolConfirmKind.LmServicePerTool,
			scope: "workspace",
		});
	...
	test("object storage format with arguments round-trips across restart", () => {
		...
		> = {
			"tool1:combination:12345": {
				label: "Allow reading foo.txt",
				arguments: '["foo.txt"]',
			},
		...
		const ref2: ILanguageModelToolConfirmationRef = {
			...createToolRef("tool2"),
			combination: {
				label: "Allow command with args",
				key: "tool2:combination:67890",
			},
		};
