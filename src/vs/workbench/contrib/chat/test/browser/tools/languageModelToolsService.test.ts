...
import {
	...
	CopilotToolId,
} from "../../../common/tools/copilotToolIds.js";
...
class TestTelemetryService implements Partial<ITelemetryService> {
	...
	publicLog2<E extends Record<string, any>, T extends Record<string, any>>(
		eventName: string,
		data?: E,
	): void {
...
function stubGetSession(
	...
	options?: {
		requestId?: string;
		capture?: { invocation?: any };
		modeInfo?: { permissionLevel?: ChatPermissionLevel };
	},
): IChatModel {
	...
	const fakeModel = {
		...
		getRequests: () => [
			{ id: requestId, modelId: "test-model", modeInfo: options?.modeInfo },
		],
	} as ChatModel;
...
async function waitForPublishedInvocation(
	capture: { invocation?: any },
	tries = 10,
 ...
 */
function createTestToolsService(
	store: ReturnType<typeof ensureNoDisposablesAreLeakedInTestSuite>,
	options?: TestToolsServiceOptions,
): TestToolsServiceSetup {
	...
	instaService.stub(
		...
		new MockLanguageModelToolsConfirmationService(),
	);
	...
	if (options?.accessibilitySignalService) {
		instaService.stub(
			IAccessibilitySignalService,
			options.accessibilitySignalService as unknown as IAccessibilitySignalService,
		);
	}
	...
	if (options?.commandService) {
		instaService.stub(
			ICommandService,
			options.commandService as ICommandService,
		);
	}
...
suite("LanguageModelToolsService", () => {
	...
	function setupToolsForTest(service: LanguageModelToolsService, store: any) {
		...
		const extTool1: IToolData = {
			...
			source: {
				type: "extension",
				label: "My Extension",
				extensionId: new ExtensionIdentifier("my.extension"),
			},
			canBeReferencedInPrompt: true,
		};
		...
		const internalToolSet = store.add(
		...
		const userToolSet = store.add(
			service.createToolSet(
				{
					type: "user",
					label: "User",
					file: URI.file("/test/userToolSet.json"),
				},
				"userToolSet",
				"userToolSetRefName",
				{ description: "Test Set" },
		...
		const mcpDataSource: ToolDataSource = {
			type: "mcp",
			label: "My MCP Server",
			serverLabel: "MCP Server",
			instructions: undefined,
			collectionId: "testMCPCollection",
			definitionId: "testMCPDefId",
		};
		...
		const mcpToolSet = store.add(
			service.createToolSet(mcpDataSource, "mcpToolSet", "mcpToolSetRefName", {
	...
	test("invocation parameters are overridden by input toolSpecificData", async () => {
		...
		stubGetSession(chatService, sessionId, {
			requestId: "requestId-io",
			capture,
		});
		const dto = tool.makeDto({ a: 1 }, { sessionId });

		const invokeP = service.invokeTool(
			dto,
			async () => 0,
			CancellationToken.None,
		);
		const published = await waitForPublishedInvocation(capture);
		IChatToolInvocation.confirmWith(published, {
	...
	test("chat invocation injects input toolSpecificData for confirmation when alwaysDisplayInputOutput", async () => {
		...
		assert.deepStrictEqual(
			published.toolSpecificData?.rawInput,
			dto.parameters,
		);
	...
	test("chat invocation waits for user confirmation before invoking", async () => {
		...
		assert.strictEqual(
			invoked,
			false,
			"invoke should not run before confirmation",
		);
		...
		assert.strictEqual(
			invoked,
			true,
			"invoke should have run after confirmation",
		);
	...
	test("selectedCustomButton is passed to tool invoke when user selects a custom button", async () => {
		...
		const tool = registerToolForTest(service, store, "testToolCustomButton", {
			...
			invoke: async (invocation) => {
				...
				return {
					content: [
						{ kind: "text", value: invocation.selectedCustomButton ?? "none" },
					],
				};
			},
		});
		...
		const capture: { invocation?: any } = {};
		stubGetSession(chatService, sessionId, {
			requestId: "requestId-custom-btn",
			capture,
		});
	...
	test("selectedCustomButton is not set when user confirms without custom button", async () => {
		...
		const capture: { invocation?: any } = {};
		stubGetSession(chatService, sessionId, {
			requestId: "requestId-no-custom-btn",
			capture,
		});
	...
	test("confirmationMessages with customOptions disables allowAutoConfirm", async () => {
		...
		const capture: { invocation?: any } = {};
		stubGetSession(chatService, sessionId, {
			requestId: "requestId-custom-noauto",
			capture,
		});
		...
		assert.ok(published, "expected ChatToolInvocation to be published");
		assert.deepStrictEqual(
			published.confirmationMessages?.customOptions?.map((o) => o.label),
			["Yes", "No"],
		);
	...
	test("skipping modified-files confirmation returns the shared skip message and does not invoke the tool", async () => {
		...
		const tool = registerToolForTest(
			service,
			store,
			"testModifiedFilesConfirmationSkip",
			{
				prepareToolInvocation: async () => ({
					confirmationMessages: {
						title: "Confirm",
		...
		const capture: { invocation?: any } = {};
		stubGetSession(chatService, sessionId, {
			requestId: "requestId-modified-files-skip",
			capture,
		});
	...
	test("rejects tool invocation for cancelled request id", async () => {
		...
		assert.strictEqual(
			invoked,
			false,
			"Tool implementation should not run after request cancellation",
		);
	});
	...
	test("toFullReferenceNames", () => {
		...
		const extTool1 = service.getToolByFullReferenceName(
			"my.extension/extTool1RefName",
		);
		const mcpToolSet = service.getToolByFullReferenceName(
			"mcpToolSetRefName/*",
		);
		const mcpTool1 = service.getToolByFullReferenceName(
			"mcpToolSetRefName/mcpTool1RefName",
		);
		const internalToolSet = service.getToolByFullReferenceName(
			"internalToolSetRefName",
		);
		const internalTool = service.getToolByFullReferenceName(
			"internalToolSetRefName/internalToolSetTool1RefName",
		);
		const userToolSet = service.getToolSet("userToolSet");
		const unknownTool = {
			id: "unregisteredTool",
			toolReferenceName: "unregisteredToolRefName",
			modelDescription: "Unregistered Tool",
			displayName: "Unregistered Tool",
			source: ToolDataSource.Internal,
			canBeReferencedInPrompt: true,
		} satisfies IToolData;
		const unknownToolSet = service.createToolSet(
			ToolDataSource.Internal,
			"unknownToolSet",
			"unknownToolSetRefName",
			{ description: "Unknown Test Set" },
		);
		...
		// Test with some enabled tool
		{
			...
			const expectedFullReferenceNames = [
				"tool1RefName",
				"my.extension/extTool1RefName",
				"mcpToolSetRefName/*",
			];
			assert.deepStrictEqual(
				fullReferenceNames.sort(),
				expectedFullReferenceNames.sort(),
				"toFullReferenceNames should return the original enabled names",
			);
		}
		// Test with user data
		{
	...
	test("toToolAndToolSetEnablementMap", () => {
		...
		// Test with enabled tool
		{
			...
			const result1 = service.toToolAndToolSetEnablementMap(
				fullReferenceNames,
				undefined,
			);
			assert.strictEqual(
				result1.size,
				numOfTools,
				`Expected ${numOfTools} tools and tool sets`,
			);
			assert.strictEqual(
				[...result1.entries()].filter(([_, enabled]) => enabled).length,
				1,
				"Expected 1 tool to be enabled",
			);
		...
		// Test with multiple enabled tools
		{
			...
			assert.strictEqual(
				[...result1.entries()].filter(([_, enabled]) => enabled).length,
				4,
				"Expected 4 tools to be enabled",
			);
			assert.strictEqual(
				result1.get(extTool1),
				true,
				"extTool1 should be enabled",
			);
			assert.strictEqual(
				result1.get(mcpToolSet),
				true,
				"mcpToolSet should be enabled",
			);
			assert.strictEqual(
				result1.get(mcpTool1),
				true,
				"mcpTool1 should be enabled because the set is enabled",
			);
			assert.strictEqual(
				result1.get(internalTool),
				true,
				"internalTool should be enabled because the set is enabled",
			);
			...
			assert.deepStrictEqual(
				fullReferenceNames1.sort(),
				fullReferenceNames.sort(),
				"toFullReferenceNames should return the expected names",
			);
		}
		// Test with all enabled tools, redundant names
		{
			const result1 = service.toToolAndToolSetEnablementMap(
				allFullReferenceNames,
				undefined,
			);
			assert.strictEqual(
				result1.size,
			...
			assert.strictEqual(
				[...result1.entries()].filter(([_, enabled]) => enabled).length,
				12,
				"Expected 12 tools to be enabled",
			); // +4 including the vscode, execute, read, agent toolsets
		...
		// Test with no enabled tools
		{
			...
			assert.strictEqual(
				[...result1.entries()].filter(([_, enabled]) => enabled).length,
				0,
				"Expected 0 tools to be enabled",
			);
		...
		// Test with unknown tool
		{
			...
			assert.deepStrictEqual(
				fullReferenceNames1.sort(),
				[],
				"toFullReferenceNames should return no enabled names",
			);
		}
		// Test with legacy tool names
		{
			...
			assert.strictEqual(
				result1.get(internalTool),
				true,
				"internalTool should be enabled",
			);

			const fullReferenceNames1 = service.toFullReferenceNames(result1);
			const expectedFullReferenceNames: string[] = [
				"my.extension/extTool1RefName",
				"mcpToolSetRefName/*",
				"internalToolSetRefName/internalToolSetTool1RefName",
			];
		...
		// Test with tool in user tool set
		{
			...
			assert.strictEqual(
				[...result1.entries()].filter(([_, enabled]) => enabled).length,
				2,
				"Expected 1 tool and user tool set to be enabled",
			);
			assert.strictEqual(result1.get(tool2), true, "tool2 should be enabled");
			assert.strictEqual(
				result1.get(userToolSet),
				true,
				"userToolSet should be enabled",
			);
	...
	test("toToolAndToolSetEnablementMap with extension tool", () => {
		...
		const enabledNames = [toolData1].map((t) =>
			service.getFullReferenceName(t),
		);
		const result = service.toToolAndToolSetEnablementMap(
			enabledNames,
			undefined,
		);

		assert.strictEqual(
			result.get(toolData1),
			true,
			"individual tool should be enabled",
		);
	...
	test("toToolAndToolSetEnablementMap with tool sets", () => {
		...
		// Test enabling the tool set
		const enabledNames = [toolSet, toolData1].map((t) =>
			service.getFullReferenceName(t),
		);
		const result = service.toToolAndToolSetEnablementMap(
			enabledNames,
		...
		assert.strictEqual(
			result.get(toolSetTool1),
			true,
			"tool set tool 1 should be enabled",
		);
		assert.strictEqual(
			result.get(toolSetTool2),
			true,
			"tool set tool 2 should be enabled",
		);
	...
	test("toToolAndToolSetEnablementMap with non-existent tool names", () => {
		...
		const enabledNames = [toolData, unregisteredToolData].map((t) =>
			service.getFullReferenceName(t),
		);
		const result = service.toToolAndToolSetEnablementMap(
			enabledNames,
		...
		assert.strictEqual(
			result.get(toolData),
			true,
			"existing tool should be enabled",
		);
		// Non-existent tools should not appear in the result map
		assert.strictEqual(
			result.get(unregisteredToolData),
			undefined,
			"non-existent tool should not be in result",
		);
	...
	test("toToolAndToolSetEnablementMap with legacy names", () => {
		...
		const toolSetWithLegacy = store.add(
			service.createToolSet(
				ToolDataSource.Internal,
				"newToolSet",
				"newToolSetRef",
				{
					description: "New Tool Set",
					legacyFullNames: ["oldToolSet", "deprecatedToolSet"],
				},
			),
		);
		...
		// Test 1: Using legacy tool reference name should enable the tool
		{
			...
			assert.strictEqual(
				result.get(toolWithLegacy),
				true,
				"tool should be enabled via legacy name",
			);
			...
			assert.deepStrictEqual(
				fullReferenceNames,
				["newToolRef"],
				"should return current full reference name, not legacy",
			);
		}
		...
		// Test 2: Using another legacy tool reference name should also work
		{
			...
			assert.strictEqual(
				result.get(toolWithLegacy),
				true,
				"tool should be enabled via another legacy name",
			);
		...
		// Test 3: Using legacy toolset name should enable the entire toolset
		{
			...
			assert.strictEqual(
				result.get(toolSetWithLegacy),
				true,
				"toolset should be enabled via legacy name",
			);
			assert.strictEqual(
				result.get(toolInSet),
				true,
				"tool in set should be enabled when set is enabled via legacy name",
			);
		...
		// Test 4: Using deprecated toolset name should also work
		{
			...
			assert.strictEqual(
				result.get(toolSetWithLegacy),
				true,
				"toolset should be enabled via another legacy name",
			);
			assert.strictEqual(
				result.get(toolInSet),
				true,
		...
		{
			const result = service.toToolAndToolSetEnablementMap(
				["newToolRef", "oldToolSet"],
				undefined,
			);
			assert.strictEqual(
				result.get(toolWithLegacy),
				true,
				"tool should be enabled via current name",
			);
			assert.strictEqual(
				result.get(toolSetWithLegacy),
				true,
			...
			assert.strictEqual(
				result.get(toolInSet),
				true,
				"tool in set should be enabled",
			);

			const fullReferenceNames = service.toFullReferenceNames(result);
			assert.deepStrictEqual(
				fullReferenceNames.sort(),
				["newToolRef", "newToolSetRef"].sort(),
				"should return current full reference names",
			);
		}
		...
		{
			const result = service.toToolAndToolSetEnablementMap(
				["newToolRef", "oldToolName", "deprecatedToolName"],
				undefined,
			);
			assert.strictEqual(
				result.get(toolWithLegacy),
				true,
				"tool should be enabled (redundant legacy names should not cause issues)",
			);
			...
			assert.deepStrictEqual(
				fullReferenceNames,
				["newToolRef"],
				"should return single current full reference name",
			);
		}
	});
	...
	test("toToolAndToolSetEnablementMap with orphaned toolset in legacy names", () => {
		...
		{
			const result = service.toToolAndToolSetEnablementMap(
				["oldToolSet/oldToolName"],
				undefined,
			);
			assert.strictEqual(
				result.get(toolWithOrphanedToolSet),
				true,
				"tool should be enabled via full legacy name",
			);
			...
			assert.deepStrictEqual(
				fullReferenceNames,
				["newToolRef"],
				"should return current full reference name",
			);
		}
		...
		// Test 2: Using just the orphaned toolset name should also enable the tool
		{
			...
			assert.strictEqual(
				result.get(toolWithOrphanedToolSet),
				true,
				"tool should be enabled via orphaned toolset name",
			);
...

		{
			...
			assert.strictEqual(
				result.get(toolWithOrphanedToolSet),
				true,
				"first tool should be enabled via orphaned toolset name",
			);
			assert.strictEqual(
				result.get(anotherToolFromOrphanedSet),
				true,
				"second tool should also be enabled via orphaned toolset name",
			);

			const fullReferenceNames = service.toFullReferenceNames(result);
			assert.deepStrictEqual(
				fullReferenceNames.sort(),
				["newToolRef", "anotherNewToolRef"].sort(),
				"should return both current full reference names",
			);
		}
...

		{
			...
			assert.strictEqual(
				result.get(toolWithOrphanedToolSet),
				true,
				"tool from oldToolSet should be enabled",
			);
			assert.strictEqual(
				result.get(anotherToolFromOrphanedSet),
				true,
				"another tool from oldToolSet should be enabled",
			);
			assert.strictEqual(
				result.get(unrelatedTool),
				false,
				"tool from different toolset should NOT be enabled",
			);
...

		{
			...
			assert.strictEqual(
				result.get(newToolSetWithSameName),
				true,
				"recreated toolset should be enabled",
			);
			assert.strictEqual(
				result.get(toolInRecreatedSet),
				true,
				"tool in recreated set should be enabled",
			);
			// The tools with legacy toolset names should ALSO be enabled because their legacy names match
			assert.strictEqual(
				result.get(toolWithOrphanedToolSet),
				true,
				"tool with legacy toolset should still be enabled",
			);
			assert.strictEqual(
				result.get(anotherToolFromOrphanedSet),
				true,
				"another tool with legacy toolset should still be enabled",
			);
			...
			assert.deepStrictEqual(
				fullReferenceNames.sort(),
				["oldToolSet", "newToolRef", "anotherNewToolRef"].sort(),
				"should return toolset and individual tools",
			);
		}
	});
	...
	test("toToolAndToolSetEnablementMap map Github to VSCode tools", () => {
		...
		const githubMcpDataSource: ToolDataSource = {
			type: "mcp",
			label: "Github",
			serverLabel: "Github MCP Server",
			instructions: undefined,
			collectionId: "githubMCPCollection",
			definitionId: "githubMCPDefId",
		};
		...
		const githubMcpToolSet = store.add(
			service.createToolSet(
				githubMcpDataSource,
				"githubMcpToolSet",
		...
		assert.equal(
			githubMcpToolSet.referenceName,
			"github",
			"github/github-mcp-server will be normalized to github",
		);

		const playwrightMcpDataSource: ToolDataSource = {
			type: "mcp",
			label: "playwright",
			serverLabel: "playwright MCP Server",
			instructions: undefined,
			collectionId: "playwrightMCPCollection",
			definitionId: "playwrightMCPDefId",
		};
		...
		const playwrightMcpToolSet = store.add(
			service.createToolSet(
		...
		assert.equal(
			playwrightMcpToolSet.referenceName,
			"playwright",
			"microsoft/playwright-mcp will be normalized to playwright",
		);

		{
			...
			assert.strictEqual(
				result.get(service.executeToolSet),
				true,
				"execute should be enabled",
			);
			assert.strictEqual(
				result.get(service.agentToolSet),
				true,
			...
			assert.deepStrictEqual(
				fullReferenceNames,
				[SpecedToolAliases.agent, SpecedToolAliases.execute].sort(),
				"toFullReferenceNames should return the VS Code tool names",
			);

			assert.deepStrictEqual(
				toolNames.map((name) => service.getToolByFullReferenceName(name)),
				[service.agentToolSet, service.executeToolSet],
			);

			assert.deepStrictEqual(
				deprecatesTo("custom-agent"),
				[SpecedToolAliases.agent],
				"customAgent should map to agent",
			);
			assert.deepStrictEqual(
				deprecatesTo("shell"),
				[SpecedToolAliases.execute],
				"shell is now execute",
			);
		}
		{
			...
			assert.strictEqual(
				result.get(githubMcpToolSet),
				true,
				"githubMcpToolSet should be enabled",
			);
			assert.strictEqual(
				result.get(playwrightMcpToolSet),
				true,
				"playwrightMcpToolSet should be enabled",
			);
			const fullReferenceNames = service.toFullReferenceNames(result).sort();
			assert.deepStrictEqual(
				fullReferenceNames,
			...
			assert.deepStrictEqual(
				toolNames.map((name) => service.getToolByFullReferenceName(name)),
				[githubMcpToolSet, playwrightMcpToolSet],
			);
...

		{
			...
			assert.strictEqual(
				result.get(githubMcpTool1),
				true,
				"githubMcpTool1 should be enabled",
			);
			assert.strictEqual(
				result.get(playwrightMcpTool1),
				true,
				"playwrightMcpTool1 should be enabled",
			);
			const fullReferenceNames = service.toFullReferenceNames(result).sort();
			assert.deepStrictEqual(
				fullReferenceNames,
				["github/create_branch", "playwright/browser_click"],
				"toFullReferenceNames should return the speced names",
			);

			assert.deepStrictEqual(
				toolNames.map((name) => service.getToolByFullReferenceName(name)),
				[githubMcpTool1, playwrightMcpTool1],
			);

			assert.deepStrictEqual(
				deprecatesTo("github/create_branch"),
				undefined,
				"github/create_branch is fine",
			);
			assert.deepStrictEqual(
				deprecatesTo("playwright/browser_click"),
				undefined,
				"playwright/browser_click is fine",
			);
		}

		{
			...
			assert.deepStrictEqual(deprecatesTo("github/github-mcp-server/*"), [
				"github/*",
			]);
			assert.deepStrictEqual(deprecatesTo("microsoft/playwright-mcp/*"), [
				"playwright/*",
			]);
		}
		{
			...
			assert.deepStrictEqual(
				deprecatesTo("github/github-mcp-server/create_branch"),
				["github/create_branch"],
			);
			assert.deepStrictEqual(
				deprecatesTo("microsoft/playwright-mcp/browser_click"),
...

		{
			...
			assert.deepStrictEqual(
				deprecatesTo("io.github.github/github-mcp-server/*"),
				["github/*"],
			);
			assert.deepStrictEqual(deprecatesTo("com.microsoft/playwright-mcp/*"), [
				"playwright/*",
			]);
		}

		{
			...
			assert.deepStrictEqual(
				deprecatesTo("io.github.github/github-mcp-server/create_branch"),
				["github/create_branch"],
			);
			assert.deepStrictEqual(
				deprecatesTo("com.microsoft/playwright-mcp/browser_click"),
				["playwright/browser_click"],
			);
		}

		{
			...
			assert.deepStrictEqual(deprecatesTo("github-mcp-server/create_branch"), [
				"github/create_branch",
			]);
		}
	});
	...
	test("accessibility signal for tool confirmation", async () => {
		...
		const testAccessibilityService =
		...
		const { service: testService, chatService: testChatService } =
			createTestToolsService(store, {
			...
			toolData.id,
			{
				prepareToolInvocation: async () => ({
					confirmationMessages: {
						...
						message: "Testing accessibility signal",
					},
				}),
				invoke: async () => ({
					content: [{ kind: "text", value: "executed" }],
				}),
			},
			toolData,
		);
		...
		stubGetSession(testChatService, sessionId, {
			requestId: "requestId-accessibility",
			capture,
		});
		...
		assert.ok(published, "expected ChatToolInvocation to be published");
		assert.ok(
			published.confirmationMessages,
			"should have confirmation messages",
		);

		// The accessibility signal should have been played
		assert.strictEqual(
			testAccessibilitySignalService.signalPlayedCalls.length,
			1,
			"accessibility signal should have been played once",
		);
		const signalCall = testAccessibilitySignalService.signalPlayedCalls[0];
		assert.strictEqual(
			signalCall.signal,
			AccessibilitySignal.chatUserActionRequired,
			"correct signal should be played",
		);
		assert.ok(
			signalCall.options?.customAlertMessage.includes("Accessibility Test"),
			"alert message should include tool title",
		);
		assert.ok(
			signalCall.options?.customAlertMessage.includes(
				"Chat confirmation required",
			),
			"alert message should include confirmation text",
		);
	...
	test("accessibility signal respects autoApprove configuration", async () => {
			...
			toolData.id,
			{
				prepareToolInvocation: async () => ({
					confirmationMessages: {
						...
						message: "Testing auto approve",
					},
				}),
				invoke: async () => ({
					content: [{ kind: "text", value: "auto approved" }],
				}),
			},
			toolData,
		);
		...
		const capture: { invocation?: any } = {};
		stubGetSession(testChatService, sessionId, {
			requestId: "requestId-auto-approve",
			capture,
		});
		...
		assert.strictEqual(
			testAccessibilitySignalService.signalPlayedCalls.length,
			0,
			"accessibility signal should not be played when auto-approve is enabled",
		);
	});
	...
	test("autopilot permission level bypasses global auto-approve check", async () => {
		...
		const tool = registerToolForTest(testService, store, "autopilotTool", {
			prepareToolInvocation: async () => ({
				confirmationMessages: {
					title: "Confirm?",
					message: "Should be auto-approved by autopilot",
				},
			}),
			invoke: async () => ({
				content: [{ kind: "text", value: "autopilot approved" }],
			}),
		});
	...
	test("autopilot finds correct request by chatRequestId", async () => {
		...
		const tool = registerToolForTest(testService, store, "autopilotIdTool", {
			prepareToolInvocation: async () => ({
				confirmationMessages: { title: "Confirm?", message: "Test" },
			}),
			invoke: async () => ({
				content: [{ kind: "text", value: "found by id" }],
			}),
		});
	...
	test("autopilot auto-approves terminal tool with confirmation messages", async () => {
		...
		const tool = registerToolForTest(testService, store, "terminalTool", {
			...
			invoke: async () => ({
				content: [{ kind: "text", value: "terminal executed" }],
			}),
		});
	...
	test("bypass approvals auto-approves terminal tool with confirmation messages", async () => {
		...
		const tool = registerToolForTest(testService, store, "terminalToolBypass", {
			...
			invoke: async () => ({
				content: [{ kind: "text", value: "bypass executed" }],
			}),
		});
	...
	test("bypass approvals does not auto-approve tools in toolIdsThatCannotBeAutoApproved for CLI sessions", async () => {
		...
		registerToolForTest(
			...
			"vscode_get_modified_files_confirmation",
			{
				...
				invoke: async () => ({
					content: [{ kind: "text", value: "confirmed" }],
				}),
			},
		);
		...
		assert.ok(
			published?.confirmationMessages,
			"tool in toolIdsThatCannotBeAutoApproved should require confirmation for CLI sessions even with Bypass Approvals",
		);
	...
	test("bypass approvals auto-approves tools in toolIdsThatCannotBeAutoApproved for local sessions", async () => {
		...
		const tool = registerToolForTest(
			...
			"vscode_get_modified_files_confirmation",
			{
				...
				invoke: async () => ({
					content: [{ kind: "text", value: "auto approved for local" }],
				}),
			},
		);
	...
	test("shouldAutoConfirm with basic configuration", async () => {
		...
		const autoTool = registerToolForTest(testService, store, "autoTool", {
			prepareToolInvocation: async () => ({
				confirmationMessages: { title: "Test", message: "Should auto-approve" },
			}),
			invoke: async () => ({
				content: [{ kind: "text", value: "auto approved" }],
			}),
	...
	test("shouldAutoConfirm with per-tool configuration object", async () => {
		...
		const approvedTool = registerToolForTest(
			testService,
			store,
			"approvedTool",
			{
				prepareToolInvocation: async () => ({
					confirmationMessages: {
						title: "Test",
						message: "Should auto-approve",
					},
				}),
				invoke: async () => ({
					content: [{ kind: "text", value: "approved" }],
				}),
			},
		);
		...
		const unspecifiedTool = registerToolForTest(
			testService,
			store,
			"unspecifiedTool",
			{
				prepareToolInvocation: async () => ({
					confirmationMessages: {
						title: "Test",
						message: "Should require confirmation",
					},
				}),
				invoke: async () => ({
					content: [{ kind: "text", value: "unspecified" }],
				}),
			},
		);

		const capture: { invocation?: any } = {};
		stubGetSession(testChatService, sessionId + "2", {
			requestId: "req2",
			capture,
		});
		...
		assert.ok(
			published?.confirmationMessages,
			"unspecified tool should require confirmation",
		);
	...
	test("eligibleForAutoApproval setting controls tool eligibility", async () => {
		...
		const eligibleTool = registerToolForTest(
			testService,
			store,
			"eligibleTool",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "eligible tool ran" }],
				}),
			},
			{
		...
		const ineligibleTool = registerToolForTest(
			testService,
			store,
			"ineligibleTool",
			{
				...
				invoke: async () => ({
					content: [
						{ kind: "text", value: "ineligible requires confirmation" },
					],
				}),
			},
			{
		...
		assert.ok(
			published?.confirmationMessages,
			"ineligible tool should require confirmation",
		);
		assert.ok(
			published?.confirmationMessages?.title,
			"should have default confirmation title",
		);
		assert.strictEqual(
			published?.confirmationMessages?.allowAutoConfirm,
			false,
			"should not allow auto confirm",
		);
		...
		assert.strictEqual(
			ineligibleResult.content[0].value,
			"ineligible requires confirmation",
		);
		...
		const unspecifiedTool = registerToolForTest(
			...
			"unspecifiedTool",
			{
				...
				invoke: async () => ({
					content: [
						{ kind: "text", value: "unspecified defaults to eligible" },
					],
				}),
			},
			{
		...
		assert.strictEqual(
			unspecifiedResult.content[0].value,
			"unspecified defaults to eligible",
		);
	});
	...
	test("tool content formatting with alwaysDisplayInputOutput", async () => {
				...
				invoke: async (invocation) => ({
					content: [
						{ kind: "text", value: "Text result" },
						{
							kind: "data",
							value: {
								data: VSBuffer.fromByteArray([1, 2, 3]),
		...
		assert.strictEqual(
			details.input,
			expectedInputJson,
			"input should be formatted JSON",
		);
	...
	test("tool error handling and telemetry", async () => {
		...
		const successEvents = testTelemetryService.events.filter(
			(e) => e.eventName === "languageModelToolInvoked",
		);
		assert.strictEqual(
			successEvents.length,
			1,
			"should have success telemetry event",
		);
		...
		const errorEvents = testTelemetryService.events.filter(
			(e) => e.eventName === "languageModelToolInvoked",
		);
		assert.strictEqual(
			errorEvents.length,
			1,
			"should have error telemetry event",
		);
	...
	test("accessibility signal with different settings combinations", async () => {
		...
		testConfigService1.setUserConfiguration(
		...
		const testAccessibilityService1 =
			new (class extends TestAccessibilityService {
				override isScreenReaderOptimized(): boolean {
					return false;
				}
		...
		const instaService1 = workbenchInstantiationService(
			{
				contextKeyService: () =>
					store.add(new ContextKeyService(testConfigService1)),
		...
		instaService1.stub(
			IAccessibilitySignalService,
			testAccessibilitySignalService as unknown as IAccessibilitySignalService,
		);
		instaService1.stub(
			ILanguageModelToolsConfirmationService,
			new MockLanguageModelToolsConfirmationService(),
		...
		const testService1 = store.add(
			instaService1.createInstance(LanguageModelToolsService),
		);
		...
		stubGetSession(chatService, sessionId1, {
			requestId: "req1",
			capture: capture1,
		});

		const promise1 = testService1.invokeTool(
			tool1.makeDto({ test: 1 }, { sessionId: sessionId1 }),
			async () => 0,
			CancellationToken.None,
		);
		...
		assert.strictEqual(
			testAccessibilitySignalService.signalPlayedCalls.length,
			1,
			"sound should be played when sound=on",
		);
		const call1 = testAccessibilitySignalService.signalPlayedCalls[0];
		assert.strictEqual(
			call1.options?.modality,
			undefined,
			"should use default modality for sound",
		);

		IChatToolInvocation.confirmWith(published1, {
			type: ToolConfirmKind.UserAction,
		});
		await promise1;
		...
		const testAccessibilityService2 =
			new (class extends TestAccessibilityService {
				override isScreenReaderOptimized(): boolean {
					return true;
				}
		...
		const instaService2 = workbenchInstantiationService(
			{
				contextKeyService: () =>
					store.add(new ContextKeyService(testConfigService2)),
		...
		const testService2 = store.add(
			instaService2.createInstance(LanguageModelToolsService),
		);

		const tool2 = registerToolForTest(
			testService2,
			store,
			"autoScreenReaderTool",
			{
				prepareToolInvocation: async () => ({
					confirmationMessages: {
						title: "Auto Test",
						message: "Testing auto with screen reader",
					},
				}),
				invoke: async () => ({
					content: [{ kind: "text", value: "executed" }],
		...
		stubGetSession(chatService, sessionId2, {
			requestId: "req2",
			capture: capture2,
		});

		const promise2 = testService2.invokeTool(
			tool2.makeDto({ test: 2 }, { sessionId: sessionId2 }),
			async () => 0,
			CancellationToken.None,
		);
		...
		assert.strictEqual(
			testAccessibilitySignalService.signalPlayedCalls.length,
			1,
			"signal should be played with screen reader optimization",
		);
		const call2 = testAccessibilitySignalService.signalPlayedCalls[0];
		assert.ok(
			call2.options?.customAlertMessage,
			"should have custom alert message",
		);
		assert.strictEqual(
			call2.options?.userGesture,
			true,
			"should mark as user gesture",
		);

		IChatToolInvocation.confirmWith(published2, {
			type: ToolConfirmKind.UserAction,
		});
		await promise2;
		...
		testConfigService3.setUserConfiguration(
			"chat.tools.global.autoApprove",
			false,
		);
		testConfigService3.setUserConfiguration(
		...
		const testAccessibilityService3 =
			new (class extends TestAccessibilityService {
				override isScreenReaderOptimized(): boolean {
					return true;
				}
		...
		const instaService3 = workbenchInstantiationService(
			{
				contextKeyService: () =>
					store.add(new ContextKeyService(testConfigService3)),
		...
		const testService3 = store.add(
			instaService3.createInstance(LanguageModelToolsService),
		);
		...
		stubGetSession(chatService, sessionId3, {
			requestId: "req3",
			capture: capture3,
		});

		const promise3 = testService3.invokeTool(
			tool3.makeDto({ test: 3 }, { sessionId: sessionId3 }),
			async () => 0,
			CancellationToken.None,
		);
		...
		assert.strictEqual(
			testAccessibilitySignalService.signalPlayedCalls.length,
			0,
			"no signal should be played when both sound and announcement are off",
		);

		IChatToolInvocation.confirmWith(published3, {
			type: ToolConfirmKind.UserAction,
		});
		await promise3;
	});
	...
	test("getToolSetByName", () => {
		store.add(
			service.createToolSet(ToolDataSource.Internal, "toolSet1", "refName1"),
		);

		store.add(
			service.createToolSet(ToolDataSource.Internal, "toolSet2", "refName2"),
		);
	...
	test("getTools with includeDisabled parameter", () => {
		...
		assert.strictEqual(
			allTools.length,
			2,
			"getAllToolsIncludingDisabled should return all tools",
		);
	});
	...
	test("invokeTool with unknown chat session throws", async () => {
		const tool = registerToolForTest(service, store, "unknownSessionTool", {
			invoke: async () => ({
				content: [{ kind: "text", value: "should not reach" }],
			}),
		});
	...
	test("context key changes trigger tool updates", async () => {
		...
		assert.strictEqual(
			changeEventFired,
			true,
			"onDidChangeTools should fire when context keys change",
		);
	});
	...
	test("configuration changes trigger tool updates", async () => {
		...
		assert.strictEqual(
			changeEventFired,
			true,
			"onDidChangeTools should fire when configuration changes",
		);
	});
	...
	test("toToolAndToolSetEnablementMap with MCP toolset enables contained tools", () => {
		...
		{
			const enabledNames = [mcpToolSet].map((t) =>
				service.getFullReferenceName(t),
			);
			const result = service.toToolAndToolSetEnablementMap(
				enabledNames,
			...
			assert.strictEqual(
				result.get(mcpToolSet),
				true,
				"MCP toolset should be enabled",
			); // Ensure the toolset is in the map
			assert.strictEqual(
				result.get(mcpTool),
				true,
				"MCP tool should be enabled when its toolset is enabled",
			); // Ensure the tool is in the map
		...
		{
			const enabledNames = [mcpTool].map((t) =>
				service.getFullReferenceName(t, mcpToolSet),
			);
			const result = service.toToolAndToolSetEnablementMap(
				enabledNames,
				undefined,
			...
			assert.strictEqual(
				result.get(mcpToolSet),
				false,
				"MCP toolset should be disabled",
			); // Ensure the toolset is in the map
			assert.strictEqual(
				result.get(mcpTool),
				true,
				"MCP tool should be enabled",
			); // Ensure the tool is in the map
	...
	test("shouldAutoConfirm with workspace-specific tool configuration", async () => {
		...
		const workspaceTool = registerToolForTest(
			testService,
			store,
			"workspaceTool",
			{
				prepareToolInvocation: async () => ({
					confirmationMessages: { title: "Test", message: "Workspace tool" },
				}),
				invoke: async () => ({
					content: [{ kind: "text", value: "workspace result" }],
				}),
			},
	...
	test("getFullReferenceNames", () => {
		...
		const fullReferenceNames = Array.from(
			service.getFullReferenceNames(),
		).sort();
		...
		assert.deepStrictEqual(
			fullReferenceNames,
			expectedNames,
			"getFullReferenceNames should return correct full reference names",
		);
	});
	...
	test("getDeprecatedFullReferenceNames", () => {
		...
		assert.deepStrictEqual(
			deprecatedNames.get("internalToolSetTool1RefName"),
			new Set(["internalToolSetRefName/internalToolSetTool1RefName"]),
		);
		assert.strictEqual(
			deprecatedNames.get("internalToolSetRefName"),
			undefined,
		);

		// For extension tools, the full reference name includes the extension ID
		assert.deepStrictEqual(
			deprecatedNames.get("extTool1RefName"),
			new Set(["my.extension/extTool1RefName"]),
		);

		// For MCP tool sets, the full reference name includes the /* suffix
		assert.deepStrictEqual(
			deprecatedNames.get("mcpToolSetRefName"),
			new Set(["mcpToolSetRefName/*"]),
		);
		assert.deepStrictEqual(
			deprecatedNames.get("mcpTool1RefName"),
			new Set(["mcpToolSetRefName/mcpTool1RefName"]),
		);
	...
	test("getDeprecatedFullReferenceNames includes namespaced legacy names for tools in toolsets", () => {
		...
		assert.deepStrictEqual(
			deprecated.get("openSimpleBrowser"),
			new Set(["vscode/openIntegratedBrowser"]),
		);

		// The namespaced legacy name should also map to the full reference name
		assert.deepStrictEqual(
			deprecated.get("vscode/openSimpleBrowser"),
			new Set(["vscode/openIntegratedBrowser"]),
		);
	});
	...
	test("getToolByFullReferenceName", () => {
		...
		const extTool = service.getToolByFullReferenceName(
			"my.extension/extTool1RefName",
		);
		assert.ok(extTool);
		assert.strictEqual(extTool.id, "extTool1");

		const mcpTool = service.getToolByFullReferenceName(
			"mcpToolSetRefName/mcpTool1RefName",
		);
		assert.ok(mcpTool);
		...
		const toolInSet = service.getToolByFullReferenceName(
			"internalToolSetRefName",
		);
		assert.ok(toolInSet);
	...
	test("eligibleForAutoApproval setting can be configured via policy", async () => {
		...
		const toolA = registerToolForTest(
			testService,
			store,
			"toolA",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "toolA executed" }],
				}),
			},
			{
		...
		const toolB = registerToolForTest(
			testService,
			store,
			"toolB",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "toolB executed" }],
				}),
			},
			{
		...
		assert.ok(
			published?.confirmationMessages,
			"toolB should require confirmation due to policy",
		);
		assert.strictEqual(
			published?.confirmationMessages?.allowAutoConfirm,
			false,
	...
	test("eligibleForAutoApproval with legacy tool reference names - eligible", async () => {
		...
		const renamedTool = registerToolForTest(
			testService,
			store,
			"renamedTool",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "tool executed via legacy name" }],
				}),
			},
			{
	...
	test("eligibleForAutoApproval with legacy tool reference names - ineligible", async () => {
		...
		const renamedTool = registerToolForTest(
			...
			"renamedTool2",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "tool requires confirmation" }],
				}),
			},
			{
		...
		assert.ok(
			published?.confirmationMessages,
			"tool should require confirmation when legacy name is ineligible",
		);
		assert.strictEqual(
			published?.confirmationMessages?.allowAutoConfirm,
			false,
	...
	test("eligibleForAutoApproval with multiple legacy names", async () => {
		...
		const multiLegacyTool = registerToolForTest(
			testService,
			store,
			"multiLegacyTool",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "multi legacy executed" }],
				}),
			},
			{
	...
	test("eligibleForAutoApproval current name takes precedence over legacy names", async () => {
		...
		const tool = registerToolForTest(
			...
			"precedenceTool",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "precedence test" }],
				}),
			},
			{
		...
		assert.ok(
			published?.confirmationMessages,
			"current name should take precedence over legacy name",
		);
		assert.strictEqual(
			published?.confirmationMessages?.allowAutoConfirm,
			false,
	...
	test("eligibleForAutoApproval with legacy full reference names from toolsets", async () => {
		...
		const migratedTool = registerToolForTest(
			testService,
			store,
			"migratedTool",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "migrated tool" }],
				}),
			},
			{
		...
		assert.ok(
			published?.confirmationMessages,
			"tool should be ineligible via legacy full reference name",
		);
		assert.strictEqual(
			published?.confirmationMessages?.allowAutoConfirm,
			false,
	...
	test("eligibleForAutoApproval mixed current and legacy names", async () => {
		...
		const tool1 = registerToolForTest(
			testService,
			store,
			"tool1",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "modern executed" }],
				}),
			},
			{
		...
		const tool2 = registerToolForTest(
			...
			"tool2",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "legacy needs confirmation" }],
				}),
			},
			{
		...
		// Unchanged tool
		const tool3 = registerToolForTest(
			testService,
			store,
			"tool3",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "unchanged executed" }],
				}),
			},
			{
		...
		const published2 = await waitForPublishedInvocation(capture2);
		assert.ok(
			published2?.confirmationMessages,
			"tool2 should require confirmation via legacy name",
		);
	...
	test("eligibleForAutoApproval with namespaced legacy names - full tool name eligible", async () => {
		...
		testConfigService.setUserConfiguration(
			ChatConfiguration.EligibleForAutoApproval,
			{
		...
		const tool = registerToolForTest(
			...
			"gitCommitTool",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "commit executed" }],
				}),
			},
			{
		...
		assert.strictEqual(
			published,
			undefined,
			"tool should not require confirmation when legacy trimmed name is eligible",
		);
	...
	test("eligibleForAutoApproval with namespaced legacy names - full tool name ineligible", async () => {
		...
		const tool = registerToolForTest(
			...
			"gitCommitTool3",
			{
				prepareToolInvocation: async () => ({}),
				invoke: async () => ({
					content: [{ kind: "text", value: "commit blocked" }],
				}),
			},
			{
				toolReferenceName: "commit",
		...
		assert.ok(
			published?.confirmationMessages,
			"tool should require confirmation when legacy full name is ineligible",
		);
		assert.strictEqual(
			published?.confirmationMessages?.allowAutoConfirm,
			false,
	...
	test("eligibleForAutoApproval with namespaced and renamed toolname - just last segment ineligible", async () => {
		...
		assert.ok(
			published?.confirmationMessages,
			"tool should require confirmation when legacy trimmed name is ineligible",
		);
		assert.strictEqual(
			published?.confirmationMessages?.allowAutoConfirm,
			false,
	...
	test("beginToolCall returns undefined for unknown tool", () => {
		...
		assert.strictEqual(
			invocation,
			undefined,
			"beginToolCall should return undefined for unknown tools",
		);
	});
	...
	test("beginToolCall returns undefined for tool without handleToolStream", () => {
		...
		assert.strictEqual(
			invocation,
			undefined,
			"beginToolCall should return undefined when tool lacks handleToolStream",
		);
	});
	...
	test("beginToolCall with force creates invocation even without handleToolStream", () => {
		...
		assert.ok(
			invocation,
			"beginToolCall with force should return an invocation",
		);
		assert.strictEqual(invocation.toolId, tool.id);
	});
	...
	test("updateToolStream calls handleToolStream on tool implementation", async () => {
		...
		await service.updateToolStream(
			"call-stream",
			partialInput,
			CancellationToken.None,
		);
		...
		assert.strictEqual(
			handleToolStreamCalled,
			true,
			"handleToolStream should be called",
		);
		assert.deepStrictEqual(
			receivedRawInput,
			partialInput,
			"should receive the partial input",
		);
	});
	...
	test("toToolAndToolSetEnablementMap with model metadata filters tools", () => {
		...
		assert.ok(
			gpt4Tool && anyModelTool && claudeTool,
		...
		assert.strictEqual(
			result.get(gpt4Tool),
			true,
			"gpt4Tool should be enabled",
		);
		// anyModelTool should be enabled (no model restriction)
		assert.strictEqual(
			result.get(anyModelTool),
			true,
			"anyModelTool should be enabled",
		);
		// claudeTool should NOT be in the enablement map (filtered out by model)
		assert.strictEqual(
			result.has(claudeTool),
			false,
			"claudeTool should be filtered out by model",
		);
	});
	...
	test("gpt-5.5 readFile setting controls Copilot read tool availability", () => {
		...
		configurationService.setUserConfiguration(
			CopilotChatSettingId.Gpt55ReadFileToolEnabled,
			false,
		);

		const disabledTools = Array.from(service.getTools(gpt55Model));
		assert.ok(
			!disabledTools.some((tool) => tool.id === CopilotToolId.ReadFile),
			"readFile should not be returned from getTools when disabled for gpt-5.5",
		);

		const disabledReadToolSet = Array.from(
			service.getToolSetsForModel(gpt55Model),
		).find((toolSet) => toolSet.id === "read");
		assert.ok(disabledReadToolSet, "read tool set should exist");
		assert.ok(
			!Array.from(disabledReadToolSet.getTools()).some(
				(tool) => tool.id === CopilotToolId.ReadFile,
			),
			"readFile should not be included as a read tool-set member when disabled for gpt-5.5",
		);

		const disabledEnablementMap = service.toToolAndToolSetEnablementMap(
			["read/readFile"],
			gpt55Model,
		);
		assert.strictEqual(
			disabledEnablementMap.has(readTool),
			false,
			"readFile should not be included in explicit enablement maps when disabled for gpt-5.5",
		);
		...
		assert.ok(
			enabledTools.some((tool) => tool.id === CopilotToolId.ReadFile),
			"readFile should be returned from getTools when enabled for gpt-5.5",
		);

		const enabledReadToolSet = Array.from(
			service.getToolSetsForModel(gpt55Model),
		).find((toolSet) => toolSet.id === "read");
		assert.ok(enabledReadToolSet, "read tool set should exist");
		assert.ok(
			Array.from(enabledReadToolSet.getTools()).some(
				(tool) => tool.id === CopilotToolId.ReadFile,
			),
			"readFile should be included as a read tool-set member when enabled for gpt-5.5",
		);

		const enabledEnablementMap = service.toToolAndToolSetEnablementMap(
			["read/readFile"],
			gpt55Model,
		);
		assert.strictEqual(
			enabledEnablementMap.get(readTool),
			true,
			"readFile should be included in explicit enablement maps when enabled for gpt-5.5",
		);
	});
	...
	test("invokeTool with chatStreamToolCallId correlates with pending streaming call", async () => {
		const tool = registerToolForTest(service, store, "correlatedTool", {
			invoke: async () => ({
				content: [{ kind: "text", value: "correlated result" }],
			}),
			handleToolStream: async () => ({ invocationMessage: "Processing..." }),
		});
	...
	test("getAllToolsIncludingDisabled returns tools regardless of when clause", () => {
		...
		assert.ok(
			allTools.some((t) => t.id === "enabledTool"),
			"should include enabled tool",
		);
		assert.ok(
			allTools.some((t) => t.id === "disabledTool"),
			"should include disabled tool",
		);
		...
		assert.strictEqual(
			enabledTools.length,
			1,
			"getTools should only return matching tools",
		);
		assert.strictEqual(enabledTools[0].id, "enabledTool");
	});
	...
	test("getTools filters by model id using models property", () => {
		...
		assert.ok(
			tools.some((t) => t.id === "gpt4Tool"),
			"should include GPT-4 tool",
		);
		assert.ok(
			tools.some((t) => t.id === "universalTool"),
			"should include universal tool",
		);
		assert.ok(
			!tools.some((t) => t.id === "claudeTool"),
			"should NOT include Claude tool",
		);
	});
	...
	test("getTools filters by model family using models property", () => {
		...
		assert.strictEqual(
			tools[0].id,
			"gpt4FamilyTool",
			"should include GPT-4 family tool",
		);
	});
	...
	test("getTools with undefined model skips model filtering", () => {
		...
		assert.strictEqual(
			tools.length,
			2,
			"should return all tools when model is undefined",
		);
		assert.ok(
			tools.some((t) => t.id === "gpt4Tool"),
			"should include GPT-4 tool",
	...
	test("IToolData models property stores selector information", () => {
		...
		assert.strictEqual(
			tool.models.length,
			2,
			"tool should have 2 model selectors",
		);
		assert.deepStrictEqual(tool.models[0], {
			vendor: "openai",
			family: "gpt-4",
		});
		assert.deepStrictEqual(tool.models[1], {
			vendor: "anthropic",
			family: "claude-3",
		});
	});
	...
	test("tools with extension tools disabled setting are filtered", () => {
		...
		assert.ok(
			tools.some((t) => t.id === "extensionTool"),
			"extension tool should be included when enabled",
		);
		...
		assert.ok(
			!tools.some((t) => t.id === "extensionTool"),
			"extension tool should be excluded when disabled",
		);
	...
	test("observeTools changes when context key changes", () => {
		const testCtxKey = contextKeyService.createKey<string>(
			"dynamicTestKey",
			"value1",
		);
		...
		assert.strictEqual(
			tools[0].id,
			"dynamicTool2",
			"should be dynamicTool2 after context change",
		);
	});
	...
	test("isPermitted allows tools in permitted toolsets when agent mode is disabled", () => {
		...
		assert.ok(
			toolIds.includes("readToolInSet"),
			"Tool in read toolset should be permitted when agent mode is disabled",
		);
		assert.ok(
			!toolIds.includes("standaloneTool"),
			"Standalone tool not in permitted toolset should NOT be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted allows all tools when agent mode is enabled", () => {
		...
		assert.ok(
			toolIds.includes("readToolEnabled"),
			"Tool in read toolset should be permitted when agent mode is enabled",
		);
		assert.ok(
			toolIds.includes("standaloneToolEnabled"),
			"Standalone tool should be permitted when agent mode is enabled",
		);
	});
	...
	test("isPermitted filters toolsets when agent mode is disabled", () => {
		...
		assert.ok(
			toolSetIds.includes("read"),
			"read toolset should be permitted when agent mode is disabled",
		);
		assert.ok(
			!toolSetIds.includes("customToolSet"),
			"custom toolset should NOT be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted allows execute toolset tools when agent mode is enabled", () => {
		...
		assert.ok(
			toolIds.includes("executeToolInSet"),
			"Tool in execute toolset should be permitted when agent mode is enabled",
		);
	});
	...
	test("isPermitted blocks execute toolset tools when agent mode is disabled", () => {
		...
		assert.ok(
			!toolIds.includes("executeToolBlocked"),
			"Tool in execute toolset should NOT be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted allows search toolset tools when agent mode is disabled", () => {
		...
		assert.ok(
			toolIds.includes("searchToolInSet"),
			"Tool in search toolset should be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted allows web toolset tools when agent mode is disabled", () => {
		...
		assert.ok(
			toolIds.includes("webToolInSet"),
			"Tool in web toolset should be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted allows vscode_fetchWebPage_internal special case when agent mode is disabled", () => {
		...
		assert.ok(
			toolIds.includes("vscode_fetchWebPage_internal"),
			"vscode_fetchWebPage_internal should be permitted as special case when agent mode is disabled",
		);
	});
	...
	test("isPermitted blocks extension tools not in permitted toolsets when agent mode is disabled", () => {
		...
		assert.ok(
			!toolIds.includes("extensionToolBlocked"),
			"Extension tool not in permitted toolset should NOT be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted blocks MCP tools not in permitted toolsets when agent mode is disabled", () => {
			...
			service.createToolSet(
				{
					...
					instructions: undefined,
					collectionId: "testMcp",
					definitionId: "testMcpDef",
				},
				"mcpToolSetBlocked",
		...
		assert.ok(
			!toolIds.includes("mcpToolBlocked"),
			"MCP tool should NOT be permitted when agent mode is disabled",
		);
		...
		assert.ok(
			!toolSetIds.includes("mcpToolSetBlocked"),
			"MCP toolset should NOT be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted blocks agent toolset tools when agent mode is disabled", () => {
		...
		assert.ok(
			!toolIds.includes("agentToolBlocked"),
			"Tool in agent toolset should NOT be permitted when agent mode is disabled",
		);
		...
		assert.ok(
			!toolSetIds.includes("agent"),
			"agent toolset should NOT be permitted when agent mode is disabled",
		);
	});
	...
	test("isPermitted includes tool in multiple toolsets if one is permitted", () => {
		...
		assert.ok(
			toolIds.includes("multiSetTool"),
			"Tool should be permitted if it belongs to at least one permitted toolset",
		);
	});
	...
	test("isPermitted allows internal tools with canBeReferencedInPrompt=false when agent mode is disabled (issue #292935)", () => {
		...
		assert.ok(
			toolIds.includes("infrastructureToolInternal"),
			"Internal infrastructure tool with canBeReferencedInPrompt=false should be permitted when agent mode is disabled",
		);
		assert.ok(
			!toolIds.includes("referencableTool"),
			"Internal tool with canBeReferencedInPrompt=true should NOT be permitted when agent mode is disabled",
		);
		assert.ok(
			!toolIds.includes("undefinedTool"),
			"Internal tool with canBeReferencedInPrompt=undefined should NOT be permitted when agent mode is disabled",
		);
	});
	...
	suite("ToolSet when clause filtering (issue #291154)", () => {
		test("ToolSet.getTools filters tools by when clause", () => {
			...
			assert.ok(
				toolIds.includes("toolWithWhenFalse"),
				"Tool with when=false should be in tool set when context key is false",
			);
			assert.ok(
				toolIds.includes("toolWithoutWhen"),
				"Tool without when clause should be in tool set",
			);
			assert.ok(
				!toolIds.includes("toolWithWhenTrue"),
				"Tool with when=true should NOT be in tool set when context key is false",
			);
		});

		test("ToolSet.getTools updates when context key changes", () => {
			// Create a context key for testing
			const testKey = contextKeyService.createKey<string>(
				"dynamicTestKey",
				"value1",
			);
			...
			// Create a tool set and add the tools
			const dynamicToolSet = store.add(
				service.createToolSet(
					ToolDataSource.Internal,
					"dynamicToolSet",
					"dynamicToolSetRef",
			...
			assert.strictEqual(
				toolIds[0],
				"toolWithValue2",
				"Should be toolWithValue2 after context change",
			);
		});
		...
		test("ToolSet.getTools with complex when expressions", () => {
			...
			// Create a tool set and add the tools
			const complexToolSet = store.add(
				service.createToolSet(
					ToolDataSource.Internal,
					"complexToolSet",
					"complexToolSetRef",
			...
			assert.ok(
				toolIds.includes("toolWithAnd"),
				"Tool with AND should be in tool set (has(featureA) AND has(featureC) = true)",
			);
			assert.ok(
				toolIds.includes("toolWithOr"),
				"Tool with OR should be in tool set (has(featureA) OR has(featureC) = true)",
			);
			assert.ok(
				toolIds.includes("toolWithNot"),
				"Tool with NOT should be in tool set (NOT has(featureB) = true)",
			);
		});
		...
		test("ToolSet.getTools filters nested tool sets by when clause", () => {
			...
			assert.ok(
				toolIds.includes("parentTool"),
				"Parent tool should be in tool set",
			);
			assert.ok(
				toolIds.includes("childToolWithoutWhen"),
				"Child tool without when should be in tool set",
			);
			assert.ok(
				!toolIds.includes("childToolWithWhen"),
				"Child tool with when=true should NOT be in tool set when context key is false",
			);
		});
	});
	...
	suite("preToolUse hooks", () => {
		...
		test("when hook denies, tool returns error and creates cancelled invocation", async () => {
			const tool = registerToolForTest(hookService, store, "hookDenyTool", {
				invoke: async () => ({
					content: [{ kind: "text", value: "should not run" }],
				}),
			});

			const capture: { invocation?: ChatToolInvocation } = {};
			stubGetSession(hookChatService, "hook-test", {
				requestId: "req1",
				capture,
			});
			...
			assert.ok(
				(result.toolResultError as string).includes(
					"Destructive operations require approval",
				),
			);
			assert.strictEqual(result.content[0].kind, "text");
			assert.ok(
				(result.content[0] as IToolResultTextPart).value.includes(
			...
			if (state.type === IChatToolInvocation.StateKind.Cancelled) {
				...
				assert.strictEqual(
					state.reasonMessage,
					"Denied by PreToolUse hook: Destructive operations require approval",
				);
			}
		});
		...
		test("when hook allows, tool executes normally", async () => {
			...
			const capture: { invocation?: ChatToolInvocation } = {};
			stubGetSession(hookChatService, "hook-test-allow", {
				requestId: "req1",
				capture,
			});
			...
			assert.strictEqual(result.content[0].kind, "text");
			assert.strictEqual(
				(result.content[0] as IToolResultTextPart).value,
				"success",
			);
			assert.ok(!result.toolResultError);
		});
		...
		test("when hook returns undefined, tool executes normally", async () => {
			...
			stubGetSession(hookChatService, "hook-test-undefined", {
				requestId: "req1",
			});
		...
		test("when hook denies, tool invoke is never called", async () => {
			...
			stubGetSession(hookChatService, "hook-test-no-invoke", {
				requestId: "req1",
				capture,
			});

			const dto = tool.makeDto(
				{ test: 1 },
				{ sessionId: "hook-test-no-invoke" },
			);
			dto.preToolUseResult = {
				permissionDecision: "deny",
			...
			assert.strictEqual(
				invokeCalled,
				false,
				"Tool invoke should not be called when hook denies",
			);
		});
		...
		test("when hook returns ask, tool is not auto-approved", async () => {
			...
			const capture: { invocation?: ChatToolInvocation } = {};
			stubGetSession(hookChatService, "hook-test-ask", {
				requestId: "req1",
				capture,
			});
			...
			assert.strictEqual(
				invokeCompleted,
				true,
				"Tool should complete after confirmation",
			);
		});
		...
		test("when hook returns allow, tool is auto-approved", async () => {
			...
			const capture: { invocation?: ChatToolInvocation } = {};
			stubGetSession(hookChatService, "hook-test-auto-approve", {
				requestId: "req1",
				capture,
			});
			...
			const dto = tool.makeDto(
				{ test: 1 },
				{ sessionId: "hook-test-auto-approve" },
			);
			dto.preToolUseResult = {
				permissionDecision: "allow",
			};
			...
			assert.strictEqual(
				invokeCompleted,
				true,
				"Tool should complete immediately when hook allows",
			);
			assert.strictEqual(result.content[0].kind, "text");
			assert.strictEqual(
				(result.content[0] as IToolResultTextPart).value,
		...
		test("when hook returns updatedInput, tool is invoked with replaced parameters", async () => {
			...
			stubGetSession(hookChatService, "hook-test-updated-input", {
				requestId: "req1",
			});

			const dto = tool.makeDto(
				{ originalCommand: "rm -rf /" },
				{ sessionId: "hook-test-updated-input" },
			);
			dto.preToolUseResult = {
				permissionDecision: "allow",
		...
		test("when hook returns updatedInput that fails schema validation, original parameters are kept", async () => {
			...
			stubGetSession(setup.chatService, "hook-test-validation-fail", {
				requestId: "req1",
			});

			const dto = tool.makeDto(
				{ command: "original" },
				{ sessionId: "hook-test-validation-fail" },
			);
			dto.preToolUseResult = {
				permissionDecision: "allow",
		...
		test("when hook returns updatedInput that passes schema validation, parameters are replaced", async () => {
			...
			stubGetSession(setup.chatService, "hook-test-validation-pass", {
				requestId: "req1",
			});

			const dto = tool.makeDto(
				{ command: "original" },
				{ sessionId: "hook-test-validation-pass" },
			);
			dto.preToolUseResult = {
				permissionDecision: "allow",
