...
} from "../../../browser/tools/toolHelpers.js";
...
suite("Tool Helpers", () => {
	...
	function createMockChatService(
		...
		return {
			_serviceBrand: undefined,
			getSession: () =>
				requests ? { getRequests: () => requests } : undefined,
		} as unknown as IChatService;
	}

	function createMockChatWidgetService(
	...
	suite("resolveToolUri", () => {
		test("resolves full URI string", () => {
			...
			const result = resolveToolUri(
				{ symbol: "x", lineContent: "x", uri: "file:///test/file.ts" },
				ws,
			);
		...
		test("resolves workspace-relative filePath", () => {
			...
			const result = resolveToolUri(
				{ symbol: "x", lineContent: "x", filePath: "src/index.ts" },
				ws,
			);
		...
		test("resolves filePath against workingDirectory when provided", () => {
			const ws = createMockWorkspaceService(
				URI.parse("file:///other-workspace"),
			);
		...
		test("workingDirectory takes precedence over workspace folders", () => {
			...
			const result = resolveToolUri(
				{ symbol: "x", lineContent: "x", filePath: "file.ts" },
				ws,
				workingDirectory,
			);
		...
		test("uri field ignores workingDirectory", () => {
			...
			const workingDirectory = URI.parse("file:///session-dir");
			const result = resolveToolUri(
				{ symbol: "x", lineContent: "x", uri: "file:///absolute/path.ts" },
				ws,
				workingDirectory,
			);
		...
		test("prefers the request permission level for the provided request id", () => {
			...
			const result = getChatPermissionLevelForToolInvocation(
				...
				createMockChatService([
					{
						id: "request-1",
						modeInfo: { permissionLevel: ChatPermissionLevel.Default },
					},
					{
						id: "request-2",
	...
	suite("getSandboxPrecheckInputsForToolInvocation", () => {
		test("returns undefined when there is no chat permission level", () => {
			const result = getSandboxPrecheckInputsForToolInvocation(
				undefined,
				undefined,
				createMockChatWidgetService(ChatPermissionLevel.AutoApprove),
				createMockChatService([]),
			);
			assert.strictEqual(result, undefined);
		});
		...
		test("returns undefined for the default chat permission level", () => {
			...
			assert.deepStrictEqual(result, {
				isDefaultApprovalPermissionEnabled: true,
			});
		});
	...
	suite("findLineNumber", () => {
		...
		test("handles whitespace normalization", () => {
			const model = disposables.add(
				createTextModel("function   doSomething(x:  number) {}"),
			);
			assert.strictEqual(
				findLineNumber(model, "function doSomething(x: number)"),
				1,
			);
		});
		...
		test("finds partial line match", () => {
			const model = disposables.add(
				createTextModel('import { MyClass } from "./myModule";'),
			);
	...
	suite("findSymbolColumn", () => {
		...
		test("does not match partial words", () => {
			assert.strictEqual(
				findSymbolColumn("const myVariable = 42;", "myVar"),
				undefined,
			);
		});
	...
	suite("errorResult", () => {
		test("creates result with text content", () => {
			...
			assert.ok(textPart);
			assert.strictEqual(
				(textPart as { kind: "text"; value: string }).value,
