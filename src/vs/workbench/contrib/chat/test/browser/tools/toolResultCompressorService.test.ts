...
function makeService(opts: {
	...
	const config = new TestConfigurationService({
		chat: { tools: { compressOutput: { enabled: opts.enabled } } },
	});
	...
	return new ToolResultCompressorService(
		config,
		NullTelemetryService,
		opts.log ?? new NullLogService(),
	);
}
...
suite("ToolResultCompressorService", () => {
	...
	const make = (opts: { enabled: boolean; log?: ILogService }) =>
		store.add(makeService(opts));

	test("returns undefined when disabled", () => {
		const svc = make({ enabled: false });
		svc.registerFilter(replaceWithFooFilter);
		strictEqual(
			svc.maybeCompress(TOOL, {}, textResult(longText("hello"))),
			undefined,
		);
	});
	...
	test("disables a throwing filter for the rest of the pass and warns once", () => {
		...
		svc.registerFilter(replaceWithFooFilter);
		const out = svc.maybeCompress(
			TOOL,
			{},
			textResult(longText("a"), longText("b"), longText("c")),
		);
		ok(out);
	...
	test("preserves non-text parts unchanged", () => {
		const svc = make({ enabled: true });
		svc.registerFilter(replaceWithFooFilter);
		const dataPart: IToolResultDataPart = {
			kind: "data",
			value: {
				mimeType: "application/octet-stream",
				data: VSBuffer.wrap(new Uint8Array([1, 2, 3])),
			},
		};
		const result: IToolResult = {
			content: [dataPart, { kind: "text", value: longText("hello") }],
		};
	...
	test("preserves text part audience metadata when rewriting", () => {
		...
		svc.registerFilter(replaceWithFooFilter);
		const audience = [
			LanguageModelPartAudience.Assistant,
			LanguageModelPartAudience.User,
		];
		const result: IToolResult = {
			content: [{ kind: "text", value: longText("hello"), audience }],
		};
		const out = svc.maybeCompress(TOOL, {}, result);
		ok(out);
