...
suite("RetryChatAction", () => {
	...
	function createMockEditingSession(
		entriesModifiedByRequest: IModifiedFileEntry[],
	): IChatEditingSession {
		return {
			entries: observableValue("entries", entriesModifiedByRequest),
			restoreSnapshot: async (
				_requestId: string,
				_undoIndex: number | undefined,
			) => {},
	...
	function createMockWidget(
		...
		lastResponseItem?: IChatResponseViewModel,
	): Partial<IChatWidget> {
		return {
			input: {
	...
	test("retry action should not throw when using accessor synchronously", async () => {
		...
		await mockConfigService.setUserConfiguration(
		...
		const mockChatAccessibilityService =
		...
		instantiationService.set(
			IChatAccessibilityService,
			mockChatAccessibilityService,
		);

		// Get the action handler
		const commandHandler = CommandsRegistry.getCommand(
	...
	test("retry action should work with confirmation dialog (accessor used after await)", async () => {
		...
		const strictAccessor = {
			get<T>(id: ServiceIdentifier<T>): T {
				if (disposed) {
					throw new Error(
						`Accessor was used after being disposed. Tried to get service: ${id.toString()}`,
					);
				}
		...
		if (threwError) {
			assert.fail(
				`Action threw an error because accessor was used after await: ${errorMessage}`,
			);
		}
