...
function widget(
	overrides: Partial<Pick<IChatWidget, "viewModel" | "lockedAgentId">>,
): Pick<IChatWidget, "viewModel" | "lockedAgentId"> {
	return {
		viewModel: undefined,
...
suite("ChatContext", () => {
	...
	test("shows Open Editors for regular Copilot CLI sessions with eligible editors", () => {
		assert.strictEqual(
			shouldShowOpenEditorsContext(
				widgetWithSession(URI.parse("copilotcli:/session-1")),
				true,
			),
			true,
		);
	...
	test("hides Open Editors for Agent Host sessions with eligible editors", () => {
		assert.strictEqual(
			shouldShowOpenEditorsContext(
				widgetWithSession(URI.parse("agent-host-copilotcli:/session-1")),
				true,
			),
			false,
		);
	...
	test("hides Open Editors for locked Agent Host ids without a session resource", () => {
		assert.strictEqual(
			shouldShowOpenEditorsContext(
				widget({ lockedAgentId: "agent-host-copilotcli" }),
				true,
			),
			false,
		);
