...
} from "../../../browser/actions/chatExecuteActions.js";
import {
	IChatMode,
	IChatModes,
	IChatModeService,
...
async function runCommandAsync<T>(
	handler: Function,
	...args: unknown[]
): Promise<T> {
...
function createMockMode(
	overrides: Partial<IChatMode> & { id: string; kind: ChatModeKind },
): IChatMode {
	return {
...
suite("GetHandoffsAction", () => {
	...
	test("should return all modes when no sourceCustomAgent is specified", async () => {
		...
		instantiationService.set(
			IChatModeService,
			new MockChatModeService({ builtin: [askMode], custom: [planMode] }),
		);

		const handler = CommandsRegistry.getCommand(GetHandoffsActionId)?.handler;
		assert.ok(handler);

		const result = await runCommandAsync<ICustomAgentInfo[]>(
			handler,
	...
	test("should return empty array for non-matching sourceCustomAgent", async () => {
		...
		instantiationService.set(
			IChatModeService,
			new MockChatModeService({ builtin: [askMode], custom: [] }),
		);
...
suite("ExecuteHandoffAction", () => {
	...
	function createMockWidget(
		...
		chatModes: IChatModes,
	): { widget: Partial<IChatWidget>; executeHandoffCalls: IHandOff[] } {
	...
	test("should return error when neither id nor label is provided", async () => {
		const handler = CommandsRegistry.getCommand(
			ExecuteHandoffActionId,
		)?.handler;
		assert.ok(handler);

		const result = await runCommandAsync<IExecuteHandoffResult>(
			handler,
			instantiationService,
			{},
		);
	...
	test("should fall back to lastFocusedWidget when sessionResource is omitted", async () => {
		...
		const { widget, executeHandoffCalls } = createMockWidget(
			planMode,
	...
	test("should resolve widget by sessionResource", async () => {
		...
		const mockWidgetService = new (class extends MockChatWidgetService {
			override getWidgetBySessionResource(resource: URI) {
				return resource.toString() === sessionUri.toString()
					? (widget as IChatWidget)
					: undefined;
			}
		})();
	...
	test("should resolve sourceCustomAgent to look up handoffs from a different mode", async () => {
		...
		const modeService = new MockChatModeService({
			builtin: [askMode],
			custom: [planMode],
		});
		const { widget, executeHandoffCalls } = createMockWidget(
		...
		); // widget is in "ask" mode (no handoffs)
