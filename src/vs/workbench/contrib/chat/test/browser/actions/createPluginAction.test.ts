...
function makeResourceItem(
	overrides: Partial<IResourceTreeItem> &
		Pick<IResourceTreeItem, "label" | "resourceType">,
): IResourceTreeItem {
...
suite("CreatePluginAction helpers", () => {
	...
	suite("serializeHookCommand", () => {
		test("serializes basic command", () => {
			assert.deepStrictEqual(
				serializeHookCommand({ type: "command", command: "echo hello" }),
				{
					type: "command",
					command: "echo hello",
				},
		...
		test("omits empty env", () => {
			const result = serializeHookCommand({
				type: "command",
				command: "test",
				env: {},
			});
...
suite("writePluginToDisk", () => {
	...
	test("creates manifest with correct structure", async () => {
		...
		assert.deepStrictEqual(
			await readJson(URI.joinPath(pluginRoot, ".plugin", "plugin.json")),
			{
				name: "my-plugin",
				version: "1.0.0",
				description: "",
	...
	test("copies instructions to rules/", async () => {
		...
		await fileService.writeFile(
			sourceUri,
			VSBuffer.fromString("# My coding rules"),
		);
		...
		const content = await fileService.readFile(
			URI.joinPath(pluginRoot, "rules", "coding.instructions.md"),
		);
	...
	test("preserves .mdc suffix for rule files", async () => {
		...
		const content = await fileService.readFile(
			URI.joinPath(pluginRoot, "rules", "prefer-const.mdc"),
		);
	...
	test("copies prompts to commands/", async () => {
		...
		await fileService.writeFile(
			sourceUri,
			VSBuffer.fromString("Review this code"),
		);
		...
		const content = await fileService.readFile(
			URI.joinPath(pluginRoot, "commands", "review.md"),
		);
	...
	test("copies agents to agents/", async () => {
		...
		const content = await fileService.readFile(
			URI.joinPath(pluginRoot, "agents", "reviewer.md"),
		);
		assert.strictEqual(
	...
	test("copies skill directories recursively", async () => {
		...
		await fileService.writeFile(
			URI.joinPath(skillDir, "SKILL.md"),
			VSBuffer.fromString("# My Skill"),
		);
		await fileService.writeFile(
			URI.joinPath(skillDir, "helper.md"),
			VSBuffer.fromString("helper content"),
		);
		...
		const skillMd = await fileService.readFile(
			URI.joinPath(pluginRoot, "skills", "my-skill", "SKILL.md"),
		);
		assert.strictEqual(skillMd.value.toString(), "# My Skill");
		const helperMd = await fileService.readFile(
			URI.joinPath(pluginRoot, "skills", "my-skill", "helper.md"),
		);
	...
	test("merges hooks into hooks/hooks.json", async () => {
		...
		assert.deepStrictEqual(
			await readJson(URI.joinPath(pluginRoot, "hooks", "hooks.json")),
			{
				hooks: {
					SessionStart: [{ type: "command", command: "echo start" }],
					PreToolUse: [{ type: "command", command: "echo pre" }],
		...
		await writePluginToDisk(fileService, pluginRoot, "test-plugin", [
			makeResourceItem({
				...
				mcpServer: {
					...
					} as IResourceTreeItem["mcpServer"] extends undefined
						? never
						: NonNullable<IResourceTreeItem["mcpServer"]>["collection"],
					definition: {
						id: "def1",
						label: "my-server",
						launch: {
						...
						? never
						: NonNullable<IResourceTreeItem["mcpServer"]>["definition"],
				},
			}),
		]);

		assert.deepStrictEqual(
			await readJson(URI.joinPath(pluginRoot, ".mcp.json")),
			{
				mcpServers: {
					"my-server": {
						type: "stdio",
	...
	test("strips namespace prefix from plugin resource names", async () => {
		...
		const content = await fileService.readFile(
			URI.joinPath(pluginRoot, "rules", "writing-rules.instructions.md"),
		);
	...
	test("does not create directories for empty resource types", async () => {
		...
		assert.ok(
			await fileService.exists(
				URI.joinPath(pluginRoot, ".plugin", "plugin.json"),
			),
		);
		assert.ok(!(await fileService.exists(URI.joinPath(pluginRoot, "rules"))));
		assert.ok(
			!(await fileService.exists(URI.joinPath(pluginRoot, "commands"))),
		);
		...
		assert.ok(
			!(await fileService.exists(URI.joinPath(pluginRoot, ".mcp.json"))),
		);
	});
});
...
suite("updateMarketplaceIfNeeded", () => {
	...
	test("adds plugin to existing marketplace.json", async () => {
		...
		await fileService.writeFile(
			URI.joinPath(root, "marketplace.json"),
			VSBuffer.fromString(JSON.stringify(marketplace)),
		);
	...
	test("detects .plugin/marketplace.json", async () => {
		...
		await fileService.writeFile(
			URI.joinPath(root, ".plugin", "marketplace.json"),
			VSBuffer.fromString(JSON.stringify(marketplace)),
		);
	...
	test("does nothing when no marketplace.json exists", async () => {
		await updateMarketplaceIfNeeded(fileService, root, "my-plugin");
		assert.ok(
			!(await fileService.exists(URI.joinPath(root, "marketplace.json"))),
		);
	});
