...
suite("Queue/Steer keybinding resolution", () => {
	...
	function buildResolverForCommands(commandIds: string[]): KeybindingResolver {
		...
		for (const item of KeybindingsRegistry.getDefaultKeybindingsForOS(OS)) {
			...
			const resolved = USLayoutResolvedKeybinding.resolveKeybinding(
				item.keybinding,
				OS,
			)[0];
			items.push(
	...
	function lookupForConfig(defaultAction: "steer" | "queue") {
		const config = new TestConfigurationService({
			[ChatConfiguration.RequestQueueingDefaultAction]: defaultAction,
		});
		...
		const resolver = buildResolverForCommands([
		...
		return {
			result: {
				queue:
					resolver
						.lookupPrimaryKeybinding(ChatQueueMessageAction.ID, overlay, true)
						?.resolvedKeybinding?.getDispatchChords()[0] ?? null,
				steer:
					resolver
