...
} from "../../browser/defaultModelContribution.js";
import {
	UtilityModelContribution,
	UtilitySmallModelContribution,
} from "../../browser/utilityModelContribution.js";
import {
...
class TestLanguageModelsService implements Partial<ILanguageModelsService> {
	...
	addModel(metadata: ILanguageModelChatMetadata): void {
		...
		this._models.set(
			`${this._modelCounter++}:${metadata.vendor}/${metadata.id}`,
			metadata,
		);
	}
...
function makeMetadata(
	overrides: Partial<ILanguageModelChatMetadata> &
		Pick<ILanguageModelChatMetadata, "id" | "name" | "vendor">,
): ILanguageModelChatMetadata {
	return {
		version: "1.0",
		family: "test",
...
suite("DefaultModelContribution", () => {
	...
	function setup(opts?: {
		...
		vendors?: ILanguageModelProviderDescriptor[];
		storageFormat?: "qualifiedName" | "vendorAndId";
	}) {
		...
		const contribution = store.add(
			new TestContribution(
				arrays,
				opts?.storageFormat,
				service as unknown as ILanguageModelsService,
			),
		);
	...
	test("utility model settings exclude Copilot vendor models", () => {
		...
		store.add(
			new UtilityModelContribution(
				service as unknown as ILanguageModelsService,
				new NullLogService(),
			),
		);
		store.add(
			new UtilitySmallModelContribution(
				service as unknown as ILanguageModelsService,
				new NullLogService(),
			),
		);
		...
		assert.deepStrictEqual(
			{
				utility: {
					ids: UtilityModelContribution.modelIds,
					labels: UtilityModelContribution.modelLabels,
				},
				utilitySmall: {
					ids: UtilitySmallModelContribution.modelIds,
					labels: UtilitySmallModelContribution.modelLabels,
				},
			},
			{
				utility: {
					ids: ["", "anthropic/claude-haiku-4.5"],
					labels: ["Default", "Claude Haiku 4.5 (Anthropic)"],
				},
				utilitySmall: {
					ids: ["", "anthropic/claude-haiku-4.5"],
					labels: ["Default", "Claude Haiku 4.5 (Anthropic)"],
