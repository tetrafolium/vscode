...
suite("AgentPluginRepositoryService", () => {
	...
	function stubPluginGit(
		overrides?: Partial<IPluginGitService>,
	): IPluginGitService {
		return {
	...
	function createService(
		...
		instantiationService.stub(IUserDataProfileService, {
			currentProfile: { agentPluginsHome: URI.file("/cache/agentPlugins") },
	...
	test("uses cacheSegments path for GitHub shorthand plugin references", () => {
		...
		const uri = service.getRepositoryUri(
	...
	test("uses ref-specific cache path for GitHub shorthand plugin references", () => {
		...
		assert.strictEqual(
			uri.path,
			"/cache/agentPlugins/github.com/microsoft/vscode/ref_marketplace",
		);
	});
	...
	test("uses same cache path for equivalent GitHub shorthand and URI references", () => {
		const service = createService();
		const shorthandPlugin = createPlugin(
			"microsoft/vscode",
			"plugins/myPlugin",
		);
		const uriPlugin = createPlugin(
			"https://github.com/microsoft/vscode.git",
			"plugins/myPlugin",
		);

		const shorthandUri = service.getRepositoryUri(
			shorthandPlugin.marketplaceReference,
			shorthandPlugin.marketplaceType,
		);
		const uriRefUri = service.getRepositoryUri(
			uriPlugin.marketplaceReference,
			uriPlugin.marketplaceType,
		);
	...
	test("ensures plugin repositories via cacheSegments path", async () => {
		...
		const plugin = createPlugin("microsoft/vscode", "plugins/myPlugin");
		const uri = await service.ensureRepository(plugin.marketplaceReference, {
	...
	test("concurrent ensureRepository calls for the same marketplace clone only once", async () => {
		...
		const [uri1, uri2] = await Promise.all([
			service.ensureRepository(plugin.marketplaceReference, {
				marketplaceType: plugin.marketplaceType,
			}),
			service.ensureRepository(plugin.marketplaceReference, {
				marketplaceType: plugin.marketplaceType,
	...
	test("uses indexed repository URI when available", () => {
		...
		storage.store(
			"chat.plugins.marketplaces.index.v1",
			JSON.stringify({
				"github:microsoft/vscode": {
					repositoryUri: URI.file(
						"/cache/agentPlugins/indexed/microsoft/vscode",
					),
					marketplaceType: MarketplaceType.Copilot,
				},
			}),
	...
	test("builds revision-aware install URI for github plugin sources", () => {
		...
		assert.strictEqual(
			uri.path,
			"/cache/agentPlugins/github.com/owner/repo/ref_release_v1",
		);
	});
	...
	suite("cleanupPluginSource", () => {
		function createServiceWithDel(
			...
			instantiationService.stub(IFileService, {
				...
				resolve: async (resource: URI) =>
					options?.resolve?.(resource) ?? { children: [] },
			} as unknown as IFileService);
			instantiationService.stub(ILogService, new NullLogService());
			instantiationService.stub(INotificationService, {
				notify: () => undefined,
		...
		test("deletes parent cache dir for npm plugin source", async () => {
			...
			assert.ok(
				deleted[0].includes("/npm/"),
				`Expected npm path, got: ${deleted[0]}`,
			);
		});
		...
		test("prunes empty parent directories up to cache root", async () => {
			...
			assert.ok(
				deleted.length >= 2,
				`Expected at least 2 deletions (repo + parent), got ${deleted.length}: ${deleted.join(", ")}`,
			);
			assert.ok(
				deleted[0].includes("github.com/owner/repo"),
				"First delete should be the repo dir",
			);
			assert.ok(
				deleted.some((p) => p.endsWith("/owner")),
				"Should prune empty owner directory",
			);
		});
