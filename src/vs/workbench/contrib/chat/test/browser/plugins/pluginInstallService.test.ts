...
suite("PluginInstallService", () => {
	...
	function createPlugin(
		overrides: Partial<IMarketplacePlugin> & {
			sourceDescriptor: IPluginSourceDescriptor;
		},
	): IMarketplacePlugin {
		return {
			...
			marketplaceReference:
				overrides.marketplaceReference ??
				makeMarketplaceRef("microsoft/vscode"),
	...
	function createDefaults(): MockState {
		return {
			...
			ensureRepositoryResult: URI.file(
	...
	function createService(stateOverrides?: Partial<MockState>): {
		...
		instantiationService.stub(ITerminalService, {
			createTerminal: async () => {
				let finishedCallback:
					| ((cmd: { id: string; exitCode: number }) => void)
					| undefined;
				return {
					...
					runCommand: (command: string, _addNewLine?: boolean) => {
						...
						if (finishedCallback) {
							finishedCallback({
								id: "command",
								exitCode: state.terminalExitCode,
							});
						}
					},
					capabilities: {
						get: () =>
							state.terminalCompletes
								? {
										onCommandFinished: (
											callback: (cmd: { id: string; exitCode: number }) => void,
										) => {
		...
		const makeMockPackageRepo = (kind: PluginSourceKind): IPluginSource => ({
			...
			runInstall: async (
				...
				if (state.terminalExitCode !== 0) {
					state.notifications.push({
						severity: 3,
						message: `Plugin installation command failed: Command exited with code ${state.terminalExitCode}`,
					});
					return undefined;
				}
				...
				const exists =
					typeof state.fileExistsResult === "function"
				...
				if (!exists) {
					...
					state.notifications.push({
						severity: 3,
						message: `${label} package '${pkg}' was not found after installation.`,
					});
					return undefined;
				}
		...
		const mockSourceRepos = new Map<PluginSourceKind, IPluginSource>([
			[
				PluginSourceKind.RelativePath,
				{
					kind: PluginSourceKind.RelativePath,
					getCleanupTarget: () => undefined,
					getInstallUri: () => {
						throw new Error();
					},
					ensure: async () => {
					...
					getLabel: (d) => (d as { path: string }).path || ".",
				},
			],
			[
				PluginSourceKind.GitHub,
				{
					...
					getCleanupTarget: () => URI.file("/mock"),
					getInstallUri: () => URI.file("/mock"),
					ensure: async () => URI.file("/mock"),
					update: async () => true,
					getLabel: (d) => (d as { repo: string }).repo,
				},
			],
			[
				PluginSourceKind.GitUrl,
				{
					...
					update: async () => true,
					getLabel: (d) => (d as { url: string }).url,
				},
			],
		...
		instantiationService.stub(IAgentPluginRepositoryService, {
			getPluginInstallUri: (plugin: IMarketplacePlugin) => {
				if (plugin.sourceDescriptor.kind !== PluginSourceKind.RelativePath) {
					return (
						state.pluginSourceInstallUris.get(plugin.sourceDescriptor.kind) ??
						URI.file(
							`/cache/agentPlugins/${plugin.sourceDescriptor.kind}/default`,
						)
					);
				}
			...
			ensureRepository: async (
				_marketplace: IMarketplaceReference,
			...
			getPluginSourceInstallUri: (descriptor: IPluginSourceDescriptor) => {
				...
				return (
					state.pluginSourceInstallUris.get(key) ??
					URI.file(`/cache/agentPlugins/${key}/default`)
				);
			},
			ensurePluginSource: async () => state.ensurePluginSourceResult,
			updatePluginSource: async (
				plugin: IMarketplacePlugin,
				options?: IPullRepositoryOptions,
			) => {
		...
		instantiationService.stub(IConfigurationService, {
			...
			inspect: (key: string) => {
				if (key === ChatConfiguration.PluginMarketplaces) {
					return {
						userValue: state.configuredMarketplaces,
	...
	suite("getPluginInstallUri", () => {
		test("delegates to getPluginInstallUri for relative-path plugins", () => {
			...
			assert.strictEqual(
				uri.path,
				"/cache/agentPlugins/github.com/microsoft/vscode/plugins/myPlugin",
			);
		});

		test("delegates to getPluginSourceInstallUri for npm plugins", () => {
			const npmUri = URI.file(
				"/cache/agentPlugins/npm/my-pkg/node_modules/my-pkg",
			);
			const { service } = createService({
	...
	suite("installPlugin — relative path", () => {
		...
		test("does not install when ensureRepository throws", async () => {
			...
			instantiationService.stub(
				IAgentPluginRepositoryService,
				repoService as unknown as IAgentPluginRepositoryService,
			);
			instantiationService.stub(IFileService, {
				exists: async () => true,
			} as unknown as IFileService);
			instantiationService.stub(INotificationService, {
				notify: (n: { severity: number; message: string }) => {
			...
			instantiationService.stub(IProgressService, {
				withProgress: async (_o: unknown, cb: () => Promise<unknown>) => cb(),
			} as unknown as IProgressService);
			instantiationService.stub(ILogService, new NullLogService());
			instantiationService.stub(IPluginMarketplaceService, {
				addInstalledPlugin: () => {},
	...
	suite("installPlugin — git sources", () => {
		test("installs a GitHub plugin when source exists after clone", async () => {
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/repo",
				),
			});
			const plugin = createPlugin({
				sourceDescriptor: { kind: PluginSourceKind.GitHub, repo: "owner/repo" },
		...
		test("installs a GitUrl plugin when source exists after clone", async () => {
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/example.com/repo",
				),
			});
			const plugin = createPlugin({
				sourceDescriptor: {
	...
	suite("installPlugin — npm", () => {
		...
		test("includes version in npm install command", async () => {
			const { service, state } = createService({
				...
				pluginSourceInstallUris: new Map([
					[
						"npm",
						URI.file("/cache/agentPlugins/npm/my-pkg/node_modules/my-pkg"),
					],
				]),
			});
			const plugin = createPlugin({
		...
		test("includes registry in npm install command", async () => {
			...
			assert.ok(
				state.terminalCommands[0].includes("https://custom.registry.com"),
			);
		});
	...
	suite("installPlugin — pip", () => {
		...
		test("includes version with == syntax in pip install command", async () => {
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file("/cache/agentPlugins/pip/my-pkg"),
				pluginSourceInstallUris: new Map([
					["pip", URI.file("/cache/agentPlugins/pip/my-pkg")],
				]),
			});
			const plugin = createPlugin({
				sourceDescriptor: {
		...
		test("includes registry with --index-url in pip install command", async () => {
			...
			const plugin = createPlugin({
				sourceDescriptor: {
					kind: PluginSourceKind.Pip,
					package: "my-pkg",
					registry: "https://pypi.custom.com/simple",
				},
			});
			...
			assert.ok(
				state.terminalCommands[0].includes("https://pypi.custom.com/simple"),
			);
		});
	...
	suite("installPlugin — marketplace trust", () => {
		test("skips trust prompt when marketplace is already trusted", async () => {
			...
			assert.strictEqual(state.addedPlugins.length, 1);
			assert.strictEqual(
				state.trustedMarketplaces.length,
				0,
				"should not re-trust",
			);
		});
		...
		test("does not install when user declines trust", async () => {
			...
			await assert.rejects(
				...
				(err: unknown) => isCancellationError(err as Error),
			);
		...
		test("trust prompt applies to all source kinds", async () => {
			...
			for (const sourceDescriptor of kinds) {
				await assert.rejects(
					() => service.installPlugin(createPlugin({ sourceDescriptor })),
					(err: unknown) => isCancellationError(err as Error),
				);
			}

			assert.strictEqual(
				state.addedPlugins.length,
				0,
				"no plugins should be installed when trust is declined",
			);
		});
	});
	...
	suite("installPluginFromSource", () => {
		...
		test("installs single plugin from GitHub shorthand with marketplace.json", async () => {
			...
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/my-plugin",
				),
			...
			assert.strictEqual(state.addedPlugins.length, 1);
			assert.strictEqual(
				state.addedPlugins[0].plugin.name,
				"my-discovered-plugin",
			);
		});
		...
		test("shows error when no marketplace.json found", async () => {
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/cool-tool",
				),
				readPluginsResult: [],
			});
		...
		test("shows quick pick for multi-plugin repos", async () => {
			...
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/multi-repo",
				),
		...
		test("shows error when no plugins found in git URL", async () => {
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/my-tool",
				),
				readPluginsResult: [],
			});

			await service.installPluginFromSource(
				"https://github.com/owner/my-tool.git",
			);
		...
		test("shows error when clone directory does not exist", async () => {
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/missing",
				),
				fileExistsResult: false,
			});
		...
		test("falls back to single-plugin manifest when no marketplace.json exists", async () => {
			...
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/single-plugin-repo",
				),
				readPluginsResult: [],
		...
		test("reports error when single-plugin manifest name does not match options.plugin", async () => {
			...
			const result = await service.installPluginFromValidatedSource(
				"owner/single-plugin-repo",
		...
		test('still reports "no plugins found" when neither marketplace.json nor single-plugin manifest exists', async () => {
			const { service, state } = createService({
				ensurePluginSourceResult: URI.file(
					"/cache/agentPlugins/github.com/owner/empty-repo",
				),
				readPluginsResult: [],
				singlePluginManifestResult: undefined,
			});
