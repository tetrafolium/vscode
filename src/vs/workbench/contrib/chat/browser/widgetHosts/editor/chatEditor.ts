...
export class ChatEditor extends AbstractEditorWithViewState<IChatEditorViewState> {
	...
	protected override createEditor(parent: HTMLElement): void {
		...
		ChatContextKeys.inChatEditor
			.bindTo(this._scopedContextKeyService)
			.set(true);
		...
		this._register(
			this.widget.onDidChangeViewModel((e) => {
				if (e.currentSessionResource && this.input instanceof ChatEditorInput) {
					const newModel = this.chatService.getSession(
	...
	private showLoadingInChatWidget(message: string): void {
		...
		if (this._loadingContainer) {
			...
			const existingText = this._loadingContainer.querySelector(
				".chat-loading-content span",
			);
			if (existingText) {
		...
		this._loadingContainer = dom.append(
			this._editorContainer,
			dom.$(".chat-loading-overlay"),
		);
		...
		const loadingContent = dom.append(
			this._loadingContainer,
			dom.$(".chat-loading-content"),
		);
	...
	override async setInput(
		...
		if (chatSessionType !== localChatSessionType) {
			const loadingMessage = nls.localize(
		...
		if (chatSessionType !== localChatSessionType) {
			try {
				await raceCancellationError(
					this.chatSessionsService.canResolveChatSession(chatSessionType),
					token,
				);
				const contributions =
		...
		try {
			...
			if (!editorModel) {
				throw new Error(
					`Failed to get model for chat editor. resource: ${input.sessionResource}`,
				);
			}
			...
			if (
				isContributedChatSession &&
				options?.title?.preferred &&
	...
	protected computeEditorViewState(
		_resource: URI,
	): IChatEditorViewState | undefined {
		if (!this._widget) {
			return undefined;
		}
