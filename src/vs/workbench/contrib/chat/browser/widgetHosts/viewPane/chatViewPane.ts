...
} from "../../agentSessions/agentSessionsFilter.js";
...
export class ChatViewPane extends ViewPane implements IViewWelcomeDelegate {
	...
	private readonly lastDimensionsPerOrientation: Map<
		AgentSessionsViewerOrientation,
	...
	constructor(
		...
		this.sessionsViewerSidebarWidth = Math.max(
			...
			this.viewState.sessionsSidebarWidth ??
				ChatViewPane.SESSIONS_SIDEBAR_DEFAULT_WIDTH,
		);

		// Contextkeys
		this.chatViewLocationContext =
			ChatContextKeys.panelLocation.bindTo(contextKeyService);
		this.sessionsViewerOrientationContext =
			ChatContextKeys.agentSessionsViewerOrientation.bindTo(contextKeyService);
		this.sessionsViewerPositionContext =
			ChatContextKeys.agentSessionsViewerPosition.bindTo(contextKeyService);
		this.sessionsViewerVisibilityContext =
			ChatContextKeys.agentSessionsViewerVisible.bindTo(contextKeyService);
	...
	private updateContextKeys(): void {
		...
		this.sessionsViewerPositionContext.set(
			position === Position.RIGHT
				? AgentSessionsViewerPosition.Right
				: AgentSessionsViewerPosition.Left,
		);
	}
	...
	private getSessionHoverPosition() {
		...
		if (
			this.sessionsViewerOrientation ===
			AgentSessionsViewerOrientation.SideBySide
		) {
			return viewLocation === ViewContainerLocation.Sidebar &&
		...
		return {
			...
			viewLocation === ViewContainerLocation.Panel
				? this.layoutService.getPanelPosition()
				: sideBarPosition
		];
	}
	...
	private createControls(parent: HTMLElement): void {
		...
		// Controls Listeners
		this.registerControlsListeners(
			sessionsControl,
			chatWidget,
			welcomeController,
		);
	...
	private static readonly SESSIONS_SIDEBAR_SNAP_THRESHOLD =
		this.SESSIONS_SIDEBAR_MIN_WIDTH / 2; // snap to hide when dragged below half of minimum width
	...
	private static readonly SESSIONS_SIDEBAR_VIEW_MIN_WIDTH =
		this.CHAT_WIDGET_DEFAULT_WIDTH + this.SESSIONS_SIDEBAR_DEFAULT_WIDTH;
	...
	private sessionsViewerOrientationConfiguration: "stacked" | "sideBySide" =
		"sideBySide";
	...
	private readonly sessionsViewerSashDisposables = this._register(
		new MutableDisposable<DisposableStore>(),
	);

	private createSessionsControl(parent: HTMLElement): AgentSessionsControl {
		const sessionsContainer = (this.sessionsContainer = parent.appendChild(
			$(".agent-sessions-container"),
		));

		// Sessions Title
		const sessionsTitleContainer = (this.sessionsTitleContainer = append(
			sessionsContainer,
			$(".agent-sessions-title-container"),
		));
		const sessionsTitle = (this.sessionsTitle = append(
			sessionsTitleContainer,
			$("span.agent-sessions-title"),
		));
		sessionsTitle.textContent = localize("sessions", "Sessions");
		this._register(
			addDisposableListener(sessionsTitle, EventType.CLICK, () => {
		...
		// Sessions Toolbar
		const sessionsToolbarContainer = append(
			sessionsTitleContainer,
			$(".agent-sessions-toolbar"),
		);
		const sessionsToolbar = this._register(
			this.instantiationService.createInstance(
		...
		// Sessions Filter
		const sessionsFilter = this._register(
			this.instantiationService.createInstance(AgentSessionsFilter, {
				filterMenuId: MenuId.AgentSessionsViewerFilterSubMenu,
				groupResults: () =>
					this.sessionsViewerOrientation ===
					AgentSessionsViewerOrientation.Stacked
						? AgentSessionsGrouping.Capped
						: AgentSessionsGrouping.Date,
			}),
		);
		this._register(
			Event.runAndSubscribe(sessionsFilter.onDidChange, () => {
				sessionsToolbarContainer.classList.toggle(
					"filtered",
					!sessionsFilter.isDefault(),
				);
			}),
		);

		// New Session Button
		const newSessionButtonContainer = (this.sessionsNewButtonContainer = append(
			sessionsContainer,
			$(".agent-sessions-new-button-container"),
		));
		const newSessionButton = this._register(
			new Button(newSessionButtonContainer, {
				...defaultButtonStyles,
				secondary: true,
			}),
		);
				...
				this.sessionsControlContainer,
				{
					...
					overrideSessionOpenOptions: (openEvent) => {
						if (
							this.sessionsViewerOrientation ===
								AgentSessionsViewerOrientation.Stacked &&
							!openEvent.sideBySide
						) {
							return {
								...openEvent,
								editorOptions: {
									...openEvent.editorOptions,
									preserveFocus: false /* focus the chat widget when opening from stacked sessions viewer since this closes the stacked viewer */,
								},
							};
						}
						return openEvent;
		...
		this._register(
			Event.runAndSubscribe(
				Event.filter(this.configurationService.onDidChangeConfiguration, (e) =>
					e.affectsConfiguration(ChatConfiguration.ChatViewSessionsOrientation),
				),
				(e) => {
					const newSessionsViewerOrientationConfiguration =
						this.configurationService.getValue<
							"stacked" | "sideBySide" | unknown
						>(ChatConfiguration.ChatViewSessionsOrientation);
					this.doUpdateConfiguredSessionsViewerOrientation(
						newSessionsViewerOrientationConfiguration,
						{ updateConfiguration: false, layout: !!e },
					);
				},
			),
		);
	...
	updateConfiguredSessionsViewerOrientation(
		orientation: "stacked" | "sideBySide" | unknown,
	): void {
	...
	private doUpdateConfiguredSessionsViewerOrientation(
		orientation: "stacked" | "sideBySide" | unknown,
		options: { updateConfiguration: boolean; layout: boolean },
	): void {
		const oldSessionsViewerOrientationConfiguration =
		...
		if (
			oldSessionsViewerOrientationConfiguration ===
			this.sessionsViewerOrientationConfiguration
		) {
		...
		if (
			...
			if (
				...
				newSessionsContainerVisible =
					(!!this.chatEntitlementService.sentiment.completed ||
						this.chatEntitlementService.hasByokModels) && // chat is setup (otherwise make room for terms and welcome)
					(!this._widget ||
						(this._widget.isEmpty() &&
							!!this._widget.viewModel &&
							!this._widget.viewModel.model.title)) && // chat widget empty (but not when model is loading or has a title)
			...
			// Sessions control: sidebar
				...
				newSessionsContainerVisible =
					!this.welcomeController?.isShowingWelcome.get() && // welcome not showing
					!!this.lastDimensions &&
					this.lastDimensions.width >=
						ChatViewPane.SESSIONS_SIDEBAR_VIEW_MIN_WIDTH; // has sessions or is showing all sessions
			}
		}

		this.viewPaneContainer.classList.toggle(
			"has-sessions-control",
			newSessionsContainerVisible,
		);
	...
	private createChatTitleControl(parent: HTMLElement): void {
		this.titleControl = this._register(
			this.instantiationService.createInstance(ChatViewTitleControl, parent, {
	...
	private registerControlsListeners(
		...
		welcomeController: ChatViewWelcomeController,
	): void {
		...
		this._register(
			Event.any(
				...
				Event.filter(this.contextKeyService.onDidChangeContext, (e) =>
					e.affectsSome(hasByokModelsContextKeys),
				),
			)(() => {
				if (
					this.sessionsViewerOrientation ===
	...
	private async _applyModel(): Promise<void> {
		const sessionResource = this.getTransferredOrPersistedSessionInfo();
		const modelRef = sessionResource
	...
	private doLayoutBody(height: number, width: number): void {
		...
		// Sessions Control
		const { heightReduction, widthReduction } = this.layoutSessionsControl(
			remainingHeight,
			remainingWidth,
		);
		...
		this._widget.setInputPartMaxHeightOverride(
	...
	private layoutSessionsControl(
		height: number,
		width: number,
	): { heightReduction: number; widthReduction: number } {
		...
		if (
			...
			!this.sessionsControlContainer ||
			!this.sessionsControl ||
			!this.viewPaneContainer ||
		...
		if (
			...
			this.viewPaneContainer.classList.toggle(
				"sessions-control-orientation-sidebyside",
				true,
			);
			this.viewPaneContainer.classList.toggle(
			...
			this.sessionsViewerOrientationContext.set(
				AgentSessionsViewerOrientation.SideBySide,
			);
		} else {
			this.viewPaneContainer.classList.toggle(
		...
		const { visible: sessionsContainerVisible } =
			this.updateSessionsControlVisibility();

		// Handle Sash (only visible in side-by-side)
		if (
			!sessionsContainerVisible ||
			this.sessionsViewerOrientation === AgentSessionsViewerOrientation.Stacked
		) {
		...
		let availableSessionsHeight =
			height - this.sessionsTitleContainer.offsetHeight;
		if (
			this.sessionsViewerOrientation === AgentSessionsViewerOrientation.Stacked
		) {
			...
			availableSessionsHeight -=
				this.sessionsNewButtonContainer?.offsetHeight ?? 0;
		}
		...
		if (
			...
			const sessionsViewerSidebarWidth =
				this.computeEffectiveSideBySideSessionsSidebarWidth(width);
	...
	private computeEffectiveSideBySideSessionsSidebarWidth(
		width: number,
		sessionsViewerSidebarWidth = this.sessionsViewerSidebarWidth,
	): number {
		return Math.max(
	...
	getLastDimensions(
		orientation: AgentSessionsViewerOrientation,
	...
	private createSessionsViewerSash(
		...
		const disposables = (this.sessionsViewerSashDisposables.value =
			new DisposableStore());

		const sash = (this.sessionsViewerSash = disposables.add(
			new Sash(
				container,
				{
					getVerticalSashLeft: () => {
						const sessionsViewerSidebarWidth =
							this.computeEffectiveSideBySideSessionsSidebarWidth(
		...
		disposables.add(
			sash.onDidStart(() => (sashStartWidth = this.sessionsViewerSidebarWidth)),
		);
		...
		disposables.add(
			sash.onDidChange((e) => {
				...
				this.sessionsViewerSidebarWidth =
					this.computeEffectiveSideBySideSessionsSidebarWidth(
						this.lastDimensions.width,
						newWidth,
					);
