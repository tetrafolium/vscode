...
export class ChatContextUsageDetails extends Disposable {
	...
	constructor(
		...
		const updateActionsVisibility = () => {
			...
			const hasActions =
				actions.length > 0 && actions.some(([, items]) => items.length > 0);
	...
	private renderTokenDetails(
		details: readonly IChatContextUsagePromptTokenDetail[] | undefined,
		...
		const categoryMap = new Map<
			string,
			{ label: string; percentageOfPrompt: number }[]
		>();
		...
		for (const [category, items] of categoryMap) {
			...
			const visibleItems = items.filter((item) => {
				const contextRelativePercentage =
					(item.percentageOfPrompt / 100) * contextWindowPercentage;
