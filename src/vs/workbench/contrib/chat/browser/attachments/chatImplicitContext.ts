	...
	implements IWorkbenchContribution
{
		...
		this._register(
			Event.runAndSubscribe(editorService.onDidActiveEditorChange, () => {
				...
				if (webviewEditor) {
					activeEditorDisposables.add(
						Event.debounce(
							(webviewEditor.input as WebviewInput).webview.onMessage,
							() => undefined,
							500,
						)(() => {
							this.updateImplicitContext();
	...
	private async updateImplicitContext(
		...
		const useSuggestedContext = this.configurationService.getValue<boolean>(
		...
		if (model) {
			...
			providerContext = await this.chatContextService.contextForResource(
				model.uri,
				languageId,
			);
		}
		...
		if (webviewEditor?.input?.resource) {
			const webviewContext = await this.chatContextService.contextForResource(
				webviewEditor.input.resource,
			);
			if (webviewContext) {
		...
		const uri =
			newValue instanceof URI
				? newValue
				: isStringImplicitContextValue(newValue)
					? undefined
					: newValue?.uri;
		if (
			uri &&
		...
		for (const widget of widgets) {
			...
			if (setting === "always" || (setting === "first" && isFirstInteraction)) {
				...
				if (
					newValue !== undefined ||
					!widget.input.implicitContext.hasValue ||
					!hasActiveEditor
				) {
					widget.input.implicitContext.setValues([
						{ value: newValue, isSelection },
						{ value: providerContext, isSelection: false },
					]);
				}
			} else {
				widget.input.implicitContext.setValues([]);
...
function isEntireCellVisible(
	cellModel: ITextModel,
	visibleRanges: IRange[],
): boolean {
	if (
		visibleRanges.length === 1 &&
		visibleRanges[0].startLineNumber === 1 &&
		visibleRanges[0].startColumn === 1 &&
		visibleRanges[0].endLineNumber === cellModel.getLineCount() &&
		visibleRanges[0].endColumn ===
			cellModel.getLineMaxColumn(visibleRanges[0].endLineNumber)
	) {
		return true;
	}
	return false;
...
export class ChatImplicitContexts extends Disposable {
	...
	private _values: DisposableMap<ChatImplicitContext, DisposableStore> =
		this._register(new DisposableMap());
	private readonly _valuesDisposables: DisposableStore = this._register(
		new DisposableStore(),
	);
	...
	get hasNonUri(): boolean {
		return this.values.some(
			(v) => v.value !== undefined && !URI.isUri(v.value),
		);
	}

	getLocations(): Location[] {
		return this.values
			.filter((v) => isLocation(v.value))
			.map((v) => v.value as Location);
	}

	getUris(): URI[] {
		return this.values
			.filter((v) => URI.isUri(v.value))
			.map((v) => v.value as URI);
	}

	get hasNonStringContext(): boolean {
		return this.values.some(
			(v) => v.value !== undefined && !isStringImplicitContextValue(v.value),
		);
	}
	...
	implements IChatRequestImplicitVariableEntry
{
	...
	get name(): string {
		...
		if (isStringImplicitContextValue(this.value)) {
			if (
				...
				this.value.resourceUri === undefined
			) {
				throw new Error(
	...
	get modelDescription(): string {
		if (URI.isUri(this.value)) {
			...
			return (
				this.value.modelDescription ??
				`User's active context from ${contextName}`
			);
