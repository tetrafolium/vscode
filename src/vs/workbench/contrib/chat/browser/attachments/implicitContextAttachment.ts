...
export class ImplicitContextAttachmentWidget extends Disposable {
	...
	private render() {
		...
		for (const context of this.attachment.values) {
			...
			const targetRange = isLocation(context.value)
				? context.value.range
				: undefined;
			const targetHandle = isStringImplicitContextValue(context.value)
	...
	private renderMainContext(
		...
		const contextLabel =
			context.name ??
			(file ? basename(file) : localize("implicitContextFallback", "context"));
		...
		if (isSuggestedEnabled) {
			if (!isSelection) {
				...
				this.renderDisposables.add(
					toggleButton.onDidClick(async (e) => {
						e.stopPropagation();
						e.preventDefault();
						if (!context.enabled) {
			...
			const buttonMsg = context.enabled
				? localize("disable", "Disable current {0} context", attachmentTypeName)
		...
		const label = this.renderDisposables.add(
			this.resourceLabels.create(contextNode, { supportIcons: true }),
		);
	...
	private renderString(
		...
		markdownTooltip: IMarkdownString | undefined,
		defaultTitle: string,
	): string | undefined {
		...
		if (
			icon &&
			(ThemeIcon.isFile(icon) || ThemeIcon.isFolder(icon)) &&
			resourceUri
		) {
			const fileKind = ThemeIcon.isFolder(icon)
				? FileKind.FOLDER
				: FileKind.FILE;
	...
	private renderResource(
		attachmentValue: Location | URI | undefined,
		isSelection: boolean,
		enabled: boolean,
		label: IResourceLabel,
		contextNode: HTMLElement,
	): string | undefined {
		const file = URI.isUri(attachmentValue)
			? attachmentValue
			: attachmentValue!.uri;
		const range =
			URI.isUri(attachmentValue) || !isSelection
				? undefined
				: attachmentValue!.range;
	...
	private async convertToRegularAttachment(
		attachment: ChatImplicitContext,
	): Promise<void> {
		if (!attachment.value) {
			return;
		}
