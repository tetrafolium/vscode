...
export interface IChatAttachmentResolveService {
	...
	resolveBrowserViewAttachContext(
		browserId: string,
	): Promise<IBrowserViewVariableEntry | undefined>;
	...
	resolveSourceControlHistoryItemAttachContext(
		data: SCMHistoryItemTransferData[],
	): ISCMHistoryItemVariableEntry[];
	resolveDirectoryImages(
		directoryUri: URI,
	): Promise<IChatRequestVariableEntry[]>;
}
...
export class ChatAttachmentResolveService implements IChatAttachmentResolveService {
	...
	constructor(
		...
		@IBrowserViewWorkbenchService
		private browserViewService: IBrowserViewWorkbenchService,
	) {}
	...
	public async resolveBrowserViewAttachContext(
		browserId: string,
	): Promise<IBrowserViewVariableEntry | undefined> {
	...
	public async resolveDirectoryImages(
		directoryUri: URI,
	): Promise<IChatRequestVariableEntry[]> {
	...
	private async _collectDirectoryImages(
		directoryUri: URI,
		results: IChatRequestVariableEntry[],
	): Promise<void> {
		let stat;
		try {
		...
		for (const child of stat.children) {
			if (child.isDirectory && !child.isSymbolicLink) {
				...
				SUPPORTED_IMAGE_EXTENSIONS_REGEX.test(child.resource.path)
			) {
				childPromises.push(
					this.resolveImageEditorAttachContext(child.resource)
						.then((entry) => {
							if (entry) {
								results.push(entry);
							}
	...
	public resolveSourceControlHistoryItemAttachContext(
		data: SCMHistoryItemTransferData[],
	): ISCMHistoryItemVariableEntry[] {
		return data.map(
			(d) =>
