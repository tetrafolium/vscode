...
} from "./utilityModelContribution.js";
import {
	...
	properties: {
		...
		[ChatConfiguration.EnabledPlugins]: {
			...
			policy: {
				...
				minimumVersion: "1.122",
				value: (policyData) =>
					policyData.enabledPlugins
						? JSON.stringify(policyData.enabledPlugins)
						: undefined,
				localization: {
					description: {
		...
		[ChatConfiguration.ExtraMarketplaces]: {
			...
			policy: {
				...
				minimumVersion: "1.122",
				value: (policyData) => {
					const obj = extraKnownMarketplacesToConfigDict(
						policyData.extraKnownMarketplaces,
					);
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				const chatModes = focusedWidget
					.read(reader)
					?.input.currentChatModesObs.read(reader);
