...
type ToolResultCompressedClassification = {
	...
	toolId: {
		...
		comment: "The tool whose output was compressed.";
	};
	filters: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Comma-separated filter ids that fired.";
	};
	beforeChars: {
		classification: "SystemMetaData";
		purpose: "PerformanceAndHealth";
		isMeasurement: true;
		comment: "Total text part length in UTF-16 code units before compression.";
	};
	afterChars: {
		classification: "SystemMetaData";
		purpose: "PerformanceAndHealth";
		isMeasurement: true;
		comment: "Total text part length in UTF-16 code units after compression.";
	};
	cacheHit: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "True when the compressed result came from a session-memory cache hit (response dedup) rather than from filters.";
	};
};
...
export class ToolResultCompressorService
	extends Disposable
	implements IToolResultCompressor
{
	...
	maybeCompress(
		...
		const matchingFilters =
			filters?.filter((f) => {
				try {
					...
					this._logService.warn(
						`[ToolResultCompressor] filter ${f.id} threw in matches on tool ${toolId}: ${getErrorMessage(err)}`,
						err,
					);
					return false;
				}
		...
		const newContent = result.content.map((part) => {
			...
			for (let i = 0; i < activeFilters.length /* manual increment */; ) {
				...
				try {
					...
					if (!disabledFilterIds.has(filter.id)) {
						...
						this._logService.warn(
							`[ToolResultCompressor] filter ${filter.id} threw on tool ${toolId}; disabled for this pass: ${getErrorMessage(err)}`,
							err,
						);
					}
				}
			...
			if (current !== original) {
				...
				// untouched.
				const banner = formatCompressionBanner(
					partFilterIds,
					original.length,
					current.length,
				);
	...
	private _buildCacheHitResult(
		original: IToolResult,
		hit: { text: string; timestamp: number },
	): IToolResult {
	...
	private _recordInCaches(
		toolId: string,
		input: unknown,
		result: IToolResult,
		caches: readonly IToolResultCache[] | undefined,
	): void {
		if (!caches || caches.length === 0) {
			return;
		}
	...
	private _sendTelemetry(
		...
		this._telemetryService.publicLog2<
			ToolResultCompressedEvent,
			ToolResultCompressedClassification
