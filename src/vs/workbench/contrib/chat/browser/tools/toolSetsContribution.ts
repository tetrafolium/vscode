...
abstract class RawToolSetsShape {
	...
	private constructor(
		entries: Map<string, Exclude<IRawToolSetContribution, "name">>,
	) {
		this.entries = Object.freeze(new Map(entries));
	}
}

export class UserToolSetsContributions
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private _initToolSets(): void {
		...
		const fileEventSig = observableSignalFromEvent(
			this,
			Event.filter(this._fileService.onDidFilesChange, (e) =>
				e.affects(promptFolder.get()),
			),
		);
		...
		this._store.add(
			autorun(async (r) => {
				...
				for (const entry of entries) {
					if (
						!entry.isFile ||
						!RawToolSetsShape.isToolSetFileName(entry.resource)
					) {
						// not interesting
						continue;
					}
					...
					for (const [name, value] of data.entries) {
						...
						value.tools.forEach((name) => {
							...
							const toolSet =
								this._languageModelToolsService.getToolSetByName(name);
							if (toolSet) {
						...
						transaction((tx) => {
							...
							toolSets.forEach((toolSet) =>
								store.add(toolset.addToolSet(toolSet, tx)),
							);
						});
					}
				}
...
export class ConfigureToolSets extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const picks: (
			| (IQuickPickItem & { toolset?: IToolSet })
			| IQuickPickSeparator
		)[] = [];
