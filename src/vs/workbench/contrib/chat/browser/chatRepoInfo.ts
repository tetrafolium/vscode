...
function getRemoteHost(remoteUrl: string): string | undefined {
	try {
		...
		const hostAndPath =
			atIndex !== -1 ? remoteUrl.slice(atIndex + 1) : remoteUrl;
...
export async function generateUnifiedDiff(
	...
	try {
		...
		const modifiedEndsWithNewline =
			modifiedContent.length > 0 && modifiedContent.endsWith("\n");
 ...
 */
export function captureRepoMetadata(
	scmService: ISCMService,
): IExportableRepoData | undefined {
 ...
 */
export async function captureRepoInfo(
	scmService: ISCMService,
	fileService: IFileService,
): Promise<IExportableRepoData | undefined> {
	const repositories = [...scmService.repositories];
	if (repositories.length === 0) {
		return undefined;
	}
	...
	if (remoteUrl) {
		...
		if (host === "github.com") {
			...
			host === "dev.azure.com" ||
			(host && host.endsWith(".visualstudio.com"))
		) {
			remoteVendor = "ado";
		} else {
			remoteVendor = "other";
	...
	const baseRepoData: Omit<
		IExportableRepoData,
		"diffs" | "diffsStatus" | "changedFileCount"
	> = {
		workspaceType,
		syncStatus,
		remoteUrl,
	...
	for (const group of repository.provider.groups) {
		for (const resource of group.resources) {
			const relPath =
				relativePath(rootUri, resource.sourceUri) ?? resource.sourceUri.path;
			const changeType = determineChangeType(resource, group.id);

			const diffPromise = (async (): Promise<
				IExportableRepoDiff | undefined
			> => {
 ...
 */
export class ChatRepoInfoContribution
	extends Disposable
