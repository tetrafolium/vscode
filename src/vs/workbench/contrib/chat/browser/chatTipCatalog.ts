...
export function getCommandLabel(commandId: string): string {
	...
	if (command?.title) {
		...
		return typeof command.title === "string"
			? command.title
			: command.title.value;
	}
...
export const TIP_CATALOG: readonly ITipDefinition[] = [
	...
	},
	{
		...
		when: ContextKeyExpr.and(
			...
			ContextKeyExpr.notEquals(
				"config.workbench.browser.enableChatTools",
				true,
			),
		),
	...
	},
	{
		...
		buildMessage(ctx) {
			...
			const experimentalTemplate = ctx.experimentalTipMessages.get(
	...
	},
	{
		...
		excludeWhenCommandsExecuted: [
			"workbench.action.chat.openNewChatSessionInPlace.copilotcli",
		],
	},
	{
