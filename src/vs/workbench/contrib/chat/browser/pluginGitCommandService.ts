...
export class BrowserPluginGitCommandService implements IPluginGitService {
	...
	private async _pruneStaleEntries(
		cache: Map<string, IBrowserPluginCacheEntry>,
		knownDirs: ReadonlyMap<string, URI>,
	): Promise<void> {
		...
		await Promise.all(
			Array.from(knownDirs, async ([key, uri]) => {
				try {
	...
	private _setCacheEntry(
		targetDir: URI,
		entry: IBrowserPluginCacheEntry,
	): void {
