...
function installedPluginToItem(
	...
	outdated?: IObservable<IMarketplacePlugin | undefined>,
): IInstalledPluginItem {
...
class DropDownActionViewItem extends ActionViewItem {
	...
	showMenu(actionGroups: IAction[][]): void {
		...
		const actions = actionGroups.flatMap((group) => [
			...group,
			new Separator(),
		]);
		if (actions.length > 0) {
...
class AgentPluginRenderer implements IPagedRenderer<
	IAgentPluginItem,
	IAgentPluginTemplateData
> {
	...
	renderElement(
		element: IAgentPluginItem,
		_index: number,
		data: IAgentPluginTemplateData,
	): void {
		this.disposeElement(undefined, 0, data);
		...
		data.elementDisposables.push(
			autorun((reader) => {
				data.root.classList.toggle(
					"disabled",
					element.kind === AgentPluginItemKind.Installed &&
						!isContributionEnabled(element.plugin.enablement.read(reader)),
				);
			}),
		);
	...
	disposeElement(
		_element: IAgentPluginItem | undefined,
		_index: number,
		data: IAgentPluginTemplateData,
	): void {
...
export class AgentPluginsListView extends AbstractExtensionsListView<IAgentPluginItem> {
	...
	private readonly refreshOnPluginsChangedScheduler = this._register(
	...
	async show(query: string): Promise<IPagedModel<IAgentPluginItem>> {
		...
		if (isRecommended) {
			const recommended =
				this.pluginMarketplaceService.recommendedPlugins.get();
	...
	private queryInstalled(): IInstalledPluginItem[] {
		const marketplaceObs = derived((reader) => {
			...
			for (const entry of this.pluginMarketplaceService.installedPlugins.read(
				reader,
			)) {
		...
		return plugins.map((p) => {
			const isOutdated = derived((reader) => {
				const { marketplaceByKey, installedByUri } =
...
class CheckForPluginUpdatesCommand extends Action2 {
	...
	async run(accessor: ServicesAccessor) {
		await accessor
			.get(IPluginInstallService)
			.updateAllPlugins({}, CancellationToken.None);
	}
}
...
class ForceUpdatePluginsCommand extends Action2 {
	...
	async run(accessor: ServicesAccessor) {
		await accessor
			.get(IPluginInstallService)
			.updateAllPlugins({ force: true }, CancellationToken.None);
	}
}
...
export class AgentPluginsViewsContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const hasInstalledKey =
			HasInstalledAgentPluginsContext.bindTo(contextKeyService);
		this._register(
			autorun((reader) => {
		...
		Registry.as<IViewsRegistry>(ViewExtensions.ViewsRegistry).registerViews(
			[
				...
				},
				{
					...
					when: ContextKeyExpr.and(
						DefaultViewsContext,
						HasInstalledAgentPluginsContext.toNegated(),
						ChatContextKeys.Setup.hidden.negate(),
					),
					weight: 30,
					order: 5,
