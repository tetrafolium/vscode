...
export const IChatContextService =
	createDecorator<IChatContextService>("chatContextService");
...
export class ChatContextService extends Disposable {
	...
	private _lastResourceContext: Map<
		StringChatContextValue,
		{ originalItem: IChatContextItem; provider: IChatResourceContextProvider }
	> = new Map();
	...
	setExecuteCommandCallback(
		callback: (itemHandle: number) => Promise<void>,
	): void {
	...
	registerChatWorkspaceContextProvider(
		id: string,
		provider: IChatWorkspaceContextProvider,
	): void {
		const providerEntry = this._providers.get(id) ?? {};
	...
	registerChatExplicitContextProvider(
		id: string,
		provider: IChatExplicitContextProvider,
	): void {
		const providerEntry = this._providers.get(id) ?? {};
	...
	getWorkspaceContextItems(): IChatRequestWorkspaceVariableEntry[] {
		...
		for (const workspaceContexts of this._workspaceContext.values()) {
			for (const item of workspaceContexts) {
				...
				const derivedLabel =
					item.label ??
					(item.resourceUri ? basename(item.resourceUri) : "Unknown");
				items.push({
					value: item.value,
					name: derivedLabel,
	...
	async contextForResource(
		uri: URI,
		language?: string,
	): Promise<StringChatContextValue | undefined> {
		return this._contextForResource(uri, false, language);
	}

	private async _contextForResource(
		uri: URI,
		withValue: boolean,
		language?: string,
	): Promise<StringChatContextValue | undefined> {
	...
	async resolveChatContext(
		context: StringChatContextValue,
		language?: string,
	): Promise<StringChatContextValue> {
	...
	private _asPicker(
		...
		const asPicker = (): IChatContextPicker => {
			...
			const picks = async (): Promise<IChatContextItem[]> => {
				...
				const results =
					await providerEntry?.explicitProvider!.provideChatContext(
						CancellationToken.None,
					);
				return results || [];
			};
					...
					return items.map((item) => {
						...
						return {
							...
							iconClass: item.icon
								...
								: undefined,
							asAttachment:
								async (): Promise<IGenericChatRequestVariableEntry> => {
									...
									if (
										...
										contextValue =
											await providerEntry.explicitProvider.resolveChatContext(
												item,
												CancellationToken.None,
											);
									}
									...
									const resolvedLabel =
										contextValue.label ??
										(contextValue.resourceUri
											? basename(contextValue.resourceUri)
											: "Unknown");
									return {
										kind: "generic",
										id: resolvedLabel,
