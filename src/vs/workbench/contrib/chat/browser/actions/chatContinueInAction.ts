 ...
 */
async function resolveGitRemoteNwo(
	repoPath: string,
	...
	try {
		...
		const remoteMatch = configText.match(
...
export class ChatContinueInSessionActionItem extends ActionWidgetDropdownActionViewItem {
	constructor(
		...
		super(
			action,
			{
				actionProvider: ChatContinueInSessionActionItem.actionProvider(
					chatSessionsService,
				...
				actionBarActions:
					ChatContinueInSessionActionItem.getActionBarActions(openerService),
				reporter: {
					id: "ChatContinueInSession",
					name: "ChatContinueInSession",
					includeOptions: true,
				},
			},
			actionWidgetService,
	...
	private static actionProvider(
		...
		return {
			getActions: () => {
				...
				const backgroundContrib = contributions.find(
					(contrib) => contrib.type === AgentSessionProviders.Background,
				);
				...
				const cloudContrib = contributions.find(
					(contrib) => contrib.type === AgentSessionProviders.Cloud,
				);
	...
	private static toAction(
		...
		contrib: ResolvedChatSessionsExtensionPoint,
		instantiationService: IInstantiationService,
		location: ActionLocation,
			...
			run: () =>
				instantiationService.invokeFunction((accessor) => {
					if (location === ActionLocation.Editor) {
						return new CreateRemoteAgentJobFromEditorAction().run(
							accessor,
							contrib,
						);
					}
	...
	protected override renderLabel(element: HTMLElement): IDisposable | null {
		if (this.location === ActionLocation.Editor) {
			...
			const icon = this.contextKeyService.contextMatchesRules(
				ChatContextKeys.remoteJobCreating,
			)
				? Codicon.sync
				: Codicon.forward;
...
export class CreateRemoteAgentJobAction {
	...
	private openUntitledEditor(
		...
		continuationTarget: ResolvedChatSessionsExtensionPoint,
	) {
		commandService.executeCommand(
			`${NEW_CHAT_SESSION_ACTION_ID}.${continuationTarget.type}`,
		);
	}
	 ...
	 */
	private async extractRepoNwoFromSession(
		...
		if (agentSession?.metadata) {
			...
			const workingDir = (metadata.workingDirectoryPath ??
				...
				metadata.worktreePath) as string | undefined;
			if (workingDir) {
		...
		for (const optionId of ["repositories", "repository"]) {
			const repoOption = chatSessionsService.getSessionOption(
				sessionResource,
				optionId,
			);
			if (repoOption) {
				const optionValue =
					typeof repoOption === "string"
						? repoOption
						: (repoOption as { id: string }).id;
				if (optionValue) {
					...
					if (isAbsolute(optionValue)) {
						const nwoFromGit = await resolveGitRemoteNwo(
							optionValue,
							fileService,
						);
						if (nwoFromGit) {
	...
	async run(
		...
		const remoteJobCreatingKey =
			ChatContextKeys.remoteJobCreating.bindTo(contextKeyService);
		...
		try {
			...
			if (
				isSessionsWindow &&
				sourceProvider &&
				sourceProvider !== continuationTargetType
			) {
				...
				let transcript = chatRequests
					.map((req) => {
						const userMsg = `User: ${req.message.text}`;
						const respMsg = req.response?.response
							? `Assistant: ${req.response.response.getMarkdown()}`
							: "";
				...
				await commandService.executeCommand(actionId, {
					...
					initialSessionOptions:
						initialSessionOptions.size > 0 ? initialSessionOptions : undefined,
				});
				return;
			}
			...
			if (ChatSendResult.isSent(sendResult)) {
				await widget.handleDelegationExitIfNeeded(
					defaultAgent,
					sendResult.data.agent,
				);
			}
		} catch (e) {
...
export class ContinueChatInSessionActionRendering
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const disposable = actionViewItemService.register(
			...
			ContinueChatInSessionAction.ID,
			(action, options, instantiationService2) => {
