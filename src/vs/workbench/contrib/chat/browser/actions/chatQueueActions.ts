...
const steerIsDefault = ContextKeyExpr.equals(
	`config.${ChatConfiguration.RequestQueueingDefaultAction}`,
	"steer",
);
...
function isRemovePendingRequestContext(
	context: unknown,
): context is IChatRemovePendingRequestContext {
	return (
		!!context &&
		...
		typeof (context as IChatRemovePendingRequestContext).pendingRequestId ===
			"string"
	);
}
...
export class ChatSendPendingImmediatelyAction extends Action2 {
	...
	override async run(
		...
		const targetIndex = pendingRequests.findIndex(
			(r) => r.request.id === context.id,
		);
		if (targetIndex === -1) {
			return;
		}
		...
		const reordered = [
			...
			...pendingRequests
				...
				.map((r) => ({ requestId: r.request.id, kind: r.kind })),
		];
...
export class ChatRemoveAllPendingRequestsAction extends Action2 {
	...
	override run(accessor: ServicesAccessor, ...args: unknown[]): void {
		...
		const widget =
			(isRequestVM(context) &&
				widgetService.getWidgetBySessionResource(context.sessionResource)) ||
