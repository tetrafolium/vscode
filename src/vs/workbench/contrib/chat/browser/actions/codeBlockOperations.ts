...
export class InsertCodeBlockOperation {
	...
	public async run(context: ICodeBlockActionContext) {
		...
		if (isResponseVM(context.element)) {
			...
			const request = context.element.session
				.getItems()
				.find((item) => item.id === requestId && isRequestVM(item)) as
				| IChatRequestViewModel
				| undefined;
			...
			const codeBlockInfo = context.element.model.codeBlockInfos?.at(
	...
	private async handleTextEditor(
		...
		this.codeEditorService
			.listCodeEditors()
			.find((editor) => isEqual(editor.getModel()?.uri, activeModel.uri))
			?.focus();
		return true;
	}
...
type IComputeEditsResult = {
	readonly editsProposed: boolean;
...
export class ApplyCodeBlockOperation {
	...
	public async run(context: ICodeBlockActionContext): Promise<void> {
		...
		if (
			codemapperUri &&
			!isEqual(activeEditorControl?.getModel().uri, codemapperUri) &&
			!this.notebookService.hasSupportedNotebooks(codemapperUri)
		) {
	...
	private async evaluateURIToUse(
		resource: URI | undefined,
		activeEditorControl: IActiveCodeEditor | undefined,
	...
	private async handleNotebookEditor(
		...
		if (!codeMapper) {
			this.notify(
				localize("applyCodeBlock.noCodeMapper", "No code mapper available."),
			);
			return undefined;
		}
		...
		try {
			const iterable = await this.progressService.withProgress<
				AsyncIterable<[URI, TextEdit[]] | ICellEditOperation[]>
			>(
				{
				...
				async (progress) => {
					...
					const editsIterable = this.getNotebookEdits(
						codeBlock,
						chatSessionResource,
			...
			editsProposed = await this.applyNotebookEditsWithInlinePreview(
				iterable,
				uri,
				cancellationTokenSource,
			);
		...
		try {
			...
			>(
				...
				async (progress) => {
					...
					const editsIterable = this.getTextEdits(
						codeBlock,
						chatSessionResource,
						cancellationTokenSource.token,
					);
	...
	private getTextEdits(
		codeBlock: ICodeMapperCodeBlock,
		chatSessionResource: URI | undefined,
	...
	private getNotebookEdits(
		...
		return new AsyncIterableProducer<[URI, TextEdit[]] | ICellEditOperation[]>(
			async (executor) => {
				const request: ICodeMapperRequest = {
					codeBlocks: [codeBlock],
					chatSessionResource,
	...
	private async waitForFirstElement<T>(
	...
	private async applyNotebookEditsWithInlinePreview(
		edits: AsyncIterable<[URI, TextEdit[]] | ICellEditOperation[]>,
		uri: URI,
		tokenSource: CancellationTokenSource,
	): Promise<boolean> {
		return this.instantiationService.invokeFunction(
...
function getActiveNotebookEditor(
...
function getEditableActiveCodeEditor(
	editorService: IEditorService,
): IActiveCodeEditor | undefined {
	const activeCodeEditorInNotebook =
		getActiveNotebookEditor(editorService)?.activeCodeEditor;
	if (
...
function isReadOnly(
	...
	const activeTextModel =
		textFileService.files.get(model.uri) ??
		textFileService.untitled.get(model.uri);
