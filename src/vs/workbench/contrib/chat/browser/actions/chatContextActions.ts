...
async function withChatView(
	...
	if (
		!lastFocusedWidget ||
		lastFocusedWidget.location === ChatAgentLocation.Chat
	) {
...
abstract class AttachResourceAction extends Action2 {
	...
	protected _getResources(
		...
		const contexts = isEditorCommandsContext(args[1])
...
class AttachSelectionToChatAction extends Action2 {
	...
	override async run(
		...
		const widget = await accessor
			.get(IInstantiationService)
			.invokeFunction(withChatView);
		if (!widget) {
			return;
		}
...
export class AttachSearchResultAction extends Action2 {
	...
	async run(accessor: ServicesAccessor) {
		...
		const originalRange =
			editor.getSelection() ??
			editor.getModel()?.getFullModelRange().collapseToEnd();
...
type IQuickPickServicePickItem =
	| IGotoSymbolQuickPickItem
	| ISymbolQuickPickItem
	| IQuickPickItemWithResource;
...
function isIGotoSymbolQuickPickItem(
	obj: unknown,
): obj is IGotoSymbolQuickPickItem {
	return (
		isObject(obj) &&
...
function isIQuickPickItemWithResource(
	obj: unknown,
): obj is IQuickPickItemWithResource {
	return (
		isObject(obj) && URI.isUri((obj as IQuickPickItemWithResource).resource)
	);
}
...
export class AttachContextAction extends Action2 {
	...
	override async run(
		...
		const context = args[0] as
			| { widget?: IChatWidget; placeholder?: string }
			| undefined;
	...
	private _show(
		accessor: ServicesAccessor,
		widget: IChatWidget,
		additionPicks: IContextPickItemItem[] | undefined,
		placeholder?: string,
	) {
		...
		const providerOptions: AnythingQuickAccessProviderRunOptions = {
			filter: (pick) => {
				if (isIQuickPickItemWithResource(pick) && pick.resource) {
					return instantiationService.invokeFunction((accessor) =>
						isSupportedChatFileScheme(accessor, pick.resource!.scheme),
					);
				}
				return true;
			},
			additionPicks,
			handleAccept: async (
				item: IQuickPickServicePickItem | IContextPickItemItem,
				...
				if (isIContextPickItemItem(item)) {
					...
					if (item.item.type === "valuePick") {
						...
						isDone = await this._handleContextPickerItem(
							quickInputService,
							commandService,
							item.item,
							widget,
		...
		quickInputService.quickAccess.show("", {
			...
			placeholder:
				placeholder ??
				localize("chatContext.attach.placeholder", "Search attachments"),
			providerOptions,
		});
	}

	private async _handleQPPick(
		accessor: ServicesAccessor,
		widget: IChatWidget,
		...
		if (isIQuickPickItemWithResource(pick) && pick.resource) {
			if (/\.(png|jpg|jpeg|bmp|gif|tiff)$/i.test(pick.resource.path)) {
				...
				const entry =
					await chatAttachmentResolveService.resolveEditorAttachContext({
	...
	private async _handleContextPick(
		item: IChatContextValueItem,
		widget: IChatWidget,
	) {
	...
	private async _handleContextPickerItem(
		quickInputService: IQuickInputService,
		commandService: ICommandService,
		item: IChatContextPickerItem,
		widget: IChatWidget,
	): Promise<boolean> {
