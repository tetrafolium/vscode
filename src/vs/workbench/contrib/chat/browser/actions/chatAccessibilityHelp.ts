...
export function getAccessibilityHelpText(
	type:
		| "panelChat"
		| "inlineChat"
		| "agentView"
	...
	if (type === "editsView" || type === "agentView") {
		...
		content.push(
			localize("chatEditing.helpfulCommands", "Some helpful commands include:"),
		);
		content.push(
			localize(
...
export function getChatAccessibilityHelpProvider(
	accessor: ServicesAccessor,
	editor: ICodeEditor | undefined,
	type: "panelChat" | "inlineChat" | "quickChat" | "editsView" | "agentView",
): AccessibleContentProvider | undefined {
	const widgetService = accessor.get(IChatWidgetService);
	const keybindingService = accessor.get(IKeybindingService);
	const inputEditor: ICodeEditor | undefined =
	...
	return new AccessibleContentProvider(
		type === "panelChat"
			...
			: type === "inlineChat"
				...
				: type === "agentView"
					? AccessibleViewProviderId.AgentChat
...
function getChatFocusKeybindingLabel(
	keybindingService: IKeybindingService,
	type: "agentView" | "panelChat" | "inlineChat" | "quickChat",
	focus?: "lastFocused" | "last" | "input",
): string | undefined {
	let kbs;
	...
	if (type === "agentView" || type === "panelChat") {
		if (focus !== "input") {
			kb = kbs
				.find((kb) => kb.getAriaLabel()?.includes("UpArrow"))
				?.getAriaLabel();
		} else {
			kb = kbs
				.find((kb) => kb.getAriaLabel()?.includes("DownArrow"))
				?.getAriaLabel();
		}
	} else {
		// Quick chat
