		...
		store.add(
			inputBox.onDidAccept(async () => {
				...
				const validationError =
					pluginInstallService.validatePluginSource(source);
				if (validationError) {
				...
				try {
					...
					const result =
						await pluginInstallService.installPluginFromValidatedSource(source);
					if (!result.success) {
						if (result.message) {
...
class ManagePluginMarketplacesAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const { userValues, extraValues, effectiveValues } =
			readConfiguredMarketplaces(configurationService);
		const refs = parseMarketplaceReferences(effectiveValues);
		const policyCanonicalIds = new Set(
			parseMarketplaceReferences(extraValues).map((r) => r.canonicalId),
		);
		...
		const items: IMarketplaceQuickPickItem[] = refs.map((ref) => ({
			...
			description:
				ref.kind === MarketplaceReferenceKind.LocalFileUri
		...
		if (repoExists) {
			actionItems.push({
				id: "openDirectory",
				label: localize("openMarketplaceDirectory", "Open Folder"),
			});
		}

		actionItems.push({
			id: "removeMarketplace",
			label: localize("removeMarketplace", "Remove Marketplace"),
		});
		...
		switch (action.id) {
			...
			case "removeMarketplace": {
				...
				const updated = userValues.filter(
					(v) => typeof v === "string" && v.trim() !== ref.rawValue,
				);
