...
export function registerChatOpenAgentDebugPanelAction() {
	registerAction2(
		class OpenAgentDebugPanelAction extends Action2 {
			constructor() {
				super({
			...
			async run(accessor: ServicesAccessor): Promise<void> {
				...
				const options: IChatDebugEditorOptions = {
					pinned: true,
					viewHint: "home",
				};
	...
	registerAction2(
		class OpenAgentDebugPanelForSessionAction extends Action2 {
			constructor() {
				super({
	...
	const debugLogFilters = [
		{
			name: localize("chatDebugLog.file.label", "Agent Debug Log"),
			extensions: ["json"],
		},
	];

	registerAction2(
		class ExportAgentDebugLogAction extends Action2 {
			constructor() {
				super({
					id: "workbench.action.chat.exportAgentDebugLog",
					title: localize2(
						"chat.exportAgentDebugLog.label",
						"Export Agent Debug Log...",
					),
			...
			async run(accessor: ServicesAccessor): Promise<void> {
				...
				if (!sessionResource) {
					notificationService.notify({
						...
						message: localize(
							"chatDebugLog.noSession",
							"No active debug session to export. Navigate to a session first.",
						),
					});
					return;
				}
				...
				const rawIdentifier =
					localSessionId ??
					(sessionResource.path.replace(/^\//, "") ||
						sessionResource.authority);
				const sessionIdentifier = rawIdentifier
					?.replace(/[/\\:*?"<>|.]+/g, "_")
					.replace(/^_+|_+$/g, "");
				const exportFileName = sessionIdentifier
					? `agent-debug-log-${sessionIdentifier}.json`
					: defaultDebugLogFileName;
				const defaultUri = joinPath(
					await fileDialogService.defaultFilePath(),
					exportFileName,
				);
				const outputPath = await fileDialogService.showSaveDialog({
					defaultUri,
				...
				if (!data) {
					notificationService.notify({
						...
						message: localize(
							"chatDebugLog.exportFailed",
							"Export is not supported by the current provider.",
						),
					});
					return;
				}
				...
				telemetryService.publicLog2<
					ChatDebugExportEvent,
					ChatDebugExportClassification
				>("chatDebugLogExported", {
				...
				notificationService.prompt(
					Severity.Info,
					localize(
						"chatDebugLog.exportSuccess",
						"Agent debug log exported successfully.",
					),
					[
						{
	...
	registerAction2(
		class ImportAgentDebugLogAction extends Action2 {
			constructor() {
				super({
					id: "workbench.action.chat.importAgentDebugLog",
					title: localize2(
						"chat.importAgentDebugLog.label",
						"Import Agent Debug Log...",
					),
			...
			async run(accessor: ServicesAccessor): Promise<void> {
				...
				if (stat.size !== undefined && stat.size > maxImportSize) {
					telemetryService.publicLog2<
						ChatDebugImportEvent,
						ChatDebugImportClassification
					>("chatDebugLogImported", {
					...
					await dialogService.warn(
						...
						localize(
							"chatDebugLog.fileTooLargeDetail",
							"The selected file ({0} MB) exceeds the 50 MB size limit for debug log imports.",
				...
				const sessionUri = await chatDebugService.importLog(
				...
				if (!sessionUri) {
					...
					notificationService.notify({
						severity: Severity.Warning,
						message: localize(
							"chatDebugLog.importFailed",
							"Import is not supported by the current provider.",
						),
					});
					return;
				}
...
type ChatDebugExportClassification = {
	fileSizeBytes: {
		...
		isMeasurement: true;
		comment: "Size of the exported chat debug log file in bytes.";
	};
	owner: "vijayu";
...
type ChatDebugImportClassification = {
	fileSizeBytes: {
		...
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Size of the imported chat debug log file in bytes.";
	};
	result: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Outcome of the chat debug file import: success, fileTooLarge, or providerFailed.";
	};
	owner: "vijayu";
