...
} from "../../../markdown/common/markedKatexExtension.js";
...
export class ChatCopyActionViewItem extends MenuEntryActionViewItem {
	...
	private readonly actionRunnerListener = this._register(
	...
	override render(container: HTMLElement): void {
		...
		const iconContainer = dom.append(
			this.label,
			dom.$(".chat-copy-action-icons"),
		);
		const copyIcon = dom.append(
			iconContainer,
			dom.$(".chat-copy-action-icon.chat-copy-action-icon-copy"),
		);
		copyIcon.classList.add(...copyIconClasses);
		copyIcon.setAttribute("aria-hidden", "true");

		const copiedIcon = dom.append(
			iconContainer,
			dom.$(".chat-copy-action-icon.chat-copy-action-icon-copied"),
		);
...
export class ChatCopyActionRendering
	extends Disposable
...
export function registerChatCopyActions() {
	registerAction2(
		class CopyAllAction extends Action2 {
			...
			run(accessor: ServicesAccessor, context?: ChatTreeItem) {
				...
				const widget =
					((isRequestVM(context) || isResponseVM(context)) &&
					...
					const sessionAsText = viewModel
						...
						.filter(
							(item): item is IChatRequestViewModel | IChatResponseViewModel =>
								isRequestVM(item) ||
								(isResponseVM(item) && !item.errorDetails?.responseIsFiltered),
						)
		...
		class CopyItemAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, ...args: unknown[]) {
				...
				if (
					...
					dom.isAncestor(dom.getActiveElement(), widget.domNode)
				) {
	...
	registerAction2(
		class CopyFinalResponseAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class CopyKatexMathSourceAction extends Action2 {
			constructor() {
				super({
			...
			async run(accessor: ServicesAccessor, ...args: unknown[]) {
				...
				if (
					...
					dom.isAncestor(activeElement, widget.domNode)
				) {
				...
				if (!selectedElement) {
					...
					selectedElement =
						activeElement?.querySelector(`.${katexContainerClassName}`) ?? null;
				}

				// Extract the LaTeX source from the annotation element
				const katexElement = dom.isHTMLElement(selectedElement)
					? selectedElement.closest(`.${katexContainerClassName}`)
					: null;
				const latexSource =
					katexElement?.getAttribute(katexContainerLatexAttributeName) || "";
				if (latexSource) {
