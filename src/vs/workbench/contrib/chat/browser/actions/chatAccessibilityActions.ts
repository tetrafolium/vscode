...
} from "../../../../contrib/accessibility/browser/accessibilityConfiguration.js";
import {
	CHAT_ACCESSIBLE_VIEW_INCLUDE_THINKING_STORAGE_KEY,
	isThinkingContentIncludedInAccessibleView,
} from "../accessibility/chatResponseAccessibleView.js";

export const ACTION_ID_FOCUS_CHAT_CONFIRMATION =
	"workbench.action.chat.focusConfirmation";
export const ACTION_ID_TOGGLE_THINKING_CONTENT_ACCESSIBLE_VIEW =
	"workbench.action.chat.toggleThinkingContentAccessibleView";
...
class AnnounceChatConfirmationAction extends Action2 {
	constructor() {
		super({
			...
			title: {
				value: localize("focusChatConfirmation", "Focus Chat Confirmation"),
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const pendingWidget = chatWidgetService
			.getAllWidgets()
			.find((widget) => widget.viewModel?.model.requestNeedsInput.get());
		...
		if (isResponseVM(lastResponse)) {
			...
			const confirmationWidgets =
				pendingWidget.domNode.querySelectorAll<HTMLElement>(
					".chat-confirmation-widget-container[tabindex]",
				);
			if (confirmationWidgets.length > 0) {
				firstConfirmationElement =
					confirmationWidgets[confirmationWidgets.length - 1];
			}
		}
		...
		if (firstConfirmationElement) {
			...
			if (
				firstConfirmationElement.contains(
					pendingWidget.domNode.ownerDocument.activeElement,
				)
			) {
			...
			alert(
				localize("noConfirmationRequired", "No chat confirmation required"),
			);
		}
	}
}
...
class ToggleThinkingContentAccessibleViewAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const includeThinking =
			isThinkingContentIncludedInAccessibleView(storageService);
