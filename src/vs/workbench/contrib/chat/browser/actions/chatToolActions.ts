...
export const AcceptToolPostConfirmationActionId =
	"workbench.action.chat.acceptToolPostExecution";
export const SkipToolPostConfirmationActionId =
	"workbench.action.chat.skipToolPostExecution";
...
abstract class ToolConfirmationAction extends Action2 {
	...
	run(accessor: ServicesAccessor, context?: IToolConfirmationActionContext) {
		...
		for (const item of lastItem.model.response.value) {
			const state =
				item.kind === "toolInvocation" ? item.state.get() : undefined;
			if (
...
export class ConfigureToolsAction extends Action2 {
	...
	override async run(
		...
		try {
			const result = await instaService.invokeFunction(
				...
				description,
				() => entriesMap.get(),
				widget.input.selectedLanguageModel.get()?.metadata,
				cts.token,
			);
			if (result) {
		...
		telemetryService.publicLog2<SelectedToolData, SelectedToolClassification>(
			...
			{
				total: tools.size,
				enabled: Iterable.reduce(
					tools,
					(prev, [_, enabled]) => (enabled ? prev + 1 : prev),
					0,
				),
			},
		);
	...
	private extractSource(args: unknown[]): string | undefined {
		...
		function isChatActionSource(obj: unknown): obj is ChatActionSource {
			return (
				!!obj && typeof obj === "object" && !!(obj as ChatActionSource).source
			);
		}
