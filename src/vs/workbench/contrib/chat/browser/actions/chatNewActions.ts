...
function isNewEditSessionActionContext(
	arg: unknown,
): arg is INewEditSessionActionContext {
	if (arg && typeof arg === "object") {
		...
		if (
			obj.isPartialQuery !== undefined &&
			typeof obj.isPartialQuery !== "boolean"
		) {
			return false;
		}
		return true;
...
export function registerNewChatActions() {
	...
	registerAction2(
		class NewChatAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, ...args: unknown[]) {
				const executeCommandContext = isNewEditSessionActionContext(args[0])
					? args[0]
					: undefined;
	...
	const iconVariants = [
		...
		{
			idSuffix: ".newSessionIcon",
			iconValue: "new-session",
						...
						menu: [
							{
								...
								group: "1_open",
								order: 1,
								when: ChatContextKeys.newChatButtonExperimentIcon.isEqualTo(
									variant.iconValue,
								),
							},
						],
	...
	registerAction2(
		class NewLocalChatAction extends Action2 {
			constructor() {
				super({
	...
	MenuRegistry.appendMenuItem(MenuId.ChatViewSessionTitleNavigationToolbar, {
		...
		when: ChatContextKeys.agentSessionsViewerOrientation.notEqualsTo(
	...
	registerAction2(
		class RedoChatEditInteractionAction extends EditingSessionAction {
			...
			async runEditingSessionAction(
				...
				chatService
					.getSession(editingSession.chatSessionResource)
	...
	registerAction2(
		class RedoChatCheckpoints extends EditingSessionAction {
			constructor() {
				super({
			...
			async runEditingSessionAction(
				...
				const currentWidget = widget.getWidgetBySessionResource(
					editingSession.chatSessionResource,
				);
				const requestText =
					currentWidget?.viewModel?.model.checkpoint?.message.text;
...
async function runNewChatAction(
	...
	if (
		model &&
		!(await handleCurrentEditingSession(model, undefined, dialogService))
	) {
		return;
	}
	...
	if (typeof executeCommandContext.agentMode === "boolean") {
		widget.input.setChatMode(
			executeCommandContext.agentMode ? ChatModeKind.Agent : ChatModeKind.Edit,
		);
	} else if (
		widget.input.currentModeKind === ChatModeKind.Edit &&
		configurationService.getValue<boolean>(ChatConfiguration.EditModeHidden)
	) {
