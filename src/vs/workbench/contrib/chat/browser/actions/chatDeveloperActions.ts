...
} from "../../common/chatPermissionStorageKeys.js";
...
function formatChatModelReferenceInspection(
	accessor: ServicesAccessor,
): string {
	const chatService = accessor.get(IChatService);
	const agentSessionsService = accessor.get(IAgentSessionsService);
	const debugInfo = chatService.getChatModelReferenceDebugInfo();
	const referencedModels = debugInfo.models.filter(
		(model) => model.referenceCount > 0,
	);
	const pendingEditModels = debugInfo.models.filter(
		(model) => model.hasPendingEdits,
	);
	const pendingDisposalModels = debugInfo.models.filter(
		(model) => model.pendingDisposal,
	);
	...
	output +=
		"Created by shows who loaded or created the model. Holders shows who currently keeps the model alive.\n\n";
	...
	for (const model of debugInfo.models) {
		...
		const age = liveModel
			? fromNow(liveModel.timing.created, true, true, true)
			: "unknown";
...
class InspectChatModelReferencesAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		await editorService.openEditor({
			resource: undefined,
			contents: instantiationService.invokeFunction(
				formatChatModelReferenceInspection,
			),
			languageId: "markdown",
			options: {
				pinned: true,
