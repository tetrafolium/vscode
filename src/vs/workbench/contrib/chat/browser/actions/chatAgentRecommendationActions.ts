...
export class ChatAgentRecommendation
	extends Disposable
	implements IWorkbenchContribution
{
	static readonly ID = "workbench.contrib.chatAgentRecommendation";

	private readonly availabilityContextKeys = new Map<
		string,
		IContextKey<boolean>
	>();
	...
	private registerRecommendation(
		recommendation: IChatSessionRecommendation,
	): void {
		...
		const availabilityContext = new RawContextKey<boolean>(
			...
			registerAction2(
				class extends Action2 {
					...
					override async run(accessor: ServicesAccessor): Promise<void> {
						...
						const installPreReleaseVersion =
						...
						await runPostInstallCommand(
							commandService,
							chatService,
							recommendation.postInstallCommand,
						);
					}
				},
			),
	...
	private refreshInstallAvailability(): void {
		...
		this.extensionManagementService.getInstalled().then(
			(installedExtensions) => {
				...
				const installed = new Set(
					installedExtensions.map((ext) =>
						ExtensionIdentifier.toKey(ext.identifier.id),
					),
				);
...
async function runPostInstallCommand(
