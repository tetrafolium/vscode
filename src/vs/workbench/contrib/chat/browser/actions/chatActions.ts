...
} from "../../../scm/browser/scmHistoryChatContext.js";
...
export const CHAT_SETUP_SUPPORT_ANONYMOUS_ACTION_ID =
	"workbench.action.chat.triggerSetupSupportAnonymousAction";
const TOGGLE_CHAT_ACTION_ID = "workbench.action.chat.toggle";

export const GENERATE_AGENT_INSTRUCTIONS_COMMAND_ID =
	"workbench.action.chat.generateAgentInstructions";
export const GENERATE_ON_DEMAND_INSTRUCTIONS_COMMAND_ID =
	"workbench.action.chat.generateOnDemandInstructions";
export const GENERATE_PROMPT_COMMAND_ID =
...
export const INSERT_FORK_CONVERSATION_COMMAND_ID =
	"workbench.action.chat.insertForkConversationCommand";
export const INSERT_TROUBLESHOOT_COMMAND_ID =
	"workbench.action.chat.insertTroubleshootCommand";
...
abstract class OpenChatGlobalAction extends Action2 {
	...
	override async run(
		...
		const switchToMode = opts?.mode
			? chatWidget.input.currentChatModesObs.get().findModeByName(opts.mode)
			: this.mode;
		if (switchToMode) {
		...
		if (opts?.attachHistoryItemChanges) {
			for (const historyItemChange of opts.attachHistoryItemChanges) {
				const repository = scmService.getRepository(
					URI.file(historyItemChange.uri.path),
				);
				...
				const historyItem = await historyProvider.resolveHistoryItem(
					historyItemChange.historyItemId,
				);
				if (!historyItem) {
					continue;
				}
		...
		if (opts?.attachHistoryItemChangeRanges) {
			for (const historyItemChangeRange of opts.attachHistoryItemChangeRanges) {
				const repository = scmService.getRepository(
					URI.file(historyItemChangeRange.end.uri.path),
				);
				const historyProvider = repository?.provider.historyProvider.get();
				...
				const [historyItemStart, historyItemEnd] = await Promise.all([
					historyProvider.resolveHistoryItem(
						historyItemChangeRange.start.historyItemId,
					),
					historyProvider.resolveHistoryItem(
						historyItemChangeRange.end.historyItemId,
					),
				]);
		...
		if (opts?.blockOnResponse) {
			...
			if (response) {
				const autoReplyEnabled = configurationService.getValue<boolean>(
...
function getPendingConfirmationInfo(
...
): IChatPendingConfirmationInfo | undefined {
	for (const part of response.response.value) {
		...
		if (
			...
			part.state.get() === ElicitationState.Pending
		) {
...
export function registerChatActions() {
		...
		class NewChatEditorAction extends Action2 {
					...
					menu: [
						...
						},
						{
							...
							when: ContextKeyExpr.and(
								...
								ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo(
									"copilot",
								),
								ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo(
									"new-session",
			...
			async run(accessor: ServicesAccessor) {
				...
				await widgetService.openSession(
					...
					ACTIVE_GROUP,
					{ pinned: true } satisfies IChatEditorOptions,
				);
			}
		},
	);
	...
	registerAction2(
		class NewChatEditorCopilotIconAction extends Action2 {
			constructor() {
				super({
					...
					menu: [
						{
							...
							when: ContextKeyExpr.and(
								ActiveEditorContext.isEqualTo(ChatEditorInput.EditorID),
								ChatContextKeys.newChatButtonExperimentIcon.isEqualTo(
									"copilot",
								),
							),
							order: 1,
	...
	registerAction2(
		class NewChatEditorNewSessionIconAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class NewChatEditorCommentIconAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class NewChatEditorToSideAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class NewChatWindowAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class FocusMostRecentlyFocusedChatAction extends EditorAction2 {
			constructor() {
				super({
	...
	registerAction2(
		class FocusTodosViewAction extends Action2 {
			...
			constructor() {
				super({
					...
					category: CHAT_CATEGORY,
					f1: true,
					precondition: ChatContextKeys.chatModeKind.isEqualTo(
						ChatModeKind.Agent,
					),
					keybinding: [
						{
	...
	registerAction2(
		class FocusQuestionCarouselAction extends Action2 {
	...
	registerAction2(
		class PreviousQuestionCarouselQuestionAction extends Action2 {
	...
	registerAction2(
		class NextQuestionCarouselQuestionAction extends Action2 {
	...
	registerAction2(
		class FocusQuestionCarouselTerminalAction extends Action2 {
			static readonly ID =
	...
	registerAction2(
		class FocusTipAction extends Action2 {
	...
	registerAction2(
		class ShowContextUsageAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class ToggleShowContextUsageAction extends Action2 {
			constructor() {
				super({
					...
					toggled: ContextKeyExpr.equals(
						`config.${ChatConfiguration.ChatContextUsageEnabled}`,
						true,
					),
					menu: {
	...
	registerAction2(
		class extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				openerService.open(
					URI.parse(
						defaultAccountService.resolveGitHubUrl(GitHubPaths.copilotSettings),
					),
				);
			}
		},
		...
		class ShowExtensionsUsingCopilot extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				extensionsWorkbenchService.openSearch(
					`@contribute:${CopilotUsageExtensionFeatureId}`,
				);
			}
		},
	);
	...
	registerAction2(
		class ResetTrustedToolsAction extends Action2 {
			...
			override run(accessor: ServicesAccessor): void {
				accessor
					.get(ILanguageModelToolsConfirmationService)
	...
	registerAction2(
		class GenerateInstructionsAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class GenerateInstructionAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class GeneratePromptAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class GenerateSkillAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class GenerateAgentAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class GenerateHookAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class InsertForkConversationSlashCommandAction extends Action2 {
			constructor() {
				super({
					id: INSERT_FORK_CONVERSATION_COMMAND_ID,
					title: localize2(
						"insertForkConversationSlashCommand",
						"Insert Fork Command",
					),
					shortTitle: localize2(
						"insertForkConversationSlashCommand.short",
						"Insert /fork",
					),
					category: CHAT_CATEGORY,
	...
	registerAction2(
		class InsertTroubleshootSlashCommandAction extends Action2 {
			constructor() {
				super({
 ...
 */
export function computeToolEnablementMap(
	options: IToolFilteringOptions,
): IToolFilteringResult {
	...
	const enabledToolCount = Array.from(enablementMap.entries()).filter(
		([item, enabled]) => enabled && !isToolSet(item),
	).length;
	...
	for (const toolSet of allToolSets) {
		...
		const allToolsEnabled =
			...
			toolSetTools.every((t) => enablementMap.get(t) === true);
 ...
 * in the sidebar.
 */
export async function clearChatSessionPreservingType(
	widget: IChatWidget,
	...
	const newSessionType =
		sessionType ??
		(currentResource
			? getChatSessionType(currentResource)
...
registerAction2(
	class EditToolApproval extends Action2 {
		...
		async run(
			...
			const confirmationService = accessor.get(
				ILanguageModelToolsConfirmationService,
			);
			const toolsService = accessor.get(ILanguageModelToolsService);
			confirmationService.manageConfirmationPreferences(
				[...toolsService.getAllToolsIncludingDisabled()],
				scope ? { defaultScope: scope } : undefined,
			);
		}
	},
);
