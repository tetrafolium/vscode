...
} from "../widget/chatContentParts/codeBlockPart.js";
...
export class CodeBlockActionRendering
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const copyCodeBlockActionRendering = this._register(
...
export function registerChatCodeBlockActions() {
	...
	CopyAction?.addImplementation(50000, "chat-codeblock", (accessor) => {
		...
		const noSelection =
			editor.getSelections()?.length === 1 && editor.getSelection()?.isEmpty();
		...
		if (isResponseVM(element)) {
			...
			const codeBlockInfo = element.model.codeBlockInfos?.at(
				context.codeBlockIndex,
			);
			aiEditTelemetryService.handleCodeAccepted({
					...
					menu: [
						{
							...
							when: ContextKeyExpr.and(
								...shellLangIds.map((e) =>
									ContextKeyExpr.notEquals(EditorContextKeys.languageId.key, e),
								),
							),
							order: 10,
						},
						{
							...
							when: ContextKeyExpr.or(
								...shellLangIds.map((e) =>
									ContextKeyExpr.equals(EditorContextKeys.languageId.key, e),
								),
							),
						},
					],
		...
		class RunInTerminalAction extends ChatCodeBlockAction {
			...
			override async runWithContext(
				...
				if (
					...
					terminal = await terminalService.createAndFocusTerminal({
...
function getContextFromEditor(
	...
	const element = widget?.viewModel
		?.getItems()
		.find((item) => item.id === codeBlockInfo.elementId);
	return {
		element,
...
export function registerChatCodeCompareBlockActions() {
	...
	registerAction2(
		class ToggleDiffViewModeAction extends ChatCompareCodeBlockAction {
			constructor() {
				super({
	...
	registerAction2(
		class OpenCompareBlockInDiffEditor extends ChatCompareCodeBlockAction {
			constructor() {
				super({
