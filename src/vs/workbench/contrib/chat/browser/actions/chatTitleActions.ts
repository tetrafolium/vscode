	...
	registerAction2(
		class RetryChatAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, ...args: unknown[]) {
				...
				const chatAccessibilityService = accessor.get(
				...
				if (isChatEditingActionContext(item)) {
					...
					item = chatWidgetService
						.getWidgetBySessionResource(item.sessionResource)
						?.viewModel?.getItems()
						.at(-1);
				}
				if (!isResponseVM(item)) {
				...
				if (
					chatModel &&
					(mode === ChatModeKind.Edit || mode === ChatModeKind.Agent)
				) {
					...
					const shouldPrompt =
						entriesModifiedInLastRequest.length > 0 &&
					...
					const confirmation = shouldPrompt
						? await dialogService.confirm({
								...
								message:
									entriesModifiedInLastRequest.length === 1
										? localize(
								...
								cells: splitContents.map((content) => {
									const kind =
										content.type === "markdown"
											...
											: CellKind.Code;
									const language =
										content.type === "markdown" ? "markdown" : content.language;
									const mime =
										content.type === "markdown"
											? "text/markdown"
											: `text/x-${content.language}`;
									return {
										cellKind: kind,
										language,
										mime,
