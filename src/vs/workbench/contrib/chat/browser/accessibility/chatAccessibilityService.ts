	...
	implements IChatAccessibilityService
{
	...
	constructor(
		...
		super();
		this._register(
			this._widgetService.onDidBackgroundSession((e) => {
	...
	acceptResponse(
		...
		const responseContent =
			typeof response === "string" ? response : response?.response.toString();
		this._accessibilitySignalService.playSignal(
		...
		const plainTextResponse = renderAsPlaintext(
		...
		this._showOSNotification(
			widget,
			container,
			plainTextResponse + errorDetails,
		);
		if (
			!isVoiceInput ||
	...
	acceptElicitation(elicitation: IChatElicitationRequest): void {
		...
		const title =
			typeof elicitation.title === "string"
				? elicitation.title
				: elicitation.title.value;
		const message =
			typeof elicitation.message === "string"
				? elicitation.message
				: elicitation.message.value;
		alert(title + " " + message);
		this._accessibilitySignalService.playSignal(
	...
	private async _showOSNotification(
		widget: ChatWidget,
		container: HTMLElement,
		responseContent: string,
	): Promise<void> {
		const mode = this._configurationService.getValue<ChatNotificationMode>(
		...
		const title = widget?.viewModel?.model.title
			? localize("chatTitle", "Chat: {0}", widget.viewModel.model.title)
			: localize("chat.untitledChat", "Untitled Chat");
		...
		const { clicked } = await this._hostService.showToast(
			{ title, body: localize("notificationDetail", "New chat response.") },
			cts.token,
		);
