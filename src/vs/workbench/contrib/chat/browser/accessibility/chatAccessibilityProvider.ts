		...
		.map((v) => {
			...
			if (v.toolSpecificData) {
				if (v.toolSpecificData.kind === "terminal") {
					...
					input = localize(
						"modifiedFilesConfirmationInput",
						"{0} files",
						v.toolSpecificData.modifiedFiles.length,
					);
				}
			}
			const titleObj = state.confirmationMessages?.title;
			const title =
				typeof titleObj === "string" ? titleObj : titleObj?.value || "";
			return {
	...
	if (text.some((t) => t.detail)) {
		message +=
			...
			localize(
				...
				"Details: {0}",
				text.map((t) => (t.detail ? t.detail : "")).join(" "),
			);
	}
...
export class ChatAccessibilityProvider implements IListAccessibilityProvider<ChatTreeItem> {
	...
	private _getLabelWithInfo(element: IChatResponseViewModel): string {
		...
		const toolInvocation = element.response.value.filter(
			(v) => v.kind === "toolInvocation",
		);
		...
		const tableCount =
			marked
				...
				.filter((token) => token.type === "table")?.length ?? 0;
		let tableCountHint = "";
		switch (tableCount) {
			case 0:
				break;
		...
		const fileTreeCount =
			element.response.value.filter((v) => v.kind === "treeData").length ?? 0;
		...
		const elicitationCount = element.response.value.filter(
			(v) => v.kind === "elicitation2" || v.kind === "elicitationSerialized",
		);
		let elicitationHint = "";
