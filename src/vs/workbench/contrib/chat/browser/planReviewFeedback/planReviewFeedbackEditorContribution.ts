...
import {
	...
	PlanReviewFeedbackMenuId,
} from "./planReviewFeedbackEditorActions.js";
...
class PlanReviewFeedbackInputWidget implements IOverlayWidget {
	...
	constructor(private readonly _editor: ICodeEditor) {
		...
		this._actionBar.push(this._addAction, {
			icon: true,
			label: false,
			keybinding: localize("planReviewFeedback.enter", "Enter"),
		});
	...
	private _autoSize(): void {
		...
		const width = Math.max(
			PlanReviewFeedbackInputWidget._MIN_WIDTH,
			Math.min(textWidth + 10, PlanReviewFeedbackInputWidget._MAX_WIDTH),
		);
...
class PlanReviewFeedbackOverlayWidget implements IOverlayWidget {
	...
	private readonly _navigationBearings = observableValue<{
		activeIdx: number;
		totalCount: number;
	}>("planReviewFeedbackBearings", { activeIdx: -1, totalCount: 0 });
			...
			this._instaService.createInstance(
				...
				PlanReviewFeedbackMenuId,
				{
					...
					actionViewItemProvider: (action, options) => {
						...
						return new ActionViewItem(undefined, action, {
							...options,
							icon: true,
							label: false,
							keybindingNotRenderedWithLabel: true,
...
export class PlanReviewFeedbackEditorContribution
	extends Disposable
	implements IEditorContribution
{
	...
	constructor(
		...
		this._hasFeedbackContextKey =
			hasPlanReviewFeedback.bindTo(contextKeyService);
		...
		this._register(
			this._planReviewFeedbackService.onDidChangeRegistrations(() =>
				this._onModelChanged(),
			),
		);
		this._register(
			this._planReviewFeedbackService.onDidChangeFeedback(() =>
				this._updateDecorations(),
			),
		);
		this._register(
			this._planReviewFeedbackService.onDidChangeNavigation(() =>
				this._updateDecorations(),
			),
		);
	...
	private _onModelChanged(): void {
		...
		this._isActivePlan =
			!!model && this._planReviewFeedbackService.isActivePlanReview(model.uri);
		this._updateDecorations();
	}
	...
	private _registerWidgetListeners(
		widget: PlanReviewFeedbackInputWidget,
	): void {
		this._widgetListeners.clear();
	...
	private _updateDecorations(): void {
		...
		if (items.length > 0) {
			const bearings = this._planReviewFeedbackService.getNavigationBearing(
				model.uri,
			);
	...
	private _ensureOverlayWidget(): PlanReviewFeedbackOverlayWidget {
		if (!this._overlayWidget) {
			this._overlayWidget = new PlanReviewFeedbackOverlayWidget(
				this._editor,
	...
	private _showOverlayToolbar(bearings: {
		activeIdx: number;
		totalCount: number;
	}): void {
...
registerEditorContribution(
	PlanReviewFeedbackEditorContribution.ID,
	PlanReviewFeedbackEditorContribution,
