...
export interface IChatOutputItemRenderer {
	renderOutputPart(
		...
		data: Uint8Array,
		webview: IWebview,
		context: IChatOutputRenderContext,
...
export const IChatOutputRendererService =
	createDecorator<IChatOutputRendererService>("chatOutputRendererService");
...
export interface IChatOutputRendererService {
	...
	registerRenderer(
		viewType: string,
		renderer: IChatOutputItemRenderer,
	...
	renderOutputPart(
		...
		data: Uint8Array,
		parent: HTMLElement,
		webviewOptions: RenderOutputPartWebviewOptions,
		token: CancellationToken,
	): Promise<RenderedOutputPart>;

	renderCodeBlock(
		languageIdentifier: string,
		data: Uint8Array,
		parent: HTMLElement,
		webviewOptions: RenderCodeBlockWebviewOptions,
		token: CancellationToken,
	): Promise<RenderedOutputPart>;
}
...
export class ChatOutputRendererService
	extends Disposable
	implements IChatOutputRendererService
{
	...
	constructor(
		...
		this._originStore = new ExtensionKeyedWebviewOriginStore(
			"chatOutputRenderer.origins",
			storageService,
		);

		this._register(
			chatOutputRenderContributionPoint.setHandler((extensions) => {
	...
	hasCodeBlockRenderer(languageIdentifier: string): boolean {
		return Array.from(this._contributions.values()).some((value) =>
			value.codeBlockLanguageIdentifiers.some((identifier) =>
				equalsIgnoreCase(identifier, languageIdentifier),
			),
		);
	}

	async renderOutputPart(
		mime: string,
		data: Uint8Array,
		parent: HTMLElement,
		webviewOptions: RenderOutputPartWebviewOptions,
		...
		return this.doRenderOutputPart(
			rendererData,
			mime,
			data,
			{},
	...
	async renderCodeBlock(
		...
		const rendererData = await this.getRendererForCodeBlock(
			languageIdentifier,
			token,
		);
		if (token.isCancellationRequested) {
		...
		if (!rendererData) {
			throw new Error(
				`No renderer registered found for code block language identifier: ${languageIdentifier}`,
			);
		}

		return this.doRenderOutputPart(
			rendererData,
			"text/x-vscode-chat-code-block",
			data,
			{ codeBlockContext: { languageIdentifier } },
			parent,
			webviewOptions,
			token,
		);
	...
	private async doRenderOutputPart(
		...
		const webview = store.add(
			this._webviewService.createWebviewElement({
				...
				extension: rendererData.options.extension
					? rendererData.options.extension
					: undefined,
			}),
		);
		webview.setContextKeyService(
			store.add(this._contextKeyService.createScoped(parent)),
		);
		if (webviewOptions.chatSessionResource) {
			store.add(
				this.delegateScrollToChatWidget(
					webview,
					webviewOptions.chatSessionResource,
				),
			);
		}
		...
		await rendererData.renderer.renderOutputPart(
			mime,
			data,
			webview,
			context,
	...
	private delegateScrollToChatWidget(
		webview: IWebview,
		chatSessionResource: URI,
	): IDisposable {
		return webview.onDidWheel((e) => {
			this._chatWidgetService
				.getWidgetBySessionResource(chatSessionResource)
	...
	private getOrigin(rendererData: RendererEntry): string | undefined {
		return rendererData.options.extension
			? this._originStore.getOrigin(
					rendererData.viewType,
					rendererData.options.extension.id,
				)
			: undefined;
	}

	private async getRendererForMime(
		mime: string,
		token: CancellationToken,
	): Promise<RendererEntry | undefined> {
		return this.getRenderer(
			(value) => value.mimes.some((m) => matchesMimeType(m, [mime])),
			token,
		);
	}

	private async getRendererForCodeBlock(
		languageIdentifier: string,
		token: CancellationToken,
	): Promise<RendererEntry | undefined> {
		return this.getRenderer(
	...
	private async getRenderer(
		matches: (value: ContributionEntry) => boolean,
		token: CancellationToken,
	): Promise<RendererEntry | undefined> {
		...
		for (const [id, value] of this._contributions) {
			if (matches(value)) {
				await raceCancellationError(
					this._extensionService.activateByEvent(`onChatOutputRenderer:${id}`),
					token,
				);
	...
	private updateContributions(
		extensions: readonly IExtensionPointUser<
			readonly IChatOutputRendererContribution[]
		>[],
	) {
			...
			for (const contribution of extension.value) {
				if (this._contributions.has(contribution.viewType)) {
					extension.collector.error(
						`Chat output renderer with view type '${contribution.viewType}' already registered`,
					);
					continue;
				}

				const mimeTypes = contribution.mimeTypes ?? [];
				const codeBlockLanguageIdentifiers =
					contribution.codeBlockLanguageIdentifiers ?? [];
				if (!mimeTypes.length && !codeBlockLanguageIdentifiers.length) {
					extension.collector.error(
						`Chat output renderer with view type '${contribution.viewType}' must specify at least one mime type or code block language identifier`,
					);
					continue;
				}
...
type IChatOutputRendererContribution = TypeFromJsonSchema<
