...
export class CreateSlashCommandsUsageTracker extends Disposable {
			...
			this._chatService.onDidSubmitRequest((e) => {
				...
				for (const part of message.parts) {
					if (part.kind === ChatRequestSlashCommandPart.Kind) {
						...
						if (
							CreateSlashCommandsUsageTracker._isCreateSlashCommand(
								slash.slashCommand.command,
							)
						) {
							this._markUsed();
							return;
				...
				const match =
					/^\/(create-(?:instructions|prompt|agent|skill))(?:\s|$)/.exec(
						trimmed,
					);
				if (
					match &&
					CreateSlashCommandsUsageTracker._isCreateSlashCommand(match[1])
				) {
					this._markUsed();
				}
			}),
	...
	syncContextKey(contextKeyService: IContextKeyService): void {
		const used = this._storageService.getBoolean(
			CreateSlashCommandsUsageTracker._USED_CREATE_SLASH_COMMANDS_KEY,
			StorageScope.APPLICATION,
			false,
		);
		ChatContextKeys.hasUsedCreateSlashCommands
