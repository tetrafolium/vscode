	...
	implements CodeLensProvider
{
	...
	constructor(
		...
		this._register(
			CommandsRegistry.registerCommand(this.cmdId, (_accessor, ...args) => {
				...
				if (
					isITextModel(model) &&
					Range.isIRange(rangeArg) &&
					isBoolean(isStringArg) &&
					Array.isArray(toolsArg) &&
					isTarget(targetArg)
				) {
					this.updateTools(
						model as ITextModel,
	...
	async provideCodeLenses(
		...
		const codeLens: CodeLens = {
			...
			command: {
				...
				arguments: [
					model,
					toolsAttr.value.range,
					toolsAttr.value.type === "scalar",
					selectedTools,
					target,
				],
			},
	...
	private async updateTools(
		...
		this.instantiationService
			...
			.rewriteTools(model, newSelectedAfter, range, isString);
	}
}
