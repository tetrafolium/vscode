...
const PICKER_CONFIGURE_AGENTS_ACTION_ID =
	"workbench.action.chat.picker.customagents";

function createPickerConfigureAgentsActionConfig(disabled: boolean) {
	const config = {
		id: disabled
			? PICKER_CONFIGURE_AGENTS_ACTION_ID + ".disabled"
			: PICKER_CONFIGURE_AGENTS_ACTION_ID,
		title: localize2("select-agent", "Configure Custom Agents..."),
		tooltip: disabled
			? localize("managedByOrganization", "Managed by your organization")
			: undefined,
		...
		menu: {
			...
			when: disabled
				? ChatContextKeys.Modes.agentModeDisabledByPolicy
				: ChatContextKeys.Modes.agentModeDisabledByPolicy.negate(),
		},
	};
	return config;
 ...
 */
const CONFIGURE_AGENTS_ACTION_ID =
	"workbench.action.chat.configure.customagents";

function createManageAgentsActionConfig(disabled: boolean) {
	const base = {
		id: disabled
			? CONFIGURE_AGENTS_ACTION_ID + ".disabled"
			: CONFIGURE_AGENTS_ACTION_ID,
