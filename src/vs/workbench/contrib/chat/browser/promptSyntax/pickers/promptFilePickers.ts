 ...
 */
function newHelpButton(
	type: PromptsType,
): IQuickInputButton & { helpURI: URI } {
...
function isHelpButton(
	button: IQuickInputButton,
): button is IQuickInputButton & { helpURI: URI } {
...
function isPromptFileItem(
	item: IPromptPickerQuickPickItem | IQuickPickSeparator,
): item is IPromptPickerQuickPickItem & { promptFileUri: URI } {
 ...
 */
function isExtensionPromptPath(
	prompt: IPromptPath,
): prompt is IExtensionPromptPath {
	return prompt.storage === PromptsStorage.extension && !!prompt.extension;
}

type IPromptQuickPick = IQuickPick<
...
export class PromptFilePickers {
	...
	async selectPromptFile(
		...
		const quickPick: IPromptQuickPick =
		...
		try {
			...
			const activeItem =
				...
				(fileOptions.find(
					(f) =>
						f.type === "item" &&
						extUri.isEqual(f.promptFileUri, options.resource),
				) as IPromptPickerQuickPickItem | undefined);
			if (activeItem) {
		...
		return new Promise<ISelectPromptResult | undefined>((resolve) => {
			...
			const refreshItems = async () => {
				...
				const newItems = await this._createPromptPickItems(
					options,
			...
			disposables.add(
				quickPick.onDidTriggerItemButton(async (e) => {
					const shouldRefresh = await this._handleButtonClick(
						quickPick,
						e,
						options,
					);
	...
	private async _createPromptPickItems(
	...
	): Promise<(IPromptPickerQuickPickItem | IQuickPickSeparator)[]> {
		...
		let getVisibility: (p: IPromptPath) => boolean | undefined = () =>
			undefined;
		if (options.optionVisibility) {
			const disabled = this._promptsService.getDisabledPromptFiles(
				options.type,
			);
		...
		const sortByLabel = (
			items: IPromptPickerQuickPickItem[],
		): IPromptPickerQuickPickItem[] =>
			items.sort((a, b) => a.label.localeCompare(b.label));

		const locals = await this._promptsService.listPromptFilesForStorage(
			options.type,
		...
		if (locals.length) {
			result.push({
				type: "separator",
				label: localize("separator.workspace", "Workspace"),
			});
			result.push(
				...sortByLabel(
					await Promise.all(
						locals.map((l) =>
							this._createPromptPickItem(l, buttons, getVisibility(l), token),
						),
					),
				),
			);
		...
		if (options.type === PromptsType.instructions) {
			const agentInstructionUris =
				await this._promptsService.listAgentInstructions(token);
			agentInstructionFiles = agentInstructionUris.map(
				(agentInstructionFile) => {
					const folderName = this._labelService.getUriLabel(
						dirname(agentInstructionFile.uri),
						{ relative: true },
					);
					...
					return {
						...
						description:
							agentInstructionFile.type !==
							AgentInstructionFileType.copilotInstructionsMd
								? folderName
								: undefined,
		...
		if (agentInstructionFiles.length) {
			...
			result.push(
				...sortByLabel(
					await Promise.all(
						agentInstructionFiles.map((l) =>
							this._createPromptPickItem(
								l,
								agentButtons,
								getVisibility(l),
								token,
		...
		const exts = (
			await this._promptsService.listPromptFilesForStorage(
				options.type,
				PromptsStorage.extension,
				token,
			)
		).filter(isExtensionPromptPath);
		if (exts.length) {
			...
			const sortedGroupedExts = Array.from(groupedExts.entries()).sort((a, b) =>
		...
		const users = await this._promptsService.listPromptFilesForStorage(
			options.type,
			PromptsStorage.user,
			token,
		);
		if (users.length) {
			result.push({
				type: "separator",
				label: localize("separator.user", "User Data"),
			});
			result.push(
				...sortByLabel(
					await Promise.all(
						users.map((u) =>
							this._createPromptPickItem(u, buttons, getVisibility(u), token),
						),
					),
				),
			);
		...
		const plugins = await this._promptsService.listPromptFilesForStorage(
			options.type,
		...
		if (plugins.length) {
			...
			result.push({
				type: "separator",
				label: localize("separator.plugins", "Plugins"),
			});
			result.push(
				...sortByLabel(
					await Promise.all(
	...
	private async _createPromptPickItem(
		promptFile: IPromptPath,
		buttons: IQuickInputButton[] | undefined,
		visibility: boolean | undefined,
		token: CancellationToken,
	): Promise<IPromptPickerQuickPickItem> {
		const parsedPromptFile = await this._promptsService
			.parseNew(promptFile.uri, token)
			.catch(() => undefined);
		let promptName =
			(parsedPromptFile?.header?.name ?? promptFile.name) ||
			(promptFile.type === PromptsType.skill
				? getSkillFolderName(promptFile.uri)
				: getCleanPromptName(promptFile.uri));
		const promptDescription =
			parsedPromptFile?.header?.description ?? promptFile.description;
	...
	private async keepQuickPickOpen<T>(
		quickPick: IPromptQuickPick,
		work: () => Promise<T>,
	): Promise<T> {
	...
	private async _handleButtonClick(
		...
		if (button === RUN_IN_CHAT_BUTTON) {
			const commandId =
				quickPick.keyMods.ctrlCmd === true
		...
		if (button === RENAME_BUTTON || button === COPY_BUTTON) {
			return await this.keepQuickPickOpen(quickPick, async () => {
				...
				await this._instaService
					.createInstance(PromptFileRewriter)
					.openAndRewriteName(
						newFile,
						getCleanPromptName(newFile),
						CancellationToken.None,
					);
		...
		if (button === DELETE_BUTTON) {
			...
			return await this.keepQuickPickOpen(quickPick, async () => {
				...
				const message = isSkill
					? localize(
							"commands.prompts.use.select-dialog.delete-skill.confirm.message",
