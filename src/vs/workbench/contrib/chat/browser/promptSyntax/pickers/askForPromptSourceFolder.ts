...
export async function askForPromptSourceFolder(
	...
	if (resolvedFolders.length === 0) {
		await instantiationService.invokeFunction((accessor) =>
			showNoFoldersDialog(accessor, type),
		);
		return;
	}
	...
	const foldersList = resolvedFolders.map<IFolderQuickPickItem>((resolved) => {
		...
		const isDefault =
			defaultFolder && isEqual(folderUri, defaultFolder.searchRoot);
		const sourceDescription = getSourceDescription(resolved.source);
		const detail =
			existingFolder && isEqual(folderUri, existingFolder)
		...
		const basePath =
			isMultiRoot && resolved.storage === PromptsStorage.local
				? labelService.getUriLabel(folderUri, { relative: true })
				: (resolved.displayPath ??
					labelService.getUriLabel(folderUri, {
						relative: resolved.storage === PromptsStorage.local,
					}));
		const label = isDefault
			? localize("pathWithDefault", "{0} (default)", basePath)
			: basePath;

		const folder: IPromptPath = {
			uri: folderUri,
