...
export class PromptFileRewriter {
	...
	public async openAndRewriteTools(
		uri: URI,
		newTools: IToolAndToolSetEnablementMap | undefined,
		...
		const toolsAttr = promptAST.header.getAttribute(
	...
	public rewriteTools(
		model: ITextModel,
		newTools: IToolAndToolSetEnablementMap,
		range: Range,
		isString: boolean,
	): void {
		const newToolNames =
			this._languageModelToolsService.toFullReferenceNames(newTools);
		const newEntries = newToolNames
			.map((toolName) => formatArrayValue(toolName))
			.join(", ");
	...
	private rewriteAttribute(
		...
		model.pushEditOperations(
			null,
			[EditOperation.replaceMove(range, newValue)],
			() => null,
		);
		model.pushStackElement();
	}

	public async openAndRewriteName(
		uri: URI,
		newName: string,
		token: CancellationToken,
	): Promise<void> {
