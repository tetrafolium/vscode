	...
	implements IWorkbenchContribution, IURLHandler
{
	...
	async handleURL(uri: URI): Promise<boolean> {
		...
		try {
			...
			const result = await this.requestService.request(
				{
					type: "GET",
					url: urlString,
					callSite: "promptUrlHandler.resolveUrl",
				},
				CancellationToken.None,
			);
			if (result.res.statusCode !== 200) {
				this.logService.error(
					`[PromptUrlHandler] Failed to fetch URL: ${urlString}`,
				);
				this.notificationService.error(
					localize("failed", "Failed to fetch URL: {0}", urlString),
				);
				return true;
			}
			...
			const newFolder = await this.instantiationService.invokeFunction(
			...
			const newName = await this.instantiationService.invokeFunction(
				askForPromptFileName,
				promptType,
				newFolder.uri,
				getCleanPromptName(url),
			);
			if (!newName) {
				return true;
			}
			...
			await this.fileService.createFile(
				promptUri,
				VSBuffer.fromString(responseData),
			);
			...
			this.logService.error(
				`Error handling prompt URL ${uri.toString()}`,
				error,
			);
			return true;
		}
	...
	private async shouldBlockInstall(
