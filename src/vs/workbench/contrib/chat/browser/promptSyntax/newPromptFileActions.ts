			...
			SnippetController2.get(editor)?.apply([
				{
					...
					template: getDefaultContentSnippet(
						this.type,
						cleanName,
						getTarget(this.type, promptUri),
					),
				},
			]);
		}
...
function getDefaultContentSnippet(
...
class NewAgentFileAction extends AbstractNewPromptFileAction {
	constructor() {
		super(
			...
			localize("commands.new.agent.local.title", "New Custom Agent..."),
			PromptsType.agent,
		);
	}
}
...
class NewSkillFileAction extends Action2 {
	...
	public override async run(
		...
		if (
			editor &&
			editor.hasModel() &&
			isEqual(editor.getModel().uri, skillFileUri)
		) {
			SnippetController2.get(editor)?.apply([
				{
					range: editor.getModel().getFullModelRange(),
