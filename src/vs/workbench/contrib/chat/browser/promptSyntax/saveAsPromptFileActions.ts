...
class BaseSaveAsPromptFileAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, configUri?: string): Promise<void> {
		...
		const newFolder = await instantiationService.invokeFunction(
		...
		const newName = await instantiationService.invokeFunction(
			askForPromptFileName,
			this.promptType,
			newFolder.uri,
			getCleanPromptName(model.uri),
		);
		if (!newName) {
			return;
		}
...
export const SAVE_AS_PROMPT_FILE_ACTION_ID =
	"workbench.action.chat.save-as-prompt";
	...
	constructor() {
		super(
			createOptions(
				...
				localize2("promptfile.savePromptFile", "Save As Prompt File"),
				localize2(
...
export const SAVE_AS_AGENT_FILE_ACTION_ID =
	"workbench.action.chat.save-as-agent";
	...
	constructor() {
		super(
			createOptions(
				...
				localize2("promptfile.saveAgentFile", "Save As Agent File"),
				localize2("promptfile.saveAgentFile.description", "Save as agent file"),
				AGENT_LANGUAGE_ID,
			),
			PromptsType.agent,
		);
...
export const SAVE_AS_INSTRUCTIONS_FILE_ACTION_ID =
	"workbench.action.chat.save-as-instructions";
