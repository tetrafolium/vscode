...
function findNthCommandNode(
	...
	if (
		!hookTypeArray ||
		hookTypeArray.type !== "array" ||
		!hookTypeArray.children
	) {
		return undefined;
	}
	...
	for (let i = 0; i < hookTypeArray.children.length; i++) {
		...
		const nestedHooksNode = findNodeAtLocation(tree, [
			"hooks",
			hookType,
			i,
			"hooks",
		...
		if (
			nestedHooksNode &&
			nestedHooksNode.type === "array" &&
			nestedHooksNode.children
		) {
			// Iterate through nested hooks
			for (let j = 0; j < nestedHooksNode.children.length; j++) {
				if (currentIndex === targetIndex) {
					return findNodeAtLocation(tree, [
						"hooks",
						hookType,
						i,
						"hooks",
 ...
 */
export function findHookCommandSelection(
	content: string,
	hookType: string,
	index: number,
	fieldName: string,
): ITextEditorSelection | undefined {
 ...
 */
export function findHookCommandInYaml(
	content: string,
	commandText: string,
): ITextEditorSelection | undefined {
	const commandFieldKeys = [
		"command",
		"windows",
		"linux",
		"osx",
...
export async function parseAllHookFiles(
	...
	const hookFiles = await promptsService.listPromptFiles(
		PromptsType.hook,
		token,
	);
		...
		try {
			...
			for (const [hookType, { hooks: commands, originalId }] of hooks) {
				...
				for (let i = 0; i < commands.length; i++) {
					...
					const commandLabel =
						formatHookCommandLabel(command, os) ||
						nls.localize("commands.hook.emptyCommand", "(empty command)");
					parsedHooks.push({
						hookType,
						...
						filePath: labelService.getUriLabel(hookFile.uri, {
							relative: true,
						}),
						index: i,
			...
			console.error(
				"Failed to read or parse hook file",
				hookFile.uri.toString(),
				error,
			);
		}
	}
	...
	if (options?.additionalDisabledFileUris) {
		for (const uri of options.additionalDisabledFileUris) {
			try {
				...
				const { hooks } = parseHooksIgnoringDisableAll(
					uri,
					json,
					workspaceRootUri,
					userHome,
				...
				console.error(
					"Failed to read or parse disabled hook file",
					uri.toString(),
					error,
				);
			}
