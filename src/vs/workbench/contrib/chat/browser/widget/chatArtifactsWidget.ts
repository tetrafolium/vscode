...
function isGroupNode(
	element: ArtifactTreeElement,
): element is IArtifactGroupNode {
	return element.kind === "group";
}

function isLeafNode(
	element: ArtifactTreeElement,
): element is IArtifactLeafNode {
...
export class ChatArtifactsWidget extends Disposable {
	...
	private readonly _sessionResource = observableValue<URI | undefined>(
		this,
		undefined,
	);
	...
	private readonly _treeData = derived(this, (reader) => {
		...
		const treeElements = buildTreeElementsFromGroups(
			groups,
			multiSource,
			(source) => this._clearSource(source),
		);
		const visibleCount = countVisibleRows(treeElements);
		const itemsShown = Math.min(
			visibleCount,
			ChatArtifactsWidget.MAX_ITEMS_SHOWN,
		);
		return {
			totalCount,
			treeElements,
	...
	constructor(
		...
		private readonly _chatImageCarouselService: IChatImageCarouselService,
	) {
		super();
		...
		this._register(
			autorun((reader) => {
				...
				const headerButton = store.add(
					new Button(expandoContainer, { supportIcons: true }),
				);
				...
				const tree = store.add(
					this._instantiationService.createInstance(
						...
						[
							...
							new ChatArtifactLeafRenderer((artifact) =>
								this._saveArtifact(artifact),
							),
						],
						{
				...
				store.add(
					headerButton.onDidClick(() => {
						this._isCollapsed.set(
							!this._isCollapsed.read(undefined),
							undefined,
						);
					}),
				);
				...
				store.add(
					autorun((reader) => {
						...
						headerButton.element.setAttribute(
							"aria-expanded",
							String(!collapsed),
						);
	...
	private async _openGroupInCarousel(group: IArtifactGroupNode): Promise<void> {
		...
		if (first?.uri) {
			await this._chatImageCarouselService.openCarouselAtResource(
				URI.parse(first.uri),
			);
		}
	}
	...
	private _openLeafArtifact(artifact: IChatArtifact): void {
		if (
			...
			if (ChatMemoryFileResource.isChatMemoryFileUri(uri)) {
				...
				const editorOverride = getEditorOverrideForChatResource(
					uri,
	...
	private async _openScreenshotInCarousel(
		clicked: IChatArtifact,
	): Promise<void> {
		if (clicked.uri) {
			await this._chatImageCarouselService.openCarouselAtResource(
				URI.parse(clicked.uri),
			);
		}
	}
	...
	private async _openMemoryFileArtifact(uri: URI): Promise<void> {
		...
		const resolvedUriStr: string | undefined =
...
function sourceDisplayName(source: ArtifactSource): string {
	switch (source.kind) {
		...
		case "subagent":
			return (
				source.name ?? localize("chat.artifacts.source.subagent", "Subagent")
			);
	}
}

function buildTreeElementsFromGroups(
	sourceGroups: readonly IArtifactSourceGroup[],
	multiSource: boolean,
	onClearSource: (source: ArtifactSource) => void,
): IObjectTreeElement<ArtifactTreeElement>[] {
	const elements: IObjectTreeElement<ArtifactTreeElement>[] = [];

	for (const sourceGroup of sourceGroups) {
		const prefix = multiSource
			? sourceDisplayName(sourceGroup.source)
			: undefined;
		const clearable = sourceGroup.source.kind !== "rules";
		const onClear = clearable
			? () => onClearSource(sourceGroup.source)
			: undefined;
		const groups = new Map<
			string,
			{
				config: { groupName: string; onlyShowGroup: boolean };
		...
		for (const [, group] of groups) {
			...
			if (group.config.onlyShowGroup) {
				...
				elements.push({
					...
					collapsible: true,
					collapsed: false,
					children: group.artifacts.map(
						(a): IObjectTreeElement<ArtifactTreeElement> => ({
							element: { kind: "leaf", artifact: a },
						}),
					),
				});
			}
...
function countVisibleRows(
	elements: IObjectTreeElement<ArtifactTreeElement>[],
): number {
	let count = 0;
...
class ChatArtifactsAccessibilityProvider implements IListAccessibilityProvider<ArtifactTreeElement> {
	getAriaLabel(element: ArtifactTreeElement): string | null {
		if (isGroupNode(element)) {
			return localize(
				"chat.artifacts.group.aria",
				"{0} ({1} items)",
				element.groupName,
				element.artifacts.length,
			);
		}
...
class ChatArtifactGroupRenderer implements ITreeRenderer<
	ArtifactTreeElement,
	void,
	IArtifactGroupTemplate
> {
	...
	renderTemplate(container: HTMLElement): IArtifactGroupTemplate {
		...
		const actionsContainer = dom.append(
			row,
			dom.$(".chat-artifacts-list-actions"),
		);
	...
	renderElement(
		node: ITreeNode<ArtifactTreeElement>,
		_index: number,
		templateData: IArtifactGroupTemplate,
	): void {
		...
		const icon =
			(firstType && ARTIFACT_TYPE_ICONS[firstType]) || Codicon.archive;
		templateData.iconElement.className =
			"chat-artifacts-list-icon " + ThemeIcon.asClassName(icon);
	...
	disposeElement(
		_element: ITreeNode<ArtifactTreeElement>,
		_index: number,
		templateData: IArtifactGroupTemplate,
	): void {
		templateData.elementDisposables.clear();
...
class ChatArtifactLeafRenderer implements ITreeRenderer<
	ArtifactTreeElement,
	void,
	IArtifactLeafTemplate
> {
	...
	renderTemplate(container: HTMLElement): IArtifactLeafTemplate {
		...
		const descriptionElement = dom.append(
			row,
			dom.$(".chat-artifacts-list-description"),
		);
		const actionsContainer = dom.append(
			row,
			dom.$(".chat-artifacts-list-actions"),
	...
	renderElement(
		node: ITreeNode<ArtifactTreeElement>,
		_index: number,
		templateData: IArtifactLeafTemplate,
	): void {
		...
		const icon =
			(artifact.type && ARTIFACT_TYPE_ICONS[artifact.type]) || Codicon.archive;
		templateData.iconElement.className =
			"chat-artifacts-list-icon " + ThemeIcon.asClassName(icon);
