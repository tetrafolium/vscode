...
export class ChatTipContentPart extends Disposable {
	...
	private readonly _toolbar = this._register(
		new MutableDisposable<MenuWorkbenchToolBar>(),
	);
	...
	constructor(
		...
		this.domNode.setAttribute(
			...
			localize("chatTipRoleDescription", "tip"),
		);

		this._inChatTipContextKey = ChatContextKeys.inChatTip.bindTo(
			this._contextKeyService,
		);
		this._multipleChatTipsContextKey = ChatContextKeys.multipleChatTips.bindTo(
			this._contextKeyService,
		);
		const focusTracker = this._register(dom.trackFocus(this.domNode));
		this._register(
			focusTracker.onDidFocus(() => this._inChatTipContextKey.set(true)),
		);
		this._register(
			focusTracker.onDidBlur(() => this._inChatTipContextKey.set(false)),
		);
		this._register({
			dispose: () => {
		...
		this._register(
			this._chatTipService.onDidDismissTip(() => {
				this._onDidHide.fire();
			}),
		);

		this._register(
			this._chatTipService.onDidNavigateTip((tip) => {
				this._renderTip(tip);
		...
		this._register(
			this._chatTipService.onDidHideTip(() => {
				this._onDidHide.fire();
			}),
		);

		this._register(
			this._chatTipService.onDidDisableTips(() => {
				this._onDidHide.fire();
			}),
		);
	...
	private _renderTip(tip: IChatTip): void {
		...
		this._toolbar.clear();
		this._multipleChatTipsContextKey.set(
			this._chatTipService.hasMultipleTips(),
		);
		...
		this._toolbar.value = this._instantiationService.createInstance(
			MenuWorkbenchToolBar,
			toolbarContainer,
			MenuId.ChatTipToolbar,
			{
				menuOptions: {
		...
		const textContent =
			markdownContent.element.textContent ?? localize("chatTip", "Chat tip");
	...
	private async _handleTipAction(
		link: string,
		mdStr: IMarkdownString,
	): Promise<void> {
		if (link.startsWith("command:") && this._shouldTriggerSetup()) {
			const setupSucceeded = await this._commandService.executeCommand<
				boolean | undefined
...
registerAction2(
	class PreviousTipAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class NextTipAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class DismissTipToolbarAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class DismissTipAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class DisableTipsAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class ResetDismissedTipsAction extends Action2 {
		constructor() {
			super({
