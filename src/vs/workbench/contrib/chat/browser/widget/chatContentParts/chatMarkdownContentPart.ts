	...
	implements IChatContentPart
{
	...
	constructor(
		...
		@IChatOutputRendererService
		private readonly chatOutputRendererService: IChatOutputRendererService,
	) {
		super();
		...
		const doRenderMarkdown = () => {
			...
			for (const ref of this.allRefs) {
				if (ref.object instanceof ChatOutputCodeBlockPart) {
					const outputRef =
						ref as IDisposableReference<ChatOutputCodeBlockPart>;
	...
	private renderChatOutputCodeBlock(
		...
		context: IChatContentPartRenderContext,
		isComplete: boolean,
		reusableOutputCodeBlockRefs: Map<
			string,
			IDisposableReference<ChatOutputCodeBlockPart>
		>,
	): IDisposableReference<ChatOutputCodeBlockPart> {
		const reuseKey = ChatOutputCodeBlockPart.reuseKey(
			context.element.id,
			codeBlockIndex,
			identifier,
		);
	...
	hasSameContent(other: IChatProgressRenderableResponseContent): boolean {
		...
		if (
			...
			equalsInlineReferences(
				other.inlineReferences,
				this.markdown.inlineReferences,
			)
		) {
			return true;
		}
	...
	onDidRemount(): void {
		for (const ref of this.allRefs) {
			if (
				...
				ref.object instanceof ChatOutputCodeBlockPart
			) {
...
function equalsInlineReferences(
	a: Record<string, IChatContentInlineReference> | undefined,
	b: Record<string, IChatContentInlineReference> | undefined,
): boolean {
	if (a === b) {
		return true;
	}
...
function equalsInlineReference(
	a: IChatContentInlineReference | undefined,
	b: IChatContentInlineReference | undefined,
): boolean {
	if (!a || !b) {
		return !a && !b;
	}
...
type WorkspaceSymbolInlineReference = Extract<
	InlineReferenceValue,
	{ name: string; location: unknown }
>;
type WorkspaceSymbolComparer = (
	a: WorkspaceSymbolInlineReference,
	b: WorkspaceSymbolInlineReference,
) => boolean;

const workspaceSymbolComparers: {
	readonly [K in keyof WorkspaceSymbolInlineReference]-?: WorkspaceSymbolComparer;
} = {
	...
	location: (a, b) =>
		isEqual(a.location.uri, b.location.uri) &&
		Range.equalsRange(a.location.range, b.location.range),
};

const workspaceSymbolComparerKeys = Object.keys(
	workspaceSymbolComparers,
) as (keyof WorkspaceSymbolInlineReference)[];

function equalsInlineReferenceValue(
	a: InlineReferenceValue,
	b: InlineReferenceValue,
): boolean {
...
function equalsWorkspaceSymbol(
	a: WorkspaceSymbolInlineReference,
	b: WorkspaceSymbolInlineReference,
): boolean {
	return workspaceSymbolComparerKeys.every((key) =>
		workspaceSymbolComparers[key](a, b),
	);
}

function equalsSymbolTags(
	a: readonly SymbolTag[] | undefined,
	b: readonly SymbolTag[] | undefined,
): boolean {
	if (a === b) {
		return true;
	}
...
class ChatOutputCodeBlockPart extends Disposable {
	...
	private readonly _renderedOutputPart = this._register(
		new MutableDisposable<RenderedOutputPart>(),
	);
	...
	constructor(
		...
		super();
		this.reuseKey = ChatOutputCodeBlockPart.reuseKey(
			context.element.id,
			codeBlockIndex,
			identifier,
		);
		...
		this.element = $(
			".interactive-result-code-block.chat-output-code-block.tool-output-part",
		);
		...
		const partState: IOutputPartState = this.stateCache.get(stateCacheKey) ?? {
			height: 0,
		};
		...
		this.chatOutputRendererService
			.renderCodeBlock(
				identifier,
	...
	hasSameContent(
		...
		return (
			identifier.toLowerCase() === this.identifier.toLowerCase() &&
			text === this.text &&
		...
		this.progressStore.add(
			autorun((r) => {
				if (isStreaming.read(r)) {
					...
					this.setStatus(
						codicon,
						localize("chat.codeblock.applyingEdits", "Applying edits"),
					);
					const entry = editSession.readEntry(uri, r);
					...
					this.setStatus(
						Codicon.check,
						localize("chat.codeblock.edited", "Edited"),
					);
					this.setProgressFill(undefined);
					this.setLabelDetail("");
				}
