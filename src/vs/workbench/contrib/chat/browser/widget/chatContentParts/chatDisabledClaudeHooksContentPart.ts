...
export class ChatDisabledClaudeHooksContentPart
	extends Disposable
	implements IChatContentPart
{
	...
	constructor(
		...
		const enableLink = createMarkdownCommandLink({
			...
			tooltip: localize(
				"chat.disabledClaudeHooks.enableLink.tooltip",
				"Open settings to enable Claude Code hooks",
			),
		});
		...
		const rendered = this._register(
			this._markdownRendererService.render(content, {
				actionHandler: (href) =>
