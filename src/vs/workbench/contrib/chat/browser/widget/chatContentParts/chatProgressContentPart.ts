	...
	implements IChatContentPart
{
	...
	private readonly renderedMessage = this._register(
	...
	constructor(
		...
		this.showSpinner =
			forceShowSpinner ?? shouldShowSpinner(followingContent, context.element);
		this.isHidden =
		...
		const isLoadingIcon =
			icon &&
			ThemeIcon.isEqual(icon, ThemeIcon.modify(Codicon.loading, "spin"));
	...
	updateMessage(content: IMarkdownString): void {
		...
		const result = this._register(
			this.chatContentMarkdownRenderer.render(content),
		);
		result.element.classList.add("progress-step");
	...
	hasSameContent(
		...
		if (
			other.kind === "progressMessage" &&
			other.content.value !== this.currentContent.value
		) {
			return false;
		}
...
export class ChatProgressSubPart extends Disposable {
	...
	constructor(
		...
		if (tooltip) {
			this._register(
				hoverService.setupDelayedHover(iconElement, {
					content: tooltip,
					style: HoverStyle.Pointer,
				}),
			);
			this._register(
				hoverService.setupDelayedHover(messageElement, {
					content: tooltip,
					style: HoverStyle.Pointer,
				}),
			);
	...
	implements IChatContentPart
{
	...
	constructor(
		...
		const messageContainer = $(
			"div.rendered-markdown.progress-step.chat-working-progress-step",
		);
		this.labelElement = $("p");
		...
		this._register(
			languageModelToolsService.onDidPrepareToolCallBecomeUnresponsive((e) => {
				if (isEqual(context.element.sessionResource, e.sessionResource)) {
					this.updateWorkingContent(
						new MarkdownString(
							localize(
								"toolCallUnresponsive",
								"Waiting for tool '{0}' to respond...",
								e.toolData.displayName,
							),
						),
					);
				}
