...
export class ToolRiskBadgeWidget extends Disposable {
	...
	constructor(@IHoverService private readonly _hoverService: IHoverService) {
		...
		this._detailsIconEl.classList.add(
			...ThemeIcon.asClassNameArray(Codicon.info),
		);
		this._detailsIconEl.tabIndex = 0;
		this._detailsIconEl.setAttribute("role", "button");
		this._detailsIconEl.setAttribute(
			"aria-label",
			localize("toolRisk.detailsIconLabel", "Risk assessment details"),
		);
	...
	private _buildDetailsMarkdown(): IMarkdownString {
		...
		const md = new MarkdownString(undefined, {
			...
			isTrusted:
				typeof details === "object" && details ? details.isTrusted : undefined,
		});
