...
export class ChatModifiedFilesConfirmationSubPart extends AbstractToolConfirmationSubPart {
		...
		const confirmWidget = this._register(
			this.instantiationService.createInstance(
				...
				this.context,
				{
					title: this.getTitle(),
					icon:
						tool?.icon && hasKey(tool.icon, { id: true })
							? tool.icon
							: Codicon.tools,
					subtitle:
						typeof toolInvocation.originMessage === "string"
							? toolInvocation.originMessage
							: toolInvocation.originMessage?.value,
					buttons: this.createButtons(data.options),
					message: this.createWidgetContentElement(
	...
	private createWidgetContentElement(
		...
		data: IChatModifiedFilesConfirmationData,
	): HTMLElement {
	...
	private createModifiedFilesElement(
		...
		const overview = dom.append(
			container,
			dom.$(".chat-editing-session-overview"),
		);
		const title = dom.append(overview, dom.$(".working-set-title"));
		const titleButton = this._register(
			new ButtonWithIcon(title, {
		...
		const actions = dom.append(
			overview,
			dom.$(".chat-editing-session-actions"),
		);
		const countsContainer = dom.$(".working-set-line-counts");
		const addedSpan = dom.append(
			countsContainer,
			dom.$(".working-set-lines-added"),
		);
		const removedSpan = dom.append(
			countsContainer,
		...
		if (hasDiffStats) {
			...
			titleButton.element.setAttribute(
				"aria-label",
				localize(
		...
		const viewAllChangesButton = this._register(
			new Button(actions, {
				...defaultButtonStyles,
				secondary: true,
				small: true,
		...
		this._register(
			viewAllChangesButton.onDidClick(async () => {
		...
		const listItems = data.modifiedFiles.map<IChatCollapsibleListItem>(
			(file) => {
				const resource = URI.revive(file.uri);
				const originalUri = file.originalUri
					? URI.revive(file.originalUri)
					: undefined;
				const modifiedContentUri = file.modifiedContentUri
					? URI.revive(file.modifiedContentUri)
					: undefined;
				const originalContentUri = file.originalContentUri
					? URI.revive(file.originalContentUri)
					: undefined;
				return {
					kind: "reference",
					reference: resource,
		...
		const workingSetContainer = dom.append(
			container,
			dom.$(".chat-editing-session-list.collapsed"),
		);
		list.layout(height);
		...
		this._register(
			dom.addDisposableListener(overview, "click", (e) => {
	...
	private async openAllChanges(
		...
		await this.commandService.executeCommand("_workbench.openMultiDiffEditor", {
			...
			resources: data.modifiedFiles.map((file) => ({
				originalUri: file.originalContentUri
					? URI.revive(file.originalContentUri)
					: file.originalUri
						? URI.revive(file.originalUri)
						: undefined,
				modifiedUri: file.modifiedContentUri
					? URI.revive(file.modifiedContentUri)
					: URI.revive(file.uri),
			})),
		});
