...
export class CodeBlockPart extends Disposable {
	...
	private get verticalPadding(): number {
		return (
			this.currentCodeBlockData?.renderOptions?.verticalPadding ??
	...
	constructor(
		...
		this._register(
			dom.addDisposableListener(this.element, "mouseleave", () => {
				this._isHovered = false;
	...
	private createEditor(
		...
		return this._register(
			instantiationService.createInstance(CodeEditorWidget, parent, options, {
	...
	private getEditorOptionsFromConfig(): IEditorOptions {
		...
		const scrollbar: IEditorOptions["scrollbar"] | undefined =
			renderOptions?.maxHeightInLines
				? {
						vertical: "auto",
		...
		return {
			...
			fontLigatures:
				this.editorOptions.configuration.resultEditor.fontLigatures,
			bracketPairColorization:
				this.editorOptions.configuration.resultEditor.bracketPairColorization,
			fontFamily:
				this.editorOptions.configuration.resultEditor.fontFamily === "default"
	...
	layout(width = this.lastLayoutWidth): void {
		...
		width =
			width -
			editorBorder -
			(this.currentCodeBlockData?.renderOptions?.reserveWidth ?? 0);
		// !!!!
		...
		this.editor.layout(
			{
				width: isRequestVM(this.currentCodeBlockData?.element)
					? width * 0.9
					: width,
				height,
			},
	...
	private setLanguage(languageId: string): void {
		const vscodeLanguageId =
			this.languageService.getLanguageIdByLanguageName(languageId);
		if (
			vscodeLanguageId &&
			vscodeLanguageId !== this._textModel.getLanguageId()
		) {
			this._textModel.setLanguage(vscodeLanguageId);
		} else if (
			!vscodeLanguageId &&
			this._textModel.getLanguageId() !== PLAINTEXT_LANGUAGE_ID
		) {
...
export class CodeCompareBlockPart extends Disposable {
	...
	private async updateEditor(
		...
		this.toolbar.context = {
			...
			toggleDiffViewMode: () => {
				const isCurrentlyInline = !!this.diffEditor
					.getModifiedEditor()
					.contextKeyService.getContextKeyValue(
						EditorContextKeys.diffEditorInlineMode.key,
					);
