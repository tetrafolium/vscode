	...
	implements IChatContentPart
{
	...
	private readonly interactionMd = this._register(
		new MutableDisposable<IRenderedMarkdown>(),
	);
	private readonly showSpecificServersScheduler = this._register(
		new RunOnceScheduler(
			() => this.updateDetailedProgress(this.data.state!.get()),
			2500,
		),
	);
	private readonly previousParts = new Lazy(() => {
		...
		return this.context.element.session
			.getItems()
			.filter(
				(r, i): r is IChatResponseViewModel =>
					isResponseVM(r) && i < this.context.elementIndex,
			)
			.flatMap((i) =>
				i.response.value.filter((c) => c.kind === "mcpServersStarting"),
			)
			.map((p) => p.state?.get());
	});
	...
	private updateForState(state: IAutostartResult): void {
		...
		const requiringInteraction = state.serversRequiringInteraction.filter(
			(s) => {
				...
				if (
					this.previousParts.value.some((p) =>
						p?.serversRequiringInteraction.some((s2) => s.id === s2.id),
					)
				) {
					return false;
				}
		...
		if (requiringInteraction.length > 0) {
			if (!this.interactionMd.value) {
				...
				this.updateInteractionRequired(
					this.interactionMd.value.element,
	...
	private updateDetailedProgress(state: IAutostartResult): void {
		...
		if (state.starting.length === 0) {
			content = new MarkdownString(undefined, { isTrusted: true })
				.appendText(
					localize("mcp.working.mcp", "Activating MCP extensions...") + " ",
				)
				.appendMarkdown(skipText);
		} else {
		...
		if (this.workingProgressPart) {
			...
			this.workingProgressPart = this._register(
	...
	private renderInteractionRequired(
		...
		const { messageMd } = this.createInteractionMessage(
	...
	private createInteractionMessage(
		...
		const messageMd = (this.interactionMd.value =
			this._markdownRendererService.render(str, {
		...
		const startLink = [...messageMd.element.querySelectorAll("a")].find(
			(a) => !a.getAttribute("data-href")?.startsWith("command:"),
		);
		if (!startLink) {
			// Should not happen
		...
		try {
			...
			for (let i = 0; i < serversToStart.length; i++) {
				...
				const server = this.mcpService.servers
					.get()
					.find((s) => s.definition.id === serverInfo.id);
				if (server) {
