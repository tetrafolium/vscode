...
export class ChatPlanReviewPart extends Disposable implements IChatContentPart {
	...
	private readonly _messageContentDisposables = this._register(
	...
	private readonly _planReviewRegistration = this._register(
		new MutableDisposable(),
	);
	private readonly _commentRowDisposables = this._register(
		new DisposableStore(),
	);
	...
	private readonly _messageEditorDisposables = this._register(
		new DisposableStore(),
	);
	...
	constructor(
		...
		@IPlanReviewFeedbackService
		private readonly _planReviewFeedbackService: IPlanReviewFeedbackService,
		...
		this._selectedAction =
			review.actions.find((a) => a.default) ?? review.actions[0];

		if (
			review instanceof ChatPlanReviewData &&
			typeof review.draftCollapsed === "boolean"
		) {
		...
		if (review.planUri && review.canProvideFeedback && !this._isSubmitted) {
			...
			registrationStore.add(
				this._planReviewFeedbackService.registerPlanReview(
					planUri,
					(result) => {
			...
			registrationStore.add(
				this._planReviewFeedbackService.onDidChangeFeedback((uri) => {
					// Match the plan URI or the editing URI used while the editor is mounted.
					const uriString = uri.toString();
					if (
						uriString === planUriString ||
						uriString === this._editingUri?.toString()
					) {
		...
		const elements = dom.h(
			...
			[
				dom.h(".chat-confirmation-widget2.chat-plan-review@root", [
					dom.h(
						".chat-confirmation-widget-title.chat-plan-review-title@title",
						[
					...
					dom.h(
						".chat-confirmation-widget-message.chat-plan-review-body@message",
					),
		...
		this.domNode.setAttribute(
			"aria-label",
			localize("chat.planReview.ariaLabel", "Plan review: {0}", review.title),
		);
		...
		this._register(
			this._hoverService.setupDelayedHover(elements.titleLabel, {
		...
		if (review.planUri) {
			...
			const reviewButton = this._register(
				new Button(this._titleActionsEl, {
					...defaultButtonStyles,
					secondary: true,
					supportIcons: true,
			...
			reviewButton.element.classList.add(
				"chat-plan-review-title-button",
				"chat-plan-review-review-button",
			);
			this._reviewButton = reviewButton;
			this._register(
				reviewButton.onDidClick(() => {
		...
		this._restoreButton = this._register(
			new Button(this._titleActionsEl, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
		...
		this._restoreButton.element.classList.add(
			"chat-plan-review-title-button",
		...
		this._collapseButton = this._register(
			new Button(this._titleActionsEl, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
		...
		this._collapseButton.element.classList.add(
			"chat-plan-review-title-button",
			"chat-plan-review-title-icon-button",
		);
		this._register(
			this._collapseButton.onDidClick(() => this.toggleCollapsed()),
		);
		...
		this._messageScrollable = this._register(
			new DomScrollableElement(this._messageEl, {
		...
		messageParent.insertBefore(
			this._messageScrollable.getDomNode(),
			messageNextSibling,
		);
		...
		this._register(
			resizeObserver.observe(this._messageScrollable.getDomNode()),
		);
		...
		if (
			this._feedbackTextarea &&
			review instanceof ChatPlanReviewData &&
			review.draftFeedback
		) {
	...
	private renderMarkdown(): void {
		...
		const rendered = store.add(
			this._markdownRendererService.render(
				new MarkdownString(this.review.content, {
	...
	private renderFeedback(section: HTMLElement): void {
		...
		const header = dom.append(
			section,
			dom.$(".chat-plan-review-feedback-header"),
		);
		...
		const headerActions = dom.append(
			header,
			dom.$(".chat-plan-review-feedback-header-actions"),
		);
		...
		if (this.review.planUri) {
			...
			const clearAllButton = this._register(
				new Button(headerActions, {
					...defaultButtonStyles,
					secondary: true,
					supportIcons: true,
					title: clearAllLabel,
			...
			clearAllButton.element.classList.add(
				"chat-plan-review-title-button",
				"chat-plan-review-feedback-clear-all",
			);
			clearAllButton.label = clearAllLabel;
			this._register(
				clearAllButton.onDidClick(() => this.clearAllInlineFeedback()),
			);
		...
		if (this.review.planUri) {
			...
			const closeButton = this._register(
				new Button(headerActions, {
					...defaultButtonStyles,
					secondary: true,
					supportIcons: true,
		...
		this._commentsListScrollable = this._register(
			new DomScrollableElement(this._commentsListEl, {
				vertical: ScrollbarVisibility.Auto,
				horizontal: ScrollbarVisibility.Hidden,
				consumeMouseWheelIfScrollbarIsNeeded: true,
			}),
		);
		this._commentsListScrollable
			.getDomNode()
			.classList.add("chat-plan-review-comments-list-scrollable");
		dom.append(section, this._commentsListScrollable.getDomNode());
		dom.hide(this._commentsListScrollable.getDomNode());
		this.renderCommentsList();

		const textarea = dom.append(
			section,
			dom.$<HTMLTextAreaElement>("textarea.chat-plan-review-feedback-textarea"),
		);
		textarea.rows = 1;
		...
		this._register(
			dom.addDisposableListener(textarea, dom.EventType.INPUT, () => {
				autoResize();
	...
	private renderCommentsList(): void {
		...
		for (const item of items) {
			const row = dom.append(
				...
				dom.$(".chat-plan-review-comment-row"),
			);
			...
			const revealButton = dom.append(
				row,
				dom.$<HTMLButtonElement>("button.chat-plan-review-comment-reveal"),
			);
			revealButton.type = "button";
			revealButton.setAttribute("aria-label", rowLabel);

			const lineEl = dom.append(
				revealButton,
				dom.$("span.chat-plan-review-comment-line"),
			);
			...
			const textEl = dom.append(
				revealButton,
				dom.$("span.chat-plan-review-comment-text"),
			);
			textEl.textContent = item.text;

			this._commentRowDisposables.add(
				dom.addDisposableListener(revealButton, dom.EventType.CLICK, () => {
			...
			const removeButton = dom.append(
				row,
				dom.$<HTMLButtonElement>("button.chat-plan-review-comment-remove"),
			);
			...
			this._commentRowDisposables.add(
				dom.addDisposableListener(removeButton, dom.EventType.CLICK, (e) => {
					e.stopPropagation();
	...
	private getInlineFeedbackItems(): readonly IPlanReviewFeedbackItem[] {
		const editingItems = this._editingUri
			? this._planReviewFeedbackService.getFeedback(this._editingUri)
			: [];
		const planItems = this.review.planUri
			? this._planReviewFeedbackService.getFeedback(
					URI.revive(this.review.planUri),
				)
			: [];
	...
	private async revealInlineComment(
		itemId: string,
		line: number,
		column: number,
	): Promise<void> {
	...
	private onInlineFeedbackChanged(): void {
		...
		if (
			items.length > 0 &&
			!this._suppressFeedbackModeAutoOpen &&
	...
	private renderCurrentActionButtons(): void {
		...
		const target = this._isCollapsed
			? this._inlineActionsEl
			: this._footerButtonsEl;
		const other = this._isCollapsed
			? this._footerButtonsEl
			: this._inlineActionsEl;
		dom.clearNode(other);
	...
	private renderActionButtons(
		...
		if (this._isFeedbackMode) {
			...
			this._buttonStore.add(
				submitButton.onDidClick(() => void this.submitFeedback()),
			);
			...
			if (includeReject) {
				...
				this._buttonStore.add(
					rejectButton.onDidClick(() => this.submitRejection()),
				);
			}
			return;
		}
		...
		if (moreActions.length > 0) {
			...
			approveButton = new Button(container, {
				...defaultButtonStyles,
				supportIcons: true,
			});
		}
		...
		this._buttonStore.add(
			approveButton.onDidClick(() => this.submitApproval(primary)),
		);
	...
	private updateCollapsedPresentation(): void {
		this.domNode.classList.toggle(
			"chat-plan-review-collapsed",
			this._isCollapsed,
		);
		this._restoreButton.element.classList.toggle(
			"chat-plan-review-hidden",
			this._isCollapsed,
		);
		...
		if (this._reviewButton) {
			...
			this._reviewButton.element.classList.toggle(
				"chat-plan-review-title-icon-button",
				isIconOnly,
			);
			let label: string;
			...
			if (isIconOnly) {
				...
				const fileName = this.review.planUri
					? basename(URI.revive(this.review.planUri))
					: "";
	...
	private updateExpandedPresentation(): void {
		this.domNode.classList.toggle(
			"chat-plan-review-expanded",
			this._isExpanded && !this._isCollapsed,
		);
	...
	private async submitApproval(action: IChatPlanApprovalAction): Promise<void> {
		...
		const textareaFeedback = ridesAlong
			? this._feedbackTextarea?.value.trim()
			: undefined;
		this._options.onSubmit({
			...
			rejected: false,
			...(textareaFeedback
				? { feedback: textareaFeedback, feedbackOverall: textareaFeedback }
				: {}),
		});
		void this.markUsed();
	}
	...
	private async enterFeedbackMode(options?: {
		focus?: boolean;
	}): Promise<void> {
		if (this._isFeedbackMode) {
			if (options?.focus) {
	...
	private async mountInlineEditor(mountId: number): Promise<void> {
		...
		this._messageEditorDisposables.add(
			toDisposable(() => editor.setModel(null)),
		);
		...
		const editingRegistration =
			this._planReviewFeedbackService.registerPlanReview(editingUri, () => {
				void this.submitFeedback();
			});
		...
		const preExistingComments =
			this._planReviewFeedbackService.getFeedback(planUri);
		if (preExistingComments.length > 0) {
			for (const item of preExistingComments) {
				this._planReviewFeedbackService.addFeedback(
					editingUri,
					item.line,
					item.column,
					item.text,
	...
	private canContinueInlineEditorMount(mountId: number): boolean {
		return (
			this._inlineEditorMountId === mountId &&
			!this._messageEditor &&
			this._isFeedbackMode &&
			!this._isSubmitted &&
	...
	private async unmountInlineEditor(options?: {
		writeBack?: boolean;
	}): Promise<void> {
		...
		const model = editingUri
			? this._modelService.getModel(editingUri)
			: undefined;
		const planUri = this.review.planUri
			? URI.revive(this.review.planUri)
			: undefined;
