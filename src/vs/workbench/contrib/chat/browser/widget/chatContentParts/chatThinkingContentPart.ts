 ...
 */
export function getEffectiveThinkingDisplayMode(
	configurationService: IConfigurationService,
	contextKeyService: IContextKeyService,
): ThinkingDisplayMode {
	if (
		contextKeyService.getContextKeyValue<boolean>(
			SESSIONS_IS_PHONE_LAYOUT_KEY,
		) === true
	) {
		return ThinkingDisplayMode.CollapsedPreview;
	}
	return (
		configurationService.getValue<ThinkingDisplayMode>(
...
function extractTextFromPart(content: IChatThinkingPart): string {
	const raw = Array.isArray(content.value)
		? content.value.join("")
		: content.value || "";
	return raw.trim();
}
...
export function getToolInvocationIcon(
	toolId: string,
...
type ThinkingPhrasesConfiguration = {
	mode?: "replace" | "append";
	phrases?: string[];
};

function getCustomThinkingPhrases(
	configurationService: IConfigurationService,
): { customPhrases: string[]; replaceDefaults: boolean } {
	const config = configurationService.getValue<ThinkingPhrasesConfiguration>(
...
export function maybePickFunWorkingMessage(
	configurationService: IConfigurationService,
	random = Math.random,
): string | undefined {
...
export function buildPhrasePool(
	...
	const { customPhrases, replaceDefaults } =
		getCustomThinkingPhrases(configurationService);

	if (customPhrases.length > 0) {
		return replaceDefaults
			? [...customPhrases]
			: [...defaults, ...customPhrases];
	}
	return [...defaults];
}

export class ChatThinkingContentPart
	extends ChatCollapsibleContentPart
	implements IChatContentPart
{
	private static _codeBlockRendererSync(
		_languageId: string,
		text: string,
		_raw?: string,
	): HTMLElement {
	...
	private readonly workingTitle = localize(
		"chat.thinking.header.working",
		"Working",
	);
	private textContainer!: HTMLElement;
	private readonly _markdownResult = this._register(
	...
	private availableMessagesByCategory = new Map<
	...
	private readonly toolDisposables = this._register(
	...
	private readonly _pendingAppendRefresh = this._register(
	...
	private getRandomWorkingMessage(
		category: WorkingMessageCategory = WorkingMessageCategory.Tool,
	): string {
	...
	constructor(
		...
		const extractedTitle =
			extractTitleFromThinkingContent(initialText) ??
		...
		const configuredMode = getEffectiveThinkingDisplayMode(
			this.configurationService,
			contextKeyService,
		);

		this.fixedScrollingMode =
			configuredMode === ThinkingDisplayMode.FixedScrolling;
		...
		this._register(
			this._externalResourceWidget.onDidChangeHeight(() =>
		...
		this._register(
			autorun((r) => {
				...
				if (
					isExpanded &&
					!this.shouldAllowExpansion() &&
					(this.streamingCompleted || this.element.isComplete)
				) {
					this.setExpanded(false);
					return;
				}
		...
		if (this._collapseButton) {
			this._register(
				this._collapseButton.onDidClick(() => {
	...
	protected override initContent(): HTMLElement {
		...
		const hasLazyThinkingItems = this.lazyItems.some(
		...
		if (
			!this.streamingCompleted &&
			!this.element.isComplete &&
			!(this.fixedScrollingMode && this.showProgressDetails)
		) {
		...
		if (this.fixedScrollingMode) {
			...
			this._register(
				this.scrollableElement.onScroll((e) => this.handleScroll(e.scrollTop)),
			);
			...
			const mutationObserver = new MutationObserver(() => {
				...
				pendingMutationRefresh = scheduleAtNextAnimationFrame(
			...
			this.childResizeObserver = this._register(
				new DisposableResizeObserver("ChatThinkingContentPart.child", () => {
					if (
						this.streamingCompleted ||
						!this.domNode.classList.contains("chat-used-context-collapsed")
					) {
			...
			if (this.workingSpinnerElement) {
				this._register(
					this.childResizeObserver.observe(this.workingSpinnerElement),
				);
			}

			// Cache wrapper scrollHeight post-layout via ResizeObserver to avoid forced reflows.
			const wrapperResizeObserver = this._register(
				new DisposableResizeObserver(
					"ChatThinkingContentPart.wrapper",
					(entries) => {
						if (entries[0]) {
			...
			this.wrapperResizeObserverDisposable = this._register(
			...
			this._register(
				this._onDidChangeHeight.event(() => {
					if (
						!this.streamingCompleted &&
						this.wrapperResizeObserverDisposable
					) {
						this.refreshContentHeight();
						this.updateScrollDimensionsFromCache();
						return;
	...
	private handleScroll(scrollTop: number): void {
		...
		this.autoScrollEnabled =
			maxScrollTop <= 0 || scrollTop >= maxScrollTop - 10;
	...
	private updateFadeClasses(
		...
		const currentViewportHeight =
			viewportHeight ??
			Math.min(currentContentHeight, THINKING_SCROLL_MAX_HEIGHT);
	...
	private syncDimensionsAndScheduleScroll(): void {
		...
		this.pendingScrollDisposable = scheduleAtNextAnimationFrame(
	...
	private updateScrollDimensionsFromCache(): void {
		...
		const isCollapsed = this.domNode.classList.contains(
	...
	private renderMarkdown(content: string, reuseExisting?: boolean): void {
		...
		const target = reuseExisting
			? this._markdownResult.value?.element
			: undefined;

		const rendered = this.chatContentMarkdownRenderer.render(
			new MarkdownString(contentToRender),
			{
		...
		if (!target) {
			if (this.textContainer) {
				...
				this.textContainer.appendChild(
					createThinkingIcon(Codicon.circleFilled),
				);
		...
		if (this.diffStatsByPartId.size > 0) {
			...
			if (added > 0 || removed > 0) {
				...
				const insertionsFragment =
					added === 1
						? localize("chat.thinking.insertions.one", "1 insertion")
						: localize("chat.thinking.insertions", "{0} insertions", added);
				const deletionsFragment =
					removed === 1
						? localize("chat.thinking.deletions.one", "1 deletion")
						: localize("chat.thinking.deletions", "{0} deletions", removed);
	...
	private setDropdownClickable(clickable: boolean): void {
		if (this._collapseButton) {
			this._collapseButton.element.style.pointerEvents = clickable
				? "auto"
				: "none";
		}
	...
	private shouldAllowExpansion(): boolean {
		...
		if (this.wrapper) {
			const meaningfulChildren = Array.from(this.wrapper.children).filter(
				(child) => child !== this.workingSpinnerElement,
			).length;
	...
	private appendToWrapper(element: HTMLElement): void {
		...
		if (
			this.workingSpinnerElement &&
			this.workingSpinnerElement.parentNode === this.wrapper
		) {
	...
	private updateWorkingSpinnerVisibility(reader?: IReader): void {
		...
		const hasRunningTerminalTool = this.toolInvocations.some(
			(toolInvocation) => {
				const terminalData = toolInvocation.toolSpecificData as
					| IChatTerminalToolInvocationData
					| undefined;
				if (
					terminalData?.kind !== "terminal" ||
					terminalData.terminalCommandState?.exitCode !== undefined
				) {
					return false;
				}
	...
	public updateThinking(content: IChatThinkingPart): void {
		...
		const reuseExisting = !!(
			...
			next.startsWith(previousValue) &&
			next.length > previousValue.length
		);
	...
	public finalizeTitleIfDefault(): void {
		...
		const existingTitle =
			this.toolInvocations.find((t) => t.generatedTitle)?.generatedTitle ??
		...
		if (allSerialized) {
			...
			if (!LocalChatSessionUri.isLocalSession(this.element.sessionResource)) {
				const lastToolInvocation =
					this.toolInvocations[this.toolInvocations.length - 1];
				if (lastToolInvocation) {
					const cachedTitle = this.getCachedTitle(
						lastToolInvocation.toolCallId,
					);
					if (cachedTitle) {
		...
		if (
			...
			this.hookCount === 0 &&
			this.currentThinkingValue.trim() === ""
		) {
			// If singleItemInfo wasn't set (item was lazy/deferred), materialize it now
			if (!this.singleItemInfo) {
				const lazyItem = this.lazyItems.find(
					(item) => item.kind === "tool" && item.originalParent,
				);
				if (lazyItem && lazyItem.kind === "tool") {
					const toolInvocation =
						lazyItem.toolInvocationOrMarkdown &&
						(lazyItem.toolInvocationOrMarkdown.kind === "toolInvocation" ||
							lazyItem.toolInvocationOrMarkdown.kind ===
			...
			const toolIsComplete =
				!this.singleItemInfo?.toolInvocation ||
				IChatToolInvocation.isComplete(this.singleItemInfo.toolInvocation);
			if (
				toolIsComplete &&
				this.singleItemInfo &&
	...
	private loadTitleCache(): Record<
		string,
		{ title: string; storedAt: number }
	> {
		return (
			this.storageService.getObject<
				Record<string, { title: string; storedAt: number }>
			>(TITLE_CACHE_STORAGE_KEY, StorageScope.PROFILE) ?? {}
		);
	}

	private saveTitleCache(
		cache: Record<string, { title: string; storedAt: number }>,
	): void {
		if (Object.keys(cache).length === 0) {
			this.storageService.remove(TITLE_CACHE_STORAGE_KEY, StorageScope.PROFILE);
		} else {
			this.storageService.store(
				TITLE_CACHE_STORAGE_KEY,
...
${
	this.hookCount > 0
		? `BLOCKED/DENIED CONTENT (hooks detected):
...
${
	this.hookCount > 0
		? `EXAMPLES WITH BLOCKED CONTENT (from hooks):
	...
	private restoreSingleItemToOriginalPosition(): boolean {
		...
		const { element, originalParent, originalNextSibling, toolInvocation } =
			this.singleItemInfo;
		...
		const finalLabel =
			this.appendedItemCount > 0
				? this.appendedItemCount === 1
					? localize(
							"chat.thinking.finished.withStepsSingular",
							"Finished with 1 step",
						)
					: localize(
							"chat.thinking.finished.withStepsPlural",
							"Finished with {0} steps",
							this.appendedItemCount,
						)
	...
	public appendItem(
		...
		if (this.workingSpinnerLabel) {
			const isTerminalTool =
				...
				(toolInvocationOrMarkdown.kind === "toolInvocation" ||
					toolInvocationOrMarkdown.kind === "toolInvocationSerialized") &&
				toolInvocationOrMarkdown.toolSpecificData?.kind === "terminal";
			const category = isTerminalTool
				? WorkingMessageCategory.Terminal
				: WorkingMessageCategory.Tool;
			this.workingSpinnerLabel.textContent =
				this.getRandomWorkingMessage(category);
		}

		// If expanded or has been expanded once, render immediately
		if (
			this.isExpanded() ||
			this.hasExpandedOnce ||
			(this.fixedScrollingMode && !this.streamingCompleted)
		) {
		...
		const toolInvocationsIndex = this.toolInvocations.findIndex(
			(t) =>
				(t.kind === "toolInvocation" ||
					t.kind === "toolInvocationSerialized") &&
	...
	public removeEditPillByPartId(partId: string): void {
		...
		const lazyIndex = this.lazyItems.findIndex(
			(item) => item.kind === "tool" && item.toolInvocationId === partId,
		);
		if (lazyIndex !== -1) {
	...
	public removeLazyItem(toolInvocationId: string): boolean {
		const index = this.lazyItems.findIndex(
			(item) =>
				item.kind === "tool" && item.toolInvocationId === toolInvocationId,
		);
		if (index === -1) {
			return false;
		}
		...
		if (
			removedItem.kind === "tool" &&
			removedItem.toolInvocationOrMarkdown &&
			(removedItem.toolInvocationOrMarkdown.kind === "toolInvocation" ||
				removedItem.toolInvocationOrMarkdown.kind ===
					"toolInvocationSerialized")
		) {
	...
	private schedulePendingRemovalsFlush(): void {
		...
		this.pendingRemovalFlushDisposable = scheduleAtNextAnimationFrame(
			getWindow(this.domNode),
			() => {
				this.pendingRemovalFlushDisposable = undefined;
				if (this._store.isDisposed) {
	...
	private removeStreamingToolEntry(
		...
		const lazyIndex = this.lazyItems.findIndex(
			(item) =>
				...
				(item.toolInvocationOrMarkdown.kind === "toolInvocation" ||
					item.toolInvocationOrMarkdown.kind === "toolInvocationSerialized") &&
				item.toolInvocationOrMarkdown.toolCallId === toolCallId,
		);
		if (lazyIndex !== -1) {
			const removedLazyItem = this.lazyItems[lazyIndex];
			if (
				removedLazyItem.kind === "tool" &&
				removedLazyItem.toolInvocationOrMarkdown &&
				(removedLazyItem.toolInvocationOrMarkdown.kind === "toolInvocation" ||
					removedLazyItem.toolInvocationOrMarkdown.kind ===
						"toolInvocationSerialized")
			) {
	...
	private trackToolMetadata(
		...
		if (isToolInvocation && toolInvocationOrMarkdown.invocationMessage) {
			const message =
				typeof toolInvocationOrMarkdown.invocationMessage === "string"
					? toolInvocationOrMarkdown.invocationMessage
					: toolInvocationOrMarkdown.invocationMessage.value;
			...
			const isStreamingEditTool =
				toolInvocationOrMarkdown.kind === "toolInvocation" &&
				IChatToolInvocation.isStreaming(toolInvocationOrMarkdown) &&
				isGenericEditToolId(toolInvocationOrMarkdown.toolId);
			...
			if (toolInvocationOrMarkdown.kind === "toolInvocation") {
				...
				let isStreaming = IChatToolInvocation.isStreaming(
					toolInvocationOrMarkdown,
				);

				const toolStore = new DisposableStore();
				this.toolDisposables.set(
					toolInvocationOrMarkdown.toolCallId,
					toolStore,
				);
				...
				const autorunDisposable = autorun((reader) => {
					...
					if (
						isStreaming &&
						currentState.type !== IChatToolInvocation.StateKind.Streaming
					) {
						isStreaming = false;

						// Update terminal tool icon based on sandbox wrapping state
						const termData = toolInvocationOrMarkdown.toolSpecificData as
							| IChatTerminalToolInvocationData
							| undefined;
						if (termData?.kind === "terminal") {
							const iconEl = this.toolIconsByCallId.get(toolCallId);
							if (iconEl) {
								const newIcon = termData.commandLine?.isSandboxWrapped
					...
					if (
						...
						if (
							toolInvocationOrMarkdown.presentation === "hidden" ||
							toolInvocationOrMarkdown.presentation === "hiddenAfterComplete"
						) {
							this.pendingRemovals.push({
								toolCallId: toolInvocationOrMarkdown.toolCallId,
								toolLabel: currentToolLabel,
			...
			const codeblockInfo = extractCodeblockUrisFromText(
				toolInvocationOrMarkdown.content.value,
			);
	...
	private updateExternalResourceParts(
		...
		const extractedImages = extractImagesFromToolInvocationOutputDetails(
			toolInvocation,
			this.element.sessionResource,
		);
		if (extractedImages.length === 0) {
			return;
		}

		const parts: IChatCollapsibleIODataPart[] = extractedImages.map(
			(image) => ({
				kind: "data",
		...
		this._externalResourceWidget.setToolInvocationParts(
	...
	private appendItemToDOM(
		...
		const isSearchTool =
			...
			(toolInvocationOrMarkdown.kind === "toolInvocation" ||
				toolInvocationOrMarkdown.kind === "toolInvocationSerialized") &&
			toolInvocationOrMarkdown.toolSpecificData?.kind === "search";
		const toolInvocationIcon =
			toolInvocationOrMarkdown &&
			(toolInvocationOrMarkdown.kind === "toolInvocation" ||
				toolInvocationOrMarkdown.kind === "toolInvocationSerialized")
				? toolInvocationOrMarkdown.icon
				: undefined;
		...
		if (isMarkdownEdit || isExternalEdit) {
			...
			const terminalData = (
				toolInvocationOrMarkdown as
					| IChatToolInvocation
					| IChatToolInvocationSerialized
			).toolSpecificData as {
				kind: "terminal";
				terminalCommandState?: { exitCode?: number };
				commandLine?: { isSandboxWrapped?: boolean };
			};
			...
			icon = toolInvocationId
				? getToolInvocationIcon(toolInvocationId, toolInvocationIcon)
				: Codicon.tools;
		}
		...
		if (isToolInvocation && toolInvocationOrMarkdown.toolCallId) {
			this.toolWrappersByCallId.set(
				toolInvocationOrMarkdown.toolCallId,
				itemWrapper,
			);
			this.toolIconsByCallId.set(
				toolInvocationOrMarkdown.toolCallId,
				iconElement,
			);
		}
		...
		if (this.fixedScrollingMode && this.scrollableElement) {
			...
			if (this.childResizeObserver && !this.streamingCompleted) {
				...
				const toolCallId = isToolInvocation
					? toolInvocationOrMarkdown.toolCallId
					: undefined;
				if (toolCallId) {
	...
	private scheduleAppendRefresh(): void {
		...
		this._pendingAppendRefresh.value = scheduleAtNextAnimationFrame(
			getWindow(this.wrapper),
			() => {
	...
	private materializeLazyItem(item: ILazyItem): void {
		...
		if (this.workingSpinnerLabel) {
			const isTerminalTool =
				...
				(item.toolInvocationOrMarkdown.kind === "toolInvocation" ||
					item.toolInvocationOrMarkdown.kind === "toolInvocationSerialized") &&
				item.toolInvocationOrMarkdown.toolSpecificData?.kind === "terminal";
			const category = isTerminalTool
				? WorkingMessageCategory.Terminal
				: WorkingMessageCategory.Tool;
			this.workingSpinnerLabel.textContent =
		...
		if (result.disposable) {
			const toolCallId =
				...
				(item.toolInvocationOrMarkdown.kind === "toolInvocation" ||
					item.toolInvocationOrMarkdown.kind === "toolInvocationSerialized")
					? item.toolInvocationOrMarkdown.toolCallId
					: undefined;
			if (toolCallId) {
				this.ownedToolParts.set(toolCallId, result.disposable);
			} else {
