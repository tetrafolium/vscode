...
export class ChatElicitationContentPart
	extends Disposable
	implements IChatContentPart
{
	...
	constructor(
		...
		if (elicitation.kind === "elicitation2") {
			...
			const hasElicitationKey =
				ChatContextKeys.Editing.hasElicitationRequest.bindTo(
		...
		confirmationWidget.setShowButtons(
			elicitation.kind === "elicitation2" &&
				elicitation.state.get() === ElicitationState.Pending,
		);

		this._register(
			confirmationWidget.onDidClick(async ({ button: e }) => {
				if (elicitation.kind !== "elicitation2") {
					return;
				}
		...
		this.domNode.ariaLabel =
			elicitation.title +
			" " +
			(typeof messageToRender === "string"
				? messageToRender
				: messageToRender.value || "");
	}

	private getMessageToRender(
		elicitation: IChatElicitationRequest | IChatElicitationRequestSerialized,
	): IMarkdownString | string {
		if (!elicitation.acceptedResult) {
			return elicitation.message;
		}

		const messageMd = isMarkdownString(elicitation.message)
			? MarkdownString.lift(elicitation.message)
