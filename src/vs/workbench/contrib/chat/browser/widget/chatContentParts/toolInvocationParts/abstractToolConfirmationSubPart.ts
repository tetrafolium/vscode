...
export interface IAbstractToolPrimaryAction extends IChatConfirmationButton<
	() => void
> {
...
export abstract class AbstractToolConfirmationSubPart extends BaseChatToolInvocationSubPart {
	...
	protected render(config: IToolConfirmationConfig) {
		...
		if (customOptions && customOptions.length > 0) {
			...
			const sessionAction = this.useAllowOnceAsPrimary()
				? undefined
				: additionalActions.find(
						(action): action is IAbstractToolPrimaryAction =>
							"scope" in action && action.scope === "session",
					);
	...
	private buildCustomOptionButtons(
		toolInvocation: IChatToolInvocation,
		options: readonly ConfirmationOption[],
