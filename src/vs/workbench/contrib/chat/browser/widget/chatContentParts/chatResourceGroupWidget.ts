...
export class ChatResourceGroupWidget extends Disposable {
	...
	private async _fillInResourceGroup(
		parts: IChatCollapsibleIODataPart[],
		...
		const deferredImageParts: {
			index: number;
			part: IChatCollapsibleIODataPart;
		}[] = [];
		...
		attachments.contextMenuHandler = (attachment, event) => {
			...
			if (part) {
				...
				this._contextMenuService.showContextMenu({
					...
					getActionsContext: () =>
						({ parts: [part] }) satisfies IChatToolOutputResourceToolbarContext,
				});
			}
		};
...
class SaveResourcesAction extends Action2 {
	...
	async run(
		...
		context: IChatToolOutputResourceToolbarContext,
	) {
