...
export class ChatCheckpointFileChangesSummaryContentPart
	extends Disposable
	implements IChatContentPart
{
	...
	private readonly diffsBetweenRequests = new Map<
		string,
		IObservable<IEditSessionEntryDiff | undefined>
	>();

	private fileChangesDiffsObservable: IObservable<
	...
	private computeFileChangesDiffs({
		requestId,
		sessionResource,
	}: IChatFileChangesSummaryPart) {
		return this.chatService.chatModels
			.map((models) =>
				Iterable.find(models, (m) =>
					isEqual(m.sessionResource, sessionResource),
				),
			)
			.map((model) =>
				model?.editingSession?.getDiffsForFilesInRequest(requestId),
			)
			.map((diffs, r) => diffs?.read(r) || Iterable.empty());
	}

	public getCachedEntryDiffBetweenRequests(
		editSession: IChatEditingSession,
		uri: URI,
	...
	private renderHeader(container: HTMLElement): IDisposable {
		...
		this._register(
			autorun((r) => {
				...
				viewListButton.label =
					diffs.length === 1
						? `Changed 1 file`
						: `Changed ${diffs.length} files`;
			}),
		);

		const setExpansionState = () => {
			viewListButton.icon = this.isCollapsed
	...
	private renderViewAllFileChangesButton(container: HTMLElement): IDisposable {
		...
		return combinedDisposable(
			hoverDisposable,
			dom.addDisposableListener(button, "click", (e) => {
				const resources: { originalUri: URI; modifiedUri?: URI }[] =
					this.fileChangesDiffsObservable.get().map((diff) => ({
	...
	hasSameContent(
		...
		return (
			other.kind === "changesSummary" &&
			other.requestId === this.content.requestId
		);
	}
...
class CollapsibleChangesSummaryListRenderer implements IListRenderer<
	IEditSessionEntryDiff,
...
> {
	...
	readonly templateId: string =
		CollapsibleChangesSummaryListRenderer.TEMPLATE_ID;
	...
	renderElement(
		data: IEditSessionEntryDiff,
		index: number,
		templateData: ICollapsibleChangesSummaryListTemplate,
	): void {
