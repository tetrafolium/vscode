 ...
 */
export class ChatExternalEditContentPart
	extends ChatEditPillElement
	implements IChatContentPart
{
	...
	private render(edit: IChatExternalEdit): void {
		...
		if (edit.diff && (edit.diff.added > 0 || edit.diff.removed > 0)) {
			...
			this.setAriaLabel(
				`${label} ${this.labelService.getUriBasenameLabel(edit.uri)}`,
			);
		}
	}
	...
	private openEdit({
		...
		if (this.edit.beforeContentUri && this.edit.afterContentUri) {
			...
			this.editorService.openEditor(
				{ resource: this.edit.beforeContentUri, options },
				group,
			);
		} else if (this.edit.editKind !== "delete") {
			this.editorService.openEditor(
				{ resource: this.edit.uri, options },
				group,
			);
		}
	}
...
function describeEdit(edit: IChatExternalEdit): {
	icon: ThemeIcon;
	label: string;
} {
	switch (edit.editKind) {
		case "create":
			return {
				icon: Codicon.check,
				label: localize("chat.externalEdit.created", "Created"),
			};
		case "delete":
			return {
				icon: Codicon.check,
				label: localize("chat.externalEdit.deleted", "Deleted"),
			};
		case "rename":
			return {
				icon: Codicon.check,
				label: localize("chat.externalEdit.renamed", "Renamed"),
			};
		case "edit":
			return {
				icon: Codicon.check,
				label: localize("chat.codeblock.edited", "Edited"),
			};
	}
}
