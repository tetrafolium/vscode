...
class CollapsibleListRenderer implements IListRenderer<
	IChatCollapsibleListItem,
	ICollapsibleListTemplate
> {
	...
	renderElement(
		...
		if (typeof reference === "object" && "variableName" in reference) {
			...
			if (
				...
				templateData.label.setResource(
					{ resource: uri, name: uri.toString(true) },
					{
		...
		if (data.state !== undefined) {
			if (templateData.actionBarContainer || data.showModifiedState) {
				...
				if (diffMeta) {
					if (
						!templateData.fileDiffsContainer ||
						!templateData.addedSpan ||
						!templateData.removedSpan
					) {
						return;
					}
