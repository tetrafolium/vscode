...
class TodoListRenderer implements IListRenderer<IChatTodo, ITodoListTemplate> {
	...
	renderTemplate(container: HTMLElement): ITodoListTemplate {
		...
		const statusIcon = dom.append(
			todoElement,
			dom.$(".todo-status-icon.codicon"),
		);
		statusIcon.setAttribute("aria-hidden", "true");

		const todoContent = dom.append(todoElement, dom.$(".todo-content"));
		const iconLabel = templateDisposables.add(
			new IconLabel(todoContent, { supportIcons: false }),
		);
	...
	renderElement(
		todo: IChatTodo,
		index: number,
		templateData: ITodoListTemplate,
	): void {
...
export class ChatTodoListWidget extends Disposable {
	...
	constructor(
		...
		this._inChatTodoListContextKey =
			ChatContextKeys.inChatTodoList.bindTo(contextKeyService);
		...
		this._register(
			focusTracker.onDidFocus(() => this._inChatTodoListContextKey.set(true)),
		);
		this._register(
			focusTracker.onDidBlur(() => this._inChatTodoListContextKey.set(false)),
		);
	...
	private createChatTodoWidget(): HTMLElement {
		...
		this.expandoButton = this._register(
			new Button(expandoContainer, {
				supportIcons: true,
			}),
		);
		this.expandoButton.element.setAttribute(
			"aria-expanded",
			String(this._isExpanded),
		);
		this.expandoButton.element.setAttribute(
			"aria-controls",
			"todo-list-container",
		...
		this.expandIcon.classList.add(
			this._isExpanded ? "codicon-chevron-down" : "codicon-chevron-right",
		);
	...
	private createClearButton(): void {
		...
		this._register(
			this.clearButton.onDidClick(() => {
				const todoCount = this._currentSessionResource
					? this.chatTodoListService.getTodos(this._currentSessionResource)
							.length
					: 0;
				this.telemetryService.publicLog2<
					ChatTodoListWidgetEvent,
					ChatTodoListWidgetClassification
				>("chatTodoListWidget", {
					action: "clear",
					todoCount,
				});
	...
	public clear(sessionResource: URI | undefined, force: boolean = false): void {
		...
		const shouldClear =
			...
			(currentTodos.length > 0 &&
				!currentTodos.some((todo) => todo.status !== "completed"));
		if (shouldClear) {
			this.clearAllTodos();
		}
	}
	...
	public hasTodos(): boolean {
		return (
			this.domNode.classList.contains("has-todos") &&
			!!this._todoList &&
	...
	private updateTodoDisplay(): void {
		...
		const todoList = this.chatTodoListService.getTodos(
	...
	private renderTodoList(todoList: IChatTodo[]): void {
		...
		const allIncomplete = todoList.every(
				...
				this.instantiationService.createInstance(
					...
					[new TodoListRenderer()],
					{
						...
						accessibilityProvider: {
							...
							getWidgetAriaLabel: () =>
								localize("chatTodoList", "Chat Todo List"),
						},
					},
				),
			);
		...
		const hasInProgressTask = todoList.some(
			(todo) => todo.status === "in-progress",
		);
		const hasCompletedTask = todoList.some(
		...
		if (
			(hasInProgressTask || hasCompletedTask) &&
			this._isExpanded &&
			!this._userManuallyExpanded
		) {
	...
	private toggleExpanded(): void {
		...
		this.expandIcon.classList.toggle(
		...
		const todoCount = this._currentSessionResource
			? this.chatTodoListService.getTodos(this._currentSessionResource).length
			: 0;
		this.telemetryService.publicLog2<
			ChatTodoListWidgetEvent,
			ChatTodoListWidgetClassification
	...
	private updateClearButtonState(): void {
		...
		const isRequestInProgress =
			ChatContextKeys.requestInProgress.getValue(this.contextKeyService) ??
			false;
		...
		if (shouldDisable) {
			...
			this.clearButton.setTitle(
				localize("chat.todoList.clearButton", "Clear all todos"),
			);
		}
	}
	...
	private updateTitleElement(
		...
		const inProgressTodos = todoList.filter(
			(todo) => todo.status === "in-progress",
		);
		const firstInProgressTodo =
			inProgressTodos.length > 0 ? inProgressTodos[0] : undefined;
		const notStartedTodos = todoList.filter(
			(todo) => todo.status === "not-started",
		);
		const firstNotStartedTodo =
			notStartedTodos.length > 0 ? notStartedTodos[0] : undefined;
...
type ChatTodoListWidgetClassification = {
	action: {
		...
		comment: "The user action on the todo list widget (expand, collapse, or clear).";
	};
	todoCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "The number of todos at the time of the action.";
	};
	owner: "bhavyaus";
