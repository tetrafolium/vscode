...
export class ChatTerminalToolConfirmationSubPart extends BaseChatToolInvocationSubPart {
	...
	constructor(
		...
		private readonly terminalChatService: ITerminalChatService,
		@IHoverService hoverService: IHoverService,
		@ILanguageModelToolsService
		private readonly languageModelToolsService: ILanguageModelToolsService,
		@IChatToolRiskAssessmentService
		private readonly riskAssessmentService: IChatToolRiskAssessmentService,
	) {
		...
		const { title, message, disclaimer, terminalCustomActions } =
		...
		let moreActions:
			| (
					| IChatConfirmationButton<TerminalNewAutoApproveButtonData>
					| Separator
			  )[]
			| undefined = undefined;
		...
		const languageId =
			this.languageService.getLanguageIdByLanguageName(
				terminalData.presentationOverrides?.language ??
		...
		const confirmWidget = this._register(
			this.instantiationService.createInstance(
				ChatCustomConfirmationWidget<
					TerminalNewAutoApproveButtonData | boolean
				>,
				this.context,
				{
					title,
		...
		if (terminalData.requestUnsandboxedExecution) {
			const reasonText =
				(terminalData.requestUnsandboxedExecutionReason &&
					terminalData.requestUnsandboxedExecutionReason.trim()) ||
				localize(
		...
		if (riskBadge && detailParts.length) {
			const combined = new MarkdownString(undefined, {
				...
				isTrusted: detailParts.reduce<MarkdownString["isTrusted"]>(
					(acc, part) => {
						...
						if (typeof part.isTrusted === "object" && part.isTrusted) {
							const enabled = new Set([
								...(typeof acc === "object" && acc?.enabledCommands
									? acc.enabledCommands
									: []),
			...
			detailParts.forEach((part, i) => {
				...
				combined.appendMarkdown(
					`**${escapeMarkdownSyntaxTokens(part.hoverLabel)}** ${part.hoverBody}`,
				);
			});
		...
		this._register(
			confirmWidget.onDidClick(async ({ button, isTouchClick }) => {
				let doComplete = true;
				...
				if (typeof data === "boolean") {
					...
					switch (data.type) {
						...
						case "newRule": {
							...
							const sessionRules = newRules.filter(
								(r) => r.scope === "session",
							);
							const workspaceRules = newRules.filter(
								(r) => r.scope === "workspace",
							);
							...
							if (workspaceRules.length > 0) {
								...
								if (isObject(oldValue)) {
									...
									throw new ErrorNoTelemetry(
										`Cannot add new rule, existing workspace setting is unexpected format`,
									);
								}
							}
							...
							if (userRules.length > 0) {
								...
								const oldValue =
									(inspect.userValue as Record<string, unknown> | undefined) ??
									{};
								if (isObject(oldValue)) {
									const newValue: Record<string, unknown> = { ...oldValue };
									...
									throw new ErrorNoTelemetry(
										`Cannot add new rule, existing setting is unexpected format`,
									);
								}
							}

							function formatRuleLinks(
								rules: ITerminalNewAutoApproveRule[],
								scope: "session" | "workspace" | "user",
							): string {
								return rules
									.map((e) => {
						...
						case "sessionApproval": {
							...
							this.terminalChatService.setChatSessionAutoApproval(
								sessionResource,
								true,
							);
							...
							terminalData.autoApproveInfo = new MarkdownString(
								`${localize("sessionApproval", "All commands will be auto approved for this session")} ([${localize("sessionApproval.disable", "Disable")}](${disableUri.toString()}))`,
								mdTrustSettings,
							);
							toolConfirmKind = ToolConfirmKind.UserAction;
							break;
				...
				if (doComplete) {
					...
					if (!isTouchClick) {
						this.chatWidgetService
							.getWidgetBySessionResource(this.context.element.sessionResource)
							?.focusInput();
					}
				}
			}),
	...
	private _createButtons(
		moreActions:
			...
			| undefined,
	): IChatConfirmationButton<boolean | TerminalNewAutoApproveButtonData>[] {
		const getLabelAndTooltip = (
			label: string,
			actionId: string,
			tooltipDetail: string = label,
		...
		return [
			...
			{
				...getLabelAndTooltip(
					localize("tool.skip", "Skip"),
					SkipToolConfirmationActionId,
					localize("skip.detail", "Proceed without executing this command"),
				),
				data: { type: "skip" },
				isSecondary: true,
			},
		...
		const promptResult = await this.dialogService.prompt({
			...
			custom: {
				...
				markdownDetails: [
					...
					{
						markdown: new MarkdownString(
							`[${localize("autoApprove.markdown2", "Learn more about the potential risks and how to avoid them.")}](https://code.visualstudio.com/docs/copilot/security#_security-considerations)`,
						),
					},
				],
			},
	...
	private _createRiskBadge(
		...
		if (cached) {
			...
			(async () => {
				try {
					const result = await this.riskAssessmentService.assess(
						tool,
						parameters,
						cts.token,
					);
	...
	private _appendMarkdownPart(
		...
		codeBlockRenderOptions: ICodeBlockRenderOptions,
	) {
		const part = this._register(
			this.instantiationService.createInstance(
				ChatMarkdownContentPart,
				{
					...
					content:
						typeof message === "string"
							? new MarkdownString().appendMarkdown(message)
							: message,
				},
				this.context,
				this.editorPool,
