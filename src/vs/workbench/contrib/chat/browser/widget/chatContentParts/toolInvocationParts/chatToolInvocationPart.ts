	...
	implements IChatContentPart
{
	...
	constructor(
		...
		if (toolInvocation.toolSpecificData?.kind === "todoList") {
			...
			const todos = toolInvocation.toolSpecificData.todoList.map(
				(todo, index) => {
		...
		if (toolInvocation.kind === "toolInvocation") {
			...
			this._register(
				autorun((reader) => {
					const stateChanged =
						toolInvocation.state.read(reader).type !== initialState;
					const dataKindChanged =
		...
		const render = () => {
			...
			const isConfirmation =
				this.subPart instanceof ToolConfirmationSubPart ||
		...
		if (mcpAppRenderData) {
			const shouldRender = derived((r) => {
				const outcome = IChatToolInvocation.executionConfirmedOrDenied(
					toolInvocation,
					r,
				);
				return (
					!!outcome &&
					outcome.type !== ToolConfirmKind.Denied &&
					outcome.type !== ToolConfirmKind.Skipped
				);
			});
	...
	private createToolInvocationSubPart(): BaseChatToolInvocationSubPart {
		if (this.toolInvocation.kind === "toolInvocation") {
			...
			if (state.type === IChatToolInvocation.StateKind.WaitingForConfirmation) {
				if (
					this.toolInvocation.toolSpecificData?.kind === "terminal" &&
					!isLegacyChatTerminalToolInvocationData(
						this.toolInvocation.toolSpecificData,
					) &&
					this.toolInvocation.toolSpecificData.missingSandboxDependencies
						?.length
				) {
					return this.instantiationService.createInstance(
					...
					this.toolInvocation.toolSpecificData?.kind ===
		...
		if (
			this.toolInvocation.toolSpecificData?.kind === "resources" &&
			this.toolInvocation.toolSpecificData.values.length > 0
		) {
			return this.instantiationService.createInstance(
				...
				this.toolInvocation.toolSpecificData.values,
				this.listPool,
			);
		}
		...
		const resultDetails = IChatToolInvocation.resultDetails(
			this.toolInvocation,
		);
		...
		if (
			...
			!IChatToolInvocation.isComplete(this.toolInvocation)
		) {
			return this.instantiationService.createInstance(
				ChatInputOutputMarkdownProgressPart,
				this.toolInvocation,
	...
	private getMcpAppRenderData(): IMcpAppRenderData | undefined {
		...
		if (toolSpecificData?.kind === "input" && toolSpecificData.mcpAppData) {
			const rawInput =
				typeof toolSpecificData.rawInput === "string"
