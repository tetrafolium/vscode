 ...
 */
export function getEditorOverrideForChatResource(
	resource: URI,
...
export class InlineAnchorWidget extends Disposable {
	...
	constructor(
		...
		if (this.data.kind === "symbol") {
			...
			const filePathLabel =
				this.metadata?.linkText ??
				labelService.getUriBasenameLabel(location.uri);
			...
			if (location.range && this.data.kind !== "symbol") {
				...
				const editorName =
					this.editorService.findEditors(location.uri)[0]?.editor?.getName() ??
		...
		this._register(
			dom.addDisposableListener(element, "click", async (e) => {
