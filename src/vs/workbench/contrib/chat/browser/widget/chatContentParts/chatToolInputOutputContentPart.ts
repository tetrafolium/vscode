...
export type ChatCollapsibleIOPart =
...
export class ChatCollapsibleInputOutputContentPart extends Disposable {
	...
	constructor(
		...
		const expanded = (this._expanded = observableValue(
			this,
			initiallyExpanded,
		));
		this._register(
		...
		const topLevelResources = this.output?.parts
			...
			.filter(
				(p) =>
					!p.audience || p.audience.includes(LanguageModelPartAudience.User),
			);
		if (topLevelResources?.length) {
			const resourceSubPart = this._register(
