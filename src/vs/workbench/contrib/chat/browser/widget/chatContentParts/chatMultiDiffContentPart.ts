...
export class ChatMultiDiffContentPart
	extends Disposable
	implements IChatContentPart
{
	...
	private renderContributedButtons(container: HTMLElement): IDisposable {
		const buttonsContainer = container.appendChild(
			$(".chat-multidiff-contributed-buttons"),
		);
		const disposables = new DisposableStore();
		...
		const nestedInsta = disposables.add(
			...
			this.instantiationService.createInstance(
				...
				],
				{
					...
					accessibilityProvider: {
						...
						getWidgetAriaLabel: () =>
							localize("chatMultiDiffList", "File Changes"),
					},
				},
			),
		);
	...
	hasSameContent(other: IChatRendererContent): boolean {
		return (
			other.kind === "multiDiffData" &&
			this.diffData.get().resources.length ===
				(isObservable(other.multiDiffData)
					? other.multiDiffData.get().resources.length
					: other.multiDiffData.resources.length)
		);
	}
...
class ChatMultiDiffListRenderer implements IListRenderer<
	IChatMultiDiffItem,
	IChatMultiDiffItemTemplate
> {
	...
	renderElement(
		element: IChatMultiDiffItem,
		_index: number,
		templateData: IChatMultiDiffItemTemplate,
	): void {
		...
		if (element.diff?.added || element.diff?.removed) {
			const changesSummary = labelElement.appendChild(
				$(`.${ChatMultiDiffListRenderer.CHANGES_SUMMARY_CLASS_NAME}`),
			);
