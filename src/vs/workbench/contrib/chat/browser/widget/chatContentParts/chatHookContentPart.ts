...
export class ChatHookContentPart
	...
	implements IChatContentPart
{
	...
	protected override initContent(): HTMLElement {
		...
		if (this.hookPart.stopReason) {
			const reasonElement = $(
				".chat-hook-reason",
				undefined,
				this.hookPart.stopReason,
			);
			content.appendChild(reasonElement);
		}

		const isToolHook =
			this.hookPart.hookType === HookType.PreToolUse ||
			this.hookPart.hookType === HookType.PostToolUse;
		if (
			this.hookPart.systemMessage &&
			(isToolHook || !this.hookPart.stopReason)
		) {
	...
	hasSameContent(
		...
		return (
			other.hookType === this.hookPart.hookType &&
