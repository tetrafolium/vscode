...
export class ChatWorkspaceEditContentPart
	extends Disposable
	implements IChatContentPart
{
	...
	constructor(
		...
		const renderEntry = (message: string, icon: ThemeIcon) => {
			const result = this._register(
				chatContentMarkdownRenderer.render(
					new MarkdownString(message, { isTrusted: true }),
				),
			);
		...
		for (const edit of workspaceEdit.edits) {
			if (edit.oldResource && !edit.newResource) {
				...
				renderEntry(
					localize("created", "Created []({0})", edit.newResource.toString()),
					Codicon.newFile,
				);
			} else if (edit.oldResource && edit.newResource) {
				renderEntry(
	...
	hasSameContent(
		...
		for (let i = 0; i < other.edits.length; i++) {
			...
			if (
				a.oldResource?.toString() !== b.oldResource?.toString() ||
