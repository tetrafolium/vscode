...
export class ChatInputOutputMarkdownProgressPart extends BaseChatToolInvocationSubPart {
	...
	constructor(
		...
		const collapsibleListPart = (this.collapsibleListPart = this._register(
			instantiationService.createInstance(
							...
							parts: processedOutput.map((o, i): ChatCollapsibleIOPart => {
								...
								if (o.type === "ref") {
									...
									if (!o.isText) {
										...
										return {
											kind: "data",
											value: new TextEncoder().encode(o.value),
											mimeType: o.mimeType,
											uri: permalinkUri,
											audience: o.audience,
										};
				...
				(isError &&
					...
					(ChatInputOutputMarkdownProgressPart._expandedByDefault.get(
						toolInvocation,
					) ??
						false),
		...
		const progressObservable =
			toolInvocation.kind === "toolInvocation"
				? toolInvocation.state.map((s, r) =>
						s.type === IChatToolInvocation.StateKind.Executing
							? s.progress.read(r)
							: undefined,
					)
				: undefined;
		...
		if (progressObservable) {
			this._register(
				autorun((reader) => {
					...
					if (
						progress?.progress &&
						!IChatToolInvocation.isComplete(toolInvocation, reader)
					) {
