...
export class ChatToolOutputContentSubPart extends Disposable {
	...
	private addResourceGroup(
		...
		const widget = this._register(
			this._instantiationService.createInstance(ChatResourceGroupWidget, parts),
		);
	...
	private addCodeBlock(
		...
		if (firstPart.title) {
			...
			const renderedTitle = this._register(
				this._markdownRendererService.render(this.toMdString(firstPart.title)),
			);
