	...
	implements IChatWidgetService
{
	...
	private readonly _onDidChangeFocusedWidget = this._register(
		new Emitter<IChatWidget | undefined>(),
	);
	readonly onDidChangeFocusedWidget = this._onDidChangeFocusedWidget.event;

	private readonly _onDidChangeFocusedSession = this._register(
		new Emitter<void>(),
	);
	...
	getWidgetBySessionResource(sessionResource: URI): IChatWidget | undefined {
		return this._widgets.find((w) =>
			isEqual(w.viewModel?.sessionResource, sessionResource),
		);
	}
	...
	async revealWidget(
		...
		return (
			await this.viewsService.openView<ChatViewPane>(ChatViewId, !preserveFocus)
		)?.widget;
	}

	async reveal(widget: IChatWidget, preserveFocus?: boolean): Promise<boolean> {
		if (widget.viewModel?.sessionResource) {
			const alreadyOpenWidget = await this.revealSessionIfAlreadyOpen(
	...
	openSession(
		sessionResource: URI,
		target?: typeof ChatViewPaneTarget,
	): Promise<IChatWidget | undefined>;
	openSession(
		sessionResource: URI,
	...
	async openSession(
		sessionResource: URI,
		target?: typeof ChatViewPaneTarget | PreferredGroup,
		options?: IChatEditorOptions,
	): Promise<IChatWidget | undefined> {
		const t0 = Date.now();
		const targetKind =
		...
		this.logService.trace(
			`[ChatWidgetService] openSession start uri=${sessionResource.toString()} target=${targetKind}`,
		);
		...
		if (target === ChatViewPaneTarget || typeof target === "undefined") {
			const chatView = await this.viewsService.openView<ChatViewPane>(
				ChatViewId,
	...
	private async revealSessionIfAlreadyOpen(
		sessionResource: URI,
		options?: IChatEditorOptions,
	): Promise<IChatWidget | undefined> {
		...
		if (
			chatView?.widget.viewModel?.sessionResource &&
			isEqual(chatView.widget.viewModel.sessionResource, sessionResource)
		) {
			const view = await this.viewsService.openView(
				ChatViewId,
				!options?.preserveFocus,
		...
		if (existingEditor) {
			...
			const isGroupActive = () =>
				dom.getWindow(this.layoutService.activeContainer).vscodeWindowId ===
				existingEditorWindowId;

			let ensureFocusTransfer: Promise<void> | undefined;
			if (!isGroupActive() && !options?.preserveFocus) {
				ensureFocusTransfer = Event.toPromise(
					Event.once(
						Event.filter(
			...
			const pane = await existingEditor.group.openEditor(
	...
	private async prepareSessionForMove(
		sessionResource: URI,
		target: typeof ChatViewPaneTarget | PreferredGroup | undefined,
	): Promise<void> {
		...
		if (existingWidget) {
			...
			if (
				isIChatViewViewContext(existingWidget.viewContext) &&
				target === ChatViewPaneTarget
			) {
				return;
			}
			...
			if (
				...
				target !== ChatViewPaneTarget &&
				existingEditor &&
				this.isSameEditorTarget(existingEditor.group.id, target)
			) {
				return;
			}
			...
			if (existingEditor) {
				...
				await this.editorService.closeEditor(
					{ editor: existingEditor.editor, groupId: existingEditor.group.id },
					{ preserveFocus: true },
				);
			} else {
	...
	private findExistingChatEditorByUri(
		sessionUri: URI,
	): { editor: ChatEditorInput; group: IEditorGroup } | undefined {
		for (const group of this.editorGroupsService.groups) {
			for (const editor of group.editors) {
				if (
					editor instanceof ChatEditorInput &&
					isEqual(editor.sessionResource, sessionUri)
				) {
					return { editor, group };
				}
			}
	...
	private isSameEditorTarget(
		...
		return (
			(typeof target === "number" && target === currentGroupId) ||
			(target === ACTIVE_GROUP &&
				this.editorGroupsService.activeGroup?.id === currentGroupId) ||
	...
	register(newWidget: IChatWidget): IDisposable {
		...
		return combinedDisposable(
			...
			newWidget.onDidChangeViewModel(
				({ previousSessionResource, currentSessionResource }) => {
					if (
						this._lastFocusedWidget === newWidget &&
						!isEqual(previousSessionResource, currentSessionResource)
					) {
						this._onDidChangeFocusedSession.fire();
					}
					...
					void timeout(200).then(() => {
						if (
							!this.getWidgetBySessionResource(previousSessionResource) &&
							this.chatService.getSession(previousSessionResource)
						) {
