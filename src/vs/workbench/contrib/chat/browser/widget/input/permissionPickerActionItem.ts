...
export interface IPermissionPickerDelegate {
	 ...
	 */
	readonly getExtensionPermissions?: () =>
		| IExtensionPermissionState
		| undefined;
	readonly setExtensionPermission?: (
		groupId: string,
...
export class PermissionPickerActionItem extends ChatInputPickerActionViewItem {
	...
	constructor(
		...
		const isAutoApprovePolicyRestricted = () =>
					...
					return ext.items.map(
						(item) =>
							({
								...
								enabled: !item.locked,
								tooltip: item.locked
									? localize("permissions.ext.locked", "This option is locked")
									: "",
				...
				actionBarActions: [
					{
						...
						run: async () => {
							const ext = delegate.getExtensionPermissions?.();
							const url =
								ext?.sessionType === SessionType.ClaudeCode
	...
	protected override renderLabel(element: HTMLElement): IDisposable | null {
		...
		if (ext && ext.items.length > 0) {
			const selected =
				ext.items.find((i) => i.id === ext.selectedId) ??
		...
		element.classList.toggle(
			"warning",
			!ext && level === ChatPermissionLevel.Autopilot,
		);
		element.classList.toggle(
			"info",
			!ext && level === ChatPermissionLevel.AutoApprove,
		);
