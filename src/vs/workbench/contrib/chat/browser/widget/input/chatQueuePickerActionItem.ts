...
} from "../../actions/chatQueueActions.js";
	...
	constructor(
		...
		this._primaryActionAction = this._register(
			new Action(
				...
				!!this.contextKeyService.getContextKeyValue(
					ChatContextKeys.inputHasText.key,
				),
		...
		this._register(
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(ChatConfiguration.RequestQueueingDefaultAction)
				) {
		...
		this._register(
			ModifierKeyEmitter.getInstance().event((status) => {
 ...
 */
export class ChatQueuePickerRendering
	extends Disposable
