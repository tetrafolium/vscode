...
export function getControlModelsForEntitlement(
	manifest: IModelsControlManifest,
	entitlement: ChatEntitlement,
): IStringDictionary<IModelControlEntry> {
	return isProUser(entitlement) && entitlement !== ChatEntitlement.EDU
		? manifest.paid
		: manifest.free;
}
...
function getVendorDisplayName(
	...
	const descriptor = languageModelsService
		.getVendors()
 ...
 */
function buildModelToProviderGroupMap(
	languageModelsService: ILanguageModelsService,
...
function getProviderGroupForModel(
	...
	return {
		...
		groupName: getVendorDisplayName(
...
type ChatModelChangeClassification = {
	...
	fromModel?: {
		...
		comment: "The previous chat model";
	};
	toModel: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The new chat model";
	};
};
...
type ChatModelPickerInteraction =
	| "disabledModelContactAdminClicked"
	| "premiumModelUpgradePlanClicked"
	| "otherModelsExpanded"
...
type ChatModelPickerInteractionClassification = {
	...
	interaction: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The model picker interaction that occurred";
	};
};
...
function createModelItem(
	...
	const hover =
		model && openerService
			? getModelHoverContent(model, openerService, isUBB)
			: undefined;
	return {
		item: action,
		...
		submenuActions: action.toolbarActions?.length
...
function resolveConfigProperty(
	...
	const currentConfig =
		languageModelsService.getModelConfiguration(model.identifier) ?? {};
 ...
 */
function getPriceCategoryLabel(
 ...
 */
function getModelConfigurationDescription(
	model: ILanguageModelChatMetadataAndIdentifier,
	languageModelsService: ILanguageModelsService,
): string | undefined {
	const schema = model.metadata.configurationSchema;
...
function createModelAction(
	...
	const pricingForDescription = isMultiplierPricing(model)
		? model.metadata.pricing
		: undefined;
	const priceCategoryLabel = isUBB
		? getPriceCategoryLabel(model.metadata.priceCategory)
		: undefined;
	// In PRU mode, show the current configuration value (e.g. thinking effort "High") in the description
	const configDescription = !isUBB
		? getModelConfigurationDescription(model, languageModelsService)
		: undefined;
	...
	const textParts = [configDescription, detail, pricingForDescription].filter(
		Boolean,
	);
	...
	const toolbarActions = !isUBB
		? languageModelsService.getModelConfigurationActions(model.identifier)
		: undefined;
	...
	const ariaDescription = priceCategoryLabel
		? textDescription
			? textDescription + " · " + priceCategoryLabel
			: priceCategoryLabel
		: undefined;
	return { action, ariaDescription };
}

function shouldShowManageModelsAction(
	chatEntitlementService: IChatEntitlementService,
): boolean {
	return (
		chatEntitlementService.clientByokEnabled ||
...
function createManageModelsAction(
	...
	if (useGroupedModelPicker) {
		...
		if (models.length) {
			...
			const resolveModel = (id: string) =>
				allModelsMap.get(id) ?? modelsByMetadataId.get(id);

			const getUnavailableReason = (
				entry: IModelControlEntry,
			): "upgrade" | "update" | "admin" => {
				const isBusinessOrEnterpriseUser =
					chatEntitlementService.entitlement === ChatEntitlement.Business ||
					chatEntitlementService.entitlement === ChatEntitlement.Enterprise;
				...
				if (
					entry.minVSCodeVersion &&
					!isVersionAtLeast(currentVSCodeVersion, entry.minVSCodeVersion)
				) {
					return "update";
				}
				return "admin";
			...
			if (autoModel) {
				...
				const { action: autoAction, ariaDescription: autoAriaDesc } =
					createModelAction(
						autoModel,
						selectedModelId,
						onSelect,
			...
			const allGroupKeys = new Set(
				models.map((m) => {
					const info = getProviderGroupForModel(
						m,
						modelToGroup,
						languageModelsService!,
					);
			...
			const makePinAction = (model: ILanguageModelChatMetadataAndIdentifier) =>
				onTogglePin
					? createPinAction(
							model.identifier,
							pinnedModelIds.includes(model.identifier),
							onTogglePin,
						)
					: undefined;
			...
			if (pinnedModels.length > 0) {
				items.push({
					...
					label: localize("chat.modelPicker.pinned", "Pinned"),
				});
				for (const model of pinnedModels) {
					const groupLabel = showGroupLabel
						...
						: undefined;
					const { action: pinnedAction, ariaDescription: pinnedAriaDesc } =
						createModelAction(
							model,
							selectedModelId,
							onSelect,
			...
			const filteredRecentIds = recentModelIds
			...
			if (promotedItems.length > 0) {
				...
				promotedItems.sort((a, b) => {
					...
					const aName =
						a.kind === "available" ? a.model.metadata.name : a.entry.label;
					const bName =
						b.kind === "available" ? b.model.metadata.name : b.entry.label;
			...
			otherModels = models.filter(
				(m) => !placed.has(m.identifier) && !placed.has(m.metadata.id),
			);
			...
			if (otherModels.length > 0) {
				...
				items.push({
					item: {
						...
						run: () => {
							/* toggle handled by isSectionToggle */
						},
					},
					kind: ActionListItemKind.Action,
					label: localize("chat.modelPicker.otherModels", "Other Models"),
				...
				for (const bucket of sortedBuckets) {
					...
					const sortedBucketModels = [...bucket.models].sort((a, b) => {
						const aEntry =
							controlModels[a.metadata.id] ?? controlModels[a.identifier];
						const bEntry =
							controlModels[b.metadata.id] ?? controlModels[b.identifier];
						const aAvail =
							aEntry?.minVSCodeVersion &&
							!isVersionAtLeast(currentVSCodeVersion, aEntry.minVSCodeVersion)
								? 1
								: 0;
						const bAvail =
							bEntry?.minVSCodeVersion &&
							!isVersionAtLeast(currentVSCodeVersion, bEntry.minVSCodeVersion)
								? 1
								: 0;
						if (aAvail !== bAvail) {
							return aAvail - bAvail;
					...
					for (const model of sortedBucketModels) {
						const entry =
							controlModels[model.metadata.id] ??
							controlModels[model.identifier];
						if (
							entry?.minVSCodeVersion &&
							!isVersionAtLeast(currentVSCodeVersion, entry.minVSCodeVersion)
						) {
							...
							const { action: bucketAction, ariaDescription: bucketAriaDesc } =
								createModelAction(
									model,
									selectedModelId,
									onSelect,
		...
		const autoModel = models.find((m) => isAutoModel(m));
		if (autoModel) {
			const { action: flatAutoAction, ariaDescription: flatAutoAriaDesc } =
				createModelAction(
					autoModel,
					selectedModelId,
					onSelect,
		...
		for (const model of sortedModels) {
			const { action: flatAction, ariaDescription: flatAriaDesc } =
				createModelAction(
					model,
					selectedModelId,
					onSelect,
...
export function getModelPickerAccessibilityProvider() {
	return {
		getAriaLabel(element: IActionListItem<IActionWidgetDropdownAction>) {
			...
			return [element.label, element.badge, description]
		...
		isChecked(element: IActionListItem<IActionWidgetDropdownAction>) {
			...
			return element.kind === ActionListItemKind.Action
...
export class ModelPickerWidget extends Disposable {
	private readonly _onDidChangeSelection = this._register(
		new Emitter<ILanguageModelChatMetadataAndIdentifier>(),
	);
	...
	constructor(
		...
		@IChatEntitlementService
		private readonly _entitlementService: IChatEntitlementService,
		...
		this._register(
			this._entitlementService.onDidChangeUsageBasedBilling(() => {
				this._renderLabel();
			}),
		);
	}
	...
	render(container: HTMLElement): void {
		this._domNode = dom.append(
			container,
			dom.$("div.action-label.model-picker-split"),
		);
		...
		// Model name button
		this._nameButton = dom.append(
			this._domNode,
			dom.$("a.model-picker-section.model-picker-name"),
		);
		...
		this._effortButton = dom.append(
			this._domNode,
			dom.$("a.model-picker-section.model-picker-effort"),
		);
		...
		this._tokensButton = dom.append(
			this._domNode,
			dom.$("a.model-picker-section.model-picker-tokens"),
		);
		...
		this._registerButtonAction(this._effortButton, () =>
			this._showEffortPicker(),
		);
		this._registerButtonAction(this._tokensButton, () =>
			this._showTokensPicker(),
		);
	...
	private _registerButtonAction(
		...
		this._register(
			dom.addDisposableGenericMouseDownListener(element, (e) => {
				if (e.button !== 0) {
					return;
				}
	...
	show(anchor?: HTMLElement): void {
		...
		const onSelect = (model: ILanguageModelChatMetadataAndIdentifier) => {
			this._telemetryService.publicLog2<
				ChatModelChangeEvent,
				ChatModelChangeClassification
			>("chat.modelChange", {
				fromModel:
					previousModel?.metadata.vendor === "copilot"
						? new TelemetryTrustedValue(previousModel.identifier)
						: "unknown",
				toModel:
					model.metadata.vendor === "copilot"
						? new TelemetryTrustedValue(model.identifier)
						: "unknown",
			});
			this._selectedModel = model;
			this._renderLabel();
		...
		const isSignedOut =
			this._entitlementService.entitlement === ChatEntitlement.Unknown;
		...
		const controlModelsForTier: IStringDictionary<IModelControlEntry> =
			isSignedOut
				? {}
		...
		const canShowManageModelsAction =
			this._delegate.showManageModelsAction() &&
			shouldShowManageModelsAction(this._entitlementService);
		const manageModelsAction = canShowManageModelsAction
			? createManageModelsAction(this._commandService)
			: undefined;
		const logModelPickerInteraction = (
			interaction: ChatModelPickerInteraction,
		) => {
			this._telemetryService.publicLog2<
				ChatModelPickerInteractionEvent,
				ChatModelPickerInteractionClassification
			>("chat.modelPickerInteraction", { interaction });
		};
		const manageSettingsUrl = this._defaultAccountService.resolveGitHubUrl(
		...
		const items = buildModelPickerItems(
			...
			this._languageModelsService
				...
				.filter((id) => !this._languageModelsService.isModelHidden(id)),
			this._languageModelsService
				.getPinnedModelIds()
				.filter((id) => !this._languageModelsService.isModelHidden(id)),
		...
		const listOptions = {
			...
			filterActions:
				!isUBB && manageModelsAction ? [manageModelsAction] : undefined,
			...
			onDidToggleSection: (section: string, collapsed: boolean) => {
				if (section === ModelPickerSection.Other) {
					logModelPickerInteraction(
						collapsed ? "otherModelsCollapsed" : "otherModelsExpanded",
					);
				}
			},
			linkHandler: (uri: URI) => {
				if (
					uri.scheme === "command" &&
					uri.path === "workbench.action.chat.upgradePlan"
				) {
		...
		if (
			dom.isHTMLInputElement(activeElement) &&
			activeElement.classList.contains("action-list-filter-input")
		) {
	...
	private _renderLabel(): void {
		...
		const configDescription =
			!isUBB && this._selectedModel
				? getModelConfigurationDescription(
						this._selectedModel,
						this._languageModelsService,
					)
				: undefined;
		const fullLabel = configDescription
			...
			: modelLabel;
		nameChildren.push(
			dom.$("span.chat-input-picker-label", undefined, fullLabel),
		);
		if (this._badgeIcon) {
		...
		if (effortConfig && this._effortButton) {
			...
			const enumIndex =
				effortConfig.schema.enum?.indexOf(effortConfig.value) ?? -1;
			const effortLabel =
				enumIndex >= 0 && effortConfig.schema.enumItemLabels?.[enumIndex]
					? effortConfig.schema.enumItemLabels[enumIndex]
					: String(effortConfig.value);
			dom.reset(
				this._effortButton,
				dom.$("span.chat-input-picker-label", undefined, effortLabel),
			);
		...
		if (tokensConfig && this._tokensButton) {
			...
			const tokensLabel =
				idx >= 0 && tokensConfig.schema.enumItemLabels?.[idx]
					? tokensConfig.schema.enumItemLabels[idx]
					: formatTokenCount(Number(tokensConfig.value));
			dom.reset(
				this._tokensButton,
				dom.$("span.chat-input-picker-label", undefined, tokensLabel),
			);
			...
			items.push({
				...
				group: {
					...
					icon: ThemeIcon.fromId(
						config.value === value ? Codicon.check.id : Codicon.blank.id,
					),
				},
				hideIcon: false,
			});
...
export function getModelHoverContent(
	...
	openerService: IOpenerService,
	isUBB?: boolean,
): { element: HTMLElement; disposable: DisposableStore } | undefined {
	...
	container.appendChild(
		dom.$(".chat-model-hover-name", undefined, model.metadata.name),
	);
	...
	if (!isAuto && isUBB) {
		...
		const buildCostLines = (
			...
			if (input !== undefined) {
				lines.push({
					...
					value: formatCostValue(input),
				});
			}
			if (cache !== undefined) {
				lines.push({
					label: localize("models.cacheCostLabel", "Cached input"),
					value: formatCostValue(cache),
				});
			}
			if (output !== undefined) {
				lines.push({
					label: localize("models.outputCostLabel", "Output"),
					value: formatCostValue(output),
				});
			}
			return lines;
		};
		const appendCostSection = (
			parent: HTMLElement,
			title: string,
			...
			titleRow.appendChild(
				dom.$(".chat-model-hover-cost-title", undefined, title),
			);
			if (categoryLabel) {
				titleRow.appendChild(
					dom.$("span.chat-model-hover-cost-tag", undefined, categoryLabel),
				);
			}
			...
			for (const line of lines) {
				section.appendChild(
					dom.$(
						...
						undefined,
						dom.$(
							"span.chat-model-hover-cost-line-label",
							undefined,
							`${line.label}: `,
						),
		...
		if (costLines.length > 0) {
			appendCostSection(
				container,
				localize("models.priceTitle", "Cost (per 1M tokens)"),
				costLines,
				priceCategoryLabel,
			);

			// Long-context pricing — only when it differs from default
			const longContextCostLines = buildCostLines(
			...
			titleRow.appendChild(
				dom.$(
					...
					undefined,
					localize("models.priceCategoryTitle", "Cost"),
				),
			);
			titleRow.appendChild(
				dom.$("span.chat-model-hover-cost-tag", undefined, priceCategoryLabel),
			);
			...
			costSection.appendChild(
				dom.$(
					"span",
					undefined,
					localize("models.cost", "Cost: {0}", model.metadata.pricing),
				),
			);
			container.appendChild(costSection);
		}
	...
	if (
		!isAuto &&
		(model.metadata.maxInputTokens || model.metadata.maxOutputTokens)
	) {
		const totalTokens =
		...
		contextSection.appendChild(
			dom.$(
				...
				undefined,
				localize("models.contextSize", "Max context"),
			),
		);
		contextSection.appendChild(
			dom.$(
	...
	if (!isAuto && isUBB && model.metadata.configurationSchema?.properties) {
		...
		for (const [, propSchema] of Object.entries(
			model.metadata.configurationSchema.properties,
		)) {
			if (propSchema.enum && propSchema.enum.length >= 2) {
		...
		if (configurableLabels.length > 0) {
			...
			configRow.appendChild(
				dom.$(
					"span.chat-model-hover-configurable-label",
					undefined,
					localize("models.configurable", "Configurable:"),
				),
			);
			for (const label of configurableLabels) {
				configRow.appendChild(
					dom.$("span.chat-model-hover-configurable-tag", undefined, label),
				);
			}
	...
	return container.children.length > 0
		? { element: container, disposable: disposables }
		: undefined;
}
...
function isAutoModel(model: ILanguageModelChatMetadataAndIdentifier): boolean {
	return (
		model.metadata.id === "auto" &&
		(model.metadata.vendor === "copilot" ||
			model.metadata.vendor === "copilotcli")
	);
}
