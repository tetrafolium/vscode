...
} from "./chatInputNotificationService.js";
...
export class ChatInputNotificationWidget extends Disposable {
	...
	private _render(): void {
		...
		const notification = this._notificationService.getActiveNotification((n) =>
			this._matchesSession(n),
		);
		if (!notification) {
	...
	private _renderNotification(notification: IChatInputNotification): void {
		...
		const headerRow = dom.append(
			container,
			$(".chat-input-notification-header"),
		);

		// Severity icon
		const iconElement = dom.append(
			headerRow,
			$(".chat-input-notification-icon"),
		);
		iconElement.appendChild(
			dom.$(ThemeIcon.asCSSSelector(severityToIcon[notification.severity])),
		);

		// Title
		const titleElement = dom.append(
			headerRow,
			$(".chat-input-notification-title"),
		);
		if (isMarkdownString(notification.message)) {
			const rendered = this._contentDisposables.add(
				this._markdownRendererService.render(notification.message),
			);
		...
		const ariaTitle = isMarkdownString(notification.message)
			? notification.message.value
			: notification.message;

		// Dismiss button (in header row, pushed to the right)
		if (notification.dismissible) {
			const dismissButton = dom.append(
				headerRow,
				$(".chat-input-notification-dismiss"),
			);
			...
			this._contentDisposables.add(
				dom.addDisposableListener(dismissButton, dom.EventType.CLICK, dismiss),
			);
			this._contentDisposables.add(
		...
		if (hasBody) {
			...
			if (notification.description) {
				const descriptionElement = dom.append(
					bodyRow,
					$(".chat-input-notification-description"),
				);
			...
			if (notification.actions.length > 0) {
				const actionsContainer = dom.append(
					bodyRow,
					$(".chat-input-notification-actions"),
				);
				...
				for (let i = 0; i < notification.actions.length; i++) {
					...
					const button = this._contentDisposables.add(
						new Button(actionsContainer, {
							...defaultButtonStyles,
							...(!isLast
								? {
