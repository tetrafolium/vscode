...
namespace ToolEnablementStates {
	...
	): ToolEnablementStates {
		const toolSets: Map<string, boolean> = new Map(),
	...
	function isStoredDataV1(
		data: StoredDataV1 | StoredDataV2 | undefined,
	): data is StoredDataV1 {
		return (
			isObject(data) &&
			data.version === undefined &&
			(data.disabledTools === undefined || Array.isArray(data.disabledTools)) &&
			(data.disabledToolSets === undefined ||
				Array.isArray(data.disabledToolSets))
		);
	}

	function isStoredDataV2(
		data: StoredDataV1 | StoredDataV2 | undefined,
	): data is StoredDataV2 {
		return (
			isObject(data) &&
			data.version === 2 &&
			Array.isArray(data.toolSetEntries) &&
...
export class ChatSelectedTools extends Disposable {
	...
	private readonly _sessionStates = new ObservableMap<
		string,
		ToolEnablementStates | undefined
	>();
	...
	constructor(
		...
		this._currentTools = languageModel
			.map((lm) => _toolsService.observeTools(lm?.metadata))
			.map((o, r) => o.read(r));
	}
	 ...
	 */
	public readonly entriesMap: IObservable<IToolAndToolSetEnablementMap> =
		derived((r) => {
			...
			let currentMap = this._sessionStates.observable
				.read(r)
				.get(currentMode.id);
			if (!currentMap && currentMode.kind === ChatModeKind.Agent) {
				const modeTools = currentMode.customTools?.read(r);
				if (modeTools) {
					currentMap = ToolEnablementStates.fromMap(
						this._toolsService.toToolAndToolSetEnablementMap(modeTools, lm),
					);
				}
			}
			if (!currentMap) {
			...
			for (const toolSet of this._toolsService.getToolSetsForModel(lm, r)) {
				...
				for (const tool of toolSet.getTools(r)) {
					map.set(
						tool,
						toolSetEnabled || currentMap.tools.get(tool.id) === true,
					); // if unknown, use toolSetEnabled
				}
			}
			return map;
		});

	public readonly userSelectedTools: IObservable<UserSelectedTools> = derived(
		(r) => {
	...
	private async updateCustomModeTools(
		uri: URI,
		enablementMap: IToolAndToolSetEnablementMap,
	): Promise<void> {
		await this._instantiationService
			.createInstance(PromptFileRewriter)
			.openAndRewriteTools(uri, enablementMap, CancellationToken.None);
	}
}
