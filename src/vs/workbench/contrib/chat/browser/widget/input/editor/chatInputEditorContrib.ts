...
} from "../../../../common/widget/chatColors.js";
import {
...
import {
	...
	slashReg,
	variableReg,
} from "../../../../common/requestParser/chatRequestParser.js";
...
function exactlyOneSpaceAfterPart(
	parsedRequest: readonly IParsedChatRequestPart[],
	...
	return (
		nextPart && nextPart instanceof ChatRequestTextPart && nextPart.text === " "
	);
}
...
class InputEditorDecorations extends Disposable {
	...
	private mouseDownPromptSlashCommand:
	...
	private readonly updateThrottle = this._register(
		new ThrottledDelayer<void>(InputEditorDecorations.UPDATE_DELAY),
	);
	...
	constructor(
		...
		this._register(
			this.widget.inputEditor.onMouseDown((e) => {
				...
				if (
					...
					!clickablePromptSlashCommand.range.containsPosition(e.target.position)
				) {
					return;
				}
		...
		this._register(
			this.widget.inputEditor.onMouseUp((e) => {
				...
				if (
					!mouseDownPromptSlashCommand.range.containsPosition(
						e.target.position,
					) ||
					!Position.equals(
						mouseDownPromptSlashCommand.position,
						e.target.position,
					)
				) {
					return;
				...
				void this.editorService.openEditor({
					resource: mouseDownPromptSlashCommand.uri,
				});
			}),
		);
		this._register(
		...
		this._register(
			this.customizationHarnessService.onDidChangeSlashCommands((e) => {
				const sessionResource = this.widget.viewModel?.sessionResource;
				if (
					sessionResource &&
					e.sessionType === getChatSessionType(sessionResource)
				) {
					this.triggerInputEditorDecorationsUpdate();
				}
			}),
	...
	private registeredDecorationTypes() {
		...
		this._register(
			this.codeEditorService.registerDecorationType(
				decorationDescription,
				clickableSlashPromptTextDecorationType,
				{
					color: themeColorFromId(chatSlashCommandForeground),
					backgroundColor: themeColorFromId(chatSlashCommandBackground),
					borderRadius: "3px",
	...
	private triggerInputEditorDecorationsUpdate(): void {
		...
		this.updateThrottle
			.trigger((token) => this.updateAsyncInputEditorDecorations(token))
			.catch((err) => {
	...
	private updateInputPlaceholderDecoration(): void {
		...
		if (!inputValue) {
			...
			const placeholder =
				mode.argumentHint?.get() ?? mode.description.get() ?? "";
		...
		if (onlyAgentAndWhitespace) {
			...
			if (
				...
				exactlyOneSpaceAfterPart(parsedRequest, agentPart)
			) {
		...
		if (onlyAgentAndAgentCommandAndWhitespace) {
			...
			if (
				...
				exactlyOneSpaceAfterPart(parsedRequest, agentSubcommandPart)
			) {
				placeholderDecoration = [
					{
	...
	private async updateAsyncInputEditorDecorations(
		...
		const slashPromptPart = parsedRequest.find(
			(p): p is ChatRequestSlashPromptPart =>
				p instanceof ChatRequestSlashPromptPart,
		);

		// first, fetch all async context
		const promptSlashCommand = slashPromptPart
			? await this.customizationHarnessService.resolvePromptSlashCommand(
					slashPromptPart.name,
		...
		if (slashPromptPart && promptSlashCommand) {
			...
			if (
				...
				exactlyOneSpaceAfterPart(parsedRequest, slashPromptPart) &&
				promptSlashCommand
			) {
				const description =
					promptSlashCommand.argumentHint ?? promptSlashCommand.description;
				if (description) {
					this.widget.inputEditor.setDecorationsByType(
						decorationDescription,
						placeholderDecorationType,
		...
		if (slashCommandPart) {
			textDecorations.push({
				range: slashCommandPart.editorRange,
				hoverMessage: new MarkdownString(slashCommandPart.slashCommand.detail),
			});
		}
		...
		const toolParts = parsedRequest.filter(
			(p): p is ChatRequestToolPart =>
				p instanceof ChatRequestToolPart || p instanceof ChatRequestToolSetPart,
		);
		for (const tool of toolParts) {
			varDecorations.push({ range: tool.editorRange });
		}

		const dynamicVariableParts = parsedRequest.filter(
			(p): p is ChatRequestDynamicVariablePart =>
				p instanceof ChatRequestDynamicVariablePart,
		);
		...
		if (isEditingPreviousRequest) {
			for (const variable of dynamicVariableParts) {
				varDecorations.push({
					range: variable.editorRange,
					hoverMessage: URI.isUri(variable.data)
						? new MarkdownString(
								this.labelService.getUriLabel(variable.data, {
									relative: true,
								}),
							)
						: undefined,
	...
	private updateAriaPlaceholder(value: string | undefined): void {
		...
		if (nativeEditContext) {
			...
			const textAreaEditContext = TextAreaEditContextRegistry.get(
				this.widget.inputEditor.getId(),
			);
			...
			this.widget.inputEditor.onDidChangeModelContent((e) => {
				...
				if (e.changes.length === 1) {
					...
					if (change.text.length > 0 && change.rangeLength === 1) {
						...
						if (
							slashReg.test(change.text) ||
							agentReg.test(change.text) ||
							variableReg.test(change.text)
						) {
							insertedTokenRange = new Range(
						...
						prevInsertTokenRange &&
						change.range.endColumn === prevInsertTokenRange.endColumn
					) {
