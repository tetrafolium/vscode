...
} from "./chatModelSelectionLogic.js";
import {
	getChatSessionType,
	LocalChatSessionUri,
...
} from "../../agentSessions/agentHost/agentHostChatInputPicker.js";
import {
...
} from "../../agentSessions/agentHost/agentHostChatInputPicker.contribution.js";
...
} from "./chatPhoneInputPresenter.js";
import { IChatContextService } from "../../contextContrib/chatContextService.js";
import { IDisposableReference } from "../chatContentParts/chatCollections.js";
import {
	ChatPlanReviewPart,
	IChatPlanReviewPartOptions,
} from "../chatContentParts/chatPlanReviewPart.js";
import {
	ChatQuestionCarouselPart,
	IChatQuestionCarouselOptions,
} from "../chatContentParts/chatQuestionCarouselPart.js";
import {
	ChatToolConfirmationCarouselPart,
	ToolInvocationPartFactory,
	ScrollToSubagentCallback,
} from "../chatContentParts/toolInvocationParts/chatToolConfirmationCarouselPart.js";
...
import {
	...
	IModelPickerDelegate,
} from "./modelPickerActionItem.js";
import {
	IModePickerDelegate,
	ModePickerActionItem,
} from "./modePickerActionItem.js";
import {
	IPermissionPickerDelegate,
	PermissionPickerActionItem,
} from "./permissionPickerActionItem.js";
...
const emptyInputState = observableMemento<IChatModelInputState | undefined>({
	...
	fromStorage(value) {
		...
		if (obj.selectedModel && !obj.selectedModel.metadata.isDefaultForLocation) {
			...
			type OldILanguageModelChatMetadata = ILanguageModelChatMetadata & {
				isDefault?: boolean;
			};
			const oldIsDefault = (
				obj.selectedModel.metadata as OldILanguageModelChatMetadata
			).isDefault;
	...
	implements IHistoryNavigationWidget
{
	...
	private _workingSetCollapsed = observableValue(
		"chatInputPart.workingSetCollapsed",
		true,
	);
	private _stableInputPartWidth = observableValue(
		"chatInputPart.stableInputPartWidth",
		0,
	);
	private readonly _chatInputTodoListWidget = this._register(
		new MutableDisposable<ChatTodoListWidget>(),
	);
	private readonly _chatArtifactsWidget = this._register(
		new MutableDisposable<ChatArtifactsWidget>(),
	);
	private readonly _chatQuestionCarouselWidgets = this._register(
		new DisposableMap<string, ChatQuestionCarouselPart>(),
	);
	...
	private readonly _chatPlanReviewWidgets = this._register(
		new DisposableMap<string, ChatPlanReviewPart>(),
	);
	...
	private readonly _chatToolConfirmationCarousels = this._register(
		new DisposableMap<string, ChatToolConfirmationCarouselPart>(),
	);
	private readonly _chatEditingTodosDisposables = this._register(
		new DisposableStore(),
	);
	...
	private readonly _toolbarRelayoutScheduler = this._register(
	...
	readonly onDidChangeContext: Event<{
		removed?: IChatRequestVariableEntry[];
		added?: IChatRequestVariableEntry[];
	}> = this._onDidChangeContext.event;
	...
	readonly onDidAcceptFollowup: Event<{
		followup: IChatFollowup;
		response: IChatResponseViewModel | undefined;
	}> = this._onDidAcceptFollowup.event;
	...
	private readonly followupsDisposables: DisposableStore = this._register(
		new DisposableStore(),
	);
	...
	private readonly _notificationWidget = this._register(
		new MutableDisposable<ChatInputNotificationWidget>(),
	);

	private contextUsageWidget?: ChatContextUsageWidget;
	private contextUsageWidgetContainer!: HTMLElement;
	private readonly _contextUsageDisposables = this._register(
	...
	private readonly _modelSyncDisposables = this._register(
		new DisposableStore(),
	);
	private readonly _currentChatModes = this._register(
		new MutableDisposable<IChatModes & IDisposable>(),
	);
	...
	private editingSentRequestKey!: IContextKey<
		ChatContextKeys.EditingRequestType | undefined
	>;
	...
	private readonly permissionWidgetDisposeListener = this._register(
	...
	private readonly chatSessionPickerWidgets = this._register(
		new DisposableMap<string, ChatSessionPickerActionItem>(),
	);
	...
	private readonly _waitForPersistedLanguageModel: MutableDisposable<IDisposable> =
		this._register(new MutableDisposable<IDisposable>());
	private readonly _waitForSessionHistoryLanguageModel: MutableDisposable<IDisposable> =
		this._register(new MutableDisposable<IDisposable>());
	private readonly _chatSessionOptionEmitters = this._register(
		new DisposableMap<string, Emitter<IChatSessionProviderOptionItem>>(),
	);
	 ...
	 */
	private readonly _optionContextKeys: Map<string, IContextKey<string>> =
		new Map();

	private _currentLanguageModel = observableValue<
		ILanguageModelChatMetadataAndIdentifier | undefined
	>("_currentLanguageModel", undefined);

	get currentLanguageModel() {
		return this._currentLanguageModel.get()?.identifier;
	}

	get selectedLanguageModel(): IObservable<
		ILanguageModelChatMetadataAndIdentifier | undefined
	> {
		return this._currentLanguageModel;
	}

	private _onDidChangeCurrentChatMode: Emitter<void> = this._register(
		new Emitter<void>(),
	);
	...
	public get currentModeInfo(): IChatRequestModeInfo {
		...
		const modeId: "ask" | "agent" | "edit" | "custom" | undefined =
			mode.isBuiltin ? this.currentModeKind : "custom";
		...
		return {
			...
			modeInstructions: modeInstructions
				? {
						...
						toolReferences: this.toolService.toToolReferences(
							modeInstructions.toolReferences,
						),
	...
	readonly inputUri: URI = URI.parse(
		`${Schemas.vscodeChatInput}:input-${ChatInputPart._counter++}`,
	);

	private _workingSetLinesAddedSpan = new Lazy(() =>
		dom.$(".working-set-lines-added"),
	);
	private _workingSetLinesRemovedSpan = new Lazy(() =>
		dom.$(".working-set-lines-removed"),
	);

	private readonly _chatEditsActionsDisposables: DisposableStore =
		this._register(new DisposableStore());
	private readonly _chatEditsDisposables: DisposableStore = this._register(
		new DisposableStore(),
	);
	private readonly _renderingChatEdits = this._register(
		new MutableDisposable(),
	);

	private _chatEditsListPool: CollapsibleListPool;
	private _chatEditList:
		| IDisposableReference<WorkbenchList<IChatCollapsibleListItem>>
		| undefined;
	...
	private _pendingDelegationTarget: AgentSessionProviders | undefined =
		undefined;
	...
	constructor(
		...
		private readonly sharedWebExtracterService: ISharedWebContentExtractorService,
		@IChatEntitlementService
		private readonly entitlementService: IChatEntitlementService,
		@IChatModeService private readonly chatModeService: IChatModeService,
		@ILanguageModelToolsService
		private readonly toolService: ILanguageModelToolsService,
		@IChatSessionsService
		private readonly chatSessionsService: IChatSessionsService,
		@IChatContextService
		private readonly chatContextService: IChatContextService,
		...
		private readonly _chatAttachmentWidgetRegistry: IChatAttachmentWidgetRegistry,
		@IChatInputNotificationService
		private readonly chatInputNotificationService: IChatInputNotificationService,
		@IChatPhoneInputPresenter
		private readonly chatPhoneInputPresenter: IChatPhoneInputPresenter,
	) {
		super();

		// Initialize debounced text sync scheduler
		this._syncTextDebounced = this._register(
		...
		this._currentModeObservable = observableValue<IChatMode>(
			"currentMode",
			this.options.defaultMode ?? ChatMode.Agent,
		);
		const localModes = this.chatModeService.createModes(
			LocalChatSessionUri.getNewSessionUri(),
		);
		this._currentChatModes.value = localModes;
		this._currentChatModesObservable = observableValue<IChatModes>(
			"currentChatModes",
			localModes,
		);
		this._currentPermissionLevel = observableValue<ChatPermissionLevel>(
			"permissionLevel",
			this.getDefaultPermissionLevel(),
		);
		this._register(
		...
		this._register(
			this.chatSessionsService.onDidChangeSessionOptions((e) => {
		...
		this._register(
			this.chatSessionsService.onDidChangeOptionGroups((chatSessionType) => {
				const sessionResource = this._widget?.viewModel?.model.sessionResource;
				if (sessionResource) {
					const delegateSessionType =
						this.options.sessionTypePickerDelegate?.getActiveSessionProvider?.();
					if (
						getChatSessionType(sessionResource) === chatSessionType ||
						delegateSessionType === chatSessionType
					) {
						this.refreshChatSessionPickers();
					}
				}
		...
		if (
			this.options.sessionTypePickerDelegate?.onDidChangeActiveSessionProvider
		) {
			this._register(
				this.options.sessionTypePickerDelegate.onDidChangeActiveSessionProvider(
					async (newSessionType) => {
						this.getVisibleOptionGroupsModeAndUpdateContextKeys(
							this.getCurrentSessionResource(),
						);
						this.agentSessionTypeKey.set(newSessionType);
						this.chatSessionSupportsDelegationKey.set(
							this.chatSessionsService.supportsDelegationForSessionType(
								newSessionType,
							),
						);
		...
		this._register(
			this._attachmentModel.onDidChange(() => this._syncInputStateToModel()),
		);
		...
		const padding =
			this.options.renderStyle === "compact"
				? INPUT_EDITOR_PADDING.compact
				: INPUT_EDITOR_PADDING.default;
		this.inputEditorMinHeight = this.options.inputEditorMinLines
			? this.options.inputEditorMinLines * INPUT_EDITOR_LINE_HEIGHT +
				padding.top +
				padding.bottom
			: undefined;

		this.inputEditorHasText =
			ChatContextKeys.inputHasText.bindTo(contextKeyService);
		this.inputEditorHasSendableContent =
			ChatContextKeys.inputHasSendableContent.bindTo(contextKeyService);
		this.chatCursorAtTop =
			ChatContextKeys.inputCursorAtTop.bindTo(contextKeyService);
		this.inputEditorHasFocus =
			ChatContextKeys.inputHasFocus.bindTo(contextKeyService);
		this._hasQuestionCarouselContextKey =
			ChatContextKeys.Editing.hasQuestionCarousel.bindTo(contextKeyService);
		this.chatModeKindKey =
			ChatContextKeys.chatModeKind.bindTo(contextKeyService);
		this.chatModeNameKey =
			ChatContextKeys.chatModeName.bindTo(contextKeyService);
		this.chatModelIdKey = ChatContextKeys.chatModelId.bindTo(contextKeyService);
		this.permissionLevelKey =
			ChatContextKeys.chatPermissionLevel.bindTo(contextKeyService);
		this.permissionLevelKey.set(this._currentPermissionLevel.get());
		this.withinEditSessionKey =
			ChatContextKeys.withinEditSessionDiff.bindTo(contextKeyService);
		this.filePartOfEditSessionKey =
			ChatContextKeys.filePartOfEditSession.bindTo(contextKeyService);
		this.chatSessionHasOptions =
			ChatContextKeys.chatSessionHasModels.bindTo(contextKeyService);
		this.chatSessionOptionsValid =
			ChatContextKeys.chatSessionOptionsValid.bindTo(contextKeyService);
		this.agentSessionTypeKey =
			ChatContextKeys.agentSessionType.bindTo(contextKeyService);
		this.chatSessionSupportsDelegationKey =
			ChatContextKeys.chatSessionSupportsDelegation.bindTo(contextKeyService);
		...
		this.chatSessionHasCustomAgentTarget =
			ChatContextKeys.chatSessionHasCustomAgentTarget.bindTo(contextKeyService);
		this.chatSessionHasTargetedModels =
			ChatContextKeys.chatSessionHasTargetedModels.bindTo(contextKeyService);
		...
		this._register(
			this.configurationService.onDidChangeConfiguration((e) => {
				...
				if (e.affectsConfiguration("editor.wordSegmenterLocales")) {
					newOptions.wordSegmenterLocales = this.configurationService.getValue<
						string | string[]
					>("editor.wordSegmenterLocales");
				}
				if (e.affectsConfiguration("editor.autoClosingBrackets")) {
					newOptions.autoClosingBrackets = this.configurationService.getValue(
						"editor.autoClosingBrackets",
					);
				}
				if (e.affectsConfiguration("editor.autoClosingQuotes")) {
					newOptions.autoClosingQuotes = this.configurationService.getValue(
						"editor.autoClosingQuotes",
					);
				}
				if (e.affectsConfiguration("editor.autoSurround")) {
					newOptions.autoSurround = this.configurationService.getValue(
						"editor.autoSurround",
					);
				}
		...
		this._hasFileAttachmentContextKey =
			ChatContextKeys.hasFileAttachments.bindTo(contextKeyService);
		...
		const resetCurrentLanguageModelIfUnavailable = () => {
			...
			if (canLog(this.logService.getLevel(), LogLevel.Debug)) {
				...
				const messageparts: string[] = [
					...
					`vendors: ${this.languageModelsService
						.getVendors()
						.map((v) => v.vendor)
						.join(", ")}`,
		...
		this._register(
			this.languageModelsService.onDidChangeLanguageModels(
				resetCurrentLanguageModelIfUnavailable,
			),
		);
		this._register(
		...
		this._register(
			this.onDidChangeCurrentChatMode(() => {
				this.accessibilityService.alert(
					this._currentModeObservable.get().label.get(),
				);
		...
		this._register(
			autorun((reader) => {
				...
				reader.store.add(
					modes.onDidChange(() => this.validateCurrentChatMode()),
				);
			}),
		);
		this._register(
	...
	private setImplicitContextEnablement() {
		if (
			...
			this.implicitContext.setEnabled(
				this._currentModeObservable.get().name.get().toLowerCase() === "ask",
			);
		}
	}

	public setIsWithinEditSession(
		inInsideDiff: boolean,
		isFilePartOfEditSession: boolean,
	) {
	...
	public setEditing(
		enabled: boolean,
		editingSentRequest: ChatContextKeys.EditingRequestType | undefined,
	) {
	...
	public switchModel(
		modelMetadata: Pick<ILanguageModelChatMetadata, "vendor" | "id" | "family">,
	) {
		const models = this.getModels();
		const model = models.find(
			(m) =>
				m.metadata.vendor === modelMetadata.vendor &&
				m.metadata.id === modelMetadata.id &&
				m.metadata.family === modelMetadata.family,
		);
		if (model) {
	...
	public switchModelByQualifiedName(
		qualifiedModelNames: readonly string[],
	): boolean {
		const models = this.getModels();
		for (const qualifiedModelName of qualifiedModelNames) {
			const model = models.find((m) =>
				ILanguageModelChatMetadata.matchesQualifiedName(
					qualifiedModelName,
					m.metadata,
				),
			);
	...
	public switchToNextModel(): void {
		...
		if (models.length > 0) {
			const currentIndex = models.findIndex(
				(model) =>
					model.identifier === this._currentLanguageModel.get()?.identifier,
			);
	...
	public switchToNextPinnedModel(): void {
		...
		const currentIndex = pinnedModels.findIndex(
			(model) =>
				model.identifier === this._currentLanguageModel.get()?.identifier,
		);
	...
	private _showCombinedPhonePickerSheet(): void {
		...
		this.chatPhoneInputPresenter
			...
			.catch((err) =>
				this.logService.error("[ChatInputPart] phone picker sheet failed", err),
			);
	}
	...
	private _createModePickerDelegate(): IModePickerDelegate {
		return {
			...
			customAgentTarget: () => {
				...
				return (
					(sessionResource &&
						this.chatSessionsService.getCustomAgentTargetForSessionType(
	...
	private createChatSessionPickerWidgets(
		...
		const sessionResource = this.getCurrentSessionResource();
		const visibleOptionGroups =
			this.getVisibleOptionGroupsModeAndUpdateContextKeys(sessionResource);
	...
	setInputModel(
		...
		forSessionResource: URI,
	): void {
		logChangesToStateModel(
			this._inputModel,
			`setInputModel for ${forSessionResource.toString()}`,
			model.state.get(),
			undefined,
			this.logService,
		);
		...
		if (chatSessionIsEmpty) {
			logChangesToStateModel(
				this._inputModel,
				`(1) setting empty model state for ${forSessionResource.toString()}`,
				undefined,
				undefined,
				this.logService,
			);
			...
			this._modelSyncDisposables.add(
				this.configurationService.onDidChangeConfiguration((e) => {
					if (
						this._chatSessionIsEmpty &&
						e.affectsConfiguration(ChatConfiguration.DefaultNewSessionMode)
					) {
						logChangesToStateModel(
							this._inputModel,
							`(2) setting empty model state for ${forSessionResource.toString()}`,
							undefined,
							undefined,
							this.logService,
						);
			...
			this._modelSyncDisposables.add(
				this._currentChatModesObservable.get().onDidChange(() => {
					if (this._chatSessionIsEmpty) {
						logChangesToStateModel(
							this._inputModel,
							`(3) setting empty model state for ${forSessionResource.toString()}`,
							undefined,
							undefined,
							this.logService,
						);
	...
	private _setEmptyModelState() {
		logChangesToStateModel(
			this._inputModel,
			`setting empty model state for ${this._widget?.viewModel?.sessionResource.toString()} in ${this._currentSessionKey}`,
			undefined,
			undefined,
			this.logService,
		);
		...
		const rawDefaultMode = this.configurationService.getValue<string>(
		...
		if (typeof rawDefaultMode === "string") {
			...
			if (defaultMode) {
				...
				const resolved =
					modes.findModeById(defaultMode) ??
					modes.findModeByName(defaultMode) ??
					modes.custom.find(
						(m) => m.name.get().toLowerCase() === defaultModeLower,
					);
				if (resolved) {
	...
	private _syncFromModel(
		...
		try {
			...
			if (
				...
				const targetLevel = this.getPermittedPermissionLevel(
					state?.permissionLevel ?? ChatPermissionLevel.Default,
				);
	...
	public setCurrentLanguageModel(
		model: ILanguageModelChatMetadataAndIdentifier,
	) {
		const modelDetails = this.getModels()
			.map((m) => `${m.identifier} (${m.metadata.id})`)
			.join(", ");
		logChangesToStateModel(
			this._inputModel,
			`setCurrentLanguageModel to ${model.identifier} in ${this._currentSessionKey}, modelDetials = ${modelDetails}`,
			undefined,
			undefined,
			this.logService,
		);
	...
	private getAllMergedModels(): ILanguageModelChatMetadataAndIdentifier[] {
		const cachedModels = this.storageService.getObject<
			ILanguageModelChatMetadataAndIdentifier[]
		>(CachedLanguageModelsKey, StorageScope.APPLICATION, []);
		const liveModels = this.languageModelsService
			.getLanguageModelIds()
			.map((modelId) => ({
				identifier: modelId,
		...
		const contributedVendors = new Set(
			this.languageModelsService.getVendors().map((v) => v.vendor),
		);
		...
		const models = mergeModelsWithCache(
			liveModels,
			cachedModels,
			contributedVendors,
			resolvedVendors,
	...
	private getModels(): ILanguageModelChatMetadataAndIdentifier[] {
		...
		const sessionFiltered = filterModelsForSession(
			models,
			this.getCurrentSessionType(),
			this.currentModeKind,
			this.location,
		...
		return sessionFiltered.filter(
			(m) => !this.languageModelsService.isModelHidden(m.identifier),
		);
	}
	...
	private hasModelsTargetingSessionType(): boolean {
		return hasModelsTargetingSession(
			this.getAllMergedModels(),
			this.getCurrentSessionType(),
		);
	}
	...
	private checkModeInSessionPool(sessionType?: string): void {
		...
		const customAgentTarget =
			this.chatSessionsService.getCustomAgentTargetForSessionType(sessionType);
	...
	private preselectModelFromSessionHistory(): void {
		...
		if (
			!requests ||
			requests.length === 0 ||
			getChatSessionType(sessionResource) !== SessionType.CopilotCLI
		) {
			return;
		}
		...
		const tryMatch = () => {
			...
			if (
				models.length === 1 &&
				models[0].metadata.id.toLocaleLowerCase() === "auto"
			) {
				return;
			}
		...
		this._waitForSessionHistoryLanguageModel.value =
			this.languageModelsService.onDidChangeLanguageModels(() => {
				const currentSessionResource =
					this._widget?.viewModel?.model.sessionResource;
				if (
					!currentSessionResource ||
					!isEqual(currentSessionResource, sessionResource)
				) {
					this._waitForSessionHistoryLanguageModel.clear();
					return;
				}
	...
	private setCurrentLanguageModelToDefault() {
		...
		if (defaultModel) {
			logChangesToStateModel(
				this._inputModel,
				`setCurrentLanguageModelToDefault to ${defaultModel.identifier} in ${this._currentSessionKey}`,
				undefined,
				undefined,
				this.logService,
			);
	...
	private _getAriaLabel(): string {
		...
		let kbLabel;
		if (verbose) {
			kbLabel = this.keybindingService
		...
		const modelInfo = modelName
			? localize("chatInput.model", ", {0}. ", modelName)
			: "";
	...
	private validateCurrentChatMode() {
		const currentMode = this._currentModeObservable.get();
		const validMode = this._currentChatModesObservable
			.get()
			.findModeById(currentMode.id);
		const isAgentModeEnabled = this.configurationService.getValue<boolean>(
			ChatConfiguration.AgentEnabled,
		);
		if (!validMode) {
			this.setChatMode(
				isAgentModeEnabled ? ChatModeKind.Agent : ChatModeKind.Ask,
			);
			return;
		}
	...
	logInputHistory(): void {
		const historyStr = this.history.values
	...
	async restoreAttachments(
		...
		if (restored.length > 0) {
			restored = (
				await Promise.all(
					restored.map(async (attachment) => {
						if (
							isImageVariableEntry(attachment) &&
							!attachment.value &&
							...
							try {
								const imageBinary = currReference
									.toString(true)
									.startsWith("http")
									? await this.sharedWebExtracterService.readImage(
											currReference,
								...
								newAttachment.value =
									isImageVariableEntry(attachment) && attachment.isPasted
										? imageBinary.buffer
										: await resizeImage(imageBinary.buffer);
	...
	validateAgentMode(): void {
		if (
			...
			this._currentModeObservable.get().kind === ChatModeKind.Agent
		) {
	...
	private _getFilteredEntry(
		...
		const attachmentsWithoutImageValues = inputState.attachments.map(
			(attachment) => {
				if (
					isImageVariableEntry(attachment) &&
					attachment.references?.length &&
	...
	private _handleAttachedContextChange() {
		this._hasFileAttachmentContextKey.set(
			Boolean(this._attachmentModel.attachments.find((a) => a.kind === "file")),
		);
	...
	private _updateInputContentContextKeys(): void {
		...
		this.inputEditorHasSendableContent.set(
			inputHasText ||
				this._attachmentModel.attachments.some(
					isExplicitFileOrImageVariableEntry,
				),
		);
	}
	...
	): Emitter<IChatSessionProviderOptionItem> {
	...
	private getOrCreateOptionContextKey(
		...
		if (!contextKey) {
			const rawKey = new RawContextKey<string>(
				`chatSessionOption.${optionGroupId}`,
				"",
			);
	 ...
	 */
	private evaluateOptionGroupVisibility(optionGroup: {
		id: string;
		when?: string;
	}): boolean {
		if (!optionGroup.when) {
	...
	private getVisibleOptionGroupsModeAndUpdateContextKeys(
		...
		this.chatSessionHasCustomAgentTarget.set(
			customAgentTarget !== Target.Undefined,
		);

		// Check if this session type requires custom models
		const requiresCustomModels =
			sessionResource &&
			this.chatSessionsService.requiresCustomModelsForSessionType(
				getChatSessionType(sessionResource),
			);
		...
		const allOptionsValid = sessionResource
			? this.areAllOptionsValid(sessionResource, visibleOptionGroups)
			: true;
	...
	private areAllOptionsValid(
		sessionResource: URI,
		visibleOptionGroups: readonly IChatSessionProviderOptionGroup[],
	): boolean {
		for (const optionGroup of visibleOptionGroups) {
			const currentOption = this.chatSessionsService.getSessionOption(
				sessionResource,
				optionGroup.id,
			);
			if (currentOption) {
				const currentOptionId =
					typeof currentOption === "string" ? currentOption : currentOption.id;
				// TODO: @osortega @joshspicer should we add a `placeHolder` item to option groups to straighten this check?
				if (
					!optionGroup.items.some((item) => item.id === currentOptionId) &&
	...
	private getAllOptionsGroups(
		...
		const effectiveSessionType =
			delegateSessionType ??
			(sessionResource ? getChatSessionType(sessionResource) : undefined);
		if (!effectiveSessionType) {
			return [];
		}
	...
	private getVisibleOptionGroups(
		...
		for (const optionGroup of allOptionGroups) {
			...
			const hasItems =
				optionGroup.items.length > 0 || (optionGroup.commands || []).length > 0;
			...
			const sessionHasOption =
				!sessionResource ||
				this.chatSessionsService.getSessionOption(
					sessionResource,
					optionGroup.id,
				) !== undefined;
	...
	private getActiveExtensionPermissionGroup(
		sessionResource: URI | undefined,
	): IChatSessionProviderOptionGroup | undefined {
		const allOptionGroups = this.getAllOptionsGroups(sessionResource);
	...
	private refreshChatSessionPickers(): void {
		...
		const needsRecreation =
			...
			!visibleOptionGroups.every((group) =>
				currentWidgetGroupIds.has(group.id),
			);

		if (
			needsRecreation &&
			this._lastSessionPickerAction &&
			this.chatSessionPickerContainer
		) {
			const widgets = this.createChatSessionPickerWidgets(
				this._lastSessionPickerAction,
				this._lastSessionPickerOptions,
			);
		...
		if (sessionResource) {
			for (const [optionGroupId] of this.chatSessionPickerWidgets) {
				...
				if (currentOption) {
					const optionGroup = allOptionsGroups.find(
					...
					if (optionGroup) {
						const currentOptionId =
							typeof currentOption === "string"
								? currentOption
								: currentOption.id;
						const item = optionGroup.items.find(
							(m: IChatSessionProviderOptionItem) => m.id === currentOptionId,
						);
	...
	private getCurrentOptionForGroup(
		...
		const effectiveSessionType = this.getEffectiveSessionType(sessionResource);
		const optionGroups = effectiveSessionType
			? this.chatSessionsService.getOptionGroupsForSessionType(
					effectiveSessionType,
				)
			: undefined;
		...
		const currentOptionValue = this.chatSessionsService.getSessionOption(
			sessionResource,
			optionGroupId,
		);
	...
	private updateAgentSessionTypeContextKey(): void {
		...
		const delegateSessionType =
			delegate?.setActiveSessionProvider &&
			delegate?.getActiveSessionProvider?.();
		const sessionType =
			delegateSessionType ||
			(sessionResource ? getChatSessionType(sessionResource) : "");

		this.agentSessionTypeKey.set(sessionType);
		this.chatSessionSupportsDelegationKey.set(
			this.chatSessionsService.supportsDelegationForSessionType(sessionType),
		);
	}
	...
	private ensureNotificationWidget(): void {
		if (!this._notificationWidget.value) {
			...
			this.chatInputNotificationContainer.appendChild(
				this._notificationWidget.value.domNode,
			);
		}
	}
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		this._register(
			widget.onDidChangeViewModel((e: IChatWidgetViewModelChangeEvent) => {
				...
				if (
					this._questionCarouselSessionResources.size > 0 &&
					(!e.currentSessionResource || !hasMatchingResource)
				) {
				...
				if (
					this._planReviewSessionResources.size > 0 &&
					(!e.currentSessionResource || !hasMatchingPlanReviewResource)
				) {
				...
				if (
					...
					newSessionType !== this._currentSessionType
				) {
		...
		if (this.options.renderStyle === "compact") {
			elements = dom.h(".interactive-input-part", [
				dom.h(".interactive-input-and-edit-session", [
					...
					dom.h(
						".chat-question-carousel-widget-container@chatQuestionCarouselContainer",
					),
					dom.h(
						".chat-tool-confirmation-carousel-container@chatToolConfirmationCarouselContainer",
					),
					dom.h(
						".chat-input-notification-container@chatInputNotificationContainer",
					),
					dom.h(
						".chat-todo-list-widget-container@chatInputTodoListWidgetContainer",
					),
					dom.h(
						".chat-artifacts-widget-container@chatArtifactsWidgetContainer",
					),
					dom.h(".chat-editing-session@chatEditingSessionWidgetContainer"),
					dom.h(
						".chat-getting-started-tip-container@chatGettingStartedTipContainer",
					),
		...
		this.chatEditingSessionWidgetContainer =
			elements.chatEditingSessionWidgetContainer;
		this.chatInputTodoListWidgetContainer =
			elements.chatInputTodoListWidgetContainer;
		this.chatArtifactsWidgetContainer = elements.chatArtifactsWidgetContainer;
		this.chatGettingStartedTipContainer =
			elements.chatGettingStartedTipContainer;
		...
		this.chatToolConfirmationCarouselContainer =
			elements.chatToolConfirmationCarouselContainer;
		dom.hide(this.chatToolConfirmationCarouselContainer);
		this.chatInputNotificationContainer =
			elements.chatInputNotificationContainer;
		this.contextUsageWidgetContainer = elements.contextUsageWidgetContainer;

		if (
			this.options.isSessionsWindow ||
			this.options.renderStyle === "compact"
		) {
		...
		this.contextUsageWidget = this._register(
			this.instantiationService.createInstance(ChatContextUsageWidget),
		);
		this.contextUsageWidgetContainer.appendChild(
			this.contextUsageWidget.domNode,
		);
		...
		this._register(
			this._attachmentModel.onDidChange((e) => {
				if (e.added.length > 0) {
					this._indexOfLastAttachedContextDeletedWithKeyboard = -1;
				}
				this._handleAttachedContextChange();
		...
		this.currentlyEditingInputKey =
			ChatContextKeys.currentlyEditingInput.bindTo(
				inputScopedContextKeyService,
			);
		this.editingSentRequestKey = ChatContextKeys.editingRequestType.bindTo(
		...
		options.autoClosingBrackets = this.configurationService.getValue(
			"editor.autoClosingBrackets",
		);
		options.autoClosingQuotes = this.configurationService.getValue(
			"editor.autoClosingQuotes",
		);
		options.autoSurround = this.configurationService.getValue(
			"editor.autoSurround",
		);
		options.suggest = {
			showIcons: true,
		...
		this._register(
			this._inputEditor.onKeyDown((e) => {
				if (e.keyCode === KeyCode.Enter && !hasModifierKeys(e)) {
					...
					for (const keybinding of this.keybindingService.lookupKeybindings(
						ChatSubmitAction.ID,
					)) {
		...
		this._register(
			this._inputEditor.onDidChangeModelContent(() => {
				const currentHeight = Math.min(
					...
					this._effectiveInputEditorMaxHeight,
				);
		...
		const pickerOptions: IChatInputPickerOptions = {
			...
			compact: derived(
				(reader) =>
					this._stableInputPartWidth.read(reader) <
					CHAT_INPUT_PICKER_COLLAPSE_WIDTH,
			),
		};
				...
				MenuId.ChatInput,
				{
					...
					responsiveBehavior: {
						...
						getActionMinWidth: (action) =>
							shorterChatInputActionIds.has(action.id) ? 22 : undefined,
					},
					actionViewItemProvider: (action, options) => {
						...
						if (this.chatPhoneInputPresenter.enabled.get()) {
							if (
								action.id === OpenModelPickerAction.ID &&
								...
								action.id === OpenModePickerAction.ID &&
								action instanceof MenuItemAction
							) {
						...
						if (
							...
							const getActiveSessionType = () => {
								...
								return sessionResource
									? (getAgentSessionProvider(sessionResource) ??
											getChatSessionType(sessionResource))
									: undefined;
							};
							const delegate: ISessionTypePickerDelegate = this.options
								...
								setPendingDelegationTarget: (
									provider: AgentSessionProviders,
								) => {
									const isActive = getActiveSessionType() === provider;
									this._pendingDelegationTarget = isActive
										? undefined
										: provider;
							...
							const isWelcomeViewMode =
								!!this.options.sessionTypePickerDelegate
							...
							return (this.sessionTargetWidget =
								this.instantiationService.createInstance(
									Picker,
									action,
		...
		this._register(
			this.inputActionsToolbar.onDidChangeMenuItems(() => {
				...
				const primaryPickerContainer = toolbarElement.querySelector(
					".chat-sessionPicker-container",
				);
				if (primaryPickerContainer) {
					this.chatSessionPickerContainer =
						primaryPickerContainer as HTMLElement;
				}
				if (
					this.cachedWidth &&
					typeof this.cachedInputToolbarWidth === "number" &&
					this.cachedInputToolbarWidth !==
		...
		const genericChipsLane = this._register(
			this.instantiationService.createInstance(
		...
		this.secondaryToolbar = this._register(
			this.instantiationService.createInstance(
				MenuWorkbenchToolBar,
				...
				MenuId.ChatInputSecondary,
				{
					...
					responsiveBehavior: {
						...
						getActionMinWidth: (action) =>
							agentHostShortPickerMinWidths.get(action.id),
					},
					actionViewItemProvider: (action, options) => {
						if (
							(action.id === OpenSessionTargetPickerAction.ID ||
							...
							const delegate: IPermissionPickerDelegate = {
								...
								getExtensionPermissions: () => {
									const sessionResource = this.getCurrentSessionResource();
									const group =
										this.getActiveExtensionPermissionGroup(sessionResource);
									if (!group) {
										return undefined;
									}
									const current = sessionResource
										? this.chatSessionsService.getSessionOption(
												sessionResource,
												group.id,
											)
									...
									const defaultId =
										group.selected?.id ??
										group.items.find((i) => i.default)?.id;
									const rawSelectedId =
										current === undefined
											? defaultId
									...
									const selectedId =
										...
										group.items.some((i) => i.id === rawSelectedId)
											? rawSelectedId
											: defaultId;
		...
		this._register(
			this.secondaryToolbar.onDidChangeMenuItems(() => {
				...
				// eslint-disable-next-line no-restricted-syntax
				const container = toolbarElement.querySelector(
					".chat-sessionPicker-container",
				);
		...
		if (!inputModel) {
			inputModel = this._register(
				this.modelService.createModel("", null, this.inputUri, true),
			);
		}

		this.textModelResolverService
			.createModelReference(this.inputUri)
			.then((ref) => {
		...
		const updateCarouselMaxHeightScheduler = this._register(
			new dom.AnimationFrameScheduler(this.container, () =>
				this.updateToolConfirmationCarouselMaxHeight(),
			),
		);
		const inputResizeObserver = this._register(
			new dom.DisposableResizeObserver("ChatInputPart.containerHeight", () => {
		...
		if (this.options.renderStyle === "compact") {
			const toolbarsResizeObserver = this._register(
				new dom.DisposableResizeObserver(
					"ChatInputPart.compactToolbars",
					() => {
	...
	public toggleChatInputOverlay(editing: boolean): void {
		...
		if (editing) {
			this.overlayClickListener.value = dom.addStandardDisposableListener(
				this.chatInputOverlay,
				dom.EventType.CLICK,
				(e) => {
					e.preventDefault();
					e.stopPropagation();
	...
	public renderAttachedContext() {
		...
		dom.setVisibility(
			hasAttachments || hasImplicitContext,
		...
		const maxImagesPerRequest = getImageAttachmentLimit(
			this._currentLanguageModel.get()?.metadata,
		);
		const imageAttachments = attachments.filter(([, a]) =>
			isImageVariableEntry(a),
		);
		if (
			maxImagesPerRequest !== undefined &&
			imageAttachments.length > maxImagesPerRequest
		) {
			...
			for (let i = excessCount; i < imageAttachments.length; i++) {
				if (
					imageAttachments[i][1].omittedState ===
					OmittedState.ImageLimitExceeded
				) {
		...
		for (const [index, attachment] of attachments) {
			const resource = URI.isUri(attachment.value)
				...
				: isLocation(attachment.value)
					...
					: undefined;
			const range = isLocation(attachment.value)
				? attachment.value.range
				: undefined;
			const shouldFocusClearButton =
				index ===
					Math.min(
			...
			if (attachment.kind === "tool" || attachment.kind === "toolset") {
				...
				attachmentWidget =
					this._chatAttachmentWidgetRegistry.createWidget(
						attachment,
						options,
						container,
					) ??
			...
			if (
				index ===
				Math.min(this._indexOfLastOpenedContext, this.attachmentModel.size - 1)
			) {
				attachmentWidget.element.focus();
			}
	...
	private handleAttachmentDeletion(
		...
		if (
			this.configurationService.getValue<boolean>(
				"chat.implicitContext.enableImplicitContext",
			)
		) {
			...
			for (const implicitContext of this._implicitContext?.values || []) {
				const implicitValue =
					URI.isUri(implicitContext?.value) &&
					URI.isUri(attachment.value) &&
					isEqual(implicitContext.value, attachment.value);
	...
	private handleAttachmentNavigation(e: StandardKeyboardEvent): void {
		...
		const attachments = Array.from(
			this.attachedContextContainer.querySelectorAll(
		...
		const activeElement = dom.getWindow(this.attachmentsContainer).document
			.activeElement;
		const currentIndex = attachments.findIndex(
			(attachment) => attachment === activeElement,
		);
	...
	async renderChatTodoListWidget(chatSessionResource: URI) {
		...
		if (!this._chatInputTodoListWidget.value) {
			const widget = this._chatEditingTodosDisposables.add(
				this.instantiationService.createInstance(ChatTodoListWidget),
			);
	...
	renderArtifactsWidget(chatSessionResource: URI): void {
		...
		if (!this._chatArtifactsWidget.value) {
			const widget = this._register(
				this.instantiationService.createInstance(ChatArtifactsWidget),
			);
	...
	renderQuestionCarousel(
		...
		options: IChatQuestionCarouselOptions,
	): ChatQuestionCarouselPart {
		const carouselKey =
			carousel.resolveId ??
			`${isResponseVM(context.element) ? context.element.requestId : ""}_${context.contentIndex}`;
		...
		if (isResponseVM(context.element)) {
			this._questionCarouselResponseIds.set(
				carouselKey,
				context.element.requestId,
			);
			this._questionCarouselSessionResources.set(
				carouselKey,
	...
	clearQuestionCarousel(responseId?: string, resolveId?: string): void {
		...
		this._hasQuestionCarouselContextKey?.set(
			this._chatQuestionCarouselWidgets.size > 0,
		);
	}
	...
	get questionCarousel(): ChatQuestionCarouselPart | undefined {
		...
		return this._chatQuestionCarouselWidgets.size > 0
			? this._chatQuestionCarouselWidgets.values().next().value
			: undefined;
	}
	...
	renderPlanReview(
		...
		context: IChatContentPartRenderContext,
		options: IChatPlanReviewPartOptions,
	): ChatPlanReviewPart {
		const key =
			review.resolveId ??
			`${isResponseVM(context.element) ? context.element.requestId : ""}_${context.contentIndex}`;
		...
		if (isResponseVM(context.element)) {
			...
			this._planReviewSessionResources.set(
				key,
				context.element.sessionResource,
			);
		}
	...
	get planReview(): ChatPlanReviewPart | undefined {
		return this._chatPlanReviewWidgets.size > 0
			? this._chatPlanReviewWidgets.values().next().value
			: undefined;
	}
	...
	renderToolConfirmationCarousel(
		tool: IChatToolInvocation,
		factory: ToolInvocationPartFactory,
		...
		if (!key) {
			throw new Error(
				"Cannot render tool confirmation carousel without an active session",
			);
		}

		const part = new ChatToolConfirmationCarouselPart(
			factory,
			[],
			scrollToSubagent,
			subAgentInvocationId,
	...
	addToolToConfirmationCarousel(
		...
		if (existing) {
			...
			this.renderToolConfirmationCarousel(
				tool,
				factory,
				subAgentInvocationId,
				agentName,
	...
	activateCarouselForSubagent(subAgentInvocationId: string): void {
		this._currentToolConfirmationCarousel?.activateFirstToolForSubagent(
			subAgentInvocationId,
		);
	}
	...
	hasToolInConfirmationCarousel(toolCallId: string): boolean {
		return (
			this._currentToolConfirmationCarousel?.hasToolInvocation(toolCallId) ??
			false
		);
	}
	...
	renderChatEditingSessionState(
		...
		if (chatEditingSession) {
			if (
				!isEqual(
					chatEditingSession.chatSessionResource,
					this._lastEditingSessionResource,
				)
			) {
		...
		const modifiedEntries = derivedOpts<IModifiedFileEntry[]>(
		...
		const sessionFileChanges = observableFromEvent(
			...
			() => {
				...
				return model?.changes instanceof Array
					? model.changes
					: Iterable.empty();
			},
		);
		...
		const sessionFiles = derived((reader) =>
			sessionFileChanges.read(reader).map(
				(entry): IChatCollapsibleListItem => ({
		...
		const shouldRender = derived(
			(reader) =>
				editSessionEntries.read(reader).length > 0 ||
				sessionFiles.read(reader).length > 0,
		);
	...
	private renderChatEditingSessionWithEntries(
		...
		const innerContainer =
			(this.chatEditingSessionWidgetContainer.querySelector(
		...
		// eslint-disable-next-line no-restricted-syntax
		const overviewTitle =
			(overviewRegion.querySelector(".working-set-title") as HTMLElement) ??
		...
		const scopedContextKeyService = this._chatEditsActionsDisposables.add(
			this.contextKeyService.createScoped(actionsContainer),
		);
		if (sessionResource) {
			scopedContextKeyService.createKey(
				ChatContextKeys.agentSessionType.key,
				getChatSessionType(sessionResource),
			);
		}
		...
		const scopedInstantiationService = this._chatEditsActionsDisposables.add(
			this.instantiationService.createChild(
		...
		// Working set
		// eslint-disable-next-line no-restricted-syntax
		const workingSetContainer =
			(innerContainer.querySelector(
				".chat-editing-session-list",
			) as HTMLElement) ??
			dom.append(innerContainer, $(".chat-editing-session-list"));

		const button = this._chatEditsActionsDisposables.add(
			new ButtonWithIcon(overviewTitle, {
				supportIcons: true,
				secondary: true,
				ariaLabel: localize(
		...
		const topLevelStats = derived((reader) => {
			...
			const topLevelIsSessionMenu =
				entries.length === 0 && sessionEntries.length > 0;
			const shouldShowEditingSession =
				entries.length > 0 || sessionEntries.length > 0;
		...
		const topLevelIsSessionMenu = topLevelStats.map(
			(t) => t.topLevelIsSessionMenu,
		);
					...
					scopedInstantiationService.createInstance(
							...
							: MenuId.ChatEditingWidgetToolbar,
						{
							...
							menuOptions: sessionResource
								? isSessionMenu
									? {
											args: [
												sessionResource,
												this.agentSessionsService.getSession(sessionResource)
													?.metadata,
											],
										}
									: {
							...
							buttonConfigProvider: (action) => {
								if (
									action.id === ChatEditingShowChangesAction.ID ||
									action.id === ViewPreviousEditsAction.Id
								) {
									return {
										showIcon: true,
										showLabel: false,
		...
		store.add(
			autorun((reader) => {
				const { files, added, removed, shouldShowEditingSession } =
		...
		const toggleWorkingSet = () => {
			this._workingSetCollapsed.set(
				!this._workingSetCollapsed.get(),
				undefined,
			);
		};
		...
		this._chatEditsActionsDisposables.add(
			addDisposableListener(overviewRegion, "click", (e) => {
		...
		if (!this._chatEditList) {
			...
			this._chatEditsDisposables.add(
				list.onDidOpen(async (e) => {
					if (
						e.element?.kind === "reference" &&
						URI.isUri(e.element.reference)
					) {
						...
						if (pane) {
							entry
								...
								.reveal(true, e.editorOptions.preserveFocus);
						}
					}
				}),
			);
	...
	private updateToolConfirmationCarouselMaxHeight(): void {
		...
		const carouselHeight =
			this.chatToolConfirmationCarouselContainer.offsetHeight;
		const otherInputHeight = Math.max(
			0,
			this.container.offsetHeight - carouselHeight,
		);
	...
	private _updateWorkingProgressAnimationDuration(width: number): void {
		...
		if (
			this._lastAnimDurationS !== undefined &&
			Math.abs(this._lastAnimDurationS - duration) < 0.05
		) {
			return;
		}
		this._lastAnimDurationS = duration;
		this.inputContainer.style.setProperty(
	...
	private get _effectiveInputEditorMaxHeight(): number {
		...
		const nonEditorHeight = Math.max(
			0,
			this.height.get() - currentEditorHeight,
		);
	...
	private _layout(width: number, allowRecurse = true): void {
		...
		const inputEditorHeight = this.inputEditorMinHeight
			? Math.min(
	...
	private getLayoutData() {
		...
		const inputSideToolbarWidth = this.inputSideToolbarContainer
			? dom.getTotalWidth(this.inputSideToolbarContainer)
			: 0;
		...
		const getToolbarsWidthCompact = () => {
			...
			const inputToolbarWidth = (this.cachedInputToolbarWidth =
				this.inputActionsToolbar.getItemsWidth());
			const executeToolbarPadding =
				(this.executeToolbar.getItemsLength() - 1) * toolbarItemGap;
			const inputToolbarPadding = this.inputActionsToolbar.getItemsLength()
				? (this.inputActionsToolbar.getItemsLength() - 1) * toolbarItemGap
				: 0;
			const contextUsageWidth = dom.getTotalWidth(
				this.contextUsageWidgetContainer,
			);
			const inputToolbarsPadding = 12; // pdading between input toolbar/execute toolbar/contextUsage.
			return (
				executeToolbarWidth +
				executeToolbarPadding +
				contextUsageWidth +
				(this.options.renderInputToolbarBelowInput
					? 0
					: inputToolbarWidth + inputToolbarPadding + inputToolbarsPadding)
			);
		};
		...
		return {
			...
			inputPartHorizontalPadding:
				this.options.renderStyle === "compact" ? 16 : 24,
			inputPartHorizontalPaddingInside:
				this.options.renderStyle === "compact" ? 12 : 10,
			toolbarsWidth:
				this.options.renderStyle === "compact" ? getToolbarsWidthCompact() : 0,
			sideToolbarWidth:
				inputSideToolbarWidth > 0 ? inputSideToolbarWidth + 4 /*gap*/ : 0,
		};
	}
					...
					return {
						location:
							sideBarPosition === Position.LEFT
								? ChatWidgetLocation.SidebarRight
								: ChatWidgetLocation.SidebarLeft,
