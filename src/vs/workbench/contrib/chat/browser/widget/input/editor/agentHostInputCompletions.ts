 ...
 */
export class AgentHostInputCompletions extends AgentHostInputCompletionsBase<
	IChatWidget,
	string
> {
	...
	constructor(
		...
		this._register(
			this._chatSessionsService.onDidChangeContentProviderSchemes(
				({ added, removed }) => {
	...
	private async _registerForScheme(scheme: string): Promise<void> {
		...
		if (
			!this._chatSessionsService.getContentProviderSchemes().includes(scheme)
		) {
			return;
		}
	...
	protected override _resolveContext(
		model: ITextModel,
		scheme: string,
	): { sessionResource: URI; context: IChatWidget } | undefined {
			...
			case "skill": {
				...
				return {
					label: attachment.displayName
						? { label, description: attachment.displayName }
						: label,
					insertText: item.insertText,
					filterText: item.insertText,
					range: replaceRange,
			...
			default: {
				...
				return {
					...
					command: {
						...
						title: "",
						arguments: [
							new AgentHostReferenceArgument(
								widget,
								attachment.uri,
								attachment.displayName,
								!!attachment.isDirectory,
								replaceRange.replace.setEndPosition(
