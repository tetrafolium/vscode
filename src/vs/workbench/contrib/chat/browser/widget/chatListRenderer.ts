...
} from "./chatContentParts/chatThinkingContentPart.js";
	...
	implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate>
{
	...
	private fireItemHeightChange(
		...
		if (typeof originalStoredHeight === "number") {
			...
			dom.scheduleAtNextAnimationFrame(
				dom.getWindow(template.rowContainer),
				() => {
					if (
						template.currentElement !== element ||
						element.currentRenderedHeight !== scheduledHeight
					) {
						return;
					}
					this._onDidChangeItemHeight.fire({
	...
	private renderChatRequest(
		...
		if (!element.confirmation) {
			...
			const attachmentSummary = !element.messageText.trim()
		...
		partsToRender.forEach((partToRender, contentIndex) => {
			...
			if (
				...
				(partToRender.kind === "toolInvocation" ||
					...
					partToRender.kind === "externalEdit" ||
	...
	private finalizeCurrentThinkingPart(
		...
		const style = getEffectiveThinkingDisplayMode(
			this.configService,
	...
	private setupConfirmationTransitionWatcher(
		...
		const moveConfirmationWidgetOutOfThinking = (): ChatToolInvocationPart => {
			...
			if (createdPart?.domNode) {
				...
				(templateData.movedOutToolParts ??= new DisposableMap()).set(
	...
	private renderExternalEdit(
		content: IChatExternalEdit,
		context: IChatContentPartRenderContext,
		templateData: IChatListItemTemplate,
	): IChatContentPart {
		const editPart = this.instantiationService.createInstance(
		...
		if (
			isResponseVM(context.element) &&
			collapsedToolsMode !== CollapsedToolsDisplayMode.Off &&
			this.shouldPinPart(content, context.element)
		) {
