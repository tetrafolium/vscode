...
export class ChatDragAndDrop extends Themable {
	...
	private async resolveAttachmentsFromDragEvent(
		e: DragEvent,
		...
		if (containsDragType(e, CodeDataTransfers.NOTEBOOK_CELL_OUTPUT)) {
			...
			if (notebookOutputData) {
				return this.chatAttachmentResolveService.resolveNotebookOutputAttachContext(
					notebookOutputData,
				);
			}
		}
		...
		if (containsDragType(e, CodeDataTransfers.SCM_HISTORY_ITEM)) {
			...
			if (scmHistoryItemData) {
				return this.chatAttachmentResolveService.resolveSourceControlHistoryItemAttachContext(
					scmHistoryItemData,
				);
			}
		}
		...
		if (markerData) {
			return this.chatAttachmentResolveService.resolveMarkerAttachContext(
				markerData,
			);
		}
		...
		if (containsDragType(e, CodeDataTransfers.SYMBOLS)) {
			...
			return this.chatAttachmentResolveService.resolveSymbolsAttachContext(
				symbolsData,
			);
		}
			...
			return coalesce(
				await Promise.all(
					editorDragData.map((editorInput) => {
						return this.chatAttachmentResolveService.resolveEditorAttachContext(
							editorInput,
						);
					}),
				),
		...
		if (internal) {
			...
			if (uriList.length) {
				return coalesce(
					await Promise.all(
						uriList.map((uri) =>
							this.chatAttachmentResolveService.resolveEditorAttachContext({
		...
		if (
			...
			(containsDragType(e, Mimes.html) ||
				containsDragType(
					e,
					Mimes.text,
				)) /* Text mime needed for safari support */
		) {
	...
	private async downloadImageAsUint8Array(
		url: string,
	...
	private async resolveHTMLAttachContext(
		e: DragEvent,
	): Promise<IChatRequestVariableEntry[]> {
		const existingAttachmentNames = new Set<string>(
			this.attachmentModel.attachments.map((attachment) => attachment.name),
		);
		...
		const getImageTransferDataFromUrl = async (
			url: string,
		): Promise<ImageTransferData | undefined> => {
		...
		return await this.chatAttachmentResolveService.resolveImageAttachContext(
			imageTransferData,
		);
	}
...
function extractUrlsFromDragEvent(
	e: DragEvent,
