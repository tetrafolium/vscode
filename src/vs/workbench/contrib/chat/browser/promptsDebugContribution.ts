 ...
 */
export class PromptsDebugContribution
	extends Disposable
	implements IWorkbenchContribution
{
	 ...
	 */
	private readonly _discoveryEventDetails = new Map<
		string,
		IPromptDiscoveryInfo
	>();
	private readonly _customizationEventDetails = new Map<
		string,
		ICustomizationEventData
	>();
	...
	constructor(
		...
		this._register(
			chatAgentService.onWillInvokeAgent(async (e) => {
				...
				if (isFirstInvocation) {
					...
					try {
						const discoveryInfos = await Promise.all(
							[
								PromptsType.agent,
								...
								this.promptsService.getDiscoveryInfo(type, cts.token),
							),
						);
						for (const discoveryInfo of discoveryInfos) {
							const { name, details } =
								this.getDiscoveryLogEntry(discoveryInfo);
							...
							if (
								this._discoveryEventDetails.size >
								PromptsDebugContribution.MAX_DISCOVERY_DETAILS
							) {
							...
							const loaded = discoveryInfo.files
								...
								.map(
									(f) =>
										f.promptPath.name ??
										f.promptPath.uri.path.split("/").pop() ??
										f.promptPath.uri.toString(),
								);
							const skipped = discoveryInfo.files
							...
							const folders =
								discoveryInfo.sourceFolders?.map((sf) => sf.uri.path) ?? [];
							const parts: string[] = [];
							if (details) {
								parts.push(details);
							}
						...
						logService.error(
							"Error while logging prompt discovery info to chat debug service",
							error,
						);
					} finally {
						cts.dispose();
				...
				if (!isFirstInvocation && lastResult) {
					...
					try {
						const hookDiscoveryInfo =
							(await this.promptsService.getDiscoveryInfo(
								PromptsType.hook,
								CancellationToken.None,
							)) as IHookDiscoveryInfo;
						resolvedHooks = hookDiscoveryInfo.hooksInfo?.hooks;
					} catch (error) {
						logService.warn(
							"Error while fetching hooks for customization debug event",
							error,
						);
					}
					...
					if (collectionEvent.applyingInstructionsCount > 0) {
						parts.push(
							localize(
								"customizations.applying",
								"{0} applying",
								collectionEvent.applyingInstructionsCount,
							),
						);
					}
					if (collectionEvent.referencedInstructionsCount > 0) {
						parts.push(
							localize(
								"customizations.referenced",
								"{0} referenced",
								collectionEvent.referencedInstructionsCount,
							),
						);
					}
					if (collectionEvent.agentInstructionsCount > 0) {
						parts.push(
							localize(
								"customizations.agent",
								"{0} agent",
								collectionEvent.agentInstructionsCount,
							),
						);
					}
					if (collectionEvent.listedInstructionsCount > 0) {
						parts.push(
							localize(
								...
								"{0} listed",
								collectionEvent.listedInstructionsCount,
							),
						);
					}
					...
					const summary =
						parts.length > 0
							? localize(
									"customizationsResolved.details",
							...
							: localize(
									"customizationsResolved.none",
									"No customizations resolved",
								);
					...
					this._customizationEventDetails.set(customizationEventId, {
						debugInfo,
						hooks: resolvedHooks,
					});
					...
					// Evict oldest entries when the map exceeds the cap.
					if (
						this._customizationEventDetails.size >
						PromptsDebugContribution.MAX_DISCOVERY_DETAILS
					) {
		...
		this._register(
			chatDebugService.registerProvider({
				provideChatDebugLog: async () => undefined,
				resolveChatDebugLogEvent: async (eventId) => {
					return (
						this._resolveDiscoveryEvent(eventId) ??
						this._resolveCustomizationEvent(eventId)
					);
				},
			}),
		);
	...
	private getDiscoveryLogEntry(discoveryInfo: IPromptDiscoveryInfo): {
		...
		switch (discoveryInfo.type) {
			case PromptsType.prompt:
				return {
					name: localize(
						"promptsService.loadSlashCommands",
						"Slash Commands Discovery",
					),
					details:
						loadedCount === 1
							? localize(
									"promptsDebugContribution.resolvedSlashCommand",
									"Resolved {0} slash command in {1}ms",
									loadedCount,
									durationInMillis,
								)
							: localize(
									"promptsDebugContribution.resolvedSlashCommands",
			...
			case PromptsType.agent:
				return {
					...
					details:
						loadedCount === 1
							? localize(
									"promptsDebugContribution.resolvedAgent",
									"Resolved {0} agent in {1}ms",
									loadedCount,
									durationInMillis,
								)
							: localize(
									"promptsDebugContribution.resolvedAgents",
			...
			case PromptsType.skill:
				return {
					...
					details:
						loadedCount === 1
							? localize(
									"promptsDebugContribution.resolvedSkill",
									"Resolved {0} skill in {1}ms",
									loadedCount,
									durationInMillis,
								)
							: localize(
									"promptsDebugContribution.resolvedSkills",
			...
			case PromptsType.instructions:
				return {
					...
					details:
						loadedCount === 1
							? localize(
									"promptsDebugContribution.resolvedInstruction",
									"Resolved {0} instruction in {1}ms",
									loadedCount,
									durationInMillis,
								)
							: localize(
									"promptsDebugContribution.resolvedInstructions",
	...
	private _resolveDiscoveryEvent(
		eventId: string,
	): IChatDebugResolvedEventContent | undefined {
	...
	private _resolveCustomizationEvent(
		eventId: string,
	): IChatDebugResolvedEventContent | undefined {
			...
			for (const hookType of Object.values(HookType)) {
				...
				if (commands && commands.length > 0) {
					for (const cmd of commands) {
						const commandLabel =
							formatHookCommandLabel(cmd, OS) ||
							localize("hook.unknownCommand", "(unknown command)");
						logs.push({
							category: "hook",
							name: commandLabel,
							reason: hookType,
	...
	private _toFileListContent(
		info: IPromptDiscoveryInfo,
	): IChatDebugEventFileListContent {
		return {
			kind: "fileList",
...
function truncateList(items: string[]): string {
	...
	return (
		items.slice(0, MAX_LIST_ITEMS).join(", ") +
