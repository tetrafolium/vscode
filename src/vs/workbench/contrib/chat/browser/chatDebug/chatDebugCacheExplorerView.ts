...
import {
	IChatDebugEventModelTurnContent,
	IChatDebugMessageSection,
...
interface ITurnGroup {
	...
	readonly turns: readonly {
		readonly turn: IChatDebugModelTurnEvent;
...
export class ChatDebugCacheExplorerView extends Disposable {
	private readonly _onNavigate = this._register(
		new Emitter<CacheExplorerNavigation>(),
	);
	...
	private readonly resolvedCache = new Map<
		string,
		IChatDebugEventModelTurnContent | undefined
	>();
	...
	constructor(
		...
		this._register(
			this.sash.onDidStart(() => (sashStartWidth = this.railWidth)),
		);
		this._register(
			this.sash.onDidEnd(() => {
		...
		this._register(
			this.sash.onDidChange((e) => {
				...
				const next = Math.max(
					RAIL_MIN_WIDTH,
					Math.min(RAIL_MAX_WIDTH, sashStartWidth + delta),
				);
				this.railWidth = next;
		...
		this.refreshScheduler = this._register(
			new RunOnceScheduler(() => this.render(), 50),
		);
	}
	...
	refresh(): void {
		if (
			...
			!this.refreshScheduler.isScheduled()
		) {
	...
	updateBreadcrumb(): void {
		...
		this.breadcrumbWidget.setItems([
			...
			new TextBreadcrumbItem(
				localize("chatDebug.cacheExplorer", "Cache Explorer"),
			),
		]);
	}
	...
	private async render(): Promise<void> {
		...
		this.modelTurns = events.filter(
			(e): e is IChatDebugModelTurnEvent => e.kind === "modelTurn",
		);
		const userMessages = events.filter(
			(e): e is IChatDebugUserMessageEvent => e.kind === "userMessage",
		);
		...
		if (
			...
			this.selectedIndex >= this.modelTurns.length
		) {
		...
		const aEvent =
			this.selectedIndex > 0
				? this.modelTurns[this.selectedIndex - 1]
				: undefined;
		...
		const [a, b] = await Promise.all([
			this.resolveSide(aEvent),
			this.resolveSide(bEvent),
		]);
	...
	private async resolveSide(
		event: IChatDebugModelTurnEvent,
	): Promise<ISideData> {
		...
		if (system) {
			while (
				stripFrom < rawMessages.length &&
				rawMessages[stripFrom].role === "system"
			) {
				stripFrom++;
			}
		}
		const inputMessages =
			stripFrom > 0 ? rawMessages.slice(stripFrom) : rawMessages;
		return {
			event,
			content,
			system,
	...
	private renderRail(groups: readonly ITurnGroup[]): void {
		for (const group of groups) {
			...
			const headerLine = DOM.append(
				topLine,
				$(".chat-debug-cache-group-prompt"),
			);
			headerLine.textContent =
				group.userMessage?.message?.trim() ||
				localize("chatDebug.cache.unknownPrompt", "(no prompt captured)");
			const countBadge = DOM.append(
				topLine,
				$("span.chat-debug-cache-group-count"),
			);
			...
			this.loadDisposables.add(
				DOM.addDisposableListener(header, DOM.EventType.CLICK, toggle),
			);
			this.loadDisposables.add(
			...
			for (const { turn: evt, index: i } of group.turns) {
				...
				this.loadDisposables.add(
					DOM.addDisposableListener(row, DOM.EventType.CLICK, select),
				);
				this.loadDisposables.add(
					DOM.addDisposableListener(
						row,
	...
	private renderSummary(
		a: ISideData,
		b: ISideData,
		diff: ICacheDiffResult,
	): void {
		const row = DOM.append(this.content, $(".chat-debug-cache-summary"));
		row.appendChild(
			this.renderSideCard(
				a,
				localize("chatDebug.cache.previousRequest", "Previous request"),
			),
		);
		row.appendChild(
			this.renderSideCard(
				b,
				localize("chatDebug.cache.requestTitle", "Request"),
			),
		);
		...
		DOM.append(
			...
			$(
				...
				undefined,
				localize("chatDebug.cache.performance", "Cache performance"),
			),
		);
		...
		const headline = DOM.append(
			breakCard,
			$(".chat-debug-cache-card-headline"),
		);
		if (expiration) {
			headline.textContent = localize(
		...
		DOM.append(
			...
			$(
				".chat-debug-cache-perf-section-h",
				undefined,
				localize("chatDebug.cache.diffSummary", "Diff summary"),
			),
		);
		...
		if (optionsDiff.length > 0) {
			...
			optsLine.textContent = localize(
				...
				optionsDiff
					.map(
						(d) =>
							`${d.key} (${formatOptionValue(d.previous)} \u2192 ${formatOptionValue(d.current)})`,
					)
					.join(", "),
			);
		}
	...
	private renderSideCard(data: ISideData, title?: string): HTMLElement {
		...
		this.appendKv(
			card,
			localize("chatDebug.cache.model", "model"),
			data.event.model ?? "\u2014",
		);
		this.appendKv(
			card,
			localize("chatDebug.cache.inputTok", "input tok"),
			formatTokens(data.event.inputTokens),
		);
		this.appendKv(
			card,
			localize("chatDebug.cache.cachedTok", "cached tok"),
			formatTokens(data.event.cachedTokens),
		);
		this.appendKv(
			card,
			localize("chatDebug.cache.cacheHit", "cache hit"),
			`${formatCachePct(computeCacheHit(data.event))}%`,
		);
		this.appendKv(
			card,
			localize("chatDebug.cache.requestShape", "shape"),
			data.requestShape.label,
		);

		const startTime = data.event.created;
		const endTime =
			data.event.durationInMillis !== undefined
				? new Date(startTime.getTime() + data.event.durationInMillis)
				: undefined;
		this.appendKv(
			card,
			localize("chatDebug.cache.startTime", "startTime"),
		...
		if (endTime) {
			this.appendKv(
				card,
				localize("chatDebug.cache.endTime", "endTime"),
				endTime.toISOString(),
				true,
			);
		}
		if (data.event.durationInMillis !== undefined) {
			this.appendKv(
				card,
				localize("chatDebug.cache.duration", "duration"),
				`${numberFormatter.value.format(Math.round(data.event.durationInMillis))}ms`,
			);
		}
		...
		if (ttft !== undefined) {
			this.appendKv(
				card,
				localize("chatDebug.cache.ttft", "timeToFirstToken"),
				`${numberFormatter.value.format(Math.round(ttft))}ms`,
			);
		}
		const requestId =
			data.content?.requestId ?? data.event.parentEventId ?? data.event.id;
		if (requestId) {
			this.appendKv(
				card,
				localize("chatDebug.cache.requestId", "requestId"),
				requestId,
				true,
			);
		}
	...
	private renderSingleSummary(b: ISideData): void {
		...
		DOM.append(
			...
			$(
				".chat-debug-cache-card-h",
				undefined,
				localize("chatDebug.cache.firstRequest", "First request in session"),
			),
		);
	...
	private renderSignature(
		...
		for (const role of [
			...
			const entry = DOM.append(
				legend,
				$("span.chat-debug-cache-sig-legend-entry"),
			);
			DOM.append(
				entry,
				$(`span.chat-debug-cache-sig-swatch.role-${roleClass(role)}`),
			);
			DOM.append(
				entry,
				DOM.$(
		...
		DOM.append(
			...
			DOM.$(
				"span",
				undefined,
				localize("chatDebug.cache.driftLegend", "drift"),
			),
		);
		...
		const toSegments = (side: ISideData, isA: boolean): ISegment[] => {
			...
			side.inputMessages.forEach((m, i) => {
				...
				const drift =
					...
					(kind === CacheDiffKind.ContentDrift ||
		...
		const buildLane = (
			label: string,
			segs: readonly ISegment[],
			breakPos: number | undefined,
		): HTMLElement => {
			...
			for (const s of segs) {
				...
				const seg = DOM.append(
					bar,
					$(`span.chat-debug-cache-sig-seg.role-${roleClass(s.role)}`),
				);
				if (s.drift) {
				...
				seg.title =
					`${s.label}: ${numberFormatter.value.format(s.chars)} chars` +
					(s.drift ? ` \u2014 drift` : "");
				if (s.chars > max * 0.05) {
			...
			if (sideTotal < max) {
				const pad = DOM.append(
					bar,
					$("span.chat-debug-cache-sig-seg.role-empty"),
				);
			...
			DOM.append(
				...
				$(
					...
					localize(
						...
						"{0} chars",
						numberFormatter.value.format(sideTotal),
					),
				),
			);
			return row;
		...
		lanes.appendChild(
			buildLane(
				localize("chatDebug.cache.lanePrevious", "Previous"),
				aSegs,
				breakCharPos(aSegs),
			),
		);
		lanes.appendChild(
			buildLane(
				localize("chatDebug.cache.laneCurrent", "Current"),
				bSegs,
				breakCharPos(bSegs),
			),
		);
	...
	private renderRequestOptions(a: ISideData, b: ISideData): void {
		...
		DOM.append(
			section,
			$(
				"h3.chat-debug-cache-section-h",
				undefined,
				localize("chatDebug.cache.requestOptionsHeading", "Request Options"),
			),
		);
		...
		DOM.append(
			...
			$(
				...
				undefined,
				localize("chatDebug.cache.optionsKey", "Option"),
			),
		);
		DOM.append(
			head,
			$(
				...
				undefined,
				localize("chatDebug.cache.optionsPrev", "Previous"),
			),
		);
		DOM.append(
			head,
			$(
				".chat-debug-cache-options-cell",
				undefined,
				localize("chatDebug.cache.optionsCurr", "Current"),
			),
		);
		...
		for (const key of sortedKeys) {
			...
			DOM.append(
				row,
				$(".chat-debug-cache-options-cell", undefined, formatOptionValue(av)),
			);
			DOM.append(
				row,
				$(".chat-debug-cache-options-cell", undefined, formatOptionValue(bv)),
			);
		}
	}

	private renderComponents(
		drift: readonly IComponentDrift[],
		a: ISideData,
		b: ISideData,
		compareInputMessages: boolean,
	): void {
		...
		DOM.append(
			...
			$(
				"h3.chat-debug-cache-section-h",
				undefined,
				localize("chatDebug.cache.componentsHeading", "Components"),
			),
		);
		...
		const effectiveDrift =
			!compareInputMessages &&
			b.requestShape.isContinuation &&
			b.inputMessages.length > 0
		...
		for (const c of effectiveDrift) {
			...
			if (c.role) {
				DOM.append(name, $("span.role", undefined, c.role));
			}
			DOM.append(name, DOM.$("span", undefined, c.name));
			const badge = DOM.append(
				head,
				$(`span.chat-debug-cache-acc-badge.${c.status}`),
			);
			...
			sizes.textContent = localize(
				...
				"{0} → {1} chars",
				formatTokens(c.aSize),
				formatTokens(c.bSize),
			);

			const body = DOM.append(item, $(".chat-debug-cache-acc-body"));
			const aText =
				c.name === CURRENT_CONTINUATION_DELTA_COMPONENT
					? ""
					: textForComponent(c, a);
			const bText =
				c.name === CURRENT_CONTINUATION_DELTA_COMPONENT
					? continuationDeltaText(b)
					: textForComponent(c, b);
			...
			body.appendChild(
				this.renderComponentDiff(aText, bText, c.aSize, c.bSize),
			);

			this.loadDisposables.add(
				DOM.addDisposableListener(head, DOM.EventType.CLICK, () => {
					if (this.openComponents.has(c.name)) {
		...
		DOM.append(
			...
			$(
				...
				localize(
					...
					"Previous \u00b7 {0} chars",
					numberFormatter.value.format(aSize),
				),
			),
		);
		...
		DOM.append(
			...
			$(
				...
				localize(
					...
					"Current \u00b7 {0} chars",
					numberFormatter.value.format(bSize),
				),
			),
		);
...
function findSection(
	sections: readonly IChatDebugMessageSection[] | undefined,
	name: string,
): string | undefined {
	if (!sections) {
		return undefined;
...
function buildTurnGroups(
	turns: readonly IChatDebugModelTurnEvent[],
	userMessages: readonly IChatDebugUserMessageEvent[],
): readonly ITurnGroup[] {
	...
	for (const um of userMessages) {
		...
		const stripped = um.id.startsWith("user-msg-")
			? um.id.slice("user-msg-".length)
			: um.id;
	...
	const groups = new Map<
		string,
		{
			userMessage: IChatDebugUserMessageEvent | undefined;
			turns: { turn: IChatDebugModelTurnEvent; index: number }[];
		}
	>();
	const order: string[] = [];
...
function continuationDeltaText(side: ISideData): string {
	return side.requestShape.isContinuation
		? side.inputMessages
				.map((m, index) => `input[${index}] ${m.role}\n${m.text}`)
				.join("\n\n")
		: "";
}
...
function currentDeltaComponent(side: ISideData): IComponentDrift {
	...
	return {
		...
		role:
			side.requestShape.inputItemTypes.join(", ") ||
			side.inputMessages.map((m) => m.role).join(", ") ||
			undefined,
 ...
 */
function describeBreakKind(
	kind: Exclude<CacheDiffKind, CacheDiffKind.Identical>,
	diff: ICacheDiffResult,
	b: ISideData,
): string {
	...
	const charsB = bMsg
		? numberFormatter.value.format(bMsg.charLength)
		: undefined;
	switch (kind) {
		case CacheDiffKind.OnlyInB:
			return charsB
...
function describeRequestShape(
	inputMessages: readonly INormalizedMessage[],
	...
	const hasToolSearchOutput =
		inputItemTypes.includes("tool_search_output") ||
		inputMessages.some((m) => m.role === "tool_search");
	const hasOnlyToolOutput =
		inputMessages.length > 0 && inputMessages.every((m) => m.role === "tool");
...
function parseRequestShapeMetadata(
	requestShapeJson: string | undefined,
): IRequestShapeMetadata | undefined {
	if (!requestShapeJson) {
		return undefined;
	}
	try {
 ...
 * rendering.
 */
function computeOptionsDiff(
	a: ISideData,
	b: ISideData,
): readonly IOptionDelta[] {
	const prev = sideOptions(a);
	const curr = sideOptions(b);
	const keys = new Set<string>([...Object.keys(prev), ...Object.keys(curr)]);
 ...
 */
function isLikelyCacheExpiration(
	hitPct: number,
	diff: ICacheDiffResult,
	optionsDiff: readonly IOptionDelta[],
...
function renderInlineDiff(
	...
	const result = linesDiffComputers
		.getDefault()
		.computeDiff(prevLines, currLines, DIFF_OPTIONS);
...
function appendChangedLine(
	host: HTMLElement,
	text: string,
	ranges: readonly IInnerChangeRange[] | undefined,
	kind: "add" | "remove",
): void {
	const line = DOM.append(host, $(`.chat-debug-cache-diff-line.${kind}`));
	...
	for (const r of sorted) {
		if (r.startColumn > cursor) {
			DOM.append(
				line,
				document.createTextNode(text.substring(cursor - 1, r.startColumn - 1)),
			);
		}
