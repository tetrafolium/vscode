...
const aicFormatter = safeIntl.NumberFormat(undefined, {
...
export class ChatDebugOverviewView extends Disposable {
	private readonly _onNavigate = this._register(
		new Emitter<OverviewNavigation>(),
	);
	...
	constructor(
		...
		this.refreshScheduler = this._register(
			new RunOnceScheduler(() => this.doRefresh(), 100),
		);
	...
	private doRefresh(): void {
		...
		if (this.metricsContainer && this.currentSessionResource) {
			...
			const events = this.chatDebugService.getEvents(
	...
	private load(): void {
		...
		const titleActions = DOM.append(
			titleRow,
			$(".chat-debug-overview-title-actions"),
		);

		const revealSessionBtn = this.loadDisposables.add(
			new Button(titleActions, {
				ariaLabel: localize(
					"chatDebug.revealChatSession",
					"Reveal Chat Session",
				),
				title: localize("chatDebug.revealChatSession", "Reveal Chat Session"),
			}),
		);
	...
	private renderSessionDetails(sessionUri: URI): void {
		...
		const sessionTypeName =
			...
			(sessionType === localChatSessionType
				...
				: sessionType);
		details.push({
			label: localize("chatDebug.detail.sessionType", "Session Type"),
		...
		if (model) {
			...
			details.push({
				label: localize("chatDebug.detail.location", "Location"),
				value: locationLabel,
			});
			...
			details.push({
				label: localize("chatDebug.detail.status", "Status"),
				value: statusLabel,
			});

			const timing = model.timing;
			details.push({
				label: localize("chatDebug.detail.created", "Created"),
				value: new Date(timing.created).toLocaleString(),
			});

			if (timing.lastRequestEnded) {
				details.push({
					label: localize("chatDebug.detail.lastActivity", "Last Activity"),
		...
		if (details.length > 0) {
			...
			DOM.append(
				section,
				$(
					"h3.chat-debug-overview-section-label",
					undefined,
					localize("chatDebug.sessionDetails", "Session Details"),
				),
			);

			const detailsGrid = DOM.append(
				section,
			...
			for (const detail of details) {
				...
				DOM.append(
					row,
					$("span.chat-debug-overview-detail-label", undefined, detail.label),
				);
				DOM.append(
					row,
					$("span.chat-debug-overview-detail-value", undefined, detail.value),
				);
			}
		}
	}
	...
	private renderDerivedOverview(
		...
		const metricsSection = DOM.append(
			this.content,
			$(".chat-debug-overview-section"),
		);
		DOM.append(
			...
			$(
				"h3.chat-debug-overview-section-label",
				undefined,
				localize("chatDebug.summary", "Summary"),
			),
		);

		this.metricsContainer = DOM.append(
			metricsSection,
		...
		// Explore actions
		const actionsSection = DOM.append(
			this.content,
			$(".chat-debug-overview-section"),
		);
		DOM.append(
			...
			$(
				"h3.chat-debug-overview-section-label",
				undefined,
				localize("chatDebug.exploreTraceData", "Explore Trace Data"),
			),
		);

		const row = DOM.append(actionsSection, $(".chat-debug-overview-actions"));

		const viewLogsBtn = this.loadDisposables.add(
			new Button(row, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
				title: localize("chatDebug.viewLogs", "View Logs"),
			}),
		);
		...
		this.loadDisposables.add(
			viewLogsBtn.onDidClick(() => {
				this._onNavigate.fire(OverviewNavigation.Logs);
			}),
		);

		const flowChartBtn = this.loadDisposables.add(
			new Button(row, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
				title: localize("chatDebug.agentFlowChart", "Agent Flow Chart"),
			}),
		);
		...
		this.loadDisposables.add(
			flowChartBtn.onDidClick(() => {
				this._onNavigate.fire(OverviewNavigation.FlowChart);
			}),
		);

		const cacheBtn = this.loadDisposables.add(
			new Button(row, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
				title: localize("chatDebug.cacheExplorer", "Cache Explorer"),
			}),
		);
	...
	private renderMetricsShimmer(container: HTMLElement): void {
		...
		for (const label of placeholderLabels) {
			...
			DOM.append(
				card,
				$("div.chat-debug-overview-metric-label", undefined, label),
			);
			const valueEl = DOM.append(
				card,
				$("div.chat-debug-overview-metric-value"),
			);
			const shimmer = DOM.append(
				valueEl,
				$("span.chat-debug-overview-metric-shimmer"),
			);
	...
	private renderMetricsContent(
		...
		const totalInputTokens = modelTurns.reduce(
			(sum, e) => sum + (e.inputTokens ?? 0),
			0,
		);
		const totalOutputTokens = modelTurns.reduce(
			(sum, e) => sum + (e.outputTokens ?? 0),
			0,
		);
		const totalCachedTokens = modelTurns.reduce(
			(sum, e) => sum + (e.cachedTokens ?? 0),
			0,
		);
		const totalTokens = modelTurns.reduce(
			(sum, e) => sum + (e.totalTokens ?? 0),
			0,
		);
		const totalCopilotUsageNanoAiu = modelTurns.reduce(
			(sum, e) => sum + (e.copilotUsageNanoAiu ?? 0),
			0,
		);
		...
		const metrics: OverviewMetric[] = [
			{
				label: localize("chatDebug.metric.modelTurns", "Model Turns"),
				value: fmt.format(modelTurns.length),
			},
			{
				label: localize("chatDebug.metric.toolCalls", "Tool Calls"),
			...
			{
				label: localize("chatDebug.metric.totalTokens", "Total Tokens"),
				value: fmt.format(totalTokens),
			},
			{
				label: localize("chatDebug.metric.errors", "Errors"),
		...
		if (totalCopilotUsageNanoAiu > 0) {
			...
			metrics.push({
				label: localize("chatDebug.metric.copilotUsage", "Copilot Usage (AIC)"),
		...
		for (const metric of metrics) {
			...
			DOM.append(
				card,
				$("div.chat-debug-overview-metric-label", undefined, metric.label),
			);
			DOM.append(
				card,
				$("div.chat-debug-overview-metric-value", undefined, metric.value),
			);
		}
	}
}
