...
type ChatDebugViewSwitchedClassification = {
	viewState: {
		...
		comment: "The view the user navigated to (home, overview, logs, flowchart, cache).";
	};
	owner: "vijayu";
...
export class ChatDebugEditor extends EditorPane {
	...
	protected override createEditor(parent: HTMLElement): void {
		...
		this._register(
			this.overviewView.onNavigate((nav) => {
				switch (nav) {
		...
		this._register(
			this.flowChartView.onNavigate((nav) => {
				switch (nav) {
		...
		this._register(
			this.cacheExplorerView.onNavigate((nav) => {
				switch (nav) {
		...
		this._register(
			this.chatDebugService.onDidAddEvent((event) => {
				if (this.viewState === ViewState.Home) {
					this.homeView?.render();
				} else if (
					this.chatDebugService.activeSessionResource &&
					event.sessionResource.toString() ===
						this.chatDebugService.activeSessionResource.toString()
				) {
		...
		this._register(
			this.chatService.onDidCreateModel((model) => {
					...
					model.onDidChange((e) => {
						if (e.kind === "setCustomTitle") {
							if (this.viewState === ViewState.Home) {
								...
								this.viewState === ViewState.Overview ||
								this.viewState === ViewState.Logs ||
								this.viewState === ViewState.FlowChart ||
								this.viewState === ViewState.CacheExplorer
							) {
		...
		this._register(
			this.chatService.onDidDisposeSession(() => {
				if (this.viewState === ViewState.Home) {
					this.homeView?.render();
				}
			}),
	...
	private showView(state: ViewState): void {
		...
		this.telemetryService.publicLog2<
			ChatDebugViewSwitchedEvent,
			ChatDebugViewSwitchedClassification
	...
	navigateToSession(
		sessionResource: URI,
		view?: "logs" | "overview" | "flowchart" | "cache",
	): void {
		...
		if (
			...
			previousSessionResource.toString() !== sessionResource.toString()
		) {
