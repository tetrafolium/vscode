...
export function formatEventDetail(event: IChatDebugEvent): string {
	switch (event.kind) {
		case "toolCall": {
			const parts = [
				localize("chatDebug.detail.tool", "Tool: {0}", event.toolName),
			];
			if (event.toolCallId) {
				parts.push(
					localize("chatDebug.detail.callId", "Call ID: {0}", event.toolCallId),
				);
			}
			if (event.result) {
				parts.push(
					localize("chatDebug.detail.result", "Result: {0}", event.result),
				);
			}
		...
		case "modelTurn": {
			const parts = [
				event.model ?? localize("chatDebug.detail.modelTurn", "Model Turn"),
			];
			if (event.inputTokens !== undefined) {
				parts.push(
					localize(
						...
						"Input tokens: {0}",
						numberFormatter.value.format(event.inputTokens),
					),
				);
			}
			...
			if (event.totalTokens !== undefined) {
				parts.push(
					localize(
						...
						"Total tokens: {0}",
						numberFormatter.value.format(event.totalTokens),
					),
				);
			}
			if (event.durationInMillis !== undefined) {
		...
		case "subagentInvocation": {
			const parts = [
				localize("chatDebug.detail.agent", "Agent: {0}", event.agentName),
			];
			...
			if (event.status) {
				parts.push(
					localize("chatDebug.detail.status", "Status: {0}", event.status),
				);
			}
			if (event.durationInMillis !== undefined) {
				parts.push(
			...
			if (event.toolCallCount !== undefined) {
				parts.push(
					localize(
						...
						"Tool calls: {0}",
						numberFormatter.value.format(event.toolCallCount),
					),
				);
			}
