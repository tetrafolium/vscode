...
export class ChatDebugHomeView extends Disposable {
	...
	constructor(
		...
		this.scrollContent = DOM.append(
			this.container,
			$("div.chat-debug-home-content"),
		);
		...
		this._register(
			this.chatDebugService.onDidChangeAvailableSessionResources(() => {
				if (this.container.style.display !== "none") {
					this.render();
				}
			}),
	...
	render(): void {
		const isFileLoggingEnabled = this.configurationService.getValue<boolean>(
			AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING,
		);
		this._lastKnownSessionCount =
			this.chatDebugService.getSessionResources().length;

		const sessionResources = isFileLoggingEnabled
			? this._getFilteredSessionResources(
	...
	private _getFilteredSessionResources(resources: readonly URI[]): URI[] {
		...
		return [...resources].filter(
			(r) =>
				!cliSessionTypes.has(getChatSessionType(r)) ||
				!isUntitledChatSession(r),
		);
	}
	...
	private _renderWithSessions(sessionResources: URI[]): void {
		...
		DOM.append(
			this.scrollContent,
			$(
				"h2.chat-debug-home-title",
				undefined,
				localize("chatDebug.title", "Agent Debug Logs"),
			),
		);

		const isEnabled = this.configurationService.getValue<boolean>(
			AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING,
		);
		if (!isEnabled) {
			DOM.append(
			...
			const enableButton = this.renderDisposables.add(
				new Button(this.scrollContent, {
					...defaultButtonStyles,
					secondary: true,
				}),
			);
			...
			this.renderDisposables.add(
				enableButton.onDidClick(() => {
					this.preferencesService.openSettings({
						jsonEditor: false,
						query: `@id:${AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING}`,
					});
				}),
			);
			return;
		...
		const bubbleToTop = (resource: URI | undefined) => {
			...
			const idx = sessionResources.findIndex(
				(r) => r.toString() === resource.toString(),
			);
			if (idx > 0) {
		...
		if (sessionResources.length > 0) {
			...
			sessionList.setAttribute(
				"aria-label",
				localize("chatDebug.sessionList", "Chat sessions"),
			);
			...
			for (const sessionResource of visibleSessions) {
				...
				const agentSession =
					this.agentSessionsService.model.getSession(sessionResource);
				const rawTitle =
					agentSession?.label ??
					this.chatService.getSessionTitle(sessionResource);
				const importedTitle =
					this.chatDebugService.getImportedSessionTitle(sessionResource);
				const historicalTitle =
					this.chatDebugService.getHistoricalSessionTitle(sessionResource);
				...
				if (rawTitle && !isUUID(rawTitle)) {
					...
					const shortId =
						pathId || sessionResource.authority || sessionResource.toString();
					sessionTitle = localize(
				...
				const isActive =
					...
					sessionResource.toString() === activeSessionResource.toString();

				const item = DOM.append(
					sessionList,
					$<HTMLButtonElement>("button.chat-debug-home-session-item"),
				);
				...
				const titleSpan = DOM.append(
					item,
					$("span.chat-debug-home-session-item-title"),
				);
				...
				if (isActive) {
					DOM.append(
						item,
						$(
							"span.chat-debug-home-session-badge",
							undefined,
							localize("chatDebug.active", "Active"),
						),
					);
				}
			...
			if (sessionResources.length > this._visibleCount) {
				...
				const showMoreButton = this.renderDisposables.add(
					new Button(this.scrollContent, {
						...defaultButtonStyles,
						secondary: true,
					}),
