...
import {
	...
	IChatDebugEventCustomizationSummaryContent,
 ...
 */
function getSettingsKeyForDiscoveryType(
 ...
 */
function getFileLocationLabel(
	file: { uri: URI; storage?: string; extensionId?: string },
	...
	const parentDir =
		discoveryType === "skill" ? dirname(dirname(file.uri)) : dirname(file.uri);
...
function createInlineFileLink(
	...
	const hoverText = hoverSuffix
		? `${relativeLabel} ${hoverSuffix}`
		: relativeLabel;
	disposables.add(
	...
	disposables.add(
		DOM.addDisposableListener(link, DOM.EventType.CLICK, (e) => {
			e.preventDefault();
			e.stopPropagation();
 ...
 */
function setupFileListNavigation(
	listEl: HTMLElement,
	rows: { element: HTMLElement; activate: () => void }[],
	disposables: DisposableStore,
): void {
	if (rows.length === 0) {
		return;
 ...
 */
export function renderCustomizationDiscoveryContent(
	content: IChatDebugEventFileListContent,
	openerService: IOpenerService,
	modelService: IModelService,
	languageService: ILanguageService,
	hoverService: IHoverService,
	...
	const capitalizedType =
		content.discoveryType.charAt(0).toUpperCase() +
		content.discoveryType.slice(1);
	DOM.append(
		container,
		$(
	...
	if (loaded.length > 0) {
		const section = DOM.append(
			container,
			$("div.chat-debug-file-list-section"),
		);
		DOM.append(
			section,
			$(
		...
		for (const file of loaded) {
			const key = getFileLocationLabel(
				file,
				labelService,
				content.discoveryType,
			);
	...
	if (skipped.length > 0) {
		...
		listEl.setAttribute(
			"aria-label",
			localize("chatDebug.skippedFilesList", "Skipped files"),
		);
		...
		for (const [reasonLabel, files] of groups) {
			...
			for (const file of files) {
				...
				if (detail) {
					DOM.append(
						row,
						$("span.chat-debug-file-list-detail", undefined, ` (${detail})`),
					);
				}
				const relativeLabel = labelService.getUriLabel(file.uri, {
					relative: true,
	...
	if (content.sourceFolders && content.sourceFolders.length > 0) {
		const sectionEl = DOM.append(
			container,
			$("div.chat-debug-message-section"),
		);
		...
		const contentEl = DOM.append(
			sectionEl,
			$("div.chat-debug-source-folder-content"),
		);
		contentEl.tabIndex = 0;
		contentEl.setAttribute("role", "region");
		contentEl.setAttribute(
			"aria-label",
			localize("chatDebug.sourceFoldersContent", "Source folders"),
		);

		const capitalizedType =
			content.discoveryType.charAt(0).toUpperCase() +
			content.discoveryType.slice(1);
		const sourcesCaption = capitalizedType.endsWith("s")
			? capitalizedType
			: capitalizedType + "s";
		DOM.append(
			contentEl,
		...
		for (let i = 0; i < content.sourceFolders.length; i++) {
			...
			DOM.append(
				row,
				$("span.chat-debug-source-folder-index", undefined, `${i + 1}.`),
			);
			DOM.append(
				row,
				$("span.chat-debug-source-folder-label", undefined, folder.uri.path),
			);
		}
 ...
 */
export function fileListToPlainText(
	content: IChatDebugEventFileListContent,
): string {
	const lines: string[] = [];
	const capitalizedType =
	...
	if (loaded.length > 0) {
		lines.push(
			localize("chatDebug.plainText.loaded", "Loaded ({0})", loaded.length),
		);
		// Group by location
		const groups = new Map<string, typeof loaded>();
		for (const f of loaded) {
			const parentDir =
				content.discoveryType === "skill"
					? dirname(dirname(f.uri))
					: dirname(f.uri);
	...
	if (skipped.length > 0) {
		lines.push(
			localize("chatDebug.plainText.skipped", "Skipped ({0})", skipped.length),
		);
		// Group by skip reason
		const skippedGroups = new Map<string, typeof skipped>();
		for (const f of skipped) {
			const key =
				f.skipReason ?? localize("chatDebug.plainText.unknown", "unknown");
 ...
 */
function getCategorySectionTitle(
	category: IChatDebugCustomizationLogEntry["category"],
	count: number,
): string {
	switch (category) {
		case "applying":
			return localize(
				"chatDebug.customization.instructions",
				"Instructions ({0})",
				count,
			);
		case "referenced":
			return localize(
				"chatDebug.customization.referenced",
				"Referenced ({0})",
				count,
			);
		case "skill":
		...
		case "custom-agent":
			return localize(
				"chatDebug.customization.customAgent",
				"Agents ({0})",
				count,
			);
		case "hook":
		...
		case "skipped":
			return localize(
				"chatDebug.customization.skipped",
				"Skipped ({0})",
				count,
			);
	}
...
export function renderCustomizationSummaryContent(
	content: IChatDebugEventCustomizationSummaryContent,
	openerService: IOpenerService,
	modelService: IModelService,
	languageService: ILanguageService,
	hoverService: IHoverService,
	...
	DOM.append(
		...
		$(
			...
			undefined,
			localize(
				"chatDebug.customizationTitle",
				"Customization Resolution Results",
			),
		),
	);
	DOM.append(
	...
	const instructionEntries = content.resolutionLogs.filter(
		(e) => e.category === "applying" || e.category === "referenced",
	);
	const skillEntries = content.resolutionLogs.filter(
		(e) => e.category === "skill",
	);
	const agentEntries = content.resolutionLogs.filter(
		(e) => e.category === "custom-agent",
	);
	const hookEntries = content.resolutionLogs.filter(
		(e) => e.category === "hook",
	);
	const skippedEntries = content.resolutionLogs.filter(
		(e) => e.category === "skipped",
	);

	const sections: {
		title: string;
		icon: ThemeIcon;
		entries: readonly IChatDebugCustomizationLogEntry[];
	}[] = [
		{
			title: getCategorySectionTitle("applying", instructionEntries.length),
			icon: Codicon.book,
			entries: instructionEntries,
		},
		{
			title: getCategorySectionTitle("skill", skillEntries.length),
			icon: Codicon.lightbulb,
			entries: skillEntries,
		},
		{
			title: getCategorySectionTitle("custom-agent", agentEntries.length),
			icon: Codicon.agent,
			entries: agentEntries,
		},
		{
			title: getCategorySectionTitle("hook", hookEntries.length),
			icon: Codicon.zap,
			entries: hookEntries,
		},
		{
			title: getCategorySectionTitle("skipped", skippedEntries.length),
			icon: Codicon.close,
	...
	for (const { title, icon, entries } of sections) {
		...
		DOM.append(
			section,
			$("div.chat-debug-file-list-section-title", undefined, title),
		);
		...
		if (isHookSection) {
			...
			for (const [hookType, groupEntries] of groupedByType) {
				if (hookType) {
					DOM.append(
						listEl,
						$("div.chat-debug-file-list-group-header", undefined, hookType),
					);
				}
				for (const entry of groupEntries) {
					...
					DOM.append(
						row,
						$(`span.chat-debug-file-list-icon${ThemeIcon.asCSSSelector(icon)}`),
					);
			...
			for (const entry of entries) {
				...
				const showReason =
					entry.category !== "skill" && entry.category !== "custom-agent";
				...
				if (showReason && entry.reason) {
					DOM.append(
						row,
						$(
							"span.chat-debug-file-list-detail",
							undefined,
							` — ${entry.reason}`,
						),
					);
 ...
 */
export function customizationSummaryToPlainText(
	content: IChatDebugEventCustomizationSummaryContent,
): string {
	const lines: string[] = [];
	...
	lines.push(
		localize(
			"chatDebug.plainText.customizationTitle",
			"Customization Resolution Results",
		),
	);
	lines.push(
