...
} from "./chatDebugModelTurnContentRenderer.js";
import {
	renderHookContent,
	hookContentToPlainText,
} from "./chatDebugHookContentRenderer.js";
...
export class ChatDebugDetailPanel extends Disposable {
	...
	async show(event: IChatDebugEvent): Promise<void> {
		...
		const resolved = event.id
			? await this.chatDebugService.resolveEvent(event.id)
			: undefined;
		...
		if (resolved && resolved.kind === "fileList") {
			...
			const { element: contentEl, disposables: contentDisposables } =
			...
			const { element: contentEl, disposables: contentDisposables } =
				await renderResolvedMessageContent(
					resolved,
					this.languageService,
					this.clipboardService,
					this.scrollable,
			...
			const { element: contentEl, disposables: contentDisposables } =
				await renderUserMessageContent(
					event,
					this.languageService,
					this.clipboardService,
					this.scrollable,
			...
			this.currentDetailText = messageEventToPlainText(event);
			const { element: contentEl, disposables: contentDisposables } =
				await renderAgentResponseContent(
					event,
					this.languageService,
					this.clipboardService,
					this.scrollable,
