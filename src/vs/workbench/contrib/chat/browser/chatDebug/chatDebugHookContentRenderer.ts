 ...
 */
export async function renderHookContent(
	content: IChatDebugEventHookContent,
	...
	DOM.append(
		container,
		$("div.chat-debug-message-content-title", undefined, content.hookType),
	);
	...
	if (content.exitCode !== undefined) {
		statusParts.push(
			localize("chatDebug.hook.exitCode", "Exit Code: {0}", content.exitCode),
		);
	}
	if (content.durationInMillis !== undefined) {
		statusParts.push(
			localize("chatDebug.hook.duration", "{0}ms", content.durationInMillis),
		);
	}
	...
	if (content.command) {
		...
		renderSection(
			sectionsContainer,
			localize("chatDebug.hook.command", "Command"),
			plainText,
			tokenizedHtml,
			disposables,
			false,
	...
	if (content.input) {
		...
		renderSection(
			sectionsContainer,
			localize("chatDebug.hook.input", "Input"),
			plainText,
			tokenizedHtml,
			disposables,
			false,
	...
	if (content.output) {
		...
		renderSection(
			sectionsContainer,
			localize("chatDebug.hook.output", "Output"),
			plainText,
			tokenizedHtml,
			disposables,
			false,
	...
	if (content.errorMessage) {
		...
		renderSection(
			sectionsContainer,
			localize("chatDebug.hook.error", "Error"),
			plainText,
			tokenizedHtml,
			disposables,
			false,
...
function formatHookResult(result: ChatDebugHookResult): string {
	switch (result) {
		...
		case ChatDebugHookResult.NonBlockingError:
			return localize(
				"chatDebug.hook.result.nonBlockingError",
				"Non-blocking Error",
			);
		default:
 ...
 */
export function hookContentToPlainText(
	content: IChatDebugEventHookContent,
): string {
	const lines: string[] = [];
	lines.push(
		localize("chatDebug.hook.typeLabel", "Hook Type: {0}", content.hookType),
	);

	if (content.result !== undefined) {
		lines.push(
			localize(
				"chatDebug.hook.resultLabel",
				"Result: {0}",
				formatHookResult(content.result),
			),
		);
	}
	if (content.exitCode !== undefined) {
