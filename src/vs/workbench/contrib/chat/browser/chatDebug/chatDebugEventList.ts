...
export function getEventNameText(element: IChatDebugEvent): string {
	switch (element.kind) {
		case "toolCall":
			return safeStr(
				element.toolName,
				localize("chatDebug.unknownEvent", "(unknown)"),
			);
		case "modelTurn":
			return (
				safeStr(element.model) || localize("chatDebug.modelTurn", "Model Turn")
			);
		case "generic":
			return safeStr(
				element.name,
	...
	switch (element.kind) {
		...
		case "modelTurn":
			return [
				...
				element.totalTokens !== undefined
					? localize(
							"chatDebug.tokens",
							"{0} tokens",
							numberFormatter.value.format(element.totalTokens),
						)
					: "",
			]
				.filter(Boolean)
...
function renderEventToTemplate(
	element: IChatDebugEvent,
	templateData: IChatDebugEventTemplate,
): void {
	...
	const isError =
		(element.kind === "generic" && element.level === ChatDebugLogLevel.Error) ||
		(element.kind === "toolCall" && element.result === "error");
	const isWarning =
		element.kind === "generic" && element.level === ChatDebugLogLevel.Warning;
	const isTrace =
		element.kind === "generic" && element.level === ChatDebugLogLevel.Trace;
...
export class ChatDebugEventRenderer implements IListRenderer<
	IChatDebugEvent,
	IChatDebugEventTemplate
> {
...
export class ChatDebugEventTreeRenderer implements ITreeRenderer<
	IChatDebugEvent,
	void,
	IChatDebugEventTemplate
> {
	...
	renderElement(
		node: ITreeNode<IChatDebugEvent, void>,
		index: number,
		templateData: IChatDebugEventTemplate,
	): void {
