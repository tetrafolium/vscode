...
type ChatModelCountClassification = {
	totalModels: {
		...
		isMeasurement: true;
		comment: "Total number of live chat models.";
	};
	modelsOpenInWidgets: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of chat models that are open in a chat widget or editor.";
	};
	backgroundModels: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of chat models with no open widget.";
	};
	backgroundModels_modifiedEditsKeepAlive: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of background models held alive by the ChatModel#modifiedEditsKeepAlive reference (has pending edits).";
	};
	backgroundModels_requestInProgressKeepAlive: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of background models held alive by the ChatModel#requestInProgressKeepAlive reference (request is running).";
	};
	backgroundModels_otherHolders: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of background models with unrecognized holders (potential leaks).";
	};
};
...
type ChatModelCreatedClassification = ChatModelCountClassification & {
	newModelLocation: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The ChatAgentLocation of the newly created chat model.";
	};
	owner: "roblourens";
 ...
 */
export class ChatModelCountTelemetry
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			this.chatService.onDidCreateModel((model) =>
				this.onDidCreateModel(model.initialLocation),
			),
		);
	}
	...
	private logStartupTelemetry(): void {
		this.telemetryService.publicLog2<
			ChatModelCountEvent,
			ChatModelsAtStartupClassification
	...
	private onDidCreateModel(newModelLocation: ChatAgentLocation): void {
		...
		this.telemetryService.publicLog2<
			ChatModelCreatedEvent,
			ChatModelCreatedClassification
	...
	private getSnapshot(): ChatModelCountEvent {
		...
		for (const model of snapshot.models) {
			if (
				this.chatWidgetService.getWidgetBySessionResource(model.sessionResource)
			) {
				modelsOpenInWidgets++;
			} else {
				backgroundModels++;
