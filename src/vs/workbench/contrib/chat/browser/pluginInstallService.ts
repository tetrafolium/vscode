...
export class PluginInstallService implements IPluginInstallService {
	...
	async installPluginFromValidatedSource(
		source: string,
		options?: IInstallPluginFromSourceOptions,
	...
	private async _doInstallFromSource(
		...
		const sourceDescriptor =
			reference.kind === MarketplaceReferenceKind.GitHubShorthand
				? {
						kind: PluginSourceKind.GitHub as const,
		...
		let repoDir: URI;
		try {
			repoDir = await this._pluginRepositoryService.ensurePluginSource(
				tempPlugin,
				{
		...
		const discoveredPlugins =
			await this._pluginMarketplaceService.readPluginsFromDirectory(
				repoDir,
				reference,
			);
		...
		if (discoveredPlugins.length === 0) {
			...
			const singlePlugin =
				await this._pluginMarketplaceService.readSinglePluginManifest(
					repoDir,
					reference,
				);
			if (singlePlugin) {
		...
		const picks: (IQuickPickItem & { plugin: IMarketplacePlugin })[] =
	...
	async updateAllPlugins(
		...
		const doUpdate = async () => {
			...
			for (const entry of installed) {
				...
				gitTasks.push(
					(async () => {
						...
						try {
							const changed =
								await this._pluginRepositoryService.pullRepository(ref, {
			...
			const marketplacePlugins =
				await this._pluginMarketplaceService.fetchMarketplacePlugins(token);
			...
			for (const entry of installed) {
				if (
					entry.plugin.sourceDescriptor.kind === PluginSourceKind.RelativePath
				) {
					continue;
				}
				...
				if (
					desc.kind === PluginSourceKind.Npm ||
					desc.kind === PluginSourceKind.Pip
				) {
				...
				independentGitTasks.push(
					(async () => {
						...
						try {
							const changed =
								await this._pluginRepositoryService.updatePluginSource(
									livePlugin,
									{
			...
			for (const { installed: _installed, marketplace } of packagePlugins) {
				...
				try {
					...
					if (changed) {
						...
						const pluginUri =
							this._pluginRepositoryService.getPluginSourceInstallUri(
			...
			this._notificationService.notify({
				...
				actions: {
					primary: [
						new Action(
							"showGitOutput",
							localize("showOutput", "Show Output"),
							undefined,
							true,
							() => {
	...
	private async _ensureMarketplaceTrusted(
		plugin: IMarketplacePlugin,
	): Promise<boolean> {
		if (
			this._pluginMarketplaceService.isMarketplaceTrusted(
		...
		const { confirmed } = await this._dialogService.confirm({
			...
			primaryButton: localize(
				{ key: "trustAndInstall", comment: ["&& denotes a mnemonic"] },
				"&&Trust",
			),
			custom: {
				icon: Codicon.shield,
	...
	private async _installRelativePathPlugin(
		plugin: IMarketplacePlugin,
	): Promise<void> {
		try {
			await this._pluginRepositoryService.ensureRepository(
				plugin.marketplaceReference,
				{
					progressTitle: localize(
						"installingPlugin",
	...
	private async _installGitPlugin(plugin: IMarketplacePlugin): Promise<void> {
		...
		let pluginDir: URI;
		try {
			pluginDir = await this._pluginRepositoryService.ensurePluginSource(
				plugin,
				{
					progressTitle: localize(
						"installingPlugin",
	...
	private async _installPackagePlugin(
		plugin: IMarketplacePlugin,
		silent?: boolean,
	): Promise<boolean> {
		const repo = this._pluginRepositoryService.getPluginSource(
