...
export class AgentPluginRecommendations
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			this._chatService.onDidSubmitRequest(() => {
	...
	private async _checkForRecommendedPlugins(): Promise<void> {
		...
		let fetched: readonly IMarketplacePlugin[] =
			this._pluginMarketplaceService.lastFetchedPlugins.get();
		if (fetched.length === 0) {
			try {
		...
		this._notificationService.prompt(
			...
			uninstalledCount === 1
				? localize(
						"agentPluginRecommendation.one",
						"This workspace recommends 1 agent plugin.",
					)
				: localize(
						"agentPluginRecommendation.many",
						"This workspace recommends {0} agent plugins.",
						uninstalledCount,
					),
			[
				{
