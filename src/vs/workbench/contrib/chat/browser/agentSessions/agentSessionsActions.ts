	...
	constructor() {
		super({
			...
			toggled: ContextKeyExpr.equals(
				`config.${ChatConfiguration.ChatViewSessionsEnabled}`,
				true,
			),
			menu: {
...
const agentSessionsOrientationSubmenu = new MenuId(
	"chatAgentSessionsOrientationSubmenu",
);
...
export class SetAgentSessionsOrientationStackedAction extends Action2 {
	constructor() {
		super({
			...
			toggled: ContextKeyExpr.equals(
				`config.${ChatConfiguration.ChatViewSessionsOrientation}`,
				"stacked",
			),
...
export class ArchiveAllAgentSessionsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor) {
		...
		const sessionsToArchive = agentSessionsService.model.sessions.filter(
...
export class MarkAllAgentSessionsReadAction extends Action2 {
	...
	async run(accessor: ServicesAccessor) {
		...
		const sessionsToMarkRead = agentSessionsService.model.sessions.filter(
			(session) => !session.isArchived() && !session.isRead(),
		);
...
export class ArchiveAgentSessionSectionAction extends Action2 {
	...
	async run(
		...
		const skipConfirmation = storageService.getBoolean(
...
abstract class BaseAgentSessionAction extends Action2 {
	async run(
		...
		if (isMarshalledAgentSessionContext(context)) {
			sessions = coalesce(
				(context.sessions ?? [context.session]).map((session) =>
					agentSessionsService.getSession(session.resource),
				),
			);
		} else if (context) {
			sessions = [context];
		...
		if (sessions.length === 0) {
			const chatView =
				viewsService.getActiveViewWithId<ChatViewPane>(ChatViewId);
...
export class ArchiveAgentSessionAction extends BaseAgentSessionAction {
	...
	async runWithSessions(
		...
		for (const session of sessions) {
			...
			if (
				chatModel &&
				!(await showClearEditingSessionConfirmation(chatModel, dialogService, {
...
export class RenameAgentSessionAction extends BaseAgentSessionAction {
	constructor() {
		super({
			...
			keybinding: {
				...
				when: ContextKeyExpr.and(
					ChatContextKeys.agentSessionsViewerFocused,
					ChatContextKeys.agentSessionType.isEqualTo(
			...
			menu: {
				...
				group: "1_edit",
				order: 3,
				when: ChatContextKeys.agentSessionType.isEqualTo(
					AgentSessionProviders.Local,
				),
			},
		});
	...
	async runWithSessions(
		...
		const title = await quickInputService.input({
			prompt: localize("newChatTitle", "New agent session title"),
			value: session.label,
		});
		if (title) {
...
export class DeleteAllLocalSessionsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		const localSessionsCount = agentSessionsService.model.sessions.filter(
			(session) => isLocalAgentSessionItem(session),
		).length;
		...
		await Promise.all(
			widgetService.getAllWidgets().map((widget) => widget.clear()),
		);
...
export class RefreshAgentSessionsViewerAction extends Action2 {
	...
	override run(
		accessor: ServicesAccessor,
		agentSessionsControl?: IAgentSessionsControl,
	) {
		const control =
			agentSessionsControl ??
			accessor.get(IViewsService).getActiveViewWithId<ChatViewPane>(ChatViewId)
				?.agentSessionsControl;
		if (control) {
			control.refresh();
		} else {
...
export class FindAgentSessionInViewerAction extends Action2 {
	...
	override run(
		...
		if (control) {
			...
			return accessor
				.get(ICommandService)
				.executeCommand("sessionsViewPane.find");
		}
	}
}
...
abstract class UpdateChatViewWidthAction extends Action2 {
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const canResizeView =
			chatLocation !== ViewContainerLocation.Panel ||
			panelPosition === Position.LEFT ||
			panelPosition === Position.RIGHT;

		// Update configuration if needed
		const chatViewSessionsEnabled = configurationService.getValue<boolean>(
			ChatConfiguration.ChatViewSessionsEnabled,
		);
		...
		const configuredOrientation = configurationService.getValue<
		...
		if (
			configuredOrientation === "stacked" ||
			configuredOrientation === "sideBySide"
		) {
		...
		const lastWidthForOrientation =
			chatView?.getLastDimensions(newOrientation)?.width;

		if (
			(!canResizeView || validatedConfiguredOrientation === "sideBySide") &&
			newOrientation === AgentSessionsViewerOrientation.Stacked
		) {
			chatView.updateConfiguredSessionsViewerOrientation("stacked");
		} else if (
			(!canResizeView || validatedConfiguredOrientation === "stacked") &&
			newOrientation === AgentSessionsViewerOrientation.SideBySide
		) {
		...
		const sideBySideMinWidth =
			chatViewDefaultWidth + sessionsViewDefaultWidth + 1; // account for possible theme border

		if (
			(newOrientation === AgentSessionsViewerOrientation.SideBySide &&
				currentSize.width >= sideBySideMinWidth) || // already wide enough to show side by side
			(newOrientation === AgentSessionsViewerOrientation.Stacked &&
				chatLocation === ViewContainerLocation.AuxiliaryBar &&
				layoutService.isAuxiliaryBarMaximized()) // try to not leave maximized state if maximized
		) {
			return;
		}
		...
		if (newOrientation === AgentSessionsViewerOrientation.SideBySide) {
			newWidth = Math.max(
				sideBySideMinWidth,
				lastWidthForOrientation ||
					Math.round(layoutService.mainContainerDimension.width / 2),
			);
		} else {
			newWidth =
				lastWidthForOrientation ||
				Math.max(
					...
					currentSize.width - sessionsViewDefaultWidth,
				);
		}
	...
	constructor() {
		super({
			...
			precondition: ContextKeyExpr.and(
				...
				ChatContextKeys.agentSessionsViewerOrientation.isEqualTo(
...
export class FocusAgentSessionsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const chatView = await viewsService.openView<ChatViewPane>(
			ChatViewId,
			true,
		);
		...
		const configuredSessionsViewerOrientation = configurationService.getValue<
			"stacked" | "sideBySide" | unknown
		>(ChatConfiguration.ChatViewSessionsOrientation);
