...
export class AgentSessionApprovalModel extends Disposable {
	private readonly _approvals = new Map<
		string,
		ISettableObservable<IAgentSessionApprovalInfo | undefined>
	>();
			...
			autorunIterableDelta(
				...
				({ addedValues, removedValues }) => {
					...
					for (const model of removedValues) {
						...
						this._approvals
							.get(model.sessionResource.toString())
	...
	getApproval(
		sessionResource: URI,
	): IObservable<IAgentSessionApprovalInfo | undefined> {
	...
	private _trackModel(model: IChatModel): IDisposable {
		...
		const setIfChanged = (value: IAgentSessionApprovalInfo | undefined) => {
			...
			if (
				...
				current.languageId === value.languageId
			) {
				return;
			}
		...
		return autorun((reader) => {
			...
			for (const part of lastResponse.response.value) {
				if (
					...
					part.toolSpecificData?.kind === "modifiedFilesConfirmation"
				) {
