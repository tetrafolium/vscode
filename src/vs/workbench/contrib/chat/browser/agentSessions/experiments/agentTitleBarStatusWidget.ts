...
type AgentStatusClickClassification = {
	source: {
		...
		comment: "Which part of the agent status widget was clicked.";
	};
	action: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The action taken in response to the click.";
	};
	owner: "joshspicer";
...
function shouldForceHiddenAgentStatus(
	...
	const aiFeaturesDisabled =
		configurationService.getValue<boolean>(ChatConfiguration.AIDisabled) ===
		true;
	const aiCustomizationsDisabled =
		configurationService.getValue<boolean>("disableAICustomizations") ===
			true ||
		configurationService.getValue<boolean>(
			"workbench.disableAICustomizations",
		) === true;

	return aiFeaturesDisabled && aiCustomizationsDisabled;
}

function getAgentStatusSettingMode(
	configurationService: IConfigurationService,
	contextKeyService: IContextKeyService,
): AgentStatusSettingMode {
...
export class AgentTitleBarStatusWidget extends BaseActionViewItem {
	...
	constructor(
		...
		@IAgentTitleBarStatusService
		private readonly agentTitleBarStatusService: IAgentTitleBarStatusService,
		...
		this._register(
			this.agentTitleBarStatusService.onDidChangeMode(() => {
				this._render();
			}),
		);

		this._register(
			this.agentTitleBarStatusService.onDidChangeSessionInfo(() => {
				this._render();
			}),
		);
		...
		this._register(
			this.editorGroupsService.onDidChangeEditorPartOptions(
		...
		this._register(
			this._commandCenterMenu.onDidChange(() => {
		...
		this._register(
			this.chatWidgetService.onDidBackgroundSession(() => {
				this._render();
			}),
		);
	}
	...
	override render(container: HTMLElement): void {
		...
		container.setAttribute(
			"aria-label",
			localize("agentStatusToolbarLabel", "Agent Status"),
		);
	...
	private _render(): void {
		...
		try {
			...
			const { activeSessions, unreadSessions, attentionNeededSessions } =
				this._getSessionStats();

			// Get attention session info for state computation
			const attentionSession =
				attentionNeededSessions.length > 0
			...
			const statusMode = getAgentStatusSettingMode(
			...
			if (this.agentTitleBarStatusService.mode === AgentStatusMode.Session) {
				...
				this.agentTitleBarStatusService.mode === AgentStatusMode.SessionReady
			) {
	...
	private _setupRovingTabIndex(disposables: DisposableStore): void {
		...
		disposables.add(
			addDisposableListener(this._container, EventType.KEY_DOWN, (e) => {
				const index = this._rovingElements.findIndex(
	...
	private _renderChatInputMode(disposables: DisposableStore): void {
		...
		inputArea.setAttribute(
			"aria-label",
			localize("openQuickAccess", "Open Quick Access"),
		);
		...
		const { progress: progressText } = this._getSessionNeedingAttention(
			attentionNeededSessions,
		);
		const defaultLabel = isCompactMode
			? this._getLabel()
			: (progressText ?? this._getLabel());

		if (!isCompactMode && progressText) {
			label.classList.add("has-progress");
		}

		const hoverLabel = localize(
		...
		if (isCompactMode) {
			...
			disposables.add(
				addDisposableListener(inputArea, EventType.MOUSE_ENTER, () => {
					reset(leftIcon, renderIcon(Codicon.searchSparkle));
			...
			disposables.add(
				addDisposableListener(inputArea, EventType.MOUSE_LEAVE, () => {
					reset(leftIcon, renderIcon(Codicon.searchSparkle));
				}),
			);
		} else {
		...
		disposables.add(
			this.hoverService.setupManagedHover(hoverDelegate, inputArea, () => {
				const kbForTooltip = this.keybindingService
					.lookupKeybinding(UNIFIED_QUICK_ACCESS_ACTION_ID)
					?.getLabel();
				return kbForTooltip
		...
		disposables.add(
			addDisposableListener(inputArea, EventType.CLICK, (e) => {
				e.preventDefault();
				e.stopPropagation();
				this.telemetryService.publicLog2<
					AgentStatusClickEvent,
					AgentStatusClickClassification
				>("agentStatusWidget.click", {
					source: "pill",
					action: "quickAccess",
				});
				const useUnifiedQuickAccess =
					this.configurationService.getValue<boolean>(
						ChatConfiguration.UnifiedAgentsBar,
					) === true;
				this.commandService.executeCommand(
					useUnifiedQuickAccess
						? UNIFIED_QUICK_ACCESS_ACTION_ID
						: QUICK_OPEN_ACTION_ID,
				);
			}),
		);
		...
		// Keyboard handler
		disposables.add(
			addDisposableListener(inputArea, EventType.KEY_DOWN, (e) => {
	...
	private _renderSessionMode(disposables: DisposableStore): void {
		...
		const hoverDelegate = getDefaultHoverDelegate("mouse");
		disposables.add(
			this.hoverService.setupManagedHover(hoverDelegate, pill, () => {
				const sessionInfo = this.agentTitleBarStatusService.sessionInfo;
				return sessionInfo
					? localize(
		...
		disposables.add(
			addDisposableListener(pill, EventType.MOUSE_DOWN, exitHandler),
		);
	...
	private _renderSessionReadyMode(disposables: DisposableStore): void {
		...
		const sessionInfo = this.agentTitleBarStatusService.sessionInfo;
		titleLabel.textContent =
			sessionInfo?.title ?? localize("agentSessionReady", "Review Changes");
		pill.appendChild(titleLabel);
		...
		const enterHandler = (e: Event) => {
			...
			const sessionInfo = this.agentTitleBarStatusService.sessionInfo;
			if (sessionInfo) {
				const session = this.agentSessionsService.getSession(
					sessionInfo.sessionResource,
				);
				if (session) {
					this.commandService.executeCommand(
						EnterAgentSessionProjectionAction.ID,
						session,
					);
				}
			}
		...
		disposables.add(
			addDisposableListener(pill, EventType.MOUSE_DOWN, enterHandler),
		);
	...
	private _renderCommandCenterToolbar(
		...
		for (const [, actions] of this._commandCenterMenu.getActions({
	...
	private _renderSearchButton(
		...
		searchButton.setAttribute(
			"aria-label",
			localize("openQuickOpen", "Open Quick Open"),
		);
		...
		// Setup hover
		const hoverDelegate = getDefaultHoverDelegate("mouse");
		const searchKb = this.keybindingService
			.lookupKeybinding(QUICK_OPEN_ACTION_ID)
			?.getLabel();
		...
		// Click handler
		disposables.add(
			addDisposableListener(searchButton, EventType.CLICK, (e) => {
				e.preventDefault();
				e.stopPropagation();
		...
		// Keyboard handler
		disposables.add(
			addDisposableListener(searchButton, EventType.KEY_DOWN, (e) => {
				if (e.key === "Enter" || e.key === " ") {
					e.preventDefault();
					e.stopPropagation();
					this.commandService.executeCommand(QUICK_OPEN_ACTION_ID);
	...
	private _renderStatusBadge(
		...
		const menuActions: IAction[] = Separator.join(
			...this._chatTitleBarMenu
		...
		disposables.add(
			addDisposableListener(sparkleContainer, EventType.KEY_DOWN, (e) => {
				if (e.key === "Enter" || e.key === " ") {
					e.preventDefault();
					e.stopPropagation();
		...
		if (
			...
			disposables.add(
				addDisposableListener(unreadSection, EventType.CLICK, (e) => {
					e.preventDefault();
					e.stopPropagation();
			...
			disposables.add(
				addDisposableListener(unreadSection, EventType.KEY_DOWN, (e) => {
					if (e.key === "Enter" || e.key === " ") {
						e.preventDefault();
						e.stopPropagation();
						this._openSessionsWithFilter("unread");
		...
		if (viewSessionsEnabled && hasAttentionNeeded) {
			...
			needsInputSection = $(
				"span.agent-status-badge-section.active.needs-input",
			);
			...
			disposables.add(
				addDisposableListener(needsInputSection, EventType.CLICK, (e) => {
					e.preventDefault();
					e.stopPropagation();
			...
			disposables.add(
				addDisposableListener(needsInputSection, EventType.KEY_DOWN, (e) => {
					if (e.key === "Enter" || e.key === " ") {
						e.preventDefault();
						e.stopPropagation();
						this._openSessionsWithFilter("needsInput");
		...
		const inProgressOnly = activeSessions.filter(
		...
		if (viewSessionsEnabled && inProgressOnly.length > 0) {
			...
			disposables.add(
				addDisposableListener(activeSection, EventType.CLICK, (e) => {
					e.preventDefault();
					e.stopPropagation();
			...
			disposables.add(
				addDisposableListener(activeSection, EventType.KEY_DOWN, (e) => {
					if (e.key === "Enter" || e.key === " ") {
						e.preventDefault();
						e.stopPropagation();
						this._openSessionsWithFilter("inProgress");
			...
			const activeTooltip =
				inProgressOnly.length === 1
					? localize(
							"activeSessionsTooltip1",
							"{0} session in progress",
							inProgressOnly.length,
						)
					: localize(
	...
	private _clearFilterIfCategoryEmpty(
		...
		if (
			...
			this._badgeFilterAppliedByThisWindow === "needsInput" &&
			!hasAttentionNeeded
		) {
			this._restoreUserFilter();
		}
	...
	private _getCurrentFilterState(): {
		...
		isFilteredToInProgress: boolean;
		isFilteredToNeedsInput: boolean;
	} {
		...
		const isFilteredToUnread =
			filter.read === true && filter.states.length === 0;
		// Detect if filtered to in-progress only (3 excluded states including NeedsInput)
		const isFilteredToInProgress =
			filter.states?.length === 3 &&
			filter.states.includes(AgentSessionStatus.NeedsInput) &&
			filter.read === false;
		// Detect if filtered to needs-input only (3 excluded states including InProgress)
		const isFilteredToNeedsInput =
			filter.states?.length === 3 &&
			filter.states.includes(AgentSessionStatus.InProgress) &&
			filter.read === false;
	...
	private _openSessionsWithFilter(
		filterType: "unread" | "inProgress" | "needsInput",
	): void {
		const {
			isFilteredToUnread,
			isFilteredToInProgress,
		...
		const isToggleOff =
			(filterType === "unread" && isFilteredToUnread) ||
	...
	private _renderEscapeButton(
		...
		escButton.setAttribute(
			"aria-label",
			localize("exitAgentSessionProjection", "Exit Agent Session Projection"),
		);
		...
		// Click handler
		disposables.add(
			addDisposableListener(escButton, EventType.MOUSE_DOWN, (e) => {
				e.preventDefault();
				e.stopPropagation();
				this.commandService.executeCommand(ExitAgentSessionProjectionAction.ID);
			}),
		...
		disposables.add(
			addDisposableListener(escButton, EventType.CLICK, (e) => {
				e.preventDefault();
				e.stopPropagation();
				this.commandService.executeCommand(ExitAgentSessionProjectionAction.ID);
			}),
		...
		// Keyboard handler
		disposables.add(
			addDisposableListener(escButton, EventType.KEY_DOWN, (e) => {
				if (e.key === "Enter" || e.key === " ") {
					e.preventDefault();
					e.stopPropagation();
					this.commandService.executeCommand(
						ExitAgentSessionProjectionAction.ID,
					);
				}
			}),
		);
	...
	private _renderEnterButton(
		...
		enterButton.textContent =
			keybinding?.getLabel() ?? localize("review", "Review");
		enterButton.setAttribute("role", "button");
		enterButton.setAttribute(
			"aria-label",
			localize("enterAgentSessionProjection", "Enter Agent Session Projection"),
		);
		...
		// Click handler
		disposables.add(
			addDisposableListener(enterButton, EventType.MOUSE_DOWN, enterProjection),
		);
		disposables.add(
			addDisposableListener(enterButton, EventType.CLICK, enterProjection),
		);

		// Keyboard handler
		disposables.add(
			addDisposableListener(enterButton, EventType.KEY_DOWN, (e) => {
				if (e.key === "Enter" || e.key === " ") {
					enterProjection(e);
				}
			}),
	...
	private _getSessionNeedingAttention(
		attentionNeededSessions: IAgentSession[],
	): { session: IAgentSession | undefined; progress: string | undefined } {
		...
		const progress =
			typeof mostRecent.description === "string"
	...
	private _getLabel(): string {
		...
		if (this._windowTitle.isCustomTitleFormat()) {
			...
			!label &&
			this.editorGroupsService.partOptions.showTabs === "none"
		) {
 ...
 */
export class AgentTitleBarStatusRendering
	extends Disposable
	...
	constructor(
		...
		this._register(
			actionViewItemService.register(
				...
				MenuId.AgentsTitleBarControlMenu,
				(action, options) => {
		...
		const chatEnabledKey =
			contextKeyService.getContextKeyValue<boolean>("chatIsEnabled");
		...
		const updateClass = () => {
			...
			const enabled =
				commandCenterEnabled && chatEnabled && statusMode !== "hidden";
			const enhanced = enabled && statusMode === "compact";

			mainWindow.document.body.classList.toggle(
		...
		this._register(
			contextKeyService.onDidChangeContext((e) => {
				if (
					e.affectsSome(new Set(["chatIsEnabled", InEditorZenModeContext.key]))
				) {
					chatEnabled =
						!!contextKeyService.getContextKeyValue<boolean>("chatIsEnabled");
					updateClass();
