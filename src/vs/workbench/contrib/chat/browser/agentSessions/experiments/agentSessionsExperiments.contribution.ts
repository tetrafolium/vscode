...
} from "./agentSessionProjectionService.js";
import {
 ...
 */
class AgentSessionReadyContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		@IAgentSessionProjectionService
		private readonly agentSessionProjectionService: IAgentSessionProjectionService,
		@IChatEditingService
		...
		this._register(
			this.agentSessionProjectionService.onDidChangeProjectionMode(
				(isActive) => {
					if (!isActive) {
		...
		this._register(
			autorun((reader) => {
				...
				const currentWidget = this.chatWidgetService
					.getAllWidgets()
					.find((w) => w.location === ChatAgentLocation.Chat);
				if (currentWidget) {
	...
	private _checkSession(sessionResource: URI | undefined): void {
		...
		if (
			sessionResource?.toString() !== this._watchedSessionResource?.toString()
		) {
		...
		if (this.agentSessionProjectionService.isActive) {
			...
			if (
				sessionResource &&
				activeSession &&
				sessionResource.toString() !== activeSession.resource.toString()
			) {
				const newSession =
	...
	private _updateSessionReadyState(sessionResource: URI | undefined): void {
		...
		if (session.providerType === AgentSessionProviders.Local) {
			...
			const editingSession =
				this.chatEditingService.getEditingSession(sessionResource);
			if (!editingSession) {
				this._clearEntriesWatcher();
				this.agentTitleBarStatusService.exitSessionReadyMode();
				return;
			...
			hasPendingChanges = entries.some(
				(entry) => entry.state.get() === ModifiedFileEntryState.Modified,
			);

			if (hasPendingChanges && !this._suppressSessionReady) {
				this.agentTitleBarStatusService.enterSessionReadyMode(
					session.resource,
					session.label,
				);
				...
				if (
					...
					this._watchedSessionResource.toString() !== sessionResource.toString()
				) {
					this._clearEntriesWatcher();
...
MenuRegistry.appendMenuItem(MenuId.CommandCenter, {
	...
	when: ContextKeyExpr.and(
		...
		ContextKeyExpr.notEquals(
			`config.${ChatConfiguration.AgentStatusEnabled}`,
			"hidden",
		),
		ContextKeyExpr.notEquals(
			`config.${ChatConfiguration.AgentStatusEnabled}`,
...
MenuRegistry.appendMenuItem(MenuId.TitleBar, {
	...
	when: ContextKeyExpr.and(
		...
		ContextKeyExpr.and(ChatContextKeys.Setup.hidden.negate()),
