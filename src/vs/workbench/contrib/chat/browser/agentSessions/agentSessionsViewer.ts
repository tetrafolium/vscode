...
import {
	...
	IAgentSessionShowLess,
	IAgentSessionShowMore,
	IAgentSessionsModel,
	isAgentSession,
		...
		>
{
	...
	static getApprovalRowHeight(label: string): number {
		const lineCount = Math.min(
			label.split(/\r?\n/).length,
			AgentSessionRenderer.APPROVAL_ROW_MAX_LINES,
		);
		return (
			lineCount * AgentSessionRenderer._APPROVAL_ROW_LINE_HEIGHT +
			AgentSessionRenderer._APPROVAL_ROW_OVERHEAD
		);
	}

	readonly templateId = AgentSessionRenderer.TEMPLATE_ID;

	private readonly sessionHover = this._register(
		new MutableDisposable<AgentSessionHoverWidget>(),
	);
	...
	renderTemplate(container: HTMLElement): IAgentSessionItemTemplate {
		...
		const contextKeyService = disposables.add(
			this.contextKeyService.createScoped(elements.item),
		);
		...
		return {
			...
			icon: elements.icon,
			title: disposables.add(
				new IconLabel(elements.title, {
	...
	renderElement(
		session: ITreeNode<IAgentSession, FuzzyScore>,
		index: number,
		template: IAgentSessionItemTemplate,
		...
		if (this.options.useStatusOnlyIcons) {
			...
			template.icon.style.color = statusIcon.color
				? asCssVariable(statusIcon.color.id)
				: "";
			...
			template.icon.style.color = icon.color
				? asCssVariable(icon.color.id)
				: "";
		...
		ChatContextKeys.isArchivedAgentSession
			.bindTo(template.contextKeyService)
			.set(session.element.isArchived());
		ChatContextKeys.isPinnedAgentSession
			.bindTo(template.contextKeyService)
			.set(session.element.isPinned());
		ChatContextKeys.isReadAgentSession
			.bindTo(template.contextKeyService)
			.set(session.element.isRead());
		ChatContextKeys.agentSessionType
			.bindTo(template.contextKeyService)
			.set(session.element.providerType);
		template.titleToolbar.context = session.element;

		// Pinned indicator
		const isPinned = session.element.isPinned();
		template.pinnedIndicator.className =
			"agent-session-pinned-indicator " + ThemeIcon.asClassName(Codicon.pinned);
		...
		if (
			!isSessionInProgressStatus(session.element.status) &&
			diff &&
			hasValidDiff(diff)
		) {
		...
		const hideDetails =
			hasDescription && isSessionInProgressStatus(session.element.status);
		template.badge.classList.toggle("has-badge", hasBadge && !hideDetails);
		template.diffContainer.classList.toggle(
			"has-diff",
			hasDiff && !hideDetails,
		);
		template.statusContainer.classList.toggle("hidden", hideDetails);
		template.separator.classList.toggle(
			"has-separator",
			!hideDetails && hasBadge && hasDiff,
		);
		template.description.classList.toggle(
			"has-separator",
			hasDescription && !hideDetails && (hasBadge || hasDiff),
		);
	...
	private triggerResolve(
		...
		this.chatSessionsService
			.resolveChatSessionItem(
				session.element.providerType,
				session.element.resource,
				cts.token,
			)
			.catch(() => {
	...
	private renderBadge(
		...
		const badgeValue =
			typeof normalisedBadge === "string"
				? normalisedBadge
				: normalisedBadge.value;
		if (!badgeValue) {
			return false;
		}
	...
	private renderStatus(
		...
		const repoPrefix =
			session.element.isPinned() && this.options.isGroupedByRepository?.()
		...
		timer.cancelAndSet(
			() => (template.statusTime.textContent = getStatusText(session.element)),
			session.element.status === AgentSessionStatus.InProgress
				? 1000 /* every second */
	...
	private renderHover(
		...
		if (
			!isSessionInProgressStatus(session.element.status) &&
			session.element.isRead()
		) {
			return; // the hover is complex and large, for now limit it to in-progress sessions only
		}

		const reducedDelay =
			session.element.status === AgentSessionStatus.NeedsInput;
		template.elementDisposable.add(
			this.hoverService.setupDelayedHover(
				template.element,
				() => this.buildHoverContent(session.element),
				{ groupId: "agent.sessions", reducedDelay },
			),
		);
	}
	...
	private buildHoverContent(session: IAgentSession): IDelayedHoverOptions {
		if (
			this.sessionHover.value?.session.resource.toString() !==
			...
			this.sessionHover.value = this.instantiationService.createInstance(
	...
	private renderApprovalRow(
		...
		if (this._approvalModel === undefined) {
			throw new BugIndicatingError(
				"Approval model is required to render approval row",
			);
		}
		...
		const initialInfo = approvalModel
			.getApproval(session.element.resource)
			.get();
		...
		template.elementDisposable.add(
			autorun((reader) => {
				...
				if (info) {
					...
					const isActive =
						this._activeSessionResource.read(reader)?.toString() ===
						session.element.resource.toString();
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<IAgentSession>, FuzzyScore>,
		index: number,
		templateData: IAgentSessionItemTemplate,
		details?: ITreeElementRenderDetails,
	): void {
	...
	disposeElement(
		element: ITreeNode<IAgentSession, FuzzyScore>,
		index: number,
		template: IAgentSessionItemTemplate,
		details?: ITreeElementRenderDetails,
	): void {
...
export class AgentSessionSectionRenderer implements ICompressibleTreeRenderer<
	IAgentSessionSection,
	FuzzyScore,
...
> {
	...
	renderTemplate(container: HTMLElement): IAgentSessionSectionTemplate {
		...
		const contextKeyService = disposables.add(
			this.contextKeyService.createScoped(elements.container),
		);
		const scopedInstantiationService = disposables.add(
			this.instantiationService.createChild(
				new ServiceCollection([IContextKeyService, contextKeyService]),
	...
	renderElement(
		element: ITreeNode<IAgentSessionSection, FuzzyScore>,
		index: number,
		template: IAgentSessionSectionTemplate,
		details?: ITreeElementRenderDetails,
	): void {
		// Label
		...
		// Toolbar
		ChatContextKeys.agentSessionSection
			.bindTo(template.contextKeyService)
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<IAgentSessionSection>, FuzzyScore>,
		index: number,
		templateData: IAgentSessionSectionTemplate,
		details?: ITreeElementRenderDetails,
	): void {
		throw new Error(
			"Should never happen since section header is incompressible",
		);
	}
...
> {
	...
	constructor(
		private readonly options?: IAgentSessionShowMoreRendererOptions,
	) {}

	renderTemplate(container: HTMLElement): IAgentSessionShowMoreTemplate {
		const disposables = new DisposableStore();

		const elements = h("div.agent-session-show-more@container", [
	...
	renderElement(
		element: ITreeNode<IAgentSessionShowMore, FuzzyScore>,
		_index: number,
		template: IAgentSessionShowMoreTemplate,
	): void {
		template.label.textContent = this.options?.compactLabel
			? localize(
					"agentSessions.showMoreCompact",
					"+{0} more",
					element.element.remainingCount,
				)
			: localize(
					"agentSessions.showMore",
					"Show {0} More...",
					element.element.remainingCount,
				);
		template.container.setAttribute(
			"data-section-label",
			element.element.sectionLabel,
		);
	}
...
export class AgentSessionShowLessRenderer implements ICompressibleTreeRenderer<
	IAgentSessionShowLess,
	FuzzyScore,
	IAgentSessionShowMoreTemplate
> {
	...
	renderElement(
		element: ITreeNode<IAgentSessionShowLess, FuzzyScore>,
		_index: number,
		template: IAgentSessionShowMoreTemplate,
	): void {
		template.label.textContent = localize(
			"agentSessions.showLess",
			"Show less",
		);
		template.container.setAttribute(
			"data-section-label",
...
export class AgentSessionsAccessibilityProvider implements IListAccessibilityProvider<AgentSessionListItem> {
	...
	getAriaLabel(element: AgentSessionListItem): string | null {
		if (isAgentSessionSection(element)) {
			...
			if (count === 1) {
				return localize(
					"agentSessionSectionAriaLabel.singular",
					"{0} sessions section, {1} session",
					element.label,
					count,
				);
			}
			return localize(
				"agentSessionSectionAriaLabel.plural",
...
export class AgentSessionsDataSource
	extends Disposable
	implements IAsyncDataSource<IAgentSessionsModel, AgentSessionListItem>
{
	...
	private readonly _onDidExpandRepositoryGroup = this._register(
		new Emitter<void>(),
	);
	readonly onDidExpandRepositoryGroup: Event<void> =
		this._onDidExpandRepositoryGroup.event;
	...
	constructor(
		...
		if (this.filter) {
			...
			this._register(
				this.filter.onDidChange(() => {
					const currentCapped =
						this.filter!.getExcludes().repositoryGroupCapped;
	...
	getChildren(
		element: IAgentSessionsModel | AgentSessionListItem,
	): Iterable<AgentSessionListItem> {
		// Sessions model
		if (isAgentSessionsModel(element)) {
			// Apply filter if configured
			let filteredSessions = element.sessions.filter(
				(session) => !this.filter?.exclude(session),
			);

			// Apply sorter unless we group into sections or we are to limit results
			const limitResultsCount = this.filter?.limitResults?.();
			if (
				!this.filter?.groupResults?.() ||
				typeof limitResultsCount === "number"
			) {
		...
		// Sessions	section
			...
			const isCappingEnabled =
				this.repositoryGroupLimit &&
				this.filter?.getExcludes().repositoryGroupCapped;
			if (
				isCappingEnabled &&
				element.section === AgentSessionSection.Repository &&
				element.sessions.length > this.repositoryGroupLimit
			) {
				if (!this.expandedRepositoryGroups.has(element.label)) {
					...
					return [
						...element.sessions,
						{ showLess: true as const, sectionLabel: element.label },
					];
				}
			}
	...
	private groupSessionsIntoSections(
		...
		const isCapped =
			this.filter?.groupResults?.() === AgentSessionsGrouping.Capped;

		const sorter = this.sorter;
		const sortedSessions =
			sorter instanceof AgentSessionsSorter
				? sessions.sort((a, b) =>
						sorter.compare(
							a,
							b,
							true /* prioritize active sessions to keep in-progress/needs-input ones top within each group */,
						),
					)
		...
		if (isCapped) {
			...
			this.filter?.groupResults?.() === AgentSessionsGrouping.Repository
		) {
	...
	private groupSessionsCapped(
		sortedSessions: IAgentSession[],
	): AgentSessionListItem[] {
		const result: AgentSessionListItem[] = [];

		const firstArchivedIndex = sortedSessions.findIndex((session) =>
			session.isArchived(),
		);
		const nonArchivedCount =
			firstArchivedIndex === -1 ? sortedSessions.length : firstArchivedIndex;
		...
		const pinnedSessions = nonArchivedSessions.filter((session) =>
			session.isPinned(),
		);
		const unpinnedSessions = nonArchivedSessions.filter(
			(session) => !session.isPinned(),
		);

		// Take up to N non-pinned sessions from the sorted order (preserves NeedsInput prioritization)
		const topUnpinned = unpinnedSessions.slice(
			0,
			AgentSessionsDataSource.CAPPED_SESSIONS_LIMIT,
		);
		const remainingUnpinned = unpinnedSessions.slice(
			AgentSessionsDataSource.CAPPED_SESSIONS_LIMIT,
		);
	...
	private groupSessionsByRepository(
		...
		const repoMap = new Map<
			string,
			{ label: string; sessions: IAgentSession[] }
		>();
...
export function getRepositoryName(session: IAgentSession): string | undefined {
	...
	if (metadata) {
		...
		const workingDirectoryPath = metadata.workingDirectoryPath as
			| string
			| undefined;
...
export function groupAgentSessionsByDate(
	sessions: IAgentSession[],
	sortBy?: AgentSessionsSorting,
): Map<AgentSessionSection, IAgentSessionSection> {
	const now = Date.now();
...
export class AgentSessionsIdentityProvider implements IIdentityProvider<
	IAgentSessionsModel | AgentSessionListItem
> {
	...
	getGroupId(
		element: IAgentSessionsModel | AgentSessionListItem,
	): number | NotSelectableGroupIdType {
...
export class AgentSessionsSorter implements ITreeSorter<IAgentSession> {
	...
	compare(
		sessionA: IAgentSession,
		sessionB: IAgentSession,
		prioritizeActiveSessions = false,
	): number {
...
export class AgentSessionsKeyboardNavigationLabelProvider implements ICompressibleKeyboardNavigationLabelProvider<AgentSessionListItem> {
	...
	getCompressedNodeKeyboardNavigationLabel(
		elements: AgentSessionListItem[],
	): { toString(): string | undefined } | undefined {
		return undefined; // not enabled
	}
}

export class AgentSessionsDragAndDrop
	extends Disposable
	implements ITreeDragAndDrop<AgentSessionListItem>
{
	constructor(
		@IInstantiationService
		private readonly instantiationService: IInstantiationService,
	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
		const elements = (data.getData() as AgentSessionListItem[]).filter((e) =>
			isAgentSession(e),
		);
	...
	getDragURI(element: AgentSessionListItem): string | null {
		if (
			...
			isAgentSessionShowMore(element) ||
			isAgentSessionShowLess(element)
		) {
	...
	onDragOver(
		...
		targetElement: AgentSessionListItem | undefined,
