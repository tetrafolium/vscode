...
export class AgentSessionHoverWidget extends Disposable {
	...
	constructor(
		...
		this.contentElement = dom.append(
			this.domNode,
			dom.$(".agent-session-hover-content"),
		);
		this.loadingElement = dom.append(
			this.contentElement,
			dom.$(".agent-session-hover-loading"),
		);
		dom.append(
			this.loadingElement,
		...
		this.renderScheduler = this._register(
			new RunOnceScheduler(() => this.render(), 200),
		);
	}
	...
	private async loadModel() {
		const modelRef = await this.chatService.acquireOrLoadSession(
			...
			this.cts.token,
			"AgentSessionHoverWidget#loadModel",
		);
	...
	private async render() {
		...
		const viewModel = this._register(
			this.instantiationService.createInstance(ChatViewModel, model, {
				maxVisibleItems: 2,
			}),
		);
		...
		const container = dom.append(
			this.contentElement,
			dom.$(".interactive-list"),
		);
		const listWidget = this._register(
			this.instantiationService.createInstance(ChatListWidget, container, {
				rendererOptions: {
					renderStyle: "compact",
					noHeader: true,
					editable: false,
		...
		const viewModelScheudler = this._register(
			new RunOnceScheduler(() => listWidget.refresh(), 500),
		);
		this._register(
		...
		this._register(
			listWidget.onDidClickFollowup(async (followup) => {
				const widget = await this.chatWidgetService.openSession(
	...
	private buildHeader(): void {
		...
		const header = dom.append(
			this.domNode,
			dom.$(".agent-session-hover-header"),
		);
		...
		const detailsRow = dom.append(
			header,
			dom.$(".agent-session-hover-details"),
		);
		...
		dom.append(
			detailsRow,
			dom.$("span", undefined, getAgentSessionProviderName(provider)),
		);
		...
		if (session.timing.lastRequestEnded && session.timing.lastRequestStarted) {
			...
			dom.append(
				detailsRow,
				dom.$("span", undefined, fromNow(startTime, true, true)),
			);
		}
		...
		const diffSeparator = dom.append(
			detailsRow,
			dom.$("span.separator", undefined, "•"),
		);
		const diffContainer = dom.append(
			detailsRow,
			dom.$(".agent-session-hover-diff"),
		);
		diffSeparator.style.display = "none";
		diffContainer.style.display = "none";

		const observed = this.agentSessionsService.model.observeSession(
			session.resource,
		);
		this._register(
			autorun((reader) => {
				...
				if (diff && hasValidDiff(latest.changes)) {
					...
					if (diff.files > 0) {
						dom.append(
							...
							dom.$(
								...
								undefined,
								diff.files === 1
									? localize("tooltip.file", "1 file")
									: localize("tooltip.files", "{0} files", diff.files),
							),
						);
					}
					if (diff.insertions > 0) {
						dom.append(
							diffContainer,
							dom.$("span.insertions", undefined, `+${diff.insertions}`),
						);
					}
					if (diff.deletions > 0) {
						dom.append(
							diffContainer,
							dom.$("span.deletions", undefined, `-${diff.deletions}`),
						);
					}
				} else {
					diffSeparator.style.display = "none";
		...
		if (session.status !== AgentSessionStatus.Completed) {
			...
			dom.append(
				detailsRow,
				dom.$("span", undefined, this.toStatusLabel(session.status)),
			);
		}
		...
		if (session.isArchived()) {
			...
			dom.append(
				detailsRow,
				dom.$("span", undefined, localize("tooltip.archived", "Archived")),
			);
		}
	}
	...
	private buildFallbackTooltip(session: IAgentSession): IMarkdownString {
		...
		if (session.timing.lastRequestEnded && session.timing.lastRequestStarted) {
			...
			timeLabel =
				duration ?? fromNow(session.timing.lastRequestStarted, true, true);
		} else {
			const startTime =
				session.timing.lastRequestStarted ?? session.timing.created;
