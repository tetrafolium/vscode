...
export async function openSession(
	...
	for (const participant of sessionOpenerRegistry.getParticipants()) {
		try {
			const handled = await instantiationService.invokeFunction((accessor) =>
				participant.handleOpenSession(accessor, session, openOptions),
			);
			if (handled) {
	...
	return instantiationService.invokeFunction((accessor) =>
		openSessionDefault(accessor, session, openOptions),
	);
}

async function openSessionDefault(
	accessor: ServicesAccessor,
	session: IAgentSession,
	openOptions?: ISessionOpenOptions,
): Promise<IChatWidget | undefined> {
	...
	try {
		...
		const isLocalChatSession =
			session.resource.scheme === Schemas.vscodeChatEditor ||
			getChatSessionType(session.resource) === localChatSessionType;
		if (
			!isLocalChatSession &&
			!(await chatSessionsService.canResolveChatSession(
