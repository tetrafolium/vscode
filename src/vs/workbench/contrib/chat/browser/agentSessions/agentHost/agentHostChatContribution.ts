 ...
 */
export class AgentHostContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		_agentHostFileSystemService: IAgentHostFileSystemService,
		...
		this._loggedConnection = this._register(
		...
		this._register(
			_agentHostFileSystemService.registerAuthority(
				"local",
				this._agentHostService,
			),
		);
		...
		this._register(
			this._agentHostService.rootState.onDidChange((rootState) => {
		...
		this._register(
			this._agentHostService.onAgentHostStart(() => {
	...
	private _handleRootStateChange(rootState: RootState): void {
		...
		this._authenticateWithServer(rootState.agents).catch(() => {
			/* best-effort */
		});
	...
	private _registerAgent(agent: AgentInfo): void {
		...
		store.add(
			this._chatSessionsService.registerChatSessionContribution({
				...
				customAgentTarget: this._isSessionsWindow
					? undefined
		...
		const agentRegistration = store.add(
		...
		const sessionHandler = store.add(
			this._instantiationService.createInstance(AgentHostSessionHandler, {
				...
				resolveAuthentication: (resources) =>
					this._resolveAuthenticationInteractively(resources),
			}),
		);
		store.add(
		...
		try {
			await authenticateProtectedResources(agents, {
				...
				authenticate: (request) =>
					this._loggedConnection!.authenticate(request),
			});
		} catch (err) {
			this._logService.error(
				"[AgentHost] Failed to authenticate with server",
				err,
			);
	...
	private async _resolveAuthenticationInteractively(
		...
		try {
			...
			this._logService.error(
				"[AgentHost] Interactive authentication failed",
				err,
			);
			this._loggedConnection!.logError(
				"resolveAuthenticationInteractively",
				err,
			);
		}
