...
export class AgentCustomizationContentExpander {
		...
		try {
			...
			for (let i = 0; i < dirNames.length; i++) {
				...
				if (promptType === PromptsType.skill) {
					children.push(
						...(await this.collectFromSkillDir(
							stat.stat.children,
							pluginUri,
							source,
							groupKey,
					...
					children.push(
						...(await this.collectFromRegularDir(
							stat.stat.children,
							pluginUri,
							source,
							promptType,
	 ...
	 */
	private async collectFromSkillDir(
		entries: readonly { name: string; resource: URI; isDirectory: boolean }[],
		pluginUri: URI,
		source: AICustomizationSource,
		groupKey: string,
		isBundleItem: boolean,
		...
		for (const child of entries) {
			...
			readMetaDataPromises.push(
				this.readPromptMetadata(
					joinPath(child.resource, SKILL_FILENAME),
					token,
				),
			);
		}
	 ...
	 */
	private async collectFromRegularDir(
		entries: readonly { name: string; resource: URI; isDirectory: boolean }[],
		pluginUri: URI,
		source: AICustomizationSource,
		...
		for (const child of entries) {
			...
			if (
				ext !== ".md" &&
				!(promptType === PromptsType.instructions && ext === ".mdc")
			) {
				continue;
			}
			eligible.push(child);
		...
		const promptMetadata = parseMetadata
			? await Promise.all(
					eligible.map((child) =>
						this.readPromptMetadata(child.resource, token),
					),
				)
			: undefined;
		...
		for (let i = 0; i < eligible.length; i++) {
			...
			items.push({
				...
				pluginUri: isBundleItem ? undefined : pluginUri,
				userInvocable:
					promptType === PromptsType.agent ? meta?.userInvocable : undefined,
			} satisfies ICustomizationItem);
		}
		return items;
	}
	 ...
	 */
	private async readPromptMetadata(
		promptFileUri: URI,
		token: CancellationToken,
	): Promise<
		| {
	...
	> {
		...
		try {
			...
			if (frontmatter) {
				...
				const userInvocable =
					userInvocableStr === "true"
						? true
						: userInvocableStr === "false"
							? false
							: undefined;
