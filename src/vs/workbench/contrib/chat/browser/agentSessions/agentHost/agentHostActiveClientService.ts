...
export const IAgentHostActiveClientService =
	createDecorator<IAgentHostActiveClientService>(
...
export class AgentHostActiveClientService
	extends Disposable
	implements IAgentHostActiveClientService
{
	declare readonly _serviceBrand: undefined;

	private readonly _customizationsByType: ISettableObservable<
		ReadonlyMap<string, IObservable<readonly ClientPluginCustomization[]>>
	>;
	readonly clientTools: IObservable<readonly ToolDefinition[]>;
	...
	constructor(
		...
		super();
		this._customizationsByType = observableValue(
			"agentHostCustomizationsByType",
			new Map(),
		);
		...
		const allowlistObs = observableConfigValue<string[]>(
			...
			return allToolsObs
				...
				.filter(
					(t) =>
						...
						allowlist.has(t.toolReferenceName),
				)
	...
	registerForAgent(sessionType: string): IAgentRegistration {
		...
		const syncProvider = store.add(
			new AgentCustomizationSyncProvider(sessionType, this._storageService),
		);
	...
	getActiveClient(sessionType: string, clientId: string): SessionActiveClient {
		return {
			...
			customizations: [
				...(this._customizationsByType.get().get(sessionType)?.get() ?? []),
			],
		};
	}
	...
	getCustomizations(
		...
		return derived(
			(reader) =>
				this._customizationsByType
					.read(reader)
					.get(sessionType)
					?.read(reader) ?? EMPTY_CUSTOMIZATIONS,
		);
	}
}

const EMPTY_CUSTOMIZATIONS: readonly ClientPluginCustomization[] =
	Object.freeze([]);
