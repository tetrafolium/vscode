...
export class AgentCustomizationItemProvider
	extends Disposable
	implements ICustomizationItemProvider
{
	...
	constructor(
		...
		private readonly _getItemActions:
			| ((
					customization: PluginCustomization,
					clientId: string | undefined,
			  ) => ICustomizationItemAction[] | undefined)
			| undefined,
		@IFileService private readonly _fileService: IFileService,
		@ILogService private readonly _logService: ILogService,
		@IAgentHostCustomizationService
		private readonly _customAgentsService: IAgentHostCustomizationService,
	) {
		super();
		...
		this._register(
			this._customAgentsService.onDidChangeCustomizations(() => {
	...
	private toBadge(
		customization: PluginCustomization,
		fromClient: boolean,
	): { badge?: string; badgeTooltip?: string; groupKey?: string } {
		if (fromClient) {
			return {
	...
	private toDirectoryItems(
		customization: DirectoryCustomization,
		source: AICustomizationSource,
	...
	private toDirectoryChildItem(
		child: ChildCustomization,
		source: AICustomizationSource,
		groupKey: string | undefined,
	): ICustomizationItem | undefined {
	...
	async provideChatSessionCustomizations(
		...
		const customizations =
			this._customAgentsService.getCustomizations(sessionResource);
		...
		for (const sessionCustomization of customizations) {
			if (isDirectoryCustomization(sessionCustomization)) {
				...
				const childGroupKey = isClientSynced
					? REMOTE_CLIENT_GROUP
					: REMOTE_HOST_GROUP;
				...
				if (!isBundleItem) {
					...
					item = {
						uri: this.toRemoteUri(sessionCustomization.uri),
						type: "plugin",
		...
		const workingDirectory =
			this._customAgentsService.getWorkingDirectory(sessionResource);

		for (const sessionCustomization of directoryCustomizations) {
			const source =
				workingDirectory &&
				sessionCustomization.uri.startsWith(workingDirectory)
					? AICustomizationSources.local
					: AICustomizationSources.user;
			const groupKey = sessionCustomization.clientId
				? REMOTE_CLIENT_GROUP
				: undefined;
			for (const child of this.toDirectoryItems(
				sessionCustomization,
				source,
				groupKey,
			)) {
	 ...
	 */
	private async _expandPluginContents(
		plugin: PluginMeta,
		token: CancellationToken,
...
function toStatusString(
	load: CustomizationLoadState | undefined,
): "loading" | "loaded" | "degraded" | "error" | undefined {
	return load?.kind;
}
...
function toStatusMessage(
	...
	if (
		load?.kind === CustomizationLoadStatus.Degraded ||
		load?.kind === CustomizationLoadStatus.Error
	) {
		return load.message;
	}
	return undefined;
...
function isDirectoryCustomization(
	customization: Customization,
): customization is DirectoryCustomization {
	return customization.type === CustomizationType.Directory;
}

function toPromptsType(
	type: ChildCustomization["type"],
