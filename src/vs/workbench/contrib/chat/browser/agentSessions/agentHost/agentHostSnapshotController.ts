 ...
 */
export class AgentHostSnapshotController
	extends Disposable
	implements IChatEditingSession
{
	...
	readonly state: IObservable<ChatEditingSessionState> = constObservable(
		...
		derivedOpts(
			{
				equalsFn: (a, b) =>
					a.length === b.length &&
					a.every((v, i) => v.requestId === b[i].requestId),
			},
			(reader) => {
	...
	addToolCallEdits(requestId: string, tc: ToolCallState): void {
		...
		for (const edit of fileEdits) {
			...
			const entry: IToolCallFileEdit = {
				kind: edit.kind,
				resource,
				originalResource: edit.originalResource
					? toAgentHostUri(edit.originalResource, authority)
					: undefined,
				beforeContentUri: edit.beforeContentUri
					? toAgentHostUri(edit.beforeContentUri, authority)
					: undefined,
				afterContentUri: edit.afterContentUri
					? toAgentHostUri(edit.afterContentUri, authority)
					: undefined,
			...
			const existingIdx = cp.edits.findIndex(
				(e) => e.resource.toString() === resource.toString(),
			);
			if (existingIdx < 0) {
				cp.edits.push(entry);
			} else {
	...
	getSnapshotUri(
		...
		if (
			!cp ||
			!cp.edits.some((e) => e.resource.toString() === uri.toString())
		) {
			return undefined;
		}
		return URI.from({
	...
	async getSnapshotContents(
		...
		try {
			...
			this._logService.warn(
				`[AgentHostSnapshotController] Failed to fetch snapshot content`,
				err,
			);
			return undefined;
		}
	...
	async getSnapshotModel(
		_requestId: string,
		_undoStop: string | undefined,
		_snapshotUri: URI,
	...
	getEntryDiffBetweenRequests(
		_uri: URI,
		_startRequestId: string,
		_stopRequestId: string,
	...
	startStreamingEdits(
		_resource: URI,
		_responseModel: IChatResponseModel,
		_inUndoStop: string | undefined,
	): IStreamingEdits {
		throw new Error("Not supported for agent host sessions");
	}
	applyWorkspaceEdit(
		_edit: IChatWorkspaceEdit,
		_responseModel: IChatResponseModel,
		_undoStopId: string,
	): void {
		throw new Error("Not supported for agent host sessions");
	...
	private async _writeCheckpointContent(
		checkpoint: IAgentHostCheckpoint,
					...
					switch (edit.kind) {
						...
						case FileEditKind.Delete:
							if (edit.beforeContentUri) {
								const content = await this._fileService.readFile(
 ...
 */
function mergeFileEdit(
	prev: IToolCallFileEdit,
	next: IToolCallFileEdit,
): IToolCallFileEdit {
