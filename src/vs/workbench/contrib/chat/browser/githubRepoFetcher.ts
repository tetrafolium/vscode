...
async function loggedRequest(
	...
	try {
		...
		throw new Error(
			`Network error during ${options.callSite} (GET ${options.url}): ${reason}`,
...
export async function resolveGitHubRefToSha(
	...
	const ctx = await loggedRequest(
		requestService,
		{ url, headers, callSite: "pluginGit.resolveSha" },
		token,
	);
	...
	if (status === 403 && isRateLimited(ctx.res.headers)) {
		throw new GitHubRateLimitError(
			`GitHub rate limit hit resolving ref '${ref ?? "HEAD"}' on ${repo.owner}/${repo.repo}`,
			retryAfterFromHeaders(ctx.res.headers),
		);
	}
	if (status === 401 || status === 403) {
		throw new GitHubAuthRequiredError(
			`GitHub returned ${status} resolving ref '${ref ?? "HEAD"}' on ${repo.owner}/${repo.repo}`,
		);
	}
	if (status === 404) {
		throw new Error(
			`GitHub repository or ref not found: ${repo.owner}/${repo.repo}@${ref ?? "HEAD"}`,
		);
	}
	if (!isSuccess(ctx)) {
		throw new Error(
			`GitHub returned ${status}${isClientError(ctx) ? " (client error)" : ""} resolving ref for ${repo.owner}/${repo.repo}`,
		);
	}
	const body = await asJson<IGitHubCommitResponse>(ctx);
	if (!body || typeof body.sha !== "string") {
		throw new Error(
			`GitHub commit response for ${repo.owner}/${repo.repo} missing 'sha' field`,
		);
	}
	return body.sha;
}
...
export async function fetchAndExtractGitHubRepo(
	...
	if (tree.truncated) {
		...
		logService.warn(
			`[GitHubRepoFetcher] Tree for ${repo.owner}/${repo.repo}@${sha} is truncated; some files will be missing from the install`,
		);
	}
	...
	try {
		...
		for (const entry of tree.tree) {
			...
			if (entry.type === "commit") {
				logService.trace(
					`[GitHubRepoFetcher] Skipping submodule entry ${entry.path}`,
				);
				continue;
			}
			if (entry.mode === "120000") {
				logService.trace(
					`[GitHubRepoFetcher] Skipping symlink entry ${entry.path}`,
				);
				continue;
			}
			...
			if (!dest) {
				logService.warn(
					`[GitHubRepoFetcher] Skipping unsafe tree entry path: ${entry.path}`,
				);
				continue;
			}
			if (entry.type === "tree") {
			...
			if (entry.type !== "blob") {
				logService.trace(
					`[GitHubRepoFetcher] Skipping tree entry with unsupported type '${entry.type}': ${entry.path}`,
				);
				continue;
			}
			...
			if (
				parent.toString() !== dest.toString() &&
				!createdDirs.has(parent.toString())
			) {
		...
		await Promise.all(
			blobsToFetch.map(({ entry, dest }) =>
				limiter.queue(async () => {
					if (token.isCancellationRequested) {
						throw new CancellationError();
					}
					const content = await fetchGitHubBlob(
						requestService,
						repo,
						sha,
						entry,
		...
		try {
			...
			logService.warn(
				`[GitHubRepoFetcher] Failed to clean up staging dir ${stagingDir.toString()}:`,
				cleanupErr,
			);
		}
		throw err;
...
async function fetchGitHubTree(
	...
	const ctx = await loggedRequest(
		requestService,
		{ url, headers, callSite: "pluginGit.tree" },
		token,
	);
	if (token.isCancellationRequested) {
		throw new CancellationError();
	...
	if (status === 403 && isRateLimited(ctx.res.headers)) {
		throw new GitHubRateLimitError(
			`GitHub rate limit hit fetching tree for ${repo.owner}/${repo.repo}@${sha}`,
			retryAfterFromHeaders(ctx.res.headers),
		);
	}
	if (status === 401 || status === 403) {
		throw new GitHubAuthRequiredError(
			`GitHub returned ${status} fetching tree for ${repo.owner}/${repo.repo}@${sha}`,
		);
	}
	if (status === 404) {
		throw new Error(
			`GitHub repository or commit not found: ${repo.owner}/${repo.repo}@${sha}`,
		);
	}
	if (!isSuccess(ctx)) {
		throw new Error(
			`GitHub returned ${status}${isClientError(ctx) ? " (client error)" : ""} fetching tree for ${repo.owner}/${repo.repo}@${sha}`,
		);
	}
	const body = await asJson<IGitHubTreeResponse>(ctx);
	if (!body || !Array.isArray(body.tree)) {
		throw new Error(
			`GitHub tree response for ${repo.owner}/${repo.repo}@${sha} missing 'tree' array`,
		);
	}
	return body;
}
...
async function fetchGitHubBlob(
	...
	const ctx = await loggedRequest(
		requestService,
		{ url, headers, callSite: "pluginGit.blob" },
		token,
	);
	if (token.isCancellationRequested) {
		throw new CancellationError();
	...
	if (status === 403 && isRateLimited(ctx.res.headers)) {
		throw new GitHubRateLimitError(
			`GitHub rate limit hit fetching blob '${entry.path}' for ${repo.owner}/${repo.repo}@${commitSha}`,
			retryAfterFromHeaders(ctx.res.headers),
		);
	}
	if (status === 401 || status === 403) {
		throw new GitHubAuthRequiredError(
			`GitHub returned ${status} fetching blob '${entry.path}' for ${repo.owner}/${repo.repo}@${commitSha}`,
		);
	}
	if (!isSuccess(ctx)) {
		throw new Error(
	...
	if (!body || typeof body.content !== "string") {
		throw new Error(
			`GitHub blob response for '${entry.path}' missing 'content' field`,
		);
	}
	if (body.encoding !== "base64") {
		throw new Error(
			`GitHub blob response for '${entry.path}' has unsupported encoding '${body.encoding}'`,
		);
	}
