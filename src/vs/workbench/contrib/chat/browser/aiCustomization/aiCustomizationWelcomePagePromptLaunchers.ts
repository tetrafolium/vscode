...
import type {
	...
	IWelcomePageCallbacks,
} from "./aiCustomizationWelcomePage.js";
...
export class PromptLaunchersAICustomizationWelcomePage
	extends Disposable
	implements IAICustomizationWelcomePageImplementation
{
	...
	constructor(
		...
		const resizeObserver = this._register(
			new DOM.DisposableResizeObserver(
				"AICustomizationWelcomePagePromptLaunchers.scrollable",
				() => this.scrollable.scanDomNode(),
			),
		);
		this._register(resizeObserver.observe(scrollableNode));

		const welcomeInner = DOM.append(
			this.container,
			$(".welcome-prompts-inner"),
		);
		...
		if (this.welcomePageFeatures?.showGettingStartedBanner !== false) {
			const gettingStarted = DOM.append(
				welcomeInner,
				$(".welcome-prompts-primary"),
			);
			const header = DOM.append(
				gettingStarted,
				$(".welcome-prompts-section-label"),
			);
			const icon = DOM.append(
				header,
				$("span.welcome-prompts-section-label-icon.codicon.codicon-sparkle"),
			);
			...
			const description = DOM.append(
				gettingStarted,
				$("p.welcome-prompts-input-helper"),
			);
			...
			const inputRow = DOM.append(
				gettingStarted,
				$(".welcome-prompts-input-row"),
			);
			this.inputRow = inputRow;
			this.inputElement = DOM.append(
				inputRow,
				$("input.welcome-prompts-input"),
			) as HTMLInputElement;
			...
			const submitBtn = DOM.append(
				inputRow,
				$("button.welcome-prompts-input-submit"),
			);
			this.submitBtn = submitBtn;
			submitBtn.setAttribute(
				"aria-label",
				localize("workflowSubmitAriaLabel", "Customize agent"),
			);
			this._register(
				this.hoverService.setupManagedHover(
					getDefaultHoverDelegate("element"),
					submitBtn,
					localize("workflowSubmitTooltip", "Open in Chat"),
				),
			);
			...
			const updateSubmitState = () => {
				...
				submitBtn.classList.toggle(
					"welcome-prompts-input-submit-disabled",
					!hasValue,
				);
			};
			...
			const submit = () => {
				...
				this.sentLabel = DOM.append(
					inputRow,
					$("span.welcome-prompts-sent-label"),
				);
			...
			this._register(
				DOM.addDisposableListener(submitBtn, "click", (e) => {
					e.stopPropagation();
					submit();
				}),
			);
			...
			this._register(
				DOM.addDisposableListener(this.inputElement, "input", () => {
					updateSubmitState();
	...
	rebuildCards(
		...
		for (const category of this.categoryDescriptions) {
			...
			const labelEl = DOM.append(
				cardHeader,
				$("span.welcome-prompts-card-label"),
			);
			...
			if (category.promptType) {
				const generateBtn = DOM.append(
					footer,
					$("button.welcome-prompts-card-action"),
				);
				generateBtn.textContent = localize("new", "New...");
				generateBtn.setAttribute(
					"aria-label",
					localize("newCategoryAriaLabel", "New {0}...", category.label),
				);
				this.cardDisposables.add(
					DOM.addDisposableListener(generateBtn, "click", (e) => {
						e.stopPropagation();
						this.callbacks.closeEditor();
						if (this.workspaceService.isSessionsWindow) {
							const typeLabel = category.label.toLowerCase().replace(/s$/, "");
							this.callbacks.prefillChat(
								`Create me a custom ${typeLabel} that `,
								{ isPartialQuery: true, newChat: true },
							);
						} else {
				...
				browseBtn.setAttribute(
					"aria-label",
					localize("browseCategoryAriaLabel", "Browse {0}...", category.label),
				);
				this.cardDisposables.add(
					DOM.addDisposableListener(browseBtn, "click", (e) => {
						e.stopPropagation();
			...
			this.cardDisposables.add(
				DOM.addDisposableListener(card, "click", () => {
					this.callbacks.selectSection(category.id);
				}),
			);
			this.cardDisposables.add(
				DOM.addDisposableListener(card, "keydown", (e) => {
