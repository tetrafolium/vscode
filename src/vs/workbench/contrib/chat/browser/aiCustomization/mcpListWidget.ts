...
function isCopilotExtension(id: ExtensionIdentifier): boolean {
	return COPILOT_EXTENSION_IDS.some((copilotId) =>
		ExtensionIdentifier.equals(id, copilotId),
	);
}

function getPluginUriFromCollectionId(
	collectionId: string | undefined,
): string | undefined {
	return collectionId?.startsWith(PLUGIN_COLLECTION_PREFIX)
		? collectionId.slice(PLUGIN_COLLECTION_PREFIX.length)
		: undefined;
}
...
type IMcpListEntry =
	| IMcpGroupHeaderEntry
	| IMcpServerItemEntry
	| IMcpBuiltinItemEntry;
...
class McpServerItemDelegate implements IListVirtualDelegate<IMcpListEntry> {
	getHeight(element: IMcpListEntry): number {
		...
		if (
			element.type === "server-item" &&
			element.server.gallery &&
 ...
 */
class McpServerItemRenderer implements IListRenderer<
	IMcpServerItemEntry | IMcpBuiltinItemEntry,
	IMcpServerItemTemplateData
> {
	...
	renderElement(
		element: IMcpServerItemEntry | IMcpBuiltinItemEntry,
		index: number,
		templateData: IMcpServerItemTemplateData,
	): void {
			...
			if (pluginUriStr) {
				templateData.disposables.add(
					this.hoverService.setupDelayedHover(templateData.container, () => {
						const plugin = this.agentPluginService.plugins
							.get()
							.find((p) => p.uri.toString() === pluginUriStr);
						if (plugin) {
							return {
		...
		const server = this.mcpService.servers
			.get()
			.find((s) => s.definition.id === element.server.id);
		templateData.disposables.add(
			autorun((reader) => {
				const disabled = server
					? isContributionDisabled(server.enablement.read(reader))
					: false;
	...
	private updateStatus(
		...
		state: McpConnectionState.Kind | "disabled" | undefined,
	): void {
 ...
 */
class McpGalleryItemRenderer implements IListRenderer<
	IMcpServerItemEntry,
	IMcpGalleryItemTemplateData
> {
	...
	renderElement(
		element: IMcpServerItemEntry,
		_index: number,
		templateData: IMcpGalleryItemTemplateData,
	): void {
		templateData.elementDisposables.clear();

		templateData.name.textContent = element.server.label;
		templateData.publisher.textContent = element.server.publisherDisplayName
			? `by ${element.server.publisherDisplayName}`
			: "";
		...
		templateData.elementDisposables.add(
			this.mcpWorkbenchService.onChange((changed) => {
...
export class McpListWidget extends Disposable {
	...
	private readonly _onDidSelectServer = this._register(
		new Emitter<IWorkbenchMcpServer>(),
	);
	...
	private readonly _onDidRequestShowPlugin = this._register(
	...
	private create(): void {
		...
		const backButtonContainer = DOM.append(
			buttonContainer,
			$(".list-add-button-container"),
		);
		this.backButton = this._register(
			new Button(backButtonContainer, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
		...
		this._register(
			this.hoverService.setupManagedHover(
				...
				localize("addServerTooltip", "Add Server"),
			),
		);
		this._register(
			this.addButton.onDidClick(() => {
		...
		const groupHeaderRenderer =
			new CustomizationGroupHeaderRenderer<IMcpGroupHeaderEntry>(
				"mcpGroupHeader",
				this.hoverService,
			);
		const localRenderer = this.instantiationService.createInstance(
			McpServerItemRenderer,
		);
		const galleryRenderer = new McpGalleryItemRenderer(
			this.mcpWorkbenchService,
		);
				...
				[groupHeaderRenderer, localRenderer, galleryRenderer],
				{
					...
					accessibilityProvider: {
						getAriaLabel: (element: IMcpListEntry) => {
							...
							return element.type === "builtin-item"
								? element.label
		...
		this._register(
			this.list.onContextMenu((e) =>
				this.onContextMenu(e as IListContextMenuEvent<IMcpListEntry>),
			),
		);

		// Listen to MCP service changes
		this._register(
			this.mcpWorkbenchService.onChange(() => {
				if (!this.browseMode) {
					this.refresh();
				}
			}),
		...
		if (disabled) {
			...
			this.disabledIcon.classList.add(
				...ThemeIcon.asClassNameArray(
					policyLocked ? Codicon.shield : mcpServerIcon,
				),
			);
	...
	private updateGalleryList(): void {
		...
		const entries: IMcpListEntry[] = this.galleryServers.map((server) => ({
	...
	private filterServers(): void {
		...
		const builtinServers = this.mcpService.servers
			.get()
			.filter((s) => !localIds.has(s.definition.id))
			.filter(
				(s) => !query || s.definition.label.toLowerCase().includes(query),
			);
		...
		const collectionSources = new Map(
			this.mcpRegistry.collections.get().map((c) => [c.id, c.source]),
		);
		...
		for (const server of builtinServers) {
			...
			if (server.collection.id.startsWith(PLUGIN_COLLECTION_PREFIX)) {
				...
				source instanceof ExtensionIdentifier &&
				!isCopilotExtension(source)
			) {
		...
		if (pluginServers.length > 0) {
			...
			entries.push({
				...
				description: localize(
					...
					"MCP servers provided by installed plugins.",
				),
				collapsed,
			});
			if (!collapsed) {
	...
	private onContextMenu(e: IListContextMenuEvent<IMcpListEntry>): void {
		...
		if (e.element.type === "builtin-item") {
			...
			const showPluginAction = disposables.add(
				new Action(
			...
			const uninstallAction = disposables.add(
				new Action(
		...
		const disposables = new DisposableStore();
		const mcpServer =
			this.mcpWorkbenchService.local.find(
				(local) => local.id === serverEntry.server.id,
			) || serverEntry.server;

		// Get context menu actions from the MCP module
		const groups: IAction[][] = getContextMenuActions(
			mcpServer,
			false,
