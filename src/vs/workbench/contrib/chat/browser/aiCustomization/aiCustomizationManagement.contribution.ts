...
type CustomizationEditorDeleteItemClassification = {
	promptType: {
		...
		comment: "The type of customization being deleted.";
	};
	storage: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The storage location of the deleted item.";
	};
	owner: "joshspicer";
...
Registry.as<IEditorPaneRegistry>(
	...
	[
		...
		new SyncDescriptor(
			AICustomizationManagementEditorInput as unknown as {
...
function extractSource(
	context: AICustomizationContext,
): AICustomizationSource | undefined {
 ...
 */
function extractPromptType(
	context: AICustomizationContext,
): PromptsType | undefined {
	if (URI.isUri(context) || typeof context === "string") {
		return undefined;
	}
 ...
 */
function parseHookItemId(
	itemId: string,
): { originalId: string; index: number } | undefined {
			...
			if (
				...
				codeEditor.updateOptions({
					...
					readOnlyMessage: new MarkdownString(
						localize(
							"readonlyPluginFile",
							"This file is provided by a plugin or extension and cannot be edited.",
						),
					),
				});
			}
	...
	class extends Action2 {
		...
		async run(
			...
			if (source === AICustomizationSources.plugin) {
				...
				const plugin = agentPluginService.plugins
					.get()
					.find((p) => isEqualOrParent(uri, p.uri));
				if (plugin) {
					const result = await dialogService.confirm({
						message: localize(
							"cannotDeletePluginItem",
							"This item is provided by the plugin '{0}'",
							plugin.label,
						),
						detail: localize(
							"cannotDeletePluginItemDetail",
							"Individual components from a plugin cannot be removed separately. Would you like to uninstall the entire plugin?",
						),
			...
			const hookName =
				typeof context !== "string" && !URI.isUri(context)
					? context.name
					: undefined;
			const message = isSkill
				...
				: hookInfo && hookName
					? localize(
							"confirmDeleteHook",
							"Are you sure you want to delete the '{0}' hook?",
							hookName,
						)
					: localize(
							"confirmDelete",
			...
			if (confirmation.confirmed) {
				try {
					telemetryService.publicLog2<
						CustomizationEditorDeleteItemEvent,
						CustomizationEditorDeleteItemClassification
					>("chatCustomizationEditor.deleteItem", {
				...
				if (hookInfo) {
					try {
						...
						const edits = removeProperty(
							text,
							["hooks", hookInfo.originalId, hookInfo.index],
						...
						await dialogService.error(
							localize(
								"deleteHookItemFailed",
								"Unable to delete this hook entry because the file contents have changed.",
							),
							localize(
								"deleteHookItemFailedDetail",
				...
				const useTrash = fileService.hasCapability(
					deleteTarget,
...
const INSTALL_CHAT_CUSTOMIZATION_EXTENSION_ID =
	"aiCustomizationManagement.installChatCustomizationExtension";
const CHAT_CUSTOMIZATION_EXTENSION_ID =
	"ms-vscode.vscode-chat-customizations-evaluations";
const CHAT_CUSTOMIZATION_EXTENSION_NOT_INSTALLED_CONTEXT =
	new RawContextKey<boolean>("chat.customizationExtensionNotInstalled", false);
const CHAT_CUSTOMIZATION_EXTENSION_NOT_INSTALLED =
	CHAT_CUSTOMIZATION_EXTENSION_NOT_INSTALLED_CONTEXT.isEqualTo(true);
registerAction2(
	class extends Action2 {
		constructor() {
			super({
...
const WHEN_ITEM_IS_PLUGIN = ContextKeyExpr.and(
	...
	ContextKeyExpr.regex(
		AI_CUSTOMIZATION_ITEM_PLUGIN_URI_KEY,
		new RegExp(`^${SYNCED_CUSTOMIZATION_SCHEME}:`),
	).negate(),
);
...
MenuRegistry.appendMenuItem(AICustomizationManagementItemMenuId, {
	command: {
		id: RUN_PROMPT_MGMT_ID,
		title: localize("runPrompt", "Run Prompt"),
		icon: Codicon.play,
	},
	group: "2_run",
	order: 1,
...
MenuRegistry.appendMenuItem(AICustomizationManagementItemMenuId, {
	...
	when: ContextKeyExpr.or(
		ContextKeyExpr.regex(
			AI_CUSTOMIZATION_ITEM_URI_KEY,
			new RegExp(`^${Schemas.file}:`),
		),
		ContextKeyExpr.regex(
			AI_CUSTOMIZATION_ITEM_URI_KEY,
			new RegExp(`^${Schemas.vscodeUserData}:`),
		),
	),
});
...
const UNINSTALL_PLUGIN_AI_CUSTOMIZATION_ID =
	"aiCustomizationManagement.uninstallPlugin";
registerAction2(
	class extends Action2 {
		constructor() {
			super({
...
MenuRegistry.appendMenuItem(AICustomizationManagementItemMenuId, {
	command: {
		id: UNINSTALL_PLUGIN_AI_CUSTOMIZATION_ID,
		title: localize("uninstallPlugin", "Uninstall Plugin"),
		icon: Codicon.trash,
	},
	group: "inline",
	order: 10,
...
registerAction2(
	class extends Action2 {
		...
		async run(
			...
			const plugin = agentPluginService.plugins
				.get()
				.find((p) => p.uri.toString() === pluginUri.toString());
			if (!plugin) {
				return;
			}
...
MenuRegistry.appendMenuItem(AICustomizationManagementItemMenuId, {
	command: {
		id: SHOW_PLUGIN_AI_CUSTOMIZATION_ID,
		title: localize("showPlugin", "Show Plugin"),
	},
	group: "1_open",
	order: 2,
	when: WHEN_ITEM_IS_PLUGIN,
...
// Disable item action
const DISABLE_AI_CUSTOMIZATION_MGMT_ITEM_ID =
	"aiCustomizationManagement.disableItem";
registerAction2(
	class extends Action2 {
		constructor() {
			super({
...
// Enable item action
const ENABLE_AI_CUSTOMIZATION_MGMT_ITEM_ID =
	"aiCustomizationManagement.enableItem";
registerAction2(
	class extends Action2 {
		constructor() {
			super({
...
MenuRegistry.appendMenuItem(AICustomizationManagementItemMenuId, {
	command: {
		id: DISABLE_AI_CUSTOMIZATION_MGMT_ITEM_ID,
		title: localize("disable", "Disable"),
	},
	group: "5_toggle",
	order: 1,
	when: ContextKeyExpr.and(
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		super();
		this.chatCustomizationExtensionNotInstalledContext =
			CHAT_CUSTOMIZATION_EXTENSION_NOT_INSTALLED_CONTEXT.bindTo(
				contextKeyService,
			);

		const refreshExtensionContext = () =>
			...
			registerAction2(
				class extends Action2 {
					...
					async run(
						...
						const input = AICustomizationManagementEditorInput.getOrCreate();
						const pane = await editorService.openEditor(input, {
							pinned: true,
						});
