...
export async function generateCustomizationDebugReport(
	...
	lines.push(
		`Window: ${workspaceService.isSessionsWindow ? "Sessions" : "Core VS Code"}`,
	);
	lines.push(
		`Active root: ${workspaceService.getActiveProjectRoot()?.fsPath ?? "(none)"}`,
	);
	...
	if (activeDescriptor) {
		...
		lines.push(
			`  hiddenSections: ${activeDescriptor.hiddenSections ? `[${activeDescriptor.hiddenSections.join(", ")}]` : "(none)"}`,
		);
		lines.push(
			`  workspaceSubpaths: ${activeDescriptor.workspaceSubpaths ? `[${activeDescriptor.workspaceSubpaths.join(", ")}]` : "(none)"}`,
		);
		lines.push(
			`  hideGenerateButton: ${activeDescriptor.hideGenerateButton ?? false}`,
		);
		lines.push(
			`  requiredAgentId: ${activeDescriptor.requiredAgentId ?? "(none)"}`,
		);
		lines.push(
			`  instructionFileFilter: ${activeDescriptor.instructionFileFilter ? `[${activeDescriptor.instructionFileFilter.join(", ")}]` : "(none)"}`,
		);
	}
	lines.push("");
...
async function getPromptFilesByStorage(
	promptsService: IPromptsService,
	promptType: PromptsType,
): Promise<IPromptFilesByStorage> {
	const [localFiles, userFiles, extensionFiles] = await Promise.all([
		promptsService.listPromptFilesForStorage(
			promptType,
...
async function appendProviderData(
	lines: string[],
	itemSource: IAICustomizationItemSource,
	promptType: PromptsType,
	label: string,
	...
	if (allItems.length === 0) {
		lines.push(
			`  Total items from provider: 0 (or provider returned undefined and the item source normalized it to an empty array)`,
		);
	} else {
	...
	for (const [type, items] of byType) {
		...
		for (const item of items) {
			const path =
				item.uri.scheme === "file" ? item.uri.fsPath : item.uri.toString();
			...
			if (item.status) {
				lines.push(
					`      status: ${item.status}${item.statusMessage ? ` (${item.statusMessage})` : ""}`,
				);
			}
	...
	lines.push(
		`  Items matching current section (${promptType}): ${sectionItems.length}`,
	);
	lines.push("");
}

async function appendRawServiceData(
	lines: string[],
	promptsService: IPromptsService,
	promptType: PromptsType,
): Promise<void> {
	...
	const { localFiles, userFiles, extensionFiles } =
		await getPromptFilesByStorage(promptsService, promptType);
	...
	lines.push(
		`  listPromptFilesForStorage(ext):    ${extensionFiles.length} files`,
	);
	appendFileList(lines, extensionFiles);

	const allFiles = await promptsService.listPromptFiles(
		promptType,
		CancellationToken.None,
	);
	...
	if (promptType === PromptsType.instructions) {
		const agentInstructions = await promptsService.listAgentInstructions(
			CancellationToken.None,
			undefined,
		);
		lines.push(
			`  listAgentInstructions (extra):     ${agentInstructions.length} files`,
		);
	...
	if (promptType === PromptsType.skill) {
		...
		lines.push(
			`  findAgentSkills:                   ${skills?.length ?? 0} skills`,
		);
	...
	if (promptType === PromptsType.prompt) {
		const commands = await promptsService.getPromptSlashCommands(
			CancellationToken.None,
		);
		lines.push(
			`  getPromptSlashCommands:            ${commands.length} commands`,
		);
		for (const c of commands) {
			lines.push(
				`    /${c.name} [${c.storage}] ${c.uri.fsPath} (type=${c.type})`,
			);
		}
	}
...
async function appendFilteredData(
	lines: string[],
	promptsService: IPromptsService,
	promptType: PromptsType,
	...
	lines.push(
		`    local:     ${filtered.filter((f) => f.storage === PromptsStorage.local).length}`,
	);
	lines.push(
		`    user:      ${filtered.filter((f) => f.storage === PromptsStorage.user).length}`,
	);
	lines.push(
		`    extension: ${filtered.filter((f) => f.storage === PromptsStorage.extension).length}`,
	);
...
function appendWidgetState(lines: string[], state: IDebugWidgetState): void {
	...
	lines.push(
		`    local:     ${state.allItems.filter((i) => i.source === AICustomizationSources.local).length}`,
	);
	lines.push(
		`    user:      ${state.allItems.filter((i) => i.source === AICustomizationSources.user).length}`,
	);
	lines.push(
		`    extension: ${state.allItems.filter((i) => i.source === AICustomizationSources.extension).length}`,
	);
	lines.push(
		`    plugin:    ${state.allItems.filter((i) => i.source === AICustomizationSources.plugin).length}`,
	);
	lines.push(
		`    built-in:  ${state.allItems.filter((i) => i.source === AICustomizationSources.builtin).length}`,
	);
	...
	lines.push(
		`  displayEntries (after filterItems): ${state.displayEntries.length}`,
	);
	const fileEntries = state.displayEntries.filter(
		(e) => e.type === "file-item",
	);
	lines.push(`    file items shown: ${fileEntries.length}`);
	const groupEntries = state.displayEntries.filter(
		(e) => e.type === "group-header",
	);
	for (const g of groupEntries) {
		lines.push(
			`    group "${g.label}": count=${g.count}, collapsed=${g.collapsed}`,
		);
	}
	lines.push("");
}

async function appendSourceFolders(
	lines: string[],
	promptsService: IPromptsService,
	promptType: PromptsType,
): Promise<void> {
	...
	try {
		const resolvedFolders =
			await promptsService.getResolvedSourceFolders(promptType);
		lines.push("");
...
function appendAllHarnesses(
	...
	for (const h of harnesses) {
		...
		lines.push(
			`    hiddenSections: ${h.hiddenSections ? `[${h.hiddenSections.join(", ")}]` : "(none)"}`,
		);
...
function appendInstalledPlugins(
	lines: string[],
	agentPluginService: IAgentPluginService,
): void {
	...
	for (const p of plugins) {
		...
		if (p.fromMarketplace) {
			...
			lines.push(
				`    fromMarketplace: ${m.name}@${m.version} (marketplace=${m.marketplace}, type=${m.marketplaceType})`,
			);
		} else {
