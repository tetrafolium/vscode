...
import {
	formatDisplayName,
	truncateToFirstLine,
} from "./aiCustomizationListWidget.js";
import { ILabelService } from "../../../../../platform/label/common/label.js";
import {
	CustomizationGroupHeaderRenderer,
	ICustomizationGroupHeaderEntry,
	CUSTOMIZATION_GROUP_HEADER_HEIGHT,
	CUSTOMIZATION_GROUP_HEADER_HEIGHT_WITH_SEPARATOR,
} from "./customizationGroupHeaderRenderer.js";
import {
	ICustomizationHarnessService,
	isPluginCustomizationItem,
	type ICustomizationItem,
	type ICustomizationItemAction,
...
type IPluginListEntry =
	| IPluginGroupHeaderEntry
	| IPluginInstalledItemEntry
	| IPluginMarketplaceItemEntry
	| IPluginRemoteItemEntry;
...
class PluginInstalledItemRenderer implements IListRenderer<
	IPluginInstalledItemEntry,
	IPluginInstalledItemTemplateData
> {
	readonly templateId = "pluginInstalledItem";

	constructor(private readonly _harnessService: ICustomizationHarnessService) {}

	renderTemplate(container: HTMLElement): IPluginInstalledItemTemplateData {
		container.classList.add("mcp-server-item");

		const syncCheckboxContainer = DOM.append(
			container,
			$(".item-sync-checkbox"),
		);
	...
	renderElement(
		element: IPluginInstalledItemEntry,
		_index: number,
		templateData: IPluginInstalledItemTemplateData,
	): void {
		...
		if (element.item.description) {
			templateData.description.textContent = truncateToFirstLine(
		...
		templateData.disposables.add(
			autorun((reader) => {
				const enabled = isContributionEnabled(
					element.item.plugin.enablement.read(reader),
				);
		...
		const syncProvider =
			this._harnessService.getActiveDescriptor().syncProvider;
		if (syncProvider) {
...
class PluginRemoteItemRenderer implements IListRenderer<
	IPluginRemoteItemEntry,
	IPluginRemoteItemTemplateData
> {
	...
	renderElement(
		element: IPluginRemoteItemEntry,
		_index: number,
		templateData: IPluginRemoteItemTemplateData,
	): void {
		templateData.name.textContent = formatDisplayName(element.item.name);
...
class PluginMarketplaceItemRenderer implements IListRenderer<
	IPluginMarketplaceItemEntry,
...
> {
	...
	renderTemplate(container: HTMLElement): IPluginMarketplaceItemTemplateData {
		...
		const publisher = DOM.append(
			publisherContainer,
			$("span.publisher-name.mcp-gallery-publisher"),
		);
	...
	renderElement(
		element: IPluginMarketplaceItemEntry,
		_index: number,
		templateData: IPluginMarketplaceItemTemplateData,
	): void {
		templateData.elementDisposables.clear();

		templateData.name.textContent = element.item.name;
		templateData.publisher.textContent = element.item.marketplace
			? localize("byPublisher", "by {0}", element.item.marketplace)
			: "";
		...
		const isAlreadyInstalled = this.agentPluginService.plugins
			.get()
			.some((p) => isEqual(p.uri, installUri));
		...
		templateData.elementDisposables.add(
			templateData.installButton.onDidClick(async () => {
...
function installedPluginToItem(
	plugin: IAgentPlugin,
	...
	const description =
		...
		labelService.getUriLabel(dirname(plugin.uri), { relative: true });
...
export class PluginListWidget extends Disposable {
	...
	private readonly _onDidSelectPlugin = this._register(
		new Emitter<IAgentPluginItem>(),
	);
	...
	private readonly disabledLinkListener = this._register(
	...
	private create(): void {
		...
		this.sectionTitleHeader = DOM.append(
			this.element,
			$(".section-title-header"),
		);
		const titleRow = DOM.append(
			this.sectionTitleHeader,
			$(".section-title-row"),
		);
		const sectionTitle = DOM.append(titleRow, $("h2.section-title"));
		sectionTitle.textContent = localize("plugins", "Plugins");
		const sectionTitleDescription = DOM.append(
			this.sectionTitleHeader,
			$("p.section-title-description"),
		);
		const sectionTitleDescriptionText = DOM.append(
			sectionTitleDescription,
			$("span.section-title-description-text"),
		);
		...
		this._register(
			DOM.addDisposableListener(this.sectionLink, "click", (e) => {
				e.preventDefault();
		...
		this.searchAndButtonContainer = DOM.append(
			this.element,
		...
		this.buttonContainer = DOM.append(
			this.searchAndButtonContainer,
			$(".list-button-group"),
		);

		const browseButtonContainer = DOM.append(
		...
		this.browseButton = this._register(
			new Button(browseButtonContainer, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
				title: browseMarketplaceLabel,
				ariaLabel: browseMarketplaceLabel,
			}),
		);
		this.browseButton.element.classList.add("list-add-button");
		this._register(
			this.browseButton.onDidClick(() => this.runPrimaryButtonAction()),
		);
		...
		this.addButtonSimple = this._register(
			new Button(this.addButtonContainer, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
		...
		this._register(
			this.addButtonSimple.onDidClick(() => this.runPrimaryAddAction()),
		);

		this.addButton = this._register(
			new ButtonWithDropdown(this.addButtonContainer, {
				...defaultButtonStyles,
				secondary: true,
				supportIcons: true,
		...
		const disabledHeader = DOM.append(
		...
		this.disabledMessage = DOM.append(
			this.disabledContainer,
			$(".empty-subtext"),
		);
		...
		const groupHeaderRenderer =
			new CustomizationGroupHeaderRenderer<IPluginGroupHeaderEntry>(
				"pluginGroupHeader",
				this.hoverService,
			);
				...
				],
				{
						...
						getAriaLabel(element: IPluginListEntry) {
							if (element.type === "group-header") {
								return localize(
									...
									element.count,
									element.collapsed
										? localize("collapsed", "collapsed")
										: localize("expanded", "expanded"),
								);
							}
							const name = formatDisplayName(element.item.name);
							const description = element.item.description
								? truncateToFirstLine(element.item.description)
								: undefined;
							...
							if (element.type === "plugin-item") {
								const enabled = isContributionEnabled(
									element.item.plugin.enablement.get(),
								);
								return enabled
									? localize(
					...
					identityProvider: {
						getId(element: IPluginListEntry) {
							...
							if (element.type === "remote-item") {
								return (
									element.item.itemKey ??
									`remote-${element.item.groupKey ?? "default"}-${element.item.uri.toString()}`
								);
							}
		...
		this._register(
			this.list.onContextMenu((e) =>
				this.onContextMenu(e as IListContextMenuEvent<IPluginListEntry>),
			),
		);
		...
		this._register(
			this.pluginMarketplaceService.onDidChangeMarketplaces(() => {
				if (!this.browseMode) {
					void this.refresh();
				}
			}),
		...
		const itemProviderChangeDisposable = this._register(
			new MutableDisposable(),
		);
		this._register(
			autorun((reader) => {
				this.harnessService.activeHarness.read(reader);
				const itemProvider =
					this.harnessService.getActiveDescriptor().itemProvider;
				if (itemProvider) {
	...
	private updateAccessState(): void {
		const inspect = this.configurationService.inspect<boolean>(
		...
		if (disabled) {
			...
			this.disabledIcon.classList.add(
				...ThemeIcon.asClassNameArray(
					policyLocked ? Codicon.shield : pluginIcon,
				),
			);
			...
			if (policyLocked) {
				...
				this.disabledMessage.appendChild(
				...
				const link = DOM.append(
					this.disabledMessage,
					$("a.mcp-disabled-settings-link"),
				) as HTMLAnchorElement;
				...
				this.disabledLinkListener.value = DOM.addDisposableListener(
					link,
					"click",
					(e) => {
						e.preventDefault();
						this.commandService.executeCommand(
							...
							`@id:${ChatConfiguration.PluginsEnabled}`,
						);
					},
				);
			}
	...
	private formatActionLabel(
		action: ICustomizationItemAction,
		iconOnly = false,
	): string {
		if (!action.icon) {
			return action.label;
	...
	private updateToolbarActions(): void {
		...
		this.browseButton.element.parentElement!.style.display = this.browseMode
			? "none"
			: "";
	...
	private updateAddButton(): void {
		...
		if (hasDropdown) {
			...
			this.addButton.primaryButton.element.setAttribute(
				"aria-label",
				addPrimaryTitle,
			);
			...
			this.addButton.dropdownButton.element.setAttribute(
				"aria-label",
				moreLabel,
			);
		} else {
	...
	private getAddDropdownActions(): Action[] {
		...
		return this.buildAddActions()
			.slice(1)
			.map((action, index) =>
				this.addDropdownActions.add(
					new Action(
						`plugin_add_${index}`,
	...
	private toggleBrowseMode(browse: boolean): void {
		...
		this.browseButton.element.parentElement!.style.display = browse
			? "none"
			: "";
	...
	private async queryMarketplace(): Promise<void> {
		...
		try {
			const plugins =
				await this.pluginMarketplaceService.fetchMarketplacePlugins(cts.token);
			...
			const installedUris = new Set(
				this.agentPluginService.plugins.get().map((p) => p.uri.toString()),
			);
	...
	private updateMarketplaceList(): void {
		...
		const entries: IPluginListEntry[] = this.marketplaceItems.map((item) => ({
			type: "marketplace-item" as const,
			item,
		}));
		this.list.splice(0, this.list.length, entries);
	}

	private async getRemotePluginItems(
		query: string,
		...
		try {
			const provided = await this.itemsModel
				.getActiveItemSource()
				.fetchProviderItems();
	...
	private getRemoteGroupMetadata(groupKey: string | undefined): {
		group: string;
		label: string;
		description: string;
	} {
	...
	private appendGroup(
		entries: IPluginListEntry[],
		header: { group: string; label: string; description: string },
		items: readonly IPluginListEntry[],
		isFirst: boolean,
	): boolean {
		if (items.length === 0) {
			return isFirst;
	...
	private async filterPlugins(): Promise<void> {
		...
		const enabledPlugins = this.installedItems.filter((item) =>
			isContributionEnabled(item.plugin.enablement.get()),
		);
		const disabledPlugins = this.installedItems.filter(
			(item) => !isContributionEnabled(item.plugin.enablement.get()),
		);
		...
		const installedNames = new Set(
			this.installedItems.map((item) => item.name.toLowerCase()),
		);
		...
		for (const [groupKey, items] of remoteGroups) {
			isFirst = this.appendGroup(
				entries,
				this.getRemoteGroupMetadata(groupKey),
				items,
				isFirst,
			);
		}
	...
	private onContextMenu(e: IListContextMenuEvent<IPluginListEntry>): void {
		if (
			!e.element ||
			e.element.type === "group-header" ||
			e.element.type === "marketplace-item"
		) {
			return;
		}
		...
		if (entry.type === "plugin-item") {
			...
			if (
				actions.length > 0 &&
				actions[actions.length - 1] instanceof Separator
			) {
				actions.pop();
			}
		} else {
			...
			for (const itemAction of itemActions) {
				actions.push(
					new Action(
						...
						itemAction.label,
						itemAction.icon
							? ThemeIcon.asClassName(itemAction.icon)
							: undefined,
