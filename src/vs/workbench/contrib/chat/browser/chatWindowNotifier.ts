 ...
 */
export class ChatWindowNotifier
	extends Disposable
	implements IWorkbenchContribution
{
	static readonly ID = "workbench.contrib.chatWindowNotifier";

	private readonly _activeNotifications = this._register(
	...
	private async _notifyIfNeeded(
		sessionResource: URI,
		info: IChatRequestNeedsInputInfo,
	): Promise<void> {
		// Check configuration
		...
		const notificationTitle = info.title
			? localize("chatTitle", "Session: {0}", info.title)
			: localize("chat.untitledChat", "Untitled Session");
		...
		try {
			...
			const result = await this._hostService.showToast(
				{
					...
					body: this._getNotificationBody(
						sessionResource,
						info,
						isQuestionCarousel,
					),
			...
			if (
				...
				!isQuestionCarousel &&
				this._confirmAllow(sessionResource)
			) {
			...
			if (result.clicked || typeof result.actionIndex === "number") {
				...
				const widget =
					await this._chatWidgetService.openSession(sessionResource);
				widget?.focusInput();
			}
		} finally {
			this._clearNotification(sessionResource);
	...
	private _confirmAllow(sessionResource: URI): boolean {
		...
		for (const part of lastResponse.response.value) {
			const state =
				part.kind === "toolInvocation" ? part.state.get() : undefined;
			if (
