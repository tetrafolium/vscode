...
export class LanguageModelsConfigurationService
	extends Disposable
	implements ILanguageModelsConfigurationService
{
	...
	> = this._onDidChangeLanguageModelGroups.event;
	...
	constructor(
		...
		this.modelsConfigurationFile =
			userDataProfileService.currentProfile.languageModelsResource;
	...
	private setLanguageModelsConfiguration(
		languageModelsConfiguration: LanguageModelsProviderGroups,
	): void {
		const changedGroups: ILanguageModelsProviderGroup[] = [];
		const oldGroupMap = new Map(
			this.languageModelsProviderGroups.map((g) => [
				`${g.vendor}:${g.name}`,
				g,
			]),
		);
	...
	private async updateLanguageModelsConfiguration(): Promise<void> {
		const languageModelsProviderGroups =
			await this.withLanguageModelsProviderGroups();
	...
	async addLanguageModelsProviderGroup(
		...
		await this.withLanguageModelsProviderGroups(
			async (languageModelsProviderGroups) => {
				if (
					languageModelsProviderGroups.some(
						({ name, vendor }) =>
							name === toAdd.name && vendor === toAdd.vendor,
					)
				) {
					throw new Error(
						`Language model group with name ${toAdd.name} already exists for vendor ${toAdd.vendor}`,
					);
				}
	...
	async updateLanguageModelsProviderGroup(
	...
	async removeLanguageModelsProviderGroup(
		toRemove: ILanguageModelsProviderGroup,
	): Promise<void> {
		await this.withLanguageModelsProviderGroups(
			async (languageModelsProviderGroups) => {
				const result: LanguageModelsProviderGroups = [];
				for (const group of languageModelsProviderGroups) {
					if (
						group.name === toRemove.name &&
						group.vendor === toRemove.vendor
					) {
						continue;
					}
					result.push(group);
	...
	async configureLanguageModels(
		...
		const preferredGroup =
			this.editorGroupsService.getPart(this.editorGroupsService.activeGroup) ===
			this.editorGroupsService.activeModalEditorPart
				? MODAL_GROUP
				: undefined;
		...
		if (options.snippet) {
			...
			const targetRange =
				options.snippetTarget === "models"
					? options.group.modelsRange
					: options.group.range;
			if (!targetRange) {
				return;
			}
			...
			const isModelsArray =
				options.snippetTarget === "models" && Array.isArray(models);
			const emptyModelsArray = isModelsArray && models.length === 0;
			const insertBeforeModelsArrayEnd =
				emptyModelsArray ||
				(isModelsArray &&
	...
	private async withLanguageModelsProviderGroups(
		update?: (
			languageModelsProviderGroups: LanguageModelsProviderGroups,
		...
		try {
			const languageModelsProviderGroups =
				parseLanguageModelsProviderGroups(model);
			if (!update) {
				return languageModelsProviderGroups;
			}
...
export function parseLanguageModelsProviderGroups(
	model: ITextModel,
): LanguageModelsProviderGroups {
	...
	const visitor: JSONVisitor = {
		...
		onObjectEnd: (offset: number, length: number) => {
			const parent = currentParent as Record<string, unknown> & {
				range?: IRange;
				_parentConfigurationRange?: Mutable<IRange>;
			};
			if (parent.range) {
				const end = model.getPositionAt(offset + length);
				parent.range = {
		...
		onArrayEnd: (offset: number, length: number) => {
			const parent = currentParent as {
				_parentConfigurationRange?: Mutable<IRange>;
				_parentModelsRange?: Mutable<IRange>;
			};
			if (parent._parentConfigurationRange) {
				const end = model.getPositionAt(offset + length);
				parent._parentConfigurationRange.endLineNumber = end.lineNumber;
