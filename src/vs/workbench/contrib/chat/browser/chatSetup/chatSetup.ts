...
export type ChatSetupResultValue =
	| boolean /* success */
	| undefined /* canceled */;
...
export function refreshTokens(commandService: ICommandService): void {
	...
	commandService
		.executeCommand(defaultChat.chatRefreshTokenCommand)
		.catch(() => {
 ...
 */
export function buildUpgradeUrlWithRedirect(
	baseUpgradeUrl: string,
	urlProtocol: string,
	quality: string | undefined,
): string {
	const vscodeUri = `${urlProtocol}://${defaultChat.chatExtensionId}/upgrade-success`;
	const redirectHost =
		quality === "stable" ? "vscode.dev" : "insiders.vscode.dev";
...
export async function maybeEnableAuthExtension(
	...
	const providerExtension = extensionsWorkbenchService.local.find((e) =>
