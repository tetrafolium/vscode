...
const defaultChat = {
	...
	outputExtensionStateCommand:
		product.defaultChatAgent?.chatExtensionOutputExtensionStateCommand ?? "",
};
...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	static registerDefaultAgents(
		...
		return instantiationService.invokeFunction((accessor) => {
			...
			return SetupAgent.doRegisterAgent(
				...
				chatAgentService,
				id,
				`${defaultChat.provider.default.name} Copilot` /* Do NOT change, this hides the username altogether in Chat */,
				true,
				description,
				location,
				mode,
	...
	private static readonly CHAT_RETRY_COMMAND_ID =
		"workbench.action.chat.retrySetup";
	private static readonly CHAT_SHOW_OUTPUT_COMMAND_ID =
		"workbench.action.chat.showOutput";
	...
	private registerCommands(): void {
		...
		this._register(
			CommandsRegistry.registerCommand(
				...
				async (accessor, sessionResource: URI) => {
					...
					const widget =
						chatWidgetService.getWidgetBySessionResource(sessionResource);
					await widget?.clear();
		...
		this._register(
			CommandsRegistry.registerCommand(
				SetupAgent.CHAT_SHOW_OUTPUT_COMMAND_ID,
				async (accessor) => {
					...
					if (defaultChat.outputExtensionStateCommand) {
						...
						raceTimeout(
							commandService.executeCommand(
								defaultChat.outputExtensionStateCommand,
							),
							5000,
							() =>
								this.logService.info(
									"[chat setup] Timed out executing extension state command",
								),
						).then(undefined, (error) => {
							this.logService.info(
								"[chat setup] Failed to execute extension state command",
								error,
							);
						});
					}
					...
					if (defaultChat.outputChannelId) {
						await commandService.executeCommand(
							`workbench.action.output.show.${defaultChat.outputChannelId}`,
						);
					}
				},
			),
	...
	private async doInvokeWithoutSetup(
		...
		const requestModel = chatWidgetService
			.getWidgetBySessionResource(request.sessionResource)
		...
		progress({
			...
			content: new MarkdownString(
				localize("waitingChat", "Getting chat ready"),
			),
			shimmer: true,
		});
	...
	private async forwardRequestToChat(
	...
	private async doForwardRequestToChat(
		requestModel: IChatRequestModel,
		progress: (part: IChatProgress) => void,
		chatService: IChatService,
		languageModelsService: ILanguageModelsService,
		...
		if (
			this.pendingForwardedRequests.has(requestModel.session.sessionResource)
		) {
	...
	private async doForwardRequestToChatWhenReady(
		requestModel: IChatRequestModel,
		progress: (part: IChatProgress) => void,
		chatService: IChatService,
		languageModelsService: ILanguageModelsService,
		...
		if (
			...
			const timeoutHandle = setTimeout(() => {
				progress({
					kind: "progressMessage",
					content: new MarkdownString(
						localize("waitingChat2", "Chat is almost ready"),
					),
					shimmer: true,
				});
			}, 10000);
				...
				if (ready === "timedout") {
					...
					type ChatSetupTimeoutClassification = {
						...
						agentActivated: {
							...
							comment: "Whether the agent was activated.";
						};
						agentReady: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether the agent was ready.";
						};
						agentHasDefault: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether a default agent exists for the location and mode.";
						};
						agentDefaultIsCore: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether the default agent is a core agent.";
						};
						agentHasContributedDefault: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether a contributed default agent exists for the location.";
						};
						agentContributedDefaultIsCore: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether the contributed default agent is a core agent.";
						};
						agentActivatedCount: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							isMeasurement: true;
							comment: "Number of activated agents at timeout.";
						};
						agentLocation: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "The chat agent location.";
						};
						agentModeKind: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "The chat mode kind.";
						};
						languageModelReady: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether the language model was ready.";
						};
						languageModelCount: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							isMeasurement: true;
							comment: "Number of registered language models at timeout.";
						};
						languageModelDefaultCount: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							isMeasurement: true;
							comment: "Number of language models with isDefaultForLocation[Chat] set.";
						};
						languageModelHasRequestedModel: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether a specific model ID was requested.";
						};
						toolsModelReady: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether the tools model was ready.";
						};
						isRemote: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether this is a remote scenario.";
						};
						isAnonymous: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "Whether anonymous access is enabled.";
						};
						matchingWelcomeViewWhen: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							comment: "The when clause of the matching extension welcome view, if any.";
						};
					};
					...
					this.telemetryService.publicLog2<
						ChatSetupTimeoutEvent,
						ChatSetupTimeoutClassification
					>("chatSetup.timeout", diagnosticInfo);
					...
					if (
						...
						this.outputService.getChannelDescriptor(defaultChat.outputChannelId)
					) {
						progress({
							kind: "command",
							command: {
					...
					type ChatSetupTimeoutRecoveryClassification = {
						...
						agentActivatedCount: {
							...
							purpose: "FeatureInsight";
							isMeasurement: true;
							comment: "Number of activated agents at recovery time.";
						};
						agentLocation: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
						...
						languageModelCount: {
							...
							purpose: "FeatureInsight";
							isMeasurement: true;
							comment: "Number of registered language models at recovery time.";
						};
						languageModelDefaultCount: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
							isMeasurement: true;
							comment: "Number of language models with isDefaultForLocation[Chat] set at recovery time.";
						};
						languageModelHasRequestedModel: {
							classification: "SystemMetaData";
							purpose: "FeatureInsight";
					...
					this.telemetryService.publicLog2<
						ChatSetupTimeoutRecoveryEvent,
						ChatSetupTimeoutRecoveryClassification
					>("chatSetup.timeoutRecovery", recoveryDiagnosticInfo);
				}
			} finally {
				disposables.dispose();
			}
	...
	private async whenPanelAgentHasGuidance(
		...
		return new Promise<void>((resolve) => {
			...
			const onDidChangeRegistry = Event.map(
	...
	private whenLanguageModelReady(
		...
		return Event.toPromise(
			Event.filter(languageModelsService.onDidChangeLanguageModels, () =>
				hasModelForRequest(),
			),
		);
	}
	...
	private computeDiagnosticInfo(
		...
		languageModelsService: ILanguageModelsService,
		chatAgentService: IChatAgentService,
		modeInfo: { kind?: ChatModeKind } | undefined,
	) {
		...
		const chatViewPane = this.viewsService.getActiveViewWithId(ChatViewId) as
			| ChatViewPane
			| undefined;
		...
		return {
			...
			matchingWelcomeViewWhen:
				matchingWelcomeView?.when.serialize() ??
				(chatViewPane ? "noWelcomeView" : "noChatViewPane"),
		};
	}
	...
	private async doInvokeWithSetup(
				...
				switch (this.controller.value.step) {
						...
						progress({
							...
							content: new MarkdownString(
								localize(
									"setupChatSignIn2",
									"Signing in to {0}",
									defaultAccountService.getDefaultAccountAuthenticationProvider()
										.name,
								),
							),
							shimmer: true,
					...
					case ChatSetupStep.Installing:
						progress({
							kind: "progressMessage",
							content: new MarkdownString(
								localize("installingChat", "Getting chat ready"),
							),
							shimmer: true,
						});
						break;
		...
		try {
			result = await ChatSetup.getInstance(
				...
				forceAnonymous: this.chatEntitlementService.anonymous
					? ChatSetupAnonymous.EnabledWithoutDialog
					: undefined, // only enable anonymous selectively
			});
		} catch (error) {
			this.logService.error(
		...
		if (typeof result?.success === "boolean") {
			if (result.success) {
				...
				progress({
					kind: "warning",
					content: new MarkdownString(
						localize("chatSetupError", "Chat setup failed."),
					),
				});
			}
		}
...
export class AINewSymbolNamesProvider {
	...
	async provideNewSymbolNames(
		...
		await this.instantiationService.invokeFunction((accessor) => {
			return ChatSetup.getInstance(
				this.instantiationService,
				this.context,
				this.controller,
			).run({
				forceAnonymous: this.chatEntitlementService.anonymous
					? ChatSetupAnonymous.EnabledWithDialog
					: undefined,
			});
		});
...
export class AICodeActionsHelper {
	static warningOrErrorMarkersAtRange(
