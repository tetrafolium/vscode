...
} from "./chatSetupGrowthSession.js";
import {
	AICodeActionsHelper,
	AINewSymbolNamesProvider,
	ChatCodeActionsProvider,
	SetupAgent,
} from "./chatSetupProviders.js";
	...
	implements IWorkbenchContribution
{
	...
	private registerSetupAgents(
		...
		const codeActionsProviderDisposables = markAsSingleton(
		...
		const updateRegistration = () => {
			// Agent + Tools
			{
				if (!context.state.hidden && !context.state.disabledInWorkspace) {
					...
					if (!defaultAgentDisposables.value) {
						...
						// Panel Agents
						const panelAgentDisposables = disposables.add(
							new DisposableStore(),
						);
						for (const mode of [
							ChatModeKind.Ask,
								...
								agent.onUnresolvableError(() => {
									...
									if (panelAgentHasGuidance) {
										...
										this.logService.error(
											"[chat setup] Unresolvable error from Chat agent registration, clearing registration.",
										);
			...
			// Rename Provider
			{
				if (
					...
					!context.state.disabledInWorkspace
				) {
					if (!renameProviderDisposables.value) {
						renameProviderDisposables.value =
							AINewSymbolNamesProvider.registerProvider(
								this.instantiationService,
								context,
								controller,
							);
			...
			// Code Actions Provider
			{
				if (
					...
					if (!codeActionsProviderDisposables.value) {
						codeActionsProviderDisposables.value =
							ChatCodeActionsProvider.registerProvider(
								this.instantiationService,
							);
					}
				} else {
	...
	private registerGrowthSession(
		chatEntitlementService: ChatEntitlementService,
	): void {
		...
		const updateGrowthSession = () => {
			...
			const shouldShow =
				experimentEnabled && !chatEntitlementService.sentiment.completed;
			if (shouldShow && !growthSessionDisposables.value) {
				const disposables = new DisposableStore();
				const controller = disposables.add(
					this.instantiationService.createInstance(GrowthSessionController),
				);
				if (!controller.isDismissed) {
					disposables.add(
						registerGrowthSession(this.chatSessionsService, controller),
					);
		...
		this._register(
			chatEntitlementService.onDidChangeSentiment(() => updateGrowthSession()),
		);
		updateGrowthSession();
	}
	...
	private registerActions(
		...
		class ChatSetupTriggerAction extends Action2 {
			static CHAT_SETUP_ACTION_LABEL = localize2(
				"triggerChatSetup",
			...
			override async run(
				...
				options?: {
					...
					dialogTitle?: string;
					setupStrategy?: ChatSetupStrategy;
			...
			private resolveAgentId(
				...
				const agentByName = allAgents.find(
					(agent) => agent.name.get().toLowerCase() === nameLower,
				);
		...
		class ChatSetupTriggerSupportAnonymousAction extends Action2 {
			...
			override async run(
				...
				return commandService.executeCommand(CHAT_SETUP_ACTION_ID, undefined, {
					forceAnonymous: chatEntitlementService.anonymous
					...
					menu: [
						{
							...
							when: ContextKeyExpr.and(
								...
								ContextKeyExpr.equals(
									`config.${ChatConfiguration.TitleBarSignInEnabled}`,
									true,
								),
		...
		class UpgradePlanAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const baseUrl = defaultAccountService.resolveGitHubUrl(
					GitHubPaths.copilotUpgrade,
				);
				const upgradeUrl = buildUpgradeUrlWithRedirect(
					baseUrl,
		...
		class ManageAdditionalSpendAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				openerService.open(
					URI.parse(
						defaultAccountService.resolveGitHubUrl(GitHubPaths.billingBudgets),
					),
				);
			}
		}
				...
				async (accessor, ...args) => {
					...
					switch (coreCommand) {
						...
						case "chat.internal.fix": {
							...
							const actualCommand =
								coreCommand === "chat.internal.explain"
	...
	private registerUrlLinkHandler(): void {
		this._register(
			ExtensionUrlHandlerOverrideRegistry.registerHandler(
				this.instantiationService.createInstance(ChatSetupExtensionUrlHandler),
			),
		);
	}
	...
	private async checkExtensionInstallation(
		...
		if (this.environmentService.isExtensionDevelopment) {
			...
			if (
				this.extensionService.extensions.find((ext) =>
		...
		this._register(
			Event.runAndSubscribe<IExtension | undefined>(
				...
				(e) => {
					...
					const defaultChatExtension =
						this.extensionsWorkbenchService.local.find((value) =>
							ExtensionIdentifier.equals(
								value.identifier.id,
								defaultChat.chatExtensionId,
							),
					...
					if (installed) {
						disabled = !this.extensionEnablementService.isEnabled(
							defaultChatExtension.local,
						);
						if (disabled) {
							const state = this.extensionEnablementService.getEnablementState(
								defaultChatExtension.local,
							);
...
class ChatSetupExtensionUrlHandler implements IExtensionUrlHandlerOverride {
	private static readonly UPGRADE_SUCCESS_NOTIFICATION_ID =
		"copilot.upgradeSuccess";
	...
	async handleURL(url: URI): Promise<boolean> {
		...
		await this.commandService.executeCommand(
			CHAT_SETUP_ACTION_ID,
			agentParam,
			inputParam ? { inputValue: inputParam } : undefined,
		);
		return true;
	}
		...
		this._register(
			this.extensionsWorkbenchService.onChange((e) => {
				...
				if (
					...
					if (
						defaultChatExtension.enablementState ===
						EnablementState.EnabledWorkspace
					) {
						if (
							this.configurationService.inspect(ChatConfiguration.AIDisabled)
								.workspaceValue === true
						) {
	...
	private maybeHideAuxiliaryBar(): void {
		...
		if (
			...
			(activeContainers.length === 1 &&
				activeContainers.at(0)?.id === ChatViewContainerId) // chat view is the only view which is going to go away
		) {
		...
		class ChatSetupHideAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				preferencesService.openSettings({
					jsonEditor: false,
					query: `@id:${ChatConfiguration.AIDisabled}`,
				});
			}
		}
