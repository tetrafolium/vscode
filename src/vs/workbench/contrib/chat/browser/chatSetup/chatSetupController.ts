...
export class ChatSetupController extends Disposable {
		...
		try {
			...
			if (signIn) {
				...
				if (!result.defaultAccount) {
					const provider =
						options.useSocialProvider ??
						(options.useEnterpriseProvider
	...
	private async install(
		...
		const wasRunning =
			this.context.state.completed && !this.context.state.disabled;
		...
		if (
			typeof signUpResult === "boolean" /* not an error case */ ||
			typeof signUpResult === "undefined" /* already signed up */
		) {
			this.telemetryService.publicLog2<
				InstallChatEvent,
				InstallChatClassification
