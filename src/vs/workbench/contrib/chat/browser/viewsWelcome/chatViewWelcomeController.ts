...
export class ChatViewWelcomeController extends Disposable {
	...
	private readonly _isShowingWelcome: ISettableObservable<boolean> =
...
export class ChatViewWelcomePart extends Disposable {
		...
		try {
			...
			if (content.tips) {
				...
				const tipsResult = this._register(
					this.markdownRendererService.render(content.tips),
				);
	...
	private renderMarkdownMessageContent(
		...
		const messageResult = this._register(
			this.markdownRendererService.render(content),
		);
