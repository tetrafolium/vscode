 ...
 */
function groupNearbyChanges<T extends LineRangeMapping>(
	changes: readonly T[],
 ...
 */
export class ChatEditingExplanationWidget
	extends Disposable
	implements IOverlayWidget
{
	...
	private readonly _explanationItems: Map<
		number,
		{ item: HTMLElement; readIndicator: HTMLElement; textElement: HTMLElement }
	> = new Map();
	...
	private _setupEventHandlers(): void {
		...
		this._eventStore.add(
			addDisposableListener(this._readIndicator, "click", (e) => {
				e.stopPropagation();
	...
	private _buildExplanationItems(): void {
		...
		for (let i = 0; i < this._explanations.length; i++) {
			...
			this._eventStore.add(
				addDisposableListener(replyButton, "click", async (e) => {
					e.stopPropagation();
					...
					if (this._chatSessionResource) {
						chatWidget = await this._chatWidgetService.openSession(
							this._chatSessionResource,
						);
					} else {
	...
	setExplanationByLineNumber(
		...
		for (let i = 0; i < this._explanations.length; i++) {
			const exp = this._explanations[i];
			if (
				exp.startLineNumber === startLineNumber &&
				exp.endLineNumber === endLineNumber
			) {
...
export class ChatEditingExplanationWidgetManager extends Disposable {
	...
	private _handleExplanations(
		uri: URI,
		explanations: readonly IChangeExplanationModel[],
	): void {
