...
abstract class ChatEditingEditorAction extends Action2 {
	...
	abstract runChatEditingCommand(
		accessor: ServicesAccessor,
		session: IChatEditingSession,
		entry: IModifiedFileEntry,
		integration: IModifiedFileEntryEditorIntegration,
		...args: unknown[]
	): Promise<void> | void;
}
...
abstract class NavigateAction extends ChatEditingEditorAction {
	...
	override async runChatEditingCommand(
		accessor: ServicesAccessor,
		session: IChatEditingSession,
		entry: IModifiedFileEntry,
		ctrl: IModifiedFileEntryEditorIntegration,
	): Promise<void> {
		const instaService = accessor.get(IInstantiationService);
...
async function openNextOrPreviousChange(
	accessor: ServicesAccessor,
	session: IChatEditingSession,
	entry: IModifiedFileEntry,
	next: boolean,
...
abstract class KeepOrUndoAction extends ChatEditingEditorAction {
	constructor(
		...
		super({
			...
			menu: {
				...
				order: _keep ? 0 : 1,
				when: ContextKeyExpr.and(
					!_keep ? ctxReviewModeEnabled : undefined,
					ContextKeyExpr.or(
						ctxIsGlobalEditingSession,
	...
	override async runChatEditingCommand(
		...
		session: IChatEditingSession,
		entry: IModifiedFileEntry,
		_integration: IModifiedFileEntryEditorIntegration,
	): Promise<void> {
		const instaService = accessor.get(IInstantiationService);
...
abstract class AcceptRejectHunkAction extends ChatEditingEditorAction {
	constructor(private readonly _accept: boolean) {
		super({
			...
			shortTitle: _accept
				? localize2("acceptHunkShort", "Keep")
				: localize2("undoShort", "Undo"),
			precondition: ContextKeyExpr.and(
				ctxHasEditorModification,
				ctxIsCurrentlyBeingModified.negate(),
			),
	...
	override async runChatEditingCommand(
		...
		if (this._accept) {
			await ctrl.acceptNearestChange(
				args[0] as IModifiedFileEntryChangeHunk | undefined,
			);
		} else {
			await ctrl.rejectNearestChange(
				args[0] as IModifiedFileEntryChangeHunk | undefined,
			);
		}
...
class ToggleDiffAction extends ChatEditingEditorAction {
	...
	override runChatEditingCommand(
		_accessor: ServicesAccessor,
		_session: IChatEditingSession,
...
class ExplainMultiDiffAction extends Action2 {
	private readonly _widgetsByInput = new WeakMap<
		EditorInput,
		DisposableStore
	>();
	...
	constructor() {
		super({
			...
			menu: {
				when: ContextKeyExpr.and(
					ContextKeyExpr.or(
						...explainMultiDiffSchemes.map((scheme) =>
							ContextKeyExpr.equals("resourceScheme", scheme),
						),
					),
					ContextKeyExpr.has(
						`config.${ChatConfiguration.ExplainChangesEnabled}`,
					),
				),
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]): Promise<void> {
		const editorService = accessor.get(IEditorService);
		const explanationModelManager = accessor.get(
			IChatEditingExplanationModelManager,
		);
		...
		if (
			input instanceof MultiDiffEditorInput &&
			input.resource?.scheme === CHAT_EDITING_MULTI_DIFF_SOURCE_RESOLVER_SCHEME
		) {
			chatSessionResource = parseChatMultiDiffUri(
				input.resource,
			).chatSessionResource;
		}
		if (!chatSessionResource) {
			...
			const fileUris = items
				.map((item) => {
					const docDiffItem = item.documentDiffItem as
						| IDocumentDiffItemWithMultiDiffEditorItem
						| undefined;
					...
					const modifiedUri =
						docDiffItem?.multiDiffEditorItem?.modifiedUri ?? item.modifiedUri;
					if (modifiedUri?.path) {
		...
		if (allDiffInfos.length > 0) {
			widgetsStore.add(
				explanationModelManager.generateExplanations(
					allDiffInfos,
					chatSessionResource,
