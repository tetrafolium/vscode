...
export function adjustCellDiffForKeepingADeletedCell(
	...
	const diffs = sortCellChanges(cellDiffInfo)
		.filter(
			(d) =>
				!(d.type === "delete" && d.originalCellIndex === originalCellIndex),
		)
		.map((diff) => {
			if (
				diff.type !== "insert" &&
...
export function adjustCellDiffForRevertingAnInsertedCell(
	...
	cellDiffInfo = sortCellChanges(cellDiffInfo)
		.filter(
			(d) =>
				!(d.type === "insert" && d.modifiedCellIndex === modifiedCellIndex),
		)
		.map((d) => {
	...
	if (cells.length) {
		for (let i = 0; i < cellDiffInfo.length; i++) {
			...
			if (
				...
				diff.modifiedCellIndex === change[0]
			) {
				diffEntryIndex = i;
...
export function calculateNotebookRewriteRatio(
		...
		(totalUpdatedLines, value) => {
			const getUpadtedLineCount = () => {
				...
				if (value.type === "delete") {
					return (
						originalModel.cells[
							value.originalCellIndex
						].textModel?.getLineCount() ?? 0
					);
				}
				if (value.type === "insert") {
					return (
						modifiedModel.cells[
							value.modifiedCellIndex
						].textModel?.getLineCount() ?? 0
					);
				}
	...
	return totalNumberOfLines === 0
		? 0
		: Math.min(1, totalNumberOfUpdatedLines / totalNumberOfLines);
}
