...
export class ChatEditingNotebookEditorIntegration
	extends Disposable
	implements IModifiedFileEntryEditorIntegration
{
	...
	acceptNearestChange(
		change: IModifiedFileEntryChangeHunk | undefined,
	): Promise<void> {
...
class ChatEditingNotebookEditorWidgetIntegration
	extends Disposable
	implements IModifiedFileEntryEditorIntegration
{
	...
	private readonly cellEditorIntegrations = new Map<
		NotebookCellTextModel,
		{
			integration: ChatEditingCodeEditorIntegration;
			diff: ISettableObservable<IDocumentDiff2>;
		}
	>();
	...
	private changeIndexComputer: PrefixSumComputer = new PrefixSumComputer(
		new Uint32Array(0),
	);
	...
	constructor(
		...
		this._store.add(
			autorun((r) => {
				if (
					...
					cellChanges
						.read(r)
						.some((c) => c.type !== "unchanged" && !c.diff.read(r).identical)
				) {
					lastModifyingRequestId = _entry.lastModifyingRequestId;
					...
					const visibleChange = this.sortedCellChanges.find((c) => {
						...
						return this.notebookEditor.visibleRanges.some(
					...
					indexes.push(
						change.type === "insert" || change.type === "delete"
							...
							: change.type === "modified"
								? change.diff.read(r).changes.length
								: 0,
					);
				}

				this.changeIndexComputer = new PrefixSumComputer(
			...
			autorun((r) => {
				...
				if (!changes.length) {
					this.cellEditorIntegrations.forEach(({ diff }) => {
						diff.set(
							{ ...diff.read(undefined), ...nullDocumentDiff },
							undefined,
						);
					});
					return;
				...
				changes.forEach((change) => {
					if (
						change.modifiedCellIndex === undefined ||
					...
					const editor = notebookEditor.codeEditors.find(
						([vm]) =>
							vm.handle ===
							notebookModel.cells[change.modifiedCellIndex].handle,
					)?.[1];
					const modifiedModel =
						change.modifiedModel.promiseResult.read(r)?.data;
					const originalModel =
						change.originalModel.promiseResult.read(r)?.data;
					if (!cell || !originalModel || !modifiedModel) {
						return;
					}
					if (
						cell.cellKind === CellKind.Markup &&
						!this.markupCellListeners.has(cell.handle)
					) {
						...
						if (cellModel) {
							const listener = cellModel.onDidChangeState((e) => {
								if (e.editStateChanged) {
									setTimeout(
										() =>
											this.markdownEditState.set(
												cellModel.handle + "-" + cellModel.getEditState(),
												undefined,
											),
										0,
									);
					...
					if (currentDiff) {
						...
						if (
							!areDocumentDiff2Equal(currentDiff.diff.read(undefined), diff)
						) {
		...
		this._register(
			autorun((r) => {
				if (
					this.notebookEditor.textModel !== this.notebookModel ||
				...
				const changes = debouncedChanges
					.read(r)
					.filter((c) =>
						c.type === "insert" ? !c.diff.read(r).identical : true,
					);
				...
				if (changes.every((c) => c.type === "insert")) {
					...
					this.overlayToolbarDecorator?.decorate(
						changes.filter((c) => c.type === "insert" || c.type === "modified"),
					);
				}
			}),
		);
	...
	private getCurrentChange() {
		const currentIndex = Math.min(
			this._currentIndex.get(),
			this.changeIndexComputer.getTotalSum() - 1,
		);
						...
						return {
							accept() {
								const entry = cellChanges.find(
									(c) =>
										c.type === "delete" &&
										c.originalCellIndex === deletedCellIndex,
								);
								if (entry) {
	...
	reveal(firstOrLast: boolean): void {
		const changes = this.sortedCellChanges.filter(
			(c) => c.type !== "unchanged",
		);
		if (!changes.length) {
			return;
	...
	private _revealFirstOrLast(
		change: ICellDiffInfo,
		firstOrLast: boolean = true,
	) {
		switch (change.type) {
			case "insert":
			case "modified": {
				...
				const index =
					firstOrLast || change.type === "insert"
						? 0
						: change.diff.get().changes.length - 1;
	...
	private getCellViewModel(change: ICellDiffInfo) {
		if (
			change.type === "delete" ||
			change.modifiedCellIndex === undefined ||
			change.modifiedCellIndex >= this.notebookModel.cells.length
		) {
			return undefined;
		}
	...
	private async revealChangeInView(
		cell: ICellViewModel,
		...
		const focusTarget =
			cell.cellKind === CellKind.Code || change.type === "modified"
				? "editor"
				: "container";
	...
	private revertMarkupCellState() {
		for (const change of this.sortedCellChanges) {
			const cellViewModel = this.getCellViewModel(change);
			if (
				cellViewModel?.cellKind === CellKind.Markup &&
				cellViewModel.getEditState() === CellEditState.Editing &&
				(cellViewModel.editStateSource === "chatEditNavigation" ||
					cellViewModel.editStateSource === "chatEdit")
			) {
					...
					if (cellIntegration) {
						if (cellIntegration.next(false)) {
							this.updateCurrentIndex(
								...
								cellIntegration.currentIndex.get(),
							);
							return true;
						}
					}

					const isLastChangeInCell =
						currentChange.index >= lastChangeIndex(currentChange.change);
			...
			case "modified":
				{
					...
					if (change) {
						const index = isFirstChangeInCell
							? lastChangeIndex(change)
	...
	async acceptNearestChange(
		hunk: IModifiedFileEntryChangeHunk | undefined,
	): Promise<void> {
		if (hunk) {
			await hunk.accept();
		} else {
	...
	async toggleDiff(
		_change: IModifiedFileEntryChangeHunk | undefined,
		_show?: boolean,
	): Promise<void> {
...
export class ChatEditingNotebookDiffEditorIntegration
	extends Disposable
	implements IModifiedFileEntryEditorIntegration
{
	private readonly _currentIndex = observableValue(this, -1);
	...
	constructor(
		...
		this._store.add(
			autorun((r) => {
				...
				const numberOfCellChanges = cellChanges
					.read(r)
					.filter((c) => !c.diff.read(r).identical);
				if (
					numberOfCellChanges.length &&
					index >= 0 &&
					index < numberOfCellChanges.length
				) {
	...
	next(_wrap: boolean): boolean {
		const changes = this.cellChanges
			.get()
			.filter((c) => !c.diff.get().identical).length;
	...
	async acceptNearestChange(
		change: IModifiedFileEntryChangeHunk,
	): Promise<void> {
		await change.accept();
		this.next(true);
	}
...
function areDocumentDiff2Equal(
	diff1: IDocumentDiff2,
	diff2: IDocumentDiff2,
): boolean {
