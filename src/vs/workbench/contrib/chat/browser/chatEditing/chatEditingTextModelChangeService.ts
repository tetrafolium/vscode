...
export class ChatEditingTextModelChangeService extends Disposable {
	...
	private readonly _didAcceptOrRejectLines = this._register(
		new Emitter<acceptedOrRejectedLines>(),
	);
	...
	constructor(
		...
		this._register(
			autorun((r) => this.updateLineChangeCount(this._diffInfo.read(r))),
		);
	...
	private _createEditSource(responseModel: IChatResponseModel | undefined) {
		...
		const sessionId = chatSessionResourceToId(
			responseModel.session.sessionResource,
		);
		...
		const extensionId = VersionedExtensionId.tryCreate(
			agent?.extensionId.value,
			agent?.extensionVersion,
		);

		if (
			responseModel.request?.locationData?.type ===
	...
	private async _updateDiffInfoSeq(
		notifyAction: "accepted" | "rejected" | undefined = undefined,
	) {
	...
	public hasHunkAt(range: IRange) {
		...
		return this._diffInfo
			.get()
			.changes.some((c) =>
				c.modified.intersectsStrict(LineRange.fromRangeInclusive(range)),
			);
	}
