	...
	implements IModifiedFileEntry
{
	...
	get linesAdded() {
		return this._textModelChangeService.diffInfo.map((diff) => {
			...
			for (const c of diff.changes) {
				added += Math.max(
					0,
					c.modified.endLineNumberExclusive - c.modified.startLineNumber,
				);
			}
			return added;
		});
	...
	get linesRemoved() {
		return this._textModelChangeService.diffInfo.map((diff) => {
			...
			for (const c of diff.changes) {
				removed += Math.max(
					0,
					c.original.endLineNumberExclusive - c.original.startLineNumber,
				);
			}
			return removed;
		});
	...
	constructor(
		...
		this._register(
			this._textModelChangeService.onDidAcceptOrRejectLines((action) => {
				this._notifyAction({
	...
	async computeEditsFromSnapshots(
		...
		const stringEdit =
			await this._editorWorkerService.computeStringEditFromDiff(
				beforeSnapshot,
				afterSnapshot,
		...
		const editorTextEdit = EditorTextEdit.fromStringEdit(
			stringEdit,
			new StringText(beforeSnapshot),
		);
