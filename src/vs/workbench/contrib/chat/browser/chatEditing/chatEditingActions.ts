...
export function getEditingSessionContext(
	accessor: ServicesAccessor,
	args: any[],
): EditingSessionActionContext | undefined {
...
export async function discardAllEditsWithConfirmation(
	...
	const entries = currentEditingSession.entries
		.get()
...
function filterToUserAttachedContext(
...
async function restoreSnapshotWithConfirmationByRequestId(
	accessor: ServicesAccessor,
	sessionResource: URI,
	requestId: string,
): Promise<boolean> {
	...
	const shouldPrompt =
		...
		configurationService.getValue("chat.editing.confirmEditRequestRemoval") ===
			true;
...
async function restoreSnapshotWithConfirmation(
	accessor: ServicesAccessor,
	item: ChatTreeItem,
): Promise<boolean> {
	...
	class RemoveAction extends Action2 {
				...
				menu: [
					{
						...
						when: ContextKeyExpr.and(
							...
							ContextKeyExpr.equals(
								`config.${ChatConfiguration.CheckpointsEnabled}`,
								false,
							),
							ContextKeyExpr.or(
		...
		async run(accessor: ServicesAccessor, ...args: unknown[]) {
			...
			if (
				...
				const userAttachments = filterToUserAttachedContext(
...
registerAction2(
	class RestoreCheckpointAction extends Action2 {
		...
		async run(accessor: ServicesAccessor, ...args: unknown[]) {
			...
			const userAttachments = isRequestVM(item)
				? filterToUserAttachedContext(item.attachedContext)
				: [];
...
registerAction2(
	class StartOverAction extends Action2 {
		constructor() {
			super({
				id: "workbench.action.chat.startOver",
				title: localize2("chat.startOver.label", "Start Over"),
				tooltip: localize2(
					"chat.startOver.tooltip",
					"Clears the chat and undoes all changes",
				),
				f1: false,
				category: CHAT_CATEGORY,
				menu: [
...
registerAction2(
	class RestoreLastCheckpoint extends Action2 {
		constructor() {
			super({
		...
		async run(accessor: ServicesAccessor, ...args: unknown[]) {
			...
			await restoreSnapshotWithConfirmationByRequestId(
				accessor,
				sessionResource,
				checkpointRequest.id,
			);
		}
	},
);
...
registerAction2(
	class EditAction extends Action2 {
		constructor() {
			super({
	...
	class OpenWorkingSetHistoryAction extends Action2 {
		...
		override async run(
			...
			const snapshot = chatEditingService
				.getEditingSession(chatModel.sessionResource)
...
CommandsRegistry.registerCommand(
	...
	async (accessor: ServicesAccessor, resources: UriComponents[]) => {
