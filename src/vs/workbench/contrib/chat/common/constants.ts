...
const chatPermissionLevels = new Set<string>(
	Object.values(ChatPermissionLevel),
);

export function isChatPermissionLevel(
	level: unknown | undefined,
): level is ChatPermissionLevel {
 ...
 */
export function isAutoApproveLevel(
	level: ChatPermissionLevel | undefined,
): boolean {
	return (
...
export function isSupportedChatFileScheme(
...
export const OPEN_WORKSPACE_IN_AGENTS_WINDOW_COMMAND_ID =
	"workbench.action.openWorkspaceInAgentsWindow";
export const OPEN_AGENTS_WINDOW_COMMAND_ID =
...
export const CONTEXT_MODELS_EDITOR = new RawContextKey<boolean>(
	"inModelsEditor",
	false,
);
export const CONTEXT_MODELS_SEARCH_FOCUS = new RawContextKey<boolean>(
	"inModelsSearch",
	false,
);
