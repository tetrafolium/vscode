...
const languageModelToolsExtensionPoint =
	...
	>({
		...
		activationEventsGenerator: function* (
			contributions: readonly IRawToolContribution[],
		) {
...
const languageModelToolSetsExtensionPoint =
	extensionsRegistry.ExtensionsRegistry.registerExtensionPoint<
			...
			items: {
				...
				properties: {
					...
					icon: {
						markdownDescription: localize(
							"toolSetIcon",
							"An icon that represents this tool set, like {0}",
							"`$(zap)`",
						),
						type: "string",
					},
		...
		languageModelToolsExtensionPoint.setHandler((_extensions, delta) => {
			for (const extension of delta.added) {
				for (const rawTool of extension.value) {
					...
					if (
						(rawTool.name.startsWith("copilot_") ||
							rawTool.name.startsWith("vscode_")) &&
					...
					if (
						rawTool.tags?.some(
							(tag) => tag.startsWith("copilot_") || tag.startsWith("vscode_"),
						) &&
						!isProposedApiEnabled(
							extension.description,
							"chatParticipantPrivate",
					...
					if (
						rawTool.legacyToolReferenceFullNames &&
						!isProposedApiEnabled(
							extension.description,
							"chatParticipantPrivate",
						)
					...
					try {
						...
						this._registrationDisposables.set(
							toToolKey(extension.description.identifier, rawTool.name),
							disposable,
						);
					} catch (e) {
						extension.collector.error(
							`Failed to register tool '${rawTool.name}': ${e}`,
						);
					}
				}
			}
			...
			for (const extension of delta.added) {
				...
				for (const toolSet of extension.value) {
					if (isFalsyOrWhitespace(toolSet.name)) {
						extension.collector.error(
							`Tool set '${toolSet.name}' CANNOT have an empty name`,
						);
						continue;
					}

					if (
						toolSet.legacyFullNames &&
						!isProposedApiEnabled(
							extension.description,
							"contribLanguageModelToolSets",
						)
					...
					if (isFalsyOrEmpty(toolSet.tools)) {
						extension.collector.error(
							`Tool set '${toolSet.name}' CANNOT have an empty tools array`,
						);
						continue;
					}
					...
					for (const toolName of toolSet.tools) {
						...
						const toolSetObj =
							languageModelToolsService.getToolSetByName(toolName);
						if (toolSetObj) {
					...
					const existingToolSet =
						languageModelToolsService.getToolSetByName(referenceName);
					const mergeExisting =
						isBuiltinTool &&
						existingToolSet?.source === ToolDataSource.Internal;
					...
					if (mergeExisting) {
						...
						obj = languageModelToolsService.createToolSet(
							...
							{
								icon: toolSet.icon
									? ThemeIcon.fromString(toolSet.icon)
									: undefined,
					...
					transaction((tx) => {
						...
						toolSets.forEach((toolSet) =>
							store.add(obj.addToolSet(toolSet, tx)),
						);
					});
					...
					if (missingToolNames.length > 0) {
						...
						const listener = store.add(
							languageModelToolsService.onDidChangeTools(() => {
								for (const toolName of pending) {
									const toolObj =
										languageModelToolsService.getToolByName(toolName);
									if (toolObj) {
...
class LanguageModelToolDataRenderer
	extends Disposable
...
class LanguageModelToolSetDataRenderer
	extends Disposable
	implements IExtensionFeatureTableRenderer
{
	readonly type = "table";
	...
	render(manifest: IExtensionManifest): IRenderedData<ITableData> {
		...
		const rows: IRowData[][] = contribs.map((t) => {
			return [
				new MarkdownString(`\`${t.name}\``),
				t.referenceName
					? new MarkdownString(`\`#${t.referenceName}\``)
					: "none",
				t.tools.join(", "),
				t.description,
			];
