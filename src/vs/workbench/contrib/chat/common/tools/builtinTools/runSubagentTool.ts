...
export class RunSubagentTool extends Disposable implements IToolImpl {
	...
	private readonly _resolvedModels = new Map<
		string,
		{ modeModelId: string | undefined; resolvedModelName: string | undefined }
	>();
	...
	getToolData(): IToolData {
		...
		properties.model = {
			type: "string",
			description:
				'Optional model for the subagent. Format: "Model Name (Vendor)", vendor is usually "copilot". Only use to enforce a specific model.',
		};
	...
	async invoke(
		...
		try {
			...
			const gpEnabled = this.configurationService.getValue<boolean>(
				ChatConfiguration.GeneralPurposeAgentEnabled,
			);
			const isGeneralPurpose =
				gpEnabled &&
				(!subAgentName || subAgentName === GeneralPurposeAgentName);
			const effectiveSubAgentName = isGeneralPurpose
				? GeneralPurposeAgentName
				: subAgentName;
			...
			if (subAgentName && !isGeneralPurpose) {
				...
				if (subagent) {
					...
					if (cached) {
						...
						const resolved = this.resolveSubagentModel(
							subagent,
							invocation.modelId,
							args.model,
						);
					...
					modeInstructions = instructions && {
						...
						toolReferences: this.languageModelToolsService.toToolReferences(
							instructions.toolReferences,
						),
					...
					this._resolvedModels.delete(invocation.callId);
					const baseHint =
						" Try again with the correct agent name, or omit agentName to use the current agent.";
					const gpHint = gpEnabled
						? ` Additionally, you can use '${GeneralPurposeAgentName}' for a full-capability agent.`
						: "";
					throw new Error(
						`Requested agent '${subAgentName}' not found.${baseHint}${gpHint}`,
					);
				}
			} else {
			...
			const subAgentInvocationId =
				invocation.chatStreamToolCallId ??
				invocation.callId ??
				`subagent-${generateUuid()}`;
			...
			const allowInvocationsFromSubagents =
				this.configurationService.getValue<boolean>(
					ChatConfiguration.SubagentsAllowInvocationsFromSubagents,
				) ?? false;
			const maxDepth = allowInvocationsFromSubagents
			...
			if (maxDepth > 0) {
				this.logService.debug(
					`RunSubagentTool: Nested subagents enabling ${modeTools[RunSubagentTool.Id]}: session ${sessionKey}, currentDepth: ${currentDepth}, maxDepth: ${maxDepth}, allowInvocationsFromSubagents: ${allowInvocationsFromSubagents}`,
				);
			}
			...
			if (subagent?.hooks) {
				...
				if (remapped[HookType.Stop]) {
					...
					(remapped as Record<string, unknown>)[HookType.SubagentStop] =
						remapped[HookType.SubagentStop]
			...
			const agentRequest: IChatAgentRequest = {
				...
				hasHooksEnabled:
					!!collectedHooks &&
					Object.values(collectedHooks).some((arr) => arr && arr.length > 0),
			};

			// Subscribe to tool invocations to clear markdown parts when a tool is invoked
			store.add(
				this.languageModelToolsService.onDidInvokeTool((e) => {
			...
			const resultText =
				markdownParts
					.join("")
					.replace(/^\n*```\n+```\n*/g, "")
					.trim() || "Agent completed with no output";
	...
	private async getSubAgentByName(
		name: string,
	): Promise<ICustomAgent | undefined> {
	...
	private checkMultiplierConstraint(
	...
	): { exceeds: false } | { exceeds: true; reason: string } {
		if (!mainModelId || modelId === mainModelId) {
			return { exceeds: false };
		}

		const mainModelMetadata =
			this.languageModelsService.lookupLanguageModel(mainModelId);
		const modelMetadata =
		...
		if (
			mainMultiplier !== undefined &&
			modelMultiplier !== undefined &&
			modelMultiplier > mainMultiplier
		) {
			return {
				exceeds: true,
	...
	private getAvailableModelsInfo(mainModelId: string | undefined): string {
		const models = this.languageModelsService
			...
			.filter(
				(m): m is { id: string; metadata: ILanguageModelChatMetadata } =>
					!!m.metadata &&
		...
		for (const { id, metadata } of models) {
			const qualifiedName =
				ILanguageModelChatMetadata.asQualifiedName(metadata);
	...
	private resolveSubagentModel(
		...
		mainModelId: string | undefined,
		explicitModelQualifiedName?: string,
	): {
		...
		if (explicitModelQualifiedName) {
			const lm = this.languageModelsService.lookupLanguageModelByQualifiedName(
				explicitModelQualifiedName,
			);
			if (lm?.identifier) {
		...
		if (subagent && !explicitModelResolved) {
			...
			if (modeModelQualifiedNames) {
				...
				for (const qualifiedName of modeModelQualifiedNames) {
					const lmByQualifiedName =
						this.languageModelsService.lookupLanguageModelByQualifiedName(
							qualifiedName,
						);
		...
		if (modeModelId) {
			...
			if (check.exceeds) {
				const modelMetadata =
					this.languageModelsService.lookupLanguageModel(modeModelId);
				throw new Error(
		...
		const resolvedModelMetadata = modeModelId
			? this.languageModelsService.lookupLanguageModel(modeModelId)
			: undefined;
	...
	async prepareToolInvocation(
		...
		const isGeneralPurpose =
			gpEnabled &&
			(!args.agentName || args.agentName === GeneralPurposeAgentName);
		const subagent =
			args.agentName && !isGeneralPurpose
				? await this.getSubAgentByName(args.agentName)
				: undefined;
		...
		return {
			...
			toolSpecificData: {
				kind: "subagent",
				description: args.description,
				agentName: isGeneralPurpose
					? GeneralPurposeAgentName
					: (subagent?.name ?? args.agentName),
				prompt: args.prompt,
