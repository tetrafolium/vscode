...
export function createAskQuestionsToolData(): IToolData {
	const questionSchema: IJSONSchema & { properties: IJSONSchemaMap } = {
		...
		properties: {
			...
			allowFreeformInput: {
				type: "boolean",
				description:
					"Allow freeform text answers in addition to option selection. Defaults to true; set to false to restrict to predefined options only.",
			},
			message: {
				type: "string",
				description:
					"Optional markdown message to display below the question text, providing additional context or details.",
			},
			options: {
				type: "array",
				description:
					"Optional list of selectable answers. If omitted, the question is free text.",
				items: {
					type: "object",
					properties: {
						label: {
	...
	return {
		...
		modelDescription:
			"Use this tool to ask the user a small number of clarifying questions before proceeding. Provide the questions array with concise headers and prompts. Use options for fixed choices, set multiSelect when multiple selections are allowed. Users can always provide a freeform text answer alongside options unless you set allowFreeformInput to false.",
...
export class AskQuestionsTool extends Disposable implements IToolImpl {
	...
	async invoke(
		...
		this.logService.trace(
			`[AskQuestionsTool] Invoking with ${questions?.length ?? 0} question(s)`,
		);
		...
		if (
			request.modeInfo?.permissionLevel === ChatPermissionLevel.Autopilot ||
			...
			this.logService.info(
				`[AskQuestionsTool] ${reason}: auto-responding to questions`,
			);
			const { carousel, idToHeaderMap } = this.toQuestionCarousel(
				questions,
				resolveId,
			);
			carousel.terminalId = this.extractTerminalId(request);
			carousel.data = this.buildAutopilotCarouselAnswers(
				questions,
				carousel,
				idToHeaderMap,
			);
		...
		this.chatService.appendProgress(request, carousel);
		const externalAnswerListener =
			this.chatService.onDidReceiveQuestionCarouselAnswer((event) => {
		...
		const converted = this.convertCarouselAnswers(
			questions,
	...
	private extractTerminalId(request: IChatRequestModel): string | undefined {
		...
		const match = request.message.text.match(
			/\[Terminal (?<termId>\S+) notification:/,
		);
		...
		if (response) {
			...
			for (let i = parts.length - 1; i >= 0; i--) {
				...
				if (
					...
					part.toolId === TerminalToolId.RunInTerminal
				) {
	...
	private toQuestionCarousel(
		...
		carousel: ChatQuestionCarouselData;
		idToHeaderMap: Map<string, string>;
	} {
		...
		const mappedQuestions = questions.map((question, index) =>
			this.toChatQuestion(question, idToHeaderMap, carouselResolveId, index),
		);
		return {
	...
	private toChatQuestion(
		...
		if (question.options) {
			...
			if (recommendedOptions.length > 0) {
				defaultValue = question.multiSelect
					? recommendedOptions.map((opt) => opt.label)
					: recommendedOptions[0].label;
			}
		}
		...
		const displayTitle =
			truncateToLimit(question.header, HardLimits.header) ?? question.header;
	...
	protected convertCarouselAnswers(
		questions: IQuestion[],
		carouselAnswers: IChatQuestionAnswers | undefined,
		idToHeaderMap: Map<string, string>,
	): IAnswerResult {
		...
		for (const question of questions) {
			...
			const answer: IChatQuestionAnswerValue | undefined = internalId
				? carouselAnswers[internalId]
				: undefined;
			this.logService.trace(
			...
			if (answer === undefined) {
				...
				const { selectedValues, freeformValue } =
					answer as IChatMultiSelectAnswer;
				result.answers[question.header] = {
				...
				(hasKey(answer, { selectedValue: true }) ||
					hasKey(answer, { freeformValue: true }))
			) {
	...
	private createSkippedResult(questions: IQuestion[]): IToolResult {
		...
		return {
			content: [
				{ kind: "text", value: JSON.stringify({ answers: skippedAnswers }) },
			],
		};
	}
	...
	private createAutopilotResult(questions: IQuestion[]): IToolResult {
		...
		return {
			content: [
				{
					kind: "text",
					value: JSON.stringify({ answers } satisfies IAnswerResult),
				},
			],
		};
	}
