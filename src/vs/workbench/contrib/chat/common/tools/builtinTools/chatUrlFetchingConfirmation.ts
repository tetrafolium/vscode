...
export class ChatUrlFetchingConfirmationContribution implements ILanguageModelToolConfirmationContribution {
	...
	private _getConfirmActions(
		...
		const uniqueUrls = Array.from(new Set(urlsWithoutQuery)).map((u) =>
			URI.parse(u),
		);
		...
		if (urlPatterns.size === 1) {
			...
			actions.push({
				...
				select: async () => {
					const result = await this._showMoreOptions(
						ref,
						[{ uri, patterns }],
						forRequest,
					);
			...
			actions.push({
				...
				select: async () => {
					await this._showMoreOptions(
						ref,
						[...urlPatterns].map(([uri, patterns]) => ({ uri, patterns })),
						forRequest,
					);
					return true;
				},
	...
	private async _showMoreOptions(
		ref: ILanguageModelToolConfirmationRef,
		urls: { uri: URI; patterns: string[] }[],
		forRequest: boolean,
	): Promise<boolean> {
		...
		return new Promise<boolean>((resolve) => {
			...
			const quickTree = disposables.add(
				this._quickInputService.createQuickTree<IPatternTreeItem>(),
			);
				...
				for (const pattern of patterns
					...
					const requestChecked =
						typeof settings === "boolean"
							? settings
							: (settings?.approveRequest ?? false);
					const responseChecked =
						typeof settings === "boolean"
							? settings
							: (settings?.approveResponse ?? false);

					treeItems.push({
						label: getPatternLabel(uri, pattern),
						pattern,
						checked:
							requestChecked && responseChecked
								? true
								: !requestChecked && !responseChecked
									? false
									: "mixed",
			...
			const updateApprovals = () => {
				...
				for (const item of quickTree.itemTree) {
					...
					const allowPre = item.children?.find(
						(c) => c.approvalType === "request",
					)?.checked;
					const allowPost = item.children?.find(
						(c) => c.approvalType === "response",
					)?.checked;
	...
	getManageActions(): ILanguageModelToolConfirmationContributionQuickTreeItem[] {
		...
		items.push({
			...
			onDidOpen: () => {
				this._preferencesService.openUserSettings({
					query: ChatConfiguration.AutoApprovedUrls,
				});
			},
		});
	...
	private _getApprovedUrls(): Readonly<
		Record<string, boolean | IUrlApprovalSettings>
	> {
		return (
