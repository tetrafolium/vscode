...
const inputSchema: IJSONSchema & { properties: IJSONSchemaMap } = {
	...
	properties: {
		artifacts: {
			type: "array",
			description:
				"The complete list of artifacts for this session. Overwrites any existing artifacts.",
			items: {
				type: "object",
				properties: {
					label: {
					...
					uri: {
						type: "string",
						description:
							"Fully qualified URI of the artifact (e.g. https://localhost:3000 or file:///path/to/file). Must include the scheme.",
					},
					type: {
						type: "string",
...
export class SetArtifactsTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		return {
			pastTenseMessage: new MarkdownString(
				localize("tool.setArtifacts.pastTense", "Updated session artifacts"),
			),
	...
	async invoke(
		...
		if (!chatSessionResource) {
			return {
				content: [
					{ kind: "text", value: "Error: No session resource available" },
				],
			};
		}
		...
		const chatArtifacts =
			this._chatArtifactsService.getArtifacts(chatSessionResource);
		const subAgentInvocationId = invocation.subAgentInvocationId;

		if (subAgentInvocationId) {
			const agentName = this._resolveSubagentName(
				chatSessionResource,
				subAgentInvocationId,
			);
			chatArtifacts.setSubagentArtifacts(
				subAgentInvocationId,
				agentName,
				artifacts,
			);
