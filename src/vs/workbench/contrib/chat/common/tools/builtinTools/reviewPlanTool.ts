...
export function createReviewPlanToolData(): IToolData {
	const approvalActionSchema: IJSONSchema & { properties: IJSONSchemaMap } = {
		...
		properties: {
			...
			description: {
				type: "string",
				description:
					"Optional detail shown below the label in the dropdown list.",
			},
			default: {
				type: "boolean",
	...
	const inputSchema: IJSONSchema & { properties: IJSONSchemaMap } = {
		...
		properties: {
			title: {
				type: "string",
				description:
					'Title displayed in the widget header. Defaults to "Review plan" if omitted.',
			},
			plan: {
				type: "string",
				description:
					"Optional URI of an editable plan file. An Edit button in the widget header opens it in the editor.",
			},
			content: {
				type: "string",
				description:
					"Markdown content rendered in the body of the widget. May be the plan summary or full plan text.",
			},
			actions: {
				type: "array",
				description:
					"List of approval actions offered in the primary dropdown button. Order is preserved.",
			...
			canProvideFeedback: {
				type: "boolean",
				description:
					"When true, an additional feedback textarea is shown below the plan content.",
			},
		},
	...
	return {
		...
		modelDescription:
			"Use this tool to present a plan to the user for review. Provide the plan content as markdown, a list of approval actions (with optional default), and whether the user can provide freeform feedback. Optionally provide a URI to the backing plan file so the user can edit it. The tool returns the chosen action, whether the plan was rejected, and any feedback.",
	...
	async invoke(
		...
		if (!request) {
			this.logService.warn(
				"[ReviewPlanTool] Missing chat context; returning rejected result.",
			);
	...
	async prepareToolInvocation(
		...
		return {
			invocationMessage: new MarkdownString(
				localize("reviewPlanTool.invocation", "Asking you to review the plan"),
			),
