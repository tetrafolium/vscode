...
import {
	IArtifactGroupConfig,
	IArtifactRuleOverrides,
...
const inputSchema: IJSONSchema & { properties: IJSONSchemaMap } = {
	...
	properties: {
		...
		byFilePath: {
			type: "object",
			description:
				"Rules for extracting artifacts from written files by file path glob pattern. Maps glob patterns (e.g. '**/*plan*.md') to group configuration.",
			additionalProperties: artifactGroupConfigSchema,
		},
		byMemoryFilePath: {
			type: "object",
			description:
				"Rules for extracting artifacts from memory tool writes by memory file path glob pattern. Maps glob patterns to group configuration.",
			additionalProperties: artifactGroupConfigSchema,
		},
	},
};
...
export class SetArtifactRulesTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		return {
			pastTenseMessage: new MarkdownString(
				localize("tool.setArtifactRules.pastTense", "Updated artifact rules"),
			),
	...
	async invoke(
		...
		this._chatArtifactsService
			.getArtifacts(chatSessionResource)
