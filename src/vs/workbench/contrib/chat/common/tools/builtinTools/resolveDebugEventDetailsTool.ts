...
export const ResolveDebugEventDetailsToolId =
...
function formatResolvedContent(
	content: IChatDebugResolvedEventContent,
): string {
	switch (content.kind) {
		case "text":
		...
		case "fileList": {
			const lines: string[] = [
				localize(
					"formatResolvedContent.fileList",
					"File list ({0}):",
			...
			if (content.sourceFolders) {
				for (const folder of content.sourceFolders) {
					lines.push(
						localize(
							"formatResolvedContent.sourceFolder",
							"  Source folder: {0} ({1})",
							folder.uri.toString(),
							folder.storage,
						),
			...
			for (const file of content.files) {
				const status =
					file.status === "loaded"
						...
						: file.skipReason
							? localize(
									"formatResolvedContent.skippedWithReason",
									"skipped: {0}",
									file.skipReason,
								)
		...
		case "message": {
			const messageType =
				content.type === "user"
					? localize(
							"formatResolvedContent.userMessage",
							"User message: {0}",
							content.message,
						)
					: localize(
							"formatResolvedContent.agentMessage",
							"Agent message: {0}",
							content.message,
						);
		...
		case "toolCall": {
			const lines: string[] = [
				localize(
					"formatResolvedContent.toolCall",
					"Tool call: {0}",
					content.toolName,
				),
			];
			...
			if (content.durationInMillis !== undefined) {
				lines.push(
					localize(
						"formatResolvedContent.duration",
						"Duration: {0}ms",
			...
			if (content.input) {
				lines.push(
					localize("formatResolvedContent.input", "Input:") +
						"\n" +
						content.input,
				);
			}
			if (content.output) {
				lines.push(
					localize("formatResolvedContent.output", "Output:") +
						"\n" +
						content.output,
				);
			}
		...
		case "modelTurn": {
			const lines: string[] = [
				localize(
					"formatResolvedContent.modelTurn",
					"Model turn: {0}",
					content.requestName,
				),
			];
			if (content.model) {
				lines.push(
					localize("formatResolvedContent.model", "Model: {0}", content.model),
				);
			}
			if (content.status) {
				lines.push(
					localize(
						"formatResolvedContent.status",
						"Status: {0}",
						content.status,
					),
				);
			...
			if (
				content.inputTokens !== undefined ||
				content.outputTokens !== undefined
			) {
				lines.push(
					localize(
						"formatResolvedContent.tokens",
						"Tokens: input={0}, output={1}, cached={2}, total={3}",
						content.inputTokens ?? "?",
						content.outputTokens ?? "?",
						content.cachedTokens ?? "?",
						content.totalTokens ?? "?",
					),
				);
			}
		...
		case "hook": {
			const lines: string[] = [
				localize("formatResolvedContent.hook", "Hook: {0}", content.hookType),
			];
			if (content.command) {
				lines.push(
					localize(
						"formatResolvedContent.command",
						"Command: {0}",
						content.command,
					),
				);
			...
			if (content.result !== undefined) {
				const resultText =
					content.result === ChatDebugHookResult.Success
						...
						: content.result === ChatDebugHookResult.Error
							...
							: localize(
									"formatResolvedContent.hookResult.nonBlockingError",
									"Non-blocking Error",
								);
				lines.push(
					localize("formatResolvedContent.result", "Result: {0}", resultText),
				);
			}
			if (content.exitCode !== undefined) {
				lines.push(
					localize(
						"formatResolvedContent.exitCode",
						"Exit Code: {0}",
						content.exitCode,
					),
				);
		...
		case "customizationSummary": {
			...
			lines.push(
				localize(
					"formatResolvedContent.customizationCounts",
					"Customization: {0} instructions, {1} skills, {2} agents, {3} hooks, {4} skipped",
			...
			lines.push(
				localize(
					"formatResolvedContent.customizationDuration",
					"Duration: {0}ms",
			...
			if (content.resolutionLogs.length > 0) {
				lines.push("");
				lines.push(
					localize("formatResolvedContent.resolutionLogs", "Resolution logs:"),
				);
...
function getEventLabel(event: IChatDebugEvent): string {
	switch (event.kind) {
		...
		case "modelTurn":
			return (
				event.requestName ?? localize("debugEvent.modelTurn", "Model Turn")
			);
		case "userMessage":
			return localize(
				"debugEvent.userMessage",
				"User Message: {0}",
				truncate(event.message),
			);
		case "agentResponse":
			return localize(
				"debugEvent.agentResponse",
				"Agent Response: {0}",
				truncate(event.message),
			);
...
export class ResolveDebugEventDetailsTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		if (eventLabel) {
			return {
				invocationMessage: localize(
					"resolveDebugEventDetails.invocationMessageNamed",
					'Resolving details for "{0}"',
					eventLabel,
				),
				pastTenseMessage: localize(
					"resolveDebugEventDetails.pastTenseMessageNamed",
					'Resolved details for "{0}"',
					eventLabel,
				),
			};
		}
		return {
			invocationMessage: localize(
				"resolveDebugEventDetails.invocationMessage",
				"Resolving debug event details",
			),
			pastTenseMessage: localize(
				"resolveDebugEventDetails.pastTenseMessage",
				"Resolved debug event details",
			),
		};
	}
				...
				content: [
					{
						kind: "text",
						value: localize(
							"resolveDebugEventDetails.errorEventIdRequired",
				...
				content: [
					{
						kind: "text",
						value: localize(
							"resolveDebugEventDetails.errorNoSession",
							"Error: no chat session context available.",
						),
					},
				],
			};
				...
				content: [
					{
						kind: "text",
						value: localize(
							"resolveDebugEventDetails.errorEventNotFound",
							'No event with ID "{0}" found in the current session.',
							eventId,
						),
					},
				],
				...
				content: [
					{
						kind: "text",
						value: localize(
							"resolveDebugEventDetails.errorNoDetails",
							"No details found for event ID: {0}",
							eventId,
						),
					},
				],
