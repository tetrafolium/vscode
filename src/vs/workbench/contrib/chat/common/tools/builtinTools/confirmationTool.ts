...
export const ConfirmationToolWithOptionsId =
	"vscode_get_confirmation_with_options";
export const ModifiedFilesConfirmationToolId =
...
export const ModifiedFilesConfirmationToolData: IToolData = {
	...
	modelDescription:
		"A tool that shows a modified-files confirmation UI with a split primary button and a hardcoded cancel action.",
	source: ToolDataSource.Internal,
	inputSchema: {
		type: "object",
		properties: {
			...
			options: {
				...
				minItems: 1,
				description:
					"Selectable option labels. The first option is used for the primary split button and the remaining options are placed in the dropdown menu.",
			},
			modifiedFiles: {
				type: "array",
				items: {
...
export class ModifiedFilesConfirmationTool implements IToolImpl {
	async prepareToolInvocation(
		...
		const parameters =
			context.parameters as IModifiedFilesConfirmationToolParams;
		if (!parameters.title || !parameters.message) {
			throw new Error(
				"Missing required parameters for ModifiedFilesConfirmationTool",
			);
		}

		if (!parameters.options?.length) {
			throw new Error(
				"ModifiedFilesConfirmationTool requires at least one option",
			);
		}
		...
		const toolSpecificData: IChatModifiedFilesConfirmationData = {
			...
			modifiedFiles: parameters.modifiedFiles.map((file) => ({
				...
				originalUri: file.originalUri
					? URI.parse(file.originalUri).toJSON()
					: undefined,
