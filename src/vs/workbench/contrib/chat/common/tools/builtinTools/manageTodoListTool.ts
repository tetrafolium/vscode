...
export class ManageTodoListTool extends Disposable implements IToolImpl {
	...
	async invoke(
		...
		if (
			!chatSessionResource &&
			args.operation === "read" &&
			args.chatSessionResource
		) {
			try {
				chatSessionResource = URI.parse(args.chatSessionResource);
			} catch (error) {
				this.logService.error(
					"ManageTodoListTool: Invalid chatSessionResource URI",
					error,
				);
			}
		}
		...
		this.logService.debug(
			`ManageTodoListTool: Invoking with options ${JSON.stringify(args)}`,
		);
	...
	async prepareToolInvocation(
		...
		const currentTodoItems =
			this.chatTodoListService.getTodos(chatSessionResource);
		...
		const invocationLabel =
			message?.replace(/^(Starting|Completed): /i, "") ??
			localize("todo.updatingList", "Updating todo list");
		...
		return {
			...
			pastTenseMessage: new MarkdownString(
				message ?? localize("todo.updatedList", "Updated todo list"),
			),
			toolSpecificData: {
				kind: "todoList",
				todoList: todoList,
	...
	private generatePastTenseMessage(
		currentTodos: IChatTodo[],
		newTodos: IManageTodoListToolInputParams["todoList"],
	): string {
		...
		if (startedTodos.length > 0) {
			...
			const currentPosition =
				newTodos.findIndex((todo) => todo.id === startedTodo.id) + 1;
			return localize(
				"todo.starting",
				"Starting: *{0}* ({1}/{2})",
				startedTodo.title,
		...
		if (completedTodos.length > 0) {
			...
			const totalTodos = newTodos.length;
			const currentPosition =
				newTodos.findIndex((todo) => todo.id === completedTodo.id) + 1;
			return localize(
				"todo.completed",
				"Completed: *{0}* ({1}/{2})",
				completedTodo.title,
		...
		const addedTodos = newTodos.filter(
			(newTodo) => !currentTodoMap.has(newTodo.id),
		);
	...
	private handleReadOperation(chatSessionResource: URI): IToolResult {
		...
		this.telemetryService.publicLog2<
			TodoListToolInvokedEvent,
			TodoListToolInvokedClassification
		>("todoListToolInvoked", {
			operation: "read",
	...
	private handleWriteOperation(
		args: IManageTodoListToolInputParams,
	...
	private calculateStatusCounts(todos: IChatTodo[]): {
		notStartedCount: number;
		inProgressCount: number;
		completedCount: number;
	} {
		const notStartedCount = todos.filter(
...
type TodoListToolInvokedClassification = {
	operation: {
		...
		comment: "The operation performed on the todo list (read or write).";
	};
	notStartedCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "The number of tasks with not-started status.";
	};
	inProgressCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "The number of tasks with in-progress status.";
	};
	completedCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "The number of tasks with completed status.";
	};
	owner: "bhavyaus";
