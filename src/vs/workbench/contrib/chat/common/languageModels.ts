...
export interface ILanguageModelsService {
	...
	renameLanguageModelsProviderGroup(
		vendorId: string,
		providerGroupName: string,
	): Promise<void>;

	updateLanguageModelsProviderGroupApiKey(
		vendorId: string,
		providerGroupName: string,
	): Promise<void>;
	...
	openLanguageModelsProviderGroupSettings(
		vendorId: string,
		providerGroupName: string,
	): Promise<void>;
...
export class LanguageModelsService implements ILanguageModelsService {
	...
	private readonly _onDidChangePinnedModels = this._store.add(
		new Emitter<void>(),
	);
	readonly onDidChangePinnedModels = this._onDidChangePinnedModels.event;

	private readonly _onDidChangeModelVisibility = this._store.add(
		new Emitter<void>(),
	);
	...
	constructor(
		...
		this._hasNonCopilotUserSelectableModels =
			ChatContextKeys.nonCopilotLanguageModelsAreUserSelectable.bindTo(
				_contextKeyService,
			);
		...
		this._store.add(
			this.onDidChangeLanguageModels(() => {
				...
				this._hasNonCopilotUserSelectableModels.set(
					hasNonCopilotUserSelectable,
				);
	...
	async renameLanguageModelsProviderGroup(
		...
		const languageModelProviderGroups =
			this._languageModelsConfigurationService.getLanguageModelsProviderGroups();
		const existing = languageModelProviderGroups.find(
			(group) => group.vendor === vendorId && group.name === providerGroupName,
		);
		if (!existing) {
			throw new Error(
	...
	async updateLanguageModelsProviderGroupApiKey(
		...
		const existing = this._languageModelsConfigurationService
			...
			.find(
				(group) =>
					group.vendor === vendorId && group.name === providerGroupName,
			);
		if (!existing) {
			throw new Error(
				`Language model provider group ${providerGroupName} for vendor ${vendorId} not found.`,
		...
		try {
			const existingConfiguration = await this._resolveConfiguration(
				existing,
				schema,
			);
	...
	async addLanguageModelsProviderGroupModel(
		...
		const group = this._languageModelsConfigurationService
			.getLanguageModelsProviderGroups()
			.find(
				(group) =>
					group.vendor === vendorId && group.name === providerGroupName,
		...
		const snippet = hasModels
			? this.getSnippetForArrayItem(modelsSchema)
			: this.getSnippetForProperty("models", modelsSchema);
		if (!snippet) {
			return;
		}
	...
	private getDefaultSnippetBodyText(
		...
		const bodyText = arrayItem
			? Array.isArray(snippet.body) && snippet.body.length > 0
				? JSON.stringify(snippet.body[0], null, "\t")
				: undefined
			: (snippet.bodyText ?? JSON.stringify(snippet.body, null, "\t"));
		if (!bodyText) {
			return undefined;
		...
		try {
			return await new Promise<string[] | undefined>((resolve) => {
				...
				quickPick.placeholder =
					this.getDescriptionPlaintext(propertySchema) ??
	...
	private async promptForEnum(
		groupName: string,
		property: string,
		propertySchema: IJSONSchema & { enumItemLabels?: string[] },
		existing: IStringDictionary<unknown> | undefined,
	): Promise<string | undefined> {
		...
		try {
			return await new Promise<string | undefined>((resolve) => {
				const quickPick = disposables.add(
					this._quickInputService.createQuickPick<IQuickPickItem>(),
				);
				quickPick.title = `${groupName}: ${propertySchema.title ?? property}`;
	...
	getPinnedModelIds(): string[] {
		return this._pinnedModelIds.filter(
			(id) => id !== AUTO_MODEL_IDENTIFIER && this._modelCache.has(id),
		);
	}

	pinModel(modelIdentifier: string): void {
		if (
			modelIdentifier === AUTO_MODEL_IDENTIFIER ||
			this._pinnedModelIds.includes(modelIdentifier)
		) {
			return;
		}
	...
	private _readVisibility(): void {
		const raw = this._storageService.getObject<{ hiddenModels?: string[] }>(
			CHAT_MODEL_VISIBILITY_STORAGE_KEY,
			StorageScope.PROFILE,
			{},
		);
		this._hiddenModelIds = new Set(
			Array.isArray(raw?.hiddenModels) ? raw.hiddenModels : [],
		);
	}
	...
	isGroupHidden(vendor: string, groupName: string): boolean {
		...
		return (
			modelIds.length > 0 &&
			modelIds.every((id) => this._hiddenModelIds.has(id))
		);
	}
