...
import {
	...
	IPromptsService,
	IAgentSkill,
	IInstructionDiscoveryInfo,
	IInstructionDiscoveryResult,
	IInstructionFile,
	IUserPromptPath,
	PromptsStorage,
	IPromptFileContext,
	...
	IPromptSourceFolderResult,
	ICustomAgentVisibility,
	IAgentInstructionFile,
	AgentInstructionFileType,
	Logger,
	ISlashCommandDiscoveryInfo,
	ISlashCommandDiscoveryResult,
	IAgentDiscoveryInfo,
...
import {
	...
	mapClaudeModels,
	mapClaudeTools,
} from "../languageProviders/promptFileAttributes.js";
import {
	getCanonicalPluginCommandId,
	IAgentPlugin,
	IAgentPluginService,
} from "../../plugins/agentPluginService.js";
...
export class PromptsService extends Disposable implements IPromptsService {
	 ...
	 */
	private readonly cachedParsedPromptFromModels = new ResourceMap<
		[number, ParsedPromptFile]
	>();
	...
	private readonly cachedFileLocations: {
		[key in PromptsType]?: Promise<readonly IPromptPath[]>;
	} = {};
	...
	private readonly _onDidPluginPromptFilesChange = this._register(
		new Emitter<PromptsType>(),
	);
	private readonly _onDidPluginHooksChange = this._register(
		new Emitter<void>(),
	);
	private _pluginPromptFilesByType = new Map<
		PromptsType,
		readonly IPluginPromptPath[]
	>();
	...
	constructor(
		...
		this.extensionPromptFiles = this._register(
			this.instantiationService.createInstance(ExtensionPromptFileService),
		);
		const onDidChangeExtensionPromptFiles =
			this.extensionPromptFiles.onDidChange;
		...
		this._register(
			onDidChangeExtensionPromptFiles((e) => {
				this.cachedFileLocations[e.type] = undefined;
			}),
		);

		const modelChangeEvent = this._register(
			new ModelChangeTracker(this.modelService),
		).onDidPromptChange;
		this.cachedCustomAgents = this._register(
			new CachedPromise(
				...
				() =>
					Event.any(
						...
						Event.filter(
							modelChangeEvent,
							(e) => e.promptType === PromptsType.agent,
						),
						Event.filter(
						...
						Event.filter(
							...
							(e) => e.type === PromptsType.agent,
						),
						Event.filter(
							this._onDidPluginPromptFilesChange.event,
							(t) => t === PromptsType.agent,
						),
		...
		this.cachedSlashCommands = this._register(
			new CachedPromise(
				...
				() =>
					Event.any(
						...
						Event.filter(
							modelChangeEvent,
							(e) => e.promptType === PromptsType.prompt,
						),
						Event.filter(
							modelChangeEvent,
						...
						Event.filter(
							onDidChangeExtensionPromptFiles,
							(e) =>
								e.type === PromptsType.prompt || e.type === PromptsType.skill,
						),
						Event.filter(
							this._onDidPluginPromptFilesChange.event,
		...
		this.cachedSkills = this._register(
			new CachedPromise(
				...
				() =>
					Event.any(
						...
						Event.filter(
							onDidChangeExtensionPromptFiles,
							(e) => e.type === PromptsType.skill,
						),
						Event.filter(
							this._onDidPluginPromptFilesChange.event,
							(t) => t === PromptsType.skill,
						),
					),
			),
		);

		this.cachedHooks = this._register(
			new CachedPromise(
				...
				() =>
					Event.any(
						...
						Event.filter(
							this.configurationService.onDidChangeConfiguration,
							(e) =>
								e.affectsConfiguration(PromptsConfig.USE_CHAT_HOOKS) ||
								e.affectsConfiguration(PromptsConfig.USE_CLAUDE_HOOKS),
						),
		...
		this.cachedInstructions = this._register(
			new CachedPromise(
				...
				() =>
					Event.any(
						...
						Event.filter(
							onDidChangeExtensionPromptFiles,
							(e) => e.type === PromptsType.instructions,
						),
						Event.filter(
							this._onDidPluginPromptFilesChange.event,
		...
		this._register(
			this.watchPluginPromptFilesForType(PromptsType.prompt, (plugin, reader) =>
				plugin.commands.read(reader),
			),
		);
		this._register(
			this.watchPluginPromptFilesForType(PromptsType.skill, (plugin, reader) =>
				plugin.skills.read(reader),
			),
		);
		this._register(
			this.watchPluginPromptFilesForType(PromptsType.agent, (plugin, reader) =>
				plugin.agents.read(reader),
			),
		);
		this._register(
			this.watchPluginPromptFilesForType(
	...
	private watchPluginPromptFilesForType(
		...
		return autorun((reader) => {
			...
			nextFiles.sort((a, b) =>
				`${a.name ?? ""}|${a.uri.toString()}`.localeCompare(
					`${b.name ?? ""}|${b.uri.toString()}`,
				),
			);
	...
	private getFileLocatorEvent(type: PromptsType): Event<void> {
		...
		if (!event) {
			event = this.fileLocatorEvents[type] = this._register(
				this.fileLocator.createFilesUpdatedEvent(type),
			).event;
			this._register(
				event(() => {
					this.cachedFileLocations[type] = undefined;
	...
	public getParsedPromptFile(textModel: ITextModel): ParsedPromptFile {
		...
		const ast = new PromptFileParser().parse(
			textModel.uri,
			textModel.getValue(),
		);
		if (!cached || cached[0] < textModel.getVersionId()) {
			this.cachedParsedPromptFromModels.set(textModel.uri, [
	...
	private async computeListPromptFiles(
		type: PromptsType,
		token: CancellationToken,
	): Promise<readonly IPromptPath[]> {
		const prompts = await Promise.all([
			this.fileLocator
				.listFiles(type, PromptsStorage.user, token)
				.then((uris) =>
					uris.map(
						(uri) =>
							({
			...
			this.fileLocator
				.listFiles(type, PromptsStorage.local, token)
				.then((uris) =>
					uris.map(
						(uri) =>
							({
	 ...
	 */
	private async _collectSourceFolderDiagnostics(
		type: PromptsType,
	): Promise<IPromptSourceFolderResult[]> {
		const resolvedFolders =
			await this.fileLocator.getSourceFoldersInDiscoveryOrder(type);
	 ...
	 */
	public registerPromptFileProvider(
		...
		return this.extensionPromptFiles.registerPromptFileProvider(
			extension,
			type,
			provider,
		);
	...
	public async listPromptFilesForStorage(
		type: PromptsType,
		...
		switch (storage) {
			...
			case PromptsStorage.local:
				promptPaths = await this.fileLocator
					.listFiles(type, PromptsStorage.local, token)
					.then((uris) =>
						uris.map(
							(uri) =>
			...
			default:
				throw new Error(
					`[listPromptFilesForStorage] Unsupported prompt storage type: ${storage}`,
				);
		}
	...
	public async getSourceFolders(
		...
		if (type === PromptsType.hook) {
			...
			for (const uri of await this.fileLocator.getConfigBasedSourceFolders(
				type,
			)) {
	...
	public async getResolvedSourceFolders(
		type: PromptsType,
	 ...
	 */
	private async computeSlashCommandDiscoveryInfo(
		token: CancellationToken,
	): Promise<ISlashCommandDiscoveryInfo> {
		const stopWatch = StopWatch.create(true);
		const promptFiles = await this.listPromptFiles(PromptsType.prompt, token);
		const useAgentSkills = this.configurationService.getValue(
			PromptsConfig.USE_AGENT_SKILLS,
		);
		const skills = useAgentSkills
			? await this.listPromptFiles(PromptsType.skill, token)
			: [];
		...
		const parseResults = await Promise.all(
			slashCommandFiles.map(async (promptPath) => {
				try {
					...
					if (promptPath.type === PromptsType.skill) {
						...
						rawName =
							parsedPromptFile?.header?.name ??
							promptPath.name ??
							getCleanPromptName(promptPath.uri);
					}
					...
					const name =
						promptPath.source === PromptFileSource.Plugin &&
						promptPath.pluginUri
							? getCanonicalPluginCommandId(
					...
					return {
						...
						argumentHint,
						userInvocable,
					} satisfies ISlashCommandDiscoveryResult;
				} catch (e) {
					this.logger.error(
		...
		const promptSourceFolders = await this._collectSourceFolderDiagnostics(
			PromptsType.prompt,
		);
		const sourceFolders = [...promptSourceFolders];

		if (useAgentSkills) {
			const skillSourceFolders = await this._collectSourceFolderDiagnostics(
				PromptsType.skill,
			);
	...
	private slashCommandsFromDiscoveryInfo(
		discoveryInfo: ISlashCommandDiscoveryInfo,
		...
		for (const model of this.modelService.getModels()) {
			if (
				...
				model.uri.scheme === Schemas.untitled &&
				!seen.has(model.uri)
			) {
				const parsedPromptFile = this.getParsedPromptFile(model);
				const name =
					parsedPromptFile?.header?.name ?? getCleanPromptName(model.uri);
	...
	private refreshKnownPromptSlashCommandNames(): void {
		this.getPromptSlashCommands(CancellationToken.None).then(
			(commands) => {
	...
	public async resolvePromptSlashCommand(
		...
		const command = commands.find(
			(cmd) =>
				cmd.name === name && matchesSessionType(cmd.sessionTypes, sessionType),
		);
		if (command) {
			return {
				...command,
	...
	public async getPromptSlashCommandName(
		uri: URI,
		token: CancellationToken,
	): Promise<string> {
	...
	public async getCustomAgents(
		token: CancellationToken,
	): Promise<readonly ICustomAgent[]> {
	...
	private agentsFromDiscoveryInfo(
		discoveryInfo: IAgentDiscoveryInfo,
	): readonly ICustomAgent[] {
	...
	private async computeAgentDiscoveryInfo(
		token: CancellationToken,
	): Promise<IAgentDiscoveryInfo> {
		const stopWatch = StopWatch.create(true);
		...
		const useChatHooks = this.configurationService.getValue(
		...
		const userHome =
			userHomeUri.scheme === Schemas.file
				? userHomeUri.fsPath
				: userHomeUri.path;
		const defaultFolder = this.workspaceService.getWorkspace().folders[0];

		const files = await Promise.all(
			allAgentFiles.map(async (promptPath): Promise<IAgentDiscoveryResult> => {
				...
				try {
					...
					if (useChatHooks && isWorkspaceTrusted && hooksRaw) {
						const hookWorkspaceFolder =
							this.workspaceService.getWorkspaceFolder(uri) ?? defaultFolder;
						const workspaceRootUri = hookWorkspaceFolder?.uri;
						const target = getTarget(
							PromptsType.agent,
							ast.header ?? promptPath.uri,
						);
						hooks = parseSubagentHooksFromYaml(
							hooksRaw,
							workspaceRootUri,
							userHome,
							target,
					...
					if (
						...
						this.logger.error(
							`[computeAgentDiscoveryInfo] Failed to parse agent file: ${uri}`,
							error,
						);
					}
					return {
		...
		const sourceFolders = await this._collectSourceFolderDiagnostics(
			PromptsType.agent,
		);
		return {
	...
	public registerContributedFile(
		type: PromptsType,
		uri: URI,
		extension: IExtensionDescription,
		name?: string,
		...
		return this.extensionPromptFiles.registerContributedFile(
			type,
			uri,
			extension,
			name,
	...
	public async listNestedAgentMDs(
		...
		const useAgentMD = this.configurationService.getValue(
			PromptsConfig.USE_AGENT_MD,
		);
		if (!useAgentMD) {
			return [];
		}
		const useNestedAgentMD = this.configurationService.getValue(
	...
	public async listAgentInstructions(
		...
		const rootFolders = await this.fileLocator.getWorkspaceFolderRoots(
			includeParents,
			logger,
		);
		...
		const useClaudeMD = this.configurationService.getValue(
		...
		if (!useClaudeMD) {
			...
			); // CLAUDE.md in .claude folder under workspace root
			promises.push(
				this.fileLocator.findFilesInRoots(
			...
			); // CLAUDE.md in in ~/.claude folder
		}
		const useCopilotInstructionsFiles = this.configurationService.getValue(
	...
	public async findAgentSkills(
		token: CancellationToken,
	): Promise<IAgentSkill[] | undefined> {
		const useAgentSkills = this.configurationService.getValue(
			PromptsConfig.USE_AGENT_SKILLS,
		);
		if (!useAgentSkills) {
	...
	private skillsFromDiscoveryInfo(
		discoveryInfo: IPromptDiscoveryInfo,
	): IAgentSkill[] {
		const result: IAgentSkill[] = [];
		for (const file of discoveryInfo.files) {
			if (file.status === "loaded" && file.promptPath.name) {
				const sanitizedDescription = this.truncateAgentSkillDescription(
					file.promptPath.description,
					file.promptPath.uri,
				);
				result.push({
	 ...
	 */
	private async computeSkillDiscovery(
		token: CancellationToken,
		...
		type AgentSkillsFoundClassification = {
			totalSkillsFound: {
				...
				isMeasurement: true;
				comment: "Total number of agent skills found.";
			};
			claudePersonal: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of Claude personal skills.";
			};
			claudeWorkspace: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of Claude workspace skills.";
			};
			copilotPersonal: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of Copilot personal skills.";
			};
			githubWorkspace: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of GitHub workspace skills.";
			};
			agentsPersonal: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of .agents personal skills.";
			};
			agentsWorkspace: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of .agents workspace skills.";
			};
			configPersonal: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of custom configured personal skills.";
			};
			configWorkspace: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of custom configured workspace skills.";
			};
			extensionContribution: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of extension contributed skills.";
			};
			extensionAPI: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of extension API provided skills.";
			};
			plugin: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of plugin provided skills.";
			};
			skippedDuplicateName: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of skills skipped due to duplicate names.";
			};
			skippedMissingName: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of skills skipped due to missing name attribute.";
			};
			skippedMissingDescription: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of skills skipped due to missing description attribute.";
			};
			skippedNameMismatch: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of skills skipped due to name not matching folder name.";
			};
			skippedParseFailed: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				isMeasurement: true;
				comment: "Number of skills skipped due to parse failures.";
			};
			owner: "pwang347";
			comment: "Tracks agent skill usage, discovery, and skipped files.";
		};

		const totalSkillsFound = files.filter(
			(f) => f.status === "loaded" && f.promptPath.name,
		).length;
		this.telemetryService.publicLog2<
			AgentSkillsFoundEvent,
			AgentSkillsFoundClassification
		>("agentSkillsFound", {
			totalSkillsFound,
			claudePersonal: skillsBySource.get(PromptFileSource.ClaudePersonal) ?? 0,
			claudeWorkspace:
				skillsBySource.get(PromptFileSource.ClaudeWorkspace) ?? 0,
			copilotPersonal:
				skillsBySource.get(PromptFileSource.CopilotPersonal) ?? 0,
			githubWorkspace:
				skillsBySource.get(PromptFileSource.GitHubWorkspace) ?? 0,
			agentsPersonal: skillsBySource.get(PromptFileSource.AgentsPersonal) ?? 0,
			agentsWorkspace:
				skillsBySource.get(PromptFileSource.AgentsWorkspace) ?? 0,
			configWorkspace:
				skillsBySource.get(PromptFileSource.ConfigWorkspace) ?? 0,
			configPersonal: skillsBySource.get(PromptFileSource.ConfigPersonal) ?? 0,
			extensionContribution:
				skillsBySource.get(PromptFileSource.ExtensionContribution) ?? 0,
	...
	public async getInstructionFiles(
		token: CancellationToken,
	...
	private instructionsFromDiscoveryInfo(
		discoveryInfo: IInstructionDiscoveryInfo,
	): IInstructionFile[] {
	...
	private async computeInstructionFiles(
		token: CancellationToken,
	): Promise<IInstructionDiscoveryInfo> {
		return await this.getInstructionsDiscoveryInfo(token);
	}

	private async computeHooks(
		token: CancellationToken,
	): Promise<IHookDiscoveryInfo> {
		const stopWatch = StopWatch.create(true);
		const useChatHooks = this.configurationService.getValue(
			PromptsConfig.USE_CHAT_HOOKS,
		);
		...
		if (!useChatHooks || !this.workspaceTrustService.isWorkspaceTrusted()) {
			...
			const skipReason: IPromptFileDiscoveryResult["skipReason"] = !useChatHooks
				? "disabled"
				: "workspace-untrusted";
			const files = hookFiles.map((promptPath) => ({
				...
				promptPath: this.withPromptPathMetadata(
					promptPath,
					basename(promptPath.uri),
					promptPath.description,
				),
			}));
			const sourceFolders = await this._collectSourceFolderDiagnostics(
				PromptsType.hook,
		...
		const useClaudeHooks = this.configurationService.getValue<boolean>(
			PromptsConfig.USE_CLAUDE_HOOKS,
		);
		const hookFiles = await this.listPromptFiles(PromptsType.hook, token);

		this.logger.trace(
			`[PromptsService] Found ${hookFiles.length} hook file(s).`,
		);
			...
			hookFiles.map(
				async (
					...
					try {
						...
						const hookWorkspaceFolder =
							this.workspaceService.getWorkspaceFolder(hookFile.uri) ??
							defaultFolder;
						const workspaceRootUri = hookWorkspaceFolder?.uri;
						...
						if (
							format === HookSourceFormat.Claude &&
							useClaudeHooks === false
						) {
							const hasAnyCommands = [...parsedHooks.values()].some(
								({ hooks: cmds }) => cmds.length > 0,
							);
							this.logger.trace(
						...
						this.logger.warn(
							`[PromptsService] Failed to parse hook file: ${hookFile.uri}`,
							error,
						);
						return {
							file: {
		...
		// Build the result
		const result: ChatRequestHooks = Object.fromEntries(
			collectedHooks,
		) as ChatRequestHooks;

		this.logger.trace(
			`[PromptsService] Collected hooks: ${JSON.stringify(Object.keys(result))}`,
		);
		return {
			type: PromptsType.hook,
			files,
			sourceFolders,
			hooksInfo: { hooks: result, hasDisabledClaudeHooks },
			durationInMillis: stopWatch.elapsed(),
		};
	}
	 ...
	 */
	private async computeSkillDiscoveryInfo(
		token: CancellationToken,
	): Promise<IPromptFileDiscoveryResult[]> {
		const files: IPromptFileDiscoveryResult[] = [];
		...
		const extensionSkills = await this.getExtensionPromptFiles(
			PromptsType.skill,
			token,
		);
		const pluginSkills =
			this._pluginPromptFilesByType.get(PromptsType.skill) ?? [];
		...
		for (const skill of allSkills) {
			...
			try {
				...
				const disableModelInvocation =
					parsedFile.header?.disableModelInvocation === true;
	...
	private async getInstructionsDiscoveryInfo(
		token: CancellationToken,
	): Promise<IInstructionDiscoveryInfo> {
		const stopWatch = StopWatch.create(true);
		const files: IInstructionDiscoveryResult[] = [];

		const instructionsFiles = await this.listPromptFiles(
			PromptsType.instructions,
			token,
		);
		for (const promptPath of instructionsFiles) {
			...
			try {
				...
				const name =
					parsedPromptFile?.header?.name ??
					promptPath.name ??
					getCleanPromptName(uri);
				const description =
					parsedPromptFile?.header?.description ?? promptPath.description;
				const pattern = evaluateApplyToPattern(
					parsedPromptFile.header,
					isInClaudeRulesFolder(uri),
				);
				files.push({
					status: "loaded",
					pattern,
...
class CachedPromise<T> extends Disposable {
	...
	constructor(
		private readonly computeFn: (token: CancellationToken) => Promise<T>,
		private readonly getEvent: () => Event<void>,
...
class ModelChangeTracker extends Disposable {
	...
	constructor(modelService: IModelService) {
		super();
		this.onDidPromptModelChange = this._register(
			new Emitter<ModelChangeEvent>(),
		);
			...
			if (promptType !== undefined) {
				this.listeners.set(
					...
					model.onDidChangeContent(() =>
						this.onDidPromptModelChange.fire({ uri: model.uri, promptType }),
					),
				);
			}
			return promptType;
		...
		this._register(
			modelService.onModelRemoved((model) =>
				onRemove(model.getLanguageId(), model.uri),
			),
		);
	}
...
export namespace CustomAgent {
	export function fromParsedPromptFile(
		ast: ParsedPromptFile,
		extra: {
			name?: string;
			description?: string;
			source: IAgentSource;
		...
		const visibility = {
			...
			agentInvocable:
				ast.header.infer !== undefined
					? ast.header.infer === true
					: ast.header.disableModelInvocation !== true,
