...
export class ExtensionPromptFileService extends Disposable {
	...
	private readonly _providerWhenClauses = new Map<
		PromptFileProviderEntry,
		readonly string[]
	>();
	...
	public readonly onDidChange: Event<IExtensionPromptFilesChangeEvent> =
	 ...
	 */
	public async getExtensionPromptFiles(
		type: PromptsType,
		token: CancellationToken,
		...
		const contributedFiles = settledResults
			.filter(
				(result): result is PromiseFulfilledResult<IExtensionPromptPath> =>
					result.status === "fulfilled",
			)
			.map((result) => result.value);

		const activationEvent = this._getProviderActivationEvent(type);
		const providerFiles = activationEvent
			? await this._listFromProviders(type, activationEvent, token)
			: [];
	...
	public registerContributedFile(
		...
		const entryPromise = (async () => {
			...
			if (type === PromptsType.skill) {
				try {
					const validated = await this._validateAndSanitizeSkillFile(
						uri,
	...
	public registerPromptFileProvider(
		...
		disposables.add({
			dispose: () => {
				const index = this._promptFileProviders.findIndex(
					(p) => p === providerEntry,
				);
				if (index >= 0) {
	...
	private async _listFromProviders(
		type: PromptsType,
	...
	private async _validateAndSanitizeSkillFile(
		uri: URI,
		token: CancellationToken,
		...
		const sanitizedDescription = description
			? this._truncateAgentSkillDescription(description, uri)
			: undefined;
		return { name: sanitizedName, description: sanitizedDescription };
	}

	private async _parsePromptFile(
		uri: URI,
		token: CancellationToken,
