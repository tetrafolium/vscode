...
export class PromptFilesLocator {
	...
	protected getWorkspaceFolders(): readonly IWorkspaceFolder[] {
		...
		return this.workspaceService
			.getWorkspace()
			.folders.filter((f) => f.uri.scheme !== AGENT_HOST_SCHEME);
	}
	 ...
	 * Subclasses can override to filter out unsupported sources.
	 */
	protected getDefaultSourceFolders(
		type: PromptsType,
	): readonly IPromptSourceFolder[] {
		return getPromptFileDefaultLocations(type);
	}

	public async getWorkspaceFolderRoots(
	...
	private async findParentRepoFolders(
		...
		while (true) {
			try {
				const isRepoRoot = await this.fileService.exists(
					joinPath(current, ".git"),
				);
				if (isRepoRoot) {
					if (
						(
				...
				logger?.logInfo(
					`No repository root found for folder ${folderUri.toString()}. Error accessing ${joinPath(current, ".git")}: ${msg}.`,
				);
	...
	public async listFiles(
		...
		const absoluteLocations = await this.toAbsoluteLocations(
			type,
			configuredLocations.filter((loc) => loc.storage === storage),
		);
	...
	public createFilesUpdatedEvent(type: PromptsType): {
			...
			for (const folder of parentFolders) {
				if (!this.getWorkspaceFolder(folder.searchRoot)) {
					...
					const recursive =
						...
						type === PromptsType.instructions; // instructions can be in subfolders, so watch recursively
					externalFolderWatchers.add(
						this.fileService.watch(folder.searchRoot, {
							recursive,
							excludes: [],
						}),
					);
		...
		const update = async () => {
			try {
				const configuredLocations = this.getPromptSourceFolders(type);
				parentFolders = await this.toAbsoluteLocations(
					type,
					configuredLocations,
					undefined,
				);
				...
				this.logService.error(
					`Error updating prompt file watchers after config change:`,
					err,
				);
			}
		};
	 ...
	 */
	public async getConfigBasedSourceFolders(
		type: PromptsType,
	): Promise<readonly URI[]> {
		const configuredLocations = this.getPromptSourceFolders(type);
		const absoluteLocations = await this.toAbsoluteLocations(
	...
	public async getResolvedSourceFolders(
		...
		const localFolders = absoluteLocations.filter(
			(loc) => loc.storage === PromptsStorage.local,
		);
		const userFolders = absoluteLocations.filter(
			(loc) => loc.storage === PromptsStorage.user,
		);
	 ...
	 */
	public async getSourceFoldersInDiscoveryOrder(
		type: PromptsType,
	): Promise<readonly IResolvedPromptSourceFolder[]> {
		const absoluteLocations = await this.getLocalStorageFolders(type);
		const userFolders = absoluteLocations.filter(
	 ...
	 */
	private async getLocalStorageFolders(
		type: PromptsType,
	): Promise<readonly IResolvedPromptSourceFolder[]> {
		const configuredLocations = this.getPromptSourceFolders(type);
		...
		const allFolders = [
			...
			...configuredLocations.filter(
				(loc) => !defaultFolders.some((df) => df.path === loc.path),
			),
		];
	 ...
	 */
	private async toAbsoluteLocations(
		type: PromptsType,
		configuredLocations: readonly IPromptSourceFolder[],
		defaultLocations?: readonly IPromptSourceFolder[],
	): Promise<IResolvedPromptSourceFolder[]> {
		const result: IResolvedPromptSourceFolder[] = [];
		const seen = new ResourceSet();
		...
		for (const sourceFolder of validLocations) {
			...
			try {
				...
				if (isTildePath(configuredLocation)) {
					...
					if (!seen.has(uri)) {
						seen.add(uri);
						const { searchRoot, filePattern } = resolveSearchLocation(
							type,
							uri,
						);
						result.push({
				...
				this.logService.error(
					`Failed to resolve prompt file location: ${configuredLocation}`,
					error,
				);
			}
		}
	...
	private async resolveFilesAtLocation(
		...
		const isWorkspaceRoot =
			depth === 0 &&
			this.getWorkspaceFolders().some((f) => isEqual(f.uri, location));
		const recursive =
			type === PromptsType.instructions &&
			!isWorkspaceRoot &&
		...
		try {
			...
			if (info.isFile) {
				...
				for (const child of info.children) {
					if (child.isFile) {
						...
						const subFiles = await this.resolveFilesAtLocation(
							child.resource,
							type,
							token,
							depth + 1,
			...
			if (
				...
				this.logService.error(
					`Failed to resolve files at location: ${location.toString()}`,
					e,
				);
			}
		}
	 ...
	 */
	public async findAgentMDsInWorkspace(
		token: CancellationToken,
	): Promise<IAgentInstructionFile[]> {
		const result = await Promise.all(
			this.getWorkspaceFolders().map((folder) =>
				this.findAgentMDsInFolder(folder.uri, token),
			),
		);
		return result.flat(1);
	}

	private async findAgentMDsInFolder(
		folder: URI,
		token: CancellationToken,
	): Promise<IAgentInstructionFile[]> {
		// Check if a FileSearchProvider is available for this scheme
		if (this.searchService.schemeHasFileSearchProvider(folder.scheme)) {
			...
			const getExcludePattern = (folder: URI) =>
				getExcludes(
					this.configService.getValue<ISearchConfiguration>({
						resource: folder,
					}),
				) || {};
			const searchOptions: IFileQuery = {
	 ...
	 */
	private async findAgentMDsUsingFileService(
		folder: URI,
		token: CancellationToken,
	): Promise<IAgentInstructionFile[]> {
			...
			try {
				...
				if (stat.isFile && stat.name.toLowerCase() === agentsMdFileName) {
					const realPath = stat.isSymbolicLink
						? await this.fileService.realpath(stat.resource)
						: undefined;
					result.push({
						uri: stat.resource,
						realPath,
				...
				this.logService.trace(
					`[PromptFilesLocator] Error traversing ${uri.toString()}: ${error}`,
				);
			}
		};
				...
				for (const child of root.stat.children) {
					if (child.isFile) {
						const matchingPath = paths.find((p) =>
							equalsIgnoreCase(p.fileName, child.name),
						);
						if (matchingPath) {
							const realPath = child.isSymbolicLink
								? await this.fileService.realpath(child.resource)
								: undefined;
							result.push({
								uri: child.resource,
								realPath,
	...
	public getAgentFileURIFromModeFile(oldURI: URI): URI | undefined {
		if (oldURI.path.endsWith(LEGACY_MODE_FILE_EXTENSION)) {
			...
			if (workspaceFolder) {
				...
				isEqualOrParent(oldURI, this.userDataService.currentProfile.promptsHome)
			) {
				newLocation = joinPath(
	...
	private async findAgentSkillsInFolder(
		uri: URI,
		token: CancellationToken,
	): Promise<URI[]> {
		try {
			...
			if (!isCancellationError(e)) {
				this.logService.trace(
					`[PromptFilesLocator] Error searching for skills in ${uri.toString()}: ${e}`,
				);
			}
			return [];
		}
 ...
 */
export const VALID_PROMPT_FOLDER_PATTERN =
	"^(?![A-Za-z]:[\\\\/])(?!/)(?!~(?!/))(?!.*\\\\)(?!.*[*?\\[\\]{}]).*\\S.*$";
