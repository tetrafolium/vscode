...
export class PromptHoverProvider implements HoverProvider {
	...
	public async provideHover(
		...
		if (promptAST.header?.range.containsPosition(position)) {
			return this.provideHeaderHover(
				position,
				promptType,
				promptAST.header,
				target,
	...
	private async provideBodyHover(
		position: Position,
		body: PromptBody,
		target: Target,
	): Promise<Hover | undefined> {
	...
	private async provideHeaderHover(
		position: Position,
		...
		for (const attribute of header.attributes) {
			if (attribute.range.containsPosition(position)) {
				const description = getAttributeDefinition(
					attribute.key,
					promptType,
					target,
				)?.description;
	...
	private getToolHover(
		...
		if (value.type === "sequence") {
			for (const toolName of value.items) {
				if (
					toolName.type === "scalar" &&
					toolName.range.containsPosition(position)
				) {
					const description = this.getToolHoverByName(
						toolName.value,
						toolName.range,
						target,
					);
	...
	private getToolHoverByName(
		...
		if (target === Target.Claude) {
			const description = knownClaudeTools.find(
		...
		const tool =
			this.languageModelToolsService.getToolByFullReferenceName(toolName);
		if (tool !== undefined) {
			if (isToolSet(tool)) {
	...
	private getToolsetHover(toolSet: IToolSet, range: Range): Hover | undefined {
		const lines: string[] = [];
		lines.push(
			localize("toolSetName", "ToolSet: {0}\n\n", toolSet.referenceName),
		);
	...
	private getModelHover(
		...
		const modelHoverContent = (modelName: string): Hover | undefined => {
			...
			if (target === Target.Claude) {
				const claudeModel = knownClaudeModels.find(
	...
	private async getAgentHover(
		agentAttribute: IHeaderAttribute,
		position: Position,
		baseMessage: string,
	): Promise<Hover | undefined> {
		const lines: string[] = [];
		...
		if (value.type === "scalar" && value.range.containsPosition(position)) {
			...
			if (agent) {
				const description =
					agent.description.get() ||
					(isBuiltinChatMode(agent)
						? localize(
								"promptHeader.prompt.agent.builtInDesc",
								"Built-in agent",
							)
						: localize("promptHeader.prompt.agent.customDesc", "Custom agent"));
			...
			// Built-in agents
			lines.push(
				localize("promptHeader.prompt.agent.builtin", "**Built-in agents:**"),
			);
			for (const agent of agents.builtin) {
				lines.push(
					`- \`${agent.name.get()}\`: ${agent.description.get() || agent.label.get()}`,
				);
			}
			...
			if (agents.custom.length > 0) {
				lines.push("");
				lines.push(
					localize("promptHeader.prompt.agent.custom", "**Custom agents:**"),
				);
				for (const agent of agents.custom) {
					const description = agent.description.get();
					lines.push(
						`- \`${agent.name.get()}\`: ${description || localize("promptHeader.prompt.agent.customDesc", "Custom agent")}`,
					);
				}
			}
		}
	...
	private getHookCommandItemHover(
		...
		for (const item of sequence.items) {
			...
			for (const field of item.properties) {
				if (
					field.key.range.containsPosition(position) ||
					field.value.range.containsPosition(position)
				) {
	...
	private getHandsOffHover(
		...
		const handoffsBaseMessage = getAttributeDefinition(
