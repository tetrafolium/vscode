...
export class PromptBodyAutocompletion implements CompletionItemProvider {
		...
		switch (reference.type) {
			case "file":
				if (reference.contentRange.containsPosition(position)) {
					...
					await this.collectFilePathCompletions(
						model,
						position,
						reference.contentRange,
						suggestions,
					);
				} else {
					await this.collectDefaultCompletions(
						model,
						reference.range,
						promptsType,
						suggestions,
			...
			case "tool":
				if (reference.contentRange.containsPosition(position)) {
					if (
						promptsType === PromptsType.agent ||
						promptsType === PromptsType.prompt
					) {
						await this.collectToolCompletions(
							model,
							position,
							reference.contentRange,
							suggestions,
	...
	private async collectToolCompletions(
		model: ITextModel,
		position: Position,
		toolRange: Range,
	...
	private async collectFilePathCompletions(
		model: ITextModel,
		position: Position,
		pathRange: Range,
		suggestions: CompletionItem[],
	): Promise<void> {
		const pathUntilPosition = model.getValueInRange(
			pathRange.setEndPosition(position.lineNumber, position.column),
		);
		const pathSeparator =
			pathUntilPosition.includes("/") || !pathUntilPosition.includes("\\")
				? "/"
				: "\\";
		...
		if (pathUntilPosition.match(/[^\/]\.\.$/i)) {
			...
			while (
				i >= 0 &&
				![CharCode.Slash, CharCode.Backslash].includes(
					pathUntilPosition.charCodeAt(i),
				)
			) {
				i--;
			}
		...
		try {
			const currentFolder = extUri.resolvePath(
				dirname(model.uri),
				parentFolderPath,
			);
			...
			if (children) {
				for (const child of children) {
					const insertText =
						(parentFolderPath || "." + pathSeparator) + child.name;
					suggestions.push({
	 ...
	 */
	private async findVariableReference(
		model: ITextModel,
		position: Position,
		token: CancellationToken,
	): Promise<{ contentRange: Range; type: string; range: Range } | undefined> {
		if (model.getLineContent(1).trimEnd() === "---") {
			let i = 2;
			while (
				i <= model.getLineCount() &&
				model.getLineContent(i).trimEnd() !== "---"
			) {
				i++;
			}
	...
	private async collectDefaultCompletions(
		model: ITextModel,
		range: Range,
		promptFileType: PromptsType,
		suggestions: CompletionItem[],
	): Promise<void> {
		const labels =
			promptFileType === PromptsType.instructions ? ["file"] : ["file", "tool"];
