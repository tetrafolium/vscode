...
import {
	formatArrayValue,
	getQuotePreference,
} from "../utils/promptEditHelper.js";
...
export class PromptHeaderAutocompletion implements CompletionItemProvider {
	...
	public async provideCompletionItems(
		...
		if (
			...
			position.lineNumber >= headerRange.endLineNumber
		) {
		...
		const colonPosition =
			colonIndex !== -1
				? new Position(position.lineNumber, colonIndex + 1)
				: undefined;
		...
		if (!colonPosition || position.isBeforeOrEqual(colonPosition)) {
			...
			let containingAttribute = header.attributes.find(
				({ range }) =>
			...
			if (!containingAttribute) {
				...
				for (let i = header.attributes.length - 1; i >= 0; i--) {
					...
					if (
						attr.range.endLineNumber < position.lineNumber &&
						attr.value.type === "map"
					) {
						const nextAttr = header.attributes[i + 1];
						const nextStartLine = nextAttr
							? nextAttr.range.startLineNumber
							: headerRange.endLineNumber;
	...
	private async provideAttributeNameCompletions(
		...
		const getInsertText = async (key: string): Promise<string> => {
			...
			if (
				key === PromptHeaderAttributes.hooks &&
		...
		for (const attribute of attributesToPropose) {
			const item: CompletionItem = {
				label: attribute,
				documentation: getAttributeDefinition(attribute, promptType, target)
					?.description,
	...
	private async provideValueCompletions(
		...
		const attribute =
			preFoundAttribute ??
			header.attributes.find(
				({ range }) =>
					range.startLineNumber <= posLineNumber &&
					posLineNumber <= range.endLineNumber,
			);
		if (!attribute) {
			return undefined;
		}
		const target = getTarget(promptType, header);
		if (
			!getValidAttributeNames(promptType, true, target).includes(attribute.key)
		) {
			return undefined;
		}
		...
		if (promptType === PromptsType.prompt || promptType === PromptsType.agent) {
			...
			if (
				attribute.key === PromptHeaderAttributes.tools ||
				attribute.key === ClaudeHeaderAttributes.disallowedTools
			) {
		...
		for (const entry of entries) {
			const item: CompletionItem = {
				...
				range: new Range(
					position.lineNumber,
					colonPosition.column + whilespaceAfterColon + 1,
					position.lineNumber,
					model.getLineMaxColumn(position.lineNumber),
				),
			};
		...
		if (
			attribute.key === PromptHeaderAttributes.hooks &&
			promptType === PromptsType.agent
		) {
			const hookSnippet = [
				"",
			...
			const item: CompletionItem = {
				...
				kind: CompletionItemKind.Snippet,
				insertText:
					whilespaceAfterColon === 0 ? ` ${hookSnippet}` : hookSnippet,
				insertTextRules: CompletionItemInsertTextRule.InsertAsSnippet,
				range: new Range(
					position.lineNumber,
					colonPosition.column + whilespaceAfterColon + 1,
	...
	private provideHookEventCompletions(
		...
		if (hookEventOnLine) {
			...
			if (colonIdx !== -1 && position.column > colonIdx + 1) {
				const whilespaceAfterColon =
					lineText.substring(colonIdx + 1).match(/^\s*/)?.[0].length ?? 0;
				...
				return {
					suggestions: [
						{
							label: localize(
								"promptHeaderAutocompletion.newCommand",
								"New Command",
							),
							documentation: localize(
								"promptHeaderAutocompletion.newCommand.description",
								"Add a new command entry to this hook.",
							),
							kind: CompletionItemKind.Snippet,
							insertText:
								whilespaceAfterColon === 0
									? ` ${commandSnippet}`
									: commandSnippet,
							insertTextRules: CompletionItemInsertTextRule.InsertAsSnippet,
							range: new Range(
								position.lineNumber,
								colonIdx + 1 + whilespaceAfterColon + 1,
								position.lineNumber,
								model.getLineMaxColumn(position.lineNumber),
							),
						},
		...
		const rangeStartColumn = isEmptyLine
			? position.column
	...
	private provideHookCommandFieldCompletions(
		...
		const fieldStart = dashPrefixMatch
			? dashPrefixMatch[1].length
			: firstNonWhitespace;
			...
			if (isInSeq || isTrailingSeq) {
				...
				if (isTrailingSeq) {
					...
					const effectiveIndent =
						firstNonWs === -1 ? position.column - 1 : firstNonWs;
	...
	private findCommandMapInSequence(
		...
		for (let i = 0; i < sequence.items.length; i++) {
			...
			const isTrailing =
				...
				(i + 1 >= sequence.items.length ||
					sequence.items[i + 1].range.startLineNumber > position.lineNumber);
	...
	private async provideArrayCompletions(
		model: ITextModel,
		position: Position,
		arrayValue: ISequenceValue,
		getValues: () => Promise<ReadonlyArray<IValueEntry>>,
	): Promise<CompletionList | undefined> {
		const getSuggestions = async (toolRange: Range, currentItem?: IValue) => {
			...
			for (const entry of entries) {
				...
				if (!toolRange.isEmpty()) {
					...
					insertText =
						firstChar === CharCode.SingleQuote
							? `'${entryName}'`
							: firstChar === CharCode.DoubleQuote
								? `"${entryName}"`
								: entryName;
				} else {
