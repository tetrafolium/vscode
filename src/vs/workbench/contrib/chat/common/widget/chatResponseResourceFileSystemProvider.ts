...
export const IChatResponseResourceFileSystemProvider =
	createDecorator<IChatResponseResourceFileSystemProvider>(
		...
		IFileSystemProviderWithFileReadStreamCapability
{
	...
	private readonly _associated = new ResourceMap<
	...
	private findMatchingInvocation(uri: URI) {
		...
		for (let k = requests.length - 1; k >= 0; k--) {
			...
			const tc = req.response?.entireResponse.value.find(
...
export class ChatResponseResourceWorkbenchContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		@IChatResponseResourceFileSystemProvider
		chatResponseResourceFsProvider: IChatResponseResourceFileSystemProvider,
		@IFileService fileService: IFileService,
	) {
		super();
		this._register(
			fileService.registerProvider(
				ChatResponseResource.scheme,
				chatResponseResourceFsProvider,
			),
		);
	}
}
