...
export interface IChatResponseModel {
	...
	resolveInlineReference(
		resolveId: string,
		resolvedReference: IChatContentInlineReference,
	): boolean;
	updateContent(
		progress:
			| IChatProgressResponseContent
...
export class Response extends AbstractResponse implements IDisposable {
	...
	public resolveInlineReference(
		...
		for (let i = 0; i < this._responseParts.length; i++) {
			...
			if (
				current.kind !== "inlineReference" ||
				current.resolveId !== resolveId
			) {
				continue;
			}
...
class InputModel implements IInputModel {
	...
	constructor(
		initialState: IChatModelInputState | undefined,
...
export class ChatModel extends Disposable implements IChatModel {
	...
	constructor(
		...
		this._workingDirectory =
			isValidFullData && initialData.workingDirectory
				? URI.parse(initialData.workingDirectory)
				: undefined;
...
function _logChangesToStateModel(
	newState: Partial<IChatModelInputState> | undefined,
	oldState: Partial<IChatModelInputState> | undefined,
	logger: ILogService,
) {
	if (
		!canLog(logger.getLevel(), LogLevel.Info) ||
		newState?.selectedModel?.identifier === oldState?.selectedModel?.identifier
	) {
		return;
	}
...
export function logChangesToStateModel(
	model: IInputModel | undefined,
	message: string,
	newState: Partial<IChatModelInputState> | undefined,
	oldState: Partial<IChatModelInputState> | undefined,
	logger: ILogService,
