...
export function checkModeOption(
	mode: ChatModeKind,
	option: boolean | ((mode: ChatModeKind) => boolean) | undefined,
): boolean | undefined {
	if (option === undefined) {
		return undefined;
	}
 ...
 */
export function migrateLegacyTerminalToolSpecificData(
	data: IChatTerminalToolInvocationData | ILegacyChatTerminalToolInvocationData,
...
export async function awaitStatsForSession(
	model: IChatModel,
): Promise<IChatSessionStats | undefined> {
	...
	await Promise.all(
		model.editingSession.entries.get().map((entry) => entry.getDiffInfo?.()),
	);
	if (!model.editingSession) {
		return undefined;
	}
	...
	const reduceResult = diffs.reduce(
		(acc, diff) => {
	...
	if (
		reduceResult.fileUris.size > 0 &&
		(reduceResult.added > 0 || reduceResult.removed > 0)
	) {
		return {
