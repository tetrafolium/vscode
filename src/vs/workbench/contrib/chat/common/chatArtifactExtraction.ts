...
import {
	...
	IChatArtifact,
} from "./tools/chatArtifactsService.js";
...
function isToolResultOutputDetailsSerialized(
	...
	return (
		...
		typeof (obj as IToolResultOutputDetailsSerialized).output?.mimeType ===
			"string"
	);
}
				...
				if (rule) {
					...
					if (!seenUris.has(key)) {
						...
						artifacts.push({
							...
							uri: ChatMemoryFileResource.createUri(
								memoryPath,
								sessionResource,
							).toString(),
							type: "plan",
					...
					if (
						...
						if (rule) {
							...
							if (!seenUris.has(key)) {
								...
								const artifactUri = ChatResponseResource.createUri(
									sessionResource,
									part.toolCallId,
									i,
									permalinkBasename,
