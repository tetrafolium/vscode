...
const LEGACY_MARKETPLACE_INDEX_STORAGE_KEY =
...
export class FileBackedInstalledPluginsStore extends Disposable {
	private readonly _installed = observableValue<
		readonly IStoredInstalledPlugin[]
	>("file/installed.json", []);
	...
	set(
		newValue: readonly IStoredInstalledPlugin[],
	...
	private async _readFromFile(): Promise<
		readonly IStoredInstalledPlugin[] | undefined
	> {
		try {
			...
			return json.installed
				.filter(
					(entry): entry is IInstalledJsonEntry =>
						typeof entry.pluginUri === "string" &&
	...
	private async _migrateFromStorage(): Promise<void> {
		...
		try {
			...
			const migrated: IStoredInstalledPlugin[] = (
				revive(parsed) as {
					pluginUri: UriComponents;
					plugin?: {
						name?: string;
						marketplaceReference?: { rawValue?: string };
					};
				}[]
			)
				.map((entry) => {
	...
	private _rebasePluginUri(uri: URI): URI | undefined {
		...
		if (
			!isEqual(uri, oldRoot) &&
			uri.scheme === oldRoot.scheme &&
			uri.path.startsWith(oldRoot.path + "/")
		) {
