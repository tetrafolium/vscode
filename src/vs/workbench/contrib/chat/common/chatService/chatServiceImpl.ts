...
import {
	...
	ChatPendingRequestChangeClassification,
	ChatPendingRequestChangeEvent,
	ChatPendingRequestChangeEventName,
	ChatRequestQueueKind,
	ChatSendResult,
	ChatSendResultQueued,
	ChatSendResultSent,
	ChatStopCancellationNoopClassification,
	ChatStopCancellationNoopEvent,
...
} from "./chatServiceTelemetry.js";
import {
	IChatSessionsService,
	isAgentHostTarget,
	localChatSessionType,
} from "../chatSessionsService.js";
import {
	ChatSessionStore,
	IChatSessionEntryMetadata,
} from "../model/chatSessionStore.js";
...
} from "../promptSyntax/promptTypes.js";
...
class CancellableRequest implements IDisposable {
	private readonly _yieldRequested: ISettableObservable<boolean> =
...
export class ChatService extends Disposable implements IChatService {
	...
	private readonly _pendingRequests = this._register(
		new DisposableResourceMap<CancellableRequest>(),
	);
	private readonly _queuedRequestDeferreds = new Map<
		string,
		DeferredPromise<ChatSendResult>
	>();
	...
	public readonly onDidReceiveQuestionCarouselAnswer =
		this._onDidReceiveQuestionCarouselAnswer.event;

	private readonly _onDidDisposeSession = this._register(
		new Emitter<{ readonly sessionResources: URI[]; reason: "cleared" }>(),
	);
	public readonly onDidDisposeSession = this._onDidDisposeSession.event;

	private readonly _sessionFollowupCancelTokens = this._register(
		new DisposableResourceMap<CancellationTokenSource>(),
	);
	...
	constructor(
		...
		this._sessionModels = this._register(
			instantiationService.createInstance(ChatModelStore, {
				createModel: (props: IStartSessionProps) => this._startSession(props),
				willDisposeModel: async (model: ChatModel) => {
					const localSessionId = LocalChatSessionUri.parseLocalSessionId(
					...
					if (localSessionId && this.shouldStoreSession(model)) {
						...
						!localSessionId &&
						(model.getRequests().length > 0 || hasDraftInput(model))
					) {
		...
		this._register(
			this._sessionModels.onDidDisposeModel((model) => {
		...
		this.chatModels = derived(this, (reader) => [
			...this._sessionModels.observable.read(reader).values(),
		]);
	...
	public get editingSessions() {
		return [...this._sessionModels.values()]
			.map((v) => v.editingSession)
			.filter(isDefined);
	}
	...
	private saveState(): void {
		...
		const liveLocalChats = Array.from(this._sessionModels.values()).filter(
			(session) => this.shouldStoreSession(session),
		);

		const liveNonLocalChats = Array.from(this._sessionModels.values()).filter(
			(session) =>
				!LocalChatSessionUri.parseLocalSessionId(session.sessionResource),
		);
		...
		this._chatSessionStore.updateAndFlushIndexSync(
			liveLocalChats,
			liveNonLocalChats,
		);
	...
	private shouldStoreSession(session: ChatModel): boolean {
		...
		return (
			session.initialLocation === ChatAgentLocation.Chat && !session.isImported
		);
	}
			...
			.map((entry): IChatDetail => {
				...
				return {
					...
					workingDirectory: workingDirectoryStr
						? URI.parse(workingDirectoryStr)
						: undefined,
				};
			});
	}

	async getMetadataForSession(
		sessionResource: URI,
	): Promise<IChatDetail | undefined> {
		const index = await this._chatSessionStore.getIndex();
		const metadata: IChatSessionEntryMetadata | undefined =
			index[sessionResource.toString()];
		if (metadata) {
	...
	async removeHistoryEntry(sessionResource: URI): Promise<void> {
		await this._chatSessionStore.deleteSession(
			this.toLocalSessionId(sessionResource),
		);
		const model = this._sessionModels.get(sessionResource);
		if (model) {
			model.markDeleted();
	...
	private initializeSession(model: ChatModel): void {
		this.trace(
			"initializeSession",
			`Initialize session ${model.sessionResource}`,
		);
	...
	acquireExistingSession(
		...
		return this._sessionModels.acquireExisting(
			sessionResource,
			debugOwner ?? "ChatService#acquireExistingSession",
		);
	}
	...
	private async acquireOrRestoreLocalSession(
		sessionResource: URI,
		debugOwner?: string,
	): Promise<IChatModelReference | undefined> {
		this.trace("acquireOrRestoreSession", `${sessionResource}`);
		const existingRef = this.acquireExistingSession(
			sessionResource,
			debugOwner,
		);
		if (existingRef) {
		...
		if (isEqual(this.transferredSessionResource, sessionResource)) {
			...
			sessionData =
				await this._chatSessionStore.readTransferredSession(sessionResource);
		} else {
			const localSessionId =
				LocalChatSessionUri.parseLocalSessionId(sessionResource);
			if (localSessionId) {
	...
	getSessionTitle(sessionResource: URI): string | undefined {
		...
		return (
			this._sessionModels.get(sessionResource)?.title ??
	...
	loadSessionFromData(
		...
		const sessionId =
			(data as ISerializableChatData).sessionId ?? generateUuid();
	...
	private async loadRemoteSession(
		sessionResource: URI,
		location: ChatAgentLocation,
		token: CancellationToken,
		debugOwner?: string,
		...
		if (
			!(await this.chatSessionService.canResolveChatSession(
		...
		const providedSession =
			await this.chatSessionService.getOrCreateChatSession(
				sessionResource,
				token,
			);
		...
		const agentUri = findLast(
			providedSession.history.filter((m) => m.type === "request"),
			(req) => req.modeInstructions?.uri,
		)?.modeInstructions?.uri;
		const storedMetadata =
			this._chatSessionStore.getMetadataForSessionSync(sessionResource);
		...
		if (modelId || agentUri) {
			const mode: ISerializableChatModelInputState["mode"] = agentUri
				? { kind: ChatModeKind.Agent, id: agentUri.toString() }
				: { kind: ChatModeKind.Agent, id: ChatMode.Agent.id };
			const modelMetadata = modelId
				? this.languageModelsService.lookupLanguageModel(modelId)
				: undefined;
			const selectedModel: ISerializableChatModelInputState["selectedModel"] =
				modelId && modelMetadata
					? { identifier: modelId, metadata: modelMetadata }
					: undefined;
		...
		const modelRef = this._sessionModels.acquireOrCreate(
			{
				...
				inputState:
					providedSession.transferredState?.inputState ?? storedInputState,
			},
		...
		const requestParser = isAgentHostSession
			? this.instantiationService.createInstance(ChatRequestParser)
			: undefined;
		const parseAgentHostHistoryPrompt = (
			text: string,
			...
			if (requestParser) {
				try {
					const attachmentCapabilities =
						this.getAttachmentCapabilitiesForParser(chatSessionType, agent);
					...
					this.logService.warn(
						`ChatService#loadRemoteSession: failed to re-parse historical prompt for ${chatSessionType}`,
						e,
					);
				}
			}
			...
			if (message.type === "request") {
				...
				const modeInfo = message.modeInstructions
					? ({
						...
						} satisfies IChatRequestModeInfo)
					: undefined;
		...
		const hasProgressStreaming =
			providedSession.progressObs &&
			providedSession.interruptActiveResponseCallback;
		if (hasProgressStreaming) {
			...
			const createCancellationListener = (token: CancellationToken) => {
				return token.onCancellationRequested(() => {
					providedSession
						...
						.then((userConfirmedInterruption) => {
			...
			const trackNewCancellableRequest = () => {
				const cancellableRequest = this.instantiationService.createInstance(
					CancellableRequest,
				...
				cancellationListener.value = createCancellationListener(
					cancellableRequest.cancellationTokenSource.token,
				);
			};
			...
			if (providedSession.onDidStartServerRequest) {
				disposables.add(
					providedSession.onDidStartServerRequest(
						({
							prompt,
							variableData,
							isSystemInitiated,
			...
			disposables.add(
				autorun((reader) => {
					...
					const isComplete =
						providedSession.isCompleteObs?.read(reader) ?? false;
	...
	async sendRequest(
		...
		this.trace(
			"sendRequest",
			`sessionResource: ${sessionResource.toString()}, message: ${request.substring(0, 20)}${request.length > 20 ? "[...]" : ""}}`,
		);

		const hasExplicitFileOrImageAttachment = [
			...(options?.attachedContext ?? []),
			...(options?.resolvedVariables ?? []),
		].some(isExplicitFileOrImageVariableEntry);
		if (
			!request.trim() &&
			!hasExplicitFileOrImageAttachment &&
		...
		try {
			...
			if (
				...
				const initialSessionOptions =
					this.chatSessionService.getSessionOptions(sessionResource);
				...
				if (newItem) {
					...
					this.chatSessionService.registerSessionResourceAlias(
						sessionResource,
						newItem.resource,
					);

					tempRef = await this.loadRemoteSession(
						newItem.resource,
					...
					if (!model) {
						throw new Error(
							`Failed to load session for resource: ${newItem.resource}`,
						);
					}
					...
					if (initialSessionOptions) {
						this.chatSessionService.updateSessionOptions(
							model.sessionResource,
							initialSessionOptions,
						);
					}
			...
			if (options?.queue) {
				const queued = this.queuePendingRequest(
					model,
					sessionResource,
					request,
					options,
				...
				this.trace(
					"sendRequest",
					`Session ${sessionResource} already has a pending request`,
				);
			...
			const silentAgent = options?.agentIdSilent
				? this.chatAgentService.getAgent(options.agentIdSilent)
				: undefined;
			const agent =
				silentAgent ??
	...
	private getAttachmentCapabilitiesForParser(
		...
		return (
			this.chatSessionService.getCapabilitiesForSessionType(chatSessionType) ??
			agent?.capabilities
		);
	}
	...
	private parseChatRequest(
		...
		let contextAgent =
			parserContext?.forcedAgent ?? parserContext?.selectedAgent;
		if (options?.agentId) {
		...
		const attachmentCapabilities =
			parserContext?.attachmentCapabilities ??
			this.getAttachmentCapabilitiesForParser(
				getChatSessionType(sessionResource),
				contextAgent,
			);
		...
		const parsedRequest = this.instantiationService
			...
			.parseChatRequest(sessionResource, request, location, parserContext);
	...
	private _sendRequestAsync(
		...
		const followupsCancelToken =
			this.refreshFollowupsCancellationToken(sessionResource);
		...
		const requests = [...model.getRequests()];
		const requestTelemetry = this.instantiationService.createInstance(
			ChatRequestTelemetry,
			{
		...
		const sendRequestInternal = async () => {
			...
			{
				const fileLoggingEnabled = this.configurationService.getValue<boolean>(
					AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING,
				);
				if (!fileLoggingEnabled) {
					const isTroubleshootCommand =
					...
					const hasTroubleshootSkill = options?.attachedContext?.some((v) => {
						...
						return (
							uri &&
							(uri.scheme === COPILOT_SKILL_URI_SCHEME ||
								uri.path.includes(TROUBLESHOOT_SKILL_PATH))
						);
					});
					if (isTroubleshootCommand || hasTroubleshootSkill) {
						...
						const settingsArg = encodeURIComponent(
							JSON.stringify(AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING),
						);
			...
			const collectHooks = async (): Promise<{
				...
				if (agentName) {
					try {
						...
						this.logService.warn(
							"[ChatService] Failed to collect agent hooks:",
							error,
						);
					}
				}
			...
			const collectInstructions = async (): Promise<
				IChatRequestVariableEntry[]
			> => {
				...
				try {
					...
					const originalIds = new Set(
						(options?.attachedContext ?? []).map((v) => v.id),
					);
					return variableSet.asArray().filter((v) => !originalIds.has(v.id));
				} catch (err) {
					this.logService.error(
						"[ChatService] Failed to collect instructions:",
						err,
					);
					return [];
				} finally {
			...
			try {
				...
				if (agentPart || (defaultAgent && !commandPart)) {
					...
					const storedVariables = allContext.filter(
						(v) => !(isPromptTextVariableEntry(v) && v.automaticallyAdded),
					);
					...
					const buildAgentRequest = (
						...
						const agentRequest: IChatAgentRequest = {
							...
							hasHooksEnabled:
								!!collectedHooks &&
								Object.values(collectedHooks).some((arr) => arr.length > 0),
							isSystemInitiated: options?.isSystemInitiated,
						...
						store.add(
							autorun((reader) => {
								...
								if (tools && request) {
									this.chatAgentService.setRequestTools(
										agent.id,
										request.id,
										tools,
									);
					...
					if (
						...
						const chatAgentRequest = buildAgentRequest(
							defaultAgent,
							undefined,
							enableCommandDetection,
							false,
					...
					const requestProps = buildAgentRequest(
						agent,
						command,
						enableCommandDetection,
						!!detectedAgent,
					);
					this.generateInitialChatTitleIfNeeded(
						model,
						requestProps,
						defaultAgent,
						token,
						...
						store.add(
							autorun((reader) => {
								const yieldRequested =
									pendingRequest.yieldRequested.read(reader);
								if (request) {
									this.chatAgentService.setYieldRequested(
										agent.id,
										request.id,
										yieldRequested,
									);
					...
					const disabledClaudeHooksDismissedKey =
					...
					if (model.canUseTools) {
						const autostartResult = new ChatMcpServersStarting(
				...
				if (token.isCancellationRequested && !rawResult) {
					...
					shouldProcessPending =
						...
						!request.response?.response.value.some(
							(v) => v.kind === "confirmation" && !v.isUsed,
						);
					request.response?.complete();
		...
		rawResponsePromise.finally(() => {
			...
			if (
				this._pendingRequests.get(model.sessionResource) === cancellableRequest
			) {
				this._pendingRequests.deleteAndDispose(model.sessionResource);
				this.telemetryService.publicLog2<
					ChatPendingRequestChangeEvent,
				...
				>(ChatPendingRequestChangeEventName, {
					...
					requestId: cancellableRequest.requestId,
					chatSessionId: chatSessionResourceToId(model.sessionResource),
				});
			}
		...
		if (options?.userSelectedModelId) {
			this.languageModelsService.addToRecentlyUsedList(
				options.userSelectedModelId,
			);
		}
	...
	private processNextPendingRequest(model: ChatModel): void {
		...
		const nextQueued =
			steeringRequests.length === 0 ? model.dequeuePendingRequest() : undefined;
		...
		const terminalIds = new Set(
			allRequests
				.map((req) => req.sendOptions.terminalExecutionId)
		...
		const mergedTerminalExecutionId =
			terminalIds.size === 1 ? [...terminalIds][0] : undefined;

		const sendOptions: IChatSendRequestOptions = {
			...firstRequest.sendOptions,
			terminalExecutionId: mergedTerminalExecutionId,
			attachedContext: allRequests.flatMap((req) =>
				req.request.variableData.variables.slice(),
			),
		};

		const location =
			sendOptions.location ??
			sendOptions.locationData?.type ??
			model.initialLocation;
		const defaultAgent = this.chatAgentService.getDefaultAgent(
			location,
			sendOptions.modeInfo?.kind,
		);
		if (!defaultAgent) {
			this.logService.warn(
				"processNextPendingRequest",
				`No default agent for location ${location}`,
			);
		...
		try {
			if (allRequests.length > 1) {
				const combinedText = allRequests
					.map((req) => req.request.message.text)
					.join("\n\n");
				...
				parsedRequest = this.parseChatRequest(
					model.sessionResource,
					combinedText,
					location,
					{
			...
			this.logService.error(
				"processNextPendingRequest: failed to parse combined chat request",
				err,
			);
		...
		const silentAgent = sendOptions.agentIdSilent
			? this.chatAgentService.getAgent(sendOptions.agentIdSilent)
			: undefined;
		const agent =
			silentAgent ??
			parsedRequest.parts.find(
		...
		const responseState = this._sendRequestAsync(
			model,
			model.sessionResource,
			parsedRequest,
	...
	private generateInitialChatTitleIfNeeded(
		...
		const generate = async () => {
			const title = await this.chatAgentService.getChatTitle(
				defaultAgent.id,
				singleEntryHistory,
				token,
			);
		...
		if (request.response && !request.response.isComplete) {
			...
			if (cts) {
				...
				>(ChatPendingRequestChangeEventName, {
					...
					source: "adoptRequest",
					requestId: request.id,
					chatSessionId: chatSessionResourceToId(oldOwner.sessionResource),
				});
				this.telemetryService.publicLog2<
					ChatPendingRequestChangeEvent,
					ChatPendingRequestChangeClassification
				>(ChatPendingRequestChangeEventName, {
					...
					source: "adoptRequest",
					requestId: request.id,
					chatSessionId: chatSessionResourceToId(target.sessionResource),
				});
			}
		}
	}
		...
		const parsedRequest =
			typeof message === "string"
				? this.instantiationService
						.createInstance(ChatRequestParser)
						.parseChatRequest(sessionResource, message)
				: message;
	...
	removePendingRequest(sessionResource: URI, requestId: string): void {
		const model = this._sessionModels.get(sessionResource) as
			| ChatModel
			| undefined;
		if (model) {
			...
			const hasSteeringRequests = model
				.getPendingRequests()
				.some((r) => r.kind === ChatRequestQueueKind.Steering);
		...
		if (deferred) {
			deferred.complete({
				kind: "rejected",
				reason: "Request was removed from queue",
			});
	...
	async transferChatSession(
		...
		const model = this._sessionModels.get(transferredSessionResource) as
			| ChatModel
			| undefined;
		if (!model) {
			throw new Error(
	...
	appendProgress(request: IChatRequestModel, progress: IChatProgress): void {
		...
		if (!(request instanceof ChatRequestModel)) {
			throw new BugIndicatingError(
				"Can only append progress to requests of type ChatRequestModel",
			);
		}
	...
	private toLocalSessionId(sessionResource: URI) {
		...
		if (!localSessionId) {
			throw new Error(
				`Invalid local chat session resource: ${sessionResource}`,
			);
		}
...
export async function chatModelToChatDetail(
	model: IChatModel,
): Promise<IChatDetail> {
	...
	return {
		...
		lastResponseState:
			model.lastRequest?.response?.state ?? ResponseModelState.Pending,
		workingDirectory: model.workingDirectory,
	};
}
