...
export class ChatRequestParser {
	...
	parseChatRequest(
		...
		const references =
			this.variableService.getDynamicVariables(sessionResource); // must access this list before any async calls
		const selectedToolAndToolSets =
			this.variableService.getSelectedToolAndToolSets(sessionResource);
		...
		return this.parseChatRequestWithReferences(
			references,
	...
	parseChatRequestWithReferences(
		...
		selectedToolAndToolSets: IToolAndToolSetEnablementMap,
		message: string,
		location: ChatAgentLocation = ChatAgentLocation.Chat,
		context?: IChatParserContext,
	): IParsedChatRequest {
	...
	private tryToParseVariable(
		...
		toolSetsByName: ReadonlyMap<string, IToolSet>,
	...
	private tryToParseSlashCommand(
		...
		const usedAgent =
			...
			)?.agent ?? (context?.forcedAgent ? context.forcedAgent : undefined);
		if (usedAgent) {
			const subCommand = usedAgent.slashCommands.find(
				(c) => c.name === command,
			);
			if (subCommand) {
		...
		const capabilities =
			context?.attachmentCapabilities ?? usedAgent?.capabilities;
		const slashCommands = this.slashCommandService.getCommands(
			location,
			context?.mode ?? ChatModeKind.Ask,
		);
		const slashCommand = slashCommands.find(
			(c) =>
				c.command === command &&
				matchesSessionType(c.sessionTypes, context?.sessionType),
		);
		...
		if (
			!usedAgent ||
			slashCommand?.silent ||
			capabilities?.supportsPromptAttachments
		) {
			if (slashCommand) {
			...
			if (subMatch) {
				...
				if (this.promptsService.hasPromptSlashCommand(candidate)) {
					...
					const extendedRange = new OffsetRange(
						offset,
