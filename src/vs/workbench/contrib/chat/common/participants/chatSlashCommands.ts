	...
	implements IChatSlashCommandService
{
	...
	private commandsOverlap(
		dataA: IChatSlashData,
		dataB: IChatSlashData,
	): boolean {
		if (dataA.sessionTypes === undefined || dataB.sessionTypes === undefined) {
			return true;
		}

		return dataA.sessionTypes.some((sessionType) =>
			dataB.sessionTypes?.includes(sessionType),
		);
	}
	...
	private getCommand(
		...
		return this.getSessionScopedCommands(id).find((candidate) =>
			matchesSessionType(candidate.data.sessionTypes, sessionType),
		);
	}
	...
	registerSlashCommand(
		...
		if (
			commandsForId.some((candidate) =>
				this.commandsOverlap(candidate.data, data),
			)
		) {
