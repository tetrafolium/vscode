...
{
	private readonly localToDispose: DisposableStore = this._register(
		new DisposableStore(),
	);
	...
	constructor(
		...
		this._commentWidgetVisible =
			CommentContextKeys.commentWidgetVisible.bindTo(contextKeyService);
		...
		this._commentingRangeDecorator = this._register(
			new CommentingRangeDecorator(),
		);
		this._register(
	...
	public onModelChanged(): void {
		...
		this.localToDispose.add(
			this.commentService.onDidUpdateCommentThreads(async (e) => {
				...
				const pending = e.pending.filter((pending) =>
					this.uriIdentityService.extUri.isEqual(pending.uri, editorURI),
				);
	...
	private async displayCommentThread(
		...
		this.localToDispose.add(
			zoneWidget.onDidChangeExpandedState(() =>
				this._updateCommentWidgetVisibleContext(),
			),
		);
		this.localToDispose.add(
	...
	private onEditorMouseDown(e: IEditorMouseEvent): void {
		this.mouseDownInfo =
			(e.target.element?.className.indexOf("comment-range-glyph") ?? -1) >= 0
