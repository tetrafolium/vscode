...
export class CommentNode<T extends IRange | ICellRange> extends Disposable {
	...
	private readonly _md: MutableDisposable<IRenderedMarkdown> = this._register(
	...
	private updateCommentBody(body: string | IMarkdownString) {
		...
		if (typeof body === "string") {
			...
			this._md.value = this.markdownRendererService.render(
				body,
				this.markdownRendererOptions,
			);
	...
	private get commentNodeContext(): [
		{
			thread: languages.CommentThread<T>;
	...
	focus() {
		...
		this._focusClearTimer.setIfNotSet(
