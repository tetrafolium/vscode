...
export class CommentsPanel extends FilterViewPane implements ICommentsView {
	...
	constructor(
		...
		const stateMemento = new Memento<CommentsViewState>(
			VIEW_STORAGE_ID,
			storageService,
		);
	...
	private createTree(): void {
				...
				this.treeContainer,
				{
					...
					sorter: {
						compare: (a: CommentsTreeNode, b: CommentsTreeNode) => {
							...
							if (
								this.filters.sortBy === CommentsSortOrder.UpdatedAtDescending
							) {
								return a.lastUpdatedAt > b.lastUpdatedAt ? -1 : 1;
							} else if (
								this.filters.sortBy === CommentsSortOrder.ResourceAscending
							) {
								if (
									...
									if (
										a.resource.scheme !== b.resource.scheme &&
										(a.resource.scheme === workspaceScheme ||
											b.resource.scheme === workspaceScheme)
									) {
		...
		this._register(
			this.tree.onDidFocus(() => this.commentsFocusedContextKey.set(true)),
		);
		this._register(
			this.tree.onDidBlur(() => this.commentsFocusedContextKey.set(false)),
		);
	}
