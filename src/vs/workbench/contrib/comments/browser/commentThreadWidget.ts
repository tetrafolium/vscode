	...
	implements ICommentThreadWidget
{
	...
	constructor(
		...
		this._register(
			registerNavigableContainer({
				...
				focusPreviousWidget: () => {
					if (
						this._commentReply?.isCommentEditorFocused() &&
		...
		this._commentThreadContextValue =
			CommentContextKeys.commentThreadContext.bindTo(this._contextKeyService);
		this._commentThreadContextValue.set(_commentThread.contextValue);

		const commentControllerKey =
			CommentContextKeys.commentControllerContext.bindTo(
				this._contextKeyService,
			);
	...
	get hasUnsubmittedComments(): boolean {
		return (
			!!this._commentReply?.commentEditor.getValue() ||
			this._body.hasCommentsInEditMode()
		);
	}
	...
	async submitComment() {
		...
		if (activeComment) {
			...
			(this._commentReply?.getPendingComment()?.body.length ?? 0) > 0
		) {
