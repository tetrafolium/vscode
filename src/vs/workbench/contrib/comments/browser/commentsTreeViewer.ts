...
class CommentsModelVirtualDelegate implements IListVirtualDelegate<
	ResourceWithCommentThreads | CommentNode
> {
...
export class CommentNodeRenderer implements IListRenderer<
	ITreeNode<CommentNode>,
...
> {
	...
	renderElement(
		...
		templateData.threadMetadata.icon.classList.remove(
			...Array.from(templateData.threadMetadata.icon.classList.values()).filter(
				(value) => value.startsWith("codicon"),
			),
		);
		// Check if any comment in the thread has draft state
		const hasDraft = node.element.thread.comments?.some(
			(comment) => comment.state === CommentState.Draft,
		);
		templateData.threadMetadata.icon.classList.add(
			...ThemeIcon.asClassNameArray(
				this.getIcon(node.element.threadState, hasDraft),
			),
		);
		...
		if (node.element.range) {
			if (
				node.element.range.startLineNumber === node.element.range.endLineNumber
			) {
...
export interface ICommentsListOptions extends IWorkbenchAsyncDataTreeOptions<
	any,
	any
> {
...
export class Filter implements ITreeFilter<
	ResourceWithCommentThreads | CommentNode,
	FilterData
> {
