...
registerAction2(
	class Collapse extends ViewAction<CommentsPanel> {
		constructor() {
			super({
...
registerAction2(
	class Expand extends ViewAction<CommentsPanel> {
		constructor() {
			super({
				viewId: COMMENTS_VIEW_ID,
				id: "comments.expand",
	...
	class Reply extends Action2 {
		...
		override run(
			...
			revealCommentThread(
				...
				marshalledCommentThread.thread.comments![
					marshalledCommentThread.thread.comments!.length - 1
				],
				true,
			);
		}
	...
	implements IWorkbenchContribution
{
	...
	private onCommentsUpdated(): void {
		let unresolved = 0;
		for (const resource of this._commentService.commentsModel
