...
const CopilotAnonymousButton = Button(
	localize("setupCopilotButton.setup", "Use AI Features"),
	`command:workbench.action.chat.triggerSetupAnonymousWithoutDialog`,
);
...
const CopilotCompleteButton = Button(
	localize("setupCopilotButton.chatWithCopilot", "Start to Chat"),
		...
		content: {
			...
			steps: [
				createCopilotSetupStep(
					...
					"chatAnonymous && !chatSetupCompleted",
					true,
				),
				createCopilotSetupStep(
				...
				createCopilotSetupStep(
					...
					"!chatEntitlementSignedOut && (!chatSetupCompleted || chatSetupDisabled || chatPlanCanSignUp)",
					false,
				),
				{
					id: "pickColorTheme",
			...
			steps: [
				...
				},
				{
					...
					description: localize(
						...
						Button(
							localize("toggleDictation", "Voice: Start Dictation in Editor"),
