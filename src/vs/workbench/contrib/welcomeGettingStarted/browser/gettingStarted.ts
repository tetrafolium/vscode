...
export class GettingStartedPage extends EditorPane {
	...
	private readonly detailsScrollbar = this._register(
		new MutableDisposable<DomScrollableElement>(),
	);
	...
	private readonly recentlyOpenedList = this._register(
		new MutableDisposable<GettingStartedIndexList<RecentEntry>>(),
	);
	private readonly startList = this._register(
		new MutableDisposable<GettingStartedIndexList<IWelcomePageStartEntry>>(),
	);
	private readonly gettingStartedList = this._register(
		new MutableDisposable<GettingStartedIndexList<IResolvedWalkthrough>>(),
	);
		...
		this._register(
			this.gettingStartedService.onDidProgressStep((step) => {
				...
				if (category.id === this.currentWalkthrough?.id) {
					...
					if (step.done) {
						status(
							localize("stepAutoCompleted", "Step {0} completed", step.title),
						);
					}
				}
	...
	override async setInput(
		...
		const selectedCategory =
			options?.selectedCategory ?? newInput.selectedCategory;
			...
			if (!this.currentWalkthrough) {
				...
				if (this.currentWalkthrough) {
					this.buildCategorySlide(
						editorInput.selectedCategory,
						editorInput.selectedStep,
						preserveFocus,
					);
	...
	private buildRecentlyOpenedList(): GettingStartedIndexList<RecentEntry> {
		const renderRecent = (recent: RecentEntry) => {
			...
			const deleteButton = $(
				"a.codicon.codicon-close.hide-category-button.recently-opened-delete-button",
				{
					tabindex: 0,
					role: "button",
					title: localize(
		...
		const recentlyOpenedList = (this.recentlyOpenedList.value =
	...
	private filterRecentlyOpened(
		workspaces: (IRecentFolder | IRecentWorkspace)[],
	): RecentEntry[] {
		return workspaces
			.filter(
				(recent) =>
	...
	private buildGettingStartedWalkthroughsList(): GettingStartedIndexList<IResolvedWalkthrough> {
		...
		const gettingStartedList = (this.gettingStartedList.value =
			new GettingStartedIndexList({
	...
	private focusSideEditorGroup() {
		...
		if (this.groupsService.count === 1) {
			const editorGroupSplitDirection = preferredSideBySideGroupDirection(
				this.configurationService,
			);
			const sideGroup = this.groupsService.addGroup(
				this.groupsService.groups[0],
				editorGroupSplitDirection,
			);
	...
	private buildCategorySlide(
		...
		this.detailsPageDisposables.add(
			this.contextService.onDidChangeContext((e) => {
				if (
					e.affectsSome(contextKeysToWatch) &&
					this.currentWalkthrough &&
					this.editorInput
				) {
					buildStepList();
		...
		this.inProgressScroll = this.inProgressScroll.then(async () => {
			if (
				...
				if (
					...
					this.gettingStartedList.value?.itemCount
				) {
	...
	private setSlide(
		...
		if (toEnable === "categories") {
			...
			prevButton!.style.display =
				this.editorInput?.showWelcome ||
				this.editorInput?.returnToCommand ||
				this.prevWalkthrough
					? "block"
					: "none";
			// eslint-disable-next-line no-restricted-syntax
