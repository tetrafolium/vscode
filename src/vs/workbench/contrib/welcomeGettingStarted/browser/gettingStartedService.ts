...
export interface IWalkthroughStep {
	...
	media:
		...
		| {
				type: "video";
				path: { hcDark: URI; hcLight: URI; light: URI; dark: URI };
				poster?: { hcDark: URI; hcLight: URI; light: URI; dark: URI };
				root: URI;
				altText: string;
		  };
}
...
export interface IResolvedWalkthroughStep
	extends IWalkthroughStep, StepProgress {}
	...
	implements IWalkthroughsService
{
	...
	private readonly _onDidAddWalkthrough = this._register(
		new Emitter<IResolvedWalkthrough>(),
	);
	readonly onDidAddWalkthrough: Event<IResolvedWalkthrough> =
		this._onDidAddWalkthrough.event;
	private readonly _onDidRemoveWalkthrough = this._register(
		new Emitter<string>(),
	);
	readonly onDidRemoveWalkthrough: Event<string> =
		this._onDidRemoveWalkthrough.event;
	private readonly _onDidChangeWalkthrough = this._register(
		new Emitter<IResolvedWalkthrough>(),
	);
	readonly onDidChangeWalkthrough: Event<IResolvedWalkthrough> =
		this._onDidChangeWalkthrough.event;
	private readonly _onDidProgressStep = this._register(
	...
	private registerWalkthroughs() {
		walkthroughs.forEach(async (category, index) => {
			this._registerWalkthrough({
				...
				when:
					ContextKeyExpr.deserialize(category.when) ?? ContextKeyExpr.true(),
				steps: category.content.steps.map((step, index) => {
					return {
						...
						order: index,
						when:
							ContextKeyExpr.deserialize(step.when) ?? ContextKeyExpr.true(),
						media:
	...
	private async registerExtensionWalkthroughContributions(
		...
		await Promise.all(
			extension.contributes?.walkthroughs?.map(async (walkthrough, index) => {
				...
				if (
					...
					this.contextService.contextMatchesRules(
						ContextKeyExpr.deserialize(override ?? walkthrough.when) ??
							ContextKeyExpr.true(),
					)
				) {
				...
				const steps = (walkthrough.steps ?? []).map((step, index) => {
					...
					const fullyQualifiedID =
						extension.identifier.value + "#" + walkthrough.id + "#" + step.id;
					...
					return {
						...
						media,
						completionEvents:
							step.completionEvents?.filter((x) => typeof x === "string") ?? [],
						id: fullyQualifiedID,
						title: step.title,
						when:
							ContextKeyExpr.deserialize(step.when) ?? ContextKeyExpr.true(),
				...
				if (walkthrough.featuredFor) {
					...
					isFeatured = await this.instantiationService.invokeFunction((a) =>
						checkGlobFileExists(
							a,
							folders,
				...
				const walkthoughDescriptor: IWalkthrough = {
					...
					icon: iconStr
						? {
								type: "image",
								path: FileAccess.uriToBrowserUri(
									joinPath(extension.extensionLocation, iconStr),
								).toString(true),
							}
						: {
	...
	private unregisterExtensionWalkthroughContributions(
		...
		extension.contributes?.walkthroughs?.forEach((section) => {
			...
			section.steps.forEach((step) => {
				const fullyQualifiedID =
					extension.identifier.value + "#" + section.id + "#" + step.id;
	...
	getWalkthroughs(): IResolvedWalkthrough[] {
		...
		const categoriesWithCompletion = registeredCategories
			...
			.filter(
				(category) =>
					category.content.type !== "steps" || category.content.steps.length,
			)
							...
							.map(({ href }) => {
								if (href.startsWith("command:")) {
									return (
										...
										href.slice(
											"command:".length,
											href.includes("?") ? href.indexOf("?") : undefined,
										)
									);
								}
...
const convertRelativeMediaPathsToWebviewURIs = (
	basePath: URI,
	path: string | { hc: string; hcLight?: string; dark: string; light: string },
): { hcDark: URI; hcLight: URI; dark: URI; light: URI } => {
	const convertPath = (path: string) =>
