...
export class WorkbenchMcpGatewayService implements IWorkbenchMcpGatewayService {
	...
	async createGateway(
		...
		this._logService.debug(
			`[McpGateway][Workbench] createGateway requested (inRemote=${inRemote})`,
		);
		if (inRemote) {
	...
	private async _createLocalGateway(
		chatSessionResource?: URI,
	): Promise<IMcpGatewayResult> {
		this._logService.info(
			"[McpGateway][Workbench] Creating local gateway via main process",
		);
		...
		this._logService.info(
			`[McpGateway][Workbench] Local gateway created with ${servers.length} server(s)`,
		);

		const onDidChangeServers = Event.map(
			Event.filter(
				this._localChannel.listen<{
					gatewayId: string;
					servers: readonly IMcpGatewayServerInfo[];
		...
		return {
			...
			dispose: () => {
				this._logService.info(
					`[McpGateway][Workbench] Disposing local gateway: ${info.gatewayId}`,
				);
	...
	private async _createRemoteGateway(
		chatSessionResource?: URI,
	): Promise<IMcpGatewayResult | undefined> {
		const connection = this._remoteAgentService.getConnection();
		if (!connection) {
			this._logService.info(
				"[McpGateway][Workbench] No remote connection available for remote gateway",
			);
			return undefined;
		}

		this._logService.info(
			"[McpGateway][Workbench] Creating remote gateway via remote server",
		);
		return connection.withChannel(McpGatewayChannelName, async (channel) => {
			...
			const servers = reviveServers(info.servers);
			this._logService.info(
				`[McpGateway][Workbench] Remote gateway created with ${servers.length} server(s)`,
			);
			...
			return {
				...
				dispose: () => {
					this._logService.info(
						`[McpGateway][Workbench] Disposing remote gateway: ${info.gatewayId}`,
					);
					void channel.call("disposeGateway", info.gatewayId).catch((error) => {
						this._logService.warn(
							`[McpGateway][Workbench] Failed to dispose remote gateway: ${info.gatewayId}`,
							error,
						);
					});
				},
