	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				...
				if (
					!delegates.some((d) => d.canStart(defs.collection!, defs.server!))
				) {
					return;
				}
		...
		const debugMode = server
			.readDefinitions()
			.map((d) => !!d.server?.devMode?.debug);
		this._register(
		...
		this._register(
			autorun((reader) => {
				...
				const excludes = pattern
					.filter((p) => p.startsWith("!"))
					.map((p) => p.slice(1));
				reader.store.add(
					fileService.watch(wf, { includes, excludes, recursive: true }),
				);
				...
				const includeParse = includes.map((p) =>
					glob.parse({ base: wf.fsPath, pattern: p }, { ignoreCase }),
				);
				const excludeParse = excludes.map((p) =>
					glob.parse({ base: wf.fsPath, pattern: p }, { ignoreCase }),
				);
				reader.store.add(
