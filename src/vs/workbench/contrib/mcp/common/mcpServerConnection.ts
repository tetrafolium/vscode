	...
	implements IMcpServerConnection
{
	...
	>("mcpServerRequestHandler", undefined);
	private readonly _onPotentialSandboxBlock = this._register(
		new Emitter<IMcpPotentialSandboxBlock>(),
	);
				...
				if (state.state === McpConnectionState.Kind.Running && !didStart) {
					...
					McpServerRequestHandler.create(
						...
						(err) => {
							if (
								!store.isDisposed &&
								McpConnectionState.isRunning(this._state.read(undefined))
							) {
	...
	private _toPotentialSandboxBlock(
		message: string,
	): IMcpPotentialSandboxBlock | undefined {
		...
		if (
			/(?:\b(?:EACCES|EPERM|ENOENT|EROFS|fail(?:ed|ure)?)\b|not accessible|read[- ]only)/i.test(
				message,
			)
		) {
			return {
	...
	private _extractSandboxHost(value: string): string | undefined {
		const match = value.match(
			/No matching config rule, denying:\s+(?<host>[^:\s]+):\d+\.?$/i,
		);
