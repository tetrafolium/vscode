...
export class McpRegistry extends Disposable implements IMcpRegistry {
	...
	public readonly lazyCollectionState = derived((reader) => {
		...
		const hasUnknown = collections.some(
			(c) => c.lazy && c.lazy.isCached === false,
		);
		return hasUnknown
			? {
					state: LazyCollectionState.HasUnknown,
					collections: collections.filter(
						(c) => c.lazy && c.lazy.isCached === false,
					),
				}
			: { state: LazyCollectionState.AllKnown, collections: [] };
	});
	...
	private async _checkTrust(
		...
		if (collection.trustBehavior === McpServerTrust.Kind.Trusted) {
			...
			collection.trustBehavior === McpServerTrust.Kind.TrustedOnNonce
		) {
	...
	private async _promptForTrust(
		definition: McpServerDefinition,
		collection: McpCollectionDefinition,
		interaction: McpStartServerInteraction | undefined,
		...
		const trustedDefinitionIds = await new Promise<string[] | undefined>(
			(resolve) => {
	 ...
	 */
	protected async _promptForTrustOpenDialog(
		definitions: {
			definition: McpServerDefinition;
			collection: McpCollectionDefinition;
		}[],
		...
		function labelFor(r: {
			...
			const originURI =
				r.definition.presentation?.origin?.uri ||
				r.collection.presentation?.origin;
			let labelWithOrigin = originURI
				? `[\`${r.definition.label}\`](${originURI})`
				: "`" + r.definition.label + "`";
		...
		const { result } = await this._dialogService.prompt({
			...
			buttons: [
				...
				{
					label: localize("mcp.trust.pick", "Pick Trusted"),
					run: () => "pick",
				},
		...
		function isActionableButton(
			obj: IQuickInputButton,
		): obj is ActionableButton {
		...
		const picker = store.add(
			this._quickInputService.createQuickPick<
				IQuickPickItem & { definitonId: string }
			>({ useSeparators: false }),
		);
				...
				buttons.push({
					...
					action: () =>
						this._editorService.openEditor({
							resource: origin.uri,
							options: { selection: origin.range },
						}),
				});
			}
			...
			return {
				...
				description:
					collection.source instanceof ExtensionIdentifier
						...
						: definition.presentation?.origin
							? this._labelService.getUriLabel(
									definition.presentation.origin.uri,
								)
							: undefined,
				picked: false,
				buttons,
	...
	public async resolveConnection(
		opts: IMcpResolveConnectionOptions,
