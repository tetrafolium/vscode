...
export class McpContextKeysController
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const ctxHasUnknownTools =
			McpContextKeys.hasUnknownTools.bindTo(contextKeyService);
			...
			bindContextKey(
				...
				(r) =>
					mcpService.servers
						...
						.some(
							(c) =>
								c.connectionState.read(r).state ===
		...
		this._store.add(
			autorun((r) => {
				...
				ctxToolsCount.set(
					serverTools.reduce((count, tools) => count + tools.length, 0),
				);
				ctxHasUnknownTools.set(
					mcpService.lazyCollectionState.read(r).state !==
						LazyCollectionState.AllKnown ||
						servers.some((s) => {
							const toolState = s.cacheState.read(r);
							return (
								toolState === McpServerCacheState.Unknown ||
								toolState === McpServerCacheState.Outdated ||
								toolState === McpServerCacheState.RefreshingFromUnknown
							);
						}),
				);
			}),
