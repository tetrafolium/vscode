...
export const IMcpSandboxService =
	createDecorator<IMcpSandboxService>("mcpSandboxService");
...
export interface IMcpSandboxService {
	...
	getSandboxConfigSuggestionMessage(
		...
		existingSandboxConfig?: IMcpSandboxConfiguration,
	): SandboxConfigSuggestionResult | undefined;
	applySandboxConfigSuggestion(
		serverDef: McpServerDefinition,
		mcpResource: URI,
		configTarget: ConfigurationTarget,
		potentialBlocks: readonly IMcpPotentialSandboxBlock[],
		suggestedSandboxConfig?: IMcpSandboxConfiguration,
	): Promise<boolean>;
}
...
export class McpSandboxService
	extends Disposable
	implements IMcpSandboxService
{
	readonly _serviceBrand: undefined;

	private _sandboxSettingsId: string | undefined;
	private _remoteEnvDetailsPromise: Promise<IRemoteAgentEnvironment | null>;
	private readonly _defaultAllowedDomains: readonly string[] = [
		"registry.npmjs.org",
	...
	constructor(
		...
		@IMcpResourceScannerService
		private readonly _mcpResourceScannerService: IMcpResourceScannerService,
		@IRemoteAgentService
	...
	public async launchInSandboxIfEnabled(
		serverDef: McpServerDefinition,
		launch: McpServerLaunch,
		remoteAuthority: string | undefined,
		configTarget: ConfigurationTarget,
		...
		if (await this.isEnabled(serverDef, remoteAuthority)) {
			this._logService.trace(
				`McpSandboxService: Launching with config target ${configTarget}`,
			);
			const launchDetails = await this._resolveSandboxLaunchDetails(
				configTarget,
				remoteAuthority,
				launch.sandbox,
				launch.cwd,
			...
			const quotedArgs = launch.args.map((arg) =>
				this._quoteShellArgument(arg),
			);
			const sandboxArgs = this._getSandboxCommandArgs(
				quotedCommand,
				quotedArgs,
				launchDetails.sandboxConfigPath,
			);
			const sandboxEnv = await this._getSandboxEnvVariables(
				launch.env,
				launchDetails.tempDir,
				launchDetails.rgPath,
				remoteAuthority,
			);
	...
	public getSandboxConfigSuggestionMessage(
		serverLabel: string,
		potentialBlocks: readonly IMcpPotentialSandboxBlock[],
		existingSandboxConfig?: IMcpSandboxConfiguration,
	): SandboxConfigSuggestionResult | undefined {
		const suggestions = this._getSandboxConfigSuggestions(
			potentialBlocks,
		...
		if (allowedDomainsList.length) {
			const shown = allowedDomainsList
				.map((domain) => `"${domain}"`)
				.join(", ");
			suggestionLines.push(
				localize(
					"mcpSandboxSuggestion.allowedDomains",
					"Add to `sandbox.network.allowedDomains`: {0}",
					shown,
				),
			);
		}
		...
		if (allowWriteList.length) {
			...
			suggestionLines.push(
				localize(
					"mcpSandboxSuggestion.allowWrite",
					"Add to `sandbox.filesystem.allowWrite`: {0}",
					shown,
				),
			);
		}
	...
	public async applySandboxConfigSuggestion(
		serverDef: McpServerDefinition,
		mcpResource: URI,
		configTarget: ConfigurationTarget,
		potentialBlocks: readonly IMcpPotentialSandboxBlock[],
		...
		await this._mcpResourceScannerService.updateSandboxConfig(
			(data) => {
				const existingSandbox = data.sandbox;
				const suggestedAllowedDomains =
					suggestedSandboxConfig?.network?.allowedDomains ?? [];
				const suggestedAllowWrite =
					suggestedSandboxConfig?.filesystem?.allowWrite ?? [];

				const currentAllowedDomains = new Set(
					existingSandbox?.network?.allowedDomains ?? [],
				);
				...
				const currentAllowWrite = new Set(
					existingSandbox?.filesystem?.allowWrite ?? [],
				);
				...
				if (
					suggestedAllowedDomains.length === 0 &&
					suggestedAllowWrite.length === 0
				) {
					return data;
				}
	...
	private _getSandboxConfigSuggestions(
		potentialBlocks: readonly IMcpPotentialSandboxBlock[],
		existingSandboxConfig?: IMcpSandboxConfiguration,
	): SandboxConfigSuggestions | undefined {
		...
		const existingAllowWrite = new Set(
			existingSandboxConfig?.filesystem?.allowWrite ?? [],
		);
		const existingAllowedDomains = new Set(
			existingSandboxConfig?.network?.allowedDomains ?? [],
		);
		...
		for (const block of potentialBlocks) {
			if (
				block.kind === "network" &&
				block.host &&
				!existingAllowedDomains.has(block.host)
			) {
				allowedDomains.add(block.host);
			}

			if (
				block.kind === "filesystem" &&
				block.path &&
				!existingAllowWrite.has(block.path)
			) {
	...
	private async _resolveSandboxLaunchDetails(
		configTarget: ConfigurationTarget,
		remoteAuthority?: string,
		sandboxConfig?: IMcpSandboxConfiguration,
		launchCwd?: string,
		...
		const sandboxConfigPath = tempDir
			? await this._updateSandboxConfig(
					tempDir,
					configTarget,
					sandboxConfig,
					launchCwd,
			...
			: undefined;
		this._logService.debug(
			`McpSandboxService: Updated sandbox config path: ${sandboxConfigPath}`,
		);
		return { execPath, srtPath, rgPath, sandboxConfigPath, tempDir };
	}

	private async _getExecPath(
		os: OperatingSystem,
		appRoot: string,
		remoteAuthority?: string,
	...
	private async _getSandboxEnvVariables(
		baseEnv: McpServerTransportStdio["env"],
		tempDir: URI | undefined,
		rgPath: string | undefined,
		remoteAuthority?: string,
	): Promise<McpServerTransportStdio["env"]> {
		...
		if (rgPath) {
			env = {
				...env,
				PATH: env["PATH"]
					? `${env["PATH"]}${await this._getPathDelimiter(remoteAuthority)}${dirname(rgPath)}`
					: dirname(rgPath),
			};
		}
		if (!remoteAuthority) {
	...
	private async _getRemoteEnv(
		remoteAuthority?: string,
	): Promise<IRemoteAgentEnvironment | null> {
		if (!remoteAuthority) {
			return null;
		}
	...
	private async _getOperatingSystem(
		remoteAuthority?: string,
	...
	private async _getTempDir(
		...
		const environmentService = this
			._environmentService as IEnvironmentService & { tmpDir?: URI };
	...
	private async _updateSandboxConfig(
		tempDir: URI,
		configTarget: ConfigurationTarget,
		sandboxConfig?: IMcpSandboxConfiguration,
		launchCwd?: string,
	): Promise<string> {
		const normalizedSandboxConfig = this._withDefaultSandboxConfig(
			sandboxConfig,
			launchCwd,
		);
		...
		if (this._sandboxConfigPerConfigurationTarget.has(configTargetKey)) {
			configFileUri = URI.parse(
				this._sandboxConfigPerConfigurationTarget.get(configTargetKey)!,
			);
		} else {
			...
			this._sandboxConfigPerConfigurationTarget.set(
				configTargetKey,
	...
	private _withDefaultSandboxConfig(
		...
		const mergedAllowWrite = new Set(
		...
		const mergedAllowedDomains = new Set(
