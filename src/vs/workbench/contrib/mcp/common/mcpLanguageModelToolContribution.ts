...
export class McpLanguageModelToolContribution
	extends Disposable
	implements IWorkbenchContribution
{
			...
			autorun((reader) => {
				...
				for (const server of servers) {
					...
					if (previousRec) {
						...
						if (
							!previousRec.source ||
							equals(previousRec.source, mcpServerToSourceData(server, reader))
						) {
					...
					const toolSet = new Lazy(() => {
						...
						const referenceName = server.definition.label
							.toLowerCase()
							.replace(/\s+/g, "-"); // see issue https://github.com/microsoft/vscode/issues/278152
						const toolSet = store.add(
							this._toolsService.createToolSet(
								source,
	...
	private _syncTools(
		server: IMcpServer,
		collectionData: Lazy<{ toolSet: ToolSet; source: ToolDataSource }>,
		store: DisposableStore,
	) {
		const tools = new Map</* tool ID */ string, ISyncedToolData>();

		const collectionObservable = this._mcpRegistry.collections.map(
			(collections) => collections.find((c) => c.id === server.collection.id),
		);
		...
		store.add(
			autorun((reader) => {
				...
				const registerTool = (
					...
					store.add(
						this._toolsService.registerTool(
							toolData,
...
class McpToolImplementation implements IToolImpl {
	constructor(
		...
		@IImageResizeService
		private readonly _imageResizeService: IImageResizeService,
	) {}
	...
	async prepareToolInvocation(
		...
		const sandboxEnabled = await McpServer.callOn(
			server,
			async (_handler, connection) => {
		...
		if (!isSandboxedServer) {
			...
			if (!tool.definition.annotations?.readOnlyHint) {
				confirm.title = new MarkdownString(
					localize("msg.title", "Run {0}", title),
				);
		...
		const mcpUiEnabled =
			this._configurationService.getValue<boolean>(mcpAppsEnabledConfig);
		...
		return {
			...
			toolSpecificData: {
				...
				mcpAppData:
					mcpUiEnabled && tool.uiResourceUri
						? {
	...
	async invoke(
		...
		for (const item of callResult.content) {
			...
			const addAsInlineData = async (
				mimeType: string,
				value: string,
				uri?: URI,
			): Promise<VSBuffer | void> => {
				details.output.push({ type: "embed", mimeType, value, uri, audience });
				if (isForModel) {
					let finalData: VSBuffer;
					try {
						const resized = await this._imageResizeService.resizeImage(
							decodeBase64(value).buffer,
							mimeType,
						);
			...
			const addAsLinkedResource = (uri: URI, mimeType?: string) => {
				const json: IMcpToolResourceLinkContents = {
					uri,
					underlyingMimeType: mimeType,
				};
				result.content.push({
					kind: "data",
					audience,
			...
			const isForModel =
				!audience || audience.includes(LanguageModelPartAudience.Assistant);
			if (item.type === "text") {
				...
				if (isForModel) {
					if (item.mimeType && getAttachableImageExtension(item.mimeType)) {
						result.content.push({
							...
							value: {
								mimeType: item.mimeType,
								data: await this._fileService
									.readFile(uri)
									.then((f) => f.value)
									.catch(() => VSBuffer.alloc(0)),
							},
						});
					} else {
		...
		if (callResult.structuredContent) {
			details.output.push({
				type: "embed",
				isText: true,
				value: JSON.stringify(callResult.structuredContent, null, 2),
				audience: [LanguageModelPartAudience.Assistant],
			});
			result.content.push({
				kind: "text",
				value: JSON.stringify(callResult.structuredContent),
				audience: [LanguageModelPartAudience.Assistant],
			});
		}
