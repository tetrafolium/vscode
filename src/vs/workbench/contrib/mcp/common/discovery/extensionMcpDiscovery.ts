...
export class ExtensionMcpDiscovery extends Disposable implements IMcpDiscovery {
	...
	private readonly _extensionCollectionIdsToPersist = new Map<
		string,
		PersistWhen
	>();
		...
		this._register(
			storageService.onWillSaveState(() => {
				...
				for (const [
					collectionId,
					behavior,
				] of this._extensionCollectionIdsToPersist.entries()) {
			...
			_mcpExtensionPoint.setHandler((_extensions, delta) => {
				...
				for (const collections of added) {
					...
					for (const coll of collections.value) {
						...
						if (coll.when) {
							this._registerConditionalCollection(
								id,
								coll,
								collections,
	...
	private _registerCollection(
		...
		collections: extensionsRegistry.IExtensionPointUser<
			IMcpCollectionContribution[]
		>,
		...
		const dispo = this._mcpRegistry.registerCollection({
			...
			serverDefinitions: observableValue<McpServerDefinition[]>(
				this,
				serverDefs?.map(McpServerDefinition.fromSerialized) || [],
			),
			lazy: {
				isCached: !!serverDefs,
				load: () =>
					this._activateExtensionServers(coll.id).then(() => {
	...
	private _registerConditionalCollection(
		...
		const contextKeyListener =
			this._contextKeyService.onDidChangeContext(evaluate);
		evaluate();
	...
	private static _validate(
		user: extensionsRegistry.IExtensionPointUser<IMcpCollectionContribution[]>,
	): boolean {
		...
		for (const contribution of user.value) {
			if (
				...
				isFalsyOrWhitespace(contribution.id)
			) {
				user.collector.error(
			...
			if (
				...
				isFalsyOrWhitespace(contribution.label)
			) {
				user.collector.error(
					localize(
						"invalidLabel",
			...
			if (
				...
				(typeof contribution.when !== "string" ||
					isFalsyOrWhitespace(contribution.when))
			) {
				user.collector.error(
