...
export class McpServerRequestHandler extends Disposable {
	...
	readonly onDidReceiveCancelledNotification =
		this._onDidReceiveCancelledNotification.event;

	private readonly _onDidReceiveProgressNotification = this._register(
		new Emitter<MCP.ProgressNotification>(),
	);
	readonly onDidReceiveProgressNotification =
		this._onDidReceiveProgressNotification.event;

	private readonly _onDidReceiveElicitationCompleteNotification =
		this._register(new Emitter<MCP.ElicitationCompleteNotification>());
	readonly onDidReceiveElicitationCompleteNotification =
		this._onDidReceiveElicitationCompleteNotification.event;

	private readonly _onDidChangeResourceList = this._register(
		new Emitter<void>(),
	);
	...
	public static async create(
		...
		opts: IMcpServerRequestHandlerOptions,
				...
				>(
					{
						...
						params: {
							...
							capabilities: {
								...
								elicitation: opts.elicitationRequestHandler
									? { form: {}, url: {} }
									: undefined,
								tasks: {
									list: {},
									cancel: {},
									requests: {
										sampling: opts.createMessageRequestHandler
											? { createMessage: {} }
											: undefined,
										elicitation: opts.elicitationRequestHandler
											? { create: {} }
	...
	protected constructor({
		...
		this._rpc = this._register(
			new JsonRpcProtocol(
				(message) => this.send(message as MCP.JSONRPCMessage),
				{
					handleRequest: (request, token) =>
						this.handleServerRequest(
							request as MCP.JSONRPCRequest & MCP.ServerRequest,
							token,
						),
					handleNotification: (notification) =>
						this.handleServerNotification(
							notification as MCP.JSONRPCNotification & MCP.ServerNotification,
						),
				},
			),
		);
		...
		this._register(
			this._taskManager.onDidUpdateTask((task) => {
				this.send({
		...
		this._register(
			launch.onDidReceiveMessage((message) => {
		...
		this._register(
			logger.onDidChangeLogLevel((logLevel) => {
	...
	private async sendRequest<
		T extends MCP.ClientRequest,
		R extends MCP.ServerResult,
	>(
		...
		return this._rpc
			.sendRequest<R>(request, token, (id) =>
	...
	private send(mcp: MCP.JSONRPCMessage) {
		if (canLog(this.logger.getLevel(), this._requestLogLevel)) {
			...
			log(
				this.logger,
				this._requestLogLevel,
				`[editor -> server] ${JSON.stringify(mcp)}`,
			);
		}
	...
	private sendNotification<N extends MCP.ClientNotification>(
		notification: Omit<N, "jsonrpc">,
	): void {
	...
	private handleServerRequest(
		request: MCP.JSONRPCRequest & MCP.ServerRequest,
		token: CancellationToken,
	): MCP.Result | Promise<MCP.Result> {
		const mapError = (error: unknown): JsonRpcError => {
			...
			const mcpError = McpError.unknown(
		...
		try {
			...
			if (request.method === "ping") {
				...
				this._createMessageRequestHandler
			) {
				...
				if (request.params.task) {
					const taskResult = this._taskManager.createTask(
						...
						(token) =>
							this._createMessageRequestHandler!(request.params, token),
					);
					taskResult._meta ??= {};
				...
				this._elicitationRequestHandler
			) {
				// Check if this is a task-augmented request
				if (request.params.task) {
					const taskResult = this._taskManager.createTask(
	...
	private handleServerNotification(
		...
		try {
			switch (request.method) {
				...
				case "notifications/tasks/status":
					this._taskManager
						...
						?.onDidUpdateState(request.params);
					return;
				default:
			...
			this.logger.error(
				`Error handling notification ${request.method}:`,
				error,
			);
		}
	}
	...
	private handleCancelledNotification(
		request: MCP.CancelledNotification,
	): void {
	...
	private handleLoggingNotification(
		request: MCP.LoggingMessageNotification,
	): void {
	...
	listPrompts(
		...
		return Iterable.asyncToArrayFlat(
			this.sendRequestPaginated<
				...
				MCP.Prompt
			>("prompts/list", (result) => result.prompts, params, token),
		);
	}
	...
	listTools(
		...
		return Iterable.asyncToArrayFlat(
			this.sendRequestPaginated<
				...
				MCP.ListToolsResult,
				MCP.Tool
			>("tools/list", (result) => result.tools, params, token),
		);
	}
	...
	async callTool(
		...
		const response = await this.sendRequest<
			MCP.CallToolRequest,
			MCP.CallToolResult | MCP.CreateTaskResult
		>({ method: "tools/call", params }, token);

		if (isTaskResult(response)) {
			const task = new McpTask<MCP.CallToolResult>(
				response.task,
				token,
				onStatusMessage,
			);
	...
	getTask(
		...
		token?: CancellationToken,
	): Promise<MCP.GetTaskResult> {
		return this.sendRequest<MCP.GetTaskRequest, MCP.GetTaskResult>(
	...
	getTaskResult(
		...
		return this.sendRequest<
			MCP.GetTaskPayloadRequest,
			MCP.GetTaskPayloadResult
		>({ method: "tasks/result", params }, token);
	}
	...
	cancelTask(
		params: { taskId: string },
		token?: CancellationToken,
	): Promise<MCP.CancelTaskResult> {
		return this.sendRequest<MCP.CancelTaskRequest, MCP.CancelTaskResult>(
	 ...
	 */
	listTasks(
		params?: MCP.ListTasksRequest["params"],
		token?: CancellationToken,
	): Promise<MCP.Task[]> {
		return Iterable.asyncToArrayFlat(
			this.sendRequestPaginated<
				MCP.ListTasksRequest,
				MCP.ListTasksResult,
				MCP.Task
 ...
 * @internal
 */
export class McpTask<T extends MCP.Result>
	extends Disposable
	implements IMcpTaskInternal
{
	...
	private _handler = observableValue<McpServerRequestHandler | undefined>(
		"mcpTaskHandler",
		undefined,
	);
	...
	constructor(
		...
		const inputRequiredLookup = observableValue<
			ObservablePromise<MCP.Task> | undefined
		>("activeResultLookup", undefined);
		...
		store.add(
			autorun((reader) => {
				...
				if (lookup) {
					...
					return transaction((tx) => {
						if (!result) {
							...
							inputRequiredLookup.set(undefined, tx);
							if (
								result.error instanceof McpError &&
								result.error.code === MCP.INVALID_PARAMS
							) {
								this._lastTaskState.set(
									{ ...current, status: "cancelled" },
									undefined,
				...
				reader.store.add(
					disposableTimeout(() => {
						handler
							.getTask({ taskId: current.taskId }, cts.token)
			...
			autorun((reader) => {
				...
				if (status === "failed") {
					...
					if (handler) {
						...
						const cts = new CancellationTokenSource(_token);
						reader.store.add(toDisposable(() => cts.dispose(true)));
						inputRequiredLookup.set(
							new ObservablePromise<MCP.Task>(
					...
					const handler = this._handler.read(reader);
					if (handler) {
						this.promise.settleWith(
							handler.getTaskResult(
								{ taskId: _task.taskId },
								_token,
							) as Promise<T>,
						);
