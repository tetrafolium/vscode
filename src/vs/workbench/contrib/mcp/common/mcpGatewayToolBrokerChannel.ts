...
export class McpGatewayToolBrokerChannel
	extends Disposable
	implements IServerChannel<unknown>
{
	 ...
	 */
	private readonly _startupGrace = new Map<
		string,
		{ promise: Promise<boolean>; resolved: boolean }
	>();
			...
			autorun((reader) => {
				...
				if (toolsInitialized) {
					this._logService.debug(
						"[McpGateway][ToolBroker] Tools changed, firing onDidChangeTools",
					);
		...
		this._register(
			autorun((reader) => {
				...
				if (resourcesInitialized) {
					this._logService.debug(
						"[McpGateway][ToolBroker] Resources changed, firing onDidChangeResources",
					);
		...
		this._register(
			autorun((reader) => {
				...
				if (serversInitialized) {
					this._logService.debug(
						"[McpGateway][ToolBroker] Servers changed, firing onDidChangeServers",
					);
	...
	private _waitForStartup(server: IMcpServer): Promise<boolean> {
		...
		if (existing?.resolved) {
			...
			if (
				state === McpServerCacheState.Unknown ||
				state === McpServerCacheState.Outdated
			) {
	...
	async call<T>(
		...
		switch (command) {
			...
			case "callToolForServer": {
				const { serverId, name, args, chatSessionResource } =
					arg as ICallToolForServerArgs;
	...
	private async _listToolsForServer(
		serverId: string,
	): Promise<readonly MCP.Tool[]> {
	...
	private async _callToolForServer(
		serverId: string,
		name: string,
		args: Record<string, unknown>,
		chatSessionResource?: string,
	...
	private async _listResourcesForServer(
		serverId: string,
	): Promise<readonly MCP.Resource[]> {
		const server = this._getServerById(serverId);
		if (!server) {
			this._logService.warn(
		...
		try {
			const resources = await McpServer.callOn(server, (h) =>
				h.listResources(),
			);
			this._logService.debug(
	...
	private async _readResourceForServer(
		serverId: string,
		uri: string,
		token: CancellationToken = CancellationToken.None,
	...
	private async _listResourceTemplatesForServer(
		serverId: string,
		...
		try {
			const resourceTemplates = await McpServer.callOn(server, (h) =>
