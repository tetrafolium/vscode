 ...
 */
export const mcpEnterpriseManagedAuthIdpSection =
...
export const mcpContributionPoint: IExtensionPointDescriptor<
	IMcpCollectionContribution[]
> = {
...
class McpServerDefinitionsProviderRenderer
	extends Disposable
	implements IExtensionFeatureTableRenderer
{
	...
	shouldRender(manifest: IExtensionManifest): boolean {
		return (
			...
			Array.isArray(manifest.contributes.mcpServerDefinitionProviders) &&
			manifest.contributes.mcpServerDefinitionProviders.length > 0
		);
	}
	...
	render(manifest: IExtensionManifest): IRenderedData<ITableData> {
		...
		const rows: IRowData[][] = mcpServerDefinitionProviders.map(
			(mcpServerDefinitionProvider) => {
				return [
					new MarkdownString().appendMarkdown(
						`\`${mcpServerDefinitionProvider.id}\``,
					),
