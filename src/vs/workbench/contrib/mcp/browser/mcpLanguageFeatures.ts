	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		@IMcpWorkbenchService
		private readonly _mcpWorkbenchService: IMcpWorkbenchService,
		...
		this._register(
			this._secretStorageService.onDidChangeSecret((key) => {
	...
	private async _parseModel(model: ITextModel) {
		...
		const uri = model.uri;
		const inConfig: ConfigDescriptor | undefined = uri.path.endsWith(
			"/.mcp.json",
		)
			? {
	...
	private _addDiagnostics(
		...
		const serversNode = findNodeAtLocation(
			tree,
			inConfig.section ? [...inConfig.section, serversKey] : [serversKey],
		);
		if (!serversNode) {
			return;
		}
	...
	private async _provideCodeLenses(
		model: ITextModel,
						...
						},
						{
							...
							command: {
								...
								arguments: [
									...
									{ autoTrustChanges: true } satisfies IMcpServerStartOpts,
								],
							},
						},
					);
		...
		for (const node of serversNode.children || []) {
			...
			const clientIdNode = findNodeAtLocation(serverValue, [
				"oauth",
				"clientId",
			]);
		...
		const existingSecrets = await Promise.all(
			candidates.map((c) =>
				this._secretStorageService.get(
					mcpOAuthClientSecretStorageKey(c.mcpServerUrl, c.clientId),
				),
			),
		);
		for (let i = 0; i < candidates.length; i++) {
			const { clientId, mcpServerUrl, serverName, clientIdOffset } =
				candidates[i];
	...
	private async _provideInlayHints(
		...
		const serversNode = findNodeAtLocation(mcpSection, [
			inConfig.serversKey ?? "servers",
		]);
		if (serversNode) {
			...
			const tooltip = new MarkdownString(
				[
					createMarkdownCommandLink({
						...
						tooltip: localize("edit.savedValue.tooltip", "Edit saved value"),
					}),
					createMarkdownCommandLink({
						id: McpCommandIds.RemoveStoredInput,
						text: localize("clear", "Clear"),
						arguments: [inConfig!.scope, savedId],
						tooltip: localize("clear.savedValue.tooltip", "Clear saved value"),
					}),
					createMarkdownCommandLink({
						id: McpCommandIds.RemoveStoredInput,
						text: localize("clearAll", "Clear All"),
						arguments: [inConfig!.scope],
						tooltip: localize(
							"clearAll.savedValues.tooltip",
