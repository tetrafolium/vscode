...
import {
	...
	ExtensionIconBadge,
} from "../../extensions/browser/extensionsWidgets.js";
...
export abstract class McpServerWidget
	extends Disposable
...
export class McpServerIconWidget extends McpServerWidget {
	...
	constructor(
		...
		this.codiconIconElement = dom.append(
			this.element,
			dom.$(ThemeIcon.asCSSSelector(mcpServerIcon)),
		);
		...
		this._register(
			this.themeService.onDidColorThemeChange(() => this.render()),
		);
	}
	...
	render(): void {
		...
		if (this.mcpServer.icon) {
			...
			const iconUrl = isDark(type)
				? this.mcpServer.icon.dark
				: this.mcpServer.icon.light;
			if (this.iconUrl !== iconUrl) {
				this.iconElement.style.display = "inherit";
				this.codiconIconElement.style.display = "none";
				this.iconUrl = iconUrl;
				this.iconLoadingDisposable.value = dom.addDisposableListener(
					this.iconElement,
					"error",
					() => {
						this.iconElement.style.display = "none";
			...
			this.iconElement.style.display = "none";
			this.iconElement.src = "";
			this.codiconIconElement.className = this.mcpServer.codicon
				? `codicon ${this.mcpServer.codicon}`
				: ThemeIcon.asClassName(mcpServerIcon);
			this.codiconIconElement.style.display = "inherit";
			this.iconLoadingDisposable.clear();
		}
	}
	...
	render(): void {
		...
		if (this.small) {
			...
			this.element.classList.toggle(
				"clickable",
				!!this.mcpServer.gallery?.publisherUrl,
			);
			...
			this.containerHover = this.disposables.add(
				this.hoverService.setupManagedHover(
					...
					localize(
						"publisher",
						"Publisher ({0})",
						this.mcpServer.publisherDisplayName,
					),
				),
			);
			dom.append(this.element, publisherDisplayName);
...
export class StarredWidget extends McpServerWidget {
	...
	render(): void {
		...
		const parent = this.small
			? this.container
			: dom.append(this.container, dom.$("span.rating", { tabIndex: 0 }));
...
export class LicenseWidget extends McpServerWidget {
	...
	render(): void {
		...
		const parent = dom.append(
			this.container,
			dom.$("span.license", { tabIndex: 0 }),
		);
		dom.append(parent, dom.$("span" + ThemeIcon.asCSSSelector(mcpLicenseIcon)));

		const licenseElement = dom.append(
			parent,
			dom.$("span", undefined, this.mcpServer.license),
		);
...
export class McpServerHoverWidget extends McpServerWidget {
	...
	private getHoverMarkdown(): MarkdownString | undefined {
		...
		markdown.appendMarkdown(
			`**${escapeMarkdownSyntaxTokens(this.mcpServer.label)}**`,
		);
		markdown.appendText(`\n`);
		...
		if (this.mcpServer.local?.scope === LocalMcpServerScope.Workspace) {
			...
			markdown.appendMarkdown(
				localize("workspace extension", "Workspace MCP Server"),
			);
			addSeparator = true;
		}
		...
		if (this.mcpServer.local?.scope === LocalMcpServerScope.RemoteUser) {
			...
			markdown.appendMarkdown(
				localize("remote user extension", "Remote MCP Server"),
			);
			addSeparator = true;
		}
		...
		if (this.mcpServer.installState === McpServerInstallState.Installed) {
			if (this.mcpServer.starsCount) {
				...
				const starsCountLabel = StarredWidget.getCountLabel(
					this.mcpServer.starsCount,
				);
...
export class McpServerScopeBadgeWidget extends McpServerWidget {
	private readonly badge = this._register(
	...
	render(): void {
		...
		this.badge.value = this.instantiationService.createInstance(
			ExtensionIconBadge,
			icon,
			undefined,
		);
...
export const mcpStarredIconColor = registerColor(
