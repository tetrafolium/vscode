...
export class McpServersListView extends AbstractExtensionsListView<IWorkbenchMcpServer> {
	...
	private readonly modalNavigationDisposable = this._register(
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this.welcomeContainer = dom.append(
			container,
			dom.$(".mcp-welcome-container.hide"),
		);
				...
				this.mcpWorkbenchService.open(options.element!, {
					...
					modal: options.sideBySide
						...
						: buildModalNavigationForPagedList(
								...
								(server, modal) =>
									this.mcpWorkbenchService.open(server, {
										pinned: false,
										modal,
									}),
	...
	private async onContextMenu(
		e: IListContextMenuEvent<IWorkbenchMcpServer>,
	): Promise<void> {
		if (e.element) {
			const disposables = new DisposableStore();
			const mcpServer = e.element
				? this.mcpWorkbenchService.local.find(
						(local) => local.id === e.element!.id,
					) || e.element
				: e.element;
	...
	async show(query: string): Promise<IPagedModel<IWorkbenchMcpServer>> {
		...
		if (this.input.onDidChangeModel) {
			this.input.disposables.add(
				this.input.onDidChangeModel((model) => {
					if (!this.input) {
						return;
					}
	...
	private createWelcomeContent(welcomeContainer: HTMLElement): void {
		const welcomeContent = dom.append(
			welcomeContainer,
			dom.$(".mcp-welcome-content"),
		);

		const iconContainer = dom.append(
			welcomeContent,
			dom.$(".mcp-welcome-icon"),
		);
		...
		const settingsCommandLink = createMarkdownCommandLink({
			...
			tooltip: localize("mcp.welcome.settings.tooltip", "Open Settings"),
		}).toString();
		const description = dom.append(
			welcomeContent,
			dom.$(".mcp-welcome-description"),
		);
		const markdownResult = this._register(
		...
		const buttonContainer = dom.append(
			welcomeContent,
			dom.$(".mcp-welcome-button-container"),
		);
		...
		this._register(
			button.onDidClick(async () => {
				const { result } = await this.dialogService.prompt({
					...
					buttons: [
						{
							label: localize("mcp.gallery.enableDialog.enable", "Enable"),
							run: () => true,
						},
						{
							label: localize("mcp.gallery.enableDialog.cancel", "Cancel"),
							run: () => false,
						},
					],
				});
	...
	private async query(query: string): Promise<IQueryResult> {
		...
		const onDidChangeModel = disposables.add(
			new Emitter<IPagedModel<IWorkbenchMcpServer>>(),
		);
		...
		disposables.add(
			Event.debounce(
				...
				const mergedMcpServers = this.mergeChangedMcpServers(servers, [
		...
		disposables.add(
			this.mcpWorkbenchService.onReset(() =>
				onDidChangeModel.fire(
					new PagedModel([...this.mcpWorkbenchService.local]),
				),
			),
		);
		return {
	...
	private mergeChangedMcpServers(
		mcpServers: IWorkbenchMcpServer[],
		newMcpServers: IWorkbenchMcpServer[],
		...
		const findPreviousMcpServerIndex = (from: number): number => {
			...
			if (previousMcpServerInNew) {
				index = oldMcpServers.findIndex(
					(e) => e.id === previousMcpServerInNew.id,
				);
				if (index === -1) {
		...
		for (let index = 0; index < newMcpServers.length; index++) {
			...
			if (mcpServers.every((r) => r.id !== newMcpServer.id)) {
				hasChanged = true;
				mcpServers.splice(
					findPreviousMcpServerIndex(index - 1) + 1,
					0,
					newMcpServer,
				);
			}
		...
		for (let index = mcpServers.length - 1; index >= 0; index--) {
			...
			if (
				newMcpServers.every((r) => r.id !== oldMcpServer.id) &&
				newMcpServers.some((r) => r.name === oldMcpServer.name)
			) {
				hasChanged = true;
...
class McpServerRenderer implements IPagedRenderer<
	IWorkbenchMcpServer,
...
> {
	...
	renderTemplate(root: HTMLElement): IMcpServerTemplateData {
		const element = dom.append(
			root,
			dom.$(".mcp-server-item.extension-list-item"),
		);
		...
		const publisherWidget = this.instantiationService.createInstance(
			PublisherWidget,
			dom.append(footer, dom.$(".publisher-container")),
			true,
		);
		...
		const extensionContainers: McpServerContainers =
	...
	renderElement(
		...
		const updateEnablement = () =>
			data.root.classList.toggle(
				"disabled",
				!!mcpServer.runtimeStatus?.state &&
					mcpServer.runtimeStatus.state !== McpServerEnablementState.Enabled,
			);
		updateEnablement();
		data.mcpServerDisposables.push(
...
export class McpServersViewsContribution
	extends Disposable
		...
		Registry.as<IViewsRegistry>(ViewExtensions.ViewsRegistry).registerViews(
			[
				...
				},
				{
					...
					when: ContextKeyExpr.and(
						DefaultViewsContext,
						HasInstalledMcpServersContext.toNegated(),
						ChatContextKeys.Setup.hidden.negate(),
						ChatContextKeys.Setup.disabledInWorkspace.negate(),
						McpServersGalleryStatusContext.isEqualTo(
							McpGalleryManifestStatus.Available,
						),
						ContextKeyExpr.or(
							ContextKeyDefinedExpr.create(
								`config.${mcpGalleryServiceUrlConfig}`,
							),
							ProductQualityContext.notEqualsTo("stable"),
							ContextKeyDefinedExpr.create(
								`config.${mcpGalleryServiceEnablementConfig}`,
							),
						),
					),
					weight: 40,
				...
				},
				{
					...
					when: ContextKeyExpr.and(
						...
						ProductQualityContext.isEqualTo("stable"),
						ContextKeyDefinedExpr.create(
							`config.${mcpGalleryServiceEnablementConfig}`,
						).negate(),
					),
