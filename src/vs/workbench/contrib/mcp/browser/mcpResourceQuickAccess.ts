...
export class McpResourcePickHelper extends Disposable {
	private _resources = observableValue<{
		picks: Map<IMcpServer, (IMcpResourceTemplate | IMcpResource)[]>;
		isBusy: boolean;
	}>(this, { picks: new Map(), isBusy: true });
	private _pickItemsStack: LinkedList<{
		server: IMcpServer;
		resources: (IMcpResource | IMcpResourceTemplate)[];
	}> = new LinkedList();
	private _inDirectory = observableValue<
		| undefined
		| { server: IMcpServer; resources: (IMcpResource | IMcpResourceTemplate)[] }
	>(this, undefined);
	...
	public addCurrentMCPQuickPickItemLevel(
		server: IMcpServer,
		resources: (IMcpResource | IMcpResourceTemplate)[],
	): void {
		...
		if (!isValidPush) {
			...
			if (
				stackedItem?.server === server &&
				stackedItem.resources === resources
			) {
				isValidPush = false;
			} else {
				isValidPush = true;
	...
	public navigateBack(): boolean {
		...
		if (items) {
			this._inDirectory.set(
				{ server: items.server, resources: items.resources },
				undefined,
			);
			return true;
		} else {
	...
	public async navigate(
		...
		if (
			stat &&
			this._isDirectoryResource(resource) &&
			(stat.children?.length ?? 0) > 0
		) {
	...
	public toAttachment(
		resource: IMcpResource | IMcpResourceTemplate,
		server: IMcpServer,
	): Promise<ChatContextPickAttachment> | "noop" {
		const noop = "noop";
		...
		if (isMcpResourceTemplate(resource)) {
			return this._resourceTemplateToAttachment(resource).then(
				(val) => val || noop,
			);
		} else {
	...
	public async checkIfDirectoryAndPopulate(
		resource: IMcpResource | IMcpResourceTemplate,
		server: IMcpServer,
	): Promise<boolean> {
		try {
	...
	public getPicks(
		...
		const publish = () => {
			...
			this._resources.set(
				{ picks: output, isBusy: isBusyLoadingPicks },
				undefined,
			);
		};
		...
		return derived(this, (reader) => {
			...
			return directoryResource
				? {
						picks: new Map([
							[directoryResource.server, directoryResource.resources],
						]),
						isBusy: false,
					}
...
export abstract class AbstractMcpResourceAccessPick {
	...
	protected applyToPick(
		...
		const helper = store.add(
			this._instantiationService.createInstance(McpResourcePickHelper),
		);
		if (this._scopeTo) {
		...
		store.add(
			picker.onDidTriggerItemButton((event) => {
				if (event.button.tooltip === attachButton) {
					...
					const attachment = helper.toAttachment(
