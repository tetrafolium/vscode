...
export class McpAddConfigurationCommand {
	constructor(
		...
		private readonly _mcpManagementService: IWorkbenchMcpManagementService,
		@IWorkspaceContextService
	...
	private async getServerType(): Promise<AddConfigurationType | undefined> {
		type TItem = {
			kind: AddConfigurationType | "browse" | "discovery";
		} & IQuickPickItem;
		...
		try {
			aiSupported = await this._commandService.executeCommand<boolean>(
		...
		if (aiSupported) {
			...
			const elligableTypes = Object.entries(AssistedTypes)
				.map(([type, { pickLabel, pickDescription, enabledConfigKey }]) => {
					if (enabledConfigKey) {
						const enabled =
							this._configurationService.getValue<boolean>(enabledConfigKey) ??
							false;
						if (!enabled) {
							return;
						}
		...
		const discovery =
			this._configurationService.getValue<{ [K in DiscoverySource]: boolean }>(
				mcpDiscoverySection,
			);
		if (
			discovery &&
			typeof discovery === "object" &&
			allDiscoverySources.some((d) => !discovery[d])
		) {
			items.push({
				kind: "discovery",
				label: localize(
	...
	private async getStdioConfig(): Promise<
		IMcpStdioServerConfiguration | undefined
	> {
		...
		this._telemetryService.publicLog2<AddServerData, AddServerClassification>(
			"mcp.addserver",
			{
				packageType: "stdio",
			},
	...
	private async getSSEConfig(): Promise<
		IMcpRemoteServerConfiguration | undefined
	> {
	...
	private async getServerId(
		suggestion = `my-mcp-server-${generateUuid().split("-")[0]}`,
	...
	> {
		...
		if (raLabel) {
			options.push({
				...
				label: localize("mcp.target.remote", "Remote"),
				description: localize(
		...
		if (workbenchState !== WorkbenchState.EMPTY) {
			const target =
				workbenchState === WorkbenchState.FOLDER
					? this._workspaceService.getWorkspace().folders[0]
	...
	> {
		...
		const loadingQuickPick = loadingQuickPickStore.add(
			this._quickInputService.createQuickPick<
				IQuickPickItem & { id: LoadAction; helpUri?: URI }
			>(),
		);
		...
		this._commandService
			.executeCommand<ValidatePackageResult>(
					...
					targetConfig: {
						...
						properties: {
							...
							name: {
								type: "string",
								description:
									"Suggested name of the server, alphanumeric and hyphen only",
							},
						},
			...
			.then((result) => {
				if (!result || result.state === "error") {
					loadingQuickPick.title =
						result?.error || "Unknown error loading package";
					...
					if (result?.helpUri) {
						items.push({
							...
							label:
								result.helpUriLabel ??
								localize("mcp.error.openHelpUri", "Open help URL"),
					...
					items.push(
						{
							id: LoadAction.Retry,
							label: localize("mcp.error.retry", "Try a different package"),
						},
		...
		const loadingAction = await new Promise<
			{ id: LoadAction; helpUri?: URI } | undefined
		>((resolve) => {
			loadingQuickPickStore.add(
				loadingQuickPick.onDidAccept(() =>
					resolve(loadingQuickPick.selectedItems[0]),
				),
			);
			loadingQuickPickStore.add(
		...
		const config =
			await this._commandService.executeCommand<AssistedServerConfiguration>(
	...
	private showOnceDiscovered(name: string) {
		...
		store.add(
			autorun((reader) => {
				...
				const match = mapFindFirst(colls, (collection) =>
					mapFindFirst(collection.serverDefinitions.read(reader), (server) =>
						server.label === name ? { server, collection } : undefined,
					),
				);
				const server =
					match && servers.find((s) => s.definition.id === match.server.id);
	...
	public async run(): Promise<void> {
		...
		switch (serverType) {
			...
			case AddConfigurationType.DockerImage: {
				...
				config = r?.server
					? { ...r.server, type: McpServerType.LOCAL }
					: undefined;
		...
		let target: ConfigurationTarget | IWorkspaceFolder | undefined =
		...
		if (packageType) {
			this._telemetryService.publicLog2<
				AddServerCompletedData,
				AddServerCompletedClassification
	...
	public async pickForUrlHandler(
		resource: URI,
		showIsPrimary = false,
	): Promise<void> {
			...
			case "install":
				...
				try {
					...
					const {
						inputs,
						...config
					}: IMcpServerConfiguration & { inputs?: IMcpServerVariable[] } =
...
export class McpInstallFromManifestCommand {
	...
	async run(): Promise<void> {
		...
		const result = await this._fileDialogService.showOpenDialog({
			...
			filters: [
				{
					name: localize("mcp.installFromManifest.filter", "MCP Manifest"),
					extensions: ["json"],
				},
			],
			canSelectFiles: true,
		...
		const galleryManifest = manifest as IGalleryMcpServerConfiguration & {
			name?: string;
		};
		...
		if (
			Array.isArray(galleryManifest.packages) &&
			galleryManifest.packages.length > 0
		) {
			packageType = galleryManifest.packages[0].registryType;
		} else if (
			Array.isArray(galleryManifest.remotes) &&
			galleryManifest.remotes.length > 0
		) {
		...
		try {
			const { mcpServerConfiguration, notices } =
				this._mcpManagementService.getMcpServerConfigurationFromManifest(
					galleryManifest,
					packageType,
				);
