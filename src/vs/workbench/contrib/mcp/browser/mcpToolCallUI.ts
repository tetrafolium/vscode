...
export class McpToolCallUI extends Disposable {
		...
		const colorTheme = observableFromEvent(
			...
			() => {
				...
				return type === ColorScheme.DARK ||
					type === ColorScheme.HIGH_CONTRAST_DARK
					? "dark"
					: "light";
			},
		);
	...
	public async loadResource(
		token: CancellationToken,
	): Promise<IMcpAppResourceContent> {
		const server = await this._getServer(token);
		if (!server) {
			throw new Error("MCP server not found for UI resource");
		}

		const resourceResult = await McpServer.callOn(
			server,
			(h) => h.readResource({ uri: this._uiData.resourceUri }, token),
			token,
		);
