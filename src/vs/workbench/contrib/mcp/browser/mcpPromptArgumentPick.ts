...
export class McpPromptArgumentPick extends Disposable {
					...
					store.add(
						quickPick.onDidAccept(() => {
							...
							if (
								!quickPick.value &&
								arg.required &&
								(!item || item.action === "text" || item.action === "command")
							) {
	...
	private _activeFileCompletions() {
		const activeEditorChange = observableSignalFromEvent(
			this,
		...
		const resourceObs = activeEditor
			.map((e) =>
				e
					? observableSignalFromEvent(this, e.onDidChangeModel).map(
							() => e.getModel()?.uri,
						)
					: undefined,
			)
		...
		const selectionObs = activeEditor
			.map((e) =>
				e
					? observableSignalFromEvent(this, e.onDidChangeCursorSelection).map(
							() => ({ range: e.getSelection(), model: e.getModel() }),
						)
					: undefined,
			)
			.map((o, reader) => o?.read(reader));
		...
		return derived((reader) => {
			...
			if (
				...
				const lineCount =
					selection.range.endLineNumber - selection.range.startLineNumber + 1;
				const description =
					lineCount === 1
						? localize(
