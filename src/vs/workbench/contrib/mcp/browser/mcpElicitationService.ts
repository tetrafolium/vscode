...
const noneItem: IQuickPickItem = {
	id: undefined,
	label: localize("mcp.elicit.enum.none", "None"),
	description: localize("mcp.elicit.enum.none.description", "No selection"),
	alwaysShow: true,
};

type Pre20251125ElicitationParams = Omit<
	MCP.ElicitRequestFormParams,
	"mode"
> & { mode?: undefined };

function isFormElicitation(
	params: MCP.ElicitRequest["params"] | Pre20251125ElicitationParams,
): params is MCP.ElicitRequestFormParams | Pre20251125ElicitationParams {
	return (
		params.mode === "form" ||
		(params.mode === undefined &&
			!!(params as Pre20251125ElicitationParams).requestedSchema)
	);
}

function isUrlElicitation(
	params: MCP.ElicitRequest["params"],
): params is MCP.ElicitRequestURLParams {
	return params.mode === "url";
}

function isLegacyTitledEnumSchema(
	schema: MCP.PrimitiveSchemaDefinition,
): schema is MCP.LegacyTitledEnumSchema & { enumNames: string[] } {
	const cast = schema as MCP.LegacyTitledEnumSchema;
	return (
		cast.type === "string" &&
		Array.isArray(cast.enum) &&
		Array.isArray(cast.enumNames)
	);
}

function isUntitledEnumSchema(
	schema: MCP.PrimitiveSchemaDefinition,
): schema is MCP.LegacyTitledEnumSchema | MCP.UntitledSingleSelectEnumSchema {
	const cast = schema as
		| MCP.LegacyTitledEnumSchema
		| MCP.UntitledSingleSelectEnumSchema;
	return cast.type === "string" && Array.isArray(cast.enum);
}

function isTitledSingleEnumSchema(
	schema: MCP.PrimitiveSchemaDefinition,
): schema is MCP.TitledSingleSelectEnumSchema {
	const cast = schema as MCP.TitledSingleSelectEnumSchema;
	return cast.type === "string" && Array.isArray(cast.oneOf);
}

function isUntitledMultiEnumSchema(
	schema: MCP.PrimitiveSchemaDefinition,
): schema is MCP.UntitledMultiSelectEnumSchema {
	const cast = schema as MCP.UntitledMultiSelectEnumSchema;
	return cast.type === "array" && !!cast.items?.enum;
}

function isTitledMultiEnumSchema(
	schema: MCP.PrimitiveSchemaDefinition,
): schema is MCP.TitledMultiSelectEnumSchema {
...
export class McpElicitationService implements IMcpElicitationService {
	...
	public elicit(
		...
		elicitation: MCP.ElicitRequest["params"],
	...
	private async _elicitForm(
		server: IMcpServer,
		context: IMcpToolCallContext | undefined,
		elicitation: MCP.ElicitRequestFormParams | Pre20251125ElicitationParams,
		token: CancellationToken,
	): Promise<IFormModeElicitResult> {
		const store = new DisposableStore();
		const value = await new Promise<MCP.ElicitResult>((resolve) => {
			const chatModel =
				context?.chatSessionResource &&
				this._chatService.getSession(context.chatSessionResource);
			if (chatModel instanceof ChatModel) {
				const request = chatModel.getRequests().at(-1);
				if (request) {
					const { questions, idToPropertyMap } =
				...
				actions: {
					primary: [
						store.add(
							new Action(
								"mcp.elicit.give",
								localize("mcp.elicit.give", "Respond"),
								undefined,
								true,
								() => resolve(this._doElicitForm(elicitation, token)),
							),
						),
					],
					secondary: [
						store.add(
							new Action(
								"mcp.elicit.cancel",
								localize("mcp.elicit.cancel", "Cancel"),
								undefined,
								true,
			...
			store.add(
				token.onCancellationRequested(() => resolve({ action: "cancel" })),
			);
	...
	private async _elicitUrl(
		server: IMcpServer,
		context: IMcpToolCallContext | undefined,
		elicitation: MCP.ElicitRequestURLParams,
		token: CancellationToken,
	): Promise<IUrlModeElicitResult> {
		...
		const completePromise = new Promise<void>((resolve, reject) => {
			...
			promiseStore.add(
				autorun((reader) => {
					...
					if (handler) {
						reader.store.add(
							handler.onDidReceiveElicitationCompleteNotification((e) => {
						...
						!McpConnectionState.isRunning(server.connectionState.read(reader))
					) {
			...
			if (chatModel instanceof ChatModel) {
				...
				if (request) {
					const part = new ChatElicitationRequestPart(
						...
						new MarkdownString()
							.appendText(elicitation.message)
							.appendMarkdown(
								"\n\n" +
						...
						localize(
							"mcp.elicit.url.open",
							"Open {0}",
							URI.parse(elicitation.url).authority,
						),
						...
						primary: [
							store.add(
								new Action(
									...
									localize("mcp.elicit.url.open2", "Open URL"),
									undefined,
									true,
									() => resolve(this._doElicitUrl(elicitation, token)),
								),
							),
						],
						secondary: [
	...
	private async _doElicitUrl(
		elicitation: MCP.ElicitRequestURLParams,
		token: CancellationToken,
	): Promise<MCP.ElicitResult> {
		...
		try {
			if (
				await this._openerService.open(elicitation.url, {
					allowCommands: false,
				})
			) {
		...
		try {
			...
			for (let i = 0; i < properties.length; i++) {
				...
				if (schema.type === "boolean") {
					result = await this._handleEnumField(
						quickPick,
						{
							enum: [{ const: "true" }, { const: "false" }],
							default: schema.default ? String(schema.default) : undefined,
						},
						isRequired,
						store,
						token,
					...
					result = await this._handleEnumField(
						quickPick,
						{
							enum: schema.enum.map((v) => ({ const: v })),
							default: schema.default,
						},
						isRequired,
						store,
					...
					result = await this._handleEnumField(
						quickPick,
						{ enum: schema.oneOf, default: schema.default },
						isRequired,
						store,
						token,
					);
				} else if (isTitledMultiEnumSchema(schema)) {
					result = await this._handleMultiEnumField(
						quickPick,
						{ enum: schema.items.anyOf, default: schema.default },
						isRequired,
						store,
						token,
					);
					...
					result = await this._handleMultiEnumField(
						quickPick,
						{
							enum: schema.items.enum.map((v) => ({ const: v })),
							default: schema.default,
						},
						isRequired,
						store,
					...
					result = await this._handleInputField(
						quickPick,
						schema,
						isRequired,
						store,
	...
	private async _handleEnumField(
		...
		const items: IQuickPickItem[] = schema.enum.map(
			({ const: value, title }) => ({
				id: value,
				label: value,
				description: title,
		...
		>((resolve) => {
			store.add(
				token.onCancellationRequested(() => resolve({ type: "cancel" })),
			);
			store.add(
			...
			store.add(
				quickPick.onDidAccept(() => {
					...
					if (selected.id === undefined) {
						...
						resolve({
							type: "value",
							value: quickPick.selectedItems.map((i) => i.id).filter(isDefined),
						});
					}
				}),
			);
			...
			if (quickPick.value) {
				...
				if (schema.default) {
					items.push({
						...
						description: localize("mcp.elicit.useDefault", "Default value"),
					});
				}
			}
	...
	): { isValid: boolean; parsedValue?: string; message?: string } {
	...
	private _validateNumber(
		value: string,
		schema: MCP.NumberSchema,
	): { isValid: boolean; parsedValue?: number; message?: string } {
	...
	private _convertSchemaToQuestions(
		elicitation: MCP.ElicitRequestFormParams | Pre20251125ElicitationParams,
	): { questions: IChatQuestion[]; idToPropertyMap: Map<string, string> } {
		const properties = Object.entries(elicitation.requestedSchema.properties);
			...
			if (schema.type === "boolean") {
				questions.push({
					...
					options: [
						{
							id: "true",
							label: localize("mcp.elicit.true", "True"),
							value: "true",
						},
						{
							id: "false",
							label: localize("mcp.elicit.false", "False"),
							value: "false",
						},
					],
					defaultValue:
