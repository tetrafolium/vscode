...
} from "./mcpCommandsAddConfiguration.js";
import {
	McpResourceQuickAccess,
	McpResourceQuickPick,
} from "./mcpResourceQuickAccess.js";
				...
				pick.items = [
					...
					...Object.values(servers)
						...
						.flatMap((servers): (ItemType | IQuickPickSeparator)[] => [
							...
							...servers!.map((server) => {
								const disabled = isContributionDisabled(
...
export class McpConfirmationServerOptionsCommand extends Action2 {
	...
	override async run(
		...
		arg: IChatToolInvocation | IChatElicitationRequest,
	): Promise<void> {
		...
		if (arg.kind === "toolInvocation") {
			...
			if (arg.source?.type === "mcp") {
				accessor
					.get(ICommandService)
					.executeCommand(McpCommandIds.ServerOptions, arg.source.definitionId);
			}
		} else {
			assertNever(arg);
		}
...
export class McpServerOptionsCommand extends Action2 {
	...
	override async run(accessor: ServicesAccessor, id: string): Promise<void> {
		...
		const authenticationQueryService = accessor.get(
			IAuthenticationQueryService,
		);
		...
		const collection = mcpRegistry.collections
			.get()
		...
		items.push({
			type: "separator",
			label: localize("mcp.actions.status", "Status"),
		});
		...
		items.push(
			...this._getAuthActions(authenticationQueryService, server.definition.id),
		);

		const configTarget =
			serverDefinition?.presentation?.origin ||
		...
		items.push(
			{
				type: "separator",
				label: localize("mcp.actions.sampling", "Sampling"),
			},
			{
		...
		if (capabilities === undefined || capabilities & McpCapability.Resources) {
			items.push({
				type: "separator",
				label: localize("mcp.actions.resources", "Resources"),
			});
			items.push({
		...
		switch (pick.action) {
			case "enable":
				mcpService.enablementModel.setEnabled(
			...
			case "config":
				editorService.openEditor({
					...
					options: {
						selection: URI.isUri(configTarget)
							? undefined
							: configTarget!.range,
					},
				});
	...
	private _getAuthActions(
		...
		for (const [providerId, accountName] of authenticationQueryService
			.mcpServer(serverId)
			.getAllAccountPreferences()) {
			const accountQuery = authenticationQueryService
				.provider(providerId)
				.account(accountName);
	...
	private async _handleAuth(
		...
		accountQuery
			.mcpServer(definition.id)
			.setAccessAllowed(false, definition.label);
		if (signOut) {
...
export class MCPServerActionRendering
	extends Disposable
	implements IWorkbenchContribution
{
	constructor(
		...
		const config = observableConfigValue(
			mcpAutoStartConfig,
		...
		function isServer(
			s: IMcpServer | McpCollectionDefinition,
		): s is IMcpServer {
		...
		const displayedStateCurrent = derived((reader): DisplayedStateT => {
			...
			for (const server of servers) {
				...
				switch (server.cacheState.read(reader)) {
					...
					case McpServerCacheState.Outdated:
						thisState =
							server.connectionState.read(reader).state ===
			...
			if (unknownServerStates.state === LazyCollectionState.LoadingUnknown) {
				...
				serversPerState[DisplayedState.Refreshing].push(
					...unknownServerStates.collections,
				);
			} else if (unknownServerStates.state === LazyCollectionState.HasUnknown) {
				serversPerState[DisplayedState.NewTools] ??= [];
				serversPerState[DisplayedState.NewTools].push(
					...unknownServerStates.collections,
				);
			}
			...
			if (
				maxState === DisplayedState.NewTools &&
				config.read(reader) !== McpAutoStartValue.Never
			) {
		...
		const displayedState = derivedObservableWithCache<DisplayedStateT>(
			this,
			(reader, last) => {
					...
					return instaService.createInstance(
						class extends MenuEntryActionViewItem {
							override render(container: HTMLElement): void {
								...
								const stateIndicator = container.appendChild(
									...
									autorun((r) => {
										...
										if (state === DisplayedState.NewTools) {
											...
											stateIndicator.classList.add(
												"chat-mcp-state-new",
												...ThemeIcon.asClassNameArray(Codicon.refresh),
											);
											...
											stateIndicator.classList.add(
												...
												...ThemeIcon.asClassNameArray(spinningLoading),
											);
										} else {
											stateIndicator.style.display = "none";
										}
							...
							override async onClick(e: MouseEvent): Promise<void> {
								...
								if (state === DisplayedState.NewTools) {
									...
									servers
										.filter(isServer)
										.forEach((server) =>
											server.stop().then(() => server.start({ interaction })),
										);
									mcpService.activateCollections();
								} else if (state === DisplayedState.Refreshing) {
							...
							protected override getHoverContents(
								{ state, servers } = displayedStateCurrent.get(),
							): string | undefined | IManagedHoverTooltipHTMLElement {
								const link = (s: IMcpServer) =>
									createMarkdownCommandLink({
								...
								const names = servers
									.map((s) => (isServer(s) ? link(s) : "`" + s.label + "`"))
									.map((l) => (single ? l : `- ${l}`))
									.join("\n");
								...
								return {
									element: (token): HTMLElement => {
										...
										const store = new DisposableStore();
										store.add(
											toDisposable(() => hoverIsOpen.set(false, undefined)),
										);
										store.add(
...
export class AddConfigurationAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, configUri?: string): Promise<void> {
		...
		const target = configUri
			? workspaceService.getWorkspaceFolder(URI.parse(configUri))
			: undefined;
		return instantiationService
			.createInstance(McpAddConfigurationCommand, target ?? undefined)
			.run();
	}
}
...
export class EditStoredInput extends Action2 {
	...
	run(
		...
		const workspaceFolder =
			uri && accessor.get(IWorkspaceContextService).getWorkspaceFolder(uri);
		accessor
			.get(IMcpRegistry)
			.editSavedInput(
				inputId,
...
export class SetOAuthClientSecret extends Action2 {
	...
	async run(
		...
		const result = await new Promise<
			{ kind: "accept"; value: string } | { kind: "delete" } | undefined
		>((resolve) => {
...
export class StartServer extends Action2 {
	...
	async run(
		accessor: ServicesAccessor,
		serverId: string,
		opts?: IMcpServerStartOpts & { waitForLiveTools?: boolean },
	) {
		...
		const startOpts: IMcpServerStartOpts = {
			promptType: "all-untrusted",
			...opts,
		};
		if (opts?.waitForLiveTools) {
			await Promise.all(
				servers.map((s) => startServerAndWaitForLiveTools(s, startOpts)),
			);
		} else {
		...
		super({
			...
			menu: [
				...
				},
				{
					id: MenuId.ViewTitle,
					when: ContextKeyExpr.and(
						ContextKeyExpr.equals("view", InstalledMcpServersViewId),
						ChatContextKeys.Setup.hidden.negate(),
						ChatContextKeys.Setup.disabledInWorkspace.negate(),
					),
					group: "navigation",
	...
	async run(accessor: ServicesAccessor) {
		...
		if (!(await fileService.exists(resource))) {
			await fileService.createFile(
				resource,
				VSBuffer.fromString(JSON.stringify({ servers: {} }, null, "\t")),
			);
		}
...
export class OpenRemoteUserMcpResourceCommand extends OpenMcpResourceCommand {
	...
	protected override async getURI(accessor: ServicesAccessor): Promise<URI> {
		const userDataProfileService = accessor.get(IUserDataProfileService);
		const remoteUserDataProfileService = accessor.get(
			IRemoteUserDataProfilesService,
		);
		const remoteProfile = await remoteUserDataProfileService.getRemoteProfile(
	...
	async run(accessor: ServicesAccessor) {
		...
		if (workspaceFolder) {
			await editorService.openEditor({
				resource: workspaceFolder.toResource(
	...
	run(accessor: ServicesAccessor, server?: IMcpServer): void {
		if (server) {
			accessor
				...
				.createInstance(McpResourceQuickPick, server)
				.pick();
		} else {
			accessor
				.get(IQuickInputService)
				.quickAccess.show(McpResourceQuickAccess.PREFIX);
		}
	}
}
...
export class McpConfigureSamplingModels extends Action2 {
	...
	async run(accessor: ServicesAccessor, server: IMcpServer): Promise<number> {
		...
		const allItems: IQuickPickItem[] = lmService
			.getLanguageModelIds()
			.map((id) => {
				...
				return {
					...
					id,
					picked: existingIds.size
						? existingIds.has(id)
						: model.isDefaultForLocation[ChatAgentLocation.Chat],
				};
			})
			.filter(isDefined);
		...
		if (picked) {
			await mcpSampling.updateConfig(
				server,
				(c) => (c.allowedModels = picked.map((p) => p.id!)),
			);
		}
...
export class McpStartPromptingServerCommand extends Action2 {
	...
	async run(accessor: ServicesAccessor, server: IMcpServer): Promise<void> {
		...
		const range = (
			editor.getSelection() || model.getFullModelRange()
		).collapseToEnd();
		const text = mcpPromptPrefix(server.definition) + ".";

		model.applyEdits([{ range, text }]);
		editor.setSelection(
			Range.fromPositions(range.getEndPosition().delta(0, text.length)),
		);
		widget.focusInput();
