...
suite("McpGatewayToolBrokerChannel", () => {
	...
	test("lists model-visible tools for a specific server", async () => {
		...
		const channel = new McpGatewayToolBrokerChannel(
			mcpService,
			new NullLogService(),
		);

		const serverA = createServer("collectionA", "serverA", [
			createTool("mcp_serverA_echo", async () => ({
				content: [{ type: "text", text: "A" }],
			})),
			createTool(
				"app-only",
				async () => ({ content: [{ type: "text", text: "A2" }] }),
				McpToolVisibility.App,
			),
		]);
		const serverB = createServer("collectionB", "serverB", [
			createTool("mcp_serverB_echo", async () => ({
		...
		const resultA = await channel.call<readonly MCP.Tool[]>(
			undefined,
			"listToolsForServer",
			{ serverId: "serverA" },
		);
		assert.deepStrictEqual(
			resultA.map((t) => t.name),
			["mcp_serverA_echo"],
		);

		const resultB = await channel.call<readonly MCP.Tool[]>(
			undefined,
			"listToolsForServer",
			{ serverId: "serverB" },
		);
		assert.deepStrictEqual(
			resultB.map((t) => t.name),
			["mcp_serverB_echo"],
		);
	...
	test("routes tool calls to specific server", async () => {
		...
		const resultA = await channel.call<MCP.CallToolResult>(
			undefined,
			"callToolForServer",
			{
				serverId: "serverA",
		...
		const resultB = await channel.call<MCP.CallToolResult>(
			undefined,
			"callToolForServer",
			{
				serverId: "serverB",
	...
	test("emits onDidChangeTools when tool lists change", async () => {
		...
		let events = 0;
		const disposable = channel.listen<void>(
			undefined,
			"onDidChangeTools",
		)(() => {
			events++;
	...
	test("does not start server when cache state is live", async () => {
		...
		mcpService.servers.set([server], undefined);
		await channel.call<readonly MCP.Tool[]>(undefined, "listToolsForServer", {
			serverId: "serverA",
		});
	...
	test("starts server when cache state is unknown", async () => {
		...
		mcpService.servers.set([server], undefined);
		const tools = await channel.call<readonly MCP.Tool[]>(
			undefined,
			"listToolsForServer",
			{ serverId: "serverA" },
		);
	...
	test("returns empty tools and does not re-wait if server does not start within grace period", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const server = createNeverStartingServer("collectionA", "serverA", [
				createTool("echo", async () => ({
					content: [{ type: "text", text: "A" }],
				})),
			]);
			...
			const tools2 = await channel.call<readonly MCP.Tool[]>(
				undefined,
				"listToolsForServer",
				{ serverId: "serverA" },
			);
	...
	test("invalidates stale grace entry when cacheState regresses to Unknown after timeout", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const tools1 = await channel.call<readonly MCP.Tool[]>(
				undefined,
				"listToolsForServer",
				{ serverId: "serverA" },
			);
	...
	test("listServers returns all servers regardless of cache state", async () => {
		...
		const unknownServer = createServer(
			"collectionB",
			"serverB",
			[],
			McpServerCacheState.Unknown,
		...
		const servers = await channel.call<readonly IMcpGatewayServerDescriptor[]>(
			undefined,
			"listServers",
		);
	...
	test("forwards chatSessionResource as tool call context", async () => {
		...
		const server = createServer("collectionA", "serverA", [
			createToolWithContextCapture("echo", receivedContexts, async () => ({
		...
		assert.strictEqual(
			receivedContexts[0]!.chatSessionResource!.toString(),
	...
	test("emits onDidChangeServers with descriptors when servers change", () => {
		...
		const disposable = channel.listen<readonly IMcpGatewayServerDescriptor[]>(
			undefined,
			"onDidChangeServers",
		)((e) => {
			received.push(e);
		...
		assert.deepStrictEqual(received[1], [{ id: "serverB", label: "serverB" }]);
...
function createServer(
	...
	toolsValue: ReturnType<typeof observableValue<readonly IMcpTool[]>>;
	startCalls: number;
} {
	const owner = {};
	const tools = observableValue<readonly IMcpTool[]>(owner, initialTools);
	const connectionState = observableValue<McpConnectionState>(owner, {
		state: McpConnectionState.Kind.Running,
	});
	const cacheState = observableValue<McpServerCacheState>(
		owner,
		initialCacheState,
	);
	let startCalls = 0;
	...
	return {
		...
		readDefinitions: () =>
			observableValue(owner, { server: undefined, collection: undefined }),
		showOutput: async () => {},
		start: async () => {
			startCalls++;
...
function createNeverStartingServer(
	...
	startCalls: number;
	startBehavior: "hang" | "succeed";
	cacheStateValue: ReturnType<typeof observableValue<McpServerCacheState>>;
} {
	const owner = {};
	const tools = observableValue<readonly IMcpTool[]>(owner, initialTools);
	const connectionState = observableValue<McpConnectionState>(owner, {
