...
suite("Workbench - MCP - ResourceFilesystem", () => {
	...
	test("stat throws FileNotFound for nonexistent resources", async () => {
		...
		await assert.rejects(
			() =>
				fs.stat(
					URI.parse(
						"mcp-resource://746573742D736572766572/custom/nonexistent.txt",
					),
				),
			(err: Error) =>
	...
	test("readdir throws when reading a file as directory", async () => {
		...
		await assert.rejects(
			() =>
				fs.readdir(
					URI.parse(
						"mcp-resource://746573742D736572766572/custom/hello/file.txt",
					),
				),
			(err: Error) =>
				toFileSystemProviderErrorCode(err) ===
	...
	test("throws error for write operations", async () => {
		...
		await assert.rejects(
			async () =>
				fs.delete(uri, { recursive: false, useTrash: false, atomic: false }),
			(err: Error) =>
				toFileSystemProviderErrorCode(err) ===
				FileSystemProviderErrorCode.NoPermissions,
		);
		...
		await assert.rejects(
			async () =>
				fs.rename(
					uri,
					URI.parse(
						"mcp-resource://746573742D736572766572/custom/hello/newfile.txt",
					),
					{ overwrite: false },
				),
			(err: Error) =>
