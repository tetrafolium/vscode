...
suite("MCP - Sampling Log", () => {
	...
	test("logs multiple requests on the same day", async () => {
		...
		await storage.flush();
		const data = (
			storage.getObject("mcp.sampling.logs", StorageScope.APPLICATION) as [
				string,
				any,
			][]
		)[0][1];
		...
		assert.strictEqual(
			data.lastReqs[0].request[0].content.text,
			"second request",
		);
		assert.strictEqual(
			data.lastReqs[1].request[0].content.text,
			"first request",
		);
	});
	...
	test("limits the number of stored requests", async () => {
		...
		assert.strictEqual(
			(data.lastReqs[0].request[0].content as { type: "text"; text: string })
				.text,
			"request 34",
		);
		assert.strictEqual(
			(data.lastReqs[29].request[0].content as { type: "text"; text: string })
				.text,
			"request 5",
		);
	});
	...
	test("handles different content types", async () => {
		...
		assert.strictEqual(
			asArray(data.lastReqs[1].request[0].content)[0].type,
			"image",
		);
		assert.strictEqual(
			asArray(data.lastReqs[2].request[0].content)[0].type,
			"text",
		);
	});
	...
	test("handles multiple servers", async () => {
		...
		await storage.flush();
		const storageData = storage.getObject(
			"mcp.sampling.logs",
			StorageScope.APPLICATION,
		) as [string, ISamplingStoredData][];
