...
suite("MCP Icons", () => {
	suite("parseAndValidateMcpIcon", () => {
		...
		test("includes supported icons and sorts sizes ascending", () => {
			...
			assert.strictEqual(
				(result[0].src as URI).toString(true),
				"data:image/png;base64,AAA",
			);
			assert.deepStrictEqual(
				result[0].sizes.map((s) => s.width),
				[16, 64],
			);
			assert.strictEqual(
				result[1].src.toString(),
		...
		test("requires http transport with matching authority for remote icons", () => {
			...
			assert.deepStrictEqual(
				httpResult.map((icon) => icon.src.toString()),
				["https://example.com/icon.png"],
			);
