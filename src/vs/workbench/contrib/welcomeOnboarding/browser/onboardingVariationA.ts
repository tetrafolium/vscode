...
import {
	...
	ChatSetupStrategy,
} from "../../chat/browser/chatSetup/chatSetup.js";
...
type OnboardingStepViewClassification = {
	...
	step: {
		...
		comment: "The step identifier.";
	};
	stepNumber: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "The 1-based step index.";
	};
};
...
type OnboardingActionClassification = {
	...
	action: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The action performed.";
	};
	step: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The step the action was performed on.";
	};
	argument: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Optional context such as theme id, extension id, or provider.";
	};
};
 ...
 */
export class OnboardingVariationA
	extends Disposable
	implements IOnboardingService
{
	...
	show(): void {
		...
		this.previouslyFocusedElement = getActiveWindow().document.activeElement as
			| HTMLElement
			| undefined;
		...
		this.backButton = append(
			footerRight,
			$<HTMLButtonElement>(
				"button.onboarding-a-btn.onboarding-a-btn-secondary",
			),
		);
		...
		this.disposables.add(
			addDisposableListener(this.backButton, EventType.CLICK, () => {
				if (
		...
		this.disposables.add(
			addDisposableListener(this.nextButton, EventType.CLICK, () => {
				if (this._isLastStep()) {
	...
	private _updateButtonStates(): void {
		if (this.backButton) {
			const showEnterpriseBack =
				this.currentStepIndex === 0 &&
				this.enterpriseSignInUiState === "instance";
			this.backButton.style.display =
				this.currentStepIndex === 0 && !showEnterpriseBack ? "none" : "";
		}
		if (this.nextButton) {
			if (this.currentStepIndex === 0) {
				if (this._userSignedIn) {
					this.nextButton.className =
						"onboarding-a-btn onboarding-a-btn-primary";
					...
					this.nextButton.className =
						"onboarding-a-btn onboarding-a-btn-secondary";
					this.nextButton.textContent = localize(
	...
	private _renderEnterpriseInstanceForm(actions: HTMLElement): void {
		...
		const inputBox = this.stepDisposables.add(
		...
		const submit = async () => {
			...
			if (
				result.kind === GheParseResultKind.Empty ||
				result.kind === GheParseResultKind.Invalid
			) {
				validate();
				return;
			}
		...
		this.stepDisposables.add(
			addDisposableListener(input, EventType.KEY_DOWN, (e) => {
				const event = new StandardKeyboardEvent(e);
	...
	private _setEnterpriseSignInUiState(state: EnterpriseSignInUiState): void {
		...
		if (
			this.steps[this.currentStepIndex] === OnboardingStepId.SignIn &&
			this.contentEl
		) {
			this._renderStep();
			this._updateButtonStates();
	...
	private _createSignInButton(
		parent: HTMLElement,
		providerClass: "github" | "github-enterprise" | "google" | "apple",
		label: string,
		options?: {
			emphasized?: boolean;
			iconOnly?: boolean;
		...
		const btn = append(
			...
			$<HTMLButtonElement>(
				isCompact
					? "button.onboarding-a-signin-icon-btn"
					: "button.onboarding-a-signin-btn",
			),
		);
		btn.type = "button";
	...
	private async _handleEnterpriseSignIn(): Promise<void> {
		...
		if (
			typeof existingUri !== "string" ||
			!GHE_FULL_URI_REGEX.test(existingUri)
		) {
	...
	private _renderPersonalizeStep(container: HTMLElement): void {
		...
		const keymapOptions = this._detectedEditorIds
			? (product.onboardingKeymaps ?? []).filter((k) =>
					this._detectedEditorIds!.has(k.id),
				)
			: [];
		...
		if (hasOtherEditors) {
			...
			for (const keymap of keymapOptions) {
				...
				this.stepDisposables.add(
					addDisposableListener(pill, EventType.CLICK, () => {
				...
				localize(
					{
						...
						comment: [
							"This is a keyboard modifier key, Ctrl on Windows/Linux or Cmd on Mac",
						],
					},
					"{0}",
					modifier,
	...
	private _createThemeCard(
		...
		img.alt = "";
		img.src = FileAccess.asBrowserUri(
			`vs/workbench/contrib/welcomeOnboarding/browser/media/theme-preview-${theme.id}.svg`,
		).toString(true);
		...
		this.stepDisposables.add(
			addDisposableListener(card, EventType.KEY_DOWN, (e: KeyboardEvent) => {
		...
		if (isWindows) {
			...
			checks.push(
				{
					id: "sublime",
					paths: [
						URI.file("C:\\Program Files\\Sublime Text\\sublime_text.exe"),
						URI.file("C:\\Program Files\\Sublime Text 3\\sublime_text.exe"),
					],
				},
				{
					id: "intellij",
				...
				},
				{
					id: "vim",
					paths: [
						URI.joinPath(home, "_vimrc"),
						URI.joinPath(localAppData, "nvim", "init.vim"),
						URI.joinPath(localAppData, "nvim", "init.lua"),
					],
				},
				{
					id: "eclipse",
					paths: [
						URI.file("C:\\Program Files\\Eclipse\\eclipse.exe"),
						URI.file("C:\\Program Files\\eclipse\\eclipse.exe"),
					],
				},
				{
					id: "notepadpp",
					paths: [
						URI.file("C:\\Program Files\\Notepad++\\notepad++.exe"),
						URI.file("C:\\Program Files (x86)\\Notepad++\\notepad++.exe"),
					],
				},
			);
		} else if (isMacintosh) {
			checks.push(
				{ id: "sublime", paths: [URI.file("/Applications/Sublime Text.app")] },
				{
					id: "intellij",
					paths: [
						URI.file("/Applications/IntelliJ IDEA.app"),
						URI.file("/Applications/IntelliJ IDEA CE.app"),
					],
				},
				{
					id: "vim",
					paths: [
						URI.joinPath(home, ".vimrc"),
						URI.joinPath(home, ".config", "nvim", "init.vim"),
						URI.joinPath(home, ".config", "nvim", "init.lua"),
					],
				},
				{
					id: "eclipse",
					paths: [
						URI.file("/Applications/Eclipse.app"),
						URI.file("/Applications/Eclipse IDE.app"),
					],
				},
			...
			checks.push(
				{
					id: "sublime",
					paths: [
						URI.file("/usr/bin/subl"),
						URI.file("/opt/sublime_text/sublime_text"),
					],
				},
				{
					id: "intellij",
					paths: [
						URI.joinPath(home, ".local", "share", "JetBrains", "Toolbox"),
						URI.file("/opt/idea"),
					],
				},
				{
				...
				},
				{
					id: "eclipse",
					paths: [
						URI.file("/usr/bin/eclipse"),
						URI.file("/opt/eclipse/eclipse"),
						URI.joinPath(home, "eclipse", "eclipse"),
					],
				},
				{
					id: "notepadpp",
					paths: [
						URI.file("/usr/bin/notepadqq"),
						URI.file("/snap/notepad-plus-plus/current"),
					],
				},
			);
		}
	...
	private _renderAiPreferenceStep(container: HTMLElement): void {
		...
		for (const option of ONBOARDING_AI_PREFERENCE_OPTIONS) {
			...
			const icon =
				Codicon[option.icon as keyof typeof Codicon] ?? Codicon.sparkle;
			...
			this.stepDisposables.add(
				addDisposableListener(card, EventType.CLICK, () => {
					...
					for (const c of allCards) {
						...
						c.setAttribute(
							"aria-checked",
							c.dataset.id === option.id ? "true" : "false",
						);
					}
	 ...
	 */
	private _setupRadioGroupNavigation(
		items: HTMLElement[],
	...
	private _getFocusableElements(): HTMLElement[] {
		...
		].filter((element) => this._isTabbable(element));
	}

	private _focusCurrentStepElement(): void {
		const stepFocusable = this.stepFocusableElements.find((element) =>
			this._isTabbable(element),
		);
	...
	private _logStepView(): void {
		...
		this.telemetryService.publicLog2<
			...
			OnboardingStepViewClassification
	...
	private _logAction(
		action: string,
		stepOverride?: OnboardingStepId,
		argument?: string,
	): void {
		this.telemetryService.publicLog2<
			OnboardingActionEvent,
			OnboardingActionClassification
