		...
		return [
			{
				...
				buttons:
					button && defaultLogLevel !== LogLevel.Trace ? [button] : undefined,
			},
			{
				...
				buttons:
					button && defaultLogLevel !== LogLevel.Debug ? [button] : undefined,
			},
			{
				...
				buttons:
					button && defaultLogLevel !== LogLevel.Info ? [button] : undefined,
			},
			{
				...
				buttons:
					button && defaultLogLevel !== LogLevel.Warning ? [button] : undefined,
			},
			{
				...
				buttons:
					button && defaultLogLevel !== LogLevel.Error ? [button] : undefined,
			},
			{
				...
				buttons:
					button && defaultLogLevel !== LogLevel.Off ? [button] : undefined,
			},
		];
	}
			...
			this.getSessions().then((sessions) =>
				sessions.map(
					(s, index): IQuickPickItem => ({
						...
						label: basename(s),
						description:
							index === 0 ? nls.localize("current", "Current") : undefined,
					}),
				),
			),
			{
