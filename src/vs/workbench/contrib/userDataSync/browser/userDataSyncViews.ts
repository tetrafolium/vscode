						...
						super({
							...
							menu: {
								...
								when: ContextKeyExpr.and(
									...
									ContextKeyExpr.equals("viewItem", "sync-machine"),
								),
							},
						});
					}
					async run(
						...
						if (
							await dataProvider.disable(
								(selected || [handle]).map((handle) => handle.$treeItemHandle),
							)
						) {
							await treeView.refresh();
						}
		...
		if (!element) {
			...
			if (stat.isDirectory) {
				...
				this.activityDataLocation = this.uriIdentityService.extUri.joinPath(
					this.uriIdentityService.extUri.dirname(this.activityDataResource),
					"remoteActivity",
				);
				try {
...
class UserDataSyncMachinesViewDataProvider implements ITreeViewDataProvider {
		...
		try {
			...
			return machines.map(({ id, name, isCurrent, platform }) => ({
				...
				description: isCurrent
					...
					: undefined,
				themeIcon:
					platform && isWebPlatform(platform) ? Codicon.globe : Codicon.vm,
	...
	async rename(machineId: string): Promise<boolean> {
		...
		if (!machine) {
			...
			throw new Error(
				localize("not found", "machine not found with id: {0}", machineId),
			);
		}
		inputBox.busy = false;
		...
		disposableStore.add(
			inputBox.onDidChangeValue(
				() =>
					(inputBox.validationMessage = validateMachineName(inputBox.value)
						? ""
						: localize(
								"valid message",
