	...
	implements IUserDataSyncConflictsView
{
	...
	private async getTreeItems(): Promise<readonly ITreeItem[]> {
		...
		for (const previewResource of conflictResources) {
			let result =
				conflictResourcesByProfile[conflictResourcesByProfile.length - 1]?.[0]
					.id === previewResource.profile.id
						...
						super({
							...
							menu: {
								...
								when: ContextKeyExpr.and(
									ContextKeyExpr.equals("view", SYNC_CONFLICTS_VIEW_ID),
									ContextKeyExpr.equals("viewItem", "sync-conflict-resource"),
								),
								group: "inline",
								order: 1,
							},
