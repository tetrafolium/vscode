	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		if (userDataSyncWorkbenchService.enabled) {
			...
			this._register(
				userDataAutoSyncService.onError((error) => this.onAutoSyncError(error)),
			);
			...
			textModelResolverService.registerTextModelContentProvider(
				USER_DATA_SYNC_SCHEME,
				instantiationService.createInstance(UserDataRemoteContentProvider),
			);
						...
						[
							...
							},
							{
								...
								run: () => {
									...
									>("sync/showConflicts", { source: conflict.syncResource });
		...
		switch (error.code) {
			...
			case UserDataSyncErrorCode.TooLarge:
				if (
					...
					error.resource === SyncResource.Tasks
				) {
					...
					actions: {
						primary: [
							toAction({
								...
								run: () =>
									this.commandService.executeCommand(SHOW_SYNC_LOG_COMMAND_ID),
							}),
							toAction({
								id: "Report Issue",
	...
	private readonly invalidContentErrorDisposables = new Map<
		string,
		IDisposable
	>();
	...
	private async updateGlobalActivityBadge(): Promise<void> {
		...
		if (
			this.userDataSyncService.conflicts.length &&
			this.userDataSyncEnablementService.isEnabled()
		) {
			badge = new NumberBadge(this.getConflictsCount(), () =>
				localize("has conflicts", "{0}: Conflicts Detected", SYNC_TITLE.value),
			);
		...
		if (badge) {
			this.globalActivityBadgeDisposable.value =
				this.activityService.showGlobalActivity({ badge });
		}
	}
	...
	private async askToConfigure(): Promise<boolean> {
		return new Promise<boolean>((c, e) => {
			...
			const quickPick =
				this.quickInputService.createQuickPick<ConfigureSyncQuickPickItem>();
			...
			quickPick.selectedItems = items.filter((item) =>
				this.userDataSyncEnablementService.isResourceEnabled(item.id, true),
			);
	...
	private updateConfiguration(
		...
		for (const item of items) {
			const wasEnabled = this.userDataSyncEnablementService.isResourceEnabled(
				item.id,
			);
	...
	private readonly conflictsActionDisposable = this._register(
		new MutableDisposable(),
	);
							...
							menu: [
								{
									...
									when: ContextKeyExpr.and(
										...
										ContextKeyExpr.regex(
											ctxMergeBaseUri.key,
											new RegExp(`^${USER_DATA_SYNC_SCHEME}:`),
										),
									),
								},
							],
						...
						const conflict = that.userDataSyncService.conflicts.find(
							({ conflicts }) =>
								conflicts.some((conflict) =>
									isEqual(conflict.previewResource, previewResource),
								),
						);
						if (conflict) {
							return conflict;
