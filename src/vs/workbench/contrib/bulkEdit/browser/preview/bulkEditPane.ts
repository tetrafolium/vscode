...
export class BulkEditPane extends ViewPane {
	...
	protected override renderBody(parent: HTMLElement): void {
		...
		this._tree = this._instaService.createInstance(
			WorkbenchAsyncDataTree<BulkFileOperations, BulkEditElement, FuzzyScore>,
			this.id,
			treeContainer,
	...
	toggleChecked() {
		...
		if (
			(first instanceof FileElement || first instanceof TextEditElement) &&
			!first.isDisabled()
		) {
	...
	>(async (fileOperations) => {
		...
		>(async (fileOperation) => {
			...
			// rename, create, edits
				...
				try {
					(
						await this._textModelService.createModelReference(fileOperationUri)
					).dispose();
