...
export class FileElement implements ICheckable {
	...
	isChecked(): boolean {
		const model =
			this.parent instanceof CategoryElement ? this.parent.parent : this.parent;
		...
		if (
			this.parent instanceof CategoryElement &&
			this.edit.type === BulkFileOperationType.TextEdit
		) {
...
export class BulkEditDataSource implements IAsyncDataSource<
	BulkFileOperations,
	BulkEditElement
> {
	...
	async getChildren(
		...
		if (element instanceof FileElement && element.edit.textEdits.length > 0) {
			...
			const result = element.edit.textEdits.map((edit, idx) => {
				...
				for (
					let idx =
						startTokens.findTokenIndexAtOffset(range.startColumn - 1) - 1;
					prefixLen < 50 && idx >= 0;
					idx--
				) {
				...
				let suffixLen = 0;
				for (
					let idx = endTokens.findTokenIndexAtOffset(range.endColumn - 1);
					suffixLen < 50 && idx < endTokens.getCount();
					idx++
				) {
					suffixLen +=
						endTokens.getEndOffset(idx) - endTokens.getStartOffset(idx);
				}
				...
				return new TextEditElement(
					...
					!edit.textEdit.textEdit.insertAsSnippet
						...
						: SnippetParser.asInsertText(edit.textEdit.textEdit.text),
					textModel.getValueInRange(
						new Range(
							range.endLineNumber,
							range.endColumn,
...
export function compareBulkFileOperations(
	a: BulkFileOperation,
	b: BulkFileOperation,
): number {
...
export class CategoryElementRenderer implements ITreeRenderer<
	CategoryElement,
	FuzzyScore,
	...
	renderElement(
		...
		if (ThemeIcon.isThemeIcon(metadata.iconPath)) {
			...
			template.icon.style.color = metadata.iconPath.color
				? (this._themeService
						.getColorTheme()
						.getColor(metadata.iconPath.color.id)
						?.toString() ?? "")
				: "";
...
export class FileElementRenderer implements ITreeRenderer<
	FileElement,
	FuzzyScore,
	FileElementTemplate
> {
...
export class TextEditElementRenderer implements ITreeRenderer<
	TextEditElement,
	FuzzyScore,
