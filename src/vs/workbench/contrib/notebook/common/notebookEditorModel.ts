	...
	implements INotebookEditorModel
{
	...
	isOrphaned(): boolean {
		return (
			SimpleNotebookEditorModel._isStoredFileWorkingCopy(this._workingCopy) &&
	...
	private static _isStoredFileWorkingCopy(
		...
		const isUntitled =
			candidate && candidate.capabilities & WorkingCopyCapabilities.Untitled;
	...
	implements IStoredFileWorkingCopyModel, IUntitledFileWorkingCopyModel
{
	...
	constructor(
		...
		const saveWithReducedCommunication = this._configurationService.getValue(
		...
		if (saveWithReducedCommunication) {
			this.setSaveDelegate().catch((error) =>
				this._notebookLogService.error(
					"WorkingCopyModel",
					`Failed to set save delegate: ${error}`,
				),
			);
		}
	}
			...
			try {
				...
				if (!serializer) {
					...
					serializer = await this.getNotebookSerializer().catch((error) => {
						this._notebookLogService.error(
							"WorkingCopyModel",
							`Failed to get notebook serializer: ${error}`,
						);
