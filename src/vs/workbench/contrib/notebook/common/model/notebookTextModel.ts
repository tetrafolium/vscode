	...
	implements INotebookTextModel
{
	...
	constructor(
		...
		this._pauseableEmitter = this._register(
			new NotebookEventEmitter({
		...
		this._notebookLoggingService.trace(
			"notebookTextModel",
			`Initialized notebook text model for ${uri.toString()}`,
		);
	}
	...
	_initialize(cells: ICellDto2[], triggerDirty?: boolean) {
		...
		const mainCells = cells.map((cell) => {
			...
			return new NotebookCellTextModel(
				...
				this._modelService.getCreationOptions(cell.language, cellUri, false)
					.defaultEOL,
	...
	private isOnlyEditingMetadataOnNewCells(
		...
		for (const edit of rawEdits) {
			...
			if (
				hasKey(edit, { index: true }) &&
				!this.newCellsFromLastEdit.has(this.cells[edit.index].handle)
			) {
				return false;
			}
			if (
				hasKey(edit, { handle: true }) &&
				!this.newCellsFromLastEdit.has(edit.handle)
			) {
				return false;
			}
		}
	...
	applyEdits(
		...
		this._notebookLoggingService.trace(
			"textModelEdits",
			`Begin applying ${rawEdits.length} raw edits`,
		);
		this._pauseableEmitter.pause();
		try {
			...
			try {
				...
				this._notebookLoggingService.error(
					"textModelEdits",
					`Error while applying edits: ${err}`,
				);
				throw err;
			} finally {
				if (!this._pauseableEmitter.isEmpty) {
					...
					this._notebookLoggingService.trace(
						"textModelEdits",
						`End applying ${rawEdits.length} raw edits`,
					);
				}
			}
		} finally {
		...
		const editsWithDetails = rawEdits
			.map((edit, index) => {
				...
				if (hasKey(edit, { index: true })) {
					...
					this._assertIndex(
						cellIndex,
						`editType: ${edit.editType}, key: handle`,
					);
	...
	private _replaceCells(
		...
		const cells = cellDtos.map((cellDto) => {
			...
			const cell = new NotebookCellTextModel(
				...
				this.transientOptions,
				this._languageService,
				this._modelService.getCreationOptions(cellDto.language, cellUri, false)
					.defaultEOL,
				this._defaultCollapseConfig,
				this._languageDetectionService,
				this._notebookLoggingService,
	...
	private _assertIndex(index: number, context?: string) {
		if (this._indexIsInvalid(index)) {
			throw new Error(
				`model index out of range ${index} (cellCount: ${this._cells.length}${context ? `, ${context}` : ""})`,
			);
		}
	}
