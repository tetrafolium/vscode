...
abstract class KernelPickerStrategyBase implements IKernelPickerStrategy {
	...
	async showQuickPick(
		...
		const kernelDetectionTaskListener =
			this._notebookKernelService.onDidChangeKernelDetectionTasks(() => {
				quickPick.busy =
					this._notebookKernelService.getKernelDetectionTasks(notebook).length >
					0;
			});
		...
		const kernelChangeEventListener = Event.debounce<void, void>(
			...
			for (const item of currentActiveItems) {
				if (isKernelPick(item)) {
					...
					const sameItem = quickPickItems.find(
						(pi) =>
							isSourcePick(pi) && pi.action.action.id === item.action.action.id,
	...
	protected async _showKernelExtension(
		...
		for (const extId of extIds) {
			...
			if (
				...
				extension.enablementState === EnablementState.DisabledByEnvironment
			) {
				extensionsToEnable.push(extension);
			} else if (
				!extensionWorkbenchService.installed.some((e) =>
				...
				if (
					extensionWorkbenchService.installed.some(
						(e) =>
							areSameExtensions(e.identifier, extension.identifier) &&
							e.server ===
								extensionManagementServerService.remoteExtensionManagementServer,
	...
	protected async _getKernelRecommendationsQuickPickItems(
		...
		QuickPickInput<SearchMarketplacePick | InstallExtensionPick>[] | undefined
	> {
		...
		if (suggestedExtension) {
			...
			const extensions = extensionWorkbenchService.installed.filter(
				(e) =>
					(e.enablementState === EnablementState.EnabledByEnvironment ||
						e.enablementState === EnablementState.EnabledGlobally ||
						e.enablementState === EnablementState.EnabledWorkspace) &&
	...
	private getSuggestedLanguage(
		...
		const language_info = (metaData?.metadata as Record<string, unknown>)
			?.language_info as Record<string, string> | undefined;
...
export class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
	constructor(
		...
		@IExtensionManagementServerService
		_extensionManagementServerService: IExtensionManagementServerService,
				...
				quickPick.onDidTriggerItemButton(async (e) => {
					if (
						...
						const uri = URI.isUri(e.item.documentation)
							...
							: await this._commandService.executeCommand<URI>(
				...
				try {
					...
					if (selectedKernelId) {
						...
						const kernel = all.find(
							(kernel) =>
								kernel.id === `ms-toolsai.jupyter/${selectedKernelId}`,
						);
						if (kernel) {
	...
	private async _calculdateKernelSources(editor: IActiveNotebookEditor) {
		...
		for (const group of groupBy(others, (a, b) =>
			...
			const extension = this._extensionService.extensions.find(
				(extension) => extension.identifier.value === group[0].extension.value,
			);
			const source =
	...
	private async _selectOneKernel(
		...
		const quickPickItems: QuickPickInput<KernelPick>[] = kernels.map((kernel) =>
			toKernelQuickPick(kernel, undefined),
		);
		const localDisposableStore = new DisposableStore();
		const quickPick = localDisposableStore.add(
			this._quickInputService.createQuickPick<KernelQuickPickItem>({
