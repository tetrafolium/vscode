...
function shouldSkip(
	...
	context: KernelQuickPickContext | undefined,
): boolean {
	return !!(
		selected &&
		((context &&
			"skipIfAlreadySelected" in context &&
			context.skipIfAlreadySelected) ||
...
registerAction2(
	class extends Action2 {
		...
		async run(
			...
			let extensionId =
				context && "extension" in context ? context.extension : undefined;

			if (
				controllerId &&
				(typeof controllerId !== "string" || typeof extensionId !== "string")
			) {
...
export class NotebooKernelActionViewItem extends ActionViewItem {
	...
	constructor(
		...
		this._register(
			_notebookKernelService.onDidChangeNotebookAffinity(this._update, this),
		);
		this._register(
			_notebookKernelService.onDidChangeSelectedNotebooks(this._update, this),
		);
		this._register(
