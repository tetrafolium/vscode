...
async function runCell(
	...
	if (
		context.autoReveal &&
		(context.cell || context.selectedCells?.length) &&
		editorService
	) {
...
function handleAutoReveal(
	...
	if (cell.cellKind === CellKind.Markup) {
		...
		notebookEditor.revealCellRangeInView({
			start: cellIndex,
			end: cellIndex + 1,
		});
		return;
	...
	const cellEditorScrollBottom =
		cellEditorScrollTop + cell.layoutInfo.outputContainerOffset;

	const cellOutputHeight = cell.layoutInfo.outputTotalHeight;
	const cellOutputScrollBottom =
		notebookEditor.getAbsoluteBottomOfElement(cell);
	...
	const isFullyVisible =
		cellEditorScrollTop >= notebookEditor.scrollTop &&
		cellOutputScrollBottom <= notebookEditor.scrollBottom;
	const isEditorBottomVisible =
		cellEditorScrollBottom - 25 /* padding for the cell status bar */ >=
			notebookEditor.scrollTop &&
		cellEditorScrollBottom +
			25 /* padding to see a sliver of the beginning of outputs */ <=
			notebookEditor.scrollBottom;

	// Common scrolling functions
	const revealWithTopPadding = (position: number) => {
		notebookEditor.setScrollTop(position - SMART_VIEWPORT_TOP_REVEAL_PADDING);
	};
	const revealWithNoPadding = (position: number) => {
		notebookEditor.setScrollTop(position);
	};
	const revealWithBottomPadding = (position: number) => {
		notebookEditor.setScrollTop(
			position + SMART_VIEWPORT_BOTTOM_REVEAL_PADDING,
		);
	};
	...
	class RevealRunningCellAction extends NotebookAction {
		...
		async runWithContext(
			...
			const executingCells =
				notebookExecutionStateService.getCellExecutionsForNotebook(notebook);
			if (executingCells[0]) {
	...
	class RevealLastFailedCellAction extends NotebookAction {
		...
		async runWithContext(
			...
			const lastFailedCellHandle =
				notebookExecutionStateService.getLastFailedCellForNotebook(notebook);
			if (lastFailedCellHandle !== undefined) {
				const lastFailedCell =
					context.notebookEditor.getCellByHandle(lastFailedCellHandle);
				if (lastFailedCell) {
