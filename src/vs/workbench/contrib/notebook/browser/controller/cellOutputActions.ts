...
registerAction2(
	class CopyCellOutputAction extends Action2 {
		...
		async run(
			...
			const notebookEditor = getNotebookEditorFromContext(
				editorService,
				outputContext,
			);
			if (!notebookEditor) {
...
export function getOutputViewModelFromId(
	outputId: string,
...
function getNotebookEditorFromContext(
...
function getOutputViewModelFromContext(
	outputContext:
		| INotebookOutputActionContext
		| { outputViewModel: ICellOutputViewModel }
		| undefined,
...
registerAction2(
	class SaveCellOutputImageAction extends Action2 {
		constructor() {
			super({
		...
		async run(
			...
			const outputItem = outputViewModel.model.outputs.find(
			...
			const defaultUri = notebookEditor.textModel?.uri
				? URI.joinPath(
						URI.file(notebookEditor.textModel.uri.fsPath),
						"..",
						defaultFileName,
					)
				: undefined;
...
export const OPEN_OUTPUT_IN_OUTPUT_PREVIEW_COMMAND_ID =
	"notebook.cellOutput.openInOutputPreview";

registerAction2(
	class OpenCellOutputInNotebookOutputEditorAction extends Action2 {
		constructor() {
			super({
				...
				menu: {
					...
					when: ContextKeyExpr.and(
						...
						ContextKeyExpr.equals(
							"config.notebook.output.openInPreviewEditor.enabled",
							true,
						),
					),
				},
		...
		async run(
			...
			// get cell index
			const cellViewModel = notebookEditor.getCellByHandle(
				genericCellViewModel.handle,
			);
			if (!cellViewModel) {
				return;
			}
			...
			// get output index
			const outputIndex =
				genericCellViewModel.outputsViewModels.indexOf(outputViewModel);
			if (outputIndex === -1) {
				return;
			}
