...
registerAction2(
	class QuitEditAllCellsAction extends NotebookAction {
		constructor() {
			super({
		...
		async runWithContext(
			...
			editingCells.forEach((cell) => {
				cell.updateEditState(
					CellEditState.Preview,
					QUIT_EDIT_ALL_CELLS_COMMAND_ID,
				);
			});
...
async function setCellToLanguage(
	...
	if (languageId === "markdown" && context.cell?.language !== "markdown") {
		...
		await changeCellToKind(
			CellKind.Markup,
			{ cell: context.cell, notebookEditor: context.notebookEditor, ui: true },
			"markdown",
			Mimes.markdown,
		);
	...
	class CommentSelectedCellsAction extends NotebookMultiCellAction {
		...
		async runWithContext(
			...
			context.selectedCells.forEach(async (cellViewModel) => {
				...
				const initialTrackedRangesIDs: string[] = cellEditorSelections.map(
					(selection) => {
				...
				CommandExecutor.executeCommands(textModel, cellEditorSelections, [
					cellCommentCommand,
				]);
