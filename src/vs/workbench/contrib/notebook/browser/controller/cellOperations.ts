...
export function runDeleteAction(
	...
	const containingSelection = selections.find(
		(selection) =>
			selection.start <= targetCellIndex && targetCellIndex < selection.end,
	);
			...
			() => {
				if (nextCellAfterContainingSelection) {
					const cellIndex = textModel.cells.findIndex(
						(cell) => cell.handle === nextCellAfterContainingSelection.handle,
					);
					return {
						kind: SelectionStateType.Index,
	...
	if (type === CellKind.Code) {
		...
		if (cell?.cellKind === CellKind.Code) {
			...
			if (cell === undefined && direction === "above") {
				...
				language =
					viewModel.viewCells.find((cell) => cell.cellKind === CellKind.Code)
						?.language || defaultLanguage;
			} else {
				language = defaultLanguage;
			}
