	...
	constructor(
		...
		this._register(
			chatContextPickService.registerChatContextItem(
				new KernelVariableContextPicker(
					this.editorService,
		...
		// output context
		NOTEBOOK_CELL_OUTPUT_MIME_TYPE_LIST_FOR_CHAT.bindTo(contextKeyService).set(
			NOTEBOOK_CELL_OUTPUT_MIME_TYPE_LIST_FOR_CHAT_CONST,
		);
	}
...
export class SelectAndInsertKernelVariableAction extends Action2 {
	...
	override async run(
		...
		const context = args[0] as
			| { widget: IChatWidget; range?: Range; variable?: string }
			| undefined;
...
class KernelVariableContextPicker implements IChatContextPickerItem {
	...
	isEnabled(widget: IChatWidget): Promise<boolean> | boolean {
		return (
			widget.location === ChatAgentLocation.Notebook &&
			Boolean(
				getNotebookEditorFromEditorPane(
					this.editorService.activeEditorPane,
				)?.getViewModel()?.notebookDocument,
...
registerAction2(
	class AddCellOutputToChatAction extends Action2 {
		constructor() {
			super({
