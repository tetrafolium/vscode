	...
	implements INotebookEditorPane, IEditorPaneWithScrolling
{
	...
	protected override setEditorVisible(visible: boolean): void {
		...
		this._groupListener.add(
			this.group.onWillCloseEditor((e) => this._saveEditorViewState(e.editor)),
		);
		this._groupListener.add(
	...
	override async setInput(
		...
		try {
			...
			this._widgetDisposableStore.add(
				this._widget.value.onDidBlurWidget(() => this._onDidBlurWidget.fire()),
			);
					...
					toAction({
						...
						run: async () => {
							...
							const activeEditorResource =
								EditorResourceAccessor.getCanonicalUri(activeEditorPane.input);
							if (!activeEditorResource) {
								return;
							}

							if (
								activeEditorResource.toString() === input.resource?.toString()
							) {
...
export function isNotebookContainingCellEditor(
