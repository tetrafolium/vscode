	...
	implements INotebookEditorDelegate, INotebookEditor
{
	...
	constructor(
		...
		this._register(
			this.instantiationService.createInstance(NotebookEditorContextKeys, this),
		);
		...
		this._scrollBeyondLastLine = this.configurationService.getValue<boolean>(
	...
	private _createCellList(): void {
		...
		this._cellLayoutManager = new NotebookCellLayoutManager(
			this,
			this._list,
			this.logService,
		);
	...
	async setOptions(options: INotebookEditorOptions | undefined) {
		...
		if (cellOptions) {
			const cell = this.viewModel.viewCells.find(
				(cell) => cell.uri.toString() === cellOptions.resource.toString(),
			);
			if (cell) {
	...
	private async _attachModel(
		...
		const renderScrollHeightDisposable = this._localStore.add(
	...
	private layoutNotebook(
		...
		this._dimension = dimension;
		const newBodyHeight =
			this.getBodyHeight(dimension.height) - this.getLayoutInfo().stickyHeight;
	...
	private layoutContainerOverShadowElement(
		...
		const modalEditorContainer =
			this.editorGroupsService.activeModalEditorPart?.modalElement;
		const isModal =
			DOM.isHTMLElement(modalEditorContainer) &&
			modalEditorContainer.contains(anchorElement);
		const clippingContainer = isModal
			? undefined
	...
	deltaCellDecorations(
		...
		const ret =
			this.viewModel?.deltaCellDecorations(oldDecorations, newDecorations) ||
			[];
	...
	private async _warmupSelection(
