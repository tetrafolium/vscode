...
export class CellExecutionPart extends CellContentPart {
	...
	constructor(
		...
		this._executionOrderContent = DOM.append(
			this._executionOrderLabel,
			DOM.$("div"),
		);
		...
		this._register(
			this._notebookExecutionStateService.onDidChangeExecution((e) => {
				if (
					this.currentCell &&
					hasKey(e, { affectsCell: true }) &&
					e.affectsCell(this.currentCell.uri)
				) {
	...
	private _updatePosition() {
		...
		if (this.currentCell instanceof CodeCellViewModel) {
			...
			const editorBottom =
				elementTop + this.currentCell.layoutInfo.outputContainerOffset;
			...
			if (scrollBottom <= editorBottom && cellIsRunning) {
				const isAlreadyIcon =
					this._executionOrderContent.classList.contains("sticky-spinner");
				...
				if (!isAlreadyIcon) {
					...
					DOM.append(
						this._executionOrderContent,
						...renderLabelWithIcons(`$(${icon.id})`),
					);
				}
				...
				const wasStickyHere =
					this._executionOrderLabel.classList.contains("sticky");
				this._executionOrderLabel.classList.remove("sticky");
				...
				const iconIsPresent =
					this._executionOrderContent.querySelector(".codicon") !== null;
				if (wasStickyHere || iconIsPresent) {
					const executionOrder =
						this.currentCell.internalMetadata.executionOrder;
					const executionOrderLabel =
						typeof executionOrder === "number" ? `[${executionOrder}]` : "[ ]";
					this._executionOrderContent.innerText = executionOrderLabel;
				}
