...
export class CodeCell extends Disposable {
	...
	private registerCellEditorEventListeners() {
		...
		if (this._useNewApproachForEditorLayout) {
			this._register(
				this.templateData.editor.onDidScrollChange((e) => {
					...
					if (
						this._cellLayout.editorVisibility === "Invisible" ||
						!this.templateData.editor.hasTextFocus()
					) {
						return;
					}
					if (
						this._cellLayout._lastChangedEditorScrolltop === e.scrollTop ||
						this._cellLayout.isUpdatingLayout
					) {
						return;
					}
					const scrollTop = this.notebookEditor.scrollTop;
					const diff =
						e.scrollTop - (this._cellLayout._lastChangedEditorScrolltop ?? 0);
					if (
						this._cellLayout.editorVisibility === "Full (Small Viewport)" &&
						typeof this._cellLayout._lastChangedEditorScrolltop === "number"
					) {
						this._debug(
						...
						this._cellLayout.editorVisibility === "Bottom Clipped" &&
						typeof this._cellLayout._lastChangedEditorScrolltop === "number"
					) {
						this._debug(
						...
						this._cellLayout.editorVisibility === "Top Clipped" &&
						typeof this._cellLayout._lastChangedEditorScrolltop === "number"
					) {
		...
		this._register(
			this.templateData.editor.onDidChangeCursorSelection((e) => {
				if (
					...
					e.source === "restoreState" ||
					e.oldModelVersionId === 0 ||
				...
				if (
					(this._pointerDownInEditor || this._pointerDraggingInEditor) &&
					this._useNewApproachForEditorLayout
				) {
					return;
				}
		...
		this._register(
			model.onDidChangeTokens(() => {
				if (this.viewCell.isInputCollapsed && this._inputCollapseElement) {
					...
					this._inputCollapseElement.innerHTML =
						(collapsedCellTTPolicy?.createHTML(content) ?? content) as string;
	...
	private registerMouseListener() {
		...
		if (this._useNewApproachForEditorLayout) {
			this._register(
				this.templateData.editor.onMouseMove((e) => {
		...
		if (this._useNewApproachForEditorLayout) {
			...
			this._register(
				DOM.addDisposableListener(win, "mouseup", resetPointerState),
			);
			this._register(
				DOM.addDisposableListener(win, "pointerup", resetPointerState),
			);
			this._register(
				DOM.addDisposableListener(win, "pointercancel", resetPointerState),
			);
	...
	private updateForCollapseState(): boolean {
		if (
			this.viewCell.isOutputCollapsed === this._renderedOutputCollapseState &&
	...
	private _collapseInput() {
		...
		element.innerHTML = (collapsedCellTTPolicy?.createHTML(richEditorText) ??
			richEditorText) as string;
	...
	private onCellEditorHeightChange(
		dbgReasonForChange: CellLayoutChangeReason,
	): void {
		const height = this.templateData.editor.getContentHeight();
		if (!this.templateData.editor.hasModel()) {
			this._debug(
...
type CellLayoutChangeReason =
	| "nbLayoutChange"
	| "nbDidScroll"
	| "viewCellLayoutChange"
	| "init"
...
export class CodeCellLayout {
	...
	public layoutEditor(reason: CellLayoutChangeReason): void {
		...
		const editorWidth =
			this._initialized &&
			(reason === "nbLayoutChange" || reason === "viewCellLayoutChange")
		...
		const elementBottom = this.notebookEditor.getAbsoluteBottomOfElement(
			this.viewCell,
		);
		...
		if (isInit) {
			...
			const fallbackEditorContentHeight =
				gotContentHeight === -1
					? Math.max(
							editor.getLayoutInfo().height,
							this._initialEditorDimension.height,
						)
					: gotContentHeight;
			const shouldRefreshContentHeight =
				!this._initialized ||
				reason === "onDidContentSizeChange" ||
				reason === "viewCellLayoutChange" ||
				reason === "nbLayoutChange";
			if (shouldRefreshContentHeight) {
				...
				editorContentHeight =
					this._establishedContentHeight ?? fallbackEditorContentHeight;
			}
		}
		const editorBottom =
			elementTop + this.viewCell.layoutInfo.outputContainerOffset;
		const scrollBottom = this.notebookEditor.scrollBottom;
		// When loading, scrollBottom -scrollTop === 0;
		const viewportHeight =
			scrollBottom - scrollTop === 0
				? this.notebookEditor.getLayoutInfo().height
				: scrollBottom - scrollTop;
		const outputContainerOffset =
			this.viewCell.layoutInfo.outputContainerOffset;
		const scrollDirection: "down" | "up" =
			typeof this._previousScrollBottom === "number"
				? scrollBottom < this._previousScrollBottom
					? "up"
					: "down"
				: "down";
		...
		let top = Math.max(
			0,
			scrollTop - elementTop - CELL_TOP_MARGIN - CELL_OUTLINE_WIDTH,
		);
		...
		if (scrollTop <= elementTop + CELL_TOP_MARGIN) {
			...
			if (scrollBottom >= editorBottom) {
				...
				height =
					...
					2 * CELL_OUTLINE_WIDTH; // We don't want bottom border to be visible.;
				this._editorVisibility = "Bottom Clipped";
				editorScrollTop = 0;
			}
		} else {
			if (
				viewportHeight <= editorContentHeight &&
		...
		this._logService.debug(
			`${reason} (${this._editorVisibility}, ${this._initialized})`,
		);
		this._logService.debug(
		...
		try {
			...
			editor.layout(
				{
					width: this._initialized
						? editorWidth
						: this._initialEditorDimension.width,
					height,
				},
				true,
			);
