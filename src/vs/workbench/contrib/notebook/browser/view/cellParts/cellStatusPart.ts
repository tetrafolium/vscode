				...
				(e) => {
					if (
						...
						if (target && DOM.isHTMLElement(target)) {
							...
							if (
								targetElement.classList.contains("cell-status-item-has-command")
							) {
								itemHasCommand = true;
							} else if (
...
class CellStatusBarItem extends Disposable {
	...
	updateItem(item: INotebookCellStatusBarItem, maxWidth: number | undefined) {
		...
		if (!this._currentItem || this._currentItem.text !== item.text) {
			this._itemDisposables.add(new SimpleIconLabel(this.container)).text =
				item.text.replace(/\n/g, " ");
		}
		...
		if (item.command) {
			...
			this._itemDisposables.add(
				DOM.addDisposableListener(this.container, DOM.EventType.CLICK, (_e) => {
					this.executeCommand();
				}),
			);
			this._itemDisposables.add(
