	...
	implements INotebookEditorContribution
{
	...
	private hasNotebookAgent(): boolean {
		...
		return !!agents.find((agent) =>
			agent.locations.includes(ChatAgentLocation.Notebook),
		);
	}
	...
	private handleChangeExecutionState(
		changes: (ICellExecutionStateChangedEvent | IExecutionStateChangedEvent)[],
	) {
		if (!this.enabled) {
			return;
		}
	...
	private setDiagnostics(cellHandle: number) {
		...
		if (
			...
			disposables.push(
				toDisposable(() =>
					cell.executionErrorDiagnostic.set(undefined, undefined),
				),
			);
			disposables.push(
				autorun((r) => {
