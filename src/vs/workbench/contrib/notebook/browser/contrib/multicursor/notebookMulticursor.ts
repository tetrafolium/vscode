	...
	implements INotebookEditorContribution
{
		...
		this.anchorDisposables.add(
			this.anchorCell[1].onDidChangeCursorSelection((e) => {
				...
				const translation: SelectionTranslation = {
					deltaStartCol:
						e.selection.startColumn - e.oldSelections[0].startColumn,
					deltaStartLine:
						e.selection.startLineNumber - e.oldSelections[0].startLineNumber,
					deltaEndCol: e.selection.endColumn - e.oldSelections[0].endColumn,
					deltaEndLine:
						e.selection.endLineNumber - e.oldSelections[0].endLineNumber,
				};
				...
				this.trackedCells.forEach((cell) => {
					...
					const newSelections = controller.getSelections().map((selection) => {
						const newStartCol =
							selection.startColumn + translation.deltaStartCol;
						const newStartLine =
							selection.startLineNumber + translation.deltaStartLine;
						const newEndCol = selection.endColumn + translation.deltaEndCol;
						const newEndLine =
							selection.endLineNumber + translation.deltaEndLine;
	...
	private executeEditorOperation(
		...
		e: ITriggerEditorOperationEvent,
	) {
		switch (e.handlerId) {
	...
	public async findAndTrackNextSelection(
		...
		if (this.state === NotebookMultiCursorState.Idle) {
			...
			if (notebookTextModel) {
				const allMatches = notebookTextModel.findMatches(
					this.word,
					false,
					true,
					USUAL_WORD_SEPARATORS,
				);
				this.totalMatchesCount = allMatches.reduce(
					(sum, cellMatch) => sum + cellMatch.matches.length,
					0,
				);
			}
			...
			const totalSelections = this.trackedCells.reduce(
				(sum, trackedCell) => sum + trackedCell.matchSelections.length,
				0,
			);
			...
			const findResult = notebookTextModel.findNextMatch(
				this.word,
				{
					...
					position: focusedCell
						.getSelections()
						[focusedCell.getSelections().length - 1].getEndPosition(),
				},
				false,
				true,
				USUAL_WORD_SEPARATORS,
			...
			if (findResult.cell.handle === focusedCell.handle) {
				...
				this.initializeMultiSelectDecorations(
					this.trackedCells.find(
						(trackedCell) =>
							trackedCell.cellViewModel.handle === focusedCell.handle,
					),
				);
			}
		}
...
class NotebookAddMatchToMultiSelectionAction extends NotebookAction {
	...
	override async runWithContext(
		...
		const controller = editor.getContribution<NotebookMultiCursorController>(
			NotebookMultiCursorController.id,
		);
...
class NotebookDeleteRightMultiSelectionAction extends NotebookAction {
	...
	override async runWithContext(
		...
		const controller = nbEditor.getContribution<NotebookMultiCursorController>(
			NotebookMultiCursorController.id,
		);
