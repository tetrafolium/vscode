...
export class NotebookInlineDiffDecorationContribution
	extends Disposable
	implements INotebookEditorContribution
{
	...
	private readonly cellDecorators = new Map<
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				this.previous =
					this.notebookEditor.notebookOptions.previousModelToCompare.read(
						reader,
					);
				if (this.previous) {
					...
					if (this.notebookEditor.hasModel()) {
						...
						this.listeners.push(
							Event.once(this.notebookEditor.onDidAttachViewModel)(() =>
								this.initialize(),
							),
						);
					}
		...
		cellDiffs.forEach((diff) => {
			if (diff.type === "modified") {
				...
				if (editor) {
					...
					if (
						currentDecorator?.modifiedCell !== modifiedCell ||
