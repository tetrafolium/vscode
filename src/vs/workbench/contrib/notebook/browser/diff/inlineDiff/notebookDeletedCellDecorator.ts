...
export class NotebookDeletedCellDecorator
	extends Disposable
	implements INotebookDeletedCellDecorator
{
	private readonly zoneRemover = this._register(new DisposableStore());
	private readonly createdViewZones = new Map<number, string>();
	private readonly deletedCellInfos = new Map<
		number,
		{ height: number; previousIndex: number; offset: number }
	>();
	constructor(
	...
	private async _createWidgetImpl(
		...
		Array.from(this.deletedCellInfos.keys())
			...
			.forEach((originalIndex) => {
				...
				if (previousDeletedCell) {
					...
					if (deletedCell) {
						deletedCell.offset =
							previousDeletedCell.height + previousDeletedCell.offset;
					}
				}
			});
		...
		this._notebookEditor.changeViewZones((accessor) => {
			...
			const deletedCellOverviewRulereDecorationIds =
...
export class NotebookDeletedCellWidget extends Disposable {
	...
	public async render() {
		...
		const fontInfo =
			this._notebookEditor.getBaseCellEditorOptions(languageId).value;
		const fontFamilyVar = "--notebook-editor-font-family";
		const fontSizeVar = "--notebook-editor-font-size";
		const fontWeightVar = "--notebook-editor-font-weight";
		// If we have any editors, then use left layout of one of those.
		const editor = this._notebookEditor.codeEditors
			.map((c) => c[1])
			.find((c) => c);
		...
		const height =
			lineCount * (fontInfo.lineHeight || DefaultLineHeight) + 12 + 12; // We have 12px top and bottom in generated code HTML;
