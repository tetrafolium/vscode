...
registerAction2(
	class extends Action2 {
		...
		run(accessor: ServicesAccessor, context?: DiffElementCellViewModelBase) {
			...
			const modifiedCellIndex = context.mainDocumentTextModel.cells.indexOf(
				modified.textModel,
			);
			if (modifiedCellIndex === -1) {
				return;
...
Registry.as<IConfigurationRegistry>(
	...
	properties: {
		...
		"notebook.diff.experimental.toggleInline": {
			type: "boolean",
			default:
				typeof product.quality === "string" && product.quality !== "stable", // only enable as default in insiders
			markdownDescription: localize(
