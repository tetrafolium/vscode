					...
					this.disposables.add(
						placeholderItem.onUnfoldHiddenCells(() => {
							const hiddenCellViewModels =
								this.placeholderAndRelatedCells.get(placeholderItem);
...
export function prettyChanges(
	original: NotebookTextModel,
	modified: NotebookTextModel,
	...
	for (let i = 0; i < diffResult.changes.length - 1; i++) {
		...
		if (
			...
			original.cells[x].getHashValue() ===
				modified.cells[y + 1].getHashValue() &&
...
function isEqual(
	...
	for (let i = 0; i < viewModels.length; i++) {
		...
		switch (a.type) {
			case "delete": {
				if (
					originalModel.cells[a.originalCellIndex].handle !== b.original?.handle
				) {
					return false;
				}
				continue;
			...
			case "insert": {
				if (
					modifiedModel.cells[a.modifiedCellIndex].handle !== b.modified?.handle
				) {
					return false;
				}
				continue;
