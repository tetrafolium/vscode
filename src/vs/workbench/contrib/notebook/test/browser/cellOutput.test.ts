...
suite("CellOutput", () => {
		...
		instantiationService.stub(
			...
			new (class extends mock<INotebookService>() {
				override getOutputMimeTypeInfo(
					_textModel: any,
					_kernelProvides: readonly string[] | undefined,
					output: IOutputDto,
				) {
					return [
						{
						...
						output.outputs.some((output) => output.mime === info.mimeType),
					);
				}
	...
	test("Render cell output items with multiple mime types", async function () {
		...
		const output1: IOutputDto = {
			outputId: "abc",
			outputs: [outputItem, htmlOutputItem],
		};
		const output2: IOutputDto = {
			outputId: "def",
			outputs: [outputItem, htmlOutputItem],
		};
