...
suite("NotebookSelection", () => {
	...
	test("selection is never empty", function () {
		const selectionCollection = new NotebookCellSelectionCollection();
		assert.deepStrictEqual(selectionCollection.selections, [
			{ start: 0, end: 0 },
		]);
	...
	test("selections does not change when setting to empty", function () {
		...
		let changed = false;
		store.add(
			selectionCollection.onDidChangeSelection(() => {
				changed = true;
			}),
		);
	...
	test("notebook validate range", async () => {
		await withTestNotebook(
			...
			(editor, viewModel) => {
				...
				assert.deepStrictEqual(viewModel.validateRange({ start: 0, end: 0 }), {
					start: 0,
					end: 0,
				});
				assert.deepStrictEqual(viewModel.validateRange({ start: 0, end: 2 }), {
					start: 0,
					end: 2,
				});
				assert.deepStrictEqual(viewModel.validateRange({ start: 0, end: 3 }), {
					start: 0,
					end: 2,
				});
