...
} from "../../../browser/diff/diffElementViewModel.js";
...
} from "../../../browser/diff/notebookDiffEditorBrowser.js";
import {
...
suite("NotebookDiff", () => {
	...
	setup(() => {
		...
		notebookEditorWorkerService =
			new (class extends mock<INotebookEditorWorkerService>() {
		...
		heightCalculator =
			new (class extends mock<IDiffEditorHeightCalculatorService>() {
				override diffAndComputeHeight() {
	...
	async function verifyChangeEventIsNotFired(
		diffViewModel: INotebookDiffViewModel,
	) {
					...
					[
						{
							...
							outputs: [
								{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([3])) },
							],
						},
					],
	...
	test("diff different output", async () => {
		await withTestNotebookDiffModel(
					...
					[
						{
							outputId: "someId",
							outputs: [
								{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([5])) },
							],
						},
					],
			...
			async (model, disposables, accessor) => {
				...
				(
					diffViewModel.items[1] as unknown as SideBySideDiffElementViewModel
				).hideUnchangedCells();

				assert.strictEqual(diffViewModel.items.length, 2);
				assert.strictEqual(diffViewModel.items[0].type, "modified");
				assert.strictEqual(
					(diffViewModel.items[1] as IDiffElementViewModelBase).type,
					"placeholder",
				);
				assert.deepStrictEqual(eventArgs, {
					start: 1,
					...
					[
						{
							outputId: "someId",
							outputs: [
								{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([6])) },
							],
						},
					],
					{ metadata: { collapsed: false }, executionOrder: 5 },
					...
					[
						{
							outputId: "someId",
							outputs: [
								{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([2])) },
							],
						},
					],
	...
	test("diff insert 2", async () => {
		await withTestNotebookDiffModel(
			...
			async (model, disposables, accessor) => {
				const eventDispatcher = disposables.add(
	...
	test("diff insert 3", async () => {
		await withTestNotebookDiffModel(
			...
			async (model, disposables, accessor) => {
				...
				assert.strictEqual(
					(diffViewModel.items[0] as IDiffElementViewModelBase).type,
					"unchanged",
				);
				assert.strictEqual(diffViewModel.items[1].type, "unchanged");
				assert.strictEqual(
					(diffViewModel.items[2] as IDiffElementViewModelBase).type,
					"unchanged",
				);
				assert.strictEqual(diffViewModel.items[3].type, "unchanged");
				assert.strictEqual(diffViewModel.items[4].type, "insert");
				...
				(
					diffViewModel.items[0] as SideBySideDiffElementViewModel
				).hideUnchangedCells();
				assert.strictEqual(
					(diffViewModel.items[0] as IDiffElementViewModelBase).type,
					"placeholder",
	...
	test("diff output", async () => {
		await withTestNotebookDiffModel(
					...
					[
						{
							outputId: "someOtherId",
							outputs: [
								{ mime: Mimes.text, data: VSBuffer.wrap(new Uint8Array([4])) },
							],
						},
					],
					{ metadata: { collapsed: false }, executionOrder: 3 },
			...
			async (model, disposables, accessor) => {
				...
				assert.strictEqual(
					(
						diffViewModel.items[0] as unknown as SideBySideDiffElementViewModel
			...
			async (model, disposables, accessor) => {
				...
				assert.strictEqual(
					(
						...
						(
							diffViewModel
								.items[0] as unknown as SideBySideDiffElementViewModel
						).modified!.textModel,
					),
					true,
				);
				assert.strictEqual(
					(
						...
						(
							diffViewModel
								.items[1] as unknown as SideBySideDiffElementViewModel
						).modified!.textModel,
					),
					false,
				);
