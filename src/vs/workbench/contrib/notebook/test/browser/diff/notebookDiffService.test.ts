...
suite("NotebookDiff Diff Service", () => {
	...
	test("No changes", async () => {
		...
		assert.deepStrictEqual(mapping, [{ modified: 0, original: 0 }]);
	...
	test("No cells and add 1 cell", async () => {
		...
		assert.deepStrictEqual(mapping, [{ modified: 0, original: -1 }]);
		assert.deepStrictEqual(diff.cellsDiff.changes, [
			{
				originalStart: 0,
				originalLength: 0,
	...
	test("Detect modification and insertion of cells", async () => {
		const { mapping, diff } = await mapCells(
			[
				...
				{
					cell_type: "code",
					source: ["device = utils.device_widget()\n", "\n", "device"],
				},
				{
					cell_type: "markdown",
					source: [
				...
				{
					cell_type: "code",
					source: ["input_layer.any_name, [o.any_name for o in output_layers]"],
				},
				{
					cell_type: "markdown",
					source: [
	...
	test("Modification with Insertion, detected as deletion of a cell", async () => {
		const { mapping, diff } = await mapCells(
			[
				...
				{
					cell_type: "code",
					source: ['prompt = "A panda eating bamboo on a rock."'],
				},
				{
					cell_type: "markdown",
					source: [
		...
		const { mapping, diff } = await mapCells(
			[
				...
				},
				{
					...
					source: ['device = device_widget("CPU")\n', "\n", "device"],
				},
				{
					cell_type: "code",
				...
				},
				{
					...
					source: ["res = ov_yolo_model(IMAGE_PATH, iou=0.45, conf=0.2)"],
				},
				{
					cell_type: "code",
					id: "a4abf758-3a72-4ee0-afde-38076602f1c7",
					source: ["Image.fromarray(res[0].plot()[:, :, ::-1])"],
				},
				{
					cell_type: "markdown",
		...
		const { mapping, diff } = await mapCells(
			[
				...
				},
				{
					...
					source: ["## Install FastSAM, SAM, and other dependencies"],
				},
				{
					cell_type: "code",
					execution_count: null,
				...
				},
				{
					...
					source: ["## Download FastSAM and SAM weights"],
				},
				{
					cell_type: "code",
					execution_count: null,
				...
				},
				{
					...
					outputs: [],
					source: ['IMAGE_PATH = f"{HOME}/data/dog.jpeg"'],
				},
				{
					cell_type: "code",
					execution_count: null,
				...
				},
				{
					...
					source: ["## FastSAM box prompt inference"],
				},
				{
					cell_type: "markdown",
					metadata: {
				...
				},
				{
					...
					source: ["### Generate mask with FastSAM"],
				},
				{
					cell_type: "code",
					execution_count: null,
				...
				},
				{
					...
					source: ["## FastSAM point prompt inference"],
				},
				{
					cell_type: "markdown",
					metadata: {
				...
				},
				{
					...
					source: ["## FastSAM text prompt inference"],
				},
				{
					cell_type: "code",
					execution_count: null,
				...
				},
				{
					...
					outputs: [],
					source: ['IMAGE_PATH = f"{HOME}/data/robot.jpeg"'],
				},
				{
					cell_type: "markdown",
					metadata: {
				...
				},
				{
					...
					source: ["## Roboflow Benchmark Dataset"],
				},
				{
					cell_type: "code",
					execution_count: null,
		...
		const { mapping, diff } = await mapCells(
			...
			[
				...
				},
				{
					...
					metadata: {},
					outputs: [],
					source: ['plot_repo_metrics(f"{OWNER}/vscode")'],
				},
			].map(fromJupyterCell),
		);
	...
	let uriCounter = 0;
	async function mapCells(
		original: MockNotebookCell[],
		modified: MockNotebookCell[],
	) {
		const mapping = matchCellBasedOnSimilarties(
			modified.map(cellToDto),
			original.map(cellToDto),
		).map((mapping) => ({
		...
		worker.$acceptNewModel(
			...
			original.map((cell, index) =>
				cellToMainCellTdo(originalUri, cell, index),
			),
		);
		worker.$acceptNewModel(
			...
			modified.map((cell, index) =>
				cellToMainCellTdo(modifiedUri, cell, index),
			),
		);
		const diff = await worker.$computeDiff(
			originalUri.toString(),
			modifiedUri.toString(),
		);
	...
	function cellToDto(cell: MockNotebookCell): {
		getValue(): string;
	...
	function cellToMainCellTdo(
		notebookUri: URI,
		cell: MockNotebookCell,
		index: number,
	): IMainCellDto {
		return {
			source: cell[0],
