...
suite("Workbench Toolbar calculateActions (strategy always + never)", () => {
	...
	const defaultSecondaryActions: IAction[] = defaultSecondaryActionModels.map(
		(action) => action.action,
	);
	const separator: IActionModel = {
	...
	setup(function () {
		defaultSecondaryActionModels.forEach((action) =>
			disposables.add(<Action>action.action),
		);
	});
	...
	test("should move actions to secondary when they do not fit within the container width", () => {
		...
		assert.deepEqual(
			...
			[actions[1], actions[2], separator, ...defaultSecondaryActionModels].map(
				(action) => action.action,
			),
		);
	});
