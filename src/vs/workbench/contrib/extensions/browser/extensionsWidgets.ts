...
export class ExtensionIconWidget extends ExtensionWidget {
	private readonly iconLoadingDisposable = this._register(
		new MutableDisposable(),
	);
	private readonly iconErrorDisposable = this._register(
		new MutableDisposable(),
	);
	...
	constructor(container: HTMLElement) {
		...
		this.defaultIconElement = append(
			this.element,
			$(ThemeIcon.asCSSSelector(extensionDefaultIcon)),
		);
	...
	render(): void {
		...
		if (this.extension.iconUrl) {
			if (this.iconUrl !== this.extension.iconUrl) {
				...
				this.iconErrorDisposable.value = addDisposableListener(
					this.iconElement,
					"error",
					() => {
				...
				if (!this.iconElement.complete) {
					...
					this.iconLoadingDisposable.value = addDisposableListener(
						this.iconElement,
						"load",
						() => {
	...
	render(): void {
		...
		if (this.small) {
			...
			if (this.extension.ratingUrl) {
				this.disposables.add(
					onClick(element, () =>
						this.openerService.open(URI.parse(this.extension!.ratingUrl!)),
					),
				);
			}
		}
...
export class RemoteBadgeWidget extends ExtensionWidget {
	private readonly remoteBadge = this._register(
		new MutableDisposable<ExtensionIconBadge>(),
	);
	...
	private render(): void {
		...
		const applyBadgeStyle = () => {
			...
			const bgColor = this.themeService
				.getColorTheme()
				.getColor(EXTENSION_BADGE_BACKGROUND);
			const fgColor = this.themeService
				.getColorTheme()
				.getColor(EXTENSION_BADGE_FOREGROUND);
...
export class ExtensionKindIndicatorWidget extends ExtensionWidget {
	...
	render(): void {
		...
		if (this.extension?.private) {
			...
			if (
				!this.small ||
				(this.extensionGalleryManifest?.capabilities.extensions
					?.includePublicExtensions &&
					this.extensionGalleryManifest?.capabilities.extensions
...
export class ExtensionRestartRequiredWidget extends ExtensionWidget {
	...
	render(): void {
		...
		const reason =
			typeof runtimeState?.reason === "string" ? runtimeState.reason : "";
		...
		if (runtimeState && /restart|reload/i.test(reason)) {
			const element = append(
				this.container,
				$(
					"span.extension-restart-required" +
						ThemeIcon.asCSSSelector(restartRequiredIcon),
				),
			);
			append(
				this.container,
				$(
					"span.extension-restart-required-label",
					undefined,
					localize("restart required", "Restart Required"),
				),
			);
			this.disposables.add(
				this.hoverService.setupManagedHover(
...
export class ExtensionRuntimeStatusWidget extends ExtensionWidget {
		...
		this._register(
			extensionService.onDidChangeExtensionsStatus((extensions) => {
				if (
					this.extension &&
					extensions.some((e) =>
						areSameExtensions({ id: e.value }, this.extension!.identifier),
					)
				) {
					this.update();
				}
	...
	render(): void {
		...
		if (
			this.extensionViewState.filters.featureId &&
			this.extension.state === ExtensionState.Installed
		) {
		...
		const extensionStatus =
			this.extensionsWorkbenchService.getExtensionRuntimeStatus(this.extension);
...
export class ExtensionHoverWidget extends ExtensionWidget {
	...
	private getHoverMarkdown(): MarkdownString | undefined {
		...
		markdown
			.appendMarkdown(`**`)
			.appendText(this.extension.displayName)
			.appendMarkdown(`**`);
		...
		const extensionRuntimeStatus =
			this.extensionsWorkbenchService.getExtensionRuntimeStatus(this.extension);
		const extensionFeaturesAccessData =
			this.extensionFeaturesManagementService.getAllAccessDataForExtension(
				new ExtensionIdentifier(this.extension.identifier.id),
			);
		...
		if (
			...
			if (extensionRuntimeStatus) {
				...
				if (
					...
					const errorsLink = extensionRuntimeStatus.runtimeErrors.length
						? `[${extensionRuntimeStatus.runtimeErrors.length === 1 ? localize("uncaught error", "1 uncaught error") : localize("uncaught errors", "{0} uncaught errors", extensionRuntimeStatus.runtimeErrors.length)}](${createCommandUri("extension.open", this.extension.identifier.id, ExtensionEditorTab.Features)})`
						: undefined;
					const messageLink = extensionRuntimeStatus.messages.length
						? `[${extensionRuntimeStatus.messages.length === 1 ? localize("message", "1 message") : localize("messages", "{0} messages", extensionRuntimeStatus.messages.length)}](${createCommandUri("extension.open", this.extension.identifier.id, ExtensionEditorTab.Features)})`
						: undefined;
					markdown.appendMarkdown(
			...
			if (extensionFeaturesAccessData.size) {
				const registry = Registry.as<IExtensionFeaturesRegistry>(
					Extensions.ExtensionFeaturesRegistry,
				);
				for (const [featureId, accessData] of extensionFeaturesAccessData) {
					if (accessData?.accessTimes.length) {
						...
						if (feature) {
							markdown.appendMarkdown(
								localize("feature usage label", "{0} usage", feature.label),
							);
							markdown.appendMarkdown(
								`: [${localize("total", "{0} {1} requests in last 30 days", accessData.accessTimes.length, feature.accessDataLabel ?? feature.label)}](${createCommandUri("extension.open", this.extension.identifier.id, ExtensionEditorTab.Features)})`,
							);
							markdown.appendText(`\n`);
						}
					}
...
export const extensionPrivateBadgeBackground = registerColor(
