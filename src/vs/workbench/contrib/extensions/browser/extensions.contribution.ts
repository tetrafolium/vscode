	...
	properties: {
		"extensions.autoUpdate": {
			...
			enumDescriptions: [
				...
				localize(
					"extensions.autoUpdate.delayed",
					"Enabled extensions are updated automatically, but only 6 hours after a new version has been published.",
				),
				localize(
					"extensions.autoUpdate.off",
					"Extensions are not updated automatically.",
				),
			],
...
const CONTEXT_EXTENSIONS_AUTO_UPDATE_POLICY = new RawContextKey<boolean>(
	"extensionsAutoUpdatePolicy",
	false,
);
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this.autoUpdatePolicyContext =
			CONTEXT_EXTENSIONS_AUTO_UPDATE_POLICY.bindTo(contextKeyService);
	...
	private updateAutoUpdatePolicyContext(): void {
		this.autoUpdatePolicyContext.set(
			this.configurationService.inspect(AutoUpdateConfigurationKey)
	...
	private registerGlobalActions(): void {
		...
		this.registerExtensionAction({
			...
			run: async () => {
				...
				if (outdated.length) {
					...
					pluginResult.updatedNames.length === 0 &&
					pluginResult.failedNames.length === 0
				) {
		...
		const notPolicyControlledCondition =
			CONTEXT_EXTENSIONS_AUTO_UPDATE_POLICY.toNegated();
		const enableAutoUpdateWhenCondition = ContextKeyExpr.and(
			ContextKeyExpr.equals(`config.${AutoUpdateConfigurationKey}`, "off"),
			notPolicyControlledCondition,
		);
		this.registerExtensionAction({
		...
		const disableAutoUpdateWhenCondition = ContextKeyExpr.and(
			ContextKeyExpr.notEquals(`config.${AutoUpdateConfigurationKey}`, "off"),
			notPolicyControlledCondition,
		);
		this.registerExtensionAction({
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			toolsService.vscodeToolSet.addTool(SearchExtensionsToolData),
		);
	}
}
