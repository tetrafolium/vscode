...
class RuntimeStatusMarkdownRenderer
	extends Disposable
	implements IExtensionFeatureElementRenderer
{
	...
	private renderRequestsChart(
		...
		disposables.add(
			addDisposableListener(svg, EventType.MOUSE_MOVE, mouseMoveListener),
		);
		...
		disposables.add(
			addDisposableListener(svg, EventType.MOUSE_LEAVE, mouseLeaveListener),
		);
	}
}
...
class ExtensionFeatureItemRenderer implements IListRenderer<
	IExtensionFeatureDescriptor,
		...
		templateData.disposables.add(
			this.extensionFeaturesManagementService.onDidChangeEnablement(
				({ extension, featureId, enabled }) => {
					if (
						ExtensionIdentifier.equals(extension, this.extensionId) &&
...
class ExtensionFeatureView extends Disposable {
		...
		if (this.feature.renderer) {
			...
			if (renderer.type === "table") {
				...
				this.renderElementData(
					featureContentElement,
					<IExtensionFeatureElementRenderer>renderer,
				);
			}
		}
	}
								...
								...data
									.map((item) => {
										...
										if (isMarkdownString(rowData)) {
											...
											result.push(
												$("code", undefined, Color.Format.CSS.formatHex(item)),
											);
										}
										return result;
									})
	...
	private renderElementData(
		container: HTMLElement,
		renderer: IExtensionFeatureElementRenderer,
	): void {
