...
const SearchRestartRequiredExtensionsContext = new RawContextKey<boolean>(
	"searchRestartRequiredExtensions",
	false,
);
...
export const ExtensionsSearchValueContext = new RawContextKey<string>(
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this.container = Registry.as<IViewContainersRegistry>(
...
export class ExtensionsViewPaneContainer
	extends ViewPaneContainer<IExtensionsViewletState>
	implements IExtensionsViewPaneContainer
{
	...
	constructor(
		...
		this.extensionsSearchValueContextKey =
			ExtensionsSearchValueContext.bindTo(contextKeyService);
		...
		this.searchMcpServersContextKey =
			SearchMcpServersContext.bindTo(contextKeyService);
		this.searchAgentPluginsContextKey =
			SearchAgentPluginsContext.bindTo(contextKeyService);
		...
		this.searchRestartRequiredExtensionsContextKey =
			SearchRestartRequiredExtensionsContext.bindTo(contextKeyService);
	...
	override create(parent: HTMLElement): void {
			...
			this.instantiationService.createInstance(
				...
				searchContainer,
				{
					...
					provideResults: (query: string) =>
						Query.suggestions(query, this.extensionGalleryManifest),
				},
				placeholder,
		...
		this._register(
			this.searchBox.onShouldFocusResults(() => this.focusListView(), this),
		);
						...
						const vsixs = coalesce(
							(
								await this.instantiationService.invokeFunction((accessor) =>
									extractEditorsAndFilesDropData(accessor, e),
								)
							).map((editor) =>
								editor.resource && extname(editor.resource) === ".vsix"
									? editor.resource
									: undefined,
							),
						);
	...
	private renderNotificaiton(): void {
		...
		if (
			...
			append(messageContainer, $("span")).className = SeverityIcon.className(
				status.severity,
			);
			...
			if (status.action) {
				...
				this.notificationDisposables.value.add(
					addDisposableListener(actionButton, EventType.CLICK, () => {
						Promise.resolve(status.action!.run()).catch((error) =>
			...
			if (
				this.searchBox &&
				ExtensionsListView.isRestartRequiredQuery(this.searchBox.getValue())
			) {
				this.search("");
			}
		}
	...
	private doSearch(refresh?: boolean): Promise<void> {
		...
		this.contextKeyService.bufferChangeEvents(() => {
			...
			this.searchWorkspaceUnsupportedExtensionsContextKey.set(
				ExtensionsListView.isSearchWorkspaceUnsupportedExtensionsQuery(value),
			);
			this.searchDeprecatedExtensionsContextKey.set(
				ExtensionsListView.isSearchDeprecatedExtensionsQuery(value),
			);
			this.searchRestartRequiredExtensionsContextKey.set(
				ExtensionsListView.isRestartRequiredQuery(value),
			);
			...
			this.searchAgentPluginsContextKey.set(
				!!value && /@agentPlugins\s?.*/i.test(value),
			);
			this.searchMarketplaceExtensionsContextKey.set(
				!!value &&
					...
					!this.searchMcpServersContextKey.get() &&
					!this.searchAgentPluginsContextKey.get(),
			);
	...
	implements IWorkbenchContribution
{
	...
	private onServiceChange(): void {
		...
		if (
			...
			extensionsNotification.severity === Severity.Warning
		) {
		...
		if (!badge) {
			...
			const outdated = this.extensionsWorkbenchService.outdated.reduce(
				(r, e) =>
					...
					!this.extensionsWorkbenchService.isAutoUpdateDelayed(e)
						? 1
						: 0),
				0,
			);
			...
			for (const extension of this.extensionsWorkbenchService.installed) {
				if (extension.isMalicious && extension.local) {
					maliciousExtensions.push([
						extension.local,
						extension.maliciousInfoLink,
					]);
					shouldRestartExtensions =
						shouldRestartExtensions ||
						extension.runtimeState?.action ===
...
export class ExtensionMarketplaceStatusUpdater
	extends Disposable
	implements IWorkbenchContribution
{
	private readonly badgeHandle = this._register(new MutableDisposable());
	...
	constructor(
		...
		this.updateBadge();
		this._register(
			this.extensionGalleryManifestService.onDidChangeExtensionGalleryManifestStatus(
				() => this.updateBadge(),
			),
		);
	}
	...
	private async updateBadge(): Promise<void> {
		...
		switch (status) {
			case ExtensionGalleryManifestStatus.RequiresSignIn:
				badge = new NumberBadge(1, () =>
					localize("signInRequired", "Sign in required to access marketplace"),
				);
				break;
			case ExtensionGalleryManifestStatus.AccessDenied:
				badge = new WarningBadge(() =>
					localize("accessDenied", "Access denied to marketplace"),
				);
				break;
		}
		...
		if (status === ExtensionGalleryManifestStatus.RequiresSignIn) {
			...
			this.accountBadgeDisposable.value =
