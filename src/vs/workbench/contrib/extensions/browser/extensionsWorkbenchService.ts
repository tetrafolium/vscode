...
export class Extension implements IExtension {
	...
	constructor(
		...
		private readonly resourceExtensionInfo:
			| { resourceExtension: IResourceExtension; isWorkspaceScoped: boolean }
			| undefined,
		@IExtensionGalleryService
	...
	get publisherUrl(): URI | undefined {
		return this.gallery?.publisherLink
			? URI.parse(this.gallery.publisherLink)
			: undefined;
	}
	...
	private get localIconUrl(): string | undefined {
		if (this.local && this.local.manifest.icon) {
			return FileAccess.uriToBrowserUri(
				resources.joinPath(this.local.location, this.local.manifest.icon),
			).toString(true);
		}
		return undefined;
	}
	...
	public get isMalicious(): boolean | undefined {
		return (
			!!this.malicious ||
			this.enablementState === EnablementState.DisabledByMalicious
		);
	}
	...
	get outdated(): boolean {
		try {
			...
			if (
				...
				!this.productService.builtInExtensionsEnabledWithAutoUpdates?.some(
					(id) => id.toLowerCase() === this.identifier.id.toLowerCase(),
				)
			) {
				return false;
			}
			...
			if (!cache) {
				if (this.gallery.assets.manifest) {
					...
					this.logService.error(
						nls.localize("Manifest is not found", "Manifest is not found"),
						this.identifier.id,
					);
				}
			}
...
`);
	...
	setExtensionsControlManifest(
		...
		this.malicious = findMatchingMaliciousEntry(
			this.identifier,
...
class Extensions extends Disposable {
	...
	constructor(
		...
		this._register(
			server.extensionManagementService.onDidUpdateExtensionMetadata((e) =>
				this.onDidUpdateExtensionMetadata(e.local),
			),
		);
		this._register(
			server.extensionManagementService.onDidChangeProfile(() => this.reset()),
		);
		this._register(
		...
		this._register(
			Event.any(this.onChange, this.onReset)(() => (this._local = undefined)),
		);
		if (this.isWorkspaceServer) {
			this._register(
				this.workbenchExtensionManagementService.onInstallExtension((e) => {
			...
			this._register(
				this.workbenchExtensionManagementService.onDidInstallExtensions((e) => {
			...
			this._register(
				this.workbenchExtensionManagementService.onUninstallExtension((e) => {
					if (e.workspaceScoped) {
						this.onUninstallExtension(e.identifier);
					}
				}),
			...
			for (const extension of this.installing) {
				if (
					!this.installed.some((installed) =>
						areSameExtensions(installed.identifier, extension.identifier),
					)
				) {
					this._local.push(extension);
				}
	...
	async syncInstalledExtensionsWithGallery(
		...
		flagExtensionsMissingFromGallery?: IExtensionInfo[],
	): Promise<void> {
		const extensions =
		...
		for (const [extension, gallery] of extensions) {
			...
			if (
				extension.local &&
				extension.local.type !== ExtensionType.System &&
				!extension.local.identifier.uuid
			) {
		...
		if (flagExtensionsMissingFromGallery) {
			...
			for (const extension of this.local) {
				...
				if (
					!flagExtensionsMissingFromGallery.some((f) =>
						areSameExtensions(f, extension.identifier),
					)
				) {
					continue;
				}
			...
			if (extensionsToQuery.length) {
				const queryResult = await this.galleryService.getExtensions(
					extensionsToQuery.map((e) => ({
						...e.identifier,
						version: e.version,
					})),
				...
				for (const extension of extensionsToQuery) {
					...
					const gallery = queryResult.find((g) =>
						areSameExtensions(g.identifier, extension.identifier),
					);
					if (gallery) {
						extension.gallery = gallery;
					} else {
				...
				this.telemetryService.publicLog2<
					MissingFromGalleryEvent,
					MissingFromGalleryClassification
	...
	private async matchInstalledExtensionsWithGallery(
		...
		for (const extension of extensions) {
			const compatible = galleryExtensions.find((e) =>
				areSameExtensions(e.identifier, extension.identifier),
			);
			if (compatible) {
	...
	implements IExtensionsWorkbenchService, IURLHandler
{
	...
	private extensionsNotification:
		| (IExtensionsNotification & { readonly key: string })
		| undefined;
	private readonly _onDidChangeExtensionsNotification = this._register(
		new Emitter<IExtensionsNotification | undefined>(),
	);
	...
	private readonly delayedAutoUpdateCheckTimer = this._register(
	...
	private initializeAutoUpdate(): void {
		...
		this._register(
			this.extensionEnablementService.onEnablementChanged(
				(platformExtensions) => {
					if (
						this.isAutoCheckUpdatesEnabled() &&
						this.getAutoUpdateValue() !== "off" &&
	...
	getAutoUpdateValue(): AutoUpdateConfigurationValue {
		...
		if (
			...
			autoUpdate === "off" ||
			autoUpdate === "none" ||
			autoUpdate === "onlySelectedExtensions"
		) {
			return "off";
		}
	...
	private async onDidChangeRunningExtensions(
		...
		for (const desc of removed) {
			if (
				this.workspaceContextService.isInsideWorkspace(desc.extensionLocation)
			) {
	...
	async getExtensions(
		...
		extensionInfos.forEach(
			(e) =>
				(e.preRelease =
					e.preRelease ?? this.extensionManagementService.preferPreReleases),
		);
		const extensionsControlManifest =
			await this.extensionManagementService.getExtensionsControlManifest();
		const galleryExtensions = await this.galleryService.getExtensions(
	...
	private updateExtensionsNotificaiton(): void {
		...
		let extensionsNotification:
			| (IExtensionsNotification & { key: string })
			| undefined;
		if (computedNotificiations.length) {
			...
			for (const dismissedNotification of this.getDismissedNotifications()) {
				if (
					computedNotificiations.some((e) => e.key === dismissedNotification)
				) {
	...
	> {
		...
		if (disallowedExtensions.length) {
			computedNotificiations.push({
				message: this.configurationService.inspect(AllowedExtensionsConfigKey)
					.policy
					? nls.localize(
		...
		const invalidExtensions = this.local.filter(
			(e) =>
				e.enablementState === EnablementState.DisabledByInvalidExtension &&
				!e.isWorkspaceScoped,
		);
		...
		if (!this.configurationService.getValue(AutoRestartConfigurationKey)) {
			const restartRequiredExtensions = this.local.filter(
				(e) =>
					...
					(e.runtimeState.action ===
						ExtensionRuntimeActionType.RestartExtensions ||
						e.runtimeState.action === ExtensionRuntimeActionType.ReloadWindow),
			);
	...
	private resolveQueryText(text: string): string {
		...
		if (extensionRegex.test(text)) {
			text = text.replace(extensionRegex, (m, ext) => {
				...
				const languageName =
					languageId && this.languageService.getLanguageName(languageId);
	...
	async open(
		...
		if (typeof extension === "string") {
			...
			extension =
				...
				(
					await this.getExtensions([{ id: extension }], CancellationToken.None)
				)[0];
		}
		if (!extension) {
			throw new Error(`Extension not found. ${extension}`);
		}
		const useModal = this.configurationService.getValue<boolean>(
	...
	async updateRunningExtensions(
		message = nls.localize("restart", "Changing extension enablement"),
		auto: boolean = false,
	): Promise<void> {
		...
		for (const extension of this.extensionService.extensions) {
			...
			if (
				extensionsToCheck.some((e) =>
					areSameExtensions(
						{ id: extension.identifier.value, uuid: extension.uuid },
	...
	private getPrimaryExtension(extensions: IExtension[]): IExtension {
		...
		const extensionsToChoose = enabledExtensions.length
			? enabledExtensions
			: extensions;
		const manifest = extensionsToChoose.find((e) => e.local && e.local.manifest)
			?.local?.manifest;
	...
	async checkForUpdates(reason?: string, onlyBuiltin?: boolean): Promise<void> {
		...
		for (const installed of this.local) {
			...
			if (
				!installed.local?.forceAutoUpdate &&
				installed.isBuiltin &&
	...
	async downloadVSIX(
		...
		const extensionInfo = version
			? { id: extensionId, version: version.version }
			: { id: extensionId, preRelease: versionKind === "prerelease" };
		const queryOptions: IExtensionQueryOptions = version
			? {}
			: { compatible: true };
		...
		const options = [];
		for (const targetPlatform of version?.targetPlatforms ??
			galleryExtension.allTargetPlatforms) {
			if (
		...
		this.progressService.withProgress(
			...
			async (progress) => {
				try {
					progress.report({
						message: nls.localize("downloading...", "Downloading VSIX..."),
					});
	...
	private async pickVersionToDownload(
		extensionId: string,
	): Promise<IGalleryExtensionVersion | undefined> {
		const allVersions = await this.galleryService.getAllVersions({
			id: extensionId,
		});
		if (!allVersions.length) {
			await this.dialogService.info(
				nls.localize("no versions", "This extension has no other versions."),
			);
			return;
		}
		...
		await Promise.allSettled(
			extensions.map((extensions) =>
				extensions.syncInstalledExtensionsWithGallery(
					...
					flagExtensionsMissingFromGallery,
				),
			),
		);
	...
	private async autoUpdateBuiltinExtensions(): Promise<void> {
		...
		await Promises.settled(
			toUpdate.map((e) =>
				this.install(
					e,
					e.local?.preRelease ? { installPreReleaseVersion: true } : undefined,
				),
			),
		);
	}
	...
	private async autoUpdateExtensions(): Promise<void> {
		if (this.meteredConnectionService.isConnectionMetered) {
			this.logService.trace(
				"[Extensions]: Skipping auto-update because connection is metered",
			);
			return;
		}
		...
		for (const extension of this.outdated) {
			...
			if (isDelayed && !extension.local?.forceAutoUpdate) {
				...
				if (delayRemaining > 0) {
					this.logService.trace(
						"Auto update delayed for extension",
						extension.identifier.id,
					);
					soonestDelayRemaining = Math.min(
						soonestDelayRemaining,
						delayRemaining,
					);
					continue;
				}
		...
		if (soonestDelayRemaining < Number.MAX_SAFE_INTEGER) {
			this.delayedAutoUpdateCheckTimer.value = disposableTimeout(
				() => this.eventuallyCheckForUpdates(true),
				soonestDelayRemaining,
			);
		} else {
		...
		if (disabledAutoUpdate.length) {
			this.logService.trace(
				...
				disabledAutoUpdate.join(", "),
			);
		}

		if (consentRequired.length) {
			this.logService.info(
				"Auto update consent required for extensions",
				consentRequired.join(", "),
			);
		}
	...
	async shouldRequireConsentToUpdate(
		...
		if (
			extension.local.identifier.uuid &&
			extension.local.identifier.uuid !== extension.gallery.identifier.uuid
		) {
			return nls.localize(
					...
					if (enable) {
						...
						if (enabledAutoUpdateExtensions.includes(extensionOrPublisher)) {
							enabledAutoUpdateExtensions.splice(
								enabledAutoUpdateExtensions.indexOf(extensionOrPublisher),
								1,
							);
						}
					}
	...
	async install(
		...
		const extension = await this._install(
			arg,
			installOptions,
			progressLocation,
		);
		...
		if (arg instanceof URI) {
			...
			if (isString(arg)) {
				...
				if (extension?.isBuiltin) {
					if (
						this.productService.builtInExtensionsEnabledWithAutoUpdates?.some(
							(id) => id.toLowerCase() === arg.toLowerCase(),
						)
					) {
						return extension;
					}
			...
			if (gallery) {
				...
				if (installOptions.installEverywhere) {
					...
					const installableServers =
						await this.extensionManagementService.getInstallableServers(
							gallery,
						);
					for (const extensionsServer of this.extensionsServers) {
						if (
							installableServers.includes(extensionsServer.server) &&
							!extensionsServer.local.find((e) =>
								areSameExtensions(e.identifier, gallery.identifier),
							)
						) {
		...
		await this.doInstall(extension, async () => {
				...
				extension =
					(
						await this.getExtensions(
							[{ ...extension.identifier, preRelease: local.preRelease }],
							CancellationToken.None,
						)
					)[0] ?? extension;
			}
			if (extension.gallery) {
				return server.extensionManagementService.installFromGallery(
					extension.gallery,
					{ installPreReleaseVersion: local.preRelease, ...installOptions },
				);
			}
	...
	async uninstall(e: IExtension): Promise<void> {
		...
		if (
			...
			for (const packExtension of this.getAllPackedExtensions(
				extension,
				this.local,
			)) {
				if (
					...
					!extensionsToUninstall.some((e) =>
						areSameExtensions(e.extension.identifier, packExtension.identifier),
					)
				) {
		...
		for (const { extension } of extensionsToUninstall) {
					...
					await Promise.allSettled(
						this.userDataProfilesService.profiles.map(async (profile) => {
							...
							for (const local of installed) {
								extensionsFromAllProfiles?.push([
									local,
									profile.extensionsResource,
								]);
							}
			...
			for (const [local, profileLocation] of installedExtensions) {
				...
				if (
					!local.manifest.extensionDependencies ||
					local.manifest.extensionDependencies.length === 0
				) {
					continue;
				}
				if (
				...
				if (
					dependents.some((d) =>
						d.manifest.extensionPack?.some((id) =>
							areSameExtensions({ id }, local.identifier),
						),
					)
				) {
				...
				if (
					local.manifest.extensionDependencies.some((dep) =>
	...
	private getAllPackedExtensions(
		...
		if (
			checked.some((e) => areSameExtensions(e.identifier, extension.identifier))
		) {
			return [];
		}
		...
		if (extensionsPack.length) {
			...
			for (const packedExtension of packedExtensions) {
				packOfPackedExtensions.push(
					...this.getAllPackedExtensions(packedExtension, installed, checked),
				);
			}
	...
	async toggleExtensionIgnoredToSync(extension: IExtension): Promise<void> {
		const extensionsIncludingPackedExtensions = [
			extension,
			...this.getAllPackedExtensions(extension, this.local),
		];
		...
		for (const e of extensionsIncludingPackedExtensions) {
			...
			if (e.local && isIgnored && e.local.isMachineScoped) {
				...
				await this.extensionsSyncManagementService.updateIgnoredExtensions(
					e.identifier.id,
					!isIgnored,
				);
			}
	...
	async toggleApplyExtensionToAllProfiles(
		...
		await Promise.allSettled(
			extensionsIncludingPackedExtensions.map(async (e) => {
				if (
					!e.local ||
					isApplicationScopedExtension(e.local.manifest) ||
					e.isBuiltin
				) {
					return;
				}
				...
				await Promise.all(
					allExtensionServers.map(async (extensionServer) => {
						const local = extensionServer.local.find((local) =>
							areSameExtensions(e.identifier, local.identifier),
						)?.local;
	...
	private installFromGallery(
		...
		gallery: IGalleryExtension,
		installOptions: InstallExtensionOptions,
		servers: IExtensionManagementServer[] | undefined,
	): Promise<ILocalExtension> {
		installOptions = installOptions ?? {};
		installOptions.pinned =
			installOptions.pinned ??
			(extension.local?.pinned || !this.shouldAutoUpdateExtension(extension));
	...
	private async waitUntilExtensionIsEnabled(
		...
		if (
			this.extensionService.extensions.find((e) =>
				ExtensionIdentifier.equals(e.identifier, extension.identifier.id),
			)
		) {
			return;
		}
	...
	private getDependentsErrorMessageForDisablement(
		...
		for (const e of [extension, ...allDisabledExtensions]) {
			const dependentsOfTheExtension = dependents.filter((d) =>
				d.dependencies.some((id) => areSameExtensions({ id }, e.identifier)),
			);
	...
	private set enabledAuotUpdateExtensionsValue(
		...
		if (
			this.enabledAuotUpdateExtensionsValue !== enabledAuotUpdateExtensionsValue
		) {
