	...
	constructor(
		...
		this._register(
			this.contextMenuActionRunner.onDidRun(
				({ error }) => error && notificationService.error(error),
			),
		);
		...
		this.extensionsWorkbenchService.open(extension, {
			...
			modal: options.sideByside
				...
				: buildModalNavigationForPagedList(
						...
						(ext, modal) =>
							this.extensionsWorkbenchService.open(ext, {
								pinned: false,
								modal,
							}),
...
class ExtensionRenderer implements IListRenderer<
	ITreeNode<IExtensionData>,
...
class UnknownExtensionRenderer implements IListRenderer<
	ITreeNode<IExtensionData>,
...
export function buildModalNavigationForPagedList<T>(
	...
	const openAtIndex = (index: number, item: T) => {
		...
		openItem(item, {
			navigation: { total: currentTotal, current: index, navigate },
		});
	};
			...
			currentModel.resolve(index, token).then(
				...
				(error) => {
					if (!isCancellationError(error)) {
						logService.error(
							`Error while resolving item at index ${index} for modal navigation`,
							error,
						);
					}
				},
