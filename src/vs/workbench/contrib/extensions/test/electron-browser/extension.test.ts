...
suite("Extension Test", () => {
	...
	test("extension is not outdated when local and gallery are on same version and local is on web", () => {
		const extension = instantiationService.createInstance(
			...
			() => undefined,
			undefined,
			aLocalExtension("somext", {}, { targetPlatform: TargetPlatform.WEB }),
	...
	test("extension is not outdated when local and gallery are on same version and gallery is on web", () => {
		const extension = instantiationService.createInstance(
			...
			aGalleryExtension("somext", {}, { targetPlatform: TargetPlatform.WEB }),
			undefined,
		);
		assert.strictEqual(extension.outdated, false);
	});
	...
	function aLocalExtension(
		...
		properties: Partial<ILocalExtension> = {},
	): ILocalExtension {
	...
	function aGalleryExtension(
		...
		galleryExtensionProperties: Partial<IGalleryExtensionProperties> = {},
	): IGalleryExtension {
