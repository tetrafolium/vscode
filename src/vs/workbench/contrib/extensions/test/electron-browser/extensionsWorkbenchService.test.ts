...
suite("ExtensionsWorkbenchServiceTest", () => {
	...
	setup(async () => {
		...
		instantiationService.stub(IExtensionService, {
			...
			extensions: [],
			canAddExtension(extension: any) {
				return false;
			},
			canRemoveExtension(extension: any) {
				return false;
			},
	...
	test("test isAutoUpdateDelayed returns false when auto update is not set to delayed", async () => {
		...
		assert.strictEqual(
			testObject.isAutoUpdateDelayed(testObject.local[0]),
			false,
		);
	});
	...
	test("test isAutoUpdateDelayed returns true for recently published version when auto update is set to delayed", async () => {
		...
		testObject = await anOutdatedExtensionWorkbenchService(
	...
	test("test isAutoUpdateDelayed returns false for version published more than 6 hours ago", async () => {
		stubConfiguration("delayed", true);
		testObject = await anOutdatedExtensionWorkbenchService(
			Date.now() - 1000 * 60 * 60 * 7 /* 7 hours ago */,
		);
	...
	test("test isAutoUpdateDelayed returns false for version with a future published timestamp", async () => {
		stubConfiguration("delayed", true);
		testObject = await anOutdatedExtensionWorkbenchService(
			Date.now() + 1000 * 60 * 60 /* 1 hour in the future */,
		);
	...
	test("test getAutoUpdateDelayRemaining returns remaining time within the delay window", async () => {
		...
		const remaining = testObject.getAutoUpdateDelayRemaining(
			testObject.local[0],
		);
		// Published 2 hours ago, so ~4 hours of the 6 hour window remain.
		assert.ok(
			remaining > 1000 * 60 * 60 * 3 && remaining <= 1000 * 60 * 60 * 4,
			`unexpected remaining time ${remaining}`,
		);
	});
	...
	test("test getAutoUpdateDelayRemaining returns 0 when the published timestamp is missing", async () => {
		...
		assert.strictEqual(
			testObject.getAutoUpdateDelayRemaining(testObject.local[0]),
			0,
		);
	});
	...
	test("test reload prompt when disabling running extension", async () => {
		const localExtension = aLocalExtension(
			"my_extension",
			{},
			{ identifier: { id: "my_publisher.my_extension" } },
		);
		instantiationService.stubPromise(
			IExtensionManagementService,
			"getInstalled",
	...
	async function anOutdatedExtensionWorkbenchService(
		lastUpdated: number | undefined,
	): Promise<ExtensionsWorkbenchService> {
