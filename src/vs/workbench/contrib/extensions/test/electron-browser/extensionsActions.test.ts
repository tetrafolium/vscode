...
function setupTest(disposables: Pick<DisposableStore, "add">) {
	...
	instantiationService.stub(
		IWorkbenchExtensionEnablementService,
		disposables.add(new TestExtensionEnablementService(instantiationService)),
	);
	instantiationService.stub(ILabelService, {
		onDidChangeFormatters: disposables.add(new Emitter<IFormatterChangeEvent>())
			.event,
	});
	...
	instantiationService.stub(IExtensionService, {
		...
		canAddExtension: (extension: IExtensionDescription) => false,
		canRemoveExtension: (extension: IExtensionDescription) => false,
...
suite("ExtensionsActions", () => {
	...
	test("Install action is disabled when there is no extension", () => {
		const testObject: ExtensionsActions.InstallAction = disposables.add(
			instantiationService.createInstance(ExtensionsActions.InstallAction, {
	...
	test("Uninstall action is disabled when there is no extension", () => {
		const testObject: ExtensionsActions.UninstallAction = disposables.add(
			instantiationService.createInstance(ExtensionsActions.UninstallAction),
		);
		disposables.add(
			instantiationService.createInstance(ExtensionContainers, [testObject]),
		);
	...
	test("Test UpdateAction when there is no extension", () => {
		const testObject: ExtensionsActions.UpdateAction = disposables.add(
			instantiationService.createInstance(
	...
	test("Test UpdateAction when extension is installing and outdated and user extension", async () => {
		...
		const extensions = await instantiationService
			.get(IExtensionsWorkbenchService)
			.queryLocal();
		testObject.extension = extensions[0];
		const gallery = aGalleryExtension("a", {
	...
	test("Test ManageExtensionAction when there is no extension", () => {
		const testObject: ExtensionsActions.ManageExtensionAction = disposables.add(
			instantiationService.createInstance(
	...
	test("Test EnableGloballyAction when there is no extension", () => {
		const testObject: ExtensionsActions.EnableGloballyAction = disposables.add(
			instantiationService.createInstance(
	...
	test("Test EnableGloballyAction when the extension is not disabled", () => {
		...
		return instantiationService
			...
			.queryLocal()
			.then((extensions) => {
				const testObject: ExtensionsActions.EnableGloballyAction =
					disposables.add(
						instantiationService.createInstance(
							ExtensionsActions.EnableGloballyAction,
						),
	...
	test("Test EnableAction when there is no extension", () => {
		const testObject: ExtensionsActions.EnableDropDownAction = disposables.add(
			instantiationService.createInstance(
	...
	test("Test EnableDropDownAction when extension is installed and enabled", () => {
		...
		return instantiationService
			...
			.queryLocal()
			.then((extensions) => {
				const testObject: ExtensionsActions.EnableDropDownAction =
					disposables.add(
						instantiationService.createInstance(
							ExtensionsActions.EnableDropDownAction,
						),
	...
	test("Test DisableForWorkspaceAction when there is no extension", () => {
		const testObject: ExtensionsActions.DisableForWorkspaceAction =
			disposables.add(
				instantiationService.createInstance(
	...
	test("Test DisableGloballyAction when there is no extension", () => {
		const testObject: ExtensionsActions.DisableGloballyAction = disposables.add(
			instantiationService.createInstance(
		...
		return instantiationService
			...
			.then(() => {
				...
				return instantiationService
					...
					.queryLocal()
					.then((extensions) => {
						const testObject: ExtensionsActions.DisableGloballyAction =
							disposables.add(
								instantiationService.createInstance(
									ExtensionsActions.DisableGloballyAction,
								),
...
suite("ExtensionRuntimeStateAction", () => {
	...
	test("Test Runtime State when there is no extension", () => {
		const testObject: ExtensionsActions.ExtensionRuntimeStateAction =
			disposables.add(
				instantiationService.createInstance(
	...
	test("Test Runtime State when extension is uninstalled", async () => {
		instantiationService.stub(IExtensionService, {
			extensions: [
				toExtensionDescription(aLocalExtension("a", { version: "1.0.0" })),
			],
			onDidChangeExtensions: Event.None,
			canRemoveExtension: (extension) => false,
			canAddExtension: (extension) => false,
	...
	test("Test Runtime State when extension is updated while running", async () => {
		instantiationService.stub(IExtensionService, {
			extensions: [
				toExtensionDescription(aLocalExtension("a", { version: "1.0.1" })),
			],
			onDidChangeExtensions: Event.None,
			canRemoveExtension: (extension) => true,
			canAddExtension: (extension) => false,
			...
			disposables.add(
				testObject.onDidChange(() => {
					if (
						testObject.enabled &&
						testObject.tooltip ===
							`Please restart extensions to enable the updated extension.`
					) {
						c();
					}
				}),
	...
	test("Test Runtime State when extension is not installed but extension from different server is installed and running", async () => {
		...
		const workbenchService: IExtensionsWorkbenchService = disposables.add(
			instantiationService.createInstance(ExtensionsWorkbenchService),
		);
	...
	test("Test Runtime State when extension is uninstalled but extension from different server is installed and running", async () => {
		...
		localExtensionManagementService.onDidUninstallExtension =
			onDidUninstallEvent.event;
		const extensionManagementServerService =
			aMultiExtensionManagementServerService(
				instantiationService,
	...
	test("Test Runtime State when workspace extension is disabled on local server and installed in remote server", async () => {
		...
		remoteExtensionManagementService.onDidInstallExtensions =
			onDidInstallEvent.event;
		const localExtension = aLocalExtension(
			"a",
			{ extensionKind: ["workspace"] },
			{ location: URI.file("pub.a") },
	...
	test("Test Runtime State when ui extension is disabled on remote server and installed in local server", async () => {
		...
		const onDidInstallEvent = new Emitter<readonly InstallExtensionResult[]>();
		localExtensionManagementService.onDidInstallExtensions =
			onDidInstallEvent.event;
		const remoteExtension = aLocalExtension(
			"a",
			{ extensionKind: ["ui"] },
	...
	test("Test Runtime State when ui+workspace+web extension is installed in web and remote and running in remote", async () => {
		...
		const webExtension = aLocalExtension(
			...
			{ location: URI.file("pub.a").with({ scheme: Schemas.vscodeUserData }) },
		);
		const remoteExtension = aLocalExtension(
			"a",
			{ extensionKind: ["ui", "workspace"], browser: "browser.js" },
...
suite("RemoteInstallAction", () => {
	...
	test("Test remote install action when installing local workspace extension", async () => {
		...
		remoteExtensionManagementService.onInstallExtension =
	...
	test("Test remote install action is disabled for local workspace system extension", async () => {
		...
		const extensionManagementServerService =
			aMultiExtensionManagementServerService(
				instantiationService,
				createExtensionManagementService([localWorkspaceSystemExtension]),
			);
		instantiationService.stub(
			IExtensionManagementServerService,
			extensionManagementServerService,
	...
	test("Test remote install action is disabled for local ui extension if it is not installed in remote", async () => {
		...
		instantiationService.stubPromise(
			...
			"query",
			aPage(
				aGalleryExtension("a", { identifier: localUIExtension.identifier }),
			),
		);
		const testObject = disposables.add(
			instantiationService.createInstance(
...
suite("LocalInstallAction", () => {
	...
	test("Test local install action is enabled for remote ui extension", async () => {
		...
		instantiationService.stubPromise(
			...
			"query",
			aPage(
				aGalleryExtension("a", { identifier: remoteUIExtension.identifier }),
			),
		);
		const testObject = disposables.add(
			instantiationService.createInstance(ExtensionsActions.LocalInstallAction),
		);
		disposables.add(
			instantiationService.createInstance(ExtensionContainers, [testObject]),
		);
	...
	test("Test local install action when installing remote ui extension", async () => {
		...
		const localExtensionManagementService = createExtensionManagementService();
		const onInstallExtension = new Emitter<InstallExtensionEvent>();
		localExtensionManagementService.onInstallExtension =
			onInstallExtension.event;
		const remoteUIExtension = aLocalExtension(
			"a",
			{ extensionKind: ["ui"] },
	...
	test("Test local install action is enabled for disabled remote ui extension", async () => {
		...
		await instantiationService
			.get(IWorkbenchExtensionEnablementService)
			.setEnablement([localUIExtension], EnablementState.DisabledGlobally);
		instantiationService.stubPromise(
			IExtensionGalleryService,
			"query",
			aPage(
	...
	test("Test local install action is disabled when local server is not available", async () => {
		...
		const extensionManagementServerService =
			aSingleRemoteExtensionManagementServerService(
				instantiationService,
				createExtensionManagementService([remoteUIExtension]),
			);
		instantiationService.stub(
...
suite("EnableAIFeaturesInWorkspaceAction", () => {
	...
	test("test enable AI in workspace updates workspace setting when AI is disabled globally", async () => {
		...
		let updatedValue:
			| { key: string; value: unknown; target: unknown }
			| undefined;
		const originalUpdateValue =
			configurationService.updateValue.bind(configurationService);
		configurationService.updateValue = async (
			key: string,
			value: unknown,
			target?: any,
		) => {
		...
		const chatExtension = aLocalExtension(
			"copilot-chat",
			{ publisher: "GitHub" },
			{ type: ExtensionType.System },
		);
		...
		const copilotChat = extensions.find(
			(e) => e.identifier.id === "github.copilot-chat",
		);
		assert.ok(copilotChat);

		const testObject: ExtensionsActions.EnableAIFeaturesInWorkspaceAction =
			disposables.add(
				instantiationService.createInstance(
					ExtensionsActions.EnableAIFeaturesInWorkspaceAction,
				),
			);
		disposables.add(
			instantiationService.createInstance(ExtensionContainers, [testObject]),
		...
		assert.strictEqual(
			updatedValue.value,
			false,
			"workspace setting should be set to false",
		);
	});
});
...
function aSingleRemoteExtensionManagementServerService(
...
function aMultiExtensionManagementServerService(
	instantiationService: TestInstantiationService,
	localExtensionManagementService?: IProfileAwareExtensionManagementService | null,
	remoteExtensionManagementService?: IProfileAwareExtensionManagementService | null,
	webExtensionManagementService?: IProfileAwareExtensionManagementService,
): IExtensionManagementServerService {
	const localExtensionManagementServer: IExtensionManagementServer | null =
		localExtensionManagementService === null
			? null
			: {
					id: "vscode-local",
					label: "local",
	...
	const remoteExtensionManagementServer: IExtensionManagementServer | null =
		remoteExtensionManagementService === null
			? null
			: {
					id: "vscode-remote",
					label: "remote",
	...
	const webExtensionManagementServer: IExtensionManagementServer | null =
...
function createExtensionManagementService(
	...
	return <IProfileAwareExtensionManagementService>{
		...
		canInstall: async (extension: IGalleryExtension) => {
			return true;
		},
