...
suite("ExtensionsViews Tests", () => {
	...
	const localDisabledLanguage = aLocalExtension(
		...
		{ installedTimestamp: Date.now() - 50000, updated: true },
	);
	...
	setup(async () => {
		...
		instantiationService.stub(IExtensionGalleryService, <
			Partial<IExtensionGalleryService>
		>{
			query: async () => {
				return queryPage;
			},
		...
		await (<TestExtensionEnablementService>(
			instantiationService.get(IWorkbenchExtensionEnablementService)
		)).setEnablement([localDisabledTheme], EnablementState.DisabledGlobally);
		await (<TestExtensionEnablementService>(
			instantiationService.get(IWorkbenchExtensionEnablementService)
		)).setEnablement([localDisabledLanguage], EnablementState.DisabledGlobally);
	...
	test("Test query types", () => {
		...
		assert.strictEqual(
			ExtensionsListView.isLocalExtensionsQuery("@agentPlugins @installed"),
			false,
		);
		assert.strictEqual(
			ExtensionsListView.isLocalExtensionsQuery(
				"@agentPlugins @installed searchText",
			),
			false,
		);
		assert.strictEqual(
			ExtensionsListView.isLocalExtensionsQuery("@mcp @installed"),
			false,
		);
		assert.strictEqual(
			ExtensionsListView.isLocalExtensionsQuery("@mcp @installed searchText"),
			false,
		);
	});

	test("Test empty query equates to sort by install count", async () => {
		const target = <SinonStub>(
			instantiationService.stubPromise(
				IExtensionGalleryService,
				"query",
				aPage(),
	...
	test("Test default view actions required sorting", async () => {
		...
		const extension = (await workbenchService.queryLocal()).find(
			(ex) => ex.identifier.id === localEnabledLanguage.identifier.id,
		);
	...
	test("Test installed query results", async () => {
		...
		await testableView
			.show('@builtin category:"programming languages"')
			.then((result) => {
				assert.strictEqual(
					result.length,
					1,
	...
	test("Test installed query with category", async () => {
		...
		await testableView
			.show('@installed category:"programming languages"')
			.then((result) => {
				assert.strictEqual(
					result.length,
					2,
		...
		await testableView
			.show("@installed category:themes category:random")
			.then((result) => {
				assert.strictEqual(
					result.length,
					3,
		...
		await testableView
			.show('@enabled category:"programming languages"')
			.then((result) => {
				assert.strictEqual(
					result.length,
					1,
		...
		await testableView
			.show('@disabled category:"programming languages"')
			.then((result) => {
				assert.strictEqual(
					result.length,
					1,
	...
	function aPage<T>(
		objects: IGalleryExtension[] = [],
		total?: number,
	): IPager<IGalleryExtension> {
		return {
			firstPage: objects,
