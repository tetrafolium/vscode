...
suite("ExtensionRecommendationsService Test", () => {
	...
	setup(() => {
			...
			extensionRecommendations: {
				...
				"ms-dotnettools.csharp": {
					onFileOpen: [
						{
							pathGlob:
								"{**/*.cs,**/project.json,**/global.json,**/*.csproj,**/*.sln,**/appsettings.json}",
						},
					],
				},
				"msjsdiag.debugger-for-chrome": {
					onFileOpen: [
						{
							pathGlob:
								"{**/*.ts,**/*.tsx,**/*.js,**/*.jsx,**/*.es6,**/*.mjs,**/*.cjs,**/.babelrc}",
						},
					],
				},
				"lukehoban.Go": {
		...
		instantiationService.stubPromise(
			IExtensionGalleryService,
			"query",
			aPage<IGalleryExtension>(...mockExtensionGallery),
		);
		instantiationService.stubPromise(
			IExtensionGalleryService,
			"getExtensions",
	...
	async function setUpFolder(
		...
		instantiationService.stub(
			IWorkspaceExtensionsConfigService,
			disposableStore.add(
				instantiationService.createInstance(WorkspaceExtensionsConfigService),
			),
		);
		instantiationService.stub(
	...
	function testNoPromptForValidRecommendations(recommendations: string[]) {
		return setUpFolderWorkspace("myFolder", recommendations).then(() => {
			testObject = disposableStore.add(
				instantiationService.createInstance(ExtensionRecommendationsService),
			);
	...
	function testNoPromptOrRecommendationsForValidRecommendations(
	...
	test("ExtensionRecommendationsService: No Prompt for valid workspace recommendations with casing mismatch if they are already installed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			return testNoPromptForValidRecommendations(
				mockTestData.validRecommendedExtensions.map((x) => x.toUpperCase()),
			);
		}));
	...
	test("ExtensionRecommendationsService: No Recommendations of workspace ignored recommendations", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const storedRecommendations =
				'["ms-dotnettools.csharp", "ms-python.python"]';
			instantiationService
				.get(IStorageService)
				.store(
					"extensionsAssistant/workspaceRecommendationsIgnore",
	...
	test("ExtensionRecommendationsService: Able to retrieve collection of all ignored recommendations", async () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const storedRecommendations =
				'["ms-dotnettools.csharp", "ms-python.python"]';
			const globallyIgnoredRecommendations =
				'["mockpublisher2.mockextension2"]'; // ignore a workspace recommendation.
		...
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			return setUpFolderWorkspace("myFolder", []).then(() => {
				...
				return testObject.activationPromise.then(() => {
					...
					); // stored recommendation that exists in product.extensionTips
					assert.ok(
						recommendations.some(
							(extensionId) => extensionId === "ms-python.python",
						),
					); // stored recommendation that exists in product.extensionImportantTips
					assert.ok(
						recommendations.every(
							(extensionId) =>
								extensionId !== "ms-vscode.vscode-typescript-tslint-plugin",
						),
					); // stored recommendation that is no longer in neither product.extensionTips nor product.extensionImportantTips
				});
			});
		}));
	...
	test("ExtensionRecommendationsService: Get file based recommendations from storage (new format)", async () => {
		...
		assert.ok(
			recommendations.some((extensionId) => extensionId === "ms-python.python"),
		); // stored recommendation that exists in product.extensionImportantTips
		assert.ok(
			recommendations.every(
				(extensionId) =>
		...
		assert.ok(
			recommendations.every((extensionId) => extensionId !== "lukehoban.Go"),
		); //stored recommendation that is older than a week
	});
});
