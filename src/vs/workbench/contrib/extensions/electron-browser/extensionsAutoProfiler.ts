...
export class ExtensionsAutoProfiler implements IWorkbenchContribution {
	...
	private async _processCpuProfile(profile: IExtensionHostProfile) {
		...
		if (
			this._configService.getValue("application.experimental.rendererProfiling")
		) {
			const searchTree = TernarySearchTree.forUris<IExtensionDescription>();
			searchTree.fill(
				this._extensionService.extensions.map((e) => [e.extensionLocation, e]),
			);
			...
			[
				{
					...
					run: () =>
						this._editorService.openEditor(RuntimeExtensionsInput.instance, {
							pinned: true,
						}),
				},
				action,
