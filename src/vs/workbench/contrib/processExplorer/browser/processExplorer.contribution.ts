		...
		editorResolverService.registerEditor(
			...
			},
			{
				...
				canSupportResource: (resource) =>
					resource.scheme === ProcessExplorerEditorInput.RESOURCE.scheme,
			},
			{
				createEditorInput: () => {
					return {
...
const supported = ContextKeyExpr.or(
	IsWebContext.negate(),
	RemoteNameContext.notEqualsTo(""),
); // only on desktop or in web with a remote
...
class OpenProcessExplorer extends Action2 {
	...
	private static readonly DEFAULT_STATE: IProcessExplorerWindowState = {
	...
	private saveState(
		...
		const auxiliaryWindow = auxiliaryWindowService.getWindow(
			editorGroupService.getPart(group).windowId,
		);
		if (!auxiliaryWindow) {
			return;
		}
