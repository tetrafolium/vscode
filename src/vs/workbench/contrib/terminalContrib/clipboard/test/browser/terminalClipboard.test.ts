...
suite("TerminalClipboard", function () {
	...
	suite("shouldPasteTerminalText", () => {
		...
		setup(async () => {
			...
			instantiationService.stub(
				IDialogService,
				new TestDialogService(undefined, { result: { confirmed: false } }),
			);
		});
		...
		test("Single line string with trailing new line", async () => {
			...
			strictEqual(
				...
				JSON.stringify({ modifiedText: "foo" }),
			);
			strictEqual(
				JSON.stringify(
					await instantiationService.invokeFunction(
	...
	suite("TerminalClipboardContribution.paste", () => {
		...
		setup(async () => {
			...
			instantiationService.stub(
				ITerminalConfigurationService,
				upcastPartial<ITerminalConfigurationService>({}),
			);
			...
			contribution = store.add(
				instantiationService.createInstance(TerminalClipboardContribution, ctx),
			);
			contribution.xtermReady(
				upcastDeepPartial<IXtermTerminal & { raw: RawXtermTerminal }>({
		...
		test("Clipboard resource fallback when text is empty", async () => {
			await clipboardService.writeResources([
				URI.file("/foo/bar.png"),
				URI.file("/foo/baz.png"),
			]);
