...
class TerminalFindAccessibilityHelpProvider
	extends Disposable
	implements IAccessibleViewContentProvider
{
	...
	provideContent(): string {
		...
		// Header
		content.push(
			localize("terminal.header", "Accessibility Help: Terminal Find"),
		);
		content.push(
			localize(
				"terminal.context",
		...
		content.push(
			localize("terminal.statusDesc", "You are searching the terminal buffer."),
		);
		content.push("");
		...
		// Focus Behavior
		content.push(
			localize("terminal.focusHeader", "Focus Behavior (Important):"),
		);
		content.push(
			localize(
				"terminal.focusDesc1",
		...
		content.push(
			localize("terminal.keyboardHeader", "Keyboard Navigation Summary:"),
		);
		content.push("");
		content.push(
			localize("terminal.keyNavHeader", "While focused IN the Find input:"),
		);
		content.push(
			localize(
				"terminal.keyEnter",
