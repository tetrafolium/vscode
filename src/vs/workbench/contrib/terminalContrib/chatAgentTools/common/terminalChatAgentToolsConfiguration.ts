...
export const terminalChatAgentToolsConfiguration: IStringDictionary<IConfigurationPropertySchema> =
	{
		...
		[TerminalChatAgentToolsSettingId.TerminalProfileLinux]: {
			...
			type: ["object", "null"],
			default: null,
			anyOf: [{ type: "null" }, terminalChatAgentProfileSchema],
			defaultSnippets: [
				{
					body: {
						path: "${1}",
			...
			properties: {
				...
				allowWrite: {
					...
					description: localize(
						...
						"Array of additional paths to allow read/write access. Leave empty to disallow writes outside the workspace folders and sandbox temp directory.",
					),
					items: { type: "string" },
					default: [],
				},
