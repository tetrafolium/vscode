...
import {
	...
	grepFilter,
	treeFilter,
} from "../../browser/tools/terminalOutputCompressor.js";
...
suite("parseCommandHead", () => {
	...
	test("parses simple commands", () => {
		deepStrictEqual(parseCommandHead("git diff HEAD~5"), {
			head: "git",
			sub: "diff",
		});
	...
	test("skips env-var prefixes", () => {
		deepStrictEqual(parseCommandHead("CI=1 NODE_ENV=test npm install"), {
			head: "npm",
			sub: "install",
		});
	});
	...
	test("uses only first pipeline segment", () => {
		deepStrictEqual(parseCommandHead("git diff | cat"), {
			head: "git",
			sub: "diff",
		});
	});
	...
	test("skips leading long flags before the subcommand", () => {
		deepStrictEqual(parseCommandHead("git --no-pager diff src/foo.ts"), {
			head: "git",
			sub: "diff",
		});
	});
	...
	test("does not skip short-flag values before the subcommand", () => {
		deepStrictEqual(parseCommandHead("git -C /tmp/repo diff"), {
			head: "git",
			sub: "-C",
		});
	});
...
suite("gitDiffFilter", () => {
	...
	test("matches git --no-pager diff", () => {
		ok(
			gitDiffFilter.matches("run_in_terminal", {
				command: "git --no-pager diff src/foo.ts",
			}),
		);
	});
	...
	test("collapses long unchanged-context runs into a single marker", () => {
		const ctxLines = Array.from(
			{ length: 20 },
			(_, i) => ` this is context line number ${i}`,
		);
		const text = [
			"diff --git a/foo.ts b/foo.ts",
			"--- a/foo.ts",
...
suite("gitDiffFilter - regression", () => {
	...
	test("does not match `git difftool` (only diff/show)", () => {
		ok(
			!gitDiffFilter.matches("run_in_terminal", {
				command: "git difftool HEAD~1",
			}),
		);
		ok(
			!gitDiffFilter.matches("run_in_terminal", {
				command: "git difftool --tool=vscode",
			}),
		);
	});
	...
	test("does not match `git diff-tree` or `git diff-files`", () => {
		ok(
			!gitDiffFilter.matches("run_in_terminal", {
				command: "git diff-tree HEAD",
			}),
		);
		ok(
			!gitDiffFilter.matches("run_in_terminal", { command: "git diff-files" }),
		);
	});
...
suite("gitLogFilter", () => {
	...
	test("matches git log", () => {
		...
		ok(
			gitLogFilter.matches("run_in_terminal", {
				command: "git --no-pager log --oneline -n 20",
			}),
		);
	});
...
suite("gitStatusFilter", () => {
	...
	test("matches git status", () => {
		...
		ok(
			gitStatusFilter.matches("run_in_terminal", { command: "git status -s" }),
		);
	});
	test("does not match git stash", () => {
		ok(
			!gitStatusFilter.matches("run_in_terminal", {
...
suite("testRunnerFilter", () => {
	...
	test("matches common test runners", () => {
		...
		ok(
			testRunnerFilter.matches("run_in_terminal", { command: "go test ./..." }),
		);
		ok(
			testRunnerFilter.matches("run_in_terminal", {
...
suite("buildToolFilter", () => {
	...
	test("matches build commands", () => {
		...
		ok(
			buildToolFilter.matches("run_in_terminal", { command: "go build ./..." }),
		);
