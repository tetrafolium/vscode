...
suite("CommandLineAutoApprover", () => {
	...
	setup(() => {
		...
		commandLineAutoApprover = store.add(
			instantiationService.createInstance(CommandLineAutoApprover),
		);
	});
	...
	async function isAutoApproved(commandLine: string): Promise<boolean> {
		return (
			(
				await commandLineAutoApprover.isCommandAutoApproved(
					commandLine,
					shell,
					os,
					undefined,
	...
	function isCommandLineAutoApproved(commandLine: string): boolean {
		return (
			commandLineAutoApprover.isCommandLineAutoApproved(commandLine).result ===
			"approved"
		);
	}
	...
	suite("regex patterns", () => {
		...
		suite("flags", () => {
			...
			test("should handle regex patterns without flags", async () => {
				...
				ok(await isAutoApproved("echo hello"));
				ok(
					!(await isAutoApproved("ECHO hello")),
					"Should be case-sensitive without i flag",
				);
				ok(!(await isAutoApproved("rm file")));
				ok(
					!(await isAutoApproved("RM file")),
					"Should be case-sensitive without i flag",
				);
			});
		});
	...
	suite("edge cases", () => {
		...
		test("should handle string-based values with special regex characters", async () => {
			...
			ok(
				await isAutoApproved(
					"pwsh.exe -File D:\\foo.bar\\a-script.ps1 -AnotherArg",
				),
			);
		});
	...
	suite("isCommandLineAutoApproved - matchCommandLine functionality", () => {
		...
		test("should handle complex deny patterns with matchCommandLine", async () => {
			...
			ok(
				!isCommandLineAutoApproved(
					"powershell -File script.ps1 -ExecutionPolicy Bypass",
				),
			);
		});
	...
	suite("reasons", () => {
		...
		suite("command", () => {
			test("approved", async () => {
				...
				strictEqual(
					await getCommandReason("echo hello"),
					`Command 'echo hello' is approved by allow list rule: echo`,
				);
			});
			test("not approved", async () => {
				...
				strictEqual(
					await getCommandReason("echo hello"),
					`Command 'echo hello' is denied by deny list rule: echo`,
				);
			});
			test("no match", async () => {
				setAutoApprove({});
				strictEqual(
					await getCommandReason("echo hello"),
					`Command 'echo hello' has no matching auto approve entries`,
				);
			});
		});
		...
		suite("command line", () => {
			test("approved", async () => {
				...
				strictEqual(
					getCommandLineReason("echo hello"),
					`Command line 'echo hello' is approved by allow list rule: echo`,
				);
			});
			test("not approved", async () => {
				setAutoApproveWithCommandLine({
					echo: { approve: false, matchCommandLine: true },
				});
				strictEqual(
					getCommandLineReason("echo hello"),
					`Command line 'echo hello' is denied by deny list rule: echo`,
				);
			});
			test("no match", async () => {
				setAutoApproveWithCommandLine({});
				strictEqual(
					getCommandLineReason("echo hello"),
					`Command line 'echo hello' has no matching auto approve entries`,
				);
			});
		});
	});
	...
	suite("isDefaultRule logic", () => {
		async function getIsDefaultRule(
			command: string,
		...
		function getCommandLineIsDefaultRule(
			commandLine: string,
		): boolean | undefined {
			const rule =
				commandLineAutoApprover.isCommandLineAutoApproved(commandLine).rule;
			return isAutoApproveRule(rule) ? rule.isDefaultRule : undefined;
		}

		function setAutoApproveWithDefaults(
			userConfig: { [key: string]: boolean },
			defaultConfig: { [key: string]: boolean },
		) {
		...
		function setAutoApproveWithDefaultsCommandLine(
			userConfig: {
				[key: string]:
					| { approve: boolean; matchCommandLine?: boolean }
					| boolean;
			},
			defaultConfig: {
				[key: string]:
		...
		test("should correctly identify default rules vs user-defined rules", async () => {
			...
			strictEqual(
				await getIsDefaultRule("echo hello"),
				true,
				"echo is in both default and user config with same value - should be marked as default",
			);
			strictEqual(
				await getIsDefaultRule("ls -la"),
				false,
				"ls is only in user config - should be marked as user-defined",
			);
			strictEqual(
				await getIsDefaultRule("pwd"),
				false,
				"pwd is only in user config - should be marked as user-defined",
			);
			strictEqual(
				await getIsDefaultRule("cat file.txt"),
				true,
				"cat is in both default and user config with same value - should be marked as default",
			);
		});
		...
		test("should mark as default when command is only in default config but not in user config", async () => {
			...
			strictEqual(
				...
				"approved",
				"ls should be approved",
			);

			// cat should be approved because it's in the merged config
			const catResult = await commandLineAutoApprover.isCommandAutoApproved(
				"cat",
				shell,
				os,
				undefined,
			...
			strictEqual(
				catResult.result,
				"approved",
				"cat should be approved from default config",
			);

			// cat should be marked as default rule since it comes from default config only
			strictEqual(
				isAutoApproveRule(catResult.rule)
					? catResult.rule.isDefaultRule
					: undefined,
				true,
				"cat is only in default config, not in user config - should be marked as default",
			);
		});
		...
		test("should handle default rules with different values", async () => {
			...
			strictEqual(
				await getIsDefaultRule("echo hello"),
				false,
				"echo has different values in default vs user - should be marked as user-defined",
			);
			strictEqual(
				await getIsDefaultRule("rm file.txt"),
				true,
				"rm has same value in both - should be marked as default",
			);
		});
		...
		test("should handle regex patterns as default rules", async () => {
			...
			strictEqual(
				await getIsDefaultRule("git status"),
				true,
				"git pattern matches default - should be marked as default",
			);
			strictEqual(
				await getIsDefaultRule("npm install"),
				false,
				"npm pattern is user-only - should be marked as user-defined",
			);
		});
		...
		test("should handle mixed string and regex patterns", async () => {
			...
			strictEqual(
				await getIsDefaultRule("echo hello"),
				true,
				"String pattern matching default",
			);
			strictEqual(
				await getIsDefaultRule("ls -la"),
				false,
		...
		test("should handle command line rules with isDefaultRule", async () => {
			...
			strictEqual(
				getCommandLineIsDefaultRule("echo hello world"),
				true,
				"echo matches default config exactly using structural equality - should be marked as default",
			);
			strictEqual(
				getCommandLineIsDefaultRule("ls -la"),
				false,
				"ls is user-defined only - should be marked as user-defined",
			);
		});
		...
		test("should handle command line rules with different matchCommandLine values", async () => {
			...
			strictEqual(
				getCommandLineIsDefaultRule("echo hello"),
				false,
				"echo has different matchCommandLine value - should be user-defined",
			);
			strictEqual(
				getCommandLineIsDefaultRule("ls -la"),
				undefined,
				"ls matches exactly - should be default (but won't match command line check since matchCommandLine is false)",
			);
		});
		...
		test("should handle boolean vs object format consistency", async () => {
			...
			strictEqual(
				await getIsDefaultRule("echo hello"),
				true,
				"Boolean format matching - should be default",
			);
			strictEqual(
				getCommandLineIsDefaultRule("ls -la"),
				true,
				"Object format matching using structural equality - should be default",
			);
		});
		...
		test("should return undefined for noMatch cases", async () => {
			...
			strictEqual(
				await getIsDefaultRule("unknown-command"),
				undefined,
				"Command that matches neither user nor default config",
			);
			strictEqual(
				getCommandLineIsDefaultRule("unknown-command"),
				undefined,
				"Command that matches neither user nor default config",
			);
		});
		...
		test("should handle only default config with no user overrides", async () => {
			...
			strictEqual(
				await getIsDefaultRule("echo hello"),
				true,
				"Commands in default config should be marked as default rules even with empty user config",
			);
			strictEqual(
				await getIsDefaultRule("ls -la"),
				true,
		...
		test("should handle complex nested object rules", async () => {
			...
			strictEqual(
				getCommandLineIsDefaultRule("npm install"),
				true,
				"npm matches default exactly using structural equality - should be default",
			);
			strictEqual(
				getCommandLineIsDefaultRule("git status"),
				undefined,
				"git is user-defined - should be user-defined (but won't match command line since matchCommandLine is false)",
			);
		});
		...
		test("should handle PowerShell case-insensitive matching with defaults", async () => {
			...
			strictEqual(
				await getIsDefaultRule("Get-Process"),
				true,
				"Case-insensitive PowerShell command matching default",
			);
			strictEqual(
				await getIsDefaultRule("get-process"),
				true,
		...
		test("should use structural equality for object comparison", async () => {
			...
			strictEqual(
				getCommandLineIsDefaultRule("test command"),
				true,
				"Even though userConfig and defaultConfig are different object instances, they have the same structure and values, so should be considered default",
			);
		});
		...
		test("should detect structural differences in objects", async () => {
			const userConfig = { test: { approve: true, matchCommandLine: true } };
			const defaultConfig = {
				test: { approve: true, matchCommandLine: false },
			};
			...
			strictEqual(
				getCommandLineIsDefaultRule("test command"),
				false,
				"Objects have different matchCommandLine values, so should be user-defined",
			);
		});
		...
		test("should handle mixed types correctly", async () => {
			...
			strictEqual(
				await getIsDefaultRule("cmd1 arg"),
				true,
				"Boolean type should match default",
			);
			strictEqual(
				getCommandLineIsDefaultRule("cmd2 arg"),
				true,
				"Object type should match default using structural equality (even though it's a deny rule)",
			);
		});
	});
	...
	suite("ignoreDefaultAutoApproveRules", () => {
		...
		test("should include default rules when ignoreDefaultAutoApproveRules is false (default behavior)", async () => {
			...
			ok(
				await isAutoApproved("echo hello"),
				"Default rule should work when not ignored",
			);
			ok(
				await isAutoApproved("cat file.txt"),
				"Default rule should work when not ignored",
			);
		});
		...
		test("should exclude default rules when ignoreDefaultAutoApproveRules is true", async () => {
			...
			ok(
				!(await isAutoApproved("echo hello")),
				"Default rule should be ignored",
			);
			ok(
				!(await isAutoApproved("cat file.txt")),
				"Default rule should be ignored",
		...
		test("should attribute workspace-folder-scoped rules to WORKSPACE_FOLDER target", async () => {
			...
			const result = await commandLineAutoApprover.isCommandAutoApproved(
				"git status",
				shell,
				os,
				undefined,
			...
			ok(
				isAutoApproveRule(result.rule),
				"result should have an auto-approve rule",
			);
			strictEqual(
				result.rule.sourceTarget,
				ConfigurationTarget.WORKSPACE_FOLDER,
				"workspace-folder-scoped rule should have WORKSPACE_FOLDER source target",
			);
		});
	});
});
