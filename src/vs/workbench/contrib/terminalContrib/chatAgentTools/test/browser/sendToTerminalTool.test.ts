...
} from "../../browser/tools/sendToTerminalTool.js";
import {
	RunInTerminalTool,
	type IActiveTerminalExecution,
...
suite("SendToTerminalTool", () => {
	...
	function createInvocation(
		...
		return {
			parameters: {
				id,
				command,
				...(waitForOutput !== undefined ? { waitForOutput } : {}),
			},
			callId: "test-call",
	...
	function createMockExecution(
		...
		return {
			completionPromise: Promise.resolve({
				output,
			} as ITerminalExecuteStrategyResult),
			instance: {
				sendText: async (
					text: string,
					shouldExecute: boolean,
	...
	test("tool schema requires a UUID id", () => {
		const idProperty = SendToTerminalToolData.inputSchema?.properties?.id as
	...
	test("appends cancel-signal steering when input is Ctrl-C", () => {
		return runWithFakedTimers({}, async () => {
			...
			const value = (result.content[0] as { value: string }).value;
			assert.ok(
				value.includes("cancel signal"),
				"should mention cancel signal",
			);
			assert.ok(
				value.includes("not a signal to end the turn"),
				"should remind the model the turn is not done",
			);
		});
	});
	...
	test("does not append cancel-signal steering for ordinary input", () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.ok(
				!value.includes("cancel signal"),
				"should not mention cancel signal for ordinary input",
			);
		});
	});

	function createPreparationContext(
		id: string,
		command: string,
	...
	test("prepareToolInvocation skips confirmation when answering a question carousel", async () => {
		...
		assert.strictEqual(
			...
			undefined,
			"should skip confirmation when the command matches a carousel answer",
		);
	});
	...
	test("prepareToolInvocation does not skip confirmation when the command does not match a carousel answer", async () => {
		...
		assert.ok(
			prepared.confirmationMessages,
			"should require confirmation when the command does not match a carousel answer",
		);
	});
	...
	test("prepareToolInvocation skips confirmation only for exact matches in multi-question carousels", async () => {
		...
		assert.strictEqual(
			exactMatchPrepared.confirmationMessages,
			undefined,
			"should skip confirmation when the command exactly matches a carousel answer",
		);
		...
		assert.ok(
			mismatchedPrepared.confirmationMessages,
			"should require confirmation when the command does not exactly match any carousel answer",
		);
	});
	...
	test("prepareToolInvocation uses positional matching for identical answers (all defaults)", async () => {
		...
		assert.ok(
			firstMsg.value.includes("package"),
			"first call should show package name question",
		);
		...
		assert.ok(
			secondMsg.value.includes("version"),
			"second call should show version question",
		);
		...
		assert.ok(
			thirdMsg.value.includes("description"),
			"third call should show description question",
		);
	});
	...
	test("prepareToolInvocation shows confirmation in default permission mode", async () => {
		...
		assert.ok(
			prepared.confirmationMessages,
			"should show confirmation in default mode",
		);
	...
	test("prepareToolInvocation skips confirmation in auto-approve mode", async () => {
		...
		assert.strictEqual(
			prepared.confirmationMessages,
			undefined,
			"should skip confirmation in auto-approve mode",
		);
	});
	...
	test("prepareToolInvocation Focus Terminal link does not contain $(terminal)", async () => {
		...
		assert.ok(
			!message.value.includes("$(terminal)"),
			"Focus Terminal link should not contain literal $(terminal)",
		);
		assert.ok(
			message.value.includes("Focus Terminal"),
			"should contain Focus Terminal link text",
		);
	});

	test("tool schema includes waitForOutput parameter", () => {
		const waitForOutputProperty = SendToTerminalToolData.inputSchema?.properties
			?.waitForOutput as { type?: string; description?: string } | undefined;
	...
	test("preserves newlines for heredoc commands and uses bracketed paste mode", () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(mockExecution.sentTexts.length, 1);
			assert.strictEqual(
				mockExecution.sentTexts[0].text,
				heredocCommand,
				"heredoc command should preserve newlines",
			);
			assert.strictEqual(
				mockExecution.sentTexts[0].forceBracketedPasteMode,
				true,
				"multiline commands should use bracketed paste mode",
			);
		});
	});
	...
	test("single-line commands still get normalized", () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				mockExecution.sentTexts[0].text,
				"echo hello",
				"single-line command should be trimmed",
			);
		});
	});
	...
	test("line continuation commands are normalized, not treated as multiline", () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				mockExecution.sentTexts[0].text,
				"echo hello \\   world",
				"line continuation should be normalized to single line",
			);
			assert.strictEqual(
				mockExecution.sentTexts[0].forceBracketedPasteMode,
				undefined,
				"line continuation should not force bracketed paste mode",
			);
		});
	});
});
