...
suite("TerminalOutputCache", () => {
	...
	test("keys include cwd so same command in different cwd does not collide", () => {
		...
		strictEqual(
			cache.lookup(TerminalToolId.RunInTerminal, input("git status", "/a"))
				?.text,
			"A",
		);
		strictEqual(
			cache.lookup(TerminalToolId.RunInTerminal, input("git status", "/b"))
				?.text,
			"B",
		);
	});
	...
	test("Fast class expires after 30s", () => {
		...
		now = 1000 + 31_000;
		strictEqual(
			cache.lookup(TerminalToolId.RunInTerminal, input("git status")),
			undefined,
		);
	});
	...
	test("non-cacheable command does not populate cache", () => {
		...
		strictEqual(
			cache.lookup(TerminalToolId.RunInTerminal, input("rm -rf node_modules")),
			undefined,
		);
	});
