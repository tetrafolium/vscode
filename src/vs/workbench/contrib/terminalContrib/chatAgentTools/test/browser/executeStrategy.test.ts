...
suite("Execute Strategy - Prompt Detection", () => {
	...
	test("detectsCommonPromptPattern should detect PowerShell prompts", () => {
		...
		strictEqual(
			detectsCommonPromptPattern("PS C:\\Windows\\System32>").detected,
			true,
		);
		strictEqual(
			detectsCommonPromptPattern("PS C:\\Users\\test> ").detected,
			true,
		);
	});
	...
	test("detectsCommonPromptPattern should detect Command Prompt", () => {
		...
		strictEqual(
			detectsCommonPromptPattern("C:\\Windows\\System32>").detected,
			true,
		);
	...
	test("detectsCommonPromptPattern should detect starship prompts", () => {
		strictEqual(detectsCommonPromptPattern("~ \u276f ").detected, true);
		strictEqual(
			detectsCommonPromptPattern("/path/to/project \u276f").detected,
			true,
		);
	});
...
user@host:~$ `;
	...
	test("detectsCommonPromptPattern should reject non-prompt content", () => {
		...
		strictEqual(
			detectsCommonPromptPattern("error: command not found").detected,
			false,
		);
