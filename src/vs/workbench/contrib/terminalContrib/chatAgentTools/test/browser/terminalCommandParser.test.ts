...
suite("terminalCommandParser", () => {
	...
	suite("tokenize", () => {
		test("splits on whitespace", () => {
			deepStrictEqual(tokenize("git diff HEAD~1 src/foo.ts"), [
				"git",
				"diff",
				"HEAD~1",
				"src/foo.ts",
	...
	suite("parseCommand composition", () => {
		...
		test("strips leading env assignments", () => {
			...
			deepStrictEqual(parsed?.segments[0].envPrefixes, [
				"CI=1",
				"NODE_ENV=test",
			]);
		...
		test("strips sudo wrapper", () => {
			...
			deepStrictEqual(parsed?.segments[0].tokens, [
				"apt-get",
				"install",
				"-y",
				"vim",
	...
	suite("parseCommandHead", () => {
		...
		test("strips env / wrappers", () => {
			deepStrictEqual(parseCommandHead("CI=1 sudo time git status"), {
				head: "git",
				sub: "status",
			});
		});
