...
suite("NoneExecuteStrategy", () => {
	 ...
	 */
	function createMockTerminalAndXterm(
		contentsAsText: string,
	...
	test('should report "Command produced no output" when output is empty', () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const strategy = store.add(
				new NoneExecuteStrategy(
					instance,
					() => false,
					configService,
					logService,
	...
	test("should not leak sandbox command echo as output when command produces no output", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sandboxCommandEcho =
				'ELECTRON_RUN_AS_NODE=1 PATH="$PATH:/app/node_modules/@vscode/ripgrep/bin" ' +
			...
			assert.strictEqual(
				result.output?.includes("sandbox-runtime") ?? false,
				false,
				"Output should not leak sandbox-runtime path",
			);
			assert.strictEqual(
				result.output?.includes("ELECTRON_RUN_AS_NODE") ?? false,
				false,
				"Output should not leak ELECTRON_RUN_AS_NODE",
			);
