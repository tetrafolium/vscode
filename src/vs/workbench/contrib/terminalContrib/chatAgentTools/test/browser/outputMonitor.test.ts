...
import {
	...
	OutputMonitor,
} from "../../browser/tools/monitoring/outputMonitor.js";
...
import {
	...
	IPollingResult,
	OutputMonitorState,
} from "../../browser/tools/monitoring/types.js";
...
suite("OutputMonitor", () => {
	...
	test("press any key fires onDidDetectInputNeeded and stops polling", async () => {
		return runWithFakedTimers({}, async () => {
			...
			store.add(
				monitor.onDidDetectInputNeeded(() => {
					inputNeededFired = true;
				}),
			);
			...
			assert.strictEqual(
				inputNeededFired,
				true,
				"onDidDetectInputNeeded should fire for press any key",
			);
			assert.strictEqual(
				sendTextCalled,
				false,
	...
	test("onDidDetectInputNeeded fires for input-required patterns in foreground mode", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				true,
				"onDidDetectInputNeeded should fire for input-required pattern",
			);
			assert.strictEqual(pollingResult?.state, OutputMonitorState.Idle);
			assert.strictEqual(
			...
			assert.strictEqual(
				sendTextCalled,
				false,
				"no elicitation or auto-reply should send text",
			);
		});
	});
	...
	test("onDidDetectInputNeeded fires for newline-terminated input-required patterns in foreground mode", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				true,
				"onDidDetectInputNeeded should fire for newline-terminated input-required pattern",
			);
			assert.strictEqual(pollingResult?.state, OutputMonitorState.Idle);
			assert.strictEqual(
				pollingResult?.output,
	...
	test("onDidDetectInputNeeded does not fire for non-input output", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				false,
				"onDidDetectInputNeeded should not fire for non-input output",
			);
		});
	});
	...
	test("onDidDetectInputNeeded does NOT fire for log lines ending in colon-space (regression for #315476 Family 1)", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				false,
				'log-shaped output ending in ": " must not raise the input-needed wrapper',
			);
		});
	});
	...
	test("onDidDetectInputNeeded does NOT fire for oh-my-zsh git-aware shell prompts (regression for #315476 Family 2)", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				false,
				"oh-my-zsh shell prompts must not raise the input-needed wrapper",
			);
		});
	});
	...
	test("onDidDetectInputNeeded DOES fire for broad-pattern lines when execution is active (issue #315476 review feedback)", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				true,
				"broad pattern under isActive===true must fire onDidDetectInputNeeded",
			);
		});
	});
	...
	test("onDidDetectInputNeeded does NOT fire if user types during the isActive() await (TOCTOU regression)", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				false,
				"must re-check user-input guard after the isActive() await; firing here would re-pause the agent on input the user already provided",
			);
			...
			assert.ok(
				isActiveCalls >= 1,
				`expected isActive() to be called from the broad-pattern branch (got ${isActiveCalls})`,
			);
		});
	});
	...
	test("sensitive prompt fires onDidDetectSensitiveInputNeeded and not onDidDetectInputNeeded", async () => {
		return runWithFakedTimers({}, async () => {
			...
			store.add(
				monitor.onDidDetectSensitiveInputNeeded(() => {
					sensitiveFired = true;
				}),
			);
			...
			assert.strictEqual(
				sensitiveFired,
				true,
				"onDidDetectSensitiveInputNeeded should fire for sensitive prompts",
			);
			assert.strictEqual(
				inputNeededFired,
				false,
				"onDidDetectInputNeeded must not fire for sensitive prompts so the secret is not routed to the agent",
			);
		});
	});
		...
		return runWithFakedTimers({}, async () => {
			...
			monitor = store.add(
				instantiationService.createInstance(
					...
					createTestContext("1"),
					cts.token,
					"ssh host && echo hunter2 | sudo -S systemctl restart myservice",
				),
			);
			...
			assert.strictEqual(
				inputNeededFired,
				true,
				"sudo -S prompts should be treated as normal input-needed so the non-interactive flow can continue",
			);
			assert.strictEqual(
				sensitiveFired,
				false,
				"sudo -S prompts should not be treated as sensitive interactive prompts",
			);
		});
	});
	...
	test("plain sudo password prompt still fires onDidDetectSensitiveInputNeeded", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				sensitiveFired,
				true,
				"interactive sudo prompts should still be treated as sensitive",
			);
			assert.strictEqual(
				inputNeededFired,
				false,
				"interactive sudo prompts must not be routed to the agent",
			);
		});
	});
	...
	test("detectsSensitiveInputPrompt matches common secret prompts", () => {
		...
		assert.strictEqual(
			detectsSensitiveInputPrompt("[sudo] password for jdoe: "),
			true,
		);
		assert.strictEqual(
			detectsSensitiveInputPrompt(
				"Passphrase for key /Users/foo/.ssh/id_rsa: ",
			),
			true,
		);
		assert.strictEqual(
			detectsSensitiveInputPrompt("Enter your API key: "),
			true,
		);
		assert.strictEqual(detectsSensitiveInputPrompt("Token: "), true);
		assert.strictEqual(
			detectsSensitiveInputPrompt("Verification code: "),
			true,
		);
		...
		assert.strictEqual(
			detectsSensitiveInputPrompt("Enter your 2FA code: "),
			true,
		);
		...
		assert.strictEqual(
			detectsSensitiveInputPrompt("Press any key to continue..."),
			false,
		);
		assert.strictEqual(detectsSensitiveInputPrompt("Enter your name: "), false);
		assert.strictEqual(
			detectsSensitiveInputPrompt("package name: (test_npm_init) "),
			false,
		);
	});
	...
	test("extended timeout with isActive fires onDidDetectInputNeeded", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				inputNeededFired,
				true,
				"onDidDetectInputNeeded should fire on extended timeout with active process",
			);
			assert.strictEqual(
				monitor.pollingResult?.state,
				OutputMonitorState.Cancelled,
			);
		});
	});
	...
	test("non-interactive help on the last line stops monitoring before custom polling", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				customPollCalled,
				false,
				"custom poller should not run when help text is on the last line",
			);
			assert.strictEqual(pollingResult?.state, OutputMonitorState.Idle);
			assert.strictEqual(
				pollingResult?.output,
	...
	test("non-interactive help on a non-final line does not stop custom polling", async () => {
		return runWithFakedTimers({}, async () => {
			...
			assert.strictEqual(
				customPollCalled,
				true,
				"custom poller should still run when help text is not on the last line",
			);
	...
	suite("getLastLine", () => {
		...
		test("preserves the final visual line across bare carriage returns", () => {
			assert.strictEqual(
				getLastLine("Downloading package metadata\r"),
	...
	suite("detectsInputRequiredPattern", () => {
		test("detects yes/no confirmation prompts (pairs and variants)", () => {
			...
			assert.strictEqual(detectsInputRequiredPattern("Continue? (y/n) "), true);
			assert.strictEqual(
				detectsInputRequiredPattern("Overwrite file? [Y/n] "),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Are you sure? (Y/N) "),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Delete files? [y/N] "),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Proceed? (yes/no) "),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Proceed? [no/yes] "),
				true,
			);
			assert.strictEqual(detectsInputRequiredPattern("Continue? y/n "), true);
			assert.strictEqual(
				detectsInputRequiredPattern("Overwrite: yes/no "),
				true,
			);

			// No match if there's a response already
			assert.strictEqual(
				detectsInputRequiredPattern("Continue? (y/N) y"),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Continue? (y/n) n"),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Overwrite file? [Y/n] N"),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Are you sure? (Y/N) Y"),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Delete files? [y/N] y"),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Continue? y/n y\/n"),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("Overwrite: yes/no yes\/n"),
				false,
			);
		});
		...
		test("PowerShell regex does not cause catastrophic backtracking (ReDoS)", () => {
			...
			assert.ok(
				elapsed < 500,
				`Regex took ${elapsed}ms on pathological input, expected < 500ms`,
			);
		});
		test("Line ends with colon (strict path)", () => {
			...
			assert.strictEqual(
				detectsInputRequiredPattern("Enter your name: "),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("File to overwrite: "),
				false,
			);
			...
			assert.strictEqual(
				detectsInputRequiredPattern("[INFO] Starting: "),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("find: /tmp/x: No such file: "),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("error: subprocess returned: "),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("2025-05-09 12:34:56 INFO Starting: "),
				false,
			);
		});
		...
		test("detects prompts with parenthesized default values", () => {
			assert.strictEqual(
				detectsInputRequiredPattern("package name: (test) "),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("version: (1.0.0) "),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("entry point: (index.js) "),
				true,
			);
		...
		test("does NOT match git-aware shell prompts (regression for #315476)", () => {
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("➜  myrepo git:(main) "),
				false,
			);
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("➜  vscode git:(ba-work/output-monitor) "),
				false,
			);
			// bash with __git_ps1 in PROMPT_COMMAND uses a similar shape.
			assert.strictEqual(
				detectsInputRequiredPattern("[user@host ~/myrepo (main)]$ "),
				false,
			);
			// And the looser variant:
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("➜  myrepo git:(main) "),
				false,
			);
		});
		...
		test("detects chevron prompts from prompts/enquirer/inquirer libraries", () => {
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("? Do you want to install jsdom? ›"),
				true,
			);
			// allow-any-unicode-next-line
			assert.strictEqual(
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("? Pick a color ❯ "),
				true,
			);
			// allow-any-unicode-next-line
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("? Project name ▸ "),
				true,
			);
			// allow-any-unicode-next-line
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("? Do you want to install jsdom? › yes"),
				false,
			);
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("? Pick a color ❯ red"),
				false,
			);
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("  feature/foo ❯ main"),
				false,
			);
			// allow-any-unicode-next-line
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("What happened? ›"),
				false,
			);
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsInputRequiredPattern("\x1b[32m? Choose a framework \x1b[0m›"),
				true,
			);
		});
		...
		test("detects press any key prompts", () => {
			assert.strictEqual(
				detectsInputRequiredPattern("Press any key to continue..."),
				true,
			);
		...
		test("detects non-interactive help prompts without treating them as input", () => {
			assert.strictEqual(
				detectsInputRequiredPattern("press h + enter to show help"),
				false,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("press h to show help"),
				false,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press h + enter to show help"),
				true,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press h to show help"),
				true,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press h to show commands"),
				true,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press ? to see commands"),
				true,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press ? + enter for options"),
				true,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("type h + enter to show help"),
				true,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("hit ? for help"),
				true,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("type h to see options"),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("press o to open the app"),
				false,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press o to open the app"),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("press r to restart the server"),
				false,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press r to restart the server"),
				true,
			);
			assert.strictEqual(detectsInputRequiredPattern("press q to quit"), false);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press q to quit"),
				true,
			);
			assert.strictEqual(
				detectsInputRequiredPattern("press u to show server url"),
				false,
			);
			assert.strictEqual(
				detectsNonInteractiveHelpPattern("press u to show server url"),
				true,
			);
		});
		test("delegates to detectsHighConfidenceInputPattern (strict-set parity)", () => {
			...
			for (const line of strictSet) {
				assert.strictEqual(
					detectsHighConfidenceInputPattern(line),
					true,
					`precondition: ${JSON.stringify(line)} should match high-confidence`,
				);
				assert.strictEqual(
					detectsInputRequiredPattern(line),
					true,
					`delegation: ${JSON.stringify(line)} should match strict`,
				);
			}
		});
	});
	...
	suite("detectsLikelyInputRequiredPattern", () => {
		...
		test("matches the same prompts the strict path matches", () => {
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("Continue? (y/N) "),
				true,
			);
			assert.strictEqual(detectsLikelyInputRequiredPattern("Password: "), true);
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("package name: (test) "),
				true,
			);
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("Press any key to continue..."),
				true,
			);
		});
		test("matches bare colon and question-mark prompts the strict path skips", () => {
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("Enter your name: "),
				true,
			);
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("File to overwrite: "),
				true,
			);
			assert.strictEqual(detectsLikelyInputRequiredPattern("Continue? "), true);
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("Are you sure? "),
				true,
			);
		});
		test("still rejects clear non-prompt shapes", () => {
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("Running tests:"),
				false,
			);
		...
		test("documents known false positives on log-shaped output (caller-side guard required)", () => {
			...
			// `isActive === true`.
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("Last Command: "),
				true,
			);
			assert.strictEqual(
				detectsLikelyInputRequiredPattern("[INFO] Starting: "),
				true,
			);
		});
	});
	...
	suite("detectsHighConfidenceInputPattern", () => {
		test("matches y/n and PowerShell prompts", () => {
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Continue? (y/N) "),
				true,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Overwrite file? [Y/n] "),
				true,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("[Y] Yes  [N] No "),
				true,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern(
		...
		test("matches password and press-any-key prompts", () => {
			...
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Enter your password"),
				false,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Press any key to continue..."),
				true,
			);
		});
		test("matches parenthesized defaults", () => {
			...
			assert.strictEqual(
				detectsHighConfidenceInputPattern("package name: (test) "),
				true,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("version: (1.0.0) "),
				true,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("description: (a thing) "),
				true,
			);
			// Extra whitespace between `:` and `(` still matches (multiple spaces, tab).
			assert.strictEqual(
				detectsHighConfidenceInputPattern("license:  (MIT) "),
				true,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("author:\t(none) "),
				true,
			);
			// Multi-word default in the parens still matches.
			assert.strictEqual(
				detectsHighConfidenceInputPattern("repository: (github.com/foo/bar) "),
				true,
			);
		});
		test("does NOT match git-aware shell prompts (regression: oh-my-zsh, bash __git_ps1)", () => {
			...
			// allow-any-unicode-next-line
			assert.strictEqual(
				detectsHighConfidenceInputPattern("➜  myrepo git:(main) "),
				false,
			);
			// allow-any-unicode-next-line
			assert.strictEqual(
			...
			assert.strictEqual(
				detectsHighConfidenceInputPattern("[user@host ~/myrepo (main)]$ "),
				false,
			);
		});
		...
		test("does NOT match bare colon prompts (too broad for fast-path)", () => {
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Enter your name: "),
				false,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("File to overwrite: "),
				false,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Building project: "),
				false,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Running tests:"),
				false,
			);
		});
		test("does NOT match bare question prompts (too broad for fast-path)", () => {
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Continue? "),
				false,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("Are you sure? "),
				false,
			);
			assert.strictEqual(
				detectsHighConfidenceInputPattern("What happened?"),
				false,
			);
		});
	});
	...
	suite("matchTerminalPromptOption", () => {
		test("matches suggested option case-insensitively", () => {
			assert.deepStrictEqual(matchTerminalPromptOption(["Y", "n"], "y"), {
				option: "Y",
				index: 0,
			});
			assert.deepStrictEqual(matchTerminalPromptOption(["y", "N"], "n"), {
				option: "N",
				index: 1,
			});
		});
		...
		test("strips quotes and trailing punctuation", () => {
			...
			assert.deepStrictEqual(matchTerminalPromptOption(["yes", "no"], "no."), {
				option: "no",
				index: 1,
			});
		});
		...
		test("handles default suffixes by using first token", () => {
			assert.deepStrictEqual(
				matchTerminalPromptOption(["Y", "n"], "Y (default)"),
				{ option: "Y", index: 0 },
			);
			assert.deepStrictEqual(
				matchTerminalPromptOption(["Enter"], "Enter to continue"),
				{ option: "Enter", index: 0 },
			);
		});
	});
	...
	suite("detectsVSCodeTaskFinishMessage", () => {
		test("detects VS Code task completion messages", () => {
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage("Press any key to close the terminal."),
				true,
			);
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage(
			...
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage(
					"The terminal will be reused by tasks. Press any key to close. Please provide the required input to the terminal.",
				),
				true,
			);
			// Case insensitive
		...
		test("does not match generic press any key messages", () => {
			...
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage("Press any key to continue..."),
				false,
			);
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage("Press any key to exit"),
				false,
			);
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage("Press any key"),
				false,
			);
		});
		...
		test("does not match other prompts", () => {
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage("Continue? (y/n)"),
				false,
			);
			assert.strictEqual(detectsVSCodeTaskFinishMessage("Password:"), false);
			assert.strictEqual(
				detectsVSCodeTaskFinishMessage("press h to show help"),
				false,
			);
		});
	});
	...
	suite("disposable leak regression", () => {
		...
		test("disposing while monitoring is in-flight still resolves onDidFinishCommand", async () => {
			...
			return runWithFakedTimers({}, async () => {
				...
				await finished;
				assert.ok(
					monitor.pollingResult,
					"pollingResult should be defined after dispose-induced finish",
				);
				assert.strictEqual(
					monitor.pollingResult!.state,
					OutputMonitorState.Cancelled,
	...
	suite("detectsGenericPressAnyKeyPattern", () => {
		test("detects generic press any key prompts from scripts", () => {
			assert.strictEqual(
				detectsGenericPressAnyKeyPattern("Press any key to continue..."),
				true,
			);
			assert.strictEqual(
				detectsGenericPressAnyKeyPattern("Press any key to exit"),
				true,
			);
			assert.strictEqual(
				detectsGenericPressAnyKeyPattern("Press any key"),
				true,
			);
			assert.strictEqual(
				detectsGenericPressAnyKeyPattern("press a key to continue"),
				true,
			);
			// Case insensitive
			assert.strictEqual(
		...
		test("does not match other prompts", () => {
			assert.strictEqual(
				detectsGenericPressAnyKeyPattern("Continue? (y/n)"),
				false,
			);
			assert.strictEqual(detectsGenericPressAnyKeyPattern("Password:"), false);
			assert.strictEqual(
				detectsGenericPressAnyKeyPattern("press h to show help"),
				false,
			);
		});
	});
