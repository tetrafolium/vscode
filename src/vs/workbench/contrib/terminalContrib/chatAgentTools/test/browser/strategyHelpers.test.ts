...
suite("stripCommandEchoAndPrompt", () => {
	...
	test("strips sandbox-wrapped command echo (long wrapped lines)", () => {
		const sandboxCommand =
			'ELECTRON_RUN_AS_NODE=1 PATH="$PATH:/app/rg/bin" TMPDIR="/tmp/sandbox" "/app/sandbox-runtime/dist/cli.js" --settings "/tmp/sandbox-settings.json" -c \'curl -s https://example.com\'';
		const output = [
	...
	test("strips trailing prompt with various prompt styles", () => {
		...
		assert.strictEqual(
			stripCommandEchoAndPrompt(
				[
					"root@server:/var/log# echo hello",
					"hello",
					"root@server:/var/log# ",
				].join("\n"),
				"echo hello",
	...
	test("handles output with no trailing prompt (e.g. command still running)", () => {
		const output = ["user@host:~ $ echo hello", "hello"].join("\n");
	...
	test("strips partial command echo (suffix from wrapped getOutput)", () => {
		...
		assert.strictEqual(
			stripCommandEchoAndPrompt(
				output,
				"echo M1_1774133190741 ; echo M2_1774133190741 ; echo M3_1774133190741",
			),
			"M1_1774133190741\nM2_1774133190741\nM3_1774133190741",
		);
	});
