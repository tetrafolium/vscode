...
} from "../../browser/tools/getTerminalOutputTool.js";
import {
	RunInTerminalTool,
...
suite("GetTerminalOutputTool", () => {
	...
	setup(() => {
		...
		tool = store.add(
			instantiationService.createInstance(GetTerminalOutputTool),
		);
	...
	function createMockExecution(
		output: string,
		instanceId = KNOWN_TERMINAL_INSTANCE_ID,
	...
	function createMutableMockExecution(
		output: string,
		instanceId = KNOWN_TERMINAL_INSTANCE_ID,
	): IActiveTerminalExecution & { setOutput(value: string): void } {
	...
	test("tool schema requires a UUID id", () => {
		const idProperty = GetTerminalOutputToolData.inputSchema?.properties?.id as
	...
	test("returns unchanged marker for repeated output when output deltas experiment is enabled", async () => {
		...
		assert.strictEqual(
			value,
			`Output of terminal ${KNOWN_TERMINAL_ID} unchanged since previous poll (11 characters already shown). No new output.`,
		);
	});
	...
	test("returns only new output when output deltas experiment is enabled", async () => {
		...
		assert.ok(
			value.includes(
				`Output of terminal ${KNOWN_TERMINAL_ID} since previous poll`,
			),
		);
	...
	test("clears output snapshot when terminal instance is disposed", async () => {
		...
		assert.strictEqual(
			value,
			`Output of terminal ${KNOWN_TERMINAL_ID}:\nline1\nline2`,
		);
	});
