...
import {
	...
	type ITerminalSandboxPrecheckInputs,
...
class TestRunInTerminalTool extends RunInTerminalTool {
	protected override _osBackend: Promise<OperatingSystem> = Promise.resolve(
	...
	get commandLinePresenters(): ICommandLinePresenter[] {
		return (this as unknown as Record<string, ICommandLinePresenter[]>)[
			"_commandLinePresenters"
		];
	}
...
suite("RunInTerminalTool", () => {
	...
	let chatServiceDisposeEmitter: Emitter<{
	...
	function isDefaultChatPermissionSandboxPrecheckInputs(
		precheckInputs: ITerminalSandboxPrecheckInputs | undefined,
	): boolean {
	...
	setup(() => {
		...
		const chatServiceStub = {
			...
			getSession: (sessionResource: URI) =>
				chatSessions.get(sessionResource.toString()),
			...
			acquireExistingSession: () =>
				({
					...
					ReturnType<IChatService["acquireExistingSession"]>
				>,
		...
		instantiationService.stub(
			ITerminalChatService,
			store.add(instantiationService.createInstance(TerminalChatService)),
		);
		instantiationService.stub(
			IWorkspaceContextService,
		...
		terminalSandboxService = {
			...
			isEnabled: async (precheckInputs) =>
				sandboxEnabled &&
				isDefaultChatPermissionSandboxPrecheckInputs(precheckInputs),
			isSandboxAllowNetworkEnabled: async () => false,
			wrapCommand: async (
				command: string,
				requestUnsandboxedExecution?: boolean,
			) => ({
				command: requestUnsandboxedExecution
					? `unsandboxed:${command}`
					: `sandbox:${command}`,
				isSandboxWrapped: !requestUnsandboxedExecution,
			}),
			getSandboxConfigPath: async () =>
				sandboxEnabled ? "/tmp/sandbox.json" : undefined,
			checkForSandboxingPrereqs: async (
				_forceRefresh?: boolean,
				precheckInputs?: ITerminalSandboxPrecheckInputs,
			) =>
				isDefaultChatPermissionSandboxPrecheckInputs(precheckInputs)
					? sandboxPrereqResult
					: {
			...
			installMissingSandboxDependencies: async (
				missingDependencies,
				_sessionResource,
				_token,
				options,
				...
				await terminal.sendText(
					`sudo apt install -y ${missingDependencies.join(" ")}`,
					true,
				);
				return { exitCode: 0 };
			},
	...
	function setAutoApprove(value: {
		[key: string]: { approve: boolean; matchCommandLine?: boolean } | boolean;
	}) {
	...
	async function executeToolTest(
		...
		const result = await runInTerminalTool.prepareToolInvocation(
			context,
	...
	async function invokeToolTest(
		...
		const preparedInvocation = await runInTerminalTool.prepareToolInvocation(
			{ parameters } as IToolInvocationPreparationContext,
			CancellationToken.None,
		);
		ok(
			preparedInvocation?.toolSpecificData,
			"Expected toolSpecificData to be defined",
		);
	...
	function assertAutoApproved(
		...
		ok(
			...
			"Expected no confirmation messages for auto-approved command",
		);
	}
	...
	function assertConfirmationRequired(
		...
		ok(
			preparedInvocation.confirmationMessages,
			"Expected confirmation messages for non-approved command",
		);
		if (expectedTitle) {
			strictEqual(
	...
	function createChatModelWithRequest(
		sessionResource: URI,
		modeInfo?: IChatRequestModeInfo,
		requestId?: string,
	): ChatModel {
		const model = store.add(
			instantiationService.createInstance(ChatModel, undefined, {
	...
	function confirmAutomaticUnsandboxRetry(
		tool: RunInTerminalTool,
		...
		return (
			tool as unknown as Record<
				...
				(
					...
					shell: string,
					blockedDomains: string[] | undefined,
					riskAssessment: { toolId: string; parameters: unknown } | undefined,
					token: CancellationToken,
				) => Promise<boolean>
			>
		)["_confirmAutomaticUnsandboxRetry"](
			sessionResource,
			command,
			shell,
			blockedDomains,
	...
	async function assertAutomaticUnsandboxRetryElicitation(
		tool: RunInTerminalTool,
		sessionResource: URI,
		command: string,
		shell: string,
		...
		const shouldRetry = confirmAutomaticUnsandboxRetry(
			tool,
			sessionResource,
			command,
			shell,
		...
		const elicitation = response.response.value.find(
			(part) => part.kind === "elicitation2",
		);
		ok(
			elicitation?.kind === "elicitation2",
			"Expected automatic unsandbox retry elicitation",
		);
		const reject = elicitation.reject;
		ok(
			reject,
			"Expected automatic unsandbox retry elicitation to have a reject action",
		);
	...
	function getAutomaticUnsandboxRetryTitle(
		tool: RunInTerminalTool,
		shellType: string,
		blockedDomains: string[] | undefined,
	): IMarkdownString {
	...
	suite("sandbox invocation messaging", () => {
		test("should instruct models to use $TMPDIR instead of /tmp when sandboxed", async () => {
			...
			const toolData = await instantiationService.invokeFunction(
				createRunInTerminalToolData,
			);

			ok(
				toolData.modelDescription?.includes(
					"must utilize the $TMPDIR environment variable",
				),
				"Expected sandboxed tool description to require $TMPDIR usage",
			);
			ok(
				toolData.modelDescription?.includes(
					"The /tmp directory is not guaranteed to be accessible or writable and must be avoided",
				),
				"Expected sandboxed tool description to discourage /tmp usage",
			);
		});
		...
		test("should include requestUnsandboxedExecution in schema when sandbox is enabled", async () => {
			...
			const properties = toolData.inputSchema?.properties as
				| Record<string, object>
				| undefined;
			const allowToRunUnsandboxedCommandsProperty = properties?.[
				"allowToRunUnsandboxedCommands"
			] as
				| { const?: boolean; default?: boolean; description?: string }
				| undefined;
			const requestUnsandboxedExecutionProperty = properties?.[
				"requestUnsandboxedExecution"
			] as { description?: string } | undefined;
			const requestUnsandboxedExecutionReasonProperty = properties?.[
				"requestUnsandboxedExecutionReason"
			] as { description?: string } | undefined;

			ok(
				properties?.["allowToRunUnsandboxedCommands"],
				"Expected allowToRunUnsandboxedCommands in schema when sandbox is enabled",
			);
			strictEqual(
				allowToRunUnsandboxedCommandsProperty?.const,
				true,
				"Expected allowToRunUnsandboxedCommands const to match the setting value",
			);
			strictEqual(
				allowToRunUnsandboxedCommandsProperty?.default,
				true,
				"Expected allowToRunUnsandboxedCommands to default to the setting value",
			);
			ok(
				allowToRunUnsandboxedCommandsProperty?.description?.includes(
					"chat.agent.sandbox.allowUnsandboxedCommands",
				),
				"Expected allowToRunUnsandboxedCommands description to mention the source setting",
			);
			ok(
				properties?.["requestUnsandboxedExecution"],
				"Expected requestUnsandboxedExecution in schema when sandbox is enabled",
			);
			ok(
				properties?.["requestUnsandboxedExecutionReason"],
				"Expected requestUnsandboxedExecutionReason in schema when sandbox is enabled",
			);
			ok(
				requestUnsandboxedExecutionProperty?.description?.includes(
					"Only set this when the command clearly needs unsandboxed access",
				),
				"Expected schema description to require a clear need for unsandboxed access",
			);
			ok(
				requestUnsandboxedExecutionReasonProperty?.description?.includes(
					"why this command must run outside the terminal sandbox",
				),
				"Expected reason schema description to require concrete sandbox justification",
			);
		});
		...
		test("should set allowToRunUnsandboxedCommands from setting in schema when unsandboxed commands are disabled", async () => {
			...
			const allowToRunUnsandboxedCommandsProperty = properties?.[
				"allowToRunUnsandboxedCommands"
			] as { const?: boolean; default?: boolean } | undefined;
			...
			strictEqual(
				allowToRunUnsandboxedCommandsProperty?.const,
				false,
				"Expected allowToRunUnsandboxedCommands const to reflect the disabled setting",
			);
			strictEqual(
				allowToRunUnsandboxedCommandsProperty?.default,
				false,
				"Expected allowToRunUnsandboxedCommands to reflect the disabled setting",
			);
			ok(
				properties?.["requestUnsandboxedExecution"],
				"Expected requestUnsandboxedExecution to remain in schema when sandbox is enabled",
			);
			ok(
				properties?.["requestUnsandboxedExecutionReason"],
				"Expected requestUnsandboxedExecutionReason to remain in schema when sandbox is enabled",
			);
			ok(
				toolData.modelDescription?.includes(
					"Running commands outside the sandbox is disabled",
				),
				"Expected model description to explain that unsandboxed commands are disabled",
			);
		});
		...
		test("should not include requestUnsandboxedExecution in schema when sandbox is disabled", async () => {
			...
			ok(
				!properties?.["allowToRunUnsandboxedCommands"],
				"Expected no allowToRunUnsandboxedCommands when sandbox is disabled",
			);
			ok(
				!properties?.["requestUnsandboxedExecution"],
				"Expected no requestUnsandboxedExecution in schema when sandbox is disabled",
			);
			ok(
				!properties?.["requestUnsandboxedExecutionReason"],
				"Expected no requestUnsandboxedExecutionReason in schema when sandbox is disabled",
			);
		});
		...
		test("should reflect sandbox setting changes in tool data", async () => {
			...
			const toolDataBefore = await instantiationService.invokeFunction(
				createRunInTerminalToolData,
			);
			const propertiesBefore = toolDataBefore.inputSchema?.properties as
				| Record<string, object>
				| undefined;
			ok(
				!propertiesBefore?.["requestUnsandboxedExecution"],
				"Expected no requestUnsandboxedExecution before enabling sandbox",
			);
			...
			const toolDataAfter = await instantiationService.invokeFunction(
				createRunInTerminalToolData,
			);
			const propertiesAfter = toolDataAfter.inputSchema?.properties as
				| Record<string, object>
				| undefined;
			const allowToRunUnsandboxedCommandsProperty = propertiesAfter?.[
				"allowToRunUnsandboxedCommands"
			] as { const?: boolean; default?: boolean } | undefined;
			ok(
				propertiesAfter?.["allowToRunUnsandboxedCommands"],
				"Expected allowToRunUnsandboxedCommands after enabling sandbox",
			);
			strictEqual(
				allowToRunUnsandboxedCommandsProperty?.const,
				true,
				"Expected allowToRunUnsandboxedCommands const to reflect the setting after enabling sandbox",
			);
			strictEqual(
				allowToRunUnsandboxedCommandsProperty?.default,
				true,
				"Expected allowToRunUnsandboxedCommands to reflect the setting after enabling sandbox",
			);
			ok(
				propertiesAfter?.["requestUnsandboxedExecution"],
				"Expected requestUnsandboxedExecution after enabling sandbox",
			);
			ok(
				toolDataAfter.modelDescription?.includes("Sandboxing:"),
				"Expected sandbox instructions in description after enabling sandbox",
			);
		});
		...
		test("should show confirmation to install missing sandbox dependencies when prereq check fails", async () => {
			...
			ok(
				...
				"Expected confirmationMessages when deps are missing",
			);
			ok(
				result?.confirmationMessages?.customOptions?.length === 2,
				"Expected two custom options",
			);
			// missingDependencies should be in toolSpecificData so invoke can handle it
			strictEqual(
				(
					result?.toolSpecificData as
						| IChatTerminalToolInvocationData
						| undefined
				)?.missingSandboxDependencies?.length,
				1,
			);
		});
		...
		test("should show repair choices when bubblewrap is installed but unusable on Linux", async () => {
			...
			const terminalData = result?.toolSpecificData as
				| IChatTerminalToolInvocationData
				| undefined;

			ok(
				result?.confirmationMessages,
				"Expected confirmation messages for bubblewrap repair",
			);
			strictEqual(
				result?.confirmationMessages?.customOptions?.length,
				3,
				"Expected recommended repair, fallback, and cancel choices",
			);
			strictEqual(
				terminalData?.sandboxRemediations?.length,
				2,
				"Expected repair options in terminal invocation data",
			);
			strictEqual(
				terminalData?.missingSandboxDependencies,
				undefined,
				"Should not classify unusable bubblewrap as missing",
			);
		});
		...
		test("should recheck bubblewrap after dependency installation and not execute when it remains unavailable", async () => {
			...
			strictEqual(
				forceRefreshCalled,
				true,
				"Expected dependency installation to force a new prerequisite check",
			);
			strictEqual(
				createTerminalCallCount,
				1,
				"Expected only the installation terminal, not original command execution",
			);
			ok(
				(result.content[0] as { value?: string }).value?.includes("bubblewrap"),
				"Expected result to identify the failed bubblewrap verification",
			);
		});
		...
		test("should not execute when bubblewrap is unusable and no supported remediation is available", async () => {
			...
			strictEqual(
				createTerminalCallCount,
				0,
				"Expected no terminal execution for unusable bubblewrap",
			);
			ok(
				(result.content[0] as { value?: string }).value?.includes("Bubblewrap"),
				"Expected a bubblewrap capability failure message",
			);
		});
		...
		test("should include allowed and denied network domains in model description", async () => {
			...
			ok(
				toolData.modelDescription?.includes("github.com, npmjs.org"),
				"Expected allowed domains in description",
			);
			ok(
				toolData.modelDescription?.includes("evil.com"),
				"Expected denied domains in description",
			);
			ok(
				toolData.modelDescription?.includes(
					"Only set requestUnsandboxedExecution=true when there is evidence of failures caused by the sandbox",
				),
				"Expected model description to require concrete evidence before unsandboxing",
			);
		});
		...
		test("should exclude denied domains from effective allowed list", async () => {
			...
			ok(
				toolData.modelDescription?.includes("github.com, npmjs.org"),
				"Expected effective allowed list without denied domain",
			);
			ok(
				!toolData.modelDescription?.includes(
					"accessible in the sandbox (all other network access is blocked): github.com, evil.com",
				),
				"Expected denied domain removed from allowed list",
			);
		});
		...
		test("should use sandbox labels when command is sandbox wrapped", async () => {
			...
			const preparedInvocation = await executeToolTest({
				command: "echo hello",
			});

			ok(preparedInvocation, "Expected prepared invocation to be defined");
			strictEqual(
				(preparedInvocation.invocationMessage as IMarkdownString).value,
				"Running `echo hello` in sandbox",
			);

			const terminalData =
				preparedInvocation.toolSpecificData as IChatTerminalToolInvocationData;
		...
		test("should enable sandboxing when chat permission level is default", async () => {
			...
			const sessionResource = LocalChatSessionUri.forSession(
				"sandbox-default-permission-session",
			);
			instantiationService.stub(IChatWidgetService, {
				getWidgetBySessionResource: (() => ({
					input: {
						currentModeInfo: { permissionLevel: ChatPermissionLevel.Default },
					},
				})) as unknown as IChatWidgetService["getWidgetBySessionResource"],
				lastFocusedWidget: undefined,
			});
			const defaultPermissionTool = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);

			const preparedInvocation =
				await defaultPermissionTool.prepareToolInvocation(
					{
						parameters: {
							command: "echo hello",
							explanation: "Print hello",
			...
			const terminalData = preparedInvocation!
				.toolSpecificData as IChatTerminalToolInvocationData;
			strictEqual(terminalData.commandLine.isSandboxWrapped, true);
			strictEqual(
				(preparedInvocation!.invocationMessage as IMarkdownString).value,
				"Running `echo hello` in sandbox",
		...
		test("should disable sandboxing when chat permission level is elevated", async () => {
			...
			const originalWrapCommand = terminalSandboxService.wrapCommand.bind(
				terminalSandboxService,
			);
			for (const permissionLevel of [
				...
				const sessionResource = LocalChatSessionUri.forSession(
					`sandbox-${permissionLevel}-permission-session`,
				);
				instantiationService.stub(IChatWidgetService, {
					getWidgetBySessionResource: (() => ({
						input: { currentModeInfo: { permissionLevel } },
					})) as unknown as IChatWidgetService["getWidgetBySessionResource"],
					lastFocusedWidget: undefined,
				});
				const elevatedPermissionTool = store.add(
					instantiationService.createInstance(TestRunInTerminalTool),
				);

				const preparedInvocation =
					await elevatedPermissionTool.prepareToolInvocation(
						{
							parameters: {
								command: "echo hello",
								explanation: "Print hello",
				...
				const terminalData = preparedInvocation!
					.toolSpecificData as IChatTerminalToolInvocationData;
				strictEqual(
					terminalData.commandLine.isSandboxWrapped,
					false,
					`Expected no sandbox wrapping for ${permissionLevel}`,
				);
				strictEqual(
					terminalData.requestUnsandboxedExecution,
					false,
					`Expected no unsandbox confirmation for ${permissionLevel}`,
				);
				strictEqual(
					(preparedInvocation!.invocationMessage as IMarkdownString).value,
					"Running `echo hello`",
				...
				strictEqual(
					wrapCalls,
					0,
					`Expected sandbox wrapping to be skipped for ${permissionLevel}`,
				);
		...
		test("should use request permission level before current widget permission level", async () => {
			...
			const sessionResource = LocalChatSessionUri.forSession(
				"sandbox-request-permission-session",
			);
			const requestId = "sandbox-request-permission-request";
			createChatModelWithRequest(
				sessionResource,
				createChatModeInfo(ChatPermissionLevel.AutoApprove),
				requestId,
			);
			instantiationService.stub(IChatWidgetService, {
				getWidgetBySessionResource: (() => ({
			...
			const requestPermissionTool = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);

			const preparedInvocation =
				await requestPermissionTool.prepareToolInvocation(
					{
						parameters: {
							command: "echo hello",
							explanation: "Print hello",
		...
		test("should not show sandbox wrapper in chat when sandboxed async command is detached", async () => {
			...
			strictEqual(
				...
				"nohup sandbox-runtime echo hello & disown",
			);
		});
	});
	...
	suite("automatic sandbox retry", () => {
		...
		test("should show retry elicitation when prepared invocation was auto-approved", async () => {
			...
			const sessionResource = LocalChatSessionUri.forSession(
				"auto-retry-auto-approved-session",
			);
			...
			await assertAutomaticUnsandboxRetryElicitation(
				runInTerminalTool,
				sessionResource,
				"echo hello",
				"bash",
		...
		test("should auto-retry without elicitation when session is in auto-approve permission level", async () => {
			...
			const autoApproveRunInTerminalTool = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);
			const preparedInvocation =
				await autoApproveRunInTerminalTool.prepareToolInvocation(
					{
						parameters: {
			...
			const model = createChatModelWithRequest(sessionResource);
			const shouldRetry = await confirmAutomaticUnsandboxRetry(
				autoApproveRunInTerminalTool,
				sessionResource,
			...
			strictEqual(
				shouldRetry,
				true,
				"Expected auto-approve session to retry without prompting",
			);
			const elicitation = model
				.getRequests()
				.at(-1)
				?.response?.response.value.find((part) => part.kind === "elicitation2");
		...
		test("should show retry elicitation when prepared invocation required confirmation", async () => {
			...
			await assertAutomaticUnsandboxRetryElicitation(
				...
				LocalChatSessionUri.forSession(
					"auto-retry-confirmation-required-session",
				),
				"rm dangerous-file.txt",
				"bash",
				undefined,
		...
		test("should use retry confirmation title without sandbox link", () => {
			const title = getAutomaticUnsandboxRetryTitle(
				runInTerminalTool,
				"bash",
				undefined,
			);
		...
		test("should use retry confirmation title without sandbox link for blocked domains", () => {
			const title = getAutomaticUnsandboxRetryTitle(runInTerminalTool, "bash", [
				"example.com",
			]);
		...
		test("should show retry elicitation when sandbox force-approved command would otherwise require confirmation", async () => {
			...
			await assertAutomaticUnsandboxRetryElicitation(
				runInTerminalTool,
				LocalChatSessionUri.forSession(
					"auto-retry-sandbox-force-approved-session",
				),
				"rm dangerous-file.txt",
				"bash",
				undefined,
		...
		test("should auto-retry without elicitation when unsandboxed command auto approve is enabled", async () => {
			...
			const sessionResource = LocalChatSessionUri.forSession(
				"auto-retry-unsandboxed-setting-session",
			);
			...
			strictEqual(
				shouldRetry,
				true,
				"Expected unsandboxed auto approve setting to retry without prompting",
			);
			const elicitation = model
				.getRequests()
				.at(-1)
			...
			ok(
				!elicitation,
				"Expected no elicitation when unsandboxed auto approve setting is enabled",
			);
		});
	});
	...
	suite("default auto-approve rules", () => {
		const defaults = terminalChatAgentToolsConfiguration[
			...
			string,
			boolean | { approve: boolean; matchCommandLine?: boolean }
		>;
	...
	suite("retry outside sandbox", () => {
		test("should mention denied domains when sandbox denies network access explicitly", async () => {
			...
			assertConfirmationRequired(
				result,
				"Run `bash` command outside the [sandbox](https://aka.ms/vscode-sandboxing) to access `evil.com`?",
			);
			...
			ok(
				confirmationMessage.value.includes(
					"Reason for leaving the sandbox: This command accesses evil.com, which is blocked by chat.agent.deniedNetworkDomains.",
				),
			);
		});
		...
		test("should force confirmation for explicit unsandboxed execution requests", async () => {
			...
			const result = await executeToolTest({
				requestUnsandboxedExecution: true,
				requestUnsandboxedExecutionReason:
					"Needs network access outside the sandbox",
			});

			assertConfirmationRequired(
				result,
				"Run `bash` command outside the [sandbox](https://aka.ms/vscode-sandboxing)?",
			);
			strictEqual(result?.confirmationMessages?.allowAutoConfirm, undefined);
			const terminalData =
				result?.toolSpecificData as IChatTerminalToolInvocationData;
			strictEqual(terminalData.requestUnsandboxedExecution, true);
			strictEqual(
				terminalData.requestUnsandboxedExecutionReason,
				"Needs network access outside the sandbox",
			);
			strictEqual(
				terminalData.commandLine.toolEdited,
				"unsandboxed:echo hello",
			);
			...
			ok(
				confirmationMessage.value.includes(
					"Reason for leaving the sandbox: Needs network access outside the sandbox",
				),
			);
		...
		test("should reject explicit unsandboxed execution requests when unsandboxed commands are disabled", async () => {
			...
			ok(
				!result.confirmationMessages,
				"Expected no confirmation because the command will not run",
			);
			ok(
				(result.invocationMessage as IMarkdownString).value.includes(
					"Not running `echo hello` because unsandboxed execution is disabled",
				),
			);
			const terminalData =
				result.toolSpecificData as IChatTerminalToolInvocationData;
		...
		test("should not create a terminal for rejected explicit unsandboxed execution requests", async () => {
			...
			strictEqual(
				createTerminalCallCount,
				0,
				"Expected no terminal to be created",
			);
			ok(
				result.toolResultError,
				"Expected the rejected request to be returned as a tool error",
			);
			ok(
				result.content[0].kind === "text" &&
					result.content[0].value.includes("The command was not executed"),
			);
			ok(
				result.content[0].kind === "text" &&
		...
		test("should auto-approve blocked-domain unsandboxed fallback when unsandboxed auto approve is enabled", async () => {
			...
			strictEqual(
				terminalData.commandLine.toolEdited,
				"unsandboxed:curl https://evil.com",
			);
			strictEqual(
				terminalData.requestUnsandboxedExecutionReason,
				"This command accesses evil.com, which is blocked by chat.agent.deniedNetworkDomains.",
			);
		});
	...
	suite("prepareToolInvocation - auto approval behavior", () => {
		...
		test("should include auto-approve info for background commands", async () => {
			...
			ok(
				...
				"Expected autoApproveInfo to be defined for auto-approved background command",
			);
			ok(
				terminalData.autoApproveInfo.value,
				"Expected autoApproveInfo to have a value",
			);
			ok(
				terminalData.autoApproveInfo.value.includes("npm"),
				"Expected autoApproveInfo to mention the approved rule",
			);
		});
		...
		test("should handle matchCommandLine: true patterns", async () => {
			...
			const result2 = await executeToolTest({
				command: "echo this is a dangerous command",
			});
	...
	suite("confirmation title with presentation overrides", () => {
		function injectMockPresenter(
			tool: TestRunInTerminalTool,
		...
		test("should use withoutLanguage inDirectory title when presenter returns no languageDisplayName with cd prefix", async () => {
			const workspaceFolder = URI.file(
				isWindows ? "C:\\workspace\\project" : "/workspace/project",
			);
			...
			const toolWithWorkspace = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);
			...
			const result = await toolWithWorkspace.prepareToolInvocation(
				context,
				CancellationToken.None,
			);
			assertConfirmationRequired(
				result,
				`Run command in \`bash\` within \`${isWindows ? "\\tmp" : "~/tmp"}\`?`,
			);
		});
		...
		test("should not show undefined in confirmation message when explanation and goal are missing", async () => {
			...
			ok(
				...
				`Confirmation message should not contain "undefined", got: ${messageText}`,
			);
		});
		...
		test("should use withLanguage inDirectory title when presenter returns languageDisplayName with cd prefix", async () => {
			...
			assertConfirmationRequired(
				result,
				`Run \`Node.js\` command in \`bash\` within \`${isWindows ? "\\tmp" : "~/tmp"}\`?`,
			);
		});
	});
	...
	suite("prepareToolInvocation - custom actions for dropdown", () => {
		type ActionItemType =
			| {
					subCommand: SingleOrMany<string>;
					scope: "session" | "workspace" | "user";
			  }
			| { commandLine: true; scope: "session" | "workspace" | "user" }
			| "---"
			| "configure"
			| "sessionApproval";

		function assertDropdownActions(
			result: IPreparedToolInvocation | undefined,
			...
			for (const [i, item] of items.entries()) {
				...
				if (item === "---") {
					...
					if (item === "configure") {
						...
						ok(
							!Array.isArray(action.data.rule),
							"Expected rule to be an object",
						);
					} else {
						...
						const expectedLabel =
							item.scope === "session"
								? `Allow ${subCommandLabel} in this Session`
								: item.scope === "workspace"
									? `Allow ${subCommandLabel} in this Workspace`
		...
		test("should only generate configure action for explicitly denied commands", async () => {
			...
			assertConfirmationRequired(result, "Run `bash` command?");
			assertDropdownActions(result, ["sessionApproval", "---", "configure"]);
		});
		...
		test("should prevent auto approval when writing to a file outside the workspace", async () => {
			...
			assertConfirmationRequired(result);
			strictEqual(
				result?.confirmationMessages?.terminalCustomActions,
				undefined,
				"Expected no custom actions when file write is blocked",
			);
		});
	});
	...
	suite("chat session disposal cleanup", () => {
		...
		test("should restore all terminals into the session terminal map and dispose them when archived", () => {
			...
			const restoredRunInTerminalTool = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);
			const restoredSessionTerminals =
				restoredRunInTerminalTool.sessionTerminalInstances.get(sessionResource);
			strictEqual(
				restoredSessionTerminals?.size,
				2,
				"Both restored terminals should be tracked for the session",
			);
			...
			strictEqual(
				terminal1Disposed,
				true,
				"Restored background terminal should have been disposed",
			);
			strictEqual(
				terminal2Disposed,
				true,
				"Restored foreground terminal should have been disposed",
			);
			ok(
				!restoredRunInTerminalTool.sessionTerminalAssociations.has(
					sessionResource,
				),
				"Foreground terminal association should be removed after archive",
			);
			ok(
				!restoredRunInTerminalTool.sessionTerminalInstances.has(
					sessionResource,
				),
				"All restored terminals for the session should be removed after archive",
			);
		});
		...
		test("should dispose all terminals associated with a single chat session when archived", () => {
			...
			runInTerminalTool.sessionTerminalInstances.set(
				sessionResource,
				new Set([mockTerminal1, mockTerminal2]),
			);

			// Initialize lazy archive listener before firing the archive event.
			const ensureArchivedSessionListener = (
				runInTerminalTool as unknown as Record<string, () => void>
			...
			strictEqual(
				terminal1Disposed,
				true,
				"Terminal 1 should have been disposed",
			);
			strictEqual(
				terminal2Disposed,
				true,
				"Terminal 2 should have been disposed",
			);
			ok(
				!runInTerminalTool.sessionTerminalAssociations.has(sessionResource),
				"Terminal association should be removed after archive",
			);
			ok(
				!runInTerminalTool.sessionTerminalInstances.has(sessionResource),
				"All tracked terminals for the session should be removed after archive",
			);
		});
		...
		test("should not access agent sessions model when initializing archive listener", () => {
			...
			instantiationService.stub(IAgentSessionsService, {
				...
				get model() {
					modelAccessed = true;
					throw new Error(
						"model should not be accessed when wiring archive listener",
					);
				},
			} as unknown as IAgentSessionsService);

			const noModelAccessRunInTerminalTool = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);
			const ensureArchivedSessionListener = (
				noModelAccessRunInTerminalTool as unknown as Record<string, () => void>
			)["_ensureArchivedSessionListener"];
			...
			strictEqual(
				modelAccessed,
				false,
				"Agent sessions model should not be accessed when initializing archive listener",
			);
		});
		...
		test("should dispose all terminals associated with a single chat session", () => {
			...
			chatServiceDisposeEmitter.fire({
			...
			ok(
				!runInTerminalTool.sessionTerminalAssociations.has(sessionResource),
				"Terminal association should be removed after disposal",
			);
			ok(
				!runInTerminalTool.sessionTerminalInstances.has(sessionResource),
				"All tracked terminals for the session should be removed after disposal",
			);
		});
		...
		test("should dispose associated terminals when chat session is disposed", () => {
			...
			ok(
				runInTerminalTool.sessionTerminalAssociations.has(sessionResource),
				"Terminal association should exist before disposal",
			);
		...
		test("should not affect other sessions when one session is disposed", () => {
			...
			ok(
				runInTerminalTool.sessionTerminalAssociations.has(sessionResource1),
				"Session 1 terminal association should exist",
			);
			ok(
				runInTerminalTool.sessionTerminalAssociations.has(sessionResource2),
				"Session 2 terminal association should exist",
			);

			chatServiceDisposeEmitter.fire({
				sessionResources: [sessionResource1],
				reason: "cleared",
			});
			...
			strictEqual(
				terminal2Disposed,
				false,
				"Terminal 2 should NOT have been disposed",
			);
			ok(
				!runInTerminalTool.sessionTerminalAssociations.has(sessionResource1),
				"Session 1 terminal association should be removed",
			);
			ok(
				runInTerminalTool.sessionTerminalAssociations.has(sessionResource2),
				"Session 2 terminal association should remain",
			);
		});

		test("should handle disposal of non-existent session gracefully", () => {
			strictEqual(
				runInTerminalTool.sessionTerminalAssociations.size,
				0,
				"No associations should exist initially",
			);
			chatServiceDisposeEmitter.fire({
				sessionResources: [
					LocalChatSessionUri.forSession("non-existent-session"),
				],
				reason: "cleared",
			});
			strictEqual(
				runInTerminalTool.sessionTerminalAssociations.size,
				0,
				"No associations should exist after handling non-existent session",
			);
		});

		test("should not reuse a disposed cached terminal", () => {
			const sessionResource = LocalChatSessionUri.forSession(
				"disposed-terminal-session",
			);
			...
			const cachedTerminal =
				runInTerminalTool.sessionTerminalAssociations.get(sessionResource);
			ok(cachedTerminal, "Cached terminal should exist in the map");
			strictEqual(
				cachedTerminal!.instance.isDisposed,
				true,
				"Cached terminal should be disposed",
			);
			...
			const wouldReuse =
				...
				!cachedTerminal.isBackground &&
				!cachedTerminal.instance.isDisposed;
			strictEqual(
				wouldReuse,
				false,
				"Should not reuse a disposed cached terminal",
			);
		});
	});
	...
	test("should dedupe rapid repeated background input-needed notifications", () => {
		...
		const terminalInstance = {
			capabilities: {
				get: (cap: TerminalCapability) =>
					cap === TerminalCapability.CommandDetection
						? { onCommandFinished: commandFinishedEmitter.event }
						: undefined,
			},
		...
		const outputMonitor = {
			...
			onDidDetectInputNeeded: Event<void>;
			onDidDetectSensitiveInputNeeded: Event<void>;
			continueMonitoringAsync: () => void;
			dispose: () => void;
		};

		const toolSpecificData = {
			kind: "terminal",
			commandLine: { original: "npm init" },
			language: "bash",
		} as IChatTerminalToolInvocationData;

		(
			runInTerminalTool.constructor as unknown as {
				_activeExecutions: Map<string, { getOutput(): string }>;
			}
		)._activeExecutions.set(termId, {
			getOutput: () => output,
		});
		...
		inputNeededEmitter.fire();
		strictEqual(
			capturedSteeringRequests.length,
			1,
			"Expected duplicate rapid input-needed events to be suppressed",
		);
		...
		strictEqual(
			capturedSteeringRequests.length,
			2,
			"Expected a changed prompt to trigger a new notification",
		);
	});
	...
	test("should suppress input-needed after disposal and omit successful exit code from terminal-exited notice", () => {
		...
		const sessionResource = LocalChatSessionUri.forSession(
			"test-input-needed-disposed-session",
		);
		...
		const toolSpecificData = {
			kind: "terminal",
			commandLine: { original: "git --no-pager diff -- foo.ts" },
			language: "bash",
		} as IChatTerminalToolInvocationData;
		...
		strictEqual(
			capturedSteeringRequests.length,
			0,
			"Closing the terminal should not produce a spurious input-needed chat turn",
		);
		...
		strictEqual(
			capturedSteeringRequests.length,
			1,
			"Closing the terminal should send one terminal-exited notification",
		);
		ok(
			capturedSteeringRequests[0].message.includes("terminal exited."),
			"Successful terminal exit should be reported without qualification",
		);
		ok(
			!capturedSteeringRequests[0].message.includes("exit code 0"),
			"Successful terminal exit should not print exit code 0 to chat",
		);
	});
	...
	test("should suppress redundant input-needed notification for output already returned via foreground inputNeeded", () => {
		...
		const sessionResource = LocalChatSessionUri.forSession(
			"test-input-needed-already-notified-session",
		);
		...
		const toolSpecificData = {
			kind: "terminal",
			commandLine: { original: "mkdir -p foo && cd foo && npm init" },
			language: "bash",
		} as IChatTerminalToolInvocationData;
		...
		(
			runInTerminalTool as unknown as {
				_registerCompletionNotification: (
					...
					alreadyNotifiedInputNeededOutput?: string,
				) => void;
			}
		)._registerCompletionNotification(
			terminalInstance,
		...
		strictEqual(
			capturedSteeringRequests.length,
			0,
			"Should not re-notify for output the agent already received via the foreground inputNeeded race",
		);
		...
		strictEqual(
			capturedSteeringRequests.length,
			1,
			"Expected a new notification once the prompt output changes",
		);
	});
	...
	test("should preserve session terminal association after inputNeeded so fg terminal is reused", () => {
		...
		const toolSpecificData = {
			kind: "terminal",
			commandLine: { original: "ssh host" },
			language: "bash",
		} as IChatTerminalToolInvocationData;

		// This is a foreground terminal, so it should be in foregroundInstances
		(
			instantiationService.get(ITerminalService)
				.foregroundInstances as ITerminalInstance[]
		).push(terminalInstance);
		...
		(
			runInTerminalTool.constructor as unknown as {
				_activeExecutions: Map<
					string,
					{ getOutput(): string; dispose(): void }
				>;
			}
		)._activeExecutions.set(termId, {
			getOutput: () => "Password:",
		...
		strictEqual(
			capturedSteeringRequests.length,
			1,
			"Should send steering request for input needed",
		);

		// The key assertion: fg terminal association is preserved (not deleted)
		ok(
			runInTerminalTool.sessionTerminalAssociations.has(sessionResource),
			"Session terminal association should be preserved for fg reuse",
		);
		strictEqual(
			runInTerminalTool.sessionTerminalAssociations.get(sessionResource)!
				.isBackground,
			false,
			"Terminal should remain foreground",
		);
		...
		strictEqual(
			capturedSteeringRequests.length,
			2,
			"Should send a completion steering request",
		);
		ok(
			capturedSteeringRequests[1].message.includes("command completed."),
			"Successful completion should be reported without qualification",
		);
		ok(
			!capturedSteeringRequests[1].message.includes("exit code 0"),
			"Successful completion should not print exit code 0 to chat",
		);
		ok(
			runInTerminalTool.sessionTerminalAssociations.has(sessionResource),
			"Session terminal association should still be preserved after command finishes",
		);
		strictEqual(
			runInTerminalTool.sessionTerminalAssociations.get(sessionResource)!
				.isBackground,
			false,
			"Terminal should still be foreground after command finishes",
		);
	});
	...
	suite("auto approve warning acceptance mechanism", () => {
		...
		test("should include autoApproveInfo when command would be auto-approved but warning not accepted", async () => {
			...
			ok(
				terminalData.autoApproveInfo,
				"autoApproveInfo should be set for commands that would be auto-approved",
			);
		});
	...
	suite("input-needed steering text", () => {
		function buildSteeringText(
			hungHint: "none" | "timeout" | "idleSilence",
		): string {
			const sessionResource = LocalChatSessionUri.forSession(
				"input-needed-steering-session",
			);
			// eslint-disable-next-line @typescript-eslint/naming-convention
			return (
				runInTerminalTool as unknown as {
					_buildInputNeededSteeringText(
						s: URI,
						t: string,
						h: "none" | "timeout" | "idleSilence",
					): string;
				}
			)._buildInputNeededSteeringText(
				sessionResource,
		...
		test("none mode does not mention timeout, idle silence, or kill_terminal", () => {
			...
			ok(
				...
				"Expected no mention of timeout in the input-needed (none) hint",
			);
			ok(
				!text.toLowerCase().includes("no output"),
				"Expected no mention of idle silence in the input-needed (none) hint",
			);
			ok(
				!text.includes(TerminalToolId.KillTerminal),
				"Expected kill_terminal not to be advertised in the input-needed (none) hint",
			);
		});
		...
		test("timeout mode advertises kill_terminal and mentions timeout", () => {
			...
			ok(
				text.toLowerCase().includes("timeout"),
				'Expected timeout hint to mention "timeout"',
			);
			ok(
				text.includes(TerminalToolId.KillTerminal),
				"Expected timeout hint to advertise kill_terminal",
			);
		});
		...
		test('idleSilence mode advertises kill_terminal without saying "timeout"', () => {
			...
			ok(
				!text.toLowerCase().includes("timeout"),
				"Idle-silence hint must not refer to a timeout",
			);
			ok(
				text.toLowerCase().includes("no output"),
				"Expected idle-silence hint to describe the no-output condition",
			);
			ok(
				text.includes(TerminalToolId.KillTerminal),
				"Expected idle-silence hint to advertise kill_terminal",
			);
		});
	});
	...
	suite("unique rules deduplication", () => {
		test("should properly deduplicate rules with same sourceText in auto-approve info", async () => {
			...
			const autoApproveInfo = (
				result!.toolSpecificData as IChatTerminalToolInvocationData
			).autoApproveInfo!;
			ok(autoApproveInfo);
			ok(
				autoApproveInfo.value.includes("Auto approved by rule "),
				'should contain singular "rule", not plural',
			);
	...
	suite("session auto approval", () => {
		test("should auto approve all commands when session has auto approval enabled", async () => {
			const sessionId = "test-session-123";
			const sessionResource = LocalChatSessionUri.forSession(sessionId);
			const terminalChatService =
				instantiationService.get(ITerminalChatService);
			...
			let result = await runInTerminalTool.prepareToolInvocation(
				context,
				CancellationToken.None,
			);
			assertConfirmationRequired(result);
			...
			result = await runInTerminalTool.prepareToolInvocation(
				context,
				CancellationToken.None,
			);
			assertAutoApproved(result);
			...
			ok(
				terminalData.autoApproveInfo,
				"Expected autoApproveInfo to be defined",
			);
			ok(
				terminalData.autoApproveInfo.value.includes(
					"Auto approved for this session",
				),
				"Expected session approval message",
			);
		});
		...
		test("should bypass terminal auto-approve feature in Autopilot mode", async () => {
			...
			const sessionResource =
				LocalChatSessionUri.forSession("autopilot-session");
			instantiationService.stub(IChatWidgetService, {
				getWidgetBySessionResource: (() => ({
					input: {
						currentModeInfo: { permissionLevel: ChatPermissionLevel.Autopilot },
					},
				})) as unknown as IChatWidgetService["getWidgetBySessionResource"],
				lastFocusedWidget: undefined,
			});

			const autopilotRunInTerminalTool = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);
			...
			strictEqual(
				terminalData.autoApproveInfo,
				undefined,
				"Expected no terminal auto-approve info in Autopilot mode",
			);
		});
		...
		test("should bypass terminal auto-approve feature in Bypass Approvals mode", async () => {
			...
			const bypassRunInTerminalTool = store.add(
				instantiationService.createInstance(TestRunInTerminalTool),
			);
			...
			strictEqual(
				terminalData.autoApproveInfo,
				undefined,
				"Expected no terminal auto-approve info in Bypass Approvals mode",
			);
		});
	});
	...
	suite("TerminalProfileFetcher", () => {
		suite("getCopilotProfile", () => {
			(isWindows ? test : test.skip)(
				"should return custom profile when configured",
				async () => {
					...
					const result =
						await runInTerminalTool.profileFetcher.getCopilotProfile();
					strictEqual(result, customProfile);
				},
			);

			(isLinux ? test : test.skip)(
				"should fall back to default shell when no custom profile is configured",
				async () => {
					runInTerminalTool.setBackendOs(OperatingSystem.Linux);
	...
	suite("denial info in disclaimers", () => {
		function getDisclaimerValue(
			disclaimer: string | IMarkdownString | undefined,
		): string | undefined {
			if (!disclaimer) {
				return undefined;
			}
		...
		test("should include denial reason in disclaimer when command is denied by rule", async () => {
			...
			assertConfirmationRequired(result, "Run `bash` command?");
			const disclaimerValue = getDisclaimerValue(
				result?.confirmationMessages?.disclaimer,
			);
			ok(disclaimerValue, "Expected disclaimer to be defined");
			ok(
				disclaimerValue.includes("denied"),
				"Expected disclaimer to mention denial",
			);
			ok(
				disclaimerValue.includes("npm"),
				"Expected disclaimer to mention the denied rule",
			);
		});
		...
		test("should include link to settings in denial disclaimer", async () => {
			...
			ok(
				result?.confirmationMessages?.disclaimer,
				"Expected disclaimer to be defined",
			);
			...
			ok(
				typeof disclaimer !== "string" && disclaimer.isTrusted,
				"Expected disclaimer to be trusted for command links",
			);
		});
		...
		test("should not include denial info when auto-approve is disabled", async () => {
			...
			if (disclaimerValue) {
				ok(
					!disclaimerValue.includes("denied"),
					"Should not mention denial when auto-approve is disabled",
				);
			}
		});
		...
		test("should not include denial info for commands that are simply not approved", async () => {
			...
			if (disclaimerValue) {
				ok(
					!disclaimerValue.includes("denied"),
					"Should not mention denial for non-denied commands",
				);
			}
		});
	});
	...
	suite("ConfirmTerminalCommandTool", () => {
		test("should require confirmation when sandbox is enabled but sandbox rewriting is disabled", async () => {
			...
			const { ConfirmTerminalCommandTool } =
				await import("../../browser/tools/runInTerminalConfirmationTool.js");
			const confirmTool = store.add(
				instantiationService.createInstance(ConfirmTerminalCommandTool),
			);
...
suite("ChatAgentToolsContribution - tool registration refresh", () => {
	...
	setup(() => {
		...
		const terminalServiceDisposeEmitter = store.add(
			new Emitter<ITerminalInstance>(),
		);
		const chatServiceDisposeEmitter = store.add(
			new Emitter<{ sessionResources: URI[]; reason: "cleared" }>(),
		);
		...
		const terminalSandboxService: ITerminalSandboxService = {
			...
			checkForSandboxingPrereqs: async () => ({
				...
				sandboxConfigPath: sandboxEnabled ? "/tmp/sandbox.json" : undefined,
				failedCheck: undefined,
			}),
			getTempDir: () => undefined,
			setNeedsForceUpdateConfigFile: () => {},
	...
	async function createContribution(): Promise<ChatAgentToolsContribution> {
		const contribution = store.add(
			instantiationService.createInstance(ChatAgentToolsContribution),
		);
		await flushAsync();
		return contribution;
	}
	...
	test("should register run_in_terminal tool on construction", async () => {
		...
		ok(
			registeredToolData.has(TerminalToolId.RunInTerminal),
			"Expected run_in_terminal tool to be registered",
		);
	});
	...
	test("should refresh run_in_terminal tool data when sandbox setting changes", async () => {
		...
		strictEqual(
			allowToRunUnsandboxedCommandsProperty?.const,
			true,
			"Expected allowToRunUnsandboxedCommands const to match the setting value after enabling sandbox",
		);
		strictEqual(
			allowToRunUnsandboxedCommandsProperty?.default,
			true,
			"Expected allowToRunUnsandboxedCommands to default to the setting value after enabling sandbox",
		);
		ok(
			propertiesAfter?.["requestUnsandboxedExecution"],
			"Expected requestUnsandboxedExecution after enabling sandbox",
	...
	test("should refresh run_in_terminal tool data when unsandboxed command allowance changes", async () => {
		...
		const propertiesBefore = toolDataBefore.inputSchema?.properties as
			| Record<string, object>
			| undefined;
		const allowToRunUnsandboxedCommandsPropertyBefore = propertiesBefore?.[
			"allowToRunUnsandboxedCommands"
		] as { const?: boolean; default?: boolean } | undefined;
		strictEqual(
			allowToRunUnsandboxedCommandsPropertyBefore?.const,
			true,
			"Expected allowToRunUnsandboxedCommands const to start enabled",
		);
		strictEqual(
			allowToRunUnsandboxedCommandsPropertyBefore?.default,
			true,
			"Expected allowToRunUnsandboxedCommands to start enabled",
		);
		ok(
			propertiesBefore?.["requestUnsandboxedExecution"],
			"Expected requestUnsandboxedExecution before disabling unsandboxed commands",
		);
		...
		const propertiesAfter = toolDataAfter.inputSchema?.properties as
			| Record<string, object>
			| undefined;
		const allowToRunUnsandboxedCommandsPropertyAfter = propertiesAfter?.[
			"allowToRunUnsandboxedCommands"
		] as { const?: boolean; default?: boolean } | undefined;
		strictEqual(
			allowToRunUnsandboxedCommandsPropertyAfter?.const,
			false,
			"Expected allowToRunUnsandboxedCommands const to reflect disabled setting after refresh",
		);
		strictEqual(
			allowToRunUnsandboxedCommandsPropertyAfter?.default,
			false,
			"Expected allowToRunUnsandboxedCommands to reflect disabled setting after refresh",
		);
		ok(
			propertiesAfter?.["requestUnsandboxedExecution"],
			"Expected requestUnsandboxedExecution to remain after disabling unsandboxed commands",
		);
	});
	...
	test("should refresh run_in_terminal tool data when sandbox network setting changes", async () => {
		...
		configurationService.onDidChangeConfigurationEmitter.fire({
			affectsConfiguration: (key: string) =>
				key === AgentNetworkDomainSettingId.AllowedNetworkDomains,
			affectedKeys: new Set([
		...
		ok(
			toolDataAfter,
			"Expected run_in_terminal tool to still be registered after network setting change",
		);
	});
});
