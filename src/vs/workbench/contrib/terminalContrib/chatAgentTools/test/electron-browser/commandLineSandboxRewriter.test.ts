...
suite("CommandLineSandboxRewriter", () => {
	...
	const stubTreeSitterCommandParser = (
		commands: ITerminalSandboxCommand[] = [],
	): TreeSitterCommandParser =>
		({
			extractCommands: async () => commands,
		}) as unknown as TreeSitterCommandParser;

	const stubSandboxService = (
		overrides: Partial<ITerminalSandboxService> = {},
	) => {
	...
	test("returns undefined when sandbox is disabled", async () => {
		stubSandboxService();
		const rewriter = store.add(
			instantiationService.createInstance(
				CommandLineSandboxRewriter,
				stubTreeSitterCommandParser(),
			),
		);
	...
	test("wraps command when sandbox is enabled and config exists", async () => {
		...
		stubSandboxService({
			wrapCommand: async (
				command,
				_requestUnsandboxedExecution,
				_shell,
				cwd,
				commandDetails,
			) => {
				calls.push(
					`wrapCommand:${commandDetails?.map((command) => command.keyword).join(",") ?? ""}:${cwd?.path ?? ""}`,
				);
				return {
					command: `wrapped:${command}`,
					isSandboxWrapped: true,
		...
		const rewriter = store.add(
			instantiationService.createInstance(
				CommandLineSandboxRewriter,
				stubTreeSitterCommandParser([{ keyword: "node", args: [] }]),
			),
		);
	...
	test("wraps command and forwards sandbox bypass flag when explicitly requested", async () => {
		...
		stubSandboxService({
			wrapCommand: async (
				...
				calls.push(
					`commands:${commandDetails?.map((command) => command.keyword).join(",") ?? ""}`,
				);
				return {
					command: `wrapped:${command}`,
		...
		const rewriter = store.add(
			instantiationService.createInstance(
				CommandLineSandboxRewriter,
				stubTreeSitterCommandParser([{ keyword: "git", args: [] }]),
			),
		);
		const result = await rewriter.rewrite({
			...createRewriteOptions("echo hello"),
