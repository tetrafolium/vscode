...
suite("CommandLineCdPrefixRewriter", () => {
	...
	setup(() => {
		...
		rewriter = store.add(
			instantiationService.createInstance(CommandLineCdPrefixRewriter),
		);
	});
	...
	suite("cd <cwd> && <suffix> -> <suffix>", () => {
		(!isWindows ? suite : suite.skip)("Posix", () => {
			...
			test("should return undefined when no cd prefix pattern matches", () =>
				t("echo hello world", "bash", undefined));
			test("should return undefined when cd pattern does not have suffix", () =>
				t("cd /some/path", "bash", undefined));
			test("should rewrite command with ; separator when directory matches cwd", () =>
				t("cd /test/workspace; npm test", "pwsh", "npm test"));
			test("should rewrite command with && separator when directory matches cwd", () =>
				t("cd /test/workspace && npm install", "bash", "npm install"));
			...
			test("should not rewrite command when directory does not match cwd", () =>
				t("cd /different/path && npm install", "bash", undefined));
			test("should handle commands with complex suffixes", () =>
				t(
					'cd /test/workspace && npm install && npm test && echo "done"',
					"bash",
					'npm install && npm test && echo "done"',
				));
		...
		(isWindows ? suite : suite.skip)("Windows", () => {
			...
			test("should ignore any trailing back slash", () =>
				t("cd c:\\test\\workspace\\ && npm install", "cmd", "npm install"));
			test("should rewrite command with && separator when directory matches cwd", () =>
				t("cd C:\\test\\workspace && npm test", "cmd", "npm test"));
			test("should rewrite command with ; separator when directory matches cwd - PowerShell style", () =>
				t("cd C:\\test\\workspace; npm test", "pwsh", "npm test"));
			test("should not rewrite when cwd differs from cd path", () =>
				t("cd C:\\different\\path && npm test", "cmd", undefined));
			...
			test("should handle cd /d flag when directory matches cwd", () =>
				t("cd /d C:\\test\\workspace && echo hello", "pwsh", "echo hello"));
			...
			test("should not rewrite cd /d when directory does not match cwd", () =>
				t("cd /d C:\\different\\path ; echo hello", "pwsh", undefined));
