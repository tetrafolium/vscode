...
suite("CommandLineBackgroundDetachRewriter", () => {
	...
	setup(() => {
		...
		rewriter = store.add(
			instantiationService.createInstance(CommandLineBackgroundDetachRewriter),
		);
	});
	...
	test("should return undefined for foreground commands", () => {
		strictEqual(
			rewriter.rewrite(
				createOptions("echo hello", "/bin/bash", OperatingSystem.Linux, false),
			),
			undefined,
		);
	});
	...
	test("should return undefined when isBackground is not set", () => {
		strictEqual(
			rewriter.rewrite(
				createOptions("echo hello", "/bin/bash", OperatingSystem.Linux),
			),
			undefined,
		);
	});
		...
		test("should wrap with nohup on Linux", () => {
			deepStrictEqual(
				...
				),
				{
					...
					reasoning:
						"Wrapped background command with nohup to survive terminal shutdown",
	...
	suite("POSIX shell -c wrapping for compound commands and builtins", () => {
		...
		test("eval builtin should be wrapped in shell -c", () => {
			deepStrictEqual(
				rewriter.rewrite(
					createOptions(
						"eval $SETUP_ENV && opam install coq --yes",
						"/bin/bash",
						OperatingSystem.Linux,
						true,
					),
		...
		test("export builtin should be wrapped in shell -c", () => {
			deepStrictEqual(
				rewriter.rewrite(
					createOptions(
						'export PATH="/usr/local/bin:$PATH"; myapp',
						"/bin/bash",
						OperatingSystem.Linux,
						true,
					),
		...
		test("relative path ./script should NOT be wrapped in shell -c", () => {
			deepStrictEqual(
				rewriter.rewrite(
					createOptions("./start.sh", "/bin/bash", OperatingSystem.Linux, true),
				),
				{
	...
	suite("POSIX inline env var assignments", () => {
		test("single env var assignment before command should be wrapped in shell -c", () => {
			deepStrictEqual(
				rewriter.rewrite(
					createOptions(
						"OPAMROOT=/root/.opam opam install menhir",
						"/bin/bash",
						OperatingSystem.Linux,
						true,
					),
		...
		test("multiple env var assignments before command should be wrapped in shell -c", () => {
			deepStrictEqual(
				rewriter.rewrite(
					createOptions(
						"OPAMROOT=/root/.opam OPAMYES=1 opam install menhir",
						"/bin/bash",
						OperatingSystem.Linux,
						true,
					),
	...
	suite("POSIX shell operator wrapping", () => {
		...
		test("semicolon chain should be wrapped in shell -c", () => {
			deepStrictEqual(
				rewriter.rewrite(
					createOptions("cmd1; cmd2", "/bin/bash", OperatingSystem.Linux, true),
				),
				{
					rewritten: `nohup /bin/bash -c 'cmd1; cmd2' & disown`,
					reasoning:
		...
		test("cd builtin should be wrapped in shell -c", () => {
			deepStrictEqual(
				rewriter.rewrite(
					createOptions("cd /app", "/bin/bash", OperatingSystem.Linux, true),
				),
				{
	...
	suite("Windows (PowerShell)", () => {
		test("should wrap with Start-Process for pwsh", () => {
			deepStrictEqual(
				...
				{
					rewritten:
						'Start-Process -WindowStyle Hidden -FilePath "C:\\Program Files\\PowerShell\\7\\pwsh.exe" -ArgumentList "-NoProfile", "-Command", "python app.py"',
					reasoning:
						"Wrapped background command with Start-Process to survive terminal shutdown",
		...
		test("should wrap with Start-Process for Windows PowerShell", () => {
			deepStrictEqual(
				...
				{
					rewritten:
						'Start-Process -WindowStyle Hidden -FilePath "C:\\WINDOWS\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -ArgumentList "-NoProfile", "-Command", "node server.js"',
					reasoning:
						"Wrapped background command with Start-Process to survive terminal shutdown",
					forDisplay: "node server.js",
				},
		...
		test("should escape double quotes in PowerShell commands", () => {
			deepStrictEqual(
				...
				{
					rewritten:
						'Start-Process -WindowStyle Hidden -FilePath "C:\\Program Files\\PowerShell\\7\\pwsh.exe" -ArgumentList "-NoProfile", "-Command", "echo \\"hello world\\""',
					reasoning:
						"Wrapped background command with Start-Process to survive terminal shutdown",
		...
		test("should return undefined for non-PowerShell Windows shell", () => {
			strictEqual(
				rewriter.rewrite(
					createOptions("echo hello", "cmd.exe", OperatingSystem.Windows, true),
				),
				undefined,
			);
		});
	...
	suite("Interactive front-end skip", () => {
		...
		for (const cmd of interactives) {
			test(`should skip detach-wrap for interactive: ${cmd}`, () => {
				strictEqual(
					rewriter.rewrite(
						createOptions(cmd, "/bin/bash", OperatingSystem.Linux, true),
					),
					undefined,
				);
			});
