...
} from "./tools/getTerminalLastCommandTool.js";
import {
	KillTerminalTool,
	KillTerminalToolData,
} from "./tools/killTerminalTool.js";
import {
	GetTerminalOutputTool,
	GetTerminalOutputToolData,
} from "./tools/getTerminalOutputTool.js";
import {
	SendToTerminalTool,
	SendToTerminalToolData,
} from "./tools/sendToTerminalTool.js";
import {
	GetTerminalSelectionTool,
	GetTerminalSelectionToolData,
} from "./tools/getTerminalSelectionTool.js";
import {
	ConfirmTerminalCommandTool,
	ConfirmTerminalCommandToolData,
} from "./tools/runInTerminalConfirmationTool.js";
import {
	RunInTerminalTool,
	createRunInTerminalToolData,
} from "./tools/runInTerminalTool.js";
import {
	CreateAndRunTaskTool,
	CreateAndRunTaskToolData,
} from "./tools/task/createAndRunTaskTool.js";
import {
	GetTaskOutputTool,
	GetTaskOutputToolData,
} from "./tools/task/getTaskOutputTool.js";
...
class ShellIntegrationTimeoutMigrationContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		super();
		const deprecated = configurationService.inspect<number>(
			TerminalChatAgentToolsSettingId.ShellIntegrationTimeout,
		);
		const target = configurationService.inspect<number>(
			TerminalSettingId.ShellIntegrationTimeout,
		);
		if (
			deprecated.userValue !== undefined &&
			target.userValue === undefined &&
			isNumber(deprecated.userValue)
		) {
		...
		if (
			deprecated.workspaceValue !== undefined &&
			target.workspaceValue === undefined &&
			isNumber(deprecated.workspaceValue)
		) {
			configurationService.updateValue(
				TerminalSettingId.ShellIntegrationTimeout,
...
class OutputLocationMigrationContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const currentValue = configurationService.getValue<unknown>(
...
export class ChatAgentToolsContribution
	extends Disposable
	implements IWorkbenchContribution
{
	static readonly ID = "terminal.chatAgentTools";

	private readonly _runInTerminalToolRegistration = this._register(
	...
	constructor(
		...
		const confirmTerminalCommandTool = _instantiationService.createInstance(
			ConfirmTerminalCommandTool,
		);
		this._register(
			_toolsService.registerTool(
				ConfirmTerminalCommandToolData,
				confirmTerminalCommandTool,
			),
		);
		const getTerminalOutputTool = _instantiationService.createInstance(
			GetTerminalOutputTool,
		);
		this._register(
			_toolsService.registerTool(
		...
		this._register(
			_toolsService.executeToolSet.addTool(GetTerminalOutputToolData),
		);

		const killTerminalTool =
			_instantiationService.createInstance(KillTerminalTool);
		this._register(
			_toolsService.registerTool(KillTerminalToolData, killTerminalTool),
		);
		this._register(_toolsService.executeToolSet.addTool(KillTerminalToolData));

		const sendToTerminalTool =
			_instantiationService.createInstance(SendToTerminalTool);
		this._register(
			_toolsService.registerTool(SendToTerminalToolData, sendToTerminalTool),
		);
		this._register(
			_toolsService.executeToolSet.addTool(SendToTerminalToolData),
		);

		this._registerRunInTerminalTool();

		const getTerminalSelectionTool = _instantiationService.createInstance(
			GetTerminalSelectionTool,
		);
		this._register(
			_toolsService.registerTool(
		...
		const getTerminalLastCommandTool = _instantiationService.createInstance(
			GetTerminalLastCommandTool,
		);
		this._register(
			_toolsService.registerTool(
		...
		this._register(
			_toolsService.readToolSet.addTool(GetTerminalSelectionToolData),
		);
		this._register(
			_toolsService.readToolSet.addTool(GetTerminalLastCommandToolData),
		);
		...
		const getTaskOutputTool =
			_instantiationService.createInstance(GetTaskOutputTool);
		this._register(
			_toolsService.registerTool(GetTaskOutputToolData, getTaskOutputTool),
		);

		const createAndRunTaskTool =
			_instantiationService.createInstance(CreateAndRunTaskTool);
		this._register(
			_toolsService.registerTool(
				CreateAndRunTaskToolData,
				createAndRunTaskTool,
		...
		this._register(
			_toolsService.executeToolSet.addTool(CreateAndRunTaskToolData),
		);
	...
	private _registerRunInTerminalTool(): void {
		...
		this._instantiationService
			.invokeFunction(createRunInTerminalToolData)
			.then((runInTerminalToolData) => {
				if (
					this._store.isDisposed ||
					version !== this._runInTerminalToolRegistrationVersion
				) {
					return;
				}
				if (!this._runInTerminalTool) {
					this._runInTerminalTool = this._register(
						this._instantiationService.createInstance(RunInTerminalTool),
					);
				}
				...
				store.add(
					this._toolsService.registerToolImplementation(
						runInTerminalToolData.id,
						this._runInTerminalTool,
					),
				);
				store.add(
					this._toolsService.executeToolSet.addTool(runInTerminalToolData),
				);
