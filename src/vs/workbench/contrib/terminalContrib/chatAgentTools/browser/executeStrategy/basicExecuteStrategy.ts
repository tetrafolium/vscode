...
function formatExitCodeOrError(
	exitCodeOrError: number | ITerminalLaunchError | undefined,
): string {
 ...
 */
export class BasicExecuteStrategy
	extends Disposable
		...
		try {
			...
			const onDone = Promise.race([
				Event.toPromise(onCommandFinishedFiltered, store).then((e) => {
					...
					this._log(
						"onDone 1 of 2 via end event, waiting for short idle prompt",
					);
				...
				Event.toPromise(
					token.onCancellationRequested as Event<undefined>,
					store,
				).then(() => {
				...
				Event.toPromise(this._instance.onExit, store).then(
					(exitCodeOrError) => {
						this._log(
							`onDone via process exit (${formatExitCodeOrError(exitCodeOrError)})`,
						);
			...
			if (this._hasReceivedUserInput()) {
				...
				if (this._commandDetection.executingCommandObject !== undefined) {
					...
					this._log(
						"Prompt is idle; clearing pending input with Ctrl+U instead of SIGINT",
					);
			...
			const onDoneResult = await Promise.race([
				onDone,
				alternateBufferPromise.then(
					() => ({ type: "alternateBuffer" }) as const,
				),
			]);
			...
			const finishedCommand =
				onDoneResult && onDoneResult.type === "success"
					? onDoneResult.command
					: undefined;
			if (finishedCommand) {
				this._log(
