...
export function truncateOutputKeepingTail(
	output: string,
	maxLength: number,
): string {
 ...
 */
export function normalizeTerminalCommandForDisplay(
	commandLine: string,
): string {
...
export function generateAutoApproveActions(
	commandLine: string,
	subCommands: string[],
	autoApproveResult: {
	...
	if (canCreateAutoApproval) {
		...
		// command line.
		const subCommandsToSuggest = Array.from(
			new Set(
				coalesce(
					unapprovedSubCommands.map((command) => {
		...
		const firstSubcommandFirstWord =
			unapprovedSubCommands.length > 0
				? unapprovedSubCommands[0].split(" ")[0]
				: "";
		if (
...
export function dedupeRules(
	rules: ICommandApprovalResultWithReason[],
	...
	return rules.filter((result, index, array) => {
		...
		return (
			array.findIndex(
				(r) => isAutoApproveRule(r.rule) && r.rule.sourceText === sourceText,
			) === index
		);
	});
}
