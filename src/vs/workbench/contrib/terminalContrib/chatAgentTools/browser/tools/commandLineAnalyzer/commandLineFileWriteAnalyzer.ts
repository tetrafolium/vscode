...
export class CommandLineFileWriteAnalyzer
	extends Disposable
	implements ICommandLineAnalyzer
{
	...
	async analyze(
		...
		try {
			...
			console.error(e);
			this._log(
				"Failed to get file writes via grammar",
	...
	private async _getFileWrites(
		options: ICommandLineAnalyzerOptions,
	): Promise<FileWrite[]> {
		let fileWrites: FileWrite[] = [];

		// Get file writes from redirections (via tree-sitter grammar)
		const capturedFileWrites = (
			await this._treeSitterCommandParser.getFileWrites(
				options.treeSitterLanguage,
				options.commandLine,
			)
		...
		const commandFileWrites = (
			await this._treeSitterCommandParser.getCommandFileWrites(
				options.treeSitterLanguage,
				options.commandLine,
			)
		).map(this._mapNullDevice.bind(this, options));
	...
	private _mapNullDevice(
		options: ICommandLineAnalyzerOptions,
		rawFileWrite: string,
	): string | typeof nullDevice {
		if (
			options.treeSitterLanguage === TreeSitterCommandParserLanguage.PowerShell
		) {
	...
	private _getResult(
					...
					if (workspaceFolders.length > 0) {
						for (const fileWrite of fileWrites) {
							if (fileWrite === nullDevice) {
								this._log(
									"File write to null device allowed",
									URI.isUri(fileWrite) ? fileWrite.toString() : fileWrite,
								);
								continue;
							}
							...
							if (isString(fileWrite)) {
								const isAbsolute =
									options.os === OperatingSystem.Windows
										? win32.isAbsolute(fileWrite)
										: posix.isAbsolute(fileWrite);
								if (!isAbsolute) {
									isAutoApproveAllowed = false;
									this._log(
										"File write blocked due to unknown terminal cwd",
										fileWrite,
									);
									break;
								}
							}
							const fileUri = URI.isUri(fileWrite)
								? fileWrite
								: URI.file(fileWrite);
							...
							const isInsideWorkspace = workspaceFolders.some(
								(folder) =>
									...
									(fileUri.path.startsWith(folder.uri.path + "/") ||
										fileUri.path === folder.uri.path),
							);
							if (!isInsideWorkspace) {
								...
								isAutoApproveAllowed = false;
								this._log(
									"File write blocked outside workspace",
									fileUri.toString(),
								);
								break;
							}
						...
						const hasOnlyNullDevices = fileWrites.every(
							(fw) => fw === nullDevice,
						);
		...
		if (fileWrites.length > 0) {
			const fileWritesList = fileWrites
				.map(
					(fw) =>
						`\`${URI.isUri(fw) ? this._labelService.getUriLabel(fw) : fw === nullDevice ? "/dev/null" : fw.toString()}\``,
				)
				.join(", ");
