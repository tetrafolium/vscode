...
export class CreateAndRunTaskTool implements IToolImpl {
	...
	async invoke(
		...
		if (!invocation.context) {
			return {
				content: [{ kind: "text", value: `No invocation context` }],
				toolResultMessage: `No invocation context`,
			};
		}

		const tasksJsonUri = URI.file(args.workspaceFolder).with({
			path: `${args.workspaceFolder}/.vscode/tasks.json`,
		});
		...
		if (!exists) {
			await this._fileService.createFile(
				tasksJsonUri,
				VSBuffer.fromString(tasksJsonContent),
				{ overwrite: true },
			);
			...
			await this._fileService.writeFile(
				tasksJsonUri,
				VSBuffer.fromString(JSON.stringify(tasksJson, null, "\t")),
			);
			_progress.report({ message: "Updated tasks.json file" });
		}
		_progress.report({
			message: new MarkdownString(
				localize("copilotChat.fetchingTask", "Resolving the task"),
			),
		});
		...
		while (Date.now() - start < 5000 && !token.isCancellationRequested) {
			task = (await this._tasksService.tasks())?.find(
				(t) => t._label === args.task.label,
			);
			if (task) {
				break;
			}
		...
		if (!task) {
			return {
				content: [
					{ kind: "text", value: `Task not found: ${args.task.label}` },
				],
		...
		try {
			...
			const startMarkersByTerminalInstanceId = new Map<
				number,
				ReturnType<ITerminalInstance["registerMarker"]>
			>();
			...
			result =
				raceResult && typeof raceResult === "object"
					? (raceResult as ITaskSummary)
					: undefined;
		...
		for (const r of terminalResults) {
			...
			>("copilotChat.runTaskTool.createAndRunTask", {
	...
	async prepareToolInvocation(
		...
		if (allTasks?.find((t) => t._label === task.label)) {
			return {
				invocationMessage: new MarkdownString(
					localize("taskExists", "Task \`{0}\` already exists.", task.label),
				),
		...
		return {
			invocationMessage: new MarkdownString(
				localize("createdTask", "Created task \`{0}\`", task.label),
			),
			pastTenseMessage: new MarkdownString(
				localize("createdTaskPast", "Created task \`{0}\`", task.label),
			),
			...
			task: {
				...
				properties: {
					...
					command: {
						type: "string",
						description:
							"The shell command to run for the task. Use this to specify commands for building or running the application.",
					},
					args: {
						type: "array",
					...
					isBackground: {
						type: "boolean",
						description:
							"Whether the task runs in the background without blocking the UI or other tasks. Set to true for long-running processes like watch tasks or servers that should continue executing without requiring user attention. When false, the task will block the terminal until completion.",
					},
					problemMatcher: {
						type: "array",
