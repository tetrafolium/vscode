...
export class NpmScriptAutoApprover extends Disposable {
	...
	async isCommandAutoApproved(
		command: string,
		cwd: URI | undefined,
	): Promise<INpmScriptAutoApproveResult> {
		// Check if the feature is enabled
		const isNpmScriptAutoApproveEnabled =
	...
	private _extractScriptName(command: string): string | undefined {
		...
		for (const pattern of npmRunPatterns) {
			...
			if (match?.groups?.scriptName) {
				...
				if (
					pkgManager.toLowerCase() === "yarn" &&
					yarnBuiltinCommands.has(scriptName.toLowerCase())
				) {
					continue;
				}
				if (
					pkgManager.toLowerCase() === "pnpm" &&
					pnpmBuiltinCommands.has(scriptName.toLowerCase())
				) {
					continue;
				}
	...
	private _isWithinWorkspace(uri: URI): boolean {
		...
		return workspaceFolders.some((folder: IWorkspaceFolder) =>
			this._uriIdentityService.extUri.isEqualOrParent(uri, folder.uri),
		);
	}
	 ...
	 */
	private async _getPackageJsonScripts(
		cwd: URI | undefined,
	): Promise<IPackageJsonScripts | undefined> {
	 ...
	 */
	private async _readPackageJsonScripts(
		packageJsonUri: URI,
