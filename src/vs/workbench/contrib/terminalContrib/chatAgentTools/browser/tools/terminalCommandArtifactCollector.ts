...
export class TerminalCommandArtifactCollector {
	...
	async capture(
		...
		if (commandId) {
			try {
				toolSpecificData.terminalCommandUri = this._createTerminalCommandUri(
					instance,
					commandId,
				);
			} catch (error) {
			...
			const partialSnapshot = await this._capturePartialCommandOutput(
				instance,
				commandId,
			);
			if (partialSnapshot) {
				...
				this._logService.debug(
					`RunInTerminalTool: Captured partial command output for ${commandId}`,
				);
			}
		}
	...
	private async _captureCommandOutput(
		instance: ITerminalInstance,
		command: ITerminalCommand,
	): Promise<
		IChatTerminalToolInvocationData["terminalCommandOutput"] | undefined
	> {
		try {
		...
		return getCommandOutputSnapshot(xterm, command, (reason, error) => {
			...
			this._logService.debug(
				`RunInTerminalTool: Failed to snapshot command output${suffix}`,
				error,
			);
		});
	}
	 ...
	 */
	private async _capturePartialCommandOutput(
		instance: ITerminalInstance,
		commandId: string,
	): Promise<
		IChatTerminalToolInvocationData["terminalCommandOutput"] | undefined
	> {
		...
		if (
			currentCommand &&
			(currentCommand as { id?: string }).id === commandId
		) {
			...
			if (executedMarker && !executedMarker.isDisposed) {
				try {
					...
					if (lineCount > 0) {
						const text = await xterm.getRangeAsVT(
							executedMarker,
							undefined,
							true,
						);
					...
					this._logService.debug(
						`RunInTerminalTool: Failed to capture partial command output`,
						error,
					);
				}
			}
