	...
	inputSchema: {
		...
		properties: {
			...
			explanation: {
				type: "string",
				description:
					"A one-sentence description of what the command does. This will be shown to the user in the confirmation dialog.",
			},
			goal: {
				type: "string",
				description:
					"A short description of the goal or purpose of the command.",
			},
			mode: {
				type: "string",
				enum: ["sync", "async"],
...
export class ConfirmTerminalCommandTool extends RunInTerminalTool {
	...
	override async prepareToolInvocation(
		context: IToolInvocationPreparationContext,
		token: CancellationToken,
	): Promise<IPreparedToolInvocation | undefined> {
		const preparedInvocation = await super.prepareToolInvocation(
			context,
			token,
		);
		if (preparedInvocation) {
			preparedInvocation.presentation =
