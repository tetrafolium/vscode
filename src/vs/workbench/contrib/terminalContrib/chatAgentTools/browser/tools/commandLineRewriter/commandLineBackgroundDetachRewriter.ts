 ...
 */
export class CommandLineBackgroundDetachRewriter
	extends Disposable
	implements ICommandLineRewriter
{
	...
	private _readsFromStdin(commandLine: string): boolean {
		...
		if (
			/^(expect|passwd|vi|vim|nano|less|more|top|htop|sftp|ftp|telnet|gdb|lldb)\b/.test(
				trimmed,
			)
		) {
			return true;
		...
		if (
			/^psql\b/.test(trimmed) &&
			!/\s(-c|-f|--command|--file)\b/.test(trimmed)
		) {
			return true;
		}
		...
		if (
			/^sudo\b/.test(trimmed) &&
			!/\s-n\b/.test(trimmed) &&
			!/\bSUDO_ASKPASS\b/.test(commandLine)
		) {
			return true;
		}
		return false;
	...
	private _rewriteForPosix(
		...
		if (this._needsShellCWrapper(trimmed)) {
			...
			const innerCommand = endsWithBackgroundAmp
				? trimmed.replace(/\s*&$/, "")
				: trimmed;
		...
		const supportsDisown =
			isBash(options.shell, options.os) || isZsh(options.shell, options.os);
