...
export class CommandLineAutoApprover extends Disposable {
	...
	async isCommandAutoApproved(
		command: string,
		shell: string,
		os: OperatingSystem,
		cwd: URI | undefined,
		chatSessionResource?: URI,
	): Promise<ICommandApprovalResultWithReason> {
		...
		for (const rule of this._getSessionRules(chatSessionResource)
			.allowListRules) {
			if (this._commandMatchesRule(rule, command, shell, os)) {
				return {
					result: "approved",
		...
		const npmScriptResult =
			await this._npmScriptAutoApprover.isCommandAutoApproved(command, cwd);
	...
	private _getSessionRules(chatSessionResource?: URI): {
		...
		const sessionRulesConfig =
			this._terminalChatService.getSessionAutoApproveRules(chatSessionResource);
		for (const [key, value] of Object.entries(sessionRulesConfig)) {
			if (typeof value === "boolean") {
				const { regex, regexCaseInsensitive } =
	...
	private _mapAutoApproveConfigToRules(
		config: unknown,
		configInspectValue: IConfigurationValue<Readonly<unknown>>,
	): {
		denyListRules: IAutoApproveRule[];
		allowListRules: IAutoApproveRule[];
		allowListCommandLineRules: IAutoApproveRule[];
		...
		for (const [key, value] of Object.entries(config)) {
			...
			function checkTarget(
				inspectValue: Readonly<unknown> | undefined,
			): boolean {
				return (
			...
			const sourceTarget = checkTarget(configInspectValue.workspaceFolderValue)
				? ConfigurationTarget.WORKSPACE_FOLDER
				: checkTarget(configInspectValue.workspaceValue)
					? ConfigurationTarget.WORKSPACE
					: checkTarget(configInspectValue.userRemoteValue)
						? ConfigurationTarget.USER_REMOTE
						: checkTarget(configInspectValue.userLocalValue)
							? ConfigurationTarget.USER_LOCAL
							: checkTarget(configInspectValue.userValue)
								? ConfigurationTarget.USER
								: checkTarget(configInspectValue.applicationValue)
			...
			if (
				ignoreDefaults &&
				isDefaultRule &&
				sourceTarget === ConfigurationTarget.DEFAULT
			) {
				continue;
			}
	...
	private _convertAutoApproveEntryToRegex(value: string): {
		regex: RegExp;
