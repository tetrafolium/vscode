...
export class RunTaskTool implements IToolImpl {
	...
	async invoke(
		...
		if (!task) {
			return {
				...
				toolResultMessage: new MarkdownString(
					localize("chat.taskNotFound", "Task not found: \`{0}\`", args.id),
				),
			};
		}
		...
		if (activeTasks.includes(task)) {
			return {
				content: [
					{ kind: "text", value: `The task ${taskLabel} is already running.` },
				],
				toolResultMessage: new MarkdownString(
					localize(
		...
		try {
			...
			const result: ITaskSummary | undefined =
			...
			const terminals = this._terminalService.instances.filter((t) =>
				resources.some(
					(r) => r.path === t.resource.path && r.scheme === t.resource.scheme,
				),
			);
	...
	async prepareToolInvocation(
		...
		if (task && activeTasks.includes(task)) {
			return {
				invocationMessage: new MarkdownString(
					localize("chat.taskAlreadyActive", "The task is already running."),
				),
			};
		}
		...
		return {
			invocationMessage: new MarkdownString(
				localize("chat.runningTask", "Running \`{0}\`", taskLabel),
			),
			pastTenseMessage: new MarkdownString(
				task?.configurationProperties.isBackground
			...
			confirmationMessages: task
				? {
						title: localize("chat.allowTaskRunTitle", "Allow task run?"),
						message: localize(
