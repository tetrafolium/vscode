...
import {
	...
	type ICommandLineAnalyzerResult,
...
import {
	...
	type ICommandApprovalResultWithReason,
} from "./autoApprove/commandLineAutoApprover.js";
...
export class CommandLineAutoApproveAnalyzer
	extends Disposable
	implements ICommandLineAnalyzer
{
	...
	constructor(
		...
		super();
		this._commandLineAutoApprover = this._register(
	...
	async analyze(
		...
		if (
			...
			return {
				...
				autoApproveInfo: new MarkdownString(
					`${localize("autoApprove.session", "Auto approved for this session")} ([${localize("autoApprove.session.disable", "Disable")}](${disableUri.toString()}))`,
					mdTrustSettings,
				),
			};
		}
		...
		try {
			subCommands = await this._treeSitterCommandParser.extractSubCommands(
				options.treeSitterLanguage,
				trimmedCommandLine,
			);
			this._log(
			...
			console.error(e);
			this._log(
				`Failed to parse sub-commands via ${options.treeSitterLanguage} grammar`,
			);
		}
		...
		const commandLineResult =
			this._commandLineAutoApprover.isCommandLineAutoApproved(
				trimmedCommandLine,
				options.chatSessionResource,
			);
		...
		const deniedSubCommandResult = subCommandResults.find(
			(e) => e.result === "denied",
		);
		if (deniedSubCommandResult) {
			this._log("Sub-command DENIED auto approval");
			isDenied = true;
			autoApproveDefault = isAutoApproveRule(deniedSubCommandResult.rule)
				? deniedSubCommandResult.rule.isDefaultRule
				: undefined;
			autoApproveReason = "subCommand";
		} else if (commandLineResult.result === "denied") {
			this._log("Command line DENIED auto approval");
			isDenied = true;
			autoApproveDefault = isAutoApproveRule(commandLineResult.rule)
				? commandLineResult.rule.isDefaultRule
				: undefined;
			...
			if (subCommandResults.every((e) => e.result === "approved")) {
				...
				isAutoApproved = true;
				autoApproveReason = "subCommand";
				autoApproveDefault = subCommandResults.every(
					(e) => isAutoApproveRule(e.rule) && e.rule.isDefaultRule,
				);
			} else {
		...
		this._telemetry.logPrepare({
			...
			subCommands,
			autoApproveAllowed: !isAutoApproveEnabled
				? "off"
				: isAutoApproveWarningAccepted
					? "allowed"
					: "needsOptIn",
			autoApproveResult: isAutoApproved
				? "approved"
				: isDenied
					? "denied"
					: "manual",
		...
		const subCommandsLowerFirstWordOnly = subCommands.map((command) =>
			command.split(" ")[0].toLowerCase(),
		);
		if (
			!isAutoApproved &&
			(subCommandsLowerFirstWordOnly.some((command) =>
				promptInjectionWarningCommandsLower.includes(command),
			) ||
				(isPowerShell(options.shell, options.os) &&
					subCommandsLowerFirstWordOnly.some((command) =>
						promptInjectionWarningCommandsLowerPwshOnly.includes(command),
					)))
		) {
			disclaimers.push(
				localize(
		...
		if (!isAutoApproved && isAutoApproveEnabled) {
			customActions = generateAutoApproveActions(
				trimmedCommandLine,
				subCommands,
				{ subCommandResults, commandLineResult },
			);
		}
	...
	private _createAutoApproveInfo(
		...
		const formatRuleLinks = (
			result: SingleOrMany<ICommandApprovalResultWithReason>,
		): string => {
			return asArray(result)
				.filter(
					(
						e,
					): e is ICommandApprovalResultWithReason & {
		...
		if (isGlobalAutoApproved) {
			...
			return new MarkdownString(
				`${localize("autoApprove.global", "Auto approved by setting {0}", `[\`${ChatConfiguration.GlobalAutoApprove}\`](${settingsUri.toString()} "${localize("ruleTooltip.global", "View settings")}")`)}`,
				mdTrustSettings,
			);
		}
		...
		if (isAutoApproved) {
			switch (autoApproveReason) {
				...
				case "subCommand": {
					...
					const npmScriptApproval = subCommandResults.find((e) =>
						isNpmScriptAutoApproveRule(e.rule),
					);
					if (
						npmScriptApproval &&
						isNpmScriptAutoApproveRule(npmScriptApproval.rule) &&
						npmScriptApproval.rule.npmScriptResult.autoApproveInfo
					) {
			...
			switch (autoApproveReason) {
				...
				case "subCommand": {
					const uniqueRules = dedupeRules(
						subCommandResults.filter((e) => e.result === "denied"),
					);
					if (uniqueRules.length === 1) {
						return new MarkdownString(
							localize(
