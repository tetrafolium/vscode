...
import {
	...
	ElicitationState,
	type IChatExternalToolInvocationUpdate,
...
import type {
	...
	ITerminalExecuteStrategyResult,
} from "../executeStrategy/executeStrategy.js";
...
import {
	...
	ToolTerminalCreator,
	type IToolTerminal,
} from "../toolTerminalCreator.js";
import {
	TreeSitterCommandParser,
	TreeSitterCommandParserLanguage,
} from "../treeSitterCommandParser.js";
import {
	type ICommandLineAnalyzer,
	type ICommandLineAnalyzerOptions,
} from "./commandLineAnalyzer/commandLineAnalyzer.js";
...
import {
	...
	LocalChatSessionUri,
} from "../../../../chat/common/model/chatUri.js";
...
import {
	...
	type ITerminalSandboxResolvedNetworkDomains,
...
} from "./terminalToolAutoApprove.js";
...
function createPowerShellModelDescription(
	...
	if (isSandboxEnabled) {
		parts.push(
			...createSandboxLines(allowToRunUnsandboxedCommands, networkDomains),
		);
	}
...
function createSandboxLines(
	...
	if (networkDomains) {
		...
		const effectiveAllowed = networkDomains.allowedDomains.filter(
			(d) => !deniedSet.has(d),
		);
...
export async function createRunInTerminalToolData(
	...
	const profileFetcher = instantiationService.createInstance(
		TerminalProfileFetcher,
	);
	const [shell, os, isSandboxEnabled, isSandboxAllowNetworkEnabled] =
		await Promise.all([
	...
	const networkDomains =
		isSandboxEnabled && !isSandboxAllowNetworkEnabled
	...
	const sandboxProperties: IJSONSchemaMap = isSandboxEnabled
		? {
				allowToRunUnsandboxedCommands: {
					...
					description:
						"Whether this tool invocation is allowed to run commands outside the terminal sandbox. This value is set by VS Code based on chat.agent.sandbox.allowUnsandboxedCommands.",
				},
				requestUnsandboxedExecution: {
					type: "boolean",
					description:
						"Request that this command run outside the terminal sandbox. Only set this when the command clearly needs unsandboxed access. The user will be prompted before the command runs unsandboxed.",
				},
				requestUnsandboxedExecutionReason: {
					type: "string",
					description:
						"A short explanation of why this command must run outside the terminal sandbox. Only provide this when requestUnsandboxedExecution is true.",
				},
			}
		: {};
...
export function shouldAutomaticallyRetryUnsandboxed(
	options: IAutomaticUnsandboxRetryOptions,
): boolean {
	return (
		options.allowUnsandboxedCommands &&
...
export class RunInTerminalTool extends Disposable implements IToolImpl {
	...
	private readonly _archivedSessionListener = this._register(
		new MutableDisposable(),
	);

	protected readonly _sessionTerminalAssociations =
		new ResourceMap<IToolTerminal>();
	protected readonly _sessionTerminalInstances = new ResourceMap<
		Set<ITerminalInstance>
	>();
	private readonly _terminalsBeingDisposedBySessionCleanup =
		new Set<ITerminalInstance>();
	 ...
	 */
	private readonly _backgroundNotifications = this._register(
	...
	private static readonly _activeExecutions = new Map<
		string,
		IActiveTerminalExecution & { dispose(): void }
	>();
	...
	private get _autoApproveUnsandboxedCommands(): boolean {
		return (
			this._allowUnsandboxedCommands &&
			this._configurationService.getValue<boolean>(
	...
	private _getAllowToRunUnsandboxedCommands(
		...
		return (
			(args.allowToRunUnsandboxedCommands ?? this._allowUnsandboxedCommands) ===
				true && this._allowUnsandboxedCommands
		);
	}
	...
	private _shouldRejectUnsandboxedExecutionRequest(
		...
		return (
			isSandboxEnabled &&
			args.requestUnsandboxedExecution === true &&
	...
	constructor(
		...
		@ITerminalSandboxService
		private readonly _terminalSandboxService: ITerminalSandboxService,
		...
		this._osBackend = this._remoteAgentService
			.getEnvironment()
			.then((remoteEnv) => remoteEnv?.os ?? OS);

		this._terminalToolCreator =
			this._instantiationService.createInstance(ToolTerminalCreator);
		this._treeSitterCommandParser = this._register(
			this._instantiationService.createInstance(TreeSitterCommandParser),
		);
		this._telemetry = this._instantiationService.createInstance(
			RunInTerminalToolTelemetry,
		);
		this._commandArtifactCollector = this._instantiationService.createInstance(
			TerminalCommandArtifactCollector,
		);
		this._profileFetcher = this._instantiationService.createInstance(
			TerminalProfileFetcher,
		);

		this._commandLineRewriters = [
			this._register(
				this._instantiationService.createInstance(CommandLineCdPrefixRewriter),
			),
			this._register(
		...
		this._commandLineAnalyzers = [
				...
				this._instantiationService.createInstance(
					...
					(message, args) =>
						this._logService.info(
							`RunInTerminalTool#CommandLineFileWriteAnalyzer: ${message}`,
							args,
						),
				),
			),
			this._register(
				this._instantiationService.createInstance(
					...
					(message, args) =>
						this._logService.info(
							`RunInTerminalTool#CommandLineAutoApproveAnalyzer: ${message}`,
							args,
						),
				),
			),
		...
		if (this._enableCommandLineSandboxRewriting) {
			this._commandLineAnalyzers.push(
				this._register(
					this._instantiationService.createInstance(CommandLineSandboxAnalyzer),
				),
			);
		}
		...
		this._outputAnalyzers = [
			this._register(
				this._instantiationService.createInstance(SandboxOutputAnalyzer),
			),
		];
	...
	async handleToolStream(
		context: IToolInvocationStreamContext,
		_token: CancellationToken,
	): Promise<IStreamedToolInvocation | undefined> {
		const partialInput = context.rawInput as
			| Partial<IRunInTerminalInputParams>
			| undefined;
		if (
			partialInput &&
			...
			const invocationMessage = new MarkdownString(
				localize(
					...
					"Running `{0}`",
					escapeMarkdownSyntaxTokens(truncatedCommand),
				),
			);
	...
	async prepareToolInvocation(
		...
		const sandboxPrecheckInputs = this._getSandboxPrecheckInputs(
			chatSessionResource,
		...
		if (chatSessionResource) {
			const toolTerminal =
				this._sessionTerminalAssociations.get(chatSessionResource);
		...
		const [os, shell, cwd, sandboxPrereqs] = await Promise.all([
			...
			(async () => {
				...
				if (!cwd) {
					...
					const sessionModel = chatSessionResource
						? this._chatService.getSession(chatSessionResource)
						: undefined;
					if (sessionModel?.workingDirectory) {
						...
						const workspaceFolder = activeWorkspaceRootUri
		...
		const allowUnsandboxedCommands =
			this._getAllowToRunUnsandboxedCommands(args);
		const explicitUnsandboxRequest =
			isSandboxEnabled &&
			allowUnsandboxedCommands &&
			args.requestUnsandboxedExecution === true;
		let requiresUnsandboxConfirmation = explicitUnsandboxRequest;
		let requestUnsandboxedExecutionReason = explicitUnsandboxRequest
			? args.requestUnsandboxedExecutionReason
			: undefined;

		const missingDependencies =
			sandboxPrereqs.failedCheck ===
				TerminalSandboxPrerequisiteCheck.Dependencies &&
			sandboxPrereqs.missingDependencies?.length
				? sandboxPrereqs.missingDependencies
				: undefined;
		const sandboxRemediations =
			sandboxPrereqs.failedCheck ===
				TerminalSandboxPrerequisiteCheck.Bubblewrap &&
			sandboxPrereqs.remediations?.length
				? [...sandboxPrereqs.remediations]
				: undefined;
		const sandboxPrerequisiteFailure =
			sandboxPrereqs.failedCheck ===
				TerminalSandboxPrerequisiteCheck.Bubblewrap && !sandboxRemediations
				? localize(
		...
		if (
			this._shouldRejectUnsandboxedExecutionRequest(
				isSandboxEnabled,
				allowUnsandboxedCommands,
				args,
			)
			...
			return {
				invocationMessage: new MarkdownString(
					localize(
						...
						escapeMarkdownSyntaxTokens(
							buildCommandDisplayText(commandToDisplay),
						),
					),
				),
				icon: Codicon.error,
		...
		const rewriteResult = await this._rewriteCommandLine(args.command, {
			...
			os,
			isBackground: executionOptions.persistentSession,
			requestUnsandboxedExecution: allowUnsandboxedCommands
		...
		const forDisplayCommand: string | undefined =
			rewriteResult.forDisplayCommand;
		...
		requestUnsandboxedExecutionReason =
			rewriteResult.requestUnsandboxedExecutionReason;
		...
		const toolSpecificData: IChatTerminalToolInvocationData = {
			...
			commandLine: {
				original: args.command,
				toolEdited:
					rewrittenCommand === args.command ? undefined : rewrittenCommand,
				forDisplay:
					forDisplayCommand ??
					normalizeTerminalCommandForDisplay(rewrittenCommand ?? args.command),
				isSandboxWrapped,
			},
			cwd,
			language,
		...
		let sandboxPrerequisiteConfirmation: IToolConfirmationMessages | undefined =
			undefined;
		...
		if (missingDependencies) {
			...
			sandboxPrerequisiteConfirmation = {
				...
				customOptions: [
					{
						id: "install",
						label: localize("runInTerminal.missingDeps.install", "Install"),
						kind: ConfirmationOptionKind.Approve,
					},
					{
						id: "cancel",
						label: localize("runInTerminal.missingDeps.cancel", "Cancel"),
			...
			const customOptions = [];
			if (
				sandboxRemediations.includes(
					TerminalSandboxPreCheckRemediation.InstallUbuntuAppArmorProfile,
				)
			) {
				customOptions.push({
					id: TerminalSandboxPreCheckRemediation.InstallUbuntuAppArmorProfile,
					label: localize(
						"runInTerminal.bubblewrap.repairAppArmor",
						"Apply AppArmor Fix",
					),
					kind: ConfirmationOptionKind.Approve,
				});
			...
			if (
				...
				customOptions.push({
					id: TerminalSandboxPreCheckRemediation.DisableUbuntuUserNamespaceRestriction,
					label: localize(
						"runInTerminal.bubblewrap.disableRestriction",
						"Disable Restriction and Retry",
					),
					kind: ConfirmationOptionKind.Approve,
				});
			}
			customOptions.push({
				id: "cancel",
				label: localize("runInTerminal.bubblewrap.cancel", "Cancel"),
				kind: ConfirmationOptionKind.Deny,
			});
			sandboxPrerequisiteConfirmation = {
				title: localize(
				...
				message: new MarkdownString(
					sandboxRemediations.includes(
						...
						? localize(
								...
								"Bubblewrap is installed but cannot create the required sandbox namespace. Apply the recommended AppArmor fix, or disable Ubuntu's unprivileged user namespace restriction and retry. Disabling the restriction reduces system security.",
							)
						: localize(
								"runInTerminal.bubblewrap.disableOnly.message",
								"Bubblewrap is installed but cannot create the required sandbox namespace. You may disable Ubuntu's unprivileged user namespace restriction and retry. This reduces system security.",
							),
				),
				customOptions,
			};
		...
		const alternativeRecommendation = getRecommendedToolsOverRunInTerminal(
			args.command,
		...
		const commandLineAnalyzerOptions: ICommandLineAnalyzerOptions = {
			...
			os,
			shell,
			treeSitterLanguage: isPowerShell(shell, os)
			...
			hasSessionAutoApproval:
				!!chatSessionResource &&
				this._terminalChatService.hasChatSessionAutoApproval(
					chatSessionResource,
				),
		};
		...
		const commandLineAnalyzers = isSessionAutoApproved
			? this._commandLineAnalyzers.filter(
					(e) => !(e instanceof CommandLineAutoApproveAnalyzer),
				)
			: this._commandLineAnalyzers;
		const commandLineAnalyzerResults = await Promise.all(
			commandLineAnalyzers.map((e) => e.analyze(commandLineAnalyzerOptions)),
		);
		...
		if (disclaimersRaw.length > 0) {
			const disclaimerTexts = disclaimersRaw.map((d) =>
				typeof d === "string" ? d : d.value,
			);
			const hasMarkdownDisclaimer = disclaimersRaw.some(
				(d) => typeof d !== "string",
			);
			...
			disclaimer = new MarkdownString(
				`$(${Codicon.info.id}) ` + disclaimerTexts.join(" "),
				mdOptions,
			);
		}

		const analyzersIsAutoApproveAllowed = commandLineAnalyzerResults.every(
			(e) => e.isAutoApproveAllowed,
		);
		const customActions =
			isEligibleForAutoApproval() && analyzersIsAutoApproveAllowed
				? commandLineAnalyzerResults.map((e) => e.customActions ?? []).flat()
				: undefined;
		...
		const isAutoApprovedByRules =
			...
			wouldBeAutoApproved;
		const isUnsandboxedAutoApproved =
			isSandboxEnabled &&
			requiresUnsandboxConfirmation === true &&
			this._autoApproveUnsandboxedCommands;
		const isSandboxAutoApproved =
			isSandboxEnabled &&
			toolSpecificData.commandLine.isSandboxWrapped === true &&
			this._allowSandboxAutoApprove;
		const isFinalAutoApproved =
			isUnsandboxedAutoApproved ||
			isSandboxAutoApproved ||
			isAutoApprovedByRules ||
		...
		if (
			...
			(isAutoApproveEnabled &&
				commandLineAnalyzerResults.some((e) => e.autoApproveInfo))
		) {
			toolSpecificData.autoApproveInfo = commandLineAnalyzerResults.find(
				(e) => e.autoApproveInfo,
			)?.autoApproveInfo;
		}
		...
		const commandToDisplay = (
			toolSpecificData.commandLine.forDisplay ??
		...
		if (extractedCd && cwd) {
			...
			const cdPrefix = commandToDisplay.substring(
				0,
				commandToDisplay.length - extractedCd.command.length,
			);
		...
		for (const presenter of this._commandLinePresenters) {
			const presenterResult = await presenter.present({
				commandLine: { original: args.command, forDisplay: presenterInput },
				shell,
				os,
			});
			if (presenterResult) {
		...
		const shouldShowConfirmation =
			(!isFinalAutoApproved && !isSessionAutoApproved) ||
			context.forceConfirmationReason !== undefined;
		const explanation =
			args.explanation ||
			localize("runInTerminal.defaultExplanation", "No explanation provided");
		const goal =
			args.goal || localize("runInTerminal.defaultGoal", "No goal provided");
		...
		const rawDisplayCommand =
			toolSpecificData.commandLine.forDisplay ??
			toolSpecificData.commandLine.toolEdited ??
			toolSpecificData.commandLine.original;
		const displayCommand =
			rawDisplayCommand.length > 80
		...
		return {
			invocationMessage,
			icon: toolSpecificData.commandLine.isSandboxWrapped
				? Codicon.terminalSecure
				: Codicon.terminal,
			confirmationMessages:
				sandboxPrerequisiteConfirmation ?? confirmationMessages,
			toolSpecificData,
		};
	}
	...
	private _formatBlockedDomainsForTitle(blockedDomains: string[]): string {
		...
		return localize(
			"runInTerminal.unsandboxed.domain.summary",
	...
	private _getBlockedDomainReason(
		...
		if (
			deniedDomains.length === blockedDomains.length &&
		...
		if (deniedDomains.length > 0) {
			if (blockedDomains.length === 1) {
				return localize(
					"runInTerminal.unsandboxed.domain.reason.mixed.single",
			...
			return localize(
				"runInTerminal.unsandboxed.domain.reason.mixed.multi",
	...
	private async _rewriteCommandLine(
		commandLine: string,
		options: {
			cwd: URI | undefined;
			shell: string;
		...
		let requestUnsandboxedExecutionReason = options.requestUnsandboxedExecution
			? options.requestUnsandboxedExecutionReason
			: undefined;
		...
		for (const rewriter of this._commandLineRewriters) {
			...
			if (rewriteResult) {
				...
				if (rewriteResult.blockedDomains?.length) {
					...
					requestUnsandboxedExecutionReason = this._getBlockedDomainReason(
						rewriteResult.blockedDomains,
						rewriteResult.deniedDomains,
					);
				}
				this._logService.info(
	...
	private _getSandboxPrecheckInputs(
		chatSessionResource: URI | undefined,
		chatRequestId: string | undefined,
	): ITerminalSandboxPrecheckInputs | undefined {
		return getSandboxPrecheckInputsForToolInvocation(
			chatSessionResource,
			chatRequestId,
			this._chatWidgetService,
			this._chatService,
	...
	private async _confirmAutomaticUnsandboxRetry(
		...
		const chatModel =
			sessionResource && this._chatService.getSession(sessionResource);
	...
	private _registerSensitiveInputElicitation(
		...
		store.add(
			outputMonitor.onDidDetectSensitiveInputNeeded(() => {
				...
				if (!(chatModel instanceof ChatModel)) {
					...
					this._terminalService
						.revealTerminal(terminalInstance, true)
						.catch(() => {});
				...
				const part = new ChatElicitationRequestPart(
					...
					async () => {
						...
						try {
							this._terminalService.setActiveInstance(terminalInstance);
							await this._terminalService.revealTerminal(
								terminalInstance,
								true,
							);
							terminalInstance.focus();
							...
							this._logService.warn(
								`RunInTerminalTool: failed to reveal terminal for sensitive input`,
								err,
							);
						}
						return ElicitationState.Accepted;
	...
	async invoke(
		...
		const toolSpecificData = invocation.toolSpecificData as
		...
		if (toolSpecificData.sandboxPrerequisiteFailure) {
			return {
				content: [
					{ kind: "text", value: toolSpecificData.sandboxPrerequisiteFailure },
				],
			};
		}
		...
		if (toolSpecificData.missingSandboxDependencies?.length) {
			if (invocation.selectedCustomButton === "install") {
				...
				const refreshedPrereqs =
					await this._terminalSandboxService.checkForSandboxingPrereqs(
						true,
						sandboxPrecheckInputs,
					);
						...
						content: [
							{
								kind: "text",
								value:
									refreshedPrereqs.failedCheck ===
										TerminalSandboxPrerequisiteCheck.Bubblewrap &&
									refreshedPrereqs.remediations?.length
										? localize(
				...
				this._logService.info(
					"RunInTerminalTool: User cancelled sandbox dependency installation",
				);
				return {
					content: [
						{
		...
		if (toolSpecificData.sandboxRemediations?.length) {
			const selectedRemediation = invocation.selectedCustomButton as
				| TerminalSandboxPreCheckRemediation
				| undefined;
			if (
				!selectedRemediation ||
				!toolSpecificData.sandboxRemediations.includes(selectedRemediation)
			) {
				return {
					content: [
						{
					...
					content: [
						{
							...
							value:
								...
								TerminalSandboxPreCheckRemediation.InstallUbuntuAppArmorProfile
									? localize(
											"runInTerminal.bubblewrap.profileDidNotResolve",
											"The AppArmor repair completed, but bubblewrap still cannot create the required sandbox namespace. Run the command again and choose Disable Restriction and Retry only if you accept the reduced system security.",
										)
									: localize(
		...
		this._logService.debug(
			`RunInTerminalTool: Invoking with options ${JSON.stringify(args)}`,
		);
		let toolResultMessage: string | IMarkdownString | undefined;
		if (
			args.timeout !== undefined &&
			(Number.isNaN(args.timeout) || args.timeout < 0)
		) {
			return {
				content: [
					{
						kind: "text",
						value:
							"Error: timeout must be a non-negative number of milliseconds (use 0 for no timeout).",
					},
				],
			};
		}
		...
		const isNewSession =
			...
			!this._sessionTerminalAssociations.has(chatSessionResource);

		const timingStart = Date.now();
		const termId = generateUuid();
		const terminalToolSessionId = (
		...
		let pollingResult:
			| (IPollingResult & { pollDurationMs: number })
			| undefined;
		...
		if (terminalToolSessionId) {
			store.add(
				this._terminalChatService.onDidContinueInBackground((sessionId) => {
		...
		try {
			...
			const startMarkerPromise = Event.toPromise(
				execution.strategy.onDidCreateStartMarker,
			);
			const outputMonitorPollFn = executionOptions.persistentSession
				...
				: undefined;
			store.add(
				execution.strategy.onDidCreateStartMarker((startMarker) => {
					if (!outputMonitor) {
						outputMonitor = this._instantiationService.createInstance(
							OutputMonitor,
							{
								...
								getOutput: (marker?: IXtermMarker) =>
									execution.getOutput(marker ?? startMarker),
							},
							outputMonitorPollFn,
							invocation.context,
							token,
			...
			executionPromise = execution.start(
				command,
			...
			if (executionOptions.waitStrategy === "idle") {
				...
				if (outputMonitor) {
					if (timeoutRacePromise) {
						const idleRace = await Promise.race([
							Event.toPromise(outputMonitor.onDidFinishCommand).then(() => ({
								type: "idle" as const,
							})),
							timeoutRacePromise,
						]);
				...
				await this._commandArtifactCollector.capture(
					toolSpecificData,
				...
				let resultText = didSandboxWrapCommand
					? `Command is now running in terminal with ID=${termId}`
					: didUserEditCommand
				...
				const backgroundOutput =
					...
					(idleTimedOut ? execution.getOutput() : undefined);
				const outputAnalyzerMessage = backgroundOutput
					? await this._getOutputAnalyzerMessage(
							undefined,
							backgroundOutput,
							command,
				...
				if (idleTimedOut) {
					...
					pollingResult &&
					pollingResult.state === OutputMonitorState.Idle
				) {
				...
				startMarkerPromise.then(() => {
					if (outputMonitor && !raceCleanup.isDisposed) {
						raceCleanup.add(
							this._registerSensitiveInputElicitation(
								chatSessionResource,
								toolTerminal.instance,
								outputMonitor,
				...
				const raceCandidates: Promise<
					| { type: "completed"; result: ITerminalExecuteStrategyResult }
					| { type: "background" }
					| { type: "timeout" }
					| { type: "inputNeeded" }
					| { type: "idleSilence" }
				>[] = [
					...
					new Promise<{ type: "inputNeeded" }>((resolve) => {
						startMarkerPromise.then(() => {
							if (outputMonitor && !raceCleanup.isDisposed) {
								raceCleanup.add(
									outputMonitor.onDidDetectInputNeeded(() =>
										resolve({ type: "inputNeeded" as const }),
									),
								);
							}
						});
				...
				if (idleSilenceMs > 0) {
					...
					const idleSilenceScheduler = raceCleanup.add(
						new RunOnceScheduler(
							() =>
								idleSilenceDeferred.complete({ type: "idleSilence" as const }),
							idleSilenceMs,
						),
					);
					raceCleanup.add(
						toolTerminal.instance.onData(() => idleSilenceScheduler.schedule()),
					);
				...
				if (raceResult.type === "inputNeeded") {
					...
					outputLineCount = backgroundOutput
						? count(backgroundOutput.trim(), "\n") + 1
						: 0;
					...
					this._logService.debug(
						`RunInTerminalTool: Timeout reached, returning output collected so far`,
					);
					error = "timeout";
					didTimeout = true;
					isBackgroundExecution = true;
					...
					outputLineCount = timeoutOutput
						? count(timeoutOutput.trim(), "\n") + 1
						: 0;
					...
					outputLineCount = idleSilenceOutput
						? count(idleSilenceOutput.trim(), "\n") + 1
						: 0;
					...
					if (executeResult.didEnterAltBuffer) {
						...
						);
						{
							...
							if (executeResult.exitCode !== undefined) {
								...
								if (state.timestamp !== undefined) {
									state.duration =
										state.duration ?? Math.max(0, Date.now() - state.timestamp);
								}
							}
							toolSpecificData.terminalCommandState = state;
						}
						...
						outputLineCount =
							executeResult.output === undefined
								? 0
								: count(executeResult.output.trim(), "\n") + 1;
			...
			if (didTimeout && e instanceof CancellationError) {
				...
				error =
					e instanceof CancellationError ? "canceled" : "unexpectedException";
				throw e;
			}
		} finally {
			...
			if ((isBackgroundExecution || didInputNeeded) && executionPromise) {
				...
				executionPromise.catch((e: unknown) => {
					if (!(e instanceof CancellationError)) {
						this._logService.error(
							`RunInTerminalTool: Background execution error`,
							e,
						);
					}
				});
				...
				if (shouldSendNotifications) {
					...
					const alreadyNotifiedInputNeededOutput = didInputNeeded
						? terminalResult
						: undefined;
			...
			this._telemetry.logInvoke(toolTerminal.instance, {
				...
				inputToolManualAcceptCount:
					outputMonitor?.outputMonitorTelemetryCounters
						?.inputToolManualAcceptCount,
				inputToolManualRejectCount:
					outputMonitor?.outputMonitorTelemetryCounters
						?.inputToolManualRejectCount,
				inputToolManualChars:
					outputMonitor?.outputMonitorTelemetryCounters?.inputToolManualChars,
				inputToolAutoAcceptCount:
					outputMonitor?.outputMonitorTelemetryCounters
						?.inputToolAutoAcceptCount,
				inputToolAutoChars:
					outputMonitor?.outputMonitorTelemetryCounters?.inputToolAutoChars,
		...
		if (shouldAutoRetryUnsandboxed) {
			...
			const retryRewriteResult = await this._rewriteCommandLine(args.command, {
				cwd: toolSpecificData.cwd
					? URI.revive(toolSpecificData.cwd)
					: undefined,
				shell,
				os,
				isBackground: executionOptions.persistentSession,
			...
			const rewrittenRetryReason =
				retryRewriteResult.requestUnsandboxedExecutionReason ?? retryReason;
			const retryConfirmationCommand =
				toolSpecificData.presentationOverrides?.commandLine ?? command;
			...
			if (shouldRetry) {
				const retryToolSpecificData: IChatTerminalToolInvocationData = {
					...
					commandLine: {
						original: args.command,
						toolEdited:
							retryRewriteResult.rewrittenCommand === args.command
								? undefined
								: retryRewriteResult.rewrittenCommand,
						forDisplay:
							retryRewriteResult.forDisplayCommand ??
							normalizeTerminalCommandForDisplay(
								retryRewriteResult.rewrittenCommand ?? args.command,
							),
		...
		if (!didSandboxWrapCommand) {
			if (didUserEditCommand) {
				...
				const wasDetachedToBackground = /(^|\s)nohup\s|Start-Process\b/.test(
					command,
				);
		...
		const outputAnalyzerMessage = await this._getOutputAnalyzerMessage(
			exitCode,
			terminalResult,
			command,
			didSandboxWrapCommand,
		...
		const imageContent = await this._extractImagesFromOutput(
			terminalResult,
			endCwd,
		);
	...
	private async _getOutputAnalyzerMessage(
	 ...
	 */
	private async _extractImagesFromOutput(
		output: string,
		cwd: URI | undefined,
	): Promise<IToolResult["content"]> {
		...
		for (const filePath of matches) {
			try {
				...
				const stat = await this._fileService
					.stat(fileUri)
					.catch(() => undefined);
				if (
					!stat ||
					stat.isDirectory ||
					stat.size > RunInTerminalTool._maxImageFileSize
				) {
					continue;
				}
	...
	private _handleTerminalVisibility(
		...
		const chatSessionOpenInWidget =
			!!this._chatWidgetService.getWidgetBySessionResource(chatSessionResource);
		if (
			this._configurationService.getValue(
				TerminalChatAgentToolsSettingId.OutputLocation,
			) === "terminal" &&
			chatSessionOpenInWidget
		) {
	 ...
	 * parallel execution.
	 */
	private async _initTerminal(
		chatSessionResource: URI,
		termId: string,
		terminalToolSessionId: string | undefined,
		isBackground: boolean,
		token: CancellationToken,
	): Promise<IToolTerminal> {
		...
		if (!isBackground) {
			...
			if (
				...
				if (cachedTerminal.instance.exitCode !== undefined) {
					...
					this._terminalToolCreator.refreshShellIntegrationQuality(
						cachedTerminal,
					);
					this._terminalChatService.registerTerminalInstanceWithToolSession(
						terminalToolSessionId,
						cachedTerminal.instance,
					);
					...
					this._backgroundNotifications.deleteAndDispose(
						cachedTerminal.instance.instanceId,
					);
		...
		const toolTerminal = await this._terminalToolCreator.createTerminal(
			profile,
			os,
			token,
		);
		...
		await this._setupProcessIdAssociation(
			toolTerminal,
			chatSessionResource,
			termId,
			isBackground,
	...
	private _restoreTerminalAssociations(): void {
		...
		try {
			const associations: Record<number, IStoredTerminalAssociation> =
				JSON.parse(storedAssociations);
			...
			for (const instance of this._terminalService.instances) {
				if (instance.processId) {
					...
					if (association) {
						...
						const chatSessionResource = LocalChatSessionUri.forSession(
						...
						this._addSessionTerminalAssociation(
							chatSessionResource,
							toolTerminal,
						);
						this._terminalChatService.registerTerminalInstanceWithChatSession(
						...
						if (association.id) {
							RunInTerminalTool._activeExecutions.set(
								association.id,
								this._register(new RestoredTerminalExecution(instance)),
							);
						}
			...
			this._logService.debug(
				`RunInTerminalTool: Failed to restore terminal associations: ${error}`,
			);
		}
	}

	private async _setupProcessIdAssociation(
		toolTerminal: IToolTerminal,
		chatSessionResource: URI,
		termId: string,
		isBackground: boolean,
	...
	private async _associateProcessIdWithSession(
		...
		try {
			...
			this._logService.debug(
				`RunInTerminalTool: Failed to associate terminal with session: ${error}`,
			);
		}
	}
	...
	private async _removeProcessIdAssociation(pid: number): Promise<void> {
		try {
			...
			if (associations[pid]) {
				...
				this._logService.debug(
					`RunInTerminalTool: Removed terminal association for PID ${pid}`,
				);
			}
		} catch (error) {
			this._logService.debug(
				`RunInTerminalTool: Failed to remove terminal association: ${error}`,
			);
		}
	}
	...
	private _cleanupSessionTerminals(chatSessionResource: URI): void {
		const sessionTerminals =
			this._sessionTerminalInstances.get(chatSessionResource);
		const toolTerminal =
			this._sessionTerminalAssociations.get(chatSessionResource);
		const terminalsToDispose =
			sessionTerminals ??
			(toolTerminal ? new Set([toolTerminal.instance]) : undefined);
		...
		for (const [
			termId,
			execution,
		] of RunInTerminalTool._activeExecutions.entries()) {
	...
	private _ensureArchivedSessionListener(): void {
		...
		this._archivedSessionListener.value =
			this._agentSessionsService.onDidChangeSessionArchivedState((session) => {
	...
	private _removeTerminalAssociations(terminal: ITerminalInstance): void {
		...
		for (const [
			sessionResource,
			toolTerminal,
		] of this._sessionTerminalAssociations.entries()) {
		...
		for (const [
			sessionResource,
			sessionTerminals,
		] of this._sessionTerminalInstances.entries()) {
	...
	private _registerCompletionNotification(
		...
		const commandDetection = terminalInstance.capabilities.get(
			TerminalCapability.CommandDetection,
		);
		if (!commandDetection) {
			outputMonitor?.dispose();
		...
		const commandDisplay = appendEscapedMarkdownInlineCode(
			buildCompletionNotificationCommand(commandName),
		);
		...
		const sessionRef = this._chatService.acquireExistingSession(
			chatSessionResource,
			"RunInTerminalTool#completionNotification",
		);
		if (!sessionRef) {
			this._logService.warn(
		...
		if (lastRequest) {
			...
			if (lastRequest.userSelectedTools) {
				sendOptions.userSelectedTools = constObservable(
					lastRequest.userSelectedTools,
				);
			}
		}
		...
		store.add(
			autorun((reader) => {
				...
				reader.store.add(
					request.response.onDidChange((ev) => {
						if (
							ev.reason === "completedRequest" &&
							request.response!.isCanceled
						) {
							disposeNotification();
						}
					}),
		...
		if (outputMonitor) {
			...
			let lastInputNeededNotificationTime =
				alreadyNotifiedInputNeededOutput !== undefined ? Date.now() : 0;
			...
			store.add(
				outputMonitor.onDidDetectInputNeeded(() => {
					...
					const isDuplicate =
						...
						now - lastInputNeededNotificationTime <
							INPUT_NEEDED_NOTIFICATION_THROTTLE_MS;
					if (isDuplicate) {
						return;
					}
					lastInputNeededOutput = currentOutput;
					lastInputNeededNotificationTime = now;
					const inputAction = this._buildInputNeededSteeringText(
						chatSessionResource,
						termId,
						"none",
					);
					...
					this._chatService
						.sendRequest(chatSessionResource, message, {
							...sendOptions,
		...
		store.add(
			terminalInstance.onDidInputData(() => {
				if (userIsReplyingDirectly) {
					return;
				}
		...
		store.add(
			commandDetection.onCommandFinished((command) => {
				...
				const isUserVisible =
					this._terminalService.foregroundInstances.includes(terminalInstance);
				...
				this._commandArtifactCollector
					.capture(toolSpecificData, terminalInstance, command.id)
					.then(() => {
						if (
							this._terminalService.foregroundInstances.includes(
								terminalInstance,
							)
						) {
							this._logService.debug(
						...
						this._logService.debug(
							`RunInTerminalTool: Disposing finished background terminal ${termId}`,
						);
		...
		// _activeExecutions.
		const executionForDisposal =
			RunInTerminalTool._activeExecutions.get(termId);
		store.add(
		...
		store.add(
			sessionRef.object.onDidChange((e) => {
	...
	private _dismissPendingCarouselsForTerminal(
		...
		for (let i = requests.length - 1; i >= 0; i--) {
			...
			for (let j = parts.length - 1; j >= 0; j--) {
				const part = parts[j];
				if (
					part instanceof ChatQuestionCarouselData &&
					part.terminalId === termId &&
					!part.isUsed
				) {
					this._logService.debug(
...
class ActiveTerminalExecution
	extends Disposable
	implements IActiveTerminalExecution
{
	...
	constructor(
		...
		this._completionDeferred =
			new DeferredPromise<ITerminalExecuteStrategyResult>();
		...
		this._register(
			this.strategy.onDidCreateStartMarker((marker) => {
				if (marker) {
...
class RestoredTerminalExecution
	extends Disposable
	implements IActiveTerminalExecution
{
	readonly completionPromise: Promise<ITerminalExecuteStrategyResult> =
...
export class TerminalProfileFetcher {
	...
	constructor(
		...
		this.osBackend = this._remoteAgentService
			.getEnvironment()
			.then((remoteEnv) => remoteEnv?.os ?? OS);
	}
	...
	async getCopilotProfile(): Promise<ITerminalProfile> {
		...
		const defaultProfile =
			await this._terminalProfileResolverService.getDefaultProfile({
				os,
				remoteAuthority:
					this._remoteAgentService.getConnection()?.remoteAuthority,
			});
