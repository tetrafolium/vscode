...
export class GetTerminalOutputTool extends Disposable implements IToolImpl {
	...
	private readonly _lastOutputSnapshotByExecutionId = new Map<
		string,
		IOutputSnapshot
	>();
	...
	constructor(
		...
		this._register(
			terminalService.onDidDisposeInstance((instance) =>
				this._forgetTerminalInstance(instance.instanceId),
			),
		);
	}
	...
	async prepareToolInvocation(
		...
		return {
			invocationMessage: localize(
				"getTerminalOutput.progressive",
				"Checking terminal output",
			),
			pastTenseMessage: localize(
				"getTerminalOutput.past",
	...
	private _formatOutput(
		...
		const previousOutputSnapshot =
			this._lastOutputSnapshotByExecutionId.get(id);
		const currentOutputSnapshot = this._createOutputSnapshot(
			terminalInstanceId,
			output,
		);
		...
		if (
			output.length > previousOutputSnapshot.length &&
			this._hashOutput(output, previousOutputSnapshot.length) ===
				previousOutputSnapshot.hash
		) {
	...
	private _rememberOutput(id: string, snapshot: IOutputSnapshot): void {
		if (
			!this._lastOutputSnapshotByExecutionId.has(id) &&
			this._lastOutputSnapshotByExecutionId.size >=
				GetTerminalOutputTool._maxOutputSnapshots
		) {
			const oldestId = this._lastOutputSnapshotByExecutionId
				.keys()
				.next().value;
