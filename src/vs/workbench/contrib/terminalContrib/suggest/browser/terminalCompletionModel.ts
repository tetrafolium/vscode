...
const compareCompletionsFn = (
	leadingLineContent: string,
	a: TerminalCompletionItem,
	b: TerminalCompletionItem,
) => {
	...
	if (
		a.completion.kind === TerminalCompletionItemKind.InlineSuggestion &&
		a.completion.kind !== b.completion.kind
	) {
		return -1;
	}
	if (
		b.completion.kind === TerminalCompletionItemKind.InlineSuggestion &&
		a.completion.kind !== b.completion.kind
	) {
		return 1;
	}
	...
	if (
		...
		const aLabel = isString(a.completion.label)
			? a.completion.label
			: a.completion.label.label;
		const bLabel = isString(b.completion.label)
			? b.completion.label
			: b.completion.label.label;
	...
	if (
		...
		if (a.labelLowNormalizedPath && b.labelLowNormalizedPath) {
			...
			score =
				count(a.labelLowNormalizedPath, "/") -
				count(b.labelLowNormalizedPath, "/");
			if (score !== 0) {
				return score;
			}
	...
	if (a.completion.kind !== b.completion.kind) {
		...
		if (
			isResourceKind(a.completion.kind) &&
			!isResourceKind(b.completion.kind)
		) {
