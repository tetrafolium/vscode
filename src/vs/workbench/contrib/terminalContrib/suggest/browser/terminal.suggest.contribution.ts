	...
	implements ITerminalContribution
{
	...
	private readonly _lspAddons: DisposableMap<
		string,
		LspCompletionProviderAddon
	> = this.add(new DisposableMap());
	private readonly _lspModelProvider: MutableDisposable<LspTerminalModelContentProvider> =
		new MutableDisposable();
	...
	constructor(
		...
		TerminalSuggestProvidersConfigurationManager.initialize(
	...
	xtermOpen(xterm: IXtermTerminal & { raw: RawXtermTerminal }): void {
		...
		this.add(
			Event.runAndSubscribe(
				...
				async () => {
					this._refreshAddons();
					this._lspModelProvider.value?.shellTypeChanged(
	...
	private async _loadLspCompletionAddon(
		xterm: RawXtermTerminal,
	): Promise<void> {
		let lspTerminalObj = undefined;
		// TODO: Change to always load after settings update for terminal suggest provider
		if (
			!this._ctx.instance.shellType ||
			!(lspTerminalObj = getTerminalLspSupportedLanguageObj(
				this._ctx.instance.shellType,
			))
		) {
			this._lspAddons.clearAndDisposeAll();
		...
		const textVirtualModel = await this._textModelService.createModelReference(
			virtualTerminalDocumentUri,
		);
		this.add(textVirtualModel);

		const virtualProviders =
			this._languageFeaturesService.completionProvider.all(
				textVirtualModel.object.textEditorModel,
			);
		const filteredProviders = virtualProviders.filter(
			(p) => p._debugDisplayName !== "wordbasedCompletions",
		);

		// Iterate through all available providers
		for (const provider of filteredProviders) {
			const lspCompletionProviderAddon =
	...
	private _updateContainerForTarget(
	...
	private async _prepareAddonLayout(xterm: RawXtermTerminal): Promise<void> {
		...
		const xtermElement =
			xterm.element ?? (await this._waitForXtermElement(xterm));
	...
	private async _waitForXtermElement(
		xterm: RawXtermTerminal,
		...
		await Promise.race([
			Event.toPromise(
				Event.filter(
					this._ctx.instance.onDidChangeVisibility,
					(visible) => visible,
				),
			),
...
registerTerminalAction({
	...
	precondition: ContextKeyExpr.and(
		...
		ContextKeyExpr.equals(
			`config.${TerminalSuggestSettingId.SelectionMode}`,
			"never",
		),
	),
	menu: {
		...
		when: ContextKeyExpr.and(
			...
			ContextKeyExpr.or(
				ContextKeyExpr.equals(
					`config.${TerminalSuggestSettingId.QuickSuggestions}`,
					true,
				),
				ContextKeyExpr.equals(
					`config.${TerminalSuggestSettingId.SuggestOnTriggerCharacters}`,
					true,
				),
			),
		),
	...
	run: (c, accessor) => {
		accessor
			...
			.updateValue(TerminalSuggestSettingId.SelectionMode, "partial");
	},
});
registerTerminalAction({
	...
	run: (c, accessor) => {
		accessor
			.get(IConfigurationService)
			.updateValue(TerminalSuggestSettingId.SelectionMode, "always");
	},
});
registerTerminalAction({
	...
	run: (c, accessor) => {
		accessor
			.get(IConfigurationService)
			.updateValue(TerminalSuggestSettingId.SelectionMode, "never");
	},
});
...
registerTerminalAction({
	...
	menu: {
		...
		when: ContextKeyExpr.and(
			ContextKeyExpr.or(
				ContextKeyExpr.equals(
					`config.${TerminalSuggestSettingId.QuickSuggestions}.commands`,
					"on",
				),
				ContextKeyExpr.equals(
					`config.${TerminalSuggestSettingId.QuickSuggestions}.arguments`,
					"on",
				),
			),
			ContextKeyExpr.equals(
	...
	run: (c, accessor) => {
		accessor
			.get(IConfigurationService)
			.updateValue(TerminalSuggestSettingId.QuickSuggestions, {
				commands: "off",
				arguments: "off",
				unknown: "off",
			});
		accessor
			.get(IConfigurationService)
			.updateValue(TerminalSuggestSettingId.SuggestOnTriggerCharacters, false);
	},
});
...
registerTerminalAction({
	...
	run: (c, accessor) => {
		...
		accessor
			.get(IConfigurationService)
			.updateValue(TerminalSuggestSettingId.SuggestOnTriggerCharacters, true);
	},
});
...
registerTerminalAction({
	...
	run: (c, accessor) => {
		accessor
			.get(IOpenerService)
			.open("https://aka.ms/vscode-terminal-intellisense");
	},
});
...
registerActiveInstanceAction({
	...
	run: (activeInstance) =>
		TerminalSuggestContribution.get(activeInstance)?.addon?.requestCompletions(
			true,
		),
});
...
registerActiveInstanceAction({
	...
	run: (activeInstance) =>
		TerminalSuggestContribution.get(activeInstance)?.addon?.resetWidgetSize(),
});
...
registerActiveInstanceAction({
	...
	run: (activeInstance) =>
		TerminalSuggestContribution.get(activeInstance)?.addon?.toggleExplainMode(),
});
...
registerActiveInstanceAction({
	...
	run: (activeInstance) =>
		TerminalSuggestContribution.get(activeInstance)?.addon?.hideSuggestWidget(
			true,
		),
});
...
class TerminalSuggestProvidersConfigurationManager extends Disposable {
	...
	static initialize(instantiationService: IInstantiationService): void {
		if (!this._instance) {
			this._instance = instantiationService.createInstance(
				TerminalSuggestProvidersConfigurationManager,
			);
		}
	}
