...
export const TERMINAL_SUGGEST_DISCOVERABILITY_KEY =
	"terminal.suggest.increasedDiscoverability";
export const TERMINAL_SUGGEST_DISCOVERABILITY_COUNT_KEY =
	"terminal.suggest.increasedDiscoverabilityCount";
...
export class TerminalSuggestShownTracker
	extends Disposable
	implements ITerminalSuggestShownTracker
{
	...
	private _firstShownTracker:
		| { shell: Set<TerminalShellType | undefined>; window: boolean }
		| undefined = undefined;
	...
	constructor(
		...
		super();
		this._done = this._storageService.getBoolean(
	...
	update(widgetElt: HTMLElement | undefined): void {
		...
		if (
			widgetElt &&
			!widgetElt.classList.contains("increased-discoverability")
		) {
