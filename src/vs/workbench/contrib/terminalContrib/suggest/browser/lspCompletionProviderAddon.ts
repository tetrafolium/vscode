	...
	implements ITerminalAddon, ITerminalCompletionProvider
{
	...
	> {
		...
		const lineNum =
			this._textVirtualModel.object.textEditorModel.getLineCount();
		const positionVirtualDocument = new Position(lineNum, column);

		const completions: ITerminalCompletion[] = [];
		if (
			this._provider &&
			this._provider._debugDisplayName !== "wordbasedCompletions"
		) {
			...
			for (const item of result?.suggestions || []) {
				...
				const convertedKind = item.kind
					? mapLspKindToTerminalKind(item.kind)
