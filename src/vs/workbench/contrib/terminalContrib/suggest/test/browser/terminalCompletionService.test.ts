...
} from "../../browser/terminalCompletionService.js";
...
} from "../../browser/terminalGitBashHelpers.js";
...
function assertCompletions(
	actual: ITerminalCompletion[] | undefined,
	expected: IAssertionTerminalCompletion[],
	expectedConfig: IAssertionCommandLineConfig,
	pathSep?: string,
) {
 ...
 */
function assertPartialCompletionsExist(
	actual: ITerminalCompletion[] | undefined,
	expectedPartial: IAssertionTerminalCompletion[],
	expectedConfig: IAssertionCommandLineConfig,
) {
	if (!actual) {
		fail();
	...
	for (const expectedItem of expectedMapped) {
		assert.deepStrictEqual(
			actual
				...
				.find((e) => e.detail === expectedItem.detail),
			expectedItem,
		);
	}
}
...
suite("TerminalCompletionService", () => {
	...
	setup(() => {
		...
		const doesResourceExist = (resource: URI) =>
			validResources.some(
				(e) => normalizePath(e.path) === normalizePath(resource.path),
			) ||
			childResources.some(
				(e) => normalizePath(e.resource.path) === normalizePath(resource.path),
			);
		...
		terminalCompletionService = store.add(
			instantiationService.createInstance(TerminalCompletionService),
		);
	...
	suite("resolveResources should return folder completions", () => {
		setup(() => {
			...
			childResources = [
				{
					resource: URI.parse("file:///test/folder1/"),
					isDirectory: true,
					isFile: false,
				},
				{
					resource: URI.parse("file:///test/file1.txt"),
					isDirectory: false,
					isFile: true,
				},
			];
	...
	suite(
		"resolveResources should handle file and folder completion requests correctly",
		() => {
			setup(() => {
				validResources = [URI.parse("file:///test")];
				childResources = [
					{
						resource: URI.parse("file:///test/.hiddenFile"),
						isFile: true,
						executable: true,
					},
					{
						resource: URI.parse("file:///test/.hiddenFolder/"),
						isDirectory: true,
					},
	...
	suite("~ -> $HOME", () => {
		...
		setup(() => {
			...
			childResources = [
				...
				{
					resource: URI.parse("file:///home/vscode/bar.txt"),
					isFile: true,
					executable: true,
				},
			];
		...
		test("~| should return completion for ~", async () => {
			assertPartialCompletionsExist(
				await terminalCompletionService.resolveResources(
					resourceOptions,
					"~",
					1,
					provider,
	...
	suite("resolveResources edge cases and advanced scenarios", () => {
		...
		if (isWindows) {
			test("C:/Foo/| absolute paths on Windows", async () => {
				...
				childResources = [
					{
						resource: URI.parse("file:///C:/Foo/Bar"),
						isDirectory: true,
						isFile: false,
					},
					{
						resource: URI.parse("file:///C:/Foo/Baz.txt"),
						isDirectory: false,
						isFile: true,
					},
				];
			...
			test("/foo/| absolute paths NOT on Windows", async () => {
				...
				childResources = [
					{
						resource: URI.parse("file:///foo/Bar"),
						isDirectory: true,
						isFile: false,
					},
					{
						resource: URI.parse("file:///foo/Baz.txt"),
						isDirectory: false,
						isFile: true,
					},
				];
		...
		if (isWindows) {
			test(".\\folder | Case insensitivity should resolve correctly on Windows", async () => {
				...
				childResources = [
					{
						resource: URI.parse("file:///C:/test/FolderA/"),
						isDirectory: true,
					},
					{
						resource: URI.parse("file:///C:/test/anotherFolder/"),
						isDirectory: true,
					},
				];
	...
	suite("cdpath", () => {
		...
		setup(() => {
			...
			childResources = [
				{
					resource: URI.parse("file:///cdpath_value/folder1/"),
					isDirectory: true,
				},
		...
		test("cd | should support pulling from multiple paths in $CDPATH", async () => {
			...
			childResources = [
				{
					resource: URI.parse(`${uriPathPrefix}cdpath1_value/folder1/`),
					isDirectory: true,
				},
				{
					resource: URI.parse(`${uriPathPrefix}cdpath1_value/folder2/`),
					isDirectory: true,
				},
				{
					resource: URI.parse(`${uriPathPrefix}cdpath1_value/file1.txt`),
					isFile: true,
				},
				{
					resource: URI.parse(
						`${uriPathPrefix}cdpath2_value/inner_dir/folder1/`,
					),
					isDirectory: true,
				},
				{
					resource: URI.parse(
						`${uriPathPrefix}cdpath2_value/inner_dir/folder2/`,
					),
					isDirectory: true,
				},
				{
					resource: URI.parse(
						`${uriPathPrefix}cdpath2_value/inner_dir/file1.txt`,
					),
					isFile: true,
				},
			];
			...
			assertPartialCompletionsExist(
				...
				[
					{
						label: "folder1",
						detail: `CDPATH ${finalPrefix}cdpath1_value/folder1/`,
					},
					{
						label: "folder2",
						detail: `CDPATH ${finalPrefix}cdpath1_value/folder2/`,
					},
					{
						label: "folder1",
						detail: `CDPATH ${finalPrefix}cdpath2_value/inner_dir/folder1/`,
					},
					{
						label: "folder2",
						detail: `CDPATH ${finalPrefix}cdpath2_value/inner_dir/folder2/`,
					},
				],
				{ replacementRange: [3, 3] },
			);
	...
	if (isWindows) {
		suite("gitbash", () => {
			test("should convert Git Bash absolute path to Windows absolute path", () => {
				...
				assert.strictEqual(
					gitBashToWindowsPath("/c/Users/foo"),
					"C:\\Users\\foo",
				);
			...
			test("should convert Windows absolute path to Git Bash absolute path", () => {
				...
				assert.strictEqual(
					windowsToGitBashPath("C:\\Users\\foo"),
					"/c/Users/foo",
				);
				assert.strictEqual(windowsToGitBashPath("D:\\bar"), "/d/bar");
				assert.strictEqual(
					windowsToGitBashPath("E:\\some\\path"),
					"/e/some/path",
				);
			});
			...
			test("resolveResources with c:/ style absolute path for Git Bash", async () => {
				...
				childResources = [
					{
						resource: URI.file("C:\\Users\\foo\\bar"),
						isDirectory: true,
						isFile: false,
					},
					{
						resource: URI.file("C:\\Users\\foo\\baz.txt"),
						isFile: true,
						executable: true,
					},
				];
				...
				assertCompletions(
					...
					[
						...
						{
							label: "C:/Users/foo/baz.txt",
							detail: "C:\\Users\\foo\\baz.txt",
							kind: TerminalCompletionItemKind.File,
						},
					],
	...
	if (!isWindows) {
		suite("symlink support", () => {
			test("should include symlink target information in completions", async () => {
				...
				childResources = [
					{
						resource: URI.parse("file:///test/regular-file.txt"),
						isFile: true,
					},
					{
						resource: URI.parse("file:///test/symlink-file"),
						isFile: true,
						isSymbolicLink: true,
					},
					{
						resource: URI.parse("file:///test/symlink-folder"),
						isDirectory: true,
						isSymbolicLink: true,
					},
					{
						resource: URI.parse("file:///test/regular-folder"),
						isDirectory: true,
					},
				];
				...
				const symlinkFileCompletion = result?.find(
					(c) => c.label === "./symlink-file",
				);
				const symlinkFolderCompletion = result?.find(
					(c) => c.label === "./symlink-folder/",
				);
				assert.strictEqual(
					symlinkFileCompletion?.detail,
					"/test/symlink-file -> /target/actual-file.txt",
					"Symlink file detail should match target",
				);
				assert.strictEqual(
					symlinkFolderCompletion?.detail,
					"/test/symlink-folder -> /target/actual-folder",
					"Symlink folder detail should match target",
				);
			});
		});
	}
		...
		suite("remote file completion (e.g. WSL)", () => {
			...
			test("/absolute/path should preserve remote authority", async () => {
				...
				assert.ok(
					...
					"Should return completions for remote absolute path",
				);
				...
				assert.ok(
					absoluteCompletion.detail?.includes("/home/"),
					"Detail should show remote path",
				);
			});
			...
			test("~/ should preserve remote authority for tilde expansion", async () => {
				...
				childResources = [
					{
						resource: URI.from({
							scheme: "vscode-remote",
							authority: remoteAuthority,
							path: "/home/remoteuser/Documents",
						}),
						isDirectory: true,
					},
					{
				...
				assert.ok(
					result && result.length > 0,
					"Should return completions for remote tilde path",
				);
				// Verify the tilde path was resolved using the remote home directory
				const documentsCompletion = result?.find((c) =>
					c.detail?.includes("Documents"),
				);
				assert.ok(
					documentsCompletion,
			...
			test("./relative should preserve remote authority for relative paths", async () => {
				...
				childResources = [
					{
						resource: URI.from({
							scheme: "vscode-remote",
							authority: remoteAuthority,
							path: "/home/remoteuser/project/src",
						}),
						isDirectory: true,
					},
					{
						resource: URI.from({
							scheme: "vscode-remote",
							authority: remoteAuthority,
							path: "/home/remoteuser/project/docs",
						}),
						isDirectory: true,
					},
				];
				...
				assert.ok(
					result && result.length > 0,
					"Should return completions for remote relative path",
				);
				// Verify completions are from the remote filesystem
				const srcCompletion = result?.find((c) =>
					c.detail?.includes("/home/remoteuser/project/src"),
				);
				assert.ok(
					srcCompletion,
					"Should find src folder completion with remote path in detail",
				);
			});
		});
	}
	...
	suite("completion label escaping", () => {
		test("| should escape special characters in file/folder names for POSIX shells", async () => {
			...
			childResources = [
				...
				{
					resource: URI.parse("file:///test/!special$chars&/"),
					isDirectory: true,
				},
				{
					resource: URI.parse("file:///test/!special$chars2&"),
					isFile: true,
					executable: true,
				},
			];
			...
			assertCompletions(
				...
				[
					...
					{
						label: "./\!special\$chars\&/",
						detail: "/test/\!special\$chars\&/",
					},
					{
						label: "./\!special\$chars2\&",
						detail: "/test/\!special\$chars2\&",
						kind: TerminalCompletionItemKind.File,
					},
					{ label: "../", detail: "/" },
					standardTildeItem,
	...
	suite("Provider Configuration", () => {
		...
		setup(() => {
			testTerminalCompletionService = store.add(
				instantiationService.createInstance(TestTerminalCompletionService),
			);
		});
		...
		test("should enable providers by default when no configuration exists", () => {
			...
			const result =
				testTerminalCompletionService.getEnabledProviders(providers);

			// Both providers should be enabled since they're not explicitly disabled
			assert.strictEqual(
				result.length,
				2,
				"Should enable both providers by default",
			);
			assert.ok(
				result.includes(defaultProvider),
		...
		test("should disable providers when explicitly set to false", () => {
			...
			assert.ok(
				result.includes(provider2),
				"Should include unconfigured provider",
			);
			assert.ok(
				!result.includes(provider1),
				"Should not include disabled provider",
			);
		});
		...
		test("should enable providers when explicitly set to true", () => {
			...
			assert.ok(
				result.includes(provider1),
				"Should include explicitly enabled provider",
			);
			assert.ok(
				result.includes(provider2),
				"Should include unconfigured provider",
