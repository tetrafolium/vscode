...
function createItem(
	options: Partial<ITerminalCompletion>,
): TerminalCompletionItem {
...
function createFileItems(...labels: string[]): TerminalCompletionItem[] {
	return labels.map((label) =>
		createItem({ label, kind: TerminalCompletionItemKind.File }),
	);
}
...
function createFolderItems(...labels: string[]): TerminalCompletionItem[] {
	return labels.map((label) =>
		createItem({ label, kind: TerminalCompletionItemKind.Folder }),
	);
}
...
function assertItems(
	...
	labels: (string | CompletionItemLabel)[],
): void {
...
suite("TerminalCompletionModel", function () {
	...
	suite("git branch priority sorting", () => {
		test("should prioritize main and master branches for git commands", () => {
			...
			const model = new TerminalCompletionModel(
				items,
				new LineContext("git checkout ", 0),
			);
		...
		test("should work with complex label objects", () => {
			const items = [
				...
				createItem({
					kind: TerminalCompletionItemKind.Argument,
					label: { label: "main", description: "Main branch" },
				}),
			];
			const model = new TerminalCompletionModel(
				items,
		...
		test("should prioritize for git branch -d", () => {
			...
			const model = new TerminalCompletionModel(
				items,
				new LineContext("git branch -d ", 0),
			);
	...
	suite("mixed kind sorting", () => {
		test("should sort arguments before flags and options", () => {
			const items = [
				...
				createItem({
					kind: TerminalCompletionItemKind.Option,
					label: "--config",
				}),
				createItem({
					kind: TerminalCompletionItemKind.Argument,
