...
suite("TerminalChatService", () => {
	...
	let onDidDisposeSessionEmitter: Emitter<{
	...
	setup(() => {
		...
		service = store.add(
			instantiationService.createInstance(TerminalChatService),
		);
	});
	...
	test("registerTerminalInstanceWithToolSession does not accumulate onDidDisposeSession listeners when re-registering the same instance (#309906)", () => {
		...
		const instance = {
			onDisposed: onInstanceDisposed.event,
			shellLaunchConfig: {},
			persistentProcessId: undefined,
			instanceId: 1,
		...
		for (let i = 0; i < registrations; i++) {
			service.registerTerminalInstanceWithToolSession(
				`tool-session-${i}`,
				instance,
			);
		}
		...
		const addedListeners =
			listenerCount(onDidDisposeSessionEmitter) - initialListenerCount;
	...
	test("registerTerminalInstanceWithToolSession is a no-op when the same tool session id is re-registered", () => {
		...
		assert.strictEqual(
			...
			listenersAfterFirst,
			"re-registering the same (instance, id) pair should not add a new listener",
		);
		assert.strictEqual(
			service.getToolSessionIdForInstance(instance),
			"tool-session-a",
