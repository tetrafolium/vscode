...
registerActiveXtermAction({
	...
	run: (_xterm, _accessor, activeInstance, opts?: unknown) => {
		...
		if (opts) {
			function isValidOptionsObject(
				obj: unknown,
			): obj is { query: string; isPartialQuery?: boolean } {
				return (
					typeof obj === "object" &&
					obj !== null &&
					"query" in obj &&
...
registerAction2(
	class ShowChatTerminalsAction extends Action2 {
		constructor() {
			super({
				...
				f1: true,
				precondition: ContextKeyExpr.and(
					TerminalChatContextKeys.hasHiddenChatTerminals,
					ChatContextKeys.enabled,
				),
				menu: [
					{
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const visible = new Set<ITerminalInstance>([
				...groupService.instances,
				...editorService.instances,
			]);
			const toolInstances =
				terminalChatService.getToolSessionTerminalInstances();
			...
			const lastCommandLocalized = (command: string) =>
				localize2("chatTerminal.lastCommand", "Last: {0}", command).value;
			...
			for (const instance of all.values()) {
				...
				if (lastCommand) {
					...
					const displayCommand =
						firstLine.length > MAX_DETAIL_LENGTH
							? firstLine.substring(0, MAX_DETAIL_LENGTH) + "…"
							: firstLine;
...
CommandsRegistry.registerCommand(
	...
	async (accessor, scopeRaw: string) => {
...
CommandsRegistry.registerCommand(
	...
	async (accessor, chatSessionResource: URI) => {
		const terminalChatService = accessor.get(ITerminalChatService);
