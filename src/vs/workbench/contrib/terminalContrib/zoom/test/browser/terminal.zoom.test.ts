...
suite("Terminal Mouse Wheel Zoom", () => {
	...
	test("clamps font size to maximum value when above bounds", () => {
		...
		strictEqual(
			result,
			100,
			"Font size should be clamped to maximum value of 100",
		);
	});
	...
	test("preserves font size when within bounds", () => {
		...
		strictEqual(
			result,
			15,
			"Font size should remain unchanged when within bounds",
		);
	});
	...
	test("clamps font size when going below minimum", () => {
		...
		strictEqual(
			result,
			6,
			"Font size should be clamped when going below minimum",
		);
	});
	...
	test("clamps font size when going above maximum", () => {
		...
		strictEqual(
			result,
			100,
			"Font size should be clamped when going above maximum",
		);
	});
});
