	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			terminalService.onDidCreateInstance(async (instance) => {
				...
				try {
					const input = terminalEditorService.getInputFromResource(
						instance.resource,
					);
					const windowId = input.group?.windowId;
					isInAuxWindow = !!(
						windowId && isAuxiliaryWindow(getWindowById(windowId, true).window)
					);
				} catch {}
	...
	private _logCreateInstance(
		...
		>("terminal/createInstance", {
			...
			isLoginShell:
				(isString(slc.args) ? slc.args.split(" ") : slc.args)?.some(
					(arg) => arg === "-l" || arg === "--login",
				) ?? false,
	...
	private _logShellTypeChanged(instance: ITerminalInstance): void {
		...
		type TerminalShellTypeChangedTelemetryClassification = {
			...
			shellType: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				comment: "The new detected shell type for the terminal.";
			};
			terminalSessionId: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
		...
		this._telemetryService.publicLog2<
			TerminalShellTypeChangedTelemetryData,
			TerminalShellTypeChangedTelemetryClassification
