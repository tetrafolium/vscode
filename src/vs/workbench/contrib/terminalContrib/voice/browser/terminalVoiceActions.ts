...
export function registerTerminalVoiceActions() {
	registerActiveInstanceAction({
		...
		run: async (activeInstance, c, accessor) => {
			...
			const workbenchExtensionEnablementService = accessor.get(
				IWorkbenchExtensionEnablementService,
			);
			...
			const extension = extensions.find(
				(extension) => extension.identifier.id === "ms-vscode.vscode-speech",
			);
			const extensionIsDisabled =
				extension && !workbenchExtensionEnablementService.isEnabled(extension);
			...
			if (extensionIsDisabled) {
				...
				run = () =>
					workbenchExtensionEnablementService.setEnablement(
						[extension],
