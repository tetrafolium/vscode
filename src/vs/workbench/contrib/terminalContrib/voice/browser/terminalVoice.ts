...
export class TerminalVoiceSession extends Disposable {
	...
	constructor(
		...
		super();
		this._register(
			this._terminalService.onDidChangeActiveInstance(() => this.stop()),
		);
		this._register(
			this._terminalService.onDidDisposeInstance(() => this.stop()),
		);
		this._disposables = this._register(new DisposableStore());
		this._terminalDictationInProgress =
			TerminalContextKeys.terminalDictationInProgress.bindTo(contextKeyService);
	}
	...
	private _renderGhostText(e: ISpeechToTextEvent): void {
		...
		this._ghostText?.onRender((e: HTMLElement) => {
			...
			e.textContent = text;
			e.style.width =
				((xterm.cols - xterm.buffer.active.cursorX) / xterm.cols) * 100 + "%";
		});
	}
}
