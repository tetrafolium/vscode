...
class TestLinkManager extends TerminalLinkManager {
	...
	protected override async _getLinksForType(
		...
		switch (type) {
			case "word":
				return this._links?.wordLinks?.[y]
					? [this._links?.wordLinks?.[y]]
					: undefined;
			case "url":
				return this._links?.webLinks?.[y]
					? [this._links?.webLinks?.[y]]
					: undefined;
			case "localFile":
				return this._links?.fileLinks?.[y]
					? [this._links?.fileLinks?.[y]]
					: undefined;
		}
	}
...
suite("TerminalLinkManager", () => {
	...
	type TestableLinkManager = {
		_showHover: (...args: unknown[]) => IDisposable | undefined;
	};

	function overrideXtermEvent<T>(
		terminal: Terminal,
		eventName: string,
		handler: (listener: (e: T) => void) => IDisposable,
	): IDisposable {
		const originalDescriptor = Object.getOwnPropertyDescriptor(
			Object.getPrototypeOf(terminal),
			eventName,
		);
		Object.defineProperty(terminal, eventName, {
			value: handler,
			configurable: true,
		});
		return {
	...
	function mockXtermCoreRenderService(): IDisposable {
		...
		if (!xtermWithCore._core) {
			(xtermWithCore as XtermWithCore)._core = {} as IXtermCore;
		}
		xtermWithCore._core._renderService = {
	...
	suite("OSC 8 hover", () => {
		test("should cancel delayed tooltip when leave happens before hover delay", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				const testableLinkManager =
					linkManager as unknown as TestableLinkManager;
				const originalShowHover = testableLinkManager._showHover;
				testableLinkManager._showHover = () => {
					hoverShownCount++;
					return undefined;
				};
				const range: Parameters<typeof linkHandler.hover>[2] = {
					start: { x: 1, y: 1 },
					end: { x: 10, y: 1 },
				};
		...
		test("should dismiss shown tooltip on scroll", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
		...
		test("should dismiss shown tooltip on render", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				await assertHoverDismissedOnEvent((setFire) => {
					return overrideXtermEvent<{ start: number; end: number }>(
						xterm,
						"onRender",
						(listener) => {
