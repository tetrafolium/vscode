...
export class TerminalLinkManager extends DisposableStore {
	...
	constructor(
		...
		const localFolderOutsideWorkspaceOpener =
			this._instantiationService.createInstance(
				TerminalLocalFolderOutsideWorkspaceLinkOpener,
			);
		...
		this._xterm.options.linkHandler = {
			...
			hover: (e, text, range) => {
				...
				activeTooltipScheduler = new RunOnceScheduler(() => {
					...
					activeHoverListeners.add(
						this._xterm.onScroll(() => clearActiveLinkHover()),
					);
					activeHoverListeners.add(
						this._xterm.onRender((renderedRange) => {
							// Convert buffer range to viewport range and check if the
							// rendered range intersects any row of the link
							const viewportRange = convertBufferRangeToViewport(
								range,
								hoverViewportY,
							);
							if (
								viewportRange.start.y <= renderedRange.end &&
								viewportRange.end.y >= renderedRange.start
							) {
								clearActiveLinkHover();
							}
						}),
	...
	private async _openLink(link: ITerminalSimpleLink): Promise<void> {
		...
		>("terminal/openLink", {
			linkType: isString(link.type) ? link.type : `extension:${link.type.id}`,
		});
