...
class DevModeContribution extends Disposable implements ITerminalContribution {
	...
	private readonly _activeDevModeDisposables = this._register(
		...
		if (devMode) {
			if (commandDetection) {
				...
				this._activeDevModeDisposables.value =
					this._ctx.instance.capabilities.onDidAddCommandDetectionCapability(
						(e) => {
							this._updateDevMode();
						},
					);
	...
	private _isEnabled(): boolean {
		return (
			this._configurationService.getValue(TerminalSettingId.DevMode) || false
		);
	}
}
