	...
	implements ITerminalAddon, ITerminalQuickFixAddon
{
	...
	private readonly _decoration: MutableDisposable<IDecoration> = this._register(
	...
	private _registerQuickFixDecoration(): void {
		...
		const decoration = (this._decoration.value =
			this._terminal.registerDecoration({ marker, width: 2, layer: "top" }));
		if (!decoration) {
			return;
		}
	...
	for (const options of quickFixOptions.values()) {
		for (const option of options) {
			...
			if (option.type === "resolved") {
				...
				quickFixes = (option as ITerminalQuickFixInternalOptions).getQuickFixes(
					matchResult,
				);
			}
