...
export class PolicyExportContribution
	extends Disposable
	implements IWorkbenchContribution
{
	static readonly ID = "workbench.contrib.policyExport";
	static readonly DEFAULT_POLICY_EXPORT_PATH =
	...
	constructor(
		...
		if (policyDataPath !== undefined) {
			const defaultPath = join(
				this.nativeEnvironmentService.appRoot,
				PolicyExportContribution.DEFAULT_POLICY_EXPORT_PATH,
			);
			void this.exportPolicyDataAndQuit(
				policyDataPath ? policyDataPath : defaultPath,
			);
		}
	}
		...
		try {
			await this.progressService.withProgress(
				...
				async (_progress) => {
					...
					this.log(
						`Discovered ${policyData.policies.length} policies to export.`,
					);
					...
					const extensionPolicies = distroProduct[
						"extensionConfigurationPolicy"
					] as Record<string, ExtensionConfigurationPolicyEntry> | undefined;
					if (extensionPolicies) {
						...
						for (const [key, entry] of Object.entries(extensionPolicies)) {
							...
							if (!entry.description || !entry.category) {
								throw new Error(
									`Extension policy '${key}' is missing required 'description' or 'category' field.`,
								);
							}
							policyData.policies.push({
								key,
					...
					await this.fileService.writeFile(
						URI.file(policyDataPath),
						VSBuffer.fromString(policyDataFileContent),
					);
					this.log(
						`Successfully exported ${policyData.policies.length} policies to ${policyDataPath}.`,
					);
				},
			);
	...
	private async getDistroProductJson(): Promise<Record<string, unknown>> {
		...
		if (envPath) {
			this.log(
				`Reading distro product.json from DISTRO_PRODUCT_JSON=${envPath}`,
			);
			const content = (
				await this.fileService.readFile(URI.file(envPath))
			).value.toString();
		...
		const packageJsonContent = (
			await this.fileService.readFile(URI.file(packageJsonPath))
		).value.toString();
		...
		this.log(
			`Fetching distro product.json for commit ${distroCommit} from GitHub...`,
		);
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch distro product.json: ${response.status} ${response.statusText}`,
			);
		}
