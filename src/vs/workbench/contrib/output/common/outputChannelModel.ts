...
export function parseLogEntryAt(
	model: ITextModel,
	lineNumber: number,
): ILogEntry | null {
...
function* logEntryIterator<T>(
	model: ITextModel,
	process: (logEntry: ILogEntry) => T,
): IterableIterator<T> {
...
function changeStartLineNumber(
	logEntry: ILogEntry,
	lineNumber: number,
): ILogEntry {
	return {
...
class FileContentProvider extends Disposable implements IContentProvider {
	...
	private parseLogEntries(
		content: string,
		lastLogEntry: ILogEntry | undefined,
	): ILogEntry[] {
		...
		try {
			...
			let logEntryStartLineNumber = lastLogEntry
				? lastLogEntry.range.endLineNumber + 1
				: 1;
			for (const entry of logEntryIterator(model, (e) =>
				changeStartLineNumber(e, logEntryStartLineNumber),
			)) {
...
class MultiFileContentProvider extends Disposable implements IContentProvider {
	...
	private logEntries: ILogEntry[] = [];
	private readonly fileContentProviderItems: [
		FileContentProvider,
		DisposableStore,
	][] = [];
	...
	private createFileContentProvider(
		file: IOutputContentSource,
	): [FileContentProvider, DisposableStore] {
	...
	updateFiles(files: IOutputContentSource[]): void {
		...
		for (const { start, deleteCount, toInsert } of result) {
			const outputs = toInsert.map((file) =>
	...
	async getContent(): Promise<{
		...
		const outputs = await Promise.all(
			this.fileContentProviderItems.map(([output]) => output.getContent(true)),
		);
	...
	private combineLogEntries(
			...
			return [
				{
					...
					range: new Range(
						...
						logEntry.range.endLineNumber,
						name
							? logEntry.range.endColumn + name.length + 3
							: logEntry.range.endColumn,
					),
				},
				content,
		...
		try {
			for (const [logEntry, content] of logEntryIterator(model, (e) =>
				process(model, e, outputs[0].name),
			)) {
		...
		for (let index = 1; index < outputs.length; index++) {
			...
			try {
				const iterator = logEntryIterator(model, (e) =>
					process(model, e, name),
				);
				...
				while (!next.done) {
					...
					if (
						logEntry.timestamp >= logEntries[logEntries.length - 1].timestamp
					) {
						...
						for (
							next = iterator.next();
							!next.done &&
							next.value[0].timestamp <= logEntries[insertionIndex].timestamp;
							next = iterator.next()
						) {
							logEntriesToAdd.push(next.value[0]);
							contentsToAdd.push(next.value[1]);
		...
		let logEntryStartLineNumber = lastEntry
			? lastEntry.range.endLineNumber + 1
			: 1;
...
export abstract class AbstractFileOutputChannelModel
	extends Disposable
	implements IOutputChannelModel
{
	...
	private readonly modelDisposable = this._register(
	...
	abstract readonly source:
		| IOutputContentSource
		| ReadonlyArray<IOutputContentSource>;
	...
	async loadModel(): Promise<ITextModel> {
		this.loadModelPromise = Promises.withAsyncBody<ITextModel>(async (c, e) => {
			try {
				...
				const { content, consume } =
					await this.outputContentProvider.getContent();
				consume();
				...
				this.modelDisposable.value.add(
					toDisposable(() => this.outputContentProvider.unwatch()),
				);
				this.modelDisposable.value.add(
	...
	protected doUpdate(mode: OutputChannelUpdateMode, immediate: boolean): void {
		...
		if (mode === OutputChannelUpdateMode.Clear) {
			...
			this.replacePromise = this.replaceContent(this.model, token).finally(
	...
	private doAppendContent(model: ITextModel, content: string): void {
		...
		model.applyEdits([
			EditOperation.insert(new Position(lastLine, lastLineMaxColumn), content),
		]);
	}
...
export class MultiFileOutputChannelModel
	extends AbstractFileOutputChannelModel
	implements IOutputChannelModel
{
