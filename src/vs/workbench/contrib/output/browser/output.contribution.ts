			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						const entries: Array<
							IOutputChannelDescriptor | IQuickPickSeparator
						> = [];
						...
						for (const log of extensionLogs.sort((a, b) =>
							a.label.localeCompare(b.label),
						)) {
							entries.push(log);
						}
						...
						if (result?.length) {
							outputService.showChannel(
								outputService.registerCompoundLogChannel(result),
							);
						}
					}
				},
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						if (entries.length === 0) {
							notificationService.info(
								nls.localize("nocustumoutput", "No custom outputs to remove."),
							);
							return;
						}
						const result = await quickInputService.pick(entries, {
				...
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						if (channel) {
							...
							if (
								channelDescriptor &&
								isSingleSourceOutputChannelDescriptor(channelDescriptor)
							) {
			...
			registerAction2(
				class extends Action2 {
					...
					async run(
						...
						for (const log of userLogs.sort((a, b) =>
							a.label.localeCompare(b.label),
						)) {
							entries.push(log);
						}
						...
						if (arg?.outputChannelIds) {
							const requestedIdsNormalized = arg.outputChannelIds.map((id) =>
								id.trim().toLowerCase(),
							);
							const candidates = entries.filter(
								(e): e is IOutputChannelDescriptor => {
									const isSeparator =
										hasKey(e, { type: true }) && e.type === "separator";
									return !isSeparator;
								},
							);
						...
						if (selectedOutputChannels?.length) {
							await outputService.saveOutputAs(
								arg?.outputPath,
