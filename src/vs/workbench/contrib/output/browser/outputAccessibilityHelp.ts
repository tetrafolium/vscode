...
class OutputAccessibilityHelpProvider
	extends Disposable
	implements IAccessibleViewContentProvider
{
	...
	provideContent(): string {
		...
		// Header
		lines.push(
			nls.localize("output.header", "Accessibility Help: Output Panel Filter"),
		);
		lines.push(
			nls.localize(
				"output.context",
		...
		lines.push(
			nls.localize("output.statusDesc", "You are filtering the output."),
		);
		lines.push("");
		...
		lines.push(
			nls.localize("output.filterHeader", "What Happens When You Filter:"),
		);
		lines.push(
			nls.localize(
				"output.filterDesc1",
		...
		// Focus Behavior
		lines.push(
			nls.localize("output.focusHeader", "Focus Behavior (Important):"),
		);
		lines.push(
			nls.localize(
				"output.focusDesc1",
		...
		// Filter Syntax
		lines.push(
			nls.localize("output.syntaxHeader", "Filter Syntax and Patterns:"),
		);
		lines.push(
			nls.localize(
				"output.syntaxText",
		...
		lines.push(
			nls.localize("output.keyboardHeader", "Keyboard Navigation Summary:"),
		);
		lines.push(
			nls.localize(
				"output.keyDown",
