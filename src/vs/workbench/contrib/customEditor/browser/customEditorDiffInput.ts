...
function getCustomEditorSideBySideDiffInputResource(
	init: CustomEditorSideBySideDiffInputInitInfo,
): URI {
	return init.side === "original"
		? init.originalResource
...
export class CustomEditorDiffInput extends LazilyResolvedWebviewEditorInput {
	private readonly _modelRef = this._register(
		new MutableDisposable<IReference<ICustomEditorModel>>(),
	);
	...
	override async saveAs(
		...
		if (
			!(await modelRef.object.saveCustomEditorAs(
				this.modifiedResource,
				target,
				options,
			))
	...
	override async resolve(): Promise<null> {
		...
		if (!this._modelRef.value) {
			const modelRef = this.customEditorService.models.tryRetain(
				this.modifiedResource,
				this.viewType,
			);
			if (modelRef) {
				...
				this._register(
					retainedModelRef.object.onDidChangeDirty(() =>
						this._onDidChangeDirty.fire(),
					),
				);
				this._register(
					retainedModelRef.object.onDidChangeReadonly(() =>
						this._onDidChangeCapabilities.fire(),
					),
				);
				if (this.isDirty()) {
	...
	override toUntyped(
		_options?: IUntypedEditorOptions,
	): IResourceDiffEditorInput {
...
export class CustomEditorSideBySideDiffInput extends LazilyResolvedWebviewEditorInput {
	...
	public static override readonly typeId =
		"workbench.editors.customSideBySideDiffEditor";
	...
	override isDirty(): boolean {
		return this.side === "modified"
			? (this._modelRef.value?.object.isDirty() ?? false)
			: false;
	}
