...
type CustomEditorUndoRedoInput =
	| CustomEditorInput
	| CustomEditorDiffInput
	| CustomEditorSideBySideDiffInput;
	...
	implements ICustomEditorService
{
	...
	constructor(
		...
		const customEditorDiffCanToggleLayoutContextKeyProvider: IEditorGroupContextKeyProvider<boolean> =
			{
				...
				getGroupContextKeyValue: (group) =>
					this.getActiveCustomEditorDiffCanToggleLayout(group),
				onDidChange: this.onDidChangeEditorTypes,
			};
		...
		const customEditorTextDiffContextKeyProvider: IEditorGroupContextKeyProvider<boolean> =
			{
				...
				getGroupContextKeyValue: (group) =>
					this.getActiveCustomEditorTextDiff(group),
				onDidChange: this.onDidChangeEditorTypes,
			};
		...
		this._register(
			this.editorGroupService.registerContextKeyProvider(
				customEditorDiffCanToggleLayoutContextKeyProvider,
			),
		);
		this._register(
			this.editorGroupService.registerContextKeyProvider(
				customEditorTextDiffContextKeyProvider,
			),
		);
	...
	private withActiveCustomEditor(
		f: (editor: CustomEditorUndoRedoInput) => void | Promise<void>,
	...
	private getActiveCustomEditorUndoRedoInput():
		| CustomEditorUndoRedoInput
		| undefined {
		const activeEditor = this.editorService.activeEditor;
		if (
			activeEditor instanceof CustomEditorInput ||
			activeEditor instanceof CustomEditorDiffInput ||
			activeEditor instanceof CustomEditorSideBySideDiffInput
		) {
			return activeEditor;
		}
		if (
			activeEditor instanceof DiffEditorInput &&
			activeEditor.modified instanceof CustomEditorSideBySideDiffInput
		) {
						...
						},
						{
							...
							createDiffEditorInput: async (diffEditorInput, group) => {
								await this.extensionService.activateByEvent(
									`onCustomEditor:${contributedEditor.id}`,
								);
								return {
	...
	private createDiffEditorInput(
		...
		const diffEditorLayout = this.getDiffEditorLayout(
			contributedEditor,
			modifiedResource,
		);
		...
		if (diffEditorLayout === CustomEditorDiffEditorLayout.SideBySide) {
			...
			const originalOverride = CustomEditorSideBySideDiffInput.create(
				this.instantiationService,
				{
					originalResource,
					modifiedResource,
			...
			const modifiedOverride = CustomEditorSideBySideDiffInput.create(
				this.instantiationService,
				{
					originalResource,
					modifiedResource,
	...
	private getDiffEditorLayout(
	...
	): CustomEditorDiffEditorLayout | undefined {
		...
		const supportsSideBySideDiff =
			capabilities?.supportsSideBySideDiff === true;
		...
		if (supportsInlineDiff && supportsSideBySideDiff) {
			return this.textResourceConfigurationService.getValue<boolean>(
				...
				? CustomEditorDiffEditorLayout.SideBySide
				: CustomEditorDiffEditorLayout.Inline;
		}

		return supportsInlineDiff
			? CustomEditorDiffEditorLayout.Inline
	...
	private async updateCustomDiffEditorsForDiffConfigurationChange(
		...
		for (const group of this.editorGroupService.groups) {
			...
			for (const editor of group.editors) {
				...
				const contributedEditor = diffInfo
					? this._contributedEditors.get(diffInfo.viewType)
					: undefined;
				if (
					!diffInfo ||
					!contributedEditor ||
					!e.affectsConfiguration(
						diffInfo.modifiedResource,
						"diffEditor.renderSideBySide",
					) ||
					!this.getCustomEditorCapabilities(contributedEditor.id)
						?.supportsInlineDiff ||
	...
	): CustomEditorDiffInputInfo | undefined {
	...
	private getActiveCustomEditorId(group: IEditorGroup): string {
		...
		return input instanceof CustomEditorInput && input.resource
			? input.viewType
			: "";
	}
	...
	private getActiveCustomEditorDiffCanToggleLayout(
		...
		const diffInfo = this.getCustomEditorDiffInputInfo(
			group.activeEditorPane?.input,
		);
		const capabilities = diffInfo
			? this.getCustomEditorCapabilities(diffInfo.viewType)
			: undefined;
		return (
	...
	private getActiveCustomEditorTextDiff(group: IEditorGroup): boolean {
		...
		return (
			!!diffInfo &&
			this.getCustomEditorCapabilities(diffInfo.viewType)?.isTextEditor === true
		);
	}
	...
	private async handleMovedFileInOpenedFileEditors(
		...
		if (
			extname(oldResource).toLowerCase() === extname(newResource).toLowerCase()
		) {
			return;
		}
		...
		if (
			!possibleEditors.allEditors.some(
				(editor) => editor.priority.editor !== RegisteredEditorPriority.option,
			)
		) {
			return;
		}
