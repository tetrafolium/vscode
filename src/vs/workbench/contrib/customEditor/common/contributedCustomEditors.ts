...
type StoredCustomEditorPriorityInfo = Omit<
	CustomEditorPriorityInfo,
	"diff" | "merge"
> & {
...
export class ContributedCustomEditors extends Disposable {
	...
	constructor(storageService: IStorageService) {
		...
		for (const info of mementoObject[
			...
			this.add(
				new CustomEditorInfo(normalizeStoredCustomEditorDescriptor(info)),
			);
		}
	...
	private update(
		extensions: readonly IExtensionPointUser<ICustomEditorsExtensionPoint[]>[],
	) {
		this._editors.clear();

		for (const extension of extensions) {
			const hasCustomEditorPriorityProposal =
				extension.description.enabledApiProposals?.includes(
					"customEditorPriority",
				) ?? false;
...
function normalizeStoredCustomEditorDescriptor(
	descriptor: StoredCustomEditorDescriptor,
): CustomEditorDescriptor {
	return {
		id: descriptor.id,
		...
		priority:
			typeof descriptor.priority === "string"
				? {
...
function getPriorityFromContribution(
	...
	const editorPriority =
		getSinglePriorityFromContribution(
	...
	return {
		...
		diff:
			includeDiffAndMergePriority && typeof contribution !== "string"
				? (getSinglePriorityFromContribution(contribution?.diff, extension) ??
					editorPriority)
				: editorPriority,
		merge:
			includeDiffAndMergePriority && typeof contribution !== "string"
				? (getSinglePriorityFromContribution(contribution?.merge, extension) ??
					editorPriority)
				: editorPriority,
	};
}

function getSinglePriorityFromContribution(
	value: CustomEditorPriority | undefined,
	extension: IExtensionDescription,
