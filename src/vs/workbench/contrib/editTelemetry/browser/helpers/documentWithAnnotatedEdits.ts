...
export interface IDocumentWithAnnotatedEdits<
	TEditData extends IEditData<TEditData> = EditKeySourceData,
> {
	readonly value: IObservableWithChange<
		StringText,
...
{
	public readonly value: IObservableWithChange<
		StringText,
...
export class CombineStreamedChanges<
	TEditData extends (EditKeySourceData | EditSourceData) & IEditData<TEditData>,
>
	extends Disposable
	implements IDocumentWithAnnotatedEdits<TEditData>
{
	...
	async _restart(): Promise<void> {
		...
		const iterator = iterateObservableChanges(
			this._originalDoc.value,
			this._runStore,
	...
	private async _run(
		iterator: AsyncIterator<
			{
				value: StringText;
				prevValue: StringText;
				change: { edit: AnnotatedStringEdit<TEditData> }[];
			},
			any,
			any
		>,
		...
		while (true) {
			...
			if (peeked === AsyncReaderEndOfStream) {
				...
				let last = first;
				let chatEdit =
					AnnotatedStringEdit.empty as AnnotatedStringEdit<TEditData>;
