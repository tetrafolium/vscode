...
import {
	AnnotatedDocument,
	IAnnotatedDocuments,
} from "../helpers/annotatedDocuments.js";
import {
...
export class EditSourceTrackingImpl extends Disposable {
	...
	constructor(
		...
		this._states = mapObservableArrayCached(
			this,
			this._annotatedDocuments.documents,
			(doc, store) => {
				return [
					doc.document,
					store.add(
...
class TrackedDocumentInfo extends Disposable {
	public readonly longtermTracker: IObservable<
		DocumentEditSourceTracker<undefined> | undefined
	>;
	public readonly windowedTracker: IObservable<
		DocumentEditSourceTracker<undefined> | undefined
	>;
	public readonly windowedFocusTracker: IObservable<
		DocumentEditSourceTracker<undefined> | undefined
	>;
	...
	constructor(
		...
		private readonly _userAttentionService: IUserAttentionService,
	) {
		super();

		this._repo = derived(this, (reader) =>
			this._scm.getRepo(_doc.document.uri, reader),
		);

		const docWithJustReason = createDocWithJustReason(
			_doc.documentWithAnnotations,
			this._store,
		);
				...
				toDisposable(() => {
					...
					if (!t.isEmpty()) {
						this.sendTelemetry(
							...
							longtermReason,
							t,
							this._userAttentionService.totalFocusTimeMs - startFocusTime,
							Date.now() - startTime,
						);
					}
					t.dispose();
		...
		this._store.add(
			autorun((reader) => {
				...
				if (repo) {
					reader.store.add(
						runOnChange(repo.headCommitHashObs, () => {
					...
					reader.store.add(
						runOnChange(repo.headBranchNameObs, () => {
		...
		this.windowedTracker = derived((reader) => {
			...
			reader.store.add(
				this._userAttentionService.fireAfterGivenFocusTimePassed(
					10 * 60 * 1000,
					() => {
	...
	async sendTelemetry(
		...
		const entries = Object.entries(sums).filter(
			([key, value]) => value !== undefined,
		);
		entries.sort(
			reverseOrder(compareBy(([key, value]) => value!, numberComparator)),
		);
			...
			this._telemetryService.publicLog2<
				...
				},
				{
					...
					mode: {
						...
						comment: "Describes the session mode. Is either 'longterm', '10minFocusWindow', or '20minFocusWindow'.";
					};
					sourceKey: {
						classification: "SystemMetaData";
						purpose: "FeatureInsight";
						comment: "A description of the source of the edit.";
					};

					sourceKeyCleaned: {
						classification: "SystemMetaData";
						purpose: "FeatureInsight";
						comment: "The source of the edit with some properties (such as extensionId, extensionVersion and modelId) removed.";
					};
					extensionId: {
						classification: "SystemMetaData";
						purpose: "FeatureInsight";
					...
					statsUuid: {
						classification: "SystemMetaData";
						purpose: "PerformanceAndHealth";
						comment: "The unique identifier of the session for which stats are reported. The sourceKey is unique in this session.";
					};
					conversationId: {
						classification: "SystemMetaData";
						purpose: "FeatureInsight";
						comment: "The chat conversation identifier when the edit source comes from chat. Sourced from the chat edit session id.";
					};
					requestId: {
						classification: "SystemMetaData";
						purpose: "FeatureInsight";
						comment: "The chat request identifier when the edit source comes from chat.";
					};

					trigger: {
						classification: "SystemMetaData";
						purpose: "FeatureInsight";
						comment: "Indicates why the session ended.";
					};

					modifiedCount: {
						classification: "SystemMetaData";
						purpose: "PerformanceAndHealth";
						comment: "The number of characters inserted by the given edit source during the session that are still in the text document at the end of the session.";
						isMeasurement: true;
					};
					deltaModifiedCount: {
						classification: "SystemMetaData";
						purpose: "PerformanceAndHealth";
						comment: "The number of characters inserted by the given edit source during the session.";
						isMeasurement: true;
					};
					totalModifiedCount: {
						classification: "SystemMetaData";
						purpose: "PerformanceAndHealth";
						comment: "The number of characters inserted by any edit source during the session that are still in the text document at the end of the session.";
						isMeasurement: true;
					};
				}
		...
		this._telemetryService.publicLog2<
			...
			},
			{
				...
				mode: {
					classification: "SystemMetaData";
					purpose: "PerformanceAndHealth";
					comment: "longterm, 10minFocusWindow, or 20minFocusWindow";
				};
				languageId: {
					classification: "SystemMetaData";
					purpose: "FeatureInsight";
				...
				externalModifiedCount: {
					classification: "SystemMetaData";
					purpose: "PerformanceAndHealth";
					comment: "Fraction of external modified characters";
					isMeasurement: true;
				};
				isTrackedByGit: {
					classification: "SystemMetaData";
				...
				focusTime: {
					classification: "SystemMetaData";
					purpose: "FeatureInsight";
					comment: "The focus time in ms during the session.";
					isMeasurement: true;
				};
				actualTime: {
					classification: "SystemMetaData";
					purpose: "FeatureInsight";
					comment: "The actual time in ms during the session.";
					isMeasurement: true;
				};
				trigger: {
					classification: "SystemMetaData";
	...
	getTelemetryData(ranges: readonly TrackedEdit[]) {
		const getEditCategory = (source: EditSource) => {
			...
			if (
				...
				source.extensionId === "github.copilot"
			) {
			...
			if (
				source.category === "ai" &&
				source.kind === "completion" &&
				source.extensionId === "github.copilot-chat" &&
				source.providerId === "completions"
			) {
				return "inlineCompletionsCopilot";
			}
			if (
		...
		const sums = sumByCategory(
			ranges,
			(r) => r.range.length,
			(r) => getEditCategory(r.source),
		);
		const totalModifiedCharactersInFinalState = sumBy(
			ranges,
			(r) => r.range.length,
		);
