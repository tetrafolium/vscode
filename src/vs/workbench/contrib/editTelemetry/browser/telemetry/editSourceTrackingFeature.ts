...
export class EditTrackingFeature extends Disposable {
	...
	constructor(
		...
		this._editSourceTrackingShowDecorations = makeSettable(
		...
		this._editSourceTrackingShowStatusBar = observableConfigValue(
			EDIT_TELEMETRY_SHOW_STATUS_BAR,
			false,
			this._configurationService,
		);
		const editSourceDetailsEnabled = observableConfigValue(
			EDIT_TELEMETRY_DETAILS_SETTING_ID,
			false,
			this._configurationService,
		);

		const extensions = observableFromEvent(
			this._extensionService.onDidChangeExtensions,
			() => {
				return this._extensionService.extensions;
			},
		);
		const extensionIds = derived(
			(reader) =>
				new Set(extensions.read(reader).map((e) => e.id?.toLowerCase())),
		);
		function getExtensionInfoObs(
			extensionId: string,
		...
		const copilotInstalled = getExtensionInfoObs(
			"GitHub.copilot",
			this._extensionService,
		);
		const copilotChatInstalled = getExtensionInfoObs(
			"GitHub.copilot-chat",
			this._extensionService,
		);

		const shouldSendDetails = derived(
			(reader) =>
				editSourceDetailsEnabled.read(reader) ||
				!!copilotInstalled.read(reader) ||
				!!copilotChatInstalled.read(reader),
		);

		const instantiationServiceWithInterceptedTelemetry =
		...
		const impl = this._register(
			instantiationServiceWithInterceptedTelemetry.createInstance(
				EditSourceTrackingImpl,
				shouldSendDetails,
				this._annotatedDocuments,
			),
		);
		...
		this._register(
			autorun((reader) => {
				...
				const visibleEditors = observableFromEvent(
					this,
					this._editorService.onDidVisibleEditorsChange,
					() => this._editorService.visibleTextEditorControls,
				);
		...
		this._register(
			autorun((reader) => {
				...
				const statusBarItem = reader.store.add(
				...
				const tooltipMarkdownString = derived((reader) => {
					...
					const tooltipWithCommand = new MarkdownString(
						tooltipContent +
							"\n\n[View Details](command:" +
							this._showStateInMarkdownDoc +
							")",
					);
					tooltipWithCommand.appendMarkdown(
						"\n\n" +
							agenda +
							"\n\nToggle decorations: [Click here](command:" +
							this._toggleDecorations +
							")",
					);
					tooltipWithCommand.isTrusted = {
						enabledCommands: [this._toggleDecorations],
					};
				...
				reader.store.add(
					CommandsRegistry.registerCommand(this._toggleDecorations, () => {
						this._editSourceTrackingShowDecorations.set(
							!this._editSourceTrackingShowDecorations.read(undefined),
							undefined,
						);
					}),
				);
	...
	private _createEditSourceAgenda(editSources: EditSource[]): string {
		...
		for (const editSource of editSources) {
			if (!editSourcesSeen.has(editSource.toString())) {
				...
				editSourceInfo.push({
					name: editSource.toString(),
					color: editSource.getColor(),
				});
			}
		}
