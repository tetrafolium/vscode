...
export class EditTelemetryReportInlineEditArcSender extends Disposable {
	constructor(
		...
		this._register(
			runOnChange(docWithAnnotatedEdits.value, (_val, _prev, changes) => {
				const edit = AnnotatedStringEdit.compose(changes.map((c) => c.edit));

				if (
					!edit.replacements.some(
						(r) =>
							r.data.editSource.metadata.source === "inlineCompletionAccept",
					)
				) {
					return;
				}
				if (
					!edit.replacements.every(
						(r) =>
							r.data.editSource.metadata.source === "inlineCompletionAccept",
					)
				) {
					onUnexpectedError(
						new Error(
							"ArcTelemetrySender: Not all edits are inline completion accept edits!",
						),
					);
					return;
				}
				if (
					edit.replacements[0].data.editSource.metadata.source !==
				...
				const docWithJustReason = createDocWithJustReason(
					docWithAnnotatedEdits,
					this._store,
				);
				const reporter = this._store.add(
					this._instantiationService.createInstance(
						ArcTelemetryReporter,
						[0, 30, 120, 300, 600, 900].map((s) => s * 1000),
						_prev,
						docWithJustReason,
						scmRepoBridge,
						edit,
						(res) => {
							res.telemetryService.publicLog2<
								...
								},
								{
									...
									extensionId: {
										...
										comment: "The extension id which provided this inline suggestion.";
									};
									extensionVersion: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
									...
									opportunityId: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										comment: "Unique identifier for an opportunity to show an inline suggestion.";
									};
									languageId: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
									...
									correlationId: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										comment: "The correlation id of the inline suggestion.";
									};
									...
									originalLineCount: {
										...
										purpose: "FeatureInsight";
										isMeasurement: true;
										comment: "The original line count before any edits.";
									};
									originalDeletedLineCount: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										isMeasurement: true;
										comment: "The original deleted line count before any edits.";
									};
									arc: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										isMeasurement: true;
										comment: "The accepted and retained character count.";
									};
									currentLineCount: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										isMeasurement: true;
										comment: "The current line count after edits.";
									};
									currentDeletedLineCount: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										isMeasurement: true;
										comment: "The current deleted line count after edits.";
									};
								}
							>("editTelemetry.reportInlineEditArc", {
								...
								...forwardToChannelIf(
									isCopilotLikeExtension(data.$extensionId),
								),
							});
						},
						() => {
	...
	constructor(
		...
		this._register(
			runOnChange(docWithAnnotatedEdits.value, (_val, _prev, changes) => {
				...
				] as ITextModelEditSourceMetadata["source"][]);

				if (
					!edit.replacements.some((r) =>
						supportedSource.has(r.data.editSource.metadata.source),
					)
				) {
					return;
				}
				if (
					!edit.replacements.every((r) =>
						supportedSource.has(r.data.editSource.metadata.source),
					)
				) {
					onUnexpectedError(
						new Error(
							`ArcTelemetrySender: Not all edits are ${edit.replacements[0].data.editSource.metadata.source}!`,
						),
					);
					return;
				}
				let applyCodeBlockSuggestionId: EditSuggestionId | undefined =
					undefined;
				...
				const providerId = new ProviderId(
					data.props.$extensionId,
					data.props.$extensionVersion,
					...
					this._instantiationService.createInstance(
						...
						(res) => {
							res.telemetryService.publicLog2<
								...
								},
								{
									...
									sourceKeyCleaned: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										comment: "The key of the edit source.";
									};
									extensionId: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
									...
									mode: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										comment: "The mode chat was in.";
									};
									uniqueEditId: {
										classification: "SystemMetaData";
										purpose: "FeatureInsight";
										comment: "The unique identifier for the edit.";
									};
							...
							>("editTelemetry.reportEditArc", {
								...
								...forwardToChannelIf(
									isCopilotLikeExtension(data.props.$extensionId),
								),
							});
						},
						() => {
