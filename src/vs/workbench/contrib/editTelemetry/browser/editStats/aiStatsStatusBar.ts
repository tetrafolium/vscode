							...
							tooltip: {
								element: async (_token) => {
									...
									const elem = createAiStatsHover({
										...
										onOpenSettings: () =>
											openSettingsCommand({ ids: [AI_STATS_SETTING_ID] }).run(
		...
		this._telemetryService.publicLog2<
			...
			},
			{
				...
				aiRate: {
					...
					comment: "The current AI rate percentage";
				};
			}
					...
					[
						n.div(
							...
							[
								n.div({
									style: {
										width: this._aiStatsFeature.aiRate.map(
											(v) => `${v * 100}%`,
										),
...
export function createAiStatsHover(options: IAiStatsHoverOptions) {
	...
	const aiRatePercent = options.data.aiRate.map(
		(r) => `${Math.round(r * 100)}%`,
	);
	...
	return n.div(
		...
		[
			n.div(
				...
				[
					n.div({ style: { flex: 1 } }, [
						localize("aiStatsStatusBarHeader", "AI Usage Statistics"),
					]),
					n.div(
			...
			n.div({ style: { flex: 1, paddingRight: "4px" } }, [
				localize(
					...
					options.data.acceptedInlineSuggestionsToday.get(),
				),
			]),
						...
						[
							...
							n.div(
								...
								[
									createToggleButton(
										"days",
										localize("viewByDays", "Days"),
										Codicon.calendar,
									),
									createToggleButton(
										"sessions",
										localize("viewBySessions", "Sessions"),
										Codicon.listFlat,
									),
								],
							),
...
function actionBar(
	actions: { action: IAction; options: IActionOptions }[],
