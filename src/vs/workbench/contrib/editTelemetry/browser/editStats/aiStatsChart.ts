...
export function createAiStatsChart(options: IAiStatsChartOptions): HTMLElement {
		...
		dailyData.forEach((day, i) => {
			...
			if (isFirst || isLast || (isAtInterval && barCount > 2)) {
				...
				if (!isFirst && !isLast) {
					...
					if (
						distFromFirst < minLabelSpacing ||
						distFromLast < minLabelSpacing
					) {
	...
	function renderSessionsView() {
		...
		const barWidth = Math.min(
			8,
			(innerWidth - (sessionCount - 1) * 1) / sessionCount,
		);
		const gap = 1;
		...
		const firstLabel = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"text",
		);
		...
		firstLabel.textContent = firstDate.toLocaleDateString(undefined, {
			month: "short",
			day: "numeric",
		});
		...
		if (sessionCount > 1 && totalBarSpace >= minLabelSpacing) {
			...
			const lastLabel = document.createElementNS(
				"http://www.w3.org/2000/svg",
				"text",
			);
			...
			lastLabel.textContent = lastDate.toLocaleDateString(undefined, {
				month: "short",
				day: "numeric",
			});
