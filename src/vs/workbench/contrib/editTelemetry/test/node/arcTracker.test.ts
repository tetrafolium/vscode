...
suite("ArcTracker", () => {
	...
	const fixturesOutDir = FileAccess.asFileUri(
		"vs/workbench/contrib/editTelemetry/test/node/data",
	).fsPath;
	const fixturesSrcDir = resolve(fixturesOutDir)
		.replaceAll("\\", "/")
		.replace("/out/vs/workbench/", "/src/vs/workbench/");
		...
		(replacement) =>
			new StringReplacement(
				OffsetRange.ofStartAndLength(
					replacement.start,
					replacement.endEx - replacement.start,
				),
				replacement.text,
			),
	);
...
function runTestWithData(data: IEdits): unknown {
	const edits = data.edits.map((editData) =>
		createStringEditFromJson(editData),
	);
	...
	for (let i = 1; i < edits.length; i++) {
		...
		assert.ok(
			newLineNumbers <= lastLineNumbers,
			`Line numbers must not increase. Last: ${lastLineNumbers}, new: ${newLineNumbers}`,
		);
		...
		assert.ok(
			newArc <= lastArc,
			`ARC must not increase. Last: ${lastArc}, new: ${newArc}`,
		);
		lastArc = newArc;
	}
	return stats;
