...
import {
	StringEdit,
	StringReplacement,
} from "../../../../../editor/common/core/edits/stringEdit.js";
...
} from "../../browser/helpers/annotatedDocuments.js";
import {
	IObservableDocument,
	ObservableWorkspace,
	StringEditWithReason,
} from "../../browser/helpers/observableWorkspace.js";
...
} from "../../../../services/userAttention/browser/userAttentionBrowser.js";
		...
		runWithFakedTimers({}, async () => {
				...
				new TestInstantiationService(
					new ServiceCollection(
						[
							...
							new SyncDescriptor(AiEditTelemetryServiceImpl),
						],
			...
			instantiationService.stub(ITelemetryService, {
				publicLog2(eventName, data) {
					sentTelemetry.push(
						`${formatTime(Date.now())} ${eventName}: ${JSON.stringify(data)}`,
					);
				},
			});
			...
			instantiationService.stubInstance(ScmAdapter, {
				getRepo: (uri, reader) => undefined,
			});
			instantiationService.stubInstance(UriVisibilityProvider, {
				isVisible: (uri, reader) => true,
			});
			...
			const docs = disposables.add(
				new AnnotatedDocuments(w, instantiationService),
			);
			disposables.add(
				new EditSourceTrackingImpl(
...
`,
			...
			d1.applyEdit(
				StringEditWithReason.replace(
					d1.findRange("≪≫function fib(n) {"),
			...
			d1.applyEdit(
				StringEditWithReason.replace(
					d1.findRange("Computes the nth fibonacci number"),
...
export class FakeAnnotatedDocuments
	extends Disposable
...
function findOffsetRange(str: string, search: SearchString): OffsetRange {
	...
	if (
		startContextIndex !== -1 &&
		endContextIndex !== -1 &&
		endContextIndex > startContextIndex
	) {
	...
	if (startIndex === -1) {
		throw new Error(
			`Could not find context "${beforeContext}" + "${searchStr}" + "${afterContext}" in string "${str}"`,
		);
	}
	...
	implements IObservableDocument
{
	...
	public get languageId(): IObservable<string> {
	...
	applyEdit(
		...
		const e =
			edit instanceof StringEditWithReason
				? edit
				: new StringEditWithReason(edit.replacements, EditSources.unknown({}));
