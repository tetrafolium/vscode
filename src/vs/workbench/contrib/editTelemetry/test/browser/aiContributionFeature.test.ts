...
suite("AiContributionFeature", () => {
	...
	function setup(): void {
		...
		const annotatedDocuments = disposables.add(
			new AnnotatedDocuments(workspace, instantiationService),
		);
		disposables.add(
	...
	function hasAiContributions(
		uris: URI[],
		level: "chatAndAgent" | "all",
	): boolean {
	...
	function clearAiContributions(uris: URI[]): void {
		CommandsRegistry.getCommand("_aiEdits.clearAiContributions")!.handler(
			undefined!,
			uris,
		);
	}
	...
	function clearAllAiContributions(): void {
		CommandsRegistry.getCommand("_aiEdits.clearAllAiContributions")!.handler(
			undefined!,
		);
	}
	...
	test("no contributions initially", () =>
		runWithFakedTimers({}, async () => {
			setup();
			const d = disposables.add(
				workspace.createDocument(
					{ uri: fileA, initialValue: "hello" },
					undefined,
				),
			);
			await timeout(1500);
	...
	test("detects chat AI edits", () =>
		runWithFakedTimers({}, async () => {
			...
			d.applyEdit(
				StringEditWithReason.replace(d.findRange("hello"), "world", chatEdit),
			);
			await timeout(1500);
	...
	test("does not detect user edits as AI", () =>
		runWithFakedTimers({}, async () => {
			...
			d.applyEdit(
				StringEditWithReason.replace(d.findRange("hello"), "world", userEdit),
			);
			await timeout(1500);
	...
	test("clear resets contributions for specific resources", () =>
		runWithFakedTimers({}, async () => {
			...
			dA.applyEdit(
				StringEditWithReason.replace(dA.findRange("hello"), "foo", chatEdit),
			);
			dB.applyEdit(
				StringEditWithReason.replace(dB.findRange("world"), "bar", chatEdit),
			);
			await timeout(1500);
	...
	test("tracks new edits after clear", () =>
		runWithFakedTimers({}, async () => {
			...
			d.applyEdit(
				StringEditWithReason.replace(d.findRange("world"), "again", chatEdit),
			);
			await timeout(1500);
	...
	test("returns false for unknown URIs", () =>
		runWithFakedTimers({}, async () => {
			setup();
			assert.strictEqual(
				hasAiContributions([URI.parse("file:///unknown.ts")], "all"),
				false,
			);
			disposables.dispose();
		}));
