...
export class ArcTracker {
	...
	constructor(
		...
		this._trackedEdit = trackedEdit.removeCommonSuffixPrefix(
			_valueBeforeTrackedEdit.getValue(),
		);
		this._updatedTrackedEdit = this._trackedEdit.mapData(
			() => new IsTrackedEditData(true),
		);
	}
	...
	getAcceptedRestrainedCharactersCount(): number {
		const s = sumBy(this._updatedTrackedEdit.replacements, (e) =>
			e.data.isTrackedEdit ? e.getNewLength() : 0,
		);
		return s;
	}
	...
	public getLineCountInfo(): {
		deletedLineCounts: number;
		insertedLineCounts: number;
	} {
		const e = this._updatedTrackedEdit.toStringEdit(
			(r) => r.data.isTrackedEdit,
		);
