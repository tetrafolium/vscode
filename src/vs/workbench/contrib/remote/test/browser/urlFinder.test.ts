...
suite("UrlFinder", () => {
	...
	function createMockTerminalService(
		...
		localStore: DisposableStore,
	): ITerminalService {
		const onDidCreateInstance = localStore.add(
			new Emitter<ITerminalInstance>(),
		);
		const onDidDisposeInstance = localStore.add(
			new Emitter<ITerminalInstance>(),
		);
		return {
			instances,
	...
	test("should debounce terminal data processing", async () => {
		...
		const terminalService = createMockTerminalService(
			[mockInstance as unknown as ITerminalInstance],
			store,
		);
		...
		store.add(
			urlFinder.onDidMatchLocalUrl((url: { host: string; port: number }) =>
		...
		assert.strictEqual(
			matchedUrls.length,
			0,
			"URLs should not be processed immediately",
		);
	...
	test("should skip processing when data exceeds threshold", async () => {
		...
		assert.strictEqual(
			matchedUrls.length,
			0,
			"URLs should not be processed when data exceeds threshold",
		);
	});
