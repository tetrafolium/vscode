	...
	implements IWorkbenchContribution
{
	...
	private async enableForwardedPortsFeatures() {
		...
		const featuresEnabled: boolean = !!forwardedPortsFeaturesEnabled.getValue(
		...
		if (featuresEnabled || viewEnabled) {
			...
			this.contextKeyListener.value = this.contextKeyService.onDidChangeContext(
				(e) => {
					if (
						e.affectsSome(
							new Set([
	...
	private get entry(): IStatusbarEntry {
		...
		if (count === 0) {
			...
			allTunnels.push(
				...Array.from(this.remoteExplorerService.tunnelModel.detected.values()),
			);
			tooltip = nls.localize(
	...
	implements IWorkbenchContribution
{
				...
				this._register(
					configurationService.onDidChangeConfiguration((e) => {
						if (e.affectsConfiguration(PORT_AUTO_SOURCE_SETTING)) {
							...
							e.affectsConfiguration(PORT_AUTO_FALLBACK_SETTING) &&
							!this.portListener
						) {
							this.listenForPorts();
						}
	...
	private getPortAutoFallbackNumber(): number {
		...
		if (
			inspectSource.applicationValue === PORT_AUTO_SOURCE_SETTING_PROCESS ||
	...
	private listenForPorts() {
		...
		if (
			...
			this.portListener = this._register(
				this.remoteExplorerService.tunnelModel.onForwardPort(async () => {
					...
					if (
						Array.from(
							this.remoteExplorerService.tunnelModel.forwarded.values(),
						).filter((tunnel) => tunnel.source.source === TunnelSource.Auto)
							.length > fallbackAt
					) {
	...
	private setup(environment: IRemoteAgentEnvironment | null) {
		...
		if (environment?.os !== OperatingSystem.Linux) {
			if (
				this.configurationService.inspect<string>(PORT_AUTO_SOURCE_SETTING)
...
class OnAutoForwardedAction extends Disposable {
	...
	private readonly notificationDisposable = this._register(
	...
	private startUrlFinder() {
		...
		this._register(
			this.urlFinder.onDidMatchLocalUrl(async (localUrl) => {
				...
				const attributes = (
					await this.remoteExplorerService.tunnelModel.getAttributes([localUrl])
				)?.get(localUrl.port);
