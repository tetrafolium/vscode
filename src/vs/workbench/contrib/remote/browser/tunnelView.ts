...
> {
	...
	renderButton(
		...
		templateData.button = templateData.elementDisposables.add(
...
export class TunnelPanel extends ViewPane {
	...
	constructor(
		...
		const titleMenu = this._register(
			this.menuService.createMenu(MenuId.TunnelTitle, overlayContextKeyService),
		);
		const updateActions = () => {
	...
	private createTable(): void {
		...
		const actionRunner: ActionRunner = this.tableDisposables.add(
			new ActionRunner(),
		);
		...
		this.tableDisposables.add(
			this.table.onContextMenu((e) => this.onContextMenu(e, actionRunner)),
		);
		this.tableDisposables.add(
				...
				return new Promise((resolve) => {
					...
					remoteExplorerService.setEditable(tunnelItem, TunnelEditId.Label, {
						onFinish: async (value, success) => {
							...
							resolve(
								changed
									? { port: tunnelItem.remotePort, label: value }
									: undefined,
							);
						},
		...
		return async (accessor, arg) => {
			...
			if (value && (parsed = parseAddress(value))) {
				remoteExplorerService
					...
					.then((tunnel) =>
						error(notificationService, tunnel, parsed!.host, parsed!.port),
					);
			}
		};
	}
	...
	export function commandPaletteHandler(): ICommandHandler {
		return async (accessor) => {
			...
			const picks: QuickPickInput<QuickPickTunnel>[] = makeTunnelPicks(
				Array.from(remoteExplorerService.tunnelModel.forwarded.values()).filter(
					(tunnel) => tunnel.closeable,
				),
				remoteExplorerService,
				tunnelService,
			);
			...
			if (result && result.tunnel) {
				await remoteExplorerService.close(
					{ host: result.tunnel.remoteHost, port: result.tunnel.remotePort },
					TunnelCloseReason.User,
				);
			} else if (result) {
...
namespace CopyAddressAction {
	...
	export function commandPaletteHandler(): ICommandHandler {
		return async (accessor, arg) => {
			...
			).concat(Array.from(remoteExplorerService.tunnelModel.detected.values()));
			...
			if (tunnelContext) {
				...
				remoteExplorerService.setEditable(tunnelItem, TunnelEditId.LocalPort, {
					onFinish: async (value, success) => {
						...
						if (success) {
							await remoteExplorerService.close(
								{ host: tunnelItem.remoteHost, port: tunnelItem.remotePort },
					...
					validationMessage: (value) =>
						validateInput(tunnelService, value, tunnelService.canElevate),
					placeholder: nls.localize(
