	...
	implements IWorkbenchContribution
{
	...
	private readonly connectionStateContextKey: IContextKey<
	...
	constructor(
		...
		this.connectionStateContextKey = new RawContextKey<
			"" | "initializing" | "disconnected" | "connected"
						...
						constructor() {
							super({
								...
								precondition: ContextKeyExpr.and(
									ContextKeyExpr.or(RemoteNameContext, VirtualWorkspaceContext),
	...
	private registerListeners(): void {
		...
		this._register(
			this.extensionService.onDidChangeExtensions(async (result) => {
				for (const ext of result.added) {
					const index = this.remoteExtensionMetadata.findIndex((value) =>
		...
		this._register(
			this.extensionManagementService.onDidUninstallExtension(
				async (result) => {
					const index = this.remoteExtensionMetadata.findIndex((value) =>
						ExtensionIdentifier.equals(value.id, result.identifier.id),
					);
					if (index > -1) {
	...
	private validatedGroup(group: string) {
		if (
			!group.match(/^(remote|virtualfs)_(\d\d)_(([a-z][a-z0-9+.-]*)_(.*))$/)
		) {
	...
	private getRemoteMenuActions(doNotUseCache?: boolean): ActionGroup[] {
		if (!this.remoteMenuActionsGroups || doNotUseCache) {
			this.remoteMenuActionsGroups = this.remoteIndicatorMenu
				...
				.concat(this.unrestrictedRemoteIndicatorMenu.getActions());
		}
	...
	private renderRemoteStatusIndicator(
		...
		const properties: IStatusbarEntry = {
			...
			kind:
				this.networkState === "offline"
					? "offline"
					: text !== RemoteStatusIndicator.DEFAULT_REMOTE_STATUS_LABEL
						? "remote"
						: undefined, // only emphasize when applicable
			ariaLabel,
			text,
			showProgress,
			tooltip,
		...
		if (this.remoteStatusEntry) {
			...
			this.remoteStatusEntry = this.statusbarService.addEntry(
				...
				Number.POSITIVE_INFINITY /* first entry */,
			);
		}
	}
	...
	private appendTooltipLine(
		...
		if (typeof tooltip === "string") {
			...
			markdownTooltip =
				tooltip ??
				new MarkdownString("", { isTrusted: true, supportThemeIcons: true });
		}
