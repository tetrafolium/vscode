...
{
	private readonly _onDidChangeSchemaContributions = this._register(
		new Emitter<void>(),
	);
	...
	private updateConfigurationSchema(
		allProvidedKinds: Iterable<HierarchicalKind>,
	): void {
		...
		for (const codeActionKind of allProvidedKinds) {
			if (
				CodeActionKind.Source.contains(codeActionKind) &&
				!properties[codeActionKind.value]
			) {
