...
export class SearchEditor extends AbstractTextCodeEditor<SearchEditorViewState> {
	...
	protected override createEditor(parent: HTMLElement) {
		...
		const scopedContextKeyService = assertReturnsDefined(
	...
	getSelected() {
		...
		if (selection) {
			return (
				this.searchResultEditor.getModel()?.getValueInRange(selection) ?? ""
			);
		}
		return "";
	}
	...
	private reLayout() {
		if (this.dimension) {
			...
			this.searchResultEditor.layout({
				height:
					this.dimension.height - DOM.getTotalHeight(this.queryEditorContainer),
	...
	private toggleIncludesExcludes(_shouldShow?: boolean): void {
		const cls = "expanded";
		const shouldShow =
			_shouldShow ?? !this.includesExcludesContainer.classList.contains(cls);
