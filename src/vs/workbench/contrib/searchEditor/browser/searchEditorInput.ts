...
export class SearchEditorInput extends EditorInput {
	...
	override copy(): EditorInput {
		...
		return this.instantiationService.invokeFunction(
			...
			} as any, // modelUri is not in the type, but we handle it below
		);
	}
}
	...
	if (!searchEditorModelFactory.models.has(modelUri)) {
		if (existingData.from === "existingFile") {
			...
			const priorConfig = reuseOldSettings
				? (new Memento<{ searchConfig?: SearchConfiguration }>(
			...
			if (
				...
				config.contextLines =
					existingData?.config?.contextLines ?? defaultNumberOfContextLines;
			}
