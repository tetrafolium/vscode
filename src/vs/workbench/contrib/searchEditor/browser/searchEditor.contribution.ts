	...
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor) {
			...
			if (
				...
				(
					accessor.get(IEditorService).activeEditorPane as SearchEditor
				).deleteResultBlock();
			}
		}
	},
	...
	class extends Action2 {
		constructor() {
			super({
				...
				menu: [
					...[MenuId.EditorTitle, MenuId.CompactWindowEditorTitle].map(
						(id) => ({
							id,
							group: "navigation",
	...
	implements IWorkbenchContribution, IWorkingCopyEditorHandler
{
	...
	handles(workingCopy: IWorkingCopyIdentifier): boolean {
		return (
			workingCopy.resource.scheme === SearchEditorConstants.SearchEditorScheme
		);
	}
