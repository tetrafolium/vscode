...
export class NativeIssueService implements IWorkbenchIssueService {
	...
	private async openWizardReporter(
		dataOverrides: Partial<IssueReporterData>,
	): Promise<void> {
		...
		this.populateReporterDataAsync(
			issueReporterData,
			dataOverrides,
			extensionsLoaded,
		).then(
	...
	private async openLegacyReporter(
		dataOverrides: Partial<IssueReporterData>,
	): Promise<void> {
		...
		try {
			...
			const repoSession = githubSessions.find((session) =>
	...
	private async populateReporterDataAsync(
		data: IssueReporterData,
		dataOverrides: Partial<IssueReporterData>,
		extensionsLoaded?: DeferredPromise<void>,
	): Promise<void> {
		// Extensions
		try {
			const extensions = await this.extensionManagementService.getInstalled();
			...
			data.enabledExtensions = enabledExtensions.map(
