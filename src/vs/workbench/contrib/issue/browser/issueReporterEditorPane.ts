...
export class IssueReporterEditorPane extends EditorPane {
	...
	private shouldShowUpdateBanner(): boolean {
		return (
			this.updateService.state.type === StateType.AvailableForDownload ||
	...
	override async setInput(
		...
		this.inputDisposables.add(
			this.wizard.onDidRequestScreenshot(async () => {
				try {
					...
					this.logService.error(
						"[IssueReporterEditorPane] Screenshot failed:",
						err,
					);
				}
			}),
		...
		this.inputDisposables.add(
			this.wizard.onDidRequestStopRecording(async () => {
				try {
					...
					this.logService.error(
						"[IssueReporterEditorPane] Stop recording failed:",
						err,
					);
					this.wizard?.setRecordingState(RecordingState.Idle);
				}
		...
		this.inputDisposables.add(
			this.recordingService.onDidChangeState(async (state) => {
				...
				if (
					...
					this.wizard?.recordingState === RecordingState.Recording
				) {
					try {
						const recordingData = await this.recordingService.stopRecording();
						if (recordingData) {
		...
		this.inputDisposables.add(
			this.wizard.onDidRequestOpenScreenshot(async (screenshot) => {
				try {
					...
					const extension = dataUrl.startsWith("data:image/jpeg")
						? "jpg"
						: "png";
					...
					await this.fileService.writeFile(
						target,
						decodeBase64(dataUrl.substring(commaIndex + 1)),
					);
					await this.editorService.openEditor({ resource: target });
				} catch (err) {
					this.logService.error(
						"[IssueReporterEditorPane] Open screenshot failed:",
						err,
					);
				}
			}),
		...
		this.inputDisposables.add(
			this.wizard.onDidRequestOpenRecording(async (filePath) => {
				try {
		...
		this.inputDisposables.add(
			this.wizard.onDidSubmit(async ({ title, body }) => {
				if (!this.wizard) {
					return;
				}
				const opened = await this.issueFormService.submitIssue(
					this.wizard,
					data,
					title,
					body,
		...
		this.inputDisposables.add(
			this.wizard.onDidRequestGenerateTitle(async (description) => {
				try {
					...
					if (modelIds.length === 0) {
						this.logService.warn(
							"[IssueReporterEditorPane] No language models available for title generation",
						);
					...
					const title = (await getTextResponseFromStream(response))
						.trim()
						.replace(/^["']|["']$/g, "");
					...
					this.logService.error(
						"[IssueReporterEditorPane] Title generation failed:",
						err,
					);
					this.wizard?.resetGenerateButton();
				}
	...
	private async fetchPerformanceInfo(options?: {
		skipCache?: boolean;
		unbounded?: boolean;
	}): Promise<void> {
		if (!this.wizard) {
		...
		try {
			const performanceInfo =
				await this.processService.getPerformanceInfo(options);
	...
	private generateVideoThumbnail(fileUri: URI): Promise<string | undefined> {
		...
		return new Promise((resolve) => {
			...
			video.preload = "auto";
			video.style.cssText =
				"position:fixed;top:-9999px;left:-9999px;width:320px;height:240px;opacity:0;pointer-events:none;";
