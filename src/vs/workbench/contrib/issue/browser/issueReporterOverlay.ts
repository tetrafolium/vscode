...
export class IssueReporterOverlay {
	...
	private readonly _onDidSubmit = new Emitter<{
		title: string;
		body: string;
	}>();
	readonly onDidSubmit: Event<{ title: string; body: string }> =
		this._onDidSubmit.event;
	private readonly _onDidRequestScreenshot = new Emitter<void>();
	readonly onDidRequestScreenshot: Event<void> =
		this._onDidRequestScreenshot.event;
	private readonly _onDidRequestStartRecording = new Emitter<void>();
	readonly onDidRequestStartRecording: Event<void> =
		this._onDidRequestStartRecording.event;
	private readonly _onDidRequestStopRecording = new Emitter<void>();
	readonly onDidRequestStopRecording: Event<void> =
		this._onDidRequestStopRecording.event;
	private readonly _onDidRequestOpenRecording = new Emitter<string>();
	readonly onDidRequestOpenRecording: Event<string> =
		this._onDidRequestOpenRecording.event;
	private readonly _onDidRequestOpenScreenshot = new Emitter<IScreenshot>();
	readonly onDidRequestOpenScreenshot: Event<IScreenshot> =
		this._onDidRequestOpenScreenshot.event;
	...
	readonly onDidRequestGenerateTitle: Event<string> =
		this._onDidRequestGenerateTitle.event;
	...
	constructor(
		...
		private readonly resolveExtensionIssueData?: (
			extensionId: string,
		...
		this.selectedIssueSource =
			data.issueSource ??
			(data.extensionId ? IssueSource.Extension : undefined);
	...
	private createStep0Attachments(): void {
		...
		const recordShortcut = this.recordingSupported
	...
	private createFloatingCaptureBar(): void {
		...
		if (this.contextMenuProvider) {
			...
			this.disposables.add(
				delayDropdownButton.onDidClick(() => {
		...
		this.disposables.add(
			captureBtn.onDidClick(() => {
				if (
					this.getTotalAttachments() >= MAX_ATTACHMENTS ||
					!captureBtn.enabled
				) {
					return;
				}
				if (this.screenshotDelay > 0) {
					...
					const targetWindow = getWindow(this.container);
					const intervalDisposable = this.disposables.add(
						disposableWindowInterval(
							targetWindow,
							() => {
								remaining--;
		...
		if (this.recordingSupported) {
			...
			this.disposables.add(
				this.captureStripRecordBtn.onDidClick(() => {
	...
	private createStep1Describe(): void {
		...
		for (const option of this.getSourceOptions()) {
			...
			this.disposables.add(
				btn.onDidClick(() => {
					...
					if (
						option.value === IssueSource.Extension &&
		...
		this.disposables.add(
			this.extensionSelect.onDidSelect((e) => {
				void this.updateSelectedExtension(
					this.extensionOptions[e.index]?.value,
				);
			}),
		);
		...
		this.titleInput = this.disposables.add(
			new InputBox(titleGroup, undefined, {
		...
		autoGrowTextarea();
		this.disposables.add(
			addDisposableListener(this.descriptionTextarea, EventType.INPUT, () => {
	...
	private updateIssueSourceFlags(): void {
		...
		const fileOnMarketplace =
			this.selectedIssueSource === IssueSource.Marketplace;
		const fileOnProduct =
			this.selectedIssueSource === IssueSource.VSCode ||
			this.selectedIssueSource === IssueSource.Unknown;
		this.model.update({
	...
	private getExtensionOptions(): {
		...
		const sourceExtensions =
			modelData.enabledNonThemeExtesions ?? modelData.allExtensions ?? [];
		const extensions = [...sourceExtensions]
			.filter((extension) => !extension.isTheme && !extension.isBuiltin)
			.sort((a, b) =>
				(a.displayName || a.name || a.id).localeCompare(
		...
		return [
			...
			...extensions.map((extension) => ({
				label: extension.displayName || extension.name || extension.id,
				value: extension.id,
			})),
		];
	}
	...
	private getSelectedExtensionIndex(): number {
		return Math.max(
			...
			this.extensionOptions.findIndex(
				(option) =>
					option.value === this.selectedExtension?.id ||
					option.value === this.data.extensionId,
			),
		);
	}
	...
	private updateExtensionValidation(): void {
		const hasExtension =
			this.selectedIssueSource !== IssueSource.Extension ||
			!!this.selectedExtension;
		const hasExtensionIssueUrl =
			this.selectedIssueSource !== IssueSource.Extension ||
			!this.selectedExtension ||
			!!this.getSelectedExtensionIssueUrl();
		this.setFieldError(
			this.extensionField,
			this.extensionError,
	...
	private async updateSelectedExtension(
		...
		const extension = extensionId
			? this.model
					...
					.allExtensions.find(
						(candidate) =>
							candidate.id.toLowerCase() === extensionId.toLowerCase(),
					)
			: undefined;
	...
	private applyExtensionIssueData(
		extension: IssueReporterExtensionData,
		issueData: IssueReporterData,
	): void {
		...
		if (
			issueData.issueBody &&
			!this.descriptionTextarea.value.includes(issueData.issueBody)
		) {
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			...
			case IssueSource.Extension:
				return (
					this.selectedExtension?.displayName ||
					this.selectedExtension?.name ||
					localize("extensionSource", "A VS Code extension")
				);
			case IssueSource.Marketplace:
	...
	private parseGitHubUrl(
		url: string,
	): { owner: string; repositoryName: string } | undefined {
	...
	private async doSearchSimilarIssues(): Promise<void> {
		...
		try {
			...
			if (this.selectedIssueSource === IssueSource.Extension) {
				...
				const repo =
					extensionIssueUrl && this.parseGitHubUrl(extensionIssueUrl);
				results = repo
				...
				const marketplaceIssueUrl =
					product.reportMarketplaceIssueUrl ?? product.reportIssueUrl;
				const repo =
					marketplaceIssueUrl && this.parseGitHubUrl(marketplaceIssueUrl);
				results = repo
					? await this.searchGitHubIssues(
							`${repo.owner}/${repo.repositoryName}`,
							title,
	...
	private async searchVSCodeDuplicates(
		title: string,
		body: string,
	): Promise<ISimilarIssue[]> {
		const response = await fetch(
	...
	private async searchVSCodeSimilarIssues(
		title: string,
		body: string,
	): Promise<ISimilarIssue[]> {
		try {
		...
		const repo = this.getIssueTargetRepo();
		return repo
			? this.searchGitHubIssues(`${repo.owner}/${repo.repositoryName}`, title)
			: [];
	}
	...
	private updateDescriptionGuidance(): void {
		...
		switch (this.selectedIssueType) {
			case IssueType.Bug:
				this.descriptionGuidance.textContent = `${localize(
					"bugGuidance",
	...
	private updateGenerateTitleButtonState(): void {
		if (
			!this.generateTitleBtn ||
			this.generateTitleBtn.element.classList.contains("loading")
		) {
			return;
		}
	...
	private goNext(): void {
			...
			if (
				...
				if (!hasIssueSource) {
					this.issueSourceButtons
						.find((button) => !button.element.classList.contains("hidden"))
						?.element.focus();
		...
		if (this.currentStep === WizardStep.Review) {
			...
			if (
				this.selectedIssueType === IssueType.PerformanceIssue &&
				(!this.performanceInfoLoaded || this.performanceInfoRefreshing)
			) {
				return;
			}
			this.submit();
	...
	private updateStepUI(): void {
		...
		for (let i = 0; i < this.stepPages.length; i++) {
			if (i === this.currentStep) {
				...
				!this.stepPages[i].classList.contains("slide-out-left") &&
		...
		this.backButton.element.style.display =
			this.currentStep === WizardStep.Attachments ? "none" : "";
		if (this.closeButton) {
			const currentDraftPreviewed =
				this.previewedDraftKey === this.getDraftKey();
			this.closeButton.element.style.display =
				this.previewOpened &&
				currentDraftPreviewed &&
				this.currentStep === WizardStep.Review
					? ""
					: "none";
		}
		...
		// Next button label
		if (this.currentStep === WizardStep.Review) {
			const externalExtensionUrl =
				this.selectedIssueSource === IssueSource.Extension &&
				this.getIssueTargetUrl() &&
				!this.isGitHubUrl(this.getIssueTargetUrl()!);
	...
	private updateReviewDetails(): void {
		...
		const typeLabels: Record<number, string> = {
			...
			[IssueType.PerformanceIssue]: localize(
				"performanceIssue",
				"Performance Issue",
			),
		};
		catValue.textContent =
			(this.selectedIssueType !== undefined
				? typeLabels[this.selectedIssueType]
				: undefined) ?? localize("unknown", "Unknown");
		...
		titleValue.textContent =
			this.titleInput.value.trim() || localize("noTitle", "(no title)");
		...
		if (description && this.markdownRendererService) {
			...
			descValue.textContent =
				description || localize("noDescription", "(no description)");
		}
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(
							sysTable,
							"VS Code",
							modelData.versionInfo.vscodeVersion,
						);
					...
					if (modelData.systemInfo) {
						...
						this.addDiagRow(
							sysTable,
							"Screen Reader",
							modelData.systemInfo.screenReader,
						);
					}
		...
		if (
			!modelData.fileOnExtension &&
			!modelData.fileOnMarketplace &&
			nonThemeExtensions.length > 0
		) {
			diagnosticSectionCount++;
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					const extTable = append(
						container,
						$("table.review-diag-table.review-ext-table"),
					);
		...
		if (
			...
			if (this.refreshPerformanceInfo) {
				...
				this.disposables.add(
					refreshBtn.onDidClick(async () => {
						if (
							!this.refreshPerformanceInfo ||
							this.performanceInfoRefreshing
						) {
							return;
						}
	...
	private areAllVisibleDiagnosticSectionsIncluded(): boolean {
		return (
			this.diagnosticSectionStates.length > 0 &&
			this.diagnosticSectionStates.every((getState) => getState())
		);
	}
	...
	private updateDiagnosticBulkToggleButton(): void {
		...
		this.diagnosticBulkToggleButton.element.setAttribute(
			"aria-label",
			allChecked
				? localize(
	...
	private updateAttachmentButtons(): void {
		...
		const screenshotDisabled =
			...
			this.delayedScreenshotPending;
		// Record disabled when: at max, OR delayed screenshot will fill the last slot
		const recordDisabled =
			atMax || (wouldReachMax && this.delayedScreenshotPending);

		if (this.captureStripCaptureBtn) {
			this.captureStripCaptureBtn.enabled = !screenshotDisabled;
			this.captureStripCaptureBtn.element.title = screenshotDisabled
				? maxMsg
				: localize("screenshot", "Screenshot");
		}
		if (this.captureStripDelayBtn) {
			...
			this.captureStripDelayBtn.element.title = screenshotDisabled
				? maxMsg
				: localize("captureOptions", "Capture options");
		}
		if (this.captureStripRecordBtn) {
			if (this.currentRecordingState !== RecordingState.Recording) {
				...
				this.captureStripRecordBtn.element.title = recordDisabled
					? maxMsg
	...
	private renderRecordingCard(
		parent: HTMLElement,
		rec: { filePath: string; durationMs: number; thumbnailDataUrl?: string },
		index: number,
	): HTMLElement {
		const card = append(
			parent,
	...
	private updateScreenshotThumbnails(): void {
		...
		for (let i = 0; i < this.screenshots.length; i++) {
			...
			this.disposables.add(
				addDisposableListener(card, EventType.KEY_DOWN, (e) => {
		...
		if (this.getTotalAttachments() < MAX_ATTACHMENTS) {
			...
			this.disposables.add(
				addDisposableListener(addCard, EventType.CLICK, () => {
	...
	private buildIssueBody(): string {
		...
		if (
			this.includeSystemInfo &&
			(modelData.versionInfo || modelData.systemInfo || modelData.systemInfoWeb)
		) {
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.systemInfo) {
			rows.push(
				...
				[
					"GPU Status",
					Object.keys(modelData.systemInfo.gpuStatus)
						.map((key) => `${key}: ${modelData.systemInfo!.gpuStatus[key]}`)
						.join("<br>"),
				],
	...
	private generateExtensionsMd(): string {
		...
		const nonThemeExtensions =
			modelData.enabledNonThemeExtesions ??
			modelData.allExtensions.filter(
		...
		const rows = nonThemeExtensions.map(
			(extension) =>
				[
...
\`\`\``;
	...
	private createMarkdownTable(
		...
		headers: readonly string[] = ["Item", "Value"],
	): string {
	...
	updateModel(newData: Record<string, unknown>): void {
		...
		if (Array.isArray(newData.allExtensions)) {
			this.data.enabledExtensions =
				newData.allExtensions as IssueReporterExtensionData[];
			this.updateExtensionOptions();
			this.updateIssueSourceFlags();
		}
	...
	private getDraftKey(): string {
		return JSON.stringify({
			...
			screenshots: this.screenshots.map(
				(screenshot) => screenshot.annotatedDataUrl ?? screenshot.dataUrl,
			),
