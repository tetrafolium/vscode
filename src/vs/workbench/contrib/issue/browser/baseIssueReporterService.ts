...
export class BaseIssueReporterService extends Disposable {
	...
	constructor(
		...
		@IContextMenuService
		public readonly contextMenuService: IContextMenuService,
		...
		this.createAction = this._register(
			new Action(
				"issueReporter.create",
				localize("create", "Create on GitHub"),
				undefined,
				true,
				async () => {
		...
		this.previewAction = this._register(
			new Action(
				"issueReporter.preview",
				localize("preview", "Preview on GitHub"),
				undefined,
				true,
				async () => {
					this.delayedSubmit.trigger(async () => {
	...
	private updatePublicGithubButton(container: HTMLElement): void {
		...
		if (!this.acknowledged && this.needsUpdate) {
			...
			this.publicGithubButton = this._register(
				new Button(container, unthemedButtonStyles),
			);
	...
	private updatePublicRepoLink(container: HTMLElement): void {
		// eslint-disable-next-line no-restricted-syntax
		let issueRepoName = this.getElementById(
			"show-repo-name",
	...
	private updateInternalGithubButton(container: HTMLElement): void {
		...
		if (this.data.githubAccessToken && this.data.privateUri) {
			this.internalGithubButton = this._register(
				new Button(container, unthemedButtonStyles),
			);
			this._register(
	...
	private updateInternalElementsVisibility(): void {
		...
		if (this.data.githubAccessToken && this.data.privateUri) {
			...
			if (this.internalGithubButton) {
				this.internalGithubButton.enabled =
					this.publicGithubButton?.enabled ?? false;
			}
		} else {
			hide(container);
	...
	private setSubmittingState(submitting: boolean): void {
		...
		if (submitting) {
			const currentLabel =
				this.publicGithubButton instanceof ButtonWithDropdown
					? this.publicGithubButton.primaryButton.label
					: this.publicGithubButton.label;
			this.preSubmitButtonLabel =
	...
	private handleExtensionData(extensions: IssueReporterExtensionData[]) {
		...
		if (this.disableExtensions || installedExtensions.length === 0) {
			// eslint-disable-next-line no-restricted-syntax
			(<HTMLButtonElement>this.getElementById("disableExtensions")).disabled =
				true;
		}
	...
	private async sendReporterMenu(
		...
		try {
			const timeoutPromise = new Promise<undefined>((_, reject) =>
				setTimeout(
					() => reject(new Error("sendReporterMenu timed out")),
					10000,
				),
			);
	...
	private updateAcknowledgementState() {
		// eslint-disable-next-line no-restricted-syntax
		const acknowledgementCheckbox = this.getElementById<HTMLInputElement>(
	...
	public setEventHandlers(): void {
			...
			(showInfo as HTMLAnchorElement).addEventListener(
				...
				(e: MouseEvent) => {
					...
					if (label) {
						...
						const info =
							containingElement && containingElement.lastElementChild;
		...
		this.addEventListener("issue-title", "input", (e: Event) => {
			...
			if (
				title &&
				this.getIssueUrlWithTitle(title, issueUrl).length > MAX_URL_LENGTH
			) {
	...
	public async renderBlocks(): Promise<void> {
		...
		const { issueType, fileOnExtension, fileOnMarketplace, selectedExtension } =
			this.issueReporterModel.getData();
		// eslint-disable-next-line no-restricted-syntax
		...
		// eslint-disable-next-line no-restricted-syntax
		const downloadExtensionDataLink = <HTMLAnchorElement>(
			this.getElementById("extension-data-download")!
		);
		...
		if (extensionData && extensionData.length > MAX_EXTENSION_DATA_LENGTH) {
			...
			this._register({
				dispose: () =>
					downloadExtensionDataLink.removeEventListener(
						"click",
						handleLinkClick,
					),
			});
	...
	public addTemplateToUrl(
		...
		const needsTemplate =
			isVscode ||
			(isMicrosoft &&
				(repositoryName === "vscode" || repositoryName === "vscode-python"));
	...
	private async openLink(eventOrUrl: MouseEvent | string): Promise<void> {
		if (typeof eventOrUrl === "string") {
			...
			// Exclude right click
			if (event.which < 3) {
				await this.openerService.open((<HTMLAnchorElement>event.target).href, {
					openExternal: true,
				});
			}
		}
