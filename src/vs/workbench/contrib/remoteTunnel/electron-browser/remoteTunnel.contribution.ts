	...
	implements IWorkbenchContribution
{
	...
	private async recommendRemoteExtensionIfNeeded() {
		...
		const shouldRecommend = async () => {
			...
			if (
				await this.extensionService.getExtension(remoteExtension.extensionId)
			) {
				return false;
			}
	...
	> {
		...
		const quickpick = disposables.add(
			this.quickInputService.createQuickPick<
				ExistingSessionItem | AuthenticationProviderOption | IQuickPickItem
			>({ useSeparators: true }),
		);
		quickpick.ok = false;
			...
			disposables.add(
				quickpick.onDidAccept(async (e) => {
					...
					if ("provider" in selection) {
						...
						resolve(
							this.createExistingSessionItem(session, selection.provider.id),
						);
			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor) {
						...
						const asService = await new Promise<boolean | undefined>(
							(resolve) => {
								disposables.add(
