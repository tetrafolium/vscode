...
suite("ApplyFileSnippetAction", () => {
	...
	setup(() => {
		...
		const langConfigService = disposables.add(
			new TestLanguageConfigurationService(),
		);
		disposables.add(
			langConfigService.register("csharp", {
				comments: {
					lineComment: "//",
					blockComment: ["/*", "*/"],
				},
		...
		disposables.add(
			langService.registerLanguage({ id: "csharp", extensions: [".cs"] }),
		);
		...
		instantiationService.stub(
			ISnippetsService,
			new (class extends mock<ISnippetsService>() {
		...
		instantiationService.stub(
			...
			new (class extends mock<IQuickInputService>() {
				override async pick<T extends IQuickPickItem>(
	...
	test("resolves comment variables using selected file template language", async () => {
		await instantiationService.invokeFunction((accessor) =>
			new ApplyFileSnippetAction().run(accessor),
		);
