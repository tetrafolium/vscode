	...
	getSnippetsSync(languageId?: string, resourceUri?: URI): Snippet[] {
		...
		if (resourceUri) {
			return this.snippets.filter((snippet) =>
				snippet.isFileIncluded(resourceUri),
			);
		}
...
suite("SnippetsService", function () {
	...
	test('Cannot use "<?php" as user snippet prefix anymore, #26275', async function () {
		...
		assert.strictEqual(
			(completions2.items[0].completion.range as CompletionItemRanges).insert
				.startColumn,
			2,
		);
		...
		assert.strictEqual(
			(completions3.items[0].completion.range as CompletionItemRanges).insert
				.startColumn,
			2,
		);
		model.dispose();
	...
	test("Snippet replace range", async function () {
		...
		let [first] = result.suggestions;
		assert.strictEqual(
			(first.range as CompletionItemRanges).insert.endColumn,
			3,
		);
		assert.strictEqual(
			(first.range as CompletionItemRanges).replace.endColumn,
			9,
		);
	...
	test("getSnippetsSync - include pattern", function () {
		...
		let snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/project/src/foo.test.ts"),
		);
		...
		snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/project/src/foo.spec.ts"),
		);
		assert.strictEqual(snippets.length, 2);
	...
	test("getSnippetsSync - exclude pattern", function () {
		...
		snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/project/src/foo.min.js"),
		);
		assert.strictEqual(snippets.length, 1);
		assert.strictEqual(snippets[0].name, "AllSnippet");
		...
		snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/project/dist/bundle.js"),
		);
		assert.strictEqual(snippets.length, 1);
		assert.strictEqual(snippets[0].name, "AllSnippet");
	});
	...
	test("getSnippetsSync - include and exclude patterns together", function () {
		...
		snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/project/src/foo.perf.test.ts"),
		);
	...
	test("getSnippetsSync - filename-only patterns (no path separator)", function () {
		...
		snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/other/deep/path/bar.test.ts"),
		);
		assert.strictEqual(snippets.length, 1);
		assert.strictEqual(snippets[0].name, "TestSnippet");
		...
		snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/deep/nested/path/config.json"),
		);
		assert.strictEqual(snippets.length, 1);
		assert.strictEqual(snippets[0].name, "ConfigSnippet");
		...
		snippets = snippetService.getSnippetsSync(
			"fooLang",
			URI.file("/project/myconfig.json"),
		);
		assert.strictEqual(snippets.length, 0);
	});
});
