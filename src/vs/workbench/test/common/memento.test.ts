...
suite("Memento", () => {
	...
	test("Loading and Saving Memento with Scopes", () => {
		const myMemento = new Memento<{ foo: number[] | string }>(
			"memento.test",
			storage,
		);
		...
		assert.deepStrictEqual(
			JSON.parse(storage.get("memento/memento.test", StorageScope.PROFILE)!),
			{ foo: [4, 5, 6] },
		);
		assert.deepStrictEqual(
		...
		assert.strictEqual(
			storage.get("memento/memento.test", StorageScope.APPLICATION, null!),
			null,
		);
		assert.strictEqual(
	...
	test("Clear Memento", () => {
		let myMemento = new Memento<{ foo: string; bar: string }>(
			"memento.test",
			storage,
		);
