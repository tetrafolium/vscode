...
export class TestNativeHostService implements INativeHostService {
	...
	async getWindows(): Promise<IOpenedMainWindow[]> {
		return [];
	}
	...
	async isWindowAlwaysOnTop(options?: INativeHostOptions): Promise<boolean> {
		return false;
	}
	...
	async getMediaAccessStatus(
		_mediaType: "microphone" | "camera" | "screen",
	): Promise<
	...
	async relaunch(
		options?:
			| { addArgs?: string[] | undefined; removeArgs?: string[] | undefined }
			| undefined,
	): Promise<void> {}
	...
	async openDevTools(
		options?:
			| (Partial<Electron.OpenDevToolsOptions> & INativeHostOptions)
			| undefined,
	): Promise<void> {}
	...
	async readClipboardText(
		type?: "selection" | "clipboard" | undefined,
	): Promise<string> {
		return "";
	}
	...
	async profileRenderer(): Promise<any> {
		throw new Error();
	}
	...
	const instantiationService = browserWorkbenchInstantiationService(
		{
			workingCopyBackupService: () =>
				disposables.add(new TestNativeWorkingCopyBackupService()),
			...overrides,
		},
		disposables,
	);
...
export class TestServiceAccessor {
	constructor(
		...
		public workingCopyBackupService: TestNativeWorkingCopyBackupService,
...
export class TestNativeWorkingCopyBackupService
	extends NativeWorkingCopyBackupService
