...
import {
	...
	TreeSitterTree,
} from "../../../editor/common/model/tokens/treeSitter/treeSitterTree.js";
...
suite("Tree Sitter TokenizationFeature", function () {
	...
	setup(async () => {
		...
		languageService = disposables.add(
			instantiationService.createInstance(LanguageService),
		);
		instantiationService.set(ILanguageService, languageService);
		instantiationService.set(IThemeService, themeService);
		textResourcePropertiesService = instantiationService.createInstance(
		...
		const libraryService = disposables.add(
		...
		instantiationService.set(
			...
			instantiationService.createInstance(TreeSitterThemeService),
		);
	...
	function tokensContentSize(tokens: TokenUpdate[]) {
		return (
			tokens[tokens.length - 1].startOffsetInclusive +
	...
	async function getModelAndPrepTree(
		...
		const treeSitterTreeObs = disposables.add(
			(
				model.tokenization as TokenizationTextModelPart
		...
		const treeSitterTree =
			treeSitterTreeObs.get() ?? (await waitForState(treeSitterTreeObs));
		if (!treeSitterTree.tree.get()) {
			await waitForState(treeSitterTree.tree);
		}
		const tokenizationImpl =
			tokenizationImplObs.get() ?? (await waitForState(tokenizationImplObs));
	...
	function verifyTokens(tokens: TokenUpdate[] | undefined) {
		...
		for (let i = 1; i < tokens.length; i++) {
			...
			assert.deepStrictEqual(
				previousToken.startOffsetInclusive + previousToken.length,
				token.startOffsetInclusive,
			);
		}
	}
...
class y {
		...
		const changePromise = new Promise<TreeParseUpdateEvent | undefined>(
			(resolve) => {
	...
	test("File single line file", async () => {
		...
		const tokens = tokenizationImpl.getTokensInRange(
			new Range(1, 1, 1, 18),
			0,
			17,
		);
