...
} from "../../../../contrib/chat/common/aiCustomizationWorkspaceService.js";
import {
	...
	getCliUserRoots,
} from "../../../../contrib/chat/common/customizationHarnessService.js";
import {
...
import {
	getChatSessionType,
	LocalChatSessionUri,
} from "../../../../contrib/chat/common/model/chatUri.js";
import {
	IPromptsService,
...
import {
	ParsedPromptFile,
	PromptFileParser,
} from "../../../../contrib/chat/common/promptSyntax/promptFileParser.js";
import {
	IAgentPluginService,
	IAgentPlugin,
} from "../../../../contrib/chat/common/plugins/agentPluginService.js";
import {
	IPluginMarketplaceService,
	IMarketplacePlugin,
	MarketplaceType,
	PluginSourceKind,
} from "../../../../contrib/chat/common/plugins/pluginMarketplaceService.js";
...
} from "../../../../contrib/chat/browser/aiCustomization/aiCustomizationItemSource.js";
import {
	AICustomizationItemsModel,
	IAICustomizationItemsModel,
	ItemsModelSection,
} from "../../../../contrib/chat/browser/aiCustomization/aiCustomizationItemsModel.js";
...
import {
	AgentPluginItemKind,
	IAgentPluginItem,
} from "../../../../contrib/chat/browser/agentPluginEditor/agentPluginItems.js";
...
import {
	IWorkbenchLocalMcpServer,
	LocalMcpServerScope,
} from "../../../../services/mcp/common/mcpWorkbenchManagementService.js";
...
function createMockAICustomizationItemsModel(): IAICustomizationItemsModel {
	const itemSource = new (class extends mock<IAICustomizationItemSource>() {
		...
		override async fetchProviderItems() {
			return [];
		}
		override async fetchAICustomizationItems(_promptType: PromptsType) {
			return [];
		}
	})();
	...
	return new (class extends mock<IAICustomizationItemsModel>() {
		override getItems(
			_section: ItemsModelSection,
		): IObservable<readonly IAICustomizationListItem[]> {
			return constObservable([]);
		}
		override getActiveItemSource() {
			return itemSource;
		}
		...
		override async whenSectionLoaded(
			_section: ItemsModelSection,
		): Promise<void> {}
	})();
}
...
function toExtensionInfo(
	file: IFixtureFile,
): { identifier: ExtensionIdentifier; displayName?: string } | undefined {
	if (!file.extensionId) {
		return undefined;
	}
...
function createFixtureFileContent(file: IFixtureFile): string {
	...
	if (file.type === PromptsType.prompt) {
		headerLines.push(
			`argument-hint: ${JSON.stringify("Paste the failing stack trace")}`,
		);
	}
...
function createFixtureContentMap(
	files: IFixtureFile[],
	instructions: IAgentInstructionFile[],
): ResourceMap<string> {
...
function createMockPromptsService(
	...
	return new (class extends mock<IPromptsService>() {
		...
		override getPromptLocationLabel() {
			return "";
		}
		...
		override async listAgentInstructions() {
			return agentInstructions;
		}
		override async getCustomAgents() {
			return files
				.filter((f) => f.type === PromptsType.agent)
				.map((a) => ({
					uri: a.uri,
					name: a.name ?? "agent",
					...
					source: {
						...
						extensionId: a.extensionId
							? new ExtensionIdentifier(a.extensionId)
							: undefined,
					},
					visibility: { userInvocable: true, agentInvocable: true },
				})) as never[];
		}
		override async parseNew(
			uri: URI,
			_token: CancellationToken,
		...
		override async getSourceFolders() {
			return [] as never[];
		}
		override async getInstructionFiles() {
			return files
				.filter((f) => f.type === PromptsType.instructions)
				.map((f) => ({
					uri: f.uri,
					name: f.name ?? "",
					description: f.description,
		...
		override async findAgentSkills(): Promise<IAgentSkill[]> {
			return files
				.filter((f) => f.type === PromptsType.skill)
				.map((f) => ({
					uri: f.uri,
					storage: f.storage,
					name: f.name ?? "skill",
		...
		override async getPromptSlashCommands(): Promise<
			readonly IChatPromptSlashCommand[]
		> {
...
function createMockHarnessService(
	sessionResource: URI,
	descriptors: readonly IHarnessDescriptor[],
): ICustomizationHarnessService {
	const activeSessionResource = observableValue<URI>(
		"activeSessionResource",
		sessionResource,
	);
	const activeHarness = derived((reader) =>
		getChatSessionType(activeSessionResource.read(reader)),
	);
	return new (class extends mock<ICustomizationHarnessService>() {
		...
		override getStorageSourceFilter(type: PromptsType) {
			const d =
				descriptors.find((h) => h.id === activeHarness.get()) ?? descriptors[0];
		...
		override registerExternalHarness() {
			return { dispose() {} };
		}
	})();
}

function makeLocalMcpServer(
	id: string,
	label: string,
	scope: LocalMcpServerScope,
	description?: string,
	config?: IWorkbenchMcpServer["config"],
): IWorkbenchMcpServer {
	return new (class extends mock<IWorkbenchMcpServer>() {
		...
		override readonly local =
			new (class extends mock<IWorkbenchLocalMcpServer>() {
				override readonly id = id;
...
function createMockAgentFeedbackService(): IAgentFeedbackService {
	return new (class extends mock<IAgentFeedbackService>() {
		...
		override getMostRecentSessionForResource() {
			return undefined;
		}
		override async revealFeedback(): Promise<void> {}
		override getNextFeedback() {
			return undefined;
		}
...
const allFiles: IFixtureFile[] = [
	...
	{
		uri: URI.file(
			"/extensions/github.copilot-chat/instructions/coding.instructions.md",
		),
		storage: PromptsStorage.extension,
		type: PromptsType.instructions,
		name: "Copilot Coding",
		description: "Built-in coding guidance",
		extensionId: "GitHub.copilot-chat",
		extensionDisplayName: "GitHub Copilot Chat",
	},
	{
		uri: URI.file("/extensions/acme.tools/instructions/team.instructions.md"),
		storage: PromptsStorage.extension,
		type: PromptsType.instructions,
		name: "Team Conventions",
		description: "Third-party extension instructions",
	...
	{
		uri: URI.file(
			"/workspace/.github/instructions/coding-standards.instructions.md",
		),
	...
	{
		uri: URI.file("/workspace/.github/instructions/testing.instructions.md"),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Testing",
	...
	{
		uri: URI.file("/workspace/.github/instructions/security.instructions.md"),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Security",
	...
	{
		uri: URI.file(
			"/workspace/.github/instructions/accessibility.instructions.md",
		),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Accessibility",
		description: "WCAG compliance guidelines",
		applyTo: "**/*.tsx",
	},
	{
		uri: URI.file("/workspace/.github/instructions/api-design.instructions.md"),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "API Design",
	...
	{
		uri: URI.file(
			"/workspace/.github/instructions/performance.instructions.md",
		),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Performance",
	...
	{
		uri: URI.file(
			"/workspace/.github/instructions/error-handling.instructions.md",
		),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Error Handling",
	...
	{
		uri: URI.file("/workspace/.github/instructions/database.instructions.md"),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Database",
		description: "Database migration and query patterns",
		applyTo: "src/db/**",
	},
	// Instructions — user
	{
		uri: URI.file("/home/dev/.copilot/instructions/my-style.instructions.md"),
	...
	{
		uri: URI.file(
			"/home/dev/.copilot/instructions/typescript-rules.instructions.md",
		),
		storage: PromptsStorage.user,
		type: PromptsType.instructions,
	...
	{
		uri: URI.file(
			"/home/dev/.copilot/instructions/commit-messages.instructions.md",
		),
		storage: PromptsStorage.user,
		type: PromptsType.instructions,
	...
	{
		uri: URI.file("/workspace/.claude/rules/code-style.md"),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Code Style",
		description: "Claude code style rules",
	},
	{
		uri: URI.file("/workspace/.claude/rules/testing.md"),
		storage: PromptsStorage.local,
		type: PromptsType.instructions,
		name: "Testing",
		description: "Claude testing conventions",
	},
	{
		uri: URI.file("/home/dev/.claude/rules/personal.md"),
		storage: PromptsStorage.user,
		type: PromptsType.instructions,
		name: "Personal",
	...
	{
		uri: URI.file("/workspace/.github/agents/documenter.agent.md"),
		storage: PromptsStorage.local,
		type: PromptsType.agent,
		name: "Documenter",
	...
	{
		uri: URI.file("/workspace/.github/agents/tester.agent.md"),
		storage: PromptsStorage.local,
		type: PromptsType.agent,
		name: "Tester",
	...
	{
		uri: URI.file("/workspace/.github/agents/refactorer.agent.md"),
		storage: PromptsStorage.local,
		type: PromptsType.agent,
		name: "Refactorer",
	...
	{
		uri: URI.file("/workspace/.github/agents/security-auditor.agent.md"),
		storage: PromptsStorage.local,
		type: PromptsType.agent,
	...
	{
		uri: URI.file("/workspace/.github/agents/api-designer.agent.md"),
		storage: PromptsStorage.local,
		type: PromptsType.agent,
		name: "API Designer",
	...
	{
		uri: URI.file("/workspace/.github/agents/performance-tuner.agent.md"),
		storage: PromptsStorage.local,
		type: PromptsType.agent,
	...
	// Agents — user
	{
		uri: URI.file("/home/dev/.copilot/agents/planner.agent.md"),
		storage: PromptsStorage.user,
		type: PromptsType.agent,
		name: "Planner",
	...
	{
		uri: URI.file("/home/dev/.copilot/agents/debugger.agent.md"),
		storage: PromptsStorage.user,
		type: PromptsType.agent,
		name: "Debugger",
	...
	{
		uri: URI.file("/home/dev/.copilot/agents/nls-helper.agent.md"),
		storage: PromptsStorage.user,
		type: PromptsType.agent,
		name: "NLS Helper",
		description: "Natural language searching code for clarity",
	},
	// Agents - extension (built-in + third-party)
	{
		uri: URI.file(
			"/extensions/github.copilot-chat/agents/workspace-guide.agent.md",
		),
		storage: PromptsStorage.extension,
		type: PromptsType.agent,
		name: "Workspace Guide",
		description: "Built-in workspace exploration agent",
		extensionId: "GitHub.copilot-chat",
		extensionDisplayName: "GitHub Copilot Chat",
	},
	{
		uri: URI.file("/extensions/acme.tools/agents/api-helper.agent.md"),
		storage: PromptsStorage.extension,
		type: PromptsType.agent,
		name: "API Helper",
		description: "Third-party API agent",
		extensionId: "acme.tools",
		extensionDisplayName: "Acme Tools",
	},
	// Skills — workspace
	{
		uri: URI.file("/workspace/.github/skills/deploy/SKILL.md"),
		storage: PromptsStorage.local,
	...
	{
		uri: URI.file("/workspace/.github/skills/refactor/SKILL.md"),
		storage: PromptsStorage.local,
		type: PromptsType.skill,
		name: "Refactor",
	...
	{
		uri: URI.file("/workspace/.github/skills/unit-tests/SKILL.md"),
		storage: PromptsStorage.local,
		type: PromptsType.skill,
		name: "Unit Tests",
		description: "Test generation and runner integration",
	},
	{
		uri: URI.file("/workspace/.github/skills/ci-fix/SKILL.md"),
		storage: PromptsStorage.local,
		type: PromptsType.skill,
		name: "CI Fix",
	...
	{
		uri: URI.file("/workspace/.github/skills/migration/SKILL.md"),
		storage: PromptsStorage.local,
		type: PromptsType.skill,
		name: "Migration",
	...
	{
		uri: URI.file("/workspace/.github/skills/accessibility/SKILL.md"),
		storage: PromptsStorage.local,
		type: PromptsType.skill,
		name: "Accessibility",
	...
	{
		uri: URI.file("/workspace/.github/skills/docker/SKILL.md"),
		storage: PromptsStorage.local,
		type: PromptsType.skill,
		name: "Docker",
	...
	{
		uri: URI.file("/workspace/.github/skills/api-docs/SKILL.md"),
		storage: PromptsStorage.local,
		type: PromptsType.skill,
		name: "API Docs",
	...
	// Skills — user
	{
		uri: URI.file("/home/dev/.copilot/skills/git-workflow/SKILL.md"),
		storage: PromptsStorage.user,
		type: PromptsType.skill,
		name: "Git Workflow",
		description: "Branch and PR workflows",
	},
	{
		uri: URI.file("/home/dev/.copilot/skills/code-review/SKILL.md"),
		storage: PromptsStorage.user,
		type: PromptsType.skill,
		name: "Code Review",
	...
	{
		uri: URI.file("/extensions/github.copilot-chat/skills/workspace/SKILL.md"),
		storage: PromptsStorage.extension,
		type: PromptsType.skill,
		name: "Workspace Search",
		description: "Built-in workspace search skill",
		extensionId: "GitHub.copilot-chat",
		extensionDisplayName: "GitHub Copilot Chat",
	},
	{
		uri: URI.file("/extensions/acme.tools/skills/audit/SKILL.md"),
		storage: PromptsStorage.extension,
		type: PromptsType.skill,
		name: "Audit",
		description: "Third-party audit skill",
		extensionId: "acme.tools",
		extensionDisplayName: "Acme Tools",
	},
	// Skills - built-in (sessions bundled skills with UI integrations)
	{
		uri: URI.file("/app/skills/act-on-feedback/SKILL.md"),
	...
	{
		uri: URI.file("/app/skills/generate-run-commands/SKILL.md"),
		storage: BUILTIN_STORAGE as PromptsStorage,
		type: PromptsType.skill,
	...
	{
		uri: URI.file("/app/skills/commit/SKILL.md"),
		storage: BUILTIN_STORAGE as PromptsStorage,
		type: PromptsType.skill,
		name: "commit",
		description:
	...
	{
		uri: URI.file("/app/skills/create-pr/SKILL.md"),
		storage: BUILTIN_STORAGE as PromptsStorage,
		type: PromptsType.skill,
		name: "create-pr",
	...
	{
		uri: URI.file("/workspace/.github/prompts/fix-bug.prompt.md"),
		storage: PromptsStorage.local,
		type: PromptsType.prompt,
		name: "Fix Bug",
		description: "Diagnose and fix a bug from issue",
	},
	{
		uri: URI.file("/workspace/.github/prompts/write-tests.prompt.md"),
		storage: PromptsStorage.local,
		type: PromptsType.prompt,
		name: "Write Tests",
		description: "Generate unit tests for selection",
	},
	{
		uri: URI.file("/workspace/.github/prompts/add-docs.prompt.md"),
		storage: PromptsStorage.local,
		type: PromptsType.prompt,
		name: "Add Docs",
	...
	{
		uri: URI.file("/workspace/.github/prompts/optimize.prompt.md"),
		storage: PromptsStorage.local,
		type: PromptsType.prompt,
		name: "Optimize",
	...
	{
		uri: URI.file("/workspace/.github/prompts/convert-to-ts.prompt.md"),
		storage: PromptsStorage.local,
		type: PromptsType.prompt,
		name: "Convert to TS",
	...
	{
		uri: URI.file("/workspace/.github/prompts/summarize-pr.prompt.md"),
		storage: PromptsStorage.local,
		type: PromptsType.prompt,
		name: "Summarize PR",
	...
	// Prompts — user
	{
		uri: URI.file("/home/dev/.copilot/prompts/translate.prompt.md"),
		storage: PromptsStorage.user,
		type: PromptsType.prompt,
		name: "Translate",
		description: "Translate strings for i18n",
	},
	{
		uri: URI.file("/home/dev/.copilot/prompts/commit-msg.prompt.md"),
		storage: PromptsStorage.user,
		type: PromptsType.prompt,
		name: "Commit Message",
	...
	{
		uri: URI.file("/extensions/github.copilot-chat/prompts/trace.prompt.md"),
		storage: PromptsStorage.extension,
		type: PromptsType.prompt,
		name: "Trace",
		description: "Built-in tracing prompt",
		extensionId: "GitHub.copilot-chat",
		extensionDisplayName: "GitHub Copilot Chat",
	},
	{
		uri: URI.file("/extensions/acme.tools/prompts/lint.prompt.md"),
		storage: PromptsStorage.extension,
		type: PromptsType.prompt,
		name: "Lint",
		description: "Third-party lint prompt",
		extensionId: "acme.tools",
		extensionDisplayName: "Acme Tools",
	},
	// Hooks — workspace
	{
		uri: URI.file("/workspace/.github/hooks/pre-commit.json"),
		storage: PromptsStorage.local,
	...
	{
		uri: URI.file("/workspace/.github/hooks/post-save.json"),
		storage: PromptsStorage.local,
		type: PromptsType.hook,
	...
	{
		uri: URI.file("/workspace/.github/hooks/on-test-fail.json"),
		storage: PromptsStorage.local,
		type: PromptsType.hook,
		name: "On Test Failure",
		description: "Suggest fix when tests fail",
	},
	{
		uri: URI.file("/workspace/.github/hooks/pre-push.json"),
		storage: PromptsStorage.local,
		type: PromptsType.hook,
		name: "Pre-Push Check",
		description: "Run type-check before push",
	},
	{
		uri: URI.file("/workspace/.github/hooks/post-create.json"),
		storage: PromptsStorage.local,
		type: PromptsType.hook,
		name: "Post-Create",
		description: "Initialize boilerplate for new files",
	},
	{
		uri: URI.file("/workspace/.github/hooks/on-error.json"),
		storage: PromptsStorage.local,
		type: PromptsType.hook,
		name: "On Error",
		description: "Log and report unhandled errors",
	},
	{
		uri: URI.file("/workspace/.github/hooks/post-tool-call.json"),
		storage: PromptsStorage.local,
		type: PromptsType.hook,
		name: "Post Tool Call",
		description: "Echo confirmation after each tool call",
	},
	{
		uri: URI.file("/workspace/.github/hooks/on-build-fail.json"),
		storage: PromptsStorage.local,
		type: PromptsType.hook,
		name: "On Build Failure",
		description: "Auto-diagnose build errors",
	},
	// Hooks — user
	{
		uri: URI.file("/home/dev/.copilot/hooks/daily-summary.json"),
		storage: PromptsStorage.user,
		type: PromptsType.hook,
		name: "Daily Summary",
	...
	{
		uri: URI.file("/home/dev/.copilot/hooks/backup-changes.json"),
		storage: PromptsStorage.user,
		type: PromptsType.hook,
		name: "Backup Changes",
		description: "Auto-stash uncommitted changes",
	},
];

const agentInstructions: IAgentInstructionFile[] = [
	{
		uri: URI.file("/workspace/AGENTS.md"),
		realPath: undefined,
		type: AgentInstructionFileType.agentsMd,
	},
	{
		uri: URI.file("/workspace/CLAUDE.md"),
		realPath: undefined,
...
const mcpRuntimeServers = [
	{
		definition: { id: "github-copilot-mcp", label: "GitHub Copilot" },
		collection: {
			id: "ext.github.copilot/mcp",
			label: "ext.github.copilot/mcp",
		},
		enablement: constObservable(2),
		connectionState: constObservable({ state: 2 }),
	},
];
	...
	while (index < lines.length) {
		...
		if (line.startsWith("- ")) {
			...
			while (
				...
				DOM.append(list, DOM.$("li")).textContent = lines[index]
					.trimStart()
					.slice(2);
				index++;
			}
...
async function renderEditor(
	...
	// service is created.
	const modelServiceRef: { value: IModelService | undefined } = {
		value: undefined,
	};
	const languageServiceRef: { value: ILanguageService | undefined } = {
		value: undefined,
	};
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				...
				new (class extends mock<IAgentSessionsService>() {
					override readonly model = new (class extends mock<
						IAgentSessionsService["model"]
					>() {
			...
			reg.defineInstance(
				IPromptsService,
				createMockPromptsService(allFiles, agentInstructions, fileContents),
			);
			reg.defineInstance(
				IAICustomizationWorkspaceService,
				new (class extends mock<IAICustomizationWorkspaceService>() {
					override readonly isSessionsWindow = isSessionsWindow;
					override readonly welcomePageFeatures = {
						showGettingStartedBanner: true,
					};
					override readonly activeProjectRoot = observableValue(
						"root",
						URI.file("/workspace"),
					);
					override readonly hasOverrideProjectRoot = observableValue(
						"hasOverride",
						false,
					);
					override getActiveProjectRoot() {
						return URI.file("/workspace");
					}
					override getStorageSourceFilter(type: PromptsType) {
						return harnessService.getStorageSourceFilter(type);
					}
					...
					override getSkillUIIntegrations() {
			...
			reg.defineInstance(
				...
				new (class extends mock<IChatSessionsService>() {
					...
					override async getCustomizations() {
						return undefined;
					}
					override getRegisteredChatSessionItemProviders() {
						return [];
					}
					override hasCustomizationsProvider() {
						return false;
					}
				})(),
			);
			...
			reg.defineInstance(
				...
				new (class extends mock<IPathService>() {
					...
					override userHome(): URI | Promise<URI> {
						return userHome;
					}
				})(),
			);
			...
			reg.defineInstance(
				...
				new (class extends mock<IViewsService>() {
					override async openView<T extends {}>(_id: string, _focus?: boolean) {
			...
			reg.defineInstance(
				...
				new (class extends mock<IMcpService>() {
					override readonly servers = constObservable(
						mcpRuntimeServers as never[],
					);
				})(),
			);
			reg.defineInstance(
				IMcpRegistry,
				new (class extends mock<IMcpRegistry>() {
			...
			reg.defineInstance(
				...
				new (class extends mock<IProductService>() {
					override readonly defaultChatAgent = new (class extends mock<
						NonNullable<IProductService["defaultChatAgent"]>
					>() {
	...
	for (const [uri, content] of fileContents) {
		if (!modelServiceRef.value.getModel(uri)) {
			const model = modelServiceRef.value.createModel(
				content,
				null,
				uri,
				false,
	...
	const editorInput = ctx.disposableStore.add(
		AICustomizationManagementEditorInput.getOrCreate(),
	);
	...
	if (options.openFirstItem) {
		const visibleContent = [
			...ctx.container.querySelectorAll(
				".prompts-content-container, .mcp-content-container, .plugin-content-container",
			),
		].find(
		...
		const rowToOpen = openItemLabel
			? [...(visibleContent?.querySelectorAll(".monaco-list-row") ?? [])].find(
					(row): row is HTMLElement =>
						row instanceof HTMLElement &&
						row.textContent?.includes(openItemLabel),
				)
			: (visibleContent?.querySelector(
					".monaco-list-row.ai-customization-list-item, .monaco-list-row.mcp-server-item",
				) as HTMLElement | undefined);
		if (rowToOpen) {
			rowToOpen.dispatchEvent(
			...
			if (options.editorDisplayMode === "raw") {
				const modeButton = ctx.container.querySelector(
					".editor-mode-button",
...
function makeGalleryServer(
	...
	const galleryStub = new (class extends mock<
		NonNullable<IWorkbenchMcpServer["gallery"]>
	>() {})();
	return new (class extends mock<IWorkbenchMcpServer>() {
		override readonly id = id;
		override readonly name = id;
...
const galleryServers = [
	makeGalleryServer(
		"gallery-postgres",
		"PostgreSQL",
		"Access PostgreSQL databases with schema inspection and query tools",
		"Microsoft",
	),
	makeGalleryServer(
		"gallery-github",
	...
	makeGalleryServer(
		"gallery-slack",
		"Slack",
		"Send messages, manage channels, and search workspace history",
		"Slack Technologies",
	),
	makeGalleryServer(
		"gallery-docker",
		"Docker",
		"Container lifecycle management and image operations",
		"Docker Inc",
	),
	makeGalleryServer(
		"gallery-filesystem",
	...
	makeGalleryServer(
		"gallery-puppeteer",
		"Puppeteer",
		"Browser automation with screenshots, navigation, and form filling",
		"Google",
	),
	makeGalleryServer(
		"gallery-memory",
	...
	makeGalleryServer(
		"gallery-sentry",
		"Sentry",
		"Error monitoring, issue tracking, and performance tracing",
		"Sentry",
	),
	makeGalleryServer(
		"gallery-sqlite",
		"SQLite",
		"Query and manage SQLite databases with schema exploration",
		"Community",
	),
	makeGalleryServer(
		"gallery-redis",
		"Redis",
		"In-memory data store operations and key management",
		"Redis Ltd",
	),
];

async function renderMcpBrowseMode(
	ctx: ComponentFixtureContext,
): Promise<void> {
	const width = 650;
	const height = 500;
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				...
				new (class extends mock<IMcpWorkbenchService>() {
					...
					override async queryGallery(): Promise<
						IIterativePager<IWorkbenchMcpServer>
					> {
						return {
			...
			reg.defineInstance(
				...
				new (class extends mock<ICustomizationHarnessService>() {
					override readonly activeSessionResource = observableValue<URI>(
						"activeSessionResource",
						LocalChatSessionUri.getNewSessionUri(),
					);
					override readonly activeHarness = derived((reader) =>
						getChatSessionType(this.activeSessionResource.read(reader)),
					);
					override getActiveDescriptor() {
	...
	const browseButton = widget.element.querySelector(
		".list-add-button",
	) as HTMLElement;
	browseButton?.click();
...
function makeInstalledPlugin(
	...
	return new (class extends mock<IAgentPlugin>() {
		...
		override readonly enablement = constObservable(
			enabled
...
const installedPlugins: IAgentPlugin[] = [
	makeInstalledPlugin(
		"Linear",
		URI.file("/workspace/.copilot/plugins/linear"),
		true,
	),
	makeInstalledPlugin(
		"Sentry",
		URI.file("/workspace/.copilot/plugins/sentry"),
		true,
	),
	makeInstalledPlugin(
		"Datadog",
		URI.file("/workspace/.copilot/plugins/datadog"),
		true,
	),
	makeInstalledPlugin(
		"Notion",
		URI.file("/workspace/.copilot/plugins/notion"),
		true,
	),
	makeInstalledPlugin(
		"Confluence",
		URI.file("/workspace/.copilot/plugins/confluence"),
		true,
	),
	makeInstalledPlugin(
		"PagerDuty",
		URI.file("/workspace/.copilot/plugins/pagerduty"),
		false,
	),
	makeInstalledPlugin(
		"LaunchDarkly",
		URI.file("/workspace/.copilot/plugins/launchdarkly"),
		true,
	),
	makeInstalledPlugin(
		"CircleCI",
		URI.file("/workspace/.copilot/plugins/circleci"),
		true,
	),
	makeInstalledPlugin(
		"Vercel",
		URI.file("/workspace/.copilot/plugins/vercel"),
		false,
	),
	makeInstalledPlugin(
		"Supabase",
		URI.file("/workspace/.copilot/plugins/supabase"),
		true,
	),
];
...
function makeMarketplacePlugin(
	...
	return {
		...
		marketplaceReference: {
			rawValue: `example/${repo}`,
			displayLabel: repo,
			cloneUrl: `https://github.com/example/${repo}.git`,
			canonicalId: `github:example/${repo}`,
			cacheSegments: ["example", repo],
...
const marketplacePlugins: IMarketplacePlugin[] = [
	...
	makeMarketplacePlugin(
		"Figma",
		"Design system inspection and asset export",
		"figma-plugin",
	),
	makeMarketplacePlugin(
		"Stripe",
	...
	makeMarketplacePlugin(
		"Algolia",
		"Search and discovery API integration",
		"algolia-plugin",
	),
	makeMarketplacePlugin(
		"LaunchDarkly",
	...
	makeMarketplacePlugin(
		"PlanetScale",
		"Serverless MySQL database management",
		"planetscale-plugin",
	),
	makeMarketplacePlugin(
		"Vercel",
...
async function renderPluginBrowseMode(
	ctx: ComponentFixtureContext,
): Promise<void> {
	const width = 650;
	const height = 500;
	...
	const browseInstalledPlugins = [
		makeInstalledPlugin(
			"Linear",
			URI.file("/home/dev/.vscode/agent-plugins/example/linear-plugin"),
			true,
		),
		makeInstalledPlugin(
			"Sentry",
			URI.file("/home/dev/.vscode/agent-plugins/example/sentry-plugin"),
			true,
		),
		makeInstalledPlugin(
			"Datadog",
			URI.file("/home/dev/.vscode/agent-plugins/example/datadog-plugin"),
			false,
		),
	];
	...
	const instantiationService = createEditorServices(ctx.disposableStore, {
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				...
				new (class extends mock<IAgentPluginService>() {
					override readonly plugins = constObservable(
						browseInstalledPlugins as readonly IAgentPlugin[],
					);
			...
			reg.defineInstance(
				...
				new (class extends mock<IPluginInstallService>() {
					override getPluginInstallUri(plugin: IMarketplacePlugin) {
						const repo =
							plugin.sourceDescriptor.kind === PluginSourceKind.GitHub
								? plugin.sourceDescriptor.repo
								: undefined;
						return repo
							? (pluginInstallUris.get(repo) ?? URI.file("/dev/null"))
							: URI.file("/dev/null");
					}
				})(),
			);
			reg.defineInstance(
				IAICustomizationItemsModel,
				createMockAICustomizationItemsModel(),
			);
		},
	});
	...
	for (const scrollbar of widget.element.querySelectorAll<HTMLElement>(
		".scrollbar",
	)) {
...
function renderEmbeddedMcpDetail(
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				...
				new (class extends mock<IMcpWorkbenchService>() {
					override readonly onChange = Event.None;
					override readonly onReset = Event.None;
					override readonly local: IWorkbenchMcpServer[] = server
						? [server]
						: [];
					override async open() {
						/* no-op in fixture */
	...
	const host = DOM.append(
		ctx.container,
		DOM.$(".ai-customization-management-editor"),
	);
	host.style.height = "100%";
	host.style.width = "100%";
	host.style.overflow = "auto";

	const detail = ctx.disposableStore.add(
		instantiationService.createInstance(EmbeddedMcpServerDetail, host),
	);
	if (server) {
...
function renderEmbeddedPluginDetail(
	ctx: ComponentFixtureContext,
	item: IAgentPluginItem | undefined,
): void {
	const width = 480;
	const height = 320;
	ctx.container.style.width = `${width}px`;
	...
	const detail = ctx.disposableStore.add(
		instantiationService.createInstance(EmbeddedAgentPluginDetail, host),
	);
	if (item) {
...
function makeInstalledPluginItem(
	name: string,
	description: string,
): IAgentPluginItem {
	return {
		...
		plugin: makeInstalledPlugin(
			name,
			URI.file(`/workspace/.copilot/plugins/${name.toLowerCase()}`),
			true,
		),
	};
}

function makeMarketplacePluginItem(
	name: string,
	description: string,
): IAgentPluginItem {
	return {
...
export default defineThemedFixtureGroup(
	{ path: "chat/aiCustomizations/" },
	{
		...
		EmbeddedMcpDetailWorkspace: defineComponentFixture({
			labels: { kind: "screenshot" },
			render: (ctx) =>
				renderEmbeddedMcpDetail(
					ctx,
					makeLocalMcpServer(
						"mcp-postgres",
						"PostgreSQL",
						LocalMcpServerScope.Workspace,
						"Database access for the active workspace",
					),
				),
		}),
			...
			render: (ctx) =>
				renderEmbeddedMcpDetail(
					...
					makeLocalMcpServer(
						...
						"Web Search",
						LocalMcpServerScope.User,
						"Search the web from any session",
					),
				),
		}),
		...
		EmbeddedPluginDetailInstalled: defineComponentFixture({
			labels: { kind: "screenshot" },
			render: (ctx) =>
				renderEmbeddedPluginDetail(
					ctx,
					makeInstalledPluginItem(
						"Linear",
						"Issue tracking and project management integration",
					),
				),
