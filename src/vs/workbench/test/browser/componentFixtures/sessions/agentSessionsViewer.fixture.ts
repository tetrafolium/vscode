...
} from "../../../../contrib/chat/browser/agentSessions/agentSessionApprovalModel.js";
...
function createMockSession(
	...
	return new (class extends mock<IAgentSession>() {
		override readonly resource =
			overrides.resource ??
			URI.parse(
				`vscode-chat-session://${overrides.providerType}/session-${Math.random().toString(36).slice(2)}`,
			);
		...
		override readonly providerLabel =
			overrides.providerLabel ?? overrides.providerType;
...
function createMockApprovalModel(
	sessionResource: URI,
	...
	return new (class extends mock<AgentSessionApprovalModel>() {
		override getApproval(resource: URI) {
			...
			return observableValue<IAgentSessionApprovalInfo | undefined>(
				"mockApproval.empty",
				undefined,
			);
		}
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				...
				new (class extends mock<IChatSessionsService>() {
					...
					override async resolveChatSessionItem() {
						return undefined;
					}
				})(),
			);
...
export default defineThemedFixtureGroup({
		...
		render: (ctx) => {
			...
			renderSessionItem(
				...
				createMockSession({
					...
					changes: [
						{
							modifiedUri: URI.file("/src/api/routes.ts"),
							insertions: 25,
							deletions: 10,
						},
						{
							modifiedUri: URI.file("/src/api/handlers.ts"),
							insertions: 50,
							deletions: 30,
						},
						{
							modifiedUri: URI.file("/tests/api.test.ts"),
							insertions: 40,
							deletions: 5,
						},
					],
	...
	ApprovalRowPowerShell: defineComponentFixture({
		render: (ctx) => {
			...
			const resource = URI.parse(
				"vscode-chat-session://local/approval-powershell",
			);
			const approvalModel = createMockApprovalModel(resource, {
				label:
					"Start-Job -ScriptBlock { Set-Location 'c:\\some\\path'; npm install } | Out-Null",
				languageId: "pwsh",
				since: new Date(),
				confirm: () => {},
			});
	...
	ApprovalRowLongLabel: defineComponentFixture({
		render: (ctx) => {
			...
			const approvalModel = createMockApprovalModel(resource, {
				label:
					"rm -rf node_modules && npm cache clean --force && npm install --legacy-peer-deps --ignore-scripts",
				languageId: "sh",
				since: new Date(),
				confirm: () => {},
			});
