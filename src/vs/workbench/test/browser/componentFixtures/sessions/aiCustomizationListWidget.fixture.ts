...
function createMockPromptsService(
	instructionFiles: IFixtureInstructionFile[],
	...
	return new (class extends mock<IPromptsService>() {
		...
		override async findAgentSkills() {
			return [];
		}
		override async getPromptSlashCommands() {
			return [];
		}
		override async getHooks() {
			return undefined;
		}
...
function createMockWorkspaceService(): IAICustomizationWorkspaceService {
	const activeProjectRoot = observableValue<URI | undefined>(
		"mockActiveProjectRoot",
		URI.file("/workspace"),
	);
...
function createMockHarnessService(): ICustomizationHarnessService {
	...
	return new (class extends mock<ICustomizationHarnessService>() {
		...
		override readonly availableHarnesses = observableValue<
			readonly IHarnessDescriptor[]
		>("harnesses", [descriptor]);
		override findHarnessById(id: string) {
			return id === descriptor.id ? descriptor : undefined;
		}
		override getStorageSourceFilter() {
			return defaultFilter;
		}
		...
		override getSessionResourceForHarness() {
...
async function renderInstructionsTab(
	...
	const contextViewService = new (class extends mock<IContextViewService>() {
		...
		override getContextViewElement(): HTMLElement {
	...
	const instantiationService = createEditorServices(ctx.disposableStore, {
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				IPromptsService,
				createMockPromptsService(instructionFiles, agentInstructionFiles),
			);
			reg.defineInstance(
				IAICustomizationWorkspaceService,
			...
			reg.defineInstance(
				ICustomizationHarnessService,
				createMockHarnessService(),
			);
			reg.defineInstance(
				IWorkspaceContextService,
				createMockWorkspaceContextService(),
			);
			reg.defineInstance(
				...
				renderInstructionsTab(
					...
					[
						...
						},
						{
							promptPath: {
								uri: URI.file(
									"/home/dev/.copilot/instructions/my-style.instructions.md",
								),
						...
						// Always-included instruction (applyTo: **)
						{
							promptPath: {
								uri: URI.file(
									"/workspace/.github/instructions/general-guidelines.instructions.md",
								),
								storage: PromptsStorage.local,
								type: PromptsType.instructions,
							},
						...
						// On-demand instructions (with applyTo pattern)
						{
							promptPath: {
								uri: URI.file(
									"/workspace/.github/instructions/testing-guidelines.instructions.md",
								),
								storage: PromptsStorage.local,
								type: PromptsType.instructions,
							},
						...
						},
						{
							promptPath: {
								uri: URI.file(
									"/workspace/.github/instructions/security-review.instructions.md",
								),
								storage: PromptsStorage.local,
								type: PromptsType.instructions,
							},
