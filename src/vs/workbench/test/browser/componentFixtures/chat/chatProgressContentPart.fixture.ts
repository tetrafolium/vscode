...
function createMockContext(opts?: {
	...
	return {
		...
		content: opts?.hasFollowingContent
			? [{ kind: "progressMessage", content: new MarkdownString("test") }]
			: [],
		contentIndex: 0,
...
function renderProgressPart(
	...
	const markdownRenderer = instantiationService.createInstance(
			...
			render: (ctx) =>
				renderProgressPart(
					...
					createProgressMessage(
						"Reading `src/vs/workbench/contrib/chat/browser/chatWidget.ts`",
					),
					createMockContext({ isComplete: false }),
					{ forceShowSpinner: true, forceShowMessage: true, shimmer: false },
				),
		...
		LongMessage: defineComponentFixture({
			...
			render: (ctx) =>
				renderProgressPart(
					ctx,
					createProgressMessage(
						"Searching across multiple workspace folders for TypeScript files matching the pattern you described, including test files and configuration",
					),
					createMockContext({ isComplete: false }),
					{ forceShowSpinner: true, forceShowMessage: true, shimmer: false },
				),
