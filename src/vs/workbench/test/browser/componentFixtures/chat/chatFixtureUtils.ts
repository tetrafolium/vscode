...
} from "../../../../contrib/chat/common/tools/chatTodoListService.js";
 ...
 */
export function registerChatFixtureServices(
	reg: ServiceRegistration,
	options: IChatFixtureServicesOptions = {},
): void {
	...
	reg.defineInstance(
		...
		new (class extends mock<IDecorationsService>() {
			override onDidChangeDecorations = Event.None;
		})(),
	);
	reg.defineInstance(
		ITextFileService,
	...
	reg.defineInstance(
		...
		new (class extends mock<IUpdateService>() {
			override onStateChange = Event.None;
			override get state() {
	...
	reg.define(
		IChatAgentService,
		class FixtureChatAgentService extends ChatAgentService {
	...
	reg.defineInstance(
		IChatAgentNameService,
		new (class extends mock<IChatAgentNameService>() {
			override getAgentNameRestriction() {
				return true;
			}
		})(),
	);
	...
	reg.defineInstance(
		...
		new (class extends mock<IChatMarkdownAnchorService>() {
			override register() {
				return { dispose() {} };
			}
		})(),
	...
	const artifactGroups =
		options.artifactGroups ??
		observableValue<readonly IArtifactSourceGroup[]>("artifactGroups", []);
	reg.defineInstance(
		IChatArtifactsService,
