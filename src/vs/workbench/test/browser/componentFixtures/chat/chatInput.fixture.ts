...
} from "../../../../contrib/chat/common/editing/chatEditingService.js";
...
interface ChatInputFixtureOptions {
	readonly artifacts?: readonly {
		label: string;
		uri: string;
		type: "devServer" | "screenshot" | "plan" | undefined;
	}[];
...
async function renderChatInput(
	...
	fixtureOptions: ChatInputFixtureOptions = {},
): Promise<void> {
	const { container, disposableStore } = context;
	const { artifacts = [], editingSession, todos = [] } = fixtureOptions;
	const artifactGroups: IArtifactSourceGroup[] =
		artifacts.length > 0
			? [{ source: { kind: "agent" as const }, artifacts }]
			: [];
	const artifactsObs = observableValue<readonly IArtifactSourceGroup[]>(
		"artifactGroups",
		artifactGroups,
	);
	...
	await inputPart.renderChatTodoListWidget(
		URI.parse("chat-session:test-session"),
	);
...
function createMockEditingSession(
	files: { uri: string; added: number; removed: number }[],
): IChatEditingSession {
	...
	return new (class extends mock<IChatEditingSession>() {
		...
		override readonly chatSessionResource = URI.parse(
		...
		override readonly requestDisablement = observableValue<
...
export default defineThemedFixtureGroup(
	{ path: "chat/input/" },
	{
		...
		WithArtifacts: defineComponentFixture({
			render: (context) =>
				renderChatInput(context, { artifacts: sampleArtifacts }),
		}),
		WithFileChanges: defineComponentFixture({
			render: (context) =>
				renderChatInput(context, {
					editingSession: createMockEditingSession([
						{ uri: "file:///workspace/src/fibon.ts", added: 21, removed: 1 },
					]),
				}),
		}),
