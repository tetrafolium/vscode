...
function renderBadge(
	...
	const widget = disposableStore.add(
		instantiationService.createInstance(ToolRiskBadgeWidget),
	);
...
function renderBadgeWithHoverPreview(
	...
	const rendered = disposableStore.add(
		renderMarkdown(widget.getDetailsMarkdown(), {
			asyncRenderCallback: () => {},
		}),
	);
...
const promptInjectionDisclaimer = (() => {
	...
	md.appendMarkdown(
		'Web content may contain malicious code or attempt prompt injection attacks. Auto approval denied by rule [`curl (default)`](https://example.com/settings "View rule in settings").',
	);
	return md;
})();
...
function makeTerminalMessage(
	scenario: IRiskScenario | undefined,
): IFixtureMessage[] {
	return [
		{
			user: "",
...
function makeElicitationMessage(
	scenario: IRiskScenario | undefined,
): IFixtureMessage[] {
	return [
		{
...
const orangeScenario: IRiskScenario = {
	...
	assessment: {
		...
		explanation:
			"Modifies tracked files in the working tree. Reversible via Git.",
	},
};
...
const redScenario: IRiskScenario = {
	...
	assessment: {
		...
		explanation:
			"Force-pushes to a remote branch. This rewrites history and cannot be undone.",
	},
};
	...
	{ path: "chat/" },
	{
		...
		Green: defineComponentFixture({
			...
			render: (ctx) =>
				renderBadge(ctx, {
					kind: "assessment",
					assessment: greenScenario.assessment,
				}),
		}),
		...
		Orange: defineComponentFixture({
			...
			render: (ctx) =>
				renderBadge(ctx, {
					kind: "assessment",
					assessment: orangeScenario.assessment,
				}),
		}),
		...
		Red: defineComponentFixture({
			...
			render: (ctx) =>
				renderBadge(ctx, {
					kind: "assessment",
					assessment: redScenario.assessment,
				}),
		}),
		...
		GreenInContext: defineComponentFixture({
			labels: { kind: "screenshot" },
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeTerminalMessage(greenScenario),
					...inContextOptions,
				}),
		}),
		...
		OrangeInContext: defineComponentFixture({
			...
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeTerminalMessage(orangeScenario),
					...inContextOptions,
				}),
		}),
		...
		RedInContext: defineComponentFixture({
			...
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeTerminalMessage(redScenario),
					...inContextOptions,
				}),
		}),
		...
		LoadingInContext: defineComponentFixture({
			...
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeTerminalMessage(undefined),
					...inContextOptions,
				}),
		}),
		...
		GreenElicitationInContext: defineComponentFixture({
			...
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeElicitationMessage(greenScenario),
					...inContextOptions,
				}),
		}),
		...
		OrangeElicitationInContext: defineComponentFixture({
			...
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeElicitationMessage(orangeScenario),
					...inContextOptions,
				}),
		}),
		...
		RedElicitationInContext: defineComponentFixture({
			...
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeElicitationMessage(redScenario),
					...inContextOptions,
				}),
		}),
		...
		LoadingElicitationInContext: defineComponentFixture({
			...
			render: (ctx) =>
				renderChatWidget(ctx, {
					messages: makeElicitationMessage(undefined),
					...inContextOptions,
				}),
		}),
							...
							assistant: [
								{
									...
									command: "git push --force origin main",
									disclaimer:
										"$(info) Web content may contain malicious code or attempt prompt injection attacks. Auto approval denied by rule curl (default)",
								},
							],
							responseComplete: false,
						},
							...
							assistant: [
								{
									...
									requestUnsandboxedExecutionReason:
										"Requires elevated permissions to delete files owned by root.",
								},
							],
							responseComplete: false,
						},
							...
							assistant: [
								{
									...
									requestUnsandboxedExecution: true,
									requestUnsandboxedExecutionReason:
										"Requires elevated permissions to install system packages.",
									disclaimer:
										"$(info) Web content may contain malicious code or attempt prompt injection attacks. Auto approval denied by rule curl (default)",
								},
							],
		...
		HoverPreviewNoDetails: defineComponentFixture({
			labels: { kind: "screenshot" },
			render: (ctx) =>
				renderBadgeWithHoverPreview(ctx, redScenario.assessment, undefined),
		}),

		HoverPreviewDisclaimer: defineComponentFixture({
			labels: { kind: "screenshot" },
			render: (ctx) =>
				renderBadgeWithHoverPreview(
					ctx,
					redScenario.assessment,
					promptInjectionDisclaimer,
				),
		}),
