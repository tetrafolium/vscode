...
function createMockArtifactsFromGroups(
	groups: IArtifactSourceGroup[],
): IChatArtifacts {
	...
	return new (class extends mock<IChatArtifacts>() {
		...
		override setAgentArtifacts(a: IChatArtifact[]) {
			artifactGroups.set(
				a.length > 0 ? [{ source: { kind: "agent" }, artifacts: a }] : [],
				undefined,
			);
		}
		override clearAgentArtifacts() {
			artifactGroups.set([], undefined);
		}
...
function createMockArtifactsService(
...
function createMockArtifactsServiceFromGroups(
	groups: IArtifactSourceGroup[],
): IChatArtifactsService {
...
function renderArtifactsWidget(
	...
	const instantiationService = createEditorServices(disposableStore, {
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				IChatArtifactsService,
				createMockArtifactsService(artifacts),
			);
			reg.defineInstance(
...
function renderArtifactsWidgetFromGroups(
	context: ComponentFixtureContext,
	groups: IArtifactSourceGroup[],
): void {
	const { container, disposableStore } = context;
	...
	const instantiationService = createEditorServices(disposableStore, {
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				IChatArtifactsService,
				createMockArtifactsServiceFromGroups(groups),
			);
			reg.defineInstance(
				IChatImageCarouselService,
				new (class extends mock<IChatImageCarouselService>() {})(),
...
const multiSourceGroups: IArtifactSourceGroup[] = [
	...
	},
	{
		...
		artifacts: [
			{
				label: "Specification (v2 - reviewed)",
	...
	{ path: "chat/artifacts/" },
	{
		...
		MultipleArtifactsCollapsed: defineComponentFixture({
			render: (context) => {
				...
				const expandButton = context.container.querySelector<HTMLElement>(
					".chat-artifacts-expand .monaco-button",
				);
				expandButton?.click();
			},
		}),
		...
		MultiSourceHoveredRow: defineComponentFixture({
			render: (context) => {
				...
				const rows = context.container.querySelectorAll<HTMLElement>(
					".chat-artifacts-list-row",
				);
