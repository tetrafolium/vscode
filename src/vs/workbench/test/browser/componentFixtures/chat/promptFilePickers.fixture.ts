...
class FixtureQuickInputService extends QuickInputService {
	private readonly _activePicks = new Set<
		IQuickPick<IQuickPickItem, { useSeparators: boolean }>
	>();

	override createQuickPick<T extends IQuickPickItem>(options: {
		useSeparators: true;
	}): IQuickPick<T, { useSeparators: true }>;
	override createQuickPick<T extends IQuickPickItem>(options?: {
		useSeparators: boolean;
	}): IQuickPick<T, { useSeparators: false }>;
	override createQuickPick<T extends IQuickPickItem>(
		options: { useSeparators: boolean } = { useSeparators: false },
	): IQuickPick<T, { useSeparators: boolean }> {
		const quickPick = super.createQuickPick<T>(options) as IQuickPick<
			T,
			{ useSeparators: boolean }
		>;
				...
				renderPromptFilePickerFixture({
					...
					seedData: (promptsService) => {
						promptsService.localPromptFiles = [
							...
							{
								uri: URI.file("/workspace/.github/prompts/docs.prompt.md"),
								storage: PromptsStorage.local,
								type: PromptsType.prompt,
								name: "Docs Prompt",
								description: "Generate docs for symbols",
							},
						];
						promptsService.userPromptFiles = [
							{
								uri: URI.file("/home/dev/.copilot/prompts/review.prompt.md"),
				...
				renderPromptFilePickerFixture({
					...
					seedData: (promptsService) => {
						promptsService.localPromptFiles = [
							{
								uri: URI.file(
									"/workspace/.github/instructions/repo.instructions.md",
								),
								storage: PromptsStorage.local,
								type: PromptsType.instructions,
								name: "Repo Rules",
								description: "Repository-wide coding rules",
							},
						];
...
async function renderPromptFilePickerFixture({
	container,
	disposableStore,
	theme,
	type,
	placeholder,
	seedData,
}: RenderPromptPickerOptions): Promise<void> {
	...
	const promptsService = new (class extends mock<IPromptsService>() {
		override async listPromptFilesForStorage(
			...
			switch (storage) {
				case PromptsStorage.local:
					return promptsState.localPromptFiles.filter(
						(file) => file.type === type,
					);
				case PromptsStorage.user:
					return promptsState.userPromptFiles.filter(
						(file) => file.type === type,
					);
				case PromptsStorage.extension:
					return promptsState.extensionPromptFiles.filter(
						(file) => file.type === type,
					);
		...
		override setDisabledPromptFiles(
			_type: PromptsType,
			uris: ResourceSet,
		): void {
