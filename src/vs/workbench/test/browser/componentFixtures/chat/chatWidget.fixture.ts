...
import {
	...
	IChatInputStyles,
} from "../../../../contrib/chat/browser/widget/input/chatInputPart.js";
import {
	IChatWidget,
	IChatWidgetService,
...
import {
	...
	IToolRiskAssessment,
	ToolRiskLevel,
} from "../../../../contrib/chat/browser/tools/chatToolRiskAssessmentService.js";
...
export interface IFixtureMessage {
	...
	readonly assistant?: ReadonlyArray<
		...
		| {
				...
				riskAssessment?: { risk: ToolRiskLevel; explanation: string };
				riskLoading?: boolean;
				confirmation?: {
					commandLine: string;
					cwdLabel?: string;
...
export async function renderChatWidget(
	...
	options: IChatWidgetFixtureOptions,
): Promise<void> {
	const { container, disposableStore } = context;

	const widgetHolder: { current: IChatWidget | undefined } = {
		current: undefined,
	};
	...
	const hasRiskAssessment = options.messages.some((m) =>
		m.assistant?.some(
			(p) =>
				(p.kind === "terminalConfirmation" || p.kind === "elicitation") &&
				p.riskAssessment,
		),
	);
	const hasRiskLoading = options.messages.some((m) =>
		m.assistant?.some(
			(p) =>
				(p.kind === "terminalConfirmation" || p.kind === "elicitation") &&
				p.riskLoading,
	...
	const needsRiskService =
		hasRiskAssessment || hasRiskLoading || riskFeatureExplicitlyDisabled;
				...
				reg.defineInstance(
					...
					new (class extends mock<ILanguageModelToolsService>() {
						...
						override getTool(id: string) {
							return id === fixtureToolData.id ? fixtureToolData : undefined;
						}
					})(),
				);
				reg.defineInstance(
					IChatToolRiskAssessmentService,
					new (class extends mock<IChatToolRiskAssessmentService>() {
						override isEnabled() {
						...
						override async assess(): Promise<IToolRiskAssessment | undefined> {
							return new Promise(() => {});
						}
					})(),
				);
		...
		for (const part of message.assistant ?? []) {
			if (part.kind === "markdown") {
				...
				const elicitation = new ChatElicitationRequestPart(
					...
					undefined,
					undefined,
					part.riskAssessment || part.riskLoading
						? { toolId: fixtureToolData.id, parameters: undefined }
						: undefined,
				);
				...
				const toolInvocation = new ChatToolInvocation(
					{
						invocationMessage: new MarkdownString(
							`Running \`${part.command}\``,
						),
						pastTenseMessage: new MarkdownString(`Ran \`${part.command}\``),
						confirmationMessages: {
							title,
							message: new MarkdownString(`\`${part.command}\``),
							disclaimer: part.disclaimer
								? new MarkdownString(part.disclaimer, {
						...
						toolSpecificData: {
							...
							requestUnsandboxedExecutionReason:
								part.requestUnsandboxedExecutionReason,
	...
	const viewModel = disposableStore.add(
		instantiationService.createInstance(ChatViewModel, model, undefined),
	);
	...
	const menuService = instantiationService.get(
		IMenuService,
	) as FixtureMenuService;
	menuService.addItem(MenuId.ChatInput, {
		command: {
	...
	menuService.addItem(MenuId.ChatInput, {
		command: { id: "workbench.action.chat.openModePicker", title: "Agent" },
		group: "navigation",
		order: 1,
	});
	menuService.addItem(MenuId.ChatInput, {
	...
	menuService.addItem(MenuId.ChatExecute, {
		command: {
	...
	menuService.addItem(MenuId.ChatInputSecondary, {
		command: {
	...
	menuService.addItem(MenuId.ChatInputSecondary, {
		command: {
			id: "workbench.action.chat.openPermissionPicker",
	...
	const inputPart = disposableStore.add(
	...
	const listWidget = disposableStore.add(
		instantiationService.createInstance(ChatListWidget, listContainer, {
...
const SIMPLE_QA: IFixtureMessage[] = [
	{
		...
		assistant: [
			{
				kind: "markdown",
				text: "I added a recursive `fibonacci(n)` to `fibon.ts`. Note that recursion is exponential — for large `n` consider an iterative version.",
			},
		],
	},
];
		...
		assistant: [
			{
				...
				riskAssessment: {
					...
					explanation:
						"Initializes a new Git repository in the current directory. Reversible by removing the .git folder.",
				},
			},
		],
...
const MULTI_TURN: IFixtureMessage[] = [
	{
		...
		assistant: [
			{
				kind: "markdown",
				text: "This project is **Visual Studio Code**, a free source-code editor made by Microsoft for Windows, Linux and macOS.",
			},
		],
	},
	{
		...
		assistant: [
			{
				kind: "markdown",
				text: "The desktop entrypoint is in `src/vs/code/electron-main/main.ts`. The browser/server entrypoints live under `src/vs/server/`.",
			},
		],
	},
	{
		...
		assistant: [
			{
				kind: "markdown",
				text: "You are welcome — let me know if you have more questions.",
			},
		],
	},
];
...
export default defineThemedFixtureGroup(
	{ path: "chat/widget/" },
	{
		SimpleQA: defineComponentFixture({
			render: (ctx) => renderChatWidget(ctx, { messages: SIMPLE_QA }),
		}),
		Streaming: defineComponentFixture({
			labels: { kind: "animated" },
			render: (ctx) => renderChatWidget(ctx, { messages: STREAMING }),
		}),
		PendingToolApproval: defineComponentFixture({
			render: (ctx) =>
				renderChatWidget(ctx, { messages: PENDING_TOOL_APPROVAL }),
		}),
		bugs: defineThemedFixtureGroup({
			"issue-309796-missing-backslash": defineComponentFixture({
				render: (ctx) =>
					renderChatWidget(ctx, { messages: ISSUE_309796_MISSING_BACKSLASH }),
			}),
		}),
		MultiTurn: defineComponentFixture({
			render: (ctx) => renderChatWidget(ctx, { messages: MULTI_TURN }),
		}),
	},
);
