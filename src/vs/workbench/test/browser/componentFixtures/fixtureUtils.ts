...
} from "@vscode/component-explorer";
import {
	DisposableStore,
	DisposableTracker,
	IDisposable,
...
import {
	...
	TestCommandService,
} from "../../../../editor/test/browser/editorTestServices.js";
...
import {
	...
	renderSwimlanes,
} from "../../../../base/test/common/executionGraph.js";
...
class NullStorageService implements IStorageService {
	...
	onDidChangeValue(
		scope: StorageScope.APPLICATION_SHARED,
		key: string | undefined,
		disposable: DisposableStore,
	): Event<IApplicationSharedStorageValueChangeEvent>;
	onDidChangeValue(
		scope: StorageScope,
		key: string | undefined,
		disposable: DisposableStore,
...
const themeJsonModules: Record<string, string> = {
	"/extensions/theme-defaults/themes/dark_modern.json":
		dark_modern as unknown as string,
	"/extensions/theme-defaults/themes/dark_plus.json":
		dark_plus as unknown as string,
	"/extensions/theme-defaults/themes/dark_vs.json":
		dark_vs as unknown as string,
	"/extensions/theme-defaults/themes/light_modern.json":
		light_modern as unknown as string,
	"/extensions/theme-defaults/themes/light_plus.json":
		light_plus as unknown as string,
	"/extensions/theme-defaults/themes/light_vs.json":
		light_vs as unknown as string,
};

const fixtureExtensionResourceLoaderService =
	new (class implements IExtensionResourceLoaderService {
		declare readonly _serviceBrand: undefined;
		async readExtensionResource(uri: URI): Promise<string> {
			const content = themeJsonModules[uri.path];
			if (content === undefined) {
				throw new Error(
					`Fixture extension resource not found: ${uri.toString()}`,
				);
			}
			return content;
		}
		...
		isExtensionGalleryResource(): Promise<boolean> {
			return Promise.resolve(false);
		}
		getExtensionGalleryResourceURL(): Promise<URI | undefined> {
 ...
 */
export function registerWorkbenchServices(
 ...
 */
export function defineComponentFixture(
	...
	const createFixture = (theme: typeof darkTheme | typeof lightTheme) =>
		defineFixture({
			...
			render: async (container: HTMLElement, context) => {
				...
				const virtualTimeEnabled =
					(options.virtualTime?.enabled ?? true) &&
					context.host.kind !== "explorer-ui";
				...
				const leakDetectionEnabled =
					true && context.host.kind !== "explorer-ui";
				...
				const tracker = leakDetectionEnabled
				...
				const clock = new VirtualClock(
					new Date("2026-05-14T12:00:00Z").getTime(),
				);
				...
				const virtualTimeApi = createVirtualTimeApi(clock, {
					fakeRequestAnimationFrame: true,
				});
				...
				context.addDisposable({
					dispose: async () => {
						...
						try {
							...
							console.error(
								`[ComponentFixture] error disposing fixture: ${e instanceof Error ? e.stack : e}`,
							);
						}
						...
						if (virtualTimeEnabled) {
							try {
								...
								console.error(
									`[ComponentFixture] error draining virtual time during teardown: ${e instanceof Error ? e.stack : e}`,
								);
							}
						}
				...
				async function actualRender() {
					...
					if (virtualTimeEnabled) {
						...
						disposableStore.add(
							installFakeRunWhenIdle((_targetWindow, callback, _timeout?) => {
								const stackTrace = new Error().stack;
								const trace = TraceContext.instance
									.currentTrace()
									.child("runWhenIdle", stackTrace);
					...
					try {
						const disposableStackStore = disposableStore.add(
							new DisposableStackStore(),
						);
						...
						const p2 = virtualTimeEnabled
							? p.run({
									until: untilTime(
										clock.now + (options.virtualTime?.durationMs ?? 1000),
									),
									maxEvents: 200,
									maxTraceDepth: 5,
								})
						...
						if (virtualTimeEnabled && p.history.length > 0) {
							...
							console.error(
								`[ComponentFixture] ${theme === darkTheme ? "Dark" : "Light"} virtual-time history (${p.history.length} tasks):\n${renderSwimlanes(history)}`,
							);
						}
						throw e;
					} finally {
				...
				const fixtureRoot = createTraceRoot(
					`render#${++fixtureRenderCounter}(${themeLabel})`,
				);
				...
				const wantsTimeTrace =
					!!context.input &&
					typeof context.input === "object" &&
					!!(context.input as Record<string, unknown>).outputTimeTrace;
...
type ThemedFixtureGroupFixtures = Record<
	string,
	ThemedFixtures | ReturnType<typeof defineFixtureGroup>
>;
