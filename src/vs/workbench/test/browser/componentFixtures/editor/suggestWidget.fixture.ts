...
import {
	CompletionModel,
	LineContext,
} from "../../../../../editor/contrib/suggest/browser/completionModel.js";
...
function renderSuggestWidget(options: SuggestFixtureOptions): void {
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(
				...
				new (class extends mock<IMenuService>() {
					override createMenu() {
						return {
							onDidChange: new Emitter<IMenuChangeEvent>().event,
							getActions: () => [],
							dispose: () => {},
						};
					}
	...
	const provider = {
		_debugDisplayName: "suggestFixture",
		provideCompletionItems: () => completionList,
	};
	const items = completionList.suggestions.map(
		(s) => new CompletionItem(position, s, completionList, provider),
	);
	...
	suggestions: [
		{
			...
			detail:
				"(method) addEventListener(type: string, listener: EventListener): void",
		},
		{
			label: "appendChild",
			kind: CompletionItemKind.Method,
			insertText: "appendChild",
			range: {
				startLineNumber: 3,
				startColumn: 10,
				endLineNumber: 3,
			...
			detail: "(method) appendChild(node: Node): Node",
		},
		{
			label: "attributes",
		...
		},
		{
			...
			detail: "(method) blur(): void",
		},
		{
			label: "childElementCount",
			kind: CompletionItemKind.Property,
			insertText: "childElementCount",
			range: {
				startLineNumber: 3,
				startColumn: 10,
				endLineNumber: 3,
		...
		},
		{
			...
			detail: "(method) click(): void",
		},
		{
			label: "cloneNode",
			kind: CompletionItemKind.Method,
			insertText: "cloneNode",
			range: {
				startLineNumber: 3,
				startColumn: 10,
				endLineNumber: 3,
			...
			detail: "(method) cloneNode(deep?: boolean): Node",
		},
		{
			label: "closest",
			kind: CompletionItemKind.Method,
			...
			detail: "(method) closest(selectors: string): Element | null",
		},
		{
			label: "contains",
			kind: CompletionItemKind.Method,
			...
			detail: "(method) contains(other: Node | null): boolean",
		},
	],
};
