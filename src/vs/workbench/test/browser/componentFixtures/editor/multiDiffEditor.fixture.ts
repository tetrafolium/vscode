...
import {
	IResourceLabel as IMultiDiffResourceLabel,
...
function renderMultiDiffEditor({
	container,
	disposableStore,
	...
	const instantiationService = createCommonServices(
		disposableStore,
		theme,
		new TestDiffProviderFactoryService(),
	);

	const textModels = disposableStackStore.add(new DisposableStore());
	const { doc1, doc2, doc3 } = createDocuments(
		instantiationService,
		textModels,
	);
	const widget = disposableStackStore.add(
		createWidget(instantiationService, container),
	);
...
function createCommonServices(
	disposableStore: DisposableStore,
	theme: ComponentFixtureContext["theme"],
	diffProviderFactory: IDiffProviderFactoryService,
) {
	return createEditorServices(disposableStore, {
		...
		additionalServices: (reg) => {
			...
			reg.definePartialInstance(INotebookDocumentService, {
...
function createWidget(
	...
	const uiFactory = instantiationService.createInstance(
...
function createDocuments(
	...
	const original1 = textModels.add(
		createTextModel(
			instantiationService,
			ORIGINAL_CODE_1,
			URI.parse("inmemory://original/greet.ts"),
			"typescript",
		),
	);
	const modified1 = textModels.add(
		createTextModel(
			instantiationService,
			MODIFIED_CODE_1,
			URI.parse("inmemory://modified/greet.ts"),
			"typescript",
		),
	);
	const original2 = textModels.add(
		createTextModel(
			instantiationService,
			ORIGINAL_CODE_2,
			URI.parse("inmemory://original/config.ts"),
			"typescript",
		),
	);
	const modified2 = textModels.add(
		createTextModel(
			instantiationService,
			MODIFIED_CODE_2,
			URI.parse("inmemory://modified/config.ts"),
			"typescript",
		),
	);
	const original3 = textModels.add(
		createTextModel(
			instantiationService,
			ORIGINAL_CODE_3,
			URI.parse("inmemory://original/server.ts"),
			"typescript",
		),
	);
	const modified3 = textModels.add(
		createTextModel(
			instantiationService,
			MODIFIED_CODE_3,
			URI.parse("inmemory://modified/server.ts"),
			"typescript",
		),
	);
	return {
		doc1: RefCounted.createOfNonDisposable<IDocumentDiffItem>(
			{ original: original1, modified: modified1 },
			{ dispose() {} },
		),
		doc2: RefCounted.createOfNonDisposable<IDocumentDiffItem>(
			{ original: original2, modified: modified2 },
			{ dispose() {} },
		),
		doc3: RefCounted.createOfNonDisposable<IDocumentDiffItem>(
			{ original: original3, modified: modified3 },
			{ dispose() {} },
		),
	};
}
...
function renderMultiDiffEditorIncrementalUpdate() {
	return ({
		...
		const documents = new ValueWithChangeEvent<
			readonly RefCounted<IDocumentDiffItem>[]
		>([doc1]);
		const model: IMultiDiffEditorModel = { documents };
		const viewModel = disposableStackStore.add(widget.createViewModel(model));
		widget.setViewModel(viewModel);
		disposableStackStore.add(
			toDisposable(() => widget.setViewModel(undefined)),
		);
...
function renderMultiDiffEditorDocumentSwap() {
	return ({
		...
		const makeDoc = (origText: string, modText: string, name: string) => {
			const original = textModels.add(
				createTextModel(
					instantiationService,
					origText,
					URI.parse(`inmemory://original/${name}`),
					"typescript",
				),
			);
			const modified = textModels.add(
				createTextModel(
					instantiationService,
					modText,
					URI.parse(`inmemory://modified/${name}`),
					"typescript",
				),
			);
