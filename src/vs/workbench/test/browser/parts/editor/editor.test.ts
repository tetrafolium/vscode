...
suite("Workbench editor utils", () => {
	class TestEditorInputWithPreferredResource
	...
	test("EditorInputCapabilities", () => {
		...
		assert.strictEqual(
			sideBySideInput.hasCapability(EditorInputCapabilities.MultipleEditors),
			true,
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			sideBySideInput.hasCapability(EditorInputCapabilities.RequiresTrust),
			false,
		);
		assert.strictEqual(
	...
	test("EditorResourceAccessor - typed inputs", () => {
		...
		const file = disposables.add(
			new TestEditorInput(URI.file("/some/path.txt"), "editorResourceFileTest"),
		);
	...
	test("EditorResourceAccessor - untyped inputs", () => {
		...
		const sideBySideInput: IResourceSideBySideEditorInput = {
			primary: file,
			secondary: untitled,
		};
	...
	async function testWhenEditorClosed(
		sideBySide: boolean,
		custom: boolean,
		...resources: URI[]
	): Promise<void> {
		...
		const closedPromise = accessor.instantitionService.invokeFunction(
			(accessor) => whenEditorClosed(accessor, resources),
		);
