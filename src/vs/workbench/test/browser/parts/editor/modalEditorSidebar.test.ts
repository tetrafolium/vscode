...
class TestModalEditorSidebarHost extends Disposable {
	...
	private readonly _onDidLayout = this._register(
	...
	private renderContent(content: IModalEditorSidebar): void {
		this._renderCount++;
		this.contentDisposable.value = content.render(
			{} /* stub container */,
	...
	resizeSidebar(delta: number): void {
		const maxWidth = Math.max(
			...
			this.containerWidth - MODAL_MIN_WIDTH,
		);
		this._sidebarWidth = Math.min(
			maxWidth,
			Math.max(MODAL_SIDEBAR_MIN_WIDTH, this._sidebarWidth + delta),
		);
...
suite("Modal Editor Sidebar", () => {
	...
	test("addSidebar sets hasSidebar and default width", () => {
		...
		assert.deepStrictEqual(
			{
				hasSidebar: host.hasSidebar,
				sidebarWidth: host.sidebarWidth,
	...
	test("effectiveMinWidth accounts for sidebar", () => {
		...
		assert.deepStrictEqual(
			...
			{
				withoutSidebar: MODAL_MIN_WIDTH,
				withSidebar: MODAL_MIN_WIDTH + MODAL_SIDEBAR_MIN_WIDTH,
			},
		);
	});
	...
	test("resizeSidebar clamps to max width (container - modal min)", () => {
		...
		assert.strictEqual(
			host.sidebarWidth,
			host.containerWidth - MODAL_MIN_WIDTH,
		);
	});
	...
	test("layout fires onDidLayout with current dimensions", () => {
		...
		assert.deepStrictEqual(layouts, [
			{ height: 500, width: MODAL_SIDEBAR_DEFAULT_WIDTH },
		]);
	});
	...
	test("sidebar is visible by default", () => {
		...
		assert.deepStrictEqual(
			{
				visible: host.sidebarVisible,
				effectiveWidth: host.effectiveSidebarWidth,
			},
	...
	test("sidebar hidden state persists via constructor", () => {
		const host = disposables.add(
			new TestModalEditorSidebarHost(undefined, true),
		);
		host.addSidebar(stubSidebarContent());
	...
	test("hidden sidebar preserves width for when restored", () => {
		...
		assert.deepStrictEqual(
			...
			{
				effectiveWidth: MODAL_SIDEBAR_DEFAULT_WIDTH + 50,
				sidebarWidth: MODAL_SIDEBAR_DEFAULT_WIDTH + 50,
			},
		);
	});
});
