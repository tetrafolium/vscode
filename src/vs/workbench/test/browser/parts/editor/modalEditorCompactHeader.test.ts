 ...
 */
function applyCompactHeaderClass(
	modalElement: { classList: DOMTokenList },
...
suite("Modal Editor Compact Header", () => {
	...
	test("isModalEditorOptionsProvider type guard", () => {
		...
		assert.strictEqual(
			isModalEditorOptionsProvider({ getModalEditorOptions: "nope" }),
			false,
		);
		assert.strictEqual(
			isModalEditorOptionsProvider(new TestEditor({ compactHeader: true })),
			true,
		);
	});
	...
	test("compact-header class toggles based on active editor options", () => {
		...
		assert.strictEqual(
			modalElement.classList.contains("compact-header"),
			false,
		);

		// Editor that doesn't implement the provider → class not set
		applyCompactHeaderClass(modalElement, {
			/* not a provider */
		});
		assert.strictEqual(
			modalElement.classList.contains("compact-header"),
		...
		applyCompactHeaderClass(
			modalElement,
			new TestEditor({ compactHeader: false }),
		);
		assert.strictEqual(
			modalElement.classList.contains("compact-header"),
			false,
		...
		applyCompactHeaderClass(
			modalElement,
			new TestEditor({ compactHeader: true }),
		);
	...
	test("header height prefers measured offsetHeight over fallback constant", () => {
		...
		assert.strictEqual(
			resolveHeaderHeight({ offsetHeight: 0 }),
			MODAL_HEADER_HEIGHT,
		);
