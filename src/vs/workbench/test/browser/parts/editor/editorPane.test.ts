...
suite("EditorPane", () => {
	...
	test("EditorMemento - basics", function () {
		...
		let memento = disposables.add(
			new EditorMemento<TestViewState>(
				"id",
				"key",
				rawMemento,
				3,
	...
	test("EditorMemento - move", function () {
		...
		memento.saveEditorState(testGroup0, URI.file("/some/folder/file-2.txt"), {
			line: 2,
		});
	...
	test("EditoMemento - use with editor input", function () {
		...
		class TestEditorInput extends EditorInput {
			...
			override matches(other: TestEditorInput): boolean {
				return (
					other && this.id === other.id && other instanceof TestEditorInput
				);
			}
		}
