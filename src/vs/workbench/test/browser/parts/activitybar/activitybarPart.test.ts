...
suite("ActivitybarPart", () => {
	...
	function createActivitybarPart(compact: boolean): {
	...
	test("default mode returns default width constraints", () => {
		...
		assert.deepStrictEqual(
			...
			{
				min: ActivitybarPart.ACTIVITYBAR_WIDTH,
				max: ActivitybarPart.ACTIVITYBAR_WIDTH,
			},
		);
	});
	...
	test("compact mode returns compact width constraints", () => {
		...
		assert.deepStrictEqual(
			{ min: part.minimumWidth, max: part.maximumWidth },
			{
				min: ActivitybarPart.COMPACT_ACTIVITYBAR_WIDTH,
				max: ActivitybarPart.COMPACT_ACTIVITYBAR_WIDTH,
			},
		);
	});
	...
	test("fires onDidChange(undefined) when compact setting changes", () => {
		...
		assert.strictEqual(
			events[0],
			undefined,
			"should fire undefined to signal constraint change",
		);
	...
	test("updateCompactStyle sets correct CSS custom properties in default mode", () => {
		...
		assert.strictEqual(
			...
			`${ActivitybarPart.ACTIVITYBAR_WIDTH}px`,
		);
		assert.strictEqual(
			el.style.getPropertyValue("--activity-bar-action-height"),
			`${ActivitybarPart.ACTION_HEIGHT}px`,
		);
		assert.strictEqual(
			el.style.getPropertyValue("--activity-bar-icon-size"),
			`${ActivitybarPart.ICON_SIZE}px`,
		);
	...
	test("updateCompactStyle sets correct CSS custom properties in compact mode", () => {
		...
		assert.strictEqual(
			el.style.getPropertyValue("--activity-bar-icon-size"),
			`${ActivitybarPart.COMPACT_ICON_SIZE}px`,
		);
