...
import {
	...
	sshAuthorityString,
} from "../../browser/openInVSCodeUtils.js";
...
suite("resolveRemoteAuthority", () => {
	...
	function makeProvidersService(
		...
		return {
			getProvider: (id: string) =>
				remoteAddress ? { id, remoteAddress } : undefined,
	...
	function makeRemoteAgentHostService(
		entries: IRemoteAgentHostEntry[] = [],
	): IRemoteAgentHostService {
		return {
			getEntryByAddress: (address: string) =>
				entries.find((e) => getEntryAddress(e) === address),
