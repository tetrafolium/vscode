...
import {
	...
	TERMINAL_VIEW_ID,
} from "../../../../workbench/contrib/terminal/common/terminal.js";
import {
 ...
 */
function getSessionTerminalInfo(
	session: ISession | undefined,
	reader?: IReader,
): ISessionTerminalInfo | undefined {
	if (!session) {
		return undefined;
	}
 ...
 */
export class SessionsTerminalContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const profileOverride = derived((reader) => {
			...
			return (
				profiles.find((p) => p.address === address) ??
				this._agentHostTerminalService.getProfileForConnection(address)
			);
		});
		...
		this._register(
			autorun((reader) => {
				...
				if (profile) {
					reader.store.add(
						this._terminalProfileService.overrideDefaultProfile(
		...
		const terminalViewVisible =
			SessionsTerminalViewVisibleContext.bindTo(contextKeyService);
					...
					instance.getInitialCwd().then((cwd) => {
						if (cwd.toLowerCase() !== this._activeKey) {
							const availableInstance = this._getAvailableTerminal(
								instance,
								`hide restored terminal for ${cwd}`,
							);
							...
							this._logService.trace(
								`[SessionsTerminal] Hid restored terminal ${availableInstance.instanceId} (cwd: ${cwd})`,
							);
						}
					});
				}
		...
		this._register(
			this._sessionsManagementService.onDidChangeSessions((e) => {
				...
				this._logService.info(
					`[SessionsTerminal] onDidChangeSessions cleanup (removed: ${e.removed.length}, justArchived: ${justArchived.length}, liveCwdKeys: [${[...liveCwdKeys].join(", ")}], activeKey: ${this._activeKey ?? "<none>"})`,
				);
				for (const session of e.removed) {
					const info = getSessionTerminalInfo(session);
					if (info && !liveCwdKeys.has(info.cwd.fsPath.toLowerCase())) {
						this._logService.info(
							`[SessionsTerminal] Closing terminals for ${info.cwd.fsPath} (session ${session.sessionId} removed; no live session owns this cwd)`,
						);
						void this._closeTerminalsForPath(
							info.cwd.fsPath,
							`session removed (${session.sessionId})`,
						);
					}
				}
				for (const session of justArchived) {
					...
					if (info && !liveCwdKeys.has(info.cwd.fsPath.toLowerCase())) {
						this._logService.info(
							`[SessionsTerminal] Hiding terminals for ${info.cwd.fsPath} (session ${session.sessionId} archived; no live session owns this cwd)`,
						);
						void this._hideTerminalsForPath(
							info.cwd.fsPath,
	 ...
	 */
	private async _createTerminalForSession(
		cwd: URI,
		session: ISession | undefined,
	): Promise<ITerminalInstance> {
	 ...
	 */
	private _getSessionAgentHostAddress(
		session: ISession | undefined,
	): string | undefined {
		if (!session) {
			return undefined;
	...
	private async _onActiveSessionChanged(
		session: ISession | undefined,
	): Promise<void> {
		if (!session) {
			return;
	 ...
	 * the given key.
	 */
	private async _findTerminalsForKey(
		key: string,
	): Promise<ITerminalInstance[]> {
	 ...
	 */
	private async _updateTerminalVisibility(
		activeKey: string,
		forceForegroundTerminalIds: number[],
	): Promise<void> {
		...
		for (const instance of [...this._terminalService.instances]) {
			...
			const isForeground =
				this._terminalService.foregroundInstances.includes(currentInstance);
		...
		for (const instance of toHide) {
			...
			if (availableInstance) {
				this._logService.debug(
					`[SessionsTerminal] Hiding terminal ${availableInstance.instanceId} (does not belong to active key ${activeKey})`,
				);
				this._terminalService.moveToBackground(availableInstance);
			}
		}
	...
	private async _closeTerminalsForPath(
		...
		const protectedInstanceId =
			this._terminalService.activeInstance?.instanceId;
		for (const instance of [...this._terminalService.instances]) {
			...
			try {
				...
				if (
					protectedInstanceId !== undefined &&
					instance.instanceId === protectedInstanceId
				) {
					this._logService.info(
						`[SessionsTerminal] Skipping active terminal ${instance.instanceId} for ${fsPath} (user is working in it)`,
					);
					continue;
				}
				const availableInstance = this._getAvailableTerminal(
					instance,
					`close removed session terminal for ${fsPath}`,
				);
				if (!availableInstance) {
					continue;
				}
				this._logService.info(
					`[SessionsTerminal] Killing terminal ${availableInstance.instanceId} (cwd: ${fsPath}, reason: ${reason})`,
				);
	 ...
	 * #318645.
	 */
	private async _hideTerminalsForPath(
		fsPath: string,
		reason: string,
	): Promise<void> {
		const key = fsPath.toLowerCase();
		...
		for (const instance of [...this._terminalService.instances]) {
			...
			try {
				...
				const availableInstance = this._getAvailableTerminal(
					instance,
					`hide archived terminal for ${fsPath}`,
				);
				if (!availableInstance) {
					continue;
				}
				this._logService.info(
					`[SessionsTerminal] Hiding terminal ${availableInstance.instanceId} (cwd: ${fsPath}, reason: ${reason})`,
				);
				this._terminalService.moveToBackground(availableInstance);
			} catch {
				// ignore
 ...
 */
class RegisterAgentHostSessionTaskRunnerContribution
	extends Disposable
	implements IWorkbenchContribution
{
