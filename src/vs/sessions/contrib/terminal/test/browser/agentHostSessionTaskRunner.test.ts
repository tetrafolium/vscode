...
import {
	IAgentHostTerminalCreateOptions,
	IAgentHostTerminalService,
} from "../../../../../workbench/contrib/terminal/browser/agentHostTerminalService.js";
import {
...
suite("AgentHostSessionTaskRunner", () => {
	...
	setup(() => {
		...
		instantiationService.stub(
			ISessionsTasksService,
			new (class extends mock<ISessionsTasksService>() {
			...
			new (class extends mock<ISessionsProvidersService>() {
				override getProvider<T extends ISessionsProvider>(
					...
					if (
						...
						id.startsWith(REMOTE_AGENT_HOST_PROVIDER_PREFIX)
					) {
		...
		instantiationService.stub(
			ITerminalGroupService,
			new (class extends mock<ITerminalGroupService>() {
	...
	test("canRun: false for non-agent-host providers", () => {
		assert.strictEqual(
			runner.canRun(makeSession({ providerId: "copilot-chat-sessions" })),
			false,
		);
	});
	...
	test("canRun: true for local agent host", () => {
		assert.strictEqual(
			runner.canRun(makeSession({ providerId: LOCAL_AGENT_HOST_PROVIDER_ID })),
			true,
		);
	});
	...
	test("canRun: true for remote agent host", () => {
		assert.strictEqual(
			runner.canRun(makeSession({ providerId: "agenthost-myhost" })),
			true,
		);
	});
	...
	test("local agent-host sessions pass through file: cwd", async () => {
		...
		assert.deepStrictEqual(
			createdTerminals[0].options?.cwd?.toString(),
			cwd.toString(),
		);
	...
	test("unknown scheme cwds are omitted (host uses default)", async () => {
		const session = makeSession({
			providerId: "agenthost-myhost",
			cwd: URI.parse("vscode-vfs://github/owner/repo"),
		});
	...
	test("resolves dependsOn chains against the full tasks.json", async () => {
		...
		const transpile: ITaskEntry = {
			label: "Transpile Client",
			type: "shell",
			command: "npm",
			args: ["run", "transpile"],
		};
		const runDev: ITaskEntry = {
			label: "Run Dev",
			type: "shell",
			command: "npm",
			args: ["run", "dev"],
		...
		assert.deepStrictEqual(sentText, [
			{ text: "npm run transpile && npm run dev", shouldExecute: true },
		]);
	});
});
