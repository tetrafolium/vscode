...
type TestTerminalInstance = ITerminalInstance & {
	...
	_testSetShellLaunchConfig(
		shellLaunchConfig: ITerminalInstance["shellLaunchConfig"],
	): void;
};
...
function makeTerminalInstance(id: number, cwd: string): TestTerminalInstance {
	...
	let shellLaunchConfig: ITerminalInstance["shellLaunchConfig"] =
...
suite("SessionsTerminalContribution", () => {
	...
	let activeSessionObs: ReturnType<
		typeof observableValue<IActiveSession | undefined>
	>;
	...
	setup(() => {
		...
		activeSessionObs = observableValue<IActiveSession | undefined>(
			"activeSession",
			undefined,
		);
		...
		instantiationService.stub(
			ITerminalService,
			new (class extends mock<ITerminalService>() {
				...
				override get foregroundInstances(): readonly ITerminalInstance[] {
					return [...terminalInstances.values()].filter(
						(i) => !backgroundedInstances.has(i.instanceId),
					);
				}
				override get activeInstance(): ITerminalInstance | undefined {
					return activeInstanceId !== undefined
						? terminalInstances.get(activeInstanceId)
						: undefined;
				}
				...
				override async safeDisposeTerminal(
					instance: ITerminalInstance,
				): Promise<void> {
				...
				override async showBackgroundTerminal(
					instance: ITerminalInstance,
				): Promise<void> {
					backgroundedInstances.delete(instance.instanceId);
		...
		instantiationService.stub(
			...
			new (class extends mock<IAgentHostTerminalService>() {
				override readonly profiles = constObservable<never[]>([]);
				override getProfileForConnection() {
					return undefined;
				}
				override setDefaultCwd(cwd: URI | undefined): void {
					defaultCwdCalls.push(cwd);
				}
				override async createTerminalForEntry() {
					return undefined;
				}
			})(),
		);
		...
		instantiationService.stub(
			...
			new (class extends mock<ITerminalProfileService>() {
				override overrideDefaultProfile() {
		...
		instantiationService.stub(
			...
			new (class extends mock<IViewsService>() {
				override isViewVisible(): boolean {
					return false;
				}
				override onDidChangeViewVisibility = store.add(
		...
		contribution = store.add(
			instantiationService.createInstance(SessionsTerminalContribution),
		);
	});
	...
	test("uses worktree directory for a cloud agent session when workspace exists", async () => {
		...
		assert.strictEqual(createdTerminals.length, 1);
		assert.strictEqual(
			createdTerminals[0].cwd.fsPath,
			URI.file("/worktree").fsPath,
		);
	});
	...
	test("does not recreate terminal when multiple non-background sessions share the home directory", async () => {
		const session1 = makeAgentSession({
			providerType: AgentSessionProviders.Cloud,
		});
		...
		const session2 = makeAgentSession({
			providerType: AgentSessionProviders.Local,
		});
	...
	test("waits for a loading session before creating a terminal", async () => {
		...
		assert.strictEqual(
			...
			0,
			"should not create a terminal while session is loading",
		);
		assert.strictEqual(
			defaultCwdCalls.at(-1),
			undefined,
			"should not set the default cwd while session is loading",
		);
	...
	test("ensureTerminal reuses existing terminal for same path", async () => {
		...
		assert.strictEqual(
			createdTerminals.length,
			1,
			"should reuse the existing terminal",
		);
		assert.strictEqual(
			activeInstanceSet.length,
			1,
			"should only set active instance on creation",
		);
	});
	...
	test("ensureTerminal does not activate a terminal disposed during creation", async () => {
		...
		assert.ok(
			logService.traces.some((message) =>
				message.includes(
					`Cannot activate created terminal for ${cwd.fsPath}; terminal 1 is no longer available`,
				),
			),
		);
	});
	...
	test("hides (does not dispose) terminals when session is archived", async () => {
		...
		const otherSession = makeAgentSession({
			worktree: URI.file("/other"),
			providerType: AgentSessionProviders.Background,
		});
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"archived session terminal must be hidden, not disposed",
		);
		assert.deepStrictEqual(
			moveToBackgroundCalls,
			[1],
			"archived session terminal should be moved to background",
		);
	});
	...
	test("hides terminals when archived session has only a repository (no worktree)", async () => {
		...
		const archivedSession = makeAgentSession({
			repository: repoUri,
			providerType: AgentSessionProviders.Background,
			isArchived: true,
		});
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"archived repo-only session terminal must be hidden, not disposed",
		);
	...
	test("does not hide the terminal at the active session cwd when archiving (just-opened terminal is protected)", async () => {
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"terminal at the active session cwd must not be disposed",
		);
		assert.strictEqual(
			moveToBackgroundCalls.length,
			0,
			"terminal at the active session cwd must not be hidden",
		);
	});
	...
	test("does not re-hide a newly-opened terminal when an already-archived session is re-emitted", async () => {
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"re-emitted archived session must not dispose any terminal",
		);
		assert.strictEqual(
			moveToBackgroundCalls.length,
			0,
			"re-emitted archived session must not re-hide the newly-opened terminal",
		);
	});
	...
	test("does not hide terminals for a session that was already archived when the contribution started", async () => {
		...
		const worktreeUri = URI.file("/worktree");
		const archivedSession = makeAgentSession({
			sessionId: "test:restored-archived",
			worktree: worktreeUri,
			providerType: AgentSessionProviders.Background,
			isArchived: true,
		});
		...
		const freshContribution = store.add(
			instantiationService.createInstance(SessionsTerminalContribution),
		);
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"already-archived session must not dispose any terminal",
		);
		assert.strictEqual(
			moveToBackgroundCalls.length,
			0,
			"already-archived session must not be treated as a fresh archive transition",
		);
	});
	...
	test("does not dispose the focused terminal when its session is removed (graduation case)", async () => {
		...
		const skeleton = makeAgentSession({
			sessionId: "test:untitled",
			worktree: worktreeUri,
			providerType: AgentSessionProviders.Background,
		});
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"the focused terminal must not be disposed on graduation",
		);
	});
	...
	test("does not close terminal when another live session still owns the cwd (replace case)", async () => {
		...
		const fromSession = makeAgentSession({
			sessionId: "test:untitled",
			worktree: worktreeUri,
			providerType: AgentSessionProviders.Background,
		});
		const toSession = makeAgentSession({
			sessionId: "test:committed",
			worktree: worktreeUri,
			providerType: AgentSessionProviders.Background,
		});
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"terminal should be kept alive for the surviving session",
		);
	});
	...
	test("does not hide or dispose terminal when archiving one of two sessions sharing a cwd", async () => {
		...
		const liveSession = makeAgentSession({
			sessionId: "test:live",
			worktree: worktreeUri,
			providerType: AgentSessionProviders.Background,
		});
		...
		assert.strictEqual(
			disposedInstances.length,
			0,
			"terminal should be kept for the still-live session",
		);
		assert.strictEqual(
			moveToBackgroundCalls.length,
			0,
			"terminal owned by a still-live session must not be hidden",
		);
	});
	...
	test("closes terminal when the only session at a cwd is removed even if other live sessions exist elsewhere", async () => {
		...
		const otherLive = makeAgentSession({
			sessionId: "test:other",
			worktree: URI.file("/other"),
			providerType: AgentSessionProviders.Background,
		});
		const removedSession = makeAgentSession({
			sessionId: "test:gone",
			worktree: worktreeUri,
			providerType: AgentSessionProviders.Background,
		});
		...
		assert.strictEqual(
			disposedInstances.length,
			1,
			"no live session owns this cwd, terminal should be closed",
		);
	});
	...
	test("switching back to a previously used background path reuses the existing terminal", async () => {
		...
		assert.strictEqual(
			createdTerminals.length,
			2,
			"should reuse the terminal for cwd1",
		);
	});
	...
	test("hides terminals from previous session when switching to a new session", async () => {
		...
		assert.ok(
			moveToBackgroundCalls.includes(1),
			"terminal for cwd1 should be backgrounded",
		);
		assert.ok(
			backgroundedInstances.has(1),
			"terminal for cwd1 should remain backgrounded",
		);
	});
	...
	test("shows previously hidden terminals when switching back to their session", async () => {
		...
		assert.ok(
			showBackgroundCalls.includes(1),
			"terminal for cwd1 should be shown",
		);
		assert.ok(
			!backgroundedInstances.has(1),
			"terminal for cwd1 should be foreground",
		);
		// Terminal for cwd2 (id=2) should now be backgrounded
		assert.ok(
			backgroundedInstances.has(2),
			"terminal for cwd2 should be backgrounded",
		);
	});
	...
	test("only terminals of the active session are visible after multiple switches", async () => {
		...
		assert.ok(
			!backgroundedInstances.has(3),
			"terminal for cwd3 should be foreground",
		);
	});
	...
	test("shows pre-existing terminal with matching cwd instead of creating a new one", async () => {
		...
		assert.ok(
			showBackgroundCalls.includes(existingInstance.instanceId),
			"should show the existing terminal",
		);
	});
	...
	test("does not background a restored terminal that is disposed before cwd resolves", async () => {
		...
		const restoredInstance = makeTerminalInstance(
			nextInstanceId++,
			"/restored",
		);
		restoredInstance._testSetShellLaunchConfig({
			attachPersistentProcess: {} as never,
		} as ITerminalInstance["shellLaunchConfig"]);
		restoredInstance.getInitialCwd = () =>
		...
		assert.ok(
			!moveToBackgroundCalls.includes(restoredInstance.instanceId),
			"disposed restored terminal should not be backgrounded",
		);
		assert.ok(
			logService.traces.some(
				(message) =>
					message.includes(
						"Cannot hide restored terminal for /other; terminal",
	...
	test("hides pre-existing terminal with non-matching cwd when session changes", async () => {
		...
		assert.ok(
			moveToBackgroundCalls.includes(otherInstance.instanceId),
			"non-matching terminal should be backgrounded",
		);
	});
	...
	test("ensureTerminal finds a backgrounded terminal instead of creating a new one", async () => {
		...
		assert.strictEqual(
			result[0].instanceId,
			instanceId,
			"should return the existing backgrounded terminal",
		);
	});
	...
	test("visibility is determined by initial cwd, not by stored IDs", async () => {
		...
		assert.ok(
			!backgroundedInstances.has(ext1.instanceId),
			"ext1 should be foreground (matching cwd)",
		);
		assert.ok(
			backgroundedInstances.has(ext2.instanceId),
			"ext2 should be backgrounded (non-matching cwd)",
		);
		...
		assert.ok(
			backgroundedInstances.has(ext1.instanceId),
			"ext1 should now be backgrounded",
		);
		assert.ok(
			!backgroundedInstances.has(ext2.instanceId),
			"ext2 should now be foreground",
		);
	});
	...
	test("sets the terminal with the most recent command as active after visibility update", async () => {
		...
		assert.strictEqual(
			activeInstanceSet.at(-1),
			t2.instanceId,
			"should set the terminal with the most recent command as active",
		);
	});
	...
	test("does not change active instance when no terminals have command history", async () => {
		...
		assert.strictEqual(
			activeInstanceSet.length,
			activeCountBefore,
			"should not call setActiveInstance when no command history exists",
		);
	});
	...
	test("uses the unwrapped repository path for a background session with a remote agent host repository", async () => {
		const remoteRepoUri = toAgentHostUri(
			URI.file("/Users/user/repo"),
			"my-server",
		);
		const session = makeAgentSession({
			repository: remoteRepoUri,
			providerType: AgentSessionProviders.Background,
		});
		activeSessionObs.set(session, undefined);
		await tick();
		...
		assert.strictEqual(
			createdTerminals.length,
			1,
			"should create a terminal at the unwrapped repository path",
		);
		assert.strictEqual(
			createdTerminals[0].cwd.fsPath,
			URI.file("/Users/user/repo").fsPath,
		);
	});
	...
	test("does not hide hidden tool terminals when session is archived", async () => {
		...
		const toolTerminal = makeTerminalInstance(
			nextInstanceId++,
			worktreeUri.fsPath,
		);
		...
		assert.deepStrictEqual(
			moveToBackgroundCalls,
			[1],
			"only the regular terminal should be hidden, not the tool terminal",
		);
	});
	...
	test("does not dispose hidden tool terminals when session is removed", async () => {
		...
		assert.strictEqual(
			disposedInstances.length,
			1,
			"should dispose exactly one terminal",
		);
		assert.notStrictEqual(
			disposedInstances[0].instanceId,
			toolTerminal.instanceId,
			"should not dispose the tool terminal",
		);
	});
	...
	test("does not background hidden tool terminals during session switch", async () => {
		...
		assert.ok(
			!moveToBackgroundCalls.includes(toolTerminal.instanceId),
			"hidden tool terminal should not be moved to background",
		);
	});
	...
	test("does not include hidden tool terminals in ensureTerminal matches", async () => {
		...
		assert.strictEqual(
			createdTerminals.length,
			1,
			"should create a new terminal since tool terminal is hidden",
		);
	});
	...
	test("does not hide restored hidden tool terminals on session create", async () => {
		...
		assert.ok(
			!moveToBackgroundCalls.includes(toolTerminal.instanceId),
			"hidden tool terminal should not be moved to background on restore",
		);
	});
});
