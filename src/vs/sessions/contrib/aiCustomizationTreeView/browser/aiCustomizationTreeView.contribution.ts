...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor, context: ItemContext): Promise<void> {
			...
			const name =
				typeof context === "object" && !URI.isUri(context)
...
MenuRegistry.appendMenuItem(AICustomizationItemMenuId, {
	...
	when: ContextKeyExpr.equals(
		AICustomizationItemTypeContextKey.key,
		PromptsType.prompt,
	),
});
...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor, context: ItemContext): Promise<void> {
			...
			const view = viewsService.getActiveViewWithId<AICustomizationViewPane>(
				AI_CUSTOMIZATION_VIEW_ID,
			);
			view?.refresh();
		}
...
MenuRegistry.appendMenuItem(AICustomizationItemMenuId, {
	...
	when: ContextKeyExpr.and(
		...
		ContextKeyExpr.equals(
			AICustomizationItemStorageContextKey.key,
			BUILTIN_STORAGE,
		),
		ContextKeyExpr.equals(
			AICustomizationItemTypeContextKey.key,
...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			const viewsService = accessor.get(IViewsService);
			const sessionsView = await viewsService.openView<SessionsView>(
				SessionsViewId,
				false,
			);
