...
import {
	...
	BUILTIN_STORAGE,
} from "../../chat/common/builtinPromptsStorage.js";
 ...
 */
export const AICustomizationIsEmptyContextKey = new RawContextKey<boolean>(
	"aiCustomization.isEmpty",
	true,
);
 ...
 */
export const AICustomizationItemTypeContextKey = new RawContextKey<string>(
	"aiCustomizationItemType",
	"",
);
 ...
 */
export const AICustomizationItemDisabledContextKey = new RawContextKey<boolean>(
	"aiCustomizationItemDisabled",
	false,
);
 ...
 */
export const AICustomizationItemStorageContextKey = new RawContextKey<string>(
	"aiCustomizationItemStorage",
	"",
);
...
type AICustomizationTreeItem =
	| IAICustomizationTypeItem
	| IAICustomizationGroupItem
	| IAICustomizationFileItem
	| IAICustomizationLinkItem;
...
class AICustomizationCategoryRenderer implements ITreeRenderer<
	IAICustomizationTypeItem | IAICustomizationLinkItem,
	FuzzyScore,
	ICategoryTemplateData
> {
	...
	renderElement(
		...
		>,
		_index: number,
		templateData: ICategoryTemplateData,
	): void {
		templateData.icon.className = "icon";
		templateData.icon.classList.add(
			...ThemeIcon.asClassNameArray(node.element.icon),
		);
...
class AICustomizationGroupRenderer implements ITreeRenderer<
	IAICustomizationGroupItem,
	FuzzyScore,
	IGroupTemplateData
> {
	...
	renderTemplate(container: HTMLElement): IGroupTemplateData {
		const element = dom.append(
			container,
			dom.$(".ai-customization-group-header"),
		);
		const label = dom.append(element, dom.$(".label"));
	...
	renderElement(
		node: ITreeNode<IAICustomizationGroupItem, FuzzyScore>,
		_index: number,
		templateData: IGroupTemplateData,
	): void {
		templateData.label.textContent = node.element.label;
	}
...
class AICustomizationFileRenderer implements ITreeRenderer<
	IAICustomizationFileItem,
	FuzzyScore,
	IFileTemplateData
> {
	...
	renderElement(
		node: ITreeNode<IAICustomizationFileItem, FuzzyScore>,
		_index: number,
	...
	disposeElement(
		_node: ITreeNode<IAICustomizationFileItem, FuzzyScore>,
		_index: number,
		templateData: IFileTemplateData,
	): void {
		templateData.elementDisposables.clear();
 ...
 */
class UnifiedAICustomizationDataSource implements IAsyncDataSource<
	RootElement,
	AICustomizationTreeItem
> {
	...
	async getChildren(
		element: RootElement | AICustomizationTreeItem,
	): Promise<AICustomizationTreeItem[]> {
		try {
			if (element === ROOT_ELEMENT) {
			...
			if (element.type === "group") {
				return this.getFilesForStorageAndType(
					element.storage,
					element.promptType,
				);
			}
			...
			this.logService.error(
				"[AICustomization] Error fetching tree children:",
				error,
			);
			return [];
		}
	 ...
	 */
	private async getStorageGroups(
		promptType: PromptsType,
	): Promise<IAICustomizationGroupItem[]> {
		...
		if (promptType === PromptsType.skill) {
			...
			const workspaceSkills = cached.skills.filter(
				(s) => s.storage === PromptsStorage.local,
			);
			const userSkills = cached.skills.filter(
				(s) => s.storage === PromptsStorage.user,
			);
			const extensionSkills = cached.skills.filter(
				(s) => s.storage === PromptsStorage.extension,
			);
			const builtinSkills = cached.skills.filter(
				(s) => s.storage === BUILTIN_STORAGE,
			);
		...
		if (!cached.files) {
			const allItems: IPromptPath[] = [
				...(await this.promptsService.listPromptFiles(
					promptType,
					CancellationToken.None,
				)),
			];
			...
			const workspaceItems = allItems.filter(
				(item) => item.storage === PromptsStorage.local,
			);
			const userItems = allItems.filter(
				(item) => item.storage === PromptsStorage.user,
			);
			const extensionItems = allItems.filter(
				(item) => item.storage === PromptsStorage.extension,
			);
			const builtinItems = allItems.filter(
				(item) => item.storage === BUILTIN_STORAGE,
			);
		...
		if (userItems.length > 0) {
			groups.push(
				this.createGroupItem(promptType, PromptsStorage.user, userItems.length),
			);
		}
		...
		if (builtinItems.length > 0) {
			groups.push(
				this.createGroupItem(promptType, BUILTIN_STORAGE, builtinItems.length),
			);
		}
	...
	private createGroupItem(
		...
		const storageLabels: Record<string, string> = {
			...
			[AICustomizationSources.extension]: localize(
				"extensionsWithCount",
				"Extensions ({0})",
				count,
			),
			[AICustomizationSources.plugin]: localize(
				"pluginsWithCount",
				"Plugins ({0})",
				count,
			),
			[AICustomizationSources.builtin]: localize(
				"builtinWithCount",
				"Built-in ({0})",
				count,
			),
	 ...
	 */
	private async getFilesForStorageAndType(
		storage: AICustomizationPromptsStorage,
		promptType: PromptsType,
	): Promise<IAICustomizationFileItem[]> {
		...
		if (promptType === PromptsType.skill) {
			...
			const result: IAICustomizationFileItem[] = filtered.map((skill) => {
			...
			if (disabledUris.size > 0) {
				...
				for (const file of allSkillFiles) {
					if (
						...
						!seenUris.has(file.uri.toString()) &&
						disabledUris.has(file.uri)
					) {
						result.push({
							type: "file" as const,
...
export class AICustomizationViewPane extends ViewPane {
	...
	constructor(
		...
		this.isEmptyContextKey =
			AICustomizationIsEmptyContextKey.bindTo(contextKeyService);
		this.itemTypeContextKey =
			AICustomizationItemTypeContextKey.bindTo(contextKeyService);
		this.itemDisabledContextKey =
			AICustomizationItemDisabledContextKey.bindTo(contextKeyService);
		this.itemStorageContextKey =
			AICustomizationItemStorageContextKey.bindTo(contextKeyService);

		// Subscribe to prompt service events to refresh tree
		this._register(
			this.promptsService.onDidChangeCustomAgents(() => this.refresh()),
		);
		this._register(
			this.promptsService.onDidChangeSlashCommands(() => this.refresh()),
		);
	...
	private createTree(): void {
			...
			this.instantiationService.createInstance(
				...
				this.dataSource,
				{
					...
					accessibilityProvider: {
						...
						getWidgetAriaLabel: () =>
							localize("aiCustomizationTree", "Chat Customization Items"),
					},
		...
		void this.tree
			.setInput(ROOT_ELEMENT)
			.then(() => this.autoExpandCategories());
	}
	...
	private onContextMenu(
		e: ITreeContextMenuEvent<AICustomizationTreeItem | null>,
	): void {
		...
		const menu = this.menuService.getMenuActions(
			...
			this.contextKeyService,
			{ arg: context, shouldForwardArgs: true },
		);
