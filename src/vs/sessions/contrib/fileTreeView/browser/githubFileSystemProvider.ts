	...
	implements IFileSystemProviderWithFileReadWriteCapability
{
	...
	private fetchTree(
		...
		if (
			cached &&
			Date.now() - cached.fetchedAt < GitHubFileSystemProvider.CACHE_TTL_MS
		) {
		...
		if (
			...
			Date.now() - notFoundAt < GitHubFileSystemProvider.NOT_FOUND_CACHE_TTL_MS
		) {
	...
	private async doFetchTree(
		...
		const entries = new Map<
			string,
			{ type: FileType; size: number; sha: string }
		>();
		...
		for (const entry of data.tree) {
			const fileType =
				entry.type === "tree" ? FileType.Directory : FileType.File;
