...
export class ToggleRemoteConnectionsActionViewItem extends BaseActionViewItem {
	...
	constructor(
		...
		this._register(
			this._tunnelHostService.onDidChangeStatus(() => {
				this._updateState();
			}),
		);
	}
	...
	override render(container: HTMLElement): void {
		...
		// Icon
		this._iconElement = dom.append(
			this.element,
			dom.$("span.tunnel-host-icon"),
		);
		this._iconElement.append(
			...renderLabelWithIcons(`$(${Codicon.radioTower.id})`),
		);

		// Toast text (initially hidden)
		this._toastElement = dom.append(
			this.element,
			dom.$("span.tunnel-host-toast"),
		);
	...
	private _showToast(): void {
		...
		this._toastElement.textContent = localize(
			"tunnelHost.toast",
	...
	private _getHoverContent(): IManagedHoverContent {
		...
		if (this._tunnelHostService.isConnecting) {
			...
			lines.push(
				localize("tunnelHost.hover.idle", "Allow remote session access"),
			);
		}

		lines.push(
			`[${localize("tunnelHost.hover.showOutput", "Show Output")}](command:${SHOW_TUNNEL_HOST_OUTPUT_ID})`,
		);
