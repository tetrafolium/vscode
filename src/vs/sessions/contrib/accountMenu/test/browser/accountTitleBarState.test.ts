...
suite("Sessions - Account Title Bar State", () => {
	...
	function createState(
		overrides: Partial<IAccountTitleBarStateContext> = {},
	): IAccountTitleBarStateContext {
		return {
	...
	test("falls back to the codicon when no GitHub profile image URL is available", () => {
		assert.strictEqual(
			getAccountProfileImageUrl(undefined, "octocat"),
			undefined,
		);
		assert.strictEqual(
			getAccountProfileImageUrl("github-enterprise", "octocat"),
			undefined,
		);
		assert.strictEqual(
			getAccountProfileImageUrl("github", undefined),
			undefined,
		);
	});
});
