...
import {
	IJSONEditingService,
	IJSONValue,
} from "../../../../../workbench/services/configuration/common/jsonEditing.js";
...
import {
	...
	ITaskEntry,
} from "../../browser/sessionsTasksService.js";
...
} from "../../browser/sessionTaskRunner.js";

function makeSession(
	opts: { repository?: URI; worktree?: URI } = {},
): ISession {
	...
	const session = {
		...
		workspace: observableValue(
			"workspace",
			workspace as ISessionWorkspace | undefined,
		),
		title: chat.title,
...
suite("SessionsTasksService", () => {
	...
	setup(() => {
		...
		service = store.add(
			instantiationService.createInstance(SessionsTasksService),
		);
	});
	...
	test("getSessionTasks returns tasks with inAgents: true from worktree", async () => {
		...
		const userTasksUri = URI.from({
			scheme: userSettingsUri.scheme,
		...
		assert.deepStrictEqual(
			tasks.map((t) => t.task.label),
			["build", "test", "watch", "gulp-task"],
		);
	});
	...
	test("getSessionTasks does not re-read files on repeated calls for the same folder", async () => {
		...
		fileContents.set(
			worktreeTasksUri.toString(),
			tasksJsonContent([makeTask("build", "npm run build", true)]),
		);
		fileContents.set(userTasksUri.toString(), tasksJsonContent([]));
		...
		assert.strictEqual(
			...
			2,
			"should read files only once (no duplicate refresh)",
		);
	});
	...
	test("getSessionTasks skips workspace tasks when repository URI has no path", async () => {
		...
		fileContents.set(
			userTasksUri.toString(),
			tasksJsonContent([makeTask("userTask", "npm run user", true)]),
		);

		const session = makeSession({
			repository: URI.parse("unknown://workspace"),
		});
		const obs = service.getSessionTasks(session);

		await new Promise((r) => setTimeout(r, 10));
		assert.deepStrictEqual(obs.get(), [
			{ task: makeTask("userTask", "npm run user", true), target: "user" },
		]);
	});
	...
	test("getNonSessionTasks returns only tasks without inAgents", async () => {
		...
		assert.deepStrictEqual(
			nonSessionTasks.map((t) => t.task.label),
			["lint", "test", "watch", "gulp-task"],
		);
	});
	...
	test("getNonSessionTasks preserves the source target for workspace and user tasks", async () => {
		...
		fileContents.set(
			worktreeTasksUri.toString(),
			tasksJsonContent([makeTask("workspaceTask", "npm run workspace")]),
		);
		fileContents.set(
			userTasksUri.toString(),
			tasksJsonContent([makeTask("userTask", "npm run user")]),
		);
		...
		assert.deepStrictEqual(nonSessionTasks, [
			{
				task: {
					label: "workspaceTask",
					type: "shell",
					command: "npm run workspace",
				},
				target: "workspace",
			},
			{
				task: { label: "userTask", type: "shell", command: "npm run user" },
				target: "user",
			},
	...
	test("user task operations are skipped when user settings URI has no path", async () => {
		...
		const session = makeSession({ repository: repoUri });
		const task = await service.createAndAddTask(
			undefined,
			"npm run dev",
			session,
			"user",
		...
		assert.deepStrictEqual(
			{ task, nonSessionTasks, jsonEdits },
			{ task: undefined, nonSessionTasks: [], jsonEdits: [] },
		);
	});
	...
	test("addTaskToSessions writes inAgents: true to the matching task index", async () => {
		...
		assert.deepStrictEqual(jsonEdits[0].values, [
			{ path: ["tasks", 1, "inAgents"], value: true },
		]);
	});
	...
	test("addTaskToSessions does nothing when task label not found", async () => {
		...
		fileContents.set(
			worktreeTasksUri.toString(),
			tasksJsonContent([makeTask("build", "npm run build")]),
		);

		const session = makeSession({ worktree: worktreeUri, repository: repoUri });
		await service.addTaskToSessions(
			makeTask("nonexistent"),
			session,
			"workspace",
		);
	...
	test("createAndAddTask writes new task with inAgents: true", async () => {
		...
		fileContents.set(
			worktreeTasksUri.toString(),
			tasksJsonContent([makeTask("existing", "echo hi")]),
		);

		const session = makeSession({ worktree: worktreeUri, repository: repoUri });
		await service.createAndAddTask(
			undefined,
			"npm run dev",
			session,
			"workspace",
	...
	test("updateTask moves a task between workspace and user storage", async () => {
		...
		fileContents.set(
			userTasksUri.toString(),
			tasksJsonContent([makeTask("userExisting", "npm run user", true)]),
		);
	...
	test("updateTask keeps the pinned task in sync when the label changes", async () => {
		...
		assert.strictEqual(
			service.getPinnedTaskLabel(repoUri).get(),
			"build:watch",
		);
	});
