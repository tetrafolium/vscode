...
suite("resolveTaskCommand", () => {
	...
	test("os override replaces command and args", () => {
		...
		assert.strictEqual(
			resolveTaskCommand(task, { targetOS: "windows" }),
			"dir /B",
		);
		assert.strictEqual(
			resolveTaskCommand(task, { targetOS: "linux" }),
			"ls -la",
		);
	...
	function lookupFrom(
		...tasks: ITaskEntry[]
	): (label: string) => ITaskEntry | undefined {
	...
	test("nested dependencies resolve recursively", () => {
		const leaf: ITaskEntry = {
			label: "leaf",
			type: "shell",
			command: "echo",
			args: ["leaf"],
		};
		const mid: ITaskEntry = {
			label: "mid",
			type: "shell",
			command: "echo",
			args: ["mid"],
