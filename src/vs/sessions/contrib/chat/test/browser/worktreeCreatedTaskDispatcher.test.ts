...
} from "../../browser/worktreeCreatedTaskDispatcher.js";
...
function makeWorkspace(hasWorktree: boolean): ISessionWorkspace {
	...
	const workTreeUri = hasWorktree
		? URI.parse("file:///repo-worktree")
		: undefined;
	return {
		uri: root,
		label: "repo",
...
function makeSession(
	opts: {
		id?: string;
		providerId?: string;
		runsWorktreeCreatedTasks?: boolean;
		loading?: boolean;
	...
	const status = observableValue(
		"status",
		opts.status ?? SessionStatus.InProgress,
	);
	const workspace = observableValue<ISessionWorkspace | undefined>(
		"workspace",
		makeWorkspace(opts.hasWorktree ?? true),
	);
	...
	const session: ISession = {
		...
		capabilities: {
			...
			runsWorktreeCreatedTasks: opts.runsWorktreeCreatedTasks,
		},
	};
...
function entry(
	label: string,
	runOn?: "worktreeCreated" | "folderOpen" | "default",
): ISessionTaskWithTarget {
...
suite("WorktreeCreatedTaskDispatcher", () => {
	...
	function createDispatcher(): WorktreeCreatedTaskDispatcher {
		...
		instantiationService.stub(
			ISessionsTasksService,
			tasks as unknown as ISessionsTasksService,
		);
		instantiationService.stub(
			ISessionsManagementService,
			mgmt as unknown as ISessionsManagementService,
		);
		instantiationService.stub(IConfigurationService, configurationService);
		instantiationService.stub(ILogService, new NullLogService());
		return store.add(
			instantiationService.createInstance(WorktreeCreatedTaskDispatcher),
		);
	}
	...
	test("does not run for sessions only reported via onDidChangeSessions.added", async () => {
		...
		mgmt.sessionsChangedEmitter.fire({
			added: [session],
			removed: [],
			changed: [],
		});
	...
	test("waits for untitled sessions to start before running", async () => {
		createDispatcher();
		const { session, status } = makeSession({
			id: "a",
