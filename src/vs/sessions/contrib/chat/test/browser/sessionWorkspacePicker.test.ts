...
function seedStorage(
	...
	entries: { uri: URI; providerId: string; checked: boolean }[],
): void {
...
function assertSelectedProvider(
	picker: WorkspacePicker,
	expectedProviderId: string | undefined,
	message?: string,
): void {
	assert.strictEqual(
		picker.selectedResolved?.providerId,
		expectedProviderId,
		message,
	);
}
...
suite("WorkspacePicker - Connection Status", () => {
	...
	test("restore picks checked entry even when remote is disconnected (before grace period)", () => {
		...
		const remoteStatus = observableValue<RemoteAgentHostConnectionStatus>(
			"status",
	...
	test("restored remote that never connects falls back after grace period", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assertSelectedProvider(
				picker,
				"agenthost-remote-1",
				"Selection is restored synchronously",
			);
			...
			assertSelectedProvider(
				picker,
				undefined,
				"Selection cleared after grace period",
			);
			assert.deepStrictEqual(
				events,
				[undefined],
				"onDidSelectWorkspace fired with undefined",
			);
		}));

	test("restored remote that connects within grace period keeps selection", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const remoteStatus = observableValue<RemoteAgentHostConnectionStatus>(
				"status",
				RemoteAgentHostConnectionStatus.disconnected,
			...
			assertSelectedProvider(
				picker,
				"agenthost-remote-1",
				"Selection preserved after successful connect",
			);
		}));

	test("user pick during connect cancels the fallback", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			picker.setSelectedWorkspace(URI.file("/local/picked"), {
				fireEvent: false,
			});
			...
			assertSelectedProvider(
				picker,
				"local-1",
				"User pick preserved across grace-period elapse",
			);
		}));
	...
	test("connecting provider that fails falls back to no selection", () => {
		...
		assertSelectedProvider(
			picker,
			"agenthost-remote-1",
			"Selection is restored while connecting",
		);
		...
		assertSelectedProvider(
			picker,
			"agenthost-remote-1",
			"Selection preserved while connecting",
		);
		...
		assertSelectedProvider(
			picker,
			undefined,
			"Selection cleared after connection failure",
		);
		assert.deepStrictEqual(
			events,
			[undefined],
	...
	test("disconnect preserves selection (renders grayed; no auto-clear)", () => {
		...
		assertSelectedProvider(
			picker,
			"agenthost-remote-1",
			"Selection should be preserved on disconnect",
		);
	});
	...
	test("checked is globally unique after persist", () => {
		...
		const resolvedWorkspace = localProvider.resolveWorkspace(
			URI.file("/local/project"),
		);
		assert.ok(
			resolvedWorkspace,
			"resolveWorkspace should resolve file:// URIs",
		);
		picker.setSelectedWorkspace(URI.file("/local/project"), {
			fireEvent: false,
		});
		...
		assert.strictEqual(
			...
			1,
			"Only one entry should be checked",
		);
		assert.strictEqual(
			checkedEntries[0].uri.path,
			"/local/project",
			"The local entry should be checked",
		);
	});
	...
	test("local provider is never treated as unavailable", () => {
		...
		assertSelectedProvider(
			picker,
			"local-1",
			"Local provider workspace should always be selectable",
		);
	});
	...
	test("restore picks the stored workspace when its provider registers after another provider", () => {
		...
		seedStorage(storage, [
			{
				uri: URI.file("/copilot/old-project"),
				providerId: "default-copilot",
				checked: false,
			},
			{
				uri: URI.file("/agent-host/project"),
		...
		assertSelectedProvider(
			picker,
			"local-agent-host",
			"Stored workspace should be restored once its provider registers",
		);
	});
	...
	test("late-registering provider does not move selection out from under user", () => {
		...
		assertSelectedProvider(
			picker,
			undefined,
			"No fallback while checked entry pending",
		);

		// User explicitly picks a Copilot workspace.
		picker.setSelectedWorkspace(URI.file("/copilot/picked"), {
			fireEvent: false,
		});
		...
		assertSelectedProvider(
			picker,
			"default-copilot",
			"User selection is preserved across late provider registration",
		);
	});
});
...
suite("WorkspacePicker - Tab discovery", () => {
	...
	test("hides Remote group when remote agent hosts are disabled", () => {
		providersService.setProviders([
			createMockProvider("p1", {
				browseActions: [makeBrowseAction("p1", SESSION_WORKSPACE_GROUP_REMOTE)],
			}),
		]);
	...
	test("orders well-known groups Local first, then alphabetical", () => {
		providersService.setProviders([
			createMockProvider("remote", {
				browseActions: [
					makeBrowseAction("remote", SESSION_WORKSPACE_GROUP_REMOTE),
				],
			}),
			createMockProvider("cloud", {
				browseActions: [makeBrowseAction("cloud", "Cloud")],
			}),
			createMockProvider("local", {
				browseActions: [
					makeBrowseAction("local", SESSION_WORKSPACE_GROUP_LOCAL),
				],
			}),
		]);
		const picker = createTestablePicker(disposables, providersService);
	...
	test("deduplicates groups contributed by multiple providers / actions", () => {
		providersService.setProviders([
			createMockProvider("p1", {
				browseActions: [makeBrowseAction("p1", SESSION_WORKSPACE_GROUP_LOCAL)],
			}),
			createMockProvider("p2", {
				browseActions: [
					makeBrowseAction("p2", SESSION_WORKSPACE_GROUP_LOCAL),
					makeBrowseAction("p2", SESSION_WORKSPACE_GROUP_LOCAL),
				],
			}),
		]);
	...
	test("appends custom group labels after Local", () => {
		providersService.setProviders([
			createMockProvider("p1", {
				browseActions: [
					makeBrowseAction("p1", "Custom A"),
					makeBrowseAction("p1", SESSION_WORKSPACE_GROUP_LOCAL),
				],
			}),
			createMockProvider("p2", {
				browseActions: [
					makeBrowseAction("p2", "Custom B"),
					makeBrowseAction("p2", SESSION_WORKSPACE_GROUP_REMOTE),
				],
			}),
		]);
		const picker = createTestablePicker(disposables, providersService);
		...
		assert.deepStrictEqual(tabs.slice(1).sort(), [
			"Custom A",
			"Custom B",
			SESSION_WORKSPACE_GROUP_REMOTE,
		]);
	...
	test("discovers groups from recent workspaces does not add extra tabs", () => {
		...
		const storage = disposables.add(new TestStorageService());
		seedStorage(storage, [
			{ uri: URI.file("/repo"), providerId: "p1", checked: false },
		]);
		...
		const picker = disposables.add(
			instantiationService.createInstance(TestablePicker),
		);
