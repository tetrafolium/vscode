 ...
 */
export class AgentHostInputCompletionHandler extends AgentHostInputCompletionsBase<
	void,
	string
> {
	 ...
	 * attachment chip.
	 */
	private readonly _insertedTexts = new Map<
		string /* uri */,
		string /* insertText */
	>();
	...
	private async _registerForScheme(scheme: string): Promise<void> {
		const triggerCharacters =
			await this._chatSessionsService.getChatInputCompletionTriggerCharacters(
				scheme,
			);
		...
		if (
			!activeSession ||
			getChatSessionType(activeSession.resource) !== scheme
		) {
			return;
		}
	...
	protected override _resolveContext(
		_model: ITextModel,
		scheme: string,
	): { sessionResource: URI; context: void } | undefined {
	...
	protected override _buildItem(
		position: Position,
		item: IChatInputCompletionItem,
	): CompletionItem {
		const replaceRange = AgentHostInputCompletionHandler.computeRange(
			position,
			item,
		);
		...
		switch (attachment.kind) {
			...
			default: {
				...
				const kind = attachment.isDirectory
	...
	private _registerDecorations(): void {
		if (!AgentHostInputCompletionHandler._decosRegistered) {
			...
			this._codeEditorService.registerDecorationType(
				"sessions-chat",
				AgentHostInputCompletionHandler._decoType,
				{
	...
	private _updateDecorations(): void {
		...
		const attachedIds = new Set(
			this._contextAttachments.attachments.map((a) => a.id),
		);
