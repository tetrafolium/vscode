...
export class RepoPicker extends Disposable {
	...
	render(container: HTMLElement): HTMLElement {
		...
		this._renderDisposables.add(
			dom.addDisposableListener(trigger, dom.EventType.CLICK, (e) => {
	...
	showPicker(): void {
		...
		this.actionWidgetService.show<IRepoItem>(
			...
			[],
			{
				...
				getWidgetAriaLabel: () =>
					localize("repoPicker.ariaLabel", "Repository Picker"),
			},
			showFilter
				? {
						showFilter: true,
	...
	private async _browseForRepo(): Promise<void> {
		try {
			const result: string | undefined =
				await this.commandService.executeCommand(OPEN_REPO_COMMAND);
			if (result) {
	...
	private _buildItems(): IActionListItem<IRepoItem>[] {
		...
		const dedupedRepos = this._recentlyPickedRepos.filter(
			(r) => !seenIds.has(r.id),
		);
	...
	private _removeRepo(repoId: string): void {
		this._recentlyPickedRepos = this._recentlyPickedRepos.filter(
			(r) => r.id !== repoId,
		);
		this.storageService.store(
			STORAGE_KEY_RECENT_REPOS,
	...
	private _updateTriggerLabel(): void {
		...
		const label =
			this._selectedRepo?.name ?? localize("pickRepo", "Pick Repository");
