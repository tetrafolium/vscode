 ...
 */
export const SESSIONS_EXECUTE_SLASH_COMMAND_ID =
	"sessions.chat.executeSlashCommand";

CommandsRegistry.registerCommand(
	SESSIONS_EXECUTE_SLASH_COMMAND_ID,
	(_, handler: SlashCommandHandler, slashCommandStr: string) => {
...
export class SlashCommandHandler extends Disposable {
	...
	constructor(
		...
		private readonly aiCustomizationWorkspaceService: IAICustomizationWorkspaceService,
	...
	private _refreshPromptCommands(): void {
		this.aiCustomizationWorkspaceService
			.getFilteredPromptSlashCommands(CancellationToken.None)
			.then(
				(commands) => {
	...
	tryExecuteSlashCommand(query: string): boolean {
		...
		const slashCommand = this._slashCommands.find(
	...
	private _registerDecorations(): void {
		if (!SlashCommandHandler._slashDecosRegistered) {
			...
			this.codeEditorService.registerDecorationType(
				"sessions-chat",
				SlashCommandHandler._slashDecoType,
				{
			...
			this.codeEditorService.registerDecorationType(
				"sessions-chat",
				SlashCommandHandler._slashPlaceholderDecoType,
				{},
			);
		}
	...
	private _updateDecorations(): void {
		...
		const promptCommand = this._cachedPromptCommands.find(
			...
			this.languageFeaturesService.completionProvider.register(
				{ scheme: uri.scheme, hasAccessToAllModels: true },
				{
					...
					provideCompletionItems: (
						...
						const range = this._computeCompletionRanges(
							model,
							position,
							/\/\w*/g,
						);
							...
							suggestions: this._slashCommands.map((c, i): CompletionItem => {
								...
								return {
									...
									command: c.executeImmediately
										? {
												id: SESSIONS_EXECUTE_SLASH_COMMAND_ID,
												title: withSlash,
												arguments: [this, withSlash],
											}
										: undefined,
								};
							}),
				...
				{ scheme: uri.scheme, hasAccessToAllModels: true },
				{
					...
					provideCompletionItems: async (
						...
						const promptCommands =
							await this.aiCustomizationWorkspaceService.getFilteredPromptSlashCommands(
								token,
							);
