...
class CustomizationsDebugLogContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		@IAICustomizationWorkspaceService
		private readonly _workspaceService: IAICustomizationWorkspaceService,
		...
		this._logger = this._register(
			loggerService.createLogger("customizationsDebug", {
				name: "Customizations Debug",
			}),
		);

		this._register(
			this._promptsService.onDidChangeCustomAgents(() => this._logSnapshot()),
		);
		this._register(
			this._promptsService.onDidChangeSlashCommands(() => this._logSnapshot()),
		);
		this._register(
	...
	private async _doLogSnapshot(): Promise<void> {
		const root =
			this._workspaceService.getActiveProjectRoot()?.fsPath ?? "(none)";

		this._logger.info("");
		this._logger.info("=== Customizations Snapshot ===");
		this._logger.info(`  Root: ${root}`);
		this._logger.info(
			`  Sections: ${this._workspaceService.managementSections.join(", ")}`,
		);
		this._logger.info("");

		// Header
		this._logger.info(
			`  ${"Section".padEnd(16)} ${"Local".padStart(6)} ${"User".padStart(6)} ${"Ext".padStart(6)} ${"Total".padStart(7)}`,
		);
		this._logger.info(
			`  ${"--------".padEnd(16)} ${"-----".padStart(6)} ${"----".padStart(6)} ${"---".padStart(6)} ${"-----".padStart(7)}`,
		);
	...
	private _logMcpServers(): void {
		...
		for (const server of servers) {
			...
			this._logger.info(
				`     ${server.definition.label} [${stateStr}] id=${server.definition.id}`,
			);
		}
		this._logger.info("");
	}

	private async _logSectionRow(
		section: AICustomizationManagementSection,
		type: PromptsType,
		...
		try {
			const [localFiles, userFiles, extensionFiles] = await Promise.all([
				this._promptsService.listPromptFilesForStorage(
					type,
			...
			const local = filtered.filter(
				(f) => f.storage === PromptsStorage.local,
			).length;
			const user = filtered.filter(
				(f) => f.storage === PromptsStorage.user,
			).length;
			const ext = filtered.filter(
				(f) => f.storage === PromptsStorage.extension,
			).length;

			this._logger.info(
				`  ${section.padEnd(16)} ${String(local).padStart(6)} ${String(user).padStart(6)} ${String(ext).padStart(6)} ${String(filtered.length).padStart(7)}`,
			);
		} catch {
	...
	private async _logSectionDetails(
		...
		try {
			...
			if (filtered.length > 0) {
				...
				this._logger.info(
					`     Filter: sources=[${filter.sources.join(", ")}]${filter.includedUserFileRoots ? `, roots=[${filter.includedUserFileRoots.map((r) => r.fsPath).join(", ")}]` : ""}`,
				);
