...
import {
	imageToHash,
	isImage,
} from "../../../../workbench/contrib/chat/browser/widget/input/editor/chatPasteProviders.js";
import {
	CodeDataTransfers,
	containsDragType,
...
export class NewChatContextAttachments extends Disposable {
	...
	private _updateRendering(): void {
		...
		for (const entry of this._attachedContext) {
			const pill = dom.append(
				this._container,
				dom.$(".sessions-chat-attachment-pill"),
			);
			pill.tabIndex = 0;
			pill.role = "button";
			const resource = URI.isUri(entry.value)
				? entry.value
			...
			if (entry.kind === "image") {
				...
				dom.append(
					pill,
					dom.$("span.sessions-chat-attachment-name", undefined, entry.name),
				);
			} else {
				...
				if (resource) {
					label.setFile(resource, {
						fileKind:
							entry.kind === "directory" ? FileKind.FOLDER : FileKind.FILE,
						hidePath: true,
					});
				} else {
			...
			const imageData =
				entry.kind === "image" ? coerceImageBuffer(entry.value) : undefined;
			if (imageData) {
				pill.style.cursor = "pointer";
				this._renderDisposables.add(
					registerOpenEditorListeners(pill, async () => {
						if (
							...
							const imageResource =
								resource ?? URI.from({ scheme: "data", path: entry.name });
							await this.chatImageCarouselService.openCarouselAtResource(
								imageResource,
								imageData,
							);
						} else if (resource) {
							await this.openerService.open(resource, {
			...
			const removeButton = dom.append(
				pill,
				dom.$(".sessions-chat-attachment-remove"),
			);
	...
	registerDropTarget(dndContainer: HTMLElement): void {
		...
		this._register(
			new DragAndDropObserver(dndContainer, {
				onDragOver: (e) => {
				...
				onDrop: async (e) => {
					...
					if (editorDropData.length > 0) {
						for (const editor of editorDropData) {
							if (editor.resource) {
								await this._attachFileUri(
									editor.resource,
									basename(editor.resource),
								);
							}
						}
						return;
				...
				async (e: ClipboardEvent) => {
					...
					for (const item of Array.from(items)) {
						if (
							...
							!supportedMimeTypes.includes(item.type)
						) {
							continue;
						}
						const file = item.getAsFile();
	...
	showPicker(folderUri?: URI): void {
		...
		if (folderUri) {
			...
			const runSearch = (filePattern?: string) => {
				...
				picker.busy = true;
				this._collectFilePicks(folderUri, filePattern, token).then(
					(filePicks) => {
	...
	private async _collectFilePicks(
		rootUri: URI,
		filePattern?: string,
		...
		if (
			rootUri.scheme === Schemas.file ||
			rootUri.scheme === Schemas.vscodeRemote
		) {
			return this._collectFilePicksViaSearch(
				rootUri,
				maxFiles,
				filePattern,
				token,
	...
	private async _collectFilePicksViaSearch(
		rootUri: URI,
		maxFiles: number,
		filePattern?: string,
		token?: CancellationToken,
			...
			return searchResult.results.map(
				(result) =>
					({
						...
						description: this.labelService.getUriLabel(result.resource, {
							relative: true,
						}),
	...
	private async _collectFilePicksViaFileService(
		rootUri: URI,
		maxFiles: number,
		filePattern?: string,
	): Promise<IQuickPickItem[]> {
			...
			try {
				...
				for (const child of children) {
					...
					if (child.isDirectory) {
						...
						if (
							patternLower &&
							!child.name.toLowerCase().includes(patternLower)
						) {
							continue;
						}
						picks.push({
							label: child.name,
							description: this.labelService.getUriLabel(child.resource, {
								relative: true,
							}),
							iconClasses: getIconClasses(
								this.modelService,
