...
function computeRange(
	...
	position: Position,
	reg: RegExp,
): ICompletionRangeResult | undefined {
...
export class VariableCompletionHandler extends Disposable {
	...
	private _registerFileCompletions(): void {
		...
		this._register(
			this.languageFeaturesService.completionProvider.register(
				{ scheme: uri.scheme, hasAccessToAllModels: true },
				{
					...
					provideCompletionItems: async (
						...
						const range = computeRange(
							model,
							position,
							VariableCompletionHandler._wordPattern,
						);
						if (!range) {
							return null;
						}
						...
						await this._addFileAndFolderEntries(
							workspaceUri,
							result,
							range,
							token,
	...
	private async _addFileAndFolderEntries(
		workspaceUri: URI,
		result: CompletionList,
		info: ICompletionRangeResult,
		token: CancellationToken,
	): Promise<void> {
		const makeItem = (
			resource: URI,
		...
		if (
			...
			workspaceUri.scheme === Schemas.vscodeRemote
		) {
			await this._addEntriesViaSearch(
				workspaceUri,
				pattern,
				seen,
				makeItem,
			...
			await this._addEntriesViaFileService(
				workspaceUri,
				pattern,
				seen,
				makeItem,
	...
	private async _addEntriesViaSearch(
		...
		try {
			const { files, folders } = await searchFilesAndFolders(
				workspaceUri,
				pattern || "",
				true,
				token,
	...
	private async _addEntriesViaFileService(
		...
		const collect = async (uri: URI, depth: number): Promise<void> => {
			if (
				result.suggestions.length >= maxResults ||
				depth > maxDepth ||
					...
					if (child.isDirectory) {
						...
						if (!seen.has(child.resource)) {
							...
							if (!patternLower || folderName.includes(patternLower)) {
								...
								result.suggestions.push(
									makeItem(child.resource, FileKind.FOLDER),
								);
							}
						}
						...
						if (!seen.has(child.resource)) {
							...
							if (!patternLower || fileName.includes(patternLower)) {
								seen.add(child.resource);
								result.suggestions.push(
									makeItem(child.resource, FileKind.FILE),
								);
							}
						}
					}
	...
	private _registerDecorations(): void {
		if (!VariableCompletionHandler._decosRegistered) {
			...
			this.codeEditorService.registerDecorationType(
				"sessions-chat",
				VariableCompletionHandler._decoType,
				{
		...
		this._register(
			this._editor.onDidChangeModelContent(() => this._updateDecorations()),
		);
