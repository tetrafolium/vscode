...
export class NoAgentHostEmptyState extends Disposable {
	...
	render(parent: HTMLElement): void {
		...
		this._root.setAttribute(
			"aria-label",
			localize("noAgentHost.aria", "No agent hosts available"),
		);
		...
		const cliBinary =
			this._productService.quality === "stable" ? "code" : "code-insiders";
		const command = `${cliBinary} tunnel`;

		const description = dom.append(
			this._root,
			$("p.no-agent-host-description"),
		);
		renderFormattedText(
			localize(
		...
		const learnMore = dom.append(
			description,
			$("a.no-agent-host-link"),
		) as HTMLAnchorElement;
		learnMore.textContent = localize("noAgentHost.learnMore", "Learn more");
		learnMore.href = LEARN_MORE_URL;
		this._register(
			dom.addDisposableListener(learnMore, dom.EventType.CLICK, (e) => {
				e.preventDefault();
