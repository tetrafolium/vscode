	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const handleSelectAgentsFolder = (_: unknown, ...args: unknown[]) => {
			...
			const preferredSessionType =
				rawPref && typeof rawPref.sessionTypeId === "string"
					? {
							sessionTypeId: rawPref.sessionTypeId,
							providerId:
								typeof rawPref.providerId === "string"
									? rawPref.providerId
									: undefined,
						}
					: undefined;
			this.logService.info(
				`[AgentsHandoff] IPC received: folderUri=${folderUri?.toString() ?? "(none)"} initialQuery=${initialQuery ? "yes" : "no"} sessionResource=${sessionResource?.toString() ?? "(none)"} preferredSessionType=${preferredSessionType?.sessionTypeId ?? "(none)"}`,
			);
			...
			const preferredOnly =
				!!preferredSessionType && !sessionResource && !initialQuery;

			this.handleOpenIntent(folderUri, initialQuery, sessionResource).catch(
				(err) =>
					this.logService.error("[AgentsHandoff] handleOpenIntent failed", err),
			);
				...
				this.lifecycleService
					...
					.catch((err) =>
						this.logService.error(
							"[AgentsHandoff] preferred-only unsetNewSession failed",
							err,
						),
					);
			}
	...
	private async openExistingSession(sessionResource: URI): Promise<void> {
		this.logService.info(
			`[AgentsHandoff] openExistingSession: target=${sessionResource.toString()}`,
		);
		...
		if (!found) {
			this.logService.warn(
				`[AgentsHandoff] target session never appeared in providers; aborting`,
			);
			return;
		}
		...
		for (let attempt = 0; attempt < 6; attempt++) {
			try {
				...
				this.logService.warn(
					`[AgentsHandoff] openSession attempt ${attempt} resolved but active session is ${active?.resource.toString() ?? "(none)"}; retrying`,
				);
			} catch (err) {
				const message = err instanceof Error ? err.message : String(err);
				this.logService.warn(
					`[AgentsHandoff] openSession attempt ${attempt} failed: ${message}`,
				);
				const retryable =
					/canceled/i.test(message) || /not found/i.test(message);
				if (!retryable) {
					return;
				}
			}
	...
	private async waitForSessionAvailable(
		sessionResource: URI,
		timeoutMs = 15_000,
	): Promise<boolean> {
		...
		return new Promise<boolean>((resolve) => {
			...
			const timer = setTimeout(
				() =>
					done(!!this.sessionsManagementService.getSession(sessionResource)),
				timeoutMs,
			);
	...
	private async submitInitialQuery(query: string): Promise<void> {
		...
		const view = this.sessionsPartService.getSessionView(
		...
		const settledView = this.sessionsPartService.getSessionView(
			this.sessionsManagementService.activeSession.get()?.sessionId,
		);
		settledView?.sendQuery(query);
	}

	private waitForStableActiveSession(
		timeoutMs = 20_000,
		stableMs = 2_500,
	): Promise<boolean> {
		return new Promise<boolean>((resolve) => {
			...
			store.add(
				autorun((reader) => {
					const id =
						this.sessionsManagementService.activeSession.read(
							reader,
						)?.sessionId;
					if (id !== lastSeenId) {
						lastSeenId = id;
	...
	private tryResolveAndSelect(folderUri: URI): boolean {
		...
		if (
			...
			this.sessionsPartService
				.getSessionView(activeSession?.sessionId)
				?.selectWorkspace(folderUri, resolved.providerId);
		}
		return true;
	}
}
