...
} from "../../../../../workbench/test/browser/workbenchTestServices.js";
import {
	TestContextService,
	TestStorageService,
} from "../../../../../workbench/test/common/workbenchTestServices.js";
...
suite("ChangesEditorLabels", () => {
	...
	test("labels are derived from the display file URI, not the backing git blob URI", () => {
		...
		const displayUri = toAgentHostUri(
			URI.file("/workspaces/demo/src/app.ts"),
			"remotehost",
		);
		const backingUri = toAgentHostUri(
			URI.from({
				scheme: "git-blob",
