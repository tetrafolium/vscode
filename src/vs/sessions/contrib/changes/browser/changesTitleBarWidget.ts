...
const TOGGLE_CHANGES_VIEW_ID =
	"workbench.action.agentSessions.toggleChangesView";
const TOGGLE_SECONDARY_SIDEBAR_TOOLTIP = localize(
	"toggleSecondarySidebarTooltip",
	"Toggle Secondary Side Bar Visibility",
);

const secondarySidebarToggleClosedIcon = registerIcon(
	"agent-secondary-sidebar-toggle-closed",
...
const secondarySidebarToggleOpenIcon = registerIcon(
	"agent-secondary-sidebar-toggle-open",
 ...
 */
export class ChangesTitleBarContribution
	extends Disposable
...
registerAction2(
	class extends Action2 {
		...
		run(accessor: ServicesAccessor): void {
			...
			if (isVisible) {
				// Editor part
				const hasEditors = editorGroupService.groups.some(
					(group) => !group.isEmpty,
				);
				if (
					hasEditors &&
