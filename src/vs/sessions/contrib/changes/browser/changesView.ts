...
import {
	...
	getChangesMultiDiffSourceUri,
} from "./changesMultiDiffSourceResolver.js";
...
} from "../../../common/sessionsTelemetry.js";
...
} from "../../providers/agentHost/browser/agentHostSkillButtons.js";
import {
...
import {
	...
	ChangesTreeRenderer,
	IChangesFileItem,
	IChangesTreeRootInfo,
	isChangesFileItem,
	toIChangesFileItem,
} from "./changesViewRenderer.js";
...
class ChangesButtonBarWidget extends Disposable {
	constructor(
		...
		const outgoingChangesObs = derivedObservableWithCache<number | undefined>(
			this,
			(reader, lastValue) => {
				const activeSessionState = viewModel.activeSessionStateObs.read(reader);
				const hasGitOperationInProgress =
		...
		const reviewStateObs = derivedOpts<{
			isLoading: boolean;
			...
			const prReviewState = codeReviewService
				.getPRReviewState(sessionResource)
				.read(reader);
			const prReviewCommentCount =
				prReviewState.kind === PRReviewStateKind.Loaded
			...
			if (sessionChanges && sessionChanges.length > 0) {
				const reviewFiles =
					getCodeReviewFilesFromSessionChanges(sessionChanges);
				const reviewVersion = getCodeReviewVersion(reviewFiles);
				const reviewState = codeReviewService
					.getReviewState(sessionResource)
					.read(reader);

				if (
					reviewState.kind === CodeReviewStateKind.Loading &&
					reviewState.version === reviewVersion
				) {
					isLoading = true;
				} else {
					const codeReviewCommentCount =
						reviewState.kind === CodeReviewStateKind.Result &&
						reviewState.version === reviewVersion
							? reviewState.comments.length
							: 0;
					const totalReviewCommentCount =
						codeReviewCommentCount + prReviewCommentCount;
		...
		const runningLabelObs = observableValue<
	...
	private _getButtonConfiguration(
		action: IAction,
		outgoingChanges: number,
		reviewState: { isLoading: boolean; commentCount: number | undefined },
		hasGitOperationInProgress: boolean,
		runningLabelObs: IObservable<string | IMarkdownString | undefined>,
	):
		| {
				showIcon: boolean;
				showLabel: boolean;
		...
		if (
			...
			return {
				...
				showLabel: true,
				isSecondary: false,
				customLabel: `$(loading) ${labelWithCount}`,
			};
		}
		if (
		...
		if (action.id === RUN_SESSION_CODE_REVIEW_ACTION_ID) {
			if (reviewState.isLoading) {
				return {
					...
					showLabel: true,
					isSecondary: true,
					customLabel: "$(loading~spin)",
					customClass: "code-review-loading",
				};
			}
			if (reviewState.commentCount !== undefined) {
				return {
					...
					showLabel: true,
					isSecondary: true,
					customLabel: String(reviewState.commentCount),
					customClass: "code-review-comments",
				};
			}
...
export class ChangesViewPane extends ViewPane {
	...
	constructor(
		...
		super(
			{ ...options, titleMenuId: MenuId.ChatEditingSessionTitleToolbar },
			keybindingService,
			contextMenuService,
		...
		const changesMultiDiffSourceResolver =
		...
		// Context keys
		this.isMergeBaseBranchProtectedContextKey =
			ActiveSessionContextKeys.IsMergeBaseBranchProtected.bindTo(
				this.scopedContextKeyService,
			);
		this.isolationModeContextKey =
			ActiveSessionContextKeys.IsolationMode.bindTo(
				this.scopedContextKeyService,
			);
		this.hasGitRepositoryContextKey =
			ActiveSessionContextKeys.HasGitRepository.bindTo(
				this.scopedContextKeyService,
			);
		this.hasUpstreamContextKey = ActiveSessionContextKeys.HasUpstream.bindTo(
			this.scopedContextKeyService,
		);
		...
		this.hasOutgoingChangesContextKey =
			ActiveSessionContextKeys.HasOutgoingChanges.bindTo(
				this.scopedContextKeyService,
			);
		this.hasUncommittedChangesContextKey =
			ActiveSessionContextKeys.HasUncommittedChanges.bindTo(
				this.scopedContextKeyService,
			);
		this.hasBranchChangesContextKey =
			ActiveSessionContextKeys.HasBranchChanges.bindTo(
				this.scopedContextKeyService,
			);
		this.hasGitHubRemoteContextKey =
			ActiveSessionContextKeys.HasGitHubRemote.bindTo(
				this.scopedContextKeyService,
			);
		this.hasPullRequestContextKey =
			ActiveSessionContextKeys.HasPullRequest.bindTo(
				this.scopedContextKeyService,
			);
		this.hasOpenPullRequestContextKey =
			ActiveSessionContextKeys.HasOpenPullRequest.bindTo(
				this.scopedContextKeyService,
			);
		this.hasGitOperationInProgressContextKey =
			ActiveSessionContextKeys.HasGitOperationInProgress.bindTo(
				this.scopedContextKeyService,
			);
		...
		this._register(
			bindContextKey(
				ChangesContextKeys.VersionMode,
				this.scopedContextKeyService,
				(reader) => {
					return (
						this.viewModel.activeSessionChangesetObs.read(reader)?.id ?? ""
					);
				},
			),
		);
		...
		this._register(
			bindContextKey(
				ChatContextKeys.agentSessionType,
				this.scopedContextKeyService,
				(reader) => {
		...
		const hasGitOperationInProgressGlobalContextObs = observableFromEvent(
		...
		this.hasGitOperationInProgressObs = derived((reader) => {
			const hasGitOperationInProgressGlobalContext =
				hasGitOperationInProgressGlobalContextObs.read(reader);
			const hasGitOperationInProgressState =
				hasGitOperationInProgressStateObs.read(reader);
			...
			const contextKeyValue =
				hasGitOperationInProgressGlobalContext === true
		...
		const scopedServiceCollection = new ServiceCollection([
			IContextKeyService,
			this.scopedContextKeyService,
		]);
	...
	protected override renderBody(container: HTMLElement): void {
		...
		const updateHasFileIcons = () => {
			this.contentContainer!.classList.toggle(
				"has-file-icons",
				this.themeService.getFileIconTheme().hasFileIcons,
			);
		};
		updateHasFileIcons();
		this._register(
			this.themeService.onDidFileIconThemeChange(updateHasFileIcons),
		);

		// Files header
		this.filesHeaderNode = dom.append(
			this.contentContainer,
			$(".changes-files-header"),
		);

		// Changesets toolbar
		const filesHeaderToolbarContainer = dom.append(
			this.filesHeaderNode,
			$(".changes-files-header-toolbar"),
		);
		this._register(
			this.scopedInstantiationService.createInstance(
				MenuWorkbenchToolBar,
				filesHeaderToolbarContainer,
				MenuId.ChatEditingSessionChangesFileHeaderToolbar,
				{
					menuOptions: { shouldForwardArgs: true },
					actionViewItemProvider: (action) => {
						if (
							action.id === "chatEditing.versionsPicker" &&
		...
		this.fileHeaderToolbarContainer = dom.append(
			this.filesHeaderNode,
			...
			this.scopedInstantiationService.createInstance(
				...
				MenuId.ChatEditingSessionChangesFileHeaderRightToolbar,
				{
					...
					actionViewItemProvider: (action, options) => {
						if (
							action.id === ChangesDiffStatsAction.ID &&
							action instanceof MenuItemAction
						) {
							return this.scopedInstantiationService.createInstance(
		...
		this.overviewContainer = dom.append(
			this.contentContainer,
			$(".chat-editing-session-overview"),
		);
		this.summaryContainer = dom.append(
			this.overviewContainer,
			$(".changes-summary"),
		);

		// Changes card progress bar
		const progressContainer = dom.append(
			this.contentContainer,
			$(".changes-progress"),
		);
		this.changesProgressBar = this._register(
			new ProgressBar(progressContainer, defaultProgressBarStyles),
		);
		...
		this.welcomeContainer = dom.append(
			this.contentContainer,
			$(".changes-welcome"),
		);
		...
		const welcomeIcon = dom.append(
			...
			$(".changes-welcome-icon"),
		);
		welcomeIcon.classList.add(
			...ThemeIcon.asClassNameArray(Codicon.diffMultiple),
		);
		...
		this.ciStatusWidget = this._register(
			this.scopedInstantiationService.createInstance(
				CIStatusWidget,
				this.splitViewContainer,
			),
		...
		const ciMinHeight =
			CIStatusWidget.HEADER_HEIGHT + CIStatusWidget.MIN_BODY_HEIGHT;
		...
		const ciPane: IView = {
			element: ciElement,
			get minimumSize() {
				return ciWidget.collapsed ? CIStatusWidget.HEADER_HEIGHT : ciMinHeight;
			},
			get maximumSize() {
				return ciWidget.collapsed
					? CIStatusWidget.HEADER_HEIGHT
					: Number.POSITIVE_INFINITY;
			},
			onDidChange: Event.map(
				this.ciStatusWidget.onDidChangeHeight,
				() => undefined,
			),
			layout: (height) => {
		...
		this.splitView.addView(
			ciPane,
			CIStatusWidget.HEADER_HEIGHT + CIStatusWidget.PREFERRED_BODY_HEIGHT,
			1,
			true,
		);
		...
		const updateSplitViewStyles = () => {
			const borderColor = this.themeService
				.getColorTheme()
				.getColor(PANEL_SECTION_BORDER);
			this.splitView!.style({
				separatorBorder: borderColor ?? Color.transparent,
			});
		};
		updateSplitViewStyles();
		this._register(
			this.themeService.onDidColorThemeChange(updateSplitViewStyles),
		);

		// Initially hide CI pane until checks arrive
		this.splitView.setViewVisible(1, false);

		let savedCIPaneHeight =
			CIStatusWidget.HEADER_HEIGHT + CIStatusWidget.PREFERRED_BODY_HEIGHT;
		this._register(
			this.ciStatusWidget.onDidToggleCollapsed((collapsed) => {
	...
	private onVisible(): void {
		...
		if (this.actionsContainer) {
			...
			this.renderDisposables.add(
				this.scopedInstantiationService.createInstance(
					...
					this.viewModel,
					this.hasGitOperationInProgressObs,
				),
			);
		}
		...
		this.renderDisposables.add(
			autorun((reader) => {
				...
				const hasGitRepository =
					this.viewModel.activeSessionHasGitRepositoryObs.read(reader);
				...
				dom.setVisibility(
					!isUntitled && (hasGitRepository || hasEntries),
					this.filesHeaderNode!,
				);
		...
		if (!this.tree && this.listContainer) {
			this.tree = this.createChangesTree(
				this.listContainer,
		...
		if (this.tree) {
			...
			this.renderDisposables.add(
				tree.onDidChangeContentHeight(() => this.layoutSplitView()),
			);
			...
			this.renderDisposables.add(
				tree.onDidOpen((e) => {
					...
					const modalEditorMode = this.configurationService.getValue<string>(
		...
		if (this.ciStatusWidget) {
			const checksViewModel =
				this.scopedInstantiationService.createInstance(ChecksViewModel);
		...
		this.renderDisposables.add(
			autorun((reader) => {
				...
				if (viewMode === ChangesViewMode.Tree) {
					...
					const listChildren = changes.map(
						(item) =>
							({
								element: item,
								collapsible: false,
	...
	private _bindContextKeys(
		topLevelStats: IObservable<{ files: number }>,
	): void {
			...
			bindContextKey(
				...
				(reader) => {
					const activeSessionStatus =
						this.sessionManagementService.activeSession
							.read(reader)
							?.status.read(reader);
					return (
						activeSessionStatus !== SessionStatus.Completed &&
						activeSessionStatus !== SessionStatus.Error
					);
				},
			),
		);
		...
		this.renderDisposables.add(
			autorun((reader) => {
				...
				this.logService.info(
					`[ChangesViewPane][_bindContextKeys] Context keys: ${JSON.stringify(state)}`,
				);
				...
				this.scopedContextKeyService.bufferChangeEvents(() => {
					...
					this.isMergeBaseBranchProtectedContextKey.set(
						state.isMergeBaseBranchProtected === true,
					);
					this.hasGitHubRemoteContextKey.set(state.hasGitHubRemote === true);
					this.hasPullRequestContextKey.set(state.hasPullRequest === true);
					this.hasOpenPullRequestContextKey.set(
						state.hasOpenPullRequest === true,
					);
					this.hasUpstreamContextKey.set(
						state.upstreamBranchName !== undefined,
					);
					this.hasIncomingChangesContextKey.set(
						state.incomingChanges !== undefined && state.incomingChanges > 0,
					);
					this.hasOutgoingChangesContextKey.set(
						state.outgoingChanges !== undefined && state.outgoingChanges > 0,
					);
					this.hasUncommittedChangesContextKey.set(
						state.uncommittedChanges !== undefined &&
							state.uncommittedChanges > 0,
					);
					this.hasBranchChangesContextKey.set(state.hasBranchChanges === true);
					this.hasGitOperationInProgressContextKey.set(
						state.hasGitOperationInProgress === true,
					);
				});
			}),
		);
	...
	private _layoutTreeInPane(paneHeight: number): void {
		...
		const treeHeight = Math.max(
			0,
			paneHeight - filesHeaderHeight - overviewHeight,
		);
	...
	private layoutSplitView(): void {
		...
		const availableHeight = Math.max(
			0,
			bodyHeight - bodyPadding - actionsHeight - actionsMargin,
		);
	...
	private getTreeRootInfo(
		items: readonly IChangesFileItem[],
	): IChangesTreeRootInfo | undefined {
		if (items.length === 0) {
			return undefined;
		}
	...
	private renderSidebarList(
		...
		const countBadge = disposables.add(
			new CountBadge(
				headerNode,
				{ count: items.length },
				defaultCountBadgeStyles,
		...
		const tree = this.createChangesTree(
			container,
			Event.None,
			disposables,
			() =>
				tree.getSelection().filter((item) => !!item && isChangesFileItem(item)),
		);

		if (viewMode === ChangesViewMode.Tree) {
			tree.setChildren(
				null,
				buildTreeChildren(items, this.getTreeRootInfo(items)),
			);
		} else {
			tree.setChildren(
				null,
				items.map((item) => ({
					element: item as ChangesTreeElement,
					collapsible: false,
				})),
			);
		}
			...
			tree.onDidOpen((e) => {
				if (e.element && isChangesFileItem(e.element) && !updatingSelection) {
					openFileItem(
						...
						!!e.editorOptions.preserveFocus,
						!!e.editorOptions.pinned,
						false /* preserve existing sidebar */,
					);
				}
			}),
		);
		...
		disposables.add(
			Event.runAndSubscribe(this.editorService.onDidActiveEditorChange, () => {
				...
				const index = items.findIndex(
					(i) =>
						(primaryResource !== undefined &&
							isEqual(i.uri, primaryResource)) ||
						(secondaryResource !== undefined &&
							i.originalUri !== undefined &&
							isEqual(i.originalUri, secondaryResource)),
				);
				if (index >= 0) {
	...
	private createChangesTree(
		...
		const resourceLabels = disposables.add(
			this.instantiationService.createInstance(ResourceLabels, {
				onDidChangeVisibility,
			}),
		);
		const actionRunner = disposables.add(
			new ChangesViewActionRunner(
			...
			this.instantiationService.createInstance(
				...
				[
					this.instantiationService.createInstance(
						...
						() => {
							...
							const activeSession =
								this.sessionManagementService.activeSession.get();
							const folder = activeSession?.workspace.get()?.folders[0];
				...
				],
				{
					...
					accessibilityProvider: {
						getAriaLabel: (element: ChangesTreeElement) =>
							isChangesFileItem(element) ? basename(element.uri) : element.name,
						getWidgetAriaLabel: () =>
					...
					indent:
						this.viewModel.viewModeObs.get() === ChangesViewMode.List ? 0 : 8,
	...
	async openChanges(resource?: URI): Promise<void> {
		...
		if (modalEditorMode === "all") {
			...
			const changeToOpen = resource
				? changes.find((c) => isEqual(c.uri, resource))
				: undefined;
			await this._openFileItem(
				changeToOpen ?? changes[0],
				changes,
				false,
				false,
				false,
		...
		const sidebar = includeSidebar
			? {
					render: (
						...
						return this.renderSidebarList(
							container as HTMLElement,
							onDidLayout,
							items,
							this._openFileItem.bind(this),
						);
					},
				}
			: undefined;
...
class ChangesPickerActionItem extends ActionWidgetDropdownActionViewItem {
	constructor(
		...
		super(
			action,
			{ actionProvider, listOptions: { detailItemHeight: 44 } },
			actionWidgetService,
			keybindingService,
			contextKeyService,
			telemetryService,
	...
	protected override renderLabel(element: HTMLElement): IDisposable | null {
		...
		dom.reset(
			element,
			dom.$("span", undefined, changeset.label),
...
class ChangesDiffStatsActionItem extends ActionViewItem {
	private readonly diffStatsObs: IObservable<
	...
	constructor(
		...
		const diffStatsRawObs = derivedObservableWithCache<
			{ files: number; insertions: number; deletions: number } | undefined
