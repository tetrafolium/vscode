...
import {
	IResourceLabel,
	ResourceLabels,
} from "../../../../workbench/browser/labels.js";
...
export function toIChangesFileItem(
...
export type ChangesTreeElement =
	| IChangesRootItem
	| IChangesFileItem
	| IResourceNode<IChangesFileItem, undefined>;

export function isChangesFileItem(
	element: ChangesTreeElement,
): element is IChangesFileItem {
	return !ResourceTree.isResourceNode(element) && element.type === "file";
}

export function isChangesRootItem(
	element: ChangesTreeElement,
): element is IChangesRootItem {
	return !ResourceTree.isResourceNode(element) && element.type === "root";
}

export function buildTreeChildren(
	items: IChangesFileItem[],
	treeRootInfo?: IChangesTreeRootInfo,
): ICompressedTreeElement<ChangesTreeElement>[] {
	if (items.length === 0) {
		return [];
	}
	...
	const resourceTree = new ResourceTree<IChangesFileItem, undefined>(
		undefined,
		rootUri,
		extUriBiasedIgnorePathCase,
	);
	...
	function convertChildren(
		parent: IResourceNode<IChangesFileItem, undefined>,
	): ICompressedTreeElement<ChangesTreeElement>[] {
...
export class ChangesTreeRenderer implements ICompressibleTreeRenderer<
	ChangesTreeElement,
	void,
	IChangesTreeTemplate
> {
	...
	renderTemplate(container: HTMLElement): IChangesTreeTemplate {
		...
		const toolbar = templateDisposables.add(
			scopedInstantiationService.createInstance(
				MenuWorkbenchToolBar,
				actionBarContainer,
				MenuId.AgentsChangeInlineToolbar,
				{
		...
		const changeKindContextKey =
			ChangesContextKeys.ChangeKind.bindTo(contextKeyService);
	...
	renderElement(
		node: ITreeNode<ChangesTreeElement, void>,
		_index: number,
		templateData: IChangesTreeTemplate,
	): void {
	...
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<ChangesTreeElement>, void>,
		_index: number,
		templateData: IChangesTreeTemplate,
	): void {
	...
	private renderFileElement(
		...
		templateData.lineCountsContainer.style.display = showChangeDecorations
			? ""
			: "none";
		templateData.decorationBadge.style.display = showChangeDecorations
			? ""
			: "none";
		...
		templateData.elementDisposables.add(
			autorun((reader) => {
				const reviewCommentByFile =
					this.viewModel.activeSessionReviewCommentCountByFileObs.read(reader);
				const reviewCommentCount =
					reviewCommentByFile?.get(data.uri.fsPath) ?? 0;

				if (reviewCommentCount > 0) {
					templateData.reviewCommentsBadge.style.display = "";
					templateData.reviewCommentsBadge.className =
						"changes-review-comments-badge";
		...
		templateData.elementDisposables.add(
			autorun((reader) => {
				const agentFeedbackByFile =
					this.viewModel.activeSessionAgentFeedbackCountByFileObs.read(reader);
				const agentFeedbackCount =
					agentFeedbackByFile?.get(data.uri.fsPath) ?? 0;

				if (agentFeedbackCount > 0) {
					templateData.agentFeedbackBadge.style.display = "";
					templateData.agentFeedbackBadge.className =
						"changes-agent-feedback-badge";
	...
	private renderRootElement(
		data: IChangesRootItem,
		templateData: IChangesTreeTemplate,
	): void {
		templateData.label.setResource(
			{
	...
	private renderFolderElement(
		node: IResourceNode<IChangesFileItem, undefined>,
		templateData: IChangesTreeTemplate,
	): void {
	...
	disposeElement(
		_element: ITreeNode<ChangesTreeElement, void>,
		_index: number,
		templateData: IChangesTreeTemplate,
	): void {
	...
	disposeCompressedElements(
		_element: ITreeNode<ICompressedTreeNode<ChangesTreeElement>, void>,
		_index: number,
		templateData: IChangesTreeTemplate,
	): void {
		templateData.elementDisposables.clear();
