...
export class ChangesViewModel extends Disposable {
	...
	readonly activeSessionChangesetsObs: IObservable<
		readonly ISessionChangeset[] | undefined
	>;
	readonly activeSessionChangesetObs: IObservable<
		ISessionChangeset | undefined
	>;
	readonly activeSessionHasGitRepositoryObs: IObservable<boolean>;
	readonly activeSessionReviewCommentCountByFileObs: IObservable<
		Map<string, number>
	>;
	readonly activeSessionAgentFeedbackCountByFileObs: IObservable<
		Map<string, number>
	>;
	...
	constructor(
		...
		private readonly agentFeedbackService: IAgentFeedbackService,
		...
		this.activeSessionHasGitRepositoryObs = derived((reader) => {
			const isVirtualWorkspace =
				this.activeSessionIsVirtualWorkspaceObs.read(reader);
		...
		this.activeSessionReviewCommentCountByFileObs =
			this._getActiveSessionReviewComments();

		// Active session agent feedback count by file
		this.activeSessionAgentFeedbackCountByFileObs =
			this._getActiveSessionAgentFeedback();
		...
		this.activeSessionChangesetObs = derived<ISessionChangeset | undefined>(
			(reader) => {
				const selectedChangesetId = this._selectedChangesetId.read(reader);
				const activeSessionChangesets =
					this.activeSessionChangesetsObs.read(reader) ?? [];
				...
				const selectedChangeset = selectedChangesetId
					? activeSessionChangesets.find(
							(c) => c.id === selectedChangesetId && c.isEnabled.read(reader),
						)
					: undefined;

				return (
					selectedChangeset ??
					activeSessionChangesets.find((c) => c.isDefault.read(reader))
				);
			},
		);
		...
		const initialMode =
			storedMode === ChangesViewMode.Tree
	...
	private _getActiveSessionState(): {
		...
		const activeSessionStateObs = derivedObservableWithCache<
		...
		>(this, (reader, lastValue) => {
			...
			const hasGitRepository =
				this.activeSessionHasGitRepositoryObs.read(reader);
			...
			const isolationMode =
				gitRepository?.workTreeUri === undefined
			...
			const hasGitOperationInProgress =
				gitRepository?.hasGitOperationInProgress ?? false;
		...
		return {
			...
			state: derivedOpts({ equalsFn: structuralEquals }, (reader) =>
				activeSessionStateObs.read(reader),
			),
		};
	}
	...
	private _getActiveSessionReviewComments(): IObservable<Map<string, number>> {
		return derived((reader) => {
			...
			const prReviewState = this.codeReviewService
			...
			const reviewState = this.codeReviewService
	...
	private _getActiveSessionAgentFeedback(): IObservable<Map<string, number>> {
		return derived((reader) => {
			...
			observableSignalFromEvent(
				this,
				this.agentFeedbackService.onDidChangeFeedback,
			).read(reader);

			const feedbackItems =
				this.agentFeedbackService.getFeedback(sessionResource);
			const result = new Map<string, number>();
