...
export function isFailedConclusion(
...
export function buildFixChecksPrompt(
	failedChecks: ReadonlyArray<{ check: IGitHubCICheck; annotations: string }>,
): string {
	const sections = failedChecks.map(({ check, annotations }) => {
		...
		parts.push(
			...
			annotations || "No output available for this check run.",
		);
		return parts.join("\n");
	});
 ...
 */
class ActiveSessionFailedCIChecksContextContribution
	extends Disposable
	...
	constructor(
		...
		this._register(
			bindContextKey(
				...
				(reader) => {
					const ciModel =
						gitHubService.activeSessionPullRequestCIObs.read(reader);
					if (!ciModel) {
						return false;
					}
