...
export class SessionsPolicyBlockedOverlay extends Disposable {
	...
	constructor(
		...
		this._register(
			addDisposableGenericMouseDownListener(this.overlay, (e) => {
	...
	private _renderAgentDisabled(card: HTMLElement): void {
		this.overlay.setAttribute(
			"aria-label",
			localize("policyBlocked.aria", "Agents disabled by organization policy"),
		);

		append(
			card,
			$("h2", undefined, localize("policyBlocked.title", "Agents Disabled")),
		);
		...
		const button = this._register(
			new Button(card, { ...defaultButtonStyles, secondary: true }),
		);
	...
	private _renderAccountPolicyGate(
		card: HTMLElement,
		options: ISessionsBlockedOverlayOptions,
	): void {
		this.overlay.setAttribute(
			"aria-label",
			localize("accountGate.aria", "Sign-in required by your administrator"),
		);

		append(
			card,
			$("h2", undefined, localize("accountGate.title", "Sign-In Required")),
		);
		...
		this._register(
			addDisposableListener(learnMore, EventType.CLICK, (e) => {
				...
				this.openerService.open(
					URI.parse("https://code.visualstudio.com/docs/enterprise/overview"),
				);
			}),
		);

		const signInButton = this._register(
			new Button(card, { ...defaultButtonStyles }),
		);
	...
	private _openVSCode(): void {
		const scheme =
			this.productService.parentPolicyConfig?.urlProtocol ??
			this.productService.urlProtocol;
		this.openerService.open(URI.from({ scheme, query: "windowId=_blank" }), {
			openExternal: true,
		});
	}
}
