...
export class SessionBrowserViewController
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			this._editorService.onWillOpenEditor((e) => {
		...
		this._register(
			this._editorGroupsService.onDidAddGroup((group) => {
		...
		this._register(
			this._sessionManagementService.onDidChangeSessions((e) => {
				...
				const removedSessionIds = new Set(
					e.removed.map((s) => s.resource.toString()),
				);
	...
	private _attachLifecycle(input: BrowserEditorInput): void {
		...
		const session =
			this._sessionManagementService.activeSession.read(undefined);
		if (!session) {
		...
		store.add(
			runOnChange(session.isArchived, (isArchived) => {
				if (isArchived) {
					input.dispose(true);
				}
			}),
