...
const hasWorktreeAndRepositoryContextKey = new RawContextKey<boolean>(
...
class ApplyChangesToParentRepoContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const worktreeAndRepoKey =
			hasWorktreeAndRepositoryContextKey.bindTo(contextKeyService);
		...
		try {
			...
			if (!worktreeBranch) {
				notificationService.notify({
					...
					message: localize(
						"applyChangesNoBranch",
						"Could not determine worktree branch name.",
					),
				});
				return;
			}
			...
			if (!result) {
				logService.warn(
					"[ApplyChangesToParentRepo] No result from merge command",
				);
			} else {
				notificationService.notify({
					...
					message:
						...
						result.startsWith("Already up to date")
							? localize(
									"alreadyUpToDate",
									"Parent repository is up to date with worktree.",
								)
							: localize(
									"applyChangesSuccess",
									"Applied changes to parent repository.",
								),
			...
			logService.error(
				"[ApplyChangesToParentRepo] Failed to apply changes",
				err,
			);
			notificationService.notify({
				severity: Severity.Warning,
				message: localize(
					"applyChangesConflict",
					"Failed to apply changes to parent repo. The parent repo may have diverged — resolve conflicts manually.",
				),
				actions: { primary: [openFolderAction] },
			});
		}
