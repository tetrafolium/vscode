...
const canRunSessionCodeReviewContextKey = new RawContextKey<boolean>(
...
function registerSessionCodeReviewAction(
	tooltip: string,
	icon: ThemeIcon,
): Disposable {
	class RunSessionCodeReviewAction extends Action2 {
		...
		override async run(
			...
			const reviewCount =
				reviewState.kind !== CodeReviewStateKind.Idle &&
			...
			if (codeReviewCount > 0 || prReviewCount > 0) {
				...
				const first = agentFeedbackService.getNextNavigableItem(
					resource,
					comments,
					true,
				);
...
class CodeReviewToolbarContribution
	extends Disposable
	implements IWorkbenchContribution
{
	static readonly ID = "sessions.contrib.codeReviewToolbar";

	private readonly _actionRegistration = this._register(
		new MutableDisposable<Disposable>(),
	);
	...
	constructor(
		...
		const canRunCodeReviewContext =
			canRunSessionCodeReviewContextKey.bindTo(contextKeyService);
		...
		this._register(
			autorun((reader) => {
				...
				const files = getCodeReviewFilesFromSessionChanges(changes);
				const version = getCodeReviewVersion(files);
				const reviewState = this._codeReviewService
					.getReviewState(sessionResource)
					.read(reader);
				const prReviewState = this._codeReviewService
				...
				if (
					...
					icon = reviewState.didProduceComments
						? Codicon.comment
						: Codicon.codeReview;
				}
