...
} from "../../browser/codeReviewService.js";
import {
	IActiveSession,
...
suite("CodeReviewService", () => {
	...
	class MockCommandService implements ICommandService {
		...
		deferNextExecution(): void {
			...
			this.executeCommand = async function <T>(
				commandId: string,
				...args: unknown[]
			): Promise<T> {
	...
	class MockSessionsManagementService extends mock<ISessionsManagementService>() {
		...
		private readonly _activeSession: ReturnType<
		...
		constructor(disposables: DisposableStore) {
			...
			this._activeSession = observableValue<IActiveSession | undefined>(
				"test.activeSession",
				undefined,
			);
		...
		addSession(
			resource: URI,
			changes?: readonly IChatSessionFileChange2[],
			archived = false,
		): ISession {
			const changesObs = observableValue<readonly IChatSessionFileChange[]>(
				"test.changes",
				(changes ?? []).map((c) => ({
			...
			const isArchivedObs = observableValue<boolean>(
				"test.isArchived",
				archived,
			);
		...
		setActiveSession(resource: URI | undefined): void {
			this._activeSession.set(
				resource
					? (this._sessions.get(resource.toString()) as
							| IActiveSession
							| undefined)
					: undefined,
				undefined,
		...
		updateSessionChanges(
			resource: URI,
			changes: readonly IChatSessionFileChange2[] | undefined,
		): void {
			const session = this._sessions.get(resource.toString());
			if (session) {
				const obs = session.changes as ReturnType<
					typeof observableValue<readonly IChatSessionFileChange[]>
				>;
				obs.set(
					(changes ?? []).map((c) => ({
						modifiedUri: c.modifiedUri ?? c.uri,
	...
	class MockGitHubService extends mock<IGitHubService>() {
		...
		private readonly _reviewThreadsModels = new Map<
			string,
			GitHubPullRequestReviewThreadsModel
		>();
		private readonly _reviewThreadsFetchers = new Map<
			string,
			MockReviewThreadsFetcher
		>();

		getPullRequestCalls = 0;
		getPullRequestReviewThreadsCalls = 0;

		override readonly activeSessionPullRequestReviewThreadsObs: IObservable<
			GitHubPullRequestReviewThreadsModel | undefined
		>;

		constructor(sessionsManagementService: MockSessionsManagementService) {
			super();
			this._reviewThreadsFetchers.set(
				this._key("owner", "repo", 1),
				this.reviewThreadsFetcher,
			);
			...
			this.activeSessionPullRequestReviewThreadsObs = derived((reader) => {
				...
				return this.getReviewThreadsModel(
					gitHubInfo.owner,
					gitHubInfo.repo,
		...
		getReviewThreadsModel(
			...
			if (!model) {
				model = store.add(
					new GitHubPullRequestReviewThreadsModel(
						owner,
						repo,
						prNumber,
						this.getReviewThreadsFetcher(
							owner,
							repo,
							prNumber,
		...
		override createPullRequestReviewThreadsModelReference(
			owner: string,
			repo: string,
			prNumber: number,
		): IReference<GitHubPullRequestReviewThreadsModel> {
			this.getPullRequestReviewThreadsCalls++;
			return new ImmortalReference(
				this.getReviewThreadsModel(owner, repo, prNumber),
			);
		}
	...
	test("PR review state uses dedicated review threads model", async () => {
		...
		gitHubService.reviewThreadsFetcher.nextThreads = [
			makePRThread("thread-100", "src/a.ts"),
		];
			...
			assert.deepStrictEqual(
				{
					...
					getPullRequestReviewThreadsCalls:
						gitHubService.getPullRequestReviewThreadsCalls,
					legacyThreadRefreshes:
						gitHubService.legacyFetcher.getReviewThreadsCalls,
					reviewThreadRefreshes:
						gitHubService.reviewThreadsFetcher.getReviewThreadsCalls,
				},
				{
					comments: [
						{
		...
		assert.deepStrictEqual(
			{
				...
				legacyResolveThreadCalls:
					gitHubService.legacyFetcher.resolveThreadCalls,
				reviewResolveThreadCalls:
					gitHubService.reviewThreadsFetcher.resolveThreadCalls,
			},
			{
				getPullRequestCalls: 0,
	...
	test("requestReview calls command with correct arguments", async () => {
		...
		assert.strictEqual(
			commandService.lastCommandId,
			"chat.internal.codeReview.run",
		);
		const args = commandService.lastArgs?.[0] as {
			files: { currentUri: URI; baseUri?: URI }[];
		};
	...
	test("requestReview is a no-op when unresolved comments exist for the same version", async () => {
		commandService.result = {
			type: "success",
			comments: [{ uri: fileA, range: new Range(1, 1, 1, 1), body: "comment" }],
		};
		service.requestReview(session, "v1", [{ currentUri: fileA }]);
		await tick();
	...
	test("requestReview for a new version replaces loading state", async () => {
		...
		commandService.result = {
			type: "success",
			comments: [
				{ uri: fileA, range: new Range(1, 1, 1, 1), body: "v2 comment" },
			],
		};
	...
	test("dismissReview while loading resets to idle", () => {
		...
		commandService.resolveExecution({
			type: "success",
			comments: [{ uri: fileA, range: new Range(1, 1, 1, 1), body: "late" }],
		});
	});
	...
	test("different sessions are independent", async () => {
		...
		commandService.result = {
			type: "success",
			comments: [
				{ uri: fileA, range: new Range(1, 1, 1, 1), body: "session1 comment" },
			],
		};
		service.requestReview(session, "v1", [{ currentUri: fileA }]);
		await tick();
		...
		commandService.result = {
			type: "success",
			comments: [
				{ uri: fileB, range: new Range(2, 1, 2, 1), body: "session2 comment" },
			],
		};
		...
		if (
			state1.kind === CodeReviewStateKind.Result &&
			state2.kind === CodeReviewStateKind.Result
		) {
	...
	test("comments normalize VS Code API style ranges", async () => {
		...
		if (state.kind === CodeReviewStateKind.Result) {
			...
			assert.deepStrictEqual(
				state.comments[0].suggestion?.edits[0].range,
				new Range(9, 2, 9, 10),
			);
		}
	});
	...
	test("comments normalize serialized URIs and tuple ranges from API payloads", async () => {
		const serializedUri = JSON.parse(
			JSON.stringify(
				URI.parse(
					"git:/c%3A/Code/vscode.worktrees/copilot-worktree-2026-03-04T14-44-38/src/vs/sessions/contrib/changesView/test/browser/codeReviewService.test.ts?%7B%22path%22%3A%22c%3A%5C%5CCode%5C%5Cvscode.worktrees%5C%5Ccopilot-worktree-2026-03-04T14-44-38%5C%5Csrc%5C%5Cvs%5C%5Csessions%5C%5Ccontrib%5C%5CchangesView%5C%5Ctest%5C%5Cbrowser%5C%5CcodeReviewService.test.ts%22%2C%22ref%22%3A%22copilot-worktree-2026-03-04T14-44-38%22%7D",
				),
			),
		);
		...
		if (state.kind === CodeReviewStateKind.Result) {
			assert.strictEqual(
				state.comments[0].uri.toString(),
				URI.revive(serializedUri).toString(),
			);
	...
	test("reviews are restored from storage on service creation", async () => {
		...
		const service2 = store.add(
			instantiationService.createInstance(CodeReviewService),
		);
		...
		if (state.kind === CodeReviewStateKind.Result) {
			...
			assert.strictEqual(state.comments[0].uri.toString(), fileA.toString());
			assert.deepStrictEqual(state.comments[0].range, {
				startLineNumber: 1,
				startColumn: 1,
				endLineNumber: 5,
				endColumn: 1,
	...
	test("suggestions are persisted and restored correctly", async () => {
		...
		if (state.kind === CodeReviewStateKind.Result) {
			...
			assert.strictEqual(
				state.comments[0].suggestion?.edits[0].oldText,
				"let x = 1;",
			);
			assert.strictEqual(
				state.comments[0].suggestion?.edits[0].newText,
				"const x = 1;",
			);
		}
	});
	...
	test("dismissReview removes session from storage", async () => {
		commandService.result = {
			type: "success",
			comments: [{ uri: fileA, range: new Range(1, 1, 1, 1), body: "c" }],
		};
		service.requestReview(session, "v1", [{ currentUri: fileA }]);
		await tick();
		...
		assert.strictEqual(
			storageService.get("codeReview.reviews", StorageScope.WORKSPACE),
			undefined,
		);
	});
	...
	test("session with changed version has review cleaned up", async () => {
		...
		commandService.result = {
			type: "success",
			comments: [
				{ uri: fileA, range: new Range(1, 1, 1, 1), body: "stale comment" },
			],
		};
		service.requestReview(session, version, files);
		await tick();
	...
	test("session that no longer exists has review cleaned up", async () => {
		commandService.result = {
			type: "success",
			comments: [
				{ uri: fileA, range: new Range(1, 1, 1, 1), body: "orphaned comment" },
			],
		};
		service.requestReview(session, "v1", [{ currentUri: fileA }]);
		await tick();
	...
	test("session with matching version keeps review intact", async () => {
		...
		commandService.result = {
			type: "success",
			comments: [
				{ uri: fileA, range: new Range(1, 1, 1, 1), body: "valid comment" },
			],
		};
		service.requestReview(session, version, files);
		await tick();
