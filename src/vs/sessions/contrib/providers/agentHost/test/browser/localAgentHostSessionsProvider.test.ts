...
import {
	ChangesetStatus,
	SessionStatus as ProtocolSessionStatus,
	StateComponents,
	type ChangesetState,
...
const STORAGE_KEY_REMEMBERED_SESSION_CONFIG_VALUES =
	"sessions.agentHost.sessionConfigPicker.selectedValues";
...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	private _rootStateValue: RootState | Error | undefined = {
		agents: [
			{
				provider: "copilotcli",
				displayName: "Copilot",
	...
	public resolveSessionConfigResult: ResolveSessionConfigResult = {
	...
	constructor() {
		...
		this.rootState = {
			...
			get verifiedValue() {
				return self._rootStateValue instanceof Error
					? undefined
					: self._rootStateValue;
			},
	...
	override async createSession(
		...
		const uri =
			config?.session ?? URI.parse("copilotcli:///auto-" + this._nextSeq);
	...
	override async resolveSessionConfig(request: {
	...
	private readonly _sessionStateEmitters = new Map<
		string,
		Emitter<SubscriptionState>
	>();
	...
	override getSubscription<T>(
		...
		this.sessionSubscribeCounts.set(
			key,
			(this.sessionSubscribeCounts.get(key) ?? 0) + 1,
		);
		...
		const sub: IAgentSubscription<T> = {
			get value() {
				return self._sessionStateValues.get(key) as unknown as T | undefined;
			},
			get verifiedValue() {
				return self._sessionStateValues.get(key) as unknown as T | undefined;
			},
		...
		return {
			object: sub,
			dispose: () => {
				this.sessionUnsubscribeCounts.set(
					key,
					(this.sessionUnsubscribeCounts.get(key) ?? 0) + 1,
				);
			},
		};
	}
...
function createSession(
	...
	opts?: {
		...
		summary?: string;
		model?: string;
		project?: { uri: URI; displayName: string };
...
function createProvider(
	...
	instantiationService.stub(IChatSessionsService, {
		getChatSessionContribution: (chatSessionType: string) =>
			contributions.find((c) => c.type === chatSessionType),
		getAllChatSessionContributions: () => contributions,
		getOrCreateChatSession: async () => ({
			onWillDispose: () => ({ dispose() {} }),
			sessionResource: URI.from({ scheme: "test" }),
			history: [],
			dispose() {},
		}),
	});
	instantiationService.stub(IChatService, {
		...
		sendRequest:
			options?.sendRequest ??
			(async (): Promise<ChatSendResult> => ({
				kind: "sent" as const,
				data: {} as ChatSendResult extends { kind: "sent"; data: infer D }
					? D
					: never,
			})),
	});
	instantiationService.stub(IChatWidgetService, {
		openSession: async () =>
			options?.openSession
				? new (class extends mock<IChatWidget>() {})()
				: undefined,
	});
	...
	instantiationService.stub(
		IStorageService,
		options?.storageService ?? disposables.add(new InMemoryStorageService()),
	);
	instantiationService.stub(
		IGitHubService,
	...
	instantiationService.stub(
		...
		new (class extends mock<IAgentHostActiveClientService>() {
			override getActiveClient = (_sessionType: string, clientId: string) => ({
				clientId,
				tools: [],
				customizations: [],
			});
...
async function waitForSessionConfig(
	provider: LocalAgentHostSessionsProvider,
	sessionId: string,
	predicate: (config: ResolveSessionConfigResult | undefined) => boolean,
): Promise<void> {
	...
	await new Promise<void>((resolve) => {
		const disposable = provider.onDidChangeSessionConfig((changedSessionId) => {
			if (
				...
				predicate(provider.getSessionConfig(sessionId))
			) {
				disposable.dispose();
				resolve();
			}
...
function fireSessionAdded(
	agentHost: MockAgentHostService,
	rawId: string,
	opts?: {
		provider?: string;
		title?: string;
	...
	agentHost.fireNotification({
		...
		summary: {
			...
			model: opts?.model
				? {
						id: opts.model,
						...(opts.modelConfig ? { config: opts.modelConfig } : {}),
					}
				: undefined,
			project: opts?.project,
			workingDirectory: opts?.workingDirectory,
...
function fireSessionSummaryChanged(
	agentHost: MockAgentHostService,
	rawId: string,
...
suite("LocalAgentHostSessionsProvider", () => {
	...
	test("session types update when the local host advertises additional agents", () => {
		...
		assert.deepStrictEqual(
			provider.sessionTypes.map((t) => ({ id: t.id, label: t.label })),
	...
	test("session icons match the session type icon", () => {
		...
		const provider = createProvider(disposables, agentHost);
		fireSessionAdded(agentHost, "cli-sess", {
			title: "CLI",
			provider: "copilotcli",
		});
		fireSessionAdded(agentHost, "claude-sess", {
			title: "Claude",
			provider: "claude-code",
		});
		fireSessionAdded(agentHost, "unknown-sess", {
			title: "Unknown",
			provider: "unknown-agent",
		});
		...
		assert.deepStrictEqual(
			provider
				.getSessions()
				.map((s) => ({ sessionType: s.sessionType, icon: s.icon.id }))
	...
	test("getSessions populates from listSessions", () =>
	...
	test("eagerly populates and fires onDidChangeSessions after construction without a getSessions() call", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("defers eager session list fetch until authentication settles", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				changes.length,
				0,
				"no event should fire while authentication is pending",
			);
			assert.strictEqual(
				provider.getSessions().length,
				0,
				"no sessions should be cached while authentication is pending",
			);
			...
			agentHost.addSession(
				createSession("after-auth-2", { summary: "Second" }),
			);
	...
	test("recovers an empty list when the initial listSessions fails, without needing a new session", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				changes.length,
				0,
				"no event should fire after a failed initial refresh",
			);
			assert.strictEqual(
				provider.getSessions().length,
				0,
				"cache stays empty after a failed initial refresh",
			);
	...
	test("a successful empty listSessions arms no retry", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				callsAfterEagerLoad,
				1,
				"exactly one eager listSessions call",
			);
			...
			assert.strictEqual(
				agentHost.listSessionsCallCount,
				callsAfterEagerLoad,
				"no retry should be scheduled after a successful empty list",
			);
			assert.strictEqual(
				changes.length,
				0,
				"no change event for an empty list",
			);
			assert.strictEqual(provider.getSessions().length, 0);
		}));

	test("retries with backoff until listSessions succeeds", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("uses project metadata as workspace group source", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const projectUri = URI.file("/home/user/vscode");
			const workingDirectory = URI.file(
				"/tmp/copilot-worktrees/vscode-feature",
			);
			agentHost.addSession(
				createSession("project-1", {
	...
	test("listed session with only workingDirectory (no project) shows folder name", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const workingDirectory = URI.file("/home/user/standalone-folder");
			agentHost.addSession(
				createSession("wd-only-1", {
	...
	test("uses model metadata as selected model for listed sessions", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("model-1", {
			...
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Model Session");
			assert.strictEqual(
				session?.modelId.get(),
				"agent-host-copilotcli:claude-sonnet-4.5",
			);
		}));

	test("uses model metadata from session added notification", () => {
		const provider = createProvider(disposables, agentHost);
		fireSessionAdded(agentHost, "notif-model", {
			title: "Notif Model Session",
			model: "gpt-5",
		});

		const session = provider
			.getSessions()
			.find((s) => s.title.get() === "Notif Model Session");
	...
	test("setModel updates existing session model and dispatches raw model", () => {
		const provider = createProvider(disposables, agentHost);
		fireSessionAdded(agentHost, "set-model", {
			title: "Set Model Session",
			model: "old-model",
		});

		const session = provider
			.getSessions()
			.find((s) => s.title.get() === "Set Model Session");
		assert.ok(session);
		...
		assert.strictEqual(
			session!.modelId.get(),
			"agent-host-copilotcli:new-model",
		);
	...
	test("setModel preserves current model config when model id is unchanged", () => {
		const provider = createProvider(disposables, agentHost);
		fireSessionAdded(agentHost, "set-model-config", {
			title: "Set Model Config Session",
		...
		const session = provider
			.getSessions()
			.find((s) => s.title.get() === "Set Model Config Session");
		assert.ok(session);

		provider.setModel(
			session!.sessionId,
			"agent-host-copilotcli:configured-model",
		);
	...
	test("setAgent updates existing session agent and dispatches SessionAgentChanged", () => {
		...
		const session = provider
			.getSessions()
			.find((s) => s.title.get() === "Set Agent Session");
		assert.ok(session);

		provider.setAgent?.(session!.sessionId, {
			uri: "agent://review",
			name: "review",
		});
		...
		assert.deepStrictEqual(session!.mode.get(), {
			id: "agent://review",
			kind: "agent",
		});
		assert.deepStrictEqual(agentHost.dispatchedActions.at(-1)?.action, {
	...
	test("setAgent with undefined clears the selection and dispatches an empty action", () => {
		const provider = createProvider(disposables, agentHost);
		fireSessionAdded(agentHost, "clear-agent", {
			title: "Clear Agent Session",
		});

		const session = provider
			.getSessions()
			.find((s) => s.title.get() === "Clear Agent Session");
		assert.ok(session);
	...
	test("getCustomAgents collects agents from session customizations, coalesced by URI and sorted by name", async () => {
		...
		const session = provider
			.getSessions()
			.find((s) => s.title.get() === "Merge Session");
		assert.ok(session);
			...
			customizations: [
				{
					...
					children: [
						...
						{
							type: CustomizationType.Agent,
							id: "agent://session-only",
							uri: "agent://session-only",
							name: "session-only",
						},
					],
				},
	...
	test("onDidChangeCustomAgents fires on root state and session state changes", async () => {
		...
		const session = provider
			.getSessions()
			.find((s) => s.title.get() === "Cust Events");
		assert.ok(session);
		...
		assert.ok(
			fired > afterRoot,
			"expected event to fire on session state customization change",
		);
		...
		agentHost.setSessionState("cust-events", "copilotcli", {
			...
			customizations: (
				provider as unknown as { _lastSessionStates: Map<string, SessionState> }
			)._lastSessionStates.get(session!.sessionId)?.customizations,
		});
		assert.strictEqual(
			fired,
			afterFirstCustomization,
			"expected event NOT to fire when customizations are unchanged",
		);
	});
	...
	test("NewSession forwards SessionState into _lastSessionStates so the picker sees customizations before first message", async () => {
		...
		assert.ok(
			fired > 0,
			"expected onDidChangeCustomAgents to fire when SessionState arrives",
		);
		...
		assert.ok(
			fired > after,
			"expected onDidChangeCustomAgents to fire again on a second update",
		);
	});
	...
	test("NewSession dispose clears _lastSessionStates entry and fires onDidChangeCustomAgents", async () => {
		const provider = createProvider(disposables, agentHost);
		const sessionTypeId = provider.sessionTypes[0].id;
		const first = provider.createNewSession(
			URI.parse("file:///home/user/a"),
			sessionTypeId,
		);
		await timeout(0);
		...
		assert.ok(
			fired > 0,
			"expected onDidChangeCustomAgents to fire on NewSession dispose",
		);
	});
	...
	test("createNewSession seeds autoApprove from chat.permissions.default and forwards it to resolveSessionConfig", async () => {
		...
		await config.setUserConfiguration(
		...
		await waitForSessionConfig(
			provider,
			session.sessionId,
			(c) => c?.values.autoApprove === "autoApprove",
		);
		...
		assert.deepStrictEqual(
			{
				...
				forwardedToAgentHost:
					agentHost.resolveSessionConfigRequests.at(-1)?.config?.autoApprove,
			},
			{
	...
	test("createNewSession forwards seeded config to eager createSession", async () => {
		...
		assert.deepStrictEqual(agentHost.createSessionConfigs[0]?.config, {
	...
	test("setSessionConfigValue remembers string picks and ignores unsafe keys", async () => {
		...
		await provider.setSessionConfigValue(
			session.sessionId,
		...
		assert.deepStrictEqual(
			storageService.getObject(
				STORAGE_KEY_REMEMBERED_SESSION_CONFIG_VALUES,
	...
	test("setSessionConfigValue clamps autoApprove to default when policy disables global auto-approve", async () => {
		...
		assert.deepStrictEqual(
			{
				remembered: storageService.getObject(
					STORAGE_KEY_REMEMBERED_SESSION_CONFIG_VALUES,
					StorageScope.PROFILE,
					{},
				),
	...
	test("createNewSession seeds remembered values and skips unsafe remembered keys", () => {
		...
		storageService.store(
			STORAGE_KEY_REMEMBERED_SESSION_CONFIG_VALUES,
			`{"${SessionConfigKey.Isolation}":"folder","${SessionConfigKey.Branch}":"main","__proto__":"polluted"}`,
			StorageScope.PROFILE,
		...
		assert.deepStrictEqual(
			{
				seededImmediately: provider.getSessionConfig(session.sessionId)?.values,
				forwardedToAgentHost:
					agentHost.resolveSessionConfigRequests.at(-1)?.config,
			},
			{
	...
	test("createNewSession gives chat.permissions.default precedence over remembered autoApprove while normalizing by policy", async () => {
		...
		await policyRestrictedConfig.setUserConfiguration(
			"chat.permissions.default",
			"autoApprove",
		);
		const policyRestrictedProvider = createProvider(
			disposables,
			agentHost,
			undefined,
			{ configurationService: policyRestrictedConfig, storageService },
		);
		policyRestrictedProvider.createNewSession(
			URI.parse("file:///home/user/project"),
			policyRestrictedProvider.sessionTypes[0].id,
		);

		// Case 2: configured 'default' wins over remembered 'autopilot'
		const configuredDefaultConfig = new TestConfigurationService();
		await configuredDefaultConfig.setUserConfiguration(
			"chat.permissions.default",
			"default",
		);
		const configuredDefaultProvider = createProvider(
			disposables,
			agentHost,
			undefined,
			{ configurationService: configuredDefaultConfig, storageService },
		);
		configuredDefaultProvider.createNewSession(
			URI.parse("file:///home/user/project"),
			configuredDefaultProvider.sessionTypes[0].id,
		);
		...
		assert.deepStrictEqual(
			{
				policyRestricted:
					agentHost.resolveSessionConfigRequests.at(-2)?.config?.autoApprove,
				configuredDefault:
					agentHost.resolveSessionConfigRequests.at(-1)?.config?.autoApprove,
			},
			{
	...
	test("createNewSession eagerly creates the backend session with the client-allocated URI", async () => {
		...
		const rawId = session.resource.path.substring(1);
		const expectedBackendUri = AgentSession.uri(
			provider.sessionTypes[0].id,
			rawId,
		);
		assert.deepStrictEqual(
	...
	test("createNewSession disposes the previous eager backend session on workspace switch", async () => {
		...
		const second = provider.createNewSession(
			URI.parse("file:///home/user/b"),
			sessionTypeId,
		);
		await timeout(0);
	...
	test("eager createSession completes on the wire before getSubscription opens", async () => {
		...
		const rawId = session.resource.path.substring(1);
		const backendKey = AgentSession.uri(
			provider.sessionTypes[0].id,
			rawId,
		).toString();
	...
	test("workspace switch mid-createSession does not open a stale subscription", async () => {
		...
		const firstBackendKey = AgentSession.uri(
			sessionTypeId,
			first.resource.path.substring(1),
		).toString();
		const secondBackendKey = AgentSession.uri(
			sessionTypeId,
			second.resource.path.substring(1),
		).toString();
		assert.strictEqual(
	...
	test("deleteSession calls disposeSession and removes from cache", async () => {
		...
		assert.strictEqual(
			provider.getSessions().find((s) => s.title.get() === "To Delete"),
			undefined,
		);
	});
	...
	test("renameChat dispatches SessionTitleChanged action", async () => {
		...
		assert.strictEqual(
			(dispatched.action as { title: string }).title,
			"New Title",
		);
	...
	test("renameChat is no-op for unknown session", async () => {
		const provider = createProvider(disposables, agentHost);
		await provider.renameChat(
			"nonexistent-id",
			URI.parse("test://nonexistent"),
			"Ignored",
		);
	...
	test("server-echoed SessionModelChanged updates cached model", () => {
		const provider = createProvider(disposables, agentHost);
		fireSessionAdded(agentHost, "model-change", {
			title: "Model Change",
			model: "old-model",
		});
		...
		const target = provider
			.getSessions()
			.find((s) => s.title.get() === "Model Change");
		assert.ok(target);
	...
	test("turnComplete action triggers session refresh", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("turn-sess", { summary: "Before", modifiedTime: 1000 }),
			);
			...
			agentHost.addSession(
				createSession("turn-sess", { summary: "After", modifiedTime: 5000 }),
			);
			...
			assert.ok(changes.length > 0);
			const updatedSession = provider
				.getSessions()
				.find((s) => s.title.get() === "After");
			assert.ok(updatedSession);
		}));

	// ---- Session data adapter -------

	test("session adapter has correct workspace from working directory", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("ws-sess", {
					summary: "WS Test",
					workingDirectory: URI.parse("file:///home/user/myrepo"),
				}),
			);
	...
	test("session adapter without working directory has no workspace", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("session adapter uses raw ID as fallback title", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("new session stays loading when required config is missing", async () => {
		agentHost.resolveSessionConfigResult = {
			schema: {
				...
				required: ["branch"],
				properties: {
					branch: { type: "string", title: "Branch", enum: ["main"] },
				},
			},
			values: {},
		};
		...
		await waitForSessionConfig(
			provider,
			session.sessionId,
			(config) => config?.schema.required?.includes("branch") === true,
		);
	...
	test("cached session loading reflects authenticationPending", async () => {
		agentHost.setAuthenticationPending(true);
		agentHost.addSession(
			createSession("cached-auth-loading", { summary: "Cached" }),
		);
	...
	test("new session loading reflects authenticationPending until config resolves", async () => {
		...
		await waitForSessionConfig(
			provider,
			session.sessionId,
			(config) => config?.values.isolation === "worktree",
		);
	...
	test("sendRequest throws for unknown session", async () => {
		...
		await assert.rejects(
			() =>
				provider.sendRequest("nonexistent", URI.parse("untitled:chat"), {
					query: "test",
				}),
	...
	test("sendRequest forwards resolved session config to chat service", async () => {
		...
		const provider = createProvider(disposables, agentHost, undefined, {
			...
			sendRequest: async (
				...
				agentHost.addSession(
					createSession("created-from-send", { summary: "Created From Send" }),
				);
				return {
					kind: "sent" as const,
					data: {} as ChatSendResult extends { kind: "sent"; data: infer D }
		...
		await provider.sendRequest(session.sessionId, chat.resource, {
			query: "hello",
		});

		assert.deepStrictEqual(
			sendOptions.map((options) => options.agentHostSessionConfig),
			[{ isolation: "worktree" }],
		);
	});
	...
	test("getSessionConfig seeds running config from session state subscription with full schema", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("seed-1", { summary: "Seeded Session" }),
			);
			const provider = createProvider(disposables, agentHost);
			provider.getSessions();
			await timeout(0);
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Seeded Session");
			assert.ok(session);
			...
			const fakeState: SessionState = {
				summary: {
					resource: AgentSession.uri("copilotcli", "seed-1").toString(),
					provider: "copilotcli",
			...
			await waitForSessionConfig(
				provider,
				session!.sessionId,
				(c) => c?.values.autoApprove === "default",
			);
	...
	test("removing a session disposes its session-state subscription", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Sub Session");
			assert.ok(session);
			...
			assert.strictEqual(
				agentHost.sessionSubscribeCounts.get(sessionUriStr),
				1,
			);
			assert.strictEqual(
				agentHost.sessionUnsubscribeCounts.get(sessionUriStr) ?? 0,
				0,
			);
			...
			assert.strictEqual(
				agentHost.sessionUnsubscribeCounts.get(sessionUriStr),
				1,
			);
		}));

	test("session-state subscription auto-releases after the idle window", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("idle-1", { summary: "Idle Session" }),
			);
			const provider = createProvider(disposables, agentHost);
			provider.getSessions();
			await timeout(0);
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Idle Session");
			assert.ok(session);
			...
			assert.strictEqual(
				agentHost.sessionSubscribeCounts.get(sessionUriStr),
				1,
				"still one wire subscribe",
			);
			assert.strictEqual(
				agentHost.sessionUnsubscribeCounts.get(sessionUriStr) ?? 0,
				0,
				"no unsubscribe yet (timer reset)",
			);
			...
			assert.strictEqual(
				agentHost.sessionUnsubscribeCounts.get(sessionUriStr),
				1,
				"wire unsubscribe after idle window",
			);
			...
			assert.strictEqual(
				agentHost.sessionSubscribeCounts.get(sessionUriStr),
				2,
				"fresh subscribe after release",
			);
		}));

	// ---- replaceSessionConfig -------

	test("replaceSessionConfig only replaces sessionMutable, non-readOnly values and preserves everything else", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("rep-1", { summary: "Replace Session" }),
			);
			const provider = createProvider(disposables, agentHost);
			provider.getSessions();
			await timeout(0);
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Replace Session");
			assert.ok(session);
			...
			const fakeState: SessionState = {
				summary: {
					resource: AgentSession.uri("copilotcli", "rep-1").toString(),
					provider: "copilotcli",
			...
			const configChanged = agentHost.dispatchedActions.find(
				(d) =>
					d.action.type === ActionType.SessionConfigChanged &&
					d.channel === sessionUri,
			);
			assert.ok(
				configChanged,
				"a SessionConfigChanged action should be dispatched",
			);
			...
			assert.deepStrictEqual(latest?.values, {
				autoApprove: "autoApprove",
				isolation: "worktree",
				branch: "main",
			});
	...
	test("running session config writes clamp autoApprove to default when policy disables global auto-approve", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("policy-write", { summary: "Policy Write Session" }),
			);
			...
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Policy Write Session");
			assert.ok(session);
			...
			const fakeState: SessionState = {
				summary: {
					resource: AgentSession.uri("copilotcli", "policy-write").toString(),
					provider: "copilotcli",
			...
			const sessionUri = AgentSession.uri(
				"copilotcli",
				"policy-write",
			).toString();
			const setConfigChanged = agentHost.dispatchedActions.find(
				(d) =>
					d.action.type === ActionType.SessionConfigChanged &&
					d.channel === sessionUri,
			);
			...
			const replaceConfigChanged = agentHost.dispatchedActions.find(
				(d) =>
					d.action.type === ActionType.SessionConfigChanged &&
					d.channel === sessionUri,
			);
	...
	test("replaceSessionConfig is a no-op when nothing editable actually changes", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("rep-2", { summary: "No-op Session" }),
			);
			const provider = createProvider(disposables, agentHost);
			provider.getSessions();
			await timeout(0);
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "No-op Session");
			assert.ok(session);
			...
			const fakeState: SessionState = {
				summary: {
					resource: AgentSession.uri("copilotcli", "rep-2").toString(),
					provider: "copilotcli",
					title: "No-op Session",
					status: ProtocolSessionStatus.Idle,
					createdAt: 0,
			...
			assert.strictEqual(
				agentHost.dispatchedActions.length,
				before,
	...
	test("server-echoed SessionConfigChanged merges config values into the running cache by default", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("cfg-merge", { summary: "Merge Session" }),
			);
			const provider = createProvider(disposables, agentHost);
			provider.getSessions();
			await timeout(0);
			...
			const fakeState: SessionState = {
				summary: {
					resource: AgentSession.uri("copilotcli", "cfg-merge").toString(),
					provider: "copilotcli",
					title: "Merge Session",
					status: ProtocolSessionStatus.Idle,
					createdAt: 0,
			...
			assert.deepStrictEqual(updated?.values, {
				autoApprove: "autoApprove",
				isolation: "worktree",
			});
		}));

	test("server-echoed SessionConfigChanged with replace:true overwrites the running cache", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.addSession(
				createSession("cfg-replace", { summary: "Replace Session" }),
			);
			const provider = createProvider(disposables, agentHost);
			provider.getSessions();
			await timeout(0);
			...
			const fakeState: SessionState = {
				summary: {
					resource: AgentSession.uri("copilotcli", "cfg-replace").toString(),
					provider: "copilotcli",
					title: "Replace Session",
					status: ProtocolSessionStatus.Idle,
					createdAt: 0,
...
suite(
	"LocalAgentHostSessionsProvider - active-session branch changeset subscription",
	() => {
		const disposables = new DisposableStore();
		let agentHost: MockAgentHostService;
		...
		function makeActive(
			rawId: string,
			sessionType: string = "copilotcli",
		): IActiveSession {
			return {
				// providerId: 'unused',
				sessionType,
				resource: URI.from({
					scheme: `agent-host-${sessionType}`,
					path: `/${rawId}`,
				}),
			} as unknown as IActiveSession;
		}

		function branchChangesKeyFor(
			rawId: string,
			sessionType: string = "copilotcli",
		): string {
			...
			return [
				{
					...
					uriTemplate: branchChangesKeyFor(rawId, sessionType),
					additions,
					deletions,
					files: 1,
				},
		...
		function addAndObserve(
			...
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === `Session ${rawId}`);
		...
		test("rotates the subscription when the active session changes", () => {
			...
			activeSession.set(makeActive("sess-A"), undefined);
			assert.strictEqual(
				agentHost.sessionSubscribeCounts.get(branchChangesKeyFor("sess-A")) ??
					0,
				1,
				"A should be subscribed once on activation",
			);

			activeSession.set(makeActive("sess-B"), undefined);
			assert.strictEqual(
				agentHost.sessionSubscribeCounts.get(branchChangesKeyFor("sess-B")) ??
					0,
				1,
				"B should be subscribed once on activation",
			);
			assert.strictEqual(
				agentHost.sessionUnsubscribeCounts.get(branchChangesKeyFor("sess-A")) ??
					0,
				1,
				"A should be unsubscribed when no longer active",
			);
		});
		...
		test("switching back to a previously-active session re-subscribes", () => {
			...
			assert.strictEqual(
				subsForA,
				2,
				"switching back to A must open a fresh subscription",
			);
		});
		...
		test("releases the subscription when no session is active", () => {
			...
			assert.strictEqual(
				unsubsForA,
				1,
				"leaving the agents window (no active session) must release the subscription",
			);
		});
				...
				files: [
					{
						...
						edit: {
							before: {
								...
								content: { uri: "session-db:///before/file.ts" },
							},
							after: {
								uri: "file:///repo/file.ts",
								content: { uri: "file:///repo/file.ts" },
							},
				...
				[
					{
						uri: "file:///repo/file.ts",
						originalUri:
							"vscode-agent-host://local/session-db/-/before/file.ts",
		...
		test("changes summary tracks the live branch changeset while active and the catalogue once inactive", () => {
			...
			assert.deepStrictEqual(session.changesSummary?.get(), {
				additions: 2,
				deletions: 1,
				files: 1,
			});
			...
			// takes over again.
			activeSession.set(makeActive("sess-B"), undefined);
			fireSessionSummaryChanged(agentHost, "sess-A", {
				changesets: catalogueFor("sess-A", 5, 3),
			});
