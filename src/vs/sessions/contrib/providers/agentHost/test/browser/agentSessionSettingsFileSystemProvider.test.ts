...
suite("AgentSessionSettingsFileSystemProvider", () => {
	...
	interface IMockAgentHostSessionsProvider extends IAgentHostSessionsProvider {
		...
		readonly onDidChangeSessionsEmitter: Emitter<{
	...
	function createHarness(
		...
		const onDidChangeSessionsEmitter = store.add(
			new Emitter<{
				added: readonly ISession[];
				removed: readonly ISession[];
				changed: readonly ISession[];
		...
		const onDidChangeProvidersEmitter = store.add(
			new Emitter<{
		...
		const providersService: ISessionsProvidersService = {
			getProvider<T extends ISessionsProvider>(
				providerId: string,
			): T | undefined {
			...
			getProviders: () =>
				registerProvider
					? [sessionProvider as unknown as ISessionsProvider]
					: [],
		...
		const schemaRegistrar = store.add(
			instantiationService.createInstance(AgentSessionSettingsSchemaRegistrar),
		);
		const fs = store.add(
		...
		return {
			fs,
			session,
			uri: agentSessionSettingsUri(session),
			sessionProvider,
		};
	}
	...
	test("writeFile with unchanged content still forwards raw input (provider guards/short-circuits)", async () => {
		...
		await fs.writeFile(uri, current, {
			create: false,
			overwrite: true,
			unlock: false,
			atomic: false,
	...
	test("writeFile forwards the user's parsed JSON as the replace payload", async () => {
		...
		const newContent = VSBuffer.fromString(
	...
	suite("schema registration", () => {
		...
		test("readFile lazily registers a schema + association for the session", async () => {
			...
			assert.strictEqual(
				schemaRegistry.getSchemaAssociations()[schemaId],
				undefined,
			);
			...
			assert.deepStrictEqual(schemaRegistry.getSchemaAssociations()[schemaId], [
				uri.toString(),
			]);
		});
		...
		test("schema is refreshed when onDidChangeSessionConfig fires with a new schema identity", async () => {
			...
			const refreshed =
				schemaRegistry.getSchemaContributions().schemas[schemaId];
			assert.notStrictEqual(refreshed, initial);
			assert.ok(
				refreshed.properties?.["mode"],
				"refreshed schema should include the newly added property",
			);
		});
		...
		test("schema is disposed when the session is removed", async () => {
			...
			sessionProvider.onDidChangeSessionsEmitter.fire({
				added: [],
				removed: [session],
				changed: [],
			});
