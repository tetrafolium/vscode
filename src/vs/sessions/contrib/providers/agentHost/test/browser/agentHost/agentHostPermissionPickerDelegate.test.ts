...
} from "../../../browser/agentHostPermissionPickerDelegate.js";
...
interface ITestRig {
	...
	readonly activeSessionObs: ReturnType<
...
function setup(
	...
	const onDidChangeProviders = store.add(
	...
	const activeSessionObs = observableValue<IActiveSession | undefined>(
		"activeSession",
		activeSession,
	);
	...
	const delegate = store.add(
		insta.createInstance(AgentHostPermissionPickerDelegate),
	);
...
suite("AgentHostPermissionPickerDelegate", () => {
	...
	test("returns Default when there is no active session", () => {
		...
		assert.strictEqual(
			delegate.currentPermissionLevel.get(),
...
suite("isWellKnownAutoApproveSchema", () => {
	...
	function schema(
		overrides: Partial<SessionConfigPropertySchema> = {},
	): SessionConfigPropertySchema {
		return {
			title: "Auto Approve",
			description: "desc",
	...
	test('matches a subset that still contains "default"', () => {
		assert.strictEqual(
			isWellKnownAutoApproveSchema(
				schema({ enum: ["default", "autoApprove"] }),
			),
			true,
		);
		assert.strictEqual(
			isWellKnownAutoApproveSchema(schema({ enum: ["default"] })),
			true,
		);
	});
	...
	test('rejects schemas missing the required "default" value', () => {
		assert.strictEqual(
			isWellKnownAutoApproveSchema(
				schema({ enum: ["autoApprove", "autopilot"] }),
			),
			false,
		);
	});
	...
	test("rejects schemas with unknown enum values", () => {
		assert.strictEqual(
			isWellKnownAutoApproveSchema(schema({ enum: ["default", "custom"] })),
			false,
		);
	});
	...
	test("rejects non-string types and missing/empty enums", () => {
		assert.strictEqual(
			isWellKnownAutoApproveSchema(schema({ type: "number" as "string" })),
			false,
		);
		assert.strictEqual(
			isWellKnownAutoApproveSchema(schema({ enum: undefined })),
			false,
		);
		assert.strictEqual(
			isWellKnownAutoApproveSchema(schema({ enum: [] })),
			false,
		);
	});
});
...
suite("isWellKnownModeSchema", () => {
	...
	test('matches a subset that still contains "interactive"', () => {
		assert.strictEqual(
			isWellKnownModeSchema(schema({ enum: ["interactive"] })),
			true,
		);
	});
	...
	test('rejects schemas missing the required "interactive" value', () => {
		assert.strictEqual(
			isWellKnownModeSchema(schema({ enum: ["plan"] })),
			false,
		);
	});
	...
	test("rejects non-string types and missing/empty enums", () => {
		assert.strictEqual(
			isWellKnownModeSchema(schema({ type: "number" as "string" })),
			false,
		);
		assert.strictEqual(
			isWellKnownModeSchema(schema({ enum: undefined })),
			false,
		);
...
suite("isWellKnownClaudePermissionModeSchema", () => {
	...
	test('matches a subset that still contains "default"', () => {
		assert.strictEqual(
			isWellKnownClaudePermissionModeSchema(
				schema({ enum: ["default", "acceptEdits"] }),
			),
			true,
		);
	});
	...
	test("rejects non-string types and missing enums", () => {
		assert.strictEqual(
			isWellKnownClaudePermissionModeSchema(
				schema({ type: "number" as "string" }),
			),
			false,
		);
		assert.strictEqual(
			isWellKnownClaudePermissionModeSchema(schema({ enum: undefined })),
			false,
		);
	});
});
