...
suite("agentHostAgentPicker", () => {
	...
	const alpha: AgentCustomization = {
		type: CustomizationType.Agent,
		id: "agent://a",
		uri: "agent://a",
		name: "alpha",
	};
	const beta: AgentCustomization = {
		type: CustomizationType.Agent,
		id: "agent://b",
		uri: "agent://b",
		name: "beta",
	...
	suite("resolveAgentHostAgent", () => {
		...
		test("returns undefined when neither session nor stored selection matches the list", () => {
			assert.strictEqual(
				resolveAgentHostAgent(agents, undefined, "agent://missing"),
				undefined,
			);
			assert.strictEqual(
				resolveAgentHostAgent(agents, "agent://missing", undefined),
				undefined,
			);
		});
		...
		test("returns undefined for an empty agent list", () => {
			assert.strictEqual(
				resolveAgentHostAgent([], "agent://a", "agent://a"),
				undefined,
			);
			assert.strictEqual(
				resolveAgentHostAgent([], undefined, undefined),
				undefined,
			);
		});
	});
