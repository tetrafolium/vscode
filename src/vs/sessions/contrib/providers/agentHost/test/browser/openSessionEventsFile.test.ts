...
suite("openSessionEventsFile resolveEventsUri", () => {
	...
	function context(values: Record<string, ContextKeyValue>): IContext {
		return {
			getValue: <T extends ContextKeyValue = ContextKeyValue>(
				key: string,
			): T | undefined => values[key] as T | undefined,
		};
	}

	test("workbench command is disabled in the Agents window", () => {
		const workbenchPrecondition = new OpenCopilotCliStateFileAction().desc
			.precondition;
		const sessionsPrecondition = new OpenSessionEventsFileAction().desc
			.precondition;

		assert.deepStrictEqual(
			{
				workbenchVSCodeWindow: workbenchPrecondition?.evaluate(
					context({
						[ChatContextKeys.enabled.key]: true,
						[IsSessionsWindowContext.key]: false,
					}),
				),
				workbenchAgentsWindow: workbenchPrecondition?.evaluate(
					context({
						[ChatContextKeys.enabled.key]: true,
						[IsSessionsWindowContext.key]: true,
					}),
				),
				sessionsCopilotCliSession: sessionsPrecondition?.evaluate(
					context({
						[ChatContextKeys.enabled.key]: true,
						[IsAgentHostSession.key]: false,
					}),
				),
				sessionsAgentHostSession: sessionsPrecondition?.evaluate(
					context({
						[ChatContextKeys.enabled.key]: true,
	...
	test("local AH copilotcli session resolves to ~/.copilot/session-state/<id>/events.jsonl", () => {
		...
		assert.deepStrictEqual(
			...
			{
				kind: "ok",
				resource: "file:///home/me/.copilot/session-state/abc/events.jsonl",
			},
		);
	});
	...
	test("remote copilotcli session wraps host events.jsonl in vscode-agent-host URI", () => {
		...
		assert.deepStrictEqual(
			...
			},
			{
				kind: "ok",
				resource:
					"vscode-agent-host://localhost__4321/file/-/home/remote/.copilot/session-state/xyz/events.jsonl",
			},
		);
	});
