...
import {
	agentHostModelPickerStorageKey,
	resolveAgentHostModel,
} from "../../browser/agentHostModelPicker.js";
...
suite("AgentHostModelPicker", () => {
	...
	test("uses the current session model from session state", () => {
		...
		assert.strictEqual(
			resolveAgentHostModel(
				models,
				"agent-host-copilotcli:session",
				"agent-host-copilotcli:other",
			),
			models[1],
		);
	});
	...
	test("does not synthesize a model for existing sessions without one in state", () => {
		const models = [makeModel("agent-host-copilotcli:first")];

		assert.strictEqual(
			resolveAgentHostModel(models, undefined, undefined),
			undefined,
		);
	});
	...
	test("uses the stored model for new untitled sessions with no model yet", () => {
		...
		assert.strictEqual(
			resolveAgentHostModel(models, undefined, "agent-host-copilotcli:stored"),
			models[1],
		);
	});
