...
} from "../../../../../../workbench/contrib/chat/browser/actions/chatExecuteActions.js";
import {
	IChatPhoneInputPresenter,
	IChatPhonePresenterImpl,
} from "../../../../../../workbench/contrib/chat/browser/widget/input/chatPhoneInputPresenter.js";
 ...
 */
class MobileChatPhoneInputPresenter
	extends Disposable
	implements IChatPhonePresenterImpl
{
	...
	async showCombinedModeAndModelSheet(
		...
		const rawProvider = activeSession
			? this._sessionsProvidersService.getProvider(activeSession.providerId)
			: undefined;
		const agentHostProvider =
			rawProvider && isAgentHostProvider(rawProvider) ? rawProvider : undefined;
		...
		if (activeSession && agentHostProvider) {
			...
			const agentHostModels = this._languageModelsService
				.getLanguageModelIds()
				.map((id) => {
		...
		const performAction = (action: ChatPhonePickerAction): void => {
			...
			const provider = session
				...
				: undefined;
			const ahProvider =
				provider && isAgentHostProvider(provider) ? provider : undefined;
			...
			switch (action.kind) {
				case "mode":
					...
					this._commandService
						.executeCommand(ToggleAgentModeActionId, {
							modeId: action.mode.id,
							sessionResource: modeDelegate?.sessionResource(),
				...
				case "agentHostModel":
					...
					if (modelDelegate) {
						...
						this._chatWidgetService
							.getWidgetBySessionResource(session.resource)
...
class MobileChatPhoneInputPresenterContribution
	extends Disposable
