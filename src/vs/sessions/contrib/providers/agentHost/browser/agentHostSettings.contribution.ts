...
} from "./agentHostSettingsFileSystemProvider.js";
 ...
 */
class AgentHostSettingsContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const schemaRegistrar = this._register(
			instantiationService.createInstance(AgentHostSettingsSchemaRegistrar),
		);
		...
		this._register(
			fileService.registerProvider(AGENT_HOST_SETTINGS_SCHEME, provider),
		);
...
registerWorkbenchContribution2(
	AgentHostSettingsContribution.ID,
	AgentHostSettingsContribution,
	WorkbenchPhase.AfterRestored,
);

registerAction2(
	class OpenHostSettingsAction extends Action2 {
		constructor() {
			super({
