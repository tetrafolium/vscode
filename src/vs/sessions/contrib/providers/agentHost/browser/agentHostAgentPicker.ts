...
class AgentHostAgentPickerContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const factory = (
			...
			return scopedInstantiationService.createInstance(
				AgentHostModePickerActionViewItem,
				picker,
				disposableStore,
			);
		};
	...
	private _syncChatInputMode(
		...
		const nextMode = selectedAgentUri
			? { id: selectedAgentUri, kind: ChatModeKind.Agent }
			: { id: ChatMode.Agent.id, kind: ChatModeKind.Agent };
	...
	private _syncVisibleChatInputMode(session: ISession, modeId: string): void {
		...
		const apply = async () => {
			...
			if (
				widget.viewModel?.model.sessionResource.toString() !==
		...
		apply().catch((err) =>
			this.logService.error(
				"[AgentHostAgentPickerProbe] sync visible chat input mode failed",
				err,
			),
		);
	}
	...
	private _selectMode(
		...
		if (mode.id === ChatMode.Agent.id) {
			...
			this._setAgent(session, provider, {
				uri: rawAgentUri,
				name: mode.name.get(),
			});
		}
	...
	private _setAgent(
		...
		agent: ISessionAgentRef | undefined,
	): void {
		...
		provider.setAgent?.(
			session.sessionId,
			agent ? { uri: agent.uri, name: agent.name } : undefined,
		);
	}
}
