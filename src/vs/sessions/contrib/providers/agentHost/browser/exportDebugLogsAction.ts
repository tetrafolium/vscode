...
import {
	...
	IActiveAgentHostSessionForExport,
} from "../../../../../workbench/contrib/chat/browser/actions/exportAgentHostDebugLogsAction.js";
...
export class ExportAgentHostDebugLogsAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const activeAgentHostSession = isAgentHostSession(
			activeSession,
		...
		const sessionForEvents =
			activeAgentHostSession ??
			getMostRecentAgentHostSession(
				sessionsManagementService.getSessions(),
				sessionsProvidersService,
			);

		const activeSessionContext: IActiveAgentHostSessionForExport | undefined =
			sessionForEvents
				? {
...
function isAgentHostSession(
	session: ISession | undefined,
	sessionsProvidersService: ISessionsProvidersService,
): session is ISession {
	return (
		!!session &&
		sessionsProvidersService.getProvider(session.providerId) instanceof
			BaseAgentHostSessionsProvider
	);
}

function getMostRecentAgentHostSession(
	...
	for (const session of sessions) {
		...
		if (
			!mostRecent ||
			session.updatedAt.get().getTime() > mostRecent.updatedAt.get().getTime()
		) {
			mostRecent = session;
		}
	}
