...
export function resolveAgentHostModel(
	...
	return storedModelId
		? models.find((model) => model.identifier === storedModelId)
		: undefined;
}

class AgentHostModelPickerContribution
	extends Disposable
	...
	constructor(
		...
		this._register(
			actionViewItemService.register(
				...
				"sessions.agentHost.modelPicker",
				(_action, _options, scopedInstantiationService) => {
					...
					const modelPicker = scopedInstantiationService.createInstance(
						ModelPickerActionItem,
						action,
						delegate,
						pickerOptions,
					...
					const initModel = (
						...
						const storedModelId = isUntitled
							...
							: undefined;
						const resolvedModel = resolveAgentHostModel(
							models,
							sessionModelId,
							storedModelId,
						);
					...
					disposableStore.add(
						autorun((reader) => {
							const session =
								sessionsManagementService.activeSession.read(reader);
							const sessionModelId = session?.modelId.read(reader);
							const isUntitled =
								session?.status.read(reader) === SessionStatus.Untitled;
...
class AgentHostPickerActionViewItem extends BaseActionViewItem {
	constructor(
		...
		newChatModelPickerService: INewChatModelPickerService,
	) {
		super(undefined, {
			id: "",
			label: "",
