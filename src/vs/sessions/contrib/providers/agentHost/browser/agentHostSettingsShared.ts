 ...
 * `true`.
 */
export type AgentHostConfigPropertyFilter = (
	key: string,
 ...
 * service.
 */
export function convertPropertySchema(
	schema: ConfigPropertySchema,
): IJSONSchema {
 ...
 */
export function buildAgentHostConfigJsonSchema(
	config: IAgentHostConfigLike,
 ...
 */
export abstract class AbstractAgentHostConfigFileSystemProvider<
	TContext extends IAgentHostSettingsContext,
>
	extends Disposable
	implements IFileSystemProviderWithFileReadWriteCapability
{
	 ...
	 */
	protected abstract _watchChanges(
		provider: IAgentHostSessionsProvider,
		ctx: TContext,
		fire: () => void,
	): IDisposable;
	...
	protected abstract _ensureSchemaRegistered(
		provider: IAgentHostSessionsProvider,
		ctx: TContext,
	): void;

	/** Whether the backing config is currently available. */
	protected abstract _hasConfig(
		provider: IAgentHostSessionsProvider,
		ctx: TContext,
	): boolean;

	/** Dispatch a replace write of the parsed JSONC document. */
	protected abstract _replaceConfig(
		provider: IAgentHostSessionsProvider,
		ctx: TContext,
	...
	private _resolveOrThrow(resource: URI): {
		provider: IAgentHostSessionsProvider;
		ctx: TContext;
	} {
 ...
 */
export abstract class AbstractAgentHostConfigSchemaRegistrar<
	TTarget,
> extends Disposable {
	...
	protected abstract _targetsForProvider(
		provider: IAgentHostSessionsProvider,
	): readonly TTarget[];
