...
} from "../../browser/localChatSessionsProvider.js";
...
function createMockModel(
	...
	opts?: {
		title?: string;
		requestInProgress?: IObservable<boolean>;
		timing?: IChatSessionTiming;
	},
): IChatModel {
	let workingDirectory: URI | undefined;
	const requestInProgress =
		opts?.requestInProgress ??
		observableValue<boolean>("requestInProgress", false);
	const timing: IChatSessionTiming = opts?.timing ?? {
		created: 1_000,
	...
	return new (class extends mock<IChatModel>() {
		...
		override get workingDirectory() {
			return workingDirectory;
		}
		override setWorkingDirectory(uri: URI | undefined): void {
...
class MockChatService extends Disposable {
	...
	private readonly _onDidSubmitRequest = this._register(
		new Emitter<{ readonly chatSessionResource: URI }>(),
	);
...
async function commitNewSession(
	provider: LocalChatSessionsProvider,
): Promise<ISession> {
	...
	await provider.sendRequest(newSession.sessionId, chat.resource, {
		query: "hello",
	});
	return newSession;
}
...
suite("LocalChatSessionsProvider", () => {
	...
	test("declares Local session type", () => {
		...
		const provider = store.add(
			instantiationService.createInstance(LocalChatSessionsProvider),
		);
	...
	test("resolveWorkspace handles only file uris", () => {
		...
		assert.strictEqual(
			provider.resolveWorkspace(URI.parse("http://example.com")),
			undefined,
		);
	...
	test("status follows model.requestInProgress after a submit event", async () => {
		...
		chatService.registerModel(
			createMockModel(session.resource, { requestInProgress: inProgress }),
		);
