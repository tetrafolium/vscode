...
} from "./localChatSessionsProvider.js";
...
class LocalSessionsProviderContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		if (
			!configurationService.getValue<boolean>(LOCAL_SESSION_ENABLED_SETTING)
		) {
			return;
		}
...
registerWorkbenchContribution2(
	LocalSessionsProviderContribution.ID,
	LocalSessionsProviderContribution,
	WorkbenchPhase.AfterRestored,
);

registerAction2(
	class extends ForkConversationAction {
		protected override _openForkedSession(
			...
			return instantiationService.invokeFunction(async (accessor) => {
				...
				const parentSession = sessionsManagementService.getSession(
					parentSessionResource,
				);
				if (!parentSession) {
					logService.error(
						`Parent session ${parentSessionResource.toString()} not found when forking conversation`,
					);
					return super._openForkedSession(
				...
				if (!sessionsManagementService.getSession(forkedSessionResource)) {
					...
					const appeared = await raceTimeout(
						new Promise<boolean>((resolve) => {
							listener = sessionsManagementService.onDidChangeSessions(() => {
								if (
									sessionsManagementService.getSession(forkedSessionResource)
								) {
									resolve(true);
								}
							});
					...
					if (!appeared) {
						logService.error(
							`Forked session ${forkedSessionResource.toString()} did not appear within timeout`,
						);
						return;
					}
				}
