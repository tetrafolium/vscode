...
import {
	...
	type IRemoteAgentHostSessionsProviderConfig,
} from "../../browser/remoteAgentHostSessionsProvider.js";
...
class MockAgentConnection extends mock<IAgentConnection>() {
	...
	private _rootStateValue: RootState = {
		agents: [
			{
				provider: "copilotcli",
				displayName: "Copilot",
...
function createProvider(
	...
	instantiationService.stub(
		IStorageService,
		overrides?.storageService ?? disposables.add(new InMemoryStorageService()),
	);
	...
	const config: IRemoteAgentHostSessionsProviderConfig = {
		...
		name:
			overrides !== undefined &&
			Object.prototype.hasOwnProperty.call(overrides, "connectionName")
				? (overrides.connectionName ?? "")
				: "Test Host",
	};

	const providerCtor =
		overrides?.isWebPlatform !== undefined
			? class extends RemoteAgentHostSessionsProvider {
					protected override get isWebPlatform(): boolean {
						return overrides.isWebPlatform!;
					}
				}
			: RemoteAgentHostSessionsProvider;
	const provider = disposables.add(
		instantiationService.createInstance(providerCtor, config),
	);
...
async function waitForSessionConfig(
	provider: RemoteAgentHostSessionsProvider,
	sessionId: string,
...
suite("RemoteAgentHostSessionsProvider", () => {
	...
	test("session types update when the host advertises additional agents", () => {
		...
		assert.deepStrictEqual(
			provider.sessionTypes.map((t) => t.id),
	...
	test("onDidChangeSessions fires when session added notification arrives", () => {
		...
		disposables.add(
			provider.onDidChangeSessions((e: ISessionChangeEvent) => changes.push(e)),
		);
	...
	test("session added notifications ingest any advertised agent provider", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			fireSessionAdded(connection, "cop-1", {
				provider: "copilotcli",
				title: "Copilot Session",
			});
			fireSessionAdded(connection, "oai-1", {
				provider: "openai",
			...
			assert.deepStrictEqual(
				sessions
					.map((s) => ({ title: s.title.get(), sessionType: s.sessionType }))
	...
	test("uses project metadata as workspace group source", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const projectUri = URI.parse(
				"vscode-agent-host://localhost__4321/file/-/home/user/vscode",
			);
			const workingDirectory = URI.parse(
				"vscode-agent-host://localhost__4321/file/-/tmp/copilot-worktrees/vscode-feature",
			);
	...
	test("session added converts file project URIs and preserves repository URLs", () => {
		...
		const workspaces = provider
			.getSessions()
			.map((session) => session.workspace.get());
		assert.deepStrictEqual(
			workspaces.map((workspace) => workspace?.folders[0]?.root.toString()),
			[
				"vscode-agent-host://localhost__4321/file/-/home/user/vscode",
	...
	test("getSessions populates from connection.listSessions", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const changes: ISessionChangeEvent[] = [];
			disposables.add(
				provider.onDidChangeSessions((e: ISessionChangeEvent) =>
					changes.push(e),
				),
			);
	...
	test("uses model metadata as selected model for listed sessions", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				session?.modelId.get(),
				"remote-localhost__4321-copilotcli:claude-sonnet-4.5",
			);
		}));

	test("uses model metadata from session added notification", () => {
		const provider = createProvider(disposables, connection);
		fireSessionAdded(connection, "notif-model", {
		...
		assert.strictEqual(
			session?.modelId.get(),
			"remote-localhost__4321-copilotcli:gpt-5",
		);
	});

	test("setModel updates existing session model and dispatches raw model", () => {
		const provider = createProvider(disposables, connection);
		fireSessionAdded(connection, "set-model", {
		...
		provider.setModel(
			session!.sessionId,
			"remote-localhost__4321-copilotcli:new-model",
		);
	...
	test("setModel preserves current model config when model id is unchanged", () => {
		const provider = createProvider(disposables, connection);
		fireSessionAdded(connection, "set-model-config", {
		...
		provider.setModel(
			session!.sessionId,
			"remote-localhost__4321-copilotcli:configured-model",
		);
	...
	test("createNewSession returns session with correct fields", () => {
		...
		const session = provider.createNewSession(
			URI.parse("vscode-agent-host://auth/home/user/project"),
	...
	test("createNewSession clears session config when resolving config is unavailable", async () => {
		...
		const workspaceUri = URI.parse(
			"vscode-agent-host://auth/home/user/project",
		);
		const session = provider.createNewSession(
			workspaceUri,
			provider.sessionTypes[0].id,
	...
	test("deleteSession calls disposeSession with backend agent URI and removes from cache", async () => {
		...
		assert.strictEqual(
			remaining.find((s) => s.title.get() === "To Delete"),
			undefined,
		);
	});
	...
	test("renameSession increments clientSeq on successive calls", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("server-echoed SessionModelChanged updates cached model", () => {
		const provider = createProvider(disposables, connection);
		fireSessionAdded(connection, "model-change", {
			title: "Model Change",
			model: "old-model",
		});
	...
	test("renamed title survives session refresh from listSessions", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			// Simulate server persisting the renamed title: after rename, listSessions
			// returns the updated summary
			connection.addSession(
				createSession("persist-sess", { summary: "Original Title" }),
			);
			const provider = createProvider(disposables, connection);
			provider.getSessions();
			await timeout(0);
	...
	test("cached session loading reflects authenticationPending", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("cached-auth", { summary: "Cached" }),
			);
			const provider = createProvider(disposables, connection);
			await timeout(0);
	...
	test("unpublishCachedSessions hides sessions but retains persisted cache", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.deepStrictEqual(
				{
					...
					eventRemovedTitles: events.flatMap((e) =>
						e.removed.map((s) => s.title.get()),
					),
				},
			...
			const provider2 = createProvider(disposables, new MockAgentConnection(), {
				storageService,
				noConnection: true,
			});
			assert.deepStrictEqual(
	...
	test("setConnection after unpublishCachedSessions restores cached sessions", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("restore-me", { summary: "Restore Me" }),
			);
			const provider = createProvider(disposables, connection);
			await timeout(0);
			assert.strictEqual(provider.getSessions().length, 1);
			...
			reconnected.addSession(
				createSession("restore-me", { summary: "Restored" }),
			);
		...
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("ws-sess", {
					summary: "WS Test",
					workingDirectory: URI.parse(
						"vscode-agent-host://localhost__4321/file/-/home/user/myrepo",
					),
				}),
			);
	...
	test("turnComplete action triggers session refresh for matching provider", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
	...
	test("removing a session disposes its session-state subscription", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("seed-2", { summary: "Sub Session" }),
			);
			const provider = createProvider(disposables, connection);
			provider.getSessions();
			await timeout(0);
			...
			assert.strictEqual(
				connection.sessionSubscribeCounts.get(sessionUriStr),
				1,
			);
			assert.strictEqual(
				connection.sessionUnsubscribeCounts.get(sessionUriStr) ?? 0,
				0,
			);
			...
			assert.strictEqual(
				connection.sessionUnsubscribeCounts.get(sessionUriStr),
				1,
			);
		}));

	test("replacing the connection disposes all session-state subscriptions", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("seed-3", { summary: "Reconnect Session" }),
			);
			const provider = createProvider(disposables, connection);
			provider.getSessions();
			await timeout(0);
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Reconnect Session");
			assert.ok(session);
	...
	test("non-web: session workspace from project metadata includes [host] suffix", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const projectUri = URI.parse(
				"vscode-agent-host://localhost__4321/file/-/home/user/vscode",
			);
			...
			assert.strictEqual(
				provider.getSessions()[0].workspace.get()?.label,
				"vscode [Test Host]",
			);
		}));

	test("non-web: session workspace from working directory includes [host] suffix", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("ws-sess", {
					summary: "WS Test",
			...
			const wsSession = provider
				.getSessions()
				.find((s) => s.title.get() === "WS Test");
			assert.strictEqual(
				wsSession?.workspace.get()?.label,
				"myrepo [Test Host]",
			);
		}));
	...
	test("non-web: idle session description is undefined", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("desc-sess", { summary: "Desc Test" }),
			);
			...
			const session = provider
				.getSessions()
				.find((s) => s.title.get() === "Desc Test");
			assert.strictEqual(session?.description.get(), undefined);
		}));

	test("web: session description is undefined (host filter dropdown replaces it)", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			connection.addSession(
				createSession("desc-sess-web", { summary: "Desc Web" }),
			);
