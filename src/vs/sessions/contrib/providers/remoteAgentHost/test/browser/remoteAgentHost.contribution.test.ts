...
import {
	...
	sshConnectionKey,
	SSHReconnectState,
} from "../../browser/remoteAgentHost.contribution.js";
...
suite("SSHReconnectState", () => {
	...
	test("hasPendingTimer becomes false once the handler has run", async () => {
		...
		return runWithFakedTimers(
			...
			async () => {
				const state = store.add(new SSHReconnectState());
				state.scheduleRetry(1000, () => {
					/* no follow-up */
				});
				await timeout(1100);
				assert.strictEqual(
					state.hasPendingTimer,
					false,
		...
		return runWithFakedTimers(
			...
			async () => {
				...
				assert.strictEqual(
					fired,
					0,
					"pending retry must be cancelled by resetForResume",
				);
			},
		);
	});
...
suite("shouldPauseSSHReconnectAfterFailure", () => {
	...
	test("pauses reconnect after cancellation but not after regular failures", () => {
		assert.deepStrictEqual(
			{
				cancellation: shouldPauseSSHReconnectAfterFailure(
...
suite("disconnectSSHEntry", () => {
	...
	function makeSSHConfigConnection(
		overrides: Partial<IRemoteAgentHostSSHConnection> = {},
	): IRemoteAgentHostSSHConnection {
		return {
	...
	test("removes the entry from configured storage BEFORE tearing down the SSH tunnel", async () => {
		...
		const pending = disconnectSSHEntry(
			connection,
