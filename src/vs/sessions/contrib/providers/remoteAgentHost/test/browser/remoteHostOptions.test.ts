...
suite("remoteHostOptions", () => {
	...
	test("getStatusLabel covers every connection status variant", () => {
		assert.ok(
			getStatusLabel(RemoteAgentHostConnectionStatus.connected).length > 0,
		);
		assert.ok(
			getStatusLabel(RemoteAgentHostConnectionStatus.connecting).length > 0,
		);
		assert.ok(
			getStatusLabel(RemoteAgentHostConnectionStatus.disconnected).length > 0,
		);
		...
		assert.notStrictEqual(
			incompatibleLabel,
			getStatusLabel(RemoteAgentHostConnectionStatus.disconnected),
		);
	});
	...
	test("getStatusHover surfaces the host-supplied message for incompatible status", () => {
		...
		assert.ok(
			hover.includes("0.1.0"),
			"hover should mention the offered version",
		);
		assert.ok(
			hover.includes("only supports 0.2.0"),
			"hover should include the host-supplied message",
		);
		assert.ok(
			hover.includes("host.example:1234"),
			"hover should include the address when provided",
		);
	});

	test("getStatusHover omits the address line when address is undefined", () => {
		const status = RemoteAgentHostConnectionStatus.incompatible("Some reason", [
			"0.1.0",
		]);
		const hover = getStatusHover(status);
		assert.ok(hover.includes("Some reason"));
		assert.ok(
			!hover.includes("Address"),
			"hover should not include an address line when none is given",
		);
	});
});
