	...
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor): Promise<void> {
			...
			const buildItems = (): (ManageHostsPickItem | IQuickPickSeparator)[] => {
				const remoteProviders: IAgentHostSessionsProvider[] =
					sessionsProvidersService
						.getProviders()
						.filter(isAgentHostProvider)
						.filter((p: IAgentHostSessionsProvider) => !!p.remoteAddress);

				const remoteItems: IRemoteHostQuickPickItem[] = remoteProviders.map(
					(p: IAgentHostSessionsProvider) => {
						...
						const item: IRemoteHostQuickPickItem = {
							...
							description:
								status !== undefined ? getStatusLabel(status) : undefined,
							detail: p.remoteAddress,
						};
						(
							item as IRemoteHostQuickPickItem & {
				...
				for (const [, actions] of menuActions) {
					for (const action of actions) {
						if (action instanceof MenuItemAction) {
							const icon = ThemeIcon.isThemeIcon(action.item.icon)
								? action.item.icon
								: undefined;
				...
				if (menuActionItems.length > 0) {
					items.push({
						type: "separator",
						label: localize("manageHosts.actionsHeader", "Add or Manage"),
					});
			...
			const showManagePicker = () => {
				...
				const picker = quickInputService.createQuickPick<ManageHostsPickItem>({
					useSeparators: true,
				});
				store.add(picker);
				...
				store.add(
					sessionsProvidersService.onDidChangeProviders(() => refresh()),
				);
				...
				store.add(
					sessionsProvidersService.onDidChangeProviders(() =>
						subscribeToProviders(),
					),
				);
					...
					picker.onDidAccept(() => {
						...
						if (selected.kind === "remote") {
							void instantiationService
								.invokeFunction((a) =>
									showRemoteHostOptions(a, selected.provider, {
										showBackButton: true,
									}),
								)
								.then((result) => {
