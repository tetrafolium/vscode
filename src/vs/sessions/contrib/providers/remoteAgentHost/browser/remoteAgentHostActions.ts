		...
		constructor() {
			super({
				...
				precondition: ContextKeyExpr.equals(
					`config.${RemoteAgentHostsEnabledSettingId}`,
					true,
				),
			});
		}
 ...
 */
export function parseSSHHostInput(
	value: string,
): { host: string; username?: string; port?: number } | undefined {
...
async function promptToConnectViaSSH(
	...
	const configHosts = await sshService
		.listSSHConfigHosts()
		.catch(() => [] as string[]);
	...
	const result = await new Promise<"back" | SSHHostPickerItem | undefined>(
		(resolve) => {
			const store = new DisposableStore();
			const picker = store.add(
				quickInputService.createQuickPick<SSHHostPickerItem>(),
			);
	...
	if (result.kind === "add-config" || result.kind === "configure") {
		...
		const onBackToSSH = () =>
			instantiationService.invokeFunction((a) =>
				promptToConnectViaSSH(a, options),
			);
	...
	if (!parsed) {
		notificationService.error(
			validateSSHHostInput(newHost.hostInput) ??
				localize("sshHostInvalid", "Invalid SSH host."),
		);
		return;
	}
	await instantiationService.invokeFunction((accessor) =>
		promptForCredentialsAndConnect(
			accessor,
			parsed.host,
			parsed.username,
			parsed.port,
...
async function connectToConfiguredSSHHost(
	...
	if (username) {
		...
		if (connection) {
			await instantiationService.invokeFunction((accessor) =>
				promptForRemoteFolder(accessor, connection),
			);
		}
		return;
	}
...
async function promptForCredentialsAndConnect(
	...
	if (!username) {
		const usernameInput = await quickInputService.input({
			...
			validateInput: async (value) =>
				value.trim()
					? undefined
					: localize("sshUsernameEmpty", "Enter a username."),
		});
		if (!usernameInput) {
			return;
		}
	...
	if (authMethod === SSHAuthMethod.KeyFile) {
		const keyPath = await quickInputService.input({
			...
			validateInput: async (value) =>
				value.trim()
					? undefined
					: localize("sshKeyEmpty", "Enter a key file path."),
		});
		if (!keyPath) {
			return;
		}
		...
		const pw = await quickInputService.input({
			...
			ignoreFocusLost: true,
			validateInput: async (value) =>
				value ? undefined : localize("sshPasswordEmpty", "Enter a password."),
		});
		if (!pw) {
			return;
		}
	...
	const name = await quickInputService.input({
		...
		ignoreFocusLost: true,
		validateInput: async (value) =>
			value.trim() ? undefined : localize("sshNameEmpty", "Enter a name."),
	});
	if (!name) {
		return;
	}
...
async function connectWithProgress(
	...
	const progressListener = sshService.onDidReportConnectProgress?.(
		(progress) => {
...
async function promptForRemoteFolder(
	...
	const provider = sessionsProvidersService
		.getProviders()
		.find(
			(p): p is IAgentHostSessionsProvider =>
				isAgentHostProvider(p) && p.remoteAddress === connection.localAddress,
		);
	if (!provider) {
		return;
	}
	...
	sessionsPartService
		.getSessionView(sessionsManagementService.activeSession.get()?.sessionId)
		?.selectWorkspace(folderUri);
}
...
registerAction2(
	class extends Action2 {
		constructor() {
			super({
				...
				menu: {
					...
					order: 20,
					when: SessionWorkspacePickerGroupContext.isEqualTo(
		...
		override async run(
			...
			const result = await promptToConnectViaSSH(accessor, {
	...
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor): Promise<void> {
			...
			const editorPane = await editorService.openEditor({
				resource: configUri,
				options: { pinned: true } satisfies ITextEditorOptions,
			});
			if (!editorPane) {
				return;
			}
	...
	class extends Action2 {
		...
		override async run(
			...
			const picked = await new Promise<
				"back" | ISSHConfigFilePickItem | undefined
			>((resolve) => {
				const store = new DisposableStore();
				const picker = store.add(
					quickInputService.createQuickPick<ISSHConfigFilePickItem>(),
				);
...
async function promptToConnectViaTunnel(
	...
	const scopes =
		productService.tunnelApplicationConfig?.authenticationProviders?.[
			authProvider
		]?.scopes ?? [];
	try {
		if (
			!(await authenticationService.getSessions(authProvider, scopes)).length
		) {
	...
	const picked = await new Promise<"back" | ITunnelPickItem | undefined>(
		(resolve) => {
			store.add(
				tunnelPicker.onDidTriggerButton((button) => {
					if (button === quickInputService.backButton) {
						resolve("back");
						tunnelPicker.hide();
					}
	...
	await instantiationService.invokeFunction((accessor) =>
		promptForTunnelFolder(accessor, picked.tunnel),
	);
}
...
async function promptForTunnelFolder(
	...
	const provider = sessionsProvidersService
		...
		.find(
			(p): p is IAgentHostSessionsProvider =>
				isAgentHostProvider(p) && p.remoteAddress === tunnelAddress,
		);
	if (!provider) {
		return;
	}
	...
	sessionsPartService
		.getSessionView(sessionsManagementService.activeSession.get()?.sessionId)
		?.selectWorkspace(folderUri, provider.id);
}
...
registerAction2(
	class extends Action2 {
		...
		override async run(
			...
			const result = await promptToConnectViaTunnel(accessor, {
				showBackButton: !!onBack,
			});
			if (result === "back") {
				onBack?.();
