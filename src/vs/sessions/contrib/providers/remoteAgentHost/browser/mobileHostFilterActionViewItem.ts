...
export class MobileHostFilterActionViewItem extends HostFilterActionViewItem {
	private readonly _sheet = this._register(
	...
	private _showSheet(): void {
		...
		const workbenchContainer =
			dom.findParentWithClass(this.element!, "monaco-workbench") ??
			targetDocument.body;

		// --- Overlay shell ----------------------------------------------------
		const overlay = dom.append(
			workbenchContainer,
			$("div.host-picker-sheet-overlay"),
		);
		...
		sheet.setAttribute(
			"aria-label",
			localize("agentHostFilter.sheet.aria", "Hosts"),
		);
		...
		dom.append(header, $("div.host-picker-sheet-title")).textContent = localize(
			"agentHostFilter.sheet.title",
			"Hosts",
		);
		const closeBtn = dom.append(
			header,
			$("button.host-picker-sheet-close", { type: "button" }),
		) as HTMLButtonElement;
		closeBtn.setAttribute(
			"aria-label",
			localize("agentHostFilter.sheet.close", "Close"),
		);
		dom.append(closeBtn, $("span.codicon.codicon-close"));
		disposables.add(Gesture.addTarget(closeBtn));
		for (const eventType of [dom.EventType.CLICK, TouchEventType.Tap]) {
			disposables.add(
				dom.addDisposableListener(closeBtn, eventType, (e) => {
		...
		const focusRefs: {
			firstHost?: HTMLButtonElement;
			firstCheckedHost?: HTMLButtonElement;
			rediscover?: HTMLButtonElement;
		} = {};
		const renderBody = () => {
		...
		disposables.add(
			dom.addDisposableListener(sheet, dom.EventType.CLICK, (e) =>
				e.stopPropagation(),
			),
		);
		disposables.add(Gesture.addTarget(sheet));
		disposables.add(
			dom.addDisposableListener(sheet, TouchEventType.Tap, (e) =>
				dom.EventHelper.stop(e, true),
			),
		);
		...
		for (const eventType of [dom.EventType.CLICK, TouchEventType.Tap]) {
			disposables.add(
				dom.addDisposableListener(backdrop, eventType, (e) => {
					dom.EventHelper.stop(e, true);
					finish();
				}),
			);
		...
		disposables.add(
			dom.addDisposableListener(targetDocument, dom.EventType.KEY_DOWN, (e) => {
	...
	private _renderHostList(
		...
		for (const host of hosts) {
			const row = this._renderHostItem(
				disposables,
				body,
				host,
				selectedId === host.providerId,
				finish,
			);
	...
	private _renderHostItem(
		disposables: DisposableStore,
		body: HTMLElement,
		host: IAgentHostFilterEntry,
		checked: boolean,
		finish: () => void,
	): HTMLButtonElement {
		const row = dom.append(
			body,
			$("button.host-picker-sheet-item", { type: "button" }),
		) as HTMLButtonElement;
		...
		const status = dom.append(
			iconWrap,
			$("span.host-picker-sheet-item-status"),
		);
		switch (host.status) {
		...
		dom.append(text, $("span.host-picker-sheet-item-name")).textContent =
			host.label;
		dom.append(text, $("span.host-picker-sheet-item-sub")).textContent =
		...
		disposables.add(
			dom.addDisposableListener(row, dom.EventType.CLICK, (e) => select(e)),
		);
		disposables.add(
			dom.addDisposableListener(row, TouchEventType.Tap, (e) => select(e)),
		);
		return row;
	}
	...
	private _statusLabel(status: AgentHostFilterConnectionStatus): string {
		switch (status) {
			...
			case AgentHostFilterConnectionStatus.Connecting:
				return localize(
					"agentHostFilter.sheet.status.connecting",
					"Connecting…",
				);
			case AgentHostFilterConnectionStatus.Disconnected:
			default:
				return localize(
					"agentHostFilter.sheet.status.disconnected",
					"Disconnected",
				);
		}
	}

	private _renderRediscoverAction(
		disposables: DisposableStore,
		footer: HTMLElement,
	): HTMLButtonElement {
		const action = dom.append(
			footer,
			$("button.host-picker-sheet-action", { type: "button" }),
		) as HTMLButtonElement;
		action.setAttribute("role", "menuitem");
		action.setAttribute(
			"aria-label",
			localize("agentHostFilter.sheet.rediscover.aria", "Re-discover hosts"),
		);

		const iconSpan = dom.append(
			action,
			$("span.host-picker-sheet-action-icon"),
		);
		...
		disposables.add(
			dom.addDisposableListener(action, dom.EventType.CLICK, (e) => trigger(e)),
		);
		disposables.add(
			dom.addDisposableListener(action, TouchEventType.Tap, (e) => trigger(e)),
		);
		return action;
	}
}
