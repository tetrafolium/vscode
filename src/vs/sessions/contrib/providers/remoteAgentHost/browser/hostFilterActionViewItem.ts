...
export class HostFilterActionViewItem extends BaseActionViewItem {
	...
	constructor(
		...
		@IAgentHostFilterService
		protected readonly _filterService: IAgentHostFilterService,
		@IContextMenuService
	...
	private _renderTitlebar(): void {
		...
		for (const eventType of [dom.EventType.CLICK, TouchEventType.Tap]) {
			this._register(
				dom.addDisposableListener(this._dropdownElement, eventType, (e) => {
	...
	private _renderSidebar(): void {
		...
		this._register(
			this._sidebarButton.onDidClick((e) => {
				if (!this._isInteractive()) {
					return;
				}
	...
	private _wireConnectButton(connectElement: HTMLElement): void {
		...
		for (const eventType of [dom.EventType.CLICK, TouchEventType.Tap]) {
			this._register(
				dom.addDisposableListener(connectElement, eventType, (e) => {
		...
		this._register(
			dom.addDisposableListener(connectElement, dom.EventType.KEY_DOWN, (e) => {
				const event = new StandardKeyboardEvent(e);
				if (event.equals(KeyCode.Enter) || event.equals(KeyCode.Space)) {
					dom.EventHelper.stop(e, true);
					this._onConnectClick();
	...
	private _renderSidebarButtonAffordances(
		interactive: boolean,
		canRetry: boolean,
	): void {
	...
	private _update(): void {
		...
		if (interactive) {
			...
			this._dropdownElement.setAttribute(
				...
				selected
					? localize(
							"agentHostFilter.aria.singleSelected",
							"Sessions scoped to host {0}",
							selected.label,
						)
					: localize("agentHostFilter.aria.none", "No agent host selected."),
			);
	...
	private _updateConnectButton(
		selected: IAgentHostFilterEntry | undefined,
		canRetry: boolean,
		discovering: boolean,
	): void {
	...
	protected _showMenu(e: Event): void {
		...
		for (const host of hosts) {
			const label =
				host.status === AgentHostFilterConnectionStatus.Connected
					? host.label
