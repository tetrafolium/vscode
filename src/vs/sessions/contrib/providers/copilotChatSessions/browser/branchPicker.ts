...
export class BranchPicker extends Disposable {
	...
	private _getSession(): ICopilotChatSession | undefined {
		...
		return provider instanceof CopilotChatSessionsProvider
			? provider.getSession(session.sessionId)
			: undefined;
	}
	...
	showPicker(): void {
		...
		if (
			...
			branches.length === 0 ||
			session?.isolationMode.get() === "workspace"
		) {
			return;
		}
		...
		this.actionWidgetService.show<IBranchItem>(
			...
			[],
			{
				...
				getWidgetAriaLabel: () =>
					localize("branchPicker.ariaLabel", "Branch Picker"),
			},
			totalActions > FILTER_THRESHOLD
				? {
						showFilter: true,
	...
	private _updateTriggerLabel(): void {
		...
		const isDisabled =
			session?.isolationMode.get() === "workspace" || branches.length === 0;
		const label =
			session?.branch.get() ?? localize("branchPicker.select", "Branch");
