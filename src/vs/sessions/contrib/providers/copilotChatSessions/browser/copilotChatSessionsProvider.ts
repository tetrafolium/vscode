...
import {
	...
	AgentSessionTarget,
} from "../../../../../workbench/contrib/chat/browser/agentSessions/agentSessions.js";
import {
	IChatService,
...
import {
	IGitService,
	IGitRepository,
} from "../../../../../workbench/contrib/git/common/gitService.js";
import {
	IContextKeyService,
	ContextKeyExpr,
...
const SESSION_WORKSPACE_GROUP_GITHUB = localize(
...
export interface ICopilotChatSession {
	...
	setOption?(
		optionId: string,
		value: IChatSessionProviderOptionItem | string,
	): void;
...
export const COPILOT_MULTI_CHAT_SETTING =
...
function isNewSession(session: ICopilotChatSession): session is NewSession {
	return (
		session instanceof CopilotCLISession ||
		session instanceof RemoteNewSession ||
		session instanceof ClaudeCodeNewSession
	);
}
 ...
 */
function buildChatFromSession(
	chat: Omit<ICopilotChatSession, "mainChat">,
	resource?: URI,
): IChat {
	return {
...
class CopilotCLISession extends Disposable implements ICopilotChatSession {
	...
	private readonly _description: ReturnType<
	...
	private readonly _branchObservable = observableValue<string | undefined>(
		this,
		undefined,
	);
	readonly branch: IObservable<string | undefined> = this._branchObservable;

	private readonly _isolationModeObservable = observableValue<
		IsolationMode | undefined
	>(this, "worktree");
	readonly isolationMode: IObservable<IsolationMode | undefined> =
		this._isolationModeObservable;
	...
	private readonly _changes: ReturnType<
		typeof observableValue<readonly ISessionFileChange[]>
	>;
	readonly changes: IObservable<readonly ISessionFileChange[]>;

	private readonly _checkpoints: ReturnType<
		typeof observableValueOpts<IChatCheckpoints | undefined>
	>;
	readonly checkpoints: IObservable<IChatCheckpoints | undefined>;
	...
	readonly lastTurnEnd: IObservable<Date | undefined> = observableValue(
		this,
		undefined,
	);
	readonly gitHubInfo: IObservable<IGitHubInfo | undefined> = observableValue(
		this,
		undefined,
	);

	private _gitRepository: IGitRepository | undefined;
	private readonly _loadBranchesCts = this._register(
	...
	get gitRepository(): IGitRepository | undefined {
	...
	constructor(
		...
		const storedMode = storageService.get(
			STORAGE_KEY_ISOLATION_MODE,
			StorageScope.PROFILE,
		);
		const initialMode: IsolationMode =
			storedMode === "workspace" ? "workspace" : "worktree";
		...
		this._changes = observableValueOpts<readonly ISessionFileChange[]>(
			{ owner: this, equalsFn: sessionFileChangesEqual },
			[],
		);
		this.changes = this._changes;

		this._checkpoints = observableValueOpts<IChatCheckpoints | undefined>(
	...
	private _loadBranches(repo: IGitRepository): void {
		...
		repo
			.getRefs({ pattern: "refs/heads" }, cts.token)
			.then((refs) => {
				...
				const branches = refs
					...
					.filter(
						(name) =>
							!name.includes(CopilotCLISession.COPILOT_WORKTREE_PATTERN),
					);
	...
	getAgentHostSessionConfig(): Record<string, unknown> {
		const config: Record<string, unknown> = {
			[SessionConfigKey.Isolation]:
				this._isolationMode === "worktree" ? "worktree" : "folder",
		};
		if (this._isolationMode === "worktree" && this._branch) {
	...
	update(agentSession: IAgentSession): void {
		transaction((tx) => {
			const session = new AgentSessionAdapter(
				agentSession,
				this.providerId,
				this.gitHubService,
			);
...
function isRepositoriesOptionGroup(
 ...
 * pre-send configuration methods for the new-session flow.
 */
export class RemoteNewSession
	extends Disposable
	implements ICopilotChatSession
{
	// -- ISessionData fields --
	...
	readonly changes: IObservable<readonly ISessionFileChange[]> =
		observableValueOpts<readonly ISessionFileChange[]>(
			{ owner: this, equalsFn: sessionFileChangesEqual },
			[],
		);
	...
	private readonly _onDidChangeOptionGroups = this._register(
		new Emitter<void>(),
	);
	readonly onDidChangeOptionGroups: Event<void> =
		this._onDidChangeOptionGroups.event;

	readonly selectedOptions = new Map<string, IChatSessionProviderOptionItem>();

	get project(): ISessionWorkspace | undefined {
	...
	constructor(
		...
		this._register(
			this.contextKeyService.onDidChangeContext((e) => {
				if (
					this._whenClauseKeys.size > 0 &&
					e.affectsSome(this._whenClauseKeys)
				) {
					this._onDidChangeOptionGroups.fire();
				}
			}),
	...
	getOtherOptionGroups(): ISessionOptionGroup[] {
		...
		return groups
			.filter(
				(g) =>
					!isModelOptionGroup(g) &&
					!isRepositoriesOptionGroup(g) &&
					this._isOptionGroupVisible(g),
			)
	...
	private _getValueForGroup(
		...
		const sessionOption = this.chatSessionsService.getSessionOption(
			this.resource,
			group.id,
		);
...
class AgentSessionAdapter implements ICopilotChatSession {
	...
	private readonly _workspace: ReturnType<
	...
	private readonly _modelId: ReturnType<
	...
	private readonly _lastTurnEnd: ReturnType<
		typeof observableValue<Date | undefined>
	>;
	readonly lastTurnEnd: IObservable<Date | undefined>;

	private readonly _baseGitHubInfo: ReturnType<
		typeof observableValue<IGitHubInfo | undefined>
	>;
	readonly gitHubInfo: IObservable<IGitHubInfo | undefined>;

	readonly permissionLevel: IObservable<ChatPermissionLevel> = constObservable(
		ChatPermissionLevel.Default,
	);
	readonly branch: IObservable<string | undefined> = constObservable(undefined);
	readonly isolationMode: IObservable<IsolationMode | undefined> =
	...
	constructor(
		...
		this._baseGitHubInfo = observableValue(
			this,
			this._extractGitHubInfo(session),
		);
		this.gitHubInfo = derived(this, (reader) => {
			...
			const prModelRef = reader.store.add(
				this._gitHubService.createPullRequestModelReference(
					base.owner,
					base.repo,
		...
		this._changes = observableValueOpts<readonly ISessionFileChange[]>(
			{ owner: this, equalsFn: sessionFileChangesEqual },
			this._extractChanges(session),
		);
		this.changes = this._changes;

		this._checkpoints = observableValueOpts<IChatCheckpoints | undefined>(
			{ owner: this, equalsFn: structuralEquals },
			this._extractCheckpoints(session),
		);
		this.checkpoints = this._checkpoints;
		...
		this._description = observableValue(
			this,
			this._extractDescription(session),
		);
		this.description = this._description;
		this._lastTurnEnd = observableValue(
			this,
			session.timing.lastRequestEnded
				? new Date(session.timing.lastRequestEnded)
				: undefined,
		);
	...
	private _extractDescription(
		...
		return typeof session.description === "string"
			? new MarkdownString(session.description)
			: session.description;
	}
	...
	private _extractGitHubInfo(session: IAgentSession): IGitHubInfo | undefined {
		...
		const baseRefOid =
			typeof metadata.baseRefOid === "string" ? metadata.baseRefOid : undefined;
		const headRefOid =
			typeof metadata.headRefOid === "string" ? metadata.headRefOid : undefined;
	...
	private _extractPullRequestNumber(
		session: IAgentSession,
		pullRequestUri: URI,
	): number | undefined {
		const metadata = session.metadata;
	...
	private _extractOwnerRepo(session: IAgentSession): {
		...
		if (typeof metadata.pullRequestUrl === "string") {
			const match = /github\.com\/([^/]+)\/([^/]+)\/pull\/(\d+)/.exec(
				metadata.pullRequestUrl as string,
			);
			if (match) {
	...
	private _extractPullRequestStateIcon(
		session: IAgentSession,
	): ThemeIcon | undefined {
		const metadata = session.metadata;
	...
	private _extractChanges(
		...
		const summary = session.changes as {
	...
	private _extractCheckpoints(
		session: IAgentSession,
	): IChatCheckpoints | undefined {
		const metadata = session.metadata;
		if (
			typeof metadata?.firstCheckpointRef !== "string" ||
			typeof metadata?.lastCheckpointRef !== "string"
		) {
			return undefined;
		}
	...
	private _buildWorkspace(
		...
		return {
			...
			icon:
				repoUri?.scheme === GITHUB_REMOTE_FILE_SCHEME
					? Codicon.repo
					: Codicon.folder,
			group:
				repoUri?.scheme === GITHUB_REMOTE_FILE_SCHEME
	...
	private _extractRepositoryFromMetadata(session: IAgentSession): {
		...
		if (session.providerType === AgentSessionProviders.Cloud) {
			const branch =
				typeof metadata.branch === "string" ? metadata.branch : "HEAD";
		...
		return {
			...
			hasGitOperationInProgress: metadata?.hasGitOperationInProgress as
				| boolean
				| undefined,
		};
	}
 ...
 */
export class CopilotChatSessionsProvider
	extends Disposable
	implements ISessionsProvider
{
	...
	private readonly _onDidChangeSessionTypes = this._register(
		new Emitter<void>(),
	);
	...
	private readonly _onDidReplaceSession = this._register(
	...
	private _chatByRawSessionIdCache:
		| Map<string, ICopilotChatSession>
		| undefined;
	 ...
	 */
	private readonly _onDidGroupMembershipChange = this._register(
		new Emitter<{ sessionId: string }>(),
	);
	...
	constructor(
		...
		this._multiChatEnabled =
			this.configurationService.getValue<boolean>(COPILOT_MULTI_CHAT_SETTING) ??
			true;
		this._claudeEnabled = this.configurationService.getValue<boolean>(
		...
		this._register(
			this.configurationService.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration(CLAUDE_CODE_ENABLED_SETTING)) {
					const claudeEnabled = this.configurationService.getValue<boolean>(
						CLAUDE_CODE_ENABLED_SETTING,
					);
	...
	getSessionTypes(workspaceUri: URI): ISessionType[] {
		if (
			workspaceUri.scheme === GITHUB_REMOTE_FILE_SCHEME ||
			workspaceUri.scheme === SessionType.CopilotCloud
		) {
	...
	getSessions(): ISession[] {
		...
		if (!this._isMultiChatEnabled()) {
			return Array.from(this._sessionCache.values()).map((chat) =>
				this._chatToSession(chat),
			);
		}

		const allChats = Array.from(this._sessionCache.values()).sort(
	 ...
	 */
	private _clearCurrentNewSessionIfMatch(
		session: NewSession,
		leak?: boolean,
	): void {
	...
	createNewSession(workspaceUri: URI, sessionTypeId: string): ISession {
		...
		if (workspaceUri.scheme === GITHUB_REMOTE_FILE_SCHEME) {
			...
			const resource = URI.from({
				scheme: AgentSessionProviders.Cloud,
		...
		if (sessionTypeId === ClaudeCodeSessionType.id) {
			const resource = URI.from({
				scheme: AgentSessionProviders.Claude,
				path: `/untitled-${generateUuid()}`,
			});
			const session = this.instantiationService.createInstance(
				ClaudeCodeNewSession,
		...
		if (sessionTypeId !== CopilotCLISessionType.id) {
			throw new Error(
				`Unsupported session type '${sessionTypeId}' for local workspaces`,
			);
		}
		const resource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: `/untitled-${generateUuid()}`,
		});
		const session = this.instantiationService.createInstance(
			CopilotCLISession,
	...
	async archiveSession(sessionId: string): Promise<void> {
		...
		if (chatSession && isNewSession(chatSession)) {
			...
			this._onDidChangeSessions.fire({
				added: [],
				removed: [],
				changed: [this._chatToSession(chatSession)],
			});
			return;
		}
	}
	...
	async renameChat(
		...
		if (agentSession?.providerType === AgentSessionProviders.Claude) {
			await this.commandService.executeCommand(
				"github.copilot.claude.sessions.rename",
				{ resource: chatUri },
				title,
			);
			return;
	...
	async deleteChat(sessionId: string, chatUri: URI): Promise<void> {
		...
		if (!session?.capabilities.supportsMultipleChats) {
			throw new Error(
				"Deleting individual chats is not supported when multi-chat is disabled",
			);
		}
		...
		if (agentSession) {
			...
			const primaryChat = primaryChatId
				? this._sessionCache.get(this._localIdFromchatId(primaryChatId))
				: undefined;
			if (primaryChat) {
				this._onDidChangeSessions.fire({
					added: [],
					removed: [],
					changed: [this._chatToSession(primaryChat)],
				});
			}
		}
	}

	private async _deleteAgentSessions(
	...
	async createNewChat(sessionId: string, prompt?: string): Promise<IChat> {
				...
				const newItem = await this.chatSessionsService.createNewChatSessionItem(
					session.target,
					{
						...
						initialSessionOptions:
							session.selectedOptions.size > 0
								? session.selectedOptions
								: undefined,
						untitledResource: session.resource,
					},
					CancellationToken.None,
				);
				if (!newItem) {
					throw new Error(
						"[CopilotChatSessionsProvider] Failed to create Claude session item",
					);
				}
		...
		if (!this._isMultiChatEnabled()) {
			throw new Error(
				`[CopilotChatSessionsProvider] Session '${sessionId}' does not support multiple chats`,
			);
		}
	...
	private async _createNewSubsequentChat(sessionId: string): Promise<IChat> {
		...
		if (chat.sessionType !== CopilotCLISessionType.id) {
			throw new Error(
				"Multiple chats per session is only supported for Copilot CLI sessions",
			);
		}
		...
		if (!newWorkspace) {
			throw new Error(
				`Cannot resolve workspace for working directory URI: ${folder.workingDirectory.toString()}`,
			);
		}
		...
		this._onDidChangeSessions.fire({
			added: [],
			removed: [],
			changed: [this._chatToSession(session)],
		});
	...
	async sendRequest(
		...
		if (newSession && newSession.sessionId === sessionId) {
			if (
				!this.uriIdentityService.extUri.isEqual(
					newSession.mainChat.get().resource,
					chatResource,
				)
			) {
				throw new Error(
					"Chat resource does not match the main chat of the current new session",
				);
			}
		...
		if (
			!session.chats
				.get()
				.some((chat) =>
					this.uriIdentityService.extUri.isEqual(chat.resource, chatResource),
				)
		) {
			throw new Error(
				`Chat '${chatResource.toString()}' does not belong to session '${sessionId}'`,
			);
		}
		...
		if (!chatSession || !(chatSession instanceof CopilotCLISession)) {
			throw new Error(
				`Chat '${chatResource.toString()}' not found in session '${sessionId}'`,
			);
		}
	...
	private async _sendFirstChat(
		session: CopilotCLISession | RemoteNewSession | ClaudeCodeNewSession,
		chatResource: URI,
		options: ISendRequestOptions,
	): Promise<ISession> {
		...
		const modeIsBuiltin = session.chatMode
			? isBuiltinChatMode(session.chatMode)
			: true;
		...
		const sendOptions: IChatSendRequestOptions = {
			...
			agentHostSessionConfig:
				session instanceof CopilotCLISession
					? session.getAgentHostSessionConfig()
					: undefined,
		};

		const ref = await this._updateChatSessionState(
			chatResource,
			session,
			sendOptions.modeInfo?.permissionLevel,
		);
		this.logService.debug(
			`[CopilotChatSessionsProvider] Sending first chat for session ${session.sessionId} with options:`,
			{
		...
		try {
			...
			if (result.kind === "rejected") {
				...
				session.dispose();
				throw new Error(
					`[DefaultCopilotProvider] sendRequest rejected: ${result.reason}`,
				);
			}
			// Extract promises to detect cancellation vs normal completion
			const cts = new CancellationTokenSource();
			const responseCompletePromise =
				result.kind === "sent"
					? result.data.responseCompletePromise
					: undefined;
			const responseCreatedPromise =
				result.kind === "sent" ? result.data.responseCreatedPromise : undefined;
			...
			try {
				...
				if (session instanceof CopilotCLISession) {
					committedResource = await this._waitForCommittedSession(
						session.resource,
				...
				const committedChat = await this._waitForSessionInCache(
					committedResource,
					cts.token,
				);
				this._sessionCache.delete(session.resource.toString());
				...
				this._onDidChangeSessions.fire({
					added: [],
					removed: [this._chatToSession(session)],
					changed: [],
				});
				session.dispose();
				throw error;
			...
			this.logService.error(
				`[CopilotChatSessionsProvider] Failed to send first chat for session ${session.sessionId}:`,
				error,
			);
			throw error;
		} finally {
	...
	private async _createChatSession(
		resource: URI,
		session: NewSession,
		permissionLevel?: ChatPermissionLevel,
	): Promise<IDisposable> {
		await this.chatSessionsService.getOrCreateChatSession(
			resource,
			CancellationToken.None,
		);
	...
	private async _sendExistingChat(
		sessionId: string,
		newChatSession: CopilotCLISession,
		options: ISendRequestOptions,
	): Promise<ISession> {
		...
		this._onDidChangeSessions.fire({
			added: [],
			removed: [],
			changed: [this._chatToSession(newChatSession)],
		});

		const { query, attachedContext } = options;

		const contribution = this.chatSessionsService.getChatSessionContribution(
			newChatSession.target,
		);
		...
		try {
			...
			try {
				...
				const committedResource = await this._waitForCommittedSession(
					newChatSession.resource,
					responseCompletePromise,
					responseCreatedPromise,
				);
				...
				const committedChat =
					await this._waitForSessionInCache(committedResource);
				...
				const parentChat = parentChatId
					? this._sessionCache.get(this._localIdFromchatId(parentChatId))
					: undefined;
				if (parentChat) {
					this._onDidChangeSessions.fire({
						added: [],
						removed: [],
						changed: [this._chatToSession(parentChat)],
					});
				}
				throw error;
			}
	...
	private async _waitForCommittedSession(
		...
		try {
			const commitPromise = new Promise<URI>((resolve) => {
				disposables.add(
					this.chatSessionsService.onDidCommitSession((e) => {
			...
			const candidates: Promise<
				...
				| { kind: "cancelled" }
			>[] = [
				raceTimeout(commitPromise, 5_000).then((uri) =>
					uri ? { kind: "commit" as const, uri } : { kind: "timeout" as const },
				),
			];
			...
			const response = responseCreatedPromise
				? await responseCreatedPromise
				: undefined;
	 ...
	 */
	private async _waitForSessionInCache(
		resource: URI,
	...
	resolveWorkspace(uri: URI): ISessionWorkspace | undefined {
		if (
			...
			uri.scheme !== GITHUB_REMOTE_FILE_SCHEME
		) {
			return undefined;
		}
		const folder: ISessionFolder = {
	...
	private _ensureGroupingCaches(): void {
		if (
			this._chatByRawSessionIdCache &&
			this._groupIdByChatIdCache &&
			this._chatIdsByGroupIdCache
		) {
			return;
		}
		...
		const resolveGroupId = (chat: ICopilotChatSession): string => {
			...
			for (let depth = 0; depth < 100; depth++) {
				...
				if (!parentChat) {
					const syntheticGroupId =
						this._getSyntheticGroupId(parentRawSessionId);
					for (const trailChat of trail) {
		...
		for (const [groupId, groupChats] of chatsByGroupId) {
			...
			chatIdsByGroupId.set(
				groupId,
				groupChats.map((chat) => chat.sessionId),
			);
		}
	...
	private _refreshSessionCache(): void {
		...
		for (const session of this.agentSessionsService.model.sessions) {
			...
			if (existing) {
				...
				const adapter = new AgentSessionAdapter(
					session,
					this.id,
					this.gitHubService,
				);
		...
		if (
			...
			if (this._isMultiChatEnabled()) {
				this._refreshSessionCacheMultiChat(
					addedData,
					removedData,
					changedData,
					removedGroupIds!,
		...
		for (const removed of removedData) {
			...
			if (remainingChatIds.length > 0) {
				...
				if (!changedSessionIds.has(sessionId)) {
					...
					const primaryChat = this._sessionCache.get(
						this._localIdFromchatId(remainingChatIds[0]),
					);
					if (primaryChat) {
	...
	private _findChatSession(chatId: string): ICopilotChatSession | undefined {
		...
		return firstChatId
			? this._sessionCache.get(this._localIdFromchatId(firstChatId))
			: undefined;
	}
	...
	private _getDirectParentRawSessionId(
		chat: ICopilotChatSession,
	): string | undefined {
	...
	private _chatToSession(chat: ICopilotChatSession): ISession {
		...
		const groupChatsObs = observableFromEvent<readonly IChat[] | undefined>(
			this,
			Event.filter(
				this._onDidGroupMembershipChange.event,
		...
		const session: ISession = {
			...
			updatedAt: chatsObs.map(
				(chats, reader) =>
					this._latestDate(chats, (c) => c.updatedAt.read(reader))!,
			),
			status: chatsObs.map((chats, reader) =>
				this._aggregateStatus(chats, reader),
			),
			changesets: this._createChangesets(
				primaryChat.sessionType,
				primaryChat.workspace,
				chatsObs,
			),
			...
			isRead: chatsObs.map((chats, reader) =>
				chats.every((c) => c.isRead.read(reader)),
			),
			description: primaryChat.description,
			lastTurnEnd: chatsObs.map((chats, reader) =>
				this._latestDate(chats, (c) => c.lastTurnEnd.read(reader)),
			),
			chats: chatsObs,
			mainChat,
			capabilities: {
				supportsMultipleChats:
					primaryChat.sessionType === CopilotCLISessionType.id &&
					this._isMultiChatEnabled(),
				...
				runsWorktreeCreatedTasks:
					primaryChat.sessionType === CopilotCloudSessionType.id,
			},
		};
	...
	private _chatToSingleChatSession(chat: ICopilotChatSession): ISession {
		...
		const changesets = this._createChangesets(
			chat.sessionType,
			chat.workspace,
			chatsObs,
		);
		...
		return {
			...
			capabilities: {
				...
				runsWorktreeCreatedTasks:
					chat.sessionType === CopilotCloudSessionType.id,
			},
		};
	}
	...
	private _latestDate(
		...
		getter: (chat: IChat) => Date | undefined,
	): Date | undefined {
