...
import {
	IAgentSession,
	IAgentSessionsModel,
} from "../../../../../../workbench/contrib/chat/browser/agentSessions/agentSessionsModel.js";
...
function createMockAgentSession(
	...
	return new (class extends mock<IAgentSession>() {
		...
		override readonly timing = {
			created: opts?.createdAt ?? Date.now(),
			lastRequestStarted: undefined,
			lastRequestEnded: undefined,
		};
		override readonly metadata = opts?.metadata ?? {
			repositoryPath: "/test/repo",
		};
		override isArchived(): boolean {
			return archived;
		}
		override setArchived(value: boolean): void {
			archived = value;
		}
		...
		override isMarkedUnread(): boolean {
			return false;
		}
...
class MockAgentSessionsModel {
	...
	getSession(resource: URI): IAgentSession | undefined {
		return this._sessions.find(
			(s) => s.resource.toString() === resource.toString(),
		);
	}
...
function createProviderWithConfig(
	...
	configService.setUserConfiguration(
		...
		opts?.multiChatEnabled ?? true,
	);
	configService.setUserConfiguration(
		CLAUDE_CODE_ENABLED_SETTING,
		opts?.claudeEnabled ?? true,
	);
...
function createProviderForSendTests(
	...
	opts?: {
		onDidCommitSession?: Event<{ original: URI; committed: URI }>;
		claudeEnabled?: boolean;
		createNewChatSessionItem?: IChatSessionsService["createNewChatSessionItem"];
	...
	const configService =
		opts?.configurationService ?? new TestConfigurationService();
	configService.setUserConfiguration(
		"sessions.github.copilot.multiChatSessions",
		true,
	);
	...
	instantiationService.stub(IChatSessionsService, {
		...
		createNewChatSessionItem:
			opts?.createNewChatSessionItem ?? (async () => undefined),
	});
	instantiationService.stub(IChatService, {
		acquireOrLoadSession: async () => undefined,
	...
	instantiationService.stub(IGitService, {
...
suite("CopilotChatSessionsProvider", () => {
	...
	test("sessionTypes excludes Claude when setting is disabled", () => {
		...
		assert.ok(
			!provider.sessionTypes.some((t) => t.id === ClaudeCodeSessionType.id),
		);
	});

	test("onDidChangeSessionTypes fires when claude setting changes", () => {
		const { provider, configService } = createProviderWithConfig(
			disposables,
			model,
		);
		assert.strictEqual(provider.sessionTypes.length, 3);
		...
		configService.onDidChangeConfigurationEmitter.fire({
			...
			affectsConfiguration: (key: string) =>
				key === CLAUDE_CODE_ENABLED_SETTING,
		});
	...
	test("toggling claude setting refreshes sessions list", () => {
		const claudeResource = URI.from({
			scheme: AgentSessionProviders.Claude,
			path: "/claude-session",
		});
		model.addSession(
			createMockAgentSession(claudeResource, {
		...
		assert.strictEqual(
			...
			1,
			"Claude sessions should appear when enabled by default",
		);
		...
		assert.strictEqual(
			provider.getSessions().length,
			0,
			"Claude sessions should disappear after disabling",
		);
		...
		assert.strictEqual(
			provider.getSessions().length,
			1,
			"Claude sessions should reappear after re-enabling",
		);
	});
	...
	test("getSessionTypes returns only Cloud for remote workspace regardless of claude setting", () => {
		...
		const types = provider.getSessionTypes(
			URI.from({ scheme: GITHUB_REMOTE_FILE_SCHEME, path: "/owner/repo" }),
		);
	...
	test("getSessions excludes Local sessions (now owned by LocalChatSessionsProvider)", () => {
		const bgResource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/bg-session",
		});
		const localResource = URI.from({
			scheme: AgentSessionProviders.Local,
			path: "/local-session",
		});
		model.addSession(createMockAgentSession(bgResource));
		model.addSession(
			createMockAgentSession(localResource, {
	...
	test("onDidChangeSessions fires when agent model changes", () => {
		...
		model.addSession(
			createMockAgentSession(resource, { title: "New Session" }),
		);
	...
	test("Copilot CLI session maps workspace selection to Agent Host folder config", async () => {
		const provider = createProviderForSendTests(
			disposables,
			model,
			async () => ({ kind: "sent" as const, data: {} as IChatSendRequestData }),
		);
		const session = provider.createNewSession(
			URI.file("/test/project"),
			CopilotCLISessionType.id,
		);
		const providerSession = provider.getSession(
			session.sessionId,
		) as ICopilotChatSession &
			IDisposable & { getAgentHostSessionConfig(): Record<string, unknown> };
	...
	test("copilot cloud sessions do not have supportsMultipleChats capability", () => {
		...
		model.addSession(
			createMockAgentSession(resource, {
	...
	test("each session has exactly one chat initially", () => {
		...
		assert.strictEqual(
			sessions[0].mainChat.get().resource.toString(),
			resource.toString(),
		);
	});
	...
	test("groups committed chats using metadata.sessionParentId", () => {
		const rootResource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/root-session",
		});
		const child1Resource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/child-session-1",
		});
		const child2Resource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/child-session-2",
		});

		model.addSession(
			createMockAgentSession(rootResource, { title: "Root", createdAt: 1 }),
		);
		model.addSession(
			createMockAgentSession(child1Resource, {
				title: "Child 1",
				createdAt: 2,
				metadata: {
					repositoryPath: "/test/repo",
					sessionParentId: "root-session",
				},
			}),
		);
		model.addSession(
			createMockAgentSession(child2Resource, {
				title: "Child 2",
				createdAt: 3,
				metadata: {
					repositoryPath: "/test/repo",
	...
	test("orders chats within a grouped session by createdAt", () => {
		...
		const olderChildResource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/older-child",
		});
		const newerChildResource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/newer-child",
		});
		...
		model.addSession(
			createMockAgentSession(newerChildResource, {
				title: "Newer Child",
				createdAt: 30,
				metadata: {
					repositoryPath: "/test/repo",
		...
		model.addSession(
			createMockAgentSession(rootResource, { title: "Root", createdAt: 10 }),
		);
		model.addSession(
			createMockAgentSession(olderChildResource, {
				title: "Older Child",
				createdAt: 20,
				metadata: {
					repositoryPath: "/test/repo",
		...
		assert.deepStrictEqual(
			...
			[
				rootResource.toString(),
				olderChildResource.toString(),
				newerChildResource.toString(),
			],
		);
	});

	test("groups child sessions even when the parent/root session is missing", () => {
		const orphan1Resource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/orphan-child-1",
		});
		const orphan2Resource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/orphan-child-2",
		});
		const provider = createProvider(disposables, model);
		...
		model.addSession(
			createMockAgentSession(orphan1Resource, {
				title: "Orphan Child 1",
				createdAt: 1,
				metadata: {
					repositoryPath: "/test/repo",
					sessionParentId: "missing-root",
				},
			}),
		);
		model.addSession(
			createMockAgentSession(orphan2Resource, {
	...
	test("groups nested parent chains under the ultimate root", () => {
		const middleResource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/middle-session",
		});
		const leafResource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/leaf-session",
		});

		model.addSession(
			createMockAgentSession(middleResource, {
		...
		model.addSession(
			createMockAgentSession(leafResource, {
				title: "Leaf Session",
				createdAt: 3,
				metadata: {
					repositoryPath: "/test/repo",
					sessionParentId: "middle-session",
				},
			}),
		);
	...
	test("session title comes from primary (first) chat", () => {
		...
		model.addSession(
			createMockAgentSession(resource, { title: "Primary Title" }),
		);
	...
	test("changed events are not duplicated when multiple chats update", () => {
		...
		for (const change of changes) {
			...
			assert.strictEqual(
				changedIds.length,
				uniqueIds.size,
				"Changed events should not have duplicates",
			);
		}
	});
	...
	test("builds an unknown workspace fallback when repository metadata is missing", () => {
		const resource = URI.from({
			scheme: AgentSessionProviders.Background,
			path: "/unknown-workspace-session",
		});
		...
		assert.strictEqual(workspace.folders.length, 1);
		assert.strictEqual(
			workspace.folders[0].root.toString(),
			URI.parse("unknown:///").toString(),
		);
		assert.strictEqual(workspace.requiresWorkspaceTrust, true);

		// The core symptom of #310777: any of these calls must not throw.
		assert.doesNotThrow(() =>
			URI.joinPath(workspace.folders[0].root, ".vscode", "settings.json"),
		);
		assert.doesNotThrow(() =>
			URI.joinPath(workspace.folders[0].root, ".vscode/extensions.json"),
		);
	});
	...
	function makeClaudeInFlightProvider(): {
		...
		const responseCompletePromise = new Promise<void>((r) => {
			resolveComplete = r;
		});
		const responseCreatedPromise = new Promise<IChatResponseModel>((r) => {
			resolveCreated = r;
		});
		...
		const provider = createProviderForSendTests(
			...
			{
				claudeEnabled: true,
				createNewChatSessionItem: async (
					_type,
					request,
		...
		return {
			...
			commitSession: () => {
				...
				model.addSession(
					createMockAgentSession(realResource, {
						providerType: AgentSessionProviders.Claude,
					}),
				);
			},
	...
	test("createNewSession with Claude type creates a session", async () => {
		...
		const chat = await provider.createNewChat(session.sessionId);
		const sendPromise = provider.sendRequest(session.sessionId, chat.resource, {
			query: "test",
		});
		await added;
		commitSession();
	...
	test("archiveSession archives a Claude temp session", async () => {
		...
		const sendPromise = provider.sendRequest(
			session.sessionId,
			chat1.resource,
			{ query: "test" },
		);
	...
	test("sendRequest replaces temp session with committed session on success", async () => {
		...
		assert.strictEqual(
			provider.getSessions().length,
			1,
			"temp session should appear while in-flight",
		);
		...
		assert.ok(
			...
			"onDidReplaceSessions should have fired",
		);
	});
	...
	test("sendRequest keeps temp session on cancellation", async () => {
		...
		assert.strictEqual(
			provider.getSessions().length,
			1,
			"session should remain after cancellation",
		);
		assert.strictEqual(
			provider.getSessions()[0].status.get(),
	...
	suite("uncommitted temp session cleanup", () => {
		...
		test("deleteSession removes a temp session that is awaiting commit", async () => {
			...
			const newSession = provider.createNewSession(
				workspace,
				CopilotCLISessionType.id,
			);
			...
			const sendPromise = provider.sendRequest(sessionId, chat.resource, {
				query: "test",
			});
			await added;
			...
			assert.strictEqual(
				provider.getSessions().length,
				1,
				"session should appear while in-flight",
			);
			...
			assert.strictEqual(
				provider.getSessions().length,
				0,
				"session should be removed after deleteSession",
			);
		...
		test("archiveSession archives a temp session that is awaiting commit", async () => {
			...
			assert.strictEqual(
				provider.getSessions().length,
				1,
				"session should still be in the list after archiveSession",
			);
			assert.strictEqual(
				provider.getSessions()[0].isArchived.get(),
				true,
		...
		test("archiveSession archives a stopped session that was never committed", async () => {
			...
			assert.strictEqual(
				provider.getSessions().length,
				1,
				"stopped session should remain in the list",
			);
			assert.strictEqual(
				provider.getSessions()[0].status.get(),
				SessionStatus.Completed,
			...
			assert.strictEqual(
				provider.getSessions().length,
				1,
				"session should still be in the list after archiving",
			);
			assert.strictEqual(
				provider.getSessions()[0].isArchived.get(),
				true,
	...
	suite("new session default permission level", () => {
		...
		function makeConfig(opts: {
			defaultLevel?: ChatPermissionLevel;
			...
			const config = new (class extends TestConfigurationService {
				override inspect<T>(key: string): IConfigurationValue<T> {
					...
					if (
						opts.policyRestricted &&
						key === ChatConfiguration.GlobalAutoApprove
					) {
		...
		test("CLI session seeds permission level from chat.permissions.default", () => {
			const configurationService = makeConfig({
				defaultLevel: ChatPermissionLevel.Autopilot,
			});
			const provider = createProviderForSendTests(
				disposables,
				model,
			...
			const sessionInfo = provider.createNewSession(
				workspace,
				CopilotCLISessionType.id,
			);
