...
function isFeedbackFileElement(
	element: FeedbackTreeElement,
): element is IFeedbackFileElement {
...
class FeedbackFileRenderer implements ITreeRenderer<
	IFeedbackFileElement,
	void,
	IFeedbackFileTemplate
> {
	...
	renderElement(
		node: ITreeNode<IFeedbackFileElement, void>,
		_index: number,
		templateData: IFeedbackFileTemplate,
	): void {
...
class FeedbackCommentRenderer implements ITreeRenderer<
	IFeedbackCommentElement,
	void,
...
> {
	...
	renderTemplate(container: HTMLElement): IFeedbackCommentTemplate {
		...
		const row = dom.append(
			container,
			$("div.agent-feedback-hover-comment-row"),
		);

		const textElement = dom.append(
			row,
			$("div.agent-feedback-hover-comment-text"),
		);

		const actionBarContainer = dom.append(
			row,
			$("div.agent-feedback-hover-action-bar"),
		);
		const actionBar = templateDisposables.add(
			new ActionBar(actionBarContainer),
		);
		...
		const templateData: IFeedbackCommentTemplate = {
			textElement,
			row,
			actionBar,
			templateDisposables,
		...
		if (this._agentFeedbackService) {
			...
			templateDisposables.add(
				dom.addDisposableListener(row, dom.EventType.CLICK, (e) => {
	...
	renderElement(
		node: ITreeNode<IFeedbackCommentElement, void>,
		_index: number,
	...
	private _buildCommentHover(
		element: IFeedbackCommentElement,
	): IDelayedHoverOptions {
		...
		if (element.codeSelection) {
			const languageId =
				this._languageService.guessLanguageIdByFilepathOrFirstLine(
					element.resourceUri,
				);
...
export class AgentFeedbackHover extends Disposable {
	constructor(
		...
		this._store.add(
			dom.addDisposableListener(this._element, dom.EventType.CLICK, (e) => {
				e.preventDefault();
				e.stopPropagation();
				this._showHoverNow();
			}),
	...
	private _buildHoverContent(): IDelayedHoverOptions & IDisposable {
		...
		const treeContainer = dom.append(
			hoverElement,
			$(
				".results.show-file-icons.file-icon-themable-tree.agent-feedback-hover-tree",
			),
		);
		...
		const tree = disposables.add(
			this._instantiationService.createInstance(
				...
				[
					new FeedbackFileRenderer(
						resourceLabels,
						this._canDelete ? this._agentFeedbackService : undefined,
						this._attachment.sessionResource,
					),
					new FeedbackCommentRenderer(
						this._canDelete ? this._agentFeedbackService : undefined,
						this._attachment.sessionResource,
						this._hoverService,
				...
				],
				{
					...
					accessibilityProvider: {
						...
						getWidgetAriaLabel: () =>
							localize("agentFeedbackHover.tree", "Feedback Comments"),
					},
					identityProvider: {
	...
	private _buildTreeData(): {
		children: IObjectTreeElement<FeedbackTreeElement>[];
		commentElements: IFeedbackCommentElement[];
	} {
		// Group feedback items by file
		const byFile = new Map<
			string,
			{ uri: URI; comments: IFeedbackCommentElement[] }
		>();
