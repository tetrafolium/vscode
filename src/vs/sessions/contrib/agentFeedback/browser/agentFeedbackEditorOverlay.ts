...
class AgentFeedbackActionViewItem extends ActionViewItem {
	constructor(
		...
		const isIconOnly =
			action.id === navigatePreviousFeedbackActionId ||
			action.id === navigateNextFeedbackActionId;
			...
			this._instaService.createInstance(
				...
				Menus.AgentFeedbackEditorContent,
				{
					...
					actionViewItemProvider: (action, options) => {
						...
						return new AgentFeedbackActionViewItem(
							action,
							options,
...
class AgentFeedbackOverlayController {
	...
	constructor(
		...
		const hasCommentsContext =
			hasSessionEditorComments.bindTo(contextKeyService);
		const hasAgentFeedbackContext =
			hasSessionAgentFeedback.bindTo(contextKeyService);
		...
		this._store.add(
			autorun((r) => {
				...
				const candidates = getActiveResourceCandidates(
				...
				for (const candidate of candidates) {
					const sessionResource =
						agentFeedbackService.getSessionForFile(candidate)?.resource;
					if (!sessionResource) {
						continue;
					}
					...
					if (comments.length > 0) {
						navigationBearings = agentFeedbackService.getNavigationBearing(
							sessionResource,
							comments,
						);
