	...
	implements IAgentFeedbackService
{
	...
	readonly onDidAddFeedback = this._register(
		new Emitter<IAgentFeedbackAddedEvent>(),
	).event;
	readonly onDidConvertFeedback = this._register(
		new Emitter<IAgentFeedbackConvertedEvent>(),
	).event;
	readonly onDidAddReply = this._register(
		new Emitter<IAgentFeedbackReplyAddedEvent>(),
	).event;
	readonly onDidSubmitFeedback = this._register(
	...
	getSessionForFile(_resourceUri: URI): undefined {
		return undefined;
	}
