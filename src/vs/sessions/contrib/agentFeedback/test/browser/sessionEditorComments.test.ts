...
type ICodeReviewResultState = Extract<
	ICodeReviewState,
...
suite("SessionEditorComments", () => {
	...
	function reviewState(
		comments: ICodeReviewResultState["comments"],
	): ICodeReviewState {
		return {
			kind: CodeReviewStateKind.Result,
			version: "v1",
	...
	test("merges and sorts feedback and review comments by resource and range", () => {
		...
		assert.deepStrictEqual(
			comments.map(
				(comment) =>
					`${comment.resourceUri.path}:${comment.range.startLineNumber}:${comment.source}`,
			),
			[
				"/a.ts:3:codeReview",
	...
	test("groups nearby comments only within the same resource", () => {
		...
		assert.deepStrictEqual(
			...
			["/a.ts:10:agentFeedback", "/a.ts:13:codeReview"],
		);
		assert.deepStrictEqual(
			groups[1].map(
				(comment) =>
	...
	test("filters resource comments and detects authored feedback presence", () => {
		...
		assert.deepStrictEqual(
			getResourceEditorComments(fileA, comments).map(
				(comment) => comment.source,
			),
			[SessionEditorCommentSource.AgentFeedback],
		);
		assert.deepStrictEqual(
			getResourceEditorComments(fileB, comments).map(
				(comment) => comment.source,
			),
			[SessionEditorCommentSource.CodeReview],
		);
	...
	test("includes PR review comments when prReviewState is loaded", () => {
		...
		assert.deepStrictEqual(
			comments.map(
				(c) => `${c.resourceUri.path}:${c.range.startLineNumber}:${c.source}`,
			),
			["/a.ts:5:prReview", "/b.ts:1:prReview"],
		);
		assert.strictEqual(comments[0].canConvertToAgentFeedback, true);
	});
	...
	test("merges PR review comments with other sources sorted correctly", () => {
		...
		assert.deepStrictEqual(
			comments.map((c) => `${c.range.startLineNumber}:${c.source}`),
			["3:agentFeedback", "7:prReview", "10:codeReview"],
		);
	});
