...
function feedbackSummary(
	items: readonly { resourceUri: URI; range: { startLineNumber: number } }[],
): string[] {
	...
	setup(() => {
		...
		instantiationService.stub(
			...
			new (class extends mock<ISessionsManagementService>() {
				override activeSession = observableValue<IActiveSession | undefined>(
					"activeSession",
					undefined,
				);
			})(),
		...
		service = store.add(
			instantiationService.createInstance(AgentFeedbackService),
		);
...
suite("AgentFeedbackService - getSessionForFile", () => {
	...
	function pane(...resources: URI[]): any {
		...
		const input =
			resources.length === 1
				...
				: {
						primary: { resource: resources[0] },
						secondary: { resource: resources[1] },
					};
		return { input };
	}
	...
	function makeSession(
		resource: URI,
		status: SessionStatus = SessionStatus.InProgress,
	): ISession {
		return {
			resource,
	...
	setup(() => {
		visibleEditorsEmitter = store.add(
	...
	test("untracked file falls back to the active session", () => {
		...
		assert.strictEqual(
			service.getSessionForFile(fileA)?.resource.toString(),
			sessionS1.toString(),
		);
	});
	...
	test("preserves captured session after active session switches without a visibility change", () => {
		...
		assert.strictEqual(
			service.getSessionForFile(fileB)?.resource.toString(),
			sessionS2.toString(),
		);
	});
	...
	test("returns undefined when the active session has Untitled status", () => {
		sessions.set(
			sessionS1.toString(),
			makeSession(sessionS1, SessionStatus.Untitled),
		);
		setActiveSession(sessions.get(sessionS1.toString())!);
