...
suite("Sessions - Editor Contribution", () => {
	...
	test("maximize editor hides the terminal panel before maximizing", async () => {
		...
		const handler = CommandsRegistry.getCommand(
			"workbench.action.agentSessions.maximizeMainEditorPart",
		)?.handler;
		assert.ok(handler, "Command handler should be registered");

		await handler(instantiationService);

		assert.deepStrictEqual(layoutService.calls, [
			"hidePanel",
			"maximizeEditor",
		]);
	...
	test("restore editor reopens the terminal panel when maximize hid it", async () => {
		...
		const maximizeHandler = CommandsRegistry.getCommand(
			"workbench.action.agentSessions.maximizeMainEditorPart",
		)?.handler;
		const restoreHandler = CommandsRegistry.getCommand(
			"workbench.action.agentSessions.restoreMainEditorPart",
		)?.handler;
