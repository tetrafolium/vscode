...
const terminalPanelHiddenForMaximizedEditor =
	new WeakSet<IAgentWorkbenchLayoutService>();
...
class MaximizeMainEditorPartAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (
			...
			viewsService.isViewVisible(TERMINAL_VIEW_ID)
		) {
...
class RestoreMainEditorPartAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const layoutService = accessor.get(IAgentWorkbenchLayoutService);
		const shouldRestoreTerminalPanel =
			terminalPanelHiddenForMaximizedEditor.has(layoutService);

		layoutService.setEditorMaximized(false);

		if (
			shouldRestoreTerminalPanel &&
			!layoutService.isVisible(Parts.PANEL_PART)
		) {
...
class OpenEditorInModalEditorAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const multiFileDiffEditor = activeGroup.editors.find(
			(editor) => editor instanceof MultiDiffEditorInput,
		);
		...
		const editorsToMove = prepareMoveCopyEditors(
			activeGroup,
			activeGroup.editors.slice(),
			true,
		);
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (navigation) {
			...
			if (changes && navigation.current < changes.length) {
				...
				await view?.openChanges(
					changes[navigation.current].modifiedUri ??
						changes[navigation.current].originalUri,
				);
