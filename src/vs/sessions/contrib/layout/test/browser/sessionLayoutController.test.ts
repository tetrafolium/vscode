...
suite("LayoutController", () => {
	...
	function createLayoutController(): LayoutController {
		...
		setPartHiddenCalls = [];
		onDidChangePartVisibility = store.add(
		...
		instaService.stub(
			...
			new (class extends mock<IWorkbenchLayoutService>() {
				...
				override hasFocus(_part: Parts): boolean {
					return false;
				}
				override readonly onDidChangePartVisibility =
		...
		instaService.stub(
			...
			new (class extends mock<IPaneCompositePartService>() {
				override getActivePaneComposite(
					_location: ViewContainerLocation,
		...
		instaService.stub(
			...
			new (class extends mock<IEditorGroupsService>() {
				...
				override async applyWorkingSet() {
					return true;
				}
	...
	test("remembers aux bar hidden state on session switch", () => {
		...
		assert.ok(
			setPartHiddenCalls.some(
				(c) => c.part === Parts.AUXILIARYBAR_PART && c.hidden === true,
			),
	...
	test("hides panel by default when no record exists", () => {
		...
		assert.ok(
			setPartHiddenCalls.some(
				(c) => c.part === Parts.PANEL_PART && c.hidden === true,
			),
	...
	test("remembers panel visibility per session", () => {
		...
		const panelCall = setPartHiddenCalls.find(
			(c) => c.part === Parts.PANEL_PART,
		);
		assert.ok(panelCall);
		assert.strictEqual(
			panelCall!.hidden,
			false,
			"panel should be visible for session 1",
		);
	});
	...
	test("shows aux bar when turn completes with new changes", () => {
		...
		(session.changes as ISettableObservable<readonly ISessionFileChange[]>).set(
			[makeChange("/file.ts")],
			undefined,
		);
		(session.lastTurnEnd as ISettableObservable<Date | undefined>).set(
			new Date(),
			undefined,
		);
		...
		assert.ok(
			setPartHiddenCalls.some(
				(c) => c.part === Parts.AUXILIARYBAR_PART && c.hidden === false,
			),
	...
	test("clears saved state when turn produces new changes", () => {
		...
		(
			session1.changes as ISettableObservable<readonly ISessionFileChange[]>
		).set([makeChange("/new.ts")], undefined);
		(session1.lastTurnEnd as ISettableObservable<Date | undefined>).set(
			new Date(),
			undefined,
		);
	...
	test("persists state to sessions.layoutState key", () => {
		...
		const stored = storageService.get(
			"sessions.layoutState",
			StorageScope.WORKSPACE,
		);
		assert.ok(stored, "state should be persisted");

		const parsed = JSON.parse(stored!);
		const session1Entry = parsed.find(
			(e: any) => e.sessionResource === "session:1",
		);
	...
	test("migrates legacy sessions.workingSets key", () => {
		const legacyData = JSON.stringify([
			{
				...
				auxiliaryBarState: {
					visible: false,
					activeViewContainerId: "legacy.view",
				},
			},
		]);
		...
		instaService.stub(
			IViewsService,
			new (class extends mock<IViewsService>() {
				override async openViewContainer() {
					return null;
				}
				override closeViewContainer() {}
		...
		instaService.stub(
			IEditorGroupsService,
			new (class extends mock<IEditorGroupsService>() {
				override saveWorkingSet(name: string) {
					return { id: name, name };
				}
				override async applyWorkingSet() {
					return true;
		...
		const newStored = tempStorage.get(
			"sessions.layoutState",
			StorageScope.WORKSPACE,
		);
		...
		const entry = parsed.find(
			(e: any) => e.sessionResource === "session:legacy",
		);
		assert.ok(entry);
