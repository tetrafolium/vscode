 ...
 */
export const SESSIONS_CUSTOMIZATIONS_SIDEBAR_MODE_SETTING =
...
export class CustomizationLinkViewItem extends ActionViewItem {
	...
	private _readCount(
		reader: Parameters<Parameters<typeof autorun>[0]>[0],
	): number {
...
export class CustomizationsToolbarContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		for (const config of CUSTOMIZATION_ITEMS) {
			const key = new RawContextKey<boolean>(
					...
					class extends Action2 {
						...
						async run(accessor: ServicesAccessor): Promise<void> {
							...
							if (pane instanceof AICustomizationManagementEditor) {
								const mode = configurationService.getValue<string>(
									SESSIONS_CUSTOMIZATIONS_SIDEBAR_MODE_SETTING,
								);
 ...
 */
export function findHarnessIdForSession(
 ...
 */
export class ActiveSessionHarnessSyncContribution
	extends Disposable
	implements IWorkbenchContribution
{
