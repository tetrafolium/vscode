...
export class AICustomizationShortcutsWidget extends Disposable {
	...
	constructor(
		...
		this._register(
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(SESSIONS_CUSTOMIZATIONS_SIDEBAR_MODE_SETTING)
				) {
					const isSingle =
						this._readMode() === SessionsCustomizationsSidebarMode.Single;
	...
	private _readMode(): SessionsCustomizationsSidebarMode {
		...
		if (
			value === SessionsCustomizationsSidebarMode.Section ||
			value === SessionsCustomizationsSidebarMode.Single
		) {
			return value;
		}
	...
	private _totalCount() {
		return derived((reader) => {
			...
			const hidden = new Set(
				this.harnessService.getActiveDescriptor().hiddenSections ?? [],
			);
			let total = 0;
	...
	private async _openWelcomePage(): Promise<void> {
		const sessionResource =
			this.sessionsManagementService.activeSession.get()?.resource;
		if (sessionResource) {
