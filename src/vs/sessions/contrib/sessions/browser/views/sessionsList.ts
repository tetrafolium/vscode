...
} from "../../../../../workbench/contrib/chat/browser/agentSessions/agentSessionApprovalModel.js";
...
const SESSION_SECTION_FOCUS_FROM_POINTER_CLASS =
 ...
 */
function swapIconWithCrossfade(
...
export const IsSessionPinnedContext = new RawContextKey<boolean>(
	"sessionItem.isPinned",
	false,
);
export const IsSessionArchivedContext = new RawContextKey<boolean>(
	"sessionItem.isArchived",
	false,
);
export const IsSessionReadContext = new RawContextKey<boolean>(
	"sessionItem.isRead",
	true,
);
export const SessionItemHasBranchNameContext = new RawContextKey<boolean>(
	"sessionItem.hasBranchName",
	false,
);
export const SessionSectionTypeContext = new RawContextKey<string>(
	"sessionSection.type",
	"",
);
...
function isSessionSection(item: SessionListItem): item is ISessionSection {
	return (
		"sessions" in item && Array.isArray((item as ISessionSection).sessions)
	);
}
...
class SessionsTreeDelegate implements IListVirtualDelegate<SessionListItem> {
	...
	getHeight(element: SessionListItem): number {
		...
		let height = this._isPhone()
			? SessionsTreeDelegate.ITEM_HEIGHT_PHONE
			: SessionsTreeDelegate.ITEM_HEIGHT;
		if (this._approvalModel) {
			const approval = getFirstApprovalAcrossChats(
				this._approvalModel,
				element as ISession,
				undefined,
			);
	...
	hasDynamicHeight(element: SessionListItem): boolean {
		return (
			!!this._approvalModel &&
			!isSessionSection(element) &&
			!isSessionShowMore(element)
		);
	}
...
class SessionItemRenderer implements ITreeRenderer<
	SessionListItem,
	FuzzyScore,
	ISessionItemTemplate
> {
	...
	static getApprovalRowHeight(label: string): number {
		const lineCount = Math.min(
			label.split(/\r?\n/).length,
			SessionItemRenderer.APPROVAL_ROW_MAX_LINES,
		);
		return (
			lineCount * SessionItemRenderer._APPROVAL_ROW_LINE_HEIGHT +
			SessionItemRenderer._APPROVAL_ROW_OVERHEAD
		);
	}

	private readonly _onDidChangeItemHeight = new Emitter<ISession>();
	readonly onDidChangeItemHeight: Event<ISession> =
	...
	constructor(
		private readonly options: {
			...
			visibleSessions: IObservable<readonly (IActiveSession | undefined)[]>;
		},
		...
		this._motionReducedSignal = observableSignalFromEvent(
			"reduceMotion",
	...
	renderTemplate(container: HTMLElement): ISessionItemTemplate {
		...
		const title = disposables.add(
			new HighlightedLabel(DOM.append(titleRow, $(".session-title"))),
		);
		...
		for (const eventType of [
			...
			disposables.add(
				DOM.addDisposableListener(titleToolbarContainer, eventType, (e) =>
					e.stopPropagation(),
				),
			);
		}
		...
		const approvalLabel = DOM.append(
			approvalRow,
			$("span.session-approval-label"),
		);
		const approvalButtonContainer = DOM.append(
			approvalRow,
		...
		const titleToolbar = disposables.add(
		...
		return {
			...
			disposables,
			elementDisposables,
			currentIconSelector: undefined,
		};
	}

	renderElement(
		node: ITreeNode<SessionListItem, FuzzyScore>,
		_index: number,
		template: ISessionItemTemplate,
	): void {
	...
	private renderSession(
		...
		template.elementDisposables.add(
			this.hoverService.setupDelayedHover(
				template.container,
				() => ({
					content: buildSessionHoverContent(
						element,
		...
		IsSessionArchivedContext.bindTo(template.contextKeyService).set(
			element.isArchived.get(),
		);
		IsSessionReadContext.bindTo(template.contextKeyService).set(
			this.options.isRead(element),
		);
		SessionItemHasBranchNameContext.bindTo(template.contextKeyService).set(
			!!element.workspace.get()?.folders[0]?.gitRepository?.branchName?.trim(),
		);
		...
		template.elementDisposables.add(
			autorun((reader) => {
				const wrapper = this.options.visibleSessions
					.read(reader)
					.find((s) => s?.sessionId === element.sessionId);
		...
		template.elementDisposables.add(
			autorun((reader) => {
				...
				const isArchived = element.isArchived.read(reader);
				const gitHubInfo = element.workspace
					.read(reader)
					?.folders[0]?.gitRepository?.gitHubInfo.read(reader);
				...
				const applyIconSwap = (
					...
					swapIconWithCrossfade(
						template.iconContainer,
						createIcon(),
						animate,
						template.disposables,
					);
					return true;
				};
				const recolorActiveIcon = (color: string): void => {
					for (const child of Array.from(
						template.iconContainer.children,
					) as HTMLElement[]) {
				...
				const isPixelSpinner =
					(sessionStatus === SessionStatus.InProgress ||
						sessionStatus === SessionStatus.NeedsInput) &&
					!motionReduced;
				if (isPixelSpinner) {
					const isNeedsInput = sessionStatus === SessionStatus.NeedsInput;
					const variant: "grid" | "ring" = isNeedsInput ? "ring" : "grid";
					const spinnerKey = isNeedsInput
						? PIXEL_SPINNER_RING_KEY
						: PIXEL_SPINNER_GRID_KEY;
					const iconColor = isNeedsInput
						? asCssVariable("list.warningForeground")
						: asCssVariable("textLink.foreground");
				...
				template.iconContainer.classList.toggle(
					"session-icon-pulse",
					sessionStatus === SessionStatus.NeedsInput,
				);
		...
		const timeDisposable = template.elementDisposables.add(
			new MutableDisposable(),
		);
		const descriptionDisposable = template.elementDisposables.add(
			new MutableDisposable(),
		);
		template.elementDisposables.add(
			autorun((reader) => {
				...
				if (!hideDetails) {
					timeDate =
						this.options.sorting() === SessionsSorting.Updated
							? element.updatedAt.read(reader)
							: element.createdAt;
				}
				...
				if (sessionStatus !== SessionStatus.InProgress) {
					...
					const icon = workspace?.isVirtualWorkspace
						? Codicon.cloudCompact
						: isWorkspaceSession
							? Codicon.folderCompact
							: Codicon.worktreeCompact;
					const typeIconEl = DOM.append(
						template.detailsRow,
						$("span.session-details-icon"),
					);
				...
				if (!hideDetails && (changesSummary || changes.length > 0)) {
					...
					if (insertions > 0 || deletions > 0) {
						if (parts.length > 0) {
							DOM.append(
								template.detailsRow,
								$("span.session-separator.has-separator"),
							);
						}
						const diffEl = DOM.append(
							template.detailsRow,
							$("span.session-diff"),
						);
						DOM.append(diffEl, $("span.session-diff-added")).textContent =
							`+${insertions}`;
						DOM.append(diffEl, $("span.session-diff-removed")).textContent =
							`-${deletions}`;
						parts.push(diffEl);
					}
				}
				...
				if (sessionStatus === SessionStatus.InProgress) {
					...
					const statusEl = DOM.append(
						template.detailsRow,
						$("span.session-description"),
					);
					if (description) {
						descriptionDisposable.value = this.markdownRendererService.render(
							description,
							{ sanitizerConfig: { replaceWithPlaintext: true } },
							statusEl,
						);
					} else {
				...
				if (!hideDetails && timeDate) {
					...
					const formatTime = () => {
						const seconds = Math.round(
							(Date.now() - definiteTimeDate.getTime()) / 1000,
						);
						return seconds < 60
							? localize("secondsDuration", "now")
							: fromNow(definiteTimeDate, true);
					};
	...
	private renderApprovalRow(
		...
		const initialInfo = getFirstApprovalAcrossChats(
			approvalModel,
			element,
			undefined,
		);
		...
		template.elementDisposables.add(
			autorun((reader) => {
				...
				const info = getFirstApprovalAcrossChats(
					approvalModel,
					element,
					reader,
				);
				...
				if (info) {
					...
					const fullContent = new MarkdownString().appendCodeblock(
						info.languageId ?? "json",
						info.label,
					);
					buttonStore.add(
						this.hoverService.setupDelayedHover(template.approvalLabel, {
							content: fullContent,
					...
					const button = buttonStore.add(
						new Button(template.approvalButtonContainer, {
	...
	private getStatusIcon(
		...
		switch (status) {
			case SessionStatus.InProgress:
				...
				return {
					...Codicon.sessionInProgressCompact,
					color: themeColorFromId("textLink.foreground"),
				};
			case SessionStatus.NeedsInput:
				// Same as above — pixel spinner replaces the pulsing dot when motion is allowed.
				return {
					...Codicon.circleFilledCompact,
					color: themeColorFromId("list.warningForeground"),
				};
			case SessionStatus.Error:
				return {
					...Codicon.errorCompact,
					color: themeColorFromId("errorForeground"),
				};
			default:
				if (pullRequestIcon) {
				...
				return {
					...Codicon.circleSmallFilledCompact,
					color: themeColorFromId("agentSessionReadIndicator.foreground"),
				};
		}
	}

	private getWorkspaceBadgeLabel(
...
class SessionSectionRenderer implements ITreeRenderer<
	SessionListItem,
	FuzzyScore,
	ISessionSectionTemplate
> {
	static readonly TEMPLATE_ID = "session-section";
	readonly templateId = SessionSectionRenderer.TEMPLATE_ID;

	private readonly templatesByElement = new WeakMap<
		ISessionSection,
		ISessionSectionTemplate
	>();
	...
	renderElement(
		node: ITreeNode<SessionListItem, FuzzyScore>,
		_index: number,
		template: ISessionSectionTemplate,
	): void {
		const element = node.element;
		if (!isSessionSection(element)) {
			return;
		...
		const sectionType = element.id.startsWith("workspace:")
			? "workspace"
			: element.id;
		SessionSectionTypeContext.bindTo(template.contextKeyService).set(
			sectionType,
		);
	...
	disposeElement(
		node: ITreeNode<SessionListItem, FuzzyScore>,
		_index: number,
		_template: ISessionSectionTemplate,
	): void {
...
class SessionShowMoreRenderer implements ITreeRenderer<
	SessionListItem,
	FuzzyScore,
	HTMLElement
> {
	...
	renderElement(
		...
		container?.classList.toggle(
			"session-show-more-folders",
			element.kind === "folders",
		);
...
class SessionsListDragAndDrop
	extends Disposable
	implements ITreeDragAndDrop<SessionListItem>
{
	private readonly _transfer =
		LocalSelectionTransfer.getInstance<DraggedSessionIdentifier>();
	...
	getDragLabel(elements: SessionListItem[]): string | undefined {
		const sessions = elements.filter(
			(e): e is ISession => !isSessionSection(e) && !isSessionShowMore(e),
		);
	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
		const elements = (
			data instanceof ElementsDragAndDropData ? data.elements : []
		) as SessionListItem[];
		const sessions = elements.filter(
			(e): e is ISession => !isSessionSection(e) && !isSessionShowMore(e),
		);
		...
		const identifiers = sessions.map(
			(s) => new DraggedSessionIdentifier(s.sessionId, s.resource),
		);
		...
		if (originalEvent.dataTransfer) {
			...
			const payload = JSON.stringify({
				...
				resource: sessions[0].resource.toString(),
			});
			originalEvent.dataTransfer.setData(
				SessionsDataTransfers.SESSION,
				payload,
			);
		}
	}
...
export class SessionsList extends Disposable implements ISessionsList {
	...
	private static readonly EXCLUDED_TYPES_KEY =
		"sessionsListControl.excludedSessionTypes";
	private static readonly EXCLUDED_STATUSES_KEY =
		"sessionsListControl.excludedStatuses";
	private static readonly EXCLUDE_ARCHIVED_KEY =
		"sessionsListControl.excludeArchived";
	private static readonly EXCLUDE_READ_KEY = "sessionsListControl.excludeRead";
	private static readonly WORKSPACE_GROUP_CAPPED_KEY =
		"sessionsListControl.workspaceGroupCapped";
	...
	private readonly _onDidChangeFindOpenState = this._register(
		new Emitter<boolean>(),
	);
	readonly onDidChangeFindOpenState: Event<boolean> =
		this._onDidChangeFindOpenState.event;
	...
	constructor(
		...
		private readonly _sessionsListModelService: ISessionsListModelService,
		@IAgentHostFilterService
		private readonly _agentHostFilterService: IAgentHostFilterService,
		...
		this._excludeArchived = this.storageService.getBoolean(
			SessionsList.EXCLUDE_ARCHIVED_KEY,
			StorageScope.PROFILE,
			true,
		);
		this._excludeRead = this.storageService.getBoolean(
			SessionsList.EXCLUDE_READ_KEY,
			StorageScope.PROFILE,
			false,
		);
		this.workspaceGroupCapped = this.storageService.getBoolean(
			SessionsList.WORKSPACE_GROUP_CAPPED_KEY,
			StorageScope.PROFILE,
			true,
		);
		...
		this._register(
			DOM.addDisposableListener(
				this.listContainer.ownerDocument,
		...
		const approvalModel = this._register(
			instantiationService.createInstance(AgentSessionApprovalModel),
		);
		const markdownRendererService = instantiationService.invokeFunction(
			(accessor) => accessor.get(IMarkdownRendererService),
		);
		const hoverService = instantiationService.invokeFunction((accessor) =>
			accessor.get(IHoverService),
		);
		const agentSessionsService = instantiationService.invokeFunction(
			(accessor) => accessor.get(IAgentSessionsService),
		);
		const accessibilityService = instantiationService.invokeFunction(
			(accessor) => accessor.get(IAccessibilityService),
		);
		const sessionsProvidersService = instantiationService.invokeFunction(
			(accessor) => accessor.get(ISessionsProvidersService),
		);
		const sessionRenderer = new SessionItemRenderer(
			{
				...
				visibleSessions: this._sessionsManagementService.visibleSessions,
			},
			approvalModel,
		...
		this.tree = this._register(
			instantiationService.createInstance(
				...
				this.listContainer,
				delegate,
				[sessionRenderer, sectionRenderer, showMoreRenderer],
				{
		...
		this._register(
			sessionRenderer.onDidChangeItemHeight((session) => {
		...
		this._register(
			this.tree.onDidChangeFindOpenState((open) => {
				isFindOpen = open;
		...
		this._register(
			this._sessionsListModelService.onDidChange(() => {
				if (this.visible) {
					this.update();
				}
			}),
	...
	update(expandAll?: boolean): void {
		...
		if (this.excludedSessionTypes.size > 0) {
			filtered = filtered.filter(
				(s) => !this.excludedSessionTypes.has(s.sessionType),
			);
		}
		if (this.excludedStatuses.size > 0) {
			filtered = filtered.filter(
				(s) => !this.excludedStatuses.has(s.status.get()),
			);
		}
		...
		if (
			activeSession &&
			!filtered.some((s) => s.sessionId === activeSession.sessionId)
		) {
		...
		const hasTodaySessions = sections.some(
			(s) => s.id === "today" && s.sessions.length > 0,
		);
			...
			if (workspaceSections.length > 0) {
				...
				const isRecent = (section: ISessionSection) =>
					section.sessions.some(
						(s) => s.updatedAt.get().getTime() >= now - FOUR_DAYS_MS,
					);
				const isOpenWindow = (section: ISessionSection) =>
					!!this.openWindowSourceFolder &&
					section.sessions.some((s) =>
						sessionMatchesFolder(s, this.openWindowSourceFolder!),
					);
				const meetsCriteria = (section: ISessionSection) =>
					isRecent(section) || isOpenWindow(section);
		...
		const renderSection = (
			section: ISessionSection,
		): IObjectTreeElement<SessionListItem> => {
			...
			if (isCapped) {
				const visible = section.sessions.slice(
					0,
					SessionsList.WORKSPACE_GROUP_LIMIT,
				);
				...
				sectionChildren = section.sessions.map((session) => ({
					element: session as SessionListItem,
				}));
			}

			// Default collapse state for older time sections
			let defaultCollapsed: boolean | ObjectTreeElementCollapseState =
			...
			if (this.expandedMoreFolders) {
				...
				children.push({
					element: {
						...
						mode: "less" as const,
						sectionLabel: SHOW_MORE_FOLDERS_LABEL,
						remainingCount: 0,
					},
				});
			} else {
				children.push({
					element: {
						...
						mode: "more" as const,
						sectionLabel: SHOW_MORE_FOLDERS_LABEL,
						remainingCount: moreFolderSections.length,
					},
				});
			}
		}
	...
	getVisibleSessions(): readonly ISession[] {
		...
		const collect = (
			node: ITreeNode<SessionListItem | null, FuzzyScore | undefined>,
		): void => {
			if (!node.visible) {
				return;
			}
	...
	hasFocusOrSelection(): boolean {
		return (
			this.tree.getFocus().length > 0 || this.tree.getSelection().length > 0
		);
	}
	...
	private onContextMenu(
		e: ITreeContextMenuEvent<SessionListItem | null>,
	): void {
		const element = e.element;
		...
		const selection = this.tree
			.getSelection()
			.filter(
				(s): s is ISession =>
					!!s && !isSessionSection(s) && !isSessionShowMore(s),
			);
		const selectedSessions = selection.includes(element)
			? [element, ...selection.filter((s) => s !== element)]
			: [element];
		...
		const wrapForExtensions = (action: IAction): IAction => {
			...
			const wrapped = new Action(
				action.id,
				action.label,
				action.class,
				action.enabled,
				() => this.commandService.executeCommand(action.id, marshalledArg),
			);
		...
		this.contextMenuService.showContextMenu({
			getActions: () =>
				Separator.join(
					...menu
						.getActions({ arg: selectedSessions, shouldForwardArgs: true })
						.map(([, actions]) => actions.map(wrapForExtensions)),
				),
			getAnchor: () => e.anchor,
			getKeyBinding: (action) =>
				this.keybindingService.lookupKeybinding(action.id) ?? undefined,
		});
	...
	resetSectionCollapseState(): void {
		this.storageService.remove(
			SessionsList.SECTION_COLLAPSE_STATE_KEY,
			StorageScope.PROFILE,
		);
	}
	...
	private loadExcludedSessionTypes(): Set<string> {
		const raw = this.storageService.get(
			SessionsList.EXCLUDED_TYPES_KEY,
			StorageScope.PROFILE,
		);
		if (raw) {
			try {
	...
	private loadExcludedStatuses(): Set<SessionStatus> {
		const raw = this.storageService.get(
			SessionsList.EXCLUDED_STATUSES_KEY,
			StorageScope.PROFILE,
		);
		if (raw) {
			try {
...
function getFirstApprovalAcrossChats(
	approvalModel: AgentSessionApprovalModel,
	session: ISession,
	reader: IReader | undefined,
): IAgentSessionApprovalInfo | undefined {
	...
	for (const chat of session.chats.read(reader)) {
		...
		if (
			approval &&
			(!oldest || approval.since.getTime() < oldest.since.getTime())
		) {
			oldest = approval;
		}
	}
...
function sessionMatchesFolder(session: ISession, folder: URI): boolean {
	...
	for (const folder of workspace.folders) {
		if (
			folder.workingDirectory?.toString() === folderStr ||
			folder.root.toString() === folderStr
		) {
			return true;
		}
	}
...
export function groupByWorkspace(sessions: ISession[]): ISessionSection[] {
	...
	if (unknownWorkspace) {
		result.push({
			id: `workspace:${unknownWorkspaceLabel}`,
