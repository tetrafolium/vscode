...
import {
	...
	IsSessionArchivedContext,
	IsSessionReadContext,
	SessionsGrouping,
	SessionsSorting,
	ISessionSection,
...
registerAction2(
	class CloseSessionAction extends Action2 {
		constructor() {
			super({
...
const openSessionAtIndex = (
	...
	const target =
		sessionIndex === -1 ? visible[visible.length - 1] : visible[sessionIndex];
	if (!target) {
		return;
	}
...
registerAction2(
	class SortByCreatedAction extends Action2 {
		constructor() {
			super({
				...
				toggled: ContextKeyExpr.equals(
					SessionsViewSortingContext.key,
...
registerAction2(
	class SortByUpdatedAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class GroupByWorkspaceAction extends Action2 {
		constructor() {
			super({
				...
				category: SessionsCategories.Sessions,
				toggled: ContextKeyExpr.equals(
					SessionsViewGroupingContext.key,
...
registerAction2(
	class GroupByTimeAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class ShowRecentWorkspaceSessionsAction extends Action2 {
		constructor() {
			super({
		...
		override run(accessor: ServicesAccessor) {
			...
			IsWorkspaceGroupCappedContext.bindTo(
...
registerAction2(
	class ShowAllWorkspaceSessionsAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class CollapseAllGroupsAction extends Action2 {
		constructor() {
			super({
				...
				category: SessionsCategories.Sessions,
				menu: [
					{ id: SessionsViewFilterSubMenu, group: "4_collapse", order: 0 },
				],
			});
		}
		override run(accessor: ServicesAccessor) {
...
registerAction2(
	class RefreshSessionsAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class FindSessionAction extends Action2 {
		constructor() {
			super({
				id: "sessionsViewPane.find",
				title: localize2("find", "Find Session"),
...
registerAction2(
	class NewSessionForWorkspaceAction extends Action2 {
		constructor() {
			super({
		...
		async run(
			...
			if (folderUri) {
				sessionsPartService
					.getSessionView(newSession?.sessionId)
...
function getArchiveSectionConfirmationMessage(
	context: ISessionSection,
): string {
...
registerAction2(
	class ArchiveSectionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class UnarchiveSectionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class PinSessionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class UnpinSessionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class ArchiveSessionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class UnarchiveSessionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class RenameSessionAction extends Action2 {
		constructor() {
			super({
		...
		async run(
			...
			if (newTitle) {
				...
				if (trimmedTitle) {
					await sessionsManagementService.renameChat(
						session,
...
registerAction2(
	class MarkSessionReadAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class MarkSessionUnreadAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class OpenSessionToTheSideAction extends Action2 {
		constructor() {
			super({
		...
		async run(
			...
			for (let i = 0; i < sessions.length - 1; i++) {
				...
				if (lastVisible && lastVisible.sessionId !== session.sessionId) {
					sessionsManagementService.insertAt(
						session,
						lastVisible.sessionId,
						"right",
					);
				}
			}

			await openSessionToTheSide(
...
registerAction2(
	class MarkAllSessionsReadAction extends Action2 {
		constructor() {
			super({
		...
		run(accessor: ServicesAccessor): void {
			...
			const sessionsListModelService = accessor.get(ISessionsListModelService);
			const sessions = sessionsManagementService
				.getSessions()
				.filter(
					(s) =>
						!s.isArchived.get() && !sessionsListModelService.isSessionRead(s),
				);
...
registerAction2(
	class MarkSessionAsDoneAction extends Action2 {
		constructor() {
			super({
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			if (
				!activeSession ||
				activeSession.status.get() === SessionStatus.Untitled
			) {
				return;
			}
...
registerAction2(
	class RestoreSessionAction extends Action2 {
		constructor() {
			super({
