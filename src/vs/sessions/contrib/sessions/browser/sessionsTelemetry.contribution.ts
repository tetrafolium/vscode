...
} from "../../../../workbench/contrib/chat/common/attachments/chatVariableEntries.js";
import {
	getExcludes,
...
import {
	IAgentFeedbackAddedEvent,
	IAgentFeedbackConvertedEvent,
	IAgentFeedbackReplyAddedEvent,
	IAgentFeedbackService,
	IAgentFeedbackSubmittedEvent,
} from "../../agentFeedback/browser/agentFeedbackService.js";
import {
	IChat,
	ISession,
	ISessionWorkspace,
 ...
 */
export class SessionsTelemetryContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private readonly _workspaceFileCountInFlight = new Map<
		string,
		Promise<number>
	>();
	...
	constructor(
		...
		this._register(
			this._sessionsManagementService.onWillSendRequest((session) => {
		...
		this._register(
			this._sessionsManagementService.onDidSendRequest((e) =>
				this._logRequestSent(e),
			),
		);
		this._register(
			this._sessionsManagementService.onDidArchiveSession((session) =>
				this._logSessionArchived(session),
			),
		);
		this._register(
			this._sessionsManagementService.onDidUnarchiveSession((session) =>
				this._logSessionUnarchived(session),
			),
		);
		this._register(
			this._sessionsManagementService.onDidDeleteSession((session) =>
				this._logSessionDeleted(session),
			),
		);
		this._register(
			this._sessionsManagementService.onDidDeleteChat((session) =>
				this._logChatDeleted(session),
			),
		);
		this._register(
			this._sessionsManagementService.onDidRenameChat((session) =>
				this._logChatRenamed(session),
			),
		);
		this._register(
			this._sessionsManagementService.onDidToggleSessionStickiness((e) =>
				this._logSessionStickinessToggled(e.session, e.sticky),
			),
		);
		this._register(
			sessionsPartService.onDidToggleMaximizeSession((e) =>
				this._logSessionMaximizeToggled(e.session, e.maximized),
			),
		);
		...
		this._register(
			agentFeedbackService.onDidAddFeedback((e) => this._logFeedbackAdded(e)),
		);
		this._register(
			agentFeedbackService.onDidConvertFeedback((e) =>
				this._logFeedbackConverted(e),
			),
		);
		this._register(
			agentFeedbackService.onDidAddReply((e) => this._logFeedbackReplyAdded(e)),
		);
		this._register(
			agentFeedbackService.onDidSubmitFeedback((e) =>
				this._logFeedbackSubmitted(e),
			),
		);
	}
	 ...
	 */
	private _getSessionFromCommandArgs(
		...
		if (URI.isUri(first)) {
			...
			first &&
			typeof first === "object" &&
			URI.isUri((first as { resource?: unknown }).resource)
		) {
			resource = (first as { resource: URI }).resource;
		}
		return resource
			? this._sessionsManagementService.getSession(resource)
			: undefined;
	}
	...
	private _logRequestSent(e: ISendRequestSentEvent): void {
		...
		const visibleSessionsCount = this._sessionsManagementService.visibleSessions
			.get()
		...
		const sync = {
			...
			...this._incrementAndGetUserRequestCounters(
				session.providerId,
				workspace?.uri.toString(),
			),
		};
		void this._getOrFetchWorkspaceFileCount(session.sessionId, workspace).then(
			(workspaceFileCount) => {
				this._telemetryService.publicLog2<
					SessionRequestSentEvent,
					SessionRequestSentClassification
				>("agents/requestSent", {
					...sync,
	...
	private _logSessionArchived(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				SessionArchivedClassification
			>("agents/sessionArchived", payload);
		});
	}
	...
	private _logSessionUnarchived(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				SessionUnarchivedClassification
			>("agents/sessionUnarchived", payload);
		});
	}
	...
	private _logSessionDeleted(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				SessionDeletedClassification
			>("agents/sessionDeleted", payload);
		});
	}
	...
	private _logChatDeleted(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				ChatDeletedClassification
			>("agents/chatDeleted", payload);
		});
	}
	...
	private _logChatRenamed(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				ChatRenamedClassification
			>("agents/chatRenamed", payload);
		});
	}

	private _logSessionStickinessToggled(
		session: ISession,
		sticky: boolean,
	): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionStickinessToggledEvent,
				SessionStickinessToggledClassification
			>("agents/sessionStickinessToggled", {
				...payload,
				sticky,
			});
		});
	...
	private _logSessionMaximizeToggled(
		session: ISession,
		maximized: boolean,
	): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionMaximizeToggledEvent,
				SessionMaximizeToggledClassification
			>("agents/sessionMaximizeToggled", {
				...payload,
				maximized,
			});
		});
	...
	private _logCreatePullRequest(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				CreatePullRequestClassification
			>("agents/createPullRequest", payload);
		});
	}
	...
	private _logCreateDraftPullRequest(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				CreateDraftPullRequestClassification
			>("agents/createDraftPullRequest", payload);
		});
	}
	...
	private _logUpdatePullRequest(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				UpdatePullRequestClassification
			>("agents/updatePullRequest", payload);
		});
	}
	...
	private _logMergePullRequest(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				MergePullRequestClassification
			>("agents/mergePullRequest", payload);
		});
	}
	...
	private _logCheckoutPullRequest(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				CheckoutPullRequestClassification
			>("agents/checkoutPullRequest", payload);
		});
	}
	...
	private _logInitializeRepository(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				InitializeRepositoryClassification
			>("agents/initializeRepository", payload);
		});
	}
	...
	private _logCommitAndSync(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				CommitAndSyncClassification
			>("agents/commitAndSync", payload);
		});
	}
	...
	private _logSessionRestored(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				SessionRestoredClassification
			>("agents/sessionRestored", payload);
		});
	}
	...
	private _logFixCIChecks(session: ISession): void {
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				SessionActionEvent,
				FixCIChecksClassification
			>("agents/fixCIChecks", payload);
		});
	}
	...
	private _logFeedbackAdded(e: IAgentFeedbackAddedEvent): void {
		...
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				FeedbackAddedEvent,
				FeedbackAddedClassification
			>("agents/feedbackAdded", {
				...payload,
				hasSuggestion,
	...
	private _logFeedbackConverted(e: IAgentFeedbackConvertedEvent): void {
		...
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				FeedbackConvertedEvent,
				FeedbackConvertedClassification
			>("agents/feedbackConverted", {
				...payload,
				feedbackKind,
				hasSuggestion,
				hasExistingFeedbackForFile,
	...
	private _logFeedbackReplyAdded(e: IAgentFeedbackReplyAddedEvent): void {
		...
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				FeedbackReplyAddedEvent,
				FeedbackReplyAddedClassification
			>("agents/feedbackReplyAdded", {
				...payload,
				feedbackKind,
				replyCount,
			});
	...
	private _logFeedbackSubmitted(e: IAgentFeedbackSubmittedEvent): void {
		...
		void this._getSessionActionPayload(session).then((payload) => {
			this._telemetryService.publicLog2<
				...
				FeedbackSubmittedClassification
			>("agents/feedbackSubmitted", {
				...payload,
				totalCount,
				userCount,
				codeReviewCount,
	...
	private _getSessionActionPayload(
		session: ISession,
	): Promise<SessionActionEvent> {
		const workspace = session.workspace.get();
		const sessionFields = this._getSessionFields(session);
		const changesFields = this._getSessionChangesFields(session);
		return this._getOrFetchWorkspaceFileCount(
			session.sessionId,
			workspace,
		).then((workspaceFileCount) => ({
			...sessionFields,
			...this._getWorkspaceFields(workspace, workspaceFileCount),
			...changesFields,
		}));
	...
	private _getWorkspaceFields(
		...
		const hasWorktree = workspace.folders.some(
			(folder) => folder.gitRepository?.workTreeUri !== undefined,
		);
		return {
			isolationKind: hasWorktree ? "worktree" : "folder",
			workspaceHash: hash(workspace.uri.toString()).toString(16),
			hasGitRepository: workspace.folders.some(
				(folder) => folder.gitRepository !== undefined,
			),
	...
	private _getOrFetchWorkspaceFileCount(
		sessionId: string,
		workspace: ISessionWorkspace | undefined,
	): Promise<number> {
	...
	private _startWorkspaceFileCountFetch(
		workspace: ISessionWorkspace | undefined,
		...
		if (!pending) {
			pending = this._computeWorkspaceFileCount(workspace).then(
				(count) => {
	...
	private async _computeWorkspaceFileCount(
		workspace: ISessionWorkspace,
	): Promise<number> {
		const excludePattern = getExcludes(
			this._configurationService.getValue<ISearchConfiguration>({
				resource: workspace.uri,
			}),
		);
		const result = await this._searchService.fileSearch({
			folderQueries: workspace.folders.map((folder) => ({
				folder: folder.root,
	...
	private _getAllSessionsFields(
		anchorSession: ISession,
		allSessions: readonly ISession[],
	): AllSessionsFields {
	...
	private _incrementAndGetUserRequestCounters(
		providerId: string,
		workspaceUri: string | undefined,
	): UserRequestCountersFields {
		const userRequestsTotal =
		...
		if (workspaceUri) {
			...
			this._storageService.store(
				WORKSPACE_REQUESTS_KEY,
				JSON.stringify(workspaceCounts),
				StorageScope.APPLICATION,
				StorageTarget.MACHINE,
			);
		}
		...
		return {
			userRequestsTotal,
			userRequestsInWorkspace,
			userRequestsForProvider,
		};
	}
...
type SessionRequestSentClassification = {
	...
	isNewSession: {
		...
		comment: "True when the request starts a brand-new session, false when it is a new or continued chat in an existing session.";
	};
	visibleSessionsCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "How many sessions are currently visible in the sessions grid.";
	};
	agentSessionId: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Globally unique session id (providerId:resourceUri), used to correlate events for the same session.";
	};
	providerId: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The sessions provider identifier (e.g., remote agent host or local).";
	};
	providerType: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The session type identifier provided by the sessions provider.";
	};
	chatCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of chats currently in the session.";
	};
	chatModeKind: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Built-in chat mode kind (e.g., ask, agent, edit); empty when no mode is selected.";
	};
	isolationKind: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Isolation mode used by the session (worktree or folder).";
	};
	workspaceHash: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Non-reversible hash of the workspace URI, used to correlate events across the same workspace without disclosing the path.";
	};
	hasGitRepository: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether any of the workspace folders has a git repository.";
	};
	isVirtualWorkspace: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the workspace URI uses a non-file scheme (virtual/remote).";
	};
	workspaceFileCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of files in the workspace (honoring user excludes); -1 if the workspace could not be scanned.";
	};
	queryLength: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of characters in the user query. Length only, no content.";
	};
	totalAttachementCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Total number of attached context entries included with the request.";
	};
	fileAttachmentCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of file attachments included with the request.";
	};
	imageAttachmentCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of image attachments included with the request.";
	};
	attachmentKinds: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Stringified JSON object mapping each attachment kind (e.g. file, image, symbol) to its count for this request.";
	};
	currentWorkspaceFolderInProgress: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "In-progress sessions in the current workspace using folder isolation.";
	};
	currentWorkspaceFolderUnread: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Unread sessions in the current workspace using folder isolation.";
	};
	currentWorkspaceFolderWaitingForInput: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Sessions waiting for user input in the current workspace using folder isolation.";
	};
	currentWorkspaceFolderNotDone: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Sessions not marked as done in the current workspace using folder isolation.";
	};
	currentWorkspaceInProgress: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "In-progress sessions in the current workspace across all isolation modes.";
	};
	currentWorkspaceUnread: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Unread sessions in the current workspace across all isolation modes.";
	};
	currentWorkspaceWaitingForInput: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Sessions waiting for user input in the current workspace across all isolation modes.";
	};
	currentWorkspaceNotDone: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Sessions not marked as done in the current workspace across all isolation modes.";
	};
	allWorkspacesInProgress: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "In-progress sessions across all workspaces.";
	};
	allWorkspacesUnread: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Unread sessions across all workspaces.";
	};
	allWorkspacesWaitingForInput: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Sessions waiting for user input across all workspaces.";
	};
	allWorkspacesNotDone: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Sessions not marked as done across all workspaces.";
	};
	userRequestsTotal: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Cumulative number of requests the user has sent from the Agents window across all workspaces and providers (including this one).";
	};
	userRequestsInWorkspace: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Cumulative number of requests the user has sent in the current workspace (including this one).";
	};
	userRequestsForProvider: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Cumulative number of requests the user has sent for this sessions provider across all workspaces (including this one).";
	};
};
...
type SessionArchivedClassification = {
	...
	sessionFilesChanged: {
		...
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of files changed in the session at the time of the action.";
	};
	sessionLinesAdded: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Total number of lines added across all changed files in the session at the time of the action.";
	};
	sessionLinesDeleted: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Total number of lines deleted across all changed files in the session at the time of the action.";
	};
};
...
type FeedbackAddedClassification = {
	...
	hasSuggestion: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the feedback comment includes a suggested code edit.";
	};
	hasExistingFeedbackForFile: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the session already had at least one feedback comment for the same file before this one was added.";
	};
};
...
type FeedbackConvertedClassification = {
	...
	feedbackKind: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Origin of the converted comment: codeReview (in-product code review) or prReview (pull request review).";
	};
	hasSuggestion: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the converted comment includes a suggested code edit.";
	};
	hasExistingFeedbackForFile: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the session already had at least one feedback comment for the same file before the conversion.";
	};
};
...
type FeedbackReplyAddedClassification = {
	...
	feedbackKind: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Origin of the feedback thread that received the reply (user, codeReview, prReview).";
	};
	replyCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of replies on the feedback thread after the new reply was appended.";
	};
};
...
type FeedbackSubmittedClassification = {
	...
	totalCount: {
		...
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Total number of feedback items being submitted.";
	};
	userCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of user-authored feedback items being submitted.";
	};
	codeReviewCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of feedback items being submitted that originated as code review comments.";
	};
	prReviewCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Number of feedback items being submitted that originated as PR review comments.";
	};
	replyCount: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		isMeasurement: true;
		comment: "Total number of replies across all feedback items being submitted.";
	};
};
...
type SessionStickinessToggledClassification = {
	...
	sticky: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The session's stickiness state after the toggle: true when the session is now sticky, false when it was unstuck.";
	};
};
...
type SessionMaximizeToggledClassification = {
	...
	maximized: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The session view's maximized state after the toggle: true when the view is now maximized, false when it was restored.";
	};
};
