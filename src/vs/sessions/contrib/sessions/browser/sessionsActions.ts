...
registerAction2(
	class ShowSessionsPickerAction extends Action2 {
		constructor() {
			super({
		...
		override async run(accessor: ServicesAccessor) {
			...
			const activeSessionId =
				sessionsManagementService.activeSession.get()?.sessionId;
			...
			if (sessions.length > 0) {
				items.push({
					type: "separator",
					label: localize("recentSessions", "Recent Sessions"),
				});

				for (const session of sessions) {
					const title =
						session.title.get() || localize("untitledSession", "New Session");
			...
			const picker = quickInputService.createQuickPick<ISessionPickItem>({
				useSeparators: true,
			});
			picker.items = items;
			...
			super({
				...
				title: {
					...
					mnemonicTitle: localize(
						{ key: "miSessionsBack", comment: ["&& denotes a mnemonic"] },
						"&&Back",
					),
				},
				f1: true,
		...
		constructor() {
			super({
				...
				title: {
					...
					mnemonicTitle: localize(
						{ key: "miSessionsForward", comment: ["&& denotes a mnemonic"] },
						"&&Forward",
					),
				},
				f1: true,
...
registerAction2(
	class FocusActiveSessionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class AddChatToSessionBarAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class TogglePinSessionAction extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class ToggleMaximizeSessionViewAction extends Action2 {
		constructor() {
			super({
