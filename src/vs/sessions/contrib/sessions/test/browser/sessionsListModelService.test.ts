...
suite("SessionsListModelService", () => {
	...
	setup(() => {
		...
		sessionsChangedEmitter = disposables.add(
		...
		service = disposables.add(
			instantiationService.createInstance(SessionsListModelService),
		);
	});
	...
	test("pre-cutoff session becomes unread when updatedAt advances past cutoff", () => {
		...
		(session.updatedAt as ISettableObservable<Date>).set(
			AFTER_CUTOFF,
			undefined,
		);
	...
	test("marks session unread when it transitions from InProgress to Completed in background", () => {
		...
		(session.status as ISettableObservable<SessionStatus>).set(
	...
	test("does not mark active session unread when it transitions from InProgress to Completed", () => {
		...
		activeSession.set(
			{
				...session,
				activeChat: constObservable(session.mainChat.get()),
	...
	test("state is loaded from storage on construction", () => {
		...
		instantiationService.stub(ISessionsManagementService, {
			...mock<ISessionsManagementService>(),
			onDidChangeSessions: disposables.add(new Emitter<ISessionsChangeEvent>())
				.event,
			activeSession: constObservable(undefined),
		});
		const loadedService = disposables.add(
			instantiationService.createInstance(SessionsListModelService),
		);

		assert.strictEqual(
			loadedService.isSessionPinned(createSession("s1")),
			true,
		);
		assert.strictEqual(
			loadedService.isSessionPinned(createSession("s2")),
			false,
		);
