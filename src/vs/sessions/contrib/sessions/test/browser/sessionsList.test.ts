...
import {
	...
	SessionsGrouping,
	SessionsSorting,
} from "../../browser/views/sessionsList.js";
...
suite("Sessions - SessionsList Helpers", () => {
	...
	suite("groupByWorkspace", () => {
		...
		test("group ids are prefixed with workspace:", () => {
			const sessions = [createSession("1", { workspaceLabel: "MyProject" })];
	...
	suite("groupSessionsForList", () => {
		test("shows pinned sessions in a dedicated top section", () => {
			const pinned = createSession("pinned", {
				workspaceLabel: "Alpha",
				createdAt: new Date("2024-06-01"),
			});
			const regular = createSession("regular", {
				workspaceLabel: "Beta",
			...
			assert.deepStrictEqual(
				...
				["pinned", "workspace:Beta"],
			);
			assert.deepStrictEqual(
				sections[0].sessions.map((session) => session.sessionId),
				["pinned"],
			);
		});

		test("keeps archived sessions in Done even when pinned", () => {
			const archivedPinned = createSession("archived-pinned", {
				workspaceLabel: "Alpha",
				isArchived: true,
				createdAt: new Date("2024-06-01"),
			});
