...
class MockRepositoryFetcher {
	...
	async getRepository(
		...
		data: IGitHubRepository | undefined;
		statusCode: number;
		etag?: string;
	}> {
...
class MockPRFetcher {
	...
	async getPullRequest(
		...
		data: IGitHubPullRequest | undefined;
		statusCode: number;
		etag?: string;
	}> {
	...
	async getReviews(
		...
		data: readonly IGitHubPullRequestReview[] | undefined;
		statusCode: number;
		etag?: string;
	}> {
...
class MockCIFetcher {
	...
	async getCheckRuns(
		...
		data: readonly IGitHubCICheck[] | undefined;
		statusCode: number;
		etag?: string;
	}> {
	...
	test("initial state is undefined", () => {
		const model = store.add(
			new GitHubRepositoryModel(
				"owner",
				"repo",
				mockFetcher as unknown as GitHubRepositoryFetcher,
				logService,
			),
		);
...
suite("GitHubPullRequestModel", () => {
	...
	test("initial state has empty observables", () => {
		const model = store.add(
			new GitHubPullRequestModel(
				"owner",
				"repo",
				1,
				mockFetcher as unknown as GitHubPRFetcher,
				logService,
			),
		);
	...
	test("polling stops when the last client stops polling", () =>
...
suite("GitHubPullRequestReviewThreadsModel", () => {
	...
	test("refresh updates only review threads", async () => {
		...
		mockFetcher.nextThreads = [
			makeThread("thread-100", "src/a.ts"),
			makeThread("thread-200", "src/b.ts"),
		];
...
suite("GitHubPullRequestCIModel", () => {
	...
	setup(() => {
		...
		collection = new TestCIReferenceCollection(
			mockFetcher as unknown as GitHubPRCIFetcher,
			logService,
		);
	});
	...
	test("rerunFailedCheck refreshes after an in-progress refresh completes", async () => {
		...
		const rerun = model.rerunFailedCheck({
			id: 1,
			name: "build",
			status: GitHubCheckStatus.Completed,
			conclusion: GitHubCheckConclusion.Failure,
