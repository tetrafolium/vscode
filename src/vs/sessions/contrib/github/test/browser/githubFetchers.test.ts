...
import {
	GitHubPRFetcher,
	computeMergeability,
} from "../../browser/fetchers/githubPRFetcher.js";
import {
	GitHubPRCIFetcher,
	computeOverallCIStatus,
} from "../../browser/fetchers/githubPRCIFetcher.js";
import { GitHubRepositoryFetcher } from "../../browser/fetchers/githubRepositoryFetcher.js";
import {
	GitHubApiClient,
	GitHubApiError,
	IGitHubApiRequestOptions,
} from "../../browser/githubApiClient.js";
import {
...
class MockApiClient {
	...
	async request<T>(
		...
		_path: string,
		_callSite: string,
		_options?: IGitHubApiRequestOptions,
	): Promise<{ data: T | undefined; statusCode: number; etag?: string }> {
...
suite("GitHubRepositoryFetcher", () => {
	...
	setup(() => {
		...
		fetcher = new GitHubRepositoryFetcher(
			mockApi as unknown as GitHubApiClient,
		);
	});
	...
	test("getRepository propagates API errors", async () => {
		...
		await assert.rejects(
			...
			(err: Error) =>
				...
				(err as GitHubApiError).statusCode === 404,
		);
	});
});
...
suite("GitHubPRFetcher", () => {
	...
	test("getPullRequest maps open PR", async () => {
		mockApi.setNextResponse(
			makePRResponse({ state: "open", merged: false, draft: false }),
		);
	...
	test("getPullRequest maps merged PR", async () => {
		mockApi.setNextResponse(
			makePRResponse({ state: "closed", merged: true, draft: false }),
		);
	...
	test("getPullRequest maps closed PR", async () => {
		mockApi.setNextResponse(
			makePRResponse({ state: "closed", merged: false, draft: false }),
		);
	...
	test("getReviewThreads returns GraphQL thread metadata", async () => {
		mockApi.setNextResponse(
			makeGraphQLReviewThreadsResponse([
	...
	test("computeMergeability detects conflicts blocker", () => {
		...
		assert.strictEqual(result.canMerge, false);
		assert.ok(
			result.blockers.some((b) => b.kind === MergeBlockerKind.Conflicts),
		);
	});
	...
	test("computeMergeability detects changes requested blocker", () => {
		...
		assert.strictEqual(result.canMerge, false);
		assert.ok(
			result.blockers.some((b) => b.kind === MergeBlockerKind.ChangesRequested),
		);
	});
...
function makeGraphQLReviewThreadsResponse(
	threads: readonly ReturnType<typeof makeGraphQLReviewThread>[],
): unknown {
	return {
		repository: {
			pullRequest: {
...
function makeGraphQLReviewThread(
	overrides: Partial<{
		id: string;
		isResolved: boolean;
		path: string;
...
function makeGraphQLReviewComment(
	overrides: Partial<{
		databaseId: number;
		body: string;
		path: string;
	...
	return {
		...
		replyTo:
			overrides.replyToDatabaseId !== undefined
				? { databaseId: overrides.replyToDatabaseId }
				: null,
		author: {
			login: "reviewer",
