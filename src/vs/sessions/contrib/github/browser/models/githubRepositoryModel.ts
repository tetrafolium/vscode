...
export class GitHubRepositoryModelReferenceCollection extends ReferenceCollection<GitHubRepositoryModel> {
	...
	protected override createReferencedObject(
		...
		this._logService.trace(
			`[GitHubRepositoryModelReferenceCollection][createReferencedObject] Creating repository model for ${key}`,
		);
	...
	protected override destroyReferencedObject(
		key: string,
		object: GitHubRepositoryModel,
	): void {
		this._logService.trace(
			`[GitHubRepositoryModelReferenceCollection][destroyReferencedObject] Disposing repository model for ${key}`,
		);
		object.dispose();
	}
}
...
export class GitHubRepositoryModel extends Disposable {
	...
	private readonly _repository = observableValue<IGitHubRepository | undefined>(
		this,
		undefined,
	);
	...
	private async _refresh(): Promise<void> {
		try {
			const response = await this._fetcher.getRepository(
				this.owner,
				this.repo,
				this._repositoryEtag,
			);
			...
			this._logService.error(
				`${LOG_PREFIX} Failed to refresh repository ${this.owner}/${this.repo}:`,
				err,
			);
		}
	}
