...
} from "./models/githubRepositoryModel.js";
import {
	GitHubPullRequestModel,
	GitHubPullRequestModelReferenceCollection,
} from "./models/githubPullRequestModel.js";
import {
...
} from "./models/githubPullRequestCIModel.js";
...
export interface IGitHubService {
	...
	activeSessionPullRequestCIObs: IObservable<
		GitHubPullRequestCIModel | undefined
	>;
	activeSessionPullRequestReviewThreadsObs: IObservable<
	...
	createRepositoryModelReference(
		owner: string,
		repo: string,
	): IReference<GitHubRepositoryModel>;
	...
	createPullRequestCIModelReference(
		...
		prNumber: number,
		headSha: string,
	): IReference<GitHubPullRequestCIModel>;
	...
	getChangedFiles(
		...
		base: string,
		head: string,
	): Promise<readonly IGitHubChangedFile[]>;
	 ...
	 */
	findPullRequestNumberByHeadBranch(
		owner: string,
		repo: string,
		branch: string,
...
export class GitHubService extends Disposable implements IGitHubService {
	...
	readonly activeSessionPullRequestObs: IObservable<
		GitHubPullRequestModel | undefined
	>;
	readonly activeSessionPullRequestCIObs: IObservable<
		GitHubPullRequestCIModel | undefined
	>;
	readonly activeSessionPullRequestReviewThreadsObs: IObservable<
		GitHubPullRequestReviewThreadsModel | undefined
	>;
	 ...
	 */
	private readonly _findPRByBranchCache = new Map<
		string,
		Promise<number | undefined>
	>();
	...
	constructor(
		...
		this._pullRequestReviewThreadsReferences =
		...
		const gitHubInfoObs = derivedOpts<
			{ owner: string; repo: string; pullRequestNumber: number } | undefined
		>({ equalsFn: structuralEquals }, (reader) => {
			const gitHubInfo = sessionManagementService.activeSession
				.read(reader)
		...
		this.activeSessionPullRequestObs = derived((reader) => {
			...
			const prModelRef = this.createPullRequestModelReference(
				gitHubInfo.owner,
				gitHubInfo.repo,
				gitHubInfo.pullRequestNumber,
			);
		...
		this.activeSessionPullRequestCIObs = derived((reader) => {
			...
			const prModelRef = this.createPullRequestCIModelReference(
				pullRequestInfo.owner,
				pullRequestInfo.repo,
				pullRequestInfo.prNumber,
				pullRequestInfo.headSha,
			);
			reader.store.add(prModelRef);
		...
		this.activeSessionPullRequestReviewThreadsObs = derived((reader) => {
			...
			const reviewThreadsModelRef =
				this.createPullRequestReviewThreadsModelReference(
					gitHubInfo.owner,
					gitHubInfo.repo,
					gitHubInfo.pullRequestNumber,
				);
	...
	private async _fetchPullRequestNumberByHeadBranch(
		owner: string,
		repo: string,
		branch: string,
	): Promise<number | undefined> {
		...
		>("GET", path, "githubApi.findPullRequestByHeadBranch");
