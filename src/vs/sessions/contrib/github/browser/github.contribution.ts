...
export class GitHubPullRequestPollingContribution
	extends Disposable
	...
	constructor(
		...
		const activeSessionResourceObs = derivedOpts<URI | undefined>(
			...
			(reader) => {
				...
				if (
					...
					activeSession.isArchived.read(reader)
				) {
					return undefined;
				}
		...
		this._register(
			autorun((reader) => {
				...
				const model =
					this._gitHubService.activeSessionPullRequestObs.read(reader);
				model?.refresh();
			}),
		);
		...
		this._register(
			autorun((reader) => {
				...
				const model =
					this._gitHubService.activeSessionPullRequestCIObs.read(reader);
				if (!model) {
					return;
				}
