...
export class GitHubApiClient extends Disposable {
	...
	private async _request<T>(
		...
		const rateLimitRemaining = parseRateLimitHeader(
			response.res.headers?.["x-ratelimit-remaining"],
		);
		if (rateLimitRemaining !== undefined && rateLimitRemaining < 100) {
			this._logService.warn(
				`${LOG_PREFIX} GitHub API rate limit low: ${rateLimitRemaining} remaining`,
			);
		}
		...
		if (statusCode < 200 || statusCode >= 300) {
			const errorBody = await asJson<{ message?: string }>(response).catch(
				() => undefined,
			);
			throw new GitHubApiError(
				errorBody?.message ??
					`GitHub API request failed: ${method} ${pathForLogging} (${statusCode})`,
				statusCode,
				rateLimitRemaining,
			);
		}
	...
	private async _getAuthToken(): Promise<string> {
		let sessions = await this._authenticationService.getSessions("github", [], {
			silent: true,
		});
		if (!sessions || sessions.length === 0) {
			sessions = await this._authenticationService.getSessions("github", [], {
				createIfNone: true,
			});
		}
		if (!sessions || sessions.length === 0) {
		...
		const repoScopeSession = sessions.find((session) =>
