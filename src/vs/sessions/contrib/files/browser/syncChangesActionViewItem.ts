...
export class SyncChangesActionViewItem extends ActionViewItem {
	...
	private readonly _labelUpdateDisposable = this._register(
	...
	protected override updateLabel(): void {
		...
		const repository = workspaceFolder
			? Iterable.find(this.scmService.repositories, (repo) =>
					isEqual(repo.provider.rootUri, workspaceFolder.uri),
				)
			: undefined;
		...
		>({ equalsFn: structuralEquals }, (reader) => {
			...
			const syncCommand = commands?.find(
				(c) =>
					c.title.startsWith("$(sync)") || c.title.startsWith("$(sync~spin)"),
			);
		...
		this._labelUpdateDisposable.value = autorun((reader) => {
			...
			reset(
				this.label!,
				...(syncActionDetails
					? renderLabelWithIcons(syncActionDetails.title)
					: []),
			);
