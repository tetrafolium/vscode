...
export class OpenSessionInVSCodeAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const folderUri = this.getFolderUriToOpen(
		...
		return nativeHostService.openWindow([{ folderUri }], {
			forceNewWindow: true,
		});
	}

	private getFolderUriToOpen(
