...
export class ChatCompositeBar extends Disposable {
	...
	private readonly _sessionTransfer =
	...
	constructor(
		...
		this._tabsScrollbar = this._register(
			new ScrollableElement(this._tabsContainer, {
		...
		this._register(
			addDisposableListener(this._tabsContainer, EventType.SCROLL, () => {
				this._tabsScrollbar.setScrollPosition({
					scrollLeft: this._tabsContainer.scrollLeft,
				});
			}),
		);
		...
		this._register(
			this._tabsScrollbar.onScroll((e) => {
		...
		this._inlineToolbar = this._register(
			instantiationService.createInstance(
				MenuWorkbenchToolBar,
				inlineToolbarContainer,
				Menus.SessionBarInlineToolbar,
				{
		...
		const resizeObserver = this._register(
			new DisposableResizeObserver("ChatCompositeBar.activeTabReveal", () => {
		...
		this._updateStyles();
		this._register(
			this._themeService.onDidColorThemeChange(() => this._updateStyles()),
		);
	...
	private _registerDragSource(): void {
		...
		this._register(
			addDisposableListener(this._container, EventType.DRAG_END, () => {
	...
	setSession(session: IActiveSession | undefined): void {
		...
		store.add(
			autorun((reader) => {
				...
				const activeChatUri =
					session.activeChat.read(reader)?.resource.toString() ?? "";
	...
	private _createTab(chat: IChat, isMainChat: boolean): void {
		...
		this._tabDisposables.add(
			autorun((reader) => {
				...
				const isActive =
					activeChat?.resource.toString() === chat.resource.toString();
				const status = chat.status.read(reader);
		...
		if (!isMainChat) {
			const closeAction = this._tabDisposables.add(
				new Action(
					...
					async () => {
						if (this._session) {
							await this._sessionsManagementService.deleteChat(
								this._session,
								chat.resource,
							);
						}
			...
			const actionBar = this._tabDisposables.add(
				new ActionBar(tab, { actionViewItemProvider: undefined }),
			);
		...
		this._tabDisposables.add(
			addDisposableListener(tab, EventType.KEY_DOWN, (e: KeyboardEvent) => {
		...
		const renameAction = this._tabDisposables.add(
			new Action(
				"sessionCompositeBar.renameChat",
				localize("renameChat", "Rename"),
				undefined,
				true,
				async () => {
					...
					if (newTitle && this._session) {
						await this._sessionsManagementService.renameChat(
							this._session,
							chat.resource,
							newTitle,
						);
		...
		this._tabDisposables.add(
			addDisposableListener(tab, EventType.CONTEXT_MENU, (e: MouseEvent) => {
	...
	private _revealActiveTab(): void {
		const activeTab = this._tabs.find((t) =>
			t.element.classList.contains("active"),
		);
	...
	private _updateStyles(): void {
		...
		this._container.style.setProperty(
			"--chat-tab-inactive-foreground",
			inactiveFg?.toString() ?? "",
		);
		this._container.style.setProperty(
			"--chat-tab-active-border",
			activeBorder?.toString() ?? "",
		);
	}
}
