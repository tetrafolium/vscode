...
export const ISessionsPartService = createDecorator<ISessionsPartService>(
	"sessionsPartService",
);
...
export class SessionsParts extends Disposable implements ISessionsPartService {
	...
	private readonly _onDidToggleMaximizeSession = this._register(
		new Emitter<IToggleMaximizeSessionEvent>(),
	);
	readonly onDidToggleMaximizeSession: Event<IToggleMaximizeSessionEvent> =
		this._onDidToggleMaximizeSession.event;
	...
	constructor(
		...
		this._mainPart = this._register(
			instantiationService.createInstance(
				isPhoneLayout ? MobileSessionsPart : SessionsPart,
			),
		);
	}
	...
	init(): void {
		...
		this._register(
			autorun((reader) => {
				const visible =
					this.sessionsManagementService.visibleSessions.read(reader);
				const active =
		...
		this._register(
			this._mainPart.onDidFocusSession((sessionId) => {
				const session = this.sessionsManagementService.visibleSessions
					.get()
