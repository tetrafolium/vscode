 ...
 */
export function installMobileChipLaneScroll(
	lane: HTMLElement,
			...
			(e: PointerEvent) => {
				...
				if (!didDrag) {
					...
					try {
						...
						/* not all browsers support setPointerCapture on every element */
					}
				}
	...
	const endDrag = (e: PointerEvent) => {
		...
		dom
			.getWindow(lane)
			.setTimeout(() => lane.removeEventListener("click", swallow, true), 0);
	};
