...
} from "../../../../workbench/services/chat/common/chatEntitlementService.js";
import {
	getAccountTitleBarState,
	getAccountProfileImageUrl,
	getAccountTitleBarBadgeKey,
	resolveAccountInfo,
} from "../../accountTitleBarState.js";
...
export class MobileTitlebarPart extends Disposable {
	...
	private readonly accountPanelDisposable = this._register(
		new MutableDisposable<DisposableStore>(),
	);
	private readonly avatarLoadDisposable = this._register(
		new MutableDisposable(),
	);
	private readonly copilotDashboardStore = this._register(
		new MutableDisposable<DisposableStore>(),
	);
	...
	constructor(
		...
		private readonly chatDashboardService: IChatDashboardService,
		...
		const closedIconClasses = ThemeIcon.asClassNameArray(
			Codicon.layoutSidebarLeftOff,
		);
		const openIconClasses = ThemeIcon.asClassNameArray(
			Codicon.layoutSidebarLeft,
		);
		hamburgerIcon.classList.add(...closedIconClasses);
		this._register(
			addDisposableListener(hamburger, EventType.CLICK, () =>
				this._onDidClickHamburger.fire(),
			),
		);
		...
		const updateSidebarIcon = () => {
			...
			hamburgerIcon.classList.add(
				...(isOpen ? openIconClasses : closedIconClasses),
			);
			hamburger.setAttribute(
				"aria-label",
				isOpen
		...
		this.sessionTitleElement.textContent = localize(
			"mobileTopBar.newSession",
			"New Session",
		);
		this._register(
			addDisposableListener(this.sessionTitleElement, EventType.CLICK, () =>
				this._onDidClickTitle.fire(),
			),
		);
		...
		const changesPill = append(
			this.element,
			$("button.mobile-top-bar-button.mobile-changes-pill", { type: "button" }),
		) as HTMLButtonElement;
		changesPill.setAttribute(
			"aria-label",
			localize("mobileTopBar.changes", "View changes"),
		);
		changesPill.style.display = "none";
		const changesIcon = append(changesPill, $("span.mobile-changes-pill-icon"));
		changesIcon.classList.add(
			...ThemeIcon.asClassNameArray(Codicon.diffMultiple),
		);
		const changesAddedEl = append(
			changesPill,
			$("span.mobile-changes-pill-added"),
		);
		const changesRemovedEl = append(
			changesPill,
			$("span.mobile-changes-pill-removed"),
		);
		this._register(
			addDisposableListener(changesPill, EventType.CLICK, () =>
				this.showChangesPicker(),
			),
		);

		// New session button (+) — shown when in a chat, hidden on welcome.
		// Always rightmost when in a chat.
		const newSessionButton = append(
			this.element,
			$("button.mobile-top-bar-button.mobile-new-session-button"),
		);
		newSessionButton.setAttribute(
			"aria-label",
			localize("mobileTopBar.newSessionAria", "New session"),
		);
		const newSessionIcon = append(newSessionButton, $("span"));
		newSessionIcon.classList.add(...ThemeIcon.asClassNameArray(Codicon.plus));
		this._register(
			addDisposableListener(newSessionButton, EventType.CLICK, () =>
				this._onDidClickNewSession.fire(),
			),
		);

		// Account indicator — shown on welcome/new session, hidden in a chat
		this.accountButton = append(
			this.element,
			$("button.mobile-top-bar-button.mobile-account-indicator"),
		);
		this.accountButton.setAttribute(
			"aria-label",
			localize("mobileTopBar.account", "Account"),
		);
		this.accountAvatarElement = append(
			this.accountButton,
			$("img.mobile-account-avatar", { alt: "", draggable: "false" }),
		) as HTMLImageElement;
		...
		this.accountBadgeElement = append(
			this.accountButton,
			$("span.mobile-account-badge"),
		);
		this._register(
			addDisposableListener(this.accountButton, EventType.CLICK, () =>
		...
		this._register(
			autorun((reader) => {
				...
				this.sessionTitleElement.textContent =
					title || localize("mobileTopBar.newSession", "New Session");
			}),
		);
		...
		const isNewChatRef = {
			value: !!IsNewChatSessionContext.getValue(contextKeyService),
		};
	...
	private async refreshAccount(): Promise<void> {
		...
		const info = await resolveAccountInfo(
	...
	private renderAccountState(): void {
		...
		this.accountBadgeElement.classList.toggle(
			"dot-badge-warning",
			showBadge && state.dotBadge === "warning",
		);
		this.accountBadgeElement.classList.toggle(
			"dot-badge-error",
			showBadge && state.dotBadge === "error",
		);
	...
	private refreshAvatar(): void {
		const avatarUrl = getAccountProfileImageUrl(
	...
	private showAccountPanel(): void {
		...
		const badgeKey = getAccountTitleBarBadgeKey(
			getAccountTitleBarState({
				isAccountLoading: this.isAccountLoading,
				accountName: this.accountName,
				accountProviderLabel: this.accountProviderLabel,
		...
		const closeButton = append(
			header,
			$("button.mobile-account-sheet-close", { type: "button" }),
		) as HTMLButtonElement;
		closeButton.setAttribute(
			"aria-label",
			localize("mobileAccount.close", "Close"),
		);
		append(closeButton, $("span")).classList.add(
			...ThemeIcon.asClassNameArray(Codicon.close),
		);
		panelStore.add(
			addDisposableListener(closeButton, EventType.CLICK, closeSheet),
		);
		...
		if (this.loadedAvatarUrl) {
			const avatar = append(
				profile,
				$("img.mobile-account-sheet-avatar", { alt: "", draggable: "false" }),
			) as HTMLImageElement;
			avatar.src = this.loadedAvatarUrl;
			avatar.referrerPolicy = "no-referrer";
			avatar.decoding = "async";
		} else {
			const avatarPlaceholder = append(
				profile,
				$("div.mobile-account-sheet-avatar-placeholder"),
			);
			append(avatarPlaceholder, $("span")).classList.add(
				...ThemeIcon.asClassNameArray(Codicon.account),
			);
		}
		const profileInfo = append(
			profile,
			$("div.mobile-account-sheet-profile-info"),
		);
		if (this.isAccountLoading) {
			append(profileInfo, $("div.mobile-account-sheet-name")).textContent =
				localize("mobileAccount.loading", "Loading...");
			...
			if (this.accountProviderLabel) {
				append(
					profileInfo,
					$("div.mobile-account-sheet-provider"),
				).textContent = this.accountProviderLabel;
			}
		} else {
			append(profileInfo, $("div.mobile-account-sheet-name")).textContent =
				localize("mobileAccount.signedOut", "Not signed in");
		}
		...
		if (showDashboard) {
			const dashboardSection = append(
				content,
				$("div.mobile-account-sheet-section"),
			);
			const store = new DisposableStore();
			this.copilotDashboardStore.value = store;
			const dashboardElement =
				this.chatDashboardService.createDashboardElement(store);
			if (dashboardElement) {
		...
		// Actions list
		const actionsSection = append(
			content,
			$("div.mobile-account-sheet-actions"),
		);
		...
		for (const action of allActions) {
			...
			const row = append(
				actionsSection,
				$("button.mobile-account-sheet-action", { type: "button" }),
			) as HTMLButtonElement;
			row.disabled = !action.enabled;
			row.setAttribute("aria-label", action.tooltip || action.label);
			const icon = this.getActionIcon(action);
			if (icon) {
				append(row, $("span.mobile-account-sheet-action-icon")).classList.add(
					...ThemeIcon.asClassNameArray(icon),
				);
			}
			append(row, $("span.mobile-account-sheet-action-label")).textContent =
				action.label;
			panelStore.add(
				addDisposableListener(row, EventType.CLICK, async (event) => {
	...
	private getSheetActions(): IAction[] {
		...
		return rawActions.filter((action) => {
			...
			if (
				...
				action.id === "workbench.action.agenticSignIn"
			) {
				return false;
			}
