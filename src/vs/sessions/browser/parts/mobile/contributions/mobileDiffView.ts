...
export class MobileDiffView extends Disposable {
	...
	constructor(
		...
		this.siblings =
			data.siblings && data.siblings.length > 0 ? data.siblings : [data.diff];
		...
		const [originalText, modifiedText] = await Promise.all([
			diff.originalURI
				? this.textFileService
						.read(diff.originalURI, { acceptTextOnly: true })
						.then((m) => m.value)
						.catch(() => "")
				: Promise.resolve(""),
			diff.modifiedURI
	...
	private renderHunks(
		...
		for (const hunk of hunks) {
			...
			for (const line of hunk.lines) {
				...
				numEl.textContent =
					line.lineNum !== undefined ? String(line.lineNum) : "";
 ...
 */
async function tokenizeFileLines(
...
function regexTokenizeLine(line: string, lang: LangFamily): string {
	...
	while (pos < len) {
		...
		if (
			line.startsWith(commentPfx, pos) ||
	...
	return tokens
		.map((t) => buildSpan(t.kind, line.slice(t.start, t.end)))
		.join("");
}
...
function computeUnifiedDiff(original: string, modified: string): IDiffHunk[] {
	...
	const result = linesDiffComputers
		.getDefault()
		.computeDiff(origLines, modLines, {
