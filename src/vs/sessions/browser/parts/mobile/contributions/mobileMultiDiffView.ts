...
import {
	...
	tokenizeFileLines,
} from "./mobileDiffHelpers.js";
import {
	computeMobileMultiDiffItemHeight,
	computeMobileMultiDiffVirtualLayout,
	type IMobileMultiDiffVirtualItem,
	type IMobileMultiDiffVirtualItemLayout,
	type IMobileMultiDiffVirtualizerMetrics,
} from "./mobileMultiDiffVirtualizer.js";
...
type MobileMultiDiffFileLoadState =
	| "idle"
	| "loading"
	| "loaded"
	| "empty"
...
type MobileMultiDiffBodyEntry =
	| IMobileMultiDiffBodyHunkEntry
	| IMobileMultiDiffBodyLineEntry;
...
export class MobileMultiDiffView extends Disposable {
	...
	private currentLayout:
		| ReturnType<typeof computeMobileMultiDiffVirtualLayout>
		| undefined;
	...
	constructor(
		...
		this.fileStates = data.diffs.map((diff, index) => ({
			...
			loadRequestId: 0,
			estimatedHunkCount:
				diff.identical || diff.added + diff.removed === 0 ? 0 : 1,
	...
	private render(workbenchContainer: HTMLElement): void {
		...
		const overlay = DOM.append(
			workbenchContainer,
			$("div.mobile-overlay-view.mobile-multi-diff-view"),
		);
		this.viewStore.add(
			DOM.addDisposableListener(overlay, DOM.EventType.CONTEXT_MENU, (e) =>
				e.preventDefault(),
			),
		);
		...
		this.viewStore.add(
			DOM.addDisposableListener(backBtn, DOM.EventType.CLICK, () =>
				this.dispose(),
			),
		);
		this.viewStore.add(
			DOM.addDisposableListener(backBtn, TouchEventType.Tap, () =>
				this.dispose(),
			),
		);

		const fileCount = DOM.append(
			topBar,
			$("span.mobile-multi-diff-file-count"),
		);
		fileCount.textContent = localize(
			"multiDiffView.fileCount",
			"{0} {1}",
			this.data.diffs.length,
			this.data.diffs.length === 1
				? localize("multiDiffView.file", "file")
				: localize("multiDiffView.files", "files"),
		);

		// -- Scroll body
		const body = DOM.append(overlay, $("div.mobile-overlay-body"));
		this.scrollWrapper = DOM.append(body, $("div.mobile-overlay-scroll"));
		this.virtualContent = DOM.append(
			this.scrollWrapper,
			$("div.mobile-multi-diff-virtual-content"),
		);
		this.viewStore.add(
	...
	private scrollToInitialIndex(): void {
		...
		DOM.getWindow(this.scrollWrapper).requestAnimationFrame(() => {
			...
			this.scrollWrapper.scrollTop = this.computeVirtualTop(
	...
	private renderFileSection(state: IMobileMultiDiffFileState): {
		section: HTMLElement;
		content: HTMLElement;
		...
		const chevronEl = DOM.append(
			header,
			$("span.mobile-multi-diff-file-chevron", {
				role: "button",
				tabindex: "0",
				"aria-expanded": "true",
			}),
		...
		chevronEl.setAttribute(
			...
			localize(
				...
				"Toggle {0}",
				fileName || localize("multiDiffView.fileFallback", "file"),
			),
		);

		const nameEl = DOM.append(header, $("span.mobile-multi-diff-file-name"));
		if (dirPath) {
			DOM.append(nameEl, $("span.mobile-multi-diff-file-dir")).textContent =
				dirPath;
		}
		DOM.append(nameEl, $("span.mobile-multi-diff-file-base")).textContent =
			fileName;
		...
		if (!diff.identical) {
			if (diff.added) {
				DOM.append(
					statsEl,
					$("span.mobile-multi-diff-stat-added"),
				).textContent = `+${diff.added}`;
			}
			if (diff.removed) {
				DOM.append(
					statsEl,
					$("span.mobile-multi-diff-stat-removed"),
				).textContent = `-${diff.removed}`;
			}
		}

		// Content area (will be populated async)
		const content = DOM.append(
			section,
			$("div.mobile-multi-diff-file-content"),
		);
		...
		const toggle = (e: UIEvent) => {
			...
			chevronEl.classList.remove(
				...ThemeIcon.asClassNameArray(
					state.collapsed ? Codicon.chevronDown : Codicon.chevronRight,
				),
			);
			chevronEl.classList.add(
				...ThemeIcon.asClassNameArray(
					state.collapsed ? Codicon.chevronRight : Codicon.chevronDown,
				),
			);
			this.scheduleVirtualLayout();
			if (!state.collapsed) {
		...
		store.add(
			DOM.addDisposableListener(header, TouchEventType.Tap, (e) => {
				e.preventDefault();
				toggle(e);
			}),
		);
	...
	private ensureFileSectionDomOrder(
		section: HTMLElement,
		previousSection: HTMLElement | undefined,
	): void {
		const referenceNode = previousSection
			? previousSection.nextSibling
			: this.virtualContent.firstChild;
		if (section !== referenceNode) {
			this.virtualContent.insertBefore(section, referenceNode);
		}
	}

	private applyVirtualLayout(
		section: HTMLElement,
		state: IMobileMultiDiffFileState,
		item: IMobileMultiDiffVirtualItemLayout,
	): void {
		section.style.top = `${item.renderTop}px`;
		section.style.height = `${item.renderHeight}px`;
		const bodyOffset = Math.max(
			0,
			item.innerOffset - VIRTUALIZER_METRICS.fileHeaderHeight,
		);
		state.bodyScrollTop = bodyOffset;
		state.bodyViewportHeight = Math.max(
			0,
			this.scrollWrapper.clientHeight - VIRTUALIZER_METRICS.fileHeaderHeight,
		);
		const content = state.content!;
		content.classList.toggle(
			"mobile-multi-diff-file-content-placeholder",
		...
		if (state.loadState === "loaded") {
			...
			const bodyHeight = Math.max(
				0,
				item.renderHeight - VIRTUALIZER_METRICS.fileHeaderHeight,
			);
			const placeholderHeight = Math.min(
				bodyHeight || VIRTUALIZER_METRICS.placeholderHeight,
				Math.max(
					VIRTUALIZER_METRICS.placeholderHeight,
					state.bodyViewportHeight,
				),
			);
	...
	private renderCurrentFileContent(state: IMobileMultiDiffFileState): void {
		...
		switch (state.loadState) {
			case "loaded":
				...
				break;
			case "empty":
				this.renderFileMessage(
					state,
					localize("multiDiffView.noChanges", "No changes in this file."),
				);
				break;
			case "error":
				this.renderFileMessage(
			...
			case "loading":
				this.renderFileMessage(
					state,
					localize("multiDiffView.loading", "Loading…"),
				);
				break;
		}
	}
	...
	private updateFileMessageHeight(
		state: IMobileMultiDiffFileState,
		...
		const bodyHeight =
			Number.parseFloat(state.content.style.height) ||
			VIRTUALIZER_METRICS.placeholderHeight;
		const visibleHeight =
			placeholderHeight ??
			Math.min(
	...
	private renderLoadedFileContent(state: IMobileMultiDiffFileState): void {
		...
		const { startIndex, endIndex } = this.computeVisibleBodyEntryRange(
			state.renderData.bodyEntries,
			visibleTop,
			visibleBottom,
		);
	...
	private toVirtualItem(
		state: IMobileMultiDiffFileState,
	): IMobileMultiDiffVirtualItem {
		return {
	...
	private computeCurrentVirtualLayout(): ReturnType<
		typeof computeMobileMultiDiffVirtualLayout
	> {
		return computeMobileMultiDiffVirtualLayout(
			this.fileStates.map((state) => this.toVirtualItem(state)),
			{
	...
	private computeVirtualTop(index: number): number {
		...
		for (let i = 0; i < end; i++) {
			top += computeMobileMultiDiffItemHeight(
				this.toVirtualItem(this.fileStates[i]),
				VIRTUALIZER_METRICS,
			);
		}
		return top;
	...
	private cancelScheduledLoadVisibleFiles(): void {
		if (this.loadVisibleAnimationFrame !== undefined) {
			DOM.getWindow(this.scrollWrapper).cancelAnimationFrame(
				this.loadVisibleAnimationFrame,
			);
			this.loadVisibleAnimationFrame = undefined;
		}
	}
	...
	private loadVisibleFiles(): void {
		...
		const loadingCount = this.fileStates.reduce(
			(count, state) => count + (state.loadState === "loading" ? 1 : 0),
			0,
		);
	...
	private prefetchNearFile(): void {
		...
		const prefetchLoadingCount = this.fileStates.reduce(
			(count, state) =>
				count +
				(state.loadState === "loading" && state.loadKind === "prefetch"
					? 1
					: 0),
			0,
		);
		if (
			loadingCount >= MAX_CONCURRENT_FILE_LOADS ||
			prefetchLoadingCount >= MAX_CONCURRENT_PREFETCH_LOADS
		) {
			return;
		}
		...
		const prefetchDistance = Math.max(
			...
			this.scrollWrapper.clientHeight * PREFETCH_VIEWPORT_MULTIPLIER,
		);
		let virtualTop = 0;
		let nextState: IMobileMultiDiffFileState | undefined;
		let nextDistance = Number.POSITIVE_INFINITY;
		...
		for (const state of this.fileStates) {
			const virtualHeight = computeMobileMultiDiffItemHeight(
				this.toVirtualItem(state),
				VIRTUALIZER_METRICS,
			);
			const virtualBottom = virtualTop + virtualHeight;
			if (
				!mountedIndexes.has(state.index) &&
				!state.collapsed &&
				state.loadState === "idle"
			) {
				const distance =
					virtualBottom < viewportTop
	...
	private ensureFileLoaded(
		state: IMobileMultiDiffFileState,
		loadKind: MobileMultiDiffFileLoadKind,
	): void {
		...
		void this.loadFileContent(state, generation, loadRequestId)
			.catch(() => {
	...
	private abandonOffscreenLoads(visibleIndexes: ReadonlySet<number>): void {
		for (const state of this.fileStates) {
			if (
				state.loadState !== "loading" ||
				state.loadKind === "prefetch" ||
				visibleIndexes.has(state.index)
			) {
				continue;
			}
	...
	private async loadFileContent(
		...
		const hunks = await (this.data.computeDiff?.(originalText, modifiedText) ??
			Promise.resolve(computeUnifiedDiff(originalText, modifiedText)));
		if (!this.isActiveFileLoad(state, generation, loadRequestId)) {
			return;
		}
		...
		const hasRealTokens =
			hasMultipleTokenClasses(origLineHtml) ||
			hasMultipleTokenClasses(modLineHtml);
		const origLines = hasRealTokens
			? origLineHtml
			: regexTokenizeLines(originalText, languageId);
		const modLines = hasRealTokens
			? modLineHtml
			: regexTokenizeLines(modifiedText, languageId);
		...
		state.rowCount = hunks.reduce(
			(count, hunk) => count + hunk.lines.length,
			0,
		);
		const { bodyEntries, bodyHeight, maxLineCharacterCount } =
			this.createBodyEntries(hunks);
		state.renderData = {
			bodyEntries,
			bodyHeight,
	...
	private async readTextContent(resource: URI | undefined): Promise<string> {
		...
		try {
			const model = await this.textFileService.read(resource, {
				acceptTextOnly: true,
			});
			return model.value;
		} catch {
	...
	private createBodyEntries(hunks: readonly IDiffHunk[]): {
		bodyEntries: MobileMultiDiffBodyEntry[];
		bodyHeight: number;
		...
		for (const hunk of hunks) {
			...
			for (const line of hunk.lines) {
				maxLineCharacterCount = Math.max(
					maxLineCharacterCount,
					line.text.length,
				);
				bodyEntries.push({
	...
	private computeVisibleBodyEntryRange(
		...
		const endIndex = this.findFirstBodyEntryStartingAtOrAfter(
			entries,
			visibleBottom,
		);
	...
	private findFirstBodyEntryEndingAfter(
		entries: readonly MobileMultiDiffBodyEntry[],
		offset: number,
	): number {
		let low = 0;
	...
	private findFirstBodyEntryStartingAtOrAfter(
		entries: readonly MobileMultiDiffBodyEntry[],
		offset: number,
	): number {
		let low = 0;
	...
	private ensureBodyInner(state: IMobileMultiDiffFileState): HTMLElement {
		...
		if (!state.content || !state.renderData) {
			throw new Error(
				"Cannot render a loaded mobile diff body without content and render data.",
			);
		}

		DOM.clearNode(state.content);
		this.resetBodyRenderState(state);
		const inner = DOM.append(
			state.content,
			$("div.mobile-multi-diff-file-content-inner"),
		);
		inner.style.height = `${state.renderData.bodyHeight}px`;
		inner.style.minWidth = `calc(${state.renderData.maxLineCharacterCount + 8}ch + 64px)`;
	...
	private insertBodyEntryRun(
		...
		for (let index = startIndex; index < endIndex; index++) {
			htmlParts.push(
				this.renderBodyEntryHtml(
					index,
					state.renderData.bodyEntries[index],
					state.renderData.origLines,
					state.renderData.modLines,
				),
			);
		}
		...
		state.bodyInner.insertBefore(
			template.content,
			this.findNextRenderedBodyRow(state, endIndex),
		);
	}

	private findNextRenderedBodyRow(
		state: IMobileMultiDiffFileState,
		startIndex: number,
	): HTMLElement | null {
		for (
			let index = startIndex;
			index < state.renderData!.bodyEntries.length;
			index++
		) {
