...
export class SessionView extends Disposable implements ISerializableView {
	...
	private static readonly ACTIVE_BACKGROUND = asCssVariable(
		activeSessionViewBackground,
	);
	private static readonly ACTIVE_FOREGROUND = asCssVariable(
		activeSessionViewForeground,
	);
	private static readonly INACTIVE_BACKGROUND = asCssVariable(
		inactiveSessionViewBackground,
	);
	private static readonly INACTIVE_FOREGROUND = asCssVariable(
		inactiveSessionViewForeground,
	);
	...
	private readonly _currentView = this._register(
		new MutableDisposable<AbstractChatView>(),
	);
	private _lastLayout:
		| {
	...
	constructor(
		...
		this._sessionIsCreatedKey = SessionIsCreatedContext.bindTo(
			scopedContextKeyService,
		);
		this._sessionIsStickyKey = SessionIsStickyContext.bindTo(
			scopedContextKeyService,
		);
		this._sessionIsMaximizedKey = SessionIsMaximizedContext.bindTo(
			scopedContextKeyService,
		);
		this._sessionSupportsMultipleChatsKey =
			SessionSupportsMultipleChatsContext.bindTo(scopedContextKeyService);
		...
		this._compositeBar = this._register(
			scopedInstantiationService.createInstance(ChatCompositeBar),
		);
		...
		this._floatingToolbar = this._register(
			scopedInstantiationService.createInstance(SessionViewFloatingToolbar),
		);
		...
		this._register(
			this._compositeBar.onDidChangeVisibility(() => this._layoutChildren()),
		);
	}
	...
	private _handleContextKeys(session: IActiveSession | undefined): IDisposable {
		...
		this._sessionSupportsMultipleChatsKey.set(
			session.capabilities.supportsMultipleChats,
		);
	...
	private _layoutChildren(): void {
		...
		this._currentView.value?.layout(
			width,
			height - barHeight,
			top + barHeight,
			left,
	...
	private _applyActiveSessionStyles(): void {
		const background = this._isActive
			? SessionView.ACTIVE_BACKGROUND
			: SessionView.INACTIVE_BACKGROUND;
		const foreground = this._isActive
			? SessionView.ACTIVE_FOREGROUND
			: SessionView.INACTIVE_FOREGROUND;
