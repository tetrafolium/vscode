...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	static readonly activeViewSettingsKey =
		"workbench.agentsession.auxiliarybar.activepanelid";
	static readonly pinnedViewsKey =
		"workbench.agentsession.auxiliarybar.pinnedPanels";
	static readonly placeholderViewContainersKey =
		"workbench.agentsession.auxiliarybar.placeholderPanels";
	...
	private static readonly RUN_SCRIPT_ACTION_ID =
		"workbench.action.agentSessions.runScript";
	...
	constructor(
		...
		this._register(
			this.layoutService.onDidChangePartVisibility((e) => {
				if (
					e.partId === Parts.AUXILIARYBAR_PART ||
					e.partId === Parts.EDITOR_PART
				) {
	...
	protected override actionViewItemProvider(
		action: IAction,
		...
		if (
			...
			if (!this._runScriptMenu.value) {
				...
				this._runScriptMenuListener.value =
					this._runScriptMenu.value.onDidChange(() =>
