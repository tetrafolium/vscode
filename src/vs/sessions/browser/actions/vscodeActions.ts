...
export class OpenInVSCodeAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (!activeSession) {
			await openerService.open(URI.from({ scheme, query: params.toString() }), {
				openExternal: true,
			});
			return;
		}
		...
		const rawFolderUri = workspace?.isVirtualWorkspace
			? undefined
		...
		const folderUri =
			rawFolderUri.scheme === AGENT_HOST_SCHEME
				? fromAgentHostUri(rawFolderUri)
				: rawFolderUri;
...
export class OpenInVSCodeWidgetContribution
	extends Disposable
