...
export const ISessionsSetUpService = createDecorator<ISessionsSetUpService>(
...
function shouldSkipSessionsWelcome(
	...
	return (
		...
		new URLSearchParams(globalThis.location.search).has("skip-sessions-welcome")
	);
}

class SessionsSetUpWidget extends Disposable {
	private readonly dialogRef = this._register(
	...
	private _start(): void {
		...
		const isFirstLaunch = !this.storageService.getBoolean(
		...
		try {
			...
			if (sessions.length > 0) {
				this.logService.info(
					"[sessions welcome] GitHub session found on web, skipping welcome",
				);
	...
	private _watchWebAuth(): void {
		this._register(
			this.authenticationService.onDidChangeSessions(async (e) => {
				...
				this.logService.info(
					"[sessions welcome] GitHub session removed on web, re-showing welcome",
				);
	...
	private async _showAIDisabledDialog(): Promise<void> {
		...
		this.logService.info(
			"[sessions welcome] AI features disabled, showing enable dialog",
		);

		const disposables = new DisposableStore();
		this.dialogRef.value = disposables;

		const welcomeVisibleKey = SessionsWelcomeVisibleContext.bindTo(
		...
		if (button === 0) {
			this.logService.info(
				"[sessions welcome] User chose to enable AI features",
			);
			await this.configurationService.updateValue(AIDisabledConfig, false);
		}
	}
	...
	private async _showWelcome(isFirstLaunch: boolean): Promise<void> {
		...
		if (isFirstLaunch) {
			...
			this.dialogRef.value.add(
				disposableTimeout(() => overlay.element.remove(), 200),
			);
				...
				createWorkbenchDialogOptions(
					{
						...
						disableCloseButton: true,
						renderFooter: (footer) =>
							footer.appendChild(this._createWelcomeFooter(disposables)),
					},
					this.keybindingService,
					this.layoutService,
					this.hostService,
	...
	private _createWelcomeFooter(disposables: DisposableStore): HTMLElement {
		...
		const settingsUrl = this.defaultAccountService.resolveGitHubUrl(
			...
			$(
				...
				disposables.add(
					this.markdownRendererService.render(
						new MarkdownString(footer, { isTrusted: true }),
					),
				).element,
			),
		);
...
export class SessionsSetUpService
	extends Disposable
	implements ISessionsSetUpService
{
	...
	private async initialize(): Promise<void> {
		...
		try {
			...
			await this.userDataProfileStorageService.withProfileScopedStorageService(
				defaultProfile,
				async (storageService) => {
					...
					try {
						if (defaultContext.state.completed) {
							this.logService.info(
								"[sessions welcome] Setup already completed in default profile, marking done locally",
							);
							this.markDone();
						}
					} finally {
			...
			this.logService.error(
				"[sessions welcome] Failed to read setup state from default profile:",
				error,
			);
		}
	}
