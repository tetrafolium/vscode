...
export type AccountTitleBarStateKind =
	| "default"
	| "accent"
	| "warning"
	| "prominent";
...
export function getAccountProfileImageUrl(
...
export function getAccountTitleBarBadgeKey(
	state: IAccountTitleBarState,
): string | undefined {
	if (!state.dotBadge) {
		return undefined;
	}
...
export function getAccountTitleBarState(
	context: IAccountTitleBarStateContext,
): IAccountTitleBarState {
...
function getCopilotPresentation(
	...
	if (sentiment.disabled || sentiment.untrusted) {
		return {
			...
			ariaLabel: sentiment.untrusted
				? localize(
						"copilotUnavailableUntrustedAria",
						"GitHub Copilot is unavailable in untrusted workspaces",
					)
				: localize(
	...
	if (
		...
		return {
			...
			dotBadge: "error",
			ariaLabel: getQuotaReachedAriaLabel(
				chatQuotaExceeded,
	...
	if (
		...
		return {
			...
			ariaLabel: localize(
				"copilotTokensRemainingAria",
				"{0}% GitHub Copilot tokens remaining",
				remainingPercent,
			),
		};
	}
...
function getLowestPositivePercent(
	...quotas: Array<IQuotaSnapshot | undefined>
): number | undefined {
...
function getQuotaReachedAriaLabel(
