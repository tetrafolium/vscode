...
suite("sessionFileChangesEqual", () => {
	...
	test("returns false when lengths differ", () => {
		assert.strictEqual(
			sessionFileChangesEqual([v1(fileA)], [v1(fileA), v1(fileB)]),
			false,
		);
	});
	...
	test("returns false when insertions differ", () => {
		assert.strictEqual(
			sessionFileChangesEqual([v1(fileA, 1, 1)], [v1(fileA, 2, 1)]),
			false,
		);
	});
	...
	test("returns false when deletions differ", () => {
		assert.strictEqual(
			sessionFileChangesEqual([v1(fileA, 1, 1)], [v1(fileA, 1, 2)]),
			false,
		);
	});
	...
	test("returns false when one entry is v1 and the other is v2", () => {
		assert.strictEqual(
			sessionFileChangesEqual([v1(fileA)], [v2(fileA)]),
			false,
		);
	});
	...
	test("returns false when v1 modifiedUri differs", () => {
		assert.strictEqual(
			sessionFileChangesEqual([v1(fileA)], [v1(fileB)]),
			false,
		);
	});
	...
	test("returns false when v2 uri differs", () => {
		assert.strictEqual(
			sessionFileChangesEqual([v2(fileA)], [v2(fileB)]),
			false,
		);
	});
