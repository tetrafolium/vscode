...
function stubSession(
	overrides: Partial<ISession> & Pick<ISession, "sessionId" | "providerId">,
): ISession {
	return {
...
class TestProgressService extends mock<IProgressService>() {
	override async withProgress<R>(
		_options: Parameters<IProgressService["withProgress"]>[0],
...
class TestSessionsProvidersService extends mock<ISessionsProvidersService>() {
	...
	override getProvider<T extends ISessionsProvider>(
		...
		return this._providers.find((provider) => provider.id === providerId) as
			| T
			| undefined;
	}
}
...
class TestSessionsProvider extends mock<ISessionsProvider>() {
	...
	override readonly sessionTypes: readonly ISessionType[] = [
		{ id: "test", label: "Test", icon: Codicon.vm },
	];
	...
	override resolveWorkspace(): ISessionWorkspace | undefined {
		return undefined;
	}
...
function createSessionsManagementService(
	...
	chatWidgetService: TestChatWidgetService;
	agentSessionsService: TestAgentSessionsService;
} {
	...
	instantiationService.stub(
		...
		new TestSessionsProvidersService([new TestSessionsProvider(session)]),
	);
	instantiationService.stub(IUriIdentityService, {
		extUri: extUriBiasedIgnorePathCase,
	});
	...
	const service = disposables.add(
		instantiationService.createInstance(SessionsManagementService),
	);
...
suite("SessionsManagementService", () => {
	...
	test("openSession waits for a loading session before opening chat content", async () => {
		...
		const { service, agentSessionsService } = createSessionsManagementService(
			session,
			disposables,
		);
		...
		assert.deepStrictEqual(
			{ observed: agentSessionsService.observed.map((uri) => uri.toString()) },
			{ observed: [] },
		);
		...
		assert.deepStrictEqual(
			{ observed: agentSessionsService.observed.map((uri) => uri.toString()) },
			{ observed: [session.resource.toString()] },
		);
	});
	...
	test("does not change active session when added session is not displayed in any widget", async () => {
		...
		const onDidChangeSessions = disposables.add(
		...
		instantiationService.stub(
			ISessionsProvidersService,
			new TestSessionsProvidersService([provider]),
		);
		instantiationService.stub(IUriIdentityService, {
			extUri: extUriBiasedIgnorePathCase,
		});
		...
		const otherSession = stubSession({
			sessionId: "other",
			providerId: "test",
		});
	...
	test("restoreLastActiveSession waits for session to appear via onDidChangeSessions", async () => {
		const targetSession = stubSession({
			sessionId: "target",
			providerId: "test",
		});
		const onDidChangeSessions = disposables.add(
		...
		storage.store(
			...
			JSON.stringify([
				{ sessionResource: targetSession.resource.toString(), isActive: true },
			]),
		...
		await Promise.resolve();
		assert.deepStrictEqual(
			agentSessionsService.observed.map((uri) => uri.toString()),
			[],
		);
		...
		assert.deepStrictEqual(
			agentSessionsService.observed.map((uri) => uri.toString()),
			[targetSession.resource.toString()],
		);
	});
});
