...
export class SessionsNavigation extends Disposable {
	...
	private readonly _canGoBack: IObservable<boolean> = derived(
		this,
		(reader) => {
	...
	private readonly _canGoForward: IObservable<boolean> = derived(
		this,
		(reader) => {
			...
			return (
				this._currentIndex.read(reader) < this._historySize.read(reader) - 1
			);
		},
	);
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				...
				const chatResource =
					activeChat && chatStatus !== SessionStatus.Untitled
						? activeChat.resource
						: undefined;
	...
	private _pushEntry(entry: INavigationEntry): void {
		...
		const existingIdx = this._history.findIndex((e) =>
			this._isSameEntry(e, entry),
		);
		if (existingIdx >= 0) {
		...
		try {
			...
			if (session) {
				if (entry.chatResource) {
					const chatExists = session.chats
						.get()
						.some(
