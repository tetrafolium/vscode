...
} from "../../../sessions/browser/sessionsProvidersService.js";
...
class StubRemoteProvider {
	...
	constructor(
		address: string,
		label: string,
		status: RemoteAgentHostConnectionStatus = RemoteAgentHostConnectionStatus.connected,
	) {
		this.id = `agenthost-${address}`;
		this.label = label;
		this.remoteAddress = address;
		this._status = observableValue<RemoteAgentHostConnectionStatus>(
			"status",
			status,
		);
...
suite("AgentHostFilterService", () => {
	...
	function createService(
		providers: StubSessionsProvidersService,
		storage = store.add(new InMemoryStorageService()),
	) {
		const instantiationService = store.add(new TestInstantiationService());
		instantiationService.stub(
			ISessionsProvidersService,
			providers as unknown as ISessionsProvidersService,
		);
		instantiationService.stub(
			IRemoteAgentHostService,
			new StubRemoteAgentHostService() as unknown as IRemoteAgentHostService,
		);
	...
	test("defaults based on platform when none persisted", () => {
		...
		assert.strictEqual(
			...
			isWeb ? pid("localhost:4321") : undefined,
		);
	});
	...
	test("surfaces registered remote providers with their connection status", () => {
		...
		const hosts = [...service.hosts].map((h) => ({
			label: h.label,
			status: h.status,
		...
		assert.deepStrictEqual(hosts, [
			{
				...
				providerId: pid("localhost:4321"),
			},
			{
				label: "Host B",
				status: AgentHostFilterConnectionStatus.Disconnected,
				providerId: pid("localhost:9999"),
			},
		]);
	});
	...
	test("updates when a provider status changes", () => {
		...
		store.add(
			providers.registerProvider(hostA as unknown as ISessionsProvider),
		);
		const service = createService(providers);
		...
		assert.strictEqual(
			service.hosts[0].status,
			AgentHostFilterConnectionStatus.Disconnected,
		);
	...
	test("fallback selection depends on platform when selected host disappears", () => {
		...
		const hostBReg = providers.registerProvider(
			hostB as unknown as ISessionsProvider,
		);
		const service = createService(providers);
