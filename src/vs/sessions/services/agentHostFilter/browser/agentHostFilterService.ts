...
} from "../common/agentHostFilter.js";

const STORAGE_KEY = "sessions.agentHostFilter.selectedProviderId";

function mapStatus(
	s: RemoteAgentHostConnectionStatus,
): AgentHostFilterConnectionStatus {
	switch (s.kind) {
		case "connected":
 ...
 */
function isRemoteAgentHostProvider(
	provider: unknown,
): provider is IAgentHostSessionsProvider & { readonly remoteAddress: string } {
	if (!provider || typeof provider !== "object" || !("id" in provider)) {
		return false;
	}
	const p = provider as IAgentHostSessionsProvider;
	return (
		isAgentHostProvider(p) &&
		p.connectionStatus !== undefined &&
		typeof p.remoteAddress === "string"
	);
}

export class AgentHostFilterService
	extends Disposable
	implements IAgentHostFilterService
{
	...
	private readonly _onDidChangeDiscovering = this._register(
		new Emitter<void>(),
	);
	readonly onDidChangeDiscovering: Event<void> =
		this._onDidChangeDiscovering.event;
	...
	constructor(
		...
		this._selectedProviderId = this._storageService.get(
			STORAGE_KEY,
		...
		this._register(
			this._sessionsProvidersService.onDidChangeProviders(() =>
				this._rewatchProviders(),
			),
		);
	}
	...
	reconnect(providerId: string): void {
		...
		if (provider && isAgentHostProvider(provider) && provider.connect) {
			provider.connect().catch(() => {
				/* errors are surfaced by the provider */
			});
			return;
		}
	...
	private _validate(providerId: string | undefined): string | undefined {
		if (
			...
			this._hosts.some((h) => h.providerId === providerId)
		) {
			return providerId;
		}
	...
	private _rewatchProviders(): void {
		...
		const providers = this._sessionsProvidersService
			.getProviders()
			.filter(isRemoteAgentHostProvider);

		this._providerWatchers.add(
			autorun((reader) => {
				const hosts: IAgentHostFilterEntry[] = providers
					.map((provider) => ({
	...
	private _applyHosts(hosts: readonly IAgentHostFilterEntry[]): void {
		const changed =
			...
			hosts.some(
				(h, i) =>
					h.providerId !== this._hosts[i].providerId ||
		...
		const validated = isWeb
			? this._validate(this._selectedProviderId)
			: undefined;
