...
suite("Sessions ConfigurationService", () => {
		...
		configurationRegistry.registerConfiguration({
			...
			properties: {
				...
				"sessionsConfigurationService.agentsWindowObjectDefault": {
					...
					scope: ConfigurationScope.RESOURCE,
					agentsWindow: {
						default: { "*.md": "vscode.markdown.preview.editor" },
					},
				},
			},
		});
	...
	setup(async () => {
		...
		workspaceService = disposables.add(
			new SessionsWorkspaceContextService(
	...
	test("defaults", () => {
		...
		assert.strictEqual(
			testObject.getValue("sessionsConfigurationService.machineSetting"),
			"defaultValue",
		);
		assert.strictEqual(
			testObject.getValue("sessionsConfigurationService.applicationSetting"),
			"defaultValue",
		);
	});
	...
	test("user settings override defaults", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				...
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.testSetting": "userValue" }',
				),
			);
	...
	test("workspace settings from workspace configuration file override defaults", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				...
				VSBuffer.fromString(
					JSON.stringify({
						folders: [],
						settings: {
							"sessionsConfigurationService.testSetting": "workspaceValue",
						},
					}),
				),
			);
	...
	test("inspect shows workspace value from workspace configuration file", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				workspaceConfigResource,
				VSBuffer.fromString(
			...
			await testObject.reloadConfiguration();
			const inspection = testObject.inspect(
				"sessionsConfigurationService.testSetting",
			);
	...
	test("write setting to workspace target persists to workspace configuration file", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"writtenWorkspaceValue",
				ConfigurationTarget.WORKSPACE,
			);
			const content = (
				await fileService.readFile(workspaceConfigResource)
			).value.toString();
			const parsed = JSON.parse(content);
			assert.strictEqual(
				parsed.settings["sessionsConfigurationService.testSetting"],
				"writtenWorkspaceValue",
			);
			assert.strictEqual(
				testObject.getValue("sessionsConfigurationService.testSetting"),
	...
	test("write setting to workspace target does not affect user settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"workspaceOnly",
	...
	test("agentsWindow.readOnly settings are excluded from workspace configuration", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				workspaceConfigResource,
				VSBuffer.fromString(
	...
	test("workspace folder settings override workspace settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				...
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.testSetting": "folderValue" }',
				),
			);
	...
	test("folder settings are read when folders are added", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("folder settings are removed when folders are removed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("configuration change event is fired when folders with settings are removed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.ok(
				event.affectsConfiguration("sessionsConfigurationService.testSetting"),
			);
			assert.strictEqual(
				testObject.getValue("sessionsConfigurationService.testSetting", {
					resource: folder,
	...
	test("configuration change event is fired on user settings change", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const promise = Event.toPromise(testObject.onDidChangeConfiguration);
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
	...
	test("inspect returns correct values per layer", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("application settings are not read from workspace folder", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				joinPath(folder, ".vscode", "settings.json"),
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.applicationSetting": "folderValue" }',
				),
			);
			await workspaceService.addFolders([{ uri: folder }]);
			assert.strictEqual(
	...
	test("machine settings are not read from workspace folder", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				joinPath(folder, ".vscode", "settings.json"),
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.machineSetting": "folderValue" }',
				),
			);
			await workspaceService.addFolders([{ uri: folder }]);
			assert.strictEqual(
	...
	test("folder settings change fires configuration change event", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				joinPath(folder, ".vscode", "settings.json"),
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.testSetting": "initialValue" }',
				),
			);
			await workspaceService.addFolders([{ uri: folder }]);
			assert.strictEqual(
			...
			await fileService.writeFile(
				joinPath(folder, ".vscode", "settings.json"),
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.testSetting": "updatedValue" }',
				),
			);
			const event = await promise;
			assert.ok(
	...
	test("updateValue writes to user settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"writtenValue",
	...
	test("updateValue persists to settings file", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"persistedValue",
	...
	test("updateValue fires change event", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const promise = Event.toPromise(testObject.onDidChangeConfiguration);
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
	...
	test("updateValue removes setting when value equals default", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"nonDefault",
	...
	test("updateValue can update multiple settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"value1",
	...
	test("updateValue with language override", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"langValue",
	...
	test("updateValue is reflected in inspect", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"inspectedValue",
	...
	test("updateValue without target writes to workspace when value exists in workspace", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				workspaceConfigResource,
				VSBuffer.fromString(
	...
	test("updateValue without target writes to user when value exists in user", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
	...
	test("updateValue without target writes to user when no value exists anywhere", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"sessionsConfigurationService.testSetting",
				"newValue",
	...
	test("updateValue without target removes from all defined targets when setting to default", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				workspaceConfigResource,
				VSBuffer.fromString(
			...
			assert.ok(
				!userContent.includes("sessionsConfigurationService.testSetting"),
			);
		}));
	...
	test("read setting from workspace folder", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("write setting to workspace folder", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("write setting to workspace folder persists to folder settings file", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const content = (
				await fileService.readFile(joinPath(folder, ".vscode", "settings.json"))
			).value.toString();
			assert.ok(content.includes('"sessionsConfigurationService.testSetting"'));
			assert.ok(content.includes("persistedFolderValue"));
		}));

	test("write setting to workspace folder does not affect user settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("workspace folder setting overrides user setting for resource", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("inspect shows workspace folder value after write", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("removing folder clears its written settings", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("agentsWindow.default overrides the default value", () => {
		assert.strictEqual(
			testObject.getValue("sessionsConfigurationService.agentsWindowDefault"),
			"agentsDefault",
		);
	});
	...
	test("agentsWindow.readOnly setting uses overridden default", () => {
		assert.strictEqual(
			testObject.getValue("sessionsConfigurationService.agentsWindowReadOnly"),
			"readOnlyDefault",
		);
	});

	test("agentsWindow.readOnly setting rejects writes", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
	...
	test("agentsWindow.readOnly setting ignores user settings file values", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.agentsWindowReadOnly": "userValue" }',
				),
			);
			await testObject.reloadConfiguration();
			assert.strictEqual(
	...
	test("user settings override agentsWindow.default when not readOnly", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString(
					'{ "sessionsConfigurationService.agentsWindowDefault": "userValue" }',
				),
			);
			await testObject.reloadConfiguration();
			assert.strictEqual(
	...
	test("agentsWindow.readOnly setting added dynamically is picked up", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			configurationRegistry.registerConfiguration({
			...
			assert.strictEqual(
				testObject.getValue("sessionsConfigurationService.dynamicReadOnly"),
				"dynamicDefault",
			);
			await assert.rejects(
				() =>
