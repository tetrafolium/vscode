...
} from "../../../../workbench/services/configuration/browser/configuration.js";
import {
	APPLICATION_SCOPES,
	...
	RestrictedSettings,
} from "../../../../workbench/services/configuration/common/configuration.js";
...
class SessionsDefaultConfiguration extends DefaultConfiguration {
	protected override getDefaultValue(
		_key: string,
		propertySchema: IRegisteredConfigurationPropertySchema,
	): unknown {
	...
	implements IWorkbenchConfigurationService
{
	...
	private readonly cachedFolderConfigs = this._register(
		new DisposableMap<URI, FolderConfiguration>(new ResourceMap()),
	);
	...
	constructor(
		...
		this.defaultConfiguration = this._register(
			new SessionsDefaultConfiguration(logService),
		);
		...
		this.userConfiguration = this._register(
			new UserConfiguration(
				...
				userDataProfileService.currentProfile.mcpResource,
				{ exclude: [...this.agentsWindowReadOnlyKeys] },
				fileService,
				uriIdentityService,
				logService,
			),
		...
		this._register(
			this.userConfiguration.onDidChangeConfiguration((userConfiguration) =>
				this.onUserConfigurationChanged(userConfiguration),
			),
		);
		this._register(
		...
		this._register(
			this.workspaceService.onWillChangeWorkspaceFolders((e) =>
				e.join(this.loadFolderConfigurations(e.changes.added)),
			),
		);
		this._register(
	...
	async updateValue(
		...
		const overrides: IConfigurationUpdateOverrides | undefined =
			isConfigurationUpdateOverrides(arg3)
				...
				: isConfigurationOverrides(arg3)
					...
					: undefined;
		let target: ConfigurationTarget | undefined = (overrides ? arg4 : arg3) as
			| ConfigurationTarget
			| undefined;
		...
		if (this.agentsWindowReadOnlyKeys.has(key)) {
			throw new Error(
				`Unable to write ${key} because it is read-only in the Agents window.`,
			);
		}
		...
		if (
			...
			const existingOverrides =
				this._configuration.localUserConfiguration.overrides.find((override) =>
		...
		await Promises.settled(
			targets.map((t) =>
				this.writeConfigurationValue(key, value, t, overrides),
			),
		);
	}
	...
	private async writeConfigurationValue(
		...
		let path = overrides?.overrideIdentifiers?.length
			? [keyFromOverrideIdentifiers(overrides.overrideIdentifiers), key]
			: [key];

		const settingsResource = this.getSettingsResource(
			target,
	...
	private isWorkspaceConfigurationResource(resource: URI): boolean {
		...
		return !!(
			...
			this.uriIdentityService.extUri.isEqual(workspace.configuration, resource)
		);
	}
	...
	async reloadConfiguration(
		_target?: ConfigurationTarget | IWorkspaceFolder,
	): Promise<void> {
		const userModel = await this.userConfiguration.initialize();
		const previousData = this._configuration.toData();
		const change =
			this._configuration.compareAndUpdateLocalUserConfiguration(userModel);
	...
	isSettingAppliedForAllProfiles(key: string): boolean {
		const scope =
			this.configurationRegistry.getConfigurationProperties()[key]?.scope;
		...
		return (
			Array.isArray(allProfilesSettings) && allProfilesSettings.includes(key)
		);
	}
	...
	private onDefaultConfigurationChanged(
		...
		this._configuration.updateLocalUserConfiguration(
			this.userConfiguration.reparse({
				exclude: [...this.agentsWindowReadOnlyKeys],
			}),
		);
	...
	private async loadWorkspaceConfiguration(): Promise<IConfigurationChange> {
		...
		return this._configuration.compareAndUpdateWorkspaceConfiguration(
			this.workspaceConfiguration.getConfiguration(),
		);
	}
	...
	private onWorkspaceFoldersChanged(e: IWorkspaceFoldersChangeEvent): void {
		...
		for (const folder of e.removed) {
			const change = this._configuration.compareAndDeleteFolderConfiguration(
				folder.uri,
			);
	...
	private onWorkspaceFolderConfigurationChanged(
		...
		if (folderConfiguration) {
			folderConfiguration.loadConfiguration().then((configurationModel) => {
				const previousData = this._configuration.toData();
				const change = this._configuration.compareAndUpdateFolderConfiguration(
					folder.uri,
					configurationModel,
				);
				this.triggerConfigurationChange(
	...
	private async loadFolderConfigurations(
...
class ConfigurationEditing {
	...
	write(settingsResource: URI, path: JSONPath, value: unknown): Promise<void> {
		return this.queue.queue(() =>
			this.doWriteConfiguration(settingsResource, path, value),
		);
	}
		...
		if (!this._formattingOptions) {
			let eol =
				OS === OperatingSystem.Linux || OS === OperatingSystem.Macintosh
					? "\n"
					: "\r\n";
			const configuredEol = this.configurationService.getValue<string>(
				"files.eol",
