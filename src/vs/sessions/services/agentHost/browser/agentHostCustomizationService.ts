	...
	implements IAgentHostCustomizationService
{
	...
	private readonly _providerListeners = this._register(
		new DisposableMap<ISessionsProvider>(),
	);
	...
	private _getSession(sessionResource: URI): ISession | undefined {
		...
		if (
			activeSession &&
			activeSession.resource.toString() === sessionResource.toString()
		) {
	...
	getCustomAgents(sessionResource: URI): readonly AgentCustomization[] {
		...
		if (session) {
			...
			if (provider) {
				...
				return agents.length === 0 && activeMode
					? [this._agentFromMode(activeMode)]
					: agents;
			}
		}
		return [];
