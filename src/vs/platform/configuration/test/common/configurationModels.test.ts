...
suite("ConfigurationModelParser", () => {
	...
	test("parse configuration model with single override identifier", () => {
		...
		assert.deepStrictEqual(
			...
			JSON.stringify([{ identifiers: ["x"], keys: ["a"], contents: { a: 1 } }]),
		);
	});
	...
	test("parse configuration model with multiple duplicate override identifiers", () => {
		...
		assert.deepStrictEqual(
			JSON.stringify(testObject.configurationModel.overrides),
			JSON.stringify([
				{ identifiers: ["x", "y", "z"], keys: ["a"], contents: { a: 1 } },
			]),
		);
	});
	...
	test("parse configuration model with scopes filter", () => {
		...
		testObject.parse(
			JSON.stringify({ "ConfigurationModelParserTest.windowSetting": "1" }),
...
suite("ConfigurationModelParser - Excluded Properties", () => {
	...
	test("should handle excluded restricted properties correctly", () => {
		...
		assert.strictEqual(
			testObject.configurationModel.getValue("regularProperty"),
			"regularValue",
		);
		assert.strictEqual(
			testObject.configurationModel.getValue("restrictedProperty"),
			undefined,
		);
		assert.strictEqual(
			testObject.configurationModel.getValue("excludedProperty"),
			"excludedValue",
		);
		assert.strictEqual(
			testObject.configurationModel.getValue("excludedNonRestrictedProperty"),
			"excludedNonRestrictedValue",
		);
		assert.ok(
			testObject.restrictedConfigurations.includes("restrictedProperty"),
		);
		assert.ok(
			!testObject.restrictedConfigurations.includes("excludedProperty"),
		);
	});
	...
	test("should handle override properties with excluded configurations", () => {
		...
		assert.strictEqual(
			overrideConfig.getValue("regularProperty"),
			"overrideRegular",
		);
		assert.strictEqual(
			overrideConfig.getValue("restrictedProperty"),
			undefined,
		);
		assert.strictEqual(
			overrideConfig.getValue("excludedProperty"),
			"overrideExcluded",
		);
	});
	...
	test("should handle scope filtering with excluded properties", () => {
		...
		testObject.parse(JSON.stringify(testData), {
			scopes: [ConfigurationScope.WINDOW],
		});

		assert.strictEqual(
			testObject.configurationModel.getValue("windowProperty"),
			"windowValue",
		);
		assert.strictEqual(
			testObject.configurationModel.getValue("applicationProperty"),
			undefined,
		);
		assert.strictEqual(
	...
	test("should report hasExcludedProperties correctly when excluded properties are filtered", () => {
		...
		assert.notStrictEqual(
			model.raw,
			undefined,
			"Raw should be set when properties are excluded",
		);
	});
	...
	test("skipUnregistered should exclude unregistered properties", () => {
		...
		assert.strictEqual(
			testObject.configurationModel.getValue("unregisteredProperty"),
			undefined,
		);
	});
	...
	test("excluded properties are found during property schema lookup", () => {
		...
		assert.ok(
			...
			"Excluded property should be in excluded properties map",
		);
		assert.ok(
			excludedProperties["excludedNonRestrictedProperty"],
			"Excluded non-restricted property should be in excluded properties map",
		);

		const regularProperties = registry.getConfigurationProperties();
		assert.strictEqual(
			regularProperties["excludedProperty"],
			undefined,
			"Excluded property should not be in regular properties map",
		);
		assert.strictEqual(
			regularProperties["excludedNonRestrictedProperty"],
			undefined,
			"Excluded non-restricted property should not be in regular properties map",
		);

		assert.ok(
			regularProperties["regularProperty"],
			"Regular property should be in regular properties map",
		);
		assert.ok(
			regularProperties["restrictedProperty"],
			"Restricted property should be in regular properties map",
		);
	});
...
suite("ConfigurationModel", () => {
	...
	test("get overriding configuration if the value of overriding identifier is not object", () => {
		const testObject = new ConfigurationModel(
			...
			[
				{
					identifiers: ["c"],
					contents: "abc" as unknown as Record<string, unknown>,
					keys: [],
				},
			],
			[],
	...
	test("inspect when raw is same", () => {
		const testObject = new ConfigurationModel(
			{ a: 1, c: 1 },
			["a", "c"],
			[{ identifiers: ["x", "y"], contents: { a: 2, b: 1 }, keys: ["a"] }],
			undefined,
			new NullLogService(),
		);
...
suite("Configuration", () => {
	...
	function parseConfigurationModel(
...
suite("ConfigurationChangeEvent", () => {
	...
	test("changeEvent affecting keys with new configuration", () => {
		...
		assert.deepStrictEqual(
			...
			["window.zoomLevel", "workbench.editor.enablePreview", "files.autoSave"],
		);
	...
	test("changeEvent affecting overrides when configuration changed", () => {
		...
		assert.ok(
			testObject.affectsConfiguration("editor", { overrideIdentifier: "css" }),
		);
		assert.ok(
			testObject.affectsConfiguration("editor", { overrideIdentifier: "scss" }),
		);
		assert.ok(
			testObject.affectsConfiguration("editor.fontSize", {
				overrideIdentifier: "markdown",
...
suite("Configuration.Parse", () => {
	...
	test("parsing configuration only with local user configuration and raw is same", () => {
		...
		assert.deepStrictEqual(
			actual.inspect("a", { overrideIdentifier: "x" }, undefined).userLocal,
			{
				value: 1,
				override: 2,
				merged: 2,
		...
		assert.deepStrictEqual(
			actual.inspect("b", { overrideIdentifier: "x" }, undefined).userLocal,
			{
				value: undefined,
				override: 1,
				merged: 1,
		...
		assert.deepStrictEqual(
			actual.inspect("d", {}, undefined).userLocal,
			undefined,
		);

		assert.deepStrictEqual(
			actual.inspect("a", {}, undefined).userRemote,
			undefined,
		);
		assert.deepStrictEqual(
			actual.inspect("a", { overrideIdentifier: "x" }, undefined).userRemote,
			undefined,
		);
		assert.deepStrictEqual(
			actual.inspect("b", { overrideIdentifier: "x" }, undefined).userRemote,
			undefined,
		);
		assert.deepStrictEqual(
			actual.inspect("d", {}, undefined).userRemote,
			undefined,
		);
		...
		assert.deepStrictEqual(
			actual.inspect("a", { overrideIdentifier: "x" }, undefined).user,
			{
				value: 1,
				override: 2,
				merged: 2,
		...
		assert.deepStrictEqual(
			actual.inspect("b", { overrideIdentifier: "x" }, undefined).user,
			{
				value: undefined,
				override: 1,
				merged: 1,
	...
	test("parsing configuration only with local user configuration and raw is not same", () => {
		...
		assert.deepStrictEqual(
			actual.inspect("e", {}, undefined).userLocal,
			undefined,
		);
		...
		assert.deepStrictEqual(
			actual.inspect("e", {}, undefined).userRemote,
			undefined,
		);
	...
	test("parsing configuration with local and remote user configuration and raw is same for both", () => {
		...
		assert.deepStrictEqual(
			actual.inspect("b", {}, undefined).userLocal,
			undefined,
		);
		assert.deepStrictEqual(
			actual.inspect("b", { overrideIdentifier: "y" }, undefined).userLocal,
			undefined,
		);
		assert.deepStrictEqual(
			actual.inspect("c", {}, undefined).userLocal,
			undefined,
		);
		...
		assert.deepStrictEqual(
			actual.inspect("b", { overrideIdentifier: "y" }, undefined).userRemote,
			{ value: 3, override: undefined, merged: 3, overrides: undefined },
		);
		assert.deepStrictEqual(
			actual.inspect("c", {}, undefined).userRemote,
			undefined,
		);
		...
		assert.deepStrictEqual(
			actual.inspect("b", { overrideIdentifier: "y" }, undefined).user,
			{ value: 3, override: undefined, merged: 3, overrides: undefined },
		);
	...
	test("parsing configuration with local and remote user configuration and raw is not same for remote user", () => {
		...
		assert.deepStrictEqual(
			actual.inspect("a", {}, undefined).userLocal,
			undefined,
		);
		assert.deepStrictEqual(
			actual.inspect("a", { overrideIdentifier: "x" }, undefined).userLocal,
	...
	test("parsing configuration with local and remote user configuration and raw is not same for both", () => {
		...
		assert.deepStrictEqual(
			actual.inspect("a", { overrideIdentifier: "x" }, undefined).userLocal,
			{ value: 4, override: undefined, merged: 4, overrides: undefined },
		);
		assert.deepStrictEqual(actual.inspect("b", {}, undefined).userLocal, {
			value: 3,
			override: undefined,
