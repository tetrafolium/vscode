...
suite("ConfigurationService.test.ts", () => {
	...
	test("trigger configuration change event when file does not exist", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const testObject = disposables.add(
				new ConfigurationService(
					settingsResource,
			...
			return new Promise<void>((c, e) => {
				...
				fileService
					.writeFile(settingsResource, VSBuffer.fromString('{ "foo": "bar" }'))
					.catch(e);
			});
		}));

	test("trigger configuration change event when file exists", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const testObject = disposables.add(
				new ConfigurationService(
					settingsResource,
	...
	test("lookup with null", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				settingsResource,
				VSBuffer.fromString('{ "lookup.service.testNullSetting": null }'),
			);
