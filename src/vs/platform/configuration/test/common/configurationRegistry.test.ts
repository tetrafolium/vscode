...
suite("ConfigurationRegistry", () => {
	...
	test("configuration defaults - string source", async () => {
		...
		const overrides1 = [
			{ overrides: { config: { a: 1, b: 2 } }, source: "source1" },
		];
		const overrides2 = [
			{ overrides: { config: { a: 2, c: 3 } }, source: "source2" },
		];
		...
		assert.deepStrictEqual(
			configurationRegistry.getConfigurationProperties()["config"]
				.defaultValueSource instanceof Map,
			true,
		);
	...
	test("configuration defaults - deregister with string source and extension source", async () => {
		...
		const overrides1 = [
			{ overrides: { config: { a: 1, b: 2 } }, source: "stringSource" },
		];
		const overrides2 = [
			{
				overrides: { config: { a: 2, c: 3 } },
				source: { id: "extSource", displayName: "Extension Source" },
			},
		];
	...
	suite("isConfigurationDefaultSourceEquals", () => {
		test("both undefined", () => {
			assert.strictEqual(
				isConfigurationDefaultSourceEquals(undefined, undefined),
				true,
			);
		});
		...
		test("one undefined", () => {
			assert.strictEqual(
				isConfigurationDefaultSourceEquals("source", undefined),
				false,
			);
			assert.strictEqual(
				isConfigurationDefaultSourceEquals(undefined, "source"),
				false,
			);
			assert.strictEqual(
				isConfigurationDefaultSourceEquals({ id: "ext" }, undefined),
				false,
			);
			assert.strictEqual(
				isConfigurationDefaultSourceEquals(undefined, { id: "ext" }),
				false,
			);
		});
		...
		test("same string source", () => {
			assert.strictEqual(
				isConfigurationDefaultSourceEquals("source", "source"),
				true,
			);
		});
		...
		test("different string sources", () => {
			assert.strictEqual(
				isConfigurationDefaultSourceEquals("source1", "source2"),
				false,
			);
		});
		...
		test("same extension source", () => {
			assert.strictEqual(
				isConfigurationDefaultSourceEquals({ id: "ext" }, { id: "ext" }),
				true,
			);
		});
		...
		test("different extension sources", () => {
			assert.strictEqual(
				isConfigurationDefaultSourceEquals({ id: "ext1" }, { id: "ext2" }),
				false,
			);
		});
		...
		test("string vs extension source", () => {
			assert.strictEqual(
				isConfigurationDefaultSourceEquals("ext", { id: "ext" }),
				false,
			);
			assert.strictEqual(
				isConfigurationDefaultSourceEquals({ id: "ext" }, "ext"),
				false,
			);
		});
