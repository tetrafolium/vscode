...
export function isConfigurationOverrides(
	obj: unknown,
): obj is IConfigurationOverrides {
...
export function isConfigurationUpdateOverrides(
	obj: unknown,
): obj is IConfigurationUpdateOverrides {
...
export function removeFromValueTree(
	valueTree: IStringDictionary<unknown>,
	key: string,
): void {
	const segments = key.split(".");
...
function doRemoveFromValueTree(
	valueTree: IStringDictionary<unknown> | unknown,
	segments: string[],
): void {
	if (!valueTree) {
		return;
...
export function merge(
	base: IStringDictionary<unknown>,
	add: IStringDictionary<unknown>,
	overwrite: boolean,
): void {
		...
		if (key !== "__proto__") {
			if (key in base) {
				if (types.isObject(base[key]) && types.isObject(add[key])) {
					merge(
						base[key] as IStringDictionary<unknown>,
						add[key] as IStringDictionary<unknown>,
						overwrite,
					);
				} else if (overwrite) {
					base[key] = add[key];
