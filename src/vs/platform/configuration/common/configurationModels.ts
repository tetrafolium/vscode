...
export class ConfigurationModel implements IConfigurationModel {
	...
	get rawConfiguration(): ConfigurationModel {
		if (!this._rawConfiguration) {
			if (this._raw) {
				const rawConfigurationModels = (
					Array.isArray(this._raw) ? this._raw : [this._raw]
				).map((raw) => {
				...
				this._rawConfiguration = rawConfigurationModels.reduce(
					(previous, current) =>
						current === previous ? current : previous.merge(current),
	...
	get raw():
		...
		if (
			...
			this._raw.every((raw) => raw instanceof ConfigurationModel)
		) {
			return undefined;
		}
		return this._raw as
	...
	getValue<V>(section: string | undefined): V | undefined {
		return section
			? getConfigurationValue<V>(this.contents, section)
			: (this.contents as V);
	}
	...
	inspect<V>(
		...
		return {
			...
			get overrides() {
				...
				for (const { contents, identifiers, keys } of that.rawConfiguration
					.overrides) {
	...
	merge(...others: ConfigurationModel[]): ConfigurationModel {
		...
		return new ConfigurationModel(
			...
			keys,
			overrides,
			!raws.length || raws.every((raw) => raw instanceof ConfigurationModel)
				? undefined
				: raws,
			this.logService,
		);
	...
	private mergeContents(
		source: IStringDictionary<unknown>,
		target: IStringDictionary<unknown>,
	): void {
		for (const key of Object.keys(target)) {
			if (key in source) {
				if (types.isObject(source[key]) && types.isObject(target[key])) {
					this.mergeContents(
						source[key] as IStringDictionary<unknown>,
						target[key] as IStringDictionary<unknown>,
					);
					continue;
				}
			}
	...
	private updateValue(key: string, value: unknown, add: boolean): void {
		...
		if (OVERRIDE_PROPERTY_REGEX.test(key)) {
			...
			const override = {
				...
				contents: toValuesTree(overrideContents, (message) =>
...
export class ConfigurationModelParser {
	...
	protected doParseRaw(
		...
		const excludedConfigurationProperties =
			registry.getExcludedConfigurationProperties();
...
class ConfigurationInspectValue<V> implements IConfigurationValue<V> {
	...
	private get userLocalInspectValue(): InspectValue<V> {
		if (!this._userLocalInspectValue) {
			this._userLocalInspectValue = this.localUserConfiguration.inspect<V>(
				this.key,
				this.overrides.overrideIdentifier,
			);
		}
...
export class Configuration {
	...
	getValue(
		...
		const consolidateConfigurationModel =
			this.getConsolidatedConfigurationModel(section, overrides, workspace);
