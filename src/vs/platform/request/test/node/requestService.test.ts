...
suite("Request Service", () => {
	...
	test("Request cancellation during retry backoff", async () => {
		...
		try {
			...
			assert.ok(
				...
				"Error should be CancellationError",
			);
			assert.ok(
				elapsed < 200,
				`Request should be cancelled quickly, but took ${elapsed}ms`,
			);
		}
	});
	...
	test("should NOT retry POST requests", async () => {
		...
		assert.strictEqual(
			attemptCount,
			1,
			"POST request should not have been retried",
		);
	});
	...
	test("should NOT retry DELETE requests", async () => {
		...
		assert.strictEqual(
			attemptCount,
			1,
			"DELETE request should not have been retried",
		);
	});
	...
	test("should NOT retry PUT requests", async () => {
		...
		assert.strictEqual(
			attemptCount,
			1,
			"PUT request should not have been retried",
		);
	});
	...
	test("should NOT retry PATCH requests", async () => {
		...
		assert.strictEqual(
			attemptCount,
			1,
			"PATCH request should not have been retried",
		);
	});
});
