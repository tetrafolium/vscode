...
suite("AbstractRequestService", () => {
	...
	test("onDidCompleteRequest fires with correct data", async () => {
		const service = store.add(
			new TestRequestService(() => Promise.resolve(makeResponse(200))),
		);
		...
		await service.request(
			{ url: "http://test", callSite: "test.callSite" },
	...
	test("onDidCompleteRequest reports status code from response", async () => {
		const service = store.add(
			new TestRequestService(() => Promise.resolve(makeResponse(404))),
		);
		...
		await service.request(
			{ url: "http://test", callSite: "test.notFound" },
			CancellationToken.None,
		);
	...
	test("onDidCompleteRequest fires for NO_FETCH_TELEMETRY", async () => {
		...
		await service.request(
			{ url: "http://test", callSite: NO_FETCH_TELEMETRY },
			CancellationToken.None,
		);
	...
	test("onDidCompleteRequest does not fire when request throws", async () => {
		const service = store.add(
			new TestRequestService(() => Promise.reject(new Error("network error"))),
		);
		...
		await assert.rejects(() =>
			service.request(
				{ url: "http://test", callSite: "test.error" },
				CancellationToken.None,
			),
		);
	...
	test("onDidCompleteRequest fires for each request", async () => {
		...
		await service.request(
			{ url: "http://test/1", callSite: "first" },
			CancellationToken.None,
		);
		await service.request(
			{ url: "http://test/2", callSite: "second" },
			CancellationToken.None,
		);
