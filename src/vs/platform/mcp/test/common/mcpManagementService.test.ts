...
} from "../../common/mcpManagementService.js";
import {
	IGalleryMcpServer,
...
import {
	...
	IMcpServerVariable,
} from "../../common/mcpPlatformTypes.js";
...
class TestMcpResourceManagementService extends AbstractMcpResourceManagementService {
	constructor(
		...
		mcpResourceScannerService: McpResourceScannerService,
	) {
		super(
			mcpResource,
	...
	override canInstall(
		_server: IGalleryMcpServer | IInstallableMcpServer,
	): true | IMarkdownString {
		throw new Error("Not supported");
	}

	protected override getLocalServerInfo(
		_name: string,
		_mcpServerConfig: IMcpServerConfiguration,
	) {
...
suite("McpManagementService - getMcpServerConfigurationFromManifest", () => {
	...
	suite("NPM Package Tests", () => {
		test("basic NPM package configuration", () => {
			...
			const result = service.getMcpServerConfigurationFromManifest(
				manifest,
				RegistryType.NODE,
			);

			assert.strictEqual(
				result.mcpServerConfiguration.config.type,
				McpServerType.LOCAL,
			);
		...
		test("NPM package with environment variables containing variables", () => {
			...
			assert.strictEqual(
				result.mcpServerConfiguration.inputs?.[0].type,
				McpServerVariableType.PROMPT,
			);
			assert.strictEqual(
				result.mcpServerConfiguration.inputs?.[0].description,
				"Your API token",
			);
			assert.strictEqual(
				result.mcpServerConfiguration.inputs?.[0].password,
				true,
			);
		});
		...
		test("environment variable with choices but empty value should create pick input (GitHub issue #266106)", () => {
			...
			assert.strictEqual(
				result.mcpServerConfiguration.inputs?.[0].default,
				"prefer",
			);
			assert.strictEqual(
				result.mcpServerConfiguration.inputs?.[0].type,
			...
			assert.deepStrictEqual(
				result.mcpServerConfiguration.inputs?.[0].options,
	...
	suite("Docker Package Tests", () => {
		test("basic Docker package configuration", () => {
			...
			if (result.mcpServerConfiguration.config.type === McpServerType.LOCAL) {
				assert.strictEqual(
					result.mcpServerConfiguration.config.command,
					"docker",
				);
				assert.deepStrictEqual(result.mcpServerConfiguration.config.args, [
					"run",
		...
		test("Docker package arguments without values should create input variables (GitHub issue #266106)", () => {
			...
			const hostInput = result.mcpServerConfiguration.inputs?.find(
				(i: IMcpServerVariable) => i.id === "host",
			);
			...
			const dbNameInput = result.mcpServerConfiguration.inputs?.find(
				(i: IMcpServerVariable) => i.id === "database_name",
			);
			assert.strictEqual(
				dbNameInput?.description,
	...
	suite("NuGet Package Tests", () => {
		test("basic NuGet package configuration", () => {
			...
			if (result.mcpServerConfiguration.config.type === McpServerType.LOCAL) {
				...
				assert.deepStrictEqual(result.mcpServerConfiguration.config.args, [
					"Knapcode.SampleMcpServer@0.5.0",
					"--yes",
				]);
				assert.deepStrictEqual(result.mcpServerConfiguration.config.env, {
	...
	suite("Remote Server Tests", () => {
		test("SSE remote server configuration", () => {
			...
			if (result.mcpServerConfiguration.config.type === McpServerType.REMOTE) {
				assert.strictEqual(
					result.mcpServerConfiguration.config.url,
					"http://mcp-fs.anonymous.modelcontextprotocol.io/sse",
				);
				assert.strictEqual(
					result.mcpServerConfiguration.config.headers,
					undefined,
				);
			}
		});
		...
		test("streamable HTTP remote server", () => {
			...
			if (result.mcpServerConfiguration.config.type === McpServerType.REMOTE) {
				assert.strictEqual(
					result.mcpServerConfiguration.config.url,
					"https://mcp.anonymous.modelcontextprotocol.io/http",
				);
			}
		});
		...
		test("remote headers without values should create input variables", () => {
			...
			const authInput = result.mcpServerConfiguration.inputs?.find(
				(i: IMcpServerVariable) => i.id === "Authorization",
			);
			assert.strictEqual(
			...
			const customInput = result.mcpServerConfiguration.inputs?.find(
				(i: IMcpServerVariable) => i.id === "X-Custom-Header",
			);
			...
			assert.deepStrictEqual(customInput?.options, [
				"option1",
				"option2",
				"option3",
			]);
	...
	suite("Variable Interpolation Tests", () => {
		test("multiple variables in single value", () => {
			...
			if (result.mcpServerConfiguration.config.type === McpServerType.LOCAL) {
				assert.deepStrictEqual(result.mcpServerConfiguration.config.env, {
					CONNECTION_STRING:
						"server=${input:host};port=${input:port};database=${input:db_name}",
				});
			}
			...
			const portInput = result.mcpServerConfiguration.inputs?.find(
				(i: IMcpServerVariable) => i.id === "port",
			);
			assert.strictEqual(portInput?.default, "5432");

			const dbNameInput = result.mcpServerConfiguration.inputs?.find(
				(i: IMcpServerVariable) => i.id === "db_name",
			);
	...
	suite("Edge Cases and Error Handling", () => {
		...
		test("named argument with no name should generate notice", () => {
			...
			const result = service.getMcpServerConfigurationFromManifest(
				manifest as IGalleryMcpServerConfiguration,
				RegistryType.NODE,
			);
	...
	suite("Variable Processing Order", () => {
		test("should use explicit variables instead of auto-generating when both are possible", () => {
			...
			if (result.mcpServerConfiguration.config.type === McpServerType.LOCAL) {
				assert.strictEqual(
					result.mcpServerConfiguration.config.env?.["API_KEY"],
...
suite("McpResourceManagementService", () => {
	...
	setup(async () => {
		...
		const scannerService = disposables.add(
			new McpResourceScannerService(fileService, uriIdentityService),
		);
		service = disposables.add(
			new TestMcpResourceManagementService(
				mcpResource,
				fileService,
				uriIdentityService,
				scannerService,
