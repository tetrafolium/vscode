...
suite("McpGatewaySession", () => {
			...
			invoker: {
				...
				callTool: async (_name: string, args: Record<string, unknown>) => ({
					content: [
						{
							type: "text" as const,
							text: `Hello, ${typeof args.name === "string" ? args.name : "World"}!`,
						},
					],
				}),
				...
				listResourceTemplates: async () => [
					{ uriTemplate: "file:///test/{name}", name: "Test Template" },
				],
			},
		};
	}
	...
	test("negotiates to each supported protocol version", async () => {
		...
		for (const version of supportedVersions) {
			const { invoker, onDidChangeTools, onDidChangeResources } =
				createInvoker();
			const session = new McpGatewaySession(
				`session-ver-${version}`,
				new NullLogService(),
	...
	test("initialize response includes server info and capabilities", async () => {
		...
		const result = (responses[0] as IJsonRpcSuccessResponse)
			.result as MCP.InitializeResult;
	...
	test("serves tools/list and tools/call after initialized notification", async () => {
		...
		const notificationResponses = await session.handleIncoming({
			jsonrpc: "2.0",
			method: "notifications/initialized",
		});
		assert.strictEqual(notificationResponses.length, 0);

		const listResponses = await session.handleIncoming({
			jsonrpc: "2.0",
			id: 3,
			method: "tools/list",
		});
		...
		const text = (callResponse.result as { content: Array<{ text: string }> })
			.content[0].text;
	...
	test("broadcasts notifications to attached SSE clients", async () => {
		...
		assert.ok(
			response.writes.some((chunk) => chunk.includes("event: message")),
		);
		assert.ok(
			response.writes.some((chunk) =>
				chunk.includes("notifications/tools/list_changed"),
			),
		);
		assert.ok(
			response.writes.some((chunk) =>
				chunk.includes("notifications/resources/list_changed"),
			),
		);
		session.dispose();
		onDidChangeTools.dispose();
	...
	test("emits list changed on tool invoker changes", async () => {
		...
		assert.ok(
			response.writes
				.slice(writesBefore)
				.some((chunk) => chunk.includes("notifications/tools/list_changed")),
		);
		session.dispose();
		onDidChangeTools.dispose();
		onDidChangeResources.dispose();
	...
	test("serves resources/list with raw URIs", async () => {
		...
		const response = responses[0] as IJsonRpcSuccessResponse;
		const resources = (
			response.result as { resources: Array<{ uri: string; name: string }> }
		).resources;
	...
	test("serves resources/read with raw URIs", async () => {
		...
		const response = responses[0] as IJsonRpcSuccessResponse;
		const contents = (
			response.result as { contents: Array<{ uri: string; text: string }> }
		).contents;
