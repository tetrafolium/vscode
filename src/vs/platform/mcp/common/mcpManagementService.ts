...
import {
	...
	McpResourceTarget,
} from "./mcpResourceScannerService.js";
...
export abstract class AbstractCommonMcpManagementService
	extends Disposable
	...
	getMcpServerConfigurationFromManifest(
		...
		if (packageType === RegistryType.REMOTE && manifest.remotes?.length) {
			...
			const { inputs, variables } = this.processKeyValueInputs(
				url.startsWith("https://api.githubcopilot.com/mcp")
					? headers.filter((h) => h.name.toLowerCase() !== "authorization")
					: headers,
			);
			return {
		...
		switch (serverPackage.registryType) {
			...
			case RegistryType.DOCKER: {
				...
				args.push(
					serverPackage.version
						? `${dockerIdentifier}:${serverPackage.version}`
						: dockerIdentifier,
				);
				break;
			}
	...
	private processKeyValueInputs(
		...
		for (const input of keyValueInputs) {
			...
			if (inputVariables.length) {
				...
				!value &&
				(input.description || input.choices || input.default !== undefined)
			) {
...
export abstract class AbstractMcpResourceManagementService extends AbstractCommonMcpManagementService {
	...
	protected readonly _onInstallMcpServer = this._register(
	...
	protected readonly _onDidUpdateMcpServers = this._register(
		new Emitter<InstallMcpServerResult[]>(),
	);
	...
	constructor(
		...
		this.reloadConfigurationScheduler = this._register(
			new RunOnceScheduler(() => this.updateLocal(), 50),
		);
	}
	...
	private async populateLocalServers(): Promise<Map<string, ILocalMcpServer>> {
		...
		try {
			const scannedMcpServers =
				await this.mcpResourceScannerService.scanMcpServers(
					this.mcpResource,
					this.target,
				);
			if (scannedMcpServers.servers) {
				await Promise.allSettled(
					Object.entries(scannedMcpServers.servers).map(
						async ([name, scannedServer]) => {
							const server = await this.scanLocalServer(
								name,
								scannedServer,
								scannedMcpServers.sandbox,
							);
	...
	protected async updateLocal(): Promise<void> {
		try {
			...
			const removed = [...this.local.keys()].filter(
	...
	protected async scanLocalServer(
		name: string,
		config: IMcpServerConfiguration,
		rootSandbox?: IMcpSandboxConfiguration,
	): Promise<ILocalMcpServer> {
	...
	async install(
		server: IInstallableMcpServer,
		options?: Omit<InstallOptions, "mcpResource">,
	): Promise<ILocalMcpServer> {
		...
		try {
			...
			this._onDidInstallMcpServers.fire([
				{ name: server.name, error: e, mcpResource: this.mcpResource },
			]);
			throw e;
		}
	}
	...
	async uninstall(
		server: ILocalMcpServer,
		options?: Omit<UninstallOptions, "mcpResource">,
	): Promise<void> {
		...
		try {
			...
			await this.mcpResourceScannerService.removeMcpServers(
				[server.name],
				this.mcpResource,
				this.target,
			);
...
export class McpUserResourceManagementService extends AbstractMcpResourceManagementService {
	...
	async updateMetadata(
		...
		await this.updateLocal();
		const updatedLocal = (await this.getInstalled()).find(
			(s) => s.name === local.name,
		);
		if (!updatedLocal) {
	...
	protected async updateMetadataFromGallery(
		gallery: IGalleryMcpServer,
		...
		await this.fileService.writeFile(
			manifestPath,
			VSBuffer.fromString(JSON.stringify(local)),
		);

		if (gallery.readmeUrl || gallery.readme) {
			const readme = gallery.readme
				? gallery.readme
				: await this.mcpGalleryService.getReadme(
						gallery,
	...
	protected async getLocalServerInfo(
		name: string,
		mcpServerConfig: IMcpServerConfiguration,
	): Promise<ILocalMcpServerInfo | undefined> {
		...
		if (mcpServerConfig.gallery) {
			...
			try {
				...
				storedMcpServerInfo = JSON.parse(
					content.value.toString(),
				) as ILocalMcpServerInfo;

				// migrate
				if (storedMcpServerInfo.galleryUrl?.includes("/v0/")) {
					storedMcpServerInfo.galleryUrl =
						storedMcpServerInfo.galleryUrl.substring(
							0,
							storedMcpServerInfo.galleryUrl.indexOf("/v0/"),
						);
					await this.fileService.writeFile(
						manifestLocation,
						VSBuffer.fromString(JSON.stringify(storedMcpServerInfo)),
					);
				}
	...
	protected override installFromUri(
		uri: URI,
		options?: Omit<InstallOptions, "mcpResource">,
	): Promise<ILocalMcpServer> {
...
export abstract class AbstractMcpManagementService
	extends AbstractCommonMcpManagementService
	implements IMcpManagementService
{
	constructor(
		@IAllowedMcpServersService
	...
	implements IMcpManagementService
{
	...
	private readonly mcpResourceManagementServices = new ResourceMap<
		{ service: McpUserResourceManagementService } & IDisposable
	>();
	...
	private getMcpResourceManagementService(
		...
		let mcpResourceManagementService =
			this.mcpResourceManagementServices.get(mcpResource);
		if (!mcpResourceManagementService) {
			const disposables = new DisposableStore();
			const service = disposables.add(
				this.createMcpResourceManagementService(mcpResource),
			);
			disposables.add(
			...
			this.mcpResourceManagementServices.set(
				mcpResource,
				(mcpResourceManagementService = {
					service,
	...
	async getInstalled(mcpResource?: URI): Promise<ILocalMcpServer[]> {
		const mcpResourceUri =
			mcpResource || this.userDataProfilesService.defaultProfile.mcpResource;
	...
	async install(
		...
		const mcpResourceUri =
			options?.mcpResource ||
			this.userDataProfilesService.defaultProfile.mcpResource;
		return this.getMcpResourceManagementService(mcpResourceUri).install(
			server,
			options,
		);
	}
	...
	async uninstall(
		...
		const mcpResourceUri =
			options?.mcpResource ||
			this.userDataProfilesService.defaultProfile.mcpResource;
		return this.getMcpResourceManagementService(mcpResourceUri).uninstall(
			server,
			options,
		);
	}
	...
	async installFromGallery(
		...
		const mcpResourceUri =
			options?.mcpResource ||
			this.userDataProfilesService.defaultProfile.mcpResource;
		return this.getMcpResourceManagementService(
			mcpResourceUri,
	...
	protected createMcpResourceManagementService(
		mcpResource: URI,
	): McpUserResourceManagementService {
