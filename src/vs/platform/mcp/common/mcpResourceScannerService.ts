...
export type McpResourceTarget =
	| ConfigurationTarget.USER
	| ConfigurationTarget.WORKSPACE
	| ConfigurationTarget.WORKSPACE_FOLDER;

export const IMcpResourceScannerService =
	createDecorator<IMcpResourceScannerService>("IMcpResourceScannerService");
export interface IMcpResourceScannerService {
	...
	addMcpServers(
		servers: IInstallableMcpServer[],
		mcpResource: URI,
		target?: McpResourceTarget,
	): Promise<void>;
	updateSandboxConfig(
		updateFn: (data: IScannedMcpServers) => IScannedMcpServers,
		mcpResource: URI,
		target?: McpResourceTarget,
	): Promise<void>;
	removeMcpServers(
...
export class McpResourceScannerService
	extends Disposable
	implements IMcpResourceScannerService
{
	readonly _serviceBrand: undefined;

	private readonly resourcesAccessQueueMap = new ResourceMap<
		Queue<IScannedMcpServers>
	>();
	...
	async addMcpServers(
		...
		await this.withProfileMcpServers(
			mcpResource,
			target,
			(scannedMcpServers) => {
				...
				for (const { name, config, inputs } of servers) {
					...
					if (inputs) {
						const existingInputIds = new Set(
							updatedInputs.map((input) => input.id),
						);
	...
	private async withProfileMcpServers(
		mcpResource: URI,
		target?: McpResourceTarget,
		updateFn?: (data: IScannedMcpServers) => IScannedMcpServers,
	): Promise<IScannedMcpServers> {
		return this.getResourceAccessQueue(mcpResource).queue(
			async (): Promise<IScannedMcpServers> => {
				...
				try {
					...
					if (errors.length > 0) {
						throw new Error(
							"Failed to parse scanned MCP servers: " + errors.join(", "),
						);
					}
					...
					if (target === ConfigurationTarget.USER) {
						...
						const workspaceScannedMcpServers: IScannedWorkspaceMcpServers =
							result;
						if (workspaceScannedMcpServers.settings?.mcp) {
							scannedMcpServers = this.fromWorkspaceFolderMcpServers(
								workspaceScannedMcpServers.settings?.mcp,
							);
						}
					}
				} catch (error) {
				...
				if (updateFn) {
					...
					if (target === ConfigurationTarget.USER) {
						...
						await this.writeScannedMcpServersToWorkspaceFolder(
							mcpResource,
							scannedMcpServers,
						);
					} else if (target === ConfigurationTarget.WORKSPACE) {
						await this.writeScannedMcpServersToWorkspace(
							mcpResource,
							scannedMcpServers,
						);
					} else {
						assertNever(
							target,
							`Invalid Target: ${ConfigurationTargetToString(target)}`,
						);
					}
				}
				return scannedMcpServers;
	...
	private async writeScannedMcpServers(
		mcpResource: URI,
		scannedMcpServers: IScannedMcpServers,
	): Promise<void> {
		if (
			(scannedMcpServers.servers &&
				Object.keys(scannedMcpServers.servers).length > 0) ||
	...
	private async writeScannedMcpServersToWorkspaceFolder(
		mcpResource: URI,
		scannedMcpServers: IScannedMcpServers,
	): Promise<void> {
		await this.fileService.writeFile(
	...
	private async writeScannedMcpServersToWorkspace(
		mcpResource: URI,
		scannedMcpServers: IScannedMcpServers,
	): Promise<void> {
		...
		try {
			const content = await this.fileService.readFile(mcpResource);
			const errors: ParseError[] = [];
			scannedWorkspaceMcpServers = parse(content.value.toString(), errors, {
				allowTrailingComma: true,
				allowEmptyContent: true,
		...
		await this.fileService.writeFile(
			mcpResource,
			VSBuffer.fromString(
				JSON.stringify(scannedWorkspaceMcpServers, null, "\t"),
			),
		);
	}
	...
	private fromWorkspaceFolderMcpServers(
		scannedWorkspaceFolderMcpServers: IScannedMcpServers,
	): IScannedMcpServers {
	...
	private sanitizeServer(
		...
		if (
			...
			(server.type !== McpServerType.REMOTE &&
				server.type !== McpServerType.LOCAL)
		) {
			(<Mutable<ICommonMcpServerConfiguration>>server).type = (<
				IMcpStdioServerConfiguration
			>server).command
				? McpServerType.LOCAL
