...
export class McpGalleryManifestIPCService
	extends Disposable
	implements IMcpGalleryManifestService
{
	...
	readonly onDidChangeMcpGalleryManifest =
	...
	readonly onDidChangeMcpGalleryManifestStatus =
	...
	get mcpGalleryManifestStatus(): McpGalleryManifestStatus {
		return this._mcpGalleryManifest
			...
			call: async <T>(
				...
				switch (command) {
					case "setMcpGalleryManifest": {
						const manifest = Array.isArray(args)
							? (args[0] as IMcpGalleryManifest | null)
							: null;
	...
	private setMcpGalleryManifest(manifest: IMcpGalleryManifest | null): void {
		...
		this._onDidChangeMcpGalleryManifestStatus.fire(
			this.mcpGalleryManifestStatus,
		);
		this.barrier.open();
	}
}
