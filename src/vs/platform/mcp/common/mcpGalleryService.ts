...
interface IGalleryMcpServerDataSerializer {
	toRawGalleryMcpServerResult(
		input: unknown,
	): IRawGalleryMcpServersResult | undefined;
...
namespace McpServerSchemaVersion_v2025_07_09 {
	...
	interface RawGalleryMcpServer {
		...
		readonly _meta: {
			...
			readonly "io.modelcontextprotocol.registry/publisher-provided"?: Record<
				string,
				unknown
			>;
		};
	...
	class Serializer implements IGalleryMcpServerDataSerializer {
		public toRawGalleryMcpServerResult(
			input: unknown,
		): IRawGalleryMcpServersResult | undefined {
			if (
				!input ||
				typeof input !== "object" ||
				!Array.isArray((input as RawGalleryMcpServersResult).servers)
			) {
				return undefined;
			}
		...
		public toRawGalleryMcpServer(
			...
			if (
				...
				!isString(from.name) ||
				!from.description ||
				!isString(from.description) ||
				!from.version ||
				!isString(from.version)
			) {
				return undefined;
			}
			...
			const registryInfo =
				from._meta?.["io.modelcontextprotocol.registry/official"];
			...
			const gitHubInfo: RawGitHubInfo | undefined = from._meta[
				"io.modelcontextprotocol.registry/publisher-provided"
			]?.github as RawGitHubInfo | undefined;
...
namespace McpServerSchemaVersion_v0_1 {
	...
	class Serializer implements IGalleryMcpServerDataSerializer {
		...
		public toRawGalleryMcpServer(
			...
			if (
				!from.server ||
				!isObject(from.server) ||
				!from.server.name ||
				!isString(from.server.name) ||
				!from.server.description ||
				!isString(from.server.description) ||
				!from.server.version ||
				!isString(from.server.version)
			) {
				return undefined;
			}
			...
			const githubInfo = from.server._meta?.[
				"io.modelcontextprotocol.registry/publisher-provided"
			]?.github as IGitHubInfo | undefined;
...
export class McpGalleryService
	extends Disposable
	implements IMcpGalleryService
{
	_serviceBrand: undefined;

	private galleryMcpServerDataSerializers: Map<
		string,
		IGalleryMcpServerDataSerializer
	>;
	...
	constructor(
		...
		this.galleryMcpServerDataSerializers.set(
			...
			McpServerSchemaVersion_v0.SERIALIZER,
		);
		this.galleryMcpServerDataSerializers.set(
	...
	isEnabled(): boolean {
		return (
			this.mcpGalleryManifestService.mcpGalleryManifestStatus ===
	...
	async query(
		...
		const { servers, metadata } = await this.queryGalleryMcpServers(
			query,
			mcpGalleryManifest,
			token,
		);
		...
		return {
			...
			getNextPage: async (
			...
			): Promise<IIterativePage<IGalleryMcpServer>> => {
				...
				const { servers, metadata: nextMetadata } =
	...
	async getMcpServersFromGallery(
		...
		await Promise.allSettled(
			infos.map(async (info) => {
				const mcpServer = await this.getMcpServerByName(
					info,
					mcpGalleryManifest,
				);
				if (mcpServer) {
	...
	private async getMcpServerByName(
		...
		mcpGalleryManifest: IMcpGalleryManifest,
	): Promise<IGalleryMcpServer | undefined> {
		const mcpServerUrl = this.getLatestServerVersionUrl(
			name,
			mcpGalleryManifest,
		);
		if (mcpServerUrl) {
		...
		const byIdUrl = id
			? this.getServerIdUrl(id, mcpGalleryManifest)
			: undefined;
		if (byIdUrl) {
	...
	private toGalleryMcpServer(
		server: IRawGalleryMcpServer,
		manifest: IMcpGalleryManifest | null,
	): IGalleryMcpServer {
		let publisher = "";
		...
		if (server.githubInfo?.name) {
			if (!displayName) {
				displayName = server.githubInfo.name
					.split("-")
					.map((s) =>
						s.toLowerCase() === "mcp"
							? "MCP"
							: s.toLowerCase() === "github"
								? "GitHub"
		...
		if (server.githubInfo?.preferredImage) {
			...
			const lightIcon =
				server.icons.find((icon) => icon.theme === "light") ?? server.icons[0];
			const darkIcon =
				server.icons.find((icon) => icon.theme === "dark") ?? lightIcon;
			icon = {
				light: lightIcon.src,
				dark: darkIcon.src,
			};
		...
		const publisherUrl = manifest
			? this.getPublisherUrl(publisher, manifest)
			: undefined;
		...
		return {
			...
			publishDate: server.registryInfo?.publishedAt
				? Date.parse(server.registryInfo.publishedAt)
				: undefined,
			lastUpdated: server.githubInfo?.pushedAt
				? Date.parse(server.githubInfo.pushedAt)
				: server.registryInfo?.updatedAt
					? Date.parse(server.registryInfo.updatedAt)
					: undefined,
	...
	private async queryGalleryMcpServers(
		query: Query,
		mcpGalleryManifest: IMcpGalleryManifest,
		token: CancellationToken,
	): Promise<IGalleryMcpServersResult> {
		const { servers, metadata } = await this.queryRawGalleryMcpServers(
			query,
			mcpGalleryManifest,
			token,
		);
		return {
			servers: servers.map((item) =>
				this.toGalleryMcpServer(item, mcpGalleryManifest),
			),
			metadata,
		};
	}

	private async queryRawGalleryMcpServers(
		query: Query,
		mcpGalleryManifest: IMcpGalleryManifest,
		token: CancellationToken,
	): Promise<IRawGalleryMcpServersResult> {
		...
		if (!result) {
			throw new Error(
				`Failed to serialize MCP servers result from ${mcpGalleryUrl}`,
				data,
			);
		}
	...
	async getMcpServer(
		mcpServerUrl: string,
		mcpGalleryManifest?: IMcpGalleryManifest | null,
	): Promise<IGalleryMcpServer | undefined> {
		const context = await this.requestService.request(
			{
				type: "GET",
		...
		mcpGalleryManifest =
			mcpGalleryManifest && mcpServerUrl.startsWith(mcpGalleryManifest.url)
				? mcpGalleryManifest
				: null;
		...
		if (!server) {
			throw new Error(
				`Failed to serialize MCP server from ${mcpServerUrl}`,
				data,
			);
		}
	...
	private serializeMcpServersResult(
		...
		return this.getSerializer(mcpGalleryManifest)?.toRawGalleryMcpServerResult(
			data,
		);
	}
	...
	private getSerializer(
		mcpGalleryManifest: IMcpGalleryManifest | null,
	): IGalleryMcpServerDataSerializer | undefined {
