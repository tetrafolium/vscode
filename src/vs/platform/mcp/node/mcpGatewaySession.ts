...
export class McpGatewaySession extends Disposable {
	...
	constructor(
		...
		this._rpc = this._register(
			new JsonRpcProtocol((message) => this._handleOutgoingMessage(message), {
		...
		this._register(
			this._serverInvoker.onDidChangeTools(() => {
				if (!this._isInitialized) {
					return;
				}

				this._logService.info(
					`[McpGateway][session ${this.id}] Tools changed, notifying client`,
				);
		...
		this._register(
			this._serverInvoker.onDidChangeResources(() => {
				if (!this._isInitialized) {
					return;
				}

				this._logService.info(
					`[McpGateway][session ${this.id}] Resources changed, notifying client`,
				);
				this._rpc.sendNotification({
	...
	public attachSseClient(
		...
		this._logService.info(
			`[McpGateway][session ${this.id}] SSE client attached (total: ${this._sseClients.size})`,
		);

		res.on("close", () => {
			this._sseClients.delete(res);
			this._logService.info(
				`[McpGateway][session ${this.id}] SSE client detached (total: ${this._sseClients.size})`,
			);
		});
	}
	...
	public override dispose(): void {
		this._logService.info(
			`[McpGateway][session ${this.id}] Disposing session (SSE clients: ${this._sseClients.size})`,
		);
	...
	private _handleOutgoingMessage(message: JsonRpcMessage): void {
		if (isJsonRpcResponse(message)) {
			this._logService.debug(
				`[McpGateway][session ${this.id}] --> response: ${JSON.stringify(message)}`,
			);
			return;
		}
		...
		if (isJsonRpcNotification(message)) {
			this._logService.debug(
				`[McpGateway][session ${this.id}] --> notification: ${(message as IJsonRpcNotification).method}`,
			);
		...
		this._logService.warn(
			"[McpGatewayService] Ignored unsupported outgoing gateway message",
		);
	}
	...
	private _broadcastSse(message: JsonRpcMessage): void {
		if (this._sseClients.size === 0) {
			this._logService.debug(
				`[McpGateway][session ${this.id}] No SSE clients to broadcast to, dropping message`,
			);
			return;
		}
		...
		this._logService.debug(
			`[McpGateway][session ${this.id}] Broadcasting SSE event id=${eventId} to ${this._sseClients.size}`,
		);
	...
	private async _handleRequest(request: IJsonRpcRequest): Promise<unknown> {
		this._logService.debug(
			`[McpGateway][session ${this.id}] <-- request: ${request.method} (id=${String(request.id)})`,
		);
		...
		if (!this._isInitialized) {
			this._logService.warn(
				`[McpGateway][session ${this.id}] Rejected request '${request.method}': session not initialized`,
			);
		...
		switch (request.method) {
			...
			default:
				this._logService.warn(
					`[McpGateway][session ${this.id}] Unknown method: ${request.method}`,
				);
	...
	private _handleNotification(notification: IJsonRpcNotification): void {
		this._logService.debug(
			`[McpGateway][session ${this.id}] <-- notification: ${notification.method}`,
		);

		if (notification.method === "notifications/initialized") {
			this._isInitialized = true;
			this._logService.info(
				`[McpGateway][session ${this.id}] Session initialized`,
			);
			this._rpc.sendNotification({
				method: "notifications/tools/list_changed",
			});
	...
	private _handleInitialize(request: IJsonRpcRequest): MCP.InitializeResult {
		...
		const negotiatedVersion =
			clientVersion && MCP_SUPPORTED_PROTOCOL_VERSIONS.includes(clientVersion)
				? clientVersion
				: MCP_LATEST_PROTOCOL_VERSION;

		this._logService.info(
			`[McpGateway] Initialize: client=${clientInfo?.name ?? "unknown"}/${clientInfo?.version ?? "?"}, clientProtocol=${clientVersion ?? "(none)"}, negotiated=${negotiatedVersion}`,
		);
		if (clientVersion && clientVersion !== negotiatedVersion) {
			this._logService.warn(
				`[McpGateway] Client requested unsupported protocol version '${clientVersion}', falling back to '${negotiatedVersion}'`,
			);
		}
	...
	private async _handleCallTool(
		...
		this._logService.debug(
			`[McpGateway][session ${this.id}] Calling tool '${params.name}' with args: ${JSON.stringify(argumentsValue)}`,
		);

		try {
			const result = await this._serverInvoker.callTool(
				params.name,
				argumentsValue,
			);
			this._logService.debug(
				`[McpGateway][session ${this.id}] Tool '${params.name}' completed (isError=${result.isError ?? false}, content blocks=${result.content.length})`,
			);
			return result;
		} catch (error) {
			this._logService.error(
				`[McpGateway][session ${this.id}] Tool '${params.name}' invocation failed`,
				error,
			);
	...
	private async _handleListTools(): Promise<MCP.ListToolsResult> {
		...
		this._logService.debug(
			`[McpGateway][session ${this.id}] Listed ${tools.length} tool(s): [${tools.map((t) => t.name).join(", ")}]`,
		);
	...
	private async _handleListResources(): Promise<MCP.ListResourcesResult> {
		...
		this._logService.debug(
			`[McpGateway][session ${this.id}] Listed ${resources.length} resource(s)`,
		);
	...
	private async _handleReadResource(
		...
		this._logService.debug(
			`[McpGateway][session ${this.id}] Reading resource '${params.uri}'`,
		);
		try {
			const result = await this._serverInvoker.readResource(params.uri);
			this._logService.debug(
				`[McpGateway][session ${this.id}] Resource read returned ${result.contents.length} content(s)`,
			);
			return result;
		} catch (error) {
			this._logService.error(
				`[McpGateway][session ${this.id}] Resource read failed for '${params.uri}'`,
				error,
			);
			throw new JsonRpcError(MCP_INVALID_PARAMS, String(error));
		}
	...
	private async _handleListResourceTemplates(): Promise<MCP.ListResourceTemplatesResult> {
		...
		this._logService.debug(
			`[McpGateway][session ${this.id}] Listed ${resourceTemplates.length} resource template(s)`,
		);
