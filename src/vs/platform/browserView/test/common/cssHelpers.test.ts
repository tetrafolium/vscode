...
function rule(
	...
	return {
		rule: {
			...
			origin,
			style: { cssText, cssProperties: props },
		},
	};
}
			...
			inherited: [
				{
					matchedCSSRules: [
						rule(
							"body",
							"font-family: sans-serif; background: red; margin: 0;",
						),
					],
				},
			],
	...
	test("collects var references from rules", () => {
		const matched: IMatchedStyles = {
			matchedCSSRules: [
				rule(
					".x",
					"color: var(--fg-color); border: var(--border-width) solid;",
				),
			],
		};
			...
			pseudoElements: [
				{
					pseudoType: "before",
					matches: [
						rule("input::before", 'content: ""; display: block;', "user-agent"),
					],
				},
			],
		};
	...
	test("tracks user-agent property names from inherited rules (inheritable only)", () => {
		const matched: IMatchedStyles = {
			...
			inherited: [
				{
					matchedCSSRules: [
						rule("body", "font-family: sans-serif; margin: 0;", "user-agent"),
					],
				},
			],
		};
