...
type CertificateVerifyProc = Parameters<
	Electron.Session["setCertificateVerifyProc"]
>[0];
type CertificateVerifyRequest = Parameters<
	NonNullable<CertificateVerifyProc>
>[0];
...
class TestApplicationStorageMainService {
	...
	readonly remove = sinon
		.stub<[string, StorageScope], void>()
		.callsFake((key) => {
...
function createTrust(sessionId = "test-session"): {
	...
	const browserSession = new TestBrowserSession(
		sessionId,
		electronSession.asSession(),
	);
...
function createCertificate(
	fingerprint: string,
	extra?: Partial<Electron.Certificate>,
...
function invokeVerifyProc(
	electronSession: TestElectronSession,
	request: Partial<CertificateVerifyRequest> & {
		hostname: string;
...
suite("BrowserSessionTrust", () => {
	...
	test("installs certificate verify proc and tracks certificate errors", () => {
		...
		assert.deepStrictEqual(
			trust.getCertificateError("https://example.com/path"),
			{
				host: "example.com",
				fingerprint: "abc123",
	...
	test("trustCertificate persists data under the trust storage key", async () => {
		...
		assert.deepStrictEqual(storage.store.firstCall.args.slice(0, 4), [
			STORAGE_KEY,
		...
		assert.deepStrictEqual(
			persisted["test-session"].trustedCerts.map(
				(entry: { host: string; fingerprint: string }) => ({
					host: entry.host,
					fingerprint: entry.fingerprint,
				}),
			),
			[{ host: "example.com", fingerprint: "abc123" }],
		);
	});
	...
	test("trustCertificate stores expiresAt relative to current time", async () => {
		...
		const persisted = JSON.parse(storage.read(STORAGE_KEY)!);
		const [entry] = persisted["test-session"].trustedCerts as {
			host: string;
			fingerprint: string;
			expiresAt: number;
		}[];
	...
	test("connectStorage restores valid trust entries and prunes expired ones", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.deepStrictEqual(
				...
				[{ host: "valid.example.com", fingerprint: "valid" }],
			);
		}));
	...
	test("stored and reloaded trust expires and is pruned", async () => {
		...
		const firstBrowserSession = new TestBrowserSession(
			"test-session",
			firstSession.asSession(),
		);
		const firstTrust = new BrowserSessionTrust(
			firstBrowserSession.asBrowserSession(),
		);
		firstTrust.connectStorage(storage.asService());
		await firstTrust.trustCertificate(
			"reload.example.com",
			"reload-fingerprint",
		);
		...
		const secondBrowserSession = new TestBrowserSession(
			"test-session",
			secondSession.asSession(),
		);
		const secondTrust = new BrowserSessionTrust(
			secondBrowserSession.asBrowserSession(),
		);
		const webContents = new TestWebContents();
		...
		webContents.emit(
			...
			"ERR_CERT",
			createCertificate("reload-fingerprint"),
			(value: boolean) => {
				callbackResult = value;
			},
		);
	...
	test("untrustCertificate removes persisted trust and closes connections", async () => {
		...
		assert.strictEqual(
			storage.remove.calledOnceWithExactly(
				STORAGE_KEY,
				StorageScope.APPLICATION,
			),
			true,
	...
	test("untrustCertificate throws when certificate is not found", async () => {
		...
		await assert.rejects(
			...
			(error) => {
				...
				assert.strictEqual(
					error.message,
					"Certificate not found: host=missing.example.com fingerprint=missing-fingerprint",
				);
				return true;
			},
		);
	...
	test("clear removes trust, clears cert errors, and closes connections", async () => {
		...
		assert.strictEqual(electronSession.closeAllConnections.calledOnce, true);
		assert.strictEqual(
			trust.getCertificateError("https://example.com"),
			undefined,
		);
		assert.strictEqual(storage.read(STORAGE_KEY), undefined);
	});
