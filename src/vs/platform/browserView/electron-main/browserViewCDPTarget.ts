...
export class BrowserViewCDPTarget extends Disposable implements ICDPTarget {
	...
	constructor(
		...
		this._register(
			this.view.debugger.onTargetInfoChanged((info) => {
				if (info.targetId !== this._targetInfo.targetId) {
					return;
				}

				if (
					info.title !== this._targetInfo.title ||
					info.url !== this._targetInfo.url
				) {
		...
		this._register(
			this.view.debugger.onTargetDestroyed((targetId) => {
