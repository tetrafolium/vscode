...
import type {
	...
	IWindowsMxcPolicyContainment,
	IWindowsMxcSandboxPolicy,
...
} from "../../common/terminalSandboxEngine.js";
import {
	IWindowsMxcTerminalSandboxRuntime,
	WindowsMxcTerminalSandboxRuntime,
} from "../../common/terminalSandboxMxcRuntime.js";
import {
...
suite("TerminalSandboxEngine", () => {
	...
	function buildMockWindowsMxcSandboxPayload(
		commandLine: string,
		...
		const network = {
			defaultPolicy: policy.network?.allowOutbound
				? "allow"
				: ("block" as "allow" | "block"),
			...(policy.network?.allowLocalNetwork !== undefined
	...
	function createHost(
		overrides: Partial<ITerminalSandboxEngineHost> = {},
	): ITerminalSandboxEngineHost & { rootsEmitter: Emitter<void> } {
		...
		const host: ITerminalSandboxEngineHost = {
			...
			buildWindowsMxcSandboxPayload: (
				...
				Promise.resolve(
					buildMockWindowsMxcSandboxPayload(
						commandLine,
						policy,
						workingDirectory,
						containerName,
			...
			getSandboxSetting: <T>(settingId: string): T | undefined =>
				sandboxSettings.has(settingId)
					? (sandboxSettings.get(settingId) as T)
					: undefined,
	...
	function createWindowsHost(
		overrides: Partial<ITerminalSandboxEngineHost> = {},
	): ITerminalSandboxEngineHost & { rootsEmitter: Emitter<void> } {
		return createHost({
			getOS: () => Promise.resolve(OperatingSystem.Windows),
			getRuntimeInfo: () =>
				Promise.resolve({ appRoot: "C:\\app", arch: "x64" }),
			getUserHome: () =>
				Promise.resolve(URI.from({ scheme: "file", path: "/c:/Users/user" })),
			getSandboxTempDir: () =>
				Promise.resolve(
					URI.from({ scheme: "file", path: "/c:/Users/user/.test-data/tmp" }),
				),
			getWorkspaceStorageReadRoot: () =>
				Promise.resolve(
					URI.from({
						scheme: "file",
						path: "/c:/Users/user/workspaceStorage/workspace-id",
					}),
				),
			getWriteRoots: () => [
				URI.from({ scheme: "file", path: "/c:/workspace" }),
			],
			getWindowsMxcFilesystemPolicy: () =>
				Promise.resolve({
					...
					readwritePaths: ["C:\\Users\\user\\AppData\\Local\\Temp"],
				}),
	...
	test("runAsNode=true prefixes the wrapped command with ELECTRON_RUN_AS_NODE=1", async () => {
		...
		const engine = store.add(
			instantiationService.createInstance(TerminalSandboxEngine, host),
		);
		...
		ok(
			...
			`Expected ELECTRON_RUN_AS_NODE=1 prefix. Actual: ${wrapped.command}`,
		);
	});
	...
	test("runAsNode=false omits the ELECTRON_RUN_AS_NODE=1 prefix", async () => {
		...
		ok(
			...
			`Did not expect ELECTRON_RUN_AS_NODE prefix. Actual: ${wrapped.command}`,
		);
	});
	...
	test("wrapCommand adds ripgrep-universal platform-arch bin directory to PATH", async () => {
		...
		ok(
			wrapped.command.includes(
				`/app/node_modules/@vscode/ripgrep-universal/bin/linux-${arch}`,
			),
			`Expected ripgrep-universal platform-arch path in command. Actual: ${wrapped.command}`,
		);
	});
	...
	test("onDidChangeRoots triggers a sandbox config rewrite on the next wrap", async () => {
		...
		ok(
			createFileCount > initialWriteCount,
			`Expected sandbox config to be rewritten after onDidChangeRoots (initial=${initialWriteCount}, after=${createFileCount})`,
		);
		const configPath = await engine.getSandboxConfigPath();
		ok(configPath, "Config path should be defined");
		const config = JSON.parse(createdFiles.get(configPath!)!);
		ok(
			config.filesystem.allowWrite.includes("/workspace-b"),
			"Refreshed config should include the new write root",
		);
		ok(
			!config.filesystem.allowWrite.includes("/workspace-a"),
			"Refreshed config should drop the old write root",
		);
	});
	...
	test("resolves filesystem paths and expands home on Linux when writing the config", async () => {
		...
		await engine.wrapCommand("git commit -S", false, undefined, undefined, [
			{ keyword: "git", args: ["commit", "-S"] },
		]);
		...
		const config = JSON.parse(createdFiles.get(configPath)!);
		ok(
			config.filesystem.allowWrite.includes("/real/workspace"),
			"Workspace write root symlink should be resolved",
		);
		ok(
			config.filesystem.allowWrite.includes("/real/write"),
			"Configured allowWrite symlink should be resolved",
		);
		ok(
			config.filesystem.allowRead.includes("/real/read"),
			"Configured allowRead should expand ~ and resolve symlink",
		);
		ok(
			config.filesystem.allowRead.includes("/real/gnupg"),
			"Command runtime allowRead should expand ~ and resolve symlink",
		);
		ok(
			config.filesystem.allowWrite.includes("/real/gnupg"),
			"Command runtime allowWrite should expand ~ and resolve symlink",
		);
		ok(
			config.filesystem.denyRead.includes("/real/deny-read"),
			"Configured denyRead should expand ~ and resolve symlink",
		);
		ok(
			config.filesystem.denyWrite.includes("/real/deny-write"),
			"Configured denyWrite symlink should be resolved",
		);
	});
	...
	test("keeps filesystem paths without symlinks when writing the config", async () => {
		...
		const config = JSON.parse(createdFiles.get(configPath)!);
		ok(
			config.filesystem.allowWrite.includes("/workspace-plain"),
			"Workspace write root without symlink should be preserved",
		);
		ok(
			config.filesystem.allowWrite.includes("/write-plain"),
			"Configured allowWrite without symlink should be preserved",
		);
		ok(
			config.filesystem.allowRead.includes("/home/user/read-plain"),
			"Configured allowRead without symlink should expand ~ and be preserved",
		);
		ok(
			config.filesystem.denyRead.includes("/home/user/deny-read-plain"),
			"Configured denyRead without symlink should expand ~ and be preserved",
		);
		ok(
			config.filesystem.denyWrite.includes("/deny-write-plain"),
			"Configured denyWrite without symlink should be preserved",
		);
	});
	...
	test("precheck inputs can disable sandboxing when default approval permission is disabled", async () => {
		...
		strictEqual(
			await engine.isEnabled({ isDefaultApprovalPermissionEnabled: true }),
			true,
		);
		strictEqual(
			await engine.isEnabled({ isDefaultApprovalPermissionEnabled: false }),
			false,
		);
		strictEqual(
			await engine.isSandboxAllowNetworkEnabled({
				isDefaultApprovalPermissionEnabled: false,
			}),
			false,
		);
		strictEqual(
			await engine.getSandboxConfigPath(false, {
				isDefaultApprovalPermissionEnabled: false,
			}),
			undefined,
		);
		...
		deepStrictEqual(
			await engine.checkForSandboxingPrereqs(false, {
				isDefaultApprovalPermissionEnabled: false,
			}),
			{
				enabled: false,
		...
		strictEqual(
			createFileCount,
			0,
			"Disabled sandbox precheck should not create sandbox config files",
		);
	});
	...
	test("wrapCommand uses MXC executable and writes MXC config on Windows", async () => {
		...
		strictEqual(wrapped.isSandboxWrapped, true);
		ok(
			wrapped.command.startsWith(
				`& 'C:\\app\\node_modules\\@microsoft\\mxc-sdk\\bin\\x64\\wxc-exec.exe'`,
			),
			`Expected MXC executable. Actual: ${wrapped.command}`,
		);
		ok(
			wrapped.command.includes(` '${configPath}'`),
			`Expected wrapped command to pass the MXC config path. Actual: ${wrapped.command}`,
		);
		...
		strictEqual(
			...
			'"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -Command "echo hello"',
		);
		strictEqual(
			normalizeWindowsPathForAssert(config.process.cwd),
			"c:/workspace",
		);
		...
		ok(
			config.process.env.includes("USERPROFILE=C:\\Users\\user"),
			"USERPROFILE should be injected into the MXC process env",
		);
		ok(
			config.process.env.includes("APPDATA=C:\\Users\\user\\AppData\\Roaming"),
		...
		ok(
			...
			"LOCALAPPDATA should be injected into the MXC process env",
		);
		ok(
		...
		ok(
			config.filesystem.readwritePaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/workspace",
			),
			"Workspace should be writable",
		);
		ok(
			config.filesystem.readwritePaths.some((path: string) =>
				normalizeWindowsPathForAssert(path).endsWith("/.test-data/tmp"),
			),
			"Sandbox temp dir should be writable",
		);
		ok(
			config.filesystem.readwritePaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) ===
					"c:/users/user/appdata/local/temp",
			),
			"MXC temporary files policy should add host temp path to writable paths",
		);
		ok(
			config.filesystem.readonlyPaths.some((path: string) =>
				normalizeWindowsPathForAssert(path).endsWith("/.test-data/tmp"),
			),
			"Sandbox temp dir should be readable through readonly paths",
		);
		ok(
			config.filesystem.readonlyPaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/tools/node",
			),
		...
		ok(
			...
			"Resolved PowerShell executable directory should be readable",
		);
		ok(
			config.filesystem.readonlyPaths.some(
				(path: string) =>
			...
			"MXC user profile policy should add user profile paths to readonly paths",
		);
		ok(
			!config.filesystem.deniedPaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/users/user",
			),
			"User home should not be denied by default on Windows",
		);
	});
	...
	test("wrapCommand applies Windows filesystem setting to MXC config", async () => {
		...
		ok(
			serializedConfig.includes("C:\\\\configured\\\\write"),
			"Configured Windows allowWrite path should be escaped in the serialized MXC config",
		);
		ok(
			serializedConfig.includes("C:\\\\configured\\\\read"),
			"Configured Windows allowRead path should be escaped in the serialized MXC config",
		);
		ok(
			serializedConfig.includes("C:\\\\configured\\\\secret"),
			"Configured Windows denyRead path should be escaped in the serialized MXC config",
		);
		ok(
			config.filesystem.readwritePaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/configured/write",
			),
			"Configured Windows allowWrite path should be writable",
		);
		ok(
			config.filesystem.readonlyPaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/configured/read",
			),
			"Configured Windows allowRead path should be readonly",
		);
		ok(
			config.filesystem.readwritePaths.some(
				(path: string) =>
			...
			"Host temp path from Windows policy should be writable",
		);
		ok(
			config.filesystem.deniedPaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/configured/secret",
			),
			"Configured Windows denyRead path should be denied",
		);
		ok(
			!config.filesystem.deniedPaths.some(
				(path: string) =>
	...
	test("resolves Windows filesystem symlinks when writing MXC config", async () => {
		...
		fileService.setRealpath(
			"/c:/configured/write-link",
			"/c:/real/configured-write",
		);
		fileService.setRealpath(
			"/c:/configured/read-link",
			"/c:/real/configured-read",
		);
		fileService.setRealpath(
			"/c:/configured/secret-link",
			"/c:/real/configured-secret",
		);
		fileService.setRealpath("/c:/tools/node", "/c:/real/tools-node");
		const host = createWindowsHost({
			getWriteRoots: () => [
				URI.from({ scheme: "file", path: "/c:/workspace-link" }),
			],
		});
		const engine = store.add(
			instantiationService.createInstance(TerminalSandboxEngine, host),
		...
		ok(
			config.filesystem.readwritePaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/real/workspace",
			),
			"Workspace write root symlink should be resolved on Windows",
		);
		ok(
			config.filesystem.readwritePaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/real/configured-write",
			),
			"Configured Windows allowWrite symlink should be resolved",
		);
		ok(
			config.filesystem.readonlyPaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/real/configured-read",
			),
			"Configured Windows allowRead symlink should be resolved",
		);
		ok(
			config.filesystem.readonlyPaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/real/tools-node",
			),
			"Windows policy readonly symlink should be resolved",
		);
		ok(
			config.filesystem.deniedPaths.some(
				(path: string) =>
					normalizeWindowsPathForAssert(path) === "c:/real/configured-secret",
			),
			"Configured Windows denyRead symlink should be resolved",
		);
	});
	...
	test("wrapCommand uses arm64 MXC executable on Windows arm64", async () => {
		...
		const host = createWindowsHost({
			getRuntimeInfo: () =>
				Promise.resolve({ appRoot: "C:\\app", arch: "arm64" }),
		});
		const engine = store.add(
			instantiationService.createInstance(TerminalSandboxEngine, host),
		);
	...
	test("wrapCommand rewrites MXC config when Windows command changes", async () => {
		...
		ok(configPath, "Config path should be defined");
		const firstCommandLine = JSON.parse(createdFiles.get(configPath)!).process
			.commandLine;
		strictEqual(
			firstCommandLine,
			'"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -Command "echo first"',
		);
		...
		ok(configPath, "Config path should be defined");
		const secondCommandLine = JSON.parse(createdFiles.get(configPath)!).process
			.commandLine;
		strictEqual(
			secondCommandLine,
			'"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -Command "echo second"',
		);
	});
	...
	test("uses OS-specific filesystem absolute path detection", async () => {
		const linuxEngine = store.add(
			instantiationService.createInstance(TerminalSandboxEngine, createHost()),
		);
		await linuxEngine.getOS();
		const isLinuxAbsolutePath = (
			linuxEngine as unknown as {
				_isAbsoluteFileSystemPath(path: string): boolean;
			}
		)._isAbsoluteFileSystemPath.bind(linuxEngine);
		...
		const windowsEngine = store.add(
			instantiationService.createInstance(
				TerminalSandboxEngine,
				createHost({ getOS: () => Promise.resolve(OperatingSystem.Windows) }),
			),
		);
		await windowsEngine.getOS();
		const isWindowsAbsolutePath = (
			windowsEngine as unknown as {
				_isAbsoluteFileSystemPath(path: string): boolean;
			}
		)._isAbsoluteFileSystemPath.bind(windowsEngine);
	...
	test("checkForSandboxingPrereqs reports missing dependencies", async () => {
		let status: ISandboxDependencyStatus = {
