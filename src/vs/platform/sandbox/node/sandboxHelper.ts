...
type BubblewrapProbe = (
	command: string,
) => Promise<{ usable: boolean; error?: string }>;
type GetLinuxReleaseInfo = () => Promise<
	{ id: string; version_id?: string } | undefined
>;

export class SandboxHelperService implements ISandboxHelperService {
	declare readonly _serviceBrand: undefined;

	static async checkSandboxDependenciesWith(
		findCommand: FindCommand,
		linux: boolean = isLinux,
		probeBubblewrap: BubblewrapProbe = (command) =>
			SandboxHelperService._probeBubblewrap(command),
		getLinuxReleaseInfo: GetLinuxReleaseInfo = () =>
			getOSReleaseInfo(() => undefined),
		...
		const bubblewrapProbe = bubblewrapPath
			? await probeBubblewrap(bubblewrapPath)
			: { usable: false };
		const releaseInfo =
			bubblewrapPath && !bubblewrapProbe.usable
				? await getLinuxReleaseInfo()
				: undefined;
		...
		return {
			...
			supportsUbuntuAppArmorRemediation:
				releaseInfo?.id === "ubuntu" && releaseInfo.version_id === "24.04",
	...
	private static _probeBubblewrap(
		command: string,
	): Promise<{ usable: boolean; error?: string }> {
		return new Promise((resolve) => {
			execFile(
				command,
				["--unshare-net", "--dev-bind", "/", "/", "echo", "ok"],
	...
	> {
		...
		} = await import("@microsoft/mxc-sdk");
		...
		return {
			readonlyPaths: [
				...new Set([
					...availableToolsPolicy.readonlyPaths,
					...userProfilePolicy.readonlyPaths,
					...temporaryFilesPolicy.readonlyPaths,
					...(psHome ? [psHome] : []),
				]),
			],
			readwritePaths: [
				...new Set([
					...availableToolsPolicy.readwritePaths,
					...userProfilePolicy.readwritePaths,
					...temporaryFilesPolicy.readwritePaths,
				]),
			],
		};
	}
	...
	private async _getPSHome(): Promise<string | undefined> {
		...
		const powerShellPath =
			(await findExecutable("pwsh")) ?? (await findExecutable("powershell"));
