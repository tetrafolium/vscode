		...
		switch (command) {
			...
			case "buildWindowsMxcSandboxPayload": {
				const {
					...
					containerName,
					containment,
				} = _arg as IBuildWindowsMxcSandboxPayloadArgs;
				return this.service.buildWindowsMxcSandboxPayload(
					commandLine,
					policy,
					workingDirectory,
					containerName,
...
export class SandboxHelperChannelClient implements ISandboxHelperService {
	...
	buildWindowsMxcSandboxPayload(
		...
		return this.channel.call<IWindowsMxcConfig | undefined>(
			"buildWindowsMxcSandboxPayload",
			{
				commandLine,
				policy,
				...
				containerName,
				containment,
			} satisfies IBuildWindowsMxcSandboxPayloadArgs,
		);
	}
}
