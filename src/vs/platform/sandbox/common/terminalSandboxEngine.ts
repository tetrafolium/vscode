...
import {
	...
	normalizeDomain,
} from "../../networkFilter/common/domainMatcher.js";
...
import {
	ITerminalSandboxCommand,
	ITerminalSandboxPrecheckInputs,
	ITerminalSandboxPrerequisiteCheckResult,
	ITerminalSandboxResolvedNetworkDomains,
	ITerminalSandboxWrapResult,
	TerminalSandboxPrerequisiteCheck,
	TerminalSandboxPreCheckRemediation,
} from "./terminalSandboxService.js";
...
export class TerminalSandboxEngine extends Disposable {
	...
	private static readonly _sshRemoteRegex =
		/(?:^|[\s'"`])(?:[^\s@:'"`]+@)?([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})(?::[^\s'"`|&;<>]+)(?=$|[\s'"`|&;<>])/gi;
	private static readonly _hostRegex =
		/(?:^|[\s'"`(=])([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})(?::\d+)?(?=(?:\/[^\s'"`|&;<>]*)?(?:$|[\s'"`)\]|,;|&<>]))/gi;
	...
	constructor(
		...
		private readonly _windowsMxcRuntime: IWindowsMxcTerminalSandboxRuntime,
	) {
		super();
		this._register(
			Event.runAndSubscribe(this._host.onDidChangeSandboxSettings, () => {
				this.setNeedsForceUpdateConfigFile();
			}),
		);
		this._register(
			this._host.onDidChangeRoots(() => this.setNeedsForceUpdateConfigFile()),
		);
	}
	...
	isAutoApproveUnsandboxedCommands(): boolean {
		return (
			this._areUnsandboxedCommandsAllowed() &&
			this._getSettingValue<boolean>(
	...
	async wrapCommand(
	...
	): Promise<ITerminalSandboxWrapResult> {
		const normalizedCommandDetails = this._normalizeCommandDetails(
			commandDetails ?? [],
		);
		const normalizedCommandKeywords = this._normalizeCommandKeywords(
			normalizedCommandDetails.map((c) => c.keyword),
		);
		const currentReadAllowListPaths =
			getTerminalSandboxReadAllowListForCommands(
				this._os,
				this._commandAllowListKeywords,
				this._commandAllowListCommandDetails,
			);
		const nextReadAllowListPaths = getTerminalSandboxReadAllowListForCommands(
			this._os,
			normalizedCommandKeywords,
			normalizedCommandDetails,
		);
		...
		const shouldRefreshConfig =
			this._commandAllowListKeywords.length === 0 ||
			this._needsForceUpdateConfigFile ||
			!this._areStringArraysEqual(
				this._commandAllowListKeywords,
				normalizedCommandKeywords,
			) ||
			!this._areStringArraysEqual(
				currentReadAllowListPaths,
				nextReadAllowListPaths,
			) ||
			!this._areObjectsEqual(
			...
			(this._os === OperatingSystem.Windows &&
				(this._commandLine !== command || this._commandShell !== shell));
		...
		if (this._os === OperatingSystem.Windows) {
			...
			return {
				command: this._windowsMxcRuntime.wrapCommand(
					this._mxcPath,
		...
		const commandToRunInSandbox = this._getSandboxCommandWithPreservedCwd(
			command,
			cwd,
		);
	...
	async checkForSandboxingPrereqs(
		...
		precheckInputs?: ITerminalSandboxPrecheckInputs,
	): Promise<ITerminalSandboxPrerequisiteCheckResult> {
		if (!(await this._isSandboxConfiguredEnabled(precheckInputs))) {
			return {
				enabled: false,
		...
		const sandboxConfigPath = await this.getSandboxConfigPath(
			forceRefresh,
			precheckInputs,
		);
		...
		if (!(await this._checkSandboxDependencies(forceRefresh))) {
			...
			if (
				...
				this._sandboxDependencyStatus?.bubblewrapInstalled &&
				!this._sandboxDependencyStatus.bubblewrapUsable
			) {
				return {
					enabled: true,
					sandboxConfigPath,
	...
	async getMissingSandboxDependencies(): Promise<string[]> {
		...
		if (
			...
			!this._sandboxDependencyStatus.bubblewrapInstalled ||
			!this._sandboxDependencyStatus.socatInstalled
		) {
			this._sandboxDependencyStatus =
				await this._host.checkSandboxDependencies();
		}
	...
	async cleanupTempDir(): Promise<void> {
		...
		try {
			...
			this._logService.warn(
				"TerminalSandboxEngine: Failed to delete sandbox temp dir",
				error,
			);
		}
	}
	...
	private async _checkSandboxDependencies(
		forceRefresh = false,
	): Promise<boolean> {
		const os = await this.getOS();
		if (os === OperatingSystem.Windows) {
		...
		if (status && !status.bubblewrapInstalled) {
			this._logService.warn(
				"TerminalSandboxEngine: bubblewrap (bwrap) is not installed",
			);
		...
		return status
			? status.bubblewrapInstalled &&
					status.bubblewrapUsable &&
					status.socatInstalled
			: true;
	}

	private _getBubblewrapRemediations():
		| readonly TerminalSandboxPreCheckRemediation[]
		| undefined {
	...
	private _getBlockedDomains(command: string): {
		...
		for (const domain of domains) {
			if (
				deniedDomains.some((pattern) => matchesDomainPattern(domain, pattern))
			) {
			...
			if (
				!allowedDomains.some((pattern) => matchesDomainPattern(domain, pattern))
			) {
				blockedDomains.add(domain);
			}
		}
	...
	private _extractDomains(command: string): string[] {
		...
		while (
			(match = TerminalSandboxEngine._sshRemoteRegex.exec(command)) !== null
		) {
	...
	private _normalizeCommandKeywords(
		...
		return [
			...new Set(commandKeywords.map((keyword) => keyword.toLowerCase())),
		].sort();
	}
	...
	private _normalizeCommandDetails(
		...
		return result.sort(
			(a, b) =>
				...
				a.args.join("\0").localeCompare(b.args.join("\0")),
		);
	}
	...
	private _areStringArraysEqual(
		...
		return (
			a.length === b.length && a.every((keyword, index) => keyword === b[index])
		);
	}
	...
	private async _isSandboxConfiguredEnabled(
		...
		if (this._os === OperatingSystem.Windows) {
			return (
				this._getSandboxConfiguredWindowsEnabledValue() ===
				AgentSandboxEnabledValue.AllowNetwork
			);
		}
		const value = this._getSandboxConfiguredEnabledValue();
		return (
			value === AgentSandboxEnabledValue.On ||
			value === AgentSandboxEnabledValue.AllowNetwork
		);
	}
	...
	private async _resolveRuntimeInfo(): Promise<void> {
		...
		this._mxcPath = this._windowsMxcRuntime.getExecutablePath(
			this._appRoot,
			runtimeInfo.arch,
		);
	}
	...
	private async _createSandboxConfig(): Promise<string | undefined> {
		...
		const linuxFileSystemSetting =
			this._os === OperatingSystem.Linux
				? (this._getSettingValue<ITerminalSandboxFileSystemSetting>(
						AgentSandboxSettingId.AgentSandboxLinuxFileSystem,
					) ?? {})
				: {};
		const macFileSystemSetting =
			this._os === OperatingSystem.Macintosh
				? (this._getSettingValue<ITerminalSandboxFileSystemSetting>(
						AgentSandboxSettingId.AgentSandboxMacFileSystem,
					) ?? {})
				: {};
		const windowsFileSystemSetting =
			this._os === OperatingSystem.Windows
				? (this._getSettingValue<ITerminalSandboxFileSystemSetting>(
						AgentSandboxSettingId.AgentSandboxWindowsFileSystem,
					) ?? {})
				: {};
		const windowsSchemaVersion =
			this._os === OperatingSystem.Windows
				? this._getSettingValue<string>(
		...
		const runtimeSetting =
			this._getSettingValue<Record<string, unknown>>(
				AgentSandboxSettingId.AgentSandboxAdvancedRuntime,
			) ?? {};
		const commandRuntimeSetting =
			getTerminalSandboxRuntimeConfigurationForCommands(
				this._os,
				this._commandAllowListCommandDetails,
			);
		const commandRuntimeAllowReadPaths = this._getCommandRuntimeFileSystemPaths(
			commandRuntimeSetting,
			"allowRead",
		);
		const commandRuntimeAllowWritePaths =
			this._getCommandRuntimeFileSystemPaths(
				commandRuntimeSetting,
				"allowWrite",
			);
		const configFileUri = URI.joinPath(
			this._tempDir,
			`vscode-sandbox-settings-${this._sandboxSettingsId}.json`,
		);
		...
		if (this._os === OperatingSystem.Windows) {
			...
			allowWritePaths = await this._resolveFileSystemPaths([
				...this._updateAllowWritePathsWithWorkspaceFolders(
					windowsFileSystemSetting.allowWrite,
				),
			...
			denyReadPaths = await this._resolveFileSystemPaths(
				windowsFileSystemSetting.denyRead ?? [],
			);
			this._windowsMxcEnvironment = env;
		} else if (this._os === OperatingSystem.Macintosh) {
			allowWritePaths = await this._resolveFileSystemPaths(
				this._updateAllowWritePathsWithWorkspaceFolders(
					macFileSystemSetting.allowWrite,
					commandRuntimeAllowWritePaths,
				),
			);
			allowReadPaths = await this._resolveFileSystemPaths(
				await this._updateAllowReadPathsWithAllowWrite(
			...
			denyReadPaths = await this._resolveFileSystemPaths(
				this._updateDenyReadPathsWithHome(macFileSystemSetting.denyRead),
			);
			denyWritePaths = macFileSystemSetting.denyWrite
				? await this._resolveFileSystemPaths(macFileSystemSetting.denyWrite)
				: undefined;
		} else if (this._os === OperatingSystem.Linux) {
			allowWritePaths = await this._resolveFileSystemPaths(
				this._updateAllowWritePathsWithWorkspaceFolders(
					linuxFileSystemSetting.allowWrite,
					commandRuntimeAllowWritePaths,
				),
			);
			allowReadPaths = await this._resolveFileSystemPaths(
			...
			denyWritePaths = await this._resolveFileSystemPaths(
				linuxFileSystemSetting.denyWrite,
			);
		}
		const sandboxSettings =
			this._os === OperatingSystem.Windows
				? await this._windowsMxcRuntime.createConfig(
						{
				...
				: {
						network: allowNetwork
							? { allowedDomains: [], deniedDomains: [], enabled: false }
		...
		if (this._os !== OperatingSystem.Windows) {
			this._mergeAdditionalSandboxConfigProperties(
				sandboxSettings as Record<string, unknown>,
				allowNetwork
					? this._withoutNetworkRuntimeSetting(runtimeSetting)
					: runtimeSetting,
			);
			this._mergeAdditionalSandboxConfigProperties(
				sandboxSettings as Record<string, unknown>,
	...
	private readonly _buildSandboxPayload = (
		commandLine: string,
		policy: IWindowsMxcSandboxPolicy,
		workingDirectory?: string,
		containerName?: string,
		...
		return this._host.buildWindowsMxcSandboxPayload(
			commandLine,
			policy,
			workingDirectory,
			containerName,
	...
	private _getCommandRuntimeFileSystemPaths(
		runtimeSetting: Record<string, unknown>,
	...
	private _mergeAdditionalSandboxConfigProperties(
		...
		for (const [key, value] of Object.entries(additional)) {
			...
			if (
				this._isObjectForSandboxConfigMerge(existingValue) &&
				this._isObjectForSandboxConfigMerge(value)
			) {
	...
	private async _getWindowsMxcFilesystemPolicy(): Promise<IWindowsMxcFilesystemPolicy> {
		if (!this._windowsMxcFilesystemPolicy) {
			this._windowsMxcFilesystemPolicy =
				(await this._host.getWindowsMxcFilesystemPolicy()) ?? {
					readonlyPaths: [],
					readwritePaths: [],
				};
		}
	...
	private async _getWindowsMxcEnvironment(): Promise<string[]> {
		if (!this._windowsMxcEnvironment) {
			this._windowsMxcEnvironment =
				(await this._host.getWindowsMxcEnvironment()) ?? [];
		}
	...
	private _getUriPath(uri: URI): string {
		return this._os === OperatingSystem.Windows
			? this._windowsMxcRuntime.toWindowsPath(uri)
			: uri.path;
	}
	...
	private _updateAllowWritePathsWithWorkspaceFolders(
		configuredAllowWrite: string[] | undefined,
		commandRuntimeAllowWrite: string[] = [],
	): string[] {
		const writeRootPaths = this._host
			.getWriteRoots()
			.map((folder) => this._getUriPath(folder));
		return [
			...new Set([
				...writeRootPaths,
	...
	private _updateDenyReadPathsWithHome(
		...
		const userHome = this._userHome
			? this._getUriPath(this._userHome)
			: undefined;
		return [
			...new Set([
				...(configuredDenyRead ?? []),
	...
	private async _updateAllowReadPathsWithAllowWrite(
		configuredAllowRead: string[] | undefined,
		allowWrite: string[],
		commandRuntimeAllowRead: string[] = [],
	): Promise<string[]> {
		return [
			...new Set([
				...(configuredAllowRead ?? []),
	...
	private async _resolveFileSystemPaths(
		paths: string[] | undefined,
	): Promise<string[]> {
		const resolvedPaths = await Promise.all(
			(paths ?? []).map((path) => this._resolveFileSystemPath(path)),
		);
	...
	private async _resolveFileSystemPath(path: string): Promise<string> {
		const expandedPath =
			this._os === OperatingSystem.Linux ? this._expandHomePath(path) : path;
		if (!this._isAbsoluteFileSystemPath(expandedPath)) {
			return expandedPath;
		}

		try {
			const realpath = await this._fileService.realpath(
				this._toFileSystemResource(expandedPath),
			);
			const resolvedPath = realpath ? this._getUriPath(realpath) : undefined;
			return resolvedPath && resolvedPath !== expandedPath
				? resolvedPath
				: expandedPath;
		} catch {
			return expandedPath;
	...
	private _isAbsoluteFileSystemPath(path: string): boolean {
		return (this._os === OperatingSystem.Windows ? win32 : posix).isAbsolute(
			path,
		);
	}
	...
	private _toFileSystemResource(path: string): URI {
		...
		return (
			this._userHome?.with({ path }) ??
			this._tempDir?.with({ path }) ??
			this._host.getWriteRoots()[0]?.with({ path }) ??
			URI.file(path)
		);
	}
	...
	private _getSandboxRuntimeReadPaths(): string[] {
		...
		if (this._os === OperatingSystem.Windows) {
			return this._windowsMxcRuntime.getRuntimeReadPaths(
				this._appRoot,
				this._mxcPath,
			);
		}
	...
	private _getSandboxConfiguredEnabledValue(): AgentSandboxEnabledValue {
		return (
			this._getSettingValue<AgentSandboxEnabledValue>(
	...
	private _isSandboxAllowNetworkConfigured(): boolean {
		...
		return (
			this._getSandboxConfiguredEnabledValue() ===
			AgentSandboxEnabledValue.AllowNetwork
		);
	}
