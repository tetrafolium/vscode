...
suite("WebPageLoader", () => {
	...
	function createWebPageLoader(
		uri: URI,
		options?: IWebContentExtractorOptions,
		isTrustedDomain?: (uri: URI) => boolean,
		...
		const agentNetworkFilterService: IAgentNetworkFilterService = {
			...
			formatError: (u) =>
				`Access to ${u.authority} is blocked by network domain policy.`,
		};
	...
	function setupDebuggerMock(options: DebuggerMockOptions = {}): void {
		...
		window.webContents.debugger.sendCommand.callsFake(
			(command: string, params?: { frameId?: string }) => {
				switch (command) {
					case "Network.enable":
	...
	test("successful page load returns ok status with content", () =>
	...
	test("ERR_ABORTED is ignored and content extraction continues", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("ERR_BLOCKED_BY_CLIENT is ignored and content extraction continues", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("redirect to same authority is not treated as redirect", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("redirect is followed when followRedirects option is true", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
			const redirectUrl = "https://other-domain.com/redirected";
	...
	test("redirect from www to non-www same domain is allowed", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
	...
	test("redirect from non-www to www same domain is allowed", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("redirect to trusted domain is allowed", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
			...
			const loader = createWebPageLoader(
				...
				(uri) =>
					uri.authority === "trusted-domain.com" ||
					uri.authority === "another-trusted.com",
			);
			setupDebuggerMock();
	...
	test("post-load navigation to different domain is blocked silently and content is extracted", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("initial same-domain navigation is allowed but later cross-domain navigation is blocked", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("redirect to wildcard subdomain trusted domain is allowed", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
			...
			const loader = createWebPageLoader(
				...
				{ followRedirects: false },
				(uri) =>
					uri.authority.endsWith(".trusted-domain.com") ||
					uri.authority === "trusted-domain.com",
			);
			setupDebuggerMock();
	...
	test("navigation to domain blocked by isDomainAllowed returns error", async () => {
		...
		const loader = createWebPageLoader(
			...
			undefined,
			(u) => u.authority !== "blocked-domain.com",
		);
	...
	test("navigation to allowed domain is not blocked by isDomainAllowed", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("HTTP error status code returns error with content", async () => {
		...
		const mockEvent: MockElectronEvent = {};
		window.webContents.debugger.emit(
			"message",
			mockEvent,
	...
	test("tracks network requests and waits for completion", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("handles network request failures gracefully", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("extracts content from accessibility tree", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
	...
	test("falls back to DOM extraction when accessibility tree yields insufficient content", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
			...
			const domContent =
				"This is much longer content extracted from the DOM that exceeds the minimum content length requirement and should be used instead of the short accessibility tree content.";
	...
	test("returns error when accessibility tree extraction hangs", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
			const loader = createWebPageLoader(uri);
			...
			if (result.status === "error") {
				assert.ok(
					result.error.includes("Failed to extract meaningful content"),
				);
			}
	...
	test("returns error when both accessibility tree and DOM extraction yield no content", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
	...
	test("extracts content from multiple frames including iframes", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const nestedIframeNodes: AXNode[] = [
				...
				},
				{
					...
					name: {
						type: "string",
						value: "Deeply nested iframe content that should also be extracted",
					},
				},
			];
			...
			const frameTree = {
				frame: {
					id: "main-frame",
					url: "https://example.com/page-with-iframes",
				},
				childFrames: [
					{
						frame: {
						...
						childFrames: [
							{
								frame: {
									id: "nested-iframe",
									url: "https://example.com/nested-content",
								},
								childFrames: [],
							},
						],
			...
			if (result.status === "ok") {
				...
				assert.ok(
					result.result.includes("Main Page Content"),
					"Should include main frame content",
				);
				// Verify content from iframe is included
				assert.ok(
					result.result.includes("Iframe Documentation Content"),
					"Should include iframe content",
				);
				// Verify content from nested iframe is included
				assert.ok(
					result.result.includes("Deeply nested iframe content"),
					"Should include nested iframe content",
				);
			}

			// Verify Accessibility.getFullAXTree was called for each frame
			const getFullAXTreeCalls = window.webContents.debugger.sendCommand
				.getCalls()
				.filter((call) => call.args[0] === "Accessibility.getFullAXTree");
			assert.strictEqual(
				getFullAXTreeCalls.length,
				3,
				"Should call getFullAXTree for all 3 frames",
			);
		}));
	...
	test("onBeforeSendHeaders adds browser headers for navigation", () => {
		...
		const callback =
			window.webContents.session.webRequest.onBeforeSendHeaders.getCall(0)
				.args[0];
	...
	test("onHeadersReceived replaces Content-Disposition attachment with inline for text content", () => {
		...
		const listener =
			window.webContents.session.webRequest.onHeadersReceived.getCall(0)
				.args[0];
		...
		for (const contentType of [
			...
			let response:
				| { responseHeaders?: Record<string, string[]>; cancel?: boolean }
				| undefined;
			...
			assert.deepStrictEqual(
				...
				["inline"],
				`Expected inline for ${contentType}`,
			);
			assert.strictEqual(
				response!.cancel,
				false,
				`Should not cancel for ${contentType}`,
			);
		}
	});
	...
	test("onHeadersReceived cancels Content-Disposition attachment for binary content", () => {
		...
		for (const contentType of [
			...
			assert.strictEqual(
				response!.cancel,
				true,
				`Expected cancel for ${contentType}`,
			);
		}
	});
	...
	test("will-download handler cancels download and returns error", async () => {
		...
		const willDownloadCall = window.webContents.session.on
			.getCalls()
	...
	test("disposes resources after load completes", () =>
		runWithFakedTimers({ useFakeTimers: true }, async () => {
			const uri = URI.parse("https://example.com/page");
