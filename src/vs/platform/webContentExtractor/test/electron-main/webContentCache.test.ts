...
suite("WebContentCache", () => {
	...
	test("returns cached result for previously added URI", () => {
		const uri = URI.parse("https://example.com/page");
		const extractResult: WebContentExtractResult = {
			status: "ok",
			result: "Test content",
			title: "Test Title",
		};
	...
	test("different options produce different cache entries", () => {
		const uri = URI.parse("https://example.com/page");
		const resultWithRedirects: WebContentExtractResult = {
			status: "ok",
			result: "With redirects",
			title: "Redirects Title",
		};
		const resultWithoutRedirects: WebContentExtractResult = {
			status: "ok",
			result: "Without redirects",
		...
		assert.deepStrictEqual(
			cache.tryGet(uri, { followRedirects: true }),
			resultWithRedirects,
		);
		assert.deepStrictEqual(
			cache.tryGet(uri, { followRedirects: false }),
			resultWithoutRedirects,
		);
	});
	...
	test("redirect results use success cache duration", () => {
		...
		const extractResult: WebContentExtractResult = {
			status: "redirect",
			toURI: URI.parse("https://example.com/new"),
		};
	...
	test("adding same URI overwrites previous entry", () => {
		const uri = URI.parse("https://example.com/page");
		const firstResult: WebContentExtractResult = {
			status: "ok",
			result: "First content",
			title: "First Title",
		};
		const secondResult: WebContentExtractResult = {
			status: "ok",
	...
	test("different hosts produce different cache entries", () => {
		...
		const result1: WebContentExtractResult = {
			status: "ok",
			result: "Example content",
			title: "Example Title",
		};
		const result2: WebContentExtractResult = {
			status: "ok",
	...
	test("different query strings produce different cache entries", () => {
		...
		const result1: WebContentExtractResult = {
			status: "ok",
			result: "Query 1 content",
			title: "Query 1 Title",
		};
		const result2: WebContentExtractResult = {
			status: "ok",
			result: "Query 2 content",
			title: "Query 2 Title",
		};
