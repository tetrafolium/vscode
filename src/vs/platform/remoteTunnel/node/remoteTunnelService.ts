...
type RemoteTunnelEnablementClassification = {
	...
	tunnelName?: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The name of the tunnel being enabled or disabled";
	};
};
...
type RemoteTunnelConnectedClassification = {
	...
	tunnelName: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The name of the connected tunnel";
	};
	isAttached: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the connection is attached to an existing tunnel process";
	};
};
	...
	implements IRemoteTunnelService
{
	...
	private readonly _onDidTokenFailedEmitter = this._register(
		new Emitter<IRemoteTunnelSession | undefined>(),
	);
	public readonly onDidTokenFailed = this._onDidTokenFailedEmitter.event;

	private readonly _onDidChangeTunnelStatusEmitter = this._register(
		new Emitter<TunnelStatus>(),
	);
	public readonly onDidChangeTunnelStatus =
		this._onDidChangeTunnelStatusEmitter.event;

	private readonly _onDidChangeModeEmitter = this._register(
		new Emitter<TunnelMode>(),
	);
	...
	private async updateTunnelProcess(): Promise<void> {
		...
		if (hostName) {
			...
			this.setTunnelStatus(
				TunnelStates.connecting(
					localize("remoteTunnelService.openTunnel", "Opening tunnel"),
				),
			);
		}
		const args = [
		...
		const serveCommand = this.runCodeTunnelCommand(
			...
			(message: string, isErr: boolean) => {
				...
				if (m) {
					...
					this.telemetryService.publicLog2<
						RemoteTunnelConnectedEvent,
						RemoteTunnelConnectedClassification
	...
	private _getTunnelName(): string | undefined {
		let name =
			this.configurationService.getValue<string>(CONFIGURATION_KEY_HOST_NAME) ||
			hostname();
		name = name
			.replace(/^-+/g, "")
			.replace(/[^\w-]/g, "")
