...
suite("FileUserDataProvider", () => {
	...
	setup(async () => {
		...
		disposables.add(
			testObject.registerProvider(Schemas.vscodeUserData, fileUserDataProvider),
		);
	});
	...
	test("file added change event", (done) => {
		...
		disposables.add(
			testObject.onDidChangeFile((e) => {
				if (
					isEqual(e[0].resource, expected) &&
					e[0].type === FileChangeType.ADDED
				) {
					done();
				}
			}),
	...
	test("file updated change event", (done) => {
		...
		disposables.add(
			testObject.onDidChangeFile((e) => {
				if (
					isEqual(e[0].resource, expected) &&
					e[0].type === FileChangeType.UPDATED
				) {
					done();
				}
			}),
	...
	test("file deleted change event", (done) => {
		...
		disposables.add(
			testObject.onDidChangeFile((e) => {
				if (
					isEqual(e[0].resource, expected) &&
					e[0].type === FileChangeType.DELETED
				) {
					done();
				}
			}),
