...
export class LocalGitService implements ILocalGitService {
	...
	private _exec(
		...
		return new Promise((resolve, reject) => {
			this._logService.trace(
				`[LocalGitService] git ${args.join(" ")}${cwd ? ` (cwd: ${cwd})` : ""}`,
			);
				...
				(err, stdout, stderr) => {
					...
					if (err) {
						this._logService.error(
							`[LocalGitService] git ${args[0]} failed:`,
							err.message,
							stderr,
						);
						reject(err);
	...
	async pull(operationId: string, repoPath: string): Promise<boolean> {
		const before = (
			await this._exec(operationId, ["rev-parse", "HEAD"], repoPath)
		).trim();
	...
	async revParse(repoPath: string, ref: string): Promise<string> {
		return (
			await this._exec(generateUuid(), ["rev-parse", ref], repoPath)
		).trim();
	}
