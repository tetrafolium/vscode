...
namespace Setting {
	export const DETECT_COLOR_SCHEME = new Setting<boolean>(
		"window.autoDetectColorScheme",
		false,
	);
	export const DETECT_HC = new Setting<boolean>(
		"window.autoDetectHighContrast",
		true,
	);
	export const SYSTEM_COLOR_THEME = new Setting<
		"default" | "auto" | "light" | "dark"
	>("window.systemColorTheme", "default");
	export const AUXILIARYBAR_DEFAULT_VISIBILITY = new Setting<
		| "hidden"
		| "visibleInWorkspace"
		| "visible"
	...
	export const STARTUP_EDITOR = new Setting<
		| "none"
		| "welcomePage"
		| "readme"
		| "newUntitledFile"
...
export class ThemeMainService extends Disposable implements IThemeMainService {
		...
		if (!isLinux) {
			this._register(
				this.configurationService.onDidChangeConfiguration((e) => {
					if (
						e.affectsConfiguration(Setting.SYSTEM_COLOR_THEME.key) ||
						e.affectsConfiguration(Setting.DETECT_COLOR_SCHEME.key)
					) {
	...
	private logThemeSettings(): void {
		if (this.logService.getLevel() >= LogLevel.Debug) {
			const logSetting = (setting: Setting<string | boolean>) =>
				`${setting.key}=${setting.getValue(this.configurationService)}`;
			this.logService.debug(
				`[theme main service] ${logSetting(Setting.DETECT_COLOR_SCHEME)}, ${logSetting(Setting.DETECT_HC)}, ${logSetting(Setting.SYSTEM_COLOR_THEME)}`,
			);

			const logProperty = (property: keyof Electron.NativeTheme) =>
				`${String(property)}=${electron.nativeTheme[property]}`;
			this.logService.debug(
				`[theme main service] electron.nativeTheme: ${logProperty("themeSource")}, ${logProperty("shouldUseDarkColors")}, ${logProperty("shouldUseHighContrastColors")}, ${logProperty("shouldUseInvertedColorScheme")}, ${logProperty("shouldUseDarkColorsForSystemIntegratedUI")}	`,
			);
			this.logService.debug(
				`[theme main service] New color scheme: ${JSON.stringify(this.getColorScheme())}`,
			);
		}
	}
	...
	getPreferredBaseTheme(): ThemeTypeSelector | undefined {
		...
		if (
			Setting.DETECT_HC.getValue(this.configurationService) &&
	...
	private doUpdateWindowSplashOverride(
		...
		const currentWidth =
			part === "sideBar"
				...
				: splash.layoutInfo?.auxiliaryBarWidth;
		const overrideWidth =
			part === "sideBar"
	...
	private doGetWindowSplash(
		...
		const auxiliaryBarDefaultVisibility =
			Setting.AUXILIARYBAR_DEFAULT_VISIBILITY.getValue(
				this.configurationService,
			);
		const startupEditor = Setting.STARTUP_EDITOR.getValue(
			this.configurationService,
		);
		...
		if (workspace) {
			...
			if (auxiliaryBarVisible === true) {
				auxiliaryBarWidth =
					...
					partSplash.layoutInfo.auxiliaryBarWidth ||
					ThemeMainService.DEFAULT_BAR_WIDTH;
				...
				if (
					...
					(auxiliaryBarDefaultVisibility === "visible" ||
						auxiliaryBarDefaultVisibility === "visibleInWorkspace")
				) {
					auxiliaryBarWidth =
						override.layoutInfo.auxiliaryBarWidth ||
						partSplash.layoutInfo.auxiliaryBarWidth ||
					...
					startupEditor !== "agentSessionsWelcomePage" &&
					(auxiliaryBarDefaultVisibility === "maximized" ||
						auxiliaryBarDefaultVisibility === "maximizedInWorkspace")
				) {
