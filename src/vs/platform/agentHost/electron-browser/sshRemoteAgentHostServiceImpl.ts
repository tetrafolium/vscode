...
export const ISSHRelayClientFactory = createDecorator<ISSHRelayClientFactory>(
...
export interface ISSHRelayClientFactory {
	...
	createClient(
		mainService: ISSHRemoteAgentHostMainService,
		connectionId: string,
		address: string,
 ...
 */
export class SSHRemoteAgentHostService
	extends Disposable
	implements ISSHRemoteAgentHostService
{
	declare readonly _serviceBrand: undefined;
	...
	constructor(
		...
		this.onDidReportConnectProgress =
		...
		this._register(
			this._mainService.onDidCloseConnection((connectionId) => {
				this._logService.info(
					`[SSHRemoteAgentHost] onDidCloseConnection: connectionId=${connectionId}`,
				);
				const handle = this._connections.get(connectionId);
				if (handle) {
					this._logService.info(
						`[SSHRemoteAgentHost] onDidCloseConnection: found handle for ${connectionId}, cleaning up`,
					);
					...
					this._logService.info(
						`[SSHRemoteAgentHost] onDidCloseConnection: notifying protocol client for ${handle.localAddress}`,
					);
					this._remoteAgentHostService.notifyConnectionClosed(
						handle.localAddress,
					);
				} else {
					this._logService.info(
						`[SSHRemoteAgentHost] onDidCloseConnection: no renderer-side handle for ${connectionId} (already cleaned up?)`,
					);
				}
			}),
		);
		...
		this._register(
			this._mainService.onDidRequestKeyboardInteractive((request) => {
	...
	async connect(config: ISSHAgentHostConfig): Promise<ISSHAgentHostConnection> {
		...
		this._logService.trace(
			`[SSHRemoteAgentHost] SSH tunnel established, connectionId=${result.connectionId}`,
		);
	...
	async reconnect(
		...
		const result = await this._mainService.reconnect(
			sshConfigHost,
			name,
			commandOverride,
			agentForward,
	...
	private async _setupConnection(
		result: ISSHConnectResult,
	): Promise<ISSHAgentHostConnection> {
		const existing = this._connections.get(result.connectionId);
		if (existing) {
			this._logService.trace(
				"[SSHRemoteAgentHost] Returning existing connection handle",
			);
			return existing;
		}
		...
		try {
			...
			this._logService.trace(
				"[SSHRemoteAgentHost] Protocol handshake completed",
			);
			...
			await this._remoteAgentHostService.addManagedConnection(
				...
				protocolClient,
				this._createTransportDisposable(result.connectionId, handle),
			);

			return handle;
		} catch (err) {
			this._logService.error(
				"[SSHRemoteAgentHost] Connection setup failed",
				err,
			);
			if (
				registeredHandle &&
				this._connections.get(result.connectionId) === handle
			) {
	 ...
	 */
	private _createTransportDisposable(
		connectionId: string,
		handle: SSHAgentHostConnectionHandle,
	): IDisposable {
		return toDisposable(() => {
	...
	private _createRelayClient(result: {
		connectionId: string;
		address: string;
	}): RemoteAgentHostProtocolClient {
		return this._relayClientFactory.createClient(
			this._mainService,
			result.connectionId,
			result.address,
		);
	 ...
	 */
	private async _handleKeyboardInteractiveRequest(
		request: ISSHKeyboardInteractiveRequest,
	): Promise<void> {
		this._logService.info(
			`[SSHRemoteAgentHost] Keyboard-interactive prompt for ${request.displayHost} (${request.prompts.length} prompt(s))`,
		);
		...
		const cancelListener = this._mainService.onDidCancelKeyboardInteractive(
			(requestId) => {
		...
		try {
			if (request.prompts.length === 0) {
				await this._mainService.respondKeyboardInteractive(
					request.requestId,
					[],
				);
				return;
			...
			this._logService.error(
				"[SSHRemoteAgentHost] Failed handling keyboard-interactive prompt",
				err,
			);
 ...
 */
class SSHAgentHostConnectionHandle
	extends Disposable
	implements ISSHAgentHostConnection
{
