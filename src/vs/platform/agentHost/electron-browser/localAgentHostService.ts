 ...
 */
export class LocalAgentHostServiceClient
	extends Disposable
	implements IAgentHostService
{
	...
	private readonly _authenticationPending: ISettableObservable<boolean> =
		observableValue("authenticationPending", true);
	...
	constructor(
		...
		const rawProxy = ProxyChannel.toService<IAgentService>(
			getDelayedChannel(
				this._clientEventually.p.then((client) =>
					client.getChannel(AgentHostIpcChannels.AgentHost),
				),
			),
		);
		...
		this._ahpLogger = this._configurationService.getValue<boolean>(
			AgentHostAhpJsonlLoggingSettingId,
		)
			? this._register(
					instantiationService.createInstance(AhpJsonlLogger, {
			...
			: undefined;
		this._proxy = this._ahpLogger
			? wrapAgentServiceWithAhpLogging(rawProxy, this._ahpLogger)
			: rawProxy;

		this._connectionTracker = ProxyChannel.toService<IConnectionTrackerService>(
			getDelayedChannel(
				this._clientEventually.p.then((client) =>
					client.getChannel(AgentHostIpcChannels.ConnectionTracker),
				),
			),
		);
	...
	private async _connect(): Promise<void> {
		this._logService.info(
			"[AgentHost:renderer] Acquiring MessagePort to agent host...",
		);
		const port = await acquirePort(
			"vscode:createAgentHostMessageChannel",
			"vscode:createAgentHostMessageChannelResult",
		);
		this._logService.info(
			"[AgentHost:renderer] MessagePort acquired, creating client...",
		);
		...
		client.registerChannel(
			AGENT_HOST_CLIENT_RESOURCE_CHANNEL,
			new AgentHostClientResourceChannel(this._fileService, this._ahpLogger),
		);
		...
		store.add(
			this._proxy.onDidNotification((e) => {
				if (this._ahpLogger) {
					const frame = {
						jsonrpc: "2.0" as const,
						method: "notification",
						params: { notification: e },
					};
					this._ahpLogger.log(frame, "s2c");
				}
		...
		this._logService.info(
			"[AgentHost:renderer] Direct MessagePort connection established",
		);
		this._onAgentHostStart.fire();

		// Subscribe to root state
		this.subscribe(URI.parse(ROOT_STATE_URI))
			.then((snapshot) => {
			...
			.catch((err) => {
				this._logService.error(
					"[AgentHost:renderer] Failed to subscribe to root state",
					err,
				);
			});
	}
	...
	getCompletionTriggerCharacters(): Promise<readonly string[]> {
		return (this._completionTriggerCharactersOnce ??=
			this._proxy.getCompletionTriggerCharacters());
	}
