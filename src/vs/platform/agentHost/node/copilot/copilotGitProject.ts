...
export async function resolveGitProject(
	...
	const uri =
		(await gitService.getWorktreeRoots(workingDirectory))[0] ??
...
export function projectFromRepository(
	repository: string,
): IAgentSessionProjectInfo | undefined {
	const uri = repository.includes("://")
		? URI.parse(repository)
		: URI.parse(`https://github.com/${repository}`);
	const rawDisplayName =
		basename(uri.path) ||
		repository.split("/").filter(Boolean).pop() ||
		repository;
	const displayName = rawDisplayName.endsWith(".git")
		? rawDisplayName.slice(0, -".git".length)
		: rawDisplayName;
	return { uri, displayName };
}

export async function projectFromCopilotContext(
