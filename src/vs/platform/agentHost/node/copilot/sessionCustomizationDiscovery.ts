...
function getSearchRoots(
	...
	userHome: URI,
): { workspace: ISearchRoot[]; user: ISearchRoot[] } {
	return {
		workspace: [
...
export class SessionCustomizationDiscovery extends Disposable {
	...
	constructor(
		...
		this._rootUris = [
			...workspace.map((root) =>
				joinPath(this._workingDirectory, ...root.path),
			),
	...
	private _scheduleRefresh(): void {
		this._refreshDelayer
			.trigger(() => {
				this.refresh();
				this._onDidChange.fire();
			})
			.catch(() => {
				/* delayer cancelled on dispose */
			});
	}
	...
	private async _scan(): Promise<readonly IDiscoveredDirectory[]> {
		...
		await Promise.all([
			...workspace.map((root) =>
				this._scanRoot(this._workingDirectory, root, seen, result),
			),
	...
	private async _scanRoot(
		...
		try {
			const recursive =
				root.type === DiscoveredType.Skill ||
				root.type === DiscoveredType.Instruction;
			this._watchers.add(
				this._fileService.watch(rootUri, { recursive, excludes: [] }),
			);
		} catch (err) {
		...
		if (root.type === DiscoveredType.Skill) {
			...
			for (const child of stat.children) {
				if (child.isFile) {
					...
					if (
						filename.endsWith(MARKDOWN_SUFFIX) &&
						filename !== README_FILENAME &&
						!seen.has(child.resource)
					) {
			...
			const findInstructions = async (
				...
				for (const child of stat.children ?? []) {
					if (child.isFile) {
						...
						child.isDirectory &&
						recursionLevel < MAX_INSTRUCTIONS_RECURSION_DEPTH
					) {
						let childStat: IFileStatWithMetadata | undefined = undefined;
						try {
							childStat = await this._fileService.resolve(child.resource, {
								resolveMetadata: true,
							});
						} catch {
			...
			this._logService.warn(
				`[SessionCustomizationDiscovery] Unrecognized root type '${root.type}' for root '${rootUri.toString()}'`,
			);
		}
	}
}
