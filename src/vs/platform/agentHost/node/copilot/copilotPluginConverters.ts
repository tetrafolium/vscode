 ...
 */
export async function toSdkCustomAgents(
	agents: readonly INamedPluginResource[],
 ...
 */
export function toAgentCustomizations(
	agents: readonly IParsedAgent[],
): AgentCustomization[] {
 ...
 */
export function toChildCustomizations(
	plugins: readonly IParsedPlugin[],
): ChildCustomization[] {
 ...
 */
export function toSdkSkillDirectories(
	skills: readonly INamedPluginResource[],
): string[] {
 ...
 */
export function toSdkInstructionDirectories(
	instructions: readonly INamedPluginResource[],
): string[] {
	return toSdkResourceDirectories(instructions);
}

function toSdkResourceDirectories(
	resources: readonly INamedPluginResource[],
): string[] {
	...
	return new Promise<string>((resolve, reject) => {
		...
		child.on("close", (code) => {
			if (code === 0) {
				...
				reject(
					new Error(
						`Hook command exited with code ${code}: ${stderr || stdout}`,
					),
				);
			}
		});
...
async function runHookCommands(
	commands: readonly IParsedHookCommand[] | undefined,
	input: unknown,
...
export function toSdkHooks(
	...
	if (promptCommands?.length) {
		hooks.onUserPromptSubmitted = async (
 ...
 */
export function parsedPluginsEqual(
	a: readonly IParsedPlugin[],
	b: readonly IParsedPlugin[],
): boolean {
