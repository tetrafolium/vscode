...
export class CopilotSessionWrapper extends Disposable {
	...
	private _onSessionInfo:
		| Event<SessionEventPayload<"session.info">>
		| undefined;
	get onSessionInfo(): Event<SessionEventPayload<"session.info">> {
		return (this._onSessionInfo ??= this._sdkEvent("session.info"));
	}

	private _onSessionModelChange:
		| Event<SessionEventPayload<"session.model_change">>
		| undefined;
	get onSessionModelChange(): Event<
		SessionEventPayload<"session.model_change">
	> {
		return (this._onSessionModelChange ??= this._sdkEvent(
			"session.model_change",
		));
	}

	private _onSessionHandoff:
		| Event<SessionEventPayload<"session.handoff">>
		| undefined;
	...
	private _onSessionTruncation:
		| Event<SessionEventPayload<"session.truncation">>
		| undefined;
	...
	private _onSessionSnapshotRewind:
		| Event<SessionEventPayload<"session.snapshot_rewind">>
		| undefined;
	get onSessionSnapshotRewind(): Event<
		SessionEventPayload<"session.snapshot_rewind">
	> {
		return (this._onSessionSnapshotRewind ??= this._sdkEvent(
			"session.snapshot_rewind",
		));
	}

	private _onSessionShutdown:
		| Event<SessionEventPayload<"session.shutdown">>
		| undefined;
	...
	private _onSessionUsageInfo:
		| Event<SessionEventPayload<"session.usage_info">>
		| undefined;
	...
	private _onSessionCompactionStart:
		| Event<SessionEventPayload<"session.compaction_start">>
		| undefined;
	get onSessionCompactionStart(): Event<
		SessionEventPayload<"session.compaction_start">
	> {
		return (this._onSessionCompactionStart ??= this._sdkEvent(
			"session.compaction_start",
		));
	}

	private _onSessionCompactionComplete:
		| Event<SessionEventPayload<"session.compaction_complete">>
		| undefined;
	get onSessionCompactionComplete(): Event<
		SessionEventPayload<"session.compaction_complete">
	> {
		return (this._onSessionCompactionComplete ??= this._sdkEvent(
			"session.compaction_complete",
		));
	}

	private _onUserMessage:
		| Event<SessionEventPayload<"user.message">>
		| undefined;
	get onUserMessage(): Event<SessionEventPayload<"user.message">> {
		return (this._onUserMessage ??= this._sdkEvent("user.message"));
	}

	private _onPendingMessagesModified:
		| Event<SessionEventPayload<"pending_messages.modified">>
		| undefined;
	get onPendingMessagesModified(): Event<
		SessionEventPayload<"pending_messages.modified">
	> {
		return (this._onPendingMessagesModified ??= this._sdkEvent(
			"pending_messages.modified",
		));
	}

	private _onTurnStart:
		| Event<SessionEventPayload<"assistant.turn_start">>
		| undefined;
	...
	private _onReasoningDelta:
		| Event<SessionEventPayload<"assistant.reasoning_delta">>
		| undefined;
	get onReasoningDelta(): Event<
		SessionEventPayload<"assistant.reasoning_delta">
	> {
		return (this._onReasoningDelta ??= this._sdkEvent(
			"assistant.reasoning_delta",
		));
	}

	private _onTurnEnd:
		| Event<SessionEventPayload<"assistant.turn_end">>
		| undefined;
	...
	private _onToolUserRequested:
		| Event<SessionEventPayload<"tool.user_requested">>
		| undefined;
	get onToolUserRequested(): Event<SessionEventPayload<"tool.user_requested">> {
		return (this._onToolUserRequested ??= this._sdkEvent(
			"tool.user_requested",
		));
	}

	private _onToolPartialResult:
		| Event<SessionEventPayload<"tool.execution_partial_result">>
		| undefined;
	get onToolPartialResult(): Event<
		SessionEventPayload<"tool.execution_partial_result">
	> {
		return (this._onToolPartialResult ??= this._sdkEvent(
			"tool.execution_partial_result",
		));
	}

	private _onToolProgress:
		| Event<SessionEventPayload<"tool.execution_progress">>
		| undefined;
	...
	private _onSkillInvoked:
		| Event<SessionEventPayload<"skill.invoked">>
		| undefined;
	get onSkillInvoked(): Event<SessionEventPayload<"skill.invoked">> {
		return (this._onSkillInvoked ??= this._sdkEvent("skill.invoked"));
	}

	private _onSubagentStarted:
		| Event<SessionEventPayload<"subagent.started">>
		| undefined;
	...
	private _onSubagentCompleted:
		| Event<SessionEventPayload<"subagent.completed">>
		| undefined;
	...
	private _onSubagentFailed:
		| Event<SessionEventPayload<"subagent.failed">>
		| undefined;
	...
	private _onSubagentSelected:
		| Event<SessionEventPayload<"subagent.selected">>
		| undefined;
	...
	private _onSystemMessage:
		| Event<SessionEventPayload<"system.message">>
		| undefined;
	get onSystemMessage(): Event<SessionEventPayload<"system.message">> {
		return (this._onSystemMessage ??= this._sdkEvent("system.message"));
	}

	private _onSystemNotification:
		| Event<SessionEventPayload<"system.notification">>
		| undefined;
	get onSystemNotification(): Event<
		SessionEventPayload<"system.notification">
	> {
		return (this._onSystemNotification ??= this._sdkEvent(
			"system.notification",
		));
	}

	private _onSessionModeChanged:
		| Event<SessionEventPayload<"session.mode_changed">>
		| undefined;
	get onSessionModeChanged(): Event<
		SessionEventPayload<"session.mode_changed">
	> {
		return (this._onSessionModeChanged ??= this._sdkEvent(
			"session.mode_changed",
		));
	}

	private _sdkEvent<K extends SessionEventType>(
		eventType: K,
	): Event<SessionEventPayload<K>> {
		const emitter = this._register(new Emitter<SessionEventPayload<K>>());
		const unsubscribe = this.session.on(
			eventType,
			(data: SessionEventPayload<K>) => emitter.fire(data),
		);
