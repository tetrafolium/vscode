...
export class TunnelAgentHostMainService
	extends Disposable
	implements ITunnelAgentHostMainService
{
			...
			for (const tunnelName of additionalTunnelNames) {
				try {
					...
					if (tunnel) {
						...
						if (
							info &&
							info.protocolVersion >= TUNNEL_MIN_PROTOCOL_VERSION &&
							!seen.has(info.tunnelId)
						) {
							results.push(info);
							seen.add(info.tunnelId);
						}
	...
	async connect(
		...
		const { TunnelRelayTunnelClient } =
			await import("@microsoft/dev-tunnels-connections");
	...
	private async _createManagementClient(
		token: string,
		authProvider: "github" | "microsoft",
	): Promise<TunnelManagementHttpClient> {
		const mgmt = await import("@microsoft/dev-tunnels-management");
		const authHeader =
			authProvider === "github" ? `github ${token}` : `Bearer ${token}`;
	...
	private _parseTunnelInfo(tunnel: Tunnel): ITunnelInfo | undefined {
		...
		const hostConnectionCount =
			typeof rawCount === "number" ? rawCount : (rawCount?.current ?? 0);
		return {
			tunnelId,
			clusterId,
			name,
	...
	private async _createWebSocketRelay(
		...
		return new Promise((resolve, reject) => {
			...
			const ws = new WS.WebSocket(url, {
				createConnection: (() =>
					portStream) as unknown as WebSocket.ClientOptions["createConnection"],
			});

			ws.on("open", () => {
				this._logService.info(
					`${LOG_PREFIX} WebSocket relay connected to agent host via tunnel`,
				);
				resolve({
					send: (data: string) => {
			...
			ws.on("close", (code: number, reason: Buffer) => {
				this._logService.info(
					`${LOG_PREFIX} WebSocket relay closed for connection ${connectionId}; code=${code}, reason=${reason?.toString() || "(empty)"}`,
				);
				const conn = this._connections.get(connectionId);
				if (conn) {
					conn.dispose();
