 ...
 * the main thread.
 */
export class NodeWorkerDiffComputeService
	extends Disposable
	implements IDiffComputeService
{
	...
	private readonly _pending = new Map<
		number,
		{ resolve: (value: IDiffCountResult) => void; reject: (err: Error) => void }
	>();
	...
	async computeDiffCounts(
		original: string,
		modified: string,
		timeoutMs: number = DEFAULT_DIFF_TIMEOUT_MS,
		...
		return new Promise<IDiffCountResult>((resolve, reject) => {
			...
			try {
				worker.postMessage({
					id,
					fn: "computeDiffCounts",
					args: [original, modified, timeoutMs],
				});
			} catch (err) {
	...
	private _ensureWorker(): Worker {
		...
		if (!this._worker) {
			const workerPath = FileAccess.asFileUri(
