...
import {
	...
	getMediaMime,
} from "../../../base/common/mime.js";
import { equals as objectEquals } from "../../../base/common/objects.js";
import { observableValue } from "../../../base/common/observable.js";
import {
	extname as resourcesExtname,
	isEqual,
	isEqualOrParent,
	joinPath,
...
import {
	...
	formatSessionChangesetDescription,
	parseChangesetUri,
...
} from "../common/state/protocol/actions.js";
import {
	ResponsePartKind,
	SessionStatus,
	ToolCallStatus,
...
} from "./agentConfigurationService.js";
import {
	AgentHostTerminalManager,
	type IAgentHostTerminalManager,
} from "./agentHostTerminalManager.js";
import {
	ISessionDbUriFields,
	parseSessionDbUri,
} from "./shared/fileEditTracker.js";
...
} from "./agentHostFileMonitorService.js";
import {
	IAgentHostCheckpointService,
	NULL_CHECKPOINT_SERVICE,
} from "../common/agentHostCheckpointService.js";
import {
	CHANGESET_DB_METADATA_KEYS,
	ChangesetSessionCoordinator,
} from "./agentHostChangesetCoordinator.js";
import {
...
export class AgentService extends Disposable implements IAgentService {
	...
	private readonly _onDidNotification = this._register(
	...
	get stateManager(): AgentHostStateManager {
		return this._stateManager;
	}

	/** Exposes the configuration service so agent providers can share root config plumbing. */
	get configurationService(): IAgentConfigurationService {
	 ...
	 */
	private readonly _pendingSessionGc = this._register(
		new DisposableResourceMap<IDisposable>(),
	);
	 ...
	 */
	private readonly _resourceWatches = this._register(
		new DisposableMap<string, IActiveResourceWatch>(),
	);

	/** Exposes the terminal manager for use by agent providers. */
	get terminalManager(): IAgentHostTerminalManager {
		return this._terminalManager;
	}

	/** Exposes the completions service for use by agent providers (e.g. to register agent-scoped completion item providers). */
	get completionsService(): IAgentHostCompletions {
	...
	constructor(
		...
		this._stateManager = this._register(
			new AgentHostStateManager(_logService, {
				changesetStateRetention: {
					...
					canEvict: (changeset) =>
						this._changesets ? this._isChangesetEvictable(changeset) : false,
				},
			}),
		);
		this._register(
			this._stateManager.onDidEmitEnvelope((e) => this._onDidAction.fire(e)),
		);
		this._register(
			this._stateManager.onDidEmitNotification((e) =>
		...
		const configurationService: IAgentConfigurationService = this._register(
			new AgentConfigurationService(
				this._stateManager,
				this._logService,
				this._rootConfigResource,
			),
		);
		this._configurationService = configurationService;
		const fileMonitorService =
			_fileMonitorService ??
			this._register(
				new AgentHostFileMonitorService(this._fileService, this._logService),
			);
		updateAgentHostTelemetryLevelFromConfig(
			this._telemetryService,
			this._stateManager.rootState.config?.values,
		);
		...
		this._changesets = this._register(
		...
		this._changesetCoordinator = this._register(
			instantiationService.createInstance(
				ChangesetSessionCoordinator,
				this._stateManager,
			),
		...
		this._register(
			this._stateManager.onDidChangeSessionActiveTurn((e) =>
				this._changesetCoordinator.onSessionTurnActiveChanged(
					e.session,
					e.active,
				),
			),
		...
		this._completions = this._register(
			instantiationService.createInstance(AgentHostCompletions),
		);
		// Built-in generic provider: completes files in the session's workspace folder.
		const workspaceFiles = this._register(
			instantiationService.createInstance(AgentHostWorkspaceFiles),
		);
		this._register(
			this._completions.registerProvider(
				new AgentHostFileCompletionProvider(this._stateManager, workspaceFiles),
			),
		);

		this._sideEffects = this._register(
			instantiationService.createInstance(
				AgentSideEffects,
				this._stateManager,
				{
					...
					onTurnComplete: (session) => {
						const workingDirStr =
							this._stateManager.getSessionState(session)?.summary
								.workingDirectory;
						this._attachGitState(
							URI.parse(session),
							workingDirStr ? URI.parse(workingDirStr) : undefined,
						);
					},
				},
			),
		...
		this._terminalManager = this._register(
			instantiationService.createInstance(
				AgentHostTerminalManager,
				this._stateManager,
			),
	...
	registerProvider(provider: IAgent): void {
		...
		this._providerSubscriptions.add(
			this._sideEffects.registerProgressListener(provider),
		);
		if (provider.onDidMaterializeSession) {
			this._providerSubscriptions.add(
				provider.onDidMaterializeSession((e) =>
					this._onDidMaterializeSession(e),
				),
			);
		}
	...
	private _registerSkillCompletionProvider(): void {
		...
		const provider = this._register(
			new AgentHostSkillCompletionProvider(
	...
	async authenticate(params: AuthenticateParams): Promise<AuthenticateResult> {
		this._logService.trace(
			`[AgentService] authenticate called: resource=${params.resource}`,
		);
		...
		const matching = [...this._providers.values()].filter((p) =>
			p.getProtectedResources().some((r) => r.resource === params.resource),
		);
	...
	async listSessions(): Promise<IAgentSessionMetadata[]> {
			...
			flat.map(async (s) => {
				try {
					...
					try {
						...
						const changesetKeys =
							this._changesetCoordinator.getListMetadataKeys(sessionStr);
						...
						return this._changesetCoordinator.decorateListEntry(
							updated,
					...
					this._logService.warn(
						`[AgentService] Failed to read session metadata overlay for ${s.session}`,
						e,
					);
				}
				return s;
		...
		const withStatus = result.map((s) => {
			const liveState = this._stateManager.getSessionState(
				s.session.toString(),
			);
			if (liveState) {
				return {
		...
		this._logService.trace(
			`[AgentService] listSessions returned ${withStatus.length} sessions`,
		);
		return withStatus;
	}
	...
	async createSession(config?: IAgentCreateSessionConfig): Promise<URI> {
		...
		if (!provider) {
			throw new Error(
				`No agent provider registered for: ${providerId ?? "(none)"}`,
			);
		}
		...
		if (config?.fork) {
			const sourceState = this._stateManager.getSessionState(
				config.fork.session.toString(),
			);
			const sourceTurns =
				sourceState?.turns.slice(0, config.fork.turnIndex + 1) ?? [];
		...
		this._logService.trace(
			`[AgentService] createSession: initializing auto-approver and creating session...`,
		);
		...
		this._logService.trace(
			`[AgentService] createSession: initialization complete`,
		);
		...
		this._logService.trace(
			`[AgentService] createSession: provider=${provider.id} model=${config?.model?.id ?? "(default)"}`,
		);
		this._sessionToProvider.set(session.toString(), provider.id);
		this._logService.trace(
			`[AgentService] createSession returned: ${session.toString()}`,
		);
		...
		const [sessionConfig, initialCustomizations] = await Promise.all([
			...
			provider.getSessionCustomizations
				? provider.getSessionCustomizations(session).catch((err) => {
						this._logService.error(
							"[AgentService] createSession: failed to resolve initial customizations",
							err,
						);
						return undefined;
					})
		...
		if (config?.fork) {
			...
			if (sourceState && config.fork.turnIdMapping) {
				sourceTurns = sourceState.turns
					.slice(0, config.fork.turnIndex + 1)
					.map((t) => ({
						...t,
						id: config!.fork!.turnIdMapping!.get(t.id) ?? generateUuid(),
					}));
			}
			...
			const state = this._stateManager.createSession(summary, {
				emitNotification: !created.provisional,
			});
			state.config = sessionConfig;
			state.activeClient = config?.activeClient;
			if (initialCustomizations && initialCustomizations.length > 0) {
		...
		if (
			...
			Object.keys(sessionConfig.values).length > 0 &&
			!created.provisional
		) {
		...
		if (!created.provisional) {
			...
			this._attachGitState(
				session,
				created.workingDirectory ?? config?.workingDirectory,
			);
		}
	...
	private _buildInitialSummary(
		...
		return {
			...
			...(created.project
				? {
						project: {
							uri: created.project.uri.toString(),
							displayName: created.project.displayName,
						},
					}
				: {}),
			model: config?.model,
	...
	private _onDidMaterializeSession(e: IAgentMaterializeSessionEvent): void {
		...
		if (!state) {
			this._logService.warn(
				`[AgentService] onDidMaterializeSession for unknown session: ${sessionKey}`,
			);
			return;
		}
		const summary: SessionSummary = {
			...
			...(e.project
				? {
						project: {
							uri: e.project.uri.toString(),
							displayName: e.project.displayName,
						},
					}
				: {}),
			workingDirectory:
				e.workingDirectory?.toString() ?? state.summary.workingDirectory,
			modifiedAt: Date.now(),
		};
	...
	private _attachGitState(
		...
		this._gitService.getSessionGitState(workingDirectory).then(
			...
			(e) => {
				this._logService.warn(
					`[AgentService] Failed to compute git state for ${session}`,
					e,
				);
			},
		);
	...
	private _stripGitOnlyChangesetEntries(sessionKey: string): void {
		...
		const filtered = current.filter(
			(c) => c.uriTemplate !== branchUri && c.uriTemplate !== uncommittedUri,
		);
	 ...
	 */
	private _updateBranchChangesetDescription(
		sessionKey: string,
		gitState: { branchName?: string; baseBranchName?: string },
	): void {
		const description = formatSessionChangesetDescription(
			gitState.branchName,
	...
	private _persistConfigValues(
		...
		try {
			...
			this._logService.warn(
				`[AgentService] Failed to open session database to persist configValues for ${session.toString()}: ${toErrorMessage(err)}`,
			);
			return;
		}
		ref.object
			.setMetadata("configValues", JSON.stringify(values))
			.catch((err) => {
				this._logService.warn(
					`[AgentService] Failed to persist configValues for ${session.toString()}: ${toErrorMessage(err)}`,
				);
			})
			.finally(() => {
				ref.dispose();
	...
	private async _resolveCreatedSessionConfig(
		provider: IAgent,
		config: IAgentCreateSessionConfig | undefined,
	): Promise<SessionConfigState | undefined> {
		...
		try {
			...
			this._logService.error(
				`[AgentService] Failed to resolve created session config for provider ${provider.id}`,
				err,
			);
			return config.config
				? { schema: { type: "object", properties: {} }, values: config.config }
				: undefined;
		}
	}
	...
	async disposeSession(session: URI): Promise<void> {
		this._logService.trace(
			`[AgentService] disposeSession: ${session.toString()}`,
		);
	...
	async subscribe(resource: URI, clientId: string): Promise<IStateSnapshot> {
		...
		try {
			...
			if (
				snapshot &&
				parsedChangeset &&
				!this._stateManager.getSessionState(parsedChangeset.sessionUri)
			) {
				await this._changesetCoordinator.restoreSessionIfChangesetSubscription(
					resource,
					(s) => this.restoreSession(s),
				);
			...
			if (!snapshot) {
				...
				const handled = await this._changesetCoordinator.tryHandleSubscribe(
					resource,
					(s) => this.restoreSession(s),
				);
				if (handled) {
					...
					if (parsedSubagent) {
						await this._restoreSubagentSession(
							resourceStr,
			...
			if (
				sessionState &&
				readSessionGitState(sessionState._meta) === undefined
			) {
	...
	private async _runSessionGc(resource: URI): Promise<void> {
		...
		this._logService.info(
			`[AgentService] GC: disposing empty unsubscribed session ${key}`,
		);
	...
	private _maybeEvictIdleSession(resource: URI): void {
		...
		this._logService.trace(
			`[AgentService] Evicting idle session: ${evictionTargetKey} (triggered by unsubscribe of ${key})`,
		);
		...
		for (const cachedKey of this._stateManager.getSessionUrisWithPrefix(
			subagentPrefix,
		)) {
	...
	dispatchAction(
		...
		this._logService.trace(
			`[AgentService] dispatchAction: type=${action.type}, clientId=${clientId}, clientSeq=${clientSeq}`,
			action,
		);
		...
		const next = (pending ?? Promise.resolve())
			.then(async () => {
				const rewritten:
					| SessionAction
					| TerminalAction
					| IRootConfigChangedAction = this._needsAsyncRewrite(channel, action)
			...
			.catch((err) => {
				this._logService.error(
					`[AgentService] async dispatchAction failed: ${toErrorMessage(err)}`,
				);
			});

		this._clientDispatchQueues.set(
			clientId,
			next.finally(() => {
	...
	private _needsAsyncRewrite(
		channel: string,
		action: SessionAction | TerminalAction | IRootConfigChangedAction,
	): action is SessionTurnStartedAction | SessionPendingMessageSetAction {
		if (
			action.type !== ActionType.SessionTurnStarted &&
		...
		return !!action.message.attachments?.some((a) =>
			this._isRewritableAttachment(a, attachmentsRootStr),
		);
	}
	private _isRewritableAttachment(
		attachment: MessageAttachment,
		attachmentsRootStr: string,
	): boolean {
	...
	private _attachmentsRoot(session: string): URI {
		return joinPath(
			this._sessionDataService.getSessionDataDir(URI.parse(session)),
			SESSION_ATTACHMENTS_DIRNAME,
		);
	}
	 ...
	 */
	private async _rewriteUserMessageAttachments<
		T extends SessionTurnStartedAction | SessionPendingMessageSetAction,
	>(channel: string, action: T, clientId: string): Promise<T> {
	...
	private async _rewriteSingleAttachment(
		attachment: MessageAttachment,
		attachmentsRoot: URI,
		attachmentsRootStr: string,
		clientId: string,
		...
		try {
			if (attachment.type === MessageAttachmentKind.EmbeddedResource) {
				...
				const basename = this._attachmentBasename(
					attachment.label,
					attachment.contentType,
				);
				return this._writeAndRewrite(
					attachment,
					bytes,
					basename,
					attachmentsRoot,
			...
			if (
				attachment.type === MessageAttachmentKind.Resource &&
				this._isRewritableAttachment(attachment, attachmentsRootStr)
			) {
			...
			this._logService.warn(
				`[AgentService] Failed to rewrite attachment '${attachment.label}': ${toErrorMessage(err)}`,
			);
		}
		return attachment;
	}
	 ...
	 */
	private async _readClientResource(
		originalUri: URI,
		clientId: string,
	): Promise<Uint8Array> {
		const proxiedUri = clientId
			? toAgentClientUri(originalUri, clientId)
			: originalUri;
		try {
	...
	private async _writeAndRewrite(
		...
		if (
			original.type === MessageAttachmentKind.Resource &&
			original.selection
		) {
	 ...
	 */
	private _attachmentBasename(
		label: string,
		contentType: string | undefined,
	): string {
		const safeLabel = (label || "attachment").replace(
			/[\\/:*?"<>|\u0000-\u001f]/g,
			"_",
		);
	...
	async resourceList(uri: URI): Promise<ResourceListResult> {
		...
		try {
			...
			throw new ProtocolError(
				...
				`Directory not found: ${uri.toString()}`,
			);
		}
	...
	async restoreSession(session: URI): Promise<void> {
		...
		if (!agent) {
			throw new ProtocolError(
				AHP_SESSION_NOT_FOUND,
				`No agent for session: ${sessionStr}`,
			);
		}
		...
		if (!meta) {
			throw new ProtocolError(
				AHP_SESSION_NOT_FOUND,
				`Session not found on backend: ${sessionStr}`,
			);
		}
		...
		try {
			...
			throw new ProtocolError(
				JSON_RPC_INTERNAL_ERROR,
				`Failed to restore session ${sessionStr}: ${message}`,
			);
		}
					...
					try {
						...
						if (m.configValues) {
							try {
								...
								this._logService.warn(
									`[AgentService] Failed to parse persisted configValues for ${sessionStr}: ${toErrorMessage(err)}`,
								);
							}
						}
					} finally {
		...
		this._changesetCoordinator.onSessionRestored(
			sessionStr,
			changesetMetadata ?? {},
		);
		...
		this._logService.info(
			`[AgentService] Restored session ${sessionStr} with ${turns.length} turns`,
		);
	...
	private async _getSessionMetadataForRestore(
		agent: IAgent,
		session: URI,
			...
			try {
				...
				try {
					...
					if (fallbackErr instanceof ProtocolError) {
						...
						throw new ProtocolError(
							fallbackErr.code,
							`Failed to get session metadata for ${sessionStr}: ${message}; ${fallbackErr.message}`,
							fallbackErr.data,
						);
					}
					throw fallbackErr;
	...
	private async _getSessionMetadataFromCatalog(
		agent: IAgent,
		session: URI,
	): Promise<IAgentSessionMetadata | undefined> {
		const sessionStr = session.toString();
		...
		try {
			...
			throw new ProtocolError(
				JSON_RPC_INTERNAL_ERROR,
				`Failed to list sessions for ${sessionStr}: ${message}`,
			);
		}
	...
	async resourceWrite(
		...
		try {
			if (
				params.ifMatch !== undefined ||
				mode !== ResourceWriteMode.Truncate ||
				position !== 0
			) {
				await this._resourceWriteWithMode(
					fileUri,
					content,
					mode,
					position,
				...
				await this._fileService.createFile(fileUri, content, {
					overwrite: false,
				});
			} else {
			...
			if (
				...
				e.fileOperationResult === FileOperationResult.FILE_MODIFIED_SINCE
			) {
				throw new ProtocolError(
					AhpErrorCodes.Conflict,
			...
			if (code === FileSystemProviderErrorCode.FileExists) {
				throw new ProtocolError(
					AhpErrorCodes.AlreadyExists,
					`File already exists: ${fileUri.toString()}`,
				);
			}
			if (code === FileSystemProviderErrorCode.NoPermissions) {
				throw new ProtocolError(
					AhpErrorCodes.PermissionDenied,
					`Permission denied: ${fileUri.toString()}`,
				);
			}
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`Failed to write file: ${fileUri.toString()}`,
			);
		}
	}
	...
	private async _resourceWriteWithMode(
		...
		switch (mode) {
			case ResourceWriteMode.Append: {
				...
				next = VSBuffer.concat([
					base.slice(0, splitAt),
					data,
					base.slice(splitAt, eof),
				]);
				break;
			}
			case ResourceWriteMode.Insert: {
				...
				next = VSBuffer.concat([
					base.slice(0, splitAt),
					data,
					base.slice(splitAt, base.byteLength),
				]);
				break;
			}
	...
	async resourceCopy(params: ResourceCopyParams): Promise<ResourceCopyResult> {
		...
		try {
			...
			if (code === FileSystemProviderErrorCode.FileExists) {
				throw new ProtocolError(
					AhpErrorCodes.AlreadyExists,
					`Destination already exists: ${destination.toString()}`,
				);
			}
			if (code === FileSystemProviderErrorCode.NoPermissions) {
				throw new ProtocolError(
					AhpErrorCodes.PermissionDenied,
					`Permission denied: ${source.toString()}`,
				);
			}
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`Source not found: ${source.toString()}`,
			);
		}
	}
	...
	async resourceDelete(
		...
		try {
			...
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`Resource not found: ${fileUri.toString()}`,
			);
		}
	}
	...
	async resourceResolve(
		...
		try {
			...
			if (code === FileSystemProviderErrorCode.NoPermissions) {
				throw new ProtocolError(
					AhpErrorCodes.PermissionDenied,
					`Permission denied: ${uri.toString()}`,
				);
			}
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
	...
	async resourceMkdir(
		...
		try {
			...
			if (existing && !existing.isDirectory) {
				throw new ProtocolError(
					AhpErrorCodes.AlreadyExists,
					`Path exists and is not a directory: ${uri.toString()}`,
				);
			}
			...
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`Failed to create directory: ${uri.toString()}`,
			);
		}
	}
	...
	async createResourceWatch(
		...
		try {
			...
			if (code === FileSystemProviderErrorCode.NoPermissions) {
				throw new ProtocolError(
					AhpErrorCodes.PermissionDenied,
					`Permission denied: ${root.toString()}`,
				);
			}
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`Resource not found: ${root.toString()}`,
			);
		}
	...
	onResourceWatchSubscribed(channel: string): ResourceWatchState | undefined {
		...
		try {
			...
			if (descriptor.recursive) {
				...
				const watcher = this._fileService.createWatcher(root, {
					...watchOptions,
					recursive: false,
				});
			...
			disposables.dispose();
			this._logService.warn(
				`[AgentService] Failed to start IFileService watcher for ${channel}: ${e instanceof Error ? e.message : String(e)}`,
			);
			return undefined;
		}
	...
	private _dispatchResourceWatchChanges(
		channel: string,
		raw: readonly IFileChange[],
	): void {
		if (raw.length === 0) {
			return;
		}
		const items = raw.map((c) => ({
			...
			type:
				c.type === FileChangeType.ADDED
					? ResourceChangeType.Added
					: c.type === FileChangeType.DELETED
	...
	private async _fetchSessionDbContent(
		fields: ISessionDbUriFields,
	): Promise<ResourceReadResult> {
		const sessionUri = URI.parse(fields.sessionUri);
		const ref = this._sessionDataService.openDatabase(sessionUri);
		try {
			const content = await ref.object.readFileEditContent(
				fields.toolCallId,
				fields.filePath,
			);
			if (!content) {
				throw new ProtocolError(
					AhpErrorCodes.NotFound,
					`File edit not found: toolCallId=${fields.toolCallId}, filePath=${fields.filePath}`,
				);
			}
			const bytes =
				fields.part === "before" ? content.beforeContent : content.afterContent;
			if (!bytes) {
				throw new ProtocolError(
					AhpErrorCodes.NotFound,
					`No ${fields.part} content for: toolCallId=${fields.toolCallId}, filePath=${fields.filePath}`,
				);
			}
			return {
	...
	private async _fetchGitBlobContent(
		fields: IGitBlobUriFields,
	): Promise<ResourceReadResult> {
		if (!this._gitService) {
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`git service unavailable for: ${fields.repoRelativePath}`,
			);
		}
		const workingDirectory = this._stateManager.getSessionState(
			fields.sessionUri,
		)?.summary.workingDirectory;
		if (!workingDirectory) {
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`Session has no working directory for git-blob URI: ${fields.sessionUri}`,
			);
		}
		const blob = await this._gitService.showBlob(
			URI.parse(workingDirectory),
			fields.sha,
			fields.repoRelativePath,
		);
		if (!blob) {
			throw new ProtocolError(
				AhpErrorCodes.NotFound,
				`git blob not found: ${fields.sha}:${fields.repoRelativePath}`,
			);
		}
		return {
			data: blob.toString(),
	 ...
	 */
	private async _restoreSubagentSession(
		subagentUri: string,
		parentSession: URI,
	): Promise<void> {
		...
		if (!this._stateManager.getSessionState(parentSessionKey)) {
			try {
				...
				this._logService.warn(
					`[AgentService] Cannot restore parent session for subagent: ${parentSessionKey}`,
				);
				return;
			}
		}
		...
		if (agent) {
			try {
				...
				this._logService.warn(
					`[AgentService] Failed to load subagent turns for ${subagentUri}`,
					err,
				);
			}
		}
		...
		this._stateManager.restoreSession(
			{
				...
				createdAt: Date.now(),
				modifiedAt: Date.now(),
				...(parentState?.summary.project
					? { project: parentState.summary.project }
					: {}),
			},
			[...childTurns],
		);
		this._logService.info(
			`[AgentService] Restored subagent session: ${subagentUri} with ${childTurns.length} turn(s)`,
		);
	}
 ...
 * unrelated events.
 */
function collectChangesUnderRoot(
