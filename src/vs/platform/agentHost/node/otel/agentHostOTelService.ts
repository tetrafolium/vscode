...
import {
	...
	type ILocalOtlpHttpReceiver,
} from "../../../otel/node/otlp/localOtlpReceiver.js";
import {
	CompositeForwarder,
	ConsoleForwarder,
	FileForwarder,
...
export function readAgentHostOTelEnv(env: NodeJS.ProcessEnv): ResolvedConfig {
	...
	const otlpEndpoint =
		env.OTEL_EXPORTER_OTLP_ENDPOINT ?? env.COPILOT_OTEL_ENDPOINT;
	const filePath = env.COPILOT_OTEL_FILE_EXPORTER_PATH;
	const explicitlyEnabled = isTruthy(env.COPILOT_OTEL_ENABLED);
	const enabled =
		explicitlyEnabled || dbSpanExporter || !!otlpEndpoint || !!filePath;
	...
	return {
		...
		captureContent:
			env.OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT === undefined
				? undefined
...
export class AgentHostOTelService
	extends Disposable
	implements IAgentHostOTelService
{
	...
	getSpansDbPath(): URI | undefined {
		return this._config.dbSpanExporter
			? URI.file(this._spansDbPath)
			: undefined;
	}
		...
		if (!this._startPromise) {
			this._startPromise = this._start().catch((err) => {
				this._logService.error(
					"[agentHost.otel] failed to start loopback OTel pipeline",
					err,
				);
					...
					for (const span of result.spans) {
						try {
							...
							this._logService.warn(
								"[agentHost.otel] failed to insert span",
								err,
							);
						}
					}
	...
	private _buildOutboundForwarder(): IOutboundForwarder | undefined {
		...
		return children.length === 1
			? children[0]
			: new CompositeForwarder(children);
	}
}
