...
export const IAgentHostFileMonitorService =
	createDecorator<IAgentHostFileMonitorService>("agentHostFileMonitorService");

export const DEFAULT_AGENT_HOST_WATCH_EXCLUDES: readonly string[] =
	Object.freeze([
		"**/.git/objects/**",
		"**/.git/subtree-cache/**",
		"**/.git/**/*.lock",
...
export interface IAgentHostFileMonitorService extends IDisposable {
	...
	acquire(
		folder: URI,
		callback: () => void,
		options?: IAgentHostFileMonitorOptions,
...
export class AgentHostFileMonitorService
	extends Disposable
	implements IAgentHostFileMonitorService
{
	...
	private readonly _entries = this._register(
		new DisposableMap<string, IMonitorEntry>(),
	);
	...
	constructor(
		...
		this._register(
			this._fileService.onDidFilesChange((event) =>
				this._onDidFilesChange(event),
			),
		);
		this._register(
			this._fileService.onDidWatchError((error) => {
				this._logService.warn(
					"[AgentHostFileMonitorService] File watcher error",
					error,
				);
			}),
		);
	...
	acquire(
		...
		const excludes = normalizeExcludes(
			options.excludes ?? DEFAULT_AGENT_HOST_WATCH_EXCLUDES,
		);
		const debounceMs =
			options.debounceMs ?? AgentHostFileMonitorService._DEFAULT_DEBOUNCE_MS;
		...
		if (!entry) {
			try {
				...
				this._logService.warn(
					`[AgentHostFileMonitorService] Failed to watch ${canonicalFolder.toString()}`,
					err,
				);
				return undefined;
			}
	...
	private _createEntry(
		...
		try {
			...
			disposable.add(
				this._fileService.watch(folder, {
					recursive: true,
					excludes: [...excludes],
				}),
			);
	...
	private _onDidFilesChangeEntry(key: string, event: FileChangesEvent): void {
		...
		if (
			!event.affects(entry.folder) ||
			!this._hasRelevantRawChange(entry, event)
		) {
			return;
		}
		entry.debounce.value = disposableTimeout(() => {
			...
			for (const callback of [...entry.callbacks]) {
				try {
					...
					this._logService.warn(
						"[AgentHostFileMonitorService] Folder change callback failed",
						err,
					);
				}
			}
	...
	private _isExcluded(entry: IMonitorEntry, resource: URI): boolean {
		...
		if (
			...
			this._matchesExclude(entry, relativePath, basename)
		) {
			return true;
		}
	...
	private _canonicalizeFolder(folder: URI): URI {
		return extUriBiasedIgnorePathCase.removeTrailingPathSeparator(
			extUriBiasedIgnorePathCase.normalizePath(folder),
		);
	}
