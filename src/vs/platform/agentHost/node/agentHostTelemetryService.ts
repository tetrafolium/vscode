...
export class AgentHostTelemetryService
	extends Disposable
	implements IAgentHostTelemetryService
{
	...
	get sendErrorTelemetry(): boolean {
		return (
			this.telemetryLevel >= TelemetryLevel.ERROR &&
			this._delegate.sendErrorTelemetry
		);
	}
...
export function updateAgentHostTelemetryLevelFromConfig(
	telemetryService: ITelemetryService,
	config: Record<string, unknown> | undefined,
): void {
	const telemetryLevel = config?.[AgentHostTelemetryLevelConfigKey];
	const telemetryLevelValue =
		agentHostConfigValueToTelemetryLevel(telemetryLevel);
	if (
		!isAgentHostTelemetryService(telemetryService) ||
		telemetryLevelValue === undefined
	) {
		return;
	}
...
function isAgentHostTelemetryService(
	telemetryService: ITelemetryService,
): telemetryService is IAgentHostTelemetryService {
	return (
		typeof (telemetryService as IAgentHostTelemetryService)
			.updateTelemetryLevel === "function"
	);
}

export async function createAgentHostTelemetryService(
	options: IAgentHostTelemetryServiceOptions,
): Promise<IAgentHostTelemetryService> {
	const {
		environmentService,
		productService,
		fileService,
