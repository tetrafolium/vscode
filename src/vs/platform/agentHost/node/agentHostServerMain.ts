...
globalThis._VSCODE_FILE_ROOT = fileURLToPath(
...
function parseServerOptions(): IServerOptions {
	...
	const rawToken =
		connectionTokenIdx >= 0 ? argv[connectionTokenIdx + 1] : undefined;
	const tokenFilePath =
		connectionTokenFileIdx >= 0 ? argv[connectionTokenFileIdx + 1] : undefined;

	let connectionToken: string | undefined;
	if (withoutConnectionToken) {
		if (rawToken !== undefined || tokenFilePath !== undefined) {
			log(
				"Error: --without-connection-token cannot be used with --connection-token or --connection-token-file",
			);
			process.exit(1);
		}
		...
		if (rawToken !== undefined) {
			log(
				"Error: --connection-token cannot be used with --connection-token-file",
			);
			process.exit(1);
		}
		try {
		...
		if (!connectionTokenRegex.test(connectionToken!)) {
			log(
				`Error: The connection token in '${tokenFilePath}' does not adhere to the characters 0-9, a-z, A-Z, _, or -.`,
			);
			process.exit(1);
		}
		...
		if (!connectionTokenRegex.test(rawToken)) {
			log(
				`Error: The connection token '${rawToken}' does not adhere to the characters 0-9, a-z, A-Z, _, or -.`,
			);
			process.exit(1);
		}
...
async function main(): Promise<void> {
	...
	if (options.quiet) {
		...
		const logger = loggerService.createLogger("agenthost-server", {
			name: localize("agentHostServer", "Agent Host Server"),
		});
	...
	if (!options.quiet) {
		...
		diServices.set(
			IDiffComputeService,
			disposables.add(new NodeWorkerDiffComputeService(logService)),
		);
		...
		const copilotAgent = disposables.add(
		...
		if (options.claudeSdkPath) {
			...
			const claudeAgent = disposables.add(
	...
	if (options.enableMockAgent) {
		...
		import("../test/node/mockAgent.js")
			.then(({ ScriptedMockAgent }) => {
	...
	async function shutdown(): Promise<void> {
		...
		await raceTimeout(sessionDataService.whenIdle(), 3000, () => {
			logService.warn(
				"[AgentHostServer] Timed out waiting for session database writes to flush; exiting anyway.",
			);
		});
		disposables.dispose();
