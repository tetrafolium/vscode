...
export const IAgentHostGitService = createDecorator<IAgentHostGitService>(
	"agentHostGitService",
);
...
export interface IAgentHostGitService {
	...
	getSessionGitState(
		workingDirectory: URI,
	): Promise<ISessionGitState | undefined>;
	...
	computeSessionFileDiffs(
		workingDirectory: URI,
		options: IComputeSessionFileDiffsOptions,
...
export function getBranchCompletions(
	...
	filtered.sort(
		(a, b) => getCommonBranchPriority(a) - getCommonBranchPriority(b),
	);
...
export class AgentHostGitService implements IAgentHostGitService {
	...
	async isInsideWorkTree(workingDirectory: URI): Promise<boolean> {
		return (
			(
				await this._runGit(workingDirectory, [
					"rev-parse",
					"--is-inside-work-tree",
				])
			)?.trim() === "true"
	...
	async getCurrentBranch(workingDirectory: URI): Promise<string | undefined> {
		return (
			(
				await this._runGit(workingDirectory, ["branch", "--show-current"])
			)?.trim() ||
			(
				await this._runGit(workingDirectory, ["rev-parse", "--short", "HEAD"])
			)?.trim() ||
			undefined
		);
	}
	...
	async addWorktree(
		...
		const resolvedStartPoint =
			(await this._resolveRemoteTrackingBranch(repositoryRoot, startPoint)) ??
			startPoint;
		...
		await this._runGit(
			...
			{ timeout: 60_000, throwOnError: true },
		);
	}
	...
	async addExistingWorktree(
		...
		await this._runGit(
			repositoryRoot,
			["worktree", "add", worktree.fsPath, branchName],
			{ timeout: 60_000, throwOnError: true },
		);
	}
	...
	async removeWorktree(repositoryRoot: URI, worktree: URI): Promise<void> {
		await this._runGit(
			repositoryRoot,
			["worktree", "remove", "--force", worktree.fsPath],
			{ timeout: 60_000, throwOnError: true },
		);
	}
	...
	async branchExists(
		...
		const output = await this._runGit(repositoryRoot, [
			"show-ref",
			"--verify",
			"--quiet",
	...
	async computeSessionFileDiffs(
		...
		const inside = await this._runGit(workingDirectory, [
			"rev-parse",
			"--is-inside-work-tree",
		]);
		if (inside?.trim() !== "true") {
		...
		if (options.baseBranch) {
			const baseBranch =
				(await this._resolveRemoteTrackingBranch(
					repositoryRoot,
					options.baseBranch,
				)) ?? options.baseBranch;
			mergeBaseCommit = (
				await this._runGit(repositoryRoot, ["merge-base", "HEAD", baseBranch])
			)?.trim();
		}
		if (!mergeBaseCommit) {
			mergeBaseCommit = (
				await this._runGit(repositoryRoot, ["rev-parse", "HEAD"])
			)?.trim();
		}
		if (!mergeBaseCommit) {
		...
		const statusOut = await this._runGit(repositoryRoot, [
			"status",
			"--porcelain=v1",
			"-z",
			"--untracked-files=all",
		...
		if (untracked.length === 0) {
			rawDiffOutput = await this._runGit(repositoryRoot, [
				"diff",
				"--raw",
				"--numstat",
				"--diff-filter=ADMR",
			...
			rawDiffOutput = await this._runWithTempIndex(
				repositoryRoot,
				mergeBaseCommit,
			);
		}
	...
	private async _runWithTempIndex(
		repositoryRoot: URI,
		...
		const tempDir = URI.joinPath(
			this._environmentService.tmpDir,
			`agent-host-git-diff-${generateUuid()}`,
		);
		...
		try {
			const seeded = await this._runGit(repositoryRoot, ["read-tree", "HEAD"], {
				env,
			});
			if (seeded === undefined) {
				// Empty repo (no HEAD yet) - `read-tree` of the empty tree always succeeds.
				await this._runGit(repositoryRoot, ["read-tree", EMPTY_TREE_OBJECT], {
					env,
				});
			}
			...
			try {
				await this._fileService.del(tempDir, {
					recursive: true,
					useTrash: false,
				});
			} catch {
	...
	private async _resolveRemoteTrackingBranch(
		repositoryRoot: URI,
		branch: string,
	): Promise<string | undefined> {
	...
	async captureWorkingTreeAsTree(
		...
		const tempDir = URI.joinPath(
			this._environmentService.tmpDir,
			`agent-host-checkpoint-${generateUuid()}`,
		);
		await this._fileService.createFolder(tempDir);
		const indexFile = URI.joinPath(tempDir, "index").fsPath;
		...
		try {
			...
			await this._runGit(repositoryRoot, ["add", "-A", "--", ":/"], { env });
			const tree = (
				await this._runGit(repositoryRoot, ["write-tree"], { env })
			)?.trim();
	...
	async commitTree(
		...
		args.push("-m", message);
		const out = await this._runGit(repositoryRoot, args, {
			throwOnError: true,
		});
	...
	async updateRef(
		...
		await this._runGit(repositoryRoot, ["update-ref", ref, newOid], {
			throwOnError: true,
		});
	}
	...
	async deleteRefs(
		...
		await new Promise<void>((resolve) => {
			const proc = cp.execFile(
				"git",
				["update-ref", "--stdin", "-z"],
				{ cwd: repositoryRoot.fsPath, timeout: 10_000 },
				() => {
	...
	async revParse(
		...
		const out = await this._runGit(repositoryRoot, [
			"rev-parse",
			"--verify",
			"--quiet",
			expression,
	...
	async computeFileDiffsBetweenRefs(
		...
		const fromOid = (
			await this._runGit(repositoryRoot, [
				"rev-parse",
				"--verify",
				"--quiet",
				options.fromRef,
		...
		const raw = await this._runGit(repositoryRoot, [
			"diff",
			"--raw",
			"--numstat",
			"--diff-filter=ADMR",
	...
	private async _computeSessionGitState(
		workingDirectory: URI,
	): Promise<ISessionGitState | undefined> {
		...
		const [statusOutput, remotesOutput, defaultBranchRef] = await Promise.all([
			this._runGit(workingDirectory, ["status", "-b", "--porcelain=v2"]),
			this._runGit(workingDirectory, ["remote", "-v"]),
			this._runGit(workingDirectory, [
				"symbolic-ref",
				"--quiet",
				"refs/remotes/origin/HEAD",
			]),
		...
		if (
			...
			baseBranchName &&
			status.branchName &&
			status.branchName !== baseBranchName
		) {
			const ahead = await this._runGit(workingDirectory, [
				"rev-list",
				"--count",
				`${baseBranchName}..HEAD`,
			]);
					...
					if (error) {
						...
						if (stderr) {
							this._logService.warn(
								`[agentHostGitService] git ${args.join(" ")} failed; full stderr:\n${stderr}`,
							);
						}
						if (options?.throwOnError) {
							reject(
								new Error(
									formatGitError(args, timeoutMs, didTimeOut, error, stderr),
									{ cause: error },
								),
							);
							return;
 ...
 * Exported for tests.
 */
export function parseGitDiffRawNumstat(
	output: string,
	repositoryRoot: URI,
	sessionUri: string,
	beforeRef: string,
			...
			...(hasBefore && change.oldPath
				? {
						before: {
							...
							content: {
								uri: buildGitBlobUri(sessionUri, beforeRef, change.oldPath),
							},
						},
					}
				: {}),
...
/** Exported for tests. */
export function parseHasGitHubRemote(
 ...
 */
function parseGitHubOwnerRepoFromUrl(
	url: string,
): { owner: string; repo: string } | undefined {
	...
	m =
		/^[a-z+]+:\/\/(?:[^@\/\s]+@)?github\.com(?::\d+)?\/([^/\s]+)\/([^/\s]+?)(?:\.git)?$/i.exec(
			url,
		);
	if (m) {
		return { owner: m[1], repo: m[2] };
...
/** Exported for tests. */
export function parseDefaultBranchRef(
	symbolicRefOutput: string | undefined,
): string | undefined {
