...
export async function createRemoteWatchHandle(
	primitives: {
		...
		onDidAction: Event<{
			...
			action: {
				type: string;
				changes?: { items: readonly { uri: string; type: string }[] };
			};
		}>;
	},
	params: CreateResourceWatchParams,
	...
	const listener = primitives.onDidAction((envelope) => {
		if (
			...
			envelope.action.type !== "resourceWatch/changed"
		) {
			return;
		}
 ...
 */
export abstract class AHPFileSystemProvider
	extends Disposable
	implements IFileSystemProvider
{
	...
	private readonly _authorityToConnection = new Map<
		string,
	...
	watch(resource: URI, opts: IWatchOptions): IDisposable {
		...
		const params: CreateResourceWatchParams = {
			...
			...(opts.excludes.length > 0
				? { excludes: { items: [...opts.excludes] } }
				: {}),
			...(opts.includes && opts.includes.length > 0
				? {
						includes: {
							items: opts.includes.map((p) =>
		...
		connection.watchResource(params).then(
			(handle) => {
				...
				const sub = handle.onDidChange((changes) =>
	...
	async requestResourceAccess(
		resource: URI,
		opts: { readonly read?: boolean; readonly write?: boolean },
	): Promise<void> {
		const connection = this._getConnection(resource.authority);
	...
	private _mapError(
		err: unknown,
		defaultCode: FileSystemProviderErrorCode,
	): Error {
		if (
			err instanceof ProtocolError &&
			err.code === AhpErrorCodes.PermissionDenied
		) {
