...
export class AgentHostClientResourceChannel implements IServerChannel {
	...
	private async _call<T>(
		...
		switch (command) {
			case "resourceList": {
				const stat = await this._fileService.resolve(
					URI.parse(a.uri as string),
				);
				if (!stat.isDirectory) {
			...
			case "resourceWrite": {
				...
				const buf =
					params.encoding === ContentEncoding.Base64
			...
			case "resourceDelete": {
				...
				await this._fileService.del(URI.parse(params.uri), {
			...
			case "resourceMkdir": {
				...
				const uri = URI.parse(params.uri);
				const existing = await this._fileService
					.stat(uri)
					.catch(() => undefined);
		...
		throw new Error(
			`Unknown command '${command}' on AgentHostClientResourceChannel`,
		);
	}
}
 ...
 */
export function createAgentHostClientResourceConnection(
	channel: IChannel,
): IRemoteFilesystemConnection {
	return {
		resourceList: (uri) =>
			channel.call("resourceList", {
				uri: uri.toString(),
			}) as Promise<ResourceListResult>,
		resourceRead: (uri) =>
			channel.call("resourceRead", {
				uri: uri.toString(),
			}) as Promise<ResourceReadResult>,
		resourceWrite: (params) =>
			channel.call("resourceWrite", {
				...params,
				uri: params.uri.toString(),
			}) as Promise<ResourceWriteResult>,
		resourceCopy: (params) =>
			channel.call("resourceCopy", {
				...params,
				source: params.source.toString(),
				destination: params.destination.toString(),
			}) as Promise<ResourceCopyResult>,
		resourceDelete: (params) =>
			channel.call("resourceDelete", {
				...params,
				uri: params.uri.toString(),
			}) as Promise<ResourceDeleteResult>,
		resourceMove: (params) =>
			channel.call("resourceMove", {
				...params,
				source: params.source.toString(),
				destination: params.destination.toString(),
			}) as Promise<ResourceMoveResult>,
		resourceResolve: (params) =>
			channel.call("resourceResolve", {
				...params,
				uri: params.uri.toString(),
			}) as Promise<ResourceResolveResult>,
		resourceMkdir: (params) =>
			channel.call("resourceMkdir", {
				...params,
				uri: params.uri.toString(),
			}) as Promise<ResourceMkdirResult>,
	};
}
