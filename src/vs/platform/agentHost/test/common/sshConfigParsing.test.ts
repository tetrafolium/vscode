...
suite("SSH Config Parsing", () => {
	...
	suite("parseSSHConfigHostEntries", () => {
		test("extracts simple host entries", () => {
			const config = ["Host myserver", "	HostName 10.0.0.1", "	User admin"].join(
				"\n",
			);
		...
		test("handles content with only comments and blanks", () => {
			const config = ["# comment", "", "  # indented comment", ""].join("\n");

			assert.deepStrictEqual(parseSSHConfigHostEntries(config), []);
		});

		test("is case-insensitive for Host keyword", () => {
			const config = ["host lower", "HOST upper", "Host mixed"].join("\n");
	...
	suite("parseSSHGOutput", () => {
		...
		test("parses identityagent", () => {
			...
			assert.strictEqual(
				parseSSHGOutput(output).identityAgent,
				"//./pipe/pageant.user.1234",
			);
		});

		test("parses non-standard port", () => {
			const output = ["hostname example.com", "user deploy", "port 2222"].join(
				"\n",
			);
		...
		test("handles missing user", () => {
			const output = ["hostname example.com", "port 22"].join("\n");
		...
		test("handles empty user", () => {
			const output = ["hostname example.com", "user ", "port 22"].join("\n");
