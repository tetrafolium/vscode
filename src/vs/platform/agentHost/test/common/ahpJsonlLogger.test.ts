...
suite("AhpJsonlLogger", () => {
	...
	test("writes canonical JSON-RPC JSONL with metadata at the root", async () => {
		...
		const requestText =
			'{"jsonrpc":"2.0","id":"request-1","method":"initialize","params":{"protocolVersion":1}}';
		const uri = URI.parse("ahp-session:/session-1");
		logger.log(
			JSON.parse(requestText),
			"c2s",
			getAhpLogByteLength(requestText),
		);
		logger.log({ jsonrpc: "2.0", id: 2, result: { ok: true } }, "s2c");
		logger.log(
			{ jsonrpc: "2.0", id: null, error: { code: -32000, message: "Nope" } },
			"s2c",
		);
		logger.log(
			{ jsonrpc: "2.0", method: "notification", params: { uri } },
			"s2c",
		);
		await logger.flush();
	...
	test("rotates JSONL files and keeps bounded history", async () => {
		...
		const rotated1 = joinPath(
			dirname(firstResource),
			`${currentBaseName}.1.jsonl`,
		);
		const rotated2 = joinPath(
			dirname(firstResource),
			`${currentBaseName}.2.jsonl`,
		);
		...
		const lines = [
			...(await fileService.readFile(rotated1)).value
				.toString()
				.split("\n")
				.filter(Boolean),
			...(await fileService.readFile(rotated2)).value
				.toString()
				.split("\n")
				.filter(Boolean),
		];
