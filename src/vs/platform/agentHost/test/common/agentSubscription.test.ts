...
function makeEnvelope(
	...
	const resolvedChannel =
		channel ??
		(action.type.startsWith("root/")
			? ROOT_STATE_URI
			: action.type.startsWith("terminal/")
				? terminalUri
				: action.type.startsWith("changeset/")
					? changesetUri
					: sessionUri);
	return {
...
suite("RootStateSubscription", () => {
	...
	test("ignores non-root actions", () => {
		...
		sub.handleSnapshot(state, 0);
		sub.receiveEnvelope(makeEnvelope({ type: ActionType.SessionReady }, 1));
		assert.deepStrictEqual(sub.value, state);
	});
...
suite("SessionStateSubscription", () => {
	...
	function createSub(
		uri: string = sessionUri,
		clientId: string = "c1",
	): SessionStateSubscription {
		return disposables.add(
			new SessionStateSubscription(uri, clientId, () => ++seq, noop),
		);
	}
	...
	test("foreign action updates confirmed and recomputes optimistic", () => {
		...
		sub.receiveEnvelope(
			makeEnvelope({ type: ActionType.SessionReady }, 1, {
	...
	test("after all pending cleared, value falls through to verifiedValue", () => {
		...
		sub.receiveEnvelope(
			makeEnvelope({ type: ActionType.SessionTitleChanged, title: "Temp" }, 1, {
				clientId: "c1",
				clientSeq,
			}),
		);
...
suite("TerminalStateSubscription", () => {
	...
	test("accepts terminal actions matching its URI", () => {
		const sub = disposables.add(
			new TerminalStateSubscription(terminalUri, "c1", noop),
		);
		sub.handleSnapshot(makeTerminalState(), 0);

		sub.receiveEnvelope(
			makeEnvelope({ type: ActionType.TerminalData, data: "hello" }, 1),
		);
...
suite("AgentSubscriptionManager", () => {
	...
	test("second call for same resource increments refcount", async () => {
		const mgr = createManager();
		const uri = URI.parse(sessionUri);
		const ref1 = mgr.getSubscription<SessionState>(
			StateComponents.Session,
			uri,
		);
		const ref2 = mgr.getSubscription<SessionState>(
			StateComponents.Session,
			uri,
		);
	...
	test("receiveEnvelope routes to root and all active subscriptions", async () => {
		...
		assert.strictEqual(
			(ref.object.value as SessionState).summary.title,
			"Routed",
		);
	...
	test("creating session subscription for copilot: URI", async () => {
		...
		const ref = mgr.getSubscription<SessionState>(
			StateComponents.Session,
			mySessionUri,
		);
		await new Promise((r) => setTimeout(r, 0));
	...
	test("creating terminal subscription for terminal URI", async () => {
		...
		const ref = mgr.getSubscription<TerminalState>(
	...
	test("dispose clears all subscriptions and calls unsubscribe for each", async () => {
		...
		const ref2 = mgr.getSubscription<TerminalState>(
			StateComponents.Terminal,
	...
	test("getSubscriptionUnmanaged returns undefined when no subscription exists", () => {
		const mgr = createManager();
		const result = mgr.getSubscriptionUnmanaged<SessionState>(
			URI.parse("copilot:/nonexistent"),
		);
