...
suite("changesetUri", () => {
	...
	test("builders produce the documented shapes", () => {
		assert.strictEqual(
			...
			"copilot:/abc-123/changeset/session",
		);
		assert.strictEqual(
			buildUncommittedChangesetUri(sessionUri),
			"copilot:/abc-123/changeset/uncommitted",
		);
		assert.strictEqual(
			buildTurnChangesetUri(sessionUri, "t1"),
			"copilot:/abc-123/changeset/turn/t1",
		);
		assert.strictEqual(
			buildTurnChangesetUriTemplate(sessionUri),
			"copilot:/abc-123/changeset/turn/{turnId}",
		);
		assert.strictEqual(
			buildCompareTurnsChangesetUri(sessionUri, "t1", "t2"),
			"copilot:/abc-123/changeset/compare/t1/t2",
		);
		assert.strictEqual(
			buildCompareTurnsChangesetUriTemplate(sessionUri),
			"copilot:/abc-123/changeset/compare/{originalTurnId}/{modifiedTurnId}",
		);
		assert.strictEqual(
			buildChangesetUri(sessionUri, "session"),
	...
	test("parseChangesetUri identifies the well-known kinds", () => {
		...
		assert.deepStrictEqual(
			...
			{
				sessionUri,
				changesetId: "uncommitted",
				kind: ChangesetKind.Uncommitted,
			},
		);
		assert.deepStrictEqual(
	...
	test("parseTurnChangesetUri only matches expanded turn URIs", () => {
		...
		assert.strictEqual(
			parseTurnChangesetUri(buildSessionChangesetUri(sessionUri)),
			undefined,
		);
		assert.strictEqual(
			parseTurnChangesetUri(buildTurnChangesetUriTemplate(sessionUri)),
			undefined,
		);
		assert.strictEqual(
			parseTurnChangesetUri(
	...
	test("predicates match the parser semantics", () => {
		assert.strictEqual(
			isChangesetUri(buildSessionChangesetUri(sessionUri)),
			true,
		);
		assert.strictEqual(
			isChangesetUri(buildUncommittedChangesetUri(sessionUri)),
			true,
		);
		assert.strictEqual(
			isChangesetUri(buildTurnChangesetUri(sessionUri, "t1")),
			true,
		);
		assert.strictEqual(isChangesetUri(sessionUri), false);
		assert.strictEqual(
			isSessionChangesetUri(buildSessionChangesetUri(sessionUri)),
			true,
		);
		assert.strictEqual(
			isSessionChangesetUri(buildUncommittedChangesetUri(sessionUri)),
			false,
		);
		assert.strictEqual(
			isUncommittedChangesetUri(buildUncommittedChangesetUri(sessionUri)),
			true,
		);
		assert.strictEqual(
			isUncommittedChangesetUri(buildSessionChangesetUri(sessionUri)),
			false,
		);
	});
});
