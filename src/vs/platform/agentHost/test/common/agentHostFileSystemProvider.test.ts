...
suite("AgentHostAuthority - encoding", () => {
	...
	test("normal host:port address uses human-readable encoding", () => {
		...
		assert.strictEqual(
			agentHostAuthority("192.168.1.1:8080"),
			"192.168.1.1__8080",
		);
	...
	test("address with underscore falls through to base64", () => {
		...
		assert.ok(
			authority.startsWith("b64-"),
			`expected base64 for underscore address, got: ${authority}`,
		);
	});
	...
	test("ws:// prefix is normalized so authority matches bare address", () => {
		assert.strictEqual(
			agentHostAuthority("ws://127.0.0.1:8080"),
			agentHostAuthority("127.0.0.1:8080"),
		);
		assert.strictEqual(
			agentHostAuthority("ws://localhost:9090"),
			agentHostAuthority("localhost:9090"),
		);
	});
	...
	test("different addresses produce different authorities", () => {
		...
		assert.strictEqual(
			unique.size,
			cases.length,
			"all authorities must be unique",
		);
	});
	...
	test("authority is valid in a URI authority position", () => {
		...
		for (const address of addresses) {
			...
			assert.strictEqual(
				uri.authority,
				authority,
				`authority for '${address}' must round-trip through URI`,
			);
		}
	});
	...
	test("authority is valid in a URI scheme position", () => {
		...
		for (const address of addresses) {
			...
			assert.strictEqual(
				uri.scheme,
				scheme,
				`scheme for '${address}' must round-trip through URI`,
			);
		}
	});
});
...
suite("AGENT_HOST_LABEL_FORMATTER", () => {
	...
	test("stripPathSegments matches URI encoding for file URIs", () => {
		...
		const stripped = stripPath(
			encodedUri.path,
			AGENT_HOST_LABEL_FORMATTER.formatting.stripPathSegments!,
		);
...
suite("AgentHostFileSystemProvider - authority registrations", () => {
	...
	test("disposing a stale registration does not remove a newer registration for the same authority", async () => {
		...
		const firstRegistration = disposables.add(
			provider.registerAuthority("client", first),
		);
		disposables.add(provider.registerAuthority("client", second));

		firstRegistration.dispose();
		const entries = await provider.readdir(
			agentHostUri("client", "/workspace"),
		);

		assert.deepStrictEqual(
			{
				entries,
				firstCalls: first.listCalls,
				secondCalls: second.listCalls.map((uri) => uri.toString()),
			},
			{
...
suite("AgentHostFileSystemProvider - synthetic content schemes", () => {
	...
	test("stat returns File for git-blob: URIs without listing the parent", async () => {
		...
		assert.deepStrictEqual(
			connection.listCalls,
			[],
			"stat must not list a synthetic parent directory",
		);
	});
	...
	test("readFile passes the decoded synthetic URI through to the connection", async () => {
		...
		assert.deepStrictEqual(
			connection.readCalls.map((u) => u.toString()),
			[inner.toString()],
		);
	});
...
suite(
	"AgentHostFileSystemProvider - permission errors and requestResourceAccess",
	() => {
		const disposables = ensureNoDisposablesAreLeakedInTestSuite();
		...
		function permissionDenied(uri: string): ProtocolError {
			return new ProtocolError(AhpErrorCodes.PermissionDenied, "denied", {
				request: { uri, read: true },
			});
		}
		...
		test("writeFile / delete / rename / readdir all surface NoPermissions on PermissionDenied", async () => {
			...
			await collect(() =>
				provider.writeFile(wrapped, new Uint8Array(), {
					create: true,
					overwrite: true,
					unlock: false,
					atomic: false,
			...
			await collect(() =>
				provider.rename(wrapped, agentHostUri("remote", "/dst"), {
					overwrite: true,
				}),
			);
		...
		test("requestResourceAccess forwards the decoded URI and access flags", async () => {
			...
			await provider.requestResourceAccess(wrapped, {
				read: true,
				write: true,
			});
...
suite("AgentHostFileSystemProvider - resolve / mkdir / copy / watch", () => {
	...
	class FullConnection implements IRemoteFilesystemConnection {
		...
		nextWatchHandle:
			| { onDidChange: Event<readonly IFileChange[]>; dispose(): void }
			| undefined;
		watchError: unknown | undefined;
		nextResolveResult: ResourceResolveResult = {
			uri: "",
		...
		async resourceResolve(
			...
			return {
				...
				uri:
					typeof params.uri === "string"
						? params.uri
						: URI.revive(params.uri).toString(),
			};
		}
		...
		async watchResource(
			...
			onDidChange: Event<readonly IFileChange[]>;
			dispose(): void;
		}> {
	...
	test("stat uses resourceResolve when available and maps size/mtime/type", async () => {
		...
		assert.strictEqual(
			connection.resolveCalls.length,
			1,
			"resourceResolve was called",
		);
	});
	...
	test("mkdir delegates to resourceMkdir", async () => {
		...
		assert.strictEqual(
			connection.mkdirCalls[0].uri,
			fromAgentHostUri(wrapped).toString(),
		);
	});
	...
	test("copy delegates to resourceCopy with overwrite mapped to !failIfExists", async () => {
		...
		assert.strictEqual(
			connection.copyCalls[0].source,
			fromAgentHostUri(from).toString(),
		);
		assert.strictEqual(
			connection.copyCalls[0].destination,
			fromAgentHostUri(to).toString(),
		);
	...
	test("watch starts watchResource, forwards changes to onDidChangeFile, dispose tears down handle", async () => {
		...
		assert.deepStrictEqual(connection.watchCalls[0].excludes, {
			items: ["**/node_modules/**"],
		});
		...
		const incomingChange: IFileChange = {
			resource: URI.parse("file:///watched/a.txt"),
			type: FileChangeType.UPDATED,
		};
		const expectedChange: IFileChange = {
			resource: toAgentHostUri(URI.parse("file:///watched/a.txt"), "remote"),
			type: FileChangeType.UPDATED,
		};
		...
		assert.strictEqual(
			handleDisposed,
			true,
			"underlying handle should be disposed when wrapper is disposed",
		);
		sub.dispose();
	});
	...
	test("watch forwards includes patterns to watchResource", async () => {
		...
		await new Promise<void>((resolve) => queueMicrotask(resolve));
		assert.deepStrictEqual(connection.watchCalls[0].includes, {
			items: ["**/*.ts", "**/*.md"],
		});
