...
suite("resolveClaudeEffort (Phase 6.1 / Cycle E)", () => {
	...
	test("returns the SDK enum value for each accepted thinkingLevel string", () => {
		...
		const actual = accepted.map((level) =>
			resolveClaudeEffort({
				id: "claude-opus-4.6",
	...
	test("returns undefined for absent / unrecognized inputs (SDK default takes over)", () => {
		...
		const cases: readonly (ModelSelection | undefined)[] = [
			...
			{
				id: "claude-opus-4.6",
				config: { [CLAUDE_THINKING_LEVEL_KEY]: "turbo" },
			},
		];
		assert.deepStrictEqual(cases.map(resolveClaudeEffort), [
			undefined,
			undefined,
			undefined,
			undefined,
...
suite("clampEffortForRuntime (Phase 9 / Step 4)", () => {
	...
	test("clamps `max` to `xhigh`; passes other levels through; preserves undefined", () => {
		...
		const inputs: readonly (ClaudeEffortLevel | undefined)[] = [
			undefined,
			"low",
			"medium",
			"high",
		...
		assert.deepStrictEqual(inputs.map(clampEffortForRuntime), [
			undefined,
			"low",
			"medium",
			"high",
...
suite("isClaudeEffortLevel (Phase 6.1 / Cycle D3)", () => {
	...
	test("accepts the canonical 5-value union, rejects anything else", () => {
		...
		assert.deepStrictEqual(inputs.map(isClaudeEffortLevel), [
			true,
			true,
			true,
			true,
...
suite("createClaudeThinkingLevelSchema (Phase 6.1 / Cycle D3)", () => {
	...
	test("per-model variation: enum + enumLabels + default track the supplied list; empty list returns undefined", () => {
		...
		const fullUnion: readonly ClaudeEffortLevel[] = [
			"low",
			"medium",
			"high",
			"xhigh",
		...
		const lowMediumHigh: readonly ClaudeEffortLevel[] = [
			"low",
			"medium",
			"high",
		];
	...
	test(`emits default: 'high' iff 'high' is in the supported list, never substitutes another value`, () => {
		...
		const cases: readonly {
			input: readonly ClaudeEffortLevel[];
			expected: ClaudeEffortLevel | undefined;
		}[] = [
		...
		assert.deepStrictEqual(
			cases.map(
				(c) =>
					createClaudeThinkingLevelSchema(c.input)?.properties.thinkingLevel
						.default,
			),
