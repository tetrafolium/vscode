...
suite("OtlpLogEmitter", () => {
	...
	test("level <-> severity number mappings are inverse-ish", () => {
		...
		const observed = cases.map(([level]) => {
			...
			return {
				...
				severityNumber,
				severityText,
				roundTrip: severityNumberToLogLevel(severityNumber),
			};
		});
		assert.deepStrictEqual(observed, [
			{
				...
				severityNumber: 1,
				severityText: "trace",
				roundTrip: LogLevel.Trace,
			},
			{
				level: LogLevel.Debug,
				severityNumber: 5,
				severityText: "debug",
				roundTrip: LogLevel.Debug,
			},
			{
				level: LogLevel.Info,
				severityNumber: 9,
				severityText: "info",
				roundTrip: LogLevel.Info,
			},
			{
				level: LogLevel.Warning,
				severityNumber: 13,
				severityText: "warn",
				roundTrip: LogLevel.Warning,
			},
			{
				level: LogLevel.Error,
				severityNumber: 17,
				severityText: "error",
				roundTrip: LogLevel.Error,
			},
		]);
	});
	...
	test("OtlpEmitterLogger fans logs onto the shared emitter", () => {
		...
		const logger = disposables.add(
			new OtlpEmitterLogger(emitter, LogLevel.Trace),
		);
		...
		const sanitised = received.map((r) => ({
	...
	test("logger level gates which records reach the OTLP emitter", () => {
		const emitter = disposables.add(new OtlpLogEmitter());
		const otlpLogger = disposables.add(
			new OtlpEmitterLogger(emitter, LogLevel.Warning),
		);
		const received: IOtlpLogRecord[] = [];
		disposables.add(emitter.onDidLog((record) => received.push(record)));
	...
	test("iterateOtlpLogRecords tolerates malformed shapes", () => {
		const decoded = [
			...iterateOtlpLogRecords({
				resourceLogs: [
					{ scopeLogs: [{ logRecords: [null, { severityNumber: "bad" }] }] },
				],
			}),
	...
	test("buildOtlpLogsChannelUri + extractLevelFromOtlpLogsUri round-trip", () => {
		...
		assert.deepStrictEqual(
			cases.map((level) => ({
				...
				parsed: extractLevelFromOtlpLogsUri(buildOtlpLogsChannelUri(level)),
			})),
			cases.map((level) => ({
				level,
				uri: `ahp-otlp://logs/${level}`,
				parsed: level,
			})),
		);
	});
