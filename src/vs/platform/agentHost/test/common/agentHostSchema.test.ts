...
suite("agentHostSchema", () => {
	...
	suite("schemaProperty", () => {
		...
		test("assertValid path annotates array index and nested property", () => {
			...
			const err = captureProtocolError(() =>
	...
	suite("createSchema", () => {
		...
		test("toProtocol emits a JSON-Schema-compatible object", () => {
			...
			assert.deepStrictEqual(Object.keys(protocol.properties), [
				"name",
				"count",
				"level",
			]);
		...
		test("definition is preserved for spread-based composition", () => {
			...
			assert.deepStrictEqual(Object.keys(extended.toProtocol().properties), [
				"a",
				"b",
			]);
	...
	suite("validateOrDefault", () => {
		...
		test("uses defaults when input is undefined", () => {
			const schema = fixture();
			const result = schema.validateOrDefault(undefined, {
				name: "d",
				count: 7,
			});
		...
		test("omits schema keys that are missing from both values and defaults", () => {
			...
			assert.ok(
				...
				"`name` should be absent when neither values nor defaults supply it",
			);
		});
	...
	suite("platformSessionSchema", () => {
		test("validates the three autoApprove levels", () => {
			...
			for (const level of levels) {
				assert.strictEqual(
					platformSessionSchema.validate(SessionConfigKey.AutoApprove, level),
					true,
					level,
				);
			}
			assert.strictEqual(
				platformSessionSchema.validate(SessionConfigKey.AutoApprove, "bogus"),
				false,
			);
		});
		...
		test("validates permissions shape", () => {
			...
			assert.strictEqual(
				platformSessionSchema.validate(SessionConfigKey.Permissions, ok),
				true,
			);
			assert.strictEqual(
				platformSessionSchema.validate(SessionConfigKey.Permissions, {
					allow: [42],
					deny: [],
				}),
				false,
		...
		test("validates the agent modes", () => {
			...
			for (const mode of modes) {
				assert.strictEqual(
					platformSessionSchema.validate(SessionConfigKey.Mode, mode),
					true,
					mode,
				);
			}
			...
			assert.strictEqual(
				platformSessionSchema.validate(SessionConfigKey.Mode, "autopilot"),
				false,
			);
			assert.strictEqual(
				platformSessionSchema.validate(SessionConfigKey.Mode, "shell"),
				false,
			);
			assert.strictEqual(
				platformSessionSchema.validate(SessionConfigKey.Mode, 42),
				false,
			);
		});
	});
