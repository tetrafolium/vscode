...
} from "../../common/commandLineHelpers.js";
...
suite("extractCdPrefix", () => {
	...
	const cases: Array<{
		...
		{
			name: "bash: simple cd",
			commandLine: "cd /tmp && ls",
			isPowerShell: false,
			expected: { directory: "/tmp", command: "ls" },
		},
		{
			name: "bash: quoted dir",
			commandLine: 'cd "/path with spaces" && ls -la',
			isPowerShell: false,
			expected: { directory: "/path with spaces", command: "ls -la" },
		},
		{
			name: "bash: extra spaces after &&",
			commandLine: "cd /tmp &&  echo hi",
			isPowerShell: false,
			expected: { directory: "/tmp", command: "echo hi" },
		},
		...
		{
			name: "bash: cd with semicolon (not allowed in bash variant)",
			commandLine: "cd /tmp; ls",
			isPowerShell: false,
			expected: undefined,
		},
		...
		{
			name: "bash: Set-Location not bash",
			commandLine: "Set-Location /tmp && ls",
			isPowerShell: false,
			expected: undefined,
		},
		...
		{
			name: "pwsh: cd && ",
			commandLine: "cd C:\\foo && dir",
			isPowerShell: true,
			expected: { directory: "C:\\foo", command: "dir" },
		},
		{
			name: "pwsh: cd ;",
			commandLine: "cd C:\\foo; dir",
		...
		{
			name: "pwsh: cd /d",
			commandLine: "cd /d C:\\foo && dir",
			isPowerShell: true,
			expected: { directory: "C:\\foo", command: "dir" },
		},
		{
			name: "pwsh: Set-Location",
			commandLine: "Set-Location C:\\foo; dir",
			isPowerShell: true,
			expected: { directory: "C:\\foo", command: "dir" },
		},
		{
			name: "pwsh: Set-Location -Path",
			commandLine: "Set-Location -Path C:\\foo && dir",
			isPowerShell: true,
			expected: { directory: "C:\\foo", command: "dir" },
		},
		{
			name: "pwsh: quoted dir",
			commandLine: 'cd "C:\\path with spaces"; dir',
			isPowerShell: true,
			expected: { directory: "C:\\path with spaces", command: "dir" },
		},
		{
			name: "pwsh: case insensitive",
			commandLine: "CD C:\\foo && dir",
			isPowerShell: true,
			expected: { directory: "C:\\foo", command: "dir" },
		},
...
suite("stripRedundantCdPrefix", () => {
	...
	test("rewrites bash command when cd matches working directory", () => {
		const params: Record<string, unknown> = {
			command: "cd /repo/project && npm test",
		};
	...
	test("matches mixed path separators (forward-slash extracted vs native fsPath wd)", () => {
		...
		const params: Record<string, unknown> = {
			command: "cd C:/repo/project && npm test",
		};
