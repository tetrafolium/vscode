...
} from "../../../../../common/state/protocol/version/negotiation.js";
...
suite("Protocol version negotiation", () => {
	...
	test("negotiate picks the highest compatible offered version", () => {
		assert.strictEqual(
			negotiateProtocolVersion(["0.1.0", "0.1.2", "0.1.1"], "0.1.5"),
			"0.1.2",
		);
		assert.strictEqual(
			negotiateProtocolVersion(["0.1.0", "0.2.0"], "0.1.0"),
			"0.1.0",
		);
		assert.strictEqual(
			negotiateProtocolVersion(["0.0.5", "0.2.0"], "0.1.0"),
			undefined,
		);
		assert.strictEqual(negotiateProtocolVersion([], "0.1.0"), undefined);
		// Order of offered versions does not affect the result.
		assert.strictEqual(
			negotiateProtocolVersion(["0.1.2", "0.1.0"], "0.1.5"),
			"0.1.2",
		);
	});
});
