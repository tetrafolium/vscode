...
import {
	...
	formatTerminalText,
	removeServerHandledTerminalQueries,
	type ITerminalQueryFilterState,
} from "../../node/agentHostTerminalManager.js";
import {
	Osc633Event,
	Osc633EventType,
	Osc633Parser,
} from "../../node/osc633Parser.js";
...
class TestTerminalDataHandler {
	...
	cwd = "/home/user";
	private readonly _terminalQueryFilterState: ITerminalQueryFilterState = {
		pendingData: "",
	};
	...
	handlePtyData(rawData: string): string {
		...
		const cleanedData = removeServerHandledTerminalQueries(
			parseResult.cleanedData,
			this._terminalQueryFilterState,
		);
...
class TestPty implements IPty {
	...
	readonly onExit: IPty["onExit"] = (listener) =>
		this._onExit.event((data) => listener(data));
...
class TestAgentHostTerminalManager extends AgentHostTerminalManager {
	...
	protected override async _spawnPty(
		_file: string,
		_args: string[],
...
suite("AgentHostTerminalManager – command detection integration", () => {
	...
	test("formats command input with terminal enter semantics", () => {
		...
		assert.strictEqual(
			formatTerminalText("echo first\r", { shouldExecute: true }),
			"echo first\r",
		);
		assert.strictEqual(
			formatTerminalText("answer\n", { shouldExecute: false }),
			"answer\r",
		);
		assert.strictEqual(
			formatTerminalText("/tmp/foo\npwd", { shouldExecute: true }),
			"/tmp/foo\rpwd\r",
		);
		assert.strictEqual(
			formatTerminalText("echo first\necho second", {
				shouldExecute: true,
				forceBracketedPasteMode: true,
			}),
			"\x1b[200~echo first\recho second\x1b[201~\r",
		);
		assert.strictEqual(
			formatTerminalText("answer\n", {
				shouldExecute: false,
	...
	test("writes formatted command input to the PTY", async () => {
		...
		const configurationService = disposables.add(
		...
		const manager = disposables.add(
			new TestAgentHostTerminalManager(
				stateManager,
				logService,
				productService,
		...
		await manager.sendText(
			"agenthost-terminal://test/command-input",
			"echo first\necho second",
			{ shouldExecute: true },
		);
	...
	test("writes bracketed paste command input when enabled by the terminal", async () => {
		...
		await manager.sendText(
			"agenthost-terminal://test/bracketed-paste",
			"echo first\necho second",
			{ shouldExecute: true, bracketedPasteMode: true },
		);

		assert.deepStrictEqual(pty.writes, [
			"\x1b[200~echo first\recho second\x1b[201~\r",
		]);
	});
	...
	test("does not write bracketed paste command input when disabled by the terminal", async () => {
		...
		await manager.sendText(
			"agenthost-terminal://test/bracketed-paste-disabled",
			"echo first\necho second",
			{ shouldExecute: true, bracketedPasteMode: true },
		);
	...
	test("sets zsh agent fixups only for session zsh terminals", async () => {
		...
		const zshSessionManager = await createTestTerminal(
			"zsh-session-fixups",
			"/bin/zsh",
			{
		...
		assert.strictEqual(
			zshSessionManager.spawnOptions?.env?.VSCODE_AGENT_ZSH_FIXUPS,
			"1",
		);
		assert.strictEqual(
			zshSessionManager.spawnOptions?.env?.VSCODE_PREVENT_SHELL_HISTORY,
			"1",
		);

		const zshClientManager = await createTestTerminal(
			"zsh-client",
			"/bin/zsh",
			{
		...
		assert.strictEqual(
			zshClientManager.spawnOptions?.env?.VSCODE_AGENT_ZSH_FIXUPS,
			undefined,
		);

		const bashSessionManager = await createTestTerminal(
		...
		assert.strictEqual(
			bashSessionManager.spawnOptions?.env?.VSCODE_AGENT_ZSH_FIXUPS,
			undefined,
		);
		assert.strictEqual(
			bashSessionManager.spawnOptions?.env?.VSCODE_PREVENT_SHELL_HISTORY,
			"1",
		);
	});
	...
	test("resolves alt-buffer promise from headless terminal data", async () => {
		...
		const altBufferPromise = manager.createAltBufferPromise(
			uri,
			altBufferStore,
		);
	...
	test("server-handled CPR queries are stripped from client-facing data", () => {
		...
		assert.strictEqual(filter("before \x1b[?6n after"), "before  after");
		assert.strictEqual(
			filter("\x1b[5n\x1b[c\x1b[0c\x1b[>c\x1b[>0c"),
			"\x1b[5n\x1b[c\x1b[0c\x1b[>c\x1b[>0c",
		);
	...
	test("server-handled CPR queries are stripped across data chunks", () => {
		...
		assert.strictEqual(
			removeServerHandledTerminalQueries("before \x1b[", state),
			"before ",
		);
		assert.strictEqual(
			removeServerHandledTerminalQueries("6n after", state),
			" after",
		);
		...
		assert.strictEqual(
			removeServerHandledTerminalQueries("K after", state),
			"\x1b[K after",
		);
	});
	...
	test("manager data path strips CPR queries while preserving surrounding output", () => {
		...
		const cleaned = handler.handlePtyData(
			`before${osc633("A")}\x1b[6nmid\x1b[?6nafter`,
		);

		assert.strictEqual(cleaned, "beforemidafter");
		assert.deepStrictEqual(handler.content, [
			{ type: "unclassified", value: "beforemidafter" },
		]);
		assert.strictEqual(
			handler.dispatched[0].type,
			ActionType.TerminalCommandDetectionAvailable,
		);
	});
	...
	test("full command lifecycle dispatches correct actions", () => {
		...
		handler.handlePtyData(
			`${osc633("E;echo\\x20hello;test-nonce")}${osc633("C")}`,
		);
		// Command output
		...
		const executed = actions.find(
			(a) => a.type === ActionType.TerminalCommandExecuted,
		);
		assert.ok(executed);
		...
		const finished = actions.find(
			(a) => a.type === ActionType.TerminalCommandFinished,
		);
		assert.ok(finished);
	...
	test("nonce validation rejects untrusted command lines", () => {
		...
		const executed = handler.dispatched.find(
			(a) => a.type === ActionType.TerminalCommandExecuted,
		);
		assert.ok(executed);
	...
	test("multiple sequential commands get sequential IDs", () => {
		...
		const executed = handler.dispatched.filter(
			(a) => a.type === ActionType.TerminalCommandExecuted,
		);
		...
		const finished = handler.dispatched.filter(
			(a) => a.type === ActionType.TerminalCommandFinished,
		);
	...
	test("CWD property dispatches TerminalCwdChanged", () => {
		...
		const cwdAction = handler.dispatched.find(
			(a) => a.type === ActionType.TerminalCwdChanged,
		);
		assert.ok(cwdAction);
	...
	test("command output is accumulated in the command content part", () => {
		...
		assert.strictEqual(
			cmdParts[0].type === "command" && cmdParts[0].output,
			"line1\r\nline2\r\nline3\r\n",
		);
	});
});
