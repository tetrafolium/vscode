...
suite("AgentPluginManager", () => {
	...
	setup(() => {
		...
		manager = new AgentPluginManager(
			basePath,
			fileService,
			new NullLogService(),
		);
	...
	async function seedPluginDir(
		...
		for (const [fileName, content] of Object.entries(files)) {
			await fileService.writeFile(
				URI.joinPath(agentClientDir, fileName),
	...
	suite("syncCustomizations", () => {
		...
		test("returns error status without pluginDir when source missing", async () => {
			...
			assert.ok(
				results[0].customization.load?.kind === "error" &&
					results[0].customization.load.message,
			);
		...
		test("fires progress callback with changed customization status", async () => {
			...
			await manager.syncCustomizations(
				"test-client",
				[makeRef("prog", "n1")],
				(status) => {
			...
			assert.deepStrictEqual(
				progressCalls.map((call) => call.load?.kind),
				["loaded"],
			);
		});
		...
		test("skips copy when nonce matches", async () => {
			...
			assert.strictEqual(
				result1[0].pluginDir!.toString(),
				result2[0].pluginDir!.toString(),
			);
		});
	...
	suite("LRU eviction", () => {
		test("evicts least recently used plugins when limit exceeded", async () => {
			const smallManager = new AgentPluginManager(
				basePath,
				fileService,
				new NullLogService(),
				3,
			...
			for (let i = 1; i <= 4; i++) {
				...
				await smallManager.syncCustomizations("test-client", [
					makeRef(`plugin-${i}`, `n${i}`),
				]);
			}
			...
			assert.strictEqual(
				pluginDirs.length,
				3,
				"should have exactly 3 plugin dirs after eviction",
			);
		});
	});
	...
	suite("cache persistence", () => {
		test("restores nonce cache from disk on new manager instance", async () => {
			...
			const manager2 = new AgentPluginManager(
				basePath,
				fileService,
				new NullLogService(),
			);
