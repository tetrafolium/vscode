...
suite("copilotToolDisplay — edit tool classification", () => {
	...
	test("classifies str_replace_editor by command", () => {
		for (const command of ["edit", "str_replace", "insert", "create"]) {
			assert.strictEqual(
				isEditTool("str_replace_editor", command),
				true,
				command,
			);
		}
...
suite("getPermissionDisplay — cd-prefix stripping", () => {
	...
	test("does not affect non-shell custom-tool requests", () => {
		...
		assert.ok(
			display.toolInput?.includes("cd /repo/project"),
			`expected unrewritten args, got: ${display.toolInput}`,
		);
...
suite("view tool — view_range display", () => {
	...
	test('renders "lines X to Y" for a valid two-element range', () => {
		assert.ok(
			invocation({ path: "/repo/file.ts", view_range: [10, 20] }).endsWith(
				", lines 10 to 20",
			),
		);
		assert.ok(
			pastTense({ path: "/repo/file.ts", view_range: [10, 20] }).endsWith(
				", lines 10 to 20",
			),
		);
	});
	...
	test('renders "line X" when start === end', () => {
		assert.ok(
			invocation({ path: "/repo/file.ts", view_range: [10, 10] }).endsWith(
				", line 10",
			),
		);
		assert.ok(
			pastTense({ path: "/repo/file.ts", view_range: [10, 10] }).endsWith(
				", line 10",
			),
		);
	});
	...
	test('renders "line X to the end" for the -1 EOF sentinel', () => {
		assert.ok(
			invocation({ path: "/repo/file.ts", view_range: [10, -1] }).endsWith(
				", line 10 to the end",
			),
		);
		assert.ok(
			pastTense({ path: "/repo/file.ts", view_range: [10, -1] }).endsWith(
				", line 10 to the end",
			),
		);
	});
	...
	test("falls back to path-only for invalid ranges", () => {
		// end < start (and not -1)
		assert.ok(
			!invocation({ path: "/repo/file.ts", view_range: [20, 10] }).includes(
				",",
			),
		);
		// negative start
		assert.ok(
			!invocation({ path: "/repo/file.ts", view_range: [-5, 10] }).includes(
				",",
			),
		);
		// non-integer
		assert.ok(
			!invocation({ path: "/repo/file.ts", view_range: [1.5, 10] }).includes(
				",",
			),
		);
		// wrong arity
		assert.ok(
			!invocation({ path: "/repo/file.ts", view_range: [10] }).includes(","),
		);
		assert.ok(
			!invocation({ path: "/repo/file.ts", view_range: [10, 20, 30] }).includes(
				",",
			),
		);
		// non-array
		assert.ok(
			!invocation({ path: "/repo/file.ts", view_range: "whatever" }).includes(
				",",
			),
		);
	});
...
suite("copilotToolDisplay — write_/read_ shell tools", () => {
	...
	suite("getInvocationMessage", () => {
		...
		test("write_bash with command includes the command text", () => {
			const msg = getInvocationMessage("write_bash", "Write Shell Input", {
				command: "echo hello",
			});
			assert.ok(
				getText(msg).includes("echo hello"),
				`expected 'echo hello' in: ${getText(msg)}`,
			);
		});

		test("write_bash without command returns a non-empty fallback message", () => {
			const msg = getInvocationMessage(
				"write_bash",
				"Write Shell Input",
				undefined,
			);
		...
		test("write_powershell with command includes the command text", () => {
			...
			assert.ok(
				getText(msg).includes("Get-Date"),
				`expected 'Get-Date' in: ${getText(msg)}`,
			);
		});
		...
		test("write_bash message differs from bash message (distinct wording)", () => {
			const writeBashMsg = getText(
				getInvocationMessage("write_bash", "Write Shell Input", {
					command: "echo hi",
				}),
			);
			const bashMsg = getText(
				getInvocationMessage("bash", "Bash", { command: "echo hi" }),
			);
	...
	suite("getToolInputString", () => {
		test("write_bash extracts command field", () => {
			assert.strictEqual(
				getToolInputString("write_bash", { command: "echo hello" }, undefined),
				"echo hello",
			);
		});
		...
		test("write_bash falls back to rawArguments when no command field", () => {
			assert.strictEqual(
				getToolInputString("write_bash", {}, '{"command":"echo hello"}'),
				'{"command":"echo hello"}',
			);
		});
		...
		test("write_bash returns undefined when both parameters and rawArguments are absent", () => {
			assert.strictEqual(
				getToolInputString("write_bash", undefined, undefined),
				undefined,
			);
		});
		...
		test("read_bash with no parameters returns undefined", () => {
			assert.strictEqual(
				getToolInputString("read_bash", undefined, undefined),
				undefined,
			);
		});
	});
...
suite("skill events", () => {
	...
	test("hides the raw `skill` tool call and synthesizes a tool-start/complete pair from `skill.invoked`", () => {
		...
		const noPath = synthesizeSkillToolCall({ name: "plan" }, undefined);
		...
		assert.deepStrictEqual(
			...
			},
			{
				...
				withPathInvocation: {
					markdown:
						"Reading skill [plan](file:///abs/repo/skills/plan/SKILL.md)",
				},
				withPathPastTense: {
					markdown: "Read skill [plan](file:///abs/repo/skills/plan/SKILL.md)",
				},
...
suite("rg / grep search tool display", () => {
	...
	test('rg invocation/past tense use "Searching for {pattern}" wording', () => {
		...
		const past = text(
			getPastTenseMessage("rg", "Search", { pattern: "foo" }, true),
		);
		assert.deepStrictEqual(
			{ inv, past },
			{
	...
	test('grep keeps "Searching for {pattern}" wording', () => {
		const inv = text(
			getInvocationMessage("grep", "Search", { pattern: "bar" }),
		);
		const past = text(
			getPastTenseMessage("grep", "Search", { pattern: "bar" }, true),
		);
		assert.deepStrictEqual(
			{ inv, past },
			{
	...
	test("getToolInputString returns pattern for both grep and rg", () => {
		assert.strictEqual(
			getToolInputString("grep", { pattern: "abc" }, undefined),
			"abc",
		);
		assert.strictEqual(
			getToolInputString("rg", { pattern: "abc" }, undefined),
			"abc",
		);
	});
});
...
suite("web_fetch tool display", () => {
	...
	function text(
		msg:
			| ReturnType<typeof getInvocationMessage>
			| ReturnType<typeof getPastTenseMessage>,
	): string {
		return typeof msg === "string" ? msg : msg.markdown;
	}
	...
	test("uses the fetched URL for invocation and completion messages", () => {
		...
		assert.deepStrictEqual(
			{
				invocation: text(
					getInvocationMessage("web_fetch", "Fetch Web Content", parameters),
				),
				pastTense: text(
					getPastTenseMessage(
						"web_fetch",
			...
			{
				invocation:
					"Fetching [https://example.com/docs](https://example.com/docs)",
				pastTense:
					"Fetched [https://example.com/docs](https://example.com/docs)",
	...
	test("falls back to generic URL wording when the URL is absent", () => {
		assert.deepStrictEqual(
			{
				invocation: text(
					getInvocationMessage("web_fetch", "Fetch Web Content", undefined),
				),
				pastTense: text(
					getPastTenseMessage(
						"web_fetch",
...
suite("sql tool display", () => {
	...
	test("uses the SQL description for invocation and completion messages", () => {
		const parameters = {
			description: "Insert agent host study todos",
			query:
				"INSERT INTO todos (title) VALUES ('Read terminal activation docs')",
		};
		assert.strictEqual(
			text(getInvocationMessage("sql", "Execute SQL", parameters)),
			"Insert agent host study todos",
		);
		assert.strictEqual(
			text(getPastTenseMessage("sql", "Execute SQL", parameters, true)),
			"Insert agent host study todos",
		);
	});
	...
	test("falls back to generic SQL wording when description is absent", () => {
		assert.strictEqual(
			text(getInvocationMessage("sql", "Execute SQL", { query: "SELECT 1" })),
			"Executing SQL query",
		);
		assert.strictEqual(
			text(
				getPastTenseMessage("sql", "Execute SQL", { query: "SELECT 1" }, true),
			),
			"Executed SQL query",
		);
	});
...
suite("apply_patch tool display", () => {
	...
	test("renders a clickable file link for a single-file patch", () => {
		const inv = text(
			getInvocationMessage("apply_patch", "Patch", { input: singleFilePatch }),
		);
		const past = text(
			getPastTenseMessage(
				"apply_patch",
				"Patch",
				{ input: singleFilePatch },
				true,
			),
		);
		assert.deepStrictEqual(
	...
	test("lists every affected file for a multi-file patch", () => {
		const inv = text(
			getInvocationMessage("apply_patch", "Patch", { input: multiFilePatch }),
		);
		const past = text(
			getPastTenseMessage(
				"apply_patch",
	...
	test("falls back to a generic message when the patch body is missing or unparseable", () => {
		assert.strictEqual(
			getInvocationMessage("apply_patch", "Patch", undefined),
			"Editing files",
		);
		assert.strictEqual(
			getInvocationMessage("apply_patch", "Patch", { input: "not a patch" }),
			"Editing files",
		);
		assert.strictEqual(
			getPastTenseMessage("apply_patch", "Patch", undefined, true),
			"Edited files",
		);
	});
	...
	test("also accepts the patch text under the `patch` parameter (CLI shape)", () => {
		const inv = text(
			getInvocationMessage("apply_patch", "Patch", { patch: singleFilePatch }),
		);
	...
	test("git_apply_patch shares the same display path", () => {
		const inv = text(
			getInvocationMessage("git_apply_patch", "Patch", {
				input: singleFilePatch,
			}),
		);
		const past = text(
	...
	test("getEditFilePath returns the first affected file from a patch body", () => {
		assert.strictEqual(
			getEditFilePath({ input: singleFilePatch }),
			"/repo/src/foo.ts",
		);
		assert.strictEqual(
			getEditFilePath({ input: multiFilePatch }),
			"/repo/src/foo.ts",
		);
		assert.strictEqual(
			getEditFilePath({ patch: singleFilePatch }),
			"/repo/src/foo.ts",
		);
		assert.strictEqual(
			getEditFilePath(JSON.stringify({ input: singleFilePatch })),
			"/repo/src/foo.ts",
		);
	...
	test("getEditFilePaths returns every affected file from a patch body", () => {
		assert.deepStrictEqual(getEditFilePaths({ input: singleFilePatch }), [
			"/repo/src/foo.ts",
		]);
		assert.deepStrictEqual(getEditFilePaths({ input: multiFilePatch }), [
			"/repo/src/foo.ts",
			"/repo/src/bar.ts",
			"/repo/src/baz.ts",
		]);
		assert.deepStrictEqual(getEditFilePaths({ patch: multiFilePatch }), [
			"/repo/src/foo.ts",
			"/repo/src/bar.ts",
			"/repo/src/baz.ts",
		]);
		assert.deepStrictEqual(
			getEditFilePaths(JSON.stringify({ input: multiFilePatch })),
			["/repo/src/foo.ts", "/repo/src/bar.ts", "/repo/src/baz.ts"],
		);
		assert.deepStrictEqual(getEditFilePaths({ path: "/repo/src/edit.ts" }), [
			"/repo/src/edit.ts",
		]);
		...
		assert.deepStrictEqual(getEditFilePaths(multiFilePatch), [
			"/repo/src/foo.ts",
			"/repo/src/bar.ts",
			"/repo/src/baz.ts",
		]);
		assert.deepStrictEqual(getEditFilePaths(singleFilePatch), [
			"/repo/src/foo.ts",
		]);
	});
});
