...
import {
	...
	resolveGitProject,
} from "../../node/copilot/copilotGitProject.js";
...
suite("Copilot Git Project", () => {
	...
	test("resolves a repository project from a worktree working directory", async () => {
		...
		const project = await resolveGitProject(
			URI.file("/workspace/worktree-checkout"),
			gitService,
		);
	...
	test("resolves the repository itself for a normal git working directory", async () => {
		...
		const project = await resolveGitProject(
			URI.file("/workspace/normal-repo"),
			gitService,
		);
	...
	test("parses repository URLs", () => {
		const project = projectFromRepository(
