...
suite("ClaudeSdkCustomizationBundler", () => {
	...
	setup(() => {
		...
		inst.stub(IAgentPluginManager, {
			...
			syncCustomizations: async (
				_clientId: string,
				_refs: readonly ClientPluginCustomization[],
			): Promise<ISyncedCustomization[]> => [],
		} satisfies Partial<IAgentPluginManager> as unknown as IAgentPluginManager);
		bundler = disposables.add(
			inst.createInstance(ClaudeSdkCustomizationBundler, workingDir),
		);
	});
	...
	function snapshot(
		overrides: Partial<ISdkResolvedCustomizations> = {},
	): ISdkResolvedCustomizations {
		return {
			commands: [],
			agents: [],
			mcpServers: [],
	...
	test("writes manifest, agent files, and skill subdirs for a snapshot with agents and commands", async () => {
		const result = await bundler.bundle(
			snapshot({
				agents: [
					{ name: "planner", description: "Plans things", model: "claude" },
				],
				commands: [
					{ name: "doit", description: "Does it", argumentHint: "<x>" },
				],
			}),
		);
	...
	test("children include agent customizations sourced from the SDK snapshot with on-disk file URIs", async () => {
		...
		const agents = result!.children!.filter(
		...
		assert.ok(
			agents[0].uri.endsWith("/agents/a1.md"),
			`expected on-disk path, got ${agents[0].uri}`,
		);
	...
	test("repeated bundle with same snapshot is nonce-stable and does not rewrite", async () => {
		...
		assert.strictEqual(
			stat1.mtime,
			stat2.mtime,
			"unchanged snapshot must not rewrite the on-disk tree",
		);
	});
	...
	test("changed snapshot deletes prior bundle tree before writing the new one", async () => {
		...
		const rootUri = URI.parse(result!.uri);
		assert.ok(
			await fileService.exists(URI.joinPath(rootUri, "agents", "new.md")),
		);
		assert.ok(
			!(await fileService.exists(URI.joinPath(rootUri, "agents", "old.md"))),
			"previous agent file should be deleted",
		);
	});
	...
	test('sanitises agent and command names — invalid chars replaced, length capped, empty falls back to "unnamed"', async () => {
		...
		assert.ok(
			await fileService.exists(
				URI.joinPath(rootUri, "agents", "has_spaces___slashes_here.md"),
			),
		);
		assert.ok(
			await fileService.exists(
				URI.joinPath(rootUri, "agents", `${"a".repeat(128)}.md`),
			),
		);
		assert.ok(
			await fileService.exists(URI.joinPath(rootUri, "agents", "___.md")),
		);
	});
	...
	test("discoverable bundles for different working directories namespace by hash so they do not collide", async () => {
		...
		const a = await bundler.bundle(
			snapshot({ agents: [{ name: "x", description: "d", model: "m" }] }),
		);
		const b = await other.bundle(
			snapshot({ agents: [{ name: "x", description: "d", model: "m" }] }),
		);
