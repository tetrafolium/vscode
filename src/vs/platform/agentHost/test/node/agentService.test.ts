...
async function loadFixtureMessages(fixtureName: string, session: URI) {
	...
	const srcFile = thisFile.replace(/[/\\]out[/\\]/, (m) =>
		m.replace("out", "src"),
	);
	...
	const raw = readFileSync(
		`${fixtureDir}${sep}test-cases${sep}${fixtureName}`,
		"utf-8",
	);
	const events: ISessionEvent[] = raw
		.trim()
		.split("\n")
...
suite("AgentService (node dispatcher)", () => {
	...
	setup(async () => {
		...
		await fileService.createFolder(
			URI.from({ scheme: Schemas.inMemory, path: "/testDir" }),
		);
		await fileService.writeFile(
			URI.from({ scheme: Schemas.inMemory, path: "/testDir/file.txt" }),
	...
	suite("registerProvider", () => {
		...
		test("throws on duplicate provider registration", () => {
			...
			assert.throws(
				() => service.registerProvider(duplicate),
				/already registered/,
			);
		});
		...
		test("maps progress events to protocol actions via onDidAction", async () => {
			...
			assert.ok(
				envelopes.some((e) => e.action.type === ActionType.SessionResponsePart),
			);
		});
	});
				...
				for (let attempt = 0; attempt < 20; attempt++) {
					try {
						const parsed = JSON.parse(
							readFileSync(rootConfigResource.fsPath, "utf8"),
						);
						assert.deepStrictEqual(parsed.customizations, [customization]);
						persisted = true;
						break;
					} catch {
	...
	suite("user-message attachment rewriting", () => {
		...
		async function dispatchTurnAndWait(
			svc: AgentService,
			agent: MockAgent,
			session: URI,
			attachments:
		...
		test("snapshots EmbeddedResource attachments to disk and rewrites to a Resource URI under the session attachments folder", async () => {
			...
			assert.ok(
				a.uri.startsWith(attachmentsRoot.toString() + "/"),
				`attachment uri ${a.uri} should be under ${attachmentsRoot.toString()}/`,
			);
		...
		test("snapshots Resource attachments by reading the original file and rewriting to a local snapshot", async () => {
			...
			assert.notStrictEqual(
				rewritten.uri,
				sourceUri.toString(),
				`should be rewritten to the snapshot URI; warnings=${JSON.stringify(warnings)}; got ${rewritten.uri}`,
			);
		...
		test("preserves selection range on Resource rewrite", async () => {
			...
			assert.ok(
				rewritten.uri.startsWith(attachmentsRoot.toString() + "/"),
				"should be rewritten to a snapshot URI",
			);
		...
		test("does not re-snapshot attachments that already point under the session attachments folder", async () => {
			...
			await fileService.writeFile(
				existing,
				VSBuffer.fromString("already snapshotted"),
			);
			...
			assert.deepStrictEqual(
				{
					...
					listed: sessions.some(
						(s) => s.session.toString() === requestedSession.toString(),
					),
				},
				{
	...
	suite("aggregation", () => {
		...
		test("listSessions overlays custom title from session database", async () => {
			...
			const sessionDataService: ISessionDataService = {
				...
				tryOpenDatabase: async (): Promise<
					IReference<ISessionDatabase> | undefined
				> => ({
					object: db,
					dispose: () => {},
				}),
		...
		test("listSessions advertises persisted changeset counts without seeding state; changeset subscribe restores lazily", async () => {
			...
			assert.deepStrictEqual(
				{
					listCatalogueEntry: sessions[0].changesets?.find(
						(c) => c.uriTemplate === changesetUri,
					),
			...
			const snapshot = await svc.subscribe(
				URI.parse(changesetUri),
				"client-changeset",
			);
		...
		test("listSessions prefers ready live changeset state over stale persisted diffs for unopened sessions", async () => {
			...
			const persistedDiffs = [
				{
					after: {
						uri: "file:///wd/x.ts",
						content: { uri: "file:///wd/x.ts" },
					},
					diff: { added: 99, removed: 0 },
				},
				{
					after: {
						uri: "file:///wd/y.ts",
						content: { uri: "file:///wd/y.ts" },
					},
			...
			const changesetUri = svc.stateManager.registerChangeset(
		...
		test("listSessions does not request the diffs metadata key when a live source can answer", async () => {
			...
			const db: ISessionDatabase = {
				...
				getMetadataObject: async <T extends Record<string, unknown>>(
					obj: T,
				): Promise<{ [K in keyof T]: string | undefined }> => {
					requestedKeys.push(Object.keys(obj));
					return Object.fromEntries(
						Object.keys(obj).map((k) => [k, undefined]),
			...
			assert.strictEqual(
				requestedKeys[0].includes("diffs"),
				false,
				`expected listSessions to skip the 'diffs' key when ready live changeset state exists; requested=${requestedKeys[0].join(",")}`,
			);
		});
		...
		test("listSessions still reads persisted diffs when only a computing (not ready) changeset state exists", async () => {
			...
			const persistedDiffs = [
				{
					after: {
						uri: "file:///wd/p.ts",
						content: { uri: "file:///wd/p.ts" },
					},
					diff: { added: 7, removed: 1 },
				},
			];
			await db.setMetadata("diffs", JSON.stringify(persistedDiffs));
			...
			svc.stateManager.registerChangeset(
				buildSessionChangesetUri(sessionUri.toString()),
			);
		...
		test("createSession attaches git state into state _meta when working directory is present", async () => {
			...
			const gitService = {
				...
				getSessionGitState: async (uri: URI) => {
			...
			const localService = disposables.add(
				new AgentService(
					new NullLogService(),
					fileService,
					nullSessionDataService,
		...
		test("createSession keeps git-only catalogue entries for a git working directory", async () => {
			...
			assert.deepStrictEqual(state!.summary.changesets, [
				{
					label: "Branch Changes",
					uriTemplate: `${session.toString()}/changeset/session`,
					description: "main",
				},
				{
					label: "Uncommitted Changes",
					uriTemplate: `${session.toString()}/changeset/uncommitted`,
					description: "Show uncommitted changes in this session",
				},
			]);
		});
		...
		test("subscribe lazily attaches git state when an existing session has no _meta.git", async () => {
			...
			const calls: string[] = [];
			const gitService = createNoopGitService();
			gitService.getSessionGitState = async (uri: URI) => {
				calls.push(uri.fsPath);
				return gitState;
			};
			const localService = disposables.add(
		...
		test("createSession stores live session config", async () => {
			...
			assert.deepStrictEqual(
				service.stateManager.getSessionState(session.toString())?.config
					?.values,
				config,
			);
		});
		...
		test("seeds activeClient into the initial session state when provided", async () => {
				...
				customizations: [
					{
						...
						id: customizationId("file:///plugin-a"),
						uri: "file:///plugin-a",
						name: "A",
						enabled: true,
					},
			...
			assert.deepStrictEqual(
				{
					activeClient: service.stateManager.getSessionState(session.toString())
						?.activeClient,
					dispatchedActiveClientChanged: envelopes.some(
						(e) => e.action.type === ActionType.SessionActiveClientChanged,
					),
				},
				{
					activeClient,
		...
		test("omits activeClient from the initial session state when not provided", async () => {
			...
			assert.strictEqual(
				service.stateManager.getSessionState(session.toString())?.activeClient,
				undefined,
			);
		});
	});
	...
	suite("authenticate", () => {
		test("routes token to provider matching the resource", async () => {
			...
			assert.deepStrictEqual(copilotAgent.authenticateCalls, [
				{ resource: "https://api.github.com", token: "ghp_test123" },
			]);
		});
		...
		test("fans out to every provider that owns the resource", async () => {
			...
			claudeAgent.getProtectedResources = () => [
				{
					resource: "https://api.github.com",
		...
		test("isolates a provider that throws — others still authenticate", async () => {
			...
			flakyAgent.getProtectedResources = () => [
				{
					resource: "https://api.github.com",
					authorization_servers: ["https://github.com/login/oauth"],
					required: true,
	...
	suite("restoreSession", () => {
		test("restores a session with message history", async () => {
			...
			const mdPart = state!.turns[0].responseParts.find(
				(p): p is MarkdownResponsePart => p.kind === ResponsePartKind.Markdown,
			);
			assert.ok(mdPart);
		...
		test("restores a session with tool calls", async () => {
			...
			copilotAgent.sessionMessages = [
				...
				},
				{
					...
					toolRequests: [{ toolCallId: "tc-1", name: "shell" }],
				},
				{
					type: "tool_start",
					session,
			...
			const toolCallParts = turn.responseParts.filter(
				(p): p is ToolCallResponsePart => p.kind === ResponsePartKind.ToolCall,
			);
		...
		test("restores a session with subagent tool calls", async () => {
			...
			copilotAgent.sessionMessages = [
				...
				},
				{
					...
					messageId: "msg-2",
					content: "",
					toolRequests: [{ toolCallId: "tc-sub", name: "task" }],
				},
				{
					type: "tool_start",
					session,
					...
					subagentDescription: "Find related files",
				...
				},
				{
					...
					result: {
						...
						content: [{ type: ToolResultContentType.Text, text: "file1.ts" }],
					},
					parentToolCallId: "tc-sub",
				},
				{
				...
				},
				{
					...
					session,
					toolCallId: "tc-inner-2",
					result: { success: true, pastTenseMessage: "Read file1.ts" },
					parentToolCallId: "tc-sub",
				},
				// Parent tool completes
				{
					...
					result: {
						...
						content: [
							{ type: ToolResultContentType.Text, text: "Found 3 issues" },
						],
					},
				},
				{
					...
					role: "assistant",
					messageId: "msg-3",
					content: "The review found 3 issues.",
					toolRequests: [],
				},
			];
			...
			assert.strictEqual(
				state!.turns.length,
				1,
				`Expected 1 turn but got ${state!.turns.length}`,
			);
			...
			assert.strictEqual(
				...
				1,
				`Expected 1 tool call (parent only) but got ${toolCallParts.length}`,
			);
			...
			assert.strictEqual(
				parentTc._meta?.subagentDescription,
				"Find related files",
			);
			...
			const subagentEntry = content.find(
				(c) =>
					hasKey(c, { type: true }) &&
			...
			assert.ok(
				subagentEntry,
				"Completed tool call should have subagent content entry",
			);
			...
			assert.strictEqual(
				childState!.turns.length,
				1,
				"Child session should have 1 turn",
			);
			const childToolParts = childState!.turns[0].responseParts.filter(
				(p): p is ToolCallResponsePart => p.kind === ResponsePartKind.ToolCall,
			);
			assert.strictEqual(
				childToolParts.length,
				2,
				`Child session should have 2 inner tool calls but got ${childToolParts.length}`,
			);
			assert.ok(
				childToolParts.some((p) => p.toolCall.toolCallId === "tc-inner-1"),
				"Should have tc-inner-1",
			);
			assert.ok(
				childToolParts.some((p) => p.toolCall.toolCallId === "tc-inner-2"),
				"Should have tc-inner-2",
			);

			// The turn should also have the final markdown
			const mdParts = turn.responseParts.filter(
				(p): p is MarkdownResponsePart => p.kind === ResponsePartKind.Markdown,
			);
			assert.ok(
				mdParts.some((p) => p.content.includes("3 issues")),
				"Should have the final markdown response",
			);
		});
		...
		test("inner assistant messages from subagent do not create extra turns (fixture)", async () => {
			...
			copilotAgent.sessionMessages = await loadFixtureMessages(
			...
			assert.strictEqual(
				state!.turns.length,
				1,
				`Expected 1 turn but got ${state!.turns.length}: ${state!.turns.map((t) => `"${t.message.text.substring(0, 40)}"`).join(", ")}`,
			);
			assert.strictEqual(
				state!.turns[0].message.text,
				"Run a sync subagent to do some searches, just testing subagent rendering",
			);
			assert.strictEqual(state!.turns[0].state, TurnState.Complete);

			// Should have the parent subagent tool call with subagent content
			const toolCallParts = state!.turns[0].responseParts.filter(
				(p): p is ToolCallResponsePart => p.kind === ResponsePartKind.ToolCall,
			);
			const parentTc = toolCallParts.find(
				(p) => p.toolCall.toolName === "task",
			);
			...
			const nonParentTools = toolCallParts.filter(
				(p) => p.toolCall.toolCallId !== parentToolCallId,
			);
			assert.strictEqual(
				nonParentTools.length,
				0,
				`Parent turn should only contain the task tool call, but found ${nonParentTools.length} extra tool calls`,
			);
			...
			assert.ok(
				...
				`Child session should have inner tool calls but got ${childToolParts.length}`,
			);

			// Should have the final markdown
			const mdParts = state!.turns[0].responseParts.filter(
				(p): p is MarkdownResponsePart => p.kind === ResponsePartKind.Markdown,
			);
	...
	suite("subscriber refcount eviction", () => {
		test("an empty session created in this lifetime stays observable until GC fires", async () => {
			service.registerProvider(copilotAgent);
			const sessionResource = await service.createSession({
				provider: "copilot",
			});
			...
			assert.ok(
				service.stateManager.getSessionState(sessionResource.toString()),
				"empty created session must remain observable for the GC grace window",
			);
		});
		...
		test("a session with an active turn is NOT evicted when its last subscriber drops", async () => {
			...
			assert.ok(
				service.stateManager.getSessionState(sessionResource.toString()),
				"active-turn session must not be evicted",
			);
		});
		...
		test("a restored idle session is evicted when its last subscriber drops", async () => {
			...
			assert.strictEqual(
				service.stateManager.getSessionState(sessionResource.toString()),
				undefined,
				"restored idle session should be evicted",
			);
		});
		...
		test("subagent subscriber pins the parent session against eviction", async () => {
			...
			copilotAgent.sessionMessages = [
				...
				},
				{
					...
					result: {
						success: true,
						pastTenseMessage: "ran",
						content: [{ type: ToolResultContentType.Text, text: "a" }],
					},
					parentToolCallId: "tc-sub",
				},
				{
			...
			await service.restoreSession(sessionResource);
			const childUri = URI.parse(
				buildSubagentSessionUri(sessionResource.toString(), "tc-sub"),
			);
			...
			assert.ok(
				service.stateManager.getSessionState(sessionResource.toString()),
				"parent must stay while child is subscribed",
			);
			assert.ok(
				service.stateManager.getSessionState(childUri.toString()),
				"child still present",
			);
			...
			assert.strictEqual(
				service.stateManager.getSessionState(sessionResource.toString()),
				undefined,
				"parent evicted after subagent drops",
			);
			assert.strictEqual(
				service.stateManager.getSessionState(childUri.toString()),
				undefined,
				"child also evicted with parent",
			);
		});
		...
		test("nested subagent subscriber pins ancestor session against eviction", async () => {
			...
			await service.restoreSession(sessionResource);
			const childUri = URI.parse(
				buildSubagentSessionUri(sessionResource, "tc-sub"),
			);
			await service.subscribe(childUri, "client-child");
			const nestedChildUri = URI.parse(
				buildSubagentSessionUri(childUri, "tc-nested"),
			);
			...
			assert.ok(
				service.stateManager.getSessionState(sessionResource.toString()),
				"ancestor parent must stay while nested child is subscribed",
			);
			assert.ok(
				service.stateManager.getSessionState(childUri.toString()),
				"intermediate child still present",
			);
		});
		...
		test("depth-2 subagent eviction evicts the root session state", async () => {
			...
			assert.strictEqual(
				service.stateManager.getSessionState(sessionResource.toString()),
				undefined,
				"root state must be evicted when no subscribers remain",
			);
		});
	});
	...
	suite("addSubscriber triggers uncommitted refresh", () => {
		test("addSubscriber for <session>/changeset/uncommitted triggers the first git diff refresh", async () => {
			const workingDirectory = URI.from({
				scheme: Schemas.inMemory,
				path: "/wd-refresh",
			});
			...
			gitService.computeSessionFileDiffs = async (
				wd: URI,
				opts: { sessionUri: string; baseBranch?: string },
			) => {
			...
			const sessionResource = await localService.createSession({
				provider: "copilot",
			});
			const uncommittedUri = URI.parse(
				buildUncommittedChangesetUri(sessionResource.toString()),
			);
			...
			assert.ok(
				computeCalls.some(
					(c) =>
						c.baseBranch === undefined && c.wd === workingDirectory.toString(),
				),
		...
		test("addSubscriber for the session URI or session-changeset URI triggers a static refresh", async () => {
			...
			const sessionChangesetUri = URI.parse(
				buildSessionChangesetUri(sessionResource.toString()),
			);
	...
	suite("empty-session GC", () => {
		test("an empty unsubscribed session is disposed after the grace period", () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(
					copilotAgent.disposeSessionCalls.length,
					0,
					"no GC before grace expires",
				);
		...
		test("a session with at least one turn is not GC-disposed", () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(
					copilotAgent.disposeSessionCalls.length,
					0,
					"session with turns must not be GC-disposed",
				);
			});
		});
		...
		test("resubscribe within the grace period cancels GC", () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(
					copilotAgent.disposeSessionCalls.length,
					0,
					"GC must be cancelled after resubscribe",
				);
			});
		});
		...
		test("GC is rearmed after a resubscribe-then-unsubscribe cycle", () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(
					copilotAgent.disposeSessionCalls.length,
					0,
					"rearmed timer not yet fired",
				);
				await new Promise((resolve) => setTimeout(resolve, 2_000));
				assert.strictEqual(
					copilotAgent.disposeSessionCalls.length,
					1,
					"rearmed timer fires after fresh 30s",
				);
			});
		});
		...
		test("createSession on the same URI cancels a pending GC", () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				const sessionResource = await service.createSession({
					provider: "copilot",
					session: AgentSession.uri("copilot", "recreate-test"),
				});
				service.addSubscriber(sessionResource, "client-1");
				service.unsubscribe(sessionResource, "client-1");
				...
				assert.strictEqual(
					copilotAgent.disposeSessionCalls.length,
					0,
					"createSession on same URI must cancel pending GC",
				);
			});
		});
	});
	...
	suite("session config persistence", () => {
		test("createSession persists initial config values to the session DB", async () => {
			...
			assert.deepStrictEqual(JSON.parse(persisted!), {
		...
		test("restoreSession overlays persisted config values onto the resolved config", async () => {
			...
			await sessionDb.setMetadata(
				"configValues",
				JSON.stringify({ autoApprove: "autoApprove" }),
			);
		...
		test("restoreSession seeds the session changeset from persisted diffs", async () => {
			...
			const changesetSnapshot = localService.stateManager.getSnapshot(
				`${sessionResource.toString()}/changeset/session`,
			);
			assert.ok(changesetSnapshot);
			const changesetState = changesetSnapshot.state as {
				status: string;
				files: Array<{ id: string }>;
			};
			assert.strictEqual(changesetState.status, "ready");
			assert.deepStrictEqual(
				changesetState.files.map((f) => f.id),
				["file:///wd/a.ts"],
			);
		});
			...
			await assert.rejects(
				() =>
					service.resourceList(
						URI.from({ scheme: Schemas.inMemory, path: "/nonexistent" }),
					),
				/Directory not found/,
			);
		});
	...
	suite("worktree working directory", () => {
		test("createSession uses agent-resolved working directory in state", async () => {
			...
			assert.strictEqual(
				state?.summary.workingDirectory,
		...
		test("restoreSession uses agent working directory in state", async () => {
			...
			service.stateManager.deleteSession(session.toString());
			assert.strictEqual(
				service.stateManager.getSessionState(session.toString()),
				undefined,
			);
	...
	suite("item-2: initial changeset seeding at create time", () => {
		...
		function assertBackingChangesetsComputing(
			stateManager: AgentService["stateManager"],
			sessionStr: string,
		): void {
			const uncommitted = stateManager.getSnapshot(
				buildUncommittedChangesetUri(sessionStr),
			);
			const sessionWide = stateManager.getSnapshot(
				buildSessionChangesetUri(sessionStr),
			);
			assert.ok(
				uncommitted,
				`expected ${sessionStr}/changeset/uncommitted to be subscribable`,
			);
			assert.ok(
				sessionWide,
				`expected ${sessionStr}/changeset/session to be subscribable`,
			);
			assert.strictEqual(
				(uncommitted.state as { status: string }).status,
				ChangesetStatus.Computing,
			);
			assert.strictEqual(
				(sessionWide.state as { status: string }).status,
				ChangesetStatus.Computing,
			);
		}
		...
		test("createSession seeds both halves before SessionReady", async () => {
			...
			assert.ok(state);
			assert.deepStrictEqual(
				state!.summary.changesets,
				defaultCatalogue(sessionStr),
			);
		...
		test("forked createSession seeds both halves on the forked session", async () => {
			...
			const sourceSession = await service.createSession({
				provider: "copilot",
			});
			const sourceState = service.stateManager.getSessionState(
			...
			assert.notStrictEqual(
				forked.toString(),
				sourceSession.toString(),
				"fork should produce a distinct session URI",
			);
			...
			assert.deepStrictEqual(
				forkedState!.summary.changesets,
				defaultCatalogue(forkedStr),
			);
			...
			assert.ok(
				forkedState!.turns.length > 0,
				"forked session should carry copied turns",
			);
		...
		test("provisional session materialization preserves both halves", async () => {
			...
			class ProvisionalMockAgent extends MockAgent {
				private readonly _onDidMaterialize = new Emitter<{
					session: URI;
					workingDirectory: URI | undefined;
					project: { uri: URI; displayName: string } | undefined;
				}>();
				readonly onDidMaterializeSession = this._onDidMaterialize.event;
				override async createSession(
					config?: import("../../common/agentService.js").IAgentCreateSessionConfig,
				): Promise<
					import("../../common/agentService.js").IAgentCreateSessionResult
				> {
			...
			assert.deepStrictEqual(
				stateBefore!.summary.changesets,
				defaultCatalogue(sessionStr),
			);
			assertBackingChangesetsComputing(service.stateManager, sessionStr);
