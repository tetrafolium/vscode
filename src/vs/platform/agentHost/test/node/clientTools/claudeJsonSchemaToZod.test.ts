...
suite("claudeJsonSchemaToZod", () => {
	...
	test("empty / undefined schema returns empty shape", () => {
		...
		assert.deepStrictEqual(
			Object.keys(jsonSchemaToZodRawShape({ type: "object" })),
			[],
		);
	});
	...
	test("primitives + required vs optional wrapping", () => {
		...
		assert.strictEqual(
			parse(shape, { a: "x" }).success,
			true,
			"omitting optional props OK",
		);
		assert.strictEqual(
			parse(shape, {}).success,
			false,
			"missing required a fails",
		);
		assert.strictEqual(
			parse(shape, { a: "x", c: 1.5 }).success,
			false,
			"integer rejects float",
		);
		assert.strictEqual(
			parse(shape, { a: "x", c: 1, b: 2, d: true }).success,
			true,
		);
	});
	...
	test("nullable + description + default survive conversion", () => {
		...
		assert.strictEqual(
			withDefault.success && (withDefault.data as { n: string }).n,
			"d",
		);
	});
	...
	test("unsupported property schema falls back to z.any() — never rejects the tool", () => {
		...
		assert.strictEqual(
			parse(shape, { weird: 42 }).success,
			true,
			"any accepts anything",
		);
		assert.strictEqual(
			parse(shape, { weird: { nested: true }, worse: "also fine" }).success,
			true,
		);
	});
});
