...
} from "../../node/copilot/sessionCustomizationDiscovery.js";
...
suite("SessionCustomizationDiscovery + SessionPluginBundler", () => {
	...
	setup(() => {
		...
		pluginBasePath = URI.from({
			scheme: Schemas.inMemory,
			path: "/agentPlugins",
		});
		instantiationService.stub(IAgentPluginManager, {
			basePath: pluginBasePath,
		} as Partial<IAgentPluginManager>);
	});
	...
	test("discovers agents, skills, and instructions across workspace and home roots", async () => {
		const wsAgent = await seed(
			"/workspace/.github/agents/foo.agent.md",
			"agent body",
		);
		const wsSkill = await seed(
			"/workspace/.github/skills/bar/SKILL.md",
			"skill body",
		);
		const wsInstr = await seed(
			"/workspace/.github/instructions/baz.instructions.md",
			"instr body",
		);
		const userAgent = await seed(
			"/home/.copilot/agents/qux.agent.md",
			"user agent",
		);
		const userSkill = await seed(
			"/home/.agents/skills/zap/SKILL.md",
			"user skill",
		);
		// Noise that should not be picked up
		await seed("/workspace/.github/agents/not-an-agent.txt", "ignored");

		const discovery = disposables.add(
		...
		const files = directories.flatMap((directory) =>
			directory.files.map((uri) => ({ uri, type: directory.type })),
		);
	...
	test("excludes exact-case README.md inside agent folders", async () => {
		...
		const wsPlainAgent = await seed(
			"/workspace/.github/agents/plain.md",
			"plain agent body",
		);
		const wsLowerReadmeAgent = await seed(
			"/workspace/.github/agents/readme.md",
			"docs lower",
		);
		...
		const files = (await discovery.directories()).flatMap((directory) =>
			directory.files.map((uri) => ({ uri, type: directory.type })),
		);
	...
	test("includes non-README markdown files inside agent folders", async () => {
		...
		const wsLegacyMode = await seed(
			"/workspace/.github/agents/legacy.chatmode.md",
			"legacy mode body",
		);
		const wsPrompt = await seed(
			"/workspace/.github/agents/bar.prompt.md",
			"prompt body",
		);
		const wsInstruction = await seed(
			"/workspace/.github/agents/baz.instructions.md",
			"instruction body",
		);
		const wsCopilotInstructions = await seed(
			"/workspace/.github/agents/copilot-instructions.md",
			"copilot instructions body",
		);
		const wsSkill = await seed(
			"/workspace/.github/agents/SKILL.md",
			"skill body",
		);
		const wsSkillLowercase = await seed(
			"/workspace/.github/agents/skill.md",
			"skill body lowercase",
		);
	...
	test("discovers nested .instructions.md files", async () => {
		const nestedWsInstr = await seed(
			"/workspace/.github/instructions/team/security/policy.instructions.md",
			"workspace nested instruction",
		);
		const nestedUserInstr = await seed(
			"/home/.copilot/instructions/domain/tools/deep.instructions.md",
			"user nested instruction",
		);
	...
	test("bundles discovered files into the synthetic plugin tree", async () => {
		...
		const bundler = disposables.add(
			instantiationService.createInstance(SessionPluginBundler, workspace),
		);
		const directories = await discovery.directories();
		...
		const manifest = await fileService.readFile(
			URI.joinPath(root, ".plugin", "plugin.json"),
		);
		assert.match(manifest.value.toString(), /"name": "VS Code Synced Data"/);

		const agent = await fileService.readFile(
			URI.joinPath(root, "agents", "foo.agent.md"),
		);
		assert.strictEqual(agent.value.toString(), "agent body");

		const skill = await fileService.readFile(
			URI.joinPath(root, "skills", "bar", "SKILL.md"),
		);
		assert.strictEqual(skill.value.toString(), "skill body");

		const instr = await fileService.readFile(
			URI.joinPath(root, "rules", "baz.instructions.md"),
		);
	...
	test("bundles nested .instructions.md files into rules", async () => {
		...
		const root = bundler.rootUri;
		const workspaceInstr = await fileService.readFile(
			URI.joinPath(root, "rules", "policy.instructions.md"),
		);
		assert.strictEqual(
			workspaceInstr.value.toString(),
			"workspace nested instruction",
		);

		const userInstr = await fileService.readFile(
			URI.joinPath(root, "rules", "deep.instructions.md"),
		);
	...
	test("different working directories produce different bundle authorities", async () => {
		const otherWorkspace = URI.from({
			scheme: Schemas.inMemory,
			path: "/other-workspace",
		});
		const a = disposables.add(
			instantiationService.createInstance(SessionPluginBundler, workspace),
		);
		const b = disposables.add(
			instantiationService.createInstance(SessionPluginBundler, otherWorkspace),
		);
