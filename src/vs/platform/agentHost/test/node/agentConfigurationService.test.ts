...
suite("AgentConfigurationService", () => {
	...
	setup(() => {
		...
		service = disposables.add(
			new AgentConfigurationService(manager, new NullLogService()),
		);
	});
	...
	suite("getEffectiveValue", () => {
		test("returns session value when present", () => {
			...
			assert.strictEqual(
				service.getEffectiveValue(uri, schema, "level"),
				"high",
			);
		});
		...
		test("inherits from parent subagent session", () => {
			...
			assert.strictEqual(
				service.getEffectiveValue(child, schema, "level"),
				"high",
			);
		});
	...
	suite("getEffectiveWorkingDirectory", () => {
		test("returns session working directory when set", () => {
			...
			assert.strictEqual(
				service.getEffectiveWorkingDirectory(uri),
				"file:///work",
			);
		});
		...
		test("falls back to parent session working directory for subagents", () => {
			...
			manager.createSession(makeSummary(child));
			assert.strictEqual(
				service.getEffectiveWorkingDirectory(child),
				"file:///work/parent",
			);
		});
