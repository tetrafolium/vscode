...
import {
	...
	makeAuthHandler,
	type SSHAuthAttempt,
} from "../../node/sshRemoteAgentHostService.js";
...
class KeyboardInteractiveMockSSHClient {
	...
	connect(config: ConnectConfig): void {
		const authHandler = config.authHandler as
			| ((
...
function makeConfig(
	overrides?: Partial<ISSHAgentHostConfig>,
): ISSHAgentHostConfig {
	return {
		host: "10.0.0.1",
		username: "testuser",
...
class TestableSSHRemoteAgentHostMainService extends SSHRemoteAgentHostMainService {
	...
	private readonly _relayResults: Array<{
		send: (data: string) => void;
		close: () => void;
	}> = [];

	protected override async _connectSSH(_config: ISSHAgentHostConfig) {
	...
	protected override async _createWebSocketRelay(
		...
		if (this.hangRelayCreationOnCall === this.relayCalled) {
			...
			return new Promise<{ send: (data: string) => void; close: () => void }>(
				() => {
					/* never */
				},
			);
	...
	override async resolveSSHConfig(
		_host: string,
	): ReturnType<SSHRemoteAgentHostMainService["resolveSSHConfig"]> {
		return {
			hostname: "10.0.0.1",
			port: 22,
			user: "testuser",
	...
	startKeyboardInteractiveForTest(
		...
		return this._handleKeyboardInteractive(
			"ssh:test-host",
			"test-host",
			"testuser",
			"",
...
class KeyboardInteractiveConnectTestService extends SSHRemoteAgentHostMainService {
	...
	protected override async _buildAuthAttempts(
...
suite("SSHRemoteAgentHostMainService - connect flow", () => {
	...
	setup(() => {
		...
		const productService: Pick<
			IProductService,
			"_serviceBrand" | "quality" | "dataFolderName"
		> = {
	...
	test("uses sshConfigHost as connection key when present", async () => {
		...
		const result = await service.connect(
			makeConfig({ sshConfigHost: "myhost" }),
		);
	...
	test("agent-host reuse skips platform detection and CLI install", async () => {
		...
		assert.ok(
			!execCalls.some((c) => c.includes("uname")),
			`uname should not run on reuse; saw: ${JSON.stringify(execCalls)}`,
		);
		assert.ok(
			!execCalls.some((c) => c.includes("--version")),
			`--version should not run on reuse; saw: ${JSON.stringify(execCalls)}`,
		);
		assert.ok(
			!execCalls.some((c) => c.includes("test -x")),
			`test -x should not run on reuse; saw: ${JSON.stringify(execCalls)}`,
		);
		assert.ok(
			!execCalls.some((c) => c.includes("curl")),
			`curl should not run on reuse; saw: ${JSON.stringify(execCalls)}`,
		);
	});
	...
	test("sanitizes config in result (strips password and privateKeyPath)", async () => {
		...
		assert.strictEqual(
			(result.config as Record<string, unknown>)["password"],
			undefined,
		);
		assert.strictEqual(
			(result.config as Record<string, unknown>)["privateKeyPath"],
			undefined,
		);
	...
	test("fires onDidChangeConnections on connect and disconnect", async () => {
		...
		const events: string[] = [];
		disposables.add(
			service.onDidChangeConnections(() => events.push("changed")),
		);
		disposables.add(
			service.onDidCloseConnection((id) => events.push(`closed:${id}`)),
		);
	...
	test("connects to two different hosts independently", async () => {
		...
		const r1 = await service.connect(
			makeConfig({
				host: "10.0.0.1",
				remoteAgentHostCommand: "/agent",
			}),
		...
		const r2 = await service.connect(
			makeConfig({
				host: "10.0.0.2",
				remoteAgentHostCommand: "/agent",
			}),
	...
	test("disconnect one host does not affect the other", async () => {
		...
		const r2Again = await service.connect(
			makeConfig({
				host: "10.0.0.2",
				remoteAgentHostCommand: "/agent",
			}),
	...
	test("fires progress events during connect", async () => {
		...
		disposables.add(
			service.onDidReportConnectProgress((p) => progress.push(p)),
		);
		...
		assert.ok(
			progress.length >= 3,
			`expected at least 3 progress events, got ${progress.length}`,
		);
		assert.ok(progress.every((p) => p.connectionKey === "ssh:myhost"));
		assert.ok(
			progress.every((p) => p.message.length > 0),
			"all progress messages should be non-empty",
		);
	});

	test("cancelling keyboard-interactive prompt rejects connect with cancellation", async () => {
		const kbiService = disposables.add(
			new KeyboardInteractiveConnectTestService(new NullLogService(), {
				_serviceBrand: undefined,
				quality,
				dataFolderName,
			} as IProductService),
		...
		disposables.add(
			kbiService.onDidRequestKeyboardInteractive((kbiRequest) =>
				request.complete(kbiRequest),
			),
		);

		const connectPromise = kbiService.connectSSHForTest(
			makeConfig({ sshConfigHost: "test-host" }),
		);
		const kbiRequest = await request.p;
		await kbiService.respondKeyboardInteractive(
			kbiRequest.requestId,
			undefined,
		);
	...
	suite("commit-pinned install", () => {
		...
		test("always invokes cleanup of old commit-keyed CLIs", async () => {
			...
			assert.ok(
				execCalls.some(
					(c) => /ls -1t .*code-insiders-/.test(c) && /awk\s+'NR>5'/.test(c),
				),
		...
		test("downloads from commit-pinned URL when CLI is missing", async () => {
			...
			assert.ok(
				installCall,
				`should have run curl install; saw: ${JSON.stringify(execCalls)}`,
			);
			assert.ok(
		...
		test("falls back to any usable CLI when commit-pinned download fails", async () => {
			...
			assert.ok(
				execCalls.some(
					(c) =>
						/ls -1t .*code-insiders-/.test(c) &&
						c.includes(".vscode-cli-insider/code-insiders"),
				),
	...
	test("reconnect rejects with timeout when relay creation hangs (silently dead SSH client)", async () => {
		...
		assert.strictEqual(
			originalClient.ended,
			true,
			"dead SSH client should be ended",
		);
	...
	test("reconnect removes old close/error listeners from shared SSH client", async () => {
		...
		assert.ok(
			closeListenersBefore > 0,
			"should have close listeners after connect",
		);
		assert.ok(
			errorListenersBefore > 0,
			"should have error listeners after connect",
		);
...
class AuthAttemptsTestService extends SSHRemoteAgentHostMainService {
	...
	protected override async _readKeyFileIfExists(
		keyPath: string,
...
suite("SSHRemoteAgentHostMainService - _buildAuthAttempts", () => {
	...
	test("Agent + no SSH_AUTH_SOCK + only id_rsa exists → publickey id_rsa, then keyboard-interactive", async () => {
		...
		const attempts = await service.testBuildAuthAttempts(
			makeConfig({ authMethod: SSHAuthMethod.Agent }),
		);
	...
	test("KeyFile + missing privateKeyPath throws", async () => {
		await assert.rejects(
			() =>
				service.testBuildAuthAttempts(
					makeConfig({ authMethod: SSHAuthMethod.KeyFile }),
				),
			/private key path/i,
		);
	});
...
suite("SSHRemoteAgentHostMainService - makeAuthHandler", () => {
	...
	test("keyboard-interactive routes prompts to the kbi handler and is skipped without one", () => {
		...
		assert.deepStrictEqual(callsNoKbi, [
		...
		let promptArgs:
			| {
					name: string;
					instructions: string;
					prompts: ReadonlyArray<{ prompt: string; echo: boolean }>;
			  }
			| undefined;
		const handlerWithKbi = makeAuthHandler(
			kbiAttempts,
			new NullLogService(),
		...
		const callsWithKbi: Array<
			{ type: string; username: string; prompt?: Function } | false
		> = [];
		handlerWithKbi(null, false, (next) =>
			callsWithKbi.push(
				next as { type: string; username: string; prompt?: Function },
			),
		);
		assert.strictEqual(callsWithKbi.length, 1);
		assert.strictEqual(
			(callsWithKbi[0] as { type: string }).type,
			"keyboard-interactive",
		);
		const finishCalls: ReadonlyArray<string>[] = [];
		(callsWithKbi[0] as { prompt: Function }).prompt(
			"n",
			"i",
			"lang",
			[{ prompt: "Password:", echo: false }],
			(responses: ReadonlyArray<string>) => finishCalls.push(responses),
		);
