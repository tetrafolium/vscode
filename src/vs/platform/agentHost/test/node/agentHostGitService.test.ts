...
suite("AgentHostGitService", () => {
	...
	test("sorts common branch names to the top before applying limit", () => {
		assert.deepStrictEqual(
			getBranchCompletions(
				["feature/recent", "release", "master", "main", "feature/older"],
				{ limit: 3 },
			),
	...
	test("filters before sorting common branch names", () => {
		assert.deepStrictEqual(
			getBranchCompletions(
				["feature/recent", "master", "main", "maintenance"],
				{ query: "ma" },
			),
	...
	suite("parseHasGitHubRemote", () => {
		test("detects ssh github remote", () => {
			assert.strictEqual(
				parseHasGitHubRemote("origin\tgit@github.com:owner/repo.git (fetch)\n"),
				true,
			);
		});
	...
	suite("parseDefaultBranchRef", () => {
		test("strips refs/remotes/origin/ prefix", () => {
			assert.strictEqual(
				parseDefaultBranchRef("refs/remotes/origin/main\n"),
				"main",
			);
		});
	...
	suite("parseGitHubRepoFromRemote", () => {
		test("parses ssh (scp-like) origin remote", () => {
			...
			assert.deepStrictEqual(parseGitHubRepoFromRemote(out), {
				owner: "microsoft",
				repo: "vscode",
			});
		});
	...
	suite("parseUntrackedPaths", () => {
		...
		test("extracts untracked entries and skips others", () => {
			...
			// must be skipped.
			const out =
				"?? new.txt\x00 M edited.txt\x00R  to.txt\x00from.txt\x00?? other.txt\x00";
			assert.deepStrictEqual(parseUntrackedPaths(out), [
				"new.txt",
				"other.txt",
			]);
		});
	...
	suite("parseGitDiffRawNumstat", () => {
		...
		test("parses an add, modify, delete and rename in a single stream", () => {
			...
			assert.deepStrictEqual(diffs, [
				{
					before: {
						uri: "file:///repo/modified.ts",
						content: { uri: buildGitBlobUri(sessionUri, sha, "modified.ts") },
					},
					after: {
						uri: "file:///repo/modified.ts",
						content: { uri: "file:///repo/modified.ts" },
					},
					diff: { added: 5, removed: 2 },
				},
				{
					after: {
						uri: "file:///repo/added.ts",
						content: { uri: "file:///repo/added.ts" },
					},
					diff: { added: 10, removed: 0 },
				},
				{
					before: {
						uri: "file:///repo/deleted.ts",
				...
				{
					before: {
						uri: "file:///repo/old/path.ts",
						content: { uri: buildGitBlobUri(sessionUri, sha, "old/path.ts") },
					},
					after: {
						uri: "file:///repo/new/path.ts",
						content: { uri: "file:///repo/new/path.ts" },
					},
		...
		test("returns empty for empty input", () => {
			assert.deepStrictEqual(
				parseGitDiffRawNumstat("", root, sessionUri, sha),
				[],
			);
		});
		...
		test("anchors after side to afterRef when provided (ref->ref diffs)", () => {
			...
			const diffs = parseGitDiffRawNumstat(
				segments.join("\x00"),
				root,
				sessionUri,
				sha,
			...
			assert.deepStrictEqual(diffs, [
				{
					...
					after: {
						uri: buildGitBlobUri(sessionUri, toSha, "modified.ts"),
						content: { uri: buildGitBlobUri(sessionUri, toSha, "modified.ts") },
					},
					diff: { added: 5, removed: 2 },
				},
				{
					after: {
						uri: buildGitBlobUri(sessionUri, toSha, "added.ts"),
						content: { uri: buildGitBlobUri(sessionUri, toSha, "added.ts") },
					},
					diff: { added: 10, removed: 0 },
				},
				{
				...
				},
				{
					...
					after: {
						uri: buildGitBlobUri(sessionUri, toSha, "new/path.ts"),
						content: { uri: buildGitBlobUri(sessionUri, toSha, "new/path.ts") },
					},
					diff: { added: 3, removed: 3 },
				},
			]);
	...
	suite("formatGitError", () => {
		test("reports timeout when our timer fired and summarises progress-meter stderr", () => {
			...
			const progress =
				"Updating files:   0% (7/14834)\rUpdating files:   0% (149/14834)\r";
			assert.strictEqual(
				formatGitError(
					["worktree", "add", "-b", "x", "/tmp/y", "origin/main"],
					30_000,
					true,
					err,
					progress,
		...
		test("reports numeric exit code when git failed normally", () => {
			...
			assert.strictEqual(
				formatGitError(
					["worktree", "add", "/tmp/y", "missing-branch"],
					30_000,
					false,
					err,
	...
	suite("summarizeStderrForError", () => {
		...
		test("keeps the last non-empty line of a multi-line progress meter", () => {
			const progress =
				"Updating files:   0% (7/14834)\rUpdating files:   0% (149/14834)\r";
			assert.strictEqual(
				summarizeStderrForError(progress),
				"Updating files:   0% (149/14834)",
			);
		});

		test("passes through a normal single-line message", () => {
			assert.strictEqual(
				summarizeStderrForError("fatal: invalid reference: x\n"),
