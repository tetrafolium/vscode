...
} from "../../node/claude/claudeSubagentSignals.js";
import {
...
suite("claudeSubagentSignals — Phase 12 emission", () => {
	...
	test("top-level Task tool_use records a spawn; non-subagent tools do not", () => {
		...
		mapSDKMessageToAgentSignals(
			makeStreamEvent(
				SESSION_ID,
				makeContentBlockStartToolUse(0, "toolu_task", "Task"),
			),
			SESSION,
			TURN_ID,
			state,
		...
		mapSDKMessageToAgentSignals(
			makeStreamEvent(
				SESSION_ID,
				makeContentBlockStartToolUse(1, "toolu_agent", "Agent"),
			),
			SESSION,
			TURN_ID,
			state,
		...
		mapSDKMessageToAgentSignals(
			makeStreamEvent(
				SESSION_ID,
				makeContentBlockStartToolUse(2, "toolu_read", "Read"),
			),
			SESSION,
			TURN_ID,
			state,
	...
	test("top-level Task SessionToolCallStart carries _meta.toolKind=subagent so the workbench renders the subagent UI", () => {
		...
		const readSignals = mapSDKMessageToAgentSignals(
			makeStreamEvent(
				SESSION_ID,
				makeContentBlockStartToolUse(1, "toolu_read", "Read"),
			),
			SESSION,
			TURN_ID,
			state,
		...
		assert.ok(
			taskAction.kind === "action" &&
				taskAction.action.type === ActionType.SessionToolCallStart,
			"Task signal is SessionToolCallStart",
		);
		assert.ok(
			readAction.kind === "action" &&
				readAction.action.type === ActionType.SessionToolCallStart,
			"Read signal is SessionToolCallStart",
		);
	...
	test("top-level canonical assistant for Task emits SessionToolCallReady with confirmed:NotNeeded + _meta.subagentDescription/AgentName AND records metadata onto the spawn", () => {
		...
		mapSDKMessageToAgentSignals(
			makeStreamEvent(
				SESSION_ID,
				makeContentBlockStartToolUse(0, "toolu_top_task", "Task"),
			),
			SESSION,
			TURN_ID,
			state,
		...
		const canonical = makeAssistantMessage(SESSION_ID, [
			{
				...
				name: "Task",
				input: {
					description: "Count TS files",
					subagent_type: "Explore",
					prompt: "Count how many TS files...",
				},
			},
		]);
		const out = mapSDKMessageToAgentSignals(
			canonical,
			SESSION,
			TURN_ID,
			state,
		...
		const ready = out.find(
			(s) =>
				s.kind === "action" &&
				s.action.type === ActionType.SessionToolCallReady,
		);
		assert.ok(
			ready &&
				ready.kind === "action" &&
				ready.action.type === ActionType.SessionToolCallReady,
			"Ready emitted",
		);
	...
	test("inner subagent message: prepends subagent_started exactly once, tags emitted action with parentToolCallId, records inner-tool→parent edge", () => {
		...
		mapSDKMessageToAgentSignals(
			makeStreamEvent(
				SESSION_ID,
				makeContentBlockStartToolUse(0, PARENT, "Task"),
			),
			SESSION,
			TURN_ID,
			state,
		...
		const innerToolUse = makeStreamEvent(
			SESSION_ID,
			makeContentBlockStartToolUse(1, "toolu_inner", "Read"),
		);
		...
		assert.deepStrictEqual(
			{
				...
				firstStartedToolCallId:
					first[0]?.kind === "subagent_started" ? first[0].toolCallId : null,
				firstActionParent: first
					.filter((s) => s.kind === "action")
					.map((s) => (s.kind === "action" ? s.parentToolCallId : null)),
				secondKinds: second.map((s) => s.kind),
				secondActionParent: second
					.filter((s) => s.kind === "action")
					.map((s) => (s.kind === "action" ? s.parentToolCallId : null)),
				innerToolParentSpawnId:
					registry.getParentSpawn("toolu_inner")?.toolUseId,
			},
			{
	...
	test("inner subagent canonical assistant message emits text/thinking/tool_use signals + tags them with parentToolCallId, lets the matching tool_result complete", () => {
		...
		const fromAssistant = mapSDKMessageToAgentSignals(
			innerAssistant,
			SESSION,
			TURN_ID,
			state,
		...
		const innerToolResult = makeUserToolResultMessage(
			SESSION_ID,
			"toolu_inner_glob",
			"a.ts\nb.ts",
		);
		innerToolResult.parent_tool_use_id = PARENT;
		const fromToolResult = mapSDKMessageToAgentSignals(
			innerToolResult,
			SESSION,
			TURN_ID,
			state,
		...
		const allParentIds = [...fromAssistant, ...fromToolResult]
			.filter((s) => s.kind === "action")
			.map((s) => (s.kind === "action" ? s.parentToolCallId : null));
		const completeAction = fromToolResult.find(
			(s) =>
				s.kind === "action" &&
				s.action.type === ActionType.SessionToolCallComplete,
		);
		const completePastTense =
			completeAction?.kind === "action" &&
			completeAction.action.type === ActionType.SessionToolCallComplete
		...
		assert.deepStrictEqual(
			{
				...
				completedToolCallId: signals.find(
					(s) => s.kind === "subagent_completed",
				)?.toolCallId,
		...
		assert.deepStrictEqual(
			{
				...
				completedToolCallId: afterNotification.find(
					(s) => s.kind === "subagent_completed",
				)?.toolCallId,
	...
	test("buildTopLevelSubagentReadyAction omits _meta description/agentName when input fields are missing or wrong-typed; still records the spawn", () => {
		...
		const malformed = buildTopLevelSubagentReadyAction(
			{
				...
				id: "toolu_bad",
				name: "Task",
				input: { description: 42, subagent_type: null } as unknown as Record<
					string,
					unknown
				>,
			},
		...
		assert.ok(
			malformed.kind === "action" &&
				malformed.action.type === ActionType.SessionToolCallReady,
		);
	...
	test("mapSubagentSystemMessage ignores task_notification with non-terminal status, missing tool_use_id, or unknown spawn", () => {
		...
		const inProgress = mapSubagentSystemMessage(
			{
				type: "system",
				subtype: "task_notification",
				task_id: "t",
				...
				status: "in_progress",
			} as unknown as SDKMessage & { type: "system" },
			SESSION,
			registry,
		);
		const missingId = mapSubagentSystemMessage(
			{
				type: "system",
				subtype: "task_notification",
				task_id: "t",
		...
		const unknownEntry = mapSubagentSystemMessage(
			{
				type: "system",
				subtype: "task_notification",
				task_id: "t",
