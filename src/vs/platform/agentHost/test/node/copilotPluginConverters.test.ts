...
import {
	...
	parsedPluginsEqual,
	toSdkHooks,
} from "../../node/copilot/copilotPluginConverters.js";
import type {
...
suite("copilotPluginConverters", () => {
	...
	suite("toSdkMcpServers", () => {
		...
		test("omits optional fields when undefined", () => {
			...
			assert.deepStrictEqual(
				(result["minimal"] as { args?: string[] }).args,
				[],
			);
		...
		test("filters null values from env", () => {
			...
			const result = toSdkMcpServers(defs);
			const env = (result["with-null-env"] as { env?: Record<string, string> })
				.env;
	...
	suite("toSdkCustomAgents", () => {
		test("reads agent files without frontmatter and creates configs", async () => {
			const agentUri = URI.from({
				scheme: Schemas.inMemory,
				path: "/agents/helper.md",
			});
			await fileService.writeFile(
				agentUri,
				VSBuffer.fromString("You are a helpful assistant"),
			);

			const agents: INamedPluginResource[] = [
				{ uri: agentUri, name: "helper" },
			];
		...
		test("processes multiple agents, skipping failures", async () => {
			...
			const agents: INamedPluginResource[] = [
				...
				{
					uri: URI.from({ scheme: Schemas.inMemory, path: "/agents/bad.md" }),
					name: "bad",
				},
			];
			const result = await toSdkCustomAgents(agents, fileService);
		...
		test("falls back to resource name when frontmatter omits name", async () => {
			...
			const agents: INamedPluginResource[] = [
				{ uri: agentUri, name: "resource-name" },
			];
			const result = await toSdkCustomAgents(agents, fileService);
	...
	suite("toSdkInstructionDirectories", () => {
		test("extracts parent directories of instruction files", () => {
			const instructions: INamedPluginResource[] = [
				...
				{
					uri: URI.file("/plugins/rules/review.instructions.md"),
					name: "review",
				},
			];
	...
	suite("toSdkHooks", () => {
		 ...
		 */
		function echoJsonCmd(value: object): {
			command: string;
			cleanup: () => void;
		} {
		...
		test("onPostToolUse returns parsed JSON output as hook result", async () => {
			const expectedOutput = {
				additionalContext: "Before presenting the plan, run review-plan skill",
			};
		...
		test("onPostToolUse calls editTrackingHooks and returns command output", async () => {
			...
			try {
				...
				const editTrackingHooks = {
					onPreToolUse: async () => {},
					onPostToolUse: async (input: unknown) => {
						trackingInput = input;
					},
				};
				...
				const result = await hooks.onPostToolUse!(callInput, {
					sessionId: "test",
				});
				assert.deepStrictEqual(result, expectedOutput);
	...
	suite("parsedPluginsEqual", () => {
		...
		test("returns true for identical empty plugins", () => {
			assert.strictEqual(
				parsedPluginsEqual([makePlugin()], [makePlugin()]),
				true,
			);
		});
				...
				skills: [
					{
						...
						name: "a",
						customization: stubSkillCustomization("a"),
					} satisfies IParsedSkill,
				],
				mcpServers: [
					{
						name: "server",
		...
		test("returns false for different content", () => {
			...
			const b = makePlugin({
				skills: [
					{
						uri: URI.file("/b/SKILL.md"),
						name: "b",
						customization: stubSkillCustomization("b"),
					} satisfies IParsedSkill,
				],
			});
		...
		test("returns false for different lengths", () => {
			assert.strictEqual(
				parsedPluginsEqual([makePlugin()], [makePlugin(), makePlugin()]),
				false,
			);
		});
