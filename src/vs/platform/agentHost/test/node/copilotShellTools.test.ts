...
} from "../../../sandbox/common/terminalSandboxMxcRuntime.js";
import {
...
import {
	...
	type ICommandFinishedEvent,
	type ISendTextOptions,
} from "../../node/agentHostTerminalManager.js";
import {
	createShellTools,
	type IUnsandboxedCommandConfirmationRequest,
	isMultilineCommand,
	ShellManager,
	prefixForHistorySuppression,
	shellTypeForExecutable,
} from "../../node/copilot/copilotShellTools.js";
...
class TestAgentHostTerminalManager implements IAgentHostTerminalManager {
	...
	async createTerminal(
		...
		this.created.push({
			params,
			options: { ...options, shell: options?.shell ?? this.defaultShell },
		});
	}
	...
	onExit(_uri: string, cb: (exitCode: number) => void): IDisposable {
		return this._onExit.event(cb);
	}
	onClaimChanged(
		_uri: string,
		cb: (claim: TerminalClaim) => void,
	): IDisposable {
		return this._onClaimChanged.event(cb);
	}
	onCommandFinished(
		_uri: string,
		cb: (event: ICommandFinishedEvent) => void,
	): IDisposable {
	...
	getClaim(): TerminalClaim | undefined {
		return undefined;
	}
	...
	fireCommandFinished(event: ICommandFinishedEvent): void {
		this._onCommandFinished.fire(event);
	}
	fireExit(exitCode: number): void {
		this._onExit.fire(exitCode);
	}
	fireClaimChanged(claim: TerminalClaim): void {
...
suite("CopilotShellTools", () => {
	...
	function createFakeAgentConfigurationService(
		initialSandbox?: Record<string, unknown>,
	): IFakeAgentConfigurationService {
		...
		const service: IAgentConfigurationService = {
			...
			getRootValue: ((_schema: unknown, key: string) =>
				configValues[key]) as IAgentConfigurationService["getRootValue"],
			updateRootConfig: () => {
				/* no-op */
			},
			persistRootConfig: () => {
	...
	function createStubSandboxHelperService(): ISandboxHelperService {
		...
		return {
			...
			buildWindowsMxcSandboxPayload: async (
				commandLine,
				policy,
				workingDirectory,
				containerName = "vscode-terminal-sandbox",
				containment = "process",
			) => ({
	...
	function createServices(options?: {
		...
		if (options?.sandboxEnabled) {
			...
			initialSandboxValues[AgentHostSandboxKey.WindowsEnabled] =
				AgentSandboxEnabledValue.AllowNetwork;
		}
		const agentConfigurationService =
			createFakeAgentConfigurationService(initialSandboxValues);
		...
		} as Partial<IEnvironmentService> & {
			userHome: URI;
		} as IEnvironmentService);
		services.set(IProductService, {
			dataFolderName: ".test-data",
		} as Partial<IProductService> as IProductService);
	...
	async function waitForSentTexts(
		terminalManager: TestAgentHostTerminalManager,
		count: number,
	): Promise<void> {
		while (terminalManager.sentTexts.length < count) {
			...
			if (didTimeOut) {
				assert.fail(
					`Timed out waiting for ${count} sendText calls; saw ${terminalManager.sentTexts.length}`,
				);
			}
		}
	}

	function markCreatedTerminalsExist(
		terminalManager: TestAgentHostTerminalManager,
	): void {
	...
	test("uses session working directory for created shells", async () => {
		...
		const shellManager = disposables.add(
			instantiationService.createInstance(
				ShellManager,
				URI.parse("copilot:/session-1"),
		...
		(
			await shellManager.getOrCreateShell(
				"bash",
				"turn-2",
				"tool-2",
				explicitCwd,
		...
		assert.deepStrictEqual(
			terminalManager.created.map((c) => c.params.cwd),
			[worktreePath, explicitCwd],
		);
	});
	...
	test("opts every managed shell into shell-history suppression and non-interactive mode", async () => {
		...
		assert.strictEqual(
			terminalManager.created[0].options?.preventShellHistory,
			true,
		);
		assert.strictEqual(
			terminalManager.created[0].options?.nonInteractive,
			true,
		);
	});
	...
	test("uses the executable resolved by the terminal manager", async () => {
		...
		assert.strictEqual(
			terminalManager.created[0].options?.shell,
			"/custom/path/to/pwsh",
		);
	});
	...
	test("shellTypeForExecutable maps known shell basenames and falls back to platform default", () => {
		...
		const unknownDefault = shellTypeForExecutable(
	...
	test("zsh executable keeps bash tool name but uses zsh-specific guidance", async () => {
		...
		const tools = await createShellTools(
			shellManager,
			terminalManager,
			new NullLogService(),
		);
	...
	test("getOrCreateShell reuses an idle shell after the reference is disposed", async () => {
		...
		(terminalManager as unknown as { hasTerminal: () => boolean }).hasTerminal =
			() => true;
		const services = new ServiceCollection();
		services.set(ILogService, new NullLogService());
		services.set(IAgentHostTerminalManager, terminalManager);
		...
		const first = await shellManager.getOrCreateShell(
			"bash",
			"turn-1",
			"tool-1",
		);
		first.dispose();
		const second = await shellManager.getOrCreateShell(
			"bash",
			"turn-2",
			"tool-2",
		);
	...
	test("getOrCreateShell creates a new shell when the existing reference is still held", async () => {
		...
		assert.notStrictEqual(
			second.object.id,
			first.object.id,
			"should create a new shell when existing is busy",
		);
	...
	test("shell helper tools (read/write/shutdown/list/redirect) are registered with skipPermission: true", async () => {
		...
		const skipPermissionByName = Object.fromEntries(
			tools.map((t) => [t.name, t.skipPermission ?? false]),
		);
	...
	test("primary shell tool normalizes multiline command input", async () => {
		...
		assert.strictEqual(
			terminalManager.sentTexts[0].options.bracketedPasteMode,
			true,
		);
		assert.strictEqual(
			terminalManager.sentTexts[1].options.bracketedPasteMode,
			undefined,
		);
		assert.strictEqual(
			terminalManager.writes[0].data,
			" echo first\recho second\r",
		);
		assert.match(
			terminalManager.writes[1].data,
			/^echo "<<<COPILOT_SENTINEL_[a-f0-9]+_EXIT_\$\?>>>"\r$/,
		);
	});
	...
	test("primary shell tool returns alternateBuffer when shell integration enters alt buffer", async () => {
		...
		const resultPromise = bashTool.handler!(
			{ command: "vim README.md", timeout: 1000 },
			invocation,
		) as Promise<ToolResultObject>;
	...
	test("alt-buffer shell is released when command finishes", async () => {
		...
		const next = await shellManager.getOrCreateShell(
			"bash",
			"turn-2",
			"tool-2",
		);
	...
	test("backgrounded shell is not immediately reused", async () => {
		...
		await waitForSentTexts(terminalManager, 1);
		terminalManager.fireClaimChanged({
		...
		assert.match(
			...
			/continue this command in the background/,
		);
		markCreatedTerminalsExist(terminalManager);
		const shell = shellManager.listShells()[0];
	...
	test("write shell tool normalizes input without appending enter", async () => {
		...
		const shellRef = await shellManager.getOrCreateShell(
			"bash",
			"turn-1",
			"tool-1",
		);
		...
		assert.strictEqual(
			terminalManager.writes[0].uri,
			shellRef.object.terminalUri,
		);
	...
	test("getOrCreateSandboxEngine returns the same engine across calls", async () => {
		...
		assert.strictEqual(
			engineA,
			engineB,
			"Sandbox engine should be cached across calls",
		);
	});
	...
	test("primary shell tool schema only exposes requestUnsandboxedExecution params when the sandbox is enabled", async () => {
		...
		const enabledShell = disposables.add(
			enabled.instantiationService.createInstance(
				ShellManager,
				URI.parse("copilot:/session-enabled"),
				undefined,
			),
		);
		const enabledTools = await createShellTools(
			enabledShell,
			enabled.terminalManager,
			new NullLogService(),
		);
		const enabledPrimary = enabledTools[0] as Tool<unknown>;
		const enabledSchema = enabledPrimary.parameters as {
			properties: Record<string, unknown>;
		};
		const enabledPropertyNames = Object.keys(enabledSchema.properties);

		assert.ok(
			enabledPropertyNames.includes("requestUnsandboxedExecution"),
			"Sandbox-enabled schema should expose requestUnsandboxedExecution",
		);
		assert.ok(
			enabledPropertyNames.includes("requestUnsandboxedExecutionReason"),
			"Sandbox-enabled schema should expose requestUnsandboxedExecutionReason",
		);

		const disabled = createServices();
		const disabledShell = disposables.add(
			disabled.instantiationService.createInstance(
				ShellManager,
				URI.parse("copilot:/session-disabled"),
				undefined,
			),
		);
		const disabledTools = await createShellTools(
			disabledShell,
			disabled.terminalManager,
			new NullLogService(),
		);
		const disabledPrimary = disabledTools[0] as Tool<unknown>;
		const disabledSchema = disabledPrimary.parameters as {
			properties: Record<string, unknown>;
		};
		const disabledPropertyNames = Object.keys(disabledSchema.properties);

		assert.ok(
			!disabledPropertyNames.includes("requestUnsandboxedExecution"),
			"Sandbox-disabled schema should not expose requestUnsandboxedExecution",
		);
		assert.ok(
			!disabledPropertyNames.includes("requestUnsandboxedExecutionReason"),
			"Sandbox-disabled schema should not expose requestUnsandboxedExecutionReason",
		);
	});
	...
	test("primary shell tool sends commands unwrapped when the sandbox is disabled", async () => {
		...
		assert.ok(
			sentCommand.includes("echo hello"),
			`Expected the raw command to be sent. Sent: ${sentCommand}`,
		);
		assert.ok(
			!sentCommand.includes("sandbox-runtime"),
			`Sandbox wrapper should not be applied when sandbox is disabled. Sent: ${sentCommand}`,
		);
	});
	...
	test("primary shell tool wraps commands through the sandbox engine when the sandbox is enabled", async function () {
		...
		if (platform.isWindows) {
			assert.ok(
				sentCommand.includes("wxc-exec"),
				`Expected the command to be wrapped by the MXC runtime. Sent: ${sentCommand}`,
			);
		} else {
			assert.ok(
				sentCommand.includes("sandbox-runtime"),
				`Expected the command to be wrapped by the sandbox runtime. Sent: ${sentCommand}`,
			);
			assert.ok(
				sentCommand.includes("echo hello"),
				`Wrapped command should still contain the user command. Sent: ${sentCommand}`,
			);
		}
	});
	...
	test("primary shell tool writes a sandbox config exposing the working directory as writable", async () => {
		...
		const sandboxConfigEntry = [...createdFiles.entries()].find(([path]) =>
			/vscode-sandbox-settings-.*\.json$/.test(path),
		);
		assert.ok(
			sandboxConfigEntry,
			`Expected a sandbox config file to be written. Files: ${[...createdFiles.keys()].join(", ")}`,
		);
		const config = JSON.parse(sandboxConfigEntry[1]);
		const writablePaths: string[] = platform.isWindows
			? config.filesystem.readwritePaths
			: config.filesystem.allowWrite;
		assert.ok(
			Array.isArray(writablePaths),
			`Expected writable paths array. Got: ${JSON.stringify(config.filesystem)}`,
		);
		const expectedPath = platform.isWindows
			? "\\workspace\\test-workspace"
			: "/workspace/test-workspace";
		assert.ok(
			writablePaths.includes(expectedPath),
			`Expected working directory in writable paths. Got: ${JSON.stringify(writablePaths)}`,
		);
	});
	...
	test("primary shell tool merges configured filesystem allowRead paths into the sandbox config", async () => {
		...
		const createdFiles = new Map<string, string>();
		const configuredReadPath = platform.isWindows
			? "C:\\tools\\custom"
			: "/tools/custom";
		...
		const { instantiationService, terminalManager, agentConfigurationService } =
			createServices({ sandboxEnabled: true, createdFiles });
		agentConfigurationService.setSandboxValue(fileSystemKey, {
			allowRead: [configuredReadPath],
		});
		const shellManager = disposables.add(
			instantiationService.createInstance(
				ShellManager,
				URI.parse("copilot:/session-1"),
				URI.file("/workspace/test-workspace"),
			),
		);
		const tools = await createShellTools(
			shellManager,
		...
		const config = JSON.parse(sandboxConfigEntry[1]);
		const readablePaths: string[] = platform.isWindows
			? config.filesystem.readonlyPaths
			: config.filesystem.allowRead;
		assert.ok(
			Array.isArray(readablePaths),
			`Expected readable paths array. Got: ${JSON.stringify(config.filesystem)}`,
		);
		assert.ok(
			readablePaths.includes(configuredReadPath),
			`Expected configured read path in readable paths. Got: ${JSON.stringify(readablePaths)}`,
		);
	});
	...
	test("primary shell tool requests confirmation before rerunning outside the sandbox", async function () {
		...
		const { instantiationService, terminalManager, agentConfigurationService } =
			createServices({ sandboxEnabled: true });
		...
		agentConfigurationService.setSandboxValue(
		...
		assert.deepStrictEqual(confirmationRequests[0]?.blockedDomains, [
			"example.com",
		]);
		assert.ok(
			terminalManager.sentTexts.length >= 1,
			"Approved command should be sent to the terminal unsandboxed",
		);
		assert.ok(
			terminalManager.sentTexts.every(
				(entry) => !entry.data.includes("sandbox-runtime"),
			),
			"No wrapped sandbox-runtime command should be sent after approval",
		);
		assert.strictEqual(result.resultType, "success");
	});
	...
	test("primary shell tool asks for confirmation when requestUnsandboxedExecution is explicitly set", async function () {
		...
		assert.strictEqual(confirmationRequests.length, 1);
		assert.strictEqual(
			confirmationRequests[0]?.reason,
			"sandbox blocked required syscall",
		);
		assert.strictEqual(result.resultType, "failure");
		assert.strictEqual(result.error, "sandbox_blocked");
		assert.match(result.textResultForLlm ?? "", /declined/i);
	...
	test("primary shell tool returns unsandboxed_disabled when allowUnsandboxedCommands is off", async function () {
		...
		assert.strictEqual(
			terminalManager.sentTexts.length,
			0,
			"Disallowed command should not be sent to the terminal",
		);
	});
	...
	test("primary shell tool skips confirmation when autoApproveUnsandboxedCommands is enabled", async function () {
		...
		assert.strictEqual(
			confirmationRequests.length,
			0,
			"No confirmation should have been requested when auto-approve is enabled",
		);
		assert.ok(
			terminalManager.sentTexts.length >= 1,
			"Auto-approved command should be sent to the terminal unsandboxed",
		);
		assert.ok(
			terminalManager.sentTexts.every(
				(entry) => !entry.data.includes("sandbox-runtime"),
			),
			"Auto-approved command should run unsandboxed",
		);
		assert.strictEqual(result.resultType, "success");
	});
});
