...
suite("AgentHostChangesetService", () => {
	...
	setup(() => {
		...
		changesetService = disposables.add(
			new AgentHostChangesetService(
				stateManager,
				new NullLogService(),
				createNullSessionDataService(),
				createNoopGitService(),
	...
	test("registerStaticChangesets makes the two static changeset URIs subscribable with computing status", () => {
		...
		assert.deepStrictEqual(
			stateManager.getSessionState(sessionStr)?.summary.changesets,
			[
				{
	...
	test("registerStaticChangesets is idempotent across repeated calls", () => {
		...
		assert.strictEqual(
			changesets?.length,
			2,
			"expected the two default catalogue entries",
		);
	});
	...
	test("restoreStaticChangeset catalogue counts only emitted unique files", () => {
		...
		const state = snapshot?.state as
			| {
					files: Array<{
						id: string;
						edit: { diff?: { added?: number; removed?: number } };
					}>;
			  }
			| undefined;
		const catalogue =
	...
	test("restoreStaticChangeset works without a live session state (seeds the changeset for unopened sessions)", () => {
		...
		const snapshot = stateManager.getSnapshot(
			`${sessionStr}/changeset/session`,
		);
		assert.ok(
			snapshot,
			"expected the changeset URI to be subscribable even without a session state",
		);
		const state = snapshot.state as {
			status: string;
			files: Array<{ id: string }>;
	...
	suite("session diff computation", () => {
		test("git-driven path is preferred when a git service is provided and the working dir is a git work tree", async () => {
			...
			const localChangesets = disposables.add(
				new AgentHostChangesetService(
					localStateManager,
					new NullLogService(),
					sessionDataService,
			...
			disposables.add(
				localStateManager.onDidEmitEnvelope((e) => {
					envelopes.push(e);
				}),
			);
			...
			const fileSets = envelopes.filter(
				(e) => e.action.type === ActionType.ChangesetFileSet,
			) as Array<{ channel: string; action: { file: { edit: unknown } } }>;
			const sessionFileSets = fileSets.filter(
				(e) => e.channel === `${sessionUri.toString()}/changeset/session`,
			);
			const uncommittedFileSets = fileSets.filter(
				(e) => e.channel === `${sessionUri.toString()}/changeset/uncommitted`,
			);
			assert.deepStrictEqual(
				sessionFileSets.map((e) => e.action.file.edit),
				gitDiffs,
			);
			assert.deepStrictEqual(
				uncommittedFileSets.map((e) => e.action.file.edit),
				gitDiffs,
			);
			...
			assert.ok(
				persisted,
				"expected the compute pass to persist diffs to the session DB",
			);
		...
		test("falls back to the edit-tracker aggregator when the git service returns undefined", async () => {
			...
			const diffsEmitted = new Promise<void>((r) => {
				resolveDiffs = r;
			});
			disposables.add(
				localStateManager.onDidEmitEnvelope((e) => {
			...
			assert.ok(
				statusAction,
				"expected a changeset/statusChanged envelope from the fallback path",
			);
		});
		...
		test("uncommitted compute does NOT fall back to edit-tracker and preserves restored snapshot when git is unavailable", async () => {
			...
			const persistedDiffs = [
				...
				},
				{
					after: {
						uri: "file:///wd/c.ts",
						content: { uri: "file:///wd/c.ts" },
					},
					diff: { added: 1, removed: 0 },
				},
			];
			localChangesets.restoreStaticChangeset(
				sessionStr,
				"uncommitted",
				persistedDiffs,
			);
			...
			const removed = envelopes.filter(
				(e) =>
					e.action.type === ActionType.ChangesetFileRemoved &&
					e.channel === uncommittedUri,
			);
			assert.deepStrictEqual(
				removed,
				[],
				"no files should be removed when the git path is unavailable",
			);

			// 2) The persisted DB blob is unchanged (compute did not overwrite it).
			const persistedAfter = await sessionDb.getMetadata(
				"agentHost.changeset.uncommitted",
			);
			assert.strictEqual(
				persistedAfter,
				undefined,
				"compute must not persist anything when git is unavailable",
			);
			...
			const state = snapshot?.state as
				| { files: Array<{ id: string }> }
				| undefined;
			assert.deepStrictEqual(state?.files.map((f) => f.id).sort(), [
				"file:///wd/a.ts",
				"file:///wd/b.ts",
				"file:///wd/c.ts",
			]);
	...
	suite("restorePersistedStaticChangesets", () => {
		...
		test("parsePersistedStaticChangesets parses without mutating state", () => {
			...
			const result = changesetService.parsePersistedStaticChangesets(
				sessionStr,
				{
			...
			assert.deepStrictEqual(
				{
					...
					uncommittedState: stateManager.getChangesetState(
						buildUncommittedChangesetUri(sessionStr),
					),
					sessionState: stateManager.getChangesetState(
		...
		test("applyPersistedStaticChangesets seeds parsed diffs", () => {
			setupSession();
			changesetService.registerStaticChangesets(sessionStr);
			const parsed = changesetService.parsePersistedStaticChangesets(
				sessionStr,
				{
					uncommittedRaw: JSON.stringify([aDiff]),
					sessionRaw: JSON.stringify([bDiff]),
			...
			const uncommitted = stateManager.getChangesetState(
				buildUncommittedChangesetUri(sessionStr),
			);
			const session = stateManager.getChangesetState(
				buildSessionChangesetUri(sessionStr),
			);
			assert.deepStrictEqual(
				{
					uncommitted: uncommitted && {
						status: uncommitted.status,
						files: uncommitted.files.map((f) => f.id),
					},
					session: session && {
		...
		test("new uncommitted key restores only uncommitted state", () => {
			...
			const result = changesetService.restorePersistedStaticChangesets(
				sessionStr,
				{
					uncommittedRaw: JSON.stringify([aDiff]),
				},
			...
			assert.deepStrictEqual(
				result.uncommitted?.map((d) => d.after?.uri),
				["file:///wd/a.ts"],
			);
			assert.strictEqual(result.session, undefined);

			const uncommitted = stateManager.getSnapshot(
				`${sessionStr}/changeset/uncommitted`,
			);
			const session = stateManager.getSnapshot(
				`${sessionStr}/changeset/session`,
			);
			assert.strictEqual(
			...
			assert.strictEqual(
				(session?.state as { status: string }).status,
				"computing",
			);
		});
		...
		test("new sessionRaw beats legacyRaw when both are present", () => {
			...
			assert.deepStrictEqual(
				result.session?.map((d) => d.after?.uri),
				["file:///wd/a.ts"],
		...
		test("malformed JSON logs and returns undefined for that slot", () => {
			...
			assert.strictEqual(
				result.uncommitted,
				undefined,
				"malformed slot returns undefined",
			);
			assert.deepStrictEqual(
				result.session?.map((d) => d.after?.uri),
				["file:///wd/a.ts"],
				"valid slot still parses",
			);
		...
		test("seedIfEmpty honoured: live state with files is not overwritten", () => {
			...
			changesetService.restoreStaticChangeset(sessionStr, "uncommitted", [
				aDiff,
			]);
			...
			assert.deepStrictEqual(
				(before?.state as { files: Array<{ id: string }> }).files.map(
					(f) => f.id,
				),
				["file:///wd/a.ts"],
			);
			...
			const after = stateManager.getSnapshot(
				`${sessionStr}/changeset/uncommitted`,
			);
			assert.deepStrictEqual(
				(after?.state as { files: Array<{ id: string }> }).files.map(
					(f) => f.id,
				),
				["file:///wd/a.ts"],
		...
		test("with live session state, restored diffs publish ready + catalogue counts", () => {
			...
			const catalogue =
				stateManager.getSessionState(sessionStr)?.summary.changesets;
			const sessionEntry = catalogue?.find(
				(c) => c.uriTemplate === `${sessionStr}/changeset/session`,
			);
			assert.deepStrictEqual(
				sessionEntry,
				{
	...
	suite("idle changeset LRU eviction", () => {
		...
		test("idle changeset states are evicted over the soft limit", () => {
			const localStateManager = disposables.add(
				new AgentHostStateManager(new NullLogService(), {
					changesetStateRetention: { softLimit: 2 },
				}),
			);
			...
			const localStateManager = disposables.add(
				new AgentHostStateManager(new NullLogService(), {
					changesetStateRetention: {
						softLimit: 2,
						canEvict: (changeset) => changeset !== first,
					},
				}),
			);
		...
		test("LRU eviction is silent and does not dispatch ChangesetCleared", () => {
			const localStateManager = disposables.add(
				new AgentHostStateManager(new NullLogService(), {
					changesetStateRetention: { softLimit: 1 },
				}),
			);
			const envelopes: ActionEnvelope[] = [];
			const listener = disposables.add(
				localStateManager.onDidEmitEnvelope((e) => envelopes.push(e)),
			);

			localStateManager.registerChangeset(`${sessionStr}/changeset/session`);
			localStateManager.registerChangeset(
				`${sessionStr}/changeset/uncommitted`,
			);

			assert.deepStrictEqual(
				envelopes.map((e) => e.action.type),
				[],
			);
			listener.dispose();
		});
		...
		test("trimming reconsiders entries after they become evictable", () => {
			...
			const localStateManager = disposables.add(
				new AgentHostStateManager(new NullLogService(), {
					changesetStateRetention: { softLimit: 1, canEvict: () => canEvict },
				}),
			);
			const first = `${sessionStr}/changeset/session`;
			const second = `${sessionStr}/changeset/uncommitted`;
	...
	suite("per-turn live streaming", () => {
		...
		class CountingChangesetService extends AgentHostChangesetService {
			...
			override async computeTurnChangeset(
				session: string,
				turnId: string,
			): Promise<string> {
		...
		test("onTurnComplete does NOT schedule a per-turn recompute when the probe says nobody is subscribed", async () => {
			...
			assert.deepStrictEqual(
				svc.turnComputeCalls,
				[],
				"no per-turn compute when nothing observes the turn URI",
			);
		});
		...
		test("onToolCallEditsApplied fires the per-turn debounce only when subscribers exist; cancelled by onTurnComplete", () => {
			return runWithFakedTimers(
				{ useFakeTimers: true, maxTaskCount: 10_000 },
				async () => {
					setupSession();
					const svc = makeService();
					svc.setTurnSubscriberProbe(() => true);
					...
					assert.strictEqual(
						svc.turnComputeCalls.length,
						1,
						"debounce should fire one per-turn compute",
					);
					...
					assert.strictEqual(
						svc.turnComputeCalls.length,
						2,
						"onTurnComplete cancels pending debounce and runs exactly one final compute",
					);
					...
					assert.strictEqual(
						svc.turnComputeCalls.length,
						2,
						"unsubscribed turn must not get any further per-turn computes",
					);
				},
			);
		});
		...
		test("per-turn URI streams incremental ChangesetFileSet / ChangesetFileRemoved as the same turn is recomputed", async () => {
			...
			const statusReady = envelopes.find(
				(e) =>
					e.action.type === ActionType.ChangesetStatusChanged &&
					e.channel === turnUri,
			);
			assert.ok(
				statusReady,
				"first per-turn compute must transition the URI to ready",
			);
			...
			for (
				...
				!envelopes.some(
					(e) =>
						e.action.type === ActionType.ChangesetStatusChanged &&
						e.channel === `${sessionUri.toString()}/changeset/session`,
				);
				i++
			) {
				await timeout(2);
			...
			assert.ok(
				envelopes.some(
					(e) => e.action.type === ActionType.ChangesetStatusChanged,
				),
	...
	suite("computeCompareTurnsChangeset", () => {
		function makeCheckpointService(
			pairs: Record<string, { parent: string; current: string } | undefined>,
		) {
			return {
				...NULL_CHECKPOINT_SERVICE,
		...
		test("publishes diffs as Ready when both checkpoints resolve and git returns diffs", async () => {
			...
			const compareUri = await svc.computeCompareTurnsChangeset(
				sessionStr,
				"orig",
				"mod",
			);
			...
			assert.strictEqual(
				compareUri,
				`${sessionStr}/changeset/compare/orig/mod`,
			);
			assert.deepStrictEqual(calls, [
				{ fromRef: "ref-orig", toRef: "ref-mod" },
			]);
			const snapshot = stateManager.getSnapshot(compareUri);
			const state = snapshot?.state as
				| { status: string; files: Array<{ id: string }> }
				| undefined;
			assert.deepStrictEqual(
				{ status: state?.status, ids: state?.files.map((f) => f.id) },
				{
					status: "ready",
					ids: ["file:///wd/a.ts"],
				},
		...
		test("transitions to Error when either checkpoint is missing", async () => {
			...
			assert.ok(
				state?.error?.message.includes("modified turn"),
				`expected error to name the missing side, got ${state?.error?.message}`,
			);
			assert.strictEqual(
				gitCalls,
				0,
				"git must not be invoked when a checkpoint is missing",
			);
		});
		...
		test("returns empty Ready snapshot when both checkpoints point at the same ref", async () => {
			...
			const snapshot = stateManager.getSnapshot(compareUri);
			const state = snapshot?.state as
				| { status: string; files: Array<unknown> }
				| undefined;
			assert.deepStrictEqual(
				{ status: state?.status, files: state?.files },
				{ status: "ready", files: [] },
			);
			assert.strictEqual(
				gitCalls,
				0,
				"git diff must be short-circuited when both refs match",
			);
		});
		...
		test("transitions to Error when the git diff returns undefined (git failure, not empty)", async () => {
			...
			assert.strictEqual(state?.status, "error");
			assert.ok(
				state?.error?.message.includes("git"),
				`expected git-failure error message, got ${state?.error?.message}`,
			);
		});
	});
});
