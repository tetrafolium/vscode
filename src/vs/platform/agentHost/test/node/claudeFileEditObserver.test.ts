...
function createObserver(
	disposables: Pick<
		import("../../../../base/common/lifecycle.js").DisposableStore,
		"add"
	>,
): IObserverHarness {
...
function assistantMessage(
	...
	return { type: "assistant", message: { content } } as Extract<
		SDKMessage,
		{ type: "assistant" }
	>;
}
...
function userMessage(content: unknown): Extract<SDKMessage, { type: "user" }> {
	return { type: "user", message: { content } } as Extract<
		SDKMessage,
		{ type: "user" }
	>;
}
...
suite("ClaudeFileEditObserver", () => {
	...
	test("observe assistant→user round-trip caches a file edit on the mapper state", async () => {
		...
		observer.observeAssistant(
			assistantMessage([
				{
					type: "tool_use",
					id: "tu-1",
					name: "Write",
					input: { file_path: "/work/a.txt", content: "after" },
				},
			]),
		);
