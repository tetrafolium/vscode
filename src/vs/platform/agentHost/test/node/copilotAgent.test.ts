...
} from "../../node/agentHostCompletions.js";
import {
	COPILOT_AGENT_HOST_SYSTEM_MESSAGE,
	CopilotAgent,
	getCopilotBranchNameHintFromMessage,
	getCopilotWorktreeBranchName,
	getCopilotWorktreeName,
	getCopilotWorktreesRoot,
} from "../../node/copilot/copilotAgent.js";
import { NULL_CHECKPOINT_SERVICE } from "../../common/agentHostCheckpointService.js";
import {
	CopilotAgentSession,
	type SessionWrapperFactory,
...
class TestAgentPluginManager implements IAgentPluginManager {
	...
	async syncCustomizations(
		_clientId: string,
		_customizations: ClientPluginCustomization[],
		_progress?: (status: Customization) => void,
...
class TestAgentHostGitService implements IAgentHostGitService {
	...
	async getRepositoryRoot(): Promise<URI | undefined> {
	...
	async getSessionGitState(): Promise<undefined> {
		return undefined;
	}
	async computeSessionFileDiffs(): Promise<undefined> {
		return undefined;
	}
	async showBlob(): Promise<undefined> {
		return undefined;
	}
	async captureWorkingTreeAsTree(): Promise<undefined> {
		return undefined;
	}
	async commitTree(): Promise<undefined> {
		return undefined;
	}
	async updateRef(): Promise<void> {}
	async deleteRefs(): Promise<void> {}
	async revParse(): Promise<undefined> {
		return undefined;
	}
	async computeFileDiffsBetweenRefs(): Promise<undefined> {
		return undefined;
	}
}
...
class TestAgentHostTerminalManager implements IAgentHostTerminalManager {
	...
	onClaimChanged(): IDisposable {
		return Disposable.None;
	}
	onCommandFinished(): IDisposable {
		return Disposable.None;
	}
	createAltBufferPromise(_uri: string, _store: DisposableStore): Promise<void> {
		return new Promise(() => {});
	}
...
class TestSessionDataService extends Disposable implements ISessionDataService {
	...
	getSessionDataDir(session: URI): URI {
		return URI.from({
			scheme: "test",
			path: `/session-data/${AgentSession.id(session)}`,
		});
	}
	...
	async tryOpenDatabase(
		session: URI,
	): Promise<IReference<SessionDatabase> | undefined> {
...
interface ITestCopilotModelInfo {
	...
	readonly policy?: {
		readonly state?: NonNullable<ModelInfo["policy"]>["state"];
	};
...
interface ITestCopilotClient extends Pick<
	CopilotClient,
	| "start"
	| "stop"
	| "listSessions"
...
function toSdkModelInfo(model: ITestCopilotModelInfo): ModelInfo {
	return {
		...
		...(model.policy
			? { policy: { state: model.policy.state ?? "enabled", terms: "" } }
			: {}),
...
class TestCopilotClient implements ITestCopilotClient {
	...
	constructor(
		private readonly _sessions: Awaited<
			ReturnType<ITestCopilotClient["listSessions"]>
		>,
	...
	async listModels(): ReturnType<ITestCopilotClient["listModels"]> {
		return this._models.map(toSdkModelInfo);
	}
	async getSessionMetadata(
		sessionId: string,
	): ReturnType<ITestCopilotClient["getSessionMetadata"]> {
		this.getSessionMetadataCalls.push(sessionId);
		return this._sessions.find((s) => s.sessionId === sessionId);
	}
	createSession: ITestCopilotClient["createSession"] = async () => {
		throw new Error("not implemented");
	};
	resumeSession: ITestCopilotClient["resumeSession"] = async () => {
		throw new Error("not implemented");
	};
}
...
class MockCopilotSession {
	...
	on<K extends SessionEventType>(
		_eventType: K,
		_handler: TypedSessionEventHandler<K>,
	): () => void {
		return () => {};
	}
	...
	async getEvents(): Promise<SessionEventPayload<SessionEventType>[]> {
		return [];
	}
...
class TestableCopilotAgent extends CopilotAgent {
	...
	protected override async _resumeSession(
		sessionId: string,
		...
		const emitter = (
			this as unknown as {
				_onDidSessionProgress: { fire(s: AgentSignal): void };
			}
		...
		const stub = {
			...
			resetTurnState: (newTurnId: string) => {
				turnId = newTurnId;
			},
	...
	resolveWorktreeForTest(
		config: Parameters<CopilotAgent["createSession"]>[0],
		sessionId: string,
		prompt?: string,
...
function createTestAgentContext(
	disposables: Pick<DisposableStore, "add">,
	options?: {
		sessionDataService?: ISessionDataService;
		copilotClient?: ITestCopilotClient;
	...
	const fileService =
		options?.fileService ?? disposables.add(new FileService(logService));
	...
	services.set(
		ISessionDataService,
		options?.sessionDataService ?? createNullSessionDataService(),
	);
	services.set(
		IAgentPluginManager,
		options?.pluginManager ?? new TestAgentPluginManager(),
	);
	services.set(
		IAgentHostGitService,
		options?.gitService ?? new TestAgentHostGitService(),
	);
	...
	services.set(
		IAgentHostCompletions,
		disposables.add(new AgentHostCompletions(logService)),
	);
...
function createAgentSessionThroughAgent(
	agent: CopilotAgent,
	instantiationService: IInstantiationService,
): CopilotAgentSession {
	const sessionUri = AgentSession.uri("copilotcli", "test-session-1");
	const shellManager = instantiationService.createInstance(
		ShellManager,
		sessionUri,
		undefined,
	);
	const wrapperFactory: SessionWrapperFactory = async () =>
		new CopilotSessionWrapper(
			new MockCopilotSession() as unknown as CopilotSession,
		);
	return (
		agent as unknown as {
			_createAgentSession: (
...
function sdkSession(
	sessionId: string,
	cwd?: string,
): Awaited<ReturnType<ITestCopilotClient["listSessions"]>>[number] {
	return {
		sessionId,
...
suite("CopilotAgent", () => {
	...
	test("uses Git extension branch-derived worktree folder names", () => {
		assert.strictEqual(
			getCopilotWorktreeName("agents/add-agent-host-config-12345678"),
			"agents-add-agent-host-config-12345678",
		);
	});
	...
	test("derives slug branch hint from first message", () => {
		assert.strictEqual(
			getCopilotBranchNameHintFromMessage("Add agent host config"),
			"add-agent-host-config",
		);
		assert.strictEqual(
			getCopilotBranchNameHintFromMessage("  Fix: the bug!! "),
			"fix-the-bug",
		);
		assert.strictEqual(
			getCopilotBranchNameHintFromMessage("Refactor café ☕ rendering"),
			"refactor-cafe-rendering",
		);
		assert.strictEqual(
			getCopilotBranchNameHintFromMessage(
		...
		assert.strictEqual(
			getCopilotBranchNameHintFromMessage("a".repeat(100))?.length,
			48,
		);
		assert.strictEqual(
			getCopilotBranchNameHintFromMessage("!!! ??? ..."),
	...
	test("returns empty models and throws AuthRequired for sessions before authentication", async () => {
		...
		try {
			...
			await assert.rejects(
				() => agent.listSessions(),
				(error: Error) =>
					error instanceof ProtocolError && error.code === AHP_AUTH_REQUIRED,
			);
		} finally {
	...
	test("agent-created sessions can resolve session-state paths via INativeEnvironmentService", async () => {
		...
		const { agent, instantiationService } = createTestAgentContext(
			disposables,
			{
		...
		try {
			const agentSession = disposables.add(
				createAgentSessionThroughAgent(agent, instantiationService),
			);
			...
			const result = await agentSession.handlePermissionRequest({
				kind: "read",
				path: URI.file(
					"/mock-home/.copilot/session-state/test-session-1/plan.md",
				).fsPath,
	...
	test("listSessions only returns sessions with a database", async () => {
		...
		try {
			...
			assert.deepStrictEqual(
				(await agent.listSessions()).map((s) => AgentSession.id(s.session)),
				["owned"],
			);
		} finally {
			await disposeAgent(agent);
	...
	test("listSessions reads stored metadata from sessions with a database", async () => {
		...
		await legacyDb.object.setMetadata(
			"copilot.workingDirectory",
			URI.file("/workspace").toString(),
		);
		legacyDb.dispose();

		const agent = createTestAgent(disposables, {
			sessionDataService,
			copilotClient: new TestCopilotClient([sdkSession("legacy")]),
		});
		try {
			await agent.authenticate("https://api.github.com", "token");

			assert.deepStrictEqual(
				(await agent.listSessions()).map(withoutUndefinedProperties),
				[
					{
	...
	suite("createSession activeClient eager-claim", () => {
		class SpyingPluginManager extends TestAgentPluginManager {
			...
			override async syncCustomizations(
				clientId: string,
				customizations: ClientPluginCustomization[],
				_progress?: (status: Customization) => void,
			): Promise<ISyncedCustomization[]> {
		...
		test("setClientCustomizations publishes parsed agents in SessionCustomizationUpdated", async () => {
			...
			try {
				...
				const session = AgentSession.uri(
					"copilotcli",
					"sync-customizations-test",
				);
				await agent.setClientCustomizations(session, "client-1", [
					{
						type: CustomizationType.Plugin,
						id: customizationId(pluginDir.toString()),
				...
				assert.strictEqual(
					updatesWithChildren.length > 0,
					true,
					"expected SessionCustomizationUpdated to carry parsed children",
				);
				const agentChildren = updatesWithChildren
					.at(-1)!
					.customization.children!.filter(
						(c) => c.type === CustomizationType.Agent,
					);
				assert.deepStrictEqual(agentChildren, [
					{
						type: CustomizationType.Agent,
						id: customizationId(
							URI.joinPath(pluginDir, "agents", "helper.md").toString(),
						),
		...
		test("getSessionCustomizations publishes discovered files as Directory customizations", async () => {
			...
			const workspace = URI.from({
				scheme: Schemas.inMemory,
				path: "/workspace",
			});
			await fileService.createFolder(
				URI.joinPath(workspace, ".github", "agents"),
			);
			await fileService.createFolder(
				URI.joinPath(workspace, ".github", "instructions", "team"),
			);
			...
			const instructionFile = URI.joinPath(
				workspace,
				".github",
				"instructions",
				"team",
			...
			await fileService.writeFile(
				instructionFile,
				VSBuffer.fromString("instruction body"),
			);
			...
			try {
				...
				const discoveredDirectories = customizations.filter(
					(customization) => customization.type === CustomizationType.Directory,
				);
				...
				const agentDirectory = discoveredDirectories.find(
					(customization) =>
						customization.uri ===
						URI.joinPath(workspace, ".github", "agents").toString(),
				);
				...
				const instructionDirectory = discoveredDirectories.find(
					(customization) =>
						customization.uri ===
						URI.joinPath(workspace, ".github", "instructions").toString(),
				);
	...
	suite("provisional sessions", () => {
		test("createSession does not call client.createSession or create worktrees", async () => {
			...
			try {
				...
				assert.strictEqual(
					clientCreateCalls,
					0,
					"client.createSession should not be called for provisional sessions",
				);
				assert.strictEqual(
					worktreeCalls,
					0,
					"no worktree should be created for provisional sessions",
				);
			} finally {
				await disposeAgent(agent);
			}
		...
		test("disposeSession on provisional session does not touch SDK or worktree", async () => {
			...
			try {
				...
				assert.strictEqual(
					removeWorktreeCalls,
					0,
					"no worktree to remove for provisional",
				);
		...
		test("materialization passes VS Code-specific system message to the SDK", async () => {
			...
			const client = new TestCopilotClient([]);
			let capturedConfig:
				| Parameters<ITestCopilotClient["createSession"]>[0]
				| undefined;
			...
			try {
				...
				assert.ok(
					capturedConfig,
					"SDK createSession should be called during provisional materialization",
				);
		...
		test("materialization skips managed shell tools when root config disables the custom terminal tool", async () => {
			...
			const { agent, configurationService } = createTestAgentContext(
				disposables,
				{ sessionDataService, copilotClient: client },
			);
			try {
				await agent.authenticate("https://api.github.com", "token");
				...
				assert.deepStrictEqual(
					capturedConfig?.tools?.map((tool) => tool.name),
					[],
				);
			} finally {
				await disposeAgent(agent);
	...
	suite("onClientToolCallComplete", () => {
		...
		function installStubSession(
			agent: CopilotAgent,
			sessionId: string,
		): { calls: { toolCallId: string; result: ToolCallResult }[] } {
			...
			const sessions = (agent as unknown as { _sessions: Map<string, unknown> })
				._sessions;
		...
		test("routes a top-level session URI to its session entry", async () => {
			...
			try {
				...
				const { calls } = installStubSession(
					agent,
		...
		test("routes a subagent session URI to its parent session entry", async () => {
			...
			try {
				...
				const subagentUri = URI.parse(
					buildSubagentSessionUri(parentUri.toString(), "tc-parent"),
				);
				const result: ToolCallResult = {
					success: true,
					pastTenseMessage: "subagent tool done",
				};
		...
		test("routes a nested subagent session URI (depth > 1) to the root session entry", async () => {
			...
			try {
				...
				const subagentUri = URI.parse(
					buildSubagentSessionUri(rootUri.toString(), "tc-parent"),
				);
				const nestedUri = URI.parse(
					buildSubagentSessionUri(subagentUri.toString(), "tc-nested"),
				);
				const result: ToolCallResult = {
					success: true,
					pastTenseMessage: "nested done",
				};
		...
		test("is a no-op when no session entry exists for the resolved id", async () => {
			...
			try {
				...
				agent.onClientToolCallComplete(sessionUri, "tc-x", {
					success: true,
					pastTenseMessage: "noop",
				});
			} finally {
				await disposeAgent(agent);
			}
	...
	suite("_resumeSession dedup", () => {
		...
		const makeFakeSession = () =>
			({ dispose: () => {} }) as unknown as CopilotAgentSession;
		...
		test("post-init shutdown race: disposes the session and throws CancellationError instead of registering on a disposed _sessions map", async () => {
			...
			const internals = agent as unknown as {
				_registerInitializedSession: (
					id: string,
					s: CopilotAgentSession,
				) => void;
			...
			try {
				...
				assert.strictEqual(
					disposed,
					1,
					"session should be disposed by the guard",
				);
			} finally {
	...
	suite("worktree announcement", () => {
		...
		test("emits announcement live as a delta on first sendMessage and persists it for restore via getSessionMessages", async () => {
			...
			try {
				...
				const expectedBranchName = getCopilotWorktreeBranchName(
					sessionId,
					branchHint,
				);
				...
				assert.ok(
					workingDir,
					"resolveWorktreeForTest must return a worktree URI",
				);
				assert.deepStrictEqual(
					gitService.addedWorktrees.length,
					1,
					"addWorktree must be called once",
				);
				assert.strictEqual(
					gitService.addedWorktrees[0].branchName,
					expectedBranchName,
				);
				...
				assert.strictEqual(
					sendCalls,
					1,
					"underlying SDK send must still be called",
				);

				const markdownSignals = signals.filter(
					(s): s is IAgentActionSignal =>
						s.kind === "action" &&
						((s.action.type === ActionType.SessionResponsePart &&
							s.action.part.kind === ResponsePartKind.Markdown) ||
							s.action.type === ActionType.SessionDelta),
				);
				assert.strictEqual(
					markdownSignals.length,
					1,
					"exactly one markdown announcement signal should be emitted for the worktree announcement",
				);
				const announcement = markdownSignals[0];
				const announcementContent =
					announcement.action.type === ActionType.SessionResponsePart
						? (announcement.action.part as MarkdownResponsePart).content
						: (announcement.action as IDeltaAction).content;
				assert.ok(
					announcementContent.includes(expectedBranchName),
					`announcement should contain branch name '${expectedBranchName}', got '${announcementContent}'`,
				);

				// 3. Live path is one-shot: a second sendMessage must not re-emit.
				signals.length = 0;
				await agent.sendMessage(session, "follow-up");
				const reemittedMarkdown = signals.filter(
					(s) =>
						s.kind === "action" &&
						((s.action.type === ActionType.SessionResponsePart &&
							s.action.part.kind === ResponsePartKind.Markdown) ||
				...
				assert.strictEqual(
					reemittedMarkdown.length,
					0,
					"announcement must not be re-emitted on subsequent sends",
				);
				...
				const md = restored[0]?.responseParts[0] as
				...
				assert.strictEqual(
					...
					"worktree announcement must be the first response part",
				);
				assert.ok(
					md.content.includes(expectedBranchName),
					`restored markdown content should include the branch name, got '${md.content}'`,
				);
				assert.strictEqual(
					restored[0]?.responseParts[1]?.kind,
					ResponsePartKind.ToolCall,
					"existing tool calls must remain after the announcement",
				);
			} finally {
				await disposeAgent(agent);
			}
		...
		test("does not announce or persist branch metadata when isolation is not worktree", async () => {
			...
			try {
				...
				await agent.resolveWorktreeForTest(
					{ workingDirectory: repositoryRoot },
					sessionId,
				);
				assert.deepStrictEqual(
					gitService.addedWorktrees,
					[],
					"addWorktree must not be called without worktree isolation",
				);
				...
				assert.deepStrictEqual(
					markdownSignals,
					[],
					"no announcement should be emitted live",
				);

				const restored = await agent.getSessionMessages(session);
				const md = restored[0]?.responseParts.find(
					(p): p is MarkdownResponsePart =>
						p.kind === ResponsePartKind.Markdown,
				);
				assert.strictEqual(
					md?.content,
					"untouched reply",
					"restored markdown content must not be modified",
				);
			} finally {
				await disposeAgent(agent);
			}
		...
		test("onArchivedChanged removes the worktree on archive and recreates it on unarchive", async () => {
			...
			try {
				...
				assert.deepStrictEqual(
					gitService.addedExistingWorktrees.map((r) => ({
						worktree: r.worktree.fsPath,
						branchName: r.branchName,
					})),
					[
						{
							worktree: workingDir!.fsPath,
		...
		test("onArchivedChanged skips removal when worktree has uncommitted changes", async () => {
			...
			try {
				...
				await agent.onArchivedChanged(session, true);
				assert.deepStrictEqual(
					gitService.removedWorktrees,
					[],
					"must not remove a dirty worktree",
				);
			} finally {
				await disposeAgent(agent);
			}
		...
		test("onArchivedChanged skips removal when branch is missing", async () => {
			...
			try {
				...
				assert.deepStrictEqual(
					gitService.removedWorktrees,
					[],
					"must not remove a worktree whose branch is missing",
				);
			} finally {
				await disposeAgent(agent);
			}
		...
		test("onArchivedChanged unarchive skips when worktree directory already exists", async () => {
			...
			try {
				...
				await agent.onArchivedChanged(session, false);
				assert.deepStrictEqual(
					gitService.addedExistingWorktrees,
					[],
					"must not recreate a worktree whose directory already exists",
				);
			} finally {
				await disposeAgent(agent);
			}
