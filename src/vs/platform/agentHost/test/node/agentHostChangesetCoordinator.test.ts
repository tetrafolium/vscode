...
import {
	...
	IChangesetSessionMetadata,
} from "../../node/agentHostChangesetCoordinator.js";
import {
	IAgentHostChangesetService,
	IPersistedChangesetMetadata,
	IRestoredChangesetDiffs,
	StaticChangesetKind,
} from "../../node/agentHostChangesetService.js";
import {
	IAgentHostFileMonitorOptions,
	IAgentHostFileMonitorService,
} from "../../node/agentHostFileMonitorService.js";
...
suite("ChangesetSessionCoordinator", () => {
	...
	function createEnvironment(root: URI = URI.file("/repo")): {
		...
		const stateManager = disposables.add(
			new AgentHostStateManager(new NullLogService()),
		);
		const configurationService = disposables.add(
			new AgentConfigurationService(stateManager, new NullLogService()),
		);
		...
		const coordinator = disposables.add(
			new ChangesetSessionCoordinator(
				stateManager,
				changesets,
	...
	test("shares root watchers across sessions and fans out root changes to static refreshes", async () => {
		...
		createSession(
			...
			firstSession,
			"file:///repo/worktree-a",
		);
		createSession(
			environment.stateManager,
			secondSession,
			"file:///repo/worktree-b",
		);

		environment.coordinator.onFirstSubscriber(URI.parse(firstSession));
		await environment.monitor.waitForAcquisitions(1);
		environment.coordinator.onFirstSubscriber(
			URI.parse(buildUncommittedChangesetUri(secondSession)),
		);
	...
	test("releases a root watcher after the last interested session unsubscribes", async () => {
		...
		environment.coordinator.onLastSubscriber(
			URI.parse(buildUncommittedChangesetUri(secondSession)),
		);
	...
	test("attaches deferred watch interest on materialization without re-querying an unchanged root", async () => {
		...
		environment.coordinator.onFirstSubscriber(
			URI.parse(buildUncommittedChangesetUri(session)),
		);
		await tick();
		assert.deepStrictEqual(
			{
			...
			{ acquisitions: [], rootLookups: [] },
		);

		const summary = environment.stateManager.getSessionState(session)!.summary;
		environment.stateManager.markSessionPersisted(session, {
			...summary,
			workingDirectory: "file:///repo/worktree",
		});
	...
	test("defers session changeset refresh until the working directory is known", async () => {
		...
		environment.coordinator.onFirstSubscriber(
			URI.parse(buildSessionChangesetUri(session)),
		);
		await tick();
...
class TestFileMonitorService
	extends Disposable
	implements IAgentHostFileMonitorService
{
	...
	private readonly _acquisitionWaiters: Array<{
		count: number;
	...
	acquire(
		folder: URI,
		callback: () => void,
		_options?: IAgentHostFileMonitorOptions,
	...
	private _releaseAcquisitionWaiters(): void {
		for (const waiter of [...this._acquisitionWaiters]) {
			if (this.acquisitions.length >= waiter.count) {
				this._acquisitionWaiters.splice(
					this._acquisitionWaiters.indexOf(waiter),
					1,
				);
				void waiter.deferred.complete(undefined);
			}
...
class TestChangesetService implements IAgentHostChangesetService {
	...
	restoreStaticChangeset(
		...
		_diffs: readonly ISessionFileDiff[],
	): void {}
	parsePersistedStaticChangesets(
		_sessionUri: string,
		_metadata: IPersistedChangesetMetadata,
	): IRestoredChangesetDiffs {
		return {};
	}
	applyPersistedStaticChangesets(
		_sessionUri: string,
		_diffs: IRestoredChangesetDiffs,
	): void {}
	...
	isStaticChangesetComputeActive(_changesetUri: string): boolean {
		return false;
	}
	...
	setTurnSubscriberProbe(
		_probe: (session: string, turnId: string) => boolean,
	): void {}
	...
	getListMetadataKeys(_sessionStr: string): Record<string, true> | undefined {
		return undefined;
	}
	decorateListEntry(
		entry: IAgentSessionMetadata,
		_metadata: IChangesetSessionMetadata,
	): IAgentSessionMetadata {
		return entry;
	}
}
