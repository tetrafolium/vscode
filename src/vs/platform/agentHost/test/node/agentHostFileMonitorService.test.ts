...
suite("AgentHostFileMonitorService", () => {
	...
	function acquire(
		monitor: AgentHostFileMonitorService,
		folder: URI,
		callback: () => void,
		options?: {
	...
	test("shares one recursive watcher per folder/options and refcounts callbacks", () => {
		return runWithFakedTimers(
			...
			async () => {
				...
				const monitor = disposables.add(
					new AgentHostFileMonitorService(
						fileService.service,
						new NullLogService(),
					),
				);
				...
				const firstRegistration = acquire(monitor, folder, () => first++, {
					debounceMs: 10,
				});
				const secondRegistration = acquire(monitor, folder, () => second++, {
					debounceMs: 10,
				});
				...
				assert.deepStrictEqual(
					{ first, second, snapshot: fileService.snapshot() },
					{ first: 1, second: 2, snapshot: { watches: 1, disposed: 0 } },
				);
	...
	test("filters known repository metadata noise before debouncing", () => {
		return runWithFakedTimers(
			...
			async () => {
				...
				disposables.add(
					acquire(monitor, URI.file("/repo"), () => calls++, {
						debounceMs: 10,
					}),
				);
	...
	test("canonicalizes equivalent folder keys when sharing watchers", () => {
		...
		disposables.add(
			acquire(monitor, URI.file("/repo"), () => {}, { debounceMs: 10 }),
		);
		disposables.add(
			acquire(monitor, URI.file("/repo/../repo/"), () => {}, {
				debounceMs: 10,
			}),
		);
	...
	test("returns undefined when watcher acquisition fails", () => {
		...
		assert.deepStrictEqual(
			{ registration, snapshot: fileService.snapshot() },
			{ registration: undefined, snapshot: { watches: 1, disposed: 0 } },
		);
	});
	...
	test("uses one file-change listener across monitor entries", () => {
		...
		disposables.add(
			acquire(monitor, URI.file("/repo-a"), () => {}, { debounceMs: 10 }),
		);
		disposables.add(
			acquire(monitor, URI.file("/repo-b"), () => {}, { debounceMs: 10 }),
		);

		assert.deepStrictEqual(
			{
				snapshot: fileService.snapshot(),
				listeners: fileService.fileChangeListenerCount,
			},
			{
	...
	test("disposing service cleans up active watchers and pending debounce callbacks", () => {
		return runWithFakedTimers(
			...
			async () => {
				...
				await timeout(11);
				assert.deepStrictEqual(
					{ calls, snapshot: fileService.snapshot() },
					{ calls: 0, snapshot: { watches: 1, disposed: 1 } },
				);
			},
		);
	});
...
class TestFileService {
	...
	private readonly _onDidFilesChangeEvent: Event<FileChangesEvent> = (
		listener,
		thisArgs,
		disposables,
	) => {
	...
	readonly service = {
		...
		watch: (
			_resource: URI,
			_options?: Parameters<IFileService["watch"]>[1],
		): IDisposable => {
			this._watchCount++;
			if (this.failWatch) {
	...
	fire(resource: URI, type: FileChangeType = FileChangeType.UPDATED): void {
		this._onDidFilesChange.fire(
			new FileChangesEvent([{ resource, type }], false),
		);
	}
