...
suite("claudeReplayMapper", () => {
	...
	test("Fixture 1: single text turn", () => {
		...
		assert.strictEqual(
			turns[0].id,
			"u1",
			"Turn.id MUST equal user SessionMessage.uuid",
		);
	...
	test("Fixture 2: tool_use + tool_result is one Turn with one Completed ToolCall", () => {
		...
		assert.strictEqual(
			turns.length,
			1,
			"tool_result MUST NOT start a new turn",
		);
		assert.strictEqual(turns[0].state, TurnState.Complete);
		const toolCallParts = turns[0].responseParts.filter(
		...
		if (toolCall.kind === ResponsePartKind.ToolCall) {
			...
			if (toolCall.toolCall.status === ToolCallStatus.Completed) {
				...
				assert.deepStrictEqual(toolCall.toolCall.content, [
					{ type: ToolResultContentType.Text, text: "file1.txt\nfile2.txt" },
				]);
			}
		}
	});
	...
	test("Fixture 4: compact_boundary attaches as SystemNotification on the active turn", () => {
		...
		assert.strictEqual(
			turns.length,
			1,
			"compact_boundary is NOT a turn boundary",
		);
		const sysParts = turns[0].responseParts.filter(
			(p) => p.kind === ResponsePartKind.SystemNotification,
		);
	...
	test("Fixture 5: Task / Agent tool_use produces subagent marker", () => {
		...
		const toolCallPart = turns[0].responseParts.find(
			(p) => p.kind === ResponsePartKind.ToolCall,
		);
		...
		if (toolCallPart && toolCallPart.kind === ResponsePartKind.ToolCall) {
			...
			if (toolCallPart.toolCall.status === ToolCallStatus.Completed) {
				const hasSubagentMarker = toolCallPart.toolCall.content?.some(
					(c) => c.type === ToolResultContentType.Subagent,
				);
				assert.strictEqual(
					hasSubagentMarker,
					true,
					"subagent marker block must be present",
				);
			} else {
				assert.fail(
					`expected Completed status, got ${toolCallPart.toolCall.status}`,
				);
			}
		}
	});
	...
	test("Fixture 9: CLI slash-command echo and local-command-stdout entries are dropped", () => {
		...
		assert.strictEqual(
			turns.length,
			2,
			"CLI-echo user envelopes must NOT start new turns",
		);
		assert.strictEqual(turns[0].id, "u1");
