...
function createGitService(
	...
	const env: Partial<INativeEnvironmentService> = {
		tmpDir: URI.file(tmpdir()),
	};
	return new AgentHostGitService(
		fileService,
		env as INativeEnvironmentService,
		logService,
	);
}
...
function rmDirWithRetry(path: string | undefined): void {
	...
	try {
		...
		/* best-effort temp cleanup; Windows can briefly hold git handles */
	}
}
...
suite("AgentHostGitService - getSessionGitState (real git)", () => {
	...
	function initRepo(opts?: { remote?: string; baseBranch?: string }): string {
		...
		const run = (...args: string[]) =>
			cp.execFileSync("git", args, { cwd: tmpRoot!, env, stdio: "pipe" });
	...
	(hasGit ? test : test.skip)(
		"returns undefined for a non-git directory",
		async () => {
	...
	(hasGit ? test : test.skip)(
		"reports branch, github remote and clean state for a fresh repo",
		async () => {
	...
	(hasGit ? test : test.skip)(
		"reports outgoingChanges relative to base branch when local branch has no upstream",
		async () => {
			...
			try {
				cp.execFileSync("git", ["init", "-q", "--bare", "-b", "main"], {
					cwd: remoteDir,
					env,
					stdio: "pipe",
				});
...
suite("AgentHostGitService - computeSessionFileDiffs (real git)", () => {
	...
	(hasGit ? test : test.skip)(
		...
		async () => {
			...
			tmpRoot = dir;
			const result = await svc!.computeSessionFileDiffs(URI.file(dir), {
				sessionUri: "copilot:/s",
			});
			assert.strictEqual(result, undefined);
		},
	);
	...
	(hasGit ? test : test.skip)(
		"reports modified, added (untracked) and deleted files against HEAD",
		async () => {
			const fs = await import("fs/promises");
			...
			const byPath = new Map(
				result.map((d) => [d.after?.uri ?? d.before?.uri, d]),
			);
			...
			assert.ok(
				...
				`modified file should have before+after; result=${JSON.stringify(result.map((d) => ({ a: d.after?.uri, b: d.before?.uri })))}`,
			);
			assert.deepStrictEqual(kept!.diff, { added: 1, removed: 0 });
			assert.ok(
				kept!.before!.content.uri.startsWith("git-blob://"),
				"before content should be a git-blob: URI",
			);

			const fresh = findByBasename("fresh.txt");
			assert.ok(
				fresh?.after && !fresh.before,
				"untracked file should have only after",
			);

			const gone = findByBasename("gone.txt");
			assert.ok(
				gone?.before && !gone.after,
				"deleted file should have only before",
			);
			void byPath;
		},
	);
	...
	(hasGit ? test : test.skip)(
		"anchors against the merge-base of the requested base branch",
		async () => {
			const fs = await import("fs/promises");
			const { dir, run } = initRepo();
			...
			assert.ok(
				paths.some((p) => p?.endsWith("b.txt")),
				`expected b.txt in diff; got ${paths.join(", ")}`,
			);
		},
	);

	(hasGit ? test : test.skip)(
		"prefers origin base branch when local base branch is stale",
		async () => {
			const fs = await import("fs/promises");
			const { dir, run } = initRepo();
	...
	(hasGit ? test : test.skip)(
		"handles an empty repo (no HEAD) by treating files as added",
		async () => {
			const fs = await import("fs/promises");
			const { dir } = initRepo();
			...
			assert.ok(
				...
				"untracked file in empty repo should be an addition",
			);
		},
	);

	(hasGit ? test : test.skip)(
		"showBlob retrieves committed content",
		async () => {
			const fs = await import("fs/promises");
			const { dir, run } = initRepo();
...
suite("AgentHostGitService - worktree helpers (real git)", () => {
	...
	(hasGit ? test : test.skip)(
		"branchExists reports true for HEAD branch and false for missing branches",
		async () => {
			const dir = initRepo();
			assert.strictEqual(await svc!.branchExists(URI.file(dir), "main"), true);
			assert.strictEqual(
				await svc!.branchExists(URI.file(dir), "does-not-exist"),
				false,
			);
		},
	);
	...
	(hasGit ? test : test.skip)(
		"hasUncommittedChanges flips with untracked and committed work",
		async () => {
			const dir = initRepo();
			assert.strictEqual(
				await svc!.hasUncommittedChanges(URI.file(dir)),
				false,
			);
			const fs = await import("fs/promises");
			await fs.writeFile(join(dir, "a.txt"), "hello");
			assert.strictEqual(await svc!.hasUncommittedChanges(URI.file(dir)), true);
			cp.execFileSync("git", ["add", "a.txt"], {
				cwd: dir,
				env,
				stdio: "pipe",
			});
			cp.execFileSync("git", ["commit", "-q", "-m", "add a"], {
				cwd: dir,
				env,
				stdio: "pipe",
			});
	...
	(hasGit ? test : test.skip)(
		"addExistingWorktree attaches a worktree for an existing branch (no -b)",
		async () => {
			const dir = initRepo();
			cp.execFileSync("git", ["branch", "feature"], {
				cwd: dir,
				env,
				stdio: "pipe",
			});
	...
	(hasGit ? test : test.skip)(
		"addWorktree prefers origin start point when local branch is stale",
		async () => {
			const dir = initRepo();
			const fs = await import("fs/promises");
			cp.execFileSync(
			...
			cp.execFileSync("git", ["checkout", "-q", "-b", "upstream", "main"], {
				cwd: dir,
				env,
				stdio: "pipe",
			});
			...
			cp.execFileSync("git", ["commit", "-q", "-m", "upstream"], {
				cwd: dir,
				env,
				stdio: "pipe",
			});
			...
			cp.execFileSync("git", ["checkout", "-q", "main"], {
				cwd: dir,
				env,
				stdio: "pipe",
			});
			...
			const wtPath = join(dir, "..", `wt-${Date.now()}`);
			try {
				await svc!.addWorktree(
					URI.file(dir),
					URI.file(wtPath),
					"agents/test-origin-start-point",
					"main",
				);
				const stat = await fs.stat(join(wtPath, "upstream.txt"));
				assert.ok(
					stat.isFile(),
					"worktree should start from origin/main, not stale local main",
				);
				assert.throws(
					() =>
						cp.execFileSync(
				...
				try {
					cp.execFileSync(
						"git",
						["branch", "-D", "agents/test-origin-start-point"],
