...
class StubCopilotApiService implements ICopilotApiService {
	...
	messages(
		...
		return Promise.reject(
			new Error("non-streaming not used in integration test"),
		);
	}
...
class ProxyRoundTripSdkService implements IClaudeAgentSdkService {
	...
	async createSdkMcpServer(): Promise<never> {
		throw new Error("not implemented in integration test fake");
	}
	async tool(): Promise<never> {
		throw new Error("not implemented in integration test fake");
	}
	...
	async startup(params: {
		...
		const settingsEnv =
			settings && typeof settings === "object" && settings.env
				? settings.env
				: {};
		...
		if (!baseUrl || !bearer) {
			throw new Error(
				"ANTHROPIC_BASE_URL / ANTHROPIC_AUTH_TOKEN missing from settings.env",
			);
		}
...
class RoundTripWarmQuery implements WarmQuery {
	...
	query(prompt: string | AsyncIterable<SDKUserMessage>): Query {
		if (typeof prompt === "string") {
			throw new Error(
				"integration test: agent host always passes an AsyncIterable",
			);
		}
...
class RoundTripQuery implements AsyncGenerator<SDKMessage, void> {
	...
	constructor(
		...
		private readonly _sdk: ProxyRoundTripSdkService,
	) {
		...
		while (this._index < this._sdk.queryMessages.length) {
			...
			if (isCanUseToolMarker(item)) {
				...
				if (!startup?.canUseTool) {
					throw new Error(
						"integration test: canUseTool marker but Options.canUseTool not wired",
					);
				}
...
suite("ClaudeAgent integration (proxy-backed)", function () {
	...
	test("agent → proxy → CAPI → SSE → agent: end-to-end pipeline with real proxy and stubbed CAPI", async () => {
		...
		const realProxy = disposables.add(
			new ClaudeProxyService(new NullLogService(), capi),
		);
		...
		const accepted = await agent.authenticate(
		...
		const created = await agent.createSession({
			workingDirectory: URI.file("/integration-cwd"),
		});
		assert.strictEqual(
			sdk.capturedStartupOptions.length,
			0,
		...
		assert.deepStrictEqual(
			{
				...
				settingsBaseUrlIsLoopback:
					typeof settingsEnv["ANTHROPIC_BASE_URL"] === "string" &&
					settingsEnv["ANTHROPIC_BASE_URL"].startsWith("http://127.0.0.1:"),
				settingsBearerHasNonceAndSession:
					typeof settingsEnv["ANTHROPIC_AUTH_TOKEN"] === "string" &&
		...
		assert.strictEqual(
			sdk.warmQueries[0].asyncDisposeCount,
			1,
			"WarmQuery is asyncDisposed on session dispose",
		);
	});
	...
	test("proxy rejects a request whose bearer carries a wrong nonce (auth contract)", async () => {
		...
		try {
			...
			assert.strictEqual(
				capi.messagesCallCount.count,
				0,
				"auth check fires before any upstream call",
			);
		} finally {
			handle.dispose();
		}
	...
	test("Phase 7 §5.3 — canUseTool / onElicitation closures wired through to Options on materialize", async () => {
		...
		const startup = sdk.capturedStartupOptions[0];
		assert.ok(
			typeof startup.canUseTool === "function",
			"canUseTool was wired into Options",
		);
		assert.ok(
			typeof startup.onElicitation === "function",
			"onElicitation was wired into Options",
		);
	...
	test("Phase 7 §5.3 — Read tool round-trip: SDK tool_use → pending_confirmation → respondToPermissionRequest(true) → tool_result → continuation", async () => {
		...
		sdk.queryMessages = [
			...
			makeStreamEvent(
				sessionId,
				makeContentBlockStartToolUse(1, TOOL_USE_ID, "Read"),
			),
			makeStreamEvent(
				sessionId,
				makeInputJsonDelta(1, '{"file_path":"/tmp/x"}'),
			),
		...
		disposables.add(
			agent.onDidSessionProgress((s) => {
				...
				if (
					...
					s.state.toolCallId === TOOL_USE_ID
				) {
				...
				summary: [
					{
						...
						partKind: ResponsePartKind.Markdown,
						content: "",
					},
