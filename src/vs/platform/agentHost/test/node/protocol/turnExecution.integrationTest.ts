...
suite("Protocol WebSocket — Turn Execution", function () {
	...
	test("send message and receive responsePart + turnComplete", async function () {
		...
		const responsePart = await client.waitForNotification((n) =>
			isActionNotification(n, "session/responsePart"),
		);
		const responsePartAction = getActionEnvelope(responsePart)
			.action as IResponsePartAction;
		assert.strictEqual(responsePartAction.part.kind, ResponsePartKind.Markdown);
		assert.strictEqual(
			(responsePartAction.part as MarkdownResponsePart).content,
			"Hello, world!",
		);
	...
	test("tool invocation: toolCallStart → toolCallComplete → responsePart → turnComplete", async function () {
		...
		const toolComplete = await client.waitForNotification((n) =>
	...
	test("error prompt triggers session/error", async function () {
		...
		const errorNotif = await client.waitForNotification((n) =>
	...
	test("fetchTurns returns completed turn history", async function () {
		...
		const result = await client.call<FetchTurnsResult>("fetchTurns", {
			channel: sessionUri,
			limit: 10,
		});
	...
	test("usage info is captured on completed turn", async function () {
		...
		const usageNotif = await client.waitForNotification((n) =>
			isActionNotification(n, "session/usage"),
		);
		const usageAction = getActionEnvelope(usageNotif).action as {
			type: string;
			usage: {
				inputTokens: number;
				outputTokens: number;
	...
	test("modifiedAt updates on turn completion", async function () {
		...
		const initialModifiedAt = (initialSnapshot.snapshot!.state as SessionState)
			.summary.modifiedAt;
		...
		const updatedModifiedAt = (updatedSnapshot.snapshot!.state as SessionState)
			.summary.modifiedAt;
	...
	test("subagent: inner tool calls land in child session, not parent", async function () {
		...
		const parentSnapshot = await client.call<SubscribeResult>("subscribe", {
			channel: sessionUri,
		});
		const parentState = parentSnapshot.snapshot!.state as SessionState;
		const childSnapshot = await client.call<SubscribeResult>("subscribe", {
			channel: childUri,
		});
		...
		const parentToolCalls = parentTurn.responseParts.filter(
		...
		assert.deepStrictEqual(
			childToolNames,
			["echo_tool"],
			"child subagent session should contain the inner `echo_tool` call",
		);
	});
});
