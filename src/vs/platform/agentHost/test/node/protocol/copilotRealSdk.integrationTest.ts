...
(REAL_SDK_ENABLED ? suite : suite.skip)(
	"Protocol WebSocket — Real Copilot SDK (Copilot-specific)",
	function () {
		...
		test("usage reports include Copilot cost metadata", async function () {
			...
			dispatchTurn(
				...
				sessionUri,
				"turn-usage",
				'Reply with exactly "usage-ok" and do not use tools.',
				1,
			);

			const usageNotif = await client.waitForNotification(
				(n) => isActionNotification(n, "session/usage"),
				90_000,
			);
			...
			assert.ok(
				usageAction.usage.inputTokens === undefined ||
					usageAction.usage.inputTokens > 0,
			);
			assert.ok(
				usageAction.usage.outputTokens === undefined ||
					usageAction.usage.outputTokens > 0,
			);

			const cost = usageAction.usage._meta?.cost;
			if (typeof cost !== "number") {
				assert.fail(
					`expected usage._meta.cost to be numeric: ${JSON.stringify(usageAction.usage)}`,
				);
			}
			assert.ok(
				cost > 0,
				`expected usage._meta.cost to be positive: ${JSON.stringify(usageAction.usage)}`,
			);
		...
		test("strips redundant `cd <workingDirectory> &&` prefix from shell tool calls", async function () {
			...
			const escapedWorkingDirPath = expectedWorkingDirPath.replace(
				/[.*+?^${}()|[\]\\]/g,
				"\\$&",
			);
			...
			while (true) {
				...
				if (
					isActionNotification(next, "session/turnComplete") ||
					isActionNotification(next, "session/error")
				) {
					break;
				}
