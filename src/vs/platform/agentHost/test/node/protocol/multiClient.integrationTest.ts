...
suite("Protocol WebSocket — Multi-Client", function () {
	...
	test("sessionAdded notification is broadcast to all connected clients", async function () {
		...
		assert.strictEqual(
			uri1,
			uri2,
			"both clients should see the same session URI",
		);
	...
	test("sessionRemoved notification is broadcast to all connected clients", async function () {
		...
		await client2.call("initialize", {
			channel: ROOT_STATE_URI,
			protocolVersions: [PROTOCOL_VERSION],
			clientId: "test-broadcast-remove-2",
		});
		client2.clearReceived();
		...
		assert.ok(
			n2,
			"client 2 should receive sessionRemoved even without subscribing",
		);
	...
	test("two clients on same session both see actions", async function () {
		...
		const d1 = await client.waitForNotification((n) =>
			isActionNotification(n, "session/responsePart"),
		);
		const d2 = await client2.waitForNotification((n) =>
			isActionNotification(n, "session/responsePart"),
		);
		assert.ok(d1);
		assert.ok(d2);
	...
	test("client B sends message on session created by client A", async function () {
		...
		const r1 = await client.waitForNotification((n) =>
			isActionNotification(n, "session/responsePart"),
		);
		const r2 = await client2.waitForNotification((n) =>
			isActionNotification(n, "session/responsePart"),
		);
	...
	test("unsubscribe stops receiving session actions", async function () {
		...
		const sessionActions = client.receivedNotifications((n) =>
			isActionNotification(n, "session/"),
		);
		assert.strictEqual(
			sessionActions.length,
			0,
			"unsubscribed client should not receive session actions",
		);
	...
	test("unsubscribed client receives no actions but still gets notifications", async function () {
		...
		await new Promise((resolve) => setTimeout(resolve, 300));
		const sessionActions = client2.receivedNotifications(
			(n) => n.method === "action",
		);
		assert.strictEqual(
			sessionActions.length,
			0,
			"unsubscribed client should receive no session actions",
		);
		...
		assert.ok(
			removed,
			"unsubscribed client should still receive sessionRemoved notification",
		);
	...
	test("late subscriber gets current state via snapshot", async function () {
		...
		await client2.call("initialize", {
			channel: ROOT_STATE_URI,
			protocolVersions: [PROTOCOL_VERSION],
			clientId: "test-late-sub-2",
		});

		const result = await client2.call<SubscribeResult>("subscribe", {
			channel: sessionUri,
		});
		const state = result.snapshot!.state as SessionState;
		assert.ok(
			state.turns.length >= 1,
			`late subscriber should see completed turn, got ${state.turns.length}`,
		);
	...
	test("reconnect replays missed actions", async function () {
		...
		const allActions = client.receivedNotifications(
			(n) => n.method === "action",
		);
		...
		assert.ok(
			result.type === "replay" || result.type === "snapshot",
			"should receive replay or snapshot",
		);
