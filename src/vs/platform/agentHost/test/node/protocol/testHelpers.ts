...
export class TestProtocolClient {
	...
	private readonly _notifWaiters: {
		predicate: (n: AhpNotification) => boolean;
		resolve: (n: AhpNotification) => void;
		reject: (err: Error) => void;
	}[] = [];
	...
	call<T>(method: string, params?: unknown, timeoutMs = 5000): Promise<T> {
		...
		return new Promise<T>((resolve, reject) => {
			const timer = setTimeout(() => {
				...
				reject(
					new Error(
						`Timeout waiting for response to ${method} (id=${id}, ${timeoutMs}ms)`,
					),
				);
			}, timeoutMs);
	...
	receivedNotifications(
		...
		return predicate
			? this._notifications.filter(predicate)
			: [...this._notifications];
	}
...
export async function startServer(options?: {
	...
	return new Promise((resolve, reject) => {
		const serverPath = fileURLToPath(
			new URL("../../../node/agentHostServerMain.js", import.meta.url),
		);
		const args = [
			"--enable-mock-agent",
			"--port",
 ...
 */
export async function startRealServer(options?: {
...
export function nextSessionUri(): string {
	return URI.from({
		scheme: "mock",
		path: `/test-session-${++sessionCounter}`,
	}).toString();
}

export function isActionNotification(
	n: AhpNotification,
	actionType: string,
): boolean {
	if (n.method !== "action") {
...
export async function createAndSubscribeSession(
