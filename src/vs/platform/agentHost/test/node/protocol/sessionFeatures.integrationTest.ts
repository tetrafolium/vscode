...
suite("Protocol WebSocket — Session Features", function () {
	...
	test("client titleChanged updates session state snapshot", async function () {
		...
		const titleAction = getActionEnvelope(titleNotif)
			.action as ITitleChangedAction;
	...
	test("first turn immediately sets title to user message", async function () {
		...
		const sessionUri = await createAndSubscribeSession(
			client,
			"test-immediate-title",
		);

		// Verify the session starts with the default placeholder title
		const before = await client.call<SubscribeResult>("subscribe", {
			channel: sessionUri,
		});
		assert.strictEqual(
			(before.snapshot!.state as SessionState).summary.title,
			"",
		);
	...
	test("session model flows through create, subscribe, listSessions, and modelChanged", async function () {
		...
		const initialSnapshot = await client.call<SubscribeResult>("subscribe", {
		...
		assert.deepStrictEqual(
			initialList.items.find((s) => s.resource === createdSessionUri)?.model,
			{ id: "mock-model" },
		);
		...
		const modelNotif = await client.waitForNotification((n) =>
			isActionNotification(n, "session/modelChanged"),
		);
		const modelAction = getActionEnvelope(modelNotif)
			.action as IModelChangedAction;
		assert.deepStrictEqual(modelAction.model, { id: "mock-model-2" });

		const updatedSnapshot = await client.call<SubscribeResult>("subscribe", {
			channel: createdSessionUri,
		});
		...
		assert.deepStrictEqual(
			updatedList.items.find((s) => s.resource === createdSessionUri)?.model,
			{ id: "mock-model-2" },
		);
	});
	...
	test("reasoning events produce reasoning response parts and append actions", async function () {
		...
		const reasoningAction = getActionEnvelope(reasoningPart)
			.action as IResponsePartAction;
		...
		const appendNotif = await client.waitForNotification((n) =>
			isActionNotification(n, "session/reasoning"),
		);
	...
	test("queued message is auto-consumed when session is idle", async function () {
		...
		await client.waitForNotification((n) =>
			isActionNotification(n, "session/turnStarted"),
		);
		await client.waitForNotification((n) =>
		...
		assert.strictEqual(
			state.turns[state.turns.length - 1].message.text,
			"hello",
		);
		// Queue should be empty after consumption
		assert.ok(
			!state.queuedMessages?.length,
			"queued messages should be empty after consumption",
		);
	});
	...
	test("queued message waits for in-progress turn to complete", async function () {
		...
		const firstComplete = await client.waitForNotification((n) => {
			...
			return (
				(getActionEnvelope(n).action as { turnId: string }).turnId ===
				"turn-first"
			);
		});
		...
		const secondComplete = await client.waitForNotification((n) => {
			...
			return (
				(envelope.action as { turnId: string }).turnId !== "turn-first" &&
				envelope.serverSeq > firstSeq
			);
		});
		assert.ok(
			secondComplete,
			"should receive a second turnComplete from the queued message",
		);
		...
		assert.ok(
			...
			`expected >= 2 turns but got ${state.turns.length}`,
		);
	});
	...
	test("steering message is set and consumed by agent", async function () {
		...
		const setNotif = await client.waitForNotification((n) =>
			isActionNotification(n, "session/pendingMessageSet"),
		);
		...
		const removedNotif = await client.waitForNotification((n) =>
			isActionNotification(n, "session/pendingMessageRemoved"),
		);
		assert.ok(
			removedNotif,
			"should see pendingMessageRemoved after agent consumes steering",
		);
		...
		assert.ok(
			...
			"steering message should be cleared after consumption",
		);
	});
	...
	test("truncate session removes turns after specified turn", async function () {
		...
		let snapshot = await client.call<SubscribeResult>("subscribe", {
			channel: sessionUri,
		});
		...
		await client.waitForNotification((n) =>
			isActionNotification(n, "session/truncated"),
		);
	...
	test("fork creates a new session with source history", async function () {
		...
		const snapshot = await client.call<SubscribeResult>("subscribe", {
			channel: addedSession.summary.resource,
		});
		const state = snapshot.snapshot!.state as SessionState;
		...
		assert.strictEqual(
			state.turns.length,
			1,
			"forked session should have 1 turn",
		);

		// Source session should be unaffected
		const sourceSnapshot = await client.call<SubscribeResult>("subscribe", {
			channel: sessionUri,
		});
