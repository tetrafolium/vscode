...
suite("Protocol WebSocket - Session Config", function () {
	...
	test("resolveSessionConfig returns schema and re-resolves dependent read-only state", async function () {
		...
		const initial = await client.call<ResolveSessionConfigResult>(
		...
		assert.deepStrictEqual(initial.values, {
			isolation: "worktree",
			branch: "main",
		});
		assert.deepStrictEqual(Object.keys(initial.schema.properties), [
			"isolation",
			"branch",
		]);
		...
		const folder = await client.call<ResolveSessionConfigResult>(
			"resolveSessionConfig",
			{
				channel: ROOT_STATE_URI,
				provider: "mock",
	...
	test("createSession stores config schema and values on session state", async function () {
		...
		assert.deepStrictEqual(Object.keys(state.config?.schema.properties ?? {}), [
			"isolation",
			"branch",
		]);
	});
	...
	test("session/configChanged merges config values into session state", async function () {
		...
		const configChanged = await client.waitForNotification((n) =>
			isActionNotification(n, ActionType.SessionConfigChanged),
		);
		assert.strictEqual(
			getActionEnvelope(configChanged).action.type,
...
suite(
	"Protocol WebSocket - Session Config persistence across restarts",
	function () {
		...
		test("persisted config values are restored on subscribe after server restart", async function () {
			...
			try {
				...
				await client1.call("initialize", {
					channel: ROOT_STATE_URI,
					protocolVersions: [PROTOCOL_VERSION],
					clientId: "test-config-restore-1",
				});
				...
				await client1.call<SubscribeResult>("subscribe", {
					channel: sessionUri,
				});
				...
				const configChanged = await client1.waitForNotification((n) =>
					isActionNotification(n, ActionType.SessionConfigChanged),
				);
				assert.strictEqual(
					getActionEnvelope(configChanged).action.type,
				...
				await new Promise<void>((resolve) =>
					server1.process.once("exit", () => resolve()),
				);
			}
			...
			try {
				...
				await client2.call("initialize", {
					channel: ROOT_STATE_URI,
					protocolVersions: [PROTOCOL_VERSION],
					clientId: "test-config-restore-2",
				});
				...
				const snapshot = await client2.call<SubscribeResult>("subscribe", {
					channel: sessionUri,
				});
				const state = snapshot.snapshot!.state as SessionState;
				...
				assert.ok(
					state.config,
					"restored session should have state.config populated",
				);
				...
				await new Promise<void>((resolve) =>
					server2.process.once("exit", () => resolve()),
				);
			}
		});
	},
