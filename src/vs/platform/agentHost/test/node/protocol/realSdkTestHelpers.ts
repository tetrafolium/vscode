...
export function resolveGitHubToken(): string {
	...
	try {
		...
		throw new Error(
			"No GITHUB_TOKEN set and `gh auth token` failed. Run `gh auth login` first.",
		);
	}
}
...
export async function createRealSession(
	...
	await c.call(
		"initialize",
		{ channel: ROOT_STATE_URI, protocolVersions: [PROTOCOL_VERSION], clientId },
		30_000,
	);
	await c.call(
		"authenticate",
	...
	const subscribeResult = await c.call<SubscribeResult>("subscribe", {
		channel: sessionUri,
	});
...
): Record<string, SessionInputAnswer> | undefined {
		...
		request.questions.map((question) => {
			switch (question.kind) {
				...
				case SessionInputQuestionKind.SingleSelect: {
					const preferredOption =
						question.options.find(
							(option) =>
								/interactive/i.test(option.id) ||
								/interactive/i.test(option.label),
						) ??
				...
				case SessionInputQuestionKind.MultiSelect: {
					const preferredOptions = question.options.filter(
...
export function getMarkdownResponseText(c: TestProtocolClient): string {
	...
	for (const notification of c.receivedNotifications(
		(n) =>
			isActionNotification(n, "session/responsePart") ||
			isActionNotification(n, "session/delta"),
	)) {
		const action = getActionEnvelope(notification).action;
		if (
			action.type === "session/responsePart" &&
			...
			markdownPartIds.has(action.partId)
		) {
...
export async function driveTurnToCompletion(
	...
	while (true) {
		const notification = await c.waitForNotification(
			(n) =>
				!seenNotifications.has(n as object) &&
		...
		if (isActionNotification(notification, "session/toolCallReady")) {
			const action = getActionEnvelope(notification)
				.action as SessionToolCallReadyAction;
			if (!action.confirmed) {
		...
		if (isActionNotification(notification, "session/inputRequested")) {
			...
			const action = getActionEnvelope(notification)
				.action as SessionInputRequestedAction;
			c.notify("dispatchAction", {
				clientSeq: nextClientSeq++,
				action: {
...
export function terminalResourceFromContent(
	content: readonly ToolResultContent[],
): string | undefined {
	const terminalContent = content.find(
		(c) => c.type === ToolResultContentType.Terminal,
	);
...
export function findToolNameForCall(
	c: TestProtocolClient,
	toolCallId: string,
): string | undefined {
	return c
		.receivedNotifications((n) =>
 ...
 * tool calls.
 */
export function startBackgroundApprovalLoop(
	c: TestProtocolClient,
	options: IBackgroundApprovalLoopOptions,
): IBackgroundApprovalLoop {
	const errors: string[] = [];
	...
	const loop = (async () => {
		while (active) {
			try {
				...
				const matchingRule = options.allow.find(
					(rule) =>
				...
				if (!matchingRule) {
					errors.push(
						`unexpected tool call: toolName=${toolName ?? "<unknown>"} input=${JSON.stringify(action.toolInput)}`,
					);
					c.notify("dispatchAction", {
	...
	(config.enabled ? suite : suite.skip)(config.suiteTitle, function () {
		...
		test("sends a simple message and receives a response", async function () {
			...
			const sessionUri = await createRealSession(
				client,
				config,
				`real-sdk-simple-${config.provider}`,
				createdSessions,
				URI.file(tmpdir()).toString(),
			);
			dispatchTurn(
				client,
				sessionUri,
			...
			const responseParts = client.receivedNotifications((n) =>
				isActionNotification(n, "session/responsePart"),
			);
			assert.ok(
				responseParts.length > 0,
				"should have received at least one response part",
			);
		});
		...
		test("listModels returns well-shaped model entries after authenticate", async function () {
			...
			await client.call(
				"initialize",
				{
					...
					clientId: `real-sdk-list-models-${config.provider}`,
				},
				30_000,
			);
			...
			const rootResult = await client.call<SubscribeResult>(
				"subscribe",
				{ channel: ROOT_STATE_URI },
				30_000,
			);
			const initial = rootResult.snapshot!.state as RootState;
			const providerAgent = initial.agents.find(
				(a) => a.provider === config.provider,
			);
			assert.ok(
				providerAgent,
				`Expected ${config.provider} agent in root state, got: ${initial.agents.map((a) => a.provider).join(", ")}`,
			);
			...
			if (agent.models.length === 0) {
				try {
					const notif = await client.waitForNotification((n) => {
						...
						const action = getActionEnvelope(n)
							.action as RootAgentsChangedAction;
						const a = action.agents.find((a) => a.provider === config.provider);
						return !!a && a.models.length > 0;
					}, 30_000);
					const action = getActionEnvelope(notif)
						.action as RootAgentsChangedAction;
					...
					const seen = client
						.receivedNotifications((n) =>
							isActionNotification(n, "root/agentsChanged"),
						)
						.map((n) => {
							const a = getActionEnvelope(n).action as RootAgentsChangedAction;
							const entry = a.agents.find(
								(x) => x.provider === config.provider,
							);
							return entry
								? {
					...
					throw new Error(
						`${config.provider}: timed out waiting for agentsChanged with non-empty models. Observed agentsChanged: ${JSON.stringify(seen)}. Original error: ${err instanceof Error ? err.message : String(err)}`,
					);
				}
			}
			...
			assert.ok(
				...
				"Expected at least one model from listModels",
			);
			...
			for (const model of agent.models) {
				assert.strictEqual(
					typeof model.id,
					"string",
					`model.id should be a string: ${JSON.stringify(model)}`,
				);
				assert.ok(
					model.id.length > 0,
				...
				assert.strictEqual(
					typeof model.name,
					"string",
					`model.name should be a string: ${JSON.stringify(model)}`,
				);
				assert.strictEqual(
					model.provider,
					config.provider,
					`model.provider should be ${config.provider}: ${JSON.stringify(model)}`,
				);
				assert.ok(
		...
		test("tool call triggers permission request and can be approved", async function () {
			...
			const sessionUri = await createRealSession(
				client,
				config,
				`real-sdk-permission-${config.provider}`,
				createdSessions,
			...
			while (true) {
				const next = await client.waitForNotification(
					(n) =>
						(isActionNotification(n, "session/toolCallReady") &&
							(getActionEnvelope(n).action as { confirmed?: string })
								.confirmed === undefined) ||
			...
			const toolStarts = client.receivedNotifications((n) =>
				isActionNotification(n, "session/toolCallStart"),
			);
			assert.ok(toolStarts.length > 0, "expected at least one shell tool call");
		});

		(config.supportsPlanMode ? test : test.skip)(
			...
			async function () {
				...
				const sessionUri = await createRealSession(
					client,
					config,
					`real-sdk-plan-mode-${config.provider}`,
					createdSessions,
					URI.file(tempDir).toString(),
				);
				...
				assert.strictEqual(
					planTurn.sawPendingConfirmation,
					false,
					"should not have received pending-confirmation toolCallReady while writing session-state plan.md",
				);
				assert.ok(
					planTurn.sawInputRequest,
					`should reach the ${config.exitPlanModeToolName} question so the test can continue the same session`,
				);

				const extraSessionNotificationsAfterPlan = client.receivedNotifications(
					(n) =>
						n.method === NotificationType.SessionAdded &&
						(n.params as SessionAddedParams).summary.resource !== sessionUri,
				);
				assert.strictEqual(
					extraSessionNotificationsAfterPlan.length,
					0,
					"should not create a second session while answering the plan-mode question",
				);
				...
				assert.strictEqual(
					followupTurn.sawPendingConfirmation,
					false,
					"follow-up turn should not surface new pending confirmations",
				);
				assert.match(
					followupTurn.responseText,
					/hello world/i,
					"follow-up turn should retain the original plan context",
				);

				const extraSessionNotificationsAfterFollowup =
				...
				assert.strictEqual(
					extraSessionNotificationsAfterFollowup.length,
					0,
					"sending another message should stay on the same session instead of forking",
				);

				const resubscribeResult = await client.call<SubscribeResult>(
					"subscribe",
					{ channel: sessionUri },
				);
				const finalSnapshot = resubscribeResult.snapshot!.state as SessionState;
				assert.strictEqual(
					finalSnapshot.summary.resource,
					sessionUri,
					"follow-up turn should keep the original session resource",
				);
			},
		);
		...
		test("can abort a running turn", async function () {
			...
			const sessionUri = await createRealSession(
				client,
				config,
				`real-sdk-abort-${config.provider}`,
				createdSessions,
				URI.file(tmpdir()).toString(),
			);
			dispatchTurn(
		...
		test("session is created with the correct working directory", async function () {
			...
			await client.call("initialize", {
				channel: ROOT_STATE_URI,
				protocolVersions: [PROTOCOL_VERSION],
				clientId: `real-sdk-workdir-${config.provider}`,
			});
			...
			const subscribeResult = await client.call<SubscribeResult>("subscribe", {
				channel: sessionUri,
			});
		...
		(config.supportsWorktreeIsolation ? test : test.skip)(
			...
			async function () {
				...
				execSync('git config user.email "agent-host-test@example.com"', {
					cwd: tempDir,
				});
				...
				const resolvedWorkingDirectoryPath = URI.parse(
					addedSummary.workingDirectory!,
				).fsPath;
				...
				const errors = client.receivedNotifications((n) =>
					isActionNotification(n, "session/error"),
				);
				assert.strictEqual(
					errors.length,
				...
				assert.ok(
					responseParts.length > 0,
					"should have received at least one response part after session refresh",
				);

				client.clearReceived();
				dispatchTurn(
					client,
					addedSummary.resource,
					"turn-wt-terminal",
				...
				const terminalContentNotif = await client.waitForNotification((n) => {
					...
					const action = getActionEnvelope(n).action as {
						toolCallId: string;
						content: readonly ToolResultContent[];
					};
					return (
				...
				const terminalContentAction = getActionEnvelope(terminalContentNotif)
					.action as { content: readonly ToolResultContent[] };
				const terminalUri = terminalResourceFromContent(
					terminalContentAction.content,
				);
				assert.ok(
					terminalUri,
					"shell tool should expose its terminal resource",
				);

				const terminalSubscribeResult = await client.call<SubscribeResult>(
					"subscribe",
					{ channel: terminalUri },
				);
				const initialTerminalState = terminalSubscribeResult.snapshot!
					.state as TerminalState;
				assert.strictEqual(
					initialTerminalState.cwd,
					resolvedWorkingDirectoryPath,
					"terminal should be created in the resolved worktree directory",
				);
				...
				const terminalSnapshot = await client.call<SubscribeResult>(
					"subscribe",
					{ channel: terminalUri },
				);
		...
		(config.supportsSubagents ? test : test.skip)(
			...
			async function () {
				...
				const sessionUri = await createRealSession(
					client,
					config,
					`real-sdk-subagent-${config.provider}`,
					createdSessions,
					URI.file(tempDir).toString(),
				);
					...
					while (approvalsActive) {
						try {
							...
							if (!processedSeqs.has(envelope.serverSeq)) {
								processedSeqs.add(envelope.serverSeq);
								const action = envelope.action as {
									turnId: string;
									toolCallId: string;
									confirmed?: string;
								};
				...
				const subagentContentNotif = await client.waitForNotification((n) => {
					...
					return (
						...
						action.content.some(
							(c) => c.type === ToolResultContentType.Subagent,
						)
					);
				}, 120_000);

				const parentContent = (
					getActionEnvelope(subagentContentNotif).action as {
						content: readonly ToolResultContent[];
					}
				).content;
				const subagentRef = parentContent.find(
					(c): c is ToolResultSubagentContent =>
						c.type === ToolResultContentType.Subagent,
				)!;
				const subagentSessionUri = subagentRef.resource as unknown as string;
				assert.ok(
					typeof subagentSessionUri === "string" &&
						isSubagentSession(subagentSessionUri),
					`subagent session URI should be subagent-shaped, got: ${JSON.stringify(subagentSessionUri)}`,
				);

				await client.call<SubscribeResult>("subscribe", {
					channel: subagentSessionUri,
				});
				...
				const toolStarts = client
					.receivedNotifications((n) =>
						isActionNotification(n, "session/toolCallStart"),
					)
					.map((n) => ({
						channel: getActionEnvelope(n).channel,
						action: getActionEnvelope(n).action as SessionToolCallStartAction,
					}));

				const parentStarts = toolStarts
					.filter((t) => t.channel === sessionUri)
					.map((t) => t.action);
				const subagentStarts = toolStarts
					.filter((t) => t.channel === subagentSessionUri)
					.map((t) => t.action);

				const subagentToolNames = new Set<string>(config.subagentToolNames);
				const parentNonTaskStarts = parentStarts.filter(
					(a) => !subagentToolNames.has(a.toolName),
				);
