...
} from "../../../common/state/protocol/notifications.js";
...
suite("Protocol WebSocket — Session Lifecycle", function () {
	...
	test("listSessions returns sessions", async function () {
		...
		await client.waitForNotification((n) => n.method === "root/sessionAdded");
	...
	test("subscribe to a pre-existing session restores turns from agent history", async function () {
		...
		assert.ok(
			preExisting,
			"listSessions should include the pre-existing session",
		);
		...
		const result = await client.call<SubscribeResult>("subscribe", {
			channel: preExistingUri,
		});
		...
		assert.strictEqual(
			state.lifecycle,
			"ready",
			"restored session should be in ready state",
		);
		assert.ok(
			state.turns.length >= 1,
			`expected at least 1 restored turn but got ${state.turns.length}`,
		);
		...
		assert.ok(
			...
			"turn should have tool call response parts",
		);
		...
		assert.ok(
			mdParts.some((p) => p.content.includes("file1.ts")),
			"turn should have markdown part mentioning file1.ts",
		);
		...
		assert.strictEqual(
			sessionAddedNotifs.length,
			0,
			"restore should not emit sessionAdded",
		);
	});
	...
	test("isRead and isArchived flags survive in listSessions after dispatch", async function () {
		...
		await client.waitForNotification((n) =>
			isActionNotification(n, "session/isArchivedChanged"),
		);
		...
		await client.waitForNotification((n) =>
			isActionNotification(n, "session/isReadChanged"),
		);
		...
		assert.ok(
			state.summary.status & SessionStatus.IsArchived,
			"IsArchived flag should be set in snapshot",
		);
		assert.ok(
			state.summary.status & SessionStatus.IsRead,
			"IsRead flag should be set in snapshot",
		);
		...
		await client2.call("initialize", {
			channel: ROOT_STATE_URI,
			protocolVersions: [PROTOCOL_VERSION],
			clientId: "test-read-archived-flags-2",
		});
		...
		for (let i = 0; i < 20; i++) {
			...
			if (
				session &&
				session.status & SessionStatus.IsArchived &&
				session.status & SessionStatus.IsRead
			) {
				break;
			}
			await timeout(100);
		...
		assert.ok(
			session.status & SessionStatus.IsArchived,
			"IsArchived should be persisted in listSessions",
		);
		assert.ok(
			session.status & SessionStatus.IsRead,
			"IsRead should be persisted in listSessions",
		);
