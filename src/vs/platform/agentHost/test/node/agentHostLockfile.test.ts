...
suite("Agent Host Lockfile (local)", () => {
	...
	setup(async () => {
		tempDir = await fs.promises.mkdtemp(
			join(os.tmpdir(), "agent-host-lockfile-test-"),
		);
	...
	suite("getLocalAgentHostLockfilePath", () => {
		test("returns absolute path under home directory", () => {
			const result = getLocalAgentHostLockfilePath(
		...
		test("rejects unsafe server data folder names", () => {
			assert.throws(
				() => getLocalAgentHostLockfilePath("foo bar", "stable"),
			...
			assert.throws(
				() => getLocalAgentHostLockfilePath("$(whoami)", "stable"),
				/Unsafe server data folder name/,
			);
		});
		...
		test("rejects unsafe quality strings", () => {
			assert.throws(
				() => getLocalAgentHostLockfilePath(".vscode-server-oss", "foo bar"),
				/Unsafe quality/,
			);
			assert.throws(
				() => getLocalAgentHostLockfilePath(".vscode-server-oss", "/abs"),
				/Unsafe quality/,
			);
		});
	});
	...
	suite("readActiveAgentHostFromLockfile", () => {
		test("returns notFound when file is missing", async () => {
			const result = await readActiveAgentHostFromLockfile(
				lockfilePath,
				logService,
			);
