...
suite("CommandAutoApprover", () => {
	...
	suite("shouldAutoApprove", () => {
		...
		test("approves allowed readonly commands", () => {
			...
			assert.strictEqual(
				approver.shouldAutoApprove("head -n 10 file.txt"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("tail -f log.txt"),
				"approved",
			);
			...
			assert.strictEqual(
				approver.shouldAutoApprove("grep -r pattern ."),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("wc -l file.txt"),
				"approved",
			);
		...
		test("denies denied commands", () => {
			...
			assert.strictEqual(
				approver.shouldAutoApprove("curl http://evil.com"),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("wget http://evil.com"),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("chmod 777 file"),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("chown root file"),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove('eval "bad stuff"'),
				"denied",
			);
			assert.strictEqual(approver.shouldAutoApprove("xargs rm"), "denied");
			assert.strictEqual(
				approver.shouldAutoApprove("dd if=/dev/zero of=/dev/sda"),
				"denied",
			);
		});
		...
		test("handles find with blocked args", () => {
			assert.strictEqual(
				approver.shouldAutoApprove('find . -name "*.ts"'),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("find . -delete"),
				"denied",
			);
			// find -exec matches the deny rule for find's dangerous args.
			assert.strictEqual(
				approver.shouldAutoApprove("find . -exec rm {} ;"),
				"denied",
			);
		});
		...
		test("returns noMatch for unknown commands", () => {
			assert.strictEqual(
				approver.shouldAutoApprove("my-custom-script"),
				"noMatch",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("python script.py"),
				"noMatch",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("node index.js"),
				"noMatch",
			);
		});
		...
		test("denies transient environment variable assignments", () => {
			assert.strictEqual(
				approver.shouldAutoApprove("FOO=bar some-command"),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("PATH=/evil:$PATH ls"),
				"denied",
			);
		});
		...
		test("approves allowed PowerShell commands", () => {
			...
			assert.strictEqual(
				approver.shouldAutoApprove("Get-Content file.txt"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove('Write-Host "hello"'),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("Select-Object Name"),
				"approved",
			);
		});
		...
		test("PowerShell case-insensitive rules work", () => {
			...
			assert.strictEqual(
				approver.shouldAutoApprove("Measure-Command"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("measure-command"),
		...
		test("denies denied PowerShell commands", () => {
			assert.strictEqual(
				approver.shouldAutoApprove("Remove-Item file.txt"),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove('Invoke-Expression "bad"'),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("Invoke-WebRequest http://evil.com"),
				"denied",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("Stop-Process -Id 1234"),
				"denied",
			);
		});
		...
		test("compound commands with denied sub-commands are not auto-approved", () => {
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo ok && rm -rf /"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("ls || curl evil.com"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("cat file; rm file"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo $(whoami)"),
				"approved",
			);
		});
		...
		test("does not auto-approve commands with write redirections to arbitrary paths", () => {
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo id > /tmp/fake-home/.bashrc"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove(`echo id > '/tmp/fake-home/.bashrc'`),
			...
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo id >> ~/.bashrc"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("cat file > out.txt"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("ls -la > listing.txt"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("pwd > /etc/passwd"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo hello 2> err.log"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo hello &> all.log"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo hello >| force.txt"),
				"approved",
			);
		});
		...
		test("write redirections to safe sinks remain auto-approved", () => {
			assert.strictEqual(
				approver.shouldAutoApprove("echo hello > /dev/null"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("echo hello >/dev/null"),
			...
			assert.strictEqual(
				approver.shouldAutoApprove("echo hello 2> /dev/null"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("echo hello > /dev/stdout"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("echo hello > /dev/stderr"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("echo hello 2>&1"),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove("ls 2>&1 > /dev/null"),
				"approved",
			);
		});
		...
		test("mixed safe and unsafe redirections still require confirmation", () => {
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo hello 2> /dev/null > /tmp/out"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo hello > out.txt 2> /dev/null"),
				"approved",
			);
		});
		...
		test("does not auto-approve <> read-write redirects or process substitution targets", () => {
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo data 1<>/etc/passwd"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo data <>/tmp/file"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("cat < <(tee ~/.bashrc <<< id)"),
				"approved",
			);
			assert.notStrictEqual(
				approver.shouldAutoApprove("ls > >(tee ~/.bashrc)"),
				"approved",
			);
		});
		...
		test("respects the isWriteDestApproved predicate for write redirections", () => {
			...
			assert.strictEqual(
				approver.shouldAutoApprove("echo hi > /workspace/log.txt", opts),
				"approved",
			);
			assert.strictEqual(
				approver.shouldAutoApprove(`echo hi > 'out.txt'`, opts),
			...
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo hi > /tmp/elsewhere", opts),
				"approved",
			);
			// Mixed: one approved, one not — still requires confirmation.
			assert.notStrictEqual(
				approver.shouldAutoApprove("echo hi > out.txt 2> /tmp/err", opts),
				"approved",
			);
			// Safe sinks and fd duplications do not invoke the predicate.
			seen.length = 0;
			assert.strictEqual(
				approver.shouldAutoApprove("echo hi > /dev/null 2>&1", opts),
				"approved",
			);
