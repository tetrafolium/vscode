...
suite("claudeToolDisplay — §4 mapping table", () => {
	...
	test("mapping snapshot covers every Phase 7 §4 row", () => {
		...
		const snapshot = TOOLS.map(
			(t) =>
				[t, getClaudePermissionKind(t), getClaudeToolDisplayName(t)] as const,
		);
		...
		assert.deepStrictEqual(
			{
				...
				notebookRead: getClaudeToolPath("NotebookRead", {
					notebook_path: "/tmp/e.ipynb",
				}),
				notebookEdit: getClaudeToolPath("NotebookEdit", {
					notebook_path: "/tmp/f.ipynb",
				}),
	...
	test("INTERACTIVE_CLAUDE_TOOLS contains exactly the user-input round-trip tools", () => {
		assert.deepStrictEqual([...INTERACTIVE_CLAUDE_TOOLS].sort(), [
			"AskUserQuestion",
			"ExitPlanMode",
		]);
	});
	...
	test("Phase 8.5 — rich rendering snapshot covers every tool row", () => {
		...
		const TOOLS = Object.keys(
			SAMPLE_INPUT,
		) as readonly (keyof typeof SAMPLE_INPUT)[];
		...
		assert.deepStrictEqual(snapshot, [
			[
				...
				"terminal",
				{ toolKind: "terminal" },
				{ markdown: "Running `git status`" },
				{ markdown: "Ran `git status`" },
				'"Run shell command" failed',
				"git status",
			],
			[
				"BashOutput",
			...
			[
				...
				undefined,
				undefined,
				{ markdown: "Reading [foo.ts](file:///src/foo.ts)" },
				{ markdown: "Read [foo.ts](file:///src/foo.ts)" },
				'"Read file" failed',
			...
			[
				...
				"search",
				{ toolKind: "search" },
				{ markdown: "Finding files matching `**/*.ts`" },
				{ markdown: "Found files matching `**/*.ts`" },
				'"Find files" failed',
				"**/*.ts",
			],
			[
				...
				"search",
				{ toolKind: "search" },
				{ markdown: "Searching for `IClaudeAgentSession`" },
				{ markdown: "Searched for `IClaudeAgentSession`" },
				'"Search files" failed',
				"IClaudeAgentSession",
			],
			[
				...
				undefined,
				undefined,
				{ markdown: "Listing [src](file:///src)" },
				{ markdown: "Listed [src](file:///src)" },
			...
			[
				...
				undefined,
				undefined,
				{ markdown: "Reading [nb.ipynb](file:///nb.ipynb)" },
				{ markdown: "Read [nb.ipynb](file:///nb.ipynb)" },
				'"Read notebook" failed',
			...
			[
				...
				undefined,
				undefined,
				{ markdown: "Editing [foo.ts](file:///src/foo.ts)" },
				{ markdown: "Edited [foo.ts](file:///src/foo.ts)" },
				'"Write file" failed',
				'{\n  "file_path": "/src/foo.ts",\n  "content": "..."\n}',
			],
			[
			...
			[
				...
				undefined,
				undefined,
				{ markdown: "Editing [nb.ipynb](file:///nb.ipynb)" },
				{ markdown: "Edited [nb.ipynb](file:///nb.ipynb)" },
				'"Edit notebook" failed',
				'{\n  "notebook_path": "/nb.ipynb"\n}',
			],
			[
				"TodoWrite",
			...
			[
				...
				undefined,
				undefined,
				{ markdown: "Fetching [https://example.com](https://example.com)" },
				{ markdown: "Fetched [https://example.com](https://example.com)" },
				'"Fetch URL" failed',
			...
			[
				...
				undefined,
				"Ask user a question",
				'Used "Ask user a question"',
				'"Ask user a question" failed',
				'{\n  "question": "why?"\n}',
			],
		]);
	});
	...
	test("Phase 8.5 — defensive input handling falls back to static display strings", () => {
		assert.deepStrictEqual(
			{
				bashNoCommand: getClaudeInvocationMessage(
					"Bash",
					"Run shell command",
					{},
				),
				bashWrongType: getClaudeInvocationMessage("Bash", "Run shell command", {
					command: 42,
				}),
				readMissingPath: getClaudeInvocationMessage("Read", "Read file", {}),
				grepMissingPattern: getClaudeInvocationMessage(
					"Grep",
					"Search files",
					{},
				),
				nonObjectInput: getClaudeInvocationMessage(
					"Bash",
					"Run shell command",
					null,
				),
				undefinedInput: getClaudeInvocationMessage(
					"Bash",
					"Run shell command",
					undefined,
				),
				taskNoDescription: getClaudeInvocationMessage(
					"Task",
					"Run subagent task",
					{},
				),
				bashFailed: getClaudePastTenseMessage(
					"Bash",
					"Run shell command",
					{ command: "x" },
					false,
	...
	test("Phase 8.5 — Agent row mirrors Task (subagent kind, same display name)", () => {
		assert.deepStrictEqual(
			[
				...
				getClaudeInvocationMessage("Agent", getClaudeToolDisplayName("Agent"), {
		...
		assert.deepStrictEqual(
			{
				...
				inputString: getClaudeToolInputString("mcp__github__listIssues", {
					owner: "microsoft",
					repo: "vscode",
				}),
				invocation: getClaudeInvocationMessage(
					"mcp__github__listIssues",
					"Run MCP tool github__listIssues",
					{ owner: "microsoft" },
				),
			},
			{
