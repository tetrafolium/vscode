...
} from "../../node/claude/claudeModelId.js";
...
suite("parseClaudeModelId", () => {
	...
	suite("modifiers (non-date suffixes)", () => {
		...
		test("1m on opus converts to correct SDK model ID", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-opus-4.6-1m").toSdkModelId(),
				"claude-opus-4-6-1m",
			);
		});
		...
		test("1m on opus converts to correct endpoint model ID", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-opus-4-6-1m").toEndpointModelId(),
				"claude-opus-4.6-1m",
			);
		});
		...
		test("1m on non-opus model is not included in SDK model ID", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-sonnet-4-1m").toSdkModelId(),
				"claude-sonnet-4",
			);
		});
		...
		test("1m on non-opus model is not included in endpoint model ID", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-sonnet-4-1m").toEndpointModelId(),
				"claude-sonnet-4",
			);
		});
		...
		test("1m with date suffix on opus keeps only 1m in SDK model ID", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-opus-4-6-1m-20251101").toSdkModelId(),
				"claude-opus-4-6-1m",
			);
		});
		...
		test("1m with date suffix on opus keeps only 1m in endpoint model ID", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-opus-4-6-1m-20251101").toEndpointModelId(),
				"claude-opus-4.6-1m",
			);
		});
	});
	...
	suite("unparseable inputs", () => {
		test("throws for hyphenated non-Claude IDs", () => {
			assert.throws(
				() => parseClaudeModelId("gpt-4o"),
				/Unable to parse Claude model ID: 'gpt-4o'/,
			);
		});
	...
	suite("toSdkModelId", () => {
		test("produces dash-separated version for major.minor", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-opus-4.5").toSdkModelId(),
				"claude-opus-4-5",
			);
		});
		...
		test("produces single-digit version when no minor", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-sonnet-4").toSdkModelId(),
				"claude-sonnet-4",
			);
		});
		...
		test("normalizes old-format SDK IDs to new format", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-3-5-sonnet-20241022").toSdkModelId(),
				"claude-sonnet-3-5",
			);
		});
		...
		test("strips date suffix from SDK IDs", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-opus-4-5-20251101").toSdkModelId(),
				"claude-opus-4-5",
			);
		});
	});
	...
	suite("toEndpointModelId", () => {
		test("produces dot-separated version for major.minor", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-opus-4-5-20251101").toEndpointModelId(),
				"claude-opus-4.5",
			);
		});
		...
		test("produces single-digit version when no minor", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-sonnet-4-20250514").toEndpointModelId(),
				"claude-sonnet-4",
			);
		});
		...
		test("normalizes old-format SDK IDs", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-3-5-sonnet-20241022").toEndpointModelId(),
				"claude-sonnet-3.5",
			);
		});
		...
		test("is identity for endpoint-format IDs", () => {
			assert.strictEqual(
				parseClaudeModelId("claude-haiku-4.5").toEndpointModelId(),
				"claude-haiku-4.5",
			);
		});
	});
