...
suite("computeSessionDiffs", () => {
	...
	test("omits before for creates and after for deletes", async () => {
		...
		result.sort((a, b) =>
			(getDiffUri(a) ?? "").localeCompare(getDiffUri(b) ?? ""),
		);
	...
	test("skips files with no net change", async () => {
		...
		assert.deepStrictEqual(result, []);
		assert.strictEqual(
			diffService.callCount,
			0,
			"no diff computation needed for zero net change",
		);
	});
	...
	test("tracks rename chains correctly", async () => {
		...
		assert.strictEqual(
			getDiffUri(result[0]),
			URI.file("/b.txt").toString(),
			"uses terminal path after rename",
		);
	});
	...
	test("incremental: reuses previousDiffs for untouched files", async () => {
		...
		const previousDiffs: ISessionFileDiff[] = [fileDiff("/a.txt", 42, 7)];
		...
		assert.strictEqual(
			diffService.callCount,
			1,
			"only touched file should be diffed",
		);
	});
	...
	test("incremental: rename in current turn drops old URI from previousDiffs", async () => {
		...
		const previousDiffs: ISessionFileDiff[] = [fileDiff("/old.txt", 5, 0)];
		...
		assert.strictEqual(
			getDiffUri(result[0]),
			URI.file("/new.txt").toString(),
			"uses new URI after rename",
		);
	});
	...
	test("incremental: file with zero net change in current turn is excluded even if in previousDiffs", async () => {
		...
		const previousDiffs: ISessionFileDiff[] = [fileDiff("/a.txt", 10, 5)];
	...
	test("full mode recomputes all files (no incremental options)", async () => {
		...
		assert.strictEqual(
			diffService.callCount,
			2,
			"both files should be diffed in full mode",
		);
	});
	...
	test("incremental fast path: new files only uses getFileEditsByTurn, not getAllFileEdits", async () => {
		...
		const previousDiffs: ISessionFileDiff[] = [fileDiff("/old.txt", 3, 1)];
		...
		assert.strictEqual(
			db.getAllFileEditsCalls,
			0,
			"fast path should not call getAllFileEdits",
		);
	...
	test("incremental slow path: re-edit of existing file falls back to getAllFileEdits", async () => {
		...
		const previousDiffs: ISessionFileDiff[] = [fileDiff("/a.txt", 5, 0)];
		...
		assert.strictEqual(
			db.getFileEditsByTurnCalls,
			1,
			"should try turn-scoped query first",
		);
		assert.strictEqual(
			db.getAllFileEditsCalls,
			1,
			"should fall back to getAllFileEdits",
		);
	...
	test("incremental slow path: rename in current turn falls back to getAllFileEdits", async () => {
		...
		const previousDiffs: ISessionFileDiff[] = [fileDiff("/a.txt", 1, 0)];

		const diffService = createTestDiffService();
		await computeSessionDiffs(TEST_SESSION_URI, db, diffService, {
			changedTurnId: "t2",
			previousDiffs,
		});
	...
	test("incremental: no edits in turn returns previousDiffs unchanged", async () => {
		...
		const previousDiffs: ISessionFileDiff[] = [fileDiff("/a.txt", 5, 2)];
