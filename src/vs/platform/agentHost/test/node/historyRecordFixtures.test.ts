...
suite("mapSessionEventsToHistoryRecords", () => {
	...
	test("maps user and assistant messages", async () => {
		const events: ISessionEvent[] = [
			{ type: "user.message", data: { messageId: "msg-1", content: "hello" } },
			{
				...
				data: { messageId: "msg-2", content: "world" },
			},
		];

		const result = await mapSessionEventsToHistoryRecords(
			session,
			undefined,
			events,
		);
	...
	test("skips tool_complete without matching tool_start", async () => {
		const events: ISessionEvent[] = [
			{
				type: "tool.execution_complete",
				data: { toolCallId: "orphan", success: true },
			},
		];
	...
	suite("file edit restoration", () => {
		test("restores file edits from database for edit tools", async () => {
			...
			const events: ISessionEvent[] = [
				{
					...
					data: {
						toolCallId: "tc-edit",
						toolName: "edit",
						arguments: { filePath: "/workspace/file.ts" },
					},
				},
				{
					type: "tool.execution_complete",
			...
			const content = (
				complete as { result: { content?: readonly Record<string, unknown>[] } }
			).result.content;
			assert.ok(content);
			...
			const fileEdit = content[1] as {
				before: { uri: any; content: { uri: any } };
				after: { uri: any; content: { uri: any } };
		...
		test("handles multiple file edits for one tool call", async () => {
			...
			const content = (
				result[1] as {
					result: { content?: readonly Record<string, unknown>[] };
				}
			).result.content;
			assert.ok(content);
			// Two file edits (no text since result had no content)
			const fileEdits = content.filter(
				(c) => c.type === ToolResultContentType.FileEdit,
			);
				...
				},
				{
					...
					skillComplete: {
						...
						result: {
							...
							pastTenseMessage: {
								markdown:
									"Read skill [plan](file:///abs/repo/skills/plan/SKILL.md)",
							},
						},
					},
	...
	suite("cd-prefix rewriting", () => {
		...
		function getStart(
			events: ReturnType<
				typeof mapSessionEventsToHistoryRecords
		...
		test("strips redundant bash cd prefix matching workingDirectory", async () => {
			...
			assert.deepStrictEqual(JSON.parse(start.toolArguments!), {
				command: "ls -la",
			});
		});
				...
				[
					{
						...
						data: {
							toolCallId: "tc-1",
							toolName: "edit",
							arguments: { command: "cd /workspace/proj && ls" },
						},
					},
				],
				cwd,
				...
				[
					{
						...
						data: {
							toolCallId: "tc-1",
							toolName: "powershell",
							arguments: { command: "cd /workspace/proj; dir" },
						},
					},
				],
				cwd,
