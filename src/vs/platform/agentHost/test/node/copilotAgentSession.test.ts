...
} from "../../node/copilot/copilotAgentSession.js";
...
class MockCopilotSession {
	...
	planReadResult: {
		...
		content: string | null;
		path: string | null;
	} = { exists: false, content: null, path: null };
	...
	on<K extends SessionEventType>(
		...
		return () => {
			set.delete(handler as (event: SessionEvent) => void);
		};
	}

	/** Push an event through to all registered handlers of the given type. */
	fire<K extends SessionEventType>(
		type: K,
		data: SessionEventPayload<K>["data"],
		overrides?: Partial<Omit<SessionEventPayload<K>, "type" | "data">>,
	): void {
		const event = {
			type,
			data,
	...
	readonly rpc = {
		...
		plan: {
			...
			update: async (_params: { content: string }) => {
				/* no-op */
			},
			delete: async () => {
				/* no-op */
 ...
 */
function invokeClientToolHandler(
	tool: Pick<Tool, "name" | "handler">,
	toolCallId: string,
...
type ISessionInternalsForTest = {
	...
	_pendingClientToolCalls: {
		...
		set(
			toolCallId: string,
			value: DeferredPromise<ToolResultObject>,
		): Map<string, DeferredPromise<ToolResultObject>>;
...
function getInputRequest(
	signal: AgentSignal,
): SessionInputRequestedAction["request"] {
...
async function createAgentSession(
	...
	options?: {
		...
		captureWrapperCallbacks?: {
			current?: Parameters<SessionWrapperFactory>[0];
		};
	...
	signals: AgentSignal[];
	waitForSignal: (
		predicate: (signal: AgentSignal) => boolean,
	) => Promise<AgentSignal>;
	sessionConfigUpdates: ReadonlyArray<{
		session: string;
	...
	const waiters: {
		predicate: (signal: AgentSignal) => boolean;
		deferred: DeferredPromise<AgentSignal>;
	}[] = [];

	disposables.add(
		progressEmitter.event((signal) => {
			signals.push(signal);
	...
	services.set(
		ITelemetryService,
		options?.telemetryService ?? new NullTelemetryServiceShape(),
	);
	services.set(IFileService, {
		_serviceBrand: undefined,
		readFile: async (resource: URI) => {
			if (
				options?.fileReadErrors?.includes(resource.toString()) ||
				options?.fileReadErrors?.includes(resource.fsPath)
			) {
				throw new Error("read failed");
			}
			return {
				value: VSBuffer.fromString(
					options?.fileContents?.[resource.toString()] ??
						options?.fileContents?.[resource.fsPath] ??
						"",
				),
			};
		},
	...
	const sessionConfigUpdates: Array<{
		session: string;
		patch: Record<string, unknown>;
	}> = [];
	...
	const fakeConfigurationService: IAgentConfigurationService = {
		...
		getEffectiveValue: ((_session: string, _schema: unknown, key: string) =>
			configValues[key]) as IAgentConfigurationService["getEffectiveValue"],
		getEffectiveWorkingDirectory: () => undefined,
		getSessionConfigValues: () => undefined,
		updateSessionConfig: (session, patch) => {
			sessionConfigUpdates.push({ session, patch });
		},
	...
	const session = disposables.add(
		instantiationService.createInstance(CopilotAgentSession, {
			sessionUri,
...
suite("CopilotAgentSession", () => {
				...
				attachments: [
					{
						type: "blob",
						data: encodeBase64(
							VSBuffer.fromString("Feedback text for the model"),
						),
						mimeType: "text/plain",
	...
	test("emits accumulated Copilot usage metadata", async () => {
		const { session, mockSession, signals } =
	...
	suite("permission handling", () => {
		test("read permission fires tool_ready (deferred to side effects)", async () => {
			const { session, signals, waitForSignal } =
				await createAgentSession(disposables);
		...
		test("logs and rethrows permission failures", async () => {
			...
			try {
				...
				assert.strictEqual(
					entry.args[0],
					"[Copilot:test-session-1] Failed to handle permission request: kind=read, toolCallId=tc-read-plan-missing-env",
				);
			} finally {
				if (previousXdgStateHome === undefined) {
					delete process.env["XDG_STATE_HOME"];
		...
		test("does not auto-approve session-state files from another session", async () => {
			...
			try {
				...
				assert.ok(
					session.respondToPermissionRequest("tc-write-other-plan", true),
				);
				const result = await resultPromise;
				assert.strictEqual(result.kind, "approve-once");
			} finally {
		...
		test("does not auto-approve traversal paths that escape the session-state directory", async () => {
			...
			try {
				...
				assert.ok(
					session.respondToPermissionRequest("tc-write-traversal", true),
				);
				const result = await resultPromise;
				assert.strictEqual(result.kind, "approve-once");
			} finally {
		...
		test("does not auto-approve tool-output-named files outside tmpdir", async () => {
			...
			const resultPromise = session.handlePermissionRequest({
				...
				path: join(
					"/some/other/dir",
					"copilot-tool-output-1730000000000-abc123.txt",
				),
			...
			assert.ok(
				session.respondToPermissionRequest("tc-tool-output-outside", true),
			);
			const result = await resultPromise;
			assert.strictEqual(result.kind, "approve-once");
		});
		...
		test("does not auto-approve write to a tool-output temp path", async () => {
			...
			assert.ok(
				session.respondToPermissionRequest("tc-tool-output-write", true),
			);
			const result = await resultPromise;
			assert.strictEqual(result.kind, "approve-once");
		});
		...
		test("respondToPermissionRequest returns false for unknown id", async () => {
			const { session } = await createAgentSession(disposables);
			assert.strictEqual(
				session.respondToPermissionRequest("unknown-id", true),
				false,
			);
		});
	});
	...
	suite("sendSteering", () => {
		test("promotes steering to its own turn when the SDK echoes the user message", async () => {
			...
			assert.strictEqual(
				signals.find(
					(s) =>
						s.kind === "action" &&
						(s as IAgentActionSignal).action.type ===
			...
			const turnComplete = actions.find(
				(a) => a.type === ActionType.SessionTurnComplete,
			);
			const turnStarted = actions.find(
				(a) => a.type === ActionType.SessionTurnStarted,
			);
			assert.ok(
				turnComplete,
				"should complete the in-flight turn before promoting steering",
			);
			assert.strictEqual(turnComplete.turnId, "turn-original");
			assert.ok(
				turnStarted,
				"should start a new turn for the steering message",
			);
			assert.notStrictEqual(turnStarted.turnId, "turn-original");
			assert.deepStrictEqual(turnStarted.message, {
				text: "focus on tests",
				origin: { kind: MessageKind.User },
			});
		...
		test("routes subsequent SDK events into the steering turn", async () => {
			...
			assert.ok(
				...
				"expected delta to allocate a response part",
			);
			assert.strictEqual(
				responseParts[0].turnId,
				turnStarted.turnId,
				"response part should land in the steering turn, not the original",
			);
		});
		...
		test("does not flip turns for SDK-injected user messages (non-user source)", async () => {
			...
			const turnStarted = signals.find(
				(s) =>
					s.kind === "action" &&
					(s as IAgentActionSignal).action.type ===
						ActionType.SessionTurnStarted,
			...
			assert.strictEqual(
				turnStarted,
				undefined,
				"synthetic user messages should not promote steering to a turn",
			);
		});
		...
		test("does not flip turns when the user.message content does not match", async () => {
			...
			assert.strictEqual(
				turnStarted,
				undefined,
				"unrelated user messages should not consume the pending steering",
			);
		});
		...
		test("does not signal cleanup when send fails", async () => {
			...
			mockSession.send = async () => {
				throw new Error("send failed");
			};

			await session.sendSteering({
				id: "steer-fail",
				message: { text: "will fail", origin: { kind: MessageKind.User } },
			});
			...
			assert.strictEqual(
				consumed,
				undefined,
				"should not fire steering_consumed on failure",
			);
			assert.strictEqual(
				turnStarted,
				undefined,
				"should not start a new turn on failure",
			);
		});
	});
	...
	suite("system.notification", () => {
		...
		test("idle notification starts a system-initiated turn without sending another SDK message", async () => {
			...
			assert.strictEqual(
				...
				0,
				"system notification should not call session.send",
			);
		...
		test("routes subsequent SDK events into the generated system turn", async () => {
			...
			const turnStarted = getActions(signals).find(
				(a) => a.type === ActionType.SessionTurnStarted,
			)!;
			...
			const responsePart = getActions(signals).find(
				(a) =>
					a.type === ActionType.SessionResponsePart &&
					a.part.kind === ResponsePartKind.Markdown,
			);
			assert.ok(
				responsePart,
				"expected response part for follow-up assistant delta",
			);
			assert.strictEqual(
				(responsePart as SessionResponsePartAction).turnId,
				(turnStarted as { turnId: string }).turnId,
			);
		});
		...
		test("notification during an active turn appends a SystemNotification response part", async () => {
			...
			const actions = getActions(signals);
			assert.strictEqual(
				actions.find((a) => a.type === ActionType.SessionTurnStarted),
				undefined,
				"should not create a duplicate turn",
			);
			const systemPart = actions.find(
				(a) =>
					a.type === ActionType.SessionResponsePart &&
					a.part.kind === ResponsePartKind.SystemNotification,
			) as SessionResponsePartAction | undefined;
			assert.ok(systemPart, "expected system notification response part");
			assert.strictEqual(systemPart.turnId, "turn-active");
			assert.strictEqual(
				systemPart.part.kind,
				ResponsePartKind.SystemNotification,
			);
		...
		test("generated system turn completes on session.idle", async () => {
			...
			mockSession.fire(
				"session.idle",
				{} as SessionEventPayload<"session.idle">["data"],
			);

			const turnComplete = getActions(signals).find(
				(a) => a.type === ActionType.SessionTurnComplete,
			);
			assert.ok(turnComplete, "expected idle to complete the generated turn");
			assert.strictEqual(
				(turnComplete as { turnId: string }).turnId,
				turnStarted.turnId,
			);
		});
		...
		test("events after a completed turn do not target the stale previous turn id", async () => {
			...
			assert.ok(
				lateMarkdown,
				"late event still emits a no-op action for the reducer",
			);
	...
	suite("event mapping", () => {
		...
		test("live tool_start strips redundant cd prefix matching workingDirectory", async () => {
			...
			if (isAction(startSignal, ActionType.SessionToolCallStart)) {
				...
				assert.ok(
					toolArgs && toolArgs.includes('"npm test"'),
					`toolArguments should contain rewritten command, was: ${toolArgs}`,
				);
				assert.ok(
					!toolArgs?.includes("cd /repo/project"),
					"toolArguments should not contain stripped prefix",
				);
			}
		});
		...
		test("live tool_complete past-tense message reflects the rewritten command", async () => {
			...
			if (isAction(completeSignal, ActionType.SessionToolCallComplete)) {
				...
				const pastStr =
					typeof past === "string" ? past : (past?.markdown ?? "");
				assert.ok(
					!pastStr.includes("cd /repo/project"),
					`past-tense message should not contain stripped prefix, got: ${pastStr}`,
				);
				assert.ok(
					pastStr.includes("npm test"),
					`past-tense message should contain the rewritten command, got: ${pastStr}`,
				);
			}
		});
			...
			const normalizedEvents = telemetryService.events.map((event) => {
				...
				return {
					...
					data: {
						...data,
						invocationTimeMs:
							typeof data.invocationTimeMs === "number" &&
							data.invocationTimeMs >= 0,
					},
				};
			});
			...
			assert.deepStrictEqual(normalizedEvents, [
				{
					...
					data: {
						result: "success",
						chatSessionId: AgentSession.uri(
							"copilot",
							"test-session-1",
						).toString(),
						toolId: "bash",
			...
			assert.deepStrictEqual(
				{
					...
					data: {
						...
						invocationTimeMs:
							typeof (event.data as { invocationTimeMs?: number })
								.invocationTimeMs === "number",
					},
				},
				{
		...
		test("live tool_start does not rewrite when cd target differs from workingDirectory", async () => {
			...
			if (isAction(readySignal, ActionType.SessionToolCallReady)) {
				assert.strictEqual(
					(readySignal.action as SessionToolCallReadyAction).toolInput,
					"cd /tmp && ls",
				);
			}
		});
		...
		test("edit hooks resolve relative apply_patch file paths against workingDirectory", async () => {
			...
			sessionInternals._editTracker.trackEditStart = async (path) => {
				started.push(path);
			};
			sessionInternals._editTracker.completeEdit = async (path) => {
				completed.push(path);
			};
			const patch = [
				"*** Begin Patch",
			...
			assert.deepStrictEqual(
				...
				{
					started: [
						join(workingDirectory.fsPath, "foo.ts"),
						join(workingDirectory.fsPath, "src/bar.ts"),
						absolutePath,
					],
					completed: [
						join(workingDirectory.fsPath, "foo.ts"),
		...
		test("tool_complete resolves relative apply_patch file paths before taking completed edits", async () => {
			const workingDirectory = URI.file("/repo/project");
			const { session, mockSession, waitForSignal } = await createAgentSession(
				disposables,
				{ workingDirectory },
			);
			const sessionInternals = session as unknown as ISessionInternalsForTest;
			...
			await waitForSignal((s) =>
				isAction(s, ActionType.SessionToolCallComplete),
			);
		...
		test("report_intent surfaces as session activity", async () => {
			...
			if (isAction(signal, ActionType.SessionActivityChanged)) {
				assert.strictEqual(
					(signal.action as { activity: string | undefined }).activity,
					"Reading repo docs",
				);
			}
			...
			const clearSignal = signals.find(
				(s, i) => i > 0 && isAction(s, ActionType.SessionActivityChanged),
			);
			assert.ok(clearSignal, "expected activity to be cleared on idle");
			if (
				clearSignal &&
				isAction(clearSignal, ActionType.SessionActivityChanged)
			) {
				assert.strictEqual(
					(clearSignal.action as { activity: string | undefined }).activity,
					undefined,
				);
			}
		});
		...
		test("message delta is forwarded", async () => {
			...
			const hasDelta = signals.some((s) => {
				...
				if (s.action.type === ActionType.SessionResponsePart) {
					...
					return (
						part.kind === ResponsePartKind.Markdown && part.content === "Hello "
					);
				}
			...
			const hasPart = signals.some((s) => {
				...
				if (s.action.type === ActionType.SessionResponsePart) {
					...
					return (
						...
						part.content === "Let me help you."
					);
				}
				if (s.action.type === ActionType.SessionDelta) {
					return (
						(s.action as SessionDeltaAction).content === "Let me help you."
					);
				}
				return false;
			});
			...
			assert.deepStrictEqual(actual, [
				{
					...
					parts: [
						...
						},
						{
							...
							success: false,
							content: [
								{ type: ToolResultContentType.Text, text: "tests failed" },
							],
						},
		...
		test("history replay does not duplicate assistant tool requests with lifecycle events", async () => {
			...
			const turns = await session.getMessages();
			const toolCalls = turns[0].responseParts.flatMap((part) =>
				part.kind === ResponsePartKind.ToolCall ? [part.toolCall] : [],
			);
		...
		test("subagent message delta does not suppress final parent assistant message", async () => {
			...
			mockSession.fire(
				...
				} as SessionEventPayload<"subagent.started">["data"],
				{ agentId: "agent-1" },
			);
			...
			const markdownParts = signals.flatMap((signal) => {
				if (
					signal.kind !== "action" ||
					signal.action.type !== ActionType.SessionResponsePart
				) {
					return [];
				}
		...
		test("reasoning delta after tool_start starts a new reasoning response part", async () => {
			...
			const reasoningResponseParts = signals.flatMap((s) => {
				...
				return s.action.part.kind === ResponsePartKind.Reasoning
					? [s.action.part]
					: [];
			});
			assert.strictEqual(
				reasoningResponseParts.length,
				2,
				"reasoning after a tool call should allocate a new response part, not append to the part from before the tool call",
			);
			assert.notStrictEqual(
				reasoningResponseParts[0].id,
				reasoningResponseParts[1].id,
				...
				[
					{
						...
						id: "sdk-evt-user-1",
						data: {
							interactionId: "capi-interaction-1",
					...
					},
					{
						...
						id: "sdk-evt-user-2",
						data: {
							interactionId: "capi-interaction-2",
	...
	suite("user input handling", () => {
		...
		test("handleUserInputRequest with choices generates SingleSelect question", async () => {
			...
			assert.strictEqual(
				...
				SessionInputQuestionKind.SingleSelect,
			);
		...
		test("form-mode request projects schema fields to questions and accept round-trips content", async () => {
			...
			session.respondToUserInputRequest(
				...
				SessionInputResponseKind.Accept,
				{
					...
					replicas: {
						state: SessionInputAnswerState.Submitted,
						value: { kind: SessionInputAnswerValueKind.Number, value: 5 },
					},
					confirm: {
						state: SessionInputAnswerState.Submitted,
						value: { kind: SessionInputAnswerValueKind.Boolean, value: true },
					},
					region: {
						state: SessionInputAnswerState.Submitted,
						value: {
		...
		test("free-form request (no schema) returns submitted text as content.answer", async () => {
			...
			session.respondToUserInputRequest(
				...
				SessionInputResponseKind.Accept,
				{
					answer: {
						state: SessionInputAnswerState.Submitted,
						value: { kind: SessionInputAnswerValueKind.Text, value: "teal" },
					},
				},
			);
	...
	suite("SDK callback logging", () => {
		test("logs and rethrows user input callback failures", async () => {
			...
			assert.strictEqual(
				entry.args[0],
				'[Copilot:test-session-1] Failed to handle user input request: question="Need input"',
			);
		});
		...
		test("logs and rethrows onPreToolUse failures", async () => {
			...
			assert.strictEqual(
				entry.args[0],
				"[Copilot:test-session-1] Failed in onPreToolUse: tool=edit",
			);
		});
		...
		test("logs and rethrows onPostToolUse failures", async () => {
			...
			assert.strictEqual(
				entry.args[0],
				"[Copilot:test-session-1] Failed in onPostToolUse: tool=edit",
			);
		});
	});
	...
	suite("client tool calls", () => {
		...
		test("client tool handler waits for completion without emitting tool_ready", async () => {
			...
			assert.strictEqual(
				signals.filter((s) => isAction(s, ActionType.SessionToolCallStart))
					.length,
				1,
			);
			const startSignal = signals.find((s) =>
				isAction(s, ActionType.SessionToolCallStart),
			);
			assert.ok(
				startSignal && isAction(startSignal, ActionType.SessionToolCallStart),
			);
			if (isAction(startSignal!, ActionType.SessionToolCallStart)) {
				assert.strictEqual(
					(startSignal.action as SessionToolCallStartAction).toolClientId,
					"test-client",
				);
			}
			...
			assert.strictEqual(
				signals.filter(
					(s) =>
						...
						isAction(s, ActionType.SessionToolCallReady),
				).length,
				0,
			);
		...
		test("client tool handler does not emit tool_ready (permission flow owns it)", async () => {
			const { session, mockSession, signals, waitForSignal } =
				await createAgentSession(disposables, { clientSnapshot: snapshot });
			...
			assert.strictEqual(
				signals.filter((s) => s.kind === "pending_confirmation").length,
				0,
			);
			...
			await waitForSignal((s) => s.kind === "pending_confirmation");
			const permSignals = signals.filter(
				(s): s is IAgentToolPendingConfirmationSignal =>
					s.kind === "pending_confirmation",
			);
			...
			const tools = session.createClientSdkTools();
			const handlerPromise = invokeClientToolHandler(
				tools[0],
				"tc-client-perm",
			);
			...
			assert.strictEqual(
				signals.filter((s) => s.kind === "pending_confirmation").length,
				1,
				"handler should not emit a second pending_confirmation",
			);
		...
		test("client tool handler logs and rethrows failures", async () => {
			...
			assert.strictEqual(
				entry.args[0],
				"[Copilot:test-session-1] Failed in client tool handler: tool=my_tool, toolCallId=tc-client-error",
			);
		});
	...
	suite("plan mode", () => {
		...
		test("handleExitPlanModeRequest produces a single-select input request with options and recommended", async () => {
			...
			mockSession.planReadResult = {
				exists: true,
				content: "## Plan",
				path: "/sessions/abc/plan.md",
			};

			const responsePromise =
				session.handleExitPlanModeRequest(planRequestParams());

			const signal = await waitForSignal((s) =>
				isAction(s, ActionType.SessionInputRequested),
			);
			...
			const deltaContent = signals
				.flatMap((s) => {
					...
					if (s.action.type === ActionType.SessionResponsePart) {
						const part = (s.action as SessionResponsePartAction).part;
						return part.kind === ResponsePartKind.Markdown
							? [part.content]
							: [];
					}
					if (s.action.type === ActionType.SessionDelta) {
				...
				.join("");
			assert.ok(
				deltaContent.includes("Plan summary"),
				`expected delta to include plan summary; got: ${deltaContent}`,
			);
			assert.ok(
				deltaContent.includes("plan.md"),
				"delta should include a link to the plan file",
			);
		...
		test("completing the input request with autopilot resolves with approved + autopilot + autoApproveEdits", async () => {
			...
			const responsePromise = session.handleExitPlanModeRequest(
				planRequestParams({
					actions: ["autopilot", "interactive"],
			...
			session.respondToUserInputRequest(
				...
				SessionInputResponseKind.Accept,
				{
					[questionId]: {
						...
						value: {
							kind: SessionInputAnswerValueKind.Text,
							value: "Add tests for edge cases",
						},
					},
				},
			);
		...
		test("session.mode_changed → plan updates the AHP session config", async () => {
			const { mockSession, sessionConfigUpdates } =
				await createAgentSession(disposables);

			mockSession.fire("session.mode_changed", {
				previousMode: "interactive",
				newMode: "plan",
			} as SessionEventPayload<"session.mode_changed">["data"]);
			...
			assert.deepStrictEqual(sessionConfigUpdates, [
				{
					...
					patch: { mode: "interactive", autoApprove: "autopilot" },
				},
			]);
		});
		...
		test("session.mode_changed for unsupported mode is ignored", async () => {
			...
			mockSession.fire("session.mode_changed", {
				previousMode: "interactive",
				newMode: "shell",
			} as unknown as SessionEventPayload<"session.mode_changed">["data"]);
		...
		test("handleExitPlanModeRequest auto-accepts when autoApprove=autopilot (recommended action)", async () => {
			...
			const response = await session.handleExitPlanModeRequest(
				planRequestParams({
					actions: ["autopilot", "interactive", "exit_only"],
					recommendedAction: "autopilot",
				}),
			...
			assert.strictEqual(
				signals.filter((s) => isAction(s, ActionType.SessionInputRequested))
					.length,
				0,
			);
		});
