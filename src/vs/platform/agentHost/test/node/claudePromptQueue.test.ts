...
import {
	ClaudePromptQueue,
	IPendingSdkMessage,
} from "../../node/claude/claudePromptQueue.js";
...
function makeEntry(
	id: string,
	opts?: { steeringPendingId?: string; turnId?: string },
): IPendingSdkMessage {
...
async function drainOne(
	iter: AsyncIterator<SDKUserMessage>,
...
suite("ClaudePromptQueue", () => {
	...
	test("push then iterable yields then settleHead resolves the deferred", async () => {
		...
		assert.strictEqual(
			e.deferred.isSettled,
			false,
			"deferred unsettled until settleHead",
		);
	...
	test("intermediate settleHead while another entry is queued does NOT settle the popped entry — batches until full drain (M10 invariant)", async () => {
		...
		queue.settleHead();
		assert.strictEqual(
			a.deferred.isSettled,
			false,
			"a stays open while b in flight",
		);
		...
		assert.strictEqual(
			a.deferred.isSettled,
			true,
			"a batch-completes on full drain",
		);
	...
	test("failAll rejects every pending, yielded, and popped entry and clears all lists", async () => {
		...
		await Promise.all([
			yieldedPromise.then(
				() => assert.fail("yielded should reject"),
				(e) => assert.strictEqual(e, fatal),
			),
			poppedPromise.then(
				() => assert.fail("popped should reject"),
				(e) => assert.strictEqual(e, fatal),
			),
			queuedPromise.then(
				() => assert.fail("queued should reject"),
				(e) => assert.strictEqual(e, fatal),
			),
		]);
		assert.strictEqual(queue.isEmpty, true);
