...
suite("AgentHostWorkspaceFiles", () => {
	...
	test("enumerates files in the working directory", async () => {
		...
		const files = disposables.add(
			new AgentHostWorkspaceFiles(new NullLogService()),
		);
		const result = await files.getFiles(URI.file(dir), CancellationToken.None);
		const names = result.map((uri) => uri.path).sort();

		assert.ok(
			names.some((p) => p.endsWith("/a.txt")),
			`expected a.txt in ${names.join(",")}`,
		);
		assert.ok(
			names.some((p) => p.endsWith("/sub/b.txt")),
			`expected sub/b.txt in ${names.join(",")}`,
		);
	});
	...
	test("respects .gitignore", async () => {
		...
		assert.ok(
			!names.some((p) => p.endsWith("/ignored.txt")),
			`ignored.txt should not be listed: ${names.join(",")}`,
		);
	});
	...
	test("excludes the .git directory", async () => {
		...
		assert.ok(
			!names.some((p) => p.includes("/.git/")),
			`.git contents should be excluded: ${names.join(",")}`,
		);
	});
	...
	test("returns [] for non-file URIs", async () => {
		...
		const result = await files.getFiles(
			URI.parse("vscode-vfs://github/foo/bar"),
	...
	test("caches concurrent calls for the same working directory", async () => {
		...
		assert.strictEqual(
			r1,
			r2,
			"concurrent calls should share the same promise / result array",
		);
	});
	...
	test("cancelling one caller does not poison concurrent callers sharing the cache", async () => {
		...
		assert.ok(
			result.some((uri) => uri.path.endsWith("/a.txt")),
			`survivor should resolve with files even when first caller cancelled: ${result.map((u) => u.path).join(",")}`,
		);
	});
});
