...
suite("CopilotApiService", () => {
	...
	suite("Endpoint Discovery", () => {
		...
		test("uses endpoints.api from the /copilot_internal/user response as the CAPI base", async () => {
			...
			assert.strictEqual(
				captured().url,
				"https://custom.copilot.example.com/v1/messages",
			);
		});
		...
		test("throws on 403 from endpoint discovery", async () => {
			...
			await assert.rejects(
				...
				(err: Error) =>
					err.message.includes("Copilot endpoint discovery failed: 403"),
			);
		});
		...
		test("throws on 500 from endpoint discovery", async () => {
			...
			await assert.rejects(
				...
				(err: Error) =>
					err.message.includes("Copilot endpoint discovery failed: 500"),
			);
		});
		...
		test("error from CAPI does not include the github token", async () => {
			...
			await assert.rejects(
				() => service.messages("super-secret-gh-token-xyz", baseRequest),
				(err: Error) =>
					!err.message.includes("super-secret-copilot-token-xyz") &&
					!err.message.includes("super-secret-gh-token-xyz"),
			);
		});
	...
	suite("Request Format", () => {
		...
		test("streaming sends stream=true in the body", async () => {
			...
			await collect(
				service.messages("gh-tok", { ...baseRequest, stream: true as const }),
			);
			const body = JSON.parse(captured().init?.body as string);
		...
		test("sends correct CAPI headers", async () => {
			...
			assert.ok(
				...
				"CAPIClient should inject API version",
			);
			assert.ok(
				headers["VScode-SessionId"],
				"CAPIClient should inject session id",
			);
		});
	...
	suite("Streaming Responses", () => {
		...
		test("yields text deltas from content_block_delta events", async () => {
			const service = streamService([
				...
				sseLines("event: message_stop", 'data: {"type":"message_stop"}'),
			]);
		...
		test("throws on error event without message", async () => {
			const service = streamService([
				sseLines("event: error", 'data: {"type":"error","error":{}}'),
			]);
	...
	suite("Raw Event Stream (messages())", () => {
		...
		test("yields message_delta with stop_reason payload", async () => {
			...
			const msgDelta = events.find(
				(e): e is Anthropic.RawMessageDeltaEvent => e.type === "message_delta",
			);
			assert.ok(msgDelta);
	...
	suite("countTokens", () => {
		test('throws "countTokens not supported by CAPI"', async () => {
			...
			await assert.rejects(
				() =>
					service.countTokens("gh-tok", {
						model: "claude-sonnet-4-5",
						messages: [{ role: "user", content: "hi" }],
					}),
				(err: Error) =>
					err.message.includes("countTokens not supported by CAPI"),
			);
		});
	...
	suite("CopilotApiError contract", () => {
		async function captureCopilotApiError(
			promise: Promise<unknown>,
		): Promise<CopilotApiError> {
			try {
				await promise;
			} catch (err) {
				assert.ok(
					err instanceof CopilotApiError,
					`expected CopilotApiError, got: ${err instanceof Error ? err.message : String(err)}`,
				);
				return err;
			}
		...
		test("non-2xx with conforming Anthropic envelope: passthrough verbatim", async () => {
			...
			const { fetch: fetchFn } = routingFetch(
				() =>
					new Response(JSON.stringify(upstreamEnvelope), {
						status: 429,
						statusText: "Too Many Requests",
					}),
			);
	...
	suite("Cancellation", () => {
		...
		test("cancels the underlying SSE stream after message_stop terminates the generator", async () => {
			...
			await collect(
				service.messages("gh-tok", { ...baseRequest, stream: true }),
			);
			assert.strictEqual(cancelled, true);
		});
		...
		test("cancels the underlying SSE stream when the generator throws", async () => {
			...
			const body = new ReadableStream<Uint8Array>({
				start(controller) {
					controller.enqueue(
						sseLines('data: {"type":"error","error":{"message":"boom"}}'),
					);
				},
				cancel() {
					cancelled = true;
	...
	suite("Models", () => {
			...
			const fakeModels = [
				{
					...
					vendor: "anthropic",
					supported_endpoints: ["chat/messages"],
				},
				{
					id: "claude-opus-4",
					name: "Claude Opus 4",
		...
		test("routes to the models endpoint URL", async () => {
			...
			await service.models("gh-tok");
			assert.ok(
				captured().url.includes("/models"),
				`expected models URL, got: ${captured().url}`,
			);
		});
