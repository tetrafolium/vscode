...
} from "../../node/copilot/copilotSlashCommandCompletionProvider.js";
...
suite("CopilotSlashCommandCompletionProvider", () => {
	...
	suite("parseLeadingSlashCommand", () => {
		test("matches lone /plan", () => {
			assert.deepStrictEqual(parseLeadingSlashCommand("/plan"), {
				command: "plan",
				rest: "",
			});
		});
		...
		test("matches lone /compact", () => {
			assert.deepStrictEqual(parseLeadingSlashCommand("/compact"), {
				command: "compact",
				rest: "",
			});
		});
		...
		test("matches lone /research", () => {
			assert.deepStrictEqual(parseLeadingSlashCommand("/research"), {
				command: "research",
				rest: "",
			});
		});
		...
		test("captures trailing text after a space for /research", () => {
			assert.deepStrictEqual(
				parseLeadingSlashCommand("/research How does React work?"),
				{ command: "research", rest: "How does React work?" },
			);
		});

		test("matches lone /rubber-duck", () => {
			assert.deepStrictEqual(parseLeadingSlashCommand("/rubber-duck"), {
				command: "rubber-duck",
				rest: "",
			});
		});
		...
		test("captures trailing text after a space for /rubber-duck", () => {
			assert.deepStrictEqual(
				parseLeadingSlashCommand("/rubber-duck review my approach"),
				{ command: "rubber-duck", rest: "review my approach" },
			);
		});

		test("rejects /rubber-duck-extra (no separator)", () => {
			assert.strictEqual(
				parseLeadingSlashCommand("/rubber-duck-extra"),
				undefined,
			);
		});
		...
		test("captures trailing text after a space", () => {
			assert.deepStrictEqual(
				parseLeadingSlashCommand("/plan build a hello world"),
				{ command: "plan", rest: "build a hello world" },
			);
		});

		test("captures trailing text after a space for /compact", () => {
			assert.deepStrictEqual(parseLeadingSlashCommand("/compact some text"), {
				command: "compact",
				rest: "some text",
			});
		});
		...
		test("rejects /compact-hello (no separator)", () => {
			assert.strictEqual(
				parseLeadingSlashCommand("/compact-hello world"),
				undefined,
			);
		});
	...
	suite("provideCompletionItems", () => {
		...
		test('returns all items for lone "/"', async () => {
			...
			assert.deepStrictEqual(
				items.map((i) => i.insertText),
				["/plan ", "/compact", "/research ", "/rubber-duck "],
			);
		});
