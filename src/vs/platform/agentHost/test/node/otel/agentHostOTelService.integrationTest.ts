...
} from "../../../node/otel/agentHostOTelService.js";
						...
						spans: [
							{
								...
								attributes: [
									...
									},
									{
										...
										value: { stringValue: "github.copilot" },
									},
									{
										key: "gen_ai.agent.name",
										value: { stringValue: "copilotcli" },
									},
									{
...
suite("platform/agentHost - AgentHostOTelService (integration)", () => {
	...
	test("DB mode: starts loopback, persists posted spans to SQLite, and exposes db path", async () => {
		...
		try {
			...
			strictEqual(cfg!.exporterType, "otlp-http");
			ok(
				cfg!.otlpEndpoint?.startsWith("http://127.0.0.1:"),
				`expected loopback endpoint, got ${cfg!.otlpEndpoint}`,
			);
			...
			ok(
				dbPath!.fsPath
					.replace(/\\/g, "/")
					.endsWith(AgentHostOTelSpansDbSubPath),
			);
			...
			const res1 = await postOtlp(
				cfg!.otlpEndpoint!,
				makeOtlpRequest(traceId, spanIdA),
			);
			strictEqual(res1.statusCode, 200, `unexpected res1: ${res1.body}`);
			const res2 = await postOtlp(
				cfg!.otlpEndpoint!,
				makeOtlpRequest(traceId, spanIdB),
			);
			...
			try {
				...
				strictEqual(
					persisted.length,
					2,
					`expected 2 persisted spans, got ${persisted.length} (res1.body=${res1.body})`,
				);
	...
	test("DB mode + external endpoint: outbound forwarder is configured (best-effort)", async () => {
		...
		try {
			...
			const res = await postOtlp(
				cfg!.otlpEndpoint!,
				makeOtlpRequest(traceId, "00000000000000ff"),
			);
