...
function mockProject(provider: AgentProvider) {
	return {
		uri: URI.from({ scheme: "mock-project", path: `/${provider}` }),
...
export class MockAgent implements IAgent {
	...
	readonly setPendingMessagesCalls: {
		session: URI;
	...
	async createSession(
		...
		const session =
			config?.session ??
			AgentSession.uri(this.id, `${this.id}-session-${this._nextId++}`);
		const rawId = AgentSession.id(session);
		this._sessions.set(rawId, session);
		return {
			session,
			project: mockProject(this.id),
			workingDirectory: this.resolvedWorkingDirectory,
		};
	}
	...
	setPendingMessages(
		...
		this.setPendingMessagesCalls.push({
			session,
			steeringMessage,
			queuedMessages,
		});
	...
	async getSessionMessages(session: URI): Promise<readonly Turn[]> {
		...
		if (subagentInfo) {
			return buildSubagentTurnsFromHistory(
 ...
 */
export const PRE_EXISTING_SESSION_URI = AgentSession.uri(
	"mock",
...
export class ScriptedMockAgent implements IAgent {
	...
	private readonly _preExistingMessages: IHistoryRecord[] = [
		{
			type: "message",
			role: "user",
			session: PRE_EXISTING_SESSION_URI,
			messageId: "h-msg-1",
		...
		},
		{
			...
			toolCallId: "h-tc-1",
			result: {
				pastTenseMessage: "Listed files",
				content: [
					{ type: ToolResultContentType.Text, text: "file1.ts\nfile2.ts" },
				],
				success: true,
		...
		},
		{
			...
			session: PRE_EXISTING_SESSION_URI,
			messageId: "h-msg-2",
			content: "Here are the files: file1.ts and file2.ts",
		},
	];
	...
	constructor() {
		...
		this._sessions.set(
			AgentSession.id(PRE_EXISTING_SESSION_URI),
	...
	async listSessions(): Promise<IAgentSessionMetadata[]> {
		return [...this._sessions.values()].map((s) => ({
			...
			project: mockProject(this.id),
			summary:
				s.toString() === PRE_EXISTING_SESSION_URI.toString()
					? "Pre-existing session"
					: undefined,
		}));
	}
	...
	async getSessionMetadata(
		...
		return {
			...
			project: mockProject(this.id),
			summary:
				session.toString() === PRE_EXISTING_SESSION_URI.toString()
					? "Pre-existing session"
					: undefined,
		};
	}
	...
	async createSession(
		...
		const session =
			config?.session ??
			AgentSession.uri("mock", `mock-session-${this._nextId++}`);
		const rawId = AgentSession.id(session);
		this._sessions.set(rawId, session);
	...
	async resolveSessionConfig(
		...
		const isolation =
			params.config?.isolation === "folder" ||
			params.config?.isolation === "worktree"
				? params.config.isolation
				: "worktree";
		const branch =
			isolation === "worktree" && typeof params.config?.branch === "string"
	...
	async sessionConfigCompletions(
		...
		const branches = ["main", "feature/config", "release"].filter((branch) =>
		...
		switch (prompt) {
			...
			case "use-tool":
				this._fireSequence([
					...
					_toolComplete(session, sessionStr, tid, "tc-1", {
						pastTenseMessage: "Ran echo tool",
						content: [{ type: ToolResultContentType.Text, text: "echoed" }],
						success: true,
					}),
			...
			case "permission": {
				...
				(async () => {
					...
					await timeout(5);
					this._onDidSessionProgress.fire(
						_pendingConfirmation(session, "tc-perm-1", "Run a test command", {
							toolInput: "echo test",
			...
			case "write-file": {
				...
				(async () => {
					...
					await timeout(5);
					this._onDidSessionProgress.fire(
						_pendingConfirmation(session, "tc-write-1", "Write src/app.ts", {
					...
					await timeout(10);
					this._fireSequence([
						_toolComplete(session, sessionStr, tid, "tc-write-1", {
			...
			case "write-env": {
				...
				(async () => {
					...
					await timeout(5);
					this._onDidSessionProgress.fire(
						_pendingConfirmation(session, "tc-write-env-1", "Write .env", {
							permissionKind: "write",
				...
				this._pendingPermissions.set("tc-write-env-1", (approved) => {
					if (approved) {
						this._fireSequence([
							_toolComplete(session, sessionStr, tid, "tc-write-env-1", {
								pastTenseMessage: "Wrote .env",
								content: [{ type: ToolResultContentType.Text, text: "ok" }],
								success: true,
							}),
							_idle(session, sessionStr, tid),
			...
			case "run-safe-command": {
				...
				(async () => {
					...
					await timeout(5);
					this._onDidSessionProgress.fire(
						_pendingConfirmation(session, "tc-shell-1", "ls -la", {
					...
					await timeout(10);
					this._fireSequence([
						_toolComplete(session, sessionStr, tid, "tc-shell-1", {
			...
			case "run-dangerous-command": {
				...
				(async () => {
					...
					await timeout(5);
					this._onDidSessionProgress.fire(
						_pendingConfirmation(session, "tc-shell-deny-1", "rm -rf /", {
							permissionKind: "shell",
							toolInput: "rm -rf /",
				...
				this._pendingPermissions.set("tc-shell-deny-1", (approved) => {
					if (approved) {
						this._fireSequence([
							_toolComplete(session, sessionStr, tid, "tc-shell-deny-1", {
								pastTenseMessage: "Ran command",
			...
			case "with-reasoning": {
				const initialReasoning = _reasoning(
					session,
					sessionStr,
					tid,
					"Let me think",
				...
				const partId =
					initialReasoning.action.type === ActionType.SessionResponsePart &&
			...
			case "client-tool": {
				...
				(async () => {
					...
					await timeout(5);
					this._onDidSessionProgress.fire(
						_pendingConfirmation(session, "tc-client-1", "Running tests...", {
							toolInput: "{}",
						}),
					);
				})();
			...
			case "client-tool-with-permission": {
				...
				this._pendingPermissions.set("tc-client-perm-1", (approved) => {
					if (approved) {
						this._fireSequence([
							_toolComplete(session, sessionStr, tid, "tc-client-perm-1", {
								pastTenseMessage: "Ran tests",
								content: [
									{ type: ToolResultContentType.Text, text: "all passed" },
								],
								success: true,
							}),
							_markdown(
			...
			case "subagent": {
				...
				this._fireSequence([
					...
					_toolComplete(
						...
						"tc-inner-1",
						{
							pastTenseMessage: "Ran inner tool",
							content: [{ type: ToolResultContentType.Text, text: "inner-ok" }],
							success: true,
						},
						"tc-task-1",
					),
					{ kind: "subagent_completed", session, toolCallId: "tc-task-1" },
					_toolComplete(session, sessionStr, tid, "tc-task-1", {
						pastTenseMessage: "Subagent done",
						content: [{ type: ToolResultContentType.Text, text: "task-ok" }],
						success: true,
					}),
			...
			default:
				if (prompt.startsWith("terminal-edit:")) {
					...
					void (async () => {
						...
						this._fireSequence([
							_toolComplete(session, sessionStr, tid, "tc-term-edit-1", {
	...
	onClientToolCallComplete(
		...
		this._onDidSessionProgress.fire(
			_toolComplete(session, sessionStr, turnId, toolCallId, result),
		);
...
function _action(
	session: URI,
	action: import("../../common/state/sessionActions.js").SessionAction,
...
function _toolComplete(
	...
	return _action(
		session,
		{ type: ActionType.SessionToolCallComplete, turnId, toolCallId, result },
		parentToolCallId,
	);
}

/** Creates a {@link IAgentToolPendingConfirmationSignal}. */
function _pendingConfirmation(
	session: URI,
	toolCallId: string,
	invocationMessage: StringOrMarkdown,
	opts?: {
