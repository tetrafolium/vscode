...
class MockAgentService implements IAgentService {
	...
	private readonly _onDidAction = new Emitter<
		import("../../common/state/sessionActions.js").ActionEnvelope
	>();
	readonly onDidAction = this._onDidAction.event;
	private readonly _onDidNotification = new Emitter<
		import("../../common/state/sessionActions.js").INotification
	>();
	...
	async authenticate(_params: AuthenticateParams): Promise<AuthenticateResult> {
	...
	async resourceCopy(_params: ResourceCopyParams): Promise<ResourceCopyResult> {
		return {};
	}
	...
	readonly liveWatchDescriptors = new Map<
		string,
		import("../../common/state/sessionProtocol.js").ResourceWatchState
	>();
	async createResourceWatch(
		_params: import("../../common/state/sessionProtocol.js").CreateResourceWatchParams,
	): Promise<
		import("../../common/state/sessionProtocol.js").CreateResourceWatchResult
	> {
		throw new Error("Not implemented");
	}
	onResourceWatchSubscribed(
		channel: string,
	):
		| import("../../common/state/sessionProtocol.js").ResourceWatchState
		| undefined {
...
suite("ProtocolServerHandler", () => {
	...
	setup(() => {
		...
		disposables.add(
			(handler = new ProtocolServerHandler(
				agentService,
				stateManager,
				server,
				{ defaultDirectory: URI.file("/home/testuser").toString() },
				disposables.add(
					(fileSystemProvider = new AgentHostFileSystemProvider()),
				),
				new NullLogService(),
			)),
		);
	...
	test("handshake rejects unsupported protocol versions", () => {
		...
		const resp = findResponse(transport.sent, 1) as
	...
	test("upgrade method advertised when management socket env var is set", () => {
		...
		process.env.VSCODE_AGENT_HOST_MANAGEMENT_SOCKET =
			"/tmp/mock-supervisor.sock";
		try {
			const transport = new MockProtocolTransport();
			server.simulateConnection(transport);
			transport.simulateMessage(
	...
	test("_vscodeUpgrade RPC returns MethodNotFound when no supervisor is available", async () => {
		...
		const resp = (await responsePromise) as {
	...
	test("handshake with initialSubscriptions returns snapshots", () => {
		...
		assert.strictEqual(
			result.snapshots[0].resource.toString(),
			sessionUri.toString(),
		);
	});
	...
	test("subscribe request returns snapshot", async () => {
		...
		const result = (resp as unknown as { result: { snapshot: IStateSnapshot } })
			.result;
		assert.strictEqual(
			result.snapshot.resource.toString(),
			sessionUri.toString(),
		);
	});
	...
	test("client action is dispatched and echoed", () => {
		...
		const envelope = turnStarted!.params as unknown as {
	...
	test("changeset actions are scoped to subscribed changeset URIs", () => {
		...
		stateManager.dispatchServerAction(changesetUri, {
			...
			file: {
				...
				edit: {
					after: {
						uri: "file:///test/changed.ts",
						content: { uri: "file:///test/changed.ts" },
					},
		...
		assert.strictEqual(
			aActions.length,
			1,
			"changeset subscriber should receive 1 envelope",
		);
		assert.strictEqual(
			bActions.length,
			0,
			"session-only subscriber should receive 0 changeset envelopes",
		);

		const params = aActions[0].params as {
			channel: string;
			action: { type: string };
		};
	...
	test("notifications are broadcast to all clients", () => {
		...
		assert.strictEqual(
			findNotifications(transportA.sent, "root/sessionAdded").length,
			1,
		);
		assert.strictEqual(
			findNotifications(transportB.sent, "root/sessionAdded").length,
			1,
		);
	});
	...
	test("listSessions includes project metadata", async () => {
		...
		assert.deepStrictEqual(
			result.items.map((item) => item.project),
			[
				{
					uri: URI.file("/workspace/project").toString(),
					displayName: "Project",
				},
			],
		);
	...
	test("createSession returns null and broadcasts project in sessionAdded summary", async () => {
		...
		transport.simulateMessage(
			request(2, "createSession", { channel: newSession }),
		);
		const resp = await responsePromise;
	...
	test("reconnect replays missed changeset actions to changeset subscribers", async () => {
		...
		if (result.type === "replay") {
			...
			assert.ok(
				replayedTypes.includes(ActionType.ChangesetFileSet),
				"replay should include ChangesetFileSet",
			);
			assert.ok(
				replayedTypes.includes(ActionType.ChangesetStatusChanged),
				"replay should include ChangesetStatusChanged",
			);
		}
	});
	...
	test("reconnect rehydrates server-side state that was evicted while disconnected", async () => {
		...
		assert.strictEqual(
			stateManager.getSnapshot(sessionUri),
			undefined,
			"precondition: state evicted",
		);
		...
		assert.deepStrictEqual(
			subscribeCalls,
			[sessionUri],
			"reconnect should call subscribe to restore evicted state",
		);
		assert.ok(
			stateManager.getSnapshot(sessionUri),
			"state should have been re-hydrated by reconnect",
		);
	});
	...
	test("reconnect re-registers the reverse-RPC filesystem authority", async () => {
		...
		disposables.add(
			transport2.onDidSend((msg) => {
				if (isJsonRpcRequest(msg) && msg.method === "resourceList") {
					transport2.simulateMessage({
						jsonrpc: "2.0",
						id: msg.id,
						result: {
							entries: [{ name: "after-reconnect.txt", type: "file" as const }],
						},
					});
				}
			}),
		...
		const result = await fileSystemProvider.readdir(
			agentHostUri("client-fs", "/workspace"),
		);
	...
	test("client disconnect clears active client and fails owned tool calls after grace period", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				stateManager.getSessionState(sessionUri)?.activeClient,
				undefined,
			);
			let part =
				stateManager.getSessionState(sessionUri)?.activeTurn?.responseParts[0];
	...
	test("client reconnect without session subscription does not clear tool call disconnect timeout", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			reconnectTransport.simulateMessage(
				request(1, "reconnect", {
					clientId: "client-tools",
					...
					content: [
						{
							...
							text: "The client that was running Run Task disconnected, but another active client now provides Run Task. You may try calling the tool again.",
						},
					],
				},
			);
	...
	test("handshake includes defaultDirectory from side effects", () => {
		...
		const result = (resp as { result: InitializeResult }).result;
		assert.strictEqual(
			URI.parse(result.defaultDirectory!).path,
			"/home/testuser",
		);
	});
	...
	test("resourceList routes to side effect handler", async () => {
		...
		const result = (
			resp as unknown as {
				result: { entries: { name: string; uri: unknown; type: string }[] };
			}
		).result;
	...
	test("resourceList returns a JSON-RPC error when the target is invalid", async () => {
		...
		agentService.browseErrors.set(
			URI.file("/missing").toString(),
			new ProtocolError(
				JSON_RPC_INTERNAL_ERROR,
				`Directory not found: ${dirUri}`,
			),
		);
		const responsePromise = waitForResponse(transport, 2);
		transport.simulateMessage(request(2, "resourceList", { uri: dirUri }));
	...
	test("extension request preserves ProtocolError code and data", async () => {
		...
		const responsePromise = waitForResponse(transport, 2);
		transport.simulateMessage(
			request(2, "authenticate", { resource: "test", token: "bad" }),
		);
		const resp = (await responsePromise) as {
			error?: { code: number; message: string; data?: unknown };
		};
	...
	suite("createSession activeClient", () => {
		...
		test("rejects createSession when activeClient.clientId mismatches", async () => {
			...
			assert.strictEqual(
				agentService.createSessionConfigs.length,
				0,
				"agent service should not have been called",
			);
		});
	});
	...
	suite("OTLP logs channel", () => {
		...
		setup(() => {
			...
			otlpStateManager = localDisposables.add(
				new AgentHostStateManager(new NullLogService()),
			);
		...
		test("handshake advertises the logs channel template", () => {
			...
			const resp = findResponse(transport.sent, 1) as {
				result: InitializeResult & { telemetry?: { logs?: string } };
			};
			assert.deepStrictEqual(resp.result.telemetry, {
		...
		test("subscribe to logs channel returns an empty stateless result and starts forwarding records at-or-above the requested level", async () => {
			...
			transport.simulateMessage(
				request(2, "subscribe", { channel: "ahp-otlp://logs/warn" }),
			);
			...
			const bodies = logs.flatMap(({ payload }) =>
				[...iterateOtlpLogRecords(payload)].map((r) => r.body),
			);
		...
		test("unsubscribe stops forwarding without affecting other subscribers", async () => {
			...
			a.simulateMessage(
				request(2, "subscribe", { channel: "ahp-otlp://logs/trace" }),
			);
			b.simulateMessage(
				request(2, "subscribe", { channel: "ahp-otlp://logs/trace" }),
			);
			...
			a.simulateMessage(
				notification("unsubscribe", { channel: "ahp-otlp://logs/trace" }),
			);
			otlpEmitter.emit({
				timeUnixNano: "2",
				severityNumber: 9,
			...
			const aBodies = findOtlpLogs(a.sent).flatMap(({ payload }) =>
				[...iterateOtlpLogRecords(payload)].map((r) => r.body),
			);
			const bBodies = findOtlpLogs(b.sent).flatMap(({ payload }) =>
				[...iterateOtlpLogRecords(payload)].map((r) => r.body),
			);
			assert.deepStrictEqual(
				{ a: aBodies, b: bBodies },
		...
		test("multiple subscriptions to different levels each receive their own band", async () => {
			...
			transport.simulateMessage(
				request(2, "subscribe", { channel: "ahp-otlp://logs/info" }),
			);
			transport.simulateMessage(
				request(3, "subscribe", { channel: "ahp-otlp://logs/error" }),
			);
			await subscribed2;
			await subscribed3;
		...
		test("unrecognised ahp-otlp URIs do not crash subscribe", async () => {
			...
			transport.simulateMessage(
				request(2, "subscribe", { channel: "ahp-otlp://logs/verbose" }),
			);
			const resp = await waitForResponse(transport, 2);
			assert.deepStrictEqual(
				(resp as { result: unknown }).result,
				{},
				"unknown level should be acknowledged as stateless",
			);
			...
			assert.deepStrictEqual(
				findOtlpLogs(transport.sent),
				[],
				"no records should leak to an invalid level",
			);
		});
		...
		test("URI variants that parse to the same level collapse to one canonical subscription", async () => {
			...
			transport.simulateMessage(
				request(3, "subscribe", { channel: "ahp-otlp://logs/info?dup=1" }),
			);
			transport.simulateMessage(
				request(4, "subscribe", { channel: "ahp-otlp://logs/info#frag" }),
			);
			await r2;
			await r3;
			await r4;
			...
			assert.strictEqual(
				logs.length,
				1,
				"one record should produce exactly one notification",
			);
			assert.strictEqual(
				logs[0].channel,
				"ahp-otlp://logs/info",
				"channel should be canonicalised",
			);

			// Unsubscribe should remove the canonical entry regardless of
			// which URI variant the client uses to unsubscribe.
			transport.simulateMessage(
				notification("unsubscribe", { channel: "ahp-otlp://logs/info?dup=1" }),
			);
			otlpEmitter.emit({
				timeUnixNano: "2",
				severityNumber: 9,
			...
			assert.strictEqual(
				findOtlpLogs(transport.sent).length,
				1,
	...
	suite("resource watches", () => {
		test("subscribe to a resource-watch channel returns the descriptor + bumps refcount; envelopes are routed", async () => {
			...
			transport.simulateMessage(
				request(101, "subscribe", { channel: watchChannel }),
			);
			...
			stateManager.dispatchServerAction(watchChannel, {
				...
				changes: {
					items: [{ uri: "file:///workspace/a.txt", type: "updated" as never }],
				},
			...
			assert.strictEqual(
				actionMsgs.length,
				1,
				"subscriber should receive the change envelope",
			);
			const env = actionMsgs[0].params as unknown as {
				channel: string;
				action: { type: string };
			};
			...
			transport.simulateMessage(
				notification("unsubscribe", { channel: watchChannel }),
			);
			assert.deepStrictEqual(agentService.watchUnsubscribeCalls, [
				watchChannel,
			]);
		});
		...
		test("client disconnect releases the watch refcount", async () => {
			...
			transport.simulateMessage(
				request(200, "subscribe", { channel: watchChannel }),
			);
			await subPromise;
			assert.deepStrictEqual(agentService.watchSubscribeCalls, [watchChannel]);
