...
suite("AgentHostSkillCompletionProvider", () => {
	...
	setup(() => {
		...
		disposables.add(
			fileService.registerProvider(Schemas.inMemory, memoryProvider),
		);
	});
	...
	async function run(
		provider: AgentHostSkillCompletionProvider,
		text: string,
		offset = text.length,
	) {
	...
	test("announces slash as a trigger character", () => {
		...
		assert.deepStrictEqual(
			...
			[CompletionTriggerCharacter.Slash],
		);
	});
	...
	test("returns global customization skills with frontmatter metadata and trailing space", async () => {
		...
		const skillUri = await writeSkill(
			root,
			"agent-host-docs",
			[
				"---",
	...
	test("preserves session-effective order and appends global-only customizations", async () => {
		...
		assert.deepStrictEqual(
			result.map((item) => item.insertText),
			["/session-skill ", "/global-skill "],
		);
	});
	...
	test("clears cached skill metadata when customizations change", async () => {
		...
		assert.deepStrictEqual(
			(await run(provider, "/"))[0].attachment._meta?.displayName,
			"First Name",
		);
