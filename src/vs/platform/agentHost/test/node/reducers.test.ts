...
import {
	changesetReducer,
	sessionReducer,
} from "../../common/state/protocol/reducers.js";
...
suite(
	"sessionReducer – summaryStatus with tool call confirmations and input requests",
	() => {
...
suite("changesetReducer", () => {
	...
	test("ChangesetFileSet appends a new file", () => {
		const next = changesetReducer(ready, {
	...
	test("ChangesetFileSet replaces an existing file by id (upsert)", () => {
		const seeded = changesetReducer(ready, {
			type: ActionType.ChangesetFileSet,
			file: fileA,
		});
		const next = changesetReducer(seeded, {
			type: ActionType.ChangesetFileSet,
			file: fileARenamed,
		});
	...
	test("ChangesetStatusChanged → Error attaches the error", () => {
		...
		assert.deepStrictEqual(
			{ status: next.status, error: next.error },
			{ status: ChangesetStatus.Error, error: err },
		);
	});

	test("ChangesetStatusChanged → Ready strips a previous error", () => {
		const errored: ChangesetState = {
			status: ChangesetStatus.Error,
			error: { errorType: "x", message: "y" },
			files: [fileA],
		};
		const next = changesetReducer(errored, {
			type: ActionType.ChangesetStatusChanged,
			status: ChangesetStatus.Ready,
		});
		assert.deepStrictEqual(
			{ status: next.status, error: next.error, files: next.files },
			{ status: ChangesetStatus.Ready, error: undefined, files: [fileA] },
		);
	});
	...
	test("ChangesetOperationsChanged with undefined strips operations", () => {
		const seeded = changesetReducer(ready, {
			type: ActionType.ChangesetOperationsChanged,
			operations: [{ id: "stage", label: "Stage", scopes: [] }],
		});
		const next = changesetReducer(seeded, {
			type: ActionType.ChangesetOperationsChanged,
...
suite("sessionReducer – SessionCustomizationUpdated", () => {
	...
	const agentA: AgentCustomization = {
		type: CustomizationType.Agent,
		id: "file:///plugin-a/agents/helper.md",
		uri: "file:///plugin-a/agents/helper.md",
		name: "helper",
	};
	const agentB: AgentCustomization = {
		type: CustomizationType.Agent,
		id: "file:///plugin-a/agents/reviewer.md",
		uri: "file:///plugin-a/agents/reviewer.md",
		name: "reviewer",
