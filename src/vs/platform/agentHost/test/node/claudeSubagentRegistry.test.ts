...
function makeAgentToolCallTurn(
	toolCallId: string,
	opts: { suffixText?: string; toolName?: string; status?: ToolCallStatus },
): Turn {
	return {
		id: "turn-" + toolCallId,
...
suite("SubagentRegistry", () => {
	...
	test("recordSpawn is idempotent; init fields are first-writer-wins; getSpawn returns the same record across calls", () => {
		const registry = r();
		const first = registry.recordSpawn("toolu_a", {
			agentId: "agent-1",
			subagentType: "Explore",
			description: "first desc",
		});
		const second = registry.recordSpawn("toolu_a", {
			agentId: "agent-2",
			subagentType: "OverwriteAttempt",
	...
	test("primeFromTranscript scans Task tool_result text blocks for agentId suffix and records each pair (idempotent against repeat calls)", () => {
		...
		const transcript: readonly Turn[] = [
			makeAgentToolCallTurn("toolu_a", {
				suffixText: "agentId: agentaaa\n(use SendMessage with to: 'agentaaa')",
			}),
