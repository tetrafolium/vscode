...
import {
	...
	extractAtToken,
} from "../../node/agentHostFileCompletionProvider.js";
...
function assertResourceUri(
	...
	assert.strictEqual(
		attachment.type === MessageAttachmentKind.Resource && attachment.uri,
		expected,
	);
}
...
suite("AgentHostFileCompletionProvider", () => {
	...
	test('announces "@" as a trigger character via IAgentHostCompletions', () => {
		const completions = disposables.add(
			new AgentHostCompletions(new NullLogService()),
		);
		...
		disposables.add(
			completions.registerProvider(
				new AgentHostFileCompletionProvider(stateManager, workspaceFiles),
			),
		);
	...
	suite("extractAtToken", () => {
		...
		test("extracts a lone @ at end of string", () => {
			assert.deepStrictEqual(extractAtToken("look at @", 9), {
				token: "",
				rangeStart: 8,
				rangeEnd: 9,
			});
		...
		test("extracts an @-token after a space", () => {
			assert.deepStrictEqual(extractAtToken("look at @foo", 12), {
				token: "foo",
				rangeStart: 8,
				rangeEnd: 12,
			});
		...
		test("extracts an @-token at start of string", () => {
			assert.deepStrictEqual(extractAtToken("@foo", 4), {
				token: "foo",
				rangeStart: 0,
				rangeEnd: 4,
			});
		...
		test("honours offset (token = chars between @ and cursor)", () => {
			...
			assert.deepStrictEqual(extractAtToken("look at @foo", 11), {
				token: "fo",
				rangeStart: 8,
				rangeEnd: 11,
			});
	...
	suite("provideCompletionItems", () => {
		...
		function setup(opts: { workingDirectory?: URI; files?: readonly URI[] }) {
			...
			stateManager.createSession(
				makeSummary(sessionUri, opts.workingDirectory?.toString()),
			);
			const workspaceFiles = disposables.add(
				new FakeWorkspaceFiles(opts.files ?? []),
			);
			const provider = new AgentHostFileCompletionProvider(
				stateManager,
				workspaceFiles,
			);
		...
		test("returns [] for non-file working directory", async () => {
			const { sessionUri, provider } = setup({
				workingDirectory: URI.parse("vscode-vfs://github/foo/bar"),
			});
			const result = await provider.provideCompletionItems(
				{
					kind: CompletionItemKind.UserMessage,
