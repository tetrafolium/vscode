...
suite("AgentHostStateManager", () => {
	...
	test("getSnapshot returns session snapshot after creation", () => {
		...
		assert.strictEqual(
			(snapshot.state as SessionState).lifecycle,
	...
	test("root action that does not change state is not emitted", () => {
		...
		assert.strictEqual(
			manager.serverSeq,
			1,
			"serverSeq must not advance on a no-op",
		);
	...
	test("removeSession clears state without notification", () => {
		...
		disposables.add(
			manager.onDidEmitNotification((n) => notifications.push(n)),
		);
	...
	test("turnStarted dispatches root/activeSessionsChanged with correct count", () => {
		...
		const activeChanged = envelopes.filter(
			(e) => e.action.type === ActionType.RootActiveSessionsChanged,
		);
		assert.strictEqual(activeChanged.length, 1);
		assert.strictEqual(
			(activeChanged[0].action as { activeSessions: number }).activeSessions,
			1,
		);
	...
	test("active turn event follows reducer-derived active state transitions", () => {
		...
		disposables.add(
			manager.onDidChangeSessionActiveTurn((e) => events.push(e)),
		);
	...
	test("restoreSession creates session in Ready state with pre-populated turns", () => {
				...
				responseParts: [
					{
						...
						id: "p1",
						content: "world",
					} satisfies MarkdownResponsePart,
				],
				usage: undefined,
		...
		assert.strictEqual(
			(state.turns[0].responseParts[0] as MarkdownResponsePart).content,
			"world",
		);
	});
	...
	test("restoreSession does not emit sessionAdded notification", () => {
		...
		assert.strictEqual(
			...
			0,
			"should not emit notification for restored sessions",
		);
	});
		...
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				notifications.filter(
					(n) => n.type === NotificationType.SessionSummaryChanged,
				).length,
				0,
			);
			...
			assert.strictEqual(
				notification.changes.status,
				undefined,
				"unchanged fields should be omitted",
			);
		});
	});
	...
	test("coalesces multiple summary changes into one notification", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				changed.length,
				1,
				"should coalesce into one notification",
			);
			assert.strictEqual(
				(changed[0] as SessionSummaryChangedParams).changes.title,
				"Second",
			);
		});
	});
	...
	test("does not emit sessionSummaryChanged for deleted session", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				changed.length,
				0,
				"should not emit for deleted sessions",
			);
		});
	});
	...
	test("removeSession flushes pending status=Idle notification before eviction", () => {
		...
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const changed = notifications.filter(
				(n) => n.type === NotificationType.SessionSummaryChanged,
			) as SessionSummaryChangedParams[];
			assert.strictEqual(
				changed.length,
				1,
				"should emit SessionSummaryChanged synchronously in removeSession",
			);
			assert.strictEqual(
				changed[0].changes.status,
				SessionStatus.Idle,
				"status should be Idle so the spinner clears",
			);
		});
	});
	test("disposeChangeset emits ChangesetCleared and removes the state", () => {
		manager.createSession(makeSessionSummary());
		const changeset = manager.registerChangeset(
		...
		const cleared = envelopes.filter(
			(e) => e.action.type === ActionType.ChangesetCleared,
		);
		assert.strictEqual(
			cleared.length,
			1,
			"expected exactly one cleared envelope",
		);
		assert.strictEqual(cleared[0].channel, changeset);
		assert.strictEqual(
			manager.getChangesetState(changeset),
			undefined,
	...
	test("removeSession does NOT dispose per-session changesets (LRU eviction must not clear list-view chip)", () => {
		...
		assert.strictEqual(
			cleared.length,
			0,
			"removeSession must not emit ChangesetCleared",
		);
		assert.strictEqual(
			manager.getChangesetState(changeset)?.files.length,
			1,
			"changeset state should survive eviction",
		);
	});
	...
	test("deleteSession disposes per-session changesets before emitting SessionRemoved", () => {
		...
		const removed = notifications.filter(
			(n) => n.type === NotificationType.SessionRemoved,
		);
		assert.strictEqual(
			cleared.length,
			1,
			"deleteSession should emit ChangesetCleared",
		);
		assert.strictEqual(
			removed.length,
			1,
			"deleteSession should emit SessionRemoved",
		);
		assert.strictEqual(
			manager.getChangesetState(changeset),
			undefined,
			"changeset state should be gone after delete",
		);
	});
	...
	test("unknown changeset action is ignored without emitting an envelope", () => {
		...
		manager.dispatchServerAction(changesetUri, {
			...
			file: {
				id: "file:///x.ts",
				edit: {
					after: { uri: "file:///x.ts", content: { uri: "file:///x.ts" } },
					diff: { added: 1, removed: 0 },
				},
			},
		});
		...
		const registered = manager.registerChangeset(
			buildChangesetUri(sessionUri, "missing"),
		);
		...
		assert.strictEqual(
			envelopes.length,
			1,
			"registered changeset action should emit an envelope",
		);
		assert.strictEqual(
			manager.serverSeq - seqBefore,
			1,
			"serverSeq should advance for registered changeset action",
		);
	});
});
...
suite("Subagent URI helpers", () => {
	...
	test("parseSubagentSessionUri handles nested subagent URIs", () => {
		const parsed = parseSubagentSessionUri(
			"copilot:/session-1/subagent/tc-1/subagent/tc-2",
		);
		assert.deepStrictEqual(
			parsed && {
				parentSession: parsed.parentSession.toString(),
	...
	test("parseSubagentSessionUri returns undefined for non-subagent URIs", () => {
		assert.strictEqual(
			parseSubagentSessionUri("copilot:/session-1"),
			undefined,
		);
	});
	...
	test("isSubagentSession identifies subagent URIs", () => {
		assert.strictEqual(
			isSubagentSession("copilot:/session-1/subagent/tc-1"),
			true,
		);
