...
import {
	...
	SubagentRegistry,
} from "../../node/claude/claudeSubagentRegistry.js";
import {
...
class FakeSdkService implements IClaudeAgentSdkService {
	...
	async getSessionInfo(_id: string): Promise<SDKSessionInfo | undefined> {
		return undefined;
	}
	async startup(_p: {
		options: Options;
...
function makeAgentToolCallTurn(
	toolCallId: string,
	opts: {
		prompt?: string;
		suffixText?: string;
				...
				toolCall: {
					...
					toolInput:
						opts.prompt !== undefined
							? JSON.stringify({ prompt: opts.prompt, description: "d" })
							: undefined,
					success: true,
					pastTenseMessage: "task done",
					content:
						opts.suffixText !== undefined
							? [{ type: ToolResultContentType.Text, text: opts.suffixText }]
							: undefined,
				},
			},
		],
...
suite("claudeSubagentResolver — TextSuffixStrategy", () => {
	...
	test("hits when parent transcript carries the synthetic suffix; misses otherwise", async () => {
		...
		const ctx: ISubagentLookupContext = {
			...
			parentTranscript: [
				makeAgentToolCallTurn("toolu_hit", {
					suffixText: "whatever\nagentId: a7b3c1d2\n(trailing)",
				}),
				makeAgentToolCallTurn("toolu_no_suffix", {
					suffixText: "just text, no marker",
				}),
			],
...
suite("claudeSubagentResolver — scanTranscriptForAgentIds", () => {
	...
	test("extracts every (toolCallId, agentId) pair in one pass; skips unrelated tools", () => {
		const transcript: readonly Turn[] = [
			...
			makeAgentToolCallTurn("toolu_c", {
				suffixText: "agentId: agentc1",
				toolName: "Bash",
			}), // non-subagent tool
			makeAgentToolCallTurn("toolu_d", {
				suffixText: "agentId: agentd1",
				toolName: "Agent",
			}),
		];
...
suite("claudeSubagentResolver — getSubagentTranscript", () => {
	...
	test("cache hit on registry short-circuits SDK fetch; cache miss runs strategy chain and writes resolved agentId back to the registry; subsequent reads hit the cache", async () => {
		...
		const subagentUriA = URI.parse(
			buildSubagentSessionUri(parentUri, "toolu_a"),
		);
		const subagentUriB = URI.parse(
			buildSubagentSessionUri(parentUri, "toolu_b"),
		);
...
suite("claudeSubagentResolver — resolveAgentIdViaChain (free function)", () => {
	...
	test("cache hit short-circuits before any strategy runs", async () => {
		...
		assert.deepStrictEqual(
			{ out, calls, cacheWrites: deps.cacheWrites },
			{
				out: "cached-agent",
				calls: [],
				cacheWrites: [],
	...
	test("full miss returns undefined and writes nothing", async () => {
		...
		assert.deepStrictEqual(
			{ out, cacheWrites: deps.cacheWrites },
			{
				out: undefined,
				cacheWrites: [],
			},
	...
	test("cancellation between strategies stops the chain", async () => {
		...
		const out = await resolveAgentIdViaChain(
			"toolu",
...
suite("claudeSubagentResolver — extractSpawningPromptFromTranscript", () => {
	...
	test("returns prompt for matching subagent tool; rejects malformed/streaming/wrong-tool", () => {
		const transcript: readonly Turn[] = [
			...
			makeAgentToolCallTurn("toolu_streaming", {
				prompt: "unfinished",
				status: undefined,
			}),
			makeAgentToolCallTurn("toolu_wrong_tool", {
				prompt: "p",
				toolName: "Read",
			}),
		...
		(
			transcript[1].responseParts[0] as { toolCall: { status: ToolCallStatus } }
		).toolCall.status = ToolCallStatus.Streaming;
		// Mutate bad-json turn to have non-string toolInput.
		(
			transcript[3].responseParts[0] as { toolCall: { toolInput: unknown } }
		).toolCall.toolInput = "{not json";

		assert.deepStrictEqual(
			{
				match: extractSpawningPromptFromTranscript(transcript, "toolu_match"),
				streaming: extractSpawningPromptFromTranscript(
					transcript,
					"toolu_streaming",
				),
				wrongTool: extractSpawningPromptFromTranscript(
					transcript,
					"toolu_wrong_tool",
				),
				badJson: extractSpawningPromptFromTranscript(
					transcript,
					"toolu_bad_json",
				),
				missing: extractSpawningPromptFromTranscript(
					transcript,
					"toolu_unknown",
				),
			},
...
suite("claudeSubagentResolver — fetchParentTurns", () => {
	...
	test("returns ctx.parentTranscript without calling SDK; falls through to SDK; logs and returns undefined on SDK error", async () => {
		...
		const log = new NullLogService();
		const baseCtx = (
			overrides: Partial<ISubagentLookupContext>,
		): ISubagentLookupContext => ({
		...
		const fromCache = await fetchParentTurns(
			sdk,
			log,
			baseCtx({ parentTranscript: cached }),
			"L",
		);
