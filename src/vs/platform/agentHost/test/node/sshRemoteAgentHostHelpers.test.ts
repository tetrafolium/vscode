...
suite("SSH Remote Agent Host Helpers", () => {
	...
	function stateJson(
		pid: number,
		port: number,
		connectionToken: string | undefined | null,
	): string {
	...
	suite("validateShellToken", () => {
		test("accepts alphanumeric strings", () => {
			...
			assert.strictEqual(
				validateShellToken("exploration", "quality"),
				"exploration",
			);
		});
		...
		test("accepts dots, dashes, and underscores", () => {
			assert.strictEqual(
				validateShellToken("my-build_1.0", "quality"),
				"my-build_1.0",
			);
		});
		...
		test("rejects strings with spaces", () => {
			assert.throws(
				() => validateShellToken("foo bar", "quality"),
				/Unsafe quality/,
			);
		});
		...
		test("rejects strings with shell metacharacters", () => {
			assert.throws(
				() => validateShellToken("foo;rm -rf /", "quality"),
				/Unsafe quality/,
			);
			assert.throws(
				() => validateShellToken("$(whoami)", "quality"),
				/Unsafe quality/,
			);
			assert.throws(
				() => validateShellToken("foo'bar", "quality"),
	...
	suite("validateCommit", () => {
		...
		test("rejects non-hex characters", () => {
			...
			assert.throws(
				() => validateCommit("abcdef0123456789abcdef0123456789abcdef0z"),
				/Unsafe commit/,
			);
		});
	...
	suite("getRemoteCLIArchiveName", () => {
		...
		test("returns code-exploration for exploration", () => {
			assert.strictEqual(
				getRemoteCLIArchiveName("exploration"),
				"code-exploration",
			);
		});
		...
		test("falls back to code-insiders for unknown qualities", () => {
			...
			assert.strictEqual(
				getRemoteCLIArchiveName("weirdbuild"),
				"code-insiders",
			);
		});
	...
	suite("getRemoteCLIInstallRoot", () => {
		test("returns user-home anchored path under the server data folder", () => {
			assert.strictEqual(
				getRemoteCLIInstallRoot(".vscode-server-insiders"),
		...
		test("rejects unsafe server data folder names", () => {
			assert.throws(
				() => getRemoteCLIInstallRoot("foo bar"),
				/Unsafe server data folder name/,
			);
			assert.throws(
				() => getRemoteCLIInstallRoot("foo/bar"),
				/Unsafe server data folder name/,
			...
			assert.throws(
				() => getRemoteCLIInstallRoot("$(whoami)"),
				/Unsafe server data folder name/,
			);
		});
	});
	...
	suite("getRemoteCLIDataDir", () => {
		test("returns the `cli` subdir under the install root", () => {
			assert.strictEqual(
				getRemoteCLIDataDir(".vscode-server"),
				"~/.vscode-server/cli",
			);
			assert.strictEqual(
				getRemoteCLIDataDir(".vscode-server-insiders"),
	...
	suite("buildAgentHostBaseCommand", () => {
		test("includes --cli-data-dir before the agent host subcommand", () => {
			const cmd = buildAgentHostBaseCommand(
				"~/.vscode-server/code-insiders-abc",
				"~/.vscode-server/cli",
			);
			assert.strictEqual(
				cmd,
				"~/.vscode-server/code-insiders-abc --cli-data-dir ~/.vscode-server/cli agent host --port 0",
			);
		});
	});
	...
	suite("getRemoteCLIBin", () => {
		...
		test("rejects unsafe commit values", () => {
			assert.throws(
				() => getRemoteCLIBin(".vscode-server", "stable", "foo;rm"),
				/Unsafe commit/,
			);
		});
		...
		test("rejects unsafe server data folder names", () => {
			assert.throws(
				() => getRemoteCLIBin("foo bar", "stable", commit),
				/Unsafe server data folder name/,
			);
		});
	});
	...
	suite("resolveRemotePlatform", () => {
		test("detects Linux x64", () => {
			assert.deepStrictEqual(resolveRemotePlatform("Linux", "x86_64"), {
				os: "linux",
				arch: "x64",
			});
		});
		...
		test("detects Linux amd64", () => {
			assert.deepStrictEqual(resolveRemotePlatform("Linux", "amd64"), {
				os: "linux",
				arch: "x64",
			});
		});
		...
		test("detects Linux arm64 (aarch64)", () => {
			assert.deepStrictEqual(resolveRemotePlatform("Linux", "aarch64"), {
				os: "linux",
				arch: "arm64",
			});
		});
		...
		test("detects Linux arm64", () => {
			assert.deepStrictEqual(resolveRemotePlatform("Linux", "arm64"), {
				os: "linux",
				arch: "arm64",
			});
		});
		...
		test("detects Linux armhf", () => {
			assert.deepStrictEqual(resolveRemotePlatform("Linux", "armv7l"), {
				os: "linux",
				arch: "armhf",
			});
		});
		...
		test("detects Darwin x64", () => {
			assert.deepStrictEqual(resolveRemotePlatform("Darwin", "x86_64"), {
				os: "darwin",
				arch: "x64",
			});
		});
		...
		test("detects Darwin arm64", () => {
			assert.deepStrictEqual(resolveRemotePlatform("Darwin", "arm64"), {
				os: "darwin",
				arch: "arm64",
			});
		});
		...
		test("returns undefined for Windows", () => {
			assert.strictEqual(
				resolveRemotePlatform("MINGW64_NT-10.0-19041", "x86_64"),
				undefined,
			);
		});
	...
	suite("buildCLIDownloadUrl", () => {
		...
		test("rejects unsafe commit values", () => {
			assert.throws(
				() => buildCLIDownloadUrl("linux", "x64", "insider", "foo;rm"),
				/Unsafe commit/,
			);
		});
	...
	suite("buildCleanupOldCLIsCommand", () => {
		test("produces a snippet that keeps the 5 most recent commit-keyed CLIs for insider", () => {
			const cmd = buildCleanupOldCLIsCommand(
				".vscode-server-insiders",
				"insider",
			);
			// Target the commit-keyed pattern (with 40 chars), under the shared install root.
			assert.ok(
				cmd.includes("~/.vscode-server-insiders/code-insiders-"),
				`cmd missing install path: ${cmd}`,
			);
			assert.ok(
				/(\[0-9a-f\]){40}/.test(cmd),
				"cmd should match exactly 40 hex chars",
			);
			...
			assert.ok(
				/xargs\s+-I\{\}\s+rm\s+-f\s+--/.test(cmd),
				`cmd should rm safely: ${cmd}`,
			);
		});
		...
		test("uses `code-` archive name for stable", () => {
			...
			assert.ok(
				cmd.includes("~/.vscode-server/code-[0-9a-f]"),
				`cmd should target stable archive: ${cmd}`,
			);
			assert.ok(
				!cmd.includes("code-insiders-"),
				"stable cmd should not mention insiders archive",
			);
		});

		test("rejects unsafe inputs", () => {
			assert.throws(
				() => buildCleanupOldCLIsCommand("foo bar", "stable"),
				/Unsafe server data folder name/,
			);
			assert.throws(
				() => buildCleanupOldCLIsCommand(".vscode-server", "foo bar"),
				/Unsafe quality/,
			);
		});
	});
	...
	suite("buildFindFallbackCLICommand", () => {
		test("lists commit-keyed candidates then legacy paths for insider", () => {
			const cmd = buildFindFallbackCLICommand(
				".vscode-server-insiders",
				"insider",
			);
			...
			assert.ok(
				cmd.includes("~/.vscode-server-insiders/code-insiders-"),
				`cmd missing new path: ${cmd}`,
			);
			assert.ok(
				/ls -1t/.test(cmd),
				"should sort commit-keyed candidates by mtime",
			);
			// Legacy single-binary path (insider has the `-insider` dir suffix).
			assert.ok(
				cmd.includes("~/.vscode-cli-insider/code-insiders"),
				`cmd missing legacy path: ${cmd}`,
			);
		});
		...
		test("uses no-suffix legacy dir for stable", () => {
			...
			assert.ok(
				cmd.includes("~/.vscode-cli/code"),
				`cmd missing stable legacy path: ${cmd}`,
			);
			assert.ok(
				!cmd.includes(".vscode-cli-stable"),
				"stable should not get the -<quality> suffix",
			);
		});

		test("rejects unsafe inputs", () => {
			assert.throws(
				() => buildFindFallbackCLICommand("foo bar", "stable"),
				/Unsafe server data folder name/,
			);
			assert.throws(
				() => buildFindFallbackCLICommand(".vscode-server", "foo bar"),
				/Unsafe quality/,
			);
		});
	});
	...
	suite("isValidFallbackCLIPath", () => {
		...
		test("accepts commit-keyed path under the shared install root", () => {
			assert.strictEqual(
				isValidFallbackCLIPath(`~/${sdf}/code-insiders-${hex}`, sdf, q),
				true,
			);
		});
		...
		test("accepts legacy ~/.vscode-cli-<quality>/<archive> path for insider", () => {
			assert.strictEqual(
				isValidFallbackCLIPath("~/.vscode-cli-insider/code-insiders", sdf, q),
				true,
			);
		});
		...
		test("rejects commit suffix with non-hex characters", () => {
			...
			assert.strictEqual(
				isValidFallbackCLIPath(`~/${sdf}/code-insiders-${notHex}`, sdf, q),
				false,
			);
		});
		...
		test("rejects commit suffix with wrong length", () => {
			assert.strictEqual(
				isValidFallbackCLIPath(
					`~/${sdf}/code-insiders-${hex.slice(0, 39)}`,
					sdf,
					q,
				),
				false,
			...
			assert.strictEqual(
				isValidFallbackCLIPath(`~/${sdf}/code-insiders-${hex}a`, sdf, q),
				false,
			);
		});
		...
		test("rejects paths under an unexpected root", () => {
			assert.strictEqual(
				isValidFallbackCLIPath(
					`~/.something-else/code-insiders-${hex}`,
					sdf,
					q,
				),
				false,
		...
		test("rejects shell metacharacters", () => {
			...
			assert.strictEqual(
				isValidFallbackCLIPath(`~/${sdf}/code-insiders-${hex} && evil`, sdf, q),
				false,
			);
		});
	});
	...
	suite("getAgentHostLockfile", () => {
		...
		test("rejects unsafe server data folder names", () => {
			assert.throws(
				() => getAgentHostLockfile("foo bar", "stable"),
				/Unsafe server data folder name/,
			);
			assert.throws(
				() => getAgentHostLockfile("foo/bar", "stable"),
			...
			assert.throws(
				() => getAgentHostLockfile("$(whoami)", "stable"),
				/Unsafe server data folder name/,
			);
		});
		...
		test("rejects unsafe quality strings", () => {
			assert.throws(
				() => getAgentHostLockfile(".vscode-server-oss", "foo bar"),
				/Unsafe quality/,
			);
		});
	});
	...
	suite("findRunningAgentHost", () => {
		...
		test("returns notFound when no state file exists", async () => {
			const exec = createMockExec(
				new Map([["cat", { stdout: "", stderr: "", code: 1 }]]),
			);
			const result = await findRunningAgentHost(
				exec,
				logService,
	...
	suite("writeAgentHostState", () => {
		...
		test("writes lockfile with canonical metadata JSON", async () => {
			...
			assert.ok(
				commands[0].includes(`"protocolVersion":"${PROTOCOL_VERSION}"`),
			);
		...
		test("logs warning when write command fails", async () => {
			...
			capturingLog.warn = (...args: unknown[]) => {
