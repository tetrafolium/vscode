...
suite("localAhpJsonlLogging", () => {
	...
	async function readEntries(fileService: FileService, logger: AhpJsonlLogger) {
		await logger.flush();
		const content = (
			await fileService.readFile(logger.resource)
		).value.toString();
		return content
			.split("\n")
			.filter(Boolean)
	...
	test("logs request/response/notification frames around proxied calls", async () => {
		...
		const target = {
			async authenticate(params: { resource: string; token: string }) {
				return { authenticated: params.token.length > 0 };
			},
			async listSessions() {
				return ["a", "b"];
			},
			async createSession(_config: IAgentCreateSessionConfig) {
				return URI.parse("agent-host://session/1");
			},
			async disposeTerminal() {
				return undefined;
			},
			async resourceRead(uri: URI) {
				throw new Error("boom: " + uri.toString());
			},
			dispatchAction(
				channel: URI,
				action: unknown,
		...
		await assert.rejects(() =>
			wrapped.resourceRead(URI.parse("agent-host://x/y")),
		);
		wrapped.dispatchAction(
			"agent-host://session/1",
			{ type: "noop" } as never,
			"client-1",
			7,
		);
		...
		assert.deepStrictEqual(entries[0].params, [
			{ resource: "https://example.com", token: "<redacted>" },
		]);
	...
	test("logs reverse resource channel requests and responses", async () => {
		...
		await channel.call(undefined, "resourceWrite", {
			uri,
			data: "hello",
		...
		await assert.rejects(() =>
			channel.call(undefined, "resourceRead", {
				uri: URI.file("/missing.txt").toString(),
			}),
		);
