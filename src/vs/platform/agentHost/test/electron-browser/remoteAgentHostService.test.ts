...
class TestConfigurationService {
	private readonly _onDidChangeConfiguration = new Emitter<
		Partial<IConfigurationChangeEvent>
	>();
	...
	async updateValue(_key: string, value: unknown): Promise<void> {
		...
		this._onDidChangeConfiguration.fire({
			affectsConfiguration: (key: string) =>
				key === RemoteAgentHostsSettingId ||
				key === RemoteAgentHostsEnabledSettingId,
		});
	}
	...
	setEntries(entries: IRemoteAgentHostEntry[]): void {
		this._entries = entries
			.map(entryToRawEntry)
			.filter((e): e is IRawRemoteAgentHostEntry => e !== undefined);
		this._onDidChangeConfiguration.fire({
			affectsConfiguration: (key: string) =>
				key === RemoteAgentHostsSettingId ||
				key === RemoteAgentHostsEnabledSettingId,
...
suite("RemoteAgentHostService", () => {
	...
	setup(() => {
		...
		instantiationService.stub(
			IConfigurationService,
			configService as Partial<IConfigurationService>,
		);
		...
		instantiationService.stub(
			...
			mockInstantiationService as Partial<IInstantiationService>,
		);

		service = disposables.add(
			instantiationService.createInstance(RemoteAgentHostService),
		);
	});
	...
	test("parses supported remote host inputs", () => {
		assert.deepStrictEqual(
			[
				...
				parseRemoteAgentHostInput(
					"Agent host proxy listening on ws://127.0.0.1:8089",
				),
				parseRemoteAgentHostInput("127.0.0.1:8089"),
				parseRemoteAgentHostInput("ws://127.0.0.1:8089"),
				parseRemoteAgentHostInput(
					"ws://127.0.0.1:40147?tkn=c9d12867-da33-425e-8d39-0d071e851597",
				),
			...
			[
				...
				{
					parsed: {
						address: "127.0.0.1:40147",
						connectionToken: "c9d12867-da33-425e-8d39-0d071e851597",
						suggestedName: "127.0.0.1:40147",
					},
				},
				{
					parsed: {
						address: "wss://secure.example.com",
						connectionToken: undefined,
						suggestedName: "secure.example.com",
					},
				},
			],
		);
	...
	test("creates connection when setting is updated", async () => {
		...
		const connected = service.connections.filter((c) =>
			RemoteAgentHostConnectionStatus.isConnected(c.status),
		);
	...
	suite("addManagedConnection", () => {
		...
		async function addManaged(
			...
			const mockClient = disposables.add(
				new MockProtocolClient(`ws://${address}`),
			);
		...
		test("disposes transportDisposable when entry is removed via removeRemoteAgentHost", async () => {
			...
			assert.strictEqual(
				t.disposed(),
				true,
				"transport disposable runs when entry is removed",
			);
		...
		test("does NOT dispose previous transportDisposable when entry is replaced", async () => {
			...
			assert.strictEqual(
				t1.disposed(),
				false,
				"previous transport disposable is not run on replacement",
			);
			assert.strictEqual(
				t2.disposed(),
				false,
				"new transport disposable is still alive",
			);
			...
			assert.strictEqual(
				t2.disposed(),
				true,
				"new transport disposable runs on full removal",
			);
		});
		...
		test("disposes transportDisposable when service itself is disposed", async () => {
			...
			assert.strictEqual(
				t.disposed(),
				true,
				"transport disposable runs when service is disposed",
			);
		});

		test("stores SSH connection details outside the remote hosts setting", async () => {
			const mockClient = disposables.add(
				new MockProtocolClient("ssh:remote.example"),
			);
	...
	suite("host label formatter", () => {
		function formatterFor(address: string): ResourceLabelFormatter | undefined {
			...
			return registeredFormatters.find(
				(f) => f.scheme === AGENT_HOST_SCHEME && f.authority === authority,
			);
		}
		...
		test("refreshes formatter when an entry name changes", async () => {
			...
			const matching = registeredFormatters.filter(
				(f) => f.authority === agentHostAuthority("host1:8080"),
			);
			assert.strictEqual(
				matching.length,
				1,
				"old formatter is replaced, not duplicated",
			);
