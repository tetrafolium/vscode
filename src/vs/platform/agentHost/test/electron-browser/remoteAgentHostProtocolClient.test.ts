...
} from "../../browser/remoteAgentHostProtocolClient.js";
...
function isPingRequest(
	msg: ProtocolTransportMessage,
): msg is JsonRpcRequest & { method: "ping" } {
...
class TestClientProtocolTransport
	extends TestProtocolTransport
...
suite("RemoteAgentHostProtocolClient", () => {
	...
	function createClient(
		transport = disposables.add(new TestProtocolTransport()),
		permissionService = createPermissionService(),
		loadEstimator?: { hasHighLoad(): boolean },
	): {
		client: RemoteAgentHostProtocolClient;
		transport: TestProtocolTransport;
	} {
		const fileService = disposables.add(new FileService(new NullLogService()));
		const client = disposables.add(
			new RemoteAgentHostProtocolClient(
				"test.example:1234",
				transport,
				loadEstimator,
				new NullLogService(),
	...
	async function assertRemoteProtocolError(
		promise: Promise<unknown>,
		expected: { code: number; message: string; data?: unknown },
	): Promise<void> {
		try {
			await promise;
	...
	test("completes matching response and removes it from pending requests", async () => {
		...
		transport.fireMessage({
			jsonrpc: "2.0",
			id: 1,
			result: { entries: [{ name: "late", type: "file" }] },
		});
	...
	test("rejects all pending requests on transport close", async () => {
		...
		const firstRejected = assertRemoteProtocolError(first, {
			code: -32000,
			message: "Connection closed: test.example:1234",
		});
		const secondRejected = assertRemoteProtocolError(second, {
			code: -32000,
			message: "Connection closed: test.example:1234",
		});
	...
	test("rejects pending requests on dispose", async () => {
		...
		const resultPromise = client.resourceList(URI.file("/workspace"));
		const rejected = assertRemoteProtocolError(resultPromise, {
			code: -32000,
			message: "Connection disposed: test.example:1234",
		});
	...
	test("rejects requests started after transport close", async () => {
		...
		await assertRemoteProtocolError(
			client.resourceList(URI.file("/workspace")),
			{ code: -32000, message: "Connection closed: test.example:1234" },
		);
		assert.strictEqual(transport.sentMessages.length, 0);
	});
	...
	test("rejects requests started after dispose", async () => {
		...
		await assertRemoteProtocolError(
			client.resourceList(URI.file("/workspace")),
			{ code: -32000, message: "Connection disposed: test.example:1234" },
		);
		assert.strictEqual(transport.sentMessages.length, 0);
	});
	...
	test("liveness sends a ping when idle and force-closes after the ping ages out", async () => {
		return runWithFakedTimers(
			...
			async () => {
				...
				assert.ok(
					pings.length >= 1,
					`expected at least 1 ping, got ${pings.length}`,
				);
				assert.strictEqual(closeCount, 1);
				client.dispose();
			},
	...
	test("liveness keeps the connection open while pings are answered", async () => {
		return runWithFakedTimers(
			...
			async () => {
				...
				assert.ok(
					answered >= 4,
					`expected several pings to have been answered, got ${answered}`,
				);
				client.dispose();
			},
		);
	...
	test("liveness stops after the connection is closed", async () => {
		return runWithFakedTimers(
			...
			async () => {
				...
				const pingsAtClose =
					transport.sentMessages.filter(isPingRequest).length;
				...
				assert.strictEqual(
					pingsLater,
					pingsAtClose,
					"no further pings should be sent after close",
				);
				client.dispose();
			},
		);
		...
		return runWithFakedTimers(
			...
			async () => {
				...
				assert.strictEqual(
					actionCount,
					0,
					"late action notifications must be ignored after close",
				);
				client.dispose();
			},
		);
	...
	test("rejects connect when transport closes before connect completes", async () => {
		...
		const { client } = createClient(transport);
		const rejected = assertRemoteProtocolError(client.connect(), {
			code: -32000,
			message: "Connection closed: test.example:1234",
		});
		...
		assert.deepStrictEqual(telemetryLevel, {
			...
			params: {
				...
				action: {
					...
					config: {
						[AgentHostTelemetryLevelConfigKey]:
							telemetryLevelToAgentHostConfigValue(TelemetryLevel.USAGE),
					},
				},
			},
		});
	...
	test("rejects shutdown with structured JSON-RPC error", async () => {
		...
		transport.fireMessage({
			...
			id: 1,
			error: {
				code: AhpErrorCodes.TurnInProgress,
	...
	suite("reverse permission gating", () => {
		...
		test("resourceMove is denied when destination lacks write access", async () => {
			...
			assert.deepStrictEqual(transport.sentMessages.pop(), {
				...
				error: {
					...
					data: {
						request: { channel: ROOT_STATE_URI, uri: destUri, write: true },
					},
				},
			});
		});
		...
		test("reverse resourceRequest delegates to permission service and replies with empty result", async () => {
			...
			transport.fireMessage({
				...
				params: { channel: "ahp-root://", uri, read: true },
			});

			// Allow the awaited request promise to resolve.
			await new Promise((resolve) => setTimeout(resolve, 0));

			assert.deepStrictEqual(lastRequest, {
				address: "test.example:1234",
				params: { channel: "ahp-root://", uri, read: true },
			});
			assert.deepStrictEqual(transport.sentMessages.pop(), {
				jsonrpc: "2.0",
	...
	suite("implicit grants for outgoing customization actions", () => {
		function createCapturingPermissionService(): {
			service: IAgentHostPermissionService;
			calls: { address: string; uri: URI }[];
		} {
			const empty = observableValue<readonly never[]>("test", []);
					...
					customizations: [
						{
							...
							id: customizationId("file:///plugins/foo"),
							uri: "file:///plugins/foo",
							name: "Foo",
							enabled: true,
						},
						{
							type: CustomizationType.Plugin,
							id: customizationId("file:///other/bar"),
							uri: "file:///other/bar",
							name: "Bar",
							enabled: true,
						},
			...
			client.dispatch(sessionUri.toString(), {
				...
				activeClient: {
					...
					customizations: [
						...
						{
							type: CustomizationType.Plugin,
							id: customizationId("file:///plugins/bar"),
							uri: "file:///plugins/bar",
							name: "Bar",
							enabled: true,
						},
					],
		...
		test("createSession with active-client customizations grants implicit reads", async () => {
			...
			const sent = transport.sentMessages.find(
				(m): m is JsonRpcRequest =>
					"method" in m && m.method === "createSession",
			);
			assert.ok(sent);
	...
	suite("soft reconnect (transport factory)", () => {
		function findRequest(
			...
			return transport.sentMessages.find(
				(m): m is JsonRpcRequest =>
					"method" in m && (m as JsonRpcRequest).method === method && "id" in m,
			);
		}
		...
		function findNotification(
			...
			return transport.sentMessages.find(
				(m): m is JsonRpcNotification =>
					"method" in m &&
					(m as JsonRpcNotification).method === method &&
					!("id" in m),
			);
		}

		function findDispatchAction(
			transport: TestProtocolTransport,
			...
			return transport.sentMessages.find(
				(m): m is JsonRpcNotification =>
					...
					!("id" in m) &&
					(
						(m as JsonRpcNotification).params as
							| { action?: { type?: unknown } }
							| undefined
		...
		async function waitForReconnecting(
			client: RemoteAgentHostProtocolClient,
		): Promise<void> {
			...
			await Event.toPromise(
				Event.filter(
					client.onDidChangeConnectionState,
					(s) => s === AgentHostClientState.Reconnecting,
				),
			);
		}
		...
		async function waitForTransport(
			transports: TestClientProtocolTransport[],
			index: number,
		): Promise<TestClientProtocolTransport> {
		...
		async function completeHandshake(
			transport: TestClientProtocolTransport,
		...
		test("reuses clientId across transport reconnects", async function () {
			...
			return runWithFakedTimers(
				...
				async () => {
					...
					const reconnect = await waitForRequest(
						reconnectTransport,
						"reconnect",
					);

					const params = reconnect.params as {
						clientId: string;
		...
		test("replays pending optimistic actions after reconnect", async function () {
			...
			return runWithFakedTimers(
				...
				async () => {
					...
					const subRef = client.getSubscription(
					...
					client.dispatch(sessionUri.toString(), action);
					const initialDispatch = findDispatchAction(
						transports[0],
						ActionType.SessionTitleChanged,
					);
					assert.ok(
						initialDispatch,
						"optimistic dispatch should reach the original transport",
					);
					const initialSeq = (initialDispatch.params as { clientSeq: number })
						.clientSeq;
					...
					const replayed = findDispatchAction(
						reconnectTransport,
						ActionType.SessionTitleChanged,
					);
					assert.ok(
						replayed,
						"pending optimistic action should be re-sent after reconnect",
					);
					assert.strictEqual(
						(replayed.params as { clientSeq: number }).clientSeq,
						initialSeq,
						"replayed dispatch must reuse the original clientSeq",
					);
			...
			return runWithFakedTimers(
				...
				async () => {
					...
					const inFlight = client
						.resourceList(URI.file("/workspace"))
						.catch((err) => err);
					...
					const resourceList = await waitForRequest(
						reconnectTransport,
						"resourceList",
					);
					reconnectTransport.fireMessage({
		...
		test("rejected action echoed in replay buffer is not applied to confirmed state", async function () {
			...
			return runWithFakedTimers(
				...
				async () => {
					...
					const sessionUri = URI.parse("copilot:/test-session");
					const subRef = client.getSubscription<{ summary: { title: string } }>(
						StateComponents.Session,
						sessionUri,
					);
					const subscribeReq = await waitForRequest(transports[0], "subscribe");
					transports[0].fireMessage({
						...
						result: {
							snapshot: {
								resource: sessionUri.toString(),
								state: { summary: { title: "Original" }, turns: [] },
								fromSeq: 5,
							},
						},
					});
		...
		test("snapshot reconnect result reseats the root state", async function () {
			...
			return runWithFakedTimers(
				...
				async () => {
					...
					assert.ok(
						root && !(root instanceof Error),
						"root state should be hydrated from snapshot",
					);
		...
		test("non-session dispatch issued during reconnect rides retries until success", async function () {
			...
			return runWithFakedTimers(
				...
				async () => {
					...
					assert.ok(
						dispatched,
						"terminal dispatch must ride the failed attempt through to the next successful one",
					);
					client.dispose();
				},
			);
		...
		test("_sendExtensionRequest waits for the reconnect gate", async function () {
			...
			return runWithFakedTimers(
				...
				async () => {
					...
					const shutdownReq = await waitForRequest(
						reconnectTransport,
						"shutdown",
					);
					reconnectTransport.fireMessage({
