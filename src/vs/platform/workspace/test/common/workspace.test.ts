...
suite("Workspace", () => {
	...
	test("toWorkspaceFolders with single absolute folder", () => {
		...
		assert.strictEqual(
			(<IRawFileWorkspaceFolder>actual[0].raw).path,
			"/src/test",
		);
	...
	test("toWorkspaceFolders with multiple unique absolute folders", () => {
		const actual = toWorkspaceFolders(
			[{ path: "/src/test2" }, { path: "/src/test3" }, { path: "/src/test1" }],
			workspaceConfigUri,
			extUriBiasedIgnorePathCase,
		);
		...
		assert.strictEqual(
			(<IRawFileWorkspaceFolder>actual[1].raw).path,
			"/src/test3",
		);
		...
		assert.strictEqual(
			(<IRawFileWorkspaceFolder>actual[2].raw).path,
			"/src/test1",
		);
