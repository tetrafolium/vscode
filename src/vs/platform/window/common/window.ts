...
export function hasNativeContextMenu(
	...
	const windowConfigurations = configurationService.getValue<
...
export interface IWindowSettings {
	...
	readonly border:
		...
		| "default"
		| "system"
		| string /* color in RGB or other formats */;
}
...
export function getWindowControlsStyle(
	...
	if (
		isWeb ||
		isMacintosh ||
		getTitleBarStyle(configurationService) === TitlebarStyle.NATIVE
	) {
	...
	if (
		style === WindowControlsStyle.CUSTOM ||
		style === WindowControlsStyle.HIDDEN
	) {
		return style;
	}
...
export function useWindowControlsOverlay(
	...
	if (!isMacintosh) {
		...
		if (
			setting === WindowControlsStyle.CUSTOM ||
			setting === WindowControlsStyle.HIDDEN
		) {
...
export interface INativeWindowConfiguration
	extends IWindowConfiguration, NativeParsedArgs, ISandboxConfiguration {
	mainPid: number;
	handle?: VSBuffer;
