...
export function registerOpenEditorListeners(
	...
	disposables.add(
		addDisposableListener(element, EventType.CLICK, (e) => {
			...
			onOpenEditor(
				toOpenEditorOptions(new StandardMouseEvent(getWindow(element), e)),
			);
		}),
	);
	...
	disposables.add(
		addDisposableListener(element, EventType.DBLCLICK, (e) => {
			...
			onOpenEditor(
				toOpenEditorOptions(
					new StandardMouseEvent(getWindow(element), e),
					true,
				),
			);
		}),
...
export function toOpenEditorOptions(
	...
	if (event instanceof StandardKeyboardEvent) {
		...
		if (
			...
			(isMacintosh && event.equals(KeyMod.CtrlCmd | KeyCode.DownArrow))
		) {
			preserveFocus = false;
		...
		return {
			editorOptions: {
				...
				pinned: isDoubleClick || event.middleButton,
			},
