...
suite("trustedDomains", () => {
	...
	suite("isURLDomainTrusted", () => {
		test("localhost is always trusted", () => {
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("http://localhost:3000"), []),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("http://127.0.0.1:3000"), []),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("http://subdomain.localhost"), []),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://[::1]"), []),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("http://[::1]:3000"), []),
				true,
			);
		});
		...
		test("wildcard (*) matches everything", () => {
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://example.com"), ["*"]),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("http://anything.org"), ["*"]),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://github.com/microsoft"), ["*"]),
				true,
			);
		});
		...
		test("exact domain match", () => {
			...
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://example.com/path"), [
					"https://example.com",
				]),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("http://example.com"), [
					"https://example.com",
				]),
				false,
			);
		...
		test("subdomain wildcard matching", () => {
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://api.github.com"), [
					"https://*.github.com",
				]),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://github.com"), [
					"https://*.github.com",
				]),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://sub.api.github.com"), [
					"https://*.github.com",
				]),
				true,
			);
		...
		test("path matching", () => {
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://example.com/api/v1"), [
			...
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://example.com/api"), [
					"https://example.com/api/*",
				]),
				false,
			);
		...
		test("multiple trusted domains", () => {
			...
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://github.com"), trusted),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://microsoft.com"), trusted),
				true,
			);
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://google.com"), trusted),
				false,
			);
		});
		...
		test("case normalization for github", () => {
			assert.strictEqual(
				isURLDomainTrusted(URI.parse("https://github.com/Microsoft/VSCode"), [
	...
	suite("normalizeURL", () => {
		test("normalizes github.com URLs to lowercase path", () => {
			assert.strictEqual(
				normalizeURL("https://github.com/Microsoft/VSCode"),
				"https://github.com/microsoft/vscode",
			);
			assert.strictEqual(
		...
		test("does not normalize non-github URLs", () => {
			...
			assert.strictEqual(
				normalizeURL("https://microsoft.com/Products"),
	...
	suite("isAllInterfacesAuthority", () => {
		...
		test("recognizes full-form IPv6 all-interfaces [0:0:0:0:0:0:0:0]", () => {
			...
			assert.strictEqual(
				isAllInterfacesAuthority("[0:0:0:0:0:0:0:0]:3000"),
				true,
			);
		});
