...
export const OPTIONS: OptionDescriptions<Required<NativeParsedArgs>> = {
	...
	agents: {
		...
		cat: "o",
		deprecates: ["sessions"],
		description: localize("agents", "Opens the agents window."),
	},
	wait: {
		type: "boolean",
		cat: "o",
...
export function parseArgs<T>(
	...
	if (command && firstPossibleCommand) {
		...
		const reporter = errorReporter.getSubcommandReporter
			...
			: undefined;
		const subcommandOptions = parseArgs(
			newArgs,
			options as OptionDescriptions<Record<string, unknown>>,
			reporter,
		);
		...
		if (typeof val !== "undefined") {
			if (o.type === "string[]") {
				...
				if (!o.allowEmptyValue) {
					const sanitized = (val as string[]).filter(
						(v: string) => v.length > 0,
					);
...
function formatUsage(
	optionId: string,
	option: Option<"boolean"> | Option<"string"> | Option<"string[]">,
) {
	let args = "";
	if (option.args) {
...
export function formatOptions(
	options:
		...
		| Record<string, Option<"boolean"> | Option<"string"> | Option<"string[]">>,
	columns: number,
): string[] {
	const usageTexts: [string, string][] = [];
	for (const optionId in options) {
		const o = options[optionId as keyof typeof options] as
			| Option<"boolean">
			| Option<"string">
			| Option<"string[]">;
...
export function buildHelpMessage(
	...
	help.push("");
	help.push(
		`${localize("usage", "Usage")}: ${executableName}${subcommand} [${localize("options", "options")}]${inputFiles}`,
	);
	help.push("");
	...
	const optionsByCategory: {
		[P in keyof typeof helpCategories]?: Record<
			string,
			Option<"boolean"> | Option<"string"> | Option<"string[]">
		>;
	} = {};
