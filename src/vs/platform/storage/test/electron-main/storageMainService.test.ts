...
import {
	...
	IStorageMain,
	IStorageMainOptions,
} from "../../electron-main/storageMain.js";
...
suite("StorageMainService", function () {
	...
	const inMemoryProfile: IUserDataProfile = {
		...
		languageModelsResource: joinPath(
			inMemoryProfileRoot,
	...
	function createStorageService(
		lifecycleMainService: ILifecycleMainService = new TestLifecycleMainService(),
	...
	test("storage closed onWillShutdown", async function () {
		...
		strictEqual(
			workspaceStorage,
			storageMainService.workspaceStorage(workspace),
		); // same instance as long as not closed
	...
	test("application shared storage closed onWillShutdown", async function () {
		...
		const applicationSharedStorage =
			storageMainService.applicationSharedStorage;
		let didCloseApplicationSharedStorage = false;
		disposables.add(
			applicationSharedStorage.onDidCloseStorage(() => {
