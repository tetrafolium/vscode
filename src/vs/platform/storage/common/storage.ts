	...
	implements IStorageService
{
	...
	constructor(
		options: IStorageServiceOptions = {
	...
	async log(): Promise<void> {
		...
		const applicationSharedItems =
			this.getStorage(StorageScope.APPLICATION_SHARED)?.items ??
			new Map<string, string>();
		const profileItems =
			this.getStorage(StorageScope.PROFILE)?.items ?? new Map<string, string>();
...
export async function logStorage(
	...
	applicationShared: Map<string, string>,
	profile: Map<string, string>,
	workspace: Map<string, string>,
	applicationPath: string,
	applicationSharedPath: string,
	profilePath: string,
