	...
	implements IServerChannel<T>
{
		...
		switch (command) {
			...
			case "open":
				return this.open(
					uriTransformer,
					args[0] as UriComponents,
					args[1] as IFileOpenOptions,
				) as Promise<TResult>;
			case "close":
				return this.close(args[0] as number) as Promise<TResult>;
			case "read":
				return this.read(
					args[0] as number,
					args[1] as number,
			...
			case "readFile":
				return this.readFile(
					uriTransformer,
					args[0] as UriComponents,
					args[1] as IFileAtomicReadOptions,
				) as Promise<TResult>;
			case "write":
				return this.write(
					args[0] as number,
			...
			case "rename":
				return this.rename(
					...
					args[2] as IFileOverwriteOptions,
				) as Promise<TResult>;
			case "copy":
				return this.copy(
					uriTransformer,
			...
			case "watch":
				return this.watch(
					...
					args[0] as string,
					args[1] as number,
					args[2] as UriComponents,
					args[3] as IWatchOptions,
				) as Promise<TResult>;
			case "unwatch":
				return this.unwatch(
					args[0] as string,
	...
	listen<TResult>(ctx: T, event: string, args: unknown[]): Event<TResult> {
		...
		switch (event) {
			...
			case "readFileStream":
				return this.onReadFileStream(
					uriTransformer,
					args[0] as URI,
					args[1] as IFileReadStreamOptions,
				) as Event<TResult>;
		}
