...
flakySuite("IndexedDBFileSystemProvider", function () {
	...
	const initFixtures = async () => {
		...
		await Promise.all(
			(
				...
				.map(
					([path, contents]) => [userdataURIFromPaths(path), contents] as const,
				)
				.map(([uri, contents]) =>
					service.createFile(uri, VSBuffer.fromString(contents)),
				),
		);
	};
	...
	const reload = async () => {
		...
		disposables.add(
			service.registerProvider(Schemas.vscodeUserData, userdataFileProvider),
		);
			...
			async assertContentsCorrect() {
				...
				await Promise.all(
					batch.map(async (entry, i) => {
						...
						assert.strictEqual(
							new TextDecoder().decode(
								await userdataFileProvider.readFile(entry.resource),
							),
							entry.contents,
						);
					}),
	...
	test("writeFile with append - non-existent file", async () => {
		...
		await service.writeFile(resource, VSBuffer.fromString("First content"), {
			append: true,
		});
	...
	test("provider writeFile with append - direct provider API", async () => {
		...
		await userdataFileProvider.writeFile(
			resource,
			VSBuffer.fromString("First ").buffer,
			{ create: true, overwrite: true, unlock: false, atomic: false },
		);
		await userdataFileProvider.writeFile(
			resource,
			VSBuffer.fromString("Second").buffer,
			{
				create: true,
				overwrite: true,
				unlock: false,
