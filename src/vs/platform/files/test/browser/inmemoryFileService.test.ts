...
suite("InMemory File Service", () => {
	...
	test("createFolder", async () => {
		...
		assert.strictEqual(
			event.target?.resource.toString(),
			newFolderResource.toString(),
		);
	...
	test("createFolder: creating multiple folders at once", async () => {
		...
		assert.strictEqual(
			newFolder.name,
			multiFolderPaths[multiFolderPaths.length - 1],
		);
		assert.strictEqual(await service.exists(newFolderResource), true);
	...
	test("readFile - from position (with umlaut)", async () => {
		...
		const content = await service.readFile(fixture.smallUmlautTxt, {
			position: pos,
		});
	...
	test("provider open/write - append (large)", async () => {
		...
		assert.strictEqual(
			...
			prefix.byteLength + suffix.byteLength,
		);

		assert.deepStrictEqual(
			content.value.slice(0, 64).buffer,
			prefix.slice(0, 64).buffer,
		);
		assert.deepStrictEqual(
	...
	test("writeFile - append", async () => {
		...
		await service.writeFile(resource, VSBuffer.fromString(" World"), {
			append: true,
		});
	...
	test("setReadOnly toggles provider capabilities", async () => {
		...
		try {
			await service.writeFile(
				joinPath(fixture.root, "readonly.txt"),
				VSBuffer.fromString("fail"),
			);
		} catch (e) {
			error = e;
		}
...
async function createFixture(service: FileService): Promise<Fixture> {
	...
	await service.writeFile(
		...
		VSBuffer.fromString("Index"),
	);
	await service.writeFile(
		joinPath(root, "site.css"),
		VSBuffer.fromString("body { }"),
	);

	await service.writeFile(
		joinPath(root, "small.txt"),
		VSBuffer.fromString("Small File"),
	);
	await service.writeFile(
		joinPath(root, "small_umlaut.txt"),
		VSBuffer.fromString("Small File with Ümlaut"),
	);

	await service.createFolder(joinPath(root, "deep"));
	await service.writeFile(
		joinPath(root, "deep", "conway.js"),
		VSBuffer.fromString('console.log("conway");'),
	);
	await service.writeFile(
		joinPath(root, "deep", "a.txt"),
		VSBuffer.fromString("A"),
	);
	await service.writeFile(
		joinPath(root, "deep", "b.txt"),
		VSBuffer.fromString("B"),
	);
	await service.writeFile(
		joinPath(root, "deep", "c.txt"),
		VSBuffer.fromString("C"),
	);

	await service.createFolder(joinPath(root, "other", "deep"));
	await service.writeFile(
		joinPath(root, "other", "deep", "1.txt"),
		VSBuffer.fromString("1"),
	);
	await service.writeFile(
		joinPath(root, "other", "deep", "2.txt"),
		VSBuffer.fromString("2"),
	);
	await service.writeFile(
		joinPath(root, "other", "deep", "3.txt"),
		VSBuffer.fromString("3"),
	);
	await service.writeFile(
		joinPath(root, "other", "deep", "4.txt"),
		VSBuffer.fromString("4"),
	);
