...
export class TestParcelWatcher extends ParcelWatcher {
	...
	async testRemoveDuplicateRequests(
		paths: string[],
		excludes: string[] = [],
	): Promise<string[]> {
		...
		return (
			await this.removeDuplicateRequests(
				requests,
				false /* validate paths skipped for tests */,
			)
...
suite.skip("File Watcher (parcel)", function () {
	...
	setup(async () => {
		...
		testDir = URI.file(
			getRandomTestPath(realpathSync(tmpdir()), "vsctests", "filewatcher"),
		).fsPath;
	...
	test("basics", async function () {
		...
		disposables.add(
			instance.subscribe(newFilePath, (change) =>
				subscriptions1.set(change.resource.fsPath, change.type),
			),
		);
		disposables.add(
			instance.subscribe(newFilePath, (change) =>
				subscriptions2.set(change.resource.fsPath, change.type),
			),
		...
		let changeFuture: Promise<unknown> = awaitEvent(
			watcher,
			newFilePath,
		...
		disposables.add(
			instance.subscribe(newFolderPath, (change) =>
				subscriptions1.set(change.resource.fsPath, change.type),
			),
		);
		const disposable = instance.subscribe(newFolderPath, (change) =>
			subscriptions2.set(change.resource.fsPath, change.type),
		);
		disposable.dispose();
		...
		assert.strictEqual(
			...
			false /* subscription was disposed before the event */,
		);

		// Rename file
		let renamedFilePath = join(testDir, "deep", "renamedFile.txt");
		disposables.add(
			instance.subscribe(renamedFilePath, (change) =>
				subscriptions1.set(change.resource.fsPath, change.type),
			),
		);
		...
		disposables.add(
			instance.subscribe(renamedFolderPath, (change) =>
				subscriptions1.set(change.resource.fsPath, change.type),
			),
		);
		changeFuture = Promise.all([
		...
		disposables.add(
			instance.subscribe(copiedFilepath, (change) =>
				subscriptions1.set(change.resource.fsPath, change.type),
			),
		);
		...
		disposables.add(
			instance.subscribe(copiedFolderpath, (change) =>
				subscriptions1.set(change.resource.fsPath, change.type),
			),
		);
	...
	(isMacintosh /* this test seems not possible with fsevents backend */
		? test.skip
		: test)("basics (atomic writes)", async function () {
	...
	(!isLinux /* polling is only used in linux environments (WSL) */
		? test.skip
		: test)("basics (polling)", async function () {
		await watcher.watch([
			{ path: testDir, excludes: [], pollingInterval: 100, recursive: true },
		]);
	...
	test("subsequent watch updates watchers (excludes)", async function () {
		await watcher.watch([
			{ path: testDir, excludes: [realpathSync(testDir)], recursive: true },
		]);
		await watcher.watch([{ path: testDir, excludes: [], recursive: true }]);
	...
	test("subsequent watch updates watchers (includes)", async function () {
		await watcher.watch([
			{ path: testDir, excludes: [], includes: ["nothing"], recursive: true },
		]);
		await watcher.watch([{ path: testDir, excludes: [], recursive: true }]);
	...
	(!isWindows /* UNC is windows only */ ? test.skip : test)(
		"unc support",
		async function () {
	...
	(isLinux /* linux: is case sensitive */ ? test.skip : test)(
		"wrong casing",
		async function () {
	...
	(isWindows /* flaky on windows */ ? test.skip : test)(
	...
	test("should not exclude roots that do not overlap", async () => {
		if (isWindows) {
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["C:\\a"]),
				["C:\\a"],
			);
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["C:\\a", "C:\\b"]),
				["C:\\a", "C:\\b"],
			);
			assert.deepStrictEqual(
			...
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["/a"]),
				["/a"],
			);
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["/a", "/b"]),
				["/a", "/b"],
			);
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["/a", "/b", "/c/d/e"]),
				["/a", "/b", "/c/d/e"],
			);
		}
	});
	...
	test("should remove sub-folders of other paths", async () => {
		if (isWindows) {
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["C:\\a", "C:\\a\\b"]),
				["C:\\a"],
			);
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests([
			...
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["/a", "/a/b"]),
				["/a"],
			);
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["/a", "/b", "/a/b"]),
				["/a", "/b"],
			);
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["/b/a", "/a", "/b", "/a/b"]),
				["/a", "/b"],
			);
			assert.deepStrictEqual(
				await watcher.testRemoveDuplicateRequests(["/a", "/a/b", "/a/c/d"]),
				["/a"],
			);
		}
	});
	...
	test("deleting watched path emits watcher fail and delete event if correlated", async function () {
		...
		await watcher.watch([
			{ path: folderPath, excludes: [], recursive: true, correlationId: 1 },
		]);
	...
	(!isMacintosh /* Linux/Windows: times out for some reason */
		? test.skip
		: test)(
		"watch requests support suspend/resume (folder, does not exist in beginning, not reusing watcher)",
		async () => {
	...
	async function testWatchFolderDoesNotExist(reuseExistingWatcher: boolean) {
		...
		const request: IRecursiveWatchRequest = {
			path: folderPath,
			excludes: [],
			recursive: true,
		};
	...
	(!isMacintosh /* Linux/Windows: times out for some reason */
		? test.skip
		: test)(
		"watch requests support suspend/resume (folder, exist in beginning, not reusing watcher)",
		async () => {
	...
	async function testWatchFolderExists(reuseExistingWatcher: boolean) {
		...
		const requests: IRecursiveWatchRequest[] = [
			{ path: folderPath, excludes: [], recursive: true },
		];
		if (reuseExistingWatcher) {
			requests.push({ path: testDir, excludes: [], recursive: true });
		}
	...
	(isLinux ? test.skip : test)(
		"includes are case insensitive on Windows/Mac",
		async function () {
			await watcher.watch([
				{
					path: testDir,
	...
	(isLinux ? test.skip : test)(
		"excludes are case insensitive on Windows/Mac",
		async function () {
			await watcher.watch([
				{ path: testDir, excludes: ["**/DEEP/**"], recursive: true },
			]);
