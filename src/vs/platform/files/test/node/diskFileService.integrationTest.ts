...
flakySuite("Disk File Service", function () {
	...
	test("resolve - directory - with metadata", async () => {
		...
		const result = await service.resolve(
			FileAccess.asFileUri("vs/platform/files/test/node/fixtures/resolver"),
	...
	async function testResolveDirectoryWithTarget(
	...
	if (!isWindows) {
		test("stat - executable", async () => {
			const nonExecutable = FileAccess.asFileUri(
				"vs/platform/files/test/node/fixtures/executable/non_executable",
			);
			let resolved = await service.stat(nonExecutable);
			assert.strictEqual(resolved.isFile, true);
			assert.strictEqual(resolved.executable, false);

			const executable = FileAccess.asFileUri(
				"vs/platform/files/test/node/fixtures/executable/executable",
			);
	...
	(isLinux /* trash is unreliable on Linux */ ? test.skip : test)(
		"deleteFile (useTrash)",
		async () => {
	...
	async function testDeleteFile(
		...
		assert.strictEqual(
			await service.canDelete(source.resource, { useTrash, recursive }),
			true,
		);
	...
	async function testMoveAcrossProviders(
		...
		const target = URI.file(join(dirname(source.fsPath), "other.html")).with({
			scheme: testSchema,
		});
	...
	async function testMoveFolderAcrossProviders(): Promise<void> {
		...
		const target = URI.file(join(dirname(source.fsPath), "deeper")).with({
			scheme: testSchema,
		});
	...
	test("move - same file", async () => {
		...
		assert.strictEqual(
			await service.canMove(source.resource, URI.file(source.resource.fsPath)),
			true,
		);
	...
	test("copy - same file", async () => {
		...
		assert.strictEqual(
			await service.canCopy(source.resource, URI.file(source.resource.fsPath)),
			true,
		);
	...
	async function testCloneFile(): Promise<void> {
		...
		const source1Size = (
			await service.resolve(source1, { resolveMetadata: true })
		).size;

		const source2 = URI.file(join(testDir, "lorem.txt"));
		const source2Size = (
			await service.resolve(source2, { resolveMetadata: true })
		).size;
		...
		let target1Size = (
			await service.resolve(target1, { resolveMetadata: true })
		).size;
		...
		target1Size = (await service.resolve(target1, { resolveMetadata: true }))
			.size;
		...
		const target2Size = (
			await service.resolve(target2, { resolveMetadata: true })
		).size;
	...
	(isWindows /* error code does not seem to be supported on windows */
		? test.skip
	...
	test("createFile (readable)", async () => {
		return assertCreateFile((contents) =>
			...
			["0", "00", "000", "0000", "00000"].map(async (offset) => {
				...
				try {
					await provider.write(
						fd,
						0,
						VSBuffer.fromString(content).buffer,
						0,
	...
	test("provider - atomic reads (read pending when write starts)", async () => {
		...
		const fdPromise = provider
			.open(resource, { create: true, unlock: false })
			.then(async (fd) => {
				try {
	...
	test("writeFile - error when writing to file that has been updated meanwhile", async () => {
		...
		try {
			await service.writeFile(
				...
				{ etag: etag({ mtime: fakeMtime, size: fakeSize }), mtime: fakeMtime },
			);
		} catch (err) {
			error = err;
		}
	...
	async function testAppendFile() {
		...
		await service.writeFile(resource, VSBuffer.fromString(appendContent), {
			append: true,
		});
	...
	async function testAppendFileReadable() {
		...
		await service.writeFile(
			resource,
			bufferToReadable(VSBuffer.fromString(appendContent)),
			{ append: true },
		);

		assert.strictEqual(
			readFileSync(resource.fsPath).toString(),
			"Small File - Appended via readable!",
		);
	}
	...
	async function testAppendFileStream() {
		...
		await service.writeFile(
			resource,
			bufferToStream(VSBuffer.fromString(appendContent)),
			{ append: true },
		);

		assert.strictEqual(
			readFileSync(resource.fsPath).toString(),
			"Small File - Appended via stream!",
		);
	}
	...
	async function testAppendFileCreatesFile() {
		...
		await service.writeFile(resource, VSBuffer.fromString(content), {
			append: true,
		});
	...
	test("read - mixed positions", async () => {
		...
		await fileProvider.read(
			fd,
			98 /* no longer in sequence of posInFile */,
			buffer.buffer,
			0,
			9,
		);
