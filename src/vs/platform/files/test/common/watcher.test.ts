...
suite("Watcher", () => {
	(isWindows ? test.skip : test)("parseWatcherPatterns - posix", () => {
		...
		parsedPattern = parseWatcherPatterns(
			path,
			["/users/data/src/*.js"],
			false,
		)[0];
	...
	(isWindows ? test.skip : test)(
		"parseWatcherPatterns - posix (case insensitive)",
		() => {
			const path = "/users/data/src";
			let parsedPattern = parseWatcherPatterns(path, ["*.JS"], false)[0];
	...
	(!isWindows ? test.skip : test)(
		"parseWatcherPatterns - windows (case insensitive)",
		() => {
			const path = "c:\\users\\data\\src";
			let parsedPattern = parseWatcherPatterns(path, ["*.JS"], true)[0];
			...
			assert.strictEqual(parsedPattern("c:\\users\\data\\src\\TEST1.JS"), true);
			assert.strictEqual(
				parsedPattern("c:\\users\\data\\src\\bar\\test2.js"),
				true,
			);
			assert.strictEqual(
				parsedPattern("c:\\users\\data\\src\\bar\\TEST2.JS"),
				true,
			);
			assert.strictEqual(parsedPattern("c:\\users\\data\\src\\foo.js"), false);
...
suite("Watcher Events Normalizer", () => {
	...
	test("event type filter", () => {
		...
		assert.strictEqual(
			isFiltered({ resource, type: FileChangeType.ADDED }, undefined),
			false,
		);
		assert.strictEqual(
			isFiltered({ resource, type: FileChangeType.UPDATED }, undefined),
			false,
		);
		assert.strictEqual(
			isFiltered({ resource, type: FileChangeType.DELETED }, undefined),
			false,
		);
