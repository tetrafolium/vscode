...
export interface IFileService {
	...
	createWatcher(
		resource: URI,
		options: IWatchOptionsWithoutCorrelation & { recursive: false },
	): IFileSystemWatcher;
...
export interface IFileWriteOptions
	extends IFileOverwriteOptions, IFileUnlockOptions, IFileAtomicWriteOptions {
	/**
...
export function hasFileAppendCapability(
	provider: IFileSystemProvider,
): boolean {
...
export function hasFileRealpathCapability(
	...
	return !!(
		provider.capabilities & FileSystemProviderCapabilities.FileRealpath
	);
}
...
export interface IFileStatWithMetadata
	extends IFileStat, IBaseFileStatWithMetadata {
	readonly mtime: number;
	readonly ctime: number;
	readonly etag: string;
	readonly size: number;
