...
>(
	...
	connectionFactory: (simpleOptions: ISimpleConnectionOptions<O>) => Promise<T>,
): Promise<T> {
...
export abstract class PersistentConnection extends Disposable {
	...
	private _reconnectionGraceTime: number =
	...
	private async _runReconnectingLoop(): Promise<void> {
		...
		do {
			attempt++;
			const waitTime =
				attempt < TIMES.length ? TIMES[attempt] : TIMES[TIMES.length - 1];
			try {
				if (waitTime > 0) {
