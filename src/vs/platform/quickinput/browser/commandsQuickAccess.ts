	...
	implements IDisposable
{
	...
	protected async _getPicks(
		...
		const runTfidf = createSingleCallFunction(() => {
			...
			return normalizeTfIdfScores(result)
				.filter(
					(score) =>
						score.score > AbstractCommandsQuickAccessProvider.TFIDF_THRESHOLD,
				)
		...
		filteredCommandPicks.sort((commandPickA, commandPickB) => {
			...
			const isDeveloperA =
				commandPickA.commandCategory === Categories.Developer.value;
			const isDeveloperB =
				commandPickB.commandCategory === Categories.Developer.value;
		...
		for (let i = 0; i < filteredCommandPicks.length; i++) {
			...
			// Command
			commandPicks.push(
				this.toCommandPick(commandPick, runOptions, isInHistory),
			);
		}
	...
	private toCommandPick(
		...
		const tooltip =
			commandPick.tooltip ?? commandPick.commandDescription?.value;
...
export class CommandsHistory extends Disposable {
	...
	static getConfiguredCommandHistoryLength(
		...
		const config =
			configurationService.getValue<ICommandsQuickAccessConfiguration>();
