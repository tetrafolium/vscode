...
export class QuickTree<T extends IQuickTreeItem>
	extends QuickInput
	implements IQuickTree<T>
{
	...
	private readonly _ariaLabel = observableValue<string | undefined>(
		"ariaLabel",
		undefined,
	);
	private readonly _placeholder = observableValue<string | undefined>(
		"placeholder",
		undefined,
	);
	private readonly _matchOnDescription = observableValue(
		"matchOnDescription",
		false,
	);
	...
	private readonly _activeItems = observableValue<readonly T[]>(
		"activeItems",
		[],
	);
	private readonly _itemTree = observableValue<ReadonlyArray<T>>(
		"itemTree",
		[],
	);

	readonly onDidChangeValue = Event.fromObservable(this._value, this._store);
	readonly onDidChangeActive = Event.fromObservable(
		this._activeItems,
		this._store,
	);

	private readonly _onDidChangeCheckedLeafItems = this._register(
		new Emitter<T[]>(),
	);
	readonly onDidChangeCheckedLeafItems: Event<T[]> =
		this._onDidChangeCheckedLeafItems.event;
	...
	constructor(ui: QuickInputUI) {
		super(ui);
		this.onDidAccept = Event.any(
			ui.onDidAccept,
			this._onDidAcceptEmitter.event,
		);
		this._registerAutoruns();
		this._register(
			ui.tree.onDidChangeCheckedLeafItems((e) =>
				this._onDidChangeCheckedLeafItems.fire(e as T[]),
			),
		);
		this._register(
			ui.tree.onDidChangeCheckboxState((e) =>
				this._onDidChangeCheckboxState.fire(e.item as T),
			),
		);
	...
	get onDidTriggerItemButton(): Event<IQuickTreeItemButtonEvent<T>> {
		...
		return this.ui.tree.onDidTriggerButton as Event<
			IQuickTreeItemButtonEvent<T>
		>;
	}
		...
		if (!this.visible) {
			...
			this.visibleDisposables.add(
				this.ui.tree.onDidChangeCheckboxState((e) => {
					const checkAllState = getParentNodeState([
						...this.ui.tree.tree.getNode().children,
					]);
