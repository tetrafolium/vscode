...
export class QuickInputCheckboxStateHandler<T> extends Disposable {
	private readonly _onDidChangeCheckboxState = this._register(
		new Emitter<{ item: T; checked: boolean | "mixed" }>(),
	);
	public readonly onDidChangeCheckboxState =
	...
	implements ITreeRenderer<T, IQuickTreeFilterData, IQuickTreeTemplateData>
{
	 ...
	 */
	public readonly onDidDisposeFocusedElement =
		this._onDidDisposeFocusedElement.event;

	constructor(
		private readonly _hoverDelegate: IHoverDelegate | undefined,
		private readonly _buttonTriggeredEmitter: Emitter<
	...
	renderTemplate(container: HTMLElement): IQuickTreeTemplateData {
		...
		const checkbox = store.add(
			new TriStateCheckbox("", false, { ...defaultCheckboxStyles, size: 15 }),
		);
	...
	renderElement(
		...
		if (quickTreeItem.pickable === false) {
			...
			store.add(
				checkbox.onChange((e) =>
					this._checkboxStateHandler.setCheckboxState(
						quickTreeItem,
						checkbox.checked,
					),
				),
		...
		if (quickTreeItem.iconPath) {
			...
			templateData.icon.className = quickTreeItem.iconClass
				? `quick-input-tree-icon ${quickTreeItem.iconClass}`
				: "";
		}

		const {
			labelHighlights: matches,
			descriptionHighlights: descriptionMatches,
		} = node.filterData || {};
