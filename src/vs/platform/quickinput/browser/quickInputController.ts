...
export class QuickInputController extends Disposable {
	...
	private keyMods: Writeable<IKeyMods> = {
		ctrlCmd: false,
		alt: false,
		shift: false,
	};
	...
	private readonly _alignment = observableValue<QuickInputAlignment>(
		this,
		"top",
	);
	...
	constructor(
		...
		this.quickInputTypeContext =
			QuickInputTypeContextKey.bindTo(contextKeyService);
		this.endOfQuickInputBoxContext =
			EndOfQuickInputBoxContextKey.bindTo(contextKeyService);
	...
	private getUI(showInActiveContainer?: boolean): QuickInputUI {
		...
		const leftActionBar = this._register(
			new ToolBar(titleBar, this.contextMenuService, {
				...
				actionViewItemProvider: createToggleActionViewItemProvider(
					this.styles.toggle,
				),
				icon: true,
				label: false,
		...
		this._register(
			tree.tree.onDidChangeContentHeight(() => this.updateLayout()),
		);
		...
		this._register(
			dom.addDisposableListener(
				...
				(e) => {
					const ui = this.getUI();
					if (
						dom.isAncestor(e.relatedTarget as HTMLElement, ui.inputContainer)
					) {
				...
				[
					{
						...
						excludeNodes: [
							leftActionBar.getElement(),
							rightActionBar.getElement(),
						],
					},
					{
		...
		this._register(
			autorun((reader) => {
				this._alignment.set(
					this.dndController!.alignment.read(reader),
					undefined,
				);
			}),
		);
			...
			const disposables = [
				...
				input.onDidAccept(() => {
					...
					validation.then((result) => {
						if (
							!result ||
							(!isString(result) && result.severity !== Severity.Error)
						) {
							resolve(value);
							input.hide();
	...
	private show(controller: IQuickInput) {
		...
		if (controller.anchor) {
			...
			this._alignment.set(
				this.dndController?.alignment.get() ?? "top",
				undefined,
			);
		}
	...
	async accept(
		keyMods: IKeyMods = { alt: false, ctrlCmd: false, shift: false },
	) {
	...
	private updateLayout() {
		if (this.ui && this.isVisible()) {
			...
			if (this.controller?.anchor) {
				...
				const anchorWindow = isElement
					? dom.getWindow(target)
					: dom.getActiveWindow();
				const container = this.layoutService
					.getContainer(anchorWindow)
					.getBoundingClientRect();
				let anchor = getAnchorRect(target);

				listHeight = this.dimension
					? Math.min(this.dimension.height * 0.2, 200)
					: 200;
				...
				const { top, left, right, bottom, anchorAlignment, anchorPosition } =
					layout2d(container, { width, height: containerHeight }, anchor, {
...
class QuickInputDragAndDropController extends Disposable {
	readonly dndViewState = observableValue<
		{ top?: number; left?: number; done: boolean } | undefined
	>(this, undefined);
	...
	private readonly _quickInputAlignmentContext: IContextKey<
	...
	constructor(
		...
		super();
		this._quickInputAlignmentContext =
			QuickInputAlignmentContextKey.bindTo(contextKeyService);
		...
		if (
			initialViewState?.top !== undefined &&
			initialViewState?.left !== undefined
		) {
	...
	layoutContainer(
		dimension = this._layoutService.activeContainerDimension,
	): void {
		if (!this._enabled) {
			return;
		}
			...
			this.dndViewState.set(
				{
					...
					left:
						(this._getCenterXSnapValue() +
							this._quickInputContainer.clientWidth / 2) /
	...
	private registerMouseListeners(): void {
		...
		this._register(
			dom.addDisposableGenericMouseUpListener(dragArea, (event: MouseEvent) => {
				...
				this.dndViewState.set(
					{ top: undefined, left: undefined, done: true },
					undefined,
				);
				this._setAlignmentState("top");
			}),
		...
		this._register(
			dom.addDisposableGenericMouseDownListener(dragArea, (e: MouseEvent) => {
				...
				const dragAreaRect = this._quickInputContainer.getBoundingClientRect();
				const dragOffsetX =
					originEvent.browserEvent.clientX - dragAreaRect.left;
				...
				const mouseUpListener = dom.addDisposableGenericMouseUpListener(
					...
					(e: MouseEvent) => {
						if (isMovingQuickInput) {
							...
							const state = this.dndViewState.get();
							this.dndViewState.set(
								{ top: state?.top, left: state?.left, done: true },
								undefined,
							);
						}
	...
	private _layout(topCoordinate: number, leftCoordinate: number) {
		...
		const snappingToTop =
			Math.abs(topCoordinate - snapCoordinateYTop) < this._snapThreshold;
		topCoordinate = snappingToTop ? snapCoordinateYTop : topCoordinate;
		const snappingToCenter =
			Math.abs(topCoordinate - snapCoordinateY) < this._snapThreshold;
		...
		const snappingToCenterX =
			Math.abs(leftCoordinate - snapCoordinateX) < this._snapThreshold;
	...
	private _getCenterXSnapValue() {
		return (
			Math.round(this._container.clientWidth / 2) -
