...
function focusHandler(
	...
	return (accessor) => {
		...
		if (
			focusOnQuickNatigate &&
			(currentQuickPick as IQuickPick<any>).quickNavigate
		) {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
	...
	handler: (accessor) => {
		const currentQuickPick = accessor.get(IQuickInputService)
			.currentQuickInput as IQuickPick<any> | IQuickTree<any> | IInputBox;
...
registerQuickInputCommandAndKeybindingRule(
	{
		...
		handler: (accessor) => {
			const currentQuickPick =
				accessor.get(IQuickInputService).currentQuickInput;
