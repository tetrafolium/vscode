...
async function setupWaitTilShownListener(
...
suite("QuickInput", () => {
	...
	test("keepScrollPosition - works with activeItems", async () => {
		const quickpick = store.add(
			controller.createQuickPick() as QuickPick<IQuickPickItem>,
		);
	...
	test("activeItems - verify onDidChangeActive is triggered after setting items", async () => {
		...
		store.add(
			quickpick.onDidChangeActive((items) =>
				activeItemsFromEvent.push(...items),
			),
		);
	...
	test("isKeyModified - returns false when no modifiers are pressed", () => {
		...
		assert.strictEqual(
			isKeyModified({ ctrlCmd: false, alt: false, shift: false }),
			false,
		);
	});
	...
	test("isKeyModified - returns true when any modifier is pressed", () => {
		assert.strictEqual(
			isKeyModified({ ctrlCmd: true, alt: false, shift: false }),
			true,
		);
		assert.strictEqual(
			isKeyModified({ ctrlCmd: false, alt: true, shift: false }),
			true,
		);
		assert.strictEqual(
			isKeyModified({ ctrlCmd: false, alt: false, shift: true }),
			true,
		);
		assert.strictEqual(
			isKeyModified({ ctrlCmd: true, alt: true, shift: true }),
			true,
		);
	});
});
