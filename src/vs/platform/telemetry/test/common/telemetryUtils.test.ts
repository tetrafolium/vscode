...
suite("TelemetryUtils", () => {
	...
	suite("cleanRemoteAuthority", () => {
		...
		test("returns remoteName when it exists in remoteExtensionTips", () => {
			...
			assert.strictEqual(
				cleanRemoteAuthority("ssh-remote", config),
				"ssh-remote",
			);
			assert.strictEqual(
				cleanRemoteAuthority("dev-container", config),
				"dev-container",
			);
		...
		test("returns remoteName when it exists in virtualWorkspaceExtensionTips", () => {
			...
			assert.strictEqual(
				cleanRemoteAuthority("codespaces", config),
				"codespaces",
			);
		...
		test('returns "other" when remoteName is not in either config', () => {
			...
			assert.strictEqual(
				cleanRemoteAuthority("unknown-remote", config),
				"other",
			);
			assert.strictEqual(
				cleanRemoteAuthority("custom-remote", config),
				"other",
			);
		});
		...
		test("handles remoteAuthority with additional path segments", () => {
			...
			assert.strictEqual(
				cleanRemoteAuthority("ssh-remote+server1.example.com", config),
				"ssh-remote",
			);
		});
