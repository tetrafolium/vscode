...
suite("pluginParsers", () => {
	...
	suite("resolveComponentDirs", () => {
		...
		test("includes default directory when not exclusive", () => {
			const dirs = resolveComponentDirs(pluginUri, "skills", {
				paths: [],
				exclusive: false,
			});
		...
		test("allows paths that escape plugin root but stay within boundaryUri", () => {
			...
			const dirs = resolveComponentDirs(
				pluginUri,
				"skills",
				{ paths: ["../shared-skills"], exclusive: false },
				boundaryUri,
			);
			assert.strictEqual(dirs.length, 2);
	...
	suite("convertBareEnvVarsToVsCodeSyntax", () => {
		test("converts bare env vars to VS Code syntax", () => {
			...
			assert.strictEqual(
				(result.configuration as { command: string }).command,
				"${env:MY_TOOL}",
			);
			assert.deepStrictEqual(
				(result.configuration as unknown as { args: string[] }).args,
				["--key=${env:API_KEY}"],
			);
		});
