...
export class Menubar extends Disposable {
	...
	constructor(
		...
		this.menuUpdater = this._register(
			new RunOnceScheduler(() => this.doUpdateMenu(), 0),
		);

		this.menuGC = this._register(
	...
	private registerListeners(): void {
		...
		this._register(
			this.updateService.onStateChange(() => this.scheduleUpdateMenu()),
		);
	}
				...
				this.windowsMainService.getFocusedWindow()?.id,
				{
					...
					buttons: [
						isMacintosh
							...
							: nls.localize(
									{ key: "exit", comment: ["&& denotes a mnemonic"] },
									"&&Exit",
								),
						nls.localize("cancel", "Cancel"),
					],
					message: isMacintosh
						? nls.localize("quitMessageMac", "Are you sure you want to quit?")
						: nls.localize("quitMessage", "Are you sure you want to exit?"),
				},
			);
