	...
	implements INativeServerExtensionManagementService
{
	...
	private async downloadVsix(
		...
		await this.downloadService.download(
			vsix,
			location,
			"extensionManagement.downloadVsix",
		);
	...
	private async downloadAndExtractGalleryExtension(
		...
		const { verificationStatus, location } = await this.downloadExtension(
			...
			options.context?.[EXTENSION_INSTALL_CLIENT_TARGET_PLATFORM_CONTEXT] as
				| TargetPlatform
				| undefined,
		);
		try {
...
export class ExtensionsScanner extends Disposable {
	...
	private async removeStaleAutoUpdateBuiltinExtensions(): Promise<void> {
		...
		const builtinExtensions =
			await this.extensionsScannerService.scanSystemExtensions({});
		...
		const staleExtensions = userExtensions.filter((userExtension) => {
			if (
				!this.productService.builtInExtensionsEnabledWithAutoUpdates.some(
					(id) =>
						id.toLowerCase() === userExtension.identifier.id.toLowerCase(),
				)
			) {
				return false;
			}
			const builtinExtension = builtinExtensions.find((e) =>
				areSameExtensions(e.identifier, userExtension.identifier),
			);
			return (
				builtinExtension &&
				semver.lt(
					userExtension.manifest.version,
					builtinExtension.manifest.version,
				)
			);
		});
		if (staleExtensions.length) {
			this.logService.info(
				"Removing stale auto-update builtin extensions:",
				staleExtensions
					.map((e) => `${e.identifier.id}@${e.manifest.version}`)
					.join(", "),
			);
			await this.extensionsProfileScannerService.removeExtensionsFromProfile(
				staleExtensions.map((e) => e.identifier),
				this.userDataProfilesService.defaultProfile.extensionsResource,
			);
			await Promise.allSettled(
				staleExtensions.map((e) =>
					this.deleteExtension(e, "stale auto-update builtin"),
				),
			);
		}
	}
	...
	implements IInstallExtensionTask
{
	...
	protected async doRun(token: CancellationToken): Promise<ILocalExtension> {
		...
		const system = await this.extensionsScanner.scanExtensions(
			ExtensionType.System,
			this.options.profileLocation,
			this.options.productVersion,
		);
		const existingSystemExtension = system.find((i) =>
			areSameExtensions(i.identifier, this.identifier),
		);
		if (existingSystemExtension) {
			if (!existingSystemExtension.forceAutoUpdate) {
				throw new ExtensionManagementError(
					nls.localize(
						...
						existingSystemExtension.identifier.id,
			...
			if (
				semver.gt(
					existingSystemExtension.manifest.version,
