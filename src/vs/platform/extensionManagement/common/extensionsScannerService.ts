...
function getProductBuiltInExtensionsEnabledWithAutoUpdates(
	...
	for (const id of productService.builtInExtensionsEnabledWithAutoUpdates) {
		...
		if (
			environmentService.skipBuiltinExtensions?.some(
				(skipId) => skipId.toLowerCase() === toLowerCaseId,
			)
		) {
			continue;
		}
	...
	implements IExtensionsScannerService
{
	...
	async scanSystemExtensions(
		...
		if (this.environmentService.skipBuiltinExtensions?.length) {
			const skipSet = new Set(
				this.environmentService.skipBuiltinExtensions.map((id) =>
					id.toLowerCase(),
				),
			);
			allSystemExtensions = allSystemExtensions.filter(
				(ext) => !skipSet.has(ext.identifier.id.toLowerCase()),
			);
		}
	...
	private dedupExtensions(
		...
		const productBuiltInExtensionsEnabledWithAutoUpdates =
			getProductBuiltInExtensionsEnabledWithAutoUpdates(
				this.productService,
		...
		user?.forEach((extension) => {
			...
			if (
				productBuiltInExtensionsEnabledWithAutoUpdates.has(
					extension.identifier.id.toLowerCase(),
				) &&
				!extension.forceAutoUpdate
			) {
				this.logService.info(
...
class ExtensionsScanner extends Disposable {
	...
	constructor(
		...
		this.productBuiltInExtensionsEnabledWithAutoUpdates =
			getProductBuiltInExtensionsEnabledWithAutoUpdates(
				productService,
				environmentService,
			);
	...
	async scanExtension(
		...
		let extension: IRelaxedScannedExtension = {
			...
			forceAutoUpdate:
				this.productBuiltInExtensionsEnabledWithAutoUpdates.has(
					id.toLowerCase(),
				) && this.productQuality === "stable",
		};
		if (input.validate) {
