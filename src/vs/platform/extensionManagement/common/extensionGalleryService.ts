...
export abstract class AbstractExtensionGalleryService implements IExtensionGalleryService {
			...
			toFetchLatest.map(async (extensionInfo) => {
				...
				try {
					...
					>("galleryService:fallbacktoquery", {
						...
						compatible: !!options.compatible,
						errorCode:
							error instanceof ExtensionGalleryError ? error.code : "Unknown",
					});
					toQuery.push(extensionInfo);
				}
			}),
	...
	private async getLatestGalleryExtension(
		...
		const rawGalleryExtension =
			await this.getLatestRawGalleryExtensionWithFallback(
				extensionInfo,
				resourceApi,
				token,
			);
		...
		const rawGalleryExtensionVersion =
			await this.getValidRawGalleryExtensionVersionFromLatestVersions(
				rawGalleryExtension,
		...
		if (!rawGalleryExtensionVersion) {
			if (extensionInfo.currentVersion) {
				const latestVersion =
					rawGalleryExtension.versions.length > 0
						? rawGalleryExtension.versions[0].version
						: undefined;
				if (
					latestVersion &&
					semver.lt(latestVersion, extensionInfo.currentVersion)
				) {
	...
	private async getValidRawGalleryExtensionVersionFromLatestVersions(
		...
		const latestExtensionVersionsForTargetPlatform =
		...
		if (prereleaseVersion && releaseVersion) {
			return semver.gt(releaseVersion.version, prereleaseVersion.version)
				? releaseVersion
				: prereleaseVersion;
		}
		...
		if (options.compatible) {
			...
			if (releaseVersion) {
				...
				const anyPrereleaseVersion =
					await this.getValidRawGalleryExtensionVersion(
						rawGalleryExtension,
						latestExtensionVersionsForTargetPlatform,
						{
							targetPlatform,
				...
				if (
					!anyPrereleaseVersion ||
					semver.gt(releaseVersion.version, anyPrereleaseVersion.version)
				) {
	...
	private async isValidVersion(
		...
		},
		{
			...
			productVersion,
			version,
		}: Omit<ExtensionVersionCriteria, "targetPlatform"> & {
	...
	private async isEngineValid(
		...
		if (!engine) {
			this.logService.error(
				`Missing engine for the extension ${extensionId} with version ${version}`,
			);
			return false;
		}
	...
	private async getLatestRawGalleryExtensionWithFallback(
		...
		try {
			...
			return await this.getLatestRawGalleryExtension(
				extensionInfo.id,
				uri,
				token,
			);
	...
	private async getLatestRawGalleryExtension(
		...
		try {
			...
			>("galleryService:getLatest", {
				...
				errorCode,
				statusCode:
					context?.res.statusCode && context?.res.statusCode !== 200
						? `${context.res.statusCode}`
						: undefined,
				server: this.getHeaderValue(context?.res.headers, SERVER_HEADER_NAME),
				activityId: this.getHeaderValue(
	...
	async reportStatistic(
		...
		if (isWeb) {
			this.logService.info(
				"ExtensionGalleryService#reportStatistic: Skipped in web",
			);
			return undefined;
		}
	...
	async download(
		...
		const headers: IHeaders | undefined =
			activityId && typeof activityId === "string"
				? { [SEARCH_ACTIVITY_HEADER_NAME]: activityId }
				: undefined;
		const context = await this.getAsset(
			extension.identifier.id,
			downloadAsset,
	...
	private getRequestTimeout(): number {
		const configuredTimeout = this.configurationService.getValue<number>(
			ExtensionRequestsTimeoutConfigKey,
		);
		return isNumber(configuredTimeout) && configuredTimeout >= 0
			? configuredTimeout
			: 60_000;
	}
}
