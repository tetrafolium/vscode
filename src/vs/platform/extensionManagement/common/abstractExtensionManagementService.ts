...
export abstract class CommontExtensionManagementService
	extends Disposable
	implements IExtensionManagementService
{
	...
	protected async isExtensionPlatformCompatible(
		extension: IGalleryExtension,
	): Promise<boolean> {
	...
	abstract installGalleryExtensions(
	...
	abstract uninstallExtensions(
		extensions: UninstallExtensionInfo[],
	): Promise<void>;
	abstract toggleApplicationScope(
		extension: ILocalExtension,
	...
	abstract registerParticipant(
		pariticipant: IExtensionManagementParticipant,
	): void;
	...
	implements IExtensionManagementService
{
	...
	protected async installExtensions(
					...
					if (
						...
						const waitForInstallation = Event.toPromise(
							Event.filter(this.onDidInstallExtensions, (results) =>
								results.some((result) =>
									areSameExtensions(result.identifier, identifier),
								),
							),
						).then((results) => {
							this.logService.info(
		...
		try {
			...
			for (const { manifest, extension, options } of extensions) {
				...
				const isSystemExtension = systemExtensions.some((e) =>
				...
				if (existingInstallExtensionTask) {
					...
					const waitForInstallation = existingTask
					...
					if (task.options.donotIncludePackAndDependencies) {
						...
						try {
							...
							if (task.options.installPreReleaseVersion) {
								...
								task.source.hasPreReleaseVersion
							) {
							...
							for (const { gallery, manifest } of distinct(
								...
								if (
									existing &&
									existing.isApplicationScoped === !!options.isApplicationScoped
								) {
									continue;
								}
			...
			await this.joinAllSettled(
				[...installingExtensionsMap.entries()].map(
					async ([key, { task, uninstallTaskToWaitFor }]) => {
		...
		const collectDependenciesAndPackExtensionsToInstall = async (
			...
			const dependecies: string[] = manifest.extensionDependencies
				? manifest.extensionDependencies.filter(
						(dep) =>
							!installed.some((e) =>
								areSameExtensions(e.identifier, { id: dep }),
							),
					)
				: [];
					...
					if (
						...
						if (
							dependenciesAndPackExtensions.every(
								(e) => !areSameExtensions({ id: e }, { id: extension }),
							)
						) {
				...
				if (ids.length) {
					...
					for (const galleryExtension of galleryExtensions) {
						...
						const isDependency = dependecies.some((id) =>
							areSameExtensions({ id }, galleryExtension.identifier),
						);
						let compatible;
						try {
	...
	private async checkAndGetCompatibleVersion(
		...
		if (
			isMalicious(extension.identifier, extensionsControlManifest.malicious)
		) {
			throw new ExtensionManagementError(
				nls.localize(
					"malicious extension",
	...
	async uninstallExtensions(
		...
		const createUninstallExtensionTask = (
			...
			const task = this.createUninstallExtensionTask(
				extension,
				uninstallOptions,
			);
			this.uninstallingExtensions.set(
				getUninstallExtensionTaskKey(task.extension, uninstallOptions),
				task,
			);
			this.logService.info(
		...
		const allTasks: {
			task: IUninstallExtensionTask;
			installTaskToWaitFor?: IInstallExtensionTask;
		}[] = [];
		...
		for (const { extension, options } of extensions) {
			...
			if (uninstallOptions.remove || extension.isApplicationScoped) {
				...
				for (const profile of this.userDataProfilesService.profiles) {
					...
					const profileExtension = installed.find((e) =>
		...
		try {
					...
					for (const packedExtension of packedExtensions) {
						if (
							this.uninstallingExtensions.has(
								getUninstallExtensionTaskKey(packedExtension, task.options),
							)
						) {
							this.logService.info(
								"Extensions is already requested to uninstall",
			...
			if (extensionsToRemove.length) {
				await this.joinAllSettled(
					extensionsToRemove.map((extension) =>
			...
			for (const { task } of allTasks) {
				if (
					!this.uninstallingExtensions.delete(
						getUninstallExtensionTaskKey(task.extension, task.options),
					)
				) {
					this.logService.warn(
				...
				const remainingDependents = dependents.filter(
					(dependent) =>
						!extensionsToUninstall.some((e) =>
							areSameExtensions(e.identifier, dependent.identifier),
						),
				);
	...
	protected abstract removeExtension(
		extension: ILocalExtension,
		fromProfileLocation: URI,
	): Promise<void>;
