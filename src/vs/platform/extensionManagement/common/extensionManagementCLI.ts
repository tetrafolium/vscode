...
export class ExtensionManagementCLI {
	constructor(
		...
		this.extensionsForceVersionByQuality =
			this.extensionsForceVersionByQuality.map((e) => e.toLowerCase());
	}
	...
	public async installExtensions(
		...
		try {
			...
			const addInstallExtensionInfo = (
				...
				if (
					this.extensionsForceVersionByQuality?.some(
						(e) => e === id.toLowerCase(),
					)
				) {
					version =
						this.productService.quality !== "stable" ? "prerelease" : undefined;
				}
			...
			(installExtensionInfo) => {
				...
				if (installedExtension) {
					const builtinAutoUpdateMessage =
						this.validateBuiltinExtensionEnabledWithAutoUpdates(
							installedExtension,
						);
	...
	private async validateVSIX(
		...
		const existingExtension = installedExtensions.find((local) =>
			areSameExtensions(extensionIdentifier, local.identifier),
		);

		if (existingExtension) {
			const builtinAutoUpdateMessage =
				this.validateBuiltinExtensionEnabledWithAutoUpdates(existingExtension);
			if (builtinAutoUpdateMessage) {
				this.logger.info(builtinAutoUpdateMessage);
				return false;
			}
	...
	private validateBuiltinExtensionEnabledWithAutoUpdates(
		...
		if (
			...
			this.productService.builtInExtensionsEnabledWithAutoUpdates.some(
				(e) => e.toLowerCase() === extension.identifier.id.toLowerCase(),
			) &&
