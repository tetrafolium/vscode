...
function isIExtension(extension: unknown): extension is IExtension {
	return (
		(extension as IExtension).type === ExtensionType.User ||
		(extension as IExtension).type === ExtensionType.System
	);
}
	...
	implements IAllowedExtensionsService
{
	...
	constructor(
		...
		super();
		this.publisherOrgs =
			productService.extensionPublisherOrgs?.map((p) => p.toLowerCase()) ?? [];
	...
	isAllowed(
		...
		const publisherKey =
			publisherDisplayName && this.publisherOrgs.includes(publisherDisplayName)
