...
export const EXTENSION_INSTALL_SKIP_PUBLISHER_TRUST_CONTEXT =
	"skipPublisherTrust";
...
export type AllowedExtensionsConfigValueType = IStringDictionary<
	boolean | string | string[]
>;

export const IAllowedExtensionsService =
	createDecorator<IAllowedExtensionsService>("IAllowedExtensionsService");
export interface IAllowedExtensionsService {
	readonly _serviceBrand: undefined;

	readonly allowedExtensionsConfigValue:
...
export function shouldRequireRepositorySignatureFor(
	...
	if (isPrivate) {
		return (
			galleryManifest?.capabilities.signing?.allPrivateRepositorySigned === true
		);
	}
	return (
		galleryManifest?.capabilities.signing?.allPublicRepositorySigned === true
	);
}
