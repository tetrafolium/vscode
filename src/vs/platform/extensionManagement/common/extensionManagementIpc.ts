	...
	listen(context: any, event: string): Event<any> {
		...
		switch (event) {
			...
			case "onUninstallExtension": {
				return Event.map<UninstallExtensionEvent, UninstallExtensionEvent>(
	...
	implements IExtensionManagementService
{
	...
	constructor(
		...
		this._register(
			this.channel.listen<UninstallExtensionEvent>("onUninstallExtension")(
				(e) =>
	...
	toggleApplicationScope(
		...
		return this.channel
			.call<ILocalExtension>("toggleApplicationScope", [
				local,
				fromProfileLocation,
			])
			.then((extension) => transformIncomingExtension(extension, null));
