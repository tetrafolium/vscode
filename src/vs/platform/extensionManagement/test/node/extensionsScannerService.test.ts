	...
	suite("auto update builtin extensions", () => {
		test("scan user extension with matching product version is included", async () => {
			instantiationService.stub(IProductService, {
				version: "1.66.0",
				quality: "stable",
				builtInExtensionsEnabledWithAutoUpdates: ["pub.name"],
			});
			await aUserExtension(
				anExtensionManifest({
					name: "name",
			...
			const actual = await testObject.scanUserExtensions({
				profileLocation: instantiationService.get(IUserDataProfilesService)
		...
		test("scan user extension not in autoUpdateBuiltinExtensions is not filtered", async () => {
			instantiationService.stub(IProductService, {
				version: "1.66.0",
				quality: "stable",
				builtInExtensionsEnabledWithAutoUpdates: ["pub.other"],
			});
			await aUserExtension(
				anExtensionManifest({
					name: "name",
		...
		test("scan all extensions prefers matching user extension over system extension", async () => {
			...
			const actual = await testObject.scanAllExtensions(
				{},
				{
					profileLocation: instantiationService.get(IUserDataProfilesService)
						.defaultProfile.extensionsResource,
