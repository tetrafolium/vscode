	...
	protected toUserDataSyncStore(
		...
		if (
			...
			const type: UserDataSyncStoreType =
				(canSwitch ? this.userDataSyncStoreType : undefined) || defaultType;
			const url =
				type === "insiders"
...
export class UserDataSyncStoreClient extends Disposable {
	...
	async resolveResourceContent(
		...
		const url = joinPath(
			this.getResourceUrl(this.userDataSyncStoreUrl, collection, resource),
			ref,
		).toString();
		headers = { ...headers };
	...
	async getLatestData(
		headers: IHeaders = {},
	): Promise<IUserDataSyncLatestData | null> {
		if (!this.userDataSyncStoreUrl) {
			throw new Error("No settings sync store url configured.");
		}
		...
		if (serverData.collections) {
			...
			for (const collection in serverData.collections) {
				...
				for (const resource in serverData.collections[collection].resources) {
					const [resourceData] =
						serverData.collections[collection].resources[resource];
					resources[resource] = {
						content: resourceData.content,
						ref: resourceData.ref,
					};
		...
		try {
			...
			if (!(e instanceof UserDataSyncStoreError)) {
				...
				// Request path not escaped
					...
					errorMessage.includes("request path contains unescaped characters")
				) {
