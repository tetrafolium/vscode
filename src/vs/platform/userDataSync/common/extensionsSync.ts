		...
		const localExtensions = await this.withProfileScopedServices(
			...
			async (extensionEnablementService, extensionStorageService) => {
				...
				return installedExtensions.map((extension) => {
					...
					const syncExtension: ILocalSyncExtension = {
						identifier,
						preRelease,
					...
					try {
						...
						if (keys) {
							const extensionStorageState =
								...
								{};
							syncExtension.state = Object.keys(extensionStorageState).reduce(
								(state: IStringDictionary<any>, key) => {
					...
					await Promises.settled(
						[...added, ...updated].map(async (e) => {
							...
							if (installedExtension && installedExtension.isBuiltin) {
								if (
									e.state &&
									installedExtension.manifest.version === e.version
								) {
									...
									[
										{
											...e.identifier,
											version,
											preRelease: version ? undefined : e.preRelease,
										},
									],
		...
		if (removed.length) {
			const extensionsToRemove = installedExtensions.filter(
				({ identifier, isBuiltin }) =>
					!isBuiltin && removed.some((r) => areSameExtensions(identifier, r)),
			);
			await Promises.settled(
		...
		for (const skippedExtension of addToSkipped) {
			if (
				!newSkippedExtensions.some((e) =>
					areSameExtensions(e.identifier, skippedExtension.identifier),
				)
			) {
				newSkippedExtensions.push(skippedExtension);
			}
