	...
	implements IUserDataSyncService
{
	...
	async createManualSyncTask(): Promise<IUserDataManualSyncTask> {
		...
		let latestUserDataOrManifest:
			...
			| null;
		try {
			latestUserDataOrManifest =
				await this.userDataSyncStoreService.getLatestData(syncHeaders);
		} catch (error) {
			const userDataSyncError = UserDataSyncError.toUserDataSyncError(error);
			this.telemetryService.publicLog2<SyncErrorEvent, SyncErrorClassification>(
			...
			try {
				latestUserDataOrManifest = await this.userDataSyncStoreService.manifest(
					null,
					syncHeaders,
				);
			} catch (error) {
...
class ProfileSynchronizer extends Disposable {
	...
	private async getUserDataSyncConfiguration(
		...
		if (settingsSynchronizer) {
			const remote =
				await settingsSynchronizer.getRemoteUserDataSyncConfiguration(
					getRefOrUserData(
						manifestOrLatestData,
						this.collection,
...
function reportUserDataSyncError(
	...
	telemetryService.publicLog2<SyncErrorEvent, SyncErrorClassification>(
		"sync/error",
		{
			code: userDataSyncError.code,
			serverCode:
...
function getRefOrUserData(
	...
	if (isUserDataManifest(manifestOrLatestData)) {
		if (collection) {
			return manifestOrLatestData?.collections?.[collection]?.latest?.[
				resource
			];
		}
	...
	if (collection) {
		return manifestOrLatestData?.collections?.[collection]?.resources?.[
			resource
		];
	}
