...
interface IEditableResourcePreview
	extends IBaseResourcePreview, IResourcePreview {
	localChange: Change;
	remoteChange: Change;
	...
	implements IUserDataSynchroniser
{
			...
			for (const resourcePreviewResult of resourcePreviewResults) {
				...
				resourcePreviews.push([
					resourcePreviewResult,
					{
						...acceptResult,
						remoteChange:
							remoteChange !== Change.None ? remoteChange : Change.Modified,
					},
				]);
			}
	...
	protected async doSync(
		...
		try {
			...
			const merge =
				strategy === SyncStrategy.Preview ||
				(strategy === SyncStrategy.Merge && !acceptRemote);
			const apply =
				strategy === SyncStrategy.Merge || strategy === SyncStrategy.PullOrPush;
			...
			if (strategy === SyncStrategy.Merge && acceptRemote) {
				...
				for (const resourcePreview of preview.resourcePreviews) {
					preview =
						(await this.accept(resourcePreview.remoteResource)) || preview;
				}
				...
				for (const resourcePreview of preview.resourcePreviews) {
					...
					if (
						remoteUserData.ref === lastSyncUserData?.ref ||
						isRemoteDataFromCurrentMachine
					) {
						preview =
							(await this.accept(resourcePreview.localResource)) ?? preview;
					} else {
						preview =
							(await this.accept(resourcePreview.remoteResource)) ?? preview;
					}
	...
	async getLastSyncUserData(): Promise<IRemoteUserData | null> {
		const storedLastSyncUserDataStateContent =
