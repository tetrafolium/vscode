...
const PROMPT1_TEXT =
	"Write a poem about a programmer who falls in love with their code.";
const PROMPT2_TEXT = "Explain quantum physics using only emojis and cat memes.";
const PROMPT3_TEXT =
	"Create a dialogue between a toaster and a refrigerator about their daily routines.";
const PROMPT4_TEXT =
	"Describe a day in the life of a rubber duck debugging session.";
const PROMPT5_TEXT =
	"Write a short story where a bug in the code becomes a superhero.";
const PROMPT6_TEXT =
	"Imagine a world where all software bugs are sentient.\nWhat do they talk about?";
...
suite("PromptsSync", () => {
	...
	setup(async () => {
		...
		const maybeSynchronizer = testClient.getSynchronizer(
			SyncResource.Prompts,
		) as PromptsSynchronizer | undefined;
	...
	test("when prompts does not exist", async () => {
		...
		const promptsResource = testClient.instantiationService.get(
			IUserDataProfilesService,
	...
	test("merge when there are multiple prompts and all prompts are merged", async () => {
		...
		const preview = await testObject.sync(
			await testClient.getLatestRef(SyncResource.Prompts),
			true,
		);
	...
	async function updatePrompt(
		...
		const promptsResource = joinPath(
			(profile ?? userDataProfilesService.defaultProfile).promptsHome,
			name,
		);
