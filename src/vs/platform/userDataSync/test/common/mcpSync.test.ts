...
suite("McpSync", () => {
	...
	test("when mcp file does not exist", async () => {
		await runWithFakedTimers<void>({}, async () => {
			...
			const mcpResource = client.instantiationService.get(
	...
	test("when mcp file does not exist and remote has changes", async () => {
		await runWithFakedTimers<void>({}, async () => {
			...
			const mcpResource2 = client2.instantiationService.get(
				IUserDataProfilesService,
			).defaultProfile.mcpResource;
			await client2.instantiationService
				.get(IFileService)
				.writeFile(mcpResource2, VSBuffer.fromString(content));
			await client2.sync();
			...
			assert.strictEqual(
				(await fileService.readFile(mcpResource)).value.toString(),
				content,
			);
		});
	});
	...
	test("apply remote when mcp file does not exist", async () => {
		await runWithFakedTimers<void>({}, async () => {
			...
			const preview = (await testObject.sync(
				await client.getLatestRef(SyncResource.Mcp),
				true,
			))!;
	...
	test("sync profile mcp", async () => {
		await runWithFakedTimers<void>({}, async () => {
			...
			await client2.instantiationService
				.get(IFileService)
				.createFile(profile.mcpResource, VSBuffer.fromString(expected));
			await client2.sync();
			...
			const actual = (
				await client.instantiationService
					.get(IFileService)
					.readFile(syncedProfile.mcpResource)
			).value.toString();
