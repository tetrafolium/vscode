...
export class HoverWidget extends Widget implements IHoverWidget {
	...
	constructor(
		...
		const mouseTracker = (this._mouseTracker = this._register(
	...
	private computeXCordinate(target: TargetRect): void {
		...
		if (this._target.x !== undefined) {
			...
			if (this._hoverPointer) {
				this._x =
					target.center.x - this._hover.containerDomNode.clientWidth / 2;
			} else {
				this._x = target.left;
			}
	...
	private adjustVerticalHoverPosition(target: TargetRect): void {
		...
		// Position hover below the target
			...
			if (
				target.bottom +
					this._hover.containerDomNode.offsetHeight +
					hoverPointerOffset >
	...
	private adjustHoverMaxHeight(target: TargetRect): void {
		let maxHeight =
			this._targetWindow.innerHeight * this._maxHeightRatioRelativeToWindow;
