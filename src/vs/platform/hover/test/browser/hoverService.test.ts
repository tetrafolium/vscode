...
suite("HoverService", () => {
	...
	suite("showInstantHover", () => {
		test("should not show hover with empty content", () => {
			...
			assert.strictEqual(
				hover,
				undefined,
				"Hover should not be created for empty content",
			);
		});
		...
		test("should deduplicate hovers by id", () => {
			...
			assert.strictEqual(
				hover2,
				undefined,
				"Second hover with same id should not be created",
			);
		...
		test("should apply additional classes to hover DOM", () => {
			...
			assert.ok(
				domNode.classList.contains("custom-class-1"),
				"Should have custom-class-1",
			);
			assert.ok(
				domNode.classList.contains("custom-class-2"),
				"Should have custom-class-2",
			);
	...
	suite("hideHover", () => {
		test("should hide non-locked hover", () => {
			...
			assert.strictEqual(
				hover.isDisposed,
				true,
				"Hover should be disposed after hideHover",
			);
		...
		test("should not hide locked hover without force flag", () => {
			...
			assert.strictEqual(
				hover.isDisposed,
				false,
				"Locked hover should not be disposed without force",
			);
			...
			assert.strictEqual(
				hover.isDisposed,
				true,
				"Locked hover should be disposed with force=true",
			);
			assertNotInDOM(
				hover,
				"Locked hover should be removed from DOM with force",
			);
		});
	});
	...
	suite("nested hovers", () => {
		test("should keep parent hover visible when nested hover is created", () => {
			...
			assertInDOM(
				parentHover,
				"Parent hover should still be in DOM after nested hover created",
			);

			assert.strictEqual(
				parentHover.isDisposed,
				false,
				"Parent hover should remain visible",
			);
			assert.strictEqual(
				nestedHover.isDisposed,
				false,
				"Nested hover should be visible",
			);

			nestedHover.dispose();
			assertNotInDOM(
				nestedHover,
				"Nested hover should be removed from DOM after dispose",
			);
			assertInDOM(
				parentHover,
				"Parent hover should remain in DOM after nested is disposed",
			);

			parentHover.dispose();
			assertNotInDOM(
				parentHover,
				"Parent hover should be removed from DOM after dispose",
			);
		});
		...
		test("should dispose nested hover when parent is disposed", () => {
			...
			assert.strictEqual(
				nestedHover.isDisposed,
				true,
				"Nested hover should be disposed when parent is disposed",
			);
			assertNotInDOM(parentHover, "Parent hover should be removed from DOM");
			assertNotInDOM(
				nestedHover,
				"Nested hover should be removed from DOM when parent is disposed",
			);
		});
		...
		test("should dispose entire hover chain when root is disposed", () => {
			...
			for (let i = 0; i < hovers.length; i++) {
				assert.ok(
					mainWindow.document.body.contains(hovers[i].domNode),
					`Hover ${i + 1} should be in DOM`,
				);
			}
			...
			for (let i = 0; i < hovers.length; i++) {
				assert.strictEqual(
					hovers[i].isDisposed,
					true,
					`Hover ${i + 1} should be disposed`,
				);
				assert.ok(
					!mainWindow.document.body.contains(hovers[i].domNode),
					`Hover ${i + 1} should be removed from DOM`,
				);
			}
		});
		...
		test("should dispose only nested hovers when middle hover is disposed", () => {
			...
			for (const h of hovers) {
				assert.ok(
					mainWindow.document.body.contains(h.domNode),
					"All hovers should be in DOM initially",
				);
			}
			...
			assert.strictEqual(
				hovers[0].isDisposed,
				false,
				"Root hover should remain",
			);
			assert.ok(
				mainWindow.document.body.contains(hovers[0].domNode),
				"Root hover should remain in DOM",
			);

			assert.strictEqual(
				hovers[1].isDisposed,
				true,
				"Middle hover should be disposed",
			);
			assert.ok(
				!mainWindow.document.body.contains(hovers[1].domNode),
				"Middle hover should be removed from DOM",
			);

			assert.strictEqual(
				hovers[2].isDisposed,
				true,
				"Innermost hover should be disposed",
			);
			assert.ok(
				!mainWindow.document.body.contains(hovers[2].domNode),
				"Innermost hover should be removed from DOM",
			);
		...
		test("should enforce maximum nesting depth", () => {
			...
			assert.strictEqual(
				hovers.length,
				3,
				"Should create exactly 3 hovers (max depth)",
			);
			...
			assert.strictEqual(
				fourthHover,
				undefined,
				"Fourth hover should not be created due to max nesting depth",
			);
		...
		test("should allow new hover chain after disposing previous chain", () => {
			...
			for (const h of firstChain) {
				assert.ok(
					mainWindow.document.body.contains(h.domNode),
					"First chain hover should be in DOM",
				);
			}
			...
			for (const h of firstChain) {
				assert.ok(
					!mainWindow.document.body.contains(h.domNode),
					"First chain hover should be removed from DOM",
				);
			}
			...
			assert.strictEqual(
				secondChain.length,
				3,
				"Should create new chain after disposing previous",
			);
			for (const h of secondChain) {
				assert.ok(
					mainWindow.document.body.contains(h.domNode),
					"Second chain hover should be in DOM",
				);
			}
		...
		test("hideHover should close innermost hover first", () => {
			...
			assert.ok(
				mainWindow.document.body.contains(hovers[0].domNode),
				"Outer hover should be in DOM",
			);
			assert.ok(
				mainWindow.document.body.contains(hovers[1].domNode),
				"Inner hover should be in DOM",
			);
			...
			assert.strictEqual(
				hovers[0].isDisposed,
				false,
				"Outer hover should remain",
			);
			assert.ok(
				mainWindow.document.body.contains(hovers[0].domNode),
				"Outer hover should remain in DOM",
			);
			...
			assert.strictEqual(
				hovers[0].isDisposed,
				true,
				"Outer hover should be disposed on second call",
			);
			assert.ok(
				!mainWindow.document.body.contains(hovers[0].domNode),
				"Outer hover should be removed from DOM",
			);
		});
	});
	...
	suite("setupDelayedHover", () => {
		test("should evaluate function options on mouseover", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(
					callCount,
					1,
					"Options function should be called on first mouseover",
				);
				...
				assert.strictEqual(
					callCount,
					2,
					"Options function should be called on second mouseover",
				);
		...
		test("should use reduced delay when reducedDelay is true", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				).setUserConfiguration("workbench.hover.reducedDelay", 150);

				const disposable = hoverService.setupDelayedHover(
					target,
					{ content: "Reduced delay" },
					{ reducedDelay: true },
				);
				...
				await timeout(75);
				const hoversBefore =
					mainWindow.document.querySelectorAll(".monaco-hover");
				assert.strictEqual(
					hoversBefore.length,
					0,
					"Hover should not be visible before delay completes",
				);
				...
				assert.strictEqual(
					hoversAfter.length,
					1,
					"Hover should be visible after reduced delay",
				);
	...
	suite("setupManagedHover", () => {
		test("should use native title attribute when showNativeHover is true", () => {
			...
			assert.strictEqual(
				...
				null,
				"Title should be removed on dispose",
			);
		});
		...
		test("should not re-show hover on focus when relatedTarget is from a dismissed hover", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const target = createTarget();
				const delegate = store.add(
				...
				assert.ok(
					hoversBefore.length > 0,
					"Hover should be visible after focus",
				);
				...
				assert.strictEqual(
					hoversAfter.length,
					0,
					"Hover should not re-show when focus comes from dismissed hover",
				);
			}));

		test("should not re-show hover on focus when relatedTarget is null (window reactivation)", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const target = createTarget();
				const delegate = store.add(
					instantiationService.createInstance(
				...
				target.dispatchEvent(
					new FocusEvent("focus", { bubbles: true, relatedTarget: null }),
				);
				await timeout(500);
				...
				assert.strictEqual(
					hovers.length,
					0,
					"Hover should not re-show on window reactivation",
				);
			}));
	});
	...
	suite("showDelayedHover", () => {
		test("should reject hover when current hover is locked and target is outside", () => {
			...
			assert.strictEqual(
				rejectedHover,
				undefined,
				"Should reject hover when locked hover exists",
			);
			assertInDOM(
				lockedHover,
				"Locked hover should remain in DOM after rejection",
			);

			lockedHover.dispose();
			assertNotInDOM(
				lockedHover,
				"Locked hover should be removed from DOM after dispose",
			);
		});
		...
		test("should use reduced delay when reducedDelay is true", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				).setUserConfiguration("workbench.hover.reducedDelay", reducedDelay);
	...
	suite("hover locking", () => {
		test("isLocked should be settable on hover widget", () => {
			...
			assert.strictEqual(
				widget.isLocked,
				true,
				"Should be locked after setting",
			);
			...
			assert.strictEqual(
				widget.isLocked,
				false,
				"Should be unlocked after unsetting",
			);
		...
		test("sticky option should set isLocked to true", () => {
			...
			assert.strictEqual(
				asHoverWidget(hover).isLocked,
				true,
				"Should be locked when sticky",
			);

			hover.dispose();
			assertNotInDOM(
				hover,
				"Sticky hover should be removed from DOM after dispose",
			);
		});
	});
	...
	suite("showAndFocusLastHover", () => {
		test("should recreate last disposed hover", () => {
			...
			assert.ok(
				...
				"A hover should be recreated and in the DOM",
			);
			...
			// Verify cleanup
			const remainingHovers =
				mainWindow.document.querySelectorAll(".monaco-hover");
			assert.strictEqual(
				remainingHovers.length,
				0,
				"No hovers should remain in DOM after cleanup",
			);
		});
	});

	suite("layout and resize", () => {
		test("layout should suppress pending mouseout so content resize does not dismiss hover", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const target = createTarget();
				...
				assertInDOM(
					hover,
					"Hover should remain in DOM after layout suppresses mouseout",
				);
		...
		test.skip("hover should still dismiss on mouseout when no layout occurs", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const target = createTarget();
				const content = document.createElement("div");
				...
				assertNotInDOM(
					hover,
					"Hover should be dismissed after mouseout without layout",
				);
			}));

		test.skip("suppression clears after mouse re-enters and a new mouseleave dismisses normally", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const target = createTarget();
				const content = document.createElement("div");
				content.textContent = "Resizable content";
				...
				assertNotInDOM(
					hover,
					"Hover should dismiss on normal mouseout after suppression was cleared",
				);
			}));

		test("clicking outside should dismiss non-sticky hover", () =>
			runWithFakedTimers({ useFakeTimers: true }, async () => {
				const target = createTarget();
				const content = document.createElement("div");
				content.textContent = "Content";
				...
				assertNotInDOM(
					hover,
					"Non-sticky hover should be dismissed after clicking outside",
				);
			}));
	});
});
