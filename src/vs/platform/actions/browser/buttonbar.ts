...
export class WorkbenchButtonBar extends ButtonBar {
	...
	update(actions: IAction[], secondary: IAction[]): void {
		const configProvider: IButtonConfigProvider =
		...
		for (let i = 0; i < actions.length; i++) {
			...
			if (
				...
				actionOrSubmenu.actions.length > 1
			) {
			...
			const composeLabel = (
				labelValue: string | IMarkdownString,
				...
				if (
					...
					return isMarkdownString(labelValue)
						? new MarkdownString(
								`$(${action.item.icon.id}) ${labelValue.value}`,
								{
			...
			const applyLabel = (labelValue: string | IMarkdownString) => {
				...
				const ariaLabelWithKeybinding =
					this._keybindingService.appendKeybinding(labelStringValue, action.id);
