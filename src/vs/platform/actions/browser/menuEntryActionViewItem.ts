...
} from "../../../base/browser/ui/animations/animations.js";
...
> extends ActionViewItem {
	...
	protected override getTooltip() {
		...
		let title = this._keybindingService.appendKeybinding(
			tooltip,
...
export class DropdownWithDefaultActionViewItem extends BaseActionViewItem {
	...
	private readonly _defaultActionDisposables = this._register(
		new DisposableStore(),
	);
	private readonly _dropdown: DropdownMenuActionViewItem;
	private _container: HTMLElement | null = null;
	private readonly _storageKey: string;
	private readonly _primaryActionListener = this._register(
	...
	private registerTogglePrimaryActionListener(): void {
		this._primaryActionListener.value = this._dropdown.actionRunner.onDidRun(
			(e: IRunEvent) => {
		...
		super(
			...
			action.actions.map((a) =>
				a.id === Separator.ID
					...
					: { text: a.label, isDisabled: !a.enabled },
			),
			0,
			contextViewService,
		...
		if (action.item.isSelection) {
			...
			return instaService.createInstance(
				...
				action,
				{
					...options,
					togglePrimaryAction:
						typeof action.item.isSplitButton !== "boolean"
							? action.item.isSplitButton.togglePrimaryAction
							: false,
				},
			);
		} else {
