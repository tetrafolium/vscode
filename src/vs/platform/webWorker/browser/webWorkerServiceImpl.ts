...
export class WebWorkerService implements IWebWorkerService {
	...
	createWorkerClient<T extends object>(
		...
		if (
			workerDescriptor instanceof Worker ||
			isPromiseLike<Worker>(workerDescriptor)
		) {
	...
	protected _createWorker(descriptor: WebWorkerDescriptor): Promise<Worker> {
		...
		const worker = new Worker(
			ttPolicy
				? (ttPolicy.createScriptURL(workerUrlWithNls) as unknown as string)
				: workerUrlWithNls,
	...
	protected _getWorkerLoadingFailedErrorMessage(
		_descriptor: WebWorkerDescriptor,
	): string | undefined {
		return undefined;
	}
...
const ttPolicy = ((): ReturnType<typeof createTrustedTypesPolicy> => {
	...
	if (
		...
		return createTrustedTypesPolicy("defaultWorkerFactory", {
...
export class WebWorker extends Disposable implements IWebWorker {
	...
	private readonly _onError = this._register(
		new Emitter<MessageEvent | ErrorEvent>(),
	);
