	...
	updateWindowControls(options: {
		...
		if (!isMacintosh && useWindowControlsOverlay(this.configurationService)) {
			...
			const backgroundColor =
				...
				this.lastWindowControlColors?.backgroundColor;
			const foregroundColor =
				options.foregroundColor ??
				this.lastWindowControlColors?.foregroundColor;
			...
			const effectiveBackgroundColor =
				this.windowControlsDimmed && backgroundColor
					? this.dimColor(backgroundColor)
					: backgroundColor;
			const effectiveForegroundColor =
				this.windowControlsDimmed && foregroundColor
					? this.dimColor(foregroundColor)
					: foregroundColor;

			win.setTitleBarOverlay({
				color:
					effectiveBackgroundColor?.trim() === ""
						? undefined
						: effectiveBackgroundColor,
				symbolColor:
					effectiveForegroundColor?.trim() === ""
						? undefined
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	private onConfigurationUpdated(e?: IConfigurationChangeEvent): void {
		...
		if (
			isMacintosh &&
			(!e || e.affectsConfiguration("workbench.editor.swipeToNavigate"))
		) {
			const swipeToNavigate = this.configurationService.getValue<boolean>(
	...
	private readonly swipeListenerDisposable = this._register(
		new MutableDisposable(),
	);

	private registerSwipeListener(): void {
		this.swipeListenerDisposable.value = Event.fromNodeEventEmitter<string>(
			this._win,
			"swipe",
			(event: Electron.Event, cmd: string) => cmd,
		)((cmd) => {
			if (!this.isReady) {
	...
	load(
		...
		if (process.env.VSCODE_DEV && process.env.VSCODE_DEV_SERVER_URL) {
			...
			windowUrl = FileAccess.asBrowserUri(
				`vs/sessions/electron-browser/sessions${this.environmentMainService.isBuilt ? "" : "-dev"}.html`,
			).toString(true);
		} else {
			windowUrl = FileAccess.asBrowserUri(
