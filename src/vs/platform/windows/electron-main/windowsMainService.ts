	...
	implements IWindowsMainService
{
	...
	async openAgentsWindow(
		...
		initialQuery?: string,
		sessionResource?: URI,
		preferredSessionType?: { providerId?: string; sessionTypeId: string },
	): Promise<ICodeWindow[]> {
		...
		if (
			(folderUri || initialQuery || sessionResource || preferredSessionType) &&
			windows.length > 0
		) {
			windows[0].sendWhenReady(
				"vscode:selectAgentsFolder",
				CancellationToken.None,
				folderUri?.toJSON(),
				initialQuery,
				sessionResource?.toJSON(),
	...
	private async ensureAgentsWindow(
		openConfig: IOpenConfiguration,
	): Promise<IOpenConfiguration> {
		const agentSessionsWorkspaceUri =
			this.environmentMainService.agentSessionsWorkspace;
		...
		const workspaceExists = await this.fileService.exists(
			agentSessionsWorkspaceUri,
		);
		if (!workspaceExists) {
			const emptyWorkspaceContent = JSON.stringify({ folders: [] }, null, "\t");
			await this.fileService.writeFile(
				agentSessionsWorkspaceUri,
				VSBuffer.fromString(emptyWorkspaceContent),
			);
		}
	...
	async open(openConfig: IOpenConfiguration): Promise<ICodeWindow[]> {
		...
		if (
			(openConfig.addMode || openConfig.removeMode) &&
				...
				if (focusLastActive) {
					const lastActiveWindow = usedWindows.filter(
						(window) =>
							this.windowsStateHandler.state.lastActiveWindow &&
							window.backupPath ===
								this.windowsStateHandler.state.lastActiveWindow.backupPath,
					);
						...
						if (
							(usedWindow.openedWorkspace &&
								untitledWorkspacesToRestore.some(
									(workspace) =>
										usedWindow.openedWorkspace &&
										workspace.workspace.id === usedWindow.openedWorkspace.id,
								)) || // skip over restored workspace
							(usedWindow.backupPath &&
								emptyWindowsWithBackupsToRestore.some(
									(empty) =>
										usedWindow.backupPath &&
										empty.backupFolder === basename(usedWindow.backupPath),
	...
	private handleChatRequest(
		...
		if (usedWindows.length === 1) {
			...
			if (chatRequestFolder) {
				windowHandlingChatRequest = findWindowOnWorkspaceOrFolder(
					usedWindows,
					URI.file(chatRequestFolder),
				);
			}
		}

		if (windowHandlingChatRequest) {
			windowHandlingChatRequest.sendWhenReady(
	...
	private async doOpen(
		...
		if (
			...
			(foldersToAdd.length > 0 || foldersToRemove.length > 0)
		) {
			const authority =
				foldersToAdd.at(0)?.remoteAuthority ??
				foldersToRemove.at(0)?.remoteAuthority;
			const lastActiveWindow = this.getLastActiveWindowForAuthority(authority);
			if (lastActiveWindow) {
				addUsedWindow(
					this.doAddRemoveFoldersInExistingWindow(
						lastActiveWindow,
						foldersToAdd.map((folderToAdd) => folderToAdd.workspace.uri),
						foldersToRemove.map(
		...
		const potentialNewWindowsCount =
			foldersToOpen.length + workspacesToOpen.length + emptyToRestore.length;
		if (filesToOpen && potentialNewWindowsCount === 0) {
			...
			if (windowToUseForFiles) {
				...
				// Window is single folder
					...
					isSingleFolderWorkspaceIdentifier(windowToUseForFiles.openedWorkspace)
				) {
					foldersToOpen.push({
						workspace: windowToUseForFiles.openedWorkspace,
						remoteAuthority: windowToUseForFiles.remoteAuthority,
				...
				if (
					windowsOnWorkspace.some(
						(window) =>
							...
							window.openedWorkspace.id === workspaceToOpen.workspace.id,
					)
				) {
		...
		if (
			filesToOpen ||
			(maybeOpenEmptyWindow &&
				(openConfig.forceEmpty || usedWindows.length === 0))
		) {
	...
	): { windowToUse: ICodeWindow | undefined; forceNewWindow: boolean } {
	...
	private doOpenFolderOrWorkspace(
		openConfig: IOpenConfiguration,
		folderOrWorkspace: IWorkspacePathToOpen | ISingleFolderWorkspacePathToOpen,
		forceNewWindow: boolean,
		filesToOpen: IFilesToOpen | undefined,
	...
	private async getPathsToOpen(
			...
			if (foldersToOpen.length > 1) {
				...
				if (
					foldersToOpen.every((folderToOpen) =>
						isEqualAuthority(folderToOpen.remoteAuthority, remoteAuthority),
					)
				) {
					let workspace: IWorkspaceIdentifier | undefined;

					const lastSessionWorkspaceMatchingFolders =
						await this.doGetWorkspaceMatchingFoldersFromLastSession(
							remoteAuthority,
							foldersToOpen,
						);
					if (lastSessionWorkspaceMatchingFolders) {
						...
						workspace =
							await this.workspacesManagementMainService.createUntitledWorkspace(
								foldersToOpen.map((folder) => ({ uri: folder.workspace.uri })),
							);
					}
		...
		if (
			openConfig.initialStartup &&
			!isRestoringPaths &&
			this.configurationService.getValue<IWindowSettings | undefined>("window")
	...
	private getRestoreWindowsSetting(): RestoreWindowsSetting {
		...
		if (this.lifecycleMainService.wasRestarted) {
			...
			if (
				!["preserve", "all", "folders", "one", "none"].includes(restoreWindows)
			) {
	...
	private async doGetWorkspaceMatchingFoldersFromLastSession(
		remoteAuthority: string | undefined,
		folders: ISingleFolderWorkspacePathToOpen[],
	): Promise<IWorkspaceIdentifier | undefined> {
		const workspaces = (await this.doGetPathsFromLastSession()).filter((path) =>
			isWorkspacePathToOpen(path),
		);
		...
		for (const { workspace } of workspaces) {
			...
			const folderSet = new ResourceSet(folderUris, (uri) =>
				extUriBiasedIgnorePathCase.getComparisonKey(uri),
			);
			if (
				resolvedWorkspace.folders.every((folder) => folderSet.has(folder.uri))
			) {
		...
		const { response, checkboxChecked } =
			await this.dialogMainService.showMessageBox({
				...
				detail: localize(
					...
					getPathLabel(uri, { os: OS, tildify: this.environmentMainService }),
				),
	...
	private async openInBrowserWindow(
		...
		const defaultProfile =
			newWindowProfile ??
			(lastActiveWindow?.profile?.isAgentsWindowProfile
				? undefined
		...
		if (!options.forceNewWindow && !options.forceNewTabbedWindow) {
			window =
				options.windowToUse ||
				(lastActiveWindow?.config?.isSessionsWindow
					? undefined
					: lastActiveWindow);
			if (window) {
				window.focus();
		...
		const configuration: INativeWindowConfiguration = {
			...
			tmpDir: this.environmentMainService.tmpDir.with({ scheme: Schemas.file })
				.fsPath,
			...
			isSessionsWindow:
				isWorkspaceIdentifier(options.workspace) &&
				isEqual(
					options.workspace.configPath,
					this.environmentMainService.agentSessionsWorkspace,
				),
		};
		...
		// Existing window
			...
			if (
				...
				configuration["disable-extension"] =
					currentWindowConfig["disable-extension"];
			}
		}
	...
	private async doOpenInBrowserWindow(
		...
		if (configuration.isSessionsWindow) {
			configuration.profiles.profile =
				this.userDataProfilesMainService.profiles.find(
					(p) => p.isAgentsWindowProfile,
				) ??
				(await this.userDataProfilesMainService.createAgentsWindowProfile());
		} else {
