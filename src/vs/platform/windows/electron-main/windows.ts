	...
	if (hideNativeTitleBar) {
		...
		if (useWindowControlsOverlay(configurationService)) {
			if (isMacintosh) {
				...
				const titleBarColor =
					themeMainService.getWindowSplash(undefined)?.colorInfo
						.titleBarBackground ?? themeMainService.getBackgroundColor();
		...
		if (displays.length === 1) {
			...
			if (displayWorkingArea) {
				...
				if (state.x > displayWorkingArea.x + displayWorkingArea.width - 128) {
					...
					// the screen
					state.x =
						displayWorkingArea.x + displayWorkingArea.width - state.width;
				}
				...
				if (state.y > displayWorkingArea.y + displayWorkingArea.height - 128) {
					...
					// the screen
					state.y =
						displayWorkingArea.y + displayWorkingArea.height - state.height;
				}
