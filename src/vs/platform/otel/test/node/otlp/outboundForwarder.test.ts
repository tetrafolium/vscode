...
suite("platform/otel - outboundForwarder", () => {
	...
	test("OtlpHttpForwarder re-POSTs raw body with custom headers", async () => {
		...
		strictEqual(
			upstream.received[0].body.toString("utf8"),
			'{"resourceSpans":[]}',
		);
	...
	test("OtlpHttpForwarder logs warning on upstream 500 and does not throw", async () => {
		...
		ok(
			logger.messages.some(
				(m) => m.level === "Warning" && m.msg.includes("500"),
			),
		);
	});
	...
	test("resolveOtlpTracesEndpoint appends path on base URL, leaves explicit path alone", () => {
		strictEqual(
			resolveOtlpTracesEndpoint("http://localhost:4318"),
		...
		strictEqual(
			resolveOtlpTracesEndpoint("http://localhost:4318/custom/path"),
	...
	test("FileForwarder appends one JSON line per span", async () => {
		const path = join(
			tmpdir(),
			`vscode-otel-forwarder-${Date.now()}-${Math.random().toString(36).slice(2)}.jsonl`,
		);
		const fwd = store.add(
			new FileForwarder({ filePath: path }, store.add(new CapturingLogger())),
		);
		fwd.forwardSpans(
			makeResult([
				makeSpan("a", "1111111111111111"),
				makeSpan("b", "2222222222222222"),
			]),
		);
	...
	test("FileForwarder ignores empty result", async () => {
		const path = join(
			tmpdir(),
			`vscode-otel-forwarder-empty-${Date.now()}.jsonl`,
		);
		const fwd = store.add(
			new FileForwarder({ filePath: path }, store.add(new CapturingLogger())),
		);
	...
	test("ConsoleForwarder logs one info per span", () => {
		...
		fwd.forwardSpans(
			makeResult([
				makeSpan("invoke_agent copilot", "1111111111111111", {
