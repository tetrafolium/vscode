		...
		resourceSpans: [
			{
				resource: resourceAttrs
					? { attributes: resourceAttrs as never }
					: undefined,
				scopeSpans: [
					{
...
suite("platform/otel - otlpJsonDecode", () => {
	...
	test("handles an empty request", () => {
		deepStrictEqual(decodeExportTraceRequest({ resourceSpans: [] }), {
			spans: [],
			rejected: 0,
			errors: [],
		});
		deepStrictEqual(decodeExportTraceRequest(undefined), {
			spans: [],
			rejected: 0,
			errors: [],
		});
			...
			[
				{
					...
					endTimeUnixNano: endNs,
					attributes: [
						{ key: "service.name", value: { stringValue: "span-wins" } },
					],
				},
			],
				...
				attributes: [
					{
						key: "mixed",
						value: {
							arrayValue: { values: [{ stringValue: "x" }, { intValue: "7" }] },
						},
					},
					{
						key: "nested",
