...
class FakeContextViewService implements Partial<IContextViewService> {
	...
	showContextView(delegate: IContextViewDelegate): { close: () => void } {
		...
		if (
			result &&
			typeof (result as { dispose?: () => void }).dispose === "function"
		) {
...
function createWidget(disposables: DisposableStore): {
	widget: TabbedActionListWidget;
	contextView: FakeContextViewService;
} {
	const instantiationService = disposables.add(new TestInstantiationService());
	const contextView = new FakeContextViewService();
	instantiationService.stub(
		IContextViewService,
		contextView as IContextViewService,
	);
	...
	instantiationService.stub(ILayoutService, {
		...
		onDidChangeMainContainer: () => ({ dispose: () => {} }),
	} as unknown as ILayoutService);

	const widget = disposables.add(
		instantiationService.createInstance(TabbedActionListWidget),
	);
...
suite("TabbedActionListWidget", () => {
	...
	test("onDidHide fires when the popup dismisses", () => {
		...
		assert.strictEqual(
			hidden,
			1,
			`expected onDidHide to fire once, got ${hidden}; widget visible: ${widget.isVisible}`,
		);
		assert.strictEqual(widget.isVisible, false);
	});
});
