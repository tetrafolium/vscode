...
export class ActionWidgetDropdown extends BaseDropdown {
	...
	override show(): void {
		...
		const optionBeforeOpen: IActionWidgetDropdownAction | undefined =
			actions.find((a) => a.checked);
		let selectedOption: IActionWidgetDropdownAction | undefined =
			optionBeforeOpen;
		...
		const auxiliaryActionIds = new Set(
			actionBarActions.map((action) => action.id),
		);
			...
			getRole: (e) => {
				switch (e.kind) {
					case ActionListItemKind.Action:
						...
						return e.item && auxiliaryActionIds.has(e.item.id)
							? "menuitem"
							: "menuitemcheckbox";
	...
	private _emitCloseEvent(
		...
		if (this._options.reporter) {
			this.telemetryService.publicLog2<
				ActionWidgetDropdownClosedEvent,
				ActionWidgetDropdownClosedClassification
			>("actionWidgetDropdownClosed", {
				...
				optionIdBefore: this._options.reporter.includeOptions
					? optionBefore?.id
					: undefined,
				optionIdAfter: this._options.reporter.includeOptions
					? optionAfter?.id
					: undefined,
				optionLabelBefore: this._options.reporter.includeOptions
					? optionBefore?.label
					: undefined,
				optionLabelAfter: this._options.reporter.includeOptions
					? optionAfter?.label
					: undefined,
			});
		}
