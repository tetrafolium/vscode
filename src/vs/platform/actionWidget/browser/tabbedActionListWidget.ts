...
export interface ITabbedActionListShowOptions<T> {
	...
	readonly accessibilityProvider?: Partial<
		...
		this._contextViewService.showContextView(
			{
				...
				render: (container: HTMLElement) => {
					...
					const tabBar = dom.append(
						widget,
						dom.$(".tabbed-action-list-tabbar"),
					);
					if (options.tabBarClassName) {
						tabBar.classList.add(options.tabBarClassName);
					}
					const radio = renderDisposables.add(
						new Radio({
					...
					const list = renderDisposables.add(
					...
					renderDisposables.add(
						dom.addStandardDisposableListener(widget, "keydown", (e) => {
							const target = e.target as HTMLElement | null;
							const onTabBar = !!target?.closest(".tabbed-action-list-tabbar");
							const onEditable = !!target?.closest(
							...
							const currentIndex = options.tabs.findIndex(
								(t) => t.id === activeTab,
							);
							if (currentIndex < 0) {
								return;
							...
							const nextIndex =
								(currentIndex + delta + options.tabs.length) %
								options.tabs.length;
							e.preventDefault();
							e.stopPropagation();
