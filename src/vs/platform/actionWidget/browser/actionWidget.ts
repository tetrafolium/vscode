...
export interface IActionWidgetService {
	...
	show<T>(
		...
		accessibilityProvider?: Partial<
			IListAccessibilityProvider<IActionListItem<T>>
		>,
...
class ActionWidgetService extends Disposable implements IActionWidgetService {
	...
	get isVisible() {
		return (
			ActionWidgetContextKeys.Visible.getValue(this._contextKeyService) || false
		);
	}
	...
	private _renderWidget(
		...
		renderDisposables.add(
			dom.addDisposableGenericMouseDownListener(block, (e) =>
				e.stopPropagation(),
			),
		);
		...
		renderDisposables.add(
			dom.addDisposableGenericMouseDownListener(pointerBlock, () =>
				pointerBlock.remove(),
			),
		);
		...
		const filterFocusedContext = ActionWidgetContextKeys.FilterFocused.bindTo(
			this._contextKeyService,
		);
		...
		if (this._list.value?.filterInput) {
			...
			renderDisposables.add(
				dom.addDisposableListener(filterInput, "focus", () =>
					filterFocusedContext.set(true),
				),
			);
			renderDisposables.add(
				dom.addDisposableListener(filterInput, "blur", () =>
					filterFocusedContext.set(false),
				),
			);
		}
		...
		renderDisposables.add(
			focusTracker.onDidBlur(() => {
				...
				if (
					activeElement?.closest(".action-widget-hover") ||
					activeElement?.closest(".action-list-submenu-panel")
				) {
					return;
				}
				this.hide(true);
		...
		constructor() {
			super({
				...
				precondition: ContextKeyExpr.and(
					...
					ActionWidgetContextKeys.FilterFocused,
				),
				keybinding: {
					weight: weight + 1,
					primary: KeyCode.Escape,
