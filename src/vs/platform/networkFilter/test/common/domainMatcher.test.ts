...
suite("domainMatcher", () => {
	...
	suite("normalizeDomain", () => {
		...
		test("handles complex valid domains", () => {
			assert.strictEqual(
				normalizeDomain("sub.domain.example.com"),
	...
	suite("extractDomainPattern", () => {
		...
		test("extracts authority from URL", () => {
			assert.strictEqual(
				extractDomainPattern("https://example.com/path"),
				"example.com",
			);
		});
		...
		test("extracts authority with port from URL", () => {
			assert.strictEqual(
				extractDomainPattern("http://example.com:8080/path"),
				"example.com:8080",
			);
		});
	});
	...
	suite("matchesDomainPattern", () => {
		test("exact match", () => {
			...
			assert.strictEqual(
				matchesDomainPattern("example.com", "other.com"),
				false,
			);
		});
		...
		test("wildcard prefix matches subdomains", () => {
			...
			assert.strictEqual(
				matchesDomainPattern("deep.sub.example.com", "*.example.com"),
				true,
			);
			assert.strictEqual(
				matchesDomainPattern("example.com", "*.example.com"),
		...
		test("wildcard prefix does not match unrelated domains", () => {
			assert.strictEqual(
				matchesDomainPattern("notexample.com", "*.example.com"),
				false,
			);
		});
		...
		test("matches domain from URL pattern", () => {
			assert.strictEqual(
				matchesDomainPattern("example.com", "https://example.com/page"),
				true,
			);
		});
	...
	suite("extractDomainFromUri", () => {
		test("extracts domain from https URI", () => {
			assert.strictEqual(
				extractDomainFromUri(URI.parse("https://example.com/path")),
				"example.com",
			);
		});
		...
		test("strips port", () => {
			assert.strictEqual(
				extractDomainFromUri(URI.parse("https://example.com:443/path")),
				"example.com",
			);
		});
		...
		test("returns undefined for empty authority", () => {
			assert.strictEqual(
				extractDomainFromUri(URI.from({ scheme: "file", path: "/tmp/test" })),
				undefined,
			);
		});
	});
	...
	suite("isDomainAllowed", () => {
		...
		test("denied takes precedence over allowed", () => {
			assert.strictEqual(
				isDomainAllowed("evil.com", ["*.com"], ["evil.com"]),
				false,
			);
		});
		...
		test("allowed list restricts to matching domains", () => {
			assert.strictEqual(
				isDomainAllowed("example.com", ["example.com"], []),
				true,
			);
			assert.strictEqual(
				isDomainAllowed("other.com", ["example.com"], []),
				false,
			);
		});
		...
		test("subdomain matching in allow/deny", () => {
			assert.strictEqual(
				isDomainAllowed("api.example.com", ["*.example.com"], []),
				true,
			);
			assert.strictEqual(
				isDomainAllowed("api.example.com", [], ["*.example.com"]),
