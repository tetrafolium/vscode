...
suite("AgentNetworkFilterService", () => {
	...
	test("allows all domains when filter is disabled, regardless of configured lists", async () => {
		...
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://example.com")),
			true,
		);
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://anything.test")),
			true,
		);
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://blocked.com")),
			true,
		);
	});
	...
	test("blocks denied domains", async () => {
		...
		const service = await createService();
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://evil.com")),
			false,
		);
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://good.com")),
			true,
		);
	});
	...
	test("restricts to allowed domains", async () => {
		...
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://other.com")),
			false,
		);
	});
	...
	test("denied takes precedence over allowed", async () => {
		...
		const service = await createService();
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://safe.com")),
			true,
		);
		assert.strictEqual(
			service.isUriAllowed(URI.parse("https://evil.com")),
	...
	suite("isUriAllowed", () => {
		...
		test("checks domain for http/https URIs", async () => {
			...
			const service = await createService();
			assert.strictEqual(
				service.isUriAllowed(URI.parse("https://example.com/page")),
				true,
			);
			assert.strictEqual(
				service.isUriAllowed(URI.parse("https://other.com/page")),
				false,
			);
		});
	});
