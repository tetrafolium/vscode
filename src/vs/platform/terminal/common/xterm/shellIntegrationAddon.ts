	...
	implements IShellIntegration, ITerminalAddon
{
	...
	private readonly _onDidChangeStatus = this._register(
		new Emitter<ShellIntegrationStatus>(),
	);
	readonly onDidChangeStatus = this._onDidChangeStatus.event;
	private readonly _onDidChangeSeenSequences = this._register(
		new Emitter<ReadonlySet<string>>(),
	);
	...
	setNextCommandId(command: string, commandId: string): void {
		if (this._terminal) {
			this._createOrGetCommandDetection(this._terminal).setNextCommandId(
				command,
				commandId,
			);
		}
	...
	private _doHandleVSCodeSequence(data: string): boolean {
		...
		switch (command) {
			...
			case VSCodeOscPt.Property: {
				const arg0 = args[0];
				const deserialized =
					arg0 !== undefined ? deserializeVSCodeOscMessage(arg0) : "";
				...
				switch (key) {
					...
					case "Cwd": {
						...
						this._updateCwd(
							value,
							nonce !== undefined && nonce === this._nonce,
						);
						return true;
					}
					case "IsWindows": {
