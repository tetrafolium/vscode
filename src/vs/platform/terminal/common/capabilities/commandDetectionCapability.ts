	...
	implements ICommandDetectionCapability
{
	...
	private _nextCommandId:
		| { command: string; commandId: string | undefined }
		| undefined;
	...
	get executingCommandConfidence(): "low" | "medium" | "high" | undefined {
		const casted = this._currentCommand as
			| PartialTerminalCommand
			| ITerminalCommand;
		return isFullTerminalCommand(casted)
	...
	constructor(
		...
		this._ptyHeuristicsHooks =
			new (class implements ICommandDetectionHeuristicsHooks {
	...
	handlePromptStart(options?: IHandleCommandOptions): void {
		...
		if (
			...
			lastCommand.executedMarker.line <
				this._terminal.buffer.active.baseY +
					this._terminal.buffer.active.cursorY
		) {
			this._logService.debug(
	...
	handleCommandExecuted(options?: IHandleCommandOptions): void {
		this._ensureCurrentCommandId(
			this._currentCommand.command ?? this._currentCommand.extractCommandLine(),
		);
...
class WindowsPtyHeuristics extends Disposable {
		...
		if (rowsDifference > 0) {
			this._waitForCursorMove().then(() => {
				...
				for (let i = this._capability.commands.length - 1; i >= 0; i--) {
					...
					(
						this._terminal as IXtermWithCore
	...
	private _getWindowsPrompt(
		...
		if (
			...
			(lineText === this._capability.promptTerminator ||
