...
> {
	...
	if (isWindows && windowsBuildNumber < 18309) {
		return {
			type: "failure",
			reason: ShellIntegrationInjectionFailureReason.UnsupportedWindowsBuild,
		};
	}
	...
	if (isWindows) {
		if (shell === "pwsh.exe" || shell === "powershell.exe") {
			envMixin["VSCODE_A11Y_MODE"] = options.isScreenReaderOptimized
				? "1"
				: "0";
				...
				try {
					...
					if (err.message.includes("ENOENT")) {
						try {
							...
							logService.error(
								`Failed to create zdotdir at ${zdotdir}: ${err}`,
							);
							return {
								type: "failure",
								reason:
						...
						try {
							...
							logService.error(
								`Failed to set sticky bit on ${zdotdir}: ${err}`,
							);
							return {
								type: "failure",
								reason:
 ...
 */
const sensitiveEnvVarNames =
	/^(?:.*_)?(?:API_?KEY|TOKEN|SECRET|PASSWORD|PASSWD|PWD|CREDENTIAL|AUTH|PRIVATE_?KEY|ACCESS_?KEY|CLIENT_?SECRET|APIKEY)(?:_.*)?$/i;
