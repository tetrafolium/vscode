	...
	implements ITerminalChildProcess
{
	...
	constructor(
		...
		const useConpty =
			process.platform === "win32" && getWindowsBuildNumberSync() >= 18309;
	...
	> {
		...
		if (injection.type === "injection") {
			...
			if (this._options.shellIntegration.nonce) {
				this._ptyOptions.env ||= {};
				this._ptyOptions.env["VSCODE_NONCE"] =
					this._options.shellIntegration.nonce;
			}
		}

		try {
			const injectionConfig: IShellIntegrationConfigInjection | undefined =
				injection.type === "injection" ? injection : undefined;
			...
			if (errorMessage?.includes("Cannot launch conpty")) {
				return {
					message: localize(
						...
						"https://code.visualstudio.com/updates/v1_109#_removal-of-winpty-support",
						"terminal.integrated.windowsUseConptyDll",
					),
				};
			}
			return {
				message: `A native exception occurred during launch (${errorMessage})`,
			};
		}
	}
	...
	> {
		...
		const envPaths: string[] | undefined =
			slc.env && slc.env.PATH ? slc.env.PATH.split(path.delimiter) : undefined;
	...
	private async setupPtyProcess(
		...
		const sanitizedOptions = {
			...options,
			env: sanitizeEnvForLogging(
				options.env as IProcessEnvironment | undefined,
			),
		};
		this._logService.trace(
			"node-pty.IPty#spawn",
	...
	private async _throttleKillSpawn(): Promise<void> {
		...
		if (
			!isWindows ||
			!hasConptyOption(this._ptyOptions) ||
	...
	acknowledgeDataEvent(charCount: number): void {
		...
		if (
			this._isPtyPaused &&
			this._unacknowledgedCharCount < FlowControlConstants.LowWatermarkChars
		) {
			this._logService.trace(
...
function hasConptyOption(
	obj: IPtyForkOptions | IWindowsPtyForkOptions,
): obj is IWindowsPtyForkOptions {
