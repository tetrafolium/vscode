...
suite("AccessibilityService", () => {
	...
	suite("isTransparencyReduced", () => {
		...
		test("adds CSS class when config is on", () => {
			...
			assert.strictEqual(
				container.classList.contains("monaco-reduce-transparency"),
				true,
			);
		});
	...
	suite("isMotionReduced", () => {
		...
		test("adds CSS classes when config is on", () => {
			...
			assert.strictEqual(
				container.classList.contains("monaco-reduce-motion"),
				true,
			);
			assert.strictEqual(
				container.classList.contains("monaco-enable-motion"),
				false,
			);
		});
