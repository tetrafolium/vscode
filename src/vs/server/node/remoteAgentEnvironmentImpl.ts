...
export class RemoteAgentEnvironmentChannel implements IServerChannel {
	...
	private async _getEnvironmentData(
		...
		if (
			profile &&
			!this._userDataProfilesService.profiles.some((p) => p.id === profile)
		) {
		...
		this._logService.trace(
			`[reconnection-grace-time] Server sending grace time to client: ${this._environmentService.reconnectionGraceTime}ms (${Math.floor(this._environmentService.reconnectionGraceTime / 1000)}s)`,
		);
		return {
			pid: process.pid,
			connectionToken:
			...
			profiles: {
				...
				all: [...this._userDataProfilesService.profiles].map((profile) => ({
					...profile,
				})),
			},
			isUnsupportedGlibc,
