...
export class RemoteExtensionsScannerService implements IRemoteExtensionsScannerService {
	...
	private readonly _whenBuiltinExtensionsReady =
		Promise.resolve<InstallExtensionSummary>({ failed: [] });
				...
				.then(
					...
					async (error) => {
						...
						const alreadyInstalled =
							await this._extensionManagementService.getInstalled(
								ExtensionType.User,
							);
	...
	private async _scanBuiltinExtensions(
		...
		const scannedExtensions =
			await this._extensionsScannerService.scanSystemExtensions({ language });
