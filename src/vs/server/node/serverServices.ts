...
export async function setupServerServices(
	...
	services.set(
		...
		new McpGalleryManifestIPCService(socketServer),
	);
	services.set(
		IExtensionGalleryService,
	...
	const hasUsableSpawnPort =
		Number.isFinite(spawnPortNumber) && spawnPortNumber > 0;
	const bridgePort =
		args["agent-host-bridge-port"] ??
		(hasUsableSpawnPort ? spawnPort : undefined);
	...
	const bridgeToken =
		...
		((bridgePort || bridgePath) &&
		spawnAgentHost &&
		connectionToken.type === ServerConnectionTokenType.Mandatory
			? connectionToken.value
			: undefined);
	if (bridgePort || bridgePath) {
		const agentHostBridge = disposables.add(
			new AgentHostChannel<RemoteAgentConnectionContext>(
				socketServer,
				{
					host: bridgeHost,
					port: bridgePort,
		...
		socketServer.registerChannel(
			AgentHostIpcChannels.RemoteProxy,
			agentHostBridge,
		);
		logService.info(
			`[AgentHostChannel] Registered IPC channel '${AgentHostIpcChannels.RemoteProxy}' (upstream: ${bridgePath ?? `${bridgeHost}:${bridgePort}`})`,
		);
	} else {
		socketServer.registerChannel(
			AgentHostIpcChannels.RemoteProxy,
			new UnavailableAgentHostChannel<RemoteAgentConnectionContext>(),
		);
		logService.info(
			`[AgentHostChannel] Registered unavailable IPC channel '${AgentHostIpcChannels.RemoteProxy}': no --agent-host-bridge-port / --agent-host-bridge-path set.`,
		);
	}

	services.set(
		IAllowedMcpServersService,
		new SyncDescriptor(AllowedMcpServersService),
	);
	services.set(
		IMcpResourceScannerService,
		new SyncDescriptor(McpResourceScannerService),
	);
	...
	instantiationService.invokeFunction((accessor) => {
		...
		socketServer.registerChannel(
			SANDBOX_HELPER_CHANNEL_NAME,
			new SandboxHelperChannel(new SandboxHelperService()),
		);
		...
		socketServer.registerChannel(
			McpGatewayChannelName,
			instantiationService.createInstance(
				McpGatewayChannel<RemoteAgentConnectionContext>,
				socketServer,
			),
		);
