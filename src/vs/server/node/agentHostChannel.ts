...
const agentHostProxyUnavailableMessage =
	"Agent host proxy is not available because no upstream agent host endpoint was configured.";
...
export type UpstreamConnectionFactory = (
	endpoint: IAgentHostUpstreamEndpoint,
) => IUpstreamConnection;
 ...
 */
const defaultUpstreamFactory =
	(
		logService: ILogService,
		lifetime: IServerLifetimeService,
	): UpstreamConnectionFactory =>
	(endpoint) =>
		new WebSocketUpstreamConnection(endpoint, logService, lifetime);

class WebSocketUpstreamConnection
	extends Disposable
	implements IUpstreamConnection
{
		...
		return new Promise<void>((resolve, reject) => {
			const onOpen = () => {
				...
				this._lifetimeToken.value =
					this._serverLifetimeService.active("AgentHostChannel");
			...
			const onError = (err: Error) => {
				...
				this._logService.warn(
					"[AgentHostChannel] Upstream connection failed",
					err,
				);
				this._fireClose();
				reject(err);
	...
	implements IServerChannel<TContext>
{
	...
	constructor(
		...
		super();
		this._upstreamFactory =
			upstreamFactory ??
			defaultUpstreamFactory(_logService, serverLifetimeService);
		this._register(
			ipcServer.onDidRemoveConnection((c) =>
				this._disposeCtx(c.ctx as unknown as TContext),
			),
		);
	}
	...
	listen<T>(ctx: TContext, event: string): Event<T> {
		...
		switch (event) {
			case "frame":
				return conn.onFrame as Event<unknown> as Event<T>;
			case "close":
				return conn.onClose as Event<unknown> as Event<T>;
		}
		throw new Error(`Invalid listen: ${event}`);
	}
