	...
	implements IServerChannel<RemoteAgentConnectionContext>
{
	...
	async call(
		...
		switch (command) {
			...
			case RemoteTerminalChannelRequest.SendSignal:
				return this._ptyHostService.sendSignal.apply(
					this._ptyHostService,
					args,
				);
			...
			case RemoteTerminalChannelRequest.SetNextCommandId:
				return this._ptyHostService.setNextCommandId.apply(
					this._ptyHostService,
					args,
				);
	...
	listen<T>(
		_: unknown,
		event: RemoteTerminalChannelEvent,
		_arg: unknown,
	): Event<T> {
		switch (event) {
			case RemoteTerminalChannelEvent.OnPtyHostExitEvent:
				return (this._ptyHostService.onPtyHostExit || Event.None) as Event<T>;
			case RemoteTerminalChannelEvent.OnPtyHostStartEvent:
				return (this._ptyHostService.onPtyHostStart || Event.None) as Event<T>;
			case RemoteTerminalChannelEvent.OnPtyHostUnresponsiveEvent:
				return (this._ptyHostService.onPtyHostUnresponsive ||
					Event.None) as Event<T>;
			case RemoteTerminalChannelEvent.OnPtyHostResponsiveEvent:
				return (this._ptyHostService.onPtyHostResponsive ||
					Event.None) as Event<T>;
			case RemoteTerminalChannelEvent.OnPtyHostRequestResolveVariablesEvent:
				return (this._ptyHostService.onPtyHostRequestResolveVariables ||
					Event.None) as Event<T>;
			case RemoteTerminalChannelEvent.OnProcessDataEvent:
				return this._ptyHostService.onProcessData as Event<T>;
			case RemoteTerminalChannelEvent.OnProcessReadyEvent:
				return this._ptyHostService.onProcessReady as Event<T>;
			case RemoteTerminalChannelEvent.OnProcessExitEvent:
				return this._ptyHostService.onProcessExit as Event<T>;
			case RemoteTerminalChannelEvent.OnProcessReplayEvent:
				return this._ptyHostService.onProcessReplay as Event<T>;
			case RemoteTerminalChannelEvent.OnProcessOrphanQuestion:
				return this._ptyHostService.onProcessOrphanQuestion as Event<T>;
			case RemoteTerminalChannelEvent.OnExecuteCommand:
				return this.onExecuteCommand as Event<T>;
			case RemoteTerminalChannelEvent.OnDidRequestDetach:
				return (this._ptyHostService.onDidRequestDetach ||
					Event.None) as Event<T>;
			case RemoteTerminalChannelEvent.OnDidChangeProperty:
				return this._ptyHostService.onDidChangeProperty as Event<T>;
		}
	...
	private async _createProcess(
		...
		const shellLaunchConfig: IShellLaunchConfig = {
			...
			cwd:
				...
				typeof args.shellLaunchConfig.cwd === "undefined"
					...
					: URI.revive(
							uriTransformer.transformIncoming(args.shellLaunchConfig.cwd),
						),
			...
			shellIntegrationEnvironmentReporting:
				args.shellLaunchConfig.shellIntegrationEnvironmentReporting,
		};
		...
		const commandsExecuter: ICommandsExecuter = {
			executeCommand: <T>(id: string, ...args: unknown[]): Promise<T> =>
				this._executeCommand(persistentProcessId, id, args, uriTransformer),
		};
	...
	private _executeCommand<T>(
		...
		this._pendingCommands.set(reqId, {
