...
export async function buildUserEnvironment(
	...
	env.VSCODE_RECONNECTION_GRACE_TIME = String(
		environmentService.reconnectionGraceTime,
	);
	logService.trace(
		`[reconnection-grace-time] Setting VSCODE_RECONNECTION_GRACE_TIME env var for extension host: ${environmentService.reconnectionGraceTime}ms (${Math.floor(environmentService.reconnectionGraceTime / 1000)}s)`,
	);
...
export class ExtensionHostConnection extends Disposable {
	...
	private async _pipeSockets(
		...
		disposables.add(
			Event.fromNodeEventEmitter<void>(extHostSocket, "end")(stopAndCleanup),
		);
		disposables.add(
			Event.fromNodeEventEmitter<void>(extHostSocket, "close")(stopAndCleanup),
		);
		disposables.add(
			Event.fromNodeEventEmitter<void>(extHostSocket, "error")(stopAndCleanup),
		);
	...
	public async start(
		...
		try {
			...
			this._extensionHostProcess.on("exit", (code: number, signal: string) => {
				this._extensionHostStatusService.setExitInfo(this._reconnectionToken, {
					code,
					signal,
				});
				this._log(
