...
suite("server.main directory creation", () => {
	...
	test("should create nested directories with recursive option", function () {
		...
		try {
			...
			assert.strictEqual(
				fs.existsSync(join(testDir, "parent")),
				true,
				"Parent directory should exist",
			);
			assert.strictEqual(
				fs.existsSync(join(testDir, "parent", "child")),
				true,
	...
	test("should not fail when parent directories do not exist", function () {
		...
		const testDir = getRandomTestPath(
			os.tmpdir(),
			"vsctests",
			"server-main-nonexistent",
		);
		const deeplyNestedPath = join(
			testDir,
			"level1",
			"level2",
			"level3",
		...
		try {
			...
			assert.strictEqual(
				fs.existsSync(deeplyNestedPath),
				true,
