...
import {
	AgentHostChannel,
	IAgentHostUpstreamEndpoint,
	IUpstreamConnection,
	UnavailableAgentHostChannel,
} from "../../node/agentHostChannel.js";
...
class FakeIPCServer {
	...
	readonly onDidRemoveConnection: Event<Client<string>> =
...
suite("AgentHostChannel", () => {
	...
	function createChannel(): {
		channel: AgentHostChannel<string>;
		...
		let nextCtxId = 0;
		const factory = (
			_endpoint: IAgentHostUpstreamEndpoint,
		): IUpstreamConnection => {
		...
		const channel = ds.add(
			new AgentHostChannel<string>(
...
suite("UnavailableAgentHostChannel", () => {
	...
	test("rejects connect without reporting an unknown IPC channel", async () => {
		...
		await assert.rejects(
			() => channel.call("renderer1", "connect"),
			/Agent host proxy is not available/,
		);
