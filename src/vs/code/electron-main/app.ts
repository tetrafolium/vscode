...
} from "../../platform/browserView/electron-main/browserViewMainService.js";
import {
	BrowserViewGroupMainService,
	IBrowserViewGroupMainService,
} from "../../platform/browserView/electron-main/browserViewGroupMainService.js";
...
export class CodeApplication extends Disposable {
				...
				try {
					...
					const win = frame
						? BrowserWindow.getAllWindows().find(
								(w) => w.webContents.mainFrame === frame,
							)
						: undefined;
					...
					let match = cachedScreenSources.find(
						(s) => s.display_id === String(targetDisplay.id),
					);
					if (!match) {
						...
						match = cachedScreenSources.find(
							(s) => s.display_id === String(targetDisplay.id),
						);
					}
	...
	async startup(): Promise<void> {
		...
		appInstantiationService.invokeFunction((accessor) => {
			(
				accessor.get(IMeteredConnectionService) as MeteredConnectionMainService
			).setTelemetryService(accessor.get(ITelemetryService));
		});
	...
	private async handleProtocolUrl(
		...
		if (session !== null) {
			this.logService.trace(
				`app#handleProtocolUrl() found 'session' as parameter:`,
				uri.toString(true),
			);
	...
	private async initServices(
		...
		// Metered Connection
		const meteredConnectionService = new MeteredConnectionMainService(
		...
		services.set(
			ITerminalSandboxService,
			new SyncDescriptor(NullTerminalSandboxService),
		);
		services.set(
			IAgentNetworkFilterService,
			new SyncDescriptor(AgentNetworkFilterService, undefined, true),
		);
		services.set(
	...
	private initChannels(
		...
		// Metered Connection
		const meteredConnectionChannel = new MeteredConnectionChannel(
			accessor.get(IMeteredConnectionService) as MeteredConnectionMainService,
		);
		mainProcessElectronServer.registerChannel(
		...
		// Browser View
		const browserViewChannel = ProxyChannel.fromService(
		...
		sharedProcessClient.then((client) =>
			client.registerChannel(ipcBrowserViewChannelName, browserViewChannel),
		);

		// Browser View Group
		const browserViewGroupChannel = ProxyChannel.fromService(
			accessor.get(IBrowserViewGroupMainService),
			disposables,
		);
		mainProcessElectronServer.registerChannel(
			ipcBrowserViewGroupChannelName,
			browserViewGroupChannel,
		);
		sharedProcessClient.then((client) =>
			client.registerChannel(
				ipcBrowserViewGroupChannelName,
		...
		// Sandbox Helper
		const sandboxHelperChannel = ProxyChannel.fromService(
			accessor.get(ISandboxHelperMainService),
			disposables,
		);
		mainProcessElectronServer.registerChannel(
			"sandboxHelper",
		...
		const mcpGatewayChannel = this._register(
			new McpGatewayChannel(
				mainProcessElectronServer,
				accessor.get(IMcpGatewayService),
		...
		// Logger
		const loggerChannel = this._register(
			new LoggerChannel(accessor.get(ILoggerMainService)),
		);
	...
	private async openFirstWindow(
		...
		const macOpenFiles: string[] =
			(global as { macOpenFiles?: string[] }).macOpenFiles ?? [];
	...
	private afterWindowOpen(instantiationService: IInstantiationService): void {
		...
		instantiationService.invokeFunction((accessor) => {
			...
			type PowerEventClassification = {
				idleState: {
					...
					comment: "The system idle state (active, idle, locked, unknown).";
				};
				idleTime: {
					classification: "SystemMetaData";
					purpose: "PerformanceAndHealth";
					isMeasurement: true;
					comment: "The system idle time in seconds.";
				};
				thermalState: {
					classification: "SystemMetaData";
					purpose: "PerformanceAndHealth";
					comment: "The system thermal state (unknown, nominal, fair, serious, critical).";
				};
				onBattery: {
					classification: "SystemMetaData";
					purpose: "PerformanceAndHealth";
					isMeasurement: true;
					comment: "Whether the system is running on battery power.";
				};
				owner: "chrmarti";
			...
			this._register(
				Event.fromNodeEventEmitter(
					...
					telemetryService.publicLog2<PowerEvent, PowerEventClassification>(
						"power.suspend",
						getPowerEventData(),
					);
				}),
		...
		if (isMacintosh) {
			instantiationService.invokeFunction((accessor) => {
				...
				const initialGpuFeatureStatus =
					app.getGPUFeatureStatus() as GPUFeatureStatusWithSkiaGraphite;
				const skiaGraphiteEnabled: string =
					initialGpuFeatureStatus["skia_graphite"];
				if (skiaGraphiteEnabled === "enabled") {
					const gpuInfoUpdate = Event.fromNodeEventEmitter(
						app,
						"gpu-info-update",
					);
					const pendingGpuInfoListener = this._register(
						...
						Event.fromNodeEventEmitter<{ details: Details }>(
							...
							if (details.type === "GPU" && details.reason === "crashed") {
								...
								pendingGpuInfoListener.value = Event.once(gpuInfoUpdate)(() => {
									...
									const currentRasterizationStatus: string =
										currentGpuFeatureStatus["rasterization"];
									if (currentRasterizationStatus !== "enabled") {
										...
										type GpuCrashClassification = {
											gpuFeatureStatus: {
												classification: "SystemMetaData";
												purpose: "PerformanceAndHealth";
												comment: "Current GPU feature status.";
											};
											gpuLogMessages: {
												classification: "SystemMetaData";
												purpose: "PerformanceAndHealth";
												comment: "Last 10 GPU log messages collected after the crash and GPU process restart.";
											};
											owner: "deepak1556";
...

		{
			...
			type AppWithNetworkProcessEvents = typeof app & {
				on(
					event: "network-process-launched",
					listener: (
						event: Electron.Event,
						details: NetworkProcessLaunchedDetails,
					) => void,
				): typeof app;
				on(
					event: "network-process-gone",
					listener: (
						event: Electron.Event,
						details: NetworkProcessGoneDetails,
					) => void,
				): typeof app;
			};
			...
			instantiationService.invokeFunction((accessor) => {
				...
				type NetworkProcessGoneClassification = {
					exitCode: {
						...
						purpose: "PerformanceAndHealth";
						isMeasurement: true;
						comment: "The exit code of the network process.";
					};
					crashed: {
						classification: "SystemMetaData";
						purpose: "PerformanceAndHealth";
						comment: "Whether the network process crashed.";
					};
					crashedPreIPC: {
						classification: "SystemMetaData";
						purpose: "PerformanceAndHealth";
						comment: "Whether the network process crashed before IPC was established.";
					};
					owner: "deepak1556";
				...
				this._register(
					Event.fromNodeEventEmitter<NetworkProcessLaunchedDetails>(
						customApp,
						"network-process-launched",
						(_event, details) => details,
					)((details) => {
						this.logService.info(
							`[network process] launched with pid ${details.pid}`,
						);

						telemetryService.publicLog2<
							{},
							NetworkProcessLaunchedClassification
						>("networkProcess.launched", {});
					}),
				);

				this._register(
					Event.fromNodeEventEmitter<NetworkProcessGoneDetails>(
						customApp,
						"network-process-gone",
						(_event, details) => details,
					)((details) => {
						this.logService.info(
							`[network process] gone - pid: ${details.pid}, exitCode: ${details.exitCode}, crashed: ${details.crashed}, crashedPreIPC: ${details.crashedPreIPC}`,
						);

						telemetryService.publicLog2<
							{ exitCode: number; crashed: boolean; crashedPreIPC: boolean },
							NetworkProcessGoneClassification
						>("networkProcess.gone", {
