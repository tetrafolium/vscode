...
class CodeMain {
	...
	private async startup(): Promise<void> {
		...
		try {
			...
			await instantiationService.invokeFunction(async (accessor) => {
				...
				const innoSetupActive = await this.checkInnoSetupMutex(
					productService,
					logService,
				);
				if (innoSetupActive) {
	...
	private createServices(): [
		...
		const policyProductName = isWindows
			? (productService.parentPolicyConfig?.win32RegValueName ??
				productService.win32RegValueName)
			: (productService.parentPolicyConfig?.darwinBundleIdentifier ??
				productService.darwinBundleIdentifier);
		if (isWindows && policyProductName) {
			policyService = disposables.add(
				new NativePolicyService(logService, policyProductName),
			);
	...
	private async checkInnoSetupMutex(
		...
		try {
			...
			const pollIntervalMs = 250,
				retries = 120; // 30s total
			logService.info(
				`checkInnoSetupMutex: ${updatingMutexName} is held, waiting up to ${(pollIntervalMs * retries) / 1000}s for setup to finish...`,
			);
			...
			try {
				...
				logService.info(
					`checkInnoSetupMutex: ${updatingMutexName} released after ${Date.now() - start}ms`,
				);
				return false;
			} catch {
				logService.warn(
					`checkInnoSetupMutex: ${updatingMutexName} still held after ${Date.now() - start}ms, giving up`,
				);
				return true;
			}
		} catch (error) {
