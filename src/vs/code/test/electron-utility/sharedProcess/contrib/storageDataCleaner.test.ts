...
suite("UnusedWorkspaceStorageDataCleaner", () => {
	...
	test("preserves agents window workspace storage", async () => {
		const agentSessionsWorkspace = URI.file(
			join(testDir, "agent-sessions.code-workspace"),
		);
		const agentsWindowFolder = getWorkspaceIdentifier(
		...
		const environmentService = {
			workspaceStorageHome: URI.file(workspaceStorageHome).with({
				scheme: Schemas.file,
			}),
			agentSessionsWorkspace,
		...
		const cleaner = disposables.add(
			new UnusedWorkspaceStorageDataCleaner(
				environmentService,
				new NullLogService(),
				nativeHostService,
				mainProcessService,
		...
		assert.deepStrictEqual(
			remaining,
			[agentsWindowFolder, md5LikeFolder, "ext-dev"].sort(),
		);
	});
});
