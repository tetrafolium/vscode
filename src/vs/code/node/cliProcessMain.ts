...
class CliMain extends Disposable {
	...
	> {
		...
		await Promise.all(
			[
				this.allowWindowsUNCPath(
					environmentService.appSettingsHome.with({ scheme: Schemas.file })
						.fsPath,
				),
		...
		services.set(
			...
			new SyncDescriptor(McpGalleryManifestService, undefined, true),
		);
		services.set(
			IMcpGalleryService,
	...
	private async doRun(
		...
		const profileLocation = (profile ?? userDataProfilesService.defaultProfile)
			.extensionsResource;
		...
		// Install Extension
			...
			const installOptions: InstallOptions = {
				...
				donotIncludePackAndDependencies:
					!!this.argv["do-not-include-pack-dependencies"],
				profileLocation,
			};
			return instantiationService
				.createInstance(
				...
				.installExtensions(
					this.asExtensionIdOrVSIX(this.argv["install-extension"] || []),
