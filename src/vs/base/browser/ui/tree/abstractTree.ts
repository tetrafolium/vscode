...
class TreeNodeListDragAndDrop<T, TFilterData, TRef> implements IListDragAndDrop<
...
> {
	...
	constructor(
		...
		private readonly renderedIndentGuides: SetMap<
			ITreeNode<T, TFilterData>,
			HTMLDivElement
		>,
	...
	updateOptions(options: ITreeRendererOptions<T> = {}): void {
		...
		if (
			...
			typeof options.defaultIndent !== "undefined"
		) {
			const indent =
				typeof options.indent !== "undefined"
					? clamp(options.indent, 0, 40)
					: this.indent;
			const needsRerender =
				indent !== this.indent || typeof options.defaultIndent !== "undefined";

			if (needsRerender) {
				this.indent = indent;

				for (const [node, templateData] of this.renderedNodes) {
					templateData.indentSize =
						this.defaultIndent + (node.depth - 1) * this.indent;
...
export interface IFindFilter<T> extends ITreeFilter<
	T,
...
export class FindFilter<T> implements IFindFilter<T>, IDisposable {
	...
	filter(
		...
		const label =
			this._keyboardNavigationLabelProvider.getKeyboardNavigationLabel(element);
...
export abstract class AbstractFindController<
	T,
	TFilterData,
> implements IDisposable {
	...
	open(): void {
		...
		const widgetContainer =
			this.options.findWidgetContainer ?? this.tree.getHTMLElement();
...
export class FindController<T, TFilterData> extends AbstractFindController<
	T,
	TFilterData
> {
	...
	protected applyPattern(pattern: string): void {
		...
		if (pattern) {
			this.tree.focusNext(
				...
				undefined,
				(node) =>
					!FuzzyScore.isDefault(node.filterData as unknown as FuzzyScore),
			);
		}
...
function stickyScrollNodeStateEquals<T, TFilterData>(
	...
	return (
		node1.position === node2.position && stickyScrollNodeEquals(node1, node2)
	);
}
...
class StickyScrollController<T, TFilterData, TRef> extends Disposable {
			...
			model.onDidSpliceRenderedNodes((e) => {
				...
				const hasRemovedStickyNode =
					...
					state.stickyNodes.some(
						(stickyNode) =>
							!this.model.has(this.model.getNodeLocation(stickyNode.node)),
					);
	...
	private update() {
		...
		if (
			!firstVisibleNode ||
			this.tree.scrollTop <= this.paddingTop ||
			this.view.renderHeight === 0
		) {
...
class StickyScrollWidget<T, TFilterData, TRef> implements IDisposable {
	...
	private createElement(
		...
		if (
			nodeCopy === this.tree.getNode(this.tree.getNodeLocation(stickyNode.node))
		) {
...
class StickyScrollFocus<T, TFilterData, TRef> extends Disposable {
	...
	private _onContextMenu = this._register(
		new Emitter<ITreeContextMenuEvent<T>>(),
	);
	readonly onContextMenu: Event<ITreeContextMenuEvent<T>> =
	...
	constructor(
		...
		this._register(
			addDisposableListener(this.container, "focus", () => this.onFocus()),
		);
		this._register(
			...
			// Move down, if last sticky node is focused, move focus into first child of last sticky node
				...
				if (this.focusedIndex >= this.state.count - 1) {
					const nodeIndexToFocus =
						this.state.stickyNodes[this.state.count - 1].startIndex + 1;
					this.view.domFocus();
...
export interface IAbstractTreeOptions<T, TFilterData = void>
	extends IAbstractTreeOptionsUpdate<T>, IListOptions<T> {
...
class Trait<T> {
	...
	onDidModelSplice({
		insertedNodes,
		deletedNodes,
	}: ITreeModelSpliceEvent<T, unknown>): void {
...
interface ITreeNodeListOptions<T, TFilterData, TRef> extends IListOptions<
	ITreeNode<T, TFilterData>
> {
...
> implements IDisposable {
	...
	constructor(
		...
		this.treeDelegate = new ComposedTreeDelegate<T, ITreeNode<T, TFilterData>>(
			delegate,
		);
		...
		if (_options.keyboardSupport !== false) {
			const onKeyDown = Event.chain(this.view.onKeyDown, ($) =>
				$.filter((e) => !isEditableElement(e.target as HTMLElement)).map(
			...
			this.modelDisposables.add(
				model.onDidChangeCollapseState((e) => {
					...
					if (node.collapsible && !deep && this.isDOMFocused()) {
						alert(
							node.collapsed
								? localize("treeNodeCollapsed", "collapsed")
								: localize("treeNodeExpanded", "expanded"),
						);
					}
				}),
			);
