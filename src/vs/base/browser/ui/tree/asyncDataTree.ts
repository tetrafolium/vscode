...
interface IAsyncDataTreeNodeRequiredProps<TInput, T> extends Partial<
...
class AsyncDataTreeNodeWrapper<TInput, T, TFilterData> implements ITreeNode<
	TInput | T,
	TFilterData
> {
	get element(): T {
...
class AsyncDataTreeNodeListDragAndDrop<TInput, T> implements IListDragAndDrop<
	IAsyncDataTreeNode<TInput, T>
> {
...
> {
	...
	constructor(
		...
		super(
			tree as unknown as AbstractTree<T, TFilterData, unknown>,
			filter,
			contextViewProvider,
			options,
		);
		...
		options && {
			...
			twistieAdditionalCssClass:
				typeof options.twistieAdditionalCssClass === "undefined"
					...
					: (((e: IAsyncDataTreeNode<TInput, T>) =>
							(
								options.twistieAdditionalCssClass as (
									e: T,
								) => string | undefined
...
export interface IAsyncDataTreeUpdateChildrenOptions<
	T,
...
> implements IDisposable {
	...
	get onDidUpdateOptions(): Event<
		IAsyncDataTreeOptionsUpdate<IAsyncDataTreeNode<TInput, T>>
	> {
	...
	updateOptions(
		optionsUpdate: IAsyncDataTreeOptionsUpdate<IAsyncDataTreeNode<
			TInput,
			T
		> | null> = {},
	): void {
			...
			diffIdentityProvider: options.diffIdentityProvider && {
				...
				getGroupId: options.diffIdentityProvider!.getGroupId
					? (
							...
							return options.diffIdentityProvider!.getGroupId!(
								node.element as T,
							);
						}
					: undefined,
...
export interface ICompressibleAsyncDataTreeOptions<
	T,
	TFilterData = void,
