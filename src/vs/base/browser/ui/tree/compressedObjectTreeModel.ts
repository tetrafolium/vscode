...
interface ICompressedObjectTreeModelOptions<
	T,
	TFilterData,
...
export const DefaultElementMapper: ElementMapper<unknown> = (elements) =>
...
class CompressedTreeNodeWrapper<T, TFilterData> implements ITreeNode<
	T | null,
	TFilterData
> {
	get element(): T | null {
	...
	return {
		...
		identityProvider: options.identityProvider && {
			...
			getGroupId: options.identityProvider!.getGroupId
				? (node: ICompressedTreeNode<T>): number | NotSelectableGroupIdType => {
						return options.identityProvider!.getGroupId!(
...
export interface ICompressibleObjectTreeModelOptions<
	T,
	TFilterData,
...
> implements IObjectTreeModel<T, TFilterData> {
	...
	constructor(
		...
		this.elementMapper =
			options.elementMapper || (DefaultElementMapper as ElementMapper<T>);
