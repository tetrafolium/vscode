	...
	implements ISelectBoxDelegate, IListVirtualDelegate<ISelectOptionItem>
{
	...
	private readonly _selectionDetailsDisposables = this._register(
		new DisposableStore(),
	);
	...
	private renderSelectDropDown(
		...
		const computedFontSize = dom
			.getWindow(this.selectElement)
			.getComputedStyle(this.selectElement).fontSize;
		if (computedFontSize) {
	...
	private layoutSelectDropDown(preLayoutPosition?: boolean): boolean {
		...
		if (this.selectList) {
			...
			const maxVisibleOptionsBelow = Math.floor(
				(maxSelectDropDownHeightBelow - maxDetailsPaneHeight) /
					this.getHeight(),
			);
			const maxVisibleOptionsAbove = Math.floor(
				(maxSelectDropDownHeightAbove - maxDetailsPaneHeight) /
					this.getHeight(),
			);
		...
		this._register(
			this.selectList.onMouseOver(
				(e) =>
					...
					!this.options[e.index]?.isDisabled &&
	...
	private renderDescriptionMarkdown(
		text: string,
		actionHandler?: MarkdownActionHandler,
	): IRenderedMarkdown {
	...
	private onPageDown(e: StandardKeyboardEvent): void {
		...
		setTimeout(() => {
			...
			while (
				candidate < this.options.length - 1 &&
				this.options[candidate].isDisabled
			) {
				candidate++;
			}
			if (this.options[candidate].isDisabled) {
