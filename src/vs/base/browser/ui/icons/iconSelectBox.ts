...
export class IconSelectBox extends Disposable {
		...
		if (icons.length) {
			for (let index = 0; index < icons.length; index++) {
				...
				const iconContainer = dom.append(
					container,
					dom.$(".icon-container", { id: `${this.domId}_icons_${index}` }),
				);
	...
	focusPrevious(): void {
		this.focusIcon(
			(this.focusedItemIndex - 1 + this.renderedIcons.length) %
	...
	focusNextRow(): void {
		...
		if (nextRowIndex >= this.renderedIcons.length) {
			...
			nextRowIndex =
				nextRowIndex >= this.renderedIcons.length ? 0 : nextRowIndex;
		}
	...
	focusPreviousRow(): void {
		...
		if (previousRowIndex < 0) {
			...
			previousRowIndex =
				this.focusedItemIndex + this.numberOfElementsPerRow * numberOfRows - 1;
			previousRowIndex =
				previousRowIndex < 0
