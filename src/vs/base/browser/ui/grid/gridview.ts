	...
	private updateBoundarySashes(): void {
		for (let i = 0; i < this.children.length; i++) {
			this.children[i].boundarySashes = {
				...
				orthogonalStart:
					i === 0 ? this.boundarySashes.start : this.splitview.sashes[i - 1],
				orthogonalEnd:
	...
	trySet2x2(other: BranchNode): IDisposable {
		...
		return toDisposable(() => {
			...
			otherFirstChild.linkedHeightNode = otherFirstChild.linkedWidthNode =
				undefined;
			otherSecondChild.linkedHeightNode = otherSecondChild.linkedWidthNode =
				undefined;
		});
	}
...
class LeafNode implements ISplitView<ILayoutContext>, IDisposable {
	...
	constructor(
		...
		this._onDidViewChange = Event.map(
			onDidChange,
			(e) =>
				e && (this.orientation === Orientation.VERTICAL ? e.width : e.height),
			this.disposables,
		);
...
export class GridView implements IDisposable {
	...
	set orientation(orientation: Orientation) {
		...
		const { size, orthogonalSize, absoluteOffset, absoluteOrthogonalOffset } =
			this._root;
	...
	private _getViews(
		...
		for (let i = 0; i < node.children.length; i++) {
			...
			children.push(
				this._getViews(child, orthogonal(orientation), cachedVisibleSize),
			);
		}
