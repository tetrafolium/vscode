...
function isListElementDescendantOfClass(
	e: HTMLElement,
	className: string,
): boolean {
	...
	private onCtrlA(e: StandardKeyboardEvent): void {
		...
		const referenceGroupId =
			focusedElements.length > 0
				? this.list.getElementGroupId(focusedElements[0])
				: undefined;
...
export const DefaultKeyboardNavigationDelegate =
	new (class implements IKeyboardNavigationDelegate {
...
class TypeNavigationController<T> implements IDisposable {
	...
	private state: TypeNavigationControllerState =
		TypeNavigationControllerState.Idle;
	...
	private enable(): void {
		...
		const onChar = Event.chain(
			this.enabledDisposables.add(new DomEmitter(this.view.domNode, "keydown"))
				.event,
			($) =>
				$.filter((e) => !isEditableElement(e.target as HTMLElement))
					.filter(
						() => this.mode === TypeNavigationMode.Automatic || this.triggered,
					)
	...
	private onInput(word: string | null): void {
		...
		for (let i = 0; i < this.list.length; i++) {
			...
			if (this.list.options.typeNavigationEnabled) {
				...
				typeof labelStr === "undefined" ||
				matchesPrefix(word, labelStr)
			) {
				this.previouslyFocused = start;
				this.list.setFocus([index]);
				this.list.reveal(index);
...
export class MouseController<T> implements IDisposable {
	...
	private readonly _onPointer = this.disposables.add(
		new Emitter<IListMouseEvent<T>>(),
	);
	get onPointer() {
	...
	private changeSelection(e: IListMouseEvent<T> | IListTouchEvent<T>): void {
		...
		if (this.isSelectionRangeChangeEvent(e)) {
			...
			if (selectedElement !== undefined) {
				...
				if (referenceGroupId !== undefined) {
					rangeSelection = this.list.filterIndicesByGroup(
						rangeSelection,
						referenceGroupId,
					);
				}
			...
			if (selection.length === newSelection.length) {
				const itemsToBeSelected =
					focusGroupId !== undefined
						? this.list.filterIndicesByGroup(
								[...newSelection, focus],
								focusGroupId,
							)
						: [...newSelection, focus];
...
export interface IListAccessibilityProvider<
	T,
...
export const unthemedListStyles: IListStyles = {
	...
	treeInactiveIndentGuidesStroke: Color.fromHex("#a9a9a9")
		.transparent(0.4)
		.toString(),
	tableColumnsBorder: Color.fromHex("#cccccc").transparent(0.2).toString(),
	tableOddRowsBackgroundColor: Color.fromHex("#cccccc")
		.transparent(0.04)
		.toString(),
...
class AccessibiltyRenderer<T> implements IListRenderer<
	T,
	IAccessibilityTemplateData
> {
	...
	renderElement(
		element: T,
		index: number,
		data: IAccessibilityTemplateData,
	): void {
...
export class List<T> implements ISpliceable<T>, IDisposable {
	...
	@memoize get onKeyDown(): Event<KeyboardEvent> {
		return this.disposables.add(new DomEmitter(this.view.domNode, "keydown"))
			.event;
	}
	@memoize get onKeyUp(): Event<KeyboardEvent> {
		return this.disposables.add(new DomEmitter(this.view.domNode, "keyup"))
			.event;
	}
	@memoize get onKeyPress(): Event<KeyboardEvent> {
		return this.disposables.add(new DomEmitter(this.view.domNode, "keypress"))
			.event;
	}

	@memoize get onDidFocus(): Event<void> {
		return Event.signal(
			this.disposables.add(new DomEmitter(this.view.domNode, "focus", true))
				.event,
		);
	}
	@memoize get onDidBlur(): Event<void> {
		return Event.signal(
			this.disposables.add(new DomEmitter(this.view.domNode, "blur", true))
				.event,
		);
	}
	...
	setSelection(indexes: number[], browserEvent?: UIEvent): void {
		...
		indexes = indexes.filter(
			(i) => this.getElementGroupId(i) !== NotSelectableGroupId,
		);
	...
	getElementGroupId(
		index: number,
	): number | NotSelectableGroupIdType | undefined {
	...
	async focusNextPage(
		...
		if (
			...
			(currentlyFocusedElementIndex === undefined ||
				lastPageIndex > currentlyFocusedElementIndex)
		) {
	...
	async focusPreviousPage(
		...
		if (
			...
			if (
				...
				currentlyFocusedElementIndex !== firstGoodPageIndex
			) {
