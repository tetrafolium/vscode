...
export interface IPagedRenderer<TElement, TTemplateData> extends IListRenderer<
	TElement,
	TTemplateData
> {
...
class PagedRenderer<TElement, TTemplateData> implements IListRenderer<
	number,
	ITemplateData<TTemplateData>
> {
	...
	renderElement(
		...
		promise.then((entry) =>
			this.renderer.renderElement(entry, index, data.data!, details),
		);
	}
...
export class PagedList<T> implements IDisposable {
	...
	set model(model: IPagedModel<T>) {
		...
		this.modelDisposables.add(
			model.onDidIncrementLength((newLength) =>
				this.list.splice(
					this.list.length,
					0,
					range(this.list.length, newLength),
				),
			),
		);
	}
