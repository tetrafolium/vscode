...
function fillInIncompleteTokensOnce(
	...
	while (
		...
		(tokens[lastInterestingIdx].type === "space" ||
			tokens[lastInterestingIdx].type === "html")
	) {
		lastInterestingIdx--;
	}
	const lastInterestingToken =
		lastInterestingIdx >= 0 ? tokens[lastInterestingIdx] : undefined;
	...
	if (!newTokens && lastInterestingToken?.type === "list") {
		const newListToken = completeListItemPattern(
			lastInterestingToken as marked.Tokens.List,
		);
		if (newListToken) {
	...
	if (!newTokens && lastInterestingToken?.type === "paragraph") {
		...
		const newToken = completeSingleLinePattern(
			lastInterestingToken as marked.Tokens.Paragraph,
		);
		if (newToken) {
