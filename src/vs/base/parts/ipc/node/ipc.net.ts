...
export function upgradeToISocket(
	...
	implements ISocket, ISocketTracer
{
	...
	constructor(
		...
		super();
		this.socket = socket;
		this._maxSocketMessageLength = enableMessageSplitting
			? Constants.MaxWebSocketMessageLength
			: Infinity;
		this.traceSocketEvent(SocketDiagnosticsEventType.Created, {
...
export function connect(
	hook: { host: string; port: number } | string,
	clientId: string,
): Promise<Client> {
