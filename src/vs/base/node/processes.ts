...
export async function findExecutable(
	...
	cwd?: string,
	paths?: string[],
	env: Platform.IProcessEnvironment = processCommon.env,
	fileExists: (path: string) => Promise<boolean> = fileExistsDefault,
	...
	for (const pathEntry of paths) {
		...
		if (Platform.isWindows) {
			const pathExt =
				(getCaseInsensitive(env, "PATHEXT") as string) || ".COM;.EXE;.BAT;.CMD";
	...
	return new Promise<void>((resolve, reject) => {
		...
		child.on("exit", (code) => {
			if (code === 0) {
				...
				reject(
					new Error(
						`taskkill exited with code ${code}: ${Buffer.concat(stdout).toString()}`,
					),
				);
			}
		});
