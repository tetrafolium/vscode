...
} from "../../browser/ui/toggle/toggle.js";
...
suite("Actionbar", () => {
	...
	suite("ToggleActionViewItemProvider", () => {
		test("renders toggle for actions with checked state", function () {
			...
			const toggleAction = store.add(
				new Action("toggle", "Toggle", undefined, true, undefined),
			);
			...
			assert(
				actionbar.viewItems[0] instanceof ToggleActionViewItem,
				"Action with checked state should render as ToggleActionViewItem",
			);
		});
		...
		test("renders button for actions without checked state", function () {
			...
			assert.strictEqual(actionbar.viewItems.length, 1);
			assert(
				actionbar.viewItems[0] instanceof ActionViewItem,
				"Action without checked state should render as ActionViewItem",
			);
			assert(
				!(actionbar.viewItems[0] instanceof ToggleActionViewItem),
				"Action without checked state should not render as ToggleActionViewItem",
			);
		});
		...
		test("handles mixed actions (toggles and buttons)", function () {
			...
			assert(
				actionbar.viewItems[0] instanceof ToggleActionViewItem,
				"First action should be a toggle",
			);
			assert(
				actionbar.viewItems[1] instanceof ActionViewItem,
				"Second action should be a button",
			);
			assert(
				!(actionbar.viewItems[1] instanceof ToggleActionViewItem),
				"Second action should not be a toggle",
			);
		});
		...
		test("toggle state changes when action checked changes", function () {
			...
			assert(
				toggleViewItem instanceof ToggleActionViewItem,
				"Toggle view item should exist",
			);
			...
			assert.strictEqual(
				...
				true,
				"Toggle action should update checked state",
			);
		});

		test("quick input button with toggle property creates action with checked state", async function () {
			const { quickInputButtonToAction } =
				await import("../../../platform/quickinput/browser/quickInputUtils.js");
			...
			assert.strictEqual(
				action.checked,
				true,
				"Action should have checked property set to true",
			);
			...
			const regularAction = quickInputButtonToAction(
				regularButton,
				"test-id-2",
				() => {},
			);
			...
			assert.strictEqual(
				regularAction.checked,
				undefined,
				"Regular action should not have checked property",
			);
		});
	});
});
