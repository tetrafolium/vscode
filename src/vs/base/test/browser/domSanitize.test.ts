...
suite("DomSanitize", () => {
	...
	test("allows custom tags via config", () => {
		{
			...
			assert.strictEqual(
				result.toString(),
				"removed<custom-tag>hello</custom-tag>",
			);
		}
		{
			...
			assert.strictEqual(
				result.toString(),
				"<div>kept</div><augmented-tag>world</augmented-tag>",
			);
		}
	});
	...
	test("Supports dynamic attribute sanitization", () => {
		const html =
			'<div title="a" other="1">text1</div><div title="b" other="2">text2</div>';
		const result = sanitizeHtml(html, {
			allowedAttributes: {
				override: [
					{
		...
		assert.strictEqual(
			result.toString(),
			'<div>text1</div><div title="b">text2</div>',
		);
	});

	test("Supports changing attributes in dynamic sanitization", () => {
		const html =
			'<div title="abc" other="1">text1</div><div title="xyz" other="2">text2</div>';
		const result = sanitizeHtml(html, {
			allowedAttributes: {
				override: [
					{
		...
		assert.strictEqual(
			result.toString(),
			'<div>text1</div><div title="xyzxyz">text2</div>',
		);
	});
	...
	test("Attr name should clear previously set dynamic sanitizer", () => {
		...
		assert.strictEqual(
			result.toString(),
			'<div title="abc">text1</div><div title="xyz">text2</div>',
		);
	});
	...
	suite("replaceWithPlaintext", () => {
		test("replaces unsupported tags with plaintext representation", () => {
			...
			assert.strictEqual(
				str,
				`<div>safe&lt;script&gt;alert(1)&lt;/script&gt;content</div>`,
			);
		});
		...
		test("handles self-closing tags correctly", () => {
			...
			assert.strictEqual(
				result.toString(),
				'<div>&lt;input type="text"&gt;&lt;custom-input&gt;&lt;/custom-input&gt;</div>',
			);
		});

		test("handles tags with attributes", () => {
			const html =
				'<div><unknown-tag class="test" id="myid">content</unknown-tag></div>';
			const result = sanitizeHtml(html, {
				replaceWithPlaintext: true,
			});
			assert.strictEqual(
		...
		test("handles nested unsupported tags", () => {
			...
			assert.strictEqual(
				result.toString(),
				"<div>&lt;outer&gt;&lt;inner&gt;nested&lt;/inner&gt;&lt;/outer&gt;</div>",
			);
		});
		...
		test("works with custom allowed tags configuration", () => {
			const html =
				"<div><custom>allowed</custom><forbidden>not allowed</forbidden></div>";
			const result = sanitizeHtml(html, {
				replaceWithPlaintext: true,
