...
suite("Grid", function () {
	...
	test("getNeighborViews should work on single view layout", function () {
		...
		assert.deepStrictEqual(grid.getNeighborViews(view1, Direction.Up, true), [
			view1,
		]);
		assert.deepStrictEqual(
		...
		assert.deepStrictEqual(grid.getNeighborViews(view1, Direction.Left, true), [
			view1,
		]);
	});
	...
	test("getNeighborViews should work on simple layout", function () {
		...
		assert.deepStrictEqual(grid.getNeighborViews(view2, Direction.Up, true), [
			view1,
		]);
		assert.deepStrictEqual(
		...
		assert.deepStrictEqual(grid.getNeighborViews(view2, Direction.Left, true), [
			view2,
		]);
		...
		assert.deepStrictEqual(grid.getNeighborViews(view3, Direction.Up, true), [
			view2,
		]);
		assert.deepStrictEqual(
		...
		assert.deepStrictEqual(grid.getNeighborViews(view3, Direction.Left, true), [
			view3,
		]);
	});
...
suite("SerializableGrid", function () {
	...
	test("sanitizeGridNodeDescriptor", () => {
		const nodeDescriptor: GridNodeDescriptor<any> = {
	...
	test("createSerializedGrid - issue #85601, should not allow single children groups", () => {
		...
		const deserializer =
			new (class implements IViewDeserializer<ISerializableView> {
