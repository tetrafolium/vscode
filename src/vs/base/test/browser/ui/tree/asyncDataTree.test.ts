...
class Renderer implements ICompressibleTreeRenderer<
	Element,
	void,
	HTMLElement
> {
	...
	renderElement(
		element: ITreeNode<Element, void>,
		index: number,
		templateData: HTMLElement,
	): void {
		templateData.textContent =
			element.element.id + (element.element.suffix || "");
	}
	disposeTemplate(templateData: HTMLElement): void {
		// noop
	}
	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<Element>, void>,
		index: number,
		templateData: HTMLElement,
	): void {
		const result: string[] = [];
...
suite("AsyncDataTree", function () {
	...
	test("Collapse state should be preserved across refresh calls", async () => {
		...
		const tree = store.add(
			new AsyncDataTree<Element, Element>(
				"test",
				container,
				new VirtualDelegate(),
				[new Renderer()],
	...
	test("issue #199441", async () => {
		...
		const compressionDelegate =
			new (class implements ITreeCompressionDelegate<Element> {
		...
		const tree = store.add(
			new CompressibleAsyncDataTree<Element, Element>(
				"test",
				container,
				new VirtualDelegate(),
				compressionDelegate,
	...
	test("Tree Navigation: AsyncDataTree", async () => {
		...
		assert.deepStrictEqual(
			...
			["a", "aa", "aa.txt", "ab", "ab.txt", "b", "c", "ca", "ca.txt", "cb"],
		);
	...
	test("Test Navigation: CompressibleAsyncDataTree", async () => {
		...
		assert.deepStrictEqual(
			...
			["a/aa", "aa.txt", "b/ba", "ba.txt", "c", "ca", "ca.txt", "cb", "cb.txt"],
		);
