 ...
 */
function withSmartSplice(
	fn: (options: IIndexTreeModelSpliceOptions<number, any>) => void,
) {
	fn({});
...
suite("IndexTreeModel", () => {
	...
	test("smart diff consistency", () => {
		...
		for (let i = 0; i < times; i++) {
			const list: ITreeNode<number>[] = [];
			const options = {
				diffIdentityProvider: { getId: (n: number) => String(n) },
			};
			const model = new IndexTreeModel<number>("test", -1);
			const disposable = bindListToModel(list, model);
			...
			for (
				let edits = Math.random() * (maxEdits - minEdits) + minEdits;
				edits > 0;
				edits--
			) {
				...
				if (Math.random() < 0.5) {
					...
					inserts.push(
						...elements.map(({ element }) => ({ element, children: [] })),
					);
				}
	...
	test("refilter", () => {
		...
		const filter = new (class implements ITreeFilter<number> {
			filter(element: number): TreeVisibility {
				return !shouldFilter || element % 2 === 0
					? TreeVisibility.Visible
					: TreeVisibility.Hidden;
			}
		})();
