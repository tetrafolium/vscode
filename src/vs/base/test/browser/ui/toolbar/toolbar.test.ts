...
import {
	ToggleMenuAction,
	ToolBar,
} from "../../../../browser/ui/toolbar/toolbar.js";
...
suite("ToolBar", () => {
	...
	test("keeps the last primary action shrinkable when overflow is inserted", () => {
		...
		const toolbar = store.add(
			new TestToolBar(container, contextMenuProvider, {
				responsiveBehavior: {
					enabled: true,
					kind: "last",
					minItems: 1,
				...
				actionViewItemProvider: (action) => {
					...
					return typeof width === "number"
						? new FixedWidthActionViewItem(action, width)
						: undefined;
				},
			}),
		);
		...
		actionBar.getWidth = (index: number) => {
			...
			return action
				? (widths.get(action.id) ?? originalGetWidth(index))
				: originalGetWidth(index);
		};

		const originalGetBoundingClientRect = toolbar
			.getElement()
			.getBoundingClientRect.bind(toolbar.getElement());
		(
			toolbar.getElement() as HTMLElement & { getBoundingClientRect(): DOMRect }
		...
		const actions = [
			store.add(
				new Action("workbench.action.chat.attachContext", "Add Context..."),
			),
			store.add(
				new Action("workbench.action.chat.openModePicker", "Open Agent Picker"),
			),
			store.add(
				new Action(
		...
		assert.strictEqual(
			toolbar.getItemAction(0)?.id,
			"workbench.action.chat.attachContext",
		);
		assert.strictEqual(
			toolbar.getItemAction(1)?.id,
			"workbench.action.chat.openModePicker",
		);
		assert.strictEqual(
			toolbar.getItemAction(2)?.id,
			"workbench.action.chat.openModelPicker",
		);
	...
	test("applies per-action responsive min widths", () => {
		const toolbar = store.add(
			new ToolBar(container, contextMenuProvider, {
				responsiveBehavior: {
					enabled: true,
					kind: "last",
					minItems: 1,
					...
					getActionMinWidth: (action) =>
						action.id === "workbench.action.chat.openModelPicker"
							? 28
							: undefined,
				},
				actionViewItemProvider: (action) =>
		...
		assert.strictEqual(
			toolbar
				.getElement()
				.style.getPropertyValue("--vscode-toolbar-action-min-width"),
			"28px",
		);
	});
	...
	test("relayout re-evaluates responsive overflow after action width changes", () => {
		...
		const actions = [
			...
			store.add(
				new Action("workbench.action.chat.openModePicker", "Open Mode Picker"),
			),
			store.add(
				new Action(
					"workbench.action.chat.openModelPicker",
