...
suite("MarkdownRenderer", () => {
	...
	suite("Sanitization", () => {
		...
		test("Strips links with disallowed schemes (default config)", () => {
			const markdown = {
				value: `Read [](vscode-agent-host://my-host/file/-/path/to/foo.ts)`,
			};
			const result: HTMLElement = store.add(renderMarkdown(markdown)).element;
		...
		test("Preserves link when scheme is allowed via allowedLinkSchemes.augment", () => {
			...
			assert.strictEqual(
				anchor!.dataset.href,
				"vscode-agent-host://my-host/file/-/path/to/foo.ts",
			);
		});
	});
	...
	suite("ThemeIcons Support On", () => {
		...
		test("render icon in link", () => {
			...
			assert.strictEqual(
				result.innerHTML,
				`<p><a href="" title="#link" draggable="false" data-href="#link"><span class="codicon codicon-zap"></span>-link</a></p>`,
			);
		});
...
`,
	...
	suite("Alerts", () => {
		test("Should render alert with data-severity attribute and icon", () => {
			...
			const blockquote = result.querySelector(
				'blockquote[data-severity="note"]',
			);
			assert.ok(blockquote, 'Should have blockquote with data-severity="note"');
			assert.ok(
				result.innerHTML.includes("This is a note alert"),
				"Should contain alert text",
			);
			assert.ok(
				result.innerHTML.includes("codicon-info"),
		...
		test("Should render regular blockquote when supportAlertSyntax is disabled", () => {
			...
			assert.strictEqual(
				blockquote?.getAttribute("data-severity"),
				null,
				"Should not have data-severity attribute",
			);
			assert.ok(
				result.innerHTML.includes("[!NOTE]"),
				"Should contain literal [!NOTE] text",
			);
		});

		test("Should not transform blockquotes without alert syntax", () => {
			const markdown = new MarkdownString("> This is a regular blockquote", {
				supportAlertSyntax: true,
			});
			const result = store.add(renderMarkdown(markdown)).element;
	...
	test("npm Hover Run Script not working #90855", function () {
		...
		const data = <{ script: string; documentUri: URI }>(
			parse(decodeURIComponent(uri.query))
		);
		assert.ok(data);
	...
	test("Should render command links in trusted strings", () => {
		...
		assert.strictEqual(
			result.innerHTML,
			`<p><a href="" title="command:doFoo" draggable="false" data-href="command:doFoo">command1</a> <a href="" data-href="command:doFoo">command2</a></p>`,
		);
	});
	...
	test("Should support relative links if baseurl is set", () => {
		const md = new MarkdownString(
			`[text](./foo) <a href="./bar">bar</a> <img src="cat.gif">`,
			{
				isTrusted: true,
				supportHtml: true,
			},
		...
		assert.strictEqual(
			result.innerHTML,
			`<p><a href="" title="./foo" draggable="false" data-href="https://example.com/path/foo">text</a> <a href="" data-href="https://example.com/path/bar">bar</a> <img src="https://example.com/path/cat.gif"></p>`,
		);
	});
	...
	test("Should not override explicit title for file:// links", () => {
		const fileUri = URI.file("/home/user/project/lib.d.ts");
		const md = new MarkdownString(
			`[log](${fileUri.toString()} "Go to definition")`,
			{},
		);
	...
	- item 2
		...
		suite("link", () => {
			...
			test("incomplete link target with extra stuff and incomplete arg", () => {
				const incomplete =
					'[before `text` after](http://microsoft.com "more text ';
				const tokens = marked.marked.lexer(incomplete);
				const newTokens = fillInIncompleteTokens(tokens);
