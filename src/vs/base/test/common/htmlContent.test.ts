...
suite("htmlContent", () => {
	...
	suite("appendEscapedMarkdownInlineCode", () => {
		test("wraps plain text in single backticks", () => {
			...
			assert.strictEqual(
				appendEscapedMarkdownInlineCode("foo bar"),
				"`foo bar`",
			);
		});
		...
		test("chooses a fence longer than any backtick run in the content", () => {
			...
			assert.strictEqual(appendEscapedMarkdownInlineCode("a``b"), "```a``b```");
			assert.strictEqual(
				appendEscapedMarkdownInlineCode("a```b```c"),
				"````a```b```c````",
			);
		});
		...
		test("pads with spaces when the content begins or ends with a backtick", () => {
			assert.strictEqual(appendEscapedMarkdownInlineCode("`"), "`` ` ``");
			assert.strictEqual(
				appendEscapedMarkdownInlineCode("`hello"),
				"`` `hello ``",
			);
			assert.strictEqual(
				appendEscapedMarkdownInlineCode("hello`"),
			...
			assert.strictEqual(
				appendEscapedMarkdownInlineCode("`a`b`"),
				"`` `a`b` ``",
			);
		});
	...
	suite("escapeMarkdownLinkLabel", () => {
		test("passes plain text through unchanged", () => {
			...
			assert.strictEqual(
				escapeMarkdownLinkLabel("heap-snapshot-analysis"),
		...
		test("does not escape characters that are safe in link text", () => {
			...
			assert.strictEqual(
				escapeMarkdownLinkLabel("a*b_c#d-e.f!g~h+i(j)k{l}m"),
				"a*b_c#d-e.f!g~h+i(j)k{l}m",
			);
		});
	});
});
