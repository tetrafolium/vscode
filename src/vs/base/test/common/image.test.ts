...
function makeGif(width: number, height: number): VSBuffer {
	return buf(
		...
		0x00,
		0x00,
		0x00, // packed field, bg color index, pixel aspect ratio
	);
}
...
function makeWebPVp8l(width: number, height: number): VSBuffer {
	...
	return buf(
		...
		0,
		0,
		0, // padding to satisfy the 30-byte minimum
	);
}
...
suite("readImageDimensions", () => {
	...
	test("parses dimensions across supported formats and rejects unrecognized buffers", () => {
		const fixtures = {
			...
			webpVp8MissingStartCode: {
				input: corruptByte(makeWebPVp8(300, 200), 23, 0x00),
				expected: undefined,
			},
			webpVp8lMissingSignature: {
				input: corruptByte(makeWebPVp8l(1024, 768), 20, 0x00),
				expected: undefined,
			},
			webpVp8ChunkTooSmall: {
				input: corruptByte(makeWebPVp8(300, 200), 16, 4),
				expected: undefined,
			},
			webpVp8xChunkTooSmall: {
				input: corruptByte(makeWebPVp8x(8000, 6000), 16, 4),
				expected: undefined,
			},
			jpegZeroSegmentLength: {
				input: buf(
					...
					0xe0,
					0x00,
					0x00, // APP0 with invalid length 0
					0,
					0,
					0,
					0,
					...
					0,
					0,
					0, // padding to pass the outer < 12 check
				),
				expected: undefined,
			},
		};
