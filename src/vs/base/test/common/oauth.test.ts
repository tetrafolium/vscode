...
suite("OAuth", () => {
	...
	suite("Type Guards", () => {
		...
		test("isAuthorizationServerMetadata should correctly identify server metadata", () => {
			...
			assert.throws(
				() => isAuthorizationServerMetadata({}),
				/Authorization server metadata must have an issuer/,
			);
			assert.throws(
				() =>
	...
	suite("Scope Matching", () => {
		...
		test("scopesMatch should return true for scopes in different order", () => {
			const scopes1 = [
				"6f1cc985-85e8-487e-b0dd-aa633302a731/.default",
	...
	suite("Parsing Functions", () => {
		...
		test("parseWWWAuthenticateHeader should correctly parse parameters with equal signs", () => {
			const result = parseWWWAuthenticateHeader(
				'Bearer resource_metadata="https://example.com/.well-known/oauth-protected-resource?v=1"',
			);
			assert.strictEqual(result.length, 1);
			assert.strictEqual(result[0].scheme, "Bearer");
			assert.deepStrictEqual(result[0].params, {
				resource_metadata:
					"https://example.com/.well-known/oauth-protected-resource?v=1",
			});
		});

		test("parseWWWAuthenticateHeader should correctly parse multiple", () => {
			const result = parseWWWAuthenticateHeader(
				'Bearer realm="api", error="invalid_token", error_description="The access token expired", Basic realm="hi"',
			);
	...
	suite("fetchResourceMetadata", () => {
		...
		test("should successfully fetch and validate resource metadata", async () => {
			...
			assert.strictEqual(
				fetchStub.firstCall.args[1].headers["Accept"],
				"application/json",
			);
		});
		...
		test("should not include same-origin headers when origins differ", async () => {
			const targetResource = "https://example.com/api";
			const resourceMetadataUrl =
				"https://other-domain.com/.well-known/oauth-protected-resource";
			const sameOriginHeaders = {
				"X-Test-Header": "test-value",
			};
			const expectedMetadata = {
		...
		test("should throw error when fetch returns non-200 status", async () => {
			...
			await assert.rejects(
				async () =>
					fetchResourceMetadata(targetResource, resourceMetadataUrl, {
						fetch: fetchStub,
					}),
				(error: any) => {
					...
					assert.ok(
						error instanceof AggregateError ||
							/Failed to fetch resource metadata from.*404 Not Found/.test(
								error.message,
							),
					);
					return true;
		...
		test("should handle error when response.text() throws", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					assert.ok(
						error instanceof AggregateError ||
							/Failed to fetch resource metadata from.*500 Internal Server Error/.test(
								error.message,
							),
					);
					return true;
				...
				(error: any) => {
					...
					assert.ok(
						error.errors.some((e: Error) =>
							/does not match expected value/.test(e.message),
						),
					);
					return true;
				},
		...
		test("should throw error when response is not valid resource metadata", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					assert.ok(
						error instanceof AggregateError ||
							/Invalid resource metadata/.test(error.message),
					);
					return true;
				},
			);
		...
		test("should handle same origin with different ports", async () => {
			...
			const resourceMetadataUrl =
				"https://example.com:9090/.well-known/oauth-protected-resource";
			const sameOriginHeaders = {
				"X-Test-Header": "test-value",
			};
			const metadata = {
		...
		test("should include error details in message with resource values", async () => {
			...
			try {
				await fetchResourceMetadata(targetResource, resourceMetadataUrl, {
					fetch: fetchStub,
				});
				...
				const errorMessage =
					error instanceof AggregateError
						...
						: error.message;
				assert.ok(
					/does not match expected value/.test(errorMessage),
					"Error message should mention mismatch",
				);
				assert.ok(
					/https:\/\/different\.com\/other/.test(errorMessage),
					"Error message should include actual resource value",
				);
				assert.ok(
					/https:\/\/example\.com\/api/.test(errorMessage),
					"Error message should include expected resource value",
				);
			}
		});
		...
		test("should fallback to well-known URI with path when no resourceMetadataUrl provided", async () => {
			...
			const result = await fetchResourceMetadata(targetResource, undefined, {
				fetch: fetchStub,
			});
		...
		test("should throw error when all well-known URIs fail", async () => {
			...
			await assert.rejects(
				async () =>
					fetchResourceMetadata(targetResource, undefined, {
						fetch: fetchStub,
					}),
				(error: any) => {
					assert.ok(
						error instanceof AggregateError,
						"Should be an AggregateError",
					);
					assert.strictEqual(error.errors.length, 2, "Should contain 2 errors");
					assert.ok(
						/Failed to fetch resource metadata from.*\/api\/v1.*404/.test(
							error.errors[0].message,
						),
						"First error should mention /api/v1 and 404",
					);
					assert.ok(
						/Failed to fetch resource metadata from.*\.well-known.*404/.test(
							error.errors[1].message,
						),
						"Second error should mention .well-known and 404",
					);
					return true;
				},
			);
		...
		test("should throw AggregateError when fetchImpl throws on all URLs", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					assert.strictEqual(error.errors.length, 2, "Should contain 2 errors");
					assert.ok(
						/Network connection failed/.test(error.errors[0].message),
						"First error should mention network failure",
					);
					assert.ok(
						/Network connection failed/.test(error.errors[1].message),
						"Second error should mention network failure",
					);
					return true;
				},
			);
		...
		test("should handle mix of fetch error and non-200 response", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					assert.strictEqual(error.errors.length, 2, "Should contain 2 errors");
					assert.ok(
						/Connection timeout/.test(error.errors[0].message),
						"First error should be network error",
					);
					assert.ok(
						/Failed to fetch resource metadata.*404/.test(
							error.errors[1].message,
						),
						"Second error should be 404",
					);
					return true;
				},
			);
		...
		test("should reject PRM with full path resource when using root discovery fallback", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					assert.ok(
						/does not match expected value/.test(error.errors[1].message),
					);
					// Check that validation was against root URL (origin) not full path
					assert.ok(
						/https:\/\/example\.com\/api\/v1.*https:\/\/example\.com/.test(
							error.errors[1].message,
						),
					);
					return true;
		...
		test("should handle fetchImpl throwing error with explicit resourceMetadataUrl", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					assert.ok(
						error instanceof AggregateError ||
							/DNS resolution failed/.test(error.message),
					);
					return true;
				},
			);
	...
	suite("fetchAuthorizationServerMetadata", () => {
		...
		test("should include additional headers in all requests", async () => {
			...
			const result = await fetchAuthorizationServerMetadata(
				authorizationServer,
				{ fetch: fetchStub, additionalHeaders },
			);
		...
		test("should throw AggregateError when all discovery endpoints fail", async () => {
			...
			await assert.rejects(
				async () =>
					fetchAuthorizationServerMetadata(authorizationServer, {
						fetch: fetchStub,
					}),
				(error: any) => {
					assert.ok(
					...
					assert.strictEqual(
						error.errors.length,
						3,
						"Should contain 3 errors (one for each URL)",
					);
					assert.strictEqual(
						error.message,
					...
					assert.ok(
						/oauth-authorization-server.*404/.test(error.errors[0].message),
						"First error should mention OAuth discovery and 404",
					);
					assert.ok(
						/openid-configuration.*404/.test(error.errors[1].message),
						"Second error should mention OpenID path insertion and 404",
					);
					assert.ok(
						/openid-configuration.*404/.test(error.errors[2].message),
						"Third error should mention OpenID path addition and 404",
					);
					return true;
				},
			);
		...
		test("should throw AggregateError when multiple URLs fail with mixed error types", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					// Second error is 404
					assert.ok(
						/404.*Not Found/.test(error.errors[1].message),
						"Second error should be 404",
					);
					// Third error is 500
					assert.ok(
						/500.*Internal Server Error/.test(error.errors[2].message),
						"Third error should be 500",
					);
					return true;
				},
			);
		...
		test("should throw error when network fails on all endpoints", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					assert.ok(
						/Network error/.test(error.errors[0].message),
						"First error should be network error",
					);
					assert.ok(
						/Network error/.test(error.errors[1].message),
						"Second error should be network error",
					);
					assert.ok(
						/Network error/.test(error.errors[2].message),
						"Third error should be network error",
					);
					return true;
				},
			);
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					for (const err of error.errors) {
						assert.ok(
							/500 Internal Server Error/.test(err.message),
							`Error should mention 500 and statusText: ${err.message}`,
						);
					}
					return true;
				},
		...
		test("should handle deeply nested paths", async () => {
			...
			assert.strictEqual(
				result.discoveryUrl,
				"https://auth.example.com/.well-known/oauth-authorization-server/tenant/org/sub",
			);
			assert.deepStrictEqual(result.errors, []);
			assert.strictEqual(fetchStub.callCount, 1);
		...
		test("should handle 200 response with non-metadata JSON", async () => {
			...
			await assert.rejects(
				...
				(error: any) => {
					...
					for (const err of error.errors) {
						assert.ok(
							/Failed to fetch authorization server metadata from/.test(
								err.message,
							),
							`Error should mention failed fetch: ${err.message}`,
						);
					}
					return true;
				},
