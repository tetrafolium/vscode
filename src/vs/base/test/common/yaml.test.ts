...
function assertScalar(
	...
	expected: {
		value: string;
		format?: "single" | "double" | "none" | "literal" | "folded";
	},
): void {
...
function assertMap(
	...
	assert.strictEqual(
		...
		expectedKeyCount,
		`Expected ${expectedKeyCount} properties but got ${map.properties.length}`,
	);
	return map;
}
...
function assertSequence(
	...
	assert.strictEqual(
		node.type,
		"sequence",
		`Expected sequence but got ${node.type}`,
	);
	const seq = node as YamlSequenceNode;
	assert.strictEqual(
		seq.items.length,
		expectedItemCount,
		`Expected ${expectedItemCount} items but got ${seq.items.length}`,
	);
	return seq;
}
...
suite("YAML Parser", () => {
	...
	suite("Scalars", () => {
		...
		test("literal block scalar format", () => {
			const input = ["text: |", "  line one", "  line two"].join("\n");
			const node = parseOk(input);
			const map = assertMap(node, 1);
			assertScalar(input, map.properties[0].value, {
				value: "line one\nline two\n",
				format: "literal",
			});
		});
	...
	suite("Block mappings", () => {
		...
		test("multiple key-value pairs", () => {
			const input = ["name: John Doe", "age: 30"].join("\n");
			const node = parseOk(input);
		...
		test("nested mappings", () => {
			...
			assertScalar(input, address.properties[0].value, {
				value: "123 Main St",
			});
		});
		...
		test("mapping with quoted keys and values", () => {
			...
			assert.strictEqual(
				(map.properties[0].value as YamlScalarNode).format,
				"single",
			);
		});
	...
	suite("Block sequences", () => {
		test("simple sequence", () => {
			const input = ["- Apple", "- Banana", "- Cherry"].join("\n");
			const node = parseOk(input);
	...
	suite("Flow sequences", () => {
		...
		test("flow sequence with quoted strings", () => {
			...
			const node = parseOk(input);
			const seq = assertSequence(node, 3);
			assertScalar(input, seq.items[0], {
				value: "Sammy Sosa",
				format: "single",
			});
		});
	...
	suite("Mixed structures", () => {
		test("object with scalars, arrays, inline objects and arrays", () => {
			...
			assertScalar(input, obj.properties[1].value, {
				value: "Example City",
				format: "double",
			});
			...
			assertScalar(input, arr.items[1], {
			...
			assertScalar(input, inlineArr.items[0], {
				value: "Sammy Sosa",
				format: "single",
			});
	...
	suite("Comments", () => {
		test("comment-only lines are ignored", () => {
			const input = ["# This is a comment", "name: John"].join("\n");
			const node = parseOk(input);
			const map = assertMap(node, 1);
			assert.strictEqual(map.properties[0].key.value, "name");
		});
		...
		test("inline comment after value", () => {
			...
			assertScalar(input, rbi.items[1], {
				value: "Ken Griffey#part of the value, not a comment",
			});
		});
	});
	...
	suite("Error handling", () => {
		...
		test("duplicate keys emit errors", () => {
			const errors: YamlParseError[] = [];
			const input = ["name: John", "name: Jane"].join("\n");
			const node = parse(input, errors);
			assertMap(node, 2);
		...
		test("wrong indentation emits error but still parses", () => {
			const errors: YamlParseError[] = [];
			const input = ["parent:", "  child1: a", "    child2: b"].join("\n");
			const node = parse(input, errors);
			assert.ok(node);
	...
	suite("Nested sequences", () => {
		test("block sequence in block sequence (dash-dash)", () => {
			const input = ["- - s1_i1", "  - s1_i2", "- s2"].join("\n");
		...
		test("sequence at same indent as parent mapping key", () => {
			const input = ["one:", "- 2", "- 3", "four: 5"].join("\n");
		...
		test("sequence indented under mapping key", () => {
			const input = ["foo:", "  - 42", "bar:", "  - 44"].join("\n");
			const map = assertMap(parseOk(input), 2);
	...
	suite("Multiline plain scalars", () => {
		...
		test("multiline scalar with multiple continuation lines", () => {
			...
			const map = assertMap(parseOk(input), 1);
			assertScalar(input, map.properties[0].value, {
				value: "This unquoted scalar spans many lines.",
			});
		});
		...
		test("multiline scalar with empty line preserves newline", () => {
			const input = ["a: val1", " val2", "", " val3"].join("\n");
			const map = assertMap(parseOk(input), 1);
		...
		test("multiline scalar stops at comment", () => {
			const input = ["value1", "# a comment", "value2"].join("\n");
	...
	suite("Old test suite", () => {
		...
		test("flow map with quoted keys and values", () => {
			...
			const node = parseOk(input);
			const map = assertMap(node, 2);
			assertScalar(input, map.properties[0].key, {
				value: "name",
				format: "double",
			});
			assertScalar(input, map.properties[0].value, {
			...
			assertScalar(input, map.properties[1].key, {
				value: "age",
				format: "double",
			});
			assertScalar(input, map.properties[1].value, { value: "30" });
		...
		test("inline array with comment continuation", () => {
			const input = ["[one # comment about two", ",two, three]"].join("\n");
			const node = parseOk(input);
			const seq = assertSequence(node, 3);
		...
		test("multi-line flow sequence", () => {
			const input = ["[", "    geen, ", "    yello, red]"].join("\n");
			const node = parseOk(input);
			const seq = assertSequence(node, 3);
		...
		test("mapping with sequence containing a mapping", () => {
			const input = ["items:", "- name: John", "  age: 30"].join("\n");
			const node = parseOk(input);
			const map = assertMap(node, 1);
		...
		test("flow map with nested flow sequence", () => {
			...
			assertScalar(input, config.properties[0].value, {
				value: "prod",
				format: "double",
			});
		...
		test("unexpected indentation with recovery", () => {
			const errors: YamlParseError[] = [];
			const input = ["key: 1", "    stray: value"].join("\n");
			const node = parse(input, errors);
			const map = assertMap(node, 2);
		...
		test("multiple keys with empty values", () => {
			const errors: YamlParseError[] = [];
			const input = ["key1:", "key2:", "key3:"].join("\n");
			const node = parse(input, errors);
			const map = assertMap(node, 3);
			assertScalar(input, map.properties[0].key, { value: "key1" });
			assertScalar(input, map.properties[0].value, { value: "" });
		...
		test("comment in unclosed flow sequence", () => {
			const input = ["mode: agent", "tools: [#r"].join("\n");
			const errors: YamlParseError[] = [];
			const node = parse(input, errors);
			const map = assertMap(node, 2);
	...
	suite("parseMarkdown", () => {
		...
		test("frontmatter with body", () => {
			...
			assert.strictEqual(
				(map.properties[0].value as YamlScalarNode).value,
				"Hello",
			);
			assert.strictEqual(
				(map.properties[1].value as YamlScalarNode).value,
				"World",
			);
		...
		test("comment-only frontmatter strips delimiters and preserves body", () => {
			const input = ["---", "# note", "---", "Body text here."].join("\n");
			const result = parseFrontMatter(input);
			assert.ok(result);
			assert.strictEqual(result.header, undefined);
		...
		test("getStringValue returns the scalar for a known key", () => {
			...
			assert.deepStrictEqual(result.getStringArrayValue("tools"), [
				"foo",
				"bar",
			]);
		});
		...
		test("getStringArrayValue returns array for a sequence key", () => {
			...
			const result = parseFrontMatter(input);
			assert.ok(result);
			assert.deepStrictEqual(result.getStringArrayValue("tags"), [
				"foo",
				"bar",
				"baz",
			]);
		...
		test("getStringArrayValue splits comma-separated scalar into array", () => {
			const input = ["---", "tags: foo, bar, baz", "---"].join("\n");
			const result = parseFrontMatter(input);
			assert.ok(result);
			assert.deepStrictEqual(result.getStringArrayValue("tags"), [
				"foo",
	...
	suite("parseCommaSeparatedList", () => {
		...
		test("mixed quoted and unquoted items", () => {
			...
			assert.deepStrictEqual(
				[items[0].value, items[0].format],
				["plain", "none"],
			);
			assert.deepStrictEqual(
				[items[1].value, items[1].format],
				["double", "double"],
			);
			assert.deepStrictEqual(
				[items[2].value, items[2].format],
				["single", "single"],
			);
		});
		...
		test("offsets are relative to the provided offset", () => {
			...
			for (const item of items) {
				assert.strictEqual(
					doc.substring(item.startOffset, item.endOffset),
					item.rawValue,
				);
			}
		});
