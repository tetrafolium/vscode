...
suite("Glob", () => {
	...
	function assertGlobMatch(
		...
		assert(
			glob.match(pattern, input, { ignoreCase }),
			`${JSON.stringify(pattern)} should match ${input}`,
		);
		assert(
			glob.match(pattern, nativeSep(input), { ignoreCase }),
	...
	test("expression support (single)", function () {
		...
		assert.strictEqual(
			"**/*.js",
			glob.parse(expression)("test.js", undefined, hasSibling),
		);
		assert.strictEqual(
			glob.parse(expression)("test.js", undefined, () => false),
			null,
		);
		assert.strictEqual(
			glob.parse(expression)("test.js", undefined, (name) => name === "te.ts"),
			null,
		);
	...
	test("expression support (multiple)", function () {
		...
		assert.strictEqual(
			"**/*.as",
			glob.parse(expression)("test.as", undefined, hasSibling),
		);
		assert.strictEqual(
			"**/*.bananas",
			glob.parse(expression)("test.bananas", undefined, hasSibling),
		);
		assert.strictEqual(
			"**/*.bananas",
			glob.parse(expression)("test.bananas", undefined),
		);
		assert.strictEqual(
			glob.parse(expression)("test.foo", undefined, hasSibling),
			null,
		);
	});
	...
	test("expression with two basename globs and a siblings expression", function () {
		...
		assert.strictEqual(
			glob.parse(expr)("bar", undefined, hasSibling),
			"**/bar",
		);
		assert.strictEqual(glob.parse(expr)("foo", undefined, hasSibling), null);
		assert.strictEqual(
			glob.parse(expr)("foo/bar", undefined, hasSibling),
			"**/bar",
		);
		if (isWindows) {
		...
		assert.strictEqual(
			glob.parse(expr)("foo/foo", undefined, hasSibling),
			null,
		);
		assert.strictEqual(
			glob.parse(expr)("foo.js", undefined, hasSibling),
			"**/*.js",
		);
