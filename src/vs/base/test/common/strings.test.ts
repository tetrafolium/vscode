...
suite("Strings", () => {
	...
	test("rtrim", () => {
		...
		assert.strictEqual(strings.rtrim("---hello", "---"), "---hello");
		assert.strictEqual(
			strings.rtrim("hello world" + "---".repeat(10), "---"),
			"hello world",
		);
	...
	test("isFullWidthCharacter", () => {
		...
		assert.strictEqual(
			strings.isFullWidthCharacter("Ａ".charCodeAt(0)),
			true,
			"Ａ U+FF21 fullwidth A",
		);
		assert.strictEqual(
			strings.isFullWidthCharacter("？".charCodeAt(0)),
			true,
		...
		// Hiragana (3040-309F)
		assert.strictEqual(
			strings.isFullWidthCharacter("あ".charCodeAt(0)),
			true,
			"あ U+3042 hiragana",
		);
		...
		assert.strictEqual(
			strings.isFullWidthCharacter("A".charCodeAt(0)),
			false,
			"A regular ASCII",
		);
		assert.strictEqual(
	...
	test("startsWithUTF8BOM", () => {
		...
		assert(
			strings.startsWithUTF8BOM(strings.UTF8_BOM_CHARACTER + "aaaaaaaaaa"),
		);
		assert(!strings.startsWithUTF8BOM(" " + strings.UTF8_BOM_CHARACTER));
	...
	test("getGraphemeBreakType", () => {
		assert.strictEqual(
			strings.getGraphemeBreakType(0xbc1),
	...
	test("removeAnsiEscapeCodesFromPrompt", () => {
		assert.strictEqual(
			strings.removeAnsiEscapeCodesFromPrompt("\u001b[31m$ \u001b[0m"),
			"$ ",
		);
		assert.strictEqual(
	...
	test("containsInvisibleCharacter", () => {
		...
		assert.strictEqual(
			strings.InvisibleCharacters.containsInvisibleCharacter("a\u{e004e}b"),
			true,
		);
		assert.strictEqual(
			strings.InvisibleCharacters.containsInvisibleCharacter(
	...
	test("multibyteAwareBtoa", () => {
		...
		assert.ok(
			strings.multibyteAwareBtoa(new Array(100000).fill("vs").join("")).length >
				0,
		); // https://github.com/microsoft/vscode/issues/112013
	});
