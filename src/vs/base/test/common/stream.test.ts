...
suite("Stream", () => {
	...
	test("WriteableStream - end with empty string works", async () => {
		const reducer = (strings: string[]) =>
			strings.length > 0 ? strings.join() : "error";
	...
	test("peekReadable", () => {
		for (let i = 0; i < 5; i++) {
			...
			const consumedOrReadable = peekReadable(
				readable,
			...
			if (typeof consumedOrReadable === "string") {
				...
				const consumed = consumeReadable(consumedOrReadable, (strings) =>
					strings.join(),
				);
				assert.strictEqual(consumed, "1,2,3,4,5");
			}
	...
	test("peekStream", async () => {
		for (let i = 0; i < 5; i++) {
			...
			if (result.ended) {
				...
				assert.strictEqual(
					[...result.buffer, ...additionalResult].join(),
					"1,2,3,4,5",
				);
			}
		}
	...
	test("prefixedReadable", () => {
		...
		// Empty
		readable = prefixedReadable("empty", arrayToReadable<string>([]), (val) =>
			val.join(","),
		);
		assert.strictEqual(
			consumeReadable(readable, (val) => val.join(",")),
	...
	test("prefixedStream", async () => {
		...
		let prefixStream = prefixedStream<string>("1,2", stream, (val) =>
			val.join(","),
		);
		assert.strictEqual(
		...
		prefixStream = prefixedStream<string>("1,2", stream, (val) =>
			val.join(","),
		);
		assert.strictEqual(
			await consumeStream(prefixStream, (val) => val.join(",")),
		...
		prefixStream = prefixedStream<string>("error", stream, (val) =>
			val.join(","),
		);
