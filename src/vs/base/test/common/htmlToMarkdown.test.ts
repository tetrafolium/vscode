...
suite("htmlToMarkdown", () => {
	...
	test("converts headings", () => {
		...
		assert.strictEqual(
			convertHtmlToMarkdown("<h2>Subtitle</h2>"),
			"## Subtitle",
		);
		assert.strictEqual(
			convertHtmlToMarkdown("<h3>Section</h3>"),
			"### Section",
		);
		assert.strictEqual(
			convertHtmlToMarkdown("<h4>Sub-section</h4>"),
			"#### Sub-section",
		);
		assert.strictEqual(convertHtmlToMarkdown("<h5>Minor</h5>"), "##### Minor");
		assert.strictEqual(
			convertHtmlToMarkdown("<h6>Smallest</h6>"),
			"###### Smallest",
		);
	});
	...
	test("converts syntax-highlighted code blocks by stripping inner tags", () => {
		assert.strictEqual(
			convertHtmlToMarkdown(
				'<pre><code><span class="kw">const</span> x = <span class="num">1</span>;</code></pre>',
			),
			"```\nconst x = 1;\n```",
		);
	});
	...
	test("converts line breaks", () => {
		...
		assert.strictEqual(
			convertHtmlToMarkdown("hello<br/>world"),
			"hello\nworld",
		);
	});
	...
	test("converts horizontal rules", () => {
		assert.strictEqual(
			convertHtmlToMarkdown("above<hr>below"),
			"above\n---\nbelow",
		);
	});
	...
	test("converts strikethrough", () => {
		assert.strictEqual(
			convertHtmlToMarkdown("<del>removed</del>"),
			"~~removed~~",
		);
	...
	test("strips unknown tags", () => {
		assert.strictEqual(
			convertHtmlToMarkdown('<span class="x">hello</span>'),
			"hello",
		);
	});
