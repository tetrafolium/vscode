...
export function renderSwimlanes(history: ExecutionHistory): string {
	...
	for (let i = 0; i < n; i++) {
		...
		displayLabelOf[i] =
			frames.length > 0 ? `${e.label} · ${frames[0]}` : e.label;
	...
	const maxTime =
		n > 0 ? Math.max(...events.map((e) => Math.round(e.time))) : 0;
...
export function renderLaneGraph(history: ExecutionHistory): string {
	...
	for (const e of events) {
		...
		const parent = e.parent
			? nodeByEvent.get(e.parent)!
			: syntheticForRoot.get(e.root);
		const node: Node = {
	...
	if (totalCols === 0) {
		return events
			.map((e) => `[+${`${e.time}ms`.padStart(5)}] ${e.label}`)
			.join("\n");
	}
	...
	for (let i = 0; i < n; i++) {
		...
		const last =
			pIdx >= 0 && childrenOf[pIdx][childrenOf[pIdx].length - 1] === i;
