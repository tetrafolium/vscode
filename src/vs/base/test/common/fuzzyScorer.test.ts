...
suite("Fuzzy Scorer", () => {
	...
	test("scoreItem - matches are proper", function () {
		...
		// Ellipsis Match
		const ellipsisRes = scoreItem(
			resource,
			"…me/path/someFile123.txt",
			true,
			ResourceAccessor,
		);
	...
	test("fuzzyScore2 (multiple queries)", function () {
		...
		function assertScore() {
			assert.ok(
				(multiScore ?? 0) >= (firstSingleScore ?? 0) + (secondSingleScore ?? 0),
			);
	...
	test("Workspace symbol search with special characters (#, *)", function () {
		...
		assert.ok(
			...
			'Should match "main" symbol when query is "main#"',
		);
		...
		assert.ok(
			typeof score === "number" && score > 0,
			'Should match "foo" symbol when query is "foo*"',
		);
		assert.ok(matches.length > 0);
		...
		assert.ok(
			typeof score === "number" && score > 0,
			'Should match "MyClass" symbol when query is "MyClass#*"',
		);
		assert.ok(matches.length > 0);
		...
		assert.ok(
			typeof score === "number" && score > 0,
			'Should fuzzy match "MyClass" symbol when query is "MC#"',
		);
		assert.ok(matches.length > 0);
		...
		assert.ok(
			typeof score === "number" && score > 0,
			'Should match "#SpecialFunction" symbol when query is "#SpecialFunction"',
		);
		assert.ok(matches.length > 0);
		...
		assert.strictEqual(
			query.normalized,
			"#",
			"Standalone # should not be removed",
		);
		[score, matches] = _doScore2("#", "#");
		assert.ok(
			typeof score === "number" && score > 0,
			'Should match "#" symbol when query is "#"',
		);
		assert.ok(matches.length > 0);
	});
