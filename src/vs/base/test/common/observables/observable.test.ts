...
suite("observables", () => {
	...
	suite("autorunPerKeyedItem", () => {
		test("runs setup once per key, fires per-key observable on in-place value change, disposes on removal", () => {
			const log = new Log();
			const items = observableValue<readonly { id: string; v: number }[]>(
				"items",
				[],
			);
		...
		test("does not fire per-key observable when same item identity is reused", () => {
			...
			ds.add(
				autorunPerKeyedItem(
					...
					(_key, value, store) => {
						store.add(
							autorun((reader) => log.log(`v=${value.read(reader).v}`)),
						);
					},
				),
			);
		...
		test("per-key setup fires when items derived through observableFromEvent chain updates", () => {
			...
			const sessionState$ = observableFromEvent(
				undefined,
				fakeSub.onDidChange,
				() => fakeSub.value,
			);
			...
			ds.add(
				autorunPerKeyedItem(
					...
					(key, p$, store) => {
						...
						store.add(
							autorun((reader) =>
								log.log(`${key}=${p$.read(reader).content.length}`),
							),
						);
					},
				),
			...
			fire({
				active: { id: "t1", parts: [{ id: "p1", content: "hello world" }] },
			});
