...
suite("throttledObservable", () => {
	...
	test("emits initial value immediately without delay", () =>
	...
	test("delays first change by throttleMs", () =>
		runWithFakedTimers({}, async () => {
			const log: string[] = [];
			const source = observableValue("source", 0);
			const throttled = throttledObservable(source, 100);
	...
	test("does not reset timer on subsequent changes (no starvation)", () =>
		runWithFakedTimers({}, async () => {
			const log: string[] = [];
			const source = observableValue("source", 0);
			const throttled = throttledObservable(source, 100);
	...
	test("emits again after previous throttle window completes", () =>
		runWithFakedTimers({}, async () => {
			const log: string[] = [];
			const source = observableValue("source", 0);
			const throttled = throttledObservable(source, 100);
	...
	test("records correct timing sequence", () =>
		runWithFakedTimers({}, async () => {
	...
	test("cleanup on dispose stops pending timer", () =>
		runWithFakedTimers({}, async () => {
			const log: string[] = [];
			const source = observableValue("source", 0);
			const throttled = throttledObservable(source, 100);
