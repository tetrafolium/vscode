...
function traceInfo(t: Trace): {
	labels: string[];
	rootLabel: string;
	depth: number;
} {
	const labels: string[] = [];
	for (let c: Trace | undefined = t; c; c = c.parent) {
		labels.push(c.label);
	}
...
suite("virtualScheduling - Trace + TraceContext", () => {
	...
	test("runAsHandler leaks trace across awaited microtasks", async () => {
		...
		await TraceContext.instance.runAsHandler(
			root,
			async () => {
	...
	test("virtual setInterval: each tick gets a fresh child trace", async () => {
		await runWithFakedTimers({}, async () => {
			...
			assert.deepStrictEqual(observed.map(traceInfo), [
				{
					labels: ["tick #1", "setInterval(5ms)", "root"],
					rootLabel: "root",
					depth: 2,
				},
				{
					labels: ["tick #2", "setInterval(5ms)", "root"],
					rootLabel: "root",
					depth: 2,
				},
				{
					labels: ["tick #3", "setInterval(5ms)", "root"],
					rootLabel: "root",
					depth: 2,
				},
			]);
...
suite("virtualScheduling - VirtualTimeProcessor termination policies", () => {
	...
	test("untilTime: pre-scheduled events run before deadline", async () => {
		...
		await p.run({ until: untilTime(100) });
		assert.deepStrictEqual(
			{ log, virtualNow: clock.now },
			{ log: ["a"], virtualNow: 100 },
		);
		store.dispose();
	});
