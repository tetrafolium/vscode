...
suite("Iterable", function () {
	...
	test("every", function () {
		...
		assert.strictEqual(
			Iterable.every([], () => false),
			true,
		);

		// All elements match predicate
		assert.strictEqual(
			Iterable.every([2, 4, 6, 8], (x) => x % 2 === 0),
			true,
		);
		assert.strictEqual(
			Iterable.every([1, 2, 3], (x) => x > 0),
			true,
		);

		// Not all elements match predicate
		assert.strictEqual(
			Iterable.every([1, 2, 3, 4], (x) => x % 2 === 0),
			false,
		);
		assert.strictEqual(
			Iterable.every([1, 2, 3], (x) => x > 2),
			false,
		);

		// Single element - matches
		assert.strictEqual(
			Iterable.every([5], (x) => x === 5),
			true,
		);

		// Single element - doesn't match
		assert.strictEqual(
			Iterable.every([5], (x) => x === 6),
			false,
		);
		...
		assert.strictEqual(
			Iterable.every(numbers, (x, i) => x === 10 + i),
			true,
		);
		assert.strictEqual(
			Iterable.every(numbers, (x, i) => i < 2),
			false,
		);
		...
		assert.strictEqual(
			Iterable.every([1, 2, 3], (x) => x),
			true,
		);
		assert.strictEqual(
			Iterable.every([1, 0, 3], (x) => x),
			false,
		);
		assert.strictEqual(
			Iterable.every(["a", "b", "c"], (x) => x),
			true,
		);
		assert.strictEqual(
			Iterable.every(["a", "", "c"], (x) => x),
			false,
		);
	});
});
