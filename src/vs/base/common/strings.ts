...
export function splitLinesIncludeSeparators(str: string): string[] {
	...
	for (let i = 0; i < Math.ceil(splitLinesAndSeparators.length / 2); i++) {
		linesWithSeparators.push(
			splitLinesAndSeparators[2 * i] +
				(splitLinesAndSeparators[2 * i + 1] ?? ""),
		);
	}
...
export function replaceAsync(
	...
	for (const match of str.matchAll(search)) {
		...
		parts.push(
			replacer(match[0], ...match.slice(1), match.index, str, match.groups),
		);
	}
...
export function endsWithIgnoreCase(str: string, candidate: string): boolean {
	...
	return (
		start >= 0 && compareSubstringIgnoreCase(str, candidate, start, len) === 0
	);
}
...
function breakBetweenGraphemeBreakType(
