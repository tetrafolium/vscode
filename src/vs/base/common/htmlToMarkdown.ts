...
export function convertHtmlToMarkdown(html: string): string {
	...
	md = md.replace(/<ol[^>]*>([\s\S]*?)<\/ol>/gi, (_m, inner) => {
		let index = 0;
		const numbered = inner.replace(
			/<li[^>]*>([\s\S]*?)<\/li>/gi,
			(_liM: string, liInner: string) => {
				index++;
	...
	md = md.replace(
		/<li[^>]*>([\s\S]*?)<\/li>/gi,
		(_m, inner) => `- ${inlineClean(inner).trim()}\n`,
	);
	md = md.replace(/<\/?ul[^>]*>/gi, "\n");
	...
	md = md.replace(
		/<a\s[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/gi,
		(_m, href, text) => sanitizeLink(href, inlineClean(text).trim()),
	);
	...
	md = md.replace(
		/<(strong|b)(\s[^>]*)?>([\s\S]*?)<\/\1>/gi,
		(_m, _tag, _attrs, inner) => `**${inlineClean(inner)}**`,
	);
	...
	md = md.replace(
		/<code(\s[^>]*)?>([\s\S]*?)<\/code>/gi,
		(_m, _attrs, inner) => `\`${decodeEntities(inner)}\``,
	);

	// Strikethrough
	md = md.replace(
		/<(del|s|strike)(\s[^>]*)?>([\s\S]*?)<\/\1>/gi,
		(_m, _tag, _attrs, inner) => `~~${inlineClean(inner)}~~`,
	);
...
function decodeEntities(text: string): string {
	return text
		...
		.replace(/&#39;/g, "'")
		.replace(/&nbsp;/g, " ")
		.replace(/&#x(?<hex>[0-9a-fA-F]+);/g, (...args) =>
			safeFromCodePoint(parseInt(args.at(-1).hex, 16)),
		)
		.replace(/&#(?<dec>\d+);/g, (...args) =>
			safeFromCodePoint(parseInt(args.at(-1).dec, 10)),
		);
}
