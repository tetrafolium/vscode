...
export function matchesBaseContiguousSubString(
	word: string,
	wordToMatchAgainst: string,
): IMatch[] | null {
	if (word.length > wordToMatchAgainst.length) {
...
function _matchesWords(
	...
	const computed = _matchesWordsCompute(
		word,
		target,
		wordIndex,
		targetIndex,
...
function _matchesWordsCompute(
	...
	if (
		!charactersMatch(word.charCodeAt(wordIndex), target.charCodeAt(targetIndex))
	) {
