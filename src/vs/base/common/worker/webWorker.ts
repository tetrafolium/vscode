...
class WebWorkerProtocol {
	...
	public handleMessage(message: unknown): void {
		...
		if (
			this._workerId !== -1 &&
			(message as Message).vsWorker !== this._workerId
		) {
			return;
		}
	...
	constructor(
		...
		this._protocol = new WebWorkerProtocol({
			...
			handleMessage: (
				...
				method: string,
				args: unknown[],
			): Promise<unknown> => this._handleMessage(channel, method, args),
			handleEvent: (
				channel: string,
				eventName: string,
				arg: unknown,
			): Event<unknown> => this._handleEvent(channel, eventName, arg),
		});
