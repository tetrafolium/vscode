	...
	private scanLine(): void {
		...
		if (
			...
			if (this.pos < this.input.length) {
				...
				const end =
					this.peekChar() === "\r" && this.input[this.pos + 1] === "\n"
						? this.pos + 2
						: this.pos + 1;
...
class YamlParser {
	...
	private parseMappingValue(baseIndent: number, colonToken: Token): YamlNode {
		...
		return (
			this.parseValue(baseIndent) ?? this.makeEmptyScalar(colonToken.endOffset)
		);
	}
	...
	private parseSequenceItemValue(
		...
		return (
			this.parseValue(baseIndent) ?? this.makeEmptyScalar(dashToken.endOffset)
		);
	}
