...
import type {
	IDiffComputer as IExternalDiffComputer,
	createDiffComputer as createExternalDiffComputer,
} from "@vscode/diff";
...
type ExternalDiffModule = {
	createDiffComputer: typeof createExternalDiffComputer;
};
...
function loadExternalComputer(
	useWasm: boolean,
): Promise<IExternalDiffComputer> {
	if (useWasm) {
		if (!externalWasmDiffComputerPromise) {
			externalWasmDiffComputerPromise = loadExternalModule().then((m) =>
				m.createDiffComputer({ useWasm: true }),
			);
		}
		return externalWasmDiffComputerPromise;
	}
	if (!externalDiffComputerPromise) {
		externalDiffComputerPromise = loadExternalModule().then((m) =>
			m.createDiffComputer({ useWasm: false }),
		);
	}
...
class ExternalLinesDiffComputer implements ILinesDiffComputer {
	...
	computeDiff(
		...
		const result = this._computer.computeDiff(
			originalText.value,
			modifiedText.value,
			{
		...
		for (const r of result.edits.replacements) {
			...
			const originalRange = originalTransformer.getRange(
				new OffsetRange(r.range.start, r.range.endExclusive),
			);
			const modifiedRange = modifiedTransformer.getRange(
				new OffsetRange(modifiedStart, modifiedEndExclusive),
			);
		...
		if (options.computeMoves) {
			for (const move of result.moves) {
				const originalStartPos = originalTransformer.getPosition(
					move.range.original.start,
				);
				const originalEndPos = originalTransformer.getPosition(
					move.range.original.endExclusive,
				);
				const modifiedStartPos = modifiedTransformer.getPosition(
					move.range.modified.start,
				);
				const modifiedEndPos = modifiedTransformer.getPosition(
					move.range.modified.endExclusive,
				);
				const originalLineRange = new LineRange(
					originalStartPos.lineNumber,
					originalEndPos.lineNumber,
				);
				const modifiedLineRange = new LineRange(
					modifiedStartPos.lineNumber,
					modifiedEndPos.lineNumber,
				);
				moves.push(
					new MovedText(
