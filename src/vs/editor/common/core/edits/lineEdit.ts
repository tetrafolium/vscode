...
export class LineEdit {
	...
	public static fromTextEdit(
		...
		for (let i = 0; i < edits.length; i++) {
			...
			const singleEdit = TextReplacement.joinReplacements(
				currentEdits,
				initialValue,
			);
			currentEdits.length = 0;

			const singleLineEdit = LineReplacement.fromSingleTextEdit(
				singleEdit,
				initialValue,
			);
	...
	constructor(
		...
		assert(
			checkAdjacentItems(
				replacements,
				(i1, i2) =>
					i1.lineRange.endLineNumberExclusive <= i2.lineRange.startLineNumber,
			),
		);
	}
	...
	public touches(other: LineEdit): boolean {
		return this.replacements.some((e1) =>
			other.replacements.some((e2) => e1.lineRange.intersect(e2.lineRange)),
		);
	}
	...
	public rebase(base: LineEdit): LineEdit {
		return new LineEdit(
			this.replacements.map(
				(e) => new LineReplacement(base.mapLineRange(e.lineRange), e.newLines),
			),
		);
	}
...
export class LineReplacement {
	...
	public static fromSingleTextEdit(
		...
		const editEndLineNumberMaxColumn =
			initialValue.getTransformer().getLineLength(edit.range.endLineNumber) + 1;
		...
		const startBeforeNewLine =
			...
			initialValue.getTransformer().getLineLength(edit.range.startLineNumber) +
				1;
		...
		if (this.newLines.length === 0) {
			...
			if (this.lineRange.endLineNumberExclusive === textLen.lineCount + 2) {
				...
				if (this.lineRange.startLineNumber > 1) {
					...
					const startColumn =
						initialValue.getTransformer().getLineLength(startLineNumber) + 1;
