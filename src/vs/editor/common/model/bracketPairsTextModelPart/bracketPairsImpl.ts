	...
	implements IBracketPairsTextModelPart
{
	...
	public getBracketPairsInRangeWithMinIndentation(
		...
		this.updateBracketPairsTree();
		return (
			this.bracketPairsTree.value?.object.getBracketPairsInRange(range, true) ||
			CallbackIterable.empty
		);
	}
	...
	public findPrevBracket(_position: IPosition): IFoundBracket | null {
		...
		if (this.canBuildAST) {
			...
			this.updateBracketPairsTree();
			return (
				this.bracketPairsTree.value?.object.getFirstBracketBefore(position) ||
				null
			);
		}
	...
	public findNextBracket(_position: IPosition): IFoundBracket | null {
		...
		if (this.canBuildAST) {
			...
			this.updateBracketPairsTree();
			return (
				this.bracketPairsTree.value?.object.getFirstBracketAfter(position) ||
				null
			);
		}
