...
export class TextModelEditSource {
	...
	public toKey(
		level: number,
		filter: { [TKey in ITextModelEditSourceMetadataKeys]?: boolean } = {},
	): string {
		const metadata = this.metadata;
	...
	public get props(): Record<
		ITextModelEditSourceMetadataKeys,
		string | undefined
	> {
...
function createEditSource<T extends Record<string, any>>(
	metadata: T,
): TextModelEditSourceT<T> {
	// eslint-disable-next-line local/code-no-any-casts, @typescript-eslint/no-explicit-any
...
export type ITextModelEditSourceMetadata = Values<{
	[TKey in keyof typeof EditSources]: ReturnType<
		(typeof EditSources)[TKey]
	>["metadataT"];
}>;
type ITextModelEditSourceMetadataKeys = Values<{
	[TKey in keyof typeof EditSources]: keyof ReturnType<
		(typeof EditSources)[TKey]
	>["metadataT"];
}>;
...
export class EditDeltaInfo {
	...
	public static fromEdit(
		...
		const linesRemoved = sumBy(
			lineEdit.replacements,
			(r) => r.lineRange.length,
		);
...
export namespace EditSuggestionId {
	...
	export function newId(
		genPrefixedUuid?: (ns: string) => string,
	): EditSuggestionId {
