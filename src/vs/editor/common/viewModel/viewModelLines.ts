...
export class ViewModelLinesFromProjectedModel implements IViewModelLines {
	...
	public createLineBreaksComputer(
		_context?: ILineBreaksComputerContext,
	): ILineBreaksComputer {
	...
	public onModelLinesDeleted(
		...
		const outputFromLineNumber =
			fromLineNumber === 1
				? 1
				: this.projectedModelLineLineCounts.getPrefixSum(fromLineNumber - 1) +
					1;
	...
	public onModelLineChanged(
		...
		if (oldOutputLineCount > newOutputLineCount) {
			changeFrom =
				this.projectedModelLineLineCounts.getPrefixSum(lineNumber - 1) + 1;
	...
	private _ensureAtLeastOneVisibleLine(): void {
		if (this.getViewLineCount() === 0 && this.modelLineProjections.length > 0) {
			this.modelLineProjections[0] =
				this.modelLineProjections[0].setVisible(true);
			this.projectedModelLineLineCounts.setValue(
				0,
				this.modelLineProjections[0].getViewLineCount(),
			);
		}
	}
	...
	public getViewLineData(viewLineNumber: number): ViewLineData {
		const info = this.getViewLineInfo(viewLineNumber);
		const baseViewLineNumber =
			this.projectedModelLineLineCounts.getPrefixSum(info.modelLineNumber - 1) +
			1;
	...
	public getViewLinesData(
		...
		for (
			...
			const baseViewLineNumber =
				this.projectedModelLineLineCounts.getPrefixSum(modelLineIndex) + 1;
	...
	public convertModelPositionToViewPosition(
		...
		if (lineIndexChanged) {
			...
			r = this.modelLineProjections[
				inputLineNumber - 1
			].getViewPositionOfModelPosition(deltaLineNumber, inputColumn, affinity);
		}
