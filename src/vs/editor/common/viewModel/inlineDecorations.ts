...
export class InlineModelDecorationsComputer implements IInlineDecorationsComputer {
	...
	public getInlineDecorations(modelLineNumber: number): InlineDecoration[][] {
		...
		const viewRange =
			this.coordinatesConverter.convertModelRangeToViewRange(modelRange);
		...
		for (let i = 0, len = modelDecorations.length; i < len; i++) {
			...
			if (decorationOptions.beforeContentClassName) {
				if (
					...
					if (decorationOptions.affectsFont) {
						hasVariableFonts[viewRange.startLineNumber - startLineNumber] =
							true;
					}
				}
			}
...
export class InjectedTextInlineDecorationsComputer implements IInlineDecorationsComputer {
	constructor(
		private readonly context: IInjectedTextInlineDecorationsComputerContext,
	) {}
	...
	public getInlineDecorations(modelLineNumber: number): InlineDecoration[][] {
		...
		for (
			...
			outputLineIndex < breakOffsets.length;
			outputLineIndex++
		) {
			...
			const lineStartOffsetInInputWithInjections =
				outputLineIndex > 0 ? breakOffsets[outputLineIndex - 1] : 0;
					...
					if (options.inlineClassName) {
						...
						if (start !== end) {
							const viewLineNumber =
								this.context.getBaseViewLineNumber(modelLineNumber) +
								outputLineIndex;
