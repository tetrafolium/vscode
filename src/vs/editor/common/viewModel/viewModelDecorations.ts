...
export class ViewModelDecorations implements IDisposable {
	...
	constructor(
		...
		this._inlineDecorationsComputer = new InlineModelDecorationsComputer(
			context,
			model,
	...
	public getMinimapDecorationsInRange(range: Range): ViewModelDecoration[] {
		return this._inlineDecorationsComputer.getDecorations(range, true, false)
			.decorations;
	}
	...
	public getDecorationsViewportData(
		...
		if (!cacheIsValid) {
			this._cachedModelDecorationsResolver =
				this._inlineDecorationsComputer.getDecorations(viewRange, false, false);
	...
	public getDecorationsOnLine(
		...
		return this._inlineDecorationsComputer.getDecorations(
			range,
			onlyMinimapDecorations,
			onlyMarginDecorations,
		);
