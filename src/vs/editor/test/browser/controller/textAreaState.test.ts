...
suite("TextAreaState", () => {
	...
	function testDeduceAndroidCompositionInput(
		...
		selectionEnd: number,
		expected: string,
		expectedReplacePrevCharCnt: number,
		expectedReplaceNextCharCnt: number,
		expectedPositionDelta: number,
	): void {
		prevState = prevState || TextAreaState.EMPTY;
	...
	suite("SimplePagedScreenReaderStrategy", () => {
		function testPagedScreenReaderStrategy(
			...
			const screenReaderContentState = screenReaderStrategy.fromEditorSelection(
				model,
				selection,
				10,
				true,
