...
suite("TextAreaInput", () => {
	...
	type OutoingEvent =
		| OutgoingType
		| OutgoingCompositionStart
		| OutgoingCompositionUpdate
		| OutgoingCompositionEnd;
	...
	async function simulateInteraction(
		...
		const wrapper = disposables.add(
			new (class extends Disposable implements ICompleteTextAreaWrapper {
				...
				private _onCompositionEnd = this._register(
					new Emitter<CompositionEvent>(),
				);
				...
				setSelectionRange(
					...
					this._state.selectionDirection =
						selectionStart !== selectionEnd ? "forward" : "none";
				}
	...
	function interpretTypeEvents(
		...
		for (const event of events) {
			if (event.type === "type") {
				...
				selectionStart =
					selectionStart - event.replacePrevCharCnt + event.text.length;
