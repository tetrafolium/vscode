...
suite("Editor Controller - Cursor", () => {
	...
	function runTest(
		callback: (editor: ITestCodeEditor, viewModel: ViewModel) => void,
	): void {
	...
	test("Independent model edit 1", () => {
		runTest((editor, viewModel) => {
			...
			editor
				.getModel()
				.applyEdits([EditOperation.delete(new Range(2, 1, 2, 2))]);
...
suite("Editor Controller", () => {
	...
	setup(() => {
		...
		disposables.add(
			languageService.registerLanguage({ id: emptyClosingSurroundLanguageId }),
		);
		disposables.add(
			languageConfigurationService.register(emptyClosingSurroundLanguageId, {
	...
	function setupIndentRulesLanguage(
		languageId: string,
	...
	function usingCursor(
		...
		const editorOptions: TestCodeEditorInstantiationOptions =
			opts.editorOpts || {};
	...
	function extractAutoClosingSpecialColumns(
		maxColumn: number,
		annotatedLine: string,
	): AutoClosingColumnType[] {
	...
	test("issue #256039: paste from multiple cursors with empty selections and multiCursorPaste full", () => {
		usingCursor(
			...
			(editor, model, viewModel) => {
				...
				assert.strictEqual(
					model.getValue(),
					["line1", "line1", "line2", "line2", "line3"].join("\n"),
				);
			},
		);
	});
	...
	test("Double-click on punctuation should select the character, not adjacent space", () => {
		...
		withTestCodeEditor(model, {}, (editor, viewModel) => {
			...
			CoreNavigationCommands.WordSelect.runCoreEditorCommand(viewModel, {
				position: new Position(1, 42),
			});
			assert.deepStrictEqual(
				viewModel.getSelection(),
				new Selection(1, 42, 1, 43),
				"Should select \\ character",
			);
		});
	});
	...
	test("issue #112301: new stickyTabStops feature interferes with word wrap", () => {
		withTestCodeEditor(
			...
			{ wordWrap: "wordWrapColumn", wordWrapColumn: 32, stickyTabStops: true },
			(editor, viewModel) => {
	...
	test("issue #123178: sticky tab in consecutive wrapped lines", () => {
		const model = createTextModel("    aaaa        aaaa", undefined, {
			tabSize: 4,
		});
				...
				for (let i = 0, len = autoClosePositions.length; i < len; i++) {
					...
					for (let column = 1; column < autoCloseColumns.length; column++) {
						...
						if (autoCloseColumns[column] === AutoClosingColumnType.Special1) {
							...
							autoCloseColumns[column] === AutoClosingColumnType.Special2
						) {
							assertType(
								editor,
								model,
...
suite("Overtype Mode", () => {
	...
	test("paste multi-line text", () => {
		...
		withTestCodeEditor(model, {}, (editor, viewModel) => {
			...
			assert.strictEqual(
				model.getValue(EndOfLinePreference.LF),
				["1234aaaaaaa", "bbbbbbb", "123456789"].join("\n"),
				"assert1",
			);
		});
