	...
	function doTest(
		...
		const scrollbarOptions: InternalEditorScrollbarOptions = {
			...
			horizontalSliderSize:
				EditorOptions.scrollbar.defaultValue.horizontalSliderSize,
			verticalScrollbarSize: input.verticalScrollbarWidth,
			verticalSliderSize:
				EditorOptions.scrollbar.defaultValue.verticalSliderSize,
		...
		const lineNumbersOptions: InternalEditorRenderLineNumbersOptions = {
			renderType: input.showLineNumbers
				? RenderLineNumbersType.On
		...
		const actual = EditorLayoutInfoComputer.computeLayout(options, {
			...
			viewLineCount:
				input.maxLineNumber || Math.pow(10, input.lineNumbersDigitCount) - 1,
