...
suite("ViewModel", () => {
	...
	function assertGetPlainTextToCopy(
		text: string[],
		ranges: Range[],
	...
	test("issue #256039: getPlainTextToCopy with multiple cursors and empty selections should return array", () => {
		...
		assertGetPlainTextToCopy(
			USUAL_TEXT,
			[new Range(2, 1, 2, 1), new Range(3, 1, 3, 1)],
			true,
			["line2\n", "line3\n"],
		);
	});
	...
	test("getPlainTextToCopy 0/3 - emptySelectionClipboard", () => {
		assertGetPlainTextToCopy(
			USUAL_TEXT,
			[new Range(2, 2, 2, 2), new Range(2, 3, 2, 3), new Range(3, 2, 3, 2)],
			true,
			["line2\n", "line3\n"],
		);
	});
	...
	test("normalizePosition with multiple touching injected text", () => {
		testViewModel(["just some text"], {}, (viewModel, model) => {
			...
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 8), PositionAffinity.None),
				new Position(1, 8),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 9), PositionAffinity.None),
				new Position(1, 8),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 11), PositionAffinity.None),
				new Position(1, 11),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 12), PositionAffinity.None),
				new Position(1, 11),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 13), PositionAffinity.None),
				new Position(1, 13),
			);

			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 8), PositionAffinity.Left),
				new Position(1, 8),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 9), PositionAffinity.Left),
				new Position(1, 8),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 11), PositionAffinity.Left),
				new Position(1, 8),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 12), PositionAffinity.Left),
				new Position(1, 8),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 13), PositionAffinity.Left),
				new Position(1, 8),
			);

			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 8), PositionAffinity.Right),
				new Position(1, 13),
			);
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 9), PositionAffinity.Right),
				new Position(1, 13),
			);
			assert.deepStrictEqual(
	...
	suite("hidden areas must always leave at least one visible line", () => {
		test("replacing the only visible line content does not make it hidden", () => {
			...
			testViewModel(text, {}, (viewModel, model) => {
				...
				assert.ok(
					...
					`expected at least 1 view line but got ${viewModel.getLineCount()}`,
				);
			});
		});
		...
		test("hidden areas from multiple sources that overlap produce valid merged result", () => {
			...
			testViewModel(text, {}, (viewModel, model) => {
				...
				viewModel.setHiddenAreas(
					[new Range(2, 1, 3, 1), new Range(5, 1, 6, 1), new Range(8, 1, 9, 1)],
					"sourceB",
				);
