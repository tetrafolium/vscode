...
suite("Decoration Render Options", () => {
	...
	test("css properties", () => {
		...
		assert(
			sheet.indexOf(
				`{background:url('${CSS.escape("https://github.com/microsoft/vscode/blob/main/resources/linux/code.png")}') center center no-repeat;background-size:contain;}`,
			) >= 0,
		);
		assert(
			sheet.indexOf(
	...
	test("css properties, gutterIconPaths", () => {
		...
		assert(
			readStyleSheet(styleSheet).indexOf(
				`{background:url('${CSS.escape("data:image/svg+xml;base64,PHN2ZyB4b+")}') center center no-repeat;}`,
			) > 0,
		);
		s.removeDecorationType("example");
		...
		function assertBackground(url1: string, url2: string) {
			...
			assert(
				actual.indexOf(`{background:url('${url1}') center center no-repeat;}`) >
					0 ||
					actual.indexOf(
		...
		if (platform.isWindows) {
			...
			assertBackground(
				CSS.escape("file:///c:/files/miles/more.png"),
				CSS.escape("vscode-file://vscode-app/c:/files/miles/more.png"),
			);
			s.removeDecorationType("example");
			...
			assertBackground(
				CSS.escape("file:///c:/files/foo/b'ar.png"),
				CSS.escape("vscode-file://vscode-app/c:/files/foo/b'ar.png"),
			);
			s.removeDecorationType("example");
		} else {
			...
			assertBackground(
				CSS.escape("file:///Users/foo/bar.png"),
				CSS.escape("vscode-file://vscode-app/Users/foo/bar.png"),
			);
			s.removeDecorationType("example");
			...
			assertBackground(
				CSS.escape("file:///Users/foo/b'ar.png"),
				CSS.escape("vscode-file://vscode-app/Users/foo/b'ar.png"),
			);
			s.removeDecorationType("example");
		}
		...
		assert(
			readStyleSheet(styleSheet).indexOf(
				`{background:url('${CSS.escape("http://test/pa'th")}') center center no-repeat;}`,
			) > 0,
		);
		s.removeDecorationType("example");
	});
