...
function createModelDecoration(
	id: string,
	range: Range,
...
suite("InlineModelDecorationsComputer", () => {
	...
	test("no decorations", () => {
		...
		const computer = new InlineModelDecorationsComputer(
			context,
			model,
			coordinatesConverter,
		);
		const result = computer.getDecorations(
			new Range(1, 1, 1, 12),
			false,
			false,
		);
	...
	test("decoration spanning multiple lines", () => {
		...
		const expectedInlineDecoration = new InlineDecoration(
			new Range(1, 3, 3, 5),
			"multi-line",
			InlineDecorationType.Regular,
		);
	...
	test("decoration cache is used for same decoration id", () => {
		...
		const result1 = computer.getDecorations(
			new Range(1, 1, 1, 12),
			false,
			false,
		);
		const result2 = computer.getDecorations(
			new Range(1, 1, 1, 12),
			false,
			false,
		);
...
suite("InjectedTextInlineDecorationsComputer", () => {
	...
	test("injection without inlineClassName produces no inline decorations", () => {
		const injectionOptions: InjectedTextOptions[] = [{ content: "injected" }];
		const context: IInjectedTextInlineDecorationsComputerContext = {
			getInjectionOptions: () => injectionOptions,
			getInjectionOffsets: () => [5],
			getBreakOffsets: () => [18],
