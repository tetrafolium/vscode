...
function parseAnnotatedText(annotatedText: string): {
	text: string;
	indices: number[];
} {
	let text = "";
...
function assertLineBreaks(
	factory: ILineBreaksComputerFactory,
	tabSize: number,
	breakAfter: number,
	annotatedText: string,
	wrappingIndent = WrappingIndent.None,
	wordBreak: "normal" | "keepAll" = "normal",
): ModelLineProjectionData | null {
...
suite("Editor ViewModel - MonospaceLineBreaksComputer", () => {
	...
	function assertLineBreakDataEqual(
		a: ModelLineProjectionData | null,
		b: ModelLineProjectionData | null,
	): void {
		if (!a || !b) {
		...
		for (let i = 0; i < a.breakOffsetsVisibleColumn.length; i++) {
			const diff =
				a.breakOffsetsVisibleColumn[i] - b.breakOffsetsVisibleColumn[i];
			assert.ok(diff < 0.001);
		}
	}

	function assertIncrementalLineBreaks(
		factory: ILineBreaksComputerFactory,
		text: string,
		tabSize: number,
		breakAfter1: number,
		annotatedText1: string,
		breakAfter2: number,
		annotatedText2: string,
		wrappingIndent = WrappingIndent.None,
		columnsForFullWidthChar: number = 2,
	): void {
	...
	test("issue #258022: wrapOnEscapedLineFeeds: should work correctly after editor resize", () => {
		...
		const initialBreakData = getLineBreakData(
			factory,
			4,
			30,
			2,
		...
		assert.ok(
			initialAnnotatedText.includes("with\\n"),
		...
		const resizedBreakData = getLineBreakData(
			factory,
			4,
			35,
			2,
		...
		const freshBreakData = getLineBreakData(
			factory,
			4,
			35,
			2,
		...
		assert.ok(
			freshAnnotatedText.includes("with\\n"),
