...
function createBoth(values: number[]): IPrefixSumComputer[] {
	...
	const wrapped: IPrefixSumComputer = {
		...
		getPrefixSum: (count: number) =>
			count === 0 ? 0 : psc.getPrefixSum(count - 1),
		getIndexOf: (sum: number) => psc.getIndexOf(sum),
		setValue: (index: number, value: number) => {
			psc.setValue(index, value);
		},
		insertValues: (insertIndex: number, insertArr: number[]) => {
			psc.insertValues(insertIndex, toUint32Array(insertArr));
		},
		removeValues: (start: number, deleteCount: number) => {
...
function forBoth(
	values: number[],
	callback: (psc: IPrefixSumComputer) => void,
): void {
...
suite("Editor ViewModel - PrefixSumComputer", () => {
	...
	test("comprehensive setValue and getIndexOf", () => {
		forBoth([1, 1, 2, 1, 3], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(0),
				new PrefixSumIndexOfResult(0, 0),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(1),
				new PrefixSumIndexOfResult(1, 0),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(2),
				new PrefixSumIndexOfResult(2, 0),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(3),
				new PrefixSumIndexOfResult(2, 1),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(4),
				new PrefixSumIndexOfResult(3, 0),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(5),
				new PrefixSumIndexOfResult(4, 0),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(6),
				new PrefixSumIndexOfResult(4, 1),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(7),
				new PrefixSumIndexOfResult(4, 2),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(8),
				new PrefixSumIndexOfResult(4, 3),
			);
	...
	test("getIndexOf with single element", () => {
		forBoth([5], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(1),
				new PrefixSumIndexOfResult(0, 1),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(4),
				new PrefixSumIndexOfResult(0, 4),
			);
		});
	});
	...
	test("getIndexOf with leading zeroes", () => {
		forBoth([0, 0, 3], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(2),
				new PrefixSumIndexOfResult(2, 2),
			);
		});
	});
	...
	test("getIndexOf with trailing zeroes", () => {
		forBoth([3, 0, 0], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(2),
				new PrefixSumIndexOfResult(0, 2),
			);
		});
	});
	...
	test("getIndexOf with interleaved zeroes", () => {
		forBoth([0, 1, 0, 2, 0], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(2),
				new PrefixSumIndexOfResult(3, 1),
			);
		});
	});
	...
	test("getIndexOf with all ones", () => {
		forBoth([1, 1, 1, 1, 1], (psc) => {
			for (let i = 0; i < 5; i++) {
				assert.deepStrictEqual(
					psc.getIndexOf(i),
					new PrefixSumIndexOfResult(i, 0),
				);
			}
		});
	});
	...
	test("getIndexOf with large value in single element", () => {
		forBoth([1000], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(500),
				new PrefixSumIndexOfResult(0, 500),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(999),
				new PrefixSumIndexOfResult(0, 999),
			);
		});
	});
	...
	test("setValue increase", () => {
		forBoth([1, 2, 3], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(5),
				new PrefixSumIndexOfResult(1, 4),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(6),
				new PrefixSumIndexOfResult(2, 0),
	...
	test("setValue decrease", () => {
		forBoth([1, 5, 3], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(2),
				new PrefixSumIndexOfResult(1, 1),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(3),
				new PrefixSumIndexOfResult(2, 0),
	...
	test("setValue from zero", () => {
		forBoth([0, 0, 0], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(2),
				new PrefixSumIndexOfResult(1, 2),
			);
		});
	});
	...
	test("setValue on first element", () => {
		forBoth([1, 2, 3], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(9),
				new PrefixSumIndexOfResult(0, 9),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(10),
				new PrefixSumIndexOfResult(1, 0),
	...
	test("setValue on last element", () => {
		forBoth([1, 2, 3], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(12),
				new PrefixSumIndexOfResult(2, 9),
			);
		});
	});
	...
	test("set all values to zero then restore", () => {
		forBoth([1, 2, 3], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(3),
				new PrefixSumIndexOfResult(0, 3),
			);
		});
	});
	...
	test("setValue multiple times on same index", () => {
		forBoth([1, 1, 1], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(10),
				new PrefixSumIndexOfResult(1, 9),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(11),
				new PrefixSumIndexOfResult(2, 0),
	...
	test("removeValues from the middle", () => {
		forBoth([1, 2, 3, 4], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(4),
				new PrefixSumIndexOfResult(1, 3),
			);
		});
	});
	...
	test("removeValues single element", () => {
		forBoth([5, 10, 15], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(19),
				new PrefixSumIndexOfResult(1, 14),
			);
		});
	});
	...
	test("large values", () => {
		forBoth([100, 200, 300], (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(99),
				new PrefixSumIndexOfResult(0, 99),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(100),
				new PrefixSumIndexOfResult(1, 0),
			...
			assert.deepStrictEqual(
				psc.getIndexOf(299),
				new PrefixSumIndexOfResult(1, 199),
			);
			assert.deepStrictEqual(
				psc.getIndexOf(300),
				new PrefixSumIndexOfResult(2, 0),
			...
			assert.deepStrictEqual(
				psc.getIndexOf(599),
				new PrefixSumIndexOfResult(2, 299),
			);
		});
	});
	...
	test("many elements all zeroes", () => {
		forBoth(new Array(100).fill(0), (psc) => {
			...
			assert.deepStrictEqual(
				psc.getIndexOf(0),
				new PrefixSumIndexOfResult(99, 0),
			);
		});
	});
