...
suite("Default Document Colors Computer", () => {
	...
	test("hsl 100 percent saturation works with decimals", () => {
		const model = new TestDocumentModel(
			"const color = hsl(253, 100.00%, 47.10%);",
		);
		const colors = computeDefaultDocumentColors(model);
	...
	test("hsl with decimal hue values should work", () => {
		...
		testCases.forEach((testCase) => {
			...
			assert.strictEqual(
				colors.length,
				1,
				`Should detect hsl color with ${testCase.name}: ${testCase.content}`,
			);
		});
	});
	...
	test("hsla with decimal values should work", () => {
		const testCases = [
			{
				content: "hsla(253.5, 100%, 50%, 0.5)",
				name: "decimal hue with alpha",
			},
			{
				content: "hsla(360.0, 50.5%, 50.5%, 1)",
				name: "all decimals with alpha 1",
			},
			{
				content: "hsla(0.5, 50%, 50%, 0.25)",
				name: "small decimal hue with alpha",
			},
		];
		...
		testCases.forEach((testCase) => {
			...
			assert.strictEqual(
				colors.length,
				1,
				`Should detect hsla color with ${testCase.name}: ${testCase.content}`,
			);
		});
	});
	...
	test("hsl with space separator (CSS Level 4 syntax) should work", () => {
		...
		const testCases = [
			...
			{
				content: "hsl(253.5 100% 50%)",
				name: "space-separated with decimal hue",
			},
			{
				content: "hsla(253 100% 50% / 0.5)",
				name: "hsla with slash separator for alpha",
			},
			{
				content: "hsla(253.5 100% 50% / 0.5)",
				name: "hsla with decimal hue and slash separator",
			},
			{
				content: "hsla(253 100% 50% / 1)",
				name: "hsla with slash and alpha 1",
			},
		];
	...
	test("rgb and rgba with CSS Level 4 space-separated syntax should work", () => {
		...
		const testCases = [
			...
			{ content: "rgb(128 128 128)", name: "rgb space-separated gray" },
			{
				...
				name: "rgba with slash separator for alpha",
			},
			{
				content: "rgba(128 128 128 / 0.8)",
				name: "rgba gray with slash separator",
			},
			...
			{
				content: "rgba(255, 0, 0, 0.5)",
				name: "rgba comma-separated (traditional)",
			},
		];
		...
		testCases.forEach((testCase) => {
			...
			assert.strictEqual(
				colors.length,
				1,
				`Should detect rgb/rgba color with ${testCase.name}: ${testCase.content}`,
			);
		});
	});
});
