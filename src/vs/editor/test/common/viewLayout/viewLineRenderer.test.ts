...
function createRenderLineInputOptions(
	opts: IRelaxedRenderLineInputOptions,
): IRenderLineInputOptions {
	return {
...
function createRenderLineInput(
	opts: IRelaxedRenderLineInputOptions,
): RenderLineInput {
...
suite("renderViewLine", () => {
	...
	test("issue-91178", async () => {
		...
		const lineTokens = createViewLineTokens([createPart(16, 1)]);
	...
	test("issue-20624", async () => {
		...
		await assertSnapshot(
			inflateRenderLineOutput(actual).html.join(""),
			HTML_EXTENSION,
		);
	});
