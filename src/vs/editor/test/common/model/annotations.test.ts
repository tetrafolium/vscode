...
import {
	...
	IAnnotation,
	IAnnotationUpdate,
} from "../../../common/model/tokens/annotations.js";
 ...
 */
function parseVisualAnnotations(visual: string): {
	...
	while (i < visual.length) {
		if (visual[i] === "[") {
			...
			baseString += text;
			annotations.push({
				range: new OffsetRange(startOffset, baseString.length),
				annotation: id,
			});
			i = closeIdx + 1;
		} else {
...
function toVisualString(
	...
	for (const ann of sortedAnnotations) {
		...
		const annotatedText = baseString.substring(
			ann.range.start,
			ann.range.endExclusive,
		);
...
class VisualAnnotatedString {
	...
	clone(): VisualAnnotatedString {
		return new VisualAnnotatedString(
			this.annotatedString.clone() as AnnotatedString<string>,
			this.baseString,
		);
	}
}
...
function fromVisual(visual: string): VisualAnnotatedString {
	...
	return new VisualAnnotatedString(
		new AnnotatedString<string>(annotations),
		baseString,
	);
}
 ...
 */
function parseVisualUpdate(visual: string): {
	updates: IAnnotationUpdate<string>[];
	baseString: string;
} {
...
function assertVisual(
	...
	for (let i = 0; i < actualAnnotations.length; i++) {
		...
		if (
			actualAnn.range.start !== expected.range.start ||
			actualAnn.range.endExclusive !== expected.range.endExclusive
		) {
			assert.fail(
...
suite("Annotations Suite", () => {
	...
	test("setAnnotations 2", () => {
		...
		vas.setAnnotations(
			updateFromVisual("Lorem <_:ipsum dolor sit amet>", "[5:Lor]"),
		);
	...
	test("setAnnotations 4", () => {
		...
		const vas = fromVisual(
			"Lorem ipsum dolor sit amet, consectetur adipiscing el[:it]",
		);
		vas.setAnnotations(
			updateFromVisual(
				"Lorem ipsum dolor sit amet, consectetur adipiscing el<_:i>t",
			),
		);
		assertVisual(
			vas,
	...
	test("applyEdit 2 - deletion and insertion within annotation", () => {
		...
		assert.strictEqual(
			result.after,
			"[1:LXXXXXem] ipsum [2:dolor] sit [3:amet]",
		);
	});
	...
	test("applyEdit 4 - deletion between annotations", () => {
		...
		assert.strictEqual(
			result.after,
			"[1:Lorem ip]suand [2:dolor] sit [3:amet]",
		);
	});
	...
	test("applyEdit 9 - insertion at end of annotation", () => {
		...
		assert.strictEqual(
			result.after,
			"[1:Lorem] ipsum [2:dolor]XXX sit [3:amet]",
		);
	});
	...
	test("applyEdit 10 - insertion in middle of annotation", () => {
		...
		assert.strictEqual(
			result.after,
			"[1:Lorem] ipsum [2:doXXXlor] sit [3:amet]",
		);
	});
	...
	test("rebase", () => {
		const a = new VisualAnnotatedString(
			new AnnotatedString<string>([
				{ range: new OffsetRange(2, 5), annotation: "1" },
			]),
			"sitamet",
		);
		const b = a.clone();
		const update: AnnotationsUpdate<string> = AnnotationsUpdate.create([
			{ range: new OffsetRange(4, 5), annotation: "2" },
		]);
