...
import {
	...
	TokenQuality,
	TokenStore,
} from "../../../common/model/tokens/treeSitter/tokenStore.js";
...
suite("TokenStore", () => {
	...
	test("creates deep tree structure", () => {
		...
		assert.ok(
			((root.children[0] as ListNode).children[0] as ListNode).children,
		);
		assert.strictEqual(
			((root.children[0] as ListNode).children[0] as ListNode).children.length,
			2,
		);
	});
	...
	test("replace a token with a slightly larger token", () => {
		...
		store.update(
			...
			TokenQuality.Accurate,
		); // removes 4 chars within token 1 and partially token 2
