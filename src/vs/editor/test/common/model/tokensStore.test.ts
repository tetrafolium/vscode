...
suite("TokensStore", () => {
	...
	test("BUG: setPartial with split that creates empty first piece with invalid line numbers", () => {
			...
			[
				SparseMultilineTokens.create(
					...
					new Uint32Array([
						...
						5,
						10,
						1, // line 11 (deltaLine=10 from startLineNumber=1), chars 5-10
					]),
				),
			],
			false,
		...
		assert.strictEqual(
			store.isEmpty(),
			false,
			"Store should still have the token on line 11",
		);

		// The token at line 11 should be retrievable after the split
		const lineTokens = store.addSparseTokens(
			11,
			new LineTokens(new Uint32Array([22, 1]), `    test line text    `, codec),
		);
		assert.strictEqual(
			lineTokens.getCount(),
			3,
			"Should have 3 tokens: base token start + semantic token from line 11 + base token end",
		);
		assert.strictEqual(
			lineTokens.getStartOffset(1),
			5,
			"Semantic token should start at offset 5",
		);
		assert.strictEqual(
			lineTokens.getEndOffset(1),
			10,
			"Semantic token should end at offset 10",
		);
	});
	...
	test("piece with startLineNumber 0 and endLineNumber -1 after encompassing deletion", () => {
		...
		const piece = SparseMultilineTokens.create(
			5,
			new Uint32Array([
				0,
				0,
		...
		assert.strictEqual(
			piece.isEmpty(),
			true,
			"Piece should be empty after encompassing deletion",
		);
		...
		assert.strictEqual(
			store.isEmpty(),
			true,
			"Store should be empty after all tokens are deleted by encompassing edit",
		);
	});
});
