...
suite("Editor Model - Model", () => {
	...
	function withEventCapturing(
		callback: () => void,
	): ModelRawContentChangedEvent | null {
		let e: ModelRawContentChangedEvent | null = null;
		const spyViewModel = new (class extends mock<IViewModel>() {
			override onDidChangeContentOrInjectedText(
				_e: InternalModelContentChangeEvent | ModelInjectedTextChangedEvent,
			) {
			...
			override emitContentChangeEvent(
	...
	test("model insert text with one newline eventing", () => {
		...
		assert.deepStrictEqual(
			e,
			new ModelRawContentChangedEvent(
				[new ModelRawLineChanged(1, 1), new ModelRawLinesInserted(2, 2, 1)],
				2,
				false,
				false,
			),
	...
	test("model delete text from two lines eventing", () => {
		...
		assert.deepStrictEqual(
			e,
			new ModelRawContentChangedEvent(
				[new ModelRawLineChanged(1, 1), new ModelRawLinesDeleted(2, 2, 1)],
				2,
				false,
				false,
			),
	...
	test("model delete text from many lines eventing", () => {
		...
		assert.deepStrictEqual(
			e,
			new ModelRawContentChangedEvent(
				[new ModelRawLineChanged(1, 1), new ModelRawLinesDeleted(2, 3, 1)],
				2,
				false,
				false,
			),
