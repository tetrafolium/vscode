...
suite("TextModelSearch", () => {
	...
	function assertFindMatch(
		actual: FindMatch | null,
		expectedRange: Range,
		expectedMatches: string[] | null = null,
	): void {
	...
	test("isMultilineRegexSource correctly identifies multiline patterns", () => {
		...
		for (const pattern of singleLinePatterns) {
			assert.strictEqual(
				isMultilineRegexSource(pattern),
				false,
				`Pattern should not be multiline: ${pattern}`,
			);
		}
		...
		for (const pattern of multiLinePatterns) {
			assert.strictEqual(
				isMultilineRegexSource(pattern),
				true,
				`Pattern should be multiline: ${pattern}`,
			);
		}
	});
