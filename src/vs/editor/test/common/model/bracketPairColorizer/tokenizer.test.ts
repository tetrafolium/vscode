...
suite("Bracket Pair Colorizer - Tokenizer", () => {
	...
	test("Basic", () => {
		...
		const encodedMode1 =
			languageService.languageIdCodec.encodeLanguageId(mode1);
		...
		disposableStore.add(
			TokenizationRegistry.register(mode1, document.getTokenizationSupport()),
		);
		disposableStore.add(
...
function toArr(
	tokens: Token[],
	model: TextModel,
	keyProvider: DenseKeyProvider<string>,
): any[] {
