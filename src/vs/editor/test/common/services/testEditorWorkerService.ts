...
export class TestEditorWorkerService implements IEditorWorkerService {
	...
	async computedUnicodeHighlights(uri: URI): Promise<IUnicodeHighlightsResult> {
		return {
			ranges: [],
			hasMore: false,
	...
	async computeMoreMinimalEdits(
		resource: URI,
		edits: TextEdit[] | null | undefined,
	): Promise<TextEdit[] | undefined> {
		return undefined;
	}
	async computeHumanReadableDiff(
		resource: URI,
		edits: TextEdit[] | null | undefined,
	): Promise<TextEdit[] | undefined> {
		return undefined;
	...
	async findSectionHeaders(uri: URI): Promise<SectionHeader[]> {
		return [];
	}
	async computeDefaultDocumentColors(
		uri: URI,
