...
suite("Editor Modes - textToHtmlTokenizer", () => {
	...
	test("tokenizeLineToHTML with tabs and non-zero startOffset #263387", () => {
		...
		const text = "\ta\tb";
		const lineTokens = new TestLineTokens([
			new TestLineToken(1, (1 << MetadataConsts.FOREGROUND_OFFSET) >>> 0),
			new TestLineToken(2, (3 << MetadataConsts.FOREGROUND_OFFSET) >>> 0),
			new TestLineToken(3, (1 << MetadataConsts.FOREGROUND_OFFSET) >>> 0),
			new TestLineToken(4, (4 << MetadataConsts.FOREGROUND_OFFSET) >>> 0),
		]);
