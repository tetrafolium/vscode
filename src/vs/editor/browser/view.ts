...
export class View extends ViewEventHandler {
	...
	private _connectEditContextClipboardEvents(): void {
		...
		this._editContextClipboardListeners.add(
			this._editContext.onWillCopy((e) => this._onWillCopy.fire(e)),
		);
		this._editContextClipboardListeners.add(
			this._editContext.onWillCut((e) => this._onWillCut.fire(e)),
		);
		this._editContextClipboardListeners.add(
			this._editContext.onWillPaste((e) => this._onWillPaste.fire(e)),
		);
	}
	...
	private _computeGlyphMarginLanes(): IGlyphMarginLanesModel {
		...
		glyphs = glyphs.concat(
			model.getAllMarginDecorations().map((decoration) => {
				const lane =
					decoration.options.glyphMargin?.position ?? GlyphMarginLane.Center;
	...
	public change(
		callback: (changeAccessor: IViewZoneChangeAccessor) => unknown,
	): void {
...
class CodeEditorWidgetFocusTracker extends Disposable {
	...
	constructor(
		...
		this._domFocusTracker = this._register(
			userInteractionService.createDomFocusTracker(domElement),
		);
