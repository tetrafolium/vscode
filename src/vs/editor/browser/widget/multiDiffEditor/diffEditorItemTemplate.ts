	...
	implements IPooledObject<TemplateData>
{
	...
	constructor(
		...
		super();
		this._viewModel = observableValue<DocumentDiffItemViewModel | undefined>(
			this,
			undefined,
		);
		...
		this.contentHeight = derived(this, (reader) => {
			const h = this._collapsed.read(reader)
				? 0
				: this._editorContentHeight.read(reader);
		...
		this.isModifedFocused = observableCodeEditor(
			this.editor.getModifiedEditor(),
		).isFocused;
		this.isOriginalFocused = observableCodeEditor(
		...
		if (this._workbenchUIElementFactory.headerClickToCollapse) {
			...
			this._register(
				addDisposableListener(this._elements.header, EventType.CLICK, (e) => {
		...
		this._register(
			autorun((reader) => {
				...
				if (this._workbenchUIElementFactory.headerClickToCollapse) {
					this._elements.header.setAttribute(
						"aria-expanded",
						String(!collapsed),
					);
				}
					...
					actionRunner: this._register(
						new ActionRunnerWithContext(
							() =>
								...
								this._viewModel.get()?.originalUri,
						),
					),
					menuOptions: {
