...
export class MultiDiffEditorWidgetImpl extends Disposable {
	...
	constructor(
					...
					new VirtualizedViewItem(
						...
						(delta) => {
							this._scrollableElement.setScrollPosition({
								scrollTop:
									this._scrollableElement.getScrollPosition().scrollTop + delta,
							});
						},
					),
				);
		...
		this._contextKeyService.createKey(
			EditorContextKeys.inMultiDiffEditor.key,
			true,
		);
	...
	private _navigateToChange(direction: "next" | "previous"): void {
		...
		const currentIndex = activeViewModel
			? viewItems.findIndex((v) => v.viewModel === activeViewModel)
			: -1;
		...
		const nextIndex =
			(currentIndex + (direction === "next" ? 1 : -1) + viewItems.length) %
			viewItems.length;
...
class VirtualizedViewItem extends Disposable {
	private readonly _templateRef = this._register(
		disposableObservableValue<IReference<DiffEditorItemTemplate> | undefined>(
			this,
			undefined,
		),
	);
