...
import {
	OffsetRange,
	OffsetRangeSet,
} from "../../../../common/core/ranges/offsetRange.js";
		...
		const movedBlockViewZones = derived((reader) => {
			...
			return d.movedTexts.map((move) => ({
				...
				original: new PlaceholderViewZone(
					constObservable(move.lineRangeMapping.original.startLineNumber - 1),
					18,
				),
				modified: new PlaceholderViewZone(
					constObservable(move.lineRangeMapping.modified.startLineNumber - 1),
					18,
				),
			}));
		});
			...
			autorunHandleChanges(
				...
				(reader) => {
					...
					if (diff && lastChangedEditor === "original") {
						...
						if (originalPos) {
							movedText = diff.movedTexts.find((m) =>
								m.lineRangeMapping.original.contains(originalPos.lineNumber),
							);
						}
					}
					...
					if (diff && lastChangedEditor === "modified") {
						...
						if (modifiedPos) {
							movedText = diff.movedTexts.find((m) =>
								m.lineRangeMapping.modified.contains(modifiedPos.lineNumber),
							);
						}
					}
