	...
	constructor(
		...
		const unchangedRegions = this._diffModel.map((m, reader) => {
			...
			if (
				...
				regions[0].lineCount ===
					this._editors.modifiedModel.read(reader)?.getLineCount()
			) {
				return [];
			}
			return regions;
				...
				if (compactMode) {
					{
						const d = derived(
							this,
							(reader) =>
								/** @description hiddenOriginalRangeStart */ r.getHiddenOriginalRange(
									reader,
								).startLineNumber - 1,
						);
					...
					}
					{
						const d = derived(
							this,
							(reader) =>
								/** @description hiddenModifiedRangeStart */ r.getHiddenModifiedRange(
									reader,
								).startLineNumber - 1,
						);
		...
		this._register(
			applyObservableDecorations(
				...
				derived(this, (reader) => {
					...
					const curUnchangedRegions = unchangedRegions.read(reader);
					const result = curUnchangedRegions.map<IModelDeltaDecoration>(
						(r) => ({
			...
			this._editors.modified.onMouseUp((event) => {
				if (
					...
					const region = model.unchangedRegions
						.get()
						.find((r) => r.modifiedUnchangedRange.contains(lineNumber));
					if (!region) {
						return;
					}
		...
		this._register(
			this._editors.original.onMouseUp((event) => {
				if (
					...
					const region = model.unchangedRegions
						.get()
						.find((r) => r.originalUnchangedRange.contains(lineNumber));
					if (!region) {
						return;
					}
					region.collapseAll(undefined);
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				...
				if (!this._hide) {
					const lineCount =
						this._unchangedRegion.getHiddenModifiedRange(reader).length;
