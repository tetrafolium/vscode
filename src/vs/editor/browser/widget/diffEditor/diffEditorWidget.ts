...
export class DiffEditorWidget extends DelegatingEditor implements IDiffEditor {
	...
	constructor(
		...
		this._diffModel = derived<DiffEditorViewModel | undefined>(
			this,
		...
		this._boundarySashes = observableValue<IBoundarySashes | undefined>(
			this,
			undefined,
		);
		...
		this._movedBlocksLinesPart = observableValue<
			...
			if (sideBySide) {
				...
				if (shouldHideOriginalLineNumbers) {
					...
					originalWidth = Math.max(
						5,
						this._editors.originalObs.layoutInfoDecorationsLeft.read(reader),
					);
				}
		...
		const viewZoneManager = derivedDisposable(
			...
			(reader /** @description ViewZoneManager */) =>
				this._instantiationService.createInstance(
					...
					() =>
						isUpdatingViewZones ||
						unchangedRangesFeature.read(undefined).isUpdatingHiddenAreas,
		...
		const isInitializingDiff = this._diffModel.map(this, (m, reader) => {
			...
			return (
				m.diff.read(reader) === undefined && !m.isDiffUpToDate.read(reader)
			);
		});
		this._register(
	...
	revertFocusedRangeMappings() {
		...
		const selectedRange = LineRange.fromRange(
			selection || new Range(curLineNumber, 0, curLineNumber, 0),
		);
		...
		modifiedEditor.executeEdits(
			...
			diffsToRevert.map((d) => ({
				...
				text: model.model.original.getValueInRange(
					d.lineRangeMapping.original.toExclusiveRange(),
				),
			})),
		);
		modifiedEditor.pushUndoStop();
	...
	goToDiff(target: "previous" | "next"): void {
		...
		if (target === "next") {
			const modifiedLineCount = this._editors.modified
				.getModel()!
				.getLineCount();
