		...
		this._register(
			autorun((reader) => {
				...
				const newScrollTopModified =
					...
					(this._originalScrollOffsetAnimated.read(undefined) -
		...
		this._register(
			autorun((reader) => {
				...
				const newScrollTopOriginal =
					...
					(this._modifiedScrollOffsetAnimated.read(undefined) -
			...
			autorun((reader) => {
				...
				if (this._editors.modified.hasTextFocus()) {
					this._originalScrollOffset.set(
						this._modifiedScrollOffset.read(undefined) - deltaOrigToMod,
						undefined,
						true,
					);
				} else {
					this._modifiedScrollOffset.set(
						this._originalScrollOffset.read(undefined) + deltaOrigToMod,
						undefined,
						true,
					);
				}
