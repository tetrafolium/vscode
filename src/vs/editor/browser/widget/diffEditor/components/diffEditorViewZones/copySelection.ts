...
export function enableCopySelection(
	options: IEnableViewZoneCopySelectionOptions,
): DisposableStore {
	const {
		domNode,
		renderLinesResult,
	...
	viewZoneDisposable.add(
		addDisposableListener(domNode, "copy", (e) => {
			...
			const startPosition = renderLinesResult.getModelPositionAt(
				startElement,
				domRange.startOffset,
			);
			const endPosition = renderLinesResult.getModelPositionAt(
				endElement,
				domRange.endOffset,
			);
			...
			const adjustedStart = startPosition.delta(
				diffEntry.original.startLineNumber - 1,
			);
			const adjustedEnd = endPosition.delta(
				diffEntry.original.startLineNumber - 1,
			);
