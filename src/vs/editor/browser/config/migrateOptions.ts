...
export class EditorSettingMigration {
	...
	private static _read(source: unknown, key: string): unknown {
		...
		if (firstDotIndex >= 0) {
			...
			return this._read(
				(source as Record<string, unknown>)[firstSegment],
	...
	private static _write(target: unknown, key: string, value: unknown): void {
		...
		if (firstDotIndex >= 0) {
			const firstSegment = key.substring(0, firstDotIndex);
			(target as Record<string, unknown>)[firstSegment] =
				(target as Record<string, unknown>)[firstSegment] || {};
			this._write(
				(target as Record<string, unknown>)[firstSegment],
				key.substring(firstDotIndex + 1),
...
registerEditorSettingMigration(
	...
	(value, read, write) => {
		if (typeof value === "boolean") {
			...
			if (
				typeof read("codeActionWidget.includeNearbyQuickFixes") === "undefined"
			) {
