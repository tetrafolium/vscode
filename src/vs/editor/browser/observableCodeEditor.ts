...
export class ObservableCodeEditor extends Disposable {
	...
	private constructor(public readonly editor: ICodeEditor) {
		...
		this.positions = derivedOpts<readonly Position[] | null>(
			...
			(reader) =>
				this.selections.read(reader)?.map((s) => s.getStartPosition()) ?? null,
		);
		...
		this.layoutInfoVerticalScrollbarWidth = this.layoutInfo.map(
		...
		this._onDidChangeViewZones = observableSignalFromEvent(
			this,
			this.editor.onDidChangeViewZones,
		);
		this._onDidHiddenAreasChanged = observableSignalFromEvent(
			this,
			this.editor.onDidChangeHiddenAreas,
		);
		this._onDidLineHeightChanged = observableSignalFromEvent(
			this,
			this.editor.onDidChangeLineHeight,
		);
	...
	public readonly versionId: IObservableWithChange<
		number | null,
		IModelContentChangedEvent | undefined
	>;

	private readonly _selections;
	public readonly selections: IObservableWithChange<
		Selection[] | null,
		ICursorSelectionChangedEvent | undefined
	>;
	...
	public observeLineOffsetRange(
		lineRange: IObservable<LineRange>,
		store: DisposableStore,
	): IObservable<OffsetRange> {
	...
	getLeftOfPosition(position: Position, reader: IReader | undefined): number {
		...
		let offset = this.editor.getOffsetForColumn(
			position.lineNumber,
			position.column,
		);
		if (offset === -1) {
	...
	public observePosition(
		position: IObservable<Position | null>,
		store: DisposableStore,
	): IObservable<Point | null> {
			...
			afterRender: (position, coordinate) => {
				...
				if (model && pos && pos.lineNumber > model.getLineCount()) {
					...
					result.set(
						new Point(
							0,
							this.editor.getBottomForLineNumber(model.getLineCount()) -
								this.scrollTop.get(),
						),
						undefined,
					);
	...
	isTargetHovered(
		predicate: (target: IEditorMouseEvent) => boolean,
		store: DisposableStore,
	): IObservable<boolean> {
	...
	observeLineHeightForPosition(
		position: IObservable<Position> | Position,
	): IObservable<number>;
	observeLineHeightForPosition(position: IObservable<null>): IObservable<null>;
	observeLineHeightForPosition(
		position: IObservable<Position | null> | Position,
	): IObservable<number | null> {
		return derived((reader) => {
			const pos =
				position instanceof Position ? position : position.read(reader);
			if (pos === null) {
				return null;
			}
	...
	observeLineHeightForLine(
		lineNumber: IObservable<number> | number,
	): IObservable<number>;
	observeLineHeightForLine(lineNumber: IObservable<null>): IObservable<null>;
	observeLineHeightForLine(
		lineNumber: IObservable<number | null> | number,
	): IObservable<number | null> {
		...
		return derived((reader) => {
			...
			return this.observeLineHeightForPosition(new Position(line, 1)).read(
				reader,
			);
		});
	}

	observeLineHeightsForLineRange(
		lineNumber: IObservable<LineRange> | LineRange,
	): IObservable<number[]> {
		return derived((reader) => {
			const range =
				lineNumber instanceof LineRange ? lineNumber : lineNumber.read(reader);
