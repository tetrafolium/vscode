...
export class TextAreaEditContext extends AbstractEditContext {
	...
	constructor(
		...
		const simplePagedScreenReaderStrategy =
			...
			getScreenReaderContent: (): TextAreaState => {
				if (this._accessibilitySupport === AccessibilitySupport.Disabled) {
					...
					if (
						...
						!selection.isEmpty() &&
						this._context.viewModel.getValueLengthInRange(
							selection,
				...
				const screenReaderContentState =
					simplePagedScreenReaderStrategy.fromEditorSelection(
		...
		this._register(
			this._textAreaInput.onWillCopy((e) => this._onWillCopy.fire(e)),
		);
		this._register(
			this._textAreaInput.onWillCut((e) => this._onWillCut.fire(e)),
		);
		this._register(
			this._textAreaInput.onWillPaste((e) => this._onWillPaste.fire(e)),
		);
	...
	private _render(): void {
		if (this._visibleTextArea) {
			...
			if (
				...
				const lineHeight = this._context.viewLayout.getLineHeightForLineNumber(
					startPosition.lineNumber,
				);
				const fontSize = this._context.viewModel.getFontSizeAtPosition(
					this._primaryCursorPosition,
				);
