...
export class NativeEditContext extends AbstractEditContext {
	...
	constructor(
		...
		this._focusTracker = this._register(
			new FocusTracker(
				...
				(newFocusValue: boolean) => {
					logService.trace(
						"NativeEditContext#handleFocusChange : ",
						newFocusValue,
					);
		...
		this._register(
			addDisposableListener(this.domNode.domNode, "cut", (e) => {
				...
				this.logService.trace(
					"NativeEditContext#cut (before viewController.cut)",
				);
		...
		this._register(
			addDisposableListener(this.domNode.domNode, "paste", (e) => {
				...
				if (pasteEvent.metadata) {
					...
					multicursorText =
						typeof pasteEvent.metadata.multicursorText !== "undefined"
							? pasteEvent.metadata.multicursorText
							: null;
					mode = pasteEvent.metadata.mode;
				}
				this.logService.trace(
					"NativeEditContext#paste (before viewController.paste)",
				);
				...
				for (const change of e.changes) {
					if (
						change.range.startLineNumber <=
							this._editContextPrimarySelection.endLineNumber &&
	...
	public handleWillCopy(): void {
		...
		this.logService.trace(
			"NativeEditContext#isFocused : ",
			this.domNode.domNode === getActiveElement(),
		);
	}
	...
	private _updateEditContext(): void {
		...
		if (newText !== this._previousEditContextText) {
			this._editContext.updateText(
				0,
				this._previousEditContextText.length,
				newText,
			);
		...
		if (
			editContextState.selectionStartOffset !==
				this._previousEditContextSelection.start ||
			editContextState.selectionEndOffset !==
				this._previousEditContextSelection.endExclusive
		) {
