...
const ttPolicy = createTrustedTypesPolicy("richScreenReaderContent", {
	createHTML: (value) => value,
});

const LINE_NUMBER_ATTRIBUTE = "data-line-number";

export class RichScreenReaderContent
	extends Disposable
	implements IScreenReaderContent
{
	...
	private _strategy: RichPagedScreenReaderStrategy =
	...
	public updateScreenReaderContent(primarySelection: Selection): void {
		...
		if (isScreenReaderOptimized) {
			...
			if (!this._renderedSelection.equalsSelection(primarySelection)) {
				...
				this._setSelectionOnScreenReaderContent(
					this._context,
					this._renderedLines,
					primarySelection,
				);
	...
	public updateScrollTop(primarySelection: Selection): void {
		...
		const verticalOffsetOfStateStartLineNumber =
			viewLayout.getVerticalOffsetForLineNumber(stateStartLineNumber);
		const verticalOffsetOfPositionLineNumber =
			viewLayout.getVerticalOffsetForLineNumber(
				primarySelection.positionLineNumber,
			);
		this._domNode.domNode.scrollTop =
			verticalOffsetOfPositionLineNumber - verticalOffsetOfStateStartLineNumber;
	}
	...
	private _setSelectionChangeListener(): IDisposable {
		...
		return addDisposableListener(
			this._domNode.domNode.ownerDocument,
			"selectionchange",
			() => {
	...
	private _renderScreenReaderContent(
		state: RichScreenReaderState,
	): Map<number, RichRenderedScreenReaderLine> {
		const nodes: HTMLDivElement[] = [];
		const renderedLines = new Map<number, RichRenderedScreenReaderLine>();
		for (const interval of state.intervals) {
			for (
				let lineNumber = interval.startLine;
				lineNumber <= interval.endLine;
				lineNumber++
			) {
	...
	private _renderLine(viewLineNumber: number): RichRenderedScreenReaderLine {
		...
		const useMonospaceOptimizations =
		...
		const lineHeight =
			viewModel.viewLayout.getLineHeightForLineNumber(viewLineNumber) + "px";
		...
		return new RichRenderedScreenReaderLine(
			domNode,
	...
	private _setSelectionOnScreenReaderContent(
		...
		const modelStartRange =
			coordinatesConverter.convertViewRangeToModelRange(startRange);
		const characterCountForStart =
			model.getCharacterCountInRange(modelStartRange);
		...
		const modelEndRange =
			coordinatesConverter.convertViewRangeToModelRange(endRange);
		const characterCountForEnd = model.getCharacterCountInRange(modelEndRange);
		const startDomPosition = startRenderedLine.characterMapping.getDomPosition(
			characterCountForStart,
		);
		const endDomPosition =
			endRenderedLine.characterMapping.getDomPosition(characterCountForEnd);
	...
	private _getScreenReaderContentLineIntervals(
		...
		return this._strategy.fromEditorSelection(
	...
	private _getEditorSelectionFromDomRange(): Selection | undefined {
		...
		if (
			!startSpanElement ||
			!isHTMLElement(startSpanElement) ||
			!endSpanElement ||
			!isHTMLElement(endSpanElement)
		) {
			return;
		}
		...
		const startLineNumberAttribute = startLineDomNode.getAttribute(
			LINE_NUMBER_ATTRIBUTE,
		);
		const endLineNumberAttribute = endLineDomNode.getAttribute(
			LINE_NUMBER_ATTRIBUTE,
		);
		...
		const startMapping =
			this._renderedLines.get(startLineNumber)?.characterMapping;
		const endMapping = this._renderedLines.get(endLineNumber)?.characterMapping;
		if (!startMapping || !endMapping) {
			return;
		}
		const startColumn = getColumnOfNodeOffset(
			startMapping,
			startSpanElement,
			range.startOffset,
		);
		const endColumn = getColumnOfNodeOffset(
			endMapping,
			endSpanElement,
			range.endOffset,
		);
...
class RichPagedScreenReaderStrategy implements IPagedScreenReaderStrategy<RichScreenReaderState> {
	...
	public fromEditorSelection(
		...
		const selectionStartPage = this._getPageOfLine(
			viewSelection.startLineNumber,
			linesPerPage,
		);
		const selectionStartPageRange = this._getRangeForPage(
			context,
			selectionStartPage,
			linesPerPage,
		);
		const selectionEndPage = this._getPageOfLine(
			viewSelection.endLineNumber,
			linesPerPage,
		);
		const selectionEndPageRange = this._getRangeForPage(
			context,
			selectionEndPage,
			linesPerPage,
		);
