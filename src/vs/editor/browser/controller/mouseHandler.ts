...
class MouseDownOperation extends Disposable {
	...
	private _getPositionOutsideEditor(e: EditorMouseEvent): IMouseTarget | null {
		...
		if (e.relativePos.x <= xLeftBoundary) {
			...
			const isRtl =
				model.getTextDirection(possibleLineNumber) === TextDirection.RTL;
			return MouseTarget.createOutsideEditor(
				mouseColumn,
				new Position(
					possibleLineNumber,
					isRtl ? model.getLineMaxColumn(possibleLineNumber) : 1,
				),
				"left",
				outsideDistance,
			);
		...
		if (e.relativePos.x >= xRightBoundary) {
			...
			return MouseTarget.createOutsideEditor(
				...
				new Position(
					possibleLineNumber,
					isRtl ? 1 : model.getLineMaxColumn(possibleLineNumber),
				),
				"right",
				outsideDistance,
			);
