...
export class TextureAtlas extends Disposable {
	...
	private _initFirstPage() {
		...
		const nullRasterizer = new GlyphRasterizer(
			1,
			"",
			1,
			this._decorationStyleCache,
		);
	...
	private _tryGetGlyph(
		...
		return (
			this._pages[pageIndex].getGlyph(
				rasterizer,
				chars,
				tokenMetadata,
				decorationStyleSetId,
	...
	private _warmUpAtlas(rasterizer: IGlyphRasterizer): void {
		...
		const taskQueue = (this._warmUpTask.value =
			this._instantiationService.createInstance(IdleTaskQueue));
