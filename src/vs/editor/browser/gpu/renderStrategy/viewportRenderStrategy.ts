...
export class ViewportRenderStrategy extends BaseRenderStrategy {
	...
	update(viewportData: ViewportData, viewLineOptions: ViewLineOptions): number {
							...
							for (const r of rule.style) {
								...
								switch (r) {
									...
									case "text-decoration-thickness": {
										...
										if (match) {
											decorationStyleSetStrikethroughThickness = parseFloat(
												match[1],
											);
										}
										break;
									...
									case "text-decoration-color": {
										...
										const varMatch = value.match(
											/^var\((--[^,]+),\s*(?:initial|inherit)\)$/,
										);
										if (varMatch) {
											colorValue =
												ViewGpuContext.decorationCssRuleExtractor.resolveCssVariable(
													this._viewGpuContext.canvas.domNode,
													varMatch[1],
												);
										}
		...
		if (viewportLineCount < this._lastViewportLineCount) {
			...
			const staleStartOffset =
				visibleObjectCount * Float32Array.BYTES_PER_ELEMENT;
			const staleByteCount =
				staleLineCount * lineIndexCount * Float32Array.BYTES_PER_ELEMENT;
