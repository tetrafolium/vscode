...
registerThemingParticipant((theme, collector) => {
	...
	if (lineHighlight) {
		collector.addRule(
			`.monaco-editor.focused .view-overlays .current-line { background-color: ${lineHighlight}; }`,
		);
		collector.addRule(
	...
	if (inactiveLineHighlight) {
		collector.addRule(
			`.monaco-editor .view-overlays .current-line { background-color: ${inactiveLineHighlight}; }`,
		);
		collector.addRule(
