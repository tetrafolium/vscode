...
export class ViewLine implements IVisibleLine {
	...
	public onSelectionChanged(): boolean {
		if (
			...
			this._renderedViewLine?.input.renderWhitespace ===
	...
	public renderLine(
		...
		const renderWhitespace =
			options.experimentalWhitespaceRendering === "off"
...
class RenderedViewLine implements IRenderedViewLine {
	...
	public resetCachedWidth(): void {
		...
		if (this._pixelOffsetCache !== null) {
			for (
				let column = 0, len = this._pixelOffsetCache.length;
				column < len;
				column++
			) {
				this._pixelOffsetCache[column] = -1;
	...
	private _readRawVisibleRangesForRange(
		...
		if (
			this.input.isLTR &&
			startColumn === 1 &&
			endColumn === this._characterMapping.length
		) {
