	...
	protected _render(
		...
		for (const d of decorations) {
			...
			for (
				...
				lineIndex <= prevEndLineIndex;
				lineIndex++
			) {
				output[lineIndex].add(
...
export class GlyphMarginWidgets extends ViewPart {
	...
	public render(ctx: RestrictedRenderingContext): void {
		...
		for (let i = 0; i < this._decorationGlyphsToRender.length; i++) {
			...
			const top =
				ctx.viewportData.relativeVerticalOffset[
					decLineNumber - ctx.viewportData.startLineNumber
				];
			...
			const lineHeight =
				this._context.viewLayout.getLineHeightForLineNumber(decLineNumber);
