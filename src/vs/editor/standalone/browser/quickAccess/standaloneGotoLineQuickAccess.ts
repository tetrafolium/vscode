...
export class StandaloneGotoLineQuickAccessProvider extends AbstractGotoLineQuickAccessProvider {
	...
	constructor(
		...
		@IStorageService
		protected override readonly storageService: IStorageService,
	) {
		super();
	}
...
export class GotoLineAction extends EditorAction {
	...
	run(accessor: ServicesAccessor): void {
		accessor
			...
			.quickAccess.show(
				StandaloneGotoLineQuickAccessProvider.GO_TO_LINE_PREFIX,
			);
	}
}
...
class GotoOffsetAction extends EditorAction {
	...
	run(accessor: ServicesAccessor): void {
		accessor
			.get(IQuickInputService)
			.quickAccess.show(
				StandaloneGotoLineQuickAccessProvider.GO_TO_OFFSET_PREFIX,
			);
	}
}
...
Registry.as<IQuickAccessRegistry>(
	...
	helpEntries: [
		{
			description: GoToLineNLS.gotoOffsetActionLabel,
			commandId: GotoOffsetAction.ID,
		},
	],
});
