...
suite("Editor Contrib - Line Operations", () => {
	...
	suite("DeleteDuplicateLinesAction", () => {
		...
		test("should remove duplicate lines in multiple selections", function () {
			withTestCodeEditor(
				["alpha", "beta", "beta", "omicron", "", "alpha", "alpha", "beta"],
				{},
				(editor) => {
					const model = editor.getModel()!;
					const deleteDuplicateLinesAction = new DeleteDuplicateLinesAction();
	...
	suite("ReverseLinesAction", () => {
		test("reverses lines", function () {
			withTestCodeEditor(["alice", "bob", "charlie"], {}, (editor) => {
				const model = editor.getModel()!;
		...
		test("preserves selected text when selections do not span lines", function () {
			withTestCodeEditor(["alice", "bob", "charlie", ""], {}, (editor) => {
				...
				assert.deepStrictEqual(
					editor.getSelections().map((s) => model.getValueInRange(s)),
		...
		test("updates selection positions after reversal", function () {
			withTestCodeEditor(["line1", "line2", "line3", "line4"], {}, (editor) => {
				const reverseLinesAction = new ReverseLinesAction();
		...
		test("applies to whole document when selection is single line", function () {
			withTestCodeEditor(["line1", "line2", "line3"], {}, (editor) => {
				const model = editor.getModel()!;
				const reverseLinesAction = new ReverseLinesAction();
	...
	test("toggle case", function () {
		...
		withTestCodeEditor(
			...
			(editor) => {
				...
				executeAction(camelcaseAction, editor);
				assert.strictEqual(
					model.getValueInRange(new Selection(9, 1, 11, 3)),
		...
		withTestCodeEditor(
			...
			(editor) => {
				...
				executeAction(pascalCaseAction, editor);
				assert.strictEqual(
					model.getValueInRange(new Selection(9, 1, 10, 11)),
					"ParseHTML4String\nKebabCase",
				);
	...
	test("InsertLineBeforeAction", () => {
		function testInsertLineBefore(
			lineNumber: number,
			column: number,
			callback: (model: ITextModel, viewModel: ViewModel) => void,
		): void {
	...
	function testDeleteLinesCommand(
		initialText: string[],
		_initialSelections: Selection | Selection[],
		resultingText: string[],
		_resultingSelections: Selection | Selection[],
	): void {
