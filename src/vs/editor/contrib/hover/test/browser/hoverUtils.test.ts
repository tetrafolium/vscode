...
suite("Hover Utils", () => {
	...
	suite("isMousePositionWithinElement", () => {
		...
		test("returns true when mouse is on element edges", () => {
			const element = createMockElement(100, 100, 200, 100);
			assert.strictEqual(
				isMousePositionWithinElement(element, 100, 100),
				false,
			); // top-left corner
			assert.strictEqual(
				isMousePositionWithinElement(element, 300, 100),
				false,
			); // top-right corner
			assert.strictEqual(
				isMousePositionWithinElement(element, 100, 200),
				false,
			); // bottom-left corner
			assert.strictEqual(
				isMousePositionWithinElement(element, 300, 200),
				false,
			); // bottom-right corner
		});
		...
		test("returns false when mouse is right of element", () => {
			const element = createMockElement(100, 100, 200, 100);
			assert.strictEqual(
				isMousePositionWithinElement(element, 301, 150),
				false,
			);
			assert.strictEqual(
				isMousePositionWithinElement(element, 400, 150),
				false,
			);
		});
		...
		test("returns false when mouse is below element", () => {
			const element = createMockElement(100, 100, 200, 100);
			assert.strictEqual(
				isMousePositionWithinElement(element, 200, 201),
				false,
			);
			assert.strictEqual(
				isMousePositionWithinElement(element, 200, 300),
				false,
			);
		});
		...
		test("handles element at origin (0,0)", () => {
			...
			assert.strictEqual(
				isMousePositionWithinElement(element, 101, 101),
				false,
			);
		});
		...
		test("handles small elements (1x1)", () => {
			...
			assert.strictEqual(
				isMousePositionWithinElement(element, 102, 102),
				false,
			);
		});
	});
