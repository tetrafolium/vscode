	...
	implements IEditorContribution
{
	...
	private _onEditorMouseDown(mouseEvent: IEditorMouseEvent): void {
		...
		const shouldKeepHoverWidgetVisible =
			this._shouldKeepHoverWidgetVisible(mouseEvent);
	...
	private _shouldKeepCurrentHover(mouseEvent: IEditorMouseEvent): boolean {
		...
		const isMouseOnColorPickerOrChoosingColor = (
			mouseEvent: IEditorMouseEvent,
		): boolean => {
			const isColorPickerVisible = contentWidget.isColorPickerVisible;
			const isMouseOnContentHoverWidget =
				this._isMouseOnContentHoverWidget(mouseEvent);
			const isMouseOnHoverWithColorPicker =
				isColorPickerVisible && isMouseOnContentHoverWidget;
		...
		const isStickyAndVisibleFromKeyboard =
			this._hoverSettings.sticky && contentWidget.isVisibleFromKeyboard;
	...
	private _shouldRescheduleHoverComputation(): boolean {
		...
		return (
			isContentHoverWidgetVisible &&
			this._hoverSettings.sticky &&
			hidingDelay > 0
		);
	}
