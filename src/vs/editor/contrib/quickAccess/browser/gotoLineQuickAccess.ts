...
export abstract class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
	...
	protected provideWithTextEditor(
		...
		const updatePickerAndEditor = () => {
			const inputText = picker.value
				.trim()
				.substring(
					AbstractGotoLineQuickAccessProvider.GO_TO_LINE_PREFIX.length,
				);
			const { inOffsetMode, lineNumber, column, label } = this.parsePosition(
				editor,
				inputText,
			);
		...
		disposables.add(
			picker.onDidTriggerButton((button) => {
				if (button === offsetButton) {
					this.useZeroBasedOffset =
						button.toggle?.checked ?? !this.useZeroBasedOffset;
	...
	protected parsePosition(
		...
		if (value.startsWith(":")) {
			...
			const realColumn = CursorColumns.columnFromVisibleColumn(
				lineContent,
				column - 1,
				tabSize,
			);
