...
suite("DefaultDocumentColorProvider", () => {
	...
	test("Color presentations should not include alpha channel when alpha is 1", () => {
		...
		const opaquePresentations = provider.provideColorPresentations(
			null!,
			opaqueColorInfo,
			CancellationToken.None,
		);
		assert.strictEqual(
			opaquePresentations[0].label,
			"rgb(255, 0, 0)",
			"RGB should not include alpha when alpha is 1",
		);
		assert.strictEqual(
			opaquePresentations[1].label,
			"hsl(0, 100%, 50%)",
			"HSL should not include alpha when alpha is 1",
		);
		assert.strictEqual(
			opaquePresentations[2].label,
			"#ff0000",
			"HEX should not include alpha when alpha is 1",
		);
	});
	...
	test("Color presentations should include alpha channel when alpha is not 1", () => {
		...
		const transparentPresentations = provider.provideColorPresentations(
			null!,
			transparentColorInfo,
			CancellationToken.None,
		);
		assert.strictEqual(
			transparentPresentations[0].label,
			"rgba(0, 0, 0, 0)",
			"RGB should include alpha when alpha is 0",
		);
		assert.strictEqual(
			transparentPresentations[1].label,
			"hsla(0, 0%, 0%, 0.00)",
			"HSL should include alpha when alpha is 0",
		);
		assert.strictEqual(
			transparentPresentations[2].label,
			"#00000000",
			"HEX should include alpha when alpha is 0",
		);
	});
	...
	test("Color presentations should include alpha channel when alpha is between 0 and 1", () => {
		...
		const semiTransparentPresentations = provider.provideColorPresentations(
			null!,
			semiTransparentColorInfo,
			CancellationToken.None,
		);
		assert.strictEqual(
			semiTransparentPresentations[0].label,
			"rgba(171, 0, 0, 0.67)",
			"RGB should include alpha when alpha is 0.67",
		);
		assert.strictEqual(
			semiTransparentPresentations[1].label,
			"hsla(0, 100%, 34%, 0.67)",
			"HSL should include alpha when alpha is 0.67",
		);
		assert.strictEqual(
			semiTransparentPresentations[2].label,
			"#ab0000ab",
			"HEX should include alpha when alpha is 0.67",
		);
	});
	...
	test("Regression test for issue #243746: opacity should be preserved when switching to hex format", () => {
		...
		assert.strictEqual(
			presentations[2].label,
			"#80808080",
			"HEX format should preserve opacity (issue #243746)",
		);
	});
	...
	test("Regression test for issue #256853: fully opaque colors should not add unnecessary alpha suffix", () => {
		...
		assert.strictEqual(
			presentations[2].label,
			"#945ca6",
			"HEX format should not add ff suffix when fully opaque (issue #256853)",
		);
	});
});
