...
export class FloatingEditorToolbar
	extends Disposable
...
export class FloatingEditorToolbarWidget extends Disposable {
	...
	constructor(
		...
		const menu = this._register(
			menuService.createMenu(_menuId, _scopedContextKeyService),
		);
		const menuGroupsObs = observableFromEvent(this, menu.onDidChange, () =>
			menu.getActions(),
		);
		...
		this.hasActions = derived(
			(reader) => menuPrimaryActionsObs.read(reader).length > 0,
		);
		...
		this._register(
			autorun((reader) => {
				...
				const menuPrimaryActionId = hasActions
					? primaryActions[0].id
					: undefined;
