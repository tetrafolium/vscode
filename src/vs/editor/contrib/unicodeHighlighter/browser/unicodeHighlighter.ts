...
export class UnicodeHighlighterHoverParticipant implements IEditorHoverParticipant<MarkdownHover> {
	...
	computeSync(
		...
		const unicodeHighlighter = this._editor.getContribution<UnicodeHighlighter>(
					...
					if (isBasicASCII(highlightInfo.reason.confusableWith)) {
						reason = nls.localize(
							...
							formatCodePointMarkdown(
								highlightInfo.reason.confusableWith.codePointAt(0)!,
							),
						);
					} else {
						reason = nls.localize(
...
export class ShowExcludeOptions extends Action2 {
	...
	public async run(accessor: ServicesAccessor, args: any): Promise<void> {
		...
		options.push({
			...
			run: () =>
				excludeCharFromBeingHighlighted(configurationService, [codePoint]),
		});
		...
		function getTitle(options: Action2) {
			return typeof options.desc.title === "string"
				? options.desc.title
				: options.desc.title.value;
		}
