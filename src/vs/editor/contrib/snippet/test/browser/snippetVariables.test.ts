	...
	test("Time-based snippet variables have deterministic millisecond and unix values", function () {
		...
		try {
			...
			assertVariableResolve(
				resolver,
				"CURRENT_YEAR_SHORT",
				String(expectedDate.getFullYear()).slice(-2),
			);
			assertVariableResolve(
				resolver,
				"CURRENT_MONTH",
				pad(expectedDate.getMonth() + 1, 2),
			);
			assertVariableResolve(
				resolver,
				"CURRENT_DATE",
				pad(expectedDate.getDate(), 2),
			);
			assertVariableResolve(
				resolver,
				"CURRENT_HOUR",
				pad(expectedDate.getHours(), 2),
			);
			assertVariableResolve(
				resolver,
				"CURRENT_MINUTE",
				pad(expectedDate.getMinutes(), 2),
			);
			assertVariableResolve(
				resolver,
				"CURRENT_SECOND",
				pad(expectedDate.getSeconds(), 2),
			);
			assertVariableResolve(
				resolver,
				"CURRENT_MILLISECOND",
				pad(expectedDate.getMilliseconds(), 3),
			);
			assertVariableResolve(
				resolver,
