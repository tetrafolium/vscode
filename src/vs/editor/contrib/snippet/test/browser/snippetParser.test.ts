...
suite("SnippetParser", () => {
	...
	function assertMarker(
		input: TextmateSnippet | Marker[] | string,
		...ctors: Function[]
	) {
		let marker: Marker[];
	...
	test("Parser, placeholder with choice", () => {
		...
		const expected: ((m: Marker) => boolean)[] = [
			...
			(m) =>
				m instanceof Choice &&
				m.options.length === 3 &&
				m.options.every((x) => x instanceof Text),
		];
		snippet.walk((marker) => {
	...
	test("Transform -> FormatString#resolve", function () {
		...
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("barFoo"),
			"bar-foo",
		);
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("BarFoo"),
		...
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("ABarFoo"),
			"a-bar-foo",
		);
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("bar42Foo"),
			"bar42-foo",
		);
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("snake_AndPascalCase"),
			"snake-and-pascal-case",
		);
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("kebab-AndCamelCase"),
			"kebab-and-camel-case",
		);
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("_justPascalCase"),
			"just-pascal-case",
		);
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("__UPCASE__"),
			"upcase",
		);
		assert.strictEqual(
			new FormatString(1, "kebabcase").resolve("__BAR_FOO__"),
			"bar-foo",
		);
		assert.strictEqual(
			new FormatString(1, "snakecase").resolve("bar-foo"),
			"bar_foo",
		);
		assert.strictEqual(
			new FormatString(1, "snakecase").resolve("bar-42-foo"),
			"bar_42_foo",
		);
		assert.strictEqual(
			new FormatString(1, "snakecase").resolve("snake_AndPascalCase"),
			"snake_and_pascal_case",
		);
		assert.strictEqual(
			new FormatString(1, "snakecase").resolve("kebab-AndPascalCase"),
			"kebab_and_pascal_case",
		);
		assert.strictEqual(
			new FormatString(1, "snakecase").resolve("_justPascalCase"),
			"_just_pascal_case",
		);
		assert.strictEqual(
		...
		// if
		assert.strictEqual(
			new FormatString(1, undefined, "foo", undefined).resolve(undefined),
			"",
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			new FormatString(1, undefined, "foo", undefined).resolve("bar"),
			"foo",
		);

		// else
		assert.strictEqual(
			new FormatString(1, undefined, undefined, "foo").resolve(undefined),
			"foo",
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			new FormatString(1, undefined, undefined, "foo").resolve("bar"),
			"bar",
		);

		// if-else
		assert.strictEqual(
			new FormatString(1, undefined, "bar", "foo").resolve(undefined),
			"foo",
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			new FormatString(1, undefined, "bar", "foo").resolve("baz"),
			"bar",
		);
	});
	...
	test("Unicode Variable Transformations", () => {
		...
		function assertTransform(
			...
			assert.strictEqual(
				resolved,
				expected,
				`${transformName} failed for ${varName}`,
			);
		}
	...
	test("Snippet cannot escape closing bracket inside conditional insertion variable replacement #78883", function () {
		...
		assert.strictEqual(
			(<FormatString>variable.transform.children[0]).ifValue,
			"import { hello } from world",
		);
		assert.strictEqual(
			(<FormatString>variable.transform.children[0]).elseValue,
			undefined,
		);
	});
