...
export class SnippetController2 implements IEditorContribution {
	...
	static readonly InSnippetChoice = new RawContextKey(
		"inSnippetChoice",
		false,
		localize(
			"inSnippetChoice",
	...
	constructor(
		...
		this._inSnippetChoice =
			SnippetController2.InSnippetChoice.bindTo(contextKeyService);
	}
			...
			const provider: CompletionItemProvider = {
				...
				provideCompletionItems: (model: ITextModel, position: Position) => {
					...
					const isAnyOfOptions = Boolean(
						activeChoice.choice.options.find((o) => o.value === word),
					);
