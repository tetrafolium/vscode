...
export class ContextMenuController implements IEditorContribution {
		...
		this._toDispose.add(
			this._editor.onMouseWheel((e: IMouseWheelEvent) => {
				if (this._contextMenuIsBeingShownCount > 0) {
					...
					if (
						!(
							target.shadowRoot && dom.getShadowRoot(view) === target.shadowRoot
						)
					) {
	...
	private _doShowContextMenu(
		...
		this._contextMenuService.showContextMenu({
			...
			getActionViewItem: (action) => {
				...
				const customAction = action as IAction & {
					getActionViewItem?: () => ActionViewItem;
				};
	...
	private _showScrollbarContextMenu(anchor: IMouseEvent): void {
		...
		if (
			isStandaloneEditorWorkspace(this._workspaceContextService.getWorkspace())
		) {
