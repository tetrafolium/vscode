		...
		if (!this._domNode) {
			...
			this._disposables.add(
				this._inputWithButton.onDidInputChange(() => {
					...
					if (
						this._renameCandidateProvidersCts?.token.isCancellationRequested ===
						false
					) {
...
class RenameCandidateListView {
	...
	private static _createListWidget(
		...
		const virtualDelegate =
			new (class implements IListVirtualDelegate<NewSymbolName> {
		...
		const renderer = new (class implements IListRenderer<
			NewSymbolName,
			RenameCandidateView
		> {
...
class InputWithButton implements IDisposable {
	...
	private readonly _onDidInputChange = this._disposables.add(
		new Emitter<void>(),
	);
...
class RenameCandidateView {
	...
	public static getLayoutInfo({ lineHeight }: { lineHeight: number }): {
		...
		const totalHeight =
			lineHeight + RenameCandidateView._PADDING * 2; /* top & bottom padding */
		return { totalHeight };
	}
