	...
	implements IEditorHoverParticipant<MarkdownHover>
{
	...
	override computeAsync(
		...
		return new AsyncIterableProducer<MarkdownHover>(async (executor) => {
			...
			if (itemTooltip) {
				executor.emitOne(
					new MarkdownHover(this, anchor.range, [itemTooltip], false, 0),
				);
			}
			...
			if (partTooltip) {
				executor.emitOne(
					new MarkdownHover(this, anchor.range, [partTooltip], false, 1),
				);
			}
			...
			if (part.part.location || part.part.command) {
				...
				const useMetaKey =
					this._editor.getOption(EditorOption.multiCursorModifier) === "altKey";
				const kb = useMetaKey
				...
				if (linkHint) {
					executor.emitOne(
						new MarkdownHover(this, anchor.range, [linkHint], false, 10000),
					);
				}
			}
