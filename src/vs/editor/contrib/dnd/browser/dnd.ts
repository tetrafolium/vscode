	...
	implements IEditorContribution
{
	...
	constructor(editor: ICodeEditor) {
		...
		this._register(
			this._editor.onMouseDropCanceled(() => this._onEditorMouseDropCanceled()),
		);
		this._register(
	...
	private _onEditorMouseDrag(mouseEvent: IEditorMouseEvent): void {
		...
		if (this._dragSelection === null) {
			...
			const possibleSelections = selections.filter(
				(selection) =>
					target.position && selection.containsPosition(target.position),
			);
		...
		if (
			...
			if (this._dragSelection === null) {
				...
				if (mouseEvent.event.shiftKey) {
					...
					if (primarySelection) {
						const { selectionStartLineNumber, selectionStartColumn } =
							primarySelection;
						newSelections = [
							new Selection(
