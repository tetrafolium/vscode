...
class ExecCommandCopyWithSyntaxHighlightingAction extends EditorAction {
	...
	public run(accessor: ServicesAccessor, editor: ICodeEditor): void {
		...
		editor.focus();
		logService.trace(
			"ExecCommandCopyWithSyntaxHighlightingAction (before execCommand copy)",
		);
		executeClipboardCopyWithWorkaround(editor, clipboardService);
		logService.trace(
			"ExecCommandCopyWithSyntaxHighlightingAction (after execCommand copy)",
		);
		CopyOptions.forceCopyWithSyntaxHighlighting = false;
	}
}

function executeClipboardCopyWithWorkaround(
	...
	if (
		...
		const { dataToCopy } = generateDataToCopyAndStoreInMemory(
...
function registerExecCommandImpl(
	...
	target.addImplementation(
		...
		(accessor: ServicesAccessor, args: unknown) => {
			...
			if (
				focusedEditor &&
				focusedEditor.hasTextFocus() &&
				focusedEditor.hasModel()
			) {
				...
				if (
					focusedEditor.getOption(EditorOption.effectiveEditContext) &&
					browserCommand === "cut"
				) {
					...
					logCopyCommand(focusedEditor);
					logService.trace(
						"registerExecCommandImpl (before execCommand " +
							browserCommand +
							")",
					);
					...
					logService.trace(
						"registerExecCommandImpl (after execCommand " +
							browserCommand +
							")",
					);
				}
	...
	target.addImplementation(
		...
		(accessor: ServicesAccessor, args: unknown) => {
			...
			logService.trace(
				"registerExecCommandImpl (before execCommand " + browserCommand + ")",
			);
			getActiveDocument().execCommand(browserCommand);
			logService.trace(
				"registerExecCommandImpl (after execCommand " + browserCommand + ")",
			);
			return true;
		},
	);
...
if (PasteAction) {
	...
	PasteAction.addImplementation(
		...
		(accessor: ServicesAccessor, args: unknown) => {
			const logService = accessor.get(ILogService);
			logService.trace(
				"registerExecCommandImpl (addImplementation code-editor for : paste)",
			);
			...
			if (
				...
				const triggerPaste = clipboardService.triggerPaste(
					getActiveWindow().vscodeWindowId,
				);
				if (triggerPaste) {
					...
					return triggerPaste.then(async () => {
						...
						return (
							CopyPasteController.get(focusedEditor)?.finishedPaste() ??
							Promise.resolve()
						);
					});
				} else {
	...
	PasteAction.addImplementation(
		...
		(accessor: ServicesAccessor, args: unknown) => {
			const logService = accessor.get(ILogService);
			logService.trace(
				"registerExecCommandImpl (addImplementation generic-dom for : paste)",
			);
			const triggerPaste = accessor
				.get(IClipboardService)
				.triggerPaste(getActiveWindow().vscodeWindowId);
