	...
	implements IEditorContribution
{
	...
	constructor(
		...
		this._postDropWidgetManager = this._register(
			instantiationService.createInstance(
				...
				() =>
					DropIntoEditorController._configureDefaultAction
						? [DropIntoEditorController._configureDefaultAction]
						: [],
			),
		);
		...
		const p = createCancelablePromise(async (token) => {
			...
			try {
				...
				if (editSession.edits.length) {
					...
					const canShowWidget =
						editor.getOption(EditorOption.dropIntoEditor).showDropSelector ===
						"afterDrop";
	...
	private getInitialActiveEditIndex(
		model: ITextModel,
		edits: ReadonlyArray<DocumentDropEdit>,
	): number {
		const preferredProviders = this._configService.getValue<
			PreferredDropConfiguration[]
		>(dropAsPreferenceConfig, { resource: model.uri });
