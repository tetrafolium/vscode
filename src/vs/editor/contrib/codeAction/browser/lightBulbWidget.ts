...
export function computeLightBulbInfo(
	...
	if (actions.allAIFixes) {
		...
		if (actions.hasAIFix) {
			icon = forGutter
				? GUTTER_LIGHTBULB_AIFIX_AUTO_FIX_ICON
				: Codicon.lightbulbSparkleAutofix;
		} else {
			icon = forGutter
				? GUTTER_LIGHTBULB_AUTO_FIX_ICON
				: Codicon.lightbulbAutofix;
		}
	...
	if (autoRun) {
		title = nls.localize(
			"codeActionAutoRun",
			"Run: {0}",
			actions.validActions[0].action.title,
		);
...
export class LightBulbWidget extends Disposable implements IContentWidget {
	...
	private readonly _state = observableValue<LightBulbState.State>(
		this,
		LightBulbState.Hidden,
	);
	private readonly _gutterState = observableValue<LightBulbState.State>(
		this,
		LightBulbState.Hidden,
	);
	...
	public readonly lightBulbInfo: IObservable<LightBulbInfo | undefined> =
		this._combinedInfo;
	...
	private readonly _preferredKbLabel = observableValue<string | undefined>(
		this,
		undefined,
	);
	private readonly _quickFixKbLabel = observableValue<string | undefined>(
		this,
		undefined,
	);
		...
		this._register(
			dom.addStandardDisposableGenericMouseDownListener(this._domNode, (e) => {
				...
				if (
					...
					state.widgetPosition.position.lineNumber <
						state.editorPosition.lineNumber
				) {
					pad += lineHeight;
				}
			...
			this._editor.onMouseDown(async (e: IEditorMouseEvent) => {
				if (
					...
					!this.lightbulbClasses.some(
						(cls) =>
							e.target.element && e.target.element.classList.contains(cls),
					)
				) {
					return;
				}
				...
				if (
					...
					gutterState.widgetPosition.position.lineNumber <
						gutterState.editorPosition.lineNumber
				) {
					pad += lineHeight;
				}
	...
	getPosition(): IContentWidgetPosition | null {
		const state = this._state.get();
		return state.type === LightBulbState.Type.Showing
	...
	public update(
		...
		if (
			options.get(EditorOption.lightbulb).enabled === ShowLightbulbIconMode.Off
		) {
			this.gutterHide();
			return this.hide();
		}
