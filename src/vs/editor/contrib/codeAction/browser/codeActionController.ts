	...
	implements IEditorContribution
{
	...
	private readonly _lightBulbInfoObs = observableValue<
	...
	public readonly lightBulbState: IObservable<LightBulbInfo | undefined> =
		derivedOpts<LightBulbInfo | undefined>(
			{
				owner: this,
	...
	constructor(
		...
		this._register(
			this._editor.onDidLayoutChange(() => this._actionWidgetService.hide()),
		);
	}
			...
			onFocus: (action: CodeActionItem | undefined) => {
				if (action && action.action) {
					...
					if (ranges && ranges.length > 0) {
						...
						const decorations: IModelDeltaDecoration[] = diagnostics.map(
							(diagnostic) => ({
								range: diagnostic,
								options: CodeActionController.DECORATION,
							}),
