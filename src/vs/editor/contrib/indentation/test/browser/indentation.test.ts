...
suite("Auto Indent On Type - Visual Basic", () => {
	...
	test("issue #118932: dedent on End Module", () => {
		...
		const model = createTextModel(
			["Module Test", "    Private x As Integer", "    End Modul"].join("\n"),
			languageId,
			{},
		);
		disposables.add(model);
		...
		withTestCodeEditor(
			...
			(editor, viewModel) => {
				...
				assert.strictEqual(
					model.getValue(),
					["Module Test", "    Private x As Integer", "End Module"].join("\n"),
				);
			},
		);
	});
	...
	test("issue #118932: dedent on End Select", () => {
		...
		const model = createTextModel(
			["Sub Test()", "    Select Case x", "        End Selec"].join("\n"),
			languageId,
			{},
		);
		disposables.add(model);
		...
		withTestCodeEditor(
			...
			(editor, viewModel) => {
				...
				assert.strictEqual(
					model.getValue(),
					["Sub Test()", "    Select Case x", "    End Select"].join("\n"),
				);
			},
		);
	});
	...
	test("issue #118932: dedent and indent on Catch", () => {
		...
		const model = createTextModel(
			["Sub Test()", "    Try", "        DoSomething()", "        Catc"].join(
				"\n",
			),
			languageId,
			{},
		...
		withTestCodeEditor(
			...
			(editor, viewModel) => {
				...
				assert.strictEqual(
					model.getValue(),
					["Sub Test()", "    Try", "        DoSomething()", "    Catch"].join(
						"\n",
					),
				);
			},
	...
	test("issue #118932: dedent on End Class", () => {
		...
		const model = createTextModel(
			["Class MyClass", "    Private x As Integer", "    End Clas"].join("\n"),
			languageId,
			{},
		);
		disposables.add(model);
		...
		withTestCodeEditor(
			...
			(editor, viewModel) => {
				...
				assert.strictEqual(
					model.getValue(),
					["Class MyClass", "    Private x As Integer", "End Class"].join("\n"),
				);
			},
		);
	});
