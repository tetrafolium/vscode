...
export class AutoIndentOnPaste implements IEditorContribution {
	...
	public trigger(range: Range): void {
		...
		const containsOnlyWhitespace = this.rangeContainsOnlyWhitespaceCharacters(
			model,
			range,
		);
		...
		if (
			!this.editor.getOption(EditorOption.autoIndentOnPasteWithinString) &&
		...
		if (!/\S/.test(firstLineText.substring(0, range.startColumn - 1))) {
			...
			if (indentOfFirstLine !== null) {
				...
				if (newSpaceCnt !== oldSpaceCnt) {
					...
					firstLineText =
						newIndent + firstLineText.substring(oldIndentation.length);
				} else {
