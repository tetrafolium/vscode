	...
	implements IFindController
{
	...
	constructor(
		...
		this._findWidgetSearchHistory =
			FindWidgetSearchHistory.getOrCreate(_storageService);
		this._replaceWidgetHistory =
			ReplaceWidgetHistory.getOrCreate(_storageService);
	}
...
async function matchFindAction(
	editor: ICodeEditor,
	next: boolean,
): Promise<void> {
	const controller = CommonFindController.get(editor);
	...
	const runMatch = (): boolean => {
		...
		if (result) {
			...
			if (!previousSelection && currentSelection) {
				...
				previousSelection &&
				currentSelection &&
				!previousSelection.equalsSelection(currentSelection)
			) {
				landedOnMatch = true;
			}
		}
