...
export class MiddleScrollController
	extends Disposable
			...
			autorun((reader) => {
				...
				reader.store.add(
					this._editor.onMouseDown((e) => {
						...
						const mousePos = observeWindowMousePos(
							getWindow(editorDomNode),
							initialPos,
							store,
						);
						const mouseDeltaAfterThreshold = mousePos.map((v) =>
							v.subtract(initialPos).withThreshold(5),
						);
				...
				reader.store.add(
					autorun((reader) => {
						...
						reader.store.add(
							autorun((reader) => {
								AnimationFrameScheduler.instance.invalidateOnNextAnimationFrame(
									reader,
								);
						...
						reader.store.add(
							autorun((reader) => {
								editorDomNode.setAttribute(
									"data-scroll-direction",
									directionAttr.read(reader),
								);
							}),
						);
					}),
					...
					n
						.div({
							class: [
								...
								scrollingSession.map((session) => (session ? "" : "hidden")),
							],
							style: {
								left: scrollingSession.map((session) =>
									session ? session.initialMousePosInEditor.x : 0,
								),
								top: scrollingSession.map((session) =>
									session ? session.initialMousePosInEditor.y : 0,
								),
							},
						})
				...
				reader.store.add(
					appendRemoveOnDispose(editorDomNode, dotDomElem.element),
				);

				reader.store.add(
					autorun((reader) => {
						const session = scrollingSession.read(reader);
						editorDomNode.classList.toggle(
...
function observeWindowMousePos(
	window: Window,
	initialPos: Point,
