	...
	getDecorations(): ExpectedDecoration[] {
		...
		for (const decoration of decorations) {
			if (decoration.options === TestDecorationProvider.hiddenDecoration) {
				...
				decoration.options === TestDecorationProvider.collapsedDecoration
			) {
				res.push({ line: decoration.range.startLineNumber, type: "collapsed" });
			} else if (
				decoration.options === TestDecorationProvider.expandedDecoration
			) {
...
suite("Folding Model", () => {
	...
	function assertDecorations(
		...
		const decorationProvider =
			foldingModel.decorationProvider as TestDecorationProvider;
