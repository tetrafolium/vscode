		...
		return new Promise((resolve, reject) => {
			...
			const oracle = editor.invokeWithinContext((accessor) =>
				accessor
					...
					.createInstance(SuggestModel, editor),
			);
			...
			return withOracle((suggestOracle, editor) => {
				...
				return new Promise<void>((resolve, reject) => {
					const sub = suggestOracle.onDidSuggest(() => {
						...
						reject(
							new Error(
								'Quick suggestions should have been suppressed by string shorthand "off"',
							),
						);
					});
...
suite(
	"SuggestModel - offWhenInlineCompletions with InlineCompletionsController",
	function () {
		ensureNoDisposablesAreLeakedInTestSuite();
			...
			await runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				try {
					...
					await withAsyncTestCodeEditor(
						text,
						{ serviceCollection },
						async (editor, _editorViewModel, instantiationService) => {
			...
			await withSuggestModelAndInlineCompletions(
				...
				async (suggestModel, editor) => {
					...
					assert.strictEqual(
						didSuggest,
						false,
						"Quick suggestions should have been suppressed when inline completions are showing",
					);
				},
			);
		});
			...
			await withSuggestModelAndInlineCompletions(
				...
				async (suggestModel, editor) => {
					...
					assert.strictEqual(
						didSuggest,
						true,
						"Quick suggestions should have been triggered after inline completions resolved empty",
					);
				},
			);
		});
			...
			await withSuggestModelAndInlineCompletions(
				...
				async (suggestModel, editor) => {
					...
					assert.strictEqual(
						didSuggest,
						true,
						"Quick suggestions should have been triggered when inlineSuggest is disabled",
					);
				},
			);
		});
