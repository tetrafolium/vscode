...
suite("getSecondaryEdits", () => {
	...
	test("cursor not on same line as primary edit 1", async function () {
		...
		assert.deepStrictEqual(
			TextEdit.fromParallelReplacementsUnsorted(
				secondaryEdits.filter(isDefined),
			).toString(textModel.getValue()),
			"...ction fib(❰\n↦) {\n\t... 0;\n}❱",
		);
		textModel.dispose();
	});
	...
	test("cursor not on same line as primary edit 2", async function () {
		...
		assert.deepStrictEqual(secondaryEdits, [
			new TextReplacement(
				new Range(4, 1, 4, 1),
				["	public x: number = 0;", "   public y: number = 0;", "}"].join("\n"),
			),
		]);
		textModel.dispose();
	});
