...
export function formatRecordableLogEntry<T extends IRecordableLogEntry>(
	...
	return (
		...
		JSON.stringify({
			...entry,
			modelUri: (entry as any).modelUri?.toString(),
			sourceId: undefined,
		})
	);
}
...
> extends Disposable {
	...
	constructor(
		...
		@IDataChannelService
		private readonly _dataChannelService: IDataChannelService,
	) {
		super();
	...
	public log(data: T): boolean {
		...
		this._dataChannelService
			.getDataChannel<T>("structuredLogger:" + this._key)
			.sendData(data);
		return true;
	}
}
