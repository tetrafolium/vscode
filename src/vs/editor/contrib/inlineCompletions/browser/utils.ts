...
export function getModifiedRangesAfterApplying(
	edits: readonly TextReplacement[],
): Range[] {
...
export function removeTextReplacementCommonSuffixPrefix(
	...
	const stringReplacements = edits.map((edit) =>
		transformer.getStringReplacement(edit),
	);
	const minimalStringReplacements = stringReplacements.map((replacement) =>
		replacement.removeCommonSuffixPrefix(text),
	);
	return minimalStringReplacements.map((replacement) =>
		transformer.getTextReplacement(replacement),
	);
}
...
export class ErrorResult<T = void> {
	...
	public logError(): void {
		if (this.message) {
			...
			console.error(
				`ErrorResult: An unexpected error-case occurred, usually caused by invalid input.`,
				this.error,
			);
		}
	}
