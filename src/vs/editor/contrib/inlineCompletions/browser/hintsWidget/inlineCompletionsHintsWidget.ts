		...
		this._register(
			autorunWithStore((reader, store) => {
				...
				const contentWidgetValue = derived((reader) => {
					...
					reader.store.add(
						toDisposable(() => editor.removeContentWidget(contentWidget)),
					);
	...
	implements IContentWidget
{
	...
	private createCommandAction(
		...
		action.tooltip = this.keybindingService.appendKeybinding(
			label,
			commandId,
			...
			.div(
				{
					...
					style: {
						...
						margin: 4,
						marginBottom: 4,
						display: derived((reader) =>
							this._warning.read(reader) ? "block" : "none",
						),
					},
				},
