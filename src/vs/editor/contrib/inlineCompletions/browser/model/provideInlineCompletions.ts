...
export type InlineCompletionContextWithoutUuid = Omit<
	InlineCompletionContext,
	"requestUuid"
>;
					...
					if (result) {
						for (const item of result.inlineSuggestions.items) {
							...
							if (item.insertText !== undefined) {
								const t = new TextReplacement(
									Range.lift(item.range) ?? defaultReplaceRange,
									item.insertText,
								);
...
export function runWhenCancelled(
...
function toInlineSuggestData(
	...
	const uri = inlineCompletion.uri
	...
	if (inlineCompletion.jumpToPosition !== undefined) {
		...
		if (!inlineCompletion.hint) {
			return ErrorResult.message(
				"Inline completion has no insertText, jumpToPosition nor hint.",
			);
		}
	}
...
export type IInlineSuggestDataAction =
	| IInlineSuggestDataActionEdit
	| IInlineSuggestDataActionJumpTo;
...
export class InlineSuggestData {
	public static createForTest(
		action: IInlineSuggestDataAction | undefined,
		targetUri: URI,
	): InlineSuggestData {
	...
	private _lastSetEndOfLifeReason: InlineCompletionEndOfLifeReason | undefined =
		undefined;
	private _isPreceeded = false;
	private _partiallyAcceptedCount = 0;
	private _partiallyAcceptedSinceOriginal: PartialAcceptance = {
		characters: 0,
		ratio: 0,
		count: 0,
	};
	...
	public reportPartialAccept(
		...
		this._partiallyAcceptedSinceOriginal.characters +=
			partialAcceptance.characters;
		this._partiallyAcceptedSinceOriginal.ratio = Math.min(
			this._partiallyAcceptedSinceOriginal.ratio +
				(1 - this._partiallyAcceptedSinceOriginal.ratio) *
					partialAcceptance.ratio,
			1,
		);
		...
		if (this.source.provider.handleEndOfLifetime) {
			const summary: LifetimeSummary = {
				...
				partiallyAcceptedCountSinceOriginal:
					this._partiallyAcceptedSinceOriginal.count,
				...
				timeUntilProviderRequest:
					this._providerRequestInfo.startTime - this._requestInfo.startTime,
				timeUntilProviderResponse:
					this._providerRequestInfo.endTime - this._requestInfo.startTime,
				...
				typingIntervalCharacterCount:
					this._requestInfo.typingIntervalCharacterCount,
				skuPlan: this._requestInfo.sku?.plan,
				skuType: this._requestInfo.sku?.type,
				availableProviders: this._requestInfo.availableProviders
					.map((p) => p.toString())
					.join(","),
