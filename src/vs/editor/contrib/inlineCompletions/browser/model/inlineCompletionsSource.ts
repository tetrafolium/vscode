...
} from "../../../../../platform/dataChannel/browser/forwardingTelemetryService.js";
...
import {
	InlineCompletionEndOfLifeEvent,
...
export class InlineCompletionsSource extends Disposable {
		...
		update: (reader, previousValue, changes) => {
			const edit = StringEdit.compose(
				changes.changes
					.map((c) =>
						c.change
							? offsetEditFromContentChanges(c.change.changes)
							: StringEdit.empty,
					)
					.filter(isDefined),
			);
			...
			try {
				return {
					inlineCompletions:
						previousValue.inlineCompletions.createStateWithAppliedEdit(
							edit,
							this._textModel,
						),
	...
	public readonly inlineCompletions = this._state.map(
		this,
		(v) => v.inlineCompletions,
	);
	public readonly suggestWidgetInlineCompletions = this._state.map(
		this,
		(v) => v.suggestWidgetInlineCompletions,
	);
	...
	constructor(
		...
		this._renameProcessor = this._store.add(
			this._instantiationService.createInstance(RenameSymbolProcessor),
		);
		...
		const enablementSetting =
			product.defaultChatAgent?.completionsEnablementSetting ?? undefined;
	...
	private _updateCompletionsEnablement(enalementSetting: string) {
		const result =
			this._configurationService.getValue<Record<string, boolean>>(
				enalementSetting,
			);
		if (!isObject(result)) {
	...
	public fetch(
		...
		const promise = (async () => {
			...
			const loadingReset = store.add(
				new RunOnceScheduler(() => decreaseLoadingCount(), 10 * 1000),
			);
			loadingReset.schedule();

			const inlineSuggestionsProviders = providers.filter((p) => p.providerId);
			const requestResponseInfo = new RequestResponseData(
				context,
				requestInfo,
				inlineSuggestionsProviders,
			);
			...
			try {
				...
				if (
					this._loggingEnabled.get() ||
					this._structuredFetchLogger.isEnabled.get()
				) {
					this._log({
				...
				runWhenCancelled(source.token, () =>
					providerResult.cancelAndDispose({ kind: "tokenCancellation" }),
				);
				...
				for await (const list of providerResult.lists) {
					...
					store.add(
						toDisposable(() =>
							list.removeRef(
								list.inlineSuggestionsData.length === 0
									? { kind: "empty" }
									: { kind: "notTaken" },
							),
						),
					...
					for (const item of list.inlineSuggestionsData) {
						...
						if (
							targetUri &&
							targetUri.toString() !== this._textModel.uri.toString()
						) {
							const modelRef =
								await this._textModelService.createModelReference(targetUri);
				...
				providerSuggestions.forEach((s) =>
					s.addPerformanceMarker("providersResolved"),
				);

				const suggestions: InlineSuggestionItem[] = await Promise.all(
					providerSuggestions.map(async (s) => {
						return this._renameProcessor.proposeRenameRefactoring(
							this._textModel,
							s,
							context,
						);
				...
				requestResponseInfo.setRequestUuid(
					providerResult.contextWithUuid.requestUuid,
				);
				...
				suggestions.forEach((s) =>
					s.addPerformanceMarker("minShowDelayPassed"),
				);
						...
						: this._store.isDisposed
							...
							: this._textModel.getVersionId() !== request.versionId
								...
								: userJumpedToActiveCompletion.get()
									...
									: "unknown";
					suggestions.forEach((s) =>
						s.setNotShownReasonIfNotSet(notShownReason),
					);
					return false;
				}
				...
				transaction((tx) => {
					...
					if (context.selectedSuggestionInfo) {
						this._state.set(
							{
								inlineCompletions: InlineCompletionsState.createEmpty(),
								suggestWidgetInlineCompletions:
									v.suggestWidgetInlineCompletions.createStateWithAppliedResults(
										suggestions,
										request,
										this._textModel,
										cursorPosition,
						...
						this._state.set(
							{
								inlineCompletions:
									v.inlineCompletions.createStateWithAppliedResults(
										suggestions,
										request,
										this._textModel,
										cursorPosition,
								...
								suggestWidgetInlineCompletions:
									InlineCompletionsState.createEmpty(),
							},
							tx,
						);
					}
	...
	public seedInlineCompletionsWithSuggestWidget(): void {
		...
		transaction((tx) => {
			...
			if (
				!inlineCompletions ||
				(suggestWidgetInlineCompletions.request?.versionId ?? -1) >
					(inlineCompletions.request?.versionId ?? -1)
			) {
	...
	private _sendInlineCompletionsRequestTelemetry(
		...
		if (
			requestResponseInfo.requestUuid === undefined ||
			requestResponseInfo.hasProducedSuggestion
		) {
			return;
		}
		...
		if (
			!requestResponseInfo.providers.some((p) =>
				isCopilotLikeExtension(p.providerId?.extensionId),
			)
		) {
			return;
		}
		...
		const emptyEndOfLifeEvent: InlineCompletionEndOfLifeEvent = {
			...
			typingIntervalCharacterCount:
				requestResponseInfo.requestInfo.typingIntervalCharacterCount,
			languageId: requestResponseInfo.requestInfo.languageId,
			selectedSuggestionInfo:
				!!requestResponseInfo.context.selectedSuggestionInfo,
			availableProviders: requestResponseInfo.providers
		...
		const dataChannel = this._instantiationService.createInstance(
			DataChannelForwardingTelemetryService,
		);
...
export class InlineCompletionsState extends Disposable {
	 ...
	 */
	public createStateWithAppliedEdit(
		edit: StringEdit,
		textModel: ITextModel,
	): InlineCompletionsState {
		const newInlineCompletions = this.inlineCompletions
			.map((i) => i.withEdit(edit, textModel))
			.filter(isDefined);
	...
	public createStateWithAppliedResults(
		...
		if (itemIdToPreserveAtTop) {
			...
			if (
				...
				itemToPreserveCandidate.canBeReused(textModel, request.position)
			) {
				itemToPreserve = itemToPreserveCandidate;

				const updatedItemToPreserve = updatedSuggestions.find(
					(i) => i.hash === itemToPreserveCandidate.hash,
				);
				if (updatedItemToPreserve) {
					updatedSuggestions = moveToFront(
						updatedItemToPreserve,
						updatedSuggestions,
					);
				} else {
		...
		const preferInlineCompletions = itemToPreserve
			...
			: // Otherwise: prefer inline completion if there is a visible one
				updatedSuggestions.some(
					(i) => !i.isInlineEdit && i.isVisible(textModel, cursorPosition),
				);
		...
		updatedItems.sort(
			compareBy((i) => i.showInlineEditMenu, booleanComparator),
		);
