...
} from "../../../../../base/browser/animatedValue.js";
...
import {
	TextModelEditSource,
	EditSources,
} from "../../../../common/textModelEditSource.js";
import { ICodeEditorService } from "../../../../browser/services/codeEditorService.js";
import {
	InlineCompletionViewData,
	InlineCompletionViewKind,
} from "../view/inlineEdits/inlineEditsViewInterface.js";
...
export class InlineCompletionsModel extends Disposable {
	...
	private readonly _selectedInlineCompletionId = observableValue<
		string | undefined
	>(this, undefined);
	public readonly primaryPosition = derived(
		this,
		(reader) => this._positions.read(reader)[0] ?? new Position(1, 1),
	);
	public readonly allPositions = derived(this, (reader) =>
		this._positions.read(reader),
	);

	private readonly sku = observableValue<InlineSuggestSku | undefined>(
		this,
		undefined,
	);

	private _isAcceptingPartially = false;
	private readonly _appearedInsideViewport = derived<boolean>(
		this,
		(reader) => {
	...
	constructor(
		...
		private readonly _selectedSuggestItem: IObservable<
			SuggestItemInfo | undefined
		>,
		public readonly _textModelVersionId: IObservableWithChange<
			number | null,
		...
		this.lastTriggerKind = this._source.inlineCompletions.map(
			this,
		...
		this._inlineEditsShowCollapsedEnabled = inlineSuggest.map(
			(s) => s.edits.showCollapsed,
		);
		this._triggerCommandOnProviderChange = inlineSuggest.map(
			(s) => s.triggerCommandOnProviderChange,
		);
		this._minShowDelay = inlineSuggest.map((s) => s.minShowDelay);
		this._showOnSuggestConflict = inlineSuggest.map(
			(s) => s.experimental.showOnSuggestConflict,
		);
		this._suppressInSnippetMode = inlineSuggest.map(
			(s) => s.suppressInSnippetMode,
		);

		const snippetController = SnippetController2.get(this._editor);
		this._isInSnippetMode =
			snippetController?.isInSnippetObservable ?? constObservable(false);

		defaultAccountService
			.getDefaultAccount()
			.then(
				createDisposableCb(
					(account) => this.sku.set(skuFromAccount(account), undefined),
					this._store,
				),
			);
		this._register(
			defaultAccountService.onDidChangeDefaultAccount((account) =>
				this.sku.set(skuFromAccount(account), undefined),
			),
		);
...

		{
			...
			const isNotebook =
				this.textModel.uri.scheme === Schemas.vscodeNotebookCell;
			...
			recomputeInitiallyAndOnChange(this.state, (s) => {
				if (s && s.inlineSuggestion) {
					this._inlineCompletionsService.reportNewCompletion(
						s.inlineSuggestion.requestUuid,
					);
				}
			}),
		);
			...
			autorun((reader) => {
				const jumpToReset = this.state
					.map(
						(s, reader) =>
							!s ||
							(s.kind === "inlineEdit" && !s.cursorAtInlineEdit.read(reader)),
					)
					.read(reader);
				if (jumpToReset) {
		...
		this._register(
			autorun((reader) => {
				const inlineSuggestion = this.state
					.map((s) => s?.inlineSuggestion)
					.read(reader);
				if (inlineSuggestion) {
		...
		const inlineEditSemanticId = this.inlineEditState.map(
			(s) => s?.inlineSuggestion.semanticId,
		);
			...
			autorun((reader) => {
				...
				if (id) {
					...
					this._lastShownInlineCompletionInfo = {
						alternateTextModelVersionId:
							this.textModel.getAlternativeVersionId(),
			...
			changeTracker: {
				...
				handleChange: (ctx, changeSummary) => {
					changeSummary.didUndo =
						ctx.didChange(this._textModelVersionId) && !!ctx.change?.isUndoing;
					return true;
				},
			},
		},
	...
	public debugGetSelectedSuggestItem(): IObservable<
		SuggestItemInfo | undefined
	> {
				...
				handleChange: (ctx, changeSummary) => {
					...
					if (ctx.didChange(this._textModelVersionId)) {
						...
						changeSummary.changeReason =
							detailedReasons.length > 0 ? detailedReasons[0].getType() : "";
		...
		(reader, changeSummary) => {
			...
			const shouldUpdate =
				((this._enabled.read(reader) &&
					this._selectedSuggestItem.read(reader)) ||
			...
			const suggestWidgetInlineCompletions =
				this._source.suggestWidgetInlineCompletions.read(undefined);
			...
			if (
				...
				if (
					...
					context = {
						...context,
						includeInlineCompletions:
							!this._lastShownInlineCompletionInfo.inlineCompletion
								.isInlineEdit,
						includeInlineEdits:
							this._lastShownInlineCompletionInfo.inlineCompletion.isInlineEdit,
					};
				}
			}

			const itemToPreserveCandidate =
				this.selectedInlineCompletion.read(undefined) ??
				this._inlineSuggestionItems.read(undefined)?.inlineEdit;
			...
			const userJumpedToActiveCompletion = this._jumpedToId.map(
				(jumpedTo) =>
					!!jumpedTo &&
					jumpedTo ===
						this._inlineSuggestionItems.read(undefined)?.inlineEdit?.semanticId,
			);
			...
			requestInfo.availableProviders = availableProviders
				.map((p) => p.providerId)
				.filter(isDefined);
	...
	private getAvailableProviders(
		...
		const suppressedProviderGroupIds =
			this._suppressedInlineCompletionGroupIds.get();
		const unsuppressedProviders = providers.filter(
			(provider) =>
				!(provider.groupId && suppressedProviderGroupIds.has(provider.groupId)),
		);
	...
	public async triggerExplicitly(
		tx?: ITransaction,
		onlyFetchInlineEdits: boolean = false,
	): Promise<void> {
	...
	private readonly _inlineSuggestionItems = derivedOpts(
		{ owner: this },
		(reader) => {
	...
	public readonly activeCommands = derivedOpts<InlineCompletionCommand[]>(
		{ owner: this, equalsFn: arrayEqualsC() },
		(r) =>
			this.selectedInlineCompletion.read(r)?.source.inlineSuggestions
				.commands ?? [],
	);
	...
	private readonly _hasVisiblePeekWidgets = derived(
		this,
	...
	>(
		...
		(reader) => {
			...
			if (
				this._suppressInSnippetMode.read(reader) &&
				this._isInSnippetMode.read(reader)
			) {
				return undefined;
			}
			...
			if (inlineEditResult) {
				...
				const stringEdit =
					inlineEditResult.action?.kind === "edit"
						? inlineEditResult.action.stringEdit
						: undefined;
				const replacements = stringEdit
					? TextEdit.fromStringEdit(
							stringEdit,
			...
			if (!this._shouldShowOnSuggestConflict.read(reader) && suggestItem) {
				...
				const primaryGhostText =
					ghostTexts[0] ?? new GhostText(fullEdit.range.endLineNumber, []);
				return {
					kind: "ghostText",
					edits,
					primaryGhostText,
	...
	private _computeAugmentation(
		suggestCompletion: TextReplacement,
	...
	public readonly showCollapsed = derived<boolean>(this, (reader) => {
		...
		if (
			state.inlineSuggestion.hint ||
			state.inlineSuggestion.action?.kind === "jumpTo"
		) {
			return false;
		}

		const isCurrentModelVersion =
			state.inlineSuggestion.updatedEditModelVersion ===
			this._textModelVersionId.read(reader);
		return (
			(this._inlineEditsShowCollapsedEnabled.read(reader) ||
	...
	private readonly _tabShouldIndent = derived(this, (reader) => {
		...
		function getNonIndentationRange(
			model: ITextModel,
			lineNumber: number,
		): Range {
		...
		return selections?.some((s) => {
			if (s.isEmpty()) {
				...
				return (
					isMultiLine(s) ||
					s.containsRange(
						getNonIndentationRange(this.textModel, s.startLineNumber),
					)
				);
			}
		});
	...
	public readonly tabShouldJumpToInlineEdit = derived(this, (reader) => {
		...
		if (
			this._inAcceptFlow.read(reader) &&
			this._appearedInsideViewport.read(reader)
		) {
			return false;
		}
	...
	public readonly tabShouldAcceptInlineEdit = derived(this, (reader) => {
		...
		if (
			s.inlineSuggestion.targetRange.startLineNumber ===
	...
	public async accept(
		editor: ICodeEditor = this._editor,
		alternativeAction: boolean = false,
	): Promise<void> {
		...
		try {
			...
			if (!completion.originalTextRef.targets(this.textModel)) {
				...
				if (targetEditor) {
					...
					targetEditor.revealLineInCenter(
						completion.targetRange.startLineNumber,
					);
				}
				...
				if (alternativeAction && action.alternativeAction) {
					...
					const edit = TextEdit.fromParallelReplacementsUnsorted([
						mainEdit,
						...additionalEdits,
					]);
					editor.edit(
						edit,
						this._getMetadata(completion, this.textModel.getLanguageId()),
					);

					editor.setPosition(
						action.snippetInfo.range.getStartPosition(),
					...
					if (state.kind === "ghostText") {
						minimalEdits = removeTextReplacementCommonSuffixPrefix(
							edits,
							this.textModel,
						);
					}
					const selections = getEndPositionsAfterApplying(minimalEdits).map(
	...
	public async acceptNextWord(): Promise<void> {
		await this._acceptNext(
			...
			(pos, text) => {
				...
				const config =
					this._languageConfigurationService.getLanguageConfiguration(langId);
	...
	private readonly _jumpedToId = observableValue<undefined | string>(
		this,
		undefined,
	);
	...
	public jump(): void {
		...
		try {
			transaction((tx) => {
				...
				if (
					isSingleLineChange ||
					s.inlineSuggestion.action?.kind === "jumpTo"
				) {
					this._editor.revealPosition(targetPosition, ScrollType.Smooth);
				} else {
					const revealRange = new Range(
						targetRange.startLineNumber - 1,
						1,
						targetRange.endLineNumber + 1,
						1,
					);
...
export function getSecondaryEdits(
	...
	const secondaryOffsets = positions
		.slice(1)
		.map((pos) => textTransformer.getOffset(pos));

	primaryTextRepl = primaryTextRepl.removeCommonPrefixAndSuffix(text);
	const primaryStringRepl =
		textTransformer.getStringReplacement(primaryTextRepl);

	const deltaFromOffsetToRangeStart =
		primaryStringRepl.replaceRange.start - primaryOffset;
	const primaryContextRange = primaryStringRepl.replaceRange.join(
		OffsetRange.emptyAt(primaryOffset),
	);
	...
	const replacements = secondaryOffsets
		.map((secondaryOffset) => {
			...
			const stringRepl = new StringReplacement(
				range,
				primaryStringRepl.newText,
			);
...
class FadeoutDecoration extends Disposable {
	constructor(editor: ICodeEditor, ranges: Range[], onDispose?: () => void) {
		...
		const val = new ObservableAnimatedValue(
			AnimatedValue.startNow(1, 0, 1000, easeOutCubic),
		);
		...
		this._register(
			autorun((reader) => {
				...
				editor
					.getContainerDomNode()
					.style.setProperty("--animation-opacity", opacity.toString());
...
function skuFromAccount(
	account: IDefaultAccount | null,
): InlineSuggestSku | undefined {
	if (
		account?.entitlementsData?.access_type_sku &&
		account?.entitlementsData?.copilot_plan
	) {
		return {
