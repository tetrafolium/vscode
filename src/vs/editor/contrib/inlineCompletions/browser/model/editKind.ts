...
export function computeEditKind(
	...
	return new InlineSuggestionEditKind(
		edit.replacements.map((rep) =>
			computeSingleEditKind(rep, textModel, cursorPosition),
		),
	);
}
...
function computeSingleEditKind(
	...
	const kind = replaceRange.isEmpty
		? "insert"
		: newText.length === 0
			? "delete"
			: "replace";
	switch (kind) {
		case "insert":
			return {
				operation: "insert",
				properties: computeInsertProperties(
					replaceRange.start,
					newText,
					textModel,
					cursorPosition,
		...
		case "delete": {
			const deletedText = textModel
				.getValue()
				.substring(replaceRange.start, replaceRange.endExclusive);
			return {
				operation: "delete",
				properties: computeDeleteProperties(
					replaceRange.start,
...
function computeInsertProperties(
	...
	if (cursorPosition) {
		...
		const beforeCursorOnSameLine =
			cursorLine === insertLine && insertColumn < cursorColumn;
		const afterCursorOnSameLine =
			cursorLine === insertLine && insertColumn > cursorColumn;
		const linesAbove =
			insertLine < cursorLine ? cursorLine - insertLine : undefined;
		const linesBelow =
			insertLine > cursorLine ? insertLine - cursorLine : undefined;
...
function computeDeleteProperties(
	startOffset: number,
	endOffset: number,
	textModel: ITextModel,
): DeleteProperties {
...
function computeReplaceProperties(
	oldText: string,
	newText: string,
): ReplaceProperties {
	...
	const isSingleLineToSingleLine =
		oldShape.kind === "singleLine" && newShape.kind === "singleLine";
	const isSingleLineToMultiLine =
		oldShape.kind === "singleLine" && newShape.kind === "multiLine";
	const isMultiLineToSingleLine =
		oldShape.kind === "multiLine" && newShape.kind === "singleLine";
