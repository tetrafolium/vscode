...
export const providerIdSchemaUri =
	"vscode://schemas/inlineCompletionProviderIdArgs";

export function inlineCompletionProviderGetMatcher(
...
const argsValidator = vUnion(
	vObj({
		...
		providerId: vOptionalProp(
			vWithJsonSchemaRef(providerIdSchemaUri, vString()),
		),
	...
	public override async run(
		...
		const provider = validatedArgs?.providerId
			? languageFeaturesService.inlineCompletionsProvider
					.all(editor.getModel()!)
					.find((p) =>
						inlineCompletionProviderGetMatcher(p).some(
							(m) => m === validatedArgs.providerId,
						),
					)
			: undefined;
