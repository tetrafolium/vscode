...
} from "../../../linesOperations/browser/linesOperations.js";
...
setInlineCompletionsControllerGetter((editor) =>
...
export class InlineCompletionsController extends Disposable {
	...
	public readonly model = derivedDisposable<InlineCompletionsModel | undefined>(
		this,
		(reader) => {
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				...
				const nextEditUri =
					state.kind === "inlineEdit" ? state.nextEditUri : undefined;
				for (const ctrl of InlineCompletionsController._instances) {
					if (ctrl === this) {
						continue;
					} else if (
						nextEditUri &&
						isEqual(nextEditUri, ctrl.editor.getModel()?.uri)
					) {
		...
		this._register(
			this._commandService.onDidExecuteCommand((e) => {
				if (
					triggerCommands.has(e.commandId) &&
					editor.hasTextFocus() &&
					this._enabled.get()
				) {
					let noDelay = false;
		...
		this._register(
			autorun((reader) => {
				...
				if (
					model.state.read(undefined)?.inlineSuggestion
						?.isFromExplicitRequest &&
			...
			contextKeySvcObs.bind(
				...
				(reader) => {
					...
					return state?.primaryGhostText && state?.inlineSuggestion
						? state.inlineSuggestion.source.inlineSuggestions
								.suppressSuggestions
						: undefined;
				},
			),
			...
			contextKeySvcObs.bind(
				...
				(reader) => {
					...
					return (
						action &&
						action.kind === "edit" &&
						action.alternativeAction !== undefined
					);
				},
			),
		);
		...
		const firstGhostTextPos = derived(this, (reader) => {
			...
			const firstPartPos = new Position(
				primaryGhostText.lineNumber,
				primaryGhostText.parts[0].column,
			);
			return firstPartPos;
		});
		this._register(
