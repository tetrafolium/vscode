...
export class WordReplacementsViewData implements IEquatable<WordReplacementsViewData> {
	constructor(
		...
		public readonly alternativeAction:
	...
	equals(other: WordReplacementsViewData): boolean {
		return (
			...
			this.alternativeAction === other.alternativeAction
		);
	}
}
...
export class InlineEditsWordReplacementView
	extends Disposable
	implements IInlineEditsView
{
	...
	constructor(
		...
		this._end = this._editor.observePosition(
			constObservable(this._viewData.edit.range.getEndPosition()),
			this._store,
		);
		this._line = document.createElement("div");
		this._primaryElement = observableValue<ObserverNodeWithElement | null>(
			this,
			null,
		);
		this._secondaryElement = observableValue<ObserverNodeWithElement | null>(
			this,
			null,
		);
		this.isHovered = derived(this, (reader) => {
			...
			return this._userInteractionService
				.createHoverTracker(elem.element, reader.store)
				.read(reader);
		});
		this._renderTextEffect = derived(this, (_reader) => {
			...
			const origLine = tm.getLineContent(
				this._viewData.edit.range.startLineNumber,
			);
		...
		const modifiedLineHeight = this._editor.observeLineHeightForPosition(
			this._viewData.edit.range.getStartPosition(),
		);
		const altCount = observableFromPromise(
			this._viewData.alternativeAction?.count ??
		...
		const altModifierActive = derived(
			...
			(reader) =>
				this._userInteractionService.readModifierKeyStatus(
					this._editor.editor.getDomNode()!,
					reader,
				).shiftKey,
		);
		this._layout = derived(this, (reader) => {
			...
			if (
				!widgetStart ||
				!widgetEnd ||
				widgetStart.x > widgetEnd.x ||
				widgetStart.y > widgetEnd.y
			) {
				return undefined;
			}
			...
			if (this._viewData.alternativeAction) {
				...
				alternativeAction = {
					label:
						count !== undefined ? (active ? occurrencesLabel : label) : label,
					tooltip: occurrencesLabel
						? `${occurrencesLabel}\n${keybindingTooltip}`
						: undefined,
			...
			const modifiedLine = codeLine.withWidth(
				codeLine.width +
					(alternativeAction
						? alternativeAction.label.length * w + 8 + 4 + 12
						: 0),
			);
		...
		this.minEditorScrollHeight = derived(this, (reader) => {
			...
			return (
				layout.read(reader).modifiedLine.bottom +
				BORDER_WIDTH +
				...
				[
					derived(this, (reader) => {
						...
						const alternativeActionActive = derived(
							(reader) =>
								(alternativeAction.read(reader)?.active.read(reader) ??
									false) ||
								secondaryElementHovered.read(reader),
						);
						...
						const hcBorderColor = isHighContrast
							? observeColor(contrastBorder, this._themeService).read(reader)
							: null;
						...
						const secondaryActiveStyles = {
							borderColor: hcBorderColor
								? hcBorderColor.toString()
								: asCssVariable(inlineEditIndicatorPrimaryBorder),
						...
						const primaryActionStyles = derived(this, (r) =>
							alternativeActionActive.read(r)
								? primaryActiveStyles
								: primaryActiveStyles,
						);
										...
										[
											n.div(
												{
													...
													style: {
														...
														borderRadius: `${INLINE_EDITS_BORDER_RADIUS}px`,
														border: primaryActionStyles.map(
															(s) => `${BORDER_WIDTH}px solid ${s.borderColor}`,
														),
														boxSizing: "border-box",
														padding: `${BORDER_WIDTH}px`,
														opacity: primaryActionStyles.map((s) => s.opacity),
														background: primaryActionStyles.map(
															(s) => s.backgroundColor,
														),
														display: "flex",
														justifyContent: "left",
											...
											derived(this, (reader) => {
												...
												const keybindingLabel = reader.store.add(
													new KeybindingLabel(keybinding, OS, {
														...unthemedKeybindingLabelOptions,
														disableTitle: true,
													}),
												);
												...
												return n.div(
													{
														...
														style: {
															...
															borderColor: secondaryActionStyles.map(
																(s) => s.borderColor,
															),
															opacity: secondaryActionStyles.map(
																(s) => s.opacity,
															),
															...
															padding: "0 4px 0 1px",
															marginLeft: "4px",
															background: secondaryActionStyles.map(
																(s) => s.backgroundColor,
															),
															cursor: "pointer",
															textWrap: "nowrap",
														...
														ref: (elem) => {
															if (altAction.tooltip) {
																reader.store.add(
																	this._hoverService.setupDelayedHoverAtMouse(
																		elem,
																		{
									...
									n.div(
										{
											style: {
												...
												...rectToProps((reader) =>
													layout
														.read(reader)
														.originalLine.withMargin(BORDER_WIDTH),
												),
												boxSizing: "border-box",
												borderRadius: `${INLINE_EDITS_BORDER_RADIUS}px`,
									...
									n.svg(
										{
											...
											style: {
												...
												top: layout.map(
													(l) =>
														l.modifiedLine.top +
														Math.round((l.lineHeight - 14 - 5) / 2),
												),
												pointerEvents: "none",
											},
											onmousedown: (e) => this._mouseDown(e),
	...
	private _mouseDown(e: MouseEvent): void {
		...
		this._onDidClick.fire(
			InlineEditClickEvent.create(e, target_id === DOM_ID_RENAME),
		);
	}
}

function traverseParentsUntilId(
	element: HTMLElement,
	ids: Set<string>,
): string | null {
