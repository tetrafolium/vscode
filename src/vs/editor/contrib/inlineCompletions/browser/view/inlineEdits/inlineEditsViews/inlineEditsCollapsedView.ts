	...
	implements IInlineEditsView
{
	...
	constructor(
		...
		const firstEdit = this._edit.map(
			(inlineEdit) => inlineEdit?.edit?.replacements[0] ?? null,
		);
		...
		return n.svg(
			{
				...
				style: {
					position: "absolute",
					...rectToProps((r) =>
						getEditorValidOverlayRect(this._editorObs).read(r),
					),
					overflow: "hidden",
					pointerEvents: "none",
				},
