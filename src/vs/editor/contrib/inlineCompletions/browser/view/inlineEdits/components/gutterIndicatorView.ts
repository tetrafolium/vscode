...
export class InlineSuggestionGutterMenuData {
	public static fromInlineSuggestion(
		suggestion: InlineSuggestionItem,
	): InlineSuggestionGutterMenuData {
		const alternativeAction =
			suggestion.action?.kind === "edit"
				? suggestion.action.alternativeAction
				: undefined;
		...
		return new InlineSuggestionGutterMenuData(
			...
			suggestion.source.provider.displayName ??
...
export class SimpleInlineSuggestModel {
	public static fromInlineCompletionModel(
...
export class InlineEditsGutterIndicator extends Disposable {
	...
	readonly onDidCloseWithCommand: Event<string> =
		this._onDidCloseWithCommand.event;

	constructor(
		private readonly _editorObs: ObservableCodeEditor,
		private readonly _data: IObservable<
			InlineEditsGutterIndicatorData | undefined
		>,
		...
		this._originalRangeObs = mapOutFalsy(
			this._data.map((d) => d?.originalRange),
		);
	...
	private readonly _modifierPressed = derived(
		this,
		(reader) =>
			this._userInteractionService.readModifierKeyStatus(
				this._editorObs.editor.getDomNode()!,
				reader,
			).shiftKey,
	);
	...
	private readonly _lineNumberToRender = derived(this, (reader) => {
		...
		const lineNumberOptions = this._editorObs
			.getOption(EditorOption.lineNumbers)
			.read(reader);
		...
		if (lineNumberOptions.renderType === RenderLineNumbersType.Interval) {
			...
			if (
				lineNumber % 10 === 0 ||
				(cursorPosition && cursorPosition.lineNumber === lineNumber)
			) {
	...
	private readonly _availableWidthForIcon = derived(this, (reader) => {
		...
		const gutterWidth =
			layout.decorationsLeft + layout.decorationsWidth - layout.glyphMarginLeft;
		...
		const lineNumberOptions = this._editorObs
			...
			.read(reader);
		if (
			lineNumberOptions.renderType ===
				RenderLineNumbersType.Relative /* likely to flicker */ ||
			lineNumberOptions.renderType === RenderLineNumbersType.Off
		) {
			return () => gutterWidth;
		}
	...
	private readonly _layout = derived(this, (reader) => {
		...
		const lineHeight = this._editorObs
			.observeLineHeightForLine(s.range.map((r) => r.startLineNumber))
			.read(reader);
		...
		const gutterWidthWithoutPadding =
			layout.decorationsLeft +
			layout.decorationsWidth -
			layout.glyphMarginLeft -
			2 * gutterViewPortPaddingLeft;
		const gutterHeightWithoutPadding =
			layout.height - 2 * gutterViewPortPaddingTop;
		...
		const gutterViewPortWithoutStickyScrollWithoutPaddingTop =
			gutterViewPortWithStickyScroll.withTop(
				this._stickyScrollHeight.read(reader),
			);
		const gutterViewPortWithoutStickyScroll =
			gutterViewPortWithStickyScroll.withTop(
				gutterViewPortWithoutStickyScrollWithoutPaddingTop.top +
					gutterViewPortPaddingTop,
			);
		...
		const pillFullyDockedRect = gutterEditArea
			...
			.translateY(pillOffset);
		const pillIsFullyDocked =
			gutterViewPortWithoutStickyScrollWithoutPaddingTop.containsRect(
				pillFullyDockedRect,
			);
		...
		const iconNoneDocked = customIcon
			...
			: this._tabAction.map((action) =>
					action === InlineEditTabAction.Accept
						? Codicon.keyboardTab
						: Codicon.arrowRight,
				);
		const iconDocked = customIcon
			...
			: derived(this, (reader) => {
					...
					const cursorLineNumber =
						this._editorObs.cursorLineNumber.read(reader) ?? 0;
					const editStartLineNumber = s.range.read(reader).startLineNumber;
					return cursorLineNumber <= editStartLineNumber
		...
		const iconWidth = (pillRect: Rect) => {
			const availableIconAreaWidth =
				this._availableWidthForIcon.read(undefined)(
					pillRect.bottom + this._editorObs.editor.getScrollTop(),
				) - gutterViewPortPaddingLeft;
			return Math.max(
				Math.min(availableIconAreaWidth, idealIconAreaWidth),
				CODICON_SIZE_PX,
			);
		};
		...
		if (pillIsFullyDocked) {
			...
			if (layout.lineNumbersWidth === 0) {
				widthUntilLineNumberEnd = Math.max(
					...
					Math.min(
						Math.max(
							layout.lineNumbersLeft - gutterViewPortWithStickyScroll.left,
							0,
						),
				...
				widthUntilLineNumberEnd = Math.max(
					layout.lineNumbersLeft +
						layout.lineNumbersWidth -
						gutterViewPortWithStickyScroll.left,
					0,
				);
			}
			...
			const iconWidth = Math.min(
				pillRect.width - widthUntilLineNumberEnd,
				idealIconAreaWidth,
			);
		...
		const pillPartiallyDockedPossibleArea =
			gutterViewPortWithStickyScroll.intersect(gutterEditArea); // The area in which the pill could be partially docked
		const pillIsPartiallyDocked =
			pillPartiallyDockedPossibleArea &&
			pillPartiallyDockedPossibleArea.height >= pillHeight;
		...
		const iconDirection =
			pillRect.top < pillFullyDockedRect.top
				? ("top" as const)
				: ("bottom" as const);
	...
	private readonly _isHoveredOverIconDebounced: IObservable<boolean> =
		debouncedObservable(this._isHoveredOverIcon, 100);
						...
						n.div(
							{
								...
								onclick: () => {
									...
									if (!data) {
										throw new BugIndicatingError(
											"Gutter indicator data not available",
										);
									}
								...
								style: {
									...
									["--vscodeIconForeground" as any]:
										this._gutterIndicatorStyles.map((v) => v.foreground),
									...
									borderRadius: `${INLINE_EDITS_BORDER_RADIUS}px`,
									display: "flex",
									justifyContent: layout.map((l) =>
										l.iconDirection === "bottom" ? "flex-start" : "flex-end",
									),
								...
								n.div(
									{
										...
										style: {
											lineHeight: layout.map((l) =>
												l.lineNumberRect ? l.lineNumberRect.height : 0,
											),
											display: layout.map((l) =>
												l.lineNumberRect ? "flex" : "none",
											),
											alignItems: "center",
											justifyContent: "flex-end",
											width: layout.map((l) =>
												l.lineNumberRect ? l.lineNumberRect.width : 0,
											),
											height: "100%",
								...
								n.div(
									{
										style: {
											transform: layout.map(
												(l) =>
													`rotate(${getRotationFromDirection(l.iconDirection)}deg)`,
											),
											transition:
											...
											marginRight: layout.map(
												(l) =>
													l.pillRect.width -
													l.iconRect.width -
													(l.lineNumberRect?.width ?? 0),
											),
											width: layout.map((l) => l.iconRect.width),
											position: "relative",
											right: layout.map((l) =>
												l.iconDirection === "top" ? "1px" : "0",
											),
											color: this._data.map((d) =>
												d?.customization?.icon?.color
													? asCssVariable(d.customization.icon.color.id)
													: undefined,
											),
										},
									},
									[
										layout.map((l, reader) =>
											withStyles(renderIcon(l.icon.read(reader)), {
												fontSize: toPx(
													Math.min(
														l.iconRect.width - CODICON_PADDING_PX,
														CODICON_SIZE_PX,
													),
												),
											}),
...
function withStyles<T extends HTMLElement>(
	element: T,
