...
export class LongDistancePreviewEditor extends Disposable {
	...
	constructor(
		...
		private readonly _properties: IObservable<
			ILongDistancePreviewProps | undefined
		>,
		...
		this._register(
			autorun((reader) => {
				...
				const cursorPosition =
					this._parentEditorObs.cursorPosition.read(reader);
				if (cursorPosition) {
		...
		this._register(
			autorun((reader) => {
				...
				const lineNumberDigets =
					state.visibleLineRange.startLineNumber.toString().length;
				this.previewEditor.updateOptions({
					lineNumbersMinChars: lineNumberDigets + 1,
				});
			}),
		);
	...
	>(this, (reader) => {
		...
		if (props.nextCursorPosition !== null) {
			...
			if (props.diff[0].innerChanges?.every((c) => c.modifiedRange.isEmpty())) {
				mode = "original";
				visibleRange = LineRange.ofLength(
					props.diff[0].original.startLineNumber,
					1,
				);
			} else {
				mode = "modified";
				visibleRange = LineRange.ofLength(
					props.diff[0].modified.startLineNumber,
					1,
				);
			}
		}
	...
	private _getHorizontalContentRangeInPreviewEditorToShow(
		...
		if (jumpToPos) {
			...
			firstCharacterChange =
				state.mode === "modified"
					? diff[0].innerChanges[0].modifiedRange
		...
		const indentCol = editor
			.getModel()!
			.getLineFirstNonWhitespaceColumn(preferredRange.startLineNumber);
	...
	public layout(
		...
		desiredPreviewEditorScrollLeft: number,
	): void {
 ...
 */
function growUntilVariableBoundaries(
