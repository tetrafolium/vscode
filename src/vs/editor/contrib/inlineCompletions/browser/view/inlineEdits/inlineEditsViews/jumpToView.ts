...
export class JumpToView extends Disposable {
	...
	constructor(
		...
		private readonly _data: IObservable<
			{ jumpToPosition: Position } | undefined
		>,
	...
	private readonly _layout = derived(this, (reader) => {
		...
		const lineHeight = this._editor
			.observeLineHeightForLine(constObservable(position.lineNumber))
			.read(reader);
			...
			return n.div(
				{
					style: {
						...
						["--vscodeIconForeground" as string]: this._styles.map(
							(s) => s.foreground,
						),
				...
				[
					this._style === "cursor"
						...
						: [
								derived(() =>
									n.elem("div", {}, keybindingLabel(this._keybinding)),
								),
								n.elem(
									"div",
									{
										style: {
											lineHeight: this._layout.map((l) => l?.widgetRect.height),
											marginTop: "-2px",
										},
									},
									["to jump"],
			...
			ref: (elem) => {
				const keybindingLabel = _reader.store.add(
					new KeybindingLabel(elem, OS, {
						...
						keybindingLabelForeground: asCssVariable(
							inlineEditIndicatorPrimaryForeground,
						),
						keybindingLabelBackground: "transparent",
						keybindingLabelBorder: asCssVariable(
							inlineEditIndicatorPrimaryForeground,
						),
