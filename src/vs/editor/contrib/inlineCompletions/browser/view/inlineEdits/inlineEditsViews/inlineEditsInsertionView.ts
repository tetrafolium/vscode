...
} from "../../../../../../common/viewModel/inlineDecorations.js";
import { GhostText, GhostTextPart } from "../../../model/ghostText.js";
import { InlineCompletionEditorType } from "../../../model/provideInlineCompletions.js";
import {
	GhostTextView,
	IGhostTextWidgetData,
} from "../../ghostText/ghostTextView.js";
import {
	IInlineEditsView,
...
export class InlineEditsInsertionView
	extends Disposable
	implements IInlineEditsView
{
	...
	private readonly _trimVertically = derived(this, (reader) => {
		...
		const lineHeight = this._editor.getLineHeightForPosition(
			new Position(state.lineNumber, 1),
		);
	...
	private readonly _maxPrefixTrim = derived(this, (reader) => {
		...
		if (trimVertically.linesTop === 0) {
			modifiedLines[0] =
				textModel.getLineContent(state.lineNumber) + modifiedLines[0];
		}
	...
	private readonly _display = derived(this, (reader) =>
		!!this._state.read(reader) ? "block" : "none",
	);
	...
	private readonly _editorMaxContentWidthInRange = derived(this, (reader) => {
		...
		const textBeforeInsertion = state.text.startsWith(eol)
			...
			: textModel.getValueInRange(
					new Range(state.lineNumber, 1, state.lineNumber, state.column),
				);
	...
	public readonly startLineOffset = this._trimVertically.map(
		(v) => v.topOffset,
	);
	...
	private readonly _overlayLayout = derived(this, (reader) => {
		...
		this._editorObs
			.observePosition(
				observableValue(this, new Position(state.lineNumber, state.column)),
				reader.store,
			)
			.read(reader);
		...
		const prefixLeftOffset =
			this._maxPrefixTrim.read(reader).prefixLeftOffset ??
			0; /* fix due to observable bug? */
		const left =
			editorLayout.contentLeft + prefixLeftOffset - horizontalScrollOffset;
		if (right <= left) {
			return null;
		}

		const { topOffset: topTrim, bottomOffset: bottomTrim } =
			this._trimVertically.read(reader);

		const scrollTop = this._editorObs.scrollTop.read(reader);
		const height =
			this._ghostTextView.height.read(reader) - topTrim - bottomTrim;
		const top =
			this._editor.getTopForLineNumber(state.lineNumber) - scrollTop + topTrim;
		...
		return {
			...
			minContentWidthRequired:
				prefixLeftOffset + overlay.width + verticalScrollbarWidth,
		};
			...
			derived(this, (reader) => {
				...
				const overlayRect = overlayLayoutObs.map((l) =>
					l.overlay
						...
						.intersectHorizontal(
							new OffsetRange(overlayHider.left, Number.MAX_SAFE_INTEGER),
						),
				);
				const underlayRect = overlayRect.map((rect) =>
					rect.withMargin(separatorWidth, separatorWidth),
				);

				const editorBackground = getEditorBackgroundColor(
					this._input.read(undefined)?.editorType ??
