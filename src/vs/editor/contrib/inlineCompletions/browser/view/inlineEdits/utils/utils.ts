...
export function getEditorValidOverlayRect(
	...
	const width = derived({ name: "editor.validOverlay.width" }, (r) => {
		...
		const editorWidth = Math.max(
			0,
			editor.layoutInfoWidth.read(r) - contentLeft.read(r),
		);
 ...
 */
function indentSizeToIndentLength(
	line: string,
	indentSize: number,
	tabSize: number,
): number {
...
export function createReindentEdit(
	text: string,
	range: LineRange,
	tabSize: number,
): TextEdit {
	...
	range.forEach((lineNumber) => {
		const indentLength = indentSizeToIndentLength(
