...
import {
	AbstractText,
	StringText,
} from "../../../../../common/core/text/abstractText.js";
...
export class InlineEditsView extends Disposable {
	...
	private readonly _tabAction = derived<InlineEditTabAction>(
		(reader) =>
			this._model.read(reader)?.tabAction.read(reader) ??
			InlineEditTabAction.Inactive,
	);
	...
	constructor(
		...
		private readonly _simpleModel: IObservable<
			SimpleInlineSuggestModel | undefined
		>,
		private readonly _inlineSuggestInfo: IObservable<
			InlineSuggestionGutterMenuData | undefined
		>,
		...
		this._inlineCollapsedView = this._register(
			this._instantiationService.createInstance(
				...
				this._model.map((m, reader) =>
					this._uiState.read(reader)?.state?.kind ===
					InlineCompletionViewKind.Collapsed
						? m?.inlineEdit
						: undefined,
				),
			),
		...
		this._customView = this._register(
			this._instantiationService.createInstance(
				...
				this._model.map((m, reader) =>
					this._uiState.read(reader)?.state?.kind ===
					InlineCompletionViewKind.Custom
						? m?.displayLocation
						: undefined,
				),
				this._tabAction,
				this._uiState.map(
					(s) => s?.editorType ?? InlineCompletionEditorType.TextEditor,
				),
			),
		);
		...
		this._inlineDiffViewState = derived<
		...
		const minEditorScrollHeight = derived(this, (reader) => {
			return Math.max(
				...this._wordReplacementViews
					.read(reader)
					.map((v) => v.minEditorScrollHeight.read(reader)),
			...
			autorun((reader) => {
				...
				this._editor.changeViewZones((accessor) => {
					...
					const viewZoneHeight =
						minScrollHeight -
						scrollHeight +
						1; /* Add 1px so there is a small gap */
	...
	public readonly displayRange = derived<LineRange | undefined>(
		this,
		(reader) => {
			...
			if (
				state.target.uri.toString() !==
				this._editorObs.model.read(reader)?.uri.toString()
			) {
				return undefined;
			}
	...
	private _getLongDistanceHintState(
		...
		if (
			model.inlineEdit.originalText.uri.toString() !==
			this._editorObs.model.read(reader)?.uri.toString()
		) {
			return {
				isVisible: true,
		...
		if (
			this._currentInlineEditCache?.inlineSuggestionIdentity !==
			model.inlineEdit.inlineCompletion.identity
		) {
	...
	private readonly _uiState = derived<
		| {
				state: ReturnType<
					typeof InlineEditsView.prototype._determineRenderState
				>;
	...
	>(this, (reader) => {
		...
		if (state.kind === InlineCompletionViewKind.SideBySide) {
			...
			newText = new StringText(
				indentationAdjustmentEdit.applyToString(newText.getValue()),
			);
		...
		if (previousNewText !== newText.getValue()) {
			...
			const updateOldValueEdit = StringEdit.replace(
				new OffsetRange(0, previousNewText.length),
				newText.getValue(),
			);
			const updateOldValueEditSmall =
				updateOldValueEdit.removeCommonSuffixPrefix(previousNewText);

			const textEdit = getPositionOffsetTransformerFromTextModel(
				this._previewTextModel,
			).getTextEdit(updateOldValueEditSmall);
			this._previewTextModel.edit(textEdit);
		}

		if (this._showCollapsed.read(reader)) {
			state = {
				kind: InlineCompletionViewKind.Collapsed as const,
		...
		const nextCursorPosition =
			inlineEdit.action?.kind === "jumpTo" ? inlineEdit.action.position : null;
	...
	public readonly inlineEditsIsHovered = derived(this, (reader) => {
		return (
			...
			this._wordReplacementViews
				.read(reader)
				.some((v) => v.isHovered.read(reader)) ||
			...
			this._longDistanceHint
				.map((v, r) => v?.isHovered.read(r) ?? false)
				.read(reader)
		);
	});
	...
	private _determineView(
		...
		const canUseCache =
			...
			this._previousView?.uri.toString() ===
				this._editorObs.model.get()!.uri.toString();
		const reconsiderViewEditorWidthChange =
			this._previousView?.editorWidth !==
				this._editorObs.layoutInfoWidth.read(reader) &&
		...
		if (model.editorType !== InlineCompletionEditorType.DiffEditor) {
			...
			const allInnerChangesNotTooLong = inner.every(
				(m) =>
					...
					TextLength.ofRange(m.modifiedRange).columnCount <
						InlineEditsWordReplacementView.MAX_LENGTH,
			);
			if (
				...
				const modifiedText = inner.map((m) =>
					newText.getValueOfRange(m.modifiedRange),
				);
				const originalText = inner.map((m) =>
					model.inlineEdit.originalText.getValueOfRange(m.originalRange),
				);
				if (
					!modifiedText.some((v) => v.includes("\t")) &&
					!originalText.some((v) => v.includes("\t"))
				) {
					...
					if (
						...
						!growEditsUntilWhitespace(
							...
							(e) =>
								e.range.isEmpty() &&
								TextLength.ofRange(e.range).columnCount <
									InlineEditsWordReplacementView.MAX_LENGTH,
						)
					) {
						return InlineCompletionViewKind.WordReplacements;
		...
		if (numOriginalLines > 0 && numModifiedLines > 0) {
			if (
				...
				numModifiedLines === 1 &&
				model.editorType !==
					InlineCompletionEditorType.DiffEditor /* prefer side by side in diff editor */
			) {
				return InlineCompletionViewKind.LineReplacement;
			}
	...
	private _determineRenderState(
		...
		switch (view) {
			case InlineCompletionViewKind.InsertionInline:
				return {
					kind: InlineCompletionViewKind.InsertionInline as const,
					viewData,
				};
			...
			case InlineCompletionViewKind.Custom:
				return {
					kind: InlineCompletionViewKind.Custom as const,
		...
		const replacements = stringChanges.map(
			(m) => new TextReplacement(m.originalRange, m.modified),
		);
		...
		if (view === InlineCompletionViewKind.LineReplacement) {
			return {
				...
				modifiedLines: inlineEdit.modifiedLineRange.mapToLineArray((line) =>
					newText.getLineAt(line),
				),
		...
		for (const cursorPosition of inlineEdit.multiCursorPositions) {
			if (
				...
				cursorPosition.lineNumber ===
					inlineEdit.originalLineRange.startLineNumber + 1
			) {
				return true;
			}
			...
			if (
				...
				cursorPosition.lineNumber <
					inlineEdit.modifiedLineRange.endLineNumberExclusive +
...
const emptyViewData = new InlineCompletionViewData(
	-1,
	-1,
	-1,
	-1,
...
function getViewData(
	...
	const startsWithEOL =
		stringChanges.length === 0
			? false
			: stringChanges[0].modified.startsWith(textModel.getEOL());
	const viewData = new InlineCompletionViewData(
		inlineEdit.edit.replacements.length === 0
			? 0
			: inlineEdit.edit.replacements[0].range.getStartPosition().column -
					cursorPosition.column,
		inlineEdit.lineEdit.lineRange.startLineNumber -
			cursorPosition.lineNumber +
			(startsWithEOL &&
			inlineEdit.lineEdit.lineRange.startLineNumber >= cursorPosition.lineNumber
				? 1
				: 0),
		...
		stringChanges.every(
			(r) =>
				r.original === stringChanges[0].original &&
				r.modified === stringChanges[0].modified,
		),
	);
	return viewData;
}
...
function isSingleLineInsertion(diff: DetailedLineRangeMapping[]) {
	...
	function isWordInsertion(r: RangeMapping) {
		...
		const isInsertionWithinLine =
			r.modifiedRange.startLineNumber === r.modifiedRange.endLineNumber;
...
function isSingleLineInsertionAfterPosition(
	...
	return diff.every((m) =>
		m.innerChanges!.every((r) => isStableWordInsertion(r)),
	);
...
function isSingleMultiLineInsertion(diff: DetailedLineRangeMapping[]) {
	...
	if (
		change.modifiedRange.startLineNumber === change.modifiedRange.endLineNumber
	) {
		return false;
	}
...
function growEditsToEntireWord(
	...
	return _growEdits(replacements, originalText, (char) =>
		/^[a-zA-Z]$/.test(char),
	);
}
...
function _growEdits(
	...
	for (const edit of replacements) {
		...
		if (
			...
			newEdit = TextReplacement.joinReplacements(
				[result.pop()!, newEdit],
				originalText,
			);
		}
