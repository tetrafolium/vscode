...
import {
	ILongDistancePreviewProps,
	LongDistancePreviewEditor,
} from "./longDistancePreviewEditor.js";
import {
	...
	implements IInlineEditsView
{
	...
	private _viewWithElement:
		| ObserverNodeWithElement<HTMLDivElement>
		| undefined = undefined;
	...
	constructor(
		...
		private readonly _viewState: IObservable<
			ILongDistanceViewState | undefined
		>,
		...
		this._styles = derived((reader) => {
			...
			const widgetBorderColor = observeColor(
				editorWidgetBorder,
				this._themeService,
			).read(reader);
			const isHighContrast = observableFromEvent(
			...
			if (isHighContrast) {
				...
				borderColor = getEditorBlendedColor(border, this._themeService).read(
					reader,
				);
			}
			...
			return {
				...
				background: getEditorBackgroundColor(
					this._viewState
						.map((s) => s?.editorType ?? InlineCompletionEditorType.TextEditor)
						.read(reader),
				),
			};
		});
		...
		this._register(
			autorun((reader) => {
				...
				this._previewEditor.layout(
					layoutInfo.codeEditorSize.toDimension(),
	...
	private readonly _hintTextPosition = derived(this, (reader) => {
		const viewState = this._viewState.read(reader);
		return viewState
			? new Position(viewState.hint.lineNumber, Number.MAX_SAFE_INTEGER)
			: null;
	});
	...
	private readonly _lineSizesAroundHintPosition = derived(this, (reader) => {
		...
		const top = this._editorObs
			.observeTopForLineNumber(range.startLineNumber)
			.read(reader);

		return splitIntoContinuousLineRanges(
			range,
			sizes,
			top,
			this._editorObs,
	...
	private readonly _isVisibleDelayed = debouncedObservable2(
		...
		(lastValue, newValue) =>
			lastValue === true && newValue === false ? 200 : 0,
	);
	...
	private readonly _previewEditorLayoutInfo = derived(this, (reader) => {
		...
		const previewEditorContentLayout =
			this._previewEditor.horizontalContentRangeInPreviewEditorToShow.read(
				reader,
			);
		...
		const editorTrueContentWidth =
			editorLayout.contentWidth - editorLayout.verticalScrollbarWidth;
		const editorTrueContentRight =
			editorLayout.contentLeft + editorTrueContentWidth;
		...
		const previewEditorHeight =
			previewContentHeight + extraGutterMarginToAvoidScrollBar;
		...
		const endOfLinePadding = (lineNumber: number) =>
			lineNumber === viewState.hint.lineNumber ? 40 : 20;
			...
			if (showRects) {
				const rects2 = stackSizesDown(
					new Point(
						...
						continousLineRange.top - editorScrollTop,
					),
					placementContext.availableSpaceSizes as Size2D[],
					"right",
				);
				debugView(
					debugLogRects({ ...rects2 }, this._editor.getDomNode()!),
					reader,
				);
			}
		...
		if (!possibleWidgetOutline) {
			...
			const maxAvailableWidth = Math.min(
				editorLayout.width - editorLayout.contentLeft,
				MAX_WIDGET_WIDTH.OVERLAY,
			);
		...
		if (showAvailableSpace) {
			debugView(
				debugLogRects({ rectAvailableSpace }, this._editor.getDomNode()!),
				reader,
			);
		}
		...
		const maxWidgetWidth = Math.min(
			position === "overlay"
				? MAX_WIDGET_WIDTH.OVERLAY
				: MAX_WIDGET_WIDTH.EMPTY_SPACE,
			previewEditorContentLayout.maxEditorWidth +
				previewEditorMargin +
				widgetPadding,
		);
		...
		if (showRects2) {
			debugView(
				debugLogRects(
					{ spaceBeforeRect, widgetRect, spaceAfterRect },
		...
		if (showEditorRect) {
			debugView(
				debugLogRects({ previewEditorRect }, this._editor.getDomNode()!),
				reader,
			);
		}

		const previewEditorContentWidth =
			previewEditorRect.width - previewEditorContentLayout.nonContentWidth;
		...
		return {
			...
			desiredPreviewEditorScrollLeft:
				desiredPreviewEditorScrollLeft.newScrollPosition,
		};
	});
	...
	private readonly _view = n.div(
		{
			...
			style: {
				...
				display: derived(this, (reader) =>
					!!this._previewEditorLayoutInfo.read(reader) ? "block" : "none",
				),
			},
		},
			...
			n.div(
				{
					...
					style: {
						...
						opacity: derived((reader) =>
							this._viewState.read(reader)?.hint.isVisible ? "1" : "0",
						),
						transition: "opacity 200ms ease-in-out",
						...rectToProps(
							(reader) =>
								this._previewEditorLayoutInfo.read(reader)?.widgetRect,
						),
					},
					onmousedown: (e) => {
					...
					n.div(
						{
							...
							style: {
								overflow: "hidden",
								padding: this._previewEditorLayoutInfo.map(
									(i) => i?.previewEditorMargin,
								),
					...
					n.div(
						{
							...
							style: {
								...
								pointerEvents: "none",
								margin: "0 4px",
								height: this._previewEditorLayoutInfo.map(
									(i) => i?.lowerBarHeight,
								),
								display: "flex",
						...
						[
							derived(this, (reader) => {
								...
								const isCrossFileEdit =
									targetUri &&
									(!currentUri ||
										targetUri.toString() !== currentUri.toString());
								...
								if (isCrossFileEdit) {
									...
									const originalTargetLineNumber =
										this._originalTargetLineNumber.read(reader);
									const outlineItems =
										source
											?.getAt(originalTargetLineNumber, reader)
											.slice(0, 1) ?? [];
	...
	private readonly _originalOutlineSource = derivedDisposable(
		this,
		(reader) => {
			const m = this._editorObs.model.read(reader);
			const factory =
				HideUnchangedRegionsFeature._breadcrumbsSourceFactory.read(reader);
			return !m || !factory
				? undefined
...
function lengthsToOffsetRanges(
	lengths: number[],
	initialOffset = 0,
): OffsetRange[] {
...
export function drawEditorWidths(e: ICodeEditor, reader: IReader) {
	...
	const minimap = OffsetRange.ofStartAndLength(
		trueContent.endExclusive,
		layoutInfo.minimap.minimapWidth,
	);
	const verticalScrollbar = OffsetRange.ofStartAndLength(
		minimap.endExclusive,
...
export function scrollToReveal(
	...
	const visibleRange = new OffsetRange(
		currentScrollPosition,
		currentScrollPosition + windowWidth,
	);
	...
	if (contentRangeToReveal.endExclusive > visibleRange.endExclusive) {
		return {
			newScrollPosition: contentRangeToReveal.endExclusive - windowWidth,
		};
	}
