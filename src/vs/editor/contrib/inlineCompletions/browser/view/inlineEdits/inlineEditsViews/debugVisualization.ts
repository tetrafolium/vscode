...
export function debugLogHorizontalOffsetRange(
	rect: Rect,
	elem: HTMLElement,
	name: string,
): Rect {
	setVisualization(
		rect,
		new HtmlHorizontalOffsetRangeVisualizer(rect, elem, name, 0, "above"),
	);
	return rect;
}

export function debugLogHorizontalOffsetRanges(
	rects: Record<string, Rect> | Rect[],
	elem: HTMLElement,
): object {
	if (Array.isArray(rects)) {
...
class ManyHorizontalOffsetRangeVisualizer implements IVisualizationEffect {
	...
	visualize(): IDisposable {
		...
		keys.forEach((key, index) => {
			...
			const v = new HtmlHorizontalOffsetRangeVisualizer(
				this._rects[key],
				this._elem,
				key,
				index * 12,
...
class HtmlHorizontalOffsetRangeVisualizer implements IVisualizationEffect {
	...
	visualize(): IDisposable {
		...
		const updatePosition = () => {
			...
			const centerY =
				this._rect.top + this._rect.height / 2 + this._verticalOffset;
