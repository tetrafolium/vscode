...
export class OriginalEditorInlineDiffView
	extends Disposable
	implements IInlineEditsView
{
	...
	constructor(
		...
		this.isHovered = observableCodeEditor(this._originalEditor).isTargetHovered(
			(p) =>
				...
				p.target.detail.injectedText?.options.attachedData instanceof
					InlineEditAttachedData &&
				p.target.detail.injectedText.options.attachedData.owner === this,
			this._store,
		);
		this._tokenizationFinished = modelTokenizationFinished(
		...
		this._decorations = derived(this, (reader) => {
			...
			const hasOneInnerChange =
				diff.diff.length === 1 && diff.diff[0].innerChanges?.length === 1;
			...
			const diffLineDeleteDecorationBackground =
				...
				if (m.modified.isEmpty || m.original.isEmpty) {
					...
					for (const i of m.innerChanges || []) {
						...
						if (useInlineDiff) {
							...
							const lineTokens =
								this._modifiedTextModel.tokenization.getLineTokens(
