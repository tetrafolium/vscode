	...
	implements IInlineEditsView
{
	...
	constructor(
		...
		this._register(
			this._editorObs.createOverlayWidget({
				...
				minContentWidthInPx: derivedObservableWithCache<number>(
					this,
					(reader, prev) => {
						const s = state.read(reader);
						if (!s) {
	...
	private fitsInsideViewport(
		...
		const editorContentLeft =
			this._editorObs.layoutInfoContentLeft.read(reader);
		const editorVerticalScrollbar =
			this._editor.getLayoutInfo().verticalScrollbarWidth;
		const minimapWidth =
		...
		const maxModifiedContent = getContentRenderWidth(
			displayLabel,
			this._editor,
	...
	private getState(displayLocation: InlineSuggestHint): {
		...
		const rect = derived(this, (reader) => {
			...
			if (
				...
				lineWidthBelow +
					MIN_END_OF_LINE_PADDING -
					HORIZONTAL_OFFSET_WHEN_ABOVE_BELOW -
					PADDING_HORIZONTALLY <
					startContentLeftOffset
			) {
			...
			return textRect
				.withMargin(PADDING_VERTICALLY, PADDING_HORIZONTALLY)
				.translateX(deltaX)
				.translateY(deltaY);
		});
