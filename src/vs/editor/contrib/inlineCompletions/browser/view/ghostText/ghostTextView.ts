...
export class GhostTextWidgetWarning {
	public static from(
		warning: InlineCompletionWarning | undefined,
	): GhostTextWidgetWarning | undefined {
		if (!warning) {
			return undefined;
		}
...
export class GhostTextView extends Disposable {
	...
	constructor(
		...
		this._minReservedLineCount =
			options.minReservedLineCount ?? constObservable(0);
		this._useSyntaxHighlighting =
			options.useSyntaxHighlighting ?? constObservable(true);
		...
		this._additionalLinesWidget = this._register(
			new AdditionalLinesWidget(
				this._editor,
				derivedOpts(
					{ owner: this, equalsFn: equalsIfDefinedC(thisEqualsC()) },
					(reader) => {
						/** @description lines */
						const uiState = this._state.read(reader);
						return uiState
							? new AdditionalLinesData(
									uiState.lineNumber,
	...
	private readonly _nonWhitespaceCount = derived(this, (reader) => {
		...
		const allText = ghostText.parts
			.map((p) => p.lines.map((l) => l.line).join(""))
			.join("");
	...
	private readonly _extraClassNames = derived(this, (reader) => {
		...
		if (
			this._highlightShortText &&
			nonWhitespaceCount &&
			nonWhitespaceCount < 3
		) {
	...
	private readonly _state = derived(this, (reader) => {
		...
		const cursorColumn = this._editor
			.getSelection()
			?.getStartPosition().column!;
		const disjointInlineTexts = inlineTextsWithTokens.filter(
			(inline) => inline.text !== "",
		);
		const hasInsertionOnCurrentLine = disjointInlineTexts.length !== 0;
		const telemetryViewData = new InlineCompletionViewData(
			(hasInsertionOnCurrentLine ? disjointInlineTexts[0].column : 1) -
				cursorColumn,
			hasInsertionOnCurrentLine
				? 0
				: additionalLines.findIndex((line) => line.content !== "") + 1,
			...
			sum(disjointInlineTexts.map((inline) => inline.text.length)) +
				sum(
					tokenizedAdditionalLines.map((line) => line.content.getTextLength()),
				),
			disjointInlineTexts.length + (additionalLines.length > 0 ? 1 : 0),
			disjointInlineTexts.length > 1 && tokenizedAdditionalLines.length === 0
				? disjointInlineTexts.every(
						(inline) => inline.text === disjointInlineTexts[0].text,
					)
				: undefined,
		);
...
export class AdditionalLinesWidget extends Disposable {
		...
		if (
			...
			this._register(
				this._editor.onBeforeExecuteEdit(
					(e) =>
						(this.hasBeenAccepted = e.source === "inlineSuggestion.accept"),
				),
			);
		}
	...
	private updateLines(
		lineNumber: number,
		additionalLines: readonly LineData[],
