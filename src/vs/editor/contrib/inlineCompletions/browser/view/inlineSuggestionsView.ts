...
import {
	...
	InlineEditTabAction,
} from "./inlineEdits/inlineEditsViewInterface.js";
...
export class InlineSuggestionsView extends Disposable {
	...
	private readonly _inlineEdit = derived(
		this,
		(reader) =>
			this._model.read(reader)?.inlineEditState.read(reader)?.inlineSuggestion,
	);
	private readonly _everHadInlineEdit = derivedObservableWithCache<boolean>(
		...
		(reader, last) =>
			...
			!!this._model.read(reader)?.inlineCompletionState.read(reader)
				?.inlineSuggestion?.showInlineEditMenu,
	);

	// To break a cyclic dependency
	private readonly _indicatorIsHoverVisible = observableValue<
...
.monaco-editor .ghost-text {
			...
			this._instantiationService.createInstance(
				...
				derived((reader) => {
					...
					return new InlineEditsGutterIndicatorData(
						InlineSuggestionGutterMenuData.fromInlineSuggestion(
							s.inlineSuggestion,
						),
						s.displayRange,
						SimpleInlineSuggestModel.fromInlineCompletionModel(s.model),
						s.inlineSuggestion.action?.kind === "edit"
							? s.inlineSuggestion.action.alternativeAction
							: undefined,
					);
				}),
				this._gutterIndicatorState.map(
					(s, reader) =>
						s?.tabAction?.read(reader) ?? InlineEditTabAction.Inactive,
				),
				this._gutterIndicatorState.map(
					(s, reader) => s?.gutterIndicatorOffset?.read(reader) ?? 0,
				),
				this._inlineEditWidget.map(
					(w, reader) => w?.view.inlineEditsIsHovered.read(reader) ?? false,
				),
				this._focusIsInMenu,
			),
		);
		this._indicatorIsHoverVisible.set(
			this._indicator.isHoverVisible,
			undefined,
		);
	...
	private _createGhostText(
		ghostText: IObservable<GhostTextOrReplacement>,
	): GhostTextView {
		return this._instantiationService.createInstance(
			GhostTextView,
			this._editor,
			derived((reader) => {
				...
				const inlineCompletion =
					model?.inlineCompletionState.read(reader)?.inlineSuggestion;
	...
	private readonly _gutterIndicatorState = derived((reader) => {
		...
		if (
			...
			state.inlineSuggestion?.showInlineEditMenu
		) {
			return {
				displayRange: LineRange.ofLength(state.primaryGhostText.lineNumber, 1),
				tabAction: derived<InlineEditTabAction>(this, (reader) =>
					this._editorObs.isFocused.read(reader)
						? InlineEditTabAction.Accept
						: InlineEditTabAction.Inactive,
				),
				gutterIndicatorOffset: constObservable(
					getGhostTextTopOffset(state.inlineSuggestion, this._editor),
				),
...
function getGhostTextTopOffset(
