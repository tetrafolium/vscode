...
class StickyModelFromCandidateOutlineProvider extends StickyModelCandidateProvider<OutlineModel> {
	...
	private _stickyModelFromOutlineModel(
		...
		if (
			Iterable.first(outlineModel.children.values()) instanceof OutlineGroup
		) {
			...
			if (provider) {
				...
				let optimalOutlineGroup: OutlineGroup | OutlineElement | undefined =
					undefined;
...
class StickyModelFromCandidateSyntaxFoldingProvider extends StickyModelFromCandidateFoldingProvider {
	private readonly provider: MutableDisposable<SyntaxRangeProvider> =
		this._register(new MutableDisposable<SyntaxRangeProvider>());
