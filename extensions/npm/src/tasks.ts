	...
	public async resolveTask(_task: Task): Promise<Task | undefined> {
		...
		if (npmTask) {
			...
			if (kind.script === INSTALL_SCRIPT) {
				task = await createInstallationTask(
					this.context,
					_task.scope,
					packageJsonUri,
				);
			} else {
				task = await createScriptRunnerTask(
					this.context,
					kind.script,
					_task.scope,
					packageJsonUri,
...
export async function detectPackageManager(
	...
	if (
		...
		window
			.showInformationMessage(multiplePMWarning, learnMore, neverShowAgain)
			.then((result) => {
				switch (result) {
					case neverShowAgain:
...
function escapeCommandLine(cmd: string[]): (string | ShellQuotedString)[] {
	return cmd.map((arg) => {
		if (/\s/.test(arg)) {
			return {
				value: arg,
				quoting: arg.includes("--") ? ShellQuoting.Weak : ShellQuoting.Strong,
			};
		} else {
			return arg;
		}
...
export async function createScriptRunnerTask(
	...
	const args = await getRunScriptCommand(
		script,
		folder.uri,
		context,
		showWarning,
	...
	const task = new Task(
		...
		taskName,
		"npm",
		new ShellExecution(scriptRunner, escapeCommandLine(args), { cwd: cwd }),
	);
...
async function getInstallDependenciesCommand(
	folder: Uri,
	context?: ExtensionContext,
	showWarning = true,
): Promise<string[]> {
...
export async function createInstallationTask(
	...
	const task = new Task(
		...
		taskName,
		"npm",
		new ShellExecution(packageManager, escapeCommandLine(args), { cwd: cwd }),
	);
	task.detail = scriptValue;
...
export function getPackageJsonUriFromTask(task: Task): Uri | null {
	if (isWorkspaceFolder(task.scope)) {
		if (task.definition.path) {
			return Uri.file(
				path.join(task.scope.uri.fsPath, task.definition.path, "package.json"),
			);
		} else {
...
export async function startDebugging(
	...
	const runScriptCommand = await getRunScriptCommand(
		scriptName,
		folder.uri,
		context,
		true,
