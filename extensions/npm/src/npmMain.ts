...
export async function activate(
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration((e) => {
			if (
				e.affectsConfiguration("npm.exclude") ||
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("npm.runSelectedScript", runSelectedScript),
	);

	if (await hasPackageJson()) {
		vscode.commands.executeCommand(
			"setContext",
			"npm:showScriptExplorer",
			true,
		);
	}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("npm.scriptRunner", (args) => {
	...
	context.subscriptions.push(
		vscode.window.registerTerminalQuickFixProvider("ms-vscode.npm-command", {
...
function registerExplorer(
	context: vscode.ExtensionContext,
): NpmScriptsTreeDataProvider | undefined {
	if (vscode.workspace.workspaceFolders) {
		const treeDataProvider = new NpmScriptsTreeDataProvider(
			context,
			taskProvider!,
		);
		const view = vscode.window.createTreeView("npm", {
