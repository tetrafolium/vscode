...
import {
	...
	normalizeVersion,
	parseVersion,
} from "./extensionEngineValidation";
...
const allowedBadgeProvidersRegex: RegExp[] = (
	product.extensionAllowedBadgeProvidersRegex || []
).map((r: string) => new RegExp(r));
const extensionEnabledApiProposals: Record<string, string[]> =
	product.extensionEnabledApiProposals ?? {};
const reservedImplicitActivationEventPrefixes = ["onNotebookSerializer:"];
const redundantImplicitActivationEventPrefixes = [
	"onLanguage:",
	"onView:",
...
function isTrustedSVGSource(uri: Uri): boolean {
	return (
		allowedBadgeProviders.includes(uri.authority.toLowerCase()) ||
...
export class ExtensionLinter {
		...
		for (const document of Array.from(this.packageJsonQ)) {
			...
			if (tree && info.isExtension) {
				...
				if (badges && badges.type === "array" && badges.children) {
					badges.children
						.map((child) => findNodeAtLocation(child, ["url"]))
				...
				const enabledApiProposals = findNodeAtLocation(tree, [
					"enabledApiProposals",
				]);
				if (
					...
					if (
						Array.isArray(effectiveProposalNames) &&
						enabledApiProposals.children
					) {
						for (const child of enabledApiProposals.children) {
							const proposalName =
								child.type === "string" ? getNodeValue(child) : undefined;
							if (
								typeof proposalName === "string" &&
								!effectiveProposalNames.includes(proposalName.split("@")[0])
							) {
				...
				if (
					activationEventsNode?.type === "array" &&
					activationEventsNode.children
				) {
					for (const activationEventNode of activationEventsNode.children) {
						const activationEvent = getNodeValue(activationEventNode);
						const isImplicitActivationSupported =
							info.engineVersion &&
							(info.engineVersion.majorBase > 1 ||
								(info.engineVersion.majorBase === 1 &&
									info.engineVersion.minorBase >= 75));
						// Redundant Implicit Activation
						if (
							isImplicitActivationSupported &&
							info.implicitActivationEvents?.has(activationEvent) &&
							redundantImplicitActivationEventPrefixes.some((prefix) =>
								activationEvent.startsWith(prefix),
							)
						) {
							const start = document.positionAt(activationEventNode.offset);
							const end = document.positionAt(
								activationEventNode.offset + activationEventNode.length,
							);
							diagnostics.push(
								new Diagnostic(
									new Range(start, end),
						...
						for (const implicitActivationEventPrefix of reservedImplicitActivationEventPrefixes) {
							if (
								isImplicitActivationSupported &&
								activationEvent.startsWith(implicitActivationEventPrefix)
							) {
								const start = document.positionAt(activationEventNode.offset);
								const end = document.positionAt(
									activationEventNode.offset + activationEventNode.length,
	...
	private async lintWhenClauses(
					...
					case "property":
						if (node.children && node.children.length === 2) {
							...
							switch (value.type) {
								case "string":
									if (
										key.value === clauseName &&
										typeof value.value ===
											"string" /* careful: `.value` MUST be a string 1) because a when/enablement clause is string; so also, type cast to string below is safe */
									) {
		...
		const parseResults = await commands.executeCommand<
			{ errorMessage: string; offset: number; length: number }[][]
		>(
	...
	private async lintReadme() {
		for (const document of this.readmeQ) {
			...
			const tokensAndPositions: TokenAndPosition[] =
				function toTokensAndPositions(
					...
					const tokensAndPositions = tokens.map<TokenAndPosition>((token) => {
						...
						const image =
							token.type === "image" &&
							this.locateToken(text, begin, end, token, token.attrGet("src"));
						const other =
							image || this.locateToken(text, begin, end, token, token.content);
						return (
							other || {
								token,
								begin,
					...
					return tokensAndPositions.concat(
						...tokensAndPositions
							.filter((tnp) => tnp.token.children && tnp.token.children.length)
							.map((tnp) =>
			...
			tokensAndPositions
				.filter((tnp) => tnp.token.type === "image" && tnp.token.attrGet("src"))
				.map((inp) => {
	...
	private readPackageJsonInfo(folder: Uri, tree: JsonNode | undefined) {
		...
		const parsedEngineVersion =
			engine?.type === "string"
				? normalizeVersion(parseVersion(engine.value))
				: null;
		...
		if (
			oldInfo &&
			(oldInfo.isExtension !== info.isExtension ||
				oldInfo.hasHttpsRepository !== info.hasHttpsRepository)
		) {
	...
	private packageJsonChanged(folder: Uri) {
		...
		workspace.textDocuments
			.filter(
				(document) =>
					this.getUriFolder(document.uri).toString().toLowerCase() === str,
			)
	...
	private addDiagnostics(
		...
		const uri = parseUri(
			src,
			info.repository ? info.repository.toString() : document.uri.toString(),
		);
		if (!uri) {
			return;
		}
		...
		if (hasScheme && scheme === "data") {
			...
			diagnostics.push(
				new Diagnostic(range, dataUrlsNotValid, DiagnosticSeverity.Warning),
			);
		}
		...
		if (!hasScheme && !info.hasHttpsRepository && context !== Context.ICON) {
			...
			const message = (() => {
				switch (context) {
					case Context.BADGE:
						return relativeBadgeUrlRequiresHttpsRepository;
					default:
...
function parseImplicitActivationEvents(tree: JsonNode): Set<string> {
	...
	const authenticationProviders = findNodeAtLocation(tree, [
		"contributes",
		"authentication",
	]);
	...
	// languages
	const languageContributions = findNodeAtLocation(tree, [
		"contributes",
		"languages",
	]);
	...
	// customEditors
	const customEditors = findNodeAtLocation(tree, [
		"contributes",
		"customEditors",
	]);
	...
	// walkthroughs
	const walkthroughs = findNodeAtLocation(tree, [
		"contributes",
		"walkthroughs",
	]);
	...
	// notebookRenderers
	const notebookRenderers = findNodeAtLocation(tree, [
		"contributes",
		"notebookRenderer",
	]);
	...
	// terminalProfiles
	const terminalProfiles = findNodeAtLocation(tree, [
		"contributes",
		"terminal",
		"profiles",
	]);
	...
	// terminalQuickFixes
	const terminalQuickFixes = findNodeAtLocation(tree, [
		"contributes",
		"terminal",
		"quickFixes",
	]);
