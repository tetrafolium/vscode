...
const packageNlsJsonSelector: vscode.DocumentSelector = {
	language: "json",
	pattern: "**/package.nls.json",
};

export class PackageDocumentL10nSupport
	implements
		...
		vscode.Disposable
{
	...
	constructor() {
		this._disposables.push(
			vscode.languages.registerDefinitionProvider(packageJsonSelector, this),
		);
		this._disposables.push(
			vscode.languages.registerDefinitionProvider(packageNlsJsonSelector, this),
		);

		this._disposables.push(
			vscode.languages.registerReferenceProvider(packageNlsJsonSelector, this),
		);
		this._disposables.push(
			vscode.languages.registerReferenceProvider(packageJsonSelector, this),
		);
	}
	...
	private async provideNlsValueDefinition(
		packageJsonDoc: vscode.TextDocument,
		position: vscode.Position,
	): Promise<vscode.DefinitionLink[] | undefined> {
		...
		const nlsUri = vscode.Uri.joinPath(
			packageJsonDoc.uri,
			"..",
			"package.nls.json",
		);
	...
	private async provideNlsKeyDefinition(
		nlsDoc: vscode.TextDocument,
		position: vscode.Position,
	): Promise<vscode.DefinitionLink[] | undefined> {
	...
	private async resolveNlsDefinition(
		origin: { key: string; range: vscode.Range },
		nlsUri: vscode.Uri,
	): Promise<vscode.DefinitionLink[] | undefined> {
	...
	private getNlsReferenceAtPosition(
		packageJsonDoc: vscode.TextDocument,
		position: vscode.Position,
	): { key: string; range: vscode.Range } | undefined {
		const location = getLocation(
			packageJsonDoc.getText(),
			packageJsonDoc.offsetAt(position),
		);
		...
		const nodeEnd = packageJsonDoc.positionAt(
	...
	private async provideNlsKeyReferences(
		nlsDoc: vscode.TextDocument,
		position: vscode.Position,
		context: vscode.ReferenceContext,
	): Promise<vscode.Location[] | undefined> {
		...
		return this.findAllNlsReferences(
			nlsKey.key,
			packageJsonUri,
			nlsDoc.uri,
			context,
	...
	private async provideNlsValueReferences(
		packageJsonDoc: vscode.TextDocument,
		position: vscode.Position,
		context: vscode.ReferenceContext,
	): Promise<vscode.Location[] | undefined> {
	...
	private async findAllNlsReferences(
		nlsKey: string,
		packageJsonUri: vscode.Uri,
		nlsUri: vscode.Uri,
		context: vscode.ReferenceContext,
	): Promise<vscode.Location[]> {
		const locations = await this.findNlsReferencesInPackageJson(
			nlsKey,
			packageJsonUri,
		);
	...
	private async findNlsKeyDeclaration(
		nlsKey: string,
		nlsUri: vscode.Uri,
	): Promise<vscode.Location | undefined> {
		try {
	...
	private async findNlsReferencesInPackageJson(
		nlsKey: string,
		packageJsonUri: vscode.Uri,
	): Promise<vscode.Location[]> {
		...
		visit(text, {
			onLiteralValue(value, offset, length) {
				if (value === needle) {
					...
					locations.push(
						new vscode.Location(packageJsonUri, new vscode.Range(start, end)),
					);
				}
			},
		});
	...
	private getNlsKeyDefinitionAtPosition(
		nlsDoc: vscode.TextDocument,
		position: vscode.Position,
	): { key: string; range: vscode.Range } | undefined {
