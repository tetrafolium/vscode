...
export class FileWatcherManager {
	...
	watchFile(
		...
		if (
			this.enabledExperimentalTypeAcquisition &&
			...
			this.watchPort.postMessage({
				type: "watchFile",
				uri: mapUri(uri, "vscode-global-typings"),
				id: watchIds[1],
			});
		}
		return {
	...
	private updateWatch(
		...
		for (const watch of Array.from(this.watchDirectories.keys()).filter((dir) =>
			path.startsWith(dir),
		)) {
