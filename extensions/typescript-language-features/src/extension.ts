...
export function activate(context: vscode.ExtensionContext): Api {
	...
	context.subscriptions.push(
		conditionalRegistration(
			[
				requireGlobalUnifiedConfig("experimental.useTsgo", {
			...
			() => {
				...
				const commandManager = disposables.add(new CommandManager());
				const activeJsTsEditorTracker = disposables.add(
				...
				const lazyClientHost = createLazyClientHost(
					...
					onCaseInsensitiveFileSystem(),
					{
						...
						serviceConfigurationProvider:
							new ElectronServiceConfigurationProvider(),
						experimentTelemetryReporter,
						logger: new Logger(),
					},
					(item) => {
