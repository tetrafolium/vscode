		...
		await withRandomFileEditor(
			...
			async (_editor: vscode.TextEditor, doc: vscode.TextDocument) => {
				...
				assert.strictEqual(
					...
					0,
					"Expected interface IFoo to have a CodeLens",
				);
				assert.strictEqual(
					lenses?.[1].range.start.line,
					2,
					"Expected abstract class AbstractBase to have a CodeLens",
				);
			},
		);
	});
		...
		await withRandomFileEditor(
			...
			async (_editor: vscode.TextEditor, doc: vscode.TextDocument) => {
				...
				assert.strictEqual(
					lenses?.[0].range.start.line,
					0,
					"Expected abstract class to have a CodeLens",
				);
				assert.strictEqual(
					lenses?.[1].range.start.line,
					1,
					"Expected abstract method to have a CodeLens",
				);
				assert.strictEqual(
					lenses?.[2].range.start.line,
					2,
					"Expected abstract property to have a CodeLens",
				);
				assert.strictEqual(
					lenses?.[3].range.start.line,
					3,
					"Expected abstract getter to have a CodeLens",
				);
			},
		);
	});
		...
		await withRandomFileEditor(
			...
			async (_editor: vscode.TextEditor, doc: vscode.TextDocument) => {
				...
				assert.strictEqual(
					lenses?.[0].range.start.line,
					0,
					"Expected class A to have a CodeLens",
				);
				assert.strictEqual(
					lenses?.[1].range.start.line,
					1,
					"Expected method A.foo to have a CodeLens",
				);
				assert.strictEqual(
					lenses?.[2].range.start.line,
					4,
					"Expected method B.foo to have a CodeLens",
				);
			},
		);
	});
		...
		await withRandomFileEditor(
			...
			async (_editor: vscode.TextEditor, doc: vscode.TextDocument) => {
				...
				assert.strictEqual(
					lenses?.[2].range.start.line,
					3,
					"Expected method A.baz to have a CodeLens",
				);
				assert.strictEqual(
					lenses?.[3].range.start.line,
					6,
				...
				assert.strictEqual(
					lenses?.[4].range.start.line,
					7,
					"Expected method B.baz to have a CodeLens",
				);
			},
		);
	});
