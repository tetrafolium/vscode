...
suite("CachedResponse", () => {
	...
	test("should not cache cancelled responses", async () => {
		...
		const cancelledResponder =
			createEventualResponder<ServerResponse.Cancelled>();
...
function assertResult(
	result: ServerResponse.Response<Proto.Response>,
	command: string,
) {
...
function createEventualResponder<T>(): {
	promise: Promise<T>;
	resolve: (x: T) => void;
} {
