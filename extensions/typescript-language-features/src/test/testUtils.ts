...
export function withRandomFileEditor(
	...
	return createRandomFile(contents.replace(CURSOR, ""), fileExtension).then(
		(file) => {
...
export async function createTestEditor(uri: vscode.Uri, ...lines: string[]) {
	...
	await editor.insertSnippet(
		new vscode.SnippetString(joinLines(...lines)),
		new vscode.Range(0, 0, 1000, 0),
	);
	return editor;
}

export function assertEditorContents(
		...
		assert.deepStrictEqual(
			{
				...
				character: editor.selection.active.line,
			},
...
export function onChangedDocument(
	documentUri: vscode.Uri,
