	...
	private getReferencesLinks(document: vscode.TextDocument, root: jsonc.Node) {
		return mapChildren(
			...
			(child) => {
				...
				return (
					pathNode &&
					this.tryCreateTsConfigLink(
						document,
						pathNode,
						TsConfigLinkType.References,
					)
				);
			},
		);
...
async function resolveNodeModulesPath(
	...
	const moduleBasePath = baseCandidate
		.split(posix.sep)
		.slice(0, sepIndex)
		.join(posix.sep);
	while (true) {
		const moduleAbsoluteUrl = vscode.Uri.joinPath(
			currentUri,
			"node_modules",
			moduleBasePath,
		);
		...
		if (moduleStat && moduleStat.type & vscode.FileType.Directory) {
			for (const uriCandidate of pathCandidates
				.map((relativePath) =>
					relativePath.split(posix.sep).slice(sepIndex).join(posix.sep),
				)
				// skip empty paths within module
				.filter(Boolean)
				.map((relativeModulePath) =>
					vscode.Uri.joinPath(moduleAbsoluteUrl, relativeModulePath),
				)) {
...
async function getTsconfigPath(
	...
	return resolveNodeModulesPath(baseDirUri, [
		...
		...(pathValue.endsWith(".json")
			? []
			: [`${pathValue}.json`, `${pathValue}/tsconfig.json`]),
	]);
}
...
export function register() {
	...
	return vscode.Disposable.from(
		vscode.commands.registerCommand(
			openExtendsLinkCommandId,
			async ({
				resourceUri,
				extendsValue,
				linkType,
			}: OpenExtendsLinkCommandArgs) => {
				const tsconfigPath = await getTsconfigPath(
					Utils.dirname(vscode.Uri.from(resourceUri)),
					extendsValue,
					linkType,
				);
