...
function toWorkspaceEdit(
	...
	edits: readonly Proto.FileCodeEdits[],
...
class SelectRefactorCommand implements Command {
	...
	public async execute(args: SelectRefactorCommand.Args): Promise<void> {
		...
		const selected = await vscode.window.showQuickPick(
			args.refactor.actions.map(
				(
					action,
				): vscode.QuickPickItem & { action: Proto.RefactorActionInfo } => ({
					action,
					label: action.name,
		...
		const tsAction = new InlinedCodeAction(
			this.client,
			args.document,
			args.refactor,
			selected.action,
			args.rangeOrSelection,
			args.trigger,
		);
		...
		if (tsAction.edit) {
			if (
				!(await vscode.workspace.applyEdit(tsAction.edit, {
					isRefactoring: true,
				}))
			) {
				vscode.window.showErrorMessage(
					vscode.l10n.t("Could not apply refactoring"),
				);
				return;
			}
		}
		...
		if (tsAction.command) {
			await vscode.commands.executeCommand(
				tsAction.command.command,
				...(tsAction.command.arguments ?? []),
			);
		}
	}
}
...
class MoveToFileRefactorCommand implements Command {
	...
	private async getTargetFile(
		...
		const updateItems = () => {
			const relativeQuery = ["./", "../"].find((str) =>
				quickPick.value.startsWith(str),
			);
			if (quickPickInRelativeMode === false && !!relativeQuery === false) {
				return;
			}
			quickPickInRelativeMode = !!relativeQuery;
			const destinationItems = body.files.map(
				(file): DestinationItem | undefined => {
					...
					if (workspaceFolder) {
						if (uri.scheme === Schemes.file) {
							...
							description = path.posix.relative(
						...
						if (relativeQuery) {
							...
							const relativePath = convertRelativePath(
								path.relative(path.dirname(document.uri.fsPath), uri.fsPath),
							);
					...
					return {
						...
						description: relativeQuery
							? description
							: path.join(description, filename),
					};
				},
			);
			quickPick.items = [
				...
				selectNewFileItem,
				{
					label: vscode.l10n.t("destination files"),
...
const Rewrite_Property_GenerateAccessors = Object.freeze<CodeActionKind>({
	...
	matches: (refactor) =>
		refactor.name.startsWith("Generate 'get' and 'set' accessors"),
});
...
class InlinedCodeAction extends vscode.CodeAction {
			...
			arguments: [
				{
					action: action.name,
					trigger,
				} satisfies DidApplyRefactoringCommand.Args,
			],
		};
	}
	...
	private static getKind(refactor: Proto.RefactorActionInfo) {
		if ((refactor as Proto.RefactorActionInfo & { kind?: string }).kind) {
			return vscode.CodeActionKind.Empty.append(
				(refactor as Proto.RefactorActionInfo & { kind?: string }).kind!,
			);
		}
		const match = allKnownCodeActionKinds.find((kind) =>
			kind.matches(refactor),
		);
			...
			arguments: [
				{
					...
					range,
					trigger,
				} satisfies MoveToFileRefactorCommand.Args,
			],
		};
	}
}
			...
			arguments: [
				{
					...
					rangeOrSelection,
					trigger,
				} satisfies SelectRefactorCommand.Args,
			],
		};
	}
}
...
class TypeScriptRefactorProvider implements vscode.CodeActionProvider<TsCodeAction> {
	...
	private static isOnSignatureName(
		...
		if (this._declarationKinds.has(node.kind)) {
			...
			if (
				!node.nameSpan &&
				/\bexport\b/.test(node.kindModifiers) &&
				node.spans.length
			) {
		...
		return (
			node.childItems?.some((child) => this.isOnSignatureName(child, range)) ??
			false
		);
	}
	...
	constructor(
		...
		const didApplyRefactoringCommand = new DidApplyRefactoringCommand(
			telemetryReporter,
		);
		...
		commandManager.register(
			new MoveToFileRefactorCommand(this.client, didApplyRefactoringCommand),
		);
		commandManager.register(
	...
	public async provideCodeActions(
		...
		const applicableRefactors = this.convertApplicableRefactors(
			document,
			context,
			response.body,
			rangeOrSelection,
		);
		const actions = coalesce(
			await Promise.all(
				Array.from(applicableRefactors, async (action) => {
					if (this.client.apiVersion.lt(API.v430)) {
						...
						if (
							!context.only &&
							action.kind?.value === "refactor.rewrite.function.returnType"
						) {
							return undefined;
						}
					}
					...
					if (
						this.client.apiVersion.lt(API.v540) &&
						context.triggerKind === vscode.CodeActionTriggerKind.Automatic
					) {
						if (
							action.kind?.value === Move_NewFile.kind.value ||
							action.kind?.value === Move_File.kind.value
						) {
							const file = this.client.toOpenTsFilePath(document);
							if (!file) {
								return undefined;
							...
							const navTree = await this.cachedNavTree.execute(document, () =>
								this.client.execute("navtree", { file }, token),
							);
							if (
								navTree.type !== "response" ||
								!navTree.body ||
								!TypeScriptRefactorProvider.isOnSignatureName(
									navTree.body,
									rangeOrSelection,
								)
							) {
	...
	private interruptGetErrIfNeeded<R>(
		context: vscode.CodeActionContext,
		f: () => R,
	): R {
	...
	private toTsTriggerReason(
		context: vscode.CodeActionContext,
	): Proto.RefactorTriggerReason | undefined {
		...
		for (const refactor of refactors) {
			if (refactor.inlineable === false) {
				...
				for (const action of refactor.actions) {
					for (const codeAction of this.refactorActionToCodeActions(
						document,
						context,
						refactor,
						action,
	...
	private refactorActionToCodeActions(
		...
		for (const codeAction of codeActions) {
			codeAction.isPreferred = TypeScriptRefactorProvider.isPreferred(
				action,
				allActions,
			);
		}
	...
	private static isPreferred(
		...
		if (Extract_Constant.matches(action)) {
			...
			return allActions
				.filter(
					(otherAtion) =>
						otherAtion !== action && Extract_Constant.matches(otherAtion),
				)
				.every((otherAction) => {
	...
	private pruneInvalidActions(
		...
		for (const action of actions) {
			...
			if (
				action.kind &&
				(Extract_Constant.kind.contains(action.kind) ||
					Extract_Function.kind.contains(action.kind))
			) {
		...
		const topNInvalid = prioritizedActions
			.filter((action) => !only || (action.kind && only.contains(action.kind)))
			.slice(0, numberOfInvalid);
