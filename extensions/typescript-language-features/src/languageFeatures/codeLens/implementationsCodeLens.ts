...
import {
	...
	requireHasModifiedUnifiedConfig,
...
export default class TypeScriptImplementationsCodeLensProvider extends TypeScriptBaseCodeLensProvider {
	...
	public constructor(
		...
		this._enabled = this._register(
			new ResourceUnifiedConfigValue<boolean>(Config.enabled, false),
		);
		this._register(this._enabled.onDidChange(() => this.changeEmitter.fire()));

		this._showOnInterfaceMethods = this._register(
			new ResourceUnifiedConfigValue<boolean>(
				Config.showOnInterfaceMethods,
				false,
			),
		);
		this._register(
			this._showOnInterfaceMethods.onDidChange(() => this.changeEmitter.fire()),
		);

		this._showOnAllClassMethods = this._register(
			new ResourceUnifiedConfigValue<boolean>(
				Config.showOnAllClassMethods,
				false,
			),
		);
		this._register(
			this._showOnAllClassMethods.onDidChange(() => this.changeEmitter.fire()),
		);
	}

	override async provideCodeLenses(
