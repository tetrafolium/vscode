...
export default class FileConfigurationManager extends Disposable {
	...
	private getFormatOptions(
		...
		const fallbackSection = isTypeScriptDocument(document)
			? "typescript"
			: "javascript";
		...
		return {
			...
			newLineCharacter: "\n",
			insertSpaceAfterCommaDelimiter: readUnifiedConfig<boolean>(
				"format.insertSpaceAfterCommaDelimiter",
				true,
				{ scope: document, fallbackSection },
			),
			insertSpaceAfterConstructor: readUnifiedConfig<boolean>(
				"format.insertSpaceAfterConstructor",
				false,
				{ scope: document, fallbackSection },
			),
			insertSpaceAfterSemicolonInForStatements: readUnifiedConfig<boolean>(
				"format.insertSpaceAfterSemicolonInForStatements",
				true,
				{ scope: document, fallbackSection },
			),
			insertSpaceBeforeAndAfterBinaryOperators: readUnifiedConfig<boolean>(
				"format.insertSpaceBeforeAndAfterBinaryOperators",
				true,
				{ scope: document, fallbackSection },
			),
			insertSpaceAfterKeywordsInControlFlowStatements:
				readUnifiedConfig<boolean>(
			...
			insertSpaceBeforeFunctionParenthesis: readUnifiedConfig<boolean>(
			...
			insertSpaceAfterTypeAssertion: readUnifiedConfig<boolean>(
				"format.insertSpaceAfterTypeAssertion",
				false,
				{ scope: document, fallbackSection },
			),
			placeOpenBraceOnNewLineForFunctions: readUnifiedConfig<boolean>(
				"format.placeOpenBraceOnNewLineForFunctions",
				false,
				{ scope: document, fallbackSection },
			),
			placeOpenBraceOnNewLineForControlBlocks: readUnifiedConfig<boolean>(
				"format.placeOpenBraceOnNewLineForControlBlocks",
				false,
				{ scope: document, fallbackSection },
			),
			semicolons: readUnifiedConfig<Proto.SemicolonPreference>(
				"format.semicolons",
				"ignore" as Proto.SemicolonPreference,
				{ scope: document, fallbackSection },
			),
			indentSwitchCase: readUnifiedConfig<boolean>(
	...
	private getPreferences(document: vscode.TextDocument): Proto.UserPreferences {
		...
		const preferences: Proto.UserPreferences = {
			...
			jsxAttributeCompletionStyle: getJsxAttributeCompletionStyle(
				document,
				fallbackSection,
			),
			allowTextChangesInNewFiles: document.uri.scheme === fileSchemes.file,
			providePrefixAndSuffixTextForRename: readUnifiedConfig<boolean>(
				"preferences.useAliasesForRenames",
				true,
				{ scope: document, fallbackSection },
			),
			allowRenameOfImportPath: true,
			includeAutomaticOptionalChainCompletions: readUnifiedConfig<boolean>(
				"suggest.includeAutomaticOptionalChainCompletions",
				true,
				{ scope: document, fallbackSection },
			),
			provideRefactorNotApplicableReason: true,
			generateReturnInDocTemplate: readUnifiedConfig<boolean>(
				"suggest.jsdoc.generateReturns",
				true,
				{ scope: document, fallbackSection },
			),
			includeCompletionsForImportStatements: readUnifiedConfig<boolean>(
				"suggest.includeCompletionsForImportStatements",
				true,
				{ scope: document, fallbackSection },
			),
			includeCompletionsWithSnippetText: true,
			includeCompletionsWithClassMemberSnippets: readUnifiedConfig<boolean>(
			...
			autoImportSpecifierExcludeRegexes: readUnifiedConfig<
				string[] | undefined
			>("preferences.autoImportSpecifierExcludeRegexes", undefined, {
				scope: document,
				fallbackSection,
			}),
			preferTypeOnlyAutoImports: readUnifiedConfig<boolean>(
			...
			includeCompletionsForModuleExports: readUnifiedConfig<boolean>(
				"suggest.autoImports",
				true,
				{ scope: document, fallbackSection },
			),
	...
	private getAutoImportFileExcludePatternsPreference(
		...
		const patterns = readUnifiedConfig<string[] | undefined>(
			"preferences.autoImportFileExcludePatterns",
			undefined,
			{ scope, fallbackSection },
		);
		return (
			workspaceFolder &&
			patterns?.map((p) => {
					...
					: p.startsWith("*")
						...
						: isRelative
							? this.client.toTsFilePath(
									vscode.Uri.joinPath(workspaceFolder, p),
								)!
	...
	private getMaximumHoverLength(document: vscode.TextDocument): number {
		...
		const maximumHoverLength = vscode.workspace
			.getConfiguration("js/ts", document)
			.get<number>("hover.maximumLength", defaultMaxLength);
...
function withDefaultAsUndefined<T, O extends T>(
	value: T,
	def: O,
...
export function getInlayHintsPreferences(
	...
	return {
		includeInlayParameterNameHints: getInlayParameterNameHintsPreference(
			scope,
			fallbackSection,
		),
		...
		includeInlayFunctionParameterTypeHints: readUnifiedConfig<boolean>(
			InlayHintSettingNames.parameterTypesEnabled,
			false,
			{ scope, fallbackSection },
		),
		includeInlayVariableTypeHints: readUnifiedConfig<boolean>(
		...
		includeInlayPropertyDeclarationTypeHints: readUnifiedConfig<boolean>(
			InlayHintSettingNames.propertyDeclarationTypesEnabled,
			false,
			{ scope, fallbackSection },
		),
		includeInlayFunctionLikeReturnTypeHints: readUnifiedConfig<boolean>(
			InlayHintSettingNames.functionLikeReturnTypesEnabled,
			false,
			{ scope, fallbackSection },
		),
		includeInlayEnumMemberValueHints: readUnifiedConfig<boolean>(
...
function getQuoteStylePreference(
	scope: UnifiedConfigurationScope,
	fallbackSection: string,
) {
	switch (
		readUnifiedConfig<string>("preferences.quoteStyle", "auto", {
			scope,
			fallbackSection,
		})
	) {
...
function getImportModuleSpecifierPreference(
	...
	switch (
		readUnifiedConfig<string>("preferences.importModuleSpecifier", "shortest", {
			scope,
			fallbackSection,
		})
	) {
...
function getJsxAttributeCompletionStyle(
	scope: UnifiedConfigurationScope,
	fallbackSection: string,
) {
	switch (
...
function getOrganizeImportsPreferences(
	scope: UnifiedConfigurationScope,
	fallbackSection: string,
): Proto.UserPreferences {
	const organizeImportsCollation = readUnifiedConfig<"ordinal" | "unicode">(
		"preferences.organizeImports.unicodeCollation",
		"ordinal",
		{ scope, fallbackSection },
	);
	const organizeImportsCaseSensitivity = readUnifiedConfig<
		"auto" | "caseInsensitive" | "caseSensitive"
	>("preferences.organizeImports.caseSensitivity", "auto", {
		scope,
		fallbackSection,
	});
	return {
		...
		organizeImportsTypeOrder: withDefaultAsUndefined(
			readUnifiedConfig<"auto" | "last" | "inline" | "first">(
		...
		organizeImportsIgnoreCase:
			organizeImportsCaseSensitivity === "caseInsensitive"
				? true
				: organizeImportsCaseSensitivity === "caseSensitive"
					? false
					: "auto",
		...
		...(organizeImportsCollation === "unicode"
			? {
					organizeImportsCaseFirst:
						organizeImportsCaseSensitivity === "caseInsensitive"
							? undefined
							: withDefaultAsUndefined(
									readUnifiedConfig<"default" | "upper" | "lower" | false>(
					...
					organizeImportsAccentCollation: readUnifiedConfig<
						boolean | undefined
					>("preferences.organizeImports.accentCollation", undefined, {
						scope,
						fallbackSection,
					}),
					organizeImportsLocale: readUnifiedConfig<string | undefined>(
						"preferences.organizeImports.locale",
						undefined,
						{ scope, fallbackSection },
					),
					organizeImportsNumericCollation: readUnifiedConfig<
						boolean | undefined
					>("preferences.organizeImports.numericCollation", undefined, {
						scope,
						fallbackSection,
					}),
				}
