...
namespace CompletionConfiguration {
	...
	export function getConfigurationForResource(
		...
		return {
			completeFunctionCalls: readUnifiedConfig<boolean>(
				CompletionConfiguration.completeFunctionCalls,
				false,
				{ scope: scope, fallbackSection: modeId },
			),
			pathSuggestions: readUnifiedConfig<boolean>(
				CompletionConfiguration.pathSuggestions,
				true,
				{ scope: scope, fallbackSection: modeId },
			),
			autoImportSuggestions: readUnifiedConfig<boolean>(
				CompletionConfiguration.autoImportSuggestions,
				true,
				{ scope: scope, fallbackSection: modeId },
			),
			nameSuggestions: readUnifiedConfig<boolean>(
				CompletionConfiguration.nameSuggestions,
				true,
				{ scope: scope, fallbackSection: modeId },
			),
			importStatementSuggestions: readUnifiedConfig<boolean>(
...
class TypeScriptCompletionItemProvider implements vscode.CompletionItemProvider<MyCompletionItem> {
	...
	public async provideCompletionItems(
		...
		if (
			!readUnifiedConfig<boolean>("suggest.enabled", true, {
				scope: document,
				fallbackSection: this.language.id,
			})
		) {
			return undefined;
		}
		...
		const isIncomplete =
			...
			!!(response.metadata as Record<string, unknown>)?.isIncomplete;
