...
export default class LanguageProvider extends Disposable {
	constructor(
		...
		const scope: vscode.ConfigurationScope = {
			languageId: this.description.languageIds[0],
		};

		const validateConfig = this._register(
			new UnifiedConfigValue<boolean>("validate.enabled", true, {
				scope,
				fallbackSection: this.id,
				fallbackSubSectionNameOverride: "validate.enable",
			}),
		);
		this.updateValidate(validateConfig.getValue());
		this._register(
			validateConfig.onDidChange((value) => this.updateValidate(value)),
		);

		const suggestionsConfig = this._register(
			new UnifiedConfigValue<boolean>("suggestionActions.enabled", true, {
				scope,
				fallbackSection: this.id,
			}),
		);
		...
		this._register(
			suggestionsConfig.onDidChange((value) =>
				this.updateSuggestionDiagnostics(value),
			),
		);
	...
	private async registerProviders(): Promise<void> {
		...
		await Promise.all([
			...
			import("./languageFeatures/copyPaste").then((provider) =>
				this._register(
					provider.register(
						selector,
						this.description,
			...
			import("./languageFeatures/directiveCommentCompletions").then(
				(provider) => this._register(provider.register(selector, this.client)),
			),
			...
			import("./languageFeatures/documentSymbol").then((provider) =>
				this._register(
					provider.register(selector, this.client, cachedNavTreeResponse),
				),
			),
			import("./languageFeatures/fileReferences").then((provider) =>
				this._register(provider.register(this.client, this.commandManager)),
			),
	...
	public handlesUri(resource: vscode.Uri): boolean {
		...
		return (
			this.description.standardFileExtensions.includes(ext) ||
			this.handlesConfigFile(resource)
		);
	}

	public handlesDocument(doc: vscode.TextDocument): boolean {
		return (
			this.description.languageIds.includes(doc.languageId) ||
			this.handlesConfigFile(doc.uri)
		);
	}
	...
	public diagnosticsReceived(
		...
		if (
			...
			!this.client.hasCapabilityForResource(file, ClientCapability.Semantic)
		) {
			return;
		}
		...
		if (
			diagnosticsKind === DiagnosticKind.Semantic &&
			file.scheme === Schemes.notebookCell
		) {
			return;
		}
