...
export class SingleTsServer extends Disposable implements ITypeScriptServer {
	...
	private tryCancelRequest(request: Proto.Request, command: string): boolean {
		...
		if (callback?.traceId !== undefined) {
			this._telemetryReporter.logTraceEvent(
				"TSServer.tryCancelRequest",
				callback.traceId,
				JSON.stringify({ command, cancelled: true }),
			);
		}
		try {
	...
	private dispatchResponse(response: Proto.Response) {
		...
		if (callback.traceId !== undefined) {
			this._telemetryReporter.logTraceEvent(
				"TSServerRequest.dispatchResponse",
				callback.traceId,
				JSON.stringify({
			...
			result = new Promise<ServerResponse.Response<Proto.Response>>(
				(resolve, reject) => {
					const item: CallbackItem<
						ServerResponse.Response<Proto.Response> | undefined
					> =
						typeof request.arguments?.$traceId === "string"
							? {
	...
	private sendRequest(requestItem: RequestItem): void {
		...
		try {
			...
			if (typeof serverRequest.arguments?.$traceId === "string") {
				this._telemetryReporter.logTraceEvent(
					"TSServer.sendRequest",
					serverRequest.arguments.$traceId,
					JSON.stringify({ command: serverRequest.command }),
				);
			}
		} catch (err) {
...
export class GetErrRoutingTsServer
	extends Disposable
	implements ITypeScriptServer
{
			...
			[
				{
					...
					canRun: (command) => ["geterr", "geterrForProject"].includes(command),
				},
				{
...
export class SyntaxRoutingTsServer
	extends Disposable
	implements ITypeScriptServer
{
	/**
