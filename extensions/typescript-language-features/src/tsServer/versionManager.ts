...
const suppressPromptWorkspaceTsdkStorageKey =
	"typescript.suppressPromptWorkspaceTsdk";
...
export class TypeScriptVersionManager extends Disposable {
		...
		return this.versionProvider.localVersions.map((version) => {
			return {
				label:
					...
					this.currentVersion.eq(version)
						? "• "
				...
				run: async () => {
					...
					if (trusted) {
						...
						await vscode.workspace
							.getConfiguration(unifiedConfigSection)
							.update("tsdk.path", version.pathLabel, false);
	...
	private getNativePreviewPickItem(): QuickPickItem | undefined {
		const nativePreviewExtension =
			vscode.extensions.getExtension(tsNativeExtensionId);
		if (!nativePreviewExtension) {
			return undefined;
		}

		const isUsingTsgo = readUnifiedConfig<boolean>(
		...
		return {
			label:
				(isUsingTsgo ? "• " : "") +
				vscode.l10n.t("Use TypeScript Native Preview (Experimental)"),
			description: nativePreviewExtension.packageJSON.version,
			run: async () => {
				await vscode.commands.executeCommand(
					"typescript.native-preview.enable",
				);
			},
		};
	}
