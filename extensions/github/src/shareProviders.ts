	...
	implements vscode.ShareProvider, vscode.Disposable
{
	...
	private initializeGitHubRepoContext() {
		if (
			this.gitAPI.repositories.find((repo) => repositoryHasGitHubRemote(repo))
		) {
			...
			this.disposables.push(
				this.gitAPI.onDidOpenRepository(async (e) => {
					await e.status();
	...
	private ensureShareProviderRegistration() {
		if (
			...
			const shareProviderRegistration = vscode.window.registerShareProvider(
				{ scheme: "file" },
				this,
			);
	...
	async provideShare(
		...
		repository.state.remotes.find((remote) => {
			if (remote.fetchUrl) {
				...
				if (
					foundRepo &&
					remote.name === repository.state.HEAD?.upstream?.remote
				) {
					repo = foundRepo;
					return;
		...
		const filepathSegment = encodeURIComponentExceptSlashes(
			item.resourceUri.path.substring(repository?.rootUri.path.length),
		);
...
function getRangeSegment(item: vscode.ShareableItem) {
	if (item.resourceUri.scheme === "vscode-notebook-cell") {
		const notebookEditor = vscode.window.visibleNotebookEditors.find(
			(editor) => editor.notebook.uri.fsPath === item.resourceUri.fsPath,
		);
		const cell = notebookEditor?.notebook
			.getCells()
			.find(
				(cell) => cell.document.uri.fragment === item.resourceUri?.fragment,
			);
