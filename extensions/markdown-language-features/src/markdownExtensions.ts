...
export namespace MarkdownContributions {
	...
	function previewScriptEqual(
		a: MarkdownPreviewScript,
		b: MarkdownPreviewScript,
	): boolean {
	...
	export function equal(
		...
		return (
			arrays.equals(a.previewScripts, b.previewScripts, previewScriptEqual) &&
	...
	function* resolvePreviewScripts(
		extension: vscode.Extension<any>,
		scripts: unknown,
	): Iterable<MarkdownPreviewScript> {
	...
	function getPreviewScriptContribution(
		script: unknown,
	): { path: string; type?: MarkdownPreviewScript["type"] } | undefined {
	...
	implements MarkdownContributionProvider
{
		...
		this._register(
			vscode.extensions.onDidChange(() => {
				...
				const existingContributions =
					this.#contributions || MarkdownContributions.Empty;
				if (
	...
	readonly #onContributionsChanged = this._register(
