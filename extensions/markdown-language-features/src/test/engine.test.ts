...
suite("markdown.engine", () => {
	suite("rendering", () => {
		const input = "# hello\n\nworld!";
		const output =
			'<h1 data-line="0" class="code-line" dir="auto" id="hello">hello</h1>\n' +
	...
	suite("image-caching", () => {
		const input =
			"![](img.png) [](no-img.png) ![](http://example.org/img.png) ![](img.png) ![](./img2.png)";
		...
		test("Extracts all images", async () => {
			...
			assert.deepStrictEqual(
				[...result.containingImages],
				["img.png", "http://example.org/img.png", "./img2.png"],
			);
		});
	});
	...
	suite("front-matter", () => {
		...
		suiteSetup(() => {
			originalValue = vscode.workspace
				...
				.inspect<string>(settingName)?.globalValue;
		});

		suiteTeardown(async () => {
			await vscode.workspace
				.getConfiguration("markdown")
				.update(settingName, originalValue, vscode.ConfigurationTarget.Global);
		});

		async function setStyle(style: string) {
			await vscode.workspace
				.getConfiguration("markdown")
				.update(settingName, style, vscode.ConfigurationTarget.Global);
		}
		...
		test('Renders frontmatter as a code block when style is "codeBlock"', async () => {
			...
			assert.match(
				html,
				/<pre[^>]*class="[^"]*frontmatter[^"]*"[^>]*>[\s\S]*<\/pre>/,
			);
			assert.ok(
				html.includes("title"),
				`Expected frontmatter content to be rendered. Got: ${html}`,
			);
			assert.ok(
				html.includes('<h1 data-line="4"'),
				`Expected body to render after frontmatter. Got: ${html}`,
			);
		});
		...
		test("Shows an error when frontmatter has invalid YAML", async () => {
			...
			assert.ok(
				html.includes('<h1 data-line="4"'),
				`Expected body to render after error. Got: ${html}`,
			);
		});
		...
		test("Ignores frontmatter that is not at the start of the document", async () => {
			...
			const html = (await engine.render("# World\n\n---\ntitle: Hello\n---"))
				.html;
			assert.ok(
				!html.includes('<table class="frontmatter">'),
				`Expected no frontmatter table. Got: ${html}`,
			);
		});
