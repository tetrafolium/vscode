...
class AddToIgnoreLinksQuickFixProvider implements vscode.CodeActionProvider {
	...
	public static register(
		...
		const reg = vscode.languages.registerCodeActionsProvider(
			...
			AddToIgnoreLinksQuickFixProvider.#metadata,
		);
		...
		for (const diagnostic of context.diagnostics) {
			switch (diagnostic.code) {
				...
				case DiagnosticCode.link_noSuchHeaderInFile: {
					const hrefText = (diagnostic as unknown as Record<string, any>).data
						?.hrefText;
					if (hrefText) {
						...
						fix.command = {
							command:
								AddToIgnoreLinksQuickFixProvider.#addToIgnoreLinksCommandId,
							title: "",
...
function registerMarkdownStatusItem(
	...
	const commandSub = commandManager.register({
		...
		execute: (enabled: boolean) => {
			vscode.workspace
				...
				.update(enabledSettingId, enabled);
		},
	});
	...
	const update = () => {
		...
		const markdownDoc =
			activeDoc && isMarkdownFile(activeDoc) ? activeDoc : undefined;
