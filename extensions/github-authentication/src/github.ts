...
function isGitHubAuthenticationProviderOptions(
	object: any,
): object is GitHubAuthenticationProviderOptions {
	if (!object || typeof object !== "object") {
		throw new Error("Options are not an object");
	}
	if (
		object.provider !== undefined &&
		!isSocialSignInProvider(object.provider)
	) {
		throw new Error(`Provider is invalid: ${object.provider}`);
	}
	if (object.extraAuthorizeParameters !== undefined) {
		if (
			!object.extraAuthorizeParameters ||
			typeof object.extraAuthorizeParameters !== "object"
		) {
			throw new Error(
				"Extra parameters must be a record of string keys and string values.",
			);
		}
		...
		try {
			...
			*/
			...
			const token = await this._githubServer.login(
				scopeString,
				signInProvider,
				options?.extraAuthorizeParameters,
				loginWith,
			);
