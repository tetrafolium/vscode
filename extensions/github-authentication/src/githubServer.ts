...
export class GitHubServer implements IGitHubServer {
	...
	constructor(
		...
		this.friendlyName =
			this._type === AuthProviderType.github ? "GitHub" : _ghesUri?.authority!;
	}
	...
	private async getRedirectEndpoint(): Promise<string> {
		...
		if (this._type === AuthProviderType.github) {
			...
			if (
				...
				new URL(proxyEndpoints.github).hostname === "insiders.vscode.dev"
			) {
	...
	public async login(
		...
		const callbackUri = await vscode.env.asExternalUri(
			vscode.Uri.parse(
				`${vscode.env.uriScheme}://vscode.github-authentication/did-authenticate?nonce=${encodeURIComponent(nonce)}`,
			),
		);
	...
	public async logout(session: vscode.AuthenticationSession): Promise<void> {
		...
		const authHeader =
			"Basic " +
			base64Encode(`${Config.gitHubClientId}:${Config.gitHubClientSecret}`);
	...
	public async getUserInfo(
		...
		if (result.ok) {
			try {
				...
				this._logger.error(
					`Unexpected error parsing response from GitHub: ${e.message ?? e}`,
				);
				throw e;
			}
		} else {
	...
	private async checkUserDetails(
		...
		try {
			...
			if (result.ok) {
				const json: { student: boolean; faculty: boolean } =
					await result.json();
				edu = json.student ? "student" : json.faculty ? "faculty" : "none";
			} else {
				this._logger.info(
					`Unable to resolve optional EDU details. Status: ${result.status} ${result.statusText}`,
				);
				edu = "unknown";
			}
		} catch (e) {
