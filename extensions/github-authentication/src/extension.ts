...
function initGHES(
	...
	try {
		...
		vscode.window.showErrorMessage(
			vscode.l10n.t(
				"GitHub Enterprise Server URI is not a valid URI: {0}",
				e.message ?? e,
			),
		);
...
export function activate(context: vscode.ExtensionContext) {
	...
	const beforeFetchSetting = vscode.workspace
		.getConfiguration()
		.get<boolean>("github-authentication.useElectronFetch", true);
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration(async (e) => {
			if (e.affectsConfiguration("github-authentication.useElectronFetch")) {
				const afterFetchSetting = vscode.workspace
					.getConfiguration()
					.get<boolean>("github-authentication.useElectronFetch", true);
					...
					const selection = await vscode.window.showInformationMessage(
						vscode.l10n.t("GitHub Authentication - Reload required"),
						{
							...
							detail: vscode.l10n.t(
								"A reload is required for the fetch setting change to take effect.",
							),
						},
