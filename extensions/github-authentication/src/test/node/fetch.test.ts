...
suite("fetching", () => {
	...
	test("should fall back to Node.js fetch", async () => {
		const res = await createFetch()(
			`http://localhost:${port}/json?expectAgent=node`,
			{
				logger,
				retryFallbacks: true,
				expectJSON: true,
	...
	test("should fall back to Node.js fetch 2", async () => {
		const res = await createFetch()(
			`http://localhost:${port}/json?expectAgent=node&error=html`,
			{
				logger,
				retryFallbacks: true,
				expectJSON: true,
	...
	test("should fall back to Node.js http/s", async () => {
		const res = await createFetch()(
			`http://localhost:${port}/json?expectAgent=undefined`,
			{
				logger,
				retryFallbacks: true,
				expectJSON: true,
	...
	test("should not retry with other fetchers on 429 status", async () => {
		...
		let requestCount = 0;
		const oldListener = server.listeners("request")[0] as (
		...
		try {
			...
			assert.strictEqual(
				requestCount,
				1,
				"Should only make one request for 429 status",
			);
