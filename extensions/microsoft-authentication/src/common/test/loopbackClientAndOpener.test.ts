...
suite("UriHandlerLoopbackClient", () => {
	...
	setup(async () => {
		callbackUri = await env.asExternalUri(
		...
		client = new UriHandlerLoopbackClient(
			uriHandler,
			redirectUri,
			callbackUri,
			window.createOutputChannel("test", { log: true }),
		);
	});
	...
	suite("openBrowser", () => {
		test("should open browser with correct URL", async () => {
			...
			const expectedUri = Uri.parse(
				testUrl + `&state=${encodeURI(callbackUri.toString(true))}`,
			);
	...
	suite("listenForAuthCode", () => {
		...
		test("should return auth error from URL", async () => {
			...
			const errorUri = "uri";
			const testUrl = Uri.parse(
				`http://example.com?error=${error}&error_description=${errorDescription}&error_uri=${errorUri}`,
			);
