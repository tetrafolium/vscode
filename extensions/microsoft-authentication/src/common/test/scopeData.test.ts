...
suite("ScopeData", () => {
	...
	test("should not duplicate default scopes if already present", () => {
		...
		assert.deepStrictEqual(scopeData.allScopes, [
			"custom_scope",
			"email",
			"offline_access",
			"openid",
	...
	test("should create a space-separated string of all scopes", () => {
		...
		assert.strictEqual(
			scopeData.scopeStr,
			"custom_scope email offline_access openid profile",
		);
	});
	...
	test("should add TACK ON scope if all scopes are OIDC scopes", () => {
		...
		assert.deepStrictEqual(scopeData.scopesToSend, [
			"email",
			"offline_access",
			"openid",
			"profile",
	...
	test("should extract tenant from authorization server URL path", () => {
		const authorizationServer = Uri.parse(
			"https://login.microsoftonline.com/tenant123/oauth2/v2.0",
		);
		const scopeData = new ScopeData(
			["custom_scope"],
			undefined,
	...
	test("should prioritize authorization server URL over VSCODE_TENANT scope", () => {
		const authorizationServer = Uri.parse(
			"https://login.microsoftonline.com/url_tenant/oauth2/v2.0",
		);
		const scopeData = new ScopeData(
			["custom_scope", "VSCODE_TENANT:scope_tenant"],
			undefined,
			authorizationServer,
		);
	...
	test("should extract tenant from v1.0 authorization server URL path", () => {
		const authorizationServer = Uri.parse(
			"https://login.microsoftonline.com/tenant123",
		);
		const scopeData = new ScopeData(
			["custom_scope"],
			undefined,
	...
	test("should use explicit clientId over VSCODE_CLIENT_ID scope", () => {
		const scopeData = new ScopeData(
			["custom_scope", "VSCODE_CLIENT_ID:scope_id"],
			undefined,
			undefined,
			"override-id",
		);
	...
	test("should ignore empty or whitespace-only clientId and fall back to default", () => {
		...
		const whitespace = new ScopeData(
			["custom_scope"],
			undefined,
			undefined,
			"   ",
