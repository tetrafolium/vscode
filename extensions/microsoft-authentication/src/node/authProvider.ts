...
export class MsalAuthProvider implements AuthenticationProvider {
	...
	private async _migrateSessions() {
		...
		for (const {
			...
			tenant,
			refreshTokens,
		} of clientTenantMap.values()) {
			await this._publicClientManager.getOrCreate(clientId, {
				refreshTokensToMigrate: refreshTokens,
				tenant,
			});
		}
	}
	...
	async getSessionsFromChallenges(
		...
		this._logger.info(
			...
			"starting with",
			constraint.challenges.length,
			"challenges",
		);

		// Use scopes from challenges if provided, otherwise use fallback scopes
		const scopes =
			this.extractScopesFromChallenges(constraint.challenges) ??
			constraint.fallbackScopes;
		if (!scopes || scopes.length === 0) {
			throw new Error(
				"No scopes found in authentication challenges or fallback scopes",
			);
		}
	...
	async createSessionFromChallenges(
		constraint: AuthenticationConstraint,
		options: AuthenticationProviderSessionOptions,
	): Promise<AuthenticationSession> {
		this._logger.info(
		...
		const effectiveScopes =
			scopes.length > 0 ? scopes : ["https://graph.microsoft.com/User.Read"];
		...
		for (const flow of flows) {
			...
			try {
				...
				if (
					...
					e.errorCode === ClientAuthErrorCodes.userCanceled
				) {
					this._logger.info(
						"[createSessionFromChallenges]",
						`[${scopeData.scopeStr}]`,
	...
	private extractClaimsFromChallenges(
		challenges: readonly AuthenticationChallenge[],
	): string | undefined {
		for (const challenge of challenges) {
			if (
				...
				try {
					...
					this._logger.warn(
						...
						"failed to decode claims... checking if it is already JSON",
						e,
					);
					try {
						...
						this._logger.error(
							"[extractClaimsFromChallenges]",
							"failed to parse claims as JSON... returning undefined",
							e,
						);
					}
				}
		...
		return this._eventBufferer.bufferEventsAsync(async () => {
			for (const account of filteredAccounts) {
				try {
					...
					this._logger.trace(
						`[getAllSessionsForPca] [${scopeData.scopeStr}] [${account.environment}] [${account.username}] acquiring token silently with${forceRefresh ? " " : "out "}force refresh${claims ? " and claims" : ""}${resource ? " and resource" : ""}...`,
					);
					...
					this._logger.info(
						`[getAllSessionsForPca] [${scopeData.scopeStr}] [${account.username}] failed to acquire token silently, skipping account`,
						JSON.stringify(e),
					);
					continue;
				}
