...
class RecordingCachedPca implements ICachedPublicClientApplication {
	...
	readonly deviceCodeRequests: Array<
		Omit<DeviceCodeRequest, "deviceCodeCallback">
	> = [];

	private readonly _accountsChange = new EventEmitter<{
		added: AccountInfo[];
		changed: AccountInfo[];
		deleted: AccountInfo[];
	}>();
...
suite("MSAL flow trigger", () => {
	...
	suiteSetup(() => {
		logger = window.createOutputChannel("msal-flow-trigger-test", {
			log: true,
		});
	});
