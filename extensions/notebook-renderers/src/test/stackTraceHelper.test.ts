...
suite("StackTraceHelper", () => {
	...
	test("IPython stack line numbers are linkified for IPython 8.3.6", () => {
		...
		assert.ok(
			...
			"Missing line link in " + cleanStack,
		);
		assert.ok(
			cleanStack.indexOf(
				"<a href='vscode-notebook-cell:?execution_count=3&line=2'>2</a>",
			) > 0,
			"Missing frame link in " + cleanStack,
		);
		assert.ok(
			cleanStack.indexOf("<a href='C:\\venvs\\myLib.py:2'>2</a>") > 0,
			"Missing frame link in " + cleanStack,
		);
		assert.equal(
			errorLocation,
			"<a href='vscode-notebook-cell:?execution_count=3&line=2'>line 2</a>",
		);
	});
	...
	test("Does not have catastrophic backtracking https://github.com/microsoft/vscode/issues/251731", () => {
		...
		assert.ok(
			cleanStack.indexOf(
				"<a href='vscode-notebook-cell:?execution_count=1&line=2'>2</a>",
			) > 0,
			"Missing frame link in " + cleanStack,
		);
		assert.equal(
			errorLocation,
			"<a href='vscode-notebook-cell:?execution_count=1&line=2'>line 2</a>",
		);
	});
	...
	test("Stack trace is not linkified when HTML is not trusted", () => {
		...
		formattedLines.forEach((line) =>
			assert.ok(
				!/<a href=.*>/.test(line),
				"line should not contain a link: " + line,
			),
		);
	});
	...
	test("IPython stack line numbers are linkified for IPython 8.3", () => {
		...
		assert.ok(
			formatted.indexOf(
				"Input <a href='vscode-notebook-cell:?execution_count=2'>In [2]</a>, in <cell line: 5>",
			) > 0,
			"Missing cell link in " + formatted,
		);
		assert.ok(
			formatted.indexOf(
				"Input <a href='vscode-notebook-cell:?execution_count=1'>In [1]</a>, in myfunc()",
			) > 0,
			"Missing cell link in " + formatted,
		);
		assert.ok(
			formatted.indexOf(
				"<a href='vscode-notebook-cell:?execution_count=2&line=5'>5</a>",
			) > 0,
			"Missing frame link in " + formatted,
		);
	});
	...
	test("IPython stack without line numbers are not linkified", () => {
		...
		assert.ok(
			/<a href='vscode-notebook-cell.*>/.test(formattedLines[0]),
			"line should contain a link: " + formattedLines[0],
		);
		formattedLines
			.slice(1)
			.forEach((line) =>
	...
	test("background (40-49) ANSI colors are removed", () => {
		...
		assert.ok(
			!/4\d/.test(formattedLines[0]),
			"should not contain background colors " + formattedLines[0],
		);
		formattedLines
			.slice(1)
			.forEach((line) =>
