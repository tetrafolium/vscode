...
export async function startClient(
	...
	const formatterRegistrations: FormatterRegistration[] = documentSelector.map(
		(languageId) => ({
			languageId,
			settingId: `${languageId}.format.enable`,
			provider: undefined,
		}),
	);
		...
		middleware: {
			provideCompletionItem(
				...
				function isThenable<T>(obj: unknown): obj is Thenable<T> {
					return (
						!!obj && typeof (obj as unknown as Thenable<T>).then === "function"
					);
				}
					...
					registration.languageId,
					{
						provideDocumentRangeFormattingEdits(
							...
							const fileFormattingOptions = {
								...
								trimFinalNewlines:
									filesConfig.get<boolean>("trimFinalNewlines"),
								insertFinalNewline:
							...
							const formatterSettings = workspace
								.getConfiguration(registration.languageId, document)
