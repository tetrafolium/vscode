...
suite("Completions in settings.json", () => {
	...
	test("window.title", async () => {
		{
			...
			const content = ["{", '  "window.title": "custom|"', "}"].join("\n");
			const resultText = [
				"{",
		...
		}
		{
			...
			const content = ["{", '  "window.title": "${a|"', "}"].join("\n");
		...
		{
			// inserting a literal
			const content = ["{", '  "window.title": |', "}"].join("\n");
			const resultText = [
				"{",
				'  "window.title": "${activeEditorMedium}"',
				"}",
	...
	test("files.exclude", async () => {
		{
			const content = ["{", '  "files.exclude": {', "    |", "  }", "}"].join(
				"\n",
			);
			const resultText = [
				"{",
	...
	test("files.defaultLanguage", async () => {
		{
			const content = ["{", '  "files.defaultLanguage": "json|"', "}"].join(
				"\n",
			);
		...
		{
			const content = ["{", '  "files.defaultLanguage": |', "}"].join("\n");
			const resultText = ["{", '  "files.defaultLanguage": "jsonc"', "}"].join(
				"\n",
			);
			const expected = { label: '"jsonc"', resultText };
...
async function testCompletion(
	...
	const actualCompletions = (await vscode.commands.executeCommand(
	...
	if (expected.notAvailable) {
		...
		assert.strictEqual(
			matches.length,
			1,
			`${expected.label} should only existing once: Actual: ${actualCompletions.items.map((c) => c.label).join(", ")}`,
		);
		...
		if (expected.resultText) {
			...
			if (match.range && match.insertText) {
				const range =
					match.range instanceof vscode.Range
						? match.range
						: match.range.replacing;
				const text =
					typeof match.insertText === "string"
						? match.insertText
						: match.insertText.value;
