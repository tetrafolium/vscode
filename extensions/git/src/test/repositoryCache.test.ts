...
class TestRepositoryCache extends RepositoryCache {
	constructor(
		...
		private readonly _workspaceFileProp: Uri | undefined,
		private readonly _workspaceFoldersProp:
...
suite("RepositoryCache", () => {
	test("set & get basic", () => {
		...
		const cache = new TestRepositoryCache(
			memento,
			new MockLogOutputChannel(),
			undefined,
			[{ uri: folder, name: "workspace", index: 0 }],
		);

		cache.set("https://example.com/repo.git", folder.fsPath);
		const folders = cache
			.get("https://example.com/repo.git")!
			.map((folder) => folder.workspacePath);
	...
	test("inner LRU capped at 10 entries", () => {
		...
		for (let i = 1; i <= 12; i++) {
			workspaceFolders.push({
				uri: Uri.file(`/ws/folder-${i.toString().padStart(2, "0")}`),
				name: `folder-${i.toString().padStart(2, "0")}`,
				index: i - 1,
			});
		}
		const cache = new TestRepositoryCache(
		...
		for (let i = 1; i <= 12; i++) {
			cache.set(
				repo,
				Uri.file(`/ws/folder-${i.toString().padStart(2, "0")}`).fsPath,
			);
		}
		...
		assert.strictEqual(
			folders.length,
			10,
			"should only retain 10 most recent folders",
		);
		assert.ok(
			!folders.includes(Uri.file("/ws/folder-01").fsPath),
			"oldest folder-01 should be evicted",
		);
		assert.ok(
			!folders.includes(Uri.file("/ws/folder-02").fsPath),
			"second oldest folder-02 should be evicted",
		);
		assert.ok(
			folders.includes(Uri.file("/ws/folder-12").fsPath),
			"latest folder should be present",
		);
	});
	...
	test("outer LRU capped at 30 repos", () => {
		...
		assert.strictEqual(
			cache.get("https://example.com/r1.git"),
			undefined,
			"oldest repo should be trimmed",
		);
		assert.ok(
			cache.get("https://example.com/r35.git"),
			"newest repo should remain",
		);
	});
	...
	test("delete removes folder and prunes empty repo", () => {
		...
		assert.deepStrictEqual(
			new Set(cache.get(repo)?.map((folder) => folder.workspacePath)),
			new Set([a, b]),
		);
		cache.delete(repo, a);
		assert.deepStrictEqual(
			cache.get(repo)!.map((folder) => folder.workspacePath),
			[b],
		);
		cache.delete(repo, b);
		assert.strictEqual(
			cache.get(repo),
			undefined,
			"repo should be pruned when last folder removed",
		);
	});
	...
	test("normalizes URLs with trailing .git", () => {
		...
		assert.deepStrictEqual(
			withGit,
			withoutGit,
			"should return same result regardless of .git suffix",
		);
	});
	...
	test("normalizes URLs with trailing slashes and .git", () => {
		...
		for (let i = 1; i < results.length; i++) {
			assert.deepStrictEqual(
				results[i],
				results[0],
				`variation ${variations[i]} should return same result`,
			);
		}
	});
