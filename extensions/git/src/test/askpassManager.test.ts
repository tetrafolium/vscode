...
suite("askpassManager", () => {
	...
	setup(async () => {
		...
		tempDir = await fs.promises.mkdtemp(
			path.join(os.tmpdir(), "askpass-test-"),
		);
	...
	test("garbage collection removes old directories", async function () {
		...
		for (const dirName of oldDirs) {
			...
			assert.strictEqual(
				exists,
				false,
				`Old directory ${dirName} should have been removed`,
			);
		}

		// Check that recent directory still exists
		const recentExists = await fs.promises
			.access(recentDir)
			.then(() => true)
			.catch(() => false);
		assert.strictEqual(
		...
		// Check logs
		assert.ok(
			logger.hasLog("info", "Removing old askpass directory"),
	...
	test("garbage collection handles empty askpass directory", async function () {
		...
		assert.ok(
			true,
			"Should handle empty or non-existent askpass directory gracefully",
		);
	});
	...
	test("current content-addressed directory is not removed", async function () {
		...
		assert.strictEqual(
			exists,
			true,
			"Current content-addressed directory should not be removed",
		);
	});
});
