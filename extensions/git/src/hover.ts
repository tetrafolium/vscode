...
export function getCommitHover(
	authorAvatar: string | undefined,
	authorName: string | undefined,
	authorEmail: string | undefined,
	authorDate: Date | number | undefined,
	message: string,
	shortStats: CommitShortStat | undefined,
...
export function getHistoryItemHover(
	authorAvatar: string | undefined,
	authorName: string | undefined,
	authorEmail: string | undefined,
	authorDate: Date | number | undefined,
...
function appendContent(
	...
	if (authorName) {
		...
		if (authorDate && !isNaN(new Date(authorDate).getTime())) {
			...
			markdownString.appendText(
				` ${fromNow(authorDate, true, true)} (${dateString})`,
			);
		}
	...
	markdownString.appendMarkdown(
		`${emojify(message.replace(/!\[/g, "&#33;&#91;").replace(/\r\n|\r|\n/g, "\n\n"))}`,
	);
	markdownString.appendMarkdown(`\n\n---\n\n`);
}
...
function appendShortStats(
	markdownString: MarkdownString,
	shortStats: { files: number; insertions: number; deletions: number },
): void {
	// Short stats
	markdownString.appendMarkdown(
		`<span>${
			shortStats.files === 1
				...
				: l10n.t("{0} files changed", shortStats.files)
		}</span>`,
	);
	...
	if (shortStats.insertions) {
		markdownString.appendMarkdown(
			`,&nbsp;<span style="color:var(--vscode-scmGraph-historyItemHoverAdditionsForeground);">${
				shortStats.insertions === 1
					? l10n.t("{0} insertion{1}", shortStats.insertions, "(+)")
					: l10n.t("{0} insertions{1}", shortStats.insertions, "(+)")
			}</span>`,
		);
	}
	...
	if (shortStats.deletions) {
		markdownString.appendMarkdown(
			`,&nbsp;<span style="color:var(--vscode-scmGraph-historyItemHoverDeletionsForeground);">${
				shortStats.deletions === 1
					? l10n.t("{0} deletion{1}", shortStats.deletions, "(-)")
					: l10n.t("{0} deletions{1}", shortStats.deletions, "(-)")
			}</span>`,
		);
	}
...
function appendCommands(
	...
	for (let index = 0; index < commands.length; index++) {
		...
		const commandsMarkdown = commands[index].map(
			(command) =>
				`[${command.title}](command:${command.command}?${encodeURIComponent(JSON.stringify(command.arguments))} "${command.tooltip}")`,
		);
		markdownString.appendMarkdown(commandsMarkdown.join("&nbsp;"));
	}
}

export function getHoverCommitHashCommands(
	documentUri: Uri,
	hash: string,
): Command[] {
	return [
...
export function processHoverRemoteCommands(
	commands: Command[],
	hash: string,
): Command[] {
	return commands.map(
