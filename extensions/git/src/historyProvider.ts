...
function compareSourceControlHistoryItemRef(
	ref1: SourceControlHistoryItemRef,
	ref2: SourceControlHistoryItemRef,
): number {
	...
	implements SourceControlHistoryProvider, FileDecorationProvider, IDisposable
{
	...
	private async onDidRunWriteOperation(result: OperationResult): Promise<void> {
			...
			case RefType.Head: {
				if (this.repository.HEAD.name !== undefined) {
					...
					if (this._HEAD?.name !== this.repository.HEAD.name) {
						...
						const mergeBaseModified = delta.modified.find(
							(ref) => ref.id === this._currentHistoryItemBaseRef?.id,
						);
		...
		if (this._HEAD?.ahead !== this.repository.HEAD?.ahead) {
			commands.executeCommand(
				"setContext",
				"git.currentHistoryItemIsAhead",
				(this.repository.HEAD?.ahead ?? 0) > 0,
			);
		}
		if (this._HEAD?.behind !== this.repository.HEAD?.behind) {
			commands.executeCommand(
				"setContext",
				"git.currentHistoryItemIsBehind",
				(this.repository.HEAD?.behind ?? 0) > 0,
			);
		}
		...
		// Auto-fetch
		const silent =
			result.operation.kind === OperationKind.Fetch &&
			result.operation.showProgress === false;
	...
	async provideHistoryItems(
		...
		try {
			...
			const commits =
				typeof options.filterText === "string" && options.filterText !== ""
					? await this._searchHistoryItems(
							options.filterText.trim(),
							logOptions,
							token,
						)
			...
			const commitAvatars = await provideSourceControlHistoryItemAvatar(
				this.historyItemDetailProviderRegistry,
				this.repository,
				avatarQuery,
			);
			...
			for (const commit of commits) {
				...
				const tooltip = getHistoryItemHover(
					avatarUrl,
					commit.authorName,
					commit.authorEmail,
			...
			this.logger.error(
				`[GitHistoryProvider][provideHistoryItems] Failed to get history items with options '${JSON.stringify(options)}': ${err}`,
			);
			return [];
		}
	}
	...
	async resolveHistoryItem(
		historyItemId: string,
		token: CancellationToken,
	): Promise<SourceControlHistoryItem | undefined> {
		try {
			...
			this.logger.error(
				`[GitHistoryProvider][resolveHistoryItem] Failed to resolve history item '${historyItemId}': ${err}`,
			);
			return undefined;
		}
	}
	...
	async resolveHistoryItemChatContext(
		...
		try {
			...
			this.logger.error(
				`[GitHistoryProvider][resolveHistoryItemChatContext] Failed to resolve history item '${historyItemId}': ${err}`,
			);
		}
	...
	async resolveHistoryItemChangeRangeChatContext(
		...
		try {
			const changes = await this.repository.showChangesBetween(
				historyItemParentId,
				historyItemId,
				path,
			);
			...
			this.logger.error(
				`[GitHistoryProvider][resolveHistoryItemChangeRangeChatContext] Failed to resolve history item change range '${historyItemId}' for '${path}': ${err}`,
			);
		}
	...
	async resolveHistoryItemRefsCommonAncestor(
		...
		try {
			if (historyItemRefs.length === 0) {
				...
				historyItemRefs.length === 1 &&
				historyItemRefs[0] === this.currentHistoryItemRef?.id
			) {
				// Remote
				if (this.currentHistoryItemRemoteRef) {
					const ancestor = await this.repository.getMergeBase(
						historyItemRefs[0],
						this.currentHistoryItemRemoteRef.id,
					);
					return ancestor;
				}
				...
				if (this.currentHistoryItemBaseRef) {
					const ancestor = await this.repository.getMergeBase(
						historyItemRefs[0],
						this.currentHistoryItemBaseRef.id,
					);
					return ancestor;
				}
	...
	private async resolveHEADMergeBase(): Promise<Branch | undefined> {
		try {
			...
			this.logger.error(
				`[GitHistoryProvider][resolveHEADMergeBase] Failed to resolve merge base for ${this.repository.HEAD?.name}: ${err}`,
			);
			return undefined;
		}
	}

	private async _searchHistoryItems(
		filterText: string,
		options: LogOptions,
		token: CancellationToken,
	): Promise<Commit[]> {
		...
		const [authorResults, grepResults] = await Promise.all([
			this.repository.log(
				{ ...options, refNames: undefined, author: filterText, silent: true },
				token,
			),
			this.repository.log(
				{ ...options, refNames: undefined, grep: filterText, silent: true },
				token,
			),
		]);
			...
			case RefType.RemoteHead:
				return {
					...
					description: ref.commit
						? l10n.t(
								"Remote branch at {0}",
								truncate(ref.commit, this.commitShortHashLength, false),
							)
						: undefined,
					revision: ref.commit,
					icon: new ThemeIcon("cloud"),
