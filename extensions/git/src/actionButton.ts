...
function isActionButtonStateEqual(
	...
	return (
		...
		state1.repositoryHasChangesToCommit ===
			state2.repositoryHasChangesToCommit &&
		state1.repositoryHasUnresolvedConflicts ===
			state2.repositoryHasUnresolvedConflicts
	);
}
...
export class ActionButton {
	...
	constructor(
		...
		this.disposables.push(
			repository.onDidChangeBranchProtection(() => this._onDidChange.fire()),
		);
		this.disposables.push(
		...
		this.disposables.push(
			workspace.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration("git.enableSmartCommit", root) ||
	...
	get button(): SourceControlActionButton | undefined {
		...
		actionButton =
			actionButton ??
			this.getPublishBranchActionButton() ??
			this.getSyncChangesActionButton() ??
			this.getCommitActionButton();

		this.logger.trace(
			`[ActionButton][getButton] ${JSON.stringify({
			...
			enabled:
				(this.state.repositoryHasChangesToCommit ||
					(this.state.isRebaseInProgress &&
						!this.state.repositoryHasUnresolvedConflicts) ||
	...
	private getCommitActionButtonPrimaryCommand(): Command {
		...
		if (this.state.isMergeInProgress) {
			return {
				...
				title: l10n.t("{0} Continue", "$(check)"),
				tooltip: this.state.isCommitInProgress
					? l10n.t("Continuing Merge...")
					: l10n.t("Continue Merge"),
				arguments: [this.repository.sourceControl, null],
			};
		}
	...
	private getPublishBranchActionButton():
		...
		if (
			this.state.HEAD?.type === RefType.Tag ||
			!this.state.HEAD?.name ||
			this.state.HEAD?.upstream ||
			this.state.isCommitInProgress ||
			this.state.isMergeInProgress ||
			this.state.isRebaseInProgress ||
			!showActionButton.publish
		) {
			return undefined;
		}

		// Button icon
		const icon = this.state.isSyncInProgress
			? "$(sync~spin)"
			: "$(cloud-upload)";
	...
	private getSyncChangesActionButton(): SourceControlActionButton | undefined {
		...
		const showActionButton = config.get<{ sync: boolean }>("showActionButton", {
			sync: true,
		});
		...
		const ahead = this.state.HEAD.ahead
			...
			: "";
		const behind = this.state.HEAD.behind
			? ` ${this.state.HEAD.behind}$(arrow-down)`
			: "";
	...
	private onDidRunGitStatus(): void {
		this.state = {
			...
			repositoryHasChangesToCommit: this.repositoryHasChangesToCommit(),
			repositoryHasUnresolvedConflicts:
