...
async function createModel(
...
): Promise<{ model: Model; cloneManager: CloneManager }> {
	...
	const askpassPaths = await getAskpassPaths(
		__dirname,
...
export async function _activate(
	...
	if (!enabled) {
		...
		const onEnabled = filterEvent(
			onConfigChange,
			() =>
				workspace.getConfiguration("git", null).get<boolean>("enabled") ===
				true,
		);
		const result = new GitExtensionImpl();

		eventToPromise(onEnabled).then(async () => {
			const { model, cloneManager } = await createModel(
				context,
				logger,
				telemetryReporter,
				disposables,
