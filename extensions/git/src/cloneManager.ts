...
export class CloneManager {
	...
	async clone(url?: string, options: CloneOptions = {}) {
		...
		if (cachedRepository && cachedRepository.length > 0) {
			return this.tryOpenExistingRepository(
				cachedRepository,
				url,
				options.postCloneAction,
				options.parentPath,
				options.ref,
			);
		}
	...
	private async doPostCloneAction(
		...
		if (postCloneAction && postCloneAction === "none") {
			...
			if (openAfterClone === "always") {
				...
				openAfterClone === "whenNoFolderOpen" &&
	...
	private async chooseExistingRepository(
		url: string,
		existingCachedRepositories: RepositoryCacheInfo[],
		...
		try {
			const items: {
				label: string;
				description?: string;
				item?: RepositoryCacheInfo;
			}[] = existingCachedRepositories.map((knownFolder) => {
				const isWorkspace =
					knownFolder.workspacePath.endsWith(".code-workspace");
				const label = isWorkspace
					? l10n.t(
							"Workspace: {0}",
							path.basename(knownFolder.workspacePath, ".code-workspace"),
						)
					: path.basename(knownFolder.workspacePath);
				return {
					label,
					description: knownFolder.workspacePath,
					item: knownFolder,
				};
			});
	...
	private async tryOpenExistingRepository(
		...
		const existingCachedRepositories: RepositoryCacheInfo[] = (
			await Promise.all<RepositoryCacheInfo | undefined>(
				cachedRepository.map(async (folder) => {
					const stat = await fs.promises
						.stat(folder.workspacePath)
						.catch(() => undefined);
					if (stat) {
						return folder;
					}
		...
		).filter<RepositoryCacheInfo>(
			(folder): folder is RepositoryCacheInfo => folder !== undefined,
		);
		...
		const matchingInCurrentWorkspace = existingCachedRepositories?.find(
			(cachedRepo) => {
		...
		let repoForWorkspace: string | undefined =
			existingCachedRepositories.length === 1
				? existingCachedRepositories[0].workspacePath
				: undefined;
		if (!repoForWorkspace) {
			repoForWorkspace = await this.chooseExistingRepository(
				url,
				existingCachedRepositories,
				ref,
				parentPath,
				postCloneAction,
			);
