...
export class GitPostCommitCommandsProvider implements PostCommitCommandsProvider {
	...
	getCommands(apiRepository: ApiRepository): Command[] {
		const repository = this._repositoryResolver.getRepository(
			apiRepository.rootUri,
		);
		if (!repository) {
			return [];
		}
		...
		const alwaysPrompt =
			isBranchProtected && branchProtectionPrompt === "alwaysPrompt";
		const alwaysCommitToNewBranch =
			isBranchProtected && branchProtectionPrompt === "alwaysCommitToNewBranch";

		// Icon
		const isCommitInProgress =
			repository.operations.isRunning(OperationKind.Commit) ||
			repository.operations.isRunning(OperationKind.PostCommitCommand);
...
export class CommitCommandsCenter {
	...
	constructor(
		...
		this.migratePostCommitCommandStorage().then(() => {
	...
	getPrimaryCommand(): Command {
		...
		return (
			commandFromStorage ?? commandFromSetting ?? this.getCommitCommands()[0]
		);
	}
	...
	private getCommitCommands(): Command[] {
		...
		return [
			{
				command: "git.commit",
				title: l10n.t("{0} Commit", icon ?? "$(check)"),
				tooltip,
				arguments: [this.repository.sourceControl, null],
			},
			{
				command: "git.commitAmend",
				title: l10n.t("{0} Commit (Amend)", icon ?? "$(check)"),
				tooltip,
				arguments: [this.repository.sourceControl, null],
			},
		];
	...
	private getPostCommitCommandStringFromSetting(): string | undefined {
		...
		return postCommitCommandSetting === "push" ||
			postCommitCommandSetting === "sync"
