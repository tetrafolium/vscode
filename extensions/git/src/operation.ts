...
export type Operation =
	...
	| DiffOperation
	| FetchOperation
	| FindTrackingBranchesOperation
	| GetBranchOperation
	| GetBranchesOperation
	| GetCommitTemplateOperation
	| GetObjectDetailsOperation
	| GetObjectFilesOperation
	| GetRefsOperation
	...
	| ResetOperation
	| RebaseOperation
	| RebaseAbortOperation
	| RebaseContinueOperation
	| RefreshOperation
	| RestoreOperation
	| RevertFilesOperation
	| RevListOperation
	| RevParseOperation
	| SetBranchUpstreamOperation
	| ShowOperation
	| StageOperation
	| StatusOperation
	| StashOperation
...
export type DeleteRemoteRefOperation = BaseOperation & {
	kind: OperationKind.DeleteRemoteRef;
};
...
export type WorktreeOperation = BaseOperation & {
	kind: OperationKind.Worktree;
};

export const Operation = {
	Add: (showProgress: boolean): AddOperation => ({
	...
	Blame: (showProgress: boolean) =>
		({
			kind: OperationKind.Blame,
			blocking: false,
			readOnly: true,
			remote: false,
			retry: false,
	...
	CherryPick: {
		...
		retry: false,
		showProgress: true,
	} as CherryPickOperation,
	Checkout: (refLabel: string) =>
		({
			kind: OperationKind.Checkout,
			blocking: true,
			readOnly: false,
	...
	CheckoutTracking: (refLabel: string) =>
		({
			kind: OperationKind.CheckoutTracking,
			blocking: true,
			readOnly: false,
	...
	FindTrackingBranches: {
		...
		retry: false,
		showProgress: true,
	} as FindTrackingBranchesOperation,
	GetBranch: {
		kind: OperationKind.GetBranch,
		blocking: false,
		readOnly: true,
	...
	GetObjectFiles: {
		...
		retry: false,
		showProgress: false,
	} as GetObjectFilesOperation,
	GetRefs: {
		kind: OperationKind.GetRefs,
		blocking: false,
		readOnly: true,
		remote: false,
		retry: false,
	...
	HashObject: {
		...
		retry: false,
		showProgress: true,
	} as HashObjectOperation,
	Ignore: {
		kind: OperationKind.Ignore,
		blocking: false,
		readOnly: false,
		remote: false,
		retry: false,
	...
	RevertFiles: (showProgress: boolean) =>
		({
			kind: OperationKind.RevertFiles,
			blocking: false,
			readOnly: false,
			...
			retry: false,
			showProgress,
		}) as RevertFilesOperation,
	RevList: {
		kind: OperationKind.RevList,
		blocking: false,
		readOnly: true,
		remote: false,
		retry: false,
	...
	SetBranchUpstream: {
		...
		retry: false,
		showProgress: true,
	} as SetBranchUpstreamOperation,
	Show: {
		kind: OperationKind.Show,
		blocking: false,
		readOnly: true,
		remote: false,
		retry: false,
...
export class OperationManager implements IOperationManager {
	...
	start(operation: Operation): void {
		...
		this.logger.trace(
			`[OperationManager][start] ${operation.kind} (blocking: ${operation.blocking}, readOnly: ${operation.readOnly}; retry: ${operation.retry}; showProgress: ${operation.showProgress})`,
		);
	}
		...
		this.logger.trace(
			`[OperationManager][end] ${operation.kind} (blocking: ${operation.blocking}, readOnly: ${operation.readOnly}; retry: ${operation.retry}; showProgress: ${operation.showProgress})`,
		);
	}
