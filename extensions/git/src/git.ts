...
function getGitErrorCode(stderr: string): string | undefined {
	if (
		...
		/contains modified or untracked files|use --force to delete it/.test(stderr)
	) {
...
export class Git {
	...
	async getRepositoryDotGit(repositoryPath: string): Promise<IDotGit> {
		let dotGitPath: string | undefined,
			commonDotGitPath: string | undefined,
			superProjectPath: string | undefined;
		...
		[dotGitPath, commonDotGitPath, superProjectPath] = result.stdout
			.split("\n")
			.map((r) => r.trim());
		...
		const raw = await fs.readFile(
			path.join(commonDotGitPath ?? dotGitPath, "config"),
			"utf8",
		);
		const coreSections = GitConfigParser.parse(raw).find(
			(s) => s.name === "core",
		);
		...
		return {
			...
			superProjectPath: superProjectPath
				? path.normalize(superProjectPath)
				: undefined,
		};
	}
...
export class Repository {
	...
	async apply(
		patch: string,
		options?: { reverse?: boolean; threeWay?: boolean; allowEmpty?: boolean },
	): Promise<void> {
	...
	async diffBetweenWithStats(
		ref: string,
		options: { path?: string; similarityThreshold?: number },
	): Promise<DiffChange[]> {
	...
	private async diffFiles(
		...
		options: { cached: boolean; similarityThreshold?: number },
	): Promise<Change[]> {
	...
	private async diffFilesShortStat(
		ref: string | undefined,
		options: { cached: boolean; path?: string },
	): Promise<CommitShortStat> {
	...
	private async getWorktreesFS(): Promise<Worktree[]> {
		...
		try {
			if (!this.dotGit.isBare) {
				...
				const mainRepositoryWorktreeName = path.basename(
					path.dirname(mainRepositoryPath),
				);
