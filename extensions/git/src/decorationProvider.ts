...
function equalSourceControlHistoryItemRefs(
...
class GitIgnoreDecorationProvider implements FileDecorationProvider {
	...
	constructor(private model: Model) {
		...
		this.disposables.push(
			onDidChangeRepository(() => this._onDidChangeDecorations.fire(undefined)),
		);
...
class GitIncomingChangesFileDecorationProvider implements FileDecorationProvider {
	...
	constructor(private readonly repository: Repository) {
		this.disposables.push(
			window.registerFileDecorationProvider(this),
			runAndSubscribeEvent(
				repository.historyProvider.onDidChangeCurrentHistoryItemRefs,
				() => this.onDidChangeCurrentHistoryItemRefs(),
			),
		);
	}
	...
	private async onDidChangeCurrentHistoryItemRefs(): Promise<void> {
		...
		const currentHistoryItemRemoteRef =
		...
		const uris = new Set(
			[...this._decorations.keys()].concat([...decorations.keys()]),
		);
		...
		try {
			...
			const ancestor =
				await historyProvider.resolveHistoryItemRefsCommonAncestor([
					currentHistoryItemRef.id,
					currentHistoryItemRemoteRef.id,
				]);
			if (!ancestor) {
				return [];
			}
