...
async function assertRanges(
	...
	try {
		const actualRanges = await getSelectionRanges(languageModes, document, [
...
suite("HTML SelectionRange", () => {
	test("Embedded JavaScript", async () => {
		await assertRanges(
			"<html><head><script>  function foo() { return ((1|+2)*6) }</script></head></html>",
			[
				[48, "1"],
				[48, "1+2"],
				[47, "(1+2)"],
				...
				[
					6,
					"<head><script>  function foo() { return ((1+2)*6) }</script></head>",
				],
				[
					0,
					"<html><head><script>  function foo() { return ((1+2)*6) }</script></head></html>",
	...
	test("Embedded CSS", async () => {
		await assertRanges(
			"<html><head><style>foo { display: |none; } </style></head></html>",
			[
				[34, "none"],
				[25, "display: none"],
				[24, " display: none; "],
