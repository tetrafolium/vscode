...
} from "./javascriptSemanticTokens.js";
...
function getLanguageServiceHost(scriptKind: ts.ScriptKind) {
	...
	const jsLanguageService = import(
		/* webpackChunkName: "javascriptLibs" */ "./javascriptLibs.js"
	).then((libs) => {
...
export function getJavaScriptMode(
	...
	return {
					...
					await Promise.all(
						definition.map(async (d) => {
							if (d.fileName === jsDocument.uri) {
								...
								const libDocument = TextDocument.create(
									libUri,
									languageId,
									1,
									content,
		...
		async getSemanticTokens(
			...
			const jsLanguageService = await host.getLanguageService(jsDocument);
			return [
				...getSemanticTokens(jsLanguageService, jsDocument, jsDocument.uri),
			];
		},
		getSemanticTokenLegend(): { types: string[]; modifiers: string[] } {
			return getSemanticTokenLegend();
		},
		async getTextDocumentContent(
