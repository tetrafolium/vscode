...
async function createCommandDescriptionsCache(): Promise<void> {
	...
	> = new Map();
	let output = "";
	const zshVersionOutput = await execAsync("zsh --version").then(
		(r) => r.stdout,
	);
	...
	if (zshVersion < latestZshVersion) {
		console.error(
			`\x1b[31mZsh version must be ${latestZshVersion} or higher\x1b[0m`,
		);
		process.exit(1);
	}
	try {
	...
	for (const [command, lines] of commands) {
		...
		const args = (
			await execAsync(
				`pandoc --from markdown --to plain <<< "${formattedArgs}"`,
			)
		).stdout.trim();
...
const main = async () => {
	try {
		...
		for (const [
			command,
			entry,
		] of zshBuiltinsCommandDescriptionsCache.entries()) {
