		...
		test("cache should include executables found via symbolic links", async () => {
			...
			const realDoc =
				typeof realDocRaw === "string"
					? realDocRaw
					: realDocRaw && Object.hasOwn(realDocRaw, "value")
						? realDocRaw.value
						: undefined;
			const symlinkDoc =
				typeof symlinkDocRaw === "string"
					? symlinkDocRaw
					: symlinkDocRaw && Object.hasOwn(symlinkDocRaw, "value")
						? symlinkDocRaw.value
						: undefined;
		...
		suite("WindowsExecutableExtensionsCache", () => {
			test("returns default extensions when not configured", () => {
				...
				for (const ext of windowsDefaultExecutableExtensions) {
					strictEqual(
						extensions.has(ext),
						true,
						`expected default extension ${ext}`,
					);
				}
			});
			...
			test("recomputes only after update is called", () => {
				...
				strictEqual(
					third === first,
					false,
					"expected cache to recompute after update",
				);
			});
		});
	}
