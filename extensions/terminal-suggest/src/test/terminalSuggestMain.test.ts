		...
		suite(suiteSpec.name, () => {
			...
			for (const testSpec of suiteSpec.testSpecs) {
				...
				if (testSpec.expectedResourceRequests) {
					...
					if (
						testSpec.expectedResourceRequests.cwd &&
				...
				test(`'${testSpec.input}' -> ${expectedString}`, async () => {
					...
					strictEqual(
						result.showFiles,
						showFiles,
						"Show files different than expected, got: " + result.showFiles,
					);
					strictEqual(
						result.showDirectories,
						showDirectories,
						"Show directories different than expected, got: " +
							result.showDirectories,
					);
