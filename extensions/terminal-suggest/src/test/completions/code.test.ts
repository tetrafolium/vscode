...
export function createCodeTestSpecs(executable: string): ITestSpec[] {
		...
		const expectedCompletions = [
			{
				label: executable,
				description:
					executable === codeCompletionSpec.name
						? (codeCompletionSpec as Fig.Subcommand).description
						: (codeInsidersCompletionSpec as Fig.Subcommand).description,
			},
		];
	...
	return [
		...
		{
			input: `${executable} --update-extensions |`,
			expectedCompletions: codeSpecOptionsAndSubcommands.filter(
				(c) => c !== "--update-extensions",
			),
			expectedResourceRequests: { type: "both", cwd: testPaths.cwd },
		},
		// Test for --disable-lcd-text
		{
			input: `${executable} --disable-lcd-text |`,
			expectedCompletions: codeSpecOptionsAndSubcommands.filter(
				(c) => c !== "--disable-lcd-text",
			),
			expectedResourceRequests: { type: "both", cwd: testPaths.cwd },
		},
		// Test for --disable-chromium-sandbox
		{
			input: `${executable} --disable-chromium-sandbox |`,
			expectedCompletions: codeSpecOptionsAndSubcommands.filter(
				(c) => c !== "--disable-chromium-sandbox",
			),
			expectedResourceRequests: { type: "both", cwd: testPaths.cwd },
		},
		// Test for --enable-proposed-api variadic
		{
			input: `${executable} --enable-proposed-api |`,
		...
		{
			input: `${executable} --disable-extension |`,
			expectedCompletions: [executable],
		},
		{ input: `${executable} --log |`, expectedCompletions: logOptions },
		...
		{
			input: `${executable} chat --mode |`,
			expectedCompletions: ["agent", "ask", "edit"],
		},
		{
			input: `${executable} chat --add-file |`,
			expectedResourceRequests: { type: "files", cwd: testPaths.cwd },
		},
...
export function createCodeTunnelTestSpecs(executable: string): ITestSpec[] {
		...
		const expectedCompletions = [
			{
				...
				description:
					executable === codeCompletionSpec.name ||
					executable === codeTunnelCompletionSpec.name
						? (codeCompletionSpec as Fig.Subcommand).description
						: (codeInsidersCompletionSpec as Fig.Subcommand).description,
			},
		];
	...
	return [
		...
		{
			input: `${executable} |`,
			expectedCompletions: subcommandAndFlags,
			expectedResourceRequests: { type: "both", cwd: testPaths.cwd },
		},
		{
			input: `${executable} tunnel |`,
			expectedCompletions: tunnelSubcommandsAndFlags,
		},
		{
			input: `${executable} tunnel user |`,
			expectedCompletions: ["help", "login", "logout", "show"],
		},
		{
			input: `${executable} tunnel prune |`,
			expectedCompletions: [...commonFlags],
		},
		{
			input: `${executable} tunnel kill |`,
			expectedCompletions: [...commonFlags],
		},
		{
			input: `${executable} tunnel restart |`,
			expectedCompletions: [...commonFlags],
		},
		{
			input: `${executable} tunnel status |`,
			expectedCompletions: [...commonFlags],
		},
		{
			input: `${executable} tunnel rename |`,
			expectedCompletions: [...commonFlags],
		},
		{
			input: `${executable} tunnel unregister |`,
			expectedCompletions: [...commonFlags],
		},
		{
			input: `${executable} tunnel service |`,
			expectedCompletions: [
				...commonFlags,
				"help",
				"install",
		...
		{
			input: `${executable} tunnel help |`,
			expectedCompletions: helpSubcommands,
		},
		{
		...
		{
			input: `${executable} serve-web |`,
			expectedCompletions: serveWebSubcommandsAndFlags,
		},
		{ input: `${executable} ext |`, expectedCompletions: extSubcommands },
		{
			input: `${executable} ext list |`,
			expectedCompletions: [
				...commonFlags,
				"--category [<category>]",
				"--show-versions",
