	...
	implements vscode.CompletionItemProvider
{
	...
	private provideCompletionItemsInternal(
		...
		let validateLocation =
			syntax === "html" || syntax === "jsx" || syntax === "xml";
		...
		if (syntax === "html") {
			...
			if (validateLocation) {
				...
				const foundNode = getHtmlFlatNode(
					document.getText(),
					emmetRootNode,
					positionOffset,
					false,
				...
				if (foundNode) {
					if (foundNode.name === "script") {
						...
						if (typeNode) {
							...
							if (
								typeAttrValue === "application/javascript" ||
						...
						if (
							styleNode &&
							styleNode.value.start <= positionOffset &&
