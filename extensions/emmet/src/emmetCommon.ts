...
export function activateEmmetExtension(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("editor.emmet.action.balanceOut", () => {
			balanceOut();
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("editor.emmet.action.splitJoinTag", () => {
			return splitJoinTag();
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("editor.emmet.action.mergeLines", () => {
			mergeLines();
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("editor.emmet.action.toggleComment", () => {
			toggleComment();
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("editor.emmet.action.nextEditPoint", () => {
			fetchEditPoint("next");
		}),
	);

	context.subscriptions.push(
		vscode.commands.registerCommand("editor.emmet.action.prevEditPoint", () => {
			fetchEditPoint("prev");
		}),
	);
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration((e) => {
			if (
				e.affectsConfiguration("emmet.includeLanguages") ||
				e.affectsConfiguration("emmet.useInlineCompletions")
			) {
				refreshCompletionProviders(context);
			}
	...
	context.subscriptions.push(
		vscode.workspace.onDidSaveTextDocument((e) => {
			...
			if (
				basefileName.startsWith("snippets") &&
	...
	context.subscriptions.push(
		vscode.workspace.onDidOpenTextDocument((e) => {
			...
			if (
				syntaxes.markup.includes(emmetMode) ||
				syntaxes.stylesheet.includes(emmetMode)
			) {
 ...
 */
const languageMappingForCompletionProviders: Map<string, string> = new Map<
	string,
	string
>();
...
function refreshCompletionProviders(_: vscode.ExtensionContext) {
	...
	const useInlineCompletionProvider = vscode.workspace
		.getConfiguration("emmet")
		.get<boolean>("useInlineCompletions");
	const includedLanguages = getMappingForIncludedLanguages();
	Object.keys(includedLanguages).forEach((language) => {
		if (
			languageMappingForCompletionProviders.has(language) &&
			languageMappingForCompletionProviders.get(language) ===
