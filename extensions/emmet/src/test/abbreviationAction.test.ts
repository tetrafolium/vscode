...
suite("Tests for Expand Abbreviations (HTML)", () => {
	...
	test("Expand snippets in completion list (HTML)", () => {
		return testHtmlCompletionProvider(
			new Selection(3, 23, 3, 23),
			"img",
			'<img src=\"\" alt=\"\">',
		);
	...
	test("Expand snippets when no parent node in completion list (HTML)", () => {
		return withRandomFileEditor("img", "html", async (editor, _doc) => {
			...
			if (completionList) {
				...
				assert.strictEqual(
					(<string>completionList.items[0].documentation || "").replace(
						/\|/g,
						"",
					),
					'<img src=\"\" alt=\"\">',
	...
	test("Expand css when inside style tag (HTML)", () => {
		return withRandomFileEditor(htmlContents, "html", async (editor, _doc) => {
			...
			assert.strictEqual(
				...
				htmlContents.replace("m10", "margin: 10px;"),
			);
			return Promise.resolve();
		});
	});
	...
	test("Expand css when inside style attribute (HTML)", () => {
		...
		return withRandomFileEditor(
			...
			async (editor, _doc) => {
				...
				assert.strictEqual(
					editor.document.getText(),
					styleAttributeContent.replace("m10", "margin: 10px;"),
				);
				return Promise.resolve();
			},
		);
	...
	test("Expand html when inside script tag with html type (HTML)", () => {
		return withRandomFileEditor(htmlContents, "html", async (editor, _doc) => {
			...
			assert.strictEqual(
				editor.document.getText(),
				htmlContents.replace("span.hello", '<span class="hello"></span>'),
			);
			return Promise.resolve();
		});
	});
	...
	test("Expand html when inside script tag with javascript type if js is mapped to html (HTML)", async () => {
		const oldConfig = workspace
			...
			.inspect("includeLanguages")?.globalValue;
		await workspace
			.getConfiguration("emmet")
			.update(
				"includeLanguages",
		...
		await withRandomFileEditor(htmlContents, "html", async (editor, _doc) => {
			...
			assert.strictEqual(
				editor.document.getText(),
				htmlContents.replace("span.bye", '<span class="bye"></span>'),
			);
		});
		await workspace
			.getConfiguration("emmet")
			.update("includeLanguages", oldConfig, ConfigurationTarget.Global);
	});
	...
	test("Expand html in completion list when inside script tag with javascript type if js is mapped to html (HTML)", async () => {
		...
		await withRandomFileEditor(htmlContents, "html", async (editor, _doc) => {
			...
			assert.strictEqual(
				emmetCompletionItem.label,
				abbreviation,
				`Label of completion item (${emmetCompletionItem.label}) doesnt match.`,
			);
			assert.strictEqual(
				(<string>emmetCompletionItem.documentation || "").replace(/\|/g, ""),
				expandedText,
	...
	test("No expanding when html is excluded in the settings in completion list", async () => {
		const oldConfig = workspace
			.getConfiguration("emmet")
			.inspect("excludeLanguages")?.globalValue;
		await workspace
			.getConfiguration("emmet")
			.update("excludeLanguages", ["html"], ConfigurationTarget.Global);
		await testHtmlCompletionProvider(new Selection(9, 6, 9, 6), "", "", true);
		await workspace
			.getConfiguration("emmet")
			.update("excludeLanguages", oldConfig, ConfigurationTarget.Global);
	});
...
suite("Tests for jsx, xml and xsl", () => {
	const oldValueForSyntaxProfiles = workspace
		.getConfiguration("emmet")
		.inspect("syntaxProfiles");
	teardown(closeAllEditors);
				...
				return workspace
					...
					.update(
						...
						oldValueForSyntaxProfiles
							? oldValueForSyntaxProfiles.globalValue
							: undefined,
						ConfigurationTarget.Global,
					);
			},
...
function testExpandAbbreviation(
	syntax: string,
	selection: Selection,
	abbreviation: string,
	expandedText: string,
	...
	return withRandomFileEditor(htmlContents, syntax, async (editor, _doc) => {
		...
		if (!expandPromise) {
			if (!shouldFail) {
				assert.strictEqual(
					1,
					2,
					`Problem with expanding ${abbreviation} to ${expandedText}`,
				);
			}
			return Promise.resolve();
		}
		...
		assert.strictEqual(
			editor.document.getText(),
			htmlContents.replace(abbreviation, expandedText),
		);
		return Promise.resolve();
	});
}

function testHtmlCompletionProvider(
	selection: Selection,
	abbreviation: string,
	expandedText: string,
	shouldFail?: boolean,
