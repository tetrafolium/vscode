...
suite("Tests for completion in CSS embedded in HTML", () => {
	...
	test("style attribute & attribute value in html", async () => {
		await testCompletionProvider("html", '<div style="|"', [
			{ label: "padding: ;" },
		]);
		await testCompletionProvider("html", `<div style='|'`, [
			{ label: "padding: ;" },
		]);
		await testCompletionProvider("html", `<div style='p|'`, [
			{ label: "padding: ;" },
		]);
		await testCompletionProvider("html", `<div style='color: #0|'`, [
			{ label: "#000000" },
		]);
	});
	...
	test("microsoft/vscode#86941, widows should be completed after width", async () => {
		...
		try {
			...
			assert.strictEqual(
				e.message,
				`Didn't find completion item with label widows: ;`,
			);
		}
		try {
			await testCompletionProvider("css", `.foo { wid| }`, [
	...
	test("microsoft/vscode#117020, ! at end of abbreviation should have completion", async () => {
		await testCompletionProvider("css", `.foo { bdbn!| }`, [
			{
				label: "border-bottom: none !important;",
				documentation: `border-bottom: none !important;`,
			},
		]);
	});
...
function testCompletionProvider(
	...
	contents: string,
	expectedItems: TestCompletionItem[] | undefined,
): Thenable<boolean> {
	const cursorPos = contents.indexOf("|");
	const slicedContents =
		contents.slice(0, cursorPos) + contents.slice(cursorPos + 1);
	...
	return withRandomFileEditor(
		...
		async (editor, _doc) => {
			...
			expectedItems!.forEach((eItem) => {
				const matches = completionList.items.filter(
					(i) => i.label === eItem.label,
				);
				const match = matches && matches.length > 0 ? matches[0] : undefined;
				assert.ok(
					match,
					`Didn't find completion item with label ${eItem.label}`,
				);
				...
				if (match) {
					assert.strictEqual(
						match.detail,
						"Emmet Abbreviation",
						`Match needs to come from Emmet`,
					);
					...
					if (eItem.documentation) {
						assert.strictEqual(
							match.documentation,
							eItem.documentation,
							`Emmet completion Documentation doesn't match`,
						);
					}
				}
			});
