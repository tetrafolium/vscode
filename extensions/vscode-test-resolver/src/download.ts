...
async function downloadVSCodeServerArchive(
	updateUrl: string,
	commit: string,
	quality: string,
	destDir: string,
	...
	return new Promise((resolve, reject) => {
		...
		https.get(requestOptions, (res) => {
			...
			const archivePath = path.resolve(
				destDir,
				`vscode-server-${commit}.${archiveUrl.endsWith(".zip") ? "zip" : "tgz"}`,
			);
 ...
 */
function unzipVSCodeServer(
	vscodeArchivePath: string,
	extractDir: string,
	destDir: string,
	log: (messsage: string) => void,
) {
	...
	if (vscodeArchivePath.endsWith(".zip")) {
		...
		fs.renameSync(
			path.join(
				tempDir,
				process.platform === "win32"
					? "vscode-server-win32-x64"
					: "vscode-server-darwin-x64",
			),
			extractDir,
		);
	} else {
...
export async function downloadAndUnzipVSCodeServer(
	...
	if (fs.existsSync(extractDir)) {
		...
		log(
			`Downloading VS Code Server ${quality} - ${commit} into ${extractDir}.`,
		);
		try {
			const vscodeArchivePath = await downloadVSCodeServerArchive(
				updateUrl,
				commit,
				quality,
				destDir,
			...
			throw Error(
				`Failed to download and unzip VS Code ${quality} - ${commit}`,
			);
		}
	}
