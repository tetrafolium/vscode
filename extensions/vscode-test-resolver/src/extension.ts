...
export function activate(context: vscode.ExtensionContext) {
	...
	function getTunnelFeatures(): vscode.TunnelInformation["tunnelFeatures"] {
		return {
			elevation: true,
			privacyOptions: vscode.workspace
				.getConfiguration("testresolver")
				.get("supportPublicPorts")
				? [
						{
							id: "public",
							label: "Public",
	...
	function doResolve(
		...
		const serverPromise = new Promise<vscode.ResolvedAuthority>(
			async (res, rej) => {
				...
				function processOutput(output: string) {
					...
					for (let i = 0; i < output.length; i++) {
						...
						if (chr === CharCode.LineFeed) {
							const match = lastProgressLine.match(
							...
							lastProgressLine = lastProgressLine.substr(
								0,
								lastProgressLine.length - 1,
							);
						} else {
				...
				if (typeof delay === "number") {
					...
					outputChannel.append(
						`Delaying startup by ${remaining} seconds (configured by "testresolver.startupDelay").`,
					);
					while (remaining > 0) {
						progress.report({
							message: `Delayed resolving: Remaining ${remaining}s`,
						});
						await sleep(1000);
						remaining--;
					}
				...
				const env = getNewEnv();
				const remoteDataDir =
					process.env["TESTRESOLVER_DATA_FOLDER"] ||
					path.join(
						os.homedir(),
						`${serverDataFolderName || dataFolderName}-testresolver`,
					);
				...
				if (
					typeof agentHostPort === "number" &&
					agentHostPort > 0 &&
					typeof agentHostBridgePort === "number" &&
					agentHostBridgePort > 0
				) {
				...
				if (
					...
					commandArgs.push(
						...
						String(agentHostBridgePort),
					);
				}
				const agentHostBridgeToken = getConfiguration(
				...
				if (!commit) {
					// dev mode
					const serverCommand =
						process.platform === "win32" ? "code-server.bat" : "code-server.sh";
					...
					outputChannel.appendLine(
						`Launching server: "${serverCommandPath}" ${commandArgs.join(" ")}`,
					);
					const shell = process.platform === "win32";
					// Skip prelaunch to avoid redownloading electron while it may be in use
					env["VSCODE_SKIP_PRELAUNCH"] = "1";
					extHostProcess = cp.spawn(serverCommandPath, commandArgs, {
						env,
						cwd: vscodePath,
						shell,
					});
					...
					const extensionToInstall =
						process.env["TESTRESOLVER_INSTALL_BUILTIN_EXTENSION"];
					...
					if (!serverLocation) {
						...
						serverLocation = await downloadAndUnzipVSCodeServer(
							updateUrl,
							commit,
							quality,
							serverBin,
					...
					const shell = process.platform === "win32";
					extHostProcess = cp.spawn(
						path.join(serverLocation, "bin", serverCommand),
						commandArgs,
						{ env, cwd: serverLocation, shell },
					);
				}
		...
		return serverPromise.then((serverAddr): Promise<vscode.ResolverResult> => {
			if (authority.includes("managed")) {
				...
				return Promise.resolve(
					new vscode.ManagedResolvedAuthority(async () => {
						...
						await new Promise((res, rej) => {
							remoteSocket
								.on("data", (d) =>
									dataEmitter.fire(d as Uint8Array<ArrayBuffer>),
								)
								.on("error", (err) => {
									rej();
				...
				const proxyServer = net.createServer((proxySocket) => {
					...
					const handleConnectionPause = () => {
						...
						if (isDisconnected !== newIsDisconnected) {
							outputChannel.appendLine(
								`Connection state: ${newIsDisconnected ? "open" : "paused"}`,
							);
					...
					proxySocket.on("close", () => {
						outputChannel.appendLine(
							`Proxy socket closed, closing remote socket.`,
						);
						remoteSocket.end();
					});
					remoteSocket.on("close", () => {
						outputChannel.appendLine(
							`Remote socket closed, closing proxy socket.`,
						);
						proxySocket.end();
					});
					context.subscriptions.push({
				...
				return vscode.window.withProgress(
					{
						...
						title:
							"Open TestResolver Remote ([details](command:vscode-testresolver.showLog))",
						cancellable: false,
					},
					async (progress) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("vscode-testresolver.currentWindow", () => {
			return vscode.commands.executeCommand("vscode.newWindow", {
				remoteAuthority: "test+test",
				reuseWindow: true,
			});
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"vscode-testresolver.currentWindowManaged",
			() => {
				return vscode.commands.executeCommand("vscode.newWindow", {
					remoteAuthority: "test+managed",
					reuseWindow: true,
				});
			},
		),
	...
	const pauseStatusBarEntry = vscode.window.createStatusBarItem(
	...
	pauseStatusBarEntry.backgroundColor = new vscode.ThemeColor(
		"statusBarItem.errorBackground",
	);

	context.subscriptions.push(
		vscode.commands.registerCommand(
			"vscode-testresolver.toggleConnectionPause",
			() => {
				if (!connectionPaused) {
	...
	const slowdownStatusBarEntry = vscode.window.createStatusBarItem(
		vscode.StatusBarAlignment.Left,
	);
	slowdownStatusBarEntry.text = "Remote connection slowed down. Click to undo";
	slowdownStatusBarEntry.command =
		"vscode-testresolver.toggleConnectionSlowdown";
	slowdownStatusBarEntry.backgroundColor = new vscode.ThemeColor(
		"statusBarItem.errorBackground",
	);
		...
		vscode.commands.registerCommand(
			...
			async () => {
				const result = await vscode.window.showInputBox({
					...
					value: "5000",
					validateInput: (input) =>
						/^[\d]+$/.test(input) ? undefined : "Not a valid number",
				});
				if (result) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"vscode-testresolver.startRemoteServer",
			async () => {
				const result = await vscode.window.showInputBox({
...
function getProductConfiguration(): IProductConfiguration {
	const content = fs
		.readFileSync(path.join(vscode.env.appRoot, "product.json"))
		.toString();
...
async function tunnelFactory(
	tunnelOptions: vscode.TunnelOptions,
	tunnelCreationOptions: vscode.TunnelCreationOptions,
): Promise<vscode.Tunnel> {
	outputChannel.appendLine(
		`Tunnel factory request: Remote ${tunnelOptions.remoteAddress.port} -> local ${tunnelOptions.localAddressPort}`,
	);
	if (tunnelCreationOptions.elevationRequired) {
		await vscode.window.showInformationMessage(
			"This is a fake elevation message. A real resolver would show a native elevation prompt.",
			{ modal: true },
			"Ok",
		);
	}
	...
	function newTunnel(localAddress: {
		...
		return {
			...
			public:
				!!vscode.workspace
					.getConfiguration("testresolver")
					.get("supportPublicPorts") && tunnelOptions.public,
		...
		return new Promise<vscode.Tunnel>((res, _rej) => {
			const proxyServer = net.createServer((proxySocket) => {
				const remoteSocket = net.createConnection({
					...
					port: tunnelOptions.remoteAddress.port,
				});
			...
			proxyServer.listen(localPort, "127.0.0.1", () => {
				...
				outputChannel.appendLine(
					`New test resolver tunnel service: Remote ${tunnelOptions.remoteAddress.port} -> local ${localPort}`,
				);
