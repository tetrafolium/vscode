	...
	switch (outputType) {
		...
		default: {
			const isError =
				...
				output.items.every((item) => item.mime === CellOutputMimeTypes.error);
...
export function serializeNotebookToString(data: NotebookData): string {
	...
	const indentAmount =
		data.metadata && typeof data.metadata.indentAmount === "string"
			? data.metadata.indentAmount
			: " ";

	return serializeNotebookToJSON(notebookContent, indentAmount);
}
function serializeNotebookToJSON(
...
export function getNotebookMetadata(document: NotebookDocument | NotebookData) {
	const existingContent: Partial<nbformat.INotebookContent> =
		document.metadata || {};
