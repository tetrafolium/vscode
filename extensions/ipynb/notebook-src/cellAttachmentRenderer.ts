...
export async function activate(ctx: RendererContext<void>) {
	...
	markdownItRenderer.extendMarkdownIt((md: MarkdownIt) => {
		...
		md.renderer.rules.image = (
			...
			const attachments: Record<string, Record<string, string>> | undefined =
				env.outputItem.metadata?.attachments;
			if (attachments && src && src.startsWith("attachment:")) {
				const imageAttachment =
					attachments[tryDecodeURIComponent(src.replace("attachment:", ""))];
				if (imageAttachment) {
					...
					if (objEntries.length) {
						...
						const b64Markdown =
							"data:" + attachmentKey + ";base64," + attachmentVal;
