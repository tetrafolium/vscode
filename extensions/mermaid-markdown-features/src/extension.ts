...
export function activate(context: vscode.ExtensionContext) {
	...
	const editorManager = new MermaidEditorManager(
	...
	context.subscriptions.push(
		registerChatSupport(context, webviewManager, editorManager),
	);

	// Register commands
	context.subscriptions.push(
		vscode.commands.registerCommand(
			"_mermaid-markdown.resetPanZoom",
			(ctx?: { mermaidWebviewId?: string }) => {
			...
			extendMarkdownItWithMermaid(md, {
				languageIds: () =>
					vscode.workspace
						...
						.get<readonly string[]>("languages", ["mermaid"]),
			});
