...
import {
	...
	createSchemaLoadIssueItem,
	createSchemaLoadStatusItem,
} from "./languageStatus";
import {
...
export namespace CommandIds {
	...
	export const configureTrustedDomainsCommandId =
		"_json.configureTrustedDomains";
...
async function startClientWithParticipants(
	...
	let schemaAssociationsCache: Promise<ISchemaAssociation[]> | undefined =
		undefined;
	...
	let schemaDownloadEnabled = !!workspace
		.getConfiguration()
		.get(SettingIds.enableSchemaDownload);
	let trustedDomains = workspace
		.getConfiguration()
		.get<Record<string, boolean>>(SettingIds.trustedDomains, {});
	...
	const schemaLoadStatusItem = createSchemaLoadStatusItem(
	...
	function handleSchemaErrorDiagnostics(
		uri: Uri,
		...
		middleware: {
			...
			provideDiagnostics: async (uriOrDoc, previousResolutId, token, next) => {
				...
				if (
					...
					diagnostics.items = handleSchemaErrorDiagnostics(
						uri,
						diagnostics.items,
					);
				}
	...
	client.onRequest(VSCodeContentRequest.type, async (uriPath: string) => {
		...
		if (uri.scheme === "untitled") {
			throw new ResponseError(
				SchemaRequestServiceErrors.UntitledAccessError,
				l10n.t("Unable to load {0}", uriString),
			);
		}
		if (uri.scheme === "vscode") {
			try {
				...
				throw new ResponseError(
					SchemaRequestServiceErrors.VSCodeAccessError,
					e.toString(),
					e,
				);
			}
			...
			try {
				...
				throw new ResponseError(
					SchemaRequestServiceErrors.OpenTextDocumentAccessError,
					e.toString(),
					e,
				);
			}
			...
			if (!workspace.isTrusted) {
				throw new ResponseError(
					SchemaRequestServiceErrors.UntrustedWorkspaceError,
					l10n.t("Downloading schemas is disabled in untrusted workspaces"),
				);
			}
			if (!(await isTrusted(uri))) {
				throw new ResponseError(
					SchemaRequestServiceErrors.UntrustedSchemaError,
					l10n.t("Location {0} is untrusted", uriString),
				);
			}
			if (
				runtime.telemetry &&
			...
			try {
				...
				throw new ResponseError(
					SchemaRequestServiceErrors.HTTPError,
					e.toString(),
					e,
				);
			}
			...
			throw new ResponseError(
				SchemaRequestServiceErrors.HTTPDisabledError,
				l10n.t(
	...
	toDispose.push(
		commands.registerCommand(
			CommandIds.retryResolveSchemaCommandId,
			triggerValidation,
		),
	);
	...
	toDispose.push(
		commands.registerCommand(
			CommandIds.configureTrustedDomainsCommandId,
							...
							case ErrorCodes.UntrustedSchemaError:
								{
									...
									if (schemaUri) {
										action.command = {
											command: CommandIds.configureTrustedDomainsCommandId,
											arguments: [schemaUri.toString()],
											title,
										};
									} else {
										action.command = {
											command: CommandIds.workbenchActionOpenSettings,
											arguments: [SettingIds.trustedDomains],
											title,
										};
									}
	...
	client.sendNotification(
		...
		await getSchemaAssociations(false),
	);
	...
	toDispose.push(
		extensions.onDidChange(async (_) => {
			client.sendNotification(
				SchemaAssociationNotification.type,
				await getSchemaAssociations(true),
			);
		}),
	);
	...
	async function triggerValidation() {
		...
		if (
			activeTextEditor &&
			languageParticipants.hasLanguage(activeTextEditor.document.languageId)
		) {
	...
	async function configureTrustedDomains(schemaUri: string): Promise<void> {
		...
		const updateTrustedDomains = async (
			updateDomain: string,
		): Promise<void> => {
			const config = workspace.getConfiguration();
			const currentDomains = config.get<Record<string, boolean>>(
				SettingIds.trustedDomains,
				{},
			);
		...
		try {
			...
			items.push({
				...
				execute: async () => {
					...
					await commands.executeCommand(
						CommandIds.workbenchActionOpenSettings,
						SettingIds.trustedDomains,
					);
				},
			});
			...
			runtime.logOutputChannel.error(
				`Failed to parse schema URI: ${schemaUri}`,
			);
		}
...
export namespace ErrorCodes {
	...
	export const UntrustedSchemaError =
		SchemaResolveError + SchemaRequestServiceErrors.UntrustedSchemaError;
	export const HTTPDisabledError =
		SchemaResolveError + SchemaRequestServiceErrors.HTTPDisabledError;
}
