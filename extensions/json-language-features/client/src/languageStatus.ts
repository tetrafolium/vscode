...
export function createLanguageStatusItem(
	...
	const showSchemasCommand = commands.registerCommand(
		CommandIds.showAssociatedSchemaList,
		showSchemaList,
	);
...
export function createSchemaLoadStatusItem(
	newItem: (fileSchemaError: Diagnostic) => Disposable,
) {
	let statusItem: Disposable | undefined;
	...
	function update(uri: Uri, diagnostics: Diagnostic[]) {
		...
		if (fileSchemaError === undefined) {
			...
			if (
				statusItem &&
				uriString === window.activeTextEditor?.document.uri.toString()
			) {
				statusItem.dispose();
				statusItem = undefined;
			}
...
export function createSchemaLoadIssueItem(
	documentSelector: DocumentSelector,
	schemaDownloadEnabled: boolean | undefined,
	...
	if (!workspace.isTrusted) {
		...
		statusItem.command = {
			command: CommandIds.workbenchTrustManage,
		...
		diagnostic.code === ErrorCodes.UntrustedSchemaError
	) {
		...
		statusItem.command = {
			command: CommandIds.retryResolveSchemaCommandId,
			title: "Retry",
		};
	}
	return Disposable.from(statusItem);
