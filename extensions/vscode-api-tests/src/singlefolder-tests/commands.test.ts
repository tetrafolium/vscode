...
suite("vscode API - commands", () => {
	...
	test("api-command: vscode.diff", function () {
		...
		const c = commands
			.executeCommand(
				...
				Uri.parse("sc:b"),
				"Title",
				{ selection: new Range(new Position(1, 1), new Position(1, 2)) },
			)
			.then((value) => {
				assert.ok(value === undefined);
				registration.dispose();
		...
		const d = commands.executeCommand("vscode.diff").then(
			() => assert.ok(false),
			() => assert.ok(true),
		);
		const e = commands.executeCommand("vscode.diff", 1, 2, 3).then(
			() => assert.ok(false),
			() => assert.ok(true),
		);
	...
	test("api-command: vscode.open", async function () {
		...
		const uri = Uri.parse(
			workspace.workspaceFolders[0].uri.toString() + "/far.js",
		);

		await commands.executeCommand("vscode.open", uri);
		assert.strictEqual(window.tabGroups.all.length, 1);
		assert.strictEqual(
			window.tabGroups.all[0].activeTab?.group.viewColumn,
			ViewColumn.One,
		);
		...
		assert.strictEqual(
			window.tabGroups.all[1].activeTab?.group.viewColumn,
			ViewColumn.Two,
		);
	...
	test("api-command: vscode.open with untitled supports associated resource (#138925)", async function () {
		const uri = Uri.parse(
			workspace.workspaceFolders![0].uri.toString() + "/untitled-file.txt",
