...
suite("vscode API - types", () => {
	...
	test("static properties, es5 compat class", function () {
		...
		assert.ok(
			vscode.CodeActionKind.RefactorExtract instanceof vscode.CodeActionKind,
		);
		assert.ok(
			vscode.CodeActionKind.RefactorInline instanceof vscode.CodeActionKind,
		);
		assert.ok(
			vscode.CodeActionKind.RefactorMove instanceof vscode.CodeActionKind,
		);
		assert.ok(
			vscode.CodeActionKind.RefactorRewrite instanceof vscode.CodeActionKind,
		);
		assert.ok(vscode.CodeActionKind.Source instanceof vscode.CodeActionKind);
		assert.ok(
			vscode.CodeActionKind.SourceOrganizeImports instanceof
				vscode.CodeActionKind,
		);
		assert.ok(
			vscode.CodeActionKind.SourceFixAll instanceof vscode.CodeActionKind,
		);
