	...
	() => {
				...
				await new Promise<void>((resolve, reject) => {
					https
						.get(
							...
							{
								servername: "localhost-proxy-test",
								testCertificates: [certPEM],
		...
		test("basic auth", async () => {
			...
			try {
					...
					https
						.get(url, (res) => {
							if (res.statusCode === 418) {
								...
								reject(
									new Error(
										`Unexpected status code (expected 418): ${res.statusCode}`,
									),
								);
							}
						})
					...
					https
						.get(url, (res) => {
							if (res.statusCode === 407) {
								...
								reject(
									new Error(
										`Unexpected status code (expected 407): ${res.statusCode}`,
									),
								);
							}
						})
					...
					https
						.get(url, (res) => {
							if (res.statusCode === 204) {
								...
								reject(
									new Error(
										`Unexpected status code (expected 204): ${res.statusCode}`,
									),
								);
							}
						})
				...
				await vscode.workspace
					.getConfiguration()
					.update("http.proxy", undefined, vscode.ConfigurationTarget.Global);
				await change;
				await vscode.workspace
					.getConfiguration()
					.update(
		...
		(vscode.env.remoteName ? test : test.skip)(
			...
			async () => {
				...
				await vscode.workspace
					.getConfiguration()
					.update("http.proxy", localProxy, vscode.ConfigurationTarget.Global);
				await p1;
				const actualLocalProxy2 = vscode.workspace
					.getConfiguration()
					.get("http.proxy");
				...
				await p2;
				const actualRemoteProxy1 = vscode.workspace
					.getConfiguration()
					.get("http.proxy");
				...
				await vscode.workspace
					.getConfiguration()
					.update("http.proxy", remoteProxy, vscode.ConfigurationTarget.Global);
				await p3;
				const actualRemoteProxy2 = vscode.workspace
					.getConfiguration()
					.get("http.proxy");
				...
				await vscode.workspace
					...
					.update("http.proxy", undefined, vscode.ConfigurationTarget.Global);
				await p4;
				const actualRemoteProxy3 = vscode.workspace
					.getConfiguration()
					.get("http.proxy");
				...
				await p5;
				const actualLocalProxy3 = vscode.workspace
					.getConfiguration()
					.get("http.proxy");
				...
				await vscode.workspace
					...
					.update("http.proxy", undefined, vscode.ConfigurationTarget.Global);
				await p6;
				const actualLocalProxy4 = vscode.workspace
					.getConfiguration()
					.get("http.proxy");
