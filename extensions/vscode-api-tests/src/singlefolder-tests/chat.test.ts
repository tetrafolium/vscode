...
suite("chat", () => {
	...
	setup(() => {
		...
		disposables.push(
			lm.registerLanguageModelChatProvider("copilot", {
	...
	test.skip("workbench.action.chat.open.blockOnResponse defaults to non-blocking for backwards compatibility", async () => {
		const toolRegistration = lm.registerTool<void>(
			"requires_confirmation_tool",
			{
				invoke: async (_options, _token) => null,
				prepareInvocation: async (_options, _token) => {
					return {
						invocationMessage: "Invoking",
						pastTenseMessage: "Invoked",
		...
		const participant = chat.createChatParticipant(
			"api-test.participant",
			async (_request, _context, _progress, _token) => {
		...
		await commands.executeCommand("workbench.action.chat.newChat");
		const result = await commands.executeCommand("workbench.action.chat.open", {
			query: "hello",
		});
	...
	test.skip("workbench.action.chat.open.blockOnResponse resolves when an error is hit", async () => {
		const participant = chat.createChatParticipant(
			...
			async (_request, _context, _progress, _token) => {
				return {
					errorDetails: {
						code: "rate_limited",
						message: `You've been rate limited. Try again later!`,
					},
				};
			},
		);
		...
		assert.strictEqual(
			(<PartialChatAgentResult>result).errorDetails.code,
			"rate_limited",
		);
	});
