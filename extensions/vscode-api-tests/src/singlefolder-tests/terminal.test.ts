...
(env.uiKind === UIKind.Web ? suite.skip : suite)(
	"vscode API - terminal",
	() => {
		...
		suite("Terminal", () => {
			...
			test("creationOptions should be set and readonly for TerminalOptions terminals", async () => {
				...
				throws(
					() => (terminalOptions.name = "bad"),
					"creationOptions should be readonly at runtime",
				);
			});
			...
			suite("selection", () => {
				...
				test("should be undefined after clearing a selection", async () => {
					...
					await poll<void>(
						() => Promise.resolve(),
						() => terminal.selection === undefined,
						"selection should not be defined",
					);
					terminal.dispose();
				});
			});
			...
			suite("window.onDidWriteTerminalData", () => {
				...
				test.skip("should listen to all future terminal data events", function (done) {
					...
					disposables.push(
						window.onDidOpenTerminal((e) => openEvents.push(e.name)),
					);
						...
						pty: {
							...
							open: async () => {
								...
								await new Promise<void>((resolve) => {
									resolveOnceDataWritten = resolve;
								});
			...
			suite("Extension pty terminals", () => {
				...
				test.skip("should respect dimension overrides", async () => {
					...
					const overrideDimensionsEmitter =
						new EventEmitter<TerminalDimensions>();
					const pty: Pseudoterminal = {
					...
					// has high latency
					if (
						terminal.dimensions?.columns === 10 &&
						terminal.dimensions?.rows === 5
					) {
						return;
					}
					...
					await new Promise<void>((r) => {
						...
						disposables.push(
							window.onDidChangeTerminalDimensions((e) => {
								console.log(
									`window.onDidChangeTerminalDimensions event, dimensions = ${e.dimensions?.columns}x${e.dimensions?.rows}`,
								);
						...
						console.log(
							`listening for window.onDidChangeTerminalDimensions, current dimensions = ${terminal.dimensions?.columns}x${terminal.dimensions?.rows}`,
						);
						terminal.show();
					});
				});
				...
				test("should change terminal name", async () => {
					...
					await new Promise<void>((r) => {
						disposables.push(
							window.onDidOpenTerminal((t1) => {
								if (t1 === created) {
									disposables.push(
										window.onDidCloseTerminal((t2) => {
											if (t2 === created) {
							...
							window.onDidOpenTerminal((t1) => {
								if (t1 === created) {
									...
									disposables.push(
										window.onDidCloseTerminal((t2) => {
											if (t2 === created) {
												deepStrictEqual(created.exitStatus, {
													code: undefined,
			...
			(process.platform === "win32" ? suite.skip : suite)(
				...
				() => {
					test("should have collection variables apply to terminals immediately after setting", async () => {
						...
						try {
							await poll<void>(
								() => Promise.resolve(),
								() => data.includes("~a2~"),
								"~a2~ should be printed",
							);
							await poll<void>(
								() => Promise.resolve(),
								() => data.includes("b1~b2~"),
								"b1~b2~ should be printed",
							);
							await poll<void>(
								() => Promise.resolve(),
								() => data.includes("~c2~c1"),
								"~c2~c1 should be printed",
							);
						} catch (err) {
					...
					test("should have collection variables apply to environment variables that don't exist", async () => {
						...
						try {
							...
							await poll<void>(
								() => Promise.resolve(),
								() => data.includes("~b2~"),
								"~b2~ should be printed",
							);
							await poll<void>(
								() => Promise.resolve(),
								() => data.includes("~c2~"),
								"~c2~ should be printed",
							);
						} catch (err) {
							console.error("DATA UP UNTIL NOW:", data);
							throw err;
					...
					test("should respect clearing entries", async () => {
						...
						try {
							await poll<void>(
								() => Promise.resolve(),
								() => data.includes("~a1~"),
								"~a1~ should be printed",
							);
							await poll<void>(
								() => Promise.resolve(),
								() => data.includes("~b1~"),
								"~b1~ should be printed",
							);
						} catch (err) {
							console.error("DATA UP UNTIL NOW:", data);
							throw err;
					...
					test("get and forEach should work", () => {
						...
						deepStrictEqual(collection.get("A"), {
							value: "~a2~",
							type: EnvironmentVariableMutatorType.Replace,
							options: defaultOptions,
							variable: "A",
						});
						deepStrictEqual(collection.get("B"), {
							value: "~b2~",
							type: EnvironmentVariableMutatorType.Append,
							options: defaultOptions,
							variable: "B",
						});
						deepStrictEqual(collection.get("C"), {
							value: "~c2~",
					...
					test("get and forEach should work (scope)", () => {
						...
						deepStrictEqual(expectedScopedCollection.get("A"), {
							value: "scoped~a2~",
							type: EnvironmentVariableMutatorType.Replace,
							options: defaultOptions,
							variable: "A",
						});
						deepStrictEqual(expectedScopedCollection.get("B"), {
							value: "scoped~b2~",
							type: EnvironmentVariableMutatorType.Append,
							options: defaultOptions,
							variable: "B",
						});
						deepStrictEqual(expectedScopedCollection.get("C"), {
							value: "scoped~c2~",
							type: EnvironmentVariableMutatorType.Prepend,
							options: defaultOptions,
							variable: "C",
