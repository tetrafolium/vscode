...
(vscode.env.uiKind === vscode.UIKind.Web ? suite.skip : suite)(
	...
	() => {
		...
		setup(async () => {
			...
			const browserConfig =
				vscode.workspace.getConfiguration("workbench.browser");
			await browserConfig.update(
				"enableChatTools",
				true,
		...
		test("open_browser_page opens a browser tab and returns a page ID", async function () {
			...
			assert.match(
				output,
				/Page ID:/,
				`Expected output to contain "Page ID:", got: ${output}`,
			);
		});
		...
		test("basic browser tool interactions", async function () {
			...
			assert.match(
				openOutput,
				/Page ID:/,
				`Expected open output to contain "Page ID:", got: ${openOutput}`,
			);
			...
			assert.match(
				runOutput,
				/Result: "done"/,
				`Expected run_playwright_code output to contain result "done", got: ${runOutput}`,
			);

			// Read the page to verify the output element was populated
			const readOutput = await invokeTool("read_page", { pageId });
			assert.ok(
				readOutput.includes("test message"),
				`Expected page to contain worker response "test message", got: ${readOutput}`,
			);
		});
	},
);
