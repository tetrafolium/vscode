...
suite("vscode API - env", () => {
	...
	test("env.remoteName", function () {
		...
		const knownUiAndWorkspaceExtension = extensions.getExtension(
		...
		if (typeof remoteName === "undefined") {
			...
			assert.strictEqual(
				ExtensionKind.UI,
				knownUiAndWorkspaceExtension!.extensionKind,
			);
			...
			assert.strictEqual(
				...
				knownWorkspaceExtension!.extensionKind,
			);
		} else {
			assert.fail();
		}
	...
	test("env.uiKind", async function () {
		const uri = Uri.parse(
			`${env.uriScheme}:://vscode.vscode-api-tests/path?key=value&other=false`,
		);
