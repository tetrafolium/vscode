...
(vscode.env.uiKind === vscode.UIKind.Web ? suite.skip : suite)(
	...
	() => {
		...
		test("CDP debugging golden scenario", async function () {
			...
			const pageAttach = await cdpSend(
			...
			const evalResult = await cdpSend(
				"Runtime.evaluate",
				{
			...
			const expected: CdpLog[] = [
				...
				{ direction: "resp", method: "Target.setAutoAttach", result: {} },
				{
					...
					params: {
						targetInfo: {
							...
							canAccessOpener: false,
							targetId: "<target-2>",
							title: "<omitted>/worker.js",
							type: "worker",
							url: "<omitted>/worker.js",
						},
					},
					sessionId: "<session-0>",
				},
				...
				},
				{
					direction: "recv",
					method: "Target.detachedFromTarget",
					params: { sessionId: "<session-2>", targetId: "<target-2>" },
					sessionId: "<session-1>",
				},
				{
					direction: "resp",
