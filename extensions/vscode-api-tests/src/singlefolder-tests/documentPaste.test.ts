...
suite.skip("vscode API - Copy Paste", function () {
	...
	test("Copy should be able to overwrite text/plain", async () => {
		...
		testDisposables.push(
			vscode.languages.registerDocumentPasteEditProvider(
				{ language: "plaintext" },
				new (class implements vscode.DocumentPasteEditProvider {
					async prepareDocumentPaste(
						...
						if (existing) {
							...
							dataTransfer.set(
								textPlain,
								new vscode.DataTransferItem(reversed),
							);
						}
					}
				})(),
				{
					providedPasteEditKinds: [
						vscode.DocumentDropOrPasteEditKind.Empty.append("test"),
					],
	...
	test("Earlier invoked copy providers should win when writing values", async () => {
		...
		const providerAFinished = new Promise<void>(
			(resolve) => (providerAResolve = resolve),
		);
			...
			vscode.languages.registerDocumentPasteEditProvider(
				...
				new (class implements vscode.DocumentPasteEditProvider {
					async prepareDocumentPaste(
						...
						dataTransfer.set(
							textPlain,
							new vscode.DataTransferItem(reverseString(str)),
						);
					}
				})(),
				{
