...
suite("vscode API - editors", () => {
	...
	test("insert snippet with replacement, avoid adjusting indentation", () => {
		const snippetString = new SnippetString().appendText(
			"This is line 2\n  This is line 3",
		);

		return withRandomFileEditor("This is line 1\n  ", (editor, doc) => {
			editor.selection = new Selection(new Position(1, 3), new Position(1, 3));

			return editor
				.insertSnippet(snippetString, undefined, {
					undoStopAfter: false,
	...
	async function testEditorContents(relativePath: string) {
		...
		assert.strictEqual(
			document.getText(),
			Buffer.from(await workspace.fs.readFile(file)).toString(),
		);
	}
