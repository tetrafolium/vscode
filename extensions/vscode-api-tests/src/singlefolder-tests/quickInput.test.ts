...
suite("vscode API - quick input", function () {
		...
		async function waitForHide(quickPick: QuickPick<QuickPickItem>) {
			...
			try {
				await Promise.race([
					new Promise(
						(resolve) =>
							(disposable = quickPick.onDidHide(() => resolve(true))),
					),
	...
	test("createQuickPick, match item by label derived from resourceUri", function (_done) {
		...
		quickPick.items = [
			{
				label: "a1",
				resourceUri: baseUri.with({ path: baseUri.path + "/test1.txt" }),
			},
			{
				label: "",
				resourceUri: baseUri.with({ path: baseUri.path + "/test2.txt" }),
			},
			{
				label: "a3",
				resourceUri: baseUri.with({ path: baseUri.path + "/test3.txt" }),
			},
		];
	...
	quickPick.onDidChangeActive((items) => {
		...
		try {
			...
			assert.strictEqual(
				...
				`onDidChangeActive (event ${eventIndex})`,
			);
			const expectedItems = expected.activeItems.shift();
			assert.deepStrictEqual(
				items.map((item) => item.label),
				expectedItems,
				`onDidChangeActive event items (event ${eventIndex})`,
			);
			assert.deepStrictEqual(
				quickPick.activeItems.map((item) => item.label),
				expectedItems,
				`onDidChangeActive active items (event ${eventIndex})`,
			);
		} catch (err) {
			done(err);
		}
	...
	quickPick.onDidChangeSelection((items) => {
		...
		try {
			...
			assert.strictEqual(
				"selection",
				expected.events.shift(),
				`onDidChangeSelection (event ${eventIndex})`,
			);
			const expectedItems = expected.selectionItems.shift();
			assert.deepStrictEqual(
				items.map((item) => item.label),
				expectedItems,
				`onDidChangeSelection event items (event ${eventIndex})`,
			);
			assert.deepStrictEqual(
				quickPick.selectedItems.map((item) => item.label),
				expectedItems,
				`onDidChangeSelection selected items (event ${eventIndex})`,
			);
		} catch (err) {
			done(err);
		}
