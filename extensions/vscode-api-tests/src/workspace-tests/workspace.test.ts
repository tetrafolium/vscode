...
suite("vscode API - workspace", () => {
	...
	test("workspaceFile", () => {
		assert.ok(
			pathEquals(
				...
				join(__dirname, "../../testworkspace.code-workspace"),
			),
		);
	});
	...
	test("getWorkspaceFolder", () => {
		const folder = vscode.workspace.getWorkspaceFolder(
			vscode.Uri.file(join(__dirname, "../../testWorkspace2/far.js")),
		);
		assert.ok(!!folder);

		if (folder) {
			assert.ok(
				pathEquals(folder.uri.fsPath, join(__dirname, "../../testWorkspace2")),
			);
		}
	});
});
