...
const fixturesRootsFolder = path.join(
	__dirname,
	'../../src/extension/test/node/fixtures/patch',
);
const simulationsRootFolder = path.join(__dirname, '../../.simulation');

async function main(
	simulationFolder: string | undefined,
	all: boolean | undefined,
	annotationFilter: string = 'invalid patch',
): Promise<void> {
	...
	const outputFixturesFolder = path.join(
		fixturesRootsFolder,
	...
	for (const entry of entries) {
		for (let testRun = 0; testRun < 10; testRun++) {
			const simTextPath = path.join(
				simulationFolder,
				entry,
				`0${testRun}-inline-simulator.txt`,
			);
			if (!(await checkExists(simTextPath))) {
				break;
			}
			const simText = JSON.parse(
				await fs.readFile(simTextPath, 'utf8'),
			) as any[];
			if (!all) {
			...
			const simRequest = JSON.parse(
				await fs.readFile(
					path.join(
						simulationFolder,
						entry,
						`0${testRun}-sim-requests.txt`,
					),
					'utf8',
				),
			) as any[];
			...
			const original = await fs.readFile(
				path.join(simulationFolder, originalFileEntry.relativeDiskPath),
				'utf8',
			);
					...
					if (
						...
						for (const changedFile of data.changedFiles) {
							if (
								...
								originalFileEntry.workspacePath
							) {
			...
			const modified = await fs.readFile(
				path.join(simulationFolder, modifedFilePath),
				'utf8',
			);
			...
			console.log(
				`Writing fixtures for ${name} at ${outputFixturesFolder}`,
			);
			await fs.mkdir(outputFixturesFolder, { recursive: true });
			await fs.writeFile(
				path.join(outputFixturesFolder, `${name}.original.txt`),
				original,
			);
			await fs.writeFile(
				path.join(outputFixturesFolder, `${name}.expected.txt`),
				modified,
			);
			await fs.writeFile(
				path.join(outputFixturesFolder, `${name}.patch.txt`),
				response,
			);
		}
	}
...
async function findLastRun(
	simulationsRootFolder: string,
): Promise<string | undefined> {
	const entries = await fs.readdir(simulationsRootFolder);
	return entries
		.filter((entry) => entry.match(/^out-\d{8}-\d{6}$/))
		.sort()
		.pop();
}
...
if (require.main === module) {
	...
	if (parsedArgs.help) {
		console.log(
			'Usage: npx tsx editFromPatchTests.ts --simulation-folder <path-to-simulation-folder> --all --annotation <annotation>',
		);
		process.exit(0);
	}

	main(
		parsedArgs['simulation-folder'],
		parsedArgs['all'],
		parsedArgs['annotation'],
	).catch((err) => {
		console.error(err);
		process.exit(1);
	});
