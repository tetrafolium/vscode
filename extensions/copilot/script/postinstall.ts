...
async function removeCopilotCLIShim() {
	...
	await fs.promises.rm(shimsPath, { force: true }).catch(() => {
		/* ignore */
	});
}
...
async function createClaudeSymlinks() {
	...
	console.log(
		'Creating symlinks for Claude session storage and instructions...',
	);
	...
	const symlinks: { link: string; target: string }[] = [
		{
			link: path.join(claudeDir, 'CLAUDE.md'),
			target: path.join('..', '.github', 'copilot-instructions.md'),
		},
		{
			link: path.join(claudeDir, 'skills'),
...
async function main() {
	await fs.promises.mkdir(path.join(REPO_ROOT, '.build'), {
		recursive: true,
	});
	...
	await copyStaticAssets(
		[
			...treeSitterGrammars.map(
				(grammar) =>
					`node_modules/@vscode/tree-sitter-wasm/wasm/${grammar.name}.wasm`,
			),
	...
	if (!fs.existsSync(baseCachePath)) {
		console.warn(
			`Warning: Base cache file does not exist at ${baseCachePath}. Please ensure that you have git lfs installed and initialized before the repository is cloned.`,
		);
	}
