...
import {
	...
	OutputType,
	RunOutput,
} from '../../../test/simulation/shared/sharedTypes';
import {
	ISpawnSimulationOptions,
	SIMULATION_MAIN_PATH,
	extractJSONL,
	spawnSimulation,
} from '../../../test/simulation/workbench/utils/simulationExec';
...
export async function activate(context: vscode.ExtensionContext) {
	...
	try {
		const pkg = JSON.parse(
			String(await fs.promises.readFile(join(REPO_ROOT, 'package.json'))),
		);
		isCorrectRepo = pkg.name === 'copilot-chat';
	} catch (err) {
		console.error(
			'[STEST] error reading ' + join(REPO_ROOT, 'package.json'),
		);
		console.error(err);
		isCorrectRepo = false;
	}
	...
	if (!isCorrectRepo) {
		console.log(
			'[STEST] NO activation because in wrong REPO/WORKSPACE',
			REPO_ROOT,
		);
		return;
	}
	...
	ctrl.refreshHandler = async (token) => {
		const stream = spawnSimulation<
			IDetectedTestOutput | IDetectedSuiteOutput
		>(
			{
				ignoreNonJSONLines: true,
				args: ['--list-tests', '--list-suites', '--json'],
			},
			token,
		);
		...
		for await (const item of stream) {
			const testItem = ctrl.createTestItem(
				item.name,
				item.name,
				item.location && URI.file(item.location.path),
			);
			testItem.range =
				item.location &&
				new vscode.Range(
	...
	const simulationAsTestRun = async (
		request: vscode.TestRunRequest,
		options: { extraArgs: string[]; debug?: boolean },
		...
		run.appendOutput(
			'[STEST] will SPAWN simulation with: ' + args.join(' ') + '\r\n',
		);

		try {
			const stream = !options.debug
				? spawnSimulation<RunOutput>(
						{ ignoreNonJSONLines: true, args },
						token,
					)
				: debugSimulation<RunOutput>(
						{ ignoreNonJSONLines: true, args },
						token,
					);
			...
			for await (const output of stream) {
				...
				if (output.type === OutputType.initialTestSummary) {
					...
					if (runs.done(output.pass)) {
						run.passed(test);
						run.appendOutput(
							`[STEST] DONE with ${output.name}, ${runs.passes} passes and ${runs.fails} fails`,
							undefined,
							test,
						);
					}
	...
	const defaultRunProfile = ctrl.createRunProfile(
		'STest',
		vscode.TestRunProfileKind.Run,
		(request, token) =>
			simulationAsTestRun(request, { extraArgs: ['-p', '20'] }, token),
	);
	context.subscriptions.push(defaultRunProfile);
	defaultRunProfile.isDefault = true;

	const defaultDebugProfile = ctrl.createRunProfile(
		'STest: debug',
		vscode.TestRunProfileKind.Debug,
		(request, token) =>
			simulationAsTestRun(
				request,
				{ extraArgs: ['--n', '1', '-p', '1'], debug: true },
				token,
			),
	);
	context.subscriptions.push(defaultDebugProfile);

	const visualizeDebugProfile = ctrl.createRunProfile(
		'STest: inspect and visualize',
		vscode.TestRunProfileKind.Debug,
		(request, token) => {
			const args = {
	...
	const updateBaselineProfile = ctrl.createRunProfile(
		'STest: update-baseline',
		vscode.TestRunProfileKind.Run,
		(request, token) =>
			simulationAsTestRun(
				request,
				{ extraArgs: ['--update-baseline', '-p', '20'] },
				token,
			),
	);
...
function debugSimulation<T>(
	...
	token.onCancellationRequested(() => {
		sessions.forEach(
			(s) => !s.parentSession && vscode.debug.stopDebugging(s),
		);
		source.resolve();
	});
	...
	store.add(
		vscode.debug.registerDebugAdapterTrackerFactory('*', {
			createDebugAdapterTracker: (session) => {
				...
				const __key =
					session.configuration.__key ??
					session.parentSession?.configuration.__key;
				if (__key !== key) {
					return;
				}
