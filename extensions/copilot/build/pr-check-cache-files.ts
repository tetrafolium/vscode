...
async function getCommit(repository: string, sha: string): Promise<Commit> {
	const { stdout, stderr } = await execAsync(
		`gh api -H "Accept: application/vnd.github+json" /repos/${repository}/commits/${sha}`,
...
async function getPullRequestFiles(
	...
	const { stdout, stderr } = await execAsync(
		`gh api -H "Accept: application/vnd.github+json" /repos/${repository}/pulls/${pullRequestNumber}/files --paginate`,
		{ maxBuffer: 25 * 1024 * 1024 },
	);
...
async function getPullRequestCommits(
	...
	const { stdout, stderr } = await execAsync(
		`gh api -H "Accept: application/vnd.github+json" /repos/${repository}/pulls/${pullRequestNumber}/commits --paginate`,
		{ maxBuffer: 25 * 1024 * 1024 },
	);
...
async function checkDatabaseFile(
	...
	const baseFile = files.find(
		(f) => f.filename.toLowerCase() === 'test/simulation/cache/base.sqlite',
	);
	if (!baseFile) {
	...
	console.log(
		`   - 🗄️ ${baseFile.filename}; Status: ${baseFile.status} ${statusCheck ? '✅' : '⛔'}`,
	);

	return statusCheck;
}

async function checkDatabaseLayerFiles(
	repository: string,
	pullRequestNumber: string,
	...
	collaboratorCheck: boolean;
}> {
	const layerFiles = files.filter((f) =>
		f.filename.toLowerCase().startsWith('test/simulation/cache/layers/'),
	);
	...
	const commitsWithDetails = await Promise.all(
		pullRequestCommits.map((sha) => getCommit(repository, sha)),
	);

	let statusCheckResult = true,
		verifiedCheckResult = true,
		collaboratorCheckResult = true;
	console.log(
		`🔍 Pull request contains ${layerFiles.length} layer files. Checking status and author...`,
	);
	...
	for (const file of layerFiles) {
		...
		console.log(
			`     - Status: ${file.status} ${statusCheck ? '✅' : '⛔'}`,
		);
		...
		for (const commit of commits) {
			const collaboratorCheck = collaborators.find(
				(c) => c === commit.committer.login,
			);
			const verifiedCheck =
				commit.commit.verification.verified &&
				commit.commit.verification.reason === 'valid';
			console.log(
				`       - ${commit.sha} by ${commit.committer.login}. Collaborator: ${collaboratorCheck ? '✅' : '⛔'} Verified: ${verifiedCheck ? '✅' : '⛔'}`,
			);
	...
	return {
		...
		verifiedCheck: verifiedCheckResult,
		collaboratorCheck: collaboratorCheckResult,
	};
}
...
async function main() {
	try {
		...
		if (!repository || !pullRequestNumber) {
			throw new Error(
				'Missing required environment variables: REPOSITORY or PULL_REQUEST',
			);
		}

		console.log(
			`🔍 Checking pull request #${pullRequestNumber} in repository "${repository}"...`,
		);
		...
		const layerCheckResult = await checkDatabaseLayerFiles(
			repository,
			pullRequestNumber,
			files,
		);
		...
		if (!baseCheckResult) {
			throw new Error(
				'Base file can only be modified in a pull request.',
			);
		}
		if (!layerCheckResult.statusCheck) {
			throw new Error(
				'Cache layer files can only be added or deleted, never modified',
			);
		}
		if (
			!layerCheckResult.verifiedCheck ||
			!layerCheckResult.collaboratorCheck
		) {
			throw new Error(
				'Cache layer files can only be added by VS Code team members with signed commits',
			);
		}
	} catch (error) {
