...
const defaultChat = {
	...
	enterpriseProviderName:
		product.defaultChatAgent?.enterpriseProviderName ?? '',
...
class SetupChatAgent extends Disposable implements IChatAgentImplementation {
	static registerDefaultAgents(
	...
	): { disposable: IDisposable; agent: SetupChatAgent } {
		return instantiationService.invokeFunction((accessor) => {
			...
			return SetupChatAgent.registerAgents(
				instantiationService,
				chatAgentService,
				id,
				`${defaultChat.providerName} Copilot`,
				true,
				description,
				location,
				mode,
	...
	static registerAgents(
		...
		disposables.add(
			chatAgentService.registerAgent(id, {
				...
				when:
					mode === ChatMode.Agent
						? ToolsAgentWhen?.serialize()
						: undefined,
				slashCommands: [],
				disambiguation: [],
				locations: [location],
				metadata: {
					helpTextPrefix: SetupChatAgent.SETUP_NEEDED_MESSAGE,
				},
				description,
	...
	private readonly pendingForwardedRequests = new Map<
		string,
		Promise<void>
	>();
	...
	async invoke(
		...
		return this.instantiationService.invokeFunction(
			async (accessor) /* using accessor for lazy loading */ => {
				...
				const languageModelsService = accessor.get(
	...
	private whenLanguageModelReady(
		...
		return Event.toPromise(
			Event.filter(
				...
				(e) =>
					e.added?.some((added) => added.metadata.isDefault) ?? false,
			),
		);
	}
	...
	private whenToolsModelReady(
		...
		return Event.toPromise(
			Event.filter(languageModelToolsService.onDidChangeTools, (_) => {
				for (const tool of languageModelToolsService.getTools()) {
					if (tool.source.type !== 'internal') {
	...
	private async whenDefaultAgentFailed(
		...
		return new Promise<void>((resolve) => {
			chatService
				.activateDefaultAgent(this.location)
				.catch(() => resolve());
		});
	}
				...
				if (result.dialogSkipped) {
					progress({
						...
						content: new MarkdownString(
							localize(
								'copilotSetupSuccess',
								'Copilot setup finished successfully.',
							),
						),
					});
...
class SetupTool extends Disposable implements IToolImpl {
	static registerTools(
		instantiationService: IInstantiationService,
		toolData: IToolData,
	): { disposable: IDisposable; tool: SetupTool } {
		return instantiationService.invokeFunction((accessor) => {
			const disposables = new DisposableStore();
...
class ChatSetup {
	...
	private async showDialog(): Promise<ChatSetupStrategy> {
				...
				createWorkbenchDialogOptions(
					{
						...
						renderBody: (body) =>
							body.appendChild(this.createDialog(disposables)),
		...
		return button === 0
			? (result ?? ChatSetupStrategy.DefaultSetup)
	...
	implements IWorkbenchContribution
{
	...
	private registerSetupAgents(
		...
		const defaultAgentDisposables = markAsSingleton(
			new MutableDisposable(),
		); // prevents flicker on window reload
		const otherAgentsAndToolsDisposable = markAsSingleton(
			new MutableDisposable(),
		);
			...
			if (!disabled) {
				if (!defaultAgentDisposables.value) {
					...
					for (const mode of [
						...
						const { agent, disposable } =
							SetupChatAgent.registerDefaultAgents(
								this.instantiationService,
				...
				if (
					...
					!otherAgentsAndToolsDisposable.value
				) {
					const disposables = (otherAgentsAndToolsDisposable.value =
						new DisposableStore());
					// VSCode Agent
					disposables.add(
						SetupChatAgent.registerOtherAgents(
							this.instantiationService,
							'setup.vscode',
							'vscode',
							false,
					...
					// Tools
					disposables.add(
						SetupTool.registerTools(this.instantiationService, {
		...
		this._register(
			Event.runAndSubscribe(context.onDidChange, () =>
				updateRegistration(),
			),
		);
	}
	...
	private registerActions(
		...
		class ChatSetupTriggerWithoutDialogAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const chatWidget = await showCopilotView(
					viewsService,
					layoutService,
				);
				...
				chatWidget?.acceptInput(
					localize('setupCopilot', 'Set up Copilot.'),
				);
			}
		}
		...
		class ChatSetupHideAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const location =
					viewsDescriptorService.getViewLocationById(ChatViewId);
				...
				if (location === ViewContainerLocation.AuxiliaryBar) {
					const activeContainers = viewsDescriptorService
						...
						.filter(
							(container) =>
								viewsDescriptorService.getViewContainerModel(
									container,
					...
					if (activeContainers.length === 0) {
						...
						); // hide if there are no views in the secondary sidebar
					}
				}
			}
		}
		...
		class UpgradePlanAction extends Action2 {
			...
			private async onWindowFocus(
				...
				if (focus) {
					...
					const entitlements =
						await requests.forceResolveEntitlement(undefined);
...
class ChatSetupController extends Disposable {
	...
	private async install(
		...
		let signUpResult: boolean | { errorCode: number } | undefined =
			undefined;
