...
export function insertRangeMarkers(
	source: string,
	ranges: MarkerRange[],
	opts?: Partial<InsertMarkersOptions>,
) {
	opts = {
		...{
			withOffsets: false,
			includeArrayIndex: false,
			includeKindIndex: true,
		}, // default
		...opts,
	};
	...
	const markedOffsets = unsortedMarkedOffsets.sort(
		(
			{ offset, offsetKind, len, indexInArray },
			{
				offset: otherOffset,
				offsetKind: otherOffsetKind,
				len: otherLen,
				indexInArray: otherIndexInArray,
			},
		) => {
			...
			if (offsetKind === otherOffsetKind) {
				...
				return indexInArray === otherIndexInArray
					? offsetKind === 'start'
						? -1
						: 1 // if same node, start should come first
					: offsetKind === 'start'
						? 1
						: -1;
			}
	...
	for (const {
		...
		const conditionalArrayIndex = includeArrayIndex
