...
export class ParserServiceImpl implements IParserService {
	...
	constructor(useWorker: boolean) {
		this._parser = new WorkerOrLocal<ParserType>(
			parser,
			workerPath,
			useWorker,
		);
	...
	getTreeSitterAST(textDocument: {
		...
		return this.getTreeSitterASTForWASMLanguage(
			wasmLanguage,
	...
	getTreeSitterASTForWASMLanguage(
		...
		return {
			getFunctionBodies: () =>
				parserProxy._getFunctionBodies(wasmLanguage, source),
			getCoarseParentScope: (range: TreeSitterPointRange) =>
				parserProxy._getCoarseParentScope(wasmLanguage, source, range),
			getFixSelectionOfInterest: (
				range: TreeSitterPointRange,
				maxNumberOfLines: number,
			) =>
				parserProxy._getFixSelectionOfInterest(
					wasmLanguage,
					source,
					range,
					maxNumberOfLines,
				),
			getCallExpressions: (selection: TreeSitterOffsetRange) =>
				parserProxy._getCallExpressions(
					wasmLanguage,
					source,
					selection,
				),
			getFunctionDefinitions: () =>
				parserProxy._getFunctionDefinitions(wasmLanguage, source),
			getClassReferences: (selection: TreeSitterOffsetRange) =>
				parserProxy._getClassReferences(
					wasmLanguage,
					source,
					selection,
				),
			getClassDeclarations: () =>
				parserProxy._getClassDeclarations(wasmLanguage, source),
			getTypeDeclarations: () =>
				parserProxy._getTypeDeclarations(wasmLanguage, source),
			getTypeReferences: (selection: TreeSitterOffsetRange) =>
				parserProxy._getTypeReferences(wasmLanguage, source, selection),
			getSymbols: (selection: TreeSitterOffsetRange) =>
				parserProxy._getSymbols(wasmLanguage, source, selection),
			getDocumentableNodeIfOnIdentifier: (range: TreeSitterOffsetRange) =>
				parserProxy._getDocumentableNodeIfOnIdentifier(
					wasmLanguage,
					source,
					range,
				),
			getNodeToExplain: (range: TreeSitterOffsetRange) =>
				parserProxy._getNodeToExplain(wasmLanguage, source, range),
			getNodeToDocument: (range: TreeSitterOffsetRange) =>
				parserProxy._getNodeToDocument(wasmLanguage, source, range),
			getFineScopes: (selection: TreeSitterOffsetRange) =>
				parserProxy._getFineScopes(wasmLanguage, source, selection),
			getStructure: () => parserProxy._getStructure(wasmLanguage, source),
			findLastTest: () => parserProxy._findLastTest(wasmLanguage, source),
			getParseErrorCount: () =>
				parserProxy._getParseErrorCount(wasmLanguage, source),
		};
	}
...
class WorkerOrLocal<T extends object> {
	...
	private _restart(): void {
		...
		this._worker = new Lazy(
			() =>
				new WorkerWithRpcProxy<T>(this._workerPath, {
					name: 'Parser worker',
				}),
		);
	}
			...
			get(_target, prop) {
				return async (...args: unknown[]) => {
					...
					let timeoutHandle:
						...
						| undefined;
					const timeoutPromise = new Promise<typeof timedOut>(
						(resolve) => {
