...
export {
	_getDocumentableNodeIfOnIdentifier,
	_getNodeToDocument,
	NodeToDocumentContext,
} from './docGenParsing';
...
function queryTypeDeclarations(
	language: WASMLanguage,
	root: Parser.SyntaxNode,
): Parser.QueryMatch[] {
 ...
 */
export async function _getCallExpressions(
	language: WASMLanguage,
	source: string,
	...
	try {
		...
		const positions = results.reduce<TreeSitterExpressionInfo[]>(
			(acc, res) => {
				const fn = res.captures.find(
					(c) => c.name === 'call_expression',
				)!.node;
				if (TreeSitterOffsetRange.doIntersect(selection, fn)) {
					let identifier;
					let identifierNode;
					if (language === 'ruby') {
						// strip preceding : from any captured simple symbols
						identifierNode = res.captures.find(
							(c) => c.name === 'symbol',
						)?.node;
 ...
 */
export async function _getFunctionDefinitions(
	language: WASMLanguage,
	source: string,
): Promise<TreeSitterExpressionInfo[]> {
	const treeRef = await _parse(language, source);
	try {
		...
		const positions = results.map((res) => {
			...
			const identifier = res.captures.find((c) => c.name === 'identifier')
				?.node.text;
			return {
				identifier: identifier ?? '',
				text: fn.text,
...
export async function _getClassDeclarations(
	language: WASMLanguage,
	source: string,
): Promise<TreeSitterExpressionInfo[]> {
	const treeRef = await _parse(language, source);
...
export async function _getTypeDeclarations(
	language: WASMLanguage,
	source: string,
): Promise<TreeSitterExpressionInfo[]> {
	const treeRef = await _parse(language, source);
	try {
		...
		const positions = results.map((res) => {
			const fn = res.captures.find(
				(c) => c.name === 'type_declaration',
			)!.node;
			let identifier = res.captures.find(
				(c) => c.name === 'type_identifier',
			)?.node.text;
			if (!identifier) {
				// TODO@joyceerhl debt: move this into query captures
				identifier = fn?.children.find(
...
export async function _getTypeReferences(
	language: WASMLanguage,
	source: string,
	selection: TreeSitterOffsetRange,
): Promise<TreeSitterExpressionInfo[]> {
	...
	try {
		...
		const positions = results.reduce(
			(acc: TreeSitterExpressionInfo[], res: Parser.QueryMatch) => {
				const typeIdentifier = res.captures.find(
					(c) => c.name === 'type_identifier',
				)!.node;
				if (
					TreeSitterOffsetRange.doIntersect(selection, typeIdentifier)
				) {
					acc.push({
...
export async function _getClassReferences(
	language: WASMLanguage,
	source: string,
	selection: TreeSitterOffsetRange,
): Promise<TreeSitterExpressionInfo[]> {
...
export async function _getSymbols(
	language: WASMLanguage,
	source: string,
	selection: TreeSitterOffsetRange,
): Promise<TreeSitterExpressionInfo[]> {
...
export async function _getSemanticChunkTree(
	language: WASMLanguage,
	source: string,
...
export async function _getSemanticChunkNames(
	language: WASMLanguage,
	source: string,
 ...
 */
export async function _getFunctionBodies(
	language: WASMLanguage,
	source: string,
): Promise<TreeSitterOffsetRange[]> {
	const treeRef = await _parse(language, source);
	try {
		const results = queryFunctions(language, treeRef.tree.rootNode);
		const positions = results.map((res) => {
...
export async function _getCoarseParentScope(
	language: WASMLanguage,
	source: string,
	...
	try {
		...
		for (const scope of scopes) {
			...
			const captureNodeRange =
				TreeSitterPointRange.ofSyntaxNode(captureNode);
			if (TreeSitterPointRange.doesContain(captureNodeRange, range)) {
				parentNode = captureNode;
			}
			if (
				TreeSitterPoint.isBefore(
					range.endPosition,
					captureNodeRange.startPosition,
				)
			) {
				break;
			}
 ...
 */
export async function _getFixSelectionOfInterest(
	language: WASMLanguage,
	source: string,
	range: TreeSitterPointRange,
	...
	const treeRef = await _parse(language, source);
	try {
		const smallestNode = treeRef.tree.rootNode.descendantForPosition(
			range.startPosition,
			range.endPosition,
		);
		const initialRange = {
			startPosition: smallestNode.startPosition,
			endPosition: smallestNode.endPosition,
		};
		const biggestRange = _getBiggestRangeContainingNodeSmallerThan(
			language,
			smallestNode,
			maxNumberOfLines,
			range,
 ...
 */
function _getSmallestRangeContainingNode(
	language: WASMLanguage,
	...
	const { filteredRanges, indexOfInterest } =
		_findFilteredRangesAndIndexOfInterest(
			language,
			parent.children,
			range,
			false,
	...
	if (
		indexOfInterest - 1 >= 0 &&
		indexOfInterest + 1 <= filteredRanges.length - 1
	) {
		const siblingAbove = filteredRanges[indexOfInterest - 1];
		const siblingBelow = filteredRanges[indexOfInterest + 1];
		return {
			startPosition: siblingAbove.startPosition,
			endPosition: siblingBelow.endPosition,
		};
	}
 ...
 */
function _getBiggestRangeContainingNodeSmallerThan(
	language: WASMLanguage,
	node: Parser.SyntaxNode,
	maxNumberOfLines: number,
	range: TreeSitterPointRange,
	...
	if (lengthSpannedByNode <= maxNumberOfLines) {
		const newRange = _isScope(language, node)
			...
			: _getBiggestRangeContainingNodeAmongNodesSmallerThan(
					language,
					children,
					maxNumberOfLines,
					range,
		...
		const parent = node.parent;
		return parent
			? _getBiggestRangeContainingNodeSmallerThan(
					language,
					parent,
					maxNumberOfLines,
					newRange,
	...
	return _getBiggestRangeContainingNodeAmongNodesSmallerThan(
		language,
		children,
		maxNumberOfLines,
		range,
...
function _numberOfLinesSpannedByRanges(
	range1: TreeSitterPointRange,
	range2: TreeSitterPointRange,
) {
 ...
 */
function _getBiggestRangeContainingNodeAmongNodesSmallerThan(
	language: WASMLanguage,
	nodes: Parser.SyntaxNode[],
	maxNumberOfLines: number,
	lastRange: TreeSitterPointRange,
	firstCall: boolean,
): TreeSitterPointRange {
	if (nodes.length === 0) {
		return lastRange;
	...
	while (
		_numberOfLinesSpannedByRanges(siblingAbove, siblingBelow) >
		maxNumberOfLines
	) {
		if (siblingAboveIndex === siblingBelowIndex) {
			...
			indexOfInterest - siblingAboveIndex <
			siblingBelowIndex - indexOfInterest
		) {
			siblingBelowIndex--;
 ...
 */
function _findFilteredRangesAndIndexOfInterest(
	language: WASMLanguage,
	nodes: Parser.SyntaxNode[],
	range: TreeSitterPointRange,
	firstCall: boolean,
): { filteredRanges: TreeSitterPointRange[]; indexOfInterest: number } {
	let filteredRanges: TreeSitterPointRange[];
	let indexOfInterest: number;
	if (firstCall) {
		filteredRanges = nodes.filter(
			(child) =>
				_isScope(language, child) || _isStatement(language, child),
		);
		indexOfInterest = findInsertionIndexInSortedArray(
			filteredRanges,
			range,
			(a, b) =>
				TreeSitterPoint.isBefore(a.startPosition, b.startPosition),
		);
		filteredRanges.splice(indexOfInterest, 0, range);
	} else {
		filteredRanges = nodes.filter(
			(child) =>
				TreeSitterPointRange.doesContain(child, range) ||
				_isScope(language, child) ||
				_isStatement(language, child),
		);
		indexOfInterest = filteredRanges.findIndex((child) =>
			TreeSitterPointRange.doesContain(child, range),
		);
	}
...
export async function _getFineScopes(
	language: WASMLanguage,
	source: string,
	selection: TreeSitterOffsetRange,
): Promise<TreeSitterOffsetRange[]> {
	...
	const treeRef = await _parse(language, source);
	const syntaxNode = treeRef.tree.rootNode.descendantForIndex(
...
export async function _getNodeToExplain(
	...
	try {
		...
		const identifier = isSelectionEmpty
			? undefined
			: _getNodeMatchingSelection(treeRef.tree, selection, language);
		const fullDefinition = isSelectionEmpty
			? undefined
			: _getNodeMatchingSelection(
					treeRef.tree,
					selection,
					language,
					isExplainableNode,
...
export function getBlockNameTree(
	language: WASMLanguage,
	queryMatches: Parser.QueryMatch[],
	root: Parser.SyntaxNode,
): QueryMatchTree<BlockNameDetail> {
	...
	queryMatches.forEach((n) => {
		...
		let definitionNode = captures.find(
			(v) => v.name === 'definition',
		)?.node;

		let keyword;
		if (
			language === WASMLanguage.Cpp &&
			definitionNode?.type === 'function_definition'
		) {
			keyword = definitionNode
		...
		if (definitionNode && bodyNode) {
			switch (language) {
				...
				case WASMLanguage.JavaScript: {
					const { definition } =
						getCommentsAndDefFromTSJSDefinition(definitionNode);
			...
			if (!existingMatch) {
				matches.set(definitionNode.id, {
					mainBlock:
						TreeSitterChunkHeaderInfo.ofSyntaxNode(definitionNode),
					detailBlocks: {
	...
	return new QueryMatchTree(
		groups,
		TreeSitterChunkHeaderInfo.ofSyntaxNode(root),
	);
}
...
function getQueryMatchTree(
	language: WASMLanguage,
	queryMatches: Parser.QueryMatch[],
	root: Parser.SyntaxNode,
): QueryMatchTree<DetailBlock> {
	...
	switch (language) {
		...
		case WASMLanguage.Ruby:
			...
			break;
		default: {
			groups = queryCapturesToGenericSemanticGroup(
				queryMatches,
				language,
			);
			break;
	...
	const queryTree = new QueryMatchTree(
		groups,
		TreeSitterChunkHeaderInfo.ofSyntaxNode(root),
	);
...
function queryCapturesToGenericSemanticGroup(
	queryMatches: Parser.QueryMatch[],
	wasmLang: WASMLanguage,
): MatchGroup<GenericDetail>[] {
		...
		if (definitionNode && bodyNode) {
			...
			switch (wasmLang) {
				...
				case WASMLanguage.Rust:
					commentNodes =
						getCommentsFromJavaRustDefinition(definitionNode);
					break;
				default: {
			...
			if (!existingMatch) {
				matches.set(definitionNode.id, {
					mainBlock:
						TreeSitterChunkHeaderInfo.ofSyntaxNode(definitionNode),
					detailBlocks: {
						comments: commentNodes.map((e) =>
							TreeSitterChunkHeaderInfo.ofSyntaxNode(e),
						),
						body: TreeSitterChunkHeaderInfo.ofSyntaxNode(bodyNode),
					},
				});
...
function queryCapturesToRubySemanticGroup(
	queryMatches: Parser.QueryMatch[],
): MatchGroup<GenericDetail>[] {
	const matches: Map<number, MatchGroup<GenericDetail>> = new Map(); // map nodes to their starting position to ensure that we get rid of duplicates
	queryMatches.forEach((n) => {
		...
		const definitionNode = captures.find(
			(v) => v.name === 'definition',
		)?.node;
		if (definitionNode) {
			...
			if (startChild) {
				...
				const childText = definitionNode.text.substring(
					startChild.startIndex - definitionNode.startIndex,
					...
					matches.set(definitionNode.id, {
						...
						detailBlocks: {
							...
							body: {
								range: <TreeSitterPointRange>{
									startPosition: {
										row: startChild.startPosition.row,
										column: startChild.startPosition.column,
									},
									endPosition: {
										row: endChild.endPosition.row,
										column: endChild.endPosition.column,
									},
								},
...
function queryCapturesToPythonSemanticGroup(
	queryMatches: Parser.QueryMatch[],
): MatchGroup<PythonDetail>[] {
		...
		if (definitionNode && bodyNode) {
			...
			matches.set(definitionNode.id, {
				mainBlock:
					TreeSitterChunkHeaderInfo.ofSyntaxNode(definitionNode),
				detailBlocks: {
					docstring: docstringNode
						? TreeSitterChunkHeaderInfo.ofSyntaxNode(docstringNode)
						: undefined,
					decorator: decoratorNode
						? TreeSitterChunkHeaderInfo.ofSyntaxNode(decoratorNode)
						: undefined,
					body: TreeSitterChunkHeaderInfo.ofSyntaxNode(bodyNode),
				},
			});
 ...
 */
function getCommentsFromDefinition(
	definition: Parser.SyntaxNode,
	commentNodeNames = ['comment'],
): Parser.SyntaxNode[] {
	...
	while (
		prevSibling &&
		commentNodeNames.some((e) => e === prevSibling?.type)
	) {
 ...
 */
function getCommentsFromJavaRustDefinition(
	definition: Parser.SyntaxNode,
): Parser.SyntaxNode[] {
	return getCommentsFromDefinition(definition, [
		'block_comment',
		'line_comment',
	]);
}
...
function getDocstringFromBody(body: Parser.SyntaxNode) {
	...
	return potentialDocstring?.type === 'string'
		? potentialDocstring
		: undefined;
}

export function _getStructure(
	lang: WASMLanguage,
	source: string,
...
export async function _getParseErrorCount(
	language: WASMLanguage,
	source: string,
): Promise<number> {
	const treeRef = await _parse(language, source);
