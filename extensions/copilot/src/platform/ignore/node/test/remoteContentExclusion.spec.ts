...
suite('RemoteContentExclusion', () => {
	...
	describe('repository root caching', () => {
		test('should cache repository lookup and reuse for subsequent files', async () => {
			...
			await remoteContentExclusion.isIgnored(
				file1,
		...
		test('should handle nested repositories correctly by matching longest path', async () => {
			...
			const nestedRepoRoot =
				'/workspace/parent-repo/submodules/nested-repo';
			mockGitService.getRepositoryFetchUrls = vi
				.fn()
				.mockImplementation(() => {
			...
			const nestedFile = URI.file(
				'/workspace/parent-repo/submodules/nested-repo/src/index.ts',
			);
			await remoteContentExclusion.isIgnored(
				nestedFile,
				CancellationToken.None,
		...
		test('should handle files outside any git repository', async () => {
			...
			const result = await remoteContentExclusion.isIgnored(
				nonGitFile,
				CancellationToken.None,
			);
	...
	describe('loadRepos', () => {
		test('should populate the cache when loading repos', async () => {
			...
			const repos = [
				{
					rootUri: URI.file('/workspace/repo1'),
					remoteFetchUrls: ['https://github.com/org/repo1.git'],
				},
				{
					rootUri: URI.file('/workspace/repo2'),
					remoteFetchUrls: ['https://github.com/org/repo2.git'],
				},
			];
