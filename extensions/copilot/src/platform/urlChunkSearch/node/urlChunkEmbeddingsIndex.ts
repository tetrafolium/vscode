...
export class UrlChunkEmbeddingsIndex extends Disposable {
	private readonly _cache = new SimpleUrlContentCache<
		readonly FileChunkWithEmbedding[]
	>();
	...
	constructor(
		...
		private readonly _availableEmbeddingTypesService: IGithubAvailableEmbeddingTypesService,
	) {
		super();
	}
	...
	async findInUrls(
		...
		const embeddingType = await raceCancellationError(
			this._availableEmbeddingTypesService.getPreferredType(
				/*silent*/ false,
			),
			token,
		);
		...
		if (!authToken) {
			this._logService.error(
				'urlChunkEmbeddingsIndex: Unable to get auth token',
			);
			...
			await raceCancellationError(
				Promise.all([
					...
					this.getEmbeddingsForFiles(
						...
						embeddingType,
						files.map(
							(file) => new UrlContent(file.uri, file.content),
						),
	...
	private async computeEmbeddings(
		...
		const embeddings = await this._embeddingsComputer.computeEmbeddings(
			...
			new TelemetryCorrelationId(
				'UrlChunkEmbeddingsIndex::computeEmbeddings',
			),
			token,
		);
	...
	private async getEmbeddingsForFiles(
		authToken: string,
		embeddingType: EmbeddingType,
		files: readonly UrlContent[],
		qos: EmbeddingsComputeQos,
		token: CancellationToken,
	): Promise<(readonly FileChunkWithEmbedding[])[]> {
		if (!files.length) {
			return [];
		}
		...
		const result = await Promise.all(
			files.map(async (file) => {
				const result = await this.getChunksAndEmbeddings(
					authToken,
					embeddingType,
					file,
					batchInfo,
	...
	private computeChunkScores(
		...
		return fileChunksAndEmbeddings.map((file) =>
			file.map(
				({ chunk, embedding }): FileChunkAndScore => ({
					chunk,
	...
	private async getChunksAndEmbeddings(
		...
		const existing = await raceCancellationError(
			this._cache.get(content),
			token,
		);
		if (existing) {
			return existing;
		...
		const chunksAndEmbeddings = await raceCancellationError(
			this._chunkingEndpointClient.computeChunksAndEmbeddings(
				authToken,
				embeddingType,
				content,
				batchInfo,
				...
				new Map(),
				new CallTracker(
					'UrlChunkEmbeddingsIndex::getChunksAndEmbeddings',
				),
				token,
			),
			token,
	...
	private async tryGetAuthToken(): Promise<string | undefined> {
		return (
			await this._authService.getGitHubSession('any', {
				createIfNone: {
					detail: l10n.t(
						'Sign in to GitHub to access URL chunk embeddings.',
					),
				},
			})
		)?.accessToken;
