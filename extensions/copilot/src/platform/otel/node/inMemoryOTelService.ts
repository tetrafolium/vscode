...
export class InMemoryOTelService implements IOTelService {
	...
	storeTraceContext(key: string, context: TraceContext): void {
		...
		if (
			...
			InMemoryOTelService._MAX_TRACE_CONTEXT_STORE_SIZE
		) {
	...
	runWithTraceContext<T>(
		...
		return this._contextStorage.run(
			{ spanId: traceContext.spanId, traceId: traceContext.traceId },
			fn,
		);
	}
