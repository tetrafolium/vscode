...
describe('normalizeResponseModel', () => {
	it('echoes the request model when the response model only differs by `.` vs `-` (issue #318805)', () => {
		expect(
			normalizeResponseModel('claude-opus-4.6', 'claude-opus-4-6'),
		).toBe('claude-opus-4.6');
		expect(
			normalizeResponseModel('claude-haiku-4.5', 'claude-haiku-4-5'),
		).toBe('claude-haiku-4.5');
		expect(
			normalizeResponseModel('claude-sonnet-4.6', 'claude-sonnet-4-6'),
		).toBe('claude-sonnet-4.6');
	});

	it('returns the resolved model when it adds specificity (e.g. dated snapshot)', () => {
		expect(
			normalizeResponseModel('gpt-5.4-mini', 'gpt-5.4-mini-2026-03-17'),
		).toBe('gpt-5.4-mini-2026-03-17');
		expect(normalizeResponseModel('gpt-4o', 'gpt-4o-2024-08-06')).toBe(
			'gpt-4o-2024-08-06',
		);
	});
	...
	it('echoes the request model when the response strips a request-only suffix (e.g. reasoning effort)', () => {
		...
		expect(
			normalizeResponseModel('claude-opus-4.7-high', 'claude-opus-4-7'),
		).toBe('claude-opus-4.7-high');
		expect(
			normalizeResponseModel('claude-opus-4.6-medium', 'claude-opus-4-6'),
		).toBe('claude-opus-4.6-medium');
	...
	it('returns undefined when no response model is provided', () => {
		expect(
			normalizeResponseModel('claude-opus-4.6', undefined),
		).toBeUndefined();
	...
	it('returns the response model when no request model is available', () => {
		expect(normalizeResponseModel(undefined, 'claude-opus-4-6')).toBe(
			'claude-opus-4-6',
		);
	});
});
