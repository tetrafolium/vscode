...
describe('createResponsesRequestBody tools', () => {
	...
	beforeEach(() => {
		...
		services.define(IToolDeferralService, {
			...
			isNonDeferredTool: (name: string) => coreNonDeferred.has(name),
		});
	...
	it('adds client tool_search and defer_loading for supported models', () => {
		...
		expect(
			tools.find((t) => t.name === 'grep_search')?.defer_loading,
		).toBeUndefined();
		...
		expect(
			tools.find((t) => t.name === 'another_deferred_tool'),
		).toBeUndefined();
	});
	...
	it('converts tool_search history for unsupported models', () => {
		...
		const toolSearchOutput = input.find(
			(i) => i.type === 'tool_search_output',
		);
		...
		const loadedToolNames = (toolSearchOutput.tools as any[]).map(
			(t: any) => t.name,
		);
		expect(loadedToolNames).toEqual([]);

		// Should not have any function_call with name tool_search
		const badFunctionCall = input.find(
			(i) => i.type === 'function_call' && i.name === 'tool_search',
		);
	...
	it('converts tool_search history when current request has no tools', () => {
		...
		expect(
			input.find((i) => i.type === 'tool_search_output'),
		).toMatchObject({
	...
	it('excludes non-deferred tools from tool_search_output history', () => {
		const messages: Raw.ChatMessage[] = [
			...
			},
			{
				...
				content: [
					{
						type: Raw.ChatCompletionContentPartKind.Text,
						text: '["file_search","some_mcp_tool"]',
					},
				],
			},
			{
		...
		const fileSearchCall = input.find(
			(i) => i.type === 'function_call' && i.name === 'file_search',
		);
		const mcpToolCall = input.find(
			(i) => i.type === 'function_call' && i.name === 'some_mcp_tool',
		);
				...
				content: [
					{
						...
						value: {
							...
							value: { modelId, marker: statefulMarker },
						},
					},
				],
				toolCalls: [
	...
	it('preserves rich JSON Schema keywords and emits strict: false on both the inline and deferred paths (issue #315526)', () => {
		...
		const inlineReadFile = (
			inlineBody.tools as Array<{
				name?: string;
				strict?: unknown;
				parameters?: unknown;
			}>
		).find((t) => t.name === 'read_file');
		const deferredToolSearchOutput = (
			deferredBody.input as Array<{
				type?: string;
				tools?: Array<{
					name: string;
					strict?: unknown;
		...
		const deferredRunInShell = deferredToolSearchOutput?.tools?.find(
			(t) => t.name === 'run_in_shell',
		);

		expect({
			inline: {
				strict: inlineReadFile?.strict,
				parameters: inlineReadFile?.parameters,
			},
			deferred: {
