 ...
 */
function makeMockTokenResponse(body: {
...
describe('AutomodeService', () => {
	...
	beforeEach(() => {
		...
		mockAuthService = {
			getCopilotToken: vi
				.fn()
				.mockResolvedValue({ token: 'test-auth-token' }),
			onDidAuthenticationChange: vi
				.fn()
		...
		mockInstantiationService = {
			createInstance: vi
				.fn()
				.mockImplementation(
					(_ctor: any, wrappedEndpoint: IChatEndpoint) =>
						wrappedEndpoint,
				),
	...
	describe('resolveAutoModeEndpoint', () => {
		...
		it('should use router for panel chat when enabled', async () => {
			...
			(
				mockCAPIClientService.makeRequest as ReturnType<typeof vi.fn>
			).mockImplementation((_body: any, opts: any) => {
		...
		it('should include context signals in router request body', async () => {
			...
			(
				mockCAPIClientService.makeRequest as ReturnType<typeof vi.fn>
			).mockImplementation((req: any, opts: any) => {
				if (opts?.type === RequestType.ModelRouter) {
	...
	describe('model selection', () => {
		...
		it('should prefer same provider model on token refresh', async () => {
			...
			const firstResult = await automodeService.resolveAutoModeEndpoint(
				chatRequest as ChatRequest,
				[openaiEndpoint, openaiMiniEndpoint, claudeEndpoint],
			);
			...
			const secondResult = await automodeService.resolveAutoModeEndpoint(
				chatRequest as ChatRequest,
				[openaiEndpoint, openaiMiniEndpoint, claudeEndpoint],
			);
		...
		it('should fall back to first known endpoint when no available models match', async () => {
			...
			expect(mockLogService.warn).toHaveBeenCalledWith(
				expect.stringContaining(
					'No available_models matched knownEndpoints; using fallback endpoint',
				),
			);
		});
	});
	...
	describe('router fallback', () => {
		function mockRouterResponse(
			available_models: string[],
			routerResult: { chosen_model: string; candidate_models: string[] },
			session_token = 'test-token',
		): void {
			(
				mockCAPIClientService.makeRequest as ReturnType<typeof vi.fn>
		...
		it('should fall back to default selection with routerTimeout reason when router times out', async () => {
			...
			const resultPromise = automodeService.resolveAutoModeEndpoint(
				chatRequest as ChatRequest,
				[claudeEndpoint, gpt4oEndpoint],
			);
		...
		it('should fall back to default selection when router returns unknown model', async () => {
			...
			mockRouterResponse(['gpt-4o'], {
				chosen_model: 'unknown-model',
				candidate_models: ['unknown-model'],
			});
		...
		it('should skip router on subsequent turns and return cached model', async () => {
			...
			mockRouterResponse(['gpt-4o', 'gpt-4o-mini', 'claude-sonnet'], {
				chosen_model: 'gpt-4o',
				candidate_models: ['gpt-4o', 'gpt-4o-mini', 'claude-sonnet'],
			});
			...
			const firstResult = await automodeService.resolveAutoModeEndpoint(
				chatRequest1 as ChatRequest,
				[gpt4oEndpoint, gpt4oMiniEndpoint, claudeEndpoint],
			);
			expect(firstResult.model).toBe('gpt-4o');
			...
			mockRouterResponse(['gpt-4o', 'gpt-4o-mini', 'claude-sonnet'], {
				...
				candidate_models: ['claude-sonnet', 'gpt-4o-mini'],
			});
		...
		it('should re-route on subsequent turns after invalidateRouterCache', async () => {
			...
			// First turn: router picks gpt-4o
			mockRouterResponse(['gpt-4o', 'claude-sonnet'], {
				chosen_model: 'gpt-4o',
			...
			const firstResult = await automodeService.resolveAutoModeEndpoint(
				chatRequest1 as ChatRequest,
				[gpt4oEndpoint, claudeEndpoint],
			);
			expect(firstResult.model).toBe('gpt-4o');

			// Invalidate the cache (simulates compaction)
			automodeService.invalidateRouterCache({
				sessionId: 'session-no-same-provider',
			} as ChatRequest);
		...
		it('should not re-route when prompt has not changed (tool-calling iteration)', async () => {
			...
			const routerCallCount = (
				mockCAPIClientService.makeRequest as ReturnType<typeof vi.fn>
			).mock.calls.filter(
				(call: any[]) => call[1]?.type === RequestType.ModelRouter,
			).length;
		...
		it('should send has_image to router for image requests', async () => {
			...
			const routerCall = (
				...
				([, opts]) => opts?.type === RequestType.ModelRouter,
			);
				...
				if (opts?.type === RequestType.ModelRouter) {
					return Promise.resolve({
						...
						text: vi
							.fn()
							.mockResolvedValue(
								JSON.stringify({ error: 'no_vision_models' }),
							),
					});
				}
				return Promise.resolve(
		...
		it('should re-run router after invalidateRouterCache is called', async () => {
			...
			const cachedResult = await automodeService.resolveAutoModeEndpoint(
				chatRequest2 as ChatRequest,
				[gpt4oEndpoint, claudeEndpoint],
			);
			...
			const reEvalResult = await automodeService.resolveAutoModeEndpoint(
				chatRequest3 as ChatRequest,
				[gpt4oEndpoint, claudeEndpoint],
			);
	...
	describe('vision fallback', () => {
		it('should fall back to vision-capable model when selected model does not support vision', async () => {
			const nonVisionEndpoint = createEndpoint('gpt-4o-mini', 'OpenAI', {
				supportsVision: false,
			});
			const visionEndpoint = createEndpoint('gpt-4o', 'OpenAI', {
				supportsVision: true,
			});
			...
			const result = await automodeService.resolveAutoModeEndpoint(
				chatRequest as ChatRequest,
				[nonVisionEndpoint, visionEndpoint],
			);
			expect(result.model).toBe('gpt-4o');
		});
		...
		it('should keep vision-capable model when it is already selected', async () => {
			...
			const nonVisionEndpoint = createEndpoint(
				'claude-sonnet',
				'Anthropic',
				{ supportsVision: false },
			);
			...
			const result = await automodeService.resolveAutoModeEndpoint(
				chatRequest as ChatRequest,
				[visionEndpoint, nonVisionEndpoint],
			);
			expect(result.model).toBe('gpt-4o');
		});
		...
		it('should warn and keep selected model when no vision-capable model is available', async () => {
			const nonVisionEndpoint1 = createEndpoint('gpt-4o-mini', 'OpenAI', {
				supportsVision: false,
			});
			const nonVisionEndpoint2 = createEndpoint(
				'claude-sonnet',
				'Anthropic',
				{ supportsVision: false },
			);
			...
			const result = await automodeService.resolveAutoModeEndpoint(
				chatRequest as ChatRequest,
				[nonVisionEndpoint1, nonVisionEndpoint2],
			);
	...
	describe('routerModelSelection telemetry', () => {
		...
		it('should emit routerModelSelection with candidateModel and actualModel when router is used', async () => {
			...
			const telemetryCalls =
				mockTelemetryService.sendMSFTTelemetryEvent.mock.calls;
			const selectionEvent = telemetryCalls.find(
				(call: unknown[]) =>
					call[0] === 'automode.routerModelSelection',
			);
		...
		it('should emit overrideReason=clientOverride when vision fallback changes the model', async () => {
			...
			const claudeEndpoint = createEndpoint(
				'claude-sonnet',
				'Anthropic',
				{ supportsVision: false },
			);

			mockRouterResponse(['claude-sonnet', 'gpt-4o'], {
				chosen_model: 'claude-sonnet',
	...
	describe('available_models / knownEndpoints sync', () => {
		...
		it('should iterate all candidate_models when first candidate has no endpoint', async () => {
			...
			mockRouterResponse(['gpt-4.1'], {
				chosen_model: 'gpt-4.1',
				candidate_models: ['unknown-new-model', 'gpt-4.1'],
			});
		...
		it('should fall back to first known endpoint when all available_models are unknown', async () => {
			...
			(
				...
				if (opts?.type === RequestType.ModelRouter) {
					throw new Error(
						'Router should not be called when no models are routable',
					);
				}
				return Promise.resolve(
					makeMockTokenResponse({
			...
			expect(mockLogService.warn).toHaveBeenCalledWith(
				expect.stringContaining(
					'No available_models matched knownEndpoints',
				),
			);
		});
	});
