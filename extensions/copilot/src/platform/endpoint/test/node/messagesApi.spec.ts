...
function findBlock<T extends ContentBlockParam>(
...
suite('rawMessagesToMessagesAPI', function () {
	test('places cache_control on tool_result block, not inside content', function () {
		...
		if (Array.isArray(toolResult!.content)) {
			for (const inner of toolResult!.content) {
				expect(
					'cache_control' in inner ? inner.cache_control : undefined,
				).toBeUndefined();
			}
		}
	});
		...
		const messages: Raw.ChatMessage[] = [
			{
				...
				content: [
					...
					},
					{
						type: Raw.ChatCompletionContentPartKind.Image,
						imageUrl: {
							url: 'http://insecure.example.com/image.png',
						},
					},
				],
			},
	...
	suite('custom tool search tool_reference conversion', function () {
		function makeToolSearchMessages(
			toolNames: string[],
		): Raw.ChatMessage[] {
			return [
				{
		...
		test('converts tool search results into tool_reference blocks', function () {
			const messages = makeToolSearchMessages([
		...
		test('filters tool_reference blocks against validToolNames', function () {
			...
			const content = toolResult!.content as ToolReferenceBlockParam[];
			expect(content).toHaveLength(2);
			expect(content.map((c) => c.tool_name)).toEqual([
				'mcp__github__list_issues',
				'read_file',
			]);
		});
...
suite('addToolsAndSystemCacheControl', function () {
	...
	test('adds cache_control to last non-deferred tool and last system block', function () {
		...
		const system: TextBlockParam[] = [
			makeSystemBlock('You are a helpful assistant.'),
		];
	...
	test('skips deferred tools and marks last non-deferred tool', function () {
		const tools = [
			makeTool('read_file'),
			makeTool('edit_file'),
			makeTool('deferred_a', true),
			makeTool('deferred_b', true),
		];
	...
	test('does nothing when all tools are deferred and system already has cache_control', function () {
		...
		const system: TextBlockParam[] = [
			makeSystemBlock('System prompt', true),
		];
		const messagesResult = { messages: makeMessages(), system };
	...
	test('applies extended 1h ttl to tools and system when cacheTtl is "1h"', function () {
		...
		const system: TextBlockParam[] = [
			makeSystemBlock('System A'),
			makeSystemBlock('System B'),
		];
		const messagesResult = { messages: makeMessages(), system };
		...
		expect(system[1].cache_control).toEqual({
			type: 'ephemeral',
			ttl: '1h',
		});
	});
...
suite('modelSupportsExtendedCacheTtl', function () {
	test('matches Opus 4.5/4.6/4.7, Sonnet 4.5/4.6, and Haiku 4.5 variants and rejects everything else', function () {
		expect({
			'claude-opus-4.6-1m':
				modelSupportsExtendedCacheTtl('claude-opus-4.6-1m'),
			'claude-opus-4-6-1m':
				modelSupportsExtendedCacheTtl('claude-opus-4-6-1m'),
			'claude-opus-4.7-1m-internal': modelSupportsExtendedCacheTtl(
			...
			'claude-sonnet-4.6':
				modelSupportsExtendedCacheTtl('claude-sonnet-4.6'),
			'claude-sonnet-4.5':
				modelSupportsExtendedCacheTtl('claude-sonnet-4.5'),
			...
			'claude-haiku-4-5':
				modelSupportsExtendedCacheTtl('claude-haiku-4-5'),
	...
	test('matches via endpoint.family when the model id is unknown', function () {
		const fake = (family: string, model: string): IChatEndpoint =>
			({ family, model }) as unknown as IChatEndpoint;
		expect({
			'preview-id + family=claude-opus-4.7':
				modelSupportsExtendedCacheTtl(
					fake('claude-opus-4.7', 'preview-opus-internal'),
				),
			'preview-id + family=claude-sonnet-4.5':
				modelSupportsExtendedCacheTtl(
					fake('claude-sonnet-4.5', 'preview-sonnet-internal'),
				),
			'preview-id + family=claude-opus-4 (unsupported)':
				modelSupportsExtendedCacheTtl(
					fake('claude-opus-4', 'preview-opus-old'),
				),
			'preview-id + family=mystery': modelSupportsExtendedCacheTtl(
				fake('mystery-family', 'preview-anything'),
			),
		}).toEqual({
...
suite('modelSupportsMemory', function () {
	test('matches via endpoint.family when the model id is unknown', function () {
		const fake = (family: string, model: string): IChatEndpoint =>
			({ family, model }) as unknown as IChatEndpoint;
		expect({
			'preview-id + family=claude-opus-4.6': modelSupportsMemory(
				fake('claude-opus-4.6', 'preview-opus-internal'),
			),
			'preview-id + family=claude-haiku-4-5': modelSupportsMemory(
				fake('claude-haiku-4-5', 'preview-haiku-internal'),
			),
			'preview-id + family=mystery': modelSupportsMemory(
				fake('mystery-family', 'preview-anything'),
			),
		}).toEqual({
...
suite('isExtendedCacheTtlEnabled', function () {
	...
	test('returns false for non-Agent chat locations', function () {
		...
		enableConfig();
		expect({
			Panel: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
				ChatLocation.Panel,
			...
			Editor: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
				ChatLocation.Editor,
			...
			Terminal: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
				ChatLocation.Terminal,
				false,
			),
			Notebook: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
			...
			EditingSession: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
			...
			Other: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
				ChatLocation.Other,
			...
			MessagesProxy: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
				ChatLocation.MessagesProxy,
				false,
			),
			ResponsesProxy: isExtendedCacheTtlEnabled(
				ELIGIBLE_MODEL,
				configurationService,
				experimentationService,
				ChatLocation.ResponsesProxy,
				false,
			),
		}).toEqual({
			Panel: false,
...
suite('addMessagesApiCacheControl', function () {
	function ccPositions(args: {
		messages: MessageParam[];
		system?: TextBlockParam[];
		tools?: AnthropicMessagesTool[];
	}): string[] {
		const out: string[] = [];
		args.tools?.forEach((t, i) => {
			if (t.cache_control) {
				out.push(`tools[${i}]`);
			}
		});
		args.system?.forEach((b, i) => {
			if (b.cache_control) {
				out.push(`system[${i}]`);
			}
		});
	...
	test('marks the last cacheable block of the two most recent cacheable messages', function () {
		...
		expect(ccPositions({ messages })).toEqual([
			'messages[1].block[2]',
			'messages[2].block[1]',
		]);
	});
	...
	test('handles single message and empty array', function () {
		const single: MessageParam[] = [
			{ role: 'user', content: [{ type: 'text', text: 'hi' }] },
		];
		addMessagesApiCacheControl({ messages: single });
		expect(ccPositions({ messages: single })).toEqual([
			'messages[0].block[0]',
		]);
	...
	test('cc anchors shift forward each iteration of an agent loop', function () {
		...
		const r1Use: ContentBlockParam = {
			type: 'tool_use',
			id: 'a',
			name: 't',
			input: {},
		};
		const r1Res: ContentBlockParam = {
			type: 'tool_result',
			tool_use_id: 'a',
			content: 'r1',
		};
		const r2Use: ContentBlockParam = {
			type: 'tool_use',
			id: 'b',
			name: 't',
			input: {},
		};
		const r2Res: ContentBlockParam = {
			type: 'tool_result',
			tool_use_id: 'b',
			content: 'r2',
		};
		...
		expect(ccPositions({ messages: iterA })).toEqual([
			'messages[1].block[0]',
			'messages[2].block[0]',
		]);
		...
		expect(ccPositions({ messages: iterB })).toEqual([
			'messages[3].block[0]',
			'messages[4].block[0]',
		]);
	});
	...
	test('propagates cacheTtl to the emitted cache_control blocks', function () {
		...
		expect({
			withTtl: collect(withTtl),
			withoutTtl: collect(withoutTtl),
		}).toEqual({
			withTtl: [
...
describe('createMessagesRequestBody reasoning effort', () => {
	...
	function createMinimalOptions(
		overrides: Partial<ICreateEndpointBodyOptions> = {},
	): ICreateEndpointBodyOptions {
		return {
			debugName: 'test',
	...
	test('includes effort in output_config when model supports reasoning effort and thinking is adaptive', () => {
		...
		const body = instantiationService.invokeFunction(
			createMessagesRequestBody,
			options,
			endpoint.model,
			endpoint,
...
describe('createMessagesRequestBody tool search deferral', () => {
	...
	test('defers MCP tools when tool_search is in the request tool list', () => {
		...
		const tools = body.tools as AnthropicMessagesTool[];
		expect(
			tools.find((t) => t.name === 'read_file')?.defer_loading,
		).toBeUndefined();
		expect(
			tools.find((t) => t.name === CUSTOM_TOOL_SEARCH_NAME)
				?.defer_loading,
		).toBeUndefined();
		expect(
			tools.find((t) => t.name === 'some_mcp_tool')?.defer_loading,
		).toBe(true);
	});
...
function createNonStreamingHeaders(
	contentType = 'application/json',
): HeadersImpl {
	return HeadersImpl.fromMap(
		new Map([
...
function createNonStreamingResponse(
	...
	return Response.fromText(
		200,
		'OK',
		createNonStreamingHeaders(contentType),
		JSON.stringify(body),
		'node-fetch',
	);
}
...
suite('processNonStreamingResponseFromMessagesEndpoint', () => {
	test('parses text content from non-streaming response', async () => {
		...
		const completions =
			await processNonStreamingResponseFromMessagesEndpoint(
		...
		expect(results[0].message.content[0]).toEqual({
			type: Raw.ChatCompletionContentPartKind.Text,
			text: 'Hello world',
		});
	...
	test('handles error responses gracefully', async () => {
		...
		await expect(async () => {
			for await (const _c of completions) {
				/* consume */
			}
	...
	test('surfaces 1h/5m cache_creation split when present', async () => {
		...
		expect(
			details?.anthropic_cache_creation?.ephemeral_1h_input_tokens,
		).toBe(17);
		expect(
			details?.anthropic_cache_creation?.ephemeral_5m_input_tokens,
		).toBe(8);
	});
	...
	test('surfaces thinking_tokens as completion_tokens_details.reasoning_tokens', async () => {
		...
		expect(
			results[0].usage?.completion_tokens_details?.reasoning_tokens,
		).toBe(580);
	});
	...
	test('rejects on malformed JSON', async () => {
		const response = Response.fromText(
			200,
			'OK',
			createNonStreamingHeaders(),
			'not json at all',
			'node-fetch',
		);
		const telemetryData = TelemetryData.createAndMarkAsIssued();
...
suite('processResponseFromMessagesEndpoint routing', () => {
	test('routes non-streaming content-type to non-streaming handler', async () => {
		...
		const response = Response.fromText(
			200,
			'OK',
			createNonStreamingHeaders('application/json'),
			JSON.stringify(body),
			'node-fetch',
		);
		const telemetryData = TelemetryData.createAndMarkAsIssued();
