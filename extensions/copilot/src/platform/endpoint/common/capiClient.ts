...
export abstract class BaseCAPIClientService
	extends CAPIClient
	implements ICAPIClientService
{
	readonly _serviceBrand: undefined;
	...
	override makeRequest<T>(
		request: MakeRequestOptions,
		...
		if (this.abExpContext) {
			...
			request.headers['X-Copilot-Client-Exp-Assignment-Context'] =
				this.abExpContext;
		}
...
export const ICAPIClientService =
	createServiceIdentifier<ICAPIClientService>('ICAPIClientService');
