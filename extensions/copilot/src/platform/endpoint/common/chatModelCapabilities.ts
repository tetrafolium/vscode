...
export function getModelCapabilityOverride(
	...
	const map = configurationService.getConfig(
...
export function isHiddenModelB(
	model: LanguageModelChat | IChatEndpoint | string,
) {
	const h = getCachedSha256Hash(
		typeof model === 'string' ? model : model.family,
	);
...
export function isHiddenModelM(
	model: LanguageModelChat | IChatEndpoint | string,
) {
	const family_hash = getCachedSha256Hash(
		typeof model === 'string' ? model : model.family,
	);
	return HIDDEN_FAMILY_M_HASHES.includes(family_hash);
}

export function isGpt53Codex(
	model: LanguageModelChat | IChatEndpoint | string,
) {
	const family = typeof model === 'string' ? model : model.family;
...
export function isVSCModelA(model: LanguageModelChat | IChatEndpoint) {
	...
	const family_hash = getCachedSha256Hash(model.family);
	return (
		VSC_MODEL_HASHES_A.includes(ID_hash) ||
		VSC_MODEL_HASHES_A.includes(family_hash)
	);
}
...
export function isVSCModelB(model: LanguageModelChat | IChatEndpoint) {
	...
	const family_hash = getCachedSha256Hash(model.family);
	return (
		VSC_MODEL_HASHES_B.includes(ID_hash) ||
		VSC_MODEL_HASHES_B.includes(family_hash)
	);
}

export function isVSCModelReplaceStringSet(
	model: LanguageModelChat | IChatEndpoint,
) {
	const ID_hash = getCachedSha256Hash(getModelId(model));
	const family_hash = getCachedSha256Hash(model.family);
	return (
		VSC_MODEL_HASHES_EDIT_TOOL_SET.includes(ID_hash) ||
		VSC_MODEL_HASHES_EDIT_TOOL_SET.includes(family_hash)
	);
}
...
export function isVSCModelC(model: LanguageModelChat | IChatEndpoint) {
	...
	const family_hash = getCachedSha256Hash(model.family);
	return (
		VSC_MODEL_HASHES_C.includes(ID_hash) ||
		VSC_MODEL_HASHES_C.includes(family_hash)
	);
}
...
export function isVSCModelD(model: LanguageModelChat | IChatEndpoint) {
	...
	const family_hash = getCachedSha256Hash(model.family);
	return (
		VSC_MODEL_HASHES_D.includes(ID_hash) ||
		VSC_MODEL_HASHES_D.includes(family_hash)
	);
}

export function isGpt52CodexFamily(
	model: LanguageModelChat | IChatEndpoint | string,
): boolean {
	const family = typeof model === 'string' ? model : model.family;
 ...
 */
export function modelSupportsApplyPatch(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	...
	return (
		(model.family.startsWith('gpt') && !model.family.includes('gpt-4o')) ||
 ...
 */
export function modelPrefersJsonNotebookRepresentation(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return (
		(model.family.startsWith('gpt') && !model.family.includes('gpt-4o')) ||
 ...
 */
export function modelSupportsReplaceString(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return (
		isGeminiFamily(model) ||
		model.family.includes('grok-code') ||
		modelSupportsMultiReplaceString(model) ||
		isHiddenModelF(model) ||
		isMinimaxFamily(model) ||
		isHiddenFamilyH(model)
	);
}
 ...
 */
export function modelSupportsMultiReplaceString(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return (
		isAnthropicFamily(model) ||
		isHiddenModelE(model) ||
		isVSCModelReplaceStringSet(model) ||
		isMinimaxFamily(model) ||
		isHiddenFamilyH(model)
	);
}
 ...
 */
export function modelCanUseReplaceStringExclusively(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return (
		isAnthropicFamily(model) ||
 ...
 */
export function modelShouldUseReplaceStringHealing(
	model: LanguageModelChat | IChatEndpoint,
) {
 ...
 */
export function modelCanUseMcpResultImageURL(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
 ...
 */
export function modelCanUseImageURL(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return true;
}
 ...
 * without needing insert_edit_into_file.
 */
export function modelCanUseApplyPatchExclusively(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	// only using replace string as edit tool, disable apply_patch for VSC Models
	if (isVSCModelReplaceStringSet(model)) {
 ...
 * replace_string.
 */
export function modelNeedsStrongReplaceStringHint(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
 ...
 */
export function modelSupportsSimplifiedApplyPatchInstructions(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return isGpt5PlusFamily(model) || isVSCModelA(model) || isVSCModelB(model);
}

export function isAnthropicFamily(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return (
		model.family.startsWith('claude') ||
		model.family.startsWith('Anthropic')
	);
}

export function isGeminiFamily(
	model: LanguageModelChat | IChatEndpoint | string,
): boolean {
	const family = typeof model === 'string' ? model : model.family;
	return (
		family.toLowerCase().startsWith('gemini') ||
		getCachedSha256Hash(family) === HIDDEN_MODEL_K_HASH
	);
}

export function isMinimaxFamily(
	model: LanguageModelChat | IChatEndpoint,
): boolean {
	return model.family.toLowerCase().includes('minimax');
}

export function isGpt5PlusFamily(
 ...
 */
export function isGptCodexFamily(
	model: LanguageModelChat | IChatEndpoint | string | undefined,
): boolean {
	if (!model) {
		return false;
 ...
 */
export function getVerbosityForModelSync(
	model: IChatEndpoint,
 ...
 */
export function modelSupportsToolSearch(
	model: LanguageModelChat | IChatEndpoint | string,
): boolean {
 ...
 */
export function modelSupportsContextEditing(
	model: LanguageModelChat | IChatEndpoint | string,
): boolean {
	const id = typeof model === 'string' ? model : getModelId(model);
	const family = typeof model === 'string' ? model : model.family;
