...
} from '../common/setupTestExtensions';
...
export interface ISetupTestsDetector {
	...
	shouldSuggestSetup(
		...
		output: vscode.ChatResponseStream,
	): Promise<SetupTestAction | undefined>;
...
export const ISetupTestsDetector = createServiceIdentifier<ISetupTestsDetector>(
	'ISetupTestsDetector',
);
...
export type SetupTestAction =
	...
	| {
			type: SetupTestActionType.CustomExtensionCommand;
			message: string;
			command?: vscode.Command;
	  };
...
export const isStartSetupTestConfirmation = (confirmationData: any) =>
	confirmationData &&
	confirmationData.$isSetupSuggestion &&
	confirmationData.command === CommandIds.OpenChat;
...
export class SetupTestsDetector implements ISetupTestsDetector {
	...
	constructor(
		...
		@ITestDepsResolver
		private readonly _testDepsResolver: ITestDepsResolver,
		...
		@IExtensionsService
		private readonly _extensionsService: IExtensionsService,
	...
	public showSuggestion(
		...
		const frameworkQuery = (framework: string) =>
		...
		switch (action.type) {
			...
			case SetupTestActionType.InstallExtensionForLanguage:
				output.push(
					new ChatResponseConfirmationPart(
						l10n.t(
							'We recommend installing an extension to run {0} tests.',
							action.type ===
								SetupTestActionType.InstallExtensionForFramework
								? action.framework
								: action.language,
						),
						l10n.t(
			...
			case SetupTestActionType.Remind: {
				...
				switch (action2.type) {
					...
					case SetupTestActionType.InstallExtensionForLanguage: {
						...
						output.push(
							new ChatResponseExtensionsPart([
								action2.extension.id,
							]),
						);
						break;
					}
					case SetupTestActionType.SearchForFramework: {
						const s = new MarkdownString(
							l10n.t(
								...
								commandUri('workbench.extensions.search', [
									frameworkQuery(action2.framework),
								]),
								action2.framework,
							),
						);
						s.isTrusted = {
							enabledCommands: ['workbench.extensions.search'],
						};
						output.push(new ChatResponseMarkdownPart(s));
						break;
					}
	 ...
	 */
	public async shouldSuggestSetup(
		{ document }: IDocumentContext,
		request: vscode.ChatRequest,
		output: vscode.ChatResponseStream,
	): Promise<SetupTestAction | undefined> {
		if (
			request.rejectedConfirmationData?.some((r) => r.$isSetupSuggestion)
		) {
			return undefined; // said "not now" to setup
		}

		const confirmed = request.acceptedConfirmationData?.find(
			(r) => r.$isSetupSuggestion,
		);
		if (confirmed) {
			const exe = this._commandService.executeCommand(
				confirmed.command,
				...confirmed.arguments,
			);
		...
		if (
			!this._configurationService.getConfig(ConfigKey.SetupTests) ||
			(await this._testService.hasAnyTests())
		) {
			return undefined;
		}
	...
	private async waitForExtensionInstall(
		prom: Promise<void>,
		...
		const testSection = (extension.packageJSON as IPackageJson)?.copilot
			?.tests;
		const command =
			testSection?.setupTests || testSection?.getSetupConfirmation;
		return command
			? await this.getDelegatedAction(command, document)
			: undefined;
	}

	private getDidAlreadyPrompt() {
		if (
			this._extensionContext.extensionMode === ExtensionMode.Development
		) {
	...
	private async getDelegatedAction(
		command: string,
		doc: TextDocumentSnapshot,
	): Promise<SetupTestAction | undefined> {
		try {
			const result: SetupConfirmationResult | undefined =
				await this._commandService.executeCommand(command, doc.uri);
			if (result) {
				return {
					type: SetupTestActionType.CustomExtensionCommand,
	...
	private async getExtensionRecommendationAndDelegate(
		extensionInfo: ISetupTestExtension,
		doc: TextDocumentSnapshot,
		ifNotInstalledThen: SetupTestAction,
	): Promise<SetupTestAction | undefined> {
		const extension = this._extensionsService.getExtension(
			extensionInfo.id,
		);
		if (!extension) {
		...
		return command
			? await this.getDelegatedAction(command, doc)
			: undefined;
	}

	private async getSuggestActionInner(
		doc: TextDocumentSnapshot,
	): Promise<SetupTestAction | undefined> {
		...
		if (languageExt) {
			return this.getExtensionRecommendationAndDelegate(
				languageExt,
				doc,
				{
					type: SetupTestActionType.InstallExtensionForLanguage,
		...
		const frameworks = await this._testDepsResolver.getTestDeps(
			doc.languageId,
		);
