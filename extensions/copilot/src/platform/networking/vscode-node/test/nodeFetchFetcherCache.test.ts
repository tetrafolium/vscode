...
suite('NodeFetchFetcher cache - integration', function () {
	...
	suiteSetup(async function () {
		if (
			typeof (globalThis as { __vscodeCreateFetchPatch?: unknown })
				.__vscodeCreateFetchPatch !== 'function'
		) {
			this.skip();
		}
	...
	test('serves the second cache-opted request from the in-memory store', async () => {
		const first = await fetcher.fetch(`${origin}/cacheable?key=hit-miss`, {
			callSite: 'test',
			cache: true,
		});
		const firstBody = await first.json();
		const second = await fetcher.fetch(`${origin}/cacheable?key=hit-miss`, {
			callSite: 'test',
			cache: true,
		});
		...
		assert.deepStrictEqual(
			secondBody,
			firstBody,
			'cached body should match origin body',
		);
		assert.notStrictEqual(
			second.headers.get('age'),
			null,
			'cache hit should carry age header',
		);
	});

	test('leaves requests uncached when the caller does not opt in', async () => {
		const first = await fetcher.fetch(`${origin}/cacheable?key=no-opt-in`, {
			callSite: 'test',
		});
		await first.text();
		const second = await fetcher.fetch(
			`${origin}/cacheable?key=no-opt-in`,
			{ callSite: 'test' },
		);
		await second.text();
		...
		assert.strictEqual(
			originHits,
			2,
			'every request should hit the origin',
		);
	...
	test('respects cache-control: no-store from the origin', async () => {
		const first = await fetcher.fetch(`${origin}/no-store?key=no-store`, {
			callSite: 'test',
			cache: true,
		});
		await first.text();
		const second = await fetcher.fetch(`${origin}/no-store?key=no-store`, {
			callSite: 'test',
			cache: true,
		});
		await second.text();
		...
		assert.strictEqual(
			originHits,
			2,
			'no-store responses must not be served from cache',
		);
		assert.strictEqual(first.cacheStatus, 'miss');
	...
	test('does not cache POST requests', async () => {
		const first = await fetcher.fetch(`${origin}/cacheable?key=post`, {
			callSite: 'test',
			cache: true,
			method: 'POST',
			body: '{}',
		...
		await first.text();
		const second = await fetcher.fetch(`${origin}/cacheable?key=post`, {
			callSite: 'test',
			cache: true,
			method: 'POST',
			body: '{}',
		...
		assert.strictEqual(
			originHits,
			2,
			'POST requests must always reach the origin',
		);
		assert.strictEqual(first.cacheStatus, 'miss');
		assert.strictEqual(second.cacheStatus, 'miss');
	});
	...
	test('caches different URLs independently', async () => {
		const a1 = await fetcher.fetch(`${origin}/cacheable?key=urlA`, {
			callSite: 'test',
			cache: true,
		});
		await a1.text();
		const b1 = await fetcher.fetch(`${origin}/cacheable?key=urlB`, {
			callSite: 'test',
			cache: true,
		});
		await b1.text();
		const a2 = await fetcher.fetch(`${origin}/cacheable?key=urlA`, {
			callSite: 'test',
			cache: true,
		});
		await a2.text();
		const b2 = await fetcher.fetch(`${origin}/cacheable?key=urlB`, {
			callSite: 'test',
			cache: true,
		});
		await b2.text();
		...
		assert.strictEqual(
			originHits,
			2,
			'each distinct URL should hit the origin once',
		);
	...
	test('keys cache entries by query string', async () => {
		const v1 = await fetcher.fetch(`${origin}/cacheable?v=1`, {
			callSite: 'test',
			cache: true,
		});
		await v1.text();
		const v2 = await fetcher.fetch(`${origin}/cacheable?v=2`, {
			callSite: 'test',
			cache: true,
		});
		await v2.text();
		...
		assert.strictEqual(
			originHits,
			2,
			'different query strings should not collide',
		);
	...
	test('preserves status, body and response headers on a cache hit', async () => {
		const first = await fetcher.fetch(`${origin}/cacheable?key=headers`, {
			callSite: 'test',
			cache: true,
		});
		const firstBody = await first.json();
		const second = await fetcher.fetch(`${origin}/cacheable?key=headers`, {
			callSite: 'test',
			cache: true,
		});
		const secondBody = await second.json();
	...
	test('caches Cache-Control: private responses in the private store', async () => {
		const first = await fetcher.fetch(`${origin}/private?key=private`, {
			callSite: 'test',
			cache: true,
		});
		await first.text();
		const second = await fetcher.fetch(`${origin}/private?key=private`, {
			callSite: 'test',
			cache: true,
		});
		await second.text();
		...
		assert.strictEqual(
			originHits,
			1,
			'private responses should be cached in the private store',
		);
		assert.strictEqual(first.cacheStatus, 'miss');
		assert.strictEqual(second.cacheStatus, 'hit');
	});
	...
	test('reports revalidated when the origin returns 304 Not Modified', async () => {
		const first = await fetcher.fetch(`${origin}/etag?key=etag`, {
			callSite: 'test',
			cache: true,
		});
		const firstBody = await first.json();
		const second = await fetcher.fetch(`${origin}/etag?key=etag`, {
			callSite: 'test',
			cache: true,
		...
		assert.strictEqual(
			originHits,
			2,
			'a 304 still counts as an origin request',
		);
		assert.strictEqual(first.cacheStatus, 'miss');
		...
		assert.strictEqual(
			second.status,
			200,
			'revalidated response is served with the cached 200 status',
		);
		assert.deepStrictEqual(
			secondBody,
			firstBody,
			'revalidated response carries the cached body',
		);
	});

	test('coalesces or caches concurrent requests so the second is not an extra miss', async () => {
		const [a, b] = await Promise.all([
			fetcher.fetch(`${origin}/cacheable?key=concurrent`, {
				callSite: 'test',
				cache: true,
			}),
			fetcher.fetch(`${origin}/cacheable?key=concurrent`, {
		...
		const third = await fetcher.fetch(
			`${origin}/cacheable?key=concurrent`,
			{ callSite: 'test', cache: true },
		);
		await third.text();
		assert.ok(
			originHits <= 2,
			`origin hit at most twice for concurrent fetches, got ${originHits}`,
		);
