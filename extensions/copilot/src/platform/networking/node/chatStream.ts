...
export function sendEngineMessagesLengthTelemetry(
	...
	if (!modelCallId) {
		...
		logService?.warn(
			'[TELEMETRY] modelCallId not found in telemetryData, input/output messages cannot be linked',
		);
		return;
	}
	...
	telemetryService.sendEnhancedGHTelemetryEvent(
		...
		multiplexProperties(telemetryDataWithPrompt.properties),
		telemetryDataWithPrompt.measurements,
	);
...
function sendModelRequestOptionsTelemetry(
	telemetryService: ITelemetryService,
	telemetryData: TelemetryData,
	logService?: ILogService,
): string | undefined {
	...
	const conversationId =
		...
		telemetryData.properties.sessionId ||
		'unknown';
	const headerRequestId =
		telemetryData.properties.headerRequestId || 'unknown';
	...
	for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
		const requestOptionsData = TelemetryData.createAndMarkAsIssued(
			...
			telemetryData.measurements,
		); // Include measurements from original telemetryData

		telemetryService.sendInternalMSFTTelemetryEvent(
			'model.request.options.added',
			requestOptionsData.properties,
			requestOptionsData.measurements,
		);
	}
...
function sendNewRequestAddedTelemetry(
	telemetryService: ITelemetryService,
	telemetryData: TelemetryData,
	logService?: ILogService,
): void {
	...
	if (!conversationId && mainHeaderRequestIdTracker.headerRequestId) {
		const mostRecentTurn = headerRequestIdTracker.get(
			mainHeaderRequestIdTracker.headerRequestId,
		);
		filteredProperties.mostRecentConversationHeaderRequestId =
			mainHeaderRequestIdTracker.headerRequestId;
		if (mostRecentTurn !== undefined) {
			filteredProperties.mostRecentConversationHeaderRequestIdTurn =
				mostRecentTurn.toString();
		}
	}

	// Create telemetry data for the request
	const requestData = TelemetryData.createAndMarkAsIssued(
		filteredProperties,
		telemetryData.measurements,
	);
...
function sendIndividualMessagesTelemetry(
	telemetryService: ITelemetryService,
	messages: CAPIChatMessage[],
	telemetryData: TelemetryData,
	messageDirection: 'input' | 'output',
	logService?: ILogService,
): Array<{ uuid: string; headerRequestId: string }> {
...
function sendModelCallTelemetry(
	telemetryService: ITelemetryService,
	messageData: Array<{ uuid: string; headerRequestId: string }>,
	telemetryData: TelemetryData,
	messageDirection: 'input' | 'output',
	logService?: ILogService,
) {
	...
	if (messageDirection === 'input') {
		requestOptionsId = sendModelRequestOptionsTelemetry(
			telemetryService,
			telemetryData,
			logService,
		);
	...
	for (const [headerRequestId, messageUuids] of messagesByHeaderRequestId) {
		...
		for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
			...
			const parentHeaderRequestId =
				telemetryData.properties.parentHeaderRequestId;
			const parentModelCallId =
				telemetryData.properties.parentModelCallId;
			const modelCallData = TelemetryData.createAndMarkAsIssued(
				{
					...
					...(requestTurn !== undefined && {
						requestTurn: requestTurn.toString(),
					}), // Add requestTurn only for input calls
					...(requestOptionsId && { requestOptionsId }), // Add requestOptionsId for input calls
					...(telemetryData.properties.turnIndex && {
						turnIndex: telemetryData.properties.turnIndex,
					}), // Add turnIndex from original telemetryData
					...(telemetryData.properties.iterationNumber && {
						iterationNumber:
							telemetryData.properties.iterationNumber,
					}), // Add iterationNumber from tool calling loop
			...
			telemetryService.sendInternalMSFTTelemetryEvent(
				eventName,
				modelCallData.properties,
				modelCallData.measurements,
			);
		}
	}
}

function sendModelTelemetryEvents(
	telemetryService: ITelemetryService,
	messages: CAPIChatMessage[],
	telemetryData: TelemetryData,
	isOutput: boolean,
	...
	if (
		...
		(messageSource && messageSource.startsWith('api.'))
	) {
		return;
	}
	...
	const isRetryRequest =
		telemetryData.properties.retryAfterFilterCategory !== undefined;
	...
	const messageData = sendIndividualMessagesTelemetry(
		telemetryService,
		messages,
		telemetryData,
...
export function sendEngineMessagesTelemetry(
	telemetryService: ITelemetryService,
	messages: CAPIChatMessage[],
	telemetryData: TelemetryData,
	isOutput: boolean,
	...
	sendEngineMessagesLengthTelemetry(
		telemetryService,
		messages,
		telemetryData,
		isOutput,
...
export function prepareChatCompletionForReturn(
	...
	if (c.finishOffset !== undefined) {
		...
		logService.debug(
			`message ${c.index}: early finish at offset ${c.finishOffset}`,
		);
	...
	logService.info(
		`message ${c.index} returned. finish reason: [${c.reason}]`,
	);
	logService.debug(
