...
type CacheInterceptorOptions = NonNullable<
	Parameters<typeof undici.interceptors.cache>[0]
>;
...
export class NodeFetchFetcher extends BaseFetchFetcher {
	...
	constructor(
		...
		const factory = (globalThis as any).__vscodeCreateFetchPatch as
			| FetchPatchFactory
			| undefined;
		const interceptor =
			cacheOptions.mode !== 'off' && factory
				? createCacheInterceptor(cacheOptions)
				: undefined;
		super(
			getFetch(interceptor, factory),
			envService,
			NodeFetchFetcher.ID,
			reportEvent,
...
function createCacheInterceptor(
	options: NodeFetchCacheOptions,
): undici.Dispatcher.DispatcherComposeInterceptor | undefined {
...
function createCacheStore(
	...
	if (options.mode === 'persistent') {
		const SqliteCacheStore = (
			undici as unknown as {
				cacheStores?: {
					SqliteCacheStore?: new (init?: object) => CacheStore;
				};
			}
		).cacheStores?.SqliteCacheStore;
	...
	const MemoryCacheStore = (
		undici as unknown as {
			cacheStores?: {
				MemoryCacheStore?: new (init?: object) => CacheStore;
			};
		}
	).cacheStores?.MemoryCacheStore;
	if (!MemoryCacheStore) {
		return undefined;
	}
...
function getFetch(
	cacheInterceptor:
		...
		| undefined,
	createFetchPatch: FetchPatchFactory | undefined,
): FetchImpl {
	const defaultFetch =
		(globalThis as any).__vscodePatchedFetch || globalThis.fetch;
	const cachedFetch =
		cacheInterceptor && createFetchPatch
			? createFetchPatch({ interceptors: [cacheInterceptor] })
			: undefined;
	return function (input, init, useCache) {
		...
		const dispatcher =
			(init as { dispatcher?: undici.Dispatcher } | undefined)
				?.dispatcher ?? agent.value;
...
export function createWebSocket(
	...
	wsAgent.dispatch = function (
		dispatchOptions: undici.Dispatcher.DispatchOptions,
		...
		const wrappedHandler: undici.Dispatcher.DispatchHandler = {
			...
			onUpgrade(
				...
				if (rawHeaders) {
					responseHeaders = HeadersImpl.fromMap(
	...
	return {
		...
		get responseHeaders() {
			...
			return wsResponseHeaders
				? new HeadersImpl(wsResponseHeaders)
				: responseHeaders;
		},
		get responseStatusCode() {
			return (
				(webSocket as { responseStatusCode?: number })
					.responseStatusCode ?? responseStatusCode
			);
		},
		get responseStatusText() {
			return (
				(webSocket as { responseStatusText?: string })
					.responseStatusText ?? responseStatusText
			);
		},
		get networkError() {
			return (
				(webSocket as { networkError?: Error }).networkError ??
				undefined
			);
		},
	};
...
function parseRawHeaders(
	rawHeaders: readonly (Buffer | string)[],
): Map<string, string> {
