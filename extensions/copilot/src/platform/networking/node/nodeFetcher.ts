...
export class NodeFetcher implements IFetcher {
	...
	async fetch(url: string, options: FetchOptions): Promise<Response> {
		...
		headers[userAgentLibraryHeader] = this._userAgentLibraryUpdate
			? this._userAgentLibraryUpdate(this.getUserAgentLibrary())
	...
	private _fetch(
		...
		return new Promise((resolve, reject) => {
			...
			const req = module.request(url, { method, headers }, (res) => {
				...
				const nodeFetcherResponse = new NodeFetcherResponse(
					req,
					res,
					signal,
				);
