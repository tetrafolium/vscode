...
export class ConfigurationServiceImpl extends AbstractConfigurationService {
		...
		vscode.workspace.onDidChangeConfiguration((changeEvent) => {
			...
			this._onDidChangeConfiguration.fire({
				affectsConfiguration: (
					...
					const oldId =
						globalConfigRegistry.configs.get(
							section,
						)?.fullyQualifiedOldId;
					if (
						oldId &&
						changeEvent.affectsConfiguration(oldId, scope)
					) {
						return true;
					}
					return false;
	...
	getConfig<T>(key: Config<T>, scope?: vscode.ConfigurationScope): T {
		...
		const config =
			scope === undefined
				? this.config
				: vscode.workspace.getConfiguration(CopilotConfigPrefix, scope);
		...
		if (key.advancedSubKey) {
			...
			if (advancedConfigFlatStyleValue !== undefined) {
				...
				const advancedConfig =
					config.get<Record<string, any>>('advanced');
			...
			if (key.oldId && key.oldId.startsWith('chat.advanced')) {
				...
				if (this.isConfigured(key, scope)) {
					configuredValue =
						config.get<T>(key.id) ??
						(key.oldId ? config.get<T>(key.oldId) : undefined);
				}
			} else {
				configuredValue =
					config.get<T>(key.id) ??
		...
		if (value.error) {
			console.error(
				`Could not read "${key.fullyQualifiedId}": ${value.error.message}`,
			);
			return this.getDefaultValue(key);
		}
	...
	inspectConfig<T>(
		...
		const languageIds = distinct([
			...(inspectResult?.languageIds ?? []),
			...(oldInspectResult?.languageIds ?? []),
		]);
		return {
			defaultValue:
				inspectResult?.defaultValue ?? oldInspectResult?.defaultValue,
			globalValue:
				inspectResult?.globalValue ?? oldInspectResult?.globalValue,
			workspaceValue:
				inspectResult?.workspaceValue ??
				oldInspectResult?.workspaceValue,
			workspaceFolderValue:
				inspectResult?.workspaceFolderValue ??
				oldInspectResult?.workspaceFolderValue,
			defaultLanguageValue:
				inspectResult?.defaultLanguageValue ??
				oldInspectResult?.defaultLanguageValue,
			globalLanguageValue:
				inspectResult?.globalLanguageValue ??
				oldInspectResult?.globalLanguageValue,
			workspaceLanguageValue:
				inspectResult?.workspaceLanguageValue ??
				oldInspectResult?.workspaceLanguageValue,
			workspaceFolderLanguageValue:
				inspectResult?.workspaceFolderLanguageValue ??
				oldInspectResult?.workspaceFolderLanguageValue,
	...
	private _getTargetFromInspect(inspect?: {
		key: string;
	...
	private _toVSCodeConfigurationTarget(
		target: ConfigTarget,
	): vscode.ConfigurationTarget {
		switch (target) {
	...
	async setConfig<T>(
		...
		if (key.advancedSubKey) {
			...
			if (hasFlatStyle) {
				throw new Error(
					`Cannot write to "${key.fullyQualifiedId}". Please update the setting manually to ${JSON.stringify(value)}.`,
				);
			}
			...
			return this.config.update(
				...
				currentValue,
				target === undefined
					? this._getTargetFromInspect(
							this.config.inspect('advanced'),
						)
					: this._toVSCodeConfigurationTarget(target),
			);
		}
		...
		return this.config.update(
			...
			value,
			target === undefined
				? this._getTargetFromInspect(inspect)
				: this._toVSCodeConfigurationTarget(target),
		);
	}
	...
	override getExperimentBasedConfig<T extends ExperimentBasedConfigType>(
		...
		const configuredValue =
			this._getUserConfiguredValueForExperimentBasedConfig(key, scope);
		if (configuredValue !== undefined) {
			return configuredValue;
		}

		if (key.experimentName) {
			const expValue = experimentationService.getTreatmentVariable<
				Exclude<T, undefined>
			>(key.experimentName);
		...
		const expValue2 = experimentationService.getTreatmentVariable<
			Exclude<T, undefined>
		...
		if (key.fullyQualifiedOldId) {
			const oldExpValue = experimentationService.getTreatmentVariable<
				Exclude<T, undefined>
			>(`copilotchat.config.${key.oldId}`);
			if (oldExpValue !== undefined) {
				return oldExpValue;
			}

			const oldExpValue2 = experimentationService.getTreatmentVariable<
				Exclude<T, undefined>
			>(`config.${key.fullyQualifiedOldId}`);
	...
	private _getUserConfiguredValueForExperimentBasedConfig<
			...
			for (const key in extensionConfigProps) {
				...
				if (typeof value === 'object' && value !== null) {
					...
					Object.keys(value)
						...
						.forEach(
							(k) =>
								(configProperties[`${key}.${k}`] =
									stringOrStringify(value[k])),
						);
				} else {
	...
	override updateExperimentBasedConfiguration(treatments: string[]): void {
		...
		this._onDidChangeConfiguration.fire({
			affectsConfiguration: (
				...
				if (
					oldId &&
					treatments.some((t) => t.startsWith(`config.${oldId}`))
				) {
					return true;
				}
				return false;
	...
	protected override getDefaultValueForConfig<T>(
		key: BaseConfig<T>,
	): T | undefined {
		const inspect =
			this.config.inspect<T>(key.id) ??
			(key.oldId ? this.config.inspect<T>(key.oldId) : undefined);
