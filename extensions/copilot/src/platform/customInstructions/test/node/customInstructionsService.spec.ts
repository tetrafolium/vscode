...
suite('CustomInstructionsService - Skills', () => {
	...
	beforeEach(async () => {
		...
		services.define(
			IWorkspaceService,
			new SyncDescriptor(TestWorkspaceService, [workspaceFolders, []]),
		);
		...
		fileSystemService = accessor.get(
			IFileSystemService,
	...
	suite('getSkillInfo', () => {
		test('should return skill info for file in .github/skills folder', () => {
			const skillFileUri = URI.file(
				'/workspace/.github/skills/myskill/SKILL.md',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(skillFileUri);

			expect(skillInfo).toBeDefined();
			expect(skillInfo?.skillName).toBe('myskill');
			expect(skillInfo?.skillFolderUri.toString()).toBe(
				URI.file('/workspace/.github/skills/myskill').toString(),
			);
		});

		test('should return skill info for file in .claude/skills folder', () => {
			const skillFileUri = URI.file(
				'/workspace/.claude/skills/myskill/SKILL.md',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(skillFileUri);
			...
			expect(skillInfo?.skillName).toBe('myskill');
			expect(skillInfo?.skillFolderUri.toString()).toBe(
				URI.file('/workspace/.claude/skills/myskill').toString(),
			);
		});

		test('should return skill info for nested file in skill folder', () => {
			const skillFileUri = URI.file(
				'/workspace/.github/skills/myskill/subfolder/helper.ts',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(skillFileUri);
		...
		test('should return undefined for non-skill file', () => {
			...
			const skillInfo =
				customInstructionsService.getSkillInfo(regularFileUri);
		...
		test('should return skill info for skill with hyphenated name', () => {
			const skillFileUri = URI.file(
				'/workspace/.github/skills/my-skill-name/SKILL.md',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(skillFileUri);
	...
	suite('isSkillFile', () => {
		test('should return true for file in skill folder', () => {
			...
			expect(customInstructionsService.isSkillFile(skillFileUri)).toBe(
				true,
			);
		});
		...
		test('should return true for nested file in skill folder', () => {
			const skillFileUri = URI.file(
				'/workspace/.github/skills/myskill/subfolder/code.ts',
			);
			expect(customInstructionsService.isSkillFile(skillFileUri)).toBe(
				true,
			);
		...
		test('should return false for non-skill file', () => {
			const regularFileUri = URI.file('/workspace/src/file.ts');
			expect(customInstructionsService.isSkillFile(regularFileUri)).toBe(
				false,
			);
		});
		...
		test('should return true for file in .claude/skills folder', () => {
			const skillFileUri = URI.file(
				'/workspace/.claude/skills/test/file.ts',
			);
			expect(customInstructionsService.isSkillFile(skillFileUri)).toBe(
				true,
			);
	...
	suite('isSkillMdFile', () => {
		test('should return true for SKILL.md in skill folder', () => {
			const skillMdUri = URI.file(
				'/workspace/.github/skills/myskill/SKILL.md',
			);
			expect(customInstructionsService.isSkillMdFile(skillMdUri)).toBe(
				true,
			);
		});
		...
		test('should return false for other .md files in skill folder', () => {
			const otherMdUri = URI.file(
				'/workspace/.github/skills/myskill/README.md',
			);
			expect(customInstructionsService.isSkillMdFile(otherMdUri)).toBe(
				false,
			);
		});

		test('should return false for non-md files in skill folder', () => {
			const codeFileUri = URI.file(
				'/workspace/.github/skills/myskill/code.ts',
			);
			expect(customInstructionsService.isSkillMdFile(codeFileUri)).toBe(
				false,
			);
		});
		...
		test('should return false for SKILL.md outside skill folder', () => {
			...
			expect(customInstructionsService.isSkillMdFile(nonSkillUri)).toBe(
				false,
			);
		});
	...
	suite('isExternalInstructionsFolder', () => {
		test('should return true for skill folder', () => {
			const skillFolderUri = URI.file(
				'/workspace/.github/skills/myskill',
			);
			expect(
		...
		test('should return true for nested folder in skill', () => {
			const nestedFolderUri = URI.file(
				'/workspace/.github/skills/myskill/subfolder',
			);
			expect(
				customInstructionsService.isExternalInstructionsFolder(
					nestedFolderUri,
	...
	suite('chat.agentSkillsLocations config', () => {
		test('should return skill info for file in absolute path skill location', async () => {
			...
			expect(skillInfo?.skillName).toBe('myskill');
			expect(skillInfo?.skillFolderUri.toString()).toBe(
				URI.file('/custom/skills/myskill').toString(),
			);
		});
		...
		test('should return skill info for nested file in absolute path skill location', async () => {
			...
			const skillFileUri = URI.file(
				'/custom/skills/myskill/subfolder/code.ts',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(skillFileUri);
		...
		test('should return skill info for file in tilde path skill location', async () => {
			...
			const skillFileUri = URI.file(
				'/home/testuser/my-skills/myskill/SKILL.md',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(skillFileUri);
			...
			expect(skillInfo?.skillName).toBe('myskill');
			expect(skillInfo?.skillFolderUri.toString()).toBe(
				URI.file('/home/testuser/my-skills/myskill').toString(),
			);
		});
		...
		test('should return skill info for file in relative path skill location', async () => {
			...
			const skillFileUri = URI.file(
				'/workspace/custom-skills/myskill/SKILL.md',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(skillFileUri);
			...
			expect(skillInfo?.skillName).toBe('myskill');
			expect(skillInfo?.skillFolderUri.toString()).toBe(
				URI.file('/workspace/custom-skills/myskill').toString(),
			);
		});
		...
		test('should handle multiple skill locations', async () => {
			...
			// Check absolute path
			const skill1 = customInstructionsService.getSkillInfo(
				URI.file('/custom/skills/skill1/SKILL.md'),
			);
			expect(skill1?.skillName).toBe('skill1');

			// Check tilde path
			const skill2 = customInstructionsService.getSkillInfo(
				URI.file('/home/testuser/my-skills/skill2/SKILL.md'),
			);
			expect(skill2?.skillName).toBe('skill2');

			// Check relative path
			const skill3 = customInstructionsService.getSkillInfo(
				URI.file('/workspace/local-skills/skill3/SKILL.md'),
			);
		...
		test('config-based locations should not interfere with default locations', async () => {
			...
			const defaultSkillFile = URI.file(
				'/workspace/.github/skills/myskill/SKILL.md',
			);
			const skillInfo =
				customInstructionsService.getSkillInfo(defaultSkillFile);
	...
	suite('chat.instructionsFilesLocations config', () => {
		test('should return workspace and home copilot instruction files from getAgentInstructions', async () => {
			...
			await mockFiles(fileSystemService, [
				...
				{
					path: '/home/testuser/.copilot/copilot-instructions.md',
					contents: ['Home instructions'],
				},
			]);

			expect(
				(await customInstructionsService.getAgentInstructions()).map(
					(uri) => uri.path,
				),
			).toEqual([
		...
		test('should recognize instruction file in absolute path location', async () => {
			...
			const instructionFileUri = URI.file(
				'/custom/instructions/setup.instructions.md',
			);
			expect(
				await customInstructionsService.isExternalInstructionsFile(
					instructionFileUri,
		...
		test('should recognize instruction file in tilde path location', async () => {
			...
			// userHome is /home/testuser in NullNativeEnvService
			const instructionFileUri = URI.file(
				'/home/testuser/.copilot/instructions/setup.instructions.md',
			);
			expect(
				await customInstructionsService.isExternalInstructionsFile(
					instructionFileUri,
		...
		test('should not recognize non-instruction file in tilde path location', async () => {
			...
			const regularFileUri = URI.file(
				'/home/testuser/.copilot/instructions/notes.txt',
			);
			expect(
				await customInstructionsService.isExternalInstructionsFile(
					regularFileUri,
		...
		test('should handle both absolute and tilde paths together', async () => {
			...
			const absoluteFileUri = URI.file(
				'/custom/instructions/setup.instructions.md',
			);
			expect(
				await customInstructionsService.isExternalInstructionsFile(
