...
suite('Tokenization', function () {
	...
	beforeAll(() => {
		multiModelTokenizer = new TokenizerProvider(
			false,
	...
	test('Counts tokens - basic', async function () {
		const tokens = await multiModelTokenizer
			.acquireTokenizer({ tokenizer: TokenizerType.O200K })
			.tokenLength('Hello world!');
	...
	test('Counts tokens - advanced', async function () {
		const tokens = await multiModelTokenizer
			.acquireTokenizer({ tokenizer: TokenizerType.O200K })
			.tokenLength(
				'functionfibonacci(n:number):number{if(n<=0){return0;}elseif(n==1){return1;}else{returnfibonacci(n-1)+fibonacci(n-2);}}',
			);
