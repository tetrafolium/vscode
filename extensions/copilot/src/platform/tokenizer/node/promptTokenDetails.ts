...
const tagToLabelMapping: Record<string, { category: string; label: string }> = {
	...
	error: {
		category: PromptTokenCategory.UserContext,
		label: PromptTokenLabel.ToolResults,
	},
	errors: {
		category: PromptTokenCategory.UserContext,
		label: PromptTokenLabel.ToolResults,
...
async function parseTextContentTokens(
	...
	while ((tagMatch = allTagsRegex.exec(text)) !== null) {
		...
		if (!isNested) {
			...
			counts[mapping.category][mapping.label] =
				(counts[mapping.category][mapping.label] || 0) + tokens;
	...
	for (const message of messages) {
		...
		switch (message.role) {
			case Raw.ChatRole.System:
				counts[PromptTokenCategory.System][
					PromptTokenLabel.SystemInstructions
				] =
					(counts[PromptTokenCategory.System][
						PromptTokenLabel.SystemInstructions
					] || 0) + messageTokens;
			...
			case Raw.ChatRole.User: {
				...
				for (const part of message.content) {
					if (part.type === Raw.ChatCompletionContentPartKind.Text) {
						...
						const taggedTokens = await parseTextContentTokens(
							part.text,
							tokenizer,
							counts,
						);
						...
						counts[PromptTokenCategory.UserContext][
							PromptTokenLabel.Files
						] =
							(counts[PromptTokenCategory.UserContext][
								PromptTokenLabel.Files
							] || 0) + partTokens;
