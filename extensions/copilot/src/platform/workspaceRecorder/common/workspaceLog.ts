...
export type ApplicationStartLogEntry = {
...
export type DocumentChangedLogEntry = DocumentLogEntry & {
	kind: 'changed';
	edit: ISerializedEdit;
	v: number;
	metadata?: IChangedMetadata;
};

export type DocumentFocusChangedLogEntry = DocumentLogEntry & {
	kind: 'focused';
};
...
export type DocumentEncounteredLogEntry = DocumentLogEntry & {
	kind: 'documentEncountered';
	relativePath: string;
};

export type DocumentEventLogEntry = DocumentLogEntry & {
	kind: 'documentEvent';
	data: unknown;
};
...
export function deserializeEdit(serialized: ISerializedEdit): StringEdit {
	return StringEdit.create(
		serialized.map((e) =>
			StringReplacement.replace(new OffsetRange(e[0], e[1]), e[2]),
		),
	);
}

export type DocumentEventLogEntryData =
