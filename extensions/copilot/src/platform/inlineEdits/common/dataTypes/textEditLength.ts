...
import {
	...
	lengthToObj,
	toLength,
} from './textEditLengthHelper/length';
...
export class TextLengthEdit {
	...
	public static fromTextEdit(textEdit: TextEdit): TextLengthEdit {
		const edits = textEdit.replacements.map(
			(e) => new SingleTextEditLength(e.range, TextLength.ofText(e.text)),
		);
	...
	public getRange(): Range | undefined {
		...
		return Range.fromPositions(
			...
			this.edits.at(-1)!.range.getEndPosition(),
		);
	}
