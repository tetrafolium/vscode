...
export function serializeSingleEdit(
	edit: BaseStringReplacement,
...
function deserializeSingleEdit(
	serialized: SerializedReplacement,
): StringReplacement {
...
export type SerializedReplacement = [
	startOffset: number,
	endOffsetEx: number,
	newText: string,
];
 ...
 */
export function decomposeStringEdit<
	TEdit extends BaseStringEdit<BaseStringReplacement, TEdit>,
>(edit: TEdit, permutation?: Permutation): SingleEdits<TEdit> {
	...
	if (edit.replacements.length !== permutation.arrayLength) {
		throw illegalArgument(
			`Number of edits ${edit.replacements.length} does not match ${permutation.arrayLength}`,
		);
	}
	...
	for (let i = 0; i < edit.replacements.length; ++i) {
		...
		for (let j = idxInEdits; j < sortedSingleEdits.length; ++j) {
			const offsetDelta =
				singleEdit.newText.length - singleEdit.replaceRange.length;
