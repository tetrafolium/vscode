...
export class IgnoreEmptyLineAndLeadingTrailingWhitespaceChanges {
	public static filterEdit(
		...
		const filteredEdits = singleEdits.filter(
			(e) =>
				!IgnoreEmptyLineAndLeadingTrailingWhitespaceChanges._isWhitespaceOnlyChange(
					e,
					resultDocument.documentAfterEditsLines,
				),
		);
		return filteredEdits;
	}

	private static _isWhitespaceOnlyChange(
...
export class IgnoreWhitespaceOnlyChanges {
	public static filterEdit(
		...
		return singleEdits.filter(
			(e) =>
				!IgnoreWhitespaceOnlyChanges._isFormattingOnlyChange(
					resultDocument.documentAfterEditsLines,
					e,
				),
		);
	 ...
	 */
	public static _isFormattingOnlyChange(
		baseLines: string[],
		singleEdit: LineReplacement,
	): boolean {
		const originalLines = singleEdit.lineRange
			.toOffsetRange()
			.slice(baseLines)
			.join('')
			.replace(/\s/g, '');
...
export function editWouldDeleteWhatWasJustInserted(
...
export function editIsDeletion(edit: StringEdit): boolean {
	const deletedChars = edit.replacements.reduce(
		(acc, singleEdit) => acc + singleEdit.replaceRange.length,
		0,
	);
	const insertedChars = edit.replacements.reduce(
		(acc, singleEdit) => acc + singleEdit.newText.length,
		0,
	);
	return insertedChars === 0 && deletedChars > 0;
}

export function editWouldDeleteWhatWasJustInserted2(
	activeDocument: StatelessNextEditDocument,
	lineEdit: LineEdit,
) {
	let edit = lineEdit.toEdit(activeDocument.documentAfterEdits);
