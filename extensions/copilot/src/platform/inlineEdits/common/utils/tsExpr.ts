	...
	if (data && typeof data === 'object') {
		...
		if (Array.isArray(data)) {
			...
			return (
				`[\n` +
				entries
					.map((e) => indentLine(e, newLineIndentation + 1) + ',\n')
					.join('') +
		...
		for (const [key, value] of Object.entries(data)) {
			entries.push(
				`${serializeObjectKey(key)}: ${_serializeToTs(value, newLineIndentation + 1)},\n`,
			);
		}
		return (
			`{\n` +
...
function indentNonFirstLines(str: string, indentation: number): string {
	const lines = str.split('\n');
	return lines
		.map((line, i) => (i === 0 ? line : indentLine(line, indentation)))
		.join('\n');
}
