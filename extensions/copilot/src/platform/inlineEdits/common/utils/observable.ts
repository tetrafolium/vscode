...
type ObservableResult<T> =
	T extends IObservableWithChange<infer U, any> ? U : never;

type ObservableArrayToChangesData<
	T extends Record<string, IObservableWithChange<any, any>>,
> = {
	[Key in keyof T]: {
...
export function autorunWithChanges<
	T extends Record<string, IObservableWithChange<any, any>>,
>(
	owner: object,
	observables: T,
	...
	const observableToKey = new Map(
		Object.entries(observables).map(
			([key, value]) => [value, key] as const,
		),
	);

	const previousValues = new Map(
		Object.keys(observables).map((key) => [key, undefined]),
	);
	...
	return autorunHandleChanges(
		...
		(reader, data) => {
			for (const [key, value] of Object.entries(observables)) {
				...
				data[key].value = v;
				data[key].previous =
					previousValues.get(key) === undefined
						? undefined
