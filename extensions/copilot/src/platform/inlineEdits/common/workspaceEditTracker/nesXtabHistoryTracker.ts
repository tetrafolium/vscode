...
export type XtabEditMergeStrategy =
	...
	| {
			readonly kind: DiffHistoryMergeStrategy.Proximity;
	 ...
	 */
	| {
			readonly kind: DiffHistoryMergeStrategy.Hybrid;
			readonly lineGap: number;
			readonly splitAfterMs: number;
	  };

export namespace XtabEditMergeStrategy {
	export const sameStartLine: XtabEditMergeStrategy = {
		kind: DiffHistoryMergeStrategy.SameStartLine,
	};
	...
	export function fromConfig(
		strategyKind: DiffHistoryMergeStrategy,
		lineGap: number,
		splitAfterMs: number,
	): XtabEditMergeStrategy {
		switch (strategyKind) {
 ...
 */
function areLineEditsWithinProximity(
	a: LineEdit,
	b: LineEdit,
	maxLineGap: number,
): boolean {
	...
	for (const repA of a.replacements) {
		const closeToSomeInB = b.replacements.some(
			(repB) =>
				repA.lineRange.distanceToRange(repB.lineRange) <= maxLineGap,
		);
		if (!closeToSomeInB) {
			return false;
		}
...
export class NesXtabHistoryTracker extends Disposable {
	...
	private readonly idToEntry: Map<
		DocumentId,
		{
	...
	constructor(
		...
		this.maxHistorySize =
			maxHistorySize ?? NesXtabHistoryTracker.MAX_HISTORY_SIZE;

		this.mergeStrategy = derived((reader) =>
			XtabEditMergeStrategy.fromConfig(
				this._configurationService
					.getExperimentBasedConfigObservable(
						ConfigKey.TeamInternal.InlineEditsXtabDiffMergeStrategy,
						this._expService,
					)
					.read(reader),
				this._configurationService
					.getExperimentBasedConfigObservable(
						ConfigKey.TeamInternal.InlineEditsXtabDiffMergeLineGap,
						this._expService,
					)
					.read(reader),
				this._configurationService
					.getExperimentBasedConfigObservable(
						ConfigKey.TeamInternal
							.InlineEditsXtabDiffMergeSplitAfterMs,
						this._expService,
					)
					.read(reader),
			),
			...
			(doc, store) => {
				...
				store.add(
					autorunWithChanges(
						...
						(data) => {
							if (
								data.rootedEdits.changes.length > 0 &&
								data.rootedEdits.previous !== undefined
							) {
	...
	private handleVisibleRangesChange(
		...
		visibleRangesChange: DocumentSelectionChangedEvent,
	) {
		...
		switch (strategy.kind) {
			case DiffHistoryMergeStrategy.SameStartLine:
				return (
					...
					lastLineEdit.replacements[0].lineRange.startLineNumber ===
						currentLineEdit.replacements[0].lineRange
							.startLineNumber
				);

			case DiffHistoryMergeStrategy.Proximity:
				return areLineEditsWithinProximity(
					currentLineEdit,
					lastLineEdit,
					strategy.lineGap,
				);
			...
			case DiffHistoryMergeStrategy.Hybrid: {
				const withinTimeWindow =
					now() - lastEditTimestamp <= strategy.splitAfterMs;
				return (
					withinTimeWindow &&
					areLineEditsWithinProximity(
						currentLineEdit,
						lastLineEdit,
						strategy.lineGap,
					)
	...
	private handleEdits(
		doc: IObservableDocument,
		rootedEdits: DocumentChangedEvent,
	) {
		assert(
			rootedEdits.previous !== undefined,
			`Document has previous version`,
		);
		assert(
			rootedEdits.changes.length === 1,
			`Expected 1 edit change but got ${rootedEdits.changes.length}`,
		);
		...
		if (
			this.shouldMerge(
				lastLineEdit,
				currentLineEdit,
				previousRecord.lastEditTimestamp,
			)
		) {
			// merge edits
			previousRecord.removeFromHistory();
