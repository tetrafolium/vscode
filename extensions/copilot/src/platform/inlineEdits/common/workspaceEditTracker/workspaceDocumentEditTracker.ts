...
export class WorkspaceDocumentEditHistory extends Disposable {
	...
	constructor(
		...
		mapObservableArrayCached(
			...
			(doc, store) => {
				const state = new DocumentEditHistory(
					doc.value.get(),
					historyLength,
				);
	...
	public getRecentEdits(
		docId: DocumentId,
	): DocumentHistoryDifference | undefined {
...
class DocumentEditHistory {
	...
	public handleEdit(edit: StringEdit, isInCooldown: boolean): void {
		...
		const annotatedEdit = edit.mapData<DocumentEdit>(
			(r) => new DocumentEdit(stateIdentifier),
		);
		const updatedRecentEdits = this._recentEdits.compose(annotatedEdit);
		const { e1: newRecentEdits, e2: oldEdits } =
			updatedRecentEdits.decomposeSplit(
				(r) =>
					r.data.value > this._documentStateID - this._historyLength,
			);
	...
	public getNRecentEdits(n: number): DocumentHistoryDifference {
		const { e1: nRecentEdits, e2: oldEdits } =
			this._recentEdits.decomposeSplit(
