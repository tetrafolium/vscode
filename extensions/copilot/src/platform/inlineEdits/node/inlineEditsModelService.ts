...
} from '../common/inlineEditsModelService';
...
export class InlineEditsModelService
	extends Disposable
	implements IInlineEditsModelService
{
	...
	private _copilotTokenObs = observableFromEvent(
		this,
	...
	private _fetchedModelsObs = observableFromEvent(
		this,
		this._proxyModelsService.onModelListUpdated,
		() => this._proxyModelsService.nesModels,
	);

	private _preferredModelNameObs =
		this._configService.getExperimentBasedConfigObservable(
			ConfigKey.Advanced.InlineEditsPreferredModel,
			this._expService,
		);
	private _localModelConfigObs = this._configService.getConfigObservable(
		ConfigKey.Advanced.InlineEditsXtabProviderModelConfiguration,
	);
	private _expBasedModelConfigObs =
		this._configService.getExperimentBasedConfigObservable(
			ConfigKey.TeamInternal
				.InlineEditsXtabProviderModelConfigurationString,
			this._expService,
		);
	private _defaultModelConfigObs =
		this._configService.getExperimentBasedConfigObservable(
			ConfigKey.TeamInternal
				.InlineEditsXtabProviderDefaultModelConfigurationString,
			this._expService,
		);
	private _useSlashModelsObs =
		this._configService.getExperimentBasedConfigObservable(
			ConfigKey.TeamInternal.InlineEditsUseSlashModels,
			this._expService,
		);
	private _undesiredModelsObs = observableFromEvent(
		this,
		this._undesiredModelsManager.onDidChange,
		() => this._undesiredModelsManager,
	);
	...
	constructor(
		...
		@IProxyModelsService
		private readonly _proxyModelsService: IProxyModelsService,
		@IUndesiredModelsManager
		private readonly _undesiredModelsManager: IUndesiredModelsManager,
		@IConfigurationService
		...
		this._modelsObs = derived((reader) => {
			...
			return this.aggregateModels({
				...
				defaultModelConfigString:
					this._defaultModelConfigObs.read(reader),
				useSlashModels: this._useSlashModelsObs.read(reader),
			});
		}).recomputeInitiallyAndOnChange(this._store);

		this._currentModelObs = derived<ModelConfigurationWithSource, void>(
			(reader) => {
				logger.trace('computing current model');
				const undesiredModelsManager =
					this._undesiredModelsObs.read(reader);
				return this._pickModel({
					preferredModelName:
						this._preferredModelNameObs.read(reader),
	...
	get modelInfo(): vscode.InlineCompletionModelInfo | undefined {
		const models: vscode.InlineCompletionModel[] = this._modelsObs
			.get()
			.map((m) => ({
				id: m.modelName,
				name: m.modelName,
	...
	setCurrentModelId(newPreferredModelId: string): Promise<void> {
		return this._setModelQueue.schedule(() =>
			this._setCurrentModelIdCore(newPreferredModelId),
		);
	}

	private async _setCurrentModelIdCore(
		newPreferredModelId: string,
	): Promise<void> {
		const currentPreferredModelId =
		...
		const newPreferredModel = models.find(
			(m) => m.modelName === newPreferredModelId,
		);

		if (newPreferredModel === undefined) {
			this._logService.error(
				`New preferred model id ${newPreferredModelId} not found in model list.`,
			);
			return;
		}
		...
		if (currentPreferredModel.source === ModelSource.ExpConfig) {
			await this._undesiredModelsManager.addUndesiredModelId(
				currentPreferredModel.modelName,
			);
		}

		if (
			this._undesiredModelsManager.isUndesiredModelId(newPreferredModelId)
		) {
			await this._undesiredModelsManager.removeUndesiredModelId(
				newPreferredModelId,
			);
		}
		...
		const expectedDefaultModel = this._pickModel({
			preferredModelName: 'none',
			models,
			undesiredModelsManager: this._undesiredModelsManager,
		});
		if (
			newPreferredModel.source === ModelSource.ExpConfig || // because exp-configured model already takes highest priority
			(newPreferredModelId === expectedDefaultModel.modelName &&
				!models.some((m) => m.source === ModelSource.ExpConfig))
		) {
			this._logger.trace(
				`New preferred model id ${newPreferredModelId} is the same as the default model, resetting user setting.`,
			);
			...
			this._logger.trace(
				`New preferred model id ${newPreferredModelId} is different from the default model, updating user setting to ${newPreferredModelId}.`,
			);
			await this._configService.setConfig(
				ConfigKey.Advanced.InlineEditsPreferredModel,
				newPreferredModelId,
	...
	private aggregateModels({
		...
		if (localModelConfig) {
			if (
				models.some((m) => m.modelName === localModelConfig.modelName)
			) {
				logger.trace(
					'Local model configuration already exists in the model list, skipping.',
				);
			} else {
				logger.trace(
					`Adding local model configuration: ${localModelConfig.modelName}`,
				);
				models.push({
					...localModelConfig,
		...
		if (modelConfigString) {
			...
			const parsedConfig = this.parseModelConfigString(
				modelConfigString,
				ConfigKey.TeamInternal
					.InlineEditsXtabProviderModelConfigurationString,
			);
			if (
				parsedConfig &&
				!models.some((m) => m.modelName === parsedConfig.modelName)
			) {
				logger.trace(
					`Adding model from modelConfigurationString: ${parsedConfig.modelName}`,
				);
				models.push({ ...parsedConfig, source: ModelSource.ExpConfig });
			} else {
				logger.trace(
					'No valid model found in modelConfigurationString.',
				);
			}
		}
		...
		if (useSlashModels && fetchedNesModels && fetchedNesModels.length > 0) {
			logger.trace(
				`Processing ${fetchedNesModels.length} fetched models...`,
			);
			const filteredFetchedModels = filterMap(fetchedNesModels, (m) => {
				...
				if (
					models.some((knownModel) => knownModel.modelName === m.name)
				) {
					logger.trace(
						`Fetched model ${m.name} already exists in the model list, skipping.`,
					);
					return undefined;
				}
				return {
			...
			logger.trace(
				`Adding ${filteredFetchedModels.length} fetched models after filtering.`,
			);
			pushMany(models, filteredFetchedModels);
		} else {
			// push default model if /models doesn't give us any models
			logger.trace(
				`adding built-in default model: useSlashModels ${useSlashModels}, fetchedNesModels ${fetchedNesModels?.length ?? 'undefined'}`,
			);

			const defaultModel = this.determineDefaultModel(
				copilotToken,
				defaultModelConfigString,
			);
			if (defaultModel) {
				if (
					models.some((m) => m.modelName === defaultModel.modelName)
				) {
					logger.trace(
						'Default model configuration already exists in the model list, skipping.',
					);
				} else {
					logger.trace(
						`Adding default model configuration: ${defaultModel.modelName}`,
					);
	...
	public defaultModelConfiguration(): ModelConfiguration {
		...
		if (models && models.length > 0) {
			const defaultModels = models.filter(
				(m) => !this.isConfiguredModel(m),
			);
			if (defaultModels.length > 0) {
				return toModelConfiguration(defaultModels[0]);
			}
		}
		return toModelConfiguration(
			this.determineDefaultModel(
				this._copilotTokenObs.get(),
				this._defaultModelConfigObs.get(),
			),
		);
	}
	...
	private determineDefaultModel(
		...
		if (defaultModelConfigString) {
			...
			if (parsedConfig) {
				return {
					...parsedConfig,
					source: ModelSource.ExpDefaultConfig,
				};
			}
		}
	...
	private _pickModel({
		...
		const expConfiguredModel = models.find(
			(m) => m.source === ModelSource.ExpConfig,
		);
		if (expConfiguredModel) {
			const isUndesiredModelId =
				undesiredModelsManager.isUndesiredModelId(
					expConfiguredModel.modelName,
				);
			if (isUndesiredModelId) {
				this._logger.trace(
					`Exp-configured model ${expConfiguredModel.modelName} is marked as undesired by the user. Skipping.`,
				);
			} else {
		...
		if (userHasPreferredModel) {
			const preferredModel = models.find(
				(m) => m.modelName === preferredModelName,
			);
			if (preferredModel) {
		...
		softAssert(
			models.length > 0,
			'InlineEdits model list should have at least one model',
		);
		...
		return this.determineDefaultModel(
			this._copilotTokenObs.get(),
			this._defaultModelConfigObs.get(),
		);
	}
...
function toModelConfiguration(
	model: ModelConfigurationWithSource,
): ModelConfiguration {
...
export namespace UndesiredModels {
	...
	export class Manager extends Disposable implements IUndesiredModelsManager {
		...
		private _getModels(): string[] {
			return (
				this._vscodeExtensionContext.globalState.get<UndesiredModelsValue>(
					UNDESIRED_MODELS_KEY,
				) ?? []
			);
		}
		...
		private _setModels(models: string[]): Promise<void> {
			return new Promise((resolve, reject) => {
				this._vscodeExtensionContext.globalState
					.update(UNDESIRED_MODELS_KEY, models)
					.then(resolve, reject);
			});
		}
	}
