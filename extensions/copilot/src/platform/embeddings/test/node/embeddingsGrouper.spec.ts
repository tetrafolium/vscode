...
describe('EmbeddingsGrouper', () => {
	...
	describe('addNodes (bulk)', () => {
		...
		it('should add multiple nodes and cluster them efficiently', () => {
			...
			const totalNodesInClusters = clusters.reduce(
				(sum, cluster) => sum + cluster.nodes.length,
				0,
			);
		...
		it('should allow deferring clustering with reclusterAfter=false', () => {
			...
			expect(clustersAfterRecluster.length).toBeLessThanOrEqual(
				clusters.length,
			);
		});
	});
	...
	describe('removeNode', () => {
		...
		it('should update cluster when node is removed from multi-node cluster', () => {
			...
			const clusterWithMultipleNodes = initialClusters.find(
				(c) => c.nodes.length > 1,
			);

			if (
				clusterWithMultipleNodes &&
				clusterWithMultipleNodes.nodes.length > 1
			) {
				const nodeToRemove = clusterWithMultipleNodes.nodes[0];
				grouper.removeNode(nodeToRemove);

				const updatedCluster = grouper.getClusterForNode(
					clusterWithMultipleNodes.nodes[1],
				);
				expect(updatedCluster).toBeDefined();
				expect(
					updatedCluster!.nodes.some((n) => n === nodeToRemove),
				).toBe(false);
			}
		});
	});
	...
	describe('recluster', () => {
		...
		it('should respect minimum cluster size option', () => {
			const grouper = new EmbeddingsGrouper<TestTool>({
				minClusterSize: 3,
			});
			const embeddings = createSimilarEmbeddings();
			const nodes = [
	...
	describe('adaptive threshold behavior', () => {
		it('should work with different similarity percentiles', () => {
			const strictGrouper = new EmbeddingsGrouper<TestTool>({
				eps: 0.95,
			}); // High similarity required
			const lenientGrouper = new EmbeddingsGrouper<TestTool>({
				eps: 0.8,
			}); // Lower similarity required
			...
			expect(strictClusters.length).toBeGreaterThanOrEqual(
				lenientClusters.length,
			);
		});
	});
		...
		it('should compute correct centroids for clusters', () => {
			...
			clusters.forEach((cluster) => {
				...
				const magnitude = Math.sqrt(
					cluster.centroid.reduce((sum, val) => sum + val * val, 0),
				);
				if (magnitude > 0) {
	...
	describe('threshold tuning optimization', () => {
		...
		it('should apply percentile and recluster efficiently', () => {
			...
			expect(strictClusterCount).toBeGreaterThanOrEqual(
				lenientClusterCount,
			);

			// All nodes should still be assigned
			const allClusters = grouper.getClusters();
			const totalNodes = allClusters.reduce(
				(sum, cluster) => sum + cluster.nodes.length,
				0,
			);
		...
		it('should cache similarities for efficient repeated tuning', () => {
			...
			expect(result1.clusterCount).toBeGreaterThanOrEqual(
				result2.clusterCount,
			);
			expect(result2.clusterCount).toBeGreaterThanOrEqual(
				result3.clusterCount,
			);
		});
	...
	describe('edge cases', () => {
		...
		it('should handle large number of nodes efficiently', () => {
			...
			for (let i = 0; i < 100; i++) {
				...
				const embedding = [
					groupId === 0
						? 1 - withinGroupVariation * 0.2
						: withinGroupVariation * 0.2,
					groupId === 1
						? 1 - withinGroupVariation * 0.2
						: withinGroupVariation * 0.2,
					groupId === 2
