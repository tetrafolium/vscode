...
suite('Pack Embedding', () => {
	test('Text3small should pack and unpack to same values', () => {
		const embedding: Embedding = {
			...
			value: Array.from(
				Float32Array.from({ length: 512 }, () => Math.random()),
			),
		};

		const serialized = packEmbedding(embedding);
		const deserialized = unpackEmbedding(
	...
	test('Metis should use binary storage', () => {
		const embedding: Embedding = {
			...
			value: Array.from({ length: 1024 }, () =>
				Math.random() < 0.5 ? 0.03125 : -0.03125,
			),
		};
	...
	test('Unpack should work with buffer offsets', () => {
		...
		const buffer = new Uint8Array(
			serialized.length + prefixAndSuffixSize * 2,
		);
		for (let i = 0; i < serialized.length; i++) {
			buffer[i + prefixAndSuffixSize] = serialized[i];
		}

		const serializedCopy = new Uint8Array(
			buffer.buffer,
			prefixAndSuffixSize,
			serialized.length,
		);
