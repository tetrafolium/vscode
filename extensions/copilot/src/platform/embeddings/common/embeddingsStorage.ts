...
export function packEmbedding(embedding: Embedding): Uint8Array {
	...
	if (embeddingMetadata?.quantization.document === 'binary') {
		...
		if (embedding.value.length % 8 !== 0) {
			throw new Error(
				`Embedding value length must be a multiple of 8 for ${embedding.type.id}, got ${embedding.value.length}`,
			);
		}
...
export function unpackEmbedding(
	...
	if (embeddingMetadata?.quantization.document === 'binary') {
		...
		if (
			!(
				type.equals(EmbeddingType.metis_1024_I16_Binary) &&
				data.length >= 1024
			)
		) {
