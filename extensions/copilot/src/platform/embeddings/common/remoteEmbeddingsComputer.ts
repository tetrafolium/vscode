...
import {
	...
	GenAiOperationName,
	GenAiProviderName,
} from '../../otel/common/genAiAttributes';
import {
	IOTelService,
	SpanKind,
	SpanStatusCode,
} from '../../otel/common/otelService';
...
export class RemoteEmbeddingsComputer implements IEmbeddingsComputer {
	...
	public async computeEmbeddings(
		...
		try {
			return await logExecTime(
				this._logService,
				'RemoteEmbeddingsComputer::computeEmbeddings',
				async () => {
					...
					const copilotToken =
						await this._authService.getCopilotToken();
					if (copilotToken.isNoAuthUser) {
						const embeddings = await this.computeCAPIEmbeddings(
							inputs,
							options,
							cancellationToken,
						);
						return (
							embeddings ?? { type: embeddingType, values: [] }
						);
					}
					...
					for (let i = 0; i < inputs.length; i += this.batchSize) {
						...
						*/
							...
							throw new Error(
								`Error fetching embeddings: ${response.status}`,
							);
						}
						...
						const jsonResponse: EmbeddingResponse =
							await response.json();

						const resolvedType = new EmbeddingType(
							jsonResponse.embedding_model,
						);
						if (!resolvedType.equals(embeddingType)) {
							throw new Error(
								`Unexpected embedding model. Got: ${resolvedType}. Expected: ${embeddingType}`,
							);
						}

						if (batch.length !== jsonResponse.embeddings.length) {
							throw new Error(
								`Mismatched embedding result count. Expected: ${batch.length}. Got: ${jsonResponse.embeddings.length}`,
							);
						}

						embeddingsOut.push(
							...jsonResponse.embeddings.map((embedding) => ({
								type: resolvedType,
	...
	private async computeCAPIEmbeddings(
		...
		const typeInfo = getWellKnownEmbeddingTypeInfo(
			EmbeddingType.text3small_512,
		);
		if (!typeInfo) {
			throw new Error(
				`Embeddings type info not found: ${EmbeddingType.text3small_512}`,
			);
		}
		const endpoint =
			await this._endpointProvider.getEmbeddingsEndpoint('text3small');
		...
		return this.fetchResponseWithBatches(
			typeInfo,
			endpoint,
			inputs,
			cancellationToken,
	...
	private async fetchResponseWithBatches(
		...
		for (const input of inputs) {
			const inputTokenLength = await endpoint
				.acquireTokenizer()
				.tokenLength(input);
		...
		const limiter = new Limiter<CAPIEmbeddingResults | undefined>(
			parallelism,
		);
		try {
			for (let i = 0; i < inputs.length; i += batchSize) {
				...
				promises.push(
					limiter.queue(async () => {
						...
						const r = await this.rawEmbeddingsFetchWithTelemetry(
							type,
							endpoint,
							generateUuid(),
							currentBatch,
						...
						if (r.type === 'failed') {
							throw new Error(
								'Embeddings request failed ' + r.reason,
							);
						}
						return r;
					}),
			...
			embeddings = (await Promise.all(promises)).flatMap(
	...
	private async rawEmbeddingsFetchWithTelemetry(
		...
		const rawRequest = await this.rawEmbeddingsFetch(
			type,
			endpoint,
			requestId,
			inputs,
		...
		const tokenCounts = await Promise.all(
			inputs.map((input) => tokenizer.tokenLength(input)),
		);
		const inputTokenCount = tokenCounts.reduce(
			(acc, count) => acc + count,
			0,
		);
		this._telemetryService.sendMSFTTelemetryEvent(
	...
	public async rawEmbeddingsFetch(
		...
		try {
			...
			if (response.status === 200 && jsonResponse.data) {
				return {
					type: 'success',
					embeddings: jsonResponse.data.map(
						(d: EmbeddingResponse) => d.embedding,
					),
				};
			} else {
