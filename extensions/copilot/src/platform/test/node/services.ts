...
import {
	CopilotTokenStore,
	ICopilotTokenStore,
} from '../../authentication/common/copilotTokenStore';
...
} from '../../editSurvivalTracking/common/editSurvivalTrackerService';
...
import {
	NullEnvService,
	NullNativeEnvService,
} from '../../env/common/nullEnvService';
...
import {
	IIgnoreService,
	NullIgnoreService,
} from '../../ignore/common/ignoreService';
import {
	IImageService,
	nullImageService,
} from '../../image/common/imageService';
...
} from '../../notification/common/notificationService';
...
} from '../../prompts/common/promptPathRepresentationService';
import {
	BasicCodeSearchAuthenticationService,
	ICodeSearchAuthenticationService,
} from '../../remoteCodeSearch/node/codeSearchRepoAuth';
...
} from '../../simulationTestContext/common/simulationTestContext';
import {
	ISnippyService,
	NullSnippyService,
} from '../../snippy/common/snippyService';
import {
...
import {
	ISurveyService,
	NullSurveyService,
} from '../../survey/common/surveyService';
...
} from '../../telemetry/common/nullExperimentationService';
...
} from '../../workspaceChunkSearch/node/workspaceChunkSearchService';
 ...
 */
export class TestingServiceCollection
	implements IDisposable, IInstantiationServiceBuilder
{
	...
	define<T>(
		...
		descOrInstance: SyncDescriptor<T> | T,
	): void {
		if (this._accessor) {
...
export class TestingServicesAccessor implements ITestingServicesAccessor {
	constructor(private readonly _instaService: IInstantiationService) {}
...
export function _createBaselineServices(): TestingServiceCollection {
	...
	testingServiceCollection.define(
		IChatQuotaService,
		new SyncDescriptor(ChatQuotaService),
	);
	testingServiceCollection.define(
		ICopilotTokenStore,
	...
	testingServiceCollection.define(
		IExperimentationService,
		new SyncDescriptor(NullExperimentationService),
	);
	testingServiceCollection.define(
		IDiffService,
	...
	testingServiceCollection.define(
		ILogService,
		new SyncDescriptor(LogServiceImpl, [[new ConsoleLog()]]),
	);
	testingServiceCollection.define(
		IParserService,
		new SyncDescriptor(ParserServiceImpl, [/*useWorker*/ false]),
	);
	testingServiceCollection.define(
		IFetcherService,
		new SyncDescriptor(NodeFetcherService),
	);
	testingServiceCollection.define(
		ITelemetryUserConfig,
		new SyncDescriptor(TelemetryUserConfigImpl, ['tid=test', true]),
	);
	// Notifications from the monolith when fetching a token can trigger behaviour that require these objects.
	testingServiceCollection.define(
		IUrlOpener,
		new SyncDescriptor(NullUrlOpener),
	);
	testingServiceCollection.define(
		ICopilotTokenManager,
		new SyncDescriptor(SimulationTestCopilotTokenManager),
	);
	testingServiceCollection.define(
	...
	testingServiceCollection.define(
		IHeaderContributors,
		new SyncDescriptor(HeaderContributors),
	);

	testingServiceCollection.define(
		IConversationOptions,
		new SyncDescriptor(
			class implements IConversationOptions {
	...
	testingServiceCollection.define(
		IChatAgentService,
		new SyncDescriptor(TestChatAgentService),
	);
	testingServiceCollection.define(
		IFileSystemService,
		new SyncDescriptor(MockFileSystemService),
	);
	testingServiceCollection.define(
		IGithubRepositoryService,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(AuthenticationChatUpgradeService),
	);
	testingServiceCollection.define(
		IOctoKitService,
	...
	testingServiceCollection.define(
		IInteractionService,
		new SyncDescriptor(InteractionService),
	);
	testingServiceCollection.define(
		IWorkbenchService,
		new SyncDescriptor(TestWorkbenchService),
	);
	testingServiceCollection.define(
		ICustomInstructionsService,
		new SyncDescriptor(CustomInstructionsService),
	);
	testingServiceCollection.define(
		ISurveyService,
		new SyncDescriptor(NullSurveyService),
	);
	testingServiceCollection.define(
		IEditSurvivalTrackerService,
		new SyncDescriptor(NullEditSurvivalTrackerService),
	);
	testingServiceCollection.define(
		IWorkspaceChunkSearchService,
	...
	testingServiceCollection.define(
		ICodeSearchAuthenticationService,
		new SyncDescriptor(BasicCodeSearchAuthenticationService),
	);
	testingServiceCollection.define(
		IOTelService,
		new SyncDescriptor(NoopOTelService, [
			resolveOTelConfig({
				env: {},
...
export function createPlatformServices(
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(InMemoryConfigurationService, [
			disposables.add(new DefaultsOnlyConfigurationService()),
		]),
	);
	testingServiceCollection.define(
		IEnvService,
		new SyncDescriptor(NullEnvService),
	);
	testingServiceCollection.define(
		INativeEnvService,
	...
	testingServiceCollection.define(
		IExtensionsService,
		new SyncDescriptor(TestExtensionsService),
	);
	testingServiceCollection.define(
		ISearchService,
	...
	testingServiceCollection.define(
		IPromptPathRepresentationService,
		new SyncDescriptor(TestPromptPathRepresentationService),
	);
	testingServiceCollection.define(
		IRequestLogger,
	...
	testingServiceCollection.define(
		IScopeSelector,
		new SyncDescriptor(
			class implements IScopeSelector {
				_serviceBrand: undefined;
	...
	testingServiceCollection.define(
		ISnippyService,
		new SyncDescriptor(NullSnippyService),
	);
	testingServiceCollection.define(
		IChatWebSocketManager,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(
			class implements IInteractiveSessionService {
				_serviceBrand: undefined;
	...
	testingServiceCollection.define(
		ITasksService,
		new SyncDescriptor(TestTasksService),
	);
