...
export class SnippyService implements ISnippyService {
	...
	constructor(
		...
		this.notifier =
			this.instantiationService.createInstance(SnippyNotifier);
		this.fetcher =
			this.instantiationService.createInstance(SnippyFetchService);
	}

	public async handlePostInsertion(
		documentUri: URI,
		documentBeforeEdits: StringText,
		singleEdit: StringReplacement,
	): Promise<void> {
		const sourceToCheck = this.computeSourceToCheck(
			documentBeforeEdits,
			singleEdit,
		);
		if (!sourceToCheck) {
		...
		try {
			matchResponse = await this.fetcher.fetchMatch(
		...
		const citationPromises = snippets.map(async (snippet) => {
			const response = await this.fetcher.fetchFilesForMatch(
				snippet.cursor,
				CancellationToken.None,
			);
			...
			const { file_matches: files, license_stats: licenseStats } =
				response.val;
			return {
				match: snippet,
				files,
		...
		const filteredCitations: types.SnippetStatistics[] = citations.filter(
			(c) => !!c,
		);
	...
	private computeSourceToCheck(
		documentBeforeEdits: StringText,
		singleEdit: StringReplacement,
	) {
		if (singleEdit.newText === '') {
			// If the edit is a deletion, snippy shouldn't do anything (?)
			return;
		}
		...
		let potentialMatchContext = documentAfterEdits.value.substring(
			newTotalRange.start,
			newTotalRange.endExclusive,
		);
		...
		if (!SnippyCompute.hasMinLexemeLength(potentialMatchContext)) {
			const textWithoutCompletion = documentAfterEdits.value.slice(
				0,
				newTotalRange.start,
			);
			...
			potentialMatchContext = documentAfterEdits.value.slice(
				minLexemeStartOffset,
				newTotalRange.start + singleEdit.newText.length,
			);
		}
