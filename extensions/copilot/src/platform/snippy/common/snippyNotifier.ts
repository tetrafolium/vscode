...
export class SnippyNotifier {
	private static readonly matchCodeMessage =
	...
	public static readonly CodeReferenceKey =
		'copilot.chat.codeReference.notified';
	...
	public notify() {
		const didNotify = this.context.globalState.get<boolean>(
			SnippyNotifier.CodeReferenceKey,
		);
		...
		const messageItems = [
			SnippyNotifier.MatchAction,
			SnippyNotifier.SettingAction,
		];

		void this.notificationService
			.showInformationMessage(
				SnippyNotifier.matchCodeMessage,
				...messageItems,
			)
			.then((action) => {
				switch (action) {
