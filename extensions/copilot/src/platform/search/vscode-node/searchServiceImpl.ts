...
export class SearchServiceImpl extends BaseSearchServiceImpl {
	...
	override async findFilesWithDefaultExcludes(
	...
	override async findFilesWithDefaultExcludes(
		include: vscode.GlobPattern,
		maxResults: 1 | number | undefined,
		token: vscode.CancellationToken,
	): Promise<vscode.Uri | vscode.Uri[] | undefined> {
		const copilotIgnoreExclude =
			await this._ignoreService.asMinimatchPattern();
		const results = await this._findFilesWithDefaultExcludesAndExcludes(
			include,
			copilotIgnoreExclude,
			maxResults,
			token,
		);
		if (!this._ignoreService.isRegexExclusionsEnabled || !results) {
			...
			return (await this._ignoreService.isCopilotIgnored(results))
				? undefined
				: results;
		}
	}
			...
			options = {
				...
				exclude: copilotIgnoreExclude
					? options.exclude.map((e) =>
							combineGlob(e, copilotIgnoreExclude),
						)
					: options.exclude,
			};
		} else {
	...
	override async findTextInFiles(
		...
		const result = await super.findTextInFiles(
			query,
			options,
			ignoreSupportedProgress,
			token,
		);
