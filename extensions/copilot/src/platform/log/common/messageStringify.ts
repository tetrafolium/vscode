...
export function messageToMarkdown(
		...
		str += message.content
			.map((item) => {
				if (item.type === Raw.ChatCompletionContentPartKind.Text) {
					...
					item.type === Raw.ChatCompletionContentPartKind.Image
				) {
					return JSON.stringify(item);
				} else if (
					item.type === Raw.ChatCompletionContentPartKind.Opaque
				) {
					const asThinking = rawPartAsThinkingData(item);
					if (asThinking) {
						const parts: string[] = [];
						if (asThinking.text) {
							const thinkingText = Array.isArray(asThinking.text)
								? asThinking.text.join('\n')
								: asThinking.text;
							parts.push(`reasoning: ${thinkingText}`);
						}
						if (asThinking.encrypted?.length) {
							parts.push(
								`encrypted_content=${asThinking.encrypted.length} chars`,
							);
						}
	...
	const statefulMarker = mapFindFirst(message.content, (c) =>
		c.type === Raw.ChatCompletionContentPartKind.Opaque
			? rawPartAsStatefulMarker(c)
			: undefined,
	);
