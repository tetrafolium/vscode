...
const reviewDiffReposContextKey =
	'github.copilot.chat.reviewDiff.enabledRootUris';
...
export class ReviewServiceImpl implements IReviewService {
	...
	private _commentController = this._disposables.add(
	...
	constructor(
		...
		private _contextService: IVSCodeExtensionContext,
		@IGitExtensionService
		private _gitExtensionService: IGitExtensionService,
	) {
		this._disposables.add(
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(
						ConfigKey.CodeFeedback.fullyQualifiedId,
					)
				) {
					vscode.commands.executeCommand(
						'setContext',
						ConfigKey.CodeFeedback.fullyQualifiedId,
						this.isCodeFeedbackEnabled(),
					);
				}
				if (
					e.affectsConfiguration('github.copilot.advanced') ||
					e.affectsConfiguration(
						'github.copilot.advanced.review.intent',
					)
				) {
					vscode.commands.executeCommand(
						'setContext',
						ConfigKey.Advanced.ReviewIntent.fullyQualifiedId,
						this.isIntentEnabled(),
					);
				}
			}),
		);
		this._disposables.add(
			this._authenticationService.onDidAuthenticationChange(() => {
				vscode.commands.executeCommand(
					'setContext',
					reviewDiffContextKey,
					this.isReviewDiffEnabled(),
				);
			}),
		);
		this._disposables.add(this._repositoryDisposables);
		this._disposables.add(
			this._gitExtensionService.onDidChange(() => {
	...
	private updateRepositoryListeners() {
		...
		if (api) {
			this._repositoryDisposables.add(
	...
	private updateReviewDiffReposContext() {
		...
		const reviewDiffReposString = reviewDiffRepos
	...
	isCodeFeedbackEnabled() {
		const inspect = this._configurationService.inspectConfig(
			ConfigKey.CodeFeedback,
		);
		return (
			inspect?.workspaceFolderValue ??
			inspect?.workspaceValue ??
			inspect?.globalValue ??
			this._configurationService.getDefaultValue(ConfigKey.CodeFeedback)
		);
	}

	isReviewDiffEnabled() {
		return (
			(this._configurationService.getConfig(ConfigKey.ReviewAgent) &&
	...
	isIntentEnabled(): boolean {
		return this._configurationService.getConfig(
			ConfigKey.Advanced.ReviewIntent,
		);
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			const thread = this._commentController.createCommentThread(
				comment.uri,
				comment.range,
				this.createUIComments(comment),
			);
			thread.contextValue = 'hasNoSuggestion';
			thread.canReply = false;
			if (
				!this._comments.find(
					(c) => c.comment.uri.toString() === comment.uri.toString(),
				)
			) {
			...
			if (this._comments.length === 1) {
				...
				this._monitorActiveThread = setInterval(() => {
					...
					const active =
						raw &&
						this._comments.find((c) => c.thread.label === raw.label)
							?.thread; // https://github.com/microsoft/vscode/issues/223025
		...
		const comments = [
			{
				body:
					typeof comment.body === 'string'
						? `${comment.body}${change}${appendText}`
						: new vscode.MarkdownString(
								`${comment.body.value}${change}${appendText}`,
							),
				mode: vscode.CommentMode.Preview,
				author: {
					name: l10n.t('Code Review'),
					iconPath: URI.joinPath(
						this._contextService.extensionUri,
						'assets',
						'copilot.png',
					),
				},
	...
	collapseReviewComment(comment: ReviewComment) {
		const internalComment = this._comments.find(
			(c) => c.comment === comment,
		);
		if (!internalComment) {
			return;
		...
		oldThread.dispose();
		const newThread = this._commentController.createCommentThread(
			comment.uri,
			comment.range,
			oldThread.comments,
		);
	...
	findReviewComment(
		...
		const internalComment = this._comments.find(
			(c) =>
				c.thread === threadOrComment ||
				c.thread.comments[0] === threadOrComment,
		);
