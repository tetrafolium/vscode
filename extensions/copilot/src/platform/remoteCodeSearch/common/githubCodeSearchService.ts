...
import {
	stripChunkTextMetadata,
	truncateToMaxUtf8Length,
...
export type GithubCodeSearchScope =
	| GithubCodeSearchRepoInfo
	| GithubCodeSearchOrgInfo;

export const IGithubCodeSearchService = createDecorator(
...
export class GithubCodeSearchService implements IGithubCodeSearchService {
	...
	async getRemoteIndexState(
		...
		if (repoNwo.startsWith('microsoft/simuluation-test-')) {
			return Result.ok({
				status: RemoteCodeSearchIndexStatus.NotYetIndexed,
			});
		}
		...
		if (!authToken) {
			this._logService.error(
				`GithubCodeSearchService::getRemoteIndexState(${repoNwo}). Failed to fetch indexing status. No valid github auth token.`,
			);
		...
		try {
			const statusRequest = await raceCancellationError(
				this._capiClientService.makeRequest<Response>(
					{
						method: 'GET',
						headers: {
					...
					{
						type: RequestType.EmbeddingsIndex,
						repoWithOwner: repoNwo,
					},
				),
				token,
			);
			if (!statusRequest.ok) {
				...
				*/
				...
				this._logService.error(
					`GithubCodeSearchService::getRemoteIndexState(${repoNwo}). Failed to fetch indexing status. Response: ${statusRequest.status}. ${await statusRequest.text()}`,
				);
				return Result.error<RemoteCodeSearchError>({
					type: 'generic-error',
					error: new Error(
						`Failed to fetch indexing status. Response: ${statusRequest.status}.`,
					),
				});
			}

			const preCheckResult = await raceCancellationError(
				statusRequest.json(),
				token,
			);
			if (
				preCheckResult.semantic_code_search_ok &&
				preCheckResult.semantic_commit_sha
			) {
				const indexedCommit = preCheckResult.semantic_commit_sha;
				this._logService.trace(
					`GithubCodeSearchService::getRemoteIndexState(${repoNwo}). Found indexed commit: ${indexedCommit}.`,
				);
				return Result.ok({
			...
			if (preCheckResult.semantic_indexing_enabled) {
				if (
					await raceCancellationError(
						this.isEmptyRepo(authToken, githubRepoId, token),
						token,
					)
				) {
					this._logService.trace(
						`GithubCodeSearchService::getRemoteIndexState(${repoNwo}). Semantic indexing enabled but repo is empty.`,
					);
					return Result.ok({
						status: RemoteCodeSearchIndexStatus.Ready,
				...
				this._logService.trace(
					`GithubCodeSearchService::getRemoteIndexState(${repoNwo}). Semantic indexing enabled but not yet indexed.`,
				);

				return Result.ok({
					status: RemoteCodeSearchIndexStatus.BuildingIndex,
				});
			} else {
				this._logService.trace(
					`GithubCodeSearchService::getRemoteIndexState(${repoNwo}). semantic_indexing_enabled was false. Repo not yet indexed but possibly can be.`,
				);
				return Result.ok({
					status: RemoteCodeSearchIndexStatus.NotYetIndexed,
				});
			...
			this._logService.error(
				`GithubCodeSearchService::getRemoteIndexState(${repoNwo}). Error: ${e}`,
			);
			return Result.error<RemoteCodeSearchError>({
				type: 'generic-error',
	...
	public async triggerIndexing(
		...
		const response = await this._capiClientService.makeRequest<Response>(
			...
			{
				type: RequestType.EmbeddingsIndex,
				repoWithOwner: toGithubNwo(githubRepoId),
			},
		);

		if (!response.ok) {
			this._logService.error(
				`GithubCodeSearchService.triggerIndexing(${triggerReason}). Failed to request indexing for '${githubRepoId}'. Response: ${response.status}. ${await response.text()}`,
			);
			...
			*/
			...
			return Result.error({
				type: 'generic-error',
				error: new Error(
					`Failed to request indexing for '${githubRepoId}'. Response: ${response.status}.`,
				),
			});
		}
	...
	async semanticSearch(
		...
		const response = await raceCancellationError(
			this._instantiationService.invokeFunction(postRequest, {
				...
				additionalHeaders: getGithubMetadataHeaders(
					telemetryInfo.callTracker,
					this._envService,
				),
				cancelToken: token,
		...
		if (!response.ok) {
			...
			*/
			...
			throw new Error(
				`Code search semantic search failed with status: ${response.status}`,
			);
		}
	...
	async lexicalSearch(
		...
		const scopeQualifier =
			scope.kind === 'org'
				? `org:${scope.org}`
				: `repo:${toGithubNwo(scope.githubRepoId)}`;
		...
		if (!Array.isArray(body.items)) {
			throw new Error(
				`Code search lexical search unexpected response json shape`,
			);
		}

		const result = await raceCancellationError(
			parseLexicalSearchResponse(
				body,
				scope,
				options,
				this._ignoreService,
	...
	private async isEmptyRepo(
		...
		const response = await raceCancellationError(
			fetch(
				this._capiClientService.dotcomAPIURL +
					`/repos/${toGithubNwo(githubRepoId)}`,
				{
					headers: {
						Authorization: `Bearer ${authToken}`,
		...
		if (!response.ok) {
			this._logService.error(
				`GithubCodeSearchService.isEmptyRepo(${toGithubNwo(githubRepoId)}). Failed to fetch repo info. Response: ${response.status}. ${await response.text()}`,
			);
			return false;
		}
...
export async function parseGithubCodeSearchResponse(
	body: ResponseShape,
	repo: GithubCodeSearchRepoInfo,
	options: CodeSearchOptions & { skipVerifyRepo?: boolean },
	...
	await Promise.all(
		body.results.map(
			async (result): Promise<FileChunkAndScore | undefined> => {
				if (
					!options.skipVerifyRepo &&
					result.location.repo.nwo.toLowerCase() !==
						toGithubNwo(repo.githubRepoId)
				) {
					return;
				}
	...
	if (firstResult) {
		...
		try {
			...
			const host =
				parsed.authority === 'api.github.com'
					? 'github.com'
					: parsed.authority.replace(/^api\./, '');
...
export async function parseLexicalSearchResponse(
	body: LexicalSearchResponseShape,
	scope: GithubCodeSearchScope & { skipVerifyRepo?: boolean },
	options: CodeSearchOptions & { skipVerifyRepo?: boolean },
	ignoreService: IIgnoreService,
): Promise<LexicalCodeSearchResult> {
	...
	await Promise.all(
		body.items.map(async (item): Promise<void> => {
			if (
				!options.skipVerifyRepo &&
				scope.kind === 'repo' &&
				item.repository.full_name.toLowerCase() !==
					toGithubNwo(scope.githubRepoId)
			) {
				return;
			}
			if (
				!options.skipVerifyRepo &&
				scope.kind === 'org' &&
				item.repository.full_name.toLowerCase().split('/')[0] !==
					scope.org.toLowerCase()
			) {
				return;
			}

			const localRepoRoot =
				scope.kind === 'repo' ? scope.localRepoRoot : undefined;
			let fileUri: URI;
			if (localRepoRoot) {
			...
			const textMatches = item.text_matches?.filter(
				(m) => m.property === 'content',
			);
