...
export const ICodeSearchAuthenticationService =
	createServiceIdentifier<ICodeSearchAuthenticationService>(
		'ICodeSearchAuthentication',
	);
...
export class BasicCodeSearchAuthenticationService implements ICodeSearchAuthenticationService {
	...
	async tryAuthenticating(
		remoteInfo: ResolvedRepoRemoteInfo | undefined,
	): Promise<void> {
		if (remoteInfo?.repoId?.type === 'ado') {
			await this._authenticationService.getAdoAccessTokenBase64({
				createIfNone: true,
			});
			return;
		}
		...
		await this._authenticationService.getGitHubSession('any', {
			createIfNone: {
				detail: l10n.t('Sign in to GitHub to use remote code search.'),
			},
		});
	}
	...
	async tryReauthenticating(
		...
		await this._authenticationService.getGitHubSession('permissive', {
			createIfNone: {
				detail: l10n.t(
					'Sign in to GitHub with additional permissions for remote code search.',
				),
			},
		});
	}
