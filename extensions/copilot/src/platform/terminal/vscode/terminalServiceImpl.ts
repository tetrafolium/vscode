...
export class TerminalServiceImpl
	extends Disposable
	implements ITerminalService
{
	...
	contributePath(
		...
		const entry = this.pathContributions.find(
	...
	removePathContribution(contributor: string): void {
		const index = this.pathContributions.findIndex(
			(c) => c.contributor === contributor,
		);
		if (index !== -1) {
	...
	private updateEnvironmentPath(): void {
		...
		const allDescriptions = coalesce(
			this.pathContributions
				.map((c) =>
		...
		if (allCommands.length === 1) {
			commandsDescription = l10n.t(
				'Enables use of {0} command in the terminal',
				allCommands[0],
			);
		} else if (allCommands.length > 1) {
			const commands = `${allCommands.slice(0, -1).join(', ')} ${l10n.t('and')} ${allCommands[allCommands.length - 1]}`;
			commandsDescription = l10n.t(
				'Enables use of {0} commands in the terminal',
				commands,
			);
		}

		const description = [descriptions, commandsDescription]
			.filter((d) => d)
			.join(' and ');
		this.context.environmentVariableCollection.description =
			description || 'Enables additional commands in the terminal.';
		...
		if (this.pathContributions.some((c) => c.prepend)) {
			const pathVariableChange =
				allPaths.join(path.delimiter) + path.delimiter;
			...
			const pathVariableChange =
				path.delimiter + allPaths.join(path.delimiter);
