...
export class AuthenticationService extends BaseAuthenticationService {
	...
	constructor(
		...
		this._register(
			authentication.onDidChangeSessions((e) => {
				if (
					e.provider.id === authProviderId(configurationService) ||
	...
	override async getGitHubSession(
		...
		if (kind === 'permissive') {
			const func = () =>
				getAlignedSession(this._configurationService, options);
			...
			const session =
				options?.createIfNone || options?.forceNewSession
					? await func()
					: await this._taskSingler.getOrCreate('permissive', func);
			this._permissiveGitHubSession = session;
			return session;
		} else {
			const func = () =>
				getAnyAuthSession(this._configurationService, options);
			// If we are doing an interactive flow, don't use the singler so that we don't get hung up on the user's choice
			const session =
				options?.createIfNone || options?.forceNewSession
					? await func()
					: await this._taskSingler.getOrCreate('any', func);
			this._anyGitHubSession = session;
			return session;
		}
	}

	protected async getAnyAdoSession(
		options?: AuthenticationGetSessionOptions,
	): Promise<AuthenticationSession | undefined> {
		...
		const session =
			options?.createIfNone || options?.forceNewSession
				? await func()
				: await this._taskSingler.getOrCreate('ado', func);
	...
	async getAdoAccessTokenBase64(
		...
		return session
			? Buffer.from(`PAT:${session.accessToken}`, 'utf8').toString(
					'base64',
				)
			: undefined;
	}
