...
export class VSCodeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	async getCopilotToken(force?: boolean): Promise<CopilotToken> {
		const failWith = this.configurationService.getConfig(
			ConfigKey.Advanced.DebugGitHubAuthFailWith,
		);
		if (failWith) {
		...
		if (
			!this.copilotToken ||
			this.copilotToken.expires_at - 60 * 5 /* 5min */ < nowSeconds() ||
			force
		) {
			try {
				this._logService.debug(
					`Getting CopilotToken (force: ${force})...`,
				);
				this.copilotToken = await this._authShowWarnings();
				this._logService.debug(`Got CopilotToken (force: ${force}).`);
			} catch (e) {
				this._logService.debug(
					`Getting CopilotToken (force: ${force}) threw error: ${e}`,
				);
				this.copilotToken = undefined;
				throw e;
			}
	...
	private async _auth(): Promise<TokenInfoOrError> {
		...
		const allowNoAuthAccess =
			this.configurationService.getNonExtensionConfig<boolean>(
				'chat.allowAnonymousAccess',
			);
		const session = await getAnyAuthSession(this.configurationService, {
			silent: true,
		});
		...
		if (session) {
			...
			const tokenResult = await this.authFromGitHubToken(
				session.accessToken,
				session.account.label,
			);
			if (tokenResult.kind === 'success') {
				this._logService.info(
					`Got Copilot token for ${session.account.label}`,
				);
				this._logService.info(
					`Copilot Chat: ${this._envService.getVersion()}, VS Code: ${this._envService.vscodeVersion}`,
				);
			}
			return tokenResult;
		} else {
	...
	private async _authShowWarnings(): Promise<ExtendedTokenInfo> {
		const tokenResult = await this._taskSingler.getOrCreate('auth', () =>
			this._auth(),
		);
		this.sendTokenResultErrorTelemetry(tokenResult);

		if (
			tokenResult.kind === 'failure' &&
			tokenResult.reason === 'NotAuthorized'
		) {
		...
		if (
			tokenResult.kind === 'failure' &&
			tokenResult.reason === 'GitHubLoginFailed'
		) {
		...
		if (
			tokenResult.kind === 'failure' &&
			tokenResult.reason === 'RateLimited'
		) {
			throw new RateLimitedError(
				`Your account has exceeded GitHub's API rate limit. Please try again later.`,
			);
		}

		if (tokenResult.kind === 'failure') {
			throw Error(
				'Failed to get copilot token. reason: ' + tokenResult.reason,
			);
		}
