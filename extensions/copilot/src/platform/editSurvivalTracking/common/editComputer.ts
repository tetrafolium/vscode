...
export class EditComputer extends Disposable {
	...
	async compute(): Promise<ISyncEditProvider> {
		const diffEdits = await stringEditFromDiff(
			this._baseText,
			this._baseDocumentText,
			this._diffService,
		);
		return {
			...
			getEditsSinceInitial: () => {
				if (this._store.isDisposed) {
					throw new BugIndicatingError(
						'EditComputer has been disposed',
					);
				}
...
export class DocumentEditRecorder extends Disposable {
		...
		this._register(
			this._workspaceService.onDidChangeTextDocument((e) => {
				if (
					...
					const edits = stringEditFromTextContentChange(
						e.contentChanges,
					);
	...
	getEdits(): StringEdit {
		if (this._store.isDisposed) {
			throw new BugIndicatingError(
				'DocumentEditRecorder has been disposed',
			);
		}
		return this._edits;
	}
