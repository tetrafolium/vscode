...
export class ArcTracker {
	...
	constructor(
		...
		this._updatedTrackedEdit = eNormalized.mapData(
	...
	handleEdits(edit: BaseStringEdit): void {
		...
		const onlyTrackedEdit = composedEdit.decomposeSplit(
			(e) => !e.data.isTrackedEdit,
		).e2;
		this._updatedTrackedEdit = onlyTrackedEdit;
	}

	getAcceptedRestrainedCharactersCount(): number {
		const s = sum(this._updatedTrackedEdit.replacements, (e) =>
			e.getNewLength(),
		);
		return s;
	}
