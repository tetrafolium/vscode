...
} from '../../common/promptPathRepresentationService';
...
describe('PromptPathRepresentationService', () => {
	...
	describe.skipIf(!isWindows)('resolveFilePath (Windows simulation)', () => {
		...
		beforeEach(() => {
			workspaceService = new TestWorkspaceService();
			windowsService = new WindowsPromptPathRepresentationService(
				workspaceService,
			);
		});

		it('resolves Windows drive-letter paths', () => {
			const result = windowsService.resolveFilePath(
	...
	describe('getExampleFilePath', () => {
		it.skipIf(isWindows)(
			'returns a posix-style file path on non-Windows',
			() => {
				const result = service.getExampleFilePath(
		...
		it.skipIf(!isWindows)(
			'returns a Windows-style path when on Windows',
			() => {
				workspaceService = new TestWorkspaceService();
				const windowsService =
					new WindowsPromptPathRepresentationService(
						workspaceService,
					);
				const result = windowsService.getExampleFilePath(
					'/workspace/src/file.ts',
				);
