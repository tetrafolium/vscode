...
export class TasksService extends DisposableStore implements ITasksService {
	...
	private latestTerminalForTaskDefinition: Map<
		...
		this.add(
			vscode.tasks.onDidStartTask((e) => {
				const terminal: vscode.Terminal | undefined = (
					e.execution as unknown as { terminal?: vscode.Terminal }
				).terminal;
				if (!terminal) {
					return;
				}
				this.latestTerminalForTaskDefinition.set(
				...
				const closeListener = vscode.window.onDidCloseTerminal(
					(closedTerminal) => {
						if (
							closedTerminal === terminal &&
							this.latestTerminalForTaskDefinition.has(
								e.execution.task.definition,
							)
						) {
							this.latestTerminalForTaskDefinition.delete(
								e.execution.task.definition,
							);
	...
	private getTasksFromConfig(workspaceFolder: URI): vscode.TaskDefinition[] {
		const tasks = vscode.workspace.getConfiguration(
			'tasks',
			workspaceFolder,
		);
	...
	getTerminalForTask(
		...
		for (const [
			key,
			terminal,
		] of this.latestTerminalForTaskDefinition.entries()) {
			if (key.id) {
			...
			this.logService.debug(
				`getTerminalForTask: no terminal found for task definition: ${JSON.stringify(taskDefinition)} matching ${JSON.stringify(key)}`,
			);
			this.logService.debug(
				`getTerminalForTask: current stored terminals: ${[...this.latestTerminalForTaskDefinition.values()].map((t) => t.name).join(', ')}`,
			);
		}
	}
	private _getTaskId(
		...
		if (
			!taskDefinition.type ||
			(taskDefinition.command === undefined &&
				taskDefinition.script === undefined)
		) {
			return undefined;
		}
		return (
			taskDefinition.type +
			',' +
			(taskDefinition.command ?? taskDefinition.script) +
			','
		);
	}

	async getTaskConfigPosition(
		workspaceFolder: URI,
		def: vscode.TaskDefinition,
	) {
		const index = this.getTasksFromConfig(workspaceFolder).findIndex((t) =>
			this.matchesTask(t, def),
		);
		if (index === -1) {
			return undefined;
		}
	...
	isTaskActive(task: vscode.TaskDefinition): boolean {
		...
		for (const a of activeTasks) {
			if (
				a.task.definition.type === task.type &&
				a.task.name === task.label
			) {
				return true;
			}
		}
	...
	getTasks(
		workspaceFolder?: URI,
	): [URI, vscode.TaskDefinition[]][] | vscode.TaskDefinition[] {
		if (workspaceFolder) {
		...
		return this.workspaceService
			.getWorkspaceFolders()
			.map((folder): [URI, vscode.TaskDefinition[]] => [
				folder,
				this.getTasksFromConfig(folder),
			])
	...
	async executeTask(
		...
		try {
			...
			return await new Promise<TaskResult>((resolve) => {
						...
						if (e.execution.task === task) {
							if (
								...
								resolve({
									...
									error: new Error(
										`Task exited with code ${processExitCode}`,
									),
								});
							} else {
				...
				if (task) {
					const existing = vscode.tasks.taskExecutions.find((e) =>
						equals(e.task.definition, task!.definition),
					);
					adopted = !!existing;

					Promise.resolve(
						existing || vscode.tasks.executeTask(task),
					).then(
						(_execution) => {
							...
							if (token.isCancellationRequested) {
								...
								let resolved = false;
								disposables.add(
									vscode.tasks.onDidEndTaskProblemMatchers(
										async (e) => {
											resolved = true;
											if (e.execution.task === task) {
												if (e.hasErrors) {
													...
													resolve({
														status: TaskStatus.Error,
														error: new Error(
															'Task exited with errors in the following files: ' +
																diagnostics.join(
																	', ',
																),
														),
						...
						(error: Error) =>
							resolve({ status: TaskStatus.Error, error }),
					);
				} else {
