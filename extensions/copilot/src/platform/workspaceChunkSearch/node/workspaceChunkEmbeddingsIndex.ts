...
} from '../../chunking/common/chunk';
import {
	ComputeBatchInfo,
	EmbeddingsComputeQos,
	IChunkingEndpointClient,
} from '../../chunking/common/chunkingEndpointClient';
import {
	distance,
	Embedding,
	EmbeddingType,
...
import {
	...
	IWorkspaceChunkAndEmbeddingCache,
} from './workspaceChunkAndEmbeddingCache';
...
export class WorkspaceChunkEmbeddingsIndex extends Disposable {
	...
	private readonly _onDidChangeWorkspaceIndexState = this._register(
		new Emitter<void>(),
	);
	public readonly onDidChangeWorkspaceIndexState = Event.debounce(
		this._onDidChangeWorkspaceIndexState.event,
		() => {},
		2500,
		undefined,
		undefined,
	...
	private readonly _onDisposeCts = this._register(
	...
	constructor(
		...
		@IWorkspaceFileIndex
		private readonly _workspaceIndex: IWorkspaceFileIndex,
		@IChunkingEndpointClient
	...
	async getIndexState(): Promise<
		WorkspaceChunkEmbeddingsIndexState | undefined
	> {
		...
		let indexedCount = 0;
		await Promise.all(
			allWorkspaceFiles.map(async (file) => {
	...
	async triggerIndexingOfWorkspace(
		...
		return logExecTime(
			this._logService,
			'WorkspaceChunkEmbeddingIndex.triggerIndexingOfWorkspace',
			async () => {
				await raceCancellationError(
					this._workspaceIndex.initialize(),
					token,
				);

				await this.indexAllWorkspaceFiles(
					trigger,
					{},
	...
	async triggerIndexingOfFile(
		uri: URI,
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<void> {
		if (
			!(await this._workspaceIndex.shouldIndexWorkspaceFile(uri, token))
		) {
			return;
		}
		...
		if (authToken) {
			await this.getChunksAndEmbeddings(
				authToken,
				file,
				new ComputeBatchInfo(),
				EmbeddingsComputeQos.Batch,
				telemetryInfo.callTracker.add(
					'WorkspaceChunkEmbeddingsIndex::triggerIndexingOfFile',
				),
				token,
			);
		}
	...
	async searchWorkspace(
		...
		return logExecTime(
			this._logService,
			'WorkspaceChunkEmbeddingIndex.searchWorkspace',
			async () => {
				const [queryEmbedding, fileChunksAndEmbeddings] =
					await raceCancellationError(
						Promise.all([
							query,
							this.getAllWorkspaceEmbeddings(
								'manual',
				...
				return this.rankEmbeddings(
					queryEmbedding,
					fileChunksAndEmbeddings,
					maxResults,
				);
			},
			(execTime, status) => {
	...
	async searchSubsetOfFiles(
		...
		return logExecTime(
			this._logService,
			'WorkspaceChunkEmbeddingIndex.searchSubsetOfFiles',
			async () => {
				const [queryEmbedding, fileChunksAndEmbeddings] =
					await raceCancellationError(
						Promise.all([
			...
			*/
				this._telemetryService.sendMSFTTelemetryEvent(
					'workspaceChunkEmbeddingsIndex.perf.searchSubsetOfFiles',
					{
						status,
						workspaceSearchSource:
							telemetry.info.callTracker.toString(),
						workspaceSearchCorrelationId:
							telemetry.info.correlationId,
					},
					{ execTime },
				);
			},
	...
	public async toSemanticChunks(
		query: Promise<Embedding>,
		tfidfResults: readonly FileChunk[],
		options: {
		...
		await Promise.all(
			Array.from(chunksByFile.entries(), async ([uri, chunks]) => {
				const file = this._workspaceIndex.get(uri);
				if (!file) {
				...
				const qos = this._simulationTestContext.isInSimulationTests
					? EmbeddingsComputeQos.Batch
					: EmbeddingsComputeQos.Online;

				let semanticChunks:
					| readonly FileChunkWithOptionalEmbedding[]
					| undefined;
				if (authToken) {
					const cts = new CancellationTokenSource(token);
					try {
						semanticChunks = await raceTimeout(
							this.getChunksWithOptionalEmbeddings(
								authToken,
								file,
								batchInfo,
								qos,
								options.telemetryInfo.callTracker.add(
									'toSemanticChunks',
								),
								cts.token,
							),
							options.semanticTimeout ?? Infinity,
							() => cts.cancel(),
						);
					} finally {
						cts.dispose();
				...
				if (!semanticChunks) {
					this._logService.error(
						`toSemanticChunks - Could not get semantic chunks for ${uri}`,
					);
					...
					for (const chunk of chunks) {
						for (const semanticChunk of semanticChunks) {
							if (
								semanticChunk.chunk.range.intersectRanges(
									chunk.range,
								)
							) {
								const key =
									semanticChunk.chunk.range.toString();
								resolvedFileSemanticChunks.set(key, {
									chunk: semanticChunk.chunk,
									distance: semanticChunk.embedding
										? distance(
												await query,
												semanticChunk.embedding,
											)
										: undefined,
								});
							}
						...
						if (!resolvedFileSemanticChunks.size) {
							this._logService.error(
								`No semantic chunk found for in ${uri} for chunk ${chunk.range}`,
							);
	...
	private rankEmbeddings(
		...
		fileChunksAndEmbeddings: readonly FileChunkWithEmbedding[],
		maxResults: number,
	): FileChunkAndScore[] {
		return rankEmbeddings(
			queryEmbedding,
			fileChunksAndEmbeddings.map((x) => [x.chunk, x.embedding]),
			maxResults,
		).map(
			(x): FileChunkAndScore => ({
				chunk: x.value,
	 ...
	 */
	private async indexAllWorkspaceFiles(
		trigger: BuildIndexTriggerReason,
		include: GlobIncludeOptions,
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<void> {
		const allWorkspaceFiles = Array.from(this._workspaceIndex.values());
		...
		return logExecTime(
			this._logService,
			'WorkspaceChunkEmbeddingIndex.indexAllWorkspaceFiles',
			async () => {
				const authToken =
					trigger === 'auto'
						? (
								await this._authService.getGitHubSession(
									'any',
									{ silent: true },
								)
							)?.accessToken
						: (
								await this._authService.getGitHubSession(
									'any',
									{
										createIfNone: {
											detail: l10n.t(
												'Sign in to GitHub to index workspace files.',
											),
										},
									},
								)
							...
							return limiter.queue(async () => {
								...
								if (shouldInclude(file.uri, include)) {
									await this.getChunksAndEmbeddings(
										...
										EmbeddingsComputeQos.Batch,
										telemetryInfo.callTracker.add(
											'WorkspaceChunkEmbeddingsIndex::getAllWorkspaceEmbeddings',
										),
										token,
									);
								}
				...
				this._telemetryService.sendMSFTTelemetryEvent(
					...
					},
					{
						...
						recomputedTotalContentLength:
							batchInfo.sentContentTextLength,
					},
				);
			},
		);
	...
	private async getAllWorkspaceEmbeddings(
		trigger: BuildIndexTriggerReason,
		include: GlobIncludeOptions,
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<FileChunkWithEmbedding[]> {
		await this.indexAllWorkspaceFiles(
			trigger,
			include,
			telemetryInfo,
		...
		const limiter = new Limiter<
			readonly FileChunkWithEmbedding[] | undefined
		>(maxParallelEmbeddingOps);
		const perFileChunks = await raceCancellationError(
			Promise.all(
				allFiles.map((file) => {
					return limiter.queue(async () => {
						if (token.isCancellationRequested) {
	...
	private async getEmbeddingsForFiles(
		files: readonly URI[],
		include: GlobIncludeOptions,
		...
		return logExecTime(
			this._logService,
			'workspaceChunkEmbeddingsIndex.getEmbeddingsForFiles',
			async () => {
				this._logService.trace(
					`workspaceChunkEmbeddingsIndex: Getting auth token `,
				);
				const authToken = await this.tryGetAuthToken();
				if (!authToken) {
					throw new Error('Unable to get auth token');
				...
				const limiter = new Limiter<
					readonly FileChunkWithEmbedding[] | undefined
				>(maxParallelEmbeddingOps);
				const chunksAndEmbeddings = await raceCancellationError(
					Promise.all(
						files.map((uri) => {
							return limiter.queue(async () => {
								if (token.isCancellationRequested) {
								...
								const file = await raceCancellationError(
									this._workspaceIndex.tryLoad(uri),
									token,
								);
								if (!file) {
									return;
								...
								return raceCancellationError(
									this.getChunksAndEmbeddings(
										...
										batchInfo,
										qos,
										telemetry.info.callTracker.add(
											'WorkspaceChunkEmbeddingsIndex::getEmbeddingsForFiles',
										),
										token,
									),
	 ...
	 */
	private async getChunksAndEmbeddings(
		authToken: string,
	 ...
	 */
	private async getChunksWithOptionalEmbeddings(
		authToken: string,
		file: FileRepresentation,
		batchInfo: ComputeBatchInfo,
		qos: EmbeddingsComputeQos,
		...
		const cachedChunks = cache.getCurrentChunksForUri(file.uri);
		return this._chunkingEndpointClient.computeChunks(
			authToken,
			this._embeddingType,
			file,
			batchInfo,
