 ...
 */
const BLACKBIRD_EMBEDDINGS_URL =
...
export class ScenarioAutomationWorkspaceChunkSearchService implements IWorkspaceChunkSearchService {
	...
	async searchFileChunks(
		...
		const nwo = repoInfo
			? toGithubNwo(repoInfo.id)
			: (process.env.SWEBENCH_REPO ?? '');

		const queryText = query.queryText;
		const maxResults = sizing.maxResults ?? 20;

		this._logService.trace(
			`ScenarioAutomationWorkspaceChunkSearchService: searching for "${queryText}" in repo ${nwo}`,
		);

		if (!nwo) {
			this._logService.error(
				'ScenarioAutomationWorkspaceChunkSearchService: no repo NWO available (git has no remotes and SWEBENCH_REPO is unset)',
			);
			return { chunks: [] };
		}
		...
		try {
			response = await this._fetcherService.fetch(
				BLACKBIRD_EMBEDDINGS_URL,
				{
					callSite:
						'ScenarioAutomationWorkspaceChunkSearchService.searchFileChunks',
					method: 'POST',
			...
			if (
				...
				this._fetcherService.isAbortError(e)
			) {
				this._logService.trace(
					'ScenarioAutomationWorkspaceChunkSearchService: search cancelled',
				);
				return { chunks: [] };
			}
			this._logService.error(
				`ScenarioAutomationWorkspaceChunkSearchService: fetch failed: ${e instanceof Error ? e.message : e}`,
			);
			return { chunks: [] };
		} finally {
		...
		if (!response.ok) {
			...
			this._logService.error(
				`ScenarioAutomationWorkspaceChunkSearchService: search failed with status ${response.status}, body: ${errorBody}`,
			);
			return { chunks: [] };
		}
		...
		try {
			...
			this._logService.error(
				`ScenarioAutomationWorkspaceChunkSearchService: failed to parse response JSON: ${e instanceof Error ? e.message : e}`,
			);
			return { chunks: [] };
		}
		...
		if (!Array.isArray(parsedBody.results)) {
			this._logService.error(
				'ScenarioAutomationWorkspaceChunkSearchService: unexpected response shape',
			);
			return { chunks: [] };
		}

		const embeddingType = new EmbeddingType(
			parsedBody.embedding_model ??
		...
		this._logService.trace(
			`ScenarioAutomationWorkspaceChunkSearchService: got ${chunks.length} chunks`,
		);
		return { chunks };
	}
