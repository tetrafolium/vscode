...
} from '../../../remoteCodeSearch/common/remoteCodeSearch';
...
export namespace TriggerRemoteIndexingError {
	...
	export const notIndexable: TriggerIndexingError = {
		id: 'not-indexable',
		userMessage: l10n.t(
			'No indexable repos found and support for indexing non-GitHub repos is not available',
		),
	};
	...
	export const couldNotCheckIndexStatus: TriggerIndexingError = {
		...
		userMessage: l10n.t(
			'Could not check the remote index status for this repo',
		),
	};

	export function errorTriggeringIndexing(
		repoId: GithubRepoId | AdoRepoId,
...
export type RemoteCodeSearchState =
	| {
			readonly status:
				| CodeSearchRepoStatus.BuildingIndex
				| CodeSearchRepoStatus.CheckingStatus
				| CodeSearchRepoStatus.CouldNotCheckIndexStatus
				| CodeSearchRepoStatus.NotAuthorized
				| CodeSearchRepoStatus.NotIndexable
				| CodeSearchRepoStatus.NotResolvable
				| CodeSearchRepoStatus.Resolving
				| CodeSearchRepoStatus.NotYetIndexed;
	  }
	| {
...
export interface CodeSearchRepo extends IDisposable {
	...
	refreshStatusFromEndpoint(
		...
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<RemoteCodeSearchState | undefined>;
}

abstract class BaseRemoteCodeSearchRepo
	extends Disposable
	implements CodeSearchRepo
{
	...
	private _onDidChangeStatus = this._register(
		new Emitter<CodeSearchRepoStatus>(),
	);
		...
		this.initTask = createCancelablePromise<void>(async (initToken) => {
			try {
				...
				await raceCancellationError(
					this.refreshStatusFromEndpoint(
						false,
						new TelemetryCorrelationId('CodeSearchRepo::init'),
						initToken,
					),
					initToken,
				);
				...
				if (!isCancellationError(e)) {
					this._logService.error(
						`CodeSearchChunkSearch.openGitRepo(${repoInfo.rootUri}). Failed to initialize repo state from endpoint. ${e}`,
					);
				}
			}
		});
	...
	public async initialize(): Promise<void> {
		try {
			...
			this._logService.error(
				`Error during repo initialization: ${error}`,
			);
		}
	}
	...
	public abstract triggerRemoteIndexingOfRepo(
		triggerReason: BuildIndexTriggerReason,
		telemetryInfo: TelemetryCorrelationId,
	): Promise<Result<true, TriggerIndexingError>>;
	public abstract prepareSearch(
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<boolean>;

	public async refreshStatusFromEndpoint(
		force = false,
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<RemoteCodeSearchState | undefined> {
		if (
			!force &&
			(this.status === CodeSearchRepoStatus.Ready ||
				this.status === CodeSearchRepoStatus.NotAuthorized)
		) {
			return;
		}

		this._logService.trace(
			`CodeSearchChunkSearch.updateRepoStateFromEndpoint(${this.repoInfo.rootUri}). Checking status from endpoint.`,
		);

		const newState = await raceCancellationError(
			this.fetchRemoteIndexState(telemetryInfo, token),
			token,
		);
		this._logService.trace(
			`CodeSearchChunkSearch.updateRepoStateFromEndpoint(${this.repoInfo.rootUri}). Updating state to ${newState.status}.`,
		);
	...
	protected async fetchRemoteIndexState(
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<RemoteCodeSearchState> {
		this._logService.trace(
			`CodeSearchChunkSearch.getRepoIndexStatusFromEndpoint(${this.repoInfo.rootUri}`,
		);

		const statusResult = await this.doFetchRemoteIndexState(
			telemetryInfo,
			token,
		);
		if (!statusResult.isOk()) {
			if (statusResult.err.type === 'not-authorized') {
				this._logService.error(
					`CodeSearchChunkSearch::getIndexedStatus(${this.remoteInfo.repoId}). Failed to fetch indexing status. Unauthorized.`,
				);
				return { status: CodeSearchRepoStatus.NotAuthorized };
			} else {
				this._logService.error(
					`CodeSearchChunkSearch::getIndexedStatus(${this.remoteInfo.repoId}). Failed to fetch indexing status. Encountered error: ${statusResult.err.error}`,
				);
				return {
					status: CodeSearchRepoStatus.CouldNotCheckIndexStatus,
				};
			}
		}
		...
		switch (statusResult.val.status) {
			...
			case RemoteCodeSearchIndexStatus.BuildingIndex:
				return { status: CodeSearchRepoStatus.BuildingIndex };
			case RemoteCodeSearchIndexStatus.NotYetIndexed:
				return { status: CodeSearchRepoStatus.NotYetIndexed };
			case RemoteCodeSearchIndexStatus.NotIndexable:
				return { status: CodeSearchRepoStatus.NotResolvable };
		}
	}

	protected abstract doFetchRemoteIndexState(
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	...
	private pollForRepoIndexingToComplete(): Promise<void> {
		this._logService.trace(
			`CodeSearchChunkSearch.startPollingForRepoIndexingComplete(${this.repoInfo.rootUri})`,
		);
		...
		const runPoll = async () => {
			try {
				while (true) {
					if (this._isDisposed) {
						this._logService.trace(
							`CodeSearchChunkSearch.startPollingForRepoIndexingComplete(${this.repoInfo.rootUri}). Repo no longer tracked.`,
						);
						return;
					}
					...
					if (this.status !== CodeSearchRepoStatus.BuildingIndex) {
						this._logService.trace(
							`CodeSearchChunkSearch.startPollingForRepoIndexingComplete(${this.repoInfo.rootUri}). Found unexpected repo state: ${this.status}. Stopping polling`,
						);
						return;
					}
					...
					if (attemptNumber >= this._maxPollingAttempts) {
						this._logService.trace(
							`CodeSearchChunkSearch.startPollingForRepoIndexingComplete(${this.repoInfo.rootUri}). Max attempts reached. Stopping polling.`,
						);
						if (!this._isDisposed) {
							this.updateState({
								status: CodeSearchRepoStatus.CouldNotCheckIndexStatus,
					...
					const delay =
						this._initialPollingDelay *
						Math.pow(1.5, attemptNumber);
					await timeout(delay);
					if (this._isDisposed) {
						return;
					}
					...
					this._logService.trace(
						`CodeSearchChunkSearch.startPollingForRepoIndexingComplete(${this.repoInfo.rootUri}). Checking endpoint for status.`,
					);
					let polledState: RemoteCodeSearchState | undefined;
					try {
						polledState = await this.fetchRemoteIndexState(
							new TelemetryCorrelationId(
								new CallTracker('CodeSearchRepo::poll'),
							),
					...
					this._logService.trace(
						`CodeSearchChunkSearch.startPollingForRepoIndexingComplete(${this.repoInfo.rootUri}). Got back new status from endpoint: ${polledState?.status}.`,
					);

					switch (polledState?.status) {
						case CodeSearchRepoStatus.Ready: {
							this._logService.trace(
								`CodeSearchChunkSearch.startPollingForRepoIndexingComplete(${this.repoInfo.rootUri}). Repo indexed successfully.`,
							);
							if (!this._isDisposed) {
...
export class GithubCodeSearchRepo extends BaseRemoteCodeSearchRepo {
	...
	public override async searchRepo(
		authOptions: { silent: boolean },
		embeddingType: EmbeddingType,
		resolvedQuery: string,
		maxResultCountHint: number,
		...
		const result = await this._githubCodeSearchService.semanticSearch(
			authOptions,
			embeddingType,
			{
				kind: 'repo',
	...
	protected async doFetchRemoteIndexState(
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<Result<RemoteCodeSearchIndexState, RemoteCodeSearchError>> {
	...
	public async triggerRemoteIndexingOfRepo(
		triggerReason: BuildIndexTriggerReason,
		telemetryInfo: TelemetryCorrelationId,
	): Promise<Result<true, TriggerIndexingError>> {
		this._logService.trace(
			`Triggering indexing for repo: ${this.remoteInfo.repoId} `,
		);
		...
		const triggerSuccess =
			await this._githubCodeSearchService.triggerIndexing(
				{ silent: true },
				triggerReason,
				this._githubRepoId,
				telemetryInfo,
		...
		if (!triggerSuccess) {
			this._logService.error(
				`RepoTracker::TriggerRemoteIndexing(${triggerReason}). Failed to request indexing for '${this.remoteInfo.repoId}'.`,
			);

			this.updateState({ status: CodeSearchRepoStatus.NotYetIndexed });

			return Result.error(
				TriggerRemoteIndexingError.errorTriggeringIndexing(
					this.remoteInfo.repoId,
				),
			);
		}
					...
					(async () => {
						if (this.status === CodeSearchRepoStatus.Ready) {
							const timeSinceLastRefresh =
								Date.now() - this._lastIndexStateRefreshTime;
							if (
								timeSinceLastRefresh >=
									this._indexStateRefreshInterval ||
								this._hadOutOfSyncResult
							) {
								this._hadOutOfSyncResult = false;
								const newState = await raceCancellationError(
									this.fetchRemoteIndexState(
										telemetryInfo,
										token,
									),
									token,
						...
						if (
							this.status === CodeSearchRepoStatus.NotYetIndexed
						) {
							const triggerResult = await raceCancellationError(
								this.triggerRemoteIndexingOfRepo(
									'auto',
									telemetryInfo,
								),
								token,
							...
							if (triggerResult.isError()) {
								throw new Error(
									`CodeSearchChunkSearch: Triggering indexing of '${this.remoteInfo.repoId}' failed: ${triggerResult.err.id}`,
								);
							}
						...
						if (
							this.status === CodeSearchRepoStatus.BuildingIndex
						) {
							...
							while (attemptsRemaining-- > 0) {
								...
								if (
									currentStatus === CodeSearchRepoStatus.Ready
								) {
									...
									throw new Error(
										`CodeSearchChunkSearch: Checking instant indexing status of '${this.remoteInfo.repoId}' failed. Found unexpected status: '${currentStatus}'`,
									);
								}

								await raceCancellationError(
									timeout(delayBetweenAttempts),
									token,
								);
							}
						}
					})(),
					unindexRepoInitTimeout,
				),
			(execTime, status) => {
...
export class AdoCodeSearchRepo extends BaseRemoteCodeSearchRepo {
	...
	public searchRepo(
		...
		return this._adoCodeSearchService.searchRepo(
			authOptions,
			{
	...
	public override async prepareSearch(
		telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<boolean> {
	...
	public override async triggerRemoteIndexingOfRepo(
		triggerReason: BuildIndexTriggerReason,
		telemetryInfo: TelemetryCorrelationId,
	): Promise<Result<true, TriggerIndexingError>> {
	...
	protected override doFetchRemoteIndexState(
		_telemetryInfo: TelemetryCorrelationId,
		token: CancellationToken,
	): Promise<Result<RemoteCodeSearchIndexState, RemoteCodeSearchError>> {
		return this._adoCodeSearchService.getRemoteIndexState(
			{ silent: true },
			this._adoRepoId,
			token,
		);
