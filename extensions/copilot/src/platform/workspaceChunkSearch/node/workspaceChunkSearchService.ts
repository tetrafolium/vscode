...
export const IWorkspaceChunkSearchService =
	createServiceIdentifier<IWorkspaceChunkSearchService>(
...
export class WorkspaceChunkSearchService
	extends Disposable
	implements IWorkspaceChunkSearchService
{
	...
	constructor(
		...
		private readonly _availableEmbeddingTypes: IGithubAvailableEmbeddingTypesService,
	...
	private async tryInit(
		silent: boolean,
	): Promise<WorkspaceChunkSearchServiceImpl | undefined> {
		if (
		...
		try {
			const best =
				await this._availableEmbeddingTypes.getPreferredType(silent);
			...
			if (best) {
				this._logService.info(
					`WorkspaceChunkSearchService: using embedding type ${best}`,
				);
				this._impl = this._register(
				...
				this._register(
					this._impl.onDidChangeIndexState(() =>
	...
	implements IWorkspaceChunkSearchService
{
	...
	private readonly shouldEagerlyIndexKey =
		'workspaceChunkSearch.shouldEagerlyIndex';
	private readonly hasPromptedForExternalIngestKey =
		'workspaceChunkSearch.externalIngest.prompted';
	...
	constructor(
		...
		this._codeSearchChunkSearch = this._register(
	...
	async getIndexState(): Promise<WorkspaceIndexState> {
		return {
			remoteIndexState: this._codeSearchChunkSearch.getRemoteIndexState(
				this.hasPromptedForExternalIngest,
			),
		};
	}

	async isAvailable(): Promise<boolean> {
		return this._codeSearchChunkSearch.isAvailable(
			new TelemetryCorrelationId(
				'WorkspaceChunkSearchServiceImpl.isAvailable',
			),
			false,
	...
	triggerIndexing(
		...
		return this._codeSearchChunkSearch.triggerIndexing(
			trigger,
			onProgress,
			telemetryInfo,
			token,
	...
	deleteExternalIngestWorkspaceIndex(): Promise<void> {
		return this._codeSearchChunkSearch.deleteExternalIngestWorkspaceIndex(
			new TelemetryCorrelationId(
				'WorkspaceChunkSearchService::deleteExternalIngestWorkspaceIndex',
			),
			CancellationToken.None,
		);
	}
	...
	async searchFileChunks(
		...
		const wasFirstSearchInWorkspace =
			!this._extensionContext.workspaceState.get(
				this.shouldEagerlyIndexKey,
				false,
			);
		...
		await raceCancellationError(
			this.promptForExternalIngestIfNeeded(token),
			token,
		);

		return logExecTime(
			this._logService,
			'WorkspaceChunkSearch.searchFileChunks',
			async (): Promise<WorkspaceChunkSearchResult> => {
				...
				const queryWithEmbeddings = this.toQueryWithEmbeddings(
					query,
					token,
				);
				...
				const searchTask = this.doSearchFileChunks(
					stratSizing,
					queryWithEmbeddings,
					options,
					telemetryInfo,
				...
				progress?.report(
					new ChatResponseProgressPart2(
						l10n.t('Collecting workspace information'),
						async () => {
							await searchTask;
						},
					),
			...
			*/
				this._telemetryService.sendMSFTTelemetryEvent(
					'workspaceChunkSearchStrategy',
					{
						...
						errorDiagMessage: searchResult.isError()
							? searchResult.err.errorDiagMessage
							: undefined,
						embeddingType: this._embeddingType.id,
					...
					},
					{
						...
						workspaceIndexFileCount:
							this._workspaceFileIndex.fileCount,
						wasFirstSearchInWorkspace: wasFirstSearchInWorkspace
							? 1
							: 0,
					},
				);
				...
				if (searchResult.isError()) {
					this._logService.error(
						`WorkspaceChunkSearch.searchFileChunks: no strategies succeeded`,
					);
					if (this._simulationTestContext.isInSimulationTests) {
						throw new Error(
							'All workspace search strategies failed',
						);
					}
				...
				this._logService.trace(
					`WorkspaceChunkSearch.searchFileChunks: found ${searchResult.val.chunks.length} chunks`,
				);

				const filteredChunks = await raceCancellationError(
					this.filterIgnoredChunks(searchResult.val.chunks),
					token,
				);
				if (this._simulationTestContext.isInSimulationTests) {
				...
				if (options.enableRerank && this._rerankerService.isAvailable) {
					try {
						const reranked = await this._rerankerService.rerank(
							query.queryText,
							filteredResult.result.chunks,
							token,
						);
						return {
							chunks: reranked.slice(
								0,
								this.getMaxChunks(sizing),
							),
						...
						this._logService.error(
							e,
							'Reranker service failed; falling back to local rerank',
						);
					}
				}

				return this.rerankResultIfNeeded(
					queryWithEmbeddings,
					filteredResult,
					this.getMaxChunks(sizing),
					telemetryInfo,
	...
	private get hasPromptedForExternalIngest(): boolean {
		return this._extensionContext.workspaceState.get(
			this.hasPromptedForExternalIngestKey,
			false,
		);
	}
	...
	private async promptForExternalIngestIfNeeded(
		token: CancellationToken,
	): Promise<void> {
		if (
			token.isCancellationRequested ||
			this.hasPromptedForExternalIngest ||
			this._codeSearchChunkSearch.getExternalIngestEnablement() !==
		...
		const result = await this._notificationService.showInformationMessage(
			l10n.t(
				'Enable external ingest for semantic codebase search in this workspace? External ingest availability is controlled by your Copilot policy and can be disabled in workspace settings.',
			),
			enable,
		);
	...
	private toQueryWithEmbeddings(
		...
		const queryEmbeddings: Promise<Embedding> = logExecTime(
			...
			async () => {
				const result = await this.computeEmbeddings(
					'query',
					[query.queryText],
					token,
				);
	...
	private async doSearchFileChunks(
		...
		this._logService.debug(
			`Searching for ${sizing.maxResultCountHint} chunks in workspace`,
		);

		try {
			await raceCancellationError(
				this._codeSearchChunkSearch.prepareSearchWorkspace(
					telemetryInfo,
					token,
				),
				token,
			...
			const result = await raceCancellationError(
				this._codeSearchChunkSearch.searchWorkspace(
					sizing,
					query,
					options,
					telemetryInfo,
		...
		return Result.error<StrategySearchErr>({
			...
			alerts: [
				new ChatResponseWarningPart(
					l10n.t(
						'Semantic search is not available for this workspace.',
					),
				),
			],
		});
		...
		if (typeof sizing.maxResults === 'number') {
			maxResults =
				typeof maxResults === 'number'
					? Math.min(sizing.maxResults, maxResults)
					: sizing.maxResults;
		}

		if (typeof maxResults !== 'number') {
			throw new Error(
				'Either maxResults or tokenBudget must be provided',
			);
		}
	...
	private async filterIgnoredChunks(
		chunks: readonly FileChunkAndScore[],
	...
	@LogExecTime(
		...
		'WorkspaceChunkSearch::rerankResultIfNeeded',
	)
	...
	@LogExecTime(
		(self) => self._logService,
		'WorkspaceChunkSearch::rerankChunks',
	)
	private async rerankChunks(
		query: WorkspaceChunkQueryWithEmbeddings,
		inChunks: readonly FileChunkAndScore[],
		maxResults: number,
		telemetryInfo: TelemetryCorrelationId,
		progress: vscode.Progress<vscode.ChatResponsePart> | undefined,
		token: CancellationToken,
		...
		try {
			...
			const firstChunkEmbeddingType =
				inChunks.at(0)?.distance?.embeddingType;
			if (
				firstChunkEmbeddingType &&
				inChunks.every(
					(x) =>
						typeof x.distance !== 'undefined' &&
						x.distance.embeddingType.equals(
							firstChunkEmbeddingType,
						),
				)
			) {
				sortedChunks = [
					...(inChunks as Array<ScoredFileChunk<FileChunk>>),
				...
				const chunksPlusIndexes = inChunks.map((x, i) => ({
					...x.chunk,
					distance: x.distance,
					index: i,
				}));

				const unscoredChunks = chunksPlusIndexes.filter(
					(entry) =>
						typeof entry.distance === 'undefined' ||
						!entry.distance.embeddingType.equals(
							this._embeddingType,
						),
				);
				let newlyScoredChunks:
					| Array<ScoredFileChunk<FileChunk & { index: number }>>
					| undefined;

				if (unscoredChunks.length) {
					this._logService.debug(
						`WorkspaceChunkSearch.rerankChunks. Scoring ${unscoredChunks.length} new chunks`,
					);

					// Only show progress when we're doing a potentially long running operation
					const scoreTask = this.scoreChunks(
						query,
						unscoredChunks,
						telemetryInfo,
						token,
					...
					progress?.report(
						new ChatResponseProgressPart2(
							l10n.t('Filtering to most relevant information'),
							async () => {
								await scoreTask;
							},
						),
					);
					newlyScoredChunks = await raceCancellationError(
						scoreTask,
						token,
					);
				}
				...
				sortedChunks = out
					.filter((chunk) =>
						chunk?.distance?.embeddingType.equals(
							this._embeddingType,
						),
					)
					.sort((a, b) => b.distance.value - a.distance.value);
			...
			const filteredChunks = sortedChunks.filter(
				(x) => x.distance.value >= lowestAllowedScore,
			);
			this._logService.debug(
				`Eagerly filtered out ${sortedChunks.length - filteredChunks.length} chunks due to low quality`,
			);
			...
			if (!isCancellationError(e)) {
				this._logService.error(
					e,
					'Failed to search chunk embeddings index',
				);
			}
	...
	private async scoreChunks<T extends FileChunk>(
		...
		return logExecTime(
			this._logService,
			'WorkspaceChunkSearch.scoreChunks',
			async () => {
				if (!chunks.length) {
					return [];
				}
				...
				const chunkStrings = chunks.map((chunk) =>
					this.chunkToIndexString(chunk),
				);

				const [queryEmbeddings, chunkEmbeddings] =
					await raceCancellationError(
						Promise.all([
				...
				return chunkEmbeddings.values.map(
					(embedding, index): ScoredFileChunk<T> => ({
						chunk: chunks[index],
	...
	private computeEmbeddings(
		...
		return this._embeddingsComputer.computeEmbeddings(
			this._embeddingType,
			strings,
			{ inputType },
			new TelemetryCorrelationId(
				'WorkspaceChunkSearchService::computeEmbeddings',
			),
			token,
		);
	}
	...
	private chunkToIndexString(chunk: FileChunk): string {
		...
		const displayPath = getWorkspaceFileDisplayPath(
	...
	private toStringForEmbeddingsComputer(
		chunk: FileChunk,
		displayPath: string,
	) {
...
export class NullWorkspaceChunkSearchService implements IWorkspaceChunkSearchService {
	...
	triggerIndexing(
		_trigger?: BuildIndexTriggerReason,
