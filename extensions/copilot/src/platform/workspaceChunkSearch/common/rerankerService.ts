...
export const IRerankerService =
	createServiceIdentifier<IRerankerService>('IRerankerService');
...
export interface IRerankerService {
	...
	rerank(
		query: string,
		documents: readonly FileChunkAndScore[],
		token: CancellationToken,
	): Promise<readonly FileChunkAndScore[]>;
	/**
...
export class RerankerService implements IRerankerService {
	...
	private get _endpoint(): string | undefined {
		return this._expService
			.getTreatmentVariable<string>('rerankEndpointUrl')
			?.trim();
	}
		...
		try {
			...
			if (!response.ok) {
				this._logService.error(
					`RerankerService::rerank request failed. status=${response.status}`,
				);
				throw new Error(
			...
			const json = await raceCancellationError(
				response.json() as Promise<RemoteRerankResponse>,
				token,
			);
