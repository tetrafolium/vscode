...
export function isXmlContent(text: string): boolean {
	return (
		text.includes(StartDelimter) ||
		text.includes(EndDelimter) ||
		text.includes(StartEmptyCellDelimter)
	);
}
...
export class AlternativeXmlNotebookContentProvider extends BaseAlternativeNotebookContentProvider {
	...
	public stripCellMarkers(text: string): string {
		...
		if (
			lines.length &&
			(lines[0].startsWith(StartDelimter) ||
				lines[0].startsWith(StartEmptyCellDelimter))
		) {
			lines.shift();
		}
		if (
			lines.length &&
			lines[lines.length - 1].trim().endsWith(EndDelimter)
		) {
	...
	public async *parseAlternateContent(
		...
		for await (const lineOfText of inputStream) {
			...
			if (
				(line.startsWith(StartDelimter) ||
					line.startsWith(StartEmptyCellDelimter)) &&
				(index < 0 ||
					endDelimiterSeen ||
					(previousLineEndedWithEndCellMarker && previousLine))
			) {
				if (
					!endDelimiterSeen &&
					previousLineEndedWithEndCellMarker &&
					previousLine
				) {
					// Last line didn't finish, emit that, but strip the end delimiter.
					previousLine.line = previousLine.line.substring(
						0,
						previousLine.line.lastIndexOf(EndDelimter),
					);
					yield previousLine;
	...
	public override getAlternativeDocument(
		...
		const cellContent = cells.map((cell) => {
			...
			return {
				content: `${prefix}${cell.source.join(EOL)}${EOL}${EndDelimter}`,
				prefix,
		...
		const cellOffsetMap = cellContent.map((cellContent) => ({
			offset:
				content.indexOf(cellContent.content) +
				cellContent.prefix.length,
