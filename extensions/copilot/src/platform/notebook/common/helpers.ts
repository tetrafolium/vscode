...
export function getCellIdMap(
	notebook: NotebookDocument,
): Map<string, NotebookCell> {
...
export const CellIdPatternRe = new RegExp(
	`(\\s+|^|\\b|\\W)(#VSC-[a-f0-9]{${CELL_ID_HASH_LENGTH}})\\b`,
	'gi',
);
...
export function normalizeCellId(cellId: string): string {
	...
	if (
		cellId.toLowerCase().startsWith('vscode-') &&
		cellId.length === CELL_ID_HASH_LENGTH + 7
	) {
	...
	return cellId.length === CELL_ID_HASH_LENGTH
		? `${CELL_ID_PREFIX}${cellId}`
		: cellId;
}
...
export function requestHasNotebookRefs(
	...
	if (
		options?.checkPromptAsWell &&
		notebookTermsToLookFor.some((term) => prompt.includes(term))
	) {
		return true;
	}
...
export function parseAndCleanStack(jsonString: string): string {
	try {
		...
		return (
			removeAnsiEscapeCodes(parsed?.stack || parsed.message || '') ||
			parsed.message ||
			parsed.name ||
			jsonString
		);
