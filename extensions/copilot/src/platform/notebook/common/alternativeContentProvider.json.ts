...
class AlternativeJsonDocument extends AlternativeNotebookDocument {
	override fromCellPosition(
		...
		const positionOfSource = alternativeContentText.indexOf(
			`"source": [`,
		...
		const firstLineIndexOfCellSource =
			this.positionAt(positionOfSource).line + 1;
		const leadingCharacters = cell.document.getText(
		...
		const linePositionInAltContent =
			position.line + firstLineIndexOfCellSource;
		// -1 to exclude to trailing `"`
		return new Position(
			linePositionInAltContent,
			characterPositionInAltContent.length,
		);
	}
...
export class AlternativeJsonNotebookContentProvider extends BaseAlternativeNotebookContentProvider {
	...
	private parseAlternateContentImpl(
		notebookOrUri: NotebookDocument | URI,
		inputStream: AsyncIterable<LineOfText>,
		token: CancellationToken,
	): AsyncIterable<LineOfCellText> {
		return new AsyncIterableObject<LineOfCellText>(async (emitter) => {
			const cellIdMap = URI.isUri(notebookOrUri)
			...
			const defaultLanguage = URI.isUri(notebookOrUri)
				? 'python'
				: getDefaultLanguage(notebookOrUri);
			...
			const emitCell = (endOffset: number) => {
				...
				if (
					cellInfo.id &&
					cellIdMap.get(cellInfo.id)?.document.languageId ===
						cellInfo.language
				) {
				...
				cellInfo.source.forEach((cellLine) =>
					emitter.emitOne({
						index: cellInfo.index,
						type: 'line',
						line: cellLine,
