...
export function generateCellTextMarker(
	cell: SummaryCell,
	lineComment: string,
): string {
...
class AlternativeTextDocument extends AlternativeNotebookDocument {
	constructor(
		text: string,
		private readonly cellOffsetMap: {
			offset: number;
			sourceOffset: number;
			cell: NotebookCell;
		}[],
	...
	override fromCellPosition(
		...
		const markdownOffset =
			cell.kind === NotebookCellKind.Markup
				? blockComment[0].length + eolLength
				: 0;
		const offset =
			alternativeContentText.indexOf(cellMarker) +
			cellMarker.length +
			eolLength +
			markdownOffset +
			offsetInCell;
	...
	override toCellPosition(
		position: Position,
	): { cell: NotebookCell; position: Position } | undefined {
		const offset = this.offsetAt(position);
		const cell = findLast(
			this.cellOffsetMap,
			(cell) => cell.sourceOffset <= offset,
		);
		if (!cell) {
			return undefined;
		}
		const cellPosition = cell.cell.document.positionAt(
			offset - cell.sourceOffset,
		);
...
export class AlternativeTextNotebookContentProvider extends BaseAlternativeNotebookContentProvider {
	...
	public override getSummaryOfStructure(
		...
		notebook.getCells().forEach((cell) => {
			if (cellsToInclude.includes(cell)) {
				const cellSummary = summarize(cell);
				if (
					cellSummary.source.length &&
					cellSummary.source[0].trim().length
				) {
					cellSummary.source = [
						cellSummary.source[0],
						existingCodeMarkerWithComment,
					];
				} else if (
					cellSummary.source.length &&
					cellSummary.source.some((line) => line.trim().length)
				) {
					cellSummary.source = [
						existingCodeMarkerWithComment,
						cellSummary.source.filter(
				...
				!lines.length ||
				lines[lines.length - 1] !== existingCodeMarkerWithComment
			) {
	...
	public override async *parseAlternateContent(
		...
		const lineCommentStart = getLineCommentStart(
			isNotebook ? notebookOrUri : undefined,
		);
		const blockComment = getBlockComment(
			isNotebook ? notebookOrUri : undefined,
		);
		const defaultLanguage = isNotebook
			? getLanguage(getDefaultLanguage(notebookOrUri)).languageId
			: undefined;
		...
		for await (const lineOfText of inputStream) {
			...
			const isLineCommentForEmptyCellWithoutCellMarker =
				line.startsWith(`${lineCommentStart}%% [`) &&
				line.trimEnd().endsWith(']');
			const isLineCommentWithCellMarker = line.startsWith(
				`${lineCommentStart}%% vscode.cell`,
			);
			...
			const cellParts =
				isLineCommentWithCellMarker ||
				isLineCommentForEmptyCellWithoutCellMarker
			...
			if (
				...
				if (
					cellParts.id &&
					cellIdMap.get(cellParts.id)?.document.languageId ===
						cellParts.language
				) {
				...
				lineOfCellText.kind =
					cell?.kind ||
					(lineOfCellText.language === 'markdown'
	...
	public override getAlternativeDocumentFromText(
		text: string,
		notebook: NotebookDocument,
	...
	public override getAlternativeDocument(
		notebook: NotebookDocument,
		...
		const cellOffsetMap = cellContent.map((cellContent) => {
			...
			return {
				offset,
				sourceOffset,
				cell: notebook.cellAt(cellContent.cell.index),
			};
		});
...
function generateAlternativeCellTextContent(
	cell: SummaryCell,
	lineCommentStart: string,
	blockComment: [string, string],
): { content: string; prefix: string } {
