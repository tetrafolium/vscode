		...
		describe(`${provider.kind} Edit Generator`, () => {
			test(`Generate a single Notebook Edit instead of deleting all when receiving an invalid format`, async () => {
				...
				const alternativeContents =
					'# Cell 1: Print a simple number\nprint(1234)';
				const alternativeContentLines = AsyncIterableObject.fromArray(
				...
				for await (const edit of edits) {
					if (Array.isArray(edit)) {
						throw new Error(
							'Expected a NotebookEdit, but got TextEdit',
						);
					} else {
				...
				).toEqual([`# Cell 1: Print a simple number`, `print(1234)`]);
