...
export type ChatFetchError =
	...
	| {
			...
			rateLimitKey: string;
			isAuto: boolean;
			capiError?: { code?: string; message?: string };
	  }
	/**
...
function getRateLimitMessage(
	...
	const retryAfterString = fetchResult.retryAfter
		? secondsToHumanReadableTime(fetchResult.retryAfter)
		: 'a moment';
	if (fetchResult.capiError?.code?.startsWith('agent_mode_limit_exceeded')) {
		// Rate limited in agent mode
		return l10n.t({
			message:
				'Sorry, you have exceeded the agent mode rate limit. Please switch to ask mode and try again in {0}. [Learn More]({1})',
			args: [
				retryAfterString,
				'https://aka.ms/github-copilot-rate-limit-error',
			],
			comment: [`{Locked=']({'}`],
		});
	}
	if (
		fetchResult.capiError?.code?.startsWith('model_overloaded') ||
		fetchResult.capiError?.code?.startsWith('upstream_provider_rate_limit')
	) {
	...
	if (fetchResult.capiError?.code?.startsWith('user_weekly_rate_limited')) {
		if (fetchResult.retryAfter) {
			const resetDate = new Date(
				Date.now() + fetchResult.retryAfter * 1000,
			);
			const resetDateString = resetDate.toLocaleString(undefined, {
				year: 'numeric',
				month: 'long',
				day: 'numeric',
				hour: 'numeric',
		...
		if (fetchResult.isAuto) {
			return l10n.t({
				...
				args: [
					retryAfterString,
					'https://aka.ms/github-copilot-rate-limit-error',
					fetchResult.capiError.message,
					fetchResult.capiError.code,
				],
				comment: [`{Locked=']({'}`],
			});
		}
...
export function getQuotaMessageForPlan(
	copilotPlan: string | undefined,
	isUsageBasedBilling?: boolean,
	quotaResetDate?: string,
): string {
	const resetDateString = quotaResetDate
		? new Date(quotaResetDate).toLocaleString(undefined, {
				year: 'numeric',
				month: 'long',
				day: 'numeric',
				hour: 'numeric',
...
function getQuotaHitMessage(
	...
	if (fetchResult.capiError?.code === 'quota_exceeded') {
		...
		fetchResult.capiError?.code === 'additional_spend_limit_reached'
	) {
		return l10n.t(
			`You've reached your additional usage limit for your plan. Upgrade your plan to keep going.`,
		);
	} else if (
		fetchResult.capiError?.code === 'billing_not_configured' &&
		fetchResult.capiError?.message
	) {
...
export function getErrorDetailsFromChatFetchError(
	...
	return {
		...
		...getErrorDetailsFromChatFetchErrorInner(
			fetchResult,
			copilotPlan,
			gitHubOutageStatus,
			isUsageBasedBilling,
...
function getErrorDetailsFromChatFetchErrorInner(
	fetchResult: ChatFetchError,
	copilotPlan: string | undefined,
	gitHubOutageStatus: GitHubOutageStatus,
	isUsageBasedBilling?: boolean,
	...
	switch (fetchResult.type) {
		...
		case ChatFetchResponseType.QuotaExceeded:
			details = {
				...
				...(fetchResult.capiError?.code && {
					code: fetchResult.capiError.code,
				}),
			};
			break;
	...
	if (gitHubOutageStatus !== GitHubOutageStatus.None) {
		const outageMsg = l10n.t({
			message:
				'Note: GitHub is currently experiencing a service disruption. This may be affecting Copilot. Check [GitHub Status]({0}) for details.',
...
export function getFilteredMessage(
	category: FilterReason,
