...
import {
	...
	parseRemoteUrl,
	toGithubWebUrl,
} from '../../common/gitService';

function assertGitIdEquals(
	a: GithubRepoId | undefined,
	b: { org: string; repo: string; host?: string } | undefined,
	message?: string,
) {
...
suite('parseRemoteUrl', () => {
	...
	test('Should handle basic Azure dev ops url', () => {
		assert.deepStrictEqual(
			parseRemoteUrl(
				'https://test@dev.azure.com/test/project/_git/vscode-stuff',
			),
			{
				host: 'dev.azure.com',
				rawHost: 'dev.azure.com',
				path: '/test/project/_git/vscode-stuff',
			},
		);
	});
	...
	test('Should handle basic visual studio url', () => {
		assert.deepStrictEqual(
			parseRemoteUrl(
				'https://test.visualstudio.com/project/one/_git/two',
			),
			{
				host: 'test.visualstudio.com',
				rawHost: 'test.visualstudio.com',
				path: '/project/one/_git/two',
			},
		);
	});
	...
	test('Should strip user ids', () => {
		...
		assert.deepStrictEqual(
			...
			parseRemoteUrl(
				'https://myname:ghp_1234556@github.com/owner/repo.git',
			),
			{
				host: 'github.com',
				rawHost: 'github.com',
	...
	test('Should preserve rawHost for SSH alias hostnames', () => {
		const result = parseRemoteUrl(
			'git@my-user-name-github.com:owner/repo.git',
		);
	...
	test('Should handle GHE SSH shorthand with alias prefix', () => {
		const result = parseRemoteUrl(
			'msdemo-eu@msdemo-eu.ghe.com:sandbox/repo.git',
		);
...
suite('getGithubRepoIdFromFetchUrl', () => {
	...
	test('should return the repo name for git shorthand URLs', () => {
		...
		assertGitIdEquals(
			getGithubRepoIdFromFetchUrl(
				'org-1234@xyz.github.com:owner-xyz/some-repo.git',
			),
	...
	test('should return the repo name for ssh:// URLs', () => {
		...
		assertGitIdEquals(
			getGithubRepoIdFromFetchUrl(
				'ssh://git@myco.ghe.com/owner/repo.git',
			),
			{ org: 'owner', repo: 'repo' },
			'ghe.com subdomain',
		);
	...
	test('should support urls with custom user names and PAT in urls', () => {
		assertGitIdEquals(
			getGithubRepoIdFromFetchUrl(
				'https://myname@github.com/owner/repo.git',
			),
			{ org: 'owner', repo: 'repo' },
			'https, name only',
		);
		...
		assertGitIdEquals(
			getGithubRepoIdFromFetchUrl(
				'https://ghp_1234556@github.com/owner/repo.git',
			),
			{ org: 'owner', repo: 'repo' },
			'https, PAT only',
		);
	...
	test('should support github urls that are likely ssh aliases', () => {
		...
		assertGitIdEquals(
			getGithubRepoIdFromFetchUrl(
				'git@github.com-my-user-name:owner/repo.git',
			),
			{ org: 'owner', repo: 'repo' },
	...
	test('should set host to github.com for github.com URLs', () => {
		const result = getGithubRepoIdFromFetchUrl(
	...
	test('should set host to ghe.com subdomain for GHE URLs', () => {
		const result = getGithubRepoIdFromFetchUrl(
			'https://xyz.ghe.com/owner/repo.git',
		);
	...
	test('should set host to ghe.com subdomain for GHE SSH shorthand URLs', () => {
		const result = getGithubRepoIdFromFetchUrl(
			'git@msdemo-eu.ghe.com:sandbox/repo.git',
		);
	...
	test('should resolve host for GHE HTTPS with credentials', () => {
		const result = getGithubRepoIdFromFetchUrl(
			'https://user@myco.ghe.com/org/repo.git',
		);
		assert.ok(result, 'Should parse successfully');
		assert.strictEqual(result.host, 'myco.ghe.com');
...
suite('toGithubWebUrl', () => {
	...
	test('should preserve case in org and repo', () => {
		...
		assert.strictEqual(
			toGithubWebUrl(id),
			'https://myco.ghe.com/MyOrg/MyRepo',
		);
	});
	...
	test('should handle deeply nested GHE subdomain', () => {
		...
		assert.strictEqual(
			toGithubWebUrl(id),
			'https://dept.company.ghe.com/org/repo',
		);
	});
});
...
suite('parseRemoteUrl rawHost', () => {
	...
	test('should strip user from rawHost', () => {
		const result = parseRemoteUrl(
			'https://user@myco.ghe.com/owner/repo.git',
		);
		assert.strictEqual(result?.rawHost, 'myco.ghe.com');
	});
});
...
suite('Sanitize Remote Repo Urls', () => {
	...
	test('SSH ports are normalized and removed', () => {
		...
		assert.strictEqual(
			result,
			'https://bitbucket.company.pl/project/repo.git',
		);
	});
...
suite('getAdoRepoIdFromFetchUrl', () => {
	...
	test('should parse https format with _optimized', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'https://dev.azure.com/organization/project/_git/_optimized/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse https format with _full', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'https://dev.azure.com/organization/project/_git/_full/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse legacy https format with _optimized', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'https://organization.visualstudio.com/project/_git/_optimized/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse legacy https format with _full', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'https://organization.visualstudio.com/project/_git/_full/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse ssh format with _optimized', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'git@ssh.dev.azure.com:v3/organization/project/_optimized/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse ssh format with _full', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'git@ssh.dev.azure.com:v3/organization/project/_full/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse legacy ssh format', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'git@organization.visualstudio.com:v3/organization/project/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse legacy ssh format with _optimized', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'git@organization.visualstudio.com:v3/organization/project/_optimized/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
	...
	test('should parse legacy ssh format with _full', () => {
		assert.deepStrictEqual(
			getAdoRepoIdFromFetchUrl(
				'git@organization.visualstudio.com:v3/organization/project/_full/repository',
			),
			new AdoRepoId('organization', 'project', 'repository'),
		);
	});
