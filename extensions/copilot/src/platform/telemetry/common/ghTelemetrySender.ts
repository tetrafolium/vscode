...
import {
	TelemetryData,
	eventPropertiesToSimpleObject,
} from '../common/telemetryData';
...
export class BaseGHTelemetrySender implements ITelemetrySender {
	...
	constructor(
		...
		protected readonly _createTelemetryLogger: (
			enhanced: boolean,
		) => TelemetryLogger,
		...
		this._disposables.add(
			this._domainService.onDidChangeDomains(() => {
				this._standardTelemetryLogger.dispose();
				this._standardTelemetryLogger =
					this._createTelemetryLogger(false);
				if (this._enhancedTelemetryLogger) {
					this._enhancedTelemetryLogger.dispose();
					this._enhancedTelemetryLogger =
						this._createTelemetryLogger(true);
				}
			}),
		);
	}
	...
	sendTelemetryEvent(
		...
		this._standardTelemetryLogger.logUsage(
			eventName,
			this.markAsIssuedAndMakeReadyForSending(properties, measurements),
		);
	}
	sendTelemetryErrorEvent(
		...
		this._standardTelemetryLogger.logError(
			eventName,
			this.markAsIssuedAndMakeReadyForSending(properties, measurements),
		);
	}
	...
	sendEnhancedTelemetryEvent(
		...
		if (this._enhancedTelemetryLogger) {
			this._enhancedTelemetryLogger?.logUsage(
				eventName,
				this.markAsIssuedAndMakeReadyForSending(
					properties,
					measurements,
				),
			);
	...
	sendEnhancedTelemetryErrorEvent(
		eventName: string,
		properties?: TelemetryEventProperties,
		measurements?: TelemetryEventMeasurements,
	): void {
		if (this._enhancedTelemetryLogger) {
			this._enhancedTelemetryLogger?.logError(
				eventName,
				this.markAsIssuedAndMakeReadyForSending(
					properties,
					measurements,
	...
	sendExceptionTelemetry(maybeError: unknown, origin: string) {
		...
		const definedTelemetryDataStub = TelemetryData.createAndMarkAsIssued({
			...
			reason: this._enhancedTelemetryLogger
		...
		this.sendTelemetryEvent(
			'exception',
			definedTelemetryDataStub.properties,
			definedTelemetryDataStub.measurements,
		);
	...
	private markAsIssuedAndMakeReadyForSending(
		properties?: TelemetryEventProperties,
		measurements?: TelemetryEventMeasurements,
	): {
		properties: TelemetryEventProperties;
		...
		telemetryData.makeReadyForSending(
			this._configService,
			this._envService,
			this._telemetryConfig,
		);
		...
		for (const key in telemetryData.properties) {
			newPropeties[key] = new TelemetryTrustedValue(
				telemetryData.properties[key],
			);
		}
