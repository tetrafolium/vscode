...
export class TelemetryData {
	...
	extendWithEditorAgnosticFields(envService: IEnvService): void {
		...
		this.properties['editor_plugin_version'] = envService
			.getEditorPluginInfo()
			.format();
		...
		this.properties['copilot_version'] =
			`copilot/${envService.getVersion()}`;

		this.properties['common_extname'] =
			envService.getEditorPluginInfo().name;
		this.properties['common_extversion'] =
			envService.getEditorPluginInfo().version;
		this.properties['common_vscodeversion'] = envService
			.getEditorInfo()
			.format();
	}
	...
	extendWithConfigProperties(
		...
		if (telemetryConfig.organizationsList) {
			configProperties['organizations_list'] =
				telemetryConfig.organizationsList;
		}
		if (telemetryConfig.enterpriseList) {
			configProperties['enterprise_list'] =
				telemetryConfig.enterpriseList;
		}
	 ...
	 */
	static maybeRemoveRepoInfoFromPropertiesHack(
		secure: boolean,
		map: { [key: string]: any },
	): { [key: string]: any } {
		if (secure) {
	...
	updateTimeSinceIssuedAndDisplayed(): void {
		...
		if (
			this.measurements.timeSinceDisplayedMs === undefined &&
	...
	makeReadyForSending(
		...
		this.extendWithConfigProperties(
			configService,
			envService,
			telemetryConfig,
		);
...
export function eventPropertiesToSimpleObject(
