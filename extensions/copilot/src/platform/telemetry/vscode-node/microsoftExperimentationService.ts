...
export enum RelatedExtensionsFilter {
	...
	CopilotRelatedPluginVersionCopilot = CopilotRelatedPluginVersionPrefix +
		'githubcopilot',
	CopilotRelatedPluginVersionCopilotChat = CopilotRelatedPluginVersionPrefix +
		'githubcopilotchat',
}
...
class RelatedExtensionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		this._logService.trace(
			`[RelatedExtensionsFilterProvider]::getFilters looking up related extensions`,
		);
		...
		for (const extension of this._getRelatedExtensions()) {
			const filterName =
				...
				extension.name.replace(/[^A-Za-z]/g, '').toLowerCase();
			if (
				!Object.values<string>(RelatedExtensionsFilter).includes(
					filterName,
				)
			) {
				this._logService.warn(
					`[RelatedExtensionsFilterProvider]::getFilters A filter could not be registered for the unrecognized related plugin "${extension.name}".`,
				);
				continue;
			}
		...
		this._logService.trace(
			`[RelatedExtensionsFilterProvider]::getFilters Filters: ${JSON.stringify(Array.from(filters.entries()))}`,
		);
...
class CopilotExtensionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		const copilotExtensionversion =
			vscode.extensions.getExtension('github.copilot')?.packageJSON
				.version;
		const copilotChatExtensionVersion = packageJson.version;
		const completionsCoreVersion = packageJson.completionsCoreVersion;

		this._logService.trace(
			`[CopilotExtensionsFilterProvider]::getFilters Copilot Extension Version: ${copilotExtensionversion}, Copilot Chat Extension Version: ${copilotChatExtensionVersion}, Completions Core Version: ${completionsCoreVersion}`,
		);
		const filters = new Map<string, string>();
		filters.set(
			RelatedExtensionsFilter.CopilotRelatedPluginVersionCopilot,
			copilotExtensionversion,
		);
		filters.set(
			RelatedExtensionsFilter.CopilotRelatedPluginVersionCopilotChat,
			copilotChatExtensionVersion,
		);
		filters.set(
			'X-VSCode-CompletionsInChatExtensionVersion',
...
class CopilotCompletionsFilterProvider implements IExperimentationFilterProvider {
	constructor(
		private _getCompletionsFilters: () => Map<string, string>,
	...
	getFilters(): Map<string, string> {
		...
		this._logService.trace(
			`[CopilotCompletionsFilterProvider]::getFilters Filters: ${JSON.stringify(Array.from(filters.entries()))}`,
		);
		return filters;
	}
}
...
class GithubAccountFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string | undefined> {
		...
		this._logService.trace(
			`[GithubAccountFilterProvider]::getFilters SKU: ${this._userInfoStore.sku}, Internal Org: ${this._userInfoStore.internalOrg}, IsFcv1: ${this._userInfoStore.isFcv1}, IsSn: ${this._userInfoStore.isSn}, IsVscodeTeamMember: ${this._userInfoStore.isVscodeTeamMember}, OrganizationList: ${orgListString}`,
		);
		const filters = new Map<string, string | undefined>();
		filters.set('X-GitHub-Copilot-SKU', this._userInfoStore.sku);
		filters.set(
			'X-Microsoft-Internal-Org',
			this._userInfoStore.internalOrg,
		);
		filters.set(
			'X-GitHub-Copilot-IsFcv1',
			this._userInfoStore.isFcv1 ? '1' : '0',
		);
		filters.set(
			'X-GitHub-Copilot-IsSn',
			this._userInfoStore.isSn ? '1' : '0',
		);
		filters.set(
			'X-GitHub-Copilot-IsVscodeTeamMember',
			this._userInfoStore.isVscodeTeamMember ? '1' : '0',
		);
...
class PlatformAndReleaseDateFilterProvider implements IExperimentationFilterProvider {
	...
	private _initReleaseDate(): string | undefined {
		try {
			...
			this._logService.warn(
				`[PlatformAndReleaseDateFilterProvider]::_initReleaseDate Failed to read product.json for release date: ${error}`,
			);
			return undefined;
		}
	}
	...
	getFilters(): Map<string, string> {
		...
		this._logService.trace(
			`[PlatformAndReleaseDateFilterProvider]::getFilters Filters: ${JSON.stringify(Array.from(filters.entries()))}`,
		);
		return filters;
	}
}
...
export class MicrosoftExperimentationService extends BaseExperimentationService {
	constructor(
		...
		const delegateFn = (
			...
			const wrappedMemento = new ExpMementoWrapper(
				globalState,
				envService,
			);
			return getExperimentationService(
				...
				new CopilotCompletionsFilterProvider(
					() => self?.getCompletionsFilters() ?? new Map(),
					logService,
				),
...
class ExpMementoWrapper implements vscode.Memento {
	...
	get<T>(key: string, defaultValue?: T): T | undefined {
		...
		const age =
			new Date().getTime() - new Date(value.savedDateTime).getTime();
