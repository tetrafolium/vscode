...
export class SpyingTelemetryService implements ITelemetryService {
	...
	sendInternalMSFTTelemetryEvent(
		...
		this.addEvent(
			TelemetryServiceEventType.internal,
			eventName,
			properties,
			measurements,
		);
	...
	sendMSFTTelemetryEvent(
		...
		this.addEvent(
			TelemetryServiceEventType.default,
			eventName,
			properties,
			measurements,
		);
	...
	sendMSFTTelemetryErrorEvent(
		...
		this.addEvent(
			TelemetryServiceEventType.error,
			eventName,
			properties,
			measurements,
		);
	...
	sendGHTelemetryException(maybeError: unknown, origin: string): void {
		this.addEvent(TelemetryServiceEventType.error, 'exception', {
			origin,
	...
	public installSpyingSenders(ghTelemetry: IGHTelemetryService) {
		...
		const getSender = (eventType: TelemetrySenderEventType) => ({
			...
			sendErrorData(
				...
				senderEvents.push({
					eventType,
					eventName: error.message || error.toString(),
					data,
				});
			},
		});
		ghTelemetry.setReporter(getSender(TelemetrySenderEventType.insecure));
		ghTelemetry.setSecureReporter(
			getSender(TelemetrySenderEventType.secure),
		);
	}

	public getEvents(): {
		telemetryServiceEvents: ITelemetryEvent[];
		telemetrySenderEvents: any[];
	} {
		return {
	...
	public getFilteredEvents<
		TEventNames extends Partial<
			Record<
				TelemetryEventMap[keyof TelemetryEventMap]['eventName'],
				true
			>
		>,
	>(
		eventNames: TEventNames,
	): TelemetryEventMap[TelemetryEventMapKeysFilteredByEventName<
		keyof TEventNames
	>][] {
		const set = new Set(Object.keys(eventNames));
		return this.telemetryServiceEvents.filter((e) =>
			set.has(e.eventName as any),
		) as any;
	}
}

type TelemetryEventMapKeysFilteredByEventName<TEventName> = {
	[TKey in keyof TelemetryEventMap]: TelemetryEventMap[TKey]['eventName'] extends TEventName
		? TKey
		: never;
