...
import {
	addPullRequestCommentGraphQLRequest,
	AssignableActor,
	closePullRequest,
	getPullRequestFromGlobalId,
	makeGitHubAPIRequest,
	makeSearchGraphQLRequest,
	PullRequestComment,
...
export type IGetRepositoryInfoResponseData =
	Endpoints['GET /repos/{owner}/{repo}']['response']['data'];

export const IGithubRepositoryService =
	createServiceIdentifier<IGithubRepositoryService>(
		'IGithubRepositoryService',
	);
export const IOctoKitService =
	createServiceIdentifier<IOctoKitService>('IOctoKitService');
...
export interface IGithubRepositoryService {
	...
	getRepositoryInfo(
		owner: string,
		repo: string,
	): Promise<IGetRepositoryInfoResponseData>;
	getRepositoryItems(
		org: string,
		repo: string,
		path: string,
...
export interface IOctoKitService {
	 ...
	 */
	getRecentlyCommittedRepositories(
		authOptions: AuthOptions,
	): Promise<{ owner: string; name: string }[]>;
...
export class BaseOctoKitService {
	...
	private _cachedOutageStatus:
		| { value: GitHubOutageStatus; timestamp: number }
		| undefined;

	private static readonly _userReposScopeCacheTTL = 5 * 60 * 1000; // 5 minutes
	private _cachedUserReposScope:
		| { token: string; qualifiers: string; timestamp: number }
		| undefined;
	...
	async getCurrentAuthedUserWithToken(
		token: string,
	): Promise<IOctoKitUser | undefined> {
		return this._makeGHAPIRequest(
			'user',
			'GET',
			token,
			undefined,
	...
	async getGitHubOutageStatus(): Promise<GitHubOutageStatus> {
		const now = Date.now();
		if (
			this._cachedOutageStatus &&
			now - this._cachedOutageStatus.timestamp <
				BaseOctoKitService._outageStatusCacheTTL
		) {
		...
		try {
			...
			const response = await this._fetcherService.fetch(
				...
				{ method: 'GET', callSite: 'github-status' },
			);
	...
	protected async _makeGHAPIRequest(
		routeSlug: string,
		method: 'GET' | 'POST',
		token: string,
		body?: { [key: string]: any },
	...
	protected async getOpenPullRequestForUserWithToken(
		owner: string,
		repo: string,
		user: string,
		token: string,
		...
		return makeSearchGraphQLRequest(
			this._fetcherService,
			this._logService,
			this._telemetryService,
			this._capiClientService.dotcomAPIURL,
	...
	protected async findPullRequestByHeadBranchWithToken(
		owner: string,
		repo: string,
		headBranch: string,
		token: string,
		...
		const results = await makeSearchGraphQLRequest(
			this._fetcherService,
			this._logService,
			this._telemetryService,
			this._capiClientService.dotcomAPIURL,
	...
	protected async addPullRequestCommentWithToken(
		pullRequestId: string,
		commentBody: string,
		token: string,
	): Promise<PullRequestComment | null> {
		return addPullRequestCommentGraphQLRequest(
			this._fetcherService,
			this._logService,
			this._telemetryService,
			this._capiClientService.dotcomAPIURL,
	...
	protected async createPullRequestWithToken(
		owner: string,
		repo: string,
		title: string,
		body: string,
		...
		if (!response?.html_url || typeof response.number !== 'number') {
			throw new Error(
				`Failed to create pull request for ${owner}/${repo}`,
			);
		}
	...
	protected async getPullRequestFromSessionWithToken(
		globalId: string,
		token: string,
	): Promise<PullRequestSearchItem | null> {
		return getPullRequestFromGlobalId(
			this._fetcherService,
			this._logService,
			this._telemetryService,
			this._capiClientService.dotcomAPIURL,
	...
	protected async getPullRequestFilesWithToken(
		owner: string,
		repo: string,
		pullNumber: number,
		token: string,
		...
		const result = await makeGitHubAPIRequest(
			...
			'GET',
			token,
			{ version: '2022-11-28', callSite: 'github-rest-get-pr-files' },
		);
		return result || [];
	}

	protected async closePullRequestWithToken(
		owner: string,
		repo: string,
		pullNumber: number,
		token: string,
	...
	protected async getFileContentWithToken(
		owner: string,
		repo: string,
		ref: string,
		path: string,
		...
		const response = await makeGitHubAPIRequest(
			...
			'GET',
			token,
			{ callSite: 'github-rest-get-file-content' },
		);
		...
		if (typedResponse.content && typedResponse.encoding === 'base64') {
			return decodeBase64(
				typedResponse.content.replace(/\n/g, ''),
			).toString();
		}

		if (typedResponse.sha) {
			const blob = await this.getBlobContentWithToken(
				owner,
				repo,
				typedResponse.sha,
				token,
		...
		this._logService.error(
			`Failed to get file content for ${owner}/${repo}/${path} at ref ${ref}`,
		);
		return '';
	}

	protected async getUserOrganizationsWithToken(
		token: string,
		pageSize: number = 100,
	): Promise<string[]> {
		const result = await this._makeGHAPIRequest(
			`user/orgs?per_page=${pageSize}`,
			'GET',
			token,
			undefined,
			undefined,
			'github-rest-get-user-orgs',
		);
	...
	protected async isUserMemberOfOrgWithToken(
		org: string,
		token: string,
	): Promise<boolean> {
		try {
			...
			const result = await this._makeGHAPIRequest(
				...
				undefined,
				undefined,
				'github-rest-check-org-membership',
			);
	...
	protected async getOrganizationRepositoriesWithToken(
		org: string,
		token: string,
		pageSize: number = 100,
	): Promise<string[]> {
		const result = await this._makeGHAPIRequest(
			`orgs/${org}/repos?per_page=${pageSize}&sort=updated`,
			'GET',
			token,
			undefined,
			{ silent404: true },
			'github-rest-get-org-repos',
		);
	...
	protected async getUserRepositoriesWithToken(
		token: string,
		query?: string,
	): Promise<{ owner: string; name: string }[]> {
	...
	private async searchUserRepositoriesWithToken(
		token: string,
		query: string,
	): Promise<{ owner: string; name: string }[]> {
	...
	private async _getUserReposSearchScope(
		token: string,
	): Promise<string | undefined> {
		const now = Date.now();
		if (
			this._cachedUserReposScope &&
			this._cachedUserReposScope.token === token &&
			now - this._cachedUserReposScope.timestamp <
				BaseOctoKitService._userReposScopeCacheTTL
		) {
			return this._cachedUserReposScope.qualifiers;
		}

		const [user, orgs] = await Promise.all([
			this._makeGHAPIRequest(
				'user',
				'GET',
				token,
				undefined,
	...
	protected async getRecentlyCommittedReposWithToken(
		token: string,
	): Promise<{ owner: string; name: string }[]> {
		// First, get the authenticated user's login
		const user = await this._makeGHAPIRequest(
			'user',
			'GET',
			token,
			undefined,
	...
	private async getBlobContentWithToken(
		owner: string,
		repo: string,
		sha: string,
		token: string,
		...
		const blobResponse = await makeGitHubAPIRequest(
			this._fetcherService,
			this._logService,
			this._telemetryService,
			this._capiClientService.dotcomAPIURL,
			...
			'GET',
			token,
			{ version: '2022-11-28', callSite: 'github-rest-get-blob' },
		);
