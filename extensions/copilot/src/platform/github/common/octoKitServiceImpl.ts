...
export class OctoKitService
	extends BaseOctoKitService
	implements IOctoKitService
{
	...
	private _getPermissiveSession(
		authOptions: AuthOptions,
	): Promise<AuthenticationSession | undefined> {
		if (authOptions.createIfNone) {
			return this._authService.getGitHubSession('permissive', {
	...
	async getCurrentAuthedUser(): Promise<IOctoKitUser | undefined> {
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getCurrentAuthedUser',
			);
			return undefined;
		}
	...
	async getOpenPullRequestsForUser(
		owner: string,
		repo: string,
		authOptions: AuthOptions,
		...
		if (!auth?.accessToken) {
			this._logService.trace(
				'No authentication token available for getOpenPullRequestsForUser',
			);
			return [];
		}
	...
	async getCopilotSessionsForPR(
		prId: string,
		authOptions: AuthOptions,
	): Promise<SessionInfo[]> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))
				?.accessToken;
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getCopilotSessionsForPR',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
				await this._capiClientService.makeRequest<Response>(
					{
						method: 'GET',
						headers: {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch copilot sessions for PR ${prId}: ${response.statusText}`,
				);
			}
	...
	async getSessionLogs(
		...
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getSessionLogs',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch session logs for session ${sessionId}: ${response.statusText}`,
				);
			}
	...
	async getSessionInfo(
		...
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getSessionInfo',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch session info for session ${sessionId}: ${response.statusText}`,
				);
			}
	...
	async postCopilotAgentJob(
		...
		RemoteAgentJobResponse | ErrorResponseWithStatusCode | undefined
	> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))
				?.accessToken;
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for postCopilotAgentJob',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
	...
	async getJobByJobId(
		...
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getJobByJobId',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch job info for job ${jobId}: ${response.statusText}`,
				);
			}
	...
	async getJobBySessionId(
		...
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getJobBySessionId',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch job info for session ${sessionId}: ${response.statusText}`,
				);
			}
			return (await response.json()) as JobInfo;
		} catch (e) {
	...
	async addPullRequestComment(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for addPullRequestComment',
			);
			throw new PermissiveAuthRequiredError();
		}
		return this.addPullRequestCommentWithToken(
			pullRequestId,
			commentBody,
			authToken,
		);
	...
	async createPullRequest(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for createPullRequest',
			);
			throw new PermissiveAuthRequiredError();
		}
		return this.createPullRequestWithToken(
			owner,
			repo,
			title,
			body,
		...
		try {
			...
			if (!authToken) {
				this._logService.debug(
					`[getAllSessions] No authentication token available (nwo=${nwo})`,
				);
				throw new PermissiveAuthRequiredError();
			}
			this._logService.debug(
				`[getAllSessions] Fetching sessions for nwo=${nwo}, open=${open}`,
			);
			const result = await this._capiClientService.makeRequest<
				SessionInfo[]
			>(
				{
					method: 'GET',
				...
				},
				{
					...
					nwo,
					resourceState: open ? 'draft,open' : undefined,
				},
			);
			this._logService.debug(
				`[getAllSessions] Got ${Array.isArray(result) ? result.length : 'non-array'} sessions for nwo=${nwo}`,
			);
			return result;
		} catch (e) {
			if (e instanceof PermissiveAuthRequiredError) {
				this._logService.trace(
					`[getAllSessions] No permissive GitHub session (nwo=${nwo})`,
				);
			} else if (e instanceof Error) {
				this._logService.error(e, 'Error in getAllSessions');
				this._logService.debug(
					`[getAllSessions] Error for nwo=${nwo}: ${e.message}`,
				);
			} else {
				this._logService.error('Non-Error thrown in getAllSessions');
				this._logService.debug(
					`[getAllSessions] Non-Error thrown for nwo=${nwo}: ${String(e)}`,
				);
			}
			return [];
		}
	...
	async getPullRequestFromGlobalId(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getPullRequestFromGlobalId',
			);
			throw new PermissiveAuthRequiredError();
		}
	...
	async getCustomAgents(
		owner: string,
		repo: string,
		options: CustomAgentListOptions,
		authOptions: AuthOptions,
		...
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getCustomAgents',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch custom agents for ${owner} ${repo}: ${response.statusText}`,
				);
			}
			const data = (await response.json()) as {
			...
			if (e instanceof PermissiveAuthRequiredError) {
				this._logService.trace(
					'[getCustomAgents] No permissive GitHub session',
				);
			} else {
				this._logService.error(e);
			}
	...
	async getCustomAgentDetails(
		...
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getCustomAgentDetails',
				);
				throw new PermissiveAuthRequiredError();
			}
			...
			if (!response.ok) {
				if (response.status === 404) {
					this._logService.trace(
						`Custom agent '${agentName}' not found for ${owner}/${repo}`,
					);
					return undefined;
				}
				throw new Error(
					`Failed to fetch custom agent details for ${agentName}: ${response.statusText}`,
				);
			}
	...
	async getPullRequestFiles(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getPullRequestFiles',
			);
			return [];
		}
		return this.getPullRequestFilesWithToken(
			owner,
			repo,
			pullNumber,
			authToken,
	...
	async closePullRequest(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for closePullRequest',
			);
			return false;
		}
		return this.closePullRequestWithToken(
			owner,
			repo,
			pullNumber,
			authToken,
	...
	async findPullRequestByHeadBranch(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for findPullRequestByHeadBranch',
			);
			return undefined;
		}
		return this.findPullRequestByHeadBranchWithToken(
			owner,
			repo,
			headBranch,
			authToken,
	...
	async getFileContent(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getFileContent',
			);
			throw new PermissiveAuthRequiredError();
		}
	...
	async getUserOrganizations(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getUserOrganizations',
			);
			throw new PermissiveAuthRequiredError();
		}
	...
	async isUserMemberOfOrg(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for isUserMemberOfOrg',
			);
			return false;
		}
	...
	async getOrganizationRepositories(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getOrganizationRepositories',
			);
			throw new PermissiveAuthRequiredError();
		}
		return this.getOrganizationRepositoriesWithToken(
			org,
			authToken,
			pageSize,
		);
	...
	async getOrgCustomInstructions(
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					`Failed to fetch custom instructions for org ${orgLogin}: ${response.statusText}`,
				);
			}
	...
	async getUserRepositories(
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getUserRepositories',
			);
			throw new PermissiveAuthRequiredError();
		}
	...
	async getRecentlyCommittedRepositories(
		authOptions: AuthOptions,
	): Promise<{ owner: string; name: string }[]> {
		...
		if (!authToken) {
			this._logService.trace(
				'No authentication token available for getRecentlyCommittedRepositories',
			);
			throw new PermissiveAuthRequiredError();
		}
	...
	async getCopilotAgentModels(authOptions: AuthOptions): Promise<CCAModel[]> {
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for getCopilotAgentModels',
				);
				throw new PermissiveAuthRequiredError();
			}
			const response =
			...
			if (!response.ok) {
				this._logService.trace(
					`Failed to fetch Copilot agent models: ${response.statusText}`,
				);
				return [];
			}
			...
			if (e instanceof PermissiveAuthRequiredError) {
				this._logService.trace(
					'[getCopilotAgentModels] No permissive GitHub session',
				);
			} else {
				this._logService.error(e);
			}
	...
	async getAssignableActors(
		...
		if (!auth?.accessToken) {
			this._logService.trace(
				'No authentication token available for getAssignableActors',
			);
			throw new PermissiveAuthRequiredError();
		}
	...
	async isCCAEnabled(
		...
		try {
			...
			if (!authToken) {
				this._logService.trace(
					'No authentication token available for isCCAEnabled',
				);
			...
			switch (response.status) {
				...
				default:
					this._logService.trace(
						`Unexpected status code for isCCAEnabled: ${response.status}`,
					);
