...
class RequestRateLimiter extends Disposable {
	...
	constructor(
		...
		this._maxParallelChunksRequests =
			experimentationService.getTreatmentVariable<number>(
				'workspace.embeddingIndex.maxParallelChunksRequests',
			) ?? 8;
	}
		...
		try {
			...
			while (!this.requestQueue.isEmpty()) {
				...
				if (elapsedSinceLastSend < RequestRateLimiter._abuseLimit) {
					await timeout(
						RequestRateLimiter._abuseLimit - elapsedSinceLastSend,
					);
				}
				...
				if (this._latestRateLimitHint) {
					...
					if (currentTime < this._latestRateLimitHint.resetAt) {
						if (
							this._latestRateLimitHint.remaining -
								this._numberInFlightRequests <=
							0
						) {
							// There are no remaining requests, wait until reset
							const resetTimeSpan =
								this._latestRateLimitHint.resetAt - currentTime;
				...
				if (
					...
					this._latestQuotaUsed.quota > this.targetQuota
				) {
					const currentTime = Date.now();
					const quotaDelta =
						this._latestQuotaUsed.quota - this.targetQuota;
					const quotaDeltaTime =
						currentTime - this._latestQuotaUsed.timestamp;

					const decayTime = 2500; // Estimated time for quota to reset
					const maxDelay = 1000;

					let quotaAdjustment = quotaDelta / (100 - this.targetQuota);
					quotaAdjustment *= Math.max(
						1.0 - quotaDeltaTime / decayTime,
						0,
					); // Adjust by time passed
					...
					.then((response) => {
						...
						if (e.attempt < this._maxAttempts) {
							if (
								...
								const retryAfter_seconds =
									this.getRequestRetryDelay(response);
								if (retryAfter_seconds > 0) {
									this._rateLimitTimeout = timeout(
										retryAfter_seconds * 1000,
									);
								}
	...
	private updateQuotasFromResponse(response: Response) {
		...
		try {
			...
			const totalQuotaUsed = response.headers.get(
				'x-github-total-quota-used',
			);
			if (totalQuotaUsed) {
...
export class ChunkingEndpointClientImpl
	extends Disposable
	implements IChunkingEndpointClient
{
	...
	constructor(
		...
		this._requestLimiter = this._register(
			this._instantiationService.createInstance(RequestRateLimiter),
		);
	}

	public computeChunks(
		authToken: string,
		...
		return this.doComputeChunksAndEmbeddings(
			authToken,
			embeddingType,
			content,
			batchInfo,
			{ qos, computeEmbeddings: false },
			cache,
			telemetryInfo,
			token,
		);
	...
	public async computeChunksAndEmbeddings(
		...
		const result = await this.doComputeChunksAndEmbeddings(
			authToken,
			embeddingType,
			content,
			batchInfo,
			{ qos, computeEmbeddings: true },
			cache,
			telemetryInfo,
			token,
		);
	...
	private async doComputeChunksAndEmbeddings(
		...
		try {
			const makeRequest = async (attempt: number) => {
				return logExecTime(
					this._logService,
					`ChunksEndpointEmbeddingComputer.fetchChunksRequest(${content.uri}, attempt=${attempt})`,
					() =>
			...
			const response = await raceCancellationError(
				this._requestLimiter.enqueue(makeRequest, token),
				token,
			);
			if (!response.ok) {
				this._logService.debug(
					`Error chunking '${content.uri}'. Status: ${response.status}. Status Text: ${response.statusText}.`,
				);
			...
			return coalesce(
				body.chunks.map(
					(chunk): FileChunkWithOptionalEmbedding | undefined => {
						...
						if (chunk.embedding?.embedding) {
							const returnedEmbeddingsType = new EmbeddingType(
								body.embedding_model,
							);
							if (!returnedEmbeddingsType.equals(embeddingType)) {
								throw new Error(
									`Unexpected embedding model. Got: ${returnedEmbeddingsType}. Expected: ${embeddingType}`,
								);
							}
