...
suite('Path redaction', () => {
	...
	test('leaves urls intact', function () {
		...
		assert.deepStrictEqual(
			output,
			'foo http://github.com/github/copilot bar',
		);
	});
	...
	test('path in parenthesis', function () {
		assertRedacted(
			'See details (/Users/copilot)',
			'See details ([redacted]',
		);
	});
	...
	test('filter windows path', function () {
		...
		assertRedacted(
			'foo Z:\\projects/MainTest.kt bar',
			'foo [redacted] bar',
		);
	});
	...
	test('filter unc path', function () {
		assertRedacted(
			'foo \\server-name\\shared-resource-pathname bar',
			'foo [redacted] bar',
		);
		assertRedacted(
			'foo file://\\server-name\\shared-resource-pathname bar',
			'foo file://[redacted] bar',
		);
	});
	...
	test('file urls', function () {
		...
		assertRedacted(
			'Invalid file file:///Users/copilot bar',
			'Invalid file file://[redacted] bar',
		);
	});
