...
suite('Anomaly Repetition Tests', function () {
	test('recognizes sequence consisting of single repeated token', function () {
		const tokens =
			'Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar'.split(
				' ',
			);
		...
		assert.strictEqual(
			repetitive,
			true,
			'Repetition should be recognized.',
		);
	});
	...
	test('does nothing on a too short sequence of single repeated token', function () {
		...
		assert.strictEqual(
			repetitive,
			false,
			'Repetition should not be recognized.',
		);
	});

	test('recognizes single repeated token in proper suffix', function () {
		const tokens =
			'Baz Baz Baz Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar Bar'.split(
				' ',
			);
		const repetitive = isRepetitive(tokens);
		assert.strictEqual(
	...
	test('does nothing in absence of a pattern', function () {
		...
		assert.strictEqual(
			repetitive,
			false,
			'No repetition should be claimed.',
		);
	});
