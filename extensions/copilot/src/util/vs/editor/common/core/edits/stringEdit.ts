...
> extends BaseEdit<T, TEdit> {
	...
	public static composeOrUndefined<T extends BaseStringEdit>(
		edits: readonly T[],
	): T | undefined {
		if (edits.length === 0) {
			return undefined;
	 ...
	 */
	public static trySwap(
		e1: BaseStringEdit,
		e2: BaseStringEdit,
	): { e1: StringEdit; e2: StringEdit } | undefined {
		// TODO make this more efficient
		const e1Inv = e1.inverseOnSlice((start, endEx) =>
			' '.repeat(endEx - start),
		);
	 ...
	 */
	public inverseOnSlice(
		getOriginalSlice: (start: number, endEx: number) => string,
	): StringEdit {
	...
	public inverse(original: string): StringEdit {
		return this.inverseOnSlice((start, endEx) =>
			original.substring(start, endEx),
		);
	}
	...
	private _tryRebase(
		...
		while (
			ourIdx < this.replacements.length ||
			...
			if (!ourEdit) {
				...
				newEdits.push(
					new StringReplacement(transformedRange, ourEdit.newText),
				);
				ourIdx++;
			} else if (
				ourEdit.replaceRange.intersects(baseEdit.replaceRange) ||
				areConcurrentInserts(
	...
	public removeCommonSuffixAndPrefix(source: string): TEdit {
		return this._createNew(
			this.replacements.map((e) => e.removeCommonSuffixAndPrefix(source)),
		).normalize();
	}
	...
	public mapData<TData extends IEditData<TData>>(
		f: (replacement: T) => TData,
	): AnnotatedStringEdit<TData> {
...
export class StringEdit extends BaseStringEdit<StringReplacement, StringEdit> {
	 ...
	 */
	public static composeSequentialReplacements(
		replacements: readonly StringReplacement[],
	): StringEdit {
		...
		for (const r of replacements) {
			...
			if (!last || r.replaceRange.isBefore(last.replaceRange)) {
				...
				edit = edit.compose(
					StringEdit.create(curEditReplacements.reverse()),
				);
...
export class StringReplacement extends BaseStringReplacement<StringReplacement> {
	...
	public static fromJson(
		data: ISerializedStringReplacement,
	): StringReplacement {
	...
	override tryJoinTouching(
		other: StringReplacement,
	...
	override slice(
		...
		return new StringReplacement(
			range,
			rangeInReplacement
				? rangeInReplacement.substring(this.newText)
				: this.newText,
		);
	}
}
...
> {
	...
	public static single<T extends IEditData<T>>(
		replacement: AnnotatedStringReplacement<T>,
	): AnnotatedStringEdit<T> {
	...
	public static insert<T extends IEditData<T>>(
		offset: number,
		replacement: string,
		data: T,
	): AnnotatedStringEdit<T> {
	...
	public static delete<T extends IEditData<T>>(
		range: OffsetRange,
		data: T,
	): AnnotatedStringEdit<T> {
		return new AnnotatedStringEdit([
	...
	public static compose<T extends IEditData<T>>(
		edits: readonly AnnotatedStringEdit<T>[],
	): AnnotatedStringEdit<T> {
		if (edits.length === 0) {
	...
	public toStringEdit(
		filter?: (replacement: AnnotatedStringReplacement<T>) => boolean,
	): StringEdit {
		const newReplacements: StringReplacement[] = [];
		for (const r of this.replacements) {
			if (!filter || filter(r)) {
				newReplacements.push(
					new StringReplacement(r.replaceRange, r.newText),
				);
			}
		}
...
export class AnnotatedStringReplacement<
	T extends IEditData<T>,
> extends BaseStringReplacement<AnnotatedStringReplacement<T>> {
	public static insert<T extends IEditData<T>>(
		offset: number,
		text: string,
		data: T,
	...
	tryJoinTouching(
		other: AnnotatedStringReplacement<T>,
	): AnnotatedStringReplacement<T> | undefined {
 ...
 */
function isInsertStrictlyInsideRange(
	insert: OffsetRange,
	range: OffsetRange,
): boolean {
	return (
		insert.isEmpty &&
		range.start < insert.start &&
		insert.start < range.endExclusive
	);
}
