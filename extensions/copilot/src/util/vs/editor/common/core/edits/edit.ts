...
> {
	...
	public normalize(): TEdit {
		...
		for (const r of this.replacements) {
			...
			if (
				lastReplacement &&
				lastReplacement.replaceRange.endExclusive ===
	...
	public compose(other: TEdit): TEdit {
		...
		for (const r2 of edits2.replacements) {
			...
			while (true) {
				...
				if (
					!r1 ||
					r1.replaceRange.start + edit1ToEdit2 >
	...
	public decomposeSplit(shouldBeInE1: (repl: T) => boolean): {
		e1: TEdit;
		e2: TEdit;
	} {
		const e1: T[] = [];
		...
		for (const edit of this.replacements) {
			if (shouldBeInE1(edit)) {
				...
				e2.push(
					edit.slice(
						edit.replaceRange.delta(e2delta),
						new OffsetRange(0, edit.getNewLength()),
					),
				);
			}
		}
	 ...
	 */
	public applyToOffsetOrUndefined(
		...
		for (const edit of this.replacements) {
			if (edit.replaceRange.start <= originalOffset) {
				...
				accumulatedDelta +=
					edit.getNewLength() - edit.replaceRange.length;
			} else {
				break;
			}
		}
...
export class Edit<T extends BaseReplacement<T>> extends BaseEdit<T, Edit<T>> {
	...
	public static single<T extends BaseReplacement<T>>(
		replacement: T,
	): Edit<T> {
