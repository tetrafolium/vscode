...
export class DevToolsLogger implements IObservableLogger {
	...
	private readonly _declarations = new Map<
		/* declarationId + type */ string,
		IObsDeclaration
	>();
	private readonly _instanceInfos = new WeakMap<
		object,
		IObservableInfo | IAutorunInfo
	>();
	private readonly _aliveInstances = new Map<
		ObsInstanceId,
		IObservable<any> | AutorunObserver
	>();
	private readonly _activeTransactions = new Set<TransactionImpl>();

	private readonly _channel = registerDebugChannel<ObsDebuggerApi>(
		'observableDevTools',
		() => {
			return {
				notifications: {
	...
	private _getInfo(
		observer: IObserver,
		queue: (observer: IObserver) => void,
	): ObserverInstanceState | undefined {
	...
	private _formatObservable(
		obs: IObservable<any>,
	): { name: string; instanceId: ObsInstanceId } | undefined {
	...
	handleObservableCreated(
		...
		const declarationId = this._getDeclarationId(
			'observable/value',
			location,
		);
