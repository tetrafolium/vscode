...
class LeakageMonitor {
	...
	check(stack: Stacktrace, listenerCount: number): undefined | (() => void) {
		...
		if (this._warnCountdown <= 0) {
			...
			const emitterName = /^[0-9a-f]+$/i.test(this.name)
				? undefined
				: this.name;
			...
			const kind =
				topCount / listenerCount > 0.3 ? 'dominated' : 'popular';
			const error = new ListenerLeakError(
				kind,
				message,
				topStack,
				listenerCount,
	...
	static is(err: unknown): err is ListenerLeakError {
		return (
			...
			(err instanceof Error &&
				typeof (
					err as Error & { kind: unknown; listenerCount: unknown }
				).kind === 'string' &&
				typeof (
					err as Error & { kind: unknown; listenerCount: unknown }
				).listenerCount === 'number')
	...
	get event(): Event<T> {
		this._event ??= (
			...
			if (
				...
				const kind =
					tuple[1] / this._size > 0.3 ? 'dominated' : 'popular';
...
function addToDisposables(
	result: IDisposable,
	disposables: DisposableStore | IDisposable[] | undefined,
) {
