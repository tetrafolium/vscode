...
describe('Notebook Common', () => {
	...
	it('Finds a notebook', async () => {
		...
		for (const notebook of notebooks) {
			for (const cell of notebook.getCells()) {
				...
				assert.equal(
					findNotebook(cell.document.uri, notebooks),
					notebook,
				);

				assert.isUndefined(
					getNotebookCellOutput(cell.document.uri, notebooks),
				);
			}
		}
	});
	...
	function createSampleNotebooks() {
		return [
			...
			ExtHostNotebookDocumentData.fromNotebookData(
				URI.file('three.ipynb').with({ scheme: 'ssh' }),
				new NotebookData(
					createCells([
						['markdown', '# Title'],
						['code', 'import sys'],
