...
export function translateDisplayDataOutput(output: any): NotebookCellOutput {
	...
	switch (outputType) {
		...
		default: {
			...
			const outputType: OutputType =
				<OutputType>customMetadata?.outputType ||
...
export class ExtHostNotebookDocumentData {
	public static createJupyterNotebook(
		uri: Uri,
		contents: string,
		simulationWorkspace?: ISimulationWorkspace,
	): ExtHostNotebookDocumentData {
		const notebook = JSON.parse(contents);
		const codeLanguageId =
			notebook.metadata?.language_info?.language ??
			notebook.metadata?.language_info?.name ??
			'python';
		const notebookDocument = new ExtHostNotebookDocumentData(
			uri,
			'jupyter-notebook',
			notebook.metadata,
			[],
			...
			if (cell.cell_type === 'code') {
				const doc = createTextDocumentData(
					uri.with({
						...
						fragment: generateCellFragment(index),
					}),
					content,
					codeLanguageId,
				);
				...
				const cellOutputs = Array.isArray(cell.outputs)
					? cell.outputs
					: [];
	...
	public static createGithubIssuesNotebook(
		uri: Uri,
		contents: string,
		simulationWorkspace?: ISimulationWorkspace,
	): ExtHostNotebookDocumentData {
	...
	public static fromNotebookData(
		uri: Uri,
		data: NotebookData,
		notebookType: string,
		simulationWorkspace?: ISimulationWorkspace,
	...
	public static applyEdits(
		notebookDocument: ExtHostNotebookDocumentData,
		...
		for (const edit of edits) {
			...
			if (edit.newCells) {
				ExtHostNotebookDocumentData.replaceCells(
					notebookDocument,
					edit.range,
					edit.newCells,
					simulationWorkspace,
				...
				notebookDocument._cells.splice(
					edit.range.start,
	...
	private static replaceCells(
		...
		const docs = cells.map((cell, index) => {
			const doc = createTextDocumentData(
				uri.with({
					scheme: Schemas.vscodeNotebookCell,
					fragment: generateCellFragment(
						notebookDocument.cells.length + index + 1,
					),
				}),
				cell.value,
				cell.languageId,
	...
	get document(): vscode.NotebookDocument {
		if (!this._notebook) {
			...
			const apiObject: vscode.NotebookDocument = {
				...
				get isUntitled() {
					return that.uri.scheme === 'untitled';
				},
				get isClosed() {
		...
		return JSON.stringify(
			{
				cells: this._cells.map((cell) => ({
					...
					outputs: (cell.apiCell.outputs || []).map(
