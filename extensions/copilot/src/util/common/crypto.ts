...
class StringSHA256Insecure {
	...
	constructor() {
		this._buff = new Uint8Array(
			SHA256Constant.BLOCK_SIZE + 3 /* to fit any utf-8 */,
		);
	...
	public digest(): string {
		if (!this._finished) {
			...
			if (this._leftoverHighSurrogate) {
				...
				this._buffLen = this._push(
					this._buff,
					this._buffLen,
					SHA256Constant.UNICODE_REPLACEMENT,
				);
			}
		...
		return (
			...
			toHexString(this._h5) +
			toHexString(this._h6) +
			toHexString(this._h7)
		);
	}
	...
	private _step(): void {
		...
		for (let j = 0; j < 64; j++) {
			...
			const ch = (e & f) ^ (~e & g);
			const temp1 =
				(h + S1 + ch + k[j] + bigBlock32.getUint32(j * 4, false)) >>> 0;
			const S0 =
