...
function exceptionToErrorMessage(exception: unknown, verbose: boolean): string {
	if (
		verbose &&
		((exception as any).stack || (exception as any).stacktrace)
	) {
		const stackStr =
			stackToString((exception as any).stack) ||
			stackToString((exception as any).stacktrace);
...
function detectSystemErrorMessage(exception: any): string {
	...
	if (exception.code === 'ERR_UNC_HOST_NOT_ALLOWED') {
		return l10n.t(
			"{0}. Please update the '{1}' setting if you want to allow this host.",
			exception.message,
