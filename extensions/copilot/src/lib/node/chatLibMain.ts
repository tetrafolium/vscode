...
} from '../../extension/completions-core/vscode-node/bridge/src/completionsTelemetryServiceBridge';
import {
	CopilotExtensionStatus,
	ICompletionsExtensionStatus,
} from '../../extension/completions-core/vscode-node/extension/src/extensionStatus';
import {
	CopilotTokenManagerImpl,
	ICompletionsCopilotTokenManager,
} from '../../extension/completions-core/vscode-node/lib/src/auth/copilotTokenManager';
import {
	ICompletionsCitationManager,
	IPCitationDetail,
	IPDocumentCitation,
} from '../../extension/completions-core/vscode-node/lib/src/citationManager';
import {
	CompletionNotifier,
	ICompletionsNotifierService,
} from '../../extension/completions-core/vscode-node/lib/src/completionNotifier';
...
} from '../../extension/completions-core/vscode-node/lib/src/config';
import {
	ICompletionsUserErrorNotifierService,
	UserErrorNotifier,
} from '../../extension/completions-core/vscode-node/lib/src/error/userErrorNotifier';
...
} from '../../extension/completions-core/vscode-node/lib/src/fileReader';
...
} from '../../extension/completions-core/vscode-node/lib/src/ghostText/asyncCompletions';
import {
	CompletionsCache,
...
} from '../../extension/completions-core/vscode-node/lib/src/ghostText/configBlockMode';
...
} from '../../extension/completions-core/vscode-node/lib/src/ghostText/current';
...
import {
	...
	LastGhostText,
} from '../../extension/completions-core/vscode-node/lib/src/ghostText/last';
...
} from '../../extension/completions-core/vscode-node/lib/src/ghostText/speculativeRequestCache';
...
import {
	LogLevel as CompletionsLogLevel,
	ICompletionsLogTargetService,
} from '../../extension/completions-core/vscode-node/lib/src/logger';
...
} from '../../extension/completions-core/vscode-node/lib/src/notificationSender';
import {
	ICompletionsOpenAIFetcherService,
	LiveOpenAIFetcher,
} from '../../extension/completions-core/vscode-node/lib/src/openai/fetch';
import {
	AvailableModelsManager,
	ICompletionsModelManagerService,
} from '../../extension/completions-core/vscode-node/lib/src/openai/model';
import {
	ICompletionsStatusReporter,
	StatusChangedEvent,
	StatusReporter,
} from '../../extension/completions-core/vscode-node/lib/src/progress';
import {
	CompletionsPromptFactory,
	ICompletionsPromptFactoryService,
} from '../../extension/completions-core/vscode-node/lib/src/prompt/completionsPromptFactory/completionsPromptFactory';
import {
	ContextProviderBridge,
	ICompletionsContextProviderBridgeService,
} from '../../extension/completions-core/vscode-node/lib/src/prompt/components/contextProviderBridge';
import {
...
} from '../../extension/completions-core/vscode-node/lib/src/prompt/contextProviderRegistry';
import {
	ContextProviderStatistics,
	ICompletionsContextProviderService,
} from '../../extension/completions-core/vscode-node/lib/src/prompt/contextProviderStatistics';
import {
	FullRecentEditsProvider,
	ICompletionsRecentEditsProviderService,
} from '../../extension/completions-core/vscode-node/lib/src/prompt/recentEdits/recentEditsProvider';
...
import {
	...
	TelemetryUserConfig,
} from '../../extension/completions-core/vscode-node/lib/src/telemetry/userConfig';
import {
	INotebookDocument,
	ITextDocument,
	TextDocumentIdentifier,
} from '../../extension/completions-core/vscode-node/lib/src/textDocument';
import {
...
import {
	...
	PromiseQueue,
} from '../../extension/completions-core/vscode-node/lib/src/util/promiseQueue';
import {
	ICompletionsRuntimeModeService,
	RuntimeMode,
} from '../../extension/completions-core/vscode-node/lib/src/util/runtimeMode';
import {
	DocumentContext,
	WorkspaceFolder,
} from '../../extension/completions-core/vscode-node/types/src';
import { DebugRecorder } from '../../extension/inlineEdits/node/debugRecorder';
import {
	INextEditProvider,
	NESInlineCompletionContext,
	NextEditProvider,
} from '../../extension/inlineEdits/node/nextEditProvider';
import {
...
import {
	IPowerService,
	NullPowerService,
} from '../../extension/power/common/powerService';
import { ChatMLFetcherImpl } from '../../extension/prompt/node/chatMLFetcher';
import {
	ISimilarFilesContextService,
	NullSimilarFilesContextService,
} from '../../extension/xtab/common/similarFilesContextService';
...
import {
	CopilotTokenStore,
	ICopilotTokenStore,
} from '../../platform/authentication/common/copilotTokenStore';
...
import {
	IInteractionService,
	InteractionService,
} from '../../platform/chat/common/interactionService';
import {
	BaseConfig,
	Config,
	ConfigKey,
...
import {
	...
	IUndesiredModelsManager,
	NullUndesiredModelsManager,
} from '../../platform/inlineEdits/common/inlineEditsModelService';
...
} from '../../platform/simulationTestContext/common/simulationTestContext';
import {
	ISnippyService,
	NullSnippyService,
...
import {
	ITokenizerProvider,
	TokenizerProvider,
} from '../../platform/tokenizer/node/tokenizer';
import {
	IWorkspaceService,
...
export function createNESProvider(
	options: INESProviderOptions,
...
class NESProvider extends Disposable implements INESProvider<NESResult> {
	private readonly _nextEditProvider: INextEditProvider<
		INextEditResult,
		LlmNESTelemetryBuilder
	>;
	...
	constructor(
		...
		const statelessNextEditProvider =
			instantiationService.createInstance(XtabProvider);
		...
		const xtabDiffNEntries =
			this._configurationService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsXtabDiffNEntries,
				this._expService,
			);
		const xtabHistoryTracker = new NesXtabHistoryTracker(
			this._options.workspace,
			xtabDiffNEntries,
			_configurationService,
			_expService,
		);
		this._debugRecorder = this._register(
			new DebugRecorder(this._options.workspace),
		);

		this._nextEditProvider = instantiationService.createInstance(
			NextEditProvider,
			this._options.workspace,
		...
		this._telemetrySender = this._register(
	...
	handleAcceptance(result: NESResult): void {
		...
		this._nextEditProvider.handleAcceptance(
			result.docId,
	...
	handleRejection(result: NESResult): void {
		...
		this._nextEditProvider.handleRejection(
			result.docId,
			result.internalResult,
		);
		this.handleEndOfLifetime(result);
	...
	handleIgnored(
		result: NESResult,
		supersededByRequestUuid: NESResult | undefined,
	): void {
		if (supersededByRequestUuid) {
			result.telemetryBuilder.setSupersededBy(
				supersededByRequestUuid.requestUuid,
			);
		}
		this._nextEditProvider.handleIgnored(
			result.docId,
			result.internalResult,
			supersededByRequestUuid?.internalResult,
		);
		this.handleEndOfLifetime(result);
	}
	...
	private handleEndOfLifetime(result: NESResult): void {
		try {
			this._telemetrySender.sendTelemetryForBuilder(
				result.telemetryBuilder,
			);
		} finally {
	...
	async getNextEdit(
		...
		try {
			const internalResult = await this._nextEditProvider.getNextEdit(
				docId,
				context,
				logContext,
				cancellationToken,
	...
	async setConfigs(overrides: Map<string, unknown>) {
		for (const [key, value] of overrides) {
			const config = globalConfigRegistry.configs.get(
				`${CopilotConfigPrefix}.${key}`,
			);
			if (config) {
...
function setupServices(options: INESProviderOptions) {
	const { fetcher, copilotTokenManager, telemetrySender, logTarget } =
		options;
	const builder = new InstantiationServiceBuilder();
	builder.define(
		IConfigurationService,
		new SyncDescriptor(OverridableConfigurationService, [
			options.configOverrides ?? new Map(),
		]),
	);
	builder.define(
		IExperimentationService,
		new SyncDescriptor(SimpleExperimentationService, [
			options.waitForTreatmentVariables,
		]),
	);
	builder.define(
		ISimulationTestContext,
		new SyncDescriptor(NulSimulationTestContext),
	);
	...
	builder.define(
		...
		new SyncDescriptor(LogServiceImpl, [
			[logTarget || new ConsoleLog(undefined, InternalLogLevel.Trace)],
		]),
	);
	builder.define(
		IGitExtensionService,
		new SyncDescriptor(NullGitExtensionService),
	);
	builder.define(
		ILanguageContextProviderService,
		new SyncDescriptor(NullLanguageContextProviderService),
	);
	builder.define(
		ILanguageDiagnosticsService,
		options.languageDiagnosticsService ||
			new SyncDescriptor(TestLanguageDiagnosticsService),
	);
	...
	builder.define(
		IFetcherService,
		new SyncDescriptor(SingleFetcherService, [fetcher]),
	);
	builder.define(
		ITelemetryService,
		new SyncDescriptor(SimpleTelemetryService, [telemetrySender]),
	);
	builder.define(
		IAuthenticationService,
	...
	builder.define(
		IChatWebSocketManager,
		new SyncDescriptor(NullChatWebSocketManager),
	);
	builder.define(
		IOTelService,
		new NoopOTelService(
	...
	builder.define(
		ITokenizerProvider,
		new SyncDescriptor(TokenizerProvider, [false]),
	);
	...
	builder.define(
		IInlineEditsModelService,
		new SyncDescriptor(InlineEditsModelService),
	);
	builder.define(
		IUndesiredModelsManager,
		options.undesiredModelsManager ||
			new SyncDescriptor(NullUndesiredModelsManager),
	);
	builder.define(
		ITerminalService,
		options.terminalService || new SyncDescriptor(NullTerminalService),
	);
	builder.define(
		ISimilarFilesContextService,
		new SyncDescriptor(NullSimilarFilesContextService),
	);
	...
	override async setConfig<T>(
		...
		this._onDidChangeConfiguration.fire({
			affectsConfiguration: (section) => {
				return (
					fullyQualifiedKey === section ||
					fullyQualifiedKey.startsWith(section + '.') ||
					section.startsWith(fullyQualifiedKey + '.')
				);
			},
		});
		return;
	...
	override getExperimentBasedConfig<T extends ExperimentBasedConfigType>(
		...
		if (this._overrides.has(key.id)) {
			...
			if (key.validator) {
				const result = key.validator.validate(overriddenValue);
				if (result.error) {
					return super.getExperimentBasedConfig(
						key,
...
class NullEndpointProvider implements IEndpointProvider {
	...
	async getAllCompletionModels(): Promise<[]> {
		return [];
	}
	async getAllChatEndpoints(): Promise<[]> {
		return [];
	}
...
export class SimpleExperimentationService
	extends Disposable
	implements IExperimentationService
{
	...
	private readonly _onDidTreatmentsChange = this._register(
	...
	updateTreatmentVariables(
		...
		if (changedVariables.length > 0) {
			this._onDidTreatmentsChange.fire({
				affectedTreatmentVariables: changedVariables,
			});
...
class SingleFetcherService implements IFetcherService {
	...
	constructor(private readonly _fetcher: IFetcher) {}
	...
	createWebSocket(
		...
		return {
			webSocket: new WebSocket(url, options),
...
class SimpleTelemetryService implements ITelemetryService {
	...
	sendGHTelemetryEvent(
		...
		this._telemetrySender.sendTelemetryEvent(
			eventName,
			eventPropertiesToSimpleObject(properties),
			measurements,
		);
	}
	...
	sendEnhancedGHTelemetryEvent(
		...
		if (this._telemetrySender.sendEnhancedTelemetryEvent) {
			this._telemetrySender.sendEnhancedTelemetryEvent(
				eventName,
				eventPropertiesToSimpleObject(properties),
				measurements,
			);
...
export type CompletionsContextProviderMatchFunction = (
...
export type ICompletionsTextDocumentChangeEvent =
...
export type ICompletionsTextDocumentFocusedEvent =
	Event<TextDocumentFocusedEvent>;
export type ICompletionsWorkspaceFoldersChangeEvent =
...
export interface ICompletionsTextDocumentManager {
	...
	): ICompletionsNotebookDocument | undefined;
...
export type IGetInlineCompletionsOptions = Exclude<
	Partial<GetGhostTextOptions>,
	'promptOnly'
> & {
...
export interface IInlineCompletionsProvider {
	...
	getInlineCompletions(
		...
		options?: IGetInlineCompletionsOptions,
...
class InlineCompletionsProvider
	extends Disposable
	implements IInlineCompletionsProvider
{
	...
	constructor(
		...
		private readonly _expService: IExperimentationService,
		@ICompletionsSpeculativeRequestCache
		private readonly _speculativeRequestCache: ICompletionsSpeculativeRequestCache,
		@ILogService private readonly _logService: ILogService,
		@IConfigurationService
		private readonly _configurationService: IConfigurationService,
		@ICompletionsConfigProvider
		private readonly _completionsConfigProvider: ICompletionsConfigProvider,
	) {
		super();
	...
	async setConfigs(overrides: Map<string, unknown>) {
		...
		if (this._completionsConfigProvider instanceof InMemoryConfigProvider) {
			this._completionsConfigProvider.setCopilotSettings(
	...
	async getInlineCompletions(
		...
		return await this.ghostText.getInlineCompletions(
			textDocument,
			position,
...
class UnwrappingTelemetrySender implements ITelemetrySender {
	...
	sendTelemetryEvent(
		...
		this.sender.sendTelemetryEvent(
	...
	sendEnhancedTelemetryEvent(
		...
		if (this.sender.sendEnhancedTelemetryEvent) {
			this.sender.sendEnhancedTelemetryEvent(
				this.normalizeEventName(eventName),
				properties,
				measurements,
			);
...
function setupCompletionServices(
	options: IInlineCompletionsProviderOptions,
	...
	const logTarget =
		options.logTarget || new ConsoleLog(undefined, InternalLogLevel.Trace);

	const builder = new InstantiationServiceBuilder();
	builder.define(
		ICompletionsLogTargetService,
		new (class implements ICompletionsLogTargetService {
			declare _serviceBrand: undefined;
			logIt(
				level: CompletionsLogLevel,
				category: string,
				...extra: unknown[]
			): void {
				logTarget.logIt(
					this.toExternalLogLevel(level),
					category,
					...extra,
				);
			}
			private toExternalLogLevel(level: CompletionsLogLevel): LogLevel {
				switch (level) {
					case CompletionsLogLevel.DEBUG:
						return LogLevel.Debug;
					case CompletionsLogLevel.INFO:
						return LogLevel.Info;
					case CompletionsLogLevel.WARN:
						return LogLevel.Warning;
					case CompletionsLogLevel.ERROR:
	...
	builder.define(
		IIgnoreService,
		options.ignoreService || new NullIgnoreService(),
	);
	builder.define(
		ITelemetryService,
		new SyncDescriptor(SimpleTelemetryService, [
			new UnwrappingTelemetrySender(telemetrySender),
		]),
	);
	builder.define(
		IConfigurationService,
	...
	builder.define(
		ICAPIClientService,
		options.capiClientService || new SyncDescriptor(CAPIClientImpl),
	);
	builder.define(
		IFetcherService,
		new SyncDescriptor(SingleFetcherService, [fetcher]),
	...
	builder.define(
		ICompletionsRuntimeModeService,
		RuntimeMode.fromEnvironment(options.isRunningInTest ?? false),
	);
	...
	builder.define(
		ICompletionsNotificationSender,
		new (class implements ICompletionsNotificationSender {
			declare _serviceBrand: undefined;
			async showWarningMessage(
				message: string,
				...actions: IActionItem[]
			): Promise<IActionItem | undefined> {
				return await options.notificationSender.showWarningMessage(
					message,
					...actions,
				);
			}
	...
	builder.define(
		ICompletionsEditorAndPluginInfo,
		new (class implements ICompletionsEditorAndPluginInfo {
			declare _serviceBrand: undefined;
	...
	builder.define(
		...
		new (class implements ICompletionsObservableWorkspace {
			declare _serviceBrand: undefined;
	...
	builder.define(
		ICompletionsStatusReporter,
		new (class extends StatusReporter {
	...
	builder.define(
		ICompletionsCopilotTokenManager,
		new SyncDescriptor(CopilotTokenManagerImpl, [false]),
	);
	builder.define(
		ICompletionsTextDocumentManagerService,
		new SyncDescriptor(
			class extends TextDocumentManager {
				onDidChangeTextDocument =
					documentManager.onDidChangeTextDocument;
				...
				onDidChangeWorkspaceFolders =
					documentManager.onDidChangeWorkspaceFolders;
				getTextDocumentsUnsafe(): ITextDocument[] {
	...
	builder.define(
		ICompletionsRecentEditsProviderService,
		new SyncDescriptor(FullRecentEditsProvider, [undefined]),
	);
	builder.define(
		ICompletionsNotifierService,
	...
	builder.define(
		...
		new SyncDescriptor(CompletionsFetchService),
	);
	builder.define(
		ICompletionsModelManagerService,
		new SyncDescriptor(AvailableModelsManager, [true]),
	);
	builder.define(
		ICompletionsAsyncManagerService,
	...
	builder.define(
		...
		new SyncDescriptor(CompositeRelatedFilesProvider),
	);
	...
	builder.define(
		...
		new SyncDescriptor(CachedContextProviderRegistry, [
			CoreContextProviderRegistry,
			(
				_: IInstantiationService,
				sel: DocumentSelector,
				docCtx: DocumentContext,
			) => options.contextProviderMatch(sel, docCtx),
		]),
	);
	builder.define(ICompletionsPromiseQueueService, new PromiseQueue());
	builder.define(
		ICompletionsCitationManager,
		new (class implements ICompletionsCitationManager {
			declare _serviceBrand: undefined;
			register(): IDisposable {
				return Disposable.None;
			}
			async handleIPCodeCitation(
				citation: IPDocumentCitation,
			): Promise<void> {
				if (options.citationHandler) {
					return await options.citationHandler.handleIPCodeCitation(
						citation,
					);
				}
			}
	...
	builder.define(
		...
		new (class implements ICompletionsFetcherService {
			declare _serviceBrand: undefined;
			getImplementation():
				| ICompletionsFetcherService
	...
	builder.define(
		IEnvService,
		new (class implements IEnvService {
			declare _serviceBrand: undefined;
			...
			readonly uiKind =
				editorSession.uiKind === 'web' ? 'web' : 'desktop';
			readonly OS =
			...
			isProduction(): boolean {
				return BuildInfo.isProduction();
			}
			isPreRelease(): boolean {
				return BuildInfo.isPreRelease();
			}
			isSimulation(): boolean {
				return options.isRunningInTest === true;
			}
			...
			getBuild(): string {
				return BuildInfo.getBuild();
			}
			getName(): string {
				return options.editorInfo.name;
			}
			getEditorInfo(): NameAndVersion {
				return new NameAndVersion(
					options.editorInfo.name,
					options.editorInfo.version,
				);
			}
			getEditorPluginInfo(): NameAndVersion {
				return new NameAndVersion(
					options.editorPluginInfo.name,
					options.editorPluginInfo.version,
				);
			}
	...
	builder.define(
		ILanguageContextProviderService,
		options.languageContextProvider ??
