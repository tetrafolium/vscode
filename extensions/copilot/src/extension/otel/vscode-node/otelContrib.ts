...
export class OTelContrib extends Disposable implements IExtensionContribution {
	constructor(
		...
		if (this._otelService.config.enabled) {
			this._logService.info(
				`[OTel] Instrumentation enabled — exporter=${this._otelService.config.exporterType} endpoint=${this._otelService.config.otlpEndpoint} captureContent=${this._otelService.config.captureContent}`,
			);
		} else {
		...
		this._register(
			vscode.commands.registerCommand(
				'github.copilot.chat.otel.flush',
				async () => {
					if (!this._otelService.config.enabled) {
						return;
					}
					this._logService.info(
						'[OTel] Flush requested — exporting pending traces, metrics, and events',
					);
		...
		this._register(
			vscode.commands.registerCommand(
				'github.copilot.chat.otel.exportAgentTracesDB',
				async (savePath?: vscode.Uri | string) => {
					...
					this._logService.info(
						`[OTel] Exported agent-traces.db to ${dest.fsPath}`,
					);
	...
	private _watchForReloadRequiredChanges(): void {
		...
		const initialValues = new Map(
			reloadSettings.map((s) => [
				s.fullyQualifiedId,
				vscode.workspace.getConfiguration().get(s.fullyQualifiedId),
			]),
		);
		...
		this._register(
			vscode.workspace.onDidChangeConfiguration(async (e) => {
				...
				const changed = reloadSettings.some(
					(s) =>
						e.affectsConfiguration(s.fullyQualifiedId) &&
						currentConfig.get(s.fullyQualifiedId) !==
							initialValues.get(s.fullyQualifiedId),
				);
				if (!changed) {
					return;
				}
	...
	override dispose(): void {
		...
		if (this._otelService.config.enabled) {
			this._logService.info(
				'[OTel] Shutting down — flushing pending traces, metrics, and events',
			);
		}
	...
	private _fireActivatedTelemetry(): void {
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('otel.activated', {
			...
			hasCustomEndpoint: String(
				config.enabled &&
					config.otlpEndpoint !== DEFAULT_OTLP_ENDPOINT &&
					config.otlpEndpoint !== DEFAULT_OTLP_ENDPOINT + '/',
			),
			hasCustomServiceName: String(config.serviceName !== 'copilot-chat'),
			hasResourceAttributes: String(
				Object.keys(config.resourceAttributes).length > 0,
			),
		});
	}
}
