...
export class ScmContextProviderContribution extends Disposable {
	...
	private registerContextProvider(): IDisposable {
		...
		try {
			...
			disposables.add(
				this._languageContextProviderService.registerContextProvider(
					provider,
			...
			this._logService.error(
				'Error registering SCM context provider:',
				error,
			);
		}
	...
	private _registerCacheInvalidation(): IDisposable {
		...
		const resourcesToConsider = this._gitService.activeRepository.map(
			(repository) => {
				if (!repository) {
				...
				const workingTreeChanges =
					repository.changes?.workingTree.map((change) =>
						change.uri.toString(),
					) ?? [];
...
class ScmContextResolver implements Copilot.ContextResolver<Copilot.SupportedContextItem> {
	...
	async *resolve(
		...
		const inputValidationLength =
			this._configurationService.getNonExtensionConfig<number>(
				'git.inputValidationLength',
			) ?? 72;
		const inputValidationSubjectLength =
			this._configurationService.getNonExtensionConfig<number>(
	...
	private async _getStagedChangesDiffs(
		token: CancellationToken,
	): Promise<Diff[]> {
		...
		try {
			...
			if (!repository) {
				this._logService.trace(
					'[ScmContextResolver] No active repository found',
				);
				return [];
			}
			...
			if (!changes) {
				this._logService.trace(
					'[ScmContextResolver] No changes available',
				);
				return [];
			}
			...
			if (changedFiles.length === 0) {
				this._logService.trace(
					'[ScmContextResolver] No changed files found',
				);
				return [];
			}
			...
			if (this._cachedDiffs && this._cachedDiffs.cacheKey === cacheKey) {
				this._logService.trace(
					'[ScmContextResolver] Returning cached diffs',
				);
				return this._cachedDiffs.diffs;
			}

			const diffs = await this._gitDiffService.getChangeDiffs(
				repository.rootUri,
				changedFiles,
			);
			this._logService.trace(
				`[ScmContextResolver] Retrieved ${diffs.length} diffs`,
			);
			...
			this._logService.error(
				'[ScmContextResolver] Error getting staged changes diffs:',
				error,
			);
			return [];
		}
