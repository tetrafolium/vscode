...
export class InlineChat2Prompt extends PromptElement<InlineChat2PromptProps> {
	...
	override render(state: void, sizing: PromptSizing): Promise<any> {
		...
		const variables = new ChatVariablesCollection(
			this.props.request.references,
		);
		const filepath = this._promptPathRepresentationService.getFilePath(
			snapshotAtRequest.uri,
		);
			...
			<>
				<SystemMessage priority={1000}>
					...
					<Tag name="instructions">
						You are an AI coding assistant that is used for quick,
						inline code changes. Changes are scoped to a single file
						or to some selected code in that file. You can ONLY edit
						that file and must use a tool to make these edits.
						<br />
						The user is interested in code changes grounded in the
						user's prompt. So, focus on coding, no wordy
						explanations, and do not ask back for clarifications.
						<br />
						Make all changes in a single invocation of the edit-tool
						(there is no tool calling loop).
						<br />
						{isLargeFile && readToolName && (
							<>
								The file is large and only a portion is shown
								below. If you need to see more of the file to
								make the requested change, use the{' '}
								{readToolName} tool to read additional parts of
								this file before editing. Do NOT use it to read
								other files.
								<br />
							</>
						)}
						Do not make code changes that are not directly and
						logically related to the user's prompt. When you cannot
						make a code change, reply with just a few words.
						<br />
					</Tag>
				...
				<UserMessage>
					{isLargeFile ? (
						...
						<>
							<>
								The filepath is `{filepath}` and this is its
								content:
								<br />
							</>
							<Tag name="file">
								<CodeBlock
									...
									languageId={snapshotAtRequest.languageId}
									uri={snapshotAtRequest.uri}
									references={[
										new PromptReference(
											snapshotAtRequest.uri,
											undefined,
									...
									code={snapshotAtRequest.getText()}
								/>
							</Tag>
						</>
					)}
					...
					<Tag name="reminder">
						{selection.isEmpty ? (
							<>
								Make changes or write new code anywhere in the
								file.
								<br />
							</>
						) : (
							<>
								Focus on the selection, and try to make changes
								to the selected code and its context.
								<br />
							</>
						)}
						Do not make code changes that are not directly and
						logically related to the user's prompt.
						<br />
						ONLY change the `{filepath}` file, make all changes in a
						single invocation of the edit-tool, and change NO other
						file.
					</Tag>
					<cacheBreakpoint type={CacheType} />
				</UserMessage>
				<UserMessage>
					<Tag name="prompt">
						<UserQuery
							...
							priority={900}
							chatVariables={variables}
							query={this.props.request.prompt}
						/>
					</Tag>
					<cacheBreakpoint type={CacheType} />
				</UserMessage>
				<ToolCallRoundsElement
					...
					documentVersionAtRequest={
						this.props.snapshotAtRequest.version
					}
...
class CroppedFileContentElement extends PromptElement<CroppedFileContentElementProps> {
	override render() {
		...
		if (selection.isEmpty) {
			...
			cropStart = Math.max(
				0,
				selStart - LARGE_FILE_SELECTION_CONTEXT_LINES,
			);
			cropEnd = Math.min(
				totalLines - 1,
				selEnd + LARGE_FILE_SELECTION_CONTEXT_LINES,
			);
		}

		const croppedText = snapshot.getText(
			new Range(
		...
		return (
			<>
				<>
					The filepath is `{filepath}` ({totalLines} lines total).
					Showing lines {shownFrom}-{shownTo}:<br />
				</>
				<Tag name="file">
					<CodeBlock
						includeFilepath={false}
...
export class FileContextElement extends PromptElement<FileContextElementProps> {
	override render(
		...
		while (startLine > 0) {
			seenNonEmpty =
				seenNonEmpty ||
				!this.props.snapshot.lineAt(startLine).isEmptyOrWhitespace;
			startLine--;
			n++;
			if (n >= 3 && seenNonEmpty) {
				break;
		...
		while (endLine < this.props.snapshot.lineCount - 1) {
			seenNonEmpty =
				seenNonEmpty ||
				!this.props.snapshot.lineAt(endLine).isEmptyOrWhitespace;
			endLine++;
			n++;
			if (n >= 3 && seenNonEmpty) {
				break;
		...
		const textBefore = this.props.snapshot.getText(
			new Range(
				this.props.position.with({ line: startLine, character: 0 }),
				this.props.position,
			),
		);
		const textAfter = this.props.snapshot.getText(
			new Range(
				this.props.position,
				this.props.position.with({
					line: endLine,
					character: Number.MAX_SAFE_INTEGER,
				}),
			),
		...
		return (
			<>
				<Tag name="file-cursor-context">
					<CodeBlock
						includeFilepath={false}
						languageId={this.props.snapshot.languageId}
						uri={this.props.snapshot.uri}
						references={[
							new PromptReference(
								this.props.snapshot.uri,
								undefined,
								undefined,
							),
						]}
...
export class FileSelectionElement extends PromptElement<FileSelectionElementProps> {
	override render(
		...
		const selectedLines = this.props.snapshot.getText(
			this.props.selection.with({
				start: this.props.selection.start.with({ character: 0 }),
				end: this.props.selection.end.with({
					character: Number.MAX_SAFE_INTEGER,
				}),
			}),
		);
			...
			<>
				{this.props.previousRounds.map((round) => {
					...
					return (
						<>
							<AssistantMessage
								toolCalls={round.calls.map(([toolCall]) => ({
				...
				{this.props.hasFailedEdits && (
					<UserMessage>
						{documentNow.version ===
							this.props.documentVersionAtRequest && (
							<>
								<Tag name="feedback">
									Editing this file did not produce the
									desired result. No changes were made.
									Understand the previous edit attempts and
									the original file content, and <br />
						...
						{documentNow.version !==
							...
							<>
								<Tag name="feedback">
									Editing this file did not produce the
									desired result. Understand the previous edit
									attempts and the current file content, and{' '}
									<br />
									produce a better edit. This is the current
									file content:
									<br />
								</Tag>
								{this.props.isLargeFile ? (
									<CroppedFileContentElement
										snapshot={TextDocumentSnapshot.create(
											documentNow,
										)}
									...
									<Tag name="file">
										<CodeBlock
											includeFilepath={false}
											languageId={documentNow.languageId}
											uri={documentNow.uri}
											references={[
												new PromptReference(
													documentNow.uri,
