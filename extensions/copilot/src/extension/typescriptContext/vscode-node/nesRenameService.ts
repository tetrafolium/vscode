...
type PrepareNesRenameRequestArgs = Omit<
	protocol.PrepareNesRenameRequestArgs,
...
namespace PrepareNesRenameRequestArgs {
	export function create(
		...
		oldName: string,
		newName: string,
		lastSymbolRename: vscode.Range | undefined,
		startTime: number,
		timeBudget: number,
	): PrepareNesRenameRequestArgs {
		return {
			...
			lastSymbolRename: lastSymbolRename
				? {
						start: {
							line: lastSymbolRename.start.line + 1,
							character: lastSymbolRename.start.character + 1,
						},
						end: {
							line: lastSymbolRename.end.line + 1,
							character: lastSymbolRename.end.character + 1,
						},
					}
				: undefined,
			startTime: startTime,
...
type NesRenameRequestArgs = Omit<
	protocol.NesRenameRequestArgs,
	'file' | 'projectFileName' | 'line' | 'offset'
> & {
	file: vscode.Uri;
	line: number;
...
class TelemetrySender {
	...
	public sendPrepareNesRenameTelemetry(
		requestId: string,
		timeTaken: number,
		canRename: protocol.RenameKind,
		timedOut: boolean,
	): void {
		/* __GDPR__
		...
		*/
		...
		this.logService.info(
			`NES Rename Prepare: canRename=${canRename}, timeTaken=${timeTaken}, timedOut=${timedOut}`,
		);
	}

	public sendPrepareNesRenameFailureTelemetry(
		requestId: string,
...
export class NesRenameContribution implements vscode.Disposable {
	...
	constructor(
		...
		this.disposables.add(
			vscode.commands.registerCommand(
				...
				async (
					...
					const no: protocol.PrepareNesRenameResult.No = {
						canRename: protocol.RenameKind.no,
						timedOut: false,
					};
					const params = this.resolvePrepareParams(
						uri,
						position,
						oldName,
						newName,
					...
					const args: PrepareNesRenameRequestArgs =
						PrepareNesRenameRequestArgs.create(
							document,
							position,
							oldName,
							newName,
					...
					try {
						const result =
							await vscode.commands.executeCommand<protocol.PrepareNesRenameResponse>(
								'typescript.tsserverRequest',
								'_.copilot.prepareNesRename',
								args,
								NesRenameContribution.ExecConfig,
								tokenSource.token,
							);
						if (protocol.PrepareNesRenameResponse.isError(result)) {
							this.telemetrySender.sendPrepareNesRenameFailureTelemetry(
								requestId,
								result.body,
							);
							return no;
						} else if (
							protocol.PrepareNesRenameResponse.isOk(result)
						) {
							const timedOut =
								result.body.canRename === protocol.RenameKind.no
									? result.body.timedOut
									: false;
							this.telemetrySender.sendPrepareNesRenameTelemetry(
								requestId,
								Date.now() - startTime,
								result.body.canRename,
								timedOut,
							);
							return result.body;
						} else {
		...
		this.disposables.add(
			vscode.commands.registerCommand(
				...
				async (
					...
					const params = this.resolveRenameParams(
						uri,
						position,
						oldName,
						newName,
					...
					oldName = params.oldName;
					newName = params.newName;
					const args: NesRenameRequestArgs =
						NesRenameRequestArgs.create(
							document,
							position,
							oldName,
							newName,
					...
					try {
						const result =
							await vscode.commands.executeCommand<protocol.NesRenameResponse>(
								'typescript.tsserverRequest',
		...
		this.disposables.add(
			vscode.commands.registerCommand(
				'github.copilot.debug.validateNesRename',
				async () => {
					...
					if (!activated) {
						vscode.window.showErrorMessage(
							'TypeScript NES Rename plugin is not activated.',
						);
						return;
					}
					...
					try {
						...
						if (protocol.PrepareNesRenameResponse.isError(result)) {
							vscode.window.showErrorMessage(
								`Prepare NES Rename error: ${result.message}`,
							);
						} else if (
							protocol.PrepareNesRenameResponse.isOk(result)
						) {
							...
							if (body.canRename === protocol.RenameKind.yes) {
								vscode.window.showInformationMessage(
									`Prepare NES Rename: Can rename '${oldName}' to '${newName}'.`,
								);
							} else if (
								body.canRename === protocol.RenameKind.maybe
							) {
								vscode.window.showWarningMessage(
									`Prepare NES Rename: Maybe can rename '${oldName}' to '${newName}'.`,
								);
							} else {
								vscode.window.showErrorMessage(
									`Prepare NES Rename: Cannot rename '${oldName}' to '${newName}'. Reason: ${body.reason ?? 'Not provided'}`,
								);
							}
						}
					} finally {
	...
	private async getUserParams(): Promise<
		| {
				document: vscode.TextDocument;
				position: vscode.Position;
				oldName: string;
	...
	> {
		...
		const newName = await vscode.window.showInputBox({
			prompt: 'Enter the new name for NES rename',
		});
