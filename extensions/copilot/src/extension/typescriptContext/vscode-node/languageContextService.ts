...
namespace TypeScriptServerError {
	export function is(value: Error): value is TypeScriptServerError {
		...
		return (
			...
			typeof candidate.version.displayName === 'string'
		);
	}
}
...
class TelemetrySender {
	...
	public sendSpeculativeRequestTelemetry(
		...
		const shouldSendTelemetry =
			sampleTelemetry === 1 ||
			this.sendSpeculativeRequestTelemetryCounter % sampleTelemetry === 0;
		...
		this.logService.debug(
			`TypeScript Copilot context speculative request: [${context.requestId} - ${originalRequestId}, numberOfItems: ${numberOfItems}]`,
		);
	}
	...
	public willLogRequestTelemetry(context: RequestContext): boolean {
		...
		return (
			sampleTelemetry === 1 ||
			this.sendRequestTelemetryCounter % sampleTelemetry === 0
		);
	}

	public sendRequestTelemetry(
		document: vscode.TextDocument,
		position: vscode.Position,
		context: RequestContext,
		data: ContextItemSummary,
		timeTaken: number,
		cacheState: { before: CacheState; after: CacheState } | undefined,
		cacheRequest: string | undefined,
	): void {
			...
			*/
			this.telemetryService.sendMSFTTelemetryEvent(
				'typescript-context-plugin.completion-context.request',
				{
					...
					isSpeculative: (context.proposedEdits !== undefined &&
					context.proposedEdits.length > 0
						? true
						: false
					).toString(),
	...
	public sendRequestOnTimeoutTelemetry(
		context: RequestContext,
		data: ContextItemSummary,
		...
		*/
		...
		this.logService.debug(
			`TypeScript Copilot context on timeout: [${context.requestId}, ${JSON.stringify(stats, undefined, 0)}]`,
		);
	}

	public sendRequestFailureTelemetry(
		context: RequestContext,
		data: { error: protocol.ErrorCode; message: string; stack?: string },
	): void {
		/* __GDPR__
	...
	public sendRequestCancelledTelemetry(
		context: RequestContext,
		timeTaken: number,
	): void {
		/* __GDPR__
		...
		*/
		...
		this.logService.debug(
			`TypeScript Copilot context request ${context.requestId} got cancelled.`,
		);
	}

	public sendActivationTelemetry(
		response: protocol.PingResponse | undefined,
		error: unknown | undefined,
	): void {
		if (response !== undefined) {
			const body: protocol.PingResponse['body'] | undefined =
				response?.body;
			if (body?.kind === 'ok') {
				/* __GDPR__
				...
				*/
				...
				this.sendActivationFailedTelemetry(
					ErrorLocation.Server,
					ErrorPart.ServerPlugin,
					body.message,
					body.stack,
				);
			} else {
				this.sendUnknownPingResponseTelemetry(
					ErrorLocation.Server,
					ErrorPart.ServerPlugin,
					response,
				);
			...
			this.sendActivationFailedTelemetry(
				ErrorLocation.Client,
				ErrorPart.ServerPlugin,
				'Neither response nor error received.',
				undefined,
			);
		}
	}

	public sendActivationFailedTelemetry(
	...
	private sendUnknownPingResponseTelemetry(
		location: ErrorLocation,
		part: ErrorPart,
		response: object,
	): void {
	...
	public sendIntegrationTelemetry(
		requestId: string,
		document: string,
	...
	public sendInlineCompletionProviderTelemetry(
		source: KnownSources,
		registered: boolean,
	): void {
		if (registered) {
...
class RunnableResultManager implements vscode.Disposable {
	...
	private results: Map<
		protocol.ContextRunnableResultId,
		ResolvedRunnableResult
	>;
	private readonly withInRangeRunnableResults: {
		resultId: protocol.ContextRunnableResultId;
		range: vscode.Range;
	}[];
	private readonly outsideRangeRunnableResults: {
		resultId: protocol.ContextRunnableResultId;
		ranges: vscode.Range[];
	}[] = [];
	private readonly neighborFileRunnableResults: {
		resultId: protocol.ContextRunnableResultId;
	}[];
	...
	constructor() {
				...
				(event: vscode.TextDocumentChangeEvent) => {
					...
					if (
						...
						for (const change of event.contentChanges) {
							...
							for (
								let i = 0;
								i < this.withInRangeRunnableResults.length;
							) {
								const entry =
									this.withInRangeRunnableResults[i];
								if (entry.range.contains(changeRange)) {
									entry.range =
										this.applyTextContentChangeEventToWithinRange(
											change,
											entry.range,
										);
									i++;
									...
									this.withInRangeRunnableResults.splice(
										i,
										1,
									);
								}
							...
							for (
								let i = 0;
								i < this.outsideRangeRunnableResults.length;
							) {
								const entry =
									this.outsideRangeRunnableResults[i];
								const ranges =
									this.applyTextContentChangeEventToOutsideRanges(
										change,
										entry.ranges,
									);
								if (ranges === undefined) {
									const id = entry.resultId;
									this.results.delete(id);
									this.outsideRangeRunnableResults.splice(
										i,
										1,
									);
								} else {
		...
		this.disposables.add(
			vscode.workspace.onDidCloseTextDocument(
				(document: vscode.TextDocument) => {
					if (
						this.requestInfo?.document === document.uri.toString()
					) {
						this.clear();
					}
				},
	...
	public update(
		...
		position: vscode.Position,
		context: RequestContext,
		body: protocol.ComputeContextResponse.OK,
		...
		if (
			body.runnableResults === undefined ||
			body.runnableResults.length === 0 ||
			body.path === undefined ||
			body.path.length === 0 ||
			body.path[0] === 0
		) {
		...
		const updateRunnableResult = (
			resultItem: protocol.ContextRunnableResultTypes,
		): ResolvedRunnableResult | undefined => {
			let result: ResolvedRunnableResult | undefined;
			if (
				resultItem.kind ===
				protocol.ContextRunnableResultKind.ComputedResult
			) {
				...
				for (const contextItem of resultItem.items) {
					if (contextItem.kind === protocol.ContextKind.Reference) {
						const referenced: protocol.FullContextItem | undefined =
							itemMap.get(contextItem.key);
				...
				resultItem.kind === protocol.ContextRunnableResultKind.Reference
			) {
			...
			if (result.cache !== undefined) {
				if (
					result.cache.scope.kind ===
					protocol.CacheScopeKind.WithinRange
				) {
					const scopeRange = result.cache.scope.range;
					const range = new vscode.Range(
						scopeRange.start.line,
						scopeRange.start.character,
						scopeRange.end.line,
						scopeRange.end.character,
					);
					this.withInRangeRunnableResults.push({
						range,
						resultId: result.id,
					});
				} else if (
					result.cache.scope.kind ===
					protocol.CacheScopeKind.NeighborFiles
				) {
					this.neighborFileRunnableResults.push({
						resultId: result.id,
					});
				} else if (
					result.cache.scope.kind ===
					protocol.CacheScopeKind.OutsideRange
				) {
					...
					this.outsideRangeRunnableResults.push({
						resultId: result.id,
						ranges,
					});
				}
	...
	public getRunnableResult(
		id: protocol.ContextRunnableResultId,
	): ResolvedRunnableResult | undefined {
		return this.results.get(id);
	}

	public getCachedRunnableResults(
		document: vscode.TextDocument,
		position: vscode.Position,
		emitMode?: protocol.EmitMode,
	): ResolvedRunnableResult[] {
		const results: ResolvedRunnableResult[] = [];
		...
		if (
			this.cacheInfo.version !== document.version ||
			this.cacheInfo.state === CacheState.NotPopulated ||
			this.requestInfo.path.length === 0 ||
			this.requestInfo.path[0] === 0
		) {
			return results;
		}
		for (const item of this.results.values()) {
			...
			if (
				scope === undefined ||
				scope.kind !== protocol.CacheScopeKind.WithinRange
			) {
				results.push(item);
			} else {
				const r = scope.range;
	...
	public getContextRequestState(
		document: vscode.TextDocument,
		position: vscode.Position,
	): ContextRequestState | undefined {
		if (this.requestInfo?.document !== document.uri.toString()) {
			return undefined;
		}
		if (
		...
		const items: Map<protocol.ContextItemKey, protocol.FullContextItem> =
			new Map();
		...
		if (this.isCacheFullyUpToDate(document, position)) {
			...
			const canSkipItems = (
				...
				if (rr.state === protocol.ContextRunnableState.IsFull) {
					...
					return (
						kind === protocol.CacheScopeKind.WithinRange ||
						kind === protocol.CacheScopeKind.NeighborFiles ||
						kind === protocol.CacheScopeKind.File
					);
				}
				return false;
			};
			const handleRunnableResult = (
				id: string,
				rr: ResolvedRunnableResult,
			) => {
				...
				if (cache !== undefined) {
					...
					if (emitMode === protocol.EmitMode.ClientBased) {
						...
						emitMode === protocol.EmitMode.ClientBasedOnTimeout
					) {
				...
				for (const item of rr.items) {
					...
					cachedResult.items.push(
						protocol.CachedContextItem.create(key, size),
					);
					items.set(key, item);
				}
			};
		...
		return {
			...
			server,
			itemMap: items,
			resultMap: new Map(this.results),
		};
	}

	private isCacheFullyUpToDate(
		document: vscode.TextDocument,
		position: vscode.Position,
	): boolean {
		...
		if (
			this.requestInfo.version === document.version &&
			this.requestInfo.position.isEqual(position)
		) {
			return true;
		}
		...
		const range = position.isBefore(this.requestInfo.position)
			? new vscode.Range(position, this.requestInfo.position)
			: new vscode.Range(this.requestInfo.position, position);
	...
	private applyTextContentChangeEventToWithinRange(
		...
		const linesDelta =
			(eventText.match(/\n/g) || []).length -
			(eventRange.end.line - eventRange.start.line);
		...
		if (eventRange.end.line === range.end.line) {
			...
			const newTextLength =
				lastNewLineIndex !== -1
					? eventText.length - lastNewLineIndex - 1
					: eventText.length;
			const oldTextLength =
	...
	private applyTextContentChangeEventToOutsideRanges(
		event: vscode.TextDocumentContentChangeEvent,
		ranges: vscode.Range[],
		...
		const linesDelta =
			(eventText.match(/\n/g) || []).length -
			(changeRange.end.line - changeRange.start.line);
			...
			if (range.end.isBefore(changeRange.start)) {
				...
				if (linesDelta === 0) {
					...
					adjustedRanges.push(
						new vscode.Range(
							new vscode.Position(
								range.start.line + linesDelta,
								range.start.character,
							),
							new vscode.Position(
								range.end.line + linesDelta,
								range.end.character,
							),
						),
					);
...
class NeighborFileModel implements vscode.Disposable {
	...
	constructor() {
		...
		this.visible = new LRUCache<string, string>({
			max: NeighborFileModel.MAX_ITEMS,
		});
		this.notVisible = new LRUCache<string, string>({
			max: NeighborFileModel.MAX_ITEMS,
		});
		this.disposables.add(
			vscode.window.onDidChangeActiveTextEditor(
		...
		for (const group of vscode.window.tabGroups.all) {
			for (const tab of group.tabs) {
				...
				if (
					uri !== undefined &&
					textDocumentsToConsider.has(uri.toString())
				) {
...
type ComputeContextRequestArgs = Omit<
	protocol.ComputeContextRequestArgs,
	'file' | 'projectFileName' | 'line' | 'offset'
> & {
	file: vscode.Uri;
	line: number;
	offset: number;
...
namespace ComputeContextRequestArgs {
	export function create(
		...
		startTime: number,
		timeBudget: number,
		willLogRequestTelemetry: boolean,
		neighborFiles: readonly string[] | undefined,
		clientSideRunnableResults:
			| readonly protocol.CachedContextRunnableResult[]
			| undefined,
		includeDocumentation: boolean,
	): ComputeContextRequestArgs {
		return {
			...
			neighborFiles:
				neighborFiles !== undefined && neighborFiles.length > 0
					? neighborFiles
					: undefined,
...
class InflightRequestInfo {
	...
	constructor(
		...
		serverPromise: Thenable<protocol.ComputeContextResponse>,
	) {
		this.document = document.uri.toString();
		this.position = position;
	...
	public matches(
		...
		return (
			this.document === document.uri.toString() &&
			this.position.isEqual(position)
		);
	}
...
namespace ContextItemUsageMode {
	export function fromString(value: string): ContextItemUsageMode {
		switch (value) {
			case 'minimal':
				return ContextItemUsageMode.minimal;
			case 'double':
				return ContextItemUsageMode.double;
			case 'fillHalf':
				return ContextItemUsageMode.fillHalf;
			case 'fill':
				return ContextItemUsageMode.fill;
			default:
				return ContextItemUsageMode.minimal;
		}
	}
...
export class LanguageContextServiceImpl
	implements ILanguageContextService, vscode.Disposable
{
	...
	private static readonly ExecConfig: ExecConfig = {
	...
	constructor(
		...
		this.isDebugging = process.execArgv.some((arg) =>
		...
		this.disposables.add(
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					...
					this.cachePopulationTimeout =
						this.getCachePopulationBudget();
				} else if (
					e.affectsConfiguration(
		...
		this._onCachePopulated = this.disposables.add(
			new vscode.EventEmitter<OnCachePopulatedEvent>(),
		);
		this.onCachePopulated = this._onCachePopulated.event;

		this._onContextComputed = this.disposables.add(
			new vscode.EventEmitter<OnContextComputedEvent>(),
		);
		this.onContextComputed = this._onContextComputed.event;

		this._onContextComputedOnTimeout = this.disposables.add(
			new vscode.EventEmitter<OnContextComputedOnTimeoutEvent>(),
		);
		this.onContextComputedOnTimeout =
			this._onContextComputedOnTimeout.event;
	}
	...
	async isActivated(
		...
		const languageId =
			typeof documentOrLanguageId === 'string'
				? documentOrLanguageId
				: documentOrLanguageId.languageId;
	...
	private async doIsTypeScriptActivated(
		languageId: string,
	): Promise<boolean> {
		let activated = false;
		...
		try {
			...
			const response: protocol.PingResponse | undefined =
				await vscode.commands.executeCommand(
					...
					'_.copilot.ping',
					LanguageContextServiceImpl.ExecConfig,
			...
			if (response !== undefined) {
				if (response.body?.kind === 'ok') {
					...
					this.logService.error(
						'TypeScript server plugin not activated:',
						response.body?.message ?? 'Message not provided.',
					);
				}
			} else {
				this.logService.error(
			...
			this.logService.error(
				'Error pinging TypeScript server plugin:',
				error,
			);
		}
	...
	async populateCache(
		...
		if (this.inflightCachePopulationRequest !== undefined) {
			if (
				!this.inflightCachePopulationRequest.matches(document, position)
			) {
				// We have a request running. Do not issue another cache request but remember the pending request.
				this.pendingRequest = new PendingRequestInfo(
					document,
					position,
					context,
				);
		...
		const contextRequestState =
			this.runnableResultManager.getContextRequestState(
				document,
				position,
			);
		if (
			contextRequestState !== undefined &&
			contextRequestState.server.length === 0
		) {
			// There is nothing to do on the server. Cache is up to date.
			return;
		}
		const neighborFiles: string[] =
			this.neighborFileModel.getNeighborFiles(document);
		const timeBudget = this.cachePopulationTimeout;
		const willLogRequestTelemetry =
			this.telemetrySender.willLogRequestTelemetry(context);
		const args: ComputeContextRequestArgs =
			ComputeContextRequestArgs.create(
				document,
				position,
				context,
				startTime,
				timeBudget,
				willLogRequestTelemetry,
				neighborFiles,
				contextRequestState?.server,
				this.includeDocumentation,
			);
		try {
			const isDebugging = this.isDebugging;
			const forDebugging: ContextItem[] | undefined = isDebugging
				? []
				: undefined;
			...
			try {
				const promise: Thenable<protocol.ComputeContextResponse> =
				...
				inflightRequest = new InflightRequestInfo(
					document,
					position,
					context,
					tokenSource,
			...
			if (protocol.ComputeContextResponse.isCancelled(response)) {
				this.telemetrySender.sendRequestCancelledTelemetry(
					context,
					timeTaken,
				);
				...
				const { resolved, cached, referenced, serverComputed } =
					this.runnableResultManager.update(
						document,
						documentVersion,
						position,
						context,
				...
				if (resolved.length > 0) {
					...
					for (const runnableResult of resolved) {
						for (const converted of contextItemResult.update(
							runnableResult,
						)) {
			...
			this.logService.error(
				error,
				`Error populating cache for document: ${document.uri.toString()} at position: ${position.line + 1}:${position.character + 1}`,
			);
		}
		if (this.pendingRequest !== undefined) {
			...
			if (textEditor !== undefined) {
				...
				if (
					document.uri.toString() === pendingRequest.document &&
					document.version === pendingRequest.version &&
					document
						.validatePosition(pendingRequest.position)
						.isEqual(pendingRequest.position)
				) {
					this.populateCache(
						document,
	...
	public async *getContext(
		...
		const cachePopulationRequestInflight =
			this.inflightCachePopulationRequest !== undefined &&
			this.inflightCachePopulationRequest.matchesDocument(document);
		...
		const { mandatory, optional, onTimeout } = this.getRunnables(
			document,
			position,
			cachePopulationRequestInflight,
		);
		if (this.onTimeoutData !== undefined) {
			this.onTimeoutData.addRunnableResults(onTimeout);
		}
		outer: for (const runnableResult of mandatory) {
			for (const { item, size } of contextItemResult.update(
				runnableResult,
				true,
			)) {
		...
		if (!token.isCancellationRequested) {
			...
			if (
				...
				const timeOut = Math.max(
					...
					Math.min(
						context.timeBudget ??
							LanguageContextServiceImpl.defaultCachePopulationRaceTimeout,
						LanguageContextServiceImpl.defaultCachePopulationRaceTimeout,
					),
				);
				...
				if (result !== 'timedOut') {
					...
					if (this.onTimeoutData !== undefined) {
						...
						const runnableResults =
							this.runnableResultManager.getCachedRunnableResults(
								document,
								position,
								protocol.EmitMode.ClientBasedOnTimeout,
							);
						for (const runnableResult of runnableResults) {
							for (const { item } of contextItemResult.update(
								runnableResult,
							)) {
								forDebugging?.push(item);
								yield item;
		...
		if (isSpeculativeRequest) {
			this.telemetrySender.sendSpeculativeRequestTelemetry(
				context,
				this.runnableResultManager.getRequestId() ?? 'unknown',
				contextItemResult.stats.yielded,
			);
		} else {
			const cacheState = this.runnableResultManager.getCacheState();
	...
	private getRunnables(
		document: vscode.TextDocument,
		position: vscode.Position,
		cachePopulationInflight: boolean,
	): {
		mandatory: readonly ResolvedRunnableResult[];
		optional: readonly ResolvedRunnableResult[];
		onTimeout: readonly ResolvedRunnableResult[];
	} {
		const mandatory: ResolvedRunnableResult[] = [];
		const optional: ResolvedRunnableResult[] = [];
		const onTimeout: ResolvedRunnableResult[] = [];
		for (const runnable of this.runnableResultManager.getCachedRunnableResults(
			document,
			position,
		)) {
			if (
				cachePopulationInflight &&
				runnable.cache?.emitMode ===
					protocol.EmitMode.ClientBasedOnTimeout
			) {
				...
				if (
					priority === protocol.Priorities.Expression ||
					priority === protocol.Priorities.Locals ||
					priority === protocol.Priorities.Inherited ||
					priority === protocol.Priorities.Traits
				) {
	...
	public getContextOnTimeout(
		...
		try {
			...
			if (
				!this.onTimeoutData.matches(document, position) ||
				this.onTimeoutData.resultBuilder === undefined
			) {
				return [];
			}
	...
	private getCachePopulationBudget(): number {
		const result = this.configurationService.getExperimentBasedConfig(
			ConfigKey.TypeScriptLanguageContextCacheTimeout,
			this.experimentationService,
		);
		return (
			result ?? LanguageContextServiceImpl.defaultCachePopulationBudget
		);
	}

	private getUsageMode(): ContextItemUsageMode {
		const value = this.configurationService.getExperimentBasedConfig(
			ConfigKey.TypeScriptLanguageContextMode,
			this.experimentationService,
		);
	...
	private getIncludeDocumentation(): boolean {
		return this.configurationService.getExperimentBasedConfig<boolean>(
			ConfigKey.TypeScriptLanguageContextIncludeDocumentation,
			this.experimentationService,
		);
	}

	private getCharacterBudget(
		context: RequestContext,
		document: vscode.TextDocument,
	): CharacterBudget {
		...
		switch (this.usageMode) {
			...
			case ContextItemUsageMode.double:
				return new CharacterBudget(
					chars,
					Math.min(chars, document.getText().length),
				);
...
class CachePopulationTrigger implements vscode.Disposable {
	...
	constructor(
		languageContextService: ILanguageContextService,
		tokenBudgetProvider: TokenBudgetProvider,
	) {
		...
		this.selectionChangeDebouncer = this.disposables.add(
			new ThrottledDebouncer(),
		);
		this.disposables.add(
	...
	private didChangeActiveTextEditor(
		editor: vscode.TextEditor | undefined,
	): void {
	...
	private didChangeTextEditorSelection(
	...
	private getPosition(
		...
		if (
			this.lastDocumentChange !== undefined &&
			this.lastDocumentChange.document === document.uri.toString() &&
			time - this.lastDocumentChange.time < 500
		) {
			return undefined;
		}
		return range.start;
	...
	private populateCache(
		...
		if (check) {
			...
			if (
				!selection.isEmpty ||
				selection.start.line !== position.line ||
		...
		this.languageContextService
			.populateCache(document, position, context)
			.catch(() => {
...
export class InlineCompletionContribution
	implements vscode.Disposable, TokenBudgetProvider
{
	private readonly disposables: DisposableStore;
		...
		if (languageContextService instanceof LanguageContextServiceImpl) {
			this.disposables.add(
				vscode.commands.registerCommand(
					...
					async () => {
						await vscode.commands.executeCommand(
							'setContext',
							showContextInspectorViewContextKey,
							true,
						);
						await vscode.commands.executeCommand(
							'context-inspector.focus',
						);
					},
				),
			);
			this.disposables.add(
				vscode.window.registerTreeDataProvider(
					'context-inspector',
					new InspectorDataProvider(languageContextService),
				),
			);
		}
	...
	private typeScriptFileOpen(): void {
		...
		this.disposables.add(
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(
						ConfigKey.TypeScriptLanguageContext.fullyQualifiedId,
					)
				) {
					this.checkRegistration();
				}
	...
	private checkRegistration(): void {
		this.registrationQueue
			...
			.catch((error) =>
				this.logService.error(
					error,
					'Error checking TypeScript context provider registration',
				),
			);
	}
	...
	private async register(): Promise<void> {
		...
		try {
			...
			this.registrations.add(
				new CachePopulationTrigger(this.languageContextService, this),
			);
			...
			const resolver: Copilot.ContextResolver<Copilot.SupportedContextItem> =
				{
					resolve(
						...
						| AsyncIterable<Copilot.SupportedContextItem> {
						// console.log(`Resolve request ${Date.now()}`);
						const isSpeculativeRequest =
							request.documentContext.proposedEdits !== undefined;
						...
						if (tokenBudget <= 0) {
							telemetrySender.sendRequestTelemetry(
								document,
								position,
								{
								...
								ContextItemSummary.DefaultExhausted,
								0,
								undefined,
								undefined,
							);
						...
						const context: RequestContext = {
							...
							source:
								request.source === 'nes'
									? KnownSources.nes
									: KnownSources.completion,
							trigger: TriggerKind.completion,
							proposedEdits: isSpeculativeRequest
								? []
								: undefined,
							sampleTelemetry: self.getSampleTelemetry(
								request.activeExperiments,
							),
						};
						const items = languageContextService.getContext(
							document,
							position,
							context,
							token,
						...
						if (Array.isArray(items)) {
							...
							typeof (items as AsyncIterable<ContextItem>)[
								Symbol.asyncIterator
							] === 'function'
						) {
							return mapAsyncIterable(
								items as AsyncIterable<ContextItem>,
			...
			if (
				typeof languageContextService.getContextOnTimeout === 'function'
			) {
				resolver.resolveOnTimeout = (request) => {
					if (
						typeof languageContextService.getContextOnTimeout !==
						'function'
					) {
						return;
					}
					const [document, position] =
						self.getDocumentAndPosition(request);
					if (document === undefined || position === undefined) {
						return;
					}
					const context: RequestContext = {
					...
					const items = languageContextService.getContextOnTimeout(
						document,
						position,
						context,
					);
			...
			if (copilotAPI !== undefined) {
				this.registrations.add(
					copilotAPI.registerContextProvider(provider),
				);
			}
			...
			this.registrations.add(
				this.languageContextProviderService.registerContextProvider(
					provider,
					[ProviderTarget.Completions, ProviderTarget.NES],
				),
			);
			this.telemetrySender.sendInlineCompletionProviderTelemetry(
				KnownSources.completion,
				true,
			);
			logService.info(
				'Registered TypeScript context provider with Copilot inline completions.',
			);
		} catch (error) {
			logService.error(
				'Error checking if server plugin is installed:',
				error,
			);
		}
	}
	...
	private async isTypeScriptRunning(): Promise<boolean> {
		...
		if (typeScriptExtension === undefined) {
			this.telemetrySender.sendActivationFailedTelemetry(
				ErrorLocation.Client,
				ErrorPart.TypescriptPlugin,
				'TypeScript extension not found',
				undefined,
			);
		...
		try {
			...
			if (error instanceof Error) {
				...
				this.logService.error(
					'Error checking if TypeScript plugin is installed:',
					error.message,
				);
			} else {
				this.telemetrySender.sendActivationFailedTelemetry(
				...
				this.logService.error(
					'Error checking if TypeScript plugin is installed: Unknown error',
				);
			}
			return false;
		}
		...
		if (document.version > request.documentContext.version) {
			if (!token?.isCancellationRequested) {
				this.telemetrySender.sendIntegrationTelemetry(
					request.completionId,
					request.documentContext.uri,
					`Version mismatch: ${document.version} !== ${request.documentContext.version}`,
				);
			}
			return [undefined, undefined];
		}
	...
	private convertItem(
		item: ContextItem,
	): Copilot.SupportedContextItem | undefined {
		if (item.kind === ContextKind.Snippet) {
			...
			if (item.additionalUris !== undefined) {
				converted.additionalUris = item.additionalUris.map((uri) =>
					uri.toString(),
				);
			}
			return converted;
	...
	private getConfig(): 'off' | 'on' {
		const expFlag = this.configurationService.getExperimentBasedConfig(
	...
	public getTokenBudget(document: vscode.TextDocument): number {
		return Math.trunc(
			currentTokenBudget - document.getText().length / 4 - 256,
		);
	}

	private getSampleTelemetry(
