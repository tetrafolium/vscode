...
import {
	FunctionLikeContextProvider,
	FunctionLikeContextRunnable,
...
abstract class ClassPropertyBlueprintSearch<
	T extends tt.MethodDeclaration | tt.ConstructorDeclaration,
> extends Search<tt.ClassDeclaration> {
	...
	public isSame(other: T): boolean {
		return (
			this.declaration === other ||
			(this.declaration.getSourceFile().fileName ===
				other.getSourceFile().fileName &&
				this.declaration.pos === other.pos)
		);
	}

	public score(program: tt.Program, context: RequestContext): number {
		if (
			program.getSourceFile(this.declaration.getSourceFile().fileName) ===
			undefined
		) {
			return 0;
		}
...
abstract class MethodBlueprintSearch extends ClassPropertyBlueprintSearch<tt.MethodDeclaration> {
	constructor(
		program: tt.Program,
		symbols: Symbols,
		declaration: tt.MethodDeclaration,
		stateProvider: StateProvider,
	) {
	...
	public static create(
		...
		declaration: tt.MethodDeclaration,
		stateProvider: StateProvider,
	): ClassPropertyBlueprintSearch<tt.MethodDeclaration> | undefined {
		...
		if (!isPrivate) {
			...
			const name = ts.escapeLeadingUnderscores(
			...
			for (const [
				typeSymbol,
				superTypeSymbol,
			] of symbols.getAllSuperTypesWithPath(symbol)) {
				...
				if (method !== undefined) {
					if (
						Symbols.isInterface(superTypeSymbol) ||
						Symbols.isTypeLiteral(superTypeSymbol)
					) {
		...
		if (isPrivate) {
			const extendsClause =
				ClassDeclarations.getExtendsClause(classDeclaration);
			if (
				extendsClause === undefined ||
				extendsClause.types.length === 0
			) {
				return undefined;
			} else {
				const extendsSymbol = symbols.getLeafSymbolAtLocation(
					extendsClause.types[0].expression,
				);
				if (
					extendsSymbol === undefined ||
					!Symbols.isClass(extendsSymbol)
				) {
					return undefined;
				}
				return new PrivateMethodBlueprintSearch(
					program,
					symbols,
					classDeclaration,
					extendsSymbol,
			...
			if (classToCheck !== undefined) {
				return new FindMethodInSubclassSearch(
					program,
					symbols,
					classDeclaration,
					declaration,
				...
				return new FindMethodInHierarchySearch(
					program,
					symbols,
					classDeclaration,
					declaration,
					filteredTypesToCheck,
					stateProvider,
				);
			} else {
				return undefined;
...
abstract class FindInSiblingClassSearch<
	T extends tt.MethodDeclaration | tt.ConstructorDeclaration,
> extends ClassPropertyBlueprintSearch<T> {
	...
	constructor(
		program: tt.Program,
		symbols: Symbols,
		search: FindInSiblingClassSearch<T>,
	);
	constructor(
		program: tt.Program,
		symbols: Symbols,
		classDeclaration: tt.ClassDeclaration,
		extendsSymbol: tt.Symbol,
		declaration: T,
		stateProvider: StateProvider,
	);
	constructor(
		program: tt.Program,
		symbols: Symbols,
		classDeclarationOrSearch:
			| tt.ClassDeclaration
			| FindInSiblingClassSearch<T>,
		extendsSymbol?: tt.Symbol,
		declaration?: T,
		stateProvider?: StateProvider,
	) {
		if (classDeclarationOrSearch instanceof FindInSiblingClassSearch) {
			const search =
				classDeclarationOrSearch as FindInSiblingClassSearch<T>;
			const methodDeclaration = Search.getNodeInProgram(
				program,
				search.declaration,
			);
			...
			for (const declaration of declarations) {
				if (ts.isClassDeclaration(declaration)) {
					const heritageClause =
						ClassDeclarations.getExtendsClause(declaration);
					if (
					...
					extendsSymbol = this.symbols.getLeafSymbolAtLocation(
	...
	public run(
		...
		for (const subType of this.symbols.getDirectSubTypes(
			this.extendsSymbol,
...
class PrivateMethodBlueprintSearch extends FindInSiblingClassSearch<tt.MethodDeclaration> {
	constructor(
		program: tt.Program,
		symbols: Symbols,
		search: PrivateMethodBlueprintSearch,
	);
	constructor(
		program: tt.Program,
		symbols: Symbols,
...
class FindMethodInSubclassSearch extends MethodBlueprintSearch {
	...
	constructor(
		program: tt.Program,
		symbols: Symbols,
		search: FindMethodInSubclassSearch,
	);
	constructor(
		program: tt.Program,
		symbols: Symbols,
	...
	constructor(
		...
		if (classDeclarationOrSearch instanceof FindMethodInSubclassSearch) {
			const search =
				classDeclarationOrSearch as FindMethodInSubclassSearch;
			const declaration = Search.getNodeInProgram(
				program,
				search.declaration,
			);
			...
			if (declarations === undefined || declarations.length === 0) {
				throw new RecoverableError(
					'No declarations found for start class',
					RecoverableError.NoDeclaration,
				);
			}
			...
			for (const declaration of declarations) {
				...
				symbol = this.symbols.getLeafSymbolAtLocation(
					declaration.name ? declaration.name : declaration,
				);
				if (symbol !== undefined) {
					break;
				}
			...
			if (symbol === undefined) {
				throw new RecoverableError(
					'No symbol found for start class',
					RecoverableError.SymbolNotFound,
				);
			}
	...
	public run(
		...
		const callableName = ts.escapeLeadingUnderscores(
			this.declaration.name.getText(),
		);
		for (const subType of this.symbols.getAllSubTypes(
			this.startClass,
...
class FindMethodInHierarchySearch extends MethodBlueprintSearch {
	...
	constructor(
		program: tt.Program,
		symbols: Symbols,
		search: FindMethodInHierarchySearch,
	);
	constructor(
		program: tt.Program,
		symbols: Symbols,
	...
	constructor(
		...
		if (classDeclarationOrSearch instanceof FindMethodInHierarchySearch) {
			const search =
				classDeclarationOrSearch as FindMethodInHierarchySearch;
			const declaration = Search.getNodeInProgram(
				program,
				search.declaration,
			);
			...
			for (const symbolToCheck of search.typesToCheck) {
				...
				for (const declaration of declarations) {
					...
					if (
						symbol !== undefined &&
						symbol.flags === symbolToCheck.flags
					) {
						break;
					}
				}
			...
			for (const subType of this.symbols.getDirectSubTypes(
				...
				if (Symbols.isClass(subType)) {
					...
					if (member !== undefined && !Symbols.isAbstract(member)) {
						const declaration =
							ClassDeclarations.fromSymbol(subType);
...
abstract class SimilarPropertyRunnable<
	T extends tt.MethodDeclaration | tt.ConstructorDeclaration,
> extends FunctionLikeContextRunnable<T> {
	constructor(
	...
	protected override createRunnableResult(
		...
		const cacheInfo: CacheInfo | undefined =
			scope !== undefined
				? { emitMode: EmitMode.ClientBased, scope }
				: undefined;
	...
	protected override run(
		...
		if (search !== undefined) {
			const [program, candidate] = this.session.run(
				search,
				this.context,
				token,
			);
			if (program !== undefined && candidate !== undefined) {
				...
				const snippetBuilder = new CodeSnippetBuilder(
					this.context,
					this.context.getSymbols(program),
					sourceFile,
				);
	...
	protected abstract createSearch(
		token: tt.CancellationToken,
	): Search<tt.ClassDeclaration> | undefined;
}
...
class SimilarMethodRunnable extends SimilarPropertyRunnable<tt.MethodDeclaration> {
	...
	protected override createSearch(): Search<tt.ClassDeclaration> | undefined {
		return MethodBlueprintSearch.create(
			this.getProgram(),
			this.symbols,
			this.declaration,
			this.session,
...
abstract class ClassPropertyContextProvider<
	T extends
		| tt.MethodDeclaration
		| tt.ConstructorDeclaration
		| tt.GetAccessorDeclaration
		| tt.SetAccessorDeclaration,
> extends FunctionLikeContextProvider {
...
class PropertiesTypeRunnable extends AbstractContextRunnable {
	...
	constructor(
		...
		declaration:
			...
			| tt.GetAccessorDeclaration
			| tt.SetAccessorDeclaration,
		priority: number = Priorities.Properties,
	) {
		super(
			session,
			languageService,
			context,
			'PropertiesTypeRunnable',
	...
	protected override createRunnableResult(
		...
		const cacheInfo: CacheInfo | undefined = {
			...
			scope: this.createCacheScope(this.declaration),
		};
		return result.createRunnableResult(
			this.id,
			this.priority,
	...
	protected override run(
		...
		const containerSymbol = symbols.getLeafSymbolAtLocation(
			containerDeclaration.name
				? containerDeclaration.name
				: containerDeclaration,
		);
		if (
			containerSymbol === undefined ||
			!Symbols.isClass(containerSymbol)
		) {
			return;
		}
			...
			for (const member of type.members.values()) {
				...
				if (
					!this.handleMember(
						...
						member,
						symbols,
						ts.ModifierFlags.Public | ts.ModifierFlags.Protected,
					)
				) {
					return;
				}
	...
	private handleMember(
		...
		for (const [typeSymbol, name] of this.getEmitMemberData(
			symbol,
			symbols,
		)) {
			...
			continueResult =
				continueResult && this.handleSymbol(typeSymbol, name, true);
			if (!continueResult) {
				break;
			}
		}
	...
	private static readonly NoEmitData: readonly [
		tt.Symbol | undefined,
		string | undefined,
	] = Object.freeze<[tt.Symbol | undefined, string | undefined]>([
		undefined,
		undefined,
	]);
	private *getEmitMemberData(
		symbol: tt.Symbol,
		symbols: Symbols,
	): IterableIterator<readonly [tt.Symbol | undefined, string | undefined]> {
		if (Symbols.isProperty(symbol)) {
			...
			const declaration = Symbols.getDeclaration<tt.PropertyDeclaration>(
				symbol,
...
export class AccessorProvider extends ClassPropertyContextProvider<
	tt.GetAccessorDeclaration | tt.SetAccessorDeclaration
> {
	constructor(
		declaration: tt.GetAccessorDeclaration | tt.SetAccessorDeclaration,
		tokenInfo: TokenInfo,
		computeContext: ProviderComputeContext,
	) {
		super(declaration, tokenInfo, computeContext);
...
class ConstructorBlueprintSearch extends FindInSiblingClassSearch<tt.ConstructorDeclaration> {
	constructor(
		program: tt.Program,
		symbols: Symbols,
		search: ConstructorBlueprintSearch,
	);
	constructor(
		program: tt.Program,
		symbols: Symbols,
		classDeclaration: tt.ClassDeclaration,
		extendsSymbol: tt.Symbol,
		declaration: tt.ConstructorDeclaration,
		stateProvider: StateProvider,
	);
	constructor(
		program: tt.Program,
	...
	public with(program: tt.Program): ConstructorBlueprintSearch {
		...
		return new ConstructorBlueprintSearch(
			program,
			new Symbols(program),
			this,
		);
...
class SimilarConstructorRunnable extends SimilarPropertyRunnable<tt.ConstructorDeclaration> {
	...
	protected override createSearch(): Search<tt.ClassDeclaration> | undefined {
		...
		if (extendsClause === undefined || extendsClause.types.length === 0) {
			...
			const extendsSymbol = this.symbols.getLeafSymbolAtLocation(
				extendsClause.types[0].expression,
			);
			if (
				extendsSymbol === undefined ||
...
export class ConstructorContextProvider extends ClassPropertyContextProvider<tt.ConstructorDeclaration> {
	...
	public override provide(
		...
		if (session.enableBlueprintSearch()) {
			result.addPrimary(
				new SimilarConstructorRunnable(
					session,
					languageService,
					context,
					this.declaration,
