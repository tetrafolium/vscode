...
export class CompilerOptionsRunnable extends AbstractContextRunnable {
	public static VersionTraitKey: string = Trait.createContextItemKey(
		TraitKind.Version,
	);
	...
	public static traitsToCollect: [
		...
		[
			TraitKind.Module,
			'The TypeScript module system used in this project is ',
			Trait.createContextItemKey(TraitKind.Module),
			ts.ModuleKind,
		],
		[
			TraitKind.ModuleResolution,
			'The TypeScript module resolution strategy used in this project is ',
			Trait.createContextItemKey(TraitKind.ModuleResolution),
			ts.ModuleResolutionKind,
		],
		[
			TraitKind.Target,
			'The target version of JavaScript for this project is ',
			Trait.createContextItemKey(TraitKind.Target),
			ts.ScriptTarget,
		],
		[
			TraitKind.Lib,
			'Library files that should be included in TypeScript compilation are ',
			Trait.createContextItemKey(TraitKind.Lib),
			undefined,
		],
	];
	...
	protected override run(
		...
		if (
			!result.addFromKnownItems(CompilerOptionsRunnable.VersionTraitKey)
		) {
			result.addTrait(
				TraitKind.Version,
				'The TypeScript version used in this project is ',
				ts.version,
			);
		}
		for (const [
			...
			key,
			enumType,
		] of CompilerOptionsRunnable.traitsToCollect) {
			if (result.addFromKnownItems(key)) {
				continue;
			}
			let traitValue =
				compilerOptions[traitKind as keyof tt.CompilerOptions];
			if (traitValue) {
				if (typeof traitValue === 'number') {
					const enumName = CompilerOptionsRunnable.getEnumName(
...
export abstract class FunctionLikeContextRunnable<
	T extends tt.FunctionLikeDeclarationBase = tt.FunctionLikeDeclarationBase,
> extends AbstractContextRunnable {
...
export class SignatureRunnable extends FunctionLikeContextRunnable {
	constructor(
		...
		priority: number = Priorities.Locals,
	) {
		super(
			session,
			languageService,
			context,
			SignatureRunnable.computeId(session, declaration),
			declaration,
			priority,
			ComputeCost.Low,
		);
	...
	protected override createRunnableResult(
		...
		const cacheInfo: CacheInfo | undefined =
			scope !== undefined
				...
				: undefined;
		const runnableResult = result.createRunnableResult(
			this.id,
			this.priority,
			SpeculativeKind.emit,
			cacheInfo,
	...
	private getDebugPath(): string | undefined {
		...
		const { sourceFile, startPos, endPos } =
			SignatureRunnable.getSourceFileAndPositions(this.declaration);
	...
	private static computeId(
		...
		const { sourceFile, startPos, endPos } =
			SignatureRunnable.getSourceFileAndPositions(declaration);
	...
	private static getSourceFileAndPositions(
		declaration: tt.FunctionLikeDeclarationBase,
	): { sourceFile: tt.SourceFile; startPos: number; endPos: number } {
...
export class TypeOfLocalsRunnable extends AbstractContextRunnable {
	...
	protected override createRunnableResult(
		...
		const cacheInfo: CacheInfo | undefined =
			this.cacheScope !== undefined
				? {
						emitMode: EmitMode.ClientBasedOnTimeout,
						scope: this.cacheScope,
					}
				: undefined;
		this.runnableResult = result.createRunnableResult(
			this.id,
			this.priority,
			SpeculativeKind.emit,
			cacheInfo,
	...
	protected override run(
		...
		const token =
			this.tokenInfo.previous ??
			this.tokenInfo.token ??
			this.tokenInfo.touching;
		...
		let variableDeclarations: Set<tt.VariableDeclarationList> | undefined =
			this.cacheScope === undefined ? new Set() : undefined;
		...
		for (const symbol of inScope) {
			...
			const declaration: tt.VariableDeclaration | undefined =
				Symbols.getDeclaration(
					symbol,
			...
			if (variableDeclarations !== undefined) {
				variableDeclarations = this.addScopeNode(
		...
		if (
			...
			variableDeclarations.size > 0 &&
			this.runnableResult !== undefined
		) {
			this.runnableResult.setCacheInfo({
				emitMode: EmitMode.ClientBasedOnTimeout,
				scope: CacheScopes.createOutsideCacheScope(
					variableDeclarations,
					sourceFile,
				),
			});
...
export class TypesOfNeighborFilesRunnable extends AbstractContextRunnable {
	...
	private static SymbolsToInclude: number =
	...
	constructor(
		...
		context: RequestContext,
		tokenInfo: tss.TokenInfo,
		priority: number = Priorities.NeighborFiles,
	) {
		super(
			session,
			languageService,
	...
	protected override createRunnableResult(
		...
		const cacheInfo: CacheInfo = {
			emitMode: EmitMode.ClientBased,
			scope: { kind: CacheScopeKind.NeighborFiles },
		};
		return result.createRunnableResult(
			this.id,
			this.priority,
	...
	protected override run(
		...
		for (const neighborFile of this.context.neighborFiles) {
			...
			const neighborSourceFile =
				this.getProgram().getSourceFile(neighborFile);
			if (
				neighborSourceFile === undefined ||
				this.skipSourceFile(neighborSourceFile)
			) {
				continue;
			}
			const sourceFileSymbol =
				symbols.getLeafSymbolAtLocation(neighborSourceFile);
			...
			if (sourceFileSymbol.exports !== undefined) {
				for (const member of sourceFileSymbol.exports) {
					...
					if (
						(memberSymbol.flags &
							TypesOfNeighborFilesRunnable.SymbolsToInclude) ===
						0
					) {
						continue;
					}
...
type ImportBlock = {
	before: tt.Node | undefined;
	imports: tt.ImportDeclaration[];
	after: tt.Node | undefined;
};
export class ImportsRunnable extends AbstractContextRunnable {
	...
	constructor(
		...
		tokenInfo: tss.TokenInfo,
		excludes: Set<tt.Symbol>,
		priority: number = Priorities.Imports,
	) {
		super(
			session,
			languageService,
		...
		this.cacheInfo =
			scopeNode === undefined
				...
				: {
						emitMode: EmitMode.ClientBased,
						scope: this.createCacheScope(scopeNode),
					};
	}
	...
	public override useCachedResult(
		...
		if (
			cacheInfo.emitMode === EmitMode.ClientBased &&
			cached.state === ContextRunnableState.Finished
		) {
	...
	protected override run(
			...
			if (this.cacheInfo === undefined) {
				...
				const start =
					block.before !== undefined
						? CacheScopes.createRange(block.before, sourceFile).end
						: CacheScopes.createRange(block.imports[0], sourceFile)
								.start;
				const end =
					block.after !== undefined
						? CacheScopes.createRange(block.after, sourceFile).start
						: CacheScopes.createRange(
								block.imports[block.imports.length - 1],
								sourceFile,
							).end;
		...
		if (
			...
			outSideRanges !== undefined &&
			outSideRanges.length > 0
		) {
			result.setCacheInfo({
				emitMode: EmitMode.ClientBased,
				scope: {
	...
	private getCacheScopeNode(): tt.Node | undefined {
		...
		if (
			current === undefined ||
			current.kind === ts.SyntaxKind.EndOfFileToken ||
			current.kind === ts.SyntaxKind.Unknown
		) {
			return undefined;
		}
...
export class TypeOfExpressionRunnable extends AbstractContextRunnable {
	...
	constructor(
		...
		priority: number = Priorities.Expression,
	) {
		super(
			session,
			languageService,
	...
	public static create(
		...
		tokenInfo: tss.TokenInfo,
		_token: tt.CancellationToken,
	): TypeOfExpressionRunnable | undefined {
		const previous = tokenInfo.previous;
		if (previous === undefined || previous.parent === undefined) {
			return;
		}
		if (
			(ts.isIdentifier(previous) ||
				previous.kind === ts.SyntaxKind.DotToken) &&
			ts.isPropertyAccessExpression(previous.parent)
		) {
			const identifier = this.getRightMostIdentifier(
				previous.parent.expression,
				0,
			);
			if (identifier !== undefined) {
				return new TypeOfExpressionRunnable(
					session,
					languageService,
					context,
					identifier,
	...
	private static getRightMostIdentifier(
		node: tt.Node,
		count: number,
	): tt.Identifier | undefined {
		if (count === 32) {
			return undefined;
		}
		switch (node.kind) {
			...
			case ts.SyntaxKind.PropertyAccessExpression:
				return this.getRightMostIdentifier(
					(node as tt.PropertyAccessExpression).name,
					count + 1,
				);
			case ts.SyntaxKind.ElementAccessExpression:
				return this.getRightMostIdentifier(
					(node as tt.ElementAccessExpression).argumentExpression,
					count + 1,
				);
			case ts.SyntaxKind.CallExpression:
				return this.getRightMostIdentifier(
					(node as tt.CallExpression).expression,
					count + 1,
				);
			default:
				return undefined;
...
export abstract class FunctionLikeContextProvider extends ContextProvider {
	...
	public override provide(
		...
		token.throwIfCancellationRequested();
		result.addPrimary(
			new SignatureRunnable(
				session,
				languageService,
				context,
				this.functionLikeDeclaration,
			),
		);
		...
		result.addPrimary(
			new TypeOfLocalsRunnable(
				...
				this.tokenInfo,
				excludes,
				CacheScopes.fromDeclaration(this.functionLikeDeclaration),
			),
		);
	...
	protected abstract getTypeExcludes(
		languageService: tt.LanguageService,
		context: RequestContext,
	): Set<tt.Symbol>;
}
