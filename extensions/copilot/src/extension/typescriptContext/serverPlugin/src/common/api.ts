...
} from './methodContextProvider';
import { ModuleContextProvider } from './moduleContextProvider';
import {
	validateNesRename,
	type PrepareNesRenameResult,
} from './nesRenameValidator';
import {
	RenameKind,
	type FilePath,
	type Range,
...
class ProviderComputeContextImpl implements ProviderComputeContext {
	...
	public update(contextProvider: ContextProvider): ContextProvider {
		if (
			this.firstCallableProvider === undefined &&
			contextProvider.isCallableProvider !== undefined &&
			contextProvider.isCallableProvider === true
		) {
...
class ContextProviders {
	...
	>([
		[
			...
			(_node, tokenInfo, computeContext) =>
				new SourceFileContextProvider(tokenInfo, computeContext),
		],
		[
			ts.SyntaxKind.FunctionDeclaration,
			(node, tokenInfo, computeContext) =>
				new FunctionContextProvider(
					node as tt.FunctionDeclaration,
					tokenInfo,
					computeContext,
				),
		],
		...
		[
			...
			(node, tokenInfo, computeContext) =>
				new AccessorProvider(
					node as tt.GetAccessorDeclaration,
					tokenInfo,
					computeContext,
				),
		],
		[
			...
			(node, tokenInfo, computeContext) =>
				new AccessorProvider(
					node as tt.SetAccessorDeclaration,
					tokenInfo,
					computeContext,
				),
		],
		[
			...
			ClassContextProvider.create as unknown as ContextProviderFactory,
		],
		[
			ts.SyntaxKind.Constructor,
			(node, tokenInfo, computeContext) =>
				new ConstructorContextProvider(
					node as tt.ConstructorDeclaration,
					tokenInfo,
					computeContext,
				),
		],
		[
			...
			(node, tokenInfo, computeContext) =>
				new MethodContextProvider(
					node as tt.MethodDeclaration,
					tokenInfo,
					computeContext,
				),
		],
	...
	public execute(
		...
		const collector = this.getContextRunnables(
			session,
			languageService,
			result.context,
			token,
		...
		result.addPath(
			tss.StableSyntaxKinds.getPath(
				this.tokenInfo.touching ?? this.tokenInfo.token,
			),
		);
	...
	private executeRunnables(
		runnables: ContextRunnable[],
		result: ContextResult,
		token: tt.CancellationToken,
	): void {
	...
	private getContextRunnables(
		session: ComputeContextSession,
		languageService: tt.LanguageService,
		context: RequestContext,
		token: tt.CancellationToken,
	): ContextRunnableCollector {
		const result: ContextRunnableCollector = new ContextRunnableCollector(
			context.clientSideRunnableResults,
		);
		result.addPrimary(
			new CompilerOptionsRunnable(
				session,
				languageService,
				context,
				this.tokenInfo.token.getSourceFile(),
			),
		);
	...
	private computeProviders(): ContextProvider[] {
		...
		if (token === undefined) {
			if (
				this.tokenInfo.token === undefined ||
				this.tokenInfo.token.kind === ts.SyntaxKind.EndOfFileToken
			) {
		...
		if (
			token === undefined ||
			token.kind === ts.SyntaxKind.EndOfFileToken
		) {
			return result;
		}
		let current = token;
...
export function computeContext(
	...
	if (program === undefined) {
		result.addErrorData(
			new RecoverableError(
				`No program found on language service`,
	...
	if (sourceFile === undefined) {
		result.addErrorData(
			new RecoverableError(
				`No source file found for document`,
				RecoverableError.NoSourceFile,
			),
		);
		return;
	}
...
export function prepareNesRename(
	...
	lastSymbolRename: Range | undefined,
	token: tt.CancellationToken,
): void {
	...
	if (renameInfo.displayName !== oldName) {
		result.setCanRename(
			RenameKind.no,
			`Old name '${oldName}' does not match symbol name '${renameInfo.displayName}'`,
		);
		return;
	}
	doPrepareNesRename(
...
function runPrepareNesRenameOnOldState(
	result: PrepareNesRenameResult,
	session: ComputeContextSession,
	languageService: tt.LanguageService,
	sourceFile: tt.SourceFile,
	...
	const [oldText, oldPos] = getOldText(
		sourceFile,
		position,
		oldName,
		newName,
	...
	tss.LanguageServiceHost.runWithTemporaryFileUpdate(
		session.languageServiceHost,
		sourceFile.fileName,
		oldText,
		(updatedProgram, _originalProgram, updatedSourceFile) => {
			const renameInfo = languageService.getRenameInfo(
				updatedSourceFile.fileName,
				oldPos,
				{},
			);
			if (!renameInfo.canRename) {
			...
			if (
				result.getCanRename() === RenameKind.maybe ||
				result.getCanRename() === RenameKind.yes
			) {
	...
	tss.LanguageServiceHost.runWithTemporaryFileUpdate(
		...
		(updatedProgram, _originalProgram, updatedSourceFile) => {
			...
			for (const loc of renameLocations) {
				...
				const start = sf.getLineAndCharacterOfPosition(
					loc.textSpan.start,
				);
				const end = sf.getLineAndCharacterOfPosition(
					loc.textSpan.start + loc.textSpan.length,
				);
				if (
					loc.fileName === document &&
					start.line === lastSymbolRename.start.line &&
					start.character === lastSymbolRename.start.character &&
					end.line === lastSymbolRename.end.line &&
					end.character === lastSymbolRename.end.character - delta
				) {
					continue;
				}
				group.changes.push({
...
function getOldText(
	...
	const startPos = sourceFile.getPositionOfLineAndCharacter(
		lastSymbolRename.start.line,
		lastSymbolRename.start.character,
	);
	const endPos = sourceFile.getPositionOfLineAndCharacter(
		lastSymbolRename.end.line,
		lastSymbolRename.end.character,
	);
	return [
		text.substring(0, startPos) + oldName + text.substring(endPos),
		position < startPos
			? position
			: position - (newName.length - oldName.length),
	];
}
