...
suite('doReview', () => {
	describe('handleReviewResult', () => {
		...
		function createMockDeps(): MockDeps {
			...
			return {
				get infoMessages() {
					return tracker.infoMessages;
				},
				get logShown() {
					return tracker.logShown;
				},
				get addedComments() {
					return tracker.addedComments;
				},
				get buttonToReturn() {
					return tracker.buttonToReturn;
				},
				set buttonToReturn(value: string | undefined) {
					tracker.buttonToReturn = value;
				},
				notificationService: {
				...
				logService: {
					show: () => {
						tracker.logShown = true;
					},
				} as unknown as ILogService,
				reviewService: {
					addReviewComments: (comments: ReviewComment[]) => {
						tracker.addedComments.push(...comments);
					},
		...
		test('shows excluded comments message when no comments but excluded exist', async () => {
			...
			assert.strictEqual(
				deps.infoMessages[0].message,
				'Reviewing your code did not provide any feedback.',
			);
	...
	describe('getReviewTitle', () => {
		...
		test('returns title for file group with index', () => {
			...
			assert.strictEqual(
				title,
				'Reviewing staged changes in component.tsx...',
			);
		});
		...
		test('returns title for file group with workingTree', () => {
			...
			assert.strictEqual(
				title,
				'Reviewing unstaged changes in utils.js...',
			);
		});
	});
	...
	describe('ReviewSession', () => {
		...
		class MockAuthService {
			...
			getCopilotToken(): Promise<CopilotToken> {
				...
				return Promise.resolve(
					new CopilotToken(
						createTestExtendedTokenInfo({ token: 'test-token' }),
					),
				);
			}
		}
		...
		class MockNotificationService {
			...
			async showQuotaExceededDialog(_options: {
			...
			showInformationMessage(
				message: string,
				_optionsOrItem?: MessageOptions | string,
				..._items: string[]
			): Promise<string | undefined> {
		...
		class MockScopeSelector implements IScopeSelector {
			...
			async selectEnclosingScope(
				_editor: TextEditor,
				_options?: { reason?: string; includeBlocks?: boolean },
		...
		beforeEach(() => {
			...
			serviceCollection.define(
				IReviewService,
				new SyncDescriptor(MockReviewService),
			);
			serviceCollection.define(
		...
		test('returns undefined when user is not authenticated (isNoAuthUser)', async () => {
			...
			mockAuth.copilotToken = new CopilotToken(
				createTestExtendedTokenInfo({
					token: 'test',
			...
			Object.defineProperty(mockAuth.copilotToken, 'isNoAuthUser', {
				value: true,
			});
			...
			serviceCollection.define(
				...
				mockAuth as unknown as IAuthenticationService,
			);
			serviceCollection.define(
				INotificationService,
				mockNotification as unknown as INotificationService,
			);
		...
		test('returns undefined when selection group but no editor', async () => {
			const mockAuth = new MockAuthService();
			mockAuth.copilotToken = new CopilotToken(
				createTestExtendedTokenInfo({ token: 'test' }),
			);
			...
			serviceCollection.define(
				...
				mockTabs as unknown as ITabsAndEditorsService,
			);
		...
		test('returns undefined when selection group and scopeSelector returns undefined', async () => {
			...
			serviceCollection.define(
				IScopeSelector,
				mockScope as unknown as IScopeSelector,
			);
		...
		test('proceeds with empty selection when scopeSelector throws non-cancellation error (fall-through behavior)', async () => {
			...
			assert.ok(
				result !== undefined,
				'Review should proceed when scopeSelector throws non-cancellation error',
			);
		...
		test('uses legacy review path when code_review_enabled is false', async () => {
			...
			mockAuth.tokenToReturn = new CopilotToken(
				createTestExtendedTokenInfo({
					token: 'test',
