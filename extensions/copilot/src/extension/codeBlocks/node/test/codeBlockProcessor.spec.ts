...
suite('CodeBlockProcessor', () => {
	type ReportedMarkdown = {
		markdown: string;
		codeBlock: CodeBlockInfo | undefined;
		vulnerabilities: ChatVulnerability[] | undefined;
	};

	function newCodeBlockProcessor(
		reportedCodeblocks: CodeBlock[] = [],
		reportedMarkdown: ReportedMarkdown[] = [],
		lineProcessor?: LineProcessor,
	) {
		return new CodeBlockProcessor(
			(path) => URI.file(path),
			(markdown, codeBlockInfo, vulnerabilities) =>
				reportedMarkdown.push({
	...
	test('append multi line text', () => {
		...
		const tracker = newCodeBlockProcessor(
			reportedCodeblocks,
			reportedMarkdown,
		);
		...
		assert.deepEqual(reportedMarkdown, [
			...
			{
				markdown: '```ts\n',
				codeBlock: { index: 0, resource, language },
				vulnerabilities: undefined,
			},
			{
	...
	test('multiple code blocks', () => {
		...
		assert.deepEqual(reportedMarkdown, [
			...
			{
				markdown: '```ts\n',
				codeBlock: { index: 1, resource, language },
				vulnerabilities: undefined,
			},
			{
	...
	test('line handler', () => {
		...
		lines
			.join('')
			.split('')
			.forEach((s) => tracker.processMarkdown(s));
		tracker.flush();
		...
		assert.deepEqual(
			reportedMarkdown.map((m) => m.markdown).join(''),
			expectedLines.join(''),
		);
	});
});
