...
import {
	...
	ChatLocation as DeprecatedChatLocation,
	Position,
	TextSearchMatch2,
...
import {
	...
	MAX_CHUNKS_RESULTS,
} from '../../prompts/node/panel/workspace/workspaceContext';
...
export class SemanticSearchTextSearchProvider
	extends Disposable
	implements vscode.AITextSearchProvider
{
	...
	private resetFeedbackContext() {
		this._commandService.executeCommand(
			'setContext',
			SemanticSearchTextSearchProvider.feedBackSentKey,
			false,
		);
	}

	private getPreviewRange(
		previewText?: string,
		symbolsToHighlight?: TreeSitterExpressionInfo[] | undefined,
	): VSCodeRange {
		if (!previewText) {
		...
		const firstNonWhitespaceIndex =
			strings.firstNonWhitespaceIndex(previewText);
		const startIndex =
			firstNonWhitespaceIndex !== -1 &&
			firstNonWhitespaceIndex !== previewText.length
	...
	): vscode.ProviderResult<vscode.TextSearchComplete2> {
		this.resetFeedbackContext();
		const sw = new StopWatch();
		const getResults = async () => {
			const chatProgress: vscode.Progress<
				ChatResponseReferencePart | vscode.ChatResponseProgressPart
			> = {
				report(_obj) {},
			};
			...
			SemanticSearchTextSearchProvider.feedBackTelemetry.chunkSearchDuration =
				Date.now() - chunkSearchDuration;
			SemanticSearchTextSearchProvider.feedBackTelemetry.chunkCount =
				result.chunks.length;
			this.treeSitterAIKeywords(
				query,
				progress,
				result.chunks.map((chunk) => chunk.chunk),
				token,
			);

			const chunkResults = result.chunks.map((c) => c.chunk);
			const intent = this._intentService.getIntent(
				'searchPanel',
				ChatLocation.Other,
			);
			if (intent) {
				...
				const intentInvocation = await intent.invoke({
				...
				const prompt = await intentInvocation.buildPrompt(
					buildPromptContext,
					progress,
					token,
				);
				...
				const fetchResult =
					await intentInvocation.endpoint.makeChatRequest(
						'searchPanel',
						prompt.messages,
						async (text, _, delta) => {
							return undefined;
				...
				SemanticSearchTextSearchProvider.feedBackTelemetry.llmFilteringDuration =
					Date.now() - llmFilteringDuration;
				searchResult =
					fetchResult.type === 'success'
						? fetchResult.value
						: fetchResult.type === 'length'
							? fetchResult.truncatedValue
							: '';
				SemanticSearchTextSearchProvider.feedBackTelemetry.rankResult =
					fetchResult.type;
			}
			...
			try {
				...
				SemanticSearchTextSearchProvider.feedBackTelemetry.parseResult =
					'success';
			} catch (error) {
				SemanticSearchTextSearchProvider.feedBackTelemetry.parseResult =
					'failed';
			}
			SemanticSearchTextSearchProvider.feedBackTelemetry.rawLlmRankingResultsCount =
				rankingResults.length;

			const combinedRank = combinedRanking(
				[...result.chunks],
				rankingResults,
			);
			SemanticSearchTextSearchProvider.feedBackTelemetry.rankResultsCount =
				rankingResults.length;
			SemanticSearchTextSearchProvider.feedBackTelemetry.combinedResultsCount =
				combinedRank.length;

			if (rankingResults.length > 0) {
				const rankingInsights = combineRankingInsights(
					[...result.chunks],
					rankingResults,
				);
				SemanticSearchTextSearchProvider.feedBackTelemetry.llmBestRank =
					rankingInsights.llmBestRank;
				SemanticSearchTextSearchProvider.feedBackTelemetry.llmWorstRank =
					rankingInsights.llmWorstRank;
				SemanticSearchTextSearchProvider.feedBackTelemetry.llmSelectedCount =
					combinedRank.filter((chunk) => chunk.llmSelected).length;
			}

			const combinedChunks = combinedRank.map((chunk) => chunk.chunk);
			await this.reportSearchResults(
				rankingResults,
				combinedChunks,
				progress,
				token,
			...
			if (this._rerankerService.isAvailable) {
				try {
					this.workspaceChunkSearch
						...
						.then((rerankResult) => {
							if (rerankResult && rankingResults.length > 0) {
								const rerankInsights = combineRankingInsights(
									[...rerankResult.chunks],
									rankingResults,
								);
								SemanticSearchTextSearchProvider.feedBackTelemetry.llmBestInRerank =
									rerankInsights.llmBestRank;
								SemanticSearchTextSearchProvider.feedBackTelemetry.llmWorstInRerank =
									rerankInsights.llmWorstRank;
							}
					...
					this._logService.error(
						`SemanticSearchTextSearchProvider::provideAITextSearchResults rerank failed. error=${ex}`,
					);
				}
			} else {
				this.reportTelemetry();
	...
	reportTelemetry() {
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent(
			...
			{
				rankResult:
					SemanticSearchTextSearchProvider.feedBackTelemetry
						.rankResult,
				parseResult:
					SemanticSearchTextSearchProvider.feedBackTelemetry
						.parseResult,
		...
		if (
			...
			*/
			...
			if (
				SemanticSearchTextSearchProvider.feedBackTelemetry
					.llmWorstInRerank !== undefined &&
				SemanticSearchTextSearchProvider.feedBackTelemetry
					.llmBestInRerank !== undefined &&
				(SemanticSearchTextSearchProvider.feedBackTelemetry
					.llmWorstInRerank >
					SemanticSearchTextSearchProvider.feedBackTelemetry
						.llmWorstRank ||
				...
				this._telemetryService.sendInternalMSFTTelemetryEvent(
					...
					{
						keyword:
							SemanticSearchTextSearchProvider.latestQuery || '',
					},
					{
						llmBestRank:
							SemanticSearchTextSearchProvider.feedBackTelemetry
	...
	async reportSearchResults(
		rankingResults: IRankResult[],
		combinedChunks: FileChunk[],
		...
		const onResult: vscode.Progress<vscode.TextSearchResult> = {
			report: async (result: vscode.TextSearchMatch) => {
				...
				const resultAST = this._parserService.getTreeSitterAST({
					...
					getText: () => docContainingRef.getText(),
				});
				const symbolsToHighlight = await resultAST?.getSymbols({
					startIndex: docContainingRef.offsetAt(
						result.ranges instanceof Array
							? result.ranges[0].start
							: result.ranges.start,
					),
					endIndex: docContainingRef.offsetAt(
						result.ranges instanceof Array
							? result.ranges[0].end
							: result.ranges.end,
					),
						...
						? result.ranges.map((r) => {
								return {
									sourceRange: new VSCodeRange(
										...
										new Position(
											r.end.line,
											(result.preview.text?.length || 0) +
												r.end.character,
										),
									),
									previewRange: this.getPreviewRange(
										result.preview.text,
										symbolsToHighlight,
									),
								};
						...
						: [
								{
									sourceRange: new VSCodeRange(
										new Position(
											result.ranges.start.line,
											result.ranges.start.character,
										),
										new Position(
											result.ranges.end.line,
											(result.preview.text?.length || 0) +
												result.ranges.end.character,
										),
									),
									previewRange: this.getPreviewRange(
										result.preview.text,
		...
		for (const chunk of combinedChunks.slice(rankingResults.length)) {
			...
			const rangeText = docContainingRef
				.getText()
				.split('\n')
				.slice(chunk.range.startLineNumber, chunk.range.endLineNumber)
				.join('\n');
			const match: vscode.TextSearchMatch2 = new TextSearchMatch2(
				chunk.file,
				[
	...
	async treeSitterAIKeywords(
		...
		const searchKeywordsIntent = this._intentService.getIntent(
			'searchKeywords',
			ChatLocation.Other,
		);
		if (searchKeywordsIntent) {
			...
			const intentInvocation = await searchKeywordsIntent.invoke({
				location: ChatLocation.Other,
				request,
			});
			...
			keywordResult.split('\n').map((entry) => {
				...
				if (trimmedEntry !== '' && !trimmedEntry.startsWith('```')) {
					const cleanedKeyword = this.processKeyword(
						trimmedEntry,
						chunks,
					);
					if (cleanedKeyword) {
...
): { sourceRange: vscode.Range; previewRange: vscode.Range }[] {
...
export async function getSearchResults(
	...
	const getResultsRanges = async () => {
		...
		await Promise.all(
			Object.keys(fileChunks).map(async (filePath) => {
