...
export const IClaudeToolPermissionService =
	createDecorator<IClaudeToolPermissionService>(
...
export class ClaudeToolPermissionService implements IClaudeToolPermissionService {
	...
	private readonly _handlerCache = new Map<
		ClaudeToolNames,
		IClaudeToolPermissionHandler
	>();
	...
	public async canUseTool(
		...
		if (handler?.handle) {
			return handler.handle(
				toolName as ClaudeToolNames,
				input as ClaudeToolInputMap[ClaudeToolNames],
				context,
			);
		}
		...
		if (handler?.canAutoApprove) {
			const canAutoApprove = await handler.canAutoApprove(
				toolName as ClaudeToolNames,
				input as ClaudeToolInputMap[ClaudeToolNames],
				context,
			);
	...
	private _getHandler(
		toolName: ClaudeToolNames,
	): IClaudeToolPermissionHandler | undefined {
		// Check cache first
		...
		const registration = getToolPermissionHandlerRegistry().find((r) =>
			r.toolNames.includes(toolName),
		);
		if (!registration) {
			return undefined;
		}
	...
	private _getDefaultConfirmationParams(
		toolName: string,
		input: Record<string, unknown>,
	): IClaudeToolConfirmationParams {
		return {
