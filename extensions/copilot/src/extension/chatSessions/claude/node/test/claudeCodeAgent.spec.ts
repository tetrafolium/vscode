...
const TEST_FOLDER_INFO: ClaudeFolderInfo = {
	cwd: '/test/project',
...
describe('ClaudeAgentManager', () => {
	...
	beforeEach(() => {
		...
		mockService = accessor.get(
			IClaudeCodeSdkService,
		) as MockClaudeCodeSdkService;
	...
	it('resolves image references as ImageBlockParam content blocks', async () => {
		...
		expect(
			textBlocks.some((b) => b.text === 'What is in this image?'),
		).toBe(true);
	});
	...
	it('normalizes image/jpg to image/jpeg', async () => {
		...
		const imageRef: vscode.ChatPromptReference = {
			id: 'image-1',
			name: 'image-1',
			value: new ChatReferenceBinaryData('image/jpg', () =>
		...
		const blocks = mockService.receivedMessages[0].message
		...
		expect(
			(imageBlock.source as Anthropic.Base64ImageSource).media_type,
		).toBe('image/jpeg');
	});
	...
	it('skips unsupported image MIME types', async () => {
		...
		const imageRef: vscode.ChatPromptReference = {
			id: 'image-1',
			name: 'image-1',
			value: new ChatReferenceBinaryData('image/bmp', () =>
	...
	it('handles mixed image and file references', async () => {
		...
		const imageRef: vscode.ChatPromptReference = {
			...
			name: 'image-1',
			value: new ChatReferenceBinaryData('image/png', () =>
				Promise.resolve(new Uint8Array([0x89])),
			),
		};
		...
		expect(textBlocks.some((b) => b.text.includes(fileUri.fsPath))).toBe(
			true,
		);
...
describe('ClaudeCodeSession', () => {
	...
	it('queues multiple requests and processes them sequentially', async () => {
		...
		const promise1 = session.invoke(
			createMockChatRequest('First'),
			stream1,
			undefined,
			CancellationToken.None,
		);
		const promise2 = session.invoke(
			createMockChatRequest('Second'),
			stream2,
			undefined,
			CancellationToken.None,
		);
	...
	it('handles multiple sessions with different session IDs', async () => {
		...
		await Promise.all([
			session1.invoke(
				createMockChatRequest('Hello from session 1'),
				stream1,
				undefined,
				CancellationToken.None,
			),
			session2.invoke(
				createMockChatRequest('Hello from session 2'),
				stream2,
				undefined,
				CancellationToken.None,
			),
	...
	it('calls setModel when model changes instead of restarting session', async () => {
		const mockServer = createMockLangModelServer();
		const mockService = instantiationService.invokeFunction((accessor) =>
			accessor.get(IClaudeCodeSdkService),
		) as MockClaudeCodeSdkService;
		mockService.queryCallCount = 0;
		...
		sessionStateService.setModelIdForSession(
			'test-session',
			TEST_MODEL_ID_ALT,
		);
		...
		const stream2 = new MockChatResponseStream();
		await session.invoke(
			createMockChatRequest('Hello again'),
			stream2,
			undefined,
			CancellationToken.None,
		);
	...
	it('uses session state model for initial Options when starting a new session', async () => {
		...
		expect(mockService.lastQueryOptions?.model).toBe(
			TEST_MODEL_ID_ALT.toSdkModelId(),
		);
	});
	...
	it('uses session state permission mode for initial Options when starting a new session', async () => {
		...
		expect(mockService.lastQueryOptions?.permissionMode).toBe(
			'bypassPermissions',
		);
	});
	...
	it('calls applyFlagSettings when effort level changes instead of restarting session', async () => {
		...
		// Change effort level
		sessionStateService.setReasoningEffortForSession(
			'test-session',
			'high',
		);
		...
		expect(mockService.lastAppliedFlagSettings).toEqual({
			effortLevel: 'high',
		});
	});
...
describe('ClaudeCodeSession - settings change restart', () => {
	...
	it('restarts session when settings files change between requests', async () => {
		...
		const claudeMdUri = URI.joinPath(
...
describe('ClaudeCodeSession - tools restart', () => {
	...
	it('restarts session when MCP tools change', async () => {
		...
		const mcpTool = {
			name: 'mcp-tool',
			source: new LanguageModelToolMCPSource(
				'test-server',
				'test-server',
				undefined,
			),
