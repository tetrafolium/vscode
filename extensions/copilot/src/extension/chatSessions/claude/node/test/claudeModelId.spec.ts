...
describe('parseClaudeModelId', () => {
	...
	describe('unparseable inputs', () => {
		it('throws for hyphenated non-Claude IDs', () => {
			expect(() => parseClaudeModelId('gpt-4o')).toThrow(
	...
	describe('tryParseClaudeModelId', () => {
		...
		it('returns a result for bare names', () => {
			expect(tryParseClaudeModelId('foo')).toEqual(
				expect.objectContaining({
					name: 'foo',
					version: '',
				}),
		...
		it('returns a result for valid Claude IDs', () => {
			expect(tryParseClaudeModelId('claude-sonnet-4')).toEqual(
				expect.objectContaining({
					name: 'sonnet',
					version: '4',
				}),
	...
	describe('toSdkModelId', () => {
		it('produces dash-separated version for major.minor', () => {
			expect(parseClaudeModelId('claude-opus-4.5').toSdkModelId()).toBe(
				'claude-opus-4-5',
			);
		});

		it('produces single-digit version when no minor', () => {
			expect(parseClaudeModelId('claude-sonnet-4').toSdkModelId()).toBe(
				'claude-sonnet-4',
			);
		});
