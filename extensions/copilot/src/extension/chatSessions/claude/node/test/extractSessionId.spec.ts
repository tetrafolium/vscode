...
} from '../claudeLanguageModelServer';
...
describe('extractSessionId', () => {
	describe('Authorization Bearer header', () => {
		...
		it('returns invalid for wrong Bearer nonce', () => {
			const result = extractSessionId(
				{ authorization: 'Bearer wrong-nonce.session' },
				NONCE,
			);
			assert.deepStrictEqual(result, {
				valid: false,
	...
	describe('missing headers', () => {
		...
		it('returns invalid for non-Bearer Authorization header', () => {
			const result = extractSessionId(
				{ authorization: `Basic ${NONCE}.session` },
				NONCE,
			);
			assert.deepStrictEqual(result, {
				valid: false,
...
describe('filterSupportedBetas', () => {
	it('allows exact match from supported list', () => {
		assert.strictEqual(
			filterSupportedBetas('interleaved-thinking-2025-05-14'),
	...
	it('allows prefix match for context-management', () => {
		assert.strictEqual(
			filterSupportedBetas('context-management-2025-06-27'),
	...
	it('allows prefix match for advanced-tool-use', () => {
		assert.strictEqual(
			filterSupportedBetas('advanced-tool-use-2025-11-20'),
	...
	it('filters out unsupported betas', () => {
		assert.strictEqual(
			filterSupportedBetas('unsupported-beta-123'),
			undefined,
		);
	});
	...
	it('filters a comma-separated list to only supported betas', () => {
		assert.strictEqual(
			filterSupportedBetas(
				'interleaved-thinking-2025-05-14,unsupported-beta,context-management-2025-06-27',
			),
