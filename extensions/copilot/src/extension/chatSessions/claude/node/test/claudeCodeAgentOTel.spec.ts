 ...
 */
function createToolCallSdkService(
	messageFactory: (
		sessionId: string,
	) => AsyncGenerator<
		SDKAssistantMessage | SDKUserMessageType | SDKResultMessage,
		void,
		unknown
	>,
	...
	return {
		...
		async query(options: {
			prompt: AsyncIterable<SDKUserMessageType>;
			options: Options;
		}) {
...
describe('Claude Session OTel Tool Spans', () => {
	...
	it('emits an execute_tool span for a successful tool call', async () => {
		...
		const sdkService = createToolCallSdkService((sid) =>
			(async function* () {
		...
		const localSessionStateService = accessor.get(
			IClaudeSessionStateService,
		);

		commitTestState(localSessionStateService, sessionId);
		const session = store.add(
			localInstantiationService.createInstance(
				ClaudeCodeSession,
				createMockLangModelServer(),
				sessionId,
				true,
			),
		);
		...
		await session.invoke(
			createMockChatRequest('read file'),
			stream,
			undefined,
		...
		expect(toolSpan!.attributes['copilot_chat.chat_session_id']).toBe(
			sessionId,
		);
		...
		expect(toolSpan!.attributes['gen_ai.tool.call.result']).toContain(
			'file contents here',
		);
	});
	...
	it('emits an execute_tool span with ERROR status for a failed tool call', async () => {
				...
				yield makeAssistantMessage(sid, [
					{
						...
						id: 'tu-err',
						name: 'Write',
						input: { file_path: '/readonly.ts', content: 'x' },
					},
				]);
		...
		await session.invoke(
			createMockChatRequest('write file'),
			stream,
			undefined,
			CancellationToken.None,
		);
	...
	it('correctly correlates multiple concurrent tool calls', async () => {
		...
		await session.invoke(
			createMockChatRequest('read and glob'),
			stream,
			undefined,
			CancellationToken.None,
		);
		...
		expect(readSpan!.attributes['gen_ai.tool.call.result']).toContain(
			'read result',
		);
		expect(globSpan!.attributes['gen_ai.tool.call.result']).toContain(
			'glob result',
		);
	...
	it('emits user_message span for user prompts', async () => {
		...
		expect(userMsgSpan!.attributes['copilot_chat.chat_session_id']).toBe(
			sessionId,
		);
	});
	...
	it('records tool_input as TOOL_CALL_ARGUMENTS', async () => {
		...
		await session.invoke(
			createMockChatRequest('run command'),
			stream,
			undefined,
			CancellationToken.None,
		);
