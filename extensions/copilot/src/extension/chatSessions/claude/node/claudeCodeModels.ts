...
export interface IClaudeCodeModels {
	 ...
	 */
	resolveEndpoint(
		requestedModel: ParsedClaudeModelId | string | undefined,
		fallbackModelId: ParsedClaudeModelId | undefined,
...
export const IClaudeCodeModels =
	createServiceIdentifier<IClaudeCodeModels>('IClaudeCodeModels');
...
export class ClaudeCodeModels extends Disposable implements IClaudeCodeModels {
	...
	public registerLanguageModelChatProvider(lm: (typeof vscode)['lm']): void {
		...
		this._register(
			lm.registerLanguageModelChatProvider('claude-code', provider),
		);
	...
	private async _provideLanguageModelChatInfo(): Promise<
		...
		return endpoints.map((endpoint) => {
			...
			return {
				...
				longContextCacheCost:
					endpoint.tokenPricing?.longContext?.cacheReadTokenPrice,
				...
				capabilities: {
					...
					editTools: endpoint.supportedEditTools
	...
	public async resolveReasoningEffort(
		requestedModel: ParsedClaudeModelId | string | undefined,
		requestedReasoningEffort: string | undefined,
	): Promise<EffortLevel | undefined> {
	...
	public async resolveEndpoint(
		...
		if (requestedModel) {
			...
			const mappedModel =
				parsedModel?.toEndpointModelId() ?? requestedModel;
			const exact = endpoints.find(
				(e) => e.family === mappedModel || e.model === mappedModel,
			);
			if (exact) {
				return exact;
			}
		...
		if (fallbackModelId) {
			const fallback = endpoints.find(
				(e) => e.model === fallbackModelId.toEndpointModelId(),
			);
			if (fallback) {
				return fallback;
			}
		...
		const sonnet = endpoints.find(
			(e) => e.family?.includes('sonnet') || e.model.includes('sonnet'),
		);
		if (sonnet) {
			return sonnet;
		}
		...
		// 4. Newest Haiku
		const haiku = endpoints.find(
			(e) => e.family?.includes('haiku') || e.model.includes('haiku'),
		);
		if (haiku) {
			return haiku;
		}
	...
	private async _fetchAvailableEndpoints(): Promise<IChatEndpoint[]> {
		try {
			...
			const claudeEndpoints = endpoints.filter(
				(e) =>
					e.supportsToolCalls &&
					e.showInModelPicker &&
			...
			if (claudeEndpoints.length === 0) {
				this.logService.trace(
					'[ClaudeCodeModels] No Anthropic models with Messages API found',
				);
				return [];
			}
			...
			this.logService.error(
				`[ClaudeCodeModels] Failed to fetch models`,
				ex,
			);
			return [];
		}
...
export function pickReasoningEffort(
	...
	if (
		...
		endpoint.supportsReasoningEffort.length === 0
	) {
		return undefined;
	}
	if (
		requestedReasoningEffort &&
		isEffortLevel(requestedReasoningEffort) &&
		endpoint.supportsReasoningEffort.includes(requestedReasoningEffort)
	) {
		return requestedReasoningEffort;
	}
	if (
		endpoint.supportsReasoningEffort.length === 1 &&
		isEffortLevel(endpoint.supportsReasoningEffort[0])
	) {
