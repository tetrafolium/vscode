...
function getInvocationMessageText(
	part: ChatToolInvocationPart | undefined,
): string {
	if (!part) {
		return '';
	}
	...
	if (msg instanceof MarkdownString) {
		return (msg as any).value ?? '';
	}
	return msg.value ?? '';
}
...
const delegationSummary =
	new (class extends mock<IChatDelegationSummaryService>() {
		override extractPrompt(
			sessionId: string,
			message: string,
		): { prompt: string; reference: ChatPromptReference } | undefined {
			return undefined;
		}
	})();
...
describe('CopilotCLITools', () => {
	...
	describe('isCopilotCliEditToolCall', () => {
		it('detects StrReplaceEditor edit commands (non-view)', () => {
			expect(
				isCopilotCliEditToolCall({
					...
					arguments: { command: 'str_replace', path: '/tmp/a' },
				}),
			).toBe(true);
			expect(
				isCopilotCliEditToolCall({
			...
			expect(
				isCopilotCliEditToolCall({
					toolName: 'str_replace_editor',
					arguments: { command: 'create', path: '/tmp/a' },
				}),
			).toBe(true);
		});
		it('excludes StrReplaceEditor view command', () => {
			expect(
				isCopilotCliEditToolCall({
					toolName: 'str_replace_editor',
					arguments: { command: 'view', path: '/tmp/a' },
				}),
			).toBe(false);
		});
	...
	describe('getAffectedUrisForEditTool', () => {
		it('returns URI for edit tool with path', () => {
			const [uri] = getAffectedUrisForEditTool({
				toolName: 'str_replace_editor',
				arguments: { command: 'str_replace', path: '/tmp/file.txt' },
			});
	...
	describe('stripReminders', () => {
		...
		it('removes pr_metadata tags', () => {
			const input =
				'<pr_metadata uri="u" title="t" description="d" author="a" linkTag="l"/> Body';
		...
		it('removes multiple constructs mixed', () => {
			const input =
				'<reminder>x</reminder>One<current_datetime>y</current_datetime> <pr_metadata uri="u" title="t" description="d" author="a" linkTag="l"/>Two';
	...
	describe('formatModelDetails', () => {
		it('prefers credits over multiplier when credits are available', () => {
			expect(formatModelDetails('GPT 5.4', 2, 5)).toBe(
				'GPT 5.4 \u2022 5 credits',
			);
		});

		it('falls back to multiplier when credits are undefined', () => {
			expect(formatModelDetails('GPT 5.4', 2, undefined)).toBe(
				'GPT 5.4 \u2022 2x',
			);
		});
	...
	describe('formatModelDetailsWithCredits', () => {
		it('formats integer credits as plural', () => {
			expect(formatModelDetailsWithCredits('GPT 5.4', 5)).toBe(
				'GPT 5.4 \u2022 5 credits',
			);
		});

		it('formats exactly 1 credit as singular', () => {
			expect(formatModelDetailsWithCredits('GPT 5.4', 1)).toBe(
				'GPT 5.4 \u2022 1 credit',
			);
		});

		it('formats fractional credits with one decimal place', () => {
			expect(formatModelDetailsWithCredits('GPT 5.4', 16.31565)).toBe(
				'GPT 5.4 \u2022 16.3 credits',
			);
		});
	});
	...
	describe('formatModelDetailsWithMultiplier', () => {
		it('formats with multiplier suffix', () => {
			expect(formatModelDetailsWithMultiplier('GPT 5.4', 2)).toBe(
				'GPT 5.4 \u2022 2x',
			);
		});

		it('formats 0x multiplier for included models', () => {
			expect(formatModelDetailsWithMultiplier('GPT 5.4', 0)).toBe(
				'GPT 5.4 \u2022 0x',
			);
		});
	...
	describe('buildChatHistoryFromEvents', () => {
		it('builds turns with user and assistant messages including PR metadata', () => {
			const events: any[] = [
				...
				},
				{
					...
					data: {
						content:
							'<pr_metadata uri="https://example.com/pr/1" title="Fix&amp;Improve" description="Desc" author="Alice" linkTag="PR#1"/>This is the PR body.',
					},
				},
			];
			const turns = buildChatHistoryFromEvents(
				'',
				undefined,
				events,
				getVSCodeRequestId,
			...
			const parts: any[] =
				responseParts.parts ?? responseParts._parts ?? responseParts;
			// First part should be PR metadata
			const prPart = parts.find(
				(p) => p instanceof ChatResponsePullRequestPart,
			);
			expect(prPart).toBeTruthy();
			const markdownPart = parts.find(
				(p) => p instanceof ChatResponseMarkdownPart,
			);
			...
			if (prPart) {
				...
				expect((prPart as any).command.command).toBe(
					'github.copilot.chat.openPullRequestReroute',
				);
			}
			if (markdownPart) {
				expect(
					(markdownPart as any).value?.value ||
						(markdownPart as any).value,
				).toContain('This is the PR body.');
			}
		});

		it('createCopilotCLIToolInvocation formats str_replace_editor view with range', () => {
			const invocation = createCopilotCLIToolInvocation({
				toolName: 'str_replace_editor',
				toolCallId: 'id3',
				arguments: {
					command: 'view',
			...
			const msg =
				typeof invocation.invocationMessage === 'string'
		...
		it('includes tool invocation parts and thinking progress without duplication', () => {
			const events: any[] = [
				{
					type: 'user.message',
					data: { content: 'Run a command', attachments: [] },
				},
				{
					...
					data: {
						toolName: 'think',
						toolCallId: 'think-1',
						arguments: { thought: 'Considering options' },
					},
				},
				{
		...
		it('renders task_complete summary as markdown in chat history', () => {
			const events: any[] = [
				{
					type: 'user.message',
					data: { content: 'Finish task', attachments: [] },
				},
				{
					type: 'tool.execution_start',
					data: {
						...
						toolCallId: 'tc-1',
						arguments: { summary: 'All tests are passing.' },
					},
				},
				{
					type: 'tool.execution_complete',
			...
			const parts: any[] =
				responseParts.parts ?? responseParts._parts ?? responseParts;
			const markdownParts = parts.filter(
				(p) => p instanceof ChatResponseMarkdownPart,
			);
			expect(markdownParts).toHaveLength(1);
			expect(
				(markdownParts[0] as any).value?.value ||
					(markdownParts[0] as any).value,
			).toContain('All tests are passing.');
		});
		...
		it('preserves response details on the final rebuilt response turn', () => {
			...
			const turns = buildChatHistoryFromEvents(
				...
				undefined,
				undefined,
				new Map([['base', { name: 'Base', multiplier: 2 }]]),
			);
			expect(turns).toHaveLength(2);
			const responseTurn = turns[1] as ChatResponseTurn2;
		...
		it('uses session model changes for each rebuilt response turn', () => {
			...
			const events: any[] = [
				{ type: 'session.start', data: { selectedModel: 'opus-4.6' } },
				{
					type: 'user.message',
					id: 'u1',
					data: { content: 'First', attachments: [] },
				},
				{ type: 'assistant.message', data: { content: 'One' } },
				{
					type: 'session.model_change',
					data: { previousModel: 'opus-4.6', newModel: 'opus-4.7' },
				},
				{
					type: 'user.message',
					id: 'u2',
					data: { content: 'Second', attachments: [] },
				},
				{ type: 'assistant.message', data: { content: 'Two' } },
				{
					type: 'session.model_change',
					data: { previousModel: 'opus-4.7', newModel: 'gpt-5.4' },
				},
				{
					type: 'user.message',
					id: 'u3',
					data: { content: 'Third', attachments: [] },
				},
				{ type: 'assistant.message', data: { content: 'Three' } },
				{
					type: 'session.model_change',
					data: { previousModel: 'gpt-5.4', newModel: 'gpt-5.3' },
				},
				{
					type: 'user.message',
					id: 'u4',
					data: { content: 'Fourth', attachments: [] },
				},
			...
			expect(
				turns
					.filter((turn) => turn instanceof ChatRequestTurn2)
					.map((turn) => (turn as ChatRequestTurn2).modelId),
			).toEqual(['opus-4.6', 'opus-4.7', 'gpt-5.4', 'gpt-5.3']);
			expect(
				turns
					.filter((turn) => turn instanceof ChatResponseTurn2)
					.map((turn) => (turn as ChatResponseTurn2).result),
			).toEqual([
				{ details: 'Opus 4.6 • 4x' },
				{ details: 'Opus 4.7 • 4x' },
				{ details: 'GPT 5.4 • 2x' },
		...
		it('uses assistant usage model for the active rebuilt response turn', () => {
			...
			const turns = buildChatHistoryFromEvents(
				...
				undefined,
				undefined,
				new Map([['gpt-5.4', { name: 'GPT 5.4', multiplier: 2 }]]),
			);

			expect(turns).toHaveLength(2);
			expect((turns[0] as ChatRequestTurn2).modelId).toBe('gpt-5.4');
			expect((turns[1] as ChatResponseTurn2).result).toEqual({
				details: 'GPT 5.4 • 2x',
			});
		});
		...
		it('uses persisted responseModelId to recover model details on reload for auto sessions', () => {
			...
			const lookup = (sdkRequestId: string) =>
				detailsByEventId[sdkRequestId];
			...
			const turns = buildChatHistoryFromEvents(
				...
				new Map([
					...
					[
						'claude-opus-4.7',
						{ name: 'Claude Opus 4.7', multiplier: 4 },
					],
				]),
			);
			...
			const events: any[] = [
				{
					...
					data: {
						...
						attachments: [
							{
								type: 'file',
								path: '/workspace/myFile.ts',
								displayName: 'myFile.ts',
							},
						],
					},
				},
		...
		it('filters out instruction file attachments', () => {
			const events: any[] = [
				{
					...
					data: {
						...
						attachments: [
							...
							{
								type: 'file',
								path: '/workspace/.github/instructions/custom.md',
			...
			expect(paths).not.toContain(
				'/workspace/.github/instructions/custom.md',
			);
		...
		it('does not duplicate file attachments when URI already exists in extracted references', () => {
			...
			const appRefs = requestTurn.references.filter(
				(r) => r.id === '/workspace/src/app.ts',
			);
		...
		it('excludes subagent markdown from top-level history', () => {
			const events: any[] = [
				{
					type: 'user.message',
					id: 'u1',
					data: { content: 'Do something', attachments: [] },
				},
				// Top-level assistant message (no parentToolCallId)
				{
					type: 'assistant.message',
					id: 'a1',
					data: { messageId: 'msg-1', content: 'Top-level reply' },
				},
				// Sub-agent delta (has parentToolCallId) — should be excluded
				{
					...
					data: {
						messageId: 'msg-2',
						deltaContent: 'sub-agent thinking...',
				...
				// Top-level assistant message after subagent
				{
					type: 'assistant.message',
					id: 'a4',
					data: { messageId: 'msg-4', content: 'Final answer' },
				},
			];
			const turns = buildChatHistoryFromEvents(
				'',
			...
			const responseTurn = turns[1] as ChatResponseTurn2;
			const parts: any[] =
				(responseTurn as any).response.parts ??
				(responseTurn as any).response._parts ??
				(responseTurn as any).response;
			const markdownParts = parts.filter(
				(p) => p instanceof ChatResponseMarkdownPart,
			);
			const allText = markdownParts
				.map((p) => (p as any).value?.value ?? (p as any).value)
				.join('');
		...
		it('populates modeInstructions2 on ChatRequestTurn2 from stored modeInstructions', () => {
			const events: any[] = [
				...
				{
					type: 'assistant.message',
					data: { content: 'Hi there', messageId: 'msg-1' },
				},
			];
			const getRequestId = (
				sdkRequestId: string,
			): RequestIdDetails | undefined => {
			...
			expect(requestTurn.modeInstructions2!.content).toBe(
				'You are a helpful agent',
			);
			expect(requestTurn.modeInstructions2!.uri?.toString()).toBe(
				'file:///workspace/.github/agents/my-agent.agent.md',
			);
			expect(requestTurn.modeInstructions2!.metadata).toEqual({
				key: 'value',
			});
	...
	describe('createCopilotCLIToolInvocation', () => {
		...
		it('formats bash tool invocation with description', () => {
			...
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
		...
		it('formats str_replace_editor create', () => {
			const part = createCopilotCLIToolInvocation({
				toolName: 'str_replace_editor',
				toolCallId: 'e1',
				arguments: { command: 'create', path: '/tmp/x.ts' },
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			const msg = getInvocationMessageText(
				part as ChatToolInvocationPart,
			);
		...
		it.skip('formats show_file invocation with path', () => {
			...
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
			).toMatch(/Showing.*file\.ts/);
		});
		it.skip('formats show_file invocation with diff mode', () => {
			const part = createCopilotCLIToolInvocation({
				toolName: 'show_file',
				toolCallId: 'sf2',
				arguments: { path: '/tmp/file.ts', diff: true },
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
		...
		it.skip('formats show_file invocation with view_range', () => {
			const part = createCopilotCLIToolInvocation({
				toolName: 'show_file',
				toolCallId: 'sf3',
				arguments: { path: '/tmp/file.ts', view_range: [10, 20] },
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			const msg = getInvocationMessageText(
				part as ChatToolInvocationPart,
		...
		it('returns markdown part for task_complete invocation with summary', () => {
			...
			expect((part as ChatResponseMarkdownPart).value.value).toContain(
				'Fixed the bug',
			);
		});
		...
		it('formats ask_user invocation with structured message', () => {
			...
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
			).toContain('Pick a deployment target');
		});
		...
		it('formats lsp invocation without file', () => {
			const part = createCopilotCLIToolInvocation({
				toolName: 'lsp',
				toolCallId: 'lsp2',
				arguments: { operation: 'workspaceSymbol', query: 'MyClass' },
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
			).toContain('workspaceSymbol');
		});
		it('formats create_pull_request invocation', () => {
			...
			expect((part as ChatToolInvocationPart).originMessage).toContain(
				'Summary of changes',
			);
		});
		it('formats search_code_subagent invocation', () => {
			const part = createCopilotCLIToolInvocation({
				...
				toolCallId: 'sc1',
				arguments: { query: 'find auth middleware' },
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
			).toContain('find auth middleware');
		});
		...
		it('formats write_agent invocation with agent_id', () => {
			const part = createCopilotCLIToolInvocation({
				...
				toolCallId: 'wa1',
				arguments: { agent_id: 'agent-42', message: 'Hello agent' },
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
		...
		it('formats mcp_validate invocation with path', () => {
			const part = createCopilotCLIToolInvocation({
				...
				toolCallId: 'mv1',
				arguments: {
					path: '/home/user/.copilot/config/mcp-config.json',
				},
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
			).toMatch(/mcp-config\.json/i);
		});
		it('formats tool_search_tool_regex invocation with pattern', () => {
			const part = createCopilotCLIToolInvocation({
				toolName: 'tool_search_tool_regex',
				toolCallId: 'ts1',
				arguments: { pattern: 'search.*file' },
			});
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(
				getInvocationMessageText(part as ChatToolInvocationPart),
			).toContain('search.*file');
		});
	...
	describe('process tool execution lifecycle', () => {
		it('marks tool invocation complete and confirmed on success', () => {
			const pending = new Map<
				string,
				[
					ChatToolInvocationPart | ChatResponseThinkingProgressPart,
					toolData: ToolCall,
			...
			const completeEvent: any = {
				type: 'tool.execution_complete',
				data: { toolName: 'bash', toolCallId: 'bash-1', success: true },
			};
		...
		it('marks tool invocation error and unconfirmed when denied', () => {
			...
			const completeEvent: any = {
				...
				data: {
					...
					toolCallId: 'bash-2',
					success: false,
					error: { message: 'Denied', code: 'denied' },
				},
			};
			const [completed] = processToolExecutionComplete(
				completeEvent,
	...
	describe('extractCdPrefix', () => {
		it('extracts cd prefix from bash command', () => {
			const result = extractCdPrefix(
				'cd /home/user/project && npm run test',
				false,
			);
		...
		it('extracts Set-Location prefix from powershell command', () => {
			const result = extractCdPrefix(
				'Set-Location C:\\project; npm start',
				true,
			);
			expect(result).toEqual({
				directory: 'C:\\project',
		...
		it('extracts Set-Location -Path prefix from powershell command', () => {
			const result = extractCdPrefix(
				'Set-Location -Path C:\\project && npm start',
				true,
			);
			expect(result).toEqual({
				directory: 'C:\\project',
	...
	describe('formatShellInvocation with presentationOverrides', () => {
		it('sets presentationOverrides when cd prefix matches workingDirectory', () => {
			...
			const part = createCopilotCLIToolInvocation(
				{
					...
					toolCallId: 'b-cd-1',
					arguments: {
						command: 'cd /home/user/project && npm run unit',
						description: 'Run tests',
					},
				},
				undefined,
			...
			expect(data.presentationOverrides).toEqual({
		...
		it('does not set presentationOverrides when cd prefix does not match workingDirectory', () => {
			...
			const data = part.toolSpecificData as any;
			expect(data.commandLine.original).toBe(
				'cd /home/user/project && npm run unit',
			);
			...
			processToolExecutionStart(
				{
					...
					data: {
						...
						toolCallId: 'b-cd-2',
						arguments: {
							command: 'cd /workspace && make build',
							description: 'Build',
						},
					},
				} as any,
	...
	describe('isCopilotCLIToolThatCouldRequirePermissions', () => {
		const makeEvent = (data: Record<string, unknown>) =>
			({ type: 'tool.execution_start', data }) as any;
	...
	describe('integration edge cases', () => {
		...
		it('handles multiple user messages flushing response parts correctly', () => {
			const events: any[] = [
				...
				{
					type: 'user.message',
					data: { content: 'Follow up', attachments: [] },
				},
			...
			expect(
				turns.filter((t) => t instanceof ChatResponseTurn2),
			).toHaveLength(2);
			expect(
				turns.filter((t) => t instanceof ChatRequestTurn2),
			).toHaveLength(1);
		});
		...
		it('creates markdown part only when cleaned content not empty after stripping PR metadata', () => {
			const events: any[] = [
				{
					...
					data: {
						content:
							'<pr_metadata uri="u" title="t" description="d" author="a" linkTag="l"/>',
					},
				},
			];
			const turns = buildChatHistoryFromEvents(
			...
			const responseTurns = turns.filter(
				(t) => t instanceof ChatResponseTurn2,
			) as ChatResponseTurn2[];
			expect(responseTurns).toHaveLength(1);
			const responseParts: any = (responseTurns[0] as any).response;
			const parts: any[] =
				responseParts.parts ?? responseParts._parts ?? responseParts;
			const prCount = parts.filter(
				(p) => p instanceof ChatResponsePullRequestPart,
			).length;
			const mdCount = parts.filter(
				(p) => p instanceof ChatResponseMarkdownPart,
			).length;
	...
	describe('buildChatHistoryFromEvents with subagent events', () => {
		it('enriches task tool with subagent.started metadata during history rebuild', () => {
			const events: any[] = [
				{
					type: 'user.message',
					id: 'u1',
					data: { content: 'Do a review', attachments: [] },
				},
				{
					type: 'tool.execution_start',
					data: {
				...
				},
				{
					...
					data: {
						...
						agentDescription: 'Reviews code carefully',
					},
				},
				{
					type: 'tool.execution_complete',
			...
			expect(responseTurns).toHaveLength(1);
			const parts: any[] =
				(responseTurns[0] as any).response.parts ??
				(responseTurns[0] as any).response._parts ??
				(responseTurns[0] as any).response;
			const toolParts = parts.filter(
				(p: any) => p instanceof ChatToolInvocationPart,
			);
		...
		it('sets subAgentInvocationId on nested tool calls within a subagent', () => {
			const events: any[] = [
				{
					type: 'user.message',
					id: 'u1',
					data: { content: 'Review my code', attachments: [] },
				},
			...
			const nestedTaskPart = toolParts.find(
				(p) => p.toolCallId === 'task-nested',
			)!;
	...
	describe('isTodoRelatedSqlQuery', () => {
		it('returns true for INSERT into todos', () => {
			expect(
				isTodoRelatedSqlQuery(
					"INSERT INTO todos (title, status) VALUES ('task', 'pending')",
				),
			).toBe(true);
		});
		...
		it('returns true for DELETE from todos', () => {
			expect(
				isTodoRelatedSqlQuery('DELETE FROM todos WHERE id = 1'),
			).toBe(true);
		});
		...
		it('returns true for queries targeting todo_deps table', () => {
			expect(
				isTodoRelatedSqlQuery(
					'INSERT INTO todo_deps (todo_id, dep_id) VALUES (1, 2)',
				),
			).toBe(true);
		});
		...
		it('returns true for ALTER TABLE todo_deps', () => {
			expect(
				isTodoRelatedSqlQuery(
					'ALTER TABLE todo_deps ADD COLUMN priority INTEGER',
				),
			).toBe(true);
		});
	});
	...
	describe('updateTodoListFromSqlItems', () => {
		it('invokes the manage_todo_list tool with mapped items', async () => {
			...
			expect(toolsService.invokeToolCalls[0].name).toBe(
				'manage_todo_list',
			);
		...
		it('maps unknown status to not-started', async () => {
			...
			const input = toolsService.invokeToolCalls[0].input as {
