...
export const IChatDelegationSummaryService =
	createServiceIdentifier<IChatDelegationSummaryService>(
...
export interface IChatDelegationSummaryService {
	...
	trackSummaryUsage(
		sessionId: string,
		summary: string,
	): Promise<ChatPromptReference | undefined>;
	extractPrompt(
		sessionId: string,
		message: string,
...
export class ChatDelegationSummaryService implements IChatDelegationSummaryService {
	...
	async summarize(
		...
		return (
			(await this._chatSummarizer.provideChatSummary(context, token)) ??
			undefined
		);
	}
	...
	async trackSummaryUsage(
		...
		const reference: ChatPromptReference = {
			...
			modelDescription:
				'Summary of previous chat history for delegated request',
			value: uri,
		};

		summary = summary.substring(0, 100);
		await this._mementoUpdater.queue(async () => {
			const details = this.context.globalState.get<
				Record<string, { summary: string; createdDateTime: number }>
			>(DelegationSummaryMementoKey, {});
	...
	extractPrompt(
		...
		const promptSuffix = l10n.t(
			'Complete the task as described in the {0}',
