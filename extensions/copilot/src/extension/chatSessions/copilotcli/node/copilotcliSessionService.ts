...
import type {
	AutoModeSessionManager as SDKAutoModeSessionManager,
	AutoModeSessionResult,
	internal,
	LocalSession,
...
const COPILOT_CLI_WORKSPACE_JSON_FILE_KEY =
	'github.copilot.cli.workspaceSessionFile';
const AUTO_MODE_REFRESH_LEAD_TIME_MS = 300 * 1000;
export const COPILOT_CLI_CHAT_PANEL_SYSTEM_MESSAGE =
...
type AutoModeResolveResult = Awaited<
	ReturnType<SDKAutoModeSessionManager['resolve']>
>;
...
class AutoModeSessionManagerCompat {
	...
	constructor(
		private readonly sdkPackage: Pick<
			SDKPackage,
			| 'AutoModeUnavailableError'
			| 'AutoModeUnsupportedError'
			| 'acquireAutoModeSession'
			| 'isAutoModel'
			| 'refreshAutoModeSession'
		>,
	) {}
	...
	getDiscountPercent(): number | undefined {
		...
		const selectedModelDiscount = this.current?.selectedModel
			? discountedCosts[this.current.selectedModel]
			: undefined;
		...
		return Math.round(
			(allDiscounts.reduce((sum, discount) => sum + discount, 0) /
				allDiscounts.length) *
				100,
		);
	}
	...
	handleModelChange(
		...
		nextModel: string,
		settings?: AutoModeResolveArgs['settings'],
	): void {
		if (
			this.sdkPackage.isAutoModel(nextModel) &&
			!this.sdkPackage.isAutoModel(prevModel)
		) {
	...
	private async doResolve(
		args: AutoModeResolveArgs,
	): Promise<AutoModeResolveResult> {
		const { logger, settings } = args;

		if (this.current) {
			try {
				const refreshed = await this.sdkPackage.refreshAutoModeSession({
					...args,
					existingToken: this.current.sessionToken,
				});
				this.current = refreshed;
				this.applySessionToken(settings, refreshed.sessionToken);
				this.notify();
				return {
					modelId: refreshed.selectedModel,
					sessionToken: refreshed.sessionToken,
				};
			} catch (error) {
				if (this.isUnauthorizedError(error)) {
					logger.debug(
						'Auto-mode refresh unauthorized; acquiring a new session',
					);
				} else if (
					error instanceof this.sdkPackage.AutoModeUnsupportedError
				) {
					logger.debug(
						`Auto-mode refresh unsupported: ${error.message}`,
					);
					this.current = undefined;
					this.notify();
					return undefined;
				} else if (
					error instanceof this.sdkPackage.AutoModeUnavailableError
				) {
					logger.debug(
						`Auto-mode unavailable during refresh: ${error.message}`,
					);
					this.current = undefined;
					this.notify();
					return undefined;
					...
					logger.debug(
						`Auto-mode refresh failed; reusing last token until expiry: ${this.formatError(error)}`,
					);
					this.applySessionToken(settings, this.current.sessionToken);
					return {
						modelId: this.current.selectedModel,
						sessionToken: this.current.sessionToken,
					};
				}
			}
		}
		...
		try {
			...
			this.notify();
			logger.debug(
				`Auto-mode session acquired: selected_model=${acquired.selectedModel}${acquired.expiresAt ? ` expires_at=${acquired.expiresAt}` : ''}`,
			);
			return {
				modelId: acquired.selectedModel,
			...
			logger.debug(
				`Auto-mode acquire failed: ${this.formatError(error)}`,
			);
			return undefined;
		}
	}
	...
	private isFresh(): boolean {
		return this.current
			? this.current.expiresAt
				? this.current.expiresAt * 1000 - Date.now() >
					AUTO_MODE_REFRESH_LEAD_TIME_MS
				: true
			: false;
	}
...
export interface ICopilotCLISessionService {
	...
	): Promise<ICopilotCLISessionItem | undefined>;
	getSessionTitle(
		sessionId: string,
		token: CancellationToken,
	): Promise<string>;
	getAllSessions(
		token: CancellationToken,
	): Promise<readonly ICopilotCLISessionItem[]>;
	...
	getSession(
		options: IGetSessionOptions,
		token: CancellationToken,
	): Promise<IReference<ICopilotCLISession> | undefined>;
	createSession(
		options: ICreateSessionOptions,
		token: CancellationToken,
	): Promise<IReference<ICopilotCLISession>>;
	getChatHistory(
		options: { sessionId: string; workspace: IWorkspaceInfo },
		token: CancellationToken,
	): Promise<(ChatRequestTurn2 | ChatResponseTurn2)[]>;
	forkSession(
		options: {
			sessionId: string;
	...
	tryGetPartialSessionHistory(
		sessionId: string,
	): Promise<readonly (ChatRequestTurn2 | ChatResponseTurn2)[] | undefined>;
}

export const ICopilotCLISessionService =
	createServiceIdentifier<ICopilotCLISessionService>(
		'ICopilotCLISessionService',
	);

export class CopilotCLISessionService
	extends Disposable
	implements ICopilotCLISessionService
{
	...
	private readonly _partialSessionHistories = new Map<
		string,
		readonly (ChatRequestTurn2 | ChatResponseTurn2)[]
	>();

	private readonly _onDidChangeSessions = this._register(new Emitter<void>());
	public readonly onDidChangeSessions = this._onDidChangeSessions.event;

	private readonly _onDidDeleteSession = this._register(
		new Emitter<string>(),
	);
	public readonly onDidDeleteSession = this._onDidDeleteSession.event;

	private readonly _onDidChangeSession = this._register(
		new Emitter<ICopilotCLISessionItem>(),
	);
	...
	private readonly _sessionWorkingDirectories = new Map<
		...
		Uri | undefined
	>();
	private readonly _onDidChangeSessionsThrottler = this._register(
		new ThrottledDelayer<void>(500),
	);
	...
	private readonly _customAgentLookup = new Map<
		string,
		[ChatCustomAgent, Lazy<Promise<string>>]
	>();
	constructor(
		...
		@ICopilotCLIMCPHandler
		private readonly mcpHandler: ICopilotCLIMCPHandler,
		...
		private readonly _promptVariablesService: IPromptVariablesService,
		@IChatDebugFileLoggerService
		private readonly _debugFileLogger: IChatDebugFileLoggerService,
		@IPromptsService private readonly _promptsService: IPromptsService,
		@ICopilotCLIModels
		private readonly _copilotCLIModels: ICopilotCLIModels,
	) {
		super();
		this.showExternalSessions = this.configurationService.getConfig(
		...
		this._sessionManager = new Lazy<Promise<internal.LocalSessionManager>>(
			async () => {
				try {
					const sdkPackage = await this.getSDKPackage();
					const { internal, createLocalFeatureFlagService } =
						sdkPackage;
					...
					if (this._otelService.config.enabled) {
						const otelEnv = deriveCopilotCliOTelEnv(
					...
					return new internal.LocalSessionManager(
						{
							...
							telemetryService:
								new internal.NoopTelemetryService(),
							autoModeManager:
								this.createAutoModeManager(sdkPackage),
						},
						{
					...
					this.logService.error(
						`Failed to initialize Copilot CLI Session Manager: ${error}`,
					);
					throw error;
				}
			},
		);
		this._sessionTracker = this.instantiationService.createInstance(
	...
	private createAutoModeManager(
		sdkPackage: SDKPackage,
	): SDKAutoModeSessionManager {
		...
		this.logService.warn(
			'Failed to construct SDK AutoModeSessionManager, using compatibility fallback.',
		);
		return new AutoModeSessionManagerCompat(
			sdkPackage,
		) as unknown as SDKAutoModeSessionManager;
	}
	...
	protected monitorSessionFiles() {
		try {
			...
			const watcher = this._register(
				this.fileSystem.createFileSystemWatcher(
					new RelativePattern(sessionDir, '**/*.jsonl'),
				),
			);
			this._register(
				watcher.onDidCreate(async (e) => {
					const sessionId = extractSessionIdFromEventPath(
						sessionDir,
						e,
					);
					if (
						sessionId &&
						this._sessionsBeingCreatedViaFork.has(sessionId)
					) {
						return;
					}
					this.triggerSessionsChangeEvent();
					const sessionItem = sessionId
						? await this.getSessionItemImpl(
								sessionId,
								'disk',
			...
			this._register(
				watcher.onDidChange((e) => {
					...
					if (
						Array.from(this._sessionWrappers.keys()).some(
							(sessionId) => e.path.includes(sessionId),
						)
					) {
						return;
					}
					if (sessionId) {
						this.triggerOnDidChangeSessionItem(
							sessionId,
							'fileSystemChange',
						);
					}
			...
			this.logService.error(
				`Failed to monitor Copilot CLI session files: ${error}`,
			);
		}
	}
	...
	private triggerOnDidChangeSessionItem(
		sessionId: string,
		reason: 'fileSystemChange' | 'statusChange',
	) {
		this._sessionChangeNotifierByKey
			.queue(sessionId, async () => {
				...
				if (reason === 'fileSystemChange') {
					await new Promise<void>((resolve) =>
						disposableTimeout(resolve, 500, this._store),
					);
				...
				const sessionItem = await this.getSessionItemImpl(
					sessionId,
					reason === 'statusChange' ? 'inMemorySession' : 'disk',
					CancellationToken.None,
				);
				if (sessionItem) {
					this._onDidChangeSession.fire(sessionItem);
			...
			.catch((error) => {
				this.logService.error(
					`Failed to trigger session change event for session ${sessionId}: ${error}`,
				);
			});
	}
	...
	public async getSessionItemImpl(
		sessionId: string,
		source: 'inMemorySession' | 'disk',
		token: CancellationToken,
	): Promise<ICopilotCLISessionItem | undefined> {
		...
		if (
			...
			(source === 'inMemorySession' ||
				wrappedSession.object.status === ChatSessionStatus.InProgress)
		) {
			const item = await this.constructSessionItemFromWrappedSession(
				wrappedSession,
				token,
			);
			if (item) {
		...
		const sessionManager = await raceCancellation(
	 ...
	 */
	private async getSessionTitleImpl(
		sessionId: string,
		metadata: LocalSessionMetadata | undefined,
		token: CancellationToken,
	): Promise<string> {
		const explicitTitle =
			this._sessionWrappers.get(sessionId)?.object.title ??
			metadata?.name ??
			(await this.customSessionTitleService.getCustomSessionTitle(
				sessionId,
			));
		if (explicitTitle) {
		...
		if (
			summarizedTitle &&
			!summarizedTitle.endsWith('...') &&
			!summarizedTitle.includes('<')
		) {
			return summarizedTitle;
		}

		const firstUserMessage = await this.getFirstUserMessageFromSession(
			sessionId,
			token,
		);
	...
	private _getAllSessionsProgress:
		| Promise<readonly ICopilotCLISessionItem[]>
		| undefined;
	...
	async _getAllSessions(
		...
		try {
			const sessionManager = await raceCancellationError(
				this.getSessionManager(),
				token,
			);
			...
			const diskSessions: ICopilotCLISessionItem[] = coalesce(
				await Promise.all(
					sessionMetadataList.map(
						async (
							metadata,
						): Promise<ICopilotCLISessionItem | undefined> => {
							const workingDirectory = metadata.context?.cwd
								? URI.file(metadata.context.cwd)
								: undefined;
							this._sessionWorkingDirectories.set(
								metadata.sessionId,
								workingDirectory,
							);
							if (
								!(await this.shouldShowSession(
									metadata.sessionId,
									metadata.context,
								))
							) {
							...
							const label = await this.getSessionTitleImpl(
								metadata.sessionId,
								metadata,
								token,
							);
			...
			const newSessions = coalesce(
				await Promise.all(
					Array.from(this._sessionWrappers.values())
						.filter(
							(session) =>
								!diskSessionIds.has(session.object.sessionId),
						)
						.filter(
							(session) =>
			...
			const allSessions = diskSessions
				.map((session): ICopilotCLISessionItem => {
					return {
						...session,
						status: this._sessionWrappers.get(session.id)?.object
							?.status,
					};
				})
				.concat(newSessions);
			...
			allSessions.forEach((session) =>
				this._cachedSessionItems.set(session.id, session),
			);
			return allSessions;
		} catch (error) {
	...
	private async constructSessionItem(
		metadata: LocalSessionMetadata,
		token: CancellationToken,
	): Promise<ICopilotCLISessionItem | undefined> {
		const sessionItem = await this.constructSessionItemImpl(
			metadata,
			token,
		);
		if (sessionItem) {
	...
	private async constructSessionItemFromWrappedSession(
		session: RefCountedSession,
		token: CancellationToken,
	): Promise<ICopilotCLISessionItem | undefined> {
		const label =
			(await this.getSessionTitle(session.object.sessionId, token)) ||
			this._cachedSessionItems.get(session.object.sessionId)?.label ||
			labelFromPrompt(session.object.pendingPrompt ?? '');
		const createTime = Date.now();
		return {
			id: session.object.sessionId,
			label,
			status: session.object.status,
			timing: this._cachedSessionItems.get(session.object.sessionId)
				?.timing ?? { created: createTime, startTime: createTime },
		};
	}

	private async constructSessionItemImpl(
		metadata: LocalSessionMetadata,
		token: CancellationToken,
	): Promise<ICopilotCLISessionItem | undefined> {
		const workingDirectory = metadata.context?.cwd
		...
		const shouldShowSession = await this.shouldShowSession(
			metadata.sessionId,
			metadata.context,
		);
		...
		const endTime = metadata.modifiedTime.getTime();
		const label =
			(await this.getSessionTitleImpl(
				metadata.sessionId,
				metadata,
				token,
			)) ?? labelFromPrompt(metadata.summary ?? '');
	...
	public async createSession(
		options: ICreateSessionOptions,
		token: CancellationToken,
	): Promise<RefCountedSession> {
		const resource = options.sessionId
			? SessionIdForCLI.getResource(options.sessionId)
			: URI.from({
					scheme: 'copilot-cli',
					path: `mcp-gateway-${generateUuid()}`,
				});
		const { mcpConfig: mcpServers, disposable: mcpGateway } =
			await this.mcpHandler.loadMcpConfig(resource);
		try {
			const sessionOptions = await this.createSessionsOptions({
				...options,
				mcpServers,
			});
			const sessionManager = await raceCancellationError(
			...
			const wasNewSession = this._newSessionIds.delete(
			...
			if (wasNewSession) {
				promises.push(
					this.customSessionTitleService
						.getCustomSessionTitle(sdkSession.sessionId)
						.then((stagedTitle) => {
							if (stagedTitle) {
			...
			this.logService.trace(
				`[CopilotCLISession] Created new CopilotCLI session ${sdkSession.sessionId}.`,
			);

			const session = this.createCopilotSession(
				sdkSession,
				options.workspace,
				options.agent?.name,
				sessionManager,
			...
			// Set origin
			void this._chatSessionMetadataStore.setSessionOrigin(
				session.object.sessionId,
			);

			// Set session parent id
			if (options.sessionParentId) {
				void this._chatSessionMetadataStore.setSessionParentId(
					session.object.sessionId,
					options.sessionParentId,
				);
			}
	...
	private _installBridgeIfNeeded(): void {
		...
		try {
			...
			const delegate =
				(globalProvider as unknown as Record<string, unknown>)
					._delegate ?? globalProvider;
			const activeProcessor = (
				delegate as unknown as Record<string, unknown>
			)._activeSpanProcessor as Record<string, unknown> | undefined;
			const processorArray = activeProcessor?._spanProcessors;

			if (Array.isArray(processorArray)) {
				this._bridgeProcessor = new CopilotCliBridgeSpanProcessor(
					this._otelService,
				);
				processorArray.push(this._bridgeProcessor);
				this.logService.info(
					'[CopilotCLISession] Bridge SpanProcessor installed on SDK TracerProvider',
				);
			} else {
				this.logService.warn(
					'[CopilotCLISession] Could not access SDK TracerProvider internals — debug panel will not show SDK spans',
				);
			}
		} catch (err) {
			this.logService.warn(
				`[CopilotCLISession] Failed to install bridge SpanProcessor: ${err}`,
			);
		}
	}

	private async shouldShowSession(
		sessionId: string,
		...
		if (!this.showExternalSessions) {
			const sessionOrigin =
				await this._chatSessionMetadataStore.getSessionOrigin(
					sessionId,
				);
		...
		const sessionTrackerVisibility =
			this._sessionTracker.shouldShowSession(sessionId);
		...
		const workspaceFolder =
			await this.workspaceFolderService.getSessionWorkspaceFolder(
				sessionId,
			);
		if (
			workspaceFolder &&
			this.workspaceService.getWorkspaceFolder(workspaceFolder)
		) {
			return true;
		}
		...
		if (
			sessionTrackerVisibility.isOldGlobalSession &&
			!workspaceFolder &&
			!worktree &&
	...
	protected async createSessionsOptions(
		options: ICreateSessionOptions & {
			mcpServers?: SessionOptions['mcpServers'];
		},
	): Promise<Readonly<SessionOptions>> {
		...
		const variablesContext =
			this._promptVariablesService.buildTemplateVariablesContext(
				options.sessionId,
				options.debugTargetSessionIds,
			);
		...
		if (options.model) {
			allOptions.model =
				options.model as unknown as SessionOptions['model'];
		}
		if (options.mcpServers && Object.keys(options.mcpServers).length > 0) {
			allOptions.mcpServers = options.mcpServers;
		}
		if (skillLocations.length > 0) {
			allOptions.skillDirectories = skillLocations.map(
				(uri) => uri.fsPath,
			);
		}
		if (
			options.mcpServerMappings?.size &&
			customAgents &&
			options.mcpServers
		) {
			remapCustomAgentTools(
	...
	private getSandboxConfig(): SessionOptions['sandboxConfig'] {
		const sandboxSettingId =
			process.platform === 'win32'
				? 'chat.agent.sandbox.enabledWindows'
				: 'chat.agent.sandbox.enabled';
		const rawSandboxSetting =
			this.configurationService.getNonExtensionConfig<unknown>(
				sandboxSettingId,
			);
		const sandboxSetting =
			typeof rawSandboxSetting === 'string'
				? rawSandboxSetting
				: rawSandboxSetting === true
					? 'on'
					: rawSandboxSetting === false
						? 'off'
						: undefined;
		const rawFileSystemSetting =
			this.configurationService.getNonExtensionConfig<unknown>(
				'chat.agent.sandbox.fileSystem',
			);
		const fileSystemSetting =
			rawFileSystemSetting && typeof rawFileSystemSetting === 'object'
				? (rawFileSystemSetting as IAgentSandboxFileSystemSettings)
				: undefined;
		const allowedHosts = readStringArraySetting(
			this.configurationService,
			'chat.agent.allowedNetworkDomains',
		);
		const blockedHosts = readStringArraySetting(
			this.configurationService,
	...
	public async getSession(
		options: IGetSessionOptions,
		token: CancellationToken,
	): Promise<RefCountedSession | undefined> {
		// https://github.com/microsoft/vscode/issues/276573
		const lock =
			this.sessionMutexForGetSession.get(options.sessionId) ??
			new Mutex();
		...
		try {
			{
				...
				if (session) {
					this.logService.trace(
						`[CopilotCLISession] Reusing CopilotCLI session ${options.sessionId}.`,
					);
					this._partialSessionHistories.delete(options.sessionId);
					session.acquire();
					if (options.agent) {
						await session.object.sdkSession.selectCustomAgent(
							options.agent.name,
						);
					} else {
			...
			const [
				sessionManager,
				{ mcpConfig: mcpServers, disposable: mcpGateway },
			] = await Promise.all([
			...
			try {
				...
				if (!sdkSession) {
					this.logService.error(
						`[CopilotCLISession] CopilotCLI failed to get session ${options.sessionId}.`,
					);
					return undefined;
				}
	...
	public async getChatHistory(
		{
			sessionId,
			workspace,
		}: { sessionId: string; workspace: IWorkspaceInfo },
		token: CancellationToken,
	): Promise<(ChatRequestTurn2 | ChatResponseTurn2)[]> {
		const { history } = await this.getChatHistoryImpl(
			{ sessionId, workspace },
			token,
		);
		return history;
	...
	private async getChatHistoryImpl(
		{
			sessionId,
			workspace,
		}: { sessionId: string; workspace: IWorkspaceInfo },
		...
		const existingSession =
			this._sessionWrappers.get(sessionId)?.object?.sdkSession;
		if (existingSession) {
			...
			try {
				...
				shutdown = sessionManager
					...
					.catch((error) => {
						this.logService.error(
							`[CopilotCLISession] Failed to close session ${sessionId} after fetching chat history: ${error}`,
						);
					});
			} catch (error) {
				this.logService.error(
					`[CopilotCLISession] Failed to read session ${sessionId}, it may be corrupted: ${error}`,
				);
				return { history: [], events: [] };
			} finally {
				await shutdown;
		...
		const [agentId, storedDetails] = await Promise.all([
			agentIdPromise,
			requestDetailsPromise,
		]);
		...
		const defaultModeInstructions = agentId
			? await this.resolveAgentModeInstructions(agentId)
			: undefined;
		...
		for (const d of storedDetails) {
			if (d.copilotRequestId) {
				...
				detailsByCopilotId.set(d.copilotRequestId, {
					requestId: d.vscodeRequestId,
					toolIdEditMap: d.toolIdEditMap,
					modeInstructions,
		...
		const modelDetailsById = this.configurationService.getConfig(
		...
		if (legacyMappings.length > 0) {
			void this._chatSessionMetadataStore
				.updateRequestDetails(sessionId, legacyMappings)
				.catch((error) => {
					this.logService.error(
						`[CopilotCLISession] Failed to update chat session metadata store with legacy mappings for session ${sessionId}`,
						error,
					);
				});
		}
	...
	private createCustomAgentLookup(): Promise<void> {
		...
		this._customAgentLookupRebuild = (async () => {
			...
			try {
				const agents = await this._promptsService.getCustomAgents(
					CancellationToken.None,
				);
				...
				for (const agent of agents) {
					...
					const lazyContent = new Lazy(() =>
						this._promptsService
							.parseFile(agent.uri, CancellationToken.None)
							.then((parsed) => parsed.body?.getContent() ?? ''),
					);
			...
			this._customAgentLookupRebuild = undefined;
		});
		return this._customAgentLookupRebuild;
	}

	private async resolveAgentModeInstructions(
		agentId: string | undefined,
	): Promise<StoredModeInstructions | undefined> {
		if (!agentId) {
			return undefined;
		}
	...
	private async getModelDetailsById(): Promise<
		ReadonlyMap<string, ModelDetailsInfo>
	> {
		...
		for (const model of models) {
			detailsById.set(model.id.trim().toLowerCase(), {
				name: model.name,
	...
	public async forkSession(
		...
		const [
			sessionManager,
			title,
			{ history, events: originalSessionEvents },
		] = await Promise.all([
			raceCancellationError(this.getSessionManager(), token),
			this.getSessionTitle(sessionId, token),
			requestId
				? this.getChatHistoryImpl({ sessionId, workspace }, token)
				: Promise.resolve({ history: [], events: [] }),
		]);

		let toEventId: string | undefined;
		if (requestId) {
			const requestToTruncateTo = history.find(
				(event) =>
					event instanceof ChatRequestTurn2 && event.id === requestId,
			);
			if (requestToTruncateTo) {
				const storedDetails =
					await this._chatSessionMetadataStore.getRequestDetails(
						sessionId,
					);
				const translatedSDKEvent = storedDetails.find(
					(d) =>
						d.vscodeRequestId === requestToTruncateTo.id ||
						d.copilotRequestId === requestToTruncateTo.id,
				)?.copilotRequestId;
				const sdkEvent = originalSessionEvents.find(
					(e) =>
						e.type === 'user.message' &&
						e.id === requestToTruncateTo.id,
				)?.id;
				toEventId = translatedSDKEvent ?? sdkEvent;
				if (!toEventId) {
					this.logService.warn(
						`[CopilotCLISession] Cannot find SDK event id for request id ${requestId} in session ${sessionId}. Will fork without truncation.`,
					);
				}
			} else {
				this.logService.warn(
					`[CopilotCLISession] Failed to find request ${requestId} in session ${sessionId} history. Will fork without truncation.`,
				);
			}
		}

		const { sessionId: newSessionId } = await sessionManager.forkSession(
			sessionId,
			toEventId,
		);
		...
		try {
			...
			const customTitle = title.startsWith(forkedTitlePrefix)
				? title
				: l10n.t('Forked: {0}', title);
			await this._chatSessionMetadataStore.storeForkedSessionMetadata(
				sessionId,
				newSessionId,
				customTitle,
			);
	...
	public async tryGetPartialSessionHistory(
		sessionId: string,
	): Promise<readonly (ChatRequestTurn2 | ChatResponseTurn2)[] | undefined> {
		...
		try {
			...
			const sessionStartEvent = events.find(
				(
					event,
				): event is Extract<SessionEvent, { type: 'session.start' }> =>
			...
			const history = buildChatHistoryFromEvents(
				...
				this._delegationSummaryService,
				this.logService,
				workingDirectory ? URI.file(workingDirectory) : undefined,
			);
			this._partialSessionHistories.set(sessionId, history);
			return history;
		} catch (error) {
			this.logService.warn(
				`[CopilotCLISession] Failed to reconstruct partial session ${sessionId}: ${error}`,
			);
			return undefined;
		}
	}

	private async getFirstUserMessageFromSession(
		sessionId: string,
		token: CancellationToken,
	): Promise<string | undefined> {
		const cached =
			await this._chatSessionMetadataStore.getSessionFirstUserMessage(
				sessionId,
			);
		...
		try {
			const events = await raceCancellation(
				readSessionEventsFile(sessionId, 'user.message'),
				token,
			);
			if (events?.length) {
				// Find the first user message and use that as the title.
				firstUserMessage = events.find(
					(msg: SessionEvent) => msg.type === 'user.message',
				)?.data.content;
			}
		} catch (error) {
			this.logService.warn(
				`[CopilotCLISession] Failed to get session title for session ${sessionId}: ${error}`,
			);
		}

		if (!firstUserMessage) {
			try {
				const { events } = await this.getChatHistoryImpl(
					{ sessionId, workspace: emptyWorkspaceInfo() },
					token,
				);
				firstUserMessage = events.find(
					(msg: SessionEvent) => msg.type === 'user.message',
				...
				this.logService.warn(
					`[CopilotCLISession] Failed to load session for first user message ${sessionId}: ${error}`,
				);
			}
		}

		this._chatSessionMetadataStore
			.setSessionFirstUserMessage(sessionId, firstUserMessage ?? '')
			.catch((err) => {
				this.logService.warn(
					`[CopilotCLISession] Failed to store first user message for session ${sessionId}: ${err}`,
				);
			});
	...
	private createCopilotSession(
		...
		sessionManager: internal.LocalSessionManager,
	): RefCountedSession {
		...
		this._debugFileLogger.startSession(session.sessionId).catch((err) => {
			this.logService.error(
				'[CopilotCLISession] Failed to start debug log session',
				err,
			);
		});
		session.add(
			toDisposable(() => {
				this._debugFileLogger
					...
					.catch((err) => {
						this.logService.error(
							'[CopilotCLISession] Failed to end debug log session',
							err,
						);
					});
			}),
		...
		if (otelLifecycle) {
			session.setSdkTraceContextUpdater((traceparent, tracestate) =>
				otelLifecycle.updateParentTraceContext(
					sdkSession.sessionId,
					traceparent,
					tracestate,
				),
				...
				(async () => {
					if (sdkSession.isAbortable()) {
						await sdkSession.abort().catch((error) => {
							this.logService.error(
								`Failed to abort session ${sdkSession.sessionId}: ${error}`,
							);
						});
					}
					await sessionManager
						.closeSession(sdkSession.sessionId)
						.catch((error) => {
							this.logService.error(
								`Failed to close session ${sdkSession.sessionId}: ${error}`,
							);
						});
		...
		try {
			{
				...
				if (session) {
					session.dispose();
					this.logService.warn(
						`Delete an active session ${sessionId}.`,
					);
				}
			}
	...
	private async updateSdkSessionMetadata(
		sessionId: string,
		title: string,
		operation: (sdkSession: LocalSession) => Promise<void>,
	): Promise<void> {
		let sessionManager: internal.LocalSessionManager | undefined;
		let shouldCloseSession = false;
		const sdkSession =
			(this._sessionWrappers.get(sessionId)?.object.sdkSession as
				| LocalSession
				| undefined) ??
			(await (async () => {
		...
		try {
			...
			if (shouldCloseSession && sessionManager) {
				await sessionManager.closeSession(sessionId).catch((error) => {
					this.logService.error(
						`[CopilotCLISession] Failed to close session ${sessionId} after updating title metadata: ${error}`,
					);
				});
			}
		}
	...
	public async renameSession(
		...
		await this.updateSdkSessionMetadata(sessionId, title, (sdkSession) =>
			sdkSession.renameSession(title),
		);
		this._sessionLabels.delete(sessionId);
		this._onDidChangeSessions.fire();
	}
	...
	public async updateSessionSummary(
		...
		await this.updateSdkSessionMetadata(sessionId, title, (sdkSession) =>
			sdkSession.updateSessionSummary(title),
		);
...
async function readSessionEventsFile(
	...
	try {
		for await (const line of reader) {
			...
			if (
				findFirstEventType &&
				sessionEvent.type === findFirstEventType
			) {
				break;
			}
		}
...
export class CopilotCLISessionWorkspaceTracker {
	private readonly _initializeSessionStorageFiles: Lazy<
		Promise<{ global: Uri; workspace: Uri }>
	>;
	...
	constructor(
		...
		this._initializeSessionStorageFiles = new Lazy<
			Promise<{ global: Uri; workspace: Uri }>
		>(async () => {
			const globalFile = joinPath(
				this.context.globalStorageUri,
				'copilot.cli.oldGlobalSessions.json',
			);
			let workspaceFile = joinPath(
				this.context.globalStorageUri,
			...
			if (this.workspaceService.getWorkspaceFolders().length) {
				let workspaceFileName = this.context.workspaceState.get<
					string | undefined
				>(COPILOT_CLI_WORKSPACE_JSON_FILE_KEY);
			...
			await Promise.all([
				// Load old sessions
				(async () => {
					const oldSessions = await this.fileSystem
						.readFile(globalFile)
						.then((c) => new TextDecoder().decode(c).split(','))
						.catch(() => undefined);
					if (oldSessions) {
				...
				(async () => {
					const workspaceSessions =
						this.workspaceService.getWorkspaceFolders().length
							? await this.fileSystem
									.readFile(workspaceFile)
									.then((c) =>
										new TextDecoder().decode(c).split(','),
									)
									.catch(() => [])
							: [];
					workspaceSessions.forEach((s) =>
						this._workspaceSessions.add(s),
					);
				})(),
			]);
	 ...
	 */
	public shouldShowSession(sessionId: string): {
		isOldGlobalSession?: boolean;
 ...
 */
function extractSessionIdFromEventPath(
	sessionDir: URI,
	deletedFileUri: URI,
): string | undefined {
...
export class Mutex {
	...
	acquire(token: CancellationToken): Promise<IDisposable | undefined> {
		return raceCancellation(
			new Promise<IDisposable | undefined>((resolve) => {
				const tryAcquire = () => {
...
export class RefCountedSession
	extends RefCountedDisposable
	implements IReference<CopilotCLISession>
{
...
export function buildSandboxConfigForCLI(
	...
	const allowedHosts =
		!allowAllNetwork && networkHosts?.allowedHosts?.length
			? [...networkHosts.allowedHosts]
			: undefined;
	const blockedHosts =
		!allowAllNetwork && networkHosts?.blockedHosts?.length
			? [...networkHosts.blockedHosts]
			: undefined;
