...
export class CopilotCLIPromptResolver {
	constructor(
		@ICopilotCLIImageSupport
		private readonly imageSupport: ICopilotCLIImageSupport,
	...
	public async resolvePrompt(
		...
		additionalReferences: vscode.ChatPromptReference[],
		...
		const allReferences = new ChatVariablesCollection(
			request.references.concat(
				additionalReferences.filter(
					(ref) => !request.references.includes(ref),
				),
			),
		);
		prompt = prompt ?? request.prompt;
		const [variables, attachments] =
			await this.constructChatVariablesAndAttachments(
				allReferences,
				workspaceInfo,
	 ...
	 */
	private buildFolderToWorktreeMap(
		primaryWorkspaceInfo: IWorkspaceInfo,
		additionalWorkspaces: IWorkspaceInfo[],
	): ResourceMap<vscode.Uri> {
		const map = new ResourceMap<vscode.Uri>();
		if (primaryWorkspaceInfo.worktree && primaryWorkspaceInfo.repository) {
			map.set(
				primaryWorkspaceInfo.repository,
				primaryWorkspaceInfo.worktree,
			);
		}
	...
	private async constructChatVariablesAndAttachments(
		variables: ChatVariablesCollection,
		workspaceInfo: IWorkspaceInfo,
		additionalWorkspaces: IWorkspaceInfo[],
		token: vscode.CancellationToken,
	): Promise<[variables: ChatVariablesCollection, Attachment[]]> {
		...
		const isolationEnabled =
			isIsolationEnabled(workspaceInfo) ||
			additionalWorkspaces.some((ws) => isIsolationEnabled(ws));
		const folderToWorktreeMap = this.buildFolderToWorktreeMap(
			workspaceInfo,
			additionalWorkspaces,
		);
		const hasAnyWorkingDirectory =
			getWorkingDirectory(workspaceInfo) ||
			additionalWorkspaces.some((ws) => getWorkingDirectory(ws));
		...
		await Promise.all(
			Array.from(variables).map(async (variable) => {
				...
				if (
					...
					isCustomizationsIndex(variable)
				) {
					return;
				}
				// No need to include skill prompt files as an attachment if CLI already knows about them.
				const promptFileUri = isPromptFile(variable)
					? variable.value
					: undefined;
				if (promptFileUri) {
					if (
						knownSkillLocations.some((loc) =>
					...
					const directory = URI.file(
						path.dirname(promptFileUri.fsPath),
					);
					if (
						promptFileUri.fsPath.endsWith('plan.prompt.md') &&
						path.basename(directory.fsPath) === 'prompts' &&
						extUriBiasedIgnorePathCase.isEqualOrParent(
				...
				if (
					isolationEnabled &&
					isWorkspaceRepoInformationItem(variable)
				) {
					return;
				}
				const variableRef =
					!isolationEnabled || !hasAnyWorkingDirectory
						? variable.reference
						: await this.translateWorkspaceRefToWorkingDirectoryRef(
								variable.reference,
								workspaceInfo,
								additionalWorkspaces,
								folderToWorktreeMap,
		...
		const [attachments, imageAttachments] =
			await this.constructFileOrFolderAttachments(
		...
		for (const reference of builtinSlashCommandReferences) {
			...
			if (
				isGitHubPullRequestReference(reference) &&
				URI.isUri(reference.value)
			) {
				attachments.push({
					type: 'blob',
					mimeType: 'text/plain',
			...
			if (
				isGitMergeChangesReference(reference) &&
	...
	private async constructFileOrFolderAttachments(
		fileOrFolderReferences: vscode.ChatPromptReference[],
		token: vscode.CancellationToken,
	): Promise<[Attachment[], image: Attachment[]]> {
		const attachments: Attachment[] = [];
		const images: Attachment[] = [];
		await Promise.all(
			fileOrFolderReferences.map(async (ref) => {
				if (ref.value instanceof ChatReferenceBinaryData) {
					...
					try {
						...
						const uri = await this.imageSupport.storeImage(
							buffer,
							ref.value.mimeType,
						);
						attachments.push({
						...
						this.logService.error(
							`[CopilotCLISession] Failed to store image: ${error}`,
						);
					}
					return;
				}
				...
				if (isLocation(ref.value)) {
					try {
						...
						const document = await raceCancellation(
							this.workspaceService.openTextDocument(
								ref.value.uri,
							),
							token,
						);
						...
						attachments.push({
							...
							selection: {
								start: {
									...
									character:
										ref.value.range.start.character + 1,
								},
								end: {
									line: ref.value.range.end.line + 1,
									character:
										ref.value.range.end.character + 1,
								},
							},
						...
						this.logService.error(
							`[CopilotCLISession] Failed to attach location ${ref.value.uri.fsPath}: ${ex}`,
						);
					}
					return;
				}
				...
				try {
					const stat = await raceCancellation(
						this.fileSystemService.stat(uri),
						token,
					);
					if (!stat) {
						return;
					...
					if (!type) {
						this.logService.error(
							`[CopilotCLISession] Ignoring attachment as it's not a file/directory (${uri.fsPath})`,
						);
						return;
					}
					attachments.push({
	...
	private async translateWorkspaceRefToWorkingDirectoryRef(
		ref: vscode.ChatPromptReference,
		workspaceInfo: IWorkspaceInfo,
		additionalWorkspaces: IWorkspaceInfo[],
		folderToWorktreeMap: ResourceMap<vscode.Uri>,
		token: vscode.CancellationToken,
	): Promise<vscode.ChatPromptReference> {
		try {
			if (ref.value instanceof ChatReferenceBinaryData) {
				return ref;
			}
			...
			if (isLocation(ref.value)) {
				const uri =
					await this.translateWorkspaceUriToWorkingDirectoryUri(
						ref.value.uri,
						workspaceInfo,
						additionalWorkspaces,
						folderToWorktreeMap,
				...
				const diagnostics = await Promise.all(
					ref.value.diagnostics.map(async ([uri, diags]) => {
						const translatedUri =
							await this.translateWorkspaceUriToWorkingDirectoryUri(
								uri,
								workspaceInfo,
			...
			this.logService.error(
				error,
				`[CopilotCLISession] Failed to translate workspace reference`,
			);
			return ref;
		}
	}

	private async translateWorkspaceUriToWorkingDirectoryUri(
		uri: vscode.Uri,
		workspaceInfo: IWorkspaceInfo,
		additionalWorkspaces: IWorkspaceInfo[],
		folderToWorktreeMap: ResourceMap<vscode.Uri>,
		...
		const matchingWorktree = workspaceFolder
			? folderToWorktreeMap.get(workspaceFolder)
			: undefined;
		if (!workspaceFolder || !matchingWorktree) {
			return (
				(await this.findMatchingWorktree(
					uri,
					workspaceInfo,
					additionalWorkspaces,
					token,
		...
		const candidateStat = await raceCancellation(
			this.fileSystemService.stat(candidate),
			token,
		).catch(() => undefined);
		return candidateStat ? candidate : uri;
	}

	private async findMatchingWorktree(
		uri: vscode.Uri,
		workspaceInfo: IWorkspaceInfo,
		additionalWorkspaces: IWorkspaceInfo[],
		token: vscode.CancellationToken,
...
function isWorkspaceRepoInformationItem(variable: PromptVariable): boolean {
	...
	return (
		(ref.modelDescription.startsWith(
			'Information about one of the current repositories',
		) ||
			ref.modelDescription.startsWith(
...
function isGitHubPullRequestReference(
	ref: vscode.ChatPromptReference,
): boolean {
