...
import {
	...
	formatDiffAsUnified,
} from '../../../tools/node/editFileToolUtils';
 ...
 */
export type PermissionRequestResult = Parameters<
	import('@github/copilot/sdk').Session['respondToPermission']
>[1];
...
export async function handleReadPermission(
	...
	if (isFileFromSessionWorkspace(file, workspaceInfo)) {
		logService.trace(
			`[CopilotCLISession] Auto Approving request to read file in session workspace ${permissionRequest.path}`,
		);
		return { kind: 'approve-once' };
	}
	...
	if (workspaceService.getWorkspaceFolder(file)) {
		logService.trace(
			`[CopilotCLISession] Auto Approving request to read workspace file ${permissionRequest.path}`,
		);
		return { kind: 'approve-once' };
	}

	// Auto-approve reads of internal session resources (e.g. plan.md).
	const sessionDir = Uri.joinPath(
		Uri.file(getCopilotCLISessionStateDir()),
		sessionId,
	);
	if (extUriBiasedIgnorePathCase.isEqualOrParent(file, sessionDir)) {
		logService.trace(
			`[CopilotCLISession] Auto Approving request to read Copilot CLI session resource ${permissionRequest.path}`,
		);
		return { kind: 'approve-once' };
	}
	...
	if (
		attachments.some(
			(attachment) =>
				...
				isEqual(Uri.file(attachment.path), file),
		)
	) {
		logService.trace(
			`[CopilotCLISession] Auto Approving request to read attached file ${permissionRequest.path}`,
		);
		return { kind: 'approve-once' };
	}
	...
	const toolParams: CoreConfirmationToolParams = {
		...
		input: {
			title: 'Read file(s)',
			message:
				permissionRequest.intention ||
				permissionRequest.path ||
				codeBlock(permissionRequest),
			confirmationType: 'basic',
		},
	};
	return invokeConfirmationTool(
		toolParams,
		toolParentCallId,
		toolsService,
		toolInvocationToken,
...
export async function handleWritePermission(
	...
	if (workingDirectory && editFile) {
		...
		const isWorkingDirectoryFile =
			!workspaceService.getWorkspaceFolder(workingDirectory) &&
		...
		if (autoApprove) {
			logService.trace(
				`[CopilotCLISession] Auto Approving request ${editFile.fsPath}`,
			);
			await trackEditIfNeeded(
				editTracker,
				toolCall,
				editFile,
				stream,
	...
	if (
		editFile &&
		extUriBiasedIgnorePathCase.isEqualOrParent(editFile, sessionDir)
	) {
		logService.trace(
			`[CopilotCLISession] Auto Approving request to write to Copilot CLI session resource ${editFile.fsPath}`,
		);
		return { kind: 'approve-once' };
	}
	...
	const toolParams = await getFileEditConfirmationToolParams(
		instantiationService,
		permissionRequest,
		toolCall,
		workspaceFolderForFile,
	);
	if (!toolParams) {
	...
	const result = await invokeConfirmationTool(
		toolParams,
		toolParentCallId,
		toolsService,
		toolInvocationToken,
...
export async function handleShellPermission(
	...
	const toolParams = buildShellConfirmationParams(
		permissionRequest,
...
export function buildShellConfirmationParams(
	...
	isWindows =
		typeof isWindows === 'boolean' ? isWindows : platform() === 'win32';
	const isPowershell = isWindows;
	const fullCommandText = permissionRequest.fullCommandText || '';
	const userFriendlyCommand = fullCommandText
		? getCdPresentationOverrides(
				fullCommandText,
				isPowershell,
				workingDirectory,
	...
	try {
		...
		const result = await toolsService.invokeTool(
			tool,
			{
				input,
				toolInvocationToken,
				subAgentInvocationId: toolParentCallId,
			},
			token,
		);
		...
		if (
			...
			typeof firstResultPart.value === 'string' &&
			firstResultPart.value.toLowerCase() === 'yes'
		) {
			return { kind: 'approve-once' };
		}
	} catch (error) {
 ...
 */
export function isFileFromSessionWorkspace(
	file: URI,
	workspaceInfo: IWorkspaceInfo,
): boolean {
	const workingDirectory = getWorkingDirectory(workspaceInfo);
	if (
		workingDirectory &&
		extUriBiasedIgnorePathCase.isEqualOrParent(file, workingDirectory)
	) {
		return true;
	}
	if (
		workspaceInfo.folder &&
		extUriBiasedIgnorePathCase.isEqualOrParent(file, workspaceInfo.folder)
	) {
		return true;
	}
 ...
 */
async function trackEditIfNeeded(
	editTracker: ExternalEditTracker,
	toolCall: ToolCall | undefined,
	editFile: URI,
	stream: ChatResponseStream | undefined,
	...
	if (toolCall && stream) {
		try {
			...
			logService.error(
				error,
				`[CopilotCLISession] Failed to track edit for toolCallId ${toolCall.toolCallId}`,
			);
		}
	}
}

export async function requiresFileEditconfirmation(
	instaService: IInstantiationService,
	permissionRequest: PermissionRequest,
	toolCall?: ToolCall | undefined,
	workingDirectory?: URI,
): Promise<boolean> {
	const confirmationInfo = await getFileEditConfirmationToolParams(
		instaService,
		permissionRequest,
		toolCall,
		workingDirectory,
...
async function getFileEditConfirmationToolParams(
	instaService: IInstantiationService,
	permissionRequest: PermissionRequest,
	toolCall?: ToolCall | undefined,
	workingDirectory?: URI,
): Promise<CoreConfirmationToolParams | undefined> {
	...
	const details = async (accessor: ServicesAccessor) => {
		if (!toolCall) {
			...
			if (
				toolCall.arguments.command === 'edit' ||
				toolCall.arguments.command === 'str_replace'
			) {
				return getDetailsForFileEditPermissionRequest(
					accessor,
					toolCall.arguments,
				);
			} else if (toolCall.arguments.command === 'create') {
				return getDetailsForFileCreatePermissionRequest(
					accessor,
					toolCall.arguments,
				);
			} else if (toolCall.arguments.command === 'insert') {
				return getDetailsForFileInsertPermissionRequest(
					accessor,
					toolCall.arguments,
				);
			}
	...
	const getDetails = () =>
		instaService.invokeFunction(details).then((d) => d || '');
	const confirmationInfo = await instaService.invokeFunction((accessor) =>
		createEditConfirmation(
			accessor,
			[file],
			undefined,
...
async function getDetailsForFileInsertPermissionRequest(
	accessor: ServicesAccessor,
	args: Extract<ToolCall, { toolName: 'insert' }>['arguments'],
): Promise<string | undefined> {
	if (args.path && args.new_str) {
		return formatDiffAsUnified(
			accessor,
			URI.file(args.path),
			'',
			args.new_str,
...
async function getDetailsForFileCreatePermissionRequest(
	accessor: ServicesAccessor,
	args: Extract<ToolCall, { toolName: 'create' }>['arguments'],
): Promise<string | undefined> {
...
async function getDetailsForFileEditPermissionRequest(
	accessor: ServicesAccessor,
	args: Extract<ToolCall, { toolName: 'edit' | 'str_replace' }>['arguments'],
): Promise<string | undefined> {
...
export declare type PermissionRequest =
	PermissionRequestedEvent['data']['permissionRequest'];
