...
describe('Copilot CLI shims', () => {
	...
	it('creates runtime ripgrep and node-pty shims for separate extension installs before SDK import', async () => {
		...
		).resolves.toBe('Shims created successfully');
		const sdkRipgrepDir = join(
			extensionPath,
			'node_modules',
			'@github',
			'copilot',
		...
		await expect(readFile(join(sdkRipgrepDir, 'rg'), 'utf8')).resolves.toBe(
			'ripgrep',
		);
	...
	it('skips runtime node-pty shimming for bundled installs when shims.txt already exists', async () => {
		...
		await mkdir(join(extensionPath, 'node_modules', '@github', 'copilot'), {
			recursive: true,
		});
		...
		await expect(readFile(placeholder, 'utf8')).resolves.toBe(
			'already created',
		);
		await expect(
			stat(
