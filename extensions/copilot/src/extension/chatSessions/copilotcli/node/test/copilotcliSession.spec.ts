...
class MockSdkSession {
	...
	private _pendingUserInputs = new Map<
		string,
		{ resolve: (result: unknown) => void }
	>();
	private _userInputCounter = 0;
	private _pendingExitPlanMode = new Map<
		string,
		{ resolve: (result: unknown) => void }
	>();
	...
	async emitUserInputRequest(request: {
		question: string;
		choices?: string[];
	 ...
	 */
	async emitExitPlanModeRequest(data: {
		summary: string;
		actions?: string[];
	}): Promise<unknown> {
	...
	public lastSendOptions:
		| { prompt: string; mode?: string; source?: string; agentMode?: string }
		| undefined;
	...
	getCurrentToolMetadata(): unknown[] | undefined {
		return this._toolMetadata;
	}
	private _toolMetadata: unknown[] | undefined;
	set toolMetadata(value: unknown[] | undefined) {
		this._toolMetadata = value;
	}
	...
	async setSelectedModel(model: string, _reasoningEffort?: string) {
...
describe('CopilotCLISession', () => {
	...
	beforeEach(async () => {
		...
		sessionWorkspaceInfo = workspaceInfoFor(
			workspaceService.getWorkspaceFolders()![0],
		);
	...
	it('routes in-flight output to the latest attached stream', async () => {
		...
		sdkSession.send = async ({ prompt }) => {
			...
			sdkSession.emit('assistant.message', {
				messageId: 'after-reattach',
				content: 'After reattach',
			});
		};

		const session = await createSession();
		const firstStream = new MockChatResponseStream();
		const firstStreamAttachment = disposables.add(
	...
	it('routed response stream is not thenable', async () => {
		const session = await createSession();
		const routedStream = (
			session as unknown as { readonly _stream: unknown }
		)._stream;

		await expect(
			Promise.race([
				Promise.resolve(routedStream).then(
					(value) => value === routedStream,
				),
	...
	it('falls back when async stream methods reject as closed', async () => {
		class ClosedExternalEditStream extends MockChatResponseStream {
			override externalEdit(): Promise<string> {
				return Promise.reject(
					new Error('Response stream has been closed'),
				);
			}
		}
		...
		const routedStream = (
			session as unknown as { readonly _stream: ChatResponseStream }
		)._stream;
		let callbackRan = false;

		await expect(
			routedStream.externalEdit(
	...
	it('emits status events on successful request', async () => {
		...
		const listener = disposables.add(
			session.onDidChangeStatus((s) => statuses.push(s)),
		);
		const stream = new MockChatResponseStream();
		session.attachStream(stream);
		await session.handleRequest(
	...
	it('auto-approves read permission inside workspace without external handler', async () => {
		...
		sdkSession.send = async ({ prompt }: any) => {
			...
			result = await sdkSession.emitPermissionRequest({
				kind: 'read',
				path: path.join('/workspace', 'file.ts'),
				intention: 'Read file',
			});
	...
	it('auto-approves read permission inside working directory without external handler', async () => {
		...
		sdkSession.send = async ({ prompt }: any) => {
			...
			result = await sdkSession.emitPermissionRequest({
				kind: 'read',
				path: path.join('/workingDirectory', 'file.ts'),
				intention: 'Read file',
			});
			sdkSession.emit('assistant.turn_end', {});
		};
	...
	it('auto-approves read permission for files in workspace folder when worktree is the working directory', async () => {
		...
		sessionWorkspaceInfo = {
			...
			worktreeProperties: {
				...
				worktreePath: '/worktrees/session1',
			},
		};
		sdkSession.send = async ({ prompt }: any) => {
			sdkSession.emit('assistant.turn_start', {});
			...
			result = await sdkSession.emitPermissionRequest({
				kind: 'read',
				path: path.join('/original-repo', 'src/main.ts'),
				intention: 'Read file',
			});
			sdkSession.emit('assistant.turn_end', {});
		};
	...
	it('auto-approves read permission for files in the worktree when workspace has both worktree and repository', async () => {
		...
		sdkSession.send = async ({ prompt }: any) => {
			...
			result = await sdkSession.emitPermissionRequest({
				kind: 'read',
				path: path.join('/worktrees/session1', 'src/main.ts'),
				intention: 'Read file',
			});
			sdkSession.emit('assistant.turn_end', {});
		};
	...
	it('preserves order of edit toolCallIds and permissions for multiple pending edits', async () => {
		...
		const trackSpy = vi
			.spyOn(ExternalEditTracker.prototype, 'trackEdit')
			.mockImplementation(async function (this: any, editKey: string) {
		...
		expect(permissionResults.every((r) => r.kind === 'approve-once')).toBe(
			true,
		);
	...
	it('delays tool invocation messages for permission-requiring tools until permission is resolved', async () => {
		...
		const bashToolCall: ToolCall = {
			toolName: 'bash',
			toolCallId: 'bash-delay-1',
			arguments: { command: 'echo hi', description: 'Echo test' },
		};
		sdkSession.emit('tool.execution_start', bashToolCall);
		await new Promise((r) => setTimeout(r, 0));

		// No ChatToolInvocationPart should be pushed yet for the bash tool
		const toolPartsBeforePermission = pushedParts.filter(
		...
		const toolPartsAfterPermission = pushedParts.filter(
			(p) => p instanceof ChatToolInvocationPart,
		);
	...
	it('emits languageModelToolInvoked telemetry for each completed tool invocation', async () => {
		...
		const invokedEvents = recorder.events.filter(
			(e) => e.name === 'languageModelToolInvoked',
		);
		const invokedSnapshot = invokedEvents.map((e) => ({
			...
			hasInvocationTimeMs:
				typeof e.measurements?.invocationTimeMs === 'number',
		}));
		expect(invokedSnapshot).toEqual([
			{
				result: 'success',
				chatSessionId: 'copilotcli:/test-session',
				toolId: 'bash',
				toolExtensionId: undefined,
				toolSourceKind: 'copilotCli',
				hasInvocationTimeMs: true,
			},
			{
				result: 'error',
				chatSessionId: 'copilotcli:/test-session',
			...
			},
			{
				result: 'success',
				chatSessionId: 'copilotcli:/test-session',
				toolId: 'completion_tool',
				toolExtensionId: undefined,
				toolSourceKind: 'copilotCli',
				hasInvocationTimeMs: false,
			},
		]);
	});
	...
	it('uses remote permission responses when Mission Control is active', async () => {
		...
		sdkSession.send = async () => {
			permissionResult = await sdkSession.emitPermissionRequest({
				...
				commands: [
					{ identifier: 'echo "Hello world"', readOnly: false },
				],
				intention: 'Run command',
				...
				if (
					...
					return await new Promise((resolve) => {
						token?.onCancellationRequested(() =>
							resolve({
								content: [new LanguageModelTextPart('no')],
							}),
						);
					});
				}
		...
		const remoteState = {
			...
			mcSessionResource: Uri.file(
				'/workspace',
			) as unknown as import('vscode').Uri,
		};
		Object.defineProperty(session, '_mcState', {
			value: remoteState,
			configurable: true,
		});
				...
				getPendingCommands: async () => [
					{
						...
						content: JSON.stringify({
							promptId: 'remote-permission-tool',
							approved: true,
							scope: 'once',
						}),
						state: 'in_progress',
		...
		const confirmationToolCalls = invokeToolSpy.mock.calls.filter(
			(call) =>
				call[0] === 'vscode_get_confirmation' ||
				call[0] === 'vscode_get_terminal_confirmation',
		);
	...
	it('uses remote ask user responses when Mission Control is active', async () => {
		...
		Object.defineProperty(session, '_userQuestionHandler', {
			value: {
				...
				async askUserQuestion(
					_question: IQuestion,
					_toolInvocationToken: ChatParticipantToolToken,
					token: CancellationToken,
					_toolCallId?: string,
				): Promise<IQuestionAnswer | undefined> {
					localPromptToken = token;
					return await new Promise<IQuestionAnswer | undefined>(
						(resolve) => {
							token.onCancellationRequested(() =>
								resolve(undefined),
							);
		...
		expect(notifiedAnswers).toEqual([
			{
				...
				question: {
					question:
						'What is your favorite VS Code feature or extension?',
					options: [],
	...
	it('reports enabled remote control status when /remote is invoked without arguments', async () => {
		...
		expect(output).toContain(
			'Use the button below to open in your browser, or scan to steer from the GitHub mobile app.',
		);
		expect(output).not.toContain('Scan with GitHub mobile:');
		expect(output).toContain(
			'QR code to open this remote session in GitHub mobile',
		);
	...
	it('shows /remote usage for unsupported arguments', async () => {
		...
		expect(stream.output.join('\n')).toContain(
			'Usage: /remote, /remote on, /remote off',
		);
	});
	...
	it('forwards session.idle to Mission Control so remote running state clears', async () => {
		...
		expect((remoteState.mcEventBuffer[0] as { type: string }).type).toBe(
			'session.idle',
		);
	});
	...
	it('forwards session.title_changed to Mission Control as an ephemeral event', async () => {
		...
		expect(remoteState.mcEventBuffer).toHaveLength(1);
		expect(
			(remoteState.mcEventBuffer[0] as { type: string; ephemeral?: true })
				.type,
		).toBe('session.title_changed');
		expect(
			(remoteState.mcEventBuffer[0] as { ephemeral?: true }).ephemeral,
		).toBe(true);
		expect(
			(remoteState.mcEventBuffer[0] as { data: { title: string } }).data
				.title,
	...
	it('prefers existing session history over the current /remote prompt when deriving the Mission Control title', async () => {
		...
		await expect(
			(session as any)._getMissionControlSessionTitle(),
		).resolves.toBe('hey');
	});
		...
		vi.spyOn(sdkSession, 'getEvents').mockReturnValue([
			{
				...
				data: {
					content:
						'/remote <reminder>IMPORTANT: hidden context</reminder><attachments><attachment id="microsoft/vscode-tools">repo</attachment></attachments><userRequest></userRequest>',
				},
			},
		] as any);
	...
	it('sanitizes hidden prompt markup before forwarding user messages to Mission Control', async () => {
		...
		expect(remoteState.mcEventBuffer).toHaveLength(1);
		expect(
			(remoteState.mcEventBuffer[0] as { data: { content: string } }).data
				.content,
		).toBe('/remote');
	});
	...
	it('strips shell tool descriptions before forwarding tool starts to Mission Control', async () => {
		...
		expect(
			(
				remoteState.mcEventBuffer[0] as {
					data: {
						arguments: { command: string; description?: string };
					};
				}
			).data.arguments,
		).toEqual({ command: 'echo hello' });
	});
	...
	it('strips task descriptions before forwarding tool starts to Mission Control', async () => {
		...
		).toEqual({ prompt: 'Run echo', agent_type: 'task' });
	});
	...
	it('does not forward report_intent tool events to Mission Control', async () => {
		...
		expect(
			(remoteState.mcEventBuffer[0] as { data: { toolName: string } })
				.data.toolName,
		).toBe('bash');
	});
	...
	it('forwards command-sourced user messages and acknowledges the command with the echoed turn', async () => {
		...
		expect(
			(
				remoteState.mcEventBuffer[1] as {
					type: string;
		...
		expect(
			(remoteState.mcEventBuffer[1] as { parentId: string | null })
				.parentId,
		).toBe('remote-command-message');
	});
	...
	it('announces remote control disabled to Mission Control before detaching locally', async () => {
		...
		const remoteState = {
			...
			mcCompletedCommandIds: [],
			mcPendingPermissionRequests: new Map([
				['prompt-1', { resolve: pendingRequest }],
			]),
			mcFlushInterval: undefined,
			mcPollInterval: undefined,
			mcLastEventId: null,
		...
		expect(pendingRequest).toHaveBeenCalledWith({
	...
	it('immediately pushes invocation messages for non-permission-requiring tools like MCP', async () => {
		...
		const toolParts = pushedParts.filter(
			(p) => p instanceof ChatToolInvocationPart,
		);
	...
	it('flushes delayed invocation messages when assistant message arrives', async () => {
		...
		sdkSession.emit('tool.execution_start', {
			toolName: 'bash',
			toolCallId: 'bash-flush-1',
			arguments: { command: 'ls', description: 'List' },
		});
		await new Promise((r) => setTimeout(r, 0));

		expect(
			pushedParts.filter((p) => p instanceof ChatToolInvocationPart),
		).toHaveLength(0);

		// Emit an assistant message delta - should flush
		sdkSession.emit('assistant.message_delta', {
			deltaContent: 'Hello',
			messageId: 'msg-1',
		});
		await new Promise((r) => setTimeout(r, 0));
	...
	describe('steering (sending messages to a busy session)', () => {
		it('allows steering after an earlier failed request', async () => {
			...
			expect(sdkSession.lastSendOptions?.prompt).toBe(
				'Steer after failure',
			);
			...
			const steeringAttachments = [
				{
					type: 'file' as const,
					path: attachedFilePath,
					displayName: 'steering-file.ts',
				},
			];
			const steeringRequest = session.handleRequest(
				{ id: 'req-2', toolInvocationToken: undefined as never },
				{ prompt: 'Use that file' },
				steeringAttachments as any,
				undefined,
				authInfo,
				CancellationToken.None,
			);
		...
		it('updates the toolInvocationToken on each request including steering', async () => {
			...
			const token1 = {
				toString: () => 'token-1',
			} as unknown as ChatParticipantToolToken;
			const token2 = {
				toString: () => 'token-2',
			} as unknown as ChatParticipantToolToken;
	...
	describe('exit_plan_mode.requested', () => {
		it('does not attach the exit_plan_mode.requested handler when plan exit mode is disabled', async () => {
			...
			sdkSession.send = async (options: any) => {
				sdkSession.lastSendOptions = options;
				expect(
					sdkSession.onHandlers.get('exit_plan_mode.requested')
						?.size ?? 0,
				).toBe(0);
				sdkSession.emit('assistant.turn_start', {});
				sdkSession.emit('assistant.message', {
		...
		function setupSendWithExitPlanMode(
			data: { summary: string; actions?: string[] },
			resultHolder: { value: unknown },
		) {
			sdkSession.send = async (options: any) => {
				sdkSession.emit('assistant.turn_start', {});
				sdkSession.emit('assistant.message', {
				...
				resultHolder.value =
					await sdkSession.emitExitPlanModeRequest(data);
				sdkSession.emit('assistant.turn_end', {});
			};
		}
		...
		it('auto-approves with exit_only action when choices include "exit_only" but not "autopilot" or "interactive"', async () => {
			const result = { value: undefined as unknown };
			setupSendWithExitPlanMode(
				{ summary: 'Plan ready', actions: ['exit_only'] },
				result,
			);
			const session = await createSession();
			session.setPermissionLevel('autopilot');
		...
		it('denies when no toolInvocationToken is present in non-autopilot mode', async () => {
			const result = { value: undefined as unknown };
			setupSendWithExitPlanMode(
				{ summary: 'Plan ready', actions: ['autopilot'] },
				result,
			);
			const session = await createSession();
		...
		it('approves when user confirms via user question handler in non-autopilot mode', async () => {
			...
			await session.handleRequest(
				{ id: '', toolInvocationToken: mockToken },
				{ prompt: 'Plan' },
				[],
				undefined,
				authInfo,
		...
		it('sets autoApproveEdits when user confirms with autoApprove permission level', async () => {
			const result = { value: undefined as unknown };
			setupSendWithExitPlanMode(
				{ summary: 'Here is the plan', actions: ['exit_only'] },
				result,
			);
			userQuestionAnswer = {
				selected: ['exit_only'],
	...
	describe('usage reporting', () => {
		it('reports usage from assistant.usage event with per-call tokens', async () => {
			...
			const usageFromEvent = stream.usages.find(
				(u) => u.promptTokens === 200 && u.completionTokens === 80,
			);
		...
		it('reports usage from session.usage_info event immediately', async () => {
			...
			const usageFromInfo = stream.usages.find(
				(u) => u.promptTokens === 500,
			);
		...
		it('populates promptTokenDetails in assistant.usage event when usage_info was previously received', async () => {
			...
			const assistantUsage = stream.usages.find(
				(u) => u.promptTokens === 400 && u.completionTokens === 60,
			);
			expect(assistantUsage).toBeDefined();
			expect(assistantUsage!.promptTokenDetails).toBeDefined();
			expect(assistantUsage!.promptTokenDetails!.length).toBeGreaterThan(
				0,
			);
		});
