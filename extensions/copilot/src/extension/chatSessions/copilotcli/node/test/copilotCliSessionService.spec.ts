...
class MockLocalSession {
	static async fromEvents(events: readonly { type: string }[]): Promise<{}> {
		const unknownEvent = events.find(
			(event) => event.type === 'custom.unknown',
		);
		if (unknownEvent) {
			throw new Error(
				`Unknown event type: ${unknownEvent.type}. Failed to deserialize session.`,
			);
		}
		return {};
	}
...
class NullChatSessionWorkspaceFolderService extends mock<IChatSessionWorkspaceFolderService>() {
	...
	override clearWorkspaceChanges: IChatSessionWorkspaceFolderService['clearWorkspaceChanges'] =
		vi.fn((_sessionIdOrFolderUri: string | Uri) => []);
}

class NullChatSessionWorktreeService extends mock<IChatSessionWorktreeService>() {
	override getWorktreeProperties: IChatSessionWorktreeService['getWorktreeProperties'] =
...
class NullCustomSessionTitleService implements ICustomSessionTitleService {
	...
	async getCustomSessionTitle(
		sessionId: string,
	): Promise<string | undefined> {
		return this.titles.get(sessionId);
	}
	async setCustomSessionTitle(
		sessionId: string,
		title: string,
	): Promise<void> {
	...
	async generateSessionTitle(
		_sessionId: string,
		_request: { prompt?: string; command?: string },
	): Promise<string | undefined> {
		return undefined;
	}
}
...
describe('CopilotCLISessionService', () => {
	...
	beforeEach(async () => {
		...
		instantiationService = {
			invokeFunction(
				fn: (accessor: unknown, ...args: any[]) => any,
				...args: any[]
			): any {
		...
		service = disposables.add(
			new CopilotCLISessionService(
				...
				delegationService,
				metadataStore,
				new NullAgentSessionsWorkspace(),
				new NullChatSessionWorkspaceFolderService(),
				new NullChatSessionWorktreeService(),
				new NoopOTelService(
					resolveOTelConfig({
						env: {},
						extensionVersion: '0.0.0',
		...
		manager =
			(await service.getSessionManager()) as unknown as MockCliSdkSessionManager;
	});
	...
	it('falls back to a compatibility auto-mode manager when the SDK export is not constructable', async () => {
		const sdk = {
			getPackage: vi.fn(async () => ({
				...
				acquireAutoModeSession: vi.fn(async () => {
					throw new Error('unexpected auto-mode acquire');
				}),
				refreshAutoModeSession: vi.fn(async () => {
					throw new Error('unexpected auto-mode refresh');
				}),
		...
		const delegationService =
			new (class extends mock<IChatDelegationSummaryService>() {
				...
				override async summarize(): Promise<string | undefined> {
					return undefined;
				}
			})();
		const localService = disposables.add(
			new CopilotCLISessionService(
				...
				new NullCustomSessionTitleService(),
				configurationService,
				new MockSkillLocations(),
				delegationService,
				new MockChatSessionMetadataStore(),
		...
		const localManager =
			(await localService.getSessionManager()) as unknown as MockCliSdkSessionManager & {
				opts: { autoModeManager: Record<string, unknown> };
			};

		expect(localManager.opts.autoModeManager).toEqual(
	...
	describe('CopilotCLISessionService.getChatHistory', () => {
		it('refreshes cached custom agent mode instructions when custom agents change', async () => {
			const agentUri = URI.file(
				'/workspace/.github/agents/review.agent.md',
			);
			promptsService.setFileContent(
				agentUri,
				'---\nname: Review\n---\nFirst instructions',
			);
			promptsService.setCustomAgents([
				createCustomAgent(agentUri, 'Review'),
			]);

			const firstSessionId = 'history-agent-first';
			await metadataStore.updateRequestDetails(firstSessionId, [
				{
			...
			const firstSession = new MockCliSdkSession(
				firstSessionId,
			...
			const firstHistory = await service.getChatHistory(
				{
					sessionId: firstSessionId,
					workspace: workspaceInfoFor(undefined),
				},
				CancellationToken.None,
			);
			const firstRequest = firstHistory.find(
				(turn) => turn instanceof ChatRequestTurn2,
			);
			if (!(firstRequest instanceof ChatRequestTurn2)) {
				throw new Error('Expected a request turn');
			}
			expect(firstRequest.modeInstructions2?.content).toBe(
				'First instructions',
			);
			...
			await metadataStore.updateRequestDetails(secondSessionId, [
				{
			...
			const secondSession = new MockCliSdkSession(
				secondSessionId,
			...
			secondSession.events.push(
				...
				},
				{
					...
					type: 'user.message',
					data: {
						content: 'Review the second change',
						attachments: [],
					},
				},
			);
			...
			const secondHistory = await service.getChatHistory(
				{
			...
			const secondRequest = secondHistory.find(
				(turn) => turn instanceof ChatRequestTurn2,
			);
		...
		it('issues a single getCustomAgents call when multiple turns resolve in parallel after agent change', async () => {
			const agentUri = URI.file(
				'/workspace/.github/agents/parallel.agent.md',
			);
			promptsService.setFileContent(
				agentUri,
				'---\nname: Parallel\n---\nInstructions',
			...
			promptsService.setCustomAgents([
				createCustomAgent(agentUri, 'Parallel'),
			]);
			...
			const session = new MockCliSdkSession(
				sessionId,
				new Date('2024-01-01T00:00:00.000Z'),
			);
			session.events.push({
			...
			for (const t of turns) {
				session.events.push({
					id: t.copilotRequestId,
					type: 'user.message',
					data: { content: 'm', attachments: [] },
				});
			}
			...
			await service.getChatHistory(
				{ sessionId, workspace: workspaceInfoFor(undefined) },
				CancellationToken.None,
			);
			promptsService.setCustomAgents([
				createCustomAgent(agentUri, 'Parallel'),
			...
			const getCustomAgentsSpy = vi.spyOn(
				promptsService,
				'getCustomAgents',
			);
			await service.getChatHistory(
	...
	describe('CopilotCLISessionService.createSession', () => {
		it('get session will return the same session created using createSession', async () => {
			const session = await service.createSession(
				{ model: 'gpt-test', ...sessionOptionsFor(URI.file('/tmp')) },
				CancellationToken.None,
			);
			...
			const existingSession = await service.getSession(
				{
					...
					...sessionOptionsFor(URI.file('/tmp')),
				},
				CancellationToken.None,
			);
		...
		it('get session will return new once previous session is disposed', async () => {
			...
			expect(existingSession?.object.sessionId).toBe(
		...
		it('preserves prompt variable context separately from the VS Code Copilot CLI identity system message', async () => {
			const variableContext =
				'Resolved template variables are available here.';
			vi.spyOn(
				NullPromptVariablesService.prototype,
				'buildTemplateVariablesContext',
			).mockReturnValue(variableContext);
			const createSessionSpy = vi.spyOn(manager, 'createSession');
			await service.createSession(
				{ model: 'gpt-test', ...sessionOptionsFor(URI.file('/tmp')) },
				CancellationToken.None,
			...
			const systemMessage =
				createSessionSpy.mock.calls[0][0].systemMessage;
	...
	describe('CopilotCLISessionService.getSession', () => {
		it('passes reasoningEffort to session manager when creating a new session', async () => {
			...
			manager.sessions.set(
				targetId,
				new MockCliSdkSession(targetId, new Date()),
			);
	...
	describe('CopilotCLISessionService.getSession concurrency & locking', () => {
		it('concurrent getSession calls for same id create only one wrapper', async () => {
			...
			const getSessionSpy = vi.fn(
				(
					opts: SessionOptions & { sessionId: string },
					writable: boolean,
				) => {
					// Introduce delay to force overlapping acquire attempts
					return new Promise((resolve) =>
						setTimeout(
							() => resolve(originalGetSession(opts, writable)),
							20,
						),
					);
				},
			...
			manager.getSession =
				getSessionSpy as unknown as typeof manager.getSession;

			const promises: Promise<
				IReference<ICopilotCLISession> | undefined
			>[] = [];
			for (let i = 0; i < 10; i++) {
				promises.push(
					service.getSession(
						{ sessionId: targetId, ...sessionOptionsFor() },
						CancellationToken.None,
					),
				);
			}
			...
			(first.object as CopilotCLISession).add(
				toDisposable(() => {
					sentinelDisposed = true;
				}),
			);
		...
		it('getSession for different ids does not block on mutex for another id', async () => {
			...
			manager.sessions.set(
				slowId,
				new MockCliSdkSession(slowId, new Date()),
			);
			manager.sessions.set(
				fastId,
				new MockCliSdkSession(fastId, new Date()),
			);
			...
			const slowPromise = service
				.getSession(
					{ sessionId: slowId, ...sessionOptionsFor() },
					CancellationToken.None,
				)
				.then(() => 'slow');
			const fastPromise = service
				.getSession(
					{ sessionId: fastId, ...sessionOptionsFor() },
					CancellationToken.None,
				)
				.then(() => 'fast');
		...
		it('session only fully disposes after all acquired references dispose', async () => {
			...
			for (let i = 0; i < 5; i++) {
				sessions.push(
					(await service.getSession(
						{ sessionId: id, ...sessionOptionsFor() },
						CancellationToken.None,
					))!,
				);
			}
			...
			(lastSession.object as CopilotCLISession).add(
				toDisposable(() => {
					sentinelDisposed = true;
				}),
			);
	...
	describe('CopilotCLISessionService.getSession missing', () => {
		it('returns undefined when underlying manager has no session', async () => {
			const session = await service.getSession(
				{ sessionId: 'does-not-exist', ...sessionOptionsFor() },
				CancellationToken.None,
			);
	...
	describe('CopilotCLISessionService.renameSession', () => {
		it('renames an inactive session through copilot/sdk', async () => {
			...
			manager.sessions.set(
				sessionId,
				new MockCliSdkSession(sessionId, new Date()),
			);

			await service.renameSession(sessionId, 'Renamed From VS Code');

			expect(manager.sessions.get(sessionId)?.title).toBe(
				'Renamed From VS Code',
			);
			expect(
				await service.getSessionTitle(
					sessionId,
					CancellationToken.None,
				),
			).toBe('Renamed From VS Code');
		});

		it('renames an active wrapped session through copilot/sdk', async () => {
			const session = await service.createSession(
				{
					sessionId: 'rename-active',
					...sessionOptionsFor(URI.file('/tmp')),
				},
				CancellationToken.None,
			);
			...
			expect(manager.sessions.get(session.object.sessionId)?.title).toBe(
				'Wrapped Session Name',
			);
			expect(
				await service.getSessionTitle(
		...
		it('updates session summaries through copilot/sdk for untitled sessions', async () => {
			...
			expect(manager.sessions.get(sessionId)?.summary).toBe(
				'Generated Summary',
			);
			expect(
				await service.getSessionTitle(
		...
		it('syncs staged titles for newly created vscode sessions into copilot/sdk', async () => {
			...
			await (
				service as unknown as {
					customSessionTitleService: ICustomSessionTitleService;
				}
			).customSessionTitleService.setCustomSessionTitle(
				sessionId,
				'Staged Session Title',
			);

			const session = await service.createSession(
				{ sessionId, ...sessionOptionsFor(URI.file('/tmp')) },
				CancellationToken.None,
			);
		...
		it('keeps an established session list title stable while a new request is pending', async () => {
			...
			(
				session!.object as unknown as {
					_pendingPrompt: string | undefined;
				}
			)._pendingPrompt = 'Latest in-flight request';

			const sessions = await service.getAllSessions(
				CancellationToken.None,
			);
			expect(sessions.find((item) => item.id === sessionId)?.label).toBe(
	...
	describe('CopilotCLISessionService.tryGetPartialSesionHistory', () => {
		it('reconstructs history from persisted files', async () => {
			tempStateHome = await mkdtemp(
				join(tmpdir(), 'copilot-cli-session-service-'),
			);
			...
			const partialService = disposables.add(
				new CopilotCLISessionService(
					logService,
					sdk,
					instantiationService,
			...
			await writeNodeFile(
				join(sessionDir.fsPath, 'events.jsonl'),
				[
					JSON.stringify({
						id: '1',
						type: 'session.start',
						...
						data: {
							...
							context: {
								cwd: URI.file('/workspace/project').fsPath,
								gitRoot: URI.file('/workspace/repo').fsPath,
								repository: URI.file('/workspace/repo').fsPath,
							},
						},
					}),
					JSON.stringify({
					...
					JSON.stringify({
						...
						parentId: '2',
						data: { content: 'Recovered history' },
					}),
				].join('\n'),
			);

			const partialHistory =
				await partialService.tryGetPartialSessionHistory(sessionId);

			expect(partialHistory).toBeDefined();
			expect(partialHistory).toHaveLength(2);
			expect(
				partialService.getSessionWorkingDirectory(sessionId)?.fsPath,
			).toBe(URI.file('/workspace/project').fsPath);
		});
			...
			await writeNodeFile(
				...
				[
					...
					JSON.stringify({
						...
						parentId: '1',
						data: {
							content: 'First call fills cache',
							attachments: [],
						},
					}),
				].join('\n'),
		...
		it('returns undefined when the events file does not exist', async () => {
			...
			const result = await service.tryGetPartialSessionHistory(
	...
	describe('CopilotCLISessionService.getAllSessions', () => {
		...
		it('falls back to partial session data when getSession fails with an unknown event type', async () => {
			...
			const partialManager =
				(await partialService.getSessionManager()) as unknown as MockCliSdkSessionManager;
			...
			const sessions = await partialService.getAllSessions(
				CancellationToken.None,
			);
		...
		it('does not emit session when summary is truncated and no user turns exist', async () => {
			...
			expect(sessions[0].label).toBe(
				'Summary without user turns <current_dateti...',
			);
		});
	});
	...
	describe('CopilotCLISessionService.deleteSession', () => {
		...
		it('fires onDidDeleteSession with the session id', async () => {
			...
			disposables.add(session);
			disposables.add(
				service.onDidDeleteSession((deletedId) =>
		...
		it('clears partial session history cache and working directory on delete', async () => {
			...
			const partialHistories = (service as any)
				._partialSessionHistories as Map<string, readonly unknown[]>;
			const workingDirs = (service as any)
				._sessionWorkingDirectories as Map<string, Uri | undefined>;
	...
	describe('CopilotCLISessionService.getSession cache clearing', () => {
		it('clears partial session history when reusing an existing active session', async () => {
			...
			const reused = await service.getSession(
				{ sessionId: id, ...sessionOptionsFor() },
				CancellationToken.None,
			);
	...
	describe('CopilotCLISessionService.label generation', () => {
		...
		it('uses clean summary from metadata without loading the full session', async () => {
			...
			s.events.push({
				type: 'user.message',
				data: { content: 'Fix the login bug in auth.ts' },
				timestamp: Date.now().toString(),
			});
			manager.sessions.set(s.sessionId, s);
		...
		it('falls through to session load when summary contains angle bracket', async () => {
			...
			s.events.push({
				type: 'user.message',
				data: { content: 'Fix the bug in the parser' },
				timestamp: Date.now().toString(),
			});
			manager.sessions.set(s.sessionId, s);
		...
		it('uses cached label on second call without loading session again', async () => {
			...
			s.events.push({
				type: 'user.message',
				data: { content: 'Refactor the tests' },
				timestamp: Date.now().toString(),
			});
			manager.sessions.set(s.sessionId, s);

			// First call - loads session and caches the label
			const sessions1 = await service.getAllSessions(
				CancellationToken.None,
			);
			...
			const sessions2 = await service.getAllSessions(
				CancellationToken.None,
			);
		...
		it('uses metadata summary over stale internal label cache', async () => {
			...
			s.events.push({
				type: 'user.message',
				data: { content: 'Original label from events' },
				timestamp: Date.now().toString(),
			});
			manager.sessions.set(s.sessionId, s);
			...
			expect(sessions1.find((i) => i.id === 'priority1')?.label).toBe(
				'Original label from events',
			);
			...
			expect(sessions2.find((i) => i.id === 'priority1')?.label).toBe(
				'Original label from events',
			);
		});
		...
		it('populates cache after loading session for label', async () => {
			...
			s.events.push({
				type: 'user.message',
				data: { content: 'Add unit tests for auth' },
				timestamp: Date.now().toString(),
			});
			manager.sessions.set(s.sessionId, s);
			...
			const labelCache = (service as any)._sessionLabels as Map<
				string,
				string
			>;
	...
	describe('CopilotCLISessionService.forkSession', () => {
		it('delegates to sessionManager.forkSession and returns the new session id', async () => {
			...
			manager.sessions.set(
				sourceId,
				new MockCliSdkSession(sourceId, new Date()),
			);
			const forkSpy = vi.spyOn(manager, 'forkSession');

			const newId = await service.forkSession(
				{
					sessionId: sourceId,
					requestId: undefined,
					workspace: workspaceInfoFor(URI.file('/workspace')),
				},
				CancellationToken.None,
			);
		...
		it('stores forked session metadata via storeForkedSessionMetadata', async () => {
			...
			const storeMetadataSpy = vi.spyOn(
				metadataStore,
			...
			const localManager =
				(await localService.getSessionManager()) as unknown as MockCliSdkSessionManager;
			localManager.sessions.set(
				sourceId,
				new MockCliSdkSession(sourceId, new Date()),
			);

			const newId = await localService.forkSession(
				{
					sessionId: sourceId,
					requestId: undefined,
					workspace: workspaceInfoFor(URI.file('/workspace')),
		...
		it('fires onDidCreateSession with the forked session id and title', async () => {
			...
			disposables.add(
				service.onDidCreateSession((item) => created.push(item)),
			);
		...
		it('passes toEventId to sessionManager.forkSession when requestId matches a stored copilot request id', async () => {
			...
			const sdk = {
				...
				getRequestId: vi.fn(() => ({
					vscodeRequestId: 'vsc-req-1',
					copilotRequestId: 'sdk-event-1',
				})),
			} as unknown as ICopilotCLISDK;
			const services = disposables.add(
				createExtensionUnitTestingServices(),
			...
			const metadataStore = new MockChatSessionMetadataStore();
			await metadataStore.updateRequestDetails(sourceId, [
				{
					vscodeRequestId: 'vsc-req-1',
					copilotRequestId: 'sdk-event-1',
					toolIdEditMap: {},
			...
			await localService.forkSession(
				{
					sessionId: sourceId,
					requestId: 'vsc-req-1',
					workspace: workspaceInfoFor(URI.file('/workspace')),
