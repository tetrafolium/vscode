...
export class MockCliSdkSession {
	...
	public readonly updateSessionSummary = async (
		summary: string,
	): Promise<void> => {
		if (!this.name) {
			this.title = summary;
		...
		return Promise.resolve(
			session
				? {
						...
						modifiedTime: session.startTime,
...
export class NullCopilotCLIMCPHandler implements ICopilotCLIMCPHandler {
	...
	async loadMcpConfig(
		...
		mcpConfig:
			| Record<string, NonNullable<SessionOptions['mcpServers']>[string]>
			| undefined;
...
export class NullCopilotCLIModels implements ICopilotCLIModels {
	_serviceBrand: undefined;
	async resolveModel(_modelId: string): Promise<string | undefined> {
		return undefined;
	}
	async getDefaultModel(): Promise<string | undefined> {
		return undefined;
	}
	async setDefaultModel(_modelId: string | undefined): Promise<void> {
		return;
	}
	async getModels(): Promise<CopilotCLIModelInfo[]> {
		return [];
	}
	registerLanguageModelChatProvider(): void {
		return;
	}
}
