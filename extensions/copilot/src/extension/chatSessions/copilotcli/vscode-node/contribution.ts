...
export class CopilotCLIContrib extends Disposable {
	constructor(
		...
		const httpServer = this._register(
			new InProcHttpServer(logger, this.sessionTracker),
		);
		...
		this._register(
			httpServer.onDidClientDisconnect((sessionId) => {
				diffState.closeAllForSession(sessionId);
			}),
		);

		// Clean up any stale lockfiles from previous sessions
		cleanupStaleLockFiles(logger)
			.then((cleanedCount) => {
				if (cleanedCount > 0) {
					logger.info(
						`Cleaned up ${cleanedCount} stale lock file(s).`,
					);
				}
			})
			.catch((err) => {
		...
		try {
			const { serverUri, headers } = await httpServer.start({
				...
				registerPushNotifications: () => {
					for (const d of registerSelectionChangedNotification(
						logger,
						httpServer,
						selectionState,
					)) {
					...
					for (const d of registerDiagnosticsChangedNotification(
						logger,
						httpServer,
					)) {
						this._register(d);
			...
			this._register(
				vscode.workspace.onDidChangeWorkspaceFolders(() => {
					...
					logger.info(
						'Workspace folders changed, lock file updated.',
					);
				}),
			);
