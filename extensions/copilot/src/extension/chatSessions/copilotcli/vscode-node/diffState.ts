...
function isDiffTab(
	tab: vscode.Tab,
): tab is vscode.Tab & { input: vscode.TabInputTextDiff } {
...
export class DiffStateManager {
	...
	register(diff: ActiveDiff): void {
		this._logger.trace(
			`[DIFF] registerActiveDiff: tabName=${diff.tabName}, diffId=${diff.diffId}, mapSize=${this._activeDiffs.size}`,
		);
		this._activeDiffs.set(diff.diffId, diff);
		this._logger.trace(
			`[DIFF] After register, mapSize=${this._activeDiffs.size}`,
		);
		this._updateContext();
	}
	...
	unregister(diffId: string): void {
		...
		this._logger.trace(
			`[DIFF] unregisterActiveDiff: diffId=${diffId}, found=${!!diff}, mapSize=${this._activeDiffs.size}`,
		);
		this._activeDiffs.delete(diffId);
		this._logger.trace(
			`[DIFF] After unregister, mapSize=${this._activeDiffs.size}`,
		);
		this._updateContext();
	}
	...
	getByTab(tab: vscode.Tab): ActiveDiff | undefined {
		if (!isDiffTab(tab)) {
			this._logger.trace(
				'[DIFF] getActiveDiffByTab: tab is not a diff tab',
			);
			return undefined;
		}
		const modifiedUri = tab.input.modified.toString();
		this._logger.trace(
			`[DIFF] getActiveDiffByTab: looking for modifiedUri=${modifiedUri}, mapSize=${this._activeDiffs.size}`,
		);
		for (const diff of this._activeDiffs.values()) {
			this._logger.trace(
				`[DIFF]   checking diff.modifiedUri=${diff.modifiedUri.toString()}`,
			);
	...
	getForCurrentTab(): ActiveDiff | undefined {
		...
		this._logger.trace(
			`[DIFF] getActiveDiffForCurrentTab: activeTab=${activeTab?.label ?? 'none'}`,
		);
		if (activeTab) {
	...
	closeAllForSession(sessionId: string): void {
		...
		this._logger.info(
			`[DIFF] Closing ${toClose.length} diff(s) for disconnected session ${sessionId}`,
		);
	...
	private _updateContext(): void {
		const activeTab = vscode.window.tabGroups.activeTabGroup.activeTab;
		const isActiveDiff = activeTab
			? this.getByTab(activeTab) !== undefined
			: false;
		void vscode.commands
			.executeCommand(
				'setContext',
				'github.copilot.chat.copilotCLI.hasActiveDiff',
				isActiveDiff,
			)
			.then(undefined, (err) => {
				this._logger.error(
					`[DIFF] Failed to update hasActiveDiff context: ${String(err)}`,
				);
			});
	}
}
