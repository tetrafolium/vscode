...
export function registerOpenDiffTool(
	server: McpServer,
	logger: ILogger,
	...
	const schema = {
		...
		new_file_contents: z
			.string()
			.describe('The new file contents to compare against'),
	...
	server.registerTool(
		...
		async (args: {
			...
			logger.info(
				`[DIFF] ===== OPEN_DIFF START ===== file=${original_file_path}, tab=${tab_name}`,
			);
			try {
				...
				const originalUri = createReadonlyUri(
					original_file_path,
					`original-${uniqueSuffix}`,
				);
				const newUri = createReadonlyUri(
					original_file_path,
					`modified-${uniqueSuffix}`,
				);
				...
				const result = await new Promise<{
					...
					const cleanup = () => {
						logger.info(
							`[DIFF] cleanup() called, cleanedUp=${cleanedUp}, diffId=${diffId}`,
						);
						if (cleanedUp) {
							return;
						}
					...
					const closeDiffTab = async () => {
						logger.info(
							`[DIFF] closeDiffTab() looking for modifiedUri=${newUri.toString()}`,
						);
								...
								if (
									...
									if (tabModifiedUri === newUri.toString()) {
										...
										try {
											...
											logger.info(
												`[DIFF] Tab close error: ${e instanceof Error ? e.message : String(e)}`,
											);
										}
										return;
									}
					...
					const wrappedResolve = (result: {
						...
						logger.info(
							`[DIFF] wrappedResolve() status=${result.status}, trigger=${result.trigger}`,
						);
						cleanup();
						void closeDiffTab();
						resolve(result);
					...
					disposables.push(
						vscode.window.tabGroups.onDidChangeTabs((event) => {
							logger.info(
								`[DIFF] onDidChangeTabs: opened=${event.opened.length}, closed=${event.closed.length}, changed=${event.changed.length}, myDiffId=${diffId}`,
							);
							for (const closedTab of event.closed) {
								if (
									closedTab.input instanceof
									vscode.TabInputTextDiff
								) {
									logger.info(
										`[DIFF] closedTab modifiedUri=${closedTab.input.modified.toString()}`,
									);
								}
								const diff = diffState.getByTab(closedTab);
								logger.info(
									`[DIFF] getActiveDiffByTab returned: ${diff?.diffId ?? 'undefined'}`,
								);
								if (diff && diff.diffId === diffId) {
									logger.info(
										`[DIFF] MATCH - Tab closed manually: ${tab_name}`,
									);
									cleanup();
					...
					logger.info(
						'[DIFF] Setup complete, waiting for user action',
					);
				});

				logger.info(
					`[DIFF] ===== OPEN_DIFF END ===== result=${result.status}`,
				);
