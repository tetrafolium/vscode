...
export function registerGetDiagnosticsTool(
	server: McpServer,
	logger: ILogger,
): void {
	const schema = {
		uri: z
			...
			.optional()
			.describe(
				'File URI to get diagnostics for. Optional. If not provided, returns diagnostics for all files.',
			),
	};
	server.registerTool(
		'get_diagnostics',
		...
		async (args: { uri?: string }) => {
			const { uri } = args;
			logger.debug(
				`Getting diagnostics${uri ? ` for: ${uri}` : ' for all files'}`,
			);
			...
			const result = diagnostics
				.map(([fileUri, fileDiagnostics]) => ({
					...
					diagnostics: fileDiagnostics.map((d) => ({
						message: d.message,
						severity:
							vscode.DiagnosticSeverity[d.severity].toLowerCase(),
						range: {
							start: {
