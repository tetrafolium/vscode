...
describe('CopilotCLI SDK Upgrade', function () {
	...
	const copilotSDKPath = path.join(
		extensionPath,
		'node_modules',
		'@github',
		'copilot',
	...
	it('should not contain new native binaries nor removed native binaries', async function () {
		...
		const ripgrepFilesWeCopy = path.join(
			copilotSDKPath,
			'sdk',
			'ripgrep',
			'bin',
		...
		for (const binary of existingBinaries) {
			...
			if (
				binaryName.startsWith('keytar') ||
				binaryName.startsWith('clipboard')
			) {
				continue;
			}
			if (!knownBinaries.has(binary)) {
				errors.push(
					`Unexpected native binary found in Copilot CLI SDK: ${path.relative(copilotSDKPath, binary)}`,
				);
			}
		}
		...
		for (const binary of knownBinaries) {
			...
			if (!existingBinaries.has(binary)) {
				errors.push(
					`Expected native binary missing from Copilot CLI SDK: ${path.relative(copilotSDKPath, binary)}`,
				);
			}
		}
