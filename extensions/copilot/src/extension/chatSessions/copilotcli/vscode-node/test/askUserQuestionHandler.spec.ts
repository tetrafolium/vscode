...
function makeAskQuestionsTool(
	invokeResult: LanguageModelToolResult | undefined,
	resolveInput?: unknown,
): ICopilotTool<unknown> {
	return {
...
function makeToolsService(
	tool: ICopilotTool<unknown> | undefined,
): IToolsService {
	return new (class extends mock<IToolsService>() {
		override invokeTool(
			...
			return (tool as any).invoke(
				options,
				token,
			) as Thenable<LanguageModelToolResult2>;
		}
	})();
}
...
describe('UserQuestionHandler', () => {
	describe('askUserQuestion', () => {
		...
		it('returns undefined when the answer key is missing', async () => {
			...
			const tool = makeAskQuestionsTool(
				new LanguageModelToolResult([
					new LanguageModelTextPart(JSON.stringify(answers)),
				]),
			);
			const handler = makeHandler(tool);
			const result = await handler.askUserQuestion(
		...
		it('passes question text, choices, and freeform flag to the tool', async () => {
			...
			const handler = makeHandler(tool);
			await handler.askUserQuestion(
				question,
				toolInvocationToken,
				CancellationToken.None,
			);
			const invokeArg = (tool.invoke as ReturnType<typeof vi.fn>).mock
				.calls[0][0];
