...
function makeSweAgent(
	name: string,
	description = '',
	displayName?: string,
): Readonly<SweCustomAgent> {
	return {
		name,
...
class MockCopilotCLIAgents extends mock<ICopilotCLIAgents>() {
	...
	setAgents(agents: CLIAgentInfo[]) {
		this._agents = agents;
	}
	override async getAgents(): Promise<readonly CLIAgentInfo[]> {
		return this._agents;
	}
	fireAgentsChanged() {
...
class TestCustomInstructionsService extends MockCustomInstructionsService {
	...
	setAgentInstructionUris(uris: URI[]) {
		this._agentInstructions = uris;
	}
	override getAgentInstructions(): Promise<URI[]> {
...
describe('CopilotCLICustomizationProvider', () => {
	...
	beforeEach(() => {
		...
		provider = disposables.add(
			new CopilotCLICustomizationProvider(
	...
	describe('metadata', () => {
		it('has correct label and icon', () => {
			expect(CopilotCLICustomizationProvider.metadata.label).toBe(
				'Copilot CLI',
			);
			expect(CopilotCLICustomizationProvider.metadata.iconId).toBe(
				'copilot',
			);
		});
		...
		it('supports Agent, Skill, Instructions, Hook, and Plugins types', () => {
			const supported =
				CopilotCLICustomizationProvider.metadata.supportedTypes;
		...
		it('only returns items whose type is in supportedTypes', async () => {
			...
			const supported = new Set(
				CopilotCLICustomizationProvider.metadata.supportedTypes!.map(
					(t) => t.id,
				),
			);
			for (const item of items) {
				expect(
					...
					`item "${item.name}" has type "${item.type.id}" not in supportedTypes`,
				).toBe(true);
			}
		});
		...
		it('does not set groupKey for items with synthetic URIs (vscode infers grouping)', async () => {
			...
			for (const item of builtinItems) {
				expect(
					item.groupKey,
					`item "${item.name}" should not have groupKey (vscode infers)`,
				).toBeUndefined();
			}
		});
	});
	...
	describe('provideChatSessionCustomizations', () => {
		...
		it('returns agents from ICopilotCLIAgents with source URIs', async () => {
			...
			const agentItems = items.filter(
				(i: vscode.ChatSessionCustomizationItem) =>
					i.type === FakeChatSessionCustomizationType.Agent,
			);
		...
		it('uses file URI from sourceUri for file-backed agents', async () => {
			...
			mockCopilotCLIAgents.setAgents([
				makeFileAgentInfo('explore', fileUri, 'Explore agent'),
			]);
		...
		it('uses displayName from agents when available', async () => {
			mockCopilotCLIAgents.setAgents([
				makeAgentInfo('code-review', 'Reviews code', 'Code Review'),
			]);
		...
		it('returns instructions with on-demand groupKey when no applyTo pattern', async () => {
			...
			mockPromptsService.setInstructions([
				makeInstruction(uri, 'copilot-instructions', undefined),
			]);
		...
		it('returns skills', async () => {
			const uri = URI.file(
				'/workspace/.github/skills/lint-check/SKILL.md',
			);
		...
		it('derives skill name from parent directory for SKILL.md files', async () => {
			const uri = URI.file(
				'/workspace/.copilot/skills/my-skill/SKILL.md',
			);
		...
		it('returns all matching types combined', async () => {
			...
			mockPromptsService.setInstructions([
				makeInstruction(
					URI.file('/workspace/.github/b.instructions.md'),
					'b instructions',
					undefined,
				),
			]);
			mockPromptsService.setSkills([
				makeSkill(
					URI.file('/workspace/.github/skills/c/SKILL.md'),
					'c',
				),
			]);
			mockPromptsService.setHooks([
				makeHook(URI.file('/workspace/.copilot/hooks/pre-commit.json')),
			]);
			mockPromptsService.setPlugins([
				makePlugin(URI.file('/workspace/.copilot/plugins/my-plugin')),
			]);
		...
		it('strips .json extension from hook file name', async () => {
			mockPromptsService.setHooks([
				makeHook(
					URI.file('/workspace/.copilot/hooks/security-checks.json'),
				),
			]);
		...
		it('returns multiple hooks', async () => {
			mockPromptsService.setHooks([
				...
				makeHook(
					URI.file('/workspace/.copilot/hooks/diagnostics.json'),
				),
			]);
			...
			const hookItems = items.filter(
				(i: vscode.ChatSessionCustomizationItem) =>
					i.type === FakeChatSessionCustomizationType.Hook,
			);
	...
	describe('instruction groupKeys and badges', () => {
		...
		it('emits agent instructions not in chatPromptFileService.instructions', async () => {
			...
			expect(
				instrItems.every((i) => i.groupKey === 'agent-instructions'),
			).toBe(true);
			expect(instrItems.map((i) => i.name)).toEqual([
				'AGENTS.md',
				'CLAUDE.md',
		...
		it('discovers AGENTS.md and CLAUDE.md from workspace roots via filesystem', async () => {
			...
			const testProvider = disposables.add(
				new CopilotCLICustomizationProvider(
					mockCopilotCLIAgents,
					mockCustomInstructionsService,
					mockPromptsService,
			...
			const items = await testProvider.provideChatSessionCustomizations(
				testSessionResource,
				undefined!,
			);
			const instrItems = items.filter(
		...
		it('uses context-instructions groupKey with badge for instructions with applyTo pattern', async () => {
			...
			mockPromptsService.setInstructions([
				makeInstruction(uri, 'style instructions', 'src/**/*.ts'),
			]);
		...
		it('uses "always added" badge when applyTo is **', async () => {
			...
			mockPromptsService.setInstructions([
				makeInstruction(uri, 'global instructions', '**'),
			]);
		...
		it('categorizes mixed instructions correctly', async () => {
			...
			const contextUri = URI.file(
				'/workspace/.github/style.instructions.md',
			);
			const onDemandUri = URI.file(
				'/workspace/.github/refactor.instructions.md',
			);
			mockPromptsService.setInstructions([
				makeInstruction(agentUri, 'copilot instructions', undefined),
				makeInstruction(contextUri, 'style instructions', 'src/**'),
				makeInstruction(
					onDemandUri,
					'refactor instructions',
					undefined,
			...
			mockPromptsService.setFileContent(
				contextUri,
				"---\napplyTo: 'src/**'\n---\nStyle rules.",
			);
			mockPromptsService.setFileContent(
			...
			const agent = instrItems.find(
				(i) => i.groupKey === 'agent-instructions',
			);
			const context = instrItems.find(
				(i) => i.groupKey === 'context-instructions',
			);
			const onDemand = instrItems.find(
				(i) => i.groupKey === 'on-demand-instructions',
			);
		...
		it('falls back to on-demand-instructions when file has no YAML header', async () => {
			...
			mockPromptsService.setInstructions([
				makeInstruction(uri, 'plain instructions', undefined),
			]);
			mockPromptsService.setFileContent(
				uri,
