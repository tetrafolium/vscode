...
export type CustomUriHandler = vscode.UriHandler & {
	canHandleUri(uri: vscode.Uri): boolean;
};

export class ChatSessionsUriHandler
	extends Disposable
	implements CustomUriHandler
{
	constructor(
		...
		switch (uri.path) {
			case UriHandlerPaths.OpenSession: {
				...
				if (type?.startsWith('copilot') && prId) {
					...
					return this._openGitHubSession(
						'copilot-cloud-agent',
						prId,
						url,
						branch,
	...
	private async waitAndGetGlobalState(): Promise<
		PendingChatSession | undefined
	> {
		let timeout = 500;
		let state = undefined;
		while (!state && timeout > 0) {
			state = this._extensionContext.globalState.get<PendingChatSession>(
	...
	private async _openGitHubSession(
		type: string,
		id: string,
		url: string | null,
		branch: string | null,
		...
		if (gitAPI && url && branch) {
			...
			const pendingSessionUri = vscode.Uri.joinPath(
			...
			const cachedWorkspaces: vscode.Uri[] | null =
				await gitAPI.getRepositoryWorkspace(uri);

			let folderToOpen: vscode.Uri | null = null;
			if (
				!cachedWorkspaces ||
				(cachedWorkspaces && cachedWorkspaces.length > 1)
			) {
				...
				const cloneRepoItem: vscode.QuickPickItem & {
					uri?: vscode.Uri;
				} = {
					label: 'Clone Repository and Open',
					description:
						'Clone the repository to a new local folder and open it',
					uri: undefined,
				};
				...
				if (selected) {
					if (selected === selectFolderItem) {
						...
						folderToOpen = await gitAPI.clone(
							vscode.Uri.parse(url),
							{ postCloneAction: 'none', ref: branch },
						);
					}
				}
			} else {
			...
			const forceReuseWindow =
				vscode.workspace.workspaceFile === undefined &&
				vscode.workspace.workspaceFolders === undefined;
			vscode.commands.executeCommand('vscode.openFolder', folderToOpen, {
				forceReuseWindow,
			});
			return;
		}
	...
	public canHandleUri(uri: vscode.Uri): boolean {
		return Object.values(UriHandlerPaths).includes(
			uri.path as UriHandlerPaths,
		);
	}
	...
	public async openPendingSession(details?: {
		...
		if (!details) {
			...
			if (pendingSession.timestamp > tenMinutesAgo) {
				...
				const gitAPI = await this.waitForGitExtensionAPI(
					this._gitExtensionService,
				);
				if (!gitAPI) {
					return;
				}
				repository = this._getAlreadyOpenWorkspace(
					gitAPI,
					pendingSession.url,
				);
				...
				this._logService.warn(
					'Found pending sessions but they have expired at ' +
						new Date(pendingSession.timestamp).toISOString(),
				);
			}
		} else {
		...
		const repoNwo = getGithubRepoIdFromFetchUrl(
			repository.rootUri.toString(),
		);
		const repoIds = await getRepoId(this._gitService);
		const repoId = repoIds?.filter(
			(r) => r.org === repoNwo?.org && r.repo === repoNwo?.repo,
		);
		...
		const uri = vscode.Uri.from({
			scheme: 'copilot-cloud-agent',
			path: '/' + pullRequest.number.toString(),
		});
		await this._extensionContext.globalState.update(
			PENDING_CHAT_SESSION_STORAGE_KEY,
			undefined,
	...
	private async waitForGitExtensionAPI(
	...
	private _getAlreadyOpenWorkspace(
		gitApi: API,
		cloneUri: string,
		...
		for (const repo of gitApi.repositories) {
			...
			if (repo.kind === 'repository') {
				...
				for (const remote of remotes) {
					for (const url of remote.fetchUrl
						? [remote.fetchUrl]
						: []) {
