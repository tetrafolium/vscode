...
export const body_suffix = vscode.l10n.t(
...
export function truncatePrompt(
	...
	prompt: string,
	context?: string,
): { problemStatement: string; isTruncated: boolean } {
	...
	if (prompt.length >= MAX_PROBLEM_STATEMENT_LENGTH) {
		logService.warn(
			`Truncation: Prompt length ${prompt.length} exceeds max of ${MAX_PROBLEM_STATEMENT_LENGTH}`,
		);
	...
	if (
		context &&
		prompt.length + context.length >= MAX_PROBLEM_STATEMENT_LENGTH
	) {
		const availableLength =
			MAX_PROBLEM_STATEMENT_LENGTH - prompt.length - 2; /* new lines */
		logService.warn(
			`Truncation: Combined prompt and context length ${prompt.length + context.length} exceeds max of ${MAX_PROBLEM_STATEMENT_LENGTH}`,
		);
...
export function formatBodyPlaceholder(title: string | undefined): string {
	return vscode.l10n.t(
		'Cloud agent has begun work on **{0}** and will update this pull request as work progresses.',
		title || vscode.l10n.t('your request'),
	);
}

export async function getRepoId(
	gitService: IGitService,
): Promise<GithubRepoId[] | undefined> {
...
export namespace SessionIdForPr {
	...
	export function parse(
		resource: vscode.Uri,
	): { prNumber: number; sessionIndex: number } | undefined {
...
export async function toOpenPullRequestWebviewUri(params: {
	...
	const extensionId =
		UriHandlers[UriHandlerPaths.External_OpenPullRequestWebview];
	return await vscode.env.asExternalUri(
		vscode.Uri.from({
			scheme: vscode.env.uriScheme,
			authority: extensionId,
			path: UriHandlerPaths.External_OpenPullRequestWebview,
			query,
		}),
	);
}
