...
export const ISessionRequestLifecycle =
	createServiceIdentifier<ISessionRequestLifecycle>(
		'ISessionRequestLifecycle',
	);

export class SessionRequestLifecycle
	extends Disposable
	implements ISessionRequestLifecycle
{
	...
	constructor(
		...
		private readonly checkpointService: IChatSessionWorktreeCheckpointService,
	...
	async startRequest(
		...
		if (isFirstRequest) {
			...
			if (workingDirectory && !isIsolationEnabled(workspace)) {
				void this.workspaceFolderService.trackSessionWorkspaceFolder(
					...
					workspace.repositoryProperties,
				);
			}
		}

		const modeInstructions: StoredModeInstructions | undefined =
		...
		const requests =
			this.pendingRequestBySession.get(sessionId) ??
		...
		try {
			if (session.status === vscode.ChatSessionStatus.Completed) {
				...
				if (isIsolationEnabled(session.workspace)) {
					...
					await this.worktreeService.handleRequestCompleted(
						sessionId,
					);
				...
				await this.checkpointService.handleRequestCompleted(
					sessionId,
					request.id,
				);
			...
			this.prDetectionService.handlePullRequestCreated(
				sessionId,
				session.createdPullRequestUrl,
			);
		} finally {
			this.untrackRequest(sessionId, request);
		}
