 ...
 */
export const UNTRUSTED_FOLDER_MESSAGE = l10n.t(
	'The selected folder is not trusted. Please trust the folder to continue with the {0}.',
	'Copilot CLI',
);
 ...
 */
export abstract class FolderRepositoryManager
	extends Disposable
	implements IFolderRepositoryManager
{
	 ...
	 */
	protected readonly _newSessionFolders = new Map<
		string,
		{ uri: vscode.Uri; lastAccessTime: number }
	>();
	...
	setNewSessionFolder(sessionId: string, folderUri: vscode.Uri): void {
		this._newSessionFolders.set(sessionId, {
			uri: folderUri,
	 ...
	 */
	protected abstract getSessionFallbackFolder(
		sessionId: string,
	...
	async getFolderRepository(
		...
		if (isUntitledSessionId(sessionId)) {
			if (options) {
				const { folder, repository, repositoryProperties, trusted } =
					await this.getFolderRepositoryForNewSession(
						sessionId,
						undefined,
						options.stream,
						_token,
				...
				const folder =
					this._newSessionFolders.get(sessionId)?.uri ??
		...
		const worktreeProperties =
			await this.worktreeService.getWorktreeProperties(sessionId);
		...
		const sessionWorkspaceFolderEntry =
			await this.workspaceFolderService.getSessionWorkspaceFolderEntry(
				sessionId,
			);
		if (sessionWorkspaceFolderEntry) {
			const repositoryProperties =
				await this.workspaceFolderService.getRepositoryProperties(
					sessionId,
				);
			let trusted: boolean | undefined;
			if (options) {
				trusted = await this.verifyTrust(
					vscode.Uri.file(sessionWorkspaceFolderEntry.folderPath),
					options.stream,
				);
			}
	...
	protected async getFolderRepositoryForNewSession(
		sessionId: string | undefined,
		selectedFolder: vscode.Uri | undefined,
		...
		if (
			...
			folderUri =
				repositoryUri ?? this.workspaceService.getWorkspaceFolders()[0];

			// If we're in a single folder workspace, possible the user has opened the worktree folder directly.
			if (sessionId && folderUri) {
				const worktreeSessionIds =
					this.metadataStore.getWorktreeSessions(folderUri);
				worktreeProperties = worktreeSessionIds.length
					? await this.worktreeService.getWorktreeProperties(
							worktreeSessionIds[0],
						)
					: undefined;
				worktree = worktreeProperties
			...
			if (worktreeProperties) {
				repositoryUri = vscode.Uri.file(
					worktreeProperties.repositoryPath,
				);
			} else {
				const repoContext =
					await this.gitService.getRepository(selectedFolder);
				const branchBase =
					repoContext?.headBranchName && repoContext.headCommitHash
						? await this.gitService.getBranchBase(
								repoContext.rootUri,
								repoContext.headBranchName,
							)
						: undefined;

				const mergeBaseCommit =
					repoContext?.headBranchName && branchBase?.commit
						? await this.gitService.getMergeBase(
								repoContext.rootUri,
								repoContext.headBranchName,
								branchBase.commit,
							)
				...
				const uncommittedChanges =
					(repoContext?.changes?.mergeChanges.length ?? 0) +
					...
					? ({
							...
							baseBranchName:
								...
								branchBase.name
									...
									: undefined,
							upstreamBranchName:
								repoContext?.upstreamRemote &&
								repoContext?.upstreamBranchName
	...
	async initializeFolderRepository(
		...
		if (isolation === 'workspace') {
			this.logService.info(
				`[FolderRepositoryManager] Workspace isolation mode selected for session ${sessionId}, skipping worktree creation`,
			);
			return {
		...
		if (!worktreeProperties) {
			uncommittedChangesAction =
				await this.promptForUncommittedChangesAction(
					sessionId,
					repository,
					branch,
					toolInvocationToken,
		...
		try {
			newBranchName = options.newBranch
				? await options.newBranch
				: undefined;
		} catch (ex) {
			const error = ex instanceof Error ? ex : new Error(String(ex));
			this.logService.error(
				error,
				'Failed to generate a new branch name for worktree creation',
			);
		}
		worktreeProperties =
			worktreeProperties ??
		...
		if (!worktreeProperties) {
			stream.warning(
				l10n.t(
					'Failed to create worktree. Proceeding without isolation.',
				),
			);
	...
	async initializeMultiRootFolderRepositories(
		...
		const folderInfos = await Promise.all(
			allFolders.map((folder) =>
				this.getFolderRepositoryForNewSession(
					sessionId,
					folder,
					stream,
					token,
		...
		if (isolation === 'workspace') {
			const primary = trustedInfos.find(
				(t) => t.folder.fsPath === primaryFolder.fsPath,
			)?.info ?? {
				folder: primaryFolder,
				repository: undefined,
				repositoryProperties: undefined,
		...
		for (const { folder, info } of trustedInfos) {
			...
			const modifiedFiles = await this.getModifiedFilesForConfirmation(
				info.repository,
				repo,
				token,
			);
		...
		if (reposWithChanges.length > 0) {
			const allModifiedFiles = reposWithChanges.flatMap(
				(r) => r.modifiedFiles,
			);
			uncommittedChangesAction =
				await this._promptForMultiRootUncommittedChanges(
					toolInvocationToken,
					allModifiedFiles,
					token,
				);
			...
			trustedInfos.map(async ({ folder, info }) => {
				...
				if (!worktreeProperties) {
					this.logService.warn(
						`[FolderRepositoryManager] Multi-root: failed to create worktree for ${info.repository.fsPath}, proceeding without isolation`,
					);
					return { folder, info };
				}

				this.logService.info(
					`[FolderRepositoryManager] Multi-root: created worktree for ${info.repository.fsPath}: ${worktreeProperties.worktreePath}`,
				);
				return {
					folder,
					info: {
		...
		for (const result of worktreeCreationResults) {
			if (result.status === 'fulfilled') {
				...
				this.logService.error(
					`[FolderRepositoryManager] Multi-root: worktree creation failed: ${result.reason}`,
				);
			}
		}
		...
		if (
			...
			const reposWithChangesSet = new Set(
				reposWithChanges.map((r) => r.repository.fsPath),
			);
				...
				results
					.filter(
						(r) =>
							r.info.repository &&
							r.info.worktree &&
							reposWithChangesSet.has(r.info.repository.fsPath),
					)
					.map((r) =>
						this.moveOrCopyChangesToWorkTree(
							r.info.repository!,
							r.info.worktree!,
							uncommittedChangesAction!,
							stream,
		...
		const primaryResult = results.find(
			(r) => r.folder.fsPath === primaryFolder.fsPath,
		)?.info ?? {
			folder: primaryFolder,
			repository: undefined,
			repositoryProperties: undefined,
	...
	private async _promptForMultiRootUncommittedChanges(
		...
		const message = l10n.t(
			'Some repositories have uncommitted changes. Should these changes be included in the new worktrees?',
		);
		...
		const selection = this.getSelectedUncommittedChangesAction(
			result,
			options,
		);
	...
	async getFolderMRU(): Promise<FolderRepositoryMRUEntry[]> {
		...
		for (const {
			uri,
			lastAccessTime,
		} of this._newSessionFolders.values()) {
			if (seenUris.has(uri)) {
				continue;
			}
			seenUris.add(uri);
	...
	private async promptForUncommittedChangesAction(
		...
		const message = isDelegation
			? l10n.t(
					'Copilot CLI will work in an isolated worktree to implement your requested changes.',
				) +
				'\n\n' +
				l10n.t(
					'The selected repository has uncommitted changes. Should these changes be included in the new worktree?',
				)
			: l10n.t(
					'The selected repository has uncommitted changes. Should these changes be included in the new worktree?',
				);
	...
	private getSelectedUncommittedChangesAction(
		...
		for (const part of result.content) {
			...
			const matchedOption = options.find(
				(option) => option.toUpperCase() === part.value.toUpperCase(),
			);
			if (matchedOption) {
	...
	private async getModifiedFilesForConfirmation(
		...
		repository: NonNullable<
			ReturnType<IGitService['activeRepository']['get']>
		>,
		token: vscode.CancellationToken,
	): Promise<
		Array<{
	...
	> {
		...
		for (const change of [
			...
			const fileUri =
				change.uri ??
				(changePath
					? vscode.Uri.joinPath(repositoryUri, changePath)
					: undefined);
	...
	private async moveOrCopyChangesToWorkTree(
		...
		try {
			...
			const worktreeRepo = await raceCancellation(
				new Promise<typeof activeRepository | undefined>((resolve) => {
					disposables.add(
						this.gitService.onDidOpenRepository((repo) => {
					...
					disposables.add(
						createTimeout(10_000, () => resolve(undefined)),
					);
				}),
				token,
			);
			...
			if (!worktreeRepo) {
				stream.warning(
					l10n.t(
						'Failed to get worktree repository. Proceeding without migration.',
					),
				);
			} else {
				await this.gitService.migrateChanges(
					worktreeRepo.rootUri,
			...
			stream.warning(
				l10n.t(
					'Failed to migrate some changes: {0}. Continuing with worktree creation.',
...
export class CopilotCLIFolderRepositoryManager extends FolderRepositoryManager {
	 ...
	 * @inheritdoc
	 */
	protected async getSessionFallbackFolder(
		sessionId: string,
	): Promise<vscode.Uri | undefined> {
...
export class ClaudeFolderRepositoryManager extends FolderRepositoryManager {
	...
	protected async getSessionFallbackFolder(
		...
		const folderInfo =
			this.sessionStateService.getFolderInfoForSession(sessionId);
		if (
			folderInfo &&
			(await checkPathExists(
				vscode.Uri.file(folderInfo.cwd),
				this.fileSystem,
			))
		) {
