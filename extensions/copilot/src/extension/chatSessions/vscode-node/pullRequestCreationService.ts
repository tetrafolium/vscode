...
export const IPullRequestCreationService =
	createServiceIdentifier<IPullRequestCreationService>(
		'IPullRequestCreationService',
	);

export class PullRequestCreationService
	extends Disposable
	implements IPullRequestCreationService
{
	...
	async createPullRequest(
		...
		const remoteInformation = githubRepoInfo
			? repository.state.remotes.find(
					(remote) =>
						githubRepoInfo.remoteUrl === remote.fetchUrl ||
						githubRepoInfo.remoteUrl === remote.pushUrl,
				)
			: undefined;
		...
		if (
			context &&
			(context.commitMessages.length > 0 || context.patches.length > 0)
		) {
...
async function collectPullRequestContext(
	...
	const diffChanges =
		(await repository.diffBetweenWithStats(mergeBase, branchName)) ?? [];
	const patches: Diff[] = [];
	for (const change of diffChanges) {
		const patch = await repository.diffBetweenPatch(
			mergeBase,
			branchName,
			change.uri.fsPath,
		);
