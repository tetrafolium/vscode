...
import {
	formatModelDetails,
	formatModelDetailsWithMultiplier,
...
interface InputStateReactivePipeline {
	...
	readonly folderItems: ISettableObservable<
		readonly vscode.ChatSessionProviderOptionItem[]
	>;
...
function getSelectedFolderUri(
	...
	const selectedFolderId = inputState?.groups.find(
		(group) => group.id === FOLDER_OPTION_ID,
	)?.selected?.id;
	...
	implements vscode.ChatSessionContentProvider
{
	...
	constructor(
		...
		this._controller = this._register(
			instantiationService.createInstance(
				ClaudeChatSessionItemController,
			),
		);
	}
	...
	provideHandleOptionsChange(
		...
		for (const update of updates) {
			...
			if (
				update.optionId === PERMISSION_MODE_OPTION_ID &&
				value &&
				isPermissionMode(value)
			) {
				this.sessionStateService.setPermissionModeForSession(
					sessionId,
					value,
				);
			}
	...
	createHandler(): ChatExtendedRequestHandler {
		return async (
			...
			if (!chatSessionContext) {
				...
				stream.markdown(
					vscode.l10n.t('Start a new Claude Agent session'),
				);
				stream.button({
					command: `workbench.action.chat.openNewSessionEditor.${ClaudeSessionUri.scheme}`,
			...
			const slashResult = await this.slashCommandService.tryHandleCommand(
				request,
				stream,
				token,
			);
			...
			const effectiveSessionId = ClaudeSessionUri.getSessionId(
			...
			const sessionUri =
				ClaudeSessionUri.forSessionId(effectiveSessionId);
			...
			const selectedPermissionId =
				chatSessionContext.inputState.groups.find(
					(group) => group.id === PERMISSION_MODE_OPTION_ID,
				)?.selected?.id;
			if (
				!selectedPermissionId ||
				!isPermissionMode(selectedPermissionId)
			) {
				throw new Error(
					`Permission mode not set for session ${effectiveSessionId}`,
				);
			}
			const permissionMode = selectedPermissionId;
			const selectedFolderUri = getSelectedFolderUri(
				chatSessionContext.inputState,
			);
			const folderInfo = await this._controller.getFolderInfoForSession(
				effectiveSessionId,
				selectedFolderUri,
			);

			// Commit UI state to session state service before invoking agent manager
			this.sessionStateService.setModelIdForSession(
				effectiveSessionId,
				modelId,
			);
			this.sessionStateService.setPermissionModeForSession(
			...
			this.sessionStateService.setFolderInfoForSession(
				effectiveSessionId,
				folderInfo,
			);
			...
			const endpoint = await this.claudeModels.resolveEndpoint(
				modelId.toEndpointModelId(),
				undefined,
			);
			const rawReasoningEffort =
				request.modelConfiguration?.[CLAUDE_REASONING_EFFORT_PROPERTY];
			...
			this.sessionStateService.setReasoningEffortForSession(
				effectiveSessionId,
				reasoningEffort,
			);

			// Set usage handler to report token usage for context window widget
			this.sessionStateService.setUsageHandlerForSession(
				effectiveSessionId,
				(usage) => {
					stream.usage(usage);
				},
			...
			this.sessionStateService.setTurnIdForSession(
				effectiveSessionId,
				request.id,
			);
			...
			try {
				...
				await this._controller.updateItemStatus(
					effectiveSessionId,
					vscode.ChatSessionStatus.InProgress,
					prompt,
				);
				result = await this.claudeAgentManager.handleRequest(
					effectiveSessionId,
					request,
					stream,
					token,
				...
				creditsUsed = this._chatQuotaService.getCreditsForTurn(
					request.id,
				);
				this._chatQuotaService.resetTurnCredits(request.id);
			}
	...
	async provideChatSessionContent(
		...
		const detailsByModelId =
			existingSession && modelDetailsEnabled
				? await this._buildModelDetailsLookup(existingSession, token)
				: undefined;
		const history = existingSession
			? buildChatHistory(
					existingSession,
					detailsByModelId
						? (id) => detailsByModelId.get(id)
						: undefined,
				)
			: [];
	 ...
	 */
	private async _buildModelDetailsLookup(
...
export class ClaudeChatSessionItemController extends Disposable {
	...
	constructor(
		@IClaudeCodeSessionService
		private readonly _claudeCodeSessionService: IClaudeCodeSessionService,
		@IClaudeSessionStateService
		...
		private readonly _claudeWorkspaceFolderService: IClaudeWorkspaceFolderService,
		@IAuthenticationService _authenticationService: IAuthenticationService,
		@IExperimentationService
		experimentationService: IExperimentationService,
	) {
		super();
		this._optionBuilder = new ClaudeSessionOptionBuilder(
		...
		this._bypassPermissionsEnabled = observableFromEvent(
			this,
			Event.filter(_configurationService.onDidChangeConfiguration, (e) =>
			...
			() =>
				_configurationService.getConfig(
					ConfigKey.ClaudeAgentAllowDangerouslySkipPermissions,
				) as boolean,
		);
		...
		this._autoPermissionsEnabled = observableFromEvent(
			...
			Event.any(
				Event.filter(
					...
					(e) =>
						e.affectsConfiguration(
							ConfigKey.ClaudeAgentAllowAutoPermissions
								.fullyQualifiedId,
						),
				),
			...
			() =>
				_configurationService.getExperimentBasedConfig(
					ConfigKey.ClaudeAgentAllowAutoPermissions,
					experimentationService,
				) &&
				...
				(_authenticationService.copilotToken?.isEditorPreviewFeaturesEnabled() ??
					true),
		);
		...
		const workspaceFoldersSubscription =
			_workspaceService.onDidChangeWorkspaceFolders(() =>
				workspaceFoldersEmitter.fire(),
			);
		this._register({
			dispose: () => workspaceFoldersSubscription.dispose(),
		});
		...
		this._controller.newChatSessionItemHandler = async (
			context,
			_token,
		) => {
			...
			const folderInfo = await this.getFolderInfoForSession(
				newSessionId,
				selectedFolderUri,
			);
			if (folderInfo.cwd) {
				item.metadata = await this._buildSessionMetadata(
					folderInfo.cwd,
				);
			}
		...
		this._controller.forkHandler = async (
			sessionResource: vscode.Uri,
			request: vscode.ChatRequestTurn2 | undefined,
			token: CancellationToken,
		): Promise<vscode.ChatSessionItem> => {
			const item = this._controller.items.get(sessionResource);
			const title = vscode.l10n.t(
				'Forked: {0}',
				item?.label ?? request?.prompt ?? 'Claude Session',
			);
			...
			if (request) {
				...
				const session = await this._claudeCodeSessionService.getSession(
					sessionResource,
					token,
				);
				if (!session) {
					...
					this._logService.error(
						`Failed to fork session: session not found for resource ${sessionResource.toString()}`,
					);
					...
					if (messageIndex === -1) {
						this._logService.error(
							`Failed to fork session: request with id ${request.id} not found in session ${sessionResource.toString()}`,
						);
						throw new Error(
							'Unable to fork: the selected message could not be found.',
						);
					}
					if (messageIndex === 0) {
						this._logService.error(
							`Failed to fork session: cannot fork at the first message`,
						);
			...
			const newItem = this._controller.createChatSessionItem(
				ClaudeSessionUri.forSessionId(result.sessionId),
				title,
			);
			...
			if (item?.metadata?.workingDirectoryPath) {
				newItem.metadata = await this._buildSessionMetadata(
			...
			const parentPermission =
				this._sessionStateService.getPermissionModeForSession(
					parentSessionId,
				);
			const parentFolder =
				this._sessionStateService.getFolderInfoForSession(
					parentSessionId,
				);
			this._sessionStateService.setPermissionModeForSession(
				result.sessionId,
				parentPermission,
			);
			if (parentFolder) {
				this._sessionStateService.setFolderInfoForSession(
					result.sessionId,
					{
						...parentFolder,
						additionalDirectories: [
							...(parentFolder.additionalDirectories ?? []),
						],
					},
				);
			}
		...
		this._register(
			_gitService.onDidOpenRepository(() => {
				this._showBadge = this._computeShowBadge();
				void this._refreshItems(CancellationToken.None);
			}),
		);
		this._register(
			_gitService.onDidCloseRepository(() => {
				this._showBadge = this._computeShowBadge();
				void this._refreshItems(CancellationToken.None);
			}),
		);
	...
	private _createInputStateReactivePipeline(
		...
		const permissionMode = observableValue<PermissionMode>(
			this,
			seed.permissionMode,
		);
		const folderUri = observableValue<URI | undefined>(
			this,
			seed.folderUri,
		);
		const folderItems = observableValue<
			readonly vscode.ChatSessionProviderOptionItem[]
		>(this, seed.folderItems);
		const isSessionStarted = observableValue<boolean>(
			this,
			seed.isSessionStarted,
		);
					...
					folderItems.set(
						folders.map((f) =>
							toWorkspaceFolderOptionItem(
								f,
								this._workspaceService.getWorkspaceFolderName(
									f,
								) || basename(f),
							),
						),
		...
		const permissionModeGroup = derived((reader) => {
			...
			const selectedItem =
				group.items.find((i) => i.id === selectedMode) ??
				group.items[0];
			return { ...group, selected: selectedItem };
		});

		const folderGroup = derived<
	...
	private _setupInputState(): void {
		this._controller.getChatSessionInputState = async (
			sessionResource,
			context,
			token,
		) => {
			...
			if (context.previousInputState) {
				state = this._controller.createChatSessionInputState([
					...context.previousInputState.groups,
				]);
				pipeline = this._createInputStateReactivePipeline(state);
			} else {
				const isExistingSession =
					sessionResource &&
					(await this._claudeCodeSessionService.getSession(
						sessionResource,
						token,
					)) !== undefined;
				...
				state =
					this._controller.createChatSessionInputState(initialGroups);
				pipeline = this._createInputStateReactivePipeline(state);
			}
			...
			if (sessionResource) {
				...
				const externalPermissionMode = observableFromEvent(
					this,
					Event.filter(
						this._sessionStateService.onDidChangeSessionState,
						(e) =>
							e.sessionId === sessionId &&
							e.permissionMode !== undefined,
					),
					() =>
						this._sessionStateService.getPermissionModeForSession(
							sessionId,
				...
				pipeline.store.add(
					autorun((reader) => {
						...
						pipeline.permissionMode.set(
							externalPermissionMode.read(reader),
							undefined,
						);
					}),
				);
			...
			pipeline.store.add(
				state.onDidDispose(() => pipeline.store.dispose()),
			);
			return state;
		};
	}
	...
	private _computeSeedValues(
		...
		let permissionMode: PermissionMode =
			this._optionBuilder.lastUsedPermissionMode;
		const permissionGroup = groups.find(
			(g) => g.id === PERMISSION_MODE_OPTION_ID,
		);
		if (
			permissionGroup?.selected &&
			isPermissionMode(permissionGroup.selected.id)
		) {
		...
		if (folderGroup) {
			...
			if (
				folderGroup.selected?.locked ||
				folderGroup.items.some((i) => i.locked)
			) {
	...
	private async _buildExistingSessionGroups(
		sessionResource: vscode.Uri,
		...
		if (workspaceFolders.length !== 1) {
			const stateFolder =
				this._sessionStateService.getFolderInfoForSession(sessionId);
			if (stateFolder) {
		...
		return this._optionBuilder.buildExistingSessionGroups(
			permissionMode,
			folderUri,
		);
	}
	...
	async getFolderInfoForSession(
		sessionId: string,
		selectedFolderUri?: URI,
	): Promise<ClaudeFolderInfo> {
		const workspaceFolders = this._workspaceService.getWorkspaceFolders();
		...
		const folderUri =
			selectedFolderUri ?? (await this._resolveSessionFolder(sessionId));
	...
	private async _resolveSessionFolder(
		sessionId: string,
	...
	async updateItemStatus(
		...
		if (!item) {
			...
			if (session) {
				...
				item = await this._createClaudeChatSessionItem(
		...
		if (status === vscode.ChatSessionStatus.InProgress) {
			...
			if (status === vscode.ChatSessionStatus.Completed) {
				...
				if (
					session?.cwd &&
					(await this._workspaceService.isResourceTrusted(
						URI.file(session.cwd),
					))
				) {
					item.changes =
						await this._claudeWorkspaceFolderService.getWorkspaceChanges(
							session.cwd,
							session.gitBranch,
							undefined,
							true,
	...
	private async _refreshItems(
		...
		const sessions =
			await this._claudeCodeSessionService.getAllSessions(token);
		const results = await Promise.allSettled(
			sessions.map((session) =>
				this._createClaudeChatSessionItem(session),
			),
		);
		...
		for (let i = 0; i < results.length; i++) {
			...
			if (result.status === 'fulfilled') {
				...
				this._logService.warn(
					`Failed to create Claude chat session item for ${session.id} (${session.label}) ${result.reason}`,
				);
			}
		}
	...
	private async _createClaudeChatSessionItem(
		session: IClaudeCodeSessionInfo,
	): Promise<vscode.ChatSessionItem> {
		...
		item.iconPath = new vscode.ThemeIcon('claude');
		if (session.cwd) {
			const isTrusted = await this._workspaceService.isResourceTrusted(
				URI.file(session.cwd),
			);
			if (isTrusted) {
	...
	private async _buildSessionMetadata(
		cwd: string,
		isTrusted?: boolean,
		...
		if (
			!(
				isTrusted ??
				(await this._workspaceService.isResourceTrusted(cwdUri))
			)
		) {
		...
		const uncommittedChanges = changes
			? changes.mergeChanges.length +
				changes.indexChanges.length +
				changes.workingTree.length +
				changes.untrackedChanges.length
			: 0;
		...
		return {
			...
			upstreamBranchName:
				repoContext.upstreamRemote && repoContext.upstreamBranchName
					...
					: undefined,
			hasGitHubRemote:
				getGitHubRepoInfoFromContext(repoContext) !== undefined,
	...
	private _registerCommands(): void {
		this._register(
			vscode.commands.registerCommand(
				...
				async (sessionItem?: vscode.ChatSessionItem) => {
					...
					const sessionId = ClaudeSessionUri.getSessionId(
						sessionItem.resource,
					);
					...
					if (newTitle) {
						...
						if (trimmedTitle) {
							try {
								await this._sdkService.renameSession(
									sessionId,
									trimmedTitle,
								);
								...
								this._logService.error(
									e,
									`[ClaudeChatSessionItemController] Failed to rename session: ${sessionId}`,
								);
							}
						}
					}
		...
		this._registerPromptCommand(
			'github.copilot.claude.sessions.commit',
			builtinSlashCommands.commit,
		);
		this._registerPromptCommand(
			'github.copilot.claude.sessions.commitAndSync',
			`${builtinSlashCommands.commit} and ${builtinSlashCommands.sync}`,
		);
		this._registerPromptCommand(
			'github.copilot.claude.sessions.sync',
			builtinSlashCommands.sync,
		);

		this._register(
			vscode.commands.registerCommand(
				'github.copilot.claude.sessions.initializeRepository',
				async (
					sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri,
				) => {
					const resource = getSessionResource(sessionItemOrResource);
					...
					const sessionId = ClaudeSessionUri.getSessionId(resource);
					const folderInfo =
						await this.getFolderInfoForSession(sessionId);
					const workspaceFolder = URI.file(folderInfo.cwd);

					const repository =
						await this._gitService.initRepository(workspaceFolder);
					if (!repository) {
						return;
					}
