...
} from '../copilotcli/common/pendingRequestContext';
...
import {
	...
	SessionInitOptions,
} from '../copilotcli/vscode-node/copilotCLIChatSessionInitializer';
		...
		ICopilotCLIChatSessionItemProvider
{
	...
	constructor(
		...
		@ISessionOptionGroupBuilder
		private readonly _optionGroupBuilder: ISessionOptionGroupBuilder,
		@IGitService private readonly _gitService: IGitService,
		@IChatSessionWorkspaceFolderService
		private readonly _workspaceFolderService: IChatSessionWorkspaceFolderService,
		@IChatSessionMetadataStore
		private readonly _metadataStore: IChatSessionMetadataStore,
		@IWorkspaceService
		private readonly _workspaceService: IWorkspaceService,
		@IChatSessionWorktreeService
		chatSessionWorktreeService: IChatSessionWorktreeService,
	) {
		super();
		...
		const refreshSessions = async () => {
			...
			void this._metadataStore
				.refresh()
				.catch((error) =>
					this.logService.error(
						error,
						'Failed to refresh session metadata store during session list refresh',
					),
				);
			try {
				...
				const items = await Promise.all(
					sessions.map(async (session) =>
						this.toChatSessionItem(session),
					),
				);
		...
		const controller = (this.controller = this._register(
		...
		this._register(
			this._workspaceFolderService.onDidChangeWorkspaceFolderChanges(
				(e) => {
					this.refreshSession({
						reason: 'update',
		...
		this._register(
			chatSessionWorktreeService.onDidChangeWorktreeChanges((e) => {
				this.refreshSession({
					reason: 'update',
					sessionId: e.sessionId,
				});
		...
		controller.newChatSessionItemHandler = async (context) => {
			...
			this.customSessionTitleService
				...
				.then(async (title) => {
					if (title) {
						await this.customSessionTitleService.setCustomSessionTitle(
							sessionId,
							title,
						);
					}
					...
					if (this.controller.items.get(resource)) {
						this.refreshSession({
							...
							/* expected if session was deleted */
						});
					}
				})
				.catch((ex) =>
		...
		if (
			...
			controller.forkHandler = async (
				...
				const item = await this.sessionService.getSessionItem(
					forkedSessionId,
					token,
				);
				if (!item) {
		...
		if (
			...
			controller.resolveChatSessionItem = async (item, token) => {
				...
				const session = await this.sessionService.getSessionItem(
					sessionId,
					token,
				);
				...
				const updatedItem = await this.toChatSessionItem(
					session,
					{ includeChanges: true },
					token,
				);
		...
		this._register(
			this.sessionService.onDidDeleteSession(async (e) => {
				controller.items.delete(SessionIdForCLI.getResource(e));
			}),
		);
		this._register(
			this.sessionService.onDidChangeSession(async (e) => {
		...
		this._register(
			this.sessionService.onDidCreateSession(async (e) => {
		...
		if (controller.onDidChangeChatSessionItemState) {
			this._register(
				controller.onDidChangeChatSessionItemState(async (item) => {
					...
					try {
						await this._metadataStore.setSessionArchived(
							sessionId,
							!!item.archived,
						);
					} catch (error) {
						this.logService.error(
							`[CopilotCLI] Failed to persist archived state for session ${sessionId}:`,
							error,
						);
					}
					if (item.archived) {
						...
						const worktreePath =
							await this.copilotCLIWorktreeManagerService.getWorktreePath(
								sessionId,
							);
						if (worktreePath) {
							const siblings =
								await getBlockingSiblingSessionsForFolder(
									worktreePath,
									sessionId,
									this._metadataStore,
									this._workspaceFolderService,
								);
							if (siblings.length > 0) {
								this.logService.trace(
									`[CopilotCLI] Skipping worktree cleanup for archived session ${sessionId}: ${siblings.length} other session(s) still use the worktree`,
								);
								return;
							}
						}
						try {
							const result =
								await this.copilotCLIWorktreeManagerService.cleanupWorktreeOnArchive(
									sessionId,
								);
							this.logService.trace(
								`[CopilotCLI] Worktree cleanup for session ${sessionId}: ${result.cleaned ? 'cleaned' : result.reason}`,
							);
						} catch (error) {
							this.logService.error(
								`[CopilotCLI] Failed to cleanup worktree for archived session ${sessionId}:`,
								error,
							);
						}
					} else {
						try {
							const result =
								await this.copilotCLIWorktreeManagerService.recreateWorktreeOnUnarchive(
									sessionId,
								);
							this.logService.trace(
								`[CopilotCLI] Worktree recreation for session ${sessionId}: ${result.recreated ? 'recreated' : result.reason}`,
							);
							if (result.recreated) {
							...
							this.logService.error(
								`[CopilotCLI] Failed to recreate worktree for unarchived session ${sessionId}:`,
								error,
							);
						}
					}
		...
		controller.getChatSessionInputState = async (
			sessionResource,
			context,
			token,
		) => {
			const isExistingSession =
				sessionResource &&
				!this.sessionService.isNewSessionId(
					SessionIdForCLI.parse(sessionResource),
				);
			if (isExistingSession) {
				const groups =
					await this._optionGroupBuilder.buildExistingSessionInputStateGroups(
						sessionResource,
						token,
					);
				...
				const groups =
					await this._optionGroupBuilder.provideChatSessionProviderOptionGroups(
		...
		this._register(
			this._gitService.onDidFinishInitialization(refreshActiveInputState),
		);
		this._register(
			this._gitService.onDidOpenRepository(refreshActiveInputState),
		);
		this._register(
			this._gitService.onDidCloseRepository(refreshActiveInputState),
		);
		this._register(
	...
	public async refreshSession(
		...
		if (refreshOptions.reason === 'delete') {
			...
			refreshOptions.reason === 'update' &&
			hasKey(refreshOptions, { sessionIds: true })
		) {
			await Promise.allSettled(
				refreshOptions.sessionIds.map(async (sessionId) => {
					const item = await this.sessionService.getSessionItem(
						sessionId,
						CancellationToken.None,
					);
					if (item) {
						...
						const chatSessionItem = await this.toChatSessionItem(
							item,
							{ includeChanges: true },
						);
	...
	private async buildBadge(
		...
		const repositories = this._gitService.repositories.filter(
			(r) => r.kind !== 'worktree',
		);
		const shouldShow =
	...
	private async hasCachedChanges(
		...
		const [hasCachedWorktreeChanges, hasCachedWorkspaceChanges] =
			await Promise.all([
				this.copilotCLIWorktreeManagerService.hasCachedChanges(
					sessionId,
				),
		...
		if (
			...
			changes.push(
				...((await raceCancellation(
					this.copilotCLIWorktreeManagerService.getWorktreeChanges(
						sessionId,
					),
					token,
				)) ?? []),
			...
			const workspaceChanges =
				(await raceCancellation(
					this._workspaceFolderService.getWorkspaceChanges(sessionId),
					token,
				)) ?? [];
	...
	private async buildMetadata(
		...
		if (worktreeProperties) {
			const parentInfo =
				await this._metadataStore.getSessionParentId(sessionId);
		...
		const [parentInfo, sessionRequestDetails, repositoryProperties] =
			await Promise.all([
	...
	async provideChatSessionContent(
		...
		try {
			...
			if (
				copilotcliSessionId.startsWith('untitled:') ||
				copilotcliSessionId.startsWith('untitled-')
			) {
				return {
					history: [],
			...
			if (this.sessionService.isNewSessionId(copilotcliSessionId)) {
				...
				const [history, title, optionGroups] = await Promise.all([
					...
					this.sessionService.getSessionTitle(
						copilotcliSessionId,
						token,
					),
					this._optionGroupBuilder.buildExistingSessionInputStateGroups(
						resource,
						token,
					),
				]);
...
export class CopilotCLIChatSessionParticipant extends Disposable {
	constructor(
		...
		private readonly sessionRequestLifecycle: ISessionRequestLifecycle,
		@IPullRequestDetectionService
		private readonly prDetectionService: IPullRequestDetectionService,
		@ISessionOptionGroupBuilder
		private readonly _optionGroupBuilder: ISessionOptionGroupBuilder,
		...
		this._register(
			this.prDetectionService.onDidDetectPullRequest((sessionId) => {
				this.sessionItemProvider
					.refreshSession({ reason: 'update', sessionId })
					.catch((error) =>
						this.logService.error(
							error,
							'Failed to refresh session after PR detection',
						),
					);
			}),
		);
	...
	private sendTelemetryForHandleRequest(
		...
		const sessionId = chatSessionContext
			? SessionIdForCLI.parse(chatSessionContext.chatSessionItem.resource)
			: undefined;
		const isUntitled = sessionId
			? String(this.sessionService.isNewSessionId(sessionId))
			: 'false';
	...
	> {
		const authInfo = await this.copilotCLISDK
			.getAuthInfo()
			.catch((ex) => this.logService.error(ex, 'Authorization failed'));
		if (!authInfo) {
	...
	private async resolveInput(
		...
		const contextForRequest = takePendingCopilotCLIRequestContext(
			session.sessionId,
		);
		...
		if (contextForRequest) {
			return {
				input: {
					prompt: contextForRequest.prompt,
					source: contextForRequest.source,
				},
				attachments: contextForRequest.attachments,
			};
		}

		if (request.command && !request.prompt && !isNewSession) {
			const input = (copilotCLICommands as readonly string[]).includes(
				request.command,
			)
		...
		const input =
			request.command &&
			(copilotCLICommands as readonly string[]).includes(request.command)
				? { command: request.command as CopilotCLICommand, prompt }
				: { prompt };
		return { input, attachments };
	}

	private generateNewBranchName(
		request: vscode.ChatRequest,
		token: vscode.CancellationToken,
		...
		const branchNamePromise =
			request.prompt && this.branchNameGenerator
	...
	private async handleRequestImpl(
		...
		try {
			...
			const invalidSessionMessage =
				_invalidCopilotCLISessionIdsWithErrorMessage.get(sessionId);
			...
			const branchNamePromise = isNewSession
				? this.generateNewBranchName(request, token)
				: Promise.resolve(undefined);

			if (isNewSession) {
				this._optionGroupBuilder.lockInputStateGroups(
					chatSessionContext.inputState,
				);
			}

			const selectedOptions = getSelectedSessionOptions(
				chatSessionContext.inputState,
			);
			...
			const sessionResult = await this.getOrCreateSession(
				...
				{ ...selectedOptions, newBranch: branchNamePromise, stream },
				disposables,
				token,
			);
			...
			if (isNewSession && !sessionResult.trusted) {
				await this._optionGroupBuilder.rebuildInputState(
					chatSessionContext.inputState,
				);
			}
			...
			if (isNewSession && session.object.workspace.worktreeProperties) {
				const branchName =
					session.object.workspace.worktreeProperties.branchName;
				this._optionGroupBuilder.updateBranchInInputState(
					chatSessionContext.inputState,
					branchName,
				);
			}
			...
			await this.sessionRequestLifecycle.startRequest(
				sdkSessionId,
				request,
			...
			if (request.command === 'delegate') {
				...
				const { input, attachments } = await this.resolveInput(
					request,
					session.object,
					isNewSession,
					token,
			...
			if (sdkSessionId && session) {
				await this.sessionRequestLifecycle.endRequest(
					...
					request,
					{
						...
						workspace: session.object.workspace,
						createdPullRequestUrl:
							session.object.createdPullRequestUrl,
					},
					token,
				);
				if (shouldRefreshSessionItem) {
					this.sessionItemProvider
						...
						.catch((error) =>
							this.logService.error(
								error,
								'Failed to refresh session item after handling request',
							),
						);
				}
			}
	...
	private async getOrCreateSession(
		...
		const result = await this.sessionInitializer.getOrCreateSession(
			request,
			chatResource,
			options,
			disposables,
		...
		if (isNewSession && request.command !== 'remote') {
			this.sessionItemProvider.refreshSession({
				reason: 'update',
	...
	private async handleDelegationFromAnotherChat(
		...
		const { workspaceInfo, cancelled } =
			await this.sessionInitializer.initializeWorkingDirectory(
				undefined,
				{
					stream,
					isolation: IsolationMode.Worktree,
					newBranch: branchNamePromise,
				},
		...
		const session = await this.sessionInitializer.createDelegatedSession(
			request,
			workspaceInfo,
			{ mcpServerMappings },
			token,
		...
		setPendingCopilotCLIRequestContext(session.object.sessionId, {
			prompt,
			attachments,
		});
		void vscode.commands
			.executeCommand(
				...
				{
					resource: SessionIdForCLI.getResource(
						session.object.sessionId,
					),
			...
			.then(undefined, (error) => {
				...
				this.logService.error(
					error,
					'[CopilotCLIChatSessionContentProvider] Failed to open Copilot CLI session',
				);
			});
...
export function registerCLIChatCommands(
			...
			async (sessionItem?: vscode.ChatSessionItem) => {
				if (sessionItem?.resource) {
					...
					const existingTerminal =
						await sessionTracker.getTerminal(id);
					if (existingTerminal) {
					...
					try {
						...
						const cwd =
							...
							folderInfo.folder;
						const terminal = await terminalIntegration.openTerminal(
							terminalName,
							cliArgs,
							cwd?.fsPath,
						);
						if (terminal) {
							...
							terminalIntegration.setTerminalSessionDir(
								terminal,
			...
			async (sessionItem?: vscode.ChatSessionItem) => {
				...
				if (newTitle) {
					...
					if (trimmedTitle) {
						...
						await contentProvider.refreshSession({
							reason: 'update',
							sessionId: id,
						});
					}
	...
	const createCopilotCLITerminal = async (
	...
	disposableStore.add(
		vscode.commands.registerCommand(
			...
			async () => {
				await createCopilotCLITerminal(
					'editorBeside',
					l10n.t('Copilot CLI'),
				);
			},
	...
	disposableStore.add(
		vscode.commands.registerCommand(
			...
			async ({
				...
				const mruItems =
					await copilotCLIFolderMruService.getRecentlyUsedFolders(
						CancellationToken.None,
					);
				...
				if (
					...
					await copilotCLIFolderMruService.deleteRecentlyUsedFolder(
						selectedFolderUri,
					);
				...
				if (inputState) {
					await contentProvider.updateInputStateAfterFolderSelection(
						inputState,
						selectedFolderUri,
					);
				}
		...
		vscode.commands.registerCommand(
			...
			async (args?: {
				...
				try {
					...
					const sessionIds =
						await contentProvider.getAssociatedSessions(
							worktreeUri,
						);
					await Promise.all(
