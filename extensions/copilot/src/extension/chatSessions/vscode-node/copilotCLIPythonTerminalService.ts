...
export class PythonTerminalService {
	...
	public async createTerminal(options: vscode.TerminalOptions) {
		try {
			const workspaceUri = vscode.workspace.workspaceFolders?.length
			...
			if (
				...
				!env.sysPrefix
					.toLowerCase()
					.startsWith(workspaceUri.fsPath.toLowerCase())
			) {
				return;
			}
			...
			this.logService.error(
				'Failed to create terminal with Python environment',
				ex.toString(),
			);
		}
	}
