...
import {
	...
	parseGitChangesRaw,
} from '../../../platform/git/vscode-node/utils';
...
export class ChatSessionWorktreeService
	extends Disposable
	implements IChatSessionWorktreeService
{
	declare _serviceBrand: undefined;

	private _sessionWorktrees: Map<
		string,
		string | ChatSessionWorktreeProperties
	> = new Map();

	private readonly _onDidChangeWorktreeChanges = this._register(
		new vscode.EventEmitter<{ sessionId: string }>(),
	);
	readonly onDidChangeWorktreeChanges =
		this._onDidChangeWorktreeChanges.event;

	private readonly _updatePropertiesSequencer = new SequencerByKey<string>();

	constructor(
		@IAgentSessionsWorkspace
		private readonly agentSessionsWorkspace: IAgentSessionsWorkspace,
		@IConfigurationService
		private readonly configurationService: IConfigurationService,
		@IGitCommitMessageService
		private readonly gitCommitMessageService: IGitCommitMessageService,
	...
	async createWorktree(
		...
		return new Promise<ChatSessionWorktreeProperties | undefined>(
			(resolve) => {
				stream.progress(
					l10n.t(
						'Creating isolated worktree for Copilot CLI session...',
					),
					async (progress) => {
						const result = await this._createWorktree(
							repositoryPath,
							progress,
							baseBranch,
							branchName,
	...
	private async _createWorktree(
		repositoryPath: vscode.Uri,
		progress?: vscode.Progress<vscode.ChatResponsePart>,
		baseBranch?: string,
		branchName?: string,
	): Promise<ChatSessionWorktreeProperties | undefined> {
		try {
			...
			if (!activeRepository) {
				progress?.report(
					new vscode.ChatResponseWarningPart(
						vscode.l10n.t(
							'Failed to create worktree for isolation, using default workspace directory',
						),
					),
				);
				this.logService.error(
					'[ChatSessionWorktreeService][_createWorktree] No active repository found to create worktree for isolation.',
				);
				return undefined;
			}

			const autoCommit = this.configurationService.getConfig<boolean>(
				ConfigKey.Advanced.CLIAutoCommitEnabled,
			);

			let baseCommit: string | undefined = undefined;
			const branch = await this.generateBranchName(
				branchName,
				activeRepository,
			);
			...
			if (
				...
				try {
					...
					const branchDetails = await this.gitService.getBranch(
						activeRepository.rootUri,
						baseBranch,
					);
					if (
						branchDetails?.upstream?.remote &&
						branchDetails.upstream?.name
					) {
						...
						try {
							...
							const upstreamBranch =
								await this.gitService.getBranch(
									activeRepository.rootUri,
									upstreamBranchName,
								);
							if (upstreamBranch) {
							...
							this.logService.warn(
								`[ChatSessionWorktreeService][_createWorktree] Failed to resolve upstream branch ${upstreamBranchName}. Error: ${errorMessage}`,
							);
						}
					}
				} catch (error) {
					...
					this.logService.warn(
						`[ChatSessionWorktreeService][_createWorktree] Failed to resolve base branch ${baseBranch}. Error: ${errorMessage}`,
					);
				}
			}

			const worktreePath = await this.gitService.createWorktree(
				activeRepository.rootUri,
				{ branch, commitish: baseBranch, noTrack: true },
			);

			if (
				worktreePath &&
				activeRepository.headCommitHash &&
				activeRepository.headBranchName
			) {
				const baseBranchName =
					baseBranch ?? activeRepository.headBranchName;
				const baseBranchProtected =
				...
				this.logService.trace(
					`[ChatSessionWorktreeService][_createWorktree] Base branch for worktree: ${baseBranchName}, protected: ${baseBranchProtected}`,
				);
				...
				const incomingChanges =
					activeRepository.headIncomingChanges ?? 0;
				const outgoingChanges =
					activeRepository.headOutgoingChanges ?? 0;
				const uncommittedChanges =
					(activeRepository.changes?.mergeChanges.length ?? 0) +
				...
				return {
					...
					baseBranchProtected,
					upstreamBranchName:
						activeRepository.upstreamRemote &&
						activeRepository.upstreamBranchName
			...
			progress?.report(
				new vscode.ChatResponseWarningPart(
					vscode.l10n.t(
						'Error creating worktree for isolation: {0}',
						error instanceof Error ? error.message : String(error),
					),
				),
			);
			this.logService.error(
				'[ChatSessionWorktreeService][_createWorktree] Error creating worktree for isolation: ',
				error,
			);
			return undefined;
		}
	...
	private async generateBranchName(
		...
		const branchPrefixConfig =
			vscode.workspace
				.getConfiguration('git')
				.get<string>('branchPrefix') ?? '';
		const branchPrefix = this.agentSessionsWorkspace
		...
		if (preferredName) {
			...
			const refs = await this.gitService.getRefs(repository.rootUri, {
		...
		const randomBranchName = await this.gitService.generateRandomBranchName(
			repository.rootUri,
		);

		const branch = randomBranchName
			? `${branchPrefixConfig}${branchPrefix}/${randomBranchName.substring(branchPrefixConfig.length)}`
	...
	async updateWorktreeProperties(
		sessionId: string,
		properties: Partial<ChatSessionWorktreeProperties>,
	): Promise<void> {
		await this._updatePropertiesSequencer.queue(sessionId, async () => {
			const currentProperties =
				await this.getWorktreeProperties(sessionId);
	...
	async applyWorktreeChanges(sessionId: string): Promise<void> {
		...
		if (
			...
			const activeRepository = worktreeProperties?.repositoryPath
				? await this.gitService.getRepository(
						vscode.Uri.file(worktreeProperties.repositoryPath),
					)
			...
			await this.gitService.migrateChanges(
				activeRepository.rootUri,
				worktreePath,
				{
					confirmation: false,
		...
		const patchFilePath = path.join(
			worktreeProperties.repositoryPath,
			'.git',
			`${worktreeProperties.branchName}.patch`,
		);
		...
		try {
			// Apply patch
			await this.gitService.applyPatch(
				vscode.Uri.file(worktreeProperties.repositoryPath),
				patchFilePath,
			);
		} catch (error) {
			this.logService.error(
				`[ChatSessionWorktreeService][applyWorktreeChanges] Error applying patch file ${patchFilePath} to repository ${worktreeProperties.repositoryPath}: `,
				error,
			);
			throw error;
		} finally {
		...
		const ref = await this.gitService.getRefs(
			vscode.Uri.file(worktreeProperties.repositoryPath),
			{
		...
		if (
			ref.length === 1 &&
			ref[0].commit &&
			ref[0].commit !== worktreeProperties.baseCommit
		) {
	...
	async getWorktreeChanges(
		sessionId: string,
	): Promise<readonly vscode.ChatSessionChangedFile[] | undefined> {
		const worktreeProperties = await this.getWorktreeProperties(sessionId);
		if (!worktreeProperties || typeof worktreeProperties === 'string') {
			return undefined;
		}
		...
		if (worktreeProperties.changes) {
			return worktreeProperties.changes.map((change) =>
				this._toChatSessionChangedFile2(
					sessionId,
					change,
					worktreeProperties,
				),
		...
		try {
			...
			if (
				...
				const changes =
					(await this._getWorktreeChangesFromIndex(
						worktreeProperties,
					)) ?? [];
				await this.setWorktreeProperties(sessionId, {
					...worktreeProperties,
			...
			if (
				...
				const properties = vscode.workspace.isAgentSessionsWorkspace
					? await this._getWorktreeChanges(
							sessionId,
							worktreeProperties,
						)
					: await this._getWorktreeChangesFromCommits(
							worktreeProperties,
						);
			...
			const properties = await this._getWorktreeChanges(
				sessionId,
				worktreeProperties,
			);
			if (properties) {
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			this.logService.warn(
				`[ChatSessionWorktreeCheckpointService][getWorktreeChanges] Session ${sessionId}: error computing diff for committed changes, returning empty. Error: ${errorMessage}`,
			);
			await this.setWorktreeProperties(sessionId, {
				...worktreeProperties,
				changes: [],
	...
	async handleRequestCompleted(sessionId: string): Promise<void> {
		...
		if (worktreeProperties.autoCommit === false) {
			this.logService.trace(
				`[ChatSessionWorktreeService][handleRequestCompleted] Auto-commit is disabled, skipping commit of worktree changes for session ${sessionId}`,
			);
		...
		const repository = await this.gitCommitMessageService.getRepository(
			vscode.Uri.file(worktreePath),
		);
		if (!repository) {
			this.logService.error(
				`[ChatSessionWorktreeService][handleRequestCompleted] Unable to find repository for working directory ${worktreePath}`,
			);
			throw new Error(
		...
		if (
			...
			this.logService.trace(
				`[ChatSessionWorktreeService][handleRequestCompleted] No changes to commit in working directory ${worktreePath}`,
			);
		...
		try {
			this.logService.trace(
				`[ChatSessionWorktreeService][handleRequestCompleted] Generating commit message for working directory ${worktreePath}. Repository state: ${JSON.stringify(repository.state)}`,
			);
			message = await this.gitCommitMessageService.generateCommitMessage(
				repository,
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			this.logService.error(
				`[ChatSessionWorktreeService][handleRequestCompleted] Error generating commit message for working directory ${worktreePath}. Repository state: ${JSON.stringify(repository.state)}. Error: ${errorMessage}`,
			);
		}
		...
		if (!message) {
			...
			this.logService.warn(
				`[ChatSessionWorktreeService][handleRequestCompleted] Unable to generate commit message for working directory ${worktreePath}. Repository state: ${JSON.stringify(repository.state)}`,
			);
		...
		this.logService.trace(
			`[ChatSessionWorktreeService] Committed all changes in working directory ${worktreePath}`,
		);
	...
	async cleanupWorktreeOnArchive(
		sessionId: string,
		...
		try {
			...
			this.logService.trace(
				`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Worktree path does not exist: ${worktreePath}`,
			);
		...
		if (!repository) {
			this.logService.warn(
				`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Unable to find repository for worktree ${worktreePath}`,
			);
		...
		if (hasUncommittedChanges) {
			...
			if (worktreeProperties.autoCommit !== false) {
				this.logService.trace(
					`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Auto-committing changes before cleanup for session ${sessionId}`,
				);
				try {
					...
					const errorMessage =
						error instanceof Error ? error.message : String(error);
					this.logService.error(
						`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Failed to auto-commit: ${errorMessage}`,
					);
				...
				this.logService.trace(
					`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Skipping cleanup for session ${sessionId}: has uncommitted changes and auto-commit is disabled`,
				);
		...
		try {
			...
			if (!refs || refs.length === 0) {
				this.logService.warn(
					`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Branch ${worktreeProperties.branchName} not found, skipping cleanup`,
				);
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			this.logService.warn(
				`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Failed to verify branch: ${errorMessage}`,
			);
		...
		// Delete the worktree
		try {
			const parentRepository = await this.gitService.getRepository(
				vscode.Uri.file(worktreeProperties.repositoryPath),
				true,
			);
			if (!parentRepository) {
				this.logService.warn(
					`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] No parent repository found for ${worktreeProperties.repositoryPath}`,
				);
				return { cleaned: false, reason: 'no-parent-repository' };
			}
			await this.gitService.deleteWorktree(
				parentRepository.rootUri,
				worktreePath,
			);
			this.logService.trace(
				`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Deleted worktree ${worktreePath} for session ${sessionId}`,
			);
			return { cleaned: true };
		} catch (error) {
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			this.logService.error(
				`[ChatSessionWorktreeService][cleanupWorktreeOnArchive] Failed to delete worktree: ${errorMessage}`,
			);
	...
	async recreateWorktreeOnUnarchive(
		sessionId: string,
	): Promise<{ recreated: boolean; reason?: string }> {
		const worktreeProperties = await this.getWorktreeProperties(sessionId);
		if (!worktreeProperties) {
		...
		try {
			await fs.access(worktreePath);
			this.logService.trace(
				`[ChatSessionWorktreeService][recreateWorktreeOnUnarchive] Worktree already exists at ${worktreePath}`,
			);
		...
		try {
			...
			if (!refs || refs.length === 0) {
				this.logService.warn(
					`[ChatSessionWorktreeService][recreateWorktreeOnUnarchive] Branch ${worktreeProperties.branchName} no longer exists`,
				);
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			this.logService.warn(
				`[ChatSessionWorktreeService][recreateWorktreeOnUnarchive] Failed to verify branch: ${errorMessage}`,
			);
		...
		try {
			...
			if (!parentRepository) {
				this.logService.warn(
					`[ChatSessionWorktreeService][recreateWorktreeOnUnarchive] No parent repository found for ${worktreeProperties.repositoryPath}`,
				);
			...
			const createdPath = await this.gitService.createWorktree(
				parentRepository.rootUri,
				{
					path: worktreePath,
			...
			if (!createdPath) {
				this.logService.error(
					`[ChatSessionWorktreeService][recreateWorktreeOnUnarchive] createWorktree returned no path`,
				);
				return { recreated: false, reason: 'create-failed' };
			}

			this.logService.trace(
				`[ChatSessionWorktreeService][recreateWorktreeOnUnarchive] Recreated worktree at ${createdPath} for session ${sessionId}`,
			);
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			this.logService.error(
				`[ChatSessionWorktreeService][recreateWorktreeOnUnarchive] Failed to recreate worktree: ${errorMessage}`,
			);
			return { recreated: false, reason: 'create-failed' };
		}
	}

	private async _getWorktreeChangesFromIndex(
		...
		const worktreeRepository =
			await this.gitService.getRepository(worktreePath);
		...
		for (const change of [
			...worktreeRepository.changes.indexChanges,
			...worktreeRepository.changes.workingTree,
		]) {
			try {
				const fileStats =
					await this.gitService.diffIndexWithHEADShortStats(
						change.uri,
					);
				changes.push({
	...
	private async _getWorktreeChangesFromCommits(
		worktreeProperties: ChatSessionWorktreePropertiesV2,
	): Promise<{ changes: readonly ChatSessionWorktreeFile[] } | undefined> {
		...
		const repository = await this.gitService.getRepository(
			vscode.Uri.file(worktreeProperties.repositoryPath),
		);
	...
	private async _getWorktreeChanges(
		sessionId: string,
		worktreeProperties: ChatSessionWorktreeProperties,
	): Promise<
		| {
	...
	> {
		if (worktreeProperties.version !== 2) {
			this.logService.warn(
				`[ChatSessionWorktreeService][_getWorktreeChanges] Worktree properties for session ${sessionId} is not version 2.`,
			);
			return undefined;
		}
		...
		const worktreeRepository = await this.gitService.getRepository(
			vscode.Uri.file(worktreeProperties.worktreePath),
		);

		if (!worktreeRepository) {
			this.logService.warn(
				`[ChatSessionWorktreeService][_getWorktreeChanges] Unable to open worktree repository for session ${sessionId} at path ${worktreeProperties.worktreePath}`,
			);
			return undefined;
		}
		...
		if (hasUntrackedChanges) {
			...
			try {
				...
				await fs.mkdir(path.dirname(diffIndexFile), {
					recursive: true,
				});

				// Populate temp index from HEAD
				await this.gitService.exec(
					worktreePath,
					['read-tree', 'HEAD'],
					env,
				);
				...
				const uncommittedFilePaths =
					getUncommittedFilePaths(worktreeRepository);
				await fs.writeFile(
					pathspecFile,
					uncommittedFilePaths.join('\n'),
					'utf8',
				);
				await this.gitService.exec(
					worktreePath,
					['add', '-A', `--pathspec-from-file=${pathspecFile}`],
					env,
				);

				// Diff the temp index with the base branch
				const result = await this.gitService.exec(
					worktreePath,
					[
						'diff',
						'--cached',
				...
				this.logService.error(
					`[ChatSessionWorktreeService][_getWorktreeChanges] Error while processing worktree changes for session ${sessionId}: ${error}`,
				);
				return undefined;
			} finally {
				try {
					await fs.rm(path.dirname(diffIndexFile), {
						recursive: true,
						force: true,
					});
				} catch (error) {
					this.logService.error(
						`[ChatSessionWorktreeService][_getWorktreeChanges] Error while cleaning up temp index file for session ${sessionId}: ${error}`,
					);
				}
			}
		} else {
			// Tracked changes
			try {
				const result = await this.gitService.exec(worktreePath, [
					'diff',
					'--raw',
					'--numstat',
					'--diff-filter=ADMR',
		...
		try {
			mergeBaseCommit = await this.gitService.getMergeBase(
				worktreePath,
				worktreeProperties.branchName,
				worktreeProperties.baseBranchName,
			);
			...
			this.logService.error(
				`[ChatSessionWorktreeService][_getWorktreeChanges] Error while getting merge base (${worktreeProperties.branchName}, ${worktreeProperties.baseBranchName}) for session ${sessionId}: ${error}`,
			);
		}

		const changes = diffChanges.map(
			(change) =>
				({
					filePath: change.uri.fsPath,
					originalFilePath:
		...
		const repositoryState = {
			...
			upstreamBranchName:
				worktreeRepository.upstreamRemote &&
				worktreeRepository.upstreamBranchName
	...
	async getAdditionalWorktreeProperties(
		sessionId: string,
	): Promise<ChatSessionWorktreeProperties[]> {
		const additionalWorkspaces =
			await this.metadataStore.getAdditionalWorkspaces(sessionId);
	...
	async handleRequestCompletedForWorktree(
		...
		if (worktreeProperties.autoCommit === false) {
			this.logService.trace(
				`[ChatSessionWorktreeService][handleRequestCompletedForWorktree] Auto-commit is disabled, skipping commit for worktree ${worktreeProperties.worktreePath}`,
			);
			return;
		}
		...
		if (!repository) {
			this.logService.error(
				`[ChatSessionWorktreeService][handleRequestCompletedForWorktree] Unable to find repository for working directory ${worktreePath}`,
			);
			throw new Error(
				`Unable to find repository for working directory ${worktreePath}`,
			);
		...
		if (
			...
			this.logService.trace(
				`[ChatSessionWorktreeService][handleRequestCompletedForWorktree] No changes to commit in working directory ${worktreePath}`,
			);
			return;
		}
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			this.logService.error(
				`[ChatSessionWorktreeService][handleRequestCompletedForWorktree] Error generating commit message for ${worktreePath}: ${errorMessage}`,
			);
		}
		...
		this.logService.trace(
			`[ChatSessionWorktreeService][handleRequestCompletedForWorktree] Committed all changes in working directory ${worktreePath}`,
		);
	}
}
