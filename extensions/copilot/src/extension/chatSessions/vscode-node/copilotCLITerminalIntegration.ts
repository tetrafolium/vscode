...
import {
	CopilotCLITerminalLinkProvider,
	SessionDirResolver,
} from './copilotCLITerminalLinkProvider';
...
export const ICopilotCLITerminalIntegration =
	createServiceIdentifier<ICopilotCLITerminalIntegration>(
		'ICopilotCLITerminalIntegration',
	);

export class CopilotCLITerminalIntegration
	extends Disposable
	implements ICopilotCLITerminalIntegration
{
	...
	constructor(
		...
		if (
			configurationService.getConfig(ConfigKey.Advanced.CLITerminalLinks)
		) {
			this._linkProvider = new CopilotCLITerminalLinkProvider(
				logService,
				workspaceService,
			);
			this._register(
				window.registerTerminalLinkProvider(this._linkProvider),
			);
		}
	...
	private async initialize(): Promise<void> {
		...
		if (process.platform === 'win32') {
			this.powershellScriptPath = path.join(
				storageLocation,
				`${COPILOT_CLI_COMMAND}.ps1`,
			);
...
`;
			this.shellScriptPath = path.join(
				storageLocation,
				`${COPILOT_CLI_COMMAND}.bat`,
			);
			...
			this.posixShellScriptPath = path.join(
				storageLocation,
				COPILOT_CLI_COMMAND,
			);
...
ELECTRON_RUN_AS_NODE=1 "${process.execPath}" "${path.join(storageLocation, COPILOT_CLI_SHIM_JS)}" "$@"`;
			await fs.copyFile(
				path.join(__dirname, COPILOT_CLI_SHIM_JS),
				path.join(storageLocation, COPILOT_CLI_SHIM_JS),
			);
			this.shellScriptPath = path.join(
				storageLocation,
				COPILOT_CLI_COMMAND,
		...
		const provideTerminalProfile = async () => {
			...
			const options = await getCommonTerminalOptions(
				'GitHub Copilot CLI',
		...
		this._register(
			window.registerTerminalProfileProvider('copilot-cli', {
	...
	public async openTerminal(
		...
		if (
			shellPathAndArgs &&
			shellPathAndArgs.shell !== 'powershell' &&
			shellPathAndArgs.shell !== 'pwsh'
		) {
			const terminal =
				await this.pythonTerminalService.createTerminal(options);
			if (terminal) {
				this._register(terminal);
				this._linkProvider?.registerTerminal(terminal);
				const command = this.buildCommandForPythonTerminal(
					shellPathAndArgs?.copilotCommand,
					cliArgs,
					shellPathAndArgs,
				);
				await this.sendCommandToTerminal(
					terminal,
					command,
					true,
					shellPathAndArgs,
		...
		if (!shellPathAndArgs) {
			...
			const command = this.buildCommandForTerminal(
				terminal,
				COPILOT_CLI_COMMAND,
				cliArgs,
			);
	...
	private sendTerminalOpenTelemetry(
		sessionType: string,
		shell: string,
		terminalCreationMethod: string,
		location: TerminalOpenLocation,
	): void {
		/* __GDPR__
	...
	private buildCommandForPythonTerminal(
		copilotCommand: string,
		cliArgs: string[],
	...
	private async sendCommandToTerminal(
		terminal: Terminal,
		command: string,
		waitForPythonActivation: boolean,
		shellInfo: IShellInfo | undefined = undefined,
	): Promise<void> {
		// Wait for shell integration to be available
		const shellIntegrationTimeout = 3000;
		let shellIntegrationAvailable = terminal.shellIntegration
			? true
			: false;
		const disposables = new DisposableStore();
		const integrationPromise = shellIntegrationAvailable
			? Promise.resolve()
			: new Promise<void>((resolve) => {
					const disposable = disposables.add(
						this.terminalService.onDidChangeTerminalShellIntegration(
							(e) => {
								if (
									e.terminal === terminal &&
									e.shellIntegration
		...
		try {
			...
			if (waitForPythonActivation) {
				...
				const delay =
					shellInfo?.shell === 'powershell' ||
					shellInfo?.shell === 'pwsh'
						? 3000
						: 1000;
				await new Promise<void>((resolve) =>
					disposables.add(disposableTimeout(resolve, delay)),
	...
	private async getShellInfo(
		...
		const defaultProfileName = workspace
			.getConfiguration('terminal')
			.get<
				string | undefined
			>(`integrated.defaultProfile.${configPlatform}`);
		...
		if (defaultProfileName) {
			const profiles = workspace
				...
				.get<
					Record<
						string,
						{ path?: string | string[]; args?: string[] }
					>
				>(`integrated.profiles.${configPlatform}`);
		...
		if (shellBasename === 'zsh' && this.shellScriptPath) {
			return {
				...
				shellPath,
				shellArgs: [
					`-ci${shellArgs.includes('-l') ? 'l' : ''}`,
					quoteArgsForShell(this.shellScriptPath, cliArgs),
				],
				iconPath,
			...
			(shellBasename === 'bash' || shellBasename === 'bash.exe') &&
			(configPlatform === 'windows'
				? this.posixShellScriptPath
			...
			const bashScriptPath =
				configPlatform === 'windows'
					? windowsToGitBashPath(scriptPath)
					: scriptPath;
			return {
				shell: 'bash',
				shellPath,
				shellArgs: [
					`-${shellArgs.includes('-l') ? 'l' : ''}ic`,
					quoteArgsForShell(bashScriptPath, cliArgs),
				],
				iconPath,
			...
			(shellBasename === 'pwsh' || shellBasename === 'pwsh.exe') &&
			this.powershellScriptPath
		) {
			return {
				shell: 'pwsh',
				shellPath,
			...
			(shellBasename === 'powershell' ||
				shellBasename === 'powershell.exe') &&
			this.powershellScriptPath &&
			configPlatform === 'windows'
		) {
			return {
			...
			(shellBasename === 'cmd' || shellBasename === 'cmd.exe') &&
			this.shellScriptPath &&
			configPlatform === 'windows'
		) {
			return {
...
function quoteArgsForShell(shellScript: string, args: string[]): string {
	...
	return args.length
		? `${escapeArg(shellScript)} ${escapedArgs.join(' ')}`
		: escapeArg(shellScript);
}

async function getCommonTerminalOptions(
	name: string,
	authenticationService: IAuthenticationService,
	otelService: IOTelService,
	location: TerminalOpenLocation = 'editor',
): Promise<TerminalOptions> {
	...
	if (location === 'panel') {
		...
		options.location = {
			viewColumn:
				location === 'editorBeside'
					? ViewColumn.Beside
					: ViewColumn.Active,
		};
	}
	const session = await authenticationService.getGitHubSession('any', {
		silent: true,
	});
	if (session) {
		options.env = {
