 ...
 */
export class ChatSessionWorkspaceFolderService
	extends Disposable
	implements IChatSessionWorkspaceFolderService
{
	declare _serviceBrand: undefined;

	private static readonly EMPTY_TREE_OBJECT =
		'4b825dc642cb6eb9a060e54bf8d69288fbee4904';
	private readonly _onDidChangeWorkspaceFolderChanges = this._register(
		new vscode.EventEmitter<{ sessionId: string }>(),
	);
	readonly onDidChangeWorkspaceFolderChanges =
		this._onDidChangeWorkspaceFolderChanges.event;
	...
	private readonly workspaceFolderChanges = new Map<
		string,
		ChatSessionWorktreeFile[]
	>();
	private readonly sessionsAssociatedWithFolders = new ResourceMap<
		Set<string>
	>();
	...
	async deleteTrackedWorkspaceFolder(sessionId: string): Promise<void> {
		...
		if (entry?.folderPath) {
			...
			this.sessionsAssociatedWithFolders
				.get(folderUri)
				?.delete(sessionId);
		}
	...
	async trackSessionWorkspaceFolder(
		...
		const sessionIds =
			this.sessionsAssociatedWithFolders.get(folderUri) ??
			new Set<string>();
		...
		if (repositoryProperties) {
			...
			await this.metadataStore.storeRepositoryProperties(
				sessionId,
				repositoryProperties,
			);
		}
		this.logService.trace(
			`[ChatSessionWorkspaceFolderService] Tracked workspace folder ${workspaceFolderUri} for session ${sessionId}`,
		);
	}
	...
	async getSessionWorkspaceFolderEntry(
		...
		return await this.metadataStore.getSessionWorkspaceFolderEntry(
			sessionId,
		);
	}
	...
	async hasCachedChanges(sessionId: string): Promise<boolean> {
		...
		const cachedChanges = existingRepoKey
			? this.workspaceFolderChanges.get(existingRepoKey)
			: undefined;
	...
	async getWorkspaceChanges(
		...
		return this.workspaceChangesSequencer.queue(sessionId, async () => {
			...
			const repositoryProperties =
				await this.getRepositoryProperties(sessionId);
			if (!repositoryProperties) {
				this.logService.warn(
					`[ChatSessionWorkspaceFolderService][getWorkspaceChanges] No repository properties found for session ${sessionId}`,
				);
			...
			return this.repoChangesSequencer.queue(repoKey, async () => {
				...
				const properties = await this.computeWorkspaceChanges(
					repositoryProperties,
					sessionId,
				);
				this.workspaceFolderChanges.set(
					repoKey,
					properties?.changes ?? [],
				);
	...
	private async computeWorkspaceChanges(
		repositoryProperties: RepositoryProperties,
		sessionId: string,
	): Promise<
		| {
	...
	> {
		...
		if (repository) {
			const sessionIds =
				this.sessionsAssociatedWithFolders.get(repository.rootUri) ??
				new Set<string>();
			sessionIds.add(sessionId);
			this.sessionsAssociatedWithFolders.set(
				repository.rootUri,
				sessionIds,
			);
		}
		if (!repository?.changes) {
			this.logService.warn(
				`[ChatSessionWorkspaceFolderService][getWorkspaceChanges] No repository found for session ${sessionId}`,
			);
			return undefined;
		}
		...
		const hasUntrackedChanges =
			repositoryProperties.branchName === repository.headBranchName
				? [
		...
		if (hasUntrackedChanges) {
			...
			try {
				...
				try {
					...
					await this.gitService.exec(
						...
						[
							'read-tree',
							ChatSessionWorkspaceFolderService.EMPTY_TREE_OBJECT,
						],
						env,
					);
				}
				...
				const uncommittedFilePaths =
					getUncommittedFilePaths(repository);
				await fs.writeFile(
					pathspecFile,
				...
				diffChanges.push(
					...parseGitChangesRaw(repository.rootUri.fsPath, result),
				);
			} catch (error) {
				this.logService.error(
					`[ChatSessionWorkspaceFolderService][getWorkspaceChanges] Error while processing workspace changes: ${error}`,
				);
				return undefined;
			} finally {
				try {
					...
					this.logService.error(
						`[ChatSessionWorkspaceFolderService][getWorkspaceChanges] Error while cleaning up temp index file: ${error}`,
					);
				}
			}
		} else {
			// Tracked changes
			try {
				const result = await this.gitService.exec(repository.rootUri, [
					'diff',
					'--raw',
					'--numstat',
					'--diff-filter=ADMR',
		...
		try {
			if (
				repositoryProperties.branchName &&
				repositoryProperties.baseBranchName
			) {
			...
			this.logService.error(
				`[ChatSessionWorkspaceFolderService][getWorkspaceChanges] Error while getting merge base (${repositoryProperties.branchName}, ${repositoryProperties.baseBranchName}): ${error}`,
			);
		}
		...
		const repositoryState = {
			mergeBaseCommit,
			hasGitHubRemote:
				getGitHubRepoInfoFromContext(repository) !== undefined,
			upstreamBranchName:
	...
	clearWorkspaceChanges(sessionIdOrFolderUri: string | vscode.Uri): string[] {
		const sessionIds =
			typeof sessionIdOrFolderUri === 'string'
				? [sessionIdOrFolderUri]
				: this.getAssociatedSessions(sessionIdOrFolderUri);
