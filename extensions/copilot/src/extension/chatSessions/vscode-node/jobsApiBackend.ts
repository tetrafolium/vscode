...
export class JobsApiBackend implements PrCloudAgentBackend {
	...
	async fetchSessionList(
		...
		for (const session of sessions) {
			...
			if (
				!existing ||
				new Date(session.last_updated_at) >
					new Date(existing.last_updated_at)
			) {
		...
		const uniqueGlobalIds = new Set(
			Array.from(latestSessionsMap.values()).map(
				(s) => s.resource_global_id,
			),
		);
		const prFetches = Array.from(uniqueGlobalIds).map(async (globalId) => {
			try {
				const pr =
					await this._octoKitService.getPullRequestFromGlobalId(
						globalId,
						{},
					);
				return { globalId, pr };
				...
				this._logService.warn(
					`Failed to fetch PR for global ID ${globalId}: ${e instanceof Error ? e.message : String(e)}`,
				);
		...
		const prMap = new Map(
			prResults.filter((r) => r.pr).map((r) => [r.globalId, r.pr!]),
		);
	...
	fetchSessionsForPullRequest(
		...
		return this._octoKitService.getCopilotSessionsForPR(
			pr.fullDatabaseId.toString(),
			CLOUD_SESSIONS_AUTH_OPTIONS,
		);
	}

	async sendFollowUpToPullRequest(
		prGlobalId: string,
		prompt: string,
		targetAgent?: string,
	): Promise<FollowUpResult | undefined> {
		const agent =
			targetAgent && targetAgent.length > 0 ? targetAgent : 'copilot';
		// Trailing space preserved for byte-identical bodies vs the pre-seam inline implementation.
		const body = `@${agent} ${prompt} `;
		const commentResult = await this._octoKitService.addPullRequestComment(
			prGlobalId,
			body,
			CLOUD_SESSIONS_AUTH_OPTIONS,
		);
	...
	async createSession(
		...
		*/
		...
		this._logService.debug(
			`[postCopilotAgentJob] Invoking cloud agent job with payload: ${JSON.stringify(payload)}`,
		);
		const response = await this._octoKitService.postCopilotAgentJob(
			params.owner,
			params.repo,
			JOBS_API_VERSION,
			payload,
		...
		this._logService.debug(
			`[postCopilotAgentJob] Received response from cloud agent job invocation: ${JSON.stringify(response)}`,
		);
		if (!this.validateRemoteAgentJobResponse(response)) {
			const statusCode = (response as { status?: number } | undefined)
				?.status;
			switch (statusCode) {
				case 401:
					throw new Error(
				...
				case 403:
					throw new Error(
						vscode.l10n.t(
							...
							`https://${params.host}/settings/copilot/coding_agent`,
						),
					);
				case 404:
					throw new Error(
						vscode.l10n.t(
							'The repository `{0}/{1}` was not found or you do not have access to it.',
							params.owner,
							params.repo,
						),
					);
				case 422:
					throw new Error(
						vscode.l10n.t(
							'Cloud agent was unable to create a pull request with the specified base branch `{0}`. Please push the branch to the remote and verify repository rules allow this operation. For empty repos, push an initial commit and try again.',
							params.baseRef,
						),
					);
				case 500:
					throw new Error(
						vscode.l10n.t(
							'Cloud agent service encountered an internal error. Please try again later.',
						),
					);
				default:
					throw new Error(
						vscode.l10n.t(
							'Received invalid response {0} from cloud agent.',
		...
		const jobInfo = await this.waitForJobWithPullRequest(
			params.owner,
			params.repo,
			response.job_id,
			token,
		...
		if (!jobInfo || !jobInfo.pull_request) {
			throw new Error(
				vscode.l10n.t(
					'Failed to retrieve pull request information from job',
				),
			);
		}
		...
		if (!number || isNaN(number)) {
			throw new Error(
				vscode.l10n.t(
					'Invalid pull request number received from cloud agent',
				),
			);
		}
		return {
	...
	async waitForSessionReady(
		...
		do {
			sessionInfo = await this._octoKitService.getSessionInfo(
				sessionId,
				CLOUD_SESSIONS_AUTH_OPTIONS,
			);
			...
			if (waitForQueuedCount < waitForQueuedMaxRetries) {
				...
				await new Promise((resolve) =>
					setTimeout(resolve, waitForQueuedDelay),
				);
			}
			++waitForQueuedCount;
		} while (
			waitForQueuedCount <= waitForQueuedMaxRetries &&
		...
		this._logService.trace(
			`Session ${sessionInfo.id} is queued, waiting for transition to in_progress...`,
		);
		while (
			Date.now() - startTime < maxWaitTime &&
			(!token || !token.isCancellationRequested)
		) {
			const info = await this._octoKitService.getSessionInfo(
				sessionId,
				CLOUD_SESSIONS_AUTH_OPTIONS,
			);
		...
		this._logService.error(
			`Timed out waiting for session ${sessionId} to transition from queued to in_progress.`,
		);
		return undefined;
	}

	// https://github.com/github/sweagentd/blob/main/docs/adr/0001-create-job-api.md
	private validateRemoteAgentJobResponse(
		response: unknown,
	): response is RemoteAgentJobResponse {
		return (
	...
	private async fetchAllSessions(
		...
		const all = await Promise.all(
			repoIds.map((repo) =>
				this._octoKitService.getAllSessions(
					`${repo.org}/${repo.repo}`,
					refresh,
					{},
				),
	...
	private async waitForJobWithPullRequest(
		...
		this._logService.trace(
			`Waiting for job ${jobId} to have pull request information...`,
		);
		...
		while (
			...
			const jobInfo = await this._octoKitService.getJobByJobId(
				owner,
				repo,
				jobId,
				'vscode-copilot-chat',
			...
			if (
				jobInfo &&
				jobInfo.pull_request &&
				jobInfo.pull_request.number
			) {
				/* __GDPR__
				...
				*/
				...
				this._logService.trace(
					`Job ${jobId} now has pull request #${jobInfo.pull_request.number}`,
				);
				return jobInfo;
			}
			await new Promise((resolve) => setTimeout(resolve, pollInterval));
		...
		this._logService.warn(
			`Timed out waiting for job ${jobId} to have pull request information`,
		);
		return undefined;
	}
}
