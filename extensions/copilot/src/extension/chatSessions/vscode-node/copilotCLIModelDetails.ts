 ...
 */
export async function getCopilotCLIModelDetails(
	session: ICopilotCLISession,
	requestModel: { model: string; reasoningEffort?: string } | undefined,
	copilotCLIModels: ICopilotCLIModels,
	logService: ILogService,
	enabled: boolean,
	creditsUsed?: number,
): Promise<CopilotCLIModelDetails> {
	if (!enabled) {
	...
	const modelInfo = [responseModelId, selectedModelId, requestModel?.model]
		.map((modelId) =>
			modelId
				? models.find((model) => matchesCopilotCLIModel(model, modelId))
				: undefined,
		)
 ...
 */
export async function persistCopilotCLIResponseModelId(
	sessionId: string,
	requestId: string,
	...
	try {
		await chatSessionMetadataStore.updateRequestDetails(sessionId, [
			{ vscodeRequestId: requestId, responseModelId, creditsUsed },
		]);
	} catch (ex) {
