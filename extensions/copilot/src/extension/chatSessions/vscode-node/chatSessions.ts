...
import {
	...
	LanguageModelServer,
} from '../../agents/node/langModelServer';
...
} from '../claude/common/claudeToolPermissionService';
import {
	ClaudePlanFileTracker,
	IClaudePlanFileTracker,
} from '../claude/common/claudePlanFileTracker';
...
} from '../claude/node/sessionParser/claudeCodeSessionService';
import {
	ClaudeSlashCommandService,
	IClaudeSlashCommandService,
} from '../claude/vscode-node/claudeSlashCommandService';
...
import {
	ChatDelegationSummaryService,
	IChatDelegationSummaryService,
} from '../copilotcli/common/delegationSummaryService';
...
import {
	CopilotCLIImageSupport,
	ICopilotCLIImageSupport,
} from '../copilotcli/node/copilotCLIImageSupport';
...
import {
	CopilotCLISkills,
	ICopilotCLISkills,
} from '../copilotcli/node/copilotCLISkills';
import {
...
import {
	CopilotCLIContrib,
	getServices,
} from '../copilotcli/vscode-node/contribution';
...
import {
	CopilotCLIChatSessionInitializer,
...
} from './copilotCLIChatSessions';
import {
	CopilotCLIChatSessionContentProvider as CopilotCLIChatSessionContentProviderV1,
	CopilotCLIChatSessionItemProvider as CopilotCLIChatSessionItemProviderV1,
	CopilotCLIChatSessionParticipant as CopilotCLIChatSessionParticipantV1,
	registerCLIChatCommands as registerCLIChatCommandsV1,
	...
	implements IExtensionContribution
{
	...
	constructor(
		...
		const summarizer = instantiationService.createInstance(
			ChatSummarizerProvider,
		);
		const delegationSummary = instantiationService.createInstance(
			ChatDelegationSummaryService,
			summarizer,
		);
		this._register(
			vscode.workspace.registerTextDocumentContentProvider(
				delegationSummary.scheme,
				{
					provideTextDocumentContent: (
						uri: vscode.Uri,
					): string | undefined =>
						delegationSummary.provideTextDocumentContent(uri),
				},
			),
		);
		...
		const { sessionMetadata } = useController
			? this.registerCopilotCLIServices(
					instantiationService,
					delegationSummary,
					logService,
				)
			: this.registerCopilotCLIServicesV1(
					instantiationService,
					delegationSummary,
					logService,
				);
		...
		const claudeAgentInstaService = instantiationService.createChild(
			new ServiceCollection(
				[
					...
					new SyncDescriptor(AgentSessionsWorkspace),
				],
				[
					IClaudeCodeSessionService,
					new SyncDescriptor(ClaudeCodeSessionService),
				],
				[
					IClaudeCodeSdkService,
					new SyncDescriptor(ClaudeCodeSdkService),
				],
				[IClaudeCodeModels, new SyncDescriptor(ClaudeCodeModels)],
				[ILanguageModelServer, new SyncDescriptor(LanguageModelServer)],
				[
					IClaudeToolPermissionService,
					new SyncDescriptor(ClaudeToolPermissionService),
				],
				[
					IClaudePlanFileTracker,
				...
				[
					...
					new SyncDescriptor(ClaudeSessionStateService),
				],
				[
					IClaudeSlashCommandService,
				...
				[
					...
					new SyncDescriptor(ChatSessionWorktreeService),
				],
				[
					IChatSessionWorktreeCheckpointService,
					new SyncDescriptor(ChatSessionWorktreeCheckpointService),
				],
				[
					IChatSessionWorkspaceFolderService,
					new SyncDescriptor(ChatSessionWorkspaceFolderService),
				],
				[
					IClaudeWorkspaceFolderService,
					new SyncDescriptor(ClaudeWorkspaceFolderService),
				],
				[
					IFolderRepositoryManager,
					new SyncDescriptor(ClaudeFolderRepositoryManager),
				],
				[
					IChatFolderMruService,
				...
				[
					...
					new SyncDescriptor(ClaudeRuntimeDataService),
				],
				[IClaudePluginService, new SyncDescriptor(ClaudePluginService)],
			),
		);
		const claudeAgentManager = this._register(
			claudeAgentInstaService.createInstance(ClaudeAgentManager),
		);
		const claudeModels = claudeAgentInstaService.invokeFunction(
			(accessor) => accessor.get(IClaudeCodeModels),
		);
		claudeModels.registerLanguageModelChatProvider(vscode.lm);
		const chatSessionContentProvider = this._register(
			claudeAgentInstaService.createInstance(
		...
		const chatParticipant = vscode.chat.createChatParticipant(
			...
			chatSessionContentProvider.createHandler(),
		);
		...
		const claudeCustomizationProvider = this._register(
			claudeAgentInstaService.createInstance(ClaudeCustomizationProvider),
		);
		this._register(
			vscode.chat.registerChatSessionCustomizationProvider(
				ClaudeSessionUri.scheme,
				ClaudeCustomizationProvider.metadata,
				claudeCustomizationProvider,
			),
		);
	...
	private registerCopilotCLIServices(
		instantiationService: IInstantiationService,
		delegationSummary: IChatDelegationSummaryService,
		...
		const copilotcliAgentInstaService = instantiationService.createChild(
			new ServiceCollection(
				...
				[
					ICopilotCLIImageSupport,
					new SyncDescriptor(CopilotCLIImageSupport),
				],
				[
					ICopilotCLISessionService,
					new SyncDescriptor(CopilotCLISessionService),
				],
				[IChatDelegationSummaryService, delegationSummary],
				...
				[
					ICopilotCLITerminalIntegration,
					new SyncDescriptor(CopilotCLITerminalIntegration),
				],
				[
					IChatSessionWorktreeService,
					new SyncDescriptor(ChatSessionWorktreeService),
				...
				[
					ICopilotCLIMCPHandler,
					new SyncDescriptor(CopilotCLIMCPHandler),
				],
				[
					IFolderRepositoryManager,
					new SyncDescriptor(CopilotCLIFolderRepositoryManager),
				],
				[IUserQuestionHandler, new SyncDescriptor(UserQuestionHandler)],
				[
					ICustomSessionTitleService,
					new SyncDescriptor(CustomSessionTitleService),
				],
				[ICopilotCLISkills, new SyncDescriptor(CopilotCLISkills)],
				[
					IChatSessionMetadataStore,
					new SyncDescriptor(ChatSessionMetadataStore),
				],
				[
					IChatFolderMruService,
					new SyncDescriptor(CopilotCLIFolderMruService),
				],
				[
					IPullRequestCreationService,
					new SyncDescriptor(PullRequestCreationService),
				],
				[
					IPullRequestDetectionService,
					new SyncDescriptor(PullRequestDetectionService),
				],
				[
					ISessionOptionGroupBuilder,
					new SyncDescriptor(SessionOptionGroupBuilder),
				],
				[
					ISessionRequestLifecycle,
					new SyncDescriptor(SessionRequestLifecycle),
				],
				[
					ICopilotCLIChatSessionInitializer,
		...
		const copilotcliChatSessionContentProvider =
			copilotcliAgentInstaService.createInstance(
				CopilotCLIChatSessionContentProvider,
			);
		this._register(
			copilotcliAgentInstaService.createInstance(
				ChatSessionRepositoryTracker,
				undefined,
			),
		);
		const promptResolver = copilotcliAgentInstaService.createInstance(
			CopilotCLIPromptResolver,
		);
		const gitService = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(IGitService),
		);
		const gitCommitMessageService =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IGitCommitMessageService),
			);
		const sessionTracker = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(ICopilotCLISessionTracker),
		);
		const terminalIntegration = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(ICopilotCLITerminalIntegration),
		);
		const aiGeneratedBranchNames = instantiationService.invokeFunction(
			(accessor) =>
				accessor
					.get(IConfigurationService)
					.getConfig(ConfigKey.Advanced.CLIAIGenerateBranchNames),
		);
		const branchNameGenerator = aiGeneratedBranchNames
			? copilotcliAgentInstaService.createInstance(GitBranchNameGenerator)
			: undefined;

		const copilotcliChatSessionParticipant = this._register(
			copilotcliAgentInstaService.createInstance(
				CopilotCLIChatSessionParticipant,
				copilotcliChatSessionContentProvider,
				promptResolver,
		...
		const copilotCLISessionService =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(ICopilotCLISessionService),
			);
		const copilotCLIWorktreeManagerService =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IChatSessionWorktreeService),
			);
		const copilotCLIWorktreeCheckpointService =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IChatSessionWorktreeCheckpointService),
			);
		const copilotCLIWorkspaceFolderSessions =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IChatSessionWorkspaceFolderService),
			);
		const folderRepositoryManager =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IFolderRepositoryManager),
			);
		const nativeEnvService = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(INativeEnvService),
		);
		const fileSystemService = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(IFileSystemService),
		);
		const copilotModels = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(ICopilotCLIModels),
		);
		const copilotCLIFolderMruService =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IChatFolderMruService),
			);
		const pullRequestCreationService =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IPullRequestCreationService),
			);
		const sessionMetadata = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(IChatSessionMetadataStore),
		);

		this._register(
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(ICopilotCLISessionTracker),
			),
		);
		this._register(
			copilotcliAgentInstaService.createInstance(CopilotCLIContrib),
		);

		copilotModels.registerLanguageModelChatProvider(vscode.lm);

		const copilotcliParticipant = vscode.chat.createChatParticipant(
		...
		const copilotcliCustomizationProvider = this._register(
			copilotcliAgentInstaService.createInstance(
		...
		this._register(
			registerCLIChatCommands(
				copilotCLISessionService,
				copilotCLIWorktreeManagerService,
				copilotCLIWorktreeCheckpointService,
				gitService,
				gitCommitMessageService,
				copilotCLIWorkspaceFolderSessions,
				copilotcliChatSessionContentProvider,
				folderRepositoryManager,
				copilotCLIFolderMruService,
				nativeEnvService,
				fileSystemService,
				sessionTracker,
				terminalIntegration,
	...
	private registerCopilotCLIServicesV1(
		instantiationService: IInstantiationService,
		delegationSummary: IChatDelegationSummaryService,
		logService: ILogService,
	) {
		...
		const copilotcliSessionItemProvider = this._register(
			copilotcliAgentInstaService.createInstance(
				CopilotCLIChatSessionItemProviderV1,
			),
		);
		...
		const copilotcliChatSessionContentProvider =
			copilotcliAgentInstaService.createInstance(
				CopilotCLIChatSessionContentProviderV1,
				copilotcliSessionItemProvider,
			);
		const promptResolver = copilotcliAgentInstaService.createInstance(
			CopilotCLIPromptResolver,
		...
		const gitExtensionService = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(IGitExtensionService),
		);
		const toolsService = copilotcliAgentInstaService.invokeFunction(
			(accessor) => accessor.get(IToolsService),
		);
		const aiGeneratedBranchNamesV1 = instantiationService.invokeFunction(
			(accessor) =>
				accessor
					.get(IConfigurationService)
					.getConfig(ConfigKey.Advanced.CLIAIGenerateBranchNames),
		);
		const branchNameGeneratorV1 = aiGeneratedBranchNamesV1
			? copilotcliAgentInstaService.createInstance(GitBranchNameGenerator)
			: undefined;
		...
		const copilotCLIMetadataStore =
			copilotcliAgentInstaService.invokeFunction((accessor) =>
				accessor.get(IChatSessionMetadataStore),
			);

		// Handle worktree cleanup/recreation when archive state changes
		const onDidChangeChatSessionItemState = (
			providerRegistration as {
				onDidChangeChatSessionItemState?: vscode.Event<vscode.ChatSessionItem>;
			}
		).onDidChangeChatSessionItemState;
		if (onDidChangeChatSessionItemState) {
			this._register(
				onDidChangeChatSessionItemState(async (item) => {
					...
					try {
						await copilotCLIMetadataStore.setSessionArchived(
							sessionId,
							!!item.archived,
						);
					} catch (error) {
					...
					if (item.archived) {
						...
						try {
							const result =
								await copilotCLIWorktreeManagerService.cleanupWorktreeOnArchive(
									sessionId,
								);
							logService.trace(
						...
						try {
							const result =
								await copilotCLIWorktreeManagerService.recreateWorktreeOnUnarchive(
									sessionId,
								);
							logService.trace(
							...
							if (result.recreated) {
								copilotcliSessionItemProvider.refreshSession({
									reason: 'update',
									sessionId,
								});
							}
	...
	private registerCopilotCloudAgent() {
		...
		const cloudSessionsProvider = this.copilotCloudRegistrations.add(
			this.copilotAgentInstaService.createInstance(
		...
		this.copilotCloudRegistrations.add(
			vscode.commands.registerCommand(
				...
				async (chatSessionItem: vscode.ChatSessionItem) => {
		...
		this.copilotCloudRegistrations.add(
			vscode.commands.registerCommand(
				CLOSE_SESSION_PR_CMD,
				async (ctx: CrossChatSessionWithPR) => {
					try {
						const success =
							await this.octoKitService.closePullRequest(
						...
						if (!success) {
							this.logService.error(
								`${CLOSE_SESSION_PR_CMD}: Failed to close PR #${ctx.pullRequestDetails.number}`,
							);
						}
						cloudSessionsProvider.refresh();
					} catch (e) {
						this.logService.error(
							`${CLOSE_SESSION_PR_CMD}: Exception ${e}`,
						);
					}
				},
			),
		...
		this.copilotCloudRegistrations.add(
			vscode.commands.registerCommand(
				'github.copilot.cloud.sessions.installPRExtension',
				async () => {
	...
	private async installPullRequestExtension(): Promise<void> {
		...
		try {
			const isInsiders = this.envService
				.getEditorInfo()
				.version.includes('insider');
			const installOptions = {
				enable: true,
				installPreReleaseVersion: isInsiders,
				justification: vscode.l10n.t(
					'Enable additional pull request features, such as checking out and applying changes.',
				),
			};
			...
			while (elapsed < maxWaitTime) {
				if (this.isPullRequestExtensionInstalled()) {
					vscode.window.showInformationMessage(
						vscode.l10n.t(
							'GitHub Pull Request extension installed successfully.',
						),
					);
					break;
				}
			...
			if (!this.isPullRequestExtensionInstalled()) {
				vscode.window.showWarningMessage(
					vscode.l10n.t(
						'GitHub Pull Request extension is taking longer than expected to install.',
					),
				);
			}
			await vscode.commands.executeCommand(
				'setContext',
				prExtensionInstalledContextKey,
				true,
			);
		} catch (error) {
			vscode.window.showErrorMessage(
				vscode.l10n.t(
					'Failed to install GitHub Pull Request extension: {0}',
