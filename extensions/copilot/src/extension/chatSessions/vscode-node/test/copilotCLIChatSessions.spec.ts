	...
	(vscodeShim as Record<string, unknown>).chat = {
		createChatSessionItemController: () => ({
			...
			createChatSessionItem: (
				resource: vscode.Uri,
				label: string,
			): vscode.ChatSessionItem => ({ resource, label }),
			dispose: () => {},
		}),
	};
...
function createProvider() {
	...
	const metadataStore = new (class extends mock<IChatSessionMetadataStore>() {
		...
		override getSessionParentId = vi.fn<
			IChatSessionMetadataStore['getSessionParentId']
		>(async () => undefined);
	})();
	...
	const optionGroupBuilder =
		new (class extends mock<ISessionOptionGroupBuilder>() {
			declare readonly _serviceBrand: undefined;
			override provideChatSessionProviderOptionGroups = vi.fn(
				async () => [],
			);
			...
			override buildExistingSessionInputStateGroups = vi.fn(
				async () => [],
			);
...
describe('CopilotCLIChatSessionContentProvider', () => {
	...
	it('skips session-open detection for merged pull requests', async () => {
		const { prDetectionService, worktreeService, octoKitService } =
			createProvider();
		...
		await vi.waitFor(() =>
			expect(worktreeService.getWorktreeProperties).toHaveBeenCalled(),
		);
		expect(
...
describe('CopilotCLIChatSessionParticipant', () => {
	...
	it('keeps local /remote output visible for new sessions', async () => {
		...
		const sessionService = new TestSessionService();
		sessionService.isNewSessionId.mockImplementation(
			(id) => id === 'new-session',
		);
		...
		const participant = new CopilotCLIChatSessionParticipant(
			...
			new (class extends mock<ICopilotCLISDK>() {
				declare readonly _serviceBrand: undefined;
				override getAuthInfo = vi.fn(async () => ({
					type: 'token' as const,
					token: 'token',
...
describe('re-exported dropdown helpers', () => {
	it('resolveBranchSelection is callable', () => {
		...
		expect(resolveBranchSelection(branches, 'main', undefined)?.id).toBe(
			'main',
		);
	});
	...
	it('resolveIsolationSelection is callable', () => {
		expect(
			resolveIsolationSelection(IsolationMode.Workspace, undefined),
	...
	it('returns matching terminal sessions before non-matching ones', async () => {
		...
		const tracker: ICopilotCLISessionTracker = {
			...
			getTerminal: vi.fn(async (id: string) =>
				id === 'session-a' ? terminal : otherTerminal,
			),
...
describe('CopilotCLIChatSessionContentProvider (additional)', () => {
	...
	it('does not call refreshSession when PR detection finds no update', async () => {
		const { provider, prDetectionService, worktreeService } =
			createProvider();
		const refreshSpy = vi
			.spyOn(provider, 'refreshSession')
			.mockResolvedValue();
