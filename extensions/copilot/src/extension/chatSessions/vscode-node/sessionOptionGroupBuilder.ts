...
export const OPEN_REPOSITORY_COMMAND_ID =
	'github.copilot.cli.sessions.openRepository';
...
export function resolveIsolationSelection(
	...
	if (
		previousSelectionId === IsolationMode.Workspace ||
		previousSelectionId === IsolationMode.Worktree
	) {
...
function optionItemsEqual(
	a: vscode.ChatSessionProviderOptionItem | undefined,
	b: vscode.ChatSessionProviderOptionItem | undefined,
): boolean {
	if (a === b) {
		return true;
	}
...
export function getSelectedOption(
	...
	groupId: string,
): vscode.ChatSessionProviderOptionItem | undefined {
 ...
 */
export function getSelectedSessionOptions(
	inputState: vscode.ChatSessionInputState,
): { folder?: vscode.Uri; branch?: string; isolation?: IsolationMode } {
	const repoId = getSelectedOption(
		inputState.groups,
		REPOSITORY_OPTION_ID,
	)?.id;
	const branch = getSelectedOption(inputState.groups, BRANCH_OPTION_ID)?.id;
	const isolationId = getSelectedOption(
		inputState.groups,
		ISOLATION_OPTION_ID,
	)?.id;
	return {
		...
		isolation:
			isolationId === IsolationMode.Workspace ||
			isolationId === IsolationMode.Worktree
				? isolationId
				: undefined,
	};
}

export function isBranchOptionFeatureEnabled(
 ...
 */
function forceWorkspaceIsolation(
	...
	if (isolationIdx !== -1) {
		...
		const workspaceItem = isolationGroup.items.find(
			(i) => i.id === IsolationMode.Workspace,
		);
		if (workspaceItem) {
			groups[isolationIdx] = {
				...isolationGroup,
				items: isolationGroup.items.map((i) => ({
					...i,
					locked: true,
				})),
...
function resetIsolationLock(
	...
	if (isolationIdx !== -1) {
		const isolationGroup = groups[isolationIdx];
		const unlock = (
			item: vscode.ChatSessionProviderOptionItem,
		...
		groups[isolationIdx] = {
			...
			selected: isolationGroup.selected
				? unlock(isolationGroup.selected)
				: undefined,
		};
	}
}

export function isIsolationOptionFeatureEnabled(
	configurationService: IConfigurationService,
): boolean {
...
export function toRepositoryOptionItem(
...
export function toWorkspaceFolderOptionItem(
...
export function folderMRUToChatProviderOptions(
...
export interface ISessionOptionGroupBuilder {
	...
	provideChatSessionProviderOptionGroups(
	...
	rebuildInputState(
		...
		selectedFolderUri?: vscode.Uri,
	): Promise<void>;
	buildExistingSessionInputStateGroups(
		resource: vscode.Uri,
		token: vscode.CancellationToken,
	): Promise<vscode.ChatSessionProviderOptionGroup[]>;
	getBranchOptionItemsForRepository(
		repoUri: Uri,
		headBranchName: string | undefined,
	 ...
	 */
	updateBranchInInputState(
		state: vscode.ChatSessionInputState,
		branchName: string,
	): void;
}
export const ISessionOptionGroupBuilder =
	createServiceIdentifier<ISessionOptionGroupBuilder>(
...
export class SessionOptionGroupBuilder implements ISessionOptionGroupBuilder {
	...
	private readonly _pendingBuildGroups = new WeakMap<
		vscode.ChatSessionInputState,
		Promise<vscode.ChatSessionProviderOptionGroup[]>
	>();
	// Keeps track of the new folders selected by user
	private readonly _inputStateNewFolders = new WeakMap<
		vscode.ChatSessionInputState,
		vscode.Uri
	>();
	constructor(
		...
		@IChatFolderMruService
		private readonly copilotCLIFolderMruService: IChatFolderMruService,
	 ...
	 */
	private async getTrustedRepository(
	...
	async provideChatSessionProviderOptionGroups(
		...
		const isolationEnabled = isIsolationOptionFeatureEnabled(
			this.configurationService,
		);
		const previouslySelectedIsolationOption = previousInputState
			? getSelectedOption(previousInputState.groups, ISOLATION_OPTION_ID)
			: undefined;
		let currentIsolation: IsolationMode | undefined;
		if (isolationEnabled) {
			const lastUsed = this.context.globalState.get<IsolationMode>(
				LAST_USED_ISOLATION_OPTION_KEY,
				IsolationMode.Workspace,
			);
			currentIsolation = resolveIsolationSelection(
				lastUsed,
				previouslySelectedIsolationOption?.id,
			);
			const items = [
				{
			...
			const selectedId =
				previouslySelectedIsolationOption?.id ?? currentIsolation;
			optionGroups.push({
		...
		const isSingleFolderWorkspace =
			!isWelcomeView(this.workspaceService) &&
			!this.agentSessionsWorkspace.isAgentSessionsWorkspace &&
			folders?.length === 1;
		let defaultRepoUri =
			selectedFolderUri ??
			(isSingleFolderWorkspace ? folders![0] : undefined);
		if (isWelcomeView(this.workspaceService)) {
			const commands: vscode.Command[] = [];
			const previouslySelected = previousInputState
				? getSelectedOption(
						previousInputState.groups,
						REPOSITORY_OPTION_ID,
					)
				: undefined;
			...
			const addFolderToList = async (uri: Uri) => {
				const newFolderRepo = await this.getTrustedRepository(
					uri,
					true,
				);
			...
			const previouslySelectedUri = previouslySelected
				? vscode.Uri.file(previouslySelected.id)
				: undefined;
			...
			const newFolder = previousInputState
				? this._inputStateNewFolders.get(previousInputState)
				: undefined;
			if (newFolder) {
				await addFolderToList(newFolder);
			}
			const selectedFolderItem = selectedFolderUri
				? items.find((i) => i.id === selectedFolderUri.fsPath)
				: undefined;
			const previouslySelectedItem = previouslySelected
				? items.find((i) => i.id === previouslySelected.id)
				: undefined;
			const selectedItem =
				selectedFolderItem ?? previouslySelectedItem ?? items[0];
			if (selectedItem) {
			...
			if (repositories.length > 1) {
				const previouslySelected = previousInputState
					...
					: undefined;
				const selectedFolderRepo = selectedFolderUri
					? repositories.find(
							(repository) =>
								repository.id === selectedFolderUri.fsPath,
						)
					: undefined;
				const selectedRepository =
					...
					(previouslySelected
						? (repositories.find(
								(repository) =>
									repository.id === previouslySelected.id,
							) ?? repositories[0])
						: repositories[0]);
				defaultRepoUri = selectedRepository.id
					? vscode.Uri.file(selectedRepository.id)
					: defaultRepoUri;
				optionGroups.push({
					id: REPOSITORY_OPTION_ID,
					name: l10n.t('Folder'),
		...
		if (repo && isBranchOptionFeatureEnabled(this.configurationService)) {
			const branches = await this.getBranchOptionItemsForRepository(
				repo.rootUri,
				repo.headBranchName,
			);
			const previouslySelectedBranchItem = previousInputState
				? getSelectedOption(previousInputState.groups, BRANCH_OPTION_ID)
				: undefined;
			const branchGroup = this.buildBranchOptionGroup(
				branches,
				repo.headBranchName,
				isolationEnabled,
				currentIsolation,
				previouslySelectedBranchItem,
			);
			if (branchGroup) {
	...
	buildBranchOptionGroup(
		...
		const { locked } = resolveBranchLockState(
			isolationEnabled,
			currentIsolation,
		);
		// const locked = false;
		...
		const selectedItem = resolveBranchSelection(
			branches,
			headBranchName,
			locked ? undefined : previousSelection,
		);
		const lockedSelected =
			selectedItem && locked ? { ...selectedItem, locked } : undefined;
		return {
			id: BRANCH_OPTION_ID,
			name: l10n.t('Branch'),
			description: l10n.t('Pick Branch'),
			items: lockedSelected
				? [lockedSelected]
				: locked
					? branches.map((b) => ({ ...b, locked }))
					: branches,
	 ...
	 */
	async handleInputStateChange(
		state: vscode.ChatSessionInputState,
	): Promise<void> {
		// Persist the user's isolation choice so it's remembered across sessions
		const currentIsolation = getSelectedOption(
			state.groups,
			ISOLATION_OPTION_ID,
		)?.id as IsolationMode | undefined;
		if (currentIsolation) {
	...
	private _buildGroupsOnce(
		...
		const promise = this.provideChatSessionProviderOptionGroups(
			state,
			selectedFolderUri,
		).finally(() => {
	...
	async buildExistingSessionInputStateGroups(
		resource: vscode.Uri,
		token: vscode.CancellationToken,
	): Promise<vscode.ChatSessionProviderOptionGroup[]> {
		...
		const repositories = isWelcomeView(this.workspaceService)
		...
		const folderOrRepoId =
			folderInfo.repository?.fsPath ?? folderInfo.folder?.fsPath;
		const existingItem = folderOrRepoId
			? repositories.find((repo) => repo.id === folderOrRepoId)
			: undefined;
		...
		const branchName =
			...
			folderInfo.repositoryProperties?.branchName;
		if (branchName) {
	...
	async getBranchOptionItemsForRepository(
		repoUri: Uri,
		headBranchName: string | undefined,
	): Promise<vscode.ChatSessionProviderOptionItem[]> {
	...
	updateBranchInInputState(
		...
		const existingIdx = state.groups.findIndex(
...
export function lockInputStateGroups(
	state: vscode.ChatSessionInputState,
): void {
