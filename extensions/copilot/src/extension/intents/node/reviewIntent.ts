...
} from '../../linkify/common/linkifiedText';
import {
	...
	implements IIntentInvocation
{
	readonly linkification = {
		additionaLinkifiers: [
			{
				create: () =>
					new LineLinkifier(this.documentContext.document.uri),
			},
		],
	};
	...
	async createRenderer(
		...
		if (query === reviewLocalChangesMessage) {
			...
			const documentsAndChanges = await Promise.all<CurrentChangeInput>(
			...
			if (editor) {
				input.push({
					...
					relativeDocumentPath: path.basename(
...
class InlineReviewIntentInvocation
	extends ReviewIntentInvocation
	implements IIntentInvocation
{
	processResponse(
		...
		const replyInterpreter = this.instantiationService.createInstance(
...
class LineLinkifier implements IContributedLinkifier {
	...
	async linkify(
		...
		for (const match of parsedResponse) {
			...
			parts.push(
				new LinkifyLocationAnchor(
					this.file.with({ fragment: String(match.from + 1) }),
					newText.substring(
						match.linkOffset,
						match.linkOffset + match.linkLength,
					),
				),
			);
	...
	async processResponse(
		...
		for await (const part of inputStream) {
				...
				[
					{
						...
						relativeDocumentPath: path.basename(
							this.documentContext.document.uri.fsPath,
						),
			...
			if (comments.length > this.comments.length) {
				this.reviewService.addReviewComments(
					comments.slice(this.comments.length),
				);
		...
		outputStream.markdown(
			l10n.t(
				'Reviewed your code and generated {0} suggestions.',
				comments.length,
			),
		);
	}
