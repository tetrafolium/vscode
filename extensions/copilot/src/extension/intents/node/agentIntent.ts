...
import {
	...
	modelSupportsSimplifiedApplyPatchInstructions,
...
import {
	getRequestedToolCallIterationLimit,
...
import {
	AgentPrompt,
	AgentPromptProps,
} from '../../prompts/node/agent/agentPrompt';
import {
	BackgroundSummarizationState,
	BackgroundSummarizationThresholds,
	BackgroundSummarizer,
	IBackgroundSummarizationResult,
	shouldKickOffBackgroundSummarization,
} from '../../prompts/node/agent/backgroundSummarizer';
import {
	BackgroundTodoDecision,
	BackgroundTodoProcessor,
	IBackgroundTodoExecutionContext,
} from '../../prompts/node/agent/backgroundTodoProcessor';
import {
	AgentPromptCustomizations,
	PromptRegistry,
} from '../../prompts/node/agent/promptRegistry';
import {
	extractSummary,
...
import {
	...
	EditCodeIntentInvocationOptions,
	mergeMetadata,
	toNewChatReferences,
} from './editCodeIntent';
import { ToolCallingLoop } from './toolCallingLoop';

function isResponsesCompactionContextManagementEnabled(
	...
	return (
		...
		configurationService.getExperimentBasedConfig(
			ConfigKey.ResponsesApiContextManagementEnabled,
 ...
 */
export function isTodoToolExplicitlyEnabled(
	request: vscode.ChatRequest,
): boolean {
	const todoReferenceName = 'todo';
	return request.toolReferences.some(
		(ref) =>
 ...
 * @internal - exported for testing
 */
export function resolveSummarizeThresholdTokens(
	...
	if (value < 100) {
		throw new Error(
			`Setting github.copilot.${settingId} is too low; use a ratio in the range (0, 1] (e.g. 0.8 for 80%) or an absolute token count of at least 100.`,
		);
	}
...
export const getAgentTools = async (
	...
	if (learned) {
		// a learning-enabled (BYOK) model, we should go with what it prefers
		...
		allowTools[ToolName.ApplyPatch] =
			modelSupportsApplyPatch(model) &&
			!!toolsService.getTool(ToolName.ApplyPatch);

		if (
			allowTools[ToolName.ApplyPatch] &&
			modelCanUseApplyPatchExclusively(model)
		) {
		...
		if (
			allowTools[ToolName.ReplaceString] &&
			modelSupportsMultiReplaceString(model)
		) {
	...
	const searchSubagentEnabled = configurationService.getExperimentBasedConfig(
		ConfigKey.Advanced.SearchSubagentToolEnabled,
		experimentationService,
	);
	const exploreAgentEnabled = configurationService.getExperimentBasedConfig(
		ConfigKey.ExploreAgentEnabled,
		experimentationService,
	);
	const isGptOrAnthropic = isGptFamily(model) || isAnthropicFamily(model);
	allowTools[ToolName.SearchSubagent] =
		isGptOrAnthropic && searchSubagentEnabled && exploreAgentEnabled;
	allowTools[ToolName.ExploreSubagent] =
		isGptOrAnthropic && searchSubagentEnabled && !exploreAgentEnabled;

	const executionSubagentEnabled =
		configurationService.getExperimentBasedConfig(
			ConfigKey.Advanced.ExecutionSubagentToolEnabled,
			experimentationService,
		);
	const executionSubagentModel =
		configurationService.getExperimentBasedConfig(
	...
	if (executionSubagentModel.toLowerCase().includes('gemini-3-flash')) {
		...
		const hasGemini3Flash = allEndpoints.some((ep) =>
			ep.family.toLowerCase().includes('gemini-3-flash'),
		);
		allowTools[ToolName.ExecutionSubagent] =
			isGptOrAnthropic && executionSubagentEnabled && hasGemini3Flash;
	} else {
		allowTools[ToolName.ExecutionSubagent] =
			isGptOrAnthropic && executionSubagentEnabled;
	}

	const skillToolEnabled = configurationService.getExperimentBasedConfig(
		ConfigKey.Advanced.SkillToolEnabled,
		experimentationService,
	);
	allowTools[ToolName.Skill] = skillToolEnabled;

	const getSCMChangesEnabled = configurationService.getExperimentBasedConfig(
		ConfigKey.Advanced.GetChangedFilesToolEnabled,
		experimentationService,
	);
		...
		Iterable.some(
			...
			([t, enabled]) =>
				(typeof t === 'string' ? t : t.name) ===
					ContributedToolName.EditFilesPlaceholder &&
				enabled === false,
		)
	) {
	...
	if (
		model.family.toLowerCase().includes('gemini-3') &&
		configurationService.getExperimentBasedConfig(
			ConfigKey.Advanced.Gemini3MultiReplaceString,
			experimentationService,
		)
	) {
		allowTools[ToolName.MultiReplaceString] = true;
	...
	if (
		modelSupportsSimplifiedApplyPatchInstructions(model) &&
		configurationService.getExperimentBasedConfig(
	...
	if (
		configurationService.getExperimentBasedConfig(
			ConfigKey.Advanced.BatchReplaceStringDescriptions,
			experimentationService,
		)
	) {
		...
		if (rs !== -1) {
			tools[rs] = {
				...tools[rs],
				description: replaceStringBatchDescription,
			};
		}
		const mrs = tools.findIndex(
			(t) => t.name === ToolName.MultiReplaceString,
		);
		if (mrs !== -1) {
			tools[mrs] = {
				...tools[mrs],
				description: multiReplaceStringPrimaryDescription,
			};
		}
	}
...
export class AgentIntent extends EditCodeIntent {
	...
	private readonly _backgroundSummarizers = new Map<
		string,
		BackgroundSummarizer
	>();
	private readonly _backgroundTodoProcessors = new Map<
		string,
		BackgroundTodoProcessor
	>();
	...
	getOrCreateBackgroundSummarizer(
		sessionId: string,
		...
		if (!summarizer) {
			summarizer = new BackgroundSummarizer(
	...
	getOrCreateBackgroundTodoProcessor(
		sessionId: string,
	): BackgroundTodoProcessor {
	...
	override async handleRequest(
		...
		if (request.command === 'compact') {
			return this.handleSummarizeCommand(
				conversation,
				request,
				stream,
				token,
		...
		try {
			...
			if (
				...
				const todoProcessor = this._backgroundTodoProcessors.get(
					conversation.sessionId,
				);
				const currentTurn = conversation.getLatestTurn();
				const invocation = currentTurn.getMetadata(
					IntentInvocationMetadata,
				)?.value;
				const executionContext =
					invocation instanceof AgentIntentInvocation
						? invocation.getBackgroundTodoExecutionContext()
						: undefined;
				if (todoProcessor && executionContext) {
					todoProcessor.requestFinalReview(
						currentTurn.id,
						executionContext,
					);
	...
	private async handleSummarizeCommand(
		...
		if (history.length === 0) {
			stream.markdown(
				l10n.t('Nothing to compact. Start a conversation first.'),
			);
			return {};
		}
		...
		if (!lastRoundId) {
			stream.markdown(
				l10n.t(
					'Nothing to compact. Start a conversation with tool calls first.',
				),
			);
			return {};
		}
		...
		if (
			...
			stream.markdown(
				l10n.t(
					'Compaction is already managed by context management for this session.',
				),
			);
			return {};
		}
		...
		const availableTools = await this.instantiationService.invokeFunction(
			getAgentTools,
			request,
			endpoint,
		);
		...
		try {
			const propsBuilder = this.instantiationService.createInstance(
				SummarizedConversationHistoryPropsBuilder,
			);
			...
			const summaryMetadata = result.metadata.get(
				SummarizedConversationHistoryMetadata,
			);
			if (!summaryMetadata) {
			...
			stream.markdown(
				l10n.t('Failed to compact conversation: {0}', message),
			);
			return {};
		}
	}
...
export class AgentIntentInvocation
	extends EditCodeIntentInvocation
	implements IIntentInvocation
{
	...
	protected prompt:
		...
		| typeof NotebookInlinePrompt = AgentPrompt;
	...
	private _lastModelCapabilities:
		| {
	...
	private _backgroundTodoExecutionContext:
		| IBackgroundTodoExecutionContext
		| undefined;
	...
	constructor(
		...
		private readonly sessionTranscriptService: ISessionTranscriptService,
	) {
		super(
			intent,
			location,
	...
	public override getAvailableTools(): Promise<
	...
	override async buildPrompt(
		...
		this._resolvedCustomizations =
			await PromptRegistry.resolveAllCustomizations(
				this.instantiationService,
				this.endpoint,
			);
		...
		const summarizeThresholdOverride = this.configurationService.getConfig<
			number | undefined
		>(ConfigKey.Advanced.SummarizeAgentConversationHistoryThreshold);

		// Apply context size override if configured by the user in the model picker
		const configuredContextSize =
			this.request.modelConfiguration?.contextSize;
		const effectiveMaxTokens =
			typeof configuredContextSize === 'number' &&
			configuredContextSize < this.endpoint.modelMaxPromptTokens
		...
		const summarizeThresholdTokens = resolveSummarizeThresholdTokens(
			summarizeThresholdOverride,
			effectiveMaxTokens,
			ConfigKey.Advanced.SummarizeAgentConversationHistoryThreshold.id,
		);
		...
		const responsesCompactionContextManagementEnabled =
			isResponsesCompactionContextManagementEnabled(
				this.endpoint,
				this.configurationService,
				this.expService,
		...
		const summarizationEnabled =
			this.configurationService.getConfig(
				ConfigKey.SummarizeAgentConversationHistory,
			) &&
			this.prompt === AgentPrompt &&
			!responsesCompactionContextManagementEnabled;
		...
		const messageBudget = Math.max(
			1,
			Math.floor((baseBudget - toolTokens) * 0.9),
		);
		...
		const endpoint =
			toolTokens > 0
				? this.endpoint.cloneWithTokenOverride(safeBudget)
				: this.endpoint;

		this.logService.debug(
			`[Agent] rendering with budget=${safeBudget} (baseBudget: ${baseBudget}, toolTokens: ${toolTokens}, totalTools: ${tools?.length ?? 0}, toolSearchEnabled: ${toolSearchEnabled}), summarizationEnabled=${summarizationEnabled}`,
		);
		...
		const props: AgentPromptProps = {
			...
			promptContext: {
				...
				tools: promptContext.tools && {
					...promptContext.tools,
					toolReferences: this.stableToolReferences.filter(
						(r) => r.name !== ToolName.Codebase,
					),
				},
			},
		...
		//
		const backgroundSummarizer = summarizationEnabled
			? this._getOrCreateBackgroundSummarizer(
		...
		const lastTurnPromptTokens = findLast(
			promptContext.conversation?.turns ?? [],
			(turn) => !!turn.getMetadata(TurnTokenUsageMetadata),
		)?.getMetadata(TurnTokenUsageMetadata)?.promptTokens;
		const contextRatio =
			backgroundSummarizer && baseBudget > 0
				? Math.max(
						this._lastRenderTokenCount + toolTokens,
		...
		if (
			...
			backgroundSummarizer?.state ===
				BackgroundSummarizationState.Completed
		) {
			if (
				contextRatio >= BackgroundSummarizationThresholds.applyMinRatio
			) {
				const bgResult = backgroundSummarizer.consumeAndReset();
				if (bgResult) {
					this.logService.debug(
						`[ConversationHistorySummarizer] applying completed background summary (roundId=${bgResult.toolCallRoundId})`,
					);
					progress.report(
						new ChatResponseProgressPart2(
							l10n.t('Compacted conversation'),
							async () => l10n.t('Compacted conversation'),
						),
					);
					this._applySummaryToRounds(bgResult, promptContext);
					this._persistSummaryOnTurn(
						bgResult,
						promptContext,
						this._lastRenderTokenCount,
					);
					this._sendBackgroundCompactionTelemetry(
						'preRender',
						'applied',
						contextRatio,
						promptContext,
					...
					this.logService.warn(
						`[ConversationHistorySummarizer] background compaction state was Completed but consumeAndReset returned no result`,
					);
					this._sendBackgroundCompactionTelemetry(
						'preRender',
						'noResult',
					...
					this._recordBackgroundCompactionFailure(
						promptContext,
						'preRender',
					);
				}
				...
				this.logService.debug(
					`[ConversationHistorySummarizer] discarding completed background summary; contextRatio ${(contextRatio * 100).toFixed(0)}% below applyMinRatio ${(BackgroundSummarizationThresholds.applyMinRatio * 100).toFixed(0)}% (likely model switch or context-size override)`,
				);
				this._sendBackgroundCompactionTelemetry(
					'preRender',
					'discardedBelowApplyMinRatio',
					contextRatio,
					promptContext,
				);
			}
		...
		const renderWithoutSummarization = async (
			reason: string,
			renderProps: AgentPromptProps = props,
		): Promise<RenderPromptResult> => {
			this.logService.debug(
				`[Agent] ${reason}, rendering without summarization`,
			);
			const renderer = PromptRenderer.create(
				this.instantiationService,
				this.endpoint,
			...
			try {
				...
				if (e instanceof BudgetExceededError) {
					this.logService.error(
						e,
						`[Agent] fallback render failed due to budget exceeded`,
					);
		...
		const renderWithSummarization = async (
			reason: string,
			renderProps: AgentPromptProps = props,
		): Promise<RenderPromptResult> => {
			...
			const previousForegroundSummary = turn?.getMetadata(
				SummarizedConversationHistoryMetadata,
			);
			if (
				previousForegroundSummary?.source === 'foreground' &&
				previousForegroundSummary.outcome &&
				previousForegroundSummary.outcome !== 'success'
			) {
				this.logService.debug(
					`[ConversationHistorySummarizer] ${reason}, skipping repeated foreground summarization after prior failure (${previousForegroundSummary.outcome})`,
				);
				/* __GDPR__
				...
				*/
				this.telemetryService.sendMSFTTelemetryEvent(
					'triggerSummarizeSkipped',
					{
						previousOutcome: previousForegroundSummary.outcome,
						model: renderProps.endpoint.model,
					},
				);
				GenAiMetrics.incrementAgentSummarizationCount(
					this.otelService,
					'skipped',
				);
				return renderWithoutSummarization(
					`skipping repeated foreground summarization after prior failure (${previousForegroundSummary.outcome})`,
					renderProps,
				);
			}

			this.logService.debug(
				`[ConversationHistorySummarizer] ${reason}, triggering summarization`,
			);
			try {
				const renderer = PromptRenderer.create(
					this.instantiationService,
				...
				*/
				this.telemetryService.sendMSFTTelemetryEvent(
					'triggerSummarizeFailed',
					{ errorKind, model: renderProps.endpoint.model },
				);
				GenAiMetrics.incrementAgentSummarizationCount(
					this.otelService,
					'failed',
				...
				return renderWithoutSummarization(
					`summarization failed (${errorKind})`,
					renderProps,
				);
			}
		};
		...
		try {
			...
			if (e instanceof BudgetExceededError && summarizationEnabled) {
				...
				e.metadata.getAll(ToolResultMetadata).forEach((metadata) => {
					promptContext.toolCallResults![metadata.toolCallId] =
						metadata.result;
				});
				...
				if (
					backgroundSummarizer &&
					(backgroundSummarizer.state ===
						BackgroundSummarizationState.InProgress ||
						backgroundSummarizer.state ===
							BackgroundSummarizationState.Completed)
				) {
					...
					if (
						...
						this.logService.debug(
							`[ConversationHistorySummarizer] budget exceeded — waiting on in-progress background compaction instead of new request`,
						);
						...
						this.logService.debug(
							`[ConversationHistorySummarizer] budget exceeded — applying already-completed background compaction`,
						);
						progress.report(
							new ChatResponseProgressPart2(
								l10n.t('Compacted conversation'),
					...
					if (bgResult) {
						this.logService.debug(
							`[ConversationHistorySummarizer] background compaction applied after budget exceeded (roundId=${bgResult.toolCallRoundId})`,
						);
						this._applySummaryToRounds(bgResult, promptContext);
						this._persistSummaryOnTurn(
							bgResult,
						...
						didSummarizeThisIteration = true;
						try {
							const reRenderer = PromptRenderer.create(
								this.instantiationService,
								endpoint,
								this.prompt,
								{ ...props, promptContext },
							...
							if (reRenderError instanceof BudgetExceededError) {
								this.logService.debug(
									`[ConversationHistorySummarizer] re-render after background compaction still exceeded budget — falling back`,
								);
								this._sendBackgroundCompactionTelemetry(
									budgetExceededTrigger,
									'appliedButReRenderFailed',
									contextRatio,
									promptContext,
								);
								result = await renderWithoutSummarization(
									'budget exceeded after background compaction applied',
									{ ...props, promptContext },
								);
							} else {
								throw reRenderError;
						...
						this.logService.debug(
							`[ConversationHistorySummarizer] background compaction produced no usable result after budget exceeded — falling back to synchronous summarization`,
						);
						this._sendBackgroundCompactionTelemetry(
							budgetExceededTrigger,
							'noResult',
						...
						result = await renderWithSummarization(
							`budget exceeded(${e.message}), background compaction failed`,
						);
						didSummarizeThisIteration = true;
					}
				} else {
		...
		const summaryMeta = result.metadata.get(
			SummarizedConversationHistoryMetadata,
		);
		if (summaryMeta) {
			const turn = promptContext.conversation?.getLatestTurn();
		...
		if (
			summarizationEnabled &&
			backgroundSummarizer &&
			!didSummarizeThisIteration
		) {
			const localPostRender = result.tokenCount + toolTokens;
			const effectivePostRender = Math.max(
				localPostRender,
				lastTurnPromptTokens ?? 0,
			);
			const postRenderRatio =
				baseBudget > 0 ? effectivePostRender / baseBudget : 0;

			const idleOrFailed =
				backgroundSummarizer.state ===
					BackgroundSummarizationState.Idle ||
				backgroundSummarizer.state ===
					BackgroundSummarizationState.Failed;

			const cacheWarm = (promptContext.toolCallRounds?.length ?? 0) > 0;

			const kickOff = shouldKickOffBackgroundSummarization(
				postRenderRatio,
				cacheWarm,
				this._thresholdRng,
			);
			...
			if (kickOff && idleOrFailed) {
				...
				const strippedMessages =
					ToolCallingLoop.stripInternalToolCallIds(result.messages);
				const rawEffort =
					this.request.modelConfiguration?.reasoningEffort;
				...
				const shouldDisableThinking =
					!!promptContext.isContinuation &&
					isAnthropicFamily(this.endpoint) &&
					!ToolCallingLoop.messagesContainThinking(strippedMessages);
				this._lastModelCapabilities = {
					enableThinking: !shouldDisableThinking,
					reasoningEffort:
						typeof rawEffort === 'string' ? rawEffort : undefined,
					enableToolSearch:
						!isSubagent && !!this.endpoint.supportsToolSearch,
				...
				this._startBackgroundSummarization(
					backgroundSummarizer,
					result.messages,
					promptContext,
					props,
		...
		if (lastMessage?.role === Raw.ChatRole.User) {
			...
			if (
				currentTurn &&
				!currentTurn.getMetadata(RenderedUserMessageMetadata)
			) {
				currentTurn.setMetadata(
					new RenderedUserMessageMetadata(lastMessage.content),
				);
			}
		}
		...
		if (!errorDetails.responseIsFiltered) {
			errorDetails.confirmationButtons = [
				...(errorDetails.confirmationButtons ?? []),
				{
					...
					} satisfies IContinueOnErrorConfirmation,
	...
	getAdditionalVariables(
		promptContext: IBuildPromptContext,
	): ChatVariablesCollection | undefined {
	...
	private _startBackgroundSummarization(
		...
		this.logService.debug(
			`[ConversationHistorySummarizer] context at ${(contextRatio * 100).toFixed(0)}% — starting background compaction`,
		);
		...
		const normalizedTools = availableTools?.length
			? normalizeToolSchema(
					...
					(tool, rule) => {
						this.logService.warn(
							`[ConversationHistorySummarizer] Tool ${tool} failed validation: ${rule}`,
						);
					},
				)
			: undefined;
		...
		const associatedRequestId =
			promptContext.conversation?.getLatestTurn()?.id;
		...
		backgroundSummarizer.start(async (bgToken) => {
			try {
				...
				if (response.type !== ChatFetchResponseType.Success) {
					throw new Error(
						`Background summarization request failed: ${response.type}`,
					);
				}
				const rawSummaryText = extractSummary(response.value);
				if (rawSummaryText === undefined) {
					throw new Error(
						'Background summarization: no <summary> tags found in response',
					);
				}
				if (!toolCallRoundId) {
					throw new Error(
						'Background summarization: no round ID to apply summary to',
					);
				}
				...
				if (
					conversationId &&
					this.sessionTranscriptService.getTranscriptPath(
						conversationId,
					)
				) {
				...
				const summaryText = conversationId
					? appendTranscriptHintToSummary(
							rawSummaryText,
							conversationId,
							this.sessionTranscriptService,
						)
					: rawSummaryText;
				this.logService.debug(
					`[ConversationHistorySummarizer] background compaction completed (${summaryText.length} chars, roundId=${toolCallRoundId})`,
				);

				// Send summarizedConversationHistory telemetry for parity
				// with the standard ConversationHistorySummarizer path.
				const { numRounds, numRoundsSinceLastSummarization } =
					computeSummarizationRoundCounts(history, rounds);
				const numRoundsInCurrentTurn = rounds.length;
				const lastUsedTool =
					rounds.at(-1)?.toolCalls?.at(-1)?.name ??
					history.at(-1)?.rounds.at(-1)?.toolCalls?.at(-1)?.name ??
					'none';
				const promptTypes = messages
					.map(
						(msg) =>
							`${msg.role}${'name' in msg && msg.name ? `-${msg.name}` : ''}:${getTextPart(msg.content).length}`,
					)
					.join(',');
				/* __GDPR__
					"summarizedConversationHistory" : {
				...
				*/
				...
				this.logService.error(
					err,
					`[ConversationHistorySummarizer] background compaction failed`,
				);
	 ...
	 */
	private _getOrCreateBackgroundSummarizer(
		sessionId: string | undefined,
	): BackgroundSummarizer | undefined {
		...
		return this.intent.getOrCreateBackgroundSummarizer(
			sessionId,
			this.endpoint.modelMaxPromptTokens,
			endpointId,
		);
	 ...
	 */
	private _applySummaryToRounds(
		bgResult: { summary: string; toolCallRoundId: string },
		promptContext: IBuildPromptContext,
	): void {
		// Check current-turn rounds first
		const currentRound = promptContext.toolCallRounds?.find(
			(r) => r.id === bgResult.toolCallRoundId,
		);
		if (currentRound) {
			...
			if (!found) {
				this.logService.warn(
					`[ConversationHistorySummarizer] background compaction round ${bgResult.toolCallRoundId} not found in toolCallRounds or history — summary dropped`,
				);
			}
		}
	 ...
	 */
	private _persistSummaryOnTurn(
		bgResult: IBackgroundSummarizationResult,
		promptContext: IBuildPromptContext,
		...
		if (chatResult) {
			const metadata = (chatResult.metadata ?? {}) as Record<
				string,
				unknown
			>;
			const existingSummaries =
				(metadata['summaries'] as unknown[]) ?? [];
			existingSummaries.push({
				toolCallRoundId: bgResult.toolCallRoundId,
				text: bgResult.summary,
			});
		...
		const usage =
			bgResult.promptTokens !== undefined &&
			bgResult.outputTokens !== undefined
				? {
						prompt_tokens: bgResult.promptTokens,
						completion_tokens: bgResult.outputTokens,
						total_tokens:
							bgResult.promptTokens + bgResult.outputTokens,
						...(bgResult.promptCacheTokens !== undefined
							? {
									prompt_tokens_details: {
										cached_tokens:
											bgResult.promptCacheTokens,
									},
								}
							: {}),
					}
		...
		turn?.setMetadata(
			new SummarizedConversationHistoryMetadata(
				...
				bgResult.summary,
				{
					...
					numRoundsSinceLastSummarization:
						bgResult.numRoundsSinceLastSummarization,
	 ...
	 */
	private _recordBackgroundCompactionFailure(
		promptContext: IBuildPromptContext,
		trigger: string,
	): void {
		const turn = promptContext.conversation?.getLatestTurn();
	 ...
	 */
	private _getOrCreateBackgroundTodoProcessor(
		sessionId: string | undefined,
	): BackgroundTodoProcessor | undefined {
		if (!sessionId || !(this.intent instanceof AgentIntent)) {
			return undefined;
		}
	...
	getBackgroundTodoExecutionContext():
		| IBackgroundTodoExecutionContext
		| undefined {
	...
	private _maybeStartBackgroundTodoPass(
		...
		const { decision, reason, delta } = processor.shouldRun({
			backgroundTodoAgentEnabled: isBackgroundTodoAgentEnabled(
				this.configurationService,
				this.expService,
				this.request,
			),
			todoToolExplicitlyEnabled: isTodoToolExplicitlyEnabled(
				this.request,
			),
		...
		this.logService.debug(
			`[BackgroundTodo] policy decision: ${decision} (${reason})`,
		);
