...
} from './summarizedDocumentWithSelection';
...
export class TestFromTestInvocation implements IIntentInvocation {
	...
	async buildPrompt(
		...
		this.replyInterpreter =
			result.metadata.get(ReplyInterpreterMetaData)?.replyInterpreter ??
			null;
	...
	override async render(_state: void, sizing: PromptSizing) {
		...
		if (isNotebookCellOrNotebookChatInput(context.document.uri)) {
			throw illegalArgument(
				'TestFromTestPrompt should not be used for notebooks',
			);
		}

		const requestAndUserQuery =
			...
			<>
				...
				>
					<InstructionMessage priority={1000}>
						The user has a {context.language.languageId} file opened
						in a code editor.
						<br />
						The user includes some code snippets from the file.
						<br />
						Answer with a single {context.language.languageId} code
						block.
						<br />
						Your expertise is strictly limited to software
						development topics.
						<br />
					</InstructionMessage>
				</HistoryWithInstructions>
