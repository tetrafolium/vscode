...
import {
	ISetupTestExtension,
	testExtensionsForLanguage,
} from '../../../../platform/testing/common/setupTestExtensions';
...
} from '../../../prompt/common/fileTreeParser';
import { IBuildPromptContext } from '../../../prompt/common/intents';
import {
	IToken,
	StreamingGrammar,
} from '../../../prompt/common/streamingGrammar';
import {
	IIntent,
	IIntentInvocation,
...
export class SetupTestsInvocation implements IIntentInvocation {
	...
	constructor(
		...
		private readonly workspaceMutationManager: IWorkspaceMutationManager,
		@IExtensionsService
	...
	async processResponse(
			...
			for (const token of tokens) {
				if (token.transitionTo === State.FileTree) {
					...
					outputStream.markdown(
						grammar.accumulate(0, grammar.tokens.length - 1),
					);
				} else if (token.transitionTo === State.FoundTree) {
					const tree = grammar.accumulate(
						undefined,
						undefined,
						State.FileTree,
					);
					...
					token.transitionTo === undefined &&
					token.state !== State.FileTree &&
					grammar.visited(State.FileTree)
				) {
		...
		const command =
			this.setupConfirmation?.command || this.delegatedSetup?.command;
		if (command) {
			...
			await this.recommendExtension(
				grammar.accumulate(undefined, undefined, State.LookingForTree),
				outputStream,
				token,
			);
		} else {
	...
	private async doFrameworkQuery(
		context: IResponseProcessorContext,
		outputStream: vscode.ChatResponseStream,
		token: CancellationToken,
	) {
		const invocation = this.instantiationService.createInstance(
			SetupTestsFrameworkQueryInvocationRaw,
			this.endpoint,
			undefined,
		);
		const prompt = await invocation.buildPrompt(
			this.buildPromptContext,
			undefined,
			token,
		);
		const inputStream = new FetchStreamSource();
		const responseProcessing = invocation.processResponse(
			context,
			inputStream.stream,
			outputStream,
			token,
	...
	private async getSetupConfirmation() {
		...
		const extension = extensionInfo
			? this.extensionsService.getExtension(extensionInfo.id)
			: undefined;
		const command = (extension?.packageJSON as IPackageJson)?.copilot?.tests
	...
	private recommendedExtension = new Lazy(() =>
		getKnownExtensionInText(this.prompt),
	);

	private async recommendExtension(
		outputText: string,
		outputStream: vscode.ChatResponseStream,
		token: CancellationToken,
	) {
		...
		if (this.recommendedExtension.value) {
			...
			searchText = await this.deriveFrameworkFromResponse(
				outputText,
				token,
			);
		...
		if (extensionInfo) {
			outputStream.markdown(
				l10n.t(
					'I also recommend installing the {0} extension to make tests easy to run in VS Code:',
					extensionInfo.name,
				),
			);
			outputStream.markdown('\n\n');
			outputStream.push(
				new ChatResponseExtensionsPart([extensionInfo.id]),
			);
		} else {
			outputStream.markdown(
				l10n.t(
	...
	private async deriveFrameworkFromResponse(
		outputText: string,
		token: CancellationToken,
	) {
		const deriveResponsePrompt = await PromptRenderer.create(
			this.instantiationService,
			this.endpoint,
			TestFrameworkFromResponsePrompt,
			{
				query: outputText,
			},
		).render();
	...
	private handleFileTree(
		...
		const workspaceFolder = this.workspaceService
			.getWorkspaceFolders()
			.at(0);
		if (!workspaceFolder) {
			return;
		...
		return (
			<>
				<SystemMessage priority={1000}>
					You are a software engineer with expert knowledge around
					...
					# Additional Rules
					<br />
					1. The user will tell you what testing framework they want
					to set up, and provide you their workspace structure.
					<br />
					2. Determine how to test up the desired testing framework.
					<br />
					3. Generate a markdown file tree structure listing files you
					want to create or edit in order to set up the testing
					framework. The tree MUST NOT include files that don't need
					to be modified.
					<br />
					4. Make sure to include a basic "hello world" test to help
					the user get started. If you see existing test files in the
					workspace, make sure to try to match their naming
					convention.
					<br />
					5. Do not attempt to modify the file content yourself and
					simply respond with the file tree structure.
					<br />
					6. After listing the file tree structure, respond with any
					terminal commands the user should execute to finish
					installing the testing framework. Terminal commands should
					be wrapped in a code fence tagged with the "sh" language.
					<br />
					7. Finally, provide a command line a user can execute to run
					their tests.
					<br />
					{setupConfirmation && (
						<>
							8. At the end, include a phrase that conveys '
							{setupConfirmation.message}', but rephrase this to
							indicate that this is the last step the user needs
							to take to enable rich UI integration in VS Code.
							{setupConfirmation.command &&
								` This message will be followed by a button that says "${setupConfirmation.command.title}".`}
							<br />
						</>
					)}
					<br />
					...
					<br />
					I am working in a workspace that has the following
					structure:
					<br />
					{`\`\`\`
src/
...
\`\`\``}
					...
					## Response:
					<br />
					Let's create a `.mocharc.js` file to configure your test
					settings, as well as a "hello world" test:
					<br />
					<br />
					{`\`\`\`filetree
${projectNameToken}
...
class TestFrameworkFromResponsePrompt extends PromptElement<
	{ query: string } & BasePromptElementProps
> {
		...
		return (
			<>
				<InstructionMessage priority={1000}>
					# Rules:
					<br />
					1. The user will give you instructions they were told
					regarding how to set up a testing framework.
					<br />
					2. Your job is to print the name of the testing framework
					referred to in the response.
					<br />
					3. Do not print any other information except for the name of
					the framework.
					<br />
					<br />
					# Example
					...
					## Question:
					<br />
					Given the structure of your workspace, I recommend using
					Mocha for testing. To set up Mocha, you should create a
					`.mocharc.js` file to configure your test settings, as well
					as a "hello world" test.
					<br />
					## Response:
					<br />
					mocha
...
function getKnownExtensionInText(text: string) {
	...
	return mapFindFirst(testExtensionsForLanguage.values(), (ext) => {
		if (
			ext.forLanguage?.associatedFrameworks?.some((f) =>
				haystack.includes(f),
			)
		) {
		...
		return (
			ext.perFramework &&
			mapFindFirst(ext.perFramework, ([f, ext]) =>
				haystack.includes(f) ? ext : undefined,
			)
		);
	});
}
