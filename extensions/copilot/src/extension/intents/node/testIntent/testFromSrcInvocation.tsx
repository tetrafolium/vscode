...
import {
	IParserService,
	treeSitterOffsetRangeToVSCodeRange as toRange,
	vscodeToTreeSitterOffsetRange as toTSOffsetRange,
...
import {
	isTestFile,
	suggestUntitledTestFileLocation,
	TestFileFinder,
} from '../../../prompt/node/testFiles';
...
import {
	createPromptingSummarizedDocument,
	InlineReplyInterpreter,
} from '../../../prompts/node/inline/promptingSummarizedDocument';
import {
	ISummarizedDocumentSettings,
	ProjectedDocument,
	RemovableNode,
} from '../../../prompts/node/inline/summarizedDocument/summarizeDocument';
...
export class TestFromSourceInvocation implements IIntentInvocation {
	...
	private _additionalResponseParts:
		| vscode.ExtendedChatResponsePart[]
		| undefined;
	...
	constructor(
		...
		private readonly alreadyConsumedChatVariable:
		...
		this._testFileFinder =
			this.instantiationService.createInstance(TestFileFinder);
	}
	...
	async buildPrompt(
		...
		assert(
			!isTestFile(this.documentContext.document),
			'TestFromSourceInvocation should not be invoked from a test file',
		);
		...
		if (
			testExampleFile !== null &&
			testExampleFile.kind === 'candidateTestFile'
		) {
			...
			const testFileUri = suggestUntitledTestFileLocation(
		...
		if (this._testFileToWriteTo.kind === 'new') {
			...
			const testFile =
				await this.workspaceService.openTextDocument(testFileUri);

			const testFileAST = this._parserService.getTreeSitterAST(testFile);

			const lastTest = testFileAST
				? await testFileAST.findLastTest()
				: null;
		...
		progress.report(
			new ChatResponseMovePart(this._testFileToWriteTo.uri, range) as any,
		); // FIXME@ulugbekna

		if (this.location === ChatLocation.Panel && !promptContext.query) {
			promptContext = {
				...promptContext,
				query: 'Write a set of detailed unit test functions for the code above.',
			};
		}
	...
	async processResponse(
		...
		if (testFileKind === undefined || testFileUri === undefined) {
			assertType(
				additionalParts,
				'Expected to have a textual response without a test file',
			);
		} else if (testFileKind === 'new') {
			const range = new Range(0, 0, 0, 0);

			const projectedDoc = new ProjectedDocument(
				'',
				StringEdit.empty,
				doc.languageId,
			);
			...
			const replyInterpreter = new InlineReplyInterpreter(
				...
				(lineFilter, streamingWorkingCopyDocument) =>
			...
			await replyInterpreter.processResponse(
				context,
				inputStream,
				outputStream,
				token,
			...
			const splitDoc =
				summarizedDocument.splitAroundOriginalSelectionEnd();
	 ...
	 */
	private async findTestFileForSourceFile(
		token: CancellationToken,
	): Promise<TestExampleFile | null> {
		let kind: 'anyTestFile' | 'candidateTestFile';

		let testExampleFile =
			await this._testFileFinder.findTestFileForSourceFile(
				this.documentContext.document,
				token,
			);
		...
		if (testExampleFile !== undefined) {
			...
			const anyTestFile =
				await this._testFileFinder.findAnyTestFileForSourceFile(
					this.documentContext.document,
					token,
				);
		...
		if (
			testExampleFile === undefined ||
			(await this.ignoreService.isCopilotIgnored(testExampleFile))
		) {
			return null;
		}
...
class Prompt extends PromptElement<Props> {
	...
	override async render(state: void, sizing: PromptSizing) {
		...
		const treeSitterAST = this.parserService.getTreeSitterAST(
			context.document,
		);
		...
		if (treeSitterAST !== undefined) {
			const node = await treeSitterAST.getNodeToDocument(
				toTSOffsetRange(context.selection, context.document),
			);
		...
		const documentSummarizationSettings:
			...
			['typescript', 'typescriptreact'].includes(
				context.document.languageId,
			) &&
			nodeKind !== undefined &&
			['class_declaration', 'method_definition'].includes(nodeKind)
				? {
						costFnOverride: (
							node: RemovableNode,
							currentScore: number,
		...
		const extraContext = await this.computeLangSpecificExtraGuidelines(
			context,
			testExampleFile,
		);
		...
		const srcFilePath =
			relativeToWorkspace(
				...
				context.document.uri.path,
			) ?? path.basename(context.document.uri.path);

		const filteredChatVariables =
			alreadyConsumedChatVariable === undefined
				? chatVariables
				: chatVariables.filter(
						(v) => v.reference !== alreadyConsumedChatVariable,
					);
		...
		return (
			<>
					...
					<InstructionMessage priority={1000}>
						{
							location === ChatLocation.Editor ? (
								<>
									The user has a {languageId} file opened in a
									code editor.
									<br />
									The user includes some code snippets from
									the file.
									<br />
									Answer with a single {languageId} code
									block.
								</>
							) : location === ChatLocation.Panel ? (
								<>
									First think step-by-step - describe your
									plan for what to build in pseudocode,
									...
									<br />
									Make sure to include the programming
									language name at the start of the Markdown
									code blocks.
									<br />
									Avoid wrapping the whole response in triple
									backticks.
									<br />
									The user works in an IDE called Visual
									Studio Code which has a concept for editors
									with open files, integrated unit test
									support, an output pane that shows the
									output of running the code as well as an
									integrated terminal.
									<br />
									The active document is the source code the
				...
				<UserMessage>
					...
					<CustomInstructions
						chatVariables={filteredChatVariables}
						languageId={context.language.languageId}
						includeTestGenerationInstructions={true}
						priority={725}
					/>
					...
					/>
					{testExampleFile !== null && (
						<TestExample priority={750} {...testExampleFile} />
					)}
					<Tag name="currentFile" priority={900}>
						Here is the current file at `{srcFilePath}`:
						<br />
						<br />
						<CodeBlock
	...
	private async computeLangSpecificExtraGuidelines(
		context: IDocumentContext,
		testExampleFile: TestExampleFile | null,
	): Promise<string> {
		const extraContext = [];
		...
		if (context.document.languageId === 'python') {
			const usingExistingTestFile =
				testExampleFile !== null &&
				testExampleFile.kind === 'candidateTestFile';

			if (!usingExistingTestFile) {
				extraContext.push(
					'Make sure your answer imports the function to test as this is a total new file.',
				);

				// this will be true if there is not a candidate test file so goal is creating a new test file which will require imports
				const parent: string = path.dirname(
					context.document.uri.fsPath,
				);
				const init_search: string = path.join(parent, '__init__.py');
				const workspaceRootPath: URI | undefined =
					this.workspaceService.getWorkspaceFolder(
						context.document.uri,
					);
				try {
					await this.workspaceService.openTextDocument(
						Uri.file(init_search),
					);
					if (
						workspaceRootPath !== undefined &&
						path.resolve(parent) ===
							path.resolve(workspaceRootPath?.fsPath ?? '')
					) {
						/* current file is at the root of the workspace */
						extraContext.push(
							'The file is in the root of the workspace, which has an __init__.py but use an absolute import to import the function to test.',
						);
					} else {
						extraContext.push(
							'The parent directory of the given file has an __init__.py file making it a regular package. Use a relative import to import the function to test.',
						);
					}
				} catch (error) {
					extraContext.push(
						'The parent directory of the given file has no __init__.py file making it a namespace package. Use an absolute import to import the function to test.',
					);
				}
			}
		}
