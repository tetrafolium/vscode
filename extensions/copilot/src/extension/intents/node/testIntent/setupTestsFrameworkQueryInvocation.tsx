...
export class SetupTestsFrameworkQueryInvocationRaw {
	...
	public async processResponse(
		...
		const frameworks = grammar
			.accumulate(undefined, undefined, State.Frameworks)
			.split('\n')
		...
		if (frameworks.length) {
			...
			outputStream.markdown(
				l10n.t("Use chat to tell me which framework you'd prefer."),
			);
		}
		...
		return (
			<>
				<SystemMessage priority={1000}>
					...
					<br />
					2. Determine the best testing frameworks that should be used
					for the project.
					<br />
					3. Give a brief explanation why a user would choose one
					framework over the other, but be concise and never give the
					user steps to set up the framework.
					<br />
					4. If you're unsure which specific framework is best, you
					can suggest multiple frameworks.
					<br />
					5. Suggest only frameworks that are used to run tests. Do
					not suggest things like assertion libraries or build tools.
					<br />
					6. After determining the best framework to use, write out
					the name of 1 to 3 suggested frameworks prefixed by the
					phrase "{frameworkPrefix}", for example: "{frameworkPrefix}
					vitest".
					<br />
					<br />
					DO NOT mention that you cannot read files in the workspace.
					<br />
					DO NOT ask the user to provide additional information about
...
\`\`\``}
					...
					## Response:
					<br />
					Because you have a `vite.config.ts` file, it looks like
					you're working on a browser or Node.js application. If
					you're working on a browser application, I recommend using
					Playwright. Otherwise, Vitest is a good choice for Node.js.
					<br />
					{frameworkPrefix}playwright
					<br />
					{frameworkPrefix}vitest
					<br />
				</SystemMessage>
				{this.props.document && (
					<PreferredExtensions document={this.props.document} />
				)}
...
class PreferredExtensions extends PromptElement<
	{ document: TextDocumentSnapshot } & BasePromptElementProps
> {
	override render(): PromptPiece | undefined {
		const extensions = testExtensionsForLanguage.get(
		...
		return (
			<SystemMessage priority={600}>
				These are the preferred test frameworks for{' '}
				{this.props.document.languageId}:<br />
				<br />
				{[...extensions.perFramework.keys()]
					.map((f) => `- ${f}`)
					.join('\n')}
				<br />
			</SystemMessage>
