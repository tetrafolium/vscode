...
describe('AgentIntent /summarize command', () => {
	...
	function createConversationWithHistory(): Conversation {
		...
		const previousTurn = new Turn('turn1', {
			type: 'user',
			message: 'Create a file for me',
		});
		...
		previousTurn.setResponse(
			TurnStatus.Success,
			{ type: 'model', message: 'Done!' },
			'responseId1',
			previousTurnResult,
		);

		// Create the current turn (the /summarize command turn)
		const currentTurn = new Turn('turn2', {
			type: 'user',
			message: '/summarize',
		});
	...
	function createConversationWithNoHistory(): Conversation {
		...
		const currentTurn = new Turn('turn1', {
			type: 'user',
			message: '/summarize',
		});
	...
	test('returns summary metadata when enabled and history exists', async () => {
		...
		expect(result.metadata?.summary?.toolCallRoundId).toBe(
			'toolCallRoundId1',
		);
	...
	test('returns friendly message when no history exists', async () => {
		...
		expect(
			stream.output.some((msg) =>
				msg.toLowerCase().includes('nothing to compact'),
			),
		).toBe(true);
	});
	...
	test('cancels any pending background summarizer for the session', async () => {
		...
		const before = intent.getOrCreateBackgroundSummarizer(
			'sessionId',
			100_000,
			'test-model',
		);
		...
		// next render.
		const after = intent.getOrCreateBackgroundSummarizer(
			'sessionId',
			100_000,
			'test-model',
		);
	...
	test('endpoint switch on the same session cancels the previous summarizer', () => {
		...
		const onModelA = intent.getOrCreateBackgroundSummarizer(
			'sessionId',
			100_000,
			'model-a',
		);
		...
		const onModelB = intent.getOrCreateBackgroundSummarizer(
			'sessionId',
			200_000,
			'model-b',
		);
		...
		const onModelAAgain = intent.getOrCreateBackgroundSummarizer(
			'sessionId',
			100_000,
			'model-a',
		);
	...
	test('same endpoint returns the same summarizer instance', () => {
		...
		const first = intent.getOrCreateBackgroundSummarizer(
			'sessionId',
			100_000,
			'model-a',
		);
		const second = intent.getOrCreateBackgroundSummarizer(
			'sessionId',
			100_000,
			'model-a',
		);
