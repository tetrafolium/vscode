...
import {
	isOpenAIContextManagementResponse,
	OpenAiFunctionDef,
...
import {
	cancelText,
	IToolCallIterationIncrease,
} from '../../prompt/common/specialRequestTypes';
import {
	ThinkingDataItem,
	ToolCallRound,
} from '../../prompt/common/toolCallRound';
import {
	IBuildPromptResult,
	IResponseProcessor,
...
export type ToolCallingLoopFetchOptions = Required<
	Pick<
		IMakeChatRequestOptions,
		| 'messages'
		| 'finishedCb'
...
> &
	Pick<
		IMakeChatRequestOptions,
		'modelCapabilities' | 'summarizedAtRoundId' | 'topLevelTurnId'
	> & { iterationNumber: number; isKeepAliveProbe?: boolean };
 ...
 * programmatic use.
 */
export abstract class ToolCallingLoop<
	TOptions extends IToolCallingLoopOptions = IToolCallingLoopOptions,
> extends Disposable {
	...
	private readonly _onDidBuildPrompt = this._register(
		new Emitter<{
			result: IBuildPromptResult;
			tools: LanguageModelToolInformation[];
	...
	private readonly _onDidReceiveResponse = this._register(
		new Emitter<IToolCallingResponseEvent>(),
	);
	...
	protected get agentName(): string | undefined {
		return (
			(this.options.request as { subAgentName?: string }).subAgentName ??
	...
	constructor(
		...
		private readonly _authenticationChatUpgradeService: IAuthenticationChatUpgradeService,
		@ITelemetryService
		protected readonly _telemetryService: ITelemetryService,
		@IConfigurationService
		protected readonly _configurationService: IConfigurationService,
		@IExperimentationService
		protected readonly _experimentationService: IExperimentationService,
		@IChatHookService private readonly _chatHookService: IChatHookService,
		@ISessionTranscriptService
		protected readonly _sessionTranscriptService: ISessionTranscriptService,
		@IFileSystemService
	...
	protected createPromptContext(
		...
		if (this.stopHookReason) {
			...
			this._logService.info(
				`[ToolCallingLoop] Using stop hook reason as query: ${query}`,
			);
		...
		const history = this.options.conversation.turns
			.slice(0, -1)
			.filter(
		...
		return {
			...
			chatVariables,
			tools: {
				toolReferences: request.toolReferences.map(
	 ...
	 */
	protected async executeStopHook(
		input: StopHookInput,
		sessionId: string,
		outputStream: ChatResponseStream | undefined,
		token: CancellationToken,
		...
		try {
			const results = await this._chatHookService.executeHook(
				'Stop',
				this.options.request.hooks,
				input,
				sessionId,
				token,
			);
			...
			processHookResults({
				...
				onSuccess: (output) => {
					if (typeof output === 'object' && output !== null) {
						...
						this._logService.trace(
							`[ToolCallingLoop] Checking hook output: decision=${specific?.decision}, reason=${specific?.reason}`,
						);
						if (specific?.decision === 'block' && specific.reason) {
							this._logService.trace(
								`[ToolCallingLoop] Stop hook blocked: ${specific.reason}`,
							);
				...
				onError: (errorMessage) => {
					if (errorMessage) {
						this._logService.trace(
							`[ToolCallingLoop] Stop hook error collected as blocking reason: ${errorMessage}`,
						);
			...
			this._logService.error(
				'[ToolCallingLoop] Error executing Stop hook',
				error,
			);
			return { shouldContinue: false };
		}
	 ...
	 */
	protected showStopHookBlockedMessage(
		outputStream: ChatResponseStream | undefined,
		...
		this._logService.trace(
			`[ToolCallingLoop] Stop hook blocked stopping: ${reasons.join('; ')}`,
		);
	}
	 ...
	 */
	protected shouldAutopilotContinue(
		result: IToolCallSingleResult,
	): string | undefined {
		if (this.taskCompleted) {
			this._logService.info(
				'[ToolCallingLoop] Autopilot: task_complete was called, stopping',
			);
			return undefined;
		}

		// might have called task_complete alongside other tools in an earlier round
		const calledTaskComplete = this.toolCallRounds.some((round) =>
		...
		if (calledTaskComplete) {
			...
			this._logService.info(
				'[ToolCallingLoop] Autopilot: task_complete found in history, stopping',
			);
			return undefined;
		}
		...
		if (
			result.round.toolCalls.length === 0 &&
			result.round.response.trim().length > 0
		) {
			this._logService.info(
				'[ToolCallingLoop] Autopilot: model produced a text-only response, treating as done',
			);
			return undefined;
		}
		...
		if (
			...
			ToolCallingLoop.MAX_AUTOPILOT_ITERATIONS
		) {
			this._logService.info(
				`[ToolCallingLoop] Autopilot: hit max iterations (${ToolCallingLoop.MAX_AUTOPILOT_ITERATIONS}), letting it stop`,
			);
			return undefined;
		}
		...
		// loop stop.
		if (
			this.autopilotStopHookActive &&
			result.round.toolCalls.length === 0
		) {
			this._logService.info(
				'[ToolCallingLoop] Autopilot: prior nudge produced no tool calls, stopping to avoid wasted requests',
			);
			return undefined;
		}
	 ...
	 */
	protected ensureAutopilotTools(
		availableTools: LanguageModelToolInformation[],
	): LanguageModelToolInformation[] {
		...
		if (
			availableTools.some(
				(t) => t.name === ToolCallingLoop.TASK_COMPLETE_TOOL_NAME,
			)
		) {
			return availableTools;
		}
		const taskCompleteTool = this._instantiationService.invokeFunction(
			(accessor) =>
				accessor
					.get(IToolsService)
					.getTool(ToolCallingLoop.TASK_COMPLETE_TOOL_NAME),
		);
		if (taskCompleteTool) {
			this._logService.info(
				'[ToolCallingLoop] Added task_complete tool for autopilot mode',
			);
			return [...availableTools, taskCompleteTool];
		}
		this._logService.warn(
			'[ToolCallingLoop] task_complete tool not found — autopilot completion may not work',
		);
		return availableTools;
	}
	 ...
	 */
	protected async executeSessionStartHook(
		...
		try {
				...
				onSuccess: (output) => {
					if (typeof output === 'object' && output !== null) {
						...
						if (additionalContext) {
							...
							this._logService.trace(
								`[ToolCallingLoop] SessionStart hook provided context: ${additionalContext.substring(0, 100)}...`,
							);
						}
					}
				},
			...
			this._logService.error(
				'[ToolCallingLoop] Error executing SessionStart hook',
				error,
			);
			return {};
		}
	 ...
	 * @returns Result containing additional context from hooks
	 */
	protected async executeSubagentStartHook(
		...
		try {
				...
				onSuccess: (output) => {
					if (typeof output === 'object' && output !== null) {
						...
						if (additionalContext) {
							additionalContexts.push(additionalContext);
							this._logService.trace(
								`[ToolCallingLoop] SubagentStart hook provided context: ${additionalContext.substring(0, 100)}...`,
							);
						}
					}
				},
			...
			this._logService.error(
				'[ToolCallingLoop] Error executing SubagentStart hook',
				error,
			);
			return {};
		}
	 ...
	 * @returns Result indicating whether to continue and the reasons
	 */
	protected async executeSubagentStopHook(
		...
		try {
			...
			processHookResults({
				...
				onSuccess: (output) => {
					if (typeof output === 'object' && output !== null) {
						...
						const specific = hookOutput.hookSpecificOutput;
						this._logService.trace(
							`[ToolCallingLoop] Checking SubagentStop hook output: decision=${specific?.decision}, reason=${specific?.reason}`,
						);
						if (specific?.decision === 'block' && specific.reason) {
							this._logService.trace(
								`[ToolCallingLoop] SubagentStop hook blocked: ${specific.reason}`,
							);
							blockingReasons.add(specific.reason);
						}
					}
				...
				onError: (errorMessage) => {
					if (errorMessage) {
						this._logService.trace(
							`[ToolCallingLoop] SubagentStop hook error collected as blocking reason: ${errorMessage}`,
						);
						blockingReasons.add(errorMessage);
					}
				},
			...
			this._logService.error(
				'[ToolCallingLoop] Error executing SubagentStop hook',
				error,
			);
			return { shouldContinue: false };
		}
	 ...
	 */
	protected showSubagentStopHookBlockedMessage(
		outputStream: ChatResponseStream | undefined,
		reasons: readonly string[],
	): void {
		if (outputStream) {
		...
		this._logService.trace(
			`[ToolCallingLoop] SubagentStop hook blocked stopping: ${reasons.join('; ')}`,
		);
	}
	...
	public async runStartHooks(
		...
		if (this.options.request.subAgentInvocationId) {
			...
			if (startHookResult.additionalContext) {
				...
				this._logService.info(
					`[ToolCallingLoop] SubagentStart hook provided context for subagent ${this.options.request.subAgentInvocationId}`,
				);
			}
		} else {
			...
			if (hasHooks) {
				...
				const history: IHistoricalTurn[] = priorTurns.map((turn) => ({
					...
					rounds: turn.rounds.map((round) => ({
						...
						reasoningText: round.thinking
							? Array.isArray(round.thinking.text)
								? round.thinking.text.join('')
								: round.thinking.text
							: undefined,
				...
				await this._sessionTranscriptService.startSession(
					sessionId,
					undefined,
			...
			if (isFirstTurn) {
				...
				if (startHookResult.additionalContext) {
					this.additionalHookContext =
						startHookResult.additionalContext;
					this._logService.info(
						'[ToolCallingLoop] SessionStart hook provided context for session',
					);
				}
			}
		}
	...
	public async run(
		...
		const modeInstructions = (
			this.options.request as {
				modeInstructions2?: { name?: string; isBuiltin?: boolean };
			}
		).modeInstructions2;
		const customModeName =
			modeInstructions?.name && !modeInstructions.isBuiltin
				? modeInstructions.name
				: undefined;
		const agentType: 'builtin' | 'custom' =
			modeInstructions && modeInstructions.isBuiltin === false
				? 'custom'
				: 'builtin';
		...
		const parentTraceContext =
			(subAgentInvocationId
				? this._otelService.getStoredTraceContext(
						`subagent:invocation:${subAgentInvocationId}`,
					)
				: undefined) ??
			(() => {
				...
				const ctx = this._otelService.getStoredTraceContext(
					`subagent:request:${parentRequestId}`,
				);
				if (ctx) {
					this._otelService.storeTraceContext(
						`subagent:request:${parentRequestId}`,
						ctx,
					);
				}
		...
		const parentChatSessionId =
			getCurrentCapturingToken()?.parentChatSessionId;
		...
		return this._otelService.startActiveSpan(
			`invoke_agent ${agentName}`,
			{
				...
				attributes: {
					...
					[GenAiAttr.CONVERSATION_ID]:
						this.options.conversation.sessionId,
					[CopilotChatAttr.SESSION_ID]:
						this.options.conversation.sessionId,
					...(chatSessionId
					...
					...(parentChatSessionId
						...
						: {}),
					...(debugLogLabel
						? { [CopilotChatAttr.DEBUG_LOG_LABEL]: debugLogLabel }
						: {}),
					...(customModeName
						? { [CopilotChatAttr.MODE_NAME]: customModeName }
						: {}),
			...
			async (span) => {
				...
				if (chatSessionId) {
					...
					if (parentChatSessionId) {
						const childLabel =
							debugLogLabel ?? `runSubagent-${agentName}`;
						...
						if (invokeSpanId) {
							fileLogger.registerSpanSession(
								invokeSpanId,
								chatSessionId,
							);
						}
						...
						fileLogger.startSession(chatSessionId).catch(() => {
							/* best effort */
						});
					}
				}
				...
				try {
					const endpoint =
						await this._endpointProvider.getChatEndpoint(
				...
				let turnIndex = 0;
				const tokenListener = this.onDidReceiveResponse(
					({ response }) => {
	...
	private async _runLoop(
		...
		while (true) {
			if (lastResult && i++ >= this.options.toolCallLimit) {
				...
				if (
					permLevel === 'autopilot' &&
					this.options.toolCallLimit < 200
				) {
					this.options.toolCallLimit = Math.min(
						Math.round((this.options.toolCallLimit * 3) / 2),
						200,
					);
					this.showAutopilotProgress(
						outputStream,
						l10n.t('Autopilot: extending tool call limit\u2026'),
						l10n.t('Autopilot extended tool call limit'),
					);
				} else {
					lastResult = this.hitToolCallLimit(
						outputStream,
						lastResult,
					);
					break;
			...
			if (lastResult && this.options.yieldRequested?.()) {
				if (
					this.options.request.permissionLevel !== 'autopilot' ||
					this.taskCompleted
				) {
					break;
				}
			...
			try {
				...
				this._sessionTranscriptService.logAssistantTurnStart(
					sessionId,
					turnId,
				);
				agentSpan?.addEvent('turn_start', {
					turnId,
					...(chatSessionId
						? { [CopilotChatAttr.CHAT_SESSION_ID]: chatSessionId }
						: {}),
				...
				if (lastRequestMessagesStartingIndexForRun === undefined) {
					lastRequestMessagesStartingIndexForRun =
						result.lastRequestMessages.length - 1;
				}
				lastResult = {
					...result,
					hadIgnoredFiles:
						lastResult?.hadIgnoredFiles || result.hadIgnoredFiles,
				};

				this.toolCallRounds.push(result.round);
				this._sessionTranscriptService.logAssistantTurnEnd(
					sessionId,
					turnId,
				);
				agentSpan?.addEvent('turn_end', {
					turnId,
					...(chatSessionId
						? { [CopilotChatAttr.CHAT_SESSION_ID]: chatSessionId }
						: {}),
				...
				if (
					...
					!result.round.toolCalls.some(
						(tc) =>
							tc.name === ToolCallingLoop.TASK_COMPLETE_TOOL_NAME,
					)
				) {
				...
				if (
					...
					if (
						...
						this.shouldAutoRetry(result.response)
					) {
						this.autopilotRetryCount++;
						this._logService.info(
							`[ToolCallingLoop] Auto-retrying on error (attempt ${this.autopilotRetryCount}/${ToolCallingLoop.MAX_AUTOPILOT_RETRIES}): ${result.response.type}`,
						);
						if (
							this.options.request.permissionLevel === 'autopilot'
						) {
							this.showAutopilotProgress(
								outputStream,
								l10n.t(
									'Autopilot: recovering from a request error\u2026',
								),
								l10n.t(
							...
							this.showAutopilotProgress(
								outputStream,
								l10n.t('Recovering from a request error\u2026'),
								l10n.t('Recovered from a request error'),
							);
						}
					...
					if (this.options.request.subAgentInvocationId) {
						const stopHookResult =
							await this.executeSubagentStopHook(
								{
									agent_id:
										this.options.request
											.subAgentInvocationId,
									agent_type:
										this.options.request.subAgentName ??
										'default',
						...
						const joinedReasons =
							stopHookResult.reasons?.join('; ');
						this._logService.info(
							`[ToolCallingLoop] Subagent stop hook result: shouldContinue=${stopHookResult.shouldContinue}, reasons=${joinedReasons}`,
						);
						if (
							stopHookResult.shouldContinue &&
							stopHookResult.reasons?.length
						) {
							// The stop hook blocked stopping - show reasons and continue
							this.showSubagentStopHookBlockedMessage(
								outputStream,
								stopHookResult.reasons,
							);
							...
							result.round.hookContext = formatHookContext(
								stopHookResult.reasons,
							);
							this._logService.info(
								`[ToolCallingLoop] Subagent stop hook blocked, continuing with reasons: ${joinedReasons}`,
							);
							stopHookActive = true;
							continue;
						}
						...
						const joinedReasons =
							stopHookResult.reasons?.join('; ');
						this._logService.info(
							`[ToolCallingLoop] Stop hook result: shouldContinue=${stopHookResult.shouldContinue}, reasons=${joinedReasons}`,
						);
						if (
							stopHookResult.shouldContinue &&
							stopHookResult.reasons?.length
							...
							// The stop hook blocked stopping - show reasons and continue
							this.showStopHookBlockedMessage(
								outputStream,
								stopHookResult.reasons,
							);
							// Store the joined reasons so it can be passed to the model in the next prompt
							...
							this._logService.info(
								`[ToolCallingLoop] Stop hook blocked, continuing with reasons: ${joinedReasons}`,
							);
							stopHookActive = true;
					...
					if (
						...
						const autopilotContinue =
							this.shouldAutopilotContinue(result);
						if (autopilotContinue) {
							this._logService.info(
								`[ToolCallingLoop] Autopilot internal stop hook: continuing because task may not be complete`,
							);
							this.showAutopilotProgress(
								outputStream,
								l10n.t(
									'Autopilot: verifying task is done\u2026',
								),
		...
		this.emitReadFileTrajectories().catch((err) => {
			this._logService.error(
				'Error emitting read file trajectories',
				err,
			);
		});

		const toolCallRoundsToDisplay = lastResult.lastRequestMessages
			.slice(lastRequestMessagesStartingIndexForRun ?? 0)
			.filter(
				(m): m is Raw.ToolChatMessage => m.role === Raw.ChatRole.Tool,
			);
			...
			if (result instanceof LanguageModelToolResult2) {
				for (const part of result.content) {
					if (
						...
						part.mimeType === 'application/pull-request+json' &&
						part.audience?.includes(LanguageModelPartAudience.User)
					) {
						const data: {
							uri: string;
							title: string;
							...
							author: string;
							linkTag: string;
						} = JSON.parse(part.data.toString());
						outputStream?.push(
							new ChatResponsePullRequestPart(
								{
									command:
									...
									arguments: [
										Number(data.linkTag.substring(1)),
									],
								},
								data.title,
								data.description,
				...
				*/
				this._telemetryService.sendMSFTTelemetryEvent(
					...
					},
					{
						...
						avgChunkSize:
							chunkSizeNo > 0
								? Math.round(chunkSizeTotal / chunkSizeNo)
								: -1,
					},
				);
			}
	...
	private hitToolCallLimit(
		...
		if (
			stream &&
			this.options.onHitToolCallLimit === ToolCallLimitBehavior.Confirm
		) {
			const messageString = new MarkdownString(
				l10n.t({
					message:
						'Copilot has been working on this problem for a while. It can continue to iterate, or you can send a new message to refine your prompt. [Configure max requests]({0}).',
					args: [
						`command:workbench.action.openSettings?${encodeURIComponent('["chat.agent.maxRequests"]')}`,
					],
					comment:
						'Link to workbench settings for chat.maxRequests, which controls the maximum number of requests Copilot will make before stopping. This is used in the tool calling loop to determine when to stop iterating on a problem.',
				}),
			);
			messageString.isTrusted = {
			...
			stream.confirmation(
				...
				messageString,
				{
					copilotRequestedRoundLimit: Math.round(
						(this.options.toolCallLimit * 3) / 2,
					),
				} satisfies IToolCallIterationIncrease,
	...
	public async runOne(
		...
		markChatExt(
			this.options.conversation.sessionId,
			ChatExtPerfMark.WillBuildPrompt,
		);
		...
		const keepAliveEndpoint = await this._endpointProvider.getChatEndpoint(
			this.options.request,
		);
		const keepAlive = this.startBuildPromptKeepAlive(
			token,
			keepAliveEndpoint,
		);
		try {
			buildPromptResult = await this.buildPrompt2(
				context,
				outputStream,
				token,
			);
			...
			markChatExt(
				this.options.conversation.sessionId,
				ChatExtPerfMark.DidBuildPrompt,
			);
		}
		...
		const promptOverride = this._configurationService.getConfig(
			ConfigKey.Advanced.DebugPromptOverrideString,
		);
		const promptOverrideFile = this._configurationService.getConfig(
			ConfigKey.Advanced.DebugPromptOverrideFile,
		);
		...
		if (promptOverride || promptOverrideFile) {
			...
			effectiveBuildPromptResult = {
				...buildPromptResult,
				messages: overrideResult.messages,
			};
		...
		const isToolInputFailure = effectiveBuildPromptResult.metadata.get(
			ToolFailureEncountered,
		);
		const conversationSummary = effectiveBuildPromptResult.metadata.get(
		...
		const promptTokenLength = await tokenizer.countMessagesTokens(
			effectiveBuildPromptResult.messages,
		);
		const toolTokenCount =
			availableTools.length > 0
				? await tokenizer.countToolTokens(availableTools)
				: 0;
		this.throwIfCancelled(token);
		this._onDidBuildPrompt.fire({
			result: effectiveBuildPromptResult,
		...
		const interactionOutcomeComputer = new InteractionOutcomeComputer(
			this.options.interactionContext,
		);

		const that = this;
		const responseProcessor = new (class implements IResponseProcessor {
			private readonly context = new ResponseProcessorContext(
				that.options.conversation.sessionId,
				that.turn,
				effectiveBuildPromptResult.messages,
			...
			async processResponse(
				...
				if (that.options.responseProcessor) {
					chatResult =
						await that.options.responseProcessor.processResponse(
							this.context,
							inputStream,
							responseStream,
							token,
					...
					const responseProcessor =
						that._instantiationService.createInstance(
							PseudoStopStartResponseProcessor,
							[],
							undefined,
							{
								subagentInvocationId:
									that.options.request.subAgentInvocationId,
							},
						);
		...
		if (outputStream) {
			this.options.streamParticipants?.forEach((fn) => {
				streamParticipants.push(
					fn(streamParticipants[streamParticipants.length - 1]),
				);
			});
			const stream = streamParticipants[streamParticipants.length - 1];

			fetchStreamSource = new FetchStreamSource();
			processResponsePromise = responseProcessor.processResponse(
				undefined,
		...
		const shouldDisableThinking =
			isContinuation &&
			isAnthropicFamily(endpoint) &&
			!ToolCallingLoop.messagesContainThinking(
				effectiveBuildPromptResult.messages,
			);
		...
		markChatExt(
			this.options.conversation.sessionId,
			ChatExtPerfMark.WillFetch,
		);
		const fetchOptions: ToolCallingLoopFetchOptions = {
			messages: this.applyMessagePostProcessing(
				effectiveBuildPromptResult.messages,
				{ stripOrphanedToolCalls: isGeminiFamily(endpoint) },
			),
			turnId: this.options.request.id,
			topLevelTurnId:
				this.options.request.parentRequestId ?? this.turn.id,
			summarizedAtRoundId,
			finishedCb: async (text, index, delta) => {
				fetchStreamSource?.update(text, delta);
				if (delta.copilotToolCalls) {
					toolCalls.push(
						...delta.copilotToolCalls.map(
							(call): IToolCall => ({
								...call,
		...
		if (fetchResult.type === ChatFetchResponseType.Success) {
			this.lastHeaderRequestId =
				fetchResult.serverRequestId || fetchResult.requestId;
		...
		if (
			...
			fetchResult.usage &&
			stream &&
			this.shouldReportUsageToContextWidget()
		) {
			stream.usage({
		...
		if (
			...
			this._authenticationChatUpgradeService.showPermissiveSessionUpgradeInChat(
				outputStream,
				this.options.request,
			);
		...
		const toolInputRetry = isToolInputFailure
			? (this.toolCallRounds.at(-1)?.toolInputRetry || 0) + 1
			: 0;
		if (fetchResult.type === ChatFetchResponseType.Success) {
			if (fetchResult.usage) {
				this.turn.setMetadata(
					new TurnTokenUsageMetadata(
			...
			const transcriptToolRequests: ToolRequest[] = toolCalls.map(
				(tc) => ({
					toolCallId: tc.id,
					name: tc.name,
					arguments: tc.arguments,
			...
			this._sessionTranscriptService.logAssistantMessage(
				...
				thinkingItem
					? Array.isArray(thinkingItem.text)
						? thinkingItem.text.join('')
						: thinkingItem.text
					: undefined,
			);
	...
	private applyMessagePostProcessing(
		messages: Raw.ChatMessage[],
		options?: { stripOrphanedToolCalls?: boolean },
	): Raw.ChatMessage[] {
	...
	public static stripInternalToolCallIds(
		messages: Raw.ChatMessage[],
	): Raw.ChatMessage[] {
	...
	public static messagesContainThinking(
		messages: Raw.ChatMessage[],
	): boolean {
			...
			return (
				...
				m.content.some(
					(part) =>
						...
						rawPartAsThinkingData(part) !== undefined,
				)
			);
		}
		return false;
	 ...
	 */
	public static validateToolMessagesCore(
		messages: Raw.ChatMessage[],
		options?: { stripOrphanedToolCalls?: boolean },
	): {
		messages: Raw.ChatMessage[];
		...
		const filtered = messages.filter((m) => {
			if (m.role === Raw.ChatRole.Assistant) {
				...
				const toolCall = previousAssistantMessage.toolCalls.find(
					(tc) => tc.id === m.toolCallId,
				);
				if (!toolCall) {
		...
		for (let i = 0; i < filtered.length; i++) {
			...
			const orphanedToolCalls = m.toolCalls.filter(
			...
			if (orphanedToolCalls.length > 0) {
				...
				(m as Mutable<Raw.AssistantChatMessage>).toolCalls =
	...
	private validateToolMessages(
		...
		} = ToolCallingLoop.validateToolMessagesCore(messages, options);

		if (filterReasons.length || strippedToolCallCount > 0) {
			const allReasons =
				strippedToolCallCount > 0
					? [
							...filterReasons,
							`orphanedToolCalls:${strippedToolCallCount}`,
						]
					: filterReasons;
			const filterReasonsStr = allReasons.join(', ');
			this._logService.warn(
				'Filtered invalid tool messages: ' + filterReasonsStr,
			);
			/* __GDPR__
	 ...
	 */
	private startBuildPromptKeepAlive(
		...
		if (
			!this._configurationService.getExperimentBasedConfig(
				ConfigKey.Advanced.LongToolCallCachePreservation,
		...
		const hasExecutionSubagent =
			latestRound?.toolCalls.some(
				(tc) => tc.name === ToolName.ExecutionSubagent,
			) ?? false;
		if (!hasExecutionSubagent) {
			return Disposable.None;
		}

		const maxProbes = this._configurationService.getExperimentBasedConfig(
			ConfigKey.Advanced.LongToolCallCachePreservationMaxProbes,
			this._experimentationService,
		);
		...
		const probe = async () => {
			...
			if (probeCount >= maxProbes) {
				this._logService.info(
					`[ToolCallingLoop] Keep-alive: max probes reached (${probeCount}), stopping`,
				);
				return;
			}
			probeCount++;
			...
			const probeMessages: Raw.ChatMessage[] =
				lastMessage.role === Raw.ChatRole.Assistant
					? [
							...processedMessages,
							{
			...
			this._logService.info(
				`[ToolCallingLoop] Keep-alive: sending probe ${probeCount}/${maxProbes}`,
			);
			try {
				await this.fetch(
					{
				...
				if (!isCancellationError(err)) {
					this._logService.warn(
						`[ToolCallingLoop] Keep-alive probe failed: ${err instanceof Error ? err.message : String(err)}`,
					);
				}
			} finally {
				inFlight.delete(cts);
	...
	private async buildPrompt2(
		...
		const buildPromptResult = await this.buildPrompt(
			buildPromptContext,
			progress,
			token,
		);
		for (const metadata of buildPromptResult.metadata.getAll(
			ToolResultMetadata,
		)) {
		...
		if (
			buildPromptResult.metadata
				.getAll(ToolResultMetadata)
				.some((r) => r.isCancelled)
		) {
			throw new CancellationError();
		}
	...
	private logToolResult(
		...
		let originalCall = lastTurn?.toolCalls.find(
			(tc) => tc.id === metadata.toolCallId,
		);
		if (!originalCall) {
			const byRef = buildPromptContext.tools?.toolReferences.find(
				(r) => r.id === metadata.toolCallId,
			);
			if (byRef) {
				originalCall = {
					id: byRef.id,
					arguments: JSON.stringify(byRef.input),
					name: byRef.name,
				};
			}
		}
