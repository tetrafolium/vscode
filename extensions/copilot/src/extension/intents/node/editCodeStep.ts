...
} from '../../../platform/notebook/common/alternativeContent';
...
export class PreviousEditCodeStep {
	public static fromChatResultMetaData(
		...
		if (isEditHistoryDTO(edits)) {
			...
			const promptInstructions =
				edits.promptInstructions?.map((entry) => {
					return {
						document: {
							uri: URI.revive(entry.uri),
							version: entry.version,
			...
			return new PreviousEditCodeStep(
				entries,
				edits.request,
				edits.response,
				promptInstructions,
	...
	public static fromTurn(turn: Turn): PreviousEditCodeStep | undefined {
		...
		if (!editCodeStep && turn.responseChatResult) {
			editCodeStep = PreviousEditCodeStep.fromChatResultMetaData(
				turn.responseChatResult,
			);
			if (editCodeStep) {
	...
	public static fromEditCodeStep(editCodeStep: EditCodeStep) {
		...
		const promptInstructions = editCodeStep.promptInstructions.map(
			(entry) => ({
				document: {
					uri: entry.uri,
					version: entry.version,
		...
		return new PreviousEditCodeStep(
			workingSet,
			editCodeStep.userMessage,
			editCodeStep.assistantReply,
			promptInstructions,
		);
	}
	...
	public setWorkingSetEntryState(
		uri: URI,
		state: { accepted: boolean; hasRemainingEdits: boolean },
	): void {
		for (const entry of this.workingSet) {
			if (isEqual(entry.document.uri, uri)) {
				entry.state = this._getUpdatedState(
					entry,
					state.accepted,
					state.hasRemainingEdits,
				);
			}
		}
	}

	private _getUpdatedState(
		workingSetEntry: IPreviousWorkingSetEntry,
		accepted: boolean,
		...
		if (
			state === WorkingSetEntryState.Accepted ||
			state === WorkingSetEntryState.Rejected
		) {
			return state;
		}
...
export class EditCodeStep {
	...
	): Promise<EditCodeStepChatVariablesPair> {
		const factory =
			instantiationService.createInstance(EditCodeStepFactory);
...
class EditCodeStepFactory {
	...
	public async createNextStep(
		...
		const getCurrentOrPreviousWorkingSetEntryState = (uri: Uri) => {
			...
			if (prevStep) {
				const previousStepEntry = prevStep.workingSet.find((entry) =>
		...
		const addWorkingSetEntry = async (
			documentOrCellUri: URI,
			isMarkedReadonly: boolean | undefined,
			range?: Range,
		) => {
			try {
				const uri = this._notebookService.hasSupportedNotebooks(
					documentOrCellUri,
				)
					? (findNotebook(
							documentOrCellUri,
							this._workspaceService.notebookDocuments,
						)?.uri ?? documentOrCellUri)
					: documentOrCellUri;
				if (!getWorkingSetEntry(uri)) {
					...
					if (this._notebookService.hasSupportedNotebooks(uri)) {
						...
						const [document, notebook] = await Promise.all([
							this._workspaceService.openNotebookDocumentAndSnapshot(
								uri,
								format,
							),
						...
						if (cell) {
							range =
								...
								new Range(
									...
									cell.document.lineAt(
										cell.document.lineCount - 1,
									).range.end,
								);
							range = getAltNotebookRange(
								range,
								cell.document.uri,
								document.document,
								format,
		...
		for (const chatVariable of chatVariables) {
			if (
				isInstructionFile(chatVariable) ||
				isCustomizationsIndex(chatVariable)
			) {
				...
				if (isUri(chatVariable.value)) {
					const textDocument =
						await this._workspaceService.openTextDocument(
							chatVariable.value,
						);
					promptInstructions.push(
						TextDocumentSnapshot.create(textDocument),
					);
				}
			} else if (isNotebookVariable(chatVariable.value)) {
				const [notebook] = getNotebookAndCellFromUri(
					chatVariable.value,
					this._workspaceService.notebookDocuments,
				);
				if (!notebook) {
				...
				await addWorkingSetEntry(
					chatVariable.value,
					chatVariable.isMarkedReadonly,
				);
			} else if (isLocation(chatVariable.value)) {
				await addWorkingSetEntry(
					chatVariable.value.uri,
					chatVariable.isMarkedReadonly,
					chatVariable.value.range,
				);
			} else {
				otherChatVariables.push(chatVariable.reference);
			}
...
export function isNotebookVariable(
	chatVariableValue?: unknown,
): chatVariableValue is URI | Uri {
	if (!chatVariableValue || !isUri(chatVariableValue)) {
		return false;
	}
	return (
		chatVariableValue.scheme === Schemas.vscodeNotebookCell ||
		chatVariableValue.scheme === Schemas.vscodeNotebookCellOutput
	);
}
...
type IMutableWorkingSetEntry =
	| ITextDocumentMutableWorkingSetEntry
	| INotebookMutableWorkingSetEntry;
...
function isWorkingSetEntryDTO(data: any): data is WorkingSetEntryDTO {
	return (
		data &&
		isUriComponents(data.uri) &&
		isString(data.text) &&
		isNumber(data.version) &&
		isString(data.languageId) &&
		isNumber(data.state)
	);
}
...
function isEditHistoryDTO(data: any): data is EditHistoryDTO {
	return (
		data &&
		Array.isArray(data.workingSet) &&
		data.workingSet.every(isWorkingSetEntryDTO) &&
		isString(data.request) &&
		isString(data.response)
	);
}
