...
import {
	NewNotebookCodeGenerationPrompt,
	NewNotebookCodeImprovementPrompt,
...
export class NewNotebookResponseProcessor {
	...
	private _resolvedContentDeferredPromise: DeferredPromise<
		ChatResponseFileTreePart | ChatResponseMarkdownPart
	> = new DeferredPromise();
	...
	constructor(
		...
		@IAlternativeNotebookContentEditGenerator
		private readonly noteBookEditGenerator: IAlternativeNotebookContentEditGenerator,
		...
		if (this._incodeblock) {
			...
			if (codeblockEnd === -1) {
				...
				try {
					...
					this._resolvedContentDeferredPromise.complete(
		...
		if (codeblockStart !== -1) {
			...
			if (codeblockEnd !== -1) {
				...
				this.applyDeltaToProgress(
					textDelta.substring(0, codeblockStart),
					progress,
				);
				this.applyDeltaToProgress(
					textDelta.substring(codeblockEnd + 3),
					progress,
				);
				this.applyDeltaToTurn(textDelta, turn);
				this.reporting = false;
				...
				this._resolvedContentDeferredPromise.p.then((p) =>
					progress.push(p),
				);
				return;
			}
		...
		try {
			...
			if (outline) {
				...
				const newNotebook = new Lazy(async () => {
					...
					const updateMetadata = NotebookEdit.updateNotebookMetadata(
						Object.assign(
							{ new_copilot_notebook: true },
							notebook.metadata,
						),
					);
				...
				const sourceLines = filterFilePathFromCodeBlock2(
					streamLines(sourceStream.asyncIterable)
				...
				const created = this.createNewNotebook2(
					sourceLines,
					newNotebook.value,
					token,
				);
				async function finishedCb(
					text: string,
					index: number,
	...
	private async createNewNotebook2(
		lines: AsyncIterable<LineOfText>,
		newNotebook: Promise<NotebookDocument>,
		token: CancellationToken,
	) {
		...
		for await (const edit of this.noteBookEditGenerator.generateNotebookEdits(
			Uri.file('empty.ipynb'),
			lines,
			telemetryOptions,
			token,
		)) {
			if (!Array.isArray(edit)) {
				...
				promises.push(
					Promise.resolve(
						this.workspaceService.applyEdit(workspaceEdit),
					),
				);
			}
		}
...
// different than the one in the tool, this one can't rely on the output stream workaround
function filterFilePathFromCodeBlock2(
...
export async function newNotebookCodeCell(
	instantiationService: IInstantiationService,
	chatMLFetcher: IChatMLFetcher,
	endpoint: IChatEndpoint,
	options: IConversationOptions,
	history: readonly Raw.ChatMessage[] | undefined,
	description: string,
...
export async function improveNotebookCodeCell(
	instantiationService: IInstantiationService,
	chatMLFetcher: IChatMLFetcher,
	endpoint: IChatEndpoint,
	options: IConversationOptions,
