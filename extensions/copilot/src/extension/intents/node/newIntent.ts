...
import {
	NewWorkspaceGithubContentMetadata,
	NewWorkspacePrompt,
} from '../../prompts/node/panel/newWorkspace/newWorkspace';
...
} from './generateNewWorkspaceContent';
...
export const INewWorkspacePreviewContentManager =
	createServiceIdentifier<INewWorkspacePreviewContentManager>(
...
export class NewWorkspacePreviewContentManagerImpl implements INewWorkspacePreviewContentManager {
	...
	constructor(
		...
		this.copilotContentManager = instantiationService.createInstance(
			NewWorkspaceCopilotContentManager,
		);
		this.githubContentManager = instantiationService.createInstance(
	...
	set(
		...
		if (isGithubWorkspaceUri(fileTree.baseUri)) {
			...
			this.copilotContentManager.set(
				responseId,
				projectName,
				fileTree,
				serviceArgs,
	...
	get(uri: Uri): FileTreeDataWithContent | undefined {
		...
		if (isGithubWorkspaceUri(uri)) {
			fileContents = this.githubContentManager.get(
				uri.authority,
				uri.path,
			);
			...
			fileContents = this.copilotContentManager.get(
				uri.authority,
				uri.path,
...
export class NewWorkspaceCopilotContentManager {
	...
	private generatePlanPrompt = this.instantiationService.createInstance(
		ProjectSpecificationGenerator,
	);
	private generateFilePrompt = this.instantiationService.createInstance(
		FileContentsGenerator,
	);
	...
	set(
		...
		const projectSpecificationPromise = this.generatePlanPrompt.generate(
			promptArgs,
		...
		const projectData: ProjectData = {
			userPrompt,
			projectSpecification: projectSpecificationPromise,
			projectStructure,
			fileTree: fileTree,
			chatMessages,
		};
	...
	get(
		responseId: string,
		path: string,
		prevFileContents: Map<string, string>,
	): FileTreeDataWithContent | undefined {
		const { projectName, path: relativePath } =
			this._getProjectMetadata(path);
		...
		if (currentNode && !currentNode?.content) {
			...
			nodeWithMissingContent.content = this._getFileContent(
				data.userPrompt,
				...
				data.chatMessages,
				prevFileContents,
			).catch(() => (nodeWithMissingContent.content = undefined));
		}
		return currentNode;
	}
	...
	private async _getFileContent(
		...
		projectStructure: string,
		projectSpecPromise: Promise<string>,
		filePath: string,
		chatMessages: Raw.ChatMessage[],
		prevFileContents: Map<string, string>,
	): Promise<Uint8Array> {
		const promptArgs: NewWorkspaceContentsPromptProps = {
			...
			history: chatMessages,
			relavantFiles: prevFileContents.has(filePath)
				? new Map([[filePath, prevFileContents.get(filePath)!]])
				: undefined,
		};
		return this.generateFilePrompt
			.generate(promptArgs, CancellationToken.None)
...
class NewWorkspaceGitHubContentManager {
	...
	get(
		...
		const { projectName, path: relativePath } =
			this._getProjectMetadata(filePath);
		const responseScopedData = this._getResponseScopedData(responseId);
		...
		if (currentNode && !currentNode?.content && !currentNode?.children) {
			...
			const folderPath =
				rootNode.path === '.'
					? path.posix.relative(rootNode.repo, filePath)
					: path.posix.relative(rootNode.path, filePath.slice(1));
			nodeWithMissingContent.content = this.repositoryService
...
class NewWorkspaceFileContentManager {
	...
	get(
		...
		const { projectName, path: relativePath } =
			this._getFileMetadata(filePath);
		const responseScopedData = this._getResponseScopedData(responseId);
		const rootNode = responseScopedData.get(projectName);
		if (!rootNode) {
			return;
		...
		const currentNode = findMatchingNodeFromPath(fileNodes, relativePath);
		if (currentNode && !currentNode?.content && !currentNode?.children) {
			currentNode.content = Promise.resolve(
				new Uint8Array(new TextEncoder().encode(rootNode.content)),
			);
		}
		return currentNode;
	}
...
function findMatchingNodeFromPath(
	fileTree: vscode.ChatResponseFileTree[],
	pathElements: string[],
): FileTreeDataWithContent | undefined {
...
export class NewWorkspaceIntentInvocation implements IIntentInvocation {
	...
	async getShouldUseProjectTemplate() {
		const useProjectTemplates = this.configurationService.getConfig(
			ConfigKey.UseProjectTemplates,
		);
	...
	processResponse(
		...
		const responseProcessor = new NewWorkspaceResponseProcessor(
...
function convertGitHubItemsToChatResponseFileTree(
	...
	const result: { [key: string]: vscode.ChatResponseFileTree } = {
		rootName: root,
	};
	...
	const sortedTree =
		baseTree?.sort((a, b) => (a.children && !b.children ? -1 : 1)) ?? [];
	return new ChatResponseFileTreePart(
		[{ name: rootName, children: sortedTree }],
		baseUri,
	);
}
...
class NewWorkspaceResponseProcessor {
	...
	async processResponse(
		...
		const chatMessages = messages.filter(
			(message) => message.role !== Raw.ChatRole.System,
		); // Exclude system messages as we want to use a different identity for the additional prompts we run
		...
		for await (const { delta } of inputStream) {
			...
			this._p = this._p.then(async (): Promise<string> => {
				...
				if (!this._appliedText.startsWith('#')) {
					...
					const hasWholeCodeBlock = this._appliedText.match(
						/```filetree\n([\s\S]+?)\n```/,
					);
					if (
						hasWholeCodeBlock &&
						(isBufferingFileTree || !hasReportingStarted)
					) {
						isBufferingFileTree = false;
						const [before, after] = this._appliedText.split(
							hasWholeCodeBlock[0],
						);
						...
						const { chatResponseTree, projectName } =
							convertFileTreeToChatResponseFileTree(
								projectStructure,
								(fp) => getNewPreviewUri(requestId, fp),
							);
						outputStream.progress(
							l10n.t('Generating workspace preview...'),
						);
						outputStream.push(chatResponseTree);
						outputStream.markdown(after);

						this.newWorkspacePreviewContentManager.set(
							requestId,
							projectName,
							chatResponseTree,
							{ userPrompt, projectStructure, chatMessages },
						);
					} else if (
						this._appliedText.match(fileTreeStartRegex) &&
						!isBufferingFileTree &&
						!hasWholeCodeBlock
					) {
						...
						const [_, after] =
							this._appliedText.split(fileTreeStartRegex);
		...
		if (
			...
			outputStream.reference(
				Uri.parse(
					this.githubContentMetadata.githubRepoItems[0].html_url,
				),
			);
			...
			const projectName = isRepoRoot
				? this.githubContentMetadata.repo
				: this.githubContentMetadata.path.split('/')[0];
			const chatResponseTree = convertGitHubItemsToChatResponseFileTree(
				this.githubContentMetadata.githubRepoItems,
				getNewPreviewUri(turn.id, projectName, true),
				isRepoRoot,
			);
			outputStream.push(chatResponseTree);
			...
			const query = encodeURIComponent(
				`["/${newId} ${turn.request.message}"]`,
			);
			const markdownString = new MarkdownString(
				l10n.t(
					`Hint: You can [regenerate this project without using this sample](command:workbench.action.chat.open?{0}) or use this [setting](command:workbench.action.openSettings?%5B%22github.copilot.chat.useProjectTemplates%22%5D) to configure the behavior.`,
					query,
				),
			);
			...
			if (fileContentGeneration.length === 2) {
				...
				const fileTree = new ChatResponseFileTreePart(
					[{ name: `${fileName.fileName}` }],
					baseUri,
				);
				const commandstr = commandUri(OpenFileCommand, [fileTree]);
				const markdownString = new MarkdownString(
					`[${fileName.fileName}](${commandstr})`,
				);
				markdownString.isTrusted = {
					enabledCommands: [OpenFileCommand],
				};
				outputStream.markdown(
					l10n.t('Sure, here is the file you requested:'),
				);
				outputStream.markdown(markdownString);
				this.newWorkspacePreviewContentManager.set(
					turn.id,
	...
	pushCommands(
		...
		const fileTree =
			this.newWorkspacePreviewContentManager.getFileTree(turnRequestId);
		if (!fileTree) {
			return;
		}
		...
		outputStream.button(
			createProjectCommand(
				fileTree,
				workspaceFolders.length > 0 ? workspaceFolders[0] : undefined,
			),
		);
	}
}
