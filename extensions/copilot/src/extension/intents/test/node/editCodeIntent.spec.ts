...
async function getCodeblocks(
	input: string[],
	outputStream: MockChatResponseStream,
	remoteName?: string,
) {
	...
	function createUriFromResponsePath(path: string): URI | undefined {
		return new PromptPathRepresentationService(
			new TestWorkspaceService(),
		).resolveFilePath(path);
	}

	const result: CodeBlock[] = [];
	const codeBlocks = getCodeBlocksFromResponse(
		textStream,
		outputStream,
...
describe('getCodeBlocksFromResponse', () => {
	it('should process code blocks with valid URIs', async () => {
		...
		expect(outputStream.uris).toEqual(['file:///valid/path']);
	});
	...
	it('should process code blocks without URIs', async () => {
		...
		expect(result[0].code).toBe(['npm install minify\n'].join(''));
		expect(outputStream.output.join('')).toBe(
			[
				'### [path](file:///valid/path)\n',
				'\n',
	...
	it('should not linkify non-filepath headers (#11079)', async () => {
		...
		expect(result[0].code).toBe([`print('Hello, World!')\n`].join(''));
		expect(outputStream.output.join('')).toBe(
			[
				'### change summary\n',
				'\n',
		...
		expect(outputStream.uris).toEqual(['untitled:Untitled-1']);
	});
	...
	it('should not linkify other headers', async () => {
		...
		expect(outputStream.uris).toEqual(['file:///a/b']);
	});
	...
	it('using 4 backticks (#11112)', async () => {
		...
		expect(result.length).toBe(1);
		expect(result[0].resource?.toString()).toBe(
