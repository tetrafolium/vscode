...
export class NewWorkspacePreviewFileSystemProvider implements FileSystemProvider {
	constructor(
		private readonly contentManager: INewWorkspacePreviewContentManager,
	) {}
	async stat(uri: Uri): Promise<FileStat> {
		...
		const size =
			(await node.content?.then((content) => content?.length)) ?? 0;
		return {
			ctime: node.ctime ?? 0,
			mtime: node.ctime ?? 0,
			size: size,
	...
	readDirectory(
		...
		return (
			node.children?.map((child) => [
				child.name,
				child.children ? FileType.Directory : FileType.File,
			]) ?? []
		);
	}
