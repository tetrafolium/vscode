...
export class FixTestFailureContribution
	extends Disposable
...
type FixCommandArgs = {
	...
	test: vscode.TestItem | vscode.TestResultSnapshot;
	source?: 'sparkles' | 'testResultsPanel' | 'retry';
};

function registerTestMessageSparkles(
	store: DisposableStore,
	telemetryService: ITelemetryService,
	testProvider: ITestProvider,
) {
	function getLastFailureForItemOrChildren(
		item: vscode.TestItem,
	): ITestFailure | undefined {
		const failure = testProvider.getLastFailureFor(item);
		return (
			failure ||
			mapFindFirst(item.children, ([, item]) =>
				getLastFailureForItemOrChildren(item),
			)
		);
	}
	...
	async function openFixChat(args: FixCommandArgs) {
		...
		*/
		...
		await vscode.window.showTextDocument(doc, {
			...
			preview: true,
			selection: args.test.range
				? new vscode.Range(args.test.range.start, args.test.range.start)
				: undefined,
		});
...
function registerTestFailureCodeAction(
	testProvider: ITestProvider,
	...
	store.add(
		vscode.languages.registerCodeActionsProvider('*', {
			provideCodeActions(document, range, context, token) {
				const copilotCodeActionsEnabled =
					configurationService.getConfig(ConfigKey.EnableCodeActions);
				if (!copilotCodeActionsEnabled) {
					return;
				}

				const test = testProvider.getFailureAtPosition(
					document.uri,
					range.start,
				);
				if (!test) {
