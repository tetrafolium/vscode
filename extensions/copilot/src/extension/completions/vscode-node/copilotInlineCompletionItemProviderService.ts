...
import {
	createContext,
	setup,
} from '../../completions-core/vscode-node/completionsServiceBridges';
...
export class CopilotInlineCompletionItemProviderService
	extends Disposable
	implements ICopilotInlineCompletionItemProviderService
{
	...
	getOrCreateInstantiationService(): IInstantiationService {
		if (!this._completionsInstantiationService) {
			this._completionsInstantiationService =
	...
	getOrCreateProvider(): InlineCompletionItemProvider {
		if (!this._provider) {
			this._completionsInstantiationService =
				this.getOrCreateInstantiationService();
			this._completionsInstantiationService.invokeFunction(
				setup,
				this._store,
			);
			this._provider = this._register(
				this._completionsInstantiationService.createInstance(
