...
export class CompletionsCoreContribution extends Disposable {
	private readonly _copilotToken = observableFromEvent(
		...
		() => this.authenticationService.copilotToken,
	);
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				...
				const configEnabled = configurationService
				...
				const extensionUnification =
					unificationStateValue?.extensionUnification ?? false;
				...
				if (wantsProvider && copilotToken) {
					const provider =
						_copilotInlineCompletionItemProviderService.getOrCreateProvider();
					reader.store.add(
				...
				if (extensionUnification && hasInstantiatedProvider) {
					const completionsInstaService =
						_copilotInlineCompletionItemProviderService.getOrCreateInstantiationService();
					reader.store.add(
