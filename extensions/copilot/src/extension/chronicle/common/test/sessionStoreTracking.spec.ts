...
import {
	...
	isGitHubMcpTool,
} from '../sessionStoreTracking';

describe('extractFilePath', () => {
	it('extracts filePath from replace_string_in_file', () => {
		expect(
			extractFilePath('replace_string_in_file', {
				filePath: '/src/foo.ts',
				oldString: 'a',
				newString: 'b',
			}),
	...
	it('extracts filePath from multi_replace_string_in_file replacements array', () => {
		expect(
			extractFilePath('multi_replace_string_in_file', {
	...
	it('extracts filePath from insert_edit_into_file', () => {
		expect(
			extractFilePath('insert_edit_into_file', {
	...
	it('extracts filePath from edit_notebook_file', () => {
		expect(
			extractFilePath('edit_notebook_file', {
				filePath: '/nb.ipynb',
				editType: 'edit',
				cellId: 'c1',
			}),
	...
	it('extracts path from list_dir', () => {
		expect(extractFilePath('list_dir', { path: '/src' })).toBe('/src');
	});

	it('extracts dirPath from create_directory', () => {
		expect(
			extractFilePath('create_directory', { dirPath: '/src/new-dir' }),
		).toBe('/src/new-dir');
	});
	...
	it('extracts file path from apply_patch input text', () => {
		...
		expect(extractFilePath('apply_patch', { input })).toBe('/src/hello.ts');
	});

	it('extracts file path from apply_patch Add File', () => {
		const input =
			'*** Begin Patch\n*** Add File: /src/new.ts\n+export const x = 1;\n*** End Patch';
		expect(extractFilePath('apply_patch', { input })).toBe('/src/new.ts');
	});

	it('extracts file path from apply_patch Delete File', () => {
		const input =
			'*** Begin Patch\n*** Delete File: /src/old.ts\n*** End Patch';
		expect(extractFilePath('apply_patch', { input })).toBe('/src/old.ts');
	});

	it('falls back to filePath arg for apply_patch when present', () => {
		expect(
			extractFilePath('apply_patch', {
				filePath: '/from/arg.ts',
				input: '*** Update File: /from/input.ts',
			}),
		).toBe('/from/arg.ts');
	});
	...
	it('extracts path from CLI str_replace_editor (backward compat)', () => {
		expect(
			extractFilePath('str_replace_editor', { path: '/cli/file.py' }),
		).toBe('/cli/file.py');
	});

	it('extracts path from CLI create tool (backward compat)', () => {
		expect(extractFilePath('create', { path: '/cli/new.py' })).toBe(
			'/cli/new.py',
		);
	});
	...
	it('returns undefined for unknown tools', () => {
		expect(
			extractFilePath('run_in_terminal', { command: 'ls' }),
		).toBeUndefined();
		expect(
			extractFilePath('file_search', { query: '**/*.ts' }),
		).toBeUndefined();
	});

	it('returns undefined for null args', () => {
		expect(extractFilePath('create_file', null)).toBeUndefined();
	});

	it('returns undefined when filePath is not a string', () => {
		expect(
			extractFilePath('create_file', { filePath: 42 }),
		).toBeUndefined();
	});
});
...
describe('extractRefsFromMcpTool', () => {
	it('extracts PR ref from mcp_github_pull_request_read', () => {
		const refs = extractRefsFromMcpTool('mcp_github_pull_request_read', {
			pullNumber: 42,
			owner: 'ms',
			repo: 'vscode',
		});
	...
	it('extracts issue ref from mcp_github_issue_write', () => {
		const refs = extractRefsFromMcpTool('mcp_github_issue_write', {
			issue_number: 123,
			owner: 'ms',
			repo: 'vscode',
		});
	...
	it('extracts commit ref from mcp_github_get_commit', () => {
		const refs = extractRefsFromMcpTool('mcp_github_get_commit', {
			sha: 'abc123',
			owner: 'ms',
			repo: 'vscode',
		});
	...
	it('returns empty for non-matching tool name', () => {
		expect(
			extractRefsFromMcpTool('mcp_github_search_code', { query: 'foo' }),
		).toEqual([]);
	});
});
...
describe('extractRepoFromMcpTool', () => {
	it('extracts owner/repo', () => {
		expect(
			extractRepoFromMcpTool({ owner: 'microsoft', repo: 'vscode' }),
...
describe('extractRefsFromTerminal', () => {
	...
	it('returns empty for unrelated commands', () => {
		expect(
			extractRefsFromTerminal({ command: 'ls -la' }, 'output'),
		).toEqual([]);
	});
});
...
describe('extractAgentName', () => {
	it('returns trimmed attribute value when present', () => {
		expect(
			extractAgentName({
				name: 'invoke_agent',
				attributes: { 'gen_ai.agent.name': '  copilot  ' },
			}),
		).toBe('copilot');
	});
	...
	it('parses agent name from span name when attribute is absent', () => {
		expect(
			extractAgentName({ name: 'invoke_agent copilot', attributes: {} }),
		).toBe('copilot');
	});
	...
	it('returns unknown when span name is invoke_agent with no suffix', () => {
		expect(
			extractAgentName({ name: 'invoke_agent ', attributes: {} }),
		).toBe('unknown');
	});
...
describe('extractPlainTextFromContent', () => {
	...
	it('returns undefined for text part with empty text', () => {
		expect(
			extractPlainTextFromContent('[{"type":"text","text":""}]'),
		).toBeUndefined();
	});
	...
	it('returns undefined for invalid/truncated JSON', () => {
		expect(
			extractPlainTextFromContent('[{"type":"text","text":"Fix'),
		).toBeUndefined();
	});
});
