...
describe('buildStandupPrompt', () => {
	...
	it('includes extra context when provided', () => {
		const sessions: AnnotatedSession[] = [
			{ id: 'sess-1', summary: 'Work', source: 'cloud' },
		];
		const result = buildStandupPrompt(
			sessions,
			[],
			[],
			[],
			'Focus on backend changes',
		);
		expect(result).toContain(
			'Additional context: Focus on backend changes',
		);
	});
		...
		const sessions: AnnotatedSession[] = [
			{
				id: 'cloud-1',
				branch: 'main',
				summary: 'Cloud work 1',
				source: 'cloud',
			},
			{
				id: 'cloud-2',
				branch: 'feature/x',
				summary: 'Cloud work 2',
				source: 'cloud',
			},
		];
		const refs: AnnotatedRef[] = [
...
describe('extractFilePath', () => {
	it('extracts filePath from apply_patch args', () => {
		expect(
			extractFilePath('apply_patch', { filePath: '/src/index.ts' }),
		).toBe('/src/index.ts');
	});

	it('extracts path from create tool args', () => {
		expect(extractFilePath('create', { path: '/src/new.ts' })).toBe(
			'/src/new.ts',
		);
	});
	...
	it('extracts filePath from read_file args', () => {
		expect(
			extractFilePath('read_file', { filePath: '/src/index.ts' }),
		).toBe('/src/index.ts');
	});
	...
	it('returns undefined when no path field exists', () => {
		expect(
			extractFilePath('apply_patch', { content: 'hello' }),
		).toBeUndefined();
	});
});
...
describe('extractRefsFromMcpTool', () => {
	it('extracts PR number from pull_request tool', () => {
		const refs = extractRefsFromMcpTool(
	...
	it('extracts issue number from issue tool', () => {
		const refs = extractRefsFromMcpTool('github-mcp-server-issue_read', {
			issue_number: 99,
		});
	...
	it('extracts commit SHA from commit tool', () => {
		const refs = extractRefsFromMcpTool('github-mcp-server-get_commit', {
			sha: 'abc123',
		});
	...
	it('returns empty for unrecognized tool', () => {
		expect(
			extractRefsFromMcpTool('github-mcp-server-list_repos', {}),
		).toEqual([]);
	});
});
...
describe('extractRefsFromTerminal', () => {
	...
	it('returns empty for unrecognized command', () => {
		expect(
			extractRefsFromTerminal({ command: 'npm install' }, 'done'),
		).toEqual([]);
	});
});
...
describe('isGitHubMcpTool', () => {
	it('returns true for github-mcp-server-* tools', () => {
		expect(isGitHubMcpTool('github-mcp-server-pull_request_read')).toBe(
			true,
		);
	});
