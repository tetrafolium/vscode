...
export class SessionSyncStatus extends Disposable {
	...
	constructor(
		...
		this._statusItem = this._register(
			vscode.window.createChatStatusItem('copilot.sessionSyncStatus'),
		);
		this._statusItem.title = statusTitle;

		// Listen for sync state changes
		this._register(
			this._syncStateService.onDidChangeSyncState((state) =>
	...
	private _renderState(state: SessionSyncState): void {
		...
		switch (state.kind) {
			case 'not-enabled':
				...
				this._statusItem.tooltip = l10n.t(
					'Session sync is not enabled. Your data stays local to this device.',
				);
				break;
			...
			case 'disabled-by-policy':
				...
				this._statusItem.tooltip = l10n.t(
					"Session sync is disabled by your organization's policy.",
				);
				break;
			...
			case 'on':
				...
				this._statusItem.tooltip = l10n.t(
					'Your sessions are being synced and available across devices.',
				);
				break;
			...
			case 'up-to-date':
				...
				this._statusItem.detail = `[${l10n.t('Show insights?')}](command:workbench.action.chat.open?%7B%22query%22%3A%22%2Fchronicle%20tips%22%7D)`;
				this._statusItem.tooltip = l10n.t(
					'Your sessions are being synced and available across devices. Click to view them on GitHub.',
				);
				break;
