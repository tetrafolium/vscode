...
describe(
	'get nuget MCP server info using fake CLI',
	{ timeout: 30_000 },
	() => {
		...
		beforeEach(() => {
			...
			fetcherService = new FixtureFetcherService(
				new Map([
					[
						...
						{ fileName: 'nuget-service-index.json', status: 200 },
					],
					[
						'https://api.nuget.org/v3-flatcontainer/basetestpackage.dotnettool/1.0.0/readme',
						{ fileName: 'nuget-readme.md', status: 200 },
					],
				]),
			);
			commandExecutor = new FixtureCommandExecutor(
				new Map([
					[
						'dotnet --version',
						{ stdout: '10.0.100-preview.7.25358.102', exitCode: 0 },
					],
				]),
			);
		...
		it('converts legacy schema version', async () => {
			...
			const actual = nuget.prepareServerJson(
				manifest,
				'CorrectId',
				'0.2.0',
			);
		...
		it('returns package metadata', async () => {
			...
			const result = await nuget.getNuGetPackageMetadata(
				'basetestpackage.DOTNETTOOL',
			);
			...
			if (result.state === 'ok') {
				...
				await expect(result.readme).toMatchFileSnapshot(
					'fixtures/snapshots/nuget-readme.md',
				);
			} else {
				expect.fail();
			}
		...
		it('handles missing package', async () => {
			commandExecutor.fullCommandToResultMap.set(
				'dotnet package search basetestpackage.dotnettool --source https://api.nuget.org/v3/index.json --prerelease --format json',
				{
					fileName: 'dotnet-package-search-does-not-exist.json',
					exitCode: 0,
				},
			);
			const result = await nuget.getNuGetPackageMetadata(
...
describe('mapServerJsonToMcpServer', () => {
	it('handles 2025-07-09 schema version', async () => {
		const manifest = {
			...
			version: '1.0.0',
			packages: [
				{ registry_type: 'nuget', name: 'SomeId', version: '0.1.0' },
			],
		};
	...
	it('defaults to first package without matching type', async () => {
		const manifest = {
			...
			version: '1.0.0',
			packages: [
				{ registryType: 'npm', identifier: 'SomeId', version: '0.1.0' },
			],
		};
		const expected: Omit<IInstallableMcpServer, 'name'> = {
			config: {
