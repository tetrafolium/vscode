...
const RUN_DOTNET_CLI_TESTS =
	!!process.env['CI'] && !process.env['BUILD_ARTIFACTSTAGINGDIRECTORY'];

describe
	.runIf(RUN_DOTNET_CLI_TESTS)
	.skip(
		'get nuget MCP server info using dotnet CLI',
		{ timeout: 30_000 },
		() => {
			...
			it('returns mapped server.json for original schema', async () => {
				...
				if (result.state === 'ok') {
					...
					if (result.getMcpServer) {
						const mcpServer = await result.getMcpServer(
							Promise.resolve(),
						);
						expect(mcpServer).toBeDefined();
						const config = mcpServer!.config as Omit<
							IMcpStdioServerConfiguration,
							'type'
						>;
			...
			it('handles missing dotnet', async () => {
				...
				if (result.state === 'error') {
					...
					expect(result.helpUri).toBe(
						'https://aka.ms/vscode-mcp-install/dotnet',
					);
				} else {
					expect.fail();
				}
