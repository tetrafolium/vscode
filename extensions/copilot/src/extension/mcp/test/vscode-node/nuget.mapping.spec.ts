...
describe('McpManagementService - getMcpServerConfigurationFromManifest', () => {
	...
	describe('NPM Package Tests', () => {
		it('basic NPM package configuration', () => {
			...
			expect(result.mcpServerConfiguration.config.type).toBe(
				McpServerType.LOCAL,
			);
			if (
				...
				expect(result.mcpServerConfiguration.config.command).toBe(
					'npx',
				);
		...
		it('NPM package with environment variables containing variables', () => {
			...
			expect(result.mcpServerConfiguration.inputs?.[0].id).toBe(
				'api_token',
			);
			expect(result.mcpServerConfiguration.inputs?.[0].type).toBe(
				McpServerVariableType.PROMPT,
			);
			expect(result.mcpServerConfiguration.inputs?.[0].description).toBe(
				'Your API token',
			);
			expect(result.mcpServerConfiguration.inputs?.[0].password).toBe(
				true,
			);
		});
		...
		it('environment variable with choices but empty value should create pick input (GitHub issue #266106)', () => {
			...
			expect(result.mcpServerConfiguration.inputs?.[0].default).toBe(
				'prefer',
			);
			expect(result.mcpServerConfiguration.inputs?.[0].type).toBe(
				McpServerVariableType.PICK,
			);
			expect(result.mcpServerConfiguration.inputs?.[0].options).toEqual([
				'disable',
				'prefer',
				'require',
			]);
	...
	describe('Docker Package Tests', () => {
		...
		it('Docker package arguments without values should create input variables (GitHub issue #266106)', () => {
			...
			expect(dbNameInput?.description).toBe(
				'Name of the database to connect to',
			);
	...
	describe('Remote Server Tests', () => {
		it('SSE remote server configuration', () => {
			...
			if (
				...
				expect(result.mcpServerConfiguration.config.url).toBe(
					'http://mcp-fs.anonymous.modelcontextprotocol.io/sse',
				);
				expect(result.mcpServerConfiguration.config.headers).toBe(
					undefined,
				);
			}
		});
		...
		it('remote headers without values should create input variables', () => {
			...
			expect(customInput?.options).toEqual([
				'option1',
				'option2',
				'option3',
			]);
	...
	describe('Edge Cases and Error Handling', () => {
		...
		it('manifest with no matching package type should use first package', () => {
			...
			if (
				...
				expect(result.mcpServerConfiguration.config.command).toBe(
					'uvx',
				); // Python command since that's the package type
				expect(result.mcpServerConfiguration.config.args).toEqual([
					'python-server==1.0.0',
				]);
			}
		...
		it('undefined environment variables should be omitted', () => {
			...
			if (
				...
				expect(result.mcpServerConfiguration.config.env).toBe(
					undefined,
				);
			}
		});
		...
		it('named argument with no name should generate notice', () => {
			...
			expect(
				result.notices[0].includes('Named argument is missing a name'),
			).toBeTruthy();
