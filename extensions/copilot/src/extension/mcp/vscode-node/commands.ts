...
export interface IPendingSetupArgs {
	...
	readme?: string;
	getMcpServer?(
		installConsent: Promise<void>,
	): Promise<Omit<IInstallableMcpServer, 'name'> | undefined>;
}
...
export type ValidatePackageResult =
	...
	| {
			...
			helpUri?: string;
			helpUriLabel?: string;
			errorType: ValidatePackageErrorType;
	  };
...
export class McpSetupCommands extends Disposable {
	...
	constructor(
		...
		super();
		this._register(
			toDisposable(() => this.pendingSetup?.cts.dispose(true)),
		);
		this._register(
			vscode.commands.registerCommand(
				...
				async (args: { name: string }) => {
					...
					try {
						...
						if (
							this.pendingSetup?.pendingArgs.name.toUpperCase() !==
							args.name.toUpperCase()
						) {
							finalState = FlowFinalState.NameMismatch;
							vscode.window.showErrorMessage(
								vscode.l10n.t(
									"Failed to generate MCP server configuration with a matching package name. Expected '{0}' but got '{1}' from generated configuration.",
									args.name,
									this.pendingSetup?.pendingArgs.name ?? '',
								),
							);
							return undefined;
						}
				...
				*/
						this.telemetryService.sendMSFTTelemetryEvent(
							'mcp.setup.flow',
							{
								...
								packageType:
									this.pendingSetup?.validateArgs.type,
								packageName: await this.lowerHash(
									this.pendingSetup?.pendingArgs.name ||
										args.name,
								),
								packageVersion:
									this.pendingSetup?.pendingArgs.version,
							},
							{
								durationMs:
									this.pendingSetup?.stopwatch.elapsed() ??
									-1,
							},
						);
					}
		...
		this._register(
			vscode.commands.registerCommand(
				...
				async (
					args: IValidatePackageArgs,
				): Promise<ValidatePackageResult> => {
					const sw = new StopWatch();
					const result =
						await McpSetupCommands.validatePackageRegistry(
							args,
							this.logService,
							this.fetcherService,
						);
			...
			*/
					...
					return result.state === 'ok'
						...
						: {
								...
								helpUriLabel: result.helpUriLabel,
								errorType: result.errorType,
							};
				},
			),
	...
	private async enqueuePendingSetup(
		validateArgs: IValidatePackageArgs,
		pendingArgs: IPendingSetupArgs,
		sw: StopWatch,
	) {
		...
		const pickRef = new McpPickRef(
			raceCancellation(canPrompt.p, cts.token),
		);
		...
		const done = (async () => {
			...
			if (pendingArgs.getMcpServer) {
				...
				try {
					...
					this.logService
						.warn(`Unable to fetch MCP server configuration for ${validateArgs.type} package ${pendingArgs.name}@${pendingArgs.version}. Configuration will be generated from the package README.
Error: ${error}`);
				}
				...
				if (mcpServer) {
					return {
						...
						server: mcpServer.config as Omit<
							IMcpStdioServerConfiguration,
							'type'
						>,
			...
			const mcpLoop = this.instantiationService.createInstance(
				McpToolCallingLoop,
				{
			...
			if (
				toolCallLoopResult.response.type !==
				ChatFetchResponseType.Success
			) {
				vscode.window.showErrorMessage(
					vscode.l10n.t(
						'Failed to generate MCP configuration for {0}: {1}',
						validateArgs.name,
						toolCallLoopResult.response.reason,
					),
				);
				return undefined;
			}

			const { name, ...server } = mapFindFirst(
				extractCodeBlocks(toolCallLoopResult.response.value),
				(block) => {
					try {
						...
						if (
							j &&
							typeof j === 'object' &&
							j.hasOwnProperty('mcpServers')
						) {
							const [name, obj] = Object.entries(
								j.mcpServers,
							)[0] as [string, object];
							return { ...obj, name };
						}
			...
			const extracted = cloneAndChange(server, (value) => {
				if (typeof value === 'string') {
					const fromInput = pickRef.picks.find(
						(p) => p.choice === value,
					);
					if (fromInput) {
						inputs.push({
	...
	public static async validatePackageRegistry(
		args: { type: PackageType; name: string },
		...
		try {
			if (args.type === 'npm') {
				const response = await fetcherService.fetch(
					`https://registry.npmjs.org/${encodeURIComponent(args.name)}`,
					{ method: 'GET', callSite: 'mcp-npm-registry' },
				);
				if (!response.ok) {
					return {
						state: 'error',
						...
						error: vscode.l10n.t(
							'Package {0} not found in npm registry',
							args.name,
						),
					};
				}
				...
				const response = await fetcherService.fetch(
					`https://pypi.org/pypi/${encodeURIComponent(args.name)}/json`,
					{ method: 'GET', callSite: 'mcp-pypi-registry' },
				);
				if (!response.ok) {
					return {
						state: 'error',
						errorType: ValidatePackageErrorType.NotFound,
						error: vscode.l10n.t(
							'Package {0} not found in PyPI registry',
							args.name,
						),
					};
				}
				...
				const publisher =
					data.info?.author || data.info?.author_email || 'unknown';
				...
				const nuGetMcpSetup = new NuGetMcpSetup(
					logService,
					fetcherService,
				);
				...
				const response = await fetcherService.fetch(
					`https://hub.docker.com/v2/repositories/${encodeURIComponent(namespace)}/${encodeURIComponent(repository)}`,
					{ method: 'GET', callSite: 'mcp-docker-registry' },
				);
				if (!response.ok) {
					return {
						state: 'error',
						errorType: ValidatePackageErrorType.NotFound,
						error: vscode.l10n.t(
							'Docker image {0} not found in Docker Hub registry',
							args.name,
						),
					};
				}
			...
			return {
				state: 'error',
				error: vscode.l10n.t(
					'Unsupported package type: {0}',
					args.type,
				),
				errorType: ValidatePackageErrorType.UnknownPackageType,
			};
		} catch (error) {
			return {
				state: 'error',
				error: vscode.l10n.t(
					'Error querying package: {0}',
					(error as Error).message,
				),
				errorType: ValidatePackageErrorType.UnhandledError,
			};
		}
	}
}
