...
import {
	buildRemoteIndexCommandId,
	enableExternalIngestCommandId,
} from './commands';

const reauthenticateCommandId = '_copilot.workspaceIndex.signInAgain';

const codebaseSemanticSearchDocsLink =
	'https://aka.ms/vscode-copilot-workspace-remote-index';
...
const externalIngestPolicyDetail = t(
	"External ingest is disabled by your organization's policy. [Learn more]({0})",
	externalIngestPolicyLink,
);
const externalIngestPolicyNoReposDetail = t(
	"External ingest is disabled by your organization's policy, and no external repositories were found. [Learn more]({0})",
	externalIngestPolicyLink,
);
const externalIngestPolicyAvailableDetail = t(
	"External ingest is disabled by your organization's policy. Results may be incomplete or out of date. [Learn more]({0})",
	externalIngestPolicyLink,
);
...
const enableExternalIngestDetail = t(
	'External ingest is disabled in this workspace. [Enable external ingest?]({0} "{1}")',
	enableExternalIngestCommandLink,
	enableExternalIngestCommandTitle,
);
const enableExternalIngestNoReposDetail = t(
	'External ingest is disabled in this workspace, and no external repositories were found. [Enable external ingest?]({0} "{1}")',
	enableExternalIngestCommandLink,
	enableExternalIngestCommandTitle,
);
...
export class MockWorkspaceIndexStateReporter
	extends Disposable
	implements WorkspaceIndexStateReporter
{
...
export class ChatStatusWorkspaceIndexingStatus extends Disposable {
	...
	constructor(
		@IWorkspaceChunkSearchService
		workspaceChunkSearch: IWorkspaceChunkSearchService,
		...
		this._statusItem = this._register(
			vscode.window.createChatStatusItem('copilot.workspaceIndexStatus'),
		);
		this._statusItem.title = statusTitle;

		this._register(
			this._statusReporter.onDidChangeIndexState(() =>
	...
	private async _updateStatusItem(): Promise<void> {
		...
		this._logService.trace(
			`ChatStatusWorkspaceIndexingStatus::updateStatusItem(id=${id}): starting`,
		);
		...
		if (id !== this.currentUpdateRequestId) {
			this._logService.trace(
				`ChatStatusWorkspaceIndexingStatus::updateStatusItem(id=${id}): skipping`,
			);
			return;
		}
		...
		switch (state.remoteIndexState.status) {
			...
			case 'loaded': {
				const externalIngestDisabledByPolicy =
					state.remoteIndexState.externalIngestEnablement ===
					ExternalIngestEnablement.DisabledByPolicy;
				const externalIngestDisabledInWorkspace =
					state.remoteIndexState.externalIngestEnablement ===
				...
				if (
					state.remoteIndexState.repos.some(
						(repo) =>
				...
				if (
					...
					state.remoteIndexState.externalIngestState?.status ===
				...
				if (
					...
					!state.remoteIndexState.hasPromptedForExternalIngest
				) {
					return this._writeStatusItem({
						primary: {
							message: t`Disabled`,
						...
						details: {
							message: noRepos
								? enableExternalIngestNoReposDetail
								: enableExternalIngestDetail,
							busy: false,
						},
				...
				const readyRepos = state.remoteIndexState.repos.filter(
					(repo) => repo.status === CodeSearchRepoStatus.Ready,
				);
				const notAuthorizedRepos = state.remoteIndexState.repos.filter(
					(repo) =>
				...
				const errorRepos = state.remoteIndexState.repos.filter(
					(repo) =>
						repo.status ===
				...
				if (
					...
					(!state.remoteIndexState.externalIngestState &&
						...
						state.remoteIndexState.repos.every(
							(repo) =>
					...
					return this._writeStatusItem({
						primary: {
							message: t`Ready`,
							icon: externalIngestDisabledInWorkspace
								? warningCodicon
								: checkCodicon,
						},
						details: externalIngestDisabledInWorkspace
							? {
									message:
										externalIngestDisabledInWorkspaceDetail,
									busy: false,
								}
							: undefined,
				...
				if (externalIngestDisabledInWorkspace) {
					return this._writeStatusItem({
						...
						details: {
							message: noRepos
								? externalIngestDisabledInWorkspaceNoReposDetail
								: externalIngestDisabledInWorkspaceDetail,
							busy: false,
						},
						tooltip: t`External ingest is disabled in this workspace.`,
				...
				if (
					typeof state.remoteIndexState.externalIngestState !==
					'undefined'
				) {
					return this._writeStatusItem({
						primary: {
		...
		this._writeStatusItem({
			primary: {
				message:
					externalIngestDisabledByPolicy ||
					externalIngestDisabledInWorkspace
						? t`Disabled`
						: t`Not available`,
				icon:
				...
				externalIngestDisabledInWorkspace
					? {
							message: externalIngestDisabledByPolicy
								? noRepos
									? externalIngestPolicyNoReposDetail
									: externalIngestPolicyDetail
								: noRepos
									? externalIngestDisabledInWorkspaceNoReposDetail
									: externalIngestDisabledInWorkspaceDetail,
							busy: false,
	...
	private _writeStatusItem(values: ChatStatusItemState | undefined) {
		this._logService.trace(
			`ChatStatusWorkspaceIndexingStatus::_writeStatusItem()`,
		);
	...
	private registerCommands(): IDisposable {
		...
		disposables.add(
			vscode.commands.registerCommand(
				reauthenticateCommandId,
				async (repo: ResolvedRepoRemoteInfo | undefined) => {
					if (!repo) {
						return;
					}

					return this._codeSearchAuthService.tryReauthenticating(
						repo,
					);
				},
			),
