...
export const buildRemoteIndexCommandId =
	'github.copilot.buildRemoteWorkspaceIndex';
export const enableExternalIngestCommandId =
	'github.copilot.enableExternalIngest';
export const deleteExternalIngestWorkspaceIndexCommandId =
	'github.copilot.deleteExternalIngestWorkspaceIndex';
...
export function register(accessor: ServicesAccessor): IDisposable {
			...
			onlyRunOneAtATime(async () => {
				await vscode.window.withProgress(
					...
					async (progress, token) => {
						const triggerResult =
							await workspaceChunkSearch.triggerIndexing(
								'manual',
								(message) => progress.report({ message }),
								new TelemetryCorrelationId(
									'BuildRemoteIndexCommand',
								),
								token,
							);
						...
						if (triggerResult.isError()) {
							if (
								...
								TriggerRemoteIndexingError.alreadyIndexed.id
							) {
								vscode.window.showWarningMessage(
									t`Could not build codebase semantic index. ` +
										'\n\n' +
						...
						vscode.window.showInformationMessage(
							t`Codebase semantic index ready to use.`,
						);
					},
				);
			}),
	...
	disposableStore.add(
		vscode.commands.registerCommand(
			...
			onlyRunOneAtATime(async () => {
				const enabled =
					await workspaceChunkSearch.enableExternalIngest();
				if (enabled) {
					vscode.window.showInformationMessage(
						t`External ingest enabled for this workspace.`,
					);
					return;
				}
	...
	disposableStore.add(
		vscode.commands.registerCommand(
			deleteExternalIngestWorkspaceIndexCommandId,
			onlyRunOneAtATime(async () => {
				await vscode.window.withProgress(
					{
						location: vscode.ProgressLocation.Window,
					...
					async () => {
						...
						vscode.window.showInformationMessage(
							t`External ingest index deleted.`,
						);
					},
				);
			}),
		...
		vscode.commands.registerCommand(
			...
			async () => {
				const document = await vscode.workspace.openTextDocument({
					language: 'markdown',
					content: 'Collecting codebase index diagnostics...\n',
				});
