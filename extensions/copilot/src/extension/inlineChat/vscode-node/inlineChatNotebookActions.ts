...
export class NotebookExectionStatusBarItemProvider
	implements vscode.NotebookCellStatusBarItemProvider
{
	...
	async provideCellStatusBarItems(
		...
		const coreQuickFix = vscode.workspace
			.getConfiguration('notebook')
			.get<boolean>('cellFailureDiagnostics');
		if (coreQuickFix) {
			return [];
		}
		...
		if (cell.kind === vscode.NotebookCellKind.Markup) {
			...
			const enabled = vscode.workspace
				.getConfiguration('notebook.experimental')
				.get<boolean>('cellChat');
		...
		try {
			...
			const joinedMessage = [err.name, err.message]
				.filter(Boolean)
				.join(': ')
