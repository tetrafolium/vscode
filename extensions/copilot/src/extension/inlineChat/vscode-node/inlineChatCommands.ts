...
export function registerInlineChatCommands(
	...
	const doExplain = async (arg0: any, fromPalette?: true) => {
		...
		if (typeof arg0 === 'string' && arg0) {
			...
			const emptySelection = CurrentSelection.getCurrentSelection(
				tabsAndEditorsService,
				true,
			);
			if (emptySelection) {
				const severeDiagnostics = vscode.languages.getDiagnostics(
					emptySelection.activeDocument.uri,
				);
				const diagnosticsInSelection = severeDiagnostics.filter(
					(d) => !!d.range.intersection(emptySelection.range),
				);
				...
				if (filteredDiagnostics.length) {
					message +=
						QuickFixesProvider.getDiagnosticsAsText(
							severeDiagnostics,
						);
			...
			const selection = CurrentSelection.getCurrentSelection(
				tabsAndEditorsService,
			);
			if (!explainingDiagnostics && selection) {
				message += explainIntentPromptSnippet;
				selectedText = formatSelection({
					...
					const selectedScope = await SymbolAtCursor.getSelectedScope(
						...
						parserService,
						{
							document: TextDocumentSnapshot.create(
								emptySelection.activeDocument,
							),
							selection: emptySelection.range,
						},
					);
					if (
						selectedScope &&
						selectedScope.symbolAtCursorState &&
						selectedScope.symbolAtCursorState.codeAtCursor
					) {
						message += explainIntentPromptSnippet;
						const languageId =
							selectedScope.symbolAtCursorState.document
								.languageId ?? '';
						selectedText = formatSelection({
							languageId,
							selectedText:
		...
		if (
			activeDocumentUri &&
			selectedText &&
			!(await ignoreService.isCopilotIgnored(activeDocumentUri))
		) {
	...
	const doContinueInInlineChat = async (
		commentThread: vscode.CommentThread,
	) => {
		const comment = reviewService.findReviewComment(commentThread);
		if (!comment) {
...
${message}`,
	...
	const markReviewHelpful = async (comment: vscode.Comment) => {
		...
		if (reviewComment) {
			const commentThread =
				reviewService.findCommentThread(reviewComment);
			if (commentThread) {
				commentThread.contextValue = updateContextValue(
	...
	if (
		typeof extensionMode === 'number' &&
		(extensionMode !== vscode.ExtensionMode.Test || isScenarioAutomation)
	) {
		reviewService.updateContextValues();
	}
	const goToNextReview = (
		currentThread: vscode.CommentThread | undefined,
		direction: number,
	) => {
		let newComment: ReviewComment | undefined;
		if (currentThread) {
			const reviewComment =
				reviewService.findReviewComment(currentThread);
			if (!reviewComment) {
				return;
			}
			...
			const newIndex =
				(currentIndex + direction + reviewComments.length) %
				reviewComments.length;
			newComment = reviewComments[newIndex];
		} else {
			const reviewComments = reviewService.getReviewComments();
			newComment =
				reviewComments[direction > 0 ? 0 : reviewComments.length - 1];
		}
		const newThread =
			newComment && reviewService.findCommentThread(newComment);
		if (!newThread) {
			return;
		}
		if (direction !== 0) {
	...
	const doFix = () => {
		...
		const diagnostics = vscode.languages
			.getDiagnostics(activeDocument.document.uri)
			.filter((diagnostic) => {
			...
			.join(', ');
		return vscode.commands.executeCommand('vscode.editorChat.start', {
			message: `/${Intent.Fix} ${diagnostics}`,
			autoSend: true,
	...
	const doGenerateAltText = async (arg: unknown) => {
		if (
			...
			typeof arg === 'object' &&
			'isUrl' in arg &&
			'resolvedImagePath' in arg &&
			typeof arg.resolvedImagePath === 'string' &&
			'type' in arg
		) {
			const baseQuery =
			...
			const fullQuery =
				arg.type === 'generate'
					? baseQuery
					: `Refine the existing alt text for clarity and usefulness for screen readers. ${baseQuery}`;

			const uri = arg.isUrl
				? URI.parse(arg.resolvedImagePath)
				: URI.file(arg.resolvedImagePath);
			return vscode.commands.executeCommand('vscode.editorChat.start', {
				message: fullQuery,
				attachments: [uri],
				autoSend: true,
	...
	disposables.add(
		vscode.commands.registerCommand(
			...
			() => doExplain(undefined, true),
		),
	);
	disposables.add(
		vscode.commands.registerCommand('github.copilot.chat.review', () =>
			instaService
				.createInstance(ReviewSession)
				.review('selection', vscode.ProgressLocation.Notification),
		),
	);
	disposables.add(
		vscode.commands.registerCommand(
			...
			() =>
				instaService
					.createInstance(ReviewSession)
					.review('index', vscode.ProgressLocation.Notification),
		),
	);
	disposables.add(
		vscode.commands.registerCommand(
	...
	disposables.add(
		vscode.commands.registerCommand(
			...
			() =>
				instaService
					.createInstance(ReviewSession)
					.review('all', vscode.ProgressLocation.Notification),
		),
	);
	disposables.add(
		vscode.commands.registerCommand(
			...
			(resource: vscode.SourceControlResourceState) => {
				return instaService
					.createInstance(ReviewSession)
					.review(
						{ group: 'index', file: resource.resourceUri },
						vscode.ProgressLocation.Notification,
					);
			},
		),
		...
		vscode.commands.registerCommand(
			...
			(resource: vscode.SourceControlResourceState) => {
				return instaService
					.createInstance(ReviewSession)
					.review(
						{ group: 'workingTree', file: resource.resourceUri },
						vscode.ProgressLocation.Notification,
					);
			},
		),
	...
	disposables.add(
		vscode.commands.registerCommand(
			...
			(commentThread: vscode.CommentThread) =>
				doApplyReview(commentThread, true),
		),
	);
	disposables.add(
		vscode.commands.registerCommand(
	...
	disposables.add(
		vscode.commands.registerCommand(
			...
			(commentThread: vscode.CommentThread) =>
				doDiscardReview(commentThread, true),
		),
	);
	disposables.add(
		vscode.commands.registerCommand(
	...
	disposables.add(
		vscode.commands.registerCommand(
			...
			(thread) => goToNextReview(thread, -1),
		),
	);
	disposables.add(
		vscode.commands.registerCommand(
	...
	disposables.add(
		vscode.commands.registerCommand(
			...
			(thread) => goToNextReview(thread, 0),
		),
	);
	disposables.add(
		vscode.commands.registerCommand(
	...
	disposables.add(
		vscode.commands.registerCommand('github.copilot.chat.fix', doFix),
	);
	disposables.add(
		vscode.commands.registerCommand(
	...
	disposables.add(
		vscode.languages.registerCodeActionsProvider(
			'*',
			instaService.createInstance(QuickFixesProvider),
			{
				providedCodeActionKinds:
					QuickFixesProvider.providedCodeActionKinds,
			},
		),
	);
	disposables.add(
		vscode.languages.registerCodeActionsProvider(
			'*',
			instaService.createInstance(RefactorsProvider),
			{
				providedCodeActionKinds:
					RefactorsProvider.providedCodeActionKinds,
			},
		),
	);
	disposables.add(
...
function fetchSuggestion(
	...
	if (!comment || comment.suggestion || comment.skipSuggestion) {
		if (
			...
			thread.contextValue?.includes('hasNoSuggestion')
		) {
			thread.contextValue = updateContextValue(
				thread.contextValue,
				'hasSuggestion',
				'hasNoSuggestion',
			);
	...
	comment.suggestion = (async () => {
		...
		const textEditor =
			...
			(await vscode.window.showTextDocument(document.document, {
				preserveFocus: true,
				preview: false,
			}));
		...
		const request: vscode.ChatRequest = {
			...
			location2: new vscode.ChatRequestEditorData(
				textEditor,
				document.document,
				selection,
				selection,
		...
		const stream = new ChatResponseStreamImpl(
			(value) => {
				if (
					value instanceof vscode.ChatResponseTextEditPart &&
		...
		const requestHandler = instantiationService.createInstance(
...
function formatSelection(selection: {
	...
	const fileContext = selection.fileName
		? `From the file: ${path.basename(selection.fileName)}\n`
		: '';
	const { trimmedLines } = trimCommonLeadingWhitespace(
		selection.selectedText.split(/\r?\n/g),
	);
