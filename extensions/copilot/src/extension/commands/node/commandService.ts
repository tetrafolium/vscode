...
export const ICommandService =
	createServiceIdentifier<ICommandService>('ICommandService');
...
export class CommandServiceImpl implements ICommandService {
	...
	public getCommands(location: ChatLocation): CommandDetails[] {
		return this.intentService
			...
			.filter(
				(candidate) =>
					!candidate.commandInfo ||
					!candidate.commandInfo.hiddenFromUser,
			)
			.map(
				(intent) =>
					({
						...
						toolEquivalent: intent.commandInfo?.toolEquivalent,
					}) satisfies CommandDetails,
			);
	}
