 ...
 */
export async function parseSettingsAndCommands(
	workbenchService: IWorkbenchService,
	codeBlock: string,
): Promise<VSCodeParticipantMetadata[]> {
	...
	for (const block of parsedCodeBlock) {
		...
		try {
			const removeTrailingCommas = block.code.replace(
				/,\s*([\]}])/g,
				'$1',
			);
		...
		if (hasSettings) {
			...
			const settingsQuery = parsed.reduce(
				(acc: string, item: ParsedItem) => {
					if (item.details) {
		...
		if (hasCommands) {
			const item = parsed[0];
			if (
				item.details?.key === 'workbench.extensions.search' ||
				item.details?.key === 'workbench.extensions.installExtension'
			) {
				const args = (
					Array.isArray(item.details.value)
						? item.details.value
						: [item.details.value]
				...
				const allCommandsNoFilter =
					await workbenchService.getAllCommands(
						/* filterByPreCondition */ false,
					);
				const commandItem = allCommandsNoFilter.find(
					(commandItem) => commandItem.command === item.details?.key,
				);
				if (!commandItem) {
				...
				parsedMetadata.push({
					commandToRun: {
						...
						title:
							parsedMetadata.length > 1
								? l10n.t('Show "{0}"', commandItem.label ?? '')
								: l10n.t('Show in Command Palette'),
					},
				});
				return parsedMetadata;
			}
