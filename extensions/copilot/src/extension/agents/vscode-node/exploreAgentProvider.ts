 ...
 */
export class ExploreAgentProvider
	extends Disposable
	implements vscode.ChatCustomAgentProvider
{
	...
	constructor(
		...
		this._register(
			this._configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration('chat.exploreAgent.defaultModel') ||
					e.affectsConfiguration(
						ConfigKey.ExploreAgentModel.fullyQualifiedId,
					)
				) {
	...
	private async _writeCacheFile(content: string): Promise<vscode.Uri> {
		...
		const fileUri = vscode.Uri.joinPath(
			cacheDir,
			ExploreAgentProvider.AGENT_FILENAME,
		);
		...
		this._logService.trace(
			`[ExploreAgentProvider] Wrote agent file: ${fileUri.toString()}`,
		);
		return fileUri;
	}
...
Remember: Your goal is searching efficiently through MAXIMUM PARALLELISM to report concise and clear answers.`;
	...
	private _buildCustomizedConfig(): AgentConfig {
		...
		const extModel = this._configurationService.getConfig(
			ConfigKey.ExploreAgentModel,
		);
		const model: string | readonly string[] =
			coreDefaultModel || extModel || EXPLORE_AGENT_FALLBACK_MODELS;
