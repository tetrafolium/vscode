...
export interface IGitHubOrgChatResourcesService extends IDisposable {
	...
	startPolling(
		intervalMs: number,
		callback: (orgName: string) => Promise<void>,
	): IDisposable;
	...
	writeCacheFile(
		...
		filename: string,
		content: string,
		options?: { checkForChanges?: boolean },
	): Promise<boolean>;
...
export const IGitHubOrgChatResourcesService =
	createDecorator<IGitHubOrgChatResourcesService>('IGitHubPromptFileService');
...
export class GitHubOrgChatResourcesService
	extends Disposable
	implements IGitHubOrgChatResourcesService
{
	...
	constructor(
		...
		this._register(
			this.workspaceService.onDidChangeWorkspaceFolders(() => {
				this.logService.trace(
					'[GitHubOrgChatResourcesService] Workspace folders changed, invalidating cached org name',
				);
		...
		this._register(
			this.authService.onDidAuthenticationChange(() => {
				this.logService.trace(
					'[GitHubOrgChatResourcesService] Authentication changed, invalidating cached org name',
				);
				this._cachedPreferredOrgName = undefined;
			}),
		);
	...
	async getPreferredOrganizationName(): Promise<string | undefined> {
		if (!this._cachedPreferredOrgName) {
			this._cachedPreferredOrgName =
				this.computePreferredOrganizationName();
		}
		return this._cachedPreferredOrgName;
	}

	private async computePreferredOrganizationName(): Promise<
		string | undefined
	> {
		...
		if (!currentUser) {
			this.logService.trace(
				'[GitHubOrgChatResourcesService] User is not signed in',
			);
			return undefined;
		}
		...
		this.logService.trace(
			`[GitHubOrgChatResourcesService] Workspace organization: ${workspaceOrg ?? 'none'}`,
		);
		if (workspaceOrg) {
			this.logService.trace(
				`[GitHubOrgChatResourcesService] Using workspace organization: ${workspaceOrg}`,
			);
			return workspaceOrg;
		}
		...
		const copilotOrganizations =
			this.authService.copilotToken?.organizationLoginList ?? [];
		this.logService.trace(
			`[GitHubOrgChatResourcesService] Copilot organizations: ${JSON.stringify(copilotOrganizations)}`,
		);
		if (copilotOrganizations.length > 0) {
			const copilotOrg = copilotOrganizations[0];
			this.logService.trace(
				`[GitHubOrgChatResourcesService] Using Copilot sign-in organization: ${copilotOrg}`,
			);
			return copilotOrg;
		}
		...
		try {
			userOrganizations = await this.octoKitService.getUserOrganizations(
				{},
				1,
			);
			this.logService.trace(
				`[GitHubOrgChatResourcesService] User organizations: ${JSON.stringify(userOrganizations)}`,
			);
			if (userOrganizations.length === 0) {
				this.logService.trace(
					'[GitHubOrgChatResourcesService] No organizations found for user',
				);
				return undefined;
			}
		} catch (error) {
			this.logService.error(
				`[GitHubOrgChatResourcesService] Error getting user organizations: ${error}`,
			);
			return undefined;
		}
		this.logService.trace(
			`[GitHubOrgChatResourcesService] Falling back to first user organization: ${userOrganizations[0]}`,
		);
	 ...
	 */
	private async getWorkspaceRepositoryOrganization(): Promise<
		string | undefined
	> {
		...
		try {
			...
			const repoInfo = await this.gitService.getRepositoryFetchUrls(
				workspaceFolders[0],
			);
			...
			for (const fetchUrl of repoInfo.remoteFetchUrls) {
				...
				if (repoId) {
					this.logService.trace(
						`[GitHubOrgChatResourcesService] Found GitHub repo: ${repoId.org}/${repoId.repo}`,
					);
					return repoId.org;
				}
			}
			...
			this.logService.trace(
				`[GitHubOrgChatResourcesService] Error getting workspace repository: ${error}`,
			);
		}
			...
			try {
				...
				if (orgName) {
					try {
						...
						this.logService.error(
							`[GitHubOrgChatResourcesService] Error in polling callback: ${error}`,
						);
					}
				}
			} finally {
	...
	async readCacheFile(
		...
		try {
			...
			this.logService.error(
				`[GitHubOrgChatResourcesService] Cache file not found: ${filename}`,
			);
			return undefined;
		}
	}
	...
	async writeCacheFile(
		...
		if (options?.checkForChanges) {
			try {
				...
				if (existingText !== content) {
					this.logService.trace(
						`[GitHubOrgChatResourcesService] Skipped writing cache file: ${fileUri.toString()}`,
					);
					hasChanges = true;
				} else {
		...
		this.logService.trace(
			`[GitHubOrgChatResourcesService] Wrote cache file: ${fileUri.toString()}`,
		);
		return hasChanges;
	}
		...
		try {
			...
			for (const [filename, fileType] of files) {
				if (
					...
					isValidFile(type, filename) &&
					!exclude?.has(filename)
				) {
					await this.fileSystem.delete(
						vscode.Uri.joinPath(cacheDir, filename),
					);
					this.logService.trace(
						`[GitHubOrgChatResourcesService] Deleted cache file: ${filename}`,
					);
				}
			}
		} catch {
	...
	async listCachedFiles(
		...
		try {
			...
			this.logService.trace(
				`[GitHubOrgChatResourcesService] Cache directory does not exist: ${cacheDir.toString()}`,
			);
		}
