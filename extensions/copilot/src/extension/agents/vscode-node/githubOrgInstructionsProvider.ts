...
export class GitHubOrgInstructionsProvider
	extends Disposable
	implements vscode.ChatInstructionsProvider
{
	...
	constructor(
		...
		this._register(
			this.githubOrgChatResourcesService.startPolling(
				REFRESH_INTERVAL_MS,
				this.pollInstructions.bind(this),
			),
		);
	}
		...
		try {
			...
			if (!orgId) {
				this.logService.trace(
					'[GitHubOrgInstructionsProvider] No organization available for providing agents',
				);
				return [];
			}
			...
			if (token.isCancellationRequested) {
				this.logService.trace(
					'[GitHubOrgInstructionsProvider] provideCustomAgents was cancelled',
				);
				return [];
			}
			...
			this.logService.error(
				`[GitHubOrgInstructionsProvider] Error reading from cache: ${error}`,
			);
			return [];
		}
	}
	...
	private async pollInstructions(orgId: string): Promise<void> {
		try {
			const instructions =
				await this.octoKitService.getOrgCustomInstructions(orgId, {});
			if (!instructions) {
				...
				this.logService.trace(
					`[GitHubOrgInstructionsProvider] No custom instructions found for org ${orgId}`,
				);
				return;
			}
...
${instructions}`;
			...
			if (!contentChanged) {
				this.logService.trace(
					`[GitHubOrgInstructionsProvider] No changes detected in cache for org ${orgId}`,
				);
				return;
			}
			...
			this.logService.error(
				`[GitHubOrgInstructionsProvider] Error polling for instructions: ${error}`,
			);
		}
	}
}
