...
export class PromptFileContribution
	extends Disposable
	implements IExtensionContribution
{
	...
	constructor(
		...
		if ('registerCustomAgentProvider' in vscode.chat) {
			const editModeProviderRegistration = this._register(
				new MutableDisposable<vscode.Disposable>(),
			);
			const editModeHiddenSetting = 'chat.editMode.hidden';
			const updateEditModeProvider = () => {
				const isEditModeHidden =
					configurationService.getNonExtensionConfig<boolean>(
						editModeHiddenSetting,
					);
				if (!isEditModeHidden) {
					if (!editModeProviderRegistration.value) {
						editModeProviderRegistration.value =
			...
			if (
				configurationService.getConfig(
					ConfigKey.EnableOrganizationCustomAgents,
				)
			) {
				const githubOrgAgentProvider: vscode.ChatCustomAgentProvider =
					instantiationService.createInstance(
						new SyncDescriptor(GitHubOrgCustomAgentProvider),
					);
				this._register(
					vscode.chat.registerCustomAgentProvider(
						githubOrgAgentProvider,
					),
				);
			}
			...
			const planProvider =
				instantiationService.createInstance(PlanAgentProvider);
			this._register(
				vscode.chat.registerCustomAgentProvider(planProvider),
			);

			// Register Ask agent provider for read-only Q&A mode
			const askProvider =
				instantiationService.createInstance(AskAgentProvider);
			this._register(
				vscode.chat.registerCustomAgentProvider(askProvider),
			);

			// Register Explore agent provider for code research subagent
			const exploreProviderRegistration = this._register(
				new MutableDisposable<vscode.Disposable>(),
			);
			const updateExploreProvider = () => {
				const isEnabled = configurationService.getExperimentBasedConfig(
				...
				if (isEnabled) {
					if (!exploreProviderRegistration.value) {
						const provider =
							instantiationService.createInstance(
								ExploreAgentProvider,
							);
						const registration =
							vscode.chat.registerCustomAgentProvider(provider);
						exploreProviderRegistration.value = combinedDisposable(
							registration,
							provider,
						);
					}
				...
				configurationService.onDidChangeConfiguration((e) => {
					if (
						e.affectsConfiguration(
							ConfigKey.ExploreAgentEnabled.fullyQualifiedId,
						)
					) {
						updateExploreProvider();
					}
		...
		if ('registerInstructionsProvider' in vscode.chat) {
			...
			if (
				configurationService.getConfig(
					ConfigKey.EnableOrganizationInstructions,
				)
			) {
				const githubOrgInstructionsProvider: vscode.ChatInstructionsProvider =
					instantiationService.createInstance(
						new SyncDescriptor(GitHubOrgInstructionsProvider),
					);
				this._register(
					vscode.chat.registerInstructionsProvider(
						githubOrgInstructionsProvider,
					),
				);
			}
		}
