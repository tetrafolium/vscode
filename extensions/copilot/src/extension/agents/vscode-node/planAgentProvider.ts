 ...
 */
export class PlanAgentProvider
	extends Disposable
	implements vscode.ChatCustomAgentProvider
{
			...
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(
						ConfigKey.PlanAgentAdditionalTools.fullyQualifiedId,
					) ||
					e.affectsConfiguration(
						ConfigKey.Deprecated.PlanAgentModel.fullyQualifiedId,
					) ||
					e.affectsConfiguration('chat.planAgent.defaultModel') ||
					e.affectsConfiguration(
						ConfigKey.ImplementAgentModel.fullyQualifiedId,
					) ||
					e.affectsConfiguration(
	...
	private async writeCacheFile(content: string): Promise<vscode.Uri> {
		...
		const fileUri = vscode.Uri.joinPath(
			cacheDir,
			PlanAgentProvider.AGENT_FILENAME,
		);
		...
		this.logService.trace(
			`[PlanAgentProvider] Wrote agent file: ${fileUri.toString()}`,
		);
		return fileUri;
	}
...
- The plan MUST be presented to the user, don't just mention the plan file.
	...
	private buildCustomizedConfig(): AgentConfig {
		const additionalTools = this.configurationService.getConfig(
		...
		const isSearchSubagentEnabled =
			this.configurationService.getExperimentBasedConfig(
				ConfigKey.Advanced.SearchSubagentToolEnabled,
				this.experimentationService,
			);
		const coreDefaultModel =
			this.configurationService.getNonExtensionConfig<string>(
		...
		const implementAgentModelOverride = this.configurationService.getConfig(
		...
		const startImplementationHandoff: AgentHandoff = {
			...
			send: true,
			...(implementAgentModelOverride
				? { model: implementAgentModelOverride }
				: {}),
		};
		...
		return {
			...
			handoffs: [
				...
				openInEditorHandoff,
				...(BASE_PLAN_AGENT_CONFIG.handoffs ?? []),
			],
			body: PlanAgentProvider.buildAgentBody(
				isExploreEnabled,
				isSearchSubagentEnabled,
			),
