...
export const ILanguageModelServer =
...
export class LanguageModelServer implements ILanguageModelServer {
	...
	protected readonly requestHandlers = new Map<
		string,
		{
			method: string;
			handler: (
	...
	private createServer(): http.Server {
		return http.createServer(async (req, res) => {
			...
			if (req.method === 'POST') {
				const adapterFactory = this.getAdapterFactoryForPath(
					req.url || '',
				);
				if (adapterFactory) {
					try {
						...
						if (authKey !== this.config.nonce) {
							this.logService.trace(
								`[LanguageModelServer] Invalid auth key`,
							);
							res.writeHead(401, {
	...
	private getAdapterFactoryForPath(
		url: string,
	): IProtocolAdapterFactory | undefined {
		...
		try {
			...
			res.on('close', () => {
				if (!requestComplete) {
					this.logService.info(
						`[LanguageModelServer] Client disconnected before request complete`,
					);
				}
			...
			try {
				...
				const context: IStreamingContext = {
					...
					endpoint: {
						...
						modelMaxPromptTokens:
							selectedEndpoint.modelMaxPromptTokens,
					},
				};
				...
				if (adapter.generateInitialEvents) {
					const initialEvents =
						adapter.generateInitialEvents(context);
				...
				const fetchResult = await selectedEndpoint.makeChatRequest2(
					{
						...
						finishedCb: async (_fullText, _index, delta) => {
							...
							if (delta.text) {
								...
								for (const event of adapter.formatStreamResponse(
									textData,
									context,
								)) {
									res.write(
							...
							if (
								...
								delta.copilotToolCalls.length > 0
							) {
	...
	private selectEndpoint(
		...
		if (requestedModel) {
			...
			if (
				!selectedEndpoint &&
				requestedModel.startsWith('claude-haiku-4')
			) {
				selectedEndpoint =
					endpoints.find((e) =>
						e.model.includes('claude-haiku-4-5'),
					) ?? endpoints.find((e) => e.model.includes('claude'));
			} else if (
				!selectedEndpoint &&
				...
				selectedEndpoint =
					endpoints.find((e) =>
						e.model.includes('claude-sonnet-4-5'),
					) ?? endpoints.find((e) => e.model.includes('claude'));
			} else if (
				!selectedEndpoint &&
				...
				selectedEndpoint =
					endpoints.find((e) =>
						e.model.includes('claude-opus-4-5'),
					) ?? endpoints.find((e) => e.model.includes('claude'));
			}
