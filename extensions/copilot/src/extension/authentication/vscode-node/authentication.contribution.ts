...
export class AuthenticationContrib extends Disposable {
	...
	private async askToUpgradeAuthPermissions() {
		const authUpgradeAsk = this._register(
			this.instantiationService.createInstance(AuthUpgradeAsk),
		);
...
class AuthUpgradeAsk extends Disposable {
	private static readonly AUTH_UPGRADE_ASK_KEY =
		'copilot.shownPermissiveTokenModal';
	...
	private registerListeners() {
		this._register(
			this._authenticationService.onDidAuthenticationChange(async () => {
				...
				if (!this._authenticationService.anyGitHubSession) {
					...
					this._extensionContext.globalState.update(
						AuthUpgradeAsk.AUTH_UPGRADE_ASK_KEY,
						false,
					);
					return;
				}
				if (window.state.focused) {
					...
					const disposable = window.onDidChangeWindowState(
						async (e) => {
							if (e.focused) {
								disposable.dispose();
								await this.showPrompt();
