...
describe('LintErrors', () => {
	...
	function createLintErrors(document: CurrentDocument): LintErrors {
		return new LintErrors(documentId, document, diagnosticsService);
	}
	...
	describe('getFormattedLintErrors', () => {
		it('should return empty string when no diagnostics', () => {
			...
			expect(lintErrors.getFormattedLintErrors(defaultLintOptions)).toBe(
				'<|linter diagnostics|>\n\n<|/linter diagnostics|>',
			);
		});
			...
			diagnosticsService.setDiagnostics(fileUri, [
				{
					...
					code: {
						value: '6133',
						target: 'file:///test' as unknown as import('vscode').Uri,
					},
					source: 'ts',
				},
			]);
		...
		it('should include warnings when warnings option is YES', () => {
			...
			const result =
				lintErrors.getFormattedLintErrors(optionsWithWarnings);
			expect(result).toContain('This is an error');
		...
		it('should only include errors when warnings option is YES_IF_NO_ERRORS and errors exist', () => {
			...
			const result =
				lintErrors.getFormattedLintErrors(optionsYesIfNoErrors);
			expect(result).toContain('This is an error');
			expect(result).not.toContain('This is a warning');
		});
		...
		it('should filter diagnostics by distance', () => {
			...
			const result =
				lintErrors.getFormattedLintErrors(optionsSmallDistance);
...
<|/linter diagnostics|>`;
	...
	describe('lineNumberInPreviousFormattedPrompt', () => {
		it('should throw error when called before getFormattedLintErrors', () => {
			...
			expect(() =>
				lintErrors.lineNumberInPreviousFormattedPrompt(
					defaultLintOptions,
					1,
				),
			).toThrow(
	...
	describe('nRecentFiles', () => {
		...
		function createLintErrorsWithHistory(
			document: CurrentDocument,
			history: readonly IXtabHistoryEntry[],
		): LintErrors {
		...
		it('should exclude the current file from recent files', () => {
			...
			const errorCount = (result.match(/Current file error/g) || [])
				.length;
		...
		it('should respect maxLints across current and recent files', () => {
			...
			diagnosticsService.setDiagnostics(fileUri, [
				{
					message: 'Current error 1',
					range: new Range(0, 0, 0, 12),
					severity: DiagnosticSeverity.Error,
				},
				{
					message: 'Current error 2',
					range: new Range(1, 0, 1, 12),
					severity: DiagnosticSeverity.Error,
				},
			]);
		...
		it('should not show code context for recent file diagnostics', () => {
			const document = createDocument(
				['current line 0', 'current line 1', 'current line 2'],
				2,
				1,
			);
			diagnosticsService.setDiagnostics(fileUri, [
			...
			const currentLineCount = (result.match(/current line 0/g) || [])
				.length;
