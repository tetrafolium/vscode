...
suite('stream diffing', () => {
	async function run(editWindow: string[], updatedEditWindowLines: string[]) {
		const updatedEditWindowLinesStream = AsyncIterUtils.fromArray(
			updatedEditWindowLines,
		);

		const editsGen = ResponseProcessor.diff(
			editWindow,
			updatedEditWindowLinesStream,
			0,
	...
	suite('edit window with non-unique lines', () => {
		...
		test('converge at two lines', async () => {
			...
			const lineEdit = new LineEdit([
				new LineReplacement(new LineRange(4, 4), ['    z: number;']),
				new LineReplacement(new LineRange(5, 6), [
					'    constructor(x: number, y: number, z: number) {',
				]),
				new LineReplacement(new LineRange(8, 8), [
					'        this.z = z;',
				]),
		...
		test('prefers adding lines than removing them', async () => {
			...
			const lineEdit = new LineEdit([
				new LineReplacement(new LineRange(3, 5), ['c1']),
			]);
...
describe('emitFastCursorLineChange with empty lines', () => {
	async function runWithFastCursor(
		...
		cursorOffset: number,
		emitFastCursorLineChange: ResponseProcessor.EmitFastCursorLineChange = ResponseProcessor
			.EmitFastCursorLineChange.AdditiveOnly,
	) {
		const updatedEditWindowLinesStream = AsyncIterUtils.fromArray(
			updatedEditWindowLines,
		);
	...
	it('should skip fast-emit when empty cursor line is deleted and new content matches next line prefix', async () => {
		...
		const { edits } = await runWithFastCursor(
			editWindow,
			updatedEditWindow,
			1,
		);
	...
	it('should skip fast-emit when empty cursor line is deleted and new content exactly matches next line', async () => {
		...
		const { patch } = await runWithFastCursor(
			editWindow,
			['before', 'unique_after'],
			1,
		);
...
describe('isAdditiveEdit', () => {
	it('should detect simple substring additions as additive', () => {
		expect(
			ResponseProcessor.isAdditiveEdit('hello', 'hello world'),
		).toMatchInlineSnapshot(`true`);
		expect(
			ResponseProcessor.isAdditiveEdit('world', 'hello world'),
		).toMatchInlineSnapshot(`true`);
		expect(
			ResponseProcessor.isAdditiveEdit('hello world', 'hello world'),
		).toMatchInlineSnapshot(`true`);
	});
	...
	it('should detect insertions in the middle as additive', () => {
		...
		expect(
			ResponseProcessor.isAdditiveEdit(
				'function fib() {',
	...
	it('should detect character insertions as additive', () => {
		expect(
			ResponseProcessor.isAdditiveEdit('abc', 'aXbYcZ'),
		).toMatchInlineSnapshot(`true`);
		expect(
			ResponseProcessor.isAdditiveEdit('abc', 'XXXaYYYbZZZc'),
		).toMatchInlineSnapshot(`true`);
	});

	it('should detect deletions as non-additive', () => {
		expect(
			ResponseProcessor.isAdditiveEdit('hello world', 'hello'),
		).toMatchInlineSnapshot(`false`);
		expect(
			ResponseProcessor.isAdditiveEdit('hello world', 'world'),
		).toMatchInlineSnapshot(`false`);
		expect(
			ResponseProcessor.isAdditiveEdit(
				'function fib(n: number) {',
	...
	it('should detect replacements as non-additive', () => {
		...
		expect(
			ResponseProcessor.isAdditiveEdit('const x = 5', 'const x = 10'),
		).toMatchInlineSnapshot(`false`);
	});

	it('should handle empty strings', () => {
		expect(ResponseProcessor.isAdditiveEdit('', '')).toMatchInlineSnapshot(
			`true`,
		);
		expect(
			ResponseProcessor.isAdditiveEdit('', 'hello'),
		).toMatchInlineSnapshot(`true`);
		expect(
			ResponseProcessor.isAdditiveEdit('hello', ''),
		).toMatchInlineSnapshot(`false`);
	...
	it('should handle whitespace changes', () => {
		// Adding whitespace
		expect(
			ResponseProcessor.isAdditiveEdit('a b', 'a  b'),
		).toMatchInlineSnapshot(`true`);
		expect(
			ResponseProcessor.isAdditiveEdit('ab', 'a b'),
		).toMatchInlineSnapshot(`true`);
	...
	it('should handle complex code transformations', () => {
		...
		expect(
			ResponseProcessor.isAdditiveEdit('obj.prop', 'obj?.prop'),
		).toMatchInlineSnapshot(`true`);

		// Adding template literal
		expect(
			ResponseProcessor.isAdditiveEdit('`hello`', '`hello ${name}`'),
		).toMatchInlineSnapshot(`true`);

		// Adding array element
		expect(
			ResponseProcessor.isAdditiveEdit('[1, 2]', '[1, 2, 3]'),
		).toMatchInlineSnapshot(`true`);

		// Adding object property (subsequence still works)
		expect(
			ResponseProcessor.isAdditiveEdit('{ a: 1 }', '{ a: 1, b: 2 }'),
		).toMatchInlineSnapshot(`true`);
	});
	...
	it('should handle repeated characters correctly', () => {
		...
		expect(
			ResponseProcessor.isAdditiveEdit('aaa', 'aaaa'),
		).toMatchInlineSnapshot(`true`);
		expect(
			ResponseProcessor.isAdditiveEdit('aaa', 'aXaYaZ'),
		).toMatchInlineSnapshot(`true`);
		expect(
			ResponseProcessor.isAdditiveEdit('aaaa', 'aaa'),
		).toMatchInlineSnapshot(`false`);
	});
});
