...
suite('Paged clipping - current file', () => {
	...
	function createTaggedFile(
		...
		return createTaggedCurrentFileContentUsingPagedClipping(
			...
			pageSize,
			opts,
		).map((taggedCurrentFileContent) =>
			taggedCurrentFileContent.lines.join('\n'),
		);
	}
		...
		test('cursor tag appears in current file content when includeCursorTag is true', () => {
			...
			const result = constructTaggedFile(
				...
				new OffsetRange(0, 5), // area around: all lines
				{
					...
					currentFile: {
						...defaultPromptOptions.currentFile,
						includeCursorTag: true,
						includeTags: false,
					},
				},
				computeTokens,
				{
					includeLineNumbers: {
						...
						currentFileContent: IncludeLineNumbersOption.None,
					},
				},
			);
...
describe('getUserPrompt', () => {
	function createTestPromptPieces(opts: {
		...
		const currentDocument = new CurrentDocument(
			docText,
			new Position(opts.cursorLine, opts.cursorColumn),
		);
		...
		const promptOptions: PromptOptions = {
			...
			...(opts.includePostScript !== undefined
				? { includePostScript: opts.includePostScript }
				: {}),
			currentFile: {
				...DEFAULT_OPTIONS.currentFile,
				maxTokens: 10000,
				...(opts.includeLineNumbers !== undefined
	...
	test('PatchBased01 uses basePrompt only (no cursor_position, no area, no backticks)', () => {
		const pieces = createTestPromptPieces({
			cursorLine: 2,
			cursorColumn: 1,
			strategy: PromptingStrategy.PatchBased01,
		});
		const { prompt } = getUserPrompt(pieces);
...
describe('getUserPrompt — globalBudget cascade', () => {
	...
	function makePieces(
		globalBudget: PromptOptions['globalBudget'],
	): PromptPieces {
	...
	test('throws when neighborFiles is ordered before recentlyViewedDocuments', () => {
		...
		expect(() => getUserPrompt(pieces)).toThrow(
			/'recentlyViewedDocuments' before 'neighborFiles'/,
		);
	});
	...
	test('throws on duplicate part in order', () => {
		...
		expect(() => getUserPrompt(pieces)).toThrow(
			/duplicate part 'recentlyViewedDocuments'/,
		);
	});
	...
	test('throws when shares do not sum to ~1', () => {
		...
		expect(() => getUserPrompt(pieces)).toThrow(
			/shares across order must sum to ~1/,
		);
	});
	...
	test('throws when shares is missing an entry for a part in order', () => {
		...
		expect(() => getUserPrompt(pieces)).toThrow(
			/shares is missing entry for 'diffHistory'/,
		);
	});
});
