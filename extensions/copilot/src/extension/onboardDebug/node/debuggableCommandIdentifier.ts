...
export const IDebuggableCommandIdentifier =
	createServiceIdentifier<IDebuggableCommandIdentifier>(
		'IDebuggableCommandIdentifier',
	);

export class DebuggableCommandIdentifier
	extends Disposable
	implements IDebuggableCommandIdentifier
{
	...
	constructor(
		...
		private readonly languageToolsProvider: ILanguageToolsProvider,
		@IFileSystemService
		private readonly fileSystemService: IFileSystemService,
	) {
		super();
		this._register(
			workspaceService.onDidOpenTextDocument((e) => {
	...
	public async isDebuggable(
		...
		return (
			this.getSpecificTreatment(command) ??
	...
	private async isWorkspaceLocal(
		cwd: URI | undefined,
		command: string,
	): Promise<true | undefined> {
		const abs = path.isAbsolute(command)
			? URI.file(command)
			: cwd && URI.joinPath(cwd, command);
	...
	private async isModelSuggestedCommand(
		command: string,
		token: CancellationToken,
	) {
		...
		const { commands, ok } =
			await this.languageToolsProvider.getToolsForLanguages(
				languages,
				token,
			);
	...
	private isWellKnownCommand(command: string): boolean | undefined {
		...
		return (
			KNOWN_DEBUGGABLE_COMMANDS.some((tool) =>
				this.commandIncludes(command, tool),
			) || undefined
		);
	}

	private getSpecificTreatment(command: string): boolean | undefined {
		const patterns = this.configurationService.getConfig(
			ConfigKey.Advanced.TerminalToDebuggerPatterns,
		);
		for (const pattern of patterns) {
			if (
				pattern.startsWith('!') &&
				this.commandIncludes(command, pattern)
			) {
				return false;
	...
	private commandIncludes(command: string, needle: string) {
		...
		return (
			idx >= 0 &&
			(idx === 0 || command[idx - 1] === ' ') &&
			(idx + needle.length === command.length ||
				command[idx + needle.length] === ' ')
		);
	}

	private loadModelKnownCommands() {
		return this.context.globalState.get<IKnownCommandsState>(
			DEBUGGABLE_COMMAND_STORAGE_KEY,
			{
				languages: [],
				commands: [],
			},
 ...
 */
const KNOWN_DEBUGGABLE_COMMANDS = [
	'abap',
	'ant',
	'automake',
	'autotools',
 ...
 * Generated in test/simulation/debugTools.stest.ts, do not edit directly!
 */
const KNOWN_DEBUGGABLE_LANGUAGES = [
	'abap',
	'bat',
	'bibtex',
	'c',
