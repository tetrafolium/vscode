...
import {
	IDebugConfigSchema,
	IPackageJson,
} from '../../../platform/extensions/common/packageJson';
 ...
 */
export const parseLaunchConfigFromResponse = (
	response: string,
	extensionsService: IExtensionsService,
): IStartDebuggingParsedResponse | undefined => {
	const codeBlocks = extractCodeBlocks(response);
	const attempts = codeBlocks ? codeBlocks.map((c) => c.code) : [response];

	const config = tryGetJsonDataFromPart(
		attempts,
		(parsed): ILaunchJSON | undefined => {
			if (
				parsed &&
				...
				parsed.configurations = parsed.configurations.map(
					(config: IDebugConfigSchema) =>
						processSchemaProperties(config, extensionsService),
				);
				return parsed;
			}
			if (parsed && 'type' in parsed && 'request' in parsed) {
				return {
					configurations: [
						processSchemaProperties(parsed, extensionsService),
					],
				} as ILaunchJSON;
			}
		},
	...
	const tasks = tryGetJsonDataFromPart(
		attempts,
		(parsed): ITasksJSON | undefined => {
...
function tryGetJsonDataFromPart<T>(
	attempts: readonly string[],
	process: (parsed: any) => T,
) {
...
export function getSchemasForTypeAsList(
	type: string,
	extensionsService: IExtensionsService,
): string[] | undefined {
	for (const extension of extensionsService.allAcrossExtensionHosts) {
		const debuggers = (extension.packageJSON as IPackageJson)?.contributes
			?.debuggers;
		if (!debuggers) {
			continue;
		}
		const debuggersForType = debuggers.filter(
			(d) => d.type === type && !d.deprecated,
		);
		if (!Array.isArray(debuggersForType) || debuggersForType.length === 0) {
			continue;
		}

		const schemas = debuggersForType
			.filter(
				(d) =>
					!!d.configurationAttributes.launch ||
					!!d.configurationAttributes.attach,
			)
			.map((d) => {
				const properties = [
					d.configurationAttributes.launch?.properties,
					d.configurationAttributes.attach?.properties,
				]
					.filter((p) => p !== undefined)
					.flat();
				return Object.entries(properties)
					.map((p) => {
						return Object.entries(p[1])
							.map((p) => {
...
export function getSchemasForType(
	type: string,
	extensionsService: IExtensionsService,
): object | undefined {
	for (const extension of extensionsService.allAcrossExtensionHosts) {
