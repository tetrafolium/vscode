...
export class ChatSessionContextContribution extends Disposable {
	...
	constructor(
		...
		super();
		this._enableChatSessionContextProvider =
		...
		return autorun((reader) => {
			...
			if (branchName !== this._lastBranchName) {
				...
				this.logService.trace(
					`[ChatSessionContextProvider] Branch changed to: ${branchName}`,
				);
			}
		});
	}
	...
	private register(): IDisposable {
		...
		try {
			...
			this.logService.error(
				'Error registering chat session context provider:',
				error,
			);
		}
		return disposables;
...
class ContextResolver implements Copilot.ContextResolver<Copilot.SupportedContextItem> {
	constructor(
		...
		private readonly setSummaryCache: (
	...
	async resolve(
		...
		try {
			...
			if (
				branchChangeTime !== undefined &&
				firstTurnStartTime < branchChangeTime
			) {
				this.logService.trace(
					`[ChatSessionContextProvider] Skipping conversation that started before branch change`,
				);
				return [];
			}
			...
			this.logService.error(
				'[ChatSessionContextProvider] Error resolving context:',
				error,
			);
			return [];
		}
	...
	private async generateSummary(
		...
		try {
			...
			const conversationContent =
				this.buildConversationContent(conversation);
			...
			if (models.length === 0) {
				...
				if (allModels.length === 0) {
					this.logService.trace(
						'[ChatSessionContextProvider] No language models available',
					);
					return undefined;
				}
			...
			this.logService.error(
				'[ChatSessionContextProvider] Error generating summary:',
				error,
			);
			return undefined;
		}
		...
		for (const turn of turns) {
			...
			if (turn.responseMessage?.message) {
				...
				const truncatedMessage =
					...
					SINGLE_TURN_MESSAGE_LIMIT + truncatedIndicator.length
						? responseMessage.substring(
								0,
								SINGLE_TURN_MESSAGE_LIMIT / 2,
							) +
							truncatedIndicator +
							responseMessage.substring(
								responseMessage.length -
