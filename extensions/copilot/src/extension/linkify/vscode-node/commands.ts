...
import {
	openFileLinkCommand,
	OpenFileLinkCommandArgs,
	openSymbolInFileCommand,
	OpenSymbolInFileCommandArgs,
} from '../common/commands';
import { findBestSymbolByPath } from './findSymbol';

export const openSymbolFromReferencesCommand =
	'_github.copilot.openSymbolFromReferences';

export type OpenSymbolFromReferencesCommandArgs = [
	_word_unused: string,
	locations: ReadonlyArray<{ uri: UriComponents; pos: vscode.Position }>,
	requestId: string | undefined,
];

export function registerLinkCommands(telemetryService: ITelemetryService) {
	return combinedDisposable(
		vscode.commands.registerCommand(
			openFileLinkCommand,
			async (...[path, requestId]: OpenFileLinkCommandArgs) => {
				/* __GDPR__
		...
		vscode.commands.registerCommand(
			...
			async (
				...
				*/
					...
					if (matchingSymbol) {
						const range =
							matchingSymbol instanceof vscode.SymbolInformation
								? matchingSymbol.location.range
								: matchingSymbol.selectionRange;
						return vscode.commands.executeCommand(
							'vscode.open',
							fileUri,
							{
								selection: new vscode.Range(
									range.start,
									range.start,
								), // Move cursor to the start of the symbol
		...
		vscode.commands.registerCommand(
			openSymbolFromReferencesCommand,
			async (
				...[
					_word,
					locations,
					requestId,
				]: OpenSymbolFromReferencesCommandArgs
			) => {
			...
			*/
				...
				if (dest) {
					const selectionRange =
						dest.loc.targetSelectionRange ?? dest.loc.targetRange;
					return vscode.commands.executeCommand(
						'vscode.open',
						dest.loc.targetUri,
						{
					...
					return vscode.window.showWarningMessage(
						t(
							'Could not resolve this symbol in the current workspace.',
						),
					);
				}
			},
...
export async function resolveSymbolFromReferences(
	locations: ReadonlyArray<{ uri: UriComponents; pos: vscode.Position }>,
	symbolText: string | undefined,
	...
	const symbolParts = symbolText
		? Array.from(symbolText.matchAll(/[#\w$][\w\d$]*/g), (x) => x[0])
		: [];
	...
	if (!dest) {
		...
		if (firstLoc) {
			dest = {
				...
				loc: {
					targetUri: vscode.Uri.from(firstLoc.uri),
					targetRange: new vscode.Range(firstLoc.pos, firstLoc.pos),
				},
			};
		}
	}
