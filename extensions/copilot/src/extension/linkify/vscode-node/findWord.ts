...
export async function findSymbolLocationInFile(
	...
	const symbols = await getCachedFileSymbols(
		parserService,
		uri,
		token,
		cache,
	...
	if (symbolParts.length) {
		const declarationMatch = findBestSymbolPart(
	...
	return (
		findExactSymbol(genericSymbols, symbolText)?.location ??
		(symbolParts.length
			? findBestSymbolPart(genericSymbols, symbolParts)?.location
			: undefined)
	);
}
...
function findBestSymbolPart(
	...
	for (const symbol of symbols) {
		...
		if (
			matchIndex !== -1 &&
			(!bestMatch || matchIndex > bestMatch.matchIndex)
		) {
...
async function doGetFileSymbols(
	parserService: IParserService,
	uri: vscode.Uri,
	token: CancellationToken,
): Promise<FileSymbols> {
	...
	try {
		...
		const ast = parserService.getTreeSitterASTForWASMLanguage(
			wasmLanguage,
			text,
		);
		const [classDeclarations, functionDefinitions, typeDeclarations] =
			await Promise.all([
...
function toFileSymbols(
	uri: vscode.Uri,
	doc: SimpleTextDocument,
	symbols: readonly TreeSitterExpressionInfo[],
): readonly FileSymbol[] {
...
async function findWordInDoc(
	...
	documentCache?: Map<string, Promise<SimpleTextDocument | undefined>>,
): Promise<ResolvedWordLocation | undefined> {
	...
	for (const symbol of symbols) {
		if (symbol.identifier === word) {
			...
			return {
				type: ResolvedWordLocationType.SymbolMatch,
	...
	for (const match of text.matchAll(
		new RegExp(escapeRegExpCharacters(word), 'g'),
	)) {
		if (match.index) {
			const wordPos = doc.positionAt(startOffset + match.index);
			if ('getWordRangeAtPosition' in doc) {
				const wordInDoc = doc.getText(
					(doc as vscode.TextDocument).getWordRangeAtPosition(
						wordPos,
					),
				);
				if (word === wordInDoc) {
					return {
						type: ResolvedWordLocationType.TextualMatch,
						location: new vscode.Location(uri, wordPos),
					};
				}
			} else {
				const wordInDoc = doc.getText(
					new vscode.Range(
						wordPos,
						doc.positionAt(doc.offsetAt(wordPos) + word.length),
					),
				);
				if (word === wordInDoc) {
					return {
...
async function openDocument(
	uri: vscode.Uri,
	documentCache?: Map<string, Promise<SimpleTextDocument | undefined>>,
): Promise<SimpleTextDocument | undefined> {
...
async function getSymbolsInRange(
	parserService: IParserService,
	doc: SimpleTextDocument,
	range: vscode.Range,
	token: vscode.CancellationToken,
...
export class ReferencesSymbolResolver {
	...
	private async doResolve(
		...
		// Prefer exact match
		let wordMatches = await this.instantiationService.invokeFunction(
			(accessor) =>
				findWordInReferences(
					accessor,
					references,
					codeText,
					this.findWordOptions,
					token,
					this.documentCache,
				),
		);
