...
export class ResponseStreamWithLinkification implements FinalizableChatResponseStream {
	...
	constructor(
		...
		this._linkifier = linkifyService.createLinkifier(
			context,
	...
	thinkingProgress(thinkingDelta: ThinkingDelta): ChatResponseStream {
		this.enqueue(
			() => this._progress.thinkingProgress(thinkingDelta),
			false,
		);
		return this;
	}
	...
	codeCitation(
		...
		this.enqueue(
			() => this._progress.codeCitation(value, license, snippet),
			false,
		);
		return this;
	}
	...
	externalEdit(
		...
		return this.enqueue(
			() => this._progress.externalEdit(target, callback),
			true,
		);
	}
	...
	private isBlockPart(part: ChatResponsePart): boolean {
		return (
			part instanceof ChatResponseFileTreePart ||
	...
	textEdit(
		target: Uri,
		editsOrDone: TextEdit | TextEdit[] | true,
	): ChatResponseStream {
		// TS makes me do this
		if (editsOrDone === true) {
			this.enqueue(
				() => this._progress.textEdit(target, editsOrDone),
				false,
			);
		} else {
			this.enqueue(
	...
	notebookEdit(
		...
		if (editsOrDone === true) {
			this.enqueue(
				() => this._progress.notebookEdit(target, editsOrDone),
				false,
			);
		} else {
			this.enqueue(
	...
	markdownWithVulnerabilities(
		...
		this.enqueue(
			() =>
				this._progress.markdownWithVulnerabilities(
					value,
					vulnerabilities,
				),
			false,
	...
	confirmation(
		...
		this.enqueue(
			() => this._progress.confirmation(title, message, data),
			true,
		);
		return this;
	}
	...
	beginToolInvocation(
		...
		this.enqueue(
			() =>
				this._progress.beginToolInvocation(
					toolCallId,
					toolName,
					streamData,
				),
	...
	updateToolInvocation(
		...
		this.enqueue(
			() => this._progress.updateToolInvocation(toolCallId, streamData),
			false,
		);
		return this;
	}
	...
	questionCarousel(
		...
		return this.enqueue(
			() => this._progress.questionCarousel(questions, allowSkip),
			true,
		);
	}
	...
	private outputMarkdown(textToApply: LinkifiedText) {
		for (const part of textToApply.parts) {
			if (typeof part === 'string') {
				...
				const folder = this.workspaceService
					.getWorkspaceFolders()
					?.at(0);
				if (folder) {
				...
				if (part instanceof LinkifySymbolAnchor) {
					const chatPart = new ChatResponseAnchorPart(
						part.symbolInformation as any,
					);
					if (part.resolve) {
						(chatPart as any).resolve = () =>
							part.resolve!(this._token);
					}
