...
suite('Find symbol', () => {
	...
	test('Should find exact match', () => {
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a')], 'a')?.name,
			'a',
		);
		assert.strictEqual(
			findBestSymbolByPath([symbolInfo('a')], 'a')?.name,
			'a',
		);
	});
	...
	test('Should find nested', () => {
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('x', docSymbol('a'))], 'a')?.name,
			'a',
		);
	});
	...
	test('Should find child match', () => {
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a', docSymbol('b'))], 'a.b')?.name,
			'b',
		);
	});
	...
	test('Should find child match skipping level', () => {
		assert.strictEqual(
			findBestSymbolByPath(
				[docSymbol('a', docSymbol('x', docSymbol('b')))],
				'a.b',
			)?.name,
			'b',
		);
	...
	test(`Should find match even when children don't match`, () => {
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a')], 'a.b')?.name,
			'a',
		);
	});
		...
		assert.strictEqual(
			findBestSymbolByPath(
				[
					...
					docSymbol(
						'x',
						docSymbol('a', docSymbol('b', docSymbol('z'))),
					),
				],
				'a.b',
			)?.name,
	...
	test('Should ignore function call notation', () => {
		...
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a')], 'a(1, 2, 3)')?.name,
			'a',
		);
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a')], 'a(b, c)')?.name,
			'a',
		);
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a')], 'a(b: string)')?.name,
			'a',
		);
	});
	...
	test('Should ignore generic notation', () => {
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a')], 'a<T>')?.name,
			'a',
		);
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('a')], 'a<T>.b')?.name,
			'a',
		);
	});
	...
	test('Should match on symbols with _', () => {
		assert.strictEqual(
			findBestSymbolByPath([docSymbol('_a_')], '_a_')?.name,
			'_a_',
		);
	});
	...
	test('Should prefer rightmost symbol in flat symbols', () => {
		...
		assert.strictEqual(
			findBestSymbolByPath(
				[symbolInfo('TextModel'), symbolInfo('undo')],
				'TextModel.undo()',
			)?.name,
			'undo',
		);
	...
	test('Should fall back to leftmost symbol if rightmost not found in flat symbols', () => {
		...
		assert.strictEqual(
			findBestSymbolByPath(
				[symbolInfo('TextModel'), symbolInfo('someOtherMethod')],
				'TextModel.undo()',
			)?.name,
			'TextModel',
		);
	...
	test('Should handle deeply qualified names', () => {
		...
		assert.strictEqual(
			findBestSymbolByPath(
				[symbolInfo('namespace'), symbolInfo('TextModel')],
				'namespace.TextModel.undo()',
			)?.name,
			'TextModel',
		);
