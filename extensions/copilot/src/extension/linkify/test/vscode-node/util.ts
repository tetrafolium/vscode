...
export class TestParserService implements Partial<IParserService> {
	...
	public readonly genericSymbolRanges: {
...
export async function createTestFile(
	relativePath: string,
	contents: string,
): Promise<{ readonly workspace: URI; readonly uri: URI }> {
...
export function asParserService(
	parserService: TestParserService,
): IParserService {
...
export function symbol(
	...
	assert.notStrictEqual(
		startIndex,
		-1,
		`Test symbol "${identifier}" was not found in contents.`,
	);
	return {
		identifier,
		text: identifier,
...
export function declaration(
	...
	assert.notStrictEqual(
		startIndex,
		-1,
		`Test declaration text "${text}" was not found in contents.`,
	);
	return {
		identifier,
		text,
