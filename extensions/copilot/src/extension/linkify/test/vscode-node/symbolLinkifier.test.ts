...
function createTestLinkifierService(
	listOfFiles: readonly string[],
	parserService: IParserService = asParserService(new TestParserService()),
	workspace?: URI,
): ILinkifyService {
	const fs = createMockFsService(
		workspace
			? listOfFiles.map((file) => URI.joinPath(workspace, file))
			: listOfFiles,
	);
	const workspaceService = workspace
		? createWorkspaceService(workspace)
		: createMockWorkspaceService();
	const linkifier = new LinkifyService(
		fs,
		workspaceService,
		NullEnvService.Instance,
	);
	linkifier.registerGlobalLinkifier({
		create: () => new SymbolLinkifier(fs, parserService, workspaceService),
	});
	return linkifier;
}
...
suite('Symbol Linkify', () => {
	...
	test(`Should use tree-sitter for linked-backtick initial symbol locations`, async () => {
		...
		const { workspace, uri } = await createTestFile(
			'src/file.ts',
			contents,
		);
		...
		assert.strictEqual(
			parts[0].symbolInformation.location.uri.toString(),
			uri.toString(),
		);
		assert.strictEqual(
			parts[0].symbolInformation.location.range.start.line,
			2,
		);
		assert.strictEqual(
			parts[0].symbolInformation.location.range.start.character,
			6,
		);
	});

	test(`Should keep the start-of-file fallback when tree-sitter does not find a symbol`, async () => {
		const contents = ['const value = 1;', '', 'class Bar {', '}'].join(
			'\n',
		);
		const { workspace, uri } = await createTestFile(
			'src/file.ts',
	...
	test(`Should reuse the tree-sitter cache for multiple links to the same file`, async () => {
		const contents = ['class Foo {', '}', '', 'class Bar {', '}'].join(
			'\n',
		);
		...
		const fs = createMockFsService([
			URI.joinPath(workspace, 'src/file.ts'),
		]);
		const linkifier = new SymbolLinkifier(
			fs,
			asParserService(testParserService),
			createWorkspaceService(workspace),
		);

		const result = await linkifier.linkify(
			'[`Foo`](src/file.ts) [`Bar`](src/file.ts)',
		...
		assert.strictEqual(
			parts[2].symbolInformation.location.range.start.line,
			3,
		);
		assert.strictEqual(
			parts[2].symbolInformation.location.range.start.character,
			6,
		);
	});
	...
	test(`Should reuse the tree-sitter cache across streamed chunks`, async () => {
		...
		const firstResult = await linkifier.linkify(
			'[`Foo`](src/file.ts)',
			context,
			CancellationToken.None,
		);
		const secondResult = await linkifier.linkify(
			'[`Bar`](src/file.ts)',
			context,
			CancellationToken.None,
		);
		...
		assert.strictEqual(
			firstResult.parts[0].symbolInformation.location.range.start.line,
			0,
		);
		assert.strictEqual(
			secondResult.parts[0].symbolInformation.location.range.start.line,
			3,
		);
	});
		...
		const fs = new (class implements Partial<IFileSystemService> {
			async stat(path: URI): Promise<vscode.FileStat> {
				...
				try {
					...
					if (
						!files.some(
							(file) => file.toString() === path.toString(),
						)
					) {
	...
	test(`Should let LSP resolve upgrade symbol kind and location`, async () => {
		...
		try {
			...
			const parts = (await linkify(linkifier, '[`Foo`](src/file.txt)'))
				.parts;

			assert.strictEqual(parts.length, 1);
			assert(parts[0] instanceof LinkifySymbolAnchor);
			assert.strictEqual(
				parts[0].symbolInformation.kind,
			...
			assert.strictEqual(
				resolved.location.uri.toString(),
				uri.toString(),
			);
