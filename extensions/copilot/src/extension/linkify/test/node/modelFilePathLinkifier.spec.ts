...
suite('Model File Path Linkifier', () => {
	...
	test('Should decode percent-encoded targets', async () => {
		...
		const result = await linkify(
			service,
			'[space file.ts](space%20file.ts#L1)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
	...
	test('Should linkify descriptive text with anchor', async () => {
		...
		const result = await linkify(
			service,
			'[Await chat view](src/file.ts#L54)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
				workspaceFile('src/file.ts'),
				new Range(new Position(53, 0), new Position(53, 0)),
			),
		);
	...
	test('Should fallback for invalid anchor syntax', async () => {
		...
		const result = await linkify(
			service,
			'[src/file.ts](src/file.ts#Lines10-12)',
		);
	...
	test('Should handle backticks in link text with line anchor', async () => {
		...
		const result = await linkify(
			service,
			'[`src/file.ts`](src/file.ts#L42)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
				workspaceFile('src/file.ts'),
				new Range(new Position(41, 0), new Position(41, 0)),
			),
		);
	...
	test('Should handle L123-L456 anchor format with L prefix on end line', async () => {
		...
		const result = await linkify(
			service,
			'[src/file.ts](src/file.ts#L10-L15)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
	...
	test('Should handle descriptive text with L123-L456 anchor format', async () => {
		...
		const result = await linkify(
			service,
			'[Some descriptive text](src/file.ts#L20-L25)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
				workspaceFile('src/file.ts'),
				new Range(new Position(19, 0), new Position(24, 0)),
			),
		);
	...
	test('Should normalize non-standard L123-456 format to standard L123-L456', async () => {
		...
		const result = await linkify(
			service,
			'[src/file.ts](src/file.ts#L20-25)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
	...
	test('Should handle absolute paths with forward slashes on Windows', async () => {
		...
		const result = await linkify(
			service,
			`[line 67](${pathWithForwardSlashes}#L67)`,
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
				workspaceFile('src/file.ts'),
				new Range(new Position(66, 0), new Position(66, 0)),
			),
		);
	...
	test('Should NOT linkify when display text looks like filename but does not match target filename', async () => {
		...
		const result = await linkify(
			service,
			'[nonexistent.ts](src/existing.ts#L10)',
		);
	...
	test('Should linkify when display text filename matches target filename with anchor', async () => {
		...
		const expected = new LinkifyLocationAnchor(
			new Location(
				workspaceFile('src/file.ts'),
				new Range(new Position(9, 0), new Position(9, 0)),
			),
		);
	...
	test('Should linkify bare line range anchors without L prefix', async () => {
		...
		const result = await linkify(
			service,
			'[src/file.ts](src/file.ts#10-20)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
				workspaceFile('src/file.ts'),
				new Range(new Position(9, 0), new Position(19, 0)),
			),
		);
	...
	test('Should linkify descriptive text with bare line range anchor', async () => {
		...
		const result = await linkify(
			service,
			'[existing pattern in chatListRenderer](src/file.ts#1287-1290)',
		);
		const anchor = result.parts[0] as LinkifyLocationAnchor;
		const expected = new LinkifyLocationAnchor(
			new Location(
				workspaceFile('src/file.ts'),
				new Range(new Position(1286, 0), new Position(1289, 0)),
			),
		);
...
suite('Model File Path Linkifier Remote Workspace', () => {
		...
		class MockFs implements IFileSystemService {
			...
			async stat(resource: URI) {
				...
				const found = files.find(
					(f) => f.toString() === resource.toString(),
				);
				if (!found) {
				...
				return {
					...
					mtime: 0,
					size: 0,
					type: found.path.endsWith('/')
						? FileType.Directory
						: FileType.File,
				};
			}
			readDirectory(): Promise<[string, FileType][]> {
	...
	const remoteFile = URI.from({
		scheme: 'test',
		authority: 'auth',
		path: '/home/user/project/src/remote.ts',
	});

	test('Should map absolute remote path preserving scheme', async () => {
		const service = createRemoteService(remoteRoot, [remoteFile]);
		const parts = await remoteLinkify(
			service,
			'[/home/user/project/src/remote.ts](/home/user/project/src/remote.ts)',
		);
	...
	test('Should parse line range anchor on remote absolute path', async () => {
		...
		const parts = await remoteLinkify(
			service,
			'[/home/user/project/src/remote.ts](/home/user/project/src/remote.ts#L3-5)',
		);
		expect(parts.length).toBe(1);
		const anchor = parts[0] as LinkifyLocationAnchor;
