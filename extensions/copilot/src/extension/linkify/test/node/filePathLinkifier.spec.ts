...
suite('File Path Linkifier', () => {
	...
	test(`Should create links for directories`, async () => {
		{
			const linkifier = createTestLinkifierService('dir/');
			assertPartsEqual(
				(await linkify(linkifier, '[dir](dir) [dir/](dir/)')).parts,
				[
		...
		{
			const linkifier = createTestLinkifierService('dir1/dir2/');
			assertPartsEqual(
				(
					await linkify(
						linkifier,
	...
	test(`Should create file links for file paths as inline code`, async () => {
		const linkifier = createTestLinkifierService('file.ts', 'src/file.ts');
		assertPartsEqual(
			(await linkify(linkifier, '`file.ts` `src/file.ts`')).parts,
			[
				new LinkifyLocationAnchor(workspaceFile('file.ts')),
				` `,
				new LinkifyLocationAnchor(workspaceFile('src/file.ts')),
	...
	test(`Should handle posix style absolute paths`, async () => {
		const isFile = URI.file(
			isWindows ? 'c:\\foo\\isfile.ts' : '/foo/isfile.ts',
		);
		const noFile = URI.file(
			isWindows ? 'c:\\foo\\nofile.ts' : '/foo/nofile.ts',
		);
		const linkifier = createTestLinkifierService(isFile);

		assertPartsEqual(
			(
				await linkify(
					linkifier,
					[`\`${isFile.fsPath}\``, `\`${noFile.fsPath}\``].join('\n'),
				)
			).parts,
	...
	test(`Should find file links in bold elements`, async () => {
		...
		assertPartsEqual((await linkify(linkifier, '**file.ts**')).parts, [
			`**`,
			new LinkifyLocationAnchor(workspaceFile('file.ts')),
			`**`,
		]);
	...
	test(`Should NOT use reference fallback for paths with directory components`, async () => {
		...
		const linkifier = createTestLinkifierService();
		const references = [
			new PromptReference(URI.file('/workspace/src/cli.js')),
		];
	...
	test(`Should use reference fallback for simple filenames`, async () => {
		...
		const result = await linkify(linkifier, 'cli.js', references);
		assertPartsEqual(result.parts, [new LinkifyLocationAnchor(refUri)]);
	});
