...
suite('guessNamingConvention', () => {
	const testCases = [
		...
		{
			input: 'SCREAMING_SNAKE_CASE_EXAMPLE',
			expected: NamingConvention.ScreamingSnakeCase,
		},
		{
			input: 'Capital_snake_case',
			expected: NamingConvention.CapitalSnakeCase,
		},
...
suite('chunkByNamingConvention', () => {
	const testCases = [
		...
		},
		{
			...
			expected: ['screaming', 'snake', 'case', 'example'],
		},
		{
			input: 'Capital_snake_case',
			convention: NamingConvention.CapitalSnakeCase,
			expected: ['capital', 'snake', 'case'],
		},
		{
...
suite('enforceNamingConvention', () => {
	const testCases = [
		{
			givenIdent: 'snake_case_example',
			targetConvention: NamingConvention.CamelCase,
			expected: 'snakeCaseExample',
		},
		{
			givenIdent: 'camelCaseExample',
			targetConvention: NamingConvention.SnakeCase,
			expected: 'camel_case_example',
		},
		{
			givenIdent: 'PascalCaseExample',
			targetConvention: NamingConvention.SnakeCase,
			expected: 'pascal_case_example',
		},
		{
			givenIdent: 'camelCaseExample',
			targetConvention: NamingConvention.PascalCase,
			expected: 'CamelCaseExample',
		},
		{
			givenIdent: 'snake_case_example',
			targetConvention: NamingConvention.ScreamingSnakeCase,
			expected: 'SNAKE_CASE_EXAMPLE',
		},
		{
			givenIdent: 'SCREAMING_SNAKE_CASE_EXAMPLE',
			targetConvention: NamingConvention.CapitalSnakeCase,
			expected: 'Screaming_snake_case_example',
		},
		{
			givenIdent: 'Capital_snake_case',
			targetConvention: NamingConvention.KebabCase,
			expected: 'capital-snake-case',
		},
		{
			givenIdent: 'kebab-case-example',
			targetConvention: NamingConvention.Uppercase,
			expected: 'KEBAB-CASE-EXAMPLE',
		},
		{
			givenIdent: 'Uppercase',
			targetConvention: NamingConvention.LowerCase,
			expected: 'uppercase',
		},
		{
			givenIdent: 'lowercase',
			targetConvention: NamingConvention.Unknown,
			expected: 'lowercase',
		},
		{
			givenIdent: 'Unknown_Example',
			targetConvention: NamingConvention.CamelCase,
			expected:
				'unknown_example' /* TODO@ulugbekna: improve unknown convention handling */,
		},
	];

	testCases.forEach(({ givenIdent, targetConvention, expected }) => {
		test(`should enforce ${targetConvention} convention for "${givenIdent}"`, () => {
			expect(enforceNamingConvention(givenIdent, targetConvention)).toBe(
				expected,
			);
		});
	});
