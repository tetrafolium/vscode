...
import {
	generateNotebookCellContext,
	getSelectionAndCodeAroundSelection,
} from '../../../context/node/resolvers/inlineChatSelection';
import {
	CodeContextRegion,
	CodeContextTracker,
...
export class NotebookCellList extends PromptElement<
	{
		title: string;
		cells: CodeContextRegion[];
		cellIndexDelta?: number;
	} & BasePromptElementProps
> {
	override render() {
		return (
			<>
				...
				{this.props.cells.map((cell, index) => (
					<NotebookCellContent
						index={index + (this.props.cellIndexDelta ?? 0)}
						cell={cell}
					/>
				))}
			</>
...
class NotebookCellContent extends PromptElement<
	{ index: number; cell: CodeContextRegion } & BasePromptElementProps
> {
	override render() {
		return (
			<>
				CELL INDEX: {this.props.index}
				<br />
				...
				{(aboveCellsInfo.length > 0 || belowCellsInfo.length > 0) && (
					<UserMessage>
						...
						<>
							...
							document at index 1.
							<br />
							In this new cell, I am working with the following
							code:
							<br />
							```python
							<br />
							...
							method of pandas dataframe. Here's the code:
							<br />
							<br />
							```python
							<br />
...
export class InlineChatJupyterNotebookCellsContextTagBasedRenderer extends PromptElement<InlineChatJupyterNotebookCellsContextRendererProps> {
			...
			<>
				{(aboveCellsInfo.length > 0 || belowCellsInfo.length > 0) && (
					<UserMessage>
						...
						This Jupyter Notebook already contains multiple cells.
						<br />
						The content of cells are listed below, source code is
						contained in ```{lang.languageId} blocks
						<br />
						Each cell is a block of code that can be executed
						independently.
						<br />
						Below you will find a set of examples of what you should
						respond with. Please follow the exmaples on how to avoid
						repeating code.
						<br />
						<Tag name="example">
							<Tag name="cellsAbove">
								Here are the cells above the current cell that I
								am editing in this Jupyter Notebook:
								<br />
						...
						{aboveCellsInfo.length > 0 && (
							<Tag name="cellsAbove">
								...
								am editing in this Jupyter Notebook:
								<br />
								{aboveCellsInfo.map((cell, index) =>
									this._renderCellContent(cell, index),
								)}
							</Tag>
						)}
						{belowCellsInfo.length > 0 && (
							<Tag name="cellsBelow">
								Here are the cells below the current cell that I
								am editing in this Jupyter Notebook:
								<br />
								{belowCellsInfo.map((cell, index) =>
									this._renderCellContent(
										cell,
										index + aboveCellsInfo.length + 1,
									),
								)}
							</Tag>
						)}
	...
	private _renderCellContent(cell: CodeContextRegion, index: number) {
		...
		return (
			<IndexedTag name="cell" index={index}>
				<TextChunk>{code}</TextChunk>
			</IndexedTag>
		);
	}
}
...
export function generateSelectionContextInNotebook(
	...
	return generateNotebookCellContext(
...
export const CustomNotebookExamples = [
	{
		...
		exampleCells: [
			...
			{
				lan: 'csharp',
				source: 'DateTimeDataFrameColumn dateTimes = new DateTimeDataFrameColumn(\"DateTimes\");\n Int32DataFrameColumn ints = new Int32DataFrameColumn(\"Ints\", 6);\n StringDataFrameColumn strings = new StringDataFrameColumn(\"Strings\", 6);',
			},
			{
				lan: 'csharp',
				source: 'dateTimes.Append(DateTime.Parse(\"2019/01/01\"));',
			},
		],
	},
	{
		viewType: 'sql-notebook',
		exampleCells: [{ lan: 'sql', source: 'SELECT * FROM users;' }],
	},
	{
		...
		exampleCells: [
			...
			{
				lan: 'javascript',
				source: `const {display} = require('node-kernel');`,
			},
	...
	},
	{
		...
		exampleCells: [
			...
			{
				lan: 'powerbi-api',
				source: '%dax /groups/ccce57d1-10af-1234-1234-665f8bbd8458/datasets/51ba6d4b-1234-1234-8635-a7d743a5ea89\nEVALUATE INFO.TABLES()\nThis',
			},
		],
	},
	{
	...
	},
	{
		...
		exampleCells: [
			{
				lan: 'github-issues',
				source: '$vscode=repo:microsoft/vscode\n$milestone=milestone:"May 2020"',
			},
			{
				lan: 'github-issues',
				source: '$vscode $milestone is:closed author:@me -assignee:@me label:bug -label:verified',
			},
			{
				lan: 'github-issues',
				source: '$vscode assignee:@me is:open label:freeze-slow-crash-leak',
			},
		],
	},
	{
		...
		exampleCells: [
			...
			{
				lan: 'rest-book',
				source: 'GET https://www.google.com\n    ?query="fun"\n    &page=2\n    User-Agent: rest-book\n    Content-Type: application/json',
			},
		],
	},
];
...
export class CustomNotebookExampleRenderer extends PromptElement<CustomNotebookExampleRendererProps> {
	render() {
		...
		const matchedExample = CustomNotebookExamples.find(
			(example) => example.viewType === this.props.viewType,
		);
		if (!matchedExample) {
			return <></>;
		}
		...
		return (
			<UserMessage>
				Below you will find a set of example cells for a {viewType}{' '}
				notebook.
				<br />
...
export class InlineChatCustomNotebookInfoRenderer extends PromptElement<InlineChatNotebookSelectionCommonProps> {
	...
	render(state: void, sizing: PromptSizing) {
		if (
			...
			throw illegalArgument(
				'InlineChatCustomNotebookInfoRenderer should be used only with a notebook!',
			);
		}

		const notebookType = findNotebookType(
			this.workspaceService,
			this.props.documentContext.document.uri,
		);
		const matchedExample = CustomNotebookExamples.find(
			(example) => example.viewType === notebookType,
		);
		...
		return (
			<>
				{
					<UserMessage>
						I am working on a {notebookTypeName} notebook in VS
						Code.
						<br />
						{notebookTypeName} notebooks in VS Code are documents
						that contain a mix of rich Markdown, executable code
						snippets, <br />
						and accompanying rich output. These are all separated
						into distinct cells and can be interleaved in any order{' '}
						<br />A {notebookTypeName} notebook contains multiple
						cells.
						<br />
					</UserMessage>
				}
				{matchedExample && (
					<CustomNotebookExampleRenderer
						viewType={matchedExample.viewType}
					/>
				)}
			</>
		);
...
export class InlineChatCustomNotebookCellsContextRenderer extends PromptElement<InlineChatCustomNotebookCellsContextRendererProps> {
	render(state: void, sizing: PromptSizing) {
		if (
			...
			throw illegalArgument(
				'InlineChatCustomNotebookCellsContextRenderer should be used only with a notebook!',
			);
		}
					...
					<UserMessage>
						...
						{aboveCellsInfo.length > 0 && (
							<NotebookCellList
								cells={aboveCellsInfo}
								title={
									'Here are the cells in this custom notebook:\n'
								}
							/>
						)}
						{belowCellsInfo.length > 0 && (
							<NotebookCellList
								...
								cellIndexDelta={aboveCellsInfo.length + 1}
								title={
									'Here are the cells below the current cell that I am editing in this custom notebook:\n'
								}
							/>
						)}
					</UserMessage>
...
export class InlineChatNotebookVariables extends PromptElement<
	InlineChatNotebookVariablesPromptProps,
	InlineChatNotebookRuntimeState
> {
	constructor(
	...
	override async prepare(): Promise<InlineChatNotebookRuntimeState> {
		if (
			this.tabsAndEditorsService.activeNotebookEditor?.notebook.uri
				.path !== this.props.notebookURI.path
		) {
		...
		const [variables, packages] = await Promise.all([
			fetchVariables,
			fetchPackages,
		]);
		return { variables, packages };
				...
				{state.variables.length !== 0 && (
					<>
						<UserMessage priority={priorities.runtimeCore}>
							The following variables are present in this Jupyter
							Notebook:
							{state.variables.map((variable) => (
								<>
									<TextChunk>
										...
										<br />
										{variable.indexedChildrenCount > 0 && (
											<>
												Length:{' '}
				...
				{state.packages.length !== 0 && (
					<>
						<UserMessage priority={priorities.other}>
							The following pip packages are available in this
							Jupyter Notebook:
