...
import {
	...
	numberComparator,
	sumBy,
} from '../../../../../util/vs/base/common/arrays';
...
import {
	ConcatenatedStringFragment,
	LiteralStringFragment,
	OriginalStringFragment,
	pushFragment,
	StringFragment,
} from './fragments';
...
export interface ISummarizedDocumentSettings<
...
export interface ICostFnFactory<TDocument extends AbstractDocument> {
	createCostFn(
		doc: TDocument,
	): (node: RemovableNode, currentCost: number) => number | false;
}
...
export class ProjectedDocument<
	TDocument extends AbstractDocument = AbstractDocument,
> extends ProjectedText {
	constructor(
	...
	getLanguageId(
		this: ProjectedDocument<AbstractDocument & { languageId: string }>,
	): string {
...
export function summarizeDocumentsSyncImpl<TDocument extends AbstractDocument>(
	...
	for (let i = 0; i < docs.length; i++) {
		...
		const offsetSelection = selection
			? document.rangeToOffsetRange(selection)
			: undefined;
		const removableNodeRoot = createRemovableNodeFromOverlayNode(
			overlayNodeRoot,
			document,
		);
		const rootTextNode = TextNode.fromRootNode(removableNodeRoot, document);
		const rootMarkedNode = SurvivingTextNode.fromNode(
			rootTextNode,
			!!settings.tryPreserveTypeChecking,
			!!settings.alwaysUseEllipsisForElisions,
			settings.lineNumberStyle ?? SummarizedDocumentLineNumberStyle.None,
		);
		...
		bestSummarizationResultsPerDocIdx.push(
			rootMarkedNode.getTextFragment(),
		);
		...
		const scopeDistanceDown: CachedFunction<TextNode, number> =
			new CachedFunction((node) => {
				if (!offsetSelection) {
					return 0;
				}
				if (node.children.length === 0) {
					return node.range.intersectsOrTouches(offsetSelection)
						? 0
						: Number.MAX_SAFE_INTEGER;
				} else {
					return (
						min(
							node.children.map((n) => scopeDistanceDown.get(n)),
						) + 1
					);
				}
			});
		const scopeDistance: CachedFunction<TextNode, number> =
			new CachedFunction((node) => {
				const parentScopeDistance = node.parent
					? scopeDistance.get(node.parent)
					: Number.MAX_SAFE_INTEGER;
		...
		let costFn: (node: TextNode) => number | false = (node) => {
			if (
				tryPreserveTypeChecking &&
				node.node?.kind === 'import_statement'
			) {
				return 0;
			}
			return (
				100 * scopeDistance.get(node) +
				node.depth +
				10 * (distanceScoreToSelection(node) / text.length)
			);
		};

		const costFnOverride =
			typeof settings.costFnOverride === 'object'
				? settings.costFnOverride.createCostFn(document)
				: settings.costFnOverride;
	...
	for (const { node, docIdx } of allNodesWithScores) {
		...
		let totalLength = sumBy(
			rootMarkedNodes,
			(c) => c.getTextFragment().length,
		);
		if (
			settings.lineNumberStyle === SummarizedDocumentLineNumberStyle.Full
		) {
			const textLen = TextLength.sum(
				rootMarkedNodes,
				(c) => c.getTextFragment().textLength,
			);
			const maxLineNumber = docs[docIdx].document.getLineCount();
			const totalLineNumberChars =
				textLen.lineCount * getLineNumberText(maxLineNumber).length; // This is an upper bound approximation.
		...
		bestSummarizationResultsPerDocIdx[docIdx] =
			rootMarkedNodes[docIdx].getTextFragment();
	}
	...
	for (let i = 0; i < bestSummarizationResultsPerDocIdx.length; i++) {
		...
		let e = bestSummarizationResult.toEditFromOriginal(
		...
		if (
			...
			for (let curLine = 1; curLine <= lineCount; curLine++) {
				...
				lineNumberReplacements.push(
					new StringReplacement(
						OffsetRange.emptyAt(offset),
						getLineNumberText(pos.line + 1),
					),
				);
			}
		...
		r.getVisualization = () => ({
			...
			root: toAstNode(rootMarkedNode, (n) => ({
				label:
					(n.node.node?.kind || 'unknown') +
					` (${allNodesWithScores.find((nws) => nws.node === n)?.cost})`,
				range: n.node.range,
...
function createRemovableNodeFromOverlayNode(
	node: OverlayNode,
	document: AbstractDocument,
	parent: RemovableNode | undefined = undefined,
): RemovableNode {
...
class TextNode {
	public static fromRootNode(
		...
		const endGap = new OffsetRange(
			node.range.endExclusive,
			document.length,
		);
		...
		if (!startGap.isEmpty) {
			children.push(
				new TextNode(undefined, startGap, [], 0, rootNode, document),
			);
		}
		children.push(TextNode.fromNode(node, document, 1, null));
		if (!endGap.isEmpty) {
			children.push(
				new TextNode(undefined, endGap, [], 0, rootNode, document),
			);
		}
		return rootNode;
	}

	private static fromNode(
		node: RemovableNode,
		document: AbstractDocument,
		depth = 0,
		...
		if (node.children.length > 0) {
			...
			for (const n of node.children) {
				...
				children.push(
					TextNode.fromNode(n, document, depth + 1, result),
				);
			...
			if (!gap.isEmpty) {
				children.push(
					new TextNode(undefined, gap, [], depth, result, document),
				);
			}
		}
		return result;
...
class SurvivingTextNode {
	public static fromNode(
		...
		return SurvivingTextNode.fromNodeParent(
			node,
			null,
			tryPreserveTypeChecking,
			alwaysUseEllipsisForElisions,
			lineNumberStyle,
		);
	}

	private static fromNodeParent(
		node: TextNode,
		...
		const result = new SurvivingTextNode(
			node,
			parent,
			children,
			tryPreserveTypeChecking,
		...
		for (const child of node.children) {
			const childNode = SurvivingTextNode.fromNodeParent(
				child,
				result,
				tryPreserveTypeChecking,
				alwaysUseEllipsisForElisions,
	...
	private _computeSummarization(): StringFragment {
		if (
			...
			(this._surviving || !this.node.node)
		) {
			return new OriginalStringFragment(
				this.node.range,
				this.node.document.getText(),
			);
		}
		if (!this._surviving) {
		...
		const groups = Array.from(
			groupAdjacentBy(
				this.childNodes.map((n) => ({
					node: n,
					fragment: n.getTextFragment(),
				})),
				(f1, f2) =>
					(f1.fragment.length === 0) === (f2.fragment.length === 0),
			),
		);

		const getOmittedMessage = (omittedRange: OffsetRange): string => {
			if (
				this._lineNumberStyle ===
				SummarizedDocumentLineNumberStyle.OmittedRanges
			) {
				const range = this.node.document
					.getPositionOffsetTransformer()
					.getRange(omittedRange);
		...
		for (let i = 0; i < groups.length; i++) {
			...
			if (isEmpty && i > 0 && i < groups.length - 1) {
				...
				const fullRange = g
					.at(0)!
					.node.node.range.join(g.at(-1)!.node.node.range);

				if (
					prev.fragment instanceof OriginalStringFragment &&
					...
					if (
						startTrimmed.endsWith('{') &&
						endTrimmed.startsWith('}')
					) {
						groups[i - 1][groups[i - 1].length - 1].fragment =
							startTrimmed;
						g.length = 1;

						g[0].fragment = new LiteralStringFragment(
							getOmittedMessage(fullRange),
						);
				...
				if (
					this._alwaysUseEllipsisForElisions ||
					this._lineNumberStyle ===
						SummarizedDocumentLineNumberStyle.OmittedRanges
				) {
					...
					g.length = 1;
					g[0].fragment = new LiteralStringFragment(
						indentation + getOmittedMessage(fullRange) + end,
					);
				}
			}
		}
