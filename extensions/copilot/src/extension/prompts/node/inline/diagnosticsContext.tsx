			...
			diagnostics.length > 0 && (
				<>
					{diagnostics.map((d, idx) => {
						const cookbook = this.fixCookbookService.getCookbook(
						...
						return (
							<>
								...
								/>
								{this.props.includeRelatedInfos !== false && (
...
class DiagnosticDescription extends PromptElement<DiagnosticDescriptionProps> {
	render(state: void, sizing: PromptSizing) {
		...
		const content = document
			.getText(new Range(range.start.line, 0, range.end.line + 1, 0))
			.trimEnd();
		const code =
		...
		return (
			<>
				{code ? (
					<>
						This code at line {range.start.line + 1}
						<br />
						<CodeBlock
							code={code}
							uri={document.uri}
					...
					<>
						At line {range.start.line + 1}
						<br />
					</>
				)}
				has the problem reported:
				<br />
				<Tag name="compileError">{d.message}</Tag>
			</>
		);
	}
}
...
export class DiagnosticRelatedInfo extends PromptElement<DiagnosticRelatedInfoProps> {
	...
	async render(_state: void, sizing: PromptSizing) {
		const { infos, ignoredFiles, definitionRanges } =
			await this.getRelatedInfos();
			...
			<>
				...
				{infos.map((info) => (
					<CodeBlock
						...
						references={[
							new PromptReference(
								new Location(info.uri, info.range),
							),
						]}
						includeFilepath={true}
					/>
				))}
				{definitionRanges.map((range) => (
		...
		if (diagnostic.relatedInformation) {
			for (const relatedInformation of diagnostic.relatedInformation) {
				try {
					...
					if (
						await this.ignoreService.isCopilotIgnored(location.uri)
					) {
					...
					const treeSitterAST =
						this.parserService.getTreeSitterAST(document);
					let relatedCodeText: string | undefined;
					if (treeSitterAST) {
						const treeSitterLocationRange =
							vscodeToTreeSitterRange(locationRange);
						const rangeOfInterest =
							await treeSitterAST.getCoarseParentScope(
								treeSitterLocationRange,
							);
						relatedCodeText = document.getText(
							treeSitterToVSCodeRange(rangeOfInterest),
						);
					}
					if (
						!relatedCodeText ||
						relatedCodeText.length > RELATED_INFO_MAX_SIZE
					) {
						relatedCodeText = document.getText(locationRange);
					}
					if (relatedCodeText.length <= RELATED_INFO_MAX_SIZE) {
						infos.push({
							content: relatedCodeText,
							uri: location.uri,
					...
					if (treeSitterAST) {
						...
						const expressionInfos =
							await asyncComputeWithTimeBudget(
								...
								500,
								() =>
									treeSitterAST.getCallExpressions(
...
export class DiagnosticSuggestedFix extends PromptElement<DiagnosticSuggestedFixProps> {
	render(state: void, sizing: PromptSizing) {
		...
		if (suggestedFixes.length) {
			...
			return (
				<Tag name="suggestedFix">{prompt.title + prompt.message}</Tag>
			);
		}
		return null;
	}
