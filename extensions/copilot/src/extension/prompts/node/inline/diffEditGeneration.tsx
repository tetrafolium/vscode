	...
	render() {
		return (
			<>
				...
				After that sentence, add an empty line. Then add a code block
				with the fix.
				<br />
				When proposing a change in the code, use a single code block
				that starts with ```diff and that describes the changes in the
				diff format. In the diff, always use tab to indent, never
				spaces. Make sure that the diff format is valid and contains all
				changes: Removed and unchanged lines must match exactly the
				original code line. Keep the changes minimal and the diff short.
				<br />
				When proposing to fix the problem by running a terminal command,
				provide the terminal script in a code block that starts with
				```bash.
				<br />
			</>
		);
...
export class EditGenerationExampleSolution extends PromptElement {
	render() {
		return (
			<>
				The problem is that the class 'C' does not have a field or
				property named 'Field'. To fix this, you need to add a 'Field'
				property to the 'C' class.
				<br />
				<br />
				```diff
				<br />
...
export interface ReplyProcessor {
	...
	process(
		...
		documentText: string,
		lineRange: LineRange,
	): ReplyProcessorResult;
}

export type ReplyProcessorResult = {
	edits?: TextEdit[];
	content?: string;
...
export function getReplyProcessor(): ReplyProcessor {
	return {
		...
		process(
			...
			if (
				!extractResult ||
				extractResult.language === 'bash' ||
				extractResult.language === 'ps1'
			) {
			...
			if (extractResult.language === 'diff') {
				...
				lineEdits = [
					new LinesEdit(
						...
						Lines.fromString(extractResult.code),
					),
				];
			}
...
function extractAndParseFirstCodeBlock(
	text: string,
): { code: string; contentBeforeCode: string; language: string } | undefined {
	...
	if (firstBlock) {
		...
		return {
			...
			contentBeforeCode: lines
				.slice(0, firstBlock.startLine)
				.join('\n')
				.trimEnd(),
