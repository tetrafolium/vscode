	...
	async render(state: void, sizing: PromptSizing) {
		...
		if (
			...
			throw illegalArgument(
				'InlineChatGenerateMarkdownPrompt should not be used with a notebook!',
			);
		}

		if (languageId !== 'markdown') {
			throw illegalArgument(
				'InlineChatGenerateMarkdownPrompt should only be used with markdown documents!',
			);
		}
		...
		const replyInterpreterFn = (splitDoc: SummarizedDocumentSplit) =>
		...
		return (
			<>
				...
				<SystemMessage priority={1000}>
					...
					Copilot".
					<br />
					You are a world class markdown editor, very well versed in
					programming.
					<br />
					<LegacySafetyRules />
				</SystemMessage>
				...
				>
					<InstructionMessage priority={1000}>
						...
						{MarkdownBlock.FenceSequence}.<br />
						{data.hasContent && (
							<>
								The user includes existing markdown and marks
								with {data.placeholderText} where the new code
								should go.
								<br />
							</>
						)}
						{data.hasContent && (
							<>
								DO NOT include the text "{data.placeholderText}"
								in your reply.
								<br />
							</>
						)}
						{data.hasContent && (
							<>
								DO NOT repeat any markdown from the user in your
								reply.
								<br />
							</>
						)}
				...
				>
					...
					{data.hasContent && (
						<>
							...
							{MarkdownBlock.FenceSequence}. The markdown that
							would fit at {data.placeholderText} is:
						</>
					)}
				</UserMessage>
			</>
