...
import {
	SelectionSplitKind,
	SummarizedDocumentData,
} from '../../../intents/node/testIntent/summarizedDocumentWithSelection';
...
class InlineChatNotebookEditSelection extends PromptElement<
	InlineChatNotebookEditSelectionProps,
	InlineChatNotebookSelectionState
> {
	constructor(
	...
	render(state: InlineChatNotebookSelectionState, sizing: PromptSizing) {
		...
		const { hasCodeWithoutSelection, codeWithoutSelection, codeSelected } =
			this.props;
						...
						<UserMessage>
							{isMarkdown ? (
								<>
									...
									<br />
									{hasCodeWithoutSelection && (
										<>
											<CodeBlock
												uri={doc.uri}
												languageId={lang.languageId}
												code={codeWithoutSelection}
												shouldTrim={false}
											/>
											<br />
										</>
...
export class InlineChatNotebookEditPrompt extends PromptElement<
	InlineChatEditCodePromptProps,
	InlineChatNotebookAlternativeEditPromptState
> {
	constructor(
	...
	override async prepare(
		sizing: PromptSizing,
	): Promise<InlineChatNotebookAlternativeEditPromptState> {
		...
		const notebook = findNotebook(
			currentDocumentContext.document.uri,
			this.workspaceService.notebookDocuments,
		);

		const isIgnored = await this.ignoreService.isCopilotIgnored(
			activeDocumentContext.document.uri,
		);
		const wholeRange = activeDocumentContext.document.validateRange(
			activeDocumentContext.wholeRange,
		);
		...
		return {
			notebook,
			isJupyterNotebook: isJupyterNotebookUri(
				currentDocumentContext.document.uri,
			),
			summarizedDocument,
			isIgnored,
	...
	async render(
		state: InlineChatNotebookAlternativeEditPromptState,
		sizing: PromptSizing,
	) {
		if (!state.notebook) {
			throw illegalArgument(
				'InlineChatNotebookEditPrompt should be used only with a notebook!',
			);
		}
		...
		const isMarkdown = lang.languageId === 'markdown';
		const splitDoc =
			state.summarizedDocument.splitAroundAdjustedSelection();
		const { codeAbove, codeSelected, codeBelow, hasCodeWithoutSelection } =
			splitDoc;
		...
		return (
			<>
				...
				<SystemMessage priority={priorities.core}>
					...
					{hasCodeWithoutSelection && (
						<>
							...
							{data.placeholderText} where the selected code
							should go.
							<br />
						</>
					)}
				...
				<InlineChatNotebookEditSelection
					...
					hasCodeWithoutSelection={hasCodeWithoutSelection}
					codeWithoutSelection={codeWithoutSelection}
					codeSelected={codeSelected}
					priority={priorities.core}
					tagBasedDocumentSummary={tagBasedDocumentSummary}
				/>
				<InlineChatNotebookVariables
					notebookURI={context.document.uri}
				...
				<UserMessage>
					...
					/>
					<br />
					{hasCodeWithoutSelection && isMarkdown && (
						<TextChunk priority={priorities.core}>
							The modified {data.placeholderText} code without ```
							is:
						</TextChunk>
					)}
					{hasCodeWithoutSelection && !isMarkdown && (
						<TextChunk priority={priorities.core}>
							The modified {data.placeholderText} code with ```
							is:
						</TextChunk>
					)}
				</UserMessage>
