...
} from '../../../../platform/languages/common/languageDiagnosticsService';
...
import {
	...
	isNotebookCellOrNotebookChatInput,
...
import {
	...
	findFixRangeOfInterest,
	generateFixContext,
} from '../../../context/node/resolvers/fixSelection';
...
import {
	...
	PatchEditRules,
} from '../codeMapper/patchEditGeneration';
import { JupyterNotebookRules } from '../notebook/commonPrompts';
import {
	ChatToolReferences,
	ChatVariables,
	UserQuery,
} from '../panel/chatVariables';
...
import {
	NotebookPromptPriority,
	promptPriorities,
} from './inlineChatNotebookCommon';
import {
	InlineChatCellSelectionProps,
	InlineChatCustomNotebookCellsContextRenderer,
...
export class InlineFixNotebookPrompt extends PromptElement<
	InlineFixProps,
	InlineChatNotebookFixPromptState
> {
	constructor(
		...
		@ILanguageDiagnosticsService
		private readonly _languageDiagnosticsService: ILanguageDiagnosticsService,
	...
	override async prepare(
		sizing: PromptSizing,
	): Promise<InlineChatNotebookFixPromptState> {
	...
	async render(
		state: InlineChatNotebookFixPromptState,
		sizing: PromptSizing,
	) {
		...
		if (!isNotebookCellOrNotebookChatInput(documentContext.document.uri)) {
			throw illegalArgument(
				'InlineFixNotebookPrompt should not be used with a non-notebook!',
			);
		}
		...
		const diagnostics = findDiagnosticForSelectionAndPrompt(
		...
		const range =
			diagnostics.length > 0
				? rangeSpanningDiagnostics(diagnostics)
				: documentContext.selection;

		const treeSitterAST = this._parserService.getTreeSitterAST(
			documentContext.document,
		);
		const rangeOfInterest = treeSitterAST
			? await findFixRangeOfInterest(
					treeSitterAST,
					range,
		...
		if (documentContext.document.getText().length > inputDocCharLimit) {
			...
			projectedDocument = summarizeDocumentSync(
				inputDocCharLimit,
				documentContext.document,
				documentContext.wholeRange,
				structure,
				{ tryPreserveTypeChecking: true },
			);
			isSummarized = true;
		} else {
			projectedDocument = new ProjectedDocument(
				documentContext.document.getText(),
				StringEdit.empty,
				documentContext.document.languageId,
			);
		}

		const adjustedSelection = projectedDocument.projectRange(selection);
		const selectedLinesContent = documentContext.document
			.getText(
				new Range(selection.start.line, 0, selection.end.line + 1, 0),
			)
			.trimEnd();

		const contextInfo = generateNotebookCellContext(
			this._tabsAndEditorsService,
			this._workspaceService,
			documentContext,
			fixContext.contextInfo,
			fixContext.tracker,
		);
		const replyInterpreter = this._instantiationService.createInstance(
		...
		return (
			<>
				...
				<SystemMessage priority={priorities.core}>
					...
					Copilot".
					<br />
					You are a world class expert in programming, and especially
					good at {documentContext.language.languageId}.<br />
				...
				<HistoryWithInstructions
					inline={true}
					passPriority
					historyPriority={priorities.history ?? 700}
					history={history}
				>
					<InstructionMessage priority={priorities.core}>
						...
						When dealing with Jupyter Notebook, do not generate CELL
						INDEX in the code blocks in your answer, it is only used
						to help you understand the context.
						<br />
						If you suggest to run a terminal command, use a code
						block that starts with ```bash.
						<br />
						When fixing "ModuleNotFoundError" or "Import could not
						be resolved" errors, always use magic command "%pip
						install" to add the missing packages. The imports MUST
						be inserted at the top of the code block and it should
						...
						<Tag name="example" priority={100}>
							...
							<Tag name="assistant">
								The problem is that the class 'C' does not have
								a field or property named 'Field'. To fix this,
								you need to add a 'Field' property to the 'C'
								class.
								<br />
								<br />
								<PatchEditExamplePatch
				...
				/>
				<InlineChatNotebookVariables
					notebookURI={this.props.documentContext.document.uri}
					priority={priorities.runtimeCore}
				...
				<UserMessage priority={priorities.core}>
					...
					Please find a fix for my code so that the result is without
					any errors.
					<br />
					<UserQuery chatVariables={chatVariables} query={query} />
					<br />
				</UserMessage>
			</>
		);
...
class InlineChatFixNotebookSelectionRenderer extends PromptElement<InlineChatNotebookSelectionRendererProps> {
	render(state: void, sizing: PromptSizing) {
		if (
			this.props.documentContext.document.uri.scheme !==
			Schemas.vscodeNotebookCell
		) {
			throw illegalArgument(
				'InlineChatNotebookSelectionRenderer should be used only with a notebook!',
			);
		}

		const jupyterNotebook = isJupyterNotebookUri(
			this.props.documentContext.document.uri,
		);
		...
		return (
			<>
				{jupyterNotebook ? (
					<>
						<InlineChatFixNotebookCellsContextRenderer
							documentContext={this.props.documentContext}
							aboveCells={aboveCellsInfo}
							belowCells={belowCellsInfo}
						/>
					</>
				) : (
					<>
				...
				<NotebookCellSelection
					cellIndex={aboveCellsInfo.length}
					document={this.props.document}
					projectedDocument={projectedDocument}
					language={lang}
					diagnostics={this.props.diagnostics}
					selection={this.props.selection}
					adjustedSelection={this.props.adjustedSelection}
					isSummarized={this.props.isSummarized}
					selectedLinesContent={this.props.selectedLinesContent}
				/>
			</>
		);
	}
...
class NotebookCellSelection extends PromptElement<InlineChatCellSelectionProps> {
	override render() {
		...
		const notebookType = isNotebookCellOrNotebookChatInput(document.uri)
			? 'Jupyter'
			: 'custom';
		...
		return (
			<>
				<UserMessage>
					Now I create a new cell in this {notebookType} Notebook
					document at index {this.props.cellIndex}.<br />
					{isMarkdown && (
						<>
							This is a markdown cell. Markdown cell is used to
...
class NotebookCellRenderer extends PromptElement<InlineChatCellSelectionProps> {
	constructor(props: InlineChatCellSelectionProps) {
		super(props);
	}
				...
				<CompositeElement>
					{projectedDocument.text.length > 0 ? (
						<>
							{isMarkdown ? (
								<>
									I have the following markdown content in
									this cell, starting from line 1 to line{' '}
									{projectedDocument.lineCount}.<br />
								</>
							) : (
								<>
									I have the following code in this cell,
				...
				<CompositeElement>
					{selection.isEmpty ? (
						<>
							...
							{adjustedSelection.start.line + 1}, column{' '}
						...
						<>
							...
							{adjustedSelection.start.line + 1}, column{' '}
							{adjustedSelection.start.character + 1} to line{' '}
							{adjustedSelection.end.line + 1} column{' '}
							{adjustedSelection.end.character + 1}.<br />
						</>
					)}
				</CompositeElement>
				<CompositeElement>
					{selectedLinesContent.length &&
						!diagnostics.some((d) =>
							d.range.contains(selection),
						) && (
							<>
...
class InlineChatFixNotebookCellsContextRenderer extends PromptElement<InlineChatFixNotebookCellsContextRendererProps> {
	render(state: void, sizing: PromptSizing) {
		...
		const { aboveCells: aboveCellsInfo, belowCells: belowCellsInfo } =
			this.props;
		const lang = this.props.documentContext.language;
			...
			<>
				{(aboveCellsInfo.length > 0 || belowCellsInfo.length > 0) && (
					<UserMessage>
						...
						<br />
						The content of cells are listed below, each cell starts
						with CELL INDEX and a code block started with ```
						{lang.languageId}
						<br />
						Each cell is a block of code that can be executed
						independently.
						<br />
						Since it is Jupyter Notebook, if a module is already
						imported in a cell, it can be used in other cells as
						well.
						<br />
						For the same reason, if a variable is defined in a cell,
						it can be used in other cells as well.
						<br />
						We should not repeat the same import or variable
						definition in multiple cells, unless we want to
						overwrite the previous definition.
						<br />
						Do not generate CELL INDEX in your answer, it is only
						used to help you understand the context.
						<br />
						<br />
						<>
							Below you will find a set of examples of what you
							should respond with. Please follow the exmaples on
							...
							USER:
							<br />
							Now I create a new cell in this Jupyter Notebook
							document at index 1.
							<br />
							I have the following code open in this cell,
							starting from line 1 to line 1.
							<br />
							```python
							<br />
							```
							...
							---------------------------------
							<br />
							To plot the dataframe, we can use the `plot()`
						...
						{belowCellsInfo.length > 0 && (
							<NotebookCellList
								cells={belowCellsInfo}
								cellIndexDelta={aboveCellsInfo.length + 1}
								title={
