	...
	async render(state: void, sizing: PromptSizing) {
		...
		if (isNotebookCellOrNotebookChatInput(document.uri)) {
			throw illegalArgument(
				'InlineChatGenerateCodePrompt should not be used with a notebook!',
			);
		}

		if (languageId === 'markdown') {
			throw illegalArgument(
				'InlineChatGenerateCodePrompt should not be used with a markdown document!',
			);
		}
		...
		const useProjectLabels =
			this._configurationService.getExperimentBasedConfig(
				ConfigKey.Advanced.ProjectLabelsInline,
			...
			<>
					...
					<InstructionMessage priority={1000}>
						...
						{data.hasContent && (
							<>
								...
								{data.placeholderText} where the new code should
								go.
								<br />
							</>
						)}
				...
				<UserMessage priority={725}>
					...
					<LanguageServerContextPrompt
						priority={700}
						document={document}
						position={context.selection.start}
				...
				>
					...
					{data.hasContent && (
						<>
							The code that would fit at {data.placeholderText}{' '}
							with ``` is:
						</>
					)}
				</UserMessage>
