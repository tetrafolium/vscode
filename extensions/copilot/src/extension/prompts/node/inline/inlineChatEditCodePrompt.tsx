	...
	async render(state: void, sizing: PromptSizing) {
		...
		if (isNotebookCellOrNotebookChatInput(document.uri)) {
			throw illegalArgument(
				'InlineChatEditCodePrompt should not be used with a notebook!',
			);
		}

		if (languageId === 'markdown') {
			throw illegalArgument(
				'InlineChatEditCodePrompt should not be used with a markdown document!',
			);
		}
		...
		const replyInterpreterFactory = (splitDoc: SummarizedDocumentSplit) =>
		...
		return (
			<>
				...
				<UserMessage>
					{!this.props.ignoreCustomInstructions && (
						<CustomInstructions
							priority={725}
