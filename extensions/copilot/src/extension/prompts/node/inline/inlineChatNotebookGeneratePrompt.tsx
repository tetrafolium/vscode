...
import {
	generateSelectionContextInNotebook,
	InlineChatCustomNotebookCellsContextRenderer,
	InlineChatCustomNotebookInfoRenderer,
	InlineChatJupyterNotebookCellsContextRenderer,
	InlineChatJupyterNotebookCellsContextTagBasedRenderer,
	InlineChatNotebookBasePromptState,
	InlineChatNotebookSelectionCommonProps,
	InlineChatNotebookSelectionState,
...
export class InlineChatNotebookGeneratePrompt extends PromptElement<
	InlineChatEditCodePromptProps,
	InlineChatNotebookBasePromptState
> {
	constructor(
	...
	override async prepare(
		sizing: PromptSizing,
	): Promise<InlineChatNotebookBasePromptState> {
	...
	render(state: InlineChatNotebookBasePromptState, sizing: PromptSizing) {
		if (
			...
			throw illegalArgument(
				'InlineChatNotebookBasePrompt should be used only with a notebook!',
			);
		}
		...
		const splitDoc =
			state.summarizedDocument.splitAroundOriginalSelectionEnd();
		...
		const replyInterpreter = splitDoc.createReplyInterpreter(
			...
			extractCodeBlock
				...
				: TextPieceClassifiers.createAlwaysInsideCodeBlockClassifier(),
		...
		return (
			<>
					...
					<InstructionMessage priority={priorities.core}>
						{jupyterNotebook && (
							<>
								...
								{!tagBasedDocumentSummary && (
									<>
										When dealing with Jupyter Notebook, do
										not generate CELL INDEX in the code
										blocks in your answer, it is only used
						...
						{hasContent && (
							<>
								...
								$PLACEHOLDER$ where the new code should go.
								<br />
							</>
						)}
						{hasContent && (
							<>
								DO NOT include the text "$PLACEHOLDER$" in your
								reply.
								<br />
							</>
						)}
						{hasContent && (
							<>
								DO NOT repeat any code from the user in your
								reply.
								<br />
							</>
						)}
						{!hasContent && extractCodeBlock && (
							<>
								Your must generate a code block surrounded with
								``` that will be used in a new file
								<br />
							</>
						)}
						{!extractCodeBlock && (
							<>
								When generating content for markdown cell,
								provide the answer directly without any
								additional introductory text. Ensure that the
								response is structured in Markdown format to
								seamlessly integrate into the markdown file.
							</>
						)}
					</InstructionMessage>
				</HistoryWithInstructions>
				...
				<InlineChatNotebookGenerateSelection
					...
					tagBasedDocumentSummary={tagBasedDocumentSummary}
				/>
				<InlineChatNotebookVariables
				...
				<UserMessage priority={priorities.core}>
					...
					{hasContent && extractCodeBlock && (
						<>
							The code that would fit at $PLACEHOLDER$ with ```
							is:
						</>
					)}
					{hasContent && !extractCodeBlock && (
						<>
							The code that would fit at $PLACEHOLDER$ without ```
							is:
						</>
					)}
				</UserMessage>
...
class InlineChatNotebookGenerateSelection extends PromptElement<
...
> {
	...
	override async prepare(): Promise<InlineChatNotebookSelectionState> {
		...
		const inSummaryExperiment =
			...
			this.configurationService.getConfig(
				ConfigKey.Advanced.NotebookSummaryExperimentEnabled,
			);

		let executedCells: vscode.NotebookCell[] | undefined = undefined;
		if (
			inSummaryExperiment &&
			this.tabsAndEditorsService.activeNotebookEditor?.notebook &&
			this.tabsAndEditorsService.activeNotebookEditor?.notebook.uri
				.path === document.uri.path
		) {
			// experiment new notebook summary
			executedCells = this.notebookService.getCellExecutions(
				this.tabsAndEditorsService.activeNotebookEditor.notebook.uri,
			);
		}
	...
	render(state: InlineChatNotebookSelectionState, sizing: PromptSizing) {
		if (
			...
			throw illegalArgument(
				'InlineChatNotebookSelection should be used only with a notebook!',
			);
		}
				...
				{jupyterNotebook ? (
					<>
						{executedCells.length > 0 && (
							<InlineChatJupyterNotebookCellSummaryContextRenderer
								documentContext={this.props.documentContext}
						...
						<UserMessage>
							{isMarkdown ? (
								<>
									{hasContent && (
										<>
											Now I edit a markdown cell in this
											Jupyter Notebook document at index{' '}
											{aboveCellsInfo.length}.<br />
										</>
									)}
									{!hasContent && (
										<>
											Now I create a new markdown cell in
											this Jupyter Notebook document at
											index {aboveCellsInfo.length}.<br />
										</>
									)}
									This is a markdown cell. Markdown cell is
									used to describe and document your workflow.
									<br />
									{hasContent && (
										<>
											<CodeBlock
								...
								<>
									{hasContent && (
										<>
											Now I edit a cell in this Jupyter
							...
							{isMarkdown ? (
								<>
									{hasContent && (
										<>
											Now I edit a markdown cell in this
											custom Notebook document at index{' '}
											{aboveCellsInfo.length}.<br />
										</>
									)}
									{!hasContent && (
										<>
											Now I create a new markdown cell in
											this custom Notebook document at
											index {aboveCellsInfo.length}.<br />
										</>
									)}
									This is a markdown cell. Markdown cell is
								...
								<>
									{hasContent && (
										<>
											Now I edit a cell in this custom
											Notebook document at index{' '}
											{aboveCellsInfo.length}.<br />
										</>
									)}
...
class InlineChatJupyterNotebookCellSummaryContextRenderer extends PromptElement<InlineChatJupyterNotebookCellSummaryContextRendererProps> {
	render(state: void, sizing: PromptSizing) {
		if (
			...
			throw illegalArgument(
				'InlineChatJupyterNotebookCellSummaryContextRenderer should be used only with a notebook!',
			);
		}
		const lang = this.props.documentContext.language;
			...
			<>
				{
					<UserMessage>
						...
						---------------------------------
						<br />
						To plot the dataframe, we can use the `plot()` method of
						pandas dataframe. Here's the code:
						<br />
						<br />
						```python
						<br />
...
class NotebookCellContent extends PromptElement<
	{ cell: vscode.NotebookCell } & BasePromptElementProps
> {
	override render() {
		return (
			<>
