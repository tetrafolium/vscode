...
import {
	...
	IMapCodeRequest,
	IMapCodeResult,
} from '../codeMapper/codeMapperService';
import {
	getCustomMarker,
	getPatchEditReplyProcessor,
	PatchEditExamplePatch,
	PatchEditInputCodeBlock,
	PatchEditInputCodeBlockProps,
...
export class InlineFix3Prompt extends PromptElement<InlineFixProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		const { document, wholeRange, fileIndentInfo, selection, language } =
		...
		if (isNotebookCellOrNotebookChatInput(document.uri)) {
			throw illegalArgument(
				'InlineFix3PlusPrompt should not be used with a notebook!',
			);
		}
		...
		const selectedLinesContent = document
			.getText(
		...
		const enableCodeMapper = this.configurationService.getConfig(
			ConfigKey.TeamInternal.InlineChatUseCodeMapper,
		);
		...
		const GenerationRulesAndExample = enableCodeMapper
			? CodeMapperRulesAndExample
			: PatchEditFixRulesAndExample;
		const InputCodeBlock = enableCodeMapper
		...
		return (
			<>
				...
				<SystemMessage priority={1000}>
					...
					Copilot".
					<br />
					The user has a {language.languageId} file opened in a code
					editor.
					<br />
					The user expects you to propose a fix for one or more
				...
				>
					<InstructionMessage priority={1000}>
						...
						problem. After that sentence, add an empty line. Then
						provide code changes or a terminal command to run.
						<br />
				...
				<UserMessage priority={700}>
					<CustomInstructions
						/*priority={700}*/ languageId={language.languageId}
					...
					<LanguageServerContextPrompt
						...
						source={KnownSources.fix}
					/>
					<CompositeElement priority={750}>
						{...renderedChatVariables}
					</CompositeElement>
					...
					<Diagnostics
						/*priority={500}*/ documentContext={
							this.props.documentContext
						}
						diagnostics={diagnostics}
					/>
					<InlineChatWorkspaceSearch
						/*priority={200}*/ diagnostics={diagnostics}
					...
					<Tag name="userPrompt">
						<TextChunk /*priority={700}*/>
							Please find a fix for my code so that the result is
							without any errors.
						</TextChunk>
						<UserQuery
							chatVariables={chatVariables}
...
class PatchEditFixRulesAndExample extends PromptElement {
	render() {
		return (
			<>
				When proposing to fix the problem by running a terminal command,
				write `{getCustomMarker('TERMINAL')}` and provide a code block
				that starts with ```bash and contains the terminal script
				inside.
				<br />
				<PatchEditRules />
				<Tag name="example" priority={100}>
					...
					<Tag name="assistant">
						The problem is that the class 'C' does not have a field
						or property named 'Field'. To fix this, you need to add
						a 'Field' property to the 'C' class.
						<br />
						<br />
						<PatchEditExamplePatch
							changes={[
...
export class PatchEditFixReplyInterpreter implements ReplyInterpreter {
	...
	constructor(
		...
		this._replyProcessor = getPatchEditReplyProcessor(
	...
	async processResponse(
		...
		const res = this._replyProcessor.process(
			this._lastText,
			this.projectedDocument.text,
			this.documentUri,
		...
		if (edits.length) {
			...
			if (res.edits.length !== edits.length) {
				res.annotations.push({
					message:
						'Some edits were not applied because they were out of bounds.',
					label: OutcomeAnnotationLabel.SUMMARIZE_CONFLICT,
					severity: 'error',
				});
			} else {
				const annot = this._validateTextEditProject(
					res.edits,
					edits,
					this.projectedDocument,
				);
				if (annot) {
	...
	private _validateTextEditProject(
		edits: TextEdit[],
		projectedBackEdits: TextEdit[],
		projectedDocument: ProjectedDocument,
	): OutcomeAnnotation | undefined {
		for (let i = 0; i < edits.length; i++) {
			const projEditString = projectedDocument.positionOffsetTransformer
				.toOffsetRange(edits[i].range)
				.substring(projectedDocument.text);
			const origEditString =
				projectedDocument.originalPositionOffsetTransformer
					.toOffsetRange(projectedBackEdits[i].range)
					.substring(projectedDocument.originalText);
			if (projEditString !== origEditString) {
				return {
					message: `Problem projecting edits: '${projEditString}' does not match '${origEditString}' (projectecBack)`,
					label: OutcomeAnnotationLabel.INVALID_PROJECTION,
					severity: 'error',
				};
			}
		}
...
class CodeMapperRulesAndExample extends PromptElement {
	render() {
		return (
			<>
				When proposing to fix the problem by running a terminal command,
				provide a code block that starts with ```bash and contains the
				terminal script inside.
				<br />
...
class CodeMapperFixReplyInterpreter implements ReplyInterpreter {
	...
	async processResponse(
		...
		const codeblockProcessor = new CodeBlockProcessor(
			(path) => {
				return this.promptPathRepresentationService.resolveFilePath(
					path,
				);
			},
			(
			...
			(codeBlock) => {
				if (
					isCodeBlockWithResource(codeBlock) &&
					isEqual(codeBlock.resource, this.documentUri)
				) {
		...
		for await (const { delta } of inputStream) {
			...
			codeblockProcessor.processMarkdown(
				delta.text,
		...
		for (const result of results) {
			if (result && result.errorDetails) {
				outputStream.warning(
					`CodeMapper error: ${result.errorDetails}`,
				);
			}
		}
	}
