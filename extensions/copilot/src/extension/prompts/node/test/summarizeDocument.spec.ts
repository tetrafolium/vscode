...
import {
	...
	fixture,
	fromFixtureOld,
	generateSummarizedDocument,
	generateSummarizedDocumentAndExtractGoodSelection,
	generateSummarizedDocuments,
	getSummarizedSnapshotPath,
	loadFile,
	selectionDocPathInFixture,
	summarizedDocPathInFixture,
} from './utils';
...
describe('createSummarizedDocument[visualizable]', () => {
	test('[tsx] can summarize public fields', async () => {
		...
		await expect(result.text).toMatchFileSnapshot(
			summarizedDocPathInFixture(filename),
		);
	});

	test('[cpp] CppNoExtraSemicolons', async () => {
		const file = await loadFile({
			filePath: fixture('cppNoExtraSemicolons.cpp'),
			languageId: 'cpp',
		});
		const result = await generateSummarizedDocument(file, [50, 0], 628, {
			alwaysUseEllipsisForElisions: true,
		});
		await expect(result.text).toMatchFileSnapshot(
			getSummarizedSnapshotPath(file),
		);
	});

	test('[cpp] Do not throw invalid range error', async () => {
		const file = await loadFile({
			filePath: fixture('problem1.cpp'),
			languageId: 'cpp',
		});
		const result = await generateSummarizedDocument(file, [17, 10], 10, {
			alwaysUseEllipsisForElisions: true,
		});
		await expect(result.text).toMatchFileSnapshot(
			getSummarizedSnapshotPath(file),
	...
	test('[cpp] Do not throw invalid range error - 2', async () => {
		const file = await loadFile({
			filePath: fixture('problem2.cpp'),
			languageId: 'cpp',
		});
		const result = await generateSummarizedDocument(file, [6, 19], 100, {
			alwaysUseEllipsisForElisions: true,
		});
		await expect(result.text).toMatchFileSnapshot(
			getSummarizedSnapshotPath(file),
	...
	test('should include nearby code - strings.test.ts', async () => {
		const file = await loadFile({
			filePath: fixture('strings.test-example.ts'),
	...
	test('should include nearby code - strings.test.ts - SummarizedDocumentLineNumberStyle.OmittedRanges', async () => {
		...
		await expect(result.text).toMatchFileSnapshot(
			getSummarizedSnapshotPath(file, '2'),
		);
	});
	...
	test('should include nearby code - strings.test.ts - SummarizedDocumentLineNumberStyle.Full', async () => {
		...
		const result = await generateSummarizedDocument(
			...
			[344, 0],
			undefined,
			{ lineNumberStyle: SummarizedDocumentLineNumberStyle.Full },
		);
		await expect(result.text).toMatchFileSnapshot(
			getSummarizedSnapshotPath(file, '3'),
		);
	});

	test('should include whitespace touching the selection - codeEditorWidget.ts', async () => {
		const file = await loadFile({
			filePath: fixture('codeEditorWidget.ts'),
			languageId: 'typescript',
		});
		const result = await generateSummarizedDocument(
			file,
		...
		await expect(result.text).toMatchFileSnapshot(
			getSummarizedSnapshotPath(file, '1'),
		);
	});
	...
	test('should select at least one node - codeEditorWidget.ts', async () => {
		const file = await loadFile({
			filePath: fixture('editorGroupWatermark.ts'),
			languageId: 'typescript',
		});
	...
	test('issue #6614: Should include ... only once when eliding code', async () => {
		...
		const result = await generateSummarizedDocument(
			...
			[225, 0, 237, 15],
			0,
			{ alwaysUseEllipsisForElisions: false },
		);
		await expect(result.text).toMatchFileSnapshot(
			getSummarizedSnapshotPath(file),
		);
	...
	test('should expand selection end to closing brace - extHost.api.impl.ts', async () => {
		...
		await expect(otherCode).toMatchFileSnapshot(
			summarizedDocPathInFixture(filename),
		);
		await expect(selectedCode).toMatchFileSnapshot(
			selectionDocPathInFixture(filename),
		);
	});
...
describe('cutoff cost', () => {
	...
	test('cutoff cost is respected', async () => {
		...
		await expect(result.text).toMatchFileSnapshot(
			summarizedDocPathInFixture(filename) + '.view-port',
		);
	});
});
...
describe('createSummarizedDocuments', () => {
	test('summarize two document equally', async () => {
		...
		const files = [
			await loadFile({
				filePath: fixture(filenames[0]),
				languageId: 'typescript',
			}),
			await loadFile({
				filePath: fixture(filenames[1]),
				languageId: 'typescript',
			}),
		];
		...
		for (let i = 0; i < docs.length; i++) {
			...
			await expect(document.text).toMatchFileSnapshot(
				summarizedDocPathInFixture(filenames[i] + '.round1'),
			);
		}
	});
	...
	test('summarize two document un-equally', async () => {
		...
		for (let i = 0; i < docs.length; i++) {
			...
			expect(document.originalText).toBe(files[i].contents);
			await expect(document.text).toMatchFileSnapshot(
				summarizedDocPathInFixture(filenames[i] + '.round2'),
			);
		}
	});
	...
	test.skip('run on repositories', async () => {
		...
		const reposWithLangs: Record<
			string,
			{ repoPath: string; language: string }
		> = {
			'vscode-copilot': {
				repoPath: path.join(__dirname, '../../../../../src/'),
				language: 'typescript',
			},
			'llama.cpp': {
				repoPath: path.join(
		...
		async function* traverseDirectory(
			...
			const dirEntries = await fs.readdir(pathToDir, {
				withFileTypes: true,
			});
			for (const entry of dirEntries) {
				if (entry.isDirectory()) {
					...
					exts.some(
						(ext) =>
							!entry.parentPath.includes('fixture') &&
							!entry.name.includes('.summarized.') &&
		...
		for await (const filePath of traverseDirectory(repoPath)) {
			...
			try {
				...
				const selection: [number, number] = [
					Math.floor(fileLines.length / 2),
					Math.floor(
						fileLines[Math.floor(fileLines.length / 2)].length / 2,
					),
				];
				const result = await generateSummarizedDocument(
					file,
