...
export function getSummarizedSnapshotPath(
	data: ITestFile,
	version?: string,
): string {
...
export async function generateSummarizedDocument(
	...
	const doc = TextDocumentSnapshot.create(
	...
	const playgroundRunnerData = (globalThis as any as IPlaygroundRunnerGlobals)
	...
	if (playgroundRunnerData) {
		...
		globalThis.playground = new SummarizeDocumentPlayground(
			...
			(text) =>
				parserService
					.getTreeSitterAST(getDoc(text).document)!
					.getStructure(),
			(text, charLimit, selection, structure) =>
		...
		g.$$debugValueEditor_properties = [
			...
			},
			{
				...
				expression: getExprText(() =>
					globalThis.playground!.getSummarizedText(),
				),
			},
			{
			...
			},
			{
				...
				expression: getExprText(
					() => globalThis.playground!.inputDocument,
				),
			},
			{
				label: 'Input Options',
				expression: getExprText(
					() => globalThis.playground!.inputOptions,
				),
			},
		];
	...
	return {
		...
		adjustedSelection: selections
			? summarizedDoc.projectOffsetRange(selections.adjusted)
			: new OffsetRange(0, 0),
	};
}
export async function generateSummarizedDocuments(
	...
	return summarizeDocumentsSync(charLimit, settings, items);
}
