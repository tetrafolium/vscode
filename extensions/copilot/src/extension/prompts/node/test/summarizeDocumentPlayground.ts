...
export class SummarizeDocumentPlayground {
	...
	private readonly _range = observableValue<Range>(
		this,
		new Range(0, 0, 0, 0),
	);
	private readonly _charLimit = observableValue<number>(this, 10);

	private readonly _initialResult = observableValue<
		IProjectedDocumentDebugInfo | undefined
	>(this, undefined);
	...
	constructor(
		...
		private readonly _getUpdatedStructure: (
			text: string,
	...
	get inputDocument(): ITextRangeDoc {
		return {
			...
			range: {
				start: {
					lineNumber: this._range.get().start.line + 1,
					column: this._range.get().start.character + 1,
				},
				end: {
					lineNumber: this._range.get().end.line + 1,
					column: this._range.get().end.character + 1,
				},
			},
		};
	}
	...
	set inputDocument(value: ITextRangeDoc) {
		transaction((tx) => {
			...
			this._range.set(
				new Range(
					value.range.start.lineNumber - 1,
					value.range.start.column - 1,
					value.range.end.lineNumber - 1,
	...
	private readonly _structure = derived(this, (reader) => {
		return observableFromPromise(
			this._getUpdatedStructure(this._text.read(reader)),
		);
	});
	...
	private readonly _result = derived(this, (reader) => {
		...
		return this._getUpdatedResult(
			...
			this._charLimit.read(reader),
			this._range.read(reader),
			structure,
		);
	}).keepObserved(this._store);

	getAst() {
		return waitForStateOrReturn(
			this._result.map((r) => (!r ? undefined : r.getVisualization?.())),
			isDefined,
		);
	}
	...
	getSummarizedText() {
		return waitForStateOrReturn(
			this._result.map((r) => (!r ? undefined : r.text)),
			isDefined,
		);
	}
}
 ...
 */
export function waitForStateOrReturn<T, TState extends T>(
	observable: IObservable<T>,
	predicate: (state: T) => state is TState,
): Promise<TState> | TState;
export function waitForStateOrReturn<T>(
	observable: IObservable<T>,
