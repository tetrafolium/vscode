...
export class Capabilities extends PromptElement<
	CapabilitiesProps,
	CapabilitiesState
> {
	constructor(
	...
	private getIntentDescription(id: string) {
		const intent = this.commandService.getCommand(
			id,
			this.props.location,
		)?.intent;
		return !intent || intent.isListedCapability === false
			? undefined
			: intent.description;
	}
	...
	override async prepare() {
		...
		const commandDescriptions = Object.entries(agentsToCommands).reduce(
			(prev, [agent, commands]) => {
				...
				for (const command of Object.values(commands)) {
					const commandDescription =
						this.getIntentDescription(command);
	...
	render(state: CapabilitiesState) {
		return (
			<>
				You can answer general programming questions and perform the
				following tasks: {state.commandDescriptions}
				<br />
