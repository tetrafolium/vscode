...
export class ResponseTranslationRules extends PromptElement {
	...
	render() {
		const languageOverride = this.configurationService.getConfig<string>(
			ConfigKey.LocaleOverride,
		); // Locale overrides must be for one of our supported languages
		...
		const languageConfiguration =
			languageOverride !== 'auto'
				? languageOverride
				: this.envService.language; // No need to further validate VS Code's configured locale
