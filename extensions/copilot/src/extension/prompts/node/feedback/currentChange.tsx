...
export class CurrentChange extends PromptElement<
	CurrentChangeProps,
	CurrentChangeState
> {
	constructor(
	...
	override async prepare(sizing: PromptSizing): Promise<CurrentChangeState> {
		...
		for (const input of this.props.input) {
			if (
				!(await this.ignoreService.isCopilotIgnored(input.document.uri))
			) {
				allowed.push(input);
			}
		}

		const texts: Text[] = await Promise.all(
			allowed.map(async (input) => {
				...
				if (change?.hunks.length) {
					...
					textAll = [
						...
						...change.hunks
							.map((hunk, i, a) => {
								const nextHunkLine =
									i + 1 < a.length
										? a[i + 1].range.start.line
										: document.lineCount;
								return [
									CurrentChange.enumeratedChangeLines(
										hunk.text,
										hunk.range.start.line,
									),
									'\n',
									hunk.range.end.line < nextHunkLine
										? CurrentChange.enumeratedLines(
												document,
												hunk.range.end.line,
												nextHunkLine,
					...
					const selectionEndLine =
						selection.end.line +
						(selection.end.character > 0 ? 1 : 0); // Being line-based.
					textAll = CurrentChange.enumeratedSelectedLines(
						document,
						0,
						document.lineCount,
					...
					textAll = CurrentChange.enumeratedLines(
						document,
						0,
						document.lineCount,
					);
		...
		for (const text of sorted) {
			...
			if (change?.hunks.length) {
				...
				for (const hunk of change.hunks) {
					...
					if (definition) {
						const definitionEndLine =
							definition.range.end.line +
							(definition.range.end.character > 0 ? 1 : 0); // Being line-based.
						const hunkEndLine =
							hunk.range.end.line +
							(hunk.range.end.character > 0 ? 1 : 0); // Being line-based.
						const textDefinition = [
							hunk.range.start.line > definition.range.start.line
								? CurrentChange.enumeratedLines(
										document,
						...
						definitionHunks.push({
							range: new Range(
								...
								0,
								Math.max(definitionEndLine, hunkEndLine),
								0,
							),
							text: textDefinition,
						});
						definitionTokens +=
							await sizing.countTokens(textDefinition);
					} else {
						const hunkText = CurrentChange.enumeratedChangeLines(
							hunk.text,
							hunk.range.start.line,
						);
						const hunkEndLine =
				...
				if (definition) {
					...
					const selectionEndLine =
						...
						(selection.end.character > 0 ? 1 : 0); // Being line-based.
					const textDefinition =
						CurrentChange.enumeratedSelectedLines(
							document,
							definition.range.start.line,
							definitionEndLine,
							selection.start.line,
					...
					const selectionEndLine =
						selection.end.line +
						(selection.end.character > 0 ? 1 : 0); // Being line-based.
					const hunkText = CurrentChange.enumeratedSelectedLines(
						document,
						selection.start.line,
						selectionEndLine,
						selection.start.line,
						...
						<>
							{input.input.change ? (
								<>
									...
									<br />
									<br />
									Each line is annotated with the line number
									in the file.
									<br />
								</>
							) : (
								<>
									Current selection with the selected lines
									labeled as such:
									<br />
								</>
							)}
							<br />
							From the file: {input.input.relativeDocumentPath}
							<br />
							{input.hunks.map((hunk) => (
								<CodeBlock
									references={[
										new PromptReference(
											new Location(
												input.input.document.uri,
												hunk.range,
											),
										),
									]}
									uri={input.input.document.uri}
									code={hunk.text}
									languageId={`${input.input.document.languageId}/${input.input.relativeDocumentPath}: FROM_LINE: ${hunk.range.start.line + 1} - TO_LINE: ${hunk.range.end.line}`}
								/>
							))}
							<br />
							<br />
	...
	static async getCurrentChanges(
		...
		const changes = await Promise.all(
			git.repositories.map(async (repository) => {
				const stats = await (group === 'index'
					? repository.diffIndexWithHEAD()
					: group === 'workingTree'
						? repository.diffWithHEAD()
						: repository.diffWith('HEAD'));
				const changes = await Promise.all(
					stats.map(async (change) => {
						const text = await (group === 'index'
							? repository.diffIndexWithHEAD(change.uri.fsPath)
							: repository.diffWithHEAD(change.uri.fsPath));
						return {
							repository,
							uri: change.uri,
							hunks: CurrentChange.parseDiff(text).map((hunk) =>
								CurrentChange.gitHunkToHunk(hunk),
							),
						} satisfies Change;
					}),
				);
	...
	static async getChanges(
		...
		const repository = git.repositories.find(
			(r) =>
				r.rootUri.toString().toLowerCase() ===
				repositoryUri.toString().toLowerCase(),
		);
		if (!repository) {
			return;
		}
	...
	private static gitHunkToHunk(hunk: GitHunk): Hunk {
		const range = new Range(
			hunk.startAddedLine - 1,
			0,
			hunk.startAddedLine - 1 + hunk.addedLines,
			0,
		);
		return {
			range,
	...
	private static parseDiff(diff: string): GitHunk[] {
		...
		if (
			hunkTexts.length &&
			hunkTexts[hunkTexts.length - 1].endsWith('\n')
		) {
			hunkTexts[hunkTexts.length - 1] = hunkTexts[
				hunkTexts.length - 1
			].slice(0, -1);
		}
		const hunks = hunkTexts
			.map((chunk) => {
					...
					for (const line of lines) {
						...
						if (ch === '+') {
							...
							if (addStart !== undefined) {
								additions.push({
									start: addStart,
									length: startAddedLine + d - addStart,
								});
								addStart = undefined;
							}
							if (ch === ' ') {
	...
	private static enumeratedLines(
		document: TextDocumentSnapshot,
		startLine: number,
		endLine: number,
	) {
		...
		const code = lines
			.map((line, i) =>
				i === endLine - startLine
					? line
					: `/* Line ${startLine + i + 1} */${line}`,
			)
			.join('\n');
		return code;
	}

	private static enumeratedSelectedLines(
		document: TextDocumentSnapshot,
		startLine: number,
		endLine: number,
		startSelectionLine: number,
		endSelectionLine: number,
	) {
		const text = document.getText(new Range(startLine, 0, endLine, 0));
		const lines = text.split('\n');
		const code = lines
