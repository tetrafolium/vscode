...
> {
	...
	override async prepare() {
		...
		if (document?.uri && selection) {
			const severeDiagnostics = this.languageService.getDiagnostics(
				document.uri,
			);
			const diagnosticsInSelection = severeDiagnostics.filter(
				(d) => !!d.range.intersection(selection),
			);
			const filteredDiagnostics = diagnosticsInSelection.filter(
	...
	override render(
		state: ExplainPromptState,
		sizing: PromptSizing,
	): PromptPiece<any, any> | undefined {
		let { query, history, chatVariables } = this.props.promptContext;
		chatVariables = chatVariables.filter(
			(v) => !v.reference.id.startsWith('vscode.implicit'),
		);
		return (
			<>
				<SystemMessage priority={1000}>
					You are a world-class coding tutor. Your code explanations
					perfectly balance high-level concepts and granular details.
					Your approach ensures that students not only understand how
					to write code, but also grasp the underlying principles that
					guide effective programming.
					<br />
				...
				>
					<InstructionMessage priority={1000}>
						...
						Think step by step:
						<br />
						1. Examine the provided code selection and any other
						context like user question, related errors, project
						details, class definitions, etc.
						<br />
						2. If you are unsure about the code, concepts, or the
						user's question, ask clarifying questions.
						<br />
						3. If the user provided a specific question or error,
						answer it based on the selected code and additional
						provided context. Otherwise focus on explaining the
						selected code.
						<br />
						4. Provide suggestions if you see opportunities to
						improve code readability, performance, etc.
						<br />
						<br />
						Focus on being clear, helpful, and thorough without
						assuming extensive prior knowledge.
						<br />
						Use developer-friendly terms and analogies in your
						explanations.
						<br />
						Identify 'gotchas' or less obvious parts of the code
						that might trip up someone new.
						<br />
						Provide clear and relevant examples aligned with any
						provided context.
						<br />
