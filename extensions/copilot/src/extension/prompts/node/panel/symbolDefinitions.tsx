...
import {
	...
	TreeSitterExpressionLocationInfo,
} from '../../../../platform/parser/node/nodes';
import {
	IParserService,
...
} from '../../../context/node/resolvers/selectionContextHelpers';
...
export class SymbolDefinitions extends PromptElement<Props, State> {
	...
	override async prepare(): Promise<State> {
		...
		if (!activeDocument) {
			...
			activeDocument ??= TextDocumentSnapshot.create(
		...
		const refFinders = [
			{
				header: 'Relevant function implementations',
				findImpls: findAllReferencedFunctionImplementationsInSelection,
			},
			{
				header: 'Relevant class declarations',
				findImpls: findAllReferencedClassDeclarationsInSelection,
			},
			{
				header: 'Relevant type declarations',
				findImpls: findAllReferencedTypeDeclarationsInSelection,
			},
		];

		const implementations: [string, TreeSitterExpressionInfo[]][] = [];
		for (const { header, findImpls } of refFinders) {
			const impls = await findImpls(
				this.parserService,
				this.logService,
	...
	override render(
		...
		for (const [header, implementations] of state.implementations) {
			const { references, text, uris } = treeSitterInfoToContext(
				...
				<>
					...
					{text.map((t, i) => (
						<>
							<CodeBlock
								code={t}
								languageId={state.activeDocument?.languageId}
								uri={uris[i]}
...
export function treeSitterInfoToContext(
	activeDocument: TextDocumentSnapshot,
	info: TreeSitterExpressionLocationInfo[],
) {
	const references = [];
	...
	for (const impl of info) {
		...
		const range =
			impl.range ??
			treeSitterOffsetRangeToVSCodeRange(activeDocument, impl);
