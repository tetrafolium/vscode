...
export class CodeBlockFormattingRules extends PromptElement<CodeBlockFormattingRulesPromptProps> {
	public override render(state: void, sizing: PromptSizing) {
		return (
			<>
				When suggesting code changes or new content, use Markdown code
				blocks.
				<br />
				...
				{!this.props.disableCodeBlockUris && (
					<>
						If the code modifies an existing file or should be
						placed at a specific location, add a line comment with '
						{filepathCodeBlockMarker}' and the file path.
						<br />
						If you want the user to decide where to place the code,
						do not add the file path comment.
						<br />
					</>
				)}
				In the code block, use a line comment with '
				{EXISTING_CODE_MARKER}' to indicate code that is already present
				in the file.
				<br />
				<ExampleCodeBlock
