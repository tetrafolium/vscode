...
export class CurrentSelection extends PromptElement<
...
> {
	...
	override async prepare(
		...
		if (
			selection &&
			(await sizing.countTokens(selection?.selectedText)) * 1.1 >
				sizing.tokenBudget
		) {
	...
	override render(
		...
		const references = [
			new PromptReference(
				new Location(selection.activeDocument.uri, selection.range),
			),
		];
		...
		if (state.exceedsTokenBudget) {
			this.logger.info(
				`Dropped current selection (${sizing.tokenBudget} / ${sizing.endpoint.modelMaxPromptTokens} tokens)`,
			);
			return (
				<>
					<AssistantMessage
					...
					>
						...
						{selection.fileName && (
							<>
								...
								lines from {path.basename(selection.fileName)}
							</>
						)}
						) exceeded my maximum context size and was dropped.
						Please reduce the selection to the most relevant part.
					</AssistantMessage>
				</>
			);
		}
		...
		return (
			<>
				<UserMessage priority={this.props.priority}>
					...
					<CodeBlock
						code={selection.selectedText}
						languageId={selection.languageId}
						uri={selection.activeDocument.uri}
