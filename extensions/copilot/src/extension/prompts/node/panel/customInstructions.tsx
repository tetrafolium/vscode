...
export class CustomInstructions extends PromptElement<CustomInstructionsProps> {
	...
	override async render(state: void, sizing: PromptSizing) {
		...
		if (includeCodeGenerationInstructions !== false) {
			...
			const liveIndexVariable = this.props.chatVariables?.find(
				isCustomizationsIndex,
			);
			const overrideActive =
				this.props.customizationsIndexOverride !== undefined;
			const indexValue = overrideActive
				? this.props.customizationsIndexOverride!
				: liveIndexVariable &&
					  typeof liveIndexVariable.value === 'string'
					? liveIndexVariable.value
					: undefined;
				...
				for (const variable of this.props.chatVariables) {
					if (isCustomizationsIndex(variable)) {
						...
						if (!hasSeen.has(value)) {
							hasSeen.add(value);
							const element = await this.createElementFromURI(
								value,
								variable.reference.toolReferences,
							);
							if (
								element &&
								!hasSeenContent.has(element.content)
							) {
			...
			for (const instructionFile of instructionFiles) {
				if (!hasSeen.has(instructionFile)) {
					...
					const element =
						await this.createElementFromURI(instructionFile);
	...
	private async createElementFromURI(
		fileUri: URI,
		toolReferences?: readonly ChatLanguageModelToolReference[],
	): Promise<{ chuck: PromptElement; content: string } | undefined> {
		try {
			...
			const attrs: Record<string, string> = {
				filePath:
					this.promptPathRepresentationService.getFilePath(fileUri),
			};
			const folders = this.workspaceService.getWorkspaceFolders();
			if (folders.length > 1) {
				const folder =
					this.workspaceService.getWorkspaceFolder(fileUri);
				if (folder) {
					attrs.workspaceFolder =
			...
			return {
				chuck: (
					<Tag name="attachment" attrs={attrs}>
						<references
							value={[
								new InstructionFileReference(fileUri, content),
							]}
						/>
			...
			this.logService.debug(
				`Instruction file not found: ${fileUri.toString()}`,
			);
			return undefined;
		}
	}
