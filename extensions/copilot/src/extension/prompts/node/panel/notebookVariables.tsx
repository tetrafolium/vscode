...
export class NotebookVariables extends PromptElement<
	NotebookVariablesPromptProps,
...
> {
	...
	override async prepare(): Promise<InlineChatNotebookRuntimeState> {
		try {
			this.logger.trace(
				`Fetching notebook variables for ${this.props.notebook.uri.toString()}`,
			);
			const variables = await this.notebookService.getVariables(
				this.props.notebook.uri,
			);
			return { variables };
		} catch (error) {
			this.logger.error(
				`Failed to get notebook variables for ${this.props.notebook.uri.toString()}: ${error}`,
			);
			return { variables: [] };
		}
	}
	...
	render(state: InlineChatNotebookRuntimeState) {
		...
		const notebookType = isJupyterNotebook
			? 'Jupyter Notebook'
			: 'Notebook';
...
export class NotebookCellOutputVariable extends PromptElement<INotebookCellOutputProps> {
	...
	render(state: void, sizing: PromptSizing) {
		...
		const outputInfo = getNotebookCellOutput(
			outputUri,
		...
		const item = notebookCellOutput.items.length
			? notebookCellOutput.items[0]
			: undefined;
		if (
			!item ||
			(!allowedTextMimeTypes.includes(item.mime) &&
		...
		const notebookPath = this.promptPathRepresentationService.getFilePath(
			notebook.uri,
		);
			...
			if (this.promptEndpoint.supportsVision) {
				text = (
					<>
						...
						>
							<Image
								variableName={`cell-output-image-${outputIndex}`}
								variableValue={item.data}
							/>
						</Tag>
					</>
				);
				...
				text = (
					<>
						<br />
						The user attempted to attach an image which is the
						output from the cell with index: {cellIndex} of the
						notebook {notebookPath} but images cannot be sent to
						this endpoint at this time and is therefore not
						attached. <br />
						<br />
					</>
				);
