...
describe('getPromptFileSlashCommandId', () => {
	...
	test('skill file uses parent folder name', () => {
		expect(
			getPromptFileSlashCommandId(
				makePromptVariable(
					'code-review',
					URI.file('/workspace/.github/skills/code-review/SKILL.md'),
				),
			),
	...
	test('skill file is case-insensitive for SKILL.md', () => {
		expect(
			getPromptFileSlashCommandId(
				makePromptVariable(
					'my-skill',
					URI.file('/workspace/.github/skills/my-skill/skill.md'),
				),
			),
	...
	test('non-prompt, non-skill file falls back to reference name', () => {
		expect(
			getPromptFileSlashCommandId(
				makePromptVariable(
					'some-instructions.instructions.md',
					URI.file(
						'/workspace/.github/instructions/some-instructions.instructions.md',
					),
				),
			),
		).toEqual({
			name: 'some-instructions.instructions.md',
			id: 'some-instructions.instructions.md',
		});
	});
	...
	test('prompt file with nested path', () => {
		expect(
			getPromptFileSlashCommandId(
				makePromptVariable(
					'prompt:deeply-nested.prompt.md',
					URI.file('/a/b/c/d/deeply-nested.prompt.md'),
				),
			),
		).toEqual({
			name: 'prompt:deeply-nested.prompt.md',
			id: 'deeply-nested',
		});
	});
});
...
describe('buildSlashCommandUserMessage', () => {
	function makeCollection(
		entries: { name: string; uri: ReturnType<typeof URI.file> }[],
	): ChatVariablesCollection {
	...
	const chatVariables = makeCollection([
		{
			name: 'prompt:code-review.prompt.md',
	...
	test('returns follow instruction for matching slash command without args', () => {
		expect(
			buildSlashCommandUserMessage('/code-review', chatVariables),
		).toBe('Follow instructions in #prompt:code-review.prompt.md');
	});

	test('returns follow instruction with arguments when provided', () => {
		expect(
			buildSlashCommandUserMessage(
				'/code-review some-file.ts',
				chatVariables,
			),
		).toBe(
			'Follow instructions in #prompt:code-review.prompt.md with these arguments: some-file.ts',
		);
	});
	...
	test('passes multi-word arguments', () => {
		expect(
			buildSlashCommandUserMessage(
				'/code-review file1.ts file2.ts --strict',
				chatVariables,
			),
		).toBe(
			'Follow instructions in #prompt:code-review.prompt.md with these arguments: file1.ts file2.ts --strict',
		);
	});
	...
	test('matches skill slash commands', () => {
		expect(
			...
			'Follow instructions in #my-skill with these arguments: do something',
		);
	});
	...
	test('handles multiline arguments', () => {
		expect(
			...
			'Follow instructions in #prompt:code-review.prompt.md with these arguments: line1\nline2',
		);
	});
});
...
describe('parseSlashCommand', () => {
	...
	test('returns undefined for plain text query', () => {
		expect(
			parseSlashCommand('just a question', chatVariables),
		).toBeUndefined();
	});

	test('returns undefined when slash command does not match any prompt file', () => {
		expect(
			parseSlashCommand('/unknown arg', chatVariables),
		).toBeUndefined();
	});
	...
	test('matches prompt file and returns command and empty args', () => {
		...
		expect(result!.promptFile).toEqual({
			name: 'prompt:code-review.prompt.md',
			id: 'code-review',
		});
	});
	...
	test('returns the matched variable', () => {
		...
		expect(result!.variable.reference.name).toBe(
	...
	test('skips non-prompt-file references', () => {
		const mixed = new ChatVariablesCollection([
			{
				id: 'vscode.instructions.file:inst',
				name: 'inst',
				value: URI.file('/workspace/inst.md'),
			},
			{
				id: `${PromptFileIdPrefix}:prompt:review.prompt.md`,
	...
	test('returns undefined with empty collection', () => {
		expect(
			parseSlashCommand('/code-review', new ChatVariablesCollection([])),
		).toBeUndefined();
	});
});
