...
const knownBinaryFileExtensions = new Set(['.pdf']);
 ...
 */
export async function hexdumpIfBinary(
	fileService: IFileSystemService,
	uri: Uri,
	options?: HexdumpIfBinaryOptions,
	...
	if (
		options?.openTextDocuments?.some(
...
export class BinaryFileHexdump extends PromptElement<BinaryFileHexdumpProps> {
	override async render() {
		...
		const truncated =
			start !== (startByte ?? 0) || end !== (endByte ?? data.length);
