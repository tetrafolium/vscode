...
export class NotebookReminderInstructions extends PromptElement<NotebookFormatPromptProps> {
	...
	public override render(_state: void, _sizing: PromptSizing) {
		const notebookRelatedUris =
			this.props.chatVariables instanceof ChatVariablesCollection
				? getNotebookUrisFromChatVariables(
						this.props.chatVariables,
						this._workspaceService,
						this.notebookService,
					)
				: this.props.chatVariables
						.filter((entry) => isNotebookWorkingSetEntry(entry))
						.map((entry) => entry.document.uri);
		if (
			notebookRelatedUris.length ||
			queryContainsNotebookSpecificKeywords(this.props.query)
		) {
			return (
				<>
...
export class NotebookFormat extends PromptElement<NotebookFormatPromptProps> {
	...
	public override render(_state: void, _sizing: PromptSizing) {
		...
		if (
			...
			const notebookUris = getNotebookUris(
				notebookRelatedUris,
				this._workspaceService,
			);
			return (
	...
	private getNotebookFormatInstructions(notebookUris: Uri[]) {
		const hasJupyterNotebook = notebookUris.some((uri) =>
			isJupyterNotebookUri(uri),
		);
		const extension =
			hasJupyterNotebook || notebookUris.length === 0
				? '.ipynb'
				: extname(notebookUris[0]);
		const tsExampleFilePath =
			this.promptPathRepresentationService.getExampleFilePath(
				`/Users/someone/proj01/example${extension}`,
			);
		switch (
			...
			case 'xml':
				return (
					<NotebookXmlFormatPrompt
						tsExampleFilePath={tsExampleFilePath}
					/>
				);
			case 'text':
				return (
					<NotebookTextFormatPrompt
						tsExampleFilePath={tsExampleFilePath}
					/>
				);
			default:
				return (
					<NotebookJsonFormatPrompt
						tsExampleFilePath={tsExampleFilePath}
					/>
				);
		}
		...
		new ResourceSet(
			coalesce(
				uris.map((uri) => {
					...
					const info = getNotebookAndCellFromUri(
						uri,
...
function getNotebookUrisFromChatVariables(
...
export class NotebookXmlFormatPrompt extends PromptElement<NotebookFormatCommonPromptProps> {
	constructor(props: NotebookFormatCommonPromptProps) {
		super(props);
	}
	async render(_state: void, _sizing: PromptSizing) {
		return (
			<>
				...
				<br />
				1. Each cell must be wrapped in a {'<VSCode.Cell>'} with a
				`language` attribute indicating the type of content. (e.g.,
				`markdown`, `python`). <br />
				2. Existing cells must contain the `id` attribute to uniquely
				identify each cell. <br />
				3. New cells do not need an `id` attribute. <br />
				4. Ensure that each {'<VSCode.Cell>'} is valid XML and logically
				structured. <br />
				5. Do not XML encode the contents within each {
					'<VSCode.Cell>'
				}{' '}
				cell. <br />
				6. Do not reference the XML tags {`<VSCode.Cell>`} in user
				messages. <br />
				7. Do not reference Cell Ids (as users cannot see these values)
				in user messages, instead use the Cell number (starting from 1).{' '}
				<br />
				<br />
				...
				<Tag name="example">
					<ExampleCodeBlock
						languageId="xml"
						examplePath={this.props.tsExampleFilePath}
	...
	async render(_state: void, _sizing: PromptSizing) {
		return (
			<>
				...
				array property with a `metadata.language` property indicating
				the type of content (e.g., `markdown`, `python`). <br />
				2. Existing cells must contain the `metadata.id` property to
				uniquely identify each cell. <br />
				3. New cells do not need a `metadata.id` property. <br />
				4. Ensure the content is valid JSON and logically structured.{' '}
				<br />
				5. Do not reference Cell Ids (as users cannot see these values)
				in user messages, instead use the Cell number (starting from 1).{' '}
				<br />
				<br />
				Here is sample content of a Notebook document:
	...
	async render(_state: void, _sizing: PromptSizing) {
		return (
			<>
				...
				<br />
				1. Each cell must begin with a comment beginning with `#%%
				vscode.cell` followed by the cell attributes.
				<br />
				2. For existing cell in the document, use the `id` attribute to
				identify the cell. If the cell is new, DO NOT include the `id`
				attribute.
				<br />
				3. Use the `language` attribute to define the language of the
				content (e.g., `markdown`, `python`). <br />
				4. For markdown cells, use triple quotes to wrap the content.
				<br />
				5. Ensure that each cell is logically structured. <br />
				6. Do not reference Cell Ids (as users cannot see these values)
				in user messages, instead use the Cell number (starting from 1).{' '}
				<br />
				<br />
				Here is sample content of a Notebook document:
