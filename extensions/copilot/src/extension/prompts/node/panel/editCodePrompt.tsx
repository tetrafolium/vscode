...
export class EditCodePrompt extends PromptElement<EditCodePromptProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const instructionsAfterHistory = modelPrefersInstructionsAfterHistory(
			this.props.endpoint.family,
		);
		const hasFilesInWorkingSet =
			this.props.promptContext.workingSet.length > 0;
		const instructions = (
			<InstructionMessage priority={900}>
				{hasFilesInWorkingSet ? (
					...
					<>
						...
						<br />
						If you need to change existing files and it's not clear
						which files should be changed, then refuse and answer
						with "Please add the files to be modified to the working
						set
						...
						".
						<br />
						The only exception is if you need to create new files.
						In that case, follow the following instructions.
						<br />
					</>
				)}
				...
				<br />
				3. For each file, give a short summary of what needs to be
				changed followed by a code block that contains the code changes.
				<br />
				4. The code block should start with four backticks followed by
				the language.
				<br />
				5. On the first line of the code block add a comment containing
				the filepath. This includes Markdown code blocks.
				<br />
				6. Use a single code block per file that needs to be modified,
				even if there are multiple changes for a file.
				<br />
				7. The user is very smart and can understand how to merge your
				code blocks into their files, you just need to provide minimal
				hints.
				<br />
				8. Avoid repeating existing code, instead use comments to
				represent regions of unchanged code. The user prefers that you
				are as concise as possible. For example: <br />
				<ExampleCodeBlock
					languageId="languageId"
					examplePath={'/path/to/file'}
					includeFilepath={true}
					minNumberOfBackticks={4}
					code={[
				...
				Here is an example of how you should format a code block
				belonging to the file example.ts in your response:
				<br />
				<Tag name="example">
					###{' '}
					...
					<br />
					<br />
					Add a new property 'age' and a new method 'getAge' to the
					class Person.
					<br />
					<br />
					<ExampleCodeBlock
						languageId="typescript"
						examplePath={tsExampleFilePath}
						includeFilepath={true}
						minNumberOfBackticks={4}
						code={[
							`class Person {`,
		...
		return (
			<>
				...
				<EditCodeConversationHistory
					flexGrow={1}
					priority={700}
					workingSet={this.props.promptContext.workingSet}
					history={this.props.promptContext.history}
					promptInstructions={
						this.props.promptContext.promptInstructions
					}
	...
	override render() {
		...
		return (
			<TextChunk>
				{'<fileRestrictions>'}
				<br />
				The following files are readonly. Making edits to any of these
				file paths is FORBIDDEN. If you cannot accomplish the task
				without editing the files, briefly explain why, but NEVER edit
				any of these files:
				<br />
				{readonlyUris
					.map(
...
class EditCodeConversationHistory extends PromptElement<EditCodeConversationHistoryProps> {
	...
	override async render(state: void, sizing: PromptSizing) {
		...
		for (const entry of this.props.workingSet) {
			includedFilesAtVersions.set(entry.document.uri, [
		...
		for (const promptInstruction of this.props.promptInstructions) {
			includedPromptInstructions.set(promptInstruction.uri, [
				promptInstruction.version,
			]);
		}
		...
		for (const turn of this.props.history) {
			...
			if (editCodeStep) {
				history.push(
					this._renderUserMessageWithoutFiles(
						editCodeStep,
						includedFilesAtVersions,
						includedPromptInstructions,
					),
				);
				history.push(
					this._renderAssistantMessageWithoutFileTags(
						editCodeStep.response,
					),
				);
			}
		}
	...
	private _renderAssistantMessageWithoutFileTags(
		message: string,
	): AssistantMessage {
	...
	private _renderUserMessageWithoutFiles(
		editCodeStep: PreviousEditCodeStep,
		includedFilesAtVersion: ResourceMap<number[]>,
		includedPromptInstructions: ResourceMap<number[]>,
	): UserMessage {
		const filesToRemove: Uri[] = [];
		for (const entry of editCodeStep.workingSet) {
			const versions =
				includedFilesAtVersion.get(entry.document.uri) ?? [];
			const isAlreadyIncluded = versions.some(
				(version) => version === entry.document.version,
			);
		...
		for (const entry of editCodeStep.promptInstructions) {
			const versions =
				includedPromptInstructions.get(entry.document.uri) ?? [];
			const isAlreadyIncluded = versions.some(
				(version) => version === entry.document.version,
			);
			if (isAlreadyIncluded) {
		...
		let userMessage = this._removePromptInstructionsFromPastUserMessage(
			editCodeStep.request,
			promptInstructionsToRemove,
		);
		userMessage = this._removeFilesFromPastUserMessage(
			userMessage,
			filesToRemove,
		);
	...
	private _removePromptInstructionsFromPastUserMessage(
		userMessage: string,
		shouldRemove: Uri[],
	) {
		const interestingFilePaths = shouldRemove.map((uri) =>
	...
	private _removeFilesFromPastUserMessage(
		userMessage: string,
		shouldRemove: Uri[],
	) {
		const interestingFilePaths = shouldRemove.map(
			(uri) =>
				`${filepathCodeBlockMarker} ${this._promptPathRepresentationService.getFilePath(uri)}`,
		);
		return userMessage.replace(
			/<file(-selection)?>[\s\S]*?<\/file(-selection)?>/g,
			(match) => {
				if (interestingFilePaths.some((path) => match.includes(path))) {
					return '';
				}
		...
		return (
			<>
				<UserMessage>
					...
					<WorkingSet
						flexGrow={3}
						flexReserve={sizing.tokenBudget * 0.8}
						priority={810}
						workingSet={workingSet}
					/>
					<br />
					<Tag name="reminder" flexGrow={2} priority={899}>
						Avoid repeating existing code, instead use a line
						comment with `{EXISTING_CODE_MARKER}` to represent
						regions of unchanged code.
						<br />
						The code block for each file being edited must start
						with a comment containing the filepath. This includes
						Markdown code blocks.
						<br />
						For existing files, make sure the filepath exactly
						matches the filepath of the original file.
						<br />
						<NotebookReminderInstructions
							chatVariables={chatVariables}
							query={query}
...
export class WorkingSet extends PromptElement<WorkingSetPromptProps> {
	public override render(state: void, sizing: PromptSizing) {
		...
		return workingSet.length ? (
			<>
				The user has provided the following files as input. Always make
				changes to these files unless the user asks to create a new
				file.
				<br />
				Untitled files are files that are not yet named. Make changes to
			...
			return (
				<>
					...
					{workspaceFolders
						.map(
							(f) =>
								`${this._promptPathRepresentationService.getFilePath(f)}`,
						)
						.join(', ')}
					.
				</>
...
class TextDocumentWorkingSetEntry extends PromptElement<TextDocumentWorkingSetEntryPromptProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const summarized = await s.summarizeDocument(
			document,
			undefined,
			selection,
			sizing.tokenBudget,
		...
		if (workingSetEntryState === WorkingSetEntryState.Accepted) {
			userActionStateFragment =
				'I applied your suggestions for this file and accepted them. Here is the updated file:';
		} else if (workingSetEntryState === WorkingSetEntryState.Rejected) {
			userActionStateFragment =
				'I considered your suggestions for this file but rejected them. Here is the file:';
		} else if (workingSetEntryState === WorkingSetEntryState.Undecided) {
			userActionStateFragment =
				"I applied your suggestions for this file but haven't decided yet if I accept or reject them. Here is the updated file:";
		}
...
class NotebookWorkingSetEntry extends PromptElement<NotebookWorkingSetEntryPromptProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const s = this.instantiationService.createInstance(
			NotebookDocumentSummarizer,
		);
		const summarized = await s.summarizeDocument(
			document,
...
class FileSelection extends PromptElement<CurrentFileSelectionPromptProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		if (document.lineCount >= 4) {
			...
			const charactersInSelectionLines = () =>
				selectionLines.reduce((acc, line) => acc + line.length, 0);
			let selectionStartLine = Math.min(
				document.lineCount - 1,
				Math.max(0, selection.start.line),
			);
			let selectionEndLine = Math.min(
				document.lineCount - 1,
				selection.end.line,
			);
			if (
			...
			if (
				...
				selection.start.character ===
					document.lineAt(selectionStartLine).text.length
			) {
				selectionStartLine++;
			}
			...
			while (
				selectionLines.length < 4 &&
				charactersInSelectionLines() < 10
			) {
				if (above >= 0) {
			...
			return (
				<Chunk>
					...
					<CodeBlock
						...
						uri={document.uri}
						references={[
							new PromptReference(document.uri, undefined),
						]}
						code={selectionLines.join('\n')}
						shouldTrim={false}
					/>
					<br />
					&lt;/file-selection&gt;
