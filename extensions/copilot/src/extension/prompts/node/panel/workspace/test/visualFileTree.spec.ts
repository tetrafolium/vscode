...
suite('Truncated file tree', () => {
	test('Empty file tree', async () => {
		...
		assert.deepStrictEqual(
			(await visualFileTree(files, Infinity)).tree,
			'',
		);
	...
	test('Should print file nodes', async () => {
		const files = [file('abc'), file('longName')];
		assert.deepStrictEqual(
			(await visualFileTree(files, Infinity)).tree,
			'abc\nlongName',
		);
	...
	test('Should truncate file list', async () => {
		const files = [file('aa'), file('bb'), file('cc')];
		assert.deepStrictEqual((await visualFileTree(files, 0)).tree, '');
		...
		assert.deepStrictEqual(
			(await visualFileTree(files, 6)).tree,
			'aa\n...',
		);
		assert.deepStrictEqual(
			(await visualFileTree(files, 7)).tree,
			'aa\n...',
		);
		assert.deepStrictEqual(
			(await visualFileTree(files, 8)).tree,
			'aa\nbb\ncc',
		);
		assert.deepStrictEqual(
			(await visualFileTree(files, Infinity)).tree,
	...
	test('Should print directories nodes', async () => {
		const files = [file('abc'), dir('dir', [file('def')])];
		assert.deepStrictEqual(
			(await visualFileTree(files, Infinity)).tree,
			joinLines('abc', 'dir/', '\tdef'),
		);
	});
	...
	test('Should expand breadth first', async () => {
		...
		assert.deepStrictEqual(
			(await visualFileTree(files, 25)).tree,
			joinLines('dir1/', '\tdir2/', 'dir3/', '\tfile2'),
		);

		assert.deepStrictEqual(
			(await visualFileTree(files)).tree,
			joinLines('dir1/', '\tdir2/', '\t\tfile1', 'dir3/', '\tfile2'),
		);
	});
	...
	test('Should expand multi-lists breadth first', async () => {
		...
		for (let i = 9; i < 15; ++i) {
			assert.deepStrictEqual(
				(await visualFileTree(files, i)).tree,
				joinLines('file1', '...'),
			);
		}
		...
		for (let i = 15; i < 18; ++i) {
			assert.deepStrictEqual(
				(await visualFileTree(files, 15)).tree,
				joinLines('file1', 'dir1/', '...'),
			);
		}
		...
		for (let i = 29; i < 31; ++i) {
			assert.deepStrictEqual(
				(await visualFileTree(files, i)).tree,
				joinLines('file1', 'dir1/', '\tfile2', '\t...', 'dir2/'),
			);
		}
	});
});
