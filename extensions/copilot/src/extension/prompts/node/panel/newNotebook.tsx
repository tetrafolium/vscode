...
export class NewNotebookPlanningPrompt extends PromptElement<
	NewNotebookPlanningPromptProps,
	NewNotebookPlanningPromptState
> {
	...
	override render(
		state: NewNotebookPlanningPromptState,
		sizing: PromptSizing,
			...
			<>
				<>
					<SystemMessage priority={1000}>
						You are an AI that creates a detailed content outline
						for a Jupyter notebook on a given topic.
						<br />
						...
						Additional Rules
						<br />
						DO NOT include Introduction or Conclusion section in the
						outline!
						<br />
				...
				/>
				{this.props.promptContext.chatVariables &&
				Object.keys(this.props.promptContext.chatVariables).length >
					0 ? (
...
export class NewNotebookCodeGenerationPrompt extends PromptElement<
	NewNotebookCodeGenerationPromptProps,
...
> {
	...
	override render(
		...
		return (
			<>
				<>
					{isNonEmptyArray(this.props.history) && (
						<GenerateNotebookConversationHistory
							messages={this.props.history}
						/>
					)}
					<SystemMessage priority={1000}>
						You are an AI that writes Python code for a single
						...
						<br />
						Your output should be valid Python code with inline
						comments.
						<br />
						You should return the code directly without any
						explantion.
						<br />
						You should not print message to explain the code or
						purpose of the code.
						<br />
						You should return the code directly, without wrapping it
						inside \`\`\`.
						<br />
						Please make sure that the new code is syntactically
						valid Python code. It can be validated by running it in
						a Python interpreter.
						<br />
						For example, it should pass the validation through
						builtin module codeop
						\`codeop.compile_command(statement)\`.
						<br />
					</SystemMessage>
					<UserMessage priority={900}>
						Overall topic of the notebook: {this.props.description}
						<br />
						...
						<br />
						Given this information, write all the code for this
						section and this section only.
						<br />
						The request to generate the outline of the notebook is
						already completed.
						<br />
						...
						<CodeBlock
							...
							code={this.props.existingCode}
						/>
						<br />
						Please make sure the new code you generate works fine
						with the code above.
						<br />
					</UserMessage>
				</>
...
export class NewNotebookCodeImprovementPrompt extends PromptElement<
	NewNotebookCodeImprovementPromptProps,
...
> {
	...
	override render(
		state: NewNotebookCodeImprovementPromptState,
		sizing: PromptSizing,
	): PromptPiece<any, any> | undefined {
		return (
			<>
				<>
					<SystemMessage priority={1000}>
						You are an AI that improves Python code with respect to
						readability and performance for a single section of a
						Jupyter notebook.
						<br />
						<CopilotIdentityRules />
						<LegacySafetyRules />
						...
						<br />
						DO NOT explain in inline comments for your the
						improvements.
						<br />
						You should not print messages to explain the code or
						purpose of the code.
						<br />
						Make sure the new code you generate works fine with the
						code above.
						<br />
						Make sure if a module is already imported in the code
						above, it can be used in the new code directly without
						importing it again. For the same reason, if a variable
						is defined above, it can be used in new code as well.{' '}
						<br />
						Make sure to return the code only - don't give an
					...
					<UserMessage priority={900}>
						...
						/>
						<br />
						Given this information, suggest improvements for the
						following code:
						<br />
						<br />
						{this.props.code}
