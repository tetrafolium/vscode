		...
		return (
			<>
				{this.props.chunkResults
						...
						const priority =
							typeof this.props.priority !== 'undefined'
								? this.props.priority +
									(1 -
										(i + 1) /
											this.props.chunkResults.length)
								: undefined;
					...
					.map(({ chunk, priority }) => {
						const fileLabel = getWorkspaceFileDisplayPath(
		...
		return (
			<>
				<SystemMessage priority={1000}>
					You are a software engineer with expert knowledge of the
					codebase the user has open in their workspace.
					<br />
					You will be provided with a few code excerpts, file names,
					and symbols from the user's that have been extracted as
					important to the user's query.
					<br />
					Your job is to understand what the user is searching for and
					find the relevant piece of code.
					<br />
					That piece of code will be searched for using grep in the
					user's workspace.
					<br />
					<br />
				...
				>
					<InstructionMessage priority={1000}>
						...
						Think step by step:
						<br />
						1. Read the provided relevant workspace information
						(code excerpts, file names, and symbols) to understand
						the user's workspace.
						<br />
						2. Select ONLY from the provided code excerpts, file
						names, and symbols any code snippets that are relevant
						to the user's query.
						<br />
						3. Provide ONE query FOR EACH code excerpt the user
						should search for in order to find the relevant wrapping
						code, prioritizing the most meaningful code, class
						names, functions, definitions, etc.
						<br />
						<br />
						You MUST ONLY consider the included code excerpts, file
						names and symbols to provide your answer.
						<br />
						You MUST only return the file path and the query or
						phrase to search for using grep
						<br />
						You MUST avoid returning queries that are too short and
						too generic that would return a lot of noisy results
						<br />
				...
				<UserMessage>
					<SearchChunkResult
						priority={898}
						chunkResults={this.props.promptContext.chunkResults}
					/>
					<ChatToolReferences
						priority={899}
						flexGrow={3}
					...
					<ChatVariablesAndQuery
						...
						priority={900}
						query={`Here is the user query: ${query}`}
					/>
				</UserMessage>
			</>
		);
