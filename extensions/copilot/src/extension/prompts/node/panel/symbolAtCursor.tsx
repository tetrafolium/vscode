...
export class SymbolAtCursor extends PromptElement<
	SymbolAtCursorProps,
	SymbolAtCursorState
> {
	constructor(
	...
	private static getSymbolAtCursor(
		tabsAndEditorsService: ITabsAndEditorsService,
		props: Omit<SymbolAtCursorProps, 'priority'>,
	):
		| {
				selectedText: string;
				document: TextDocumentSnapshot;
		...
		const range =
			document.getWordRangeAtPosition(selection.active) ?? selection;
		const selectedText = document.getText(range);
		return { selectedText, document, range };
	}

	static async getDefinitionAtRange(
		...
		const fileIsIgnored = await ignoreService.isCopilotIgnored(
			document.uri,
		);
		if (fileIsIgnored) {
			return undefined;
		...
		if (preferDefinitions) {
			nodeContext = await treeSitterAST.getNodeToExplain(
				treeSitterOffsetRange,
			);
		}
		nodeContext ??= await treeSitterAST.getNodeToDocument(
			treeSitterOffsetRange,
		);
		const { startIndex, endIndex } =
			'nodeToDocument' in nodeContext
				? nodeContext.nodeToDocument
				: nodeContext.nodeToExplain;
		const expandedRange = treeSitterOffsetRangeToVSCodeRange(document, {
			startIndex,
			endIndex,
		});

		return {
			identifier: nodeContext.nodeIdentifier,
			text: document.getText(expandedRange),
			range: expandedRange,
			uri: document.uri,
			startIndex,
			endIndex,
	...
	static async getSelectedScope(
		...
		if (
			!props.document ||
			(await ignoreService.isCopilotIgnored(props.document.uri))
		) {
			return undefined;
		}

		const symbolAtCursor = SymbolAtCursor.getSymbolAtCursor(
			tabsAndEditorsService,
			props,
		);
		let symbolAtCursorState: SymbolAtCursorState | undefined;
		const definition = symbolAtCursor
			? await SymbolAtCursor.getDefinitionAtRange(
					ignoreService,
					parserService,
					symbolAtCursor.document,
					symbolAtCursor.range,
					false,
				)
			: undefined;
		const isExplicitScopeSelectionEnabled = configurationService.getConfig(
			ConfigKey.ExplainScopeSelection,
		);
		if (
			isExplicitScopeSelectionEnabled ||
			(symbolAtCursor &&
				(definition?.identifier !== symbolAtCursor.selectedText ||
			...
			const rangeOfEnclosingSymbol =
				await scopeSelector.selectEnclosingScope(editor, {
					reason: l10n.t('Select an enclosing range to explain'),
					includeBlocks: true,
				});
			if (rangeOfEnclosingSymbol) {
				...
				symbolAtCursorState = {
					codeAtCursor: definitionText,
					document,
					range: rangeOfEnclosingSymbol,
					definitions: [],
					references: [],
				};
			}
	...
	override async prepare(
		...
		if (definition?.identifier === symbolAtCursor?.selectedText) {
			...
			symbolAtCursorState ??= {
				codeAtCursor: symbolAtCursor.document.lineAt(
					symbolAtCursor.range.start,
				).text,
				document: symbolAtCursor.document,
				range: symbolAtCursor.document.lineAt(
					symbolAtCursor.range.start,
				).range,
				definitions: [],
				references: [],
		...
		progress.report(
			new ChatResponseProgressPart(
				l10n.t('Searching for relevant definitions...'),
			),
		);
		try {
			for (const link of await this._languageFeaturesService.getDefinitions(
				symbolAtCursor.document.uri,
				symbolAtCursor.range.start,
			)) {
				...
				const textDocument =
					await this._workspaceService.openTextDocumentAndSnapshot(
						uri,
					);
				const definition = await SymbolAtCursor.getDefinitionAtRange(
					this._ignoreService,
					this._parserService,
					textDocument,
					range,
		...
		progress.report(
			new ChatResponseProgressPart(
				l10n.t('Searching for relevant references...'),
			),
		);
		try {
			const seenReferences = new Set<string>();
			for (const link of await this._languageFeaturesService.getReferences(
				symbolAtCursor.document.uri,
				symbolAtCursor.range.start,
			)) {
				const { uri, range } = isLocationLink(link)
				...
				const reference = await SymbolAtCursor.getDefinitionAtRange(
					this._ignoreService,
					this._parserService,
					textDocument,
					range,
	...
	override render(
		...
		if (state.codeAtCursor) {
			const { startIndex, endIndex } = vscodeToTreeSitterOffsetRange(
				state.range,
				state.document,
			);
			info.push({
		...
		return (
			<>
				...
				<UserMessage>
					...
					<CodeBlock
						uri={state.document.uri}
						languageId={state.document.languageId}
						code={state.codeAtCursor}
					/>
					<br />
					{Boolean(state.definitions.length) && (
						<>
							Here are some relevant definitions for the symbols
							in my code:
							<br />
						</>
					)}
					...
					{Boolean(state.references.length) && (
						<>
							<br />
							Here are some places where the the symbols in my
							code are referenced:
							<br />
						</>
					)}
					{Boolean(state.references.length) && (
						<PrioritizedList
							priority={
								this.props.priority - state.definitions.length
							}
							descending={true}
						>
