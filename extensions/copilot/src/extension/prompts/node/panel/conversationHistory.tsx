 ...
 */
export class HistoryWithInstructions extends PromptElement<
	Omit<ConversationHistoryProps, 'priority'> & { historyPriority: number }
> {
	constructor(
		props: Omit<ConversationHistoryProps, 'priority'> & {
	...
	override render(_state: void, sizing: PromptSizing): PromptPiece {
		...
		if (
			!children?.some(
				(c) => typeof c === 'object' && c.ctor === InstructionMessage,
			)
		) {
			...
			throw new Error(
				`HistoryWithInstructions must have an InstructionMessage child`,
			);
		}
			...
			<>
				{after ? (
					<ConversationHistory
						{...props}
						passPriority={false}
						priority={this.props.historyPriority}
					/>
				) : undefined}
				{...children}
				{after ? undefined : (
...
export class ConversationHistory extends PromptElement<ConversationHistoryProps> {
	override render(
		...
		if (this.props.inline && turnHistory.length > 0) {
			...
			turnHistory = [
				new Turn(
					undefined,
					{ message: historyMessage, type: 'user' },
					undefined,
				),
			];
		}
		...
		turnHistory.forEach((turn, index) => {
			if (turn.request.type === 'user') {
				const promptVariables =
					turn.promptVariables && !this.props.omitPromptVariables
						? this.removeDuplicateVars(
								turn.promptVariables,
								this.props.currentTurnVars,
								turnHistory.slice(index + 1),
							)
						: new ChatVariablesCollection([]);
				history.push(
					<ChatVariablesAndQuery
						priority={900}
						chatVariables={promptVariables}
			...
			if (
				turn.responseMessage?.type === 'model' &&
				![TurnStatus.OffTopic, TurnStatus.Filtered].includes(
					turn.responseStatus,
				)
			) {
				history.push(
	...
	private removeDuplicateVars(
		historyVars: ChatVariablesCollection,
		currentTurnVars: ChatVariablesCollection | undefined,
		followingMessages: Turn[],
	): ChatVariablesCollection {
		...
		return historyVars.filter((v1) => {
			if (
				followingMessages.some((m) =>
					m.promptVariables?.find((v2) => variableEquals(v1, v2)),
				)
			) {
				return false;
			}
