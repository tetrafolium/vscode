...
export class DefinitionAtPosition extends PromptElement<Props, State> {
	...
	constructor(
		...
		private readonly _vscodeExtensionCtxService: IVSCodeExtensionContext,
	...
	override async prepare(): Promise<State> {
		if (
			await this._ignoreService.isCopilotIgnored(this.props.document.uri)
		) {
			return { k: 'ignored' };
		}
		...
		return (
			<>
				...
				{definitions.map((codeBlock) => {
					const lineCommentStart = getLanguage(codeBlock.languageId)
						.lineComment.start;
	...
	private async findDefinition(timeoutMs: number): Promise<CodeExcerpt[]> {
		...
		const findImplOrDefinition = async (position: vscode.Position) => {
			try {
				const impls =
					await this._languageFeaturesService.getImplementations(
						document.uri,
						position,
					);
			...
			try {
				const defs = await this._languageFeaturesService.getDefinitions(
					document.uri,
					position,
				);
		...
		const foundDefs = await asyncComputeWithTimeBudget(
			this._logService,
			this._telemetryService,
			document,
			timeoutMs * 3,
			() => findImplOrDefinition(position),
			[],
		);
		...
		const nonIgnoredDefs = arrays.coalesce(
			await Promise.all(
				foundDefs.map(async (def) => {
					const uri = isLocationLink(def)
						? def.targetUri
						: document.uri;
					return (await this._ignoreService.isCopilotIgnored(uri))
						? undefined
						: def;
				}),
			),
		...
		return Promise.all(
			nonIgnoredDefs.map(async (def) => {
				const { uri, range } = isLocationLink(def)
					? { uri: def.targetUri, range: def.targetRange }
					: def;

				const docContainingDef =
					await this._workspaceService.openTextDocumentAndSnapshot(
						uri,
					);

				const wasmLanguage = getWasmLanguage(
					docContainingDef.languageId,
				);

				let code: string;
				let excerptRange: vscode.Range;
				if (wasmLanguage === undefined) {
					// capture at least the line of the definition
					const line = docContainingDef.lineAt(range.start.line);
					code = line.text;
					excerptRange = line.range;
				} else {
					const nodeToDocument = await determineNodeToDocument(
						this._parserService,
						this._telemetryService,
						{
