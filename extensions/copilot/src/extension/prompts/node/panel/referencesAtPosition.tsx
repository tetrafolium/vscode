...
export class ReferencesAtPosition extends PromptElement<Props> {
	...
	async render(state: void, sizing: PromptSizing) {
		if (
			await this.ignoreService.isCopilotIgnored(this.props.document.uri)
		) {
		...
		return (
			<>
				{this.renderCodeExcerpts(
					`Relevant definition${definitions.length > 1 ? 's' : ''}:`,
					definitions,
				)}
		...
		if (excerts.length > 0) {
			return (
				<>
					...
					<br />
					{excerts.map((excerpt) => {
						const lineCommentStart = getLanguage(excerpt.languageId)
							.lineComment.start;
	...
	private async findReferences(
		timeoutMs: number,
	): Promise<[CodeExcerpt[], CodeExcerpt[]]> {
		const { document, position } = this.props;

		const findReference = async () => {
			try {
				const refs = await this.languageFeaturesService.getReferences(
					document.uri,
					position,
				);
				this.logService.debug(
		...
		const foundRefs = await asyncComputeWithTimeBudget(
			this.logService,
		...
		for (const ref of foundRefs) {
			...
			const treeSitterAST =
				this.parserService.getTreeSitterAST(docContainingRef);
			if (!treeSitterAST) {
				continue;
			}

			const range = vscodeToTreeSitterOffsetRange(
				ref.range,
				docContainingRef,
			);
			...
			if (calls.length > 0) {
				nonIgnoredRefs.push({
					...
					code: calls[0].text,
					excerptRange: treeSitterOffsetRangeToVSCodeRange(
						docContainingRef,
						calls[0],
					),
				} as CodeExcerpt);
				...
				functions.some((f) =>
					TreeSitterOffsetRange.doIntersect(f, range),
				)
			) {
