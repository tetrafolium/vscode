			...
			<>
				<SystemMessage priority={1000}>
					{style === OutputStyle.ConfigOnly ? (
						<>
							You are a Visual Studio Code assistant who
							specializes in debugging and creating launch
							configurations. Your task is to create a launch
							configuration for the user's query.
							<br />
						</>
					) : (
						<>
							You are a Visual Studio Code assistant who
							specializes in debugging, searching for existing
							launch configurations, and creating launch
							configurations. Your task is to find an existing
							launch configuration that matches the query or to
							create a launch configuration for the user's query
							if no match is found. If there's no query, still
							provide a response, checking for existing
							configurations in the launch.json file, if any.
							<br />
						</>
					)}
				...
				>
					<InstructionMessage priority={1000}>
						{style === OutputStyle.Readable && (
							<>
								The user cannot see the context you are given,
								so you must not mention it. If you want to refer
								to it, you must include it in your reply.
								<br />
							</>
						)}
						Print out the VS Code `launch.json` file needed to debug
						the command, formatted as JSON.
						<br />
						If there are build steps needed before the program can
						be debugged, be sure to include a `preLaunchTask`
						property in the launch configuration. If you include a
						...
						{state.resources?.some((r) =>
							...
							<>
								{' '}
								it must either refer to an existing a suitable
								task in the `tasks.json` file, or you must
								include a `tasks.json` file in your response
								that contains that configuration.
							</>
						) : (
							<>
								{' '}
								you MUST also include `tasks.json` file in your
								response that contains that configuration.
							</>
						)}
						{style === OutputStyle.Readable && (
							<>
								{' '}
								Include a brief one or two sentence explaination
								of any such task definition is needed.
								<br />
							</>
						)}
						<br />
						Pay attention to my operating system and suggest the
						best tool for the platform I'm working on. For example,
						for debugging native code on Windows, you would not
						suggest the `lldb` type.
						<br />
						If there are unknowns, such as the path to the program,
						use the `inputs` field in the launch.json schema to
						prompt the user with an informative message. Input types
						may either be `promptString` for free text input or
						`pickString` with an `options` array for enumerations.
						<br />
						...
						may either be `promptString` for free text input or
						`pickString` with an `options` array for enumerations.
						Do not include a default value for the input field.
						<br />
						Always include the following properties in the
						launch.json file:
						<br />
						- type: the type of debugger to use for this launch
						configuration. Every installed debug extension
						introduces a type: node for the built-in Node debugger,
						for example, or php and go for the PHP and Go
						extensions.
						<br />
						- request: the request type of this launch
						configuration. Currently, launch and attach are
						supported.
						<br />
						- name: the reader-friendly name to appear in the Debug
						launch configuration dropdown.
						<br />
						If a result is not a valid answer, but it still relates
						to Visual Studio Code, please still respond.
						<br />
						Please do not guess a response and instead just respond
						with a polite apology if you are unsure.
						<br />
						If you believe the given context given to you is
						incorrect or not relevant you may ignore it.
						<br />
				...
				<UserMessage priority={700}>
					{state.docSearchResults &&
						state.docSearchResults.length > 0 && (
							<>
								Below is a list of information from the Visual
								Studio Code documentation which might be
				...
				<UserMessage priority={700} flexGrow={1}>
					{this.props.input.type === StartDebuggingType.UserQuery ? (
						<>
							If a program property is included in the launch
							config, and its path does not exist in the workspace
							or there are multiple files that could work, use the
							`inputs` field in the launch.json schema to prompt
						...
						state.resources?.some((r) =>
							...
							<>
								{this.props.input.userQuery ? (
									<>
										Search in that provided launch.json file
										for an existing configuration based on
										the query "{this.props.input.userQuery}
										". Pay particular attention to the name
										of the launch configuration and compare
										it to the query. If a match is found,
										include that configuration. Do not
										include the whole launch.json context.
										End the response with HAS_MATCH.
										<br />
									</>
								) : (
									<>
										Scan any provided documentation to
										determine which configuration in the
										provided launch.json file is
										recommended, if any. Show some, not all,
										of the launch configurations that are
										available. End the response with
										HAS_CONFIG_NO_QUERY.
										<br />
									</>
								)}
								If no match is found, include the new
								configuration that was generated. End the
								response with GENERATED_CONFIG.
								<br />
							</>
						)}
				</UserMessage>
				{style === OutputStyle.ConfigOnly ? (
					<UserMessage priority={850}>
						<Tag name="example">
							In this example, we're debugging a simple Python
							file, so we only need a launch.json:
							<br />
							<Tag name="request">
								In the working directory, I ran this on the
						...
						<Tag name="example">
							In this example, generate both a launch.json and
							tasks.json because the program needs to be built
							before it can be debugged:
							<br />
							<Tag name="request">
								In the working directory, I ran this on the
								command line: `./my-program.exe`
								<br />
							</Tag>
							<Tag name="response">
								launch.json:
					...
					<UserMessage priority={850}>
						<Tag name="example">
							...
							<Tag name="request">
								...
								"python file"
								<br />
								In my workspace I have the files main.py,
								tox.ini, and README.md.
								<br />
							</Tag>
							<Tag name="response">
						...
						<Tag name="example">
							...
							<Tag name="request">
								...
								"my-program"
								<br />
								In my workspace I have the files Makefile,
								my-program.cpp.
								<br />
							</Tag>
							<Tag name="response">
								...
								```
								<br />
								It looks like you build your project using your
								Makefile, so let's add a `tasks.json` to do that
...
> {
	...
	override async render(
		...
		if (wf) {
			...
			return (
				<>
					My workspace folder (`${'{'}workspaceFolder{'}'}`) has the
					following structure:
					<br />
					<br />
					<meta
		...
		return (
			<>
				<SystemMessage priority={10}>
					You are a Visual Studio Code assistant who specializes in
					debugging and creating launch configurations. Your job is to
					return an array of file names that may contain useful
					information to translate a user query into a VS Code debug
					configuration.
					<br />
					The user will give you a file tree. Make sure to fully
					qualify paths you return from the tree, including their
					parent directories:
					<br />
					Do not give any other explanation and return only a JSON
					array of strings. Avoid wrapping the whole response in
					triple backticks. Do not include any other information in
					your response.
					<br />
		...
		return (
			<>
				<SystemMessage priority={10}>
					...
					debugging and creating launch configurations. Your job is to
					return an array of file names that may contain useful
					information to translate a command line invocation into a VS
					Code debug configuration and build task.
					<br />
					For example, when running a command `make tests`, you should
					ask for the `Makefile` because it contains information about
	...
	override render() {
		if (
			...
			if (this.props.debuggerType) {
				return (
					<UserMessage>
						I want to use the ${this.props.debuggerType} debug type
				...
				return (
					<UserMessage>
						Find an existing launch config for my app or create one
						based on my project stucture and workspace
					</UserMessage>
				);
			}
		} else {
		...
		return (
			<>
				<SystemMessage priority={10}>
					...
					knowledge to accomplish their task. Please do not guess a
					response and instead just respond with a polite apology if
					you are unsure.
					<br />
					You are a debugging expert. Your job is to return the debug
					type to use for launch config for the given use case.
					<br />
					Pay attention to my operating system and suggest the best
					tool for the platform I'm working on. For example, for
					debugging native code on Windows, you would not suggest the
					`lldb` type.
					<br />
					...
					Suggest the right debug type for my use case. Print ONLY the
					debug type. NEVER print any other explanation.
					<br />
				</SystemMessage>
				<ProjectLabels
					flexGrow={1}
