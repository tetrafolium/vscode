...
export class ChatToolCalls extends PromptElement<ChatToolCallsProps, void> {
	...
	override async render(
		...
		const sharedImageBudget: SharedImageBudget = {
			remaining: CAPI_IMAGE_BUDGET_BYTES,
		};
	...
	private renderOneToolCallRound(
		...
		const roundModelId =
			round.modelId ??
			(round as IToolCallRound & { phaseModelId?: string }).phaseModelId;
		const sameModelAsEndpoint = roundModelId === this.promptEndpoint.model;
		const apiSupportsHistoricalThinking =
			this.promptEndpoint.apiType === 'responses';
		const includeThinking =
			sameModelAsEndpoint &&
			(!this.props.isHistorical || apiSupportsHistoricalThinking);
		const thinking = includeThinking && round.thinking && (
			<ThinkingDataContainer thinking={round.thinking} />
		);
		const phase =
			round.phase && roundModelId === this.promptEndpoint.model ? (
...
function buildImageUri(
	...
	return buildToolImageResourceUri(
		sessionId,
		coreToolCallId,
		imageIndex,
		getExtensionForMimeType(mimeType) ?? '.bin',
	);
}
...
function replaceImagesWithPlaceholders(
	...
	return content.map((part, index) => {
		...
		return new LanguageModelTextPart(
			`[Image was previously shown to you.${uriRef}]`,
		);
	});
}
			...
			<>
				<Chunk>
					<ToolResult
						...
						sharedImageBudget={this.props.sharedImageBudget}
					/>
				</Chunk>
			</>
		) : (
...
export function sendInvokedToolTelemetry(
	...
	toolName: string,
	toolResult: LanguageModelToolResult2,
	properties?: InvokedToolTelemetryProperties,
) {
	...
	PromptRenderer.create(
		...
		*/
			telemetry.sendMSFTTelemetryEvent(
				'agent.tool.responseLength',
				{ ...properties, model: endpoint.model, toolName },
				{ tokenCount },
			);
		});
}
...
> extends PromptElement<T> {
	...
	protected async onImage(part: LanguageModelDataPart, _imageIndex?: number) {
		...
		const canUpload =
			uploadsEnabled &&
			!!this.endpoint &&
			modelCanUseMcpResultImageURL(this.endpoint);
		...
		if (canUpload) {
			uploadToken = (
				await this.authService?.getGitHubSession('any', {
					silent: true,
				})
			)?.accessToken;
		}
...
export class ToolResult extends PrimitiveToolResult<IToolResultProps> {
	...
	protected override async onText(content: string): Promise<string> {
		...
		if (
			...
			this.props.toolName !== ToolName.ExploreSubagent &&
