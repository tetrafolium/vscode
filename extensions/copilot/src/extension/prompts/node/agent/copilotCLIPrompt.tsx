...
class CopilotCLIAgentUserMessage extends PromptElement<AgentUserMessageProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const shouldUseUserQuery =
			this.props.endpoint.family.startsWith('grok-code');
		...
		const isResourceVariable = (variable: PromptVariable) =>
			!isScmEntry(variable.value) &&
			(URI.isUri(variable.value) || isLocation(variable.value));
		const isImageReference = (variable: PromptVariable) =>
			variable.value && variable.value instanceof ChatReferenceBinaryData;
		const isImageReferenceWithUri = (variable: PromptVariable) =>
			variable.value &&
			variable.value instanceof ChatReferenceBinaryData &&
			!!variable.value.reference
				? true
				: false;

		const resourceVariables = this.props.chatVariables.filter(
			(variable) =>
				isResourceVariable(variable) ||
				isImageReferenceWithUri(variable),
		);
		const nonResourceVariables = this.props.chatVariables.filter(
			(variable) =>
				!isResourceVariable(variable) && !isImageReference(variable),
		);
		const [nonResourceAttachments, resourceAttachments] = await Promise.all(
			[
				renderChatVariables(
					nonResourceVariables,
		...
		const hasVariables =
			resourceVariables.hasVariables() ||
			nonResourceVariables.hasVariables();
		const hasEditedFileEvents =
		...
		const instructions =
			promptVariable &&
			promptVariable.reference.name !== 'prompt:plan.prompt.md'
		...
		return (
			<UserMessage>
				...
				{hasCustomContext && (
					<>
						<br />{' '}
						{/** Add an empty line after user prompt to ensure `<reminder>` tag is on a new line */}
						<Tag name="reminder">{instructions}</Tag>
					</>
				)}
				{hasVariables && (
				...
				{hasCustomContext && (
					<Tag
						name={shouldUseUserQuery ? 'user_query' : 'userRequest'}
						priority={900}
						flexGrow={7}
					>
						{query}
...
export async function generateUserPrompt(
	...
	const endpoint = await instantiationService.invokeFunction((accessor) =>
		accessor.get(IEndpointProvider).getChatEndpoint(request),
	);
	const { messages } = await renderPromptElement(
		instantiationService,
		endpoint,
		CopilotCLIAgentUserMessage,
		{
			chatVariables,
			endpoint,
	...
	if (userMessages.length > 0) {
		...
		if (
			textParts.every(
				(part) => part.type === ChatCompletionContentPartKind.Text,
			)
		) {
...
async function renderResourceVariables(
	chatVariables: ChatVariablesCollection,
	...
	await Promise.all(
		Array.from(chatVariables).map(async (variable) => {
			...
			if (variable.value instanceof ChatReferenceBinaryData) {
				if (variable.value.reference) {
					...
					attrs.filePath =
						promptPathRepresentationService.getFilePath(
							variable.value.reference,
						);
			...
			if (isLocation(location)) {
				...
				const alwaysIncludeSummary =
					location.uri.scheme === Schemas.untitled;
				elements.push(
					<FileVariable
			...
			if (isDirectory) {
				attrs.folderPath =
					promptPathRepresentationService.getFilePath(uri);
			} else {
				attrs.filePath =
					promptPathRepresentationService.getFilePath(uri);
			}
