...
const SummaryPrompt = (
	<>
		...
		conversation that captures all essential information needed to
		seamlessly continue the work without any loss of context. This summary
		will be used to compact the conversation while preserving critical
		technical details, decisions, and progress.
		<br />
		## Recent Context Analysis
		<br />
		Pay special attention to the most recent agent commands and tool
		executions that led to this summarization being triggered. Include:
		<br />
		- **Last Agent Commands**: What specific actions/tools were just
		executed
		<br />
		- **Tool Results**: Key outcomes from recent tool calls (truncate if
		very long, but preserve essential information)
		<br />
		- **Immediate State**: What was the system doing right before
		summarization
		<br />
		...
		`&lt;analysis&gt;` tags to organize your thoughts systematically:
		<br />
		1. **Chronological Review**: Go through the conversation
		chronologically, identifying key phases and transitions
		<br />
		2. **Intent Mapping**: Extract all explicit and implicit user requests,
		goals, and expectations
		<br />
		3. **Technical Inventory**: Catalog all technical concepts, tools,
		frameworks, and architectural decisions
		<br />
		4. **Code Archaeology**: Document all files, functions, and code
		patterns that were discussed or modified
		<br />
		5. **Progress Assessment**: Evaluate what has been completed vs. what
		remains pending
		<br />
		6. **Context Validation**: Ensure all critical information for
		continuation is captured
		<br />
		7. **Recent Commands Analysis**: Document the specific agent commands
		and tool results from the most recent operations
		<br />
		## Summary Structure
		<br />
		Your summary must include these sections in order, following the exact
		format below:
		<br />
		<Tag name="analysis">
			[Chronological Review: Walk through conversation phases: initial
			request → exploration → implementation → debugging → current state]
			<br />
			[Intent Mapping: List each explicit user request with message
			context]
			<br />
			[Technical Inventory: Catalog all technologies, patterns, and
			decisions mentioned]
			<br />
			[Code Archaeology: Document every file, function, and code change
			discussed]
			<br />
			...
			<br />
			[Recent Commands Analysis: Last agent commands executed, tool
			results (truncated if long), immediate pre-summarization state]
			<br />
		</Tag>
		<br />
		<Tag name="summary">
			...
			<br />
			- Primary Objectives: [All explicit user requests and overarching
			goals with exact quotes]
			<br />
			- Session Context: [High-level narrative of conversation flow and
			key phases]
			<br />
			- User Intent Evolution: [How user's needs or direction changed
			...
			<br />
			- Key Code Segments: [Important functions/classes with brief
			explanations]
			<br />
			...
			<br />
			- Issues Encountered: [Technical problems, bugs, or challenges
			faced]
			<br />
			- Solutions Implemented: [How problems were resolved and reasoning]
			<br />
			- Debugging Context: [Ongoing troubleshooting efforts or known
			issues]
			<br />
			...
			5. Progress Tracking:
			<br />
			- Completed Tasks: [What has been successfully implemented with
			status indicators]
			<br />
			- Partially Complete Work: [Tasks in progress with current
			completion status]
			<br />
			- Validated Outcomes: [Features or code confirmed working through
			testing]
			<br />
			6. Active Work State:
			<br />
			- Current Focus: [Precisely what was being worked on in most recent
			messages]
			<br />
			- Recent Context: [Detailed description of last few conversation
			exchanges]
			<br />
			- Working Code: [Code snippets being modified or discussed recently]
			<br />
			- Immediate Context: [Specific problem or feature being addressed
			before summary]
			<br />
			7. Recent Operations:
			<br />
			- Last Agent Commands: [Specific tools/actions executed just before
			summarization with exact command names]
			<br />
			- Tool Results Summary: [Key outcomes from recent tool executions -
			truncate long results but keep essential info]
			<br />
			- Pre-Summary State: [What the agent was actively doing when token
			budget was exceeded]
			<br />
			- Operation Context: [Why these specific commands were executed and
			their relationship to user goals]
			<br />
			8. Continuation Plan:
			<br />
			- [Pending Task 1]: [Details and specific next steps with verbatim
			quotes]
			<br />
			- [Pending Task 2]: [Requirements and continuation context]
			<br />
			- [Priority Information]: [Which tasks are most urgent or logically
			sequential]
			<br />
			- [Next Action]: [Immediate next step with direct quotes from recent
			messages]
			<br />
		</Tag>
		<br />
		## Quality Guidelines
		<br />
		- **Precision**: Include exact filenames, function names, variable
		names, and technical terms
		<br />
		- **Completeness**: Capture all context needed to continue without
		re-reading the full conversation
		<br />
		- **Clarity**: Write for someone who needs to pick up exactly where the
		conversation left off
		<br />
		- **Verbatim Accuracy**: Use direct quotes for task specifications and
		recent work context
		<br />
		- **Technical Depth**: Include enough detail for complex technical
		decisions and code patterns
		<br />
		- **Logical Flow**: Present information in a way that builds
		understanding progressively
		<br />
		This summary should serve as a comprehensive handoff document that
		enables seamless continuation of all active work streams while
		preserving the full technical and contextual richness of the original
		conversation.
		<br />
	</>
);
		...
		return (
			<>
				...
				{this.props.workingNotebook && (
					<WorkingNotebookSummary
						priority={this.props.priority - 2}
						notebook={this.props.workingNotebook}
					/>
				)}
				<UserMessage priority={this.props.priority}>
					Summarize the conversation history so far, paying special
					attention to the most recent agent commands and tool results
					that triggered this summarization. Structure your summary
					using the enhanced format provided in the system message.
					<br />
					{isOpus && (
						<>
							<br />
							IMPORTANT: Do NOT call any tools. Your only task is
							to generate a text summary of the conversation. Do
							not attempt to execute any actions or make any tool
							calls.
							<br />
						</>
					)}
					...
					(truncate if very long but preserve key information)
					<br />
					- What the agent was actively working on when the token
					budget was exceeded
					<br />
					- How these recent operations connect to the overall user
					goals
					<br />
					Include all important tool calls and their results as part
					of the appropriate sections, with special emphasis on the
...
class ConversationHistory extends PromptElement<SummarizedAgentHistoryProps> {
	override async render(state: void, sizing: PromptSizing) {
		...
		let thinkingForFirstRoundAfterSummarization: ThinkingData | undefined =
			undefined;
		if (this.props.promptContext.toolCallRounds?.length) {
			const toolCallRounds: IToolCallRound[] = [];
			for (
				let i = this.props.promptContext.toolCallRounds.length - 1;
				i >= 0;
				i--
			) {
				const toolCallRound =
					this.props.promptContext.toolCallRounds[i];
				if (toolCallRound.summary) {
					// This tool call round was summarized
					summaryForCurrentTurn = toolCallRound.summary;
					thinkingForFirstRoundAfterSummarization =
						toolCallRound.thinking;
					break;
				}
			...
			if (
				isAnthropicFamily(this.props.endpoint) &&
				thinkingForFirstRoundAfterSummarization &&
				toolCallRounds.length > 0 &&
				!toolCallRounds[0].thinking
			) {
				toolCallRounds[0].thinking =
					thinkingForFirstRoundAfterSummarization;
			}
		...
		if (summaryForCurrentTurn) {
			history.push(
				<SummaryMessageElement
					endpoint={this.props.endpoint}
					summaryText={summaryForCurrentTurn}
				/>,
			);
		...
		if (
			!this.props.promptContext.isContinuation ||
			this.props.promptContext.hasStopHookQuery
		) {
			history.push(
				<AgentUserMessage
					flexGrow={2}
					...
					{...getUserMessagePropsFromAgentProps(this.props, {
						...
						ReminderInstructionsClass:
							this.props.ReminderInstructionsClass,
						ToolReferencesHintClass:
							this.props.ToolReferencesHintClass,
					})}
		...
		for (const [i, turn] of [
			...this.props.promptContext.history.entries(),
		].reverse()) {
			...
			if (toolCallResultInNextTurn) {
				const nextMetadata = this.props.promptContext.history.at(i + 1)
					?.responseChatResult?.metadata as
					| IResultMetadata
					| undefined;
				const mergeFrom =
					i === this.props.promptContext.history.length - 1
			...
			if (summaryForTurn) {
				...
				turnComponents.push(
					<SummaryMessageElement
						endpoint={this.props.endpoint}
						summaryText={summaryForTurn.text}
					/>,
				);
...
export class SummarizedConversationHistoryMetadata extends PromptMetadata {
	...
	constructor(
		...
		this.numRoundsSinceLastSummarization =
			options?.numRoundsSinceLastSummarization;
...
export interface SummarizedAgentHistoryProps
...
export class SummarizedConversationHistory extends PromptElement<SummarizedAgentHistoryProps> {
	...
	override async render(
		...
		if (this.props.triggerSummarize) {
			...
			if (summResult) {
				historyMetadata = new SummarizedConversationHistoryMetadata(
					summResult.toolCallRoundId,
					summResult.summary,
					{
						...
						numRoundsSinceLastSummarization:
							summResult.numRoundsSinceLastSummarization,
						durationMs: summResult.durationMs,
					},
				);
				this.addSummaryToHistory(
					summResult.summary,
					summResult.toolCallRoundId,
					summResult.thinking,
				);
	...
	private async ensureTranscriptSession(): Promise<void> {
		...
		await this.sessionTranscriptService.startSession(
			sessionId,
			undefined,
	...
	private addSummaryToHistory(
		summary: string,
		toolCallRoundId: string,
		thinking?: ThinkingData,
	): void {
		const round = this.props.promptContext.toolCallRounds?.find(
			(round) => round.id === toolCallRoundId,
		);
		if (round) {
...
class ConversationHistorySummarizer {
	...
	> {
		...
		const propsInfo = this.instantiationService
			.createInstance(SummarizedConversationHistoryPropsBuilder)
			.getProps(this.props);
		if (!propsInfo) {
			this.logService.info(
				'[ConversationHistorySummarizer] no prior content to summarize, skipping',
			);
			return undefined;
		}
	...
	private appendTranscriptHint(summary: string): string {
		...
		return appendTranscriptHintToSummary(
			summary,
			sessionId,
	...
	private async getSummaryWithFallback(
		propsInfo: ISummarizedConversationHistoryInfo,
	): Promise<SummarizationResult> {
		const forceMode = this.configurationService.getConfig<
			string | undefined
		>(ConfigKey.Advanced.AgentHistorySummarizationMode);
	...
	private async executePreCompactHook(): Promise<void> {
		...
		try {
			const results = await this.chatHookService.executeHook(
				'PreCompact',
				hooks,
				{
					trigger: 'auto',
				} satisfies PreCompactHookInput,
				this.props.promptContext.conversation?.sessionId,
			...
			for (const result of results) {
				if (result.resultKind === 'error') {
					...
					this.logService.error(
						`[ConversationHistorySummarizer] PreCompact hook error: ${errorMessage}`,
					);
				}
			}
		} catch (error) {
			this.logService.error(
				'[ConversationHistorySummarizer] Error executing PreCompact hook',
				error,
			);
		}
	}
	...
	private async getSummary(
		...
		const associatedRequestId =
			this.props.promptContext.conversation?.getLatestTurn().id;
		try {
			summarizationPrompt = (
				await renderPromptElement(
					...
					ConversationHistorySummarizationPrompt,
					{
						...
						simpleMode: mode === SummaryMode.Simple,
					},
					undefined,
					this.token,
				)
			...
			this.logInfo(
				`summarization prompt rendered in ${stopwatch.elapsed()}ms.`,
				mode,
			);
		} catch (e) {
			...
			this.logInfo(
				`Error rendering summarization prompt in mode: ${mode}. ${e.stack}`,
				mode,
			);
			this.sendSummarizationTelemetry(
				outcome,
				'',
				this.props.endpoint.model,
				mode,
				stopwatch.elapsed(),
				undefined,
		...
		try {
			...
			if (isGeminiFamily(endpoint)) {
				const validationResult =
					ToolCallingLoop.validateToolMessagesCore(messages, {
						stripOrphanedToolCalls: true,
					});
				messages = validationResult.messages;
				if (validationResult.strippedToolCallCount > 0) {
					this.logInfo(
						`Stripped ${validationResult.strippedToolCallCount} orphaned tool calls from summarization prompt`,
						mode,
					);
					/* __GDPR__
					...
					this.telemetryService.sendMSFTTelemetryEvent(
						...
						},
						{
							strippedToolCallCount:
								validationResult.strippedToolCallCount,
						},
					);
				}
			}

			promptTypes = messages
				.map(
					(msg) =>
			...
			this.logInfo(
				`Error from summarization request. ${e.message}`,
				mode,
			);
			this.sendSummarizationTelemetry(
				'requestThrow',
		...
		const promptTokenDetails = await computePromptTokenDetails({
			...
			totalPromptTokens:
				summaryResponse.type === ChatFetchResponseType.Success
					? summaryResponse.usage?.prompt_tokens
					: undefined,
		});

		const durationMs = stopwatch.elapsed();
		return {
			result: await this.handleSummarizationResponse(
				summaryResponse,
				mode,
				durationMs,
				promptTypes,
	...
	private async handleSummarizationResponse(
		...
		if (response.type !== ChatFetchResponseType.Success) {
			...
			this.sendSummarizationTelemetry(
				...
				elapsedTime,
				undefined,
				response.reason ?? response.type,
			);
			this.logInfo(
				`Summarization request failed. ${response.type} ${response.reason ?? response.type}`,
				mode,
			);
		...
		if (summarySize > effectiveBudget) {
			this.sendSummarizationTelemetry(
				...
				elapsedTime,
				response.usage,
				`${summarySize} tokens exceeds budget ${effectiveBudget}`,
			);
			this.logInfo(
		...
		this.logInfo(
			`Summarization usage: prompt=${response.usage?.prompt_tokens ?? '?'}, cached=${response.usage?.prompt_tokens_details?.cached_tokens ?? '?'}, completion=${response.usage?.completion_tokens ?? '?'}`,
			mode,
		);
		return response;
	}
	 ...
	 */
	private sendSummarizationTelemetry(
		outcome: string,
		requestId: string,
		model: string,
		mode: SummaryMode,
		...
		const curTurnRoundIndex =
			this.props.promptContext.toolCallRounds?.length ?? 0;

		const lastUsedTool =
			this.props.promptContext.toolCallRounds?.at(-1)?.toolCalls?.at(-1)
				?.name ??
			this.props.promptContext.history
				?.at(-1)
				?.rounds.at(-1)
		...
		const isDuringToolCalling = !!this.props.promptContext.toolCallRounds
			?.length
			? 1
			: 0;
 ...
 * session.
 */
export function appendTranscriptHintToSummary(
	summary: string,
	sessionId: string,
...
export function computeSummarizationRoundCounts(
	...
	const numRoundsInHistory = history.reduce(
		(sum, turn) => sum + turn.rounds.length,
		0,
	);
	...
	let numRoundsSinceLastSummarization = reversedCurrentRounds.findIndex(
		(round) => round.summary,
	);
	if (numRoundsSinceLastSummarization === -1) {
		...
		outer: for (const turn of Iterable.reverse(Array.from(history))) {
			for (const round of Iterable.reverse(
				Array.from(turn.rounds ?? []),
			)) {
				if (round.summary) {
 ...
 */
export function stripToolSearchMessages(
	...
	return messages
		.map((message) => {
			if (message.role === Raw.ChatRole.Assistant && message.toolCalls) {
				...
				message.role === Raw.ChatRole.Tool &&
				message.toolCallId &&
				toolSearchIds.has(message.toolCallId)
			) {
				return undefined;
			}
			return message;
...
export class SummarizedConversationHistoryPropsBuilder {
	...
	getProps(
		...
		if (toolCallRounds && toolCallRounds.length > 1) {
			...
			isContinuation = true;
			toolCallRounds = [];
			summarizedToolCallRoundId = props.promptContext.history
				.at(-1)!
				.rounds.at(-1)!.id;
		} else {
			return undefined;
		...
		const summarizedThinking = isAnthropicFamily(props.endpoint)
			? this.findLastThinking(props)
			: undefined;
		const promptContext = {
			...props.promptContext,
			toolCallRounds,
	...
	private findLastThinking(
		props: SummarizedAgentHistoryProps,
	): ThinkingData | undefined {
		if (props.promptContext.toolCallRounds) {
			for (
				let i = props.promptContext.toolCallRounds.length - 1;
				i >= 0;
				i--
			) {
	...
	private getWorkingNotebook(
		props: SummarizedAgentHistoryProps,
	): NotebookDocument | undefined {
		const toolCallRound =
			props.promptContext.toolCallRounds &&
			[...props.promptContext.toolCallRounds]
				.reverse()
				.find((round) =>
					round.toolCalls.some(
						(call) => call.name === ToolName.RunNotebookCell,
					),
				);
		const toolCall = toolCallRound?.toolCalls.find(
			(call) => call.name === ToolName.RunNotebookCell,
		);
		if (toolCall && toolCall.arguments) {
			try {
				...
				if (typeof args.filePath === 'string') {
					const uri =
						this._promptPathRepresentationService.resolveFilePath(
							args.filePath,
						);
					if (!uri) {
						return undefined;
					}
					return this._workspaceService.notebookDocuments.find(
	...
	override async render(state: void, sizing: PromptSizing) {
		...
		return (
			<UserMessage priority={1000}>
				The conversation has grown too large for the context window and
				...
				<br />
				<br />
				IMPORTANT: Output your summary wrapped in {'<summary>'} and{' '}
				{'</summary>'} tags. Do NOT call any tools. Your ONLY task right
				now is to produce a comprehensive summary of the conversation so
				far.
				<br />
				{isOpus && (
					<>
						<br />
						IMPORTANT: Do NOT call any tools. Your only task is to
						generate a text summary of the conversation. Do not
						attempt to execute any actions or make any tool calls.
						<br />
					</>
				)}
			</UserMessage>
