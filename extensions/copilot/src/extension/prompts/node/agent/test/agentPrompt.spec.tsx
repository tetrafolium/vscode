...
testFamilies.forEach((family) => {
	suite(`AgentPrompt - ${family}`, () => {
		...
		beforeAll(() => {
			...
			services.define(
				IChatMLFetcher,
				new StaticChatMLFetcher(chatResponse),
			);
		...
		test('simple case', async () => {
			...
			).toMatchFileSnapshot(getSnapshotFile('simple_case'));
		});
		...
		test('all tools', async () => {
			...
			).toMatchFileSnapshot(getSnapshotFile('all_tools'));
		});

		test('all non-edit tools', async () => {
			const toolsService = accessor.get(IToolsService);
			const editTools: Set<string> = new Set([
				ToolName.ApplyPatch,
				ToolName.EditFile,
			...
			await expect(
				await agentPromptToString(
					accessor,
					{
						...
						query: 'hello',
						tools: {
							availableTools: toolsService.tools.filter(
								(t) => !editTools.has(t.name),
							),
							toolInvocationToken: null as never,
							toolReferences: [],
						},
			...
			).toMatchFileSnapshot(getSnapshotFile('all_non_edit_tools'));
		});
		...
		test('one attachment', async () => {
			...
			).toMatchFileSnapshot(getSnapshotFile('one_attachment'));
		});
		...
		test('tool use', async () => {
			...
			).toMatchFileSnapshot(getSnapshotFile('tool_use'));
		});
		...
		test('summarization without cache breakpoints (Messages API config)', async () => {
			await expect(
				...
				getSnapshotFile('summarization_no_cache_bps'),
			);
		});
		...
		test('custom instructions not in system message', async () => {
			accessor
				.get(IConfigurationService)
				.setConfig(ConfigKey.CustomInstructionsInSystemMessage, false);
			await expect(
				await agentPromptToString(
					accessor,
					{
						...
						modeInstructions: {
							name: 'Plan',
							content: 'custom mode instructions',
						},
					},
					undefined,
				),
			).toMatchFileSnapshot(
				getSnapshotFile('custom_instructions_not_in_system_message'),
			);
		});
		...
		test('omit base agent instructions', async () => {
			accessor
				.get(IConfigurationService)
				.setConfig(ConfigKey.Advanced.OmitBaseAgentInstructions, true);
			await expect(
				await agentPromptToString(
					accessor,
					{
				...
				getSnapshotFile('omit_base_agent_instructions'),
			);
		});
			...
			await expect(
				await agentPromptToString(
					accessor,
					{
						...
						editedFileEvents: [
							{
								eventKind: ChatRequestEditedFileEventKind.Undo,
								uri: fileTsUri,
							},
							{
								eventKind:
				...
				getSnapshotFile('edited_file_events_grouped_by_kind'),
			);
		});
	});
});
