...
suite('Agent Summarization', () => {
	...
	beforeAll(async () => {
		...
		accessor
			...
			.setConfig(ConfigKey.CodeGenerationInstructions, [
				{
	...
	async function agentPromptToString(
		accessor: ITestingServicesAccessor,
		promptContext: IBuildPromptContext,
		otherProps?: Partial<AgentPromptProps>,
		...
		if (promptType === 'Agent') {
			...
			const propsInfo = instaService
				.createInstance(SummarizedConversationHistoryPropsBuilder)
				.getProps(baseProps);
			expect(
				propsInfo,
				'expected propsInfo to be defined for test scenario',
			).toBeDefined();
			const simpleMode =
				promptType === TestPromptType.SimpleSummarization;
			renderer = PromptRenderer.create(
				instaService,
				endpoint,
				ConversationHistorySummarizationPrompt,
		...
		const summarizedConversationMetadata = r.metadata.get(
			SummarizedConversationHistoryMetadata,
		);
		if (summarizedConversationMetadata && promptContext.toolCallRounds) {
			for (const toolCallRound of promptContext.toolCallRounds) {
				if (
					toolCallRound.id ===
					summarizedConversationMetadata.toolCallRoundId
				) {
	...
	function createEditFileToolResult(
		...idxs: number[]
	): Record<string, LanguageModelToolResult> {
		const result: Record<string, LanguageModelToolResult> = {};
		for (const idx of idxs) {
			result[`tooluse_${idx}`] = new LanguageModelToolResult([
				new LanguageModelTextPart('success'),
			]);
		}
		return result;
	}
	...
	test('continuation turns are not rendered in conversation history', async () => {
		const firstTurn = new Turn('id1', {
			type: 'user',
			message: 'previous turn message',
		});
		const continuationTurn = new Turn(
			'id2',
			{ type: 'user', message: 'continuation turn message' },
			undefined,
			[],
			undefined,
			undefined,
		...
		const promptContext: IBuildPromptContext = {
			chatVariables: new ChatVariablesCollection([
				{ id: 'vscode.file', name: 'file', value: fileTsUri },
			]),
	...
	test('cannot summarize with no history', async () => {
		...
		await expect(() =>
			agentPromptToString(
				...
				undefined,
				TestPromptType.FullSummarization,
			),
		).rejects.toThrow();
		await expect(() =>
			agentPromptToString(
	...
	async function testTriggerSummarizationDuringToolCalling(
		...
		).toMatchFileSnapshot(getSnapshotFile(promptType, 'duringToolCalling'));
	...
	test('trigger summarization during tool calling', async () =>
		await testTriggerSummarizationDuringToolCalling(TestPromptType.Agent));
	test('FullSummarization - trigger summarization during tool calling', async () =>
		await testTriggerSummarizationDuringToolCalling(
			TestPromptType.FullSummarization,
		));
	test('SimpleSummarization - trigger summarization during tool calling', async () =>
		await testTriggerSummarizationDuringToolCalling(
			TestPromptType.SimpleSummarization,
		));

	async function testSummaryCurrentTurn(promptType: TestPromptType) {
		const excludedPreviousRound = new ToolCallRound(
			'previous round EXCLUDED',
			[createEditFileToolCall(1)],
		);
		...
		await expect(
			await agentPromptToString(
				accessor,
				{
					...
					history: [],
					query: 'edit this file',
					toolCallRounds: [excludedPreviousRound, round],
		...
		).toMatchFileSnapshot(getSnapshotFile(promptType, 'currentTurn'));
	}

	// SummarizationPrompt test is not relevant when the last round was summarized
	test('render summary in current turn', async () =>
		await testSummaryCurrentTurn(TestPromptType.Agent));

	async function testSummaryCurrentTurnEarlierRound(
		promptType: TestPromptType,
	) {
		const round = new ToolCallRound('round 1', [createEditFileToolCall(1)]);
		round.summary = 'summarized!';
		const round2 = new ToolCallRound('round 2', [
			createEditFileToolCall(2),
		]);
		const round3 = new ToolCallRound('round 3', [
			createEditFileToolCall(3),
		]);
		await expect(
			await agentPromptToString(
				accessor,
				{
					...
					history: [],
					query: 'edit this file',
					toolCallRounds: [round, round2, round3],
					toolCallResults: createEditFileToolResult(1, 2, 3),
					tools,
				},
				{
			...
			getSnapshotFile(promptType, 'currentTurnEarlierRound'),
		);
	}

	test('render summary in previous turn', async () =>
		await testSummaryCurrentTurnEarlierRound(TestPromptType.Agent));
	test('FullSummarization - render summary in previous turn', async () =>
		await testSummaryCurrentTurnEarlierRound(
			TestPromptType.FullSummarization,
		));
	test('SimpleSummarization - render summary in previous turn', async () =>
		await testSummaryCurrentTurnEarlierRound(
			TestPromptType.SimpleSummarization,
		));

	async function testSummaryPrevTurnMultiple(promptType: TestPromptType) {
		const previousTurn = new Turn('id', {
			type: 'user',
			message: 'previous turn excluded',
		});
		...
		await expect(
			await agentPromptToString(
				accessor,
				{
					...
					query: 'edit this file',
					toolCallRounds: [
						new ToolCallRound('hello next round', [
							createEditFileToolCall(5),
						]),
					],
			...
			getSnapshotFile(promptType, 'previousTurnMultiple'),
		);
	}

	test('render summary in previous turn (with multiple)', () =>
		testSummaryPrevTurnMultiple(TestPromptType.Agent));
	test('FullSummarization - render summary in previous turn (with multiple)', () =>
		testSummaryPrevTurnMultiple(TestPromptType.FullSummarization));
	test('SimpleSummarization - render summary in previous turn (with multiple)', () =>
		testSummaryPrevTurnMultiple(TestPromptType.SimpleSummarization));
	...
	test('manual /compact summary is honored when summarization is disabled (#311503)', async () => {
		...
		const previousTurn = new Turn('id', {
			type: 'user',
			message: 'first user message EXCLUDED',
		});
		previousTurn.setResponse(
			TurnStatus.Success,
			{ type: 'user', message: 'response' },
		...
		const compactedTurn = new Turn('id', {
			type: 'user',
			message: 'second user message EXCLUDED',
		});
		compactedTurn.setResponse(
			TurnStatus.Success,
			{ type: 'user', message: 'response' },
			'responseId',
			{
				metadata: {
					...
					toolCallRounds: [
						new ToolCallRound(
							'summarized round EXCLUDED',
							[createEditFileToolCall(2)],
							undefined,
							'roundId2',
						),
						new ToolCallRound(
							'round after summary KEPT',
							[createEditFileToolCall(3)],
							undefined,
							'roundId3',
						),
	...
	async function testSummarizeWithNoRoundsInCurrentTurn(
		promptType: TestPromptType,
	) {
		const previousTurn1 = new Turn('id', {
			type: 'user',
			message: 'previous turn 1',
		});
		previousTurn1.setResponse(
			TurnStatus.Success,
			{ type: 'user', message: 'response' },
			'responseId',
			{},
		...
		const previousTurn2 = new Turn('id', {
			type: 'user',
			message: 'previous turn 2',
		});
		...
		await expect(
			...
			getSnapshotFile(promptType, 'previousTurnNoRounds'),
		);
	}

	test('summary for previous turn, no tool call rounds', async () =>
		testSummarizeWithNoRoundsInCurrentTurn(TestPromptType.Agent));
	test('FullSummarization - summary for previous turn, no tool call rounds', async () =>
		testSummarizeWithNoRoundsInCurrentTurn(
			TestPromptType.FullSummarization,
		));
	test('SimpleSummarization - summary for previous turn, no tool call rounds', async () =>
		testSummarizeWithNoRoundsInCurrentTurn(
			TestPromptType.SimpleSummarization,
		));
	...
	test('failed summarization throws from renderer (fallback is in agentIntent)', async () => {
		...
		const { instaService, endpoint, historyProps } =
			createSummarizationTestContext();
	...
	test('failed summarization does not set round.summary', async () => {
		chatResponse[0] = 'summary that is definitely too large for one token';
		const { instaService, endpoint, toolCallRounds, historyProps } =
			createSummarizationTestContext();
	...
	test('simple mode summarization with small token budget renders zero messages (repro for No messages provided)', async () => {
		...
		const renderer = PromptRenderer.create(
			...
			endpoint,
			ConversationHistorySummarizationPrompt,
			{ ...propsInfo!.props, simpleMode: true },
		);
		const result = await renderer.render();
	...
	test('failure metadata on turn prevents repeated foreground summarization attempts', async () => {
		...
		const successMeta = turn.getMetadata(
			SummarizedConversationHistoryMetadata,
		);
		const shouldSkipAfterSuccess =
			successMeta!.source === 'foreground' &&
			!!successMeta!.outcome &&
...
suite('extractSummary', () => {
	...
	test('returns undefined when no tags found', () => {
		const text =
			'This is just a normal response with no summary tags at all.';
		const result = extractSummary(text);
...
suite('stripToolSearchMessages', () => {
	...
	test('strips custom tool_search tool_use and tool_result', () => {
		...
		expect(
			result.find(
				(m) => m.role === Raw.ChatRole.Tool && m.toolCallId === 'tc2',
			),
		).toBeUndefined();
	});
...
suite('appendTranscriptHintToSummary', () => {
	class FakeTranscriptService extends NullSessionTranscriptService {
		...
		override getTranscriptPath(): URI | undefined {
			return this.path;
		}
		override getLineCount(): number | undefined {
	...
	test('returns summary unchanged when no transcript path is available', () => {
		...
		const result = appendTranscriptHintToSummary(
			'original summary',
			'session-1',
			svc,
		);
	...
	test('bakes line count snapshot into hint when available', () => {
		...
		expect(result).toContain(
			'At the time this summary was created, the transcript had 42 lines.',
		);
		expect(result).toContain(transcript.fsPath);
	});
});
