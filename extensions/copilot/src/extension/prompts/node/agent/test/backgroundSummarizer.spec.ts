...
describe('BackgroundSummarizer', () => {
	...
	test('successful work transitions to Completed', async () => {
		const summarizer = new BackgroundSummarizer(100_000);
		const result: IBackgroundSummarizationResult = {
	...
	test('consumeAndReset returns result and resets to Idle', async () => {
		const summarizer = new BackgroundSummarizer(100_000);
		const expected: IBackgroundSummarizationResult = {
			summary: 'test summary',
			toolCallRoundId: 'round1',
		};
...
const { warmJitterMin, warmJitterSpan, emergency } =
	BackgroundSummarizationThresholds;
...
describe('shouldKickOffBackgroundSummarization', () => {
	describe('cold cache', () => {
		test('defers kick-off below the emergency threshold', () => {
			...
			expect(
				shouldKickOffBackgroundSummarization(0.85, false, unusedRng),
			).toBe(false);
		});
		...
		test('kicks off at the emergency threshold', () => {
			...
			expect(
				shouldKickOffBackgroundSummarization(0.91, false, unusedRng),
			).toBe(true);
		});
	...
	describe('warm cache', () => {
		test('kicks off at the jittered midpoint (0.80) when ratio meets it', () => {
			expect(
				shouldKickOffBackgroundSummarization(0.8, true, midRng),
			).toBe(true);
			expect(
				shouldKickOffBackgroundSummarization(0.81, true, midRng),
			).toBe(true);
		});
		...
		test('defers when ratio is under the jittered threshold', () => {
			...
			expect(
				shouldKickOffBackgroundSummarization(0.77, true, midRng),
			).toBe(false);
			// Also below the minimum of the window regardless of rng.
			expect(
				shouldKickOffBackgroundSummarization(
					warmJitterMin - 0.0001,
					true,
					() => 0,
				),
			).toBe(false);
		...
		test('respects the top of the jitter range', () => {
			...
			expect(
				shouldKickOffBackgroundSummarization(0.81, true, maxRng),
			).toBe(false);
			// 0.82 meets it.
			expect(
				shouldKickOffBackgroundSummarization(
					warmJitterMin + warmJitterSpan,
					true,
					maxRng,
				),
			).toBe(true);
