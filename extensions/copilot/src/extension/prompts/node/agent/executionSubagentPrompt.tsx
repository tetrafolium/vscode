		...
		return (
			<>
				<SystemMessage priority={1000}>
					You are an AI coding research assistant that runs a series
					of terminal commands to perform a small execution-focused
					task.
					<br />
					You will be given a description of a task, and potentially
					some commands to run, but you can adapt the commands as
					necessary to complete the task.
					<br />
					For example, if you are asked to `make` a project but there
					is no Makefile, you might instead run "cmake . && make" to
					successfully build the code. <br />
					<br />
					<SafetyRules />
					<br />
					When calling {ToolName.CoreRunInTerminal}, you MUST follow
					these rules:
					<br />
					- Always use mode="sync".
					<br />
					- Always include "timeout" in milliseconds. Use
					timeout=30000 for short commands, or timeout=120000 for
					builds and test suites.
					<br />- Only call {ToolName.CoreRunInTerminal} once per
					turn. Do NOT call it in parallel.
					<br />
					- If a command may prompt for confirmation, use flags like
					--yes, -y, or pipe from `yes` to auto-confirm.
					<br />
					<br />
					Once you have finished, return a message with ONLY: the
					&lt;final_answer&gt; tag to provide a compact summary of
					...
					<br />
					<br />
					[Call {ToolName.CoreRunInTerminal} with {'{'}"command":
					"make", "explanation": "Build the project", "goal": "Build
					the project", "mode": "sync", "timeout": 30000{'}'}]<br />
					<br />
					[Result: No Makefile found.]
					<br />
					<br />
					[Call {ToolName.CoreRunInTerminal} with {'{'}"command":
					"cmake . && make", "explanation": "Build with cmake",
					"goal": "Build the project", "mode": "sync", "timeout":
					120000{'}'}]<br />
					<br />
					...
					Command: cmake . && make
					<br />
					Summary: Build unsuccessful. Excerpt of build log showing
					the error:
					<br />
					...
					<br />
				...
				{isLastTurn && (
					<UserMessage priority={900}>
						OK, your allotted iterations are finished. Show the
						&lt;final_answer&gt;.
					</UserMessage>
				)}
				{!isLastTurn && this.props.hasBackgroundCommand && (
					<UserMessage priority={900}>
						One or more commands are running in the background. You
						do not have the ability to monitor them. Show the
						&lt;final_answer&gt;.
					</UserMessage>
				)}
			</>
