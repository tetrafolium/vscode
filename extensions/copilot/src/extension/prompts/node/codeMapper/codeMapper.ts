...
import {
	...
	CodeMapperPromptProps,
} from './codeMapperPrompt';
import { ICodeMapperTelemetryInfo } from './codeMapperService';
import {
	findEdit,
	getCodeBlock,
	iterateSectionsForResponse,
	Marker,
	Patch,
	Section,
} from './patchEditGeneration';

export type ICodeMapperDocument =
...
export async function processFullRewriteNotebook(
	...
	for await (const edit of processFullRewriteNotebookEdits(
		document,
		inputStream,
...
export async function* processFullRewriteNotebookEdits(
	...
	telemetryOptions: NotebookEditGenerationTelemtryOptions,
	token: CancellationToken,
): AsyncIterable<CellOrNotebookEdit> {
	// emit start of notebook
	const cellMap = new ResourceMap<NotebookCell>();
	for await (const edit of alternativeNotebookEditGenerator.generateNotebookEdits(
		document,
		inputStream,
		telemetryOptions,
		token,
		...
		if (Array.isArray(edit)) {
			...
			const cell =
				...
				document
					.getCells()
					.find((cell) => isEqual(cell.document.uri, cellUri));
			if (cell) {
				cellMap.set(cellUri, cell);
				if (
					edit[1].length === 1 &&
					edit[1][0].range.isSingleLine &&
					cell.document.lineCount > edit[1][0].range.start.line
				) {
					if (
						cell.document.lineAt(edit[1][0].range.start.line)
							.text === edit[1][0].newText
					) {
						continue;
					}
				}
...
export async function processFullRewriteNewNotebook(
	uri: URI,
	source: string,
	outputStream: MappedEditsResponseStream,
	alternativeNotebookEditGenerator: IAlternativeNotebookContentEditGenerator,
...
function emitCodeLine(
	line: string,
	uri: Uri,
	existingDocument: TextDocumentSnapshot | undefined,
	outputStream: MappedEditsResponseStream,
	pushedLines: string[],
	token: CancellationToken,
) {
	...
	if (currentLineIndex < lineCount) {
		...
		const currentLineLength = existingDocument
			? existingDocument.lineAt(currentLineIndex).text.length
			: 0;
		outputStream.textEdit(uri, [
			TextEdit.replace(
				new Range(
					currentLineIndex,
					0,
		...
		outputStream.textEdit(uri, [
			TextEdit.replace(
				new Range(currentLineIndex, 0, currentLineIndex, 0),
				addedText,
			),
		]);
	}
...
async function processFullRewriteStream(
	uri: Uri,
	existingDocument: TextDocumentSnapshot | undefined,
...
async function handleTrailingLines(
	uri: Uri,
	existingDocument: TextDocumentSnapshot | undefined,
	outputStream: MappedEditsResponseStream,
	pushedLines: string[],
	...
	const lineCount = existingDocument ? existingDocument.lineCount : 0;
	const initialTrailingEmptyLineCount = existingDocument
		? getTrailingDocumentEmptyLineCount(existingDocument)
		: 0;
	...
	const pushedTrailingEmptyLineCount =
		getTrailingArrayEmptyLineCount(pushedLines);
	for (
		let i = pushedTrailingEmptyLineCount;
		i < initialTrailingEmptyLineCount;
		i++
	) {
		emitCodeLine(
			'',
	...
	if (currentLineIndex < lineCount) {
		const from =
			currentLineIndex === 0
				...
				: new Position(
						currentLineIndex - 1,
						pushedLines[pushedLines.length - 1].length,
					);
		outputStream.textEdit(uri, [
			TextEdit.delete(new Range(from, new Position(lineCount, 0))),
		]);
	}
}

async function processFullRewriteResponseCode(
	uri: Uri,
	existingDocument: TextDocumentSnapshot | undefined,
	inputStream: AsyncIterable<LineOfText>,
	outputStream: MappedEditsResponseStream,
	...
	const pushedLines = await processFullRewriteStream(
		uri,
		existingDocument,
		inputStream,
		outputStream,
...
export async function processPatchResponse(
	uri: URI,
...
export type ICodeMapperRequestInput =
	| ICodeMapperNewDocument
	| ICodeMapperExistingDocument;

export function isNewDocument(
	input: ICodeMapperRequestInput,
): input is ICodeMapperNewDocument {
...
export class CodeMapper {
	...
	constructor(
		...
		private readonly multiFileEditInternalTelemetryService: IMultiFileEditInternalTelemetryService,
		@IAlternativeNotebookContentEditGenerator
		...
		this.shortContextLimit =
			configurationService.getExperimentBasedConfig<number>(
				ConfigKey.Advanced.InstantApplyShortContextLimit,
	...
	public async mapCode(
		request: ICodeMapperRequestInput,
		resultStream: MappedEditsResponseStream,
		telemetryInfo: ICodeMapperTelemetryInfo | undefined,
		token: CancellationToken,
	): Promise<CodeMapperOutcome | undefined> {
		const fastEdit = await this.mapCodeUsingFastEdit(
			request,
			resultStream,
			telemetryInfo,
			token,
		...
		const longestDocumentContext = isNewDocument(request)
			? request.workingSet.reduce<ICodeMapperDocument | undefined>(
					(prev, curr) =>
						prev && prev.getText().length > curr.getText().length
							? prev
							: curr,
					undefined,
				)
		...
		const doFullRewrite = longestDocumentContext
			? (await chatEndpoint
					.acquireTokenizer()
					.tokenLength(longestDocumentContext.getText())) <
				(4096 / 4) * 3
			: true;
		...
		const cb: FinishedCallback = async (
			text: string,
			index: number,
		...
		if (doFullRewrite) {
			if (
				existingDocument &&
				existingDocument instanceof NotebookDocumentSnapshot
			) {
				// TODO@joyceerhl: Handle notebook document response processing
				const telemtryOptions: NotebookEditGenerationTelemtryOptions = {
					source: NotebookEditGenrationSource.codeMapperEditNotebook,
					requestId: undefined,
					model: chatEndpoint.model,
				};
				responsePromise = processFullRewriteNotebook(
		...
		const createOutcome = (
			...
			errorDetails: ChatErrorDetails | undefined,
		): CodeMapperOutcome => {
			return {
				errorDetails,
				annotations,
				telemetry: {
					...
					speculationRequestId: fetchResult.requestId,
					requestSource: String(telemetryInfo?.chatRequestSource),
					mapper: doFullRewrite ? 'full' : 'patch',
				},
			};
		};
		if (fetchResult.type === ChatFetchResponseType.Success) {
			...
			const errorDetails = getErrorDetailsFromChatFetchError(
				fetchResult,
				this.authenticationService.copilotToken?.copilotPlan,
				outageStatus,
				this.authenticationService.copilotToken?.tokenBasedBilling,
				this.authenticationService.copilotToken?.quotaInfo
					.quota_reset_date,
			);
				...
				[
					{
						...
						message: `request ${fetchResult.type}`,
						severity: 'error',
					},
				],
				errorDetails,
		...
		if (result.annotations.length || result.errorDetails) {
			this.logService.info(
				`[code mapper] Problems generating edits: ${result.annotations.map((a) => `${a.message} [${a.label}]`).join(', ')}, ${result.errorDetails?.message}`,
			);
		}
		return result;
	}
	...
	private async buildPrompt(
		...
		const promptRenderer = PromptRenderer.create(
			...
			{
				request,
				shouldTrimCodeBlocks: true,
			} satisfies CodeMapperPromptProps,
		);
		const uri = request.uri;

		const promptRendererResult = await promptRenderer.render(
			undefined,
			token,
		);
		const fence = isNewDocument(request)
			? '```'
			: getFenceForCodeBlock(request.existingDocument.getText());
		const languageId = isNewDocument(request)
			? getLanguageForResource(uri).languageId
			: request.existingDocument.languageId;
		const speculation = isNewDocument(request)
			? ''
			: request.existingDocument.getText();
		const messages: Raw.ChatMessage[] = [
			{
				role: Raw.ChatRole.User,
				content: [
					toTextPart(
						promptRendererResult.messages.reduce((prev, curr) => {
	...
	private async logDoneInfo(
		request: ICodeMapperRequestInput,
		prompt: IFullRewritePrompt,
		response: ICompletedRequest,
		telemetryInfo: CodeMapperOutcomeTelemetry,
		mapper: string,
		annotations: OutcomeAnnotation[],
	) {
		...
		const timeToFirstToken =
			firstTokenTime === -1 ? -1 : firstTokenTime - startTime;
		const timeToComplete = Date.now() - startTime;
		this.logService.info(
			`srequest done: ${timeToComplete}ms, chatRequestId: [${telemetryInfo?.requestId}], speculationRequestId: [${requestId}]`,
		);
		const isNoopEdit = responseText.trim() === speculation.trim();

		const { addedLines, removedLines } = await computeAdditionsAndDeletions(
			this.diffService,
			speculation,
			responseText,
		);
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent(
			'speculation.response.success',
			{
				...
				containsElidedCodeComments: String(
					responseText.includes(EXISTING_CODE_MARKER),
				),
				model: mapper,
			},
			{
				...
				addedLines,
				removedLines,
				isNotebook: this.notebookService.hasSupportedNotebooks(
					request.uri,
				)
					? 1
					: 0,
		...
		if (isNoopEdit) {
			const message =
				'Speculative response is identical to speculation, srequest: ' +
				requestId +
				',  URI: ' +
	...
	private async logError(
		request: ICodeMapperRequestInput,
		prompt: IFullRewritePrompt,
		response: Omit<ICompletedRequest, 'responseText'>,
		telemetryInfo: CodeMapperOutcomeTelemetry,
		mapper: string,
		errorMessage: string,
		error?: Error,
		...
		this.logService.error(
			`srequest failed: ${Date.now() - startTime}ms, chatRequestId: [${telemetryInfo?.requestId}], speculationRequestId: [${requestId}] error: [${errorMessage}]`,
		);
		if (error) {
	...
	private async mapCodeUsingFastEdit(
		request: ICodeMapperRequestInput,
		resultStream: MappedEditsResponseStream,
		telemetryInfo: ICodeMapperTelemetryInfo | undefined,
		token: CancellationToken,
	): Promise<CodeMapperOutcome | CodeMapperRefusal> {
		...
		if (
			...
			this.notebookService.hasSupportedNotebooks(request.uri)
		) {
			this.logService.error(
				`srequest | refuse | SD | refusing notebook from Panel | [codeMapper]`,
			);
			return new CodeMapperRefusal();
		}

		const combinedDocumentLength = isNewDocument(request)
			? request.workingSet.reduce(
					(prev, curr) => prev + curr.getText().length,
					0,
				)
			: request.existingDocument.getText().length;
		...
		if (combinedDocumentLength > promptLimit) {
			this.logService.error(
				`srequest | refuse | SD | refusing huge document | [codeMapper]`,
			);
			return new CodeMapperRefusal();
		}

		const builtPrompt = await this.buildPrompt(request, token);
		const { promptTokenCount, speculation, requestId, endpoint } =
			builtPrompt;
		...
		if (promptTokenCount > 64_000) {
			this.logService.error(
				`srequest | refuse | SD | exceeds token limit | [codeMapper]`,
			);
			return new CodeMapperRefusal();
		}
		...
		const res = await this.fetchNativePredictedOutputs(
			request,
			builtPrompt,
			resultStream,
			outcomeCorrelationTelemetry,
			token,
			true,
		);
		...
		try {
			...
			const response = {
				responseText: allResponseText.join(''),
				startTime,
				firstTokenTime,
				requestId,
			};
			await this.logDoneInfo(
				request,
				builtPrompt,
				response,
				outcomeCorrelationTelemetry,
			...
			if (telemetryInfo?.chatRequestId) {
				...
				this.editLogService.logSpeculationRequest(
					telemetryInfo.chatRequestId,
					request.uri,
					prompt,
					speculation,
					response.responseText,
				);
				this.multiFileEditInternalTelemetryService.storeEditPrompt(
					{
						prompt,
						uri: request.uri,
						isAgent: telemetryInfo.isAgent,
						document: request.existingDocument?.document,
					},
					{
						chatRequestId: telemetryInfo.chatRequestId,
						chatSessionId: telemetryInfo.chatSessionId,
						speculationRequestId: requestId,
						mapper,
					},
				);
			...
			if (err instanceof CompletionsFetchError) {
				...
				this.logError(
					request,
					builtPrompt,
					{ startTime, firstTokenTime, requestId },
					outcomeCorrelationTelemetry,
					mapper,
					err.type,
				);
			...
			errorDetails = errorDetails ?? {
				message: l10n.t(
					`Sorry, your request failed. Please try again. Request id: {0}`,
					requestId,
				),
			};
			return {
				errorDetails,
				annotations,
				telemetry: outcomeCorrelationTelemetry,
			};
		}
	}

	private async sendModelResponseInternalAndEnhancedTelemetry(
		useGPT4oProxy: boolean,
		builtPrompt: IFullRewritePrompt,
		result: ISuccessfulRewriteInfo,
		outcomeTelemetry: CodeMapperOutcomeTelemetry,
		mapper: string,
	) {
		const payload = {
	...
	private async fetchNativePredictedOutputs(
		request: ICodeMapperRequestInput,
		builtPrompt: IFullRewritePrompt,
		resultStream: MappedEditsResponseStream,
		outcomeTelemetry: CodeMapperOutcomeTelemetry,
		token: CancellationToken,
		applyEdits: boolean,
	): Promise<CodeMapperOutcome | ISuccessfulRewriteInfo> {
		const { messages, speculation, requestId, endpoint } = builtPrompt;
		const startTime = Date.now();

		const fetchResult = await this.fetchAndContinueOnLengthError(
			endpoint,
			messages,
			speculation,
			request,
		...
		if (fetchResult.result.type !== ChatFetchResponseType.Success) {
			this.logError(
				...
				{
					startTime,
					firstTokenTime: fetchResult.firstTokenTime,
					requestId,
				},
				outcomeTelemetry,
				builtPrompt.endpoint.model,
				fetchResult.result.type,
			);
			return {
		...
		const res = {
			allResponseText: fetchResult.allResponseText,
			firstTokenTime: fetchResult.firstTokenTime,
			startTime,
		...
		this.sendModelResponseInternalAndEnhancedTelemetry(
			true,
			builtPrompt,
			res,
			outcomeTelemetry,
	...
	private async fetchAndContinueOnLengthError(
		...
		token: CancellationToken,
		applyEdits: boolean,
	): Promise<ISpeculationFetchResult> {
		const allResponseText: string[] = [];
		let responseLength = 0;
		let firstTokenTime: number = -1;

		const existingDocument = request.existingDocument;
		const documentLength = existingDocument
			? existingDocument.getText().length
			: 0;
		const uri = request.uri;
		...
		const fetchStreamSource = new FetchStreamSource();
		const textStream = fetchStreamSource.stream.map(
			(part) => part.delta.text,
		);
		...
		if (applyEdits) {
			processPromise =
				existingDocument instanceof NotebookDocumentSnapshot
					? processFullRewriteNotebook(
							existingDocument.document,
							readLineByLine(textStream, token),
							resultStream,
							this.alternativeNotebookEditGenerator,
							{
								source: NotebookEditGenrationSource.codeMapperFastApply,
								model: endpoint.model,
								requestId: undefined,
							},
							token,
						) // corrected parameter passing
					: processFullRewriteStream(
							uri,
							existingDocument,
							readLineByLine(textStream, token),
							resultStream,
		...
		while (true) {
			...
			if (result.type === ChatFetchResponseType.Length) {
				if (responseLength > maxLength) {
					...
					this.logCodemapperLoopTelemetry(
						request,
						result,
						uri,
						endpoint.model,
					...
					return {
						...
						annotations: [
							{
								label: 'codemapper loop',
								message: `Code mapper might be in a loop: Rewritten length: ${responseLength}, Document length: ${documentLength}, Code block length ${request.codeBlock.length}`,
								severity: 'error',
							},
						],
					};
				...
				const promptRenderer = PromptRenderer.create(
					...
					CodeMapperFullRewritePrompt,
					{
						request,
						shouldTrimCodeBlocks: true,
						inProgressRewriteContent: result.truncatedValue,
					} satisfies CodeMapperPromptProps,
				);
				...
				if (
					applyEdits &&
					(!existingDocument ||
						existingDocument instanceof TextDocumentSnapshot)
				) {
					await handleTrailingLines(
						uri,
						existingDocument,
	...
	private logCodemapperLoopTelemetry(
		request: ICodeMapperRequestInput,
		result: ChatResponse,
		uri: Uri,
		model: string,
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent(
			'speculation.response.loop',
			{
				...
				languageId: isNewDocument(request)
					? getLanguageForResource(uri).languageId
					: request.existingDocument.languageId,
				model,
	...
	private ensureFinishReasonStopOrThrow(
		requestId: string,
		...
		switch (finishReason) {
			...
			case Completion.FinishReason.ContentFilter:
				throw new CompletionsFetchError(
					'stop_content_filter',
					requestId,
					'Content filter',
				);
...
function getTrailingDocumentEmptyLineCount(
...
export function getTrailingArrayEmptyLineCount(
...
async function computeAdditionsAndDeletions(
