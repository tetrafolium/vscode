...
import {
	...
	ICodeMapperDocument,
	ICodeMapperRequestInput,
	processFullRewriteNewNotebook,
} from './codeMapper';
...
export type ResourceTextEdits = {
	readonly target: vscode.Uri;
	readonly edits: TextEdit | TextEdit[];
};
...
export const ICodeMapperService =
	createServiceIdentifier<ICodeMapperService>('ICodeMapperService');
...
export interface ICodeMapperService {
	...
	mapCode(
		...
		responseStream: vscode.MappedEditsResponseStream,
		telemetryInfo: ICodeMapperTelemetryInfo | undefined,
		token: vscode.CancellationToken,
	): Promise<IMapCodeResult | undefined>;
}

export class CodeMapperService
	extends Disposable
	implements ICodeMapperService
{
	readonly _serviceBrand: undefined;

	private readonly _queues: ResourceMap<Queue<IMapCodeResult | undefined>> =
		new ResourceMap();
	...
	async mapCode(
		...
		return queue.queue(() =>
			this._doMapCode(request, responseStream, telemetryInfo, token),
		);
	}

	private async _doMapCode(
		request: IMapCodeRequest,
		responseStream: vscode.MappedEditsResponseStream,
		telemetryInfo: ICodeMapperTelemetryInfo | undefined,
		token: vscode.CancellationToken,
	): Promise<IMapCodeResult | undefined> {
		const codeMapper = this.notebookService.hasSupportedNotebooks(
			request.codeBlock.resource,
		)
			? this.instantiationService.createInstance(NotebookCodeMapper)
			: this.instantiationService.createInstance(DocumentCodeMapper);

		return codeMapper.mapCode(
			request,
			responseStream,
			telemetryInfo,
			token,
...
class DocumentCodeMapper extends Disposable implements ICodeMapperService {
	...
	async mapCode(
		...
		const documentContext =
			await this._getDocumentContextForCodeBlock(codeBlock);
		if (token.isCancellationRequested) {
			return undefined;
		}

		if (
			(!documentContext || documentContext.getText().length === 0) &&
			!codeBlock.code.includes(EXISTING_CODE_MARKER)
		) {
			...
			responseStream.textEdit(
				codeBlock.resource,
				new TextEdit(new Range(0, 0, 0, 0), codeBlock.code),
			);
			/* __GDPR__
		...
		if (documentContext) {
			...
			responseStream = spyResponseStream(
				responseStream,
				(_target, edits) => {
					tracker.collectAIEdits(edits);
				},
			);
		}
		...
		if (telemetry) {
			editSurvivalTracker?.startReporter((res) =>
				reportEditSurvivalEvent(res, telemetry, this._otelService),
			);
		}
		return result;
	}

	private async _getDocumentContextForCodeBlock(
		codeblock: CodeBlock,
	): Promise<TextDocumentSnapshot | undefined> {
		try {
			const existingDoc = this._workspaceService.textDocuments.find(
				(doc) => isEqual(doc.uri, codeblock.resource),
			);
			if (existingDoc) {
				return TextDocumentSnapshot.create(existingDoc);
			}

			const existsOnDisk = await this._fileSystemService
				.stat(codeblock.resource)
				.then(
					() => true,
					() => false,
				);
			...
			return await this._workspaceService.openTextDocumentAndSnapshot(
				codeblock.resource,
			);
		} catch (ex) {
			// ignore, probably an invalid URI or the like.
			console.error(
				`Failed to get document context for ${codeblock.resource.toString()}`,
				ex,
			);
			return undefined;
		}
...
class NotebookCodeMapper extends Disposable implements ICodeMapperService {
		...
		if (
			...
			await processFullRewriteNewNotebook(
				...
				this.alternativeNotebookEditGenerator,
				{
					...
					model: telemetryInfo?.chatRequestModel,
					requestId: telemetryInfo?.chatRequestId,
				},
				token,
			);
			/* __GDPR__
	...
	private async _getDocumentContextForCodeBlock(
		codeblock: CodeBlock,
	): Promise<ICodeMapperDocument | undefined> {
		try {
			const format = inferAlternativeNotebookContentFormat(
				codeblock.code,
			);
			...
			if (notebookDocument) {
				return NotebookDocumentSnapshot.create(
					notebookDocument,
					format,
				);
			}
			...
			return await this._workspaceService.openNotebookDocumentAndSnapshot(
				codeblock.resource,
				format,
			);
		} catch (ex) {
...
async function mapCode(
	request: IMapCodeRequest,
	responseStream: vscode.MappedEditsResponseStream,
	documentContext: ICodeMapperDocument | undefined,
	...
	const requestInput: ICodeMapperRequestInput =
		documentContext && documentContext.getText().length > 0
			...
			: {
					...
					workingSet:
						workingSet?.map((entry) => entry.document) || [],
				};
...
function reportTelemetry(
	telemetryService: ITelemetryService,
	{ telemetry, annotations }: IMapCodeResult,
) {
	if (!telemetry) {
		return; // cancelled
	}
...
function spyResponseStream(
	responseStream: vscode.MappedEditsResponseStream,
	callback: (target: vscode.Uri, edits: TextEdit | TextEdit[]) => void,
): vscode.MappedEditsResponseStream {
	return {
...
function reportEditSurvivalEvent(
	...
	*/
	...
	res.telemetryService.sendEnhancedGHTelemetryEvent(
