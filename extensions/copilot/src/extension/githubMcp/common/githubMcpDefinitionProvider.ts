...
export class GitHubMcpDefinitionProvider implements McpServerDefinitionProvider<McpHttpServerDefinition> {
	...
	constructor(
		...
		const configurationEvent = Event.chain(
				...
				$.filter((e) => {
					...
					if (
						e.affectsConfiguration(
							ConfigKey.GitHubMcpToolsets.fullyQualifiedId,
						)
					) {
						logService.debug(
							'GitHubMcpDefinitionProvider: Configuration change affects GitHub MCP toolsets.',
						);
						return true;
					}
					...
					if (
						e.affectsConfiguration(
							ConfigKey.GitHubMcpReadonly.fullyQualifiedId,
						)
					) {
						logService.debug(
							'GitHubMcpDefinitionProvider: Configuration change affects GitHub MCP readonly mode.',
						);
						return true;
					}
					...
					if (
						e.affectsConfiguration(
							ConfigKey.GitHubMcpLockdown.fullyQualifiedId,
						)
					) {
						logService.debug(
							'GitHubMcpDefinitionProvider: Configuration change affects GitHub MCP lockdown mode.',
						);
						return true;
					}
					...
					if (
						e.affectsConfiguration(
							ConfigKey.GitHubMcpChannel.fullyQualifiedId,
						)
					) {
						logService.debug(
							'GitHubMcpDefinitionProvider: Configuration change affects GitHub MCP channel.',
						);
						return true;
					}
					...
					if (
						e.affectsConfiguration(
							ConfigKey.Shared.AuthProvider.fullyQualifiedId,
						)
					) {
						logService.debug(
							'GitHubMcpDefinitionProvider: Configuration change affects GitHub auth provider.',
						);
						return true;
					}
					...
					if (e.affectsConfiguration(EnterpriseURLConfig)) {
						logService.debug(
							'GitHubMcpDefinitionProvider: Configuration change affects GitHub Enterprise URL.',
						);
						return true;
					}
					return false;
		...
		let havePermissiveToken =
			!!this.authenticationService.permissiveGitHubSession;
		const authEvent = Event.chain(
			...
			($) =>
				$.filter(() => {
					...
					this.logService.debug(
						`GitHubMcpDefinitionProvider: Permissive GitHub session availability changed: ${havePermissiveToken}`,
					);
				}),
		);
		this.onDidChangeMcpServerDefinitions = Event.any(
			configurationEvent,
			authEvent,
		);
	}
	...
	private get toolsets(): string[] {
		return this.configurationService.getConfig<string[]>(
			ConfigKey.GitHubMcpToolsets,
		);
	}

	private get readonly(): boolean {
		return this.configurationService.getConfig<boolean>(
	...
	private get channel(): ConfigKey.GitHubMcpChannelValue {
		return this.configurationService.getConfig<ConfigKey.GitHubMcpChannelValue>(
			ConfigKey.GitHubMcpChannel,
		);
	}

	private get gheConfig(): string | undefined {
		return this.configurationService.getNonExtensionConfig<string>(
			EnterpriseURLConfig,
		);
	}
	...
	provideMcpServerDefinitions(): McpHttpServerDefinition[] {
		...
		const basics =
			providerId === AuthProviderId.GitHubEnterprise
	...
	async resolveMcpServerDefinition(
	...
	): Promise<McpHttpServerDefinition> {
		const accessToken =
			this.authenticationService.permissiveGitHubSession?.accessToken;
		if (accessToken) {
		...
		if (this._askedForAuth) {
			throw new Error(
				'User denied authentication. Cannot connect to GitHub MCP Server.',
			);
		}

		try {
			const session = await this.authenticationService.getGitHubSession(
				'permissive',
				{
					createIfNone: {
						detail: l10n.t(
							'Additional permissions are required to use GitHub MCP Server',
						),
					},
				},
			);
