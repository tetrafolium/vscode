...
export class GitHubMcpContrib extends Disposable {
			...
			this.configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(
						ConfigKey.GitHubMcpEnabled.fullyQualifiedId,
					)
				) {
					if (this.enabled) {
						void this._registerGitHubMcpDefinitionProvider();
						...
						this.logService.trace(
							'Unregistering GitHub MCP Definition Provider.',
						);
	...
	private async _registerGitHubMcpDefinitionProvider() {
		if (!this.definitionProvider) {
			this.logService.trace(
				'Registering GitHub MCP Definition Provider.',
			);
			// Register the GitHub MCP Definition Provider
			this.definitionProvider = new GitHubMcpDefinitionProvider(
				this.configurationService,
				this.authenticationService,
				this.logService,
			);
			this.disposable = lm.registerMcpServerDefinitionProvider(
				'github',
