...
export class ConfigurationMigrationContribution implements IExtensionContribution {
		...
		this._register(
			workspace.onDidChangeWorkspaceFolders(async (e) => {
				for (const folder of e.added) {
					await this.migrateConfigurationForFolder(
						folder,
	...
	private async migrateConfigurationForFolder(
		folder: WorkspaceFolder | undefined,
		migrations: ConfigurationMigration[],
	): Promise<void> {
		await Promise.all([
			migrations.map((migration) =>
				this.migrateConfigurationsForFolder(migration, folder?.uri),
			),
		]);
	}
		...
		for (const [inspectValue, target] of targetPairs) {
			...
			if (migrationValues.length) {
				...
				await Promise.allSettled(
					migrationValues.map(async ([key, value]) => {
...
const oldCursorJumpKey =
	'github.copilot.chat.advanced.inlineEdits.nextCursorPrediction.enabled';
...
ConfigurationMigrationRegistry.registerConfigurationMigrations([
	{
		...
		migrateFn: async (
			value:
				| boolean
				| /* the rest is for backward compat: */ NextCursorLinePrediction
				| 'labelOnlyWithEdit'
				| boolean
				| undefined,
		) => {
			if (typeof value === 'string') {
				// for backward compatibility -- one of 'onlyWithEdit' | 'jump' | 'labelOnlyWithEdit'
				value = true;
