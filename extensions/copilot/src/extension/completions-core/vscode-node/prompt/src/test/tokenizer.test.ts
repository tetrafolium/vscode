...
const source = fs
	.readFileSync(resolve(__dirname, 'testdata/example.py'), 'utf8')
	.replace(/\r\n?/g, '\n');
...
suite('Tokenizer Test Suite - cl100k', function () {
	...
	test('emojis', function () {
		...
		assert.deepStrictEqual(
			tokenizer.tokenize(str),
			[15339, 62904, 233, 1917, 11410, 234, 235],
		);
		assert.strictEqual(tokenizer.detokenize(tokenizer.tokenize(str)), str);
	});
	...
	test('tokenizeStrings', function () {
		...
		assert.strictEqual(
			tokens_s.join(''),
			source,
			'tokenizeStrings does not join to form the input string',
		);
		const tokens = tokenizer.tokenize(source);
		assert.strictEqual(
			tokens_s.length,
			tokens.length,
			'tokenizeStrings should have same length as tokenize',
		);
	...
	test('takeLastTokens invariant of starting position', function () {
		...
		assert.strictEqual(
			suffix.text,
			tokenizer.takeLastTokens(source.substring(50), 25).text,
		);
		assert.strictEqual(
			suffix.text,
			tokenizer.takeLastTokens(source.substring(100), 25).text,
		);
		assert.strictEqual(
			suffix.text,
			tokenizer.takeLastTokens(source.substring(150), 25).text,
		);
		assert.strictEqual(
			suffix.text,
			tokenizer.takeLastTokens(source.substring(200), 25).text,
		);
	});

	test('takeLastTokens returns the desired number of tokens', function () {
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 30).tokens.length,
			30,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 29).tokens.length,
			29,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 28).tokens.length,
			28,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 5).tokens.length,
			5,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 0).tokens.length,
			0,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 1).tokens.length,
			1,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 1000).tokens.length,
			1000,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens(source, 100000).text,
			source,
		);
		assert.strictEqual(
			tokenizer.takeLastTokens('\n\n\n', 1).tokens.length,
			1,
		);
	});
	...
	test('takeLastTokens returns a suffix of the sought length', function () {
		function check(n: number): void {
			...
			assert.strictEqual(
				suffix,
				source.substring(source.length - suffix.length),
			);
		}
		check(0);
		check(1);
	...
	test('test takeLastLinesTokens', function () {
		...
		assert.strictEqual(
			tokenizer.takeLastLinesTokens(example, 7),
			'd e f\ng h i',
		);
		...
		assert.strictEqual(
			tokenizer.takeLastLinesTokens(example, 5),
			'a b\n\n c d',
		);
	});
...
with minor edits to make it`;
		...
		assert.strictEqual(
			prefix,
			tokenizer.takeFirstTokens(source.substring(0, 150), 29).text,
		);
		assert.strictEqual(
			prefix,
			tokenizer.takeFirstTokens(source.substring(0, 200), 29).text,
		);
	});

	test('takeFirstTokens returns the desired number of tokens', function () {
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens(source, 30).text),
			30,
		);
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens(source, 29).text),
			29,
		);
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens(source, 28).text),
			28,
		);
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens(source, 5).text),
			5,
		);
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens(source, 0).text),
			0,
		);
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens(source, 1).text),
			1,
		);
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens(source, 1000).text),
			1000,
		);
		assert.strictEqual(
			tokenizer.takeFirstTokens(source, 100000).text,
			source,
		);
		assert.strictEqual(
			tokenizer.tokenLength(tokenizer.takeFirstTokens('\n\n\n', 1).text),
			1,
		);
	});
...
suite('Tokenizer Test Suite - o200k', function () {
	...
	test('emojis', function () {
		...
		assert.deepStrictEqual(
			tokenizer.tokenize(str),
			[24912, 61138, 233, 2375, 130321, 235],
		);
		assert.strictEqual(tokenizer.detokenize(tokenizer.tokenize(str)), str);
	});
...
suite('ApproximateTokenizer', function () {
	const cl100kTokenizer = new ApproximateTokenizer(
		TokenizerName.cl100k,
		'python',
	);
	const o200kTokenizer = new ApproximateTokenizer(
		TokenizerName.o200k,
		'python',
	);
	...
	test('takeFirstTokens', function () {
		...
		assert.deepStrictEqual(
			cl100kTokenizer.tokenLength(first2Tokens.text),
			2,
		);
	});

	test('takeFirstTokens returns the full string if shorter', function () {
		const first100Tokens = cl100kTokenizer.takeFirstTokens(
			'123 456 7890',
			100,
		);
		...
		assert.deepStrictEqual(
			cl100kTokenizer.tokenLength(first100Tokens.text),
			4,
		);
	});
	...
	test('takeLastTokens', function () {
		...
		assert.deepStrictEqual(
			cl100kTokenizer.tokenLength(last2Tokens.text),
			2,
		);
	});

	test('takeLastTokens returns the full string if shorter', function () {
		const last100Tokens = cl100kTokenizer.takeLastTokens(
			'123 456 7890',
			100,
		);
		...
		assert.deepStrictEqual(
			cl100kTokenizer.tokenLength(last100Tokens.text),
			4,
		);
	});
