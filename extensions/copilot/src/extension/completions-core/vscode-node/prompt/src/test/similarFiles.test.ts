...
import {
	...
	computeScore,
} from '../snippetInclusion/jaccardMatching';
import {
	ScoredSnippetMarker,
	SortOptions,
	splitIntoWords,
} from '../snippetInclusion/selectRelevance';
import {
	SimilarFilesOptions,
...
async function retrieveAllSnippetsWithJaccardScore(
	...
	const matcher = FixedWindowSizeJaccardMatcher.FACTORY(windowLength).to(
		referenceDocWithOffset,
	);
...
async function findBestJaccardMatch(
	...
	const match = await matcher.findBestMatch(
		objectDoc,
...
suite('selectRelevance Test Suite', function () {
	...
	test('Tokenization keeps numbers attached to words', function () {
		assert.deepStrictEqual(
			splitIntoWords('def hello1:\n\treturn world49'),
			['def', 'hello1', 'return', 'world49'],
		);
	});

	test('Tokenization splits words on special characters', function () {
		assert.deepStrictEqual(
			splitIntoWords('def hello(world):\n\treturn a.b+1'),
			['def', 'hello', 'world', 'return', 'a', 'b', '1'],
		);
	});

	test('Tokenization splits words on underscores', function () {
		assert.deepStrictEqual(
			splitIntoWords(`def hello_world:\n\treturn 'I_am_a_sentence!'`),
			['def', 'hello', 'world', 'return', 'I', 'am', 'a', 'sentence'],
		);
	});
	...
	test('Find all snippets.', async function () {
		...
		assert.deepStrictEqual(
			await retrieveAllSnippetsWithJaccardScore(
				doc1,
				refDoc,
				windowLength,
				SortOptions.None,
	...
	test('Test Jaccard similarity.', function () {
		...
		assert.strictEqual(
			computeScore(
				new Set(splitIntoWords(bagOfWords1)),
				new Set(splitIntoWords(bagOfWords2)),
			),
			0,
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			computeScore(
				new Set(splitIntoWords(bagOfWords1)),
				new Set(splitIntoWords(bagOfWords3)),
			),
			5 / 6,
		);
		assert.strictEqual(
			computeScore(
				new Set(splitIntoWords(bagOfWords1)),
				new Set(splitIntoWords(bagOfWords4)),
			),
			1 / 9,
		);
		assert.strictEqual(
			computeScore(
				new Set(splitIntoWords(bagOfWords1)),
				new Set(splitIntoWords(bagOfWords5)),
			),
			1 / 9,
		);
	});
...
suite('Test getSimilarSnippets function', function () {
	...
	test('Returns correct snippet in conservative mode', async function () {
		...
		const snippetLocations = (
			await getSimilarSnippets(doc, similarFiles, options)
	...
	test('FixedWindowSizeJaccardMatcher trims reference document correctly', async function () {
		for (let windowLength = 1; windowLength < 7; windowLength++) {
			const matcherFactory =
				FixedWindowSizeJaccardMatcher.FACTORY(windowLength);
			...
			const correctReferenceTokens: string[] = [
				'1',
				'2',
				'3',
				'4',
