 ...
 */
export function clearLabels<L>(
	tree: IndentationTree<L>,
): IndentationTree<never> {
	visitTree(
		tree,
		(tree: IndentationTree<L>) => {
...
export type Rebuilder<L> = (
	tree: IndentationTree<L>,
) => IndentationTree<L> | undefined;
/**
...
export function rebuildTree<L>(
	...
	const rebuild: Rebuilder<L> = (tree: IndentationTree<L>) => {
		if (skip !== undefined && skip(tree)) {
			...
			const newSubs = tree.subs
				.map(rebuild)
				.filter((sub) => sub !== undefined) as IndentationSubTree<L>[];
			tree.subs = newSubs;
			return visitor(tree);
		}
	};
