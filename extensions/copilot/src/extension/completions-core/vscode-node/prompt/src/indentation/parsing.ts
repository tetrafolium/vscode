...
export function parseRaw(source: string): IndentationTree<never> {
	...
	function parseNode(line: number): [LineNode<never>, number] {
		...
		const node: LineNode<never> = lineNode(
			indentations[line],
			line,
			lines[line],
			subs,
	...
	function parseSubs(
	...
	): [IndentationSubTree<never>[], number] {
		...
		while (
			line < lines.length &&
			(lines[line] === '' || indentations[line] > parentIndentation)
		) {
			if (lines[line] === '') {
	...
	if (line < lines.length) {
		throw new Error(
			`Parsing did not go to end of file. Ended at ${line} out of ${lines.length}`,
		);
	}
	return topNode(subs);
}
...
export function labelLines<L>(
	tree: IndentationTree<L>,
	labelRules: LabelRule<L>[],
): void {
	function visitor(tree: IndentationTree<L>): void {
		if (isLine(tree)) {
			const rule = labelRules.find((rule) =>
				rule.matches(tree.sourceLine),
			);
			if (rule) {
...
export function buildLabelRules<
	L extends { [key: string]: RegExp | LineMatcher },
...
export function combineClosersAndOpeners<L>(
	...
	const rebuilder: Rebuilder<S> = function (tree: IndentationTree<S>) {
		if (
			tree.subs.length === 0 ||
			tree.subs.findIndex(
				(sub) => sub.label === 'closer' || sub.label === 'opener',
			) === -1
		) {
			return tree;
		}
		...
		for (let i = 0; i < tree.subs.length; i++) {
			...
			if (
				sub.label === 'opener' &&
				directOlderSibling !== undefined &&
				isLine(directOlderSibling)
			) {
			...
			// 2. if a closer following an older sibling
				...
				if (sub.subs.length > 0) {
					const firstNonVirtual = lastNew.subs.findIndex(
						(sub) => sub.label !== 'newVirtual',
					);
	...
	clearLabelsIf<S, 'newVirtual'>(
		tree,
		(arg: S): arg is 'newVirtual' => arg === 'newVirtual',
	);
...
export function groupBlocks<L>(
	...
	const rebuilder: Rebuilder<L> = function (tree: IndentationTree<L>) {
		...
		function flushBlockIntoNewSubs(
			...
			if (
				currentBlockIndentation !== undefined &&
				(newSubs.length > 0 || !final)
			) {
		...
		for (let i = 0; i < tree.subs.length; i++) {
			...
			if (!isBlank(sub)) {
				currentBlockIndentation =
					currentBlockIndentation ?? sub.indentation;
			}
		}
 ...
 */
export function flattenVirtual<L>(
	tree: IndentationTree<L>,
): IndentationTree<L> {
	const rebuilder: Rebuilder<L> = function (tree) {
		if (
			...
			tree.subs.length === 1 &&
			isVirtual(tree.subs[0]) &&
			tree.subs[0].label === undefined
		) {
...
const genericLabelRules: LabelRule<'opener' | 'closer'>[] =
	buildLabelRules(_genericLabelRules);

const LANGUAGE_SPECIFIC_PARSERS: {
	[key: string]: (raw: IndentationTree<never>) => IndentationTree<string>;
} = {};
/**
