...
import {
	...
	CMDDisableCompletionsClient,
	CMDEnableCompletionsChat,
	CMDEnableCompletionsClient,
	CMDOpenDocumentationClient,
	CMDOpenLogsClient,
	CMDOpenModelPickerChat,
	CMDOpenModelPickerClient,
	CMDToggleCompletionsChat,
	CMDToggleCompletionsClient,
	CMDToggleStatusMenuChat,
	CMDToggleStatusMenuClient,
...
} from './extension/src/extensionStatus';
...
import {
	...
	UserErrorNotifier,
} from './lib/src/error/userErrorNotifier';
...
} from './lib/src/ghostText/configBlockMode';
import {
	CurrentGhostText,
	ICompletionsCurrentGhostText,
} from './lib/src/ghostText/current';
import {
...
} from './lib/src/ghostText/speculativeRequestCache';
...
} from './lib/src/prompt/components/contextProviderBridge';
import {
...
} from './lib/src/prompt/contextProviderStatistics';
import {
	FullRecentEditsProvider,
	ICompletionsRecentEditsProviderService,
} from './lib/src/prompt/recentEdits/recentEditsProvider';
...
export function createContext(
	...
	serviceCollection.set(
		...
		new (class implements ICompletionsLogTargetService {
			...
			logIt(
				...
				switch (level) {
					case LogLevel.DEBUG:
						return logService.debug(msg);
					case LogLevel.INFO:
						return logService.info(msg);
					case LogLevel.WARN:
						return logService.warn(msg);
					case LogLevel.ERROR:
						return logService.error(msg);
				}
			}
		})(),
	);
	...
	serviceCollection.set(
		ICompletionsNotificationSender,
		new SyncDescriptor(ExtensionNotificationSender),
	);
	serviceCollection.set(
		ICompletionsEditorAndPluginInfo,
	...
	serviceCollection.set(
		...
		new SyncDescriptor(CompletionsObservableWorkspace),
	);
	serviceCollection.set(
	...
	serviceCollection.set(
		...
		new SyncDescriptor(LoggingCitationManager),
	);
	serviceCollection.set(
		ICompletionsContextProviderService,
	...
	serviceCollection.set(
		ICompletionsFetcherService,
		new SyncDescriptor(CompletionsFetcher),
	);
	serviceCollection.set(
		ICompletionsDefaultContextProviders,
	...
	return serviceAccessor
		...
		.createChild(serviceCollection, store);
}
...
export function setup(
	...
	disposables.add(
		serviceAccessor.get(ICompletionsCitationManager).register(),
	);
	...
	const defaultContextProviders = serviceAccessor.get(
		ICompletionsDefaultContextProviders,
	);
...
export function registerUnificationCommands(
...
function registerModelPickerCommands(accessor: ServicesAccessor): IDisposable {
	...
	const modelsPicker =
		instantiationService.createInstance(ModelPickerManager);
...
function registerStatusBar(accessor: ServicesAccessor): IDisposable {
	...
	const instantiationService = accessor.get(IInstantiationService);
	const copilotTokenManagerService = accessor.get(
		ICompletionsCopilotTokenManager,
	);
	...
	function registerStatusMenu(menuId: string): IDisposable {
		return registerCommandWrapper(accessor, menuId, async () => {
			...
			instantiationService
				.createInstance(CopilotStatusBarPickMenu)
				.showStatusMenu();
		});
	}
...
function registerDiagnosticCommands(accessor: ServicesAccessor): IDisposable {
	...
	disposables.add(
		registerCommandWrapper(accessor, CMDOpenDocumentationClient, () => {
	...
	disposables.add(
		registerCommandWrapper(accessor, CMDOpenLogsClient, () => {
			outputChannel.show();
		}),
	);
...
export function registerCommandWrapper(
	accessor: ServicesAccessor,
	command: string,
