...
export const ICompletionsTelemetryService =
	createServiceIdentifier<ICompletionsTelemetryService>(
...
export class CompletionsTelemetryServiceBridge implements ICompletionsTelemetryService {
	...
	sendGHTelemetryEvent(
		...
		this.telemetryService.sendGHTelemetryEvent(
			wrapEventNameForPrefixRemoval(`copilot/${eventName}`),
			properties,
			measurements,
		);
		this.getSpyReporters(
			store ?? TelemetryStore.Standard,
		)?.sendTelemetryEvent(
			eventName,
			properties as TelemetryProperties,
			measurements as TelemetryMeasurements,
		);
	}
	...
	sendGHTelemetryErrorEvent(
		...
		this.telemetryService.sendGHTelemetryErrorEvent(
			wrapEventNameForPrefixRemoval(`copilot/${eventName}`),
			properties,
			measurements,
		);
