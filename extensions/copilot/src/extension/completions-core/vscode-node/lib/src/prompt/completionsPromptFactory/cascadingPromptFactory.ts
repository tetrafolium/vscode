...
export abstract class CascadingPromptFactory implements ICompletionsPromptFactoryService {
	...
	constructor(
		...
		protected readonly featuresService: ICompletionsFeaturesService,
		@ICompletionsTelemetryService
		protected readonly completionsTelemetryService: ICompletionsTelemetryService,
		@ICompletionsContextProviderBridgeService
		protected readonly contextProviderBridge: ICompletionsContextProviderBridgeService,
		@ICompletionsLogTargetService
		protected readonly logTargetService: ICompletionsLogTargetService,
		@ICompletionsContextProviderService
		protected readonly contextProviderStatistics: ICompletionsContextProviderService,
	) {}
	...
	getComponentAllocation(
		...
		const stableContextPercent =
			this.featuresService.stableContextPercent(telemetryData);
		const volatileContextPercent =
			this.featuresService.volatileContextPercent(telemetryData);
		...
		if (stableContextPercent < 0 || stableContextPercent > 100) {
			throw new Error(
				`stableContextPercent must be between 0 and 100, but was ${stableContextPercent}`,
			);
		}

		if (volatileContextPercent < 0 || volatileContextPercent > 100) {
			throw new Error(
				`volatileContextPercent must be between 0 and 100, but was ${volatileContextPercent}`,
			);
		}

		const prefixPercent =
			100 - suffixPercent - stableContextPercent - volatileContextPercent;
		if (prefixPercent <= 1 || prefixPercent > 100) {
			throw new Error(
				`prefixPercent must be between 1 and 100, but was ${prefixPercent}`,
			);
		}
	...
	private async createPromptUnsafe(
		...
		this.renderId++;
		const { completionId, completionState, telemetryData, promptOpts } =
			opts;
		...
		if (
			...
			contextItems = await this.resolveContext(
				completionId,
				completionState,
				telemetryData,
				cancellationToken,
		...
		const renderedComponents: Partial<
			Record<PromptComponentId, RenderedComponent>
		> = {};
		...
		const { maxPromptLength } = this.instantiationService.invokeFunction(
			getPromptOptions,
			telemetryData,
			languageId,
		);
		...
		const estimatedMaxSuffixCost = this.components.suffix.estimatedCost?.(
			opts,
			contextItems,
		);
		let cascadeOrder: PromptComponentId[] = [
			'stableContext',
			'volatileContext',
			'prefix',
			'suffix',
		...
		if (
			suffixAllocation >
			SMALL_SUFFIX_THRESHOLD * (estimatedMaxSuffixCost ?? 0)
		) {
			cascadeOrder = [
				'stableContext',
				'volatileContext',
		...
		for (const id of cascadeOrder) {
			const componentBudget =
				surplusBudget + maxPromptLength * allocation[id];
		...
		const [prefix, trailingWs] = trimLastLine(
			renderedComponents.prefix!.text,
		);
	...
	private async resolveContext(
		...
		ValidatedContextItems & { resolvedContextItems: ResolvedContextItem[] }
	> {
		...
		const traits: TraitWithId[] = this.instantiationService.invokeFunction(
			getTraitsFromContextItems,
			completionId,
			matchedContextItems,
		);
	...
	private telemetrizeContext(
		...
		const promptMatcher =
			componentStatisticsToPromptMatcher(componentStatistics);
		this.contextProviderStatistics
			.getStatisticsForCompletion(completionId)
			.computeMatch(promptMatcher);
		const contextProvidersTelemetry = telemetrizeContextItems(
			this.contextProviderStatistics,
			completionId,
	...
	private async failFastPrompt(
		...
		if (
			textDocument.getText().length < MIN_PROMPT_CHARS &&
