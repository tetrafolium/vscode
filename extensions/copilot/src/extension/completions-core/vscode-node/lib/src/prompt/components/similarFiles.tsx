...
export const SimilarFiles = (
	...
	async function produceSimilarFiles(
		...
		const promptOptions = props.instantiationService.invokeFunction(
			getPromptOptions,
			telemetryData,
			doc.detectedLanguageId,
		);
		const similarSnippets = await findSimilarSnippets(
			promptOptions,
			telemetryData,
			doc,
			requestData,
	...
	async function findSimilarSnippets(
		...
		const similarFilesOptions =
			...
			props.instantiationService.invokeFunction(
		...
		return await getSimilarSnippets(
			docInfo,
			Array.from(files.docs.values()),
			similarFilesOptions,
		);
	}
...
const SimilarFile = (
	props: { snippet: SimilarFileSnippet },
	context: ComponentContext,
) => {
	return (
		<Chunk>
