...
for (const lineEnding of ['\n', '\r\n']) {
	...
	suite(
		`Split context prompt renderer (line ending: ${JSON.stringify(lineEnding)})`,
		function () {
			...
			test('renders prefix, context and suffix based on completions doc position', function () {
				...
				assert.deepStrictEqual(
					prompt.prefix,
					fullExpectedPrefixWithoutContext,
				);
			...
			test('elides context and suffix partially', function () {
				...
				assert.ok(
					prompt.context![0].length < fullExpectedContext[0].length,
					'Expected context to be elided',
				);
			...
			test('computes usage statistics after elision', function () {
				...
				assert.deepStrictEqual(
					metadata.componentStatistics.reduce(
						(acc, stats) => acc + (stats.actualTokens ?? 0),
						0,
					),
					33,
				);
			...
			test('throws error when tree does not contain completions document component', function () {
				...
				assert.strictEqual(
					...
					`Node of type ${BeforeCursor.name} not found`,
				);
			});
			...
			test('does not re-render if no data matching the expected structure is sent', async function () {
				...
				assert.deepStrictEqual(
					renderedPrompt.prefixTokens,
					renderedPromptTwo.prefixTokens,
				);
				assert.deepStrictEqual(
					renderedPrompt.suffixTokens,
					renderedPromptTwo.suffixTokens,
				);
			});
