...
suite('File Reader', function () {
	...
	setup(function () {
		...
		serviceCollection.define(
			...
			new FakeFileSystem({
				'/test.ts': FakeFileSystem.file('const foo', {
					ctime: 0,
					mtime: 0,
					size: 0.1 * 1024 * 1024,
				}), // .1MB
				'/empty.ts': '',
				'/large.ts': FakeFileSystem.file('very large file', {
					ctime: 0,
					mtime: 0,
					size: 1.1 * 1024 * 1024,
				}), // 1.1MB
	...
	test('reads file from text document manager', async function () {
		...
		const reader = accessor
			.get(IInstantiationService)
			.createInstance(FileReader);

		const docResult = await reader.getOrReadTextDocument({
			uri: 'file:///test.js',
		});
		...
		assert.deepStrictEqual(
			docResult.document?.detectedLanguageId,
			'javascript',
		);
	});
