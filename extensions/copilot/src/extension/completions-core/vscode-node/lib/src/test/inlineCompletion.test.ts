...
suite('getInlineCompletions()', function () {
	function setupCompletion(
		...
		function requestInlineCompletions(textDoc = doc, pos = position) {
			...
			return ghostText.getInlineCompletions(
				textDoc,
				pos,
	...
	test('Sends a speculative request when shown', async function () {
		...
		const networkResponse = Sinon.stub<
			[string, FetchOptions],
			Response
		>().returns(createFakeCompletionResponse('// not expected!'));
		networkResponse
			.onFirstCall()
			.returns(createFakeCompletionResponse(firstCompletionText));
		...
		const { accessor, doc, position, requestInlineCompletions } =
			setupCompletion(new StaticFetcher(networkResponse));

		const { reporter, result } = await withInMemoryTelemetry(
			accessor,
			async () => {
		...
		const expectedPrefix =
			doc.getText({ start: { line: 0, character: 0 }, end: position }) +
			firstCompletionText;
		assert.ok(
			result.prompt.endsWith(expectedPrefix),
			'Expect first completion in second request',
		);

		const issuedTelemetry = reporter.eventsMatching(
			(event) => event.name === 'ghostText.issued',
		);
		assert.strictEqual(
			issuedTelemetry.length,
			2,
			`Expected 2 issued events, got ${issuedTelemetry.length}`,
		);

		const speculativeTelemetry = reporter.eventsMatching(
			(event) =>
				event.name === 'ghostText.issued' &&
				event.properties['reason'] === 'speculative',
		);
		assert.ok(
			speculativeTelemetry.length === 1,
			'Expected one speculative request',
		);
	});
	...
	test('speculative requests apply completions the same as the editor and CLS', async function () {
		...
		const networkResponse = Sinon.stub<[], Response>().returns(
			createFakeCompletionResponse('// not expected!'),
		);
		networkResponse
			.onFirstCall()
			.returns(createFakeCompletionResponse(firstCompletion));
		...
		assert.deepStrictEqual(
			response[0].range,
			LocationFactory.range(LocationFactory.position(1, 0), position),
		);
		...
		assert.strictEqual(
			response2[0].insertText,
			firstCompletion + secondCompletion,
		);
		assert.deepStrictEqual(
			response2[0].range,
			LocationFactory.range(
				LocationFactory.position(1, 0),
				LocationFactory.position(1, firstCompletion.length),
			),
		);
	...
	test('does not send a speculative request if empty', async function () {
		...
		assert.strictEqual(
			issuedTelemetry.length,
			1,
			`Expected 1 issued events, got ${issuedTelemetry.length}`,
		);
		const speculativeTelemetry = reporter.eventsMatching(
			(event) =>
				event.name === 'ghostText.issued' &&
		...
		assert.ok(
			speculativeTelemetry.length === 0,
			'Expected no speculative request',
		);
	});
	...
	test('telemetryShown triggers speculative request only when shown', async function () {
		...
		const { reporter } = await withInMemoryTelemetry(accessor, async () => {
			...
			await delay(50);
			assert.strictEqual(
				networkResponse.callCount,
				1,
				'Expected only the initial network call',
			);
		...
		assert.strictEqual(
			networkResponse.callCount,
			2,
			'Expected 2 network calls (original + speculative)',
		);
		const shownTelemetry = reporter.eventsMatching(
			(event) => event.name === 'ghostText.shown',
		);
		assert.strictEqual(
			shownTelemetry.length,
			1,
			'Expected one shown telemetry event',
		);
		const speculativeTelemetry = reporter.eventsMatching(
			(event) =>
				event.name === 'ghostText.issued' &&
