...
suite('TextDocumentManager base class', () => {
	...
	test('should return the relative path of the document in workspace', () => {
		...
		const mockDocument = createTextDocument(
			makeFsUri('/path/to/workspace/folder/file.txt'),
			'',
			0,
			'',
		);
	...
	test('.getTextDocumentUnsafe() returns an existing document', function () {
		textDocumentManager.setTextDocument(
			'file:///path/to/file.txt',
			'plaintext',
			'file content',
		);

		const result = textDocumentManager.getTextDocumentUnsafe({
	...
	test('.getTextDocument() finds documents by normalized URI', async function () {
		const saved = textDocumentManager.setTextDocument(
			'file:///c%3A/file',
			'plaintext',
			'file content',
		);

		const retrieved = await textDocumentManager.getTextDocument({
			uri: 'file:///C:/file',
		});
	...
	test('.getTextDocument() returns undefined for anything other than an open document', async function () {
		const result = await textDocumentManager.getTextDocument({
			uri: 'file:///path/to/file.txt',
		});
	...
	test('.getTextDocument() retrieves the document synchronously', async function () {
		...
		const thenable = textDocumentManager.getTextDocument({
			uri: 'file:///path/to/file.txt',
		});
		textDocumentManager.updateTextDocument(
			'file:///path/to/file.txt',
			'new content',
		);
