...
suite('RenderNode', function () {
	...
	test('renders with exclude', function () {
		...
		assert.deepStrictEqual(
			render(node, { mask: node.id }).text,
			node.elisionMarker,
		);
		assert.deepStrictEqual(
			render(node, { mask: child.id }).text,
			`aa${child.elisionMarker}cc`,
		);
	});
	...
	test('canMerge merges adjacent elided children into a single elision marker', function () {
		...
		const child2Merge = createRenderNode({
			text: ['B'],
			cost: 1,
			weight: 1,
			canMerge: true,
		...
		assert.deepStrictEqual(
			render(nodeWithoutMerge, { mask: [child1.id, child2.id] }).text,
			'([...],[...],C)',
		);
		assert.deepStrictEqual(
			render(nodeWithMerge, { mask: [child1.id, child2Merge.id] }).text,
			'([...],C)',
		);
	});
	...
	test('renders with multiple children, one over budget', function () {
		...
		assert.deepStrictEqual(
			render(node, { budget: 8 }).text,
			`aabbcc${child2.elisionMarker}ee`,
		);
	});

	test('renders with custom costFunction', function () {
		// Use a custom elision marker since it's now counted in the cost
		const child1 = createRenderNode({
			text: ['bb'],
			cost: 2,
			weight: 2,
			elisionMarker: '.',
		});
		const child2 = createRenderNode({
			text: ['dd'],
			cost: 2,
			weight: 3,
			elisionMarker: '.',
		...
		assert.deepStrictEqual(
			render(node, { budget: 8, costFunction: (t) => t.length }).text,
			'aa.cc.ee',
		);
	});
	...
	test('redistributes weights (default weighter)', function () {
		...
		assert.ok(
			root.children.every(
				(child) => rectifiedValue(child) <= rectifiedValue(root),
			),
		);
	});
