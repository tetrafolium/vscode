...
for (const lineEnding of ['\n', '\r\n']) {
	...
	const position: Position = textDocument.positionAt(
		textDocument.getText().indexOf('|'),
	);
	suite(
		`Completions Prompt Renderer (line ending: ${JSON.stringify(lineEnding)})`,
		function () {
			...
			test('computes metadata with stable updateDataTimeMs tolerance', function () {
				...
				const expectedComponents = [
					...
					{
						componentPath:
							'$.f[0].CompletionsContext[0].AnotherComponent[0].Text[0]',
						expectedTokens: 8,
						actualTokens: 8,
					},
					{
						componentPath:
							'$.f[0].CompletionsContext[1].MyNestedComponent[0].f[0].Text[0]',
						expectedTokens: 5,
						actualTokens: 5,
					},
					{
						componentPath:
							'$.f[0].CompletionsContext[1].MyNestedComponent[0].f[1].Text[0]',
						expectedTokens: 5,
						actualTokens: 5,
					},
					{
						componentPath:
							'$.f[0].CompletionsContext[2].AnotherComponent[0].Text[0]',
						expectedTokens: 8,
						actualTokens: 8,
					},
					{
						componentPath:
							'$.f[0].CompletionsContext[3].f[0].Text[0]',
						expectedTokens: 4,
						actualTokens: 4,
					},
					{
						componentPath:
							'$.f[0].CompletionsContext[4].f[0].Text[0]',
						expectedTokens: 5,
						actualTokens: 5,
					},
					{
				...
				expectedComponents.forEach((expected) => {
					const actual = metadata1.componentStatistics.find(
						(s) => s.componentPath === expected.componentPath,
					);
					assert.ok(
						actual,
						`Component ${expected.componentPath} not found`,
					);
					assert.strictEqual(
			...
			test('propagates source via statistics', function () {
				...
				const renderedPrompt = renderer.render(
					snapshot!,
					renderingOptions,
				);
				assert.deepStrictEqual(renderedPrompt.status, 'ok');
				assert.ok(
					renderedPrompt.metadata.componentStatistics.find(
						(s) => s.source === trait,
					),
				);
				assert.ok(
			...
			test('elides both prefix and suffix partially', function () {
				...
				assert.ok(
					prompt.prefix.length < fullExpectedPrefix.length,
					'Expected prefix to be elided',
				);
				...
				if (fullExpectedSuffix.length > 0) {
					assert.ok(
						prompt.suffix.length < fullExpectedSuffix.length,
						'Expected suffix to be elided',
					);
				}
			});
			...
			test('computes usage statistics after elision', function () {
				...
				assert.deepStrictEqual(
					actualStatsFiltered.reduce(
						(acc, curr) => acc + (curr.actualTokens ?? 0),
						0,
					),
					34,
				);
			...
			function createStringWithNLines(
				n: number,
				baseText: string,
			): string {
				let result = '';
			...
			test('uses cached suffix if similar enough', async function () {
				const firstSuffix =
					createStringWithNLines(15, 'a') +
					createStringWithNLines(10, 'b');
				const secondSuffix =
					createStringWithNLines(15, 'a') +
					createStringWithNLines(10, 'c');
				...
				const position = textDocumentWithFirstSuffix.positionAt(
					textDocumentWithFirstSuffix.getText().indexOf('|'),
				);
				const prompt = (
					<>
						<CurrentFile />
				...
				const { snapshot: snapshotWithDefaultThreshold } =
					virtualPrompt.snapshot();
				...
				assert.deepStrictEqual(
					renderedWithDefaultThreshold.status,
					'ok',
				);
				assert.deepStrictEqual(
					renderedWithDefaultThreshold.suffix,
					firstSuffix,
				);
				...
				const { snapshot: snapshotWithLowerThreshold } =
					virtualPrompt.snapshot();

				// The second suffix is used, since the matching threshold is lower
				const renderedWithLowerThreshold = renderer.render(
					snapshotWithLowerThreshold!,
					renderOptionsWithSuffix,
				);
				assert.deepStrictEqual(renderedWithLowerThreshold.status, 'ok');
				assert.deepStrictEqual(
					renderedWithLowerThreshold.suffix,
					secondSuffix,
				);
			});
			...
			test('does not use cached suffix if not similar enough', async function () {
				const firstSuffix =
					...
					createStringWithNLines(10, 'b');
				const secondSuffix =
					createStringWithNLines(3, 'a') +
					createStringWithNLines(22, 'c');
				const renderOptionsWithSuffix: CompletionsPromptRenderOptions =
					{
						...renderingOptions,
						promptTokenLimit: 205,
			...
			test('does not use cached suffix when first token changes due to content shifting to prefix', async function () {
				...
				const firstPosition = firstDoc.positionAt(
					firstDoc.getText().indexOf('|'),
				);
				const prompt = (
					<>
						<CurrentFile />
				...
				const secondPosition = secondDoc.positionAt(
					secondDoc.getText().indexOf('|'),
				);
				await pipe.pump(
					createCompletionRequestData(
						accessor,
				...
				assert.ok(
					!rendered.suffix.startsWith('/**'),
					`Suffix should not start with "/**" but got: "${rendered.suffix.slice(0, 80)}..."`,
				);
				assert.ok(
					rendered.suffix.startsWith('* Last batch'),
					`Suffix should start with "* Last batch" but got: "${rendered.suffix.slice(0, 80)}..."`,
				);
			});

			test('suffix can be empty', async function () {
				const textDocumentWithoutSuffix = createTextDocument(
					fileUri,
					'typescript',
					0,
					'function f|',
				);
				const position = textDocumentWithoutSuffix.positionAt(
					textDocumentWithoutSuffix.getText().indexOf('|'),
				);
				const prompt = (
					<>
						<CurrentFile />
				...
				await pipe.pump(
					createCompletionRequestData(
						accessor,
						textDocumentWithoutSuffix,
						position,
					),
				);
				const { snapshot } = virtualPrompt.snapshot();
				const promptWithoutSuffix = renderer.render(
					snapshot!,
					renderingOptions,
				);
				...
				assert.deepStrictEqual(
					promptWithoutSuffix.prefix,
					'function f',
				);
			});

			test('prefix can be empty', async function () {
				const emptyTextDocument = createTextDocument(
					fileUri,
					'typescript',
					0,
					'|\nconst b = 2;',
				);
				const position = emptyTextDocument.positionAt(
					emptyTextDocument.getText().indexOf('|'),
				);
				const prompt = (
					<>
						<CurrentFile />
				...
				const { snapshot } = virtualPrompt.snapshot();
				const emptyPrompt = renderer.render(
					snapshot!,
					renderingOptions,
				);
			...
			test('throws error when tree does not contain completions document component', function () {
				...
				assert.strictEqual(
					...
					`Node of type ${CurrentFile.name} not found`,
				);
			});
			...
			test('does not re-render if no data matching the expected structure is sent', async function () {
				...
				const renderedPromptTwo = renderer.render(
					snapshotTwo!,
					renderingOptions,
				);
				...
				assert.deepStrictEqual(
					renderedPrompt.prefix,
					renderedPromptTwo.prefix,
				);
				assert.deepStrictEqual(renderedPrompt.suffix, 'function f');
				assert.deepStrictEqual(
					renderedPrompt.suffix,
					renderedPromptTwo.suffix,
				);
			});
			...
			test('re-renders if new data matching the expected structure is sent', async function () {
				...
				const updatedPosition = updatedTextDocument.positionAt(
					updatedTextDocument.getText().indexOf('|'),
				);
