...
export const ICompletionsUserErrorNotifierService =
	createServiceIdentifier<ICompletionsUserErrorNotifierService>(
...
export class UserErrorNotifier implements ICompletionsUserErrorNotifierService {
	...
	constructor(
		...
		private readonly _notificationSender: ICompletionsNotificationSender,
	...
	notifyUser(e: unknown) {
		...
		if (
			error.code &&
			CERTIFICATE_ERRORS.includes(error.code) &&
			!this.didNotifyBefore(error.code)
		) {
	...
	private async displayCertificateErrorNotification(
