...
export function fillInTsActiveExperiments(
	...
	try {
		const tsContextProviderParams =
			featuresService.tsContextProviderParams(telemetryData);
		if (tsContextProviderParams) {
			...
			const params =
				featuresService.getContextProviderExpSettings(
					'typescript',
				)?.params;
			if (params) {
				for (const [key, value] of Object.entries(params)) {
		...
		logger.debug(
			logTarget,
			`Failed to get the active TypeScript experiments for the Context Provider API`,
			e,
		);
		return false;
	}
