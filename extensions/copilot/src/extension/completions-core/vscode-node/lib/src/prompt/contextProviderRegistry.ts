...
export interface ResolvedContextItem<
	T extends SupportedContextItemWithId = SupportedContextItemWithId,
> {
	providerId: string;
	matchScore: number;
...
export const ICompletionsContextProviderRegistryService =
	createServiceIdentifier<ICompletionsContextProviderRegistryService>(
...
export type ActiveExperiments = Map<
	string,
	string | number | boolean | string[]
>;

export const ICompletionsDefaultContextProviders =
	createServiceIdentifier<ICompletionsDefaultContextProviders>(
...
export class CoreContextProviderRegistry implements ICompletionsContextProviderRegistryService {
	...
	constructor(
		...
		private runtimeMode: ICompletionsRuntimeModeService,
		@IInstantiationService
	...
	registerContextProvider<T extends SupportedContextItem>(
		_provider: ContextProvider<T>,
	) {
		throw new Error(
			`Should not be call. Use ILanguageContextProviderService`,
		);
	}
	...
	get providers(): ContextProvider<SupportedContextItem>[] {
		return this.registryService
			.getAllProviders([ProviderTarget.Completions])
			.slice() as ContextProvider<SupportedContextItem>[];
	}
	...
	async resolveAllProviders(
		...
		if (completionCancellationToken?.isCancellationRequested) {
			logger.debug(
				this.logTarget,
				`Resolving context providers cancelled`,
			);
			return [];
		}
		...
		const providersWithMatchScore = await this.matchProviders(
			_providers,
			documentContext,
			telemetryData,
		);
		const matchedProviders = providersWithMatchScore.filter(
			(p) => p[1] > 0,
		);
		const unmatchedProviders = providersWithMatchScore.filter(
			(p) => p[1] <= 0,
		);
		...
		const timeBudget =
			this.runtimeMode.isDebugEnabled() &&
			!this.runtimeMode.isRunningInSimulation()
				? 0
				: this.instantiationService.invokeFunction(
						getContextProviderTimeBudget,
						documentContext.languageId,
						telemetryData,
					);
		const timeoutEnd =
			timeBudget > 0 ? Date.now() + timeBudget : Number.MAX_SAFE_INTEGER;
		...
		const resolutionMap: Map<
			string,
			ResolveResult<SupportedContextItem>
		> = new Map();
		...
		for (const [provider] of matchedProviders) {
			...
			const pendingContextItem = provider.resolver.resolve(
				request,
				providerCancellationTokenSource.token,
			);
			...
			if (result) {
				if (result.status === 'error') {
					if (!isCancellationError(result.reason)) {
						logger.error(
							this.logTarget,
							`Error resolving context from ${provider.id}: `,
							result.reason,
						);
					}
					...
					const mergedItems: SupportedContextItem[] = [
						...(result.value ?? []),
					];
					if (
						result.status === 'none' ||
						...
						if (provider.resolver.resolveOnTimeout) {
							try {
								const fallbackItems =
									provider.resolver.resolveOnTimeout(request);
					...
					const [supportedItems, invalidItems] =
						filterSupportedContextItems(mergedItems);
					if (invalidItems) {
						logger.error(
							this.logTarget,
				...
				logger.error(
					this.logTarget,
					`Context provider ${provider.id} not found in results`,
				);
			}
		}
	...
	private async matchProviders(
		...
		const activeContextProviders = this.instantiationService.invokeFunction(
			getActiveContextProviders,
			documentContext.languageId,
			telemetryData,
		);
		const enableAllProviders =
			activeContextProviders.length === 1 &&
			activeContextProviders[0] === '*';

		const providersWithScore = await Promise.all(
			providers.map(async (provider) => {
				if (
					!enableAllProviders &&
					!activeContextProviders.includes(provider.id)
				) {
					return [provider, 0] as [
						ContextProvider<SupportedContextItem>,
						number,
					];
				}

				const matchScore = await this.match(
					this.instantiationService,
					provider.selector,
					documentContext,
				);
				return [provider, matchScore] as [
					ContextProvider<SupportedContextItem>,
					number,
				];
			}),
...
export class MutableContextProviderRegistry extends CoreContextProviderRegistry {
	...
	constructor(
		...
		@ICompletionsRuntimeModeService
		runtimeMode: ICompletionsRuntimeModeService,
	...
	override registerContextProvider<T extends SupportedContextItem>(
		provider: ContextProvider<T>,
	) {
		...
		if (this._providers.find((p) => p.id === provider.id)) {
			throw new Error(
				`A context provider with id ${provider.id} has already been registered`,
			);
		}
...
export class CachedContextProviderRegistry implements ICompletionsContextProviderRegistryService {
	...
	private _cachedContextItems: LRUCacheMap<string, ResolvedContextItem[]> =
		new LRUCacheMap(5);
...
export function telemetrizeContextItems(
	...
	const contextProviderStatistics =
		contextProvider.getStatisticsForCompletion(completionId);
	const contextProviderTelemetry: ContextProviderTelemetry[] =
		resolvedContextItems.map((p) => {
			const {
				providerId,
				resolution,
				resolutionTimeMs,
			...
			const providerStatistics =
				contextProviderStatistics.get(providerId);
				...
				providerStatistics?.usageDetails !== undefined
					? providerStatistics?.usageDetails.filter(
							(i) =>
								i.usage === 'full' ||
								i.usage === 'partial' ||
								i.usage === 'partial_content_excluded',
						).length
					: undefined;
			...
			if (numPartiallyUsedItems !== undefined) {
				contextProviderTelemetry.numPartiallyUsedItems =
...
export function matchContextItems(
	resolvedContextItem: ResolvedContextItem,
): boolean {
	return (
		resolvedContextItem.matchScore > 0 &&
		resolvedContextItem.resolution !== 'error'
	);
}

function getActiveContextProviders(
	accessor: ServicesAccessor,
	languageId: string,
	telemetryData: TelemetryWithExp,
): string[] {
	const expContextProviders = getExpContextProviders(
		accessor,
		languageId,
		telemetryData,
	);
	const configContextProviders: string[] =
		getConfig(accessor, ConfigKey.ContextProviders) ?? [];

	if (
		(expContextProviders.length === 1 && expContextProviders[0] === '*') ||
		(configContextProviders.length === 1 &&
			configContextProviders[0] === '*')
	) {
		return ['*'];
	}

	// Merge the two arrays and deduplicate
	const defaultContextProviders = accessor
		.get(ICompletionsDefaultContextProviders)
		.getIds();
	return Array.from(
		new Set([
 ...
 */
function getExpContextProviders(
	accessor: ServicesAccessor,
	languageId: string,
	telemetryData: TelemetryWithExp,
): string[] {
	...
	const langSpecific =
		featuresService.getContextProviderExpSettings(languageId);
...
export function useContextProviderAPI(
	accessor: ServicesAccessor,
	languageId: string,
	telemetryData: TelemetryWithExp,
) {
	return (
		getActiveContextProviders(accessor, languageId, telemetryData).length >
		0
	);
}

function getContextProviderTimeBudget(
	accessor: ServicesAccessor,
	languageId: string,
	telemetryData: TelemetryWithExp,
): number {
	const configTimeout = getConfig<number | undefined>(
		accessor,
		ConfigKey.ContextProviderTimeBudget,
	);
	...
	return accessor
		...
		.contextProviderTimeBudget(languageId, telemetryData);
}
...
export namespace DefaultDiagnosticSettings {
	...
	): DefaultDiagnosticSettings | undefined {
		if (!value) {
			return undefined;
		}
		try {
			const parsed = JSON.parse(
				value,
			) as Partial<DefaultDiagnosticSettings>;
			if (
				parsed.warnings === undefined &&
				parsed.maxDiagnostics === undefined &&
				parsed.maxLineDistance === undefined
			) {
				return undefined;
			}
			const warnings = getWarnings(parsed);
			const maxLineDistance =
				typeof parsed.maxLineDistance === 'number' &&
				parsed.maxLineDistance >= 0
					? parsed.maxLineDistance
					: 10;
			const maxDiagnostics =
				typeof parsed.maxDiagnostics === 'number' &&
				parsed.maxDiagnostics > 0
	...
	function getWarnings(
		value: Partial<DefaultDiagnosticSettings>,
	): 'no' | 'yes' | 'yesIfNoErrors' {
...
export function getDefaultDiagnosticSettings(
	accessor: ServicesAccessor,
): DefaultDiagnosticSettings | undefined {
	const configurationService = accessor.get(IConfigurationService);
	const experimentationService = accessor.get(IExperimentationService);
	const value = configurationService.getExperimentBasedConfig(
		ChatConfigKey.TeamInternal.InlineCompletionsDefaultDiagnosticsOptions,
