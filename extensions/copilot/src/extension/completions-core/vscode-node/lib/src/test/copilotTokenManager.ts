...
export class FakeCopilotTokenManager implements ICompletionsCopilotTokenManager {
	...
	constructor() {
		this._token = FakeCopilotTokenManager.createTestCopilotToken({
			token: 'tid=test;rt=1',
		});
	}
	...
	private static createTestCopilotToken(
		overrides?: Partial<Omit<ExtendedTokenInfo, 'expires_at'>>,
	): CopilotToken {
		const expires_at =
			Date.now() +
			((overrides?.refresh_in ?? 0) +
				FakeCopilotTokenManager.REFRESH_BUFFER_SECONDS) *
				1000;
		return new CopilotToken(
			createTestExtendedTokenInfo({ expires_at, ...overrides }),
		);
	}
}
