...
export const ICompletionsLastGhostText =
	createServiceIdentifier<ICompletionsLastGhostText>(
	...
	last.shownCompletions.forEach((c) => {
		if (c.displayText && c.telemetry) {
			...
			if (last.partiallyAcceptedLength) {
				...
				completionText = c.displayText.substring(
					last.partiallyAcceptedLength - 1,
				);
...
export function rejectLastShown(accessor: ServicesAccessor, offset?: number) {
	...
	if (rejectedCompletions.length > 0) {
		postRejectionTasks(
			accessor,
			'ghostText',
			offset ?? rejectedCompletions[0].offset,
			last.uri,
			rejectedCompletions,
		);
	}
...
function handleLineAcceptance(
	...
	if (last.linesLeft === undefined) {
		last.linesAccepted = countLines(
			cmp.insertText.substring(0, acceptedLength),
		);
	...
	last.partiallyAcceptedLength =
		(last.lastLineAcceptedLength || 0) + acceptedLength;
}
...
export function handleGhostTextPostInsert(
	...
	if (last.partiallyAcceptedLength) {
		suggestionStatus = {
			compType: 'full',
			acceptedLength:
				(last.partiallyAcceptedLength || 0) + cmp.displayText.length,
