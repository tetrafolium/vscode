...
export async function getDiagnosticsFromContextItems(
	...
	const diagnosticBags = filterContextItemsByType(
	...
	for (const item of diagnosticBags) {
		setupExpectationsForDiagnosticBags(
			accessor,
			completionId,
			item.data,
			item.providerId,
	...
	const diagnosticBagsWithProviderId: {
		providerId: string;
		bag: DiagnosticBagWithId;
	}[] = [];
	for (const item of diagnosticBags) {
		for (const diagnosticBag of item.data) {
			allUris.add(diagnosticBag.uri.toString());
			diagnosticBagsWithProviderId.push({
	...
	const filteredDiagnosticBags = diagnosticBagsWithProviderId
		.filter((item) => {
			const isValid =
				validationMap.get(item.bag.uri.toString())?.status === 'valid';

			// Set expectations regardless of validity
			if (isValid) {
				statistics.addExpectations(item.providerId, [
					[item.bag, 'included'],
				]);
			} else {
				statistics.addExpectations(item.providerId, [
					[item.bag, CONTENT_EXCLUDED_EXPECTATION],
				]);
			}
	...
	return filteredDiagnosticBags.sort(
		(a, b) => (a.importance ?? 0) - (b.importance ?? 0),
	);
}

function setupExpectationsForDiagnosticBags(
	accessor: ServicesAccessor,
	completionId: string,
	bags: DiagnosticBagWithId[],
	providerId: string,
) {
