...
} from './suggestions/partialSuggestions';
import {
	TelemetryStore,
	TelemetryWithExp,
	telemetry,
...
async function captureCode(
	...
	const result = await accessor
		.get(ICompletionsFileReaderService)
		.getOrReadTextDocumentWithFakeClientProperties({ uri });
	if (result.status !== 'valid') {
		postInsertionLogger.info(
			logTarget,
			`Could not get document for ${uri}. Maybe it was closed by the editor.`,
		);
		return {
			prompt: {
				prefix: '',
	...
	const hypotheticalPromptResponse =
	...
	if (hypotheticalPrompt.isFimEnabled && suffixOffset !== undefined) {
		...
		const contextIndent = contextIndentationFromText(
			documentTextBefore,
			offset,
			document.detectedLanguageId,
		);
		const indentTerminationFunction = indentationBlockFinished(
			contextIndent,
			undefined,
		);
		const terminationResult =
			indentTerminationFunction(hypotheticalResponse);
		...
		const maxOffset = Math.min(
			documentText.length,
			offset +
				(terminationResult ? terminationResult * 2 : captureCodeMargin),
		);
...
export function postRejectionTasks(
	...
	const positionTracker = instantiationService.createInstance(
		ChangeTracker,
		uri,
		insertionOffset - 1,
	);
	const suffixTracker = instantiationService.createInstance(
		ChangeTracker,
		uri,
		insertionOffset,
	);
	...
	const checkInCode = async (t: Timeout) => {
		...
		const { prompt, capturedCode, terminationOffset } =
			await instantiationService.invokeFunction(
				captureCode,
				uri,
				completionTelemetryData,
		...
		const promptTelemetry = {
			hypotheticalPromptJson: JSON.stringify({
		...
		instantiationService.invokeFunction(
			telemetry,
			insertionCategory + '.capturedAfterRejected',
			customTelemetryData,
...
export function postInsertionTasks(
	...
	const stillInCodeCheck = async (timeout: Timeout) => {
		const check = instantiationService.invokeFunction(
			checkStillInCode,
			insertionCategory,
			trimmedCompletion,
			insertionOffset,
	...
	if (
		postInsertConfiguration.triggerPostInsertionSynchroneously &&
		runtimeModeService.isRunningInTest()
	) {
...
async function citationCheck(
	...
	if (
		!copilotAnnotations ||
		(copilotAnnotations.ip_code_citations?.length ?? 0) < 1
	) {
...
function computeCitationStart(
	...
	if (
		insertedLength < completionLength &&
		citationStartOffset > insertedLength
	) {
		return undefined;
	}
...
function computeCitationEnd(
	completionLength: number,
	insertedLength: number,
	citationStopOffset: number,
): number {
