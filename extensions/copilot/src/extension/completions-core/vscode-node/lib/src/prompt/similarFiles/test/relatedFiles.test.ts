...
import {
	...
	ProviderCallback,
} from '../compositeRelatedFilesProvider';
...
suite('PromiseExpirationCacheMap', function () {
	const r: RelatedFilesType = new Map<
		NeighboringFileType,
		Map<string, string>
	>();
...
suite('relatedFiles tests', function () {
	...
	const OPEN_FILES_FOR_TEST = createOpenFiles(
		WKS_ROOTFOLDER,
		CURRENT_TIME_STAMP,
	);
	...
	test('Test scenario where 4 files provided by the C++ related files provider are identical to 2 provided by the OpenTabs`s neighborSource', async function () {
		...
		for (let i = 0; i < 2; i++) {
			CPP_OPENTAB_HEADERS.push(
				`${WKS_ROOTFOLDER}/relative/cppheader${i + 1}.h`,
			);
		}
		for (let i = 2; i < 4; i++) {
			CPP_NONOPENTAB_HEADERS.push(
				`${WKS_ROOTFOLDER}/relative/cppheader${i + 1}.h`,
			);
		}
		const CPP_ALL_HEADERS: string[] =
			CPP_NONOPENTAB_HEADERS.concat(CPP_OPENTAB_HEADERS);
		const CURRENT_TIME_STAMP = Date.now();
		...
		class MockedCppRelatedFilesProvider extends CompositeRelatedFilesProvider {
			override getRelatedFilesResponse(
				...
				return Promise.resolve({
					entries: [{ type: NeighboringFileType.RelatedCpp, uris }],
				});
			}
		...
		serviceCollection.define(
			ICompletionsRelatedFilesProviderService,
			new SyncDescriptor(MockedCppRelatedFilesProvider),
		);
		serviceCollection.define(
		...
		const result = await NeighborSource.getNeighborFilesAndTraits(
			accessor,
			FILE_CPP,
		...
		assert.strictEqual(
			result.neighborSource.has(NeighboringFileType.RelatedCpp),
			true,
		);
		assert.strictEqual(
			result.neighborSource.has(NeighboringFileType.OpenTabs),
			true,
		);
		for (const file of CPP_OPENTAB_HEADERS) {
			assert.strictEqual(
				result.neighborSource
					.get(NeighboringFileType.OpenTabs)
					?.includes(file),
				true,
			);
			assert.strictEqual(
				result.neighborSource
					.get(NeighboringFileType.RelatedCpp)
					?.includes(file),
				false,
			);
		}
	...
	test('Test scenarios where the C++ related files provider fails', async function () {
				...
				switch (this._failureType) {
					case FailureType.WithException:
						return Promise.reject(
							new Error(
								'The provider failed to provide the related files',
							),
						);
					...
					case FailureType.NoFailure:
						return Promise.resolve({
							entries: [
								{
									type: NeighboringFileType.RelatedCpp,
									uris: [DUMMY_RELATED_FILE],
								},
							],
						});
				}
		...
		const cppProvider = accessor.get(
			ICompletionsRelatedFilesProviderService,
		) as MockedCppRelatedFilesProvider;
		const telemetry = TelemetryWithExp.createEmptyConfigForTesting();
		const cppProviderGetMock = Sinon.spy(
			cppProvider,
		...
		for (let i = 0; i < RETRY_COUNT; i++) {
			cppProvider.setFailWith(
				RETRY_COUNT % 2 === 0
					? FailureType.WithException
					: FailureType.WithUndefined,
			);
	...
	suite('CompositeRelatedFilesProvider', function () {
		...
		async function compositeGetRelated(
			...
			serviceCollection.define(
				ICompletionsRelatedFilesProviderService,
				new SyncDescriptor(TestCompositeRelatedFilesProvider),
			);
			const accessor = serviceCollection.createTestingAccessor();
			...
			// Mock up the workspace folders.
			tdm.init([{ uri: WKS_ROOTFOLDER }]);
			const composite = accessor.get(
				ICompletionsRelatedFilesProviderService,
			) as TestCompositeRelatedFilesProvider;
			for (const { extensionId, languageId, callback } of providers) {
				composite.registerRelatedFilesProvider(
					extensionId,
					languageId,
					callback,
				);
			...
			const closedFiles = OPEN_FILES_FOR_TEST.map((f) => ({
				...f,
				uri: f.uri.replace('.', '2.'),
			}));
			...
			const uri =
				OPEN_FILES_FOR_TEST[filetype === 'javascript' ? 3 : 1].uri;
			...
			return (
				await getRelatedFilesAndTraits(
					accessor,
					doc,
					telemetryWithExp,
					cts.token,
				...
				[
					{
						...
						callback: (url: string) =>
							Promise.resolve({
								entries: [
									{
										type: NeighboringFileType.RelatedTypeScript,
										uris: [url.replace('.', '2.')],
									},
								],
							}),
		...
		test('Javascript provider returns a file for JS file', async function () {
			...
			assert.deepStrictEqual(
				...
				new Map([
					[
						NeighboringFileType.RelatedTypeScript,
						new Map([
							['file:///test/relative/j2.js', '// js dummy'],
						]),
					],
				]),
			);
				...
				[
					{
						...
						callback: (url: string) =>
							Promise.resolve({
								entries: [
									{
										type: NeighboringFileType.RelatedCSharpRoslyn,
										uris: [url.replace('.', '2.')],
									},
								],
							}),
			...
			assert.deepStrictEqual(
				...
				new Map([
					[
						NeighboringFileType.RelatedTypeScript,
						new Map([
							['file:///test/relative/j3.js', '// js dummy'],
						]),
					],
				]),
			);
			...
			assert.deepStrictEqual(
				...
				new Map([
					[
						NeighboringFileType.RelatedTypeScript,
						new Map([
							['file:///test/relative/j4.js', '// js dummy'],
						]),
					],
				]),
			);
		...
		test('C# provider returns a file for .cs file', async function () {
			...
			assert.deepStrictEqual(
				...
				new Map([
					[
						NeighboringFileType.RelatedCSharpRoslyn,
						new Map([['file:///test/e2.cs', '// cs dummy']]),
					],
				]),
			);
		});
				...
				[
					{
						...
						callback: (url: string) =>
							Promise.reject(
								new Error(`Error providing files for ${url}`),
							),
					},
				],
				telemetryWithExp,
				...
				[
					{
						...
						callback: (url: string) =>
							Promise.resolve({
								entries: [
									{
										type: NeighboringFileType.RelatedOther,
										uris: [url.replace('.', '4.')],
									},
								],
							}),
