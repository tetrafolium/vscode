...
import {
	getCppNumberOfSnippets,
	getCppSimilarFilesOptions,
} from './similarFileOptionsProviderCpp';

type SimilarFilesOptionsProvider = (
	accessor: ServicesAccessor,
	exp: TelemetryWithExp,
) => SimilarFilesOptions;
// Add here for more options for other language ids.
const languageSimilarFilesOptions: ReadonlyMap<
	string,
	SimilarFilesOptionsProvider
> = new Map<string, SimilarFilesOptionsProvider>([
	['cpp', getCppSimilarFilesOptions],
]);

export function getSimilarFilesOptions(
	accessor: ServicesAccessor,
	exp: TelemetryWithExp,
	langId: string,
): SimilarFilesOptions {
	const optionsProvider: SimilarFilesOptionsProvider | undefined =
		languageSimilarFilesOptions.get(langId);
	if (optionsProvider) {
...
const numberOfSnippets: ReadonlyMap<string, NumberOfSnippetsProvider> = new Map<
	string,
	NumberOfSnippetsProvider
>([['cpp', getCppNumberOfSnippets]]);

export function getNumberOfSnippets(
	exp: TelemetryWithExp,
	langId: string,
): number {
	const provider: NumberOfSnippetsProvider | undefined =
		numberOfSnippets.get(langId);
	return provider ? provider(exp) : DEFAULT_NUM_SNIPPETS;
}

export function useSubsetMatching(
	accessor: ServicesAccessor,
	telemetryWithExp: TelemetryWithExp,
): boolean {
	return (
		((telemetryWithExp.filtersAndExp.exp.variables[
			ExpTreatmentVariables.UseSubsetMatching
		] as boolean) ||
