...
export function editIsTooCloseToCursor(
	...
	if (filterByCursorLine) {
		if (
			...
			activeDocDistanceLimitFromCursor === undefined
		) {
			throw new Error(
	...
	if (
		filterByCursorLine &&
		(Math.abs(startLineNumber - cursorLine!) <=
			activeDocDistanceLimitFromCursor! ||
			Math.abs(endLineNumber - cursorLine!) <=
				activeDocDistanceLimitFromCursor!)
	) {
...
export const RecentEdits = (
	...
	context.useData(
		isCompletionRequestData,
		async (request: CompletionRequestData) => {
			if (!request.document) {
				return;
			}
			...
			const recentEditsConfig = (
				recentEditProvider as FullRecentEditsProvider
			).config;
			...
			for (let i = recentEdits.length - 1; i >= 0; i--) {
				...
				if (summarizedEdit) {
					...
					const relativePathOrUri = tdm.getRelativePath({
						uri: edit.file,
					});
					editSummaries.unshift(
						newLineEnded(`File: ${relativePathOrUri}`) +
							newLineEnded(summarizedEdit),
					);
				}
			}
