...
suite('traitsContextProvider', function () {
	...
	const resolvedContextItems: ResolvedContextItem<TraitWithId>[] = [
		...
		},
		{
			...
			data: [
				{
					name: 'trait_from_context_provider2_1',
					value: 'value_3',
					id: '3',
					type: 'Trait',
				},
	...
	test('can get traits from context text providers and flattens them', function () {
		const traits = getTraitsFromContextItems(
			accessor,
			'COMPLETION_ID',
		...
		assert.deepStrictEqual(
			traits.map((t) => t.name),
			[
				'trait_from_context_provider1_2',
				'trait_from_context_provider2_1',
				'trait_from_context_provider1_1',
			],
		);
	});
	...
	test('set expectations for contextProviderStatistics', function () {
		...
		const traitExpectations =
			statistics.expectations.get('testTraitsProvider');
		...
		const traitExpectations2 = statistics.expectations.get(
			'testTraitsProvider2',
		);
