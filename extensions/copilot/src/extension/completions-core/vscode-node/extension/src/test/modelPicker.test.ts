...
suite('ModelPickerManager unit tests', function () {
	...
	setup(function () {
		...
		getGenericCompletionModelsStub = sandbox
			.stub(availableModelsManager, 'getGenericCompletionModels')
			.returns(fakeModels);
		modelPicker = accessor
			.get(IInstantiationService)
			.createInstance(ModelPickerManager);
	});
	...
	test('selecting a model updates user selection', async function () {
		...
		const setModelStub = sandbox
			.stub(modelPicker, 'setUserSelectedCompletionModel')
			.resolves();
		...
		assert(
			...
			'model picker should have a model-b second item.',
		);
		...
		assert(
			setModelStub.calledOnce,
			'setUserSelectedCompletionModel should be called once',
		);
	...
	test('selecting the learn more link tries to open the learn more url', async function () {
		...
		assert(
			learnMoreItem !== undefined,
			'model picker should have a learn more item.',
		);
