 ...
 */
export class LoggingCitationManager
	extends Disposable
	implements ICompletionsCitationManager
{
	...
	constructor(
		...
		super();
		this.codeReference = this._register(
			this.instantiationService.createInstance(CodeReference),
		);
		const disposable = onCopilotToken(authenticationService, (_) => {
			...
			this.logger =
				instantiationService.createInstance(GitHubCopilotLogger);
			const initialNotificationCommand = commands.registerCommand(
				OutputPaneShowCommand,
	...
	async handleIPCodeCitation(citation: IPDocumentCitation): Promise<void> {
		if (
			!this.codeReference.enabled ||
			!this.logger ||
			citation.details.length === 0
		) {
			return;
		}

		const start = citation.location?.start;
		const matchLocation = start
			? `[Ln ${start.line + 1}, Col ${start.character + 1}]`
		...
		for (const detail of citation.details) {
			...
			this.logger.info(
				`License: ${license.replace('NOASSERTION', 'unknown')}, URL: ${url}`,
			);
		}
