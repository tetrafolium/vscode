...
import {
	TextEdit,
	TextReplacement,
} from '../../../../../util/vs/editor/common/core/edits/textEdit';
...
export function exception(
	...
	telemetryService.sendGHTelemetryException(
		error,
		'codeUnification.completions.exception',
	);
	handleException(accessor, error, origin, logger);
}

/** @public */
export class CopilotInlineCompletionItemProvider
	extends Disposable
	implements InlineCompletionItemProvider
{
	...
	constructor(
		...
		private readonly telemetryService: ICompletionsTelemetryService,
		...
		super();
		this.copilotCompletionFeedbackTracker = this._register(
			this.instantiationService.createInstance(
				CopilotCompletionFeedbackTracker,
			),
		);
		this.ghostTextProvider =
			this.instantiationService.createInstance(GhostTextProvider);
		this.inlineEditLogger =
			this.instantiationService.createInstance(InlineEditLogger);
		this.telemetrySender = this.instantiationService.createInstance(
			TelemetrySender,
			undefined,
		);
		this.logger = logService.createSubLogger([
			'Ghost',
	...
	async provideInlineCompletionItems(
		...
		if (context.triggerKind === InlineCompletionTriggerKind.Automatic) {
			if (
				!this.instantiationService.invokeFunction(
					isCompletionEnabledForDocument,
					doc,
				)
			) {
				return;
		...
		return await this.requestLogger.captureInvocation(
			capturingToken,
			async () => {
				const logContext = new GhostTextLogContext(
					doc.uri.toString(),
					doc.version,
					context,
				);
				...
				const logger = this.logger
					.createSubLogger('provideInlineCompletionItems')
					.withExtraTarget(
				...
				try {
					...
					return await this._provideInlineCompletionItems(
						doc,
						position,
						context,
						telemetryBuilder,
					...
					this.telemetryService.sendGHTelemetryException(
						e,
						'codeUnification.completions.exception',
					);
	...
	private async _provideInlineCompletionItems(
		...
		if (
			!copilotConfig.get(
				'respectSelectedCompletionInfo',
				quickSuggestionsDisabled() || BuildInfo.isPreRelease(),
			)
		) {
		...
		try {
			const list =
				await this.ghostTextProvider.provideInlineCompletionItems(
					doc,
					position,
					context,
					telemetryBuilder,
			...
			telemetryBuilder.nesBuilder.setHasNextEdit(
	...
	handleDidPartiallyAcceptCompletionItem(
		...
		try {
			return this.ghostTextProvider.handleDidPartiallyAcceptCompletionItem(
				item,
				acceptedLengthOrInfo,
			);
		} catch (e) {
			this.instantiationService.invokeFunction(
				exception,
	...
	handleEndOfLifetime(
		completionItem: GhostTextCompletionItem,
		reason: InlineCompletionEndOfLifeReason,
	) {
		try {
			return this.ghostTextProvider.handleEndOfLifetime(
				completionItem,
				reason,
			);
		} catch (e) {
	...
	handleListEndOfLifetime(
		...
		softAssert(
			ghostTextList.telemetryBuilder !== undefined,
			'Expected GhostTextCompletionList to have telemetryBuilder',
		);
	...
	private logSuggestion(
		...
		if (typeof firstItem.insertText !== 'string') {
			logContext.addLog(
				'Inline completion item has non-string insertText',
			);
			return;
		}

		const text = new LineBasedText(
			(lineNumber) => doc.lineAt(lineNumber - 1).text,
			doc.lineCount,
		);
		...
		const lineEdit = LineEdit.fromTextEdit(
			new TextEdit([
				new TextReplacement(
					new Range(
						firstItem.range.start.line + 1,
						firstItem.range.start.character + 1,
						firstItem.range.end.line + 1,
						firstItem.range.end.character + 1,
					),
