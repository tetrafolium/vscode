...
import {
	...
	SuggestionsPanelManagerInterface,
} from './baseSuggestionsPanel';

export interface ListDocumentInterface {
	runQuery(): Promise<void>;
}

export abstract class BaseSuggestionsPanelManager<
...
> implements SuggestionsPanelManagerInterface {
	...
	constructor(
		...
		protected readonly _extensionContext: IVSCodeExtensionContext,
	) {}
	...
	renderPanel(
		...
		const panel = window.createWebviewPanel(
			this.config.webviewId,
			title,
			ViewColumn.Two,
			{
				enableScripts: true,
				localResourceRoots: [
					Uri.joinPath(this._extensionContext.extensionUri, 'dist'),
				],
				retainContextWhenHidden: true,
			},
		);

		const suggestionPanel = this.createSuggestionsPanel(
			panel,
			document,
			this,
		);
	...
	registerCommands(): IDisposable {
		...
		disposableStore.add(
			this._instantiationService.invokeFunction(
				registerCommandWrapper,
				this.config.commands.accept,
				() => {
		...
		disposableStore.add(
			this._instantiationService.invokeFunction(
				registerCommandWrapper,
				this.config.commands.navigatePrevious,
				() => {
		...
		disposableStore.add(
			this._instantiationService.invokeFunction(
				registerCommandWrapper,
				this.config.commands.navigateNext,
				() => {
					return this.activeWebviewPanel?.postMessage({
