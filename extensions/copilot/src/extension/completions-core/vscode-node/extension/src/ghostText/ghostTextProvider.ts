...
export class GhostTextProvider {
	...
	async provideInlineCompletionItems(
		...
		const formattingOptions = window.visibleTextEditors.find(
			(e) => e.document.uri === vscodeDoc.uri,
		)?.options;
		...
		const items: GhostTextCompletionItem[] = rawCompletions.map(
			(completion) => {
	...
	handleDidShowCompletionItem(item: GhostTextCompletionItem) {
		this.instantiationService.invokeFunction(
			handleGhostTextShown,
			item.copilotCompletion,
		);
	}

	handleDidPartiallyAcceptCompletionItem(
		item: GhostTextCompletionItem,
		info: number | PartialAcceptInfo,
	) {
			...
			case InlineCompletionEndOfLifeReasonKind.Ignored: {
				...
				if (reason.supersededBy) {
					const supersededByItem =
						reason.supersededBy as GhostTextCompletionItem;
					completionItem.telemetryBuilder.setSupersededBy(
						supersededByItem.opportunityId,
					);
				}
				completionItem.telemetryBuilder.setUserTypingDisagreed(
