...
export async function launchSolutions(
	...
	const promptSetup = await setupPromptAndTelemetry(
		accessor,
		solutionManager,
		'open copilot',
		solutionsLogger,
	...
	const { prompt, trailingWs, telemetryData, repoInfo, ourRequestId } =
		promptSetup;

	// Setup completion parameters using shared function
	const { extra, postOptions, finishedCb, engineInfo } =
	...
	const res = await fetcherService.fetchAndStreamCompletions(
		completionParams,
	...
	const solutions = asyncIterableMapFilter(
		choices,
		async (apiChoice: APIChoice) => {
			let display = apiChoice.completionText;
			logger.info(
				`Open Copilot completion: [${apiChoice.completionText}]`,
			);
			...
			const displayStartPos =
				(await getNodeStartUtil(
					document,
					position,
					apiChoice.completionText,
				)) ?? LocationFactory.position(position.line, 0);
			const [displayBefore] = trimLastLine(
				document.getText(
					LocationFactory.range(displayStartPos, position),
				),
			);
			...
			if (
				trailingWs.length > 0 &&
				completionText.startsWith(trailingWs)
			) {
				completionText = completionText.substring(trailingWs.length);
			}

			const meanLogProb = apiChoice.meanLogProb;
			const meanProb: number =
				meanLogProb !== undefined ? Math.exp(meanLogProb) : 0;
...
export async function runSolutions(
	...
	return statusReporter.withProgress(async () => {
		const nextSolution = instantiationService.invokeFunction(
			launchSolutions,
			solutionManager,
		);
