...
`,
			...
			const proxyAgent = loadVSCodeModule<ProxyAgent>(
				'@vscode/proxy-agent',
			);
			const loadSystemCertificatesFromNode =
				this.configurationService.getNonExtensionConfig<boolean>(
					'http.systemCertificatesNode',
				);
			const osCertificates = proxyAgent?.loadSystemCertificates
			...
			const nodeFetchCurrent =
				!electronCurrent && !nodeCurrent && nodeFetchConfig;
			...
			for (const url of urls) {
				...
				for (const family of [4, 6]) {
					...
					try {
						...
						if (dnsResult) {
							await appendText(
								editor,
								`${dnsResult.address} (${Date.now() - start} ms)\n`,
							);
						} else {
							await appendText(
								editor,
						...
						await appendText(
							editor,
							`Error (${Date.now() - start} ms): ${err?.message}\n`,
						);
					}
				}
				...
				if (proxyAgent?.resolveProxyURL) {
					...
					try {
						const proxyURL = await Promise.race([
							proxyAgent.resolveProxyURL(url),
							timeoutAfter(timeoutSeconds * 1000),
						]);
						if (proxyURL === 'timeout') {
							await appendText(
								editor,
							...
							await appendText(
								editor,
								`${proxyURL || 'None'} (${Date.now() - start} ms)\n`,
							);
				...
				if (
					proxyAgent?.loadSystemCertificates &&
					probeProxyURL?.startsWith('https:')
				) {
					const tlsOrig: typeof tls | undefined = (tls as any)
						.__vscodeOriginal;
					if (tlsOrig) {
						...
						try {
							const result = await Promise.race([
								tlsConnect(tlsOrig, probeProxyURL, [
				...
				if (probeProxyURL) {
					const httpx: typeof https | typeof http | undefined =
						probeProxyURL.startsWith('https:')
							? (https as any).__vscodeOriginal
					...
					if (httpx) {
						...
						try {
							const result = await Promise.race([
								proxyConnect(httpx, probeProxyURL, url),
								timeout(timeoutSeconds * 1000),
							]);
							if (result) {
								const headers = Object.keys(result.headers).map(
									(header) =>
				...
				for (const [name, fetcher] of Object.entries(fetchers)) {
					await appendText(
						editor,
						`- ${name}${fetcher.current ? ' (configured)' : fetcher.fetcher?.getUserAgentLibrary() === activeFetcher ? ' (active)' : ''}: `,
					);
					if (fetcher.fetcher) {
						const start = Date.now();
						try {
							const response = await Promise.race([
								fetcher.fetcher.fetch(url, {
									headers: authHeaders,
									callSite: 'diagnostics-fetcher-probe',
								}),
								timeout(timeoutSeconds * 1000),
							]);
							if (response) {
								await appendText(
									editor,
									`HTTP ${response.status} (${Date.now() - start} ms)\n`,
								);
							} else {
								await appendText(
									editor,
							...
							await appendText(
								editor,
								`Error (${Date.now() - start} ms): ${collectErrorMessages(err)}\n`,
							);
						}
					} else {
			...
			const currentFetcher =
				Object.values(fetchers).find((fetcher) => fetcher.current)
					?.fetcher || nodeFetcher;
			const secondaryUrls = [
				{
					url: 'https://mobile.events.data.microsoft.com',
				...
				{
					url: 'https://copilot-telemetry.githubusercontent.com/_ping',
					fetcher: nodeFetcher,
				},
				{
					url: vscode.Uri.parse(
						this.capiClientService.copilotTelemetryURL,
					)
						.with({ path: '/_ping' })
						.toString(),
					fetcher: nodeFetcher,
			...
			for (const { url, fetcher } of secondaryUrls) {
				...
				try {
					const response = await Promise.race([
						fetcher.fetch(url, {
							headers: authHeaders,
							callSite: 'diagnostics-secondary-probe',
						}),
						timeout(timeoutSeconds * 1000),
					]);
					if (response) {
...
In corporate networks: [Troubleshooting firewall settings for GitHub Copilot](https://docs.github.com/en/copilot/troubleshooting-github-copilot/troubleshooting-firewall-settings-for-github-copilot).`,
		...
		this._context.subscriptions.push(
			vscode.commands.registerCommand(
				'github.copilot.debug.collectDiagnostics.internal',
				collectDiagnostics,
			),
		);
		this._context.subscriptions.push(
			vscode.commands.registerCommand(
				'github.copilot.debug.showOutputChannel.internal',
				() => outputChannel.show(),
			),
		);
		this._context.subscriptions.push(
...
async function loadSystemCertificates(
	load: NonNullable<ProxyAgent['loadSystemCertificates']>,
	loadSystemCertificatesFromNode: boolean | undefined,
...
async function proxyConnect(
	httpx: typeof https | typeof http,
	proxyUrl: string,
	targetUrl: string,
	sanitize = false,
) {
			...
			const headers = [
				...
				(acc, header) => {
					...
					if (value) {
						const doSanitize =
							sanitize &&
							!['proxy-agent', 'server'].includes(header);
						acc[header] = doSanitize
							? Array.isArray(value)
...
const alwaysShowSettingsIds = ['http.systemCertificatesNode'];
...
function getNetworkSettings() {
	...
	return networkSettingsIds
		.map((key) => {
			...
			if (
				alwaysShowSettingsIds.includes(key) ||
				(v !== i?.defaultValue &&
					!(
						Array.isArray(v) &&
						Array.isArray(i?.defaultValue) &&
						v.length === 0 &&
						i?.defaultValue.length === 0
					))
			) {
...
function collectFetcherTelemetry(accessor: ServicesAccessor): void {
	...
	if (
		...
		extensionContext.extensionMode !== vscode.ExtensionMode.Production ||
		isScenarioAutomation
	) {
		return;
	}
	...
	if (
		!configurationService.getExperimentBasedConfig(
			ConfigKey.TeamInternal.DebugCollectFetcherTelemetry,
			expService,
		)
	) {
		return;
	...
	const now = Date.now();
	const previous = extensionContext.globalState.get<number>(
		'lastCollectFetcherTelemetryTime',
		0,
	);
	...
	(async () => {
		...
		await extensionContext.globalState.update(
			'lastCollectFetcherTelemetryUUID',
			windowUUID,
		);
		await timeout(5000);
		if (
			extensionContext.globalState.get<string>(
				'lastCollectFetcherTelemetryUUID',
			) !== windowUUID
		) {
		...
		for (const fetcher of fetchers) {
			...
			try {
				const response = await sendRawTelemetry(
					...
					envService,
					extensionContext,
					'GitHub.copilot-chat/fetcherTelemetryProbe',
					{},
				);
				probeResults[key] = `Status: ${response.status}`;
				logService.debug(
					`Fetcher telemetry probe: ${library} ${probeResults[key]} (${Date.now() - requestStartTime}ms)`,
				);
			} catch (e) {
				probeResults[key] =
					`Error: ${sanitizeNetworkErrorForTelemetry(collectSingleLineErrorMessage(e, true))}`;
				logService.debug(
					`Fetcher telemetry probe: ${library} ${probeResults[key]} (${Date.now() - requestStartTime}ms)`,
				);
			}
		...
		const extensionKind =
			extensionContext.extension.extensionKind === vscode.ExtensionKind.UI
				? 'local'
				: 'remote';
		for (const fetcher of fetchers) {
			const requestStartTime = Date.now();
			try {
				...
				*/
				...
				const response = await sendRawTelemetry(
					...
					envService,
					extensionContext,
					'GitHub.copilot-chat/fetcherTelemetry',
					properties,
				);

				logService.debug(
					`Fetcher telemetry: Succeeded in ${Date.now() - requestStartTime}ms using ${fetcher.getUserAgentLibrary()} with status ${response.status} (${response.statusText}).`,
				);
			} catch (e) {
				logService.debug(
					`Fetcher telemetry: Failed in ${Date.now() - requestStartTime}ms using ${fetcher.getUserAgentLibrary()}.`,
				);
			}
		}
	})().catch((err) => {
...
async function sendRawTelemetry(
	fetcher: IFetcher,
	...
	const ariaKey =
		(extensionContext.extension.packageJSON as { ariaKey?: string })
			.ariaKey ?? '';
...
class NetworkStatus extends Disposable {
	...
	private readonly _fetchSubscription = this._register(
			...
			_configurationService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(
						ConfigKey.TeamInternal.DebugShowNetworkStatus
							.fullyQualifiedId,
					)
				) {
					this._update();
	...
	private _update(): void {
		...
		if (enabled && !this._fetchSubscription.value) {
			this._fetchSubscription.value = this._fetcherService.onDidFetch(
				(e) => this._onEvent(e),
			);
		} else if (!enabled) {
		...
		const byHostname = new Map<
			string,
			{ total: number; errors: number; cancellations: number }
		>();
		...
		tooltip.appendMarkdown(`|:--|--:|--:|--:|\n`);
		for (const [hostname, { total, errors, cancellations }] of [
			...byHostname,
