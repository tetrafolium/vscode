...
} from '../../../platform/authentication/node/copilotTokenManager';
...
} from '../../../platform/github/common/githubApiFetcherService';
...
} from '../../../platform/remoteCodeSearch/common/adoCodeSearchService';
import {
	GithubCodeSearchService,
	IGithubCodeSearchService,
} from '../../../platform/remoteCodeSearch/common/githubCodeSearchService';
...
} from '../../../platform/telemetry/node/azureInsights';
...
import {
	ITestDepsResolver,
	TestDepsResolver,
} from '../../../platform/testing/node/testDepsResolver';
import {
	ITokenizerProvider,
	TokenizerProvider,
...
import {
	IWorkspaceFileIndex,
	WorkspaceFileIndex,
} from '../../../platform/workspaceChunkSearch/node/workspaceFileIndex';
...
import {
	ApiEmbeddingsIndex,
	IApiEmbeddingsIndex,
} from '../../context/node/resolvers/extensionApi';
import {
...
import {
	IPromptCategorizerService,
	PromptCategorizerService,
} from '../../prompt/node/promptCategorizer';
...
} from '../../tools/common/memoryCleanupService';
...
export function registerServices(
	...
	builder.define(
		IMemoryCleanupService,
		new SyncDescriptor(MemoryCleanupService),
	);
	builder.define(
	...
	builder.define(
		IFetcherService,
		new SyncDescriptor(FetcherService, [undefined]),
	);
	...
	builder.define(
		ITelemetryUserConfig,
		new SyncDescriptor(TelemetryUserConfigImpl, [undefined, undefined]),
	);
	const internalAIKey =
		extensionContext.extension.packageJSON.internalLargeStorageAriaKey ??
		'';
	...
	if (isTestMode || isScenarioAutomation) {
		...
		builder.define(
			...
			getOrCreateTestingCopilotTokenManager(env.devDeviceId),
		);
	} else {
		setupTelemetry(builder, extensionContext, internalAIKey, ariaKey);
		builder.define(
			ICopilotTokenManager,
			new SyncDescriptor(VSCodeCopilotTokenManager),
		);
	}
	...
	if (isScenarioAutomation) {
		...
		builder.define(
			IEndpointProvider,
			new SyncDescriptor(ScenarioAutomationEndpointProviderImpl),
		);
		builder.define(IIgnoreService, new SyncDescriptor(NullIgnoreService));
		builder.define(
			IWorkspaceChunkSearchService,
			new SyncDescriptor(ScenarioAutomationWorkspaceChunkSearchService),
		);
	} else {
		builder.define(
			IAuthenticationService,
		...
		builder.define(
			IEndpointProvider,
			new SyncDescriptor(ProductionEndpointProvider),
		);
		builder.define(IIgnoreService, new SyncDescriptor(VsCodeIgnoreService));
		builder.define(
			IWorkspaceChunkSearchService,
			new SyncDescriptor(WorkspaceChunkSearchService),
		);
	}
	...
	builder.define(
		IParserService,
		new SyncDescriptor(ParserServiceImpl, [/*useWorker*/ true]),
	);
	...
	builder.define(
		...
		new SyncDescriptor(ChunkingEndpointClientImpl),
	);
	...
	builder.define(
		...
		new SyncDescriptor(GitCommitMessageServiceImpl),
	);
	builder.define(
		IGithubRepositoryService,
		new SyncDescriptor(GithubRepositoryService),
	);
	builder.define(
		IDevContainerConfigurationService,
		new SyncDescriptor(DevContainerConfigurationServiceImpl),
	);
	builder.define(IChatAgentService, new SyncDescriptor(ChatAgentService));
	builder.define(
		IPromptCategorizerService,
		new SyncDescriptor(PromptCategorizerService),
	);
	...
	builder.define(
		...
		new SyncDescriptor(SessionTranscriptService),
	);
	builder.define(
		IChatDebugFileLoggerService,
		new SyncDescriptor(ChatDebugFileLoggerService),
	);
	...
	builder.define(
		IChatWebSocketManager,
		new SyncDescriptor(ChatWebSocketManager),
	);
	builder.define(IFeedbackReporter, new SyncDescriptor(FeedbackReporter));
	builder.define(
		IApiEmbeddingsIndex,
		new SyncDescriptor(ApiEmbeddingsIndex, [/*useRemoteCache*/ true]),
	);
	builder.define(
		IGithubApiFetcherService,
		new SyncDescriptor(GithubApiFetcherService),
	);
	builder.define(
		IAdoCodeSearchService,
		new SyncDescriptor(AdoCodeSearchService),
	);
	builder.define(
		ISettingsEditorSearchService,
		new SyncDescriptor(SettingsEditorSearchServiceImpl),
	);
	builder.define(
		INewWorkspacePreviewContentManager,
	...
	builder.define(
		...
		new SyncDescriptor(LanguageContextServiceImpl),
	);
	builder.define(
		ILanguageContextProviderService,
		new SyncDescriptor(LanguageContextProviderService),
	);
	builder.define(
		IWorkspaceListenerService,
		new SyncDescriptor(WorkspacListenerService),
	);
	builder.define(
		ICodeSearchAuthenticationService,
		new SyncDescriptor(VsCodeCodeSearchAuthenticationService),
	);
	builder.define(
		ITodoListContextProvider,
	...
	builder.define(
		...
		new SyncDescriptor(CopilotInlineCompletionItemProviderService),
	);
	builder.define(
		ISimilarFilesContextService,
		new SyncDescriptor(SimilarFilesContextService),
	);
	builder.define(
		IGitHubOrgChatResourcesService,
	...
	if (otelConfig.enabled) {
		...
		const { NodeOTelService } =
			require('../../../platform/otel/node/otelServiceImpl') as typeof import('../../../platform/otel/node/otelServiceImpl');
		...
		const logFn: import('../../../platform/otel/node/otelServiceImpl').OTelLogFn =
			(level, msg) => {
				if (level === 'error') {
					console.error(msg);
				} else if (level === 'warn') {
		...
		builder.define(
			...
			new NodeOTelService(
				otelConfig,
				logFn,
				otelConfig.dbSpanExporter ? otelSqliteStore : undefined,
			),
		);
	} else {
...
function setupMSFTExperimentationService(
	...
	const experimentsEnabled = workspace
		.getConfiguration('workbench')
		.get<boolean>('enableExperiments', true);
	if (
		ExtensionMode.Production === extensionContext.extensionMode &&
		!isScenarioAutomation &&
		experimentsEnabled
	) {
		...
		builder.define(
			IExperimentationService,
			new SyncDescriptor(MicrosoftExperimentationService),
		);
	} else {
		builder.define(
			IExperimentationService,
