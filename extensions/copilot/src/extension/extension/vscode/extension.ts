...
export async function baseActivate(
	...
	const isStableVsCode = !(
		env.appName.includes('Insiders') ||
		env.appName.includes('Exploration') ||
		env.appName.includes('OSS')
	);
	const showSwitchToReleaseViewCtxKey =
	...
	const result = {
		getAPI(version: number) {
			if (version > CopilotExtensionApi.version) {
				throw new Error(
					'Invalid Copilot Chat extension API version. Please upgrade Copilot Chat.',
				);
			}
...
export function createInstantiationService(
	configuration: IExtensionActivationConfiguration,
