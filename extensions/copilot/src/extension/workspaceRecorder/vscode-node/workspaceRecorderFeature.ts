...
export class WorkspaceRecorderFeature extends Disposable {
	...
	constructor(
		...
		@IWorkspaceListenerService
		private readonly _workspaceListenerService: IWorkspaceListenerService,
	) {
		super();
	...
	async init(store: DisposableStore) {
		...
		const recordingDirPath = join(
			this._vscodeExtensionContext.globalStorageUri.fsPath,
			'workspaceRecordings',
		);
		await mkdir(recordingDirPath, { recursive: true });

		const workspacesIndexFile =
			await JSONFile.readOrCreate<WorkspacesIndex>(
				join(recordingDirPath, 'workspaces.json'),
				{ workspaceIdxByRoot: {} },
			);
		...
		const w = new InitializedWorkspaceRecorderFeature(
			gitApi,
			recordingDirPath,
			workspacesIndexFile,
			this._workspaceListenerService,
		);
		store.add(w);
	}
}
...
class InitializedWorkspaceRecorderFeature extends Disposable {
	private readonly _logProvider = new VirtualTextDocumentProvider(
		'copilotLogProvider',
	);
	...
	constructor(
		...
		const commandIdOpenRecordingFolder =
			'vscodeCopilot.openRecordingFolder';
		this._register(
			vscode.commands.registerCommand(
				commandIdOpenRecordingFolder,
				() => {
		...
		this._register(
			vscode.commands.registerCommand(commandIdAddBookmark, () => {
				for (const r of this.recorders.values()) {
					r.addBookmark();
				}
				vscode.window.showInformationMessage(
					'Bookmark added to recording.',
				);
			}),
		);

		const doc = this._logProvider.createDocument(
			'',
			'current.recording.w.json',
		);

		const commandIdViewRecording = 'vscodeCopilot.viewRecording';
		this._register(
			vscode.commands.registerCommand(
				commandIdViewRecording,
				async () => {
		...
		this._register(
			autorunWithStore((reader, store) => {
				...
				lines.push(
					'All recordings are stored locally and not uploaded.',
				);
				lines.push(
					'$(chevron-right) Click status bar entry to add a bookmark',
				);
				lines.push(
					`[$(chevron-right) Open local recording folder](command:${commandIdOpenRecordingFolder})`,
				);
				lines.push(
					`[$(chevron-right) View recording](command:${commandIdViewRecording})`,
				);
		...
		this._register(
			vscode.workspace.onDidOpenTextDocument((e) => {
				...
				if (workspaceRecorder) {
					workspaceRecorder.handleOnDidOpenTextDocument(
						docUri,
						e.getText(),
						e.version,
					);
		...
		this._register(
			vscode.workspace.onDidOpenNotebookDocument(async (e) => {
				...
				if (workspaceRecorder) {
					workspaceRecorder.handleOnDidOpenTextDocument(
						docUri,
						serializeNotebookDocument(e, {
							cell_uri_fragment: true,
						}),
						e.version,
					);
					workspaceRecorder.handleDocumentEvent(
						docUri,
						Date.now(),
						e.version,
					);
		...
		this._register(
			this.workspaceListenerService.onStructuredData(
				(item: IRecordableLogEntry | IRecordableEditorLogEntry) => {
					if ('modelUri' in item) {
						const docUri = item.modelUri.toString();
						const workspaceRecorder =
							this.getWorkspaceRecorder(docUri);
						if (workspaceRecorder) {
							workspaceRecorder.handleDocumentEvent(
								docUri,
								item.time,
						...
						if (recorder) {
							recorder.handleEvent(item.time, {
								...item,
								time: undefined,
							});
						}
		...
		this._register(
			vscode.workspace.onDidChangeTextDocument((e) => {
				...
				if (workspaceRecorder) {
					...
					workspaceRecorder.handleOnDidChangeTextDocument(
						docUri,
						edit,
						e.document.version,
						e.detailedReason?.metadata,
					);
				}
			}),
		);
		...
		this._register(
			vscode.workspace.onDidCloseTextDocument((e) => {
				const docUri = documentUriFromTextDocument(e);
				this.getWorkspaceRecorder(docUri)?.handleOnDidCloseTextDocument(
					docUri,
				);
			}),
		);
		...
		this._register(
			autorunWithStore((reader, store) => {
				const activeEditor =
					observableVscodeApi.activeTextEditor.read(reader);
				if (!activeEditor) {
					return;
				}
				const docUri = documentUriFromTextDocument(
				...
				store.add(
					autorun((reader) => {
						...
						const offsetRanges = selections.map((s) =>
							rangeToOffsetRange(s, activeEditor.editor.document),
						);
						workspaceRecorder.handleOnDidSelectionChange(
							docUri,
							offsetRanges,
						);
					}),
	...
	private getWorkspaceRepository(
		docUri: string,
	): gitExt.Repository | undefined {
		if (process.platform === 'win32') {
			// Use case insensitive
			return this.gitApi.repositories.find((r) =>
				startsWithIgnoreCase(docUri, r.rootUri.toString()),
			);
		}
		return this.gitApi.repositories.find((r) =>
			docUri.startsWith(r.rootUri.toString()),
		);
	}

	private getWorkspaceRecorder(
		docUri: string,
	): WorkspaceRecorder | undefined {
		...
		if (!recorder) {
			let workspaceIdxByRoot =
				this.workspacesIndexFile.value.workspaceIdxByRoot;
			...
			if (workspaceIdx === undefined) {
				...
				workspaceIdxByRoot = {
					...workspaceIdxByRoot,
					[workspaceRootKey]: workspaceIdx,
				};
				this.workspacesIndexFile.setValue({
					workspaceIdxByRoot: workspaceIdxByRoot,
				});
			}

			const checkIsIgnored = new BatchedProcessor<string, boolean>(
				async (paths) => {
			...
			const isIgnored = new CachedFunction(
			...
			const folderName =
				sanitizeFolderName(basename(workspaceRootKey)) +
				'-' +
				workspaceIdx;
			recorder = new WorkspaceRecorder(
				workspaceRoot,
				join(this.recordingDirPath, folderName),
				{
					isIgnoredDocument: (documentUri) =>
						isIgnored.get(documentUri),
				},
			);
...
function documentUriFromTextDocument(
	...
	if (textDocument.uri.scheme === Schemas.vscodeNotebookCell) {
		...
		return notebookDocument.uri
			.with({ fragment: textDocument.uri.fragment })
			.toString();
	}
