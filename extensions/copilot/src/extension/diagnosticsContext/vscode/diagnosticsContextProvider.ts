...
export class DiagnosticsContextContribution extends Disposable {
	...
	constructor(
		...
		super();
		this._enableDiagnosticsContextProvider =
	...
	private register(): IDisposable {
		...
		try {
			...
			this.logService.error(
				'Error registering diagnostics context provider:',
				error,
			);
		}
		return disposables;
...
class ContextResolver implements Copilot.ContextResolver<Copilot.SupportedContextItem> {
	...
	resolveOnTimeout(
		...
		const languageEnablement =
			this.experimentationService.getTreatmentVariable<boolean>(
				`config.github.copilot.chat.inlineEdits.diagnosticsContextProvider.${languageId}`,
			);
		...
		const linesAbove =
			this.configurationService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsXtabProviderNLinesAbove,
				this.experimentationService,
			) ?? N_LINES_ABOVE;
		const linesBelow =
			this.configurationService.getExperimentBasedConfig(
		...
		const editWindow = new Range(
			cursor.lineNumber - linesAbove,
			1,
			cursor.lineNumber + linesBelow,
	...
	private getContext(
		...
		if (options.includeDiagnosticsRange) {
			diagnostics = diagnostics.filter((d) =>
				options.includeDiagnosticsRange!.containsRange(
					toInternalRange(d.range),
				),
			);
		}
		...
		return diagnosticsToTraits(
			diagnosticsSortedByDistance.slice(0, options.maxDiagnostics),
		);
	}
}
...
function diagnosticsToString(diagnostics: Diagnostic): string {
	...
	const severity =
		diagnostics.severity === DiagnosticSeverity.Error ? 'error' : 'warning';
