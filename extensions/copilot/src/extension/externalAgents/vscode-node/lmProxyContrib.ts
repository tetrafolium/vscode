...
export class LanguageModelProxyContrib
	extends Disposable
		...
		const updateRegistration = () => {
			...
			const enableProxy =
				...
				(token.codexAgentEnabled ||
					configurationService.getNonExtensionConfig(
						'chat.experimental.codex.enabled',
					));
			if (!providerDisposable.value && enableProxy) {
				providerDisposable.value =
					vscode.lm.registerLanguageModelProxyProvider(
