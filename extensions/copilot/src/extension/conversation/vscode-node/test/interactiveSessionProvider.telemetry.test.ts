...
} from '../../../../platform/test/node/telemetry';
...
suite('Conversation telemetry tests - Integration tests', function () {
	...
	test.skip('Telemetry for user message', async function () {
		...
		const messageText =
			'Write me a function that returns the square root of a number.';

		const [messages] = await withTelemetryCapture(
			testingServiceCollection,
			async (accessor) => {
				const token = new vscode.CancellationTokenSource().token;
				const request: vscode.ChatRequest = new TestChatRequest(
					messageText,
				);
		...
		const names = messages
			...
			.filter(
				(name) => !['auth.new_login', 'auth.new_token'].includes(name),
			);
		...
		assert.ok(
			userMessage,
			'conversation.message event for user message does not exist',
		);
		...
		assert.ok(
			userMessage.data.baseData.measurements.messageCharLen ===
				messageText.length,
		...
		assert.ok(
			userMessageText,
			'conversation.messageText event for user message does not exist',
		);
		assert.ok(
			userMessageText.data.baseData.properties.messageText ===
				messageText,
		...
		assert.ok(
			userMessageRequest,
			'request.sent event for user message does not exist',
		);
		...
		assert.ok(
			userMessageResponse,
			'request.response event for user message does not exist',
		);
		...
		assert.ok(
			userMessageEngine,
			'engine.messages event for user message does not exist',
		);
		// Check that the engine.messages event has a messagesJson property with length greater than or equal to message
		assert.ok(
			userMessageEngine.data.baseData.properties.messagesJson.length >=
				messageText.length,
		...
		assert.ok(
			userMessageEngineLength,
			'engine.messages.length event for user message does not exist',
		);
		...
		assert.ok(
			modelRequestAdded,
			'model.request.added event for user message does not exist',
		);
		...
		assert.ok(
			modelMessageAdded,
			'model.message.added event for user message does not exist',
		);
		...
		assert.ok(
			modelCallInput,
			'model.modelCall.input event for user message does not exist',
		);
		...
		assert.ok(
			modelRequestOptionsAdded,
			'model.request.options.added event for user message does not exist',
		);
	});
});
