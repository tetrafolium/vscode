...
} from '../../../../platform/authentication/common/staticGitHubAuthenticationService';
import {
	FailingDevContainerConfigurationService,
...
import {
	...
	NoopSettingsEditorSearchService,
} from '../../../../platform/settingsEditor/common/settingsEditorSearchService';
...
suite('Conversation feature test suite', function () {
	...
	function createAccessor() {
		...
		testingServiceCollection.define(
			IGitCommitMessageService,
			new NoopGitCommitMessageService(),
		);
		testingServiceCollection.define(
			IDevContainerConfigurationService,
		...
		testingServiceCollection.define(
			ISettingsEditorSearchService,
			new SyncDescriptor(NoopSettingsEditorSearchService),
		);
		testingServiceCollection.define(
			IMergeConflictService,
			new SyncDescriptor(TestMergeConflictServiceImpl),
		);
	...
	test.skip(`If the 'interactive' namespace is not available, the feature is not enabled and not activated`, function () {
		...
		const conversationFeature =
			instaService.createInstance(ConversationFeature);
		try {
	...
	test('The feature activates without a Copilot token when a non-copilot (BYOK) language model is available', async function () {
		...
		sandbox
			.stub(vscode.lm, 'onDidChangeChatModels')
			.returns({ dispose: () => {} });
	...
	test('activationBlocker resolves on an auth change even when the BYOK query never settles', async function () {
		...
		sandbox.stub(vscode.lm, 'selectChatModels').returns(
			new Promise<vscode.LanguageModelChat[]>(() => {
				/* never resolves */
			}),
		);
		sandbox
		...
		const conversationFeature =
			instaService.createInstance(ConversationFeature);
		try {
			const authService = accessor.get(
				IAuthenticationService,
			) as unknown as { fireAuthenticationChange(source: string): void };
