...
class ChatAgents implements IDisposable {
	...
	constructor(
		...
		private readonly promptCategorizerService: IPromptCategorizerService,
	...
	private createAgent(
		...
		this._disposables.add(
			autorun((reader) => {
				agent.supportIssueReporting =
					this.feedbackReporter.canReport.read(reader);
			}),
		);
	...
	private registerTerminalAgent(): IDisposable {
		const terminalAgent = this.createAgent(
			terminalAgentName,
			Intent.Terminal,
		);
	...
	private registerTerminalPanelAgent(): IDisposable {
		const terminalPanelAgent = this.createAgent(
			terminalAgentName,
			Intent.Terminal,
...
Learn more about [GitHub Copilot](https://docs.github.com/copilot/using-github-copilot/getting-started-with-github-copilot?tool=vscode&utm_source=editor&utm_medium=chat-panel&utm_campaign=2024q3-em-MSFT-getstarted) in [Visual Studio Code](https://code.visualstudio.com/docs/copilot/overview). Or explore the [Copilot walkthrough](command:github.copilot.open.walkthrough).`,
		...
		defaultAgent.titleProvider =
			this.instantiationService.createInstance(ChatTitleProvider);
		defaultAgent.summarizer = this.instantiationService.createInstance(
	...
	private getChatParticipantHandler(
		...
		return async (
			...
			markChatExt(
				request.sessionId,
				ChatExtPerfMark.WillHandleParticipant,
			);
			try {
				...
				const switchToAutoConfirmation =
					getSwitchToAutoOnRateLimitConfirmation(request);
				if (switchToAutoConfirmation) {
					const action = switchToAutoConfirmation.alwaysSwitchToAuto
						? 'switchToAutoAlways'
						: 'switchToAuto';
					/* __GDPR__
						"chatRateLimitAction" : {
					...
					*/
					this.telemetryService.sendMSFTTelemetryEvent(
						'chatRateLimitAction',
						{ action, modelId: request.model?.id },
					);
					request = await this.switchToAutoModel(
						request,
						stream,
						switchToAutoConfirmation.alwaysSwitchToAuto,
					);
				} else if (isContinueOnError(request)) {
					this.telemetryService.sendMSFTTelemetryEvent(
						'chatRateLimitAction',
						{ action: 'tryAgain', modelId: request.model?.id },
					);
				}
				...
				const telemetryMessageId =
					context.history.length === 0 ? generateUuid() : undefined;

				// Categorize the first prompt (fire-and-forget)
				if (telemetryMessageId !== undefined) {
					this.promptCategorizerService.categorizePrompt(
						request,
						context,
						telemetryMessageId,
					);
				...
				const defaultIntentId =
					typeof defaultIntentIdOrGetter === 'function'
				...
				const intentId =
					request.command && commandsForAgent
				...
				if (
					...
					const switchedRequest = await this.switchToAutoModel(
						request,
						stream,
						false,
					);
					if (switchedRequest.model?.id !== previousModelId) {
						this.telemetryService.sendMSFTTelemetryEvent(
							'chatRateLimitAction',
							{ action: 'autoSwitch', modelId: previousModelId },
						);
				...
				markChatExt(
					request.sessionId,
					ChatExtPerfMark.DidHandleParticipant,
				);
	...
	private async switchToBaseModel(
		...
		if (
			this._chatQuotaService.additionalUsageEnabled ||
		...
		if (this.authenticationService.copilotToken?.isIndividual) {
			messageString = new vscode.MarkdownString(
				vscode.l10n.t({
					message:
						'You have reached your additional budget limit for this month. We have automatically switched you to {0} which is included with your plan. [Manage Budget]({1}) to keep going.',
					args: [
						baseEndpoint.name,
						'command:chat.enableAdditionalUsage',
					],
			...
			messageString.isTrusted = {
				enabledCommands: ['chat.enableAdditionalUsage'],
			};
		} else {
			messageString = new vscode.MarkdownString(
				vscode.l10n.t(
					'You have reached your additional budget limit for this month. We have automatically switched you to {0} which is included with your plan. To manage budget, contact your organization admin.',
					baseEndpoint.name,
				),
			);
		}
	...
	private async switchToAutoModel(
		request: vscode.ChatRequest,
		stream: vscode.ChatResponseStream,
		alwaysSwitchToAuto: boolean,
	): Promise<ChatRequest> {
		const autoModel = (
			await vscode.lm.selectChatModels({ id: 'auto', vendor: 'copilot' })
		)[0];
		if (!autoModel) {
			return request;
		}
		await vscode.commands.executeCommand(
			'workbench.action.chat.changeModel',
			{
				vendor: autoModel.vendor,
				id: autoModel.id,
				family: autoModel.family,
			},
		);
