...
export function startFeedbackCollection(accessor: ServicesAccessor) {
	...
	const enabled = configurationService.getConfig(
	...
	disposables.add(
		vscode.workspace.onDidChangeTextDocument(async (event) => {
			if (
				...
				try {
					logService.warn(
						'Document changed, delaying diagnostics request',
					);
					...
					if (version !== event.document.version) {
						logService.warn(
							'Skipping diagnostics request because the document has changed',
						);
						return;
					}
					const now = Date.now();
					...
					if (requestTimes.length >= maxRequests) {
						logService.warn(
							'Max requests reached, skipping diagnostics request',
						);
						return;
					}
					...
					const change = await instantiationService.invokeFunction(
					...
					if (!change) {
						logService.trace(
							'No change found in the current document at the current position.',
						);
						return [];
					}
					...
					if (result.type === 'success') {
						const diagnostics = result.comments.map(
							(comment) =>
