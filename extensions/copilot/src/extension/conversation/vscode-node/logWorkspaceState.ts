...
} from '../../../platform/workspaceState/common/promptContextModel';
...
export class LogWorkspaceStateContribution
	extends Disposable
		...
		this._register(
			vscode.commands.registerCommand(
				...
				async () => {
					...
					const state = await instantiationService
					...
					serializedState = serializedState.replace(
						new RegExp(`${workspaceRoot.fsPath}/`, 'g'),
						'./',
					);
...
export class WorkspaceStateSnapshotHelper {
	...
	constructor(
		...
		private readonly tabAndEditorsService: ITabsAndEditorsService,
		@ILanguageDiagnosticsService
		private readonly languageDiagnosticService: ILanguageDiagnosticsService,
	...
	public async captureWorkspaceStateSnapshot(
		symbolQueries: string[],
	): Promise<ISerializedWorkspaceState> {
		const workspaceFoldersFilePaths = this.workspaceService
			.getWorkspaceFolders()
			.map((w) => w.fsPath + '/');
		const notebookDocumentFilePaths =
			this.workspaceService.notebookDocuments.map((d) => d.uri.fsPath);
		const symbols = (
			await Promise.all(
				symbolQueries.map((q) =>
					this.languageService.getWorkspaceSymbols(q),
				),
			)
		).flat();
		...
		const activeFileDiagnostics = !this.tabAndEditorsService
			.activeTextEditor
			? []
			: this.languageDiagnosticService
					.getDiagnostics(
						this.tabAndEditorsService.activeTextEditor.document.uri,
					)
					.map((d) => ({
						start: d.range.start,
						end: d.range.end,
						...
						relatedInformation: d.relatedInformation?.map(
							serializeRelatedInformation,
						),
					}));
		const activeTextEditor = this.tabAndEditorsService.activeTextEditor
			? {
					selections:
						this.tabAndEditorsService.activeTextEditor?.selections.map(
							(s) => ({
								anchor: s.anchor,
								active: s.active,
					...
					documentFilePath:
						this.tabAndEditorsService.activeTextEditor?.document.uri
							.fsPath ?? '',
					visibleRanges:
						this.tabAndEditorsService.activeTextEditor?.visibleRanges.map(
							(r) => ({
								start: r.start,
								end: r.end,
							}),
		...
		const terminalLastCommand = this.terminalService.terminalLastCommand
			? {
					commandLine:
						this.terminalService.terminalLastCommand.commandLine,
					cwd:
						typeof this.terminalService.terminalLastCommand.cwd ===
						'object'
							? this.terminalService.terminalLastCommand.cwd.toString()
							: this.terminalService.terminalLastCommand.cwd,
		...
		const workspaceState: ISerializedWorkspaceState = {
			...
			textDocumentFilePaths: coalesce(
				this.workspaceService.textDocuments.map((doc) => {
					const parentFolder =
						this.workspaceService.getWorkspaceFolder(doc.uri);
					return parentFolder
						? relativePath(parentFolder, doc.uri)
						: undefined;
				}),
			),
...
function serializeRelatedInformation(
	r: vscode.DiagnosticRelatedInformation,
): ISerializedDiagnosticRelatedInformation {
	return {
