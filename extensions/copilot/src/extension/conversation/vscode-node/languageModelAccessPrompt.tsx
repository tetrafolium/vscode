...
} from '../../../platform/endpoint/common/statefulMarkerContainer';
	...
	async render() {
		...
		for (const message of this.props.messages) {
			if (message.role === vscode.LanguageModelChatMessageRole.System) {
				...
				const statefulMarkerPart = message.content.find(
					(part) =>
				...
				const statefulMarker =
					statefulMarkerPart &&
					decodeStatefulMarker(statefulMarkerPart.data);
				const filteredContent = message.content.filter(
					(part) => !(part instanceof vscode.LanguageModelDataPart),
				);
				// There should only be one string part per message
				const content = filteredContent.find(
					(part) => part instanceof LanguageModelTextPart,
				);
				const toolCalls = filteredContent.filter(
					(part) => part instanceof vscode.LanguageModelToolCallPart,
				);
				const thinking = filteredContent.find(
					(part) => part instanceof vscode.LanguageModelThinkingPart,
				);

				const statefulMarkerElement = statefulMarker && (
					<StatefulMarkerContainer statefulMarker={statefulMarker} />
				);
				const thinkingElement = thinking && thinking.id && (
