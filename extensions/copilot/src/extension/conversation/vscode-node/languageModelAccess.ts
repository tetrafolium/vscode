...
import {
	retrieveCapturingTokenByCorrelation,
	runWithCapturingToken,
} from '../../../platform/requestLogger/node/requestLogger';
...
import {
	ChatLocation as ApiChatLocation,
	ExtensionMode,
} from '../../../vscodeTypes';
...
): { value: number; description: string; isDefault: boolean }[] | undefined {
...
function buildConfigurationSchema(endpoint: IChatEndpoint): {
	...
	const properties: Record<
		string,
		NonNullable<
			vscode.LanguageModelConfigurationSchema['properties']
		>[string]
	> = {};
	...
	if (effortLevels && effortLevels.length > 1) {
		properties.reasoningEffort = buildReasoningEffortSchemaProperty(
			effortLevels,
			endpoint.family.toLowerCase(),
		);
	}
	...
	if (contextSizeOptions) {
		...
		properties.contextSize = {
			...
			enumItemLabels: contextSizeOptions.map((o) =>
				formatTokenCount(o.value),
			),
...
const utilityAliasFamilies: readonly ChatEndpointFamily[] = [
 ...
 */
function isDefaultEndpointForUtilityFamily(
	...
	switch (family) {
		case 'copilot-utility-small':
			return (
				endpoint.family === CopilotUtilitySmallChatEndpoint.capiFamily
			);
...
export function buildUtilityAliasModelInfo(
	...
	const base =
		endpoint instanceof CopilotChatEndpoint
			? models.find((m) => m.id === endpoint.model)
			: undefined;
	if (base) {
		return {
			synthesized: false,
	...
	implements IExtensionContribution
{
	...
	private readonly _resolvedUtilityEndpoints = new Map<
		ChatEndpointFamily,
	...
	constructor(
		...
		this._lmWrapper = this._instantiationService.createInstance(
			CopilotLanguageModelWrapper,
		);
		this._promptBaseCountCache = this._instantiationService.createInstance(
			LanguageModelAccessPromptBaseCountCache,
		);

		if (
			this._vsCodeExtensionContext.extensionMode === ExtensionMode.Test &&
			!isScenarioAutomation
		) {
			this._logService.warn(
				'[LanguageModelAccess] LanguageModels and Embeddings are NOT AVAILABLE in test mode.',
			);
			return;
		}
	...
	private async _registerChatProvider(): Promise<void> {
		const provider: vscode.LanguageModelChatProvider = {
			...
			provideLanguageModelChatInformation:
				this._provideLanguageModelChatInfo.bind(this),
			provideLanguageModelChatResponse:
				this._provideLanguageModelChatResponse.bind(this),
			provideTokenCount: this._provideTokenCount.bind(this),
		};
		this._register(
			vscode.lm.registerLanguageModelChatProvider('copilot', provider),
		);
		this._register(
		...
		this._register(
			this._endpointProvider.onDidModelsRefresh(() => {
	...
	private async _provideLanguageModelChatInfo(
		...
		const chatEndpoints = allEndpoints.filter(
			(e) => e.showInModelPicker || e.model === 'gpt-4o-mini',
		);
		const autoEndpoint =
			await this._automodeService.resolveAutoModeEndpoint(
				undefined,
				allEndpoints,
			);
		chatEndpoints.push(autoEndpoint);
		let defaultChatEndpoint: IChatEndpoint;
		const defaultExpModel = this._expService
			.getTreatmentVariable<string>('chat.defaultLanguageModel')
			?.replace('copilot/', '');
		if (
			this._authenticationService.copilotToken?.isNoAuthUser ||
			!defaultExpModel ||
			defaultExpModel === AutoChatEndpoint.pseudoModelId
		) {
			...
			// Find exp default
			defaultChatEndpoint =
				chatEndpoints.find((e) => e.model === defaultExpModel) ||
				autoEndpoint;
		}
		...
		for (const endpoint of chatEndpoints) {
			if (
				seenFamilies.has(endpoint.family) &&
				!endpoint.showInModelPicker
			) {
				continue;
			}
			...
			if (endpoint.degradationReason) {
				...
				const isOrgManaged =
					!!this._authenticationService.copilotToken?.isManagedPlan;
				const autoModeHint =
					this._expService.getTreatmentVariable<string>(
						'copilotchat.autoModelHint',
					);
				const showExperimentalHint =
					!isOrgManaged &&
					!!autoModeHint &&
					experimentalAutoModelHintMarkers.some((marker) =>
						autoModeHint.includes(marker),
					);
			...
			const baseCount =
				await this._promptBaseCountCache.getBaseCount(endpoint);
			const multiplier =
				endpoint.multiplier !== undefined
					? `${endpoint.multiplier}x`
					: undefined;
			let modelDetail: string | undefined;

			if (endpoint instanceof AutoChatEndpoint) {
				if (
					endpoint.discountRange.high ===
						endpoint.discountRange.low &&
					endpoint.discountRange.low !== 0
				) {
					modelDetail = `${endpoint.discountRange.low * 100}% discount`;
				} else if (
					endpoint.discountRange.high !== endpoint.discountRange.low
				) {
			...
			if (endpoint.customModel) {
				...
				modelTooltip = vscode.l10n.t(
					'{0} is contributed by {1} using {2}.',
					sanitizedModelName,
					customModel.owner_name,
					customModel.key_name,
				);
			}
			...
			const model: vscode.LanguageModelChatInformation = {
				id:
					endpoint instanceof AutoChatEndpoint
						? AutoChatEndpoint.pseudoModelId
						: endpoint.model,
				name:
					endpoint instanceof AutoChatEndpoint
						? 'Auto'
				...
				pricing:
					endpoint instanceof AutoChatEndpoint
						? undefined
						: (multiplier ??
							(endpoint.tokenPricing
								? formatPricingLabel(endpoint.tokenPricing)
								: undefined)),
				inputCost:
					endpoint instanceof AutoChatEndpoint
						? undefined
						: endpoint.tokenPricing?.default.inputPrice,
				outputCost:
					endpoint instanceof AutoChatEndpoint
						? undefined
						: endpoint.tokenPricing?.default.outputPrice,
				cacheCost:
					endpoint instanceof AutoChatEndpoint
						? undefined
						: endpoint.tokenPricing?.default.cacheReadTokenPrice,
				longContextInputCost:
					endpoint instanceof AutoChatEndpoint
						? undefined
						: endpoint.tokenPricing?.longContext?.inputPrice,
				longContextOutputCost:
					endpoint instanceof AutoChatEndpoint
						? undefined
						: endpoint.tokenPricing?.longContext?.outputPrice,
				longContextCacheCost:
					endpoint instanceof AutoChatEndpoint
						? undefined
						: endpoint.tokenPricing?.longContext
								?.cacheReadTokenPrice,
				multiplierNumeric:
					endpoint instanceof AutoChatEndpoint
						? undefined
				...
				detail: modelDetail,
				statusIcon: endpoint.degradationReason
				...
				isDefault: {
					...
					[ApiChatLocation.Editor]:
						endpoint instanceof AutoChatEndpoint, // inline chat gets 'Auto' by default
				},
	...
	private _registerUtilityAliasModels(
		...
		const session = this._authenticationService.anyGitHubSession;
		const requiresAuthorization = session
			? { label: session.account.label }
			: undefined;
		...
		for (const family of utilityAliasFamilies) {
			...
			const endpoint =
				cached?.endpoint ??
				allEndpoints.find((e) =>
					isDefaultEndpointForUtilityFamily(family, e),
				);
			if (!endpoint) {
				continue;
			}
			...
			try {
				...
				const aliasInfo = buildUtilityAliasModelInfo(
					family,
					endpoint,
					models,
					cached?.baseCount ?? 0,
				...
				this._logService.trace(
					`[LanguageModelAccess] Publishing alias '${family}' -> ${endpoint.model} (${aliasInfo.synthesized ? 'synthesized' : 'cloned'}, ${endpoint instanceof CopilotChatEndpoint ? 'copilot' : 'override'}).`,
				);
				models.push(aliasInfo.info);
			} catch (err) {
				this._logService.warn(
					`[LanguageModelAccess] Failed to publish utility alias '${family}' -> ${endpoint.model}; skipping. Error: ${err}`,
				);
			}
		}
		...
		void this._refreshUtilityOverrides().catch((err) => {
			this._logService.warn(
				`[LanguageModelAccess] Failed to refresh utility overrides: ${err}`,
			);
		});
	}
	...
	private async _refreshUtilityOverrides(): Promise<void> {
		...
		for (const family of utilityAliasFamilies) {
			...
			try {
				...
				this._logService.warn(
					`[LanguageModelAccess] Failed to resolve utility alias '${family}' in background: ${err}`,
				);
				continue;
			}
			if (!resolved) {
			...
			if (
				published &&
				published.model === resolved.model &&
				published.modelProvider === resolved.modelProvider
			) {
				continue;
			}
			...
			try {
				baseCount =
					await this._promptBaseCountCache.getBaseCount(resolved);
			} catch (err) {
				this._logService.warn(
					`[LanguageModelAccess] Failed to compute baseCount for utility alias '${family}' -> ${resolved.model}; keeping previously-published alias. Error: ${err}`,
				);
				continue;
			}
			this._resolvedUtilityEndpoints.set(family, {
				endpoint: resolved,
				baseCount,
			});
			didChange = true;
	...
	private async _getEndpointForModel(
		model: vscode.LanguageModelChatInformation,
	) {
		if (model.id === AutoChatEndpoint.pseudoModelId) {
			const allEndpoints =
				await this._endpointProvider.getAllChatEndpoints();
			return await this._automodeService.resolveAutoModeEndpoint(
				undefined,
				allEndpoints,
			);
		}
	...
	private async _provideLanguageModelChatResponse(
		...
		if (
			...
			contextSize < endpoint.modelMaxPromptTokens
		) {
	...
	private async _registerEmbeddings(): Promise<void> {
		...
		const update = async () => {
			...
			if (!model) {
				throw new Error(
					`No model found for embedding type ${embeddingType.id}`,
				);
			}

			dispo.clear();
			dispo.value = vscode.lm.registerEmbeddingsProvider(
				`copilot.${model}`,
				new (class implements vscode.EmbeddingsProvider {
					async provideEmbeddings(
						input: string[],
						token: vscode.CancellationToken,
					): Promise<vscode.Embedding[]> {
						const result =
							await embeddingsComputer.computeEmbeddings(
								embeddingType,
								input,
								{},
								new TelemetryCorrelationId(
									'EmbeddingsProvider::provideEmbeddings',
								),
								token,
							);
						return result.values.map((embedding) => ({
							values: embedding.value.slice(0),
						}));
					}
				})(),
			);
	...
	private async _getToken(): Promise<CopilotToken | undefined> {
		if (!this._authenticationService.hasCopilotTokenSource) {
			this._logService.warn(
				'[LanguageModelAccess] LanguageModel/Embeddings are not available without a Copilot token source',
			);
			return undefined;
		}
...
class LanguageModelAccessPromptBaseCountCache {
	...
	public async getBaseCount(endpoint: IChatEndpoint): Promise<number> {
		...
		if (
			cached &&
			cached.extensionVersion === this._envService.getVersion() &&
			typeof cached.baseCount === 'number'
		) {
		...
		try {
			await this._extensionContext.globalState.update(key, {
	...
	private async _computeBaseCount(endpoint: IChatEndpoint): Promise<number> {
		const baseCount = await PromptRenderer.create(
			this._instantiationService,
			endpoint,
			LanguageModelAccessPrompt,
			{ noSafety: false, messages: [] },
		).countTokens();
		return baseCount;
	}
}
...
export class CopilotLanguageModelWrapper extends Disposable {
	constructor(
		...
		private readonly _blockedExtensionService: IBlockedExtensionService,
		@IInstantiationService
		private readonly _instantiationService: IInstantiationService,
		@ILogService private readonly _logService: ILogService,
		@IAuthenticationService
	...
	private async _provideLanguageModelResponse(
		...
		const extensionInfo = !extensionId
			? { packageJSON: { version: this._envService.vscodeVersion } }
			: vscode.extensions.getExtension(extensionId, true);
		if (
			!extensionInfo ||
			typeof extensionInfo.packageJSON.version !== 'string'
		) {
			throw new Error('Invalid extension information');
		}
		const extensionVersion = <string>extensionInfo.packageJSON.version;

		const blockedExtensionMessage = vscode.l10n.t(
			'The extension has been temporarily blocked due to making too many requests. Please try again later.',
		);
		if (
			extensionId &&
			this._blockedExtensionService.isExtensionBlocked(extensionId)
		) {
			throw vscode.LanguageModelError.Blocked(blockedExtensionMessage);
		}

		const toolTokenCount = _options.tools
			? await this.countToolTokens(_endpoint, _options.tools)
			: 0;
		const baseCount = await PromptRenderer.create(
			this._instantiationService,
			_endpoint,
		...
		const tokenLimit =
			_endpoint.modelMaxPromptTokens -
			baseCount -
			BaseTokensPerCompletion -
			toolTokenCount;
		...
		const { messages, tokenCount } = await PromptRenderer.create(
			...
			LanguageModelAccessPrompt,
			{
				noSafety: extensionId === this._envService.extensionId,
				messages: _messages,
			},
		).render();
		...
		*/
		...
		if (
			_options.tools &&
			_options.tools.length > 128 &&
			!_endpoint.supportsToolSearch
		) {
		...
		const options: OptionalChatRequestParams =
			LanguageModelOptions.Default.convert(_options.modelOptions ?? {});
		...
		if (
			_options.toolMode === vscode.LanguageModelChatToolMode.Required &&
			_options.tools?.length &&
			_options.tools.length > 1
		) {
			throw new Error(
		...
		options.tool_choice =
			...
			_options.tools?.length
				? {
						type: 'function',
						function: { name: _options.tools[0].name },
					}
				: undefined;
		...
		const capturingToken = correlationId
			? retrieveCapturingTokenByCorrelation(correlationId)
			: undefined;
		...
		const parentTraceContext =
			(_options as { modelOptions?: OTelModelOptions }).modelOptions
				?._otelTraceContext ?? undefined;
			...
			endpoint.makeChatRequest2(
				{
					...
					modelCapabilities: {
						reasoningEffort:
							typeof _options.modelConfiguration
								?.reasoningEffort === 'string'
								? _options.modelConfiguration.reasoningEffort
								: undefined,
					},
				},
				token,
		...
		if (result.type !== ChatFetchResponseType.Success) {
			if (result.type === ChatFetchResponseType.ExtensionBlocked) {
				if (extensionId) {
					this._blockedExtensionService.reportBlockedExtension(
						extensionId,
						result.retryAfter,
					);
				}
				...
				throw vscode.LanguageModelError.Blocked(
					blockedExtensionMessage,
				);
			} else if (result.type === ChatFetchResponseType.QuotaExceeded) {
				const outageStatus =
					await this._octoKitService.getGitHubOutageStatus();
				const details = getErrorDetailsFromChatFetchError(
					result,
					this._authenticationService.copilotToken?.copilotPlan,
					outageStatus,
					this._authenticationService.copilotToken?.tokenBasedBilling,
					this._authenticationService.copilotToken?.quotaInfo
						.quota_reset_date,
				);
	...
	async provideLanguageModelResponse(
		...
		const finishCallback: FinishedCallback = async (
			_text,
			index,
			delta,
		): Promise<undefined> => {
				...
				for (const call of delta.copilotToolCalls) {
					try {
						...
						this._logService.error(
							err,
							`Got invalid JSON for tool call: ${call.arguments}`,
						);
		...
		const usage = await this._provideLanguageModelResponse(
			endpoint,
			messages,
			options,
			extensionId,
	...
	async provideTokenCount(
		...
		if (typeof message === 'string') {
			...
			const content = message.content
				.map((part): Raw.ChatCompletionContentPart | undefined => {
					if (part instanceof vscode.LanguageModelTextPart) {
						...
						return {
							...
							imageUrl: {
								url: `data:${part.mimeType};base64,${Buffer.from(part.data).toString('base64url')}`,
							},
						};
					} else {
						return undefined;
				...
				case vscode.LanguageModelChatMessageRole.Assistant:
					raw = {
						...
						toolCalls: message.content
							...
							.map(
								(part) =>
									part as vscode.LanguageModelToolCallPart,
							)
							.map((part) => ({
								function: {
									name: part.name,
...
class LanguageModelOptions {
	...
	constructor(
		private _description: Record<string, (value: unknown) => boolean>,
	) {}
