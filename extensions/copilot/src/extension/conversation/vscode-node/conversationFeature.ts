...
import {
	DevContainerConfigGeneratorArguments,
...
import {
	generateTerminalFixes,
	setLastCommandMatchResult,
} from './terminalFixGenerator';
...
export class ConversationFeature implements IExtensionContribution {
	...
	constructor(
		...
		private readonly embeddingIndex: ICombinedEmbeddingIndex,
		@IDevContainerConfigurationService
		private readonly devContainerConfigurationService: IDevContainerConfigurationService,
		@IGitCommitMessageService
		private readonly gitCommitMessageService: IGitCommitMessageService,
		@IMergeConflictService
		private readonly mergeConflictService: IMergeConflictService,
		...
		private readonly settingsEditorSearchService: ISettingsEditorSearchService,
	) {
		this._enabled = false;
		...
		const reevaluate = () => {
			...
			if (hasToken) {
				this.logService.info(
					`copilot token sku: ${authenticationService.copilotToken?.sku ?? ''}`,
				);
			}
			...
			if (shouldActivate && !activationBlockerDeferred.isSettled) {
				if (hasToken) {
					markChatExtGlobal(
						ChatExtGlobalPerfMark.DidWaitForCopilotToken,
					);
				}
		...
		if (authenticationService.copilotToken) {
			this.logService.info(
				`ConversationFeature: Copilot token already available`,
			);
		} else {
			markChatExtGlobal(ChatExtGlobalPerfMark.WillWaitForCopilotToken);
			this.logService.info(
				`ConversationFeature: Waiting for copilot token or BYOK model to activate conversation feature`,
			);
		}
		...
		const refreshHasByokModels = async () => {
			try {
				...
				if (value !== hasByokModels) {
					hasByokModels = value;
					this.logService.info(
						`ConversationFeature: BYOK models ${value ? 'available' : 'unavailable'}`,
					);
					reevaluate();
				}
			} catch (e) {
				this.logService.warn(
					`ConversationFeature: failed to query language models: ${e}`,
				);
			}
		};
		void refreshHasByokModels();
		this._disposables.add(
			vscode.lm.onDidChangeChatModels(() => void refreshHasByokModels()),
		);
	...
	set enabled(value: boolean) {
		...
		vscode.commands.executeCommand(
			'setContext',
			'github.copilot.interactiveSession.disabled',
			!value,
		);
	}
	...
	set activated(value: boolean) {
		...
		if (!value) {
			this.logService.info(
				'ConversationFeature: Deactivating contributions',
			);
			this._activatedDisposables.clear();
		} else {
			this.logService.info(
				'ConversationFeature: Activating contributions',
			);
			...
			this._activatedDisposables.add(
	...
	private registerSearchProvider(): IDisposable | undefined {
		if (this._searchProviderRegistered) {
			...
			if (
				...
				this.authenticationService.copilotToken?.isNoAuthUser
			) {
				this.logService.debug(
					'ConversationFeature: Skipping search provider registration - no GitHub session available',
				);
				return;
			}

			const provider = this.instantiationService.createInstance(
	...
	private registerSettingsSearchProvider(): IDisposable | undefined {
		...
		if (
			...
			this.logService.debug(
				'ConversationFeature: Skipping settings search provider registration - no GitHub session available',
			);
			return;
		}

		return vscode.ai.registerSettingsSearchProvider(
			this.settingsEditorSearchService,
		);
	}
	...
	private registerProviders(): IDisposable {
		...
		try {
			const detectionProvider =
				this.registerParticipantDetectionProvider();
			...
			const settingsSearchDisposable =
			...
			this.logService.error(
				err,
				'Registration of interactive providers failed',
			);
		}
		return disposables;
	}
	...
	private registerParticipantDetectionProvider() {
		...
		if ('registerChatParticipantDetectionProvider' in vscode.chat) {
			const provider =
				this.instantiationService.createInstance(IntentDetector);
			return vscode.chat.registerChatParticipantDetectionProvider(
				provider,
			);
		}
	}
	...
	private registerCommands(options: IConversationOptions): IDisposable {
		...
		[
			vscode.commands.registerCommand(
				...
				async () => {
					return vscode.env.openExternal(
						vscode.Uri.parse(FEEDBACK_URL),
					);
				},
			),
			vscode.commands.registerCommand('github.copilot.chat.compact', () =>
			...
			vscode.commands.registerCommand(
				...
				async () =>
					this.triggerTerminalChat({
						query: `/${TerminalExplainIntent.intentName} #terminalLastCommand`,
					}),
			),
			vscode.commands.registerCommand(
				'github.copilot.terminal.fixTerminalLastCommand',
				async () => generateTerminalFixes(this.instantiationService),
			),
			vscode.commands.registerCommand(
				...
				async () => {
					...
					const repository =
						await this.gitCommitMessageService.getRepository(uri);
					if (!repository) {
						return;
					}

					const commitMessage =
						await this.gitCommitMessageService.generateCommitMessage(
							repository,
					...
					if (commitMessage) {
						...
						const sanitizedMessage = commitMessage
							.replace(/"/g, '\\"')
							.replace(/\\/g, '\\\\')
							.replace(/\$/g, '\\$'); // CodeQL [SM02383] Backslashes are escaped as part of the second replace.
			...
			vscode.commands.registerCommand(
				...
				async (
					...
					const repository =
						await this.gitCommitMessageService.getRepository(
							rootUri,
						);
					if (!repository) {
						return;
			...
			vscode.commands.registerCommand(
				...
				async (
					...
					const resources = resourceStates
						.filter((r) => !!r)
						.map((r) => (isUri(r) ? r : r.resourceUri));
					await this.mergeConflictService.resolveMergeConflicts(
						resources,
						undefined,
					);
				},
			...
			vscode.commands.registerCommand(
				...
				async (
					...
					if (cancellationToken) {
						return this.devContainerConfigurationService.generateConfiguration(
							args,
							cancellationToken,
						);
					}
			...
			this.linkifyService.registerGlobalLinkifier({
				create: () =>
					this.instantiationService.createInstance(SymbolLinkifier),
			}),
			this.linkifyService.registerGlobalLinkifier({
				create: () =>
					disposables.add(
			...
			registerNewWorkspaceIntentCommand(
				this.newWorkspacePreviewContentManager,
				this.logService,
				options,
			),
			registerGitHubPullRequestTitleAndDescriptionProvider(
	...
	private async triggerTerminalChat(options: {
		query: string;
		isPartialQuery?: boolean;
	}) {
		const chatLocation = this.configurationService.getConfig(
			ConfigKey.TerminalChatLocation,
		);
	...
	private registerTerminalQuickFixProviders() {
		...
		return combinedDisposable(
			vscode.window.registerTerminalQuickFixProvider(
				'copilot-chat.fixWithCopilot',
				{
					provideTerminalQuickFixes(commandMatchResult, token) {
						if (
							!isEnabled() ||
							commandMatchResult.commandLine.endsWith('^C')
						) {
							return [];
						}
			...
			vscode.window.registerTerminalQuickFixProvider(
				'copilot-chat.generateCommitMessage',
				{
					provideTerminalQuickFixes: (commandMatchResult, token) => {
						return this.enabled
							? [
...
function registerSearchIntentCommand(): IDisposable {
	return vscode.commands.registerCommand(
		...
		async (arg: FindInFilesArgs) => {
			const show =
				arg.filesToExclude.length > 0 || arg.filesToInclude.length > 0;
			vscode.commands
...
function registerGitHubPullRequestTitleAndDescriptionProvider(
