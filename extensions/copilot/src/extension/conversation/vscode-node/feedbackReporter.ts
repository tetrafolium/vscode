...
} from '../../workspaceSemanticSearch/node/semanticSearchTextSearchProvider';
...
export class FeedbackReporter extends Disposable implements IFeedbackReporter {
	...
	constructor(
		...
		this.canReport = this._configurationService.getConfigObservable(
			ConfigKey.TeamInternal.DebugReportFeedback,
		);
	}

	private _findChatParamsForTurn(
		turn: Turn,
	): ILoggedPendingRequest | undefined {
		for (const request of this._requestLogger.getRequests().reverse()) {
			if (request.kind !== LoggedInfoKind.Request) {
				continue;
			}
			if (
				request.entry.type === LoggedRequestKind.MarkdownContentRequest
			) {
				continue;
			}
	...
	async reportInline(
		...
		const intentDump = promptQuery.intent
			? this._embedCodeblock('INTENT', promptQuery.intent.id)
			: '';
		const contextDump = this._embedCodeblock(
			'CONTEXT',
			JSON.stringify(
				{
		...
		if (latestMessages && latestMessages.length > 0) {
			const messagesInfo = latestMessages
				.map((message) =>
					this._embedCodeblock(
						roleToString(message.role).toUpperCase(),
			...
			messagesDump = this._embedCodeblock(
				turn.request.type.toUpperCase(),
				turn.request.message,
			);
		}

		const responseDump = this._embedCodeblock(
			'ASSISTANT',
			turn.responseMessage?.message || '',
		);
		const parsedReplyDump = this._embedCodeblock(
			'Interaction outcome',
			JSON.stringify(interactionOutcome, null, '\t'),
		);

		const output: string[] = [];
		appendPromptDetailsSection(
			output,
			intentDump,
			contextDump,
			messagesDump,
	...
	async reportChat(turn: Turn): Promise<void> {
		...
		if (params?.messages && params.messages.length > 0) {
			const messagesInfo = params.messages
				.map((message) => {
					...
					if (
						message.content.some(
							(part) =>
								part.type ===
								Raw.ChatCompletionContentPartKind
									.CacheBreakpoint,
						)
					) {
					...
					if (
						message.role === Raw.ChatRole.Assistant &&
		...
		const workspaceState = await this._instantiationService
			.createInstance(WorkspaceStateSnapshotHelper)
			.captureWorkspaceStateSnapshot([]);
		const workspaceStateDump = this._embedCodeblock(
			'WORKSPACE STATE',
			JSON.stringify(workspaceState, null, 2),
		);
		const toolsDump = params?.body?.tools
			? this._embedCodeblock(
					'TOOLS',
					JSON.stringify(params.body.tools, null, 2),
				)
			: '';
		const metadata = this._embedCodeblock(
			'METADATA',
			`requestID: ${turn.id}\nmodel: ${params?.model}`,
		);
		const edits =
			(await this._editLogService.getEditLog(turn.id))
				?.map((edit, i) => {
					return this._embedCodeblock(
						`EDIT ${i + 1}`,
...
export async function openIssueReporter(args: {
	title: string;
	issueBody?: string;
	data: string;
	public?: boolean;
	...
	await vscode.commands.executeCommand('workbench.action.openIssueReporter', {
		...
		uri: vscode.Uri.parse(
			args.public
				...
				: 'https://github.com/microsoft/vscode-copilot-issues',
		),
	});
}

function appendPromptDetailsSection(
	output: string[],
	...dumps: string[]
): void {
	output.push(
...
export function generateInlineChatSTest(turn: Turn): string[] | undefined {
	...
	return [
		...
		`			fileContents: [`,
		...splitLinesIncludeSeparators(requestInfo.initialDocumentText).map(
			(line) => `				${str(line)},`,
		),
		`			]`,
		`		})],`,
