...
export class RemoteAgentContribution implements IDisposable {
	...
	constructor(
		...
		private readonly githubRepositoryService: IGithubRepositoryService,
		@IVSCodeExtensionContext
		private readonly vscodeExtensionContext: IVSCodeExtensionContext,
		@IAuthenticationService
		private readonly authenticationService: IAuthenticationService,
		@IUserFeedbackService
		private readonly userFeedbackService: IUserFeedbackService,
		...
		this.disposables.add(
			new Disposable(() =>
				agentRegistrations.forEach((agent) => agent.dispose()),
			),
		);
		...
		this.disposables.add(
			this.authenticationService.onDidAccessTokenChange(() => {
				this.refreshRemoteAgents();
			}),
		);
	}
	...
	private async refreshRemoteAgents(): Promise<void> {
		...
		return this.refreshRemoteAgentsP.finally(
			() => (this.refreshRemoteAgentsP = undefined),
		);
	}
	...
	private async _doRefreshRemoteAgents(): Promise<void> {
		...
		try {
			const authToken =
				this.authenticationService.anyGitHubSession?.accessToken;
			if (!authToken) {
				// We have to silently wait for auth to become available so we can fetch remote agents
				this.logService.warn(
					'Unable to fetch remote agents because user is not signed in.',
				);
				return;
			}
			try {
				// First try to register the default platform agent
				if (!existingAgents.delete(GITHUB_PLATFORM_AGENT_ID)) {
					// Don't reregister it
					this.logService.info(
						'Registering default platform agent...',
					);
					agentRegistrations.set(
						GITHUB_PLATFORM_AGENT_ID,
						this.registerAgent(null),
					);
				}
			} catch (ex) {
				this.logService.info(
					`Encountered error while registering platform agent: ${JSON.stringify(ex)}`,
				);
			}
			...
			try {
				...
				if (!text.includes('access denied')) {
					this.logService.warn(
						`Invalid remote agent response: ${text} (${e})`,
					);
				}
				return;
			}
	...
	private checkAuthorized(agent: string) {
		...
		return (
			this.vscodeExtensionContext.globalState.get<boolean>(key, false) ||
			this.vscodeExtensionContext.workspaceState.get<boolean>(key, false)
		);
	}

	private async setAuthorized(agent: string, isGlobal = false) {
		const memento = isGlobal
			? this.vscodeExtensionContext.globalState
			: this.vscodeExtensionContext.workspaceState;
	...
	private registerAgent(agentData: IAgent | null): Disposable {
		...
		const description =
			...
			l10n.t('Get answers grounded in web search and code search');
		...
		const agent = store.add(
			chat.createDynamicChatParticipant(
				participantId,
				dynamicProps,
				async (
					request,
							...
							if (data?.url) {
								...
								await this.setAuthorized(
									...
									request.prompt.startsWith(
										l10n.t('Authorize for all workspaces'),
									),
								);
								...
								responseStream.markdown(
									l10n.t(
										'Please complete authorization in your browser and resend your question.',
									),
								);
								return {
									metadata,
								} satisfies ICopilotChatResult;
							} else if (
								data?.hasAcknowledgedImplicitReferences
							) {
								// Store that the user has acknowledged implicit references
								await this.setAuthorized(
									slug,
									request.prompt.startsWith(
										l10n.t('Allow for All Workspaces'),
									),
								);
								responseStream.markdown(
									l10n.t('Your preference has been saved.'),
								);
								return {
									metadata,
								} satisfies ICopilotChatResult;
								...
								request =
									await this.authenticationChatUpgradeService.handleConfirmationRequest(
										responseStream,
										request,
										context.history,
									);
								...
								if (!accessToken) {
									responseStream.markdown(
										l10n.t(
											'The additional permissions are required for this feature.',
										),
									);
									return {
										metadata,
					...
					if (!agentData?.slug) {
						...
						if (!accessToken) {
							if (this.authenticationService.isMinimalMode) {
								responseStream.markdown(
									l10n.t(
										'Minimal mode is enabled. You will need to change `github.copilot.advanced.authPermissions` to `default` to use this feature.',
									),
								);
								responseStream.button({
									...
									arguments: [
										{
											revealSetting: {
												key: 'github.copilot.advanced.authPermissions',
											},
										},
									],
								});
					...
					if (!accessToken) {
						accessToken =
							this.authenticationService.anyGitHubSession
								?.accessToken;
					}
					...
					try {
						...
						const endpoint =
							this.instantiationService.createInstance(
								RemoteAgentChatEndpoint,
								{
									...
									billing:
										selectedEndpoint.isPremium !==
											undefined ||
										selectedEndpoint.multiplier !==
											undefined
											? {
													is_premium:
														selectedEndpoint.isPremium,
													multiplier:
														selectedEndpoint.multiplier,
													restricted_to:
														selectedEndpoint.restrictedToSkus,
												}
											: undefined,
									...
									capabilities: {
										supports: {
											tool_calls:
												selectedEndpoint.supportsToolCalls,
											vision: selectedEndpoint.supportsVision,
											streaming: true,
										},
										type: 'chat',
						...
						const resolved =
							await this.promptVariablesService.resolveVariablesInPrompt(
								request.prompt,
								request.references,
							);
						...
						const { copilot_skills } =
							await this.resolveCopilotSkills(slug, request);

						let hasIgnoredFiles = false;
						try {
							const result =
								await this.prepareClientPlatformReferences(
									[...request.references],
									slug,
								);
							...
							if (
								...
								responseStream.markdown(
									l10n.t(
										'Sorry, binary files are not currently supported.',
									),
								);
								return {
									metadata,
								...
								),
								{
									...
									content:
										request.acceptedConfirmationData
											?.length ||
										request.rejectedConfirmationData?.length
											? []
											: [
													{
														type: Raw
							...
							async (result, _, delta) => {
								if (delta.copilotReferences) {
									const processReference = (
										reference: ICopilotReference,
										parentReference?: ICopilotReference,
									) => {
										const url =
											'url' in reference
												? reference.url
												: 'url' in reference.data
													? reference.data.url
													: 'html_url' in
														  reference.data
														? reference.data
										...
										if (url && typeof url === 'string') {
											if (!reportedReferences.has(url)) {
												let icon: ChatResponseReferencePart['iconPath'] =
													undefined;
											...
											const icon = reference.metadata
												...
												: new ThemeIcon('globe');
											const value = reference.metadata
												.display_url
												? Uri.parse(
														reference.metadata
															.display_url,
											...
											if (reportedProgress) {
												reportedProgress.report(
													new ChatResponseReferencePart2(
														value,
														icon,
													),
												);
											} else {
												responseStream.reference2(
													value,
													icon,
												);
											}
									...
									for (const reference of delta.copilotReferences) {
										if (
											...
											reference.data.results.forEach(
												(r) => {
													processReference(
														r,
														reference,
											...
											reference.type === 'github.text'
										) {
											continue;
										} else if (
											'html_url' in reference.data ||
											('url' in reference.data &&
												typeof reference.data.url ===
													'string') ||
											reference.metadata
										) {
								...
								const reportProgress = (
									...
									pendingProgress?.deferred.complete(
										pendingProgress.resolvedMessage,
									);
													...
													responseStream.progress(
														...
														async (progress) =>
															reportProgress(
																progress,
																l10n.t(
																	'Code search results for "{0}" in {1}',
																	data.query,
																	data.scopingQuery,
																),
															),
								...
								if (
									delta.copilotErrors &&
									typeof responseStream.warning === 'function'
								) {
						...
						metadata['copilot_references'] = [
							...new Set(reportedReferences.values()).values(),
							...agentReferences,
						];
						if (
						...
						if (response.type !== ChatFetchResponseType.Success) {
							this.logService.warn(
								`Bad response from remote agent "${slug}": ${response.type} ${response.reason}`,
							);
							if (response.reason.includes('400 no docs found')) {
								...
								const editorContext = agentData?.editor_context
									? l10n.t(
											'**@{0}** will read your active file and selection.',
											slug,
										)
									: '';
								responseStream.confirmation(
									...
									editorContext +
										...
										l10n.t({
											message:
												'Please authorize usage of **@{0}** on {1} and resend your question. [Learn more]({2}).',
											args: [
												slug,
												url.hostname,
												'https://aka.ms/vscode-github-chat-extension-editor-context',
											],
											comment: [`{Locked=']({'}`],
										}),
								...
								!hadCopilotErrorsOrConfirmations
							) {
								return {
						...
						if (
							!this.checkAuthorized(slug) &&
							agentData?.editor_context &&
							!hasShownImplicitContextAuthorizationForSession
						) {
							responseStream.confirmation(
								l10n.t('Grant access to editor context'),
								l10n.t({
									message:
										'**@{0}** would like to read your active file and selection. [Learn More]({1})',
									args: [
										slug,
										'https://aka.ms/vscode-github-chat-extension-editor-context',
									],
		...
		agent.iconPath = agentData
			? Uri.parse(agentData.avatar_url)
		...
		store.add(
			agent.onDidReceiveFeedback((e) =>
				this.userFeedbackService.handleFeedback(e, participantId),
			),
		);
	...
	private async prepareClientPlatformReferences(
		...
		const redactFileContents = async (
		...
		const getImplicitContextId = async (uri: Uri) => {
			...
			const baseUri = repository
				? repository.rootUri.toString()
				: this.workspaceService.getWorkspaceFolder(uri)?.toString();
			return baseUri
				? path.relative(baseUri, uri.toString())
				: path.basename(uri.path);
		};

		const addFileReference = async (
			document: TextDocument,
			variableName?: string,
			isImplicit?: boolean,
		) => {
		...
		const addSelectionReference = async (
			...
			const selection = activeTextEditor.selection.isEmpty
				? new Range(
						new Position(selectionStart, 0),
						new Position(selectionStart + 1, 0),
					)
			...
			clientReferences.push({
				...
				data: {
					...
					content: await redactFileContents(
		...
		if (this.checkAuthorized(slug)) {
			...
			if (
				activeTextEditor &&
				variables.find((v) => v.id.startsWith('vscode.implicit'))
			) {
		...
		for (const repository of repositories) {
			...
			try {
				const repo =
					await this.githubRepositoryService.getRepositoryInfo(
						repoId.org,
						repoId.repo,
					);
				clientReferences.push({
				...
				if (
					ex instanceof Error &&
					ex.message.includes('Failed to fetch repository info')
				) {
					...
					void this.authenticationService.getGitHubSession(
						'permissive',
						{ silent: true },
					);
				}
				this.logService.error(
					ex,
					'Failed to fetch info about current GitHub repository',
				);
			}
		}
	...
	private async listEnabledSkills(authToken: string) {
		if (!this.enabledSkillsPromise) {
			this.enabledSkillsPromise = this.capiClientService
				...
				.then((json) =>
					json?.['skills'].reduce(
						(acc: Set<string>, skill: { slug: string }) =>
							acc.add(skill.slug),
						new Set(),
					),
				);
	...
	private async resolveCopilotSkills(
		agent: string,
		request: ChatRequest,
	): Promise<{ copilot_skills: string[] }> {
		if (agent === GITHUB_PLATFORM_AGENT_NAME) {
		...
		return [
			{
				name: 'web',
				insertText: `#web`,
				description: 'Search Bing for real-time context',
				kind: 'bing-search',
				command: undefined,
			},
...
function prepareConfirmations(request: ChatRequest) {
	const confirmations = [
		...(request.acceptedConfirmationData?.map((c) => ({
			state: 'accepted',
			confirmation: c,
		})) ?? []),
		...(request.rejectedConfirmationData?.map((c) => ({
			state: 'dismissed',
			confirmation: c,
		})) ?? []),
	];
...
function prepareRemoteAgentHistory(
	agentId: string,
	context: ChatContext,
): Raw.ChatMessage[] {
	...
	for (const h of context.history) {
		...
		if (h instanceof ChatResponseTurn) {
			const copilot_references =
				h.result.metadata?.['copilot_references'];
			const content = h.response
				.map((r) => {
...
function getOrCreateSessionId(context: ChatContext): string {
	...
	for (const h of context.history) {
		if (h instanceof ChatResponseTurn) {
			const maybeSessionId = (h.result as ICopilotChatResultIn).metadata
				?.sessionId;
