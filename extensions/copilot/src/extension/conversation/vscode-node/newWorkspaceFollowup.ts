...
import {
	CopilotFileScheme,
	CopilotWorkspaceScheme,
	CreateFileCommand,
	CreateProjectCommand,
	GithubWorkspaceScheme,
...
export function registerNewWorkspaceIntentCommand(
	previewContentManager: INewWorkspacePreviewContentManager,
	logService: ILogService,
	options: IConversationOptions,
) {
	const copilotWorkspaceProvider = new NewWorkspacePreviewFileSystemProvider(
		previewContentManager,
	);
	const githubWorkspaceProvider = new NewWorkspacePreviewFileSystemProvider(
		previewContentManager,
	);
	const copilotTextDocumentProvider = new NewWorkspaceTextDocumentProvider(
		previewContentManager,
	);
	...
	return Disposable.from(
		workspace.registerFileSystemProvider(
			...
			{
				isReadonly: new MarkdownString(
					l10n.t(
						'This file preview was generated by Copilot and may contain surprises or mistakes.\n\nAsk followup questions to refine it, then press Create Workspace.',
					),
				),
			},
		),
		...
		commands.registerCommand(
			CreateProjectCommand,
			async (
				fileTreePart: ChatResponseFileTreePart,
				workspaceRoot: Uri | undefined,
			) => {
				const parentFolder = (
					await window.showOpenDialog({
						defaultUri: workspaceRoot,
						title: path.basename(fileTreePart.baseUri.path),
		...
		commands.registerCommand(
			OpenFileCommand,
			async (fileTreePart: ChatResponseFileTreePart) => {
				const pathStr = Uri.joinPath(
					fileTreePart.baseUri,
					fileTreePart.value[0].name,
				).toString();
		...
		commands.registerCommand(
			...
			async (fileTreePart: ChatResponseFileTreePart) => {
				...
				if (uri) {
					...
					window.tabGroups.all.forEach((group) => {
						group.tabs.forEach((tab) => {
							if (
								...
								tab.input.uri.scheme === CopilotFileScheme
							) {
					...
					// re-open saved file
					const fileDoc = await workspace.openTextDocument(
						Uri.file(uri.fsPath),
					);
...
async function createWorkspace(
	...
	window.tabGroups.all.forEach((group) => {
		group.tabs.forEach((tab) => {
			if (
				tab.input instanceof TabInputText &&
				tab.input.uri.scheme === CopilotWorkspaceScheme
			) {
				tabsToClose.push(tab);
			}
		});
	...
	try {
			...
			async (progress, token) => {
				for (const file of files) {
					...
					if (!isUriContained(workspaceUri, fileUri)) {
						logService.warn(
							`[newIntent] Skipping file outside of workspace: ${file}`,
						);
						continue;
					}
					progress.report({
						message: l10n.t(`Creating file {0}...`, fileUri.fsPath),
					});
					const content = await workspace.fs.readFile(
						Uri.joinPath(fileTreePart.baseUri, file),
					);
					await workspace.fs.createDirectory(
						Uri.joinPath(fileUri, '..'),
					);
		...
		if (
			workspaceRoot &&
			workspaceUri.fsPath.startsWith(workspaceRoot.fsPath + path.sep)
		) {
 ...
 */
export function resolveProjectFileUri(
	workspaceUri: Uri,
	projectRoot: string,
	file: string,
): Uri {
...
async function getUniqueProjectName(
	projectFolder: Uri,
	projectName: string,
): Promise<string> {
	let i = 0;
	...
	while (uniqueProjectNameNotFound) {
		try {
			await workspace.fs.stat(
				Uri.joinPath(projectFolder, newProjectName),
			);
...
async function updateAiGeneratedWorkspacesFile(workspaceUris: Uri) {
	...
	if (await checkFileExists(aiGeneratedFilePath)) {
		const fileContnet = await workspace.fs
			.readFile(aiGeneratedFilePath)
			.then((b) => {
		...
		await workspace.fs.writeFile(
			aiGeneratedFilePath,
			Buffer.from(JSON.stringify(workspaces, null, 2)),
		);
	} else {
		await workspace.fs.writeFile(
			aiGeneratedFilePath,
			Buffer.from(JSON.stringify([workspaceUris.toString()], null, 2)),
		);
	}
}
...
function getAiGeneratedWorkspacesFile(): Uri | undefined {
	const vscodeFolderName =
		env.appName.indexOf('Insider') > 0 ||
		env.appName.indexOf('Code - OSS Dev') >= 0
			? 'Code - Insiders'
			: 'Code';
	const homeDir = Uri.file(
		process.env.HOME ||
