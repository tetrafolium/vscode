...
export function getAdditionalWelcomeMessage(
	...
	const showHint =
		...
		!configurationService.isConfigured(
			ConfigKey.TeamInternal.InternalWelcomeHintEnabled,
		)
			? true
	...
	if (showHint) {
		...
		const messageString = new vscode.MarkdownString(
			vscode.l10n.t({
				...
				args: [
					`command:${openSettingsCommand}?${encodeURIComponent('["telemetry.telemetryLevel"]')}`,
				],
