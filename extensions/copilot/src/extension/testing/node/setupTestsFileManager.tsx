...
} from '../../../platform/testing/common/workspaceMutationManager';
...
} from '../../prompts/node/codeMapper/patchEditGeneration';
...
export class WorkspaceMutationManager implements IWorkspaceMutationManager {
	...
	create(
		...
		const mut = this.instantiationService.createInstance(
			WorkspaceMutation,
			options,
		);
	...
	get(requestId: string): IWorkspaceMutation {
		...
		if (!req) {
			throw new Error(
				l10n.t(
					`No request found, or it has expired. Please re-submit your query.`,
				),
			);
		}
		return req;
...
class WorkspaceMutation implements IWorkspaceMutation {
		...
		try {
			...
			for (const file of this.opts.files) {
				...
				await this.fileSystemService.writeFile(
					URI.joinPath(this.opts.baseURI, file),
	...
	private async generateContent(file: string) {
		...
		try {
			document = await this.workspaceService.openTextDocumentAndSnapshot(
				URI.joinPath(this.opts.baseURI, file),
			);
		} catch {
			// ignored
		...
		const promptRenderer = PromptRenderer.create(
			...
			WorkspaceMutationFilePrompt,
			{
				...
				instructionsForThisFile: descriptions?.perFile.find(
					(f) => f.file === file,
				)?.description,
			},
		);
		...
		if (fetchResult.type !== ChatFetchResponseType.Success) {
			...
			throw new Error(
				l10n.t(
					'Encountered an error while generating the file: ({0}) {1}',
					fetchResult.type,
					fetchResult.reason,
				),
			);
		...
		if (originalText && document) {
			const reply = getPatchEditReplyProcessor(
				this.promptPathRepresentationService,
			).process(fetchResult.value, originalText, document.uri);
	...
	private async getFileDescriptions() {
		...
		const promptRenderer = PromptRenderer.create(
			this.instantiationService,
			endpoint,
			WorkspaceMutationInstructionsPrompt,
			{
		...
		for (const [, file, description] of fetchResult.value.matchAll(
			/^`?(.*?)`?:\s*(.+)$/gm,
		)) {
		...
		return (
			<>
				<SystemMessage priority={1000}>
					...
					Additional Rules
					<br />
					You will be given a user query and a tree of files they wish
					to edit or create in order to accomplish a task. Think step
					by step and respond with a text description that lists and
					summarizes what needs to be done in each file to accomplish
					the user's task.
					<br />
					...
					## Valid question
					<br />
					User: I want to: add the sequelize ORM to my project and add
					a user model
					<br />
					...
					## Valid response
					<br />
					`src/models/user.ts`: This file defines and exports the User
					model for use in the application.
					<br />
					`src/routes/index.ts`: This file exports a function
					`setRoutes` which sets up the routes for the application. It
					uses the `IndexController` to handle the root route.
					<br />
					`package.json`: We need to edit the package.json to ensure
					Sequelize is defined as a dependency
					<br />
					`README.md`: We should add documentation to the readme file
					to make consumers aware of the new setup steps.
					<br />
				</SystemMessage>
				<UserMessage priority={900}>
					I want to: {this.props.query}
					<br />
					This is the project tree structure:
					<br />
					```markdown' <br />
		...
		return (
			<>
				<SystemMessage priority={1000}>
					...
					Additional Rules
					<br />
					The user will describe the task they're trying to
					accomplish, and ask you to generate or edit a file in
					persuit of that task.
					<br />
					{document ? (
						<PatchEditRules />
					) : (
						<>
							...
							<br />
							If asked to generate a test file, create a file with
							a self-contained 'hello world' test case without
							dependency on any other files or imports aside from
							the testing framwork.
							<br />
						</>
					)}
...
`}
				...
				<UserMessage priority={900}>
					...
					{instructionsForThisFile ? (
						...
						<>
							Here are the files in my workspace, including this
							one: {allInstructions}`
						</>
					)}
					<br />
