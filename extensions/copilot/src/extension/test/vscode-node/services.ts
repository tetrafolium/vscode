...
import {
	AutomodeService,
	IAutomodeService,
} from '../../../platform/endpoint/node/automodeService';
...
import {
	IImageService,
	nullImageService,
} from '../../../platform/image/common/imageService';
import {
...
import {
	...
	UndesiredModels,
} from '../../../platform/inlineEdits/node/inlineEditsModelService';
...
import {
	IMcpService,
	NullMcpService,
} from '../../../platform/mcp/common/mcpService';
import {
	EditLogService,
	IEditLogService,
} from '../../../platform/multiFileEdit/common/editLogService';
import {
	IMultiFileEditInternalTelemetryService,
	MultiFileEditInternalTelemetryService,
} from '../../../platform/multiFileEdit/common/multiFileEditQualityTelemetry';
...
import {
	IProxyModelsService,
	NullProxyModelsService,
} from '../../../platform/proxyModels/common/proxyModelsService';
import {
...
import {
	IRerankerService,
	RerankerService,
} from '../../../platform/workspaceChunkSearch/common/rerankerService';
...
} from '../../agents/vscode-node/githubOrgChatResourcesService';
import {
	CommandServiceImpl,
	ICommandService,
} from '../../commands/node/commandService';
import {
...
} from '../../context/node/resolvers/promptWorkspaceLabels';
import {
	IUserFeedbackService,
	UserFeedbackService,
} from '../../conversation/vscode-node/userActions';
import {
	ConversationStore,
	IConversationStore,
} from '../../conversationStore/node/conversationStore';
import {
	ILinkifyService,
	LinkifyService,
} from '../../linkify/common/linkifyService';
...
} from '../../onboardDebug/node/commandToConfigConverter';
import {
	DebuggableCommandIdentifier,
	IDebuggableCommandIdentifier,
} from '../../onboardDebug/node/debuggableCommandIdentifier';
import {
	ILanguageToolsProvider,
	LanguageToolsProvider,
} from '../../onboardDebug/node/languageToolsProvider';
...
import {
	IFeedbackReporter,
	NullFeedbackReporterImpl,
} from '../../prompt/node/feedbackReporter';
import { IPromptVariablesService } from '../../prompt/node/promptVariablesService';
import {
	ITodoListContextProvider,
	TodoListContextProvider,
} from '../../prompt/node/todoListContextProvider';
...
export function createExtensionTestingServices(): TestingServiceCollection {
	...
	testingServiceCollection.define(
		IAutomodeService,
		new SyncDescriptor(AutomodeService),
	);
	testingServiceCollection.define(
		IFileSystemService,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(DefaultsOnlyConfigurationService),
	);
	testingServiceCollection.define(
		IEnvService,
		new SyncDescriptor(TestEnvService),
	);
	testingServiceCollection.define(
		INativeEnvService,
		new SyncDescriptor(NullNativeEnvService),
	);
	testingServiceCollection.define(
		ISimulationTestContext,
	...
	testingServiceCollection.define(
		IRequestLogger,
		new SyncDescriptor(NullRequestLogger),
	);
	testingServiceCollection.define(
		IFeedbackReporter,
		new SyncDescriptor(NullFeedbackReporterImpl),
	);
	testingServiceCollection.define(
		IEndpointProvider,
	...
	testingServiceCollection.define(
		ICopilotTokenStore,
		new SyncDescriptor(CopilotTokenStore),
	);
	testingServiceCollection.define(
		IDomainService,
		new SyncDescriptor(DomainService),
	);
	testingServiceCollection.define(
		ICAPIClientService,
		new SyncDescriptor(CAPIClientImpl),
	);
	testingServiceCollection.define(
		INotificationService,
		new SyncDescriptor(NullNotificationService),
	);
	testingServiceCollection.define(
		ICommandService,
		new SyncDescriptor(CommandServiceImpl),
	);
	testingServiceCollection.define(
		IPromptVariablesService,
		new SyncDescriptor(PromptVariablesServiceImpl),
	);
	testingServiceCollection.define(
	...
	testingServiceCollection.define(
		IIgnoreService,
		new SyncDescriptor(NullIgnoreService),
	);
	testingServiceCollection.define(
		IRemoteRepositoriesService,
		new SyncDescriptor(RemoteRepositoriesService),
	);
	testingServiceCollection.define(
		IWorkspaceService,
	...
	testingServiceCollection.define(
		IPowerService,
		new SyncDescriptor(NullPowerService),
	);
	testingServiceCollection.define(
		IChatMLFetcher,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(TabsAndEditorsServiceImpl),
	);
	testingServiceCollection.define(
		IEmbeddingsComputer,
		new SyncDescriptor(RemoteEmbeddingsComputer),
	);
	testingServiceCollection.define(
		ITelemetryService,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(LanguageDiagnosticsServiceImpl),
	);
	testingServiceCollection.define(
		ITokenizerProvider,
		new SyncDescriptor(TokenizerProvider, [true]),
	);
	testingServiceCollection.define(
		IPromptWorkspaceLabels,
	...
	testingServiceCollection.define(
		IGitDiffService,
		new SyncDescriptor(GitDiffService),
	);
	testingServiceCollection.define(
	...
	testingServiceCollection.define(
		IGitService,
		new SyncDescriptor(GitServiceImpl),
	);
	testingServiceCollection.define(
		IOctoKitService,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(WorkspaceMutationManager),
	);
	testingServiceCollection.define(
		ITestProvider,
		new SyncDescriptor(TestProvider),
	);
	testingServiceCollection.define(
		INaiveChunkingService,
		new SyncDescriptor(NaiveChunkingService),
	);
	testingServiceCollection.define(
		ILinkifyService,
		new SyncDescriptor(LinkifyService),
	);
	testingServiceCollection.define(
		IEditToolLearningService,
		new SyncDescriptor(EditToolLearningService),
	);
	testingServiceCollection.define(
		IDebugCommandToConfigConverter,
		new SyncDescriptor(DebugCommandToConfigConverter),
	);
	testingServiceCollection.define(
		ILaunchConfigService,
		new SyncDescriptor(LaunchConfigService),
	);
	testingServiceCollection.define(
		IDebuggableCommandIdentifier,
		new SyncDescriptor(DebuggableCommandIdentifier),
	);
	testingServiceCollection.define(
		ILanguageToolsProvider,
		new SyncDescriptor(LanguageToolsProvider),
	);
	testingServiceCollection.define(
		IEditLogService,
		new SyncDescriptor(EditLogService),
	);
	testingServiceCollection.define(
		IMultiFileEditInternalTelemetryService,
		new SyncDescriptor(MultiFileEditInternalTelemetryService),
	);
	testingServiceCollection.define(
		ICodeMapperService,
		new SyncDescriptor(CodeMapperService),
	);
	testingServiceCollection.define(
		IAlternativeNotebookContentService,
		new SyncDescriptor(MockAlternativeNotebookContentService),
	);
	testingServiceCollection.define(
		IAlternativeNotebookContentEditGenerator,
	...
	testingServiceCollection.define(
		IDebugOutputService,
		new SyncDescriptor(DebugOutputServiceImpl),
	);
	testingServiceCollection.define(
		IUserFeedbackService,
		new SyncDescriptor(UserFeedbackService),
	);
	testingServiceCollection.define(
		ITerminalService,
		new SyncDescriptor(TerminalServiceImpl),
	);
	testingServiceCollection.define(
		IConversationStore,
		new SyncDescriptor(ConversationStore),
	);
	testingServiceCollection.define(
		IFixCookbookService,
		new SyncDescriptor(FixCookbookService),
	);
	testingServiceCollection.define(
		IReviewService,
	...
	testingServiceCollection.define(
		IBlockedExtensionService,
		new SyncDescriptor(BlockedExtensionService),
	);
	testingServiceCollection.define(
		IDialogService,
	...
	testingServiceCollection.define(
		ILanguageFeaturesService,
		new SyncDescriptor(NoopLanguageFeaturesService),
	);
	testingServiceCollection.define(
		IScopeSelector,
		new SyncDescriptor(ScopeSelectorImpl),
	);
	testingServiceCollection.define(
		IPromptPathRepresentationService,
		new SyncDescriptor(PromptPathRepresentationService),
	);
	testingServiceCollection.define(
		IPromptsService,
		new SyncDescriptor(PromptsServiceImpl),
	);
	testingServiceCollection.define(
		IToolsService,
		new SyncDescriptor(NullToolsService),
	);
	testingServiceCollection.define(
		IToolDeferralService,
		new ToolDeferralService(),
	);
	testingServiceCollection.define(
		IChatDiskSessionResources,
		new SyncDescriptor(ChatDiskSessionResources),
	);
	testingServiceCollection.define(
		IChatSessionService,
		new SyncDescriptor(TestChatSessionService),
	);
	testingServiceCollection.define(
		INotebookService,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(MockRunCommandExecutionService),
	);
	testingServiceCollection.define(
		ISearchService,
		new SyncDescriptor(SearchServiceImpl),
	);
	testingServiceCollection.define(
		IToolGroupingCache,
		new SyncDescriptor(ToolGroupingCache),
	);
	testingServiceCollection.define(
		IToolGroupingService,
		new SyncDescriptor(ToolGroupingService),
	);
	testingServiceCollection.define(
		ITodoListContextProvider,
		new SyncDescriptor(TodoListContextProvider),
	);
	testingServiceCollection.define(
		IGithubAvailableEmbeddingTypesService,
		new SyncDescriptor(GithubAvailableEmbeddingTypesService),
	);
	testingServiceCollection.define(
		IProxyModelsService,
		new SyncDescriptor(NullProxyModelsService),
	);
	testingServiceCollection.define(
		IInlineEditsModelService,
	...
	testingServiceCollection.define(
		IUndesiredModelsManager,
		new SyncDescriptor(UndesiredModels.Manager),
	);
	testingServiceCollection.define(
		ICopilotInlineCompletionItemProviderService,
		new SyncDescriptor(NullCopilotInlineCompletionItemProviderService),
	);
	testingServiceCollection.define(
		IRerankerService,
		new SyncDescriptor(RerankerService),
	);
	testingServiceCollection.define(
		IOTelSqliteStore,
		new OTelSqliteStore(':memory:'),
	);
	testingServiceCollection.define(
		ISessionStore,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(ToolResultContentRenderer),
	);
	testingServiceCollection.define(
		IGitHubOrgChatResourcesService,
		new SyncDescriptor(GitHubOrgChatResourcesService),
	);
