...
import {
	getAlignedSession,
	getAnyAuthSession,
} from '../../../platform/authentication/vscode-node/session';
import {
	AuthProviderId,
	Config,
	ConfigKey,
...
suite('Session tests', function () {
	...
	testingServiceCollection.define(
		ITelemetryUserConfig,
		new SyncDescriptor(TelemetryUserConfigImpl, ['test', false]),
	);
	...
	>;
	let getAccountsStub: sinon.SinonStub<
		[providerId: string],
		...
		getSessionStub.callsFake(
			(
				...
				if (sessionsWithScope) {
					if (options?.account) {
						sessionsWithScope = sessionsWithScope.filter(
							(session) =>
	...
	setup(() => {
		...
		configurationStub.returns(
			new (class implements WorkspaceConfiguration {
	...
	suite('getAnyAuthSession', () => {
		test('should return a session with ["user:email"] scope if it is available', async () => {
			...
			const result = await getAnyAuthSession(
				accessor.get(IConfigurationService),
			);

			assert.strictEqual(
				(result as AuthenticationSession)?.id,
				sessionId,
			);
		});
		...
		test('should return undefined if there are no pre-existing sessions', async () => {
			const alignedScopeSessionStub = getSessionStub.withArgs(
				'github',
				GITHUB_SCOPE_ALIGNED,
				sinon.match.any,
			);
			const userEmailSessionStub = getSessionStub.withArgs(
				'github',
				GITHUB_SCOPE_USER_EMAIL,
				sinon.match.any,
			);
			const readUserScopeSessionStub = getSessionStub.withArgs(
				'github',
				GITHUB_SCOPE_READ_USER,
				sinon.match.any,
			);
		...
		test('should use the github-enterprise provider if configured', async () => {
			...
			getAccountsStub.resolves([
				{ id: 'account', label: 'ghe-session-label' },
			]);
			const gheSessionStub = getSessionStub.withArgs(
				'github-enterprise',
				GITHUB_SCOPE_READ_USER,
				sinon.match.any,
			);
			...
			const ghSessionStub = getSessionStub.withArgs(
				'github',
				GITHUB_SCOPE_READ_USER,
				sinon.match.any,
			);
	...
	suite('getAlignedSession', () => {
		test('should return a session with more permissive scopes if there is one', async () => {
			...
			const result = await getAlignedSession(
				accessor.get(IConfigurationService),
				{ silent: true },
			);
