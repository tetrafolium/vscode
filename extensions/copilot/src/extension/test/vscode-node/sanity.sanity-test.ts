...
suite('Copilot Chat Sanity Test', function () {
	...
	suiteSetup(async function () {
		...
		assert.ok(
			realContext,
			'`extension.activate()` did not return context`',
		);
		assert.ok(
			realContext.extensionMode,
			'extension context does not have `extensionMode`',
		);
		...
		assert.strictEqual(
			typeof (activateResult as IInstantiationService).createInstance,
			'function',
			'createInstance is not a function',
		);
		assert.strictEqual(
			typeof (activateResult as IInstantiationService).invokeFunction,
			'function',
			'invokeFunction is not a function',
		);
		...
		await realInstaAccessor.invokeFunction(async (accessor) => {
			const token = await accessor
				.get(IAuthenticationService)
				.getCopilotToken();
			assert.ok(
				token,
				'Copilot token must be available before tests run',
			);
		});
	});
	...
	test('E2E Production Panel Chat Test', async function () {
		assert.ok(
			realInstaAccessor,
			'Instantiation service accessor is not available',
		);
		...
		await realInstaAccessor.invokeFunction(async (accessor) => {
			...
			let interactiveSession = instaService.createInstance(
				ChatParticipantRequestHandler,
				[],
				new TestChatRequest('Write me a for loop in javascript'),
				stream,
				fakeToken,
			...
			assert.ok(
				...
				'Expected progress after first request',
			);
			const oldText = stream.currentProgress;

			stream = new SpyChatResponseStream();
			interactiveSession = instaService.createInstance(
				ChatParticipantRequestHandler,
				[],
				new TestChatRequest('Can you make it in typescript instead'),
				stream,
				fakeToken,
				{ agentName: '', agentId: '', intentId: '' },
				() => false,
			...
			assert.ok(
				stream.currentProgress,
				'Expected progress after second request',
			);
			assert.notStrictEqual(
				stream.currentProgress,
				oldText,
				'Expected different progress text after second request',
			);

			const conversation = conversationStore.getConversation(
				result2.metadata.responseId,
			);
	...
	test('E2E Production agent mode', async function () {
		...
		await realInstaAccessor.invokeFunction(async (accessor) => {
			...
			const runAgentRequest =
				async (): Promise<SpyChatResponseStream> => {
					...
					const testRequest = new TestChatRequest(
						`Call the get_errors tool now to check the current window for errors. You must invoke the tool exactly once, then reply with a brief summary of whatever it returned.`,
					);
					...
					const onWillInvokeTool = Event.toPromise(
						toolsService.onWillInvokeTool,
					);
					const getResultPromise = interactiveSession.getResult();
					const dumpStream = () => {
						const parts = stream.items.map(
					...
					try {
						await Promise.race([
							...
							timeout(20_000).then(() =>
								Promise.reject(
									new Error(
										'timed out waiting for tool call. ' +
											dumpStream(),
									),
								),
							),
						...
						const followUpError =
							'error' in settled
								...
								: '';
						throw new Error(
							`${cause.message} | follow-up: kind=${settled.kind}${followUpError} ${dumpStream()}`,
							{ cause },
						);
					}
				};
			...
			const interactiveSession = instaService.createInstance(
				ChatParticipantRequestHandler,
				[],
				new TestChatRequest('And what is 1+1'),
				stream2,
				fakeToken,
				{ agentName: '', agentId: '', intentId: Intent.Agent },
				() => false,
	...
	test('Slash Commands work properly', async function () {
		...
		await realInstaAccessor.invokeFunction(async (accessor) => {
			...
			const interactiveSession = instaService.createInstance(
				ChatParticipantRequestHandler,
				[],
				new TestChatRequest('What is a fibonacci sequence?'),
				progressReport,
				fakeToken,
				{ agentName: '', agentId: '', intentId: 'explain' },
				() => false,
				undefined,
			);
	...
	test.skip('E2E Production Inline Chat Test', async function () {
		...
		await realInstaAccessor.invokeFunction(async (accessor) => {
			const r = vscode.lm.registerLanguageModelChatProvider(
				'test',
				new (class implements vscode.LanguageModelChatProvider {
