...
describe('Configurations', () => {
	it('package.json configuration contains stable, experimental, preview, and advanced sections', () => {
		...
		expect(
			...
			'package.json should have exactly 4 sections',
		).toHaveLength(4);

		// Should have a stable section
		const stableSection = configurationContributions.find(
			(section) => section.id === 'stable',
		);
		const preview = configurationContributions.find(
			(section) => section.id === 'preview',
		);
		const experimental = configurationContributions.find(
			(section) => section.id === 'experimental',
		);
		const advanced = configurationContributions.find(
			(section) => section.id === 'advanced',
		);

		expect(
			stableSection,
			'stable configuration section is missing',
		).toBeDefined();
		expect(
			preview,
			'preview configuration section is missing',
		).toBeDefined();
		expect(
			experimental,
			'experimental configuration section is missing',
		).toBeDefined();
		expect(
			advanced,
			'advanced configuration section is missing',
		).toBeDefined();
	});
	...
	it('package.json configuration tags are correct for each section', () => {
		...
		const previewSection = configurationContributions.find(
			(section) => section.id === 'preview',
		)!;
		...
		const experimentalSection = configurationContributions.find(
			(section) => section.id === 'experimental',
		)!;
		...
		const advancedSection = configurationContributions.find(
			(section) => section.id === 'advanced',
		)!;
	...
	it('settings in code should match package.json', () => {
		const configurationsInPackageJson =
			packageJson.contributes.configuration.flatMap((section) =>
				Object.keys(section.properties),
			);
		const advancedConfigurationsInPackageJson =
			packageJson.contributes.configuration
				.filter((section) => section.id === 'advanced')
				.flatMap((section) => Object.keys(section.properties));
		const otherConfigurationsInPackageJson =
			packageJson.contributes.configuration
				.filter((section) => section.id !== 'advanced')
				.flatMap((section) => Object.keys(section.properties));

		// Get keys from code
		const internalKeys = Object.values(ConfigKey.TeamInternal).map(
			(setting) => setting.fullyQualifiedId,
		);
		const sharedKeys = Object.values(ConfigKey.Shared).map(
			(setting) => setting.fullyQualifiedId,
		);
		const advancedPublicKeys = Object.values(ConfigKey.Advanced).map(
			(setting) => setting.fullyQualifiedId,
		);
		const otherPublicKeys = (
			Object.values(ConfigKey).filter(
				(key) =>
					key !== ConfigKey.TeamInternal &&
					key !== ConfigKey.Shared &&
					key !== ConfigKey.Advanced &&
					key !== ConfigKey.Deprecated,
			) as Config<any>[]
		...
		unregistered.forEach((key) => {
			expect(
				configurationsInPackageJson,
				'unregistered settings should not be defined in the package.json',
			).not.toContain(key);
		});
		...
		internalKeys.forEach((key) => {
			expect(
				key,
				'Internal settings must start with github.copilot.chat.advanced.',
			).toMatch(/^github\.copilot\.chat\.advanced\./);
		});

		// Validate public settings in code are in package.json
		otherPublicKeys.forEach((key) => {
			expect(
				otherConfigurationsInPackageJson,
				'Setting in code is not defined in the package.json',
			).toContain(key);
		});
		...
		advancedPublicKeys.forEach((key) => {
			expect(
				key,
				'Advanced settings must not start wih github.copilot.chat.advanced.',
			).not.toMatch(/^github\.copilot\.chat\.advanced\./);
			if (
				key ===
				ConfigKey.Advanced.DebugGitHubAuthFailWith.fullyQualifiedId
			) {
			...
			expect(
				advancedConfigurationsInPackageJson,
				`Advanced setting ${key} should be defined in the advanced section of package.json`,
			).toContain(key);
		});
		...
		configurationsInPackageJson.forEach((key) => {
			expect(
				registered,
				'Setting in package.json is not defined in code',
			).toContain(key);
		});
	});

	it('prompt override string setting uses camelCase', () => {
		const advancedSection = packageJson.contributes.configuration.find(
			(section) => section.id === 'advanced',
		)!;
		const promptOverrideStringKey =
			ConfigKey.Advanced.DebugPromptOverrideString;

		expect(promptOverrideStringKey.fullyQualifiedId).toBe(
			'github.copilot.chat.debug.promptOverrideString',
		);
		expect(promptOverrideStringKey.fullyQualifiedOldId).toBeUndefined();
		expect(Object.keys(advancedSection.properties)).toContain(
			promptOverrideStringKey.fullyQualifiedId,
		);
	});
	...
	it('all localization strings in package.json are present in package.nls.json', async () => {
		...
		const packageNlsPath = path.join(
			__dirname,
			'../../../../package.nls.json',
		);
		const [packageJsonFileContents, packageNlsFileContents] =
			await Promise.all([
		...
		const nlsReferences = Array.from(
			packageJsonFileContents.matchAll(/"%([^"]+)%"/g),
		).map((match) => match[1]);

		// Validate all references exist in package.nls.json
		const missingKeys = nlsReferences.filter(
			(key) => !nlsKeys.includes(key),
		);
		if (missingKeys.length > 0) {
			throw new Error(
				`Missing localization keys in package.nls.json but present in package.json: ${missingKeys.map((key) => `'%${key}%'`).join(', ')}`,
			);
		}
	});
});
