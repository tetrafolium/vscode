...
import {
	Reporter,
	createEditsFromPseudoDiff,
	createEditsFromRealDiff,
} from '../../prompt/node/editFromDiffGeneration';
...
suite('Real Diff Apply', function () {
	createTestsFromFixtures(
		path.join(__dirname, './fixtures/gitdiff'),
		(
			original: string,
			diff: string,
			expected: string,
			...
			const linesEdits = createEditsFromRealDiff(
				Lines.fromString(original),
				Lines.fromString(diff),
				reporter,
			);
			const actual = applyEdits(
				original,
				linesEdits.map((e) => e.toTextEdit()),
			);
			deepStrictEqual(
				Lines.fromString(actual),
				Lines.fromString(expected),
			);
...
suite('Pseudo Diff Apply', function () {
	createTestsFromFixtures(
		path.join(__dirname, './fixtures/pseudodiff'),
		(
			original: string,
			diff: string,
			expected: string,
			...
			const reporter = new ReporterImpl();
			const linesEdits = createEditsFromPseudoDiff(
				Lines.fromString(original),
				Lines.fromString(diff),
				reporter,
			);
...
function createTestsFromFixtures(
	testDir: string,
	runTest: (
		original: string,
		diff: string,
		...
		if (match) {
			test(`${match[1]} - ${match[2].replace(/_/g, ' ')}`, async () => {
				...
				const diff = await fs.readFile(
					path.join(testDir, `${entry}.diff`),
					'utf8',
				);
				const original = await fs.readFile(
					path.join(testDir, match[1]),
					'utf8',
				);
				let messages = [];
				try {
					messages = JSON.parse(
						await fs.readFile(
							path.join(testDir, `${entry}.messages`),
							'utf8',
						),
					);
				} catch (e) {
