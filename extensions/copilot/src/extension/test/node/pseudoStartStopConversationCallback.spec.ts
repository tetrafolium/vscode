...
suite('Post Report Conversation Callback', () => {
	...
	test('Simple post-report', async () => {
		...
		const testObj = instaService.createInstance(
		...
		await testObj.doProcessResponse(
			responseSource.asyncIterable,
			stream,
		...
		assert.deepStrictEqual(
			stream.items.map(
				(p) => (p as ChatResponseMarkdownPart).value.value,
			),
	...
	test('Partial stop word with extra text after', async () => {
		...
		responseSource.emitOne({
			delta: { text: 'one ', codeVulnAnnotations: annotations },
		});
		...
		assert.deepStrictEqual(
			(stream.items[0] as ChatResponseMarkdownWithVulnerabilitiesPart)
				.vulnerabilities,
			annotations.map(
				(a) =>
					({
	...
	test('Text on same line as start', async () => {
		...
		responseSource.emitOne({
			delta: { text: 'test test test test 123456' },
		});
		responseSource.emitOne({ delta: { text: 'end\n\nhello' } });
		responseSource.resolve();
	...
	test('Start word without a stop word', async () => {
		...
		assert.strictEqual(
			(stream.items[0] as ChatResponseMarkdownPart).value.value,
...
suite('Tool stream throttling', () => {
	...
	beforeEach(() => {
		...
		stream = new ChatResponseStreamImpl(
			...
			(toolCallId, streamData) =>
				updateCalls.push({ toolCallId, streamData }),
		);
	});
		...
		responseSource.emitOne({
			delta: {
				...
				copilotToolCallStreamUpdates: [
					{ id: 'tool1', name: 'myTool', arguments: '{"a":1}' },
				],
			},
		});
		responseSource.resolve();

		await processor.doProcessResponse(
			responseSource.asyncIterable,
			stream,
			CancellationToken.None,
		);
	...
	test('rapid updates within throttle window are throttled', async () => {
		...
		responseSource.emitOne({
			delta: {
				text: '',
				copilotToolCallStreamUpdates: [
					{ id: 'tool1', name: 'myTool', arguments: '{"a":2}' },
				],
			},
		});
		responseSource.emitOne({
			delta: {
				text: '',
				copilotToolCallStreamUpdates: [
					{ id: 'tool1', name: 'myTool', arguments: '{"a":3}' },
				],
			},
		});
		responseSource.resolve();
		...
		assert.strictEqual(updateCalls[0].toolCallId, 'tool1');
		assert.deepStrictEqual(updateCalls[1].streamData.partialInput, {
			a: 3,
		});
	});
	...
	test('update after throttle window elapses is emitted immediately', async () => {
		...
		assert.strictEqual(updateCalls.length, 2);
		assert.deepStrictEqual(updateCalls[0].streamData.partialInput, {
			a: 1,
		});
		assert.deepStrictEqual(updateCalls[1].streamData.partialInput, {
			a: 2,
	...
	test('different tool IDs are throttled independently', async () => {
		...
		responseSource.emitOne({
			delta: {
				text: '',
				copilotToolCallStreamUpdates: [
					{ id: 'tool2', name: 'myTool', arguments: '{"b":1}' },
				],
			},
		});
		...
		responseSource.emitOne({
			delta: {
				text: '',
				copilotToolCallStreamUpdates: [
					{ id: 'tool2', name: 'myTool', arguments: '{"b":2}' },
				],
			},
		});
		responseSource.resolve();
	...
	test('pending updates are not flushed on cancellation', async () => {
		...
		const promise = processor.doProcessResponse(
			responseSource.asyncIterable,
			stream,
			cts.token,
		);
	...
	test('retry clears pending throttle state', async () => {
		...
		responseSource.emitOne({
			delta: { text: '', retryReason: 'network_error' },
		});
