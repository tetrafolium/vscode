...
suite.skip('Conversation telemetry tests', { timeout: 10000 }, function () {
	test('Test telemetryMessage', async function () {
		...
		const messageLen = 12;
		const prompt =
			'You are a programming assistant. Respond to the question hello world!';
		const source = 'user';
		const turnIndex = 0;
		...
		const [messages] = await withTelemetryCapture(
			...
			async (accessor) => {
				...
				assert.strictEqual(
					telemetryData.measurements.messageCharLen,
					messageText.length,
				);
				assert.strictEqual(
					telemetryData.measurements.promptCharLen,
					prompt.length,
				);
				assert.strictEqual(
					telemetryData.measurements.intentClassifierScore,
					intentClassifierScore,
				);
				assert.strictEqual(
					telemetryData.measurements.intentClassifierLatency,
					intentClassifierLatency,
				);
		...
		assert.deepStrictEqual(
			names
				.filter((name) => !name.startsWith('engine.') && name !== 'log')
				.sort(),
		...
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.properties.source === source,
				),
		);
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.properties.turnIndex ===
						turnIndex.toString(),
				),
		);
		...
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.measurements.messageCharLen ===
						messageLen,
				),
		);
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.measurements.promptCharLen ===
						prompt.length,
				),
		);
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.measurements
							.intentClassifierScore === intentClassifierScore,
				),
		);
		assert.ok(
		...
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.properties.messageText ===
						messageText,
				),
		);
	});
	...
	test('Test telemetryUserAction', async function () {
		...
		const [messages] = await withTelemetryCapture(
			...
			async (accessor) => {
				...
				assert.strictEqual(
					telemetryData.properties.messageId,
					messageId,
				);
			},
		);
		...
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.properties.rating === rating,
				),
		);
		assert.ok(
			messages
				...
				.every(
					(message) =>
						message.data.baseData.properties.messageId ===
						messageId,
				),
		);
	});
	...
	test('Test getCodeBlocks with no code', async function () {
		...
		assert.strictEqual(
			noCodeResult.length,
			0,
			'Length of no code result should be 0',
		);
	});
	...
	test('Test getCodeBlocks with one code block, no language', async function () {
		...
		assert.deepEqual(
			basicNoLangResult,
			[''],
			'Basic no lang result should be an array with an empty string',
		);
	});
