...
suite('PatchEditGeneration - sync', function () {
	...
	for (const entry of entries) {
		...
		createTestsFromFixtures(fixturesFolder, (data) => {
			const replyProcessor = getPatchEditReplyProcessor(
			...
			deepStrictEqual(
				...
				Lines.fromString(data.expected),
			);
		});
	}
});
...
suite('PatchEditGeneration - async', function () {
	...
	for (const entry of entries) {
		...
		createTestsFromFixtures(fixturesFolder, async (data) => {
			...
			const promise = processPatchResponse(
...
function createTestsFromFixtures(
	...
	runTest: (data: { [key: string]: string }) => void,
) {
	...
	for (const entry of entries) {
		...
		if (match) {
			...
			const content = fs.readFile(
				path.join(fixturesFolder, entry),
				'utf8',
			);
