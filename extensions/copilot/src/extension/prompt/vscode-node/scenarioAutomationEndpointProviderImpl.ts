	...
	override async getChatEndpoint(
		...
		if (this._authService.copilotToken?.isNoAuthUser && !isProxyingCAPI) {
			...
			const getFirstNonCopilotModel = async () => {
				...
				const firstNonCopilotModel = allModels.find(
					(m) => m.vendor !== 'copilot',
				);
				if (firstNonCopilotModel) {
					this._logService.trace(
						`Using custom contributed chat model`,
					);
					return this._instantiationService.createInstance(
						ExtensionContributedChatEndpoint,
						firstNonCopilotModel,
					);
				} else {
		...
		try {
			...
			if (typeof requestOrFamilyOrModel === 'string') {
				this._logService.trace(
					`ScenarioAutomation: failed to resolve model family '${requestOrFamilyOrModel}', falling back to copilot-utility`,
				);
