...
import {
	renderDataPartToString,
	renderToolResultToStringNoBudget,
} from './requestLoggerToolResult';
		...
		.map((d, i) => {
			...
			if (d.contextManagement) {
				...
				const totalClearedTokens =
					(
						d.contextManagement as ContextManagementResponse
					)?.applied_edits?.reduce(
						(sum: number, edit) =>
...
class LoggedRequestInfo implements ILoggedRequestInfo {
	...
	toJSON(): object {
		...
		if (!this.entry.chatParams.ignoreStatefulMarker) {
			const statefulMarker = Iterable.first(
				getAllStatefulMarkersAndIndicies(
					this.entry.chatParams.messages,
				),
			);
			if (statefulMarker) {
				lastResponseId = {
		...
		const metadata = {
			url:
				typeof this.entry.chatEndpoint.urlOrRequestMetadata === 'string'
					? this.entry.chatEndpoint.urlOrRequestMetadata
					: undefined,
			requestType:
				typeof this.entry.chatEndpoint.urlOrRequestMetadata === 'object'
					? this.entry.chatEndpoint.urlOrRequestMetadata?.type
					: undefined,
			...
			maxResponseTokens:
				...
				this.entry.chatParams.body?.max_completion_tokens,
			...
			duration:
				this.entry.endTime && this.entry.startTime
			...
			requestId:
				...
				this.entry.type === LoggedRequestKind.ChatMLFailure
					? this.entry.result.requestId
					: undefined,
			serverRequestId:
				this.entry.type === LoggedRequestKind.ChatMLSuccess ||
				this.entry.type === LoggedRequestKind.ChatMLFailure
					? this.entry.result.serverRequestId
					: undefined,
			timeToFirstToken:
				this.entry.type === LoggedRequestKind.ChatMLSuccess
					? this.entry.timeToFirstToken
					: undefined,
			usage:
				this.entry.type === LoggedRequestKind.ChatMLSuccess
					? this.entry.usage
			...
			copilotUsageAic:
				this.entry.type === LoggedRequestKind.ChatMLSuccess &&
				typeof this.entry.usage?.copilot_usage?.total_nano_aiu ===
					'number'
					? this.entry.usage.copilot_usage.total_nano_aiu /
						1_000_000_000
					: undefined,
	...
	async toJSON(): Promise<object> {
		...
		for (const content of this.response.content) {
			if (content instanceof LanguageModelTextPart) {
				...
				responseData.push(
					await renderToolResultToStringNoBudget(content),
				);
			}
		}
		...
		const thinking = this.thinking?.text
			? {
					id: this.thinking.id,
					text: Array.isArray(this.thinking.text)
						? this.thinking.text.join('\n')
						: this.thinking.text,
				}
			: undefined;
...
export class RequestLogger extends AbstractRequestLogger {
	...
	constructor(
		...
		private readonly _chatDebugFileLoggerService: IChatDebugFileLoggerService,
	) {
		super();
						...
						if (format === 'json') {
							...
							switch (entry.kind) {
								...
								case LoggedInfoKind.ToolCall:
									return this._renderToolCallToMarkdown(
										entry,
									);
								case LoggedInfoKind.Request:
									return this._renderRequestToMarkdown(
										entry.id,
										entry.entry,
									);
								default:
	...
	public override logModelListCall(
		...
		this.addEntry({
			...
			markdownContent: this._renderModelListToMarkdown(
				id,
				requestMetadata,
				models,
			),
	...
	public override logContentExclusionRules(
		repos: string[],
		rules: {
			patterns: string[];
			ifAnyMatch: string[];
		...
		this.addEntry({
			...
			startTimeMs: Date.now(),
			icon: Codicon.shield,
			markdownContent: this._renderContentExclusionToMarkdown(
				repos,
				rules,
				durationMs,
			),
		...
		this._addEntry(new LoggedRequestInfo(id, entry, this.currentRequest))
			.then((ok) => {
				if (ok) {
					...
					if (
						entry.type === LoggedRequestKind.MarkdownContentRequest
					) {
						extraData = 'markdown';
					} else {
						const status =
						...
						if (
							entry.type === LoggedRequestKind.ChatMLSuccess &&
							entry.result.resolvedModel &&
							entry.result.resolvedModel !==
								entry.chatEndpoint.model
						) {
					...
					this._logService.info(
						`${ChatRequestScheme.buildUri({ kind: 'request', id: id })} | ${extraData}`,
					);
				}
			})
			.catch((e) => this._logService.error(e));
	...
	private _shouldLog(entry: LoggedRequest) {
		...
		if (
			entry.debugName === XTabProviderId &&
			!this._configService.getConfig(
				ConfigKey.TeamInternal.InlineEditsLogCancelledRequests,
			) &&
			entry.type === LoggedRequestKind.ChatMLCancelation
		) {
			return false;
	...
	private async _addEntry(entry: LoggedInfo): Promise<boolean> {
		if (this._isFirst) {
			this._isFirst = false;
			this._logService.info(
				`Latest entry: ${ChatRequestScheme.buildUri({ kind: 'latest' })}`,
			);
		}

		this._entries.push(entry);
		const maxEntries = this._configService.getConfig(
			ConfigKey.Advanced.RequestLoggerMaxEntries,
		);
	...
	private _ensureLinkProvider(): void {
		...
		const docLinkProvider = new (class implements DocumentLinkProvider {
			provideDocumentLinks(
				...
				return ChatRequestScheme.findAllUris(td.getText()).map(
					(u) =>
						new DocumentLink(
							new Range(
								td.positionAt(u.range.start),
								td.positionAt(u.range.endExclusive),
							),
							Uri.parse(u.uri),
						),
				);
...
`;
	...
	private async _renderToolCallToMarkdown(entry: ILoggedToolCall) {
		...
		if (entry.thinking?.text) {
			...
			result.push(`~~~`);
			result.push(
				Array.isArray(entry.thinking.text)
					? entry.thinking.text.join('\n')
					: entry.thinking.text,
			);
			result.push(`~~~`);
		}
	...
	private _renderRequestToMarkdown(id: string, entry: LoggedRequest): string {
		...
		if (typeof entry.chatEndpoint.urlOrRequestMetadata === 'string') {
			result.push(
				`url              : ${entry.chatEndpoint.urlOrRequestMetadata}`,
			);
		} else if (entry.chatEndpoint.urlOrRequestMetadata) {
			result.push(
				`requestType      : ${entry.chatEndpoint.urlOrRequestMetadata?.type}`,
			);
		}
		result.push(`model            : ${entry.chatParams.model}`);
		result.push(
			`maxPromptTokens  : ${entry.chatEndpoint.modelMaxPromptTokens}`,
		);
		result.push(
			`maxResponseTokens: ${entry.chatParams.body?.max_tokens ?? entry.chatParams.body?.max_output_tokens ?? entry.chatParams.body?.max_completion_tokens}`,
		);
		...
		if (entry.chatParams.body?.reasoning) {
			result.push(
				`reasoning        : ${JSON.stringify(entry.chatParams.body.reasoning)}`,
			);
		}
		...
		if (!ignoreStatefulMarker) {
			const statefulMarker = Iterable.first(
				getAllStatefulMarkersAndIndicies(entry.chatParams.messages),
			);
			if (statefulMarker) {
				result.push(
					`lastResponseId   : ${statefulMarker.statefulMarker.marker} using ${statefulMarker.statefulMarker.modelId}`,
				);
			}
		}
		...
		if (entry.type === LoggedRequestKind.ChatMLSuccess) {
			...
			if (
				typeof entry.usage?.copilot_usage?.total_nano_aiu === 'number'
			) {
				const aic =
					entry.usage.copilot_usage.total_nano_aiu / 1_000_000_000;
				result.push(
					`copilotUsage    : ${aic.toFixed(2)} AIC (${entry.usage.copilot_usage.total_nano_aiu} nano-AIU)`,
				);
			}
		...
		if (typeof entry.chatParams.body?.prediction?.content === 'string') {
			...
			result.push(
				createFencedCodeBlock(
					'markdown',
					entry.chatParams.body.prediction.content,
					false,
				),
			);
		}
		...
		if (entry.type === LoggedRequestKind.ChatMLSuccess) {
			...
			if (entry.deltas?.length) {
				result.push(
					this._renderDeltasToMarkdown('assistant', entry.deltas),
				);
			} else {
				...
				result.push(
					this._renderStringMessageToMarkdown('assistant', message),
				);
			}
		} else if (entry.type === LoggedRequestKind.ChatMLFailure) {
			result.push(``);
			...
			if (entry.result.type === ChatFetchResponseType.Length) {
				...
				result.push(
					this._renderStringMessageToMarkdown(
						'assistant',
						entry.result.truncatedValue,
					),
				);
			} else {
	...
	private _renderStringMessageToMarkdown(
		role: string,
		message: string,
	): string {
	...
	private _renderDeltasToMarkdown(
		role: string,
	...
	private _renderModelListToMarkdown(
		requestId: string,
		requestMetadata: RequestMetadata,
		models: IModelAPIResponse[],
	): string {
		...
		result.push(
			`isModelLab       : ${(requestMetadata as { type: string; isModelLab?: boolean }) ? 'yes' : 'no'}`,
		);
		if (requestMetadata.type === RequestType.ListModel) {
			result.push(
				`requestedModel   : ${(requestMetadata as { type: string; modelId: string })?.modelId || 'unknown'}`,
			);
		}
		...
		if (models.length > 0) {
			...
			result.push(
				`Chat models      : ${models.filter((m) => m.capabilities.type === 'chat').length}`,
			);
			result.push(
				`Completion models: ${models.filter((m) => m.capabilities.type === 'completion').length}`,
			);
			result.push(
				`Premium models   : ${models.filter((m) => m.billing?.is_premium).length}`,
			);
			result.push(
				`Preview models   : ${models.filter((m) => m.preview).length}`,
			);
			result.push(
				`Default chat     : ${models.find((m) => m.is_chat_default)?.id || 'none'}`,
			);
			result.push(
				`Fallback chat    : ${models.find((m) => m.is_chat_fallback)?.id || 'none'}`,
			);
			result.push(`~~~`);
		}
	...
	private _renderContentExclusionToMarkdown(
		repos: string[],
		rules: {
			patterns: string[];
			ifAnyMatch: string[];
		...
		for (let i = 0; i < repos.length; i++) {
			...
			if (
				repoRules.patterns.length === 0 &&
				repoRules.ifAnyMatch.length === 0 &&
				repoRules.ifNoneMatch.length === 0
			) {
			...
			if (repoRules.ifAnyMatch.length > 0) {
				result.push(
					`### ifAnyMatch Regex (${repoRules.ifAnyMatch.length})`,
				);
				result.push(`~~~`);
			...
			if (repoRules.ifNoneMatch.length > 0) {
				result.push(
					`### ifNoneMatch Regex (${repoRules.ifNoneMatch.length})`,
				);
				result.push(`~~~`);
	...
	private _renderRawRequestToJson(entry: LoggedInfo): string {
		...
		if (
			req.type === LoggedRequestKind.MarkdownContentRequest ||
			!req.chatParams.body
		) {
			return 'Not available';
		}
