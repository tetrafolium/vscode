...
export class GitCommitMessageServiceImpl implements IGitCommitMessageService {
	...
	private readonly _commitMessages = new Map<
		string,
		Map<string, CommitMessage>
	>();
	...
	constructor(
		...
		const initialize = () => {
			this._disposables.add(
				this._gitExtensionApi!.onDidOpenRepository(
					this._onDidOpenRepository,
					this,
				),
			);
			this._disposables.add(
				this._gitExtensionApi!.onDidCloseRepository(
		...
		if (this._gitExtensionApi) {
			...
			this._disposables.add(
				this._gitExtensionService.onDidChange((status) => {
					if (status.enabled) {
						this._gitExtensionApi =
	...
	async generateCommitMessage(
		...
		return workspace.isAgentSessionsWorkspace
			...
			: window.withProgress(
					...
					async () => {
						return this._doGenerateCommitMessage(
							repository,
							cancellationToken,
						);
					},
	...
	private async _doGenerateCommitMessage(
		repository: Repository,
		...
		const changes = await this._gitDiffService.getChangeDiffs(
			repository,
			resources,
		);
		...
		if (changes.length === 0) {
			window.showInformationMessage(
				l10n.t(
					'Cannot generate a commit message because the changes were excluded from the context due to content exclusion rules.',
				),
			);
			return undefined;
		}
		...
		const recentCommitMessages =
			await this._getRecentCommitMessages(repository);

		const repositoryName = basename(repository.rootUri);
		const branchName = repository.state.HEAD?.name ?? '';
		const gitCommitMessageGenerator =
			this._instantiationService.createInstance(
				GitCommitMessageGenerator,
			);
		const commitMessage =
			await gitCommitMessageGenerator.generateGitCommitMessage(
				repositoryName,
				branchName,
				changes,
		...
		if (
			...
			const commitMessages =
				this._commitMessages.get(repository.rootUri.toString()) ??
				new Map<string, CommitMessage>();
			commitMessages.set(repository.state.HEAD.commit, {
				attemptCount,
				changes: diffs,
				message: commitMessage,
			});

			this._commitMessages.set(
	...
	async getRepository(uri?: Uri): Promise<Repository | null> {
		...
		if (
			uri === undefined &&
			this._gitExtensionApi.repositories.length === 1
		) {
	...
	private _getAttemptCount(
		repository: Repository,
		changes: string[],
	): number {
		const commitMessages = this._commitMessages.get(
			repository.rootUri.toString(),
		);
		const commitMessage = commitMessages?.get(
			repository.state.HEAD?.commit ?? '',
		);
	...
	private async _getRecentCommitMessages(
		repository: Repository,
	): Promise<RecentCommitMessages> {
	...
	private _onDidOpenRepository(repository: Repository): void {
		if (typeof repository.onDidCommit !== undefined) {
			this._repositoryDisposables.set(
				repository,
				repository.onDidCommit(
					() => this._onDidCommit(repository),
					this,
				),
			);
		}
	...
	private async _onDidCommit(repository: Repository): Promise<void> {
		...
		const survivalRateFourGram = compute4GramTextSimilarity(
			commit.message,
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent(
			'git.generateCommitMessageSurvival',
			undefined,
			{ attemptCount: commitMessage.attemptCount, survivalRateFourGram },
		);
