...
export class SettingsEditorSearchServiceImpl implements ISettingsEditorSearchService {
	...
	async provideSettingsSearchResults(
		query: string,
		...
		try {
			embeddingResult = await this.embeddingsComputer.computeEmbeddings(
				EmbeddingType.text3small_512,
				[query],
				{},
				new TelemetryCorrelationId(
					'SettingsEditorSearchServiceImpl::provideSettingsSearchResults',
				),
				token,
			);
		} catch {
		...
		if (
			...
			!embeddingResult ||
			embeddingResult.values.length === 0
		) {
			this.reportEmptyEmbeddingsResult(query, progress);
			if (!options.embeddingsOnly) {
				this.reportEmptyLLMRankedResult(query, progress);
		...
		const embeddingSettings: SettingListItem[] =
			this.embeddingIndex.settingsIndex.nClosestValues(
		...
		if (
			embeddingSettings.length === 0 ||
			copilotToken.isFreeUser ||
			copilotToken.isNoAuthUser
		) {
			this.reportEmptyLLMRankedResult(query, progress);
			return;
		}
		...
		const endpoint =
			await this.endpointProvider.getChatEndpoint(endpointName);
		const generator = this.instantiationService.createInstance(
			SettingsEditorSearchResultsSelector,
		);
		const llmSearchSuggestions = await generator.selectTopSearchResults(
			endpoint,
			query,
			embeddingSettings,
			token,
	...
	private reportEmptyEmbeddingsResult(
		query: string,
		progress: Progress<SettingsSearchResult>,
	): void {
		progress.report({
	...
	private reportEmptyLLMRankedResult(
		query: string,
		progress: Progress<SettingsSearchResult>,
	): void {
		progress.report({
