...
} from '../../onboardDebug/common/launchConfigService';
	...
	constructor(
		...
		this._register(
			vscode.commands.registerCommand(
				...
				async () => {
					...
					const lastTurn =
						this._conversationStore.lastConversation?.getLatestTurn();
					if (lastTurn) {
		...
		this._register(
			vscode.commands.registerCommand(
				...
				async (launchConfig: IStartDebuggingParsedResponse) => {
		...
		this._register(
			vscode.commands.registerCommand(
				...
				async (config: IStartDebuggingParsedResponse, progress) => {
					const result =
					...
					if (result?.config) {
						...
						progress.markdown(
							vscode.l10n.t(
								'Could not start debugging. Please try again.',
							),
						);
						return;
					}
