...
import {
	getTextPart,
	toTextParts,
} from '../../../platform/chat/common/globalStringUtils';
...
} from '../../../platform/networking/node/chatWebSocketManager';
...
import {
	...
	ChatResults,
	FetchResponseKind,
} from '../../../platform/openai/node/fetch';
import {
	collectSystemTextsFromRequestBody,
	CopilotChatAttr,
...
import {
	calculateLineRepetitionStats,
	isRepetitive,
} from '../../../util/common/anomalyDetection';
...
export abstract class AbstractChatMLFetcher
	extends Disposable
	implements IChatMLFetcher
{
	declare _serviceBrand: undefined;

	constructor(protected readonly options: IConversationOptions) {
		super();
	}

	protected preparePostOptions(
	...
	protected readonly _onDidMakeChatMLRequest = this._register(
		new Emitter<IMadeChatRequestEvent>(),
	);
...
export class ChatMLFetcherImpl extends AbstractChatMLFetcher {
	...
	constructor(
		...
		@IChatWebSocketManager
		private readonly _webSocketManager: IChatWebSocketManager,
	...
	public async fetchMany(
		...
		if (
			...
			this._consecutiveWebSocketRetryFallbacks >=
				ChatMLFetcherImpl._maxConsecutiveWebSocketFallbacks
		) {
			this._logService.debug(
				`[ChatWebSocketManager] Disabling WebSocket for request due to ${this._consecutiveWebSocketRetryFallbacks} consecutive WebSocket failures with successful HTTP fallback.`,
			);
			useWebSocket = false;
		...
		const imageTelemetryMeasurements =
			getImageTelemetryMeasurementsFromMessages(messages);
		...
		const enableRetryOnError =
			opts.enableRetryOnError ?? opts.enableRetryOnFilter;
		const canRetryOnce =
			opts.canRetryOnceWithoutRollback ??
			!(opts.enableRetryOnFilter || opts.enableRetryOnError);
		...
		try {
			...
			const payloadValidationResult = isValidChatPayload(
				opts.messages,
				postOptions,
				chatEndpoint,
			...
			if (!payloadValidationResult.isValid) {
				...
				const countTokens = () =>
					(tokenCountPromise ??= chatEndpoint
						.acquireTokenizer()
				...
				usernameToScrub =
					copilotToken?.username ??
					this._authenticationService.copilotToken?.username;
				...
				otelInferenceSpan?.setAttribute(
				...
				if (otelInferenceSpan) {
					...
					const userMessages = capiMessages?.filter(
						(m) => m.role === 'user',
					);
					...
					const systemTexts =
						collectSystemTextsFromRequestBody(requestBody);
					const systemInstructions =
						toSystemInstructions(systemTexts);
				...
				if (otelInferenceSpan) {
					...
					if (capiMessages) {
						...
						const nonSystemMessages = capiMessages.filter(
							(m) => (m as { role?: unknown }).role !== 'system',
						);
						otelInferenceSpan.setAttribute(
					...
					const toolDefsJson = stringifyToolDefinitionsForOTel(
						requestBody.tools,
					);
					if (toolDefsJson) {
						otelInferenceSpan.setAttribute(
					...
					const requestOptions =
						pickCacheRelevantRequestOptions(requestBody);
					if (requestOptions) {
						otelInferenceSpan.setAttribute(
			...
			switch (response.type) {
				case FetchResponseKind.Success: {
					...
					if (result.type === ChatFetchResponseType.FilteredRetry) {
						if (opts.enableRetryOnFilter) {
							streamRecorder.callback('', 0, {
								text: '',
								retryReason: result.category,
							});

							const filteredContent = result.value[0];
							if (filteredContent) {
								const retryMessage =
									result.category === FilterReason.Copyright
					...
					if (
						...
						const normalizedResponseModel = normalizeResponseModel(
							chatEndpoint.model,
						...
						otelInferenceSpan?.setAttributes({
							...
							...(result.serverRequestId
								? {
										[CopilotChatAttr.SERVER_REQUEST_ID]:
											result.serverRequestId,
									}
								: {}),
							...(result.usage.completion_tokens_details
								?.reasoning_tokens
								? {
										[GenAiAttr.USAGE_REASONING_TOKENS]:
											result.usage
												.completion_tokens_details
												.reasoning_tokens,
										[GenAiAttr.USAGE_REASONING_OUTPUT_TOKENS]:
											result.usage
												.completion_tokens_details
												.reasoning_tokens,
									}
					...
					if (
						...
						const hasThinking = streamRecorder.deltas.some(
							(d) => d.thinking,
						);
						if (hasThinking) {
							const thinkingTexts = streamRecorder.deltas
								.filter(
									(d) =>
										d.thinking &&
										!isEncryptedThinkingDelta(d.thinking) &&
										d.thinking.text,
								)
								.map((d) => {
									const t = d.thinking!;
							...
							otelInferenceSpan.setAttribute(
								CopilotChatAttr.REASONING_CONTENT,
								truncateForOTel(
									reasoningText || '[encrypted]',
									this._otelService.config
										.maxAttributeSizeChars,
								),
							);
					...
					if (timeToFirstToken > 0) {
						GenAiMetrics.recordTimeToFirstToken(
							this._otelService,
							chatEndpoint.model,
				...
				case FetchResponseKind.Canceled:
					Telemetry.sendCancellationTelemetry(
						...
						{
							source:
								telemetryProperties.messageSource ?? 'unknown',
							requestId: ourRequestId,
							model: chatEndpoint.model,
							...
							interactionType,
							conversationId:
								telemetryProperties.conversationId ??
								conversationId,
							associatedRequestId:
								telemetryProperties.associatedRequestId,
							parentRequestId:
								telemetryProperties.parentRequestId,
							retryAfterError:
						...
						{
							totalTokenMax:
								chatEndpoint.modelMaxPromptTokens ?? -1,
							promptTokenCount: tokenCount,
							tokenCountMax: maxResponseTokens,
							timeToFirstToken,
							timeToFirstTokenEmitted:
								baseTelemetry &&
								streamRecorder.firstTokenEmittedTime
									? streamRecorder.firstTokenEmittedTime -
										baseTelemetry.issuedTime
									: -1,
							timeToCancelled:
								Date.now() - baseTelemetry.issuedTime,
					...
					otelInferenceSpan?.end();
					otelInferenceSpan = undefined;
					return this.processCanceledResponse(
						response,
						ourRequestId,
						streamRecorder,
						telemetryProperties,
				...
				case FetchResponseKind.Failed: {
					const processed = this.processFailedResponse(
						response,
						ourRequestId,
						isAutoModel(chatEndpoint) === 1,
					);
					// Retry on server errors based on configured status codes
					const retryServerErrorStatusCodes =
					...
					const retryAfterServerError =
						enableRetryOnError &&
						actualStatusCode !== undefined &&
						statusCodesToRetry.includes(actualStatusCode);
					const retryWithoutWebSocket =
						enableRetryOnError &&
						useWebSocket &&
						(response.failKind === ChatFailKind.ServerError ||
							response.failKind === ChatFailKind.Unknown);
			...
			if (otelInferenceSpan) {
				...
				otelInferenceSpan.setAttribute(
					GenAiAttr.RESPONSE_FINISH_REASONS,
					['error'],
				);
			...
			const processed = this.processError(
				err,
				ourRequestId,
				err.gitHubRequestId,
				usernameToScrub,
			...
			const retryWithoutWebSocket =
				...
				useWebSocket &&
				(processed.type === ChatFetchResponseType.NetworkError ||
					processed.type === ChatFetchResponseType.Failed);
			if (retryNetworkError || retryWithoutWebSocket) {
				const {
					retryResult,
					connectivityTestError,
					connectivityTestErrorGitHubRequestId,
				} = await this._retryAfterError({
					opts,
					processed,
					telemetryProperties,
					requestBody,
	...
	private async _checkNetworkConnectivity(
		...
		let connectivityTestErrorGitHubRequestId: string | undefined =
			undefined;
		for (const delay of delays) {
			this._logService.info(
				`Waiting ${delay}ms before pinging CAPI to check network connectivity...`,
			);
			await new Promise((resolve) => setTimeout(resolve, delay));
			try {
				const isGHEnterprise =
				...
				if (res.status >= 200 && res.status < 300) {
					...
					this._logService.info(
						`CAPI ping returned status ${res.status}, retrying ping...`,
					);
				}
			} catch (err) {
				connectivityTestError = collectSingleLineErrorMessage(
					err,
					true,
				);
				connectivityTestErrorGitHubRequestId = undefined; // no response headers yet
				this._logService.info(
					`CAPI ping failed with error, retrying ping: ${connectivityTestError}`,
				);
			}
		}
		return {
	...
	private async _getAuthHeaders(isGHEnterprise: boolean, url: string) {
		...
		if (isGHEnterprise) {
			...
			if (url === this._capiClientService.dotcomAPIURL) {
				token =
					this._authenticationService.anyGitHubSession?.accessToken ||
					'';
			} else {
				try {
					token = (
	...
	private async _retryAfterError(params: {
		...
		const isNetworkChangedError =
			...
			processed.reason.indexOf('net::ERR_NETWORK_CHANGED') !== -1;
		...
		const isNetworkProcessCrash =
			processed.type === ChatFetchResponseType.NetworkError &&
			processed.isNetworkProcessCrash === true &&
			fallbackEnabled;
		const useFetcher =
			isNetworkChangedError || isNetworkProcessCrash
				? 'node-fetch'
				: opts.useFetcher;
		this._logService.info(
			`Retrying chat request with ${useFetcher || 'default'} fetcher after: ${processed.reasonDetail || processed.reason}`,
		);
		const connectivity = await this._checkNetworkConnectivity(useFetcher);
		const connectivityTestError = connectivity.connectivityTestError
			? this.scrubErrorDetail(
					connectivity.connectivityTestError,
					usernameToScrub,
				)
			: undefined;
		const connectivityTestErrorGitHubRequestId =
			connectivity.connectivityTestErrorGitHubRequestId;
		if (!connectivity.retryRequest) {
			this._logService.info(
				`Not retrying chat request as network connectivity could not be re-established.`,
			);
			return {
				connectivityTestError,
				connectivityTestErrorGitHubRequestId,
		...
		if (
			opts.useWebSocket &&
			retryResult.type === ChatFetchResponseType.Success
		) {
			this._consecutiveWebSocketRetryFallbacks++;
			this._logService.info(
				`[ChatWebSocketManager] WebSocket request failed with successful HTTP fallback (${this._consecutiveWebSocketRetryFallbacks} consecutive).`,
			);
	...
	private async _fetchAndStreamChat(
		...
		result: ChatResults | ChatRequestFailed | ChatRequestCanceled;
		fetcher?: FetcherId;
		bytesReceived?: number;
		statusCode?: number;
		suspendEventSeen?: boolean;
		resumeEventSeen?: boolean;
		otelSpan?: ISpanHandle;
		modelCallId?: string;
	}> {
		const isPowerSaveBlockerEnabled =
			this._configurationService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.ChatRequestPowerSaveBlocker,
				this._experimentationService,
			);
		const blockerHandle =
			isPowerSaveBlockerEnabled && location !== ChatLocation.Other
				? this._powerService.acquirePowerSaveBlocker()
				: undefined;
		...
		const suspendListener = this._powerService.onDidSuspend(() => {
			suspendEventSeen = true;
			this._logService.info(
				`System suspended during streaming request ${ourRequestId} (${ChatLocation.toString(location)})`,
			);
		});
		...
		const resumeListener = this._powerService.onDidResume(() => {
			resumeEventSeen = true;
			this._logService.info(
				`System resumed during streaming request ${ourRequestId} (${ChatLocation.toString(location)})`,
			);
		});
		...
		try {
			...
			return {
				...fetchResult,
				suspendEventSeen: suspendEventSeen || undefined,
				resumeEventSeen: resumeEventSeen || undefined,
			};
		} catch (err) {
			if (suspendEventSeen) {
	...
	private async _doFetchAndStreamChat(
		...
		const serverAddress =
			typeof chatEndpointInfo.urlOrRequestMetadata === 'string'
				? (() => {
						try {
							return new URL(
								chatEndpointInfo.urlOrRequestMetadata,
							).hostname;
						} catch {
							return undefined;
						}
		...
		const otelSpan = this._otelService.startSpan(
			`chat ${chatEndpointInfo.model}`,
			{
				...
				attributes: {
					...
					[GenAiAttr.CONVERSATION_ID]:
						telemetryProperties?.requestId ?? ourRequestId,
					...
					...(request.top_p !== undefined
						? { [GenAiAttr.REQUEST_TOP_P]: request.top_p }
						: {}),
					[CopilotChatAttr.MAX_PROMPT_TOKENS]:
						chatEndpointInfo.modelMaxPromptTokens,
					...(serverAddress
						? { [StdAttr.SERVER_ADDRESS]: serverAddress }
						: {}),
					...(conversationId
						? { [CopilotChatAttr.SESSION_ID]: conversationId }
						: {}),
					...(chatSessionId
		...
		try {
			this._logService.debug(
				`modelMaxPromptTokens ${chatEndpointInfo.modelMaxPromptTokens}`,
			);
			this._logService.debug(
				`modelMaxResponseTokens ${request.max_tokens ?? 2048}`,
			);
			...
			if (!secretKey && !chatEndpointInfo.ownsAuthorization) {
				const urlOrRequestMetadata = stringifyUrlOrRequestMetadata(
					chatEndpointInfo.urlOrRequestMetadata,
				);
				this._logService.error(
					`Failed to send request to ${urlOrRequestMetadata} due to missing key`,
				);
			...
			GenAiMetrics.recordOperationDuration(
				this._otelService,
				durationSec,
				{
					operationName: GenAiOperationName.CHAT,
	...
	private async _doFetchViaWebSocket(
		...
		const additionalHeaders: Record<string, string> = {
			...
			...(chatEndpointInfo.getExtraHeaders
				? chatEndpointInfo.getExtraHeaders(
						location,
						interactionTypeOverride,
					)
		...
		if (
			request.messages?.some((m: CAPIChatMessage) =>
				Array.isArray(m.content)
					? m.content.some((c) => 'image_url' in c)
					: false,
			) &&
			chatEndpointInfo.supportsVision
		) {
			additionalHeaders['Copilot-Vision-Request'] = 'true';
		}
		const connection = this._webSocketManager.getOrCreateConnection(
			conversationId,
			additionalHeaders,
			ourRequestId,
		);
		...
		if (modelRequestId.serverExperiments) {
			this._telemetryService.setSharedProperty(
		...
		for (const [key, value] of Object.entries(request)) {
			...
			if (key === 'tools') {
				telemetryData.properties[`request.option.${key}`] =
					stringifyToolsRawForTelemetry(
						value as ReadonlyArray<unknown> | undefined,
					) ?? 'undefined';
				continue;
			}
			telemetryData.properties[`request.option.${key}`] =
		...
		const handle = connection.sendRequest(
			request,
			{
				userInitiated: !!userInitiatedRequest,
				turnId,
				requestId: ourRequestId,
				model: chatEndpointInfo.model,
				countTokens,
				tokenCountMax: chatEndpointInfo.maxOutputTokens,
				modelMaxPromptTokens: chatEndpointInfo.modelMaxPromptTokens,
				summarizedAtRoundId,
				modeChanged,
			},
		...
		const extendedBaseTelemetryData = baseTelemetryData.extendedBy({
			modelCallId,
		});
		const processor = this._instantiationService.createInstance(
			OpenAIResponsesProcessor,
			extendedBaseTelemetryData,
			this._telemetryService,
			modelRequestId.headerRequestId,
			modelRequestId.gitHubRequestId,
			modelRequestId.serverExperiments,
		...
		const chatCompletions = new AsyncIterableObject<ChatCompletion>(
			async (emitter) => {
				try {
					await new Promise<void>((resolve, reject) => {
						handle.onEvent((event) => {
							...
							if (event.type === 'response.completed') {
								const snapshots = (event as any)
									.copilot_quota_snapshots;
								if (
									snapshots &&
									typeof snapshots === 'object'
								) {
									this._chatQuotaService.processQuotaSnapshots(
										snapshots,
									);
								}
							}
						...
						handle.onError((error) => {
							...
							this._telemetryService.sendGHTelemetryEvent(
								...
								warningTelemetry.properties,
								warningTelemetry.measurements,
							);

							const totalTimeMs = Date.now() - requestStart;
							telemetryData.measurements.totalTimeMs =
								totalTimeMs;
							telemetryData.properties.error = error.message;

							this._logService.debug(
								`request.error: [websocket], took ${totalTimeMs} ms`,
							);
							this._telemetryService.sendGHTelemetryEvent(
								'request.error',
								telemetryData.properties,
					...
					this._logService.debug(
						`request.response: [websocket], took ${totalTimeMs} ms`,
					);
					this._telemetryService.sendGHTelemetryEvent(
						'request.response',
						telemetryData.properties,
					...
					if (
						(!messagesToLog || messagesToLog.length === 0) &&
						(request as OpenAI.Responses.ResponseCreateParams).input
					) {
						try {
							const rawMessages =
								responseApiInputToRawMessagesForLogging(
									request as OpenAI.Responses.ResponseCreateParams,
								);
							messagesToLog = rawMessageToCAPI(rawMessages);
						} catch (e) {
							this._logService.error(
								`Failed to convert Response API input to messages for telemetry:`,
								e,
							);
							messagesToLog = [];
						}
		...
		if (cancellationToken.isCancellationRequested) {
			return {
				result: {
					type: FetchResponseKind.Canceled,
					reason: 'after first WebSocket event',
				},
			};
		}
		...
		if (isCAPIWebSocketError(firstEvent)) {
			...
			this._logService.debug(
				`request.error: [websocket capi error], took ${totalTimeMs} ms`,
			);
			this._telemetryService.sendGHTelemetryEvent(
				'request.error',
				telemetryData.properties,
			...
			return {
				result: await this._handleWebSocketCAPIError(
					firstEvent,
					modelRequestId,
				),
			};
		...
		if (
			this._authenticationService.copilotToken?.isFreeUser &&
			this._authenticationService.copilotToken?.isChatQuotaExceeded
		) {
	...
	private async _doFetchViaHttp(
		...
		if (response.status !== 200) {
			...
			this._logService.info(
				'Request ID for failed request: ' + ourRequestId,
			);
			return {
				result: await this._handleError(
					telemetryData,
					response,
					ourRequestId,
				),
		...
		try {
			const completions =
				await chatEndpointInfo.processResponseFromChatEndpoint(
					this._telemetryService,
					this._logService,
					response,
			...
			chatCompletions = new AsyncIterableObject<ChatCompletion>(
				async (emitter) => {
					try {
		...
		if (response.headers.get('Copilot-Edits-Session')) {
			this._authenticationService.speculativeDecodingEndpointToken =
				response.headers.get('Copilot-Edits-Session') ?? undefined;
		}
	...
	private async _fetchWithInstrumentation(
		...
		return this._instantiationService
			...
			.then((response) => {
				...
				this._logService.debug(
					`request.response: [${stringifyUrlOrRequestMetadata(chatEndpoint.urlOrRequestMetadata)}], took ${totalTimeMs} ms`,
				);
	...
	private async _handleError(
		...
		if (400 <= response.status && response.status < 500) {
			...
			if (response.status === 401 || response.status === 403) {
				...
				return {
					...
					reason:
						jsonData?.message ||
						`token expired or invalid: ${response.status}`,
				};
			}
			...
			if (response.status === 429) {
				...
				if (
					text.includes('extension_blocked') &&
					jsonData?.code === 'extension_blocked' &&
					jsonData?.type === 'rate_limit_error'
				) {
					return {
						type: FetchResponseKind.Failed,
						modelRequestId: modelRequestIdObj,
				...
				return {
					...
					data: {
						retryAfter: response.headers.get('retry-after'),
						rateLimitKey: response.headers.get(
			...
			if (response.status === 503) {
				return {
					...
					data: {
						...
						rateLimitKey: null,
						capiError: {
							code: 'upstream_provider_rate_limit',
							message: text,
						},
					},
				};
		...
		sendCommunicationErrorTelemetry(
			this._telemetryService,
			'Unhandled status from server: ' + response.status,
			text,
		);
	...
	private async processSuccessfulResponse(
		...
		for await (const chatCompletion of response.chatCompletions) {
			Telemetry.sendSuccessTelemetry(this._telemetryService, {
				chatCompletion,
				baseTelemetry,
				userInitiatedRequest,
				interactionType,
				...
				timeToFirstToken,
				timeToFirstTokenEmitted:
					baseTelemetry && streamRecorder.firstTokenEmittedTime
						? streamRecorder.firstTokenEmittedTime -
							baseTelemetry.issuedTime
						: -1,
		...
		const successFinishReasons = new Set([
		...
		const successfulCompletions = completions.filter((c) =>
			successFinishReasons.has(c.finishReason),
		);
			...
			return {
				...
				usage:
					successfulCompletions.length === 1
						? successfulCompletions[0].usage
						: undefined,
				value: successfulCompletions.map((c) =>
					getTextPart(c.message.content),
				),
				requestId,
				serverRequestId:
					successfulCompletions[0].requestId.headerRequestId,
				modelCallId,
			};
		}
	...
	private filterImageMessages(messages: Raw.ChatMessage[]): boolean {
		return messages?.some((m) =>
			Array.isArray(m.content)
				? m.content.some((c) => 'imageUrl' in c)
				: false,
		);
	}
	...
	private isRepetitive(
		...
		const lineRepetitionStats = calculateLineRepetitionStats(
			getTextPart(chatCompletion.message.content),
		);
		...
		if (lineRepetitionStats.numberOfRepetitions >= 10) {
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent(
				...
				{
					numberOfRepetitions:
						lineRepetitionStats.numberOfRepetitions,
	...
	private checkRepetitionInDeltas(
		...
		const textContent = deltas
			.filter((delta) => delta.text?.length > 0)
	...
	private processFailedResponse(
		...
		if (response.failKind === ChatFailKind.RateLimited) {
			return {
				...
				rateLimitKey: response.data?.rateLimitKey || '',
				isAuto,
				capiError: response.data?.capiError,
			};
		}
		...
		if (
			response.failKind === ChatFailKind.TokenExpiredOrInvalid ||
			response.failKind === ChatFailKind.ClientNotSupported ||
		...
		if (response.failKind === ChatFailKind.AgentUnauthorized) {
			return {
				...
				reason,
				authorizationUrl: response.data!.authorize_url,
				requestId,
				serverRequestId,
			};
		}
		...
		if (response.failKind === ChatFailKind.ExtensionBlocked) {
			...
			return {
				...
				requestId,
				retryAfter,
				learnMoreLink: response.data?.learnMoreLink ?? '',
				serverRequestId,
			};
		}
	...
	private processError(
		...
		const capiWebSocketError = (err as any)?.capiWebSocketError as
			| CAPIWebSocketErrorEvent
			| undefined;
		if (capiWebSocketError) {
			return this._handleWebSocketError(
				capiWebSocketError,
				requestId,
				gitHubRequestId,
				isAuto,
		...
		if (
			...
			((err instanceof Error && err.message === 'Premature close') ||
				(typeof err === 'object' &&
					(err as any).code ===
						'ERR_STREAM_PREMATURE_CLOSE')) /* to be extra sure */
		) {
			return {
				type: ChatFetchResponseType.Canceled,
		...
		const scrubbedErrorDetail = this.scrubErrorDetail(
			errorDetail,
			usernameToScrub,
		);
		if (fetcher.isInternetDisconnectedError(err)) {
			...
			const isNetworkProcessCrash =
				fetcher.isNetworkProcessCrashedError(err);
			return {
				type: ChatFetchResponseType.NetworkError,
				reason: userMessage,
				reasonDetail: scrubbedErrorDetail,
				requestId: requestId,
				serverRequestId: gitHubRequestId,
				...(isNetworkProcessCrash
					? { isNetworkProcessCrash: true }
					: {}),
			};
		} else {
			return {
	...
	private async _handleWebSocketCAPIError(
		event: CAPIWebSocketErrorEvent,
		...
		if (
			codePrefix === 'rate_limited' ||
			codePrefix === 'user_model_rate_limited' ||
			codePrefix === 'user_global_rate_limited' ||
			codePrefix === 'integration_rate_limited' ||
			codePrefix === 'model_overloaded' ||
			codePrefix === 'agent_mode_limit_exceeded'
		) {
			return {
				type: FetchResponseKind.Failed,
				modelRequestId,
	...
	private scrubErrorDetail(
		...
		if (usernameToScrub) {
			const regex = new RegExp(
				escapeRegExpCharacters(usernameToScrub),
				'ig',
			);
			errorDetail = errorDetail.replaceAll(regex, '<login>');
		}
		return errorDetail.replaceAll(
			/(?<=logged in as )(?!<login>)[^\s]+/gi,
			'!<login>!',
		); // marking fallback with !
	}
}
...
function isValidChatPayload(
	...
	if (postOptions?.max_tokens && postOptions?.max_tokens < 1) {
		return {
			isValid: false,
			reason: asUnexpected('Invalid response token parameter'),
		};
	}

	const functionNamePattern = /^[a-zA-Z0-9_-]+$/;
	if (
		postOptions?.functions?.some(
			(f) => !f.name.match(functionNamePattern),
		) ||
		(postOptions?.function_call?.name &&
			!postOptions.function_call.name.match(functionNamePattern))
	) {
		return {
			isValid: false,
			reason: asUnexpected('Function names must match ^[a-zA-Z0-9_-]+$'),
		};
	}

	if (
		postOptions?.tools &&
		postOptions.tools.length > HARD_TOOL_LIMIT &&
		!endpoint.supportsToolSearch
	) {
		return {
			isValid: false,
			reason: `Tool limit exceeded (${postOptions.tools.length}/${HARD_TOOL_LIMIT}). Click "Configure Tools" in the chat input to disable ${postOptions.tools.length - HARD_TOOL_LIMIT} tools and retry.`,
		};
	}
 ...
 */
function pickCacheRelevantRequestOptions(
	body: IEndpointBody,
 ...
 */
function pickRequestShapeMetadata(
	body: IEndpointBody,
): Record<string, unknown> | undefined {
	const out: Record<string, unknown> = {};
