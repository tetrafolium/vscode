...
import {
	...
	DevContainerConfigGeneratorResult,
	DevContainerConfigIndex,
...
const excludedFeatures = ['common-utils', 'git'].map(
	(shortId) => `ghcr.io/devcontainers/features/${shortId}`,
);
...
export class DevContainerConfigGenerator {
	...
	async generate(
		...
		const templates = index.templates.filter(
			(template) => !excludedTemplates.includes(template.id),
		);
		const features = (index.features || []).filter(
			(feature) => !excludedFeatures.includes(feature.id),
		);

		const promptRenderer = PromptRenderer.create(
			this.instantiationService,
			endpoint,
			DevContainerConfigPrompt,
			{
	...
	private processGeneratedConfig(
		message: string,
		availableTemplates: DevContainerConfigTemplate[],
		availableFeatures: DevContainerConfigFeature[],
	) {
		let template = availableTemplates.find((t) =>
			new RegExp(`\\b${escapeRegExpCharacters(t.id)}\\b`).test(message),
		)?.id;
		...
		return {
			...
			features: availableFeatures
				.filter((f) =>
					new RegExp(`\\b${escapeRegExpCharacters(f.id)}\\b`).test(
						message,
					),
				)
				.map((f) => f.id),
