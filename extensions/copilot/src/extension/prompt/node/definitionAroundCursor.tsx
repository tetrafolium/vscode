...
export class DefinitionAroundCursor extends PromptElement<Props, State> {
	...
	override async prepare(
		...
		const nodeToDocument =
			this.props.nodeToDocument ??
			(await determineNodeToDocument(
				this._parserService,
				this._telemetryService,
				this.props.documentContext,
			));
		const contextInfo = generateDocContext(
			this.props.endpointInfo,
			this.props.documentContext,
			nodeToDocument.range,
		);
		return {
			k: 'found',
			nodeToDocument,
	...
	override render(
		...
		if (state.k === 'ignored') {
			return (
				<ignoredFiles
					value={[this.props.documentContext.document.uri]}
				/>
			);
		}
		const codeExcerpt = state.codeExcerptToDocument
			.generatePrompt()
			.join('\n');
		return (
			<UserMessage>
...
export async function determineNodeToDocument(
	parserService: IParserService,
	telemetryService: ITelemetryService,
	ctx: IDocumentContext,
): Promise<{ range: vscode.Range; identifier?: string }> {
	...
	const nodeToDocContext =
		await treeSitterAST.getNodeToDocument(selectionRange);
	const timeSpentMs = Date.now() - startTime;

	const wholeOffsetRange = toTSOffsetRange(ctx.wholeRange, ctx.document);
	sendNodeToDocumentTelemetry(
		telemetryService,
		selectionRange,
		wholeOffsetRange,
		nodeToDocContext,
		ctx.document.languageId,
		timeSpentMs,
	);

	const rangeOfNodeToDocument = toRange(
		ctx.document,
		nodeToDocContext.nodeToDocument,
	);
	return {
...
function generateDocContext(
	endpoint: IChatEndpoint,
	ctx: IDocumentContext,
	range: vscode.Range,
) {
	const tracker = new CodeContextTracker(
		(endpoint.modelMaxPromptTokens * 4) / 3,
	);
	const rangeInfo = new CodeContextRegion(
		tracker,
		ctx.document,
		ctx.language,
	);
	...
	const above = new CodeContextRegion(
		new CodeContextTracker(0),
		ctx.document,
		ctx.language,
	);
	const below = new CodeContextRegion(
		new CodeContextTracker(0),
		ctx.document,
		ctx.language,
	);
	...
	for (let i = range.start.line, len = range.end.line; i <= len; ++i) {
		if (
			(i === len && range.end.character === 0) || // we don't want to include the end line if it's (end.line, 0)
	...
	*/
	telemetryService.sendMSFTTelemetryEvent(
		'getNodeToDocument',
		{
			...
			nodeToDocumentStart:
				nodeToDocContext.nodeToDocument.startIndex.toString(),
			nodeToDocumentEnd:
				nodeToDocContext.nodeToDocument.endIndex.toString(),
