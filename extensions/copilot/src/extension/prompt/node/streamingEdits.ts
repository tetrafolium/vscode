...
export interface IStreamingEditsStrategy {
	processStream(
		stream: AsyncIterable<LineOfText>,
	): Promise<StreamingEditsResult>;
}

export interface IStreamingEditsStrategyFactory {
	(
		lineFilter: ILineFilter,
		streamingWorkingCopyDocument: StreamingWorkingCopyDocument,
	): IStreamingEditsStrategy;
}
...
export class InsertOrReplaceStreamingEdits implements IStreamingEditsStrategy {
	...
	public async processStream(
		_stream: AsyncIterable<LineOfText>,
	): Promise<StreamingEditsResult> {
		// console.log();
		// console.log();
		let stream = AsyncIterableObject.filter(_stream, this.lineFilter);
		if (this.collectImports) {
			stream = collectImportsIfNoneWereSentInRange(
				stream,
				this.myDocument,
				this.adjustedSelection,
			);
		}
		...
		for await (const el of this.findInitialAnchor(stream)) {
			if (el instanceof LineWithAnchorInfo) {
				...
				anchorLineIndex = this.handleSubsequentReplyLine(
					anchorLineIndex,
					el.value,
				);
			}
		...
		if (
			this.myDocument.didReplaceEdits &&
			anchorLineIndex <= this.adjustedSelection.end.line
		) {
			...
			this.myDocument.deleteLines(
				anchorLineIndex,
				this.adjustedSelection.end.line,
			);
		}
	...
	private handleFirstReplyLine(
		anchor: MatchedDocumentLine | null,
		line: string,
	): number {
		if (anchor) {
			this.replyIndentationTracker = new ReplyIndentationTracker(
				this.myDocument,
				anchor.lineIndex,
				line,
			);
			const fixedLine = this.replyIndentationTracker.reindent(
				line,
				this.myDocument.indentStyle,
			);
			if (
				this.myDocument.getLine(anchor.lineIndex).sentInCodeBlock ===
				SentInCodeBlock.Range
			) {
				// Matched a line in the range => replace the entire sent range
				return this.myDocument.replaceLines(
					this.adjustedSelection.start.line,
					anchor.lineIndex,
					fixedLine,
				);
			} else {
		...
		if (this.initialSelection.isEmpty) {
			const cursorLineContent =
				this.myDocument.getLine(firstRangeLine).content;
			if (
				...
				return this.myDocument.replaceLine(
					firstRangeLine,
					fixedLine,
					/*isPreserving*/ true,
				);
		...
		if (
			this.editStrategy === EditStrategy.FallbackToInsertBelowRange ||
			this.editStrategy === EditStrategy.ForceInsertion
		) {
	...
	private handleSubsequentReplyLine(
		...
		if (
			existingLine.indentLevel === fixedLine.adjustedIndentLevel &&
			!this.myDocument.didReplaceEdits
		) {
	...
	private matchReplyLine(
		replyLine: ReplyLine,
		minimumLineIndex: number,
	): MatchedDocumentLine | null {
		const isVeryShortReplyLine = replyLine.trimmedContent.length <= 3;

		for (
			let lineIndex = minimumLineIndex;
			lineIndex < this.myDocument.getLineCount();
			lineIndex++
		) {
			...
			if (
				documentLine.trimmedContent.length > 0 &&
				documentLine.indentLevel < replyLine.adjustedIndentLevel
			) {
				// we shouldn't proceed with the search if we need to jump over original code that is more outdented
				return null;
			}
			if (
				isVeryShortReplyLine &&
				documentLine.trimmedContent.length > 0
			) {
	...
	private findInitialAnchor(
		lineStream: AsyncIterable<LineOfText>,
	): AsyncIterable<LineOfText | LineWithAnchorInfo> {
		return new AsyncIterableObject<LineOfText | LineWithAnchorInfo>(
			async (emitter) => {
				...
				for await (const line of lineStream) {
					if (!anchorFound) {
						...
						if (accumulatedRealChars > 10) {
							const anchor =
								this.searchForEqualSentLines(accumulatedLines);
							anchorFound = true;
							emitter.emitOne(
								new LineWithAnchorInfo(
									accumulatedLines[0].value,
									anchor,
								),
							);
	 ...
	 */
	private searchForEqualSentLines(
		lines: LineOfText[],
	): MatchedDocumentLine | null {
		const trimmedLines = lines.map((line) => line.value.trim());

		for (
			let i = this.myDocument.firstSentLineIndex,
				stopAt = this.myDocument.getLineCount() - lines.length;
			i <= stopAt;
			i++
		) {
			...
			for (let j = 0; j < trimmedLines.length; j++) {
				...
				if (
					!documentLine.isSent ||
					documentLine.trimmedContent !== trimmedLines[j]
				) {
...
export class InsertionStreamingEdits implements IStreamingEditsStrategy {
	...
	public async processStream(
		...
		stream = collectImportsIfNoneWereSentInRange(
			stream,
			this._myDocument,
			new Range(this._cursorPosition, this._cursorPosition),
		);
	...
	private handleFirstReplyLine(replyLine: string): number {
		...
		const cursorLineContent =
			this._myDocument.getLine(firstRangeLine).content;

		// Cursor sitting on an empty or whitespace only line or the reply continues the line
		const shouldLineBeReplaced =
			/^\s*$/.test(cursorLineContent) ||
			replyLine.trimStart().startsWith(cursorLineContent.trimStart());
		...
		const lineNumForIndentGuessing = shouldLineBeReplaced // @ulugbekna: if we are insert line "after" (ie using `insertLineAfter`) we should guess indentation starting from where we insert the line
			? firstRangeLine
			: this._myDocument.getLineCount() <= firstRangeLine + 1
				? firstRangeLine
				: firstRangeLine + 1;

		this.replyIndentationTracker = new ReplyIndentationTracker(
			this._myDocument,
			lineNumForIndentGuessing,
			replyLine,
		);
		const fixedLine = this.replyIndentationTracker.reindent(
			replyLine,
			this._myDocument.indentStyle,
		);

		if (shouldLineBeReplaced) {
			return this._myDocument.replaceLine(
				firstRangeLine,
				fixedLine,
				/*isPreserving*/ true,
			);
...
export class ReplaceSelectionStreamingEdits implements IStreamingEditsStrategy {
	...
	public async processStream(
		...
		if (
			this._selection.end.line > this._selection.start.line &&
			...
			replaceLineCount =
				this._selection.end.line - this._selection.start.line + 1;
			initialTextOnLineAfterSelection = this._myDocument
				.getLine(this._selection.end.line)
				.content.substring(this._selection.end.character);
		}
		...
		for await (const line of stream) {
			if (!this.replyIndentationTracker) {
				...
				anchorLineIndex = this._myDocument.replaceLine(
					firstRangeLine,
					fixedLine,
				);
				replaceLineCount--;
				...
				if (replaceLineCount > 0) {
					...
					anchorLineIndex = this._myDocument.insertLineAfter(
						anchorLineIndex - 1,
						fixedLine,
					);
		...
		if (this._myDocument.didEdits && replaceLineCount > 0) {
			this._myDocument.deleteLines(
				anchorLineIndex,
				anchorLineIndex + replaceLineCount - 1,
			);
		}
		if (
			this._myDocument.didEdits &&
			initialTextOnLineAfterSelection.length > 0
		) {
			this._myDocument.replaceLine(
				anchorLineIndex - 1,
				this._myDocument.getLine(anchorLineIndex - 1).content +
					initialTextOnLineAfterSelection,
			);
		}
...
class ReplyIndentationTracker {
	...
	constructor(
		...
		this._replyIndentStyle =
			IndentUtils.guessIndentStyleFromLine(replyLine);
		const replyIndentLevel = computeIndentLevel2(
			replyLine,
			this._replyIndentStyle?.tabSize ?? 4,
		);
	...
	public reindent(
		replyLine: string,
		desiredStyle: vscode.FormattingOptions,
	): ReplyLine {
		if (replyLine === '') {
		...
		const determineAdjustedIndentLevel = (currentIndentLevel: number) => {
			...
			adjustedIndentLevel = Math.max(
				originalIndentLevel - this.indentDelta,
				0,
			);
			return adjustedIndentLevel;
		};
		const adjustedContent = IndentUtils.reindentLine(
			replyLine,
			this._replyIndentStyle ?? { insertSpaces: true, tabSize: 4 },
			desiredStyle,
			determineAdjustedIndentLevel,
		);

		return new ReplyLine(
			replyLine,
			originalIndentLevel,
			adjustedContent,
			adjustedIndentLevel,
...
export class SentLine {
	constructor(
		...
		readonly sentInCodeBlock:
			| SentInCodeBlock.Above
			| SentInCodeBlock.Range
			| SentInCodeBlock.Below
			| SentInCodeBlock.Other,
	) {}
}
...
export class StreamingWorkingCopyDocument implements IStreamingWorkingCopyDocument {
	...
	constructor(
		...
		this.indentStyle = IndentUtils.getDocumentIndentStyle(
			sourceCode,
			fileIndentInfo,
		);
		...
		for (let i = 0; i < this._originalLines.length; i++) {
			this.lines[i] = new DocumentLine(
				this._originalLines[i],
				this.indentStyle,
			);
		}
		...
		for (const sentLine of sentLines) {
			...
			this.firstSentLineIndex = Math.min(
				this.firstSentLineIndex,
				sentLine.lineIndex,
			);
		}
	...
	public replaceLine(
		index: number,
		line: ReplyLine | string,
		isPreserving: boolean = false,
	): number {
		const newLineContent =
			typeof line === 'string' ? line : line.adjustedContent;
		...
		this.outputStream.textEdit(this.uri, [
			new TextEdit(
				new Range(index, 0, index, Constants.MAX_SAFE_SMALL_INTEGER),
				newLineContent,
			),
		]);
		this._didEdits = true;
		this._didReplaceEdits =
			this._didReplaceEdits || (isPreserving ? false : true);
		return index + 1;
	}
	...
	public replaceLines(
		...
		this.lines.splice(
			fromIndex,
			toIndex - fromIndex + 1,
			new DocumentLine(line.adjustedContent, this.indentStyle),
		);
		this.outputStream.textEdit(this.uri, [
			new TextEdit(
				new Range(
	...
	public deleteLines(fromIndex: number, toIndex: number): number {
		...
		this.outputStream.textEdit(this.uri, [
			new TextEdit(new Range(fromIndex, 0, toIndex + 1, 0), ''),
		]); // TODO: what about end of document??
		this._didEdits = true;
		this._didReplaceEdits = true;
		return fromIndex + 1;
	}
...
class MatchedDocumentLine {
	constructor(public readonly lineIndex: number) {}
}
...
class DocumentLine {
	...
	public get normalizedContent(): string {
		if (this._normalizedContent === null) {
			this._normalizedContent = normalizeIndentation(
				this.content,
				this._indentStyle.tabSize,
				this._indentStyle.insertSpaces,
			);
		}
	...
	public get indentLevel(): number {
		if (this._indentLevel === -1) {
			this._indentLevel = computeIndentLevel2(
				this.content,
				this._indentStyle.tabSize,
			);
		}
...
class IndentUtils {
	public static getDocumentIndentStyle(
		sourceCode: string,
		fileIndentInfo: vscode.FormattingOptions | undefined,
		...
		return <vscode.FormattingOptions>(
			guessIndentation(Lines.fromString(sourceCode), 4, false)
		);
	}

	public static guessIndentStyleFromLine(
		line: string,
	...
	public static reindentLine(
		line: string,
		originalIndentStyle: vscode.FormattingOptions,
		desiredIndentStyle: vscode.FormattingOptions,
		getDesiredIndentLevel: (currentIndentLevel: number) => number = (n) =>
			n,
	): string {
		let indentLevel = computeIndentLevel2(
			line,
			originalIndentStyle.tabSize,
		);
		...
		return normalizeIndentation(
			line,
			desiredIndentStyle.tabSize,
			desiredIndentStyle.insertSpaces,
		);
	}

	private static _outdent(
		line: string,
		indentStyle: vscode.FormattingOptions,
	): string {
		let chrIndex = 0;
	...
	private static _getLeadingWhitespace(line: string): string {
		for (let i = 0; i < line.length; i++) {
			...
			if (char !== 32 && char !== 9) {
				// 32 is ASCII for space and 9 is ASCII for tab
...
export class LineFilters {
	public static combine(
		...filters: (ILineFilter | undefined)[]
	): ILineFilter {
		return (line: LineOfText) =>
			filters.every((filter) => (filter ? filter(line) : true));
	}
...
export interface IStreamingTextPieceClassifier {
	...
	): AsyncIterableObject<ClassifiedTextPiece>;
}
...
export class TextPieceClassifiers {
	...
	private static attemptToRecoverFromMissingCodeBlock(
		classifier: IStreamingTextPieceClassifier,
	): IStreamingTextPieceClassifier {
		return (source: AsyncIterable<string>) => {
			return new AsyncIterableObject<ClassifiedTextPiece>(
				async (emitter) => {
					...
					for await (const piece of classifier(source)) {
						if (!sawOnlyLeadingText) {
							...
							piece.kind === TextPieceKind.OutsideCodeBlock
						) {
					...
					if (sawOnlyLeadingText) {
						const allText = bufferedPieces
							.map((p) => p.value)
							.join('');
						if (looksLikeCode(allText)) {
							emitter.emitOne(
								new ClassifiedTextPiece(
									allText,
									TextPieceKind.InsideCodeBlock,
								),
							);
						} else {
							emitter.emitOne(
								new ClassifiedTextPiece(
									allText,
									TextPieceKind.OutsideCodeBlock,
								),
							);
						}
					}
	 ...
	 * Classifies lines using fenced blocks with the provided fence.
	 */
	public static createFencedBlockClassifier(
		fence: string,
	): IStreamingTextPieceClassifier {
		return (source: AsyncIterable<string>) => {
			return new AsyncIterableObject<ClassifiedTextPiece>(
					...
					while (!reader.endOfStream) {
						...
						if (text !== fence) {
							...
							while (!reader.endOfStream) {
								...
								if (piece.length > 0) {
									emitter.emitOne(
										new ClassifiedTextPiece(piece, state),
									);
								}
							...
							state =
								state === TextPieceKind.InsideCodeBlock
									? TextPieceKind.OutsideCodeBlock
									: TextPieceKind.InsideCodeBlock;
							emitter.emitOne(
								new ClassifiedTextPiece(
									lineWithFence,
									TextPieceKind.Delimiter,
								),
							);
						}
					}
...
export class PartialAsyncTextReader {
	...
	constructor(private readonly _source: AsyncIterator<string>) {}
	...
	readImmediateExcept(char: string): string {
		...
		return this.readImmediate(
			endIndex === -1 ? this._buffer.length : endIndex,
		);
	}
...
export class AsyncReader<T> {
	...
	public async readWhile(
		predicate: (value: T) => boolean,
		callback: (element: T) => unknown,
	): Promise<void> {
		do {
...
function hasImportsInRange(
	doc: IStreamingWorkingCopyDocument,
	range: vscode.Range,
): boolean {
	const startLine =
		range.start.character === 0 ? range.start.line : range.start.line + 1;
	const endLine =
		doc.getLine(range.end.line).content.length === range.end.character
			? range.end.line
			: range.end.line - 1;
...
function collectImportsIfNoneWereSentInRange(
	stream: AsyncIterableObject<LineOfText>,
	doc: IStreamingWorkingCopyDocument,
	rangeToCheckForImports: vscode.Range,
): AsyncIterableObject<LineOfText> {
