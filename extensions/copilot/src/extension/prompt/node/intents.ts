...
export class IntentError extends Error {
	...
	constructor(error: string | vscode.ChatErrorDetails) {
...
export const promptResultMetadata = (
	metadata: PromptMetadata[],
): MetadataMap => ({
	get<T extends PromptMetadata>(
		key: new (...args: any[]) => T,
	): T | undefined {
...
export class StreamingEditsController {
	...
	constructor(
		...
		const textPieceStream = textPieceClassifier(
			this._responseStream.asyncIterable,
		);
		this._streamingPromise = this._process(
			textPieceStream,
			streamingEditsStrategy,
		);
	}

	private async _process(
		textPieceStream: AsyncIterableObject<ClassifiedTextPiece>,
		streamingEditsStrategy: IStreamingEditsStrategy,
	): Promise<StreamingEditsResult> {
		const leadingMarkdown = new AsyncIterableSource<string>();

		const processedMarkdown = this._leadingMarkdownStreamPipe(
			leadingMarkdown.asyncIterable,
		);
		forEachStreamed(processedMarkdown, (item) =>
			this._outputStream.markdown(item),
		);

		const firstCodeBlockText = new AsyncIterableSource<string>();
		const firstCodeBlockLines = streamLines(
			firstCodeBlockText.asyncIterable,
		);
		const streamingEditsPromise =
			streamingEditsStrategy.processStream(firstCodeBlockLines);

		const textPieceStreamWithoutDelimiters = textPieceStream.filter(
			(piece) => piece.kind !== TextPieceKind.Delimiter,
		);
		const reader = new AsyncReader(
			textPieceStreamWithoutDelimiters[Symbol.asyncIterator](),
		);
	...
	public update(newText: string): { shouldFinish: boolean } {
		if (
			this._earlyStopping === EarlyStopping.StopAfterFirstCodeBlock &&
			this._leftFirstCodeBlock
		) {
			// stop was requested!
