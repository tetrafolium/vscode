...
import {
	...
	ChatResponseTurn,
	ChatLocation as VSChatLocation,
} from '../../../vscodeTypes';
...
export class ChatParticipantRequestHandler {
	...
	constructor(
		private readonly rawHistory: ReadonlyArray<
		...
		@IConversationStore
		private readonly _conversationStore: IConversationStore,
		...
		private readonly _authenticationUpgradeService: IAuthenticationChatUpgradeService,
		@IChatQuotaService
		...
		this.intentDetector =
			this._instantiationService.createInstance(IntentDetector);
		...
		if (request.location2 instanceof ChatRequestEditorData) {
			...
			this.stream = ChatResponseStreamImpl.filter(stream, (part) => {
				if (
					part instanceof ChatResponseReferencePart ||
					part instanceof ChatResponseProgressPart2
				) {
					const uri = URI.isUri(part.value)
						? part.value
						: (<Location>part.value).uri;
	...
	private getLocation(request: ChatRequest) {
		...
		switch (
			request.location // deprecated, but location2 does not yet allow to distinguish between panel, editing session and others
		) {
	...
	private async sanitizeVariables(): Promise<ChatRequest> {
		const variablePromises = this.request.references.map(async (ref) => {
			const uri = isLocation(ref.value)
				? ref.value.uri
				: URI.isUri(ref.value)
					? ref.value
					: undefined;
			...
			try {
				...
				removeVariable =
					await this._ignoreService.isCopilotIgnored(uri);
			} catch {
	...
	private async _shouldAskForPermissiveAuth(): Promise<boolean> {
		...
		if (findConfirmRequest) {
			this.request =
				await this._authenticationUpgradeService.handleConfirmationRequest(
					this.stream,
					this.request,
					this.rawHistory,
				);
		...
		const isWorkspaceCall = this.request.toolReferences.some(
			(ref) => ref.name === ContributedToolName.Codebase,
		);
	...
	async getResult(): Promise<ICopilotChatResult> {
		...
		this._logService.trace(
			`[${ChatLocation.toStringShorter(this.location)}] chat request received from extension host`,
		);
		try {
			...
			if (!result) {
				...
				if (!this.request.isParticipantDetected) {
					this.intentDetector.collectIntentDetectionContextInternal(
						this.turn.request.message,
						this.request.enableCommandDetection
							? intent.id
							: 'none',
				...
				if (this._authService.copilotToken?.isNoAuthUser) {
					...
					result.details = formatModelDetails(
						endpoint.name,
						endpoint.multiplier,
						creditsUsed,
					);
			...
			this._conversationStore.addConversation(
				this.turn.id,
				this.conversation,
			);
	...
	private async selectIntent(
		...
		if (!command?.intent && this.location === ChatLocation.Editor) {
			// TODO@jrieken do away with location specific code
			...
			if (
				...
				if (
					this.documentContext.selection.isEmpty &&
					this.documentContext.document
						.lineAt(this.documentContext.selection.start.line)
						.text.trim() === ''
				) {
					preferredIntent = Intent.Generate;
				} else if (
					!this.documentContext.selection.isEmpty &&
					this.documentContext.selection.start.line !==
						this.documentContext.selection.end.line
				) {
	...
	private checkCommandUsage(
		...
		if (
			...
			const commandAgent = getAgentForIntent(
				command.intent.id as Intent,
				this.location,
			);
			let usage = '';
			if (commandAgent) {
 ...
 */
function findExistingTurnFromVSCodeChatHistoryTurn(
	accessor: ServicesAccessor,
	turn: ChatRequestTurn | ChatResponseTurn,
): Turn | undefined {
	const conversationStore = accessor.get(IConversationStore);
	const responseId = getResponseIdFromVSCodeChatHistoryTurn(turn);
	const conversation = responseId
		? conversationStore.getConversation(responseId)
		: undefined;
	return conversation?.turns.find((turn) => turn.id === responseId);
}

function getResponseIdFromVSCodeChatHistoryTurn(
	turn: ChatRequestTurn | ChatResponseTurn,
): string | undefined {
...
function createTurnFromVSCodeChatHistoryTurns(
	...
	const content = chatResponseTurn.response
		.map((r) => {
		...
		.join('');
	const intentId =
		chatResponseTurn.command ||
		getChatParticipantNameFromId(chatResponseTurn.participant);
	...
	if (!chatResponseTurn.result.errorDetails) {
		...
		if (
			chatResponseTurn.result.metadata?.category === FilterReason.Prompt
		) {
		...
		chatResponseTurn.result.errorDetails.message === 'Cancelled' ||
		chatResponseTurn.result.errorDetails.message === CanceledMessage.message
	) {
	...
	currentTurn.setResponse(
		status,
		{
			message: content,
			type: 'model',
			name: command?.commandId || UnknownIntent.ID,
		},
		undefined,
		chatResponseTurn.result,
	);
	const turnMetadata = (chatResponseTurn.result as ICopilotChatResultIn)
		.metadata;
	if (turnMetadata?.renderedGlobalContext) {
		const cacheKey =
			turnMetadata.globalContextCacheKey ??
			instaService.invokeFunction(getGlobalContextCacheKey);
		currentTurn.setMetadata(
			new GlobalContextMessageMetadata(
				turnMetadata?.renderedGlobalContext,
				cacheKey,
			),
		);
	}
	if (turnMetadata?.renderedUserMessage) {
		currentTurn.setMetadata(
			new RenderedUserMessageMetadata(turnMetadata.renderedUserMessage),
		);
	}
	if (turnMetadata?.promptTokens && turnMetadata?.outputTokens) {
		currentTurn.setMetadata(
			new TurnTokenUsageMetadata(
				turnMetadata.promptTokens,
				turnMetadata.outputTokens,
			),
		);
	}
...
function anchorPartToMarkdown(
	...
	if (URI.isUri(anchor.value)) {
		...
		const label =
			anchor.title ??
			`${path}#L${anchor.value.range.start.line + 1}${anchor.value.range.start.line === anchor.value.range.end.line ? '' : `-${anchor.value.range.end.line + 1}`}`;
		text = `\`${label}\``;
	} else if (isSymbolInformation(anchor.value)) {
		path = getWorkspaceFileDisplayPath(
			workspaceService,
			anchor.value.location.uri,
		);
