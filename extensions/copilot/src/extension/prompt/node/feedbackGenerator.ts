...
import {
	...
	EditSurvivalResult,
} from '../../../platform/editSurvivalTracking/common/editSurvivalReporter';
...
export type FeedbackResult =
	| {
			...
			excludedComments?: ReviewComment[];
			reason?: string;
	  }
	| { type: 'error'; severity?: 'info'; reason: string }
	| { type: 'cancelled' };
...
export class FeedbackGenerator {
	...
	async generateComments(
		...
		const ignored = await Promise.all(
			input.map((i) => ignoreService.isCopilotIgnored(i.document.uri)),
		);
		...
		while (batches.length) {
			...
			try {
				...
				this.logService.debug(
					`[FeedbackGenerator] Rendered batch of ${batch.length} inputs.`,
				);
				prompts.push(prompt);
			} catch (err) {
				if (err.code === 'split_input') {
					...
					this.logService.debug(
						`[FeedbackGenerator] Splitting in batches of ${batches[0].length} and ${batches[1].length} inputs due to token limit.`,
					);
				} else {
					throw err;
				}
		...
		if (prompts.length > maxPrompts) {
			return {
				...
				reason:
					inputType === 'selection'
						? l10n.t(
								'There is too much text to review, try reviewing a smaller selection.',
							)
						: l10n.t(
								'There are too many changes to review, try reviewing a smaller set of changes.',
							),
			};
		}
		...
		const request: ReviewRequest = {
			...
			inputRanges: filteredInput.map((input) => ({
				...
				ranges: input.selection
					? [input.selection]
					: input.change?.hunks.map((hunk) => hunk.range) || [],
			})),
		};
		...
		const results = await Promise.all(
			prompts.map(async (prompt) => {
				...
				const finishedCb = progress
					? async (text: string) => {
							...
							if (comments.length > receivedComments.length) {
								progress.report(
									comments.slice(receivedComments.length),
								);
				...
				if (
					progress &&
					comments &&
					comments.length > receivedComments.length
				) {
		...
		const fetchResult =
			results.find((r) => r.fetchResult.type !== 'success')
				?.fetchResult || results[0].fetchResult;
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent(
			'feedback.generateDiagnostics',
			{
				...
				commentTypes:
					[
						...new Set(
							comments?.map((c) =>
								knownKinds.has(c.kind) ? c.kind : 'unknown',
							),
						).values(),
					]
						.sort()
			...
			},
			{
				...
				inputLineCount: request.inputRanges.reduce(
					(acc, r) =>
						acc +
						r.ranges.reduce(
							(acc, r) => acc + (r.end.line - r.start.line),
							0,
						),
					0,
				),
...
export function parseReviewComments(
	...
	for (const match of parseFeedbackResponse(message, dropPartial)) {
		const { relativeDocumentPath, from, to, kind, severity, content } =
			match;
		...
		const i =
			...
			input.find((i) => i.relativeDocumentPath === relativeDocumentPath);
		if (!i) {
			continue;
		}

		const document = i.document;
		const filterRanges = i.selection
			? [i.selection!]
			: i.change?.hunks.map((hunk) => hunk.range);

		const fromLine = document.lineAt(from >= 0 ? from : 0);
		const toLine = document.lineAt(
			(to <= document.lineCount ? to : document.lineCount) - 1,
		);
...
export function parseFeedbackResponse(response: string, dropPartial = false) {
	const regex =
		/(?<num>\d+)\. Line (?<from>\d+)(-(?<to>\d+))?([^:]*)( in `?(?<relativeDocumentPath>[^,:`]+))`?(, (?<kind>\w+))?(, (?<severity>\w+) severity)?: (?<content>.+?)((?=\n\d+\.|\n\n)|(?<earlyEnd>$))/gs;
	return coalesce(
		Array.from(response.matchAll(regex), (match) => {
			...
			const relativeDocumentPath =
				groups.relativeDocumentPath?.replaceAll(
					path.sep === '/' ? '\\' : '/',
					path.sep,
				);
			...
			if (blockBorders.length % 2) {
				...
				content = content.substring(0, odd.index).trim();
			}
			return {
				relativeDocumentPath,
				from,
				...
				linkLength:
					5 +
					groups.from.length +
					(groups.to ? groups.to.length + 1 : 0),
				kind,
				severity,
				content,
			};
...
export function sendReviewActionTelemetry(
	reviewCommentOrComments: ReviewComment | ReviewComment[],
	totalComments: number,
	userAction: 'helpful' | 'unhelpful' | string,
	logService: ILogService,
	telemetryService: ITelemetryService,
	instantiationService: IInstantiationService,
): void {
	logService.debug('[FeedbackGenerator] user feedback received');
	const reviewComments = Array.isArray(reviewCommentOrComments)
		? reviewCommentOrComments
		: [reviewCommentOrComments];
	const reviewComment = reviewComments[0];
	if (!reviewComment) {
		logService.warn(
			'[FeedbackGenerator] No review comment found for user feedback',
		);
		return;
	}
	...
	const commentType = knownKinds.has(reviewComment.kind)
		? reviewComment.kind
		: 'unknown';
	const sharedProps: TelemetryEventProperties = {
		source: reviewComment.request.source,
		requestId: reviewComment.request.messageId,
		documentType: isNotebookCellOrNotebookChatInput(reviewComment.uri)
			? 'notebook'
			: 'text',
	...
	const sharedMeasures: TelemetryEventMeasurements = {
		...
		inputLineCount: reviewComment.request.inputRanges.reduce(
			(acc, r) =>
				acc +
				r.ranges.reduce(
					(acc, r) => acc + (r.end.line - r.start.line),
		...
		commentLength: reviewComments.reduce(
			(acc, c) =>
				acc +
	...
	if (userAction === 'discardComment') {
		...
		instantiationService.createInstance(
			...
			document.getText(),
			StringEdit.replace(
				OffsetRange.ofStartAndLength(from, to - from),
				text,
			),
			StringEdit.empty,
			{},
			discardCommentSurvivalEvent(sharedProps, sharedMeasures),
		);
	}
}

function discardCommentSurvivalEvent(
