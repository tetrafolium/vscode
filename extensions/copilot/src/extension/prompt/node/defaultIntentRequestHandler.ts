...
import {
	...
	NullEditSurvivalTrackingSession,
...
import {
	IToolGrouping,
	IToolGroupingService,
} from '../../tools/common/virtualTools/virtualToolTypes';
...
import {
	...
	ISwitchToAutoOnRateLimitConfirmation,
} from '../common/specialRequestTypes';
import {
	ChatTelemetry,
...
export class DefaultIntentRequestHandler {
	...
	private _editSurvivalTracker: IEditSurvivalTrackingSession =
		new NullEditSurvivalTrackingSession();
	...
	async getResult(): Promise<ChatResult> {
		if (isToolCallLimitCancellation(this.request)) {
			...
			this.stream.markdown(
				l10n.t("Let me know if there's anything else I can help with!"),
			);
			return {};
		}
		...
		try {
			...
			this.turn.setMetadata(
				new IntentInvocationMetadata(intentInvocation),
			);
			...
			const capturingToken = new CapturingToken(
				...
				isSubagent
					? `runSubagent-${this.request.subAgentName ?? 'default'}`
					: undefined,
			);
			...
			const responseMessage =
				resultDetails.toolCallRounds.at(-1)?.response ?? '';
			const metadataFragment: Partial<IResultMetadata> = {
				...
				resolvedModel:
					resultDetails.response.type ===
					ChatFetchResponseType.Success
						? resultDetails.response.resolvedModel
						: undefined,
			};
			...
			if (
				...
				intentInvocation.modifyErrorDetails
			) {
			...
			if (err instanceof ToolCallCancelledError) {
				...
				this._logService.info(
					`[DefaultIntentRequestHandler] Hook ${err.hookType} aborted: ${err.stopReason}`,
				);
				return {};
			}
	...
	private _collectRelevantToolCallResults(
		toolCallRounds: IToolCallRound[],
		toolCallResults: Record<string, LanguageModelToolResult2>,
	): Record<string, LanguageModelToolResult2> | undefined {
		const resultsUsedInThisTurn: Record<string, LanguageModelToolResult2> =
			{};
		for (const round of toolCallRounds) {
			for (const toolCall of round.toolCalls) {
				resultsUsedInThisTurn[toolCall.id] =
					toolCallResults[toolCall.id];
			}
		}

		return Object.keys(resultsUsedInThisTurn).length
			? resultsUsedInThisTurn
			: undefined;
	}

	private _sendInitialChatReferences({
		result: buildPromptResult,
	}: IToolCallingBuiltPromptEvent) {
		const [includedVariableReferences, ignoredVariableReferences] = [
			getUniqueReferences(buildPromptResult.references),
			getUniqueReferences(buildPromptResult.omittedReferences),
		].map((refs) =>
			refs.reduce((acc, ref) => {
		...
		for (const reference of buildPromptResult.references) {
			...
			const options =
				reference.options ??
				('variableName' in reference.anchor &&
				ignoredVariableReferences.has(reference.anchor.variableName)
					? {
							status: {
								kind: 2,
								description: l10n.t(
									'Part of this attachment was not sent to the model due to context window limitations.',
								),
							},
						}
					: undefined);
		...
		for (const omittedReference of buildPromptResult.omittedReferences) {
			if (
				'variableName' in omittedReference.anchor &&
				!includedVariableReferences.has(
					omittedReference.anchor.variableName,
				)
			) {
				this.stream.reference2(omittedReference.anchor, undefined, {
					status: {
						kind: 3,
						description: l10n.t(
							'This attachment was not sent to the model due to context window limitations.',
						),
					},
				});
			}
	...
	private makeResponseStreamParticipants(
			...
			return ChatResponseStreamImpl.spy(
				...
				() => {
					const codeBlocksMetaData =
						codeBlockTrackingResponseStream.finish();
			...
			participants.push((stream) => {
				const firstTurnWithAIEditCollector =
					this.conversation.turns.find(
						(turn) =>
							turn.getMetadata(CopilotInteractiveEditorResponse)
								?.editSurvivalTracker,
					);
				this._editSurvivalTracker =
					...
					this._editSurvivalTrackerService.initialize(
						this.documentContext!.document.document,
					);
		...
		participants.push((stream) =>
			interactionOutcomeComputer.spyOnStream(stream),
		);

		// 4. Linkify the stream unless told otherwise, or if this is a subagent request
		if (
			!intentInvocation.linkification?.disable &&
			!this.request.subAgentInvocationId
		) {
			participants.push((stream) => {
				const linkStream = this._instantiationService.createInstance(
					ResponseStreamWithLinkification,
					{
						...
						references: this.turn.references,
					},
					stream,
				...
				return ChatResponseStreamImpl.spy(
					...
					(p) => p,
					() => {
						this._loop.telemetry.markAddedLinks(
							linkStream.totalAddedLinkCount,
						);
					},
				);
			});
	...
	private async _onDidReceiveResponse({
		response,
		toolCalls,
		interactionOutcome,
	}: IToolCallingResponseEvent) {
		const responseMessage =
			response.type === ChatFetchResponseType.Success
				? response.value
				: '';
		await this._loop.telemetry.sendTelemetry(
			response.requestId,
			response.type,
			responseMessage,
	...
	private async runWithToolCalling(
		...
		const loop = (this._loop = store.add(
			this._instantiationService.createInstance(
				DefaultToolCallingLoop,
				{
					...
					onHitToolCallLimit:
						this.handlerOptions.confirmOnMaxToolIterations !== false
							? ToolCallLimitBehavior.Confirm
							: ToolCallLimitBehavior.Stop,
					request: this.request,
					documentContext: this.documentContext,
					streamParticipants:
						this.makeResponseStreamParticipants(intentInvocation),
					temperature:
						this.handlerOptions.temperature ??
						this.options.temperature,
					location: this.location,
					overrideRequestLocation:
						this.handlerOptions.overrideRequestLocation,
					interactionContext: this.documentContext?.document.uri,
					responseProcessor:
						typeof intentInvocation.processResponse === 'function'
		...
		store.add(
			Event.once(loop.onDidBuildPrompt)(
		...
		try {
			...
			const userPromptSubmitResults =
				await this._chatHookService.executeHook(
					'UserPromptSubmit',
					this.request.hooks,
					{
			...
			processHookResults({
				...
				onSuccess: (output) => {
					if (typeof output === 'object' && output !== null) {
						const typedOutput =
							output as UserPromptSubmitHookOutput & {
								additionalContext?: string;
							};
						const additionalContext =
							typedOutput.hookSpecificOutput?.additionalContext ??
							typedOutput.additionalContext;
						...
						if (typedOutput.decision === 'block') {
							const blockReason =
								typedOutput.reason ||
								l10n.t('No reason provided');
							this._logService.info(
								`[DefaultIntentRequestHandler] UserPromptSubmit hook block decision: ${blockReason}`,
							);
							this.stream.hookProgress(
								'UserPromptSubmit',
								formatHookErrorMessage(blockReason),
							);
			...
			result.chatResult ??= {};
			if (
				(result.chatResult.metadata as IResultMetadata)
	...
	private resultWithMetadatas(
		chatResult: ChatResult | undefined,
	): ChatResult | undefined {
		const codeBlocks = this.turn.getMetadata(CodeBlocksMetadata);
		const allSummarizedConversationHistory = this.turn.getAllMetadata(
			SummarizedConversationHistoryMetadata,
		);
		const renderedUserMessageMetadata = this.turn.getMetadata(
			RenderedUserMessageMetadata,
		);
		const globalContextMetadata = this.turn.getMetadata(
			GlobalContextMessageMetadata,
		);
		const turnTokenUsageMetadata = this.turn.getMetadata(
			TurnTokenUsageMetadata,
		);
		return codeBlocks ||
			allSummarizedConversationHistory?.length ||
			renderedUserMessageMetadata ||
			...
			? {
					...
					metadata: {
						...chatResult?.metadata,
						...codeBlocks,
						...(allSummarizedConversationHistory &&
							allSummarizedConversationHistory.length > 0 && {
								summaries: allSummarizedConversationHistory,
							}),
						...renderedUserMessageMetadata,
						...globalContextMetadata,
						...turnTokenUsageMetadata,
	...
	private async handleConfirmationsIfNeeded(): Promise<
		ChatResult | undefined
	> {
		const intentInvocation = this.turn.getMetadata(
			IntentInvocationMetadata,
		)?.value;
		assertType(intentInvocation);
		if (
			(this.request.acceptedConfirmationData?.length ||
				this.request.rejectedConfirmationData?.length) &&
			intentInvocation.confirmationHandler
		) {
			await intentInvocation.confirmationHandler(
				this.request.acceptedConfirmationData,
				this.request.rejectedConfirmationData,
				this.stream,
			);
			return {};
		}
	...
	private async processSuccessfulFetchResult(
		...
		if (
			...
			!rounds.some((r) => r.toolCalls.length)
		) {
	...
	private getModeNameForTelemetry(): string {
		const modeInstructionsName =
			this.request.modeInstructions2?.name?.toLowerCase();
		if (modeInstructionsName) {
			return this.request.modeInstructions2?.isBuiltin
				? this.request.modeInstructions2.name.toLowerCase()
				: 'custom';
		}
	...
	private processOffTopicFetchResult(
		baseModelTelemetry: ConversationalBaseTelemetryData,
	): ChatResult {
		...
		this.turn.setResponse(
			TurnStatus.OffTopic,
			{
				message: this.options.rejectionMessage,
	...
	private async getErrorDetails(error: ChatFetchError) {
		...
		return getErrorDetailsFromChatFetchError(
			error,
	...
	private async processResult(
		...
		switch (fetchResult.type) {
			...
			case ChatFetchResponseType.Canceled: {
				...
				this.turn.setResponse(
					TurnStatus.Cancelled,
					{ message: errorDetails.message, type: 'user' },
					baseModelTelemetry.properties.messageId,
					chatResult,
				);
				return chatResult;
			...
			case ChatFetchResponseType.RateLimited: {
				...
				if (
					...
					fetchResult.capiError?.code?.startsWith(
					...
					if (
						this._configurationService.getConfig(
							ConfigKey.RateLimitAutoSwitchToAuto,
						)
					) {
						...
						errorDetails.confirmationButtons = [
							{
								data: {
									...
									alwaysSwitchToAuto: true,
								} satisfies ISwitchToAutoOnRateLimitConfirmation,
								label: l10n.t('Switch to Auto (always)'),
							},
							{
								data: {
									copilotSwitchToAutoOnRateLimit: true,
									alwaysSwitchToAuto: false,
								} satisfies ISwitchToAutoOnRateLimitConfirmation,
								label: l10n.t('Switch to Auto'),
							},
						];
					}
				}
			...
			case ChatFetchResponseType.Failed: {
				...
				this.turn.setResponse(
					TurnStatus.Error,
					{ message: errorDetails.message, type: 'server' },
					baseModelTelemetry.properties.messageId,
					chatResult,
				);
				return chatResult;
			...
			case ChatFetchResponseType.Filtered: {
				...
				const chatResult = {
					...
					metadata: {
						...metadataFragment,
						filterReason: fetchResult.category,
					},
				};
				this.turn.setResponse(
					TurnStatus.Filtered,
			...
			case ChatFetchResponseType.PromptFiltered: {
				...
				const chatResult = {
					...
					metadata: {
						...metadataFragment,
						filterReason: FilterReason.Prompt,
					},
				};
				this.turn.setResponse(
					TurnStatus.PromptFiltered,
					undefined,
					baseModelTelemetry.properties.messageId,
					chatResult,
				);
...
class DefaultToolCallingLoop extends ToolCallingLoop<IDefaultToolLoopOptions> {
	...
	constructor(
		...
		@IToolGroupingService
		private readonly toolGroupingService: IToolGroupingService,
		...
		this._register(
			this.onDidBuildPrompt(
				({ result, tools, promptTokenLength, toolTokenCount }) => {
					if (
						result.metadata.get(
							SummarizedConversationHistoryMetadata,
						)
	...
	protected override createPromptContext(
		...
		const extraVars =
			this.options.invocation.getAdditionalVariables?.(context);
		if (extraVars?.hasVariables()) {
			return {
				...context,
				chatVariables: ChatVariablesCollection.merge(
	...
	private _handleVirtualCalls(context: Mutable<IBuildPromptContext>) {
		...
		for (const call of context.toolCallRounds?.at(-1)?.toolCalls ||
			Iterable.empty()) {
			if (context.toolCallResults?.[call.id]) {
				continue;
			}
			const expanded = this.toolGrouping.didCall(
				context.toolCallRounds!.length,
				call.name,
			);
			if (expanded) {
	...
	protected override async buildPrompt(
		...
		const buildPromptResult = await this.options.invocation.buildPrompt(
			buildPromptContext,
			progress,
			token,
		);
	...
	protected override async fetch(
		...
		const messageSourcePrefix =
			this.options.location === ChatLocation.Editor ? 'inline' : 'chat';
		...
		return this.options.invocation.endpoint.makeChatRequest2(
			{
				...
				modelCapabilities: {
					...
					enableThinking:
						isThinkingLocation &&
						opts.modelCapabilities?.enableThinking,
					reasoningEffort,
					enableToolSearch:
						!isSubagent &&
						!!this.options.invocation.endpoint.supportsToolSearch,
					enableContextEditing:
						!isSubagent &&
						isAnthropicContextEditingEnabled(
							this.options.invocation.endpoint,
				...
				telemetryProperties: {
					...
					conversationId: this.options.conversation.sessionId,
					messageSource: opts.isKeepAliveProbe
						? 'chat.cacheKeepAlive'
						: this.options.intent?.id &&
							  this.options.intent.id !== UnknownIntent.ID
							? `${messageSourcePrefix}.${this.options.intent.id}`
							: `${messageSourcePrefix}.user`,
					subType: this.options.request.subAgentInvocationId
						? `subagent`
				...
				interactionTypeOverride: this.options.request
	...
	private didModeChangeSincePreviousRequest(): boolean {
		...
		if (
			this.currentToolCallRounds.some((round) => !!round.statefulMarker)
		) {
			return false;
		}
		...
		if (
			!previousModeInstructions &&
			!this.options.request.modeInstructions2
		) {
			return false;
		}

		const modeChanged = !areModeInstructionsEqual(
			previousModeInstructions,
			this.options.request.modeInstructions2,
		);
		if (modeChanged) {
			this._logService.trace(
				'[DefaultIntentRequestHandler] Detected mode instructions changed between requests',
			);
		}
	...
	protected override async getAvailableTools(
		...
		const tools =
			(await this.options.invocation.getAvailableTools?.()) ?? [];

		// Skip tool grouping when Anthropic tool search is enabled
		if (
			isAnthropicFamily(this.options.invocation.endpoint) &&
			this.options.invocation.endpoint.supportsToolSearch
		) {
			return tools;
		}
		...
		if (this.toolGrouping) {
			...
			this.toolGrouping = this.toolGroupingService.create(
		...
		const computePromise = this.toolGrouping.compute(
			this.options.request.prompt,
			token,
		); // Show progress if this takes a moment...
		const timeout = setTimeout(() => {
			outputStream?.progress(
				l10n.t('Optimizing tool selection'),
				async () => {
	...
	private fixMessageNames(messages: Raw.ChatMessage[]): void {
		messages.forEach((m) => {
			if (
				m.role !== Raw.ChatRole.System &&
				'name' in m &&
				m.name === this.options.intent?.id
			) {
...
function areModeInstructionsEqual(
	a: ChatRequest['modeInstructions2'],
	b: ChatRequest['modeInstructions2'],
): boolean {
	if (!a || !b) {
		return a === b;
	}
	...
	return (
		...
		serializeModeInstructionMetadata(a.metadata) ===
			serializeModeInstructionMetadata(b.metadata)
	);
}

function normalizeModeInstructionMetadata(
	metadata: ModeInstructionMetadata,
	...
	return entries.reduce<Record<string, boolean | string | number>>(
		(result, [key, value]) => {
			result[key] = value;
			return result;
		},
...
function serializeModeInstructionMetadata(
	metadata: ModeInstructionMetadata,
): string | undefined {
