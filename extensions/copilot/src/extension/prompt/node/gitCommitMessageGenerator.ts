...
type ResponseFormat =
	| 'noTextCodeBlock'
	| 'oneTextCodeBlock'
	| 'multipleTextCodeBlocks';

export class GitCommitMessageGenerator {
	constructor(
		@IConversationOptions
		private readonly conversationOptions: IConversationOptions,
	...
	async generateGitCommitMessage(
		...
		const promptRenderer = PromptRenderer.create(
			...
			{ repositoryName, branchName, changes, recentCommitMessages },
		);
		...
		*/
		...
		const [responseFormat, commitMessage] =
			this.processGeneratedCommitMessage(fetchResult.value);
		if (responseFormat !== 'oneTextCodeBlock') {
			...
			*/
			this.telemetryService.sendMSFTTelemetryEvent(
				'git.generateCommitMessageIncorrectResponseFormat',
				{ requestId: fetchResult.requestId, responseFormat },
			);
		}
	...
	private processGeneratedCommitMessage(
		raw: string,
