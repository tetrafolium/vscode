...
suite('feedbackGenerator', () => {
   ...
   \`\`\``;
		...
		test('parses single comment with all fields including linkOffset and linkLength', () => {
			const response =
				'1. Line 10 in `file.ts`, bug, high severity: This is a bug.';
		...
		test('parses comment without backticks around path', () => {
			const response =
				'1. Line 5 in file.ts, performance, medium severity: Slow code.';
			const matches = parseFeedbackResponse(response);
		...
		test('parses line range (from-to) with correct linkLength', () => {
			const response =
				'1. Line 10-15 in `file.ts`, bug, high severity: Multiple lines affected.';
			const matches = parseFeedbackResponse(response);
		...
		test('parses comment with extra text before "in" keyword', () => {
			const response =
				'1. Line 10 (modified) in `file.ts`, bug: Issue with extra text.';
			const matches = parseFeedbackResponse(response);
...
\`\`\``;
			...
			assert.strictEqual(
				matches[0].content,
				'The lambda function could be extracted.',
			);
			assert.strictEqual(matches[0].content.indexOf('```'), -1);
		});
		...
		test('removes broken code block (odd number of markers)', () => {
			const response =
				'1. Line 10 in `file.ts`, bug: Here is some code:\n```typescript\nconst x = 1;';
			const matches = parseFeedbackResponse(response);
		...
		test('preserves inline code (single backticks)', () => {
			const response =
				'1. Line 10 in `file.ts`, bug: The variable `foo` should be renamed.';
			const matches = parseFeedbackResponse(response);
		...
		test('removes trailing ``` via broken block handler when no opening marker exists', () => {
			const response =
				'1. Line 10 in `file.ts`, bug: Some text ending with ```\n\n';
			const matches = parseFeedbackResponse(response);
...
\`\`\``;
			...
			assert.strictEqual(
				matches[0].content.includes('const y = 2;'),
				false,
			);
		});
		...
		test('normalizes path separators for subdirectories', () => {
			const response =
				'1. Line 10 in `src/utils/helpers.ts`, bug: Issue.\n\n';
			const matches = parseFeedbackResponse(response);
			...
			if (path.sep === '\\') {
				assert.strictEqual(
					matches[0].relativeDocumentPath,
		...
		test('returns empty array for empty or invalid response', () => {
			...
			assert.strictEqual(
				parseFeedbackResponse(
					'This is just some text without the expected format.',
				).length,
				0,
			);
		});
		...
		test('trims whitespace from content', () => {
			const response =
				'1. Line 10 in `file.ts`, bug:    Spaces around content.   ';
			const matches = parseFeedbackResponse(response);
...
2. Line 20 in \`other.ts\`, bug: Next issue.`;
	...
	describe('parseReviewComments', () => {
		test('parses valid comment and creates ReviewComment', () => {
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 4, 6), text: content },
						],
					},
				},
			];
			...
			const message =
				'1. Line 2 in `file.ts`, bug, high severity: This is a bug.\n\n';
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 5, 6), text: content },
						],
					},
				},
			];
...
`;
			...
			assert.strictEqual(comments[0].originalIndex, 0);
			assert.strictEqual(
				typeof comments[1].body === 'string'
					? comments[1].body
					: comments[1].body.value,
				'Second issue.',
			);
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 2, 6), text: content },
						],
					},
				},
			];
		...
		test('accepts all known kinds', () => {
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 19, 7), text: content },
						],
					},
				},
			];
			...
			const message =
				knownKinds
					.map(
						(kind, i) =>
							`${i + 1}. Line ${i + 1} in \`file.ts\`, ${kind}: Issue ${i + 1}.`,
					)
					.join('\n\n') + '\n\n';
		...
		test('skips comment when relativeDocumentPath does not match any input', () => {
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 1, 6), text: content },
						],
					},
				},
			];
			const request = createReviewRequest();
			const message =
				'1. Line 1 in `file.ts`, bug: Should be skipped.\n\n';
		...
		test('matches correct input from multiple inputs', () => {
			...
			const request = createReviewRequest();
			const message =
				'1. Line 1 in `second.ts`, bug: Found in second file.\n\n';
		...
		test('clamps line number exceeding lineCount', () => {
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 2, 9), text: content },
						],
					},
				},
			];
			...
			const message =
				'1. Line 1-100 in `file.ts`, bug: Should clamp to end.\n\n';
		...
		test('filters out comment outside change hunk range', () => {
			...
			const message =
				'1. Line 5 in `file.ts`, bug: Outside hunk range.\n\n';
		...
		test('uses selection range for filtering when selection is provided', () => {
			...
			const messageOutside =
				'1. Line 1 in `file.ts`, bug: Outside selection.\n\n';
			const commentsOutside = parseReviewComments(
				request,
				input,
				messageOutside,
			);
			...
			const messageInside =
				'1. Line 3 in `file.ts`, bug: Inside selection.\n\n';
			const commentsInside = parseReviewComments(
				request,
				input,
				messageInside,
			);
		...
		test('includes comment when no filterRanges (no selection or change)', () => {
			...
			const request = createReviewRequest();
			const message =
				'1. Line 1 in `file.ts`, bug: No filter ranges.\n\n';
		...
		test('passes dropPartial to parseFeedbackResponse', () => {
			...
			const commentsKept = parseReviewComments(
				request,
				input,
				partialMessage,
				false,
			...
			const commentsDropped = parseReviewComments(
				request,
				input,
				partialMessage,
				true,
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 0, 6), text: content },
						],
					},
				},
			];
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 0, 25), text: content },
						],
					},
				},
			];
		...
		test('handles line range spanning multiple lines', () => {
			...
			const input: CurrentChangeInput[] = [
				{
					...
					change: {
						...
						uri,
						hunks: [
							{ range: new Range(0, 0, 2, 27), text: content },
						],
					},
				},
			];
			const request = createReviewRequest();
			const message =
				'1. Line 1-3 in `file.ts`, bug: Multi-line issue.\n\n';
	...
	class MockIgnoreService extends NullIgnoreService {
		...
		override get isRegexExclusionsEnabled(): boolean {
			return true;
		}
	...
	class MockChatEndpoint {
		...
		private _response: ChatResponse = {
		...
		async makeChatRequest(
			...
			if (
				this._response.type === ChatFetchResponseType.Success &&
				finishedCb
			) {
	...
	describe('FeedbackGenerator.generateComments', () => {
		...
		test('returns success with comments when endpoint returns valid response', async () => {
			...
			const content = 'line 0\nline 1\nline 2\nline 3\nline 4';
			const input = [
				createInput(uri, content, 'file.ts', {
...
`,
		...
		test('filters out ignored documents but processes non-ignored ones', async () => {
			...
			if (result.type === 'success') {
				assert.strictEqual(result.comments.length, 1);
				assert.strictEqual(
					result.comments[0].uri.toString(),
					uri2.toString(),
				);
			}
		});
		...
		test('reports progress when progress callback is provided', async () => {
			...
			await feedbackGenerator.generateComments(
				input,
				CancellationToken.None,
				progress,
			);
...
`,
			...
			if (result.type === 'success') {
				...
				assert.strictEqual(
					result.comments[1].uri.toString(),
					uri2.toString(),
				);
			}
		});
		...
		test('returns error when prompts exceed maxPrompts limit', async () => {
			...
			for (let i = 0; i < 15; i++) {
				...
				const content = 'line 0\nline 1';
				inputs.push(
					createInput(uri, content, `file${i}.ts`, {
						hunks: [
							{ range: new Range(0, 0, 1, 6), text: content },
						],
					}),
			...
			let callCount = 0;
			const originalMakeChatRequest =
				mockEndpointProvider.mockEndpoint.makeChatRequest.bind(
					mockEndpointProvider.mockEndpoint,
				);
			mockEndpointProvider.mockEndpoint.makeChatRequest = async (
				debugName,
				messages,
				finishedCb,
				token,
				...
				callCount++;
				return originalMakeChatRequest(
					debugName,
					messages,
					finishedCb,
					token,
	...
	class MockTelemetryService extends NullTelemetryService {
		...
		override sendInternalMSFTTelemetryEvent(
			eventName: string,
			properties?: TelemetryEventProperties,
			measurements?: TelemetryEventMeasurements,
		): void {
			this.internalMsftEvents.push({
				eventName,
				properties,
				measurements,
			});
	...
	describe('sendReviewActionTelemetry', () => {
		...
		function createTestReviewComment(
		...
		beforeEach(() => {
			...
			mockInstantiationService = serviceCollection
				.createTestingAccessor()
				.get(IInstantiationService);
		});
		...
		test.each([
			...
			'sends review.comment.vote telemetry for %s action',
			(action, totalComments) => {
				...
				assert.ok(
					mockLogService.debugMessages[0].includes(
				...
				assert.strictEqual(
					mockTelemetryService.msftEvents[0].eventName,
					'review.comment.vote',
				);
				assert.strictEqual(
					mockTelemetryService.msftEvents[0].properties?.userAction,
					action,
				);
				assert.strictEqual(
					mockTelemetryService.msftEvents[0].properties?.commentType,
					'bug',
				);
				assert.strictEqual(
				...
				assert.strictEqual(
					mockTelemetryService.internalMsftEvents.length,
					1,
				);
				assert.strictEqual(
					mockTelemetryService.internalMsftEvents[0].eventName,
					'review.comment.vote',
				);
			},
		);
		...
		test('returns early and warns when no comments provided', () => {
			...
			assert.ok(
				mockLogService.warnMessages[0].includes(
		...
		test('handles single comment (not array)', () => {
			...
			assert.strictEqual(mockTelemetryService.msftEvents.length, 1);
			assert.strictEqual(
				mockTelemetryService.msftEvents[0].measurements?.comments,
				1,
			);
		});
		...
		test('handles array of comments', () => {
			...
			const comment2 = createTestReviewComment({
				originalIndex: 1,
				body: new MarkdownString('Second comment'),
			});
		...
		test('calculates commentLength correctly for MarkdownString body', () => {
			const comment = createTestReviewComment({
				body: new MarkdownString('Hello world'),
			});
			...
			assert.strictEqual(
				mockTelemetryService.msftEvents[0].measurements?.commentLength,
				11,
			);
		});
		...
		test('calculates inputLineCount correctly from multiple ranges', () => {
			...
			assert.strictEqual(
				mockTelemetryService.msftEvents[0].measurements?.inputLineCount,
				15,
			);
		});
