...
export class ChatQuotaContribution
	extends Disposable
	implements IExtensionContribution
{
	...
	constructor(
		...
		super();
		this._register(
			commands.registerCommand('chat.enableAdditionalUsage', () => {
				...
				env.openExternal(
					Uri.parse('https://aka.ms/github-copilot-manage-overage'),
				);
			}),
		);
		...
		this._register(
			chatQuotaService.onDidChange(() => {
				...
				const quotas = {
					usageBasedBilling:
						!!authService.copilotToken?.isUsageBasedBilling,
					...
					sessionRateLimit: session
						? {
								percentRemaining: session.percentRemaining,
								unlimited: session.unlimited,
								resetDate: session.resetDate.toISOString(),
							}
						: undefined,
					weeklyRateLimit: weekly
						? {
