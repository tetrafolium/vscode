...
export async function generateDiffNextEdits(
	...
	const adjustedDesiredLines = eliminateTrimEmptyLinesDifference(
		currentLines,
		desiredLines,
	);
...
function eliminateTrimEmptyLinesDifference(
	sourceLines: Lines,
	resultLines: Lines,
): Lines {
	...
	const trailingResultEmptyLineCount =
		getTrailingArrayEmptyLineCount(resultLines);

	const trimResultLines = resultLines.slice(
		leadingResultEmptyLineCount,
		resultLines.length - trailingResultEmptyLineCount,
	);
	return [
		...sourceLines.slice(0, leadingEmptyLineCount),
		...trimResultLines,
		...sourceLines.slice(
			sourceLines.length - trailingEmptyLineCount,
			sourceLines.length,
		),
	];
}
...
function createLineEditFromDiff(
...
function createSingleLineEditFromDiff(
	...
	return new LineReplacement(
		diff.original,
		newLines.slice(
			diff.modified.startLineNumber - 1,
			diff.modified.endLineNumberExclusive - 1,
		),
	);
}
