...
import {
	...
	type MarkdownLoggable,
} from '../../../platform/inlineEdits/common/inlineEditLogContext';
import {
	IObservableDocument,
	ObservableWorkspace,
...
} from '../../../platform/inlineEdits/common/workspaceEditTracker/historyContextProvider';
import {
...
import {
	...
	ReusedRequestKind,
} from './nextEditProviderTelemetry';
import { INextEditResult, NextEditResult } from './nextEditResult';
import {
	SpeculativeCancelReason,
	SpeculativeRequestManager,
} from './speculativeRequestManager';
...
function computeReducedWindow(
	...
	const lineEndOffset = t.getOffset(
		cursorPosition.with(
			undefined,
			t.getLineLength(cursorPosition.lineNumber) + 1,
		),
	);
	...
	const reducedEndOffset = t.getOffset(
		reducedEndPosition.column > 1
			? reducedEndPosition.with(
					undefined,
					t.getLineLength(reducedEndPosition.lineNumber) + 1,
				)
			: reducedEndPosition,
	);
...
function convertLineEditToEdit(
	nextLineEdit: LineEdit,
	document: StringText,
): StringEdit {
	...
	if (document.value.includes('\r\n')) {
		return new StringEdit(
			suggestedEdit.replacements.map(
				(r) =>
...
function createDocStateLookupMap(
	projectedDocuments: readonly ProcessedDoc[],
	xtabEditHistory: readonly IXtabHistoryEntry[],
): CachedFunction<
	DocumentId,
	{
...
export interface NESInlineCompletionContext
	extends vscode.InlineCompletionContext {
...
export interface INextEditProvider<
	T extends INextEditResult,
	TTelemetry,
	TData = void,
> extends IDisposable {
	readonly ID: string;
...
export class NextEditProvider
	extends Disposable
	implements INextEditProvider<NextEditResult, LlmNESTelemetryBuilder>
{
	public readonly ID = this._statelessNextEditProvider.ID;

	private readonly _rejectionCollector = this._register(
		new RejectionCollector(this._workspace, this._logService),
	);
	private readonly _nextEditCache: NextEditCache;

	private _pendingStatelessNextEditRequest: StatelessNextEditRequest<CachedOrRebasedEdit> | null =
		null;
	...
	constructor(
		...
		this._logger = this._logService.createSubLogger([
			'NES',
			'NextEditProvider',
		]);
		this._nextEditCache = new NextEditCache(
			this._workspace,
			this._logService,
			this._configService,
			this._expService,
		);
		this._specManager = this._register(
			new SpeculativeRequestManager(
				this._logger.createSubLogger('SpeculativeRequestManager'),
			),
		);
		...
		mapObservableArrayCached(
			...
			(doc, store) => {
				...
				store.add(
					toDisposable(() =>
						this._specManager.onDocumentClosed(doc.id),
					),
				);
			},
	...
	private _cancelPendingRequestDueToDocChange(
		...
		const isAsyncCompletions = this._configService.getExperimentBasedConfig(
			ConfigKey.TeamInternal.InlineEditsAsyncCompletions,
			this._expService,
		);
		...
		const activeDoc =
			this._pendingStatelessNextEditRequest.getActiveDocument();
		if (
			activeDoc.id === docId &&
			activeDoc.documentAfterEdits.value !== docValue.value
		) {
	...
	public async getNextEdit(
		...
		const logger = this._logger
			.createSubLogger(context.requestUuid.substring(4, 8))
			.withExtraTarget(
				LogTarget.fromCallback((_level, msg) => {
					logContext.trace(
						`[${Math.floor(sw.elapsed()).toString().padStart(4, ' ')}ms] ${msg}`,
					);
				}),
			);
		...
		logContext.setStatelessNextEditProviderId(
			this._statelessNextEditProvider.ID,
		);

		let result: NextEditResult;
		try {
			result = await this._getNextEditCanThrow(
				docId,
				context,
				now,
	...
	private async _getNextEditCanThrow(
		...
		logger.trace(
			`invoked with trigger id = ${context.changeHint === undefined ? 'undefined' : `uuid = ${context.changeHint.data.uuid}, reason = ${context.changeHint.data.reason}`}`,
		);
		...
		if (!doc) {
			...
			throw new BugIndicatingError(
				`Document "${docId.baseName}" not found`,
			);
		}
		...
		const nesConfigs = this.determineNesConfigs(
			telemetryBuilder,
			logContext,
		);
		...
		let edit:
			| { actualEdit: StringReplacement; isFromCursorJump: boolean }
			| undefined;
		...
		if (cachedEdit) {
			...
			if (actualEdit) {
				edit = {
					actualEdit,
					isFromCursorJump: cachedEdit.isFromCursorJump,
				};
			}
			isRebasedCachedEdit = !!cachedEdit.rebasedEdit;
			isSubsequentCachedEdit =
				cachedEdit.subsequentN !== undefined &&
				cachedEdit.subsequentN > 0;
			...
			telemetryBuilder.setSubsequentEditOrder(
				cachedEdit.rebasedEditIndex ?? cachedEdit.subsequentN,
			);
			...
			logger.trace(
				`fetching next edit with shouldExpandEditWindow=${shouldExpandEditWindow}`,
			);
			const providerRequestStartDateTime =
				this._configService.getExperimentBasedConfig(
					ConfigKey.TeamInternal
						.InlineEditsDebounceUseCoreRequestTime,
					this._expService,
				)
					? (context.requestIssuedDateTime ?? undefined)
					: undefined;
			req = new NextEditFetchRequest(
				context.requestUuid,
				logContext,
				providerRequestStartDateTime,
				false,
			);
			telemetryBuilder.setHeaderRequestId(req.headerRequestId);
			...
			const result = await this.fetchNextEdit(
				req,
				doc,
				nesConfigs,
				shouldExpandEditWindow,
			...
			if (result.isError()) {
				logger.trace(
					`failed to fetch next edit ${result.err.toString()}`,
				);
				telemetryBuilder.setStatus(`noEdit:${result.err.kind}`);
				error = result.err;
			} else {
				targetDocumentId = result.val.docId ?? targetDocumentId;
				const targetDoc = targetDocumentId
					? this._workspace.getDocument(targetDocumentId)!
					: doc;
				currentDocument = targetDoc.value.get();
				const docDidChange =
					targetDocumentId === doc.id &&
					startVersion.value !== currentDocument.value;
				...
				if (docDidChange) {
					...
					const suggestedNextEdit =
						result.val.rebasedEdit || result.val.edit;
					if (!suggestedNextEdit) {
						...
						edit = {
							actualEdit: suggestedNextEdit,
							isFromCursorJump: result.val.isFromCursorJump,
						};
						isFromSpeculativeRequest =
							result.val.isFromSpeculativeRequest ?? false;
		...
		if (
			error instanceof NoNextEditReason.FetchFailure ||
			error instanceof NoNextEditReason.Unexpected
		) {
			...
			if (error.nextCursorPosition === undefined) {
				...
				telemetryBuilder.setStatus(
					'emptyEditsButHasNextCursorPosition',
				);
				return new NextEditResult(logContext.requestId, req, {
					jumpToPosition: error.nextCursorPosition,
					targetDocumentId: error.nextCursorDocumentId,
					documentBeforeEdits: documentAtInvocationTime,
					isFromCursorJump: false,
					isSubsequentEdit: false,
				});
			}
		...
		const emptyResult = new NextEditResult(
			logContext.requestId,
			req,
			undefined,
		);
		...
		if (
			this._rejectionCollector.isRejected(
				targetDocumentId,
				edit.actualEdit,
			) ||
			(currentDocument &&
				this._nextEditCache.isRejectedNextEdit(
					targetDocumentId,
					currentDocument,
					edit.actualEdit,
				))
		...
		logContext.setResult(
			RootedLineEdit.fromEdit(
				new RootedEdit(
					documentAtInvocationTime,
					new StringEdit([edit.actualEdit]),
				),
			),
		);
		...
		assert(
			...
			'should be defined if edit is defined',
		);
		...
		const delay = this.computeMinimumResponseDelay(
			{
				triggerTime,
				isRebasedCachedEdit,
				isSubsequentCachedEdit,
	...
	private determineNesConfigs(
		...
		const nesConfigs: INesConfigs = {
			isAsyncCompletions: this._configService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsAsyncCompletions,
				this._expService,
			),
			isEagerBackupRequest: this._configService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsEagerBackupRequest,
				this._expService,
			),
		};
	...
	private _processDoc(doc: DocumentHistory): ProcessedDoc {
		...
		const recentEdit = RootedLineEdit.fromEdit(
			new RootedEdit(doc.lastEdit.base, doc.lastEdits.compose()),
	...
	private async fetchNextEdit(
		...
		telemetryBuilder: LlmNESTelemetryBuilder,
		cancellationToken: CancellationToken,
	): Promise<Result<CachedOrRebasedEdit, NoNextEditReason>> {
		const curDocId = doc.id;
		const logger = parentLogger.createSubLogger('fetchNextEdit');
		const historyContext =
			this._historyContextProvider.getHistoryContext(curDocId);

		if (!historyContext) {
			return Result.error(
				new NoNextEditReason.Unexpected(
					new Error('DocumentMissingInHistoryContext'),
				),
			);
		}
		...
		const cursorInRequestEditWindow = (request: StatelessNextEditRequest) =>
			!request.requestEditWindow ||
			!cursorAtInvocationTime ||
			request.requestEditWindow.containsCursor(cursorAtInvocationTime);

		// Check if we can reuse the regular pending request
		const pendingRequestStillCurrent =
			documentAtInvocationTime.value ===
			this._pendingStatelessNextEditRequest?.documentBeforeEdits.value;
		const cursorWithinPendingEditWindow =
			!this._pendingStatelessNextEditRequest ||
			cursorInRequestEditWindow(this._pendingStatelessNextEditRequest);
		const existingNextEditRequest =
			((pendingRequestStillCurrent || nesConfigs.isAsyncCompletions) &&
				cursorWithinPendingEditWindow &&
				!this._pendingStatelessNextEditRequest?.cancellationTokenSource
		...
		const speculativeRequestMatches =
			specPending?.docId === curDocId &&
			specPending?.postEditContent === documentAtInvocationTime.value &&
			!specPending.request.cancellationTokenSource.token
				.isCancellationRequested &&
			cursorInRequestEditWindow(specPending.request);
		const speculativeRequest = speculativeRequestMatches
			? specPending?.request
			: undefined;
		...
		if (requestToReuse) {
			...
			if (speculativeRequest) {
				logger.trace(
					`reusing speculative pending request (opportunityId=${speculativeRequest.opportunityId}, headerRequestId=${speculativeRequest.headerRequestId})`,
				);
				// Detach the speculative — caller is consuming it now.
				this._specManager.consumePending();
			} else {
				logger.trace(
					`reusing in-flight pending request (opportunityId=${requestToReuse.opportunityId}, headerRequestId=${requestToReuse.headerRequestId})`,
				);
			}
			...
			const reusedRequestKind = speculativeRequest
				? ReusedRequestKind.Speculative
				: ReusedRequestKind.Async;

			if (requestStillCurrent) {
				const nextEditResult = await this._joinNextEditRequest(
					requestToReuse,
					reusedRequestKind,
					telemetryBuilder,
					logContext,
				...
				telemetryBuilder.setStatelessNextEditTelemetry(
					nextEditResult.telemetry,
				);
				if (speculativeRequest) {
					const firstEdit = await requestToReuse.firstEdit.p;
					return firstEdit.map((val) => ({
						...val,
						isFromSpeculativeRequest: true,
					}));
				}
				return nextEditResult.nextEdit.isError()
					? nextEditResult.nextEdit
					: requestToReuse.firstEdit.p;
				...
				logger.trace(
					'starting eager backup request in parallel with rebase attempt',
				);
				...
				this._pendingStatelessNextEditRequest = null;
				const backupPromise = this._executeNewNextEditRequest(
					req,
					doc,
					historyContext,
					nesConfigs,
				...
				const cancelBackupRequest = () => {
					void backupPromise
						.then((r) =>
							r.nextEditRequest.cancellationTokenSource.cancel(),
						)
						.catch(() => undefined);
				};
				...
				if (cacheResult.isOk() && cacheResult.val.edit) {
					const rebaseResult =
						this._nextEditCache.tryRebaseCacheEntry(
							cacheResult.val,
							documentAtInvocationTime,
					...
					if (rebaseResult.edit) {
						logger.trace(
							'rebase succeeded, cancelling eager backup request',
						);
						cancelBackupRequest();
						telemetryBuilder.setStatelessNextEditTelemetry(
							nextEditResult.telemetry,
				...
				if (cancellationToken.isCancellationRequested) {
					logger.trace(
						'cancelled after rebase failed (eager backup path)',
					);
					cancelBackupRequest();
					telemetryBuilder.setStatelessNextEditTelemetry(
						nextEditResult.telemetry,
					...
					return Result.error(
						new NoNextEditReason.GotCancelled('afterFailedRebase'),
					);
				}
				...
				telemetryBuilder.setStatelessNextEditTelemetry(
					backupRes.nextEditResult.telemetry,
				);
				return backupRes.nextEditResult.nextEdit.isError()
					? backupRes.nextEditResult.nextEdit
					: backupRes.nextEditRequest.firstEdit.p;
			} else {
				const nextEditResult = await this._joinNextEditRequest(
					requestToReuse,
					reusedRequestKind,
				...
				if (existingNextEditRequest2) {
					logger.trace(
						'reusing 2nd existing next edit request after rebase failed',
					);
					const nextEditResult2 = await this._joinNextEditRequest(
						existingNextEditRequest2,
						ReusedRequestKind.Async,
						telemetryBuilder,
						logContext,
						cancellationToken,
					...
					telemetryBuilder.setStatelessNextEditTelemetry(
						nextEditResult2.telemetry,
					);
					return nextEditResult2.nextEdit.isError()
						? nextEditResult2.nextEdit
						: existingNextEditRequest2.firstEdit.p;
				}

				logger.trace(
					'creating new next edit request after rebase failed',
				);
			}
		}

		const res = await this._executeNewNextEditRequest(
			req,
			doc,
			historyContext,
			nesConfigs,
		...
		return nextEditResult.nextEdit.isError()
			? nextEditResult.nextEdit
			: nextEditRequest.firstEdit.p;
	}

	private async _joinNextEditRequest(
		nextEditRequest: StatelessNextEditRequest,
		reusedRequestKind: ReusedRequestKind,
		telemetryBuilder: LlmNESTelemetryBuilder,
		logContext: InlineEditRequestLogContext,
		cancellationToken: CancellationToken,
	) {
		...
		const disp = this._hookupCancellation(
			nextEditRequest,
			cancellationToken,
		);
		try {
	...
	private _logRebaseFailure(
		failureInfo: MarkdownLoggable | undefined,
		logContext: InlineEditRequestLogContext,
	): void {
		if (failureInfo) {
	...
	private async _executeNewNextEditRequest(
		...
		const activeDocAndIdx = assertDefined(
			historyContext.getDocumentAndIdx(curDocId),
		);
		const activeDocSelection = doc.selection.get()[0] as
			| OffsetRange
			| undefined;

		const projectedDocuments = historyContext.documents.map((doc) =>
			this._processDoc(doc),
		);

		const xtabEditHistory = this._xtabHistoryTracker.getHistory();

		const firstEdit = new DeferredPromise<
			Result<CachedOrRebasedEdit, NoNextEditReason>
		>();

		const nLinesEditWindow = shouldExpandEditWindow
			? this._configService.getExperimentBasedConfig(
					ConfigKey.TeamInternal.InlineEditsAutoExpandEditWindowLines,
					this._expService,
				)
			: undefined;
		...
		this._specManager.cancelIfMismatch(
			curDocId,
			nextEditRequest.documentBeforeEdits.value,
			SpeculativeCancelReason.Superseded,
		);
				...
				? autorunWithChanges(
						...
						(data) => {
							data.value.changes.forEach((edit) => {
								if (
									nextEditRequest.intermediateUserEdit &&
									!edit.isEmpty()
								) {
									nextEditRequest.intermediateUserEdit =
										nextEditRequest.intermediateUserEdit.compose(
											edit,
										);
									if (
		...
		const statePerDoc = createDocStateLookupMap(
			projectedDocuments,
			xtabEditHistory,
		);
		...
		const processEdit = (
			...
			telemetry: IStatelessNextEditTelemetry,
			...
			myLogger.trace(
				'resetting shouldExpandEditWindow to false due to receiving an edit',
			);
			this._shouldExpandEditWindow = false;

			const targetDocState = statePerDoc.get(
				streamedEdit.targetDocument ?? curDocId,
			);

			const singleLineEdit = streamedEdit.edit;
			const lineEdit = new LineEdit([singleLineEdit]);
			const edit = convertLineEditToEdit(
				lineEdit,
				targetDocState.baseDocState,
			);
					...
					firstEdit.complete(
						Result.error(
							new NoNextEditReason.Uncategorized(
								new Error('Rebased edit is undefined'),
							),
						),
					);
				}
			...
			targetDocState.editsSoFar =
				targetDocState.editsSoFar.compose(rebasedEdit);
			...
			if (rebasedEdit.replacements.length === 0) {
				...
				myLogger.trace(
					`WARNING: ${ithEdit} has ${rebasedEdit.replacements.length} edits, but expected only 1`,
				);
			} else {
				// populate the cache
				...
				cachedEdit = this._nextEditCache.setKthNextEdit(
					...
					req,
					{
						...
						originalEditWindow: streamedEdit.originalWindow,
						cursorOffset:
							targetDocState.docId === curDocId
								? activeDocSelection?.start
								: undefined,
					},
				);
			...
			if (!firstEdit.isSettled) {
				...
				logContext.setResult(
					new RootedLineEdit(targetDocState.docContents, lineEdit),
				); // this's correct without rebasing because this's the first edit
				firstEdit.complete(
					cachedEdit
						? Result.ok(cachedEdit)
						: Result.error(
								new NoNextEditReason.Unexpected(
									new Error('No cached edit'),
								),
							),
				);
			}

			targetDocState.docContents = rebasedEdit.applyOnText(
				targetDocState.docContents,
			);
		...
		const handleStreamEnd = (
			completionReason: NoNextEditReason,
			lastTelemetry: IStatelessNextEditTelemetry,
		) => {
			...
			if (
				...
				myLogger.trace(
					'resetting shouldExpandEditWindow to false due to NoSuggestions',
				);
				this._shouldExpandEditWindow = false;
			}
			...
			if (statePerDoc.get(curDocId).nextEdits.length) {
				myLogger.trace(
					`${statePerDoc.get(curDocId).nextEdits.length} edits returned`,
				);
			} else {
				...
				if (
					...
					const reducedWindow = window
						? computeReducedWindow(
								window,
								activeDocSelection,
								documentBeforeEdits,
							)
						: undefined;
					this._nextEditCache.setNoNextEdit(
						curDocId,
						documentBeforeEdits,
						reducedWindow,
						req,
			...
			const resultForTelemetry: Result<void, NoNextEditReason> =
				statePerDoc.get(curDocId).nextEdits.length > 0
					? Result.ok(undefined)
					: Result.error(completionReason);
			const result = new StatelessNextEditResult(
				resultForTelemetry,
				lastTelemetry,
			);
			...
			const scheduled = this._specManager.consumeScheduled(
				nextEditRequest.headerRequestId,
			);
			if (scheduled) {
		...
		try {
			...
			if (res.done) {
				...
				nextEditResult = handleStreamEnd(
					completionReason,
				...
				(async () => {
					try {
						...
						while (!res.done) {
							...
							this._specManager.consumeScheduled(
								nextEditRequest.headerRequestId,
							);
						...
						logger.trace(
							`Error while streaming further edits: ${ErrorUtils.toString(err)}`,
						);
						const errorReason = new NoNextEditReason.Unexpected(
							ErrorUtils.fromUnknown(err),
						);
						handleStreamEnd(errorReason, firstTelemetry);
					}
				})();

				nextEditResult = new StatelessNextEditResult(
					Result.ok(undefined),
					firstTelemetry,
				);
			}
	...
	private _hookupCancellation(
		nextEditRequest: StatelessNextEditRequest,
		cancellationToken: CancellationToken,
		attachedDisposable?: IDisposable,
	): IDisposable {
	...
	private computeMinimumResponseDelay(
		{
			...
			triggerTime: number;
			isRebasedCachedEdit: boolean;
			isSubsequentCachedEdit: boolean;
		...
		if (!enforceCacheDelay) {
			logger.trace(
				'[minimumDelay] no minimum delay enforced due to enforceCacheDelay being false',
			);
			return 0;
		}

		const cacheDelay = this._configService.getExperimentBasedConfig(
			ConfigKey.TeamInternal.InlineEditsCacheDelay,
			this._expService,
		);
		const rebasedCacheDelay = this._configService.getExperimentBasedConfig(
			ConfigKey.TeamInternal.InlineEditsRebasedCacheDelay,
			this._expService,
		);
		const subsequentCacheDelay =
			this._configService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsSubsequentCacheDelay,
				this._expService,
			);
		const speculativeRequestDelay =
			this._configService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsSpeculativeRequestDelay,
				this._expService,
			);
		...
		if (isRebasedCachedEdit && rebasedCacheDelay !== undefined) {
			...
			isSubsequentCachedEdit &&
			subsequentCacheDelay !== undefined
		) {
			minimumResponseDelay = subsequentCacheDelay;
		} else if (
			isFromSpeculativeRequest &&
			speculativeRequestDelay !== undefined
		) {
	...
	public handleShown(suggestion: NextEditResult) {
		...
		const speculativeRequestsEnablement =
			this._configService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsSpeculativeRequests,
				this._expService,
			);
		if (
			speculativeRequestsEnablement === SpeculativeRequestsEnablement.On
		) {
			...
			if (
				originatingRequest &&
				originatingRequest.headerRequestId ===
					suggestion.source.headerRequestId
			) {
	...
	private async _triggerSpeculativeRequest(
		...
		const sw = new StopWatch();
		const logger = this._logger
			.createSubLogger('_triggerSpeculativeRequest')
			.withExtraTarget(
				LogTarget.fromCallback((_level, msg) => {
					logContext.trace(
						`[${Math.floor(sw.elapsed()).toString().padStart(4, ' ')}ms] ${msg}`,
		...
		const postEditContent = result.edit.replace(
			result.documentBeforeEdits.value,
		);
		const preciseEdit = result.edit.removeCommonSuffixPrefix(
			result.documentBeforeEdits.value,
		);
		const postEditCursorOffset =
			preciseEdit.replaceRange.start + preciseEdit.newText.length;
		const postEditCursorOffsetRange = new OffsetRange(
			postEditCursorOffset,
			postEditCursorOffset,
		);
		const selections = [postEditCursorOffsetRange];
		const rootedEdit = new RootedEdit(
			result.documentBeforeEdits,
			new StringEdit([result.edit]),
		);

		const postEditContentST = new StringText(postEditContent);
		let cachedEdit = this._nextEditCache.lookupNextEdit(
			docId,
			postEditContentST,
			selections,
		);
		...
		if (cachedEdit) {
			...
			if (cachedEdit.edit) {
				...
				logger.trace(
					'have cached no-suggestions entry for post-edit state, but it has an edit window. Checking if shifting selection based on cursor placement config can yield a cached edit',
				);
				const cursorPlacement =
					this._configService.getExperimentBasedConfig(
						ConfigKey.TeamInternal
							.InlineEditsSpeculativeRequestsCursorPlacement,
						this._expService,
					);
				if (
					cursorPlacement ===
					...
					logger.trace(
						'cursor placement config is AfterEditWindow, shifting selection to after edit window',
					);
					shiftedSelection =
						NextEditProvider.shiftSelectionAfterEditWindow(
							postEditContentST,
							cachedEdit.editWindow,
						);
					cachedEdit = this._nextEditCache.lookupNextEdit(
						docId,
						postEditContentST,
						[shiftedSelection],
					);
					if (cachedEdit?.edit) {
						logger.trace(
							'already have cached edit for post-edit state (after shifting selection)',
						);
						return;
					} else {
						logger.trace(
							'no cached edit even after shifting selection',
						);
					}
				} else {
					logger.trace(
						`cursor placement config is ${cursorPlacement}, not shifting selection`,
					);
				}
			} else {
				logger.trace(
					'already have cached no-suggestions entry for post-edit state',
				);
				return;
			}
		}
		...
		if (
			existingSpec?.docId === docId &&
			existingSpec?.postEditContent === postEditContent
		) {
			logger.trace(
				'already have speculative request for post-edit state',
			);
			return;
		}
		...
		const historyContext =
			this._historyContextProvider.getHistoryContext(docId);
		if (!historyContext) {
			logger.trace('no history context for speculative request');
			return;
		}

		const req = new NextEditFetchRequest(
			`sp-${suggestion.source.opportunityId}`,
			logContext,
			undefined,
			true,
			`sp-${generateUuid()}`,
		...
		logger.trace(
			`triggering speculative request for post-edit state (opportunityId=${req.opportunityId}, headerRequestId=${req.headerRequestId})`,
		);
		...
		try {
			const speculativeRequest = await this._createSpeculativeRequest(
				...
				result.edit,
				{
					triggeredBySpeculativeRequest:
						suggestion.source.isSpeculative,
					isSubsequentEdit:
						suggestion.result?.isSubsequentEdit ?? false,
				},
				logger,
			);
			...
			if (speculativeRequest) {
				...
				const trajectoryPrefix = preEditValue.slice(
					0,
					preciseEdit.replaceRange.start,
				);
				const trajectorySuffix = preEditValue.slice(
					preciseEdit.replaceRange.endExclusive,
				);
			...
			logger.trace(
				`speculative request failed: ${ErrorUtils.toString(e)}`,
			);
		}
	}
	...
	private async _createSpeculativeRequest(
		...
		const projectedDocuments: ProcessedDoc[] = historyContext.documents.map(
			(docHist) => {
				if (docHist.docId !== curDocId) {
					...
					const workspaceRoot =
						this._workspace.getWorkspaceRoot(curDocId);
					const postEditEdit = new StringEdit([appliedEdit]);
					const postEditLineEdit = RootedLineEdit.fromEdit(
						new RootedEdit(doc.value.get(), postEditEdit),
					).removeCommonSuffixPrefixLines().edit;
		...
		const xtabEditHistory = this._xtabHistoryTracker.getHistory();
		const suggestedEdit: IXtabHistoryEditEntry = {
			kind: 'edit',
			docId: curDocId,
			edit: rootedEdit,
		};
		...
		const autoExpandEditWindowLinesSetting =
			this._configService.getExperimentBasedConfig(
				ConfigKey.TeamInternal
					.InlineEditsSpeculativeRequestsAutoExpandEditWindowLines,
				this._expService,
			);
		...
		switch (autoExpandEditWindowLinesSetting) {
			...
			case SpeculativeRequestsAutoExpandEditWindowLines.Always:
				nLinesEditWindow = this._configService.getExperimentBasedConfig(
					ConfigKey.TeamInternal.InlineEditsAutoExpandEditWindowLines,
					this._expService,
				);
				break;
			case SpeculativeRequestsAutoExpandEditWindowLines.Smart: {
				const isModelOnRightTrack =
					triggeredBySpeculativeRequest || isSubsequentEdit;
				nLinesEditWindow = isModelOnRightTrack
					? this._configService.getExperimentBasedConfig(
							ConfigKey.TeamInternal
								.InlineEditsAutoExpandEditWindowLines,
							this._expService,
						)
					: undefined;
				break;
		...
		void this._requestLogger.captureInvocation(capturingToken, async () => {
			...
			try {
				await this._runSpeculativeProviderCall(
					nextEditRequest,
					projectedDocuments,
					curDocId,
					req,
					shiftedSelection.start,
					logger,
				);
			} catch (e) {
	...
	private async _runSpeculativeProviderCall(
		...
		try {
			...
			if (res.done) {
				...
				(async () => {
					while (!res.done) {
						...
						const rebasedEdit = edit.tryRebase(
							targetDocState.editsSoFar,
						);

						if (rebasedEdit === undefined) {
							logger.trace(
								`speculative edit ${ithEdit} rebasing failed`,
							);
							res = await editStream.next();
							continue;
						}
						...
						if (rebasedEdit.replacements.length === 1) {
							const nextEditReplacement =
								rebasedEdit.replacements[0];
							targetDocState.nextEdits.push(nextEditReplacement);
							...
							const cachedEdit =
								this._nextEditCache.setKthNextEdit(
									...
									ithEdit,
									ithEdit === 0
										? targetDocState.nextEdits
										: undefined,
									undefined, // no userEditSince for speculative
									req,
									{
										isFromCursorJump:
											streamedEdit.isFromCursorJump,
										originalEditWindow:
											streamedEdit.originalWindow,
										cursorOffset:
											targetDocState.docId === curDocId
												? cursorOffset
												: undefined,
							...
							if (
								!nextEditRequest.firstEdit.isSettled &&
								cachedEdit
							) {
								nextEditRequest.firstEdit.complete(
									Result.ok(cachedEdit),
								);
								nextEditRequest.setResult(
									new StatelessNextEditResult(
					...
					if (!nextEditRequest.firstEdit.isSettled) {
						nextEditRequest.firstEdit.complete(
							Result.error(
								new NoNextEditReason.Uncategorized(
									new Error(
										'Speculative request ended without edits',
									),
								),
							),
						);
			...
			logger.trace(
				`speculative request completed with ${ithEdit + 1} edits`,
			);
		} catch (e) {
			logger.trace(
				`speculative provider call error: ${ErrorUtils.toString(e)}`,
			);
		}
	}

	private static shiftSelectionAfterEditWindow(
		postEditContentST: StringText,
		editWindowOffsetRange: OffsetRange,
	): OffsetRange {
		const trans = postEditContentST.getTransformer();
		const endOfEditWindow = trans.getPosition(
			editWindowOffsetRange.endExclusive - 1,
		);
		const shiftedCursorLineNumber =
			endOfEditWindow.lineNumber + 1 <
			postEditContentST.lineRange.endLineNumberExclusive
				? endOfEditWindow.lineNumber + 1
				: endOfEditWindow.lineNumber;
		const shiftedSelectionCursorOffset = trans.getOffset(
			new Position(shiftedCursorLineNumber, 1),
		);
		const shiftedSelection = new OffsetRange(
			shiftedSelectionCursorOffset,
			shiftedSelectionCursorOffset,
		);
	...
	public handleAcceptance(docId: DocumentId, suggestion: NextEditResult) {
		...
		const logger = this._logger
			.createSubLogger(suggestion.source.opportunityId.substring(4, 8))
			.createSubLogger('handleAcceptance');
		if (suggestion === this._lastNextEditResult) {
			logger.trace(
				'setting shouldExpandEditWindow to true due to acceptance of last suggestion',
			);
			this._shouldExpandEditWindow = true;
		} else {
			logger.trace(
				'NOT setting shouldExpandEditWindow to true because suggestion is not the last suggestion',
			);
		}
	}
	...
	public handleRejection(docId: DocumentId, suggestion: NextEditResult) {
		assertType(
			suggestion.result,
			'@ulugbekna: undefined edit cannot be rejected?',
		);
		...
		if (shownDuration > 1000 && suggestion.result.edit) {
			...
			this._nextEditCache.rejectedNextEdit(
				suggestion.source.headerRequestId,
			);
		}
	...
	public handleIgnored(
		...
		if (wasShown && !wasSuperseded) {
			...
			this._specManager.cancelAll(
				SpeculativeCancelReason.IgnoredDismissed,
			);
	...
	private async runSnippy(docId: DocumentId, suggestion: NextEditResult) {
		if (
			...
			suggestion.result.edit === undefined
		) {
			return;
		}
		this._snippyService.handlePostInsertion(
			docId.toUri(),
			suggestion.result.documentBeforeEdits,
			suggestion.result.edit,
		);
	}

	private _addLiveLogContextEntry(
		logContext: InlineEditRequestLogContext,
		debugNameOverride?: string,
	): void {
