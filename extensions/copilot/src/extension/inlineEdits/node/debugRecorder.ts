...
export class DebugRecorder extends Disposable {
	...
	private readonly _documentHistories = new Map<
		DocumentId,
		DocumentHistory
	>();
	...
	public getTimestamp(): number {
		...
		if (
			...
			newTimestamp <= this._lastTimestamp
		) {
	...
	public getRecentLog(
		bookmark: DebugRecorderBookmark | undefined = undefined,
	): LogEntry[] | undefined {
		if (!this._workspaceRoot) {
...
class DocumentHistory {
	...
	public cleanUpHistory(): void {
		...
		while (
			...
			this._edits[0].instant < earliestTime
		) {
	...
	getDocumentLog(
		bookmark: DebugRecorderBookmark | undefined,
	): { entry: LogEntry; sortTime: number }[] {
		this.cleanUpHistory();
		...
		log.push({
			entry: { kind: 'opened', id: this.id, time: this._baseValueTime },
			sortTime: this._baseValueTime,
		});
		...
		for (const editOrSelectionChange of this._edits) {
			...
			if (editOrSelectionChange.kind === 'selections') {
				const serializedOffsetRange: ISerializedOffsetRange[] =
					editOrSelectionChange.selections.map((s) => [
						s.start,
						s.endExclusive,
					]);
				log.push({
