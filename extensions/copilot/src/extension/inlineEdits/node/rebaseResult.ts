...
export class RebaseFailureInfo implements MarkdownLoggable {
	...
	toMarkdown(): string {
		...
		for (let i = 0; i < this.originalEdits.length; i++) {
			...
			lines.push(
				`  - replaces: \`${JSON.stringify(edit.replaceRange.substring(this.originalDocument))}\``,
			);
		...
		if (this.editWindow) {
			...
			lines.push(
				`Content: \`${JSON.stringify(this.editWindow.substring(this.originalDocument))}\``,
			);
		}
		...
		if (this.currentSelection.length > 0) {
			lines.push('');
			lines.push(
				`### Cursor: ${this.currentSelection.map((s) => s.toString()).join(', ')}`,
			);
		}
		...
		lines.push('```');
		try {
			const intended = new StringEdit(this.originalEdits.slice()).apply(
	...
	private _generateTest(): string {
		...
		lines.push(
			`\tconst originalDocument = ${toBacktickLiteral(this.originalDocument)};`,
		);

		lines.push('\tconst originalEdits = [');
		for (const edit of this.originalEdits) {
			lines.push(
				`\t\tStringReplacement.replace(new OffsetRange(${edit.replaceRange.start}, ${edit.replaceRange.endExclusive}), ${toBacktickLiteral(edit.newText)}),`,
			);
		}
		lines.push('\t];');
		...
		for (const replacement of this.userEditSince.replacements) {
			lines.push(
				`\t\tStringReplacement.replace(new OffsetRange(${replacement.replaceRange.start}, ${replacement.replaceRange.endExclusive}), ${toBacktickLiteral(replacement.newText)}),`,
			);
		}
		lines.push('\t]);');

		lines.push(
			`\tconst currentDocumentContent = ${toBacktickLiteral(this.currentDocument)};`,
		);

		if (this.editWindow) {
			lines.push(
				`\tconst editWindow = new OffsetRange(${this.editWindow.start}, ${this.editWindow.endExclusive});`,
			);
		} else {
		...
		lines.push(
			`\tconst currentSelection = [${this.currentSelection.map((s) => `new OffsetRange(${s.start}, ${s.endExclusive})`).join(', ')}];`,
		);

		const configEntries: string[] = [];
		if (this.nesRebaseConfigs.absorbSubsequenceTyping) {
			configEntries.push(
				`absorbSubsequenceTyping: ${this.nesRebaseConfigs.absorbSubsequenceTyping}`,
			);
		}
		if (this.nesRebaseConfigs.reverseAgreement) {
			configEntries.push(
				`reverseAgreement: ${this.nesRebaseConfigs.reverseAgreement}`,
			);
		}
		configEntries.push(
			`maxImperfectAgreementLength: ${this.nesRebaseConfigs.maxImperfectAgreementLength}`,
		);
		...
		lines.push(
			'\texpect(userEditSince.apply(originalDocument)).toBe(currentDocumentContent);',
		);

		const configsArg = ', nesConfigs';
		lines.push(
			`\texpect(tryRebase(originalDocument, editWindow, originalEdits, [], userEditSince, currentDocumentContent, currentSelection, 'strict', logger${configsArg})).toMatchInlineSnapshot();`,
		);
