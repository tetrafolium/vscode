...
export function tryRebase(
	...
	logger: ILogger,
	nesConfigs: NesRebaseConfigs = { maxImperfectAgreementLength },
):
...
function _tryRebase(
	...
	const userEditSince =
		userEditSinceOrig.removeCommonSuffixAndPrefix(originalDocument);
	const cursorRange = currentSelection[0];
	if (editWindow && cursorRange) {
		const updatedEditWindow =
			userEditSince.applyToOffsetRangeOrUndefined(editWindow);
	...
	if (detailedEdits.length < originalEdits.length) {
		...
		for (let index = 0; index < detailedEdits.length; index++) {
			...
			intermediateDocument =
				StringEdit.single(edit).apply(intermediateDocument);
		}
		for (
			let index = detailedEdits.length;
			index < originalEdits.length;
			index++
		) {
			const edit = originalEdits[index];
	...
	const diffedEdit = AnnotatedStringEdit.compose(
		detailedEdits.map((edits) => AnnotatedStringEdit.create(edits)),
	);
	...
	for (let index = 0; index < grouped.length; index++) {
		...
		const range = OffsetRange.fromTo(
			group[0].replaceRange.start,
			group[group.length - 1].replaceRange.endExclusive,
		);
		const newText = group
			.map((edit, i, a) => {
				if (i > 0) {
					return (
						currentDocumentContent.substring(
							a[i - 1].replaceRange.endExclusive,
							edit.replaceRange.start,
						) + edit.newText
					);
				} else {
		...
		if (
			!resultEdit.removeCommonSuffixAndPrefix(currentDocumentContent)
				.isEmpty
		) {
			resultEdits.push({
				rebasedEdit: resultEdit,
				rebasedEditIndex: index,
			});
		}
	}
	if (
		...
		new SingleEdits(originalEdits).apply(originalDocument) !==
			StringEdit.create(resultEdits.map((r) => r.rebasedEdit)).apply(
				currentDocumentContent,
			)
	) {
...
function tryRebaseEdits<T extends IEditData<T>>(
	...
	while (
		ourIdx < ours.replacements.length ||
		baseIdx < base.replacements.length
	) {
		...
		if (!ourEdit) {
			...
			if (!ourE.replaceRange.containsRange(baseEdit.replaceRange)) {
				...
				if (ourE.replaceRange.start > baseEdit.replaceRange.start) {
					...
					const added = content.substring(
						baseEdit.replaceRange.start,
						ourE.replaceRange.start,
					);
				...
				// Skipping the case where there is another edit for now because we might have to merge with it first.
					...
					ourE.replaceRange.endExclusive <
			...
			if (ourE.replaceRange.intersectsOrTouches(baseEdit.replaceRange)) {
				if (
					...
					while (
						baseE &&
						ourE.replaceRange.containsRange(baseE.replaceRange)
					) {
						...
						delta +=
							baseE.newText.length - baseE.replaceRange.length;
						previousBaseE = baseE;
					...
					nesConfigs.reverseAgreement &&
					ourEdit.replaceRange.equals(baseEdit.replaceRange)
				) {
					...
					while (
						ourIdx < ours.replacements.length &&
						baseEdit.replaceRange.containsRange(
							ours.replacements[ourIdx].replaceRange,
						)
					) {
						const curOurE = ours.replacements[ourIdx];

						// Account for gap content between previous our-edit end and current our-edit start
						const gapStart = previousOurE
							? previousOurE.replaceRange.endExclusive
							: baseEdit.replaceRange.start;
						const gapText =
							gapStart < curOurE.replaceRange.start
								? content.substring(
										gapStart,
										curOurE.replaceRange.start,
									)
								: '';
						const effectiveText = gapText + curOurE.newText;

						// Try full consumption: model text found entirely within user text
						const j = baseEdit.newText.indexOf(
							effectiveText,
							baseNewTextOffset,
						);
						const strictRejected =
							...
							(j - baseNewTextOffset > maxAgreementOffset ||
								(j - baseNewTextOffset > 0 &&
									effectiveText.length >
										nesConfigs.maxImperfectAgreementLength));
						...
						const remainingBase =
							baseEdit.newText.substring(baseNewTextOffset);
						if (
							remainingBase.length > 0 &&
							effectiveText.startsWith(remainingBase)
						) {
							const consumedFromNewText = Math.max(
								0,
								remainingBase.length - gapText.length,
							);
							const unconsumedNewText =
					...
					if (
						...
						const lastOurEnd = previousOurE
							? previousOurE.replaceRange.endExclusive
							: baseEdit.replaceRange.start;
						const trailingGap = content.substring(
							lastOurEnd,
							baseEdit.replaceRange.endExclusive,
						);
...
function agreementIndexOf<T extends IEditData<T>>(
	...
	const minStart = previousBaseE
		? previousBaseE.replaceRange.endExclusive
		: ourE.replaceRange.start;
	if (minStart < baseE.replaceRange.start) {
		baseE = new StringReplacement(
			...
			content.substring(minStart, baseE.replaceRange.start) +
				baseE.newText,
		);
	}
	...
	const strictRejected =
		...
		(j > maxAgreementOffset ||
			(j > 0 &&
				baseE.newText.length > nesConfigs.maxImperfectAgreementLength));
	if (j !== -1 && !strictRejected) {
	...
	if (
		nesConfigs.absorbSubsequenceTyping &&
		isAutoClosePair(originalBaseNewText) &&
		isSubsequenceOf(
			originalBaseNewText,
			ourE.newText.substring(ourNewTextOffset),
		)
	) {
	...
	return result.changes
		.map((change) =>
			(change.innerChanges || []).map((innerChange) => {
				const range = originalText
					.getTransformer()
					.getOffsetRange(innerChange.originalRange);
				const newText = modifiedText.getValueOfRange(
