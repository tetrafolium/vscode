...
suite('filter recording for sensitive files', () => {
	...
	test('should filter out .env files and variants', () => {
		...
		expect(
			result.filter((e) => e.kind === 'documentEncountered'),
		).toHaveLength(1);
		expect(
			result.find((e) => e.kind === 'documentEncountered'),
		).toMatchObject({
			relativePath: 'src/index.ts',
		});
	});
	...
	test('should filter out private key files', () => {
		const log: LogEntry[] = [
			...
			},
			{
				kind: 'documentEncountered',
				id: 3,
				relativePath: 'cert.p12',
				time: 0,
			},
			{
				kind: 'documentEncountered',
				id: 4,
				relativePath: 'cert.pfx',
				time: 0,
			},
			{
				kind: 'documentEncountered',
	...
	test('should filter out files in sensitive directories', () => {
		const log: LogEntry[] = [
			...
			},
			{
				kind: 'documentEncountered',
				id: 3,
				relativePath: '.gnupg/private-keys-v1.d/key.gpg',
				time: 0,
			},
			{
				kind: 'documentEncountered',
	...
	test('should filter exact password/token data files but not code files with those words', () => {
		const log: LogEntry[] = [
			...
			// These should be filtered (exact sensitive data files)
			{
				kind: 'documentEncountered',
				id: 1,
				relativePath: 'password.txt',
				time: 0,
			},
			{
				kind: 'documentEncountered',
				id: 2,
				relativePath: 'passwords.json',
				time: 0,
			},
			{
				kind: 'documentEncountered',
			...
			// These should NOT be filtered (code files that deal with passwords/tokens)
			{
				kind: 'documentEncountered',
				id: 5,
				relativePath: 'passwordValidator.ts',
				time: 0,
			},
			{
				kind: 'documentEncountered',
				id: 6,
				relativePath: 'tokenAnalyzer.ts',
				time: 0,
			},
			{
				kind: 'documentEncountered',
				id: 7,
				relativePath: 'auth/refreshToken.service.ts',
				time: 0,
			},
			{
				kind: 'documentEncountered',
				id: 8,
				relativePath: 'utils/passwordStrength.ts',
				time: 0,
			},
		];
		...
		const remainingPaths = result
			.filter(
				(e): e is LogEntry & { kind: 'documentEncountered' } =>
					e.kind === 'documentEncountered',
			)
