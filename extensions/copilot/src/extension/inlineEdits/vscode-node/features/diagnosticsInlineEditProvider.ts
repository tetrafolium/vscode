...
} from './diagnosticsCompletionProcessor';
		...
		>
{
	...
	async getNextEdit(
		...
		throw new BugIndicatingError(
			'DiagnosticsNextEditProvider does not support getNextEdit, use runUntilNextEdit instead',
		);
	}
	...
	async runUntilNextEdit(
		...
		try {
			...
			if (cancellationToken.isCancellationRequested) {
				...
				return new DiagnosticsNextEditResult(
			...
			const initialResult = this._getResultForCurrentState(
				docId,
				logContext,
				tb,
			);
			...
			const asyncResult = await raceCancellation(
				new Promise<DiagnosticsNextEditResult>((resolve) => {
					...
					disposables.add(
						this._diagnosticsCompletionHandler.onDidChange(() => {
							const completionResult =
								this._getResultForCurrentState(
									docId,
									logContext,
									tb,
								);
							if (
								...
								!completionResult.workInProgress
							) {
			...
			this._logger.trace(
				'DiagnosticsInlineCompletionProvider runUntilNextEdit complete' +
	...
	private _getResultForCurrentState(
		docId: DocumentId,
		logContext: InlineEditRequestLogContext,
		tb: DiagnosticsTelemetryBuilder,
	): DiagnosticsNextEditResult {
		const completionResult =
			this._diagnosticsCompletionHandler.getCurrentState(docId);
		const telemetry = new DiagnosticsTelemetryBuilder();
		const diagnosticEditResult = this._createNextEditResult(
			completionResult,
			logContext,
			telemetry,
		);
	...
	private _createNextEditResult(
		...
		telemetry.droppedReasons.forEach((reason) =>
			tb.addDroppedReason(reason),
		);
	...
	handleAcceptance(
		...
		if (!completionResult) {
			throw new BugIndicatingError(
				'Completion result is undefined when accepted',
			);
		}

		this._lastAcceptedItem = {
			item: completionResult.item,
			time: Date.now(),
		};
		this._lastOutcome = NesOutcome.Accepted;
		this._diagnosticsCompletionHandler.handleEndOfLifetime(
			completionResult.item,
			{ kind: vscode.InlineCompletionEndOfLifeReasonKind.Accepted },
		);
	}

	private _lastAcceptedItem:
		| { item: DiagnosticCompletionItem; time: number }
		| undefined = undefined;
	...
	handleRejection(
		...
		if (!completionResult) {
			throw new BugIndicatingError(
				'Completion result is undefined when rejected',
			);
		}
	...
	handleIgnored(
		...
		const supersededByItem =
			supersededBy instanceof DiagnosticsNextEditResult
				? supersededBy?.result?.item
				: undefined;
