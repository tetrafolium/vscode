...
export class AnyDiagnosticCompletionItem extends DiagnosticCompletionItem {
	...
	protected override _getDisplayLocation():
		...
		return {
			...
			label: this._nextEditDisplayLabel,
		};
	}
}
...
export class AnyDiagnosticCompletionProvider implements IDiagnosticCompletionProvider<AnyDiagnosticCompletionItem> {
	...
	public providesCompletionsForDiagnostic(
		...
		return isDiagnosticWithinDistance(
			workspaceDocument,
			diagnostic,
			pos,
			5,
	...
	async provideDiagnosticCompletionItem(
		...
		for (const diagnostic of sortedDiagnostics) {
			...
			if (availableCodeActions === undefined) {
				log(
					`Fetching code actions likely timed out for \`${diagnostic.message}\``,
					logContext,
					this._logger,
				);
				continue;
			...
			const codeActionsFixingCodeAction = availableCodeActions.filter(
				(action) => doesCodeActionFixDiagnostics(action, diagnostic),
			);
			...
			const filteredCodeActionsWithEdit = filterCodeActions(
				codeActionsFixingCodeAction,
			);
			...
			const editDistance = Math.abs(
				joinedEdit.range.startLineNumber - pos.lineNumber,
			);
			if (editDistance > 12) {
			...
			log(
				`Created Completion Item for diagnostic: ${diagnostic.message}: ${item.toLineEdit().toString()}`,
			);
			return item;
		}
...
function doesCodeActionFixDiagnostics(
	action: CodeActionData,
	diagnostic: Diagnostic,
): boolean {
	return action.diagnostics.some(
		(d) =>
			diagnostic.data.message === d.message &&
			diagnostic.data.range.equals(d.range),
	);
}
...
function filterCodeActions(
	codeActionsWithEdit: CodeActionData[],
): CodeActionData[] {
	return codeActionsWithEdit.filter((action) => {
		...
		if (action.title === 'Infer parameter types from usage') {
			...
			if (
				edit.length === 1 &&
				['any', 'unknown', 'undefined'].some((e) =>
					edit[0].text.includes(e),
				)
			) {
				return false;
			}
