...
class ImportCodeAction {
	...
	compareTo(other: ImportCodeAction): number {
		if (
			this.hasExistingSameFileImport &&
			!other.hasExistingSameFileImport
		) {
			return -1;
		}
		if (
		...
		if (
			(this.importSource === ImportSource.local &&
				other.importSource !== ImportSource.local) ||
			(this.importSource !== ImportSource.external &&
				other.importSource === ImportSource.external)
		) {
			return -1;
		}
		...
		if (
			...
			other.importSource !== ImportSource.unknown
		) {
...
export class ImportDiagnosticCompletionItem extends DiagnosticCompletionItem {
	...
	constructor(
		...
		super(
			ImportDiagnosticCompletionItem.type,
			diagnostic,
			_importCodeAction.edit,
			workspaceDocument,
		);
		...
		if (isAbsolute(this._importCodeAction.importPath)) {
			...
			importFilePath = resolvePath(
				dirname(workspaceDocument.id.toUri()),
				this._importCodeAction.importPath,
			).path;
		}
	...
	protected override _getDisplayLocation():
		| INextEditDisplayLocation
		| undefined {
		const transformer = this._workspaceDocument.value
			.get()
			.getTransformer();
		return {
			range: transformer.getRange(this.diagnostic.range),
			label: this._importLabel,
		};
	}
}
	...
	async updateNodeModules(): Promise<void> {
		...
		const workspaceNodeModules = await Promise.all(
			workspaceFolders.map(async (folder) => {
				try {
					const nodeModulesFolder = URI.joinPath(
						folder,
						'node_modules',
					);
					...
					const resourceStat =
						await this._fileService.stat(nodeModulesFolder);
					...
					const entries =
						await this._fileService.readDirectory(
							nodeModulesFolder,
						);
...
export class ImportDiagnosticCompletionProvider implements IDiagnosticCompletionProvider<ImportDiagnosticCompletionItem> {
	...
	constructor(
		...
		this._workspaceInfo = new WorkspaceInformation(
	...
	public providesCompletionsForDiagnostic(
		workspaceDocument: IVSCodeObservableDocument,
		diagnostic: Diagnostic,
		language: LanguageId,
		pos: Position,
		...
		if (
			!isDiagnosticWithinDistance(workspaceDocument, diagnostic, pos, 12)
		) {
			return false;
		}
	...
	async provideDiagnosticCompletionItem(
		...
		const availableCodeActions = await workspaceDocument.getCodeActions(
			importDiagnosticToFix.range,
			3,
			token,
		);
		...
		if (availableImportCodeActions.length === 0) {
			log(
				'No import code actions found in the available code actions',
				logContext,
				this._logger,
			);
			return null;
		...
		const sortedImportCodeActions = availableImportCodeActions.sort(
			(a, b) => a.compareTo(b),
		);
		...
		for (const codeAction of sortedImportCodeActions) {
			const importCodeActionLabel =
				...
				codeAction.importSource !== ImportSource.external
	...
	isCompletionItemStillValid(
		...
		return (
			item.diagnostic.range.substring(
				workspaceDocument.value.get().value,
			) === item.importItemName
		);
	}

	private _hasImportBeenRejected(
		item: ImportDiagnosticCompletionItem,
	): boolean {
	...
	private _getImportCodeActions(
		...
		if (!importHandler) {
			throw new Error(
				`No import handler found for language: ${language}`,
			);
		}
		...
		for (const codeAction of codeActions) {
			...
			const joinedEdit = TextReplacement.joinReplacements(
				codeAction.edits,
				documentContent,
			);
			...
			if (codeAction.diagnostics && codeAction.diagnostics.length > 0) {
				codeActionImportName =
					codeAction.diagnostics[0].range.substring(
						documentContent.value,
					);
			}

			const importDetails = importHandler.getImportDetails(
				codeAction,
				codeActionImportName,
				workspaceInfo,
			);
			...
			if (
				codeActionImportName.length < 2 ||
				importHandler.isImportInIgnoreList(importCodeAction)
			) {
				continue;
			}
...
class JavascriptImportHandler implements ILanguageImportHandler {
	private static CodeActionTitlePrefixes = [
		'Add import from',
		'Update import from',
	];
	...
	isImportCodeAction(codeAction: CodeActionData): boolean {
		return JavascriptImportHandler.CodeActionTitlePrefixes.some((prefix) =>
	...
	isImportInIgnoreList(importCodeAction: ImportCodeAction): boolean {
		...
		if (
			importCodeAction.importSource === ImportSource.external &&
			importCodeAction.importPath.includes('/')
		) {
		...
		if (
			importCodeAction.importSource === ImportSource.external &&
			importCodeAction.importName ===
				importCodeAction.importName.toLowerCase()
		) {
		...
		if (
			JavascriptImportHandler.ImportsToIgnore.has(
				importCodeAction.importName,
			)
		) {
			return true;
		...
		if (
			JavascriptImportHandler.ModulesToIgnore.has(
				importCodeAction.importPath.split(':')[0],
			)
		) {
			return true;
		}
	...
	getImportDetails(
		...
		const importTitlePrefix =
			JavascriptImportHandler.CodeActionTitlePrefixes.find((prefix) =>
				codeAction.title.startsWith(prefix),
			);
		...
		const pathAsInTitle = codeAction.title
			.substring(importTitlePrefix.length)
			.trim();
	...
	private _getImportSource(
		...
		if (
			potentialNodeModules.some((importPath) =>
				workspaceInfo.nodeModules.has(importPath),
			)
		) {
			return ImportSource.external;
		}
...
class PythonImportHandler implements ILanguageImportHandler {
	...
	isImportCodeAction(codeAction: CodeActionData): boolean {
		return (
			codeAction.title.startsWith('Add "from') ||
			codeAction.title.startsWith('Add "import')
		);
	}
	...
	getImportDetails(
		...
		const fromImportMatch = codeAction.title.match(
			/Add "from\s+(.+?)\s+import\s(.+?)"/,
		);
		if (fromImportMatch) {
		...
		const importAsMatch = codeAction.title.match(
			/Add "import\s+(.+?)\s+as\s(.+?)"/,
		);
		if (importAsMatch) {
...
class JavaImportHandler implements ILanguageImportHandler {
	isImportDiagnostic(diagnostic: Diagnostic): boolean {
		return (
			String(diagnostic.data.code) === '16777218' ||
