...
export abstract class DiagnosticCompletionItem
	implements vscode.InlineCompletionItem
{
	static equals(
		a: DiagnosticCompletionItem,
		b: DiagnosticCompletionItem,
	): boolean {
		return (
			a.documentId.toString() === b.documentId.toString() &&
			Range.equalsRange(
				toInternalRange(a.range),
				toInternalRange(b.range),
			) &&
			a.insertText === b.insertText &&
			a.type === b.type &&
			a.isInlineEdit === b.isInlineEdit &&
			a.showInlineEditMenu === b.showInlineEditMenu &&
			displayLocationEquals(
				a.nextEditDisplayLocation,
				b.nextEditDisplayLocation,
			)
		);
	}
	...
	toOffsetEdit() {
		return StringReplacement.replace(
			this._toOffsetRange(this._edit.range),
			this._edit.text,
		);
	}
	...
	toLineEdit() {
		return LineEdit.fromTextEdit(
			this.toTextEdit(),
			this._workspaceDocument.value.get(),
		);
	}
...
export interface IDiagnosticCompletionProvider<
...
export class DiagnosticInlineEditRequestLogContext {
	getLogs(): string[] {
		...
		if (this._error) {
			...
			lines.push('```');
			lines.push(
				ErrorUtils.toString(ErrorUtils.fromUnknown(this._error)),
			);
			lines.push('```');
		}
...
export class Diagnostic {
	...
	constructor(public readonly data: DiagnosticData) {
...
function diagnosticDistanceToPosition(
	workspaceDocument: IObservableDocument,
	diagnostic: Diagnostic,
	position: Position,
) {
	function positionDistance(a: Position, b: Position) {
		return {
			lineDelta: Math.abs(a.lineNumber - b.lineNumber),
			characterDelta: Math.abs(a.column - b.column),
		};
	}

	const range = workspaceDocument.value
		.get()
		.getTransformer()
		.getRange(diagnostic.range);
...
export function isDiagnosticWithinDistance(
	...
	return (
		diagnosticDistanceToPosition(workspaceDocument, diagnostic, position)
			.lineDelta <= maxLineDistance
	);
}

export function sortDiagnosticsByDistance(
	workspaceDocument: IObservableDocument,
	...
	return diagnostics.sort((a, b) => {
		const aDistance = diagnosticDistanceToPosition(
			workspaceDocument,
			a,
			position,
		);
		const bDistance = diagnosticDistanceToPosition(
			workspaceDocument,
			b,
			position,
		);
...
export function distanceToClosestDiagnostic(
	workspaceDocument: IObservableDocument,
	diagnostics: Diagnostic[],
	position: Position,
): number | undefined {
	...
	const distances = diagnostics.map(
		(diagnostic) =>
			diagnosticDistanceToPosition(
				workspaceDocument,
				diagnostic,
				position,
			).lineDelta,
