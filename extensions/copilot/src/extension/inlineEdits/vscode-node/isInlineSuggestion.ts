...
export function toInlineSuggestion(
	...
	const nextLineInsertion = tryAdjustNextLineInsertion(
		cursorPos,
		doc,
		range,
		newText,
...
function tryAdjustNextLineInsertion(
	...
	return {
		...
		newText: lineBreak + trimmedNewText,
	};
}
...
function stripCommonLinePrefix(
	...
	while (
		commonLen < maxLen &&
		replacedText[commonLen] === newText[commonLen]
	) {
		commonLen++;
	}
	if (commonLen === 0) {
...
function validateSameLineGhostText(
	...
	const replacedText = doc.getText(range);
	const cursorOffsetInReplacedText =
		cursorPos.character - range.start.character;
