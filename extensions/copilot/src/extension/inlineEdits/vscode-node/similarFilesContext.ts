...
export class SimilarFilesContextService implements ISimilarFilesContextService {
	...
	constructor(
		...
		private readonly _copilotService: ICopilotInlineCompletionItemProviderService,
	) {}
	...
	async compute(
		...
		try {
			const result = await this._gatherSnippets(
				uri,
				languageId,
				source,
				cursorOffset,
	...
	async getSnippetsForPrompt(
		...
		try {
			...
			return snippets.map((s) => ({
				uri:
					(s.relativePath && relativePathToUri.get(s.relativePath)) ??
					uri,
				relativePath: s.relativePath,
				snippet: s.snippet,
	...
	> {
		...
		const promptOptions = completionsInstaService.invokeFunction(
			getPromptOptions,
			telemetryData,
			languageId,
		);
		...
		const tdm = completionsInstaService.invokeFunction((accessor) =>
			accessor.get(ICompletionsTextDocumentManagerService),
		);
		...
		const snippets = (
			await getSimilarSnippets(docInfo, neighborDocs, similarFilesOptions)
		)
