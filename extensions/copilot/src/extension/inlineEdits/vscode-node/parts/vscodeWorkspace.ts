...
} from '../../../../platform/notebook/common/alternativeNotebookTextDocument';
	...
	implements IDisposable
{
	...
	private readonly _filter =
		this._instaService.createInstance(DocumentFilter);
	...
	private readonly _markdownNotebookCells = new Lazy<ResourceSet>(() => {
		...
		workspace.notebookDocuments.forEach((doc) =>
			trackMarkdownCells(doc.getCells(), markdownCellUris),
		);
	...
	constructor(
		...
		const config =
			this._configurationService.getExperimentBasedConfigObservable(
				ConfigKey.TeamInternal.VerifyTextDocumentChanges,
				this._experimentationService,
			);
		this._store.add(
			autorun((reader) => {
		...
		this._store.add(
			workspace.onDidChangeTextDocument((e) => {
				const doc = this._getDocumentByDocumentAndUpdateShouldTrack(
					e.document.uri,
				);
				if (!doc) {
					return;
				...
				if (doc instanceof VSCodeObservableTextDocument) {
					const edit = editFromTextDocumentContentChangeEvents(
						e.contentChanges,
					);
					...
					transaction((tx) => {
						doc.languageId.set(
							LanguageId.create(e.document.languageId),
							tx,
						);
						doc.value.set(
							stringValueFromDoc(e.document),
							tx,
							editWithReason,
						);
					...
					doc.altNotebook.applyCellChanges(
						e.document,
						e.contentChanges,
					);
					const editWithReason = new StringEditWithReason(
					...
					transaction((tx) => {
						doc.value.set(
							stringValueFromDoc(doc.altNotebook),
							tx,
							editWithReason,
						);
		...
		if (this._useAlternativeNotebookFormat) {
			this._store.add(
				workspace.onDidOpenNotebookDocument((e) =>
					trackMarkdownCells(
						e.getCells(),
						this._markdownNotebookCells.value,
					),
				),
			);
		}
		...
		this._store.add(
			workspace.onDidChangeNotebookDocument((e) => {
				...
				if (
					...
					doc instanceof VSCodeObservableTextDocument
				) {
					return;
				}
				const edit = toAltNotebookChangeEdit(
					doc.altNotebook,
					e.contentChanges,
				);
				if (!edit) {
				...
				if (this._useAlternativeNotebookFormat) {
					e.contentChanges
						.map((c) => c.removedCells)
						.flat()
						.forEach((c) => {
							this._markdownNotebookCells.value.delete(
								c.document.uri,
							);
						});
					trackMarkdownCells(
						e.contentChanges.map((c) => c.addedCells).flat(),
						this._markdownNotebookCells.value,
					);
				}
			}),
			...
			window.onDidChangeTextEditorSelection((e) => {
				...
				const selections =
					doc instanceof VSCodeObservableTextDocument
						? coalesce(
								e.selections.map((s) =>
									doc.toOffsetRange(e.textEditor.document, s),
								),
							)
						: this.getNotebookSelections(
								doc.notebook,
								e.textEditor,
							);
				doc.selection.set(selections, undefined);
				doc.primarySelectionLine.set(
		...
		this._store.add(
			window.onDidChangeTextEditorVisibleRanges((e) => {
				...
				const visibleRanges =
					doc instanceof VSCodeObservableTextDocument
						? coalesce(
								e.visibleRanges.map((r) =>
									doc.toOffsetRange(e.textEditor.document, r),
								),
							)
		...
		this._store.add(
			languages.onDidChangeDiagnostics((e) => {
				e.uris.forEach((uri) => {
					const document =
						this._getDocumentByDocumentAndUpdateShouldTrack(uri);
					if (!document) {
						return;
					}
					const diagnostics =
						document instanceof VSCodeObservableTextDocument
							? this._createTextDocumentDiagnosticData(document)
							: this._createNotebookDiagnosticData(
									document.altNotebook,
								);
	...
	private readonly _obsDocsByDocId = derived(this, (reader) => {
		...
		const obsDocs = textDocs
			.map((d) => d.obsDoc.read(reader))
			.filter(isDefined);
		const map: Map<
			DocumentId,
			VSCodeObservableTextDocument | VSCodeObservableNotebookDocument
		> = new Map(obsDocs.map((d) => [d.id, d]));
		const notebookDocs = this._notebookDocsWithShouldTrackFlag.read(reader);
		const obsNotebookDocs = notebookDocs
			.map((d) => d.obsDoc.read(reader))
			.filter(isDefined);
	...
	private readonly _vscodeTextDocuments: IObservable<
		readonly TextDocument[]
	> = getObservableTextDocumentList(
		this._useAlternativeNotebookFormat,
		this._markdownNotebookCells.value,
	);
	private readonly _textDocsWithShouldTrackFlag = mapObservableArrayCached(
		this,
		...
		(doc, store) => {
			...
			const obsDoc = derived(this, (reader) => {
				...
				const document = new VSCodeObservableTextDocument(
					documentId,
				...
				const selections = coalesce(
					(openedTextEditor?.selections || []).map((s) =>
						document.toOffsetRange(doc, s),
					),
				);
				const visibleRanges = coalesce(
					(openedTextEditor?.visibleRanges || []).map((r) =>
						document.toOffsetRange(doc, r),
					),
				);
				transaction((tx) => {
					document.selection.set(selections, tx);
					document.primarySelectionLine.set(
						openedTextEditor?.selections[0]?.start.line,
						tx,
					);
					document.visibleRanges.set(visibleRanges, tx);
					document.diagnostics.set(
						this._createTextDocumentDiagnosticData(document),
						tx,
					);
				});
	...
	private readonly _vscodeNotebookDocuments: IObservable<
		readonly NotebookDocument[]
	> = this.getNotebookDocuments();
	private readonly _altNotebookDocs = new WeakMap<
		NotebookDocument,
		IAlternativeNotebookDocument
	>();
	private getAltNotebookDocument(
	...
	private readonly _notebookDocsWithShouldTrackFlag =
		mapObservableArrayCached(
			this,
			this._vscodeNotebookDocuments,
			(doc, store) => {
				const shouldTrack = observableValue<boolean>(this, false);
				const updateShouldTrack = () => {
					// @ulugbekna: not sure if invoking `isCopilotIgnored` on every textDocument-edit event is a good idea
				...
				const obsDoc = derived(this, (reader) => {
					...
					const diagnostics =
						this._createNotebookDiagnosticData(altNotebook);
					const language = getLanguage(
						getDefaultLanguage(altNotebook.notebook),
					).languageId;
					const document = new VSCodeObservableNotebookDocument(
						documentId,
						stringValueFromDoc(altNotebook),
						doc.version,
						selections ?? [],
						visibleRanges ?? [],
						LanguageId.create(language),
						diagnostics,
						doc,
						altNotebook,
					);
	...
	private getNotebookSelections(
		doc: NotebookDocument,
		activeCellEditor?: TextEditor,
	) {
		const altNotebook = this.getAltNotebookDocument(doc);
		const visibleTextEditors = new Map(
			window.visibleTextEditors.map((e) => [e.document, e]),
		);
		...
		const selectedCell = activeCellEditor
			? altNotebook.getCell(activeCellEditor.document)
			: selectedCellRange && selectedCellRange.start < doc.cellCount
				? doc.cellAt(selectedCellRange.start)
				: undefined;
		const selectedCellEditor = selectedCell
			? visibleTextEditors.get(selectedCell.document)
			: undefined;
		...
		return selectedCellEditor && selectedCell
			? altNotebook.toAltOffsetRange(
					selectedCell,
					selectedCellEditor.selections,
				)
			: [];
	}
	...
	private getNotebookVisibleRanges(doc: NotebookDocument) {
		...
		const cellTextEditors = coalesce(
			doc
				.getCells()
				.map((cell) =>
					visibleTextEditors.has(cell.document)
						? ([
								cell,
								visibleTextEditors.get(cell.document)!,
							] as const)
						: undefined,
				),
		);
		return cellTextEditors.flatMap((e) =>
			altNotebook.toAltOffsetRange(e[0], e[1].visibleRanges),
		);
	}

	private _getDocumentByDocumentAndUpdateShouldTrack(
		uri: URI,
	):
	...
	private _getInternalDocument(uri: Uri, reader?: IReader) {
		const document = this._obsDocsWithUpdateIgnored
			.read(reader)
			.get(uri.toString());
		return document;
	}

	private _createTextDocumentDiagnosticData(
		document: VSCodeObservableTextDocument,
	) {
		return languages
			.getDiagnostics(document.textDocument.uri)
			.map((d) => this._createDiagnosticData(d, document))
			.filter(isDefined);
	}

	private _createDiagnosticData(
		diagnostic: Diagnostic,
		doc: VSCodeObservableTextDocument,
	): DiagnosticData | undefined {
		return toDiagnosticData(diagnostic, doc.textDocument.uri, (range) =>
			doc.toOffsetRange(doc.textDocument, range),
		);
	}

	private _createNotebookDiagnosticData(
		altNotebook: IAlternativeNotebookDocument,
	) {
		return coalesce(
			altNotebook.notebook
				.getCells()
				.flatMap((c) =>
					languages
						.getDiagnostics(c.document.uri)
						.map((d) =>
							this._createNotebookCellDiagnosticData(
								d,
								altNotebook,
								c.document,
							),
	...
	private _createNotebookCellDiagnosticData(
		diagnostic: Diagnostic,
		altNotebook: IAlternativeNotebookDocument,
		doc: TextDocument,
	): DiagnosticData | undefined {
		...
		return toDiagnosticData(
			diagnostic,
			altNotebook.notebook.uri,
			(range) => {
				const offsetRanges = altNotebook.toAltOffsetRange(cell, [
					range,
				]);
	...
	private readonly _obsDocsWithUpdateIgnored = derived(this, (reader) => {
		...
		notebookDocs.forEach((d) => {
			...
			d.doc
				...
				.forEach((cell) => map.set(cell.document.uri.toString(), d));
		});
		return map;
	});
...
class VSCodeObservableTextDocument
	extends AbstractVSCodeObservableDocument
	implements IVSCodeObservableDocument
{
	constructor(
		id: DocumentId,
		value: StringText,
	...
	fromOffsetRange(range: OffsetRange): [TextDocument, Range][];
	fromOffsetRange(
		arg1: TextDocument | OffsetRange,
		range?: OffsetRange,
	): Range | undefined | [TextDocument, Range][] {
		...
		if (textDocument !== this.textDocument) {
			throw new Error(
				'TextDocument does not match the one of this observable document.',
			);
		}
		if (!offsetRange) {
	...
	toOffsetRange(
		...
		return new OffsetRange(
			textDocument.offsetAt(range.start),
			textDocument.offsetAt(range.end),
		);
	}
	...
	async getCodeActions(
		...
		return actions?.map((action) =>
			toCodeActionData(action, this, (range) =>
				this.toOffsetRange(this.textDocument, range),
			),
		);
	}
}

class VSCodeObservableNotebookDocument
	extends AbstractVSCodeObservableDocument
	implements IVSCodeObservableDocument
{
	constructor(
	...
	fromOffsetRange(
		...
		if (arg1 instanceof OffsetRange) {
			return this.altNotebook
				.fromAltOffsetRange(arg1)
				.map((r) => [r[0].document, r[1]]);
		} else if (range !== undefined) {
	...
	async getCodeActions(
		...
		return Promise.all(
			cellRanges.map(async ([cellTextDocument, range]) => {
				...
				return actions?.map((action) =>
					toCodeActionData(action, this, (range) => {
						const offsetRanges = this.altNotebook.toAltOffsetRange(
							cell,
							[range],
						);
						return offsetRanges.length
							? offsetRanges[0]
							: undefined;
					}),
				);
...
function getObservableTextDocumentList(
	excludeNotebookCells: boolean,
	markdownCellUris: ResourceSet,
	...
	return observableFromEvent(
		...
		() =>
			excludeNotebookCells
				? workspace.textDocuments.filter(
						(doc) =>
							doc.uri.scheme !== Schemas.vscodeNotebookCell ||
							markdownCellUris.has(doc.uri),
					)
...
function toDiagnosticData(
	diagnostic: Diagnostic,
	uri: URI,
	translateRange: (range: Range) => OffsetRange | undefined,
) {
	if (
		!diagnostic.source ||
...
function toCodeActionData(
	...
	const diagnostics = coalesce(
		(codeAction.diagnostics || []).map((d) =>
			toDiagnosticData(d, uri, translateRange),
		),
	).concat(getCommandDiagnostics(codeAction, uri, translateRange));

	// remove no-op edits
	let documentEdits = getDocumentEdits(codeAction, workspaceDocument);
	if (documentEdits) {
		const documentContent = workspaceDocument.value.get();
		documentEdits = documentEdits.filter(
			(edit) => documentContent.getValueOfRange(edit.range) !== edit.text,
		);
	}
...
function getCommandDiagnostics(
	codeAction: CodeAction,
	uri: URI,
	translateRange: (range: Range) => OffsetRange | undefined,
): DiagnosticData[] {
...
function getDocumentEdits(
	...
	if (workspaceDocument instanceof VSCodeObservableTextDocument) {
		return edit
			.get(workspaceDocument.id.toUri())
			.map((e) => toInternalTextEdit(e.range, e.newText));
	} else if (workspaceDocument instanceof VSCodeObservableNotebookDocument) {
		const edits = coalesce(
			workspaceDocument.notebook.getCells().flatMap((cell) =>
				edit.get(cell.document.uri).map((e) => {
					const range = workspaceDocument.toRange(
						cell.document,
						e.range,
					);
					return range
						? toInternalTextEdit(range, e.newText)
						: undefined;
				}),
			),
		);
...
async function getQuickFixCodeActions(
	uri: Uri,
	range: Range,
	itemResolveCount: number,
): Promise<CodeAction[]> {
