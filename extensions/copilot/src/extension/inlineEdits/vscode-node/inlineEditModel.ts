...
export class InlineEditModel extends Disposable {
	...
	public readonly inlineEditsInlineCompletionsEnabled =
		this._configurationService.getConfigObservable(
			ConfigKey.TeamInternal.InlineEditsInlineCompletionsEnabled,
		);
	...
	constructor(
		...
		public readonly diagnosticsBasedProvider:
		...
		this._predictor = createNextEditProvider(
			this._predictorId,
		...
		this.nextEditProvider = this._instantiationService.createInstance(
			NextEditProvider,
			this.workspace,
			this._predictor,
		...
		this._triggerer = this._register(
