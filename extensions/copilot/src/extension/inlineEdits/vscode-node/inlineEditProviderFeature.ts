...
const useEnhancedNotebookNESContextKey =
	'github.copilot.chat.enableEnhancedNotebookNES';

export class InlineEditProviderFeatureContribution
	extends Disposable
	implements IExtensionContribution
{
	constructor(
		...
		const inlineEditProviderFeature =
...
export class InlineEditProviderFeature {
	...
	private readonly _yieldToCopilot =
		this._configurationService.getExperimentBasedConfigObservable(
			ConfigKey.TeamInternal.InlineEditsYieldToCopilot,
			this._expService,
		);
	...
	public setContext(): IDisposable {
		...
		const enableEnhancedNotebookNES =
		...
		return autorun((reader) => {
			...
			void commands.executeCommand(
				'setContext',
				'github.copilot.inlineEditsEnabled',
				enabled,
			);
		});
	}
	...
	public registerProvider(): IDisposable {
		...
		return autorun((reader) => {
			...
			const logger = reader.store.add(
				this._instantiationService.createInstance(InlineEditLogger),
			);
			...
			const git = reader.store.add(
				this._instantiationService.createInstance(ObservableGit),
			);
			...
			let diagnosticsProvider: DiagnosticsNextEditProvider | undefined =
				undefined;
			...
			const isInlineEditLogFileEnabled =
				this.isInlineEditsLogFileEnabledObservable.read(reader);
			...
			const expectedEditCaptureController = reader.store.add(
				this._instantiationService.createInstance(
			...
			reader.store.add(
				languages.registerInlineCompletionItemProvider('*', provider, {
					...
					yieldTo: this._yieldToCopilot.read(reader)
						? ['github.copilot']
						: undefined,
			...
			reader.store.add(
				commands.registerCommand(learnMoreCommandId, () => {
					this._envService.openExternal(URI.parse(learnMoreLink));
				}),
			);

			reader.store.add(
				commands.registerCommand(clearCacheCommandId, () => {
			...
			reader.store.add(
				commands.registerCommand(
					...
					() => {
						...
						const doc =
							model.workspace.getDocumentByTextDocument(document);
						...
						void commands.executeCommand(reportFeedbackCommandId, {
							logContext,
						});
					},
				),
			...
			reader.store.add(
				commands.registerCommand(captureExpectedStartCommandId, () => {
			...
			reader.store.add(
				commands.registerCommand(captureExpectedAbortCommandId, () => {
					void expectedEditCaptureController.abortCapture();
				}),
			);

			reader.store.add(
				commands.registerCommand(captureExpectedSubmitCommandId, () => {
...
export const reportNotebookNESIssueCommandId =
	'github.copilot.debug.inlineEdit.reportNotebookNESIssue';
export const captureExpectedStartCommandId =
	'github.copilot.nes.captureExpected.start';
export const captureExpectedConfirmCommandId =
	'github.copilot.nes.captureExpected.confirm';
export const captureExpectedAbortCommandId =
	'github.copilot.nes.captureExpected.abort';
export const captureExpectedSubmitCommandId =
