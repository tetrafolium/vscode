...
import {
	InlineCompletionModelInfo,
	InlineCompletionProviderOption,
} from 'vscode';
...
} from '../../../platform/inlineEdits/common/dataTypes/jointCompletionsProviderOptions';
import { InlineEditRequestLogContext } from '../../../platform/inlineEdits/common/inlineEditLogContext';
import { ObservableGit } from '../../../platform/inlineEdits/common/observableGit';
import {
	checkIfCursorAtEndOfLine,
	shortenOpportunityId,
} from '../../../platform/inlineEdits/common/utils/utils';
...
} from '../../completions-core/vscode-node/extension/src/ghostText/ghostTextProvider';
...
import {
	InlineEditDebugComponent,
	reportFeedbackCommandId,
} from './components/inlineEditDebugComponent';
...
import {
	captureExpectedAbortCommandId,
	captureExpectedConfirmCommandId,
	captureExpectedStartCommandId,
	captureExpectedSubmitCommandId,
	clearCacheCommandId,
	InlineEditProviderFeature,
	InlineEditProviderFeatureContribution,
	learnMoreCommandId,
	learnMoreLink,
	reportNotebookNESIssueCommandId,
} from './inlineEditProviderFeature';
...
export class JointCompletionsProviderContribution
	extends Disposable
	implements IExtensionContribution
{
	...
	private readonly _inlineEditsProviderId = makeSettable(
	...
	private readonly _excludedProviders = this._configurationService
		.getExperimentBasedConfigObservable(
			ConfigKey.TeamInternal.InlineEditsExcludedProviders,
			this._expService,
		)
		.map((v) =>
			v
	...
	private readonly _internalActionsEnabled = derived(this, (reader) => {
		return (
			!!this._copilotToken.read(reader)?.isInternal &&
			!this._hideInternalInterface.read(reader)
		);
	});

	public readonly isInlineEditsLogFileEnabledObservable =
		this._configurationService.getConfigObservable(
	...
	constructor(
		...
		const useJointCompletionsProviderObs =
			_configurationService.getExperimentBasedConfigObservable(
				ConfigKey.TeamInternal
					.InlineEditsJointCompletionsProviderEnabled,
				_expService,
			);
		...
		this._register(
			autorun((reader) => {
				// FX
				const useJointCompletionsProvider =
					useJointCompletionsProviderObs.read(reader);
				...
				const inlineEditFeature = _instantiationService.createInstance(
					InlineEditProviderFeature,
				);
				...
				reader.store.add(
					autorun((reader) => {
						const unificationStateValue =
							unificationState.read(reader);

						const excludes = this._excludedProviders
							.read(reader)
							.slice();
						...
						if (
							!excludes.includes(
								JointCompletionsProviderContribution.NES_GROUP_ID,
							) &&
							this.inlineEditsEnabled.read(reader)
						) {
							const logger = reader.store.add(
								this._instantiationService.createInstance(
									InlineEditLogger,
								),
							);

							const statelessProviderId =
								this._inlineEditsProviderId.read(reader);

							const workspace = this._workspace.read(reader);
							const git = reader.store.add(
								this._instantiationService.createInstance(
									ObservableGit,
								),
							);
							...
							if (this._enableDiagnosticsProvider.read(reader)) {
								diagnosticsProvider = reader.store.add(
									this._instantiationService.createInstance(
										DiagnosticsNextEditProvider,
										workspace,
										git,
							...
							const recordingDirPath = join(
								this._vscodeExtensionContext.globalStorageUri
									.fsPath,
								'logContextRecordings',
							);

							const isInlineEditLogFileEnabled =
								this.isInlineEditsLogFileEnabledObservable.read(
									reader,
								);

							let logContextRecorder:
								| LogContextRecorder
								| undefined;
							if (isInlineEditLogFileEnabled) {
								logContextRecorder = reader.store.add(
									this._instantiationService.createInstance(
										LogContextRecorder,
										recordingDirPath,
										logger,
								...
								void LogContextRecorder.cleanupOldRecordings(
									recordingDirPath,
								);
							}

							const inlineEditDebugComponent = reader.store.add(
								new InlineEditDebugComponent(
									this._internalActionsEnabled,
									this.inlineEditsEnabled,
									model.debugRecorder,
									this._inlineEditsProviderId,
								),
							);

							const telemetrySender = reader.store.add(
								this._instantiationService.createInstance(
									TelemetrySender,
									workspace,
								),
							...
							const expectedEditCaptureController =
								reader.store.add(
									this._instantiationService.createInstance(
							...
							reader.store.add(
								vscode.commands.registerCommand(
									learnMoreCommandId,
									() => {
										this._envService.openExternal(
											URI.parse(learnMoreLink),
										);
									},
								),
							);
							...
							reader.store.add(
								vscode.commands.registerCommand(
									...
									() => {
										...
										const doc =
											model.workspace.getDocumentByTextDocument(
												document,
											);
										const selection =
											activeNotebook.selection;
										if (!selection || !doc) {
											return;
										}
										...
										logContext.recordingBookmark =
											model.debugRecorder.createBookmark();
							...
							reader.store.add(
								vscode.commands.registerCommand(
									captureExpectedStartCommandId,
									() => {
										void expectedEditCaptureController.startCapture(
											'manual',
										);
									},
								),
						...
						let completionsProvider:
							| CopilotInlineCompletionItemProvider
							| undefined;
						{
							const configEnabled = this._configurationService
								.getExperimentBasedConfigObservable<boolean>(
									ConfigKey.TeamInternal
										.InlineEditsEnableGhCompletionsProvider,
									this._expService,
								)
								.read(reader);
							const extensionUnification =
								unificationStateValue?.extensionUnification ??
								false;

							// respect excludes if NES is enabled
							const isExcluded =
								excludes.includes(
									JointCompletionsProviderContribution.COMPLETIONS_GROUP_ID,
								) && this.inlineEditsEnabled.read(reader);

							// @ulugbekna: note that we don't want it if modelUnification is on
							const modelUnification =
								unificationStateValue?.modelUnification ??
								false;
							if (
								(!modelUnification ||
									unificationStateValue?.codeUnification ||
									extensionUnification ||
									configEnabled ||
									this._copilotToken.read(reader)
										?.isNoAuthUser) &&
								!isExcluded
							) {
								completionsProvider =
									this._copilotInlineCompletionItemProviderService.getOrCreateProvider() as CopilotInlineCompletionItemProvider;
							}

							void vscode.commands.executeCommand(
								'setContext',
								'github.copilot.extensionUnification.activated',
								extensionUnification,
							);

							if (extensionUnification && completionsProvider) {
								const completionsInstaService =
									this._copilotInlineCompletionItemProviderService.getOrCreateInstantiationService();
								reader.store.add(
									completionsInstaService.invokeFunction(
										registerUnificationCommands,
									),
								);
							}
						}

						const singularProvider = reader.store.add(
							this._instantiationService.createInstance(
								JointCompletionsProvider,
								completionsProvider,
								inlineEditProvider,
							),
						);
						...
						reader.store.add(
							vscode.languages.registerInlineCompletionItemProvider(
								...
								{
									displayName:
										inlineEditProvider?.displayName,
...
function toCompletionsList(
	list: GhostTextCompletionList,
): SingularCompletionList {
	return {
		...list,
		items: list.items.map((item) => ({ ...item, source: 'completions' })),
		source: 'completions',
	};
}
...
function toInlineEditsList(list: NesCompletionList): SingularCompletionList {
	return {
		...list,
		items: list.items.map((item) => ({ ...item, source: 'inlineEdits' })),
		source: 'inlineEdits',
	};
}
...
class JointCompletionsProvider
	extends Disposable
	implements vscode.InlineCompletionItemProvider
{
	private readonly _onDidChangeEmitter:
		| vscode.EventEmitter<NesChangeHint>
		| undefined;
	...
	//#region Model picker
	public readonly onDidChangeModelInfo =
		this._inlineEditProvider?.onDidChangeModelInfo;
	public readonly setCurrentModelId =
		this._inlineEditProvider?.setCurrentModelId?.bind(
			this._inlineEditProvider,
		);
	...
	public readonly onDidChangeProviderOptions =
		this._inlineEditProvider?.onDidChangeProviderOptions;
	public readonly setProviderOptionValue =
		this._inlineEditProvider?.setProviderOptionValue?.bind(
			this._inlineEditProvider,
		);
	...
	constructor(
		private readonly _completionsProvider:
			| CopilotInlineCompletionItemProvider
			| undefined,
		private readonly _inlineEditProvider:
			| InlineCompletionProviderImpl
			| undefined,
		@IConfigurationService
		...
		if (this._inlineEditProvider?.onDidChange) {
			this._onDidChangeEmitter = this._register(
				new vscode.EventEmitter<NesChangeHint>(),
			);
			this.onDidChange = this._onDidChangeEmitter.event;

			this._register(
				this._inlineEditProvider.onDidChange((changeHint) => {
					const strategy =
						this._configService.getExperimentBasedConfig(
							ConfigKey.TeamInternal
								.InlineEditsJointCompletionsProviderTriggerChangeStrategy,
							this._expService,
						);
					switch (strategy) {
						...
						case JointCompletionsProviderTriggerChangeStrategy.NoTriggerOnRequestInFlight:
							if (this._isRequestInFlight) {
								this._logger.trace(
									'Skipping onDidChange event firing because request is in flight',
								);
								return;
							}
							break;
						case JointCompletionsProviderTriggerChangeStrategy.NoTriggerOnCompletionsRequestInFlight:
							if (this._isCompletionsRequestInFlight) {
								this._logger.trace(
									'Skipping onDidChange event firing because completions request is in flight',
								);
								return;
							}
							break;
	...
	public async provideInlineCompletionItems(
		...
		const logger = this._logger.createSubLogger([
			shortenOpportunityId(context.requestUuid),
			'provideInlineCompletionItems',
		]);

		const strategy = this._configService.getExperimentBasedConfig(
			ConfigKey.TeamInternal.InlineEditsJointCompletionsProviderStrategy,
			this._expService,
		);

		switch (strategy) {
			case JointCompletionsProviderStrategy.Regular:
				return this.provideInlineCompletionItemsRegular(
					document,
					position,
					context,
					token,
			...
			case JointCompletionsProviderStrategy.CursorEndOfLine:
				return this.provideInlineCompletionItemsCursorEndOfLine(
					document,
					position,
					context,
					token,
	...
	private async provideInlineCompletionItemsCursorEndOfLine(
		document: vscode.TextDocument,
		position: vscode.Position,
		context: vscode.InlineCompletionContext,
		token: vscode.CancellationToken,
		...
		try {
			if (
				this._completionsProvider === undefined &&
				this._inlineEditProvider === undefined
			) {
				logger.trace(
					'Return: neither completions nor NES provider available',
				);
				return undefined;
			} else if (
				this._completionsProvider === undefined &&
				...
				const r = await this._invokeNESProvider(
					logger,
					document,
					position,
					false,
				...
				logger.trace(
					'only completions provider is available, invoking it',
				);
				const r = await this._invokeCompletionsProvider(
					logger,
					document,
					position,
					context,
				...
				const isCursorAtEndOfLine = checkIfCursorAtEndOfLine(
					cursorLine,
					position.character,
				);
				...
				if (isCursorAtEndOfLine) {
					logger.trace(
						'cursor is at end of line, invoking ghost-text provider only',
					);
					const r = await this._invokeCompletionsProvider(
						logger,
						document,
			...
			if (!token.isCancellationRequested) {
				this._logger.trace(
					'request in flight: false -- due to provider finishing',
				);
				this._requestsInFlight.delete(token);
			}
			disp.dispose();
	...
	private async provideInlineCompletionItemsRegular(
		document: vscode.TextDocument,
		position: vscode.Position,
		context: vscode.InlineCompletionContext,
		token: vscode.CancellationToken,
		...
		const disp1 = token.onCancellationRequested(() => {
			logger.trace(
				`invocation #${invocationId}: request in flight: false -- due to cancellation`,
			);
			this._requestsInFlight.delete(token);
		});
		logger.trace(
			`invocation #${invocationId} started; request in flight: true`,
		);
		...
		try {
			...
			if (
				...
				this.lastNesSuggestion.docUri.toString() !==
					document.uri.toString()
			) {
				logger.trace(
					'last NES suggestion is not for the current document, ignoring',
				);
			...
			if (
				firstItem.uri &&
				firstItem.uri.toString() !== document.uri.toString()
			) {
				logger.trace(
					`The NES suggestion is for a different document (${firstItem.uri.toString()} vs ${document.uri.toString()}), not saving as last NES suggestion`,
				);
				return list;
			}

			const applied = applyTextEdit(
				docSnapshot,
				firstItem.range,
			...
			if (!token.isCancellationRequested) {
				logger.trace(
					`invocation #${invocationId}: request in flight: false -- due to provider finishing`,
				);
				this._requestsInFlight.delete(token);
			}
			disp1.dispose();
			...
			if (
				invocationId ===
				this.provideInlineCompletionItemsInvocationCount
			) {
				this.lastNesSuggestion = saveLastNesSuggestion;
				if (this.lastNesSuggestion) {
					logger.trace(
						`Set the last NES suggestion for document ${this.lastNesSuggestion.docUri.toString()}`,
					);
				} else {
				...
				logger.trace(
					`Not setting the last NES suggestion because a newer invocation exists`,
				);
			}
	...
	private async _provideInlineCompletionItemsRegular(
		{
			document,
			docSnapshot,
		}: { document: vscode.TextDocument; docSnapshot: StringText },
		position: vscode.Position,
		...
		tokens: {
			...
			completionsCts: CancellationTokenSource;
			nesCts: CancellationTokenSource;
		},
	): Promise<SingularCompletionList | undefined> {
		const sw = new StopWatch();
		...
		const isTriggeredDueToSelectionChange =
			context &&
			(context as NESInlineCompletionContext).changeHint !== undefined;

		if (!lastNesSuggestion || !lastNesSuggestion.completionItem.wasShown) {
			// prefer completions unless there are none
			logger.trace(
				`defaulting to yielding to completions; last NES suggestion is ${lastNesSuggestion ? 'not shown' : 'not available'}`,
			);
			const completionsP = isTriggeredDueToSelectionChange
				? undefined
				: this._invokeCompletionsProvider(
						logger,
						document,
						position,
						context,
			...
			const nesP = this._invokeNESProvider(
				logger,
				document,
				position,
				true,
			...
			return this._returnCompletionsOrOtherwiseNES(
				completionsP,
				nesP,
				docSnapshot,
				sw,
		...
		logger.trace(
			`last NES suggestion is for the current document, checking if it agrees with the current suggestion`,
		);

		const enforceCacheDelay =
			lastNesSuggestion.docVersionId !== document.version;
		const nesP = this._invokeNESProvider(
			logger,
			document,
			position,
		...
		{
			logger.trace(
				`giving the NES provider ${NES_CACHE_WAIT_MS}ms to return what it has in its cache`,
			);
			const fastNesResult = await raceCancellation(
				raceTimeout(nesP, NES_CACHE_WAIT_MS),
				tokens.coreToken,
			);
			...
			if (
				...
				this.doesNesSuggestionAgree(
					...
					(fastNesResult.items as NesCompletionItem[]).at(0),
				)
			) {
				logger.trace(
					'last NES suggestion agrees with the current suggestion, using NES',
				);
				const list: SingularCompletionList =
					toInlineEditsList(fastNesResult);
				logger.trace(
					`Return: returning NES result in ${sw.elapsed()}ms`,
				);
				return list;
			}
			...
			if (tokens.coreToken.isCancellationRequested) {
				...
				void setEndOfLifeReason(this._completionsProvider, undefined, {
					kind: vscode.InlineCompletionsDisposeReasonKind
						.TokenCancellation,
				});
				void setEndOfLifeReason(this._inlineEditProvider, nesP, {
					kind: vscode.InlineCompletionsDisposeReasonKind
						.TokenCancellation,
				});
		...
		logger.trace(
			`the NES provider did not return in ${NES_CACHE_WAIT_MS}ms so we are triggering the completions provider too`,
		);
		const completionsP = isTriggeredDueToSelectionChange
			? undefined
			: this._invokeCompletionsProvider(
		...
		if (suggestionsList === undefined) {
			logger.trace(`suggestions request was cancelled`);
			void setEndOfLifeReason(this._completionsProvider, completionsP, {
				kind: vscode.InlineCompletionsDisposeReasonKind
					.TokenCancellation,
			});
			void setEndOfLifeReason(this._inlineEditProvider, nesP, {
		...
		if (
			...
			this.doesNesSuggestionAgree(
				docSnapshot,
				lastNesSuggestion.docWithNesEditApplied,
				(suggestionsList.res.items as NesCompletionItem[]).at(0),
			)
		) {
			logger.trace(
				'last NES suggestion agrees with the current suggestion, using NES',
			);
			return this._returnNES(
				suggestionsList.res,
				{ kind: vscode.InlineCompletionsDisposeReasonKind.NotTaken },
				completionsP,
				sw,
				logger,
				tokens,
		...
		logger.trace(
			'falling back to the default because completions came first or NES disagreed',
		);
		return this._returnCompletionsOrOtherwiseNES(
			completionsP,
			nesP,
	...
	private _invokeNESProvider(
		...
		const changeHint =
			context.changeHint === undefined ||
			NesChangeHint.is(context.changeHint)
				? (context.changeHint as NesChangeHint | undefined)
				: undefined;
		const nesContext: NESInlineCompletionContext = {
			...context,
			enforceCacheDelay,
			changeHint,
		};
		...
		if (this._inlineEditProvider) {
			...
			nesP = this._inlineEditProvider.provideInlineCompletionItems(
				document,
				position,
				nesContext,
				ct,
			...
			nesP.then((nesR) => {
				logger.trace(
					`got NES response in ${sw.elapsed()}ms -- ${nesR === undefined ? 'undefined' : `with ${nesR.items.length} items`}`,
				);
			}).catch((e) => {
				logger.trace(
					`NES provider errored after ${sw.elapsed()}ms -- ${ErrorUtils.toString(ErrorUtils.fromUnknown(e))}`,
				);
			});
		} else {
			logger.trace(`- no NES provider`);
	...
	private _invokeCompletionsProvider(
		...
		if (this._completionsProvider) {
			...
			const disp = ct.onCancellationRequested(() =>
			...
			try {
				// in case the provider throws synchronously
				logger.trace(
					`- requesting completions provideInlineCompletionItems`,
				);
				completionsP =
					this._completionsProvider.provideInlineCompletionItems(
						document,
						position,
						context,
						ct,
				...
				completionsP
					.then((completionsR) => {
						logger.trace(
							`got completions response in ${sw.elapsed()}ms -- ${completionsR === undefined ? 'undefined' : `with ${completionsR.items.length} items`}`,
						);
					})
					.catch((e) => {
						logger.trace(
							`completions provider errored after ${sw.elapsed()}ms -- ${ErrorUtils.toString(ErrorUtils.fromUnknown(e))}`,
						);
					})
					.finally(() => {
						cleanup();
				...
				cleanup();
				logger.trace(
					`completions provider threw synchronously after ${sw.elapsed()}ms -- ${ErrorUtils.toString(ErrorUtils.fromUnknown(e))}`,
				);
				throw e;
			}
		} else {
	...
	private async _returnCompletionsOrOtherwiseNES(
		...
		if (completionsR && completionsR.items.length > 0) {
			const filteredCompletionR =
				JointCompletionsProvider.retainOnlyMeaningfulEdits(
					docSnapshot,
					completionsR,
				);
			if (filteredCompletionR.items.length === 0) {
				logger.trace(
					`all completions edits are no-op, ignoring completions response`,
				);
			} else {
				logger.trace(
					`using completions response, cancelling NES provider`,
				);
				return this._returnCompletions(
					filteredCompletionR,
					{
						kind: vscode.InlineCompletionsDisposeReasonKind
							.LostRace,
	...
	private _returnCompletions(
		...
		if (completionsR === undefined) {
			logger.trace(
				`Return: no completions to return in ${sw.elapsed()}ms`,
			);
			return undefined;
		}
	...
	private _returnNES(
		...
		void setEndOfLifeReason(
			this._completionsProvider,
			completionsP,
			completionsDisposeReason,
		);
	...
	private doesNesSuggestionAgree(
		doc: StringText,
		docWithNesEditApplied: StringText,
		nesEdit: NesCompletionItem | undefined,
	): boolean {
		if (
			nesEdit === undefined ||
			nesEdit.range === undefined ||
			typeof nesEdit.insertText !== 'string'
		) {
			return false;
		}
	...
	private static retainOnlyMeaningfulEdits<
		T extends vscode.InlineCompletionList,
	>(docSnapshot: StringText, list: T): T {
		// meaningful = not noop
		function isMeaningfulEdit(item: T['items'][number]): boolean {
			if (
				item.range === undefined || // must be a completion with a side-effect, eg a command invocation or something
				typeof item.insertText !== 'string' // shouldn't happen
			) {
				return true;
			}
			const originalSnippet = docSnapshot.getValueOfRange(
				new Range(
	...
	public handleDidShowCompletionItem?(
		completionItem: SingularCompletionItem,
		updatedInsertText: string,
	): void {
		switch (completionItem.source) {
			case 'completions':
				this._completionsProvider?.handleDidShowCompletionItem?.(
					completionItem,
				);
				break;
			case 'inlineEdits':
				this._inlineEditProvider?.handleDidShowCompletionItem?.(
					completionItem,
					updatedInsertText,
				);
				break;
	...
	public handleDidPartiallyAcceptCompletionItem?(
		completionItem: SingularCompletionItem,
		acceptedLength: number & vscode.PartialAcceptInfo,
	): void {
		switch (completionItem.source) {
			case 'completions':
				this._completionsProvider?.handleDidPartiallyAcceptCompletionItem?.(
					completionItem,
					acceptedLength,
				);
				break;
			case 'inlineEdits':
				softAssert(
					this._inlineEditProvider
						?.handleDidPartiallyAcceptCompletionItem === undefined,
					'InlineEditProvider does not implement handleDidPartiallyAcceptCompletionItem',
				);
				break;
			default:
				assertNever(completionItem);
	...
	public handleEndOfLifetime?(
		...
		switch (completionItem.source) {
			case 'completions':
				this._completionsProvider?.handleEndOfLifetime?.(
					completionItem,
					reason,
				);
				break;
			case 'inlineEdits':
				this._inlineEditProvider?.handleEndOfLifetime?.(
					completionItem,
					reason,
				);
				break;
	...
	public handleListEndOfLifetime?(
		list: SingularCompletionList,
		reason: vscode.InlineCompletionsDisposeReason,
	): void {
		switch (list.source) {
			case 'completions':
				this._completionsProvider?.handleListEndOfLifetime?.(
					list,
					reason,
				);
				break;
			case 'inlineEdits':
				this._inlineEditProvider?.handleListEndOfLifetime?.(
					list,
					reason,
				);
				break;
...
async function setEndOfLifeReason(
	provider: vscode.InlineCompletionItemProvider | undefined,
	promise: Promise<vscode.InlineCompletionList | undefined> | undefined,
	reason: vscode.InlineCompletionsDisposeReason,
) {
	...
	for (const item of result.items) {
		provider?.handleEndOfLifetime?.(item, {
