...
function isLlmCompletionInfo(
	item: NesCompletionInfo,
): item is LlmCompletionInfo {
	return item.source === 'provider';
}

const GoToNextEdit = l10n.t('Go To Inline Suggestion');

export class InlineCompletionProviderImpl
	extends Disposable
	implements InlineCompletionItemProvider
{
	...
	//#region Model picker
	private _isModelPickerEnabled: IObservable<boolean> =
		this._configurationService.getExperimentBasedConfigObservable(
			ConfigKey.TeamInternal.InlineEditsModelPickerEnabled,
			this._expService,
		);

	public modelInfo: InlineCompletionModelInfo | undefined;

	private readonly _onDidChangeModelInfo = this._register(
		new Emitter<void>(),
	);
	...
	private readonly _onDidChangeProviderOptions = this._register(
		new Emitter<void>(),
	);
	readonly onDidChangeProviderOptions =
		this._onDidChangeProviderOptions.event;

	setProviderOptionValue = async (
		optionId: string,
		valueId: string,
	): Promise<void> => {
	...
	constructor(
		...
		private readonly inlineEditDebugComponent:
			| InlineEditDebugComponent
			| undefined,
		...
		private readonly _modelService: IInlineEditsModelService,
	) {
		super();
		this._logger = this._logService.createSubLogger(['NES', 'Provider']);
		this._displayNextEditorNES =
			this._configurationService.getExperimentBasedConfig(
				ConfigKey.Advanced.UseAlternativeNESNotebookFormat,
				this._expService,
			);
		this._renameSymbolSuggestions =
			this._configurationService.getExperimentBasedConfigObservable(
				ConfigKey.Advanced.InlineEditsRenameSymbolSuggestions,
				this._expService,
			);
		this._inlineCompletionsAdvanced =
			this._configurationService.getExperimentBasedConfigObservable(
				ConfigKey.TeamInternal.InlineEditsInlineCompletionsAdvanced,
				this._expService,
			);
		this._nesMimicGhostTextBehavior =
			this._configurationService.getExperimentBasedConfigObservable(
				ConfigKey.TeamInternal.InlineEditsNesMimicGhostTextBehavior,
				this._expService,
			);

		this.setCurrentModelId = (modelId: string) =>
			this._modelService.setCurrentModelId(modelId);

		const modelListUpdatedObs = observableFromEvent(
			this,
			this._modelService.onModelListUpdated,
			() => this._modelService.modelInfo,
		);

		this._register(
			autorun((reader) => {
				this.modelInfo = this._isModelPickerEnabled.read(reader)
					? modelListUpdatedObs.read(reader)
					: undefined;
	...
	private _isCompletionsEnabled(document: TextDocument): boolean {
		const enabledLanguages = this._configurationService.getConfig(
			ConfigKey.Enable,
		);
		...
		return enabledLanguagesMap.has(document.languageId)
			? enabledLanguagesMap.get(document.languageId)!
			: enabledLanguagesMap.get('*')!;
	}
	...
	public async provideInlineCompletionItems(
		...
		assert(
			...
			'Expected changeHint to be of type TriggerNes or undefined',
		);
		...
		return this._requestLogger.captureInvocation(capturingToken, () =>
			this._provideInlineCompletionItems(
				document,
				position,
				nesContext,
				token,
	...
	private async _provideInlineCompletionItems(
		...
		const unification = this._configurationService.getExperimentBasedConfig(
			ConfigKey.TeamInternal.InlineEditsUnification,
			this._expService,
		);

		const isInlineEditsEnabled =
			this._configurationService.getExperimentBasedConfig(
		...
		const serveAsCompletionsProvider =
			unification && isCompletionsEnabled && !isInlineEditsEnabled;
		...
		const ignoreWhenSuggestVisible =
			this._configurationService.getExperimentBasedConfig(
				ConfigKey.TeamInternal.InlineEditsIgnoreWhenSuggestVisible,
				this._expService,
			);
		...
		if (
			...
			logger.trace(
				'Return: suggest widget is showing, not providing NES',
			);
			return undefined;
		}
		...
		const documentVersion =
			(isNotebookCell(document.uri)
				? findNotebook(document.uri, workspace.notebookDocuments)
						?.version
				: undefined) || document.version;
		...
		logContext.recordingBookmark =
			this.model.debugRecorder.createBookmark();
		...
		const telemetryBuilder = new NextEditProviderTelemetryBuilder(
			...
			this.model.nextEditProvider.ID,
			doc,
			this.model.debugRecorder,
			logContext.recordingBookmark,
		);
		telemetryBuilder.setOpportunityId(context.requestUuid);
		telemetryBuilder.setConfigIsDiagnosticsNESEnabled(
			!!this.model.diagnosticsBasedProvider,
		);
		telemetryBuilder.setIsNaturalLanguageDominated(
			LineCheck.isNaturalLanguageDominated(document, position),
		);

		const requestCancellationTokenSource = new CancellationTokenSource(
			token,
		);
		...
		try {
			...
			const { first, all } = raceAndAll(
				[
					this.model.nextEditProvider
						...
						.then((r) => ({ kind: 'llm' as const, val: r })),
					(
						this.model.diagnosticsBasedProvider?.runUntilNextEdit(
							doc.id,
							context,
							logContext,
							50,
							requestCancellationTokenSource.token,
							telemetryBuilder.diagnosticsBuilder,
						) ??
						raceCancellation(
							new Promise<undefined>(() => {}),
							requestCancellationTokenSource.token,
						)
					).then((r) => ({ kind: 'diagnostics' as const, val: r })),
				],
				onUnexpectedError,
			);
			...
			if (llmSuggestion !== undefined) {
				if (
					llmSuggestion.val.result !== undefined ||
					this.model.diagnosticsBasedProvider === undefined
				) {
					suggestion = llmSuggestion;
				} else {
					logger.trace(
						'giving some more time to diagnostics provider',
					);
					const remainingTime = clamp(
						1250 - (Date.now() - context.requestIssuedDateTime),
						0,
						1250,
					);
					timeout(remainingTime).then(() =>
				...
				if (
					diagnosticsSuggestion.val !== undefined &&
					diagnosticsSuggestion.val.result !== undefined
				) {
				...
				throw new BugIndicatingError(
					'At least one of LLM NES or Diagnostics NES must be defined',
				);
			}
			...
			const emptyList = new NesCompletionList(
				context.requestUuid,
				undefined,
				[],
				telemetryBuilder,
			...
			if (token.isCancellationRequested) {
				...
				this.telemetrySender.scheduleSendingEnhancedTelemetry(
					{ requestId: logContext.requestId, result: undefined },
					telemetryBuilder,
				);
				return emptyList;
			}
			...
			if (
				suggestion.kind === 'diagnostics' &&
				suggestion.val &&
				suggestion.val.result
			) {
				suggestionInfo = new DiagnosticsCompletionInfo(
					suggestion.val,
					doc.id,
					document,
					context.requestUuid,
				...
				suggestionInfo = new LlmCompletionInfo(
					suggestion.val,
					doc.id,
					document,
					context.requestUuid,
			...
			if (
				...
				const positionToJumpOneBased =
					suggestionInfo.suggestion.result.jumpToPosition;
				const jumpToPosition = new Position(
					positionToJumpOneBased.lineNumber - 1,
					positionToJumpOneBased.column - 1,
				);
				const targetDocumentId =
					suggestionInfo.suggestion.result.targetDocumentId;
				telemetryBuilder.setIsNESForOtherEditor(
					!!targetDocumentId && targetDocumentId !== doc.id,
				);
				const jumpToPositionCompletionItem: NesCompletionItem = {
					...
					jumpToPosition,
					...(targetDocumentId
						? { uri: Uri.parse(targetDocumentId.uri) }
						: {}),
					correlationId,
				};
				return new NesCompletionList(
					context.requestUuid,
					jumpToPositionCompletionItem,
					[],
					telemetryBuilder,
				);
			}
			...
			logger.trace(
				`using next edit suggestion from ${suggestionInfo.source}`,
			);
			...
			const targetDocumentId = isLlmCompletionInfo(suggestionInfo)
				? suggestionInfo.suggestion.result?.targetDocumentId
				: undefined;
			let resolveDoc = doc;
			if (targetDocumentId && targetDocumentId !== doc.id) {
				const targetTextDoc = this._workspaceService.textDocuments.find(
					(d) => d.uri.toString() === targetDocumentId.uri,
				);
				const targetObsDoc = targetTextDoc
					? this.model.workspace.getDocumentByTextDocument(
							targetTextDoc,
						)
					: undefined;
				if (targetObsDoc) {
					...
					logger.trace(
						'no next edit suggestion: cross-file target document not found in workspace',
					);
					telemetryBuilder.setIsNESForOtherEditor(true);
					telemetryBuilder.setStatus(
						'noEdit:crossFileTargetNotFound',
					);
					this.telemetrySender.scheduleSendingEnhancedTelemetry(
						suggestionInfo.suggestion,
						telemetryBuilder,
			...
			const documents = resolveDoc.fromOffsetRange(
				result.edit.replaceRange,
			);
			const [targetDocument, range] = documents.length
				? documents[0]
				: [undefined, undefined];

			addNotebookTelemetry(
				document,
				position,
				result.edit.newText,
				documents,
			...
			telemetryBuilder.setIsNESForOtherEditor(
				targetDocument !== undefined && targetDocument !== document,
			);
			telemetryBuilder.setIsActiveDocument(
				window.activeTextEditor?.document === targetDocument,
			);
			...
			if (!targetDocument) {
				...
				logger.trace(
					'no next edit suggestion, edits contain Notebook Cell Markers',
				);
			} else if (
				isNotebookCell(targetDocument.uri) &&
				this._displayNextEditorNES &&
				...
				// NES is for the active document
				const allowInlineCompletions =
					this.model.inlineEditsInlineCompletionsEnabled.get();
				const inlineSuggestion = allowInlineCompletions
					? toInlineSuggestion(
							position,
							document,
							range,
							result.edit.newText,
							this._inlineCompletionsAdvanced.get(),
						)
					: undefined;
				...
				completionItem =
					serveAsCompletionsProvider && !isInlineCompletion
						...
						: this.createCompletionItem(
								doc,
								document,
								inlineSuggestion?.range ?? range,
								result,
								inlineSuggestion?.newText,
							);
			} else {
				// NES is not for the active doc but a different one
				completionItem = serveAsCompletionsProvider
					? undefined
					: {
							range,
			...
			if (
				...
				suggestionInfo.suggestion.result?.cacheEntry
					?.wasRenderedAsInlineSuggestion
			) {
				logger.trace(
					'Return: previously shown as inline; current context cannot render as inline',
				);
			...
			if (this.inlineEditDebugComponent) {
				menuCommands.push(
					...this.inlineEditDebugComponent.getCommands(logContext),
				);
			}

			// telemetry
			telemetryBuilder.setPickedNESType(
				suggestionInfo.source === 'diagnostics' ? 'diagnostics' : 'llm',
			);
			logContext.setPickedNESType(
				suggestionInfo.source === 'diagnostics' ? 'diagnostics' : 'llm',
			);
			telemetryBuilder.setPostProcessingOutcome({
				edit: result.edit,
				displayLocation: result.displayLocation,
				isInlineCompletion,
			});
			telemetryBuilder.setHadLlmNES(suggestionInfo.source === 'provider');
			telemetryBuilder.setHadDiagnosticsNES(
				suggestionInfo.source === 'diagnostics',
			);
			...
			const supportsRename =
				...
				this._renameSymbolSuggestions.get();
			...
			try {
				this.telemetrySender.sendTelemetry(
					suggestionInfo?.suggestion,
					telemetryBuilder,
				);
			} finally {
	...
	private createNextEditorEditCompletionItem(
	...
	private createCompletionItem(
		...
		range: Range,
		result: NonNullable<
			(NextEditResult | DiagnosticsNextEditResult)['result']
		>,
		...
		const displayLocationRange =
			result.displayLocation &&
			doc.fromRange(
				document,
				toExternalRange(result.displayLocation.range),
			);
		const displayLocation: InlineCompletionDisplayLocation | undefined =
			result.displayLocation && displayLocationRange
				? {
	...
	public handleListEndOfLifetime(
		list: NesCompletionList,
		reason: InlineCompletionsDisposeReason,
	): void {
		const logger = this._logger.createSubLogger([
			'handleListEndOfLifetime',
			shortenOpportunityId(list.requestUuid),
		]);
		logger.trace(
			`List ${list.requestUuid} disposed, reason: ${InlineCompletionsDisposeReasonKind[reason.kind]}`,
		);

		const telemetryBuilder = list.telemetryBuilder;

		const disposeReasonStr =
			InlineCompletionsDisposeReasonKind[reason.kind];
	...
	public handleEndOfLifetime(
		...
		const logger = this._logger.createSubLogger([
			'handleEndOfLifetime',
			shortenOpportunityId(item.info.requestUuid),
		]);
		logger.trace(
			`reason: ${InlineCompletionEndOfLifeReasonKind[reason.kind]}`,
		);
		...
		switch (reason.kind) {
			...
			case InlineCompletionEndOfLifeReasonKind.Rejected: {
				...
				if (
					this.expectedEditCaptureController.isEnabled &&
					this.expectedEditCaptureController.captureOnReject
				) {
					// Get endpoint info from the log context if available (LLM suggestions only)
					const endpointInfo = isLlmCompletionInfo(item.info)
						? item.info.suggestion.source.log.endpointInfo
						: undefined;
					const metadata = {
					...
					void this.expectedEditCaptureController.startCapture(
						'rejection',
						metadata,
					);
				}
			...
			case InlineCompletionEndOfLifeReasonKind.Ignored: {
				const supersededBy = reason.supersededBy
					? (reason.supersededBy as NesCompletionItem)
					: undefined;
				logger.trace(
					`Superseded by: ${supersededBy?.info.requestUuid || 'none'}, was shown: ${item.wasShown}`,
				);
				if (supersededBy) {
					/* __GDPR__
					...
					*/
					this._telemetryService.sendMSFTTelemetryEvent(
						'supersededInlineEdit',
						{
							opportunityId: item.info.requestUuid,
							supersededByOpportunityId:
								supersededBy.info.requestUuid,
						},
					);
				}
	...
	private _handleAcceptance(item: NesCompletionItem) {
		...
		if (isLlmCompletionInfo(info)) {
			this.model.nextEditProvider.handleAcceptance(
				info.documentId,
				info.suggestion,
			);
			...
			this.model.diagnosticsBasedProvider?.handleAcceptance(
				info.documentId,
				info.suggestion,
			);
		}
	...
	private async _trackSurvivalRate(item: LlmCompletionInfo) {
		...
		const recorder = this._instantiationService.createInstance(
		...
		const diffedNextEdit = await stringEditFromDiff(
			docBeforeEdits,
			docAfterEdits,
			this._diffService,
		);
			...
			(res) => {
				...
				*/
				this._telemetryService.sendTelemetryEvent(
					...
					{
						microsoft: true,
						github: { eventNamePrefix: 'copilot-nes/' },
					},
					{
	...
	private _handleDidRejectCompletionItem(
		completionItem: NesCompletionItem,
	): void {
		this.logContextRecorder?.handleRejection(
			completionItem.info.suggestion,
		);
		...
		const info = completionItem.info;
		if (isLlmCompletionInfo(info)) {
			this.model.nextEditProvider.handleRejection(
				info.documentId,
				info.suggestion,
			);
		} else {
			this.model.diagnosticsBasedProvider?.handleRejection(
				info.documentId,
				info.suggestion,
			);
		}
	...
	private _handleDidIgnoreCompletionItem(
		item: NesCompletionItem,
		supersededBy?: NesCompletionItem,
	): void {
		if (supersededBy) {
			item.telemetryBuilder.setSupersededBy(
				supersededBy.info.requestUuid,
			);
		}

		const info = item.info;
		const supersededBySuggestion = supersededBy
			? supersededBy.info.suggestion
			: undefined;
		if (isLlmCompletionInfo(info)) {
			this.model.nextEditProvider.handleIgnored(
				info.documentId,
				info.suggestion,
				supersededBySuggestion,
			);
		} else {
			this.model.diagnosticsBasedProvider?.handleIgnored(
				info.documentId,
				info.suggestion,
				supersededBySuggestion,
			);
...
function hasNotebookCellMarker(document: TextDocument, newText: string) {
	return (
		isNotebookCell(document.uri) && newText.includes('%% vscode.cell [id=')
	);
}

function addNotebookTelemetry(
	document: TextDocument,
	position: Position,
	newText: string,
	documents: [TextDocument, Range][],
	telemetryBuilder: NextEditProviderTelemetryBuilder,
) {
	const notebook = isNotebookCell(document.uri)
		? findNotebook(document.uri, workspace.notebookDocuments)
		: undefined;
	...
	const sourceEol = newText.includes('\r\n')
		...
		: newText.includes('\n')
			? '\n'
			: targetEol;
	const nextEditor = window.visibleTextEditors.find(
		(editor) => editor.document === documents[0][0],
	);
	const isNextEditorRangeVisible =
		nextEditor &&
		nextEditor.visibleRanges.some((range) =>
			range.contains(documents[0][1]),
		);
	...
	const lineCounts = notebook
		.getCells()
		.filter((c) => c.kind === NotebookCellKind.Code)
		.map(
			(c) =>
				`${getCellPrefix(c)}${c.document.lineCount}${c === cell ? lineSuffix : ''}${c.document === documents[0][0] ? suggestionLineSuffix : ''}`,
		)
		.join(',');
	telemetryBuilder
