...
suite('IgnoreImportChangesAspect', () => {
	...
	async function computeDiff(
		...
		const edit = await stringEditFromDiff(
			val1.value,
			val2.value,
			diffService,
		);
...
`);
		...
		expect(
			IgnoreImportChangesAspect.isImportChange(
				lineEdit.replacements[0],
				'typescript',
				doc1.getLines(),
			),
		).toBe(true);
...
suite('filterEdit with ImportChanges', () => {
	...
	function makeDoc(
		...
		'languageId' | 'documentLinesBeforeEdit'
	> {
		return {
			languageId: LanguageId.create(languageId),
			documentLinesBeforeEdit: lines,
		};
	}

	const tsDocWithLocalImport = new StringText(
		[`import { foo } from './local';`, '', 'class A {}'].join('\n'),
	);
	...
	test('none filters all imports', async () => {
		...
		const lineEdit = RootedEdit.toLineEdit(
			await computeDiff(tsDocWithLocalImport, modified),
		);
		const doc = makeDoc('typescript', tsDocWithLocalImport.getLines());
		const result = IgnoreImportChangesAspect.filterEdit(
			doc as StatelessNextEditDocument,
	...
	test('none still keeps non-import edits', async () => {
		...
		const lineEdit = RootedEdit.toLineEdit(
			await computeDiff(tsDocWithBothImports, modified),
		);
