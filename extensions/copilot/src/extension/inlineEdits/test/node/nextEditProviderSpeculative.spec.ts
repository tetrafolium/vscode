...
class TestStatelessNextEditProvider implements IStatelessNextEditProvider {
	...
	public async *provideNextEdit(
		...
		_logger: ILogger,
		_logContext: InlineEditRequestLogContext,
		...
		const streamedOriginalWindow =
			this.editWindow instanceof RequestEditWindowWithCursorJump
				? this.editWindow.originalWindow
				: undefined;
		...
		try {
			if (behavior.kind === 'waitForCancellation') {
				...
				const cancelled = new NoNextEditReason.GotCancelled(
					'testCancellation',
				);
				return new WithStatelessProviderTelemetry(
					cancelled,
					telemetryBuilder.build(Result.error(cancelled)),
				);
			}

			if (
				behavior.kind ===
				'yieldEditThenWaitThenYieldEditsThenNoSuggestions'
			) {
				yield new WithStatelessProviderTelemetry(
					{
						edit: behavior.firstEdit,
						isFromCursorJump: false,
						targetDocument: activeDocId,
						window: streamedEditWindow,
						originalWindow: streamedOriginalWindow,
					},
					telemetryBuilder.build(Result.ok(undefined)),
				);
				await Promise.race([
					behavior.continueSignal.p,
					cancellationRequested.p,
				]);
...
function lineReplacement(
	...
	return new LineReplacement(
		new LineRange(lineNumberOneBased, lineNumberOneBased + 1),
		[newLine],
	);
}
...
describe('NextEditProvider speculative requests', () => {
	...
	function createProviderAndWorkspace(
		statelessProvider: IStatelessNextEditProvider,
	): {
		nextEditProvider: NextEditProvider;
		workspace: MutableObservableWorkspace;
	} {
	...
	async function getNextEdit(
		...
		try {
			return await nextEditProvider.getNextEdit(
				docId,
				context,
				logContext,
	...
	async function getNextEditWithTelemetry(
		nextEditProvider: NextEditProvider,
		docId: DocumentId,
	): Promise<{
		suggestion: Awaited<ReturnType<typeof getNextEdit>>;
		telemetry: ILlmNESTelemetry;
	}> {
		const context = createInlineContext();
		const logContext = new InlineEditRequestLogContext(
			docId.toString(),
		...
		try {
			const suggestion = await nextEditProvider.getNextEdit(
				docId,
				context,
				logContext,
				CancellationToken.None,
	...
	it('does not trigger speculative request when feature is off', async () => {
		...
		const { nextEditProvider, workspace } =
			createProviderAndWorkspace(statelessProvider);
	...
	it('reuses speculative request after acceptance without creating a third request', async () => {
		...
		statelessProvider.enqueueBehavior({
			kind: 'yieldEditThenNoSuggestions',
			edit: lineReplacement(2, 'console.log(value + 1);'),
		});
		const { nextEditProvider, workspace } =
			createProviderAndWorkspace(statelessProvider);
		...
		expect(statelessProvider.calls.length).toBe(2);
		expect(secondSuggestion.result.edit.newText).toBe(
			'console.log(value + 1);',
		);
	});
	...
	it.skip('keeps speculative alive while user types characters of the suggestion (type-through)', async () => {
		...
		statelessProvider.enqueueBehavior({
			kind: 'yieldEditThenNoSuggestions',
			edit: lineReplacement(1, 'foobarbaz();'),
		});
		statelessProvider.enqueueBehavior({ kind: 'waitForCancellation' });
		const { nextEditProvider, workspace } =
			createProviderAndWorkspace(statelessProvider);
	...
	it('cancels mismatched speculative request when starting a request for another document', async () => {
		...
		statelessProvider.enqueueBehavior({
			kind: 'yieldEditThenNoSuggestions',
			edit: lineReplacement(1, 'export const second = 2;'),
		});
		const { nextEditProvider, workspace } =
			createProviderAndWorkspace(statelessProvider);
		...
		const doc1 = workspace.addDocument({
			id: DocumentId.create(
				URI.file('/test/spec-cross-doc-1.ts').toString(),
			),
			initialValue: 'const value = 1;\nconsole.log(value);',
		});
		...
		const doc2 = workspace.addDocument({
			id: DocumentId.create(
				URI.file('/test/spec-cross-doc-2.ts').toString(),
			),
		...
		const secondDocSuggestion = await getNextEdit(
			nextEditProvider,
			doc2.id,
		);
	...
	describe('telemetry', () => {
		it('fresh request has normal headerRequestId and no reusedRequest', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/telemetry-fresh.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);

			const { suggestion, telemetry } = await getNextEditWithTelemetry(
				nextEditProvider,
				doc.id,
			);
			assert(suggestion.result?.edit);
		...
		it('reused speculative request has sp- headerRequestId and reusedRequest=speculative', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/telemetry-spec-reuse.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
			...
			const { suggestion: secondSuggestion, telemetry } =
				await getNextEditWithTelemetry(nextEditProvider, doc.id);
			assert(secondSuggestion.result?.edit);
		...
		it('skips cache delay for edits from speculative requests even when enforceCacheDelay is true', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-skip-delay.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
			...
			try {
				const secondSuggestion = await nextEditProvider.getNextEdit(
					doc.id,
					context,
					logContext,
					CancellationToken.None,
		...
		it('cached speculative result has sp- headerRequestId and isFromCache=true', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/telemetry-spec-cache.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
			...
			const { suggestion: cachedSuggestion, telemetry } =
				await getNextEditWithTelemetry(nextEditProvider, doc.id);
	...
	describe('isSpeculative and isSubsequentEdit flags', () => {
		it('normal request result has isSpeculative = false and isSubsequentEdit = false', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/flags-normal.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('reused speculative result has isSpeculative = true on source', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/flags-speculative.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
			...
			const secondSuggestion = await getNextEdit(
				nextEditProvider,
				doc.id,
			);
			assert(secondSuggestion.result?.edit);
	...
	describe('SpeculativeRequestsAutoExpandEditWindowLines', () => {
		it('Off: speculative request has expandedEditWindowNLines = undefined', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/expand-off.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
			...
			expect(
				statelessProvider.calls[1].request.expandedEditWindowNLines,
			).toBeUndefined();
		...
		it('Always: speculative request has expandedEditWindowNLines from base config', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/expand-always.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('Smart: expandedEditWindowNLines is undefined for first non-speculative edit', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/expand-smart-first.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('Smart: expandedEditWindowNLines uses base config when triggered by speculative chain', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/expand-smart-chain.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
			...
			expect(
				statelessProvider.calls[2].request.expandedEditWindowNLines,
			).toBe(20);
	...
	describe('scheduled speculative requests for multi-edit streams', () => {
		it('does not trigger speculative when shown edit is not the last in a multi-edit stream', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-multi-not-last.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('triggers speculative after stream completes when shown edit is the last one', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-last-edit.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('clears scheduled speculative on rejection before stream completes', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-reject-before-end.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('clears scheduled speculative on handleIgnored (shown, not superseded) before stream completes', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-ignored-before-end.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('fires speculative immediately when stream already completed before handleShown', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-stream-done.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('clears scheduled speculative when a new getNextEdit supersedes the originating stream', async () => {
			...
			statelessProvider.enqueueBehavior({
				kind: 'yieldEditThenNoSuggestions',
				edit: lineReplacement(1, 'const value = 3;'),
			});
			const { nextEditProvider, workspace } =
				createProviderAndWorkspace(statelessProvider);
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-stale-schedule.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('second handleShown replaces a previously scheduled speculative', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-replace-schedule.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
	...
	describe('edit window cursor check for request reuse', () => {
		it('does not reuse in-flight request when cursor moves outside edit window', async () => {
			...
			statelessProvider.editWindow = new RequestEditWindow(
				new OffsetRange(0, 20),
			);
			...
			statelessProvider.enqueueBehavior({
				kind: 'yieldEditThenWait',
				edit: lineReplacement(1, 'const value = 2;'),
				continueSignal: continueSignal1,
			});
			statelessProvider.enqueueBehavior({
				kind: 'yieldEditThenWait',
				edit: lineReplacement(1, 'const value = 3;'),
				continueSignal: continueSignal2,
			});
			const { nextEditProvider, workspace } =
				createProviderAndWorkspace(statelessProvider);
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/ew-outside.ts').toString(),
				),
				initialValue:
					'const value = 1;\nconsole.log(value);\nconst other = 3;\n',
			});
		...
		it('reuses in-flight request when cursor stays within edit window', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/ew-inside.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);\n',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('reuses in-flight request when editWindow is undefined (graceful fallback)', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/ew-undefined.ts').toString(),
				),
				initialValue:
					'const value = 1;\nconsole.log(value);\nconst other = 3;\n',
			});
		...
		it('does not reuse speculative request when cursor moves outside edit window', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/ew-spec-outside.ts').toString(),
				),
				initialValue:
					'const value = 1;\nconsole.log(value);\nconst other = 3;\n',
			});
		...
		it('reuses in-flight request when cursor is within originalWindow of cursor jump edit window', async () => {
			...
			statelessProvider.editWindow = new RequestEditWindowWithCursorJump(
				new OffsetRange(30, 50),
				new OffsetRange(0, 20),
			);
			const continueSignal = new DeferredPromise<void>();
			statelessProvider.enqueueBehavior({
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/ew-cursorjump.ts').toString(),
				),
				initialValue:
					'const value = 1;\nconsole.log(value);\nconst other = 3;\nconst extra = 4;\n',
			});
	...
	describe('cached speculative result delay', () => {
		it('uses speculativeRequestDelay (not cacheDelay) when speculative result is served from cache', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-cache-delay.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
			...
			const start = Date.now();
			try {
				const cachedSuggestion = await nextEditProvider.getNextEdit(
					doc.id,
					context,
					logContext,
					CancellationToken.None,
	...
	describe('lifecycle cancellation', () => {
		it('cancels in-flight speculative when clearCache() is called', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-clear-cache.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('cancels in-flight speculative when its target document is closed', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-doc-close.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
		...
		it('cancels in-flight speculative when the provider is disposed', async () => {
			...
			const doc = workspace.addDocument({
				id: DocumentId.create(
					URI.file('/test/spec-provider-dispose.ts').toString(),
				),
				initialValue: 'const value = 1;\nconsole.log(value);',
			});
			doc.setSelection([new OffsetRange(0, 0)], undefined);
