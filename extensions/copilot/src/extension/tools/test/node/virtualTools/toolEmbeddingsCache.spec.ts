...
import {
	IToolEmbeddingsCache,
	ToolEmbeddingsComputer,
} from '../../../common/virtualTools/toolEmbeddingsComputer';
...
describe('ToolEmbeddingsComputer', () => {
	...
	let embeddingsComputerMock: {
		...
		computeEmbeddings: Mock;
	};

	function createToolEmbeddingComputer(embeddings: Map<string, Embedding>) {
		const computer = accessor
			.get(IInstantiationService)
			.createInstance(TestToolEmbeddingsComputer, embeddings);
		return computer;
	}
	...
	it('should return empty array when no tools are available', async () => {
		...
		const result =
			await computer.retrieveSimilarEmbeddingsForAvailableTools(
				queryEmbedding,
				availableTools,
				10,
				token,
	...
	it('should return tool names for available tools', async () => {
		...
		const computer = createToolEmbeddingComputer(
			new Map([
	...
	it('should handle partial cache hits and compute missing embeddings', async () => {
		...
		expect(embeddingsComputerMock.computeEmbeddings).toHaveBeenCalledTimes(
			1,
		);
		expect(
			embeddingsComputerMock.computeEmbeddings.mock.calls[0][1],
		).toEqual(['tool3\n\nundefined', 'tool4\n\nundefined']);
	});
	...
	it('shoulds cache computed embeddings for future use', async () => {
		...
		const computer = createToolEmbeddingComputer(
			new Map([['tool1', createMockEmbedding([0.9, 0.1, 0])]]),
		);
