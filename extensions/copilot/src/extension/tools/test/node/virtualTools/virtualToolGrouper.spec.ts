...
import {
	...
	VirtualTool,
} from '../../../common/virtualTools/virtualTool';
import { VirtualToolGrouper } from '../../../common/virtualTools/virtualToolGrouper';
import {
	GROUP_WITHIN_TOOLSET,
	MIN_TOOLSET_SIZE_TO_GROUP,
	START_GROUPING_AFTER_TOOL_COUNT,
} from '../../../common/virtualTools/virtualToolsConstants';
...
describe.skip('Virtual Tools - Grouper', () => {
	...
	class TestVirtualToolGrouper extends VirtualToolGrouper {
		...
		protected override async _generateBulkGroupDescriptions(
			embeddingGroups: LanguageModelToolInformation[][],
			token: CancellationToken,
		): Promise<{ groups: ISummarizedToolCategory[]; missed: number }> {
	...
	function makeTool(
		name: string,
		source?: LanguageModelToolExtensionSource | LanguageModelToolMCPSource,
	...
	beforeEach(() => {
		...
		grouper = accessor
			...
			.createInstance(TestVirtualToolGrouper);
		root = new VirtualTool(VIRTUAL_TOOL_NAME_PREFIX, '', Infinity, {
	...
	describe('_deduplicateGroups', () => {
		function vt(name: string, possiblePrefix?: string): VirtualTool {
			return new VirtualTool(name, `VT ${name}`, 0, {}, []);
		}

		it('deduplicates VirtualTool against LM tool by prefixing existing VT', () => {
			const dupName = `${VIRTUAL_TOOL_NAME_PREFIX}foo`;
			const items = [vt(dupName, 'ext_'), makeTool(dupName)];

			const result = VirtualToolGrouper.deduplicateGroups(items) as Array<
			...
			expect(names).toContain(
				`activate_ext_${dupName.slice(VIRTUAL_TOOL_NAME_PREFIX.length)}`,
			);
			expect(
				result.find(
					(i) => i instanceof VirtualTool && i.name === dupName,
				),
			).toBeUndefined();
		});
		...
		it('deduplicates LM tool against VirtualTool by prefixing new VT', () => {
			...
			const items = [makeTool(dupName), vt(dupName, 'mcp_')];
			...
			expect(names).toContain(
				`activate_mcp_${dupName.slice(VIRTUAL_TOOL_NAME_PREFIX.length)}`,
			); // VT is cloned with prefix
		});
		...
		it('handles VT vs VT duplicate by prefixing the first and keeping the second', () => {
			...
			const result = VirtualToolGrouper.deduplicateGroups([
				first,
				second,
			]) as Array<VirtualTool | LanguageModelToolInformation>;

			const vtPrefixed = result.find(
				(i) =>
					i instanceof VirtualTool &&
					i.name ===
						`activate_ext_${dupName.slice(VIRTUAL_TOOL_NAME_PREFIX.length)}`,
			) as VirtualTool | undefined;
			const vtUnprefixed = result.find((i) => i.name === dupName) as
				| VirtualTool
				| undefined;
	...
	describe('addGroups - basic functionality', () => {
		it('should add tools directly when below START_GROUPING_AFTER_TOOL_COUNT', async () => {
			const tools = Array.from(
				{ length: START_GROUPING_AFTER_TOOL_COUNT - 1 },
				(_, i) => makeTool(`tool_${i}`),
			);
	...
	describe('addGroups - toolset grouping', () => {
		...
		it('should group extension tools by extension id', async () => {
			...
			const extensionTools = Array.from(
				{ length: GROUP_WITHIN_TOOLSET + 1 },
				(_, i) => makeTool(`ext_tool_${i}`, extensionSource),
			);
			...
			const allTools = [
				...extensionTools,
				...Array.from(
					{ length: START_GROUPING_AFTER_TOOL_COUNT },
					(_, i) => makeTool(`extra_${i}`),
				),
			];
			...
			const vt = root.contents.filter(
				(tool): tool is VirtualTool => tool instanceof VirtualTool,
			);
		...
		it('should group MCP tools by MCP source label', async () => {
			...
			const mcpTools = Array.from(
				{ length: GROUP_WITHIN_TOOLSET + 1 },
				(_, i) => makeTool(`mcp_tool_${i}`, mcpSource),
			);
		...
		it('should handle mixed toolsets correctly', async () => {
			...
			const tools = [
				...
				...Array.from({ length: GROUP_WITHIN_TOOLSET + 1 }, (_, i) =>
					makeTool(`ext_${i}`, extensionSource),
				),
				...Array.from({ length: GROUP_WITHIN_TOOLSET + 1 }, (_, i) =>
					makeTool(`mcp_${i}`, mcpSource),
				),
			];
			...
			const nonExtra = root.contents.filter(
				(tool) => !tool.name.includes('extra_'),
			);
			const builtInCount = nonExtra.filter(
				(tool) => !(tool instanceof VirtualTool),
			).length;
			const virtualCount = nonExtra.filter(
				(tool) => tool instanceof VirtualTool,
			).length;
	...
	describe('addGroups - toolset size thresholds', () => {
		it('should not group toolsets below MIN_TOOLSET_SIZE_TO_GROUP', async () => {
			...
			const smallToolset = Array.from(
				{ length: MIN_TOOLSET_SIZE_TO_GROUP - 1 },
				(_, i) => makeTool(`small_${i}`, extensionSource),
			);
			...
			const addedDirectly = root.contents.filter(
				(tool) =>
					!(tool instanceof VirtualTool) &&
					tool.name.startsWith('small_'),
			);
		...
		it('should divide large toolsets into subgroups', async () => {
			...
			const largeToolset = Array.from(
				{ length: GROUP_WITHIN_TOOLSET + 5 },
				(_, i) => makeTool(`group${i % 3}_tool_${i}`, extensionSource), // Create 3 groups
			);
	...
	describe('addGroups - state preservation', () => {
		it('should preserve expansion state of existing virtual tools', async () => {
			...
			const virtualTool = root.contents.find(
				(tool) => tool instanceof VirtualTool,
			) as VirtualTool;
			if (virtualTool) {
			...
			const newVirtualTool = root.contents.find(
				(tool) =>
					tool instanceof VirtualTool &&
					tool.name === virtualTool?.name,
			) as VirtualTool;
	...
	describe('cache integration', () => {
		it('should use cache for tool group generation', async () => {
			const tools1 = Array.from(
				{ length: GROUP_WITHIN_TOOLSET + 1 },
				(_, i) =>
					makeTool(
						`grouped_tool_${i}`,
						makeExtensionSource('cached.extension1'),
					),
			);
			const tools2 = Array.from(
				{ length: MIN_TOOLSET_SIZE_TO_GROUP + 1 },
				(_, i) =>
					makeTool(
						`summarized_tool_${i}`,
						makeExtensionSource('cached.extension2'),
					),
			);
			...
			const intoGroups = vi.spyOn(
				grouper,
				'_divideToolsIntoGroups' as any,
			);
			...
			const tools3 = Array.from(
				{ length: MIN_TOOLSET_SIZE_TO_GROUP + 2 },
				(_, i) =>
					makeTool(
						`summarized_tool_${i}`,
						makeExtensionSource('cached.extension2'),
	...
	describe('embedding-based expansion', () => {
		...
		it('should expand virtual tools containing predicted tools when embedding ranking is enabled', async () => {
			...
			vi.spyOn(
				grouper['_toolEmbeddingsComputer'],
			...
			const virtualTools = root.contents.filter(
				(tool): tool is VirtualTool => tool instanceof VirtualTool,
			);
			...
			const extVirtualTool = virtualTools.find((vt) =>
				vt.contents.some(
					(tool) =>
						tool.name === 'predicted_tool1' ||
						tool.name === 'predicted_tool2',
				),
			);
	...
	describe('recomputeEmbeddingRankings', () => {
		...
		it('should do nothing when embedding ranking is disabled', async () => {
			...
			await grouper.recomputeEmbeddingRankings(
				'query',
				root,
				CancellationToken.None,
			);
		...
		it('should create embeddings group with predicted tools', async () => {
			...
			expect(embeddingsGroup.description).toBe(
			...
			expect(embeddingsGroup.contents.map((t) => t.name)).toEqual([
				'predicted1',
				'predicted2',
			]);
		...
		it('should handle errors in embeddings computation gracefully', async () => {
			...
			const embeddingsComputer = accessor.get(IEmbeddingsComputer);
			vi.spyOn(embeddingsComputer, 'computeEmbeddings').mockRejectedValue(
				new Error('Embeddings computation failed'),
			);
			...
			await expect(
				grouper.recomputeEmbeddingRankings(
					'query',
					root,
					CancellationToken.None,
				),
