...
describe('Virtual Tools - Grouping', () => {
	...
	function createSimpleGrouper(): IToolCategorization {
		return {
			recomputeEmbeddingRankings: (query, root, token) =>
				Promise.resolve(),
			addGroups(query, root, tools, token) {
				root.contents = [...tools];
				return Promise.resolve();
	...
	beforeEach(() => {
		...
		grouping = accessor
			...
			.createInstance(TestToolGrouping, []);
	});
	...
	describe('constructor and initialization', () => {
		...
		it('should initialize with provided tools', () => {
			...
			const newGrouping = accessor
				.get(IInstantiationService)
				.createInstance(TestToolGrouping, tools);
	...
	describe('compute()', () => {
		...
		it('should handle mixed grouped and ungrouped tools', async () => {
			...
			const groupTool = result.find((t) =>
				t.name.startsWith(VIRTUAL_TOOL_NAME_PREFIX),
			);
			expect(groupTool).toBeDefined();
			expect(groupTool!.name).toBe(`${VIRTUAL_TOOL_NAME_PREFIX}file`);

			const singleTools = result.filter(
				(t) => !t.name.startsWith(VIRTUAL_TOOL_NAME_PREFIX),
			);
			expect(singleTools).toHaveLength(2);
			expect(singleTools.map((t) => t.name)).toEqual([
				'single1',
				'single2',
			]);
		});
	...
	describe('computeAll()', () => {
		it('should return complete tree including virtual tools', async () => {
			...
			const result = await grouping.computeAll(
				'',
	...
	describe('didCall()', () => {
		...
		it('should expand virtual tool and return result when called', async () => {
			...
			const result = grouping.didCall(
				0,
				`${VIRTUAL_TOOL_NAME_PREFIX}file`,
			);
			...
			expect(resultString.value).toMatchInlineSnapshot(
				`"Tools activated: file_read, file_write, file_delete"`,
			);
		});
	...
	describe('re-collapsing behavior', () => {
		it('should re-collapse least recently used tools when exceeding TRIM_THRESHOLD after cache invalidation', async () => {
			...
			for (
				;
				i < groupNumbers.length && result.length < TRIM_THRESHOLD;
				i++
			) {
				grouping.didCall(
					0,
					`${VIRTUAL_TOOL_NAME_PREFIX}group${groupNumbers[i]}`,
				);
			...
			for (let k = i - 1; k > i - 3; k--) {
				expect(result.map((r) => r.name)).toContain(
					`group${groupNumbers[k]}_tool1`,
				);
			}
		});
		...
		it('should prioritize keeping recently used tools when re-collapsing', async () => {
			...
			const expandedTools = result.filter(
				(tool) =>
					tool.name.includes('group2_tool') &&
					!tool.name.startsWith(VIRTUAL_TOOL_NAME_PREFIX),
			);
	...
	describe('canBeCollapsed metadata', () => {
		it('should respect canBeCollapsed=false during trimming', async () => {
			...
			const allResult = await grouping.computeAll(
				'',
				CancellationToken.None,
			);
			const nonCollapsibleGroup = allResult.find(
				(tool) =>
					tool instanceof VirtualTool &&
					tool.name === `${VIRTUAL_TOOL_NAME_PREFIX}noncollapsible`,
			) as VirtualTool;
			expect(nonCollapsibleGroup?.isExpanded).toBe(true);

			// Collapsible group may have been collapsed - just verify it exists
			const collapsibleGroup = allResult.find(
				(tool) =>
					tool instanceof VirtualTool &&
					tool.name === `${VIRTUAL_TOOL_NAME_PREFIX}collapsible`,
			) as VirtualTool;
