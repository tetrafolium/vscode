...
import {
	Diff,
	IGitDiffService,
} from '../../../platform/git/common/gitDiffService';
...
class GetScmChangesTool implements ICopilotTool<IGetScmChangesToolParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IGetScmChangesToolParams>,
		...
		this.logService.trace(
			`[GetScmChangesTool][invoke] Options: ${JSON.stringify(options)}`,
		);
		...
		const uri = options.input.repositoryPath
			? this.promptPathRepresentationService.resolveFilePath(
					options.input.repositoryPath,
				)
			: undefined;

		let repository = uri
			? await this.gitService.getRepository(uri)
			: undefined;
		repository = repository ?? this.gitService.activeRepository.get();

		if (!repository) {
			this.logService.warn(
				`[GetScmChangesTool][invoke] Unable to resolve the repository using repositoryPath: ${options.input.repositoryPath}`,
			);
			this.logService.warn(
				`[GetScmChangesTool][invoke] Unable to resolve the active repository: ${this.gitService.activeRepository.get()?.rootUri.toString()}`,
			);
		...
		this.logService.trace(
			`[GetScmChangesTool][invoke] Uri: ${uri?.toString()}`,
		);
		this.logService.trace(
			`[GetScmChangesTool][invoke] Repository: ${repository.rootUri.toString()}`,
		);
		...
		if (changes) {
			...
			this.logService.trace(
				`[GetScmChangesTool][invoke] Total changed files: ${changedFiles.length}`,
			);
			...
			if (truncatedCount > 0) {
				this.logService.info(
					`[GetScmChangesTool][invoke] Limiting diff processing to ${MAX_CHANGED_FILES} files (${truncatedCount} additional files will be listed without diffs)`,
				);
			}

			try {
				const diffResult = await raceTimeout(
					this.gitDiffService.getChangeDiffs(
						repository.rootUri,
						filesToDiff,
						token,
					),
				...
				if (diffResult === undefined) {
					this.logService.warn(
						`[GetScmChangesTool][invoke] Diff retrieval timed out after ${DIFF_RETRIEVAL_TIMEOUT_MS}ms`,
					);
				...
				this.logService.warn(
					`[GetScmChangesTool][invoke] Error retrieving diffs: ${e}`,
				);
				const fileList = changedFiles
					.map((f) => f.uri.fsPath)
					.join('\n');
			...
			this.logService.warn(
				`[GetScmChangesTool][invoke] Unable to retrieve changes because there is no active repository`,
			);
		}
		...
		const resultParts: (
			| typeof LanguageModelTextPart.prototype
			| typeof LanguageModelPromptTsxPart.prototype
		)[] = [];
		...
		if (truncatedCount > 0) {
			...
			const truncatedFileList = truncatedFiles
				.map((f) => f.uri.fsPath)
				.join('\n');
			resultParts.push(
				new LanguageModelTextPart(
	...
	prepareInvocation?(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IGetScmChangesToolParams>,
		...
		this.logService.trace(
			`[GetScmChangesTool][prepareInvocation] Options: ${JSON.stringify(options)}`,
		);
		this.logService.trace(
			`[GetScmChangesTool][prepareInvocation] Uri: ${uri?.toString()}`,
		);

		return uri
			? {
					invocationMessage: new MarkdownString(
						l10n.t`Reading changed files in ${formatUriForFileWidget(uri)}`,
					),
					pastTenseMessage: new MarkdownString(
						l10n.t`Read changed files in ${formatUriForFileWidget(uri)}`,
					),
				}
			: {
					invocationMessage: new MarkdownString(
						l10n.t`Reading changed files in the active git repository`,
					),
					pastTenseMessage: new MarkdownString(
						l10n.t`Read changed files in the active git repository`,
					),
				};
	}
	...
	async provideInput(): Promise<IGetScmChangesToolParams | undefined> {
		...
		this.logService.trace(
			`[GetScmChangesTool][provideInput] Active repository: ${this.gitService.activeRepository.get()?.rootUri.toString()}`,
		);

		return Promise.resolve({
			repositoryPath: this.gitService.activeRepository
				.get()
				?.rootUri.toString(),
