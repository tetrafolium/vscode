...
export class EditNotebookTool implements ICopilotTool<IEditNotebookToolParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IEditNotebookToolParams>,
		token: vscode.CancellationToken,
	) {
		let uri = this.promptPathRepresentationService.resolveFilePath(
			options.input.filePath,
		);
		if (!uri) {
			sendEditNotebookToolOutcomeTelemetry(
		...
		if (!stream) {
			sendEditNotebookToolOutcomeTelemetry(
				...
				this.endpointProvider,
				options,
				'invalid_input_no_stream',
			);
			throw new ErrorWithTelemetrySafeReason(
				`Invalid input, no stream`,
				'invalid_input_no_stream',
			);
		}
		...
		try {
			...
			if (await this.fileSystemService.stat(uri).catch(() => false)) {
				...
				const editFileToolExists =
					this.promptContext?.tools?.availableTools?.some(
						(t) => t.name === ToolName.EditFile,
					);
				const toolToCreateFile = editFileToolExists
					? ToolName.EditFile
					: ToolName.CreateFile;
		...
		const provider = this.alternativeNotebookContent.create(
		...
		const cells: ChangedCell[] = notebook
			.getCells()
			.map((cell, index) => ({ cell, index, type: 'existing' }));
		...
		const cellEditsApplied = this.waitForCellOperationComplete(
			notebook,
			done.event,
			expectedCellEdits,
			disposables,
		...
		const textEditsApplied = this.waitForCellTextEditsToComplete(
			done.event,
			expectedCellTextEdits,
			disposables,
			token,
		);
		const sendEndEdit = createSingleCallFunction(() =>
			stream.notebookEdit(notebookUri, true),
		);
		...
		try {
			...
			const { editType, language, newCode, cellId } = this.fixInput(
				options.input,
				notebook,
				provider,
			);
			...
			if (editType === 'insert') {
				...
				if (cellId === 'top') {
					...
					const entry = findLast(
						cells,
						(item) =>
							item.type === 'insert' && item.originalIndex === 0,
					);
					if (entry) {
					...
					notebookCellIndex = cells.filter(
					...
					if (!cell) {
						throw new ErrorWithTelemetrySafeReason(
							getInvalidCellErrorMessage(cellId),
							'invalid_cell_id_insert_after',
							cellId,
						);
					}
				...
				expectedCellEdits.push({
					type: 'insert',
					index: notebookCellIndex,
					cell,
					originalIndex,
				});
				...
				cells
					...
					.filter(({ index }) => index >= notebookCellIndex)
					.forEach((item) => (item.index = item.index + 1));
				cells.splice(cellsCellIndex, 0, {
					cell,
					index: notebookCellIndex,
					type: 'insert',
					originalIndex,
				...
				stream.notebookEdit(
					notebookUri,
					NotebookEdit.insertCells(notebookCellIndex, [cell]),
				);
			} else {
				const cell = cellId ? cellMap.get(cellId) : undefined;
				if (!cell) {
				...
				if (editType === 'delete') {
					const cellRange = new NotebookRange(
						cellIndex,
						cellIndex + 1,
					);
					...
					const cell = cells.find(
						({ index, type }) =>
							index === cellIndex && type === 'existing',
					)!;
					expectedCellEdits.push({
						type: 'delete',
						cell: cell.cell as vscode.NotebookCell,
						index: cellIndex,
					});
					cell.type = 'delete';
					cells
						.filter(({ type }) => type !== 'delete')
						.filter(({ index }) => index > cellIndex)
						.forEach((item) => (item.index = item.index - 1));
					stream.notebookEdit(
						notebookUri,
						NotebookEdit.deleteCells(cellRange),
					);
				} else {
					if (newCode === undefined) {
						throw new ErrorWithTelemetrySafeReason(
							'Invalid input: newCode is required for edit operation',
							'invalid_input_new_code_required',
						);
					}
					...
					const edit = new TextEdit(
						new Range(
							new Position(0, 0),
							existingCell.document.lineAt(
								existingCell.document.lineCount - 1,
							).range.end,
						),
						newCode,
					);
					stream.textEdit(existingCell.document.uri, edit);
					expectedCellTextEdits.push([
						existingCell.document.uri,
						edit,
					]);
				}
			...
			const summaryOfExpectedEdits = summarizeOriginalEdits(
				notebook,
				editType,
				cellId,
				expectedCellEdits,
			...
			const timeoutPromise = new StatefulPromise(
				new Promise<void>((resolve) => {
					const timeout = setTimeout(() => {
						if (expectedCellEdits.length) {
							const summaryOfPendingEdits =
								summarizeEdits(expectedCellEdits);
							this.logger.error(
								`[Notebook] Timed out waiting for cell operations to complete.`,
								`${summaryOfExpectedEdits}. Pending Cell Edits ${summaryOfPendingEdits}`,
							);
						}
						if (expectedCellEdits.length) {
							const summaryOfPendingEdits = summarizeTextEdits(
								notebook,
								expectedCellTextEdits,
							);
							this.logger.error(
								`[Notebook] Timed out waiting for cell text edit operations to complete.`,
								`${summaryOfExpectedEdits}. Pending Text Edits ${summaryOfPendingEdits}`,
							);
						}
						resolve();
					}, 10_000);
			...
			await raceCancellation(
				Promise.race([
					...
					Promise.all([cellEditsApplied, textEditsApplied]),
				]),
				token,
			);
			if (token.isCancellationRequested) {
				...
				new LanguageModelPromptTsxPart(
					await renderPromptElementJSON(
						...
						EditFileResult,
						{
							document: notebook,
							changes: cells,
							languageModel: this.promptContext?.request?.model,
						},
			...
			if (isCancellationError(error)) {
				...
				failureReason =
					error && error instanceof ErrorWithTelemetrySafeReason
						? error.reason
						: 'unknown';
				failureData =
					error && error instanceof ErrorWithTelemetrySafeReason
			...
			disposables.dispose();
			if (!failureReason) {
				sendEditNotebookCellOperationsTelemetry(
					this.telemetryService,
					this.endpointProvider,
					options,
					editOperation,
			...
			sendEditNotebookTelemetry(
				...
				'notebookEdit',
				notebookUri,
				this.promptContext?.requestId,
				options.model ?? this.promptContext?.request?.model,
			);
		}
	}

	async resolveInput(
		input: IEditNotebookToolParams,
		promptContext: IBuildPromptContext,
	): Promise<IEditNotebookToolParams> {
		this.promptContext = promptContext;
		return input;
	}

	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IEditNotebookToolParams>,
		token: vscode.CancellationToken,
			...
			async () => {
				const codeblock =
					...
					createFencedCodeBlock(
						options.input.language || 'python',
						asArray(options.input.newCode || '').join('\n'),
					);
				switch (options.input.editType) {
					case 'insert':
						return (
							l10n.t(
								'Insert a new cell in {0}:',
								formatUriForFileWidget(uri),
							) + codeblock
						);
					case 'edit':
						return (
		...
		return {
			...
			invocationMessage: new MarkdownString(
				l10n.t('Edit {0}', formatUriForFileWidget(uri)),
			),
		};
	}
	...
	private validateInput(
		...
		const cellMap = getCellIdMap(notebook);
		const cell =
			id && id !== 'top' && id !== 'bottom' ? cellMap.get(id) : undefined;
		if (id && id !== 'top' && id !== 'bottom' && !cell) {
			throw new ErrorWithTelemetrySafeReason(
				getInvalidCellErrorMessage(id),
				`invalidCellId${editType}`,
				cellId,
			);
		}
		switch (editType) {
			case 'insert':
				if (newCode === undefined) {
					throw new ErrorWithTelemetrySafeReason(
						'None of the edits were applied as newCode is required for insert operation',
						'missingNewCode',
					);
				}
				if (newCode.length && isJupyterNotebook(notebook)) {
					if (
						newCode.startsWith('{') &&
						newCode.includes('"cell_type') &&
						newCode.includes('"source') &&
						newCode.endsWith('}')
					) {
						// Possible the entire notebook JSON was provided as newCode.
						// This is not supported.
						throw new ErrorWithTelemetrySafeReason(
							'When inserting cell(s) do NOT provide the entire notebook JSON as newCode. Provide the code (as plain text) for the cell instead.',
							'gotEntireNotebookJson',
						);
					}
				}
				break;
			...
			case 'edit':
				...
				if (newCode === undefined) {
					throw new ErrorWithTelemetrySafeReason(
						'None of the edits were applied as newCode is required for edit operation',
						'missingNewCode',
					);
				}
				if (newCode.includes(EXISTING_CODE_MARKER)) {
					throw new ErrorWithTelemetrySafeReason(
						`When editing a cell do NOT use the marker ${EXISTING_CODE_MARKER} to identify existing code. Provide the full code instead.`,
						'gotExistingCodeMarker',
					);
				}
				break;
		}
	...
	private fixInput(
		...
		const language =
			input.language || getDefaultLanguage(notebook) || 'python'; // Default to Python if no language
	...
	async waitForCellOperationComplete(
		notebook: vscode.NotebookDocument,
		done: vscode.Event<void>,
		expectedOutputs: ChangedCell[],
			...
			store.add(
				this.workspaceService.onDidChangeNotebookDocument((e) => {
					...
					expectedOutputs
						.filter((expectedOutput) => {
							...
							for (const change of e.contentChanges) {
								if (
									...
									expectedOutput.type === 'delete' &&
									change.removedCells.some(
										(cell) => cell === expectedOutput.cell,
									)
								) {
									return true;
								...
								if (
									...
									expectedOutput.type === 'insert' &&
									change.addedCells.some(
										(cell) =>
											cell.index === expectedOutput.index,
									)
								) {
									return true;
								}
						...
						.forEach((found) => {
							const index = expectedOutputs.findIndex(
								(i) => i === found,
							);
							if (index !== -1) {
	...
	async waitForCellTextEditsToComplete(
		done: vscode.Event<void>,
		expectedTextEdits: [vscode.Uri, TextEdit][],
		disposables: DisposableStore,
		token: vscode.CancellationToken,
	): Promise<any> {
		const store = disposables.add(new DisposableStore());
		return new Promise<void>((resolve) => {
			...
			store.add(
				this.workspaceService.onDidChangeTextDocument((e) => {
					expectedTextEdits
						.filter(([uri, edit]) => {
							for (const change of e.contentChanges) {
								...
								if (
									isEqual(e.document.uri, uri) &&
									(change.range.contains(edit.range) ||
										edit.range.contains(change.range) ||
										edit.range.isEqual(change.range))
								) {
									return true;
								}
							}
						...
						.forEach((found) => {
							const index = expectedTextEdits.findIndex(
								(i) => i[0] === found[0] && i[1] === found[1],
							);
							if (index !== -1) {
...
function summarizeOriginalEdits(
	notebook: vscode.NotebookDocument,
...
function summarizeTextEdits(
	notebook: vscode.NotebookDocument,
	edits: [vscode.Uri, TextEdit][],
): string {
	const summary: string[] = [];
	for (const [index, edit] of edits.entries()) {
		...
		if (cell) {
			...
			summary.push(
				`[WARNING] ${index}. Cell Uri NOT found, ${range} to Edit (${edit[0].toString()})`,
			);
		}
	}
	return summary.join('\n');
...
export class EditFileResult extends PromptElement<IEditFileResultProps> {
	...
	override async render(state: void, sizing: PromptSizing) {
		...
		const format = this.alternativeNotebookContent.getFormat(
			this.props.languageModel,
		);
		const summary = this.alternativeNotebookContent
			.create(format)
			.getSummaryOfStructure(
				document,
				cellsToInlucdeInSummary,
				EXISTING_CODE_MARKER,
			);
		return (
			<Tag
				name="some_of_the_cells_after_edit"
				attrs={{
			...
			>
				Below is a summary of some of the inserted cells including some
				of the existing cells around the new cells.
				<br />
				NOTE: This is merely a summary and not the actual content of the
				cells nor the entire notebook.
				<br />
				<CodeBlock
...
export async function sendEditNotebookTelemetry(
	telemetryService: ITelemetryService,
	endpointProvider: IEndpointProvider | undefined,
	toolUsedToEditNotebook:
		| 'notebookEdit'
		| 'applyPatch'
		| 'stringReplace'
		| 'newNotebookIntent'
	...
	resource: vscode.Uri,
	requestId?: string,
	chatModel?: vscode.LanguageModelChat | string,
	...
	const model =
		typeof chatModel === 'string'
			...
			: (endpoint?.model ??
				(chatModel &&
					endpointProvider &&
					(await endpointProvider.getChatEndpoint(chatModel)).model));
	...
	*/
	telemetryService.sendMSFTTelemetryEvent(
		'editNotebook.toolUsed',
		{ requestId, editTool: toolUsedToEditNotebook, resourceHash, model },
		{ isNotebook: 1 },
	);
}

async function sendEditNotebookToolOutcomeTelemetry(
	telemetryService: ITelemetryService,
	endpointProvider: IEndpointProvider | undefined,
	options: vscode.LanguageModelToolInvocationOptions<IEditNotebookToolParams>,
	outcome: string,
	...
	const model =
		options.model &&
		endpointProvider &&
		(await endpointProvider.getChatEndpoint(options.model)).model;
	...
	*/
	telemetryService.sendMSFTTelemetryEvent(
		'editNotebook.toolOutcome',
		{ requestId: options.chatRequestId, outcome, model, failureData },
		{ isNotebook: 1 },
	);
}

async function sendEditNotebookCellOperationsTelemetry(
	telemetryService: ITelemetryService,
	endpointProvider: IEndpointProvider | undefined,
	options: vscode.LanguageModelToolInvocationOptions<IEditNotebookToolParams>,
	editOperation: 'insert' | 'edit' | 'delete' | undefined,
) {
	const model =
		options.model &&
		endpointProvider &&
...
async function sendEditNotebookCellTelemetry(
	telemetryService: ITelemetryService,
	...
	*/
	telemetryService.sendMSFTTelemetryEvent(
		'editNotebook.editCellWithCodeMarker',
		{ requestId: options.chatRequestId, resourceHash, model },
		{ hasCodeMarker: hasCodeMarker ? 1 : 0, isNotebook: 1 },
	);
}
