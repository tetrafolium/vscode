...
export class ToolSearchTool implements ICopilotModelSpecificTool<IToolSearchParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IToolSearchParams>,
		...
		const matchedToolNames =
			await this._toolEmbeddingsComputer.searchToolsByQuery(
				query,
				availableTools,
		...
		this._logService.trace(
			`[custom-tool-search] Query "${query}" matched ${matchedToolNames.length} tools: ${JSON.stringify(matchedToolNames)}`,
		);
		...
		inputSchema: {
			...
			properties: {
				query: {
					type: 'string',
					description:
						'Natural language description of what tool capability you are looking for. Use broad queries to cover related tools in one search (e.g., "github" instead of separate searches for issues and PRs).',
				},
			},
			required: ['query'],
		},
