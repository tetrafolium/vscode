...
export class RunNotebookCellTool implements ICopilotTool<IRunNotebookCellToolParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IRunNotebookCellToolParams>,
		...
		const telemetryEvent = new RunNotebookTelemetryEvent(
			filePath,
		...
		try {
			const { notebook, cell } = this.getNotebookAndCell(
				filePath,
				cellId,
			);
			...
			if (
				isJupyterNotebookUri(notebook.uri) &&
				notebook
					.getCells()
					.some(
						(c) => c.document.languageId.toLowerCase() === 'python',
					) &&
				!this.extensionsService.getExtension(JUPYTER_EXTENSION_ID)
			) {
				try {
			...
			if (cell.kind !== 2) {
				...
				return new LanguageModelToolResult([
					new LanguageModelTextPart(
						`Cell ${cellId} is not a code cell so it can't be executed. If this is unexpected, then use the ${ToolName.ReadFile} file tool to get the latest content of the notebook file`,
					),
				]);
				...
				return new LanguageModelToolResult([
					new LanguageModelTextPart(
						`Cell ${cellId} is empty, so it won't be executed. If this is unexpected, then use the ${ToolName.ReadFile} file tool to get the latest content of the notebook file`,
					),
				]);
			}
			...
			try {
				const cellExecution = raceCancellationError(
					this.waitForCellExecution(cell, disposables),
					token,
				);
				const autoRevealArg = this.notebookService.getFollowState();

				await this.notebookService.runCells(
					notebook.uri,
			...
			toolCallResults.push(
				new LanguageModelPromptTsxPart(
					await renderPromptElementJSON(
						...
						RunNotebookCellResultSummary,
						{ executionSummary, infoMessage },
			...
			const endpoint = this._promptContext?.request
				? await this.endpointProvider.getChatEndpoint(
						this._promptContext?.request,
					)
				: undefined;

			for (let i = 0; i < outputs.length; i++) {
				const output = outputs[i];
				const imageItem = endpoint?.supportsVision
					? output.items.find(
							(item) =>
								item.mime === 'image/png' ||
				...
				if (imageItem) {
					...
					toolCallResults.push(
						LanguageModelDataPart.image(
							imageItem.data,
							imageItem.mime === 'image/png'
								? ChatImageMimeType.PNG
								: ChatImageMimeType.JPEG,
						),
					);
					toolCallResults.push(
						new LanguageModelTextPart(`</cell-output>`),
					);
				} else {
					toolCallResults.push(
						new LanguageModelPromptTsxPart(
							await renderPromptElementJSON(
								this.instantiationService,
								RunNotebookCellOutput,
								{ output, index: i, sizeLimitRatio: 4 },
								options.tokenizationOptions,
								token,
							),
						),
	...
	prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IRunNotebookCellToolParams>,
		...
		return {
			...
			invocationMessage: new MarkdownString(
				l10n.t`Running [](${cell.document.uri.toString()})`,
			),
		};
	}

	async resolveInput(
		input: IRunNotebookCellToolParams,
		promptContext: IBuildPromptContext,
	): Promise<IRunNotebookCellToolParams> {
		this._promptContext = promptContext;
		return input;
	}

	private getNotebookAndCell(
		filePath: string,
		cellId: string,
		...
		if (!cell) {
			throw new Error(
				`Cell ${cellId} not found in the notebook, use the ${ToolName.ReadFile} file tool to get the latest content of the notebook file`,
			);
		}
			...
			disposables.push(
				this.workspaceService.onDidChangeNotebookDocument((e) => {
					for (const change of e.cellChanges) {
						if (
							change.executionSummary &&
							typeof change.executionSummary.success ===
								'boolean' &&
							change.cell === cell
						) {
		...
		return (
			<>
				<Tag name={`execution-summaries`}>
					{executionSummary &&
						this.renderSummary('cellId', executionSummary, true)}
				</Tag>
		...
		if (execution?.timing) {
			result = (
				<>
					...
					{execution.timing.endTime - execution.timing.startTime}ms{' '}
					<br />
				</>
			);
		}
...
export class RunNotebookCellOutput extends PromptElement<IRunNotebookCellOutputProps> {
	override render(state: void, sizing: PromptSizing) {
		...
		if (error) {
			...
			const isMissingPackageError = errorMessage.includes(
				'ModuleNotFoundError',
			);

			return (
				<Tag name="cell-execution-error">
					Error: {parseAndCleanStack(errorMessage)}
					<br />
		...
		if (textItem) {
			if (
				textItem.data.byteLength >
				getCharLimit(sizing.tokenBudget / this.props.sizeLimitRatio)
			) {
				return (
					<Tag
						name={`cell-output`}
						attrs={{ mimeType: textItem.mime }}
					>
						Output {index + 1}: Output is too large to be used as
						context in the language model, but the user should be
						able to see it in the notebook.
						<br />
						<br />
					</Tag>
				);
			...
			return (
				<Tag name={`cell-output`} attrs={{ mimeType: textItem.mime }}>
					Output {index + 1}: {textItem.data.toString()}
				</Tag>
			);
		}

		const largeOutput = output.items.find(
			(item) =>
		...
		if (largeOutput) {
			return (
				<Tag
					name={`cell-output`}
					attrs={{ mimeType: largeOutput.mime }}
				>
					Output {index + 1}: Output is too large to be used as
					context in the language model, but the user should be able
					to see it in the notebook.
					<br />
					<br />
				</Tag>
			);
	...
	private renderOutputFallback(
		...
		const textChunk =
			itemsText.length > limit
				? itemsText.substring(0, limit)
				: itemsText;
		return textChunk;
	}
}
