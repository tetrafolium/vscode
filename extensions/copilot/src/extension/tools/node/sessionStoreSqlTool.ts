...
import {
	...
	SESSIONS_QUERY_SQLITE,
	buildRefsQuery,
	buildFilesQuery,
	buildTurnsQuery,
	buildStandupPrompt,
} from '../../chronicle/common/standupPrompt';
...
class SessionStoreSqlTool implements ICopilotTool<SessionStoreSqlParams> {
	...
	constructor(
		...
		private readonly _runCommandService: IRunCommandExecutionService,
	) {
	...
	async invoke(
		...
		switch (action) {
			...
			case 'reindex':
				return this._invokeReindex(
					options.input.force ?? false,
					subcommand ?? 'reindex',
					token,
				);
			default:
				return this._invokeQuery(
					options.input.query ?? '',
					subcommand,
					token,
				);
	...
	private async _invokeQuery(
		...
		if (!sql) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart('Error: Empty query provided.'),
			]);
		}
		...
		for (const pattern of BLOCKED_PATTERNS) {
			if (pattern.test(sql)) {
				this._sendTelemetry({
					...
					durationMs: 0,
					success: false,
					error: 'blocked_mutating_sql',
				});
				return new LanguageModelToolResult([
					new LanguageModelTextPart(
						'Error: Blocked SQL statement. Only SELECT or WITH queries are allowed.',
					),
				]);
			}
		}
		...
		if (!/^(SELECT|WITH)\b/i.test(firstKeywordSrc)) {
			this._sendTelemetry({
				...
				durationMs: 0,
				success: false,
				error: 'blocked_not_select_or_with',
			});
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					'Error: Blocked SQL statement. Only SELECT or WITH queries are allowed.',
		...
		if (sql.includes(';')) {
			...
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					'Error: Only one SQL statement per call. Remove semicolons and split into separate calls.',
				),
			]);
		}
			...
			if (hasCloud) {
				...
				if (cloudResult && 'error' in cloudResult) {
					...
					this._sendTelemetry({
						...
						success: false,
						error: cloudResult.error.substring(0, 100),
					});
					return new LanguageModelToolResult([
						new LanguageModelTextPart(
	 ...
	 */
	private async _invokeStandup(
		subcommand: NonNullable<SessionStoreSqlParams['subcommand']>,
		...
		try {
			...
			if (capped.length > 0) {
				...
				try {
					cappedTurns = this._sessionStore.executeReadOnlyFallback(
						buildTurnsQuery(ids),
					) as unknown as SessionTurnInfo[];
				} catch {
					/* non-fatal */
				}
				try {
					cappedFiles = this._sessionStore.executeReadOnlyFallback(
						buildFilesQuery(ids),
					) as unknown as SessionFileInfo[];
				} catch {
					/* non-fatal */
				}
				...
				if (this._indexingPreference.hasCloudConsent()) {
					const cloudDetail =
						await this._queryCloudTurnsAndFiles(ids);

					if (cloudDetail.turns.length > 0) {
						const seenTurns = new Set(
							cappedTurns.map(
								(t) => `${t.session_id}:${t.turn_index}`,
							),
						);
					...
					if (cloudDetail.files.length > 0) {
						const seenFiles = new Set(
							cappedFiles.map(
								(f) => `${f.session_id}:${f.file_path}`,
							),
						);
						for (const f of cloudDetail.files) {
							if (
								!seenFiles.has(`${f.session_id}:${f.file_path}`)
							) {
								cappedFiles.push(f);
							}
						}
			...
			return new LanguageModelToolResult([
				new LanguageModelTextPart(prompt),
			]);
		} catch (err) {
			const message = err instanceof Error ? err.message : String(err);
			this._sendTelemetry({
			...
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`Error fetching standup data: ${message}`,
				),
			]);
		}
	}
	...
	private async _invokeReindex(
		...
		try {
			...
			lines.push('|---|---|---|---|');
			lines.push(
				`| Sessions | ${statsBefore.sessions} | ${statsAfter.sessions} | +${statsAfter.sessions - statsBefore.sessions} |`,
			);
			lines.push(
				`| Turns | ${statsBefore.turns} | ${statsAfter.turns} | +${statsAfter.turns - statsBefore.turns} |`,
			);
			lines.push(
				`| Files | ${statsBefore.files} | ${statsAfter.files} | +${statsAfter.files - statsBefore.files} |`,
			);
			lines.push(
				`| Refs | ${statsBefore.refs} | ${statsAfter.refs} | +${statsAfter.refs - statsBefore.refs} |`,
			);
			lines.push('');
			lines.push(
				`${result.processed} session(s) processed, ${result.skipped} skipped.`,
			);
			...
			if (!result.cancelled && !token.isCancellationRequested) {
				try {
					const cloudResult =
						(await this._runCommandService.executeCommand(
					...
					if (cloudResult && cloudResult.created > 0) {
						lines.push(
							`${cloudResult.created} session(s) synced to cloud.`,
						);
					}
				} catch {
			...
			return new LanguageModelToolResult([
				new LanguageModelTextPart(`Error during reindex: ${message}`),
			]);
		}
	}
	 ...
	 */
	private _queryLocalStore(): {
		sessions: AnnotatedSession[];
		refs: AnnotatedRef[];
	} {
		try {
			const rawSessions = this._sessionStore.executeReadOnlyFallback(
				SESSIONS_QUERY_SQLITE,
			) as unknown as SessionRow[];
			const sessions: AnnotatedSession[] = rawSessions.map((s) => ({
				...s,
			...
			if (sessions.length > 0) {
				...
				const rawRefs = this._sessionStore.executeReadOnlyFallback(
					buildRefsQuery(ids),
	...
	private async _queryCloudStore(): Promise<{
		sessions: AnnotatedSession[];
		refs: AnnotatedRef[];
	}> {
		const empty = {
			sessions: [] as AnnotatedSession[],
			refs: [] as AnnotatedRef[],
		};
		try {
			const client = new CloudSessionStoreClient(
				this._tokenManager,
			...
			const sessionsResult =
				await client.executeQuery(SESSIONS_QUERY_CLOUD);
			if (
				!sessionsResult ||
			...
			const sessions: AnnotatedSession[] = sessionsResult.rows.map(
				(r) => ({
					id: r.id as string,
					...
					agent_description: r.agent_description as
						| string
						| undefined,
	...
	private async _queryCloudTurnsAndFiles(
		sessionIds: string[],
	): Promise<{ turns: SessionTurnInfo[]; files: SessionFileInfo[] }> {
		const empty = {
			turns: [] as SessionTurnInfo[],
			files: [] as SessionFileInfo[],
		};
		try {
			const client = new CloudSessionStoreClient(
				this._tokenManager,
		...
		const cloudInputSchema = {
			...
			properties: {
				...
				query: {
					type: 'string',
					description:
						"A single DuckDB SQL query to execute. Required when action is 'query'. Read-only queries only (SELECT, WITH). Use now() - INTERVAL for date math, ILIKE for text search. Only one statement per call — do not combine multiple queries with semicolons.",
				},
			},
		};
...
function formatSqlResult(
	...
	const lines: string[] = [];
	lines.push(
		`Results: ${rows.length} rows (source: ${source})${truncated ? ' [TRUNCATED]' : ''}`,
	);
	lines.push('');
	...
	for (const row of rows) {
		const values = columns.map((c) => {
			...
			const s = String(v);
			return s.length > perCellLimit
				? s.slice(0, perCellLimit) + '...'
				: s;
		});
	...
	if (truncated) {
		lines.push('');
		lines.push(
			'⚠️ Results were truncated. Add a LIMIT clause or narrow your query.',
		);
	}
	...
	if (result.length > TOTAL_FORMAT_BUDGET) {
		result =
			result.slice(0, TOTAL_FORMAT_BUDGET) +
