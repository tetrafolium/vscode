...
export class GithubTextSearchTool implements ICopilotTool<GithubTextSearchToolParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<GithubTextSearchToolParams>,
		token: CancellationToken,
	): Promise<vscode.LanguageModelToolResult> {
		const scope = parseScope(options.input.scope);
		if (!scope) {
			throw new Error(
				l10n.t`Invalid input. Could not parse 'scope' argument`,
			);
		}
		...
		result.toolResultMessage =
			references.length === 0
				? new MarkdownString(
						l10n.t`Searched ${scopeLabel} for "${options.input.query}", no results`,
					)
				: references.length === 1
					? new MarkdownString(
							l10n.t`Searched ${scopeLabel} for "${options.input.query}", 1 result`,
						)
					: new MarkdownString(
							l10n.t`Searched ${scopeLabel} for "${options.input.query}", ${references.length} results`,
						);
	...
	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<GithubTextSearchToolParams>,
		_token: vscode.CancellationToken,
	): Promise<vscode.PreparedToolInvocation> {
		if (!options.input.scope) {
			throw new Error(
				l10n.t`Invalid input. No 'scope' argument provided`,
			);
		}
	...
	override render(
		...
		const renderedChunks = this.props.chunks
			...
			.map((chunk) => {
				...
				const displayPath =
					githubInfo?.path ?? chunk.chunk.file.toString();
...
function parseGithubFileUrl(
	uri: URI,
): { nwo: string; path: string } | undefined {
