...
export class FindFilesTool implements ICopilotTool<IFindFilesToolParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IFindFilesToolParams>,
		...
		const globResult = inputGlobToPattern(
			options.input.query,
			workingDir,
			modelFamily,
		);
		...
		const prompt = await renderPromptElementJSON(
			...
			FindFilesResult,
			{ fileResults: resultsToShow, totalResults: results.length },
		...
		result.toolResultMessage =
			resultsToShow.length === 0
				? new MarkdownString(
						l10n.t`Searched for files matching ${query}, no matches`,
					)
				: resultsToShow.length === 1
					? new MarkdownString(
							l10n.t`Searched for files matching ${query}, 1 match`,
						)
					: new MarkdownString(
							l10n.t`Searched for files matching ${query}, ${resultsToShow.length} matches`,
						);
	...
	private async sendSearchToolTelemetry(
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('findFilesToolInvoked', {
			...
			queryStartsWithFolderPath: String(
				isAbsolute(query) &&
					!!this.workspaceService.getWorkspaceFolder(URI.file(query)),
			),
			queryContainsFolderPath: String(
				patternContainsWorkspaceFolderPath(
					query,
					this.workspaceService,
				),
	...
	prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IFindFilesToolParams>,
		...
		return {
			invocationMessage: new MarkdownString(
				l10n.t`Searching for files matching ${query}`,
			),
		};
	}

	private formatQueryLabel(
		globResult: { folderName?: string; folderRelativePattern?: string },
		rawQuery: string,
	): string {
	...
	async resolveInput(
		...
		return {
			...
			query,
			maxResults:
				mode === CopilotToolMode.FullContext
			...
			<>
				{
					<TextChunk priority={20}>
						{this.props.totalResults === 1
							? '1 total result'
							: `${this.props.totalResults} total results`}
					</TextChunk>
				}
				{this.props.fileResults.map((file) => (
					<TextChunk priority={10}>
						<references
							value={[
								new PromptReference(file, undefined, {
									isFromTool: true,
								}),
							]}
						/>
				...
				{this.props.totalResults > this.props.fileResults.length && (
					<TextChunk priority={20}>{'...'}</TextChunk>
				)}
			</>
		);
	}
