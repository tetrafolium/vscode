...
import {
	BinaryFileHexdump,
	hexdumpIfBinary,
} from '../../prompts/node/panel/binaryFileHexdump';
...
export const readFileV2Description: vscode.LanguageModelToolInformation = {
	...
	description:
		'Read the contents of a file. Line numbers are 1-indexed. This tool will truncate its output at 2000 lines and may be called repeatedly with offset and limit parameters to read larger files in chunks. Binary files use offset/limit as byte offsets.',
...
const getParamRanges = (
	params: ReadFileParams,
	snapshot: NotebookDocumentSnapshot | TextDocumentSnapshot,
): IParamRanges => {
	let start: number;
	let end: number;
	let truncated = false;
	if (isParamsV2(params)) {
		...
		if (params.offset !== undefined && params.offset > snapshot.lineCount) {
			throw new Error(
				`Invalid offset ${params.offset}: file only has ${snapshot.lineCount} line${snapshot.lineCount === 1 ? '' : 's'}. Line numbers are 1-indexed.`,
			);
		}
		const limit = clamp(
...
export class ReadFileTool implements ICopilotTool<ReadFileParams> {
	...
	async invoke(
		...
		try {
			...
			if (getImageMimeType(uri)) {
				throw new Error(
					`Cannot read image files with ${ToolName.ReadFile}. Use ${ToolName.ViewImage} instead.`,
				);
			}
			...
			if (binary) {
				...
				void this.sendReadFileTelemetry(
					'success',
					options,
					{ start: 0, end: 0, truncated: false },
					uri,
				);
			...
			const useCodeFences =
				this.configurationService.getExperimentBasedConfig<boolean>(
					ConfigKey.TeamInternal.ReadFileCodeFences,
			...
			return new LanguageModelToolResult([
				new LanguageModelPromptTsxPart(
					await renderPromptElementJSON(
						...
						{
							uri,
							startLine: ranges.start,
							endLine: ranges.end,
							truncated: ranges.truncated,
							snapshot: documentSnapshot,
							languageModel: this._promptContext?.request?.model,
							useCodeFences,
						},
			...
			void this.sendReadFileTelemetry(
				'error',
				options,
				ranges || { start: 0, end: 0, truncated: false },
				uri,
			);
			throw err;
		}
	...
	async prepareInvocation(
		...
		try {
			...
			if (isExternal) {
				...
				const message =
					this.workspaceService.getWorkspaceFolders().length === 1
						? new MarkdownString(
								l10n.t`${formatUriForFileWidget(uri)} is outside of the current folder in ${formatUriForFileWidget(folderUri)}.`,
							)
						: new MarkdownString(
								l10n.t`${formatUriForFileWidget(uri)} is outside of the current workspace in ${formatUriForFileWidget(folderUri)}.`,
							);
			...
			await this.instantiationService.invokeFunction((accessor) =>
				assertFileOkForTool(accessor, uri!, this._promptContext, {
					readOnly: true,
					workingDirectory: options.workingDirectory,
				}),
			);
			...
			try {
				...
				if (String(e).includes('seems to be binary')) {
					return {
						invocationMessage: new MarkdownString(
							l10n.t`Reading binary file ${formatUriForFileWidget(uri)}`,
						),
						pastTenseMessage: new MarkdownString(
							l10n.t`Read binary file ${formatUriForFileWidget(uri)}`,
						),
					};
				}
		...
		if (start === 1 && end === documentSnapshot.lineCount) {
			if (skillInfo) {
				...
				if (this.customInstructionsService.isSkillMdFile(uri)) {
					return {
						invocationMessage: new MarkdownString(
							l10n.t`Reading skill ${formatUriForFileWidget(uri, { vscodeLinkType: 'skill', linkText: skillName })}`,
						),
						pastTenseMessage: new MarkdownString(
							l10n.t`Read skill ${formatUriForFileWidget(uri, { vscodeLinkType: 'skill', linkText: skillName })}`,
						),
					};
				} else {
					return {
						invocationMessage: new MarkdownString(
							l10n.t`Reading skill \`${skillName}\`: ${formatUriForFileWidget(uri)}`,
						),
						pastTenseMessage: new MarkdownString(
							l10n.t`Read skill \`${skillName}\`: ${formatUriForFileWidget(uri)}`,
						),
					};
				}
			}
		...
		const readLocation = new Location(
			uri,
			new Range(start - 1, 0, start - 1, 0),
		);
		if (this.customInstructionsService.isSkillFile(uri)) {
			if (skillInfo) {
				...
				if (this.customInstructionsService.isSkillMdFile(uri)) {
					return {
						invocationMessage: new MarkdownString(
							l10n.t`Reading skill ${formatUriForFileWidget(readLocation, { vscodeLinkType: 'skill', linkText: skillName })}, lines ${start} to ${end}`,
						),
						pastTenseMessage: new MarkdownString(
							l10n.t`Read skill ${formatUriForFileWidget(readLocation, { vscodeLinkType: 'skill', linkText: skillName })}, lines ${start} to ${end}`,
						),
					};
				} else {
					return {
						invocationMessage: new MarkdownString(
							l10n.t`Reading skill \`${skillName}\`: ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`,
						),
						pastTenseMessage: new MarkdownString(
							l10n.t`Read skill \`${skillName}\`: ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`,
						),
					};
				}
			}
		...
		return {
			invocationMessage: new MarkdownString(
				l10n.t`Reading ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`,
			),
			pastTenseMessage: new MarkdownString(
				l10n.t`Read ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`,
			),
		};
	}

	public alternativeDefinition(
		originTool: vscode.LanguageModelToolInformation,
	): vscode.LanguageModelToolInformation {
		if (
			this.configurationService.getExperimentBasedConfig<boolean>(
				ConfigKey.TeamInternal.EnableReadFileV2,
				this.experimentationService,
			)
		) {
	...
	private async getSnapshot(uri: URI) {
		if (this.notebookService.hasSupportedNotebooks(uri)) {
			return this.workspaceService.openNotebookDocumentAndSnapshot(
				uri,
	...
	private async sendReadFileTelemetry(
		outcome: string,
		options: Pick<
			vscode.LanguageModelToolInvocationOptions<ReadFileParams>,
			'model' | 'chatRequestId' | 'input'
		>,
		{ start, end, truncated }: IParamRanges,
		uri: URI | undefined,
		documentSnapshot?: TextDocumentSnapshot | NotebookDocumentSnapshot,
	) {
		const model =
			options.model &&
			(await this.endpointProvider.getChatEndpoint(options.model)).model;
		const extensionSkillInfo =
			uri && this.customInstructionsService.getExtensionSkillInfo(uri);
		const skillInfo =
			extensionSkillInfo ||
			(uri && this.customInstructionsService.getSkillInfo(uri));
		const fileType = skillInfo ? 'skill' : '';
		const nameField = extensionSkillInfo
			? extensionSkillInfo.skillName
			: skillInfo
				? getCachedSha256Hash(skillInfo.skillName)
				: '';
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent(
			'readFileToolInvoked',
			{
				...
				isEntireFile:
					isParamsV2(options.input) &&
					options.input.offset === undefined &&
					options.input.limit === undefined
						? 'true'
						: 'false',
				fileType,
				nameField,
		...
		if (
			...
			documentSnapshot &&
			uri &&
			this.customInstructionsService.isSkillMdFile(uri)
		) {
			const content =
				documentSnapshot instanceof TextDocumentSnapshot
					? documentSnapshot.getText()
					: '';
	...
	async resolveInput(
		input: IReadFileParamsV1,
		promptContext: IBuildPromptContext,
	): Promise<IReadFileParamsV1> {
...
class ReadFileResult extends PromptElement<ReadFileResultProps> {
	...
	override async render() {
		...
		await this.instantiationService.invokeFunction((accessor) =>
			assertFileNotContentExcluded(accessor, this.props.uri),
		);
		...
		return (
			<>
				{(this.props.useCodeFences &&
					range.end.line + 1 !== documentSnapshot.lineCount) ||
				this.props.truncated ? (
					<>
						...
						({documentSnapshot.lineCount} lines total): <br />
					</>
				) : undefined}
				<CodeBlock
					uri={this.props.uri}
					...
					references={[
						new PromptReference(this.props.uri, undefined, {
							isFromTool: true,
						}),
					]}
					lineBasedPriority
