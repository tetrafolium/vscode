...
export class NotebookSummaryTool implements ICopilotTool<INotebookSummaryToolParams> {
	...
	constructor(
		...
		protected readonly notebookStructureTracker: INotebookSummaryTracker,
		@INotebookService private readonly notebookService: INotebookService,
		@ILogService private readonly logger: ILogService,
	) {}

	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<INotebookSummaryToolParams>,
		token: vscode.CancellationToken,
	) {
		this.logger.trace(
			`Invoking Notebook Summary Tool for file ${options.input.filePath}`,
		);
		...
		const altDoc = this.alternativeNotebookContent
			.create(format)
	...
	async resolveInput(
		input: INotebookSummaryToolParams,
		promptContext: IBuildPromptContext,
	): Promise<INotebookSummaryToolParams> {
		this.promptContext = promptContext;
		return input;
	}

	prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<INotebookSummaryToolParams>,
		token: vscode.CancellationToken,
	): vscode.ProviderResult<vscode.PreparedToolInvocation> {
		return {
			invocationMessage: new MarkdownString(
				l10n.t`Retrieving Notebook summary.`,
			),
		};
	}
}
...
export class NotebookSummary extends PromptElement<NotebookStatePromptProps> {
	...
	override async render(state: void, sizing: PromptSizing) {
		try {
			...
			this.logger.error(
				`Error rendering NotebookSummary prompt element for notebook ${this.props.notebook.uri.toString()}`,
				ex,
			);
			throw ex;
		}
	...
	private getSummary() {
		this.logger.trace(
			`Generating notebook summary for ${this.props.notebook.uri.toString()}`,
		);
		const hasAnyCellBeenExecuted = this.props.notebook
			.getCells()
			.some(
				(cell) =>
		...
		return (
			<>
				...
				:<br />
				{hasAnyCellBeenExecuted
					? 'The execution count can be used to determine the order in which the cells were executed'
					: 'None of the cells have been executed'}
				.<br />
				{this.props.notebook.cellCount === 0
					? 'This notebook does not have any cells.'
					: ''}
				<br />
				{this.props.notebook.getCells().map((cell, i) => {
					...
					const altCellStartLine = includeCellLines
						? altDoc.fromCellPosition(cell, new Position(0, 0))
								.line + 1
						: -1;
					const altCellEndLine = includeCellLines
						? altDoc.fromCellPosition(
								cell,
								new Position(cell.document.lineCount - 1, 0),
							).line + 1
						: -1;
					...
					if (
						...
						const state =
							typeof cell.executionSummary?.success ===
							'undefined'
								? 'and'
					...
					if (cell.kind === NotebookCellKind.Markup) {
						executionSummary =
							'This is a markdown cell, and cannot be executed.';
					}
					const indent = '    ';
					...
					const outputs =
						...
						cell.outputs.length > 0 ? (
							<>
								{indent}Cell has outputs with mime types ={' '}
					...
					return (
						<>
							{cellNumber}. Cell Id = {cellId}
							<br />
							{indent}Cell Type = {cellType}
							{language}
							<br />
							{includeCellLines && (
								<>
									{indent}Cell Lines = {cellLines}
									<br />
								</>
							)}
							{indent}
