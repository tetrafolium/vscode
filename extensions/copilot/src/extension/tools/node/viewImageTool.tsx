...
export class ViewImageTool implements ICopilotTool<IViewImageParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IViewImageParams>,
		...
		if (!imageMimeType) {
			throw new Error(
				`Cannot view ${this.promptPathRepresentationService.getFilePath(uri)} with ${ToolName.ViewImage}. Use ${ToolName.ReadFile} for non-image files.`,
			);
		}
		...
		if (stat.size > MAX_IMAGE_FILE_SIZE) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`Cannot view image file ${this.promptPathRepresentationService.getFilePath(uri)}: file size (${Math.round(stat.size / (1024 * 1024))}MB) exceeds the maximum allowed size of ${Math.round(MAX_IMAGE_FILE_SIZE / (1024 * 1024))}MB.`,
				),
			]);
		}

		const imageData = await this.fileSystemService.readFile(uri, true);
		const resized = await this.imageService.resizeImage(
			imageData,
			imageMimeType,
		);
		return new LanguageModelToolResult([
	...
	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IViewImageParams>,
		_token: vscode.CancellationToken,
		...
		if (isExternal) {
			...
			return {
				invocationMessage: new MarkdownString(
					l10n.t`Viewing image ${formatUriForFileWidget(uri)}`,
				),
				pastTenseMessage: new MarkdownString(
					l10n.t`Viewed image ${formatUriForFileWidget(uri)}`,
				),
