...
import {
	summarizeDocuments,
	SummarizeDocumentsItem,
} from '../../prompts/node/inline/summarizedDocument/summarizeDocumentHelpers';
...
class FindTestFilesTool
	extends Disposable
	implements ICopilotTool<IFindTestFilesToolsParams>
{
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IFindTestFilesToolsParams>,
		...
		await Promise.all(
			options.input.filePaths.map(async (filePath) => {
				const uri =
				...
				if (isTestFile(document)) {
					const srcFileUri =
						await this._testFileFinder.findFileForTestFile(
							document,
							token,
						);
					if (
						srcFileUri &&
						!(await this.ignoreService.isCopilotIgnored(srcFileUri))
					) {
						const srcFileDocument =
							await this.workspaceService.openTextDocumentAndSnapshot(
								srcFileUri,
							);
						srcFiles.push({
							srcFile: srcFileUri,
							testFile: document.uri,
							document: srcFileDocument,
						});
					}
				} else {
					const testFileUri =
					...
					if (testFileUri) {
						const testFileDocument =
							await this.workspaceService.openTextDocumentAndSnapshot(
								testFileUri,
							);
						testFiles.push({
							srcFile: document.uri,
							testFile: testFileUri,
							document: testFileDocument,
						});
					}
				}
			}),
		...
		if (testFiles.length === 0 && lookForAnyTestFile) {
			...
			if (
				testFileUri &&
				!(await this.ignoreService.isCopilotIgnored(testFileUri))
			) {
				const testFileDocument =
					await this.workspaceService.openTextDocumentAndSnapshot(
						testFileUri,
				...
				anyTestFiles.push({
					srcFile: lookForAnyTestFile.uri,
					testFile: testFileUri,
					document: testFileDocument,
				});
			}
		...
		const nTestFilesFound =
			testFiles.length + srcFiles.length + anyTestFiles.length;
		...
		result.toolResultMessage =
			nTestFilesFound === 0
				? new MarkdownString(
						l10n.t`Checked ${this.formatURIs(inputUris)} for test related files, none found`,
					)
				: nTestFilesFound === 1
					? new MarkdownString(
							l10n.t`Checked ${this.formatURIs(inputUris)}, 1 file found`,
						)
					: new MarkdownString(
							l10n.t`Checked ${this.formatURIs(inputUris)}, ${nTestFilesFound} files found`,
						);
		return result;
	}

	prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IFindTestFilesToolsParams>,
	...
	): Promise<IFindTestFilesToolsParams | undefined> {
...
class FindTestFilesToolOutput extends PromptElement<IFindTestFilesToolOutputProps> {
	...
	async render(_state: void, sizing: PromptSizing) {
		if (
			this.props.testFiles.length === 0 &&
			this.props.srcFiles.length === 0 &&
			this.props.anyTestFiles.length === 0
		) {
		...
		const documentData: SummarizeDocumentsItem[] = [
			...this.props.testFiles,
			...this.props.anyTestFiles,
			...this.props.srcFiles,
		].map((info) => ({
		...
		const docs = await summarizeDocuments(
			this.parserService,
			documentData,
			tokenSizing,
		);
		...
		return (
			<>
				...
				{this.props.testFiles.map((info) => (
					<RelatedTestDescription
						info={info}
						projectedDoc={docs[index++]}
					/>
				))}
				{this.props.anyTestFiles.map((info) => (
					<ExampleTestDescription
						info={info}
						projectedDoc={docs[index++]}
					/>
				))}
				{this.props.srcFiles.map((info) => (
...
class RelatedSourceDescription extends PromptElement<IRelatedSourceDescriptionProps> {
	...
	render(state: void, sizing: PromptSizing) {
		...
		return (
			<Tag name="relatedSource">
				The test file{' '}
				{this.promptPathRepresentationService.getFilePath(
					this.props.info.testFile,
				)}{' '}
				contains tests for the following file:
				<br />
				<CodeBlock
					code={this.props.projectedDoc.text}
					uri={document.uri}
					languageId={document.languageId}
...
class RelatedTestDescription extends PromptElement<IRelatedTestDescriptionProps> {
	...
	render(state: void, sizing: PromptSizing) {
		const document = this.props.info.document;
		return (
			<Tag name="relatedTest">
				Tests for{' '}
				{this.promptPathRepresentationService.getFilePath(
					this.props.info.srcFile,
				)}{' '}
...
class ExampleTestDescription extends PromptElement<IExampleTestDescriptionProps> {
	constructor(props: PromptElementProps<IExampleTestDescriptionProps>) {
		super(props);
	}
	...
	render(state: void, sizing: PromptSizing) {
		return (
			<Tag name="sampleTest">
				...
				<CodeBlock
					code={this.props.projectedDoc.text}
					uri={this.props.info.document.uri}
					languageId={this.props.info.document.languageId}
					includeFilepath={true}
				/>
				<br />
			</Tag>
