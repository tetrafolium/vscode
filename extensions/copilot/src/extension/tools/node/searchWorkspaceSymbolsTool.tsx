...
class SearchWorkspaceSymbolsTool implements vscode.LanguageModelTool<ISearchWorkspaceSymbolsTool> {
	...
	async invoke(
		...
		toolResult.toolResultMessage =
			symbols.length === 0
				...
				: symbols.length === 1
					? new MarkdownString(
							l10n.t`Searched for ${query}, 1 result`,
						)
					: new MarkdownString(
							l10n.t`Searched for ${query}, ${symbols.length} results`,
						);
	...
	prepareInvocation?(
		options: vscode.LanguageModelToolInvocationPrepareOptions<ISearchWorkspaceSymbolsTool>,
...
class WorkspaceSymbolSearchOutput extends PromptElement<IWorkspaceSymbolSearchOutputProps> {
	...
	override async render(
		...
		const maxResultsText =
			this.props.symbols.length > 20
				? ` (additional ${this.props.symbols.length - symbols.length} results omitted)`
				: '';
			...
			<>
				{
					<TextChunk priority={20}>
						{this.props.symbols.length} total result
				...
				{symbols.map((s, i) => (
					<>
						<Tag name="symbol" priority={20 - i}>
							<references
								value={[
									new PromptReference(s.location, undefined, {
										isFromTool: true,
									}),
								]}
							/>
							...
							, lines {s.location.range.start.line} to{' '}
							{s.location.range.end.line}:<br />
							Symbol: {s.name}, containing symbol:{' '}
							{s.containerName}
						</Tag>
						<br />
					</>
				))}
				{symbols.length < this.props.symbols.length && (
