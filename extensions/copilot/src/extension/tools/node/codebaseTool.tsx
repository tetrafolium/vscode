...
export class CodebaseTool implements vscode.LanguageModelTool<ICodebaseToolParams> {
	...
	constructor(
		...
		private readonly workspaceChunkSearchService: IWorkspaceChunkSearchService,
	...
	async invoke(
			...
			async (searchToken) => {
				...
				return renderPromptElementJSON(
					...
					WorkspaceContextWrapper,
					{
						...
						query,
						maxResults: 32,
						scopedDirectories: options.input.scopedDirectories?.map(
							(dir) => URI.file(dir),
						),
		...
		if (!promptTsxResult) {
			...
			result.toolResultMessage = new MarkdownString(
				l10n.t`Semantic workspace search is not currently available`,
			);
			return result;
		}
		...
		*/
		...
		result.toolResultMessage =
			references.length === 0
				? new MarkdownString(
						l10n.t`Searched ${this.getDisplaySearchTarget(options.input)} for "${query}", no results`,
					)
				: references.length === 1
					? new MarkdownString(
							l10n.t`Searched ${this.getDisplaySearchTarget(options.input)} for "${query}", 1 result`,
						)
					: new MarkdownString(
							l10n.t`Searched ${this.getDisplaySearchTarget(options.input)} for "${query}", ${references.length} results`,
						);
	...
	private async invokeCodebaseAgent(
		...
		const promptElement = await renderPromptElementJSON(
			this.instantiationService,
			ToolCallResultWrapper,
			{ toolCallResults: toolCallLoopResult.toolCallResults },
		);
	...
	prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<ICodebaseToolParams>,
		...
		return {
			invocationMessage: new MarkdownString(
				l10n.t`Searching ${this.getDisplaySearchTarget(options.input)} for "${options.input.query}"`,
			),
		};
	}
	...
	private getDisplaySearchTarget(input: ICodebaseToolParams): string {
		...
		if (input.scopedDirectories && input.scopedDirectories.length === 1) {
			...
			input.scopedDirectories.length > 1
		) {
			targetSearch = l10n.t(
				'{0} directories',
				input.scopedDirectories.length,
			);
		} else {
	...
	private _isCodebaseAgentCall(
		options:
			| vscode.LanguageModelToolInvocationPrepareOptions<ICodebaseToolParams>
			| vscode.LanguageModelToolInvocationOptions<ICodebaseToolParams>,
	): boolean {
		const input = options.input;
		const agentEnabled = this.configurationService.getConfig(
			ConfigKey.CodeSearchAgentEnabled,
		);
		const noScopedDirectories =
