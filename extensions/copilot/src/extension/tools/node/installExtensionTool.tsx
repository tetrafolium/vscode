...
class InstallExtensionTool implements vscode.LanguageModelTool<IInstallExtensionToolInput> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IInstallExtensionToolInput>,
		token: CancellationToken,
	): Promise<vscode.LanguageModelToolResult> {
		const extensionId = options.input.id;
		const existingExtension =
			this._extensionsService.getExtension(extensionId);
		if (existingExtension) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`${options.input.name} extension is already installed`,
				),
			]);
		}

		const insiders = this.envService
			.getEditorInfo()
			.version.includes('insider');
		const args = [
			extensionId,
			{ enable: true, installPreReleaseVersion: insiders ? true : false },
		];
		...
		try {
			...
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`Installed ${options.input.name} extension successfully`,
				),
			]);
		} catch (error) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`Failed to install ${options.input.name} extension.`,
				),
			]);
		}
	}
		...
		do {
			...
			if (extension) {
				...
				if (
					...
					languageModelTools.length
				) {
					if (
						languageModelTools.every((tool) =>
							this.toolsService.getTool(tool.name),
						)
					) {
						return;
					}
	...
	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IInstallExtensionToolInput>,
		...
		const markdownString = new MarkdownString(
			l10n.t(
				`Copilot will install the extension [{0}](command:workbench.extensions.action.showExtensionsWithIds?{1}) and its dependencies.`,
				options.input.name,
				query,
			),
		);
