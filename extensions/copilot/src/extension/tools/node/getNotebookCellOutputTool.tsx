...
export class GetNotebookCellOutputTool implements ICopilotTool<IGetNotebookCellOutputToolParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IGetNotebookCellOutputToolParams>,
		...
		if (!cell) {
			...
			throw new Error(
				`Cell not found, use the ${ToolName.ReadFile} file tool to get the latest content of the notebook file.`,
			);
		}
		...
		try {
					...
					toolCallResults.push(
						new LanguageModelPromptTsxPart(
							await renderPromptElementJSON(
								...
								{ output, index: i, sizeLimitRatio: 1.2 },
			...
			result.toolResultMessage = new MarkdownString(
				`Read output of [](${cellUri?.toString()})`,
			);
	...
	prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IGetNotebookCellOutputToolParams>,
	...
	async resolveInput(
		input: IGetNotebookCellOutputToolParams,
		promptContext: IBuildPromptContext,
	): Promise<IGetNotebookCellOutputToolParams> {
...
async function sendOutcomeTelemetry(
