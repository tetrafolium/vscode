...
import {
	CellOrNotebookEdit,
	processFullRewriteNotebookEdits,
} from '../../prompts/node/codeMapper/codeMapper';
import {
	EditTools,
...
export interface IPrepareEdit {
	...
	generatedEdit:
		| {
				...
				notebookEdits?: CellOrNotebookEdit[];
				updated:
...
> implements ICopilotTool<T> {
	...
	private async _prepareEdits(
		...
		const results = await Promise.all(
			input.map((i) => this._prepareEditsForFile(options, i, token)),
		);
	...
	private _errorConflictingEdits(results: IPrepareEdit[]) {
		for (let i = 1; i < results.length; i++) {
			...
			for (let k = 0; k < i; k++) {
				...
				if (
					!other.generatedEdit.success ||
					!extUriBiasedIgnorePathCase.isEqual(current.uri, other.uri)
				) {
					continue;
				}
	...
	private async _prepareEditsForFile(
		options:
			| vscode.LanguageModelToolInvocationOptions<T>
			| vscode.LanguageModelToolInvocationPrepareOptions<T>,
		input: IAbstractReplaceStringInput,
		...
		if (!exists) {
			return {
				...
				generatedEdit: input.oldString
					? {
							success: false,
							errorMessage: `File does not exist: ${input.filePath}. Use the ${ToolName.CreateFile} tool to create it, or correct your filepath.`,
						}
					: {
							success: true,
							textEdits: [
		...
		const document = await this.instantiationService.invokeFunction(
			openDocumentAndSnapshot,
			this._promptContext,
			uri,
		);
		...
		try {
			...
			const { updatedFile, edits } = await this.generateEdit(
				uri,
				document,
				options,
				input,
			...
			let notebookEdits:
				| (vscode.NotebookEdit | [URI, vscode.TextEdit[]])[]
				| undefined;
			...
			if (document instanceof NotebookDocumentSnapshot) {
				...
				notebookEdits = await Iterable.asyncToArray(
					processFullRewriteNotebookEdits(
						document.document,
						updatedFile,
				...
				updated = NotebookDocumentSnapshot.fromNewText(
					updatedFile,
					document,
				);
			} else {
			...
			return {
				...
				input,
				healed: didHealRef.healed,
				generatedEdit: { success: false, errorMessage },
			};
		}
	}

	protected async applyAllEdits(
		options: vscode.LanguageModelToolInvocationOptions<T>,
		edits: IPrepareEdit[],
		token: vscode.CancellationToken,
	) {
		...
		logEditToolResult(
			...
			...edits.map((e) => ({
				input: e.input,
				success: e.generatedEdit.success,
				healed: e.healed,
			})),
		);
		...
		for (const { document, uri, generatedEdit, healed } of edits) {
			...
			const existingDiagnostics = document
				? existingDiagnosticMap.get(document.uri)!
				: [];
			const isNotebook = this.notebookService.hasSupportedNotebooks(uri);
			...
			if (!generatedEdit.success) {
				fileResults.push({
					...
					isNotebook,
					existingDiagnostics,
					error: generatedEdit.errorMessage,
				});
				continue;
			}
			...
			if (generatedEdit.notebookEdits) {
				...
				for (const edit of generatedEdit.notebookEdits) {
					if (edit instanceof Array) {
						...
						this._promptContext.stream.notebookEdit(
							uriToEdit,
							edit,
						);
					}
				...
				fileResults.push({
					...
					healed: healed
						? JSON.stringify(
								{
									oldString: healed.oldString,
									newString: healed.newString,
								},
								null,
								2,
							)
	...
	private async generateEdit(
		...
		options:
			...
			| vscode.LanguageModelToolInvocationPrepareOptions<T>,
		input: IAbstractReplaceStringInput,
		didHealRef: { healed?: IAbstractReplaceStringInput },
		token: vscode.CancellationToken,
	) {
		...
		try {
			const result = await this.doGenerateEdit(
				uri,
				oldString,
				newString,
				options,
	...
	protected async modelForTelemetry(
		options:
			| vscode.LanguageModelToolInvocationOptions<T>
			| vscode.LanguageModelToolInvocationPrepareOptions<T>,
	) {
		const model = this.modelObjectForTelemetry(options);
		return (
			model && (await this.endpointProvider.getChatEndpoint(model)).model
		);
	}

	protected modelObjectForTelemetry(
		options:
			| vscode.LanguageModelToolInvocationOptions<T>
			| vscode.LanguageModelToolInvocationPrepareOptions<T>,
	) {
	...
	private async recordEditSuccess(
		options:
			| vscode.LanguageModelToolInvocationOptions<T>
			| vscode.LanguageModelToolInvocationPrepareOptions<T>,
		success: boolean,
	...
	async prepareInvocation(
		...
		const allUris = replaceInputs.map((input) =>
			resolveToolInputPath(
				input.filePath,
				this.promptPathRepresentationService,
			),
		...
		const diffResults = await Promise.all(
			allPreparedEdits.map(async (preparedEdit) => {
				...
				if (preparedEdit.generatedEdit.success) {
					...
					const newContent =
						preparedEdit.generatedEdit.updated?.getText() || '';
