...
suite('TestFailureTool', () => {
	...
	async function doTest(
		...
		otherProps?: Partial<ChatToolCallsProps>,
	): Promise<RenderPromptResult> {
		const element = otherProps?.isHistorical
			? ChatToolCallsWrapper
			: ChatToolCalls;
	...
	test('tool fails on first call, not second', async () => {
		...
		const toolCallRounds: ToolCallRound[] = [
			new ToolCallRound(
				'I will run the tool',
				[{ id: id1, name: 'testTool', arguments: '{}' }],
				0,
				'id-4',
			),
		];
	...
	test('invalid JSON on first call, not second', async () => {
		...
		const toolCallRounds: ToolCallRound[] = [
			new ToolCallRound(
				'I will run the tool',
				[{ id: id1, name: 'testTool', arguments: '{ "xyz": ' }],
				0,
				'id-6',
			),
		];
		...
		toolCallRounds.push(
			new ToolCallRound(
				'I will retry the tool',
				[{ id: '2', name: 'testTool', arguments: '{ "xyz": 123}' }],
				1,
				'id-7',
			),
		);
...
suite('ChatToolCalls thinking handling', () => {
	...
	function hasThinkingPart(result: RenderPromptResult): boolean {
		return result.messages.some(
			(m) =>
				...
				m.content.some(
					(c) =>
						c.type === Raw.ChatCompletionContentPartKind.Opaque &&
						rawPartAsThinkingData(c) !== undefined,
				),
		);
	}
	...
	test('historical: drops thinking on messages API even when modelId matches', async () => {
		...
		const result = await render(
			endpoint,
			[makeThinkingRound('claude-haiku-4-5')],
			true,
		);
	...
	test('non-historical: drops thinking when modelId mismatches', async () => {
		...
		const result = await render(
			endpoint,
			[makeThinkingRound('claude-4.6-sonnet')],
			false,
		);
		expect(hasThinkingPart(result)).toBe(false);
	});
	...
	test('historical: backward-compat reads phaseModelId when modelId is missing', async () => {
		...
		(round as ToolCallRound & { phaseModelId?: string }).phaseModelId =
			'gpt-5';
		const result = await render(endpoint, [round], true);
		expect(hasThinkingPart(result)).toBe(true);
	});
