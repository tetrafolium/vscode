...
import {
	...
	mapLoopResponseToText,
} from '../searchSubagentTool';
...
function makeToolInstance(
	...
	const toolCtor = ToolRegistry.getTools().find(
		(t) => t.toolName === ToolName.SearchSubagent,
	)!;
	...
	const configService = {
		getExperimentBasedConfig(key: { id: string }) {
			if (
				key.id ===
				ConfigKey.Advanced.SearchSubagentThoroughnessEnabled.id
			) {
	...
	const instantiationService = {
		createInstance(
			_ctor: unknown,
			options: { toolCallLimit: number; thoroughness?: string },
		) {
			capturedLoopOptions.push({
			...
			return {
				run: async () => ({
					response: { type: 'error', reason: 'stub' },
					toolCallRounds: [],
					round: { response: '' },
				}),
			};
	...
	const workspaceService = {
		...
		openTextDocument:
			...
			(async () => {
				throw new Error('openTextDocument not stubbed');
			}),
	};
	...
	const tool = new (toolCtor as any)(
		...
		{
			captureInvocation: async (_token: unknown, fn: () => unknown) =>
				fn(),
		}, // requestLogger
		workspaceService,
		configService,
...
suite('SearchSubagentTool', () => {
	...
	suite('alternativeDefinition', () => {
		...
		test('adds thoroughness enum to inputSchema when enabled', () => {
			...
			const result = tool.alternativeDefinition(makeToolInfo());
			const props = (
				result.inputSchema as { properties: Record<string, unknown> }
			).properties;
			expect(props).toHaveProperty('thoroughness');
			expect((props['thoroughness'] as { enum: string[] }).enum).toEqual([
				'normal',
				'deep',
			]);
		});
	...
	suite('thoroughness tool-call limit', () => {
		test('deep doubles the base toolCallLimit', async () => {
			const baseLimit = 4;
			const { tool, capturedLoopOptions } = makeToolInstance(
				true,
				baseLimit,
			);
			tool['_inputContext'] = {
	...
	suite('parseFinalAnswerAndHydrate', () => {
		...
		test('preserves non-matching lines verbatim when the model uses a different format', async () => {
			...
			const result = await tool['parseFinalAnswerAndHydrate'](
				response,
				'/workspace',
				undefined,
				notCancelled,
		...
		test('keeps the original line when an inside-workspace path fails to open', async () => {
			const { tool } = makeToolInstance(false, 4, {
				invokeFunction: sequencedInvokeFunction(undefined, false),
...
suite('mapLoopResponseToText', () => {
	...
	test('context-overflow BadRequest is converted to the benign final_answer fallback', () => {
		...
		for (const reason of overflowReasons) {
			...
			expect(text, `reason "${reason}" should map to fallback`).toBe(
				CONTEXT_OVERFLOW_FALLBACK,
			);
		}
	});
