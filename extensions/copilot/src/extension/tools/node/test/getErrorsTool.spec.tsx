...
suite('GetErrorsTool - Tool Invocation', () => {
	...
	const eslintErrorFile = URI.file(
		'/test/workspace/eslint/eslint_unexpected_constant_condition_1.ts',
	);
	...
	beforeEach(() => {
		...
		const tsDoc1 = createTextDocumentData(
			tsFile1,
			'function test() {\n  const x = 1;\n  return x;\n}',
			'ts',
		).document;
		const tsDoc2 = createTextDocumentData(
			tsFile2,
			'interface User {\n  name: string;\n  age: number;\n}',
			'ts',
		).document;
		const jsDoc = createTextDocumentData(
			jsFile,
			'function legacy() {\n  var y = 2;\n  return y;\n}',
			'js',
		).document;
		const noErrorDoc = createTextDocumentData(
			noErrorFile,
			'',
			'ts',
		).document;
		const eslintErrorDoc = createTextDocumentData(
			eslintErrorFile,
			'if (true) {\n  console.log("This is a constant condition");\n}',
			'ts',
		).document;
		// File with a trailing empty line where the error is reported
		const emptyLineErrorDoc = createTextDocumentData(
			emptyLineErrorFile,
			'codeunit 50100 MyCU {\n  procedure Foo() {\n\n',
			'ts',
		).document;
	...
	test('getDiagnostics - filters by file path', () => {
		...
		const results = tool.getDiagnostics([
			{ uri: tsFile1, range: undefined },
		]);
	...
	test('getDiagnostics - file with no diagnostics returns empty diagnostics array', () => {
		const noErrorFile = URI.file('/test/workspace/src/noErrorFile.ts');
		const results = tool.getDiagnostics([
			{ uri: noErrorFile, range: undefined },
		]);

		expect(results).toEqual([{ uri: noErrorFile, diagnostics: [] }]);
	});
	...
	test('getDiagnostics - folder path excludes files with only Info and Hint diagnostics', () => {
		...
		const infoHintOnlyFile = URI.file(
			'/test/workspace/src/infoHintOnly.ts',
		);
	...
	test('Tool invocation - with no filePaths aggregates all diagnostics and formats workspace message', async () => {
		const result = await tool.invoke(
			{ input: {}, toolInvocationToken: null! },
	...
	test('Tool invocation - with single filePath limits diagnostics and message to that file', async () => {
		...
		const result = await tool.invoke(
			{ input: { filePaths: [filePath] }, toolInvocationToken: null! },
			CancellationToken.None,
		);
		const msg = await toolResultToString(accessor, result);
		expect(msg).toMatchSnapshot();
	...
	test('Tool invocation - filePath with range has a <compileError> entry', async () => {
		...
		const result = await tool.invoke(
			{
				input: { filePaths: [filePath], ranges: [[1, 4, 1, 4]] },
				toolInvocationToken: null!,
			},
			CancellationToken.None,
		);
