...
suite('MultiReplaceString', () => {
	...
	beforeAll(() => {
		...
		const allDocs = [
			...
			createTextDocumentData(
				URI.file('/workspace/multi-sr-bug.ts'),
				readFileSync(
					__dirname +
						'/editFileToolUtilsFixtures/multi-sr-bug-original.txt',
					'utf-8',
				),
				'ts',
			),
			createTextDocumentData(
				URI.file('/workspace/math.js'),
				readFileSync(
					__dirname + '/editFileToolUtilsFixtures/math-original.txt',
					'utf-8',
				),
				'js',
			),
	...
	async function invoke(params: IMultiReplaceStringToolParams) {
		...
		toolsService
			.getCopilotTool(ToolName.MultiReplaceString)
			?.resolveInput?.(
				params,
				{ stream } as any,
	...
	test('multi-sr bug', async () => {
		const input: IMultiReplaceStringToolParams = {
			explanation:
				'Update session imports and type annotations to use IModifiedFileEntryInternal',
			replacements: [
				{
...
import { IFile } from '../../../../../base/node/zip.js';`,
	...
	public get entries(): IObservable<readonly IModifiedFileEntry[]> {
		...
		const r = await invoke(input);
		await expect(await applyEditsInMap(r.edits)).toMatchFileSnapshot(
			__dirname + '/editFileToolUtilsFixtures/multi-sr-bug-actual.txt',
		);
	});
...
export function sumThreeFloats(a, b, c) {`,
		...
		expect(await toolResultToString(accessor, r.result))
