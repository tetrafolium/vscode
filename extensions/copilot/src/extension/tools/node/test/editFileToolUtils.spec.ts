...
import {
	isMacintosh,
	isWindows,
} from '../../../../util/vs/base/common/platform';
...
import {
	...
	ContentFormatError,
	makeUriConfirmationChecker,
	MultipleMatchesError,
	NoChangeError,
	NoMatchError,
	setSimilarityMatchThresholdForTests,
...
describe('replace_string_in_file - applyEdit', () => {
	...
	async function doApplyEdit(
		...
		const r = await applyEdit(
			...
			newString,
			workspaceService,
			notebookService as INotebookService,
			alternatveContentService,
			undefined,
		);
	...
	test('multiple exact matches - should throw error', async () => {
		setText('test\ntest\nother');
		await expect(doApplyEdit('test', 'replacement')).rejects.toThrow(
			MultipleMatchesError,
		);
	});
	...
	test('no match found - should throw error', async () => {
		...
		await expect(doApplyEdit('nonexistent', 'replacement')).rejects.toThrow(
			NoMatchError,
		);
	});
	...
	test('empty old string on existing file - should throw error', async () => {
		...
		await expect(doApplyEdit('', 'new content')).rejects.toThrow(
			ContentFormatError,
		);
	});
	...
	test('no change - identical strings should throw error', async () => {
		...
		await expect(
			doApplyEdit('unchanged text', 'unchanged text'),
		).rejects.toThrow(NoChangeError);
	});
	...
	test('case sensitive matching', async () => {
		setText('Hello World');
		await expect(doApplyEdit('hello world', 'Hi World')).rejects.toThrow(
			NoMatchError,
		);
	});
	...
	test('very long strings', async () => {
		...
		expect(result.updatedFile).toBe(
			'a'.repeat(1000) + 'CENTER' + 'b'.repeat(1000),
		);
	});
	...
	test('multiple single character matches - should throw error', async () => {
		setText('hello world');
		await expect(doApplyEdit('l', 'L')).rejects.toThrow(
			MultipleMatchesError,
		);
	});
	...
	test('fixes bad newlines in issue #9753', async () => {
		const input = JSON.parse(
			fs.readFileSync(
				__dirname + '/editFileToolUtilsFixtures/crlf-input.json',
				'utf8',
			),
		);
		const output = JSON.parse(
			fs.readFileSync(
				__dirname + '/editFileToolUtilsFixtures/crlf-output.json',
				'utf8',
			),
		).join('\r\n');
		const toolCall = JSON.parse(
			fs.readFileSync(
				__dirname + '/editFileToolUtilsFixtures/crlf-tool-call.json',
				'utf8',
			),
		);

		const crlfDoc = createTextDocumentData(
			URI.file('/my/file2.ts'),
			input.join('\r\n'),
			'ts',
			'\r\n',
		);
	...
	describe('similarity matching', () => {
		...
		test('similarity match does not trigger for low similarity', async () => {
			...
			await expect(
				doApplyEdit(
					'completely different text here with no similarity at all to the original',
					'replacement',
				),
	...
	describe('edit minimization', () => {
		test('exact match minimizes edits - preserves identical prefix/suffix', async () => {
			...
			const result = await doApplyEdit(
				'prefix unchanged middle changed suffix unchanged',
				'prefix unchanged middle updated suffix unchanged',
			);
...
describe('assertPathIsSafe (Windows scenarios)', () => {
	...
	test('accepts normal path', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\project\\file.txt', true),
		).not.toThrow();
	});

	test('rejects null byte', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\proje\0ct\\file.txt', true),
		).toThrow();
	});
	...
	test('rejects ADS suffix', () => {
		expect(() =>
			assertPathIsSafe(
				'C:\\Users\\me\\project\\file.txt:$I30:$INDEX_ALLOCATION',
				true,
			),
		).toThrow();
	});
	...
	test('rejects additional colon in component', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\file:name.txt', true),
		).toThrow();
	});

	test('rejects invalid characters', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\proj>ect\\file.txt', true),
		).toThrow();
	});
	...
	test('rejects device path prefix \\?\\', () => {
		...
		expect(() =>
			assertPathIsSafe('\\\\?\\C:\\Users\\me\\file.txt', true),
		).toThrow();
	});

	test('rejects reserved device name component', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\CON\\file.txt', true),
		).toThrow();
	});

	test('rejects trailing dot in component', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\folder.\\file.txt', true),
		).toThrow();
	});
	...
	test('rejects 8.3 short filename pattern', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\VSCODE~1\\settings.json', true),
		).toThrow();
	});

	test('allows tilde without digit', () => {
		expect(() =>
			assertPathIsSafe('C:\\Users\\me\\my~folder\\file.txt', true),
		).not.toThrow();
	});
});
...
describe('makeUriConfirmationChecker', async () => {
	...
	test('allows files within workspace folder', async () => {
		...
		const checker = makeUriConfirmationChecker(
			configService,
			workspaceService.getWorkspaceFolder.bind(workspaceService),
	...
	test('multiple workspace folders - allows files in any folder', async () => {
		...
		expect(await checker(fileInWorkspace1)).toBe(
			ConfirmationCheckResult.NoConfirmation,
		);
		expect(await checker(fileInWorkspace2)).toBe(
			ConfirmationCheckResult.NoConfirmation,
		);
	});
	...
	if (isMacintosh) {
		test('pattern matching macOS Library path', async () => {
			...
			const normalFile = URI.file(
				`${homedir()}/Library/MyApp/src/app.ts`,
			);
			expect(await checker(normalFile)).toBe(
				ConfirmationCheckResult.SystemFile,
			);
		});
		...
		test('pattern matching workspace folder on macOS Library path', async () => {
			...
			const configFile = URI.file(
				`${homedir()}/Library/MyApp/settings.config`,
			);

			expect(await checker(normalFile)).toBe(
				ConfirmationCheckResult.NoConfirmation,
			);
			expect(await checker(configFile)).toBe(
				ConfirmationCheckResult.Sensitive,
			);
		});
	}
	...
	test('handles relative workspace patterns correctly', async () => {
		...
		expect(await checker(srcFile)).toBe(
			ConfirmationCheckResult.NoConfirmation,
		);
	...
	test('pattern matching is workspace-relative', async () => {
		...
		expect(await checker(secretsInWorkspace1)).toBe(
			ConfirmationCheckResult.Sensitive,
		); // Sensitive
		expect(await checker(secretsInWorkspace2)).toBe(
			ConfirmationCheckResult.Sensitive,
		); // Sensitive
	});
	...
	describe('hookFilesLocations', () => {
		...
		test('folder pattern marks JSON files inside as sensitive', async () => {
			...
			const hookFile = URI.file(
				'/workspace/.github/hooks/pre-commit.json',
			);
			expect(await checker(hookFile)).toBe(
				ConfirmationCheckResult.Sensitive,
			);
		});
		...
		test('folder pattern marks the folder itself as sensitive', async () => {
			...
			expect(await checker(folderUri)).toBe(
				ConfirmationCheckResult.Sensitive,
			);
		});
		...
		test('folder pattern does not match non-JSON files', async () => {
			...
			expect(await checker(txtFile)).toBe(
				ConfirmationCheckResult.NoConfirmation,
			);
		});
		...
		test('direct JSON file path marks only that file as sensitive', async () => {
			...
			expect(await checker(otherFile)).toBe(
				ConfirmationCheckResult.NoConfirmation,
			);
		});
		...
		test('skips home directory patterns', async () => {
			...
			expect(await checker(fileInWorkspace)).toBe(
				ConfirmationCheckResult.NoConfirmation,
			);
		});
		...
		test('does not treat file extensions like .png as folders', async () => {
			...
			expect(await checker(pngFile)).toBe(
				ConfirmationCheckResult.Sensitive,
			);

			// But a JSON file next to it should not
			const jsonFile = URI.file(
				'/workspace/assets/icon.png/something.json',
			);
			expect(await checker(jsonFile)).toBe(
				ConfirmationCheckResult.NoConfirmation,
			);
		});
		...
		test('pattern starting with **/ is preserved as-is', async () => {
			...
			const hookFile = URI.file(
				'/workspace/deeply/nested/hooks/hook.json',
			);
			expect(await checker(hookFile)).toBe(
				ConfirmationCheckResult.Sensitive,
			);
	...
	test('complex glob patterns', async () => {
		...
		expect(await checker(testEnv)).toBe(
			ConfirmationCheckResult.NoConfirmation,
		); // exception pattern
	...
	describe.skipIf(isWindows)('symlink escape protection', () => {
		...
		beforeEach(() => {
			...
			workspaceDir = fs.realpathSync(
				fs.mkdtempSync(path.join(tmpRoot, 'workspace-')),
			);
		...
		test('non-existent file under symlinked parent resolves through the symlink', async () => {
			...
			const target = URI.file(
				path.join(workspaceDir, 'assets', 'pwned.desktop'),
			);
			expect(await checker(target)).toBe(
				ConfirmationCheckResult.OutsideWorkspace,
			);
		});
		...
		test('non-existent file under symlinked ancestor with non-existent intermediate dirs is still detected', async () => {
			...
			const target = URI.file(
				path.join(workspaceDir, 'assets', 'newdir', 'pwned.desktop'),
			);
			expect(await checker(target)).toBe(
				ConfirmationCheckResult.OutsideWorkspace,
			);
