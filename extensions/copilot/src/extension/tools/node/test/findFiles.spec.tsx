...
suite('FindFiles', () => {
	...
	suite('gpt-4.1 model glob pattern', () => {
		...
		test('adds extra pattern for gpt-4.1 with RelativePattern', async () => {
			...
			const result = await tool.invoke(
				{
					input: { query: `${workspaceFolder}/src` },
					toolInvocationToken: null!,
					model: mockLanguageModelChat,
				},
				CancellationToken.None,
...
suite('FindFiles - absolute workspace folder path', () => {
	...
	beforeEach(() => {
		...
		collection.define(
			IWorkspaceService,
			new SyncDescriptor(TestWorkspaceService, [
				[URI.file(folder1), URI.file(folder2)],
			]),
		);
	});
	...
	test('scopes search to workspace folder when absolute path is used as query', async () => {
		...
		const tool = accessor
			...
			.createInstance(FindFilesTool);
		await tool.invoke(
			{ input: { query: folder1 }, toolInvocationToken: null! },
			CancellationToken.None,
		);

		expect(searchService.lastFilePattern).toHaveLength(1);
		expect(searchService.lastFilePattern![0]).toMatchObject({
			pattern: '',
		});
		expect(
			(searchService.lastFilePattern![0] as RelativePattern).baseUri.path,
		).toBe(URI.file(folder1).path);
	});
	...
	test('absolute path survives resolveInput without corruption', async () => {
		...
		await tool.invoke(
			{ input: resolved, toolInvocationToken: null! },
			CancellationToken.None,
		);
	...
	test('passes caseInsensitive option to findFiles', async () => {
		...
		const tool = accessor
			...
			.createInstance(FindFilesTool);
		await tool.invoke(
			{ input: { query: 'src/**/*.ts' }, toolInvocationToken: null! },
			CancellationToken.None,
		);
...
class TestSearchService extends AbstractSearchService {
	...
	override async findFiles(
		...
		const expected = Array.isArray(this.expectedPattern)
...
class RecordingFindFilesSearchService extends AbstractSearchService {
	...
	override async findFiles(
		...
		this.lastFilePattern = Array.isArray(filePattern)
			? filePattern
			: [filePattern];
