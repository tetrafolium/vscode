...
suite('ReadFile', () => {
	...
	beforeAll(() => {
		...
		const singleLineDoc = createTextDocumentData(
		...
		const longLinesDoc = createTextDocumentData(
			URI.file('/workspace/longlines.ts'),
			longLinesContent,
			'ts',
		).document;
	...
	suite('IReadFileParamsV2', () => {
		...
		test('read file with offset beyond file line count should throw error', async () => {
			...
			await expect(
				...
				'Invalid offset 535: file only has 5 lines. Line numbers are 1-indexed.',
			);
		});
		...
		test('read file with offset beyond single-line file should throw error', async () => {
			...
			await expect(
				...
				'Invalid offset 10: file only has 2 lines. Line numbers are 1-indexed.',
			);
		});
		...
		test('read empty file with offset beyond bounds should throw error', async () => {
			...
			await expect(
				...
				'Invalid offset 2: file only has 1 line. Line numbers are 1-indexed.',
			);
		});
	...
	suite('prepareInvocation', () => {
		test('should return "Reading/Read skill" message for skill files', async () => {
			const testDoc = createTextDocumentData(
				URI.file('/workspace/test.skill.md'),
				'skill content',
				'markdown',
			).document;
			...
			expect((result!.invocationMessage as MarkdownString).value).toBe(
				'Reading skill [workspace](file:///workspace/test.skill.md?vscodeLinkType%3Dskill)',
			);
			expect((result!.pastTenseMessage as MarkdownString).value).toBe(
				'Read skill [workspace](file:///workspace/test.skill.md?vscodeLinkType%3Dskill)',
			);
		...
		test('should return "Reading/Read" message for non-skill files', async () => {
			...
			expect(result).toBeDefined();
			expect((result!.invocationMessage as MarkdownString).value).toBe(
				'Reading [](file:///workspace/test.ts)',
			);
			expect((result!.pastTenseMessage as MarkdownString).value).toBe(
				'Read [](file:///workspace/test.ts)',
			);
		...
		test('should return "Reading skill/Read skill" message for skill files with line range', async () => {
			...
			expect((result!.invocationMessage as MarkdownString).value).toBe(
				'Reading skill [workspace](file:///workspace/test.skill.md?vscodeLinkType%3Dskill#2-2), lines 2 to 4',
			);
			expect((result!.pastTenseMessage as MarkdownString).value).toBe(
				'Read skill [workspace](file:///workspace/test.skill.md?vscodeLinkType%3Dskill#2-2), lines 2 to 4',
			);
		...
		test('should return "Reading/Read skill" message for non-.md skill files', async () => {
			...
			expect(
				(result!.invocationMessage as MarkdownString).value,
			).toContain(
				'Reading skill `workspace`: [](file:///workspace/test.skill)',
			);
			expect(
				(result!.pastTenseMessage as MarkdownString).value,
			).toContain(
				'Read skill `workspace`: [](file:///workspace/test.skill)',
			);
		...
		test('should return "Reading/Read skill" message for non-.md skill files with line range', async () => {
			...
			expect(
				...
				'Reading skill `workspace`: [](file:///workspace/test.skill#2-2), lines 2 to 4',
			);
			expect(
				(result!.pastTenseMessage as MarkdownString).value,
			).toContain(
				'Read skill `workspace`: [](file:///workspace/test.skill#2-2), lines 2 to 4',
			);
		...
		test('should return "Reading/Read" message for non-skill files with line range', async () => {
			...
			expect((result!.invocationMessage as MarkdownString).value).toBe(
				'Reading [](file:///workspace/test.ts#2-2), lines 2 to 4',
			);
			expect((result!.pastTenseMessage as MarkdownString).value).toBe(
				'Read [](file:///workspace/test.ts#2-2), lines 2 to 4',
			);
	...
	suite('image files', () => {
		...
		test('prepareInvocation throws for image files and points to view_image', async () => {
			...
			await expect(
				readFileTool.prepareInvocation(
					{ input },
					CancellationToken.None,
				),
			).rejects.toThrow('Use view_image instead');
	...
	suite('binary files', () => {
		...
		test('does not treat text files as binary', async () => {
			...
			const textDoc = createTextDocumentData(
				textUri,
				'Hello, world!\nThis is text.\n',
				'plaintext',
			).document;
	...
	suite('skill provenance telemetry', () => {
		class CapturingTelemetryService extends NullTelemetryService {
			...
			override sendEnhancedGHTelemetryEvent(
				eventName: string,
				properties?: TelemetryEventProperties,
			): void {
		...
		test('should send separate skillContentRead event with skillStorage=local for workspace skill files', async () => {
			...
			const skillUri = URI.file(
				'/workspace/.github/skills/my-skill/SKILL.md',
			);
			const testDoc = createTextDocumentData(
				skillUri,
				skillContent,
			...
			await readFileTool.invoke(
				{ input, toolInvocationToken: null as never },
				CancellationToken.None,
			);

			const event = telemetry.events.find(
				(e) => e.eventName === 'skillContentRead',
			);
			expect(event).toBeDefined();
			expect(event!.properties!.skillStorage).toBe(
				SkillStorage.Workspace,
			);
			...
			const enhanced = telemetry.enhancedEvents.find(
				(e) => e.eventName === 'skillContentRead',
			);
			...
			expect(enhanced!.properties!.skillStorage).toBe(
				SkillStorage.Workspace,
			);
			expect(enhanced!.properties!.skillContentHash).not.toBe('');

			const internal = telemetry.internalEvents.find(
				(e) => e.eventName === 'skillContentRead',
			);
			...
			expect(internal!.properties!.skillStorage).toBe(
				SkillStorage.Workspace,
			);
		...
		test('should send skillStorage=user for personal skill files', async () => {
			...
			const skillUri = URI.file(
				'/home/testuser/.copilot/skills/personal-skill/SKILL.md',
			);
			const testDoc = createTextDocumentData(
				skillUri,
				skillContent,
		...
		test('should send skillStorage=extension with skillExtensionIdHash and skillExtensionVersion', async () => {
			...
			const skillUri = URI.file(
				'/extensions/publisher.my-ext/skills/ext-skill/SKILL.md',
			);
			const testDoc = createTextDocumentData(
				skillUri,
				skillContent,
			...
			mockCustomInstructions.setExtensionSkillInfos([
				{
					uri: skillUri,
					skillName: 'ext-skill',
					skillFolderUri: URI.file(
						'/extensions/publisher.my-ext/skills/ext-skill',
					),
		...
		test('should not send skillContentRead for non-skill files', async () => {
			...
			const skillEvent = telemetry.events.find(
				(e) => e.eventName === 'skillContentRead',
			);
			expect(skillEvent).toBeUndefined();

			const enhancedSkillEvent = telemetry.enhancedEvents.find(
				(e) => e.eventName === 'skillContentRead',
			);
			expect(enhancedSkillEvent).toBeUndefined();

			const internalSkillEvent = telemetry.internalEvents.find(
				(e) => e.eventName === 'skillContentRead',
			);
			expect(internalSkillEvent).toBeUndefined();

			// readFileToolInvoked should still fire
			const readEvent = telemetry.events.find(
				(e) => e.eventName === 'readFileToolInvoked',
			);
