...
suite('ViewImage', () => {
	test('returns image data for image file', async () => {
		...
		const viewImageTool = accessor
			.get(IInstantiationService)
			.createInstance(ViewImageTool);
		...
		const imagePart = result.content.find(
			(part) => part instanceof LanguageModelDataPart,
		);
		expect(imagePart).toBeDefined();
		expect((imagePart as LanguageModelDataPart).mimeType).toBe(
			'image/jpeg',
		);
	...
	test('returns error for oversized image files', async () => {
		...
		const mockFs = new (class extends MockFileSystemService {
			override async stat(resource: URI) {
				...
				if (
					resource.toString() ===
					URI.file('/workspace/huge.png').toString()
				) {
	...
	test('prepareInvocation returns image-specific messages', async () => {
		...
		expect((result!.invocationMessage as MarkdownString).value).toContain(
			'Viewing image',
		);
		expect((result!.pastTenseMessage as MarkdownString).value).toContain(
			'Viewed image',
		);
	...
	test('throws for non-image files and points to read_file', async () => {
		...
		await expect(
			viewImageTool.prepareInvocation({ input }, CancellationToken.None),
