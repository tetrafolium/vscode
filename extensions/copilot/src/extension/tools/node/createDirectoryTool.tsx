...
export class CreateDirectoryTool implements ICopilotTool<ICreateDirectoryParams> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<ICreateDirectoryParams>,
		token: vscode.CancellationToken,
	) {
		const uri = this.promptPathRepresentationService.resolveFilePath(
			options.input.dirPath,
		);
		if (!uri) {
	...
	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<ICreateDirectoryParams>,
		token: vscode.CancellationToken,
