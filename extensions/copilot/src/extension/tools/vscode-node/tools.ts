...
} from '../common/virtualTools/virtualToolTypes';
...
export class ToolsContribution extends Disposable {
	constructor(
			...
			vscode.commands.registerCommand(
				...
				async () => {
					...
					vscode.window.showInformationMessage(
						l10n.t(
							'Tool groups have been reset. They will be regenerated on the next agent request.',
						),
					);
				},
			),
		...
		this._register(
			vscode.commands.registerCommand(
				...
				async () => {
					...
					if (globalStorageUri) {
						const userMemoryUri = URI.joinPath(
							globalStorageUri,
					...
					if (storageUri) {
						const repoMemoryUri = URI.joinPath(
							storageUri,
					...
					const sessionResource =
						vscode.window.activeChatPanelSessionResource;
					if (storageUri && sessionResource) {
						const sessionId = extractSessionId(
							sessionResource.toString(),
						);
						const sessionMemoryUri = URI.joinPath(
							storageUri,
							'memory-tool/memories',
							sessionId,
						);
					...
					if (selected?.fileUri) {
						await vscode.commands.executeCommand(
							'vscode.open',
							vscode.Uri.from(selected.fileUri),
						);
					}
				},
			),
		...
		this._register(
			vscode.commands.registerCommand(
				...
				(
					...
					if (
						memoryPath.startsWith('/memories/session/') ||
						memoryPath === '/memories/session'
					) {
						const storageUri = this.extensionContext.storageUri;
						if (!storageUri) {
							return undefined;
						...
						memoryPath.startsWith('/memories/repo/') ||
						memoryPath === '/memories/repo'
					) {
						const storageUri = this.extensionContext.storageUri;
						if (!storageUri) {
							return undefined;
		...
		this._register(
			vscode.commands.registerCommand(
				'github.copilot.chat.tools.memory.clearMemories',
				async () => {
					...
					if (hasError) {
						vscode.window.showErrorMessage(
							l10n.t(
								'Some memories could not be cleared. Please try again.',
							),
						);
					} else if (!hasDeleted) {
						vscode.window.showInformationMessage(
						...
						vscode.window.showInformationMessage(
							l10n.t('All memories have been cleared.'),
						);
					}
				},
			),
		...
		this._register(
			autorun((reader) => {
				vscode.commands.executeCommand(
					...
					toolGroupingService.threshold.read(reader),
				);
			}),
		);
	}
