...
class FetchWebPageTool implements ICopilotTool<IFetchWebPageParams> {
	...
	async invoke(
		...
		const encodedUrls = options.input.urls.map(
			(url) => encodeUrlHostname(url).encoded,
		);
		const { content } = await lm.invokeTool(
			internalToolName,
			{ ...options, input: { ...options.input, urls: encodedUrls } },
			token,
		);
		if (encodedUrls.length !== content.length) {
			this._logService.error(
				`Expected ${encodedUrls.length} responses but got ${content.length}`,
			);
		...
		for (let i = 0; i < encodedUrls.length; i++) {
			try {
				...
				if (
					...
					if (typeof textValue === 'string') {
						...
						this._logService.warn(
							`Unsupported content type at index ${i}: ${encodedUrls[i]}`,
						);
				...
				this._logService.error(
					`Invalid URL at index ${i}: ${encodedUrls[i]}`,
					error,
				);
				invalidUrls.push(encodedUrls[i]);
			}
