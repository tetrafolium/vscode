...
import {
	ICopilotTool,
	ICopilotToolExtension,
	modelSpecificToolApplies,
	ToolRegistry,
...
export class ToolsService extends BaseToolsService {
	...
	private readonly _toolExtensions: Lazy<
		Map<ToolName, ICopilotToolExtension<unknown>>
	>;
	...
	get tools(): ReadonlyArray<vscode.LanguageModelToolInformation> {
		...
		const contributedTools = [...input]
			.sort((a, b) => {
				...
				const aIsBuiltin =
					a.name.startsWith('vscode_') ||
					a.name.startsWith('copilot_');
				const bIsBuiltin =
					b.name.startsWith('vscode_') ||
					b.name.startsWith('copilot_');
		...
		const result: vscode.LanguageModelToolInformation[] =
			contributedTools.map((tool) => {
				return {
					...
					inputSchema:
						tool.inputSchema &&
						mapContributedToolNamesInSchema(tool.inputSchema),
				};
			});
	...
	constructor(
		...
		this._copilotTools = new Lazy(
			() =>
				new Map(
					ToolRegistry.getTools().map(
						(t) =>
							[
								t.toolName,
								_instantiationService.createInstance(t),
							] as const,
					),
				),
		);
		this._toolExtensions = new Lazy(
			() =>
				new Map(
					ToolRegistry.getToolExtensions().map(
						(t) =>
							[
								t.toolName,
								_instantiationService.createInstance(t),
				...
				autorunIterableDelta(
					...
					({ addedValues, removedValues }) => {
						for (const { definition } of removedValues) {
							const prev = this._modelSpecificTools.get(
								definition.name,
							);
						...
						for (const { definition, tool } of addedValues) {
							const instance =
								this._instantiationService.createInstance(tool);
							this._modelSpecificTools.set(definition.name, {
								definition,
								tool: instance,
							});
						}
	...
	invokeTool(
		...
		vscode.LanguageModelToolResult | vscode.LanguageModelToolResult2
	> {
		...
		const parentTraceContext = (
			options as {
				parentTraceContext?: { traceId: string; spanId: string };
			}
		).parentTraceContext;
		const span = this._otelService.startSpan(`execute_tool ${name}`, {
			...
			attributes: {
				...
				[GenAiAttr.TOOL_TYPE]: isMcpTool
					? GenAiToolType.EXTENSION
					: GenAiToolType.FUNCTION,
				[GenAiAttr.TOOL_CALL_ID]:
					(options as { chatStreamToolCallId?: string })
		...
		try {
			const { attrs: paramAttrs, gatedAttrs: gatedParamAttrs } =
				extractToolParameters(String(name), options.input);
		...
		const chatStreamToolCallId = (
			options as { chatStreamToolCallId?: string }
		).chatStreamToolCallId;
		const chatRequestId = (options as { chatRequestId?: string })
			.chatRequestId;
		const subAgentInvocationId = (
			options as { subAgentInvocationId?: string }
			...
			if (traceCtx) {
				if (chatStreamToolCallId) {
					this._otelService.storeTraceContext(
						`subagent:toolcall:${chatStreamToolCallId}`,
						traceCtx,
					);
					...
					this._otelService.storeTraceContext(
						`subagent:invocation:${chatStreamToolCallId}`,
						traceCtx,
					);
				}
				...
				if (chatRequestId) {
					this._otelService.storeTraceContext(
						`subagent:request:${chatRequestId}`,
						traceCtx,
					);
				}
			}
		...
		if (ctx) {
			if (!optionsWithTrace.traceparent) {
				...
				const flags = (ctx.traceFlags ?? 0x01)
					.toString(16)
					.padStart(2, '0');
		...
		return vscode.lm
			.invokeTool(getContributedToolName(name), options, token)
			.then(
				(result) => {
					...
					try {
						...
						for (const p of result.content) {
							if (p instanceof vscode.LanguageModelTextPart) {
								...
								p instanceof vscode.LanguageModelPromptTsxPart
							) {
								...
								parts.push(
									`[${p.mimeType}: ${p.data.byteLength} bytes]`,
								);
							}
						}
						if (parts.length > 0) {
					...
					GenAiMetrics.recordToolCallCount(
						this._otelService,
						String(name),
						true,
					);
					GenAiMetrics.recordToolCallDuration(
						this._otelService,
						String(name),
						durationMs,
					);
	...
	override invokeToolWithEndpoint(
		name: string,
		options: vscode.LanguageModelToolInvocationOptions<Object>,
		...
		if (endpoint) {
			...
			for (const [overridesTool] of this.getToolOverridesForEndpoint(
				endpoint,
			)) {
	...
	override getCopilotTool(name: string): ICopilotTool<unknown> | undefined {
		return (
			this._copilotTools.value.get(name as ToolName) ||
			this.getModelSpecificTools().get(name)?.tool
		);
	}
	...
	getEnabledTools(
		...
		const requestToolsByName = new Map(
			Iterable.map(request.tools, ([t, enabled]) => [
				typeof t === 'string' ? t : t.name,
				enabled,
			]),
		);

		const modelSpecificOverrides = new Map(
			this.getToolOverridesForEndpoint(endpoint, tools),
		);
		...
		return tools
			.filter((tool) => {
				...
				if (
					...
					!this._configurationService.getExperimentBasedConfig(
						ConfigKey.EnableGpt55GetChangedFilesTool,
				...
				if (
					...
					!this._configurationService.getExperimentBasedConfig(
						ConfigKey.EnableGemini3GetChangedFilesTool,
						this._experimentationService,
					)
				) {
					return false;
				...
				if (
					...
					isGpt55(endpoint) &&
					!this._configurationService.getExperimentBasedConfig(
						ConfigKey.EnableGpt55ReadFileTool,
						this._experimentationService,
					)
				) {
					return false;
				...
				const toolPickerSelection = requestToolsByName.get(
					getContributedToolName(tool.name),
				);
				...
				for (const ref of request.toolReferences) {
					...
					if (
						usedTool?.tags.includes(
							`enable_other_tool_${tool.name}`,
						)
					) {
						return true;
					}
				...
				if (
					toolPickerSelection === undefined &&
					tool.tags.includes('extension_installed_by_tool')
				) {
					return true;
				}
			...
			.map((tool) => {
				...
				if (extension?.alternativeDefinition) {
					resultTool = extension.alternativeDefinition(
						resultTool,
						endpoint,
					);
				}
	...
	private *getToolOverridesForEndpoint(
		endpoint: IChatEndpoint,
		tools = this.tools,
	) {
		for (const tool of tools) {
			const modelSpecificTool = this.getModelSpecificTools().get(
				tool.name,
			);
			...
			if (modelSpecificTool.tool.overridesTool) {
				yield [
					modelSpecificTool.tool.overridesTool,
					{ info: tool, tool: modelSpecificTool.tool },
				] as const;
			}
		}
	}
