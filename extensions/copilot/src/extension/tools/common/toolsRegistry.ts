...
export function isVscodeLanguageModelTool(
	tool: ICopilotTool<unknown>,
): tool is vscode.LanguageModelTool<unknown> {
...
export const ToolRegistry = new (class {
	...
	private _modelSpecificTools = new ObservableMap<
		string,
		{
			definition: vscode.LanguageModelToolDefinition;
			tool: IModelSpecificToolCtor;
		}
	>();
	...
	public registerModelSpecificTool(
		definition: vscode.LanguageModelToolDefinition,
		tool: IModelSpecificToolCtor,
	): IDisposable {
		if (this._modelSpecificTools.has(definition.name)) {
			throw new Error(
				`Model specific tool for ${definition.name} is already registered`,
			);
		}
...
export function modelSpecificToolApplies(
	tool: vscode.LanguageModelToolDefinition,
	endpoint: IChatEndpoint,
) {
	if (!tool.models) {
		return true;
