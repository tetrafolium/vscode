...
export class VirtualToolGrouper implements IToolCategorization {
	...
	constructor(
		...
		private readonly _toolEmbeddingsComputer: IToolEmbeddingsComputer,
	 ...
	 */
	private shouldTriggerBuiltInGrouping(
		tools: LanguageModelToolInformation[],
	): boolean {
		const defaultToolGroupingEnabled =
			this._configurationService.getExperimentBasedConfig(
				ConfigKey.Advanced.DefaultToolsGrouped,
				this._expService,
			);

		return (
			tools.length > Constant.START_BUILTIN_GROUPING_AFTER_TOOL_COUNT &&
			defaultToolGroupingEnabled
		);
	}
	...
	async addGroups(
		...
		const toolSearchActive = tools.some(
			(t) => t.name === ToolName.ToolSearch,
		);
		...
		const shouldGroup =
			!toolSearchActive && this.shouldTriggerBuiltInGrouping(tools);

		if (
			!shouldGroup &&
			tools.length < Constant.START_GROUPING_AFTER_TOOL_COUNT
		) {
			root.contents = tools;
			return;
		}
		...
		const predictedToolsPromise = this._getPredictedTools(
			query,
			tools,
			token,
		).then((tools) => ({ tools, durationMs: predictedToolsSw.elapsed() }));

		// Separate builtin tools from extension/MCP tools
		const builtinTools =
			byToolset[BuiltInToolGroupHandler.BUILT_IN_GROUP_KEY] || [];
		const toolsetEntries = Object.entries(byToolset)
			.filter(
				([key]) => key !== BuiltInToolGroupHandler.BUILT_IN_GROUP_KEY,
			)
			.filter(
				(entry): entry is [string, LanguageModelToolInformation[]] =>
					entry[1] !== undefined,
			);
		...
		const shouldGroupBuiltin =
			!toolSearchActive &&
			this.shouldTriggerBuiltInGrouping(builtinTools);
		if (shouldGroupBuiltin) {
			const builtinGroups =
				this.builtInToolGroupHandler.createBuiltInToolGroups(
					builtinTools,
				);
		...
		if (toolsetEntries.length > 0) {
			...
			const slotAllocation = this._allocateSlots(
				toolsetEntries,
				availableSlots,
			);

			// Process each toolset individually
			const toolsetGrouped = await Promise.all(
				[...toolsetEntries].map(async ([toolsetKey, tools]) => {
					const allocatedSlots = slotAllocation.get(toolsetKey) || 0;
					return allocatedSlots > 0
						? await this._processToolset(
								tools,
								allocatedSlots,
								token,
							)
		...
		if (toolsetEntries.length > 0) {
			const totalToolsToGroup = toolsetEntries.reduce(
				(sum, [, tools]) => sum + tools.length,
				0,
			);
			const totalGroupsCreated = groupedResults.filter(
				(item) => item instanceof VirtualTool,
			).length;
	...
	async recomputeEmbeddingRankings(
		query: string,
		root: VirtualTool,
		token: CancellationToken,
	): Promise<void> {
		const predictedToolsSw = new StopWatch();
		const actualTools = [...root.all()].filter(
			(t): t is LanguageModelToolInformation =>
				!(t instanceof VirtualTool),
		);
		const matchedTools = this._getPredictedTools(
			query,
			actualTools,
			token,
		).then((tools) => ({
	...
	private _addPredictedToolsGroup(
		root: VirtualTool,
		predictedTools: LanguageModelToolInformation[],
	): void {
		const newGroup = new VirtualTool(
			EMBEDDINGS_GROUP_NAME,
			'Tools with high predicted relevancy for this query',
			Infinity,
			{
		...
		const idx = root.contents.findIndex(
			(t) => t.name === EMBEDDINGS_GROUP_NAME,
		);
		if (idx >= 0) {
	...
	private async _addEmbeddingMatchedTools(
		root: VirtualTool,
		predictedToolsPromise: Promise<{
			tools: LanguageModelToolInformation[];
			durationMs: number;
		}>,
	): Promise<void> {
	...
	public static deduplicateGroups(
		grouped: readonly (VirtualTool | LanguageModelToolInformation)[],
	) {
	 ...
	 */
	private _allocateSlots(
		toolsetEntries: Array<[string, LanguageModelToolInformation[]]>,
		...
		if (remainingSlots > 0) {
			...
			for (const { toolsetKey, proportion } of proportions) {
				...
				allocation.set(
					toolsetKey,
					allocation.get(toolsetKey)! + slotsToAdd,
				);
	...
	private async _processToolset(
		...
		if (allocatedSlots === 1) {
			const groupDescriptions = await this._generateBulkGroupDescriptions(
				[tools],
				token,
			);
			...
			return [
				new VirtualTool(
					VIRTUAL_TOOL_NAME_PREFIX + group.name,
					SUMMARY_PREFIX + group.summary + SUMMARY_SUFFIX,
					0,
					{},
					group.tools,
				),
		...
		return await this._generateEmbeddingBasedGroups(
			tools,
			allocatedSlots,
			token,
		);
	...
	private async _getPredictedTools(
		query: string,
		tools: LanguageModelToolInformation[],
		token: CancellationToken,
		...
		const queryEmbedding = await this.embeddingsComputer.computeEmbeddings(
			EMBEDDING_TYPE_FOR_TOOL_GROUPING,
			[query],
			{},
			new TelemetryCorrelationId(
				'VirtualToolGrouper::_getPredictedTools',
			),
			token,
		);
		...
		const predictedTools = toolEmbeddings
			...
			.filter(
				(
					tool: LanguageModelToolInformation | undefined,
				): tool is LanguageModelToolInformation => tool !== undefined,
			);
	 ...
	 */
	private async _generateEmbeddingBasedGroups(
		tools: LanguageModelToolInformation[],
		limit: number,
		token: CancellationToken,
	): Promise<(VirtualTool | LanguageModelToolInformation)[]> {
		...
		try {
			...
			embeddingGroups =
				await this._toolEmbeddingsComputer.computeToolGroupings(
					tools,
					limit,
					token,
				);
			...
			this._logService.trace(
				`[virtual-tools] Embedding-based grouping created ${embeddingGroups.length} groups from ${tools.length} tools`,
			);
		} catch (e) {
			this._logService.error(
				`Failed to create embedding-based groups: ${e}`,
			);
			// Let the error bubble up as requested - no fallback
			throw e;
		}

		const singles = embeddingGroups
			.filter((g) => g.length === 1)
			.map((g) => g[0]);
		...
		this._logService.trace(
			`[virtual-tools] Embedding-based grouping created ${groupDescriptions.groups.length} groups from ${tools.length} tools`,
		);

		return groupDescriptions.groups
			.map(
				(v): VirtualTool | LanguageModelToolInformation =>
					new VirtualTool(
						VIRTUAL_TOOL_NAME_PREFIX + v.name,
						SUMMARY_PREFIX + v.summary + SUMMARY_SUFFIX,
						0,
						{},
						v.tools,
					),
	 ...
	 */
	protected async _generateBulkGroupDescriptions(
		embeddingGroups: LanguageModelToolInformation[][],
		token: CancellationToken,
	) {
		const cached = await Promise.all(
			embeddingGroups.map((group) => this._cache.getDescription(group)),
		);
		...
		const described = await describeBulkToolGroups(
			endpoint,
			missing.map((m) => m.tools),
			token,
		);
		let missed = 0;
		for (let i = 0; i < described.length; i++) {
			...
			if (d) {
				...
				output.push({
					name: `group_${i}`,
					summary: `Contains the tools: ${m.tools.map((t) => t.name).join(', ')}`,
					tools: m.tools,
				});
			}
		}
