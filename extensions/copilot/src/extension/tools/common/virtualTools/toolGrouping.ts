...
export function computeToolGroupingMinThreshold(
	experimentationService: IExperimentationService,
	configurationService: IConfigurationService,
): IObservable<number> {
	return configurationService
		.getExperimentBasedConfigObservable(
			ConfigKey.VirtualToolThreshold,
...
export class ToolGrouping implements IToolGrouping {
	...
	protected _grouper: IToolCategorization =
		this._instantiationService.createInstance(VirtualToolGrouper);
	...
	didCall(
		...
		if (path.length > 1) {
			// only for tools in groups under the root
			/* __GDPR__
				"virtualTools.called" : {
					"owner": "connor4312",
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent(
				...
				},
				{
					...
					depth: path.length - 1,
					preExpanded: path.every(
						(p) => p.metadata.wasExpandedByDefault,
					)
						? 1
						: 0,
					wasEmbedding: path.some(
						(p) => p.name === EMBEDDINGS_GROUP_NAME,
					)
						? 1
						: 0,
		...
		return new LanguageModelToolResult([
			new LanguageModelTextPart(
				`Tools activated: ${[...tool.tools()].map((t) => t.name).join(', ')}`,
			),
		]);
	}
	...
	private async _doCompute(query: string, token: CancellationToken) {
		...
		if (this._trimOnNextCompute) {
			await this._grouper.recomputeEmbeddingRankings(
				query,
				this._root,
				token,
			);
