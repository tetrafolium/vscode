	...
	implements IExtensionContribution
{
	...
	constructor(@ILogService private readonly logService: ILogService) {
		...
		this._register(
			vscode.commands.registerCommand(
				'github.copilot.debug.togglePowerSaveBlocker',
				async () => {
					const isActive = await this._toggleManualPowerSaveBlocker();
					vscode.window.showInformationMessage(
						isActive
							? 'Power save blocker is now active'
							: 'Power save blocker is now inactive',
					);
				},
			),
		);
		...
		this._register(
			vscode.env.power.onDidSuspend(() => {
				this.logService.debug(
					'[Power] System is suspending (going to sleep)',
				);
			}),
		);

		this._register(
			vscode.env.power.onDidResume(() => {
		...
		this._register(
			vscode.env.power.onDidChangeOnBatteryPower((onBattery) => {
				this.logService.debug(
					`[Power] Battery power state changed: ${onBattery ? 'on battery' : 'on AC power'}`,
				);
			}),
		);
		...
		this._register(
			vscode.env.power.onDidChangeThermalState((thermalState) => {
				this.logService.debug(
					`[Power] Thermal state changed: ${thermalState}`,
				);
			}),
		);
		...
		this._register(
			vscode.env.power.onDidChangeSpeedLimit((speedLimit) => {
				this.logService.debug(
					`[Power] CPU speed limit changed: ${speedLimit}% ${speedLimit < 100 ? '(throttled)' : ''}`,
				);
			}),
		);
		...
		this._register(
			vscode.env.power.onWillShutdown(() => {
				this.logService.debug(
					'[Power] System is about to shut down or reboot',
				);
			}),
		);
		...
		this._register(
			vscode.env.power.onDidLockScreen(() => {
				this.logService.debug('[Power] Screen is being locked');
			}),
		);

		this._register(
			vscode.env.power.onDidUnlockScreen(() => {
	...
	private async logInitialPowerState(): Promise<void> {
		try {
			...
			this.logService.debug(
				`[Power] Initial power state: ${onBattery ? 'on battery' : 'on AC power'}, thermal state: ${thermalState}, system idle time: ${idleTime}s`,
			);
		} catch (error) {
			this.logService.debug(
				`[Power] Failed to retrieve initial power state: ${error}`,
			);
		}
	}
	...
	private async _toggleManualPowerSaveBlocker(): Promise<boolean> {
		if (this._manualBlocker) {
			this.logService.debug(
				`[Power] Stopping manual power save blocker, id: ${this._manualBlocker.id}`,
			);
		...
		try {
			...
			if (typeof vscode.env.power?.startPowerSaveBlocker !== 'function') {
				this.logService.debug(
					'[Power] Power save blocker API not available',
				);
				return false;
			}

			this._manualBlocker = await vscode.env.power.startPowerSaveBlocker(
				'prevent-app-suspension',
			);
			this.logService.debug(
				`[Power] Started manual power save blocker, id: ${this._manualBlocker.id}`,
			);
			return true;
		} catch (err) {
			this.logService.warn(
				`[Power] Failed to start manual power save blocker: ${err}`,
			);
			return false;
		}
	}
