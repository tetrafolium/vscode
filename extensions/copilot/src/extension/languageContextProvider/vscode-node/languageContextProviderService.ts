...
export class LanguageContextProviderService
	extends Disposable
	implements ILanguageContextProviderService
{
	...
	public registerContextProvider<T extends Copilot.SupportedContextItem>(
		provider: Copilot.ContextProvider<T>,
		targets: ProviderTarget[],
	): VscodeDisposable {
		if (targets.length === 0) {
			throw new Error(
				'At least one ProviderTarget must be specified when registering a context provider.',
			);
		}
	...
	public getAllProviders(
		...
		return this.providers
			.filter((p) => target.some((t) => p.targets.includes(t)))
			.map((p) => p.provider);
	}

	public getContextProviders(
		doc: TextDocument,
		target: ProviderTarget,
	): Copilot.ContextProvider<Copilot.SupportedContextItem>[] {
		return this.getAllProviders([target]).filter((provider) =>
			languages.match(provider.selector, doc),
		);
	}
	...
	public getContextItems(
		...
		const providers = this.getContextProviders(
			doc,
			request.source === KnownSources.nes
				? ProviderTarget.NES
				: ProviderTarget.Completions,
		);

		const items = new AsyncIterableObject<Copilot.SupportedContextItem>(
			async (emitter) => {
				async function runProvider(
					provider: Copilot.ContextProvider<Copilot.SupportedContextItem>,
				) {
					const langCtx = provider.resolver.resolve(
						request,
						cancellationToken,
					);
					if (
						typeof (langCtx as any)[Symbol.asyncIterator] ===
						'function'
					) {
		...
		const contextItems = items.map((v) =>
			LanguageContextProviderService.convertCopilotContextItem(v),
		);

		return contextItems;
	}

	private static convertCopilotContextItem(
		item: Copilot.SupportedContextItem,
	): ContextItem {
		const isSnippet =
			item && typeof item === 'object' && (item as any).uri !== undefined;
		if (isSnippet) {
			const ctx = item as Copilot.CodeSnippet;
			return {
				kind: ContextKind.Snippet,
				priority:
					LanguageContextProviderService.convertImportanceToPriority(
						ctx.importance,
					),
				uri: URI.parse(ctx.uri),
				value: ctx.value,
				additionalUris: ctx.additionalUris?.map((uri) =>
					URI.parse(uri),
				),
	...
	private static convertImportanceToPriority(
	...
	public getContextItemsOnTimeout(
		doc: TextDocument,
		request: Copilot.ResolveRequest,
	): ContextItem[] {
		const providers = this.getContextProviders(
		...
		const unprocessedResults = filterMap(providers, (p) =>
			p.resolver.resolveOnTimeout?.(request),
		);

		const copilotCtxItems = unprocessedResults.flat();

		const ctxItems = copilotCtxItems.map((v) =>
			LanguageContextProviderService.convertCopilotContextItem(v),
		);
