...
function apiContentToGeminiContent(
	...
	for (const part of content) {
		if (part instanceof LanguageModelThinkingPart) {
			...
			part instanceof LanguageModelToolResultPart ||
			part instanceof LanguageModelToolResultPart2
		) {
			...
			const functionName =
				part.callId?.split('_')[0] || 'unknown_function';
			...
			if (textContent) {
				...
				try {
					...
					responsePayload = {
						result: textContent + imageDescription,
					};
				}
...
export function apiMessageToGeminiMessage(
...
export function geminiMessagesToRawMessagesForLogging(
	contents: Content[],
	systemInstruction?: Content,
): Raw.ChatMessage[] {
	const fullMessages = geminiMessagesToRawMessages(
		contents,
		systemInstruction,
	);
...
export function geminiMessagesToRawMessages(
	contents: Content[],
	systemInstruction?: Content,
): Raw.ChatMessage[] {
				...
				if (part.text) {
					...
					if (
						...
						if ('images' in cleanResponse) {
							cleanResponse.images = response.images.map(
								(img: any) => ({
						...
						toolContent.push({
							type: Raw.ChatCompletionContentPartKind.Text,
							text: JSON.stringify(cleanResponse),
						});
					} else {
