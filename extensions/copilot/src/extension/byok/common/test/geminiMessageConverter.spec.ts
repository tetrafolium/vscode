...
import {
	...
	LanguageModelTextPart as LMText,
} from '../../../../vscodeTypes';
...
describe('GeminiMessageConverter', () => {
	it('should convert basic user and assistant messages', () => {
		const messages: LanguageModelChatMessage[] = [
			...
			},
			{
				...
				content: [
					new LanguageModelTextPart('I am doing well, thank you!'),
				],
				name: undefined,
			},
		];
		...
		expect(result.contents[1].parts![0].text).toBe(
		...
		const messages: LanguageModelChatMessage[] = [
			{
				...
				content: [
					new LanguageModelTextPart('You are a helpful assistant.'),
				],
				name: undefined,
			},
			{
	...
	it('should extract functionResponse parts from model message into subsequent user message and prune empty model', () => {
		...
		const toolResult = new LanguageModelToolResultPart('myTool_12345', [
			new LanguageModelTextPart('{"foo":"bar"}'),
		]);
		const messages: LanguageModelChatMessage[] = [
			{
				role: LanguageModelChatMessageRole.Assistant,
	...
	it('should wrap array responses in an object', () => {
		const toolResult = new LanguageModelToolResultPart('listRepos_12345', [
			new LanguageModelTextPart('["repo1", "repo2"]'),
		]);
		const messages: LanguageModelChatMessage[] = [
			{
				role: LanguageModelChatMessageRole.Assistant,
		...
		expect(fr.functionResponse.response).toEqual({
			result: ['repo1', 'repo2'],
		});
	});

	it('should be idempotent when called multiple times (no duplication)', () => {
		const toolResult = new LanguageModelToolResultPart('doThing_12345', [
			new LMText('{"value":42}'),
		]);
		const messages: LanguageModelChatMessage[] = [
			{
				role: LanguageModelChatMessageRole.Assistant,
				content: [new LMText('Result:'), toolResult],
				name: undefined,
			},
		];
	...
	describe('Image handling', () => {
		it('should handle LanguageModelDataPart as inline image data', () => {
			...
			expect(inlineData.inlineData.data).toBe(
		...
		it('should filter out StatefulMarker and CacheControl data parts', () => {
			...
			const validImage = new LanguageModelDataPart(
				imageData,
				'image/jpeg',
			);
			const statefulMarker = new LanguageModelDataPart(
				new Uint8Array([1, 2, 3]),
				CustomDataPartMimeTypes.StatefulMarker,
			);
			const cacheControl = new LanguageModelDataPart(
				new TextEncoder().encode('ephemeral'),
				CustomDataPartMimeTypes.CacheControl,
			);
		...
		it('should handle images in tool result content with text', () => {
			...
			expect(fr.functionResponse.response.images[0].mimeType).toBe(
				'image/jpeg',
			);
			expect(fr.functionResponse.response.images[0].size).toBe(
				imageData.length,
			);
		});
		...
		it('should handle images in tool result content without text', () => {
			...
			const imageData2 = new Uint8Array([
				137, 80, 78, 71, 13, 10, 26, 10,
			]); // PNG header
			const imagePart1 = new LanguageModelDataPart(
				imageData1,
				'image/jpeg',
			);
			const imagePart2 = new LanguageModelDataPart(
				imageData2,
				'image/png',
			);

			const toolResult = new LanguageModelToolResultPart(
				'generateImages_12345',
				[imagePart1 as any, imagePart2 as any],
			);
			const messages: LanguageModelChatMessage[] = [
				{
					role: LanguageModelChatMessageRole.Assistant,
			...
			expect(fr.functionResponse.response.images[0].data).toBe(
				Buffer.from(imageData1).toString('base64'),
			);

			// Second image
			expect(fr.functionResponse.response.images[1].mimeType).toBe(
				'image/png',
			);
			expect(fr.functionResponse.response.images[1].size).toBe(
				imageData2.length,
			);
			expect(fr.functionResponse.response.images[1].data).toBe(
				Buffer.from(imageData2).toString('base64'),
			);
		});
		...
		it('should handle mixed text and filtered data parts in tool results', () => {
			...
			const toolResult = new LanguageModelToolResultPart(
				'mixedContent_12345',
				[textPart, validImage as any, statefulMarker as any],
			);
			const messages: LanguageModelChatMessage[] = [
				{
					role: LanguageModelChatMessageRole.Assistant,
			...
			expect(fr.functionResponse.response.result).toContain(
				'Result text',
			);
			expect(fr.functionResponse.response.result).toContain(
				'[Contains 1 image(s) with types: image/jpeg]',
			);
			expect(fr.functionResponse.response.images).toHaveLength(1);
			expect(fr.functionResponse.response.images[0].mimeType).toBe(
				'image/jpeg',
	...
	describe('geminiMessagesToRawMessages', () => {
		it('should convert function response with images to Raw format with image content parts', async () => {
			const { geminiMessagesToRawMessages } =
				await import('../geminiMessageConverter');
