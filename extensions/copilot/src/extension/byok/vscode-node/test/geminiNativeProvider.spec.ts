...
vi.mock('@google/genai', () => {
	class MockGoogleGenAI {
		...
		public static listModelsResult: AsyncIterable<any> =
			(async function* () {})();
...
vi.mock('../../common/byokProvider', async (importOriginal) => {
	const actual =
		await importOriginal<typeof import('../../common/byokProvider')>();
	return {
		...actual,
...
function createRequestLogger(): IRequestLogger {
	...
	return {
		...
		captureInvocation: async <T>(
			_request: CapturingToken,
			fn: () => Promise<T>,
		) => fn(),
...
describe('GeminiNativeBYOKLMProvider', () => {
	...
	it.skip('throws a clear error when no API key is configured (no silent return)', async () => {
		const { GeminiNativeBYOKLMProvider } =
			await import('../geminiNativeProvider');
	...
	it.skip('prompts for a new API key when listing models fails with an invalid key', async () => {
		...
		const MockGoogleGenAI = genai.GoogleGenAI as unknown as {
			listModelsResult: AsyncIterable<any>;
		};
		...
		MockGoogleGenAI.listModelsResult = (async function* () {
			throw new Error(
				'ApiError: {"error":{"message":"API key not valid. Please pass a valid API key.","details":[{"reason":"API_KEY_INVALID"}]}}',
			);
		})();
		...
		mockHandleAPIKeyUpdate.mockResolvedValue({
			apiKey: undefined,
			deleted: false,
			cancelled: true,
		});
