...
describe('buildOTelInputFromChatMessages', () => {
		...
		const messages: LanguageModelChatMessage[] = [
			{
				...
				content: [new LanguageModelTextPart('You are helpful')],
				name: undefined,
			},
			{
	...
	it('preserves order of multiple system messages', () => {
		const messages: LanguageModelChatMessage[] = [
			{
				role: LanguageModelChatMessageRole.System,
				content: [new LanguageModelTextPart('personality')],
				name: undefined,
			},
			{
				role: LanguageModelChatMessageRole.System,
				content: [new LanguageModelTextPart('instructions')],
				name: undefined,
			},
			{
				role: LanguageModelChatMessageRole.User,
		...
		const messages: LanguageModelChatMessage[] = [
			{
				...
				content: [
					new LanguageModelToolCallPart('call_1', 'search', {
						q: 'vscode',
					}),
				],
				name: undefined,
	...
	it('falls back to a placeholder when no parts are recognized', () => {
		...
		expect(inputMsgs).toEqual([
			{
				role: 'user',
				parts: [{ type: 'text', content: '[non-text content]' }],
			},
		]);
	});
	...
	it('returns empty results for an empty messages array', () => {
		expect(buildOTelInputFromChatMessages([])).toEqual({
			systemTexts: [],
			inputMsgs: [],
		});
	});
