...
} from '../common/anthropicMessageConverter';
import {
	BYOKKnownModels,
...
import {
	...
	ExtendedLanguageModelChatInformation,
...
export class AnthropicLMProvider extends AbstractLanguageModelChatProvider {
	...
	constructor(
		...
		private readonly _toolDeferralService: IToolDeferralService,
	) {
		super(
	...
	protected async getAllModels(
		silent: boolean,
		apiKey: string | undefined,
	): Promise<
		ExtendedLanguageModelChatInformation<LanguageModelChatConfiguration>[]
	> {
		if (!apiKey && silent) {
			return [];
		}
		...
		try {
			...
			this._logService.error(
				error,
				`Error fetching available ${AnthropicLMProvider.providerName} models`,
			);
	...
	async provideLanguageModelChatResponse(
		...
		const doRequest = async () => {
			...
			const pendingLoggedChatRequest = this._requestLogger.logChatRequest(
				...
				{
					model: model.id,
					messages: anthropicMessagesToRawMessagesForLogging(
						convertedMessages,
						system,
					),
			...
			const toolSearchEnabled =
				...
				!!options.tools?.some(
					(t) => t.name === CUSTOM_TOOL_SEARCH_NAME,
				);
			...
			for (const tool of options.tools ?? []) {
				...
				const shouldDefer = toolSearchEnabled
					? !this._toolDeferralService.isNonDeferredTool(tool.name)
					: undefined;
			...
			if (
				webSearchEnabled &&
				!tools.some(
					(tool) => 'name' in tool && tool.name === 'web_search',
				)
			) {
				const maxUses = this._configurationService.getConfig(
					ConfigKey.AnthropicWebSearchMaxUses,
				);
				const allowedDomains = this._configurationService.getConfig(
					ConfigKey.AnthropicWebSearchAllowedDomains,
				);
				const blockedDomains = this._configurationService.getConfig(
					ConfigKey.AnthropicWebSearchBlockedDomains,
				);
				const userLocation = this._configurationService.getConfig(
					ConfigKey.AnthropicWebSearchUserLocation,
				);
				const shouldDeferWebSearch = toolSearchEnabled
					? !this._toolDeferralService.isNonDeferredTool('web_search')
					: undefined;
				...
				const webSearchTool: Anthropic.Beta.BetaWebSearchTool20250305 =
					{
						...
						max_uses: maxUses,
						...(shouldDeferWebSearch
							? { defer_loading: shouldDeferWebSearch }
							: {}),
					};
				...
				if (hasAllowed && hasBlocked) {
					webSearchDomainConflictError = vscode.l10n.t(
			...
			const supportsAdaptiveThinking =
				modelCapabilities?.adaptiveThinking ?? false;

			// Build context management configuration
			const thinkingEnabled =
				supportsAdaptiveThinking || (thinkingBudget ?? 0) > 0;
			const contextManagement = isAnthropicContextEditingEnabled(
				model.id,
			...
			const effort =
				...
				supportsEffort.includes(rawEffort)
			...
			const params: Anthropic.Beta.Messages.MessageCreateParamsStreaming =
				{
					...
					context_management: contextManagement as
			...
			try {
				...
				const responseDeltas: IResponseDelta[] =
					wrappedProgress.items.map((i): IResponseDelta => {
						if (i instanceof LanguageModelTextPart) {
							...
							const resultText = i.content
								.map((c) =>
									c instanceof LanguageModelTextPart
										? c.value
										: '',
								)
								.join('');
				...
				if (otelSpan && result.usage) {
					...
					if (this._otelService.config.captureContent) {
						...
						const toolCalls = wrappedProgress.items
							.filter(
								(p): p is LanguageModelToolCallPart =>
									p instanceof LanguageModelToolCallPart,
							)
							.map((tc) => ({
				...
				if (result.usage) {
					...
					const metricAttrs = {
						...
						providerName: AnthropicLMProvider.providerId,
				...
				*/
				...
				this._logService.error(
					`BYOK Anthropic error: ${toErrorMessage(err, true)}`,
				);
				pendingLoggedChatRequest.resolve(
					{
		...
		const executeRequest = async () => {
			...
			if (this._otelService.config.captureContent) {
				...
				try {
					const { systemTexts, inputMsgs } =
						buildOTelInputFromChatMessages(messages);
		...
		if (parentTraceContext) {
			return this._otelService.runWithTraceContext(
				parentTraceContext,
				executeRequest,
			);
		}
	...
	private async _makeRequest(
		...
		progress: RecordedProgress<LMResponsePart>,
		...
		ttfte: number | undefined;
		usage: APIUsage | undefined;
		contextManagement: ContextManagementResponse | undefined;
	}> {
		...
		for await (const chunk of stream) {
			...
			if (chunk.type === 'content_block_start') {
				if (
					...
					const resultBlock =
						chunk.content_block as Anthropic.Messages.WebSearchToolResultBlock;
					...
					const results = resultBlock.content.map(
						(result: Anthropic.Messages.WebSearchResultBlock) => ({
			...
			if (chunk.type === 'content_block_delta') {
				if (chunk.delta.type === 'text_delta') {
					progress.report(
						new LanguageModelTextPart(chunk.delta.text || ''),
					);
					...
					if ('citation' in chunk.delta) {
						...
						if (citation.type === 'web_search_result_location') {
							...
							progress.report(
								new LanguageModelTextPart(referenceText),
							);

							// Store the citation data in the correct format for multi-turn conversations
							progress.report(
								new LanguageModelToolResultPart('citation', [
			...
			if (chunk.type === 'message_start') {
				...
				if (chunk.usage.output_tokens) {
					...
					usage.total_tokens =
						usage.prompt_tokens + chunk.usage.output_tokens;
				}
				// Handle context management response
				if ('context_management' in chunk && chunk.context_management) {
					contextManagementResponse =
						chunk.context_management as ContextManagementResponse;
					const totalClearedTokens =
						contextManagementResponse.applied_edits.reduce(
							(sum, edit) =>
								sum + (edit.cleared_input_tokens || 0),
							0,
						);
					this._logService.info(
						`BYOK Anthropic context editing applied: cleared ${totalClearedTokens} tokens across ${contextManagementResponse.applied_edits.length} edits`,
					);
					// Emit context management via LanguageModelDataPart so it flows through to toolCallingLoop
					progress.report(
						new LanguageModelDataPart(
							new TextEncoder().encode(
								JSON.stringify(contextManagementResponse),
							),
