...
> implements LanguageModelChatProvider<T> {
	...
	protected async configureDefaultGroupWithApiKeyOnly(): Promise<
		string | undefined
	> {
		...
		if (apiKey) {
			...
			await this._byokStorageService.deleteAPIKey(
				this._name,
	...
	protected async configureDefaultGroupIfExists(
		name: string,
		configuration: C,
	): Promise<void> {
	...
	protected abstract getAllModels(
		silent: boolean,
		apiKey: string | undefined,
		configuration: C | undefined,
	): Promise<T[]>;
}

export interface OpenAICompatibleLanguageModelChatInformation<
	C extends LanguageModelChatConfiguration,
> extends ExtendedLanguageModelChatInformation<C> {
	url: string;
}

export abstract class AbstractOpenAICompatibleLMProvider<
	T extends LanguageModelChatConfiguration = LanguageModelChatConfiguration,
> extends AbstractLanguageModelChatProvider<
	T,
	OpenAICompatibleLanguageModelChatInformation<T>
> {
	...
	constructor(
		...
		protected readonly _expService: IExperimentationService,
	) {
	...
	): Promise<OpenAICompatibleLanguageModelChatInformation<T>[]> {
		const modelsUrl = this.getModelsBaseUrl(configuration);
		if (modelsUrl) {
			const models = await this.getModelsFromEndpoint(
				modelsUrl,
				silent,
				apiKey,
			);
			return byokKnownModelsToAPIInfoWithEffort(this._name, models).map(
				(model) => ({
					...model,
					url: modelsUrl,
				}),
	...
	private async getModelsFromEndpoint(
		endpoint: string,
		silent: boolean,
		apiKey: string | undefined,
	): Promise<BYOKKnownModels> {
		if (!apiKey && silent) {
			return {};
		}
		...
		try {
			...
			this._logService.error(
				error,
				`Error fetching available OpenRouter models`,
			);
			throw error;
		}
	}

	protected async createOpenAIEndPoint(
		model: OpenAICompatibleLanguageModelChatInformation<T>,
	): Promise<OpenAIEndpoint> {
		const modelInfo = this.getModelInfo(model.id, model.url);
		const url = modelInfo.supported_endpoints?.includes(
	...
	protected abstract getModelsBaseUrl(
		configuration: T | undefined,
	): string | undefined;
