 ...
 */
export function byokKnownModelToAPIInfoWithEffort(
	providerName: string,
	id: string,
	...
	return {
		...
		configurationSchema: {
			properties: {
				reasoningEffort: buildReasoningEffortSchemaProperty(
					effortLevels,
					model.family,
				),
			},
...
export function byokKnownModelsToAPIInfoWithEffort(
	...
	return Object.entries(knownModels).map(([id, capabilities]) =>
		byokKnownModelToAPIInfoWithEffort(providerName, id, capabilities),
	);
}
