...
import {
	toGeminiFunction as toGeminiFunctionDeclaration,
	ToolJsonSchema,
} from '../common/geminiFunctionDeclarationConverter';
import {
	apiMessageToGeminiMessage,
	geminiMessagesToRawMessagesForLogging,
} from '../common/geminiMessageConverter';
import {
...
export class GeminiNativeBYOKLMProvider extends AbstractLanguageModelChatProvider {
	...
	> {
		...
		try {
			...
			this._logService.error(
				error,
				`Error fetching available ${GeminiNativeBYOKLMProvider.providerName} models`,
			);
			throw error;
		}
	}
	...
	async provideLanguageModelChatResponse(
		...
		const doRequest = async () => {
			...
			const { contents, systemInstruction } = apiMessageToGeminiMessage(
			...
			const pendingLoggedChatRequest = this._requestLogger.logChatRequest(
				...
				{
					model: model.id,
					messages: geminiMessagesToRawMessagesForLogging(
						contents,
						systemInstruction,
					),
									...
									(tool) => {
										...
										const finalTool =
											toGeminiFunctionDeclaration(
												tool.name,
												tool.description,
												tool.inputSchema as ToolJsonSchema,
											);
										finalTool.description =
											tool.description ||
											finalTool.description;
										return finalTool;
									},
			...
			const cancelSub = token.onCancellationRequested(() => {
				...
				this._logService.trace(
					'Gemini request aborted via VS Code cancellation token',
				);
			});
			...
			try {
				...
				pendingLoggedChatRequest.resolve(
					...
					wrappedProgress.items.map((i): IResponseDelta => {
						return {
							text:
								i instanceof LanguageModelTextPart
									? i.value
									: '',
							copilotToolCalls:
								i instanceof LanguageModelToolCallPart
									? [
											{
												name: i.name,
				...
				*/
				...
				this._logService.error(
					`BYOK GeminiNative error: ${toErrorMessage(err, true)}`,
				);
				pendingLoggedChatRequest.resolve(
					{
	...
	private async _makeRequest(
		...
		try {
					...
					if (candidate.content && candidate.content.parts) {
						for (const part of candidate.content.parts) {
							...
							if (
								...
								progress.report(
									new LanguageModelThinkingPart(part.text),
								);
							} else if (part.text) {
								if (ttfte === undefined) {
									ttfte = Date.now() - issuedTime;
								...
								progress.report(
									new LanguageModelTextPart(part.text),
								);
							} else if (
								part.functionCall &&
				...
				if (chunk.usageMetadata) {
					...
					if (!usage) {
						...
						usage = {
							...
							total_tokens:
								chunk.usageMetadata.totalTokenCount ?? -1,
			...
			this._logService.error(
				`Gemini streaming error: ${toErrorMessage(error, true)}`,
			);
			throw error;
		}
	}
