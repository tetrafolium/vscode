...
describe('nes-datagen pipeline e2e', () => {
	describe('full pipeline run', () => {
		...
		test('output samples have correct metadata', () => {
			const tsSample = result.samples.find(
				(s) => s.metadata.language === 'typescript',
			);
			const pySample = result.samples.find(
				(s) => s.metadata.language === 'python',
			);
		...
		test('assistant response contains a diff patch for TypeScript sample', () => {
			...
			const assistantMsg = tsSample.messages.find(
				(m) => m.role === 'assistant',
			)!;
	...
	describe('expected prompt and response files', () => {
		...
		test('TypeScript sample prompt matches expected file', async () => {
			...
			const userPrompt = tsSample.messages.find(
				(m) => m.role === 'user',
			)!.content;
			await expect(userPrompt).toMatchFileSnapshot(
				snapshotPath('ts-user-prompt.txt'),
			);
		});
		...
		test('TypeScript sample system prompt matches expected file', async () => {
			...
			const systemPrompt = tsSample.messages.find(
				(m) => m.role === 'system',
			)!.content;
			await expect(systemPrompt).toMatchFileSnapshot(
				snapshotPath('ts-system-prompt.txt'),
			);
		});
		...
		test('TypeScript sample response matches expected file', async () => {
			...
			const response = tsSample.messages.find(
				(m) => m.role === 'assistant',
			)!.content;
			await expect(response).toMatchFileSnapshot(
				snapshotPath('ts-assistant-response.txt'),
			);
		});
		...
		test('Python sample prompt matches expected file', async () => {
			...
			const userPrompt = pySample.messages.find(
				(m) => m.role === 'user',
			)!.content;
			await expect(userPrompt).toMatchFileSnapshot(
				snapshotPath('py-user-prompt.txt'),
			);
		});
		...
		test('Python sample system prompt matches expected file', async () => {
			...
			const systemPrompt = pySample.messages.find(
				(m) => m.role === 'system',
			)!.content;
			await expect(systemPrompt).toMatchFileSnapshot(
				snapshotPath('py-system-prompt.txt'),
			);
		});
		...
		test('Python sample response matches expected file', async () => {
			...
			const response = pySample.messages.find(
				(m) => m.role === 'assistant',
			)!.content;
			await expect(response).toMatchFileSnapshot(
				snapshotPath('py-assistant-response.txt'),
			);
		});
	});
	...
	describe('pipeline with only invalid input', () => {
		...
		beforeAll(async () => {
			...
			await fs.writeFile(
				invalidInputPath,
				JSON.stringify([fixtures.invalid.record]),
			);
		});
