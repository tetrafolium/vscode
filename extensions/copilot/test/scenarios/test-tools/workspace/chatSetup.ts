...
const defaultChat = {
	...
	alternativeProviderId:
		product.defaultChatAgent?.alternativeProviderId ?? '',
	alternativeProviderName:
		product.defaultChatAgent?.alternativeProviderName ?? '',
...
class ChatSetup {
	...
	private getButtons(): Array<
		[string, ChatSetupStrategy, { renderAsLink: boolean } | undefined]
	> {
		if (this.context.state.entitlement === ChatEntitlement.Unknown) {
			const supportAlternateProvider =
				this.configurationService.getValue(
					'chat.setup.signInWithAlternateProvider',
				) === true && defaultChat.alternativeProviderId;
			...
			if (
				...
				return coalesce([
					...
					supportAlternateProvider
						? [
								localize(
									'continueWithProvider',
									'Continue with {0}',
									defaultChat.alternativeProviderName,
								),
