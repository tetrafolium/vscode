...
ssuite({ title: 'semanticSearchView', location: 'panel' }, (inputPath) => {
	...
	for (const scenario of scenarios) {
		for (const testCase of scenario) {
			stest(
				{ description: 'Semantic search view: ' + testCase.question },
				async (testingServiceCollection: TestingServiceCollection) => {
					const workspaceState = testCase.getState
						? testCase.getState()
						: undefined;
					...
					await provider.provideAITextSearchResults(
						question,
						{
							folderOptions: [],
							maxResults: 1000,
					...
					if (testCase.json.keywords) {
						for (const r of results) {
							const matched = testCase.json.keywords.some(
								(keyword: string) =>
									r.previewText.includes(keyword),
							);
							if (matched) {
								return { success: true };
							}
