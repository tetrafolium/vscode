...
(function () {
	ssuite.skip(
		{ title: 'newWorkspace', subtitle: 'e2e', location: 'panel' },
		(inputPath) => {
			const scenarioFolder =
				inputPath ??
				path.join(__dirname, '..', 'test/scenarios/test-new-workspace');
			const scenarios: Scenario[] =
				fetchConversationScenarios(scenarioFolder);

			for (const scenario of scenarios) {
				stest(
					{ description: scenario[0].question.replace('/new ', '') },
						...
						async (
							...
							for (const command of commands) {
								if (
									...
									const projectItems: ChatResponseFileTreePart =
										command.arguments?.[0];
									if (
										!projectItems ||
									...
									async function traverseFileTree(
										...
										if (
											...
											!looksLikeDirectory(fileTree.name)
										) {
											files.push({
												kind: 'qualifiedFile',
												uri: Uri.joinPath(
											...
											if (!decodedString) {
												return {
													success: false,
													errorMessage: `Content not found for ${itemPath}`,
												};
											}
											files.push({
												kind: 'qualifiedFile',
							...
							return Promise.resolve({
								success: false,
								errorMessage: 'Failed to parse new response',
							});
						},
					),
				);
...
async function compileWorkspace(
	...
	for (const file of files) {
		...
		switch (language.languageId) {
			case 'typescript': {
				// compute diagnostics
				const tsDiagnostics = await compileTSWorkspace(
					accessor,
					projectRoot,
				);
				...
				return {
					success: false,
					errorMessage: 'Typescript diagnostics errors: \n' + errors,
				};
			}
		}
	}
