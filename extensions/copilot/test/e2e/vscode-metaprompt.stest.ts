...
ssuite.skip(
	{ title: 'vscode', subtitle: 'metaprompt', location: 'panel' },
	async (_) => {
		const scenarios = [
			{
		...
		for (const scenario of scenarios) {
			stest(
				...
				async (testingServiceCollection) => {
					...
					const vscodePrompt = instantiationService.createInstance(
						VscodePrompt,
						{
							promptContext: {
					...
					const countTokens = (text: string) =>
						tokenizer.tokenLength(text);

					const prompt = await vscodePrompt.prepare(
						{ tokenBudget: 2048, endpoint, countTokens },
						undefined,
					...
					for (const keyword of scenario.keywords) {
						assert.ok(
							...
							`${keyword} not found in prompt query`,
						);
					}
					for (const keyword of scenario.excludedKeywords) {
						assert.ok(
							!prompt.query
								.toLowerCase()
								.includes(keyword.toLowerCase()),
							`prompt query should not include "${keyword}"`,
						);
					}
				},
			);
