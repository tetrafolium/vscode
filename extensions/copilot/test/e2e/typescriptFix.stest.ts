...
(function () {
	ssuite(
		{ title: 'fix', subtitle: 'typescript', location: 'panel' },
		(inputPath) => {
			const scenarioFolder =
				...
				path.join(
					__dirname,
					'..',
					'test/scenarios/test-scenario-fix-typescript',
				);
			...
			for (let i = 0; i < scenarios.length; i++) {
				...
				stest(
					{ description: scenario.name, language: 'typescript' },
						...
						async (accessor, question, response) => {
							...
							if (
								...
								return {
									success: false,
									errorMessage:
										'No typescript code block found in response',
								};
							}
							const contentInOutput = (
								scenario.json as IExtendedTestData
							).contentInOutput;
							if (!codeBlock.code.includes(contentInOutput)) {
								return {
									success: false,
									errorMessage: `Typescript code block does not contain ${contentInOutput}`,
								};
							}
							return { success: true };
						},
