...
ssuite.optional(
	shouldSkipAgentTests,
	{ title: 'fetchWebPageTool', subtitle: 'toolCalling', location: 'panel' },
	() => {
		const scenarioFolder = path.join(
			__dirname,
			'..',
			'test/scenarios/test-tools',
		);
		const getState = () =>
			deserializeWorkbenchState(
				scenarioFolder,
				path.join(scenarioFolder, 'tools.state.json'),
			);

		stest(
			'proper URL validation and query handling',
			generateToolTestRunner(
				{
					question:
						'fetch information about React hooks from https://react.dev/reference/react',
					scenarioFolderPath: '',
					getState,
					...
					toolCallValidators: {
						[ToolName.FetchWebPage]: async (toolCalls) => {
							assert.strictEqual(
								toolCalls.length,
								1,
								'should make exactly one fetch webpage tool call',
							);
							const input = toolCalls[0]
								.input as IFetchWebPageToolParams;
							...
							assert.strictEqual(
								input.urls[0],
								expectedUrl,
								`should have the exact URL: ${expectedUrl}`,
							);
							...
							if (input.query !== undefined) {
								assert.ok(
									...
									'query should be a string if provided',
								);
								assert.ok(
									input.query.length > 0,
									'query should not be empty if provided',
								);
							}
						},
					},
		...
		stest(
			...
			generateToolTestRunner(
				{
					question:
						'get content from https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-0.html and https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function about async/await',
					scenarioFolderPath: '',
					getState,
					expectedToolCalls: ToolName.FetchWebPage,
					tools: {
				...
				},
				{
					...
					toolCallValidators: {
						[ToolName.FetchWebPage]: (toolCalls) => {
							const expectedTypescriptUrl =
								'https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-0.html';
							const expectedMdnUrl =
								'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function';
							const expectedUrls = [
								expectedTypescriptUrl,
								expectedMdnUrl,
							];
							...
							assert.ok(
								toolCalls.length === 1 ||
									toolCalls.length === 2,
								'should make either 1 or 2 fetch webpage tool calls',
							);
							...
							if (toolCalls.length === 1) {
								...
								assert.ok(
									input.urls.includes(expectedTypescriptUrl),
									`should include the TypeScript URL: ${expectedTypescriptUrl}`,
								);
								assert.ok(
									input.urls.includes(expectedMdnUrl),
									`should include the MDN URL: ${expectedMdnUrl}`,
								);
								...
								toolCalls.forEach((toolCall) => {
									const input =
										toolCall.input as IFetchWebPageToolParams;
									assert.ok(
										Array.isArray(input.urls),
										'urls should be an array',
									);
									assert.strictEqual(
										input.urls.length,
										1,
										'each tool call should have exactly 1 URL',
									);
								...
								assert.ok(
									allUrls.includes(expectedTypescriptUrl),
									`should include the TypeScript URL: ${expectedTypescriptUrl}`,
								);
								assert.ok(
									allUrls.includes(expectedMdnUrl),
									`should include the MDN URL: ${expectedMdnUrl}`,
								);
		...
		stest(
			...
			generateToolTestRunner(
				{
					question:
						'find specific information about error handling patterns from https://nodejs.org/en/docs/guides/error-handling/',
					scenarioFolderPath: '',
					getState,
					expectedToolCalls: ToolName.FetchWebPage,
					tools: {
					...
					toolCallValidators: {
						[ToolName.FetchWebPage]: async (toolCalls) => {
							...
							assert.ok(
								...
								'should include a query when user asks for specific information',
							);
							assert.ok(
								typeof input.query === 'string',
		...
		stest(
			'multiple URLs boundary test with 6 URLs',
			generateToolTestRunner(
				{
					question:
						'gather information from these documentation sources: https://react.dev/learn/hooks-overview, https://vuejs.org/guide/essentials/reactivity-fundamentals.html, https://angular.io/guide/component-interaction, https://svelte.dev/docs/introduction, https://solid-js.com/guides/getting-started, and https://lit.dev/docs/ about component state management',
					scenarioFolderPath: '',
					getState,
					expectedToolCalls: ToolName.FetchWebPage,
					tools: {
				...
				},
				{
					...
					toolCallValidators: {
						[ToolName.FetchWebPage]: (toolCalls) => {
							...
							assert.ok(
								toolCalls.length >= 1 && toolCalls.length <= 6,
								`should make between 1 and 6 fetch webpage tool calls, but got ${toolCalls.length}`,
							);
							...
							toolCalls.forEach((toolCall, index) => {
								...
								assert.ok(
									Array.isArray(input.urls),
									`tool call ${index + 1}: urls should be an array`,
								);
								assert.ok(
									input.urls.length >= 1,
									`tool call ${index + 1}: should have at least 1 URL`,
								);
							...
							assert.strictEqual(
								allUrls.length,
								6,
								'collected URLs array should have exactly 6 URLs',
							);

							// Check that all expected URLs are present
							expectedUrls.forEach((expectedUrl) => {
								assert.ok(
									allUrls.includes(expectedUrl),
									`should include the URL: ${expectedUrl}`,
								);
							});
							...
							toolCalls.forEach((toolCall, index) => {
								...
								assert.ok(
									input.query,
									`tool call ${index + 1}: query should be defined if provided`,
								);
							});
						},
					},
