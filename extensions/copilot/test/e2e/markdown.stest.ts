...
(function () {
	ssuite(
		{ title: 'generate', subtitle: 'markdown', location: 'panel' },
		(inputPath) => {
			const scenarioFolder =
				...
				path.join(
					__dirname,
					'..',
					'test/scenarios/test-generate-markdown',
				);
			...
			for (let i = 0; i < scenarios.length; i++) {
				...
				stest(
					{ description: scenario.name, language: 'markdown' },
						...
						async (accessor, question, response) => {
							...
							if (
								...
								return {
									success: false,
									errorMessage:
										'No markdown code block found in response',
								};
							}
							if (codeBlock.startMarkup !== '````') {
								return {
									success: false,
									errorMessage: 'Did not use 4 backticks',
								};
							}
							return { success: true };
						},
