...
ssuite.optional(
	...
	(inputPath) => {
		const scenarioFolder =
			inputPath ??
			path.join(__dirname, '..', 'test/scenarios/test-notebook-tools');
		const getState = () =>
			deserializeWorkbenchState(
				scenarioFolder,
				path.join(scenarioFolder, 'Chipotle1.state.json'),
			);
				...
				},
				{
					...
					toolCallValidators: {
						[ToolName.RunNotebookCell]: async (toolCalls) => {
							...
							const solutionNotebook =
								state.notebookDocuments.find(
									(doc) =>
										doc.uri.path === activeDoc.uri.path,
								)!;
							const codeCellIds = solutionNotebook
								?.getCells()
								.filter((c) => c.kind === 2)
								.map((c) => getCellId(c));
							toolCalls.forEach((toolCall) => {
								const cellId = (
									toolCall.input as { cellId: string }
								).cellId;
								assert.ok(
									codeCellIds.includes(cellId),
									`Cell ${cellId} should be found in the notebook`,
								);
							});
						},
						...
						[ToolName.EditNotebook]: async () => {
							throw new Error(
								'EditNotebook should not be called',
							);
						},
					},
				},
		...
		stest(
			'New Notebook Tool with EditFile and EditNotebook',
			generateToolTestRunner(
				{
					scenarioFolderPath: scenarioFolder,
		...
		stest(
			'New Notebook Tool without EditFile and without EditNotebook',
			generateToolTestRunner(
				{
					scenarioFolderPath: scenarioFolder,
					question: `Create a new Jupyter Notebook using ${ContributedToolName.CreateNewJupyterNotebook} with 1 cell to that adds number 1 and 2.`,
					expectedToolCalls: {
						anyOf: [ToolName.CreateNewJupyterNotebook],
					},
					getState,
					tools: {
						[ContributedToolName.CreateNewJupyterNotebook]: true,
		...
		stest(
			'New Notebook Tool without EditFile and with EditNotebook',
			generateToolTestRunner(
				{
					scenarioFolderPath: scenarioFolder,
					question: `Create a new Jupyter Notebook using ${ContributedToolName.CreateNewJupyterNotebook} with 1 cell to that adds number 1 and 2.`,
		...
		stest(
			'Run cell tool should avoid running markdown cells',
			generateToolTestRunner(
				{
					scenarioFolderPath: scenarioFolder,
				...
				},
				{
					...
					toolCallValidators: {
						[ToolName.RunNotebookCell]: async (toolCalls) => {
							...
							const first3CodeCells = solutionNotebook
								?.getCells()
								.filter((c) => c.kind === 2)
								.map((c) => getCellId(c))
								.slice(0, 3);
							toolCalls.forEach((toolCall) => {
								...
								assert.ok(
									first3CodeCells.includes(cellId),
									`Cell ${cellId} was not one of the first three code cells`,
								);
							});
						},
						[ToolName.GetNotebookSummary]: async () => {
					...
					toolCallValidators: {
						[ToolName.RunNotebookCell]: async (toolCalls) => {
							...
							assert.equal(
								thirdCell?.kind,
								2,
								'Invalid test: The third cell should be a code cell',
							);
							toolCalls.forEach((toolCall) => {
								const cellId = (
									toolCall.input as { cellId: string }
								...
								assert.ok(
									thirdCell &&
										getCellId(thirdCell) === cellId,
									`Cell ${cellId} should be the third code cell`,
								);
							});
						},
						[ToolName.GetNotebookSummary]: async () => {
			...
			generateToolTestRunner(
				{
					scenarioFolderPath: scenarioFolder,
					question:
						'Change the header in the first markdown cell to "Hello Chipotle"',
					...
					toolCallValidators: {
						[ToolName.RunNotebookCell]: async (toolCalls) => {
							...
							toolCalls.forEach((toolCall) => {
								...
								const firstMarkdownCell = solutionNotebook
									?.getCells()
									.find((c) => c.kind === 1)!;
								assert.equal(
									getCellId(firstMarkdownCell),
									cellId,
								);

								const newCode = (
									toolCall.input as { newCode: string[] }
								).newCode;
								assert.notDeepEqual(
									newCode.indexOf('# Hello Chipotle'),
									-1,
									'The first markdown cell should be changed to "Hello Chipotle"',
								);
							});
						},
						[ToolName.GetNotebookSummary]: async () => {
