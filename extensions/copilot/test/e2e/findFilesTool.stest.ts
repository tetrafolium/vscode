...
ssuite.optional(
	...
	{ title: 'findFilesTool', subtitle: 'toolCalling', location: 'panel' },
	() => {
						...
						[ToolName.FindFiles]: async (toolCalls) => {
							if (toolCalls.length === 1) {
								const input = toolCalls[0]
									.input as IFindFilesToolParams;
								assert.ok(
									input.query.includes('**/'),
									'should match **/',
								);
								assert.ok(
									input.query.includes('tsx') &&
										input.query.includes('jsx'),
									'should match *.tsx and *.jsx',
								);
							} else if (toolCalls.length === 2) {
								const input1 = toolCalls[0]
									.input as IFindFilesToolParams;
								const input2 = toolCalls[1]
									.input as IFindFilesToolParams;
								...
								assert.ok(
									queries.includes('.tsx') &&
										queries.includes('.jsx'),
									'should match *.tsx and *.jsx',
								);
							} else {
