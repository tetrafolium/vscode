...
(function () {
	ssuite.skip(
		{ title: 'newNotebook', subtitle: 'e2e', location: 'panel' },
		(inputPath) => {
			//@Yoyokrazy, refactor for tool - debt

			const scenarioFolder =
				inputPath ??
				path.join(__dirname, '..', 'test/scenarios/test-new-notebooks');
			...
			for (const scenario of scenarios) {
				stest(
					{
						description: scenario[0].question.replace(
							'/newNotebook ',
							'',
						),
						language: 'python',
					...
					generateScenarioTestRunner(
						...
						(
							...
							return Promise.resolve({
								success: false,
								errorMessage:
									'Fail to parse newNotebook response',
							});
						},
					),
				);
