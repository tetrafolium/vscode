...
import {
	emptyWorkspaceInfo,
	IWorkspaceInfo,
} from '../../src/extension/chatSessions/common/workspaceInfo';
...
} from '../../src/extension/chatSessions/copilotcli/common/delegationSummaryService';
import {
	CopilotCLIAgents,
	CopilotCLIModels,
	CopilotCLISDK,
	...
	ICopilotCLIModels,
	ICopilotCLISDK,
} from '../../src/extension/chatSessions/copilotcli/node/copilotCli';
import {
	CopilotCLIImageSupport,
	ICopilotCLIImageSupport,
} from '../../src/extension/chatSessions/copilotcli/node/copilotCLIImageSupport';
...
} from '../../src/extension/chatSessions/copilotcli/node/copilotcliSessionService';
import {
	CopilotCLISkills,
	ICopilotCLISkills,
} from '../../src/extension/chatSessions/copilotcli/node/copilotCLISkills';
import {
	CopilotCLIMCPHandler,
	ICopilotCLIMCPHandler,
} from '../../src/extension/chatSessions/copilotcli/node/mcpHandler';
import {
	IQuestion,
	IQuestionAnswer,
	IUserQuestionHandler,
} from '../../src/extension/chatSessions/copilotcli/node/userInputHelpers';
import {
	IPromptVariablesService,
	NullPromptVariablesService,
} from '../../src/extension/prompt/node/promptVariablesService';
import { ChatSummarizerProvider } from '../../src/extension/prompt/node/summarizer';
import {
	MockChatResponseStream,
	TestChatRequest,
} from '../../src/extension/test/node/testHelpers';
...
} from '../../src/platform/chat/common/chatDebugFileLoggerService';
...
import {
	IMcpService,
	NullMcpService,
} from '../../src/platform/mcp/common/mcpService';
...
const permissionConfirmationInvocations: Array<{
	name: string;
	input: unknown;
}> = [];
...
class TestCopilotCLIToolsService extends TestToolsService {
	override async invokeTool(
		...
		if (
			...
			permissionConfirmationInvocations.push({
				name,
				input: options.input,
			});
			return new LanguageModelToolResult2([
				new LanguageModelTextPart('yes'),
			]);
		}
...
function getGitHubTokenFromEnv(): string {
	...
	if (!token) {
		throw new Error(
			'GITHUB_OAUTH_TOKEN is not set. Add it to the .env file at the repo root (it is loaded by dotenv in test/simulationMain.ts).',
		);
	}
	return token;
}
...
async function registerChatServices(
	...
	class TestCopilotCLISessionService extends CopilotCLISessionService {
		...
		protected override async createSessionsOptions(options: {
			model?: string;
			...
			const sessionOptions = await super.createSessionsOptions({
				...options,
				agent: undefined,
			});
	...
	class TestCopilotCLISDK extends CopilotCLISDK {
		...
		override async getAuthInfo(): Promise<
	...
	const delegatingSummarizerProvider = instaService.createInstance(
		ChatDelegationSummaryService,
		summarizer,
	);
	testingServiceCollection.define(
		ICopilotCLISkills,
		new SyncDescriptor(CopilotCLISkills),
	);
	testingServiceCollection.define(
		ICopilotCLISessionService,
		new SyncDescriptor(TestCopilotCLISessionService),
	);
	testingServiceCollection.define(
		ICopilotCLIModels,
		new SyncDescriptor(CopilotCLIModels),
	);
	testingServiceCollection.define(
		ICopilotCLISDK,
		new SyncDescriptor(TestCopilotCLISDK),
	);
	testingServiceCollection.define(
		ICopilotCLIAgents,
		new SyncDescriptor(CopilotCLIAgents),
	);
	testingServiceCollection.define(
		ICustomSessionTitleService,
		new SyncDescriptor(TestCustomSessionTitleService),
	);
	testingServiceCollection.define(
		ICopilotCLIMCPHandler,
	...
	testingServiceCollection.define(
		IMcpService,
		new SyncDescriptor(NullMcpService),
	);
	testingServiceCollection.define(
		IFileSystemService,
		new SyncDescriptor(NodeFileSystemService),
	);
	testingServiceCollection.define(
		ICopilotCLIImageSupport,
	...
	testingServiceCollection.define(
		IToolsService,
		new SyncDescriptor(TestCopilotCLIToolsService, [new Set()]),
	);
	testingServiceCollection.define(
		IUserQuestionHandler,
		new SyncDescriptor(UserQuestionHandler),
	);
	testingServiceCollection.define(
		IChatDelegationSummaryService,
		delegatingSummarizerProvider,
	);
	testingServiceCollection.define(
		IChatSessionMetadataStore,
		new SyncDescriptor(MockChatSessionMetadataStore),
	);
	testingServiceCollection.define(IAgentSessionsWorkspace, {
	...
	testingServiceCollection.define(
		IPromptVariablesService,
		new SyncDescriptor(NullPromptVariablesService),
	);
	testingServiceCollection.define(
		IPromptsService,
		new SyncDescriptor(MockPromptsService),
	);
	testingServiceCollection.define(
		IChatDebugFileLoggerService,
	...
	const promptResolver = instaService.createInstance(
	...
	return {
		sessionService: copilotCLISessionService,
		promptResolver,
		init: async (workingDirectory: URI) => {
...
function testRunner(
	cb: (
		services: {
			...
			init: (workingDirectory: URI) => Promise<void>;
			authInfo: NonNullable<SessionOptions['authInfo']>;
		},
		scenariosPath: string,
		toolInvocations: ChatToolInvocationPart[],
	...
	return async (testingServiceCollection: TestingServiceCollection) => {
		...
		const scenariosPath = path.join(
			tmpdir() + tmpDirCounter++,
			'vscode-copilot-chat',
			'test-cli',
		);
		await fs
			.rm(scenariosPath, { recursive: true, force: true })
			.catch(() => {
				/* Ignore */
			});
		await fs.mkdir(scenariosPath, { recursive: true });
		await fs.cp(sourcePath, scenariosPath, {
			recursive: true,
			force: true,
			errorOnExist: false,
		});
		...
		try {
			const services = await registerChatServices(
			...
			await cb(
				services,
				await fs.realpath(scenariosPath),
				toolInvocations,
				stream,
				disposables,
			);
		} finally {
			await fs.rm(scenariosPath, { recursive: true }).catch(() => {
				/* Ignore */
			});
			disposables.dispose();
		}
...
function assertStreamContains(
	...
	assert.ok(
		output.includes(expectedContent),
		message ??
			`Expected response to include "${expectedContent}", actual output: ${output}`,
	);
}
...
function assertNoErrorsInStream(stream: MockChatResponseStream) {
	...
	assert.ok(
		!output.includes('❌'),
		`Expected no errors in stream, actual output: ${output}`,
	);
	assert.ok(
...
async function assertFileContains(
	...
	assert.ok(
		fileContent.includes(expectedContent),
		`Expected to contain "${expectedContent}", contents = ${fileContent}`,
	);
	if (typeof exactCount === 'number') {
		const actualCount = Array.from(
			fileContent.matchAll(new RegExp(expectedContent, 'g')),
		).length;
		assert.strictEqual(
			actualCount,
			exactCount,
			`Expected to find "${expectedContent}" exactly ${exactCount} times, but found ${actualCount} times in contents = ${fileContent}`,
		);
	}
}
...
async function assertFileNotContains(
	...
	assert.ok(
		!fileContent.includes(expectedContent),
		`Expected not to contain "${expectedContent}", contents = ${fileContent}`,
	);
}

ssuite.skip({ title: '@cli', location: 'external' }, async (_) => {
	stest(
		{ description: 'can start a session' },
		testRunner(
			async (
				{ sessionService, init, authInfo },
				scenariosPath,
				toolInvocations,
				stream,
				disposables,
				...
				const workingDirectory = URI.file(
					path.join(scenariosPath, 'wkspc1'),
				);
				await init(workingDirectory);
				const session = await sessionService.createSession(
					sessionOptionsFor(workingDirectory),
	...
	stest(
		{ description: 'can resume a session' },
		testRunner(
			async (
				{ sessionService, init, authInfo },
				scenariosPath,
	...
	stest(
		{ description: 'can read file without permission' },
		testRunner(
			async (
				{ sessionService, init, authInfo },
				scenariosPath,
	...
	stest(
		{
			description:
				'request permission when reading file outside workspace',
		},
		testRunner(
			async (
				{ sessionService, init, authInfo },
				...
				assert.ok(
					permissionConfirmationInvocations.length > 0,
					'Expected permission to be requested for external file, output:' +
						streamOutput,
				);
			},
		),
	...
	stest(
		{ description: 'can read attachment without permission' },
		testRunner(
			async (
				{ sessionService, promptResolver, init, authInfo },
				scenariosPath,
				toolInvocations,
				stream,
				disposables,
				...
				const { prompt, attachments } =
					await resolvePromptWithFileReferences(
		...
		testRunner(
			async (
				...
				await assertFileContains(
					URI.joinPath(workingDirectory, 'math.js').fsPath,
					'function',
					1,
				);
			},
	...
	stest(
		{ description: 'can list files in directory' },
		testRunner(
			async (
				{ sessionService, promptResolver, init, authInfo },
				scenariosPath,
		...
		testRunner(
			async (
				...
				assert.ok(
					...
					`} has not been added, contents = ${contents}`,
				);
			},
		),
	);
	...
	stest(
		{ description: 'can fix multiple problems in multiple files' },
		testRunner(
			async (
				{ sessionService, promptResolver, init, authInfo },
				scenariosPath,
				...
				await init(workingDirectory);
				const tsFile = URI.joinPath(
					workingDirectory,
					'stringUtils.js',
				).fsPath;
				const tsDiag = new Diagnostic(
					new Range(7, 0, 7, 1),
					'} expected',
					DiagnosticSeverity.Error,
				);
				const pyFile = URI.joinPath(workingDirectory, 'demo.py').fsPath;
				const pyDiag1 = new Diagnostic(
					new Range(3, 21, 3, 21),
					"Expected ':', found new line",
					DiagnosticSeverity.Error,
				);
				const pyDiag2 = new Diagnostic(
					new Range(19, 13, 19, 13),
				...
				const { prompt, attachments } =
					await resolvePromptWithFileReferences(
						`Fix the problem`,
						[
							createDiagnosticReference(tsFile, [tsDiag]),
							createDiagnosticReference(pyFile, [
								pyDiag1,
								pyDiag2,
							]),
						],
				...
				assert.ok(
					tsContents.trim().endsWith('}'),
					`} has not been added, contents = ${tsContents}`,
				);
	...
	stest(
		...
		testRunner(
			async (
				...
				assert.ok(
					permissionConfirmationInvocations.some(
						(invocation) =>
							invocation.name ===
...
function createWithRequestWithFileReference(
	prompt: string,
	filesOrReferences: (string | ChatPromptReference)[],
): ChatRequest {
...
function createFileSelectionReference(
	file: string,
	range: Range,
): ChatPromptReference {
...
function resolvePromptWithFileReferences(
	prompt: string,
	filesOrReferences: (string | ChatPromptReference)[],
	promptResolver: CopilotCLIPromptResolver,
): Promise<{ prompt: string; attachments: any[] }> {
