...
export function fetchConversationScenarios(
	...
	for (const testFile of testFiles) {
		const fileContents = fs.readFileSync(
			path.join(scenarioFolderPath, testFile),
			'utf8',
		);
		...
		assert(
			...
			'Expected an array of objects with a question property',
		);
		assert(
			parsedFile.every(
				(testCase: any) =>
					!testCase.stateFile ||
					typeof testCase.stateFile === 'string',
			),
			'Expected an array of objects with a stateFile property of type string',
		);
		assert(
			parsedFile.every(
				(testCase: any) =>
					!testCase.applyChatCodeBlocks ||
					typeof testCase.applyChatCodeBlocks === 'boolean',
			),
			'Expected an array of objects with a applyChatCodeBlocks property of type boolean',
		);
		const scenario: Scenario = [];
		for (const testCase of parsedFile) {
			scenario.push({
				question: testCase.question,
				name: createTestNameFromPath(
					path.basename(scenarioFolderPath),
					testFile,
				),
				json: testCase,
				scenarioFolderPath,
...
export function discoverScenarios(rootFolder: string): Scenario[] {
	const rootFolderContents = fs.readdirSync(rootFolder, {
		withFileTypes: true,
	});
...
function fetchToolCallScenarios(
	scenarioFolderPath: string,
): IToolCallScenarioTestCase[] {
	const testFiles = fs
		.readdirSync(scenarioFolderPath)
		.filter((f) => f.endsWith('.toolcall.json'));
	...
	for (const testFile of testFiles) {
		...
		const testCase: IToolCallScenarioJson = jsoncParser.parse(
			fileContents,
		) as IToolCallScenarioJson;
		assert(
			testCase instanceof Object,
			'Expected an object with toolArgs property',
		);
		assert(
			typeof testCase.toolArgs === 'object',
			'Expected toolArgs to be an object',
		);
		assert(
		...
		assert(
			testCase.toolArgs.tool,
			'Expected toolArgs to have a tool property',
		);
		assert(
			typeof testCase.toolArgs.tool === 'string',
			'Expected toolArgs.tool to be a string',
		);
		let stateFile = testCase.stateFile;
		if (testCase.stateFile) {
			assert(
				typeof testCase.stateFile === 'string',
				'Expected stateFile to be a string',
			);
...
export function discoverToolsCalls(
	rootFolder: string,
): IToolCallScenarioTestCase[] {
	const rootFolderContents = fs.readdirSync(rootFolder, {
		withFileTypes: true,
	});
	const containsToolArgsFile = rootFolderContents.some(
		(f) => f.isFile() && f.name.endsWith('.toolcall.json'),
	);
