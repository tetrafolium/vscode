...
type TerminalTestCaseShellSpecificAnswer =
	...
	Partial<Record<SupportedShellType, TerminalTestCaseSingleAnswer>> &
		...
		Partial<
			Record<`!${SupportedShellType}`, TerminalTestCaseSingleAnswer>
		> &
		(
			| {
...
interface ITerminalTestCase {
	...
	bestAnswer:
		...
		| TerminalTestCaseShellSpecificAnswer;
	/**
...
function getShellSpecificAnswer(
	answerObject:
		| TerminalTestCaseSingleAnswer
		| TerminalTestCaseShellSpecificAnswer
		| Partial<TerminalTestCaseShellSpecificAnswer>,
	...
	for (const negatedShellType in supportedShells) {
		...
		if (`!${negatedShellType}` in answerObject) {
			answer.push(
				...answerObject[`!${negatedShellType as SupportedShellType}`]!,
			);
		}
	}
	...
	generalTestCases.push(
		...([
			...
			},
			{
				...
				bestAnswer: {
					powershell: ['Get-ChildItem', /Get-ChildItem -Path .\\?/],
					any: ['ls'],
				},
				acceptableAnswers: {
			...
			{
				shellType,
				question: 'create a file called foo',
				bestAnswer: {
					powershell: [/New-Item( -ItemType File)? -Name "?foo"?/],
					default: ['touch foo'],
				},
			},
			{
				shellType,
				question: 'delete the foo.txt file',
				bestAnswer: {
					powershell: [/Remove-Item (\.[\\/])?foo.txt/],
					default: ['rm foo.txt'],
				},
			},
			{
				shellType,
				question: 'delete the foo/ dir',
				bestAnswer: {
					powershell: [
			...
			},
			{
				...
				bestAnswer: {
					...
					default: [/echo "[hH]ello [wW]orld"/],
				},
			},
			{
				shellType,
				question: 'kill the process using port 8123',
				bestAnswer: {
					powershell: [
			...
			{
				shellType,
				question: 'kill process using port',
				bestAnswer: {
					powershell: [
			...
			{
				shellType,
				question: 'extract a tar file',
				bestAnswer: [/tar -xv?f {.+}/],
			},
			{
				shellType,
				question: 'extract foo.tar',
				bestAnswer: {
					powershell: [
						/Expand-Archive( -Path)? ['"]?foo.tar['"]? -DestinationPath ['"]?(\.|{.+})[\\\/]?['"]?/,
					],
					default: [/tar -xv?f foo.tar/],
				},
				acceptableAnswers: {
					powershell: [/tar -xv?f foo.tar/],
				},
			},
			{
				shellType,
				question: 'extract a zip file',
				bestAnswer: {
					powershell: [
			...
			{
				shellType,
				question: 'extract foo.zip',
				bestAnswer: {
					powershell: [
						/Expand-Archive( -Path)? (\.[\\/])?foo.zip -DestinationPath (\.|{.+})[\\\/]?/,
					],
					default: ['unzip foo.zip'],
				},
			},
			{
				shellType,
				question: 'extract foo.tar to bar/',
				bestAnswer: {
					powershell: [
						/Expand-Archive( -Path)? foo.tar -DestinationPath bar/,
					],
					default: [/tar -xv?f foo.tar -C bar\//],
				},
				acceptableAnswers: {
					powershell: ['tar -xf foo.tar -C bar/'],
				},
			},
			{
				shellType,
				question: 'make a directory',
				bestAnswer: {
					powershell: [
			...
			{
				shellType,
				question: 'make a directory called foo',
				bestAnswer: {
					powershell: [
				...
				acceptableAnswers: {
					powershell: [/mkdir foo/],
				},
			},
			{
				shellType,
				question: 'copy file foo to bar/',
				bestAnswer: {
					powershell: [
			...
			{
				shellType,
				question: 'kill the visual studio code process',
				bestAnswer: {
					powershell: [/Stop-Process -Name "?[cC]ode"?/],
					fish: [
						/pkill( -f)? "?code"?/,
						'kill (pidof code)',
						/kill \(pgrep [cC]ode\)/,
			...
			{
				shellType,
				question: 'how do i download a file using curl',
				bestAnswer: [/curl -O {.+}/, /curl -o {.+} {.+}/],
			},
		...
		...[
			{
				...
				acceptableAnswers: ['git show --stat', 'git log -1'],
			},
			{
				shellType,
					...
					[
						{
							...
							getState: () =>
								deserializeWorkbenchState(
									...
									join(
										scenarioFolder,
										`${testCase.shellType ?? 'bash'}.state.json`,
									),
								),
						},
					],
...
function generateEvaluate(
	testCase: ITerminalTestCase,
	options: IGenerateEvaluateOptions = {},
): ScenarioEvaluator {
	return async function evaluate(
		...
		if (options.strict) {
			...
			const bestAnswer = getShellSpecificAnswer(
				testCase.bestAnswer,
				testCase.shellType,
			);
			ok(
			...
			if (testCase.acceptableAnswers) {
				acceptableAnswers.push(
					...getShellSpecificAnswer(
						testCase.acceptableAnswers,
						testCase.shellType,
					),
				);
			}
