...
export function generateScenarioTestRunner(
	scenario: Scenario,
	...
	return async function (testingServiceCollection) {
		...
		try {
			...
			const simulationWorkspace = disposables.add(
				isInExtensionHost
			...
			const log = (message: string, err?: any) =>
				testContext.log(message, err);

			const history: (ChatRequestTurn | ChatResponseTurn)[] = [];
			for (let i = 0; i < scenario.length; i++) {
				const testCase = scenario[i];
				simulationWorkspace.resetFromDeserializedWorkspaceState(
					testCase.getState?.(),
				);
				...
				const parsedQuery = await parseQueryForScenarioTest(
					accessor,
					testCase,
					simulationWorkspace,
				);
				...
				const request: ChatRequest = {
					prompt: parsedQuery.query,
					references: parsedQuery.variables,
					...
					toolReferences: parsedQuery.toolReferences,
				...
				if (testCase.tools) {
					for (const [toolName, shouldUse] of Object.entries(
						testCase.tools,
					)) {
						request.tools.set(
							{
								name: getContributedToolName(toolName),
							} as LanguageModelToolInformation,
							shouldUse,
						);
					}
				}
				const interactiveSession = accessor
						...
						CancellationToken.None,
						{
							...
							intentId:
								...
								parsedQuery.command
									...
									: parsedQuery.command
										? agentsToCommands[
												parsedQuery.participantName as Intent
											]![parsedQuery.command]
										: parsedQuery.participantName,
						},
						() => false,
				...
				history.push(
					new ChatRequestTurn(
						...
						request.command,
						[...request.references],
						getChatParticipantIdFromName(participantId),
						[],
					),
				);
				history.push(
					new ChatResponseTurn(
						mockProgressReporter.items.filter(
							(x) =>
								x instanceof ChatResponseMarkdownPart ||
								x instanceof ChatResponseAnchorPart,
						),
						result,
						participantId,
						request.command,
				...
				if (scenario[i].applyChatCodeBlocks) {
					const codeBlocks =
						turn?.getMetadata(CodeBlocksMetadata)?.codeBlocks ?? [];
					...
					if (codeBlocks.length !== 0) {
						...
						let codeBlockApplyErrorDetails:
							| ChatErrorDetails
							| undefined = undefined;
						for (const codeBlock of codeBlocks) {
							const prevDocument =
								simulationWorkspace.activeTextEditor?.document!;
							// Set the active document if the code resource has a uri
							if (codeBlock.resource) {
								simulationWorkspace.setCurrentDocument(
									codeBlock.resource,
								);
							}
							...
							const documentContext =
								IDocumentContext.fromEditor(editor);
							const workspacePath =
								simulationWorkspace.getFilePath(document.uri);

							const previousTextContent = document.getText();
							const response: MappedEditsResponseStream = {
								textEdit(target, edits) {
									simulationWorkspace.applyEdits(
										target,
										Array.isArray(edits) ? edits : [edits],
									);
								},
								notebookEdit(target, edits) {
									simulationWorkspace.applyNotebookEdits(
										target,
										Array.isArray(edits) ? edits : [edits],
									);
								},
							...
							const postEditTextContent =
								editor.document.getText();
							if (previousTextContent !== postEditTextContent) {
								const previousChange =
									changedDocs.get(workspacePath);
								if (previousChange) {
									previousChange.postContent =
										postEditTextContent;
									changedDocs.set(
										workspacePath,
										previousChange,
									...
									changedDocs.set(workspacePath, {
										document,
										originalContent: previousTextContent,
										postContent: postEditTextContent,
									});
								}
							}
						...
						if (
							...
							testRuntime.setOutcome({
								...
								hitContentFilter:
									codeBlockApplyErrorDetails.responseIsFiltered ??
									false,
								critical: false,
							});
						}
				...
				assert.ok(
					evaluatedResponse.success,
					evaluatedResponse.errorMessage,
				);
			}
		} finally {
			disposables.dispose();
...
export function shouldSkip(scenario: Scenario): boolean {
	...
	try {
		return (
			!workspaceFolderPath ||
			fs.readdirSync(workspaceFolderPath).length === 0
		);
	} catch (e) {
		return true;
	}
