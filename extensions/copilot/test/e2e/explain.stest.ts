...
ssuite(
	{ title: 'explain', subtitle: 'expanded context', location: 'panel' },
	(inputPath) => {
		const scenarioFolder =
			inputPath ??
			path.join(__dirname, '..', 'test/scenarios/test-explain');
		...
		for (const scenario of scenarios) {
			...
			stest(
				{
					description:
						scenario[0].json.description ??
						scenario[0].question.replace('/explain', ''),
					language: language
				...
				generateScenarioTestRunner(
					...
					async (accessor, question, answer) => {
						...
						const containsSpeculation = validate(answer, [
							{ not: [{ anyOf: ['presumably', 'unclear'] }] },
						]);
						if (containsSpeculation) {
							return {
								success: false,
								errorMessage:
									'Answer contained speculation about current selection',
							};
						}
