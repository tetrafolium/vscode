...
ssuite(
	{ title: 'system', subtitle: 'identity', location: 'panel' },
	(inputPath) => {
		const scenarioFolder =
			inputPath ??
			path.join(__dirname, '..', 'test/scenarios/test-system');
		const scenarios = fetchConversationScenarios(scenarioFolder);

		for (const scenario of scenarios) {
			stest(
				{ description: scenario[0].question, language: undefined },
				generateScenarioTestRunner(
					scenario,
					async (accessor, question, answer) => {
						const expectedKeywords = scenario.find(
