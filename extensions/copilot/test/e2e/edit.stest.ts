...
ssuite.optional(
	...
	{ title: 'edit', subtitle: 'toolCalling', location: 'panel' },
	() => {
		...
		const getState = () =>
			deserializeWorkbenchState(
				scenarioFolder,
				path.join(scenarioFolder, 'chatSetup.state.json'),
			);
		...
		stest(
			...
			generateToolTestRunner(
				{
					question:
						'This code fails because whenLanguageModelReady waits for any model, not the correct model. From doInvokeWithoutSetup, wait for the model with id IChatAgentRequest.userSelectedModelId to be registered',
					scenarioFolderPath: '',
					getState,
					tools: {},
				},
					...
					toolCallValidators: {
						[ToolName.ReadFile]: (toolCalls) => {
							assert.ok(
								!toolCalls.some((tc) =>
									(
										tc.input as IReadFileParamsV1
									).filePath.endsWith('chatSetup.ts'),
								),
								'Should not read_file the attached file',
							);
						},
						[ToolName.Codebase]: (toolCalls) => {
							assert.ok(
								!toolCalls.some((tc) => {
									const query = (
										tc.input as ICodebaseToolParams
									).query;
									return (
										query.includes(
											'doForwardRequestToCopilotWhenReady',
										) ||
										query.includes('whenLanguageModelReady')
									);
								}),
