...
export function assertLooksLikeJSDoc(text: string): void {
	...
	assert(
		...
		`expected jsdoc, but got:\n${text}`,
	);
}

export function assertContainsAllSnippets(
	text: string,
	snippets: string[],
	dbgMsg: string = '',
) {
	for (let i = 0, len = snippets.length; i < len; ++i) {
		...
		assert(
			text.indexOf(snippet) !== -1,
			`${dbgMsg} (contains snippet "${snippet}")`,
		);
	}
}
 ...
 */
export function findTextBetweenMarkersFromTop(
	text: string,
	marker1: string,
	marker2: string,
): string | null {
 ...
 */
export function findTextBetweenMarkersFromBottom(
	text: string,
	marker1: string,
	marker2: string,
) {
 ...
 */
export async function assertNoDiagnosticsAsync(
 ...
 * This method validates the outcome by finding if after the edit, there remain errors
 */
export async function assertNoSyntacticDiagnosticsAsync(
	accessor: ITestingServicesAccessor,
	outcome: IOutcome,
	workspace: SimulationWorkspace,
	method: DiagnosticProviderId | DiagnosticsProvider,
	...
	const syntacticDiagnostics = diagnostics.filter(
		(d) => d.kind === 'syntactic',
	);
	...
	assert.strictEqual(
		...
		0,
		JSON.stringify(syntacticDiagnostics, undefined, 2),
	);
}
 ...
 */
export async function assertLessDiagnosticsAsync(
	accessor: ITestingServicesAccessor,
	outcome: IOutcome,
	workspace: SimulationWorkspace,
	method: DiagnosticProviderId,
) {
	assert.strictEqual(outcome.type, 'inlineEdit');
...
export function assertFileContent(
	...
	assert.fail(
		`Expected to find file ${fileName}. Files available: ${existing.join(', ')}`,
	);
}
...
export function getFileContent(
	...
	if ('srcUri' in file && 'post' in file) {
		...
		'kind' in file &&
		(file.kind === 'relativeFile' || file.kind === 'qualifiedFile')
	) {
		// Old format
		return file.fileContents;
	} else {
...
export function assertNoElidedCodeComments(
	outcome: IInlineEditOutcome | IWorkspaceEditOutcome | string,
): void {
	if (typeof outcome === 'string') {
		assert.ok(
			outcome.indexOf(EXISTING_CODE_MARKER) === -1,
			'Expected no elided code comments',
		);
...
export async function assertCriteriaMetAsync(
	accessor: ITestingServicesAccessor,
	response: string,
	criteria: string,
): Promise<void> {
...
export function validateConsistentIndentation(
	newText: string,
