...
export async function discoverTests(
	rootFolder: string,
	options: ITestDiscoveryOptions,
): Promise<SimulationSuite[]> {
	const rootFolderContents = await fs.promises.readdir(rootFolder, {
		withFileTypes: true,
	});

	const containsConversationFile = rootFolderContents.some(
		(f) => f.isFile() && f.name.endsWith('.conversation.json'),
	);

	if (containsConversationFile) {
		return [createSimulationSuite(rootFolder, options)];
	} else {
		const foldersWithScenarios = rootFolderContents
			.filter((f) => f.isDirectory())
			.map((f) => path.join(rootFolder, f.name));
		if (foldersWithScenarios.length === 0) {
			return [];
		}
		const scenarios = await Promise.all(
			foldersWithScenarios.map((f) => discoverTests(f, options)),
		);
		return scenarios.flat();
	}
}

function createSimulationSuite(
	folderWithScenarios: string,
	options: ITestDiscoveryOptions,
): SimulationSuite {
	...
	for (const scenario of scenarios) {
		if (chatKind === 'inline') {
			for (const conversation of scenario) {
				...
				const testName = conversation.name.replace(
					/.conversation\.json$/,
					'',
				);
				const conversationPath = path.join(
					conversation.scenarioFolderPath,
					conversation.name,
				);
				suite.tests.push(
					new SimulationTest(
						{ description: testName },
						{
							...
							stateFile: conversation.stateFile,
						},
						suite,
						runner,
					),
			...
			const isSlashEdit =
				scenario[0].question.startsWith(`/${Intent.Edit}`) ||
				scenario[0].question.startsWith(`/${Intent.Agent}`);
			const testName = scenario[0].name.replace(
				/.conversation\.json$/,
				'',
			);
			const conversationPath = path.join(
				scenario[0].scenarioFolderPath,
				scenario[0].name,
			);
			...
			if (isSlashEdit) {
				...
				runner = generateScenarioTestRunner(
					scenario,
					async (
						accessor,
						question,
			...
			suite.tests.push(
				new SimulationTest(
					{ description: testName },
					{
						...
						stateFile: scenario[0].stateFile,
					},
					suite,
					runner,
				),
...
function generateSlashEditScenarioTestRunner(
	scenario: Scenario,
): SimulationTestFunction {
	return async (testingServiceCollection) => {
		assert(
			scenario.length > 0,
			'Expected at least 1 conversation in the scenario',
		);
		assertType(
			scenario[0].getState !== undefined,
			'Expected state to be defined in the first conversation test case',
		);
		for (let i = 1; i < scenario.length; i++) {
			assertType(
				scenario[i].getState === undefined,
				'Expected state to be undefined in subsequent conversations',
			);
		}
		...
		const scenario2: IDeserializedWorkspaceStateBasedScenario = {
			...
			extraWorkspaceSetup: (workspace) =>
				extraWorkspaceSetup(testingServiceCollection, state, workspace),
		};

		await simulatePanelCodeMapper(
...
function generateInlineScenarioTestRunner(
	conversation: IConversationTestCase,
): (testingServiceCollection: TestingServiceCollection) => Promise<void> {
	return async (testingServiceCollection) => {
		assertType(
			conversation.getState !== undefined,
			'Expected state to be defined in conversation test case',
		);
...
function extraWorkspaceSetup(
	testingServiceCollection: TestingServiceCollection,
	state: IDeserializedWorkspaceState,
	workspace: SimulationWorkspace,
): void {
	if (state.lsifIndex) {
		testingServiceCollection.define(
			ILanguageFeaturesService,
			new SyncDescriptor(LSIFLanguageFeaturesService, [
				workspace,
	...
	if (state.testFailures && state.workspaceFolders) {
		testingServiceCollection.define(
			ITestProvider,
			new SimulationTestProvider(
				state.testFailures.map((f) => ({
					message: f.message,
					testRange: new Range(f.line, f.column, f.line, f.column),
					uri: URI.file(
						path.join(
							state.workspaceFolders![0].fsPath,
							f.file_path,
						),
					),
				})),
...
export class ExternalSimulationTestRuntime extends SimulationTestRuntime {
	constructor(baseDir: string, testOutcomeDir: string, runNumber: number) {
