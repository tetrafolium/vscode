...
ssuite({ title: 'setupTests - recommend', location: 'panel' }, () => {
	const scenarioFolder = path.join(
		__dirname,
		'..',
		'test/scenarios/test-setupTestRecommend',
	);
			...
			const runner = generateScenarioTestRunner(
				...
				async (
					...
					assert.ok(
						...
						'expected test case to have keywords',
					);
					assert.ok(
						confirmations.length,
						'expected to have a confirmation part',
					);
					const err = validate(
						confirmations[0].buttons!.join(' ').toLowerCase(),
						scenario[0].json.keywords,
					);
					if (err) {
		...
		stest({ description: scenario[0].json.name }, (collection) => {
			const runner = generateScenarioTestRunner(
				...
				async (
					...
					const files: string[] = [];
					const serializeFileTree = (
						node: ChatResponseFileTree,
						path: string = '',
					): void => {
						if (node.children) {
					...
					rubric(
						...
						() => {
							for (const pattern of e.filePatterns) {
								assert.ok(
									files.some((f) => f.match(pattern)),
									`expected file to match ${pattern}`,
								);
							}
						},
						() =>
							assert.ok(
								rawResponse.match(e.installCommandPattern),
								'expected to have an install command pattern',
							),
						() =>
							assert.ok(
								rawResponse.match(e.runCommandPattern),
								'expected to have a run command pattern',
							),
					);
