...
import {
	...
	createResultTypeFilter,
	createTestNameFilter,
} from './filterUtils';
...
export const AMLModeToolbar = mobxlite.observer(
	({
		amlProvider,
		...
		const [resultFilterSelected, setResultFilterSelected] = React.useState<
			string | undefined
		>(undefined);
		const [languageSelected, setLanguageSelected] = React.useState<
			string | undefined
		>(undefined);
		const [testNameFilterSelected, setTestNameFilterSelected] =
		...
		const onLanguageIdChange = (e: React.FormEvent<HTMLSelectElement>) => {
			let langId: string | undefined = (e.target as HTMLSelectElement)
				.value;
			langId = langId !== 'no-language-filter' ? langId : undefined;
			setLanguageSelected(langId);
		};

		const onResultFilterChange = (
			...
			resultFilter =
				resultFilter !== 'no-result-filter' ? resultFilter : undefined;
			setResultFilterSelected(resultFilter);
		};

		const onTestNameFilterChange = (
		...
		const defaultLanguageId =
			languageSelected && allLanguageIds.includes(languageSelected)
				? languageSelected
				: undefined;
			...
			>
				<div>
					...
					<div className="external-toolbar-filter">
						...
						<select
							className="external-toolbar-dropdown"
							onChange={onResultFilterChange}
							value={defaultLanguageId}
						>
							<option
								value="no-result-filter"
					...
					<div className="external-toolbar-filter">
						...
						>
							<option
								value="no-language-filter"
								key="no-language-filter"
							>
								All
							</option>
					...
					<Checkbox
						...
						onChange={() =>
							setShowOnlyTestsWithAnnotations(
								!showOnlyTestsWithAnnotations,
							)
						}
					/>
					<Dropdown
						...
						onOptionSelect={(e, o) => {
							...
							setShowOnlyTestsWithAnnotations(
								o.selectedOptions.length > 0,
							);
						}}
						onOpenChange={(e, o) =>
							o.open && updateKnownAnnotations()
						}
					>
