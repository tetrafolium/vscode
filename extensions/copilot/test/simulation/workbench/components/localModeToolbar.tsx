...
export const LocalModeToolbar = mobxlite.observer(
	({
		...
		// filtering
		const [showBaselineJSONChangedOnly, setShowBaselineJSONChangedOnly] =
			useLocalStorageState('baselineJSONChangedOnly', undefined, false);
		const [showFailedOnly, setShowFailedOnly] = useLocalStorageState(
			'showFailedOnly',
			undefined,
			false,
		);
		const [showOnlyRanTests, setShowOnlyRanTests] = useLocalStorageState(
			'showOnlyRanTests',
			undefined,
			false,
		);
		const [showWithCacheMissesOnly, setShowWithCacheMissesOnly] =
			React.useState(false);
		const [showOnlyTestsWithAnnotations, setShowOnlyTestsWithAnnotations] =
		...
		const createFilter = useCallback(
			(params: FilterParams): TestFilterer => {
				const predicates = [];
				...
				if (
					params.showOnlyTestsWithAnnotations &&
					params.selectedAnnotations.length > 0
				) {
					predicates.push(
						createAnnotationFilter(
							new Set(params.selectedAnnotations),
						),
					);
				}
		...
		const [knownAnnotations, setKnownAnnotations] = React.useState<
			string[]
		>([]);
		...
		const handleOptionSelect = useCallback(
			(_e: SelectionEvents, o: OptionOnSelectData) => {
		...
		const handleOpenChange = useCallback(
			(_e: ComboboxOpenEvents, o: ComboboxOpenChangeData) => {
				if (o.open) {
		...
		const handleNRunsChange = useCallback(
			(e: React.FormEvent<HTMLInputElement>) => {
		...
		const handleExtraArgsChange = useCallback(
			(e: React.FormEvent<HTMLInputElement>) => {
				mobx.runInAction(() => {
					runnerOptions.additionalArgs.value = (
						e.target as HTMLInputElement
					).value;
				});
			},
		...
		const handleRunFromDiskChange = useCallback(
		...
		const handleCacheModeChange = useCallback(
			(
		...
		return (
			...
			>
				...
				<div>
					...
					<div style={{ display: 'flex', gap: '10px' }}>
						...
						<Select
							...
							defaultValue={runnerOptions.cacheMode.value}
						>
							<option
								value={CacheMode.Default}
								title="Use cache if available"
							>
								Use Cache
							</option>
							<option
								value={CacheMode.Disable}
								title="Do not use cache"
							>
								No Cache
							</option>
							<option
								value={CacheMode.Require}
								title="Use cache, fail if not available"
							>
								Require Cache
							</option>
						</Select>
						<Tooltip
							relationship="label"
							content={
								"Do not send requests to the model endpoint (uses cache but doesn't write to it) (useful to make sure prompts are unchanged by observing cache misses)"
							}
						>
							<Checkbox
								label="No fetch"
						...
						<Button
							size="small"
							appearance={
								isSimulationRunning ? 'secondary' : 'primary'
							}
							icon={
								isSimulationRunning ? (
									<Stop16Regular />
						...
						{runner.selectedRun !== '' &&
							runner.state.kind !== StateKind.Running && (
								<Button
									size="small"
									onClick={handleUpdateBaselineJSON}
								>
									{' '}
				...
				<div>
					<Tooltip
						content={
							'Only modify tests displayed. Tests will be run even if not in list below'
						}
						relationship={'label'}
					>
						<Text>
							View Filters {isFiltering && '(Filtering...)'}
						</Text>
					</Tooltip>
					<div style={{ display: 'flex', alignItems: 'center' }}>
						<Checkbox
