...
export const RequestView = mobxlite.observer(
	({ request, title, baselineRequest, idx, expand }: Props) => {
		...
		function renderResponse(response: ISerialisedChatResponse): string {
			if (response.type === 'success') {
				...
				parts.push(
					request.response.copilotFunctionCalls
						?.map((c) => {
		...
		const currentRunRenderedMessages = renderRequestMessages(
			request.requestMessages,
		);
		const baselineRunRenderedMessages = baselineRequest
			? renderRequestMessages(baselineRequest.requestMessages)
			: undefined;

		const requestChangeInfo = baselineRequest && (
			<Tooltip
				content={
					'Below compares two requests - one from "Compare against" run and one from "Current run"'
				}
				relationship={'label'}
			>
				<Text weight="bold">
					{currentRunRenderedMessages === baselineRunRenderedMessages
						? '(not changed)'
						: '(changed)'}
				</Text>
			</Tooltip>
		...
		return (
			<div className="request-container">
				<div className="title" onClick={() => setExpanded(!expanded)}>
					{expanded ? '▼' : '▶'}{' '}
					{isToolCall(request) ? 'Tool Call' : 'Chat Request'} #
					{idx + 1} {title ? `- ${title}` : ``} -{' '}
					{getRequestStats(request)} {requestChangeInfo}
					{request.model && (
						<Badge
							title="Chat model used for request"
							color="informative"
							size="small"
						>
							{request.model}
				...
				{!expanded ? null : (
					...
					>
						...
						{baselineRequest && (
							<Text size={300}>
								Left editor - request from "Compare against"
								run, Right editor - request from "Current run"
							</Text>
						)}
						<div style={{ marginLeft: '-10px' }}>
							{baselineRequest ? (
								(assertType(
									baselineRunRenderedMessages,
									'must be non-undefined as long as `baselineRequest` is defined',
								),
								(
									<DiffEditor
										original={baselineRunRenderedMessages}
										modified={currentRunRenderedMessages}
										languageId="markdown"
									/>
								))
							) : (
								<Editor
									lineNumbers={false}
									languageId="markdown"
									contents={currentRunRenderedMessages}
								/>
							)}
						</div>
						...
						{baselineRequest && (
							<Text size={300}>
								Left editor - response from "Compare against"
								run, Right editor - response from "Current run"
							</Text>
						)}
						<div style={{ marginLeft: '-10px' }}>
							<div className="reply">
								{baselineRequest ? (
									<DiffEditor
										languageId="markdown"
										original={renderResponse(
											baselineRequest.response,
										)}
										modified={renderResponse(
											request.response,
										)}
									/>
								) : (request.response.type === 'success' &&
										request.response.value !== undefined) ||
								  (request.response.type === 'length' &&
										request.response.truncatedValue !==
											undefined) ? (
									<Editor
										lineNumbers={false}
										languageId="markdown"
										contents={renderResponse(
											request.response,
										)}
									/>
								) : (
	...
	if (response.type === 'success') {
		if (response.isCacheHit === true) {
			...
			if (response.cacheMetadata) {
				result.push(
					`original fetch: ${response.cacheMetadata.requestTime.replace('T', ' ').substring(0, 19)}`,
				);
				result.push(
					`duration: ${response.cacheMetadata.requestDuration} ms`,
				);
			}
		} else {
...
function renderRequestMessages(
	messages: string | ISerialisedChatMessage[],
): string {
