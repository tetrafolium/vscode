...
export const BaselineJSONPicker = mobxlite.observer(
	({ testsProvider }: Props) => {
		const id = 'baselineJSONPicker';
		const styles = useInternalToolbarPickerStyles();

		const onOptionSelect = mobx.action(
			(_e: SelectionEvents, { optionValue }: OptionOnSelectData) => {
				assertType(
					optionValue === 'beforeRunBaselineJSON' ||
						optionValue === 'workingTreeBaselineJSON',
				);
		...
		return (
			<div className={styles.root}>
				...
				<Dropdown
					...
					value={
						testsProvider.comparedBaselineJSON ===
						'beforeRunBaselineJSON'
							? 'Snapshot before run'
							: 'Working tree'
					}
