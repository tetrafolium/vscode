...
export const Scorecard = mobxlite.observer(
	({ amlProvider }: { amlProvider: AMLProvider }) => {
		const [scorecard, setScorecard] = React.useState<ScoreCard | null>(
			null,
		);
		const [compareScorecard, setCompareScorecard] =
			React.useState<ScoreCard | null>(null);
		...
		React.useEffect(() => {
			(async () => {
				if (
					...
					!amlProvider.selected.scoreCardCsvPath
				) {
					setScorecard(null);
				} else {
				...
				if (
					...
					!amlProvider.compareAgainstRun.scoreCardCsvPath
				) {
					setCompareScorecard(null);
				} else {
					const compareScorecard = parseScoreCard(
						await fs.promises.readFile(
							amlProvider.compareAgainstRun.scoreCardCsvPath,
							'utf8',
						),
		...
		const getHighlightStyle = (
			...
			const isBetter: boolean = isHigherBetter
							...
							<tbody>
								<tr>
									...
									<td>
										...
										[{scorecard.confidenceInterval[0]},{' '}
										{scorecard.confidenceInterval[1]}]
									</td>
									<td>
										{compareScorecard ? (
											<>
												{compareScorecard.count}
												<ArrowRight12Regular
													style={{
														verticalAlign: 'middle',
													}}
