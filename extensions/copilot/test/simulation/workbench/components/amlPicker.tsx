...
export const AMLPicker = mobxlite.observer(
	({ amlProvider }: { amlProvider: AMLProvider }) => {
		const selectedRun = amlProvider.selected;

		const runsToCompareAgainst = amlProvider.runs.filter(
			(run) =>
				!selectedRun ||
				(selectedRun.kind === run.kind &&
			...
			<div style={{ display: 'flex' }}>
				<div className="external-toolbar-aml-picker">
					...
					<select
						onChange={mobx.action(
							(e) =>
								(amlProvider.selectedName.value =
									e.target.value),
						)}
						className="items"
				...
				<div>
					...
					<select
						onChange={mobx.action(
							(e) =>
								(amlProvider.compareAgainstRunName.value =
									e.target.value),
						)}
						className="items"
					...
					>
						{runsToCompareAgainst.length > 0 ? (
							...
							<option key="no-comparable-runs" value="">
								No comparable runs
							</option>
						)}
					</select>
