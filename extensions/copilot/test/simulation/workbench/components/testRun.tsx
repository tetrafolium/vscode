...
import {
	...
	IRange,
	ISerializedFileEdit,
	ISerializedNesUserEditsHistory,
	InterceptedRequest,
...
import {
	...
	WorkspaceState,
} from '../stores/simulationWorkspaceState';
	...
	({
		...
		return (
			<div className="testRun">
				<div className="foldingBar" onClick={closeTestRunView}>
					<ChevronUp20Regular />
				</div>
				<div className="content">
					<TestRunVisualization
						test={test}
						run={run}
						baseline={baseline}
					...
					/>
					{run.error !== undefined && (
						<ErrorMessageBar error={run.error} />
					)}
				</div>
			</div>
		);
...
class WorkspaceFileState {
	...
	public update(state: WorkspaceState): WorkspaceFileState {
		...
		if (state.kind === 'initial') {
			if (state.file.value) {
				files.set(
					state.file.value.workspacePath,
					state.file.value.contents,
				);
				if (state.selection) {
					selections.set(
						state.file.value.workspacePath,
...
class RequestRenderer {
	...
	public take(
		stopIndex: number = this._run.requests.value.length,
	): RequestRenderData[] {
		...
		while (this._nextRequestIndex < stopIndex) {
			...
			const baselineRequest =
				this._baseline?.requests.value[this._nextRequestIndex];
			const isIntentDetection =
				Array.isArray(request.requestMessages) &&
				request.requestMessages.some((m) =>
					getTextPart(m.content).includes('Function Id: doc'),
				);
			result.push(
...
const TestRunVisualization = mobxlite.observer(
	({
		...
		baseline,
		displayOptions,
	}: Omit<TestRunViewProps, 'closeTestRunView'>) => {
		if (
			!run.requests.resolved ||
			!run.inlineChatWorkspaceStates.resolved ||
			!run.nextEditSuggestion.resolved
		) {
			return <>PENDING</>;
		}
		...
		const requestRenderer = new RequestRenderer(
			displayOptions.expandPrompts.value,
			run,
			baseline,
		);
		...
		if (
			...
			run.inlineChatWorkspaceStates.value[1].changedFiles.value.length ===
				1 &&
			run.inlineChatWorkspaceStates.value[1].changedFiles.value[0]
				.workspacePath ===
				run.inlineChatWorkspaceStates.value[0].file.value?.workspacePath
		) {
			// Optimize rendering for simple case
			files = files.update(run.inlineChatWorkspaceStates.value[0]);
			result.push(
				<InteractionState
					key={`single-state-step`}
					test={test}
					files={files}
					state={run.inlineChatWorkspaceStates.value[1]}
					requests={[]}
					expectedDiff={run.expectedDiff}
				/>,
			);
		} else {
			let stepNumber = 1;
		...
		if (
			run.generatedTestCaseCount !== undefined &&
			run.generatedTestCaseCount !== null
		) {
			result.push(
		...
		if (run.nesUserEditsHistory.value) {
			result.push(
				<NesUserEditHistory
					key="nes-user-edit-history"
					userEditsHistory={run.nesUserEditsHistory.value}
				/>,
			);
		}
		if (baseline?.nextEditSuggestion.value) {
			result.push(
				<details>
					<summary>Next edit (reference run)</summary>
					<NextEditSuggestion
						key="reference-run-next-edit-suggestion"
						runKind={'reference'}
		...
		result.push(
			<Requests
				key="leftover-requests"
				requests={requestRenderer.take()}
			/>,
		);
...
const InteractionState = mobxlite.observer(
	({
		...
		state: InteractionWorkspaceState;
		requests: RequestRenderData[];
		expectedDiff?: string;
	}) => {
		return (
			<div>
				<UserQuery key={`step-query`} text={state.interaction.query} />
				{!state.interaction.query.startsWith('/') && (
					<ChosenIntent
						key={`step-intent`}
						detectedIntent={state.interaction.detectedIntent}
						actualIntent={state.interaction.actualIntent}
					/>
				)}
...
const ChangedFiles = mobxlite.observer(
	({ files, state, test }: ChangedFilesProps) => {
		...
		for (
			...
			changedFileIndex < state.changedFiles.value.length;
			changedFileIndex++
		) {
			...
			if (
				prevFile &&
				prevFile.split('\n').length === 1 &&
				state.interaction.detectedIntent === 'tests'
			) {
				prevFile = undefined;
			}

			const fileDiagnostics =
				state.diagnostics?.[changedFile.workspacePath] ??
				getDiagnosticsFromTest(test, changedFileIndex);
			result.push(
				<ChangedFile
					...
					range={state.range}
					selections={{
						before: files.selections.get(changedFile.workspacePath),
...
function getDiagnosticsFromTest(
	testRun: ISimulationTest,
	index: number,
): IDiagnosticComparison | undefined {
	if (
		index === 0 &&
		testRun.errorsOnlyInBefore &&
		testRun.errorsOnlyInAfter
	) {
		...
		return {
			before: toDiagnostics(testRun.errorsOnlyInBefore),
			after: toDiagnostics(testRun.errorsOnlyInAfter),
		};
	}
	return undefined;
}
			...
			<div>
				{typeof prevFile !== 'undefined' ? (
					<div>
						...
						<Text size={300}>
							Left editor - the existing code before applying the
							change, Right editor - the new code after applying
							the change
						</Text>
						<DiffEditor
...
const NesUserEditHistory = mobxlite.observer(
	({
		...
		userEditsHistory: ISerializedNesUserEditsHistory;
	}) => {
			...
			<div>
				...
				{edits.map((edit, idx) => {
					return (
						<div key={idx}>
							<div className="step-title">
								{currentDocumentIndex === idx ? 'Active' : ''}{' '}
								File {edit.id ?? 'No file name'}
							</div>
							<DiffEditor
	...
	({
		...
		return (
			<div>
				<div className="step-title">
					Next Edit (
					{runKind === 'current' ? 'current run' : 'reference run'})
				</div>
				<DiffEditor
	...
	return (
		<div>
			...
			<span
				...
				className="step-query"
			>
				/{detectedIntent} (EXPECTED: /{actualIntent})
			</span>
			<div style={{ clear: 'left' }}></div>
		</div>
	);
...
const ErrorMessageBar = mobxlite.observer(({ error }: { error: string }) => {
	...
	return (
		<MessageBar intent="error" layout="singleline">
			<MessageBarBody>
				<MessageBarTitle>
					Test failed with error: {errorTitle}
				</MessageBarTitle>
...
function dumbDiffParser(
	diff: string,
): { original: string; modified: string; path: string }[] {
