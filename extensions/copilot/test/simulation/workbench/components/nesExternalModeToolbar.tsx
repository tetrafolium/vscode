...
export const NesExternalModeToolbar = mobxlite.observer(
	({
		...
		const handleScenariosPathChange = useCallback(
		...
		const hasValidScenariosPath =
			nesExternalOptions.externalScenariosPath.value.length > 0;
				...
				<div>
					...
					<div style={{ display: 'flex', gap: '10px' }}>
						...
						<Tooltip
							relationship="label"
							content={
								"Do not send requests to the model endpoint (uses cache but doesn't write to it)"
							}
						>
							<Checkbox
								label="No fetch"
						...
						<Tooltip
							relationship="label"
							content={
								!hasValidScenariosPath
									? 'Set the external scenarios path first'
									: ''
							}
						>
							<Button
