...
} from '../stores/simulationTestsProvider';
...
export const App = mobxlite.observer(
	({
		...
		const [theme, setTheme] = useLocalStorageState<ThemeKind>(
			'appTheme',
			undefined,
			'light',
		);

		const [filterer, setFilterer] = React.useState<
			TestFilterer | undefined
		>(undefined);
			...
			>
				<ContextMenuProvider>
					<div>
						...
						/>
						{testsProvider.testSource.value ===
							TestSource.External && (
						...
						{(testsProvider.testSource.value === TestSource.Local ||
							testsProvider.testSource.value ===
								TestSource.NesExternal) && (
							<TerminationMessageBar runner={runner} />
						)}
						<div
							style={{
								margin: '5px',
...
const TerminationMessageBar = mobxlite.observer(
	({ runner }: { runner: SimulationRunner }) =>
		runner.terminationReason === undefined ? null : (
	...
	({ displayOptions }: { displayOptions: DisplayOptions }) => (
		<Tooltip content="Show all tests or by suites" relationship="label">
			<ToggleButton
				...
				onClick={mobx.action(
					() =>
						(displayOptions.testsKind.value =
							displayOptions.testsKind.value === 'suiteList'
								? 'testList'
								: 'suiteList'),
				)}
...
export class DisplayOptions {
	...
	constructor(storage: SimulationStorage) {
		this.testsKind = new SimulationStorageValue(
			storage,
			'displayTestsKind',
			'suiteList',
		);
		this.expandPrompts = new SimulationStorageValue(
			storage,
			'expandPrompts',
			false,
		);
...
const TestsInfo = mobxlite.observer(
	({ tests, displayedTests }: TestsInfoProps) => {
		// TODO@ulugbekna: don't show "failing" if tests weren't run yet
		return (
			<Text size={400}>
				# of tests: {tests.length} (<span>displayed:</span>{' '}
