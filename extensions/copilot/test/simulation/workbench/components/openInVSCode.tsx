...
export const OpenInVSCodeButton = mobxlite.observer(
	({ test }: { test: ISimulationTest }) => {
		...
		return (
			<button
				...
				onClick={() => {
					openInVSCode(simulationInputPath, matchResult[0]);
				}}
			>
...
async function openInVSCode(simulationInputPath: string, caseName: string) {
	try {
		const conversation = JSON.parse(
			await fs.readFile(
				path.join(simulationInputPath, `${caseName}.conversation.json`),
				'utf8',
			),
		);
		if (
		...
		const state = JSON.parse(
			await fs.readFile(
				path.join(simulationInputPath, conversation[0].stateFile),
				'utf8',
			),
		);
