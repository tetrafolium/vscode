...
export const TestList = mobxlite.observer(
	({
		tests,
		runner,
		runnerOptions,
		...
		switch (displayOptions.testsKind.value) {
			case 'testList': {
				return (
					<Tree
						aria-label="Default"
						style={{ rowGap: '0px', paddingBottom: '30px' }}
					>
						{tests.map((test) => (
							<TestView
								key={test.name}
				...
				const suiteNameContextMenuEntries = (suiteName: string) => [
					...
					},
					{
						...
						onClick: () => {
							mobx.runInAction(
								() =>
									(runnerOptions.grep.value = `!s:${suiteName}`),
							);
							runner.startRunning({
								grep: `!s:${suiteName}`,
								cacheMode: runnerOptions.cacheMode.value,
					...
					<>
						...
						>
							{[...suites.entries()].map(([suiteName, tests]) => {
								const nTestInSuite = tests.reduce(
									(acc, test) =>
										test.runnerStatus?.isSkipped
											? acc
											: acc + 1,
									0,
								);

								const nTestsRunCompleted = tests.reduce(
									(acc, test) => {
								...
								return (
									<TreeItem
										key={suiteName}
										itemType="branch"
										className="test-runs-container"
									>
										<TreeItemLayout
											onAuxClick={(e) =>
												showMenu(
													e,
													suiteNameContextMenuEntries(
														suiteName,
													),
												)
											}
...
const CompletedTestCount = mobxlite.observer(
	({
		runner,
		nTestsRunCompleted,
		nTestInSuite,
		...
		return (
			<span style={{ whiteSpace: 'pre', fontFamily: 'monospace' }}>
				...
				runner.state.kind === StateKind.Stopped
					? `${nTestsRunCompleted.toString().padStart(digits, ' ')} / ${nTestInSuite.toString().padStart(digits, ' ')} | `
					: ''}
			</span>
		);
	},
