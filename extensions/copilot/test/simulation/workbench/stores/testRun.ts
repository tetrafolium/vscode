...
export class TestRun {
	...
	@mobx.computed
	public get nextEditSuggestion(): ObservablePromise<
		ISerializedFileEdit | undefined
	> {
		return new ObservablePromise(this._getNextEditSuggestion(), undefined);
	}

	@mobx.computed
	public get nesUserEditsHistory(): ObservablePromise<
		ISerializedNesUserEditsHistory | undefined
	> {
	...
	@mobx.computed
	public get inlineChatWorkspaceStates(): ObservablePromise<
		WorkspaceState[]
	> {
	...
	private _nextEditSuggestionPromise:
		| Promise<ISerializedFileEdit | undefined>
		| undefined;
	...
	private async _doGetNextEditSuggestion(): Promise<
		ISerializedFileEdit | undefined
	> {
		const contents = await this.getWrittenFileByTag(
			NEXT_EDIT_SUGGESTION_TAG,
		);
		if (!contents) {
			return undefined;
	...
	private _nesUserEditsHistoryPromise:
		| Promise<ISerializedNesUserEditsHistory | undefined>
		| undefined = undefined;
	private _getNesUserEditsHistory(): Promise<
		ISerializedNesUserEditsHistory | undefined
	> {
	...
	private async _doGetNesUserEditsHistory(): Promise<
		ISerializedNesUserEditsHistory | undefined
	> {
		const contents = await this.getWrittenFileByTag(
			NES_USER_EDITS_HISTORY_TAG,
	...
	private _interceptedRequestsPromise:
	...
	private async getWrittenFileByTag(
		tag: string,
		...
		const contents = await fs.promises.readFile(
			path.join(this.writtenFilesBaseDir, writtenFile.relativePath),
			'utf8',
		);
		return contents;
	}
