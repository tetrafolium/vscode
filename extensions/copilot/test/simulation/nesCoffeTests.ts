...
import {
	LogEntry,
	serializeEdit,
} from '../../src/platform/workspaceRecorder/common/workspaceLog';
...
export async function discoverCoffeTests(
	rootFolder: string,
	options: SimulationOptions,
) {
	const rootFolderContents = await fs.promises.readdir(rootFolder, {
		withFileTypes: true,
	});

	const recordingFiles = rootFolderContents.filter(
		(fileEntry) =>
			fileEntry.isFile() && fileEntry.name.endsWith(TEST_FILE_SUFFIX),
	);
	...
	const rootSuite = new SimulationSuite({
		title: 'NES',
		location: 'external',
	});

	let tests = recordingFiles.map((file) =>
...
function generateExternalStestFromRecording(
	file: fs.Dirent<string>,
	containingSuite: SimulationSuite,
	tester: InlineEditTester,
	...
	const stest = new SimulationTest(
		{ description: testName, configurations },
		{},
		containingSuite,
		async (collection) => {
			...
			const filesWithoutTargetFile = testInput.state.openFiles.filter(
				(f) => f.uri !== testInput.completion.uri,
			);

			const targetFile = testInput.state.openFiles.find(
				(f) => f.uri === testInput.completion.uri,
			);
			assertDefined(
				targetFile,
				`Target file ${testInput.completion.uri} not found in open files.`,
			);

			const targetFileId = filesWithoutTargetFile.length; // careful: needs to be in sync with loop logic

			const { targetFileBeforeEdit, edit } =
				computeTargetFileBeforeEditAndEdit(targetFile);
			...
			if (r.aiRootedEdit && r.aiRootedEdit.edit.replacements.length > 0) {
				...
				const start = baseTrans.getPosition(
					singleEdit.replaceRange.start,
				);
				const end = baseTrans.getPosition(
					singleEdit.replaceRange.endExclusive,
				);

				const trimmedEdit = rootedEdit.edit.removeCommonSuffixAndPrefix(
					rootedEdit.base.value,
				);

				completions.push({
					insertText: singleEdit.newText,
					displayText:
						trimmedEdit.replacements.at(0)?.newText ??
						'<edits disappeared during trimming>',
					range: {
						start: {
			...
			const resultFilePath = path.join(
				file.parentPath,
				`${testName}${RESULT_FILE_SUFFIX}`,
			);

			await fs.promises.writeFile(
				resultFilePath,
				JSON.stringify(completionsOutput, null, 2),
			);
		},
	);
...
function computeTargetFileBeforeEditAndEdit(
	targetFile: CompletionStests.TestDocument,
): { targetFileBeforeEdit: string; edit: StringEdit } {
	const cursorOffset = targetFile.text.indexOf('⮑');
	assert(
		cursorOffset !== -1,
		'Cursor marker ⮑ not found in target file text.',
	);

	const targetFileWithoutCursor = targetFile.text.replace('⮑', '');
	let wordAtCursorStartOffset = cursorOffset - 1;
	while (
		wordAtCursorStartOffset > 0 &&
		/(\w|\.)/.test(targetFileWithoutCursor[wordAtCursorStartOffset - 1])
	) {
		wordAtCursorStartOffset--;
	}
	const editToRemoveWordAtCursor = StringEdit.create([
		StringReplacement.delete(
			new OffsetRange(wordAtCursorStartOffset, cursorOffset),
		),
	]);
	const editToInsertWordAtCursor = editToRemoveWordAtCursor.inverse(
		targetFileWithoutCursor,
	);
	const targetFileBeforeEdit = editToRemoveWordAtCursor.apply(
		targetFileWithoutCursor,
	);
