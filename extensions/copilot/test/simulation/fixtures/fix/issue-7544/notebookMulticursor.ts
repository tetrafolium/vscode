	...
	implements INotebookEditorContribution
{
	...
	private cursorsControllers: ResourceMap<[ITextModel, CursorsController]> =
		new ResourceMap<[ITextModel, CursorsController]>();
	...
	private updateCursorsControllers() {
		...
		this.trackedMatches.forEach(async (match) => {
			...
			this.cursorsControllers.set(match.cellViewModel.uri, [
				textModel,
				controller,
			]);
		});
	...
	private updateFinalUndoRedo() {
		...
		textModels.forEach((model) => {
			const trackedMatch = this.trackedMatches.find(
				(match) =>
					match.cellViewModel.uri.toString() === model.uri.toString(),
			);
			if (!trackedMatch) {
				return;
			}
	...
	public resetToIdleState() {
		...
		this.trackedMatches[0].cellViewModel.setSelections([
			this.trackedMatches[0].selections[0],
		]);
		...
		if (this.state === NotebookMultiCursorState.Idle) {
			...
			if (findResult.cell.handle !== cell.handle) {
				...
				newMatch = {
					...
					decorationIds: [],
					undoRedo: {
						elements: this.undoRedoService.getElements(
	...
	private updateLazyDecorations() {
		...
		this.trackedMatches.forEach((match) => {
			...
			if (!controller) {
				// active cell doesn't get a stored controller from us
				selections =
