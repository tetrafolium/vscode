	...
	if (!pr) {
		if (!folderRepoManager.activePullRequest) {
			vscode.window.showErrorMessage(
				vscode.l10n.t('Unable to find current pull request.'),
			);
...
export async function openPullRequestOnGitHub(
	e: PRNode | DescriptionNode | PullRequestModel,
	telemetry: ITelemetry,
) {
	if (e instanceof PRNode || e instanceof DescriptionNode) {
		vscode.commands.executeCommand(
			'vscode.open',
			vscode.Uri.parse(e.pullRequestModel.html_url),
		);
	} else {
		vscode.commands.executeCommand(
			'vscode.open',
			vscode.Uri.parse(e.html_url),
		);
	}
}
...
export function registerCommands(
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (
				e: PRNode | DescriptionNode | PullRequestModel | undefined,
			) => {
				if (!e) {
					const activePullRequests: PullRequestModel[] =
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('pr.openAllDiffs', async () => {
			const activePullRequestsWithFolderManager =
				reposManager.folderManagers
					.filter((folderManager) => folderManager.activePullRequest)
					.map((folderManager) => {
						return {
							activePr: folderManager.activePullRequest!,
							folderManager,
						};
					});

			const activePullRequestAndFolderManager =
				activePullRequestsWithFolderManager.length >= 1
					? await chooseItem(
			...
			reviewManager.reviewModel.localFileChanges.forEach(
				(localFileChange) =>
					localFileChange.openDiff(folderManager, { preview: false }),
			);
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('review.suggestDiff', async (e) => {
			try {
				...
				const { indexChanges, workingTreeChanges } =
					folderManager.repository.state;
				...
				if (!indexChanges.length) {
					if (workingTreeChanges.length) {
						...
						vscode.window.showInformationMessage(
							vscode.l10n.t('There are no changes to suggest.'),
						);
						return;
					}
				}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.openFileOnGitHub',
			async (e: GitFileChangeNode | RemoteFileChangeNode) => {
				if (e instanceof RemoteFileChangeNode) {
					const choice = await vscode.window.showInformationMessage(
						vscode.l10n.t(
							"{0} can't be opened locally. Do you want to open it on GitHub?",
							e.changeModel.fileName,
						),
						vscode.l10n.t('Open'),
					);
					if (!choice) {
				...
				if (e.changeModel.blobUrl) {
					return vscode.commands.executeCommand(
						'vscode.open',
						vscode.Uri.parse(e.changeModel.blobUrl),
					);
				}
			},
		),
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.openOriginalFile',
			async (e: GitFileChangeNode) => {
				// if this is an image, encode it as a base64 data URI
				const folderManager = reposManager.getManagerForIssueModel(
					e.pullRequest,
				);
				if (folderManager) {
					const imageDataURI = await asImageDataURI(
	...
	async function openDiffView(
		...
		if (fileChangeNode && !(fileChangeNode instanceof vscode.Uri)) {
			...
			const editor = fileChangeNode
				? vscode.window.visibleTextEditors.find(
						(editor) =>
							editor.document.uri.toString() ===
							fileChangeNode.toString(),
					)!
			...
			const diffEditor =
				...
				tabInput.modified.toString() === editor.document.uri.toString()
					? vscode.window.activeTextEditor
					: undefined;
			if (diffEditor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (e: PRNode) => {
				...
				try {
					await folderManager.deleteLocalPullRequest(
						pullRequestModel,
					);
				} catch (e) {
					if (e.gitErrorCode === GitErrorCodes.BranchNotFullyMerged) {
						const action = await vscode.window.showErrorMessage(
							vscode.l10n.t(
								"The local branch '{0}' is not fully merged. Are you sure you want to delete it?",
								pullRequestModel.localBranchName ??
									'unknown branch',
							),
							DELETE_BRANCH_FORCE,
						);
		...
		return chooseItem<ReviewManager>(
				...
				pathLib.basename(itemValue.repository.rootUri.fsPath),
			{
				placeHolder: vscode.l10n.t(
					'Choose a repository to create a pull request in',
				),
				ignoreFocusOut: true,
			},
		);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.create',
			async (
				args?: { repoPath: string; compareBranch: string } | Repository,
			) => {
				// The arguments this is called with are either from the SCM view, or manually passed.
				if (isSourceControl(args)) {
					(
						await chooseReviewManager(args.rootUri.fsPath)
					)?.createPullRequest();
				} else {
					(
						await chooseReviewManager(args?.repoPath)
					)?.createPullRequest(args?.compareBranch);
				}
			},
		),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (args?: any | Repository) => {
				if (isSourceControl(args)) {
					const reviewManager = await chooseReviewManager(
						args.rootUri.fsPath,
					);
					if (reviewManager) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.pick',
			async (pr: PRNode | DescriptionNode | PullRequestModel) => {
				if (pr === undefined) {
					// This is unexpected, but has happened a few times.
					Logger.error(
						'Unexpectedly received undefined when picking a PR.',
					);
		...
		vscode.commands.registerCommand(
			...
			async (pr: PRNode | DescriptionNode | PullRequestModel) => {
				...
				return vscode.env.openExternal(
					vscode.Uri.parse(vscodeDevPrLink(pullRequestModel)),
				);
			},
		),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.exit',
			async (
				pr: PRNode | DescriptionNode | PullRequestModel | undefined,
			) => {
				...
				if (pr instanceof PRNode || pr instanceof DescriptionNode) {
					...
					pullRequestModel = await chooseItem<PullRequestModel>(
						reposManager.folderManagers
							...
							.filter((activePR) => !!activePR),
						(itemValue) =>
							`${itemValue.number}: ${itemValue.title}`,
						{
							placeHolder: vscode.l10n.t(
								'Choose the pull request to exit',
							),
						},
					);
				} else {
				...
				return vscode.window.withProgress(
					...
					async () => {
						const branch =
							await pullRequestModel!.githubRepository.getDefaultBranch();
						const manager =
							reposManager.getManagerForIssueModel(
								pullRequestModel,
							);
						if (manager) {
							const prBranch =
							...
							if (prBranch) {
								await manager.cleanupAfterPullRequest(
									prBranch,
									pullRequestModel!,
								);
							}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('pr.merge', async (pr?: PRNode) => {
			...
			const isCrossRepository =
				pullRequest.base &&
				pullRequest.head &&
				!pullRequest.base.repositoryCloneUrl.equals(
			...
			return vscode.window
				...
				.then(async (value) => {
					...
					if (value === yes) {
						try {
							newPR =
								await folderManager.mergePullRequest(
									pullRequest,
								);
							return newPR;
						} catch (e) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.close',
			async (pr?: PRNode | PullRequestModel, message?: string) => {
				let pullRequestModel: PullRequestModel | undefined;
				if (pr) {
					pullRequestModel =
						pr instanceof PullRequestModel
							? pr
							: pr.pullRequestModel;
				} else {
					const activePullRequests: PullRequestModel[] =
				...
				return vscode.window
					...
					.then(async (value) => {
						if (value === yes) {
							try {
								let newComment: IComment | undefined =
									undefined;
								if (message) {
									newComment =
										await pullRequest.createIssueComment(
											message,
										);
								}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('pr.dismissNotification', (node) => {
			if (node instanceof PRNode) {
				tree.notificationProvider
					.markPrNotificationsAsRead(node.pullRequestModel)
					.then(() => tree.refresh(node));
			}
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.openDescription',
			async (
				argument: DescriptionNode | PullRequestModel | undefined,
			) => {
				let pullRequestModel: PullRequestModel | undefined;
				if (!argument) {
					const activePullRequests: PullRequestModel[] =
					...
					pullRequestModel =
						argument instanceof DescriptionNode
							? argument.pullRequestModel
							: argument;
				}
				...
				const folderManager =
					reposManager.getManagerForIssueModel(pullRequestModel);
				if (!folderManager) {
					return;
				}
				...
				if (argument instanceof DescriptionNode) {
					...
					descriptionNode =
						reviewManager.changesInPrDataProvider.getDescriptionNode(
							folderManager,
						);
				}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (descriptionNode: DescriptionNode) => {
				const folderManager = reposManager.getManagerForIssueModel(
					descriptionNode.pullRequestModel,
				);
				if (!folderManager) {
					return;
				}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.openReview',
			async (thread: GHPRCommentThread) => {
	...
	function threadAndText(
		commentLike: CommentReply | GHPRCommentThread | GHPRComment | any,
	): { thread: GHPRCommentThread; text: string } {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (
				commentLike: CommentReply | GHPRCommentThread | GHPRComment,
			) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.makeSuggestion',
			async (reply: CommentReply | GHPRComment) => {
				const thread =
					reply instanceof GHPRComment ? reply.parent : reply.thread;
				const commentEditor =
					...
					Schemes.Comment
						...
						: vscode.window.visibleTextEditors.find(
								(visible) =>
									visible.document.uri.scheme ===
										Schemes.Comment &&
									visible.document.uri.query === '',
							);
				if (!commentEditor) {
					Logger.error(
				...
				const contents = editor?.document.getText(
					new vscode.Range(
						...
						editor.document.lineAt(thread.range.end.line).text
							.length,
					),
				);
				return commentEditor.edit((editBuilder) => {
					editBuilder.insert(
						commentEditor.selection.end,
						`
\`\`\`suggestion
${contents}
\`\`\``,
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.editComment',
			async (comment: GHPRComment | TemporaryComment) => {
				comment.startEdit();
			},
		),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'review.openFile',
			(value: GitFileChangeNode | vscode.Uri) => {
				const command =
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			(value: vscode.Uri) => {
				...
				const editor = vscode.window.visibleTextEditors.find(
					(editor) =>
						editor.document.uri.toString() === value.toString(),
				);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('pr.setFileListLayoutAsTree', (_) => {
			vscode.workspace
				...
				.update('fileListLayout', 'tree', true);
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('pr.setFileListLayoutAsFlat', (_) => {
			vscode.workspace
				.getConfiguration('githubPullRequests')
				.update('fileListLayout', 'flat', true);
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			(prNode: PRNode) => {
				const folderManager = reposManager.getManagerForIssueModel(
					prNode.pullRequestModel,
				);
				if (
					folderManager &&
					prNode.pullRequestModel.equals(
						folderManager?.activePullRequest,
					)
				) {
					ReviewManager.getReviewManagerForFolderManager(
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			'pr.markFileAsViewed',
			async (treeNode: FileChangeNode | vscode.Uri | undefined) => {
				try {
					...
					if (treeNode instanceof FileChangeNode) {
						...
						await manager?.activePullRequest?.markFileAsViewed(
							treeNode.path,
						);
					...
					vscode.window.showErrorMessage(
						`Marked file as viewed failed: ${e}`,
					);
				}
			},
		),
			...
			async (treeNode: FileChangeNode | vscode.Uri | undefined) => {
				try {
					...
					if (treeNode instanceof FileChangeNode) {
						...
						const manager =
							reposManager.getManagerForFile(treeNode);
						await manager?.activePullRequest?.unmarkFileAsViewed(
							treeNode.path,
						);
						manager?.setFileViewedContext();
					}
					...
					vscode.window.showErrorMessage(
						`Marked file as not viewed failed: ${e}`,
					);
				}
			},
		),
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('pr.copyVscodeDevPrLink', async () => {
			...
			const pr = await chooseItem<PullRequestModel>(
				...
				{
					placeHolder: vscode.l10n.t(
						'Pull request to create a link for',
					),
				},
			);
			if (pr) {
		...
		vscode.commands.registerCommand('pr.checkoutByNumber', async () => {
			...
			reposManager.folderManagers.forEach((manager) => {
				githubRepositories.push(
					...manager.gitHubRepositories.map((repo) => {
						return { manager, repo };
					}),
				);
			});
			const githubRepo = await chooseItem<{
				...
				githubRepositories,
				(itemValue) =>
					`${itemValue.repo.remote.owner}/${itemValue.repo.remote.repositoryName}`,
				{
					placeHolder: vscode.l10n.t(
						'Which GitHub repository do you want to checkout the pull request from?',
					),
				},
			);
			if (!githubRepo) {
			...
			const prNumber = await vscode.window.showInputBox({
				ignoreFocusOut: true,
				prompt: vscode.l10n.t('Enter the pull request number'),
				validateInput: (input: string) => {
					const matches = input.match(prNumberMatcher);
					if (
						!matches ||
						matches.length !== 2 ||
						Number.isNaN(Number(matches[1]))
					) {
			...
			if (prModel) {
				return ReviewManager.getReviewManagerForFolderManager(
					reviewManagers,
					githubRepo.manager,
				)?.switch(prModel);
			}
	...
	function chooseRepoToOpen() {
		...
		return chooseItem<GitHubRepository>(
			githubRepositories,
			(itemValue) =>
				`${itemValue.remote.owner}/${itemValue.remote.repositoryName}`,
			{
				placeHolder: vscode.l10n.t(
					'Which GitHub repository do you want to open?',
				),
			},
		);
	}
	...
	function goToNextPrevDiff(diffs: vscode.LineChange[], next: boolean) {
		...
		if (!(input instanceof vscode.TabInputTextDiff)) {
			return vscode.window.showErrorMessage(
				vscode.l10n.t("Current editor isn't a diff editor."),
			);
		}

		const editor = vscode.window.visibleTextEditors.find(
			(editor) =>
				editor.document.uri.toString() === input.modified.toString(),
		);
		if (!editor) {
			return vscode.window.showErrorMessage(
				vscode.l10n.t(
					'Unexpectedly unable to find the current modified editor.',
				),
			);
		}
		...
		if (input.original.scheme !== Schemes.Review) {
			return vscode.window.showErrorMessage(
				vscode.l10n.t("Current file isn't a pull request diff."),
			);
		}
		...
		for (const diff of iterateThroughDiffs) {
			...
			if (
				next &&
				visibleRange.end.line < practicalModifiedEndLineNumber &&
				visibleRange.end.line !== editor.document.lineCount - 1
			) {
				editor.revealRange(diffRange);
				return;
			} else if (
				!next &&
				visibleRange.start.line > diff.modifiedStartLineNumber &&
				visibleRange.start.line !== 0
			) {
				editor.revealRange(diffRange);
				return;
			}
		...
		if (!folderManager) {
			return vscode.window.showErrorMessage(
				vscode.l10n.t('Unable to find a repository for pull request.'),
			);
		}
		...
		if (!reviewManager) {
			return vscode.window.showErrorMessage(
				vscode.l10n.t('Cannot find active pull request.'),
			);
		}

		if (
			!reviewManager.reviewModel.hasLocalFileChanges ||
			reviewManager.reviewModel.localFileChanges.length === 0
		) {
			return vscode.window.showWarningMessage(
				vscode.l10n.t(
					'Pull request data is not yet complete, please try again in a moment.',
				),
			);
		}
		...
		for (
			...
			const index = next
				? i
				: reviewManager.reviewModel.localFileChanges.length - 1;
			const localFileChange =
				reviewManager.reviewModel.localFileChanges[index];
			if (
				localFileChange.changeModel.filePath.toString() ===
				editorUri.toString()
			) {
				const nextIndex = next ? index + 1 : index - 1;
				if (
					reviewManager.reviewModel.localFileChanges.length >
					nextIndex
				) {
					return reviewManager.reviewModel.localFileChanges[
						nextIndex
					].openDiff(folderManager);
				}
			}
		}
		// No further files in PR.
		const goInCircle = next
			? vscode.l10n.t('Go to first diff')
			: vscode.l10n.t('Go to last diff');
		return vscode.window
			.showInformationMessage(
				vscode.l10n.t('There are no more diffs in this pull request.'),
				goInCircle,
			)
			.then((result) => {
