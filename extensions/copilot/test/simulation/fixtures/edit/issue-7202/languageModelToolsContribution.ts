			...
			for (const extension of delta.added) {
				for (const rawTool of extension.value) {
					if (!rawTool.name || !rawTool.description) {
						logService.error(
							`Invalid tool contribution from ${extension.description.identifier.value}: ${JSON.stringify(rawTool)}`,
						);
						continue;
					}
					...
					if (!rawTool.id.match(/^[\w-]+$/)) {
						logService.error(
							`Extension '${extension.description.identifier.value}' CANNOT register participant with invalid id: ${rawTool.id}. The id must match /^[\\w-]+$/.`,
						);
						continue;
					}
