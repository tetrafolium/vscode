...
class OnigString {
	...
	public convertUtf8OffsetToUtf16(utf8Offset: number): number {
		...
		if (utf8OffsetMap && utf8Offset >= this._mappingTableStartOffset) {
			return (
				findFirstInSorted(
					utf8OffsetMap,
					utf8Offset - this._mappingTableStartOffset,
				) + this._mappingTableStartOffset
			);
		}
		return utf8Offset;
	}
	...
	public convertUtf16OffsetToUtf8(utf16Offset: number): number {
		...
		if (utf8OffsetMap && utf16Offset >= this._mappingTableStartOffset) {
			return (
				utf8OffsetMap[utf16Offset - this._mappingTableStartOffset] +
				this._mappingTableStartOffset
			);
		}
		return utf16Offset;
	...
	private encode(): void {
		...
		const u8view = new Uint8Array(
			n * 3 /* alloc max now, trim later*/ +
				1 /** null termination character */,
		);
		...
		while (i < str.length) {
			...
			if (utf16OffsetToUtf8) {
				utf16OffsetToUtf8[utf8Offset++] =
					ptrHead - mappingTableStartOffset;
			}
			...
			if (bytesRequiredToEncode === 1) {
				...
				u8view[ptrHead++] =
					(codepoint >> (6 * --bytesRequiredToEncode)) + offset;
...
class LRUCache<T, U> {
	...
	constructor(
		...
		private onDispose: (key: T, value: U) => void,
	) {
		this.cache = new Map();
	}
...
const onDispose = (scanner: OnigScanner, info: INativeOnigHInfo) => {
	...
	const status = onigasmH._disposeCompiledPatterns(
		regexTPtrsPtr,
		scanner.patterns.length,
	);
	if (status !== 0) {
		const errMessage = convertUTF8BytesFromPtrToString(
			onigasmH._getLastError(),
		);
...
export class OnigScanner {
	...
	public findNextMatch(
		...
		callback: (err: any, match?: IOnigMatch) => void,
	) {
	...
	public findNextMatchSync(
		...
		if (!onigNativeInfo) {
			...
			for (let i = 0; i < this.sources.length; i++) {
				...
				const patternStrPtr = mallocAndWriteString(
					new OnigString(pattern),
				);
				status = onigasmH._compilePattern(
					patternStrPtr,
					regexTAddrRecieverPtr,
				);
				if (status !== 0) {
					const errMessage = convertUTF8BytesFromPtrToString(
						onigasmH._getLastError(),
				...
				const regexTAddress =
					onigasmH.HEAP32[regexTAddrRecieverPtr / 4];
		...
		const onigString =
			string instanceof OnigString
				? string
				: new OnigString(this.convertToString(string));
		const strPtr = mallocAndWriteString(onigString);
		const resultInfoReceiverPtr = onigasmH._malloc(8);
		const regexTPtrsPtr = onigasmH._malloc(
			onigNativeInfo.regexTPtrs.length,
		);
		...
		if (encodedResultLength > 0) {
			const encodedResult = new Uint32Array(
				onigasmH.HEAPU32.buffer,
				encodedResultBeginAddress,
				encodedResultLength,
			);
