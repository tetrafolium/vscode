			...
			if (!this._terminalSuggestWidgetVisibleContextKey.get()) {
				if (config.quickSuggestions) {
					...
					if (
						...
						// navigating history
						if (
							this._lastUserData !== /*up*/ '\x1b[A' &&
							this._lastUserData !== /*down*/ '\x1b[B'
						) {
	...
	private _handleCompletionsSequence(
		...
		const lineContext = new LineContext(
			this._leadingLineContent +
				this._currentPromptInputState.value.substring(
					this._leadingLineContent.length,
					this._leadingLineContent.length + this._cursorIndexDelta,
				),
			this._cursorIndexDelta,
		);
			...
			switch (runOnEnterConfig) {
				...
				case 'exactMatchIgnoreExtension': {
					runOnEnter =
						...
						completionText.toLowerCase();
					if (
						completion.icon === Codicon.symbolFile ||
						completion.icon === Codicon.symbolMethod
					) {
						runOnEnter ||=
							replacementText.toLowerCase() ===
							completionText
...
export function parseCompletionsFromShell(
	...
	if (typeof rawCompletions[0] === 'string') {
		return [rawCompletions as CompressedPwshCompletion].map(
			(e) =>
				new SimpleCompletionItem({
					label: e[0],
	...
	return (rawCompletions as PwshCompletion[]).map(
		(e) =>
			new SimpleCompletionItem({
				label: e.CompletionText,
				icon: pwshTypeToIconMap[e.ResultType],
