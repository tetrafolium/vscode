...
export class ExtHostChat implements ExtHostChatShape {
	...
	>();
	private readonly _slashCommandProvider = new Map<
		number,
		ChatProviderWrapper<vscode.InteractiveSlashCommandProvider>
	>();
	...
	private readonly _onDidPerformUserAction =
		new Emitter<vscode.InteractiveSessionUserActionEvent>();
	...
	async $prepareChat(
		...
		const session = await entry.provider.prepareSession(
			initialState,
			token,
		);
		if (!session) {
	...
	async $resolveRequest(
		...
		context: any,
		token: CancellationToken,
	): Promise<Omit<IChatRequestDto, 'id'> | undefined> {
		const entry = this._chatProvider.get(handle);
		if (!entry) {
			return undefined;
		}
		...
		const request = await entry.provider.resolveRequest(
			realSession,
			context,
			token,
		);
			...
			return {
				message:
					typeof request.message === 'string'
						? request.message
						: typeConvert.ChatReplyFollowup.from(request.message),
			};
		}
	...
	async $provideWelcomeMessage(
		handle: number,
		token: CancellationToken,
	): Promise<(string | IChatReplyFollowup[])[] | undefined> {
		const entry = this._chatProvider.get(handle);
		if (!entry) {
			return undefined;
		}
	...
	async $provideFollowups(
		...
		const rawFollowups = await entry.provider.provideFollowups(
			realSession,
			token,
		);
	...
	async $provideReply(
		...
		token: CancellationToken,
	): Promise<IChatResponseDto | undefined> {
		const entry = this._chatProvider.get(handle);
		if (!entry) {
			return undefined;
		}
		...
		const requestObj: vscode.InteractiveRequest = {
			...
			message:
				typeof request.message === 'string'
					? request.message
					: typeConvert.ChatReplyFollowup.to(request.message),
		};
		...
		const progressObj: vscode.Progress<vscode.InteractiveProgress> = {
			report: (progress: vscode.InteractiveProgress) => {
				...
				const vscodeProgress: IChatProgress =
					'responseId' in progress
						? { requestId: progress.responseId }
						: progress;
				this._proxy.$acceptResponseProgress(
					handle,
					sessionId,
					vscodeProgress,
				);
		...
		try {
			result = await entry.provider.provideResponseWithProgress(
				requestObj,
				progressObj,
				token,
			);
	...
	async $provideSlashCommands(
		...
		sessionId: number,
		token: CancellationToken,
	): Promise<ISlashCommand[] | undefined> {
		const entry = this._chatProvider.get(handle);
		if (!entry) {
			return undefined;
		}
		...
		const slashCommands = await entry.provider.provideSlashCommands(
			realSession,
			token,
		);
		return slashCommands?.map(
			(c) =>
				<ISlashCommand>{
					...c,
	...
	registerSlashCommandProvider(
		...
		provider: vscode.InteractiveSlashCommandProvider,
	): vscode.Disposable {
		const wrapper = new ChatProviderWrapper(extension, provider);
		this._slashCommandProvider.set(wrapper.handle, wrapper);
		this._proxy.$registerSlashCommandProvider(
			wrapper.handle,
			chatProviderId,
		);
		return toDisposable(() => {
	...
	async $provideProviderSlashCommands(
		handle: number,
		token: CancellationToken,
	): Promise<ISlashCommand[] | undefined> {
	...
	async $resolveSlashCommand(
		...
		const resolved = await entry.provider.resolveSlashCommand(
			command,
			token,
		);
