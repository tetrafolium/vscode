...
} from 'vs/workbench/contrib/chat/browser/contrib/chatDynamicVariables';
	...
	implements MainThreadChatAgentsShape2
{
	...
	private readonly _activeResponsePartPromises = new Map<
		string,
		DeferredPromise<string | IMarkdownString | IChatTreeData>
	>();
	...
	async $handleProgressChunk(
		...
		if (progress.kind === 'asyncContent') {
			...
			const deferredContentPromise = new DeferredPromise<
				string | IMarkdownString | IChatTreeData
			>();
			...
			this._pendingProgress.get(requestId)?.({
				...progress,
				resolvedContent: deferredContentPromise.p,
			});
			return handle;
			...
			const deferredContentPromise =
				this._activeResponsePartPromises.get(responsePartId);
