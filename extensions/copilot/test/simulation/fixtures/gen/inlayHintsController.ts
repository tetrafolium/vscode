...
import {
	...
	InlayHintItem,
	InlayHintsFragments,
} from 'vs/editor/contrib/inlayHints/browser/inlayHints';
import {
	goToDefinitionWithLocation,
	showGoToContextMenu,
} from 'vs/editor/contrib/inlayHints/browser/inlayHintsLocations';
import {
	CommandsRegistry,
	ICommandService,
...
export class InlayHintsController implements IEditorContribution {
	...
	private _updateHintsDecorators(
		...
		const addInjectedText = (
			...
			attachedData?: RenderedInlayHintLabelPart,
		): void => {
		...
		for (const item of items) {
			...
			if (
				currentLineInfo.totalLen > InlayHintsController._MAX_LABEL_LEN
			) {
				continue;
			}
			...
			for (let i = 0; i < parts.length; i++) {
				...
				let tooLong = false;
				const over =
					currentLineInfo.totalLen -
					InlayHintsController._MAX_LABEL_LEN;
				if (over > 0) {
