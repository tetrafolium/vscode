...
} from '../../src/platform/notebook/common/alternativeContentEditGenerator';
...
} from '../../src/platform/test/node/simulationWorkspaceServices';
...
import {
	DiagnosticsProvider,
	ITestDiagnostic,
} from './diagnosticProviders/diagnosticsProvider';
import {
	canExecutePythonCodeWithoutErrors,
	isValidPythonFile,
} from './diagnosticProviders/python';
...
export function fromNotebookFixture(
	pathOrDirnameWithinFixturesDir: string,
	activeCell?: number /** when provided, code in other cells will be emptied */,
) {
	...
	try {
		...
		return {
			...
			fileName,
			fileContents: JSON.stringify(notebook, undefined, 2),
		};
	} catch {
...
ssuite({ title: 'notebook', subtitle: 'edit', location: 'inline' }, () => {
	stest(
		{ description: 'variables', language: 'python' },
	...
	stest(
		{ description: 'dataframe', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: 'data cleansing', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
				...
				queries: [
					{
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.ok(
								outcome.fileContents.includes('isnull') ||
									outcome.fileContents.includes('dropna'),
							);
						},
					},
				],
	...
	stest(
		{ description: 'plot', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
				...
				queries: [
					{
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.ok(!outcome.fileContents.includes('import'));
							assert.ok(
								outcome.fileContents.includes('df.plot') ||
									outcome.fileContents.includes('px.bar'),
							);
						},
					},
				],
	...
	stest(
		{
			description: '/fix notebook exection ImportError',
			language: 'python',
		},
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
				...
				queries: [
					{
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.ok(
								outcome.fileContents.includes('%pip install'),
							);
							assert.ok(
								outcome.fileContents.indexOf('pip') ===
									outcome.fileContents.lastIndexOf('pip'),
							);
						},
					},
				],
	...
	stest(
		{
			description: 'edit notebook code should not duplicate the content',
			language: 'python',
		},
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
				...
				queries: [
					{
						...
						validate: async (outcome, workspace, accessor) => {
							...
							const matches = outcome.fileContents.match(
								/(plt\.figure)|(plt\.gcf)/g,
							);
	...
	stest(
		{ description: 'set index', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
				files: [fromFixture('notebook/edit.ipynb')],
				queries: [
					{
						...
						validate: async (outcome, workspace, accessor) => {
							assert.strictEqual(outcome.type, 'inlineEdit');
							assert.ok(
								outcome.fileContents.includes('.set_index'),
							);
							assert.strictEqual(
								outcome.fileContents.match(/set\_index/g)
									?.length,
								1,
							);
							assert.strictEqual(
								outcome.fileContents.match(/DataFrame/g)
									?.length,
								1,
							);
						},
	...
	stest(
		{ description: 'group by', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
				files: [fromFixture('notebook/edit.ipynb')],
				queries: [
					{
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.strictEqual(
								outcome.fileContents.match(/groupby/g)?.length,
								1,
							);
							assert.strictEqual(
								outcome.fileContents.match(/DataFrame/g)
...
ssuite({ title: 'notebook', subtitle: 'generate', location: 'inline' }, () => {
	stest(
		{
			description: 'edit markdown cell should support code example',
			language: 'markdown',
		},
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: 'Which was the most-ordered item', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{
			description: 'How many items were orderd in total?',
			language: 'python',
		},
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{
			description:
				'create a model to predict the likelihood of a flight being delayed',
			language: 'python',
		},
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
...
ssuite(
	{ title: 'notebook', subtitle: 'generate runtime', location: 'inline' },
	() => {
		stest(
			{
				description: 'generate code uses obselete variable',
				language: 'python',
			},
			(testingServiceCollection) => {
...
ssuite(
	{ title: 'notebook', subtitle: 'fix runtime', location: 'inline' },
	() => {
		stest(
			{
				description:
					'/fix notebook execution ImportError, insert at top',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
		...
		stest(
			{
				description:
					'/fix ValueError: The truth value of an array with more than one element is ambiguous',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								if (
									!outcome.fileContents.includes('A | B') &&
									!outcome.fileContents.includes(
										'np.logical_or(A, B)',
									)
								) {
		...
		stest(
			{
				description: '/fix Tensorflow InvalidArgumentError',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								assert.ok(
									outcome.fileContents.includes('reshape'),
								);
							},
						},
					],
		...
		stest(
			{
				description: '/fix Tensorflow model has not yet been built',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
		...
		stest(
			{
				description: '/fix numpy, unsupported operand types',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
		...
		stest(
			{
				description: '/fix name conflict with builtin function',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								assert.ok(
									!outcome.fileContents.includes('max = 0'),
								);
							},
						},
					],
		...
		stest(
			{
				description: "/fix AttributeError: can't set attribute",
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								assert.ok(
									outcome.fileContents.includes('@x.setter'),
								);
							},
						},
					],
		...
		stest(
			{
				description:
					'/fix TypeError: Index does not support mutable operations',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
		...
		stest(
			{
				description: '/fix TypeError: str object is not an iterator',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								assert.ok(
									outcome.fileContents.includes('iter('),
								);
							},
						},
					],
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								assert.ok(
									outcome.fileContents.includes('float(') ||
										outcome.fileContents.includes('int(') ||
										outcome.fileContents.includes('str('),
								);
							},
						},
					],
		...
		stest(
			{
				description: "/fix Missing import, name 'array' is not defined",
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								assert.ok(
									outcome.fileContents.includes('np.array') ||
										outcome.fileContents.includes(
											'from numpy import',
										),
								);
		...
		stest(
			{
				description:
					'/fix can only concatenate list (not "str") to list',
				language: 'python',
			},
			(testingServiceCollection) => {
				return simulateInlineChat(testingServiceCollection, {
					...
					queries: [
						{
							...
							validate: async (outcome, workspace, accessor) => {
								assert.strictEqual(outcome.type, 'inlineEdit');
								assert.ok(
									outcome.fileContents.includes("['bar']") ||
										outcome.fileContents.includes(
											'foo.append',
										),
								);
								assert.ok(
									await isValidPythonFile(
										accessor,
										outcome.fileContents,
									),
								);
...
ssuite({ title: 'notebook', subtitle: 'fix', location: 'inline' }, () => {
	...
	stest(
		{ description: 'bad token', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: '(AML-10-58) not defined', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{
			description: '(AML-10-29) instance of bool has no to_string member',
			language: 'python',
		},
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: '(AML-8-110) not defined', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{
			description: '(AML-8-73) no value for argument in function call',
			language: 'python',
		},
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: 'undefined variable', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: 'general type issue', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: 'optional member access', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
	...
	stest(
		{ description: 'unbound variable', language: 'python' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
...
ssuite.optional(
	(opes) => !shouldEvaluate,
	{ title: 'notebook', subtitle: 'mbpp', location: 'inline' },
	() => {
		...
		for (const test of testScenarios) {
			...
			stest(
				{
					description: 'mbpp ' + task_id + ' ' + prompt,
					language: 'python',
				},
						...
						queries: [
							{
								...
								validate: async (
									...
									if (shouldEvaluate) {
										...
										await Promise.race([
											testTimeoutPromise,
											assertPythonCodeIsValid(
												accessor,
												outcome.fileContents,
												'Generated Code is not valid',
												'Generated not did not execute without errors',
											),
										]);
										...
										await Promise.race([
											...
											assertPythonCodeIsValid(
												accessor,
												codeWithTests,
												'Generated Code with Tests is not valid',
												'Generated did not pass the tests',
											),
										]);
									}
								},
...
function generateMBPPNotebookFixture(
	pathOrDirnameWithinFixturesDir: string,
	tests: string[],
) {
...
ssuite.optional(
	(opes) => !shouldEvaluate || true,
	{ title: 'notebook', subtitle: 'notebookEditsMbpp', location: 'panel' },
	() => {
		const mbppFile = fromFixture('notebook/filtered-mbpp.json');
		const mbppTests = JSON.parse(mbppFile.fileContents).tests;
		const testScenarios: {
		...
		for (const test of testScenarios) {
			...
			stest(
				...
				async (testingServiceCollection) => {
					const notebookFile = generateMBPPNotebookFixture(
						'notebook/mbpp.ipynb',
						test_list,
					);
						...
						async onBeforeStart(accessor) {
							(
								accessor.get<IAlternativeNotebookContentService>(
									IAlternativeNotebookContentService,
								) as MockAlternativeNotebookContentService
							).format = AlternativeNotebookFormat.json;
						},
						queries: [
							{
									...
									if (shouldEvaluate) {
										...
										await Promise.race([
											...
											assertPythonCodeIsValid(
												...
												codeWithTests,
												'Generated Code with Tests is not valid',
												'Generated code did not pass the tests',
											),
										]);
									}
								},
...
async function assertPythonCodeIsValid(
	...
	const cellExecutesWithoutErrors = await canExecutePythonCodeWithoutErrors(
		accessor,
		pythonCode,
	);
	assert.ok(
		cellExecutesWithoutErrors,
		`${executionMessage}:${EOL}${pythonCode}`,
	);
}

async function getNotebookCellDiagnostics(
	accessor: ITestingServicesAccessor,
	workspace: SimulationWorkspace,
	method: DiagnosticProviderId | DiagnosticsProvider,
): Promise<ITestDiagnostic[]> {
	const files = workspace.documents
		.filter((doc) => doc.document.uri.scheme === Schemas.vscodeNotebookCell)
		.map((doc) => ({
...
async function assertNoCellDiagnosticsAsync(
	accessor: ITestingServicesAccessor,
	outcome: IOutcome,
	workspace: SimulationWorkspace,
	method: DiagnosticProviderId | DiagnosticsProvider,
	...
	assert.strictEqual(outcome.type, 'inlineEdit');
	const diagnostics = await getNotebookCellDiagnostics(
		accessor,
		workspace,
		method,
	);
