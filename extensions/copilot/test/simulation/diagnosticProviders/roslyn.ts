...
export class RoslynDiagnosticsProvider extends CachingDiagnosticsProvider {
	...
	protected override async computeDiagnostics(
		...
		if (!this.isInstalled()) {
			throw new Error(
				'clang or dotnet must be available in this environment for csharp diagnostics.',
			);
		}
	...
	private runInDocker(
		...
		const spawnResult = cp.spawnSync('docker', args, {
			shell: true,
			encoding: 'utf-8',
		});
		if (spawnResult.status !== 0) {
			throw new Error(
				`Error while running '${command.join(' ')}' in docker : ${spawnResult.stdout} , ${spawnResult.stderr}`,
			);
		}
	...
	private runInLocal(command: string[]): string {
		const spawnResult = cp.spawnSync(command[0], command.slice(1), {
			shell: true,
			encoding: 'utf-8',
		});
		if (spawnResult.status !== 0) {
			throw new Error(
				`Error while running '${command.join(' ')}' in local OS : ${spawnResult.stderr}`,
			);
		}
		return spawnResult.stdout;
	}

	private async runDotnetCompiler(
		temporaryDirectory: string,
		files: IFile[],
	): Promise<ITestDiagnostic[]> {
		...
		if (this._isInstalled === 'docker') {
			...
			await fs.promises.writeFile(
				path.join(temporaryDirectory, `validate.sh`),
				script,
			);
			await fs.promises.writeFile(
				path.join(temporaryDirectory, `error_list.sarif`),
				'',
			); // pre-create the file so it gets the permissions of the current user and we can delete it after
			await fs.promises.writeFile(
				path.join(temporaryDirectory, `CSharp.ruleset`),
				this.rulesetFile,
			);
			await fs.promises.writeFile(
				path.join(temporaryDirectory, `proj.csproj`),
				this.csprojFile,
			);

			const basename = path.basename(temporaryDirectory);
			this.runInDocker(temporaryDirectory, basename, [
				'/bin/sh',
				`/${basename}/validate.sh`,
			]);
			...
			await fs.promises.writeFile(
				path.join(projectLocation, `${projectName}.csproj`),
				this.csprojFile,
			);
			await fs.promises.writeFile(
				path.join(projectLocation, `CSharp.ruleset`),
				this.rulesetFile,
			);
			this.runInLocal([
				'dotnet',
		...
		const fileContents = await fs.promises.readFile(
			path.join(outputLocation, `error_list.sarif`),
			'utf8',
		);
		...
		for (const error of parsedErrors) {
			const uri =
				error.locations[0].physicalLocation.artifactLocation.uri;
			const diagnostic = {
				file: path.basename(uri),
				message: error.message.text,
				code: error.ruleId,
				startLine:
					error.locations[0].physicalLocation.region.startLine - 1,
				startCharacter:
					error.locations[0].physicalLocation.region.startColumn - 1,
				endLine: error.locations[0].physicalLocation.region.endLine - 1,
				endCharacter:
					error.locations[0].physicalLocation.region.endColumn - 1,
