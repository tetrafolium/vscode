...
export abstract class CachingDiagnosticsProvider extends DiagnosticsProvider {
	...
	protected get cacheVersion(): number {
		return CACHING_DIAGNOSTICS_PROVIDER_CACHE_SALT;
	}

	override async getDiagnostics(
		...
		const cacheKey = computeSHA256(
			`${this.id}-v${this.cacheVersion}-${JSON.stringify(files)}`,
		);
	...
	protected abstract computeDiagnostics(
		files: IFile[],
	): Promise<ITestDiagnostic[]>;
}
...
export abstract class LintingDiagnosticsProvider extends CachingDiagnosticsProvider {
	protected override async computeDiagnostics(
		...
		for (const file of files) {
			...
			const spawnResult = spawnSync(command.command, command.arguments, {
				shell: true,
				encoding: 'utf-8',
				env: command.env,
			});
			const processedDiagnostics = this.processDiagnostics(
				file.fileName,
	...
	protected abstract fetchCommand(
	...
	protected abstract processDiagnostics(
		fileName: string,
		stdoutResult: any,
	): ITestDiagnostic[];
}

export async function setupTemporaryWorkspace(
	workspacePath: string,
	_files: IFile[],
): Promise<{ filePath: string; fileName: string; fileContents: string }[]> {
	...
	for (const file of files) {
		await fs.promises.mkdir(path.dirname(file.filePath), {
			recursive: true,
		});
...
export function convertTestToVSCodeDiagnostics(
	diagnostics: ITestDiagnostic[],
...
export function findIfInstalled(
	verificationCommand: { command: string; arguments: string[] },
