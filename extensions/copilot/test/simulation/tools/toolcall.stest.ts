...
const toolArgsPreprocessors: Record<string, ArgsPreprocessor> = {
	get_errors: (
		_accessor: ITestingServicesAccessor,
		args: any,
		workspaceFoldersFilePaths?: string[],
	) => {
		const filePaths = (args.filePaths ?? []).map((filePath: string) => {
			...
			return workspaceFoldersFilePaths &&
				workspaceFoldersFilePaths.length > 0
...
ssuite(
	{ title: 'tooltest', subtitle: 'toolcall', location: 'panel' },
	(inputPath) => {
		...
		for (const scenario of scenarios) {
			...
			if (scenario.json.outputPath) {
				outputFilePath = path.resolve(
					toolCallsFolder,
					scenario.json.outputPath,
				);
			}

			stest(
				{ description: scenario.name },
				async (testingServiceCollection: TestingServiceCollection) => {
					try {
						...
						const simulationWorkspace = isInExtensionHost
							? new SimulationWorkspaceExtHost()
							: new SimulationWorkspace();
						simulationWorkspace.setupServices(
							testingServiceCollection,
						);
						...
						if (scenario.stateFilePath) {
							const stateJson = await fs.promises.readFile(
								scenario.stateFilePath,
								'utf-8',
							);
							const state = JSON.parse(stateJson);
							const stateFileDir = path.dirname(
								scenario.stateFilePath,
							);
							if (state.workspaceFoldersFilePaths) {
								workspaceFoldersFilePaths =
									state.workspaceFoldersFilePaths.map(
										(folder: string) => {
		...
		async function invokeTool(
			...
			if (toolArgsPreprocessors[toolName]) {
				processedArgs = await toolArgsPreprocessors[toolName](
					accessor,
					args,
					workspaceFoldersFilePaths,
				);
			...
			if (tool.resolveInput) {
				...
				processedArgs = await tool.resolveInput(
					processedArgs,
					context,
