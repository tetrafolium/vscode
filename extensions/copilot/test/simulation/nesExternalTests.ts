...
import {
	EditNotScoredError,
	InlineEditTester,
} from './inlineEdit/inlineEditTester';
...
export async function discoverNesTests(
	rootFolder: string,
...
function generateExternalStestFromRecording(
	...
	const stest = new SimulationTest(
		...
		async (collection) => {
			...
			const { isScored, result, scoredEditsFilePath } =
				await tester.runAndScoreFromRecording(
					accessor,
					loadFile({ filePath: path.join(fileDir, basename) }),
				);

			accessor
				.get(ISimulationTestRuntime)
				.writeFile(
					`${testName}.textAfterAiEdit.txt`,
					result.textAfterAiEdit?.value ?? '<NO AI EDIT>',
					'textAfterAiEdit',
				);
			accessor
				.get(ISimulationTestRuntime)
				.writeFile(
					`${testName}.aiEdit.json`,
					result.nextEdit === undefined
						? '<NO AI EDIT>'
						: JSON.stringify(
								result.nextEdit
									? serializeSingleEdit(result.nextEdit)
									: undefined,
							),
					'nextEdit',
				);
...
function computeTestNameFromFile(file: fs.Dirent<string>): string {
	...
	if (basename === RECORDING_BASENAME) {
		...
		assert(
			...
			`Expected recording's ${path.join(file.parentPath, file.name)} parent directory name to be defined`,
		);

		if (pathChunks.at(-2)?.[0].match(/^[A-Z]/)) {
			// if the recording is at `path/to/MustHave/MyAwesomeTest/recording.w.json` - test name should be `[MustHave] MyAwesomeTest`
