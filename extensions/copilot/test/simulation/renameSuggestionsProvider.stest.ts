...
import {
	...
	NamingConvention,
} from '../../src/extension/renameSuggestions/common/namingConvention';
...
function offsetRangeToPositionRange(
...
ssuite({ title: 'Rename suggestions', location: 'external' }, () => {
	 ...
	 */
	function assertIncludesLowercased(
		newSymbolNames: vscode.NewSymbolName[],
		searchStrings: string | string[],
	) {
		searchStrings = Array.isArray(searchStrings)
			? searchStrings
			: [searchStrings];
		...
		for (const symbol of newSymbolNames) {
			...
			assert.ok(
				searchStrings.some((searchString) =>
					newSymbolNameLowercase.includes(searchString),
				),
	...
	function countMatches(
		...
		return newSymbolNames.filter((symbol) =>
			symbol.newSymbolName
				.toLowerCase()
				.includes(searchStringsLowercased),
		).length;
	}
	...
	async function provideNewSymbolNames(
		...
		try {
			...
			const renameRange = offsetRangeToPositionRange(
				annotatedRange,
				document,
			);
			...
			const provider = accessor
				.get(IInstantiationService)
				.createInstance(AlwaysEnabledNewSymbolNamesProvider);

			const symbols = await provider.provideNewSymbolNames(
				document,
				renameRange,
	...
	stest(
		...
		`;
			...
			const symbols = await provideNewSymbolNames(
			...
			assert.ok(
				countMatches(symbols, 'fib') >=
					Math.floor(symbols.length * 0.8),
				'Expected 80% of symbols to include fib: ' +
					JSON.stringify(symbols.map((s) => s.newSymbolName)),
			);
		},
	);

	stest(
		'rename follows naming convention _ - rename a function (with underscore) at its definition',
		async (testingServiceCollection) => {
			const fileContents = outdent`
		...
		`;
			...
			assertLength(symbols);
			assert.ok(
				symbols.some((s) => s.newSymbolName.startsWith('_')),
				'Expected to include symbols with underscore',
			);
	...
	stest(
		'rename a variable reference within a function',
		async (testingServiceCollection) => {
			const fileContents = outdent`
	...
	stest(
		'rename a SCREAMING_SNAKE_CASE enum member',
		async (testingServiceCollection) => {
			const fileContents = outdent`
			enum LoadStatus {
				NOT_LOADED,
		...
		`;
			...
			assert.ok(
				symbols.every(
					(symbol) =>
						guessNamingConvention(symbol.newSymbolName) ===
						NamingConvention.ScreamingSnakeCase,
				),
				'Expected all symbols to be SCREAMING_SNAKE_CASE',
			);
		},
	);

	stest(
		'respect context: infer name based on existing code - enum member',
		async (testingServiceCollection) => {
			const fileContents = outdent`
			enum Direction {
				UP,
			...
			assert.ok(
				symbols.every((symbol) =>
					[
						...
						NamingConvention.ScreamingSnakeCase,
					].includes(guessNamingConvention(symbol.newSymbolName)),
				),
				'Expected all symbols to be SCREAMING_SNAKE_CASE or UPPERCASE',
			);
		},
	);

	stest(
		'rename a function call - definition in same file',
		async (testingServiceCollection) => {
			const fileContents = outdent`
	...
	stest(
		'rename a function call - definition in different file',
		async (testingServiceCollection) => {
	...
	stest('rename type definition', async (testingServiceCollection) => {
		...
		const symbols = await provideNewSymbolNames(testingServiceCollection, [
			file,
		]);
		...
		assert.ok(
			symbols.some((s) =>
				s.newSymbolName.toLowerCase().includes('person'),
			),
			'Inludes person',
		);
	});
	...
	stest(
		'rename type definition when it is used in the same file',
		async (testingServiceCollection) => {
			const fileContents = outdent`
			type <<t>> = {
				firstName: string;
	...
	stest(
		'rename type reference - same file',
		async (testingServiceCollection) => {
			const fileContents = outdent`
			type t = {
				firstName: string;
	...
	stest(
		'rename type reference - same file with 2 possible defs',
		async (testingServiceCollection) => {
			const fileContents = outdent`
			type t = {
				firstName: string;
	...
	stest(
		'rename class reference - same file',
		async (testingServiceCollection) => {
			const fileContents = outdent`
			class P {
				firstName: string;
	...
	stest(
		'rename method with field-awareness',
		async (testingServiceCollection) => {
			const fileContents = outdent`
			class Processor {
		...
		`;
			...
			assert.ok(
				symbols.some((s) =>
					s.newSymbolName.toLowerCase().includes('stdout'),
				),
				'Knows about `stdoutBuffer`',
			);
		},
	);
	...
	stest('non-tree-sitter language', async (testingServiceCollection) => {
		...
		assert.ok(
			symbols.some((s) => s.newSymbolName.toLowerCase().includes('fib')),
			'Includes fib',
		);
	});
	...
	stest('rename class name - CSS', async (testingServiceCollection) => {
		...
		assert.ok(
			symbols.every((s) => s.newSymbolName.match(/^\.([a-zA-Z]+)/)),
			'All symbols are class names',
		);
	});
});
