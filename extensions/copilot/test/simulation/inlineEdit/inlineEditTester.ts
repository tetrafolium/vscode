...
import {
	...
	NES_LOG_CONTEXT_TAG,
	NES_USER_EDITS_HISTORY_TAG,
	NEXT_EDIT_SUGGESTION_TAG,
...
export class EditNotScoredError extends CriticalError {
	constructor(scoredEditsFilePath: string) {
		super(
			`Edit is not scored yet in \n${scoredEditsFilePath}\n! Please manually score it and run the test again.`,
		);
	}
}

export type NextCursorPosition = /* lineNumber (1-based) */
	| number
	| { cursorOffset: number };

export class InlineEditTester {
	constructor(private readonly _includeNextEditSelection: boolean = false) {}

	private readonly _renameQueues = new CachedFunction(
		{ getCacheKey: JSON.stringify },
		(arg: unknown) => new TaskQueue(),
	);

	async runAndScoreTestFromRecording(
		collection: TestingServiceCollection,
		recording: ILoadedFile,
	): Promise<void> {
		const { isScored, scoredEditsFilePath } =
			await this.runAndScoreFromRecording(collection, recording);
		if (!isScored) {
			throw new EditNotScoredError(scoredEditsFilePath);
		}
	}

	async runAndScoreFromRecording(
		collectionOrAccessor: TestingServiceCollection | ServicesAccessor,
		recording: ILoadedFile,
	): Promise<{
		result: IInlineEditTestRunResult;
		scoredEditsFilePath: string;
		isScored: boolean;
	}> {
		const accessor =
			collectionOrAccessor instanceof TestingServiceCollection
				? collectionOrAccessor.createTestingAccessor()
				: collectionOrAccessor;
		const recordingData = JSON.parse(
			recording.fileContents,
		) as IRecordingInformation;
		const result = await this.runTestFromRecording(accessor, recordingData);
		const testInfo = accessor.get(ITestInformation);

		const scoredEditsFilePath = await this._renameQueues
			.get([testInfo.testFileName, recording.filePath])
			.schedule(() =>
				getScoredEditsFilePath(testInfo, recording.filePath),
			);

		const score = await inlineEditScoringService.scoreEdit(
			scoredEditsFilePath,
			{ kind: 'recording', recording: result.recordingData },
			result.aiEditDocumentUri,
			result.aiEditDocumentValue,
			result.aiRootedEdit,
		);
	...
	async runTestFromRecording(
		accessor: ServicesAccessor,
		recordingData: IRecordingInformation,
	): Promise<IInlineEditTestRunResult> {
		const replayer = new ObservableWorkspaceRecordingReplayer(
			recordingData,
			this._includeNextEditSelection,
		);
		const obsGit = accessor
			.get(IInstantiationService)
			.createInstance(ObservableGit);
		...
		const expectedEdit = deserializeStringEdit(
			recordingData.nextUserEdit?.edit ??
				serializeStringEdit(StringEdit.empty),
		);
		const result = await this._runTest(
			accessor,
			lastDocId,
			replayer.workspace,
	...
	private async _runTest(
		...
		nesXtabHistoryTracker: NesXtabHistoryTracker,
		...
		if (stestRuntime) {
			const nesUserEditHistory: ISerializedNesUserEditsHistory = {
				edits: history.documents.map(
					(doc): ISerializedFileEdit => ({
			...
			stestRuntime.writeFile(
				'nesUserEditHistory.json',
				JSON.stringify(nesUserEditHistory, null, 2),
				NES_USER_EDITS_HISTORY_TAG,
			);
		}
		...
		const telemetryBuilder = new NextEditProviderTelemetryBuilder(
			...
			workspaceService,
			nextEditProvider.ID,
			workspace.getDocument(activeDocument.docId)!,
		);

		let nextEditResult: NextEditResult;
		try {
			nextEditResult = await nextEditProvider.getNextEdit(
		...
		if (stestRuntime) {
			stestRuntime.writeFile(
				'nesLogContext.json',
				JSON.stringify(logDocument, null, 2),
				NES_LOG_CONTEXT_TAG,
			);
		}
		...
		const targetDocument =
			targetDocId !== undefined
				? assertReturnsDefined(historyContext.getDocument(targetDocId))
				: activeDocument;

		const aiRootedEdit = new RootedEdit(
			targetDocument.lastEdit.getEditedState(),
			nextEditResult.result?.edit?.toEdit() ?? StringEdit.empty,
		);
		...
		if (stestRuntime) {
			...
			stestRuntime.writeFile(
				'proposedNextEdit.json',
				JSON.stringify(serializedNextEdit, null, 2),
				NEXT_EDIT_SUGGESTION_TAG,
			);
		}
...
function getLanguageIdFromDocumentId(docId: DocumentId): string {
	return getLanguageForResource(URI.file(`/path/file.${docId.extension}`))
		.languageId;
}
...
async function getScoredEditsFilePath(
	test: ITestInformation,
	recordingFilePath: string | undefined,
): Promise<string> {
	const paths: string[] = [];
	...
	if (test.testFileName !== undefined) {
		...
		const filePath = join(
			testDirName,
			'scores',
			sanitizeFileName(stripTestFlavor(test.fullTestName)) +
				'.scoredEdits.w.json',
		);
		paths.push(filePath);
	}
	...
	if (recordingFilePath !== undefined) {
		const path = recordingFilePath.replace(
			'recording.w.json',
			'scoredEdits.w.json',
		);
	...
	for (let i = 0; i < paths.length; i++) {
		if (i === paths.length - 1) {
			...
			if (
				(await fileExists(paths[i])) &&
				!(await fileExists(paths[i + 1]))
			) {
