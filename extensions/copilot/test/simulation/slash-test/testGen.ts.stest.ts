...
forInline((strategy, nonExtensionConfigurations, suffix) => {
	ssuite(
		...
		() => {
			stest(
				{ description: 'can add a test after an existing one' },
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						...
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/simple-ts-proj-with-test-file/src/index.ts',
								),
								fromFixture(
									'tests/simple-ts-proj-with-test-file/src/test/index.test.ts',
								),
							],
							queries: [
								{
									...
									validate: async (
										...
										const sixthLine = getFileContent(
											changedFile,
										)
											.split(/\r\n|\r|\n/g)
											.at(6);
										...
										assert(
											sixthLine !== '});',
											`new tests are inserted within the existing suite: expected NOT '});'`,
										);
									},
								},
							],
			...
			stest(
				{
					description:
						'can add a test after an existing one with empty line',
				},
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/simple-ts-proj-with-test-file-1/src/index.ts',
								),
								fromFixture(
									'tests/simple-ts-proj-with-test-file-1/src/test/index.test.ts',
								),
							],
							queries: [
								{
			...
			stest(
				{ description: 'supports chat variables' },
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/simple-ts-proj/src/index.ts',
								),
			...
			stest(
				{ description: 'BidiMap test generation (inside file)' },
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/generate-for-selection',
									'base/test/common/map.test.ts',
								),
								fromFixture(
									'tests/generate-for-selection',
									'base/common/map.ts',
							...
							queries: [
								{
									...
									validate: async (
										...
										assertNoStrings(getFileContent(first), [
											'import',
										]);
									},
								},
			...
			stest(
				{ description: 'BidiMap test generation (inside test)' },
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
							...
							queries: [
								{
									...
									validate: async (
										...
										assert.ok(
											outcome.appliedEdits.length >= 1,
										);

										assert.ok(
											outcome.appliedEdits.some(
												(edit) =>
							...
							queries: [
								{
									...
									validate: async (
										...
										assert.ok(
											outcome.appliedEdits.some((edit) =>
												edit.newText.match(
													/test\(.*findScrollWidgetState/,
												),
											),
										);
									},
...
forInline((strategy, nonExtensionConfigurations) => {
	ssuite(
		...
		() => {
				...
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						...
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/ts-leading-whitespace/charCode.ts',
								),
								fromFixture(
									'tests/ts-leading-whitespace/strings.ts',
								),
							],
							queries: [
								{
			...
			stest(
				{ description: 'issue #3699: add test for function' },
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/for-method-issue-3699/foldingRanges.ts',
								),
							],
							queries: [
								{
			...
			stest(
				{ description: 'issue #3701: add some more tests for folding' },
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/in-suite-issue-3701/notebookFolding.test.ts',
								),
							],
							queries: [
								{
									...
									validate: async (
										...
										return assertContainsAllSnippets(
											text,
											['withTestNotebook', 'assert'],
											'tests/in-suite-issue-3701',
										);
									},
								},
							],
			...
			stest(
				'add another test for containsUppercaseCharacter with other non latin chars',
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/another-unit-test/strings.test.ts',
								),
								fromFixture(
									'tests/another-unit-test/charCode.ts',
								),
								fromFixture(
									'tests/another-unit-test/strings.ts',
								),
							],
							queries: [
								{
	...
	ssuite(
		...
		function () {
			const testGenConfigOnly = [
				{
					...
					value: [
						...
						{
							text: 'use TDD instead of BDD',
							language: 'typescript',
						},
						{
							text: 'use ssuite instead of suite and stest instead of test',
							language: 'typescript',
						},
					],
				},
			...
			stest(
				{
					description:
						'[test gen config] can add a test after an existing one with empty line',
					configurations: testGenConfigOnly,
				},
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
						testingServiceCollection,
						{
							files: [
								fromFixture(
									'tests/simple-ts-proj-with-test-file-2/src/index.ts',
								),
								fromFixture(
									'tests/simple-ts-proj-with-test-file-2/src/test/index.test.ts',
								),
							],
							queries: [
								{
			...
			const codeGenAndTestGenConfig = [
				{
					...
					value: [{ text: `Add a comment: 'Generated by Copilot'` }],
				},
				{
					key: ConfigKey.TestGenerationInstructions,
					value: [
			...
			stest(
				{
					description:
						'[code gen + test gen config] can add a test after an existing one with empty line',
					configurations: codeGenAndTestGenConfig,
				},
				(testingServiceCollection) => {
					return simulateInlineChatWithStrategy(
						strategy,
...
ssuite(
	{ title: `/tests`, location: 'panel', language: 'typescript' },
	function () {
		{
			const root = join(
				__dirname,
				'../test/simulation/fixtures/tests/panel/tsq',
			);
			const path = join(root, 'workspaceState.state.json');

			stest(
				'can consume #file without active editor',
					...
					[
						{
							...
							stateFile: path,
							getState: () =>
								deserializeWorkbenchState(dirname(path), path),
						},
					],
					async (accessor, question, answer) => {
						try {
							assert.ok(
								['test', 'suite', 'describe', 'it'].some((x) =>
									answer.includes(x),
								),
							...
							assert.ok(
								answer.includes('subtract') ||
									answer.includes('add') ||
									answer.includes('multiply'),
