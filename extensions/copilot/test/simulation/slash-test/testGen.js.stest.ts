...
ssuite({ title: '/tests', location: 'inline', language: 'js' }, () => {
	...
	stest(
		{ description: 'add another test to existing file' },
	...
	stest(
		{ description: '/tests: with package.json info' },
		(testingServiceCollection) => {
			return simulateInlineChat(testingServiceCollection, {
				files: [
	...
	stest(
		{
			description:
				'issue #1261: Failed to create new test file when in an untitled file',
		},
		(testingServiceCollection) => {
				...
				queries: [
					{
						...
						validate: async (outcome, workspace, accessor) => {
							...
							// Otherwise if it's the new format (with srcUri)
								...
								assert.ok(
									file.srcUri.startsWith('untitled:'),
									'URI should be untitled scheme',
								);
							}
