...
const liftLSIFRange = (range: LSIF.types.Range): Range =>
	new Range(
		range.start.line,
...
const liftLSIFLocations = (
	locations: undefined | LSIF.types.Location | LSIF.types.Location[],
): vscode.Location[] => {
	if (!locations) {
		return [];
	}
	...
	return arr.map(
		(l) => new Location(URI.parse(l.uri), liftLSIFRange(l.range)),
	);
};
...
class SCIPGraph implements IGraph {
	...
	constructor(
		...
		this.workspaceUriLower = workspace.workspaceFolders[0]
			.toString(true)
			.toLowerCase();
	}
	...
	declarations(
		uri: string,
		position: LSIF.types.Position,
	): LSIF.types.Location | LSIF.types.Location[] | undefined {
		// https://github.com/sourcegraph/scip/blob/0504a347d36dbff48b21f53ccfedb46f3803855e/scip.proto#L501
		return this.findOccurencesOfSymbolAt(
			uri,
			position,
			(o) => !!(o.symbolRoles & 0x1),
		);
	...
	references(
		uri: string,
		position: LSIF.types.Position,
		context: LSIF.types.ReferenceContext,
	): LSIF.types.Location[] | undefined {
		return this.findOccurencesOfSymbolAt(uri, position, () => true);
	}

	private findOccurencesOfSymbolAt(
		uri: string,
		position: LSIF.types.Position,
		filter: (o: scip.Occurrence) => boolean,
	): LSIF.types.Location[] {
		...
		for (const doc of this.index.documents) {
			for (const occurence of doc.occurrences) {
				if (
					occurence.symbolRoles & 0x1 &&
					toFind.has(occurence.symbol)
				) {
					// definition
					toFind.delete(occurence.symbol);
					locations.push({
						uri: URI.joinPath(
							this.workspaceUriOriginal,
	...
	private getDoc(uriInWorkspace: string) {
		...
		return this.index.documents.find(
			(d) =>
				d.relativePath.replaceAll('\\', '/').toLowerCase() ===
				uriInWorkspace,
		);
	}
}
...
const makeTranslator = (workspace: SimulationWorkspace, indexRoot: string) => {
	...
	if (lastIndex === -1) {
		throw new Error(
			`Index path ${indexPath} does not contain 'vscode-copilot', please ensure the index is generated in the correct workspace`,
		);
	}

	const subdir = indexPath.slice(lastIndex + REPO_NAME.length + 1);
	const localRootRe = new RegExp(
		`^file:\\/\\/.*?${escapeRegExpCharacters(subdir.replaceAll('\\', '/'))}`,
		'i',
	);

	return {
		fromDatabase: (uri: string) =>
			uri.replace(localRootRe, simulationRootUri),
		toDatabase: (uri: string) =>
			uri.startsWith(simulationRootUri)
				? uri.replace(simulationRootUri, indexRoot)
				: uri,
	};
};
...
export class LSIFLanguageFeaturesService implements ILanguageFeaturesService {
	...
	private async _load(): Promise<IGraph> {
		...
		try {
			await graph.load(this.indexFilePath, (r) =>
				makeTranslator(this.workspace, r),
			);
		} catch (e) {
			throw new Error(
				`Failed to parse LSIF index from ${this.indexFilePath}: ${e}`,
			);
		}
		return graph;
	}
	...
	async getDefinitions(
		...
		return liftLSIFLocations(
			graph.definitions(uri.toString(true), position),
		);
	}
	...
	async getImplementations(
		...
		const graph = await this._getGraph();
		return liftLSIFLocations(
			graph.declarations(uri.toString(true), position),
		);
	}
	...
	async getReferences(
		...
		const graph = await this._getGraph();
		return liftLSIFLocations(
			graph.references(uri.toString(true), position, {
