...
class TSServerRPC {
	...
	private _awaitingResponse: Map<
		number /* seq id */,
		DeferredPromise<ts.server.protocol.Response>
	>;

	private _stdoutBuffer: string;

	constructor(private readonly _server: cp.ChildProcess) {
	...
	private _tryProcessStdoutBuffer() {
		do {
			...
			// try parse body
			const body = this._stdoutBuffer.substring(
				eolIndex + 4,
			...
			this._stdoutBuffer = this._stdoutBuffer.substring(
				eolIndex + 4 + contentLength,
			);

			this._handleServerMessage(
				JSON.parse(body) as ts.server.protocol.Message,
			);
		} while (true);
	}
	...
	private _handleServerMessage(msg: ts.server.protocol.Message) {
		switch (msg.type) {
			...
			case 'response': {
				...
				if (respP === undefined) {
					console.error(
						`received response for unexpected seq ${resp.request_seq}`,
					);
				} else {
					respP.complete(resp);
				}
...
export class TSServerClient {
	...
	constructor(
		private readonly _workspaceFiles: {
			fileName: string;
			fileContents: string;
		}[],
	) {
	...
	private async _init() {
		this._initPromise ??= (async () => {
			const { workspacePath, files } = await this._setUp(
	...
	async find(
		...
		const locations =
			command === ts.server.protocol.CommandTypes.DefinitionAndBoundSpan
				...
				: response.body.refs;
		const workspacePathWithSlash = path.join(
		...
		for (const location of locations) {
			if (
				path.normalize(location.file).startsWith(workspacePathWithSlash)
			) {
				...
				const fileName = location.file.substring(
					workspacePathWithSlash.length,
				);
	...
	private async _setUp(
		_files: { fileName: string; fileContents: string }[] = [],
	) {
		...
		const hasTSConfigFile = files.some(
