...
(['xml', 'json', 'text'] as const).forEach((format) => {
	...
	ssuite(
		{
			title: `notebookEdits`,
			subtitle: `modification - ${format}`,
			location: 'panel',
		},
		() => {
			stest(
				{ description: 'code cell modification', language: 'python' },
						...
						queries: [
							{
								...
								validate: async (
									...
									const notebookDocument =
										workspace.getNotebookDocuments()[0];
									if (!notebookDocument) {
									...
									assert.ok(
										cell.document
											...
											.toLowerCase()
											.indexOf('"""') > 0,
										`docstring not found in ${cell.document.getText()}`,
									);
								},
							},
						],
			...
			stest(
				{ description: 'code cell insertion', language: 'python' },
				async (testingServiceCollection) => {
					const file = fromFixture('notebook/edits/single.ipynb');
					return simulatePanelCodeMapperEx(testingServiceCollection, {
						files: [file],
			...
			stest(
				{
					description: 'code cell modification, plotting',
					language: 'python',
				},
				async (testingServiceCollection) => {
			...
			stest(
				{
					description:
						'code cell modification, convert Point2D code to Point3D',
					language: 'python',
				},
				async (testingServiceCollection) => {
								...
								validate: async (
									...
									assert.ok(
										notebookDocument
											...
											.document.getText()
											.includes(
												'distance_from_origin(point: Point3D)',
											) ||
											notebookDocument
												.cellAt(1)
												.document.getText()
										...
										'distance_from_origin not updated',
									);
								},
							},
						],
			...
			stest(
				{
					description: 'cell refactoring, plot refactoring',
					language: 'python',
				},
				async (testingServiceCollection) => {
					const file = fromFixture(
						'notebook/edits/data_visualization.ipynb',
					);
					return simulatePanelCodeMapperEx(testingServiceCollection, {
						files: [file],
						queries: [
								...
								validate: async (
									...
									assert.ok(
										notebookDocument
											...
											.document.getText()
											.includes('title'),
										`Should contain 'title' statements: ${notebookDocument.cellAt(5).document.getText()}`,
									);
									assert.ok(
										notebookDocument
											.cellAt(7)
											.document.getText()
											.includes('title'),
										`Should contain 'title' statements: ${notebookDocument.cellAt(7).document.getText()}`,
									);
									assert.ok(
										notebookDocument
											.cellAt(9)
											.document.getText()
											.includes('title'),
										`Should contain 'title' statements: ${notebookDocument.cellAt(9).document.getText()}`,
									);
								},
							},
						],
			...
			stest(
				{
					description: 'new code cells in empty notebook',
					language: 'python',
				},
				async (testingServiceCollection) => {
			...
			stest(
				{
					description: 'new julia code cells in empty notebook',
					language: 'julia',
				},
				async (testingServiceCollection) => {
					const file = fromFixture(
						'notebook/edits/empty_julia.ipynb',
					);
					return simulatePanelCodeMapperEx(testingServiceCollection, {
						files: [file],
						queries: [
			...
			stest(
				{
					description: 'notebook code cell deletion',
					language: 'python',
				},
				async (testingServiceCollection) => {
						...
						queries: [
							{
								...
								validate: async (
									...
									assert.ok(
										notebookDocument.cellCount === 2,
										'Should have 2 cells remaining after deletion',
									);
								},
							},
						],
			...
			stest(
				{
					description:
						're-organize python imports to top of the notebook',
					language: 'python',
				},
				async (testingServiceCollection) => {
					const file = fromFixture(
						'notebook/edits/data_visualization_2.ipynb',
					);
					return simulatePanelCodeMapperEx(testingServiceCollection, {
						files: [file],
						queries: [
							{
								...
								validate: async (
									...
									const firstCodeCell = notebookDocument
										.getCells()
										.filter(
											(cell) =>
												cell.kind ===
									...
									assert.ok(
										firstCodeCell.document
											.getText()
											.includes('import pandas as pd'),
										'pandas not imported',
									);
									assert.ok(
										firstCodeCell.document
									...
									assert.ok(
										firstCodeCell.document
											.getText()
											.includes('import seaborn as sns'),
										'seaborn not imported',
									);
								},
							},
						],
			...
			stest(
				{
					description: 'Insert markdown cells explaining code',
					language: 'python',
				},
				async (testingServiceCollection) => {
						...
						queries: [
							{
								...
								validate: async (
									...
									const markdownCells = notebookDocument
										.getCells()
										.filter(
											(cell) =>
												cell.kind ===
									...
									assert.ok(
										...
										`Should have a markdown cell with 'filter issues'`,
									);
									assert.ok(
										markdownCells.some((md) =>
											md.document
												.getText()
												.toLowerCase()
												.includes('assignee'),
										),
										`Should have a markdown cell with 'assignee'`,
									);
									assert.ok(
										markdownCells.some((md) =>
											md.document
										...
										`Should have a markdown cell with 'label'`,
									);
								},
							},
						],
			...
			stest(
				{
					description: 'code cell modification & insertion',
					language: 'python',
				},
				async (testingServiceCollection) => {
					const file = fromFixture('notebook/edits/multicells.ipynb');
			...
			stest(
				{
					description: 'code cell modification & deletion',
					language: 'python',
				},
				async (testingServiceCollection) => {
					const file = fromFixture('notebook/edits/multicells.ipynb');
			...
			stest(
				{
					description:
						'code cell modification with removal of unused imports',
					language: 'python',
				},
				async (testingServiceCollection) => {
			...
			stest(
				{ description: 'code cell re-ordering', language: 'python' },
				async (testingServiceCollection) => {
			...
			stest(
				{
					description:
						'code cell refactoring, modification, insertion & delection of cells',
					language: 'python',
				},
				async (testingServiceCollection) => {
					const file = fromFixture(
						'notebook/edits/matplotlib_to_plotly.ipynb',
					);
					return simulatePanelCodeMapperEx(testingServiceCollection, {
						files: [file],
						queries: [
							{
								...
								validate: async (
									...
									const codeCells = notebookDocument
										.getCells()
										.filter(
											(c) =>
												c.kind ===
									...
									assert.ok(
										markdownCells.length > 1,
										`Should have at least 2 markdown cells, got ${markdownCells.length}`,
									);
									assert.ok(
										codeCells.some((c) =>
											c.document
										...
										`Should have a code cell with a pie chart, got ${codeCells.map((c) => c.document.getText()).join(',')}`,
									);
								},
							},
						],
	...
	ssuite(
		{
			title: 'notebookEdits',
			subtitle: `bug reports - ${format}`,
			location: 'panel',
		},
		() => {
			stest(
				{ description: 'Issue #13868' },
				async (testingServiceCollection) => {
					try {
						await simulatePanelCodeMapperEx(
							testingServiceCollection,
							{
								files: [
									toFile({
										fileName:
											'multiFile/issue-13868/data.csv',
										fileContents: [
