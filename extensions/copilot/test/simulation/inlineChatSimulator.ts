...
} from '../../src/extension/inlineChat/node/promptCraftingTypes';
import { ICopilotChatResult } from '../../src/extension/prompt/common/conversation';
import {
	ChatParticipantRequestHandler,
	IChatAgentArgs,
} from '../../src/extension/prompt/node/chatParticipantRequestHandler';
...
} from '../../src/platform/test/node/services';
import {
	IFile,
	isNotebook,
	SimulationWorkspace,
...
import {
	ChatRequestTurn,
	ChatResponseTurn,
} from '../../src/util/common/test/shims/chatTypes';
...
} from '../../src/util/common/test/shims/textDocument';
...
import {
	commonPrefixLength,
	commonSuffixLength,
} from '../../src/util/vs/base/common/strings';
...
export type SimulationWorkspaceInput =
	| { files: IFile[]; workspaceFolders?: Uri[] }
...
export function setupSimulationWorkspace(
	...
	input: SimulationWorkspaceInput,
): SimulationWorkspace {
	const workspace = isInExtensionHost
		? new SimulationWorkspaceExtHost()
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(SimulationLanguageFeaturesService, [workspace]),
	);
	return workspace;
}
...
function isDeserializedWorkspaceStateBasedScenario(
	scenario: IScenario,
): scenario is IDeserializedWorkspaceStateBasedScenario {
	return 'workspaceState' in scenario;
}

export function simulateInlineChatWithStrategy(
...
export async function simulateInlineChatIntent(
	...
	const host: EditingSimulationHost = {
		...
		contributeAdditionalReferences(accessor, existingReferences) {
			...
			const diagnostics = diagnosticService.getDiagnostics(
				editor.document.uri,
			);
			...
			for (const d of diagnostics) {
				if (d.range.intersection(editor.selection)) {
					result.push({
						...
						name: d.message,
						value: new ChatReferenceDiagnostic2(
							editor.document.uri,
							d,
						),
					});
...
export async function simulateEditingScenario(
	...
	assert(
		isDeserializedWorkspaceStateBasedScenario(scenario) ||
			scenario.files.length > 0,
		`Cannot simulate scenario with no files`,
	);
	...
	const changedDocsInitialStates = new Map<
		vscode.TextDocument,
		Promise<IWorkspaceStateFile> | null
	>();
	...
	try {
		...
		for (const query of scenario.queries) {
			...
			if (query.visibleRanges) {
				workspace.setCurrentVisibleRanges(
					query.visibleRanges.map((range) => toRange(range)),
				);
			}

			if (query.activeCell) {
				const cellSelection = new NotebookRange(
					query.activeCell,
					query.activeCell + 1,
				);
			...
			const activeEditor = accessor.get(
				ITabsAndEditorsService,
			).activeTextEditor;
			if (query.file && !activeEditor) {
				throw new Error(
					`query.file is defined but no editor is active`,
				);
			}
			...
			if (typeof query.diagnostics === 'string') {
				...
				if (!activeEditor) {
					throw new Error(
						`diagnostics can only be an array if there's an active editor (is 'file' specified?)`,
					);
				}
			...
			if (query.fileIndentInfo) {
				...
				workspace.setCurrentDocumentIndentInfo(
					guessFileIndentInfo(activeEditor.document),
				);
			}
			if (isFirst && activeEditor) {
				...
				if (isNotebook(documentUri)) {
					...
					if (!notebookDocument) {
						throw new Error(
							`Missing notebook document ${documentUri}`,
						);
					}
					...
					relativeDiskPath = await testRuntime.writeFile(
						...
						INLINE_INITIAL_DOC_TAG,
					); // using .txt instead of real file extension to avoid breaking automation scripts
				} else {
					relativeDiskPath = await testRuntime.writeFile(
						workspacePath + '.txt',
			...
			try {
				const parsedQuery = parseQueryForTest(
					accessor,
					prompt,
					workspace,
				);
				...
				for (const variable of parsedQuery.variables) {
					...
					if (
						!seenFiles.find(
							(ref) =>
								URI.isUri(ref.value) && isEqual(ref.value, uri),
						)
					) {
			...
			references.push(
				...(host.contributeAdditionalReferences?.(
					accessor,
					references,
				) ?? []),
			);

			const { location, location2 } = host.prepareChatRequestLocation(
				accessor,
				range,
			);
			let request: vscode.ChatRequest = {
				...
				toolReferences,
				toolInvocationToken: (isInExtensionHost
					? undefined
					: {}) as never,
			...
			if (!request.command) {
				const intentDetector =
					instaService.createInstance(IntentDetector);
				const participants = readBuiltinIntents(location);
				const detectedParticipant =
					await intentDetector.provideParticipantDetection(
						request,
						{ history, yieldRequested: false },
						{ participants, location: ChatLocation.Editor },
						CancellationToken.None,
					);
				if (detectedParticipant?.command) {
					request = {
						...request,
						command: detectedParticipant.command,
					};
				}
			}
			...
			let stream: vscode.ChatResponseStream = new ChatResponseStreamImpl(
				(value) => {
					if (
						...
						if (!workspace.hasDocument(uri)) {
							...
							if (!changedDocsInitialStates.has(doc.document)) {
								...
								const workspaceStateFilePromise = testRuntime
									.writeFile(
										workspacePath,
										doc.document.getText(),
										INLINE_CHANGED_DOC_TAG,
									)
									.then((relativeDiskPath) => {
										return {
											workspacePath,
											relativeDiskPath,
								...
								changedDocsInitialStates.set(
									doc.document,
									workspaceStateFilePromise,
								);
							}
						}
						...
						if (!workingCopyDocument) {
							workingCopyDocument =
								new WorkingCopyOriginalDocument(
									doc.document.getText(),
								);
			...
			const documentStateBeforeInvocation =
				activeEditor?.document.getText();
			...
			const requestHandler = instaService.createInstance(
				ChatParticipantRequestHandler,
				history,
				request,
				stream,
			...
			history.push(
				new ChatResponseTurn(
					[new ChatResponseMarkdownPart(markdownChunks.join(''))],
					result,
					'',
				),
			);
			...
			let annotations =
				(await responseProcessor?.postProcess(
					accessor,
					workspace,
					stream,
					result,
			...
			let interactionOutcomeKind =
				interactionOutcomeComputer.interactionOutcome.kind;
			if (
				interactionOutcomeKind === 'inlineEdit' ||
				interactionOutcomeKind === 'workspaceEdit'
			) {
			...
			let intent: IIntent | undefined;
			{
				...
				const response = requestHandler.conversation
					.getLatestTurn()
					?.getMetadata(CopilotInteractiveEditorResponse);
				intent = response ? response.promptQuery.intent : undefined;
			}
			annotations = annotations.concat(
				requestHandler.conversation
					.getLatestTurn()
					?.getMetadata(InteractionOutcome)?.annotations ?? [],
			);
			...
			if (interactionOutcomeKind === 'none') {
				...
				for (const [
					...
					if (uri.scheme === 'file') {
						outcomeFiles.push({
							...
							fileContents:
								workspace.tryGetNotebook(uri)?.getText() ??
								workspace.getDocument(uri).getText(),
						});
					} else {
						outcomeFiles.push({
							kind: 'qualifiedFile',
					...
					if (
						activeEditor &&
						isEqual(uri, activeEditor.document.uri)
					) {
						// edit in the same document
						for (
							let i = 0;
							i < offsetEdits.replacements.length;
							i++
						) {
				...
				if (interactionOutcomeKind === 'inlineEdit') {
					if (!activeEditor) {
						throw new Error(
							`inlineEdit should always have an open editor`,
						);
					}
					outcome = {
						type: 'inlineEdit',
			...
			if (changedDocs.length > 0) {
				...
				for (const changedDoc of changedDocs) {
					...
					if (
						...
						!seenFiles.find(
							(v) =>
								URI.isUri(v.value) &&
								isEqual(v.value, changedDoc.uri),
						)
					) {
						seenFiles.push(
							createWorkingSetFileVariable(changedDoc.uri),
						);
					}
				...
				testRuntime.setOutcome({
					kind: 'edit',
					files: changedFilePaths.map((f) => ({
						srcUri: f.workspacePath,
						post: f.relativeDiskPath,
					})),
			...
			states.push({
				...
				annotations: outcome.annotations,
				fileName: activeEditor
					? workspace.getFilePath(activeEditor.document.uri)
					: undefined,
				languageId: activeEditor?.document.languageId,
				diagnostics,
				selection: activeEditor
					? toIRange(activeEditor.selection)
					: undefined,
				range: activeEditor
					? toIRange(range ?? activeEditor.selection)
					: undefined,
				interaction: {
					query: query.query,
		...
		await testRuntime.writeFile(
			...
			INLINE_STATE_TAG,
		); // TODO@test: using .txt instead of .json to avoid breaking test scripts
	}
}
...
function setupTools(
	...
	const toolsService = accessor.get(IToolsService) as
		| TestToolsService
	...
	toolsService.addTestToolOverride(
		{
			...
			description:
				'Moves the inline chat session to the richer panel chat which supports edits across files, creating new files, and multi-turn conversations between the user and the assistant.',
			inputSchema: {},
			source: undefined,
			tags: [],
		},
...
export function forInlineChatIntent(
	callback: (
		strategy: EditTestStrategy,
		configurations: NonExtensionConfiguration[] | undefined,
		suffix: string,
	) => void,
): void {
	callback(
		EditTestStrategy.InlineChatIntent,
