...
import {
	ApiEmbeddingsIndex,
	IApiEmbeddingsIndex,
} from '../../src/extension/context/node/resolvers/extensionApi';
import {
	ConversationStore,
	IConversationStore,
} from '../../src/extension/conversationStore/node/conversationStore';
import {
	IIntentService,
	IntentService,
} from '../../src/extension/intents/node/intentService';
import {
	ILinkifyService,
	LinkifyService,
} from '../../src/extension/linkify/common/linkifyService';
...
} from '../../src/extension/testing/node/aiEvaluationService';
...
} from '../../src/platform/chunking/node/naiveChunkerService';
import {
	CHAT_MODEL,
	Config,
	ConfigKey,
...
} from '../../src/platform/embeddings/common/vscodeIndex';
...
} from '../../src/platform/projectTemplatesIndex/common/projectTemplatesIndex';
...
import {
	constructGlobalStateMemento,
	MockExtensionContext,
} from '../../src/platform/test/node/extensionContext';
...
import {
	ITokenizerProvider,
	TokenizerProvider,
} from '../../src/platform/tokenizer/node/tokenizer';
import {
	IGithubAvailableEmbeddingTypesService,
	MockGithubAvailableEmbeddingTypesService,
} from '../../src/platform/workspaceChunkSearch/common/githubAvailableEmbeddingTypes';
import {
...
import {
	...
	ChunkingEndpointClientSQLiteCache,
} from './cachingChunksEndpointClient';
import {
	CachingCodeOrDocSearchClient,
	CodeOrDocSearchSQLiteCache,
} from './cachingCodeSearchClient';
...
export const ICachingResourceFetcher =
	createServiceIdentifier<ICachingResourceFetcher>('ICachingResourceFetcher');
...
export class NoFetchChatMLFetcher extends ChatMLFetcherImpl {
	public override fetchMany(...args: any[]): Promise<ChatResponses> {
		return Promise.resolve({
			...
			value: [
				'--no-fetch option is provided to simulations -- using a fixed ChatML response',
			],
...
export function createSimulationChatModelThrottlingTaskLaunchers(
	boost: boolean,
): ChatModelThrottlingTaskLaunchers {
	...
	if (boost) {
		...
		throttlingLimits[CHAT_MODEL.CLAUDE_37_SONNET] = {
			limit: 20,
			type: 'RPS',
		};
	}
...
export async function createSimulationAccessor(
	...
	if (currentTestRunInfo.isInRealExtensionHost) {
		const { addExtensionHostSimulationServices } =
			await import('./extHostContext/simulationExtHostContext');
		await addExtensionHostSimulationServices(testingServiceCollection);
	}

	testingServiceCollection.define(
		ITestInformation,
		new SyncDescriptor(TestInformation, [currentTestRunInfo.test]),
	);
	try {
		const newLocal = new Map<string, any>(
			currentTestRunInfo.test.nonExtensionConfigurations,
		);

		const configs = Object.entries(opts.configs ?? {}).map(
			([key, value]) => [lookupConfigKey(key), value] as const,
		);

		testingServiceCollection.define(
			IConfigurationService,
			new SyncDescriptor(InMemoryConfigurationService, [
				new DefaultsOnlyConfigurationService(),
				new Map<
					| ExperimentBasedConfig<ExperimentBasedConfigType>
					| Config<any>,
					unknown
				>([
					...
					[
						...
						opts.summarizeHistory,
					],
					...(currentTestRunInfo.test.configurations?.map<
						[
							(
								| ExperimentBasedConfig<ExperimentBasedConfigType>
								| Config<any>
	...
	testingServiceCollection.define(
		ISimulationEndpointHealth,
		new SyncDescriptor(SimulationEndpointHealthImpl),
	);
	testingServiceCollection.define(
		IJSONOutputPrinter,
		new SyncDescriptor(NoopJSONOutputPrinter),
	);
	testingServiceCollection.define(
		ICachingResourceFetcher,
		new SyncDescriptor(CachingResourceFetcher, [
			currentTestRunInfo,
			opts.resourcesCacheMode,
		]),
	);
	testingServiceCollection.define(
		...
		new SyncDescriptor(MockExtensionContext, [
			globalStoragePath,
			constructGlobalStateMemento(globalStatePath),
		]),
	);
	testingServiceCollection.define(
		IIntentService,
		new SyncDescriptor(IntentService),
	);

	testingServiceCollection.define(
		IAIEvaluationService,
		new SyncDescriptor(AIEvaluationService),
	);

	const docsSearchClient = new SyncDescriptor(
		ThrottlingCodeOrDocsSearchClient,
		[new SyncDescriptor(DocsSearchClient)],
	);
	testingServiceCollection.define(
		ITokenizerProvider,
	...
	const cacheTestInfo = new CachedTestInfo(
		currentTestRunInfo.test,
		currentTestRunInfo.testRunNumber,
	);
	...
	if (currentTestRunInfo.fetchRequestCollector) {
		chatMLFetcher = new SyncDescriptor(SpyingChatMLFetcher, [
	...
	if (opts.createNesFetchCache === undefined || cacheTestInfo === undefined) {
		...
		testingServiceCollection.define(
			ICompletionsFetchService,
			new SyncDescriptor(CachingCompletionsFetchService, [
	...
	if (opts.languageModelCacheMode === CacheMode.Disable) {
		...
		testingServiceCollection.define(
			ICombinedEmbeddingIndex,
			new SyncDescriptor(VSCodeCombinedIndexImpl, [
				/*useRemoteCache*/ true,
			]),
		);
		testingServiceCollection.define(
		...
		testingServiceCollection.define(
			IProjectTemplatesIndex,
			new SyncDescriptor(ProjectTemplatesIndex, [
				/*useRemoteCache*/ true,
			]),
		);
	} else {
		const embeddingCache = new EmbeddingsSQLiteCache(
			TestingCacheSalts.embeddingsCacheSalt,
			currentTestRunInfo,
		);
		testingServiceCollection.define(
			IEmbeddingsComputer,
			new SyncDescriptor(CachingEmbeddingsComputer, [embeddingCache]),
		);

		const codeOrDocSearchCache = new CodeOrDocSearchSQLiteCache(
			TestingCacheSalts.codeSearchCacheSalt,
			currentTestRunInfo,
		);
		const chunksEndpointCache = new ChunkingEndpointClientSQLiteCache(
			TestingCacheSalts.chunksEndpointCacheSalt,
			currentTestRunInfo,
		);
		testingServiceCollection.define(
			IDocsSearchClient,
			new SyncDescriptor(CachingCodeOrDocSearchClient, [
				docsSearchClient,
				codeOrDocSearchCache,
			]),
		);
		testingServiceCollection.define(
			ICombinedEmbeddingIndex,
		...
		testingServiceCollection.define(
			IApiEmbeddingsIndex,
			new SyncDescriptor(ApiEmbeddingsIndex, [/*useRemoteCache*/ false]),
		);
		testingServiceCollection.define(
			IProjectTemplatesIndex,
			new SyncDescriptor(ProjectTemplatesIndex, [
		...
		testingServiceCollection.define(
			IChunkingEndpointClient,
			new SyncDescriptor(CachingChunkingEndpointClient, [
				chunksEndpointCache,
			]),
		);
	}
	...
	testingServiceCollection.define(
		ITestProvider,
		new SyncDescriptor(NullTestProvider),
	);
	testingServiceCollection.define(
		IConversationStore,
	...
	testingServiceCollection.define(
		...
		new SyncDescriptor(ReleaseNotesService),
	);
	testingServiceCollection.define(
		IWorkspaceFileIndex,
		new SyncDescriptor(WorkspaceFileIndex),
	);
	testingServiceCollection.define(
		IGithubAvailableEmbeddingTypesService,
		new SyncDescriptor(MockGithubAvailableEmbeddingTypesService),
	);

	if (opts.useExperimentalCodeSearchService) {
		testingServiceCollection.define(
			IWorkspaceChunkSearchService,
			new SyncDescriptor(SimulationCodeSearchChunkSearchService, []),
		);
	} else {
		testingServiceCollection.define(
			IWorkspaceChunkSearchService,
...
function lookupConfigKey(
	key: string,
): ExperimentBasedConfig<ExperimentBasedConfigType> | Config<any> {
	const config = globalConfigRegistry.configs.get(key);
	if (!config) {
		throw new Error(
			`Configuration '${key}' provided does not exist in product. Double check if the configuration key exists by using it in vscode settings.json.`,
		);
	}
	return config;
}
