...
} from '../../src/platform/embeddings/common/embeddingsComputer';
...
export class CacheableEmbeddingRequest {
	...
	constructor(embeddingQuery: string, model: LEGACY_EMBEDDING_MODEL_ID) {
...
export interface IEmbeddingsCache {
	get(
		queryHash: CacheableEmbeddingRequest,
...
export class CachingEmbeddingsComputer extends RemoteEmbeddingsComputer {
	...
	public override async computeEmbeddings(
		...
		for (const input of inputs) {
			const embeddingRequest = new CacheableEmbeddingRequest(
				input,
				model,
			);
			...
			if (!cacheEntry) {
				...
				embeddingEntries.set(embeddingRequest.query, {
					type,
					value: cacheEntry,
				});
			}
		...
		if (nonCached.length) {
			const embeddingsResult = await super.computeEmbeddings(
				type,
				nonCached,
				options,
				telemetryInfo,
