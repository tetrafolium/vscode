...
} from '../../src/platform/chunking/common/chunk';
import {
	ChunkableContent,
	ComputeBatchInfo,
	EmbeddingsComputeQos,
	IChunkingEndpointClient,
} from '../../src/platform/chunking/common/chunkingEndpointClient';
...
class CacheableChunkingEndpointClientRequest {
	static async create(content: ChunkableContent) {
		const hash = await createSha256Hash(
			CHUNKING_ENDPOINT_CACHE_SALT + (await content.getText()),
		);
...
interface IChunkingEndpointClientCache {
	get(
		req: CacheableChunkingEndpointClientRequest,
	): Promise<FileChunkWithEmbedding[] | undefined>;
	set(
		req: CacheableChunkingEndpointClientRequest,
...
export class ChunkingEndpointClientSQLiteCache
	extends SQLiteCache<
		CacheableChunkingEndpointClientRequest,
		FileChunkWithEmbedding[]
	>
	implements IChunkingEndpointClientCache
{
		...
		return result?.map((cachedResponse) => {
			const chunk: FileChunkWithEmbedding = {
				chunk: {
					...
					range: new Range(
						cachedResponse.chunk.range.startLineNumber,
						cachedResponse.chunk.range.startColumn,
						cachedResponse.chunk.range.endLineNumber,
						cachedResponse.chunk.range.endColumn,
					),
...
export class CachingChunkingEndpointClient implements IChunkingEndpointClient {
	...
	constructor(
		...
		this._chunkingEndpointClient = instantiationService.createInstance(
	...
	async computeChunksAndEmbeddings(
		...
		const req =
			await CacheableChunkingEndpointClientRequest.create(content);
		...
		const result =
			await this._chunkingEndpointClient.computeChunksAndEmbeddings(
				authToken,
				embeddingType,
				content,
				batchInfo,
	...
	computeChunks(
		...
		return this.computeChunksAndEmbeddings(
			authToken,
			embeddingType,
			content,
			batchInfo,
