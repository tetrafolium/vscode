...
export class SimulationOptions {
	...
	protected constructor(processArgv: readonly string[]) {
		...
		this.isUpdateBaseline = boolean(
			argv['update-baseline'] ?? argv['u'],
			false,
		);
		...
		this.nRuns =
			typeof argv['n'] === 'number'
				? argv['n']
				: this.isUpdateBaseline || argv['ci']
					? BASELINE_RUN_COUNT
					: 10;
		...
		this.embeddingType = cliOptionsToWellKnownEmbeddingsType(
			this.argv['embedding-model'],
		);
		this.parallelism = this.argv['parallelism'] ?? this.argv['p'] ?? 20;
		this.modelCacheMode = this.argv['skip-model-cache']
			? CacheMode.Disable
			: CacheMode.Default;
		this.lmCacheMode = this.argv['skip-cache']
			? CacheMode.Disable
			: this.argv['require-cache']
				? CacheMode.Require
				: CacheMode.Default;
		this.resourcesCacheMode = this.argv['skip-resources-cache']
			? CacheMode.Disable
			: CacheMode.Default;
		...
		this.applyChatCodeBlocks = boolean(
			this.argv['apply-chat-code-blocks'],
			false,
		);
		this.stageCacheEntries = boolean(
			this.argv['stage-cache-entries'],
			false,
		);
		...
		this.installExtensions = argv['install-extension']
			? argv['install-extension'].split(',')
			: [];
		this.headless = boolean(argv['headless'], true);
		this.runNumber = Number(argv['run-number']) || 0;

		this.runServerPoweredNesProvider = boolean(
			argv['runServerPoweredNesProvider'],
			false,
		);
		...
		this.disabledTools = argv['disable-tools']
			? new Set(argv['disable-tools'].split(','))
			: new Set();
		this.useScenarioWorkspace = boolean(
			argv['scenario-workspace-folder'],
			false,
		);

		this.useExperimentalCodeSearchService = boolean(
			argv['use-experimental-code-search-service'],
			false,
		);
		...
		this.nesDatagen =
			isNesDatagen && argv['input']
				? {
						input: argv['input'],
						output: argv['out'],
						rowOffset:
							typeof argv['row-offset'] === 'number'
								? argv['row-offset']
								: 0,
	...
	private validateExternalBaseline() {
		if (this.externalBaseline && !this.externalScenarios) {
			throw new Error(
				'External scenarios must be provided for external baseline to work.',
			);
		}
	}

	private static validateNesArgument(
		nes: unknown,
	): 'external' | 'coffe' | undefined {
		if (nes === undefined || nes === null) {
			return undefined;
		}
		if (typeof nes === 'boolean') {
			// this's for backward compat because previously it was possible to just pass `--nes` to run external stests against NES
			return 'external';
		}
		...
		switch (nes) {
			...
			default:
				throw new Error(
					`--nes can only be 'external' or 'coffe', but got: ${nes}`,
				);
		}
	}

	private static validateNesUrlOverride(
		nesUrl: string | undefined,
		nesApiKey: string | undefined,
	): void {
		if (nesUrl !== undefined && nesApiKey === undefined) {
			throw new Error(
				`--nesApiKey must be provided when --nesUrl is set`,
			);
		}
		if (nesUrl === undefined && nesApiKey !== undefined) {
			throw new Error(
				`--nesUrl must be provided when --nesApiKey is set`,
			);
		}
	}
}

function cliOptionsToWellKnownEmbeddingsType(
