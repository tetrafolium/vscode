...
export function validate(
	answer: string,
	predicate: readonly KeywordPredicate[] | object,
): string | undefined {
	const predicates = Array.isArray(predicate)
		? predicate
		: Object.entries(predicate).map(([key, value]) => ({ [key]: value }));
...
function validatePredicate(
	...
	if (typeof predicate === 'string') {
		...
		if (
			!new RegExp(
				(startsWithWordChar ? '\\b' : '') +
					escapeRegExpCharacters(predicate) +
					(endsWithWordChar ? '\\b' : ''),
				'i',
			).test(answer)
		) {
		...
		if ('anyOf' in predicate) {
			const errors = (predicate.anyOf as readonly KeywordPredicate[]).map(
				(pred) => validatePredicate(answer, pred),
			);
