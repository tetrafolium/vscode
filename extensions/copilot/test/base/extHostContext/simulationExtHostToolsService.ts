...
import {
	BaseToolsService,
	IToolsService,
} from '../../../src/extension/tools/common/toolsService';
...
export class SimulationExtHostToolsService
	...
	implements IToolsService
{
	...
	private readonly _overrides = new Map<
		ToolName | string,
		{ info: LanguageModelToolInformation; tool: ICopilotTool<any> }
	>();
	...
	get tools() {
		...
		return [
			...this._inner.tools.filter(
				(t) =>
					!this._disabledTools.has(t.name) &&
					!this._overrides.has(t.name),
			),
	...
	private ensureToolsRegistered() {
		this._lmToolRegistration ??= new ToolsContribution(
			this,
			{} as any,
			{ threshold: observableValue(this, 128) } as any,
			{} as any,
			{} as any,
		);
	}
	...
	getCopilotTool(name: string): ICopilotTool<any> | undefined {
		return this._disabledTools.has(name)
			? undefined
			: this._overrides.get(name)?.tool ||
					this._inner.getCopilotTool(name);
	}
	...
	async invokeTool(
		...
		logger.debug(
			'SimulationExtHostToolsService.invokeTool',
			name,
		...
		try {
			...
			const r = await raceTimeout(
				Promise.resolve(this._inner.invokeTool(name, options, token)),
				60_000,
			);
			if (!r) {
			...
			logger.debug(
				`SimulationExtHostToolsService.invokeTool ${name} done in ${Date.now() - start}ms` +
	...
	getTool(name: string): LanguageModelToolInformation | undefined {
		return this._disabledTools.has(name)
			? undefined
			: this._overrides.get(name)?.info || this._inner.getTool(name);
	}
	...
	getEnabledTools(
		...
		return this.tools
			.map((tool) => {
				...
				const owned = this.copilotTools.get(
