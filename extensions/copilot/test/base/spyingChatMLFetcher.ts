...
export class FetchRequestCollector {
	...
	public addInterceptedRequest(
		requestPromise: Promise<InterceptedRequest>,
	): void {
		this._scheduledRequests.push(
			this._pendingRequests.schedule(async () => {
				try {
	...
	public get contentFilterCount(): number {
		return this.interceptedRequests.filter(
			(x) => x.response.type === 'filtered',
		).length;
	}
	...
	public get usage(): APIUsage {
		...
		return this.interceptedRequests.reduce((p, c): APIUsage => {
			...
			return {
				completion_tokens:
					p.completion_tokens + cUsage.completion_tokens,
				prompt_tokens: p.prompt_tokens + cUsage.prompt_tokens,
				total_tokens: p.total_tokens + cUsage.total_tokens,
				prompt_tokens_details: {
					cached_tokens:
						(p.prompt_tokens_details?.cached_tokens ?? 0) +
						(cUsage.prompt_tokens_details?.cached_tokens ?? 0),
				},
			};
		}, initial);
	}
	...
	public get averageRequestDuration(): number {
		const requestDurations = coalesce(
			this.interceptedRequests.map(
				(r) => r.response.cacheMetadata?.requestDuration,
			),
		);
		return (
	...
	public get hasCacheMiss(): boolean {
		return this.interceptedRequests.some(
			(x) => x.response.isCacheHit === false,
		);
	}

	public get cacheInfo(): TestRunCacheInfo {
		return coalesce(this.interceptedRequests.map((r) => r.cacheKey)).map(
			(key) => ({ type: 'request', key }) satisfies CacheInfo,
		);
	}
}
...
export class SpyingChatMLFetcher extends AbstractChatMLFetcher {
	...
	override async fetchMany(
		...
		const captureToolCallsCb: FinishedCallback = async (
			text,
			idx,
			delta,
		) => {
		...
		const respPromise = this.fetcher.fetchMany(
			{ ...opts, finishedCb: captureToolCallsCb },
			token,
		);

		const sw = new StopWatch(false);
		this.requestCollector.addInterceptedRequest(
			respPromise.then((resp) => {
				let cacheKey: string | undefined;
				if (typeof (resp as ResponseWithMeta).cacheKey === 'string') {
					cacheKey = (resp as ResponseWithMeta).cacheKey;
				}
				(resp as ISerialisedChatResponse).copilotFunctionCalls =
					toolCalls;
