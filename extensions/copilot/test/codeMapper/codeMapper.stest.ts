...
} from '../../src/extension/prompts/node/codeMapper/codeMapper';
...
import {
	setupSimulationWorkspace,
	teardownSimulationWorkspace,
	toIRange,
...
forEditsAndAgent((variant, model, configurations) => {
	ssuite(
		{ title: 'codeMapper' + variant, location: 'context', configurations },
		() => {
			stest(
				{
					description: 'add new function at location 1',
					language: 'typescript',
				},
				async (testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						files: [
							fromFixture('codeMapper/fibonacci_recursive.ts'),
						],
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.ok(
								outcome.editedFile.includes(
									'function fibonacci_iterative(n: number): number {',
								),
								'has fibonacci_iterative',
							);
			...
			stest(
				{ description: 'modify function', language: 'typescript' },
				async (testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						files: [
							fromFixture('codeMapper/fibonacci_recursive.ts'),
						...
						validate: async (outcome, workspace, accessor) => {
							assert.equal(
								...
								1,
								'only once occurrence of fibonacci_recursive',
							);
							assert.ok(outcome.appliedEdits.length, 'has edits');
							assert.ok(
								outcome.editedFile.includes(
			...
			stest(
				{
					description: 'replace statement with ident change',
					language: 'typescript',
				},
				async (testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
			...
			stest(
				{ description: 'move to class', language: 'typescript' },
				async (testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						files: [
							fromFixture('codeMapper/fibonacci_recursive.ts'),
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.equal(
								numOccurrences(
									outcome.editedFile,
									'console.log(function generate_fibonacci_recursive',
								),
								0,
								'no references to the old function',
							);
							assert.equal(
								numOccurrences(
									outcome.editedFile,
									'console.log(FibonacciGenerator.generate_fibonacci_recursive',
								),
								1,
			...
			stest(
				{ description: 'add import', language: 'typescript' },
				async (testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						files: [
							fromFixture('codeMapper/fibonacci_recursive.ts'),
			...
			stest(
				{
					description: 'break up function, large file',
					language: 'typescript',
				},
				async (testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						files: [
							fromFixture('codeMapper/scanner.ts'),
							fromFixture('codeMapper/scannerTypes.ts'),
						],
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.equal(
								...
								1,
								'inserted with indent',
							);
							assert.deepEqual(
								await getWorkspaceDiagnostics(
									accessor,
									workspace,
									KnownDiagnosticProviders.tsc,
								),
								[],
								'no diagnostics',
							);
			...
			stest(
				{
					description: 'make changes in package.json',
					language: 'json',
				},
				async (testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.equal(
								pgkJSON.devDependencies['vitest'],
								'^0.34.0',
								'has vitest',
							);
							assert.equal(
								pgkJSON.devDependencies['mocha'],
								undefined,
								'no longer has mocha',
							);
							assert.deepEqual(
			...
			stest(
				{
					description:
						'does not delete random parts of code (big file)',
					language: 'typescript',
				},
				(testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						...
						focusLocations: [],
						textBeforeCodeBlock:
							'Add a utility function to calculate Fibonacci numbers.',
						codeBlock: [
							`// ...existing code...`,
							``,
							`/**`,
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.ok(
								newText.includes('function fibonacci'),
								'fibonacci function not added',
							);
							assert.ok(
								newText.length >
									outcome.originalFileContent.length,
								'deleted code',
							);
							assertNoElidedCodeComments(outcome.editedFile);
							validateConsistentIndentation(
			...
			stest(
				{
					description: 'does not remove stale imports #11766',
					language: 'typescript',
				},
				(testingServiceCollection) => {
					return simulateCodeMapper(testingServiceCollection, {
						...
						focusLocations: [],
						textBeforeCodeBlock:
							'Update saveCategorization endpoint to use the new CSV utility function.',
						codeBlock: [
						...
						validate: async (outcome, workspace, accessor) => {
							...
							const newText = outcome.editedFile;
							assert.ok(
								!newText.includes(`'path'`),
								'rewritten file contains unused imports',
							);
							assert.ok(
								!newText.includes(`'fs'`),
								'rewritten file contains unused imports',
			...
			stest(
				{
					description:
						'sorted product icons (60kb) - modify & insert',
					language: 'markdown',
					model,
				},
				(testingServiceCollection) => {
					const inputFile = fromFixture(
						'codeMapper/product-icons-sorted.md',
					);
					return simulateCodeMapper(testingServiceCollection, {
						files: [inputFile],
			...
			stest(
				{
					description: 'mixed product icons (60kb) - modify & insert',
					language: 'markdown',
					model,
				},
				(testingServiceCollection) => {
					const inputFile = fromFixture(
						'codeMapper/product-icons-mixed.md',
					);
					return simulateCodeMapper(testingServiceCollection, {
						files: [inputFile],
						activeDocument: inputFile.fileName,
			...
			stest(
				{
					description: 'other tests updated similarly',
					language: 'typescript',
					model,
				},
				(testingServiceCollection) => {
					...
					const inputFile = fromFixture(
						'codeMapper/extHostExtensionActivator.test.ts',
					);
					return simulateCodeMapper(testingServiceCollection, {
						files: [inputFile],
						activeDocument: inputFile.fileName,
						...
						validate: async (outcome, workspace, accessor) => {
							...
							assert.equal(
								numRegexOccurrences(
									outcome.editedFile,
									/(?!const\s*)activator = createActivator/g,
								),
								9,
							);
							assert.equal(
			...
			stest(
				{ description: 'looping in short yaml file', language: 'yaml' },
				(testingServiceCollection) => {
					const inputFile = fromFixture(
						'codeMapper/product-build-linux.yml',
					);
					return simulateCodeMapper(testingServiceCollection, {
						files: [inputFile],
						activeDocument: inputFile.fileName,
						...
						validate: async (outcome, workspace, accessor) => {
							assert.ok(outcome.appliedEdits.length, 'has edits');
							const originalFileLineCount =
								outcome.originalFileContent.split('\n').length;
							const editedFileLineCount =
								outcome.editedFile.split('\n').length;
							assert.ok(
								editedFileLineCount >= originalFileLineCount,
								'deleted code',
							);
							assertNoElidedCodeComments(outcome.editedFile);
						},
	...
	async function simulateCodeMapper(
		testingServiceCollection: TestingServiceCollection,
		scenario: ICodeMapperScenario,
	): Promise<void> {
		const workspace = setupSimulationWorkspace(testingServiceCollection, {
		...
		try {
			...
			const codeMapper = accessor
				.get(IInstantiationService)
				.createInstance(CodeMapper);
			...
			const input: ICodeMapperExistingDocument = {
				createNew: false,
				codeBlock: codeMap,
				uri: document.uri,
				markdownBeforeBlock: scenario.textBeforeCodeBlock,
				existingDocument: documentContext.document,
			};
			const originalFileContent = document.getText();
			const result = await codeMapper.mapCode(
				input,
				response,
				undefined,
				source.token,
			...
			const appliedEdits = workingCopyDocument.transformer.toTextEdits(
				workingCopyDocument.appliedEdits,
			);
