...
export function assertDocLines(
	fileContents: string | string[],
	line: string,
	lineAssertion = (line: string) => {},
) {
	const fileLines = Array.isArray(fileContents)
		? fileContents
		: fileContents.split('\n');
 ...
 */
export function assertDocLinesForInlineComments(
	fileContents: string | string[],
	line: string,
	...
	if (lineNum === -1) {
		throw new Error(
			`given line cannot be found: either original line was changed or test was incorrectly created`,
		);
	}
