...
ssuite(
	{ title: '/review', location: 'inline' },
	forEachModel(models, (model) => {
		stest(
			{
				description: 'Binary search with incorrect stop condition',
				language: 'javascript',
				model,
			},
		...
		stest(
			{
				description: 'Binary search with correct stop condition',
				language: 'javascript',
				model,
			},
			async (testingServiceCollection) => {
		...
		stest(
			{
				description: 'Bank account with missing lock acquisition',
				language: 'python',
				model,
			},
			async (testingServiceCollection) => {
		...
		stest(
			{
				description: 'Bank account with lock acquisition',
				language: 'python',
				model,
			},
			async (testingServiceCollection) => {
				const comments = await generateComments(
					testingServiceCollection,
					'review/bank-account-2.py',
				);
				const unexpected = comments.filter(
					(c) =>
						['bug', 'consistency'].includes(c.kind) &&
		...
		stest(
			{
				description: 'InstantiationService this scoping bug',
				language: 'typescript',
				model,
			},
			async (testingServiceCollection) => {
		...
		stest(
			{
				description: 'InstantiationService this scoping fixed',
				language: 'typescript',
				model,
			},
			async (testingServiceCollection) => {
				const comments = await generateComments(
					testingServiceCollection,
					'review/nested-services-2.ts',
				);
				const unexpected = comments.filter(
					(c) => c.kind === 'bug' && c.range.start.line === 10,
				);
				assert.strictEqual(unexpected.length, 0);
			},
		);
...
async function generateComments(
	...
	const workspace = setupSimulationWorkspace(testingServiceCollection, {
		files: [fromFixture(relativeFixturePath)],
	});
	...
	try {
		...
		const document = TextDocumentSnapshot.create(
			workspace.getDocument(relativeDocumentPath).document,
		);
...
function assertComment(
	...
	const lines = Array.isArray(expectedLines)
		? expectedLines
		: [expectedLines];
	const lineComment = comments?.find(
		(d) => lines.indexOf(d.range.start.line) !== -1,
	);
	assert.ok(lineComment, `Expected comment for line(s) ${lines.join(', ')}.`);
	const message =
		typeof lineComment.body === 'string'
			? lineComment.body
			: lineComment.body.value;
	assert.ok(
		message.includes(expectedContent),
		`Expected comment for line(s) ${lines.join(', ')} to contain "${expectedContent}"`,
	);
	...
	assert.ok(
		kinds.indexOf(lineComment.kind) !== -1,
		`Expected ${kinds.join(', ')} comment, was: ${lineComment.kind}`,
	);
}
