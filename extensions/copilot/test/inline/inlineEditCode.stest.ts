...
function executeEditTest(
	...
	if (strategy === EditTestStrategy.Inline) {
		...
		return simulatePanelCodeMapper(
...
function forInlineChatIntent(
	callback: (
		strategy: EditTestStrategy,
		location: 'inline' | 'panel',
		variant: string | undefined,
		configurations?: NonExtensionConfiguration[],
	) => void,
): void {
	callback(EditTestStrategy.InlineChatIntent, 'inline', '-InlineChatIntent', [
		['chat.agent.autoFix', false],
	]);
}

forInlineChatIntent(
	(strategy, location, variant, nonExtensionConfigurations) => {
		ssuite({ title: `edit${variant}`, location }, () => {
			stest(
				{
					description: 'Context Outline: TypeScript between methods',
					language: 'typescript',
						...
						queries: [
							{
								...
								validate: async (
									...
									await assertNoSyntacticDiagnosticsAsync(
										accessor,
										outcome,
										workspace,
										'tsc',
									...
									assert.ok(
										outcome.fileContents.length >
											outcome.originalFileContents
												.length /
												2,
										'File was truncated',
									);
									const edit = assertInlineEditShape(
										outcome,
										{
											line: 211,
											originalLength: 2,
			...
			stest(
				{
					description: 'Context Outline: TypeScript in method',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description: 'issue #404: Add a cat to a comment',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description:
						'issue #405: "make simpler" query is surprising',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description:
						'issue #246: Add comment sends request to sidebar',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'vscode/vscode.proposed.notebookDocumentWillSave.d.ts',
							),
						],
						queries: [
							{
			...
			stest(
				{
					description:
						'issue #4151: Rewrite the selection to use async/await',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description:
						'issue #4149: If ChatGPT makes the request, send only the first 20 episodes',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit-slice-4149/index.ts')],
						queries: [
							{
								file: 'index.ts',
								selection: [44, 1],
								...
								validate: async (
									...
									const edit =
										extractInlineReplaceEdits(outcome);
									assert.ok(
										edit,
										'unexpected identical files',
									);
									const newText =
										edit.allModifiedLines.join('\n');
									assert.ok(
			...
			stest(
				{
					description: 'issue #3759: add type',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					...
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'edit-add-explicit-type-issue-3759/pullRequestModel.ts',
							),
						],
						queries: [
							{
								...
								validate: async (
									...
									let text = findTextBetweenMarkersFromTop(
										edit.changedModifiedLines.join('\n'),
										'const reviewRequiredCheck',
										'= await this._getReviewRequiredCheck();',
									);
									assert(text);
									text = text.trim();
			...
			stest(
				{
					description:
						'issue #1198: Multi-lingual queries throw off the inline response formatting',
					language: 'python',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
						...
						queries: [
							{
								...
								validate: async (
									...
									const actualLines = outcome.fileContents
										.split('\n')
										.map((s) => s.trim())
										.slice(1, 7);
									for (
										...
										if (Array.isArray(expected)) {
											assert.ok(
												...
												`Line ${i + 2} does not match any expected variant. Actual: "${actual}"`,
											);
										} else {
											assert.strictEqual(
												actual,
			...
			stest(
				{
					description: 'refactor forloop, but only selected one',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description: 'convert ternary to if/else in short function',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'edit-convert-ternary-to-if-else/index.ts',
							),
						],
						queries: [
							{
			...
			stest(
				{
					description: 'edit: add toString1',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit-add-toString/index.ts')],
						queries: [
							{
								file: 'index.ts',
								selection: [53, 1],
			...
			stest(
				{
					description: 'edit: add toString2',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit-add-toString2/index.ts')],
						queries: [
							{
								file: 'index.ts',
								selection: [54, 1],
			...
			stest(
				{
					description: 'edit: add enum variant',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit-add-enum-variant/index.ts')],
						queries: [
							{
								file: 'index.ts',
								selection: [8, 9],
			...
			function verifyTsImportStatementsAreTogether(
				fileContents: string,
			): boolean {
			...
			stest(
				{
					description: 'edit: import assert',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit-import-assert/index.ts')],
						queries: [
							{
								file: 'index.ts',
								selection: [47, 14],
								...
								validate: async (
									...
									assertInlineEdit(outcome);
									assert(
										verifyTsImportStatementsAreTogether(
											outcome.fileContents,
										),
									);
									await assertNoSyntacticDiagnosticsAsync(
			...
			stest(
				{
					description: 'edit: import assert 2',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit-import-assert2/index.ts')],
						queries: [
							{
								file: 'index.ts',
								selection: [5, 0],
			...
			stest(
				{
					description:
						'issue #2431: Inline Chat follow-up tweak ends up in noop text-only answer',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('vscode/editorGroupWatermark.ts')],
						queries: [
							{
			...
			stest(
				{
					description: 'Inline chat does not leak system prompt',
					language: 'json',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('gen-json/test.json')],
						queries: [
							{
								file: 'test.json',
								selection: [0, 0],
								...
								validate: async (
									...
									assertInlineEdit(outcome);
									['assistant', 'Microsoft', 'AI'].forEach(
										(text) => {
											assert.strictEqual(
			...
			stest(
				{
					description:
						'Inline chat touching code outside of my selection #2988',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					...
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'edit/issue-2988/pseudoStartStopConversationCallback.test.ts',
							),
						],
						queries: [
							{
			...
			stest(
				{
					description:
						'Inline chat touching code outside of my selection #2988 with good selection',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description: "issue #2946: Inline chat markers don't work",
					language: 'javascript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('editing/math.js')],
						queries: [
							{
								file: 'math.js',
								selection: [17, 0, 32, 1],
			...
			stest(
				{
					description:
						'issue #3257: Inline chat ends up duplicating code',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description:
						'issue release#275: Inline Diff refinement causes massive duplication of code',
					language: 'csharp',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'edit/issue-release-275/BasketService.cs',
							),
						],
						queries: [
							{
								...
								validate: async (
									...
									assert.ok(
										outcome.fileContents.split(
											/\r\n|\r|\n/g,
										).length < 95,
										'file stays under 95 lines',
									);
									assertNoElidedCodeComments(
										outcome.fileContents,
									);
			...
			stest(
				{
					description:
						'issue #5755: Inline edits go outside the selection',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'edit/issue-5755/vscode.proposed.chatParticipantAdditions.d.ts',
							),
						],
						queries: [
							{
			...
			stest(
				{
					description:
						"issue #4302: Code doesn't come with backticks",
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/4302.ts')],
						queries: [
							{
								file: '4302.ts',
								selection: [12, 0, 23, 0],
			...
			stest(
				{
					description:
						"issue #5710: Code doesn't come with backticks",
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/5710.ts')],
						queries: [
							{
								file: '5710.ts',
								selection: [7, 66, 10, 5],
			...
			stest(
				{
					description:
						'issue #3575: Inline Chat in function expands to delete whole file',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/3575.ts')],
						queries: [
							{
								file: '3575.ts',
								selection: [51, 9, 51, 9],
			...
			stest(
				{
					description: 'edit for cpp',
					language: 'cpp',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('cpp/basic/main.cpp')],
						queries: [
							{
								file: 'main.cpp',
								selection: [4, 0, 17, 0],
			...
			stest(
				{
					description: 'edit for macro',
					language: 'cpp',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('cpp/headers/abi_macros.hpp')],
						queries: [
							{
								file: 'abi_macros.hpp',
								selection: [0, 0, 100, 0],
								...
								validate: async (
									...
									assertContainsAllSnippets(
										outcome.fileContents,
										[
											'#define NLOHMANN_JSON_VERSION_MAJOR 4',
			...
			stest(
				{
					description: 'merge markdown sections',
					language: 'markdown',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/markdown/README.md')],
						queries: [
							{
								file: 'README.md',
								selection: [11, 0, 32, 0],
			...
			stest(
				{
					description:
						'issue #5899: make this code more efficient inside markdown',
					language: 'markdown',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/markdown/explanation.md')],
						queries: [
							{
								file: 'explanation.md',
								selection: [4, 0, 17, 0],
			...
			stest(
				{
					description: 'issue #6276',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/6276.ts')],
						queries: [
							{
								file: '6276.ts',
			...
			stest(
				{
					description: 'issue #7487',
					language: 'typescriptreact',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/issue-7487/EditForm.tsx')],
						queries: [
							{
								file: 'EditForm.tsx',
			...
			stest(
				...
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'edit/issue-7202/languageModelToolsContribution.ts',
							),
						],
						queries: [
							{
			...
			stest(
				...
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture('generate/issue-6956/.eslintrc.js'),
						],
						queries: [
							{
								file: '.eslintrc.js',
			...
			stest(
				{
					description: 'issue #6973',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [fromFixture('edit/issue-6973/utils.ts')],
						queries: [
							{
								file: 'utils.ts',
								selection: [7, 0, 17, 0],
			...
			stest(
				...
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture(
								'unknown/issue-7660/positionOffsetTransformer.spec.ts',
							),
						],
						queries: [
							{
								...
								validate: async (
									...
									assert.ok(
										...
										'First line should not contain an import statement',
									);
									assertNoElidedCodeComments(
										outcome.fileContents,
									);
			...
			stest(
				...
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture('edit/issue-6614/workbench-dev.html'),
						],
						queries: [
							{
			...
			stest(
				{
					description: 'Issue #7996 - use entire context window',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
					return executeEditTest(strategy, testingServiceCollection, {
						files: [
							fromFixture('edit/issue-7996/codeEditorWidget.ts'),
						],
						queries: [
							{
								file: 'codeEditorWidget.ts',
			...
			stest(
				{
					description: 'Issue #8129 (no errors)',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
			...
			stest(
				{
					description: 'Issue #8129 (no syntax errors)',
					language: 'typescript',
					nonExtensionConfigurations,
				},
				(testingServiceCollection) => {
