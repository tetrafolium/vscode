...
ssuite({ title: 'git commit message', location: 'external' }, () => {
	stest(
		{
			description: 'Generates a simple commit message',
			language: 'python',
		},
...
def print_hello_world():
        print("Hello, World!")`;
			...
			testingServiceCollection.define(
				IWorkspaceService,
				new TestWorkspaceService(undefined, [document]),
			);
...
\ No newline at end of file`;
			...
			const generator = instantiationService.createInstance(
				GitCommitMessageGenerator,
			);
			const message = await generator.generateGitCommitMessage(
				'test-repo',
				'main',
				changes,
				{ repository: [], user: [] },
	...
	stest(
		{
			description:
				'Generates a conventional commit message for a bug fix',
			language: 'python',
		},
		async (testingServiceCollection) => {
			const content = `
...
\ No newline at end of file`;
			...
			const message = await generator.generateGitCommitMessage(
				...
				'main',
				changes,
				{ repository: repoCommits, user: userCommits },
				0,
				CancellationToken.None,
			);
			...
			assert.ok(
				!userCommits.some((commit) =>
					message.toLowerCase().includes(commit),
				),
				'Commit message contains a user commit',
			);
			assert.ok(
				!repoCommits.some((commit) =>
					message.toLowerCase().includes(commit),
				),
				'Commit message contains a repo commit',
			);
			assert.ok(
				['fix:', 'chore:', 'feat:', 'refactor:'].some((prefix) =>
			...
			assert.ok(
				...
				'Commit message contains the email address',
			);
			assert.ok(
				!/#\d+/.test(message),
				'Commit message does include an issue reference',
			);
		},
	);
	...
	stest(
		{
			description:
				'Generated commit messages do not bias to conventional commit style',
			language: 'python',
		},
		async (testingServiceCollection) => {
			const content = `
...
\ No newline at end of file`;
			...
			assert.ok(
				![
					...
					'style:',
					'refactor:',
				].some((prefix) => message.toLowerCase().startsWith(prefix)),
				'Commit message should not use conventional commits format',
			);
		},
	);
});
