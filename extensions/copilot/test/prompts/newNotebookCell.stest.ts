...
ssuite({ title: 'newNotebook', subtitle: 'prompt', location: 'panel' }, () => {
	stest(
		{ description: 'generate code cell', language: 'python' },
		async (testingServiceCollection) => {
			const accessor = testingServiceCollection.createTestingAccessor();
			const endpoint = await accessor
			...
			const cells: string[] = [];
			const firstCell = await newNotebookCodeCell(
				accessor.get(IInstantiationService),
				accessor.get(IChatMLFetcher),
				endpoint,
				...
				'',
				'python',
				URI.file('sample.ipynb'),
				CancellationToken.None,
			);
			assert.ok(firstCell !== undefined, 'code should not be empty');
			assert.ok(
				firstCell.includes('import numpy as np'),
				'code should include import numpy as np',
			);

			const firstCellIsValid = await validatePythonCode(
				accessor,
				firstCell!,
			);
			assert.ok(
				firstCellIsValid,
				'first cell code should be valid python code',
			);
			cells.push(firstCell!);

			const secondCell = await newNotebookCodeCell(
				accessor.get(IInstantiationService),
				accessor.get(IChatMLFetcher),
				endpoint,
				fetchConversationOptions(),
			...
			assert.ok(
				!secondCell.includes('import numpy'),
				'code should not include import numpy again',
			);
	...
	stest(
		{ description: 'Generate code cell (numpy)', language: 'python' },
		async (testingServiceCollection) => {
			const accessor = testingServiceCollection.createTestingAccessor();
			const endpoint = await accessor
				.get(IEndpointProvider)
				.getChatEndpoint('copilot-utility');
			const topic =
				'A Jupyter notebook that creates a structured array with NumPy.';
			const sections: INotebookSection[] = [
				{
					title: 'Import Required Libraries',
					content: 'Import the necessary libraries, including NumPy.',
				...
				{
					title: 'Create Structured Array',
					content:
						'Use NumPy to create a structured array with named fields and data types.',
				},
				{
					title: 'Accessing Structured Array Elements',
					content:
						'Access individual elements of the structured array using the field names.',
				},
				{
					title: 'Iterating over Structured Arrays',
					content:
						'Iterate over the structured array using a fo…nd access the field values for each element.',
				},
			];
			...
			for (let i = 1; i < sections.length; i++) {
				const cell = await newNotebookCodeCell(
					accessor.get(IInstantiationService),
					accessor.get(IChatMLFetcher),
					endpoint,
					fetchConversationOptions(),
	...
	stest(
		{
			description: 'Generate code cell (seaborn + pandas)',
			language: 'python',
		},
		async (testingServiceCollection) => {
			const accessor = testingServiceCollection.createTestingAccessor();
			const endpoint = await accessor
				...
				.getChatEndpoint('copilot-utility');
			const topic =
				'A Jupyter notebook that loads planets data from Seaborn and performs aggregation in Pandas.';
			const sections: INotebookSection[] = [
				{
					title: 'Import Required Libraries',
					content:
						'Import the necessary libraries, including Pandas and Seaborn.',
				},

				{
					title: 'Load Planets Data',
					content:
						'Load the planets data from Seaborn into a Pandas DataFrame.',
				},

				{
					title: 'Data Cleaning',
					content:
						'Clean the data by removing missing values and converting data types if necessary.',
				},

				{
					title: 'Aggregation with GroupBy',
					content:
						'Use the groupby() function to group the data…erform aggregation operations on the groups.',
				},

				{
					title: 'Aggregation with Pivot Tables',
					content:
						'Use the pivot_table() function to create a p… summarizes the data by one or more columns.',
				},
			];
			...
			assert.ok(firstCell !== undefined, 'code should not be empty');
			assert.ok(
				firstCell.includes('import seaborn'),
				'code should include import seaborn',
			);
			assert.ok(
				firstCell.includes('import pandas'),
				'code should include import pandas',
			);
			...
			for (let i = 1; i < sections.length; i++) {
				...
				assert.ok(cell !== undefined, 'code should not be empty');
				assert.ok(
					!cell.includes('import seaborn'),
					'code should not include import seaborn again',
				);
				assert.ok(
					!cell.includes('import pandas'),
					'code should not include import pandas again',
				);
				const cellIsValid = await validatePythonCode(accessor, cell!);
				assert.ok(cellIsValid, 'code should be valid python code');
				cells.push(cell!);
