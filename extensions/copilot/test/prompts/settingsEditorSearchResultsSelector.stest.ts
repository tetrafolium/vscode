...
ssuite(
	{ title: 'settingsEditorSearchResultsSelector', location: 'external' },
	() => {
		stest.skip(
			{ description: 'Selects expected command center setting' },
			async (testingServiceCollection) => {
				...
				const selector = instantiationService.createInstance(
				...
				const results = await selector.selectTopSearchResults(
					endpoint,
					'Hide search bar at top of window',
					settingsList,
					CancellationToken.None,
				);
				assert.ok(results.length > 0, 'No settings were selected');
				assert.ok(
					results.some((result) => result === 'window.commandCenter'),
					'Expected setting "window.commandCenter" was not found',
				);
			},
		);

		stest(
			{ description: 'Selects expected agent mode setting' },
			async (testingServiceCollection) => {
				const accessor =
					testingServiceCollection.createTestingAccessor();
				const instantiationService = accessor.get(
				...
				assert.ok(results.length > 0, 'No settings were selected');
				assert.ok(
					results.some((result) => result === 'chat.agent.enabled'),
					'Expected setting "chat.agent.enabled" was not found',
				);
			},
		);
	},
