...
} from '../../src/platform/devcontainer/common/devContainerConfigurationService';
	...
	return (
		...
		(index = (async () => {
			const indexPath = path.join(
				__dirname,
				'../test/prompts/fixtures/devcontainer/devContainerIndex.json',
			); // Cached copy of https://containers.dev/static/devcontainer-index.json
			...
			const templates = index.collections
				.filter(
					(c: any) =>
						c.sourceInformation.repository ===
...
ssuite({ title: 'Dev Container Configuration', location: 'external' }, () => {
	const dataPath = path.join(
		__dirname,
		'../test/prompts/fixtures/devcontainer/devContainerConfigTestData.json',
	);
	...
	for (let i = 0; i < data.length; i++) {
		...
		stest(
			{
				description: `Suggests a devcontainer.json template (sample ${i})`,
			},
		...
		stest(
			{
				description: `Suggests devcontainer.json features (sample ${i})`,
			},
			async (testingServiceCollection) => {
				const accessor =
					testingServiceCollection.createTestingAccessor();
