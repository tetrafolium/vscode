...
import {
	ProxiedSimulationEndpointHealth,
	SimulationEndpointHealthImpl,
} from './base/simulationEndpointHealth';
import {
	BASELINE_RUN_COUNT,
	SimulationOptions,
} from './base/simulationOptions';
import {
	ProxiedSimulationOutcome,
	SimulationOutcomeImpl,
} from './base/simulationOutcome';
import { drainStdoutAndExit } from './base/stdout';
import {
	SimulationSuite,
	SimulationTest,
	SimulationTestsRegistry,
	createSimulationTestFilter,
} from './base/stest';
import {
	CollectingJSONOutputPrinter,
	ConsoleJSONOutputPrinter,
	IJSONOutputPrinter,
	ProxiedSONOutputPrinter,
} from './jsonOutputPrinter';
...
import {
	ITestDiscoveryOptions,
	discoverTests,
} from './simulation/externalScenarios';
...
async function run(opts: SimulationOptions): Promise<RunResult> {
	const jsonOutputPrinter: IJSONOutputPrinter = opts.jsonOutput
		? new ConsoleJSONOutputPrinter()
		: new CollectingJSONOutputPrinter();

	if (opts.externalCacheLayersPath) {
		process.env['EXTERNAL_CACHE_LAYERS_PATH'] =
			opts.externalCacheLayersPath;
	}
...
async function runInExtensionHost() {
	...
	if (nodeOptions) {
		...
		const parsed = minimist(
			Array.from(nodeOptions.matchAll(regex), (match) => {
				let arg = match[0];
		...
		if (parsed.require) {
			const reqPaths = Array.isArray(parsed.require)
				? parsed.require
				: [parsed.require];
	...
	rpc.registerMethod(
		'runTest',
		async (params: IRunTestParams): Promise<IRunTestResult> => {
			const { simulationTestContext, tests } = await allTests;

			simulationTestContext.baseline.clear();
			simulationTestContext.simulationEndpointHealth.failures.splice(
				0,
				simulationTestContext.simulationEndpointHealth.failures.length,
			);
	...
	const allTests = rpc
		.callMethod('init', { folder: folder.uri.fsPath } satisfies IInitParams)
		.then(async (res: IInitResult) => {
			const opts = SimulationOptions.fromArray(res.argv);
			const { testsToRun } = await loadTests(opts);
			const { simulationTestContext } = await prepareTestEnvironment(
				opts,
				new ProxiedSONOutputPrinter(rpc),
				rpc,
			);
			return {
				opts,
				tests: new Map(testsToRun.map((t) => [t.fullName, t])),
				simulationTestContext,
			};
		});
...
async function prepareTestEnvironment(
	...
	if (opts.parallelism > 1) {
		...
		ParserWithCaching.CACHE_SIZE_PER_LANGUAGE = Math.max(
			5,
			2 * opts.parallelism,
		);
	...
	const { allSuites, testsToRun, externalScenariosPath } =
	...
	if (!rpcInExtensionHost) {
		// don't clean if we're just one participant in a larger run
	...
	if (!opts.jsonOutput) {
		if (hasFilteredTests) {
			console.log(
				`Due to grep filters, will execute ${testsToRun.length} out of ${allTests.length} simulations. Each simulation runs ${opts.nRuns} time(s).\n`,
			);
		} else {
			console.log(
				`Will execute ${testsToRun.length} simulations. Each simulation runs ${opts.nRuns} time(s).\n`,
			);
		}
	}
	...
	const runningAllTests =
		opts.grep === undefined && opts.omitGrep === undefined;

	const baselinePath = opts.externalBaseline
		? (assert(
				opts.externalScenarios !== undefined,
				'externalBaseline must be set only with externalScenarios',
			),
			path.join(opts.externalScenarios, 'baseline.json'))
		: SimulationBaseline.DEFAULT_BASELINE_PATH;

	const baseline = await SimulationBaseline.readFromDisk(
		baselinePath,
		runningAllTests,
	);

	if (canUseBaseline) {
		// copy current baseline as the baseline before the run
		await fs.promises.copyFile(
			baseline.baselinePath,
			path.join(outputPath, OLD_BASELINE_FILENAME),
		);
	}

	const configs = opts.configFile
...
async function runTests(
	...
	const {
		...
		runningAllTests,
		hasFilteredTests,
	} = await prepareTestEnvironment(opts, jsonOutputPrinter);

	if (opts.gc) {
		if (opts.gc && opts.externalCacheLayersPath) {
			throw new Error(
				'--gc is currently not compatible with --external-cache-layers-path',
			);
		}
	...
	const { testResultsPromises, getGroupedScores } = await executeTests(
		simulationTestContext,
		testsToRun,
	);
	...
	for (const result of testResults) {
		for (const [idx, o] of result.outcomes.entries()) {
			if (o?.kind === 'failed' && o.critical) {
				errors.push(
					`Test failed: ${result.test}, run ${idx}\n` + o.error,
				);
			}
		}
	}
	...
	if (opts.stageCacheEntries && !opts.externalScenarios) {
		// TODO@joaomoreno
		console.warn(
			'!!! Determining new cache entries is not yet working in Redis, ask Joao to implement it',
		);
	}
	...
	if (canUseBaseline) {
		await baseline.writeToDisk(
			path.join(outputPath, PRODUCED_BASELINE_FILENAME),
		);
	}
	...
	if (opts.isUpdateBaseline) {
		if (canUseBaseline) {
			...
			errors.push(
				`Cannot update baseline for ${opts.nRuns} run(s). Please use --n=${BASELINE_RUN_COUNT}.`,
			);
		}
	}
	...
	if (opts.label) {
		...
		await fs.promises.writeFile(
			runMetadata,
			JSON.stringify({ label: opts.label }, null, '\t'),
		);
	}
	...
	if (opts.ci && !opts.isUpdateBaseline) {
		...
		if (error) {
			errors.push(
				red(
					`${error.errorMessage}. Please run 'npm run simulate-update-baseline' and check in baseline.json.`,
				),
			);
		}
	} else {
		if (simulationEndpointHealth.failures.length > 0) {
			const rateLimitedCount = simulationEndpointHealth.failures.filter(
				(f) => f.request.type === 'rateLimited',
			).length;
			const failedCount = simulationEndpointHealth.failures.filter(
				(f) => f.request.type === 'failed',
			).length;
			...
			if (opts.ci && opts.isUpdateBaseline) {
				errors.push(
					red(
						`Encountered server failures while running simulation: ${rateLimitedCount} rate limited responses, ${failedCount} other failed responses. Please rerun the simulation!`,
					),
					...simulationEndpointHealth.failures.map(
						(f) => `- ${f.testInfo.testName}: ${f.request.reason}`,
					),
				);
			}
		}
...
async function loadTests(opts: SimulationOptions) {
	...
	if (externalScenariosPath) {
		...
		if ([opts.inline, opts.sidebar, opts.nes].filter(Boolean).length > 1) {
			usageError = true;
			console.error(
				`Can only have one of --inline or --sidebar or --nes flags set`,
			);
		}
		...
		if (usageError) {
			// process.exit() if there's a usage error
			console.error(
				`Usage: npm run simulate -- --external-scenarios=<path> --inline --output=<path>`,
			);
			console.error(
				`Usage: npm run simulate -- --external-scenarios=<path> --sidebar --output=<path>`,
			);
		...
		if (!path.isAbsolute(externalScenariosPath)) {
			externalScenariosPath = path.join(
				process.cwd(),
				externalScenariosPath,
			);
		}
		...
		if (opts.scenarioTest) {
			...
			if (opts.nes) {
				if (opts.nes === 'external') {
					// run external stests
					allSuites = [
						await discoverNesTests(externalScenariosPath, opts),
					];
				} else {
					// run coffe stests
					allSuites = [
						await discoverCoffeTests(externalScenariosPath, opts),
					];
				}
			} else {
	...
	if (testsToRun.length === 0) {
		SimulationTestsRegistry.setFilters(
			opts.scenarioTest,
			opts.grep,
			opts.omitGrep,
		);
...
function listSuites(
	allSuites: readonly SimulationSuite[],
	opts: SimulationOptions,
	jsonOutputPrinter: IJSONOutputPrinter,
) {
	for (const suite of allSuites) {
		jsonOutputPrinter.print({
			...
			name: suite.fullName,
			location: suite.options.location,
		});
	}
}
...
function listTests(
	...
	for (const test of allTests) {
		jsonOutputPrinter.print({
			...
			suiteName: test.suite.fullName,
			name: test.fullName,
			location: test.options.location,
		});
		if (!opts.jsonOutput) {
...
async function listChatModels(skipCache: boolean = false) {
	const accessor = createExtensionUnitTestingServices(undefined, undefined, {
		skipModelMetadataCache: skipCache,
	...
	const sortedFamilies = Array.from(modelsByFamily.entries()).sort((a, b) =>
...
function createSimulationTestContext(
	...
	const simulationEndpointHealth = rpcInExtensionHost
		? new ProxiedSimulationEndpointHealth(rpcInExtensionHost)
		: new SimulationEndpointHealthImpl();

	let createChatMLCache:
		| ((info: CurrentTestRunInfo) => IChatMLCache)
		| undefined;
	let createNesFetchCache:
		| ((info: CurrentTestRunInfo) => ICompletionsCache)
		| undefined;
	...
	if (opts.lmCacheMode === CacheMode.Disable) {
		...
		createNesFetchCache = (info: CurrentTestRunInfo) =>
			new CompletionsSQLiteCache(
				TestingCacheSalts.nesFetchCacheSalt,
				info,
			);
	}
	...
	const simulationServicesOptions: SimulationServicesOptions = {
		...
		chatModelThrottlingTaskLaunchers:
			createSimulationChatModelThrottlingTaskLaunchers(opts.boost),
	...
	const simulationOutcome = rpcInExtensionHost
		? new ProxiedSimulationOutcome(rpcInExtensionHost)
		: new SimulationOutcomeImpl(runningAllTests);
	...
	const simulationTestContext: SimulationTestContext = {
		...
		), // this is expensive so we share it across all stests
	};
	return {
		simulationTestContext,
		simulationEndpointHealth,
...
function printOutcome(
	...
	if (shouldShowSummaries) {
		...
		for (const [suiteName, scoresPerSuite] of groupedScores.entries()) {
			...
			for (const [
				_language,
				scoresPerLanguage,
			] of scoresPerSuite.entries()) {
				for (const [
					model,
					scoresPerModel,
				] of scoresPerLanguage.entries()) {
					if (!model) {
						continue;
					}
					const data = testScores.get(model) || {
						count: 0,
						scoreSum: 0,
					};
					data.count += scoresPerModel.length;
					data.scoreSum += scoresPerModel.reduce(
						(acc, curr) => acc + curr,
						0,
					);
			...
			if (modelCount > 1) {
				const gpt4o = testScores.get(CHAT_MODEL.GPT41) ?? {
					count: 0,
					scoreSum: 0,
				};
				const gpt4oMini = testScores.get(CHAT_MODEL.GPT4OMINI) ?? {
					count: 0,
					scoreSum: 0,
				};
				const row = {
					Suite: suiteName,
					'# of tests':
						gpt4o.count === 0 || gpt4oMini.count === 0
							? gpt4o.count || gpt4oMini.count
							: `${gpt4o.count} <> ${gpt4oMini.count}`,
					'GPT-4o': gpt4o.count
						? Number((gpt4o.scoreSum / gpt4o.count) * 100).toFixed(
								2,
							)
						: '-',
					'GPT-4o-mini': gpt4oMini.count
						? Number(
								(gpt4oMini.scoreSum / gpt4oMini.count) * 100,
							).toFixed(2)
						: '-',
				};
	...
	if (canUseBaseline) {
		...
		if (changeStats.nImproved > 0) {
			console.log(
				`${green('▲')} - Score improved in ${changeStats.nImproved} scenarios`,
			);
		}
		if (changeStats.nWorsened > 0) {
			console.log(
				`${red('▼')} - Score decreased in ${changeStats.nWorsened} scenarios`,
			);
		}
	} else {
		if (!shouldBeBrief) {
			...
			console.log(
				`Overall Approximate Score: ${((score / testsToRun.length) * 100).toFixed(2)} / 100`,
			);
		}
		if (changeStats.nImproved > 0) {
			console.log(
				`${green('▲')} - Score clearly improved in ${changeStats.nImproved} scenarios`,
			);
		}
		if (changeStats.nWorsened > 0) {
			console.log(
				`${red('▼')} - Score clearly decreased in ${changeStats.nWorsened} scenarios`,
			);
		}
	}
	if (changeStats.nUnchanged > 0) {
		console.log(
			`= - Score unchanged in ${changeStats.nUnchanged} scenarios`,
		);
	}
	if (changeStats.addedScenarios > 0) {
		console.log(
			`${violet('◆')} - New scenarios count - ${changeStats.addedScenarios}`,
		);
	}
	if (changeStats.removedScenarios > 0) {
		console.log(
			`${orange('●')} - Missing ${changeStats.removedScenarios} scenarios.`,
		);
	}
	if (changeStats.skippedScenarios > 0) {
		console.log(
			`${yellow('●')} - Skipped ${changeStats.skippedScenarios} scenarios.`,
		);
	}

	if (
		changeStats.improvedScenarios.length > 0 ||
		changeStats.worsenedScenarios.length > 0
	) {
		console.log();
	}
		...
		for (const [language, scoresPerLanguage] of scoresPerSuite.entries()) {
			for (const [model, scoresPerModel] of scoresPerLanguage.entries()) {
				const row = {
					...
					'Score(%)': Number(
						(
							(scoresPerModel.reduce(
								(acc, curr) => acc + curr,
								0,
							) /
								scoresPerModel.length) *
							100
						).toFixed(2),
					),
				};
...
function validateChangeStats(
	changeStats: ICompleteBaselineComparison,
...
function writeHeapSnapshot(
	...
	const fileName =
		typeof snapshotFilename === 'string'
			? `${snapshotFilename}-${label}.heapsnapshot`
			: undefined;
...
function parseModelConfigFile(modelConfigFilePath: string): IModelConfig[] {
	const resolvedModelConfigFilePath = path.isAbsolute(modelConfigFilePath)
		? modelConfigFilePath
		: path.join(process.cwd(), modelConfigFilePath);
	const configFileContents = fs.readFileSync(
		resolvedModelConfigFilePath,
		'utf-8',
	);
	...
	try {
		...
		throw new Error(
			`Invalid JSON configuration file ${resolvedModelConfigFilePath}: ${error.message}`,
		);
	}

	if (!modelConfig || typeof modelConfig !== 'object') {
		throw new Error(
			'Invalid configuration file ' + resolvedModelConfigFilePath,
		);
	}
	...
	*/
	...
	const checkProperty = (
		...
		if (!(prop in obj)) {
			...
			throw new Error(
				`Missing property '${prop}' in model configuration file ${resolvedModelConfigFilePath}`,
			);
		}
		...
		if (typeof obj[prop] !== type) {
			throw new Error(
				`Property '${prop}' in model configuration file ${resolvedModelConfigFilePath} must be of type '${type}', but got '${typeof obj[prop]}'`,
			);
		}
	};
	...
	for (const modelId in modelConfig) {
		...
		if (typeof model !== 'object') {
			throw new Error(
				`Model configuration for '${modelId}' must be an object`,
			);
		}
		...
		if (model.type !== 'openai' && model.type !== 'azureOpenai') {
			throw new Error(
				`Model type '${model.type}' is not supported. Only 'openai' and 'azureOpenai' are allowed.`,
			);
		}
		...
		if (model.supported_endpoints) {
			if (!Array.isArray(model.supported_endpoints)) {
				throw new Error(
					`Property 'supported_endpoints' in model configuration file ${resolvedModelConfigFilePath} must be an array`,
				);
			}
			for (const endpointSuffix of model.supported_endpoints) {
				if (
					!Object.values(ModelSupportedEndpoint).includes(
						endpointSuffix as ModelSupportedEndpoint,
					)
				) {
					throw new Error(
						`Invalid endpoint suffix '${endpointSuffix}' in supported_endpoints for model '${modelId}'. Must be one of: ${Object.values(ModelSupportedEndpoint).join(', ')}`,
					);
				}
			}
		}
		...
		modelConfigs.push({
			...
			capabilities: {
				supports: {
					...
					prediction:
						model.capabilities?.supports?.prediction ?? false,
					thinking: model.capabilities?.supports?.thinking ?? false,
				},
				limits: {
					max_prompt_tokens:
						model.capabilities?.limits?.max_prompt_tokens ?? 128000,
					max_output_tokens:
			...
			supported_endpoints: model.supported_endpoints?.length
				? (model.supported_endpoints as ModelSupportedEndpoint[])
			...
			overrides: {
				requestHeaders: model.overrides?.hasOwnProperty(
					'requestHeaders',
				)
					? model.overrides.requestHeaders
					: {},
				temperature: model.overrides?.hasOwnProperty('temperature')
					? model.overrides.temperature
					: undefined,
				top_p: model.overrides?.hasOwnProperty('top_p')
					? model.overrides.top_p
					: undefined,
				snippy: model.overrides?.hasOwnProperty('snippy')
					? model.overrides.snippy
					: undefined,
				intent: model.overrides?.hasOwnProperty('intent')
					? model.overrides.intent
					: undefined,
				max_tokens: model.overrides?.hasOwnProperty('max_tokens')
					? model.overrides.max_tokens
					: undefined,
				max_completion_tokens: model.overrides?.hasOwnProperty(
					'max_completion_tokens',
				)
					? model.overrides.max_completion_tokens
					: undefined,
			},
		});
	}
