...
export class Extension extends vscode.Disposable {
	...
	constructor(_context: vscode.ExtensionContext) {
		...
		this._output = vscode.window.createOutputChannel("VS Code Extras", {
			log: true,
		});
		...
		disposables.push(
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration("vscode-extras.npmUpToDateFeature.enabled")
				) {
					this._updateNpmFeature();
				}
			}),
	...
	private _updateNpmFeature(): void {
		const enabled = vscode.workspace
			.getConfiguration("vscode-extras")
			.get<boolean>("npmUpToDateFeature.enabled", true);
