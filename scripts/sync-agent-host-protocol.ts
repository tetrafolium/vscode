...
const FORMAT_OPTIONS: ts.FormatCodeSettings = JSON.parse(
	fs.readFileSync(TSFMT_PATH, "utf-8"),
);
...
function formatTypeScript(content: string, fileName: string): string {
	...
	for (let i = edits.length - 1; i >= 0; i--) {
		const edit = edits[i];
		content =
			content.substring(0, edit.span.start) +
			edit.newText +
			content.substring(edit.span.start + edit.span.length);
	}
	ls.dispose();
	return content;
}
...
function mergeDuplicateImports(content: string): string {
	...
	content = content.replace(
		...
		(_match, typeKeyword, names, mod) => {
			const collapsed = names.replace(/\s+/g, " ").trim();
			return typeKeyword
				? `import type { ${collapsed} } from '${mod}';`
				: `import { ${collapsed} } from '${mod}';`;
		},
	);
	...
	for (const line of content.split("\n")) {
		...
		if (typeMatch) {
			...
			importsByModule.get(mod)!.typeNames.push(
				...names
					.split(",")
					.map((s) => s.trim())
					.filter((s) => s.length > 0),
			...
			importsByModule.get(mod)!.valueNames.push(
				...names
					.split(",")
					.map((s) => s.trim())
					.filter((s) => s.length > 0),
...
function main() {
	if (!fs.existsSync(TYPES_DIR)) {
		...
		console.error(
			"Clone agent-host-protocol as a sibling of the VS Code repo:",
		);
		console.error(
			"  git clone git@github.com:microsoft/agent-host-protocol.git ../agent-host-protocol",
		);
		process.exit(1);
	}
