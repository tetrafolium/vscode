...
function checkFunctionForAwaitBeforeReader(
	...
	function collectPositions(node: TSESTree.Node) {
		...
		if (node.type === "AwaitExpression") {
			...
			isReaderMethodCall(node, readerName)
		) {
		...
		switch (node.type) {
			...
			case "IfStatement":
				...
				if (node.alternate) {
					collectPositions(node.alternate);
				}
				break;
			case "TryStatement":
				collectPositions(node.block);
				if (node.handler) {
					collectPositions(node.handler.body);
				}
				if (node.finalizer) {
					collectPositions(node.finalizer);
				}
				break;
			...
			case "MemberExpression":
				...
				if (node.computed) {
					collectPositions(node.property);
				}
				break;
...
function getReaderParameterName(
	fn: TSESTree.ArrowFunctionExpression | TSESTree.FunctionExpression,
): string | null {
