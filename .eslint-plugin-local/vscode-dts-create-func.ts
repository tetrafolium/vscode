...
export default new (class ApiLiteralOrTypes implements eslint.Rule.RuleModule {
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		return {
			["TSDeclareFunction Identifier[name=/create.*/]"]: (
				node: ESTree.Node,
			) => {
				const decl = (node as TSESTree.Identifier)
					.parent as TSESTree.FunctionDeclaration;

				if (
					decl.returnType?.typeAnnotation.type !==
					AST_NODE_TYPES.TSTypeReference
				) {
					return;
				}
				if (
					decl.returnType.typeAnnotation.typeName.type !==
