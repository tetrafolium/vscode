...
type PropertyKeyNode =
	| TSESTree.Property["key"]
	| TSESTree.MemberExpression["property"];
type AssignmentTarget = TSESTree.AssignmentExpression["left"];

export default new (class NoLocalizedModelDescriptionRule
	implements eslint.Rule.RuleModule
{
	meta: eslint.Rule.RuleMetaData = {
		messages: {
			[MESSAGE_ID]:
				"modelDescription values describe behavior to the language model and must not use localized strings.",
		},
		type: "problem",
		schema: false,
	};
...
function isModelDescriptionKey(
	key: PropertyKeyNode,
...
function isModelDescriptionAssignmentTarget(
	target: AssignmentTarget,
): target is TSESTree.MemberExpression {
...
function isLocalizeCallee(callee: TSESTree.CallExpression["callee"]): boolean {
	...
	if (callee.type === "MemberExpression") {
		...
		if (
			...
			callee.property.value === "localize"
		) {
			return true;
		}
	}
