...
export default new (class NoInOperator implements eslint.Rule.RuleModule {
	readonly meta: eslint.Rule.RuleMetaData = {
		messages: {
			noInOperator:
				'The "in" operator should not be used. Use type discriminator properties and classes instead or the `hasKey`-utility.',
		},
		schema: false,
	};
		...
		function checkInOperator(inNode: ESTree.BinaryExpression) {
			...
			for (const ancestor of ancestors) {
				if (
					...
					if (
						(ancestor as { returnType?: any }).returnType?.typeAnnotation
