...
export default new (class implements eslint.Rule.RuleModule {
	readonly meta: eslint.Rule.RuleMetaData = {
		messages: {
			notAllowed:
				"Importing '{{module}}' is only allowed as a type import (`import type ...`) to prevent startup performance regressions as these modules are slow to load. Use dynamic `import('{{module}}')` for runtime access.",
		},
		schema: {
			type: "array",
			items: {
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		const targets = (context.options as { target: string }[]).map(
			(o) => o.target,
		);
		if (targets.length > 0) {
			const relativeFilename = normalize(context.getFilename())
				.substring(REPO_ROOT.length + 1)
				.replace(/\\/g, "/");
			const matched = targets.some((pattern) =>
				minimatch(relativeFilename, pattern),
			);
			if (!matched) {
		...
		return createImportRuleListener((node, path) => {
			...
			if (
				parent.type === TSESTree.AST_NODE_TYPES.ImportDeclaration &&
