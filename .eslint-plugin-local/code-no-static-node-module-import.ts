...
export default new (class implements eslint.Rule.RuleModule {
	readonly meta: eslint.Rule.RuleMetaData = {
		messages: {
			staticImport:
				"Static imports of '{{module}}' are not allowed here because they are loaded synchronously on startup. Use a dynamic `await import(...)` or `import type` instead.",
		},
		docs: {
			description:
				"Disallow static imports of node_modules packages to prevent synchronous loading on startup. Allows Node.js built-ins, electron, relative imports, and whitelisted file paths.",
		},
		schema: {
			type: "array",
			items: {
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		...
		const filePath = normalize(
			relative(REPO_ROOT, normalize(context.getFilename())),
		).replace(/\\/g, "/");

		// Skip whitelisted files
		if (
			allowedPaths.some(
				(pattern) => filePath === pattern || minimatch(filePath, pattern),
			)
		) {
			return {};
		}
