...
function isStringLiteral(
	node: TSESTree.Node | ESTree.Node | null | undefined,
...
export default new (class NoUnexternalizedStrings
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		...
		function visitLocalizeCall(node: TSESTree.CallExpression) {
			...
			if (isStringLiteral(keyNode)) {
				...
				keyNode.type === AST_NODE_TYPES.TemplateLiteral &&
				keyNode.expressions.length === 0 &&
				keyNode.quasis.length === 1
			) {
			...
			if (!isStringLiteral(messageNode)) {
				context.report({
					...
					data: {
						message: context.getSourceCode().getText(node as ESTree.Node),
					},
				});
			}
		}
		...
		function reportBadStringsAndBadKeys() {
			...
			for (const node of doubleQuotedStringLiterals) {
				context.report({
					...
					fix: enableDoubleToSingleQuoteFixes
						? (fixer) => {
								// Get the raw string content, unescaping any escaped quotes
								const content = (node as ESTree.SimpleLiteral)
									.raw!.slice(1, -1)
									.replace(/(?<!\\)\\'/g, `'`)
									.replace(/(?<!\\)\\"/g, `"`);
				...
				if (values.length > 1) {
					for (let i = 1; i < values.length; i++) {
						if (
							context
								.getSourceCode()
								.getText(values[i - 1].message as ESTree.Node) !==
							context.getSourceCode().getText(values[i].message as ESTree.Node)
						) {
							context.report({
		...
		return {
			...
			["ExpressionStatement[directive] Literal:exit"]: (
			...
			['CallExpression[callee.type="MemberExpression"][callee.object.name="nls"][callee.property.name="localize"]:exit']:
				(node: TSESTree.CallExpression) => visitLocalizeCall(node),
			...
			['CallExpression[callee.type="MemberExpression"][callee.object.property.name="l10n"][callee.property.name="t"]:exit']:
				(node: TSESTree.CallExpression) => visitL10NCall(node),
